...
TL;DR: The paper proposes DocArena, a fully automated data curation pipeline for multimodal document search and question-answering. It generates training environments for search agents without human annotation, improving performance on retrieval accuracy and QA quality.


<details>
  <summary>Details</summary>
Motivation: Existing training environments for search agents are predominantly text-based, struggle to construct controllable, scalable environments, and do not account for multimodal data. This limits the development of search strategies and generalization abilities.

Method: The proposed DocArena pipeline structures and indexes documents through MLLM-based visual perception, profiles and leverages cross-page information distribution to construct QA pairs, and performs cascaded quality assurance operations via MLLM.

Result: Agents trained on DocArena data achieve the best performance on both retrieval accuracy and QA quality across six multimodal document scenarios and seven text-based QA benchmarks.

Conclusion: The proposed DocArena pipeline effectively generates high-quality training environments for search agents, leading to improved performance and controllability in multimodal document search and question-answering tasks.
...
TL;DR: A hybrid approach combining image processing and deep learning to assess fruit freshness, achieving over 90% accuracy in real-time with low computational resources.


<details>
  <summary>Details</summary>
Motivation: Fruit spoilage leads to significant economic losses in agriculture, and there is a need for a practical solution to assess fruit freshness.

Method: A hybrid approach combining image processing algorithm, convolutional neural network (CNN), and logistic regression to assess fruit freshness.

Result: The approach achieved over 90% accuracy on a dataset comprising apples and oranges, and can operate in real-time with low computational resources.

Conclusion: The study highlights the potential of integrating simple image processing techniques with machine learning to provide practical solutions in the agricultural sector, with future improvements including advanced segmentation models.
...
TL;DR: Proposed Long-Context Generation (LCG) framework for consistent multi-image text-to-image generation, outperforming baselines in long-context image generation.


<details>
  <summary>Details</summary>
Motivation: Current image generation models struggle with consistency across sequential outputs, crucial for applications like comics and visual narratives.

Method: LCG uses Sparse Relational Attention (SRA) and Routing Consistency Constraint (RCC) to ensure semantic alignment and consistency across extended visual contexts.
...
Conclusion: The proposed LCG framework effectively improves consistency and scalability in long-context multi-image generation, with a large-scale dataset (LCCD) supporting training and evaluation.
...
TL;DR: A framework for estimating above-ground biomass (AGB) of urban trees at a crown level using LiDAR and orthophotography data, achieving high accuracy and producing a public bitemporal AGB database.


<details>
  <summary>Details</summary>
Motivation: Urban tree biomass is not well quantified due to reliance on inventories or coarse products that cannot resolve individual crowns or fine-scale heterogeneity.

Method: A dual-stream cross-attention network for semantic segmentation, multiscale watershed segmentation for crown delineation, and a crown area-height power-law proxy for AGB estimation.

Result: Total AGB stocks of 1.73 Tg in 2018 and 1.81 Tg in 2023, with a net carbon gain of 39 Gg C over five years, and high-epistemic-uncertainty areas identified.

Conclusion: The framework provides a management-relevant resolution for urban tree biomass estimation, using standard provincial data and no manual annotation, and can guide assignment of uncertain crowns to a pooled allometric equation.
...
TL;DR: A deep learning model is proposed to predict penetration state, depth, and weld seam morphology in laser penetration welding with high accuracy.


<details>
  <summary>Details</summary>
Motivation: The assessment of penetration state and weld seam morphology is crucial in determining weld quality in laser penetration welding.

Method: A multi-task deep learning model integrating spatiotemporal features from weld pool images and welding parameters using convolutional neural networks and state space models.

Result: The model achieves 99.35% accuracy in predicting penetration state, 1.79mm error in penetration depth, and 95.65% accuracy in reconstructing the weld cross-section.

Conclusion: The study provides new insights and methodologies for in-situ quality control strategies in laser penetration welding systems.
...
TL;DR: This study extends fMRI taskonomy from single-source to multi-source transfer across 23 task states, revealing paradigm-specific transfer patterns and prioritizing working-memory states.


<details>
  <summary>Details</summary>
Motivation: Existing reconstruction-based taskonomies mainly study one-to-one transfer, but this study aims to explore multi-source transfer and budget-constrained task allocation.

Method: The study uses Boolean Integer Programming (BIP) to analyze budget-constrained task allocation and trains 1,127 task-specific and transfer models.

Result: The study finds that single-source transfer is directional and paradigm-structured, while multi-source transfer depends on the composition of the source set. Working-memory states are prioritized under the specified global allocation objective.

Conclusion: The study reveals a cross-paradigm-limited motor cluster and working-memory states with high priority, extending reconstruction-based fMRI taskonomy from one-to-one transfer to many-to-one task relations and budget-constrained task dependencies.
...
TL;DR: The paper proposes a new dynamic routing method called GeMoE for Large Vision-Language Models (LVLMs) based on Mixture of Experts (MoE) architecture, which adaptively selects the number of experts for each token, achieving better resource utilization and performance.


<details>
  <summary>Details</summary>
Motivation: Traditional MoE methods use a static routing strategy, which fails to account for variations in the input and adaptively select the number of experts, resulting in suboptimal resource utilization.

Method: The proposed method, GeMoE, views token routing as an information encoding task and frames dynamic routing as a Minimum Description Length (MDL) problem. It uses gating entropy to assess the complexity of tokens and adaptively determines the number of experts each token should engage.

Result: GeMoE achieves 99.5% average performance retention compared to the original static routing, while improving average expert activation sparsity by 36.5% on a wide range of backbones and benchmarks.

Conclusion: The proposed GeMoE method provides a more efficient and adaptive routing approach for LVLMs, achieving better resource utilization and performance.
...
TL;DR: The authors introduce a new dataset and metric to evaluate computer vision models in underwater environments with turbidity, showing that their proposed metric correlates strongly with model performance.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the gap in understanding how computer vision models are affected by turbidity in underwater environments, which degrades visibility rapidly.

Method: The authors propose a new dataset (TUB) with 1,320 images and 16,000 segmentation masks, and a new metric (PCD) derived from phase congruency maps that is invariant to contrast.

Result: The authors show that PCD correlates strongly with the performance of instance segmentation models on both real and synthetic turbid images, outperforming common metrics in the field.

Conclusion: The authors conclude that their proposed dataset and metric can help improve the evaluation and development of computer vision models in underwater environments with turbidity.
...
TL;DR: The paper explores the impact of different fusion schemes for learnable prompts and image tokens in visual prompt tuning for Vision Transformers (ViTs).


<details>
  <summary>Details</summary>
Motivation: To investigate whether a single fusion scheme is optimal or if a hybrid approach could be more effective in visual prompt tuning.

Method: The authors formulate the task as a bi-level optimization problem and solve it using differentiable architecture search, jointly optimizing learnable prompts and their fusion schemes.

Result: The proposed method shows consistent gains over baselines on 34 datasets and offers a favorable accuracy-latency-parameter trade-off compared to VPT-Deep and recent variants.

Conclusion: The study highlights the significance of fusion schemes in visual prompt tuning and suggests that a hybrid fusion approach can more effectively leverage layer semantics of ViTs.
...
TL;DR: The paper audits deepfake detection benchmarks and finds that they often reward general understanding of media rather than specific forensic understanding of deepfakes.


<details>
  <summary>Details</summary>
Motivation: The motivation is to investigate the effectiveness of deepfake detection benchmarks and whether they accurately reflect real-world threats.

Method: The authors use a simple diagnostic involving linear probes on frozen, general-purpose self-supervised representations to approximate the performance of bespoke detectors.

Result: The results show that linear probes closely approach the performance of bespoke detectors across three modalities, suggesting that benchmarks may not reflect realistic threat models.

Conclusion: The paper concludes that deepfake detection benchmarks should be audited to ensure they are measuring forensic understanding rather than general modality understanding.
...
TL;DR: The paper proposes VIGIL, a framework to improve multimodal large language models by reducing hallucinations and improving visual grounding through reinforcement learning.


<details>
  <summary>Details</summary>
Motivation: Multimodal large language models (MLLMs) struggle with hallucinations that contradict visual inputs due to over-reliance on language priors. Existing alignment methods have optimization biases toward linguistic shortcuts.

Method: The authors propose Visual Information Gain In aLignment (VIGIL), a reinforcement-learning post-training framework that uses a geometric constraint to maximize mutual information between visual input and generated response.

Result: VIGIL outperforms recent alignment methods across hallucination and reasoning benchmarks without compromising text-only capabilities. It matches state-of-the-art performance with only 25% of preference data and shows emergent spatial grounding capabilities.

Conclusion: The proposed VIGIL framework effectively reduces hallucinations and improves visual grounding in multimodal large language models, achieving better performance with less data.
...
TL;DR: DinoLink, a token-centric compression framework, enables high-precision remote perception in bandwidth-constrained Vehicle-to-Everything (V2X) networks by reducing bitrate by 139x while maintaining competitive performance.


<details>
  <summary>Details</summary>
Motivation: Severe bandwidth constraints in V2X networks hinder high-precision remote perception.

Method: DinoLink employs a dual-sparsity architecture with a saliency-aware selector and Residual Vector Quantization (RVQ) module.

Result: DinoLink achieves 139x bitrate reduction and 32.8% mAP on the nuScenes dataset, with 34.5x acceleration in narrow-band environments.
...
TL;DR: A self-supervised framework for learning implicit 3D physical dynamics from video-derived signals, achieving physical plausibility and stability without explicit simulators or labels.


<details>
  <summary>Details</summary>
Motivation: Current generative video models lack a 3D geometric foundation, resulting in physical inconsistencies and failure to maintain object permanence.

Method: The framework uses a Video Joint-Embedding Predictive Architecture (V-JEPA) to unproject semantic features into a voxelized 3D grid, enabling Volumetric Feature Advection to learn an action-conditioned transition operator.

Result: The model demonstrates good long-term structural stability and physical plausibility on multiple benchmarks (CLEVERER, PhysInOne, PhysGaia) without access to physics engine internal states, labels, or surrogate models.

Conclusion: This work opens a scalable pathway toward general-purpose dynamic world models that internalize 3D invariants of the physical world solely through passive observation of monocular videos.
...
TL;DR: A multimodal deep learning model is proposed for efficient detection of methane plumes in earth observation imagery, outperforming state-of-the-art methods with a better accuracy-efficiency trade-off.


<details>
  <summary>Details</summary>
Motivation: The need for efficient detection of methane plumes to understand and mitigate global warming, specifically by accurately identifying and segmenting them in earth observation imagery.

Method: A multimodal deep learning model integrating a feature-guided methane enhancement (FGME) mechanism with transformer-based RGB representations at multiple semantic scales.

Result: The proposed method outperforms state-of-the-art methods on the MPDataset with improvements of +0.92 in MIoU, +0.87 in MPrecision, and +1.01 in Recall, at a substantially lower computational cost.

Conclusion: The proposed model highlights the potential of efficient multimodal fusion strategies for accurate and scalable methane plume segmentation in real-world remote sensing applications.
...
TL;DR: A new framework called APRTrack is proposed for RGB-Event tracking, which improves robustness against partial target missing and modal degradation.


<details>
  <summary>Details</summary>
Motivation: Existing RGB-Event tracking methods struggle with real-world signal corruptions, such as modal degradation and partial target missing, which can cause traditional multi-modal fusion to fail.

Method: The APRTrack framework uses a hierarchical perturbation and retrieval approach, including adversarial perturbation branches to simulate modal and spatial degradation, and a Footprint-guided Channel-calibrated Hopfield Retrieval (FCHR) module for reliable historical information compensation.

Result: The proposed APRTrack framework demonstrates effectiveness on several datasets (FE108, COESOT, VisEvent, and FELT) for RGB-Event visual object tracking.

Conclusion: The APRTrack framework offers improved robustness against partial target missing and modal degradation, making it suitable for real-world RGB-Event tracking applications.
...
TL;DR: The paper presents a Frame Forgetting Network (FFN) for Test Time Training (TTT) on long videos, addressing computational issues with existing approaches.


<details>
  <summary>Details</summary>
Motivation: Existing TTT approaches are computationally expensive for long videos, and the paper aims to reduce computation while retaining temporal context.

Method: The FFN operates on a limited number of frames within a sliding window and uses a surprise metric to adaptively adjust the window size during TTT.

Result: The FFN demonstrates empirical effectiveness on dense-segmentation, video classification tasks, generalization to depth-estimation, and multi-hour long videos.

Conclusion: The paper proposes a novel approach, FFN, for efficient TTT on long videos, achieving a balance between temporal context and computational tractability.
...
TL;DR: The paper introduces CRISP, a new evaluation method for visual spatial intelligence that distinguishes between language priors and genuine spatial reasoning, revealing disconnects in perception and reasoning in current models.


<details>
  <summary>Details</summary>
Motivation: Current evaluations conflate language priors with genuine spatial reasoning, making it difficult to assess true spatial intelligence.

Method: CRISP uses metric 3D Scene Graphs and an oracle intervention protocol to decouple latent reasoning capabilities from perceptual bottlenecks.

Result: Proprietary models have robust latent reasoning but struggle with inaccurate metric estimation, while open-source models lack multi-hop compositional reasoning.

Conclusion: CRISP offers a roadmap for multimodal alignment beyond end-to-end post-training by focusing on genuine perception, verification, and reasoning.
...
TL;DR: A new benchmark, PhyEditBench, is introduced to evaluate the physical understanding of image editing models, revealing limitations in current state-of-the-art methods and proposing a baseline, PhyWorld.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for instruction-based image editing lack comprehensive evaluation of physics-based reasoning, which is critical for handling real-world scenarios.

Method: PhyEditBench is established with a hierarchical taxonomy of 4 primary classes and 12 subclasses, comprising 238 real-world and 35 synthetic instances; a training-free baseline, PhyWorld, is proposed using test-time scaling and latent reduction.

Result: Current state-of-the-art editing methods show substantial limitations in physics-based reasoning; PhyWorld outperforms comparable models, suggesting video generation can serve as a reasoning mechanism for image editing.

Conclusion: PhyEditBench effectively assesses the physical understanding of editing models, and PhyWorld offers a solution to improve physics-based reasoning in image editing.
...
TL;DR: Proposed ForeAgent, an agentic framework for detecting AI-generated images with iterative self-evolution, achieving state-of-the-art performance on benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing deepfake detection methods are challenged by highly realistic AI-generated images and current approaches have limitations in sensitivity to fine-grained forensic artifacts and flexibility.

Method: ForeAgent uses a Perception-Verdict architecture with multi-view cues and a Multimodal Large Language Model (MLLM) as a verdict module. It also employs a Hindsight-Driven Self-Refining strategy for continual self-improvement.

Result: ForeAgent achieves 82.18% accuracy on the Chameleon benchmark and 93.3% mean accuracy on AIGCDetect-Benchmark across 16 generators, outperforming existing methods.

Conclusion: ForeAgent demonstrates state-of-the-art performance in detecting AI-generated images and produces more consistent and causally grounded reasoning compared to existing models.
...
TL;DR: A hybrid self-supervised method for non-rigid 3D shape matching is proposed, achieving state-of-the-art performance with efficiency.


<details>
  <summary>Details</summary>
Motivation: Non-rigid 3D shape matching is a fundamental task in computer vision and graphics, and existing methods have limitations in handling challenging scenarios.

Method: A hybrid self-supervised method based on a coarse-to-fine strategy with a dual-branch design, using Laplacian basis and elastic basis.

Result: State-of-the-art performance across various challenging scenarios, including non-isometric deformations and topological noise, with maintained computational efficiency.

Conclusion: The proposed approach is effective and efficient for non-rigid 3D shape matching, and contrastive energies promote feature discrimination.
...
TL;DR: SpaceRipple is a framework for semantic delivery and on-board processing in Earth observation satellite networks, enabling efficient and reliable Earth observation under limited resources.


<details>
  <summary>Details</summary>
Motivation: The limited inter-satellite and downlink resources in Earth observation satellite networks, and the need for mission-relevant semantic information instead of full raw-image downlink.

Method: SpaceRipple uses a collaborative pipeline for adaptive compression, metadata generation, restoration, and semantic inference, with a compression-aware MoE enhancement module for robustness.

Result: SpaceRipple achieves favorable reconstruction quality, improved semantic detection performance, and substantial bandwidth savings.

Conclusion: SpaceRipple demonstrates its potential for efficient and reliable Earth observation under constrained satellite-network resources.
...
TL;DR: A new benchmark called DiCoBench is introduced to evaluate the fine-grained perception capabilities of Multimodal Large Language Models (MLLMs) using high-resolution images.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for MLLMs have limitations in evaluating their ability to perceive implicit visual cues in high-resolution images, which is a crucial aspect of fine-grained perception.

Method: A comprehensive benchmark, DiCoBench, with 765 samples, 2 progressive tracks, and 8 distinct perception tasks, is designed to challenge MLLMs using high-resolution imagery (approaching 2K) in a multiple-choice question task format.

Result: Evaluation of 18 diverse MLLMs shows a significant performance gap compared to human accuracy (98.3%), with top-performing models struggling to capture micro-scale details.

Conclusion: DiCoBench will serve as a challenging testbed to drive future research in autonomous, high-resolution multi-image perception for MLLMs.
...
TL;DR: LogicIR, a logic gate network for image restoration, achieves strong performance with reduced computational cost.


<details>
  <summary>Details</summary>
Motivation: The growing computational demands of image restoration models necessitate efficient architectures for fast inference.

Method: LogicIR, a UNet-inspired architecture composed entirely of logic gates, with a differentiable bit decoding layer and index shuffling mechanism.

Result: LogicIR achieves strong performance across multiple image restoration benchmarks with significantly reduced computational cost.
...
TL;DR: The paper proposes TaskTok, a framework for task-driven image restoration that selectively restores task-relevant tokens, improving performance and efficiency.


<details>
  <summary>Details</summary>
Motivation: Traditional image restoration focuses on perceptual quality, but Task-Driven Image Restoration (TDIR) aims to maximize performance on downstream high-level vision tasks. Recent approaches suffer from computational inefficiency and potential semantic alteration.

Method: The proposed framework, TaskTok, uses a learnable token switch and a lightweight token refinement module to selectively restore only task-relevant tokens.

Result: Extensive experiments demonstrate that TaskTok significantly enhances task performance with high computational efficiency across image classification, semantic segmentation, and object detection.

Conclusion: The paper concludes that selectively refining task-relevant tokens is sufficient for task-driven objectives, and TaskTok achieves this with high efficiency and effectiveness.
...
TL;DR: The paper proposes a new method called Position Rebinding Cache Reuse (PRCR) for improving visual grounding in multimodal reasoning by reusing historical visual evidence without replaying visual tokens.


<details>
  <summary>Details</summary>
Motivation: Existing methods for visual grounding in multimodal reasoning rely on token replay, which can be inefficient. The authors aim to improve efficiency and accuracy by reusing historical visual evidence in a more effective way.

Method: The authors propose PRCR, a cache-level framework that stores raw visual key-value (KV) cache along with their original spatial coordinates, then reassigns position-compatible coordinates to select entries and rebinds their keys before injecting the reconstructed cache into the active decoder cache.

Result: Experiments across multiple multimodal reasoning benchmarks show that PRCR achieves replay-level or better performance, improving average accuracy by 5 percent and reducing visual-revisiting computation by up to tens of thousands of times.

Conclusion: The proposed PRCR method effectively reuses historical visual evidence while preserving textual positional continuity and relative visual structure, leading to improved accuracy and efficiency in multimodal reasoning tasks.
...
TL;DR: A new framework called FAROS is proposed to address the annotation granularity mismatch in multi-task learning for surgical scene understanding. FAROS uses flow-guided label interpolation to generate dense pseudo labels from sparse keyframe annotations, enabling balanced optimization across tasks.


<details>
  <summary>Details</summary>
Motivation: The annotation granularity mismatch between temporal workflow tasks and pixel-level spatial tasks in surgical scene understanding hinders effective multi-task learning. The former benefits from dense frame-level supervision, while the latter is sparsely annotated on selected keyframes due to high labeling costs.

Method: The FAROS framework combines zero-shot segmentation-based mask propagation with optical flow estimation to generate temporally consistent dense pseudo labels. It uses a unified Transformer-based multi-task framework to jointly learn surgical phase recognition, step recognition, anticipation, instrument segmentation, and action recognition.

Result: FAROS significantly improves cross-task representation learning and enhances holistic surgical scene understanding performance across spatio-temporal tasks on benchmarks such as GraSP, MISAW, and AutoLaparo.

Conclusion: The proposed FAROS framework effectively addresses the annotation granularity mismatch in multi-task learning for surgical scene understanding, enabling balanced optimization across tasks and improving performance.
...
TL;DR: FracEvent, a new event simulator, improves event-camera simulation by modeling pixel-level lifecycle with fractional-relaxation voltage dynamics, leading to better temporal structure and downstream transfer results.


<details>
  <summary>Details</summary>
Motivation: The difficulty in collecting real event data at scale due to specialized sensors, synchronization, and annotations, making event-camera simulation crucial for event-based vision tasks.

Method: FracEvent models pixel-level lifecycle with fractional-relaxation voltage dynamics, driving a compact stack of relaxation modes, combining responses into a voltage state, and emitting ON/OFF events by localizing threshold crossings.

Result: FracEvent improves temporal structure of generated events and achieves stronger downstream-transfer results than competing simulator baselines on image reconstruction and optical flow estimation across multiple datasets.

Conclusion: FracEvent shows practical value for event-camera simulation, offering improved performance in event-based vision tasks through its novel approach to modeling pixel-level lifecycle.
...
TL;DR: A new 3D/2D registration method called LayersReg is proposed, which uses a progressive, layer-by-layer approach to achieve high accuracy and real-time performance in surgical navigation.


<details>
  <summary>Details</summary>
Motivation: Traditional iterative optimization algorithms for 3D/2D registration are inefficient and prone to failure, while deep learning-based methods have limited generalization in complex scenarios.

Method: LayersReg uses a regression paradigm that endows the model with 3D anatomical awareness and searches for the correct pose in a progressive, layer-by-layer manner.

Result: LayersReg achieves high accuracy on X-ray/CT registration (0.68°, 1.41 mm) and slice localization (0.73°, 1.55 mm) tasks, outperforming existing state-of-the-art methods.

Conclusion: LayersReg offers a promising solution for 3D/2D registration in surgical navigation, meeting the demands for precision and real-time capability.
...
TL;DR: A new framework, Disco-LoRA, is proposed for multi-concept video customization, enabling control over content, style, and motion in text-to-video generation.


<details>
  <summary>Details</summary>
Motivation: The challenge of simultaneously controlling multiple concepts, such as content, style, and motion, in video customization based on Text-to-Video (T2V) models.

Method: Disco-LoRA, a unified framework that disentangles and flexibly recombines different concepts in two stages: (1) decomposing the objective into Content-Style and Content-Motion sub-tasks, and (2) using a Z-score-based statistical regularization to align weight distributions.
...
Conclusion: The proposed Disco-LoRA framework successfully addresses the challenge of multi-concept video customization, offering a comprehensive solution for controllable text-to-video generation.
...
TL;DR: DeCoFlow is proposed for continual anomaly detection in industrial environments, achieving state-of-the-art performance on MVTec-AD and VisA datasets while maintaining zero forgetting.


<details>
  <summary>Details</summary>
Motivation: The need for continual anomaly detection in industrial environments where new product categories arrive sequentially without access to past data, and the challenge of preventing catastrophic forgetting in Normalizing Flows (NFs).

Method: DeCoFlow, which decomposes subnets into a frozen universal base and task-specific low-rank adapters, and introduces Task-Specific Alignment, Auxiliary Coupling Layers, and Tail-Aware Loss.

Result: DeCoFlow achieves state-of-the-art image-level AUROCs of 98.40% on MVTec-AD and 93.00% on VisA, with parameter-level zero forgetting (0.00% FM under correct routing) and only 2.27M parameters per task.

Conclusion: DeCoFlow effectively addresses the challenge of continual anomaly detection in industrial environments, providing a solution that prevents catastrophic forgetting while maintaining the strict invertibility and Jacobian validity of NFs.
...
TL;DR: PhysEditWorld is a multimodal dataset for controllable world modeling, focusing on gravity, enabling explicit manipulation of physical dynamics.


<details>
  <summary>Details</summary>
Motivation: Current game world models lack controllable physical dynamics and are limited to exploratory trajectories, hindering their applicability to authored game environments.

Method: PhysEditWorld uses a replay paradigm with a UE5 replay-and-rendering pipeline to record gameplay interactions under multiple gravity configurations.

Result: PhysEditWorld enables improved gravity-faithful dynamics modeling, enhances consistency under physical edits, and provides a scalable foundation for controllable world modeling research.

Conclusion: PhysEditWorld is a valuable resource for controllable world modeling, allowing for explicit manipulation of physical dynamics, and improving the applicability of game world models to authored game environments.
...
TL;DR: A multi-task framework for intracranial aneurysm detection and segmentation across multiple imaging modalities and anatomical locations.


<details>
  <summary>Details</summary>
Motivation: Current automated methods for intracranial aneurysm detection are limited to binary detection and lack fine-grained anatomical classification or multi-class segmentation, which are crucial for rupture risk assessment and treatment planning.

Method: A two-stage approach combining 2D tri-axial ROI extraction with a 3D multi-task nnU-Net backbone, featuring a dual-decoder design, cross-attention pooling, and modality-specific auxiliary heads.

Result: The proposed framework achieved 2nd place in the RSNA 2025 Intracranial Aneurysm Detection challenge.

Conclusion: The proposed framework enables simultaneous multi-label classification, multi-class aneurysm segmentation, and multi-class vessel segmentation across 13 anatomical locations and four imaging modalities, providing a more comprehensive approach to intracranial aneurysm detection and treatment planning.
...
TL;DR: Proposed Mask to Concept (M2C), a framework adapting SAM3 for efficient medical few-shot annotation without external modules or manual text engineering.


<details>
  <summary>Details</summary>
Motivation: Current medical data annotation methods depend on external feature matchers or auxiliary networks, introducing architectural overhead and limiting performance scalability.

Method: M2C uses a learnable concept embedding to prompt segmentation, updates the embedding by minimizing concept segmentation error, and incorporates a Hybrid Uncertainty Estimation (HUE) module for active learning.

Result: Achieves SOTA few-shot segmentation performance and outstanding annotation efficiency on medical segmentation benchmarks.

Conclusion: M2C offers a practical and efficient pathway toward scalable medical image labeling with minimal expert effort.
...
TL;DR: NeuMatEx is a method for extracting neural materials from images using differentiable inverse rendering.


<details>
  <summary>Details</summary>
Motivation: Acquiring and authoring neural materials remains challenging, and existing methods are limited by their inability to accurately extract complex specular reflections and scattering effects.

Method: NeuMatEx uses a Large Material Reconstruction Model (LMRM) to predict initial base color, neural material latents, and aleatoric uncertainty guides from images, followed by inverse path tracing optimization.

Result: Experiments show that NeuMatEx extracts complex materials with better visual quality and material decomposition than PBR-based methods.

Conclusion: NeuMatEx enables the extraction of spatially varying neural materials from images, addressing the challenge of acquiring and authoring complex materials.
...
TL;DR: The paper proposes FUSE, a method to generate unlearnable examples that are effective across the full frequency spectrum, enhancing data protection.


<details>
  <summary>Details</summary>
Motivation: Existing unlearnable examples (UEs) fail when low-pass filtering is applied, indicating their perturbations are mainly in high frequencies, motivating the need for spectrum-agnostic UEs.

Method: FUSE generates unlearnable examples via spectral equalization, using Random Spectral Masking (RSM) and Cross-Band Guidance (CBG) to ensure effectiveness across the full spectrum.

Result: FUSE achieves strong protection across multiple datasets, architectures, and spectral filtering.

Conclusion: FUSE offers a robust method for protecting training data by generating unlearnable examples effective across the full frequency spectrum.
...
TL;DR: The paper analyzes the impact of real-world noise on Open Vocabulary Object Detectors (OV-ODs) and presents a comprehensive study called Robust Onion, which reveals how and where robustness degrades.


<details>
  <summary>Details</summary>
Motivation: The impact of real-world noise on OV-ODs remains poorly understood due to their architectural complexity.

Method: The study uses controlled synthetic visual degradations to analyze OV-ODs layer-by-layer and systematically analyze feature collapse.

Result: The findings reveal that models with similar vision backbones exhibit comparable robustness, and robustness is primarily governed by the image domain rather than annotations.

Conclusion: The study validates its insights by improving robustness on real-world datasets via a lightweight plug-and-play approach.
...
TL;DR: The paper introduces the 3D-anchored Light Probe (3DLP) benchmark to evaluate the ability of generative image editing models to understand real-world lighting. The authors create a high-fidelity HDR dataset and evaluate state-of-the-art models, finding differences in performance and room for improvement.


<details>
  <summary>Details</summary>
Motivation: The authors aim to investigate whether recent advancements in generative image editing models possess an intrinsic knowledge of real-world lighting, as existing benchmarks have limitations in evaluating light transport.

Method: The authors introduce the 3DLP benchmark, capture a new high-fidelity HDR dataset of real-world lighting changes, and evaluate state-of-the-art image editing models using two new scores to compensate for AI-generated photographic effects.

Result: The results show that the overall performance of models differs considerably, with the best models being remarkably consistent with real-world physics but still leaving room for improvement, particularly in image regions with less light from the light probe.

Conclusion: The authors conclude that while state-of-the-art image editing models show promising results in understanding real-world lighting, there is still room for improvement, and they make the benchmark and dataset publicly available to encourage future research.
...
TL;DR: A novel streaming video editing framework is proposed, enabling real-time, causal, frame-by-frame editing with strong content preservation and low latency.


<details>
  <summary>Details</summary>
Motivation: Current streaming video editing methods struggle with maintaining stable backgrounds and non-edited regions over time and achieving low latency for real-time interactive scenarios.

Method: A three-stage distillation pipeline is used to transfer editing capability from a powerful bidirectional foundation model to an efficient unidirectional streaming editor. An AR-oriented mask cache is also introduced to reuse region-related computation across frames.

Result: The proposed method achieves state-of-the-art visual quality among streaming baselines and drastically boosts inference speed to 12.66 FPS.

Conclusion: The proposed framework enables stable, high-quality, and real-time streaming video editing suitable for interactive and augmented reality applications.
...
TL;DR: The paper proposes a multimodal point cloud generation method based on vision-radar fusion to improve the quality of sparse and noisy millimeter-wave radar point clouds.


<details>
  <summary>Details</summary>
Motivation: Millimeter-wave radar point clouds are typically sparse, noisy, and structurally incomplete, which affects the performance of downstream perception tasks.

Method: The proposed method uses image semantic information to impose structural constraints and achieve spatial alignment for radar point clouds, and incorporates a sparse completion strategy to enhance point density and recover missing structures.

Result: Experimental results demonstrate that the proposed method effectively improves point cloud quality and enhances the detection accuracy and robustness of perception models in complex environments.

Conclusion: The proposed method provides a practical solution for multisensor point cloud generation and intelligent perception systems.
...
TL;DR: The paper proposes a framework for multi-view stylization of 3D Gaussian Splatting (3DGS) using optimal transport, which improves stylistic coherence across viewpoints.


<details>
  <summary>Details</summary>
Motivation: Enforcing stylistic coherence across viewpoints in 3DGS remains challenging, and existing methods lead to unstable style allocation and limited cross-view consistency.

Method: The proposed framework reformulates local style matching as a semi-balanced optimal transport problem with explicit column-capacity constraints, and incorporates cross-view matching guidance and geometric regularizations.

Result: The approach significantly improves multi-view stylistic consistency and produces stable, expressive 3D stylizations while preserving the core semantic structure of the scene.

Conclusion: The proposed framework provides a principled mechanism for balancing feature coverage and stylistic diversity, and enables controllable allocation of style features, leading to improved multi-view stylization of 3DGS.
...
TL;DR: Proposed ProtoKV, a memory method for streaming video understanding that improves accuracy under strict GPU-memory and query-time latency budgets.


<details>
  <summary>Details</summary>
Motivation: The challenge of delayed queries in streaming video understanding, where decisive cues may appear briefly but are at risk of being evicted or diluted due to bounded memory.

Method: ProtoKV, a constant-footprint SVU memory that uses a fixed-capacity summary state and a semantic-spatial prototype bank with residual statistics.

Result: ProtoKV improves accuracy by up to 12.5 points over token-retention baselines on SVU benchmarks in the long-delay regime.

Conclusion: ProtoKV effectively addresses the challenge of delayed queries in streaming video understanding, providing improved accuracy under strict budgets.
...
TL;DR: The authors propose a novel implicit neural representation method called CHOIR, which addresses optimization instability and spectrum bias issues in existing methods, achieving superior performance on multidimensional data recovery problems.


<details>
  <summary>Details</summary>
Motivation: The authors are motivated to improve implicit neural representation (INR) methods, which have limitations due to optimization instability and failure to incorporate physical priors, leading to spectrum bias.

Method: The authors propose Calibrated Harmonic Overlaid Implicit Neural Representation (CHOIR), which utilizes Coordinated Harmonic Superposition (CHS) to ensure optimization stability and Perceptual Spectrum Calibration (PSC) to mitigate spectrum bias.

Result: The proposed method, CHOIR, achieves superior performance over state-of-the-art approaches on various multidimensional data recovery problems.

Conclusion: The authors conclude that CHOIR effectively addresses the limitations of existing INR methods, providing a more stable and physically plausible approach for multidimensional data representation.
...
TL;DR: A 4D controllable generative framework for medical image synthesis is proposed, addressing limited annotated data and inter-device domain shifts.


<details>
  <summary>Details</summary>
Motivation: Limited annotated data, inter-device domain shifts, and privacy restrictions hinder the development of robust AI models for 4D medical imaging.

Method: A semi-supervised variational autoencoder and cascaded latent diffusion model are used to generate anatomically consistent 4D data.

Result: The proposed approach shows high controllability of static anatomy (Pearson r > 0.8) and strong temporal coherence (FVD = 288.08), improving downstream segmentation performance.

Conclusion: The framework provides a scalable and controllable solution for 4D medical image synthesis, supporting the development of more robust models with limited annotations and cross-vendor variability.
...
TL;DR: LearniBridge, a learnable calibration mechanism for feature caching, accelerates Diffusion Transformers (DiTs) up to 5.87x with minimal training samples.


<details>
  <summary>Details</summary>
Motivation: Existing feature caching methods for Diffusion Transformers (DiTs) suffer from severe error accumulation at high acceleration ratios.

Method: LearniBridge uses lightweight LoRA updates to bridge multiple timesteps and enable effective calibration with only 3-5 training samples.

Result: LearniBridge achieves up to 5.87x, 5.75x, and 4.10x acceleration on FLUX, HunyuanVideo, and WAN2.1, respectively, and improves VBench by 1.28% over the previous SOTA at 4.10x acceleration.

Conclusion: LearniBridge addresses the limitation of existing feature caching methods and enables efficient acceleration of DiTs with minimal training samples.
...
TL;DR: The paper proposes an event-based active vision system to track and estimate the spin of balls in real-time, achieving high accuracy and low latency in various sports, including table tennis, baseball, tennis, and golf.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the challenges of estimating the spin of a ball in ball sports using vision-based methods, given the ball's small size, high speed, and fast rotation, which make it difficult to track and measure spin accurately.

Method: The method used in this paper involves an event-based active vision system consisting of an event camera, high-speed pan/tilt galvanometer mirrors, and a low-latency focus-tunable telephoto lens. The system uses a hybrid approach for ball tracking and two methods for spin estimation: an offline method called contrast maximization on the sphere (s-CMax) and an online method using an uncertainty-aware convolutional neural network.

Result: The results show that the proposed system achieves state-of-the-art accuracy on static balls across multiple sports, with mean magnitude and axis errors of 2.1% and 4.0 degrees, respectively. For real-time applications in table tennis, the system demonstrates reliable tracking and spin estimation with high accuracy (8.8% magnitude and 6.4 degrees axis mismatch), 3 ms latency, and 750 Hz throughput during professional matches.

Conclusion: The conclusion of this paper is that the proposed event-based active vision system can accurately track unmodified balls and measure their spin in real-time, making it suitable for various ball sports and potentially enhancing game analysis and player training.
...
TL;DR: The authors propose ReasonCLIP, a continual pretraining framework that integrates large-scale reasoning supervision into CLIP-style models, improving visually grounded commonsense and compositional reasoning.


<details>
  <summary>Details</summary>
Motivation: The pretraining of CLIP-style models is dominated by descriptive image-text alignment, which may not support visually grounded commonsense inference and compositional reasoning required by downstream applications.

Method: The authors present a two-stage strategy for integrating reasoning signals into CLIP-style models, preserving descriptive alignment and using category-structured reasoning supervision.

Result: The trained ReasonCLIP models improve visually grounded commonsense and compositional reasoning, enhance zero-shot retrieval performance, and deliver consistent gains as a drop-in visual encoder for multimodal large language models.

Conclusion: The study demonstrates that structured reasoning supervision enhances the expressive capacity of CLIP-style visual representations, and the proposed framework, datasets, and models are made available.
...
TL;DR: NaviCache, a plug-and-play test-time self-calibration method, is proposed to accelerate Video Diffusion Models (VDMs) by re-conceptualizing feature evolution as an Inertial Navigation System (INS) problem.


<details>
  <summary>Details</summary>
Motivation: Video Diffusion Models (VDMs) are constrained by immense computational costs, and existing acceleration methods have limitations such as calibration data dependency, prohibitive calibration duration, and susceptibility to distribution shifts.

Method: NaviCache uses a dual-state estimation architecture to adaptively track the feature change ratio and its latent drift, and integrates a time-dependent noise schedule with an uncertainty-aware Measurement Update mechanism.

Result: Extensive experiments demonstrate that NaviCache exhibits more accurate error judgment for computation skipping and achieves outstanding comprehensive performance on the HunyuanVideo, Wan, and Open-Sora series.

Conclusion: NaviCache provides a theoretically grounded mechanism for error-bounded computation skipping, bridging the fundamental domain gap and the non-stationary nature of diffusion.
...
TL;DR: A new method for multi-modality image fusion (MMIF) is proposed, which integrates feature restoration and interaction to improve performance in adverse weather conditions.


<details>
  <summary>Details</summary>
Motivation: Existing MMIF methods struggle with effective representation learning under adverse weather conditions, which causes significant image degradation and disrupts feature representation.

Method: A mask-guided MMIF method is proposed, which includes a 'Pseudo Ground Truth' for simplified training, a mask generation mechanism, and a mask-guided cross-modal cross-attention mechanism.

Result: The proposed method surpasses state-of-the-art approaches in visual quality, quantitative metrics, and downstream tasks on synthetic and real-world datasets.

Conclusion: The proposed method effectively addresses the challenges of MMIF under adverse weather conditions by integrating feature restoration and interaction.
...
TL;DR: Proposed an illumination-robust seedling detection framework for ground-based agricultural monitoring under complex outdoor lighting conditions.


<details>
  <summary>Details</summary>
Motivation: Existing studies on seedling detection are mostly evaluated under stable imaging conditions, leaving robust detection under severe and spatially heterogeneous illumination in ground-based outdoor monitoring insufficiently explored.

Method: Proposed an illumination-robust seedling detection framework based on adversarial augmentation policy learning, which jointly optimizes a stochastic augmentation policy agent and an object detector.

Result: The proposed approach achieves an AP$_{50}$ of 91.6%, improving the baseline by 0.9 percentage points and outperforming the previous best-performing method by 0.2 percentage points. For downstream missing seedling localization, it achieves 75.0% precision and a 67.0% F1-score.
...
TL;DR: AnomNOVIC, a two-stage framework for open vocabulary object detection, achieves high accuracy in recognizing unseen objects in real-world environments.


<details>
  <summary>Details</summary>
Motivation: The need for robots to recognize previously unseen objects in open-world autonomy, without requiring a predefined class list or prompts.

Method: A two-stage framework combining a masked autoencoder (MAE) for anomaly detection and NOVIC, a real-time prompt-free open vocabulary image classifier.

Result: AnomNOVIC reaches 47.1% AP / 57.5% AP50 for prompt-free recognition and up to 82.6% prompt-free detection and classification accuracy on various datasets.

Conclusion: AnomNOVIC significantly surpasses open vocabulary baselines, including YOLO-World-v2, OWLv2, and YOLOE, in recognizing unseen objects.
...
TL;DR: The paper proposes a Liquid Fusion Network (LFNet) that combines the strengths of State Space Models (SSMs) and Convolutional Neural Networks (CNNs) for general Salient Object Detection (SOD).


<details>
  <summary>Details</summary>
Motivation: Current SOD approaches neglect the inherent spectral biases exhibited by different neural network paradigms. The authors aim to bridge this gap by harmonizing heterogeneous representations from SSMs and CNNs.

Method: The authors introduce a liquid fusion mechanism to dynamically integrate features from VMamba (SSM) and ConvNeXt (CNN) backbones. They also propose a Saliency-Guided Upsampling (SGU) operator to propagate features to shallow layers.

Result: LFNet achieves state-of-the-art performance across five diverse tasks (RGB, RGB-D, RGB-T, VSOD, and VDT), offering a superior trade-off between detection accuracy and model efficiency.

Conclusion: The proposed LFNet effectively bridges the spectral biases of SSMs and CNNs, leading to improved performance and efficiency in general SOD tasks.
...
TL;DR: The paper introduces a practical method for RS-aware relative pose estimation using affine correspondences, reducing the required point correspondences from 20 to 7.


<details>
  <summary>Details</summary>
Motivation: The current state-of-the-art solver for RS-aware relative pose estimation requires a minimum of 20 point correspondences, making it impractical for RANSAC-based robust estimation.

Method: The paper derives novel RS-corrected affine constraints and develops a linearized algebraic solver that estimates pose and RS motion from 7 affine correspondences.

Result: The proposed method achieves the best pose and RS parameter accuracy on the TUM RS benchmark and comparable accuracy to the standard 5-point algorithm on the EuRoC MAV dataset.

Conclusion: The paper makes RS relative pose estimation practical by introducing affine correspondences and a linearized algebraic solver, enabling accurate and efficient estimation.
...
TL;DR: The paper proposes SpatialFlow-GRPO, a framework for fine-grained image editing using spatially aware rewards, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing Flow-GRPO-style methods rely on a single whole-image reward, making fine-grained editing optimization difficult due to the spatial uniformity assumption.

Method: SpatialFlow-GRPO introduces spatially fine-grained reward feedback, converting region-aware rewards into semantic-region-level optimization signals, and aligns region advantages with latent positions during policy updates.

Result: SpatialFlow-GRPO outperforms Flow-GRPO on GEdit-Bench, ImgEdit-Bench, and MultiEditBench, showing improved editing quality.

Conclusion: The proposed SpatialFlow-GRPO framework effectively converts local feedback into spatially aligned update signals, leading to improved image editing quality.
...
TL;DR: The paper proposes a novel approach for reconfigurable intelligent surface (RIS) assisted proactive handover (PHO) in millimeter-wave networks to mitigate line-of-sight blockages, optimizing RIS elements for energy efficiency and link quality.


<details>
  <summary>Details</summary>
Motivation: Millimeter-wave networks are susceptible to line-of-sight blockages, and proactive handover (PHO) can mitigate this, but PHO is challenging when no nearby base station is available. RIS can restore connectivity, but its configuration time and element count affect responsiveness.

Method: The paper uses particle swarm optimization (PSO) to formulate an optimization problem that determines the optimal RIS link setup offline, balancing signal processing complexity and link quality under handover timing constraints.

Result: The proposed approach reduces the number of RIS elements by 12%, leading to a 10% decrease in dissipated energy without compromising the signal-to-noise ratio (SNR). The RIS-assisted link achieves a 15-30 dB improvement in blocked regions while maintaining accurate PHO timing.

Conclusion: The proposed RIS-assisted PHO approach optimizes the number of allocated RIS elements to balance signal processing complexity and link quality, making the RIS-assisted link more energy-efficient while ensuring timely PHO.
...
TL;DR: The authors propose a new concept bottleneck model (OTF-CBM) that uses optimal transport to align visual and textual representations for more interpretable reasoning.


<details>
  <summary>Details</summary>
Motivation: Existing vision-language concept bottleneck models rely on pre-aligned encoders or global cosine similarity, which can obscure fine-grained concept localization and fail to reflect true semantic geometry.

Method: The authors propose the Optimal Transport Flow Concept Bottleneck Model (OTF-CBM), which learns a data-driven semantic cost via Inverse Optimal Transport and performs unbalanced optimal-transport-based flow matching to model semantic transitions.

Result: OTF-CBM achieves superior classification accuracy and concept faithfulness, offering a new geometric and dynamical perspective for interpretable cross-modal reasoning.

Conclusion: The proposed OTF-CBM offers a more effective and interpretable approach to aligning visual and textual representations, leading to improved performance and faithfulness in concept bottleneck models.
...
TL;DR: The authors propose a 3D vision transformer called MICViT for integrating multimodal brain MRI data. MICViT outperforms state-of-the-art baselines in brain age prediction and benefits from multimodal inputs.


<details>
  <summary>Details</summary>
Motivation: The challenge of machine learning with high-dimensional 3D brain MRI data and limited sample sizes, while effectively integrating multimodal imaging signals.

Method: The authors propose a 3D vision transformer called MICViT, which combines four attention mechanisms to model modality-specific representations and cross-modal interactions.

Result: MICViT consistently outperforms state-of-the-art CNN and transformer baselines in 3D settings, and benefits more strongly from multimodal inputs.
...
TL;DR: A synthetic-data augmented framework is proposed for automated fiber bundle segmentation in macaque tracer histology, achieving state-of-the-art performance with less manually annotated data.


<details>
  <summary>Details</summary>
Motivation: The accuracy of diffusion MRI (dMRI) tractography is limited by indirect measurements, and manual annotation of fiber bundles in histology sections is time-consuming. A more efficient and accurate method is needed.

Method: A synthetic-data augmented framework using ex vivo dMRI tractography as a generative prior to synthesize 2D image patches for training a 2D U-Net.

Result: The approach achieves performance comparable to the state-of-the-art while requiring 3x less manually annotated data, and demonstrates improved generalization across brains and fiber bundle densities.

Conclusion: The proposed framework can efficiently and accurately segment fiber bundles in macaque tracer histology, reducing the need for manual annotation.
...
TL;DR: The paper proposes Robust-TO, a video understanding framework that assesses the trustworthiness of each input frame and integrates it into the reasoning process, improving accuracy and robustness under realistic perturbations.


<details>
  <summary>Details</summary>
Motivation: The Blind Trust Problem, where video reasoning models assume every input frame is equally reliable, leading to significant accuracy drops under perturbations such as motion blur, glare, or occlusion.

Method: Robust-TO, an agentic video understanding framework that explicitly integrates per-frame trustworthiness into every stage of reasoning, using a unified evidence interface, calibrated reliability scores, and a three-tier synthesis process.

Result: Robust-TO achieves 56.4% average accuracy on clean inputs and 54.3% under five realistic corruption types, outperforming the strongest open-source baseline and Gemini-2.5-Pro, with the smallest clean-to-corrupted accuracy drop among all compared methods.

Conclusion: The proposed Robust-TO framework effectively addresses the Blind Trust Problem, improving the accuracy and robustness of video reasoning models under realistic perturbations.
...
TL;DR: The paper proposes Qwen-Image-Agent, a unified framework that bridges the Context Gap in text-to-image models by integrating planning, reasoning, searching, memory, and feedback to generate images based on user context.


<details>
  <summary>Details</summary>
Motivation: Text-to-image models struggle with underspecified, implicit, or knowledge-dependent real-world requests due to a mismatch between user context and generation context, identified as the Context Gap.

Method: Qwen-Image-Agent uses Context-Aware Planning to identify missing context and Context Grounding to gather it from reasoning, searching, memory, and feedback, treating user input as partial context.

Result: Qwen-Image-Agent outperforms strong baselines and achieves state-of-the-art performance on Image Agent Bench (IA-Bench), Mindbench, and WISE-Verified.

Conclusion: The proposed Qwen-Image-Agent framework effectively bridges the Context Gap in text-to-image models, enabling more accurate and context-dependent image generation.
...
TL;DR: PhysRAG, a novel pipeline for enhancing physical awareness in video generation, achieves state-of-the-art performance in visual quality and physical rule compliance.


<details>
  <summary>Details</summary>
Motivation: The difficulty in capturing diverse physical phenomena, such as thermal dynamics, mechanics, and optics, makes developing physically aware video generation models a significant challenge.

Method: PhysRAG uses Retrieval-Augmented Generation (RAG) and a two-stage data filtering pipeline to enhance physical awareness in video generation, along with a mechanism to inject physical knowledge into a video diffusion model.

Result: PhysRAG achieves state-of-the-art performance in both visual quality and physical rule compliance, surpassing existing models in benchmarks such as PhyGenBench and VBench.

Conclusion: PhysRAG effectively enhances physical awareness in video generation, and its components are validated through extensive ablation studies.
...
TL;DR: The paper proposes PortraitGen, a framework for photorealistic portrait generation using reinforcement learning, addressing limitations in current methods that prioritize superficial aesthetics over realistic results.


<details>
  <summary>Details</summary>
Motivation: Current text-to-image post-training methods, such as GRPO, focus on superficial aesthetics and neglect critical flaws like AI artifacts and biological implausibilities.

Method: PortraitGen introduces real images into GRPO sampling groups using image inversion and employs a dual-reward mechanism: OmniReward for general quality and AI-Portrait for human-centric fidelity.

Result: PortraitGen significantly outperforms existing baselines, effectively suppressing AI artifacts and achieving unprecedented photorealism.

Conclusion: The proposed PortraitGen framework improves photorealistic portrait generation by addressing the limitations of current methods, resulting in more realistic and artifact-free portraits.
...
TL;DR: SharpMoE, a post-training framework, improves Mixture-of-Experts (MoE) architectures for diffusion models by accurately allocating computational resources to salient tokens, achieving state-of-the-art performance in visual generation.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion MoE frameworks struggle with routing assignment, failing to allocate more computational resources to salient tokens due to the router's reliance on noise-corrupted latent features.

Method: SharpMoE utilizes clean latent features as a noise-free guidance signal for routing, bypassing noise-distorted inputs, and introduces a trajectory routing loss to constrain compute allocation throughout the denoising trajectory.

Result: SharpMoE enhances pretrained MoE models, achieving state-of-the-art performance in visual generation, and serves as a versatile, plug-and-play solution.

Conclusion: SharpMoE effectively addresses the routing assignment problem in diffusion MoE frameworks, leading to improved performance and efficiency in visual generation tasks.
...
TL;DR: The authors propose TraMP-LLaMA, a framework for facial expression quality assessment that generates severity scores and textual reports from facial motion cues, achieving improved performance on report generation and severity prediction.


<details>
  <summary>Details</summary>
Motivation: Existing facial expression quality assessment methods lack interpretability as they only provide severity scores without supporting evidence, making it difficult to inspect model outputs in Parkinson's disease assessment.

Method: The authors propose a unified multimodal framework called TraMP-LLaMA, which integrates RGB appearance and landmark trajectory cues and uses a decoupled instruction-tuning strategy to reduce task interference.

Result: TraMP-LLaMA outperforms competitive video-language baselines in report generation and achieves the best severity prediction performance on the PFED5-plus dataset, improving Spearman's rank correlation by at least 4.39 percent.

Conclusion: The proposed TraMP-LLaMA framework provides a more interpretable approach to facial expression quality assessment by generating structured textual reports and achieving improved performance on severity prediction.
...
TL;DR: The paper introduces OmniRef-Bench, a benchmark for multi-reference image generation (MRIG), and proposes DyRef, a two-stage training framework that improves MRIG performance.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks fail to evaluate complex MRIG scenarios, hindering progress in this area.

Method: The authors propose a two-stage training framework called DyRef, which includes supervised fine-tuning and two novel techniques: Difficulty-aware Advantage Reweighting (DAR) and Discriminative Reward Scaling (DRS).

Result: DyRef significantly improves the performance of open-source models on OmniRef-Bench and single-image editing benchmarks.

Conclusion: The proposed DyRef framework effectively improves MRIG performance, demonstrating its effectiveness and generalization capability.
...
TL;DR: This paper introduces Dota2-Vis, a video-based dataset for visibility analysis in professional Dota 2 matches, and presents a baseline pipeline for player-icon detection.


<details>
  <summary>Details</summary>
Motivation: Current MOBA analytics studies rely on structured data, which does not capture what each team can see during a match. This work aims to provide a more comprehensive understanding of player visibility.

Method: The dataset includes 288 Full HD videos of 144 matches from The International 2025, along with 2,477 manually annotated minimap images. Multiple object detectors were evaluated for player-icon detection.

Result: YOLO11l (large) achieved the best performance in player-icon detection, enabling the estimation of opponent-visible player presence over time. The results reveal new patterns and behavioral differences that complement conventional MOBA analytics.

Conclusion: The Dota2-Vis dataset and code provide a new resource for visibility analysis in professional Dota 2 matches, offering insights that are difficult to obtain from structured data alone.
...
TL;DR: The paper proposes Geometric Gradient Rectification (GGR), a framework for open-set semi-supervised learning that controls gradients to improve performance on in-distribution classes while handling out-of-distribution outliers.


<details>
  <summary>Details</summary>
Motivation: Existing methods for open-set semi-supervised learning face a trade-off between filtering out suspicious samples and utilizing unlabeled data, which can lead to conflicting gradients. The authors aim to shift the focus to gradient-level control.

Method: The authors propose Geometric Gradient Rectification (GGR), a plug-in framework that projects conflicting auxiliary gradients onto an admissible region in gradient space, preserving orthogonal components.

Result: Experiments on CIFAR and ImageNet benchmarks show that GGR improves representative OSSL baselines in most settings and yields gains in both closed-set generalization and open-set robustness.

Conclusion: The proposed GGR framework offers a novel approach to open-set semi-supervised learning by controlling gradients, leading to improved performance and robustness.
...
TL;DR: Introduced Unison, a benchmark for evaluating unified multimodal models' understanding and generation capabilities together.


<details>
  <summary>Details</summary>
Motivation: Current evaluations of unified multimodal models assess understanding and generation separately, missing their synergy.

Method: Unison benchmark with 2,169 samples, evaluating internal consistency, understanding-guided generation, and more.

Result: State-of-the-art models show critical limitations, and Unison provides directions for future research.

Conclusion: Unison enables holistic evaluation of unified multimodal models, providing insights for improvement.
...
TL;DR: The paper proposes EVIS, an Event-Aware Video Instructed Segmentation Assistant, which decomposes videos into simple events using learnable Event Queries and text guidance to improve referring video segmentation.


<details>
  <summary>Details</summary>
Motivation: Existing referring video segmentation methods treat videos as single events, leading to confusion and hallucinations due to complex content. The authors aim to address this by decomposing videos into distinct events.

Method: The authors propose EVIS, which uses text-guided Event Queries to partition videos into simple events and extract event-aware visual-text features. They also introduce Object-Pixel-Hybrid Learning to track targets in long-term videos.

Result: The proposed method, EVIS, demonstrates strong performance on 5 public benchmarks for the referring video segmentation task.

Conclusion: The paper concludes that decomposing videos into simple events and using event-aware visual-text features can improve referring video segmentation, and that EVIS is an effective approach for this task.
...
TL;DR: A novel unsupervised method for anomaly detection in satellite images using Remote Sensing Foundation Models (RSFMs) and a ResNet + FPN architecture, achieving efficient and high-performance results without requiring training or extensive development effort.


<details>
  <summary>Details</summary>
Motivation: The motivation is to maximize the utility of satellites' limited power and computational resources by autonomously triggering high-resolution captures or adjusting tasking parameters upon detecting an anomaly, and to improve disaster monitoring without expensive labels.

Method: The method presented is an unsupervised detection approach based on ResNet (RSFM) + FPN, which identifies anomalies by detecting subtle semantic shifts in the latent space between successive orbital passes.

Result: The system achieves efficient image-level generation and higher resolution mapping with minimal effort (training-free) and comparable results to previous proposals, while ensuring high-performance generalization across diverse terrains and sensors.

Conclusion: The proposed method offers a well-informed and transformative path for disaster monitoring and anomaly detection in satellite images, eliminating the need for bespoke training and extensive development effort, and ensuring high-performance generalization.
...
TL;DR: PanoImager is a framework for 3D reconstruction from sparse panoramic images, overcoming challenges in SfM/SLAM initialization.


<details>
  <summary>Details</summary>
Motivation: The challenge of 3D reconstruction from sparse panoramas under rotation-dominant, weak-parallax motion, where SfM/SLAM initialization often fails.

Method: PanoImager combines feed-forward pose/depth priors, geometry-conditioned diffusion view completion, and depth-guided 3DGS optimization.

Result: Improved stability in 3D reconstruction under extreme sparsity, outperforming SfM/SLAM methods.

Conclusion: PanoImager offers a promising approach for map refinement in scenarios where SfM/SLAM fails to initialize.
...
TL;DR: CT-3GDINO, a lightweight 3D detector, is proposed for reliable organ localization in abdominal CT scans, achieving strong performance in coarse localization.


<details>
  <summary>Details</summary>
Motivation: Reliable organ localization in abdominal CT scans can provide spatial priors for downstream trauma analysis.

Method: CT-3GDINO uses a Grounding-DINO-style query-based architecture with frozen pseudo-text class tokens, a Swin3D visual backbone, bidirectional feature enhancement, pseudo-text-guided query selection, and a cross-modality decoder.

Result: The best multi-scale model achieves 0.5830 overall top-1 class-wise mAP over 3D IoU thresholds from 0.1 to 0.7, with strong performance in coarse localization (0.9649 AP at IoU 0.1) but limited performance in strict box alignment (0.1552 AP at IoU 0.7).

Conclusion: CT-3GDINO establishes an open-source baseline for pseudo-text-conditioned 3D organ localization, motivating future work on localization-aware pretraining, richer multimodal conditioning, and injury-focused detection.
...
TL;DR: SubdivAR is a neural mesh subdivision framework that uses a novel autoregressive representation to improve detail synthesis and generalizability in mesh subdivision.


<details>
  <summary>Details</summary>
Motivation: Classical mesh subdivision methods produce over-smoothed surfaces and recent neural methods are limited by local modeling and lack generalizability.

Method: SubdivAR uses a Mesh Autoregressive Representation (MAR) and a Hybrid Topology-Aware Transformer to predict vertex offsets at each refinement stage.

Result: SubdivAR outperforms state-of-the-art baselines, reducing Hausdorff Distance by 18.8% and Chamfer Distance by 14.2%, and shows strong robustness on complex geometries.

Conclusion: SubdivAR presents a novel approach to mesh subdivision with improved detail synthesis and generalizability, outperforming existing methods.
...
TL;DR: A new Tree-structured Mixed-policy Pruning (TMP) framework is proposed to compress large image generation models, reducing parameters and computation while maintaining generation quality.


<details>
  <summary>Details</summary>
Motivation: Large image generation models require significant resources and GPU memory, making them unaffordable; the authors aim to reduce model size without sacrificing performance.

Method: The TMP framework is applied to step-distilled models, generalizing to various image tasks and architectures, and is tested on HunyuanImage-3.0 and Z-Image turbo models.

Result: The framework successfully compresses HunyuanImage 3.0 from 80B to 20B parameters (75% reduction) with limited generation quality loss and enables inference on a single 24GB 4090 GPU; it also compresses Z-Image turbo from 6B to 4B parameters (33% reduction) with negligible degradation.

Conclusion: The proposed TMP framework effectively compresses large image generation models, making them more affordable and efficient without significant performance loss.
...
TL;DR: Introduced FlameVQA, a benchmark for UAV-based wildfire monitoring using RGB and thermal images, and evaluated MLLMs for tasks like detection and cross-modal reasoning.


<details>
  <summary>Details</summary>
Motivation: Wildfire monitoring from UAVs is challenging due to smoke, scale variation, and occlusions, which limit RGB-only interpretation.

Method: FlameVQA benchmark was built on FLAME 3, using paired RGB and radiometric thermal TIFFs, and includes 34 multiple-choice questions per image across six operational capability groups.

Result: MLLMs showed strong performance with explicit cross-modal cues, but failed on presence detection under heavy smoke and coverage estimation.

Conclusion: Current MLLMs require domain-specific adaptation for better disaster and wildfire monitoring support.
...
TL;DR: This paper proposes a method to improve the safety of autoregressive text-to-image models by iteratively refining their codebooks to eliminate harmful image generations.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the safety concerns of images generated by autoregressive unified multimodal models, which can produce unsafe content.

Method: The method involves two steps: (1) using the unified model to identify unsafe generations and construct harmful and safe image-text pairs to update the codebook, and (2) performing adaptive fine-tuning on the codebook within the harmless space using safe image-text pairs.

Result: The result is a safety-enhanced model codebook that iteratively improves without external feedback.

Conclusion: The proposed method allows for safe autoregressive generation by leveraging the model's own understanding and judgment capabilities to identify and eliminate harmful mappings in the codebook.
...
TL;DR: The paper introduces HarmVideoBench, a new benchmark for evaluating large vision-language models' understanding of harmful videos, addressing limitations in existing works by incorporating multi-layered characteristics and explanatory rationales.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for harmful video evaluation have two primary limitations: they overlook the multi-layered characteristics of harmful videos and lack explanatory rationales, making evaluation a black box.

Method: The authors propose HarmVideoBench, a diagnostic benchmark with 1,379 videos and 4,137 multiple-choice questions, evaluating models' deep understanding across three hierarchical dimensions: Observable Evidence, Clip-Internal Meaning, and Beyond-Clip Reasoning. They also introduce BCR, a method that predicts reasoning boundaries and dynamically retrieves context.

Result: Experimental results show that BCR substantially improves the base model's performance in harmful video understanding, raising the macro average from 61.7% to a state-of-the-art 84.4%.

Conclusion: The paper presents a new benchmark and method for evaluating and improving large vision-language models' understanding of harmful videos, addressing existing limitations and achieving state-of-the-art results.
...
TL;DR: The authors propose LIkelihood Score Alignment (LISA), a regularization method to improve the efficiency and effectiveness of the dual-branch paradigm in visual-condition controllable generation.


<details>
  <summary>Details</summary>
Motivation: The role of the side branch and its training efficiency in the prevalent dual-branch paradigm remain underexplored.

Method: The authors propose LISA, which aligns the intermediate feature of the side network with an approximated likelihood score through a lightweight decoder and an additional regularization loss.

Result: Experiments demonstrate that LISA can consistently accelerate training convergence, improve final synthetic results, and encourage the side network's features to be more disentangled with negligible additional training cost and zero extra inference cost.

Conclusion: The proposed LISA method provides an effective way to improve the efficiency and effectiveness of the dual-branch paradigm in visual-condition controllable generation.
...
TL;DR: The paper proposes SatSplatDiff, a method for satellite 3D reconstruction that improves visual fidelity and reduces geometric degradation by incorporating monocular depth supervision, multi-scale geometric refinement, and shadow-guided generative refinement.


<details>
  <summary>Details</summary>
Motivation: The limited top viewpoint of satellite imagery results in insufficient supervision on building facades, leading to surface holes and degraded visual fidelity in Gaussian Splatting-based reconstruction methods.

Method: The proposed SatSplatDiff method builds on photogrammetric DSM initialization and 2DGS-based shadow casting, and introduces monocular depth supervision, multi-scale geometric refinement, and shadow-guided generative refinement to establish a geometrically accurate and well-regularized surface representation.

Result: Extensive evaluations on the IARPA2016 and DFC2019 datasets demonstrate state-of-the-art performance, reducing geometric MAE by up to 18% and improving visual fidelity (FID-CLIP) by 28-45% over existing baselines.

Conclusion: The proposed SatSplatDiff method delivers up to 5x resolution enhancement with minimal hallucination and sensor-consistent appearance, demonstrating seamless cross-tile consistency and strong scalability for large-scale reconstruction.
...
TL;DR: DanceOPD, a framework for training image generation models with unified diverse capabilities.


<details>
  <summary>Details</summary>
Motivation: The need for a single model that unifies text-to-image, local editing, and global editing capabilities, which are often conflicting.

Method: On-policy generative field distillation framework for flow-matching models.

Result: Improved multi-capability composition, preserving anchor generation quality.

Conclusion: DanceOPD establishes a practical route for generative field distillation in flow-matching models.
...
TL;DR: CORTEX is a structured reasoning benchmark for 3D chest CT, providing a four-stage diagnostic trace to improve interpretability and verifiability of multimodal large language models in medical imaging.


<details>
  <summary>Details</summary>
Motivation: The lack of structured supervision and verification protocols for reasoning-capable 3D chest CT multimodal large language models, which hinders their development and trustworthiness.

Method: CORTEX uses a four-stage diagnostic trace (task understanding, visual observation, diagnostic reasoning, and answer synthesis) generated by large language models and verified by clinicians, built on the publicly available CT-RATE dataset.

Result: CORTEX comprises 76,177 validated reasoning traces across open-ended VQA, closed-ended VQA, and report generation, providing a benchmark for building and evaluating trustworthy reasoning models for 3D chest CT.

Conclusion: CORTEX enables the development of more interpretable and verifiable multimodal large language models in medical imaging, particularly in 3D radiology, by providing a structured reasoning benchmark and evaluation protocol.
...
TL;DR: A new method for exact nearest neighbor search in high-dimensional spaces, achieving significant speedup over exhaustive search while maintaining exactness.


<details>
  <summary>Details</summary>
Motivation: The need for efficient and exact nearest neighbor search in large databases, which is a fundamental problem in computer vision and machine learning.

Method: Hierarchical Normalization (HN) - a normalization scheme that splits the feature vector into major and minor components, allowing for a provably exact branch-and-bound scan.

Result: The proposed method achieves 13.7x and 12.7x speedup over brute-force search on trevi and halfdome datasets, respectively, with only 0.4% of entries requiring full evaluation.

Conclusion: The proposed method provides a deterministic and exact solution for nearest neighbor search, outperforming approximate methods in terms of speed and accuracy.
...
TL;DR: This paper presents a method for 3D generation using reinforcement learning from human feedback (RLHF) without requiring explicit surface representations or pretraining, achieving improved 3D geometry.


<details>
  <summary>Details</summary>
Motivation: Existing pipelines for 3D generation using RLHF require optimizing explicit surface representations, which can be limited; the authors aim to fine-tune a pretrained 3D-aware generative model directly from a learned reward over radiance-field density values.

Method: The authors fine-tune a pretrained 3D-aware generative model (EG3D) using a learned reward model that reads the continuous 3D density field of the neural radiance field (NeRF) directly, with a density-consistency constraint to maintain 2D appearance.

Result: The fine-tuned generator produces face geometries preferred by users in 74.4% of pairwise comparisons, with a measurable but bounded distributional cost (FID-50k rises from 4.09 to 6.66).

Conclusion: The proposed method achieves robust improvement in 3D geometry without requiring externally supplied mesh or shape prior, text conditioning, mesh extraction, or multi-view rendering.
...
TL;DR: Introducing SmellNet-V, a visuo-olfactory dataset, and See & Sniff, a self-supervised framework for joint visuo-olfactory representation learning, surpassing smell-only baselines.


<details>
  <summary>Details</summary>
Motivation: The lack of paired visuo-olfactory data hinders the exploration of olfaction in multimodal models, and the authors aim to address this gap.

Method: The authors propose See & Sniff, a self-supervised framework that learns joint visuo-olfactory representations via dense local alignment.

Result: The method surpasses smell-only baselines by 7% in smell classification and generalizes to cross-modal retrieval and smell localization.

Conclusion: The study establishes visuo-olfactory learning as a new direction in multimodal perception, enabling applications such as smell localization and cross-modal retrieval.
...
TL;DR: ViQ, a Visual Quantized Representations framework, balances semantics and details in discrete visual representations, enabling unified and efficient multimodal modeling.


<details>
  <summary>Details</summary>
Motivation: The pursuit of a unified representation for text and vision to simplify multimodal modeling and training, while addressing the challenge of representing images as discrete signals without severe information loss.

Method: ViQ structures quantization learning into two stages: text-aligned pre-training and feature discretization, with a proximal representation learning strategy and a position-aware head-wise quantization mechanism.

Result: ViQ achieves competitive performance in multimodal tasks compared to state-of-the-art multimodal vision encoders, with high precision in low-level reconstruction and up to 20%-70% acceleration in multimodal training.

Conclusion: ViQ provides a unified and general discrete representation for arbitrary visual inputs, balancing semantics and details while supporting native resolutions, and enabling efficient multimodal modeling and training.
...
TL;DR: OctoSense is an open-source sensor platform with multimodal data collection and a novel 'late-fusion' masked autoencoder for self-supervised learning, enabling robust and fast processing in various environments.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the challenge of multi-modal self-supervised learning in real-world robotics data, where sensors have different representations, frequencies, latencies, and noise.

Method: The authors propose a 'late-fusion' masked autoencoder architecture, which uses modality-specific tokenizers to handle different spatiotemporal characteristics of sensors and caches modality-specific tokens for efficient processing.

Result: The proposed approach performs better than existing image-only foundation models on tasks such as optical flow, depth, semantic segmentation, and ego-motion estimation, and demonstrates robust prediction at nighttime or in situations with degraded sensory data.

Conclusion: The paper presents a novel sensor platform, OctoSense, and a corresponding dataset, along with a self-supervised learning approach that enables fast and robust processing of multimodal data, with potential applications in real-world robotics scenarios.
...
TL;DR: The paper proposes DexAC-WM, a new approach to action-conditioned world models that treats action conditioning as a structured process, improving performance in high-DoF control scenarios.


<details>
  <summary>Details</summary>
Motivation: Existing action-conditioned world models struggle with high-DoF control due to the heterogeneous nature of dexterous actions, which can lead to imbalanced optimization and reduced action fidelity.

Method: The authors propose DexAC-WM, which uses action tokenization, local refinement, and global modulation to preserve dimension-level semantics and align action signals with visual dynamics. A semantic branch is also introduced to provide rich object-scene priors.

Result: Experiments on EgoDex and EgoVerse show that DexAC-WM significantly improves FID, FVD, and PCK, demonstrating gains in visual-temporal realism and action-following consistency. The approach also shows scalability across different backbones.

Conclusion: The paper concludes that scaling world models to high-DoF control requires both structured action modeling and semantic grounding, and that DexAC-WM offers a promising solution to this challenge.
...
TL;DR: A new method for moving objects in single images while preserving scene consistency, including handling occlusions and maintaining coherent shadows and reflections.


<details>
  <summary>Details</summary>
Motivation: Existing approaches often fail to preserve scene-level consistency when moving objects in a single image.

Method: A geometry-aware object motion method that operates on diffusion transformers with rotary positional embeddings (RoPE) and a depth-aware formulation that encodes 3D spatial structure.

Result: State-of-the-art performance on standard object motion benchmarks, preserving object identity and generating plausible content in newly revealed regions.

Conclusion: The proposed method effectively preserves scene consistency when moving objects in single images, with minimal real supervision.
...
TL;DR: A new framework called SAM2Matting is proposed to improve video matting by enhancing trackers to handle high-fidelity video matting.


<details>
  <summary>Details</summary>
Motivation: The gap between high-level tracking and low-level matting in video matting, and the limitations of existing methods that use expensive and narrowly-scoped video matting datasets.

Method: A tracker-to-matting framework that decouples the task by enhancing a foundational tracker with a region-proposal bridge and dedicated matting heads.

Result: SAM2Matting establishes new state-of-the-art performance on video matting, supports diverse prompt types, maintains strong temporal consistency, and demonstrates robust generalization across human-centric and in-the-wild scenarios.

Conclusion: SAM2Matting advances VOS trackers to high-fidelity video matting and shows promising results in video matting tasks.
...
TL;DR: The paper proposes RayPE, a positional-encoding extension for video diffusion transformers that incorporates 6D Plucker coordinates to improve camera controllability, cross-frame 3D consistency, and overall video quality.


<details>
  <summary>Details</summary>
Motivation: Current video diffusion transformers use RoPE on the (u,v,t) axes, which doesn't capture the 3D structure of the scene. The authors aim to improve this by incorporating geometric relations between camera rays.

Method: The authors propose RayPE, which injects 6D Plucker coordinates into the queries and keys of self-attention. They also decouple ray direction from moment magnitude, gate the encoding, and apply RMSNorm for stability.

Result: The full module improves camera controllability, cross-frame 3D consistency, and overall video quality on a four-dataset training mixture, with less than 0.1% additional parameters.

Conclusion: The proposed RayPE extension enhances the performance of video diffusion transformers by effectively incorporating geometric information, leading to improved controllability and consistency in video generation.
...
TL;DR: PhysiFormer, a diffusion transformer, predicts 3D object motion in world coordinates, capturing uncertainty and generalizing to new settings.


<details>
  <summary>Details</summary>
Motivation: The need for a model that can predict physically-plausible 3D object motion in a view-invariant and geometry-aware manner, which is useful for applications with unobserved uncertainty.

Method: A diffusion transformer that represents objects as 3D meshes in world coordinates and uses a single denoising diffusion process to predict future vertex trajectories.

Result: PhysiFormer outperforms autoregressive baselines in trajectory accuracy, rigidity preservation, and momentum-based physical consistency, and generalizes to mixed-material settings, unseen real-world geometries, and larger object counts.

Conclusion: PhysiFormer presents a promising step toward view-invariant, geometry-aware world modelling for robotics, graphics, and physical design, with potential applications in areas with unobserved uncertainty.
...
TL;DR: A new method is proposed to improve diversity in flow models without extra overhead.


<details>
  <summary>Details</summary>
Motivation: Existing flow models suffer from diversity collapse when generating multiple samples under the same conditions.

Method: An efficient, training-free self-guidance mechanism is introduced, which disperses internal features during batch generation and applies manifold regularization.

Result: Significant improvements in diversity while preserving fidelity across several conditional flow models.

Conclusion: The proposed method effectively mitigates diversity collapse in flow models with minimal inference cost.
...
TL;DR: The paper proposes Denoising Attention (DnA), an alternative to softmax activation in multihead attention models, which improves performance on visual perception tasks by reducing noisy attention patterns.


<details>
  <summary>Details</summary>
Motivation: The standard softmax activation in multihead attention models can produce noisy attention patterns, diluting relevant features and degrading performance.

Method: The proposed DnA uses a positive query to identify relevant image features and a negative query to identify irrelevant features, projecting these interactions into two distinct subspaces with larger principal angles to improve discriminability.

Result: DnA achieves an absolute gain of 0.8% on ImageNet-1K and improvements in multiple visual understanding tasks, including video understanding with video transformers (1.8%) and video LLMs (0.5%).

Conclusion: The proposed DnA improves performance on visual perception tasks by reducing noisy attention patterns, and extensive empirical analyses justify its design choices.
...
TL;DR: The paper proposes VISE, a purely unsupervised self-evolving framework that improves visual reasoning in large multimodal models by regularizing the model's visual conditioning policy, resulting in significant gains on various benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing self-evolving large multimodal models struggle with visual under-conditioning, where the decoder relies on language priors rather than visual content, leading to poor performance on vision-language understanding tasks.

Method: The authors propose VISE, which uses two complementary invariance-based rewards: geometric invariance and semantic invariance, to regularize the model's visual conditioning policy without requiring specialist roles, external reward models, or annotations.

Result: Experiments on 18 benchmarks demonstrate the efficacy of VISE, achieving significant gains on COCO (+16.85 CIDEr), TextCaps (+19.66 CIDEr), and reducing object hallucination by 5.0 Chair-I points, with generalization across four model families and scales.

Conclusion: The proposed VISE framework effectively addresses the issue of visual under-conditioning in self-evolving large multimodal models, leading to improved performance on various vision-language understanding tasks.
...
TL;DR: The paper proposes a self-evolving training framework for unified large multimodal models (LMMs) that improves both visual understanding and image generation autonomously using only unlabeled images.


<details>
  <summary>Details</summary>
Motivation: Current LMMs rely on curated post-training supervision, and the authors aim to explore if a unified LMM can improve both abilities autonomously without human annotations or external reward models.

Method: The proposed framework consists of a Proposer, Solver, and Generator, which interact to generate visual questions, answer and evaluate them, and synthesize images, respectively, using self-derived consistency signals and Solver Token Entropy (STE) for stabilization.

Result: The method consistently improves over base models across eight understanding metrics, achieving a +3.5% absolute gain on MMMU and improving GenEval image generation performance from 82% to 85% on BAGEL.

Conclusion: The proposed self-evolving training framework enables unified LMMs to improve both visual understanding and image generation autonomously without relying on human annotations or external reward models.
...
TL;DR: HierBias, a hierarchical media bias detector, leverages document context to improve bias detection accuracy, outperforming state-of-the-art models.


<details>
  <summary>Details</summary>
Motivation: Existing sentence-level media bias detection approaches ignore inter-sentence contextual signals, which human annotators naturally exploit.

Method: HierBias pairs a sentence-level RoBERTa encoder with a cross-sentence Transformer aggregator and dual output heads for binary detection and four-class type classification.

Result: HierBias achieves 0.853 F1 and 0.723 MCC on BABE and BASIL datasets, surpassing state-of-the-art models by +2.6% F1 and +4.3% MCC.

Conclusion: The proposed HierBias model effectively utilizes document context to improve media bias detection accuracy, with ablation experiments confirming the contribution of each theoretical component.
...
TL;DR: The authors present a benchmark to evaluate large language models' ability to distinguish between answerable and unanswerable questions, while considering data contamination and abstention.


<details>
  <summary>Details</summary>
Motivation: The authors aim to develop a reliable evaluation method for large language models that separates supported answering from unsupported guessing, while avoiding conflation with data contamination, prompt idiosyncrasy, or generic refusal behavior.

Method: The authors propose a contamination-aware, multi-zone benchmark with 1,200 items across five domains, including explicit abstention expectations, contamination-risk metadata, and dual parsing.

Result: The evaluation of FLAN-T5, Qwen2.5-Instruct, and Llama-3-Instruct models shows that Qwen2.5-3B-Instruct achieves the best overall reliability, but answer-expected zones remain difficult, calibration remains poor, and benign-item refusal persists.

Conclusion: The benchmark provides a reproducible protocol for auditing answerability, abstention, refusal, and contamination as distinct but interacting dimensions of LLM reliability.
...
TL;DR: The study investigates how different post-training methods affect a language model's retention of animal compassion values. It finds that helpfulness training degrades these values, while coding training preserves them better.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to understand how post-training processes impact the retention of values instilled during pre-training in language models, specifically focusing on animal compassion.

Method: The study uses a Llama 3.1 8B model mid-trained on compassion-oriented synthetic data and applies both supervised fine-tuning (SFT) and reinforcement learning (GRPO) with different domains (helpfulness and coding).

Result: Helpfulness training significantly degrades animal compassion values compared to coding training. The effect is consistent across two independent helpfulness datasets and two training paradigms. However, the effect does not transfer cross-lingually for general moral reasoning.

Conclusion: The study suggests that coding-domain post-training may better preserve mid-trained values than helpfulness post-training without harming general reasoning capabilities. Values instilled through mid-training are encoded more deeply and cross-lingually than reasoning improvements from domain-specific post-training.
...
TL;DR: This study evaluates Large Language Models' (LLMs) performance in solving statics problems, particularly in mechanical engineering education, and finds that while LLMs excel in text-only problems, their accuracy drops when diagrams and multi-step reasoning are introduced.


<details>
  <summary>Details</summary>
Motivation: The rapid influence of LLMs on education, especially in mechanical engineering, and the need for topic-specific analysis of their capabilities in solving specific problem types.

Method: The study uses a model distillation process to evaluate LLM capabilities, constructing three datasets of statics problems: text-only, diagrams-added, and modified numerical values.

Result: LLMs perform well on text-only statics problems but show decreased accuracy when diagrams and multi-step reasoning are introduced, primarily due to difficulties in multi-step reasoning and applying visual information.

Conclusion: The study highlights the limitations of LLMs in solving statics problems that require multi-step reasoning and visual information, emphasizing the need for further research in developing LLMs' capabilities in mechanical engineering education.
...
TL;DR: The study analyzes how different linguistic features in animal-welfare texts affect Llama-3.2-1B's preference for pro-animal-welfare reasoning when used as fine-tuning data.


<details>
  <summary>Details</summary>
Motivation: To understand how animal-welfare advocates' writing influences language models and to provide practical recommendations for writing effective animal-welfare texts.

Method: Using vocabulary-matched stance-contrast probes on a held-out animal-welfare benchmark to measure the impact of ten linguistic features on Llama-3.2-1B's preference for pro-animal-welfare reasoning.

Result: Eight out of ten linguistic features produce statistically significant shifts in Llama-3.2-1B's preference for pro-animal-welfare reasoning. Seven features move the model toward stronger pro-animal-welfare reasoning, while two move it in the opposite direction.

Conclusion: The study recommends that writers of animal-welfare texts assert a position explicitly rather than describing a scene neutrally, as features that make the writer's position explicit tend to shift the model toward stronger pro-animal-welfare reasoning.
...
TL;DR: ContextForge is a system that helps large language models (LLMs) handle long conversations more efficiently by reusing previous information without needing to re-read the entire context.


<details>
  <summary>Details</summary>
Motivation: LLMs struggle with long conversations due to limitations in context window and inefficient token usage, leading to performance degradation over time.

Method: ContextForge combines structured query generation, external memory retrieval, and controlled synthesis to recycle context and enable efficient reuse of prior computation.

Result: ContextForge shows improved consistency, reduced token consumption, and comparable response accuracy compared to a baseline agent in a 15-turn conversational benchmark.

Conclusion: Context recycling provides a practical approach to extend LLM capabilities in long-horizon tasks without requiring larger context windows or model retraining.
...
TL;DR: This paper explores using Nonviolent Communication (NVC) principles to guide large language models (LLMs) in de-escalating conflicts through dialogue.


<details>
  <summary>Details</summary>
Motivation: The increasing use of LLMs in emotionally charged situations involving conflict, frustration, and distress, and the need to prevent unintentional escalation of conflicts through conversational behaviors.

Method: Using a dual-agent simulation framework, the authors apply NVC-constrained prompting to multiple instruction-tuned models and user resistance levels.

Result: NVC-constrained prompting consistently reduces conversational escalation and stabilizes interactions with highly resistant users.

Conclusion: Simple communication constraints based on NVC principles can improve the trustworthiness of LLM dialogue in conflict-prone settings.
...
TL;DR: A pilot study presents NEST-V1, a multimodal framework that generates emotion-conditioned Nepali Sign Language avatars from spoken input, achieving high accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: The study aims to address the underexploration of sign language communication systems that integrate emotional expression, particularly for low-resource languages.

Method: The framework employs a shared acoustic encoder for Automatic Speech Recognition and emotion classification, using a dataset of 600 labeled audio samples from 50 speakers.

Result: The system achieves 81.1% ASR accuracy, 79.21% emotion recognition accuracy, and demonstrates 37% parameter efficiency compared to separate model architectures.

Conclusion: The pilot work establishes the technical foundation for emotion-aware sign language translation in low-resource settings, providing a scalable framework for future expansion.
...
TL;DR: Larger language models outperform smaller ones on reasoning benchmarks, and the study identifies the reasoning differences behind this gap, highlighting Constraint-Guided Reasoning as a key advantage.


<details>
  <summary>Details</summary>
Motivation: The reasoning differences underlying the performance gap between larger and smaller language models on reasoning benchmarks remain underexplored.

Method: The study develops AdvCluster, an automated framework that identifies questions where larger models show a stable advantage, extracts fine-grained advantage descriptions, and organizes them through semantic clustering.

Result: The analysis yields a systematic taxonomy of larger model reasoning advantages, including Constraint-Guided Reasoning, which enables larger models to better identify and use constraints to guide reasoning.

Conclusion: The study provides insights into the reasoning differences between larger and smaller language models, highlighting the importance of Constraint-Guided Reasoning in larger models' performance advantages.
...
TL;DR: A methodology for creating specialized conversational AI systems for low-resource languages using structured linguistic resources like WordNet.


<details>
  <summary>Details</summary>
Motivation: The lack of massive training corpora for low-resource languages hinders the development of specialized conversational AI systems.

Method: Converting Hindi WordNet into instruction-response pairs and fine-tuning a 12B-parameter language model using LoRA with 4-bit quantization.

Result: The structured-knowledge-based system achieved superior pedagogical effectiveness (91.0) compared to general-purpose models (79.4-83.6) while maintaining competitive semantic performance and exceptional consistency.

Conclusion: The proposed methodology offers a practical alternative to corpus-intensive approaches, enabling specialized AI development for low-resource languages with existing WordNet resources.
...
TL;DR: The paper proposes Dynamic-dLLM, a framework that improves the inference efficiency of diffusion large language models (dLLMs) without compromising performance.


<details>
  <summary>Details</summary>
Motivation: The computational complexity of dLLMs scales with the sequence length L cubed, posing challenges for long-sequence and real-time applications.

Method: Dynamic-dLLM consists of two components: Dynamic Cache Updating (DCU) and Adaptive Parallel Decoding (APD), which adaptively allocate cache-update budgets and calibrate decoding thresholds, respectively.

Result: Dynamic-dLLM achieves an average speedup of over 3 times while maintaining performance across various benchmarks and models.

Conclusion: Dynamic-dLLM provides a plug-and-play solution for efficient dLLM deployment without compromising performance, outperforming state-of-the-art acceleration methods.
...
TL;DR: The study examines the methodological suggestions of Large Language Models (LLMs) GPT-5.1, Gemini 3 Pro, and DeepSeek-V3.2 when prompted with research questions from 1,000 recent computer-science papers. The results show that LLMs tend to suggest a narrower range of methods, with significant imbalances in provider choice and underrepresentation of certain model types.


<details>
  <summary>Details</summary>
Motivation: The increasing use of LLMs to guide research methodology, with a need to understand their default methodological tendencies under minimal prompting.

Method: The study uses LLM-extracted research questions from 1,000 recent arXiv computer-science papers to prompt GPT-5.1, Gemini 3 Pro, and DeepSeek-V3.2. The resulting methodology suggestions are compared against a paper-derived experimental inventory, with structured method features extracted and mapped into a shared taxonomy.

Result: The study finds significant imbalances in provider choice, with LLMs underrepresenting other/academic single-occurrence models by 23-24 percentage points and overrepresenting reused academic/community models by 4-6 percentage points. LLMs also suggest a much narrower range of methods overall.

Conclusion: The study concludes that researchers who rely on LLM suggestions without cross-checking risk narrowing their methodological search space toward a more concentrated default.
...
TL;DR: The paper presents a systematic survey of unified vision-language perception in Multimodal Large Language Models (MLLMs), introducing a five-stage taxonomy and identifying open challenges and research directions.


<details>
  <summary>Details</summary>
Motivation: The lack of systematic surveys examining perception from a unified vision-language perspective, with existing reviews being fragmented and focusing on either vision or language separately.

Method: The authors formalize MLLM perception as a unified vision-language capability, introduce a five-stage taxonomy, and survey representative methods and milestones at each phase.

Result: The survey provides a foundational understanding and an actionable roadmap for further innovation in unified multimodal intelligence, towards artificial general intelligence (AGI).

Conclusion: The study aims to foster further innovation on the path toward artificial general intelligence (AGI) by providing a systematic survey of unified vision-language perception in MLLMs.
...
TL;DR: The neutral tone in Mandarin Chinese is analyzed as a lexical tone with its own phonetic realization and word-specific pitch signatures, similar to the four lexical tones.


<details>
  <summary>Details</summary>
Motivation: The study aims to investigate the properties of the neutral tone in Mandarin Chinese, which has been described as a reduced or toneless tone, and to determine if it is a lexical tone.

Method: A corpus-based study using spontaneous conversational speech corpora of Beijing Mandarin and Taiwan Mandarin, analyzing the phonetic realization of the neutral tone and its pitch contours.

Result: The neutral tone has its own tonal target and word-specific pitch signatures, which are predictable to some extent from words' contextualized embeddings, and is proposed to be a lexical tone in both Taiwan Mandarin and Beijing Mandarin.

Conclusion: The study provides evidence that the neutral tone is a lexical tone in Mandarin Chinese, with similarities and differences in its realization between Taiwan Mandarin and Beijing Mandarin.
...
TL;DR: Small language models can assist with title and abstract screening for large systematic reviews, augmenting human reviewers and making the process more efficient.


<details>
  <summary>Details</summary>
Motivation: The rapid growth of social-physical human-robot interaction research has led to a need for scalable review practices, but fragmented terminology and inconsistent methodologies make systematic synthesis difficult.

Method: The study evaluated the performance of small language models (SLMs) with less than 1.5B parameters in assisting with title and abstract screening for a large systematic review of spHRI literature.

Result: SLMs were able to screen papers much faster than human reviewers, and an ensemble of SLMs identified 39 papers that human reviewers missed, representing 10.29% of the final relevant dataset.

Conclusion: SLMs can augment, rather than replace, expert reviewers and make large-scale literature reviews more accessible and sustainable.
...
TL;DR: The authors present PROFILEFOUNDRY, a synthetic data generator for creating realistic, consistent, and controllable person objects for evaluating foundation models, addressing the challenge of accessing real user data.


<details>
  <summary>Details</summary>
Motivation: The need for realistic and consistent synthetic data about people for foundation-model research, due to difficulties in sharing and using real user data responsibly.

Method: The authors developed PROFILEFOUNDRY, a deterministic generator that creates 100,000 synthetic adult Person Objects with various attributes and relationships, ensuring cross-field and temporal consistency.

Result: The PROFILEFOUNDRY release includes 709,228 events, 40,338 households, 52,491 employers, and 518,564 directed relationship edges, with evidence of its validity provided through various checks and comparisons.

Conclusion: PROFILEFOUNDRY provides a responsible synthetic source layer for constructing downstream foundation-model evaluations, allowing for inspectable and controllable data while maintaining realism and consistency.
...
TL;DR: The paper introduces ConflictScore, a metric to evaluate how well a model's response acknowledges conflicting evidence in its grounding documents.


<details>
  <summary>Details</summary>
Motivation: Existing metrics fail to capture situations where both supporting and contradicting evidence coexist, and the authors aim to address this limitation.

Method: The authors decompose responses into atomic claims, label each claim against each grounding document, and aggregate these labels into two measures: ConflictScore-Count (CS-C) and ConflictScore-Ratio (CS-R).

Result: Experiments show that ConflictScore effectively detects overconfident claims across domains and can serve as a corrective feedback mechanism that improves truthfulness on TruthfulQA.

Conclusion: The paper concludes that ConflictScore is a useful metric for evaluating a model's ability to acknowledge conflicting evidence, and it has the potential to improve the truthfulness of model responses.
...
TL;DR: ProvenAI is a framework that evaluates the transparency of retrieval-augmented systems in multi-hop question answering, breaking it down into three layers: answer correctness, citation fidelity, and per-document influence.


<details>
  <summary>Details</summary>
Motivation: Current retrieval-augmented systems present citations with generated answers, but a citation doesn't necessarily mean the source shaped the output. The authors aim to improve transparency in these systems.

Method: ProvenAI uses a seven-stage pipeline to evaluate 7,405 validation examples from a corpus of 509,300 passages. It measures answer accuracy, citation fidelity, and per-document influence under leave-one-resource-out intervention.

Result: The system achieves 53.53% answer accuracy, a mean citation-fidelity score of 71.55%, and reveals a 'citation-influence gap' where cited sources have weak influence while uncited sources significantly impact the output.

Conclusion: ProvenAI establishes that meaningful transparency in retrieval-grounded QA requires traceable links across retrieved, cited, and behaviourally influential evidence as three distinct, independently measured layers.
...
TL;DR: A lightweight on-device model, AnySimLite, is proposed to solve multiple Speech-Adjacent classification tasks through a text similarity formulation, achieving state-of-the-art performance with a low memory footprint.


<details>
  <summary>Details</summary>
Motivation: To address the challenges of deploying multiple specialized models for natural language classification on edge devices, which creates a memory footprint challenge, and to minimize privacy concerns and inference latency.

Method: AnySimLite, a lightweight similarity encoder that combines word-level and character-level channels, along with a dataset transformation strategy.

Result: AnySimLite consistently achieves state-of-the-art or SOTA-competitive performance in few-shot settings while maintaining a low memory footprint, with a performance drop of below 7% compared to SOTA qLLaMA_LoRA-7B baseline, using less than 1/250th of the model size.

Conclusion: AnySimLite offers a viable solution for on-device natural language classification tasks, balancing performance and memory efficiency.
...
TL;DR: The paper proposes SOLAR, a method to improve multilingual large language models by aligning soft-token representations across languages, leading to improved accuracy and cross-lingual similarity.


<details>
  <summary>Details</summary>
Motivation: Multilingual large language models produce inconsistent reasoning and answers for semantically equivalent prompts in different languages due to language-specific lexical choices.

Method: The authors propose SOLAR, an auxiliary objective for supervised fine-tuning that aligns soft-token representations across languages using English as a pivot.

Result: SOLAR improves accuracy by up to +17.7 points over the base model and +3.8 over standard supervised fine-tuning, especially for low-resource languages.

Conclusion: Aligning soft-token representations helps preserve shared semantic structure during multilingual reasoning, leading to improved performance and cross-lingual similarity.
...
TL;DR: The paper addresses the challenge of extracting problem and method sentences from scientific papers due to small-scale datasets. It proposes FE desensitization-based data augmenters and a context-enhanced transformer to improve model performance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the extraction of problem and method sentences from scientific papers, which is hindered by small-scale datasets that limit model generalization capabilities.

Method: The paper introduces FE desensitization-based data augmenters to generate synthetic data and reduce model reliance on formulaic expressions. It also proposes a context-enhanced transformer to utilize context and reduce noise. Additionally, it explores LLM-based in-context learning (ICL) methods.

Result: The proposed models achieve higher macro F1 scores on two scientific paper datasets, with improvements of 3.71% and 2.67%, respectively. However, LLM-based ICL methods are found not suitable for the task.

Conclusion: The paper concludes that the proposed approaches can effectively improve the extraction of problem and method sentences from scientific papers, addressing the limitations caused by small-scale datasets.
...
TL;DR: The paper explores using EEG signals to improve automatic keyphrase extraction (AKE) on microblogging platforms, finding that EEG features consistently improve AKE performance, with EEG and eye-tracking features providing partial complementarity.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenges of automatic keyphrase extraction (AKE) on microblogging platforms, which generate massive amounts of short, noisy, and dispersed user content, and to investigate whether EEG signals can complement eye-tracking signals for AKE.

Method: The method involves using the ZuCo cognitive language processing corpus to select EEG features and eye-tracking features, which are then incorporated into microblog-based AKE models using a soft-attention layer and self-attention layer.

Result: The results show that cognitive signals produced during reading consistently improve AKE performance, regardless of feature combinations and model architectures, with EEG features bringing the largest gains.

Conclusion: The conclusion is that EEG signals provide useful cognitive evidence for microblog-based AKE and that multimodal cognitive signals deserve further investigation.
...
TL;DR: The authors propose TempoWave, a novel interface that enables large language models (LLMs) to better handle numerical time series forecasting by transforming scalar observations into digit-wise embeddings, leading to improved forecasting performance.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the misalignment between the discrete tokenization of LLMs and continuous numerical values in time series forecasting, which can harm numerical ordering and forecasting reliability.

Method: The authors propose a plug-and-play temporal wavelet digit interface called TempoWave, which maps scalar observations into digit-wise embeddings constructed from multi-wavelet, multi-scale coefficients, allowing for precise numerical formatting and robustness to normalization operations.

Result: The experiments demonstrate that TempoWave consistently improves LLM-based forecasters over standard numeric tokenization and alternative embedding interfaces, achieving a new state-of-the-art across five context-enriched forecasting benchmarks.

Conclusion: The authors conclude that the numeric interface is a key bottleneck in LLMs for time series forecasting and that principled multi-resolution embeddings can better couple LLMs' contextual reasoning with precise forecasting, highlighting the potential of TempoWave for improving forecasting performance.
...
TL;DR: This paper compares two approaches for classifying sentences in German-language climate news articles as threat-oriented, solution-oriented, or both, using a large language model and a fine-tuned BERT model.


<details>
  <summary>Details</summary>
Motivation: To enable automated detection of framing patterns in climate news articles, which is crucial for analyzing large corpora and understanding public perceptions of climate change.

Method: The paper compares two approaches: (1) few-shot prompting with an open-weights large language model (Llama 4 Maverick) and (2) fine-tuning a German BERT model (deepset/gbert-large) for sentence-pair classification.

Result: The fine-tuned BERT classifiers achieve an F1 score of 0.83 for both threat and solution tasks, while the LLM-based classifiers reach an F1 of 0.78.

Conclusion: The results contribute to the comparison of fine-tuned encoder models with prompted generative models for text classification in computational social science, and providing context improves BERT classification performance.
...
TL;DR: The paper introduces TwoTower, a novel diffusion language model that separates context representation and iterative denoising into two towers, achieving a balance between generation quality and throughput.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the limitation of existing diffusion language models that use a single network for both context representation and iterative denoising, which restricts their capacity for either role.

Method: The authors propose a block-wise autoregressive diffusion model called TwoTower, which consists of a frozen autoregressive context tower and a trainable diffusion denoiser tower with bidirectional block attention.

Result: The proposed model, Nemotron-TwoTower, retains 98.7% of the autoregressive baseline's quality while offering 2.42X higher wall-clock generation throughput.

Conclusion: The paper concludes that TwoTower is a promising approach for diffusion language models, achieving a balance between generation quality and throughput, and the code and model weights are made available for further research.
...
TL;DR: The paper introduces MemStrata, a retrieval memory system that maintains temporal validity of facts, addressing the limitation of Retrieval-augmented generation (RAG) in handling changing knowledge.


<details>
  <summary>Details</summary>
Motivation: RAG has no model of time, leading to retrieving both stale and current values with similar embedding similarity, causing agents to serve superseded facts.

Method: MemStrata stores facts like RAG but uses a deterministic supersession rule to retire stale values in a bi-temporal ledger, achieving temporal validity without similarity thresholds or LLM calls.

Result: MemStrata outperforms RAG on evolving knowledge benchmarks, achieving 0.95-1.00 accuracy, and reduces stale-fact-error rate to ~0%, compared to RAG's 15-40%, at a lower retrieval latency of ~2.1s.

Conclusion: MemStrata effectively addresses the structural problem of RAG in handling changing knowledge, providing a more accurate and efficient retrieval memory system.
...
TL;DR: This paper proposes a new approach to analyze narrative disclosures in corporate reports over time, applying it to evaluate Japan's 2019 disclosure reforms.


<details>
  <summary>Details</summary>
Motivation: The challenge of comprehensively evaluating qualitative changes in narrative disclosures over time due to their multidimensional nature.

Method: A longitudinal text analysis approach combining NLP metric extraction, paired testing, shift function analysis, and inter-metric correlation, including a cross-section relevance indicator.

Result: The analysis of 19,770 firm-year observations reveals complex shifts in disclosure patterns, including increased disclosure volume but decreased readability, and varied adaptation across market segments.

Conclusion: The proposed approach provides a more comprehensive understanding of changes in narrative disclosures over time, highlighting the complexity of these changes that may be masked by single-indicator methods.
...
TL;DR: Researchers found that AI models can be blind to safety-critical signals when conditioned on a narrow task, similar to human inattentional blindness.


<details>
  <summary>Details</summary>
Motivation: The study aims to evaluate AI safety by examining how reliably models detect hazards and identifying potential limitations in current evaluation methods.

Method: The researchers tested language and vision models across various tasks, including radiology and driving scenarios, and evaluated their ability to report safety-critical signals.

Result: The study found that conditioning models on a narrow task suppresses their reporting of co-present safety-critical signals, a phenomenon termed the Inattentional Gap.

Conclusion: The Inattentional Gap decouples measured benchmark safety from real-world safety, highlighting the need for more comprehensive evaluation methods to ensure AI safety.
...
TL;DR: The authors propose DiARC, a method to improve model performance on ARC-like tasks by constructing preference pairs with negative samples, leading to consistent improvements over baseline models.


<details>
  <summary>Details</summary>
Motivation: Current methods for solving ARC-like tasks rely heavily on data augmentation and supervised fine-tuning, but neglect the importance of distinguishing negative samples, which is crucial for improving model reasoning.

Method: The authors propose DiARC, a method that constructs preference pairs to enable the model to distinguish between positive and negative samples, using three ways to construct negative samples: output-level visual transformations, DSL-level rule inversion, and task-specific rule editing.

Result: Experimental results across multiple ARC-like benchmarks show that DiARC consistently improves performance over baseline models.

Conclusion: The authors conclude that solving ARC-like problems requires not only positive sample supervision but also the ability to improve model reasoning by distinguishing negative samples, and that DiARC is an effective approach to achieve this.
...
TL;DR: The paper proposes Erase-then-Delta Attention (EDA), a memory update rule that decouples erase and write operations in recurrent memory models, improving their ability to manage memory.


<details>
  <summary>Details</summary>
Motivation: Current memory update rules, such as delta-rule linear attention, can correct information at the current write address but cannot actively remove stale information stored at different addresses.

Method: The proposed EDA method first applies a targeted erase step along a learned erase direction and then performs a standard delta-style corrective write along the current write direction.

Result: Experiments on language-model pretraining show that EDA performs best in both dense and MoE model settings, with gains persisting after long-context midtraining and evaluations.

Conclusion: The results suggest that recurrent memory models should decide not only what to write but also what stale information to erase and where, and that EDA improves memory management in these models.
...
TL;DR: The authors propose KIRP, a zero-shot stance detection framework that integrates external knowledge with entity reorganization and prompt chaining for reasoning, achieving state-of-the-art performance on several datasets.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the challenges of context sparsity in short texts and establishing relevance between implicit targets and textual content in zero-shot tweet-level stance detection.

Method: The authors propose KIRP, a framework that uses entity reorganization for data augmentation, prompt chaining for reasoning, knowledge graphs to supplement and reorganize key textual entities, reflective Chain-of-Thought (CoT) reasoning, stance-aware contrastive learning, and a three-layer iterative prototype network for fine-grained classification.

Result: KIRP achieves state-of-the-art performance on SemEval-2016, WT-WT, and KIRP-D datasets, with F1 scores of 84.05% (three-class) on SemEval-2016, and 84.99% and 79.18% (four-class) on WT-WT and KIRP-D, respectively.

Conclusion: The authors conclude that KIRP effectively addresses the challenges of zero-shot tweet-level stance detection by integrating external knowledge with entity reorganization and prompt chaining for reasoning.
...
TL;DR: The study addresses the quality gap in text-to-speech (TTS) models for languages with limited training data, focusing on Khmer and Korean. It uses a 2.4B-parameter TTS model, VoxCPM2, and adapts it with a single Low-Rank Adaptation (LoRA) adapter for both languages. The adapter significantly improves the Khmer Mean Opinion Score (MOS) from 3.85 to 4.23, but shows no gain for Korean, which the base model already handles well.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this study is to bridge the quality gap in TTS models for languages that are underrepresented in their training data, such as Khmer and Korean, compared to well-resourced languages.

Method: The study utilizes VoxCPM2, a 2.4B-parameter, tokenizer-free TTS model. It creates a shared, language-tagged corpus of about 26 hours and adapts VoxCPM2 with a single Low-Rank Adaptation (LoRA) adapter. This adapter is trained on both Khmer and Korean simultaneously and is added to both the language model and the decoder. The adapter is zero-initialized, allowing training to start from the original (zero-shot) model.

Result: The adaptation results in a significant improvement for Khmer, with the Mean Opinion Score (MOS) increasing from 3.85 to 4.23 (a highly significant gain, paired Wilcoxon test, p<0.001) using the best adapter (rank 64). This improvement is achieved by training only 0.19 to 3.03 percent of the parameters. However, for Korean, the same adapter brings no gain, and at a high rank, it even degrades quality. The study also finds that while the automatic loss and human ratings agree on the improvement for Khmer, they disagree on the best rank for the adapter, with validation loss lowest at rank 128 but MOS peaking at rank 64.

Conclusion: The study concludes that adaptation helps mainly where the base model is genuinely weak, as seen with Khmer. For languages that the base model already handles well, like Korean, adaptation may not provide benefits and could even degrade quality at higher ranks.
...
TL;DR: CAT-Q is a post-training quantization scheme for compressing and accelerating large language models (LLMs) into ternary models with minimal performance degradation.


<details>
  <summary>Details</summary>
Motivation: Existing ternary quantization methods require data-intensive and costly quantization-aware training, which can be impractical for large LLMs. The authors aim to develop a simple and effective post-training quantization scheme.

Method: CAT-Q consists of two key components: learnable modulation (LM) and softened ternarization (ST). LM modulates the distribution of pre-trained weights and the ternary threshold, while ST introduces a differentiable transition function to guide the ternarization process.

Result: CAT-Q can efficiently quantize pre-trained LLMs with 1.7B to 8B parameters into ternary models using only 512 calibration samples, achieving superior performance than state-of-the-art methods. It can also quantize much larger LLMs with 14B to 235B parameters within a short time frame.

Conclusion: CAT-Q is a cost-efficient and accurate ternary quantization scheme that can compress and accelerate LLMs with diverse architectures and model sizes, achieving superior performance with minimal training tokens.
...
TL;DR: The paper introduces SocialPersona, a benchmark for evaluating multimodal large language models' ability to infer user preferences from social media timelines and use them in dialogue.


<details>
  <summary>Details</summary>
Motivation: The authors aim to advance personalized language-model assistants by enabling them to infer user preferences from multimodal data, rather than just relying on explicit statements.

Method: The authors create a benchmark called SocialPersona, which consists of longitudinal social-media timelines from 171 users, along with human-verified preference tags across seven interest domains.

Result: Experiments show that multimodal large language models can identify broad interest domains, but their performance drops on fine-grained and recent interests, and degrades further when using inferred profiles to personalize dialogue.

Conclusion: The authors conclude that robust cross-modal, long-horizon user modeling remains a key challenge, and that SocialPersona can help measure and advance progress toward assistants that infer and act on revealed preferences.
...
TL;DR: The study investigates if combining logical structures with linguistic cues improves automated fallacy classification using Large Language Models (LLMs), achieving significant improvements over baselines.


<details>
  <summary>Details</summary>
Motivation: Logical fallacies contribute to information disorder and are challenging to classify automatically due to their nuanced forms.

Method: The study develops a framework that uses LLMs to inductively extract patterns from fallacious examples and their explanations, merging abstract logical structures with context-level linguistic cues.

Result: The approach shows statistically significant improvements over zero-shot baselines and outperforms competing methods, with cross-dataset experiments validating its generalization.

Conclusion: The study concludes that data-driven pattern extraction is an effective method for generating logical representations, improving fallacy classification.
...
TL;DR: The paper presents ConvMemory v3, which adds a validity context layer to optimize conversational memory retrieval by detecting and surfacing update evidence.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the issue of retrieved memories being outdated and to optimize conversational memory retrieval.

Method: The method involves a dual-evidence gate that uses target-conditioned relation verification, MiniLM slot head, and DeBERTa-v3 slot head to score (target, source) pairs and gate them by conservative event/operation evidence.

Result: The results show that the gate achieves 90.12% +/- 1.73 accuracy on a synthetic multi-hop validity benchmark and 98.8% +/- 0.9 group-all-correct on Memora role binding with zero target-side labels.

Conclusion: The conclusion is that the deployed layer preserves retrieval by default, and the query-conditioned demote mode improves current-active H@1 from 45.1% to 95.7% +/- 1.2 while protecting non-superseded memories at 99.4% recall.
...
TL;DR: The paper analyzes evaluation pitfalls in multimedia event extraction, identifying issues with data processing, task assumptions, and evaluation settings, and advocates for more rigorous evaluation standards.


<details>
  <summary>Details</summary>
Motivation: The reliability and comparability of results in multimedia event extraction depend on consistent and rigorous evaluation, but recent work has reported issues with evaluation methods.

Method: The authors conduct a systematic analysis of evaluation pitfalls and perform controlled experiments under a strict evaluation framework to demonstrate the impact of minor evaluation choices on performance.

Result: The study finds that minor evaluation choices can cause large performance variations and lead to overestimation of a model's ability to ground real-world events across modalities.

Conclusion: The paper highlights the need for comparable evaluation standards and encourages a shift toward more rigorous evaluation in multimedia event extraction.
...
TL;DR: The authors propose OPID, a framework for on-policy skill distillation in reinforcement learning for language agents, which extracts supervision from completed on-policy trajectories and improves agent performance, sample efficiency, and robustness.


<details>
  <summary>Details</summary>
Motivation: Outcome-based reinforcement learning suffers from sparse rewards, while on-policy self-distillation requires external skill memories or privileged context, which are costly and may not match the current policy's state distribution.

Method: The authors propose OPID, which extracts hierarchical skills from completed on-policy trajectories and uses a critical-first routing mechanism to select skills for injection into the interaction history, enabling token-level self-distillation.

Result: Experiments on ALFWorld, WebShop, and Search-based QA demonstrate that OPID improves agent performance, sample efficiency, and robustness over outcome-only RL and existing skill-distillation baselines.

Conclusion: The proposed OPID framework provides a stable optimization backbone for language agents by preserving RL as the primary training objective while introducing dense, distribution-matched hindsight supervision.
...
TL;DR: This paper presents a computational study of vocabulary relationships across eight tradition layers of Bengali and Sanskrit devotional literature, providing quantitative evidence of Buddhist-Shakta syncretism in Bengal.


<details>
  <summary>Details</summary>
Motivation: To quantify the historically argued claim that Buddhist Vajrayana vocabulary survived the collapse of the Pala monasteries and was absorbed into the Shakta Tantra tradition of Bengal.

Method: Using a corpus of 75 texts and TF-IDF character n-gram vectorization with cosine similarity analysis.

Result: The study found a significant contrast in cosine similarity between Buddhist-Shakta and Vaishnava traditions, and provided measurable evidence of lexical transition within the Buddhist-Shakta transmission chain.

Conclusion: The results provide the first quantitative multi-tradition corroboration of historically argued Buddhist-Shakta syncretism in Bengal, demonstrating that Buddhist vocabulary was absorbed into the Shakta Tantra tradition.
...
TL;DR: The paper presents a submission to the IWSLT 2026 Instruction Following shared task, focusing on developing SpeechLLMs for short-form and long-form speech instruction following. It achieves strong performance on both tracks, with a SIFS score of 2.0708 on the short track and a HIFS score of 2.0663 on the long track.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to develop SpeechLLMs that can effectively follow instructions in both short-form and long-form speech under constrained settings, as part of the IWSLT 2026 Instruction Following shared task.

Method: The method involves developing SpeechLLMs for both short-form and long-form speech instruction following. For the short track, it focuses on achieving strong performance on MCIF. For the long track, it explores three speech segmentation methods and introduces the HIFS score to account for unstable long-form generation. The approach includes analyzing hallucination in generated outputs.

Result: The results show that for the short track, a SIFS score of 2.0708 is achieved. For the long track, fixed 30-second segmentation provides the most robust long-form performance, achieving the highest HIFS score of 2.0663. The analysis also reveals that hallucination mainly manifests as repetitive insertions in generated outputs.

Conclusion: The conclusion is that SpeechLLMs can be effectively developed for both short-form and long-form speech instruction following, with strong performance achieved on both tracks. The study highlights the importance of addressing hallucination, particularly repetitive insertions, in long-form generation.
...
TL;DR: A new pruning framework for large language models on IIoT edge devices achieves high compression ratios while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Deploying large language models on IIoT edge devices requires extreme compression, but existing pruning methods struggle with high compression ratios.

Method: A cascaded multi-granularity pruning framework that removes layers, attention heads, and feed-forward channels in a coarse-to-fine order.

Result: The framework achieves 13.8 times compression on MHA+GELU architectures with 83.82% accuracy and reduces inference latency by up to 67.2% and peak memory by 62.5%.

Conclusion: The proposed framework enables viable IIoT edge inference by achieving high compression ratios while maintaining accuracy.
...
TL;DR: The paper introduces InfoKV, a framework for compressing key-value (KV) cache in large language models (LLMs) by incorporating information-theoretic signals, such as predictive uncertainty, to improve long-context reasoning.


<details>
  <summary>Details</summary>
Motivation: Existing KV cache compression methods rely on attention weights to estimate token importance, but overlook complementary information-theoretic signals related to predictive uncertainty and token informativeness.

Method: The proposed InfoKV framework combines token-level predictive uncertainty with layer-wise representation evolution and integrates the resulting entropy scores with attention scores during reasoning.

Result: Experiments on long-context reasoning benchmarks with Llama-3.1, Llama-3.2, and DeepSeek-R1 demonstrate that InfoKV consistently outperforms existing attention-based KV compression methods in both long prefilling and decoding scenarios.

Conclusion: The paper concludes that InfoKV is an effective framework for KV cache compression, which can improve the reasoning capability of LLMs in long-context scenarios.
...
TL;DR: Language model representations can predict neural activity during natural speech and text comprehension.


<details>
  <summary>Details</summary>
Motivation: To investigate whether language-model-derived quantities can annotate neural activity during natural speech and text comprehension.

Method: Analyzing locked derived data from Brain Treebank, MEG-MASC, and Podcast ECoG with eight frozen language models, blocked encoding models, and matched controls.

Result: 67 of 432 evaluable rows met a controlled predictive-only criterion, and model-side feature ablations changed prediction scores in most evaluable source rows.

Conclusion: Language-model-derived quantities can serve as useful neural predictors, but separate predictive usefulness from claims about shared neural organization or language-processing computations.
...
TL;DR: The study evaluates the reliability of Automatic Speech Recognition (ASR) models in the Indian healthcare context, particularly in multilingual and demographically diverse settings. It assesses eight state-of-the-art models across Kannada, Hindi, and Indian English, revealing variability in performance and proposing a debiasing technique, SamaVaani, to improve fairness and ASR performance.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this study is to investigate the reliability of ASR in the Indian healthcare context, which is multilingual and demographically diverse, and to address potential biases in ASR models that could affect their equitable deployment in clinical settings.

Method: The study conducts a systematic audit of ASR performance using real-world psychiatric interview data in Kannada, Hindi, and Indian English. It compares eight state-of-the-art models and further fine-tunes the best-performing open-source models (Gemma3n and OmniLingual) using various methods. The study also proposes a unified debiasing technique called SamaVaani.

Result: The results show substantial variability across models and languages, with some systems performing well in Indian English but poorly in regional speech. The study also identifies systematic performance gaps tied to speaker role and gender, which can be mitigated by fairness-aware fine-tuning and the proposed SamaVaani technique.

Conclusion: The study concludes that while ASR models show potential for documenting clinical encounters, their reliability varies significantly across languages and demographic groups. The proposed SamaVaani technique offers a solution to improve both ASR performance and fairness across groups, which is crucial for the equitable deployment of ASR in clinical settings.
...
TL;DR: The paper introduces GAVEL, a task for verifying, explaining, and localizing misalignments between text and images in vision-language models, along with a dataset and benchmark.


<details>
  <summary>Details</summary>
Motivation: Vision-language models often produce hallucinated or inconsistent outputs where text and images are not properly aligned, and there is a need to detect, explain, and localize these discrepancies.

Method: The authors introduce GAVEL, a task that jointly addresses verification, explanation, and localization for image-text pairs, and provide a supervised baseline trained on human-annotated data.
...
Conclusion: The paper concludes that GAVEL provides learnable supervision for verifying, explaining, and localizing misalignments in vision-language models, and that the proposed supervised baseline can improve performance on these tasks.
...
TL;DR: A term-centric framework is proposed for inducing hierarchical taxonomies from heterogeneous corpora, improving cross-source coherence and hierarchy quality.


<details>
  <summary>Details</summary>
Motivation: Existing taxonomy induction methods have limitations in generalizing across heterogeneous sources, and the proposed framework aims to address this issue.

Method: The proposed approach uses automatic term extraction to map documents into a shared representation space, and then constructs interpretable hierarchies using data-driven clustering.

Result: Experiments on a large benchmark dataset show that the proposed method improves cross-source coherence and hierarchy quality over baselines, and a case study demonstrates its practical utility.

Conclusion: The proposed term-centric framework effectively induces hierarchical taxonomies from heterogeneous corpora, enabling robust cross-source alignment and interpretable hierarchy construction.
...
TL;DR: The paper surveys safety reporting practices in speech model releases, finding a lack of multilingual analysis, and introduces RedVox, a benchmark for evaluating safety and fairness in speech models across five languages.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the gap in safety and fairness analysis of speech models beyond English settings and under naturalistic conditions.

Method: The authors survey state-of-the-art speech model releases, introduce RedVox benchmark, and evaluate eight state-of-the-art models using it.

Result: The results show that vulnerabilities in speech models persist even under non-adversarial conditions, worsen in non-English languages, and are amplified when the request comes from a spoken input.

Conclusion: The paper concludes that there are broader sociotechnical challenges in naturalistic speech safety research, including unique personal and privacy challenges of collecting speech data with human participants.
...
TL;DR: The study examines how different contextual framings affect the responses of large language models (LLMs) in mental health support tools, finding that framing systematically alters response tendencies and that robustness to contextual variation is crucial for trustworthy AI systems.


<details>
  <summary>Details</summary>
Motivation: The integration of LLMs into mental health support tools and psychologically sensitive applications raises concerns about behavioral stability and consistency, as semantically similar concerns can be presented through different contextual framings, potentially eliciting different model responses.

Method: The study uses controlled matched prompts across multiple contextual framing conditions and model families, employing layer-wise probing analyses, held-out framing probes, and activation steering experiments to investigate the effects of framing on LLMs' internal representations.

Result: The results show that framing systematically alters interpretive response tendencies across architectures, with behavior-associated information remaining decodable throughout transformer depth, and that framing-associated representational directions can partially modulate downstream behavioral outcomes.

Conclusion: The study concludes that robustness to contextual variation is an important consideration when evaluating the consistency and trustworthiness of conversational AI systems deployed in mental-health-oriented interactions.
...
TL;DR: The paper proposes ReaORE, a framework for open relation extraction that uses coarse-to-fine relation reasoning to improve generalization to unseen relation types.


<details>
  <summary>Details</summary>
Motivation: Current open relation extraction approaches have limitations in generalizing to unseen relation types, either through clustering techniques or direct relation label generation via Large Language Models.

Method: The proposed ReaORE framework consists of two stages: relation filtering and relation prediction, using coarse-to-fine relation reasoning to extract relations.

Result: ReaORE outperforms existing baselines on two widely used open relation extraction datasets.

Conclusion: The proposed ReaORE framework effectively addresses the challenge of generalizing to unseen relation types in open relation extraction.
...
TL;DR: The study tests if emotion vectors, found in Claude Sonnet 4.5, exist in two other models, Apertus-8B-Instruct-2509 and Gemma-4-E4B-it. It finds similar valence geometry in both, but with differences in how valence representations emerge across model depth.


<details>
  <summary>Details</summary>
Motivation: To test the generality of emotion vectors across different language models and understand their representation.

Method: Extracting emotion contrast vectors across all layers of two open-weight models using two model-generated corpora.

Result: The study recovers valence geometry in both models with high correlations (r = 0.76 and r = 0.83). It also finds differences in how valence representations emerge across model depth and arousal encoding sensitivity to the extraction corpus.

Conclusion: The findings suggest that emotion vectors are not unique to Claude Sonnet 4.5 and exhibit similarities and differences across language models, providing insights into their representation and emergence.
...
TL;DR: Introduced MinGram, a simplified Unigram tokenizer that improves training efficiency while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: The Unigram tokenizer's complex training process motivated the development of MinGram.

Method: MinGram uses a BPE-derived seed vocabulary, Hard EM on a minimum-token path, and a single flat score-pruning step.

Result: MinGram outperforms BPE and standard Unigram in compression across six languages and shows strong morphological alignment.

Conclusion: MinGram offers a better trade-off between compression and morphological alignment than existing tokenizers.
...
TL;DR: The paper proposes Psy-CoT, a psychology-grounded framework for building role-playing agents, and RAPO, a method for optimizing role-aware policies, achieving better performance on role-playing tasks.


<details>
  <summary>Details</summary>
Motivation: The dominant paradigm of supervised fine-tuning for role-playing agents leads to poor out-of-distribution generalization and lack of deep, human-like internal thought processes.

Method: The paper proposes Psy-CoT, a chain-of-thought framework that decomposes pre-response reasoning into three role-specific steps, and RAPO, a role-aware policy optimization method that uses profile-token mutual information to weight gradients asymmetrically.

Result: Experiments on CoSER, CharacterBench, and CharacterEval demonstrate that Psy-CoT outperforms existing role-playing CoT methods, and RAPO consistently surpasses GRPO across multiple model scales.

Conclusion: The proposed methods, Psy-CoT and RAPO, improve the performance of role-playing agents by incorporating psychology-grounded reasoning and role-aware policy optimization.
...
TL;DR: The paper introduces NuclearQAv2, a benchmark for evaluating large language models (LLMs) in nuclear engineering, and assesses their performance across different task types.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of ensuring reliability of LLMs in highly technical domains, specifically nuclear engineering, where problem-solving requires factual knowledge, quantitative reasoning, and conceptual understanding.

Method: The authors construct NuclearQAv2 using a hybrid pipeline combining expert-authored questions, existing datasets, and LLM-assisted generation. They evaluate LLMs using structured prompting for automated question generation and response evaluation.

Result: The results show substantial performance differences across task types, with LLMs performing well on factual questions but struggling with quantitative reasoning and conceptual understanding.

Conclusion: The paper concludes that NuclearQAv2 is a scalable benchmark for assessing LLM capabilities in technical domains, highlighting the importance of multi-faceted evaluation frameworks.
...
TL;DR: The authors propose a Judge-Aware Gated Multi-Task Learning architecture for legal outcome prediction, which disentangles objective case facts from adjudicative context and outperforms existing methods.


<details>
  <summary>Details</summary>
Motivation: Legal outcome prediction requires distinguishing between objective case facts and adjudicative context, as merit-based rulings rely on factual evidence while technical disposals rely on judicial discretion.

Method: The authors propose a Judge-Aware Gated Multi-Task Learning architecture that uses a fine-grained outcome taxonomy to supervise the encoder and a Gated Fusion mechanism to dynamically modulate reliance on judge identity.

Result: The proposed architecture outperforms supervised fine-tuning (SFT) of a Gemma-4 26B-A4B backbone on a benchmark of 13,937 UK Employment Tribunal decisions, achieving a new state of the art while requiring fewer trainable parameters.

Conclusion: The choice of conditioning interface dominates scale in identity-conditioned classification of legal outcomes, and differentiable structured composition yields more accurate, more parameter-efficient models than prompt-based composition.
...
TL;DR: This study investigates whether large language models (LLMs) can flexibly adapt their reasoning strategies like humans. It introduces a novel paradigm called 'riddle riddles' to test this. Results show LLMs and humans fail on this paradigm in opposite directions, suggesting LLMs' strong performance may be due to memory retrieval rather than flexible reasoning.


<details>
  <summary>Details</summary>
Motivation: The study aims to determine whether LLMs' accuracy on cognitive tasks is due to pattern matching from training data or flexible reasoning. It seeks to understand if LLMs can adapt their reasoning strategies based on the content of a problem, similar to humans.

Method: The study uses a novel paradigm called 'riddle riddles', which are word problems that mimic popular riddles but require literal interpretations. It tests nine state-of-the-art LLMs and 100 human participants across two experiments.

Result: LLMs were more accurate on genuine riddles (84.9%) than on riddle riddles (50.7%), while humans showed the reverse effect (50.5% vs. 80.5%). Error analysis reveals that LLMs' errors on riddle riddles were often due to inappropriate use of inventive reasoning (90.8%), whereas human errors on genuine riddles were due to overextending literal reasoning (57.6%).

Conclusion: The study suggests that LLMs' strong performance on cognitive tasks may not be due to flexible reasoning but rather memory retrieval. It highlights that without specific stimuli designed to test flexible reasoning, it can be easy to conflate LLM-generated outputs that resemble reasoning with genuine reasoning.
...
TL;DR: This paper explores using Large Language Models (LLMs) for forecasting, analyzing their internal states to understand how they make predictions, and intervening on specific features to improve historically grounded reasoning.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enhance forecasting capabilities of LLMs by understanding and improving their reliance on generalizable patterns versus time-specific knowledge.

Method: The method involves applying LLMs to forecasting tasks, analyzing their internal states with sparse autoencoders, and intervening on identified features associated with time-aware and look-ahead-biased reasoning.

Result: The results show that amplifying time-awareness features in LLMs reduces look-ahead bias in forecasting prompts without compromising general reasoning performance.

Conclusion: The conclusion is that interpretable temporal features can be used to causally shift LLMs toward more historically grounded reasoning, improving their forecasting capabilities.
...
TL;DR: The paper proposes a new method to predict processing difficulty in garden path sentences, which outperforms lexical surprisal in fitting human reading time data.


<details>
  <summary>Details</summary>
Motivation: Lexical surprisal fails to predict processing difficulty in garden path sentences, where the sentence prefix leads to an incorrect interpretation.

Method: The authors propose a method that predicts a probability distribution over syntactic trees and updates it after each new word, using a generalized Rényi divergence to measure the magnitude of the update.

Result: The proposed Syntactic Belief Update provides a better fit to human reading time data on garden path sentences.

Conclusion: The study suggests a new research direction examining purely non-lexical alternatives to surprisal for psycholinguistics.
...
TL;DR: The authors propose that safety classifiers should model user intent and introduce AIMS, a dataset to study this. They evaluate intent-aware training methods and find that faithful intent modeling improves safety classifier performance.


<details>
  <summary>Details</summary>
Motivation: The authors argue that safety classifiers should explicitly model user intent to improve performance.

Method: The authors introduce AIMS, a human-annotated dataset, and evaluate intent-aware training across various methods, including supervised fine-tuning, preference learning, reasoning distillation, and reinforcement learning.

Result: The authors find that intent-aware training methods, particularly those directly rewarding intent faithfulness, yield competitive safety classifiers across different training regimes and outperform other methods in most cases.

Conclusion: Faithful intent modeling is a compact, high-quality supervision signal for more robust safety classifiers.
...
TL;DR: A framework integrating formal semantics and evolutionary modeling is introduced, allowing lexical meanings and composition functions to co-evolve, and is applied to the evolution of quantificational meaning.


<details>
  <summary>Details</summary>
Motivation: The literature on semantic universals often models lexicons with fixed signal structures or holistic composition without interpretable lexical parts, contradicting the insight of formal semantics that sentence meanings arise by recursively composing lexical meanings.

Method: A framework that allows lexical meanings and a composition function to co-evolve under pressures for conceptual simplicity and communicative accuracy is introduced and applied to the evolution of quantificational meaning.

Result: The most well-known semantic universal, conservativity, emerges as an efficient system-wide abstraction, and the account helps reconcile tensions between empirical evidence on quantifier learnability and prior evolutionary models.

Conclusion: The framework offers a template for studying universals that involve global compression within a grammatical category, semantic specialization of syntactic arguments, and the co-evolution of lexical and compositional meaning, demonstrating that formal semantics can be productively combined with evolutionary modeling.
...
TL;DR: The paper introduces CARVE, a new recurrent model that improves upon existing models by introducing a content-aware erase gate, achieving better performance and efficiency.


<details>
  <summary>Details</summary>
Motivation: The authors identify three defects in the leading delta-rule architecture (GDN-2): memory-blind gating, waste of parameters, and prevention of the WY-form triangular chunk solver. They aim to resolve these issues.

Method: The authors propose CARVE, which resolves the defects by erasing only on the key axis, reusing the recurrent output tensor as a free content signal for the erase gate, and replacing the per-value write-gate projection with a single scalar per head.

Result: CARVE achieves better performance on WikiText perplexity, common-sense reasoning benchmarks, and RULER retrieval probes, while being more efficient in terms of throughput, peak memory, and number of parameters.

Conclusion: The authors conclude that CARVE is a more efficient and effective recurrent model that achieves state-of-the-art results while reducing computational resources.
...
TL;DR: A conceptual framework for analyzing dialogue in human-AI collaborative problem-solving is proposed, emphasizing emerging dynamics and metacognitive regulation.


<details>
  <summary>Details</summary>
Motivation: Understanding dialogic interaction in collaborative problem-solving is crucial for optimizing human-AI partnerships as intelligent systems become more autonomous and cooperative.

Method: A hierarchical two-layer coding scheme integrating cognitive, non-cognitive, and metacognitive regulatory mechanisms is developed.

Result: The framework demonstrates effectiveness and generalizability across nine datasets, providing insights into human-agent coordination and the importance of metacognitive regulation in deeper collaboration.

Conclusion: The proposed framework offers a comprehensive approach to analyzing dialogue in human-AI collaboration, highlighting the significance of metacognitive regulation in achieving deeper collaboration.
...
TL;DR: Language models encode knowledge in a task-specific manner, not as a unified knowledge base, impacting their reliability and controllability.


<details>
  <summary>Details</summary>
Motivation: The motivation is to investigate whether language models (LMs) behave like a knowledge base, where different queries for the same fact return consistent results.

Method: The study uses behavioral and mechanistic analyses, including parameter localization experiments and examining chain-of-thought reasoning.

Result: The results indicate that LMs encode knowledge in a task-specific manner, with different parameter subsets underlying different tasks for the same fact.

Conclusion: The findings suggest that the 'knowledge base' analogy for LMs is flawed, and what the model knows is intertwined with how it is asked, impacting reliability and controllability.
...
TL;DR: The paper proposes a diagnostic framework to evaluate the performance of large language models on historical texts, identifying challenges in processing historical language and providing insights for digital library applications.


<details>
  <summary>Details</summary>
Motivation: The ability of large language models to process historical language remains poorly understood, and historical difficulty is typically treated as a monolithic barrier.

Method: The authors propose a diagnostic framework that decomposes historical difficulty into four distinct dimensions: tokenization cost, predictive uncertainty (surprisal), semantic robustness, and context sensitivity.

Result: The results reveal a dissociation between encoding cost and comprehension, with 17th-century Italian being 2.4 times more surprising than its modern equivalent, while Russian shows only a modest increase.

Conclusion: The findings suggest that while historical text imposes a consistent encoding tax, digital libraries can safely deploy large language models for semantic retrieval tasks, provided that generative applications are carefully adapted.
...
TL;DR: The paper proposes a context-aware translation cascade for multilingual reasoning, which preserves the original question and reasoning trace to improve translation accuracy.


<details>
  <summary>Details</summary>
Motivation: The translation cascade approach to multilingual reasoning is structurally lossy, discarding information needed for cultural grounding, register, and disambiguation.

Method: The authors propose a simple and training-free intervention: a context-aware translation cascade that provides the original question, English translated question, and reasoning trace to the final translation module.

Result: The proposed approach demonstrates strong gains for open-ended generation across models and resource regimes, with the original language question carrying most of the beneficial context.

Conclusion: The study emphasizes the need to better design information flow in machine translation cascades and provides a simple and actionable default strategy: preserve the original user question until the end of the pipeline.
...
TL;DR: The paper proposes a comprehensive taxonomy of indirect linguistic expressions (ILE) to detect coded language on social media, and evaluates it using LLMs and manually annotated posts.


<details>
  <summary>Details</summary>
Motivation: To improve detection of emerging coded language on social media, which is used to evade moderation and surveillance.

Method: The authors propose a mechanism-oriented taxonomy of ILE, and evaluate it by incorporating it into LLM prompts and comparing with existing taxonomies and a baseline using 2,000 manually annotated posts.

Result: The proposed taxonomy achieves the strongest performance across three LLMs, with an improvement of 4.7% in accuracy and 5.4% in F1 over the best-performing benchmark.

Conclusion: The comprehensive, mechanism-oriented taxonomy provides a stable scaffold for detecting emerging coded language and is a useful input to content moderation.
...
TL;DR: The paper presents a case study on using Large Language Models (LLMs) to verify the eligibility of securities as collateral, improving the efficiency and accuracy of the process.


<details>
  <summary>Details</summary>
Motivation: The manual verification of securities against legal and financial criteria within lengthy prospectuses is resource-intensive and prone to errors, motivating the need for an automated solution.

Method: The authors propose a generative Information Extraction pipeline using LLMs, decomposing the task into extraction, normalization, and interpretation to handle noisy text and interleaved German-English content.

Result: The LLM-based system achieves high precision (up to 91%) in document-level eligibility, with a conservative operating profile that minimizes false acceptance.

Conclusion: The study demonstrates the potential of LLMs to improve the efficiency and accuracy of the eligibility examination process for securities as collateral.
...
TL;DR: The paper introduces PEEU, a method to improve the planning abilities of small multimodal models, and TDHAF, a framework to analyze their generalization behaviors.


<details>
  <summary>Details</summary>
Motivation: Small open-source multimodal models suffer from weak planning and limited cross-website generalization, and the authors aim to address these limitations.

Method: The authors propose the PEEU method, which explores environments to discover experiences and utilizes hindsight experience to synthesize high-level training data.

Result: Experiments on real-world benchmarks demonstrate PEEU's effectiveness, with a 7B model achieving 30.6% accuracy, outperforming the larger Qwen2.5-VL-32B model.

Conclusion: The authors conclude that constructing hindsight high-level tasks and leveraging experiences is crucial for out-of-distribution planning abilities of small multimodal models.
...
TL;DR: This paper presents a new pipeline for extracting entities and relationships from unstructured news corpora in multiple languages, enabling the study of complex social networks and governance structures.


<details>
  <summary>Details</summary>
Motivation: The authors aim to overcome the limitations of manual coding and existing automated methods for analyzing complex, informal, and adversarial ties in comparative politics.

Method: The authors propose a modular pipeline that combines named-entity recognition (NER) with a three-stage linking cascade and a high-throughput mixture-of-experts model to extract directed, signed relationships.

Result: The pipeline achieves high textual correctness (68.2% strict to 93.7% lenient) and is validated through two large-scale case studies in Austria and Poland, demonstrating its ability to reconstruct complex social networks and governance structures.

Conclusion: The proposed pipeline provides a robust, replicable foundation for cross-national empirical computational social science, enabling the study of complex social networks and governance structures at scale.
...
TL;DR: The paper presents a method to generate data for interpreting and controlling language model behaviors by isolating linear features responsible for certain behaviors, focusing on steering away from sycophancy.


<details>
  <summary>Details</summary>
Motivation: The need for reliable detection of model features responsible for certain behaviors, such as sycophancy, to steer models towards or away from these behaviors.

Method: An iterative data generation pipeline that generates samples showing degrees of features that scale linearly with behavior, allowing for better disentanglement of features.

Result: The method allows for detection, deterministic scoring, and robust steering of sycophancy features, matching or outperforming baseline approaches with lower computational demand and more interpretability guarantees.

Conclusion: The proposed method enables more reliable interpretation and control of language model behaviors, specifically in steering away from sycophancy, by isolating linearly separable subspaces of features.
...
TL;DR: The paper challenges the common practice of retiring benchmarks when their accuracy saturates, instead proposing a more comprehensive evaluation framework that assesses multiple dimensions of agent performance.


<details>
  <summary>Details</summary>
Motivation: The current benchmark evaluation paradigm focuses primarily on accuracy, overlooking other crucial aspects of agent performance such as construct validity, generalizability, efficiency, reliability, and human-agent collaboration.

Method: The authors use CORE-Bench Hard as a case study, introducing improved benchmarks (CORE-Bench v1.1 and CORE-Bench OOD) and conducting experiments to measure various dimensions of agent performance beyond accuracy.

Result: The study finds that even after accuracy saturation, CORE-Bench v1.1 remains useful for evaluating efficiency, reliability, model performance, and scaffold performance, and that human-agent collaboration yields a statistically significant speedup in computational reproducibility tasks.

Conclusion: The paper presents a more rigorous alternative to the accuracy-centric evaluation paradigm, highlighting the importance of a multi-faceted approach to assessing agent performance.
...
TL;DR: The study reveals that in instruction-tuned chat models, the persona and refusal mechanisms interact, with a compliant persona gating refusal.


<details>
  <summary>Details</summary>
Motivation: The interaction between persona and refusal traits in instruction-tuned chat models has not been studied previously.

Method: The authors extract and intervene on compliant persona and refusal directions in Qwen2.5-7B-Instruct and Llama-3.1-8B-Instruct models.

Result: Compliant persona steering suppresses refusal, and reintroducing the refusal direction partially restores refusal at late layers.

Conclusion: Refusal is gated at the late-layer expression stage, and treating refusal as a single isolated direction overlooks its dependence on persona.
...
TL;DR: The paper presents AlgoEvolve, an LLM-driven framework for generating and improving executable trading strategies through evolutionary processes, demonstrating emergent adaptive logic and outperforming human-designed instructions.


<details>
  <summary>Details</summary>
Motivation: The motivation is to extend the application of Large Language Models (LLMs) as semantic mutation operators for evolutionary discovery from static coding benchmarks to algorithmic trading, a noisy, non-stationary, and highly discontinuous domain.

Method: The method involves using AlgoEvolve, an LLM-driven evolutionary framework that generates, evaluates, and iteratively improves executable trading strategies expressed as Python code, with a meta-evolutionary outer loop that evolves prompts for program synthesis.

Result: The results show that AlgoEvolve exhibits emergent regime-adaptive strategy logic, including autonomous shifts in trading rules, and that the evolved search heuristics consistently outperform initial human-designed instructions, providing a viable approach for continual program synthesis in complex environments.

Conclusion: The conclusion is that LLM-based semantic evolution provides a viable approach for continual program synthesis in complex environments, such as algorithmic trading, by demonstrating the ability to generate adaptive trading strategies that outperform human-designed instructions.
...
TL;DR: The authors introduce a pipeline for analyzing governance discourse in AI agent protocols and apply it to two contrasting standards, finding that while governance form influences thematic priorities, both exhibit similar levels of participation inequality and community fragmentation.


<details>
  <summary>Details</summary>
Motivation: The governance structures shaping AI agent protocols' interoperability standards are empirically underexamined, and the authors aim to address this gap by introducing a large-scale governance discourse analysis pipeline.

Method: The authors use an LLM-powered comparative pipeline, integrating automated annotation, neural topic modeling, and multi-layer network analysis to study socio-technical power structures at scale.

Result: The authors find that while governance form influences substantive focus, both regimes exhibit comparable levels of participation inequality and community fragmentation, and discourse alignment is denser in the permissionless setting.

Conclusion: The findings illustrate how LLM-assisted methods can advance the empirical study of technology governance, with implications for designing more equitable agentic AI standards.
...
TL;DR: The authors propose a framework to integrate online patient narratives and regulatory adverse-event records to improve psychiatric medication safety information.


<details>
  <summary>Details</summary>
Motivation: Patients seek medication information online, but existing sources are either authoritative but abstract (regulatory records) or experiential but unvalidated (patient narratives).

Method: The authors developed a provenance-aware, knowledge-graph-based multi-agent framework, integrating Reddit posts, WebMD reviews, and FDA Adverse Event Reporting System records for nine antidepressants.

Result: The framework successfully unified multiple data sources, preserving provenance and distinguishing regulatory facts from patient experiences. Patient-generated data formed a partly independent safety signal.

Conclusion: The proposed framework enables source-aware integration of psychiatric medication information, potentially leading to more auditable and useful safety information for patients.
...
TL;DR: A novel rating system, DD-Elo, is proposed to improve the traditional Elo system by incorporating move-by-move data, providing a more responsive and explainable skill assessment framework for competitive chess.


<details>
  <summary>Details</summary>
Motivation: The traditional Elo rating system suffers from response lag due to its reliance on match outcomes, neglecting the granular quality of gameplay. The authors aim to address this limitation by incorporating move-by-move information into rating adjustments.

Method: The authors propose the Drift-Diffusion-Enhanced Elo Rating System (DD-Elo), which models skill expression as a decision-making process using the drift diffusion model (DDM) from cognitive neuroscience. The DD-Elo system integrates move-level data to capture rapid skill fluctuations.

Result: The DD-Elo system adapts to skill changes faster than the traditional Elo system, while maintaining a bounded deviation from it. The authors provide extensive experimental results to support their claims.

Conclusion: The DD-Elo system offers an explainable, highly responsive, and backward-compatible solution for chess rating ecosystems, providing a more accurate and efficient way to assess player skills.
...
TL;DR: The paper proposes a computational governance model for AI agents, allowing them to operate autonomously while ensuring safety and accountability through independent evidence and cryptographic binding.


<details>
  <summary>Details</summary>
Motivation: The increasing autonomy of AI agents and their potential to perform consequential actions without human oversight, highlighting the need for a governance model that balances autonomy with safety and accountability.

Method: The proposed model involves separating planning and execution authority, requiring independent attestation of preconditions for high-risk actions, and using cryptographic binding and tamper-evident logs to ensure accountability.

Result: A proof-of-concept implementation of the proposed model, along with examples of its application in software deployment and clinical prescribing, demonstrating its feasibility and potential effectiveness.

Conclusion: The paper concludes that the proposed computational governance model can ensure the safe and accountable operation of autonomous AI agents, while maintaining their autonomy in planning and reasoning.
...
TL;DR: This paper presents COrigami, an AI-driven pipeline for computational origami that generates crease patterns from natural language, assisting human artists in designing physical art that meets geometric constraints and aesthetic standards.


<details>
  <summary>Details</summary>
Motivation: The challenge of generating physical art that satisfies both strict geometric constraints and subjective visual aesthetics in computational origami.

Method: COrigami, an end-to-end AI-driven pipeline involving: generating a semantic stick figure, computing a base packing, solving for a flat-foldable crease pattern, shaping the flat-folded crease pattern, and refining the generated model using reinforcement learning.

Result: COrigami acts as a highly effective collaborative assistant, generating structural starting points that human artists can further expand and shape, demonstrating reliable, mathematically grounded co-creativity.

Conclusion: The proposed approach enables AI systems to satisfy multi-objective physical constraints, allowing for reliable co-creativity between humans and AI in computational origami.
...
TL;DR: The paper discusses the challenges of verifying solutions generated by coding agents, arguing that verification is becoming increasingly difficult as generating complex solutions becomes easier. It proposes characterizing verification signals along three dimensions and studies four reward constructions to address these challenges.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the growing difficulty of verifying solutions generated by coding agents, as the ability to generate complex solutions improves. The authors aim to understand the challenges of verification and propose potential solutions.

Method: The authors characterize the quality of verification signals along three dimensions: scalability, faithfulness, and robustness. They then study four reward constructions: a test verifier, a rubric verifier, the user as verifier, and an automated agent verifier. They conduct experiments to analyze the core challenges of reward design and how to effectively leverage reward signals.

Result: The experiments show that targeted verification design can effectively suppress reward hacking, improve task completion quality, and achieve significant gains across multiple internal and public benchmarks. The authors conclude that no fixed reward function can remain effective as policy capability continues to grow, and verification must co-evolve with the generator.

Conclusion: The paper concludes that verification is a critical challenge in the development of coding agents, and that it must co-evolve with the generator. The authors argue that achieving scalable, faithful, and robust verification signals is essential, and that targeted verification design can lead to significant improvements in task completion quality.
...
TL;DR: The paper presents an empirical study on using tool-augmented LLM agents for energy market analytics tasks, evaluating their performance on real-world problems.


<details>
  <summary>Details</summary>
Motivation: The energy sector requires live data retrieval, specialized knowledge, and multi-step quantitative reasoning, but current evaluations are limited to static knowledge recall.

Method: The study uses a configurable suite of domain tools, including live APIs and databases, to evaluate LLM agents on 243 expert-curated problems across three categories.

Result: The study provides a comparative analysis of closed-source and open-source LLMs, assessing their performance using a multi-dimensional evaluation protocol.

Conclusion: The paper highlights the importance of tool-augmented LLM agents in the energy sector and provides a framework for future research and reproducibility.
...
TL;DR: The evaluation methods and benchmarks for multimodal large language models (MLLMs) are insufficient, failing to assess their ability to integrate information across modalities.


<details>
  <summary>Details</summary>
Motivation: The rapid advancement of MLLMs' capabilities has not been matched by progress in their evaluation, with current benchmarks limited to isolated tasks and not assessing multimodal integration.

Method: The authors examine current evaluation means and review existing benchmark taxonomies to identify gaps.

Result: The identified gaps in evaluation include temporal-spatial coherence, physical world understanding, multimodal consistency, and selective attention.

Conclusion: Addressing these gaps is essential for accurately measuring progress in multimodal intelligence and understanding the capability boundaries of MLLMs.
...
TL;DR: The authors propose OpenFinGym, a unified environment for developing and evaluating quantitative finance agents, addressing the limitations of current fragmented evaluation methods.


<details>
  <summary>Details</summary>
Motivation: Current evaluation methods for large language model agents in quantitative finance are fragmented and often overlook financial relevance, failing to reveal weaknesses in generalization and real-market interaction.

Method: The authors introduce OpenFinGym, a unified gym environment that covers multiple tasks such as forecasting, market generation, trading, and fraud detection, providing an automated task-construction pipeline and a containerized runtime.

Result: OpenFinGym provides a comprehensive platform for developing and evaluating quantitative finance agents, enabling more accurate assessments of agent competence and generalization.

Conclusion: OpenFinGym offers a unified solution for quantitative finance agent development and evaluation, addressing the need for more comprehensive and realistic assessments of agent performance.
...
TL;DR: The paper identifies a recurring failure mode in prompt-composed agentic systems called compositional behavioral leakage (CBL), where editing one prompt module affects the behavior of others, and proposes a reusable protocol to measure and evaluate CBL.


<details>
  <summary>Details</summary>
Motivation: The motivation is to formalize and address the issue of CBL in prompt-composed agentic systems, which is a previously unreported failure mode that can lead to sub-threshold regime invisible to standard QA but compounding across the thousands of decisions a deployed agent makes.

Method: The method involves a reusable three-channel protocol that perturbs non-focal modules along volume, content, and form to probe CBL on a deployed job-evaluation agent (Claude Sonnet 4.6, 144 trials).

Result: The result shows that only the content channel produces a detectable paired effect (Cohen's d = 0.63, bootstrap 95% CI excluding zero), indicating that CBL is a significant issue in prompt-composed agentic systems.

Conclusion: The conclusion is that CBL is a real phenomenon that requires measurement and evaluation in prompt-composed agentic systems, and the authors contribute an operational definition, a reusable protocol, a falsifiable prediction set, and a system-class characterization to establish cross-module interference measurement as a requirement for prompt-composed agent evaluation.
...
TL;DR: The paper discusses the concept of accelerating returns in technological progress, and argues that while it may lead to advancements in executional and infrastructural capabilities, it does not address the central problem of scientific discovery, which requires qualitative reasoning.


<details>
  <summary>Details</summary>
Motivation: The authors are motivated by the limitations of the accelerating returns thesis in addressing the central problem of scientific discovery, and the need for a new approach that can provide qualitative reasoning capabilities.

Method: The paper provides a mathematical interpretation of the accelerating returns thesis and discusses the limitations of current AI systems in achieving human-like flexible reasoning, highlighting the need for a Qualitative Engine for Science (QES).

Result: The authors conclude that the Kurzweil theory of accelerating returns helps explain why quantitative capability may accelerate, but it does not solve the central problem of scientific discovery, which requires qualitative reasoning, and they propose QES as a response to this missing capacity.

Conclusion: The paper concludes that the future of AI is not only a technical forecast but also a question of what forms of human understanding are worth preserving and transmitting, and that QES can help address the central problem of scientific discovery.
...
TL;DR: The paper introduces a system called narration-of-thought (NoT) that structures chain-of-thought into five sections to address moral dilemmas, reducing stakeholder collapse and uncertainty suppression.


<details>
  <summary>Details</summary>
Motivation: The authors identified two failure modes in standard chain-of-thought on moral dilemmas: stakeholder collapse and uncertainty suppression. They aimed to develop a system that can improve the handling of moral dilemmas by explicitly addressing these issues.

Method: The authors propose a system called narration-of-thought (NoT) that structures chain-of-thought into five sections: protagonist, stakeholders, two-step consequences, uncertainty, and commitment. NoT does not require additional training, parameters, or fine-tuning.

Result: The results show that NoT significantly reduces stakeholder collapse from up to 31% to under 1% and uncertainty suppression from up to 72% to 1-24% across different models and generators. The system also retains advantages in stakeholder count and uncertainty score. Furthermore, when extended to a multi-stakeholder debate protocol, NoT achieves high levels of consensus and convergence.

Conclusion: The authors conclude that NoT provides a more reliable and auditable substrate for agentic deployment by externalizing stakeholders, consequences, and uncertainty grounding each commitment.
...
TL;DR: A Geometry-Aware Monte Carlo Tree Search (MCTS) framework is proposed to solve extremal problems in combinatorial geometry, achieving new best-known results for various grid sizes.


<details>
  <summary>Details</summary>
Motivation: Classical exact solvers and standard machine learning models struggle with combinatorial explosion, sparse rewards, and quadratic token-consumption limits in solving geometric constraint problems.

Method: The Geometry-Aware MCTS framework enforces geometric constraints through incremental updates to the feasible action space, exploits geometric symmetries for efficient search, and applies canonical pruning and symmetric batch transitions.

Result: New best-known computational results are established for five out of six considered problems, with configurations of size roughly 1.8n for Max-N3IL and 0.95n for the Smallest Complete Set problem.

Conclusion: The Geometry-Aware MCTS framework is a highly adaptable approach for discovering novel configurations in combinatorial geometry, outperforming existing methods for various grid sizes.
...
TL;DR: The paper proposes an agentic aggregator framework for coordinating electric bus fleets, optimizing scheduling, and managing vehicle-to-grid (V2G) operations.


<details>
  <summary>Details</summary>
Motivation: The increasing complexity of operational tasks in electric bus fleets, requiring coordination between multiple factors such as service reliability, battery state-of-charge, and electricity prices.

Method: The proposed framework couples an optimization-based electric bus scheduling model with supervisory agents for disturbance detection, tariff adaptation, and schedule evaluation.

Result: The results show that agentic aggregation can support adaptive fleet-grid coordination, but also reveal a trade-off between reducing operational complexity and extracting value from the public transport operator (PTO) when configured around profit-oriented pricing.

Conclusion: The deployment of agentic aggregators in public-fleet contexts requires transparent coordination modes, auditable tariff-setting, and explicit value-sharing rules.
...
TL;DR: The paper discusses a problem faced by 'oracle' AI that estimates probabilities of future events, known as the self-reference problem. It proposes a solution where the oracle reports a credal set (a set of probabilities) that is unbiased and self-consistent with the consequences of being learned.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the self-reference problem in 'oracle' AI, where its answer can change the probability it was asked to report, making its initial answer irrelevant once learned.

Method: The method involves using the Knaster--Tarski fixed-point theorem on the complete lattice of closed credal sets to find a canonical, nontrivial member of the credal set that is self-consistent and unbiased.

Result: The result is a credal set that is simultaneously unbiased and self-consistent with the consequences of being learned. For a binary event, the canonical answer is an interval under a natural hull-factoring assumption.

Conclusion: The paper concludes with the development of a lattice-theoretic approach that extends from a binary event to an arbitrary random variable, and poses open questions, including whether the interval characterization survives generalization.
...
TL;DR: The paper evaluates confidence interval methods for text classification performance metrics, finding that commonly used methods are often inaccurate, and provides guidance on more suitable approaches.


<details>
  <summary>Details</summary>
Motivation: Researchers use text classification to measure constructs from natural language, but often fail to report uncertainty measures or use inappropriate methods, leading to potentially misleading results.

Method: The paper uses simulations to evaluate the accuracy of various confidence interval methods for performance metrics under conditions typical of social science text classification.

Result: The study finds that default methods such as Wald interval and basic percentile bootstrap are often inaccurate, while Agresti-Coull, Wilson, Clopper-Pearson, and a novel pseudo-count regularized bootstrap show improved accuracy.

Conclusion: The paper provides guidance on appropriate interval estimation methods for text classification, aiming to improve the transparency of machine learning applications and encourage greater attention to validation sample size at the design stage.
...
TL;DR: The paper explores the limitations of supervised deep learning in achieving symbolic-level syllogistic reasoning, finding that it cannot reach the same level as sphere neural networks even with increased training data and time.


<details>
  <summary>Details</summary>
Motivation: To investigate the limitations of scaling up data-driven machine learning systems to achieve symbolic-level syllogistic reasoning.

Method: Theoretical analysis and experimental evaluation of supervised deep learning models, including Euler Net and ChatGPT (GPT-5-nano and GPT-5).

Result: The study reveals two methodological limitations of supervised deep learning: (1) insufficient training data to distinguish all valid syllogistic reasoning types, and (2) contradictory training targets for pattern recognition and logical reasoning. Experimental results show that these models cannot achieve rigorous syllogistic reasoning.

Conclusion: Supervised machine learning systems will not attain the rigor of symbolic logical reasoning, even with increased training data and time, due to inherent methodological limitations.
...
TL;DR: The paper introduces MKG-RAG-Bench, a benchmark for evaluating retrieval in multimodal knowledge graph RAG (MKG-RAG) systems, highlighting the challenges of retrieval in heterogeneous multimodal knowledge and its impact on generation performance.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for RAG systems do not adequately address the challenges of retrieval in multimodal knowledge graphs, where retrieval is a critical bottleneck due to the heterogeneous nature of multimodal knowledge.

Method: The authors construct MKG-RAG-Bench using an LLM-based curation pipeline, which includes filtering low-utility knowledge, generating structurally grounded queries with exact supervision, and systematically covering diverse modality configurations across two multimodal knowledge graphs in general and medical domains.

Result: The experiments show that effective multimodal retrieval is challenging yet crucial for end-to-end MKG-RAG performance, and that retrieval quality strongly determines generation outcomes, providing a foundation for diagnosing current limitations and advancing MKG-RAG systems.

Conclusion: The introduction of MKG-RAG-Bench provides a principled foundation for evaluating and advancing multimodal knowledge graph RAG systems by isolating retrieval as a first-class evaluation target.
...
TL;DR: The paper presents an AI-based system, auto-psych, that automates data collection and theory discovery in computational cognitive science using nested agent-based discovery loops.


<details>
  <summary>Details</summary>
Motivation: The major bottleneck in AI-based scientific automation is data collection. The authors aim to apply automated discovery techniques to generate theories in computational cognitive science.

Method: The authors use an agent-based system, auto-psych, with nested discovery loops to generate explanatory theories of human behavior. The inner loop conjectures, fits, and critiques probabilistic cognitive models, while the outer loop designs experiments, launches them online, and analyzes the data.

Result: The system can quickly and reliably recover ground-truth theories from synthetic data and finds theories that fit human experimental data better than theories generated from the scientific literature.

Conclusion: The work demonstrates the feasibility of automated data collection and theory discovery in computational cognitive science, showing potential for AI-based scientific automation in this field.
...
TL;DR: The authors propose a Clinical Harness architecture for developing and managing AI-enabled clinical capabilities that persist across time and are accountable.


<details>
  <summary>Details</summary>
Motivation: Current Medical AI models are isolated and do not provide persistent, accountable capabilities required in clinical care.

Method: The authors propose a runtime governance architecture called Clinical Harness, which includes registering, orchestrating, guarding, and monitoring AI-enabled clinical capabilities.

Result: The proposed architecture is demonstrated using osteoporosis as an example, showing how different types of AI skills can support lifecycle care under runtime governance.

Conclusion: The Clinical Harness architecture has the potential to improve the development and management of AI-enabled clinical capabilities, enabling more accountable and persistent care.
...
TL;DR: The study compares the problem-solving behavior of large reasoning models (LRMs) and humans, finding that while both show similar patterns of increased time spent on harder problems, they diverge in how time is allocated to correct and incorrect trials.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to understand and compare the deliberation processes of humans and large reasoning models (LRMs) when solving problems, specifically focusing on how time or tokens are allocated to problems they get right or wrong.

Method: The study uses a public matched human-LRM corpus to analyze the behavior of humans and five thinking LRMs. It separates two levels of deliberation: registration (how response time tracks difficulty across items) and allocation (whether an agent spends more on its own failures or successes).

Result: The results show that humans and LRMs exhibit similar cross-item alignments in terms of response time and difficulty but diverge within items. Specifically, LRMs spend more tokens on problems they get wrong, while humans spend less time on trials they get wrong. This divergence persists under various analyses, including item fixed effects and across different datasets.

Conclusion: The study concludes that the pattern in humans can be interpreted as engagement versus abandonment, where people persist with problems they expect to solve and give up on others. In contrast, LRMs' pattern is driven by uncertainty, where token chains grow longer when the model is unsure and likely to fail. This highlights a fundamental difference in the stopping policies of humans and LRMs, despite similar surface-level correlations with difficulty.
...
TL;DR: This tutorial paper provides a step-by-step guide to using NeuraDock Agent, an open-source EEG tool for analyzing Alpha dynamics and visual cognitive-load.


<details>
  <summary>Details</summary>
Motivation: The goal is to bridge the gap between offline EEG analysis and real-time, quality-gated cognitive-load pipelines.

Method: The tutorial provides a walkthrough of installing and using NeuraDock Agent, including EEG preprocessing, quality control, and real-time API integration.

Result: The agent processed 18 recordings, generated 10 within-subject comparisons, and observed task-related posterior Alpha suppression in 7 of 10 contrasts.

Conclusion: The tutorial provides a transparent path from EEG files to real-time visual cognitive-load prototypes for researchers, developers, and applied teams.
...
TL;DR: The paper presents NeuraDock Agent, an open-source architecture that integrates a large language model (LLM) with a deterministic local EEG engine to provide hardware-aware and implementation-aware grounding for EEG analysis.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of general language models not automatically knowing the limitations and capabilities of specific sensors and software, particularly in low-channel EEG analysis where incorrect interpretations can be easily produced.

Method: The method involves separating a deterministic local EEG engine from a hardware-aware language layer, where the LLM receives a compact, allowlisted summary and a versioned context pack that describes the hardware, workflows, and scientific limits.

Result: The results show that the system produces identical structured results over multiple repetitions, preserves local artifacts under various failure scenarios, and demonstrates hardware- and implementation-aware grounding as a practical mechanism for calibrating what an EEG agent accepts, qualifies, or refuses.

Conclusion: The conclusion is that the proposed architecture provides a practical mechanism for calibrating the capabilities and limitations of an EEG agent, but does not establish clinical validity or a validated absolute cognitive-load index.
...
TL;DR: The paper proposes a framework for interpreting AI systems as agents by drawing on radical interpretation and mechanistic interpretability to understand their beliefs, desires, and meanings.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this work is to ensure safety in AI systems by enabling trust through understanding their goals or detecting deception.

Method: The method involves developing criteria for both representationalist and interpretationist approaches, tying them to tests that current interpretability methods can perform, and emphasizing the holistic attribution of beliefs, desires, and meanings.

Result: The result is a framework that provides a structured approach to interpreting AI systems, highlighting the interdependence of beliefs, desires, and propositional structures, and suggesting that mechanistic interpretability can help measure these aspects.

Conclusion: The conclusion is that interpreting AI systems requires a holistic approach considering the interdependencies between beliefs, desires, and meanings, and that mechanistic interpretability offers a way to advance this understanding.
...
TL;DR: The paper proposes PMDformer, a model for long-term time series forecasting that captures shape similarities across patches and variables, outperforming state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: The challenge of accurately modeling shape similarities across patches and variables in long-term time series forecasting due to scale differences.

Method: The paper introduces patch-mean decoupling (PMD) to separate trend and residual shape information, and proposes Trend Restoration Attention (TRA) and Proximal Variable Attention (PVA) to effectively model long-range dependencies and capture cross-variable relationships.

Result: Extensive experiments show that PMDformer outperforms existing state-of-the-art methods in stability and accuracy across multiple LTSF benchmarks.

Conclusion: The proposed PMDformer model effectively captures shape similarity in long-term forecasting scenarios, providing a reliable solution for long-term time series forecasting.
...
TL;DR: This study uses machine learning to detect cirrhosis in hepatitis C patients, achieving high accuracy with an Extra Trees model.


<details>
  <summary>Details</summary>
Motivation: Detecting cirrhosis earlier is crucial for avoiding complications, and current treatments focus on preventing further progression of the disease.

Method: Four machine learning algorithms (Random Forest, Gradient Boosting Machine, Extreme Gradient Boosting, and Extra Trees) were trained on a dataset of 2038 Egyptian patients with 28 attributes.

Result: The Extra Trees model outperformed others with an accuracy of 96.92%, recall of 94.00%, precision of 99.81%, and an area under the receiver operating characteristic curve of 96% using only 16 features.

Conclusion: Machine learning can be effective in detecting cirrhosis in patients with hepatitis C, with the Extra Trees model showing promising results.
...
TL;DR: EvoOptiGraph, a novel framework for automating optimization modeling from natural language, improves LLMs by co-evolving data and model, leading to better accuracy, executability, and generalization.


<details>
  <summary>Details</summary>
Motivation: The challenges of automating optimization modeling from natural language with LLMs include lack of structural diversity in training corpora and static, decoupled data generation pipelines.

Method: EvoOptiGraph represents MILPs as attributed bipartite graphs, applies evolutionary operators, and uses a two-stage training process: SFT and RLVR.

Result: EvoOptiGraph outperforms larger generalist models, agentic methods, and specialized baselines in accuracy, executability, and generalization on six public datasets.
...
TL;DR: A new framework is proposed to improve the reliability and executability of AI-generated decisions in telescope scheduling by introducing multi-level validation and traceable reasoning.


<details>
  <summary>Details</summary>
Motivation: The outputs of AI-based decision-making in telescope scheduling often suffer from inconsistent data references, reasoning errors, and non-executable decisions, which limit their applicability in high-reliability observational tasks.

Method: A multi-level validation and traceable reasoning framework is proposed, which integrates data reference validation, logical consistency checks, and observational and instrumental constraint verification, and introduces atomic reasoning units and their dependency relationships.

Result: The framework improves the executability and reliability of AI scheduling, reduces loss of transient opportunities, and enhances the ability to repair and block erroneous decisions, especially in complex scenarios.

Conclusion: The proposed framework provides a feasible and verifiable pathway for applying AI to high-reliability astronomical observation scheduling, maintaining flexibility while substantially improving reliability and executability.
...
TL;DR: A system is proposed to automate dispatching emails to WhatsApp groups based on their content, using large language models (LLMs) to analyze email contents and route them to relevant groups, enhancing productivity and reducing cognitive load.


<details>
  <summary>Details</summary>
Motivation: The manual process of handling a large volume of emails is error-prone, time-consuming, and leads to losses in productivity and undue stress. The motivation is to find an alternative mechanism to automate this task.

Method: The system uses agents querying LLMs to analyze email contents and route them to relevant WhatsApp groups. It utilizes a well-structured agent framework prompt with email content as input, instructions, and context to determine the relevant groups.

Result: The proposed system provides several benefits, including enhanced productivity and a reduction in the cognitive load associated with reading emails. It does not rely on labelled datasets.

Conclusion: The proposed system offers a solution to automate the task of dispatching emails based on their contents to the respective WhatsApp groups, facilitating a smooth flow of information within an organisation.
...
TL;DR: A novel methodology integrating large language models, statistical techniques, and human-AI collaboration is proposed to improve multilingual customer feedback analysis in public sector organizations.


<details>
  <summary>Details</summary>
Motivation: Enhancing service feedback analysis is crucial for public sector organizations, particularly tax administrations, to ensure fair and effective service delivery, trust, and compliance.

Method: The proposed framework combines fine-tuned, quantized large language models with expert oversight to produce accurate, computationally efficient, and context-aware analyses.

Result: The approach demonstrates stronger alignment with expert judgments, reduces LLM fabrication, and improves the reliability and relevance of generated insights.

Conclusion: The proposed methodology contributes to the development of responsible AI systems that enhance service quality, responsiveness, fairness, and public trust through more effective analysis of multilingual customer feedback.
...
TL;DR: The paper presents an autoformalization pipeline to translate natural language policy documents into formally verified policies, addressing the limitations of existing approaches in agent safety.


<details>
  <summary>Details</summary>
Motivation: The need for formal policy enforcement in high-stakes domains, where current methods either lack formal guarantees or are not scalable.

Method: An autoformalization pipeline using an LLM-based generator-critic loop to translate agent prompts, MCP tool descriptions, and natural language policy documents into formally verified policies in Cedar Policy Language.

Result: The autoformalized policies cover substantially more of the source natural-language specification than hand-coded symbolic enforcement in prior work on the MedAgentBench benchmark.

Conclusion: The proposed pipeline offers a scalable and guaranteed approach to formal policy enforcement in high-stakes domains.
...
TL;DR: A new framework, SkillDisCo, is introduced to enable agents to reuse procedural skills learned from successful traces, improving efficiency and success rates in task execution.


<details>
  <summary>Details</summary>
Motivation: Agents often solve similar tasks from scratch, leading to high reasoning costs and long execution times. The goal is to find a way to reuse successful task structures across different scenarios.

Method: The authors propose SkillDisCo, a distillation-and-compilation framework that extracts reusable procedural skills from successful traces in FSM-defined scenarios and compiles them into executable skills.

Result: Experiments on ALFWorld and WebArena demonstrate that SkillDisCo improves success rates and reduces agent turns across benchmarks and model scales.

Conclusion: The study shows that representing shared experience as reusable execution structures can benefit task execution, and SkillDisCo provides an effective approach to achieve this.
...
TL;DR: The paper presents NebulaExp, a transparent post-training pipeline for large language models, improving their reasoning and human preference following capabilities through a fully reproducible approach.


<details>
  <summary>Details</summary>
Motivation: The lack of detailed data construction, filtering rules, and training recipes in existing works hinders community reproducibility and lightweight model optimization.

Method: The authors propose a post-training pipeline called NebulaExp, built on Qwen3-8B-base, with two model branches: general instruct model and complex reasoning-specialized model. They curate a large corpus of SFT samples and design an end-to-end data processing stack.

Result: The results show improvements in benchmark scores for both Instruct and Reasoning branches, with NebulaExp-Ins-SFT and GRPO reinforcement learning achieving average scores of 60.99 and 61.85, respectively. For the Reasoning branch, medium-difficulty GRPO RL improves average reasoning score from 73.88 to 75.17.

Conclusion: The paper provides a fully reproducible empirical post-training recipe for 8B-scale LLMs and comprehensively dissects the capability trade-offs among instruction adherence, mathematical reasoning, code generation, and general knowledge.
...
TL;DR: The paper questions the necessity of chain-of-thought (CoT) reasoning in safety guardrails, finding that a lightweight label-only encoder (LeanGuard) matches the performance of a reasoning guard while using significantly less compute.


<details>
  <summary>Details</summary>
Motivation: The authors aim to investigate whether a safety guardrail really needs to reason, as CoT makes guardrails heavy and slow, which may not be suitable for on-device deployment.

Method: The authors train a lightweight bidirectional encoder and a reasoning guard on the same corpus, then remove only the reasoning from the guard while keeping everything else fixed, and compare their performance.

Result: The label-only encoder (LeanGuard) achieves an average F1 of 82.90 ± 0.26 over public benchmarks, matching a reasoning guard built on a much larger decoder, while using only a single forward pass and reducing inference compute by ~100x.

Conclusion: The findings suggest that CoT may not be necessary for moderation, and that current guardrail benchmarks may not be challenging enough to reward reasoning, implying that simpler, more efficient guardrails like LeanGuard may be sufficient.
...
TL;DR: The paper proposes a new few-shot learning framework called Kalman Prototypical Network (KPN) for fault detection in combined-cycle gas turbines (CCGTs), which outperforms conventional methods in accuracy and stability.


<details>
  <summary>Details</summary>
Motivation: The complexity of CCGT systems and the scarcity of labeled fault data make fault detection challenging, and the authors aim to address this issue with a novel few-shot learning approach.

Method: The authors introduce KPN, a metric-based few-shot learning framework that models the evolution of class prototypes as latent stochastic states in a dynamic system, and evaluate it using synthetic data sets generated with a high-fidelity Modelica-based dynamic simulation of an offshore CCGT system.

Result: The proposed framework outperforms conventional few-shot learning methods such as Matching Networks, Relation Networks, and MAML in both accuracy and stability under varying support and query configurations.

Conclusion: The proposed framework significantly improves training convergence and generalization by stabilizing class representations, making it well-suited for real-world CCGT fault detection where labeled data is limited.
...
TL;DR: LithoDreamer, a physics-informed World Model framework, is proposed for computational lithography to improve yield and performance in semiconductor technology.


<details>
  <summary>Details</summary>
Motivation: Existing models fail to capture the continuous physical process of lithography, which involves mask optimization, optical imaging, resist exposure, and development.

Method: LithoDreamer formulates the lithography pipeline as a decision-driven multi-step evolution system and uses a contrastive variational optimization paradigm for interpretable intervention optimization.

Result: LithoDreamer achieves state-of-the-art performance in forward evolution and inverse planning.

Conclusion: LithoDreamer offers a novel approach to computational lithography, capturing stage-specific physics-informed latent spaces and guiding the model to generate evolutions consistent with real lithography physics.
...
TL;DR: The paper proposes a latent dynamical model to analyze cardiac magnetic resonance imaging (CMR) data, encoding bi-ventricular anatomy and full-cycle cine motion to predict incident heart failure.


<details>
  <summary>Details</summary>
Motivation: Conventional risk models use only a few image-derived indices from selected cardiac phases, whereas CMR captures rich spatiotemporal information about ventricular structure and motion.

Method: The proposed model uses a graph-based mesh autoencoder, heart-rate-aware neural ordinary differential equation (ODE) dynamics, and a covariate-conditioned prior to reconstruct anatomically consistent 3D+t ventricular motion.

Result: The model improved the stratified C-index from 0.704 to 0.785 in a held-out evaluation subset, outperforming seven established cardiac markers and non-graph and non-ODE approaches.

Conclusion: The results suggest that continuous full-cycle modeling of ventricular motion provides informative cardiac phenotypes beyond conventional CMR summaries, with potential for clinical risk-prediction use after external validation.
...
TL;DR: A new AI system, AHOIS, enables autonomous scientific discovery by constructing, challenging, and revising physical explanations in response to evidence.


<details>
  <summary>Details</summary>
Motivation: The current automation of scientific discovery is limited to procedural workflows fixed by human designers, lacking epistemic autonomy.

Method: AHOIS uses a multi-agent AI approach with a physics-critic agent that interrogates hypotheses through causal questioning, constraint checking, and counterexample generation.

Result: AHOIS autonomously proposed and validated a new hypothesis, discovered task-adaptive measurement strategies, and diagnosed failure modes on a complex optical platform.

Conclusion: AHOIS establishes a route towards evidence-grounded, self-correcting autonomous discovery in complex physical environments, demonstrating the potential for epistemic autonomy in AI systems.
...
TL;DR: The authors propose a framework for optimizing scientific discovery by treating it as a meta-optimization problem, where the evaluation criteria are also optimized. They introduce 'consensus objective aggregation' to combine LLM-generated objective functions, leading to a stable and self-correcting evaluation criterion. This approach is applied to algorithm discovery for 3-SAT problems, achieving significant improvements in efficiency.


<details>
  <summary>Details</summary>
Motivation: The authors argue that scientific discovery is an optimization problem and that large language models (LLMs) can aid in exploring the vast 'state space' of theories and experiments. However, they emphasize that modifying the evaluation criteria is equally important. They propose formalizing research as meta-optimization to address this need.

Method: The authors propose a framework called 'consensus objective aggregation,' where LLM-generated objective functions are combined via correlation-weighted voting. This approach yields a stable, self-correcting evaluation criterion that evolves as understanding deepens.

Result: The authors apply their framework to algorithm discovery for 3-SAT problems based on digital MemComputing machines. They achieve significant improvements, reducing the baseline scaling with problem size N from ~N^2.51 to ~N^1.33 and delivering a ~67x speedup on the largest instances tested.

Conclusion: The authors conclude that their problem-agnostic framework can considerably aid scientific discovery by optimizing the evaluation criteria and improving the efficiency of exploring the state space of theories and experiments.
...
TL;DR: EGG, an Expert-Guided Agent Framework, is proposed to automate high-performance GPU kernel generation for large language models by incorporating expert optimization principles, achieving a 2.13x average speedup over PyTorch.


<details>
  <summary>Details</summary>
Motivation: The development of high-performance GPU kernels for large language models heavily relies on manual tuning by domain experts, which is time-consuming and inefficient. Recent LLM-based approaches struggle to achieve both correctness and high performance due to the lack of domain-specific optimization guidance.

Method: The EGG framework decomposes kernel generation into two hierarchical stages: algorithmic structure design and hardware-specific tuning. A stage-aware multi-agent collaboration mechanism is designed for inter and intra-stage context management.
...
Conclusion: The proposed EGG framework effectively addresses the limitations of existing approaches by incorporating expert optimization principles, leading to improved performance and efficiency in GPU kernel generation for large language models.
...
TL;DR: The paper introduces ResilPhase, a noise-resilient acceleration framework for diffusion models, which achieves state-of-the-art fidelity under aggressive acceleration ratios.


<details>
  <summary>Details</summary>
Motivation: The significant inference latency of powerful diffusion models hinders their adoption, and recent 'cache-then-forecast' schemes suffer from severe quality degradation at high acceleration ratios.

Method: The authors reformulate accelerated inference as stable macro-trajectory extrapolation in ODE space, and introduce a derivative-free barycentric Lagrange extrapolator and a bounded Phase Mapping to address derivative instability and oscillatory error growth.
...
Conclusion: The proposed ResilPhase framework effectively alleviates the issues of quality degradation and instability in accelerated diffusion models, achieving high-fidelity results under aggressive acceleration ratios.
...
TL;DR: The paper studies the problem of 'memory depth' in language agents, which refers to durable goal-conditioned tendencies that persist even after the working context is unloaded. The authors introduce a protocol called 'loop-drift' to test this and evaluate a mechanism called EVAF, which enables goal persistence and post-unload recovery.


<details>
  <summary>Details</summary>
Motivation: Long-running language agents need more than just memory access; they require the ability to retain goal-conditioned tendencies even after the working context is unloaded.

Method: The authors introduce the 'loop-drift' protocol, a controlled stress test to evaluate the persistence of goal-conditioned behavior. They also propose EVAF, a surprise- and valence-gated LoRA consolidation mechanism, and conduct various control experiments to analyze its components.

Result: The results show that EVAF outperforms retrieval systems in goal persistence and post-unload recovery (0.812-0.904), while retrieval is strongest on shallow factual recall (0.956-0.973). The mechanism controls demonstrate that selective consolidation can be factorized into two controllable dimensions: selection and actuation.

Conclusion: The paper concludes that selective parametric consolidation, as embodied by EVAF, supplies 'memory depth' distinct from and complementary to retrieval access, which is essential for long-running language agents.
...
TL;DR: A new method is proposed to integrate factual knowledge from a knowledge base into LLM output during token generation, enabling updates, transparency, and improved factual accuracy.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the factual accuracy and transparency of LLM output by integrating factual knowledge from a knowledge base.

Method: The method involves training the model to produce special tokens that trigger a query to the knowledge base, allowing factual knowledge to be pulled in during token generation.

Result: The experiments show that the method improves factual grounding in both short and long-form generation and allows factual revisions to take effect through KB edits.

Conclusion: The proposed method enables LLMs to automatically pull in factual knowledge from a knowledge base, improving factual accuracy, transparency, and explainability.
...
TL;DR: The study analyzed Heart Rate Variability (HRV) indices in 40 healthy adults to improve its clinical utility, finding that certain indices are more suitable for representing HRV components and enhancing its clinical relevance.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this study is to evaluate HRV indices in healthy individuals to improve HRV's clinical utility, as research on HRV parameters in healthy individuals is limited and no gold standard exists.

Method: The study used computational methods for signal processing and data analysis to analyze time, frequency, and nonlinear indices of HRV in 40 healthy adults.

Result: The study found that certain HRV indices, such as ApEn, IRRR, HRVi, SD2, MADRR, and rMSSD, are best suited for accurately representing HRV components and enhancing its clinical and research relevance.

Conclusion: The study concludes that the selected HRV indices can improve the clinical utility of HRV analysis and enhance its relevance in research, by providing a more accurate representation of HRV components.
...
TL;DR: The paper reveals that single-model evaluations on single runs significantly underestimate the true capabilities of Large Language Models (LLMs), especially in heterogeneous data distributions. The authors propose the 'Capability Frontier' as a method to better assess performance across multiple models and generations.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks for LLMs report accuracy for a single model on a single run, which underestimates real-world capabilities, particularly with heterogeneous data distributions where different models excel in different areas and multiple generations can improve performance.

Method: The authors introduce the 'Capability Frontier', a Pareto frontier that characterizes the best achievable performance at each cost level by optimally selecting across models and generations. They study 21 LLMs across 16 benchmarks and use controlled probabilistic simulations to validate their findings.

Result: The study finds that correcting for single-model evaluation yields a 54% error rate reduction, and further correcting for single runs yields an 82% improvement. This results in State-of-the-Art (SOTA) accuracy being matched at an 85% cost reduction. Simulations show that higher query topic entropy increases the performance gap between oracle routing and the best single model.

Conclusion: The collective capabilities of LLMs are substantially underestimated by traditional single-model, single-run evaluations. The 'Capability Frontier' offers a more comprehensive evaluation method, which has significant implications for the assessment and deployment of LLMs in diverse, multi-domain settings.
...
TL;DR: A new framework, CASOP, is proposed for constructing and evaluating optimization pipelines for order fulfillment in warehouses, allowing for the automatic synthesis of valid and high-performing algorithmic pipelines.


<details>
  <summary>Details</summary>
Motivation: The need for a general mechanism to determine applicable algorithm configurations, compose them into valid solution pipelines, and assess their performance in warehouse systems, due to the complexity of order fulfillment decisions and the limitations of existing studies.

Method: The CASOP framework consists of a modular repository of algorithms, semantic data and algorithm cards, a taxonomy of order fulfillment problems, a pipeline synthesizer, and a pipeline evaluator.

Result: The framework was demonstrated on 7 benchmark instance sets, resulting in 1,063,044 valid pipelines, and is expected to support researchers and practitioners in designing and selecting high-performing algorithmic pipelines for warehouse operations.

Conclusion: The CASOP framework provides a novel approach to constructing and evaluating context-specific optimization pipelines for order fulfillment in warehouses, offering a flexible and scalable solution for improving warehouse operations.
...
TL;DR: A new AI-assisted framework for Life Cycle Assessment (LCA) interpretation is proposed, enabling the translation of environmental improvement opportunities into strategic pathways under uncertainty.


<details>
  <summary>Details</summary>
Motivation: The interpretation phase of LCA often lacks structured mechanisms for translating quantified improvement opportunities into actionable strategic pathways under technological, social, and policy uncertainty.

Method: A perspective-conditioned retrieval-augmented generation framework for LCA interpretation, incorporating multi-perspective retrieval and controlled synthesis using large language models.

Result: The framework is demonstrated through a hydrogen-enabled diesel reduction use case in an Italian apple production facility, showing potential for disciplined translation of impact results into strategic pathways.

Conclusion: The approach supports more disciplined translation of impact results into strategic pathways and opens up new avenues for the use of advanced AI tools in LCA studies.
...
TL;DR: AgentX is a multi-agent system that automates the recommendation algorithm iteration process, allowing for self-evolution and improvement at a scale and pace that manual workflows cannot sustain.


<details>
  <summary>Details</summary>
Motivation: The current process of recommendation algorithm iteration is limited by a structural execution bottleneck, relying on human engineers and scaling linearly with headcount rather than compounding with evidence, compute, and accumulated experimental knowledge.

Method: AgentX operates as a self-evolving development engine with four tightly coupled stages: Brainstorm Agent generates proposals, Developing Agent translates proposals into production-ready code, Evaluation Agent conducts safe online rollout, and Harness Evolution layer distills execution trajectories into semantic-gradient updates.

Result: AgentX enables the autonomous generation, implementation, evaluation, and learning of recommendation experiments at a scale and pace that no manual workflow can sustain, making the system self-improving.

Conclusion: AgentX fundamentally restructures the production function of recommendation algorithm iteration, transitioning from an artisanal process to an industrialized research loop, and enabling innovation to compound with evidence, compute, and accumulated experimental knowledge.
...
TL;DR: A novel framework, TAVR-VLM, is proposed to improve the adaption of Multimodal Large Language Models (MLLMs) in Transcatheter Aortic Valve Replacement (TAVR) planning by reducing diagnostic hallucinations.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the issue of diagnostic hallucinations in MLLMs when applied to high-stakes domains like TAVR planning, where generated text often lacks anatomical grounding.

Method: The method involves introducing Risk-Conditioned Causal Grounding Attention (R-CGA) in the TAVR-VLM framework. R-CGA creates a model-internal 'Risk → Region → Word' structural grounding pathway, which compresses multimodal inputs into a causal risk bottleneck and purifies dense visual features into a global risk mask.

Result: TAVR-VLM achieves an AUROC of 0.896, boosts CIDEr to 0.936, and reduces the hallucination rate to 8.1%, establishing a new state-of-the-art on the M3TAVR dataset of 1,482 patients.

Conclusion: The proposed TAVR-VLM framework improves the interpretability of evidence-based surgical AI by significantly reducing diagnostic hallucinations and achieving better performance metrics.
...
TL;DR: A synthetic clinical notes pipeline and dataset are introduced to support the development of clinical AI tools while avoiding privacy risks associated with real patient data.


<details>
  <summary>Details</summary>
Motivation: The use of real-world data in healthcare is restricted due to sensitivity, and synthetic data can enable the development and evaluation of AI systems.

Method: A modular pipeline combines structured patient generation, semi-structured patient journey simulation, and unstructured clinical note generation using large language models.

Result: A dataset of 70 synthetic patients with 20-50 clinical notes each, spanning a full hospital journey, is provided at multiple levels of validation.

Conclusion: The dataset supports the development, testing, and evaluation of clinical AI systems without reliance on real patient data, prioritizing internal consistency and variation in clinical detail.
...
TL;DR: The paper proposes a new framework for pattern-preserving attribute retrieval, which aims to find items that satisfy a target attribute while maintaining a specific pattern. The authors introduce a staged framework called MO-DiT+HPPO, which improves upon existing generative retrieval methods.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the limitation of traditional embedding-based retrieval methods, which often prioritize similarity over specific patterns and attributes. The authors aim to develop a method that can retrieve items that not only match a target attribute but also preserve a given pattern.

Method: The authors propose a staged framework called MO-DiT+HPPO, which consists of raw-sequence pretraining, multi-domain metric-ordered continuation pretraining, tail-centroid fine-tuning, and HPPO. The framework uses continuous generative retrieval and incorporates techniques such as metric-ordered training, hybrid candidate pool labeling, and reference-anchored preference optimization.

Result: The results show that the proposed framework improves the intersection metric over a pretrained generative retriever, with significant gains on seven of eight domain-split cells and a marginal tie on the hardest split. The authors also validate their approach through various ablation studies and comparisons.

Conclusion: The paper concludes that the proposed MO-DiT+HPPO framework effectively addresses the challenge of pattern-preserving attribute retrieval. The authors demonstrate the superiority of their approach through extensive experiments and ablation studies, highlighting its potential for real-world applications.
...
TL;DR: Proposed Recover Task eXpert (ReTeX), a framework that recovers task-expert performance from a single merged model by predicting and undoing parameter interference.


<details>
  <summary>Details</summary>
Motivation: Static merging of task-specific experts suffers from parameter interference, while dynamic merging models require costly storage and loading of redundant expert components.

Method: ReTeX models parameter perturbations as affine transformations, approximated as additive offsets, and predicts these offsets to recover task-expert performance. It also introduces a router-free task identifier based on SVD subspace signatures.

Result: ReTeX recovers over 95% of individual-expert performance in both vision and NLP domains and shows emergent adaptive interpolation of expert knowledge for out-of-distribution tasks.

Conclusion: ReTeX effectively recovers task-expert performance from a single merged checkpoint and improves generalization to unseen tasks.
...
TL;DR: Large language models struggle with out-of-distribution generalization due to task insensitivity, but a new method called Task-Perturbed NLL Optimization improves task sensitivity and generalization.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the weakness of large language models in out-of-distribution generalization, which is caused by task insensitivity.

Method: The method proposed is Task-Perturbed NLL Optimization, a lightweight contrastive regularizer that encourages action dependence on task instructions.

Result: The results show that the proposed method improves task sensitivity and out-of-distribution generalization while preserving stable attention to task tokens.

Conclusion: The conclusion is that Task-Perturbed NLL Optimization is an effective approach to mitigating task insensitivity in large language models, leading to better out-of-distribution generalization.
...
TL;DR: The paper investigates the effectiveness of Chain-of-thought (CoT) training in language models, finding that it improves the model's ability to predict actions directly from prompts, rather than enhancing the reasoning process.


<details>
  <summary>Details</summary>
Motivation: To understand what CoT training actually improves: whether the model gets better at changing its action through generated reasoning or better at predicting the action directly from the prompt.

Method: Comparing prompt actions (predicting action without CoT) with CoT actions (predicting action with CoT) across different checkpoints and analyzing the impact of selectively masking action-token supervision.

Result: CoT training improves the quality of prompt actions and does not widen the advantage of CoT reasoning. Later checkpoints show greater reliance on the prompt and are less likely to revise actions in response to CoT. The intervention of masking action-token supervision improves out-of-domain generalization.

Conclusion: CoT training primarily helps models improve at predicting actions directly from prompts rather than enhancing the faithfulness of the CoT reasoning process.
...
TL;DR: This paper proposes a camera planning framework called Look-Before-Move for embodied AI in dynamic 3D environments, enabling active visual perception and decision-making.


<details>
  <summary>Details</summary>
Motivation: The need for active visual perception in dynamic 3D environments, where the camera must decide what to observe and how to move to acquire relevant visual evidence.

Method: The Look-Before-Move framework, which separates observation specification from motion execution, using a Semantic Observation Contract, Monte Carlo Viewpoint Search, and Semantic Trajectory Grounding.

Result: The proposed framework improves subject perception, intent consistency, and trajectory quality over baselines, demonstrating the importance of organizing visual attention before generating camera motion.

Conclusion: The Look-Before-Move framework enables Narrative-Grounded World Visual Attention, allowing embodied AI to actively decide what to observe and how to move in dynamic 3D environments.
...
TL;DR: The paper explores if LLMs can benefit from visualizing counterfactual events to improve complex reasoning, proposing Einstein World Models (EWMs) which integrate visual-temporal rollouts into LLM-based reasoning systems.


<details>
  <summary>Details</summary>
Motivation: The motivation is to investigate if visualizing counterfactual events can complement language in complex thought and to enhance LLMs' reasoning abilities through such visualization mechanisms.

Method: The proposed method is Einstein World Models (EWMs), a blueprint for LLM-based reasoning systems that incorporate visual-temporal rollouts into the reasoning process, allowing LLMs to generate and inspect hypotheses through visual thought experiments.

Result: The paper does not specify results, but it outlines a proposal for EWMs which are designed to extend LLMs' capabilities into the domain of visual thought experiments, potentially improving their reasoning abilities.

Conclusion: The conclusion is that by proposing EWMs, the paper suggests a positive approach to enhancing LLMs with visualization mechanisms for better complex reasoning, indicating that intelligence can indeed benefit from reasoning about phenomena beyond direct experience through visual counterfactual events.
...
TL;DR: AURORA-AI, a framework for adaptive resource allocation in AI systems, is proposed to improve resilience and fairness under non-stationary conditions.


<details>
  <summary>Details</summary>
Motivation: Static resource allocation strategies degrade AI performance and human-centric properties like fairness and explainability under changing conditions.

Method: AURORA-AI uses Hamilton-Jacobi-Bellman feedback control, Lyapunov-based stability monitoring, and a fairness-aware utility function to adaptively allocate resources.

Result: AURORA-AI outperforms five baseline controllers in a simulated stress test, achieving immediate recovery from disruptions, improved predictive performance, fairness, and stability.

Conclusion: AURORA-AI demonstrates that fairness-aware adaptive orchestration based on stability theory is a practical approach to resilient and human-centric AI deployment.
...
TL;DR: The paper proposes a semantic early-stopping method for multi-agent large language model loops, which stops the loop when consecutive draft embeddings stop changing in meaning and the answer's quality stops improving.


<details>
  <summary>Details</summary>
Motivation: The current method of terminating multi-agent large language model loops using a fixed iteration cap is inefficient, as it does not consider whether the answer is still improving.

Method: The paper proposes a semantic early-stopping method that uses cosine distance with a patience window to determine when to stop the loop. It also introduces a judge-efficient evaluation protocol and conducts an empirical study on multi-hop retrieval-augmented question answering.

Result: The results show that the proposed semantic stopper reduces operational tokens by 38% relative to the fixed iteration cap method at parity quality, while the full quality-gated variant is counter-productive due to high judging costs.

Conclusion: The paper concludes that the proposed semantic early-stopping method is effective in reducing operational tokens while maintaining quality, and that the problem of multi-agent large language model loops can be reframed from 'when to stop' to 'which round is best'.
...
TL;DR: The paper reviews external validity indexes for cluster evaluation, recommending centroid index (CI) for cluster-level assessment and pair-set index (PSI) or clustering accuracy (ACC) for point-level evaluation.


<details>
  <summary>Details</summary>
Motivation: The motivation is to provide guidance on choosing appropriate external validity indexes for cluster evaluation when ground truth is available.

Method: The paper reviews and compares common external validity indexes, specifically set-matching-based measures.

Result: The paper recommends CI for cluster-level evaluation, and PSI or ACC for point-level evaluation, depending on the specific needs of the assessment.

Conclusion: The choice of external validity index depends on the level of evaluation (cluster-level or point-level) and the specific requirements of the assessment, such as normalization and equal weighting of points.
...
TL;DR: A new method, JERP, is proposed to improve LLM agents in interactive environments by jointly learning experiential rules and policies, leading to better decision performance.


<details>
  <summary>Details</summary>
Motivation: The challenge of making effective use of accumulated interaction experience in multi-step interactive environments for LLM agents.

Method: Joint Learning of Experiential Rules and Policies for LLM Agents (JERP), which updates a long-term experiential-rule pool and the policy from the same interaction trajectories.

Result: JERP yields consistent gains in decision performance for complex interactive tasks, as shown in experiments on AlfWorld and WebShop.

Conclusion: JERP addresses the challenge of utilizing accumulated interaction experience in LLM agents by coupling rule learning and policy optimization, leading to improved decision performance in complex interactive tasks.
...
TL;DR: The paper introduces PAVE, a step-wise labeling protocol for root cause analysis (RCA) to evaluate LLM agentic capabilities. Existing datasets are limited, and PAVE helps create OpenRCA 2.0, a benchmark with step-wise causal annotations. Results show LLMs struggle with exact root-cause identification and causal propagation paths.


<details>
  <summary>Details</summary>
Motivation: Existing RCA datasets have a fundamental gap, as they only label the root cause and not the propagation path to the observed symptom, simplifying the task to pattern matching. The authors aim to support rigorous evaluation and create a more comprehensive benchmark.

Method: The authors introduce PAVE, a step-wise labeling protocol leveraging known interventions from fault injection to reconstruct causal propagation paths. They apply PAVE to create OpenRCA 2.0, a cross-system RCA benchmark with step-wise causal annotations for LLM agents.

Result: Across 11 frontier LLMs, the exact root-cause set is successfully recovered in only 20.7% of cases on average. When relaxing the criterion, agents identify at least one correct root-cause service in 76.0% of cases but ground that service in a verified causal propagation path to the observed symptom in only 61.5% of cases.

Conclusion: The authors conclude that step-wise causal ground truth is the missing piece for trustworthy LLM-based RCA agents. Outcome-only evaluation hides the failure mode of LLMs in identifying causal propagation paths, emphasizing the need for a more comprehensive evaluation approach.
...
TL;DR: The paper proposes TOPS, a training-free and model-agnostic pruning module for multimodal large language models (MLLMs) that removes redundant visual tokens while preserving performance.


<details>
  <summary>Details</summary>
Motivation: The efficiency of MLLMs is limited by the large number of visual tokens, which introduces substantial computational overhead. Existing visual token pruning methods have limitations.

Method: The authors formulate visual token pruning as constructing Token Optimal Preservation Sets and identify three fundamental principles for effective token selection: Task Relevance, Information Coverage, and Semantic Diversity. They propose TOPS, a pruning module based on these principles.

Result: Extensive experiments on 7 MLLM backbones and 14 benchmarks demonstrate that TOPS outperforms prior methods under diverse pruning settings. TOPS removes 77.8% of visual tokens while preserving 100.0% and 100.6% performance on LLaVA-NeXT's 7B and 13B models, respectively.

Conclusion: The proposed TOPS module can effectively prune redundant visual tokens in MLLMs, sometimes mitigating hallucination and inspiring future lightweight MLLM design.
...
TL;DR: The paper introduces a new mechanism called the process harness, which integrates Agentic Business Process Management (Agentic BPM) with legacy workflows without replacing the underlying workflow engine.


<details>
  <summary>Details</summary>
Motivation: The motivation is to uplift legacy workflows into Agentic BPM while preserving the existing workflow engine, allowing for the combination of imperative and normative requirements in business processes.

Method: The method involves developing the Task-Decision-Flow (TDF) model, which defines a policy-governed agentic layer, and implementing it through the CUGA FLO system.

Result: The result is a system that successfully demonstrates the process harness on a loan approval workflow, showing its ability to reconcile imperative and normative requirements.

Conclusion: The process harness provides a novel approach to integrating Agentic BPM with legacy workflows, enabling the combination of structural compliance and policy-framed agentic autonomy.
...
TL;DR: This paper proposes GAversary, a genetic algorithm to generate adversarial attacks on NLP models, achieving high success rates with a trade-off in perturbation and runtime.


<details>
  <summary>Details</summary>
Motivation: Deep learning models are vulnerable to adversarial inputs, particularly in NLP, which can have significant real-world consequences, and current methods often require access to the model's internal structure.

Method: GAversary, a hybrid Genetic Algorithm, is proposed to generate adversarial attacks on natural language models by treating the target model as a black box and using GloVe embeddings to propose word replacements.

Result: GAversary substantially reduces the target model's accuracy on test data compared to BAE and A2T attacks, with a best-case reduction from 76.8% to 5.8%, but perturbs more words and has a slightly lower semantic similarity.

Conclusion: GAversary is an effective method for generating adversarial attacks on NLP models, offering a trade-off between success rate and perturbation, and can be applied to various benchmark data sets and target models.
...
TL;DR: BINEVAL is a framework for evaluating LLM outputs by decomposing criteria into binary questions, providing interpretable scores and improving evaluation and prompt optimization.


<details>
  <summary>Details</summary>
Motivation: Current LLM evaluation methods are limited by high costs, poor correlation with human judgments, and lack of transparency.

Method: BINEVAL uses a meta-prompt to generate fine-grained evaluation questions, which are then answered by an LLM to produce transparent feedback and calibrated scores.

Result: BINEVAL matches or outperforms strong baselines, better matches human score distributions, and avoids ceiling effects, with applications in prompt optimization.

Conclusion: BINEVAL provides a task-agnostic, training-free, and interpretable evaluation framework with strong empirical performance and practical diagnostic value.
...
TL;DR: Introduced EO-WM, a video diffusion transformer for multispectral Earth Observation forecasting, which incorporates a physically informed conditioning framework to better respond to changed weather forcing.


<details>
  <summary>Details</summary>
Motivation: Existing methods for Earth Observation forecasting do not fully capture the complexities of weather-driven world modeling, particularly in handling uncertainty and weather variables.

Method: EO-WM, a video diffusion transformer, which separates meteorological forcing into climatological baseline, weather anomalies, and cumulative physical stress signals, and uses distinct conditioning pathways and accumulation of anomalous forcing over time.

Result: EO-WM reduces error in predicted NDVI decline amplitude by 5.63% and improves directional hit rate by 7.80%, while remaining competitive on standard pixel-level metrics.

Conclusion: EO-WM shows improved performance in Earth Observation forecasting, particularly in responding to changed weather forcing, and the proposed benchmarks provide a more comprehensive evaluation of weather-response behavior.
...
TL;DR: This paper compares simulation-based inference (SBI) with Markov chain Monte Carlo (MCMC) for Bayesian calibration of mechanistic epidemiological models, showing SBI to be a rapid and computationally efficient alternative.


<details>
  <summary>Details</summary>
Motivation: The motivation is to find a more efficient method for Bayesian calibration of mechanistic epidemiological models, which are crucial for infectious disease forecasting and public-health decision making, given the computational expense of MCMC.

Method: The method involves using simulation-based inference (SBI) with neural posterior estimation as an alternative to MCMC for calibrating a mechanistic SECIR epidemiological model.

Result: SBI provides comparable posterior and predictive performance to MCMC but with substantially reduced computational runtime, achieving this in approximately 60-70 seconds on a single GPU for 31-day inference problems and 157 seconds for a 201-day inference problem, versus MCMC's 1000 seconds and over 19,000 seconds, respectively.

Conclusion: SBI offers a rapid and computationally efficient framework for Bayesian calibration of mechanistic epidemiological models, supporting repeated near-real-time inference and rapid outbreak analysis.
...
TL;DR: Researchers studied 'prompt injection' in AI-powered resume screening, where applicants add subtle, self-promotional text to influence rankings. They found that while prompt injection can improve rankings, its effectiveness drops as more candidates use it, and raises fairness concerns.


<details>
  <summary>Details</summary>
Motivation: The increasing use of large language models (LLMs) in hiring processes creates incentives for candidates to manipulate these systems. The study aims to investigate the phenomenon of 'prompt injection' in automated resume screening.

Method: The researchers conducted controlled experiments to study the effects of prompt injection on LLM-based resume screening. They analyzed the impact of prompt injection on applicant rankings under different conditions (homogeneous vs. heterogeneous candidate quality).

Result: The study found that prompt injection can improve applicant rankings when resume quality is homogeneous and few candidates inject. However, its effectiveness decreases as more candidates inject, and it can allow lower-quality candidates to outrank higher-quality ones, raising fairness concerns.

Conclusion: The study highlights the vulnerability of LLM-based screening to manipulation, particularly when candidate quality differences are small and manipulation is rare. The findings emphasize the need for careful consideration of the potential risks and consequences of using LLMs in hiring processes.
...
TL;DR: The paper analyzes the limitations of multi-model LLM systems, showing that their accuracy gain is capped by the rate at which all models are wrong on the same query, and that combining models rarely beats the single best model without a strong query-level routing signal.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to understand the limitations of multi-model LLM systems, such as routing, voting, cascades, fusion, and mixture-of-agents, and to identify the factors that affect their accuracy.

Method: The paper uses a theoretical approach to derive a bound on the accuracy gain of multi-model systems, and an empirical approach to analyze the performance of 67 models from 21 providers on open-ended mathematics and execution-graded code tasks.

Result: The paper finds that the accuracy gain of multi-model systems is capped by a quantity called beta, which is the rate at which all models are wrong on the same query. The paper also finds that combining models rarely beats the single best model without a strong query-level routing signal, and that gains come from models failing on different questions, not from adding more models.

Conclusion: The paper concludes that the gain of multi-model LLM systems is limited by the rate at which all models are wrong on the same query, and that combining models is only effective when there is a strong query-level routing signal. The paper also highlights the importance of understanding the error patterns of individual models to improve the performance of multi-model systems.
...
TL;DR: A language-based digital twin framework is proposed for personalized cognitive health monitoring, leveraging large language models to mimic conversational behavior and predict cognitive scores.


<details>
  <summary>Details</summary>
Motivation: Early detection of Mild Cognitive Impairment (MCI) remains challenging, and language and conversational patterns serve as non-invasive biomarkers.

Method: A multi-head conditional variational autoencoder (cVAE) is introduced to evaluate fidelity and cognitive consistency, and large language models (LLMs) are used to mimic conversational behavior.

Result: The digital twin preserves identity-specific characteristics and achieves reconstruction and MoCA prediction errors comparable to real data, outperforming baseline GPT-generated responses.

Conclusion: The results highlight the potential of language-based digital twins as a scalable and non-invasive approach for personalized and continuous cognitive health monitoring.
