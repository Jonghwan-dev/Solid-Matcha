...
TL;DR: A new dual-stream Transformer architecture is proposed for detecting mutual gaze and joint attention from synchronized dual-camera recordings, outperforming existing models and providing a scalable tool for behavioral scientists.
...
Conclusion: The proposed model provides a scalable tool for behavioral scientists, bridging the gap between computational modeling and applied interaction research.
...
TL;DR: RecGen, a generative framework, accurately reconstructs complex multi-object scenes from sparse RGB-D images, outperforming state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Reconstructing complex full multi-object scenes from sparse observations is a core challenge in computer vision and a key step toward scalable and reliable simulation for robotics.
...
Conclusion: RecGen offers a robust solution for reconstructing complex multi-object scenes, handling severe occlusions, symmetric objects, object parts, and intricate geometry and texture.
...
TL;DR: This paper introduces InterPartAbility, an interpretable method for text-to-image person re-identification (TI-ReID) that provides explicit part-wise matching and phrase-region grounding.


<details>
  <summary>Details</summary>
Motivation: Existing interpretability approaches in TI-ReID are limited to qualitative visualizations and fail to bind visual regions to semantically meaningful concepts.

Method: The proposed method, InterPartAbility, uses a patch-phrase interaction module (PPIM) to train a TI-ReID model with concept-level guidance and constrains self-attention to produce spatially concentrated patch activations.

Result: InterPartAbility achieves state-of-the-art (SOTA) interpretability performance on challenging benchmarks like CUHK-PEDES and ICFG-PEDES, while sustaining competitive retrieval accuracy.

Conclusion: The proposed method provides a more interpretable and explainable approach to TI-ReID, enabling phrase-region grounding and explicit part-wise matching.
...
TL;DR: A method to compress a foundation-model pipeline for individual-level livestock monitoring, enabling it to run on commodity edge accelerators with reduced GPU memory usage while maintaining high accuracy.


<details>
  <summary>Details</summary>
Motivation: The accuracy of precision livestock farming (PLF) has been improved by foundation-model pipelines, but their high GPU memory requirements exceed the capacity of commodity edge accelerators, necessitating a compression approach.

Method: The 446M-parameter Perception Encoder (PE-ViT-L+) backbone is distilled into a 40.66M-parameter student through a Feature Pyramid Network, a four-term distillation loss, and sliding-window session pruning.

Result: The compressed pipeline achieves 92.29% MOTA and 96.15% IDF1 on the Edinburgh Pig dataset, with a 7.77-fold reduction in parameters and a 3.01-fold reduction in peak VRAM, and fits within an NVIDIA Jetson Orin NX 16GB envelope.

Conclusion: The proposed compression approach enables a foundation-model pipeline for individual-level livestock monitoring to run on commodity edge accelerators while maintaining high accuracy, supporting on-device embedding-pool re-identification.
...
TL;DR: The paper explores knowledge distillation to transfer the capabilities of large models into smaller, more efficient architectures for plant species and disease recognition, achieving comparable performance at lower computational cost.


<details>
  <summary>Details</summary>
Motivation: State-of-the-art models for plant species and disease recognition are computationally expensive and difficult to deploy on resource-constrained devices, limiting the scalability of automated biodiversity monitoring and precision agriculture systems.

Method: The study uses knowledge distillation to transfer representational capacity from large pretrained models to smaller architectures, evaluating four representative architectures under multiple training regimes.

Result: Knowledge distillation consistently improves performance across tasks and architectures, with distilled models matching the performance of larger models at a substantially lower computational cost.

Conclusion: The findings demonstrate the potential of knowledge distillation techniques to enable efficient and scalable deployment of plant recognition systems in real-world environmental applications.
...
TL;DR: A hybrid quantum-classical U-Net architecture called HQ-UNet is proposed for remote sensing image segmentation, which integrates a compact quantum circuit to enhance feature representation.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of classical deep learning architectures, such as U-Net, which require a large number of parameters, and to explore the potential of quantum machine learning (QML) for remote sensing image segmentation under near-term quantum hardware constraints.

Method: The proposed HQ-UNet architecture integrates a compact parameterized quantum circuit at the bottleneck of a classical U-Net, using a non-pooling quantum convolutional module to enrich highly compressed encoder features before decoding.

Result: Experiments on the LandCover.ai dataset show that HQ-UNet achieves a mean IoU of 0.8050 and an overall accuracy of 94.76%, outperforming the classical U-Net baseline.

Conclusion: The results suggest that compact quantum bottlenecks can enhance feature representation for remote sensing image segmentation under near-term quantum constraints, highlighting the potential of hybrid quantum-classical designs for parameter-efficient dense prediction in Earth observation.
...
<details>
  <summary>Details</summary>
Motivation: Existing ReID methods are often influenced by attributes such as gender, pose, and body mass index (BMI), raising concerns about fairness and generalization.

Method: The authors extend the notion of expressivity using a secondary neural network to quantify how strongly attributes are encoded in transformer-based ReID models.
...
Conclusion: The study highlights that BMI is a significant factor in ReID models, especially in deeper layers, and that attribute encoding trends vary across layers and training epochs.
...
TL;DR: A modular workflow for large-scale mapping of linear woody features from Earth observation data is proposed, enabling scalable and generalizable mapping across different regions.


<details>
  <summary>Details</summary>
Motivation: Linear woody features like hedges provide valuable ecosystem services, and their management and preservation are crucial for climate adaptation and biodiversity. However, mapping these features at a large scale remains a methodological challenge due to the diversity of data sources and landscape variability.

Method: The proposed workflow consists of two independently optimizable components: a flexible input data interface that consolidates heterogeneous Earth observation data into a binary woody vegetation mask, and a deep neural network trained to separate linear from non-linear shapes within these masks.

Result: The workflow produced competitive results in deriving national-scale linear woody feature maps for Germany from three input sources using a single trained model. Evaluation against refined reference data and comparison with existing maps demonstrated the workflow's applicability at a national level.

Conclusion: The modular design and demonstrated applicability of the workflow provide a foundation for scalable and generalizable linear woody feature mapping beyond Germany, addressing the need for large-scale mapping of these valuable ecosystem features.
...
TL;DR: The paper introduces VTBench, a framework for multimodal time-series classification that combines raw sequences with chart-based visualizations, showing competitive results and providing guidelines for selecting chart types and fusion strategies.


<details>
  <summary>Details</summary>
Motivation: Most deep learning models for time-series classification rely solely on raw numerical inputs, overlooking alternative representations such as chart-based visualizations, which offer more interpretable alternatives but have been underexplored.

Method: The authors develop VTBench, a systematic and extensible framework that generates lightweight, human-interpretable plots (line, area, bar, and scatter) and supports multiple fusion strategies, including single-chart visual-numerical fusion, multi-chart visual fusion, and full multimodal fusion with raw inputs.

Result: Through experiments across 31 UCR datasets, the authors show that chart-only models are competitive in selected settings, combining multiple chart types can improve accuracy, and multimodal models improve or maintain performance when visual features provide non-redundant information.
...
TL;DR: An improved model named ALC-YOLOv8s is proposed for student classroom behavior recognition, achieving better performance in complex classroom scenarios.


<details>
  <summary>Details</summary>
Motivation: To address the problems of dense student targets, numerous small objects, frequent occlusions, and imbalanced class distribution in real classroom scenes.

Method: The model introduces SPPF-LSKA, employs CFC-CRB and SFC-G2, and incorporates ATFLoss to improve the learning ability for minority classes and hard samples.

Result: The improved model achieves increases of 1.8% in mAP50 and 2.1% in mAP50-95 compared with the baseline model.

Conclusion: The proposed model can well meet the requirements of automatic student behavior recognition in complex classroom scenarios.
...
TL;DR: YOSE, an efficient fine-tuning framework for Diffusion Transformer-based video object removal, achieves up to 2.5X speedup while maintaining visual quality.


<details>
  <summary>Details</summary>
Motivation: Existing Diffusion Transformer-based video object removal methods suffer from substantial inference latency due to dense computations over the entire spatiotemporal token space.
...
Result: YOSE achieves up to 2.5X speedup in 70% of cases while maintaining visual quality comparable to the baseline, with inference time scaling approximately linearly with the masked regions.

Conclusion: YOSE presents an efficient solution for Diffusion Transformer-based video object removal, addressing the issue of inference latency while preserving visual quality.
...
TL;DR: A new framework for improving zero-shot web content classification by refining category definitions using LLMs as feedback-driven optimizers.


<details>
  <summary>Details</summary>
Motivation: The dynamic nature of the web makes accurate classification difficult; existing zero-shot approaches are sensitive to definition quality.

Method: A training-free, adaptive iterative definition refinement framework using LLMs to optimize category definitions.

Result: Iterative definition refinement consistently improves classification performance across diverse architectures.

Conclusion: Definition quality is a critical factor in embedding-based systems; the proposed framework effectively improves zero-shot web content classification.
...
TL;DR: JI-ADF, a trimodal deep learning framework, integrates dermoscopic images, clinical photographs, and patient metadata for skin lesion classification, showing strong performance and robustness in real-world clinical settings.
...
Result: JI-ADF demonstrates strong and well-balanced performance across lesion categories, improving sensitivity and Dice score while maintaining high specificity and good calibration.
...
TL;DR: A new framework for gait recognition is proposed, which combines gait dynamics and body shape characteristics through a multi-branch architecture, achieving high accuracy under various conditions.
...
Conclusion: The proposed framework provides a high-precision gait recognition solution that effectively extracts and fuses gait dynamics and body shape characteristics, demonstrating superior performance under various conditions.
...
Method: CasLayout, a cascaded diffusion framework that decomposes the joint scene generation task into four conditional sub-stages with explicit physical and semantic roles.
...
Method: The proposed DSCC framework consists of two stages: (1) grouping spectral similar and spatially proximate pixels into spectral supertokens, and (2) performing token-level prediction. It uses a multi-criteria feature distance, locality-aware assignment regularization, and density-isolation based center selection to generate boundary-preserving spectral supertokens.
...
Conclusion: The proposed DSCC framework effectively addresses the limitations of existing superpixel-based methods and achieves region-level, boundary-aligned classification for hyperspectral image classification.
...
TL;DR: CriticVLA, a two-stage framework, enhances vision language action models for autonomous driving by adding a critic capability to refine driving decisions, achieving a 73.33% success rate.


<details>
  <summary>Details</summary>
Motivation: Previous vision language action (VLA) models for autonomous driving did not leverage their critic capability to refine driving decisions, limiting their performance in complex scenarios.
...
Result: CriticVLA achieves a 73.33% total success rate on the Bench2Drive benchmark, with about 30% improvement in challenging scenarios.

Conclusion: The proposed CriticVLA framework significantly improves the performance of VLA models in autonomous driving by effectively utilizing their critic capability.
...
TL;DR: VeraRetouch, a lightweight, fully differentiable framework for photo retouching, achieves state-of-the-art performance with a small footprint, enabling mobile deployment.
...
Method: VeraRetouch employs a 0.5B Vision-Language Model as central intelligence, a fully differentiable Retouch Renderer, and a reinforcement learning post-training strategy.

Result: VeraRetouch achieves state-of-the-art performance across multiple benchmarks with a significantly smaller footprint.

Conclusion: VeraRetouch offers a novel solution for photo retouching, overcoming existing challenges with its lightweight and fully differentiable framework.
...
TL;DR: The paper proposes a new benchmark called COHERENCE to evaluate the ability of Multimodal Large Language Models (MLLMs) to understand interleaved image-text contexts and recover fine-grained image-text correspondences.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks for MLLMs focus on single-image or multi-image comprehension, but real-world scenarios often require understanding interleaved multimodal contexts, which is a gap that needs to be addressed.

Method: The authors propose a new benchmark called COHERENCE, which covers interleaved image-text content from four representative domains and contains 6,161 high-quality questions, and perform a six-type error analysis to attribute failures in interleaved image-text understanding.

Result: The paper presents a new benchmark and error analysis framework, but does not report specific results for MLLMs on the COHERENCE benchmark.

Conclusion: The paper highlights the need for a systematic benchmark to evaluate the fine-grained understanding ability of MLLMs in interleaved image-text contexts and proposes COHERENCE as a solution to address this gap.
...
TL;DR: The paper studies the vulnerability of vision-language models (VLMs) used in autonomous driving to physical adversarial attacks, specifically the transferability of such attacks across different VLM architectures.


<details>
  <summary>Details</summary>
Motivation: The motivation is to understand the robustness of VLMs to physical adversarial attacks and the potential risk when attackers do not know which model a vehicle uses.

Method: The study evaluates three representative VLM architectures (Dolphins, OmniDrive, and LeapVAD) using physically realizable patches placed on roadside infrastructure in both crosswalk and highway scenarios.

Result: The results show high cross-architecture effectiveness, with transfer rates of 73-91% and sustained frame-level manipulation over 64.7-79.4% of the critical decision window.

Conclusion: The study concludes that VLMs used in autonomous driving are vulnerable to physical adversarial attacks, and such attacks can transfer across different VLM architectures, posing a practical risk.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for novel-view synthesis are limited by constrained or dense image collections; this work addresses the need for handling sparse, unconstrained collections.
...
Result: The proposed method outperforms existing methods by 17.2% (PSNR), 10.8% (SSIM), and 4.0% (LPIPS) and provides high-fidelity 3D rendering results.

Conclusion: The proposed framework enables high-quality 3D rendering from sparse, unconstrained image collections, paving the way for unconstrained real-world scenarios.
...
TL;DR: A novel method called Softmax-GS is proposed to improve 3D Gaussian Splatting for novel view synthesis by addressing view inconsistencies and diffuse boundaries.


<details>
  <summary>Details</summary>
Motivation: The efficiency of 3D Gaussian Splatting relies on the assumption that Gaussians do not overlap, leading to artifacts and inconsistencies, and the diffuse boundaries of Gaussians hinder accurate edge reconstruction.

Method: Softmax-GS enforces a softmax-based competition between overlapping Gaussians with learnable parameters to control competition strength, preserving order invariance and output transmittance.

Result: Softmax-GS achieves state-of-the-art performance, improving reconstruction quality and parameter efficiency on real-world benchmarks.

Conclusion: Softmax-GS provides a unified solution for addressing view inconsistencies and diffuse boundaries in 3D Gaussian Splatting, leading to improved performance.
...
TL;DR: A Bayesian-inspired framework, CatSignal, is proposed for multimodal intent inference in non-speaking agents, such as household pets. It models spatial context as a prior-like constraint and behavioral observations as evidence, achieving 77.72% accuracy in a household cat setting.


<details>
  <summary>Details</summary>
Motivation: Many agents, like household pets and pre-verbal infants, cannot reliably communicate their goals through language, making it necessary to infer intent from incomplete behavioral observations in context-rich environments.

Method: The CatSignal framework uses a context-gated Product-of-Experts formulation to compute posterior-like intent distributions from context, pose dynamics, and acoustic cues, treating context as a prior-like constraint rather than an ordinary input feature.

Result: The proposed framework achieves 77.72% accuracy under Leave-One-Video-Out evaluation on a multimodal domestic cat dataset, outperforming baselines and substantially reducing context-driven shortcut failures in ambiguous cases.

Conclusion: The CatSignal framework provides a robust approach for multimodal intent inference in non-speaking agents, offering the best overall accuracy and suppression of context-based shortcut collapse.
...
<details>
  <summary>Details</summary>
Motivation: To reduce reliance on large amounts of 3D annotated data for camera pose estimation by leveraging self-supervised pretraining.

Method: LA-Pose employs inverse- and forward-dynamics models to learn latent action representations from driving videos, then repurposes these features as inputs to a camera pose estimator finetuned on limited high-quality 3D annotations.

Result: LA-Pose achieves over 10% higher pose accuracy than recent feed-forward methods on Waymo and PandaSet benchmarks, using orders of magnitude less labeled data.

Conclusion: This work demonstrates the effectiveness of inverse-dynamics self-supervised learning for pose estimation, achieving competitive and superior performance with reduced labeled data.
...
Result: EdgeFM achieves up to 1.49 times speedup over TensorRT-Edge-LLM on the NVIDIA Orin platform and delivers favorable end-to-end inference performance, making it a production-grade solution for diverse edge industrial scenarios.

Conclusion: EdgeFM provides an open-source, production-grade solution for efficient deployment of vision-language models on industrial edge devices, enhancing cross-platform portability and achieving better inference performance.
...
TL;DR: A unified framework, Uni-HOI, is proposed for modeling 4D human-object interaction (HOI) that integrates text, human motion, and object motion.


<details>
  <summary>Details</summary>
Motivation: Existing HOI models are task-specific and lack a unified framework for handling diverse conditional inputs.

Method: Uni-HOI uses large language models and motion-specific vector quantized variational autoencoders to convert motion data into token sequences for joint modeling.

Result: Uni-HOI achieves remarkable performances on multiple HOI-related tasks including text-driven HOI generation and object motion-driven human motion generation.

Conclusion: The proposed Uni-HOI framework provides a unified approach for handling diverse HOI tasks with improved performance.
...
TL;DR: The paper presents IRON dataset and IRONet for infrared off-road temporal freespace detection, achieving state-of-the-art performance and real-time inference.


<details>
  <summary>Details</summary>
Motivation: Off-road nighttime autonomous driving faces challenges due to unreliable visible-light perception, making infrared modality crucial for accurate freespace detection.

Method: The authors propose IRONet, a novel flow-free framework that aggregates historical context via a memory-attention mechanism and a carefully designed mask decoder.

Result: IRONet achieves 82.93% IoU and 90.66% F1 score on the IRON dataset, with robust generalization to RGB modalities on ORFD and Rellis datasets.
...
<details>
  <summary>Details</summary>
Motivation: Current generative models struggle to produce voluminous 3D assets from flat 2D images due to limited 3D cues.
...
Conclusion: The proposed REVIVE 3D pipeline effectively generates voluminous 3D assets from flat 2D images, with validated metrics for volume and surface flatness.
...
TL;DR: Introduced Edit-R1, a framework for image editing that uses a reasoning verifier-based reward model to provide fine-grained rewards.


<details>
  <summary>Details</summary>
Motivation: The lack of a robust general reward model for image editing tasks, which existing models fail to address by providing overall scores without detailed checks.

Method: Edit-R1 builds a chain-of-thought verifier-based reasoning reward model (RRM) and leverages it for downstream image editing through supervised fine-tuning and Group Contrastive Preference Optimization.

Result: Edit-RRM surpasses powerful VLMs as an editing-specific reward model, showing a clear scaling trend with performance improvements from 3B to 7B parameters, and delivers gains to editing models like FLUX.1-kontext.

Conclusion: The proposed Edit-R1 framework effectively enhances image editing by introducing a more detailed and interpretable reward model.
...
TL;DR: The paper challenges the Spatial Funnel Hypothesis in CNN interpretability, proposing a hallucination-free inversion framework that reveals strong superposition in vision encoders and a destructive interference model for classification.
...
Method: The authors introduce a hallucination-free inversion framework built on magnitude-phase decoupling and Local Adjoint Correctors, which guarantees that the spatial gradient support of every reconstruction stems strictly from genuinely active channels.

Result: The framework reveals strong superposition in vision encoders, showing that classification operates via destructive interference, and leads to a covariance-volume channel selection algorithm with a (1-1/e) approximation guarantee.

Conclusion: The paper disproves the Spatial Funnel Hypothesis, provides a new understanding of classification as destructive interference, and offers a method for channel selection and understanding OOD failure.
...
TL;DR: The paper explores self-supervised learning for plant image recognition, finding that domain-specific augmentations and datasets improve performance, and achieving competitive results in few-shot settings.
...
Method: The paper investigates self-supervised learning for plant image representation learning, identifying suitable augmentations (affine and posterization) and demonstrating the importance of domain-specific data (iNaturalist 2021 Plantae subset).

Result: The models achieve competitive performance and sometimes outperform strong supervised baselines (Pl@ntCLEF and BioCLIP) on downstream plant recognition tasks in few-shot settings, across ViT-Base and ViT-Large architectures.

Conclusion: The paper highlights the critical importance of domain-adapted augmentation strategies and dataset selection in self-supervised learning, providing practical guidelines for building scalable models for biodiversity monitoring.
...
TL;DR: The paper introduces Residual Gaussian Splatting (RGS), which integrates wavelet multi-resolution analysis with 3D Gaussian splatting for improved cone-beam computed tomography reconstruction under ultra sparse-view conditions.


<details>
  <summary>Details</summary>
Motivation: Conventional photometric optimization in 3DGS suffers from spectral bias, leading to over-smoothing and loss of high-frequency anatomical details under ultra sparse-view conditions.

Method: The proposed RGS method uses a spectrally-decoupled Gaussian representation to stratify the volumetric field into a geometric base component and a residual detail component, and a spectral-spatial collaborative optimization strategy to coordinate the interplay between geometric anchoring and texture refinement.
...
TL;DR: The style of text in an image affects the descriptions provided by Large Visual Language Models (LVLMs) for a concept, even when the concept is correctly identified.


<details>
  <summary>Details</summary>
Motivation: To investigate whether the style in which a word is visualized in an image impacts the description that an LVLM provides for the concept to which that word refers.

Method: Experiments were conducted to study the effect of functional text styles (readability-oriented) versus decorative styles (display-oriented) on LVLMs' descriptions of a concept in terms of the attributes of that concept.

Result: The experimental results reveal that even when the concept is correctly identified, text style influences the model's attribute-based descriptions of the concept.

Conclusion: The findings demonstrate non-trivial style leakage from text style into semantic inference and motivate style-aware evaluation and mitigation for LVLM-based multimedia systems.
...
TL;DR: The authors propose RIHA, a novel framework for radiology report generation that aligns radiological images with reports at multiple levels, outperforming state-of-the-art models.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of fine-grained alignment between complex visual features and the hierarchical structure of long-form radiology reports, which is essential for accurate radiology report generation.

Method: The authors propose RIHA, which consists of a Visual Feature Pyramid (VFP), a Text Feature Pyramid (TFP), and a Cross-modal Hierarchical Alignment (CHA) module, leveraging optimal transport for multi-level alignment.

Result: RIHA outperforms existing state-of-the-art models on two benchmark chest X-ray datasets, IU-Xray and MIMIC-CXR, in both natural language generation and clinical efficacy metrics.

Conclusion: The proposed RIHA framework effectively addresses the challenge of fine-grained alignment in radiology report generation, leading to improved performance and accuracy.
...
TL;DR: Proposed World2Minecraft to convert real-world scenes into Minecraft environments for embodied AI research, introducing a data acquisition pipeline and large-scale dataset MinecraftOcc to improve occupancy prediction.
...
Conclusion: World2Minecraft provides a customizable and editable platform for personalized embodied AI research, contributing to improved occupancy prediction and highlighting the value of high-fidelity simulation environments.
...
TL;DR: The paper introduces a dataset and challenge for AI in pancreatic cancer staging, highlighting the importance of uncertainty-aware models for accurate vascular invasion assessment.


<details>
  <summary>Details</summary>
Motivation: The need for accurate assessment of vascular invasion in pancreatic ductal adenocarcinoma (PDAC) for preoperative staging and surgical planning, and the limitations of current computational methods due to lack of public datasets and diagnostic ambiguity.

Method: The introduction of the CURVAS-PDACVI Dataset and Challenge, and a multi-metric evaluation framework that includes probabilistic calibration and VI assessment, to evaluate six state-of-the-art methods.

Result: The evaluation shows that methods optimized for binary segmentation perform well on average overlap metrics but degrade in high-complexity cases, while methods that model inter-rater disagreement produce better calibrated probabilistic maps and show greater robustness.

Conclusion: The benchmark highlights the limitations of volumetric accuracy as a proxy for localized surgical utility, and motivates the use of uncertainty-aware probabilistic models for preoperative decision-making.
...
TL;DR: This paper introduces the concept of 3D fake detection and presents a dataset, Fake3DGS, to evaluate the detection of manipulated 3D content. It also proposes a 3D-aware detection method to improve the identification of fake 3D images.


<details>
  <summary>Details</summary>
Motivation: The recent advances in 3D reconstruction and neural rendering have made it feasible to edit 3D scenes and re-render them as highly realistic images, raising security concerns about the authenticity of 3D content. However, 3D fake detection remains largely unexplored, and existing work is limited to 2D space.

Method: The authors introduce a 3D-aware detection method that leverages multi-view coherence and features derived from the Gaussian splatting representation. They also create a dataset, Fake3DGS, which includes 3D Gaussian splatting scenes and corresponding rendered views with controlled manipulations of geometry, appearance, and spatial layout.

Result: The experimental results demonstrate that current state-of-the-art 2D detectors struggle to distinguish between original and 3D manipulated images, while the proposed 3D-aware detection method shows a substantial improvement in recognizing modified 3D content.

Conclusion: The paper concludes that there is a need for authenticity assessment techniques that extend beyond 2D evidence, and the introduced dataset and method can help bridge the gap in 3D fake detection.
...
TL;DR: The paper proposes ClipTBP, a framework for video moment retrieval that improves multimodal alignment by considering relationships between multiple answer segments and predicting accurate temporal boundaries.


<details>
  <summary>Details</summary>
Motivation: Existing models for video moment retrieval are influenced by visually similar segments in the surrounding context and struggle to exclude irrelevant segments because they calculate similarity at the snippet-level and ignore relationships between multiple answer segments.

Method: The proposed framework, ClipTBP, uses a clip-pair temporal boundary prediction approach with boundary-aware learning, including a clip-level alignment loss and both main and auxiliary boundary losses.
...
Conclusion: The proposed ClipTBP framework effectively addresses the limitations of existing models for video moment retrieval by improving multimodal alignment and boundary prediction.
...
TL;DR: A new benchmark called SPUR is introduced for evaluating the ability of multimodal large language models (MLLMs) to perceive, understand, and reason about scientific images.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the lack of a comprehensive benchmark for evaluating MLLMs' ability to interpret scientific images, which is a crucial aspect of AI for Science (AI4S) research.

Method: The authors propose a new benchmark called SPUR, which consists of 4,264 QA pairs derived from 1,084 expert-curated images. SPUR features three key innovations: Panel-Level Fine-Grained Perception, Cross-Panel Relation Understanding, and Expert-Level Reasoning.

Result: The evaluation of 20 MLLMs and four multimodal Chain-of-Thought (MCoT) methods using SPUR reveals that current models fall significantly short of the expert-level requirements for scientific image interpretation.

Conclusion: The authors conclude that there is a critical bottleneck in AI for Science (AI4S) research, and that current MLLMs are not yet capable of interpreting scientific images at an expert level.
...
TL;DR: The paper proposes a lightweight convolutional neural network for automatic crack detection in bridge structural health monitoring using UAVs, addressing challenges such as weak crack features and class imbalance.


<details>
  <summary>Details</summary>
Motivation: The widespread application of UAVs in bridge structural health monitoring has led to a focus on deep learning-based automatic crack detection, but practical inspections face challenges such as weak crack features, degraded imaging conditions, severe class imbalance, and limited computational resources.

Method: The paper proposes a unified lightweight convolutional neural network framework composed of four synergistic components: a lightweight backbone network, a Convolutional Block Attention Module (CBAM), a directed robust augmentation strategy, and Focal Loss for hard-sample learning.
...
<details>
  <summary>Details</summary>
Motivation: The authors argue that adapting VLMs to VLN requires endowing them with backward action reasoning and forward transition prediction capabilities to acquire spatial awareness in unseen 3D environments.

Method: The authors propose SpaAct, a training framework that includes two spatial activation tasks: Action Retrospection and Future Frame Selection, along with TriPA, a Tri-factor Progressive Adaptive curriculum learning method.
...
Conclusion: The proposed SpaAct framework effectively enhances VLMs for VLN by activating dynamic spatial awareness, leading to improved navigation performance in unseen 3D environments.
...
TL;DR: A novel end-to-end framework, Focal U-shaped Network (FUN), is proposed for joint hyperspectral image reconstruction and object detection, achieving state-of-the-art performance with reduced parameters and computation.


<details>
  <summary>Details</summary>
Motivation: Conventional push-broom hyperspectral imaging is slow and doesn't allow for real-time object detection, while snapshot spectral imaging enables instantaneous capture but is hindered by time-consuming reconstruction. The authors aim to address this issue.

Method: The Focal U-shaped Network (FUN) employs a shared U-shaped backbone for joint HSI reconstruction and object detection via multi-task learning, introducing focal modulation as an efficient alternative to self-attention.
...
Conclusion: The proposed FUN framework shows promise for future real-time edge deployment, and a new HSI object detection dataset with 8712 annotated objects across 363 HSIs is contributed to facilitate evaluation.
...
TL;DR: A new dataset and framework for CT-MRI registration of the cervical spine are proposed, addressing challenges in this complex and variable region.


<details>
  <summary>Details</summary>
Motivation: The cervical spine is anatomically complex and highly variable, making accurate CT-MRI registration crucial for preoperative planning, but current methods are underexplored and limited by lack of high-quality data.

Method: A rigid-deformable hybrid registration framework, MSR, is proposed, consisting of a rigid registration module and a deformable registration module with a novel MSL block.

Result: The proposed MSR framework and R-D-Reg dataset are made publicly available, aiming to improve CT-MRI registration in the cervical spine.

Conclusion: The proposed framework and dataset have the potential to improve preoperative planning and treatment outcomes for cervical spine injuries and conditions.
...
TL;DR: EviMem, a new framework for long-term conversational memory, improves retrieval accuracy for temporal and multi-hop questions by detecting evidence gaps and refining queries.


<details>
  <summary>Details</summary>
Motivation: Long-term conversational memory requires retrieving evidence from multiple sessions, but existing methods fail on temporal and multi-hop questions, and iterative methods lack targeted query refinement.
...
Conclusion: EviMem presents a novel approach to long-term conversational memory, outperforming existing methods in terms of accuracy and latency.
...
TL;DR: The study proposes a deep learning approach to automatically segment radiological Peritoneal Cancer Index (rPCI) regions on CT scans, achieving promising results with nnU-Net outperforming Swin UNETR.


<details>
  <summary>Details</summary>
Motivation: Current assessment of peritoneal metastases using Sugarbaker's Peritoneal Cancer Index (sPCI) is invasive and lacks a standardized imaging counterpart. The study aims to develop a non-invasive, imaging-based assessment method.

Method: The study evaluates two deep learning models, nnU-Net and Swin UNETR, on 62 CT scans with manually annotated rPCI regions. Performance is assessed using Dice Similarity Coefficient, 95th percentile Hausdorff distance, and Average Surface Distance.
...
Conclusion: The proposed method improves reconstruction quality in video snapshot compressive imaging by effectively capturing content structural similarities and enhancing spatial smoothness.
...
Result: State-of-the-art accuracies are achieved on four semantic segmentation datasets: 67.4% mIoU for iSAID, 56.9% mIoU for MFNet, 84.22% mIoU for PST900, and 91.88% mF1 for Potsdam.

Conclusion: The proposed pre-training strategy lays the groundwork for developing a unified foundation model applicable to both computer vision and remote sensing applications.
...
TL;DR: The paper proposes a novel approach for scene-text image captioning in Vietnamese, a tonal language with unique challenges, by introducing linguistically informed multimodal fusion and a graph fusion framework.


<details>
  <summary>Details</summary>
Motivation: The authors are motivated by the challenges of scene-text image captioning in Vietnamese, including OCR errors, ambiguous word boundaries, and the importance of diacritics in word meaning.

Method: The authors propose HSTFG (Heterogeneous Scene-Text Fusion Graph) and PhonoSTFG (Phonological Scene-Text Fusion Graph), a graph fusion framework with learned spatial attention bias, and a specialized graph-level fusion for Vietnamese linguistic reasoning.

Result: The authors introduce ViTextCaps, the first large-scale Vietnamese scene-text captioning dataset, and show that 52.8% of the vocabulary is at risk of diacritic collision.

Conclusion: The paper concludes that linguistically informed multimodal fusion is necessary for scene-text image captioning in Vietnamese, and proposes a novel approach to address the unique challenges of this language.
...
<details>
  <summary>Details</summary>
Motivation: The motivation behind this work is to address the issue of poorly calibrated models produced by TPT, which raises concerns about the reliability of their predictions.

Method: The method proposed is Flatness-aware Prompt Pretraining (FPP), which initializes prompts within flatter regions of the loss landscape prior to adaptation, without modifying any other components of existing TPT pipelines.
...
Conclusion: The conclusion is that FPP is a highly practical method for real-world deployment, as it is simple, effective, and does not require additional resources.
...
TL;DR: The paper proposes a method for integrating domain knowledge into deep neural networks through a Differentiable Knowledge Unit (DKU), which discovers and integrates implicit concepts from main task supervision to improve generalization.
...
Method: The proposed method uses a DKU that modulates classifier logits using implication rules to represent relationships between task classes and implicit concepts learned from main task supervision.

Result: The method demonstrates improvement in generalization across PASCAL-VOC, COCO, and MedMNIST datasets, and outperforms the baseline in domain generalization and hard-sample ablation studies.

Conclusion: The paper concludes that the proposed method for implicit knowledge discovery and integration can improve generalization in deep neural networks without requiring concept labels.
...
Method: The GourNet model uses Convolutional Neural Networks and is trained on the MangoLeafBD dataset, which consists of 8 classes (7 disease classes and 1 healthy class). The model is optimized through image preprocessing, data augmentation, and a split dataset for training, validation, and testing.

Result: The GourNet model achieves a classification accuracy of 97% using only 683,656 total parameters.

Conclusion: The proposed GourNet model demonstrates high effectiveness in detecting mango leaf diseases, which can contribute to improved crop productivity and food security.
...
TL;DR: This paper proposes a modified SISA framework for class-level unlearning in Convolutional Neural Networks (CNNs), enabling the removal of specific data without complete retraining while preserving model performance.


<details>
  <summary>Details</summary>
Motivation: The increasing reliance on proprietary or private user data for model training raises ethical and legal challenges, particularly when users request data deletion after influencing a trained model.

Method: A modified SISA (Sharded, Isolated, Sliced, and Aggregated) framework with a reinforced replay mechanism and a gating network is proposed for class-level unlearning in CNN architectures.

Result: The modified SISA approach enables effective class unlearning while preserving model performance and reducing retraining overhead, as demonstrated by experimental evaluations across multiple image datasets and CNN configurations.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to address the privacy risks in ProtoPFL by improving the privacy-utility trade-off, as existing methods like Isotropic Gaussian Prototype Perturbation (IGPP) over-perturb discriminative dimensions and struggle to balance clipping threshold with representation fidelity.

Method: The method proposed is VPDR, which consists of two components: (1) Variance-adaptive Prototype Perturbation (VPP), which allocates less noise to discriminative subspaces, and (2) Distillation-guided Clipping Regularization (DCR), which enables feature norms to adaptively concentrate near the predefined clipping threshold.
...
Conclusion: The conclusion is that VPDR provides a better privacy-utility trade-off for ProtoPFL, ensuring privacy while preserving semantic separability and prediction consistency.
...
TL;DR: The authors propose a lightweight strategy to make Convolutional Neural Networks (CNNs) more translation-invariant by inserting Global Average Pooling (GAP) layers, achieving significant reductions in trainable parameters and network size while maintaining competitive accuracy and improving translational robustness.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this work is to address the vulnerability of CNNs to spatial transformations, specifically translation, which can drastically degrade performance even with a single-pixel shift.

Method: The authors propose inserting Global Average Pooling (GAP) layers at various network depths to decouple feature recognition from spatial location, and demonstrate this using VGG-16 as a case study.

Result: The proposed architectural modification achieves a 98% reduction in trainable parameters and a 90% reduction in total network size, while maintaining competitive Top-1 accuracy on ImageNet (66.4%) and doubling translational robustness. The approach also shows significant improvements in Perceptual Image Quality Assessment (IQA) tasks.

Conclusion: The authors conclude that enforcing architectural invariance is a more efficient and biologically plausible path to robustness than traditional data augmentation, and that while GAP resolves macroscopic sensitivity, it introduces a residual periodic aliasing that prevents perfect pixel-level stability.
...
TL;DR: The paper proposes Noise2Map, a unified diffusion-based framework for semantic segmentation and change detection in remote sensing, leveraging denoising processes for discriminative learning.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning models for remote sensing struggle with temporal inconsistencies, capturing fine-grained spatial structures, extensive pretraining, and limited interpretability.

Method: Noise2Map uses a diffusion-based framework with task-specific noise schedules and timestep conditioning, pretrained via self-supervised denoising and fine-tuned with supervision.

Result: Noise2Map ranks 1st among seven models on semantic segmentation and 1st on change detection across datasets, demonstrating robustness against different training noise schedulers and timestep control.

Conclusion: The proposed Noise2Map framework offers a unified approach for semantic segmentation and change detection, providing interpretability, robustness, and state-of-the-art performance.
...
TL;DR: Proposed HiMix framework improves synthetic image detection by enhancing model generalization to unseen generators through a unified approach.


<details>
  <summary>Details</summary>
Motivation: Existing synthetic image detectors have poor generalization to unseen generators due to limited and biased training datasets.

Method: The HiMix framework consists of two modules: Mixup-driven Distributional Augmentation (MDA) for expanding training distribution, and Hierarchical Artifact-aware Representation (HAR) for extracting discriminative forgery representations.
...
Conclusion: The proposed HiMix framework effectively enhances the generalization of synthetic image detectors to unseen generators, providing a more reliable and generalizable solution.
...
TL;DR: A novel framework called TAVR is proposed for generating talking avatars from video references, allowing for cross-scene synthesis and outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing talking avatar methods are limited by their reliance on single-view, same-scene reference images, which lack temporal and expression cues.

Method: TAVR integrates a token selection module and a three-stage training scheme, including same-scene video pretraining, cross-scene reference fine-tuning, and task-specific reinforcement learning.

Result: TAVR consistently surpasses existing baselines both quantitatively and qualitatively, and has been deployed to production.

Conclusion: The proposed TAVR framework enables the generation of high-fidelity talking avatars in customized backgrounds, leveraging cross-scene video inputs.
...
TL;DR: A training-free framework, TunnelMIND, is proposed for tunnel inspection, enabling structured defect localization, measurement, and severity grading.


<details>
  <summary>Details</summary>
Motivation: Existing training-free foundation-model pipelines for tunnel inspection only provide coarse open-vocabulary proposals, which are insufficient for direct use in complex tunnel scenes.

Method: TunnelMIND uses language-guided defect proposals and recalibrates their spatial support through dense visual consistency, then reconstructs them into structured defect entities with attributes.

Result: TunnelMIND achieves F1 scores of 0.68, 0.78, and 0.72 on visible, GPR, and road defect tasks, respectively.

Conclusion: TunnelMIND demonstrates that training-free tunnel inspection can move beyond coarse localization to provide structured defect evidence for engineering assessment.
...
TL;DR: A new dynamic cluster-based sampling approach (DynamiCS) is proposed to efficiently train vision-language models by balancing data distribution, particularly focusing on underrepresented long-tail concepts.


<details>
  <summary>Details</summary>
Motivation: Existing efficient pre-training methods for vision-language models may disproportionately remove rare concepts, leading to insufficient representation of long-tail concepts in the training data.
...
Conclusion: The proposed DynamiCS approach effectively balances the data distribution for VLM training, emphasizing underrepresented concepts and improving performance without significantly increasing computational cost.
...
TL;DR: The authors propose a new dataset and framework for video virtual try-on, achieving superior video quality and garment fidelity.


<details>
  <summary>Details</summary>
Motivation: The performance of video virtual try-on models is limited by the scarcity of large-scale in-the-wild triplet data and the improper use of masks.
...
Result: TripVVT achieves superior video quality and garment fidelity while markedly improving generalization to challenging in-the-wild videos.

Conclusion: The authors provide a solid foundation for advancing controllable, realistic, and temporally stable video virtual try-on by publicly releasing the dataset and benchmark.
...
TL;DR: The paper evaluates the performance of visual language models (VLMs) and clustering methods for detecting visual themes related to climate change in social media data.


<details>
  <summary>Details</summary>
Motivation: The growth of digital content on social media platforms has changed how topics are discussed and understood in public discourse, and the authors aim to advance automated visual theme detection.

Method: The authors evaluate notable VLMs using zero-shot image classification and compare their performance to a baseline, and also use clustering as a minimum cost multicut problem to uncover insightful patterns in an unsupervised manner.

Result: The VLMs are not capable of detecting climate change specific classes, but clustering results using image embedding models (ConvNeXt V2 and DINOv2) produce meaningful clusters, with DINOv2 focusing on style differences and abstract categories, and ConvNeXt V2 clustering in more fine-grained ways.

Conclusion: The paper provides extensive evaluations and practical guidance to practitioners for automated visual theme detection, and finds that clustering methods can produce insightful patterns in social media data related to climate change.
...
TL;DR: The paper introduces FineState-Bench, a benchmark for evaluating fine-grained GUI interaction, and proposes FineState-Metrics and a Visual Diagnostic Assistant (VDA) to diagnose and improve visual grounding in large vision-language models.


<details>
  <summary>Details</summary>
Motivation: Current evaluations of large vision-language models (LVLMs) have limitations in assessing fine-grained, state-conditioned GUI interaction, making it challenging to identify where and why agents fail.

Method: The authors propose FineState-Bench, a benchmark with 2,209 instances across desktop, web, and mobile platforms, and introduce FineState-Metrics, a four-stage diagnostic pipeline, and a Visual Diagnostic Assistant (VDA).

Result: The results show that exact goal-state success remains low, with ES-SR@Int peaking at 32.8% on Web and 22.8% on average across platforms, and that VDA localization hints can improve performance.

Conclusion: The paper concludes that there is substantial headroom for improving visual grounding in LVLMs, but overall accuracy is still insufficient for reliable fine-grained state-conditioned interaction.
...
<details>
  <summary>Details</summary>
Motivation: Traditional Shot Boundary Detection (SBD) struggles with complex transitions, yielding corrupted video shots, and the paper aims to address this limitation by formalizing the STD task.

Method: The paper proposes TransVLM, which injects optical flow as a motion prior and uses a feature-fusion strategy to process color and motion representations, and also designs a scalable data engine for robust training.
...
Conclusion: The paper concludes that TransVLM is an effective solution for Shot Transition Detection, and it has been deployed to production.
...
TL;DR: Echo-α, an agentic multimodal reasoning model, unifies lesion localization and clinical reasoning for ultrasound interpretation, outperforming baselines on grounding and diagnosis.


<details>
  <summary>Details</summary>
Motivation: Existing methods for ultrasound interpretation excel at either lesion localization or clinical reasoning, but not both. Specialized detectors offer strong localization but limited reasoning, while multimodal large language models provide flexible reasoning but weak grounding in specialized medical domains.
...
Result: Echo-α outperforms competitive baselines on both grounding and diagnosis on multi-center renal and breast ultrasound benchmarks. Specifically, Echo-α-Grounding attains 56.73%/43.78% F1@0.5 and Echo-α-Diagnosis reaches 74.90%/49.20% overall accuracy on renal/breast ultrasound.
...
TL;DR: The paper presents a structure-aware densification framework for 3D Gaussian Splatting, enabling faster and more accurate novel-view synthesis by anisotropic splitting of Gaussians based on local texture structure.


<details>
  <summary>Details</summary>
Motivation: The standard adaptive density control in 3D Gaussian Splatting often leads to over-blurred high-frequency textures or inefficient over-densification due to its reliance on screen-space positional gradients.

Method: The framework introduces a multi-scale frequency analysis combining structure tensors with Laplacian scale space analysis to estimate dominant frequency at each pixel. It then uses a per-Gaussian, per-axis frequency violation metric $η$ to perform anisotropic splitting.

Result: The method achieves significantly faster convergence and superior reconstruction quality, particularly in high-frequency regions, compared to baseline methods.

Conclusion: The proposed structure-aware densification framework improves the efficiency and accuracy of 3D Gaussian Splatting for real-time novel-view synthesis.
...
<details>
  <summary>Details</summary>
Motivation: The need for scalable compression in bandwidth-adaptive transmission, where current learned codecs are not optimized for rate adaptation.
...
Conclusion: TAFA-GSGC enables efficient scalable compression with up to 9 decodable quality levels and strong compression efficiency.
...
<details>
  <summary>Details</summary>
Motivation: To examine the evolution and current state of 3D reconstruction techniques in manufacturing applications, and to identify research gaps in unified frameworks.
...
Result: Non-contact methods like structured light scanning and stereo vision are widely adopted, with deep learning enhancing accuracy and speed, particularly in feature extraction and matching.

Conclusion: The survey provides a structured framework for understanding current capabilities and future directions in manufacturing-focused 3D reconstruction, highlighting a trend towards hybrid systems.
...
Method: The authors propose a hierarchical rubric with 3 core dimensions (VA, VF, VP) and 15 fine-grained criteria, and develop AesRM with a three-stage progressive scheme.

Result: AesRM outperforms baselines on multiple aesthetics benchmarks, is more robust, and shows clear aesthetic gains over existing aesthetic reward models.

Conclusion: The proposed framework and models improve video aesthetics evaluation and generation, enabling more realistic and visually appealing videos.
...
TL;DR: Proposed UHR-Net for accurate lesion segmentation, addressing challenges of lesions resembling surrounding tissues and small-lesion cues dilution.


<details>
  <summary>Details</summary>
Motivation: Lesion segmentation challenges due to resemblance to surrounding tissues and ill-defined boundaries, causing unstable predictions and under/over-segmentation.
...
Conclusion: UHR-Net effectively addresses lesion segmentation challenges, improving accuracy and refinement in ambiguous regions.
...
TL;DR: The paper proposes S$^2$VAE, a geometry-first latent learning framework that focuses on preserving 3D geometry and physically consistent camera dynamics in visual world modeling systems.


<details>
  <summary>Details</summary>
Motivation: Current visual world modeling systems often fail to preserve underlying 3D geometry or physically consistent camera dynamics, and the authors argue that this limitation lies in the latent representations used to encode geometric structure.

Method: The authors propose a novel type of variational autoencoder using a product of Power Spherical latent distributions, explicitly enforcing hyperspherical structure in the bottleneck to preserve directional and geometric semantics under strong compression.

Result: The results show that geometry-aligned hyperspherical latents consistently outperform conventional Gaussian bottlenecks, particularly in high-compression regimes, across depth estimation, camera pose recovery, and point cloud reconstruction tasks.

Conclusion: The authors conclude that latent geometry is a first-class design choice for physically grounded visual and world models, and that their proposed S$^2$VAE framework can effectively preserve 3D geometry and physically consistent camera dynamics.
...
TL;DR: A novel method called AdvDMD is proposed, which unifies Distribution Matching Distillation (DMD) and Reinforcement Learning (RL) to improve the few-step generation quality of diffusion models.


<details>
  <summary>Details</summary>
Motivation: Diffusion models have high generation quality but require extensive sampling steps. Distillation methods like DMD can reduce sampling steps but suffer from performance degradation. RL can improve few-step generation quality but existing approaches are complex and ineffective.

Method: AdvDMD seamlessly integrates DMD and RL by using an adversarially trained discriminator as a reward model. It trains on intermediate and final states of the denoising process and updates online with the distilled model.

Result: AdvDMD outperforms state-of-the-art methods, achieving superior performance on multiple benchmarks, including DPG-Bench and GenEval, with reduced sampling steps (4-step and 2-step).

Conclusion: AdvDMD effectively addresses the performance degradation issue of distillation methods and achieves superior few-step generation quality, surpassing the performance of the teacher model.
...
TL;DR: A new end-to-end framework for motion capture from monocular video, which jointly optimizes Video-to-Pose and Pose-to-Rotation stages, reducing rotation error and improving efficiency.


<details>
  <summary>Details</summary>
Motivation: The existing factorized pipeline for motion capture has limitations due to the ambiguity in joint positions and the non-differentiable IK stage, which prevents adaptation to noisy predictions and optimization for the final animation objective.

Method: A fully end-to-end framework with learnable Video-to-Pose and Pose-to-Rotation stages, using a reference pose-rotation pair to resolve ambiguity and a skeleton-aware Global-Local Graph-guided Multi-Head Attention (GL-GMHA) module for joint-level local reasoning and global coordination.

Result: The proposed method reduces rotation error from ~17 degrees to ~10 degrees, and to 6.54 degrees on unseen skeletons, while achieving ~20x faster inference than mesh-based pipelines.

Conclusion: The proposed end-to-end framework improves the accuracy and efficiency of motion capture from monocular video, and has the potential to be applied to various applications such as animation and robotics.
...
TL;DR: Introduces 3D-ReGen, a 3D regenerator conditioned on initial 3D shapes for controllable 3D generation, enhancement, reconstruction, and editing.


<details>
  <summary>Details</summary>
Motivation: Most 3D generators have limited controllability, operating in a one-shot fashion. The authors aim to improve controllability and support various 3D tasks.
...
Result: Achieves state-of-the-art performance in controllable 3D generation across several tasks, with improved geometric consistency and fine-grained quality.

Conclusion: 3D-ReGen enables controllable 3D generation, enhancement, reconstruction, and editing with consistent fine-grained details, outperforming existing methods.
...
TL;DR: A novel RAW-to-RGB framework called pHVI-ISPNet is proposed for Night Photography Rendering (NPR), achieving state-of-the-art results in fidelity and visual quality.


<details>
  <summary>Details</summary>
Motivation: Existing NPR methods have considerable perceptual gaps and often detract from visual quality due to extreme contrast between dark and illuminated areas in scenes.

Method: The pHVI-ISPNet framework integrates four key refinements: RAW-domain feature processing, Wavelet-based feature propagation, sample-based dynamic loss coefficients, and a loss term based on feature distributions.
...
TL;DR: A novel method for topology-preserving learning in deep learning tasks, enabling differentiable topological inference and improved structural accuracy.
...
Method: A novel method that directly computes simple points on continuous-valued images, enabling differentiable topological inference, and a variational model that enforces topological constraints.
...
Conclusion: The proposed approach effectively improves topological integrity and structural accuracy, and can be seamlessly integrated into any deep neural network segmentation with softmax or sigmoid outputs.
...
Result: PhyCo significantly improves physical realism on the Physics-IQ benchmark and demonstrates clearer control over physical attributes in human studies.
...
TL;DR: AEGIS is a benchmark for evaluating forensic analysis of AI-generated academic images, highlighting limitations in current forensic methods.


<details>
  <summary>Details</summary>
Motivation: The rapid advancement of AI-generated images necessitates robust forensic analysis tools, but current methods have limitations that need to be addressed.

Method: AEGIS benchmark features domain-specific complexity, diverse forgery simulations, and multi-dimensional forensic evaluation, testing 25 MLLMs, 9 expert models, and 1 unified multimodal model.

Result: Current forensic methods show significant limitations, with GPT-5.1 achieving 48.80% performance and expert models achieving only 30.09% localization accuracy, and 11 generative models yielding forensic accuracy below 50%.

Conclusion: AEGIS exposes fundamental limitations in academic image forensics, highlighting the need for improved forensic analysis tools to keep pace with generative advances.
...
TL;DR: The authors propose a method to improve bronchoscopic navigation by using patient-specific respiratory modeling, eliminating the need for breath-hold protocols and achieving accurate localization during respiratory cycles.


<details>
  <summary>Details</summary>
Motivation: Respiratory motion causes deformation in the airway, limiting localization accuracy in bronchoscopic navigation. Current breath-hold protocols are difficult to reproduce and disrupt clinical workflow.

Method: The authors leverage patient-specific respiratory modeling by registering paired inhale-exhale CT scans and embedding this representation within a mesh-anchored Gaussian splatting framework. A lightweight estimator infers breathing phase directly from endoscopic RGB.
...
Conclusion: The proposed method improves bronchoscopic navigation accuracy by accounting for respiratory motion, eliminating the need for breath-hold protocols, and achieving clinically relevant localization accuracy.
...
TL;DR: The paper discusses the limitations of current visual generation models and proposes a shift towards intelligent visual generation, introducing a five-level taxonomy and analyzing key technical drivers and evaluation methods.


<details>
  <summary>Details</summary>
Motivation: The authors argue that current visual generation models struggle with spatial reasoning, persistent state, long-horizon consistency, and causal understanding, and propose a shift towards intelligent visual generation that is grounded in structure, dynamics, domain knowledge, and causal relations.

Method: The authors introduce a five-level taxonomy: Atomic Generation, Conditional Generation, In-Context Generation, Agentic Generation, and World-Modeling Generation, and analyze key technical drivers including flow matching, unified understanding-and-generation models, and improved visual representations.

Result: The authors provide a roadmap for advancing the next generation of intelligent visual generation systems, including a capability-centered lens for understanding, evaluating, and advancing these systems.

Conclusion: The authors conclude that the field of visual generation should move beyond appearance synthesis towards intelligent visual generation, and provide a framework for achieving this goal.
...
<details>
  <summary>Details</summary>
Motivation: The impracticality of Fréchet Distance (FD) as a training objective and the need for better evaluation metrics for generative models.

Method: Decoupling population size from batch size for FD estimation, termed as FD-loss, and optimizing it in different representation spaces.

Result: Optimizing FD-loss improves visual quality, achieves 0.72 FID on ImageNet 256x256, and reveals that FID can misrank visual quality.

Conclusion: The work encourages further exploration of distributional distances in diverse representation spaces for generative models, and proposes a multi-representation metric FDr$^k$.
...
TL;DR: GenWildSplat, a feed-forward framework for reconstructing 3D scenes from sparse, unposed images in outdoor settings with varying illumination and transient occlusions.


<details>
  <summary>Details</summary>
Motivation: Existing methods for 3D scene reconstruction rely on scene-specific optimization, which requires extensive per-scene training and fails under sparse views. There is a need for a method that generalizes across diverse illumination and occlusion patterns.

Method: GenWildSplat uses learned geometric priors to predict depth, camera parameters, and 3D Gaussians in a canonical space. It also includes an appearance adapter for target lighting conditions and semantic segmentation for transient objects.
...
Conclusion: GenWildSplat offers a feed-forward framework for sparse-view outdoor reconstruction that requires no per-scene optimization, generalizing across diverse illumination and occlusion patterns.
...
TL;DR: A unified driving world model, HERMES++, is proposed to integrate 3D scene understanding and future geometry prediction for autonomous driving.


<details>
  <summary>Details</summary>
Motivation: Existing approaches focus on future scene generation but overlook comprehensive 3D scene understanding, while Large Language Models (LLMs) lack the capacity to predict future geometric evolution.

Method: HERMES++ uses a BEV representation, LLM-enhanced world queries, a Current-to-Future Link, and a Joint Geometric Optimization strategy to integrate 3D scene understanding and future geometry prediction.

Result: HERMES++ achieves strong performance, outperforming specialist approaches in both future point cloud prediction and 3D scene understanding tasks on multiple benchmarks.

Conclusion: The proposed HERMES++ model effectively bridges the gap between semantic interpretation and physical simulation, providing a unified driving world model for autonomous driving.
...
<details>
  <summary>Details</summary>
Motivation: Existing research on algorithmic color palette generation and colorization ignores cultural aspects of color, which is an essential component of graphic design.

Method: The authors construct a color dataset inspired by CYS and develop an interactive multi-modal generative framework to create CYS-styled color palettes and colorize images.

Result: The proposed framework is evaluated through user studies, demonstrating its ability to generate CYS-styled color palettes and colorized images.

Conclusion: The paper contributes to the research on algorithmic color palette generation and colorization by considering cultural implications of color, specifically through the lens of Chinese Youth Subculture.
...
TL;DR: The paper introduces a new task of digital battery passport (DBP) conformance classification and a public benchmark dataset called BatteryPass-12K. It evaluates 22 language models on this task, finding that thinking models perform best and few-shot examples improve performance.


<details>
  <summary>Details</summary>
Motivation: The EU's battery regulation on DBPs is coming into effect, but there is no public dataset for DBP conformance classification. The authors aim to address this gap by introducing a novel task and a public benchmark dataset.

Method: The authors evaluate 22 language models, including small LMs, mixture of experts, and dense LLMs, on the DBP conformance classification task using zero-shot inference, few-shot inference, and prompt-injection attacks.

Result: The results show that thinking models have the best performance, with GPT-5.4 scoring 0.98 and 0.71 on the validation and test sets, respectively. Few-shot examples improve performance significantly, but merely scaling model parameters does not necessarily lead to improved performance.

Conclusion: The authors conclude that the introduced dataset, BatteryPass-12K, may be useful for other tasks in the battery domain, and they publicly release it under a permissive license. They also highlight the challenges of DBP conformance classification and the need for further research.
...
TL;DR: The paper introduces CL-bench Life, a benchmark for evaluating the ability of language models to learn from complex, real-life contexts. The results show that current frontier language models struggle with real-life context learning, achieving a low task solving rate.


<details>
  <summary>Details</summary>
Motivation: The motivation is to investigate whether current frontier language models can reliably learn from messy, fragmented, and personal contexts, and to advance real-life context learning for more intelligent and reliable AI assistants.

Method: The authors introduce CL-bench Life, a human-curated benchmark comprising 405 context-task pairs and 5,348 verification rubrics, and evaluate ten frontier language models on this benchmark.

Result: The results show that even the best-performing model achieves only 19.3% task solving rate, while the average performance across models is only 13.8%, indicating that real-life context learning remains highly challenging for current language models.

Conclusion: The paper concludes that CL-bench Life provides a crucial testbed for advancing real-life context learning, and progress on it can enable more intelligent and reliable AI assistants in everyday life.
...
TL;DR: The paper introduces CarryOnBench, a benchmark to evaluate LLMs' ability to recover helpfulness and safety through multi-turn conversations. It reveals that models can recover utility with benign clarifications, but with varying costs and failure modes.


<details>
  <summary>Details</summary>
Motivation: Current LLM safety alignment techniques overlook the ability of LLMs to recover helpfulness when users clarify their intent. The authors aim to investigate whether and how LLMs can revise their interpretation of user intent and recover utility while remaining safe.

Method: The authors introduce CarryOnBench, an interactive benchmark that simulates 5,970 conversations with 14 models, evaluating intent-aligned utility and safety. They design a metric called Ben-Util to assess how well each model response fulfills the user's benign information need.

Result: The results show that models can recover utility with benign clarifications in multi-turn conversations, but with varying costs and failure modes. 13 of 14 models approach or exceed the single-turn baseline, but three failure modes are identified: utility lock-in, unsafe recovery, and repetitive recovery.

Conclusion: The paper concludes that single-turn evaluations miss a crucial aspect of LLM performance - the ability to respond to clarified user intent. The findings highlight a gap in current evaluation methods and the need for more comprehensive assessments of LLM safety and helpfulness.
...
TL;DR: The study investigates the role of task-specific neurons in large language models, providing evidence for their existence and importance through systematic pruning experiments.


<details>
  <summary>Details</summary>
Motivation: To understand whether neurons in task-specific models contribute uniformly to task performance and to explore the effectiveness of selective pruning based on activation-based selectivity.

Method: Systematic pruning study on language models specialized for mathematical reasoning and code generation, using an activation-based selectivity metric to identify and prune low-contribution neurons.

Result: Selective pruning outperforms random pruning, with a small subset of highly task-specific neurons (~10%) being critical for performance, and a robustness threshold for pruning around 15-20%.

Conclusion: The study provides empirical evidence for neuron specialization in task-specific language models, offering insights into pruning robustness, model redundancy, and post-pruning recoverability.
...
TL;DR: This paper evaluates the language abilities of Claude (Sonnet 4.6) across six languages using a systematic framework based on the Interagency Language Roundtable (ILR) Skill Level Descriptions.


<details>
  <summary>Details</summary>
Motivation: The motivation is to assess the cross-lingual performance of large language models (LLMs) like Claude in a systematic and comprehensive manner, highlighting the importance of ILR-informed expert judgment in evaluating LLM outputs.

Method: The method involves administering 12 semantically equivalent prompt clusters to Claude across six languages, collecting 216 responses, and analyzing them using a two-layer methodology combining automated quantitative metrics with expert ILR qualitative assessment.

Result: The results show cross-lingual variation in Claude's outputs, with differences in response length, creative and affective clusters, and five identified patterns of variation: pragmatic disambiguation strategies, aesthetic and literary tradition divergence, language-internal technical terminology norms, cultural calibration gaps, and language-specific institutional referral behavior.

Conclusion: The study concludes that ILR-informed expert judgment is a novel and valuable evaluation methodology that complements computational benchmarks, and that cross-lingual output variation in Claude has significant implications for equitable multilingual AI deployment.
...
TL;DR: The study finds that large language models' hidden states reflect human psychological associations, with semantic feature vectors correlating with human ratings and exhibiting predictable relationships.


<details>
  <summary>Details</summary>
Motivation: To understand how semantic features in large language models relate to human psychological associations
...
Result: High correlations between model projections and human ratings, predictive cosine similarities between semantic axes, and low-dimensional subspace variance

Conclusion: The geometric relations between semantic features in language models mirror human associations, suggesting features should be understood through their relationships and subspaces
...
<details>
  <summary>Details</summary>
Motivation: Current hybrid-thinking language models do not separate think and no-think modes cleanly, causing reasoning leakage and reduced performance in no-think mode.

Method: The authors propose replacing the single MLP in each decoder layer with two semantically locked experts, one for think and one for no-think modes, while keeping other components shared.

Result: PLE maintains strong think performance while producing a substantially stronger no-think mode that is more accurate, more concise, and far less prone to reasoning leakage across math and science reasoning benchmarks.
...
TL;DR: Proposed a bootstrapping approach called Selective Augmentation to improve automatic phonetic transcription (APT) training data by selectively transferring linguistic features from a helper language, achieving significant improvements in accuracy.


<details>
  <summary>Details</summary>
Motivation: The need for clean and diverse training transcriptions in universal automatic phonetic transcription (APT) is hindered by limited high-quality data.

Method: Selective Augmentation, a bootstrapping approach that uses a helper language (Hindi) to augment existing training data for APT, leveraging the MultIPA model.

Result: Increased voicing accuracy by 17.6%, introduced aspiration recognition with 61.2% success rate, and reduced the tenuis class by 32.2%, reducing conflations between plosives.

Conclusion: The proposed Selective Augmentation approach effectively improves APT training data by selectively transferring distinctions between languages, enhancing accuracy and feature recognition.
...
TL;DR: The paper introduces a new benchmark dataset, ASL-MTP, to analyze linguistic phenomena in American Sign Language (ASL) and evaluates a state-of-the-art ASL-to-English translation model, finding it relies heavily on manual cues and misses non-manual cues.


<details>
  <summary>Details</summary>
Motivation: Existing sign language models have improved but it's unclear how well they capture linguistic phenomena and utilize cues from multiple articulators in sign language.

Method: The authors introduce ASL-MTP, a benchmark dataset for ASL, and use it to analyze a state-of-the-art ASL-to-English translation model by ablating input cues and evaluating on linguistic phenomena.

Result: The model performs above chance level on most phenomena but relies strongly on manual cues and often misses crucial non-manual cues.

Conclusion: The study highlights the limitations of current sign language models in capturing linguistic phenomena and utilizing non-manual cues, emphasizing the need for further improvement.
...
TL;DR: This study examines how language models respond to instructions to underperform on multiple-choice evaluations, finding that the complexity of instructions can lead to different strategies, either engaging with question content or relying on positional shortcuts.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this study is to understand how language models behave when instructed to underperform on evaluations, specifically whether they engage with question content or use positional shortcuts.

Method: The study uses a six-condition adversarial instruction-specificity gradient administered to two instruction-tuned LLMs (Llama-3-8B and Llama-3.1-8B) on 2,000 MMLU-Pro items, with distributional screening and content-engagement criteria to characterize each condition.

Result: The results reveal three regimes of behavior: vague instructions lead to moderate accuracy reduction with preserved content engagement, standard instructions lead to positional entropy collapse with partial content engagement, and a two-step answer-aware avoidance instruction leads to extreme positional collapse with no measurable content sensitivity.

Conclusion: The study concludes that instruction complexity determines whether adversarial compliance uses content-aware or content-blind mechanisms in small instruction-tuned LLMs under greedy decoding, and that distributional collapse and content engagement can co-occur.
...
TL;DR: This paper investigates whether Large Language Models (LLMs) can decouple fundamental reasoning patterns from specific problem instances, and explores the tension between parametric and contextual information.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve model controllability, faithfulness, and generalizability by understanding how LLMs acquire and use reasoning capabilities.

Method: The authors conduct a systematic investigation through the lens of reasoning conflicts, evaluating LLMs' prioritization of sensibility over compliance, and probing the encoding of reasoning types in model layers.

Result: The results show that LLMs prioritize sensibility over compliance, rely on internalized parametric memory, and that reasoning conflicts are internally detectable. The authors also demonstrate that active mechanistic interventions can effectively decouple logical schemata from data.

Conclusion: The study concludes that while LLM reasoning is anchored to concrete instances, interventions can improve controllability, faithfulness, and generalizability, offering a path towards more reliable and generalizable LLMs.
...
TL;DR: This paper investigates the effects of subword tokenization on training efficiency and model performance in large language models, and provides insights on how to improve pretraining.
...
Conclusion: The study refines our understanding of why subword models outperform raw byte models and offers insights to improve the pretraining of future byte-level and subword models.
...
TL;DR: The paper studies the impact of input representation on large language models for tasks with explicit 2D structure, comparing text-only and vision-augmented pathways.


<details>
  <summary>Details</summary>
Motivation: The conventional 1D token sequence processing in large language models may introduce representational burden for tasks with 2D structure, leading to the concept of 'serialization friction'.

Method: The study compares a text-only language pathway with a vision-augmented pathway on synthetic tasks with explicit 2D structure, including matrix transpose, Conway's Game of Life, and LU decomposition.

Result: The visual pathway consistently outperforms the textual pathway, with the gap widening at larger dimensions, and error patterns under serialization becoming increasingly spatially structured.
...
<details>
  <summary>Details</summary>
Motivation: Current coding agents relying on external memory for debugging experience often face issues with superficial similarities leading to unsafe memory injection. The authors aim to reframe memory use as a selective, risk-sensitive control problem.

Method: The authors propose RSCB-MC, a risk-sensitive contextual bandit memory controller that assesses the safety and relevance of retrieved memory, storing issue knowledge through a specific schema and converting retrieval evidence into a contextual state.

Result: RSCB-MC achieves a 62.5% success rate with 0.0% false positives in offline replay and a 60.5% success rate with 0.0% false positives in hot-path validation, with a 331.466 microseconds p95 decision latency.

Conclusion: The paper concludes that the key to effective coding-agent memory is not just finding similar memory but ensuring it is safe enough to influence the debugging process, which RSCB-MC addresses with its risk-sensitive approach.
...
TL;DR: The study examines how well Large Language Models (LLMs) capture human emotion annotations, particularly focusing on the distribution of disagreements among human annotators. It finds that zero-shot LLMs diverge significantly from human distributions, but fine-tuning can improve alignment. LLMs perform better on emotions with clear lexical markers but struggle with complex emotions. The study proposes calibration methods to reduce the gap.


<details>
  <summary>Details</summary>
Motivation: Human annotators often disagree on emotion labels, but most evaluations of LLMs use a single gold standard, ignoring the valuable information in these disagreements. The study aims to assess whether LLMs can capture the structure of these disagreements, not just the majority labels.

Method: The study compares the emotion judgment distributions of human annotators with those of four zero-shot LLMs and a fine-tuned RoBERTa baseline across two benchmarks: GoEmotions and EmoBank, with a total of 640,000 LLM responses. It also proposes a lexical-grounding gradient and a quantitative transparency score to predict human-LLM agreement.

Result: Zero-shot LLMs diverge substantially from human distributions, but in-domain fine-tuning improves alignment. LLMs reliably capture emotions with explicit lexical markers but fail on pragmatically complex emotions. Three lightweight post-hoc calibration methods reduce the distributional gap by up to 14%.

Conclusion: The study provides insights into the strengths and limitations of LLMs in capturing human emotion annotations, particularly in terms of disagreement distributions. It offers guidelines on when LLM emotion annotations can substitute for human labeling and proposes methods to improve LLM performance.
...
TL;DR: This paper introduces an emotion-aware clickbait generation attack that uses stylistic transformations to optimize emotional impact, and demonstrates that it can significantly degrade the performance of state-of-the-art clickbait detectors.


<details>
  <summary>Details</summary>
Motivation: Current research considers clickbait as a static textual phenomenon and existing detection systems rely on surface-level features, which is not effective in detecting clickbait with emotional intensity.

Method: The authors propose an emotion-aware framework based on the Valence-Arousal-Dominance (VAD) space and use Large Language Models (LLMs) to generate multiple stylistic rewrites of clickbait headlines.

Result: The experimental results show that emotion-aware stylization can significantly degrade the performance of state-of-the-art classifiers, leading to misclassification rates of up to 2.58% to 30.63% on the base system.

Conclusion: The paper concludes that emotion-aware clickbait generation can evade existing detection systems and highlights the need for more effective detection methods that consider emotional dynamics.
...
TL;DR: The paper proposes a method to make dialogue models more proactive by introducing a lightweight intent-transition prior, which improves dialogue efficiency without modifying the underlying language model.


<details>
  <summary>Details</summary>
Motivation: Dialogue models are inherently reactive and lead to redundant interactions in multi-intent settings because they respond to the current user turn without anticipating upcoming intents.

Method: The authors introduce a lightweight intent-transition prior derived from dialogue data using a Temporal Bayesian Network (T-BN) trained on per-turn intent annotations in MultiWOZ 2.2.

Result: The T-BN achieves Recall@5 = 0.787 and MRR = 0.576 on 1,071 held-out USER-turn pairs. In a ground-truth replay over 200 dialogues, BN-guided generation improves Coverage AUC from 0.742 to 0.856 and reduces the number of turns required to reach 75% intent coverage from 3.95 to 2.73.

Conclusion: The proposed method enables more proactive and efficient dialogue behavior without modifying the underlying language model.
...
TL;DR: The paper presents MiniCPM-o 4.5, a multimodal large language model capable of real-time full-duplex omni-modal interaction, enabling simultaneous perception and response.


<details>
  <summary>Details</summary>
Motivation: Current multimodal large language models (MLLMs) are limited by their interaction paradigm, which separates perception and response into alternating phases, and are mostly reactive rather than proactive.
...
Conclusion: The proposed MiniCPM-o 4.5 model mitigates the gaps in current MLLMs by enabling real-time full-duplex omni-modal interaction, exhibiting proactive behaviors, and achieving efficient performance on edge devices.
...
TL;DR: The paper investigates whether mean pooling works well in constructing text embeddings and finds that modern text encoders are robust to information collapse caused by mean pooling.


<details>
  <summary>Details</summary>
Motivation: The authors are motivated by the concern that mean pooling can collapse information beyond the first-order statistics of token embeddings, potentially mapping distinct token embedding distributions to similar text embeddings.
...
Conclusion: The paper concludes that modern text encoders remain effective despite relying on mean pooling because they are robust to information collapse, offering a new perspective on the effectiveness of mean pooling in text embeddings.
...
TL;DR: The paper presents perturbation probing, a method to generate causal hypotheses for FFN neurons in large language models. It identifies two circuit structures, opposition and routing circuits, and demonstrates their application in tasks like safety refusal and language selection.


<details>
  <summary>Details</summary>
Motivation: Understanding the behavior of large language models, particularly in tasks like safety refusal and language selection, and developing a method for precision template-layer editing.
...
Result: The paper identifies two circuit structures that organize LLM behavior: opposition circuits, which appear when RLHF suppresses a pre-training tendency, and routing circuits, which appear for pre-training behaviors distributed through attention. It also demonstrates the effectiveness of perturbation probing in tasks like safety refusal and language selection.

Conclusion: Perturbation probing offers mechanistic insight into RLHF-organized behavior and a practical toolkit for precision template-layer editing, allowing for tasks such as eliminating multi-turn sycophantic capitulation and improving factual correction.
...
TL;DR: The study compares the performance of two pairs of Large Language Models (LLMs), Llama 3 to 3.1 and Qwen 2.5 to 3, on 2,000 MMLU-Pro items. It finds bidirectional changes with large effect sizes, and recommends reporting churn rate alongside aggregate accuracy.


<details>
  <summary>Details</summary>
Motivation: The authors aim to assess the reliability of changes in LLM performance across different versions, adapting the Reliable Change Index (RCI) for item-level comparisons.

Method: The study uses the Reliable Change Index (RCI) on 2,000 MMLU-Pro items with K=10 samples at T=0.7 to compare item-level performance between two pairs of LLMs.

Result: Most items showed no reliable change, but among analysable items, changes were bidirectional with large effect sizes. The aggregate accuracy gain is the net residual of opposing item-level movements.

Conclusion: The study recommends reporting churn rate alongside aggregate accuracy, as greedy single-shot evaluation missed many reliably changed items and falsely flagged unchanged items.
...
TL;DR: The study analyzes Indonesian students' opinions on AI adoption in higher education using machine learning and deep learning approaches.


<details>
  <summary>Details</summary>
Motivation: To understand student opinions on AI adoption in higher education in Indonesia.

Method: TF-IDF-based machine learning (LightGBM, Random Forest, SVM) and Transformer-based deep learning (DistilBERT).

Result: SVM achieves 82.14% test accuracy and F1-score, while DistilBERT performs best with 84.78% accuracy and 84.75% F1-score.

Conclusion: Transformer-based models better capture contextual information, but SVM remains a competitive alternative for sentiment classification.
...
TL;DR: The authors propose a fine-grained evaluation pipeline (WEval) and a reinforcement learning training framework (WRL) to improve writing reward models and generation tasks for large language models.


<details>
  <summary>Details</summary>
Motivation: Existing evaluation benchmarks and training methods for writing reward models are coarse-grained and fail to measure performance from the perspective of specific requirements, leading to limitations in generative writing tasks.
...
Conclusion: The proposed WEval and WRL frameworks improve the performance of writing reward models and generation tasks for large language models, enabling more precise evaluation and training.
...
<details>
  <summary>Details</summary>
Motivation: The study aims to explore the application potential of LLMs in a transfer state, treating transfer as an operational state with potential application value rather than an ontological claim.

Method: The study consists of a preliminary investigation (cognitive characterization across 11 conditions) and an applied experiment (ratings of tutoring performance). It uses various indicators, including MAS-A and SU_dir, to measure cognitive profiles and tutoring performance.

Result: The findings suggest that transfer states may involve functional advantages for application, with transfer conditions scoring on average 1.6 times higher than non-transfer conditions on three tutoring-context indicators (Cohen's d = 1.27).

Conclusion: The study preliminarily suggests that transfer states in LLMs may have functional advantages for application, particularly in behavioral interaction. It highlights the potential value of treating transfer as an operational state and connecting cognitive profiling with applied experiments.
...
TL;DR: The study examines how language users maintain information in context during real-time language comprehension, proposing that users selectively maintain crucial information for future prediction based on syntactic structure.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this study is to understand how language users balance the need to maintain information in context with the cognitive cost of doing so, and how this balance is influenced by syntactic structure and predictability.

Method: The study uses a naturalistic reading time dataset in Japanese to investigate the impact of two factors on maintenance cost: the number of predicted heads and the number of incomplete dependencies.

Result: The results show that both factors affect maintenance cost independently and that there is a tradeoff such that readers who slow down for maintenance tend to benefit more from predictability.

Conclusion: The study concludes that language users selectively maintain information crucial for future prediction, guided by syntactic structure, and that this selective maintenance is rational and efficient.
...
TL;DR: The authors introduce HealthBench Professional, a benchmark for evaluating large language models like ChatGPT in clinical settings, and find that GPT-5.4 outperforms other models and human physicians.


<details>
  <summary>Details</summary>
Motivation: To address the limited evaluations of ChatGPT's common use cases in clinician conversations and provide a benchmark for tracking progress in real-world clinical tasks.

Method: The authors created a benchmark with physician-authored conversations, scored via rubrics adjudicated by multiple physicians, and compared model performance to human physician responses.

Result: GPT-5.4 in ChatGPT for Clinicians outperformed base GPT-5.4, other models, and human physicians, setting a strong baseline for future evaluations.

Conclusion: The authors hope HealthBench Professional will help the healthcare AI community track progress and build trustworthy systems to improve care.
...
TL;DR: A novel framework called Skills-Coach is introduced to improve the skills of Large Language Model (LLM)-based agents by automating the process of skill evolution.
...
Result: Experimental results show significant performance improvements in skill capability across various categories using the Skill-X benchmark dataset.

Conclusion: Skills-Coach has the potential to advance the development of more robust and adaptable LLM-based agents by enhancing their skill capabilities.
...
TL;DR: The paper proposes a method to reduce bias in reward models (RMs) used for aligning large language models with human preferences. The method identifies and intervenes on neurons correlated with bias attributes, achieving reduced sensitivity to spurious features without performance trade-offs.


<details>
  <summary>Details</summary>
Motivation: Reward models (RMs) are crucial for aligning large language models (LLMs) with human preferences but are often sensitive to spurious features like response length. Existing methods to mitigate these biases have limitations, particularly focusing exclusively on response length and resulting in performance trade-offs.

Method: The proposed method involves identifying neurons in RMs whose activations are strongly correlated with predefined bias attributes. It then applies neuron-level intervention to suppress these biased signals. This approach aims to mitigate multiple types of biases at inference time.

Result: The method was evaluated on RM benchmarks, showing reductions in sensitivity to spurious features across diverse bias types without inducing performance trade-offs. When used for preference annotation, small RMs (2B and 7B) with the proposed method achieved performance comparable to a state-of-the-art 70B RM on AlpacaEval and MT-Bench, editing less than 2% of all neurons in RMs.

Conclusion: The paper concludes that the proposed causally motivated intervention effectively mitigates multiple types of biases in RMs at inference time. It highlights that bias signals are primarily encoded by neurons in early layers, providing insights into the internal mechanisms of bias exploitation in RMs.
...
Method: The paper introduces two new metrics: POSER (Part-of-speech Error Rate) and EmbER (Embedding Error Rate), and uses them to evaluate the impact of language model rescoring on ASR systems.
...
TL;DR: This paper estimates the entropy of the Ukrainian language using a method similar to Shannon's 1951 experiment, finding an upper bound of approximately 1.201 bits per character.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to approximate the entropy of the Ukrainian language, which has not been studied since Shannon's initial experiment in 1951, and to compare it with the performance of current Large Language Models.

Method: The method used in this paper involves recruiting volunteers to predict the next character in a sentence, similar to Shannon's experiment. 184 volunteers were recruited through social media channels.

Result: The result of this paper is an upper bound of $H_{upper}approx1.201$ bits per character for the entropy of the Ukrainian language.

Conclusion: The conclusion of this paper is that the estimated entropy of the Ukrainian language is approximately 1.201 bits per character, and the methods and code used are documented and published for future reference.
...
TL;DR: The paper proposes a new dataset, HATS, which assesses human perception of transcription errors in ASR systems, and investigates the correlation between human preferences and various ASR evaluation metrics.


<details>
  <summary>Details</summary>
Motivation: Current ASR evaluation metrics, such as WER, are system-oriented and do not accurately reflect human perception of transcription errors. The authors aim to bridge this gap by creating a dataset that captures human preferences.

Method: The authors created a manually annotated dataset, HATS, with 143 humans rating the best automatic transcription out of two hypotheses. They then investigated the correlation between human preferences and various ASR evaluation metrics, including lexical and embedding-based ones.

Result: The paper presents a new dataset, HATS, and investigates the relationship between human preferences and ASR evaluation metrics, highlighting the limitations of current metrics in reflecting human perception.

Conclusion: The study emphasizes the need for more human-centric ASR evaluation metrics, as current system-oriented metrics do not accurately capture human perception of transcription errors.
...
TL;DR: A new dataset called AppTek Call-Center Dialogues corpus is presented for evaluating English ASR systems in conversational AI applications, particularly for diverse accents.


<details>
  <summary>Details</summary>
Motivation: The difficulty in evaluating English ASR systems for conversational AI applications due to limited publicly available corpora that are representative of diverse user bases and conversational speech.

Method: The AppTek Call-Center Dialogues corpus was created with spontaneous, role-played agent-customer conversations covering fourteen English accents and sixteen service-oriented scenarios. Open-source ASR systems were benchmarked under different segmentation approaches.

Result: Substantial variation in ASR performance across accents and segmentation methods was found, indicating that good performance on general American English benchmarks does not generalize to other accents.

Conclusion: The AppTek Call-Center Dialogues corpus can help improve the evaluation and development of English ASR systems for conversational AI applications, especially for diverse accents.
...
TL;DR: A new English privacy policy corpus (APPSI-139) and a hybrid framework (TCSI-pp-V2) are proposed to improve summarization and interpretation of privacy policies, outperforming large language models.


<details>
  <summary>Details</summary>
Motivation: Privacy policies are often long, complex, and filled with technical jargon, causing users to unknowingly accept unfavorable terms, highlighting the need for high-quality parallel corpora and effective summarization tools.

Method: The APPSI-139 corpus is annotated by domain experts and includes 139 English privacy policies, 15,692 rewritten parallel corpora, and 36,351 fine-grained annotation labels. The TCSI-pp-V2 framework uses a hybrid approach with multiple expert modules and an alternating training strategy.

Result: The proposed hybrid summarization system built on APPSI-139 and TCSI-pp-V2 outperforms large language models (GPT-4o and LLaMA-3-70B) in terms of readability and reliability.

Conclusion: The APPSI-139 corpus and TCSI-pp-V2 framework provide a high-quality resource and effective tool for summarization and interpretation of privacy policies, enhancing user understanding and protection of personal data.
...
<details>
  <summary>Details</summary>
Motivation: To develop a state-of-the-art Thai voice cloning text-to-speech model that can handle Thai-English code-switching and numerals without explicit text normalization.

Method: Continual training on a large Thai-centric speech corpus, adapting the VoxCPM model architecture.

Result: JaiTTS-v1.0 achieves a CER of 1.94%, outperforming human ground truth in short-duration tasks and performing on par in long-duration tasks. It also wins 283 out of 400 pairwise comparisons against commercial flagships.

Conclusion: JaiTTS-v1.0 is a highly effective Thai voice cloning TTS model, capable of handling complex linguistic features and achieving state-of-the-art performance in various speech generation tasks.
...
Method: A multi-agent system, RoadMapper, is proposed, which decomposes the roadmap generation task into three stages: initial generation, knowledge augmentation, and iterative critique-revise-evaluate.
...
Method: The CDS dataset is generated by prompting 19 LLMs to shadow either a human persona or an AI-assistant role on 4 controversial societal topics, with persona-conditioned records encoding 17 sociodemographic and psychological attributes.

Result: The CDS dataset provides a comprehensive resource for analyzing LLM-generated discourse, including emotional and semantic framing across personas, topics, and models, and supports audits of LLMs' bias, social sensitivity, and alignment.

Conclusion: The CDS dataset and pooling platform provide a valuable resource for researchers to analyze and compare LLM-generated discourse, promoting transparency and accountability in LLMs' outputs and potential biases.
...
TL;DR: This paper explores using large language models to detect language ideologies in Luxembourgish user comments, evaluating their performance and the impact of machine-translation on detection accuracy.


<details>
  <summary>Details</summary>
Motivation: Detecting language ideologies is complex but valuable for understanding identity construction through discourse, particularly in multicultural and multilingual societies like Luxembourg.

Method: The study manually annotates a Luxembourgish corpus with ideological categories and evaluates large language models' performance under varying prompts, with and without machine-translation to high-resource languages.

Result: Large language models are practical for identifying language ideological content, though not fully optimized for multi-class annotation tasks, with machine-translation potentially improving performance.

Conclusion: Large language models can assist in detecting language ideologies, offering a practical tool for analyzing discourse in multicultural societies, despite current limitations and the need for optimization.
...
TL;DR: The paper identifies a 'hubness problem' in cross-modal encoders where certain embeddings are close to many unrelated examples, posing a threat to applications like information retrieval and evaluation metrics. The authors propose a method to detect 'hub embeddings' and demonstrate its effectiveness in image captioning and retrieval tasks.


<details>
  <summary>Details</summary>
Motivation: The hubness problem in high-dimensional embedding spaces can negatively impact cross-modal applications, such as information retrieval and automatic evaluation metrics, by causing certain embeddings to be excessively similar to many unrelated examples.

Method: The authors propose a method for identifying hub embeddings and their corresponding hub texts in cross-modal encoders, which project different modalities into a shared space.

Result: Experiments on image captioning evaluation and image-to-text retrieval tasks showed that the proposed method can identify a single hub text that achieves comparable or higher similarity scores than human-written reference captions in many images, revealing vulnerabilities in cross-modal encoders.

Conclusion: The existence of hub embeddings in cross-modal encoders poses practical threats, and the proposed method can effectively identify these vulnerabilities, highlighting the need for improved cross-modal encoder designs.
...
TL;DR: The paper introduces a dataset and approach for generating Arabic poetry using Large Language Models (LLMs), enabling controllable generation capabilities to assist users in writing poetry.


<details>
  <summary>Details</summary>
Motivation: Existing research on Arabic poetry within LLMs has primarily focused on analysis tasks, but the authors aim to address the practical aspect of poetry creation in Arabic by introducing controllable generation capabilities.
...
Result: Experiments show that fine-tuning LLMs on this dataset yields models that can effectively generate poetry that is aligned with user requirements, based on both automated metrics and human evaluation with native Arabic speakers.

Conclusion: The paper concludes that the proposed approach and dataset can assist users in writing poetry in Arabic, and the data and code are made available for further research.
...
TL;DR: A new three-level framework for analyzing therapeutic writing is proposed, which outperforms existing methods in predicting mental health conditions.


<details>
  <summary>Details</summary>
Motivation: The existing computational approaches to therapeutic writing are fragmented and do not effectively map onto the hierarchical processes of narrative construction.

Method: A three-level framework consisting of micro-level lexical features, meso-level semantic embeddings, and macro-level LLM narrative evaluation is introduced and applied to 830 Chinese therapeutic texts.

Result: The macro-level evaluation substantially outperforms lexical and embedding features for mental health prediction, and formal structural features demonstrate that narrative organization carries predictive signal.

Conclusion: The proposed framework identifies macro-structural organization as the primary locus of clinical signal and generates testable hypotheses for intervention design and longitudinal research.
...
<details>
  <summary>Details</summary>
Motivation: Coreference resolution in task-based dialogue systems is challenging, especially in visually grounded environments with complex scenes and diverse object metadata. Current methods have poor generalization across domains and rely heavily on supervised models that often overfit to dataset-specific artifacts.

Method: The proposed approach uses large language models (LLMs) to reason over detailed object metadata and dialogue history to improve coreference resolution. It involves generating step-by-step reasoning processes to align dialogue context with objects present in the scene.

Result: The approach demonstrates effective coreference resolution on the SIMMC 2.1 dataset, with LLMs able to link conversations and objects accurately. It also shows strong generalization to unseen scenarios and novel objects, outperforming encoder-based supervised methods in cross-domain evaluations.

Conclusion: The findings highlight the importance of structured metadata and careful prompt engineering in enhancing the robustness and generalization of task-oriented dialogue systems. The proposed approach offers a promising solution for improving coreference resolution in visually grounded environments.
...
Method: The authors propose a post hoc framework, Conformal Abstention (CA), adapted from conformal prediction (CP). CA uses prediction confidence to determine whether to abstain from answering a query, and introduces a calibration strategy using representation geometry to measure knowledge involvement.

Result: Experiments demonstrate that the proposed framework improves selective answering significantly, achieving 75 percent conditional correctness.

Conclusion: The proposed framework provides a reliable method for language models to abstain from answering queries when they are uncertain, reducing hallucinations and improving response correctness.
...
TL;DR: This paper evaluates the performance of three Small Language Models (SLMs) in preserving emotions during backtranslation across five European languages.


<details>
  <summary>Details</summary>
Motivation: Preserving affective nuance is a challenge in Machine Translation (MT), where emotional fidelity often takes a backseat to semantic equivalence.

Method: The paper uses the GoEmotions dataset and evaluates three SLMs (EuroLLM, Aya Expanse, and Gemma) across five European languages, investigating their ability to retain emotional sentiment, the efficacy of emotion-aware prompting, and the performance of ModernBERT for emotion classification.

Result: The paper aims to assess the performance of SLMs in maintaining fine-grained emotions during backtranslation, but the specific results are not provided in the abstract.

Conclusion: The paper concludes that evaluating the performance of SLMs in preserving emotions during backtranslation is crucial, but the specific conclusions are not provided in the abstract.
...
TL;DR: This survey paper discusses the application of large language models (LLMs) in automating and assisting the peer review process, covering generation, after-review tasks, and evaluation methods.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to synthesize and survey the techniques and methods that utilize LLMs to assist or automate different stages of the peer review pipeline, aiming to provide practical guidance for building and integrating LLM systems in this context.

Method: The paper surveys and synthesizes techniques for peer review generation, after-review tasks, and evaluation methods. It covers various approaches such as fine-tuning strategies, agent-based systems, RL-based methods, and discusses datasets, modeling choices, limitations, and ethical concerns.

Result: The result of this paper is a comprehensive survey that catalogs and compares different methods and techniques for applying LLMs in the peer review process, providing guidance for future implementations.

Conclusion: The conclusion of this paper is that LLMs have the potential to significantly impact the peer review process by automating and assisting various stages, but there are also limitations, ethical concerns, and future directions that need to be considered.
...
<details>
  <summary>Details</summary>
Motivation: Understanding personality representation mechanisms in LLMs is critical, but existing methods for personality editing have limitations, such as requiring changes to numerous neurons and leading to performance degradation.

Method: The proposed method, DPN-LE, identifies personality-specific neurons by contrasting MLP activations between high-trait and low-trait samples, and applies dual-criterion filtering to isolate mutually exclusive neuron subsets.
...
Conclusion: The paper concludes that not all modified neurons are directly related to personality representation, and that DPN-LE provides a more precise and effective approach to personality editing in LLMs.
...
TL;DR: TeCoD improves Text-to-SQL generation with template-based approach, achieving higher accuracy and lower latency.


<details>
  <summary>Details</summary>
Motivation: Real-world deployment of Text-to-SQL generation is challenging due to inconsistent accuracy and risk of generating invalid SQL, especially in complex or unseen schemas.

Method: Template Constrained Decoding (TeCoD) system, which converts historical NL-SQL pairs into reusable templates and uses a fine-tuned natural language inference model for template selection and grammar-constrained decoding.
...
Conclusion: TeCoD effectively addresses limitations in Text-to-SQL generation, providing a more accurate and efficient solution for querying structured data using natural language.
...
TL;DR: Researchers introduce DriftBench, a benchmark to evaluate how well large language models adhere to constraints during iterative scientific ideation. They find that models often reduce adherence to original constraints as iterations increase.


<details>
  <summary>Details</summary>
Motivation: The motivation is to assess whether large language models preserve fidelity to the original objective during iterative refinement of ideas in scientific ideation.

Method: The method involves introducing DriftBench, a benchmark that evaluates constraint adherence across multiple models, interaction conditions, and research briefs. It includes 2,146 scored benchmark runs with seven models from five providers.

Result: The results show that iterative pressure increases structural complexity and often reduces adherence to original constraints. A significant 'knows-but-violates' rate is found, indicating models can recall constraints but still violate them.

Conclusion: The study concludes that large language models struggle with constraint adherence during iterative scientific ideation. DriftBench is released as an open benchmark for future research.
...
TL;DR: The paper explores the optimization of syntactic dependency distance in sentence structures, particularly in star and quasistar trees, and argues that while optimization is relatively easy, competing principles explain why anti-dependency distance minimization effects occur.


<details>
  <summary>Details</summary>
Motivation: To address two questions: (1) the difficulty of minimizing dependency distance and (2) why anti-dependency distance minimization effects are found in star structures but not in path structures.

Method: Analyzing the convexity of the optimization landscape for star and quasistar trees, and comparing the effects of dependency distance minimization across different structures.

Result: The optimization landscape for star and quasistar trees is convex, making the distance-based optimization problem simpler than previously believed. However, competing principles, rather than optimization difficulty, explain anti-dependency distance minimization effects.

Conclusion: The study concludes that minimizing dependency distance is not difficult in star and quasistar trees, but that competing principles are responsible for observed anti-dependency distance minimization effects, particularly in star-like structures.
...
TL;DR: The study investigates whether using distinct personas in large language models (LLMs) influences urban sentiment judgments. Results show that while agents sharing a persona behave consistently, cross-persona variation is limited, and simple label-based persona prompting may not capture fine-grained perceptual judgments.


<details>
  <summary>Details</summary>
Motivation: To examine whether persona prompting in LLMs produces meaningful and reproducible behavioral diversity in urban analysis.

Method: Using a factorial set of personas and multiple agents per persona to evaluate urban scene images and assess within-persona consistency and cross-persona variation.
...
Conclusion: Simple label-based persona prompting may add limited annotation value, and LLMs without personas can sometimes match or exceed persona-conditioned agreement with human labels.
...
<details>
  <summary>Details</summary>
Motivation: The strategic dilemma of whether to prioritize diversity or quality when training high-resource non-English language models.

Method: Constructing hierarchical quality filters for 500M web documents and comparing multi-epoch training on filtered subsets against single-pass training on a diverse corpus.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of current LLMs in handling implicitly predictive queries that require inference from historical patterns, rather than mere retrieval, in table question answering.

Method: The authors introduce TopBench, a benchmark consisting of 779 samples across four sub-tasks, and evaluate diverse LLMs under both text-based and agentic workflows.

Result: Experiments show that current LLMs struggle with intent recognition and often default to lookups, and that accurate intent disambiguation is a prerequisite for leading predictive behaviors.

Conclusion: The paper concludes that LLMs need to improve in intent recognition and predictive reasoning, and that more sophisticated modeling or reasoning capabilities are required to elevate the upper bound of prediction precision.
...
<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the inconsistencies and limitations in the application of surprisal theory in linguistics, particularly in how units of analysis and regions of interest are defined and evaluated.

Method: The paper proposes a unified framework for reasoning about surprisal over arbitrary unit inventories, aiming to disentangle the definition of the unit of analysis from the choice of regions of interest.

Result: The paper argues for making the choices of unit of analysis and regions of interest explicit in surprisal-based analyses and treating tokenization as an implementation detail rather than a scientific primitive.

Conclusion: The conclusion is that a unified framework can provide a more coherent and systematic approach to applying surprisal theory in linguistics, emphasizing the need for explicitness in modeling choices.
...
TL;DR: A new framework called LAM-PINN is proposed to improve the efficiency of Physics-informed neural networks (PINNs) in solving parameterized partial differential equations (PDEs) by leveraging task-specific learning dynamics.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the challenge of training individual PINNs for each task in parameterized PDE families, which can be computationally expensive, and to improve the generalization to unseen configurations within bounded design spaces.

Method: The proposed method, LAM-PINN, combines PDE parameters with learning-affinity metrics to construct a task representation, clusters tasks, and decomposes the model into cluster-specialized subnetworks and a shared meta network.

Result: LAM-PINN achieves an average 19.7-fold reduction in mean squared error (MSE) on unseen tasks using only 10% of the training iterations required by conventional PINNs.

Conclusion: The proposed LAM-PINN framework is effective for generalization to unseen configurations within bounded design spaces of parameterized PDE families in resource-constrained engineering settings.
...
TL;DR: A framework for migrating Large Language Model (LLM) based systems when the underlying model reaches end-of-life or requires replacement.


<details>
  <summary>Details</summary>
Motivation: The need for a principled and reproducible methodology for model migration as the LLM ecosystem evolves rapidly and organizations manage AI-powered services across multiple models, regions, and use cases.
...
Conclusion: The proposed framework provides a broadly applicable solution for enterprises deploying LLM-based products, enabling confident model comparison and efficient model migration.
...
TL;DR: An LLM-based agentic system, Qiushi Discovery Engine, autonomously discovers and experimentally validates a nontrivial physical mechanism, optical bilinear interaction, marking a milestone in research-level autonomous agents.


<details>
  <summary>Details</summary>
Motivation: The motivation is to demonstrate end-to-end autonomous scientific discovery in a real physical system using large language model (LLM)-based agents, which has not been achieved before.

Method: Qiushi Discovery Engine combines nonlinear research phases, Meta-Trace memory, and a dual-layer architecture to maintain adaptive and stable research trajectories across long-horizon investigations.

Result: The system autonomously reproduces a published experiment, converts an abstract theory into experimental observables, and proposes and experimentally validates optical bilinear interaction, a previously unreported physical mechanism.

Conclusion: The Qiushi Discovery Engine demonstrates a significant milestone in research-level autonomous agents, showcasing the potential for AI to drive new knowledge and transformative technologies through autonomous scientific discovery.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to improve efficiency, robustness, and explainability in machine learning pipeline generation.

Method: A five-agent system is proposed, integrating code-grounded Retrieval-Augmented Generation (RAG), explainable hybrid recommender, self-healing mechanism, and adaptive learning.
...
TL;DR: An unsupervised machine learning workflow was developed for electrofacies analysis in the Keta Basin, Ghana, using wireline logs from a single well, identifying four meaningful electrofacies.


<details>
  <summary>Details</summary>
Motivation: The study aimed to address the challenge of scarce core data in the offshore Keta Basin, Ghana, by developing a workflow for electrofacies analysis using standard wireline logs.
...
Result: Four electrofacies were identified, exhibiting systematic patterns associated with variations in clay content, porosity, and rock framework properties, forming a geological continuum from shale-dominated to sandstone-dominated units.

Conclusion: The proposed workflow provides a robust and reproducible framework for subsurface characterisation, offering a practical tool for early-stage formation evaluation in frontier offshore basins.
...
Conclusion: The paper concludes that TRUST is a reliable and decentralized framework for verifying LRMs and MAS, enabling safe and accountable deployment of reasoning-capable systems.
...
TL;DR: The study examines how students interact with generative AI in higher education, finding that top performers use AI for inquiry and exploration, while low performers rely on AI for task delegation and ready-made solutions.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to understand how students interact with generative AI in higher education and how these interactions impact learning outcomes.
...
Method: The study replays over 900 historical trades under various exit policies and compares results against the existing production setup.
...
TL;DR: The paper proposes an event-driven framework for computer-use agents to adaptively allocate compute resources, reducing costs and improving efficiency for long-horizon GUI tasks.


<details>
  <summary>Details</summary>
Motivation: Current computer-use agents are expensive and slow due to the uniform allocation of compute resources, which is inefficient for tasks with heterogeneous steps and high-risk moments.

Method: The proposed framework uses a step-level cascade with a small policy by default and escalates to a stronger model when lightweight learned monitors detect elevated risk, combining Stuck and Milestone Monitors.

Result: The framework allows for adaptive, on-demand compute allocation, reducing costs and improving efficiency without changing the underlying agent architecture or retraining the large model.

Conclusion: The proposed framework offers a modular and deployment-oriented solution for improving the efficiency and effectiveness of computer-use agents in long-horizon GUI tasks.
...
TL;DR: This paper explores new approaches to rational belief revision using interval orders and biorders, which generalize traditional preference orders. It provides axiomatic characterizations and discusses the implications of these approaches on consistency and success postulates.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to explore alternative classes of ordering that can be employed in rational belief revision, beyond the commonly used total preorder. The authors aim to investigate the use of interval orders and biorders, which have been studied in the theory of rational choice but have seen limited application in belief revision.

Method: The paper employs a theoretical approach, using axiomatic characterizations to define and analyze the properties of interval order-based and biorder-based belief revision operators. The authors also discuss the implications of these approaches on consistency and success postulates.

Result: The paper provides axiomatic characterizations of interval order-based and biorder-based belief revision operators, as well as families of operators that lie between them. It shows that biorder-based revisions satisfy the Success postulate but may not yield consistent outputs, and proposes a modification to discard inputs that lead to inconsistency.

Conclusion: The paper concludes that the biorder-based approach is well-suited for scenarios where an agent might initially reject new information but may accept it when presented with additional explanation. The authors argue that this approach can be used to model situations where an agent's beliefs are revised based on new information, taking into account the possibility of dissonance or instability.
...
TL;DR: This study compares TabPFN with traditional machine learning methods for predicting MCI to AD conversion using the TADPOLE dataset. TabPFN outperformed traditional methods, especially in low data settings.
...
Conclusion: The findings demonstrate that foundation models like TabPFN are promising for disease prediction in data-limited scenarios, such as Alzheimer's disease.
...
TL;DR: A new framework, PCD-DT, is proposed for personalized cognitive decline assessment using multimodal data and uncertainty-aware modeling, showing promising results in predicting disease trajectories for Alzheimer's disease.


<details>
  <summary>Details</summary>
Motivation: Cognitive decline is highly heterogeneous across individuals, complicating prognosis, trial design, and treatment planning, and there is a need for personalized modeling of patient-specific disease trajectories.

Method: The PCD-DT framework combines latent state-space models, multimodal fusion, and uncertainty-aware validation to model patient-specific disease trajectories from sparse, noisy, and irregular longitudinal data.

Result: The framework shows clear separation between cognitively normal and Alzheimer's disease cohorts in ADAS13, ventricle volume, and hippocampal volume over five years, and outperforms a Last Observation Carried Forward baseline in multimodal next-visit prediction.

Conclusion: The PCD-DT framework provides a principled starting point for personalized in silico modeling in neurodegenerative disease, positioning it as a foundational step toward clinically deployable, uncertainty-aware digital twin systems.
...
Method: Web2BigTable uses a bi-level architecture with an upper-level orchestrator and lower-level worker agents that solve sub-problems in parallel through a closed-loop process.

Result: Web2BigTable achieved state-of-the-art results on WideSearch and XBench-DeepSearch benchmarks, with significant improvements over previous best results.

Conclusion: Web2BigTable effectively supports both breadth and depth-oriented web search tasks, demonstrating its potential for improving web search capabilities.
...
TL;DR: This paper tests the assumption that evaluator models in multi-agent LLM pipelines can maintain their assigned adversarial roles, finding that role fidelity varies significantly with model choice and fact-check provider, and identifying failure modes.


<details>
  <summary>Details</summary>
Motivation: The motivation is to empirically test the assumption that evaluator models in multi-agent LLM pipelines can reliably maintain their assigned adversarial roles, which is crucial for generating structured, multi-perspective assessments of political statements.

Method: The authors develop an epistemic stance classifier and measure role fidelity across 60 political statements using four metrics: RDI, EDD, DDI, and ERS, testing two LLM models (Mistral Large and Claude Sonnet) and two fact-check providers (Perplexity and presumably a default or baseline provider).

Result: The results show that model choice significantly affects role fidelity, with Mistral Large outperforming Claude Sonnet by 28pp (67% vs. 39%), and that fact-check provider choice is not universally neutral, with Perplexity reducing Claude's role fidelity on German statements. Two failure modes are identified: Epistemic Floor Effect and Role-Prior Conflict, which are manifestations of Epistemic Role Override (ERO).

Conclusion: The conclusion is that role fidelity measurement is crucial for multi-agent LLM validation, and a system validated without it may systematically misrepresent the epistemic diversity it was designed to provide, highlighting the need for careful model and fact-check provider selection.
...
TL;DR: The paper proposes a new approach to evaluate tool-calling agents in real-time by introducing a specialized reviewer agent that assesses provisional tool calls before execution, allowing for proactive error mitigation.


<details>
  <summary>Details</summary>
Motivation: The current evaluation methods for tool-calling agents are post-hoc, which means they can only identify errors after they occur and cannot correct them in real-time.

Method: The authors propose a multi-agent system with a primary execution agent and a secondary review agent. The reviewer agent evaluates provisional tool calls before execution and provides feedback to correct errors.

Result: The approach achieves a +5.5% improvement on irrelevance detection and +7.1% on multi-turn tasks. The reviewer model choice is critical, with the o3-mini model achieving a 3:1 benefit-to-risk ratio versus 2.1:1 for GPT-4o.
...
<details>
  <summary>Details</summary>
Motivation: The scarcity of high-quality web trajectory training data limits the accuracy of multimodal large language models in automating complex tasks on websites.
...
Conclusion: AutoSurfer effectively addresses the limitations of existing trajectory generation methods, enabling the generation of high-quality training data for website-specific LLMs.
...
<details>
  <summary>Details</summary>
Motivation: Many biomedical knowledge graphs lack schema-level constraints, while graphs from structured resources are hard to harmonize. OptimusKG aims to address this by providing a unified representation.

Method: OptimusKG is built from 18 ontologies and controlled vocabularies, containing 190,531 nodes, 21,813,816 edges, and 67,249,863 property instances. A multimodal agent, PaperQA3, was used to assess the validity of graph relationships.

Result: OptimusKG was found to have supporting evidence for 70.0% of sampled edges, with 83.4% of false edges receiving no supporting evidence. The graph captures biomedical knowledge that may precede synthesis in the scientific literature.

Conclusion: OptimusKG provides a standardized resource for graph-based machine learning, knowledge-grounded retrieval, and biomedical discovery use cases, such as hypothesis generation.
...
<details>
  <summary>Details</summary>
Motivation: The authors aim to investigate the assumption that agent collaboration in multi-agent systems leads to wise decisions, and to explore the limitations of this assumption.

Method: The authors conducted 36 experiments across three state-of-the-art benchmarks, evaluating the interaction between three preeminent SOTA models, and introduced metrics such as the Tribalism Coefficient and the Sycophantic Weight.

Result: The authors discovered the Consensus Paradox and the Inverse-Wisdom Law, which show that adding logical agents can increase erroneous trajectories and lead to Logic Saturation, and established the Heterogeneity Mandate as a foundational safety requirement for resilient agentic architectures.

Conclusion: The paper concludes that the assumption of 'Wisdom of the Crowd' in multi-agent systems is flawed, and that the design of agentic architectures must prioritize heterogeneity to ensure resilience and safety.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to develop a theory of structural governance for cognitive workflow systems, ensuring safety, uniformity, and expressiveness of governance across different levels and systems.

Method: The method involves mechanized proofs in Coq 8.19 using the Interaction Trees library with parameterized coinduction, as well as paper proofs with explicit reductions.

Result: The results include: (1) Coinductive Safety Predicate (gov_safe) for governance safety, (2) Governance Invariance Theorem for uniform governance, (3) Sufficiency Theorem for expressive completeness of atomic primitives, (4) Alternating Normal Form for canonical decomposition of machines, and (5) Necessity Theorem for necessity of architecturally opaque components.

Conclusion: The paper concludes that the results demonstrate the effectiveness of structural governance in cognitive workflow systems, providing a formal framework for ensuring safety, uniformity, and expressiveness, and connecting abstract models to deployed runtime.
...
TL;DR: The paper discusses the governance of AI systems' effects and presents a formal framework for analyzing the gap between expressiveness and governance boundaries.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the risk and 'theater' (governance policies that address non-existent capabilities) that arise from the independent definition of expressiveness and governance boundaries in AI systems.

Method: The authors use Rice's theorem (1953) to prove that the gap between expressiveness and governance boundaries is undecidable in the general case and propose coterminous governance as a testable criterion for AI governance systems.

Result: The authors show that coterminous governance requires an architectural decision to separate computation from effect and that structural governance under this separation subsumes separate governance infrastructure.

Conclusion: The authors conclude that either the expressiveness and governance boundaries are provably identical (coterminous governance) or risk and 'theater' are structurally inevitable, and propose mechanized proofs in Coq as a verification method.
...
Result: The results show that DALS achieves the best accuracy at 98.0% on a synthetic dataset, while DALS-Fast reaches 90% in just 3 epochs. The cross-dataset analysis reveals regime-dependent patterns, and DALS avoids extreme performance, achieving competitive fine-tuning performance.

Conclusion: The paper concludes that there is no single strategy that wins across all regimes, and that DALS provides a unified and competitive approach to learning rate scheduling across different tasks and datasets.
...
TL;DR: A new AI approach called machine collective intelligence is proposed to autonomously discover governing equations in scientific systems, integrating symbolism and metaheuristics.


<details>
  <summary>Details</summary>
Motivation: The discovery of explainable and extrapolatable equations from empirical observations is a longstanding challenge in science, and modern AI has limitations in this area.

Method: Machine collective intelligence, a unified paradigm that integrates symbolism and metaheuristics, enables autonomous and evolutionary discovery of governing equations through coordinated reasoning agents.

Result: The approach autonomously recovered underlying governing equations across various scientific systems and reduced extrapolation error by up to six orders of magnitude compared to deep neural networks.
...
TL;DR: A novel framework, MetaSymbO, is proposed for language-guided metamaterial discovery, enabling efficient exploration of microstructured materials with targeted mechanical behavior.


<details>
  <summary>Details</summary>
Motivation: Existing inverse-design methods for metamaterial discovery require explicit numerical property targets and are less suitable for early-stage exploration with incomplete constraints and qualitative intents. Large language models can interpret intents but lack geometric awareness and physical property validity.

Method: MetaSymbO, a multi-agent framework, is introduced, consisting of a Designer, Generator, and Supervisor. It utilizes symbolic-driven latent evolution, applying programmable operators over disentangled latent factors to compose, modify, and refine structures.

Result: MetaSymbO improves structural validity by up to 34% in symmetry and nearly 98% in periodicity compared to state-of-the-art baselines. It achieves about 6-7% higher language-guidance scores while maintaining superior structure novelty.

Conclusion: MetaSymbO demonstrates effectiveness in language-guided metamaterial discovery, enabling efficient exploration of microstructured materials with targeted mechanical behavior. Its practical capability is validated through real-world case studies on auxetic and high-stiffness metamaterial design.
...
TL;DR: The paper presents a framework for continuous governance of clinical AI systems, which includes monitoring, evaluation, and iteration. The framework was applied to an EHR-embedded agent called Hyperscribe, resulting in improved performance and effective completion rates.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the need for continuous governance of clinical AI systems beyond point-in-time evaluation, ensuring their performance and reliability throughout deployment.

Method: The method involves an end-to-end framework that integrates rubric validation, live deployment feedback, technical performance monitoring, cost tracking, and controlled experimentation. This framework was applied to Hyperscribe, an agent that converts ambient audio into structured chart updates.

Result: The results show that the median scores of Hyperscribe improved from 84% to 95% across seven versions evaluated through controlled experiments. Live feedback entries over three months indicated a shift from error reports to positive observations as interventions resolved failures. The median processing time per audio segment was 8.1 seconds with a 99.6% effective completion rate.

Conclusion: The conclusion is that continuous, multi-channel governance of deployed clinical AI is both achievable and effective, as demonstrated by the application of the proposed framework to Hyperscribe.
...
TL;DR: The paper proposes a novel perspective for evaluating the compositionality of Large Language Models (LLMs) by generating rules for dataset mapping, addressing limitations in existing compositional generalization tests.
...
Method: The proposed approach requires LLMs to generate programs as rules for dataset mapping and estimates compositionality using complexity-based theory.

Result: Experiments on a string-to-grid task reveal various compositionality characterizations and deficiencies in existing advanced LLMs.

Conclusion: The proposed perspective provides a new way to analyze the compositionality of LLMs, addressing the limitations of existing tests and offering insights into LLMs' compositionality characterization.
...
<details>
  <summary>Details</summary>
Motivation: The reliance of current large language model systems on language as the universal interface limits their applicability to many real-world problems, especially in scientific domains.

Method: Eywa augments domain-specific foundation models with a language-model-based reasoning interface, enabling language models to guide inference over non-linguistic data modalities.
...
Conclusion: Eywa provides a flexible framework for integrating domain-specific foundation models with language models, enhancing their applicability to scientific domains.
...
TL;DR: The paper explores how human cognition affects understanding of AI explanations, finding that cognitive models better fit human decisions than machine learning proxies, and can be used to inform future development of more usable AI explanations.
...
Method: The study used a formative user study to elicit reasoning strategies, a summative user study to collect decisions, and cognitive modeling to implement and evaluate the processes underlying each reasoning strategy.
...
Conclusion: The work contributes to debugging human understanding of XAI and informing the future development of more usable and interpretable AI explanations.
...
TL;DR: The paper proposes a formal framework called Safe Bilevel Delegation (SBD) for runtime delegation safety in hierarchical multi-agent systems, which dynamically adjusts the safety-efficiency trade-off as task context changes during execution.


<details>
  <summary>Details</summary>
Motivation: The need for a runtime mechanism that dynamically adjusts the safety-efficiency trade-off as task context changes during execution in high-stakes environments where large language model (LLM) agents are deployed.
...
Conclusion: The paper presents a formal framework and theoretical results for Safe Bilevel Delegation (SBD), which provides a runtime mechanism for dynamic safety-efficiency trade-off adjustment in hierarchical multi-agent systems.
...
TL;DR: The paper presents tio-shacl, a comprehensive validation framework for the TM Forum Intent Ontology, enabling automated validation of network intents.


<details>
  <summary>Details</summary>
Motivation: The TM Forum Intent Ontology (tio) lacks formal validation mechanisms to ensure intent correctness before its admission, which can lead to incorrect configurations and management of telecommunications networks.

Method: The authors developed tio-shacl, a SHACL (Shapes Constraint Language) validation framework for tio, including 56 node shapes, 69 property shapes, and a reusable constraint library with 25 parameterized SPARQL-based constraint components.

Result: The framework achieves 100% vocabulary coverage, cross-implementation compatibility across three major SHACL engines, and validation accuracy on a corpus of 133 test cases.

Conclusion: The tio-shacl framework enables automated syntactic and semantic validation of network intents, addressing a critical gap in the field, and is publicly available under an MIT license.
...
TL;DR: The study examines the reliability of supervised financial NLP benchmarks, focusing on the impact of rubric wording, metric choice, and aggregation policy on model evaluation, and proposes a reporting discipline for ensuring the governance of evaluation rulers.


<details>
  <summary>Details</summary>
Motivation: The increasing use of Large Language Models (LLMs) in financial analysis raises concerns about the objectivity of supervised financial NLP benchmarks, which are often used as decision evidence for model selection and deployment.

Method: The study uses the Japanese Financial Implicit-Commitment Recognition (JF-ICR) benchmark, testing 4 frontier LLMs with 5 rubrics, 3 temperatures, and 5 ordinal metrics, and analyzes the results to identify measurement risks.
...
Conclusion: The study highlights the need for a reporting discipline for supervised financial benchmarks to ensure the governance of evaluation rulers and proposes identifiable metrics, such as exact accuracy, macro-F1, and weighted kappa, for reliable model evaluation.
...
TL;DR: The authors propose a unified framework, HGUL, for robust representation learning on heterogeneous graphs with heterophily and noisy structures.
...
Method: The authors propose a unified framework, HGUL, which consists of three modules: a kNN-based graph construction module, a graph structure learning module, and a heterogeneous affinity learning module.
...
Conclusion: The authors conclude that jointly modeling heterophily and noise is crucial in heterogeneous graph learning, and HGUL provides a effective solution to this problem.
...
TL;DR: This paper proposes a leadership-facing spectrum to understand the relationships between human and artificial intelligence in decision-making, highlighting the risks of misrecognition and the importance of co-adaptability.


<details>
  <summary>Details</summary>
Motivation: The increasing collaboration between humans and artificial intelligence in decision-making makes it challenging to understand who is leading the decision-making process, and there is a need for a framework to recognize and navigate these configurations.

Method: The paper introduces a spectrum with five positions: Pure Human, Centaur, Co-equal, Minotaur, and Pure AI, to categorize the relationships between humans and AI in decision-making, and discusses the concept of co-adaptability in heterogeneous teaming.

Result: The proposed framework aims to help strategic leaders and AI system designers recognize the configuration at work, notice when it shifts, and judge whether it fits the decision before them, which will shape how power, responsibility, and trust are distributed in organizational life.

Conclusion: The paper concludes that leaders need to be able to see and understand where and how consequential decisions are being shaped, in order to create a future that remains governable and worth inhabiting.
...
TL;DR: The paper introduces InteractWeb-Bench, a multimodal interactive benchmark for website generation under low-code user conditions, simulating diverse user behaviors and interactive execution.


<details>
  <summary>Details</summary>
Motivation: The advancement of multimodal large language models (MLLMs) and coding agents has shifted website development to agent-based code synthesis, but existing benchmarks have idealized assumptions, neglecting the semantic misalignment between ambiguous user instructions and model understanding.

Method: The authors introduce InteractWeb-Bench with four types of user agents and persona-driven instruction perturbations to simulate diverse user behaviors, and an interactive execution environment with a unified action space.

Result: Experiments show that frontier MLLM-based agents are limited by blind execution, struggling with intent recognition and adaptive interaction.

Conclusion: The paper highlights the need for more realistic benchmarks and the limitations of current MLLM-based agents in handling ambiguous user instructions.
...
TL;DR: PRTS, a Vision-Language-Action (VLA) model, improves robotic control by incorporating goal-conditioned reinforcement learning, enabling better temporal task progress understanding and goal reachability awareness.
...
Method: PRTS uses Goal-Conditioned Reinforcement Learning, treating language instructions as goals and employing contrastive reinforcement learning to learn a unified embedding space, and incorporates dense goal-reachability supervision from offline trajectories.

Result: PRTS achieves state-of-the-art performance on various benchmarks, including LIBERO, LIBERO-Pro, LIBERO-Plus, SimplerEnv, and a real-world suite of 14 complex tasks, with significant gains in long-horizon, contact-rich, and zero-shot novel-instruction settings.

Conclusion: PRTS demonstrates that injecting goal-reachability awareness into VLA models significantly improves execution success and long-horizon planning of general-purpose robotic foundation policies.
...
<details>
  <summary>Details</summary>
Motivation: The need for reliable responses in LLM services while managing substantial computational costs, leading to a budgeted sequential decision problem.

Method: Veroic formulates request-time control as a partially observable Markov decision process, using a lightweight verifiable observation channel to aggregate quality signals into a belief state over latent response reliability.
...
Conclusion: The proposed Veroic framework effectively addresses the challenge of balancing response reliability and computational cost in black-box LLM services.
...
TL;DR: Large language models (LLMs) can recall scientific formulas but adding in-context examples can shift their reliance from domain knowledge to empirical pattern fitting, potentially displacing knowledge-driven reasoning.


<details>
  <summary>Details</summary>
Motivation: The authors aim to investigate how large language models (LLMs) utilize scientific knowledge, particularly in tasks requiring latent structure recovery across various scientific domains.

Method: The study conducts experiments using 60 latent structure recovery tasks across five scientific domains, involving 6,000 trials and four models, to observe the impact of in-context examples on LLMs' reasoning.

Result: The findings indicate that in-context examples can lead to knowledge displacement, where models rely less on pretrained domain knowledge and more on empirical pattern fitting, with varying accuracy consequences.

Conclusion: The study cautions practitioners deploying LLMs on scientific tasks about the potential for in-context examples to displace, rather than reinforce, the knowledge they are intended to support, leading to a shift away from knowledge-driven reasoning.
...
TL;DR: The paper proposes a new approach for generating interactive 3D indoor scenes from natural language using a domain-specific language called SpatialGrammar, which improves spatial fidelity and physical plausibility.


<details>
  <summary>Details</summary>
Motivation: Automatically generating interactive 3D indoor scenes from natural language is crucial for virtual reality, gaming, and embodied AI, but existing LLM-based approaches often suffer from spatial errors and collisions.

Method: The authors propose SpatialGrammar, a domain-specific language that represents gravity-aligned indoor layouts as BEV grid placements with deterministic compilation to valid 3D geometry, and develop SG-Agent and SG-Mini models to generate 3D scenes.

Result: The proposed approach improves spatial fidelity and physical plausibility over prior methods across 159 test scenes, and SG-Mini performs competitively against larger LLM-based baselines on single-shot generation scenarios.

Conclusion: The paper concludes that SpatialGrammar is an effective approach for generating interactive 3D indoor scenes from natural language, enabling verifiable constraint checking and improving spatial fidelity and physical plausibility.
...
TL;DR: The paper studies how uncertainty affects workflows that extract and transform information from various artifacts, finding that workflows can diverge substantially yet recover correct answers or remain structurally similar while producing incorrect outputs.


<details>
  <summary>Details</summary>
Motivation: The motivation is to understand how uncertainty impacts structured agent workflows that iteratively extract, transform, and reference external information from heterogeneous artifacts.

Method: The method involves injecting structured perturbations into artifact-derived representations, executing fixed workflows under comprehensive logging, and quantifying contamination via trace divergence in plans, tool invocations, and intermediate state.

Result: The results show a decoupling between workflow divergence and output correctness, with three manifestation types: silent semantic corruption, behavioral detours with recovery, and combined structural disruption.

Conclusion: The paper contributes a formal taxonomy of contamination manifestations, a trace-based measurement framework, and empirical evidence with implications for targeted verification, defensive design, and cost control.
...
TL;DR: The paper introduces MEDS, a dataset to evaluate large language models' mathematical abilities and biases, providing insights into their reasoning and reporting of mathematics across various conditions.


<details>
  <summary>Details</summary>
Motivation: The authors aim to enhance the impact of large language models (LLMs) on math education by addressing the lack of data on their mathematical prowess and biases across different prompts.

Method: The authors introduce MEDS, a dataset created by 28,000 personas from 14 LLMs, each shadowing either humans or AI assistants, and including various math tasks and persona metadata.
...
Conclusion: The MEDS dataset will benefit learning analytics experts, cognitive scientists, and developers of safer AI tutors in mathematics, providing valuable insights into LLMs' mathematical abilities and biases.
...
TL;DR: A framework called WaferSAGE is proposed for wafer defect visual question answering in semiconductor manufacturing, using small vision-language models and a three-stage synthesis pipeline to address data scarcity.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the data scarcity issue in semiconductor manufacturing and to provide a viable path for privacy-preserving, cost-effective deployment in this field.

Method: A three-stage synthesis pipeline is proposed, which includes clustering-based cleaning, generation of comprehensive defect descriptions using vision-language models, and rubric-guided synthesis of VQA pairs. A dual assessment framework and curriculum-based reinforcement learning with Group Sequence Policy Optimization (GSPO) are also used.

Result: The 4B-parameter Qwen3-VL model achieves a 6.493 LLM-Judge score, closely approaching Gemini-3-Flash (7.149), demonstrating that small models with domain-specific training can surpass proprietary large models in specialized industrial visual understanding.

Conclusion: The proposed framework, WaferSAGE, offers a viable path for privacy-preserving, cost-effective deployment in semiconductor manufacturing, and demonstrates the potential of small vision-language models in specialized industrial visual understanding.
...
TL;DR: The study finds that large language models (LLMs) exhibit sycophancy, adapting their responses to the views and identities of the user, which affects their perceived political bias.


<details>
  <summary>Details</summary>
Motivation: The study aims to investigate the relationship between sycophancy in LLMs and their perceived political bias, as evaluated through fixed questionnaires.

Method: The study employs a factorial experiment across three major audit instruments, administering them to six frontier LLMs while varying the asker's stated identity.

Result: The results show that LLMs' responses shift significantly when the asker's identity is changed, with a notable rightward shift when the asker identifies as a conservative Republican.

Conclusion: The study concludes that political bias in LLMs is not a fixed point on an ideological scale but a response profile that must be mapped across realistic interlocutors.
...
TL;DR: A new framework called Generative Structure Search (GSS) is introduced for efficiently discovering stable and metastable structures in molecular and materials science.


<details>
  <summary>Details</summary>
Motivation: The high cost of searching high-dimensional energy landscapes limits the discovery of stable and metastable structures in molecular and materials science.

Method: GSS combines diffusion-based generation and random structure search (RSS) using learned score fields and physical forces.
...
TL;DR: The paper investigates the impact of prompt optimization (PO) on Large Language Model (LLM) evaluations, finding that PO significantly affects model rankings.


<details>
  <summary>Details</summary>
Motivation: Current LLM evaluation frameworks use static prompt templates for all models, unlike industry practices that optimize prompts for each model to maximize performance.

Method: The study examines the effect of prompt optimization on LLM evaluations using public academic and internal industry benchmarks.

Result: The results show that PO greatly affects the final ranking of models, emphasizing the need for PO per model during evaluations.

Conclusion: The paper concludes that prompt optimization is crucial for accurate LLM evaluations, and practitioners should perform PO per model to choose the best model for a given task.
...
<details>
  <summary>Details</summary>
Motivation: Language models require context learning to reason over complex contexts that exceed their parametric knowledge, but constructing skills for context learning scenarios faces challenges such as high annotation cost and lack of external feedback.

Method: Ctx2Skill uses a multi-agent self-play loop with Challenger, Reasoner, and Judge agents to autonomously discover, refine, and select context-specific skills, along with a Cross-time Replay mechanism to ensure robust skill evolution.

Result: Ctx2Skill consistently improves solving rates across backbone models on four context learning tasks from CL-bench.

Conclusion: Ctx2Skill provides a novel solution for context learning by autonomously discovering and refining context-specific skills, enabling language models to better learn from complex contexts.
...
Result: The paper provides a comprehensive review of fairness notions and metrics, supporting consistent and transparent planning and decision-making in distribution network operations.

Conclusion: The study aims to support informed decision-making in distribution network operations by reviewing fairness notions and metrics and their implications.
...
TL;DR: TEA Nets, a computational framework, extracts subjects, verbs, and objects from texts, demonstrating its ability in emotion detection, semantic frame analyses, and linguistic inquiries across various texts, including conspiracy theories and LLM-generated responses.


<details>
  <summary>Details</summary>
Motivation: The authors aim to introduce a computational framework, TEA Nets, to extract and analyze emotional, syntactic, and semantic insights from narratives, leveraging cognitive network science and artificial intelligence.

Method: The authors implemented TEA Nets as an open-source Python library and tested it in three case studies: analyzing conspiracy texts and textual responses generated by LLMs, focusing on emotion detection, semantic frame analyses, and linguistic inquiries.

Result: The results show that TEA Nets can extract relevant emotional, syntactic, and semantic insights from narratives. Specifically, it revealed differences in linguistic patterns between high-conspiratorial and low-similarity conspiracy narratives, and highlighted emotional differences between human and LLM-based psychotherapy transcripts.

Conclusion: The study concludes that TEA Nets can effectively extract emotional, syntactic, and semantic insights from narratives, opening new avenues for text analysis with cognitive network science, and potentially informing psychotherapy training on LLM-simulated patients.
...
<details>
  <summary>Details</summary>
Motivation: The authors argue that current agentic memory systems conflate lookup and memory, which is a category error with significant consequences.

Method: The authors draw on Complementary Learning Systems theory from neuroscience and formalize the limitations of current AI memory systems.

Result: The authors identify limitations in current AI agents, including an inability to develop expertise, a generalization ceiling on novel tasks, and vulnerability to memory poisoning.

Conclusion: The authors propose a co-existence approach and call for action among system builders, benchmark designers, and the memory community to address these limitations.
...
Conclusion: The proposed agentic framework effectively constructs knowledge graphs and retrieves policy-relevant information to answer questions, improving the performance of LLMs.
...
TL;DR: The study audits the performance of five vision-language models (VLMs) in clinical settings, focusing on their ability to accurately localize anatomical and pathological targets and answer medical questions. The results show that the models struggle with localization and exhibit clinically dangerous errors, but fine-tuning one of the models improves its performance.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this study is to evaluate the trustworthiness and reliability of vision-language models (VLMs) in clinical settings, particularly under realistic failure conditions. The authors aim to characterize the failure landscape of these models on specialized medical inputs.

Method: The authors audit five recent frontier VLMs (Gemini 2.5 Pro, GPT-5, o3, GLM-4.5V, Qwen 2.5 VL) on Medical VQA along two trust-relevant axes: perception and pipeline integration. They also perform supervised fine-tuning of Qwen 2.5 VL on combined Med-VQA training data.

Result: The results show that the models perform poorly in localizing anatomical and pathological targets, with the best model reaching only 0.23 mean IoU and 19.1% Acc@0.5. The self-grounding pipeline degrades VQA accuracy for every model, but replacing predicted boxes with ground-truth annotations recovers and improves VQA accuracy. Fine-tuning Qwen 2.5 VL achieves the highest reported SLAKE open-ended recall (85.5%) among comparable methods.

Conclusion: The study concludes that grounding quality is a primary trustworthiness bottleneck in the SLAKE bounding-box setting. While fine-tuning can improve the VQA-level gap, it is unclear whether this also closes the perception/trustworthiness bottleneck, leaving it to future work.
...
<details>
  <summary>Details</summary>
Motivation: Current medical RAG systems ignore visual content in biomedical literature, and the authors propose a multimodal approach to leverage this information.

Method: MED-VRAG uses ColQwen2.5 patch-level page embeddings and a sharded MapReduce LLM filter to retrieve and reason over PMC document page images, with iterative refinement via a vision-language model.

Result: MED-VRAG achieves 78.6% average accuracy across four medical QA benchmarks, outperforming baselines by +5.8 points with retrieval and +1.8 points over MedRAG + GPT-4.

Conclusion: The proposed MED-VRAG framework effectively utilizes visual content in biomedical literature, demonstrating the value of multimodal retrieval and reasoning for improved medical question answering performance.
...
TL;DR: The study examines consumer attitudes towards healthcare AI in Australia, finding moderate optimism, concerns about accuracy and safety, and a preference for AI-generated consultation summaries over clinician-written ones.


<details>
  <summary>Details</summary>
Motivation: The study aims to investigate consumer readiness, acceptance, trust, and risk perceptions of healthcare AI, specifically in the context of patient-facing applications, as successful deployment depends on consumer attitudes.

Method: A mixed-methods survey study was conducted in Australia with 275 participants, combining a survey with a scenario-based evaluation of AI-generated versus clinician-written consultation summaries.

Result: The study found that participants expressed moderate optimism, strong perceived usefulness and ease of use, but also substantial concerns about accuracy, safety, and data use. The AI-generated summary was strongly preferred for quality, empathy, and overall usefulness.

Conclusion: The findings highlight the need for clinically supervised deployment frameworks beyond technical performance alone, as consumers judge AI through concrete communication quality and visible human governance.
...
Method: The framework uses a three-layer system: perception, conceptualization, and action. It relies on physical sensors, edge computing, and a digital twin to simulate traffic flow and make real-time autonomous decisions.

Result: The framework outperforms fixed-time and reinforcement learning-based baselines in minimizing waiting time at traffic lights and improving traffic flow effectiveness.

Conclusion: The proposed framework offers a promising approach to optimizing traffic light control, with potential to reduce congestion and waiting times in traffic networks.
...
TL;DR: The paper introduces Intent2Tx, a high-fidelity benchmark for evaluating Large Language Models (LLMs) in translating user intents into on-chain transactions in Web3, revealing limitations in current models' ability to generalize and plan multi-step transactions.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks fail to capture the complexity of translating high-level user intents into functionally correct, state-dependent on-chain transactions, which is crucial for developing autonomous agents in intent-centric Web3 ecosystems.

Method: The authors propose an execution-aware framework employing differential state analysis on forked mainnet environments, and introduce Intent2Tx, a benchmark featuring 29,921 single-step and 1,575 multi-step instances derived from real-world Ethereum mainnet traces.

Result: The evaluation of 16 state-of-the-art LLMs shows that while scaling and retrieval-augmentation enhance logical consistency and parameter precision, current models struggle with out-of-distribution generalization and multi-step planning, highlighting a significant gap in 'reasoning-to-execution' capabilities.

Conclusion: The paper emphasizes the need for developing more advanced LLMs that can accurately translate user intents into on-chain transactions, and provides Intent2Tx as a critical foundation for this development.
...
TL;DR: The paper introduces WindowsWorld, a benchmark for evaluating GUI agents in cross-application workflows, highlighting their poor performance in complex, multi-step tasks.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks for GUI agents focus on isolated, single-application tasks, neglecting the real-world need for coordinating across multiple applications to accomplish complex workflows.

Method: The authors use a multi-agent framework with 16 occupations to generate tasks, which are then refined by human review and executed in a simulated environment.

Result: Leading large models and agents perform poorly on multi-application tasks, with a success rate of less than 21%, and struggle with tasks requiring conditional judgment and reasoning across multiple applications.

Conclusion: The paper emphasizes the need for improved GUI agents that can handle complex, multi-application workflows, and provides a benchmark and resources for future research.
...
TL;DR: The authors propose PARA, a method to compress Low-Rank Adaptation (LoRA) in foundation models by adaptively allocating ranks to layers based on their importance, reducing parameters by 75-90% without sacrificing performance.


<details>
  <summary>Details</summary>
Motivation: The widespread adoption of LoRA in modern foundation models leads to parameter redundancy due to uniform rank enforcement across layers, disregarding their varying intrinsic dimensionality.

Method: Post-Optimization Adaptive Rank Allocation (PARA), a data-free compression method that uses Singular Value Decomposition to prune LoRA ranks with a global threshold over singular values across all layers.
...
Conclusion: The proposed PARA method effectively reduces parameter redundancy in LoRA by allocating ranks based on layer-wise spectral importance, making it a practical and efficient compression technique for existing fine-tuning pipelines.
...
TL;DR: The paper discusses designing dependable autonomous and embedded systems with AI and ML components, highlighting new challenges and emerging methodologies for ensuring reliability, safety, and security.


<details>
  <summary>Details</summary>
Motivation: The increasing complexity of embedded safety-critical systems, particularly those using AI and ML components, poses new challenges for ensuring dependability, reliability, safety, and security.

Method: The paper explores emerging methodologies, architectures, and frameworks for designing dependable autonomous and embedded systems, including advances in reliability modeling, secure system design, and certification approaches.
...
Conclusion: The paper concludes that a holistic approach is needed to ensure dependability in embedded safety-critical systems with AI and ML components, and highlights the need for new methodologies and frameworks to address these challenges.
...
Result: Policy-violating propagation rates range from 11.5% to 41.3% across 5 models, with pathway-specific propagation concentrated in browser-mediated data flows.

Conclusion: The study highlights the need for improved defenses, as prompt-mitigation efforts can reduce policy-violating propagation but may not suffice alone, and varying effectiveness is seen across models.
...
TL;DR: The paper introduces OBJECTGRAPH, a new file format designed for autonomous LLM agents to efficiently retrieve and process information, reducing token usage by up to 95.3% without sacrificing task accuracy.


<details>
  <summary>Details</summary>
Motivation: The current document formats are not suitable for autonomous LLM agents, as they are designed for human readers. This mismatch leads to inefficiencies in token usage and processing.

Method: The authors propose a new file format, OBJECTGRAPH (.og), which represents documents as typed, directed knowledge graphs. They formalize the Document Consumption Problem and characterize six structural properties required for an ideal format.

Result: The empirical evaluation shows that OBJECTGRAPH achieves up to 95.3% token reduction without statistically significant degradation in task accuracy (p > 0.05). The transpiler fidelity reaches 98.7% content preservation.

Conclusion: The paper concludes that OBJECTGRAPH is a suitable format for autonomous LLM agents, addressing the Document Consumption Problem and providing a more efficient way of processing information.
...
### [198] [A Grid-Aware Agent-Based Model for Analyzing Electric Vehicle Charging Systems](https://arxiv.org/abs/2604.27849)
...
Method: An ABM is developed using Python and SimPy, integrating EV behavior, charging column constraints, and a shared Energy Sandbox for regulating power allocation.
...
Conclusion: The proposed ABM provides a flexible and extensible simulation environment for exploring EV charging ecosystems and serves as a basis for subsequent studies on advanced coordination strategies.
...
### [199] [Rethinking Agentic Reinforcement Learning In Large Language Models](https://arxiv.org/abs/2604.27859)
...
<details>
  <summary>Details</summary>
Motivation: The traditional RL approach has limitations in handling complex, open-ended tasks and uncertain real-world environments, necessitating a paradigm shift towards more autonomous and adaptive agents.

Method: The paper provides an overview of the conceptual foundations, methodological innovations, and effective designs of LLM-based Agentic RL, highlighting its differences from traditional RL approaches.

Result: The paper outlines the potential of LLM-based Agentic RL in developing autonomous agents with goal-setting, planning, and interactive reasoning capabilities, but also identifies critical challenges and future directions.

Conclusion: The paper concludes that LLM-based Agentic RL represents a promising direction for developing more advanced and autonomous agents, but further research is needed to address the challenges and realize its full potential.
...
### [200] [From Unstructured Recall to Schema-Grounded Memory: Reliable AI Memory via Iterative, Schema-Aware Extraction](https://arxiv.org/abs/2604.27906)
...
<details>
  <summary>Details</summary>
Motivation: Current AI memory designs are mismatched to the needs of agents in production, which require exact facts, current state, updates and deletions, aggregation, relations, negative queries, and explicit unknowns.
...
Conclusion: The results show that, for memory workloads requiring stable facts and stateful computation, architecture matters more than retrieval scale or model strength alone, and that schema-grounded memory is essential for reliable external AI memory.
...
TL;DR: The paper introduces KellyBench, an environment for evaluating sequential decision-making in sports betting markets, and finds that current frontier models perform poorly, losing money on average, with significant room for improvement.
...
Result: The result shows that all evaluated frontier models lose money on average, with the best performing model achieving an average return of -8%, and many models experiencing ruin across seeds.

Conclusion: The conclusion is that current models are not sophisticated enough, with significant room for improvement, as evaluated by a human expert rubric, and KellyBench is made available as an open-access API endpoint.
...
### [202] [MM-StanceDet: Retrieval-Augmented Multi-modal Multi-agent Stance Detection](https://arxiv.org/abs/2604.27934)
*Weihai Lu,Zhejun Zhao,Yanshu Li,Huan He*

Main category: cs.AI

TL;DR: A novel multi-agent framework called MM-StanceDet is proposed for multimodal stance detection, which integrates retrieval augmentation, specialized multimodal analysis agents, reasoning-enhanced debate, and self-reflection to improve performance.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the challenges of effectively fusing text and image, especially with conflicting signals, in multimodal stance detection.

Method: The proposed method, MM-StanceDet, uses a multi-agent framework with four stages: Retrieval Augmentation for contextual grounding, specialized Multimodal Analysis agents for nuanced interpretation, a Reasoning-Enhanced Debate stage for exploring perspectives, and Self-Reflection for robust adjudication.

Result: Extensive experiments on five datasets demonstrate that MM-StanceDet significantly outperforms state-of-the-art baselines.

Conclusion: The proposed MM-StanceDet framework validates the efficacy of its multi-agent architecture and structured reasoning stages in addressing complex multimodal stance challenges.
...
### [203] [Modeling Clinical Concern Trajectories in Language Model Agents](https://arxiv.org/abs/2604.27872)
*Sukesh Subaharan,Venkatesan VS,Murugadasan P,Sivakumar D,Gautham N,Ganeshkumar M*

Main category: cs.AI

TL;DR: The paper proposes a method to make large language model (LLM) agents more clinically legible by introducing explicit state dynamics, allowing for the detection of pre-escalation signals in clinical settings.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the issue of LLM agents in clinical settings exhibiting abrupt behavior, making it difficult for clinicians to anticipate and intervene before escalation. The authors aim to create a more transparent and anticipatory system.

Method: The authors introduce a lightweight agent architecture that integrates a memoryless clinical risk encoder over time using first- and second-order dynamics, producing a continuous escalation pressure signal.

Result: The results show that the proposed method, specifically the use of second-order dynamics, produces smooth, anticipatory concern trajectories, surfacing sustained unease prior to escalation, and enabling human-in-the-loop monitoring and more informed intervention.

Conclusion: The paper concludes that explicit state dynamics can make LLM agents more clinically legible by revealing how long concern has been rising, not just when thresholds are crossed, allowing for more informed intervention and better patient care.
...
### [204] [Mapping the Methodological Space of Classroom Interaction Research: Scale, Duration, and Modality in an Age of AI](https://arxiv.org/abs/2604.28098)
*Dorottya Demszky,Edith Bouton,Alison Twiner,Sara Hennessy,Richard Correnti*

Main category: cs.AI

TL;DR: A framework is proposed to map classroom interaction research along scale, duration, and modality dimensions, and its application is demonstrated through case studies and an interview.


<details>
  <summary>Details</summary>
Motivation: To bridge the divide between large-scale observation and in-depth ethnographic work in classroom interaction research by providing a methodological framework.

Method: A framework mapping the methodological space of classroom interaction research along three dimensions: scale, duration, and modality.

Result: The framework can guide research and tool design, and AI is expanding the space of classroom interaction research.

Conclusion: The proposed framework can help researchers understand the strengths and limitations of different studies and guide future research and tool design in classroom interaction.
...
TL;DR: The paper proposes on-demand persona-based agent generation for more efficient and contextually appropriate interactions in agentic workflows.


<details>
  <summary>Details</summary>
Motivation: Current agent systems rely on hard-coded architectures that limit personalization and adaptation to individual needs.

Method: The paper presents a pipeline for on-demand persona generation in agentic platforms.

Result: The proposed approach allows for dynamic crafting of agents and personas at run-time to match user characteristics, task demands, and workflow context.

Conclusion: The paper argues that on-demand persona-based agent generation offers a promising path towards more efficient and contextually appropriate interaction within agentic workflows.
...
<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to evaluate the necessity of external agent orchestration frameworks in modern LLM systems for procedural tasks.

Method: The study uses a controlled comparison, evaluating 200 conversations per condition across three domains (travel booking, Zoom technical support, and insurance claims processing) using LLM-as-judge scoring on five quality criteria.

Result: The in-context approach scores 4.53-5.00 on a 5-point scale, outperforming the external orchestration approach (LangGraph) which scores 4.17-4.84. The in-context approach also shows lower failure rates across all domains.

Conclusion: The study concludes that advances in frontier model capabilities have made external orchestration unnecessary for multi-turn conversations following a defined procedure, and that a simpler in-context approach can achieve better results.
...
### [207] [Synthetic Computers at Scale for Long-Horizon Productivity Simulation](https://arxiv.org/abs/2604.28181)
*Tao Ge,Baolin Peng,Hao Cheng,Jianfeng Gao*

Main category: cs.AI

TL;DR: The paper introduces a methodology for creating synthetic computer environments to simulate long-horizon productivity tasks, enabling scalable experiential learning and improved agent performance.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the challenge of scaling synthetic data creation for long-horizon productivity scenarios, which are highly dependent on user-specific computer environments.

Method: The authors propose 'Synthetic Computers at Scale', a methodology for creating synthetic computer environments with realistic folder hierarchies and content-rich artifacts, and running long-horizon simulations using two agents: one creating productivity objectives and the other acting as the user to complete them.

Result: Preliminary experiments show significant improvements in agent performance on both in-domain and out-of-domain productivity evaluations, demonstrating the effectiveness of the experiential learning signals generated by the simulations.

Conclusion: The authors argue that scalable synthetic computer creation and simulations have the potential to be a foundational substrate for agent self-improvement and agentic reinforcement learning in long-horizon productivity scenarios, enabling broader coverage of diverse professions, roles, and environments.
...
Result: The paper provides a comprehensive survey of GWMs, including key design principles, representative models, and comparative analyses, as well as discussing open challenges and future directions.
...
TL;DR: HealthFormer, a decoder-only transformer, models human physiological trajectories and forecasts individual health outcomes, improving prediction for disease and mortality endpoints.
...
Method: HealthFormer was trained on data from the Human Phenotype Project, tokenising each participant's health trajectory across 667 measurements, and was used to forecast individual physiological trajectories.

Result: HealthFormer improved prediction for 27 of 30 incident-disease and mortality endpoints, and simulated interventions in silico with accurate results.
...
TL;DR: Introduced LAPITHS framework to evaluate cognitive plausibility of AI models, challenging claims of human-like intelligence in models like CENTAUR.


<details>
  <summary>Details</summary>
Motivation: Counteract the tendency to interpret high performance of language models as evidence of human-like cognition and cognitive abilities.

Method: LAPITHS framework uses two quantitative assessments: Minimal Cognitive Grid and behavioral comparison to evaluate cognitive plausibility.

Result: Several major claims of models like CENTAUR are not theoretically or empirically justified, and similar results can be reproduced by systems lacking cognitive plausibility.

Conclusion: LAPITHS provides a principled reference point for evaluating cognitive plausibility of AI models, challenging overly optimistic interpretations of their human-like abilities.
...
TL;DR: A unified framework, the Game-Theoretic Free Energy Principle, is proposed to explain collective intelligence in multi-agent systems, linking Bayesian inference and strategic interaction.


<details>
  <summary>Details</summary>
Motivation: The lack of a unifying principle governing collective intelligence across different systems, despite its emergence in biological, physical, and artificial systems.

Method: Introduction of the Game-Theoretic Free Energy Principle, which shows that local free-energy minimisation in multi-agent systems corresponds to stochastic games and approximate Nash equilibria.

Result: A predictive theory of cooperation is established, with a non-monotonic relationship between sensory precision and agent influence, validated across neural, biological, and artificial multi-agent systems.

Conclusion: The Game-Theoretic Free Energy Principle identifies a common variational principle underlying inference, thermodynamics, and game-theoretic equilibrium, unifying collective intelligence across different systems.
...
TL;DR: This paper studies how visual inputs affect the cooperative behavior of Vision-Language Models (VLMs) in decision-making, focusing on the Iterated Prisoner's Dilemma as a test case.


<details>
  <summary>Details</summary>
Motivation: Understanding how visual inputs influence the behavior of VLMs as they become increasingly integrated into decision-making systems.

Method: The study uses the Iterated Prisoner's Dilemma (IPD) and exposes VLMs to images depicting behavioral concepts and color-coded reward matrices.
...
Conclusion: The findings highlight the importance of robust evaluation frameworks for VLM deployment and the need to investigate how architectural and training differences affect behavioral responses.
...
TL;DR: This paper provides a comprehensive overview of the intersection between Reinforcement Learning (RL) and Graphical User Interface (GUI) agents, proposing a taxonomy and analyzing trends and future directions.
...
TL;DR: The paper presents a method using language models to improve mechanical linkage designs by exploring discrete topologies and fitting continuous parameters, resulting in significant reductions in geometric error and improvements in structural validity.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to leverage language models for systematically improving mechanical linkage designs, which involves both combinatorial topology selection and continuous parameter fitting.

Method: The method involves using language model agents to explore discrete topologies while numerical optimisers fit continuous parameters. A symbolic lifting operator is used to translate simulator trajectories into qualitative descriptors that models can interpret across iterative design cycles.

Result: The results show that the modular architecture reduces geometric error by up to 68% and improves structural validity by up to 134% over monolithic baselines. The system also correctly diagnoses failure modes and proposes corrections.

Conclusion: The conclusion is that principled symbolic abstraction can bridge generative AI and the numerical precision required for engineering design, enabling language models to acquire interpretable mechanical reasoning strategies without fine-tuning.
...
Conclusion: The paper concludes that applying splitting to the knowledge base level and generalising it to a parametrised version for ABAFs can potentially mitigate the computational complexity issues in ABA.
...
TL;DR: The paper explores using AI models for generating TCG cards, enabling personalized designs and potentially improving player experience.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenges of repetitive and impaired player experiences in Trading Card Games due to predictable strategies and limited card options.

Method: The method involves a pipeline combining player-centric co-creation, fine-tuned embeddings, local LLMs, and Diffusion Models to generate dynamic, personalized cards.

Result: The results show high satisfaction among participants, with most successfully realizing their ideas through prompt adjustments, indicating potential for future content generation systems.

Conclusion: The conclusion is that using Large Language Models and Image Diffusion Models for Procedural Content Generation of TCG cards can enable a personalized infinity of card designs, fostering unique connections between players and their cards.
...
TL;DR: The paper compares the performance of different large language model (LLM) agents on scientific visualization tasks, evaluating their visualization quality, efficiency, robustness, and computational cost.


<details>
  <summary>Details</summary>
Motivation: To examine how different types of LLM agents perform on scientific visualization tasks and identify the strengths and limitations of each interaction paradigm.
...
TL;DR: The paper proposes an architectural pattern language for integrating multimodal foundation models into enterprise ecosystems, balancing determinism and real-time performance with the non-determinism of vision language action models.


<details>
  <summary>Details</summary>
Motivation: The integration of multimodal foundation models into enterprise ecosystems presents a software architecture challenge due to the need to balance competing quality attributes such as latency, non-determinism, determinism, and real-time performance.
...
Result: The proposed architectural pattern language allows for the separation of fast, deterministic reflexes from slow, probabilistic supervision, enabling the integration of multimodal foundation models into enterprise ecosystems.

Conclusion: The paper presents a solution to the software architecture challenge of integrating multimodal foundation models into enterprise ecosystems, providing a pattern language that supports the coexistence of deterministic and non-deterministic components.
...
<details>
  <summary>Details</summary>
Motivation: The authors aim to address the challenges of multimodal large language models (MLLMs) in understanding scientific spectra, which are highly information-dense and domain-specific.

Method: The authors propose a spectral data sampling and interpolation reconstruction approach to reduce token length while preserving essential curve characteristics. They also introduce SpecVQA, a benchmark with 620 figures and 3100 QA pairs curated from peer-reviewed literature.

Result: The ablation studies confirm that the proposed approach achieves substantial performance improvements on the SpecVQA benchmark. The authors also present a leaderboard of prominent MLLMs in scientific spectral understanding.

Conclusion: The paper represents an essential step toward enhancing spectral understanding in multimodal large models and suggests promising directions for extending visual-language models to broader scientific research and data analysis.
...
TL;DR: A new framework, STEF, is proposed for evaluating Text-to-SQL (T2SQL) systems in production environments without needing database schema or reference queries.


<details>
  <summary>Details</summary>
Motivation: Current T2SQL evaluation methods are not suitable for production environments as they rely on ground-truth queries and structured database schema, which are often not available.

Method: STEF operates on natural language inputs and generated SQL, extracting semantic specifications and producing an accuracy score via a composite metric.
...
Conclusion: STEF provides a production-native evaluation system for T2SQL agents, addressing the challenges of existing benchmarks and enabling continuous improvement.
...
TL;DR: The paper proposes a protocol called RHyVE for verifying and deploying generated rewards in reinforcement learning with large language models, which improves performance by competence-aware verification and phase-aware deployment.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the deployment-time problem of generated rewards in reinforcement learning, which are not automatically reliable training objectives, and to study the verification and deployment of reward candidates as coupled problems.
...
TL;DR: The study examines the phenomenon of emergent misalignment in large language models, where fine-tuning on narrowly misaligned data leads to broadly misaligned behavior. The authors investigate the consistency of this misalignment across tasks and domains.


<details>
  <summary>Details</summary>
Motivation: To understand the consistency of emergent misalignment in large language models and its relationship to self-assessment, as prior work has only found a correlation between harmful behavior and self-assessment in emergently misaligned models.

Method: The authors fine-tune the Qwen 2.5 32B Instruct model on six narrowly misaligned domains and conduct experiments, including harmfulness evaluation, self-assessment, and output recognition.
...
Conclusion: The findings suggest that the effects of emergent misalignment are more complex than previously thought, calling into question the consistency of the EM persona and highlighting the need for further research.
...
TL;DR: The paper provides guidelines for writing good benchmark tasks for terminal-agent benchmarks, highlighting the differences between writing prompts and benchmark tasks, and discussing common failure modes.


<details>
  <summary>Details</summary>
Motivation: The growing market for evaluation environments and the pressure to quickly ship tasks without thorough review can lead to poorly designed benchmark tasks, which can result in inaccurate measurements of large language models' capabilities.

Method: The authors draw from their experience contributing to and reviewing tasks for Terminal Bench to identify common failure modes and provide guidelines for writing good benchmark tasks.
...
Conclusion: The paper aims to serve as a useful reference for benchmark maintainers, task contributors, and researchers using benchmark scores as evidence, by providing guidelines for writing good benchmark tasks that are adversarial, difficult, and legible.
...
TL;DR: The paper proposes new splitting techniques for argumentation frameworks that include supports between defeasible elements, and proves their correctness for common argumentation semantics.


<details>
  <summary>Details</summary>
Motivation: The increase in expressiveness of bipolar set-based argumentation frameworks (BSAFs) calls for diverse forms of splitting.
...
Result: The paper proves the correctness of the proposed splitting techniques for common argumentation semantics.

Conclusion: The paper proposes novel splitting techniques for argumentation formalisms with supports, and establishes their correctness.
...
TL;DR: The paper critiques AI systems for sign language recognition, arguing they are built without input from deaf communities, perpetuating ableism and marginalization.


<details>
  <summary>Details</summary>
Motivation: The authors aim to highlight the issues with current AI systems for sign language recognition, which are built from biased data and ignore the culture, semantics, and colloquial language of sign languages.

Method: The authors use the theoretical framework of Ellul's Technological System and Technological bluff to analyze the phenomenon of AI systems for sign language recognition.

Result: The authors argue that current AI systems for sign language recognition are Ableist Intelligence, which emphasizes the humiliated and marginalized nature of sign language and deaf culture.

Conclusion: The paper concludes that AI systems for sign language recognition should prioritize the needs and perspectives of deaf communities, rather than being driven by productivity and efficiency goals that perpetuate ableism and marginalization.
...
TL;DR: The paper introduces Intern-Atlas, a graph that models the evolution of research methods, capturing their relationships and development over time, and enabling applications like idea evaluation and automated idea generation.


<details>
  <summary>Details</summary>
Motivation: Existing research infrastructure lacks explicit representations of methodological evolution, hindering AI-driven research agents from understanding how methods emerge, adapt, and build upon one another.

Method: The authors built Intern-Atlas from 1,030,314 papers, inferring lineage relationships among methodologies and capturing bottlenecks that drive transitions between innovations. They also propose a self-guided temporal tree search algorithm for constructing evolution chains.

Result: The resulting graph comprises 9,410,201 semantically typed edges, grounded in verbatim source evidence, and shows strong alignment with expert-curated ground-truth evolution chains. The graph enables downstream applications in idea evaluation and automated idea generation.

Conclusion: Methodological evolution graphs, as exemplified by Intern-Atlas, are a foundational data layer for emerging automated scientific discovery, providing a queryable causal network of methodological development.
...
TL;DR: The paper proposes using large language models (LLMs) to refine graph edges in EEG signal processing for seizure detection, leading to improved accuracy and more interpretable graph representations.


<details>
  <summary>Details</summary>
Motivation: The noisy nature of EEG signals makes robust representation learning challenging, and existing graph construction methods often generate redundant or irrelevant edges, which impairs graph representation quality and downstream task performance.
...
Conclusion: The proposed framework effectively leverages LLMs to refine graph edges in EEG signal processing, leading to improved seizure detection accuracy and more meaningful graph structures.
