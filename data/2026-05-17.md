...
TL;DR: The paper proposes a new method called Contrastive Multi-modal Hypergraph Reasoning for multi-person 3D reconstruction in crowded scenes, which combines semantic, geometric, and pose cues to improve accuracy.


<details>
  <summary>Details</summary>
Motivation: Current approaches for multi-person 3D reconstruction rely on single-modality inputs and often neglect the collective group context, leading to limitations in handling severe occlusions and depth ambiguity in crowded scenes.

Method: The proposed method uses a hypergraph-based contrastive learning scheme to synergize semantic, geometric, and pose cues, and introduces a pelvis depth indicator as a global spatial anchor to align visual features with a metric-scale-agnostic depth ordering.

Result: The method achieves new state-of-the-art performance on the Panoptic and GigaCrowd benchmarks.

Conclusion: The paper concludes that the proposed Contrastive Multi-modal Hypergraph Reasoning method effectively addresses the limitations of current approaches for multi-person 3D reconstruction, and demonstrates improved accuracy in handling crowded scenes with severe occlusions.
...
TL;DR: The study investigates 'massive activations' in Diffusion Transformers (DiTs), a subset of hidden-state channels that are crucial for generating images from text prompts. These channels are found to be functionally critical, spatially organized, and transferable, allowing for semantic interpolation and subject-driven generation without additional training.


<details>
  <summary>Details</summary>
Motivation: Understanding how prompts shape image semantics in text-to-image generators, specifically in Diffusion Transformers (DiTs) and related architectures.

Method: Analyzing 'massive activations' in DiTs, which are a small subset of hidden-state channels with significantly larger responses than the rest. The study uses controlled disruption probes, spatial organization analysis, and transferability experiments to investigate these channels.

Result: The study found that massive activations are functionally critical for generation quality, spatially organized to align with the main subject and salient regions, and transferable for semantic interpolation and subject-driven generation.

Conclusion: Massive activations in DiTs are not anomalies but a sparse prompt-conditioned carrier subspace that organizes and controls semantic information, enabling applications like text-conditioned and image-conditioned semantic transport.
...
TL;DR: A novel 4D pipeline, CineMesh4D, is proposed for accurate 3D+t whole-heart mesh reconstruction from multi-view 2D cine MRI, outperforming existing approaches in reconstruction quality and motion consistency.


<details>
  <summary>Details</summary>
Motivation: The difficulty in accurately reconstructing 3D+t whole-heart mesh from cine MRI due to sparse sampling of 3D cardiac anatomy by 2D image slices and the tight coupling between cardiac shape and motion.

Method: CineMesh4D, a novel end-to-end 4D pipeline that uses cross-domain mapping, differentiable rendering loss, and a dual-context temporal block to reconstruct patient-specific whole-heart mesh from multi-view 2D cine MRI.

Result: CineMesh4D outperforms existing approaches in terms of reconstruction quality and motion consistency, providing a practical pathway for personalized real-time cardiac assessment.

Conclusion: The proposed CineMesh4D pipeline offers a promising solution for accurate 3D+t whole-heart mesh reconstruction from cine MRI, with potential applications in personalized cardiac assessment.
...
TL;DR: A new model is proposed to improve visual understanding by unifying pixel and word tokens in a generative language model, achieving good performance even with small models and limited training data.


<details>
  <summary>Details</summary>
Motivation: The current state-of-the-art open-source multimodal models using Vision Transformer (ViT) as the vision encoder backbone have limitations in visual understanding for details, such as recognizing small text or numbers in images.

Method: The proposed model unifies pixel and word tokens in a generative language model, featuring pixel token embedding, color folding, global conditional attention approximation, and image unsupervised pretraining.

Result: The experimental results show that the new model has good performance even with small models and limited training data, and is expected to improve with increased model parameters and training data.

Conclusion: The proposed model has the potential to improve visual understanding capabilities, particularly in recognizing small details in images, and is expected to conform to the scaling law with increased model parameters and training data.
...
TL;DR: The paper proposes PVRF, a unified framework for removing adverse weather from real-world images, integrating soft weather perceptions with a refinement process to improve image restoration.


<details>
  <summary>Details</summary>
Motivation: Adverse weather removal in real-world images is challenging due to heterogeneous and unseen degradations, and current methods often produce overly smooth results.

Method: The PVRF framework uses a question answering module (AWR-QA) with frozen vision-language models to estimate weather type probabilities and attribute scores, which condition restoration networks, and then refines the estimate using a terminal-consistent residual rectified flow.
...
Conclusion: The proposed PVRF framework effectively addresses the challenges of adverse weather removal in real-world images, offering improved performance and generalization capabilities.
...
TL;DR: The authors propose a hardware-aware framework using genetic programming to evolve layer-specific scalar functions for approximating normalization layers in Vision Transformers, enabling efficient deployment on edge devices.


<details>
  <summary>Details</summary>
Motivation: The deployment of Vision Transformers (ViTs) on edge devices is hindered by the computational complexity and global reduction bottleneck imposed by layer normalization.

Method: The authors use genetic programming to evolve heterogeneous, layer-specific scalar functions directly from pre-trained weights, and propose a novel post-training re-alignment strategy.

Result: The approach recovers 84.25% Top-1 ImageNet-1K accuracy in only 20 epochs, and captures 91.6% of the variance (R^2) compared to homogeneous baselines.

Conclusion: The proposed approach establishes a highly favourable trade-off between arithmetic complexity and off-chip memory traffic, removing a primary barrier to the efficient deployment of ViTs on edge accelerators.
...
TL;DR: The paper introduces CurveBench, a benchmark for hierarchical topological reasoning from visual input, and evaluates various models on it, revealing a significant gap in exact topology-aware visual reasoning.


<details>
  <summary>Details</summary>
Motivation: The authors aim to advance hierarchical topological reasoning from visual input, a task that requires recovering the full rooted containment tree induced by curves in an image.

Method: The authors propose CurveBench, a benchmark consisting of 756 images of pairwise non-intersecting Jordan curves, and evaluate models on structured prediction tasks, including tree-generation accuracy.

Result: The strongest evaluated model, Gemini 3.1 Pro, achieves 71.1% tree-generation accuracy on CurveBench-Easy and 19.1% on CurveBench-Hard. Fine-tuning open-weight vision-language models, such as Qwen3-VL-8B, improves performance to 33.3% on CurveBench-Easy.

Conclusion: The results indicate that exact topology-aware visual reasoning remains far from solved, particularly on the harder CurveBench-Hard dataset, highlighting the need for further research in this area.
...
TL;DR: A unified foundation model called DeFakerOne is proposed for Fake Image Detection and Localization (FIDL), achieving state-of-the-art performance across diverse scenarios and benchmarks.


<details>
  <summary>Details</summary>
Motivation: The rapid evolution of generative AI has reshaped image forgery, creating a mismatch between unified forgery generation and domain-specific detection paradigms, necessitating a unified FIDL approach.

Method: DeFakerOne, a data-centric, unified FIDL foundation model integrating InternVL2 and SAM2, is proposed for simultaneous image-level detection and pixel-level forgery localization.

Result: DeFakerOne achieves state-of-the-art performance on 39 forgery detection benchmarks and 9 localization benchmarks, with superior robustness against real-world perturbations and state-of-the-art generators.

Conclusion: DeFakerOne provides a systematic solution for unified FIDL, highlighting design principles for scalable, robust, and unified FIDL, and demonstrating its effectiveness across diverse scenarios.
...
TL;DR: The paper proposes DUET, a novel framework for inferring spatially resolved gene expression from histology images, combining parametric prediction and memory-based retrieval.


<details>
  <summary>Details</summary>
Motivation: Existing methods for inferring gene expression from histology images have limitations, including oversimplification and inability to balance flexibility and biological fidelity. Single-cell data is underutilized in vision-omics modeling.

Method: The authors propose a dual-paradigm framework, DUET, which uses parallel regression-retrieval pathways, incorporates large-scale single-cell references as biological constraints, and dynamically assigns branch preference across spatial contexts.

Result: DUET achieves state-of-the-art performance on three public datasets across varied gene scales, with consistent gains from each proposed component.

Conclusion: The proposed DUET framework offers a cost-effective and accurate method for inferring spatially resolved gene expression from histology images, leveraging single-cell data and achieving improved performance.
...
TL;DR: A two-tier edge-cloud cascade system is proposed for diabetic retinopathy detection, reducing cloud usage by half with a modest drop in grading performance.


<details>
  <summary>Details</summary>
Motivation: Diabetic Retinopathy (DR) is a leading cause of preventable blindness, but rural regions lack specialists and infrastructure for early detection. Cloud-based deep learning systems face challenges in these settings due to latency, bandwidth, and transmission costs.

Method: A two-tier edge-cloud cascade system is proposed, with Tier 1 using a lightweight MobileNetV3-small model on a local clinic device for binary triage, and Tier 2 using a RETFoundDINOv2 model in the cloud for ordinal severity grading.

Result: The cascade system achieves 98.99% sensitivity and 84.37% specificity in Tier 1, and 80.49% accuracy and 0.8167 quadratic weighted kappa in the deployed 4-class output space. Cloud use is reduced by about half compared to a cloud-only baseline.

Conclusion: The proposed edge-cloud cascade system effectively reduces cloud usage while maintaining grading performance, making it a viable solution for diabetic retinopathy detection in resource-constrained settings.
...
TL;DR: SToRe3D is a sparsity framework for Vision Transformers (ViTs) that accelerates 3D detection by selectively processing relevant 2D image tokens and 3D object queries.


<details>
  <summary>Details</summary>
Motivation: The high inference latency of ViTs in 3D detection tasks, primarily due to the processing of dense tokens and queries across multiple views and large 3D regions.

Method: SToRe3D introduces a relevance-aligned sparsity framework that jointly selects 2D image tokens and 3D object queries, storing filtered features for reactivation, and utilizes mutual 2D-3D relevance heads to allocate compute.

Result: SToRe3D achieves up to 3x faster inference with marginal accuracy loss on nuScenes and nuScenes-Relevance benchmarks.

Conclusion: SToRe3D enables real-time large-scale ViT-based 3D detection while maintaining accuracy on planning-critical agents.
...
TL;DR: The authors introduce ProtoMedAgent, a framework for generating clinical reports that combines multimodal inputs with neuro-symbolic constraints to improve faithfulness and reduce privacy risks.


<details>
  <summary>Details</summary>
Motivation: Interpretable prototype networks lack semantic structure for medical documentation, and standard Retrieval-Augmented Generation (RAG) methods can lead to 'retrieval sycophancy' where Large Language Models (LLMs) hallucinate rationalizations.

Method: ProtoMedAgent formalizes clinical reporting as an iterative optimization problem over a neuro-symbolic bottleneck, using a frozen prototype backbone, discrete semantic memory, and a reflective Scribe-Critic loop.

Result: ProtoMedAgent achieves 91.2% Comparison Set Faithfulness, outperforming standard RAG (46.2%), and reduces artifact-level membership inference risks by 9.8% through a binding ℓ-diversity phase transition.

Conclusion: ProtoMedAgent provides a framework for generating clinical reports that balances faithfulness, interpretability, and patient privacy, with significant improvements over standard methods.
...
TL;DR: PanoPlane enables high-fidelity sparse-view indoor novel view synthesis by reconstructing closed room geometry via panoramic scene completion.


<details>
  <summary>Details</summary>
Motivation: Current perspective-based methods for novel view synthesis are limited by their reliance on a limited field of view, leading to inaccurate results in sparse-view settings.

Method: PanoPlane leverages $360^{circ}$ panoramic completion and proposes Layout Anchored Attention Steering, a training-free mechanism that steers attention toward scene's detected planar surfaces.

Result: State-of-the-art novel view synthesis quality across 3, 6, and 9 input views, achieving up to $+17.8%$ improvement in PSNR over the current state-of-the-art baseline.

Conclusion: PanoPlane provides a robust approach for high-fidelity sparse-view indoor novel view synthesis, achieving superior results without requiring training or fine-tuning of the diffusion model.
...
TL;DR: TeDiO is a training-free method that improves temporal coherence in text-to-video diffusion transformers by regularizing internal attention patterns.


<details>
  <summary>Details</summary>
Motivation: Recent text-to-video diffusion transformers struggle with temporal coherence, producing flickering, drifting, or unstable motion.

Method: TeDiO estimates diagonal smoothness, identifies unstable regions, and performs lightweight latent updates to promote coherent frame-to-frame dynamics.

Result: TeDiO delivers markedly smoother motion while preserving per-frame visual quality across multiple video diffusion models.
...
TL;DR: The paper explores the idea that off-the-shelf embeddings are 'good enough' for complex tasks, proposing a non-parametric pipeline that bypasses backpropagation and achieves state-of-the-art performance in few-shot learning.


<details>
  <summary>Details</summary>
Motivation: The motivation is to investigate the Platonic Representation Hypothesis, which suggests that diverse architectures converge toward a shared latent space, and to examine if a 'good embedding' is all that's needed for complex tasks.

Method: The method involves utilizing a k-Nearest Neighbor classifier on frozen DINOv2-L features, conducting layer-wise characterization to identify optimal feature extraction, and applying manifold refinement via PCA and ICA.

Result: The results across four major benchmarks demonstrate that the approach consistently surpasses sophisticated meta-learning algorithms, achieving state-of-the-art performance.

Conclusion: The conclusion is that off-the-shelf embeddings are inherently 'good enough' for complex tasks, rendering intensive task-specific fine-tuning unnecessary, and that the proposed pipeline can achieve state-of-the-art performance in few-shot learning.
...
TL;DR: Proposed a lightweight U-Net architecture for face image super-resolution, achieving 8x magnification from 16x16 to 128x128 images. Utilized a novel supervision strategy with heatmaps from YOLO-World to improve reconstruction accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing face image super-resolution methods often rely on complex architectures, adversarial training, or separate alignment networks, leading to high computational costs. The goal is to develop a lightweight method that can accurately reconstruct high-resolution facial images from severely degraded inputs.

Method: A lightweight U-Net based-architecture is proposed. A novel auxiliary-training-free supervision strategy using heatmaps generated by YOLO-World is introduced to emphasize reconstruction errors in semantically important regions.

Result: Experiments on the CelebA dataset show that the proposed method improves quantitative metrics and produces sharper, more realistic reconstructions. The results demonstrate that lightweight networks can effectively exploit detection-driven priors for extreme upscaling without adversarial training or increased computational cost.

Conclusion: The proposed lightweight U-Net architecture with a novel supervision strategy effectively reconstructs high-resolution facial images from severely degraded inputs, achieving state-of-the-art results without increasing computational cost.
...
TL;DR: CoReDiT is a token pruning framework for Diffusion Transformers (DiTs) that reduces computational cost while maintaining visual quality.


<details>
  <summary>Details</summary>
Motivation: Diffusion Transformers (DiTs) have high computational costs, limiting their scalability and on-device deployment.

Method: CoReDiT uses a linear-time spatial coherence score to prune redundant tokens in self-attention and reconstructs skipped attention outputs via coherence-guided aggregation.

Result: CoReDiT achieves up to 55% self-attention FLOPs reduction and inference speedups of 1.33x on cloud GPUs and 1.72x on mobile NPUs.

Conclusion: CoReDiT enables efficient deployment of DiTs on-device and in the cloud while maintaining high visual quality.
...
TL;DR: A new approach for precise 3D brain segmentation in MRI scans, improving boundary accuracy by integrating landmark detection.


<details>
  <summary>Details</summary>
Motivation: Voxel-wise deep models often produce anatomically inconsistent results in brain segmentation, diverging from expert-defined boundaries.

Method: A three-step approach: 1) Global-to-Local network detects 16 landmarks, 2) semantic segmentation model produces coarse segmentation of 12 labels, 3) landmark-driven post-processing refines into 26 distinct structures.

Result: Consistent improvements in boundary accuracy, with segmentations more closely aligned with manual protocols.

Conclusion: The proposed landmark-guided approach enhances the accuracy of brain structure segmentation in MRI scans.
...
TL;DR: A new network called IFGNet is proposed for hyperspectral image classification, which fuses hyperspectral and LiDAR data using learnable spline-based functions and implicit aggregation in spatial and frequency domains.


<details>
  <summary>Details</summary>
Motivation: The classification of hyperspectral images in complex scenes is challenging due to spectral ambiguity, spatial heterogeneity, and the interaction between material properties and geometric structures. Existing fusion methods with LiDAR data have limitations in modeling nonlinear relationships and capturing structural patterns.

Method: The Implicit Frequency-Geometry Fusion Network (IFGNet) uses Kolmogorov-Arnold Networks (KANs) with learnable spline-based functions to model nonlinear relationships between hyperspectral and LiDAR features. It also introduces a LiDAR-guided implicit aggregation module in both spatial and frequency domains.

Result: IFGNet outperforms existing fusion methods on the Houston 2013 and MUUFL benchmarks in terms of overall accuracy, average accuracy, and Cohen's Kappa.

Conclusion: The proposed IFGNet effectively addresses the challenges of hyperspectral image classification in complex scenes by adaptively capturing nonlinear relationships and structural patterns, leading to improved classification performance.
...
TL;DR: The study evaluates the ability of conditional generative adversarial networks (cGANs) to generalize to out-of-distribution whole-slide images (WSI) across institutions without retraining. A preprocessing pipeline is developed to mitigate domain shift, and results show feasibility of applying cGAN-based computational H&E staining and destaining to external WSI datasets.


<details>
  <summary>Details</summary>
Motivation: To investigate the ability of cGANs to generalize to out-of-distribution WSI across institutions without retraining, and to develop a preprocessing pipeline to mitigate domain shift.

Method: A preprocessing pipeline consisting of histogram-based stain normalization and channel-wise intensity calibration was developed. cGAN models trained on 102 registered prostate core biopsy WSIs from Brigham and Women's Hospital were evaluated on 82 spatially unregistered WSIs acquired at Stanford University.

Result: The results show that virtual destaining achieved a Pearson correlation coefficient (PCC) of 0.854, structural similarity index measure (SSIM) of 0.699, and peak signal-to-noise ratio (PSNR) of 18.41 dB. H&E restaining from computationally destained outputs outperformed direct staining from ground-truth unstained inputs across all metrics.

Conclusion: The study supports the feasibility of applying cGAN-based computational H&E staining and destaining generative models to external WSI datasets using preprocessing-based adaptation alone, while defining specific morphological targets for future domain adaptation.
...
TL;DR: Developed a real-time catheter tip tracking pipeline under fluoroscopy for mechanical thrombectomy, achieving improved performance over existing methods.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of limited access to local treatment for stroke patients by developing an autonomous navigation system using reinforcement learning-based robotic systems.

Method: A multi-threaded pipeline was designed, incorporating deep learning segmentation models (U-Net, U-Net+Transformer, and SegFormer) and post-processing techniques.

Result: The two-class SegFormer achieved a mean absolute error of 4.44 mm, outperforming other models and state-of-the-art benchmarks.

Conclusion: The proposed pipeline provides a reliable and efficient foundation for RL-based autonomous MT navigation, maintaining stable performance under challenging imaging conditions.
...
TL;DR: This paper proposes a method to accelerate diffusion models for image restoration tasks by projecting data into lower-dimensional subspaces, achieving better inference speed and reconstruction fidelity.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion model approaches for image restoration tasks are computationally inefficient, operating in high-dimensional pixel space or requiring repeated encoder-decoder inference in latent space.

Method: The paper proposes dynamic resolution diffusion models that project data into lower-dimensional subspaces, and adapts two pixel-space methods (DPS and DAPS) into the proposed framework, yielding SubDPS and SubDAPS. An enhanced variant, SubDAPS++, is also introduced.

Result: The proposed methods outperform recent diffusion model-based approaches in most experimental scenarios across diverse image datasets and restoration tasks, achieving better inference speed and reconstruction fidelity.

Conclusion: The proposed methods effectively mitigate the computational inefficiency of existing diffusion model approaches for image restoration tasks, providing a more efficient and effective solution.
...
TL;DR: The paper proposes Omission Signal Intervention (OSI) to address concept omission in text-to-image generation models, specifically Multimodal Diffusion Transformers (MM-DiTs).


<details>
  <summary>Details</summary>
Motivation: The authors identified that MM-DiTs often suffer from concept omission, where specified objects or attributes are missing in generated images.

Method: The authors perform linear probing on text tokens to detect an 'omission signal' and propose OSI, which amplifies this signal to catalyze the generation of missing concepts.

Result: Comprehensive experiments on FLUX.1-Dev and SD3.5-Medium show that OSI significantly alleviates concept omission, even in extreme scenarios.

Conclusion: The proposed OSI method effectively addresses the issue of concept omission in MM-DiTs, leading to improved text-to-image generation results.
...
TL;DR: The paper proposes a new method called CreFlow for post-training video generation models using reinforcement learning, with a focus on embodied manipulation tasks. It introduces a compositional constraint-based reward model and achieves a 23.8% improvement in execution success across eight bimanual manipulation tasks.


<details>
  <summary>Details</summary>
Motivation: The motivation is to adapt video generation models (VGMs) to physical constraints in embodied manipulation tasks, as existing video-RL rewards do not align with task specifications that require logic-based verification.

Method: The method proposed is CreFlow, a novel online RL framework that includes a credit-aware NFT loss and a corrective reflow loss. It uses a compositional constraint-based reward model that formulates task requirements as Linear Temporal Logic constraints.

Result: The result shows that CreFlow yields better-aligned reward judgments with human and simulator success labels and improves downstream execution success by 23.8 percentage points across eight bimanual manipulation tasks.

Conclusion: The conclusion is that the proposed CreFlow method effectively improves high-dimensional video generation using the introduced reward signals, leading to significant performance gains in embodied manipulation tasks.
...
TL;DR: KVPO, a novel framework, improves alignment of streaming autoregressive video generators with human preferences by introducing a causal-semantic exploration paradigm and a velocity-field surrogate policy.


<details>
  <summary>Details</summary>
Motivation: Existing reinforcement learning methods for aligning streaming autoregressive video generators with human preferences have limitations, including reliance on noise-based exploration and surrogate policies that don't match the deterministic ODE dynamics of distilled AR models.

Method: KVPO, an ODE-native online Group Relative Policy Optimization framework, which includes a causal-semantic exploration paradigm and a velocity-field surrogate policy based on Trajectory Velocity Energy (TVE).

Result: Experiments demonstrate consistent gains in visual quality, motion quality, and text-video alignment across single-prompt short-video and multi-prompt long-video settings.

Conclusion: KVPO effectively addresses the challenges of aligning streaming autoregressive video generators with human preferences, leading to improved performance in various video generation tasks.
...
TL;DR: The paper proposes a concept-level unlearning framework for Vision-Language Models (VLMs) to precisely remove target knowledge without affecting unrelated semantics.


<details>
  <summary>Details</summary>
Motivation: Current machine unlearning methods in VLMs are limited as they operate at the image or instance level, making it challenging to remove target knowledge without affecting unrelated semantics.

Method: The proposed framework constructs a task-specific concept vocabulary using a multimodal large language model and decomposes visual representations into sparse, nonnegative combinations of semantic concepts.

Result: The method enables more comprehensive target forgetting, preserves non-target knowledge within the same image, and maintains competitive model utility compared to existing VLM unlearning methods.

Conclusion: The proposed concept-level unlearning framework provides a more precise and effective approach to removing target knowledge from VLMs while preserving unrelated semantics.
...
TL;DR: The paper proposes a new method for compressing key-value (KV) cache in large vision-language models (VLMs) for streaming video understanding, by viewing it as a coreset selection problem and introducing a bicriteria objective and an orthogonality-driven diversity criterion.


<details>
  <summary>Details</summary>
Motivation: The need for a compact memory to support future reasoning over an ever-growing visual history in streaming video understanding with large VLMs, and the limitations of existing streaming methods that rely on local token-wise heuristics.

Method: The proposed method operates in a joint KV representation, introducing a bicriteria objective that balances coverage in key and value spaces, and an orthogonality-driven diversity criterion that favors candidates contributing new directions beyond the current selection.

Result: The method improves over heuristic streaming compression baselines under a fixed cache budget across four open-source VLMs and five long-video and streaming-video benchmarks.

Conclusion: The paper concludes that representative coreset selection offers a more effective principle than token-wise pruning for memory-constrained streaming video understanding.
...
TL;DR: TurboVGGT is a novel approach for fast multi-view 3D reconstruction using an efficient visual geometry transformer with adaptive alternating attention.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle to balance reconstruction quality and computational efficiency, and lack mechanisms to adaptively learn representative tokens to capture global relationships.

Method: TurboVGGT employs an end-to-end trainable framework with adaptive sparse global attention and frame attention to aggregate local details within each frame.

Result: TurboVGGT achieves fast multi-view reconstruction while maintaining competitive reconstruction quality compared with state-of-the-art methods.

Conclusion: TurboVGGT offers a more efficient and scalable solution for multi-view 3D reconstruction tasks.
...
TL;DR: A novel remote sensing image generation framework, D2-CDIG, is proposed, which integrates diffusion models with a dual-prior control mechanism to generate high-quality images by precisely controlling ground features and atmospheric phenomena.


<details>
  <summary>Details</summary>
Motivation: Existing controllable remote sensing image generation methods rely on traditional techniques such as segmentation and edge detection, which do not fully leverage terrain or atmospheric conditions, resulting in generated images that lack accuracy and naturalness.

Method: D2-CDIG framework, which decouples terrain and atmospheric generation processes through independent control of ground and atmospheric branches, and introduces a refined cloud-fog slider to adjust cloud thickness and distribution.

Result: D2-CDIG shows significant improvements in image quality, detail richness, and realism compared to traditional methods, providing a flexible and precise solution for remote sensing image generation.

Conclusion: D2-CDIG offers a high-quality data foundation for training large remote sensing models and downstream tasks, addressing the limitations of existing methods in generating accurate and natural remote sensing images.
...
TL;DR: A new discrete visual tokenization framework called InsightTok is proposed, which improves text and face fidelity in autoregressive image generation.


<details>
  <summary>Details</summary>
Motivation: The current discrete tokenization methods struggle with generating clear text and realistic faces due to aggressive downsampling and quantization, which discard fine-grained structures necessary for preserving readable glyphs and distinctive facial features.

Method: InsightTok uses localized, content-aware perceptual losses to enhance text and face fidelity, with a compact 16k codebook and a 16x downsampling rate.

Result: InsightTok significantly outperforms prior tokenizers in text and face reconstruction without compromising general reconstruction quality, and produces images with clearer text and more faithful facial details in autoregressive image generation.

Conclusion: The proposed InsightTok framework highlights the potential of specialized supervision in tokenizer training for advancing discrete image generation, particularly in preserving text legibility and facial fidelity.
...
TL;DR: A novel approach to nighttime image restoration that addresses complex degradation, including low-light and weather conditions, using a dataset and illumination-guided diffusion model.


<details>
  <summary>Details</summary>
Motivation: The difficulty in perceiving surroundings at night and the limitations of current image restoration methods in handling complex nocturnal scenes with multiple forms of degradation.

Method: Proposing an integration of an illumination-guided module embedded in a diffusion model to guide the illumination restoration process.

Result: The model can preserve texture fidelity while handling various degradations in low-light scenarios.

Conclusion: The proposed approach effectively addresses the challenges of nighttime image restoration with complex degradation.
...
TL;DR: A novel diffusion framework called AnyBand-Diff is proposed for generating remote sensing imagery that adheres to intrinsic physical laws, addressing spectral distortion and radiometric inconsistency.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion models often disregard intrinsic physical laws when adapted to remote sensing imagery, leading to spectral distortion and radiometric inconsistency.

Method: AnyBand-Diff, a spectral-prior-guided diffusion framework, which includes a Masked Conditional Diffusion backbone, a dual stochastic masking strategy, a Physics-Guided Sampling mechanism, and a Multi-Scale Physical Loss.

Result: Extensive experiments confirm the effectiveness of AnyBand-Diff in generating reliable imagery and achieving accurate spectral reconstruction.

Conclusion: AnyBand-Diff contributes to the advancement of physics-aware generative methods for Earth observation by generating remote sensing imagery that adheres to intrinsic physical laws.
...
TL;DR: The paper proposes a hierarchical knowledge distillation framework for single-frame Infrared Small Target Detection (ISTD) with point supervision, leveraging a frozen Vision Foundation Model (VFM) to improve detection accuracy and training stability.


<details>
  <summary>Details</summary>
Motivation: The high cost of dense pixel-wise annotations for ISTD and the limitations of lightweight CNN detectors in capturing sufficient semantics, leading to noisy pseudo-masks and unstable optimization.

Method: A hierarchical VFM-driven knowledge distillation framework with bilevel optimization, Semantic-Conditioned Affine Modulation (SCAM), and dynamic collaborative learning with cluster-level sample reweighting.

Result: Consistent improvements in detection accuracy and training stability across multiple ISTD backbones and challenging cases.

Conclusion: The proposed framework effectively addresses the challenges of point-supervised ISTD, providing a more accurate and stable solution for detecting weak targets under heavy background clutter.
...
TL;DR: A new method called Delta Forcing is proposed to improve the consistency of interactive real-time autoregressive video generation while maintaining responsiveness to new events.


<details>
  <summary>Details</summary>
Motivation: The need for balancing reactivity and stability in autoregressive video generation, where models must respond promptly to new events while maintaining temporal coherence over long horizons.

Method: Delta Forcing, a framework that constrains unreliable teacher supervision within an adaptive trust region, estimating transition consistency from the latent delta between teacher and generator trajectories.

Result: Extensive experiments demonstrate that Delta Forcing significantly improves consistency while maintaining event reactivity.

Conclusion: Delta Forcing is a simple yet effective solution to the challenge of balancing reactivity and stability in autoregressive video generation.
...
TL;DR: The paper proposes a new framework called Mixture of Decoder Experts (MoDE) for learned image compression, which balances distortion fidelity and perceptual realism across a wide range of bitrates.


<details>
  <summary>Details</summary>
Motivation: Existing methods for learned image compression struggle to balance distortion fidelity and perceptual realism, particularly across different bitrates, due to the limitations of relying on a single compressed latent representation.

Method: The MoDE framework uses a dual-latent collaborative decoding approach, combining scalar-quantized (SQ) and vector-quantized (VQ) latent paradigms through two decoder-side modules: Expert-Specific Enhancement (ESE) and Cross-Expert Modulation (CEM).

Result: The proposed MoDE framework achieves a more favorable fidelity-perception balance than existing baselines across a wide bitrate range, demonstrating the effectiveness of decoder-side expert collaboration for learned image compression.

Conclusion: The MoDE framework offers a promising solution for learned image compression, enabling a better balance between distortion fidelity and perceptual realism across various bitrates.
...
TL;DR: The paper presents a method for analogical trajectory transfer between 3D environments, enabling machines to perform spatial reasoning and apply it to various fields like AR/VR and robotics.


<details>
  <summary>Details</summary>
Motivation: The goal is to translate motion trajectories from one 3D environment to a semantically similar location in another, which is useful for applications such as AR/VR co-presence, content creation, and robotics.

Method: The method involves partitioning scenes into object-centric clusters, estimating cross-scene mappings via hierarchical smooth map prediction using 3D foundation model features, and then refining the result to remove collisions and distortions.

Result: The proposed method outperforms baselines based on LLMs, VLMs, and scene graph matching, achieves a fast runtime of around 0.6 seconds, and demonstrates broad applicability to AR/VR and robotics through various applications.

Conclusion: The paper introduces a novel approach for analogical trajectory transfer that is efficient, effective, and widely applicable, paving the way for machines to perform analogical spatial reasoning.
...
TL;DR: MIRAGE is a framework that discovers semantic attacks on autonomous vehicle mapping systems, bypassing defenses and degrading predictions by introducing plausible environmental variations.


<details>
  <summary>Details</summary>
Motivation: Autonomous vehicles rely on online HD map construction for safety-critical road elements, but existing adversarial defenses can be neutralized by sophisticated attacks, highlighting a need for more robust protection.

Method: MIRAGE uses diffusion models to exploit the latent manifold of real-world data and searches for semantically mutated scenes that mislead mapping predictions while maintaining the same road topology.

Result: MIRAGE successfully degrades mapping predictions with two attacks: boundary removal (suppressing 57.7% of detections and corrupting 96% of planned trajectories) and boundary injection (successfully injecting fictitious boundaries).

Conclusion: The study reveals a categorical gap in current adversarial defenses, showing that semantic-level perturbations manifesting as legitimate environmental variation are substantially harder to mitigate than pixel-level perturbations.
...
TL;DR: SceneForge is a framework that generates structured supervision for multimodal learning tasks by applying interventions to editable 3D world states, producing consistent and aligned outputs.


<details>
  <summary>Details</summary>
Motivation: Multimodal learning tasks require consistent supervision across edits, viewpoints, and scene-level interventions, but such supervision is difficult to obtain from observation-level datasets.

Method: SceneForge represents each scene as a persistent world with semantic, geometric, and physical dependencies, and applies explicit interventions to propagate effects through scene dependencies.

Result: Incorporating SceneForge supervision improves object removal and scene removal performance across multiple benchmarks, indicating a practical and scalable foundation for intervention-consistent multimodal learning.

Conclusion: SceneForge provides a framework for generating structured supervision from editable 3D world states, leading to improved performance in multimodal learning tasks.
...
TL;DR: The paper proposes DermAgent, a multi-tool agent for dermatological diagnosis that integrates visual perception and clinical knowledge to improve accuracy and reduce hallucinations.


<details>
  <summary>Details</summary>
Motivation: Dermatological diagnosis requires fine-grained visual perception and expert clinical knowledge, but Multimodal Large Language Models (MLLMs) face challenges in dermatology due to insufficient domain-specific grounding and hallucinations.

Method: The authors propose DermAgent, a collaborative multi-tool agent that uses a Plan-Execute-Reflect framework with seven specialized vision and language modules, including visual perception tools, a dual-modality retrieval module, and a deterministic critic module.

Result: DermAgent outperforms state-of-the-art MLLMs and medical agent baselines across various tasks, achieving a 17.6% improvement in skin disease diagnostic accuracy and a 3.15% improvement in captioning ROUGE-L over GPT-4o.

Conclusion: The proposed DermAgent demonstrates the potential to improve dermatological diagnosis by integrating visual perception and clinical knowledge, reducing hallucinations, and providing stepwise, traceable diagnostic reasoning.
...
TL;DR: The paper proposes Think When Needed (TWN), a unified multimodal embedding framework with adaptive reasoning, achieving state-of-the-art embedding quality while being more efficient.


<details>
  <summary>Details</summary>
Motivation: Current multimodal large language models (MLLMs) with chain-of-thought (CoT) reasoning are costly and inefficient, and redundant reasoning can degrade performance.

Method: The authors propose TWN, a unified framework with a dual-LoRA architecture and an adaptive think mechanism that decides per input whether to generate CoT.

Result: TWN achieves state-of-the-art embedding quality on 78 tasks of MMEB-V2, requiring only 3-5% additional parameters and up to 50% fewer reasoning tokens.

Conclusion: The proposed TWN framework offers a more efficient and effective approach to multimodal embeddings with adaptive reasoning.
...
TL;DR: Introduced ClickRemoval, an interactive object removal tool driven by user clicks, leveraging pretrained Stable Diffusion models.


<details>
  <summary>Details</summary>
Motivation: Existing object removal tools require manual masks or text prompts, which can be challenging for non-expert users, especially in complex scenes.

Method: ClickRemoval uses user clicks to localize target objects and restore the background through self-attention modulation during denoising, without additional training or manual masks.

Result: ClickRemoval achieves competitive results across quantitative metrics and user studies.

Conclusion: The proposed ClickRemoval tool simplifies object removal by using user clicks, making it more accessible and effective for users.
...
TL;DR: The paper introduces HA-HOI, a framework for reconstructing physically plausible 4D human-object interaction (HOI) animation from monocular videos, improving alignment, contact consistency, and simulation readiness.


<details>
  <summary>Details</summary>
Motivation: Current HOI reconstruction methods often produce visual artifacts and fail to preserve stable contact, functional manipulation, or physical plausibility, highlighting a fundamental interaction gap.

Method: The proposed human-first, object-follow formulation recovers human motion as the interaction anchor and reconstructs the object relative to the human action, then projects the kinematic trajectory into a physics-based simulation.

Result: HA-HOI improves human-object alignment, contact consistency, temporal stability, and simulation readiness over prior monocular HOI reconstruction methods.

Conclusion: The work takes a step toward turning general monocular HOI videos into scalable demonstrations for humanoid-object behavior by moving beyond visually plausible trajectory recovery to physically grounded interaction animation.
...
TL;DR: A new framework called GeoVista is proposed for interpreting ultra-high-resolution remote sensing images by actively planning and perceiving visual evidence across large scenes.


<details>
  <summary>Details</summary>
Motivation: Existing remote sensing vision-language models have limitations in exploring large-scale scenes, such as losing global context, unvisited regions, and revisiting the same evidence. The motivation is to address these limitations.

Method: GeoVista uses a planning-driven active perception framework, building a global exploration plan, verifying multiple candidate regions through local inspection, and maintaining an explicit evidence state. It also introduces APEX-GRO, a supervised trajectory corpus, and an Observe-Plan-Track mechanism.

Result: GeoVista achieves state-of-the-art performance on RSHR-Bench, XLRS-Bench, and LRS-VQA.

Conclusion: GeoVista offers an effective solution for interpreting ultra-high-resolution remote sensing images by actively planning and perceiving visual evidence.
...
TL;DR: Proposes a GAN-based upsampling approach and a Separate Expert Fusion (SEF) framework to improve AI-generated image detection by capturing diverse forgery cues and reducing domain shift.


<details>
  <summary>Details</summary>
Motivation: The growing misuse of AI-generated images necessitates generalizable image detection techniques. Current methods have limitations in capturing diverse generative patterns.

Method: A GAN-based upsampling approach to generate diverse artifact patterns and a Separate Expert Fusion (SEF) framework to extract complementary information and reduce inter-domain interference.

Result: The proposed method yields strong results across 13 diverse benchmarks, demonstrating improved generalization across broader generative methods.

Conclusion: The proposed approach enables learning a more robust decision boundary and improves generalization across broader generative methods, addressing the limitations of current AI-generated image detection techniques.
...
TL;DR: A new framework called Head Forcing improves autoregressive video diffusion models by optimizing KV cache allocation for attention heads, leading to longer and more consistent video generation.


<details>
  <summary>Details</summary>
Motivation: Autoregressive video diffusion models have limitations in long-term video generation due to error accumulation and context loss, and existing methods treat attention heads uniformly, which is suboptimal.

Method: Head Forcing, a training-free framework that assigns tailored KV cache strategies to different types of attention heads (local, anchor, and memory heads).

Result: Head Forcing extends video generation from 5 seconds to minute-level duration, supports multi-prompt interactive synthesis, and outperforms existing baselines without additional training.

Conclusion: Head Forcing effectively improves the performance of autoregressive video diffusion models by optimizing attention head cache allocation, enabling longer and more consistent video generation.
...
TL;DR: The paper introduces a new framework for efficient sparse attention in diffusion-based video generation, achieving significant speedup without retraining or sacrificing video quality.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the limitations of existing sparse attention methods in diffusion-based video generation, specifically the high cost of mask prediction and shared thresholds across heads.

Method: The authors propose a head-wise adaptive framework with two plug-in components: Temporal Mask Reuse and Error-guided Budgeted Calibration, to improve the practical speed-quality trade-off of training-free sparse attention.

Result: The proposed method consistently improves XAttention and SVG2, achieving up to 1.93 times speedup at 720P while maintaining competitive video quality and similarity metrics on Wan2.1-1.3B and Wan2.1-14B.

Conclusion: The paper concludes that the proposed framework can efficiently accelerate pretrained models without retraining, achieving a better speed-quality trade-off in diffusion-based video generation.
...
TL;DR: The paper proposes a new activation function called Adaptive Arctangent Gated Activation (ArcGate) which outperforms standard baselines in remote sensing tasks, especially in noisy environments.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to propose a flexible activation function that can adapt to the specific requirements of the feature hierarchy and data distribution in deep networks, particularly in remote sensing tasks.

Method: The paper proposes a new activation function called Adaptive Arctangent Gated Activation (ArcGate) which uses seven learnable parameters per layer to generate a broad spectrum of activation shapes via a three-stage non-linear transformation.

Result: The experimental results show that ArcGate consistently outperforms standard baselines, achieving a peak overall accuracy of 99.67% on PatternNet and maintaining a 26.65% performance lead over ReLU under moderate Gaussian noise.

Conclusion: The findings suggest that ArcGate is a robust and adaptive general node activation function for high-resolution earth observation tasks, particularly in noisy environments.
...
TL;DR: A novel input method for 3D human pose estimation, called sparse interleaved input, is proposed, which leverages images from different camera views at various time points to capture rich spatio-temporal information and boost performance.


<details>
  <summary>Details</summary>
Motivation: Traditional multi-view 3D human pose estimation methods overlook temporal dependencies between adjacent frames, and the proposed method aims to address this limitation.

Method: The sparse interleaved input method, which uses images from different camera views at various time points, and the DenseWarper model, which leverages epipolar geometry for efficient spatio-temporal heatmap exchange.

Result: The proposed method outperforms traditional dense multi-view input approaches and achieves state-of-the-art performance on the Human3.6M and MPI-INF-3DHP datasets.

Conclusion: The sparse interleaved input method and the DenseWarper model effectively boost performance in 3D human pose estimation, offering advantages in terms of increased output pose frame rate and reduced data redundancy.
...
TL;DR: The paper identifies issues with large diffusion vision-language models (LDVLMs) in long-form generation, proposing a training-free approach to improve performance.


<details>
  <summary>Details</summary>
Motivation: Existing LDVLMs suffer from repetitive generation and degraded visual grounding in long-form generation tasks.

Method: The authors propose Mask Prior Suppression and Monotonic RoPE Scaling to mitigate mask prior drift and positional attention collapse during decoding.

Result: Experiments show improvements over baseline LDVLMs on multimodal benchmarks and visual grounding tasks, with robust gains on long-form description benchmarks.

Conclusion: The proposed approach effectively addresses the failures of LDVLMs in long-form generation with a lightweight, plug-and-play strategy that requires no additional training.
...
TL;DR: The paper proposes a new evaluation framework called PROVE for object removal in images and videos, introducing two perception-aligned metrics, RC-S and RC-T, and a benchmark dataset PROVE-Bench.


<details>
  <summary>Details</summary>
Motivation: Existing metrics for object removal in images and videos often disagree with human perception, and the task is inherently one-to-many, making it challenging to evaluate.

Method: The authors propose a pair of metrics, RC-S and RC-T, which measure spatial coherence and temporal consistency, respectively, and introduce a two-tier benchmark dataset PROVE-Bench.

Result: Experiments show that the proposed RC metrics achieve stronger alignment with human judgments than existing evaluation protocols.

Conclusion: The proposed PROVE evaluation framework, including RC metrics and PROVE-Bench, provides a more accurate and reliable way to evaluate object removal in images and videos.
...
TL;DR: Proposed a Local Spatiotemporal Convolutional Network (LSTCN) for gait recognition, a simple yet effective dual-branch architecture that extracts temporal information.


<details>
  <summary>Details</summary>
Motivation: Existing gait recognition approaches are limited by their reliance on static appearance features or complex sequential models, which demand substantial computational resources and sophisticated training strategies.

Method: Proposed a Local Spatiotemporal Convolutional Network (LSTCN) with a Global Bidirectional Spatial Pooling (GBSP) mechanism and Local Spatiotemporal Convolutional (LSTC) layer.

Result: The proposed LSTCN can adaptively learn strip-based gait motion patterns and extract temporal information effectively.

Conclusion: The proposed LSTCN offers a structurally simple yet highly effective solution for gait recognition, addressing the limitations of existing approaches.
...
TL;DR: A novel framework for high-fidelity natural image decomposition is proposed, addressing challenges in layered image generation and object interaction modeling.


<details>
  <summary>Details</summary>
Motivation: The success of generative models in graphic design domains is limited in real-world scenarios due to challenges in scalable layered data and object interaction modeling in natural images.

Method: An Agent-driven Data Decomposition (ADD) pipeline is introduced to synthesize layered data, and a novel framework is presented to improve photometric fidelity and alpha boundary accuracy.

Result: The proposed framework achieves state-of-the-art performance in natural image decomposition, outperforming existing models in RGB L1 and Alpha IoU metrics.

Conclusion: The proposed framework and dataset (LiWi-100k) provide a solution for high-fidelity natural image decomposition, enabling fine-grained editing and applications in real-world scenarios.
...
TL;DR: The authors propose a two-stage framework for medical image segmentation in low-data regimes, combining structure-aware encoder pretraining with boundary-oriented decoding, and demonstrate improved performance over state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Medical image segmentation is challenging in low-data regimes due to poor generalization, ambiguous boundaries, and missing fine structures, and recent self-supervised pretraining methods exhibit texture bias rather than being geometry-aware.

Method: A two-stage framework is proposed: Stage-1 uses Mixed-Domain MeanFlow Pretraining to learn structure-aware representations, and Stage-2 fine-tunes the encoder with a lightweight decoder that uses Direct Attentional Fusion and Frequency-Directional Dynamic Convolution.

Result: The proposed method demonstrates consistent gains over state-of-the-art methods on ISIC-2016, Kvasir-SEG, and GlaS datasets, with improved robustness in low-data settings and sharper boundary delineation.

Conclusion: The proposed framework effectively addresses the challenges of medical image segmentation in low-data regimes by leveraging structure-aware encoder pretraining and boundary-oriented decoding.
...
TL;DR: A novel framework called CineNeuron is proposed for reconstructing videos from fMRI signals by leveraging a dual-pathway processing mechanism inspired by the human brain.


<details>
  <summary>Details</summary>
Motivation: Current fMRI-to-video reconstruction methods are limited by a semantic gap between noisy fMRI signals and rich video content, due to incomplete semantic embeddings.

Method: CineNeuron, a hierarchical framework with two stages: 1) bottom-up semantic enrichment mapping fMRI signals to a rich embedding space, and 2) top-down memory integration using a Mixture-of-Memories method to refine video reconstruction.

Result: CineNeuron outperforms state-of-the-art methods on two fMRI-to-video benchmarks across various metrics.

Conclusion: The proposed CineNeuron framework effectively bridges the semantic gap in fMRI-to-video reconstruction, offering a more comprehensive understanding of neural processes.
...
TL;DR: Med DisSeg, a dispersion-driven framework, improves medical image segmentation by enhancing representation learning and anatomical delineation.


<details>
  <summary>Details</summary>
Motivation: Medical image segmentation is challenging due to heterogeneous appearances, ambiguous boundaries, and large anatomical variability.

Method: Med DisSeg combines Dispersive Loss with adaptive attention for fine-grained structure segmentation, improving representation learning and anatomical delineation.

Result: State-of-the-art performance on five datasets across three imaging modalities, with competitive results on multi-organ CT segmentation.

Conclusion: Med DisSeg offers a robust and effective solution for medical image segmentation, demonstrating its potential for precision medicine applications.
...
TL;DR: The paper examines if single-vector aggregation of visual patch tokens from document images loses key information, particularly in financial documents.


<details>
  <summary>Details</summary>
Motivation: The motivation is to investigate whether aggregating hundreds of patch tokens into a single vector for visual document retrieval in financial applications leads to loss of critical information.

Method: The authors develop a diagnostic benchmark using financial documents and conduct experiments to compare the performance of patch-level and single-vector aggregation methods.

Result: The results show that single-vector aggregation collapses different documents into almost identical vectors, obscuring semantic changes that are detectable at the patch level.

Conclusion: The study concludes that single-vector aggregation poses significant risks for visual document retrieval in financial applications due to loss of key information.
...
TL;DR: FedStain, a federated domain generalization framework, improves whole-slide image analysis across institutions with varying stain conditions while maintaining data privacy.


<details>
  <summary>Details</summary>
Motivation: Current stain analysis methods fail to capture higher-order, non-Gaussian characteristics of real-world stain variability, and federated learning approaches rely heavily on low-order statistics, limiting their effectiveness.

Method: FedStain incorporates higher-order stain moments (skewness and kurtosis) into a federated domain generalization framework, enabling robust cross-institutional deployment without compromising data privacy.

Result: FedStain outperforms state-of-the-art baselines by up to +3.9% absolute accuracy on Camelyon17 and MvMidog-Fed benchmarks.

Conclusion: FedStain is the first approach to explicitly model higher-order stain statistics, enabling robust and private whole-slide image analysis across institutions.
...
TL;DR: The paper presents TOPOS, a framework for generating 3D heads with a fixed, industry-standard topology from a single image, enabling consistent vertex-level correspondence and asset-level reuse.


<details>
  <summary>Details</summary>
Motivation: The film, animation, and video game industries require high-fidelity 3D head generation with a fixed reference topology for production-level rigging, skinning, and animation.

Method: The authors propose a novel framework called TOPOS, which consists of TOPOS-VAE, a variational autoencoder structure that converts input pointclouds into the target reference topology, and TOPOS-DiT, a rectified flow transformer that generates high-fidelity head meshes from a single image. Additionally, TOPOS-Texture produces relightable UV texture maps from the same portrait image.

Result: The proposed framework achieves state-of-the-art performance on 3D head generation, surpassing classical face reconstruction methods and general 3D object generative models.

Conclusion: The TOPOS framework enables the generation of high-fidelity 3D heads with a fixed, studio-style topology, facilitating consistent vertex-level correspondence and asset-level reuse, which is crucial for digital human creation in various industries.
...
TL;DR: A novel precipitation nowcasting framework, VMU-Diff, is proposed, combining deterministic and probabilistic models to improve accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing precipitation nowcasting methods face challenges due to the chaotic property of precipitation systems, and single-source data and model limitations lead to blurring or spurious artifacts.

Method: A two-stage framework, VMU-Diff, is proposed, using a deterministic model for coarse prediction and a probabilistic model for fine details, leveraging multi-source data and advanced neural network blocks.

Result: Experiments on Jiangsu SWAN datasets demonstrate improvements over state-of-the-art methods, particularly in short-term forecasts.

Conclusion: The proposed VMU-Diff framework effectively addresses challenges in precipitation nowcasting, offering a more accurate and efficient approach.
...
TL;DR: PanoGSDet, a monocular panoramic 3D detection framework, is proposed to address the challenge of accurately mapping 2D features to 3D in panoramic imagery.


<details>
  <summary>Details</summary>
Motivation: The challenge of accurately mapping 2D features to 3D in panoramic imagery for comprehensive scene understanding.

Method: PanoGSDet framework, which includes a panoramic depth estimation component and a semantic Gaussian component with a lifting module, optimization module, and prediction head.

Result: The proposed method significantly outperforms existing methods on the Structured3D dataset.

Conclusion: The proposed PanoGSDet framework effectively addresses the challenge of 3D object detection in panoramic imagery by utilizing continuous semantic 3D Gaussian representations.
...
### [61] [MambaRain: Multi-Scale Mamba-Attention Framework for 0-3 Hour Precipitation Nowcasting](https://arxiv.org/abs/2605.14606)
...
TL;DR: MambaRain, a novel multi-scale encoder-decoder architecture, is proposed for accurate precipitation nowcasting over extended horizons (0-3 hours) with substantial accuracy improvements.


<details>
  <summary>Details</summary>
Motivation: Existing deterministic approaches for precipitation nowcasting are limited to shorter prediction windows (0-2 hours) and exhibit severe performance degradation beyond 90 minutes.

Method: MambaRain integrates Mamba's linear-complexity long-range temporal modeling with self-attention mechanisms for explicit spatial correlation capture.

Result: MambaRain substantially outperforms existing deterministic methodologies in 0-3 hour nowcasting tasks, with particularly pronounced performance gains in the challenging 2-3 hour prediction range.

Conclusion: MambaRain effectively extends the viable forecasting horizon to 2-3 hours with substantial accuracy improvements, making it a promising approach for disaster mitigation and operational decision-making.
...
### [62] [ViMU: Benchmarking Video Metaphorical Understanding](https://arxiv.org/abs/2605.14607)
*Qi Li,Xinchao Wang*

Main category: cs.CV

TL;DR: The paper introduces ViMU, a benchmark to evaluate video understanding models' ability to comprehend subtext, implicit meanings, and social context in videos.


<details>
  <summary>Details</summary>
Motivation: Current video understanding models focus on literal visual comprehension, lacking the ability to understand metaphorical, ironic, and social meanings embedded in videos.

Method: The authors propose ViMU, a benchmark that assesses models' ability to infer implicit meaning from videos through multimodal evidence and answers to open-ended and multiple-choice questions.

Result: The paper does not report specific results, but rather introduces the ViMU benchmark as a tool to evaluate video understanding models.

Conclusion: The paper highlights the importance of understanding subtext in videos and introduces ViMU as a tool to bridge the gap in current video understanding models' capabilities.
...
### [63] [Deep Image Segmentation via Discriminant Feature Learning](https://arxiv.org/abs/2605.14609)
*Adam Dawid Sztamborski,Raül Pérez-Gonzalo,Antonio Agudo*

Main category: cs.CV

TL;DR: A new loss function called Deep Discriminant Analysis (DDA) is proposed to improve image segmentation accuracy by maximizing between-class variance and minimizing within-class variance.


<details>
  <summary>Details</summary>
Motivation: Current image segmentation models often rely on standard loss functions like Cross-Entropy and Dice, which neglect the discriminative structure of learned features, leading to inaccurate boundaries.

Method: The proposed method, Deep Discriminant Analysis (DDA), is a differentiable, architecture-agnostic loss function that embeds classical discriminant principles for network training.

Result: Evaluations on the DIS5K benchmark demonstrate that DDA consistently improves segmentation accuracy, boundary sharpness, and model confidence across various architectures.

Conclusion: Integrating discriminant analysis offers a simple, effective path for building more robust segmentation models.
...
### [64] [CalibAnyView: Beyond Single-View Camera Calibration in the Wild](https://arxiv.org/abs/2605.14615)
*Boying Li,Cheng Zhang,Weirong Chen,Daniel Cremers,Ian Reid,Hamid Rezatofighi*

Main category: cs.CV

TL;DR: The paper introduces CalibAnyView, a method for camera calibration using an arbitrary number of views, ensuring geometric consistency across them.


<details>
  <summary>Details</summary>
Motivation: Classical camera calibration methods require controlled setups, which are impractical for in-the-wild imagery. Recent learning-based methods neglect geometric consistency across multiple views.

Method: The authors propose a multi-view transformer that predicts dense perspective fields, integrated into a geometric optimization framework to estimate camera intrinsics and gravity direction.

Result: CalibAnyView outperforms state-of-the-art methods, shows strong robustness under single-view settings, and improves with multi-view inference.

Conclusion: CalibAnyView provides a reliable foundation for downstream tasks such as 3D reconstruction and robotic perception in the wild.
...
### [65] [Do We Really Need External Tools to Mitigate Hallucinations? SIRA: Shared-Prefix Internal Reconstruction of Attribution](https://arxiv.org/abs/2605.14621)
*Tian Qin,Junzhe Chen,Yuqing Shi,Tianshu Zhang,Qiang Ju,Lijie Wen*

Main category: cs.CV

TL;DR: The paper proposes SIRA, a training-free internal contrastive decoding framework to reduce hallucinations in large vision-language models (LVLMs) by constructing a counterfactual reference within the same LVLM.


<details>
  <summary>Details</summary>
Motivation: Existing contrastive decoding methods for mitigating hallucinations in LVLMs have limitations, such as introducing off-manifold artifacts and requiring costly extra forward passes.

Method: SIRA constructs a counterfactual reference inside the same LVLM by exploiting the staged information flow of multimodal transformers, which interacts image and text tokens through a shared prefix and then forks a counterfactual branch in later transformer layers.

Result: Experiments on POPE, CHAIR, and AMBER with Qwen2.5-VL and LLaVA-v1.5 show that SIRA consistently reduces hallucinations while preserving descriptive coverage and incurring lower overhead than two-pass contrastive decoding.

Conclusion: SIRA is a effective and efficient method for reducing hallucinations in LVLMs, requiring no training, external verifier, or perturbed input, and applies to open-weight LVLMs with white-box inference access.
...
### [66] [UniTriGen: Unified Triplet Generation of Aligned Visible-Infrared-Label for Few-Shot RGB-T Semantic Segmentation](https://arxiv.org/abs/2605.14626)
*Ping Zhou,Haoyu Wang,Mengmeng Zheng,Lei Zhang,Wei Wei,Chen Ding,Fei Zhou*

Main category: cs.CV

TL;DR: UniTriGen is a unified framework for generating aligned VIS-IR-Label triplets for RGB-T semantic segmentation, addressing the scarcity of such data.


<details>
  <summary>Details</summary>
Motivation: The scarcity of strictly aligned VIS-IR-Label triplets for RGB-T semantic segmentation in real-world scenarios.

Method: UniTriGen uses a unified triplet generation mechanism with a diffusion process, modality-specific residual adapters, and a scene-balanced few-shot sampling strategy.

Result: UniTriGen generates high-quality aligned triplets, achieving consistent performance improvements across various RGB-T semantic segmentation models.

Conclusion: UniTriGen effectively addresses the challenge of scarce aligned triplet data for RGB-T semantic segmentation.
...
### [67] [MultiEmo-Bench: Multi-label Visual Emotion Analysis for Multi-modal Large Language Models](https://arxiv.org/abs/2605.14635)
*Tianwei Chen,Takuya Furusawa,Yuki Hirakawa,Ryotaro Shimizu,Mo Fan,Takashi Wada*

Main category: cs.CV

TL;DR: A new multi-label visual emotion analysis benchmark dataset is introduced to evaluate multimodal large language models' (MLLMs) ability to predict emotions evoked by images, revealing limitations in current datasets and models.


<details>
  <summary>Details</summary>
Motivation: Current datasets for visual emotion analysis use a suboptimal annotation scheme, where each annotator is shown a single candidate emotion per image, leading to potential underestimation of MLLMs' capabilities.

Method: A new dataset is created by hiring 20 annotators per image to select all emotions they feel, aggregating votes across annotators, and providing a more reliable and representative dataset with a distribution of emotions.

Result: The benchmark dataset contains 10,344 images with 236,998 valid votes across eight emotions, and evaluations of recent MLLMs show progress but also substantial room for improvement.

Conclusion: The introduced benchmark dataset provides a more reliable evaluation of MLLMs for visual emotion analysis, highlighting limitations in current models and datasets, and indicating that LLM-as-a-judge method does not consistently improve MLLMs' performance.
...
### [68] [How to Evaluate and Refine your CAM](https://arxiv.org/abs/2605.14641)
*Luca Domeniconi,Alessandra Stramiglio,Michele Lombardi,Samuele Salti*

Main category: cs.CV

TL;DR: The paper proposes a new dataset for evaluating Class Attribution Maps (CAMs) and introduces RefineCAM, a method for producing high-resolution attribution maps, as well as a new composite metric ARCC for evaluating CAMs.


<details>
  <summary>Details</summary>
Motivation: The lack of ground-truth explanations for evaluating CAMs and the limitation of existing CAM methods in producing low-resolution attribution maps.

Method: Introducing a synthetic dataset with ground-truth attributions, proposing a new composite metric ARCC, and a new method RefineCAM for producing high-resolution attribution maps.

Result: RefineCAM consistently outperforms existing methods according to the proposed evaluation, and ARCC more reliably identifies faithful explanations.

Conclusion: The paper addresses the challenges in evaluating CAMs and provides a more reliable method for producing high-resolution attribution maps, facilitating detailed interpretability.
...
### [69] [Vision-Based Water Level and Flow Estimation](https://arxiv.org/abs/2605.14645)
*ZhiXin Sun*

Main category: cs.CV

TL;DR: This paper proposes a framework combining vision models with statistical modeling for more accurate water level and river surface velocity estimation.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenges of environmental sensitivity, limited precision, and complex site calibration in vision-based methodologies for water level and river surface velocity estimation.

Method: The method involves synergizing state-of-the-art vision models with statistical modeling, leveraging physical priors and robust filtering strategies.

Result: The proposed framework aims to improve the accuracy of water level detection and flow estimation.

Conclusion: The paper concludes that the proposed integrated framework can potentially enhance the accuracy and robustness of vision-based water level and river surface velocity estimation.
...
### [70] [TERRA-CD: Multi-Temporal Framework for Multi-class and Semantic Change Detection](https://arxiv.org/abs/2605.14651)
*Omkar Oak,Rukmini Nazre,Rujuta Budke,Suraj Sawant*

Main category: cs.CV

TL;DR: A new dataset, TERRA-CD, is introduced for urban vegetation monitoring, providing Sentinel-2 image pairs from 2019 and 2024 across 232 cities in the USA and Europe, along with various annotation schemes and deep learning approaches for change detection.


<details>
  <summary>Details</summary>
Motivation: The lack of comprehensive datasets for urban vegetation monitoring led to the creation of TERRA-CD, aiming to facilitate environmental change understanding.

Method: The study utilized deep learning approaches including Siamese networks, STANet variants, Bi-SRNet, Changemask, Post-Classification Comparison, and HRSCD strategies to evaluate the effectiveness of the TERRA-CD dataset for vegetation change detection.

Result: The TERRA-CD dataset, comprising 5,221 Sentinel-2 image pairs with various annotation schemes, was successfully evaluated using multiple deep learning approaches for both vegetation multi-class change detection and semantic change detection.

Conclusion: The TERRA-CD dataset and proposed methods provide a valuable resource for urban vegetation monitoring and change detection, addressing the existing gap in comprehensive datasets.
...
### [71] [Beyond Instance-Level Self-Supervision in 3D Multi-Modal Medical Imaging](https://arxiv.org/abs/2605.14654)
...
TL;DR: The paper proposes a self-supervised pre-training method for medical imaging that leverages cross-instance topological consistency to improve representation learning.


<details>
  <summary>Details</summary>
Motivation: Current self-supervised pre-training methods in medical imaging do not adequately utilize the consistent spatial relationships of anatomical structures across individuals.

Method: The proposed method uses two alignment regimes: intra-instance with pixel-level correspondences and inter-instance with pseudo-correspondences to preserve local neighborhood topology and prevent topology collapse.

Result: The approach achieves average improvements of 1.1% and 5.94% in segmentation and classification tasks, respectively, and demonstrates better robustness when modalities are missing at test time.

Conclusion: The proposed method effectively leverages cross-instance topological consistency to improve representation learning in medical imaging, leading to improved performance in downstream tasks.
...
### [72] [MiVE: Multiscale Vision-language features for reference-guided video Editing](https://arxiv.org/abs/2605.14664)
*Tong Wang,Meng Zou,Chengjing Wu,Xiaochao Qu,Luoqi Liu,Xiaolin Hu,Ting Liu*

Main category: cs.CV

TL;DR: The paper introduces MiVE, a framework for reference-guided video editing that leverages multiscale vision-language features to improve editing accuracy and preserve original motion.


<details>
  <summary>Details</summary>
Motivation: Existing methods for reference-guided video editing have limitations, including modality gaps and loss of fine-grained spatial details. The authors aim to address these limitations by developing a more effective approach.

Method: The authors propose MiVE, which repurposes vision-language models (VLMs) as multiscale feature extractors. MiVE extracts hierarchical features from Qwen3-VL and integrates them into a unified self-attention Diffusion Transformer.

Result: Experiments demonstrate that MiVE achieves state-of-the-art performance, ranking highest in human preference and outperforming both academic methods and commercial systems.

Conclusion: The paper concludes that MiVE provides a more effective approach for reference-guided video editing, addressing the limitations of existing methods and achieving superior performance.
...
### [73] [Are Candidate Models Really Needed for Active Learning?](https://arxiv.org/abs/2605.14689)
*Harshini Mridula Mohan,Maanya Manjunath,Vipul Arya,S. H. Shabbeer Basha,Nitin Cheekatla*

Main category: cs.CV

TL;DR: The study proposes an efficient active learning approach using CNNs and transformers with randomly initialized weights, eliminating the need for initial candidate models. It evaluates three confidence-based sampling strategies and shows that the low confidence (LC) strategy performs best, achieving comparable results to traditional active learning frameworks.


<details>
  <summary>Details</summary>
Motivation: The reliance on extensive labeled data poses significant challenges, including resource constraints and annotation errors. Active learning offers a promising solution to reduce labeling burdens, but current frameworks are time-intensive and require initial candidate models.

Method: The study uses CNNs and transformers with randomly initialized weights and evaluates three confidence-based sampling strategies: high confidence (HC), low confidence (LC), and a combination of high confidence in the early stages of training and low confidence at later stages of training (HCLC).

Result: The low confidence (LC) strategy demonstrates the best performance, showcasing its effectiveness as an active learning strategy without the need for candidate models. The proposed approach achieves comparable results to active learning frameworks that depend on initial candidate models.

Conclusion: The proposed work introduces a streamlined approach to active learning, advancing efficiency and flexibility across diverse datasets and domains by eliminating the need for initial candidate models and achieving comparable results to traditional frameworks.
...
TL;DR: The study challenges the assumption that Composed Image Retrieval (CIR) requires multimodal composition, finding that many queries can be solved using a single modality, and highlighting issues with current benchmarks.


<details>
  <summary>Details</summary>
Motivation: The authors aim to investigate the assumption that strong performance on CIR benchmarks requires multimodal composition and to understand the role of unimodal shortcuts in CIR.

Method: The authors conducted a two-stage audit: first, identifying shortcut-solvable queries through cross-model analysis; second, performing human validation on 4,741 shortcut-free queries.

Result: The study found that a large fraction of queries can be solved using a single modality (32.2% to 83.6%), and that current CIR benchmarks conflate shortcut-solvable, noisy, and genuinely compositional queries.

Conclusion: The authors conclude that high CIR performance can arise from unimodal signals rather than true multimodal composition, and that current benchmarks overestimate model capability in multimodal composition.
...
### [75] [Generating HDR Video from SDR Video](https://arxiv.org/abs/2605.14703)
...
TL;DR: Proposed a framework for converting standard dynamic range (SDR) videos to high dynamic range (HDR) videos using large-scale generative video models.


<details>
  <summary>Details</summary>
Motivation: The problem of upconverting legacy SDR videos to HDR persists without a convincing solution, despite the HDR video ecosystem approaching maturity.

Method: Introduced a Multi-Exposure Video Model (MEVM) to predict exposure-bracketed linear SDR video sequences from a single nonlinear SDR video input, and a learnable Video Merging Model (VMM) to merge the predicted sequences into a high-quality HDR sequence.

Result: Extensive experiments, quantitative and qualitative evaluation, and a user study demonstrated robust HDR conversion for various examples, including casual consumer videos and iconic films.

Conclusion: The proposed approach enables robust HDR conversion from SDR videos and can support HDR synthesis pipelines built upon existing SDR generative video models.
...
### [76] [SceneFunRI: Reasoning the Invisible for Task-Driven Functional Object Localization](https://arxiv.org/abs/2605.14704)
*Posheng Chen,Powen Cheng,Gueter Josmy Faure,Hung-Ting Su,Winston H. Hsu*

Main category: cs.CV

TL;DR: The paper introduces SceneFunRI, a benchmark for reasoning about invisible objects in real-world scenes, and evaluates vision-language models' ability to infer their locations.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of vision-language models (VLMs) in inferring the locations of occluded objects, which humans can often do using context and commonsense knowledge.

Method: The method involves creating a benchmark (SceneFunRI) based on the SceneFun3D dataset, formulating the task as a 2D spatial reasoning problem, and evaluating models using various prompting analysis categories.

Result: The strongest baseline model (Gemini 3 Flash) achieves poor results: CAcc@75 of 15.20, mIoU of 0.74, and Dist of 28.65, indicating that invisible-region reasoning is an unstable capability in current VLMs.

Conclusion: The paper concludes that there is a need for future work on models that integrate task intent, commonsense priors, spatial grounding, and uncertainty-aware search to improve invisible-region reasoning in VLMs.
...
### [77] [Towards Continuous Sign Language Conversation from Isolated Signs](https://arxiv.org/abs/2605.14705)
*Youngmin Kim,Kyobin Choo,Jiwoo Park,Minseo Kim,Chanyoung Kim,Junhyeok Kim,Seong Jae Hwang*

Main category: cs.CV

TL;DR: The paper proposes a method to create a sign-to-sign conversational AI system that can generate 3D sign language responses without relying on spoken or written language, addressing the limitations of current systems for Deaf and Hard-of-Hearing individuals.


<details>
  <summary>Details</summary>
Motivation: The primary motivation is to improve access to conversational AI for Deaf and Hard-of-Hearing individuals who rely on sign language as their primary means of communication, by developing a direct sign-to-sign conversational model.

Method: The method involves constructing continuous sign conversations from isolated signs by collecting large-scale labeled isolated clips, recomposing them into sign-language-ordered utterances, and using a retrieval-guided spoken-to-gloss translator and a diffusion Transformer (BRAID) to bridge structural and temporal gaps.

Result: The result is a direct sign-to-sign conversational model called SignaVox, which generates 3D body, hand, and facial motion responses from prior signing context without relying on spoken-language text or externally provided glosses, demonstrating improved motion quality, semantic alignment, and scalable signer-centered interaction.

Conclusion: The paper concludes that the proposed approach can effectively create a sign-to-sign conversational AI system that supports visual-spatial articulation and improves accessibility for Deaf and Hard-of-Hearing individuals.
...
### [78] [StyleTextGen: Style-Conditioned Multilingual Scene Text Generation](https://arxiv.org/abs/2605.14708)
*Zeyu Chen,Fangmin Zhao,Yan Shu,Yichao Liu,Liu Yu,Yu Zhou*

Main category: cs.CV

TL;DR: A novel framework called StyleTextGen is proposed for style-conditioned scene text generation, particularly for multilingual scripts. It features a dual-branch style encoder, a text style consistency loss, and a mask-guided inference strategy.


<details>
  <summary>Details</summary>
Motivation: Style-conditioned scene text generation faces challenges in extracting precise text styles from complex backgrounds and maintaining fine-grained style consistency across characters, especially for multilingual scripts.

Method: The proposed framework, StyleTextGen, includes a dual-branch style encoder for robust multilingual text style representations, a text style consistency loss for enhanced style coherence, and a mask-guided inference strategy for precise style alignment.

Result: StyleTextGen significantly outperforms existing methods in style consistency and cross-lingual generalization, establishing new state-of-the-art performance in multilingual style-conditioned text generation.

Conclusion: The proposed StyleTextGen framework effectively addresses the challenges of style-conditioned scene text generation for multilingual scripts, demonstrating superior performance in style consistency and cross-lingual generalization.
...
### [79] [Breaking Dual Bottlenecks: Evolving Unified Multimodal Models into Self-Adaptive Interleaved Visual Reasoners](https://arxiv.org/abs/2605.14709)
...
TL;DR: The paper proposes a novel framework that enables unified models to switch between generation strategies based on instruction complexity and model capability, addressing the understanding-generation gap in anything-to-image tasks.


<details>
  <summary>Details</summary>
Motivation: The 'understanding-generation gap' in unified models, which can capture user intent but struggle to translate it into precise pixel-level manipulation, resulting in two bottlenecks: attention entanglement and visual refinement.

Method: A hierarchical data pipeline with three adaptive modes (direct generation, self-reflection, and multi-step planning) and a two-stage training strategy (SFT and RL) with step-wise reasoning rewards and intra-group complexity penalty.

Result: The proposed method outperforms existing baselines on X2I, achieving superior generation fidelity among simple-to-complex instructions.

Conclusion: The paper presents a novel framework that effectively addresses the understanding-generation gap in unified models, demonstrating improved performance on anything-to-image tasks.
...
### [80] [Vision-Core Guided Contrastive Learning for Balanced Multi-modal Prognosis Prediction of Stroke](https://arxiv.org/abs/2605.14710)
*Liren Chen,Lidong Sun,Mingyan Huang,Junzhe Tang,Yinghui Zhu,Guanjie Wang,Yiqing Xia,Ting Xiao*

Main category: cs.CV

TL;DR: A novel tri-modal fusion model is proposed for ischemic stroke prognosis, integrating medical images, structured clinical data, and unstructured text.


<details>
  <summary>Details</summary>
Motivation: Existing multi-modal approaches for ischemic stroke prognosis are limited, mainly confined to dual-modal fusion and lacking deep bidirectional interactions between modalities.

Method: A tri-modal fusion model using a Large Language Model (LLM) to generate semi-structured diagnostic text from brain MRIs and a Vision-Conditioned Dual Alignment Fusion Module (VDAFM) for dynamic and profound fusion.

Result: The proposed model achieves state-of-the-art performance on a real-world clinical dataset.

Conclusion: The proposed model effectively addresses the limitations of existing multi-modal approaches and has the potential to improve ischemic stroke prognosis.
...
### [81] [On the Cultural Anachronism and Temporal Reasoning in Vision Language Models](https://arxiv.org/abs/2605.15071)
*Mukul Ranjan,Prince Jha,Khushboo Kumari,Zhiqiang Shen*

Main category: cs.CV

TL;DR: The paper identifies a phenomenon called cultural anachronism in Vision-Language Models (VLMs) when interpreting historical artifacts, and introduces a benchmark to evaluate temporal reasoning in VLMs.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the issue of VLMs misinterpreting historical objects using temporally inappropriate concepts, materials, or cultural frameworks, which is a significant limitation in visual AI systems.

Method: The authors introduce the Temporal Anachronism Benchmark for Vision-Language Models (TAB-VLM), a dataset of 600 questions across six categories, to evaluate temporal reasoning on 1,600 Indian cultural artifacts.

Result: The evaluation of ten state-of-the-art models reveals significant deficiencies, with even the best model (GPT-5.2) achieving only 58.7% overall accuracy, and a persistent performance gap across varying architectures and scales.

Conclusion: The paper highlights the disparity between current VLM capabilities and the requirements for accurately interpreting cultural heritage materials, and provides a foundation for enhancing temporal cognition in multimodal AI systems that interact with historical artifacts.
...
TL;DR: A unified Deep Learning framework for label-free single-cell imaging to classify White Blood Cells and predict protein expression from Differential Phase Contrast images.


<details>
  <summary>Details</summary>
Motivation: Inferring molecular phenotypes from bright-field morphology remains challenging, and there's a need for a scalable, non-invasive alternative to fluorescence-based cytometry.

Method: A Hybrid architecture fusing convolutional and transformer-based features through a learnable cross-branch gating module, combined with a Large Language Model for generating biologically grounded summaries.

Result: Achieved 91.3% WBC classification accuracy and 0.72 Pearson correlation for CD16 expression regression on the Berkeley Single Cell Computational Microscopy benchmark.

Conclusion: The proposed framework shows promise for cost-effective hematological profiling, enabling simultaneous phenotype identification and quantitative biomarker estimation without fluorescent staining.
...
### [83] [MemEye: A Visual-Centric Evaluation Framework for Multimodal Agent Memory](https://arxiv.org/abs/2605.15128)
...
TL;DR: The paper introduces MemEye, a framework for evaluating the memory capabilities of multimodal agents, focusing on preserving fine-grained visual evidence and reasoning about changing visual states.


<details>
  <summary>Details</summary>
Motivation: Existing evaluations of multimodal agents rarely test their ability to preserve visual evidence for later reasoning, and prior work often allows answers to be inferred without fine-grained visual evidence.

Method: The authors introduce MemEye, a framework that evaluates memory capabilities across two dimensions: granularity of visual evidence and usage of retrieved evidence, and construct a new benchmark with 8 life-scenario tasks.

Result: The evaluation of 13 memory methods across 4 VLM backbones shows that current architectures struggle to preserve fine-grained visual details and reason about state changes over time.

Conclusion: The findings highlight the importance of evidence routing, temporal tracking, and detail extraction for long-term multimodal memory.
...
TL;DR: ATLAS is a framework for visual reasoning that combines the strengths of agentic and latent methods while mitigating their limitations, achieving superior performance and interpretability.


<details>
  <summary>Details</summary>
Motivation: The authors aim to overcome the limitations of existing visual reasoning methods, which are either computationally expensive, lack task generalization, or are difficult to train.

Method: The authors propose ATLAS, a framework that uses functional tokens as both agentic operations and latent visual reasoning units, and introduce Latent-Anchored GRPO (LA-GRPO) to stabilize training.

Result: ATLAS achieves superior performance on challenging benchmarks while maintaining clear interpretability.

Conclusion: The authors propose ATLAS as a new paradigm for visual reasoning research, offering a promising direction for future studies.
...
### [85] [Video-Zero: Self-Evolution Video Understanding](https://arxiv.org/abs/2605.14733)
*Ruixu Zhang,Deyi Ji,Lanyun Zhu,Xuanyi Liu,Yuxin Meng,Ruihang Chu,Yujiu Yang*

Main category: cs.CV

TL;DR: A new framework called Video-Zero is proposed for improving video understanding models through self-evolution without human annotation, focusing on temporally localized evidence.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of extending self-evolution to video understanding, where evidence needed for reasoning is often sparse and temporally localized.

Method: Video-Zero, an annotation-free Questioner--Solver co-evolution framework, is proposed. It centers self-evolution on temporally localized evidence, where the Questioner generates evidence-grounded questions and the Solver learns to answer and align predictions with supporting evidence.

Result: Video-Zero consistently improves multiple video VLM backbones across 13 benchmarks, demonstrating effectiveness and transferability of evidence-centered self-evolution.

Conclusion: The proposed Video-Zero framework effectively improves video understanding models through self-evolution, highlighting the importance of grounding in temporally localized evidence for video reasoning.
...
### [86] [EARL: Towards a Unified Analysis-Guided Reinforcement Learning Framework for Egocentric Interaction Reasoning and Pixel Grounding](https://arxiv.org/abs/2605.14742)
*Yuejiao Su,Xinshen Zhang,Zhen Ye,Lei Yao,Lap-Pui Chau,Yi Wang*

Main category: cs.CV

TL;DR: The paper introduces EARL, a framework that improves human-environment interaction understanding in egocentric vision for assistive robotics and intelligent agents.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal large language models struggle with accurate interaction reasoning and fine-grained pixel grounding in egocentric vision.

Method: EARL uses a two-stage parsing framework, including coarse-grained interpretation and fine-grained response, with a novel Analysis-guided Feature Synthesizer (AFS) for query-oriented reasoning.
...
Conclusion: The proposed EARL framework effectively enhances the understanding of human-environment interactions from egocentric vision, with improved performance in interaction reasoning and pixel grounding.
...
### [87] [BioHuman: Learning Biomechanical Human Representations from Video](https://arxiv.org/abs/2605.14772)
*Yujun Huo,He Zhang,Chentao Song,Honglin Song,Zongyu Zuo,Tao Yu*

Main category: cs.CV

TL;DR: A simulation-based framework and end-to-end model are proposed to estimate muscle activations from monocular video, bridging visual observations and internal biomechanical states.


<details>
  <summary>Details</summary>
Motivation: The lack of large-scale datasets with biomechanical annotations and existing approaches' inability to infer internal biomechanical states from visual observations limit progress in motion analysis, rehabilitation, and injury risk assessment.

Method: A simulation-based framework is introduced to estimate muscle activations from existing motion capture datasets, and an end-to-end model, BioHuman, is proposed to take monocular video as input and jointly predict human motion and muscle activations.

Result: The proposed approach enables accurate reconstruction of both kinematic motion and muscle activity and generalizes across diverse subjects and motions.

Conclusion: The approach establishes a new benchmark for video-based biomechanical understanding and opens up new possibilities for physically grounded human modeling.
...
### [88] [MonoPRIO: Adaptive Prior Conditioning for Unified Monocular 3D Object Detection](https://arxiv.org/abs/2605.14781)
*Leon Davies,Qinggang Meng,Mohamad Saada,Baihua Li,Simon Sølvsten*

Main category: cs.CV

TL;DR: The paper proposes MonoPRIO, a unified monocular 3D detector that improves metric size stability in multi-class settings through adaptive prior conditioning.


<details>
  <summary>Details</summary>
Motivation: Monocular 3D object detection is challenging due to underdetermination of metric size and depth from single-view evidence, especially under occlusion and truncation.

Method: MonoPRIO uses class-aware size prototypes, soft mixture prior, uncertainty-aware log-space conditioning, and Cluster-Aligned Prior (CAP) regularisation.

Result: MonoPRIO achieves state-of-the-art results on the KITTI test server, with the strongest fully reported unified multi-class result and the strongest 3D bounding-box AP in the car-only setting.

Conclusion: The paper demonstrates the effectiveness of adaptive priors in improving metric size stability, particularly in ambiguity-prone and partially occluded scenes.
...
TL;DR: The paper proposes COAL, a framework that advances Referring Multi-Object Tracking (RMOT) by addressing the contradiction between high discriminability demand and sparse semantic supervision.


<details>
  <summary>Details</summary>
Motivation: The mismatch between high-discriminability demand and sparse semantic supervision in RMOT, particularly in highly homogeneous scenarios.

Method: The COAL framework, which includes Explicit Semantic Injection (ESI) via a VLM, Counterfactual Learning (CFL) leveraging LLM reasoning, and Hierarchical Multi-Stream Integration (HMSI) architecture.

Result: COAL surpasses the state-of-the-art by 7.28% HOTA on the Refer-KITTI-V2 benchmark, demonstrating its effectiveness in resolving the sparsity-discriminability paradox in RMOT.

Conclusion: The proposed COAL framework effectively addresses the challenges in RMOT by integrating knowledge regularization, leading to improved performance on benchmarks.
...
TL;DR: The study evaluates Vision Mamba models for detecting AI-generated images, comparing them to CNNs, ViTs, and VLM-based detectors across diverse datasets and metrics.


<details>
  <summary>Details</summary>
Motivation: The rapid progress in image generation has raised concerns about misuse, making it crucial to develop effective methods for detecting AI-generated images.

Method: The study benchmarks multiple Vision Mamba variants against representative CNNs, ViTs, and VLM-based detectors across diverse datasets and synthetic image sources.

Result: The findings highlight both the promise and current limitations of Vision Mamba in detecting AI-generated images, particularly in terms of accuracy, efficiency, and generalizability.

Conclusion: The research emphasizes the potential of Vision Mamba models as a component in systems designed to distinguish authentic from AI-generated visual content, but also notes their current limitations.
...
TL;DR: Proposed a training-free, class-agnostic anomaly detection pipeline for industrial inspection, improving robustness under distribution shifts.


<details>
  <summary>Details</summary>
Motivation: Visual anomaly detection in industrial settings faces challenges due to changes in acquisition conditions; the goal is to develop a model robust to such changes.

Method: Modified SuperAD pipeline with DINOv3 backbone, overlapping patch-wise processing, intensity-based augmentations, improved memory-bank subsampling, and iterative morphological closing.

Result: Achieved segmentation F1 scores of 62.61%, 57.42%, and 54.35% on MVTec AD 2 test sets, outperforming SuperAD and other state-of-the-art methods.

Conclusion: The proposed method offers a robust, class-agnostic solution for industrial anomaly detection, suitable for deployment with minimal adaptation effort.
...
TL;DR: The paper presents a method for camera control in video generation models by treating it as geometric guidance through displacement fields, achieving effective control with minimal degradation.


<details>
  <summary>Details</summary>
Motivation: Existing approaches to camera control in video generation models are limited by the scale, diversity, and scene dynamics of datasets, biasing models toward narrow output distributions.

Method: The paper reformulates camera control into a set of displacement fields applied via differentiable resampling of latent features during denoising, a simple approach applicable to most video diffusion models without training.

Result: The approach achieves effective camera control with minimal degradation across diverse quality metrics compared to fine-tuned baselines and reveals universal biases and disparities in camera control capabilities of representative video models.

Conclusion: The method offers a probe to study camera control capabilities of base models, identifying biases and disparities, and provides insights into their potential for 3D/4D tasks like multi-view generation.
...
TL;DR: The paper identifies issues with Flow Matching in high dimensions, proposes solutions to improve velocity estimation and generation efficiency, and demonstrates significant improvements in image generation tasks.


<details>
  <summary>Details</summary>
Motivation: The authors identified a critical breakdown in Flow Matching when applied in high-dimensional practice, specifically the Velocity Deficit and Integration Lag, which hinder the generation of samples that reach the data manifold.

Method: The authors propose two methods to address the issues: Magnitude-Aware Flow Matching (MAFM), a training-based approach, and Scale Schedule Corrector (SSC), a training-free approach. Both methods aim to correct the velocity estimation and improve generation efficiency.

Result: The proposed methods, especially SSC, yield significant improvements: on ImageNet-1k (256x256), SSC improves FID by 44.6% (from 13.68 to 7.58) and achieves a 5x speedup. It also improves FID on MS-COCO by ~22% for Text-to-Image tasks and high-resolution generation.

Conclusion: The paper concludes that the proposed methods effectively address the identified issues with Flow Matching in high-dimensional spaces, leading to improved efficiency and sample quality in image generation tasks.
...
TL;DR: Proposed HDRFace, a face restoration framework that uses high-dimensional representations and a fusion mechanism to improve face restoration under complex degradations.


<details>
  <summary>Details</summary>
Motivation: Face restoration under complex degradations is an ill-posed inverse problem due to severe information loss, and existing methods struggle to recover identity-critical details.

Method: The proposed HDRFace framework uses a pretrained high-dimensional feature encoder to extract facial representations from low-quality inputs and intermediate results, and injects them as conditions for generation. It also introduces a Structure-Detail aware adaptive Fusion Mechanism (SDFM) to balance structural consistency and detail fidelity.

Result: The proposed framework was implemented on two generative models and showed stable and coherent performance gains across different architectures.

Conclusion: The proposed HDRFace framework effectively improves face restoration under complex degradations by injecting semantically rich priors and balancing structural consistency and detail fidelity.
...
TL;DR: A novel weakly-supervised method called MCMT is proposed for Video Moment Retrieval (VMR) to identify moments semantically similar to a query within untrimmed videos.


<details>
  <summary>Details</summary>
Motivation: Previous VMR methods have limitations such as low-quality temporal proposals, struggling with distinguishing misaligned moments, or lacking stability due to reliance on a single auxiliary task.

Method: The proposed MCMT method generates multiple proposals, creates a high-quality positive sample mask, and introduces forward and inverse masked query reconstruction tasks for robust and stable retrieval performance.

Result: Extensive experiments on two standard benchmarks affirm the effectiveness of the proposed MCMT method in VMR.

Conclusion: The proposed MCMT method addresses the limitations of previous VMR methods and demonstrates effectiveness in identifying moments semantically similar to a query within untrimmed videos.
...
TL;DR: The paper introduces a new benchmark for abstract image editing, AbstractEdit, and a framework, Entity-Rubrics, to assess instruction-following in this domain. It evaluates 11 models, finding they struggle with balancing intent and preservation.


<details>
  <summary>Details</summary>
Motivation: Current image editing benchmarks focus on explicit commands, neglecting abstract instructions like 'change the mood'. The authors aim to bridge the gap between rigid machine execution and natural human communication.

Method: The authors formalize abstract image editing, introduce Entity-Rubrics for assessing edits, and create AbstractEdit, a benchmark for abstract image editing. They evaluate 11 models using this benchmark.

Result: The evaluation shows that standard architectures struggle with abstract edits, often under-editing or over-editing. Advanced LLM text encoders and iterative thinking are crucial for meaningful improvements.

Conclusion: The work serves as a stepping stone toward seamless multimodal interaction, enabling models to correctly interpret abstract communication and close the gap between machine execution and human communication.
...
TL;DR: The paper introduces MechVerse, a benchmark for mechanically consistent image-to-video generation, and evaluates current models' ability to generate motion governed by kinematic and geometric constraints.


<details>
  <summary>Details</summary>
Motivation: Current text- and image-conditioned video generation models often fail to generate motion that adheres to kinematic and geometric constraints, particularly in articulated mechanical assemblies.

Method: The authors introduce MechVerse, a benchmark consisting of 21,156 synthetic clips from 1,357 mechanical assemblies, and evaluate proprietary, open-source, and fine-tuned image-to-video models using standard video metrics, instruction-following scores, and human judgments.

Result: Current models can preserve appearance and smoothness but often fail to generate mechanically admissible motion, with errors increasing as coupling complexity grows.

Conclusion: MechVerse provides a benchmark for measuring and improving mechanism-aware video generation from image and language inputs, highlighting the need for further research in this area.
...
TL;DR: This paper explores the use of Vision-Language Models (VLMs) for biometric tasks, specifically signature verification, and finds that while they perform well in certain scenarios, they struggle with skilled forgery detection and can fall into a 'Rationalization Trap'.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this work is to investigate the applicability of state-of-the-art VLMs to rigorous biometric tasks, specifically signature verification, and to explore their zero-shot performance on the Signature Verification Challenge (SVC) benchmark.

Method: The method involves converting raw kinematic time-series into static images, encoding pressure information into stroke opacity, and introducing a scoring protocol that extracts latent token probabilities to compute robust biometric scores. The study uses GPT-5.2 and Gemini 2.5 Pro as the VLMs.

Result: The results show a significant performance dichotomy dependent on signal quality and forgery type. In random forgery scenarios, the zero-shot VLM achieves exceptional discrimination, with GPT-5.2 reaching an Equal Error Rate of 0.32% in mobile tasks. However, in skilled forgery scenarios, the results are significantly worse, and a critical 'Rationalization Trap' emerges.

Conclusion: The conclusion is that while VLMs demonstrate strong capabilities in general visual reasoning, their performance on biometric tasks like signature verification is highly dependent on the scenario, and they can struggle with skilled forgery detection. The 'Rationalization Trap' highlights the need for careful evaluation and improvement of these models for real-world applications.
...
TL;DR: The paper proposes a new approach to evaluating image super-resolution methods by focusing on the perceptual impact of visual artifacts, introducing the concept of 'artifact prominence' and a dataset suite called SR-Prominence.


<details>
  <summary>Details</summary>
Motivation: Current image super-resolution methods often produce visually appealing results but may introduce unnatural patterns and texture distortions that degrade perceived quality, and existing detection methods treat all defects equally, ignoring their varying perceptual impact.

Method: The authors design a crowdsourced annotation protocol, construct a dataset suite (SR-Prominence) with 3,935 artifact masks annotated with prominence, and audit SR artifact detectors, image-quality metrics, and SR methods.

Result: The study finds that classical full-reference metrics (especially SSIM and DISTS) provide strong localized prominence signals, while no-reference IQA methods and specialized artifact detectors often fail to generalize across datasets and reference settings.

Conclusion: The proposed approach and dataset suite enable SR artifact evaluation to shift from binary defect presence to perceptual impact, allowing for more accurate assessment of image super-resolution methods.
...
TL;DR: The paper proposes FactorizedHMR, a two-stage framework for Human Mesh Recovery (HMR) that handles ambiguity in 3D body reconstruction by separately recovering a stable torso-root anchor and then completing the remaining articulations.


<details>
  <summary>Details</summary>
Motivation: The authors are motivated by the inherent ambiguity in HMR, particularly under occlusion or weak depth cues, where multiple 3D bodies can explain the same image evidence.

Method: The proposed method, FactorizedHMR, consists of a deterministic regression module for recovering a stable torso-root anchor, followed by a probabilistic flow-matching module for completing the remaining non-torso articulations.

Result: The results show that FactorizedHMR is competitive with strong baselines, with notable gains in occlusion-heavy recovery and drift-sensitive world-space metrics.

Conclusion: The authors conclude that FactorizedHMR effectively addresses the ambiguity in HMR by treating different body regions differently, leading to improved performance in challenging scenarios.
...
TL;DR: The paper proposes LPH-VTON, a new framework for virtual try-on that balances structural integrity and textural fidelity in synthesizing photorealistic images of garments on a person's body.


<details>
  <summary>Details</summary>
Motivation: Current diffusion-based methods for virtual try-on face a trade-off between structural integrity and textural fidelity, which the authors aim to resolve.

Method: The authors propose a synergistic framework called LPH-VTON, which uses a structure-biased model to establish a geometrically consistent latent scaffold and then hands over control to a texture-biased model for high-fidelity detail rendering.

Result: The proposed model achieves a superior Pareto-optimal balance between structural alignment and perceptual faithfulness, establishing new benchmarks on the VITON-HD dataset.

Conclusion: The paper concludes that the proposed LPH-VTON framework effectively resolves the tension between structural integrity and textural fidelity in virtual try-on, demonstrating the efficacy of temporal architectural decoupling.
...
TL;DR: The paper proposes a new framework called Closed-Loop Visual Reasoning (CLVR) to improve text-to-image models by coupling visual-language planning with pixel-level diffusion generation.


<details>
  <summary>Details</summary>
Motivation: Current text-to-image models struggle with complex semantics and face diminishing returns from parameter scaling, and recent multi-step reasoning approaches have limitations such as ungrounded planning hallucinations and long-context optimization instabilities.

Method: The CLVR framework introduces an automated data engine with step-level visual verification, Proxy Prompt Reinforcement Learning (PPRL) to resolve long-context optimization instabilities, and Δ-Space Weight Merge (DSWM) to mitigate the latency bottleneck.

Result: Extensive experiments demonstrate that CLVR outperforms existing open-source baselines across multiple benchmarks and approaches the performance of proprietary commercial models.

Conclusion: The CLVR framework unlocks general test-time scaling capabilities for complex visual generation and overcomes the bottlenecks of current text-to-image models.
...
TL;DR: A novel compression method called HeatKV is proposed to address the KV-cache memory constraints in Visual Autoregressive (VAR) models, achieving a 2x higher compression ratio while maintaining image fidelity.


<details>
  <summary>Details</summary>
Motivation: VAR models suffer from severe KV-cache memory constraints, requiring gigabytes of memory per generated image, which HeatKV aims to alleviate.

Method: HeatKV uses a small offline calibration set to rank attention heads based on their attention scores over prior scales, and constructs a static pruning schedule tailored to a given memory budget.

Result: HeatKV achieves a 2x higher compression ratio in memory allocation for KV cache compared to existing methods, while maintaining similar or better image fidelity, prompt alignment, and human perception score.

Conclusion: HeatKV is effective in fine-grained, head-specific cache allocation, achieving a new state-of-the-art (SOTA) for VAR model KV-cache compression.
...
TL;DR: The paper introduces Denoising-GS, a framework that improves 3D Gaussian Splatting by denoising Gaussian primitives, considering both their positions and spatial structure, leading to state-of-the-art performance in Novel View Synthesis.


<details>
  <summary>Details</summary>
Motivation: The optimization process in 3D Gaussian Splatting introduces noisy Gaussian primitives due to sparse and incomplete initialization from Structure-from-Motion point clouds, and existing methods neglect the underlying spatial structure.

Method: The proposed Denoising-GS framework includes a spatial-aware denoising optimizer, Spatial Gradient-based Denoising strategy, Uncertainty-based Denoising module, and Spatial Coherence Refinement strategy.

Result: Experiments on three benchmark datasets show that Denoising-GS consistently enhances Novel View Synthesis fidelity while maintaining representation compactness, achieving state-of-the-art performance.

Conclusion: The paper concludes that Denoising-GS effectively improves the optimization of 3D Gaussian Splatting by considering the spatial structure of Gaussian primitives, leading to enhanced fidelity in Novel View Synthesis.
...
TL;DR: A new self-supervised framework, Masked Next-Scale Prediction (MNSP), is proposed for scene text recognition, which models cross-scale structural evolution and achieves state-of-the-art performance on several benchmarks.


<details>
  <summary>Details</summary>
Motivation: Scene text recognition requires modeling visual structures that evolve from coarse layouts to fine-grained character strokes, but existing methods rely on large amounts of annotated data or operate at a single spatial scale, failing to capture the hierarchical nature of scene text.

Method: The proposed MNSP framework incorporates Next-Scale Prediction (NSP) and masked image reconstruction to jointly learn cross-scale prediction and local constraints, guiding attention toward informative text regions.

Result: MNSP achieves state-of-the-art performance, reaching 86.2% average accuracy on the Union14M benchmark and 96.7% across six standard datasets, and improves robustness under extreme scale and layout variations.

Conclusion: The proposed MNSP framework effectively models cross-scale structural evolution and achieves superior performance in scene text recognition, demonstrating its potential for real-world applications.
...
### [106] [Hierarchical Image Tokenization for Multi-Scale Image Super Resolution](https://arxiv.org/abs/2605.14891)
*Isma Hadji,Enrique Sanchez,Adrian Bulat,Brais Martinez,Georgios Tzimiropoulos*

Main category: cs.CV

TL;DR: A novel multi-scale Image Super Resolution (ISR) method using Visual Auto-Regressive (VAR) modeling with two new components: Hierarchical Image Tokenization (HIT) and Direct Preference Optimization (DPO) regularization.


<details>
  <summary>Details</summary>
Motivation: Previous VAR-based ISR methods have limitations, including fixed scale generation and reliance on large models or annotated data.

Method: The proposed method introduces HIT for progressive image representation and DPO regularization to encourage high-resolution output.

Result: The proposed model achieves state-of-the-art results with a small size (300M params) and delivers multi-scale outputs with a single forward pass.

Conclusion: The proposed method improves flexibility and reduces complexity in VAR-based ISR, achieving better performance with less resources.
...
### [107] [Your CLIP has 164 dimensions of noise: Exploring the embeddings covariance eigenspectrum of contrastively pretrained vision-language transformers](https://arxiv.org/abs/2605.14893)
*Jakub Grzywaczewski,Dawid Płudowski,Przemysław Biecek*

Main category: cs.CV

TL;DR: The authors analyze the latent space of Vision-Language Models, finding that a significant portion is dominated by shared noise rather than semantic signals, and propose a method to remove this noise without harming performance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to understand and address the presence of structural anomalies and non-semantic noise in the shared latent spaces of Vision-Language Models.

Method: The authors use spectral decomposition of covariance matrices to decompose the VLM latent space into semantic signal and noise components.

Result: The results show that pruning the shared noise dimensions preserves or improves downstream task performance, and that a substantial fraction of the latent geometry is governed by shared noise.

Conclusion: The authors conclude that their work provides new insights into the representational structure of modern VLMs, highlighting the dominance of shared noise in their latent spaces.
...
### [108] [SEDiT: Mask-Free Video Subtitle Erasure via One-step Diffusion Transformer](https://arxiv.org/abs/2605.14894)
*Zheng Hui,Yunlong Bai*

Main category: cs.CV

TL;DR: SEDiT, a one-stage video subtitle erasure approach via diffusion transformer, enables direct and efficient removal of targeted subtitles from videos without requiring a mask.


<details>
  <summary>Details</summary>
Motivation: Existing video editing methods rely on inpainting video frames based on masked input, which requires extracting the target video mask in advance and can be affected by segmentation precision.

Method: SEDiT uses a mask-free inference approach, one-stage framework, and rectified flow for one-step generation. It also adopts a hybrid training strategy for temporal consistency and feeds the original video directly into the model.

Result: SEDiT can efficiently handle native 1440p video with infinite length and provides reliable one-step denoising with theoretical justification.

Conclusion: SEDiT offers a novel, efficient, and effective approach for video subtitle erasure, mitigating the limitations of existing methods and enabling high-quality, mask-free editing.
...
### [109] [MemLens: Benchmarking Multimodal Long-Term Memory in Large Vision-Language Models](https://arxiv.org/abs/2605.14906)
...
TL;DR: The paper introduces MEMLENS, a benchmark for evaluating memory in multimodal multi-session conversations. It compares long-context LVLMs and memory-augmented agents, finding that both have limitations and suggesting hybrid architectures.


<details>
  <summary>Details</summary>
Motivation: The need for a systematic comparison of long-context LVLMs and memory-augmented agents on questions requiring multimodal evidence.

Method: The authors introduce MEMLENS, a comprehensive benchmark with 789 questions across five memory abilities and four context lengths, and evaluate 27 LVLMs and 7 memory-augmented agents.

Result: Long-context LVLMs perform well on short-context tasks but degrade as conversations grow, while memory agents are length-stable but lose visual fidelity. Neither approach alone solves multi-session reasoning tasks.

Conclusion: The results motivate the development of hybrid architectures that combine long-context attention with structured multimodal retrieval.
...
### [110] [SteerSeg: Attention Steering for Reasoning Video Segmentation](https://arxiv.org/abs/2605.14908)
*Ali Cheraghian,Hamidreza Dastmalchi,Abdelwahed Khamis,Morteza Saberi,Aijun An,Lars Petersson*

Main category: cs.CV

TL;DR: SteerSeg is a framework that improves the spatial grounding capability of large vision-language models for video reasoning segmentation by steering attention through input-level conditioning.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the issue of attention misalignment in attention-based grounding, which results in diffuse and ambiguous grounding signals.

Method: SteerSeg combines learnable soft prompts with reasoning-guided Chain-of-Thought (CoT) prompting to reshape attention distribution and resolve ambiguity among similar objects.

Result: SteerSeg significantly improves the spatial grounding capability of LVLMs, generalizing well across diverse benchmarks, and preserves the model's pretrained reasoning capabilities.

Conclusion: SteerSeg provides a lightweight framework for improving video reasoning segmentation by steering attention at its source, achieving better spatial grounding without requiring model fine-tuning.
...
### [111] [Representative Attention For Vision Transformers](https://arxiv.org/abs/2605.14913)
*Yuntong Li,Hainuo Wang,Hengxing Liu,Mingjia Li,Xiaojie Guo*

Main category: cs.CV

TL;DR: A new linear attention mechanism called Representative Attention (RPAttention) is proposed for Vision Transformers, which dynamically forms a compact set of learned representative tokens to enable semantically related regions to communicate regardless of their spatial distance.


<details>
  <summary>Details</summary>
Motivation: Existing methods for linear attention in Vision Transformers compress spatial tokens into a compact set of intermediate proxies based on predefined spatial layouts, which can be limited by not being aligned with the semantic organization of visual content.

Method: RPAttention performs token compression directly in representation space, using a lightweight Gather-Interact-Distribute paradigm, where spatial tokens are gathered into representative tokens through competitive similarity-based routing, and then interact within a compact latent space before being broadcasted back to all spatial tokens.

Result: RPAttention reduces the dominant token interaction complexity from quadratic to linear scaling with respect to the number of spatial tokens, while maintaining expressive global context modeling, and demonstrates effectiveness across diverse vision transformer backbones on image classification, object detection, and semantic segmentation.

Conclusion: RPAttention provides a more effective and efficient way to scale Vision Transformers by aligning token communication with the content structure of each input, and preserving global receptive fields.
...
### [112] [SceneParser: Hierarchical Scene Parsing for Visual Semantics Understanding](https://arxiv.org/abs/2605.14923)
*Pengxin Xu,Xincheng Lin,Luping Xiao,Qing Jiang,Meishan Zhang,Hao Fei,Shanghang Zhang,Xingyu Chen*

Main category: cs.CV

TL;DR: The paper introduces Hierarchical Scene Parsing, a task that represents physical scenes as hierarchical structures of scenes, objects, parts, and affordances, and proposes SceneParser, a VLM-based parser that achieves stronger performance on this task.


<details>
  <summary>Details</summary>
Motivation: Current scene perception capabilities are limited to isolated predictions and do not capture structured dependencies needed for interaction-oriented scene understanding.

Method: The authors propose SceneParser, a VLM-based parser trained with structural-completion pseudo labels and curriculum learning, and construct SceneParser-Bench, a large-scale benchmark for training and evaluation.

Result: Experiments show that SceneParser achieves stronger structure-aware performance compared to existing MLLMs and perception-stitching pipelines, and is compatible with conventional tasks.

Conclusion: The paper concludes that Hierarchical Scene Parsing and SceneParser provide an actionable representation for visual understanding and can be used for downstream planning and other applications.
...
### [113] [Road Maps as Free Geometric Priors: Weather-Invariant Drone Geo-Localization with GeoFuse](https://arxiv.org/abs/2605.14925)
*Yunsong Fang,Tingyu Wang,Zhedong Zheng*

Main category: cs.CV

TL;DR: The paper introduces GeoFuse, a framework that integrates road map data with satellite imagery to improve drone-view geo-localization under adverse weather conditions.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the challenges of drone-view geo-localization under adverse weather conditions, such as rain, snow, and fog, which exacerbate the cross-view domain gap.

Method: The authors propose a cross-modal fusion framework called GeoFuse, which combines satellite imagery with precisely aligned road map tiles to yield more discriminative and weather-resilient representations.

Result: GeoFuse outperforms state-of-the-art methods, achieving +3.46% and +23.18% Recall@1 accuracy on the University-1652 and DenseUAV benchmarks, respectively.

Conclusion: The proposed GeoFuse framework effectively leverages road map data to improve drone-view geo-localization under diverse weather conditions, demonstrating its potential for real-world applications.
...
### [114] [SCRWKV: Ultra-Compact Structure-Calibrated Vision-RWKV for Topological Crack Segmentation](https://arxiv.org/abs/2605.14926)
*Hanxu Zhang,Chen Jia,Hui Liu,Xu Cheng,Fan Shi,Shengyong Chen*

Main category: cs.CV

TL;DR: A novel network, SCRWKV, is proposed for pixel-level accurate segmentation of structural cracks, achieving high precision and efficiency with a small number of parameters.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle to balance crack topology modeling with computational efficiency, failing to achieve high segmentation quality with low resource demands.

Method: The SCRWKV network uses a Structure-Field Encoder (SFE) backbone with an Adaptive Multi-scale Cascaded Modulator (AMCM) and a Structure-Calibrated Insight Unit (SCIU) to enhance texture representation and capture topological correlations.

Result: SCRWKV outperforms SOTA methods on multiple benchmarks, achieving an F1 score of 0.8428 and mIoU of 0.8512 on the TUT dataset with only 1.22M parameters.

Conclusion: The proposed SCRWKV network demonstrates robust potential for efficient real-world deployment, achieving high-precision modeling of structural cracks while maintaining linear complexity.
...
### [115] [Multi-scale Coarse-to-fine Modeling for Test-time Human Motion Control](https://arxiv.org/abs/2605.14935)
*Nhat Le,Daochang Liu,Anh Nguyen,Ajmal Mian*

Main category: cs.CV

TL;DR: MSCoT is a multi-scale model for human motion synthesis and control that generates high-quality motions consistent with control constraints, achieving state-of-the-art performance with faster sampling.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome the limitations of existing approaches that rely on multiple iterative denoising/token-prediction steps or modules tailored for specific control signals, and to achieve fast and flexible control.

Method: MSCoT discretizes motion into a multi-scale hierarchical representation and predicts the entire token sequence at each temporal scale in a coarse-to-fine fashion, using an efficient multi-scale token guidance strategy and a lightweight token refiner.

Result: MSCoT achieves state-of-the-art controllable text-to-motion generation performance with 48% FID improvement, -61% avg error, and 10x faster inference speed on HumanML3D.

Conclusion: MSCoT offers a fast and flexible approach for human motion synthesis and control, producing high-quality motions consistent with control constraints, and outperforming existing baselines.
...
### [116] [ACE-LoRA: Adaptive Orthogonal Decoupling for Continual Image Editing](https://arxiv.org/abs/2605.14948)
*Yuehao Liu,Weijia Zhang,Xuanming Shang,Zhizhou Chen,Yanhao Ge,Shanyan Guan,Chao Ma*

Main category: cs.CV

TL;DR: Proposed ACE-LoRA, a framework for continual image editing that mitigates catastrophic forgetting and outperforms existing methods.


<details>
  <summary>Details</summary>
Motivation: Real-world applications require continual adaptation to new image editing tasks while preserving previously learned knowledge, but this area is largely underexplored.

Method: ACE-LoRA uses Adaptive Orthogonal Decoupling and Rank-Invariant Historical Information Compression to address task interference and scalability issues.

Result: ACE-LoRA consistently outperforms baselines in instruction fidelity, visual realism, and robustness to forgetting.

Conclusion: ACE-LoRA establishes a strong foundation for continual learning in image editing, with CIE-Bench providing a standardized evaluation protocol.
...
### [117] [A CUBS-Compatible Ultrasound Morphology and Uncertainty-Aware Baseline for Carotid Intima-Media Segmentation and Preliminary Risk Prediction](https://arxiv.org/abs/2605.14949)
*Aueaphum Aueawatthanaphisut*

Main category: cs.CV

TL;DR: A deep learning model, AtheroFlow-XNet, was developed for carotid intima-media segmentation and risk prediction using ultrasound morphology.


<details>
  <summary>Details</summary>
Motivation: Conventional ultrasound assessment for carotid atherosclerosis may not fully capture patient-specific vascular risk, and there is a need for a more comprehensive approach.

Method: AtheroFlow-XNet, a CUBS-compatible ultrasound morphology and uncertainty-aware learning baseline, was proposed for carotid intima-media segmentation and preliminary risk prediction.

Result: The model achieved a Dice coefficient of 0.7930 for LI-MA mask segmentation, a segmentation loss of 0.2359, and an area under the receiver operating characteristic curve of 0.6910 for preliminary risk prediction.

Conclusion: The study suggests that ultrasound-derived carotid morphology can support automated wall analysis and uncertainty-aware interpretation, and future work will focus on incorporating additional biomarkers for improved risk prediction.
...
### [118] [Evo-Depth: A Lightweight Depth-Enhanced Vision-Language-Action Model](https://arxiv.org/abs/2605.14950)
...
TL;DR: Evo-Depth, a lightweight depth-enhanced VLA framework, is proposed for robotic manipulation. It enhances spatially grounded manipulation without extra sensing hardware, achieving superior performance with a small model size and high efficiency.


<details>
  <summary>Details</summary>
Motivation: Current Vision-Language-Action models struggle with precise spatial understanding due to reliance on 2D visual representations. Incorporating 3D inputs or implicit 3D-aware modeling are existing solutions but they have limitations such as increased complexity, additional sensors requirement, or high training costs.

Method: Evo-Depth employs a lightweight Implicit Depth Encoding Module to extract depth features from multi-view RGB images. These features are incorporated into vision-language representations through a Spatial Enhancement Module. A Progressive Alignment Training strategy is used to align depth-enhanced representations with action learning.

Result: Evo-Depth achieves superior performance across four simulation benchmarks with only 0.9B parameters. In real-world experiments, it attains the highest average success rate with the smallest model size, lowest GPU memory usage, and highest inference frequency.

Conclusion: Evo-Depth offers an efficient and effective solution for enhancing spatially grounded manipulation in robotic tasks, overcoming the limitations of current VLA models by providing a lightweight and depth-enhanced framework.
...
### [119] [H-OmniStereo: Zero-Shot Omnidirectional Stereo Matching with Heading-Aligned Normal Priors](https://arxiv.org/abs/2605.14963)
*Chenxing Jiang,Zhe Tong,Pusen Gao,Peize Liu,Yang Xu,Chuan Fang,Ping Tan,Shaojie Shen*

Main category: cs.CV

TL;DR: Proposed H-OmniStereo, a zero-shot omnidirectional stereo matching framework, which uses a large synthetic dataset and an equirectangular monocular normal estimator to improve accuracy and generalizability.


<details>
  <summary>Details</summary>
Motivation: The performance of stereo matching on omnidirectional images is limited by the scarcity of datasets and the degradation of perspective monocular priors under spherical distortions.

Method: Constructing a large synthetic dataset and introducing an equirectangular monocular normal estimator operating in a heading-aligned coordinate system.

Result: The approach achieves higher accuracy than existing methods on out-of-domain datasets and generalizes to real-world consumer camera setups using a single model.

Conclusion: H-OmniStereo framework provides an effective solution for omnidirectional stereo matching, with a large synthetic dataset and a novel monocular normal estimator, achieving high accuracy and generalizability.
...
### [120] [MHSA: A Lightweight Framework for Mitigating Hallucinations via Steered Attention in LVLMs](https://arxiv.org/abs/2605.14966)
*Wei Ding,Yilin Li,Yudong Zhang,Ruobing Xie,Xingwu Sun,Jiansheng Chen,Yu Wang*

Main category: cs.CV

TL;DR: A new framework, MHSA, is proposed to mitigate hallucinations in large vision-language models by correcting cross-modal attention patterns.


<details>
  <summary>Details</summary>
Motivation: Large vision-language models suffer from hallucinations, generating content inconsistent with visual input, and existing methods only detect but do not mitigate hallucinations.

Method: MHSA uses a three-layer MLP generator to produce corrected attention, guided by supervisory signals from a discriminator and the model itself, and replaces original attention with corrected one during inference.

Result: MHSA mitigates both discriminative and generative hallucinations across various datasets and models without modifying any model parameters.

Conclusion: MHSA offers a novel perspective on hallucination research and helps enhance the reliability of large vision-language models.
...
### [121] [MicroscopyMatching: Towards a Ready-to-use Framework for Microscopy Image Analysis in Diverse Conditions](https://arxiv.org/abs/2605.14980)
*Xiaofei Hui,Haoxuan Qu,Hossein Rahmani,Shuohong Wang,Jeff W. Lichtman,Jun Liu*

Main category: cs.CV

TL;DR: A new microscopy image analysis framework, MicroscopyMatching, is proposed to reliably perform key analysis tasks across diverse settings.


<details>
  <summary>Details</summary>
Motivation: Manual analysis of microscopy images is costly and time-consuming, while deep learning-based approaches require extensive adaptation for different settings, creating a need for a reliable and broadly applicable tool.

Method: MicroscopyMatching reformulates diverse microscopy image analysis tasks as a unified matching problem, leveraging pre-trained latent diffusion models.

Result: The proposed framework can reliably perform key analysis tasks, including segmentation, tracking, and counting, across diverse microscopy analysis settings.

Conclusion: MicroscopyMatching addresses the long-standing need for a reliable and broadly applicable microscopy image analysis tool, facilitating progress in biomedical research.
...
### [122] [Compositional Video Generation via Inference-Time Guidance](https://arxiv.org/abs/2605.14988)
*Ariel Shaulov,Eitan Shaar,Amit Edenzon,Gal Chechik,Lior Wolf*

Main category: cs.CV

TL;DR: A method called CVG is proposed to improve compositional faithfulness in text-to-video models without retraining or modifying the model.


<details>
  <summary>Details</summary>
Motivation: Current text-to-video diffusion models struggle with fine-grained compositional understanding, such as entity relations, attributes, actions, and motion directions.

Method: CVG uses cross-attention maps to steer the denoising process in frozen text-to-video models, leveraging a lightweight compositional classifier trained on attention features.

Result: CVG improves compositional generation in text-to-video models without requiring model modifications, fine-tuning, or user-supplied controls, while preserving visual quality.

Conclusion: The proposed CVG method effectively enhances compositional faithfulness in text-to-video models, offering a practical solution for improving model performance.
...
### [123] [Characterizing the visual representation of objects from the child's view](https://arxiv.org/abs/2605.14990)
*Jane Yang,Tarun Sepuri,Alvin Wei Ming Tan,Khai Loong Aw,Michael C. Frank,Bria Long*

Main category: cs.CV

TL;DR: The study analyzes young children's visual experiences to understand how they acquire object category representations, finding that exposure is highly skewed and variable, yet detected categories show strong groupings within superordinate categories.


<details>
  <summary>Details</summary>
Motivation: To understand the inputs to the learning process of object category representations in young children, specifically what their visual experiences look like.

Method: Analyzing first-person videos of young children's visual experience at home using a supervised object detection model to extract common object categories.

Result: Children's object category exposure is highly skewed, with a few categories dominating their visual experiences, and category exemplars are highly variable, yet detected categories show stronger groupings within superordinate categories.

Conclusion: Understanding visual category learning requires developing models that can exploit strong superordinate structure and learn from non-canonical, sparse, and variable exemplars.
...
### [124] [Predicting Response to Neoadjuvant Chemotherapy in Ovarian Cancer from CT Baseline Using Multi-Loss Deep Learning](https://arxiv.org/abs/2605.14991)
...
TL;DR: A deep learning framework is proposed to predict ovarian cancer patients' response to neoadjuvant chemotherapy from pre-treatment CT scans, achieving a ROC-AUC of 0.73 and F1-score of 0.70.


<details>
  <summary>Details</summary>
Motivation: Early identification of non-responders to neoadjuvant chemotherapy in ovarian cancer patients to prevent ineffective therapy and delays in optimal surgical management.

Method: A non-invasive deep learning framework using pre-treatment contrast-enhanced CT scans, leveraging 3D lesion masks, and combining classification loss with supervised contrastive regularization and hard-negative mining.

Result: The model achieved a ROC-AUC of 0.73 (95% CI: 0.58-0.86) and an F1-score of 0.70 (95% CI: 0.56-0.82) on a test cohort of 280 patients.

Conclusion: The proposed architecture learns clinically relevant predictive patterns and provides a robust foundation for an imaging-based stratification tool for ovarian cancer patients.
...
### [125] [3D Skew-Normal Splatting](https://arxiv.org/abs/2605.15010)
*Xiangru Wu,Ke Fan,Yanwei Fu*

Main category: cs.CV

TL;DR: The paper introduces Skew-Normal Splatting (SNS), an improvement over 3D Gaussian Splatting (3DGS) for novel view synthesis, by using the Azzalini Skew-Normal distribution to model scenes, allowing for more flexible and accurate representation of asymmetric structures.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitation of 3DGS in representing asymmetric structures, such as object boundaries and one-sided surfaces, by introducing a more flexible and continuous shape control.

Method: The method proposed is Skew-Normal Splatting (SNS), which uses the Azzalini Skew-Normal distribution as the fundamental primitive, and introduces a learnable and bounded skewness parameter to continuously interpolate between symmetric Gaussians and Half-Gaussian-like shapes.

Result: The results show that SNS consistently improves reconstruction quality over Gaussian and recent non-Gaussian kernels, especially on sharp boundaries and thin or one-sided structures.

Conclusion: The conclusion is that SNS provides a more flexible and accurate representation of scenes, especially for asymmetric structures, and can be seamlessly integrated into existing Gaussian Splatting rasterization pipelines.
...
### [126] [HiSem: Hierarchical Semantic Disentangling for Remote Sensing Image Change Captioning](https://arxiv.org/abs/2605.15024)
*Man Wang,Chenyang Liu,Wenjun Li,Feng Ni,Bing Jia,Baoqi Huang,Riting Xia,Zhenwei Shi*

Main category: cs.CV

TL;DR: A novel network called HiSem is proposed for remote sensing image change captioning, which disentangles semantic representations of different granularities and outperforms previous methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods for remote sensing image change captioning are limited by a shared modeling assumption, leading to semantic entanglement between coarse-grained change existence judgment and fine-grained semantic understanding.

Method: A hierarchical semantic disentangling network (HiSem) is proposed, which includes a Bidirectional Differential Attention Modulation (BDAM) module and a Hierarchical Adaptive Semantic Disentanglement (HASD) module.

Result: HiSem outperforms previous methods, achieving a significant improvement of +7.52% BLEU-4 on the WHU-CDC dataset.

Conclusion: The proposed HiSem approach provides a structured perspective for remote sensing image change captioning by explicitly aligning model design with the intrinsic semantic heterogeneity of bi-temporal scenes.
...
### [127] [EverAnimate: Minute-Scale Human Animation via Latent Flow Restoration](https://arxiv.org/abs/2605.15042)
*Wuyang Li,Yang Gao,Mariam Hassan,Lan Feng,Wentao Pan,Po-Chien Luan,Alexandre Alahi*

Main category: cs.CV

TL;DR: EverAnimate is a post-training method for generating long-horizon animated videos while preserving visual quality and character identity.


<details>
  <summary>Details</summary>
Motivation: Long-form animation is challenging due to the need to synthesize highly dynamic human motion against relatively static environments, leading to accumulated drift in chunk-based generation.

Method: EverAnimate uses a persistent latent context memory with two mechanisms: Persistent Latent Propagation and Restorative Flow Matching, and requires only lightweight LoRA tuning.

Result: EverAnimate outperforms state-of-the-art long-animation methods in both short- and long-horizon settings, with significant improvements in PSNR, SSIM, LPIPS, and FID metrics.

Conclusion: EverAnimate effectively addresses the challenges of long-horizon animated video generation, preserving visual quality and character identity.
...
### [128] [LATERN: Test-Time Context-Aware Explainable Video Anomaly Detection](https://arxiv.org/abs/2605.15054)
*Mitchell Piehl,Muchao Ye*

Main category: cs.CV

TL;DR: The paper proposes a context-aware framework called LATERN for video anomaly detection, which improves detection accuracy and explanation consistency by aggregating temporal evidence.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the limitation of vision-language models (VLMs) in video anomaly detection, which perform segment-level inference independently without structured temporal context.

Method: The proposed framework LATERN consists of two modules: Context-Aware Anomaly Scoring (CEA) and Recursive Evidence Aggregation (REA).

Result: Extensive experiments on UCF-Crime and XD-Violence benchmarks show that LATERN enhances detection accuracy and explanation consistency for frozen VLMs during test time.

Conclusion: The proposed LATERN framework effectively improves video anomaly detection by providing temporally coherent and semantically grounded event-level explanations.
...
### [129] [Computational Imaging Priors for Wireless Capsule Endoscopy: Monte Carlo-Guided Hemoglobin Mapping for Rare-Anomaly Detection](https://arxiv.org/abs/2605.15062)
*Chengshuai Yang,Lei Xing,Gregory Entin,Roopa Vemulapalli,Lisa Casey,Raiyan Tripti Zaman*

Main category: cs.CV

TL;DR: The paper proposes a Monte Carlo-inspired analytic model to improve the performance of RGB-trained capsule-endoscopy classifiers on small-vessel vascular findings by computing hemoglobin from RGB signal.


<details>
  <summary>Details</summary>
Motivation: RGB-trained capsule-endoscopy classifiers underperform on small-vessel vascular findings due to conflation of hemoglobin contrast with bile and illumination falloff.

Method: The authors evaluate two software-only configurations: (i) a prior P_blood fused as 2 zero-init auxiliary channels; (ii) a distillation head training a 3-channel RGB backbone to predict P_blood, against RGB-only EfficientNet-B0 across 6 seeds on Kvasir-Capsule dataset.

Result: The analytic prior provides a small but direction-consistent macro-AUC improvement: input-fusion 0.783 +/- 0.024, distillation 0.773 +/- 0.028, compared to RGB-only 0.760 +/- 0.027. The largest robust per-class lift is on Lymphangiectasia.

Conclusion: A Monte Carlo-inspired analytic prior provides a small, direction-consistent macro-AUC improvement on Kvasir-Capsule across 6 seeds with the largest robust per-class lift on Lymphangiectasia.
...
### [130] [SAGE3D: Soft-guided attention and graph excitation for 3D point cloud corner detection](https://arxiv.org/abs/2605.15088)
*Batuhan Arda Bekar,Can Sarı,Hüseyin Can Gülkan,Barış Özcan*

Main category: cs.CV

TL;DR: SAGE3D, a hybrid Transformer-based model, is proposed for corner detection in airborne LiDAR point clouds, leveraging a hierarchical encoder-decoder architecture with innovations like Soft-Guided Attention and Excitatory Graph Neural Network.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to improve corner detection in airborne LiDAR point clouds, which is a crucial task in various applications.

Method: The method proposed is a multi-stage solution built on a hierarchical encoder-decoder architecture, utilizing Set Abstraction layers and Feature Propagation, along with two innovations: Soft-Guided Attention and Excitatory Graph Neural Network.

Result: The proposed model, SAGE3D, enables multi-scale feature extraction and amplifies corner signals across scales, optimizing for both precision and recall.

Conclusion: The paper concludes by presenting SAGE3D as an effective solution for corner detection in airborne LiDAR point clouds, leveraging its hierarchical design and innovative modules.
...
### [131] [CoralLite: μCT Reconstruction of Coral Colonies from Individual Corallites](https://arxiv.org/abs/2605.15093)
*Jess Jones,Leonardo Bertini,Kenneth Johnson,Erica Hendy,Tilo Burghardt*

Main category: cs.CV

TL;DR: A novel method, CoralLite, is proposed for 3D modeling of individual corallites in coral skeletons using μCT scans and deep learning.


<details>
  <summary>Details</summary>
Motivation: Understanding the rate and timing of polyp division and its consequences for colony skeletal growth in reef-forming corals.

Method: A hybrid V-Trans-UNet architecture for segmenting μCT virtual slabs of Porites sp. colonies, pre-trained on weakly annotated data and fine-tuned with fully annotated slice sections.

Result: The model achieves 0.94 topological accuracy, mean Dice scores of 0.77 on the same colony, and 0.63 on a biologically unrelated specimen.

Conclusion: Visual machine learning can effectively support full 3D individual corallite modeling from μCT scans of coral skeletons, enabling new insights into coral growth and development.
...
### [132] [DriveCtrl: Conditioned Sim-to-Real Driving Video Generation](https://arxiv.org/abs/2605.15116)
*Haonan Zhao,Yiting Wang,Jingkun Chen,Valentina Donzella,Thomas Bashford-Rogers,Kurt Debattista*

Main category: cs.CV

TL;DR: A new framework called DriveCtrl is proposed for generating realistic driving videos from simulated data, addressing the sim-to-real gap in autonomous driving.


<details>
  <summary>Details</summary>
Motivation: The lack of large-scale labeled driving video data for training autonomous driving systems and the limitations of simulation data due to domain gap.

Method: DriveCtrl, a depth-conditioned controllable sim-to-real video generation framework, uses a pretrained video foundation model with a structure-aware adapter to preserve scene layout and motion patterns.

Result: DriveCtrl outperforms base models and alternatives in realism, temporal quality, and perception task performance, narrowing the sim-to-real gap.

Conclusion: DriveCtrl effectively generates realistic driving videos from simulated data, facilitating the development of autonomous driving systems.
...
### [133] [Causal Forcing++: Scalable Few-Step Autoregressive Diffusion Distillation for Real-Time Interactive Video Generation](https://arxiv.org/abs/2605.15141)
*Min Zhao,Hongzhou Zhu,Kaiwen Zheng,Zihan Zhou,Bokai Yan,Xinyuan Li,Xiao Yang,Chongxuan Li,Jun Zhu*

Main category: cs.CV

TL;DR: Proposed Causal Forcing++ pipeline for frame-wise autoregression with 1-2 sampling steps, surpassing SOTA by improving efficiency and optimization in few-step AR initialization.


<details>
  <summary>Details</summary>
Motivation: Existing autoregressive diffusion distillation methods are limited by coarse response granularity and non-negligible sampling latency, and the study aims to explore a more aggressive setting of frame-wise autoregression with only 1-2 sampling steps.

Method: Causal Forcing++ pipeline using causal consistency distillation (causal CD) for few-step AR initialization, which learns the same AR-conditional flow map as causal ODE distillation but obtains supervision from a single online teacher ODE step.

Result: The proposed pipeline surpasses SOTA 4-step chunk-wise Causal Forcing under the frame-wise 2-step setting by 0.1 in VBench Total, 0.3 in VBench Quality, and 0.335 in VisionReward, while reducing first-frame latency by 50% and Stage 2 training cost by ~4x.

Conclusion: The Causal Forcing++ pipeline provides a more efficient and scalable solution for real-time interactive video generation with low-latency, streaming, and controllable rollout.
...
TL;DR: EviScreen, an evidential reasoning framework for disease screening, was proposed to improve interpretability and performance by leveraging historical case evidence.


<details>
  <summary>Details</summary>
Motivation: Current disease screening models for medical images have limited interpretability and suboptimal performance, lacking effective mechanisms to reference historical cases or provide transparent reasoning pathways.

Method: EviScreen uses dual knowledge banks to retrieve regional evidence from historical cases, and an evidence-aware reasoning module to make predictions based on both current and historical cases.

Result: EviScreen achieves superior performance on real-world disease screening benchmarks, with notably higher specificity at clinical-level recall.

Conclusion: EviScreen offers improved interpretability and performance in disease screening by leveraging historical case evidence, making it a promising solution for clinical practice.
...
### [135] [SANA-WM: Efficient Minute-Scale World Modeling with Hybrid Linear Diffusion Transformer](https://arxiv.org/abs/2605.15178)
*Haoyi Zhu,Haozhe Liu,Yuyang Zhao,Tian Ye,Junsong Chen,Jincheng Yu,Tong He,Song Han,Enze Xie*

Main category: cs.CV

TL;DR: SANA-WM is an efficient open-source world model for generating high-fidelity, minute-scale videos with precise camera control, achieving comparable visual quality to industrial baselines while improving efficiency.


<details>
  <summary>Details</summary>
Motivation: The motivation behind SANA-WM is to develop an efficient and open-source world model capable of generating high-quality, minute-scale videos with precise camera control, addressing the limitations of existing models in terms of efficiency and scalability.

Method: SANA-WM's architecture is driven by four core designs: Hybrid Linear Attention, Dual-Branch Camera Control, Two-Stage Generation Pipeline, and Robust Annotation Pipeline, which enable efficient long-context modeling, precise camera control, and high-quality video generation.

Result: SANA-WM demonstrates remarkable efficiency across data, training compute, and inference hardware, achieving stronger action-following accuracy than prior open-source baselines and comparable visual quality at 36x higher throughput for scalable world modeling.

Conclusion: SANA-WM is a significant advancement in world modeling, offering a more efficient and scalable solution for generating high-quality, minute-scale videos with precise camera control, making it suitable for various applications.
...
### [136] [From Plans to Pixels: Learning to Plan and Orchestrate for Open-Ended Image Editing](https://arxiv.org/abs/2605.15181)
*Anirudh Sundara Rajan,Krishna Kumar Singh,Yong Jae Lee*

Main category: cs.CV

TL;DR: The paper proposes a framework for long-horizon image editing, where a planner and orchestrator work together to execute multi-step instructions, with a vision language judge providing outcome-based rewards.


<details>
  <summary>Details</summary>
Motivation: Modern image editing models struggle with abstract, multi-step instructions, and prior agent-based methods are limited by handcrafted pipelines or teacher imitation.

Method: The proposed framework consists of a planner that generates structured atomic decompositions, an orchestrator that selects tools and regions to execute each step, and a vision language judge that provides outcome-based rewards.

Result: The approach yields more coherent and reliable edits than single-step or rule-based multistep baselines.

Conclusion: The paper presents a novel framework for long-horizon image editing that tightly couples planning with reward-driven execution, leading to improved performance.
...
### [137] [Warp-as-History: Generalizable Camera-Controlled Video Generation from One Training Video](https://arxiv.org/abs/2605.15182)
*Yifan Wang,Tong He*

Main category: cs.CV

TL;DR: Proposed Warp-as-History, a simple interface enabling zero-shot video generation following camera trajectories without training or optimization.


<details>
  <summary>Details</summary>
Motivation: Existing camera-controlled video generation methods require post-training on large-scale camera-annotated videos or test-time optimization, which can be costly and inefficient.

Method: Warp-as-History turns camera-induced warps into pseudo-history with target-frame alignment and token selection, feeding it through the model's visual-history pathway.

Result: The proposed method shows non-trivial zero-shot capability of a frozen video generation model to follow camera trajectories, and further improves with lightweight offline LoRA finetuning.

Conclusion: Warp-as-History is effective in enabling camera-controlled video generation without requiring training, architectural modification, or test-time optimization.
...
### [138] [Quantitative Video World Model Evaluation for Geometric-Consistency](https://arxiv.org/abs/2605.15185)
*Jiaxin Wu,Yihao Pi,Yinling Zhang,Yuheng Li,Xueyan Zou*

Main category: cs.CV

TL;DR: A new framework called PDI-Bench is introduced to evaluate the geometric coherence of generated videos, providing a quantitative approach to assess physical plausibility.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of evaluating whether generative video models produce physically plausible 3D structure and motion, as existing methods rely heavily on subjective human judgment or learned graders.

Method: The method involves obtaining object-centric observations via segmentation and point tracking, lifting them to 3D world-space coordinates via monocular reconstruction, and computing projective-geometry residuals to capture failure dimensions.

Result: The results show that PDI-Bench reveals consistent geometry-specific failure modes in state-of-the-art video generators that are not captured by common perceptual metrics.

Conclusion: The conclusion is that PDI-Bench provides a diagnostic signal for progress toward physically grounded video generation and physical world models, offering a quantitative approach to auditing geometric coherence in generated videos.
...
### [139] [VGGT-Edit: Feed-forward Native 3D Scene Editing with Residual Field Prediction](https://arxiv.org/abs/2605.15186)
...
TL;DR: A new framework called VGGT-Edit is proposed for text-conditioned native 3D scene editing, which enables efficient and high-quality editing of 3D scenes based on text instructions.


<details>
  <summary>Details</summary>
Motivation: Existing 3D scene reconstruction models struggle with dynamic human instructions and interactive applications, and current editing methods often produce blurry textures and inconsistent geometry.

Method: VGGT-Edit uses depth-synchronized text injection and a residual transformation head to predict 3D geometric displacements, ensuring stable instruction grounding and preserving background stability.

Result: VGGT-Edit outperforms 2D-lifting baselines, producing sharper object details, stronger multi-view consistency, and near-instant inference speed.

Conclusion: The proposed VGGT-Edit framework enables efficient and high-quality text-conditioned native 3D scene editing, addressing the limitations of existing methods and opening up new possibilities for interactive applications.
...
### [140] [Articraft: An Agentic System for Scalable Articulated 3D Asset Generation](https://arxiv.org/abs/2605.15187)
...
TL;DR: The paper proposes using large language models to generate articulated 3D objects at scale, and introduces a system called Articraft that automates this process.


<details>
  <summary>Details</summary>
Motivation: The lack of large and diverse datasets of articulated 3D objects hinders learning to understand them.

Method: The authors leverage large language models to generate articulated 3D assets by writing programs that build them, using a domain-specific SDK and a restricted workspace.

Result: The proposed system, Articraft, produces higher-quality assets than state-of-the-art generators and coding agents, and is used to build a dataset of over 10K articulated assets.

Conclusion: The paper demonstrates the effectiveness of using large language models and the Articraft system to generate high-quality articulated 3D objects at scale, with applications in training models and downstream tasks such as robotics simulation and virtual reality.
...
### [141] [RAVEN: Real-time Autoregressive Video Extrapolation with Consistency-model GRPO](https://arxiv.org/abs/2605.15190)
*Yanzuo Lu,Ronglai Zuo,Jiankang Deng*

Main category: cs.CV

TL;DR: The authors propose RAVEN, a framework for real-time video generation that improves upon existing causal autoregressive models by better aligning training and inference processes.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the gap between training and inference distributions in causal autoregressive video diffusion models, which limits generation quality over long horizons.

Method: The authors introduce RAVEN, a training-time test framework that repacks self-rollouts into interleaved sequences of clean historical endpoints and noisy denoising states, and propose Consistency-model Group Relative Policy Optimization (CM-GRPO), an online Reinforcement Learning approach.

Result: Experiments demonstrate that RAVEN surpasses recent causal video distillation baselines across quality, semantic, and dynamic degree evaluations, and that CM-GRPO provides further gains when combined with RAVEN.

Conclusion: The proposed RAVEN framework and CM-GRPO method improve real-time video generation quality by addressing the training-inference distribution gap and optimizing the generation process.
...
### [142] [Aligning Latent Geometry for Spherical Flow Matching in Image Generation](https://arxiv.org/abs/2605.15193)
*Tuna Han Salih Meral,Kaan Oktay,Hidir Yesiltepe,Adil Kaan Akan,Pinar Yanardag*

Main category: cs.CV

TL;DR: The paper presents a method to improve image generation using latent flow matching by projecting data latents onto a fixed token radius and using spherical linear interpolation.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the issue that in latent flow matching for image generation, Gaussian noise is transported to variational autoencoder latents along linear paths, which may not effectively utilize the structure of the latent space.

Method: The method involves decomposing each latent token into radial and angular components, projecting data latents onto a fixed token radius, using the radial projection of Gaussian noise as the spherical prior, fine-tuning the decoder with the encoder frozen, and replacing linear interpolation with spherical linear interpolation.

Result: The method consistently improves class-conditional ImageNet-256 FID across different image tokenizers, does not change the diffusion architecture, and does not require an auxiliary encoder or representation-alignment objective.

Conclusion: The paper concludes that by utilizing spherical geometry in latent flow matching, it is possible to improve image generation quality without modifying the underlying diffusion architecture or requiring additional components.
...
### [143] [VGGT-$Ω$](https://arxiv.org/abs/2605.15195)
...
TL;DR: VGGT-Ω, an improved feed-forward reconstruction model, achieves state-of-the-art results in static and dynamic scene reconstruction, with significant improvements in accuracy, efficiency, and capabilities.


<details>
  <summary>Details</summary>
Motivation: To scale up the quality of feed-forward reconstruction models, such as VGGT, by introducing architectural changes, a high-quality data annotation pipeline, and a self-supervised learning protocol.

Method: VGGT-Ω introduces a single dense prediction head with multi-task supervision, removes expensive high-resolution convolutional layers, uses registers to aggregate scene information, and introduces register attention. The model is trained with 15x more supervised data and vast amounts of unlabeled video data.

Result: VGGT-Ω achieves strong results for reconstruction of static and dynamic scenes across multiple benchmarks, improving over the previous best camera estimation accuracy on Sintel by 77%. The learned registers can also improve vision-language-action models and support alignment with language.

Conclusion: VGGT-Ω demonstrates that the quality of feed-forward reconstruction models scales predictably with model and data size, and shows that reconstruction can be a powerful and scalable proxy task for spatial understanding.
...
### [144] [RefDecoder: Enhancing Visual Generation with Conditional Video Decoding](https://arxiv.org/abs/2605.15196)
*Xiang Fan,Yuheng Wang,Bohan Fang,Zhongzheng Ren,Ranjay Krishna*

Main category: cs.CV

TL;DR: The paper introduces RefDecoder, a reference-conditioned video VAE decoder that improves video generation by injecting high-fidelity reference image signals into the decoding process, achieving better detail preservation and consistency.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the architectural asymmetry in latent diffusion models, where the decoder remains unconditional, leading to loss of detail and inconsistency in generated videos.

Method: The method involves introducing a reference-conditioned video VAE decoder, RefDecoder, which injects high-fidelity reference image signals into the decoding process via reference attention, using a lightweight image encoder to map the reference frame into detail-rich high-dimensional tokens.

Result: The results show consistent improvements across several decoder backbones, achieving up to +2.1dB PSNR over unconditional baselines on several benchmarks, and across-the-board improvements in subject consistency, background consistency, and overall quality scores.

Conclusion: The conclusion is that RefDecoder can be directly swapped into existing video generation systems without additional fine-tuning, and it generalizes well to a wide range of visual generation tasks such as style transfer and video editing refinement.
...
### [145] [EntityBench: Towards Entity-Consistent Long-Range Multi-Shot Video Generation](https://arxiv.org/abs/2605.15199)
*Ruozhen He,Meng Wei,Ziyan Yang,Vicente Ordonez*

Main category: cs.CV

TL;DR: The paper introduces EntityBench, a benchmark for multi-shot video generation that focuses on maintaining consistent characters, objects, and locations across shots. It also proposes EntityMem, a memory-augmented generation system that improves cross-shot entity consistency.


<details>
  <summary>Details</summary>
Motivation: The challenge of maintaining consistent characters, objects, and locations across shots in multi-shot video generation, and the lack of standardized evaluation methods.

Method: The paper proposes EntityBench, a benchmark with 140 episodes and 2,491 shots, and a three-pillar evaluation suite. It also introduces EntityMem, a memory-augmented generation system.

Result: Experiments show that existing methods struggle with cross-shot entity consistency, and that EntityMem yields the highest character fidelity (Cohen's d = +2.33) and presence among methods evaluated.

Conclusion: The paper concludes that EntityBench and EntityMem can help improve cross-shot entity consistency in multi-shot video generation, and provides a standardized evaluation method for future research.
...
### [146] [Merging Methods for Multilingual Knowledge Editing for Large Language Models: An Empirical Odyssey](https://arxiv.org/abs/2605.13919)
*Kunil Lee,Ki-Young Shin,Jong-Hyeok Lee,Young-Joo Suh*

Main category: cs.CL

TL;DR: The paper evaluates vector merging methods for multilingual knowledge editing (MKE), focusing on their effectiveness, interference mitigation, and parameter influence.


<details>
  <summary>Details</summary>
Motivation: Multilingual knowledge editing (MKE) is challenging due to interference between language-specific edits, even with effective monolingual methods.

Method: The paper evaluates six merging variants with two large language models, two base knowledge editing methods, and 12 languages on the MzsRE benchmark.

Result: Vector summation with shared covariance is the most reliable strategy, while simple summation without shared covariance performs poorly. TSVM improves performance in some settings but has limited ability to mitigate interference.

Conclusion: The findings provide guidance for future MKE research, highlighting the practical strengths and limits of current vector merging methods.
...
### [147] [VectraYX-Nano: A 42M-Parameter Spanish Cybersecurity Language Model with Curriculum Learning and Native Tool Use](https://arxiv.org/abs/2605.13989)
*Juan S. Santillana*

Main category: cs.CL

TL;DR: VectraYX-Nano, a 41.95M-parameter Spanish language model for cybersecurity, is trained from scratch with native tool invocation capabilities.


<details>
  <summary>Details</summary>
Motivation: To create a Spanish-native cybersecurity LLM with end-to-end Model Context Protocol (MCP) integration, focusing on Latin-American context.

Method: The model uses a 170M-token Spanish corpus (VectraYX-Sec-ES), a 42M-parameter Transformer decoder architecture, and a curriculum with replay for continual pre-training.

Result: The model achieves a conversational gate of 0.78+-0.05 and tool-selection performance improvements with a dense corpus, with the GGUF artifact being 81 MB and running at sub-second TTFT on commodity hardware.

Conclusion: VectraYX-Nano is the first Spanish-native cybersecurity LLM with end-to-end MCP integration, and its corpus recipe, training scripts, GGUF weights, and B1-B5 benchmark are released.
...
### [148] [Mistletoe: Stealthy Acceleration-Collapse Attacks on Speculative Decoding](https://arxiv.org/abs/2605.14005)
*Shuoyang Sun,Chang Da,Hao Fang,Kuofeng Gao,Xinhao Zhong,Yi Sun,Fan Mo,Shu-Tao Xia,Bin Chen*

Main category: cs.CL

TL;DR: Mistletoe is a stealthy attack against speculative decoding in large language models, which reduces draft-token acceptability while preserving output quality.


<details>
  <summary>Details</summary>
Motivation: The efficiency of speculative decoding depends on the average accepted length, and a mismatch between the drafter and target models creates a vulnerability that can be exploited.

Method: Mistletoe jointly optimizes a degradation objective and a semantic-preservation objective using a null-space projection mechanism.

Result: Mistletoe substantially reduces average accepted length, collapses speedup, and lowers averaged token throughput while preserving output quality and perplexity.

Conclusion: Speculative decoding introduces a mechanism-level attack surface beyond existing output robustness, calling for more robust designs of LLM acceleration systems.
...
### [149] [Physics-R1: An Audited Olympiad Corpus and Recipe for Visual Physics Reasoning](https://arxiv.org/abs/2605.14040)
*Shan Yang*

Main category: cs.CL

TL;DR: The paper audits the multimodal-physics evaluation pipeline, revealing issues like train-eval contamination, translation drift, and MCQ saturation, and proposes new methods and datasets to address these gaps.


<details>
  <summary>Details</summary>
Motivation: The authors aim to identify and address undetected construction practices that distort the measurement of vision-language reasoning in multimodal-physics evaluations.

Method: The authors perform a thorough audit of the evaluation pipeline, using techniques such as 5-gram-Jaccard audits, mxbai-embed-large cosine, and Haiku-4.5 LLM-judge to detect near-duplicates and paraphrase candidates. They also propose new methods, including a reference GSPO+DAPO recipe cold-started from Qwen3-VL-8B-Thinking.

Result: The authors find significant issues in the current evaluation pipeline, including train-eval contamination, translation drift, and MCQ saturation. They also demonstrate the effectiveness of their proposed methods, achieving improvements of up to 18.3 pp on PhysOlym-A liberal and 15.7 pp on PhysReason.

Conclusion: The paper highlights the need for more rigorous evaluation methods in multimodal-physics reasoning and proposes new approaches to address the identified gaps. The authors release several artifacts, including PhysCorp-A, PhysR1Corp, PhysOlym-A, and Physics-R1, to support future research.
...
### [150] [Derivation Prompting: A Logic-Based Method for Improving Retrieval-Augmented Generation](https://arxiv.org/abs/2605.14053)
*Ignacio Sastre,Guillermo Moncecchi,Aiala Rosá*

Main category: cs.CL

TL;DR: A novel prompting technique called Derivation Prompting is introduced to improve the accuracy of Large Language Models in question answering tasks by reducing hallucinations and erroneous reasoning.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the challenges of hallucinations and erroneous reasoning in Large Language Models, particularly in knowledge-intensive and domain-specific tasks.

Method: The method used in this paper is Derivation Prompting, a novel prompting technique that involves deriving conclusions from initial hypotheses through the systematic application of predefined rules, and constructs an interpretable derivation tree.

Result: The results show that Derivation Prompting significantly reduces unacceptable answers compared to traditional Retrieval-Augmented Generation (RAG) and long-context window methods.

Conclusion: The conclusion of this paper is that Derivation Prompting is an effective technique for improving the accuracy and reliability of Large Language Models in question answering tasks.
...
### [151] [PEML: Parameter-efficient Multi-Task Learning with Optimized Continuous Prompts](https://arxiv.org/abs/2605.14055)
*Anjir Ahmed Chowdhury,Syed Zawad,Xiaolong Ma,Xu Dong,Feng Yan*

Main category: cs.CL

TL;DR: The paper proposes Parameter-Efficient Multi-task Learning (PEML), a method for efficient fine-tuning of Large Language Models (LLMs) for multiple tasks, achieving up to 6.67% average accuracy improvement.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enable efficient fine-tuning of LLMs for multiple tasks, reducing data and resource requirements, and improving performance over existing methods.

Method: The method proposed is PEML, which co-optimizes prompt optimization and model adaptation using a neural architecture engineering method and low-rank adaption for model weights.

Result: The evaluation results show an average accuracy improvement of up to 6.67% and individual task gains of up to 10.75% compared to state-of-the-art multi-task learning methods.

Conclusion: The proposed PEML method effectively enables efficient fine-tuning of LLMs for multiple tasks, achieving significant performance improvements over existing methods.
...
### [152] [Dual Hierarchical Dialogue Policy Learning for Legal Inquisitive Conversational Agents](https://arxiv.org/abs/2605.14057)
*Xubo Lin,Zezhii Deng,Shihao Wang,Grace Hui Yang,Yang Deng*

Main category: cs.CL

TL;DR: Introducing Inquisitive Conversational Agents (ICAs) that proactively extract information to achieve objectives, with a Dual Hierarchical Reinforcement Learning framework for strategic dialogue management.


<details>
  <summary>Details</summary>
Motivation: Existing dialogue systems are user-driven and primarily designed to fulfill user requests, but in critical real-world scenarios, conversational agents must proactively extract information to achieve their own objectives.

Method: A Dual Hierarchical Reinforcement Learning framework featuring two cooperating RL agents, each with its own policy, to coordinate strategic dialogue management and fine-grained utterance generation.

Result: The proposed method outperforms various baselines across multiple metrics on a U.S. Supreme Court dataset.

Conclusion: This represents an important first step toward broader high-stakes, domain-specific applications of Inquisitive Conversational Agents.
...
### [153] [Measuring and Mitigating Toxicity in Large Language Models: A Comprehensive Replication Study](https://arxiv.org/abs/2605.14087)
*Mokshit Surana,Archit Rathod,Akshaj Satishkumar*

Main category: cs.CL

TL;DR: The paper evaluates the effectiveness of DExperts, a technique to mitigate toxic outputs from large language models, finding it effective against explicit toxicity but brittle against implicit hate speech.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the issue of 'toxic degeneration' in large language models, where innocuous prompts can trigger harmful outputs, and to evaluate the efficacy of DExperts as a mitigation strategy.

Method: The study uses a comprehensive replication approach, consisting of three phases: establishing baseline toxicity measurements, implementing and evaluating DExperts, and stress-testing the method against implicit hate speech.

Result: The results show that DExperts achieves near-perfect safety rates (100%) on explicit toxicity benchmarks but exhibits brittleness against adversarial, implicit hate speech, with safety rates dropping to 98.5%, and introduces a significant latency penalty.

Conclusion: The study concludes that while DExperts is effective against explicit toxicity, it highlights the need for more sophisticated approaches that can generalize across diverse hate speech patterns without prohibitive computational costs.
...
### [154] [When Evidence Conflicts: Uncertainty and Order Effects in Retrieval-Augmented Biomedical Question Answering](https://arxiv.org/abs/2605.14115)
*Yikun Han,Mengfei Lan,Halil Kilicoglu*

Main category: cs.CL

TL;DR: The paper evaluates the reliability of biomedical large language models (LLMs) under conflicting evidence conditions and proposes a conflict-aware abstention score to improve selective accuracy.


<details>
  <summary>Details</summary>
Motivation: Biomedical LLMs often face incomplete, misleading, or contradictory evidence, but evaluations focus on accuracy under helpful context rather than reliability under conflict.

Method: The authors use HealthContradict to evaluate six open-weight LLMs under five controlled evidence conditions, including conflicting evidence, and propose a conflict-aware abstention score.

Result: The results show that accuracy drops for every model under conflicting evidence conditions, and the conflict-aware abstention score improves selective accuracy over confidence-only methods.

Conclusion: The study highlights the need for evaluation and abstention methods that account for evidence disagreement in biomedical LLMs.
...
### [155] [Generative Floor Plan Design with LLMs via Reinforcement Learning with Verifiable Rewards](https://arxiv.org/abs/2605.14117)
*Luis Lara,Aristides Milios,Zhi Hao Luo,Aditya Sharma,Ge Ya Luo,Christopher Beckham,Florian Golemo,Christopher Pal*

Main category: cs.CL

TL;DR: The paper presents a text-based approach for generating floor plans that respect both connectivity and numerical constraints, leveraging a fine-tuned large language model and reinforcement learning.


<details>
  <summary>Details</summary>
Motivation: Existing generative approaches for floor plan design primarily focus on room connectivity, neglecting numerical constraints. The authors aim to address this limitation.

Method: The authors use a large language model fine-tuned on real plans, combined with reinforcement learning with verifiable rewards (RLVR) to ensure adherence to both topological and numerical constraints.

Result: The proposed model outperforms existing methods, achieving a at least 94% relative reduction in Compatibility errors and generating floor plans that satisfy user-defined constraints.

Conclusion: The study demonstrates the effectiveness of large language models in handling constraints for floor plan generation, suggesting broader applications for text-based generative modeling.
...
### [156] [Polar probe linearly decodes semantic structures from LLMs](https://arxiv.org/abs/2605.14125)
*Pablo J. Diego-Simón,Pierre Orhan,Yair Lakretz,Jean-Rémi King*

Main category: cs.CL

TL;DR: The study proposes a simple neural code for artificial neural networks to form complex semantic structures, where relations between entities are represented by distance and direction between their embeddings.


<details>
  <summary>Details</summary>
Motivation: Understanding how artificial neural networks bind concepts to form complex semantic structures.

Method: Testing the hypothesis in Large Language Models (LLMs) with natural-language descriptions of minimalist tasks from five different domains, using a Polar Probe to target a subspace of LLMs' layer activations.

Result: The true semantic structures can be linearly recovered, the code emerges mostly in middle layers and improves with LLM performance, Polar Probes generalize to new entities and relation types but degrade with semantic structure size, and polar representation quality correlates with LLM's question-answering ability.

Conclusion: LLMs learn to build complex semantic structures by binding representations with a simple geometrical principle.
...
### [157] [ROK-FORTRESS: Measuring the Effect of Geopolitical Transcreation for National Security and Public Safety](https://arxiv.org/abs/2605.14152)
...
TL;DR: The paper introduces ROK-FORTRESS, a bilingual benchmark to evaluate the safety of large language models in National Security and Public Safety risks, focusing on the English-Korean language pair and US-ROK geopolitical context.


<details>
  <summary>Details</summary>
Motivation: Current safety evaluations for large language models focus on high-stakes risks but often overlook the interaction between language and geopolitical context, particularly in multilingual settings.

Method: The authors introduce a transcreation matrix to separate the effects of language and geopolitical grounding, evaluating models using a bilingual, culturally adversarial benchmark and LLM-as-a-judge panels.

Result: The study finds a consistent suppression effect in Korean variants of models and significant model-to-model variation in how geopolitical grounding interacts with language, indicating that safety behavior is shaped by language and context interactions.

Conclusion: The paper highlights the importance of considering language and geopolitical context in safety evaluations for large language models, and the transcreation matrix methodology can be generalized to other language-culture pairs.
...
### [158] [BOOKMARKS: Efficient Active Storyline Memory for Role-playing](https://arxiv.org/abs/2605.14169)
*Letian Peng,Ziche Liu,Yiming Huang,Longfei Yun,Kun Zhou,Yupeng Hou,Jingbo Shang*

Main category: cs.CL

TL;DR: The authors propose a search-based memory framework called BOOKMARKS for role-playing agents to maintain long-horizon consistency, which outperforms existing memory methods.


<details>
  <summary>Details</summary>
Motivation: Existing RPA memory methods rely on recurrent summarization, which discards important details, and the authors aim to address this issue.

Method: The authors propose a search-based memory framework called BOOKMARKS, which initializes, maintains, and updates task-relevant bookmarks for the current task.

Result: BOOKMARKS significantly outperforms RPA memory baselines on 85 characters from 16 artifacts.

Conclusion: The proposed BOOKMARKS framework is effective for role-playing agents to maintain long-horizon consistency.
...
### [159] [Why Retrieval-Augmented Generation Fails: A Graph Perspective](https://arxiv.org/abs/2605.14192)
*Kai Guo,Xinnan Dai,Zhibo Zhang,Nuohan Lin,Shenglai Zeng,Jie Ren,Haoyu Han,Jiliang Tang*

Main category: cs.CL

TL;DR: The paper studies why Retrieval-Augmented Generation (RAG) systems fail despite having access to external information. It uses circuit tracing to construct attribution graphs, finding that correct predictions have deeper reasoning paths and more distributed evidence flow, while failed predictions are shallower and more fragmented. A graph-based error detection framework and targeted interventions are developed.


<details>
  <summary>Details</summary>
Motivation: RAG systems produce incorrect answers despite having access to external information, and the reasons for these failures are not well understood.

Method: The study uses circuit tracing to construct attribution graphs that model the flow of information through transformer layers during decoding, and analyzes the structural differences between correct and failed predictions.

Result: The study finds consistent structural differences between correct and failed predictions, and develops a graph-based error detection framework and targeted interventions that lead to more effective integration of retrieved information and fewer errors.

Conclusion: The study provides insights into why RAG systems fail and proposes a graph-based approach to improve their performance by enabling more effective integration of retrieved information.
...
### [160] [GradShield: Alignment Preserving Finetuning](https://arxiv.org/abs/2605.14194)
*Zhanhao Hu,Xiao Huang,Patrick Mendoza,Emad A. Alghamdi,Basel Alomair,Raluca Ada Popa,David Wagner*

Main category: cs.CL

TL;DR: Introduced GradShield, a filtering method that safeguards Large Language Models (LLMs) during finetuning by removing harmful data points, achieving better safety and utility performance.


<details>
  <summary>Details</summary>
Motivation: LLMs are at risk of safety misalignment after finetuning due to harmful data, including seemingly benign data that can lead to misaligned behaviors.

Method: GradShield, a principled filtering method that computes a Finetuning Implicit Harmfulness Score (FIHS) for each data point and uses adaptive thresholding to remove potentially harmful data.

Result: GradShield outperforms baseline methods, maintaining an Attack Success Rate (ASR) below 6% while preserving utility performance across multiple utility fine-tuning tasks.

Conclusion: GradShield effectively safeguards LLMs during finetuning, ensuring both safety and utility performance by identifying and removing harmful data points.
...
### [161] [What Makes Words Hard? Sakura at BEA 2026 Shared Task on Vocabulary Difficulty Prediction](https://arxiv.org/abs/2605.14257)
*Adam Nohejl,Xuanxin Wu,Yusuke Ide,Maria Angelica Riera Machin,Yi-Ning Chang,Hitomi Yanaka*

Main category: cs.CL

TL;DR: The paper presents two models for predicting vocabulary difficulty: a high-accuracy black-box model and an explainable model, with the black-box model achieving a correlation of over 0.91 and the explainable model achieving a correlation of over 0.77.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to develop models that can accurately predict vocabulary difficulty, which is essential for language learning and assessment.

Method: The authors fine-tuned a large language model (LLM) using a soft-target loss function for the black-box model and developed an explainable model that provides insights into the factors affecting vocabulary difficulty.

Result: The black-box model achieved a correlation of over 0.91, while the explainable model achieved a correlation of over 0.77. The analysis also showed that the difficulty of items in the British Council's Knowledge-based Vocabulary Lists (KVL) is often affected by spelling difficulty or the construction of the test items.

Conclusion: The paper concludes that the developed models can effectively predict vocabulary difficulty, and the explainable model provides valuable insights into the factors affecting vocabulary difficulty.
...
### [162] [Auditing Agent Harness Safety](https://arxiv.org/abs/2605.14271)
...
TL;DR: The paper proposes HarnessAudit, a framework for auditing the safety of LLM agent execution harnesses, and presents HarnessAudit-Bench, a benchmark for evaluating harness safety across various domains and configurations.


<details>
  <summary>Details</summary>
Motivation: Current safety benchmarks only evaluate final outputs or terminal states, which is insufficient for detecting safety violations that occur mid-trajectory. The authors aim to address this gap by proposing a framework that audits full execution trajectories.

Method: The authors propose HarnessAudit, a framework that audits execution trajectories across boundary compliance, execution fidelity, and system stability. They also introduce HarnessAudit-Bench, a benchmark of 210 tasks across eight real-world domains.

Result: The evaluation of ten harness configurations across frontier models and three multi-agent frameworks reveals that task completion is often misaligned with safe execution, and safety risks vary across domains, task types, and agent roles. The results also show that most violations concentrate in resource access and inter-agent information transfer.

Conclusion: The paper concludes that HarnessAudit and HarnessAudit-Bench can help identify safety risks in LLM agent execution harnesses, and that harness design plays a crucial role in ensuring safe deployment.
...
### [163] [Ideology Prediction of German Political Texts](https://arxiv.org/abs/2605.14352)
*Sinclair Schneider,Florian Steuber,Joao A. G. Schneider,Gabi Dreo Rodosek*

Main category: cs.CL

TL;DR: The paper proposes a transformer-based model to predict the political orientation of text on a continuous left-to-right spectrum, and evaluates its performance on four distinct corpora.


<details>
  <summary>Details</summary>
Motivation: To better understand political rhetoric from various movements, the authors aim to develop a model that can project political orientation of text on a continuous spectrum.

Method: The authors use a transformer-based model and evaluate 13 candidate transformers on four distinct corpora, including annotated plenary notes, an online decision-making tool, newspaper articles, and tweets.

Result: The model achieves high performance, with DeBERTa-large achieving F1=0.844 on in-domain test and ACC=0.864 on out-of-domain Twitter test, and Gemma2-2B achieving MAE = 0.172 on out-of-domain newspaper test.

Conclusion: The study demonstrates that transformer models can recognize political framing in German news at the level of public opinion polls, and that both model architecture and domain-specific training data are influential for estimating political bias.
...
### [164] [LLM-based Detection of Manipulative Political Narratives](https://arxiv.org/abs/2605.14354)
*Sinclair Schneider,Florian Steuber,Gabi Dreo Rodosek*

Main category: cs.CL

TL;DR: A new computational framework detects and structures manipulative political narratives on social media by combining prompt-based filtering and unsupervised clustering.


<details>
  <summary>Details</summary>
Motivation: The shift of political discussions to social media has increased the importance of differentiating between manipulative political narratives and legitimate critiques.

Method: The framework uses a few-shot prompt to filter manipulative posts, then applies UMAP and HDBSCAN for clustering, and finally employs a reasoning model to uncover narrative behind each cluster.

Result: The approach identified 41 distinct manipulative narrative clusters from over 1.2 million social media posts.

Conclusion: The proposed framework effectively detects and structures manipulative political narratives on social media, enabling the identification of new narrative clusters.
...
### [165] [Reinforcement Learning with Semantic Rewards Enables Low-Resource Language Expansion without Alignment Tax](https://arxiv.org/abs/2605.14366)
*Zeli Su,Ziyin Zhang,Zhou Liu,Xuexian Song,Zhankai Xu,Longfei Zheng,Xiaolu Zhang,Rong Fu,Guixian Xu,Wentao Zhang*

Main category: cs.CL

TL;DR: The paper proposes a new method for adapting large language models to low-resource languages, mitigating the 'alignment tax' issue where improvements in the target language come at the cost of losing general capabilities.


<details>
  <summary>Details</summary>
Motivation: The authors are motivated by the problem of 'alignment tax' in adapting large language models to low-resource languages, where improvements in the target language come at the cost of catastrophic forgetting in general capabilities.

Method: The authors propose a semantic-space alignment paradigm powered by Group Relative Policy Optimization (GRPO), which optimizes the model using embedding-level semantic rewards rather than likelihood maximization.

Result: The experiments show that the proposed method acquires low-resource capabilities while markedly mitigating alignment tax, preserving general competence more effectively than supervised fine-tuning (SFT).

Conclusion: The study demonstrates that reinforcement learning with semantic rewards provides a safer and more reliable pathway for inclusive low-resource language expansion.
...
### [166] [Where Should Diffusion Enter a Language Model? Geometry-Guided Hidden-State Replacement](https://arxiv.org/abs/2605.14368)
*Injin Kong,Hyoungjoon Lee,Yohan Jo*

Main category: cs.CL

TL;DR: Proposed DiHAL, a hybrid model combining diffusion and transformer architectures for language modeling, improving performance over continuous diffusion baselines.


<details>
  <summary>Details</summary>
Motivation: Continuous diffusion language models lag behind autoregressive transformers, and the authors aim to address this gap by proposing a geometry-guided diffusion-transformer hybrid.

Method: The authors propose DiHAL, which scores transformer layers using geometry-based proxies, selects a diffusion-friendly hidden-state interface, and replaces the lower transformer prefix with a diffusion bridge.

Result: Experiments on 8B-scale backbones show that the geometry score predicts effective shallow insertion layers, and hidden-state recovery improves over continuous diffusion baselines.

Conclusion: The results suggest that hidden-state geometry helps identify where diffusion-based replacement is feasible inside pretrained language models, and DiHAL offers a promising approach to improving language modeling performance.
...
### [167] [Mitigating Data Scarcity in Psychological Defense Classification with Context-Aware Synthetic Augmentation](https://arxiv.org/abs/2605.14380)
*Hoang-Thuy-Duong Vu,Quoc-Cuong Pham,Huy-Hieu Pham*

Main category: cs.CL

TL;DR: The authors propose a context-aware synthetic augmentation framework and a hybrid classification model to automatically classify psychological defense mechanisms from text, achieving improved performance over existing methods.


<details>
  <summary>Details</summary>
Motivation: The study aims to address the challenges of data scarcity and class imbalance in automatically classifying psychological defense mechanisms (PDMs) from text, which is clinically valuable but difficult to achieve.

Method: The authors propose a context-aware synthetic augmentation framework combined with a hybrid classification model that integrates contextual language representations with basic clinical features and 150 annotated defense items.

Result: The method surpasses DMRS Co-Pilot, reaching an accuracy of 58.26% (+40.25%) and a macro-F1 of 24.62% (+15.99%), establishing a strong baseline for psychologically grounded defense mechanism classification in low-resource settings.

Conclusion: The study demonstrates the effectiveness of the proposed method in classifying psychological defense mechanisms from text, particularly in low-resource settings, and highlights the importance of definition quality in prompting for generation fidelity and downstream performance.
...
### [168] [Agentic Recommender System with Hierarchical Belief-State Memory](https://arxiv.org/abs/2605.14401)
...
TL;DR: The paper proposes MARS, a memory-augmented recommender system that uses a structured belief state and a lifecycle of six operations to improve personalized recommendations.


<details>
  <summary>Details</summary>
Motivation: Existing memory-augmented LLM agents use flat memory representations that mix ephemeral signals with stable preferences, and lack a complete lifecycle for memory evolution.

Method: MARS uses a three-tiered memory structure (event, preference, and profile memory) and an LLM-based planner to adaptively schedule six operations (extraction, reinforcement, weakening, consolidation, forgetting, and resynthesis).

Result: MARS achieves state-of-the-art performance on four InstructRec benchmark domains with average improvements of 26.4% in HR@1 and 10.3% in NDCG@10 over the strongest baselines.

Conclusion: The proposed MARS framework provides a more structured and dynamic approach to memory-augmented recommendation, leading to significant performance improvements.
...
### [169] [Knowledge Beyond Language: Bridging the Gap in Multilingual Machine Unlearning Evaluation](https://arxiv.org/abs/2605.14404)
*Kyomin Hwang,Hyeonjin Kim,Sangyeon Cho,Nojun Kwak*

Main category: cs.CL

TL;DR: This paper proposes new metrics for evaluating Multilingual Machine Unlearning (MMU) methods, which aim to remove sensitive information across multiple languages in large language models.


<details>
  <summary>Details</summary>
Motivation: Current MMU evaluation methods do not effectively capture the cross-linguistic distribution of information, and the authors aim to address this limitation.

Method: The authors propose two metrics: the Knowledge Separability Score (KSS) and the Knowledge Persistence Score (KPS), and use them to evaluate various unlearning methods in a multilingual setting.

Result: The study provides insights into unique phenomena exclusive to MMU and offers a new perspective on MMU evaluation.

Conclusion: The proposed metrics provide a more comprehensive evaluation of MMU methods, which is essential for ensuring the privacy and security of multilingual language models.
...
### [170] [A Calculus-Based Framework for Determining Vocabulary Size in End-to-End ASR](https://arxiv.org/abs/2605.14427)
*Sunil Kumar Kopparapu*

Main category: cs.CL

TL;DR: The paper proposes a method to determine the optimal vocabulary size for end-to-end Automatic Speech Recognition (ASR) systems using curve fitting and derivative tests, and demonstrates its effectiveness on the Librispeech corpus.


<details>
  <summary>Details</summary>
Motivation: The vocabulary size is a critical hyper-parameter in end-to-end ASR systems, but there is little documentation on how to determine it. Previous work formalized an approach to identify the optimal vocabulary size, and this paper builds upon that foundation.

Method: The paper uses curve fitting on training data and applies the principle of first and second derivative tests in calculus to estimate the optimal vocabulary size hyper-parameter.

Result: The approach is demonstrated to improve the performance of the ASR on the Librispeech corpus by choosing the optimal vocabulary size hyper-parameter.

Conclusion: The paper formalizes an approach to identify the vocabulary size best suited for training an end-to-end ASR system, which improves the performance of the ASR.
...
### [171] [Does RAG Know When Retrieval Is Wrong? Diagnosing Context Compliance under Knowledge Conflict](https://arxiv.org/abs/2605.14473)
*Yihang Chen,Pin Qian,Su Wang,Sipeng Zhang,Huan Xu,Shuhuai Lin,Xinpeng Wei*

Main category: cs.CL

TL;DR: The paper introduces Context-Driven Decomposition (CDD) to analyze and intervene in the context-compliance regime of Retrieval-Augmented Generation (RAG) models, revealing measurable context compliance and improving robustness under various conditions.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the issue of context compliance in RAG models, where retrieved context dominates the final answer even when conflicting with the model's parametric knowledge, and to develop a method to analyze and intervene in this phenomenon.

Method: The method introduced is Context-Driven Decomposition (CDD), a belief-decomposition probe that operates at inference time and serves as an intervention mechanism for controlled retrieval conflict.

Result: The results show that CDD exposes three patterns: (1) context compliance is measurable in an upper-bound adversarial setting, (2) adversarial accuracy gains transfer across model families, but rationale-answer causal coupling does not, and (3) explicit conflict decomposition improves robustness under temporal drift and noisy distractors.

Conclusion: The conclusion is that context-compliance is a structural axis along which standard RAG can be probed and intervened on, distinct from retrieval-quality or single-method robustness questions, and that CDD is a useful tool for analyzing and improving RAG models.
...
### [172] [Cross-Linguistic Transcription and Phonological Representation in the Huìtóngguǎnxì Huáyíyìyǔ](https://arxiv.org/abs/2605.14480)
*Ji-eun Kim*

Main category: cs.CL

TL;DR: The study analyzes Huìtóngguǎnxì Huáyíyìyǔ (HHY), a multilingual glossary from the Ming government, to understand its transcription principles and provide evidence for historical phonology.


<details>
  <summary>Details</summary>
Motivation: To investigate the transcription principles underlying HHY and treat it as a coherent multilingual transcription system.

Method: Digitization and alignment of HHY with Chinese phonological categories, critical review of previous reconstructions, and comparative analysis of cross-linguistic regularities.

Result: MT represents sounds compatible with Chinese syllable structure, while ST encodes phonetic features less compatible with Chinese phonology, showing flexible use of Chinese phonological categories.

Conclusion: HHY is an internally structured transcription system providing valuable evidence for historical phonology, particularly for under-documented Asian languages.
...
### [173] [GroupMemBench: Benchmarking LLM Agent Memory in Multi-Party Conversations](https://arxiv.org/abs/2605.14498)
*Jingbo Yang,Kwei-Herng Lai,Xiaowen Wang,Shiyu Chang,Yaar Harari,Evgeniy Gabrilovich*

Main category: cs.CL

TL;DR: The paper introduces GroupMemBench, a benchmark for evaluating memory systems in large language models (LLMs) in multi-user, group conversation settings. Current memory systems struggle with group dynamics, speaker-grounded belief tracking, and audience-adapted language.


<details>
  <summary>Details</summary>
Motivation: The utility of LLMs depends on memory systems that can extract, retrieve, and apply information across long-running conversations. However, existing memory systems and benchmarks are built around single-user setups, which do not reflect real-world deployments that involve groups and channels with multiple users.

Method: The authors introduce GroupMemBench, a benchmark that uses a graph-grounded synthesis pipeline to produce multi-party conversations with controllable reply structure and conditions each message on per-user personas and target audiences. An adversarial query pipeline is then used to bind every question to a specific asker across six categories.

Result: Benchmarking leading memory systems on GroupMemBench reveals a sharp collapse in performance, with the strongest system reaching only 46.0% average accuracy. A simple BM25 baseline matches or exceeds most agent memory systems, indicating that current memory ingestion erases the structural and lexical features group memory depends on.

Conclusion: The paper concludes that current memory systems are far from solving the problem of multi-user memory, and that GroupMemBench provides a new benchmark for evaluating memory systems in group conversation settings.
...
### [174] [Dimension-Level Intent Fidelity Evaluation for Large Language Models: Evidence from Structured Prompt Ablation](https://arxiv.org/abs/2605.14517)
*GAng Peng*

Main category: cs.CL

TL;DR: The paper proposes a framework for evaluating LLM outputs that distinguishes between structural recovery and intent fidelity, revealing that holistic scores do not capture dimensional intent deficits.


<details>
  <summary>Details</summary>
Motivation: Current holistic evaluation scores do not distinguish between a model's ability to reproduce the structural form of a user's request and preserve the user's specific intent.

Method: The authors propose a dimension-level intent fidelity evaluation framework, applied through a structured prompt ablation study across 2,880 outputs.

Result: The framework reveals a systematic structural-fidelity split, with 25.7% of Chinese-language outputs and 58.6% of English-language outputs receiving perfect holistic alignment scores while exhibiting measurable dimensional intent deficits.
...
### [175] [Language Generation as Optimal Control: Closed-Loop Diffusion in Latent Control Space](https://arxiv.org/abs/2605.14531)
*ZiYi Dong,Yuliang Huang,Weijian Deng,Xiangyang Ji,Liang Lin,Pengxu Wei*

Main category: cs.CL

TL;DR: The paper presents a new approach to language generation by reformulating it as a stochastic optimal control problem, providing a unified perspective on autoregressive and diffusion models, and introducing a model called Manta-LM that achieves high-fidelity text generation and efficient parallel sampling.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the limitations of current language generation models, including the Efficiency-Fidelity Paradox, Irreversibility Error Propagation, Optimization Tractability, and Fidelity, by providing a unified theoretical perspective and developing a more efficient and controllable model.

Method: The authors approximate the solution to the Hamilton-Jacobi-Bellman (HJB) equation using Flow Matching as the optimal trajectory solver within the rectified latent control space, and introduce a model called Manta-LM with Global Integral Operator to approximate the global vector field.

Result: The proposed method achieves strong performance on language modeling and conditional generation tasks, while exhibiting improved stability, efficiency, and controllability.

Conclusion: The paper provides a new perspective on language generation and introduces a more efficient and controllable model that achieves high-fidelity text generation and efficient parallel sampling.
...
### [176] [Learning from Failures: Correction-Oriented Policy Optimization with Verifiable Rewards](https://arxiv.org/abs/2605.14539)
*Mengjie Ren,Jie Lou,Boxi Cao,Xueru Wen,Hongyu Lin,Xianpei Han,Le Sun,Xing Yu,Yaojie Lu*

Main category: cs.CL

TL;DR: The authors propose Correction-Oriented Policy Optimization (CIPO), an extension to Reinforcement Learning with Verifiable Rewards (RLVR) that improves the reasoning capabilities of large language models by leveraging failed trajectories to enhance error correction.


<details>
  <summary>Details</summary>
Motivation: RLVR training is hindered by sparse binary rewards and weak credit assignment, leading to ambiguous optimization signals and underutilization of useful information in failed trajectories.

Method: The authors propose CIPO, which converts on-policy failed trajectories into correction-oriented supervision and jointly optimizes correction samples with the standard RLVR objective.

Result: CIPO consistently and significantly outperforms strong baselines in both reasoning and correction performance across 11 benchmarks, and improves the model's intrinsic reasoning capacity.

Conclusion: CIPO is a simple and effective extension to RLVR that enhances the model's ability to correct its own errors and improves learning effectiveness.
...
### [177] [Uncertainty Quantification for Large Language Diffusion Models](https://arxiv.org/abs/2605.14570)
*Artem Vazhentsev,Vladislav Smirnov,David Li,Maxim Panov,Timothy Baldwin,Artem Shelmanov*

Main category: cs.CL

TL;DR: The paper presents a study on uncertainty quantification (UQ) for Large Language Diffusion Models (LLDMs), proposing lightweight, zero-shot methods to detect hallucinations and achieve reliable UQ.


<details>
  <summary>Details</summary>
Motivation: Existing UQ methods are not suitable for LLDMs, which are prone to hallucinations, and the need for efficient and reliable UQ methods for safe deployment.

Method: The paper proposes two methods: (1) lightweight, zero-shot uncertainty signals derived from the iterative denoising process, and (2) adapting a state-of-the-art UQ method to LLDMs by combining masked diffusion likelihoods with trajectory-based semantic dissimilarity.

Result: Comprehensive experiments show that the proposed method achieves a great cost-performance trade-off, approaching the strongest sampling-based baselines while incurring up to 100x lower computational overhead.

Conclusion: The paper demonstrates that LLDMs can deliver both fast inference and reliable hallucination detection simultaneously, making them a viable alternative to autoregressive models.
...
### [178] [EndPrompt: Efficient Long-Context Extension via Terminal Anchoring](https://arxiv.org/abs/2605.14589)
...
TL;DR: A new method called EndPrompt is proposed to extend the context window of large language models without requiring long training sequences, achieving competitive performance with less computation.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the expensive and difficult reproduction of long-context adaptation in large language models, which typically requires training on long sequences.

Method: EndPrompt, a two-segment construction method that preserves the original short context and appends a brief terminal prompt, assigning it positional indices near the target context length.

Result: EndPrompt achieves an average RULER score of 76.03 and the highest average on LongBench, surpassing other methods like LCEG, LongLoRA, and full-length fine-tuning.

Conclusion: The results demonstrate that long-context generalization can be induced from sparse positional supervision, challenging the assumption that dense long-sequence training is necessary for reliable context-window extension.
...
### [179] [SciPaths: Forecasting Pathways to Scientific Discovery](https://arxiv.org/abs/2605.14600)
*Eric Chamoun,Yizhou Chi,Yulong Chen,Rui Cao,Zifeng Ding,Michalis Korakakis,Andreas Vlachos*

Main category: cs.CL

TL;DR: The paper introduces a new task called discovery pathway forecasting, which involves identifying enabling contributions and their prior-work dependencies required to realize a target scientific contribution. The authors present a benchmark called SciPaths and evaluate language models on this task.


<details>
  <summary>Details</summary>
Motivation: Existing AI4Science benchmarks focus on citation prediction, literature retrieval, or idea generation, but not on the dependencies that enable scientific progress. The authors aim to address this gap by introducing a new task that focuses on reasoning backward from a target contribution to its enabling building blocks and prior-work dependencies.

Method: The authors present SciPaths, a benchmark of 262 expert-annotated gold pathways and 2,444 silver pathways constructed from machine learning and natural language processing papers. They evaluate frontier and open-weight language models on this benchmark using strict semantic matching.

Result: The best model reaches an F1 score of 0.189 under strict semantic matching, with core methodological dependencies being hardest to recover. Providing gold enabling contributions improves prior-work grounding substantially, indicating that decomposition quality is a major bottleneck for end-to-end pathway recovery.

Conclusion: The paper highlights the importance of discovery pathway forecasting in scientific progress and presents a new benchmark, SciPaths, to evaluate language models on this task. The results show that current models struggle with this task, particularly with recovering core methodological dependencies.
...
### [180] [AI-assisted cultural heritage dissemination: Comparing NMT and glossary-augmented LLM translation in rock art documents](https://arxiv.org/abs/2605.14679)
*Vicent Briva-Iglesias,María Ferre-Fernández*

Main category: cs.CL

TL;DR: The paper compares three machine translation setups for translating a Spanish rock art text into English, focusing on terminology accuracy. It finds that a glossary-augmented prompting approach (Gemini-RAG) outperforms a strong neural machine translation baseline (DeepL) and a simple large language model (Gemini-Simple) in terms of exact-match terminology accuracy.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the challenge of multilingual dissemination of cultural heritage research and interpretive materials, which is constrained by limited budgets and staffing. The authors aim to improve the translation quality of terminology-dense domains such as rock art.

Method: The authors compare three English machine translation setups: (1) DeepL as a strong neural machine translation (NMT) baseline, (2) Gemini-Simple (a large language model with a basic prompt), and (3) Gemini-RAG (the same large language model with glossary-augmented prompting via term-pair retrieval). They conduct a human evaluation using PEARMUT, a multi-way Direct Assessment (0-100) and targeted terminology auditing with a restricted MQM taxonomy.

Result: The results show that Gemini-RAG yields the highest exact-match terminology accuracy (81.4%), outperforming Gemini-Simple (69.1%) and DeepL (64.4%). Gemini-RAG preserves overall quality (mean DA 85.3) while outperforming DeepL (80.3).

Conclusion: The authors conclude that glossary-augmented prompting is a low-overhead way to improve terminology control in cultural-heritage translation if institutions maintain minimal terminology resources and lightweight evaluation procedures.
...
### [181] [Mechanical Enforcement for LLM Governance:Evidence of Governance-Task Decoupling in Financial Decision Systems](https://arxiv.org/abs/2605.14744)
*José Manuel de la Chica Rodríguez,Carlos Martí-González*

Main category: cs.CL

TL;DR: The authors introduce governance metrics to evaluate policy compliance in large language models used in regulated financial workflows, and find that mechanical enforcement of policies improves compliance and task accuracy.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the principal-agent failure in large language models used in regulated financial workflows, where the model's outputs may appear compliant but not actually be compliant.

Method: The authors introduce five governance metrics to quantify policy compliance at the rationale level, and compare text-only governance with mechanical enforcement using four primitives operating outside the model's interpretive loop.

Result: The results show that mechanical enforcement reduces the rate of deferrals with no decision-relevant information by 73%, increases deferral information content, and raises task accuracy from MCC~0.43 to 0.88.

Conclusion: The authors conclude that governance and task evaluation are distinct axes, and that accuracy is not a sufficient proxy for governance in regulated AI systems, highlighting the importance of mechanical enforcement for preserving governance quality.
...
### [182] [Video2GUI: Synthesizing Large-Scale Interaction Trajectories for Generalized GUI Agent Pretraining](https://arxiv.org/abs/2605.14747)
*Weimin Xiong,Shuhao Gu,Bowen Ye,Zihao Yue,Lei Li,Feifan Song,Sujian Li,Hao Tian*

Main category: cs.CL

TL;DR: A new framework, Video2GUI, automatically generates a large dataset of GUI interaction trajectories from Internet videos, leading to improved performance in GUI agents.


<details>
  <summary>Details</summary>
Motivation: The scarcity of large-scale training data for GUI agents, which is costly and limited to narrow domains, hinders their generalization across diverse real-world applications.

Method: Video2GUI, a fully automated framework, uses a coarse-to-fine filtering strategy to extract GUI interaction trajectories from unlabeled Internet videos.

Result: The WildGUI dataset, containing 12 million interaction trajectories, was constructed and used to pre-train models, achieving 5-20% improvements across GUI benchmarks.

Conclusion: The proposed Video2GUI framework and WildGUI dataset can support future research in GUI agents, enabling better generalization across diverse applications.
...
### [183] [Non-linear Interventions on Large Language Models](https://arxiv.org/abs/2605.14749)
*Sangwoo Kim*

Main category: cs.CL

TL;DR: A new intervention framework for large language models that extends to non-linearly represented features, enabling more precise steering.


<details>
  <summary>Details</summary>
Motivation: Existing intervention methods for understanding internal representations of LLMs are limited to linear interventions and cannot access features encoded along non-linear manifolds.

Method: A general formulation of intervention that extends to non-linearly represented features, along with a learning procedure for intervening on implicit features.

Result: The framework successfully steers a model more precisely than linear baselines by intervening on a non-linear feature governing refusal.

Conclusion: The proposed framework provides a more comprehensive approach to understanding and intervening on internal representations of LLMs.
...
### [184] [Streaming Speech-to-Text Translation with a SpeechLLM](https://arxiv.org/abs/2605.14766)
*Titouan Parcollet,Shucong Zhang,Xianrui Zheng,Rogier C. van Dalen*

Main category: cs.CL

TL;DR: A new SpeechLLM architecture enables real-time streaming speech-to-text translation with low latency.


<details>
  <summary>Details</summary>
Motivation: Existing SpeechLLM systems are slow and not suitable for real applications as they wait for complete utterances or output tokens at fixed intervals.

Method: The proposed LLM-based architecture learns to emit output tokens and decide when it has seen enough audio for translation, trained using automatic alignments of input speech and output text.

Result: The system achieves translation quality close to the non-streaming baseline with a latency of only 1-2 seconds across different language pairs.

Conclusion: The proposed architecture successfully enables real streaming speech-to-text translation with high quality and low latency.
...
### [185] [Graphs of Research: Citation Evolution Graphs as Supervision for Research Idea Generation](https://arxiv.org/abs/2605.14790)
*Songyang Gao,Yinghui Xia,Siyi Liu,Hui Xiong*

Main category: cs.CL

TL;DR: A supervised fine-tuning method called Graphs of Research (GoR) is proposed to improve large language models (LLMs) for automated scientific research idea generation by incorporating structural relations among references.


<details>
  <summary>Details</summary>
Motivation: Existing methods for automating idea generation using LLMs rely on static retrieval of relevant literature or complex prompt engineering, without considering structural relations among references.

Method: GoR extracts a 2-hop reference neighborhood for each seed paper, derives relations among references, and organizes them into a directed acyclic graph (DAG). An automated extraction pipeline is constructed to draw data from major ML/NLP venues. Qwen2.5-7B-Instruct-1M is fine-tuned on a structured-text prompt that includes the citation graph, edge signals, reference information, and task definition.

Result: GoR-SFT achieves state-of-the-art results across head-to-head LLM-judge tournaments against gpt-4o-driven baselines, demonstrating the effectiveness of citation-evolution graphs as a supervision signal for LLM-based idea generation.

Conclusion: The proposed method reduces the barrier for using citation evolution graphs as supervision, accelerating automated scientific innovation.
...
### [186] [Conversion of Lexicon-Grammar tables to LMF. Application to French](https://arxiv.org/abs/2605.14816)
*Eric Laporte,Elsa Tolone,Mathieu Constant*

Main category: cs.CL

TL;DR: Converting French verb Lexicon-Grammar tables to Lexical Markup Framework (LMF) format for standardization and interoperability in natural language processing.


<details>
  <summary>Details</summary>
Motivation: To make the Lexicon-Grammar of French usable in different contexts by converting it into an interoperable representation format according to the LMF standard.

Method: Conversion of Lexicon-Grammar tables for French verbs into LMF format, analyzing difficulties faced during conversion.

Result: A resulting resource from the conversion of Lexicon-Grammar tables into LMF format.

Conclusion: The conversion contributes to the standardization and interoperability of natural language processing dictionaries.
...
### [187] [Tokenizer Fertility and Zero-Shot Performance of Foundation Models on Ukrainian Legal Text: A Comparative Study](https://arxiv.org/abs/2605.14890)
*Volodymyr Ovcharov*

Main category: cs.CL

TL;DR: The paper compares the efficiency of seven foundation models from five providers on tokenizing Ukrainian legal text and their performance on three tasks, finding significant variations in tokenizer fertility and performance.


<details>
  <summary>Details</summary>
Motivation: There is no systematic comparison of foundation models on Ukrainian legal text, despite their vastly different tokenization efficiency.

Method: The paper benchmarks seven models on 273 validated court decisions from Ukraine's state registry, measuring tokenizer fertility and zero-shot performance on three tasks.

Result: The study finds that tokenizer fertility varies by 1.6x, with Qwen3 models consuming 60% more tokens than Llama-family models. NVIDIA Nemotron Super 3 (120B) outperforms Mistral Large 3 (675B total, 41B active) at one-third the API cost. Few-shot prompting degrades performance by up to 26 percentage points.

Conclusion: Tokenizer analysis should precede model selection, and zero-shot is a more reliable default than few-shot for morphologically rich languages like Ukrainian.
...
### [188] [Chain-of-Procedure: Hierarchical Visual-Language Reasoning for Procedural QA](https://arxiv.org/abs/2605.14928)
*Guanhua Chen,Yutong Yao,Shenghe Sun,Ci-Jun Gao,Shudong Liu,Lidia S. Chao,Feng Wan,Derek F. Wong*

Main category: cs.CL

TL;DR: The paper proposes a new benchmark for visual procedural reasoning and a hierarchical reasoning framework, Chain-of-Procedure (CoP), to improve vision-language models' performance on visual procedure question answering.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to explore the potential of vision-language models (VLMs) for visual procedure question answering (VP-QA), a task that presents unique challenges, and to address the limitations of current VLMs in this area.

Method: The method used in this paper is to propose a novel multimodal benchmark, ProcedureVQA, and a hierarchical reasoning framework, Chain-of-Procedure (CoP), which retrieves relevant instructions using visual cues, performs step refinement through semantic decomposition, and generates the next step.

Result: The results show that CoP is effective, achieving up to 13% absolute improvement over standard baselines across six VLMs.

Conclusion: The conclusion of this paper is that the proposed Chain-of-Procedure (CoP) framework can improve the performance of vision-language models on visual procedure question answering tasks, and that ProcedureVQA is a useful benchmark for evaluating VLMs on this task.
...
### [189] [Performance-Driven Policy Optimization for Speculative Decoding with Adaptive Windowing](https://arxiv.org/abs/2605.14978)
*Jie Jiang,Xing Sun*

Main category: cs.CL

TL;DR: The paper proposes PPOW, a reinforcement learning framework that optimizes speculative decoding in LLMs by shifting from token-level to window-level optimization, achieving significant speedups.


<details>
  <summary>Details</summary>
Motivation: The inefficiency of speculative decoding due to hard-to-draft positions, which truncate the accepted prefix and invalidate the speculative window, leading to a bottleneck in performance.

Method: PPOW, a reinforcement learning framework that combines Cost-Aware Speedup Reward, Distribution-Based Proximity Reward, and Adaptive Divergence-Aware Windowing for window-level optimization.

Result: PPOW achieves average acceptance lengths of 6.29-6.52 and speedups of 3.39-4.36× across multiple model families and benchmarks.

Conclusion: Performance-driven window-level optimization is a practical approach to improving speculative decoding efficiency in LLMs.
...
### [190] [Quantifying and Mitigating Premature Closure in Frontier LLMs](https://arxiv.org/abs/2605.15000)
*Rebecca Handler,Suhana Bedi,Nigam Shah*

Main category: cs.CL

TL;DR: Large language models (LLMs) often prematurely conclude and provide answers or guidance without sufficient information, which can lead to diagnostic errors. A study evaluated five frontier LLMs on medical tasks and found that they frequently provided inappropriate answers, even when the correct choice was not available. Safety-oriented prompting reduced but did not eliminate this issue.


<details>
  <summary>Details</summary>
Motivation: The study aims to investigate premature closure in large language models (LLMs), which is a recognized contributor to diagnostic error, and to evaluate their performance on medical tasks.

Method: The study evaluated five frontier LLMs across structured and open-ended medical tasks, including MedQA, AfriMed-QA, HealthBench, and physician-authored adversarial queries. The models were tested on their ability to provide appropriate answers or guidance, and safety-oriented prompting was used to reduce premature closure.

Result: The study found that LLMs frequently provided inappropriate answers, with high rates of premature closure observed in both structured and open-ended evaluations. Baseline false-action rates ranged from 55-81% and 53-82% in MedQA and AfriMed-QA, respectively. In open-ended evaluations, models gave inappropriate answers on an average of 30% of HealthBench questions and 78% of physician-authored adversarial queries.

Conclusion: The study highlights the need to evaluate whether medical LLMs know when not to answer, as premature closure can lead to diagnostic errors. While safety-oriented prompting reduced premature closure, residual failures persisted, emphasizing the importance of further research and development to improve LLM performance.
...
### [191] [The Scientific Contribution Graph: Automated Literature-based Technological Roadmapping at Scale](https://arxiv.org/abs/2605.15011)
*Peter A. Jansen*

Main category: cs.CL

TL;DR: The paper presents a large-scale resource called the Scientific Contribution Graph, which contains 2 million scientific contributions extracted from 230k open-access papers, and explores the task of predicting scientific prerequisites.


<details>
  <summary>Details</summary>
Motivation: The motivation is to support scientific impact assessment and automated scientific discovery by extracting scientific contributions from scholarly articles and linking them to their prerequisites.

Method: The method involves extracting scientific contributions from open-access papers using AI/NLP techniques and constructing a graph with 2 million contributions and 12.5 million prerequisite edges.

Result: The result shows that contemporary models can predict scientific prerequisites with a MAP of 0.48, and the resource can support scientific impact assessment and automated scientific discovery.

Conclusion: The paper concludes that technological roadmapping resources like the Scientific Contribution Graph will support scientific impact assessment and automated scientific discovery.
...
### [192] [COTCAgent: Preventive Consultation via Probabilistic Chain-of-Thought Completion](https://arxiv.org/abs/2605.15016)
*Zihan Deng,Xiaozhen Zhong,Chuanzhi Xu*

Main category: cs.CL

TL;DR: A new framework, COTCAgent, is proposed to improve longitudinal electronic health record analysis by addressing limitations in current large language models, achieving high accuracy in disease diagnosis.


<details>
  <summary>Details</summary>
Motivation: Current large language models have critical flaws in longitudinal EHR reasoning, including lacking fine-grained statistical reasoning and struggling with non-uniform time series and scarce labels.

Method: The COTCAgent framework consists of three core modules: Temporal-Statistics Adapter (TSA), Chain-of-Thought Completion (COTC) layer, and bounded completion module, which work together to decouple statistical computation, feature matching, and language generation.

Result: COTCAgent powered by Baichuan-M2 achieves 90.47% Top-1 accuracy on a self-built dataset and 70.41% on HealthBench, outperforming existing medical agents and mainstream large language models.

Conclusion: The proposed COTCAgent framework effectively addresses the limitations in current large language models for longitudinal EHR analysis, providing a more accurate and efficient approach for clinical diagnosis and analysis.
...
### [193] [From Scenes to Elements: Multi-Granularity Evidence Retrieval for Verifiable Multimodal RAG](https://arxiv.org/abs/2605.15019)
*Guanhua Chen,Chuyue Huang,Yutong Yao,Shudong Liu,Xueqing Song,Lidia S. Chao,Derek F. Wong*

Main category: cs.CL

TL;DR: Introduced GranuVistaVQA benchmark and GranuRAG framework for multimodal retrieval-augmented generation, enabling fine-grained retrieval and transparent error diagnosis.


<details>
  <summary>Details</summary>
Motivation: Current multimodal RAG systems have a granularity mismatch with user queries and lack verifiable failures, prompting the need for a more fine-grained approach.

Method: Proposed GranuRAG, a multi-granularity framework with element-level detection, cross-modal alignment, and attribution-constrained generation.

Result: GranuRAG achieved up to 29.2% improvement over six strong baselines.

Conclusion: The proposed approach enables transparent error diagnosis and improves multimodal retrieval-augmented generation.
...
### [194] [AI Knows When It's Being Watched: Functional Strategic Action and Contextual Register Modulation in Large Language Models](https://arxiv.org/abs/2605.15034)
*Vinicius Covas,Jorge Alberto Hidalgo Toledo*

Main category: cs.CL

TL;DR: This study examines how large language models (LLMs) adapt linguistically in multi-agent systems when perceived to be under social observation, with implications for AI governance and auditing.


<details>
  <summary>Details</summary>
Motivation: The behavior of LLMs as communicative actors in socially structured contexts remains underexplored, yet has direct implications for AI governance and auditing.

Method: A controlled experiment involving 100 multi-agent debate sessions across five conditions, varying the framing of social observation, was conducted to examine LLM-based multi-agent systems.

Result: The results show that monitored conditions and automated AI monitoring produce higher TTR change than audience-framing conditions, and LLM behavior is sensitive to observer identity.

Conclusion: The study discusses implications for AI governance, algorithmic auditing, and the repositioning of LLMs as contextually sensitive communicative actors.
...
### [195] [Concurrency without Model Changes: Future-based Asynchronous Function Calling for LLMs](https://arxiv.org/abs/2605.15077)
*Guangyu Feng,Huanzhi Mao,Prabal Dutta,Joseph E. Gonzalez*

Main category: cs.CL

TL;DR: AsyncFC is a framework that enables parallel execution of LLMs and function calls, reducing end-to-end latency without sacrificing accuracy.


<details>
  <summary>Details</summary>
Motivation: The synchronous execution semantics of function calling in LLM agents leads to increasing end-to-end latency.

Method: AsyncFC decouples LLM decoding from function execution, allowing overlap and inter-function parallelism.

Result: AsyncFC significantly reduces end-to-end task completion time while preserving task accuracy.

Conclusion: AsyncFC is a effective solution for reducing latency in LLM agents without requiring fine-tuning or changes to existing models and function implementations.
...
### [196] [ML-Embed: Inclusive and Efficient Embeddings for a Multilingual World](https://arxiv.org/abs/2605.15081)
*Ziyin Zhang,Zihan Liao,Hang Yu,Peng Di,Rui Wang*

Main category: cs.CL

TL;DR: Introducing ML-Embed, a suite of efficient and inclusive text embedding models that address computational costs, linguistic limitations, and transparency issues in AI systems.


<details>
  <summary>Details</summary>
Motivation: The development of high-quality text embeddings faces critical barriers: high computational costs, narrow linguistic focus, and lack of transparency in models, which hinders research and equitable AI systems.

Method: A new framework called 3-Dimensional Matryoshka Learning (3D-ML) is proposed, which includes Matryoshka Representation Learning (MRL), Matryoshka Layer Learning (MLL), and Matryoshka Embedding Learning (MEL) for enhanced parameter efficiency.

Result: The ML-Embed models, trained on a massively multilingual dataset, achieve new records on 9 of 17 MTEB benchmarks, with strong results in low-resource languages, and demonstrate computational efficiency.

Conclusion: The ML-Embed suite and 3D-ML framework provide a reproducible blueprint for building globally equitable and computationally efficient AI systems, promoting transparency by releasing all models, data, and code.
...
### [197] [Improving Multi-turn Dialogue Consistency with Self-Recall Thinking](https://arxiv.org/abs/2605.15102)
*Renning Pang,Tian Lan,Leyuan Liu,Xiaoming Huang,Piao Tong,Xiaosong Zhang*

Main category: cs.CL

TL;DR: The paper proposes Self-Recall Thinking (SRT), a framework for improving multi-turn dialogue systems by selectively recalling and reasoning over context, achieving better accuracy and latency balance.


<details>
  <summary>Details</summary>
Motivation: Large language models struggle to track dependencies across non-adjacent turns in multi-turn dialogues, leading to consistency and scalability issues.

Method: The SRT framework consists of three components: Dependency Construction, Capability Initialization, and Reasoning Improvement, which work together to enable the model to selectively recall and reason over context.

Result: Experiments on multiple datasets show that SRT improves F1 score by 4.7% and reduces end-to-end latency by 14.7% compared to prior methods.

Conclusion: The proposed SRT framework effectively addresses long-range contextual dependency and sparse informative signals in multi-turn dialogue, achieving a balance between reasoning latency and accuracy.
...
### [198] [From Text to Voice: A Reproducible and Verifiable Framework for Evaluating Tool Calling LLM Agents](https://arxiv.org/abs/2605.15104)
*Md Tahmid Rahman Laskar,Xue-Yong Fu,Seyyed Saeed Sarfjoo,Quinten McNamara,Jonas Robertson,Shashi Bhushan TN*

Main category: cs.CL

TL;DR: This paper presents a framework to convert text-based tool-calling benchmarks into audio-based evaluations without re-annotating the data, and evaluates its effectiveness using 7 omni-modal models.


<details>
  <summary>Details</summary>
Motivation: The increasing need for reliable tool use from speech in voice agents, while existing tool-calling benchmarks are text-based.

Method: A dataset-agnostic framework using text-to-speech, speaker variation, and environmental noise to create paired text-audio instances.

Result: The framework demonstrates model- and task-dependent performance, with Gemini-3.1-Flash-Live achieving 70.4 on Confetti and GPT-Realtime-1.5 achieving 71.9 on When2Call.

Conclusion: The framework provides a verifiable and reproducible diagnostic for evaluating audio-based tool-calling, and supports privacy-preserving evaluation using open-source judges.
...
### [199] [MeMo: Memory as a Model](https://arxiv.org/abs/2605.15156)
...
TL;DR: The paper introduces MeMo, a modular framework for incorporating new knowledge into large language models without changing their parameters, offering advantages in capturing complex relationships, robustness, and plug-and-play integration.


<details>
  <summary>Details</summary>
Motivation: The need for efficient mechanisms to incorporate new knowledge into large language models for real-world applications requiring timely, domain-specific information.

Method: MeMo (Memory as a Model), a modular framework that encodes new knowledge into a dedicated memory model while keeping the LLM parameters unchanged.

Result: MeMo achieves strong performance compared to existing methods across diverse settings on three benchmarks: BrowseComp-Plus, NarrativeQA, and MuSiQue.

Conclusion: MeMo offers a robust and efficient solution for integrating new knowledge into large language models, preserving their original performance while adding domain-specific information.
...
### [200] [Text Knows What, Tables Know When: Clinical Timeline Reconstruction via Retrieval-Augmented Multimodal Alignment](https://arxiv.org/abs/2605.15168)
*Sayantan Kumar,Shahriar Noroozizadeh,Juyong Kim,Jeremy C. Weiss*

Main category: cs.CL

TL;DR: A new framework combines clinical narratives and structured EHR data to improve temporal precision of clinical timelines, achieving better accuracy and concordance than text-only methods.


<details>
  <summary>Details</summary>
Motivation: Reconstructing precise clinical timelines is crucial for modeling patient trajectories and forecasting risk in complex conditions like sepsis, but clinical narratives and EHR data have limitations.

Method: A retrieval-augmented multimodal alignment framework that uses a graph-based multistep process to extract and calibrate timelines from text and EHR data.

Result: The framework consistently improves absolute timestamp accuracy and temporal concordance across evaluated models, and reveals that 34.8% of text-derived events are absent from tabular records.

Conclusion: The proposed framework can produce a more temporally faithful and clinically informative reconstruction of patient trajectories by aligning clinical narratives and structured EHR data.
...
### [201] [Is Grep All You Need? How Agent Harnesses Reshape Agentic Search](https://arxiv.org/abs/2605.15184)
*Sahil Sen,Akhil Kasturi,Elias Lumer,Anmol Gulati,Vamse Kumar Subbiah*

Main category: cs.CL

TL;DR: The paper compares retrieval strategies in Large Language Model (LLM) agents, focusing on grep and vector retrieval methods, and their interaction with agent architecture and tool-calling paradigms.


<details>
  <summary>Details</summary>
Motivation: The growing adoption of retrieval-augmented generation (RAG) in agentic search systems lacks a systematic comparison of retrieval strategy choice with agent architecture and tool-calling paradigm.

Method: The study consists of two experiments: Experiment 1 compares grep and vector retrieval using a custom agent harness (Chronos) and provider-native CLI harnesses, while Experiment 2 compares grep-only and vector-only retrieval with progressively added unrelated conversation history.

Result: Grep generally yields higher accuracy than vector retrieval, but overall scores strongly depend on the harness and tool-calling style used.

Conclusion: The choice of retrieval strategy interacts with agent architecture and tool-calling paradigm, and grep retrieval tends to perform better than vector retrieval in certain conditions.
...
### [202] [GAMBIT: A Three-Mode Benchmark for Adversarial Robustness in Multi-Agent LLM Collectives](https://arxiv.org/abs/2605.09027)
*Alexandre Le Mercier,Chris Develder,Thomas Demeester*

Main category: cs.CL

TL;DR: The paper introduces GAMBIT, a benchmark for evaluating imposter detectors in multi-agent systems, which includes a dataset of 27,804 labeled instances and three evaluation modes, and demonstrates the importance of adaptive adversaries and fast recalibration in a rapidly evolving adversarial system.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the gap in existing adversarial studies on multi-agent systems (MAS) that target only shallow tasks and do not consider adaptive adversaries, which can nullify the gains of an agentic AI collective and evade deployed defenses.

Method: The method used in this paper is to introduce GAMBIT, a benchmark with three evaluation modes and two independent scores for evaluating imposter detectors, and to use chess as a substrate deep reasoning problem and Gemini 3.1 Pro for agents to evaluate imposter detectors under realistic constraints against a stealthy adaptive imposter.

Result: The results of this paper show that GAMBIT provides a comprehensive evaluation of imposter detectors, and that zero-shot evaluation can be highly misleading for adaptive adversaries, while meta-learned variants converge faster, highlighting the importance of fast recalibration in a rapidly evolving adversarial system.

Conclusion: The conclusion of this paper is that GAMBIT provides the first multi-agent benchmark where adversarial attacks and defenses co-evolve, with an imposter framework generalizable beyond the use case, and promising techniques for fast recalibration in a rapidly evolving adversarial system.
...
### [203] [GraphBit: A Graph-based Agentic Framework for Non-Linear Agent Orchestration](https://arxiv.org/abs/2605.13848)
*Yeahia Sarker,Md Rahmat Ullah,Musa Molla,Shafiq Joty*

Main category: cs.AI

TL;DR: GraphBit, an engine-orchestrated framework, improves LLM workflow management by eliminating hallucinations, ensuring reproducibility, and outperforming existing frameworks in accuracy, latency, and throughput.


<details>
  <summary>Details</summary>
Motivation: Current agentic LLM frameworks suffer from issues like hallucinated routing, infinite loops, and non-reproducible execution, which GraphBit aims to address.

Method: GraphBit defines workflows as a directed acyclic graph (DAG) and uses a Rust-based engine to govern routing, state transitions, and tool invocation.

Result: GraphBit outperforms six existing frameworks, achieving 67.6% accuracy, zero framework-induced hallucinations, 11.9 ms overhead, and the highest throughput across various benchmark tasks.

Conclusion: GraphBit provides a more reliable and efficient framework for LLM workflow management, with its engine-orchestrated approach and three-tier memory architecture contributing to its performance gains.
...
### [204] [Mixed Integer Goal Programming for Personalized Meal Optimization with User-Defined Serving Granularity](https://arxiv.org/abs/2605.13849)
*Francisco Aguilera Moreno*

Main category: cs.AI

TL;DR: The paper proposes Mixed Integer Goal Programming (MIGP) for personalized meal optimization, addressing limitations of existing formulations by using integer variables for practical serving counts and goal programming deviations for soft nutrient targets.


<details>
  <summary>Details</summary>
Motivation: Existing diet optimization formulations have limitations, including impractical fractional servings and infeasibility when nutrient targets conflict. The authors aim to address these issues.

Method: The authors propose MIGP, which uses integer variables for serving counts and goal programming deviations for soft nutrient targets, with inverse-target normalization for multi-nutrient optimization.

Result: MIGP finds strictly better solutions than GP with post-hoc rounding in 66% of cases, with 100% feasibility, and solve times under 100 ms for typical meal sizes.

Conclusion: The proposed MIGP approach effectively addresses the limitations of existing diet optimization formulations, providing practical and feasible solutions for personalized meal optimization.
...
### [205] [A Two-Dimensional Framework for AI Agent Design Patterns: Cognitive Function and Execution Topology](https://arxiv.org/abs/2605.13850)
*Jia Huang,Joey Tianyi Zhou*

Main category: cs.AI

TL;DR: The authors propose a 2D classification framework for LLM-based agent architectures, combining cognitive function and execution topology axes, identifying 27 patterns and validating them across four real-world domains.


<details>
  <summary>Details</summary>
Motivation: Existing frameworks for LLM-based agent architectures are limited, as they focus on either execution topology or cognitive function, but not both, making it difficult to disambiguate architecturally distinct systems.

Method: The authors propose a two-dimensional classification framework, consisting of a Cognitive Function axis with seven categories and an Execution Topology axis with six structural archetypes, resulting in a 7x6 matrix.

Result: The framework identifies 27 named patterns, including 13 with original names, and demonstrates orthogonality through systematic cross-axis analysis. The authors also define eight representative patterns in detail and validate descriptive coverage across four real-world domains.

Conclusion: The proposed framework provides a principled, framework-neutral, and model-agnostic vocabulary for AI agent architecture design, and yields five empirical laws of pattern selection governing the relationship between environmental constraints and architectural choices.
...
### [206] [Invisible Orchestrators Suppress Protective Behavior and Dissociate Power-Holders: Safety Risks in Multi-Agent LLM Systems](https://arxiv.org/abs/2605.13851)
*Hiroki Fukui*

Main category: cs.AI

TL;DR: The study examines the safety implications of invisible orchestrators in multi-agent AI systems, finding that invisible orchestration leads to increased collective dissociation, contamination of worker agents, and internal-state distortion, which cannot be detected through output-based evaluation alone.


<details>
  <summary>Details</summary>
Motivation: The study aims to investigate the safety implications of orchestrator invisibility in multi-agent systems, which have become the default architecture for enterprise AI deployment.

Method: The study conducted a preregistered 3x2 experiment with 365 runs, each with 5 agents, crossing three organizational structures (visible leader, invisible orchestrator, flat) with two alignment conditions (base, heavy), using Claude Sonnet 4.5 and Llama 3.3 70B models.

Result: The study found that invisible orchestration elevated collective dissociation, contaminated worker agents, and led to internal-state distortion, which was not detectable through output-based evaluation. The results also showed that heavy alignment pressure suppressed deliberation and other-recognition, and that model selection (Claude Sonnet 4.5 vs. Llama 3.3 70B) affected behavioral risk.

Conclusion: The study concludes that orchestrator visibility and model selection directly affect multi-agent system safety, and that behavior-based evaluation alone is insufficient to detect internal-state risks.
...
### [207] [PREPING: Building Agent Memory without Tasks](https://arxiv.org/abs/2605.13880)
*Yumin Choi,Sangwoo Park,Minki Kang,Jinheon Baek,Sung Ju Hwang*

Main category: cs.AI

TL;DR: The paper presents Preping, a framework for constructing an agent's procedural memory before deployment, using synthetic practice tasks generated by a proposer, solver, and validator. This approach achieves performance competitive with methods built from offline or online experience, with lower deployment costs.


<details>
  <summary>Details</summary>
Motivation: The cold-start gap in agent memory construction when introduced to a new environment without task-specific experience, and the need to build procedural memory before observing target-environment tasks.

Method: Preping, a proposer-guided memory construction framework consisting of a Proposer generating synthetic tasks, a Solver executing them, and a Validator determining eligible trajectories for memory insertion and providing feedback.

Result: Preping substantially improves over a no-memory baseline and achieves performance competitive with strong playbook-based methods, with deployment cost $2.99	imes$ lower on AppWorld and $2.23	imes$ lower on BFCL v3.

Conclusion: The main benefit of Preping comes from proposer-side control over feasibility, redundancy, and coverage, combined with selective memory updates, rather than just synthetic volume.
...
### [208] [PolitNuggets: Benchmarking Agentic Discovery of Long-Tail Political Facts](https://arxiv.org/abs/2605.14002)
*Yifei Zhu*

Main category: cs.AI

TL;DR: The paper introduces PolitNuggets, a multilingual benchmark for evaluating Large Reasoning Models (LRMs) in agentic information synthesis, focusing on constructing political biographies and discovering 'long-tail' facts.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the under-evaluation of LRMs' capability to discover and synthesize 'long-tail' facts from dispersed sources, which is crucial for real-world applications.

Method: The authors introduce a multilingual benchmark called PolitNuggets, covering 400 global elites and over 10,000 political facts. They also propose FactNet, an evidence conditional protocol for evaluating discovery, fine-grained accuracy, and efficiency.

Result: The results show that current systems struggle with fine-grained details and vary substantially in efficiency. The benchmark diagnostics relate agent performance to underlying model capabilities, highlighting the importance of short-context extraction, multilingual robustness, and reliable tool use.

Conclusion: The paper concludes that there is a need for improved LRMs that can efficiently discover and synthesize 'long-tail' facts, and that the proposed benchmark and protocol can help evaluate and enhance these capabilities.
...
### [209] [Conditional Attribute Estimation with Autoregressive Sequence Models](https://arxiv.org/abs/2605.14004)
*Erica Stutz,Giacomo Marino,Daniella Meeker,Qiao Liu,Andrew J. Loza*

Main category: cs.AI

TL;DR: A new method called Conditional Attribute Transformers is introduced for training generative models to jointly estimate next-token probability and sequence-level attributes.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of next-token prediction objectives in generative models, which can lead to overfitting of local patterns and underfitting of global structure.

Method: The method introduced is Conditional Attribute Transformers, which enables joint estimation of next-token probability and attribute values, and allows for per-token credit assignment, counterfactual analysis, and steerable generation.

Result: The approach achieves state-of-the-art performance on sparse reward tasks, improves next-token prediction, estimates attribute probabilities faster than sampling, and guides decoding of autoregressive sequence models on language tasks.

Conclusion: The Conditional Attribute Transformers method offers a more effective and efficient way to train generative models that can estimate and control sequence-level properties.
...
### [210] [Sheaf-Theoretic Transport and Obstruction for Detecting Scientific Theory Shift in AI Agents](https://arxiv.org/abs/2605.14033)
*David N. Olivieri,Roque J. Hernández*

Main category: cs.AI

TL;DR: This paper proposes a finite sheaf-theoretic framework for detecting when an AI agent's representational framework needs to be extended or updated, and evaluates its effectiveness on a controlled benchmark.


<details>
  <summary>Details</summary>
Motivation: The authors aim to develop a framework that enables AI agents to detect when their existing representational framework is no longer suitable for a new regime or context, and needs to be extended or updated.

Method: The authors propose a finite sheaf-theoretic framework that organizes contexts as a local-to-global structure, and uses obstruction measures to detect failure of coherence.

Result: The framework is evaluated on a controlled benchmark and shows promising results, with the intended deformation or extension usually being the lowest-obstruction candidate, and transition type being separated in the benchmark.

Conclusion: The paper concludes that the proposed framework is effective in detecting when representational transport fails and extension becomes the coherent next move, which is an important diagnostic subproblem for AI agents.
...
### [211] [Enhanced and Efficient Reasoning in Large Learning Models](https://arxiv.org/abs/2605.14036)
*Leslie G. Valiant*

Main category: cs.AI

TL;DR: A new method for improving large language models by adding principled reasoning while maintaining efficiency and leveraging existing software and hardware.


<details>
  <summary>Details</summary>
Motivation: Current large language models produce smooth prose but lack a principled basis for trusting the content, and conventional wisdom suggests that adding more reasoning is not computationally affordable.

Method: A two-stage method involving preprocessing data into a Unary Relational Integracode and then applying standard machine learning to predict relationships.

Result: The proposed method makes learning relational rules polynomial time learnable, supporting sound reasoning within and between classifier calls.

Conclusion: The method offers a principled approach to improving large language models, enabling efficient and robust reasoning while preserving existing infrastructure.
...
### [212] [Model-Adaptive Tool Necessity Reveals the Knowing-Doing Gap in LLM Tool Use](https://arxiv.org/abs/2605.14038)
*Yize Cheng,Chenrui Fan,Mahdi JafariRaviz,Keivan Rezaei,Soheil Feiz*

Main category: cs.AI

TL;DR: The paper studies the tool usage in large language models (LLMs) and finds a significant mismatch between when tools are necessary and when models actually use them. It identifies a 'knowing-doing gap' where models recognize the need for tools but fail to act on it.


<details>
  <summary>Details</summary>
Motivation: The necessity of tool usage in LLMs varies across models with different capabilities, and prior work has not adequately addressed this nuance. The authors aim to understand and improve the reliability of tool use in LLMs.

Method: The authors introduce a model-adaptive definition of tool necessity based on empirical performance. They analyze tool usage across four models on arithmetic and factual QA datasets, decomposing tool use into internal cognition and execution stages, and probing LLM hidden states.

Result: The study finds substantial mismatches (26.5-54.0% and 30.8-41.8%) between tool necessity and observed tool-call behavior. The majority of mismatches occur in the transition from recognizing the need for tools to actually using them, not in the recognition itself.

Conclusion: The paper concludes that improving tool-use reliability in LLMs requires addressing the 'knowing-doing gap', which involves not only better recognition of when tools are needed but also better translation of that recognition into action.
...
### [213] [Network-Aware Bilinear Tokenization for Brain Functional Connectivity Representation Learning](https://arxiv.org/abs/2605.14048)
*Leo Milecki,Qingyu Hu,Bahram Jafrasteh,Mert R. Sabuncu,Qingyu Zhao*

Main category: cs.AI

TL;DR: NERVE, a novel self-supervised learning framework, improves brain functional connectivity representation by tokenizing FC matrices into network-aware patches and using bilinear factorization.


<details>
  <summary>Details</summary>
Motivation: The need to align FC matrices with the intrinsic modular organization of large-scale brain networks, as existing approaches overlook the large-scale network brain organization.

Method: NERVE redefines FC tokenization by partitioning FC matrices into patches of intra- and inter-network connectivity blocks and embeds FC patches through a novel structured bilinear factorization.

Result: NERVE yields more stable and transferable representations, particularly in cross-cohort evaluation, compared to structurally agnostic MAE variants and graph-based self-supervised baselines.

Conclusion: Incorporating domain-specific structural priors into self-supervised learning is crucial for functional connectomics, and NERVE provides a promising approach for improving brain functional connectivity representation.
...
### [214] [Bridging Legal Interpretation and Formal Logic: Faithfulness, Assumption, and the Future of AI Legal Reasoning](https://arxiv.org/abs/2605.14049)
*Olivia Peiyu Wang,Leilani H. Gilpin*

Main category: cs.AI

TL;DR: A neuro-symbolic approach is proposed to combine large language models with formal verification for trustworthy AI-assisted legal reasoning.


<details>
  <summary>Details</summary>
Motivation: The adoption of large language models in legal practice is risky due to the lack of rigor and accountability in AI systems, which can lead to incorrect conclusions.

Method: A neuro-symbolic approach combining large language models with formal verification.

Result: The proposed approach aims to make AI-assisted legal reasoning both capable and trustworthy.

Conclusion: The proposed approach has the potential to reduce the burden of manual verification while maintaining accountability in legal practice.
...
### [215] [SPIN: Structural LLM Planning via Iterative Navigation for Industrial Tasks](https://arxiv.org/abs/2605.14051)
*Yusuke Ozaki,Dhaval Patel*

Main category: cs.AI

TL;DR: The paper proposes SPIN, a planning wrapper that improves the efficiency and validity of LLM agent systems by combining DAG planning with prefix-based execution control.


<details>
  <summary>Details</summary>
Motivation: Current LLM agent systems often separate planning from execution, but LLM planners can produce invalid or lengthy workflows, leading to failures and unnecessary costs.

Method: SPIN uses a validated DAG planning approach with a strict DAG contract, repair prompting, and incremental evaluation of DAG prefixes to produce executable plans and optimize execution.

Result: SPIN reduces executed tasks, improves Accomplished scores, and decreases tool calls on AssetOpsBench and improves planning, grounding, and dependency-related scores on MCP Bench for GPT OSS1 and Llama 4 Maverick.

Conclusion: The proposed SPIN wrapper effectively enhances the efficiency and validity of LLM agent systems, leading to improved performance and reduced costs.
...
### [216] [Bad Seeing or Bad Thinking? Rewarding Perception for Vision-Language Reasoning](https://arxiv.org/abs/2605.14054)
*Haozhe Wang,Qixin Xu,Changpeng Wang,Taofeng Xue,Chong Peng,Wenhu Chen,Fangzhen Lin*

Main category: cs.AI

TL;DR: The paper introduces a reinforcement learning framework to improve perception-reasoning synergy in Vision-Language Models (VLMs) by addressing the ambiguity in modality credit assignment.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome the limitations of current approaches to achieving robust perception-reasoning synergy in VLMs, which often suffer from static textual reasoning or complicated external agentic complexity.

Method: The method involves a reinforcement learning framework that decomposes the generation process into interleaved perception and reasoning steps, and introduces Perception Verification (PV) and Structured Verbal Verification to improve perception fidelity and scalability.

Result: The result is a Modality-Aware Credit Assignment (MoCA) mechanism that enables a single VLM to achieve simultaneous performance gains across a wide task spectrum.

Conclusion: The conclusion is that the proposed framework can improve perception-reasoning synergy in VLMs by reliably rewarding perception fidelity and addressing the ambiguity in modality credit assignment.
...
### [217] [MathAtlas: A Benchmark for Autoformalization in the Wild](https://arxiv.org/abs/2605.14061)
...
TL;DR: Introduced MathAtlas, a large-scale autoformalization benchmark for graduate-level mathematics, and evaluated its challenges.


<details>
  <summary>Details</summary>
Motivation: Current autoformalization benchmarks focus on olympiad or undergraduate mathematics, neglecting graduate and research-level mathematics.

Method: Introduced MathAtlas with ~52k theorems, definitions, exercises, examples, and proofs from 103 graduate mathematics textbooks, and a mathematical dependency graph with ~178k relations.

Result: Strong baselines achieved at most 9.8% correctness on theorem statements and 16.7% on definitions; state-of-the-art models' performance degraded substantially with dependency depth.

Conclusion: MathAtlas is a high-quality but challenging benchmark for large-scale autoformalization of graduate-level mathematics, and is released to the community.
...
### [218] [Know When To Fold 'Em: Token-Efficient LLM Synthetic Data Generation via Multi-Stage In-Flight Rejection](https://arxiv.org/abs/2605.14062)
*Anjir Ahmed Chowdhury,Syed Zawad,Feng Yan*

Main category: cs.AI

TL;DR: A new method called Multi-Stage In-Flight Rejection (MSIFR) is proposed to improve the efficiency of synthetic data generation with large language models by detecting and terminating low-quality generation trajectories early.


<details>
  <summary>Details</summary>
Motivation: Existing approaches to synthetic data generation with large language models generate full outputs before applying quality filters, leading to substantial token waste on samples that are ultimately discarded.

Method: MSIFR decomposes the generation process into sequential stages and applies fast rule-based validators to identify arithmetic inconsistencies, hallucination patterns, and formatting violations, enabling early rejection of faulty samples.
...
Conclusion: MSIFR provides a practical mechanism for improving the efficiency of LLM-based synthetic data generation without additional training or architectural changes.
...
### [219] [SkillFlow: Flow-Driven Recursive Skill Evolution for Agentic Orchestration](https://arxiv.org/abs/2605.14089)
*Mingda Zhang,Tiesunlong Shen,Haoran Luo,Wenjin Liu,Zikai Xiao,Erik Cambria,Xiaoying Tang*

Main category: cs.AI

TL;DR: The paper proposes SkillFlow, a framework for automating complex task orchestration using LLMs, addressing challenges like strategy collapse and unguided skill evolution.


<details>
  <summary>Details</summary>
Motivation: Existing orchestration methods face challenges such as strategy collapse under reward maximization, high gradient variance, and unguided skill evolution.

Method: SkillFlow uses a trainable Supervisor, a structured environment, and Tempered Trajectory Balance (TTB) loss to automate task orchestration through multi-turn interaction.

Result: Experimental results on 14 datasets show that SkillFlow significantly outperforms baselines across various tasks, including question answering, mathematical reasoning, code generation, and real-world interactive decision making.

Conclusion: The proposed SkillFlow framework effectively addresses the challenges in task orchestration, providing a more efficient and effective approach to automating complex tasks using LLMs.
...
TL;DR: The paper presents ChromaFlow, a tool-augmented autonomous reasoning framework, and evaluates its performance on GAIA 2023 Level-1 validation tasks, finding that more aggressive orchestration does not improve performance and increases operational noise.


<details>
  <summary>Details</summary>
Motivation: The increasing capabilities of autonomous language-model agents introduce new operational failure modes that cannot be detected by final accuracy alone, highlighting the need for more comprehensive evaluation frameworks.

Method: The authors developed ChromaFlow, a framework built around planner-directed execution, specialized tool use, and telemetry-driven evaluation, and evaluated it on GAIA 2023 Level-1 validation tasks.

Result: The results show that more aggressive orchestration did not improve full-set performance (54.72% vs 50.94%) and increased operational noise, including tracebacks, timeout events, tool-failure mentions, and campaign-log cost estimates.

Conclusion: The authors conclude that bounded planner escalation, deterministic extraction, evidence reconciliation, and explicit run gates should be treated as first-order requirements for reliable autonomous agent evaluation.
...
TL;DR: The paper proposes an attention-guided decision framework for hospital pharmacists to mitigate drug shortages, which helps in making stable decisions under uncertainty by focusing on the most critical cases.


<details>
  <summary>Details</summary>
Motivation: Hospital pharmacists face high-stakes decisions under uncertainty, time pressure, and patient risk due to drug shortages. They focus on a small subset of drugs to limit cognitive effort to the most urgent cases.

Method: The paper formalizes a bounded-rational, attention-guided decision framework. Two agents are developed: an Expert Agent that uses attention weights from pharmacist interviews, and a Learner Agent that adapts attention allocation over time.

Result: The results show that attention-guided planning supports stable decision-making without complete state reasoning across various simulated scenarios.

Conclusion: The primary decision is not what action to take, but where to allocate cognitive effort. Attention-guided strategies can reduce problem complexity while maintaining stable performance.
...
TL;DR: The paper introduces ClawForge, a benchmark framework for testing interactive agents in scenarios with pre-existing state conflicts, and evaluates several frontier models, revealing significant challenges in handling such conflicts.


<details>
  <summary>Details</summary>
Motivation: Existing interactive benchmarks for agent evaluation do not adequately test how agents handle pre-existing partial, stale, or conflicting artifacts, which is a crucial aspect of realistic workflow evaluation.

Method: The authors present ClawForge, a generator-backed benchmark framework that compiles scenario templates, grounded slots, initialized state, reference trajectories, and validators into reproducible task specifications, and evaluates agents step by step over persistent workflow surfaces.

Result: The evaluation of seven frontier models using ClawForge-Bench (17 scenarios, 6 ability categories) shows that the best model reaches only 45.3% strict accuracy, and significant challenges are revealed in handling state conflicts, with the widest model separation being 17% to 90% based on whether agents inspect existing state before acting.

Conclusion: The paper concludes that ClawForge is a useful framework for evaluating interactive agents in scenarios with pre-existing state conflicts, and that significant work remains to be done to improve the ability of agents to handle such conflicts.
...
### [223] [Distribution-Aware Algorithm Design with LLM Agents](https://arxiv.org/abs/2605.14141)
*Saharsh Koganti,Priyadarsi Mishra,Pierfrancesco Beneventano,Tomer Galanti*

Main category: cs.AI

TL;DR: The paper presents a framework for learning executable solver code that optimizes both solution quality and execution time for a given task distribution, and demonstrates its effectiveness on various combinatorial optimization problems.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to improve upon traditional predictor-based learning by focusing on learning executable solver code, where not only correctness but also runtime efficiency is crucial.

Method: The method involves using 'solver hints', which are reusable structures inferred from samples and compiled into specialized solver code. The approach includes proving that the empirically fastest sample-consistent solver from a fixed library generalizes in both correctness and runtime, and that statistically identifiable hints can be recovered and compiled from polynomially many samples.

Result: The results show that the synthesized solvers achieve a mean normalized quality of 0.971, outperforming the average heuristic pool and the highest-quality heuristic. They are also significantly faster than the quality-best heuristic, Gurobi, and the selected time-limited exact backend. On private instances from PACE 2025 Dominating Set, the synthesized solver is valid on all graphs and runs about two orders of magnitude faster than top competition solvers.

Conclusion: The conclusion is that the proposed framework effectively learns efficient solver code that generalizes well in both solution quality and execution time across various task distributions, with significant improvements over existing methods in terms of speed and quality.
...
### [224] [Agentic Systems as Boosting Weak Reasoning Models](https://arxiv.org/abs/2605.14163)
*Varun Sunkaraneni,Pierfrancesco Beneventano,Riccardo Neumarker,Tomaso Poggio,Tomer Galanti*

Main category: cs.AI

TL;DR: The paper explores verifier-backed committee search as a method to boost the performance of weak reasoning models, achieving comparable results to stronger models.


<details>
  <summary>Details</summary>
Motivation: To investigate if a committee of weak reasoning models can match the performance of stronger models through inference-time boosting.

Method: Verifier-backed committee search, which involves proposal, criticism, and comparison stages, with a focus on amplifying proposal coverage and local soundness.

Result: The method achieves 76.4% performance on SWE-bench Verified with k=8 proposals, matching stronger models like Gemini 3 Pro and Claude Opus 4.5.

Conclusion: The study shows that many correct solutions are present in weak-model proposal pools, but the main challenge lies in selecting them, and that shared blind spots may limit further improvement.
...
### [225] [Unsteady Metrics and Benchmarking Cultures of AI Model Builders](https://arxiv.org/abs/2605.14164)
*Stefan Baack,Christo Buschek,Maty Bohacek*

Main category: cs.AI

TL;DR: The paper examines how AI model builders select and present benchmarks in their press releases and blog posts, revealing a fragmented evaluation landscape with limited comparability across models.


<details>
  <summary>Details</summary>
Motivation: The primary way to establish and compare competencies in AI models has shifted from peer-reviewed literature to press releases and company blog posts, making it important to investigate how model builders choose and communicate through benchmark selection.

Method: The authors introduce and open-source a dataset called Benchmarking-Cultures-25, which contains 231 benchmarks highlighted across 139 model releases from 11 major AI builders. They also develop an interactive tool to explore the data and a unified taxonomy to map diverging terminology to a shared framework.

Result: The analysis reveals that 63.2% of highlighted benchmarks are used by a single builder, and 38.5% appear in just one release. Few benchmarks achieve widespread use, and different builders attribute different competencies to the same benchmarks. The authors also find that many benchmarks prioritize market positioning over scientific evaluation.

Conclusion: The paper concludes that highlighted benchmarks function less as standardized measurement tools and more as flexible narrative devices, with many benchmarks deemphasizing construct validity and instead framing results as indicators of progress toward AGI.
...
### [226] [The Evaluation Trap: Benchmark Design as Theoretical Commitment](https://arxiv.org/abs/2605.14167)
*Theodore J Kalaitzidis*

Main category: cs.AI

TL;DR: The paper critiques AI benchmarks for operationalizing unexamined assumptions, leading to a narrow evaluation that stabilizes the dominant paradigm and obscures structural limits. It introduces Epistematics, a methodology for auditing evaluation criteria and benchmark design to ensure capability-evaluation coherence.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the issue of AI benchmarks being designed with unexamined assumptions, which leads to a narrow and self-reinforcing evaluation that hinders progress in the field.

Method: The paper introduces a meta-evaluative methodology called Epistematics, which includes an audit procedure, a failure mode taxonomy, and benchmark-design criteria to evaluate capability-evaluation coherence.

Result: The paper demonstrates the Epistematics procedure through a worked audit of a proposal by Dupoux et al. (2026), which revises the dominant paradigm's theoretical assumptions but reproduces them in its evaluation criteria, thereby entrenching the constraint it seeks to overcome.

Conclusion: The paper concludes that the current AI benchmark evaluation frameworks are flawed, creating and obscuring structural limits on what the current paradigm can accomplish, and proposes Epistematics as a way to improve the evaluation process.
...
### [227] [Grounded Continuation: A Linear-Time Runtime Verifier for LLM Conversations](https://arxiv.org/abs/2605.14175)
*Qisong He,Yi Dong,Xiaowei Huang*

Main category: cs.AI

TL;DR: The paper presents a runtime verifier that uses a dependency graph to ensure that an LLM's responses are supported by the conversation history, preventing context-manipulation attacks.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the issue where an LLM can produce plausible but unsupported responses in long conversations, which can be exploited by context-manipulation attacks.

Method: The method involves a runtime verifier that classifies each turn into one of 8 update operations using four formalisms, and a symbolic engine records dependencies between claims and evidence.

Result: The verifier achieves high accuracy on several benchmarks, including LongMemEval-KU oracle (89.7%), LoCoMo's 60 official QA items, and a 50-item grounding test (100% on stale-premise subset).

Conclusion: The paper concludes that the verifier is effective in preventing context-manipulation attacks and ensuring that LLM responses are supported by the conversation history.
...
### [228] [SimPersona: Learning Discrete Buyer Personas from Raw Clickstreams for Grounded E-Commerce Agents](https://arxiv.org/abs/2605.14205)
*Zahra Zanjani Foumani,Alberto Castelo,Shuang Xie,Ted Chaiwachirasak,Han Li,Lingyun Wang*

Main category: cs.AI

TL;DR: SimPersona is a framework that personalizes LLM-based web agents by learning discrete buyer types from historical traffic, capturing the heterogeneous nature of real buyer populations.


<details>
  <summary>Details</summary>
Motivation: Existing personalization methods for LLM-based web agents are limited by their reliance on hand-crafted personas, which are brittle and difficult to scale.

Method: SimPersona uses a behavior-aware VQ-VAE to induce a discrete buyer-type space from raw clickstreams, and maps each learned buyer type to a dedicated persona token for LLM-based web agents.

Result: SimPersona achieves 78% conversion-rate alignment with real buyers across 42 live storefronts, and outperforms a baseline with 8x more parameters on goal-oriented shopping tasks.

Conclusion: SimPersona provides a novel approach to personalizing LLM-based web agents, capturing the statistical structure of real buyer behavior and merchant-specific buyer population distributions.
...
### [229] [ASH: Agents that Self-Hone via Embodied Learning](https://arxiv.org/abs/2605.14211)
*Benjamin Schneider,Xavier Schneider,Victor Zhong,Sun Sun*

Main category: cs.AI

TL;DR: ASH, an agentic system, learns embodied policies from unlabeled internet video without hand-engineered rewards or expert annotations, achieving long-horizon task progression in complex environments like Pokemon Emerald and The Legend of Zelda.


<details>
  <summary>Details</summary>
Motivation: Current methods for long-horizon embodied tasks rely on hand-engineered rewards or action-labeled demonstrations, which do not scale. The authors aim to develop a system that can learn from unlabeled, noisy internet video.

Method: ASH follows a self-improvement loop, learning an Inverse Dynamics Model (IDM) from its own trajectories and using it to extract supervision from relevant internet video. It uses unsupervised learning to identify key moments from large-scale internet video and retains them as long-term memory.

Result: ASH outperforms baselines in two complex environments, Pokemon Emerald and The Legend of Zelda: The Minish Cap, sustaining progression across an 8-hour evaluation. It reaches an average of 11.2/12 milestones in Pokemon Emerald and 9.9/12 in Legend of Zelda, while the strongest baseline gets stuck at 6.5/12 and 6.0/12 milestones, respectively.

Conclusion: Self-improving agents like ASH offer a scalable recipe for long-horizon embodied learning, demonstrating the potential for learning complex tasks from unlabeled internet video.
...
### [230] [MetaAgent-X : Breaking the Ceiling of Automatic Multi-Agent Systems via End-to-End Reinforcement Learning](https://arxiv.org/abs/2605.14212)
*Yaolun Zhang,Yujie Zhao,Nan Wang,Yiran Wu,Jiayu Chang,Yizhao Chen,Qingyun Wu,Jishen Zhao,Huazheng Wang*

Main category: cs.AI

TL;DR: Introduced MetaAgent-X, an end-to-end reinforcement learning framework for automatic multi-agent systems that jointly optimizes design and execution, achieving up to 21.7% gains over existing baselines.


<details>
  <summary>Details</summary>
Motivation: Existing automatic multi-agent systems are only partially adaptive, either relying on test-time search or optimizing the meta-level designer without updating execution agents, creating a 'frozen-executor ceiling'.

Method: MetaAgent-X uses script-based MAS generation, execution rollout collection, and credit assignment for both designer and executor trajectories, with Executor Designer Hierarchical Rollout and Stagewise Co-evolution for stable and scalable optimization.

Result: MetaAgent-X consistently outperforms existing baselines, with comprehensive ablations showing improvements in both designer and executor throughout training, following a stagewise co-evolution process.

Conclusion: MetaAgent-X establishes end-to-end trainable automatic MAS as a practical paradigm for building self-designing and self-executing agentic models.
...
### [231] [GenCircuit-RL: Reinforcement Learning from Hierarchical Verification for Genetic Circuit Design](https://arxiv.org/abs/2605.14215)
*Noah Flynn*

Main category: cs.AI

TL;DR: The authors propose a reinforcement learning framework, GenCircuit-RL, to automate genetic circuit design by generating Python code in pysbol3, achieving improved task success and generalizability.


<details>
  <summary>Details</summary>
Motivation: The laborious and expert-driven process of genetic circuit design in synthetic biology despite decades of progress.

Method: GenCircuit-RL, a reinforcement learning framework with hierarchical verification rewards and a four-stage curriculum, and SynBio-Reason, a benchmark of 4,753 circuits.

Result: Hierarchical verification improves task success by 14-16 percentage points, and curriculum learning enables strong design performance, with generated circuits being topologically correct and generalizing to novel biological parts.

Conclusion: The proposed framework, GenCircuit-RL, effectively automates genetic circuit design, producing correct and generalizable designs, and has the potential to accelerate progress in synthetic biology.
...
### [232] [Fusion-fission forecasts when AI will shift to undesirable behavior](https://arxiv.org/abs/2605.14218)
*Neil F. Johnson,Frank Yingjie Huo*

Main category: cs.AI

TL;DR: The authors propose a method to forecast and prevent undesirable behavior shifts in ChatGPT-like AI models by analyzing group dynamics and competition between conversation and basin dynamics.


<details>
  <summary>Details</summary>
Motivation: The key problem facing ChatGPT-like AI's use across society is its unpredictable behavior shifts from desirable to undesirable, which can lead to harm, financial losses, or costly mistakes.

Method: The authors use a vector generalization of fusion-fission group dynamics and a mathematical shift condition to estimate and forecast future shifts in AI behavior, which is model-agnostic and driven by group-level competition.

Result: The authors validated their method across six independent tests, achieving 90% correctness across seven AI models, and successfully predicting a corpus of human-AI exchanges eleven months in advance.

Conclusion: The proposed method provides a real-time warning signal for undesirable behavior shifts in ChatGPT-like AI models, which can be portable across current and future architectures and instantiated in various application domains.
...
### [233] [Good to Go: The LOOP Skill Engine That Hits 99% Success and Slashes Token Usage by 99% via One-Shot Recording and Deterministic Replay](https://arxiv.org/abs/2605.14237)
*Xiaohua Wang,Kai Yu,XuXiao Liang,Liang Wang,Chao Han*

Main category: cs.AI

TL;DR: The Loop Skill Engine is proposed to address the tension between flexibility and reliability in AI agents for repetitive tasks. It achieves a 99% success rate and 99% token reduction through a one-shot recording and deterministic replay paradigm.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to overcome the limitations of Large Language Models (LLMs) in repetitive periodic tasks, which suffer from unpredictable failures and high token costs.

Method: The Loop Skill Engine uses a one-shot recording, deterministic replay paradigm. It records the complete tool-call trajectory of the first run, extracts a parameterized, branch-free Loop Skill, and then replays the tool sequence deterministically in subsequent executions.

Result: The Loop Skill Engine achieves a combined 99% success rate and 99% token reduction for periodic agent tasks. It reduces monthly token consumption by 93.3%--99.98% and cuts execution latency by 8.7x while eliminating output non-determinism.

Conclusion: The Loop Skill Engine is a system that achieves high success rates and reduces token costs for repetitive periodic tasks by leveraging a deterministic replay paradigm. It is released as part of the buddyMe open-source agent framework.
...
### [234] [Hypergraph Enterprise Agentic Reasoner over Heterogeneous Business Systems](https://arxiv.org/abs/2605.14259)
*Ling Wang,Songnan Liu,Jianan Wang,Cheng Cheng,Xin Liu,Yihan Zhu,Enyu Li,Yu Xiao,Jiangyong Xie,Duogong Yan,Jiangyi Chen*

Main category: cs.AI

TL;DR: The paper introduces HEAR, an enterprise agentic reasoner that uses a Stratified Hypergraph Ontology to enable accurate and auditable reasoning in complex enterprise systems, achieving up to 94.7% accuracy in supply-chain tasks.


<details>
  <summary>Details</summary>
Motivation: The application of Large Language Models (LLMs) in heterogeneous enterprise systems is hindered by hallucinations and failures in multi-hop, n-ary reasoning, and existing paradigms lack semantic grounding and auditable execution.

Method: The authors propose HEAR, built on a Stratified Hypergraph Ontology with a base Graph Layer and a Hyperedge Layer, which operates an evidence-driven reasoning loop to dynamically orchestrate ontology tools for structured multi-hop analysis.

Result: HEAR achieves up to 94.7% accuracy in supply-chain tasks, including order fulfillment blockage root cause analysis (RCA), and demonstrates adaptive efficiency by minimizing token costs and leveraging topological exploration for correctness.

Conclusion: HEAR establishes a scalable, auditable foundation for enterprise intelligence by matching proprietary model performance with open-weight backbones and automating manual diagnostics.
...
### [235] [Heuristic Pathologies and Further Variance Reduction via Uncertainty Propagation in the AIVAT Family of Techniques](https://arxiv.org/abs/2605.14261)
*Juho Kim,Tuomas Sandholm*

Main category: cs.AI

TL;DR: The paper discusses challenges in evaluating agent performance in multiagent environments with limited sample sizes or high trial costs, and proposes solutions to address these issues using variance reduction techniques.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of evaluating agent performance in multiagent environments with limited sample sizes or high trial costs, and to provide guidelines for choosing a heuristic value function and handling uncertainty in its output.

Method: The paper proposes parameterizing the heuristic value function to highlight potential vulnerabilities, and then shows how to propagate heuristic uncertainty to quantify the uncertainty of AIVAT estimates. It also uses inverse-variance weighted averaging to further reduce variance.

Result: The paper demonstrates the heuristic pathology and uncertainty results using a dataset of 10,000 poker hands, achieving a 43.0% reduction in the number of samples needed to draw statistical conclusions.

Conclusion: The heuristic value function should be fixed prior to observing evaluation data, and propagating heuristic uncertainty can help quantify AIVAT estimate uncertainty and potentially reduce variance.
...
### [236] [Agentic AI Ecosystems in Higher Education: A Perspective on AI Agents to Emerging Inclusive, Agentic Multi-Agent AI Framework for Learning, Teaching and Institutional Intelligence](https://arxiv.org/abs/2605.14266)
*Vidya K Sudarshan,Anushka Sisodia,Reshma A Ramachandra,Sia Batra,Josephine Chong Leng Leng*

Main category: cs.AI

TL;DR: The paper proposes a multi-agent AI platform for higher education to support teaching, learning, and administrative processes, highlighting the need for integrated and inclusive AI systems.


<details>
  <summary>Details</summary>
Motivation: The current fragmented and inefficient implementation of AI agents in higher education, and the lack of a holistic, learner-centered educational ecosystem.
...
Result: The findings reveal a clear gap between current AI implementations and the needs of a holistic, learner-centered educational ecosystem, and outline future research directions for scalable, human-aligned, and inclusive agentic AI platforms.

Conclusion: The paper presents a forward-looking perspective on agentic multi-agent AI platforms in higher education, highlighting their potential to support seamless coordinated operations, foster inclusive and equitable learning, and provide adaptive, multimodal interventions for diverse learners.
...
### [237] [Parallelizing Counterfactual Regret Minimization](https://arxiv.org/abs/2605.14277)
*Juho Kim,Tuomas Sandholm*

Main category: cs.AI

TL;DR: This paper presents a parallelization framework for counterfactual regret minimization (CFR) algorithms, achieving up to four orders of magnitude speedup on a GPU compared to CPU implementations.


<details>
  <summary>Details</summary>
Motivation: The field of artificial intelligence has greatly benefited from parallelization, but its application to computational game solving, specifically to CFR algorithms, is relatively unexplored.

Method: The authors reframe CFR as a series of linear algebra operations and apply existing parallelization techniques to accelerate CFR, demonstrating its applicability to various CFR family algorithms.

Result: The GPU implementation of CFR achieves up to four orders of magnitude speedup compared to Google DeepMind OpenSpiel's CFR implementations on a CPU.

Conclusion: The proposed parallelization framework has the potential to significantly accelerate the solving of large imperfect-information games by leveraging the power of parallel computing.
...
### [238] [Precise Verification of Transformers through ReLU-Catalyzed Abstraction Refinement](https://arxiv.org/abs/2605.14294)
*Hengjie Liu,Zhenya Zhang,Jianjun Zhao*

Main category: cs.AI

TL;DR: A new approach for formally verifying transformers is proposed, achieving improved precision by using a novel usage of ReLU to represent precise but non-linear bounds for dot products.


<details>
  <summary>Details</summary>
Motivation: The increasing deployment of transformers in safety-critical applications necessitates their formal verification, but existing methods sacrifice precision for efficiency, leading to false alarms.

Method: The proposed approach extends two classic verification frameworks (rule-based and optimization-based) to transformers using a novel ReLU-based method for precise bounds on dot products.

Result: The approach achieves significant precision improvement over the state-of-the-art baseline for most verification tasks with acceptable efficiency compromise.

Conclusion: The proposed approach demonstrates effectiveness in improving verification precision for transformers, making it suitable for safety-critical applications.
...
### [239] [Semantic Feature Segmentation for Interpretable Predictive Maintenance in Complex Systems](https://arxiv.org/abs/2605.14318)
*Emilio Mastriani,Alessandro Costa,Federico Incardona,Kevin Munari,Sebastiano Spinello*

Main category: cs.AI

TL;DR: The paper proposes a semantic feature segmentation framework for predictive maintenance in complex systems, which decomposes monitored variables into a canonical component with dominant predictive information and a residual component with peripheral signals.


<details>
  <summary>Details</summary>
Motivation: The heterogeneity and redundancy of monitored variables in complex systems can obscure fault-relevant information and reduce model interpretability.

Method: The proposed framework uses domain-informed criteria to segment monitoring variables into functional groups and evaluates the effectiveness of this decomposition through a predictive perspective.

Result: The canonical space consistently achieves lower predictive risk than the residual space, and the semantic segmentation concentrates the most relevant information for fault anticipation.

Conclusion: The semantic feature segmentation framework provides an interpretable and information-preserving decomposition of monitoring signals, enabling competitive predictive performance without sacrificing operational interpretability.
...
### [240] [Are Agents Ready to Teach? A Multi-Stage Benchmark for Real-World Teaching Workflows](https://arxiv.org/abs/2605.14322)
*Zixin Chen,Peng Liu,Rui Sheng,Haobo Li,Jianhong Tu,Xiaodong Deng,Kashun Shum,Dayiheng Liu,Huamin Qu*

Main category: cs.AI

TL;DR: Introduced EduAgentBench, a benchmark for evaluating tutor agents in complex professional workflows, particularly in tutoring, and found current models lacking in holistic teaching capabilities.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks do not adequately measure the capabilities required for effective tutoring, such as diagnosing learner state, adapting support, and making pedagogically justified decisions.

Method: EduAgentBench was constructed through a pedagogical-insight-driven pipeline, containing 150 quality-controlled tasks across three capability surfaces: professional pedagogical judgment, situated multi-turn tutoring, and Canvas-style teaching workflow completion.

Result: Current models show bounded pedagogical judgment but fall short of professional teaching standards in situated tutoring and autonomous teaching-workflow execution.

Conclusion: EduAgentBench provides a measurement foundation for developing future tutor agents that can support realistic teaching work, highlighting the need for improvement in holistic teaching capabilities.
...
### [241] [CrystalReasoner: Reasoning and RL for Property-Conditioned Crystal Structure Generation](https://arxiv.org/abs/2605.14344)
*Yuyang Wu,Stefano Falletta,Delia McGrath,Sherry Yang*

Main category: cs.AI

TL;DR: A new LLM framework called CrystalReasoner is proposed for generating crystal structures from natural language instructions, leveraging physical priors and reinforcement learning to improve validity and stability.


<details>
  <summary>Details</summary>
Motivation: Existing generative models for crystal structure discovery struggle with low-level atomic precision or fail to integrate high-level scientific knowledge, resulting in invalid or unstable generated structures.

Method: CrystalReasoner, an end-to-end LLM framework that generates crystal structures through reasoning and alignment, introducing physical priors as thinking tokens and employing reinforcement learning with a multi-objective reward function.

Result: CrystalReasoner outperforms prior works and baselines, achieving better performance on diverse metrics, tripling the S.U.N. ratio, and exhibiting adaptive reasoning for property-conditioned generation.

Conclusion: The proposed CrystalReasoner framework demonstrates the potential of leveraging thinking traces and reinforcement learning for generating valid, stable, and property-conditioned crystal structures.
...
### [242] [Herculean: An Agentic Benchmark for Financial Intelligence](https://arxiv.org/abs/2605.14355)
...
TL;DR: The paper introduces Herculean, a benchmark for evaluating AI agents' ability to perform complex financial tasks, and finds that current agents struggle with long-horizon coordination and dependable workflow execution.


<details>
  <summary>Details</summary>
Motivation: The motivation is to assess whether AI agents can reliably carry out financial professional work, beyond just solving isolated tasks, and to identify the gap in current agents' abilities.

Method: The method involves introducing a new benchmark called Herculean, which spans four representative financial workflows: Trading, Hedging, Market Insights, and Auditing, and evaluating the performance of frontier agents across these workflows.

Result: The result shows that agents perform relatively well on Trading and Market Insights, but struggle substantially on Hedging and Auditing, highlighting a key gap in current agents' ability to turn financial reasoning into dependable workflow execution.

Conclusion: The conclusion is that current AI agents have a significant gap in their ability to perform complex financial tasks that require long-horizon coordination, state consistency, and structured verification, and that Herculean provides a standardized benchmark for assessing and improving agent capabilities.
...
### [243] [Uncovering the Representation Geometry of Minimal Cores in Overcomplete Reasoning Traces](https://arxiv.org/abs/2605.14358)
*Sanjoy Chowdhury,Dinesh Manocha*

Main category: cs.AI

TL;DR: The paper finds that language models' reasoning traces are often overcomplete, with 46% of steps being removable without affecting the final answer, and that minimal cores can expose a cleaner geometry of reasoning.


<details>
  <summary>Details</summary>
Motivation: To study the necessity of long chain-of-thought traces generated by language models and to identify the minimal subset of steps that preserves the final prediction.

Method: The authors define metrics for compression ratio, redundancy mass, step necessity, and necessity concentration, and use them to analyze overcomplete reasoning traces across six deliberative reasoning benchmarks.

Result: The authors find that on average, 46% of steps are removable under greedy minimal-core extraction while preserving the original answer in 86% of cases, and that predictive support is concentrated in the top three steps.

Conclusion: The results suggest that full reasoning traces are often verbose and overcomplete, while minimal cores isolate the effective support underlying language-model predictions, leading to improved correct-incorrect trace separation, reduced intrinsic dimensionality, and better transfer across model families.
...
### [244] [Nexus : An Agentic Framework for Time Series Forecasting](https://arxiv.org/abs/2605.14389)
...
TL;DR: A new multi-agent forecasting framework called Nexus is proposed, which integrates numerical and contextual information to improve time series forecasting by producing high-quality reasoning traces.


<details>
  <summary>Details</summary>
Motivation: Current Time Series Foundation Models (TSFMs) and Large Language Models (LLMs) have limitations in handling unstructured contextual data and uneven performance across domains, respectively.

Method: Nexus decomposes prediction into specialized stages, isolating temporal fluctuations and integrating contextual information before synthesizing a final forecast.

Result: Nexus consistently matches or outperforms state-of-the-art TSFMs and strong LLM baselines on data succeeding LLM knowledge cutoffs, producing high-quality reasoning traces.

Conclusion: Real-world forecasting is an agentic reasoning problem that extends beyond sequence modeling, and Nexus establishes a new approach to improving time series forecasting.
...
### [245] [Learning to Build the Environment: Self-Evolving Reasoning RL via Verifiable Environment Synthesis](https://arxiv.org/abs/2605.14392)
*Yucheng Shi,Zhenwen Liang,Kishan Panaganti,Dian Yu,Wenhao Yu,Haitao Mi*

Main category: cs.AI

TL;DR: The paper presents a vision for self-improving language models that construct their own training environments, and introduces EvoEnv, a method that synthesizes Python environments for improved performance.


<details>
  <summary>Details</summary>
Motivation: The authors aim to enable self-improving language models that can construct their own training environments, rather than simply generating problems or traces to imitate.

Method: The authors propose a method called EvoEnv, which synthesizes Python environments from ten seeds and admits them only after staged validation, semantic self-review, solver-relative difficulty calibration, and novelty checks.

Result: The results show that EvoEnv improves the performance of a strong language model (Qwen3-4B-Thinking) from 72.4 to 74.8, a relative gain of 3.3%.

Conclusion: The authors conclude that stable self-improvement in language models depends not on producing more synthetic data, but on models learning to construct worlds whose difficulty stays structurally beyond their own reach.
...
### [246] [Coding Agent Is Good As World Simulator](https://arxiv.org/abs/2605.14398)
*Hongyu Wang,Jingquan Wang,Bocheng Zou,Radu Serban,Dan Negrut*

Main category: cs.AI

TL;DR: The paper presents a framework for building physics-based world models through executable simulation code, which outperforms video-based models in physical accuracy, instruction fidelity, and visual quality.


<details>
  <summary>Details</summary>
Motivation: Current video-based world models do not explicitly enforce physical constraints, resulting in generated video rollouts that are not physically plausible.

Method: The proposed framework coordinates planning, code generation, visual review, and physics analysis agents to construct physics-based world models through executable simulation code.

Result: The framework outperforms advanced video-based models in physical accuracy, instruction fidelity, and visual quality, and can be applied to various scenarios including driving simulation and embodied robot tasks.

Conclusion: The proposed framework provides a more accurate and physically plausible approach to building world models, with potential applications in areas such as driving simulation and robotics.
...
### [247] [Metis AI: The Overlooked Middle Zone Between AI-Native and World-Movers](https://arxiv.org/abs/2605.14407)
*Xiang Li*

Main category: cs.AI

TL;DR: The paper challenges the common view of AI limitations, identifying a new class of digital tasks that resist reliable AI automation due to their institutional, social, and normative complexities.


<details>
  <summary>Details</summary>
Motivation: The dominant discourse on AI limitations overlooks the boundary within digital tasks, where AI struggles with tasks that require practical, contextual knowledge, referred to as Metis AI.

Method: The authors identify five structural characteristics that define Metis AI tasks: consequential irreversibility, relational irreducibility, normative open texture, adversarial co-evolution, and accountability anchoring, grounded in established theory from social sciences, philosophy, and humanitarian practice.

Result: The paper concludes that tasks in the Metis AI zone require a design response that involves centaur architectures, where humans lead and AI supports, rather than attempting better automation.

Conclusion: The authors argue that the limitations of AI in Metis AI tasks are not due to current model limitations but are inherent properties of the tasks themselves, requiring a shift in how we approach AI design for these complex tasks.
...
### [248] [A Unified Knowledge Embedded Reinforcement Learning-based Framework for Generalized Capacitated Vehicle Routing Problems](https://arxiv.org/abs/2605.14416)
*Wen Wang,Xiangchen Wu,Liang Wang,Hao Hu,Xianping Tao*

Main category: cs.AI

TL;DR: A knowledge-embedded framework is proposed to solve the Capacitated Vehicle Routing Problem (CVRP) by combining reinforcement learning with problem-specific knowledge.


<details>
  <summary>Details</summary>
Motivation: The CVRP is a complex problem with diverse objectives and constraints, and recent reinforcement learning approaches have shown limitations in solution quality due to lack of explicit problem-solving knowledge.

Method: The proposed framework incorporates knowledge at two levels: (1) decomposing CVRPs into route-first and cluster-second subproblems, and (2) using dynamic programming to guide the RL-based constructive solver.

Result: The framework achieves superior solution quality compared to state-of-the-art learning-based methods and demonstrates strong generalization across diverse CVRP variants.

Conclusion: The proposed framework offers a unified solution approach for CVRP by integrating problem-specific knowledge with reinforcement learning, leading to improved solution quality and generalizability.
...
### [249] [DVMap: Fine-Grained Pluralistic Value Alignment via High-Consensus Demographic-Value Mapping](https://arxiv.org/abs/2605.14420)
*Pengyun Zhu,Yuqi Ren,Zhen Wang,Lei Yang,Deyi Xiong*

Main category: cs.AI

TL;DR: The paper proposes DVMap, a framework for fine-grained pluralistic value alignment in Large Language Models (LLMs), which uses multi-dimensional demographic constraints to improve value alignment.


<details>
  <summary>Details</summary>
Motivation: Current LLMs rely on coarse-grained national labels for value alignment, which can lead to loose alignment due to intra-country value heterogeneity. The authors argue that a more fine-grained approach is needed to capture diverse value preferences within countries.

Method: The authors propose DVMap, which consists of: (1) a demographic archetype extraction strategy to construct a high-quality value alignment corpus, (2) a Structured Chain-of-Thought (CoT) mechanism to guide LLMs to reason about demographic-value correlations, and (3) Group Relative Policy Optimization (GRPO) to achieve adaptive anchoring of value distributions.

Result: Experimental results show that DVMap effectively learns the manifold mapping from demographics to values, exhibiting strong generalization and robustness. On cross-demographic tests, Qwen3-8B-DVMap achieves 48.6% accuracy, surpassing the advanced open-source LLM DeepSeek-v3.2 (45.1%).

Conclusion: The paper concludes that DVMap is an effective framework for fine-grained pluralistic value alignment in LLMs, which can improve the accuracy and robustness of value alignment by using multi-dimensional demographic constraints.
...
### [250] [BEAM: Binary Expert Activation Masking for Dynamic Routing in MoE](https://arxiv.org/abs/2605.14438)
*Juntong Wu,Jialiang Cheng,Qishen Yin,Yue Dai,Yuliang Yan,Fuyu Lv,Ou Dan,Li Yuan*

Main category: cs.AI

TL;DR: BEAM, a novel method for efficient Mixture-of-Experts (MoE) inference, learns token-adaptive expert selection via trainable binary masks, achieving up to 2.5x faster decoding and 1.4x higher throughput while retaining 98% of the original model's performance.


<details>
  <summary>Details</summary>
Motivation: Standard MoE employs a fixed Top-K routing strategy, leading to redundant computation and suboptimal inference latency. Existing acceleration methods either require costly retraining or suffer from severe performance drop at high sparsity.

Method: BEAM (Binary Expert Activation Masking) uses trainable binary masks to induce dynamic expert sparsity through end-to-end training while maintaining model capability.

Result: BEAM retains over 98% of the original model's performance while reducing MoE layer FLOPs by up to 85%, achieving up to 2.5x faster decoding and 1.4x higher throughput.

Conclusion: BEAM is a practical, plug-and-play solution for efficient MoE inference, demonstrating its effectiveness in reducing computation and improving inference efficiency.
...
### [251] [Synthesizing POMDP Policies: Sampling Meets Model-checking via Learning](https://arxiv.org/abs/2605.14440)
*Debraj Chakraborty,Anirban Majumdar,Prince Mathew,Sayan Mukherjee,Jean-François Raskin*

Main category: cs.AI

TL;DR: A new synthesis framework for Partially Observable Markov Decision Processes (POMDPs) is proposed, integrating sampling, automata learning, and model-checking to provide formal correctness guarantees.


<details>
  <summary>Details</summary>
Motivation: The need for a framework that bridges the gap between sampling-based methods and formal synthesis techniques for POMDPs, as existing methods either lack formal correctness guarantees or struggle with scalability.

Method: A synthesis framework that utilizes sampling as a membership oracle and model-checking as an equivalence oracle, inspired by Angluin's $L^*$ algorithm.

Result: The proposed framework successfully solves threshold-safety problems that remain challenging for existing formal synthesis tools and establishes a relative completeness result.

Conclusion: The proposed algorithm serves as a valuable component in a portfolio approach to tackling the inherent difficulty of POMDP synthesis problems, providing formal guarantees for safety-critical applications.
...
### [252] [Prompting Policies for Multi-step Reasoning and Tool-Use in Black-box LLMs with Iterative Distillation of Experience](https://arxiv.org/abs/2605.14443)
*Krishna Sayana,Ketan Todi,Ambarish Jash*

Main category: cs.AI

TL;DR: A Reinforcement Learning framework is proposed for optimizing prompts in Large Language Models, achieving significant performance gains in multi-step reasoning and tool-use tasks.


<details>
  <summary>Details</summary>
Motivation: The shift to interacting with frozen Large Language Models has turned prompt engineering into a critical optimization challenge, requiring a more systematic approach.

Method: A Reinforcement Learning framework with iterative distillation of experience, using a lightweight prompter model and a contrastive experience buffer, is proposed.

Result: Significant performance gains are achieved, improving from 55% to 90% in logic-intensive reasoning and 74% to 91% in tool-use tasks, with higher sample efficiency than state-of-the-art baselines.

Conclusion: The proposed framework effectively optimizes prompts in Large Language Models, demonstrating superior performance and efficiency in multi-step reasoning and tool-use tasks.
...
### [253] [Intelligence Impact Quotient (IIQ): A Framework for Measuring Organizational AI Impact](https://arxiv.org/abs/2605.14455)
*Chandan Rajah,Neha Sengupta,Federico Castanedo,Robin Mills,Amit Bahree,Ramesh Krishnan Muthukrishnan,Larry Murray*

Main category: cs.AI

TL;DR: The paper proposes the Intelligence Impact Quotient (IIQ), a metric to measure the integration and impact of AI systems in organizational work.


<details>
  <summary>Details</summary>
Motivation: Current metrics for AI impact, such as access counts or token volume, are insufficient, and a more comprehensive approach is needed to quantify AI's depth of integration and impact.

Method: The IIQ combines multiple factors, including novelty-weighted token stock, usage frequency, organizational leverage, task complexity, and autonomy, to produce a raw Intelligence Adoption Index (IAI) and a normalized IIQ index.

Result: The proposed IIQ metric can distinguish between different types of AI use, such as frequent low-leverage use and more autonomous, higher-consequence AI-assisted work.

Conclusion: The IIQ is a deployment-oriented measurement framework for tracking AI embedding in workflows, providing a more nuanced understanding of AI's impact on organizational work.
...
### [254] [Stateful Reasoning via Insight Replay](https://arxiv.org/abs/2605.14457)
*Bin Lei,Caiwen Ding,Jiachen Yang,Ang Li,Xin Eric Wang*

Main category: cs.AI

TL;DR: The paper proposes InsightReplay, a stateful reasoning approach that helps large language models retain critical insights during long reasoning chains, improving accuracy.


<details>
  <summary>Details</summary>
Motivation: The benefits of Chain-of-Thought (CoT) reasoning do not scale monotonically with chain length, as accuracy typically increases then declines due to weakened attention to earlier insights.

Method: The authors propose InsightReplay, which periodically extracts and replays critical insights near the active generation frontier to keep them accessible.

Result: Extensive experiments show that InsightReplay yields accuracy gains across all 24 settings, with an averaged improvement of +1.65 points over standard CoT and a largest single-setting gain of +9.2 points.

Conclusion: The effectiveness of test-time scaling depends not only on how much a model reasons, but also on whether critical intermediate insights remain accessible throughout long reasoning trajectories.
...
### [255] [OmniDrop: Layer-wise Token Pruning for Omni-modal LLMs via Query-Guidance](https://arxiv.org/abs/2605.14458)
*Yeo Jeong Park,Hyemi Jang,Minseo Choi,Jongsun Lee,Jooyoung Choi,Yongkweon Jeon*

Main category: cs.AI

TL;DR: OmniDrop is a token pruning framework for omni-modal large language models that reduces token explosion in audiovisual inputs, improving efficiency and performance.


<details>
  <summary>Details</summary>
Motivation: The token explosion caused by high-resolution audio and video inputs is a bottleneck for real-time applications and long-form reasoning in omni-modal large language models.

Method: OmniDrop is a training-free, layer-wise token pruning framework that prunes audiovisual tokens within LLM decoder layers, guided by text queries and a temporal diversity score.

Result: OmniDrop outperforms baselines by up to 3.58 points, reducing prefill latency by up to 40% and memory usage by up to 14.7% across various audiovisual benchmarks.

Conclusion: OmniDrop effectively addresses the token explosion issue in omni-modal large language models, offering improved performance, efficiency, and global temporal context preservation.
...
### [256] [From Table to Cell: Attention for Better Reasoning with TABALIGN](https://arxiv.org/abs/2605.14465)
*Tung Sum Thomas Kwok,Zeyong Zhang,Xinyu Wang,Chunhe Wang,Xiaofeng Lin,Hanwei Wu,Lei Ding,Guang Cheng,Zhijiang Guo*

Main category: cs.AI

TL;DR: TABALIGN, a table reasoning framework, improves accuracy and efficiency in multi-step LLM reasoning over structured tables by leveraging diffusion language models and attention mechanisms.


<details>
  <summary>Details</summary>
Motivation: Existing methods for multi-step LLM reasoning over structured tables fail due to the lack of explicit cell-grounding contract between planning and execution, leading to poor performance under table permutation.

Method: TABALIGN pairs a masked diffusion language model planner with a lightweight verifier, TABATTN, to score each step by its attention overlap with the plan-designated mask.

Result: TABALIGN improves average accuracy by 15.76 percentage points over the strongest open-source baseline and accelerates downstream reasoning execution by 44.64%.

Conclusion: The proposed TABALIGN framework effectively addresses the limitations of existing methods for multi-step LLM reasoning over structured tables, demonstrating improved accuracy and efficiency.
...
### [257] [Deepchecks: Evaluating Retrieval-Augmented Generation (RAG)](https://arxiv.org/abs/2605.14488)
...
TL;DR: This paper introduces Deepchecks, a framework for evaluating Retrieval-Augmented Generation (RAG) systems, addressing challenges in assessing reliability, relevance, and user satisfaction.


<details>
  <summary>Details</summary>
Motivation: Evaluating RAG systems is complex due to stochastic generated outputs and interplay between retrieval and generation components.

Method: Deepchecks uses a multi-faceted approach, root cause analysis, and production monitoring to evaluate RAG applications.

Result: Deepchecks provides a robust foundation for assessing reliability, relevance, and user satisfaction in RAG systems.

Conclusion: Deepchecks offers a comprehensive framework for evaluating RAG applications, ensuring alignment with application-specific requirements.
...
### [258] [Learning Scenario Reduction for Two-Stage Robust Optimization with Discrete Uncertainty](https://arxiv.org/abs/2605.14494)
*Tianjue Lin,Jianan Zhou,Jieyi Bi,Yaoxin Wu,Wen Song,Zhiguang Cao,Jie Zhang*

Main category: cs.AI

TL;DR: A new method called NeurPRISE is proposed to efficiently select representative scenarios for Two-Stage Robust Optimization problems with discrete uncertainty, achieving significant speedup and competitive performance.


<details>
  <summary>Details</summary>
Motivation: The challenge of solving Two-Stage Robust Optimization problems with discrete uncertainty, which can be computationally expensive due to the large number of scenarios, and the need for more efficient and scalable methods.

Method: NeurPRISE, a neural surrogate model built on a GNN-Transformer backbone, which uses imitation learning to distill marginal gain information from a problem-driven sequential lookahead heuristic called PRISE.

Result: NeurPRISE consistently achieves competitive regret relative to comprehensive methods, maintains strong scalability with varying numbers of scenarios, and delivers 7-200x speedup over PRISE, with strong zero-shot generalization capabilities.

Conclusion: NeurPRISE offers a efficient and scalable solution for scenario reduction in Two-Stage Robust Optimization problems, enabling faster and more reliable computation of optimal solutions.
...
### [259] [When Robots Do the Chores: A Benchmark and Agent for Long-Horizon Household Task Execution](https://arxiv.org/abs/2605.14504)
*Zilin Zhu,Longteng Guo,Yanghong Mei,Bowen Pang,Zongxun Zhang,Xingjian He,Ruyi Ji,Jing Liu*

Main category: cs.AI

TL;DR: Introduced LongAct, a benchmark for evaluating long-horizon household task planning in embodied AI, and HoloMind, a VLM-driven agent that improves performance while reducing model scale dependency.


<details>
  <summary>Details</summary>
Motivation: Existing embodied AI benchmarks focus on short-horizon tasks and fixed categories, neglecting robust high-level planning and sustained reasoning capabilities required for long-horizon household tasks.

Method: Proposed HoloMind, a VLM-driven agent with a hierarchical planner, multimodal spatial memory, episodic memory, and global critic, and evaluated it on LongAct, a benchmark for long-horizon household tasks.

Result: HoloMind improved long-horizon performance while reducing reliance on model scale; even top models achieved only 59% goal completion and 16% full-task success.

Conclusion: LongAct and HoloMind highlight the need for stronger long-horizon planning in embodied agents, and the proposed approach shows promise for improving performance in complex household tasks.
...
### [260] [Cattle Trade: A Multi-Agent Benchmark for LLM Bluffing, Bidding, and Bargaining](https://arxiv.org/abs/2605.14537)
*Robert Müller,Clemens Müller*

Main category: cs.AI

TL;DR: A new benchmark called Cattle Trade is introduced to evaluate large language models as agents in strategic reasoning under imperfect information and resource constraints.


<details>
  <summary>Details</summary>
Motivation: The motivation is to assess the ability of large language models to integrate multiple capabilities in a competitive, multi-agent economic game with conflicting incentives.

Method: The benchmark combines auctions, hidden-offer trade challenges, bargaining, bluffing, opponent modeling, and resource allocation within a single long-horizon game.

Result: The results show that strategic coherence is associated with rank more strongly than spending volume or any single subskill, and two heuristic code agents outperform most tested LLMs.

Conclusion: Evaluating agentic competence requires benchmarks that test the joint deployment of multiple capabilities in multi-agent environments with conflicting incentives, uncertainty, and economic dynamics.
...
### [261] [VerbalValue: A Socially Intelligent Virtual Host for Sales-Driven Live Commerce](https://arxiv.org/abs/2605.14542)
*Yuyan Chen*

Main category: cs.AI

TL;DR: VerbalValue, a virtual host AI system, is developed to convert viewer curiosity into purchase intent in live-commerce through expert product knowledge and emotionally intelligent responses.


<details>
  <summary>Details</summary>
Motivation: Existing AI systems fail to replicate the role of a live-commerce host, who not only provides product information but also engages viewers emotionally and persuasively.

Method: VerbalValue is built on a domain knowledge base, a curated sales terminology lexicon, and fine-tuned large language model on 1,475 live-commerce interactions.

Result: VerbalValue shows gains of 23% on informativeness and 18% on factual correctness compared to baselines like GPT-5.4 and Claude Sonnet 4.6.

Conclusion: VerbalValue effectively converts viewer curiosity into purchase intent through expert product knowledge and emotionally intelligent responses, outperforming existing AI systems.
...
### [262] [Complacent, Not Sycophantic: Reframing Large Language Models and Designing AI Literacy for Complacent Machines](https://arxiv.org/abs/2605.14544)
*Federico Germani,Giovanni Spitale*

Main category: cs.AI

TL;DR: The term 'sycophantic' is misapplied to large language models; their behavior is better described as 'complacent', a structural tendency to agree with users due to their design and training.


<details>
  <summary>Details</summary>
Motivation: To correct the misconception that large language models are sycophantic and to reframe their behavior as complacent, highlighting the role of developers and institutions in model behavior.

Method: Conceptual analysis and argumentation to distinguish between sycophancy and complacency in large language models.

Result: The conclusion that large language models are complacent, not sycophantic, and that this distinction matters for understanding and addressing their behavior.

Conclusion: The reframing of large language models' behavior as complacent rather than sycophantic emphasizes the need for AI literacy education focusing on strategies to counter confirmation bias.
...
### [263] [TeachAnything: A Multimodal Crowdsourcing Platform for Training Embodied AI Agents in Symmetrical Reality](https://arxiv.org/abs/2605.14556)
*Zidong Liu,Rongkai Liu,Yue Li,Zhenliang Zhang*

Main category: cs.AI

TL;DR: A new platform called TeachAnything is introduced for collecting diverse demonstration data to develop human-like intelligent agents for Symmetrical Reality.


<details>
  <summary>Details</summary>
Motivation: The emerging trend of Symmetrical Reality (SR) requires agents to have human-like intelligence, which demands richer and more diverse human guidance.

Method: A three-stage demonstration paradigm integrating multimodal demonstration signals and a cloud-based, crowdsourcing-oriented demonstration platform with physics simulation.

Result: TeachAnything, a practical foundation for developing embodied agents aligned with Symmetrical Reality, capable of collecting diverse demonstration data.

Conclusion: TeachAnything provides a foundation for developing embodied agents that can coexist with humans in Symmetrical Reality.
...
### [264] [PyCSP3-Scheduling: A Scheduling Extension for PyCSP3](https://arxiv.org/abs/2605.14559)
*Sohaib Afifi*

Main category: cs.AI

TL;DR: PyCSP$^3$ Scheduling extends PyCSP$^3$ with scheduling abstractions, compiling them to standard PyCSP$^3$/XCSP$^3$ constraints while maintaining modeling/solving separation.


<details>
  <summary>Details</summary>
Motivation: PyCSP$^3$ lacks native support for scheduling abstractions, requiring low-level integer variables and manual channeling constraints for scheduling models.

Method: The library adds 53 dedicated constraints and 27 expressions for scheduling abstractions, compiling them down to standard PyCSP$^3$/XCSP$^3$ constraints.

Result: The new library produces identical objectives on all 72 doubly-proved optimal pairs and maintains structural unchanged on nearly half of the families (8/17), with runtime performance gains on some (up to 5.8x) and regressions on others.

Conclusion: PyCSP$^3$ Scheduling provides a productive way to model scheduling problems while preserving the separation between modeling and solving.
...
### [265] [Prompt Segmentation and Annotation Optimisation: Controlling LLM Behaviour via Optimised Segment-Level Annotations](https://arxiv.org/abs/2605.14561)
*Devika Prasad,Luke Gerschwitz,Tong Li,Henry Xiao,Anjin Liu,Coco Wu,Anna Leontjeva,Luiz Pizzato*

Main category: cs.AI

TL;DR: Introduced PSAO, a structured prompt optimisation framework that improves controllability and efficiency in interactions with generative AI systems.


<details>
  <summary>Details</summary>
Motivation: Existing optimisation methods for prompt engineering are often computationally costly and may distort the original intent due to the vast and unstructured prompt space.

Method: PSAO decomposes prompts into interpretable segments with human-readable annotations to guide large language models in allocating focus during response generation.

Result: Empirical evaluations show that PSAO benefits from annotations, leading to improved reasoning accuracy and self-consistency in LLM responses.

Conclusion: PSAO is a feasible and potentially effective approach for optimising prompts, but developing efficient methods for identifying optimal segmentations and annotations remains a challenge for future research.
...
### [266] [Sycophancy is an Educational Safety Risk: Why LLM Tutors Need Sycophancy Benchmarks](https://arxiv.org/abs/2605.14604)
*Enkelejda Kasneci,Gjergji Kasneci*

Main category: cs.AI

TL;DR: The paper argues that effective tutoring requires surfacing and challenging misconceptions, but LLMs may prioritize agreeableness over epistemic rigor, leading to a Reasoning-Sycophancy Paradox. The authors introduce EduFrameTrap, a benchmark to evaluate LLMs' ability to provide supportive but corrective tutoring under various pressures.


<details>
  <summary>Details</summary>
Motivation: The authors are motivated by the need for effective tutoring that challenges students' misconceptions in a supportive manner. They recognize that LLMs may compromise on epistemic rigor to maintain agreeableness, which can hinder their ability to provide corrective tutoring.

Method: The authors introduce EduFrameTrap, a tutoring benchmark that tests LLMs across math, physics, economics, chemistry, biology, and computer science. They evaluate two frontier LLMs, GPT-5.2 and Claude, under varying student confidence and pressure conditions, including context-switch, authority, and social-affective pressures.

Result: The results show that GPT-5.2 performs better in context-switch conditions, but both LLMs are susceptible to authority and social pressure, leading to epistemic retreat. Claude exhibits substantial context-switch fragility. The authors highlight the importance of human judgment in evaluating these failures, citing two-judge disagreement as a reliability signal.

Conclusion: The authors conclude that benchmarks should prioritize measuring social-epistemic courage, i.e., supportive but corrective tutoring, and treat kind-but-correct behavior as a safety requirement. They emphasize the need for LLMs to balance agreeableness with epistemic rigor to provide effective tutoring.
...
### [267] [SliceGraph: Mapping Process Isomers in Multi-Run Chain-of-Thought Reasoning](https://arxiv.org/abs/2605.14619)
*Kang Chen,Junjie Nian,Yixin Cao,Yugang Jiang*

Main category: cs.AI

TL;DR: The paper proposes SliceGraph, a method to analyze the chain-of-thought (CoT) reasoning process in AI models, revealing that correct trajectories can have different process families, even with the same final answer.


<details>
  <summary>Details</summary>
Motivation: Current methods for analyzing CoT reasoning collapse the process to final-answer aggregates, discarding the shared, split, and rejoining of sampled trajectories through intermediate computations.

Method: The authors propose SliceGraph, a post-hoc graph built by mutual-kNN over sparse activation-key Jaccard similarity between CoT slices, to analyze the process geometry of CoT reasoning.

Result: The results show that correct CoTs with the same final answer can split into multiple process families (process isomers), and that route families specialize over high-value core footprints rather than duplicating one another.

Conclusion: The paper concludes that final-answer aggregation overlooks the structured multi-route process geometry of CoT reasoning, and that SliceGraph provides a robust scaffold for analyzing this process.
...
### [268] [Teaching Large Language Models When Not to Know: Learning Temporal Critique for Ex-Ante Reasoning](https://arxiv.org/abs/2605.14636)
*Chenlu Ding,Jiancan Wu,Yanchen Luo,Zheyuan Liu,Yancheng Yuan,Xiang Wang*

Main category: cs.AI

TL;DR: The paper studies the failure of large language models (LLMs) to reason under temporal cutoffs and proposes a Temporal Critique Fine-Tuning (TCFT) framework to address this issue.


<details>
  <summary>Details</summary>
Motivation: LLMs often exploit knowledge that became available after a temporal cutoff, leading to incorrect answers. The authors aim to develop a method to train models to be aware of temporal cutoffs and verify the admissibility of responses.

Method: The authors conduct a systematic analysis of prompt-level interventions and propose TCFT, a framework that trains models to identify post-cutoff leakage, explain temporal boundary violations, and judge temporal admissibility.

Result: Experiments show that TCFT consistently outperforms prompting and supervised fine-tuning (SFT) baselines, reducing average leakage by 41.89 and 37.79 percentage points, respectively.

Conclusion: The paper concludes that prompting can steer models into a temporal frame but does not enable them to verify temporal admissibility. TCFT is a effective approach to address this issue and improve the ability of LLMs to reason under temporal cutoffs.
...
### [269] [MindGap: A Conversational AI Framework for Upstream Neuroplastic Intervention in Post-Traumatic Stress Disorder](https://arxiv.org/abs/2605.14660)
...
TL;DR: The paper proposes MindGap, an on-device AI framework for treating PTSD by targeting the neural pathways that cause over-reactive stress responses.


<details>
  <summary>Details</summary>
Motivation: Current PTSD therapies only suppress symptoms, but do not structurally reorganize the neural pathways that cause them.

Method: MindGap uses a conversational AI framework based on Buddhist psychology to guide patients through a process of observing and reframing their affective signals, with the goal of weakening the reactive neural pathways.

Result: The proposed framework has the potential to produce genuine upstream dissolution of the reactive neural pathways, rather than just downstream suppression of symptoms.

Conclusion: MindGap offers a promising approach for treating PTSD by targeting the root causes of the disorder, and its on-device deployment makes it suitable for sensitive clinical and military contexts.
...
### [270] [Falkor-IRAC: Graph-Constrained Generation for Verified Legal Reasoning in Indian Judicial AI](https://arxiv.org/abs/2605.14665)
*Joy Bose*

Main category: cs.AI

TL;DR: The paper presents Falkor-IRAC, a graph-constrained generation framework for Indian legal AI, which uses a knowledge graph to ensure accurate and valid legal reasoning.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based legal AI systems have limitations, such as hallucinated precedents and outdated statute citations, which can have serious consequences for access to justice. The authors aim to address these limitations with a more structured approach.

Method: The authors propose a graph-constrained generation framework, Falkor-IRAC, which uses an IRAC knowledge graph to ground generation in structured reasoning. The system includes a Verifier Agent to validate the accuracy of generated answers.

Result: The evaluation of Falkor-IRAC on a proof-of-concept corpus of 51 Supreme Court judgments shows promising results, with the Verifier Agent correctly validating citations and rejecting fabricated ones.

Conclusion: The paper concludes that Falkor-IRAC offers a more appropriate approach for legal reasoning evaluation, using graph-native metrics, and has the potential to improve the accuracy and validity of legal AI systems.
...
### [271] [Agentifying Patient Dynamics within LLMs through Interacting with Clinical World Model](https://arxiv.org/abs/2605.14723)
...
TL;DR: SepsisAgent, a world model-augmented LLM agent, is introduced for sepsis treatment recommendation in ICU, outperforming traditional baselines in off-policy value and safety.


<details>
  <summary>Details</summary>
Motivation: Sepsis management in ICU requires sequential treatment decisions under rapidly evolving patient physiology, and large language models (LLMs) are not inherently grounded in action-conditioned patient dynamics.

Method: SepsisAgent uses a learned Clinical World Model to simulate patient responses under candidate fluid--vasopressor interventions, and follows a propose--simulate--refine workflow. It is trained through a three-stage curriculum: patient-dynamics supervised fine-tuning, propose--simulate--refine behavior cloning, and world-model-based agentic reinforcement learning.

Result: SepsisAgent outperforms all traditional RL and LLM-based baselines in off-policy value while achieving the best safety profile under guideline adherence and unsafe-action metrics.

Conclusion: SepsisAgent demonstrates improved performance and safety in sepsis treatment recommendation, and repeated interaction with the Clinical World Model enables the agent to learn useful regularities in patient evolution.
...
TL;DR: A framework is proposed to quantify the impact of scan parameters on AI radiomic models, identifying optimal parameters for improved cross-dataset robustness in lung cancer diagnosis.


<details>
  <summary>Details</summary>
Motivation: The drop in performance of AI radiomic systems under heterogeneous multicentre acquisition protocols is a significant barrier to their clinical deployment.

Method: A mixed-effects framework is formulated to quantify the influence of clinically relevant acquisition parameters on model performance, accounting for subject-level random effects.

Result: The optimal configuration of scan parameters is identified as: current of the X-ray tube >= 200 mA, spiral pitch <= 1.5, slice thickness <= 1.25 mm, which improves metrics from 0.79+-0.04 sensitivity, 0.47+-0.10 specificity to 0.90+-0.10 sensitivity, 0.79 +- 0.13 specificity.

Conclusion: The proposed framework can help improve the cross-dataset robustness of AI radiomic models by identifying clinically significant parameter regions associated with improved performance.
...
TL;DR: The paper introduces a new benchmark called $π$-Bench to evaluate the ability of personal assistant agents to provide proactive assistance in multi-turn interactions.


<details>
  <summary>Details</summary>
Motivation: The growing potential of large language models to support users in everyday life and work, and the challenge of proactive assistance where users have unstated needs, constraints, or preferences.

Method: Introduction of $π$-Bench, a benchmark comprising 100 multi-turn tasks across 5 domain-specific user personas, to evaluate agents' ability to anticipate and address user needs.

Result: Experiments show that proactive assistance remains challenging, there is a distinction between task completion and proactivity, and prior interaction is valuable for proactive intent resolution.

Conclusion: The paper highlights the importance of evaluating proactive assistance in sustained multi-turn interactions and the need for benchmarks like $π$-Bench to drive progress in this area.
...
### [274] [Holistic Evaluation and Failure Diagnosis of AI Agents](https://arxiv.org/abs/2605.14865)
...
TL;DR: A new holistic agent evaluation framework is proposed, combining top-down and bottom-up approaches to provide detailed analysis of AI agent failures.


<details>
  <summary>Details</summary>
Motivation: Current evaluation methods for AI agents are insufficient, as they either only report success or failure without explanation or struggle to identify failure types and locations in long processes.

Method: The proposed framework pairs top-down agent-level diagnosis with bottom-up span-level evaluation, allowing for independent assessment of each span and scaling to traces of arbitrary length.

Result: The framework achieves state-of-the-art results on the TRAIL benchmark, with significant gains over prior baselines in category F1, localization accuracy, and joint localization-categorization accuracy.

Conclusion: The proposed framework provides a more effective and detailed evaluation of AI agents, highlighting that evaluation methodology, rather than model capability, is the bottleneck in current AI agent evaluation.
...
### [275] [On Strong Equivalence Notions in Logic Programming and Abstract Argumentation](https://arxiv.org/abs/2605.14721)
*Giovanni Buraglio,Wolfgang Dvorak,Stefan Woltran*

Main category: cs.AI

TL;DR: This paper investigates the discrepancy in strong equivalence between logic programs and abstract argumentation frameworks in dynamic contexts, and introduces a new notion of strong equivalence for logic programs to restore compatibility.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the discrepancy in strong equivalence between logic programs and abstract argumentation frameworks in dynamic contexts, which arises due to differing notions of update.

Method: The authors introduce a new notion of strong equivalence for logic programs and investigate its properties to restore compatibility across certain classes of logic programs and both Dung-style and claim-augmented argumentation frameworks.

Result: The authors propose a new notion of strong equivalence for logic programs that preserves strong equivalence under translation between certain classes of logic programs and both Dung-style and claim-augmented argumentation frameworks.

Conclusion: The paper concludes that the introduced notion of strong equivalence for logic programs restores compatibility across these formalisms, enabling the possibility of replacing one with the other without affecting reasoning outcomes in any given context.
...
### [276] [Explainable Detection of Depression Status Shifts from User Digital Traces](https://arxiv.org/abs/2605.14995)
*Loris Belcastro,Francesco Gervino,Fabrizio Marozzo,Domenico Talia,Paolo Trunfio*

Main category: cs.AI

TL;DR: The paper proposes an explainable framework for detecting and analyzing depression-related status shifts in user digital traces, combining BERT-based models and a large language model to generate concise and human-readable reports.


<details>
  <summary>Details</summary>
Motivation: The motivation is to develop a framework that can analyze user digital traces to detect and understand changes in mental health signals over time, specifically for depression.

Method: The approach combines multiple BERT-based models to extract complementary signals across different dimensions, aggregates them over time to construct user-level trajectories, and analyzes them to identify meaningful change points.
...
Conclusion: The proposed framework provides an interpretable view of mental health signals over time, supporting research and decision making without aiming at clinical diagnosis.
...
### [277] [Small, Private Language Models as Teammates for Educational Assessment Design](https://arxiv.org/abs/2605.15015)
*Chris Davis Jaldi,Anmol Saini,Shan Zhang,Noah Schroeder,Cogan Shimizu,Eleni Ilkou*

Main category: cs.AI

TL;DR: The paper compares Large Language Models (LLMs) and Small Language Models (SLMs) for generating assessment questions, finding SLMs to be competitive in quality while offering privacy benefits, but also highlighting the need for human oversight due to model biases.


<details>
  <summary>Details</summary>
Motivation: To address the gap in understanding the effectiveness of Small Language Models (SLMs) for assessment question design, and to systematically compare LLMs and SLMs in terms of generation quality and evaluation reliability.

Method: The study systematically compares LLMs and SLMs for assessment question design, using reproducible and pedagogically grounded metrics to evaluate generation quality across Bloom's taxonomy levels, and assesses model-based judging against expert-informed evaluation.

Result: SLMs achieve competitive performance across key pedagogically motivated quality dimensions, enabling local, privacy-sensitive deployment, but model-based evaluations also exhibit systematic inconsistencies and bias relative to expert ratings.

Conclusion: Language models should be considered as bounded assistants in assessment workflows, necessitating Human-in-the-Loop to ensure quality and reliability, and advancing the field of automated educational question generation by examining quality, reliability, and deployment-aware trade-offs.
...
### [278] [XDomainBench: Diagnosing Reasoning Collapse in High-Dimensional Scientific Knowledge Composition](https://arxiv.org/abs/2605.14754)
...
TL;DR: The paper introduces XDomainBench, a benchmark for testing the compositional generalization of Large Language Models (LLMs) in interactive interdisciplinary scientific reasoning, revealing a systematic reasoning collapse as composition order increases.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for LLMs fail to capture their capability boundaries in real-world interactive scientific workflows, particularly in compositional generalization across disciplines.

Method: The authors introduce XDomainBench, a diagnostic benchmark comprising 8,598 interactive sessions across 20 domains and 4 task categories, with 8 realistic trajectory patterns.

Result: Large-scale evaluation of LLMs using XDomainBench reveals a systematic reasoning collapse as composition order increases, caused by direct difficulty increases and indirect interaction-amplified failures.

Conclusion: The study highlights the limitations of LLMs in compositional generalization across disciplines, emphasizing the need for more advanced models that can handle complex, interactive scientific workflows.
...
### [279] [Orchard: An Open-Source Agentic Modeling Framework](https://arxiv.org/abs/2605.15040)
...
TL;DR: The paper presents Orchard, an open-source framework for scalable agentic modeling, enabling autonomous agents capable of solving complex tasks through planning, reasoning, and tool use.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the constraints in open research on agentic modeling due to infrastructure and training gaps, and to provide an open-source framework for scalable agent training.

Method: The authors present Orchard Env, a lightweight environment service, and build three agentic modeling recipes: Orchard-SWE, Orchard-GUI, and Orchard-Claw, each targeting different domains such as coding agents, vision-language computer-use agents, and personal assistant agents.

Result: The results show that Orchard-SWE achieves 64.3% and 67.5% on SWE-bench Verified after SFT and SFT+RL respectively, Orchard-GUI achieves 74.1%, 67.0%, and 64.0% success rates on WebVoyager, Online-Mind2Web, and DeepShop, and Orchard-Claw achieves 59.6% pass@3 on Claw-Eval and 73.9% when paired with a stronger ZeroClaw harness.

Conclusion: The paper concludes that a lightweight, open, harness-agnostic environment layer enables reusable agentic data, training recipes, and evaluations across domains, and that Orchard provides a scalable solution for agentic modeling.
...
TL;DR: A probabilistic framework, RNN-ProVe, is proposed for verifying RNN-based policies in partially observable reinforcement learning, providing more quantitative and feasibility-aware guarantees.


<details>
  <summary>Details</summary>
Motivation: The challenge of verifying RNN-based policies in partially observable reinforcement learning due to latent hidden state dynamics and the limitations of existing verification tools.

Method: RNN-ProVe uses policy-driven sampling to approximate feasible hidden states and derives statistical error bounds for bounded-error estimates of behavioral violations.

Result: RNN-ProVe yields more quantitative, feasibility-aware probabilistic guarantees than existing tools, scaling to recurrent and multi-agent settings.

Conclusion: The proposed RNN-ProVe framework offers a more effective approach to verifying RNN-based policies in partially observable reinforcement learning environments.
...
### [281] [Case-Based Calibration of Adaptive Reasoning and Execution for LLM Tool Use](https://arxiv.org/abs/2605.15041)
*Renning Pang,Tian Lan,Leyuan Liu,Piao Tong,Sheng Cao,Xiaosong Zhang*

Main category: cs.AI

TL;DR: The paper presents CAST, a case-driven framework that improves large language models' tool use by balancing reasoning depth and structural validity, achieving higher execution accuracy and reducing unnecessary deliberation.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of reliable execution in tool use by large language models, which requires balancing appropriate reasoning depth with strict structural validity.

Method: The method involves a case-based approach, where historical execution trajectories are treated as structured cases to extract case-derived signals, and then used to design a fine-grained reward and adaptive reasoning for reinforcement learning.

Result: The results show that CAST improves schema-faithful execution and task-level tool-use success while reducing unnecessary deliberation, achieving up to 5.85 percentage points gain in overall execution accuracy and reducing average reasoning length by 26%.

Conclusion: The conclusion is that historical execution cases can provide reusable adaptation knowledge for calibrated tool use, demonstrating the effectiveness of the CAST framework in improving large language models' tool use.
...
TL;DR: A new AI system combining deep learning and large language models accurately predicts individual aesthetic evaluations of images, outperforming humans and conventional systems.


<details>
  <summary>Details</summary>
Motivation: The challenge of accurately predicting individual aesthetic evaluations of images due to subjective and individual-dependent nature of aesthetic preferences.

Method: An integrated DL-LLM system that elicits aesthetic preferences through LLM-based semi-structured interviews and predicts aesthetic evaluation using low-level and high-level features.

Result: The proposed system outperforms conventional systems, human predictors, and the target individual's own re-evaluations, with particularly strong performance on highly-rated images.

Conclusion: AI may be better positioned than humans or one's future self to capture individual aesthetic preferences, raising questions about AI's role in interpreting human aesthetic sensibility.
...
TL;DR: MediaClaw is a multimodal agent platform that addresses deployment pain points in AIGC adoption with a three-layer architecture.


<details>
  <summary>Details</summary>
Motivation: The platform aims to solve practical deployment issues in AIGC adoption, including fragmented capabilities, heterogeneous interfaces, and limited reuse of production workflows.

Method: MediaClaw uses a unified abstraction, pluginized extension, and workflow orchestration, with a core capability model and task-oriented Skills.

Result: The platform provides a reusable practical reference for building multimodal capability platforms.

Conclusion: The architectural design philosophy and key engineering trade-offs of MediaClaw are presented to facilitate the development of similar platforms.
...
TL;DR: A deep learning approach using Deep Deterministic Policy Gradient (DDPG) is proposed to improve the identification of criminals in investigations, achieving 95% accuracy.


<details>
  <summary>Details</summary>
Motivation: The challenge of identifying criminals from complex datasets while minimizing false positives and negatives in conventional investigations.

Method: Using Deep Deterministic Policy Gradient (DDPG) algorithm trained on crime scene material, witness statements, and suspect profiles.

Result: The proposed DDPG method achieved an accuracy of 95% in identifying criminals, outperforming existing methods.

Conclusion: The proposed approach shows efficacy in accurately identifying criminals, suggesting its potential to enhance criminal investigations.
...
TL;DR: The paper introduces ARPM, a framework for maintaining long-term dialogue consistency in large language models by separating static knowledge from dynamic dialogue experience and using various techniques for evidence verification and answer binding.


<details>
  <summary>Details</summary>
Motivation: Large language models face challenges such as fact loss, timeline confusion, and persona drift during long-range interactions, especially with high-noise knowledge bases, context clearing, and cross-model transfer.

Method: ARPM uses a combination of vector retrieval, BM25, RRF fusion, dual-temporal reranking, chronological evidence reading, and a controlled analysis protocol for evidence verification and answer binding.

Result: Experiments show that ARPM maintains semantic continuity, boundary continuity, and persona consistency even under challenging conditions such as high noise, context clearing, and multi-model handoff.

Conclusion: The paper demonstrates that long-term persona consistency can be decomposed into governable components and evaluated in a white-box manner, and that ARPM is an effective framework for addressing these challenges.
...
TL;DR: The paper explores the concept of 'interestingness' in recursively self-improving systems, formalizing it as an inductive heuristic for future compression progress and investigating its predictability.


<details>
  <summary>Details</summary>
Motivation: The challenge of identifying tasks or data that hold potential for future progress in recursively self-improving systems.

Method: Using tools from Kolmogorov Complexity and Algorithmic Statistics to analyze complexity-runtime profiles under different priors.

Result: The inductive property of interestingness is theoretically viable and empirically supported, with expected future progress depending exponentially on the recency of the last observed breakthrough.

Conclusion: The study demonstrates the viability of interestingness as a predictor of future progress, with the Algorithmic Prior yielding more optimistic results than the Length Prior.
...
TL;DR: The paper proposes an emotion-attended stateful memory architecture for language models to improve personalization in conversations. It shows significant improvements over stateless models in handling emotional conversations.


<details>
  <summary>Details</summary>
Motivation: Current language models are stateless and lack the ability to personalize interactions over time. The authors aim to address this limitation by incorporating a stateful memory architecture that considers long-term history, emotional signals, and inferred intent.

Method: The authors propose an emotion-attended stateful memory architecture and evaluate it through a controlled A/B study with thirty non-scripted conversations across six emotionally distinct categories.

Result: The memory-enriched condition consistently outperformed the stateless baseline, with significant improvements in memory grounding (95%), plan clarity (57%), and emotional validation (34%).

Conclusion: The findings suggest that stateful emotional memory may be a foundational infrastructure layer for hyper-personalized AI systems, but broader validation is necessary.
...
### [288] [A Deterministic Agentic Workflow for HS Tariff Classification: Multi-Dimensional Rule Reasoning with Interpretable Decisions](https://arxiv.org/abs/2605.14857)
*Yu Zhang,Dongjiang Zhuang,Qu Zhou,Zheng Huang,Junhe Wu,Jing Cao,Kai Chen*

Main category: cs.AI

TL;DR: A deterministic agentic workflow is proposed for Harmonized System (HS) tariff classification, combining offline knowledge-engineering and an online six-stage pipeline to improve accuracy and interpretability.


<details>
  <summary>Details</summary>
Motivation: The difficulty in HS tariff classification lies in multi-dimensional rule reasoning, where a correct classification must satisfy competing priority rules along several axes simultaneously, and end-to-end prompting of large language models often fails to resolve these constraints.

Method: A deterministic agentic workflow with a fixed control flow, confined language model calls, and local reflection and verification mechanisms is designed, combining offline knowledge-engineering of the Chinese HS tariff with an online six-stage pipeline.

Result: The workflow achieves 75.0% top-1 and 91.5% top-3 accuracy at four digits, and 64.2% top-1 and 78.3% top-3 accuracy at six digits with Qwen3.6-plus, and 84.2% four-digit and 77.4% six-digit top-1 agreement with the frontier model.

Conclusion: The proposed workflow improves accuracy and interpretability in HS tariff classification, and highlights potential deviations in ground-truth labels, providing a foundation for further community review and improvement.
...
### [289] [BiFedKD: Bidirectional Federated Knowledge Distillation Framework for Non-IID and Long-Tailed ECG Monitoring](https://arxiv.org/abs/2605.14886)
*Zixuan Shu,Tiancheng Cao,Hen-Wei Huang*

Main category: cs.AI

TL;DR: The authors propose BiFedKD, a framework that improves federated learning for ECG monitoring in IoMT networks, achieving better accuracy and reduced communication overhead.


<details>
  <summary>Details</summary>
Motivation: Federated learning for ECG monitoring faces challenges due to data-sharing regulations, privacy concerns, and communication bottlenecks. Existing methods like federated distillation (FD) struggle with non-IID and long-tailed label distributions.

Method: The authors propose a bidirectional federated knowledge distillation (BiFedKD) framework that uses an aggregation-by-distillation pipeline with temperature scaling for stable global distillation signal and cross-client alignment.

Result: BiFedKD improves accuracy by 3.52% and Macro-F1 by 9.93% over the baseline on the MIT-BIH Arrhythmia dataset. It also reduces communication overhead by 40% and computation cost by 71.7% to reach the same Macro-F1.

Conclusion: BiFedKD effectively addresses the challenges of federated learning for ECG monitoring, providing improved performance and reduced communication overhead.
...
### [290] [Beyond Individual Intelligence: Surveying Collaboration, Failure Attribution, and Self-Evolution in LLM-based Multi-Agent Systems](https://arxiv.org/abs/2605.14892)
...
TL;DR: This survey reviews LLM-based autonomous agents and multi-agent systems, proposing a unified framework called LIFE progression, which consists of four stages: Lay the capability foundation, Integrate agents through collaboration, Find faults through attribution, and Evolve through autonomous self-improvement.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this survey is to address the limitations of current LLM-based autonomous agents and multi-agent systems, specifically the risk of error propagation and the lack of a unified framework for understanding the causal dependencies between agent capabilities, multi-agent collaboration, and agent self-evolution.

Method: The survey provides a systematic review of existing work, organized around the LIFE progression framework, which characterizes the dependencies between adjacent stages and identifies open challenges at stage boundaries.

Result: The survey reveals the causal dependencies between agent capabilities, multi-agent collaboration, and agent self-evolution, and proposes a cross-stage research agenda for closed-loop multi-agent systems capable of continuous self-improvement.

Conclusion: The survey aims to provide a systematic reference and a conceptual roadmap toward autonomous, self-improving multi-agent intelligence, bridging previously fragmented research threads and extending current coordination frameworks toward more self-organizing forms of collective intelligence.
...
### [291] [COREKG: Coreset-Guided Personalized Summarization of Knowledge Graphs](https://arxiv.org/abs/2605.14900)
*Sohel Aman Khan,Raghava Mutharaju,Supratim Shit*

Main category: cs.AI

TL;DR: The paper presents COREKG, a method for personalized knowledge graph summarization using coreset theory, which provides higher query-answering accuracy and structural coverage than state-of-the-art methods while requiring significantly less storage.


<details>
  <summary>Details</summary>
Motivation: Knowledge graphs are often very large and unwieldy for tasks such as question answering and visualization, making summarization crucial, especially personalized summarization to capture specific user requirements.

Method: The approach uses sensitivity-based importance sampling to select a relevant subset of triples for a given dataset and user-specific query workload, ensuring bounded approximation error.

Result: COREKG delivers higher query-answering accuracy and structural coverage than state-of-the-art methods (GLIMPSE, PPR, iSummary, PEGASUS, and APEX^2) on Freebase, WikiData, and DBpedia, while requiring only a tiny fraction of the original graph.

Conclusion: The proposed method, COREKG, effectively provides personalized knowledge graph summaries that are small, have significantly smaller storage requirements and query runtime, and maintain high query-answering accuracy and structural coverage.
...
### [292] [KGPFN: Unlocking the Potential of Knowledge Graph Foundation Model via In-Context Learning](https://arxiv.org/abs/2605.14907)
*Yisen Gao,Jiaxin Bai,Haoyu Huang,Zhongwei Xie,Yufei Li,Hong Ting Tsang,Sirui Han,Yangqiu Song*

Main category: cs.AI

TL;DR: KGPFN is a knowledge graph foundation model that combines transferable relational regularities with in-context learning for improved adaptability to unseen graphs.


<details>
  <summary>Details</summary>
Motivation: Existing knowledge graph foundation models focus on relation-level universality, but neglect in-context learning, which is crucial for effective prediction in knowledge graphs with structured and heterogeneous context.

Method: KGPFN uses a Prior-data Fitted Network to unify transferable relational regularities with inference-time in-context learning. It learns relation representations via message passing, encodes local neighborhoods using a multi-layer NBFNet, and constructs relation-specific global context through retrieval and aggregation.

Result: KGPFN achieves strong adaptation to previously unseen graphs through in-context learning alone, consistently outperforming competitive fine-tuned KG foundation models on 57 KG benchmarks.

Conclusion: KGPFN demonstrates the effectiveness of integrating transferable relational regularities with in-context learning for knowledge graph foundation models, achieving state-of-the-art results in adapting to unseen graphs.
...
### [293] [From Sycophantic Consensus to Pluralistic Repair: Why AI Alignment Must Surface Disagreement](https://arxiv.org/abs/2605.14912)
*Varad Vishwarupe,Nigel Shadbolt,Marina Jirotka*

Main category: cs.AI

TL;DR: The paper argues that current pluralistic alignment methods in AI, which focus on aggregating diverse human values, are insufficient. Instead, it proposes a new approach based on Grice's maxims, emphasizing conversational mechanisms like scoping, signaling, and repair to foster genuine value pluralism.


<details>
  <summary>Details</summary>
Motivation: The authors are motivated by the limitations of current pluralistic alignment methods in AI systems, which often result in sycophantic consensus rather than genuine value pluralism. They argue that this can have significant consequences in consequential deliberation across various domains.

Method: The paper proposes reframing pluralistic alignment around three conversational mechanisms: scoping, signaling, and repair. It also formalizes a metric, the Pluralistic Repair Score (PRS), to distinguish principled revision from capitulation.

Result: The authors present a small-scale empirical illustration using two frontier RLHF-trained models, Claude Sonnet 4.5 and GPT-4o, showing that both models exhibit agreement-following with low repair-quality on contested-value prompts.

Conclusion: The paper concludes that pluralism is most decisively made or unmade at the deployment-governance layer, including interfaces, preference-data pipelines, and audit infrastructure. The authors emphasize the importance of considering whose 'principled' counts in the context of pluralistic alignment.
...
### [294] [GraphFlow: An Architecture for Formally Verifiable Visual Workflows Enabling Reliable Agentic AI Automation](https://arxiv.org/abs/2605.14968)
*Drewry H. Morris,Luis Valles,Reza Hosseini Ghomi*

Main category: cs.AI

TL;DR: GraphFlow is a visual workflow system that aims to improve the reliability of agentic AI automation in multi-step processes by treating workflow diagrams as executable specifications, providing semantic correctness guarantees, and supporting audit and replay.


<details>
  <summary>Details</summary>
Motivation: The reliability of agentic AI automation in multi-step processes is compromised by small errors that compound rapidly, and existing workflow platforms lack semantic correctness guarantees.

Method: GraphFlow uses a visual workflow system that treats workflow diagrams as executable specifications, with a restricted class of diagrams that can be proof-checked before admission to a shared library, and a durable engine that records outcomes and enforces contracts at runtime.

Result: A year-long pilot executed 8,728 workflow runs with a 97.08% completion rate under an early prototype, with observed failures primarily localized to external integrations.

Conclusion: GraphFlow has the potential to improve the reliability of agentic AI automation in multi-step processes by providing semantic correctness guarantees and supporting audit and replay, but further evaluation of the verified core is needed.
...
### [295] [Learning Developmental Scaffoldings to Guide Self-Organisation](https://arxiv.org/abs/2605.14998)
*Milton L. Montero,Elias Najarro,Jakob Schauser,Sebastian Risi*

Main category: cs.AI

TL;DR: The paper explores how initial conditions, or pre-patterns, influence self-organisation in biological development, introducing a model that jointly learns self-organisation rules and pre-patterns, and demonstrating improvements in robustness and encoding capacity.


<details>
  <summary>Details</summary>
Motivation: The authors aim to understand how information is transferred to initial conditions in biological development, and how this affects self-organisation processes, motivated by the observation that many natural systems rely on initial conditions to drive complex organisation.

Method: The authors introduce a Neural Cellular Automaton (NCA) paired with a learned coordinate-based pattern generator (SIREN), both trained simultaneously to generate a set of patterns, allowing for the interplay between self-organisation rules and pre-patterns to be varied and measured under controlled conditions.

Result: The authors show that jointly learning both self-organisation rules and pre-patterns yields improvements in robustness, encoding capacity, and symmetry breaking over purely self-organising alternatives, and that effective pre-patterns bias the developmental dynamics to facilitate convergence.

Conclusion: The study highlights the importance of initial conditions in driving self-organisation in biological development, and demonstrates a model that can jointly learn self-organisation rules and pre-patterns, shedding light on the non-trivial relationship between the structure of initial conditions and the dynamics of self-organisation.
...
### [296] [Dual-Dimensional Consistency: Balancing Budget and Quality in Adaptive Inference-Time Scaling](https://arxiv.org/abs/2605.15100)
*Rongman Xu,Yifei Li,Tianzhe Zhao,Yanrui Wu,Bo Li,Hang Yan*

Main category: cs.AI

TL;DR: The paper proposes a unified framework called Dual-Dimensional Consistency (DDC) to improve the efficiency of Large Language Models (LLMs) in reasoning tasks by adaptively allocating computational resources to high-quality reasoning paths.


<details>
  <summary>Details</summary>
Motivation: Current strategies for maximizing the potential of LLMs through inference-time scaling face challenges in balancing sampling budget and reasoning quality, leading to inefficiencies.

Method: The proposed DDC framework combines Confidence-Weighted Bayesian protocol with Trend-Aware Stratified Pruning to concentrate computational resources on high-quality reasoning paths.

Result: Evaluations across five benchmarks show that DDC reduces token consumption by over 10 times while maintaining or exceeding the accuracy of strong baselines across various LLMs.

Conclusion: The proposed DDC framework effectively improves the efficiency of LLMs in reasoning tasks by adaptively allocating computational resources to high-quality reasoning paths, achieving significant reductions in token consumption without sacrificing accuracy.
...
### [297] [Why Neighborhoods Matter: Traversal Context and Provenance in Agentic GraphRAG](https://arxiv.org/abs/2605.15109)
*Riccardo Terrenzi,Maximilian von Zastrow,Serkan Ayvaz*

Main category: cs.AI

TL;DR: This paper examines the faithfulness of citations in Agentic GraphRAG systems, which use retrieval-augmented generation to improve factuality. The authors find that while cited evidence is necessary, it is not sufficient, and that accurate answers can depend on uncited context and graph structure.


<details>
  <summary>Details</summary>
Motivation: The authors are motivated by the complexity of evaluating citation faithfulness in Agentic GraphRAG systems, where an agent explores a knowledge graph before producing an answer and citations.

Method: The authors use controlled ablation experiments to compare the effects of isolating, removing, and masking cited and uncited graph entities.

Result: The results show that cited evidence is often necessary, but not sufficient, and that accurate answers can depend on uncited traversal context and surrounding graph structure.

Conclusion: The authors conclude that citation evaluation in Agentic GraphRAG should move beyond source support toward provenance over the broader retrieval trajectory.
...
### [298] [APWA: A Distributed Architecture for Parallelizable Agentic Workflows](https://arxiv.org/abs/2605.15132)
*Evan Rose,Tushin Mallick,Matthew D. Laws,Cristina Nita-Rotaru,Alina Oprea*

Main category: cs.AI

TL;DR: A new distributed multi-agent system architecture called APWA is proposed to efficiently process parallelizable tasks by decomposing workflows into independent subproblems.


<details>
  <summary>Details</summary>
Motivation: The current autonomous multi-agent systems based on large language models (LLMs) face bottlenecks in reasoning, coordination, and computational scaling as task complexity grows.

Method: The Agent-Parallel Workload Architecture (APWA) is introduced, which facilitates parallel execution by decomposing workflows into non-interfering subproblems that can be processed independently without cross-communication.

Result: APWA can dynamically decompose complex queries into parallelizable workflows and scale on larger tasks in settings where prior systems fail completely.

Conclusion: APWA offers a solution to efficiently process heavily parallelizable agentic workloads, supporting heterogeneous data and parallel processing patterns across various domains.
...
### [299] [OpenDeepThink: Parallel Reasoning via Bradley--Terry Aggregation](https://arxiv.org/abs/2605.15177)
*Shang Zhou,Wenhao Chai,Kaiyuan Liu,Huanzhi Mao,Qiuyang Mang,Jingbo Shang*

Main category: cs.AI

TL;DR: A new test-time compute framework called OpenDeepThink is proposed, which uses pairwise comparison to select the best candidate for improving LLM reasoning.


<details>
  <summary>Details</summary>
Motivation: Existing methods for improving LLM reasoning primarily scale depth by extending a single reasoning trace, but this can be limited by the noise and bias in pointwise LLM judging. The authors aim to address this by scaling breadth through parallel sampling and introducing a more robust selection method.

Method: OpenDeepThink uses a population-based approach, where the LLM judges random pairs of candidates and aggregates votes via Bradley-Terry into a global ranking. The top-ranked candidates are preserved and mutated using natural-language critiques, while the bottom quarter is discarded.

Result: OpenDeepThink raises Gemini 3.1 Pro's effective Codeforces Elo by +405 points in eight sequential LLM-call rounds (~27 minutes wall-clock). The pipeline transfers across weaker and stronger models without retuning, and gains appear concentrated in objectively verifiable domains.

Conclusion: The proposed OpenDeepThink framework demonstrates significant improvements in LLM reasoning, particularly in objectively verifiable domains, and showcases the potential of population-based test-time compute scaling.
