...
TL;DR: The paper proposes a principled measure of task-relevant information loss in visual text compression (VTC) and introduces a routing criterion and foveation mechanism to improve VTC's efficiency and accuracy.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the unpredictability of VTC's downstream utility despite its efficiency in token savings, and to provide a measure of task-relevant information loss induced by visual encoding.

Method: The method involves formulating VTC in the language of measure transport, treating text and visual tokens as empirical probability measures, and estimating the transport cost using downstream-label-free probes.

Result: The results show that the proposed label-free routing criterion matches the per-dataset oracle on 17/24 datasets (70.8%), and improves the average task score by +3.3% with -10.3% average tokens relative to a pure-LLM across 24 NLP datasets.

Conclusion: The conclusion is that the proposed approach provides a principled measure of task-relevant information loss in VTC and leads to improved efficiency and accuracy in NLP tasks.
...
TL;DR: Edge deep learning integrates edge computing and deep learning for real-time decision-making, particularly in computer vision applications like medical diagnostics.


<details>
  <summary>Details</summary>
Motivation: The integration of edge computing and deep learning to enable real-time decision-making in various domains, especially in medical diagnostics.

Method: Comprehensive review of edge deep learning, including foundational principles, technical advantages, hardware platforms, and implementation approaches.

Result: The survey provides a thorough understanding of edge deep learning's current state, its applications, and future directions, highlighting its potential to revolutionize various domains.

Conclusion: Edge deep learning has the potential to profoundly impact real-life situations, particularly in computer vision applications like medical diagnostics, and further investigations are needed to advance intelligent edge deep learning solutions.
...
TL;DR: The paper presents HumanNet, a large-scale video dataset of human interactions with the physical world, designed to advance embodied intelligence by providing a scalable and diverse data infrastructure.


<details>
  <summary>Details</summary>
Motivation: The lack of large, diverse, and richly annotated human activity data hinders progress in embodied intelligence, particularly in learning physical interactions.

Method: The authors introduce HumanNet, a one-million-hour human-centric video corpus with interaction-centric annotations, and a systematic data curation paradigm for embodied learning.

Result: The validation results show that continued training with 1000 hours of egocentric video from HumanNet surpasses continued training with 100 hours of real-robot data, indicating that egocentric human video could be a scalable and cost-effective substitute for robot data.

Conclusion: The paper aims to explore the opportunity to scale embodied foundation models using human-centric videos, rather than relying solely on robot-specific data, and presents HumanNet as a scalable substrate for representation learning, activity understanding, motion generation, and human-to-robot transfer.
...
TL;DR: The paper proposes R$^3$L, a framework for improving relative spatial reasoning in 3D layout generation, addressing issues of unreliable inferred relations and inconsistencies.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the reliability and consistency of relative spatial reasoning for 3D layout generation, as recent works using MLLMs often produce unreliable relations handled by post-hoc heuristics.

Method: The method involves three main components: invariant spatial decomposition to break coupled relation chains, consistent spatial imagination through an imagine-and-revise loop, and supportive spatial optimization via global-to-local coordinate re-parameterization.

Result: The results show that R$^3$L produces more physically feasible and semantically consistent layouts across diverse scene types and instructions, highlighting the importance of resolving frame-induced inconsistencies.

Conclusion: The conclusion is that R$^3$L is a general framework that improves the reliability and consistency of relative spatial reasoning for 3D layout generation, and that resolving frame-induced inconsistencies is crucial for reliable multi-hop relative spatial reasoning.
...
TL;DR: The paper introduces LookWhen, a selector-extractor framework for efficient video recognition, which achieves a better accuracy-computation trade-off than existing models.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the high computational cost of transformer-based video recognition models, which process video tokens with expensive superlinear computational cost despite the presence of redundancy in videos.

Method: The authors propose a selector-extractor framework called LookWhen, which factorizes video recognition into learning when, where, and what to compute. The framework consists of a shallow selector and a deep extractor. The selector quickly scores all tokens across space-time, while the extractor processes only the top-K selected tokens to approximate full-video representations.

Result: The results show that LookWhen achieves a better accuracy-computation trade-off than efficient models and upgraded baselines of similar size on various datasets, including Kinetics-400, SSv2, Epic-Kitchens, Diving48, Jester, and Charades. Specifically, LookWhen Pareto-dominates in accuracy-FLOPs on 9 of 12 cases and is 6.7x faster than InternVideo2-B at equal accuracy.

Conclusion: The paper concludes that LookWhen is an efficient and effective framework for video recognition, achieving a better accuracy-computation trade-off than existing models, and demonstrates its potential for real-world applications.
...
TL;DR: The paper proposes a transfer-aware data allocation method for pretraining vision foundation models across different 3D medical imaging modalities, which outperforms traditional data-proportional sampling by up to 58%.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the pretraining of vision foundation models across different 3D medical imaging modalities (CT, MRI, and PET) by optimizing data allocation, as current mixture strategies are largely heuristic.

Method: The method involves observing the variable scaling rates of different medical imaging domains during pretraining and the asymmetric knowledge transfer between domains. It formulates data allocation as a scaling-law optimization problem, deriving allocations that reveal a hub-and-island structure.

Result: The result shows that transfer-aware allocation outperforms data-proportional sampling by up to 58% and generalizes well to unseen budgets. Downstream validation on disease classification and organ/lesion segmentation confirms that the derived transfer-aware mixtures provide stronger pretrained representations.

Conclusion: The conclusion is that the proposed transfer-aware data allocation method can provide stronger pretrained representations for clinical 3D medical imaging tasks by strategically allocating data to highly transferable domains.
...
TL;DR: A new adaptive split operator called AdpSplit is introduced for 3D Gaussian Splatting (3DGS) to improve efficiency in training schedules by reducing the number of densification iterations while preserving rendering quality.


<details>
  <summary>Details</summary>
Motivation: The vanilla 3DGS binary split operator requires many densification rounds to expose fine details, which becomes a bottleneck for efficient training schedules with fewer iterations.

Method: AdpSplit determines the number of split children and initializes child parameters from L1-pixel-error region statistics, enabling adaptive density control.

Result: AdpSplit reduces training time by 9.2%-22.3% across multiple datasets and achieves a 16.4% reduction in training time with FastGS on MipNeRF360, corresponding to a 12.6x acceleration over vanilla 3DGS.

Conclusion: AdpSplit is an effective drop-in replacement for the standard split operator in 3DGS, significantly improving training efficiency without compromising rendering quality.
...
TL;DR: The paper proposes TriDE, a method for jointly estimating pairwise translation directions in camera location estimation, improving accuracy and downstream camera locations.


<details>
  <summary>Details</summary>
Motivation: Existing estimators process image pairs independently, producing locally plausible but inconsistent directions. The authors aim to jointly estimate directions to ensure consistency across the viewing graph.

Method: TriDE refines unreliable pairwise directions through message passing between directions and their incident weighted triangles, exploiting camera-triangle consistency.

Result: TriDE improves direction accuracy by a large margin and yields better downstream camera locations, establishing a strong phase-transition bound for exact recovery under a realistic random corruption model.

Conclusion: The proposed TriDE method provides a practical link between local pairwise estimation and global camera pose geometry, demonstrating improved performance on real image graphs.
...
TL;DR: The paper presents a method to detect and mitigate label bias in image segmentation datasets without needing clean, unbiased annotations, achieving equitable performance across demographic subgroups.


<details>
  <summary>Details</summary>
Motivation: Labeled datasets can reflect biases in their annotation pipelines, leading to label bias and systematic performance disparities across demographic subgroups, which is underexplored in image segmentation.

Method: A data-centric adaptation of Confident Learning is used to detect label bias directly in the training data by comparing provided labels to the model's confident predictions.

Result: The framework reliably detects and mitigates bias without access to clean labels, achieving equitable performance across experimental conditions on three datasets.

Conclusion: The proposed method effectively addresses label bias in image segmentation, enabling the detection and mitigation of bias without requiring clean annotations, and leading to more equitable performance across demographic subgroups.
...
TL;DR: A new inference algorithm called Heterogeneous Step Allocation (HSA) is proposed to improve the efficiency of Diffusion Transformers (DiTs) for video generation by dynamically allocating denoising steps to different tokens based on their velocity dynamics.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the inefficiency of standard inference in DiTs, which applies the same number of denoising steps uniformly to every token, despite the fact that human vision can ignore redundant motion.

Method: HSA assigns varying step budgets to different spatiotemporal tokens based on their velocity dynamics, and introduces a KV-cache synchronization mechanism and a cached Euler update to resolve sequence-length mismatch and advance latent states of skipped tokens.

Result: HSA significantly outperforms previous state-of-the-art caching methods and the vanilla Flow Matching baseline, especially at aggressive acceleration regimes, and achieves a superior quality-runtime Pareto frontier without expensive offline profiling.

Conclusion: HSA is a training-free inference algorithm that can improve the efficiency of DiTs for video generation while preserving structural integrity and generation quality, making it a promising solution for real-world applications.
...
TL;DR: The authors organized the SAFE: Synthetic Video Detection Challenge to address the need for reliable methods to detect synthetic media. The challenge involved 12 teams and over 600 submissions, focusing on detecting synthetic videos and content under various conditions.


<details>
  <summary>Details</summary>
Motivation: The proliferation of generative video technologies has intensified the need for reliable methods to detect and characterize synthetic media.

Method: The challenge consisted of two primary tasks: detection of synthetic video content and detection of synthetic content following post-processing operations. It used a dataset of 20 hours of 6,000 video samples from 13 modern synthetic video models and 21 diverse sources.

Result: The findings highlight measurable progress in cross-generator generalization but also persistent vulnerabilities to post-processing artifacts.

Conclusion: The paper describes the challenge design, dataset construction, evaluation methodology, and outcomes, offering insights into the generalization and robustness of contemporary synthetic video detection methods.
...
TL;DR: A$^2$RD, an Agentic Auto-Regressive Diffusion architecture, is proposed for synthesizing consistent and coherent long videos by decoupling creative synthesis from consistency enforcement.


<details>
  <summary>Details</summary>
Motivation: Existing methods for long video synthesis suffer from semantic drift and narrative collapse over long horizons.

Method: A$^2$RD formulates long video synthesis as a closed-loop process with a Retrieve--Synthesize--Refine--Update cycle, comprising Multimodal Video Memory, Adaptive Segment Generation, and Hierarchical Test-Time Self-Improvement.

Result: A$^2$RD outperforms state-of-the-art baselines by up to 30% in consistency and 20% in narrative coherence across public and LVBench-C benchmarks.

Conclusion: A$^2$RD demonstrates notable improvements in long video synthesis, with gains in consistency, narrative coherence, motion, and transition smoothness.
...
TL;DR: The paper presents XiYOLO, a framework for energy-efficient object detection on edge devices, achieving better accuracy-energy tradeoffs than YOLO baselines.


<details>
  <summary>Details</summary>
Motivation: Object detection on edge devices faces strict constraints on energy, latency, and memory while requiring reliable perception for autonomy. Existing energy-aware NAS methods have limitations in optimizing real energy consumption, which is device-dependent and costly to measure.

Method: The framework combines an energy-aware search space, a two-stage energy estimator, and iterative search to find an energy-efficient base architecture. It then applies compound scaling to create the XiYOLO family across different deployment budgets.

Result: XiYOLO achieves a stronger energy-accuracy tradeoff than YOLO baselines on PascalVOC and COCO datasets. Specifically, on PascalVOC, the medium XiYOLO model reaches 86.15 mAP50 while reducing energy by 20.6% on GPU and 35.9% on NPU compared to YOLOv12m.

Conclusion: The proposed framework, XiYOLO, enables efficient object detection on heterogeneous edge devices with improved energy-accuracy tradeoffs, validated through experiments on benchmark datasets and real-device deployment.
...
TL;DR: FuScore, a novel method for evaluating Infrared-Visible image fusion (IVIF) quality, utilizes a Multimodal Large Language Model (MLLM) to produce continuous quality scores, enabling fine-grained discrimination among fused images.


<details>
  <summary>Details</summary>
Motivation: Existing evaluation approaches for IVIF over-optimize hand-crafted statistics and full-reference metrics, and recent efforts using scalar regression on aggregated scores have limitations, such as not leveraging MLLMs and ignoring per-image perceptual ambiguity.

Method: FuScore uses an MLLM to mimic human visual perception, producing continuous quality scores, and constructs a per-image soft label based on agreement among IVIF-specific sub-dimensions.

Result: FuScore achieves state-of-the-art correlation with human visual preferences, demonstrating its effectiveness in evaluating IVIF quality.

Conclusion: FuScore provides a more accurate and fine-grained evaluation method for IVIF, addressing the limitations of existing approaches and improving the correlation with human visual preferences.
...
TL;DR: TrajGANR, a novel multimodal self-supervised learning framework, aligns continuous movement patterns in human mobility trajectories with static location-based observations, outperforming existing geospatial models in urban mobility and road understanding tasks.


<details>
  <summary>Details</summary>
Motivation: Existing geospatial multimodal self-supervised learning methods are limited to static pairs of modalities and neglect human mobility trajectories, which are crucial for urban understanding.

Method: TrajGANR learns a continuous neural representation of trajectories and aligns them with street-view images and geographic locations using a novel multimodal self-supervised learning objective.

Result: TrajGANR consistently outperforms existing geospatial multimodal self-supervised learning frameworks and a trajectory-specific foundation model in four urban mobility and road understanding tasks.

Conclusion: The proposed TrajGANR framework and multimodal self-supervised learning objective demonstrate the importance of fine-grained geospatial alignment and multimodal learning for improved urban understanding.
...
TL;DR: LensVLM, a framework for Vision Language Models (VLMs), enables efficient processing of text as images at high compression rates while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: The need to efficiently process text as rendered images in VLMs, as current methods suffer from accuracy loss at high compression rates.

Method: LensVLM uses a learned tool to selectively expand relevant images from compressed form to uncompressed form, allowing for efficient processing while maintaining accuracy.

Result: LensVLM maintains accuracy comparable to full-text upper bound at 4.3x effective compression and outperforms baselines up to 10.1x effective compression across seven text QA benchmarks.

Conclusion: LensVLM enables VLMs to efficiently process text as images at high compression rates while maintaining accuracy, and generalizes to multimodal document and code understanding tasks.
...
TL;DR: Proposed OneViewAll, a framework for 6D object pose estimation using a single real-world RGB-D reference view per object without CAD models, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods for 6D object pose estimation rely on explicit 3D models or multi-view data, which limits their scalability. The authors aim to address the challenging single-reference model-free setting.

Method: OneViewAll uses a novel Project-and-Compare paradigm, aligning reference and query observations within a projection-equivariant space, and progressively integrates hierarchical semantic priors across three levels.

Result: OneViewAll achieves 92.5% ADD-0.1 accuracy on the LINEMOD dataset using only one real reference view, outperforming the baseline One2Any (52.6%), and yields consistent improvements on other datasets.

Conclusion: The proposed OneViewAll framework demonstrates the efficacy of symmetry-aware projection in handling symmetric, texture-less, and occluded objects, achieving state-of-the-art performance with low inference latency.
...
TL;DR: A new foundation model called Pan-FM is proposed for multimodal medical imaging across seven organs. It uses a unified backbone and masking-based self-distillation to handle missing data and introduces Saliency-Guided Masking to prevent dominant-organ bias.


<details>
  <summary>Details</summary>
Motivation: Current foundation models in medical imaging are limited as they are trained on unimodal data within isolated domains. Human aging and disease involve coordinated biological processes across organs, necessitating multimodal models that learn whole-body representations.

Method: Pan-FM is pre-trained on imaging from seven organs under realistic missing-organ scenarios. It uses a unified backbone that handles organ missingness and masking-based self-distillation. Saliency-Guided Masking (SGM) is introduced to adaptively mask dominant organs during pre-training.

Result: Pan-FM achieves stronger prediction across 13 disease categories and 14 single disease entities than baselines, with improved robustness under missing-organ settings.

Conclusion: Pan-FM serves as a scalable solution to realistic modality-missingness in multimodal learning and a step toward more generalizable whole-body foundation models.
...
TL;DR: This paper introduces a novel self-supervised vision-language (VL) tracker that can track objects in a video sequence using natural language descriptions without requiring bounding-box ground truth.


<details>
  <summary>Details</summary>
Motivation: The goal is to achieve VL tracking without relying on bounding-box ground truth, which is a significant challenge in the field.

Method: The authors propose a Dynamic Token Aggregation Module that unequally treats each visual token and aggregates them into language tokens, enhancing semantic alignment and eliminating redundant visual token noise.

Result: The proposed tracker, called {\tracker}, surpasses state-of-the-art self-supervised methods on VL tracking benchmarks.

Conclusion: The paper presents a new approach for self-supervised VL tracking that enables effective learning of language-guided tracking representations without large-scale bounding box annotations.
...
TL;DR: The authors propose a new method, ODP-Net, to detect AI-generated images across unseen architectures by disentangling universal forgery traces from generator-specific fingerprints and semantic content.


<details>
  <summary>Details</summary>
Motivation: Existing models for detecting AI-generated images often overfit to generator-specific fingerprints and semantic content, failing to generalize to unseen architectures. The authors attribute this failure to feature entanglement.

Method: The authors propose the Orthogonal Decomposition and Purification Network (ODP-Net), which employs Instance-aware Orthogonal Decomposition, Perturbation-based Purification, and Manifold Alignment to structurally disentangle universal forgery traces from generator-specific fingerprints and semantic content.

Result: ODP-Net achieves state-of-the-art performance on unseen architectures, such as Stable Diffusion 3, validating that structural disentanglement is key to generalization.

Conclusion: The authors conclude that by explicitly decoupling universal forgery traces from generator-specific fingerprints and semantic content, ODP-Net can detect AI-generated images across unseen architectures, and that structural disentanglement is crucial for achieving generalization.
...
TL;DR: The paper proposes a new world model approach called RLA-WM, which uses a novel latent action representation called Residual Latent Action (RLA) and flow matching to predict future visual features, outperforming state-of-the-art models and enabling efficient robot learning.


<details>
  <summary>Details</summary>
Motivation: Existing world models focus on image generation, but visual feature-based models offer a more efficient and less prone to hallucination alternative; however, current feature-based approaches have limitations, such as blurry or collapsed predictions in complex interactions.

Method: The paper proposes RLA-WM, which uses RLA, a new type of latent action representation learned from DINO residuals, and flow matching to predict RLA values; it also develops two robot learning techniques using RLA-WM.

Result: RLA-WM outperforms state-of-the-art feature-based and video-diffusion world models on simulation and real-world datasets, while being orders of magnitude faster than video diffusion; it also enables efficient robot learning from offline videos.

Conclusion: The paper presents a novel world model approach, RLA-WM, which offers improved efficiency, accuracy, and generalizability, and demonstrates its effectiveness in robot learning applications.
...
TL;DR: ImplantMamba, a new AI model, accurately determines surgical guide implant positions by analyzing surrounding teeth texture, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: The challenge of precisely determining implant position in surgical guides due to the lack of distinctive texture in the implant region of medical images.

Method: ImplantMamba, a hybrid network architecture combining Convolutional Neural Networks (CNNs) with Mamba layers, and a Slope-Coupled Prediction Branch (SCP) for coupled regression of implant position and slope.

Result: Superior performance of ImplantMamba compared to existing methods on a large-scale dental implant dataset.

Conclusion: ImplantMamba offers a more comprehensive understanding of the implant site, leading to improved accuracy in determining surgical guide implant positions.
...
TL;DR: This paper introduces a two-axis view to analyze channel importance in vision networks, separating the questions of task relevance and local replaceability, and finds that local replaceability is a more reliable guide to removability than target relevance.


<details>
  <summary>Details</summary>
Motivation: The authors are motivated by the observation that channel importance in vision networks is usually summarized by a single score, which hides two different questions: how much a channel is related to the task, and whether its function can be supplied by same-layer peers when the channel is removed.

Method: The authors introduce a two-axis view that separates task relevance and local replaceability, and use a Gaussian linear analysis to account for the separation of the two axes during training.

Result: The authors find that the two axes are weakly aligned, induce different channel groupings, and separate rapidly during training, and that local-axis metrics are more reliable predictors of removability than target-axis metrics across different architectures and datasets.

Conclusion: The authors conclude that local replaceability is a more reliable guide to removability than target relevance, and that an axis-level distinction rather than a universal ranking of pruning scores is needed to understand channel importance in vision networks.
...
TL;DR: InfoGeo, an information-theoretic framework, enhances cross-view geo-localization robustness and generalization by maximizing view-invariant information and minimizing view-specific noise.


<details>
  <summary>Details</summary>
Motivation: Existing approaches to cross-view geo-localization suffer from domain shifts due to regional textures and weather conditions, particularly in UAV-based scenarios with dense, fine-grained objects.

Method: InfoGeo reformulates optimization as an information bottleneck process with two objectives: (i) aligning object-centric structural relations across views, and (ii) minimizing view-specific noisy signals through cross-view knowledge constraints.

Result: InfoGeo significantly outperforms state-of-the-art methods in extensive evaluations across diverse benchmarks and challenging scenarios.

Conclusion: InfoGeo provides a robust and generalizable solution for cross-view geo-localization, addressing domain shifts and visual clutter in UAV-based scenarios.
...
TL;DR: The authors propose a neurosymbolic approach to skeleton-based human activity recognition, enabling interpretable explanations grounded in logical structure while achieving competitive recognition performance.


<details>
  <summary>Details</summary>
Motivation: Most existing skeleton-based human activity recognition models are difficult to interpret and remain black boxes.

Method: The authors introduce a framework that bridges representation learning and symbolic inference by grounding first-order logic predicates in learnable spatial and temporal motion concepts. They employ a spatio-temporal skeleton encoder, a spatio-temporal concept decoder, and differentiable first-order logic layers.

Result: The approach achieves competitive recognition performance on NTU RGB+D 60/120 and NW-UCLA datasets while providing explicit, interpretable explanations.

Conclusion: The authors highlight neurosymbolic reasoning as an effective paradigm for interpretable spatio-temporal action understanding.
...
TL;DR: Qwen3-VL-Seg is a parameter-efficient framework for open-world referring segmentation that leverages multimodal large language models (MLLMs) to achieve precise pixel-level regions.


<details>
  <summary>Details</summary>
Motivation: Existing MLLMs have limitations in dense visual prediction, and recent MLLM-based segmentation methods have overheads or struggle with continuous object boundaries.

Method: Qwen3-VL-Seg uses a lightweight box-guided mask decoder that combines multi-scale spatial feature injection, spatial-semantic query construction, box-guided high-resolution pixel fusion, and iterative mask-aware query refinement.

Result: Qwen3-VL-Seg performs strongly across closed-set and open-world settings, with advantages on language-intensive instructions and out-of-distribution generalization.

Conclusion: Qwen3-VL-Seg offers a parameter-efficient solution for open-world referring segmentation, achieving precise pixel-level regions with minimal overhead.
...
TL;DR: AGA3DNet, a novel framework for 3D brain MRI subtype classification, integrates radiology report-derived anatomical phrases with a lightweight 3D CNN and xLSTM aggregation, achieving improved performance and clinically interpretable localization.


<details>
  <summary>Details</summary>
Motivation: The need for accurate 3D brain MRI subtype classification that leverages both localized anatomical cues and long-range contextual reasoning.

Method: AGA3DNet incorporates brief anatomical phrases from radiology reports as a soft anatomical prior channel, fusing it with a lightweight 3D CNN and multi-view xLSTM aggregation.

Result: AGA3DNet achieves improved overall balance across performance metrics and supports clinically interpretable localization through the prior channel.

Conclusion: The proposed AGA3DNet framework shows promise for 3D brain MRI subtype classification, offering improved performance and interpretability, but further evaluation on larger, diverse datasets is needed.
...
TL;DR: The paper proposes TriP, a robust triangle-based framework for translation averaging in Structure-from-Motion pipelines, which recovers camera locations from pairwise relative translation directions.


<details>
  <summary>Details</summary>
Motivation: The problem of translation averaging is challenging due to the lack of distance information in direction measurements, making it highly ill-conditioned and sensitive to corrupted observations.

Method: TriP infers local relative edge scales from triangle geometry and synchronizes the scales of overlapping triangles in the logarithmic domain to recover globally consistent edge lengths and camera locations.

Result: TriP outperforms all previous translation averaging methods on both synthetic and real datasets and is fully parallelizable, computationally efficient, and scalable to large graphs.

Conclusion: The proposed TriP framework provides a robust and efficient solution for translation averaging, leveraging higher-order consistency across triangles to handle corrupted observations and avoid collapse issues.
...
TL;DR: A unified network called UniV2D is proposed for underwater salient object detection, which jointly optimizes visual restoration and object detection.


<details>
  <summary>Details</summary>
Motivation: The conventional 'enhance-then-detect' paradigm for underwater salient object detection often leads to semantic inconsistency due to the separation of low-level visual restoration and high-level semantic perception.

Method: A Unified Vision-to-Detection Network (UniV2D) with a hierarchical dual-branch architecture is proposed, featuring a self-calibrated decoder, a mask-aware restoration module, and a saliency-guided refinement module.

Result: UniV2D significantly outperforms state-of-the-art methods in both quantitative and qualitative evaluations across multiple benchmarks.

Conclusion: UniV2D establishes a new standard for joint underwater perception by breaking the sequential bottleneck of traditional methods and achieving mutually beneficial visual restoration and salient object detection.
...
TL;DR: The paper finds that Vision-Language Models (VLMs) have a latent topological map of 3D scenes, but it's overshadowed by non-geometric visual semantics. The authors propose a method to isolate and shape this spatial subspace, leading to improved spatial reasoning in VLMs.


<details>
  <summary>Details</summary>
Motivation: To investigate whether VLMs construct a 3D internal representation analogous to human cognitive maps and to improve their spatial reasoning abilities.

Method: The authors use cross-scene linear feature extraction to isolate the spatial subspace in VLMs, mathematically shape the latent representation, and propose a latent regularization method based on Dirichlet energy.

Result: The proposed method yields significant improvements on real-world spatial benchmarks, outperforming standard supervised fine-tuning and competitive baselines by up to 12.1% in spatial tasks involving scene topology understanding.

Conclusion: The paper demonstrates that VLMs possess a latent topological map of 3D scenes, which can be isolated and shaped to improve their spatial reasoning abilities, leading to better performance on spatial benchmarks.
...
TL;DR: A new dataset, Real-IAD-MVN, is introduced to improve industrial anomaly detection by providing high-fidelity surface normal maps from multiple viewpoints, outperforming existing methods that use sparse 3D point clouds or 2D images.


<details>
  <summary>Details</summary>
Motivation: Existing industrial anomaly detection methods struggle with subtle, geometric defects, and standard 2D images or 3D point clouds have limitations in detecting micro-defects.

Method: A new dataset, Real-IAD-MVN, is created using an upgraded acquisition system to capture high-fidelity surface normal maps from five distinct viewpoints. A baseline method based on reconstruction is introduced to learn cross-modal unified prototypes from image and normal map streams.

Result: Experiments on the new dataset show that incorporating dense, multi-view pseudo-3D (surface normals) yields better detection performance than using sparse 3D point cloud data. The unified prototype approach surpasses existing state-of-the-art multimodal fusion methods.

Conclusion: The Real-IAD-MVN dataset provides a comprehensive geometric representation at a micro-detail level, making previously invisible defects detectable, and has the potential to advance geometric anomaly detection.
...
TL;DR: A novel framework, DPG-CD, is proposed for joint 2D semantic and 3D height change detection in urban areas using multi-temporal cross-modal data.


<details>
  <summary>Details</summary>
Motivation: The need to capture both 2D semantic changes and 3D height changes for urban morphology analysis and emergency management, while addressing the challenges of high acquisition costs and modality differences between data sources.

Method: A depth-prior-guided multi-temporal cross-modal fusion framework, DPG-CD, which introduces an estimated depth prior into imagery, uses a gated fusion mechanism, and employs a multi-stage cross-temporal cross-modal feature fusion architecture.

Result: DPG-CD outperforms state-of-the-art methods on both 2D and 3D change detection tasks on three datasets: Hi-BCD, 3DCD, and NYC-MMCD.

Conclusion: The proposed DPG-CD framework effectively addresses the challenges of joint 2D semantic and 3D height change detection in urban areas, providing a practical solution for urban morphology analysis and emergency management.
...
TL;DR: The paper proposes PRIMED, a model for Referring Audio-Visual Segmentation (Ref-AVS) that adaptively suppresses irrelevant modalities and enhances target object segmentation.


<details>
  <summary>Details</summary>
Motivation: Existing Ref-AVS methods treat multimodal cues as homogeneous inputs, making them vulnerable to irrelevant or misleading modalities.

Method: PRIMED consists of a Modality Prior Decoder, Token Distiller, and Spatial-Aware Semantic Alignment loss to adaptively guide attention, extract global visual tokens, and enhance foreground-background discrimination.

Result: PRIMED achieves state-of-the-art overall performance on the Ref-AVS benchmark.

Conclusion: The proposed PRIMED model effectively addresses the challenge of varying modality relevance in Ref-AVS by adaptively suppressing irrelevant modalities and enhancing target object segmentation.
...
TL;DR: A new framework called HiPerfGNN is proposed for non-invasive glioma molecular subtyping using dynamic susceptibility contrast (DSC) MRI and structural MRI.


<details>
  <summary>Details</summary>
Motivation: Current methods for glioma subtyping rely on invasive tissue sampling, and while deep learning on structural MRI has emerged as a non-invasive alternative, it cannot capture hemodynamic signatures that distinguish molecular subtypes.

Method: HiPerfGNN uses a vector-quantized variational autoencoder (VQ-VAE) to learn discrete hemodynamic representations from raw time-intensity curves, and a hierarchical graph neural network to propagate information across scales for molecular prediction.

Result: The model achieved AUCs of 0.96 (IDH), 0.89 (1p/19q), and 0.84 (WHO grade) on an internal cohort, and maintained robust IDH performance (AUC 0.89) on an independent external cohort without recalibration.

Conclusion: The proposed framework demonstrates the added value of integrating perfusion dynamics into radiogenomic pipelines for glioma molecular subtyping, and shows potential for clinical deployment.
...
TL;DR: The paper proposes S2M, a framework that utilizes ground-truth mask labels to obtain structured textual features for multimodal change detection in remote sensing imagery, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Current multimodal change detection methods rely on coarse or noisy text descriptions, overlooking the valuable information encoded in ground-truth mask labels.

Method: The proposed S2M framework involves (1) transcribing change regions into semantic quadruples (where, what, how, how many) and converting them into fixed-template text descriptions, and (2) using a two-stage training strategy with a multimodal decoder and bi-directional contrastive loss.

Result: S2M achieves a Sek of 17.80% and an F$_{\text{scd}}$ of 66.14% on the Gaza-Change-v2 dataset, outperforming multimodal methods that leverage large language models.

Conclusion: The paper demonstrates that ground-truth mask labels can be used to obtain precise and noise-free multimodal supervision, leading to improved change detection performance in remote sensing imagery.
...
TL;DR: A novel method, SatSurfGS, is proposed for sparse-view satellite image surface reconstruction using 2D Gaussian Splatting, improving rendering quality, surface accuracy, and efficiency.


<details>
  <summary>Details</summary>
Motivation: Sparse-view satellite image surface reconstruction is challenging due to spatially heterogeneous reliability of multi-view matching, affected by large photometric differences, weak textures, and repetitive textures.

Method: SatSurfGS, a generalizable sparse-view surface reconstruction method based on 2D Gaussian Splatting, with a coarse-to-fine Gaussian attribute prediction framework and explicit local geometric reliability modeling at three levels.

Result: The proposed method achieves improved rendering quality, surface reconstruction accuracy, cross-dataset generalization, and inference efficiency compared to baselines and optimization methods.

Conclusion: SatSurfGS offers a more effective solution for sparse-view satellite image surface reconstruction, enhancing the reliability and accuracy of surface reconstruction under challenging satellite imaging conditions.
...
TL;DR: PicoEyes is a unified gaze estimation framework that predicts key attributes of gaze from monocular or binocular inputs, achieving state-of-the-art performance in various settings.


<details>
  <summary>Details</summary>
Motivation: The motivation is to establish a practical, end-to-end paradigm for robust and generalizable gaze estimation in mixed reality (MR) applications.

Method: The framework uses a unified approach to directly predict 3D eye parameters, eye-region segmentation, optical axis, visual axis, and depth maps from input images.

Result: PicoEyes achieves state-of-the-art performance, consistently outperforming academic and industrial gaze tracking methods across various settings.

Conclusion: PicoEyes establishes a practical, end-to-end paradigm for robust and generalizable gaze estimation in MR applications, supporting 3D eye reconstruction and gaze forecasting.
...
TL;DR: Attention Transfer in Vision Transformers (ViTs) is not universally effective and depends on the architectural match between teacher and student models.


<details>
  <summary>Details</summary>
Motivation: To investigate the universality of Attention Transfer from pre-trained teacher ViTs to student ViTs and identify factors influencing its effectiveness.

Method: Comprehensive benchmark of 20 teachers from 11 well-known ViT families, controlled analyses, and experiments with varied model sizes, training durations, transfer datasets, and out-of-distribution evaluations.

Result: Attention Transfer fails for 4 out of 11 ViT families, with a performance drop of up to 5.1% compared to training from scratch. The failure is due to architectural mismatch between teacher and student, which can be resolved by adding teacher's native architectural components to the student.

Conclusion: The effectiveness of Attention Transfer in ViTs depends on the architectural match between the pre-trained teacher and the student model. When the architectures match, attention patterns can be transferred effectively, but when they do not, the transferred attention patterns may not be functional for the student model.
...
TL;DR: A new method for 3D reconstruction from blurry images using an asynchronous RGB-event camera system, achieving state-of-the-art performance under severe motion blur.


<details>
  <summary>Details</summary>
Motivation: Existing 3D reconstruction methods fail when input images are severely motion-blurred, and event-assisted approaches are limited by low-resolution sensors and strict synchronization.

Method: A flexible, high-resolution asynchronous RGB-event dual-camera system and a reconstruction framework that reconstructs sharp images from event data, then uses a cross-domain pose estimation module and structure-driven event loss for robust initialization and optimization.

Result: State-of-the-art performance on a new high-resolution RGB-event dataset and existing benchmarks, substantially improving reconstruction robustness under severe motion blur.

Conclusion: The proposed method enables robust and high-fidelity 3D reconstruction from blurry images, making it practical for handheld 3D capture on common devices like smartphones.
...
TL;DR: The paper proposes a new method called Closed-Form Linear-Probe Dataset Distillation (CLP-DD) for compressing large training sets into small synthetic sets while preserving downstream training utility, specifically for the setting of frozen pre-trained encoders and lightweight linear probing.


<details>
  <summary>Details</summary>
Motivation: Existing dataset distillation methods do not effectively exploit the properties of frozen-feature linear probing, which admits a closed-form solution determined directly by the pre-trained features. The authors aim to improve the efficiency and effectiveness of dataset distillation for modern visual transfer learning.

Method: The proposed CLP-DD method uses a bilevel formulation with a sample-space kernel ridge solver to compute the linear probe induced by the synthetic set. The synthetic images are then updated by evaluating this induced classifier on real features through a temperature-scaled softmax cross-entropy.

Result: CLP-DD substantially improves over LGM without DSA and approaches LGM with DSA at a fraction of the computational cost on ImageNet-100 with four pre-trained backbones. On ImageNet-1K, CLP-DD matches or surpasses LGM with DSA on three of four backbones while running roughly $14	imes$ faster and using less than one-eighth of the GPU memory.

Conclusion: The paper concludes that CLP-DD is an effective and efficient method for dataset distillation in the setting of frozen pre-trained encoders and lightweight linear probing, achieving comparable or better performance than state-of-the-art methods at a significantly lower computational cost.
...
TL;DR: The paper introduces ForeSight, a planning framework for autonomous driving that uses future scene imagination to make anticipatory decisions, outperforming reactive methods.


<details>
  <summary>Details</summary>
Motivation: Current autonomous driving planners are reactive, relying on historical and present observations. The authors argue that autonomous agents should imagine future scenes like human drivers do.

Method: ForeSight operates in two stages: generating plausible future visual worlds via a pretrained world model, and planning actions conditioned on these imagined futures.

Result: Extensive experiments on NAVSIM and nuScenes demonstrate that ForeSight significantly outperforms previous state-of-the-art alternatives.

Conclusion: The authors conclude that ForeSight enables genuinely anticipatory rather than reactive planning, navigating dynamic scenarios more effectively by grounding decisions in anticipated contexts.
...
TL;DR: The paper presents GS-DIFF, a method for scene change detection using Gaussian splatting, operating directly on primitives rather than rendered images, achieving multi-view consistency and separate scoring of geometric and appearance changes.


<details>
  <summary>Details</summary>
Motivation: Current scene change detection methods with Gaussian splatting follow a render-then-compare paradigm, treating the problem as a question about pixels. The authors aim to treat it as a question about primitives.

Method: The authors propose GS-DIFF, which uses anisotropic models of geometric and photometric drift, along with a per-primitive observability term, to compare scenes directly in primitive space.

Result: GS-DIFF surpasses the prior state-of-the-art approach by approximately 17% in mean Intersection over Union on real-world benchmarks.

Conclusion: The paper concludes that native primitive attributes are sufficient for scene change detection and that GS-DIFF offers advantages over render-then-compare methods, including multi-view consistency and separate scoring of changes.
...
TL;DR: A new network, LoHGNet, is proposed for infrared small target detection, integrating Lorentz geometric encoding with high-order relation learning to improve target discrimination in complex backgrounds.


<details>
  <summary>Details</summary>
Motivation: Infrared small target detection remains challenging due to scarcity of useful target cues and severe background clutter, and current methods have limitations in describing subtle differences of weak targets and contextual relations.

Method: LoHGNet, which uses Lorentz manifold based feature learning and a High-Order Relation Learning Module (HORL) to model contextual dependencies via hypergraph construction.

Result: Experimental results on three datasets demonstrate competitive performance in detection accuracy and adaptability to complex scenes.

Conclusion: The proposed LoHGNet offers a new approach to infrared small target detection, providing new discriminative cues and improving target discrimination in complex backgrounds.
...
TL;DR: The authors propose a framework called DINO-MVR for efficient medical segmentation using frozen foundation models, achieving strong performance with minimal annotations.


<details>
  <summary>Details</summary>
Motivation: Adapting foundation models to medical segmentation is challenging when annotations are scarce, and the authors aim to address this issue.

Method: The authors propose DINO-MVR, a Multi-View Readout framework that trains lightweight MLP probes on frozen DINOv3 backbone features and uses entropy-weighted fusion and spatial regularization for inference.

Result: DINO-MVR achieves strong performance on various benchmarks, including 0.895 Dice on Kvasir-SEG, 0.897 Dice on ISIC 2018, and 0.908 Dice on BraTS FLAIR whole-tumor segmentation, and recovers 98.4% of the performance obtained by the 40-patient BraTS reference run with only five annotated patients.

Conclusion: The authors conclude that frozen self-supervised vision backbones can support accurate medical segmentation when paired with an effective multi-view readout.
...
TL;DR: The authors propose CASCADE, a method for efficient autoregressive image generation using speculative decoding, achieving up to 3.6x acceleration without sacrificing image quality.


<details>
  <summary>Details</summary>
Motivation: Autoregressive image generation is computationally demanding and slow, and existing speculative decoding approaches fail to achieve significant efficiency gains due to high uncertainty in the target model.

Method: The authors identify patterns in the target model's behavior, formalizing properties of semantic interchangeability and convergence, and use these to improve speculative decoding by capturing redundancies in the predicted token tree and enhancing drafter performance.

Result: CASCADE achieves state-of-the-art speedups for drafter-based speculative decoding, with up to 3.6x acceleration, while maintaining image quality and text-prompt fidelity across multiple text-to-image models and drafter architectures.

Conclusion: The proposed method, CASCADE, effectively addresses the bottleneck in autoregressive image generation, achieving significant speedups without compromising image quality.
...
TL;DR: A novel multi-modal approach for detecting AI-generated video forgeries, which integrates semantic, visual, and audio branches, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: The increasing risk of semantic distortion and misuse of AI-generated videos necessitates reliable detection tools.

Method: A joint architecture integrating LMM semantic, spatio-temporal visual, and multi-scale partial-spoof audio branches for multi-modal detection and fine-grained temporal localization.

Result: The proposed approach outperforms existing state-of-the-art methods in detecting AI-generated video forgeries.

Conclusion: The study presents a robust multi-modal approach for detecting and localizing AI-generated video forgeries, addressing existing limitations in single- or partial-modality data modeling.
...
TL;DR: The paper reveals a vulnerability in visual context compression for MLLMs, where lower image resolution can lead to jailbreaking and decreased safety defenses.


<details>
  <summary>Details</summary>
Motivation: The motivation is to identify and address the risks associated with visual context compression in MLLMs, specifically the impact of image resolution on safety defenses.

Method: The authors conducted experiments with varying image resolutions and visual perturbations to observe the effect on safety defenses, and proposed a 'Structured Cognitive Offloading' strategy to mitigate the risks.

Result: The results show that lower image resolution leads to a sharp deterioration in safety defenses, even when text remains legible, and that the proposed strategy can effectively mitigate these risks.

Conclusion: The paper concludes that visual context compression poses a significant risk to the safety of MLLMs, and that careful design considerations are necessary to ensure secure future developments.
...
TL;DR: The paper proposes LENS, a noise modulation framework that accelerates image generation in diffusion models while maintaining quality.


<details>
  <summary>Details</summary>
Motivation: The motivation is to mitigate the trade-off between image generation speed and quality in distilled diffusion models.

Method: The method proposed is LENS (Low-frequency Eigen Noise Shaping), which operates in a low-dimensional subspace to efficiently modulate noise.

Result: LENS achieves competitive image quality while significantly reducing computational costs (FLOPs, model parameters, and inference-time overhead).

Conclusion: The proposed LENS framework effectively accelerates image generation in diffusion models without compromising image quality.
...
TL;DR: The paper presents a novel method for multi-view mesh reconstruction, introducing a compact polynomial kernel and stochastic regularization to improve the preservation of high-frequency details and geometric fidelity.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the challenge of recovering high-frequency geometry from sparse observations in multi-view mesh reconstruction, and to improve the joint optimization of geometry and appearance.

Method: The method proposed in this paper uses a compact polynomial kernel with local support and greater flexibility, allowing better control over frequency content and improved geometric fidelity. Additionally, stochastic regularization with Laplacian filtering is incorporated to enhance fine details.

Result: The paper reports state-of-the-art performance in both surface reconstruction and rendering, yielding more accurate geometry and sharper visuals from multi-view data.

Conclusion: The conclusion of this paper is that the proposed method improves the preservation of high-frequency structure while maintaining stable optimization, leading to more accurate geometry and sharper visuals in multi-view mesh reconstruction.
...
TL;DR: Proposed TAS-LoRA, a method that mitigates feature collapse in Transformer architecture search (TAS) by introducing low-rank adaptation and a Mixture-of-LoRAExperts strategy, improving performance over state-of-the-art TAS methods.


<details>
  <summary>Details</summary>
Motivation: Existing TAS methods suffer from feature collapse due to shared weights in a supernet, limiting subnet performance.

Method: TAS-LoRA uses parameter-efficient low-rank adaptation (LoRA) and a Mixture-of-LoRAExperts (MoLE) strategy to enable subnet-specific feature learning.

Result: TAS-LoRA improves performance over state-of-the-art TAS methods on ImageNet and several transfer learning benchmarks.

Conclusion: TAS-LoRA effectively mitigates feature collapse in TAS, leading to improved performance and efficiency.
...
TL;DR: The paper introduces AdaptSP, a method to address the semantic collapsing problem in generative personalization by adjusting the embedding of personalized concepts to improve prompt fidelity and contextual alignment.


<details>
  <summary>Details</summary>
Motivation: The semantic collapsing problem (SCP) in generative personalization, where a learned personalized concept overpowers the rest of the text prompt, causing the model to ignore important contextual details.

Method: Test-time Embedding Adjustment with Adaptive Subspace Projection (AdaptSP), a training-free method that uses the stable, pre-trained embedding as an anchor to isolate and adjust the semantic drift responsible for SCP.

Result: The experiments show that AdaptSP significantly improves prompt fidelity and contextual alignment.

Conclusion: The proposed method, AdaptSP, effectively mitigates the semantic collapsing problem in generative personalization, leading to better contextual alignment and prompt fidelity.
...
TL;DR: Sat3R is a feed-forward framework for accurate Digital Surface Model (DSM) reconstruction from satellite imagery, achieving competitive accuracy with optimization-based methods but with much faster computation.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for DSM reconstruction face a trade-off between accuracy and computation time, with optimization-based methods being accurate but slow and generalizable geometry foundation models being fast but inaccurate due to domain gap.

Method: Sat3R uses RPC-aware metric depth fine-tuning of Depth Anything V2 with Scale-Invariant Logarithmic (SiLog) loss and constructs physically consistent pseudo depth supervision from RPC geometry to adapt a monocular depth foundation model to the satellite domain.

Result: Sat3R reduces MAE by 38% over zero-shot feed-forward baselines and achieves competitive accuracy against optimization-based methods, with over 300x speedup.

Conclusion: Sat3R demonstrates that feed-forward models can match optimization-based accuracy at a fraction of the computational cost, paving the way for practical large-scale satellite DSM reconstruction.
...
TL;DR: CloudWeb is an attack on multimodal RAG systems that modifies input images to hijack atmospheric evidence retrieval, leading to compromised evidence and potentially false responses.


<details>
  <summary>Details</summary>
Motivation: The study aims to address the underexplored area of input-space threats to the evidence retrieval stage of remote sensing multimodal RAG systems.

Method: CloudWeb overlays cloud- and haze-like patterns on remote sensing images and optimizes them with a retrieval-oriented objective to pull adversarial image embeddings toward target atmospheric evidence.

Result: CloudWeb consistently outperforms baselines in injecting weather-related evidence into top-ranked results, with a significant increase in Weather@5 from 0.71% to 43.29% on GeoRSCLIP ViT-B/32.

Conclusion: The study reveals a practical failure mode where natural-looking atmospheric changes can compromise evidence retrieval before generation begins, highlighting the vulnerability of multimodal RAG systems to input-space threats.
...
TL;DR: SplatWeaver is a novel view synthesis framework that dynamically allocates Gaussian primitives to different regions, improving rendering quality and efficiency.


<details>
  <summary>Details</summary>
Motivation: Current methods allocate a fixed number of Gaussians to each pixel or voxel, wasting primitives in smooth regions and providing insufficient capacity for complex geometry and high-frequency details.

Method: SplatWeaver uses cardinality Gaussian experts and a pixel-level routing scheme to adaptively determine the number of Gaussian primitives for each spatial location.

Result: SplatWeaver consistently outperforms state-of-the-art methods, delivering more faithful novel-view renderings with fewer Gaussian primitives.

Conclusion: The proposed SplatWeaver framework enables a more expressive yet compact 3D scene representation, improving generalizable novel view synthesis.
...
TL;DR: The paper proposes Sword, a robust World Model framework that improves the generalization and reliability of Vision-Language-Action models by introducing Structure-Guided Style Augmentation and Dynamic Latent Bootstrapping.


<details>
  <summary>Details</summary>
Motivation: Existing World Models suffer from poor generalization, long-horizon error accumulation, and sensitivity to initial-state perturbations, limiting their reliability as simulators.

Method: The proposed Sword framework uses Structure-Guided Style Augmentation to disentangle visual textures from task-relevant dynamics and Dynamic Latent Bootstrapping to maintain consistency between training and inference.

Result: Extensive experiments on the LIBERO benchmark show that Sword significantly outperforms the baseline WoVR in terms of generalization, generation quality, robustness, fidelity, and the success rate of reinforcement-learning post-training for VLA models.

Conclusion: The proposed Sword framework mitigates the problems of existing World Models, providing a more reliable and robust approach for integrating Vision-Language-Action models with World Models.
...
TL;DR: A new benchmark, EgoPro-Bench, is introduced for training and evaluating proactive interaction capabilities in multimodal large language models, enabling more accurate and timely human-machine interactions.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal large language models are primarily reactive and lack the ability to continuously perceive environments or proactively assist users, with current benchmarks neglecting personalized context and precise timing of interactions.

Method: EgoPro-Bench is proposed, leveraging simulated user profiles and streaming egocentric videos to generate diverse user intentions and high-fidelity interaction data, along with a specialized evaluation protocol and metrics.

Result: Experiments demonstrate that EgoPro-Bench substantially enhances the intention understanding capabilities of multimodal large language models and enables accurate identification of appropriate timings for human-machine interactions.

Conclusion: EgoPro-Bench lays a solid foundation for next-generation user-centric proactive interactive agents by improving the proactive interaction capabilities of multimodal large language models.
...
TL;DR: The paper proposes a new quantization method called Amortized-Precision Quantization (APQ) and a bi-level framework called Mutual Adaptive Quantization with Early Exiting (MAQEE) to improve the stability and efficiency of Vision Transformers (ViTs) with low-precision early exiting.


<details>
  <summary>Details</summary>
Motivation: The deployment of Vision Transformers (ViTs) with low-precision early exiting remains fragile due to the instability of existing quantization methods when exit decisions are perturbed by quantization noise.

Method: The authors introduce APQ, a utilization-aware formulation that accounts for layer-wise stochastic exposure to quantization noise, and MAQEE, a bi-level framework that jointly optimizes exit thresholds and bit-widths under explicit risk control.

Result: MAQEE establishes a superior Pareto frontier in the accuracy-efficiency trade-off, reducing BOPs by up to 95% while maintaining accuracy and outperforming strong baselines by up to 20% across classification, detection, and segmentation tasks.

Conclusion: The proposed method MAQEE improves the stability and efficiency of ViTs with low-precision early exiting, achieving a better trade-off between accuracy and efficiency.
...
TL;DR: A new LiDAR world model called GEM is proposed, which uses a deformable Mamba architecture to improve the fidelity and imaginative capability of simulating environmental dynamics in autonomous driving.


<details>
  <summary>Details</summary>
Motivation: The progress in LiDAR-based world models has been lagging behind those built on camera videos or occupancy data due to the inherent disorder of LiDAR point clouds and the difficulty of distinguishing dynamic objects from static structures.

Method: GEM leverages a custom LiDAR scene tokenizer, unsupervised disentanglement of tokenized features via a dynamic-static separator, and a tri-path deformable Mamba for selective scanning and adaptive gating fusion.

Result: GEM achieves state-of-the-art performances across diverse benchmarks and evaluation settings, demonstrating its superiority and effectiveness.

Conclusion: The proposed GEM model shows promising results in addressing the challenges of LiDAR-based world models, and has the potential to generate 'what-if' scenarios for autonomous rollout.
...
TL;DR: A simplified one-step distillation method for diffusion models using a single drifting loss and intermediate hidden states, achieving competitive image quality and diversity.


<details>
  <summary>Details</summary>
Motivation: Existing distillation methods for diffusion models are complex and require multiple auxiliary networks or carefully designed training stages.

Method: Using a single drifting loss and intermediate hidden states of the pretrained teacher model as feature representation, with a lightweight mode coverage loss to mitigate mode collapse.

Result: Achieving FID scores of 1.58 on ImageNet-64x64 and 18.4 on SDXL, with efficient one-step generation and competitive image quality and diversity.

Conclusion: The proposed method simplifies the distillation framework while preserving semantically meaningful feature geometry and encouraging diverse teacher-supported regions.
...
TL;DR: GC-ART, a lightweight module, improves image classification robustness by predicting adaptive tone curves, achieving better results than baselines under various corruptions.


<details>
  <summary>Details</summary>
Motivation: The authors aim to enhance image classification robustness against various corruptions, such as darkening and contrast corruption, with a lightweight pre-processing module.

Method: GC-ART uses a 643-parameter MLP to predict a rational tone curve from per-channel soft histograms, applies the curve pointwise, and trains end-to-end with cross-entropy and a soft monotonicity penalty.

Result: GC-ART matches or improves upon baseline accuracy under various corruptions, including 48.45% on contrast corruption, and uses substantially fewer FLOPs than convolutional learned enhancers.

Conclusion: GC-ART effectively learns useful global tone corrections, preserving edge locations, and has potential for systematic improvement with tuned hyperparameters.
...
TL;DR: RCoT-Seg, a video-of-thought framework, improves video reasoning segmentation by separating temporal reasoning and spatial perception, achieving better performance than state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods for video reasoning segmentation (VRS) have limitations, including narrow-scope keyframe choices and brittle localization in complex scenes, due to limited supervision and frame-language similarity rules.

Method: RCoT-Seg factorizes VRS into temporal video reasoning (TVR) and keyframe target perception (KTP), using an agentic keyframe selection module and SAM2-based methods for high-resolution segmentation and mask propagation.

Result: RCoT-Seg achieves favorable performance against state-of-the-art methods, demonstrating improved temporal reasoning and spatial precision.

Conclusion: RCoT-Seg offers a more effective approach to video reasoning segmentation by explicitly separating temporal reasoning and spatial perception, leading to better performance and potential for future applications.
...
TL;DR: A new dataset called ShellfishNet is created for monitoring shellfish biodiversity using AI, comprising 8,691 images across 32 taxa, to improve the robustness of vision models in complex underwater environments.


<details>
  <summary>Details</summary>
Motivation: The decline of global shellfish biodiversity threatens coastal ecosystems, and existing marine benthic datasets lack adaptation to real-world complexities, making it challenging to generalize vision models for ecological monitoring.

Method: The ShellfishNet dataset is constructed through field photography and web scraping, and 80 representative neural network models are evaluated, including CNNs, ViTs, SSMs, and SSL methods.

Result: The study evaluates the performance of various models, including FGVC models and MLLMs, and assesses their robustness using image corruption benchmark tests, enabling trustworthy decisions on ecological protection.

Conclusion: ShellfishNet provides a comprehensive data foundation and model-evaluation benchmark for intelligent monitoring of benthic organisms, addressing the challenges of real-world ecological monitoring.
...
TL;DR: SoLAR is a novel free-viewpoint video framework that maintains stable reconstruction quality on long sequences without group-of-pictures partitioning, achieving state-of-the-art performance with minimal storage overhead.


<details>
  <summary>Details</summary>
Motivation: Previous methods for free-viewpoint video suffer from performance degradation on long video sequences, motivating the development of an error-resilient streamable framework.

Method: SoLAR, which utilizes Anchor Activation Dynamics (AAD) and Latent Discrepancy Aware Recalibration (LaDAR) to enable dynamic anchors and mitigate error propagation.

Result: SoLAR achieves state-of-the-art reconstruction performance on long-horizon free-viewpoint video while maintaining minimum storage overhead.

Conclusion: SoLAR advances the practical deployment of immersive systems by providing a new direction for long-horizon free-viewpoint video reconstruction.
...
TL;DR: The paper proposes SplitGS-Loc, a framework for improving visual localization using Gaussian Splatting-based Feature Fields (GSFFs) by addressing issues with 2D-3D matching.


<details>
  <summary>Details</summary>
Motivation: The authors highlight that photometrically optimized GSFFs are not suitable for 2D-3D matching due to many-to-one pixel-to-point mappings and superfluous Gaussians without multi-view consistency.

Method: The authors propose a Mixture-of-Gaussians-based splitting approach to decompose each Gaussian into smaller ones, creating precise one-to-one correspondences, and exploit composition weights to select consistent and discriminative Gaussians.

Result: The proposed framework, SplitGS-Loc, achieves state-of-the-art performance on localization benchmarks, enabling stable PnP convergence without per-scene training or iterative pose refinement.

Conclusion: The paper concludes that SplitGS-Loc extends the utility of photometric GSFFs to accurate and efficient localization by exploiting Gaussian attributes.
...
TL;DR: Proposed Temporal Token Fusion (TTF), a training-free method to compress visual tokens in video-language models, reducing inference costs while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Video-language models face high inference costs due to the large number of visual tokens generated from video frames, causing LLM prefill to become a throughput bottleneck.

Method: TTF compresses visual tokens by selecting an anchor frame, performing local window similarity search, and fusing tokens that exceed a threshold, maintaining positional consistency.

Result: TTF removes about 67% of visual tokens while retaining 99.5% of baseline accuracy and introducing only ≈0.16 GFLOPs of matching overhead on Qwen3-VL-8B.

Conclusion: TTF offers a practical and efficient solution for video understanding by reducing visual token counts without sacrificing accuracy.
...
TL;DR: A unified multimodal framework for joint 2D-3D semantic segmentation is proposed, combining 2D and 3D encoders to extract shared and private features, and achieving improved segmentation accuracy and generalization.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenges of cross-modal alignment and stable fusion in 2D-3D semantic segmentation, leveraging the shared semantics between 2D and 3D features.

Method: The method combines a SAM-based vision encoder with a SPTNet-based geometric encoder, decomposes features into shared and private subspaces, and uses a lightweight attention-based fusion module.

Result: The proposed framework achieves consistent improvements in segmentation accuracy over multimodal baselines on SemanticKITTI and nuScenes benchmarks, with competitive computational efficiency and strong generalization under distribution shifts.

Conclusion: The proposed unified multimodal framework demonstrates effectiveness in joint 2D-3D semantic segmentation, with improved accuracy, efficiency, and generalization capabilities.
...
TL;DR: A novel ISP framework, UniISP, is proposed to generate images that are both visually appealing to humans and effective for computer vision tasks.


<details>
  <summary>Details</summary>
Motivation: Raw sensor data provides richer information than RGB images, especially in challenging conditions. Traditional ISP pipelines can compromise information integrity, and existing methods for computer vision tasks often don't align with human visual preferences.

Method: UniISP framework, incorporating a Hybrid Attention Module (HAM) and a Feature Adapter module, uses supervised learning to generate visually appealing images and effectively propagate features to downstream networks.

Result: State-of-the-art performance across various scenarios and multiple datasets, demonstrating generalizability and effectiveness.

Conclusion: The proposed UniISP framework successfully meets the requirements of both human visual perception and computer vision applications.
...
TL;DR: RELO, a reinforcement-learning-to-localize method, improves visual object tracking by learning a localization policy and achieving superior results on multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Conventional visual object trackers use handcrafted spatial priors which are not well aligned with tracking optimization and evaluation metrics.

Method: RELO formulates target localization as a Markov decision process and uses reinforcement learning to learn a localization policy with rewards combining IoU and AUC.

Result: RELO achieves 57.5% AUC on LaSOText without template updates and superior results across multiple benchmarks.

Conclusion: Reward-driven localization provides an effective alternative to prior-driven localization for visual object tracking.
...
TL;DR: Proposed YOLO-MD, a detection framework for marine debris in low-quality images, outperforming state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Marine debris detection is crucial for ecological protection, but low-quality images with blur, complex backgrounds, and small targets degrade performance.

Method: Enhanced YOLO-based detection framework (YOLO-MD) with Dual-Branch Convolutional Enhanced Self-Attention (DB-CASA) module, lightweight shift-based operation, and SFG-Loss.

Result: YOLO-MD achieves 0.875 precision, 0.822 F1-score, and 0.849 mAP50 on UODM dataset, outperforming state-of-the-art methods.

Conclusion: YOLO-MD is effective for marine debris detection in low-quality images, verified through experiments on UODM dataset and real-world robotic edge deployment.
...
TL;DR: The paper proposes ST-Gen4D, a 4D generation framework that incorporates 4D spatiotemporal cognition to improve the generation of coherent and dynamic 4D content.


<details>
  <summary>Details</summary>
Motivation: Existing 4D generative models struggle to capture local dynamics and ensure spatiotemporal consistency in the physical world, leading to a need for a more comprehensive approach.

Method: The authors propose a 4D generation framework called ST-Gen4D, which includes four key components: spatiotemporal representation, spatiotemporal cognition, spatiotemporal reasoning, and spatiotemporal generation.

Result: The proposed ST-Gen4D model outperforms existing methods in 3D and 4D generation tasks, as demonstrated through extensive experiments on aggregated public 4D datasets and a self-built subset.

Conclusion: The paper concludes that the proposed ST-Gen4D framework, which integrates 4D intrinsic cognition with generative priors, can guarantee structural rationality and topological consistency in 4D generation.
...
TL;DR: The authors propose a balanced reinforcement learning framework for image captioning that optimizes utility, correctness, and linguistic quality, achieving consistent improvements in caption quality across different models.


<details>
  <summary>Details</summary>
Motivation: Existing image captioning methods and evaluation metrics focus on a narrow notion of caption quality, causing trade-offs across key dimensions, such as utility, fluency, and descriptiveness.

Method: The authors propose a multi-objective reinforcement learning framework that jointly optimizes utility-aware correctness, reference coverage, and linguistic quality, using GDPO-style reward-decoupled normalization and length-conditional reward masking.

Result: The proposed method consistently improves caption quality across LLaVA-1.5-7B and Qwen2.5-VL 3B and 7B base models, with peak gains of +13.6 DCScore, +9.0 CaptionQA, and +29.0 CapArena.

Conclusion: The authors' proposed framework provides a more balanced approach to image captioning, addressing the limitations of existing methods and achieving improved caption quality across different models.
...
TL;DR: The paper proposes SpInShield, a defense framework to protect spatiotemporal deepfake detectors from evasion attacks by decoupling semantic motion from manipulatable spectral artifacts.


<details>
  <summary>Details</summary>
Motivation: Spatiotemporal deepfake detectors are vulnerable to evasion attacks because they overfit on fragile temporal spectrum cues rather than learning robust semantic causality.

Method: The authors propose a learnable spectral adversary that dynamically synthesizes severe spectral deformations and employ a shortcut suppression optimization strategy to compel the encoder to extract reliable forensic cues.

Result: SpInShield obtains competitive performance on widely used datasets and outperforms the strongest baseline by 21.30 percentage points in AUC under simulated amplitude spectral attacks.

Conclusion: The proposed SpInShield framework effectively mitigates the vulnerability of spatiotemporal deepfake detectors to evasion attacks by decoupling semantic motion from manipulatable spectral artifacts.
...
TL;DR: The paper reveals a vulnerability in Diffusion Vision-Language Models (dVLMs) to jailbreak attacks, proposing a new paradigm called Global Probability Optimization (GPO) and a visual-modality jailbreak framework called GPO-V.


<details>
  <summary>Details</summary>
Motivation: The authors investigate the safety of dVLMs and find that they are vulnerable to jailbreak attacks, which could have significant implications for their deployment.

Method: The authors propose a new jailbreak paradigm called Global Probability Optimization (GPO) and a visual-modality jailbreak framework called GPO-V, which manipulates the global generative dynamics of diffusion models.

Result: The empirical results demonstrate that GPO-V produces stealthy perturbations with exceptional cross-model transferability, revealing a critical security gap in non-sequential generative architectures.

Conclusion: The findings underscore the critical urgency of addressing safety alignment in dVLMs and necessitate an immediate and fundamental re-evaluation of current defense paradigms to mitigate the unique risks of diffusion-based generation.
...
TL;DR: A novel method called InsHuman is proposed for natural and identity-preserving human insertion into a target background.


<details>
  <summary>Details</summary>
Motivation: Existing image editing models often produce failure cases when inserting humans into a new background, such as incorrect human pose, inconsistent number of people, and modified facial identity.

Method: InsHuman consists of Human-Background Adaptive Fusion (HBAF) for coherent adaptation of human regions to the target background, Face-to-Face ID-Preserving (FFIP) for identity consistency, and Bidirectional Data Pairing (BDP) strategy for constructing a high-quality dataset.

Result: InsHuman achieves significant improvements in generating plausible images while keeping human identity unchanged.

Conclusion: The proposed InsHuman method effectively addresses the challenges of human insertion, producing natural and identity-preserving results.
...
TL;DR: A new benchmark for chart referring expression grounding is introduced, addressing limitations in existing benchmarks. A code-driven synthesis pipeline and instance segmentation model improve performance on this benchmark and a real-chart grounding benchmark.


<details>
  <summary>Details</summary>
Motivation: Existing chart referring expression grounding benchmarks have limitations, including constrained localization precision, assumption of single or two referred target instances, over-reliance on textual cues, and limited chart types.

Method: A chart referring expression grounding benchmark is introduced, supporting multiple localization forms, multiple referred targets, diverse grounding cues, and diverse chart types. A code-driven synthesis pipeline and instance segmentation model are also proposed.

Result: Representative multimodal large models show a significant performance gap on the new benchmark. The proposed system consistently outperforms baselines on the new benchmark and generalizes well to a real-chart grounding benchmark.

Conclusion: The introduced benchmark and proposed system improve chart referring expression grounding, addressing existing limitations and outperforming baselines.
...
TL;DR: The paper proposes a method to recover accurate metric depth from monocular depth estimation (MDE) using sparse metric anchors, by modeling the scale field as a linear combination of image-adaptive basis maps.


<details>
  <summary>Details</summary>
Motivation: Monocular depth estimation (MDE) typically produces depth estimations that are defined up to an unknown scale or shift, and recovering accurate metric depth becomes challenging when only sparse metric anchors are available.

Method: The method formulates metric depth recovery as image-adaptive scale field modeling, where the correction is reformulated as a low-dimensional linear combination of image-adaptive basis maps derived from semantic and geometric cues.

Result: The approach yields improved metric depth accuracy, strong robustness under extreme anchor sparsity, and an interpretable decomposition of spatial scale variations.

Conclusion: The proposed approach demonstrates effectiveness and general applicability across multiple datasets and representative MDE models.
...
TL;DR: Proposed MagicBokeh, a unified framework for high-quality and efficient bokeh rendering on mobile devices, especially for low-resolution images.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with photos taken under high digital zoom levels, which have reduced resolution and lose fine details, and two-stage pipelines reduce efficiency and introduce error accumulation.

Method: A unified diffusion-based framework that jointly optimizes bokeh rendering and super-resolution through an alternative training strategy and focus-aware masked attention mechanism.

Result: MagicBokeh efficiently produces photorealistic bokeh effects, especially on real-world low-resolution images.

Conclusion: MagicBokeh paves the way for future advancements in bokeh rendering by enabling high-quality and efficient rendering on mobile devices.
...
TL;DR: Proposed a novel, lightweight framework called SAEgis for detecting adversarial attacks on vision-language models (VLMs) using sparse autoencoders (SAEs).


<details>
  <summary>Details</summary>
Motivation: The safety of VLMs has received limited attention despite their rapid advancement and deployment in real-world applications. Existing VLMs are vulnerable to adversarial attacks, posing significant risks to downstream applications.

Method: The proposed framework, SAEgis, involves inserting an SAE module into a pretrained VLM and training it with standard reconstruction objectives to capture attack-relevant signals in sparse latent features.

Result: SAEgis achieved strong performance in detecting adversarial attacks across various settings, including in-domain, cross-domain, and cross-attack scenarios, with notable improvements in cross-domain generalization.

Conclusion: The proposed method provides a practical approach for improving the safety of real-world VLM systems with minimal overhead and no additional adversarial training.
...
TL;DR: EditTransfer++ is a framework for visual-prompt-guided edit transfer that improves faithfulness and efficiency in image transformations.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion transformer-based methods for visual-prompt-guided edit transfer often fail to reproduce demonstrated edits due to structural mismatches and stochastic instability.

Method: EditTransfer++ combines progressively structured training with an efficient conditioning scheme, including text-decoupled training and a best-worst contrastive refinement mechanism.

Result: EditTransfer++ achieves state-of-the-art visual prompt faithfulness with substantially faster inference than prior methods.

Conclusion: EditTransfer++ offers a promising direction for scalable prompt-guided image editing and broader visual in-context learning.
...
TL;DR: A new framework called EditRefiner is proposed for fine-grained image editing refinement, leveraging a dataset of human feedback and a hierarchical agentic approach to improve image editing quality.


<details>
  <summary>Details</summary>
Motivation: Existing image editing refinement approaches have limitations, such as relying on costly iterative regeneration or employing vision-language models with weak spatial grounding, leading to semantic drift and unreliable local corrections.

Method: A hierarchical, interpretable, and human-aligned agentic framework called EditRefiner, which includes perception, reasoning, action, and evaluation agents to perform post-editing correction in a human-like perception-reasoning-action-evaluation loop.

Result: EditRefiner consistently outperforms state-of-the-art methods in distortion localization, diagnose accuracy, and human perception alignment, establishing a new paradigm for self-corrective and perceptually reliable image editing.

Conclusion: The proposed EditRefiner framework offers a more reliable and perceptually aligned approach to fine-grained image editing refinement, leveraging human feedback and a hierarchical agentic approach.
...
TL;DR: An end-to-end framework is proposed for automatically classifying normal versus fatty pancreas from abdominal ultrasound images, achieving high accuracy and F1 score.


<details>
  <summary>Details</summary>
Motivation: Diagnosis of Non-alcoholic fatty pancreas disease (NAFPD) typically relies on subjective visual assessment of ultrasound images by clinicians, and there is a need for an automated classification system.

Method: The proposed method employs a TransUNet-based segmentation architecture with a ResNet encoder and transformer bottleneck, followed by anatomically-guided patch extraction and patient-level classification through pairwise texture comparison.

Result: The proposed framework achieves a mean cross-validated accuracy of 89.7% ± 1.8% and F1 of 0.898 ± 0.019 on a clinical dataset of 214 abdominal ultrasound images.

Conclusion: The proposed end-to-end automated framework for fatty pancreas classification from ultrasound using segmentation-guided texture analysis shows promising results and has the potential to improve diagnosis of NAFPD.
...
TL;DR: The paper introduces Consensus Entropy (CE), a training-free metric for estimating output reliability in Optical Character Recognition (OCR) tasks, and CE-OCR, a lightweight framework that improves OCR accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: Despite progress in OCR accuracy, state-of-the-art Vision-Language Models (VLMs) struggle with detecting sample-level errors and lack effective unsupervised quality control.

Method: The authors propose CE, a model-agnostic metric that measures inter-model agreement entropy, and CE-OCR, a multi-model framework that verifies outputs by ensemble agreement and selects the best outputs.

Result: Experiments demonstrate that CE improves F1 scores by 42.1% over VLM-as-Judge, and CE-OCR achieves consistent OCR gains, outperforming baselines at the same cost.

Conclusion: The proposed CE and CE-OCR approach enables plug-and-play integration, requires no training or supervision, and improves OCR accuracy and efficiency.
...
TL;DR: Proposed ForgeVLA, a federated framework for training Vision-Language-Action models from distributed vision-action pairs without centralizing data or requiring manual annotations.


<details>
  <summary>Details</summary>
Motivation: The high cost of acquiring annotated training data for Vision-Language-Action models and the challenges of centralizing and handling heterogeneous vision-action pairs from various domains.

Method: ForgeVLA uses an embodied instruction classifier to map vision-action pairs to a predefined instruction set, and combines client-side contrastive planning loss with server-side adaptive aggregation to learn task-discriminative representations.

Result: ForgeVLA significantly outperforms other baselines across multiple benchmarks, and ablation studies validate the contribution of each component.

Conclusion: ForgeVLA offers an efficient and effective solution for federated Vision-Language-Action training, addressing the challenges of data decentralization and heterogeneity.
...
TL;DR: A new dataset and evaluation model for text-guided image editing (TIE) are proposed, enabling more transparent and trustworthy assessment.


<details>
  <summary>Details</summary>
Motivation: Existing TIE evaluation methods lack interpretability, and high-quality interpretation datasets are scarce.

Method: A dataset called ReasonEdit-22K is introduced, along with a multimodal large language model (MLLM)-based reward model called RE-Reward and an evaluation model called ReasonEdit.

Result: ReasonEdit achieves superior alignment with human preferences and exhibits strong generalization across public benchmarks, generating high-quality interpretable evaluation text.

Conclusion: The proposed dataset and evaluation model enable more transparent and trustworthy assessment for image editing, addressing the limitations of existing TIE evaluation methods.
...
TL;DR: The paper proposes AudioFace, a framework that uses linguistic and phonetic information to improve speech-driven facial animation, achieving better performance than traditional methods.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of accurately mapping speech audio to facial coefficients, particularly for mouth movements, by incorporating linguistic and phonetic structure into the process.

Method: The method proposed is AudioFace, a language-assisted framework that uses multimodal large language models and introduces transcript- and phoneme-level cues to predict mouth-related facial coefficients.

Result: The results show that AudioFace achieves superior performance across multiple evaluation metrics, validating its effectiveness.

Conclusion: The conclusion is that the proposed AudioFace framework is effective in improving speech-driven facial animation by leveraging linguistic and articulatory information.
...
TL;DR: A novel framework for implicit multi-camera system calibration using Gaussian Process regression is proposed, which is data-efficient and provides uncertainty quantification.


<details>
  <summary>Details</summary>
Motivation: Conventional calibration methods are limited by rigid mathematical models and struggle with complex distortions, while existing neural network-based approaches are data-hungry and lack uncertainty quantification.

Method: A Gaussian Process-based model that learns the non-linear mapping from 2D image coordinates to 3D world coordinates, integrated with Active Learning for data efficiency.

Result: The proposed approach provides a robust, data-efficient, and reliable calibration solution with statistically-sound confidence bounds, and uncertainty for 3D predictions is higher closer to the cameras.

Conclusion: The proposed framework is particularly effective in practical scenarios where collecting extensive calibration data is a constraint, and is relevant for calibration of complex multi-camera systems.
...
TL;DR: The paper presents PureDocBench, a new benchmark for document parsing, and evaluates 40 models, finding that document parsing is far from solved and specialist parsers can rival general-purpose VLMs.


<details>
  <summary>Details</summary>
Motivation: The current benchmark, OmniDocBench, has saturated scores above 90%, but an audit pipeline revealed 2,580 errors (12.08%) in its annotations, questioning its rankings. The authors aim to address these issues with a new benchmark.

Method: The authors created PureDocBench, a programmatically generated benchmark with verifiable annotations, covering 10 domains and 66 subcategories. They evaluated 40 models, including pipeline specialists, end-to-end specialists, and general-purpose VLMs.

Result: The best model scored ~74 out of 100, with a 44.6-point gap between the strongest and weakest models. Specialist parsers with <=4B parameters rivaled or surpassed general VLMs that were 5-100x larger. Formula recognition remains a shared bottleneck, with no model exceeding 67%.

Conclusion: The paper concludes that document parsing is far from solved, and PureDocBench provides a more reliable benchmark for evaluating document parsing models. The authors publicly released all data, code, and artifacts.
...
TL;DR: Proposed DIMoE-Adapters, a dynamic framework for continual learning in vision-language models, addressing stability-plasticity dilemma.


<details>
  <summary>Details</summary>
Motivation: Continual learning in vision-language models faces challenges in multi-domain task-incremental learning, specifically the stability-plasticity dilemma and catastrophic forgetting.

Method: DIMoE-Adapters framework with two components: Self-Calibrated Expert Evolution (SCEE) and Prototype-Guided Expert Selection (PGES).

Result: DIMoE-Adapters outperforms state-of-the-art methods in various settings.

Conclusion: DIMoE-Adapters effectively balances stability and plasticity in continual learning for vision-language models.
...
TL;DR: A simple and effective approach for unpaired smartphone ISP problem, achieving 3rd best SSIM and ΔE in NTIRE 2026 challenge.


<details>
  <summary>Details</summary>
Motivation: Unpaired smartphone ISP problem due to lack of scene and color alignment between RAW and target RGB images.

Method: Reconstructing larger images, extracting semantic embeddings with DINOv2, and using FGW optimal transport to build pseudo pairs for training a lightweight CNN.

Result: Achieving 22.569 PSNR, 0.675 SSIM, and 8.067 ΔE on the final hidden test set, outperforming the baseline.

Conclusion: Proposed method effectively addresses unpaired smartphone ISP problem with a lightweight CNN, achieving competitive results in NTIRE 2026 challenge.
...
TL;DR: A deep learning framework, 4DPrecipNet, is introduced to recover 4D precipitation structure from geostationary infrared observations, overcoming conventional limitations.


<details>
  <summary>Details</summary>
Motivation: Accurate 4D precipitation information is crucial for understanding Earth's energy and water cycles, but remains unresolved at global scales.

Method: A physically constrained deep learning framework, 4DPrecipNet, integrating multi-channel infrared radiances and radar-derived precipitation profiles with a moisture-first constraint.

Result: The framework captures deep convective structures and their evolution, with robust performance across large samples and independent radar comparisons.

Conclusion: Sub-cloud precipitation is physically encoded in cloud-top infrared observations, establishing a new pathway for continuous global monitoring of precipitation structure.
...
TL;DR: The paper proposes Diffusion-APO, a trajectory-aware algorithm for aligning large-scale video diffusion models with human intent, and a unified RLHF framework for flexible, multi-stage preference alignment.


<details>
  <summary>Details</summary>
Motivation: The need for a scalable and trajectory-aware pathway to bridge the discrepancy between training noise distributions and practical inference trajectories in large-scale video diffusion models.

Method: Diffusion-APO, a trajectory-aware algorithm that synchronizes training noise with inference-time denoising paths, and a unified RLHF framework that integrates online ranking, half-online anchoring, offline refinement, and distillation-aware drift correction.

Result: Diffusion-APO consistently outperforms standard baselines in visual quality and instruction following, while preserving generative fidelity during model acceleration.

Conclusion: The proposed method provides a robust, end-to-end pathway for scalable video diffusion alignment, enabling flexible, multi-stage preference alignment across diverse data and computational constraints.
...
TL;DR: Introduces InterLV-Search, a new benchmark for multimodal agentic search that evaluates the use of textual and visual evidence in an interleaved search trajectory.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks for multimodal agentic search do not adequately assess the use of visual evidence in search trajectories, and the authors aim to address this limitation.

Method: The authors construct a new benchmark, InterLV-Search, with 2061 examples across three levels, and provide a standardized agent, InterLV-Agent, for evaluation.

Result: Experiments show that current multimodal agents have an overall accuracy below 50%, highlighting challenges in visual evidence seeking, search control, and multimodal evidence integration.

Conclusion: The authors conclude that InterLV-Search provides a new benchmark for evaluating multimodal agentic search and highlights the need for further research in this area.
...
TL;DR: The paper proposes HDSD, a framework for continual learning in vision-language models that mitigates catastrophic forgetting by decoupling parameter updates into general and task-specific subspaces.


<details>
  <summary>Details</summary>
Motivation: Existing methods for class-incremental learning overlook structural properties of parameter updates in high-dimensional spaces, leading to cross-task subspace interference and severe forgetting.

Method: The proposed HDSD framework consists of a Feature Modulation Module (FMM) that decomposes the parameter space into general and task-specific subspaces, a General Fusion Module (GFM) that captures stable and transferable knowledge, and a Hierarchical Learning Module (HLM) that performs structured parameter decomposition via SVD.

Result: Extensive experiments on conventional benchmarks show that HDSD achieves state-of-the-art results.

Conclusion: The proposed HDSD framework effectively reduces subspace interference and parameter drift, leading to improved performance in continual learning for vision-language models.
...
TL;DR: The authors propose Implicit Preference Alignment (IPA), a data-efficient framework for improving hand motion generation in human image animation without needing paired preference data.


<details>
  <summary>Details</summary>
Motivation: The challenge of generating high-fidelity hand motions in human image animation due to their high degrees of freedom and motion complexity, and the impracticality of curating strict preference pairs for dynamic hand regions.

Method: Implicit Preference Alignment (IPA), a post-training framework that uses implicit reward maximization to align the model by maximizing the likelihood of self-generated high-quality samples and penalizing deviations from the pretrained prior, along with a Hand-Aware Local Optimization mechanism.

Result: The method achieves effective preference optimization to enhance hand generation quality while significantly reducing the need for constructing preference data.

Conclusion: The proposed IPA framework offers a data-efficient solution for improving hand motion generation in human image animation, eliminating the need for paired preference data and reducing the barrier for constructing preference data.
...
TL;DR: The paper applies Conformal Prediction (CP) to object detection, improving uncertainty quantification by scaling prediction intervals based on input-dependent uncertainty estimates, achieving better interval sharpness without sacrificing coverage.


<details>
  <summary>Details</summary>
Motivation: Object detection requires reliable uncertainty quantification, but classical CP theory is complicated by structured multi-output predictions, and standard CP produces fixed-width prediction intervals that are not optimal for low-uncertainty predictions.

Method: The paper applies CP coordinate-wise to bounding box corners with a Bonferroni correction, scales intervals using per-prediction aleatoric uncertainty estimates, and extends to a two-step pipeline that constructs prediction sets for the class using RAPS.

Result: Scaled CP improves interval sharpness over unscaled CP, achieving up to 19% higher IoU and 39% lower interval scores, without sacrificing coverage, across three autonomous driving datasets.

Conclusion: The proposed method yields more actionable uncertainty estimates for real-time, real-world object detection, enabling reliable uncertainty quantification in safety-critical applications.
...
TL;DR: The paper introduces a new paradigm for 3D reconstruction from disjoint views, called Generative Reconstruction from Disjoint Views, and proposes a framework called GLADOS to address the limitations of existing methods in scenarios with no visual overlap.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome the limitation of 3D vision systems that require visual overlap, which is often impossible in real-world scenarios such as distributed swarm robotics or crowd-sourced data collection.

Method: The method proposed is a general, modular framework called GLADOS, which operates through three stages: Generative Bridging, Robust Coarse 3D Reconstruction, and Iterative Context Expansion and Consistency Optimization.

Result: The result shows that existing state-of-the-art methods fail catastrophically on this task, while GLADOS enables seamless integration of future advances in generation, reconstruction, and inpainting, and is available as open-source code.

Conclusion: The conclusion is that GLADOS provides a new approach to 3D reconstruction from disjoint views, addressing the limitations of existing methods and enabling potential advances in scenarios with no visual overlap.
...
TL;DR: A new architecture called VIMCAN is proposed for multimodal 3D human pose estimation, combining the efficiency of Mamba with the spatial reasoning of Cross-Attention, achieving high accuracy and real-time inference.


<details>
  <summary>Details</summary>
Motivation: The current state-of-the-art human pose estimation pipelines rely on Transformers, which have quadratic complexity and make real-time processing impractical. Mamba addresses this issue but struggles with complex spatial dependencies in multimodal settings.

Method: VIMCAN, a hybrid architecture that combines Mamba's efficient sequence modeling with Cross-Attention's spatial reasoning, and performs robust visual-inertial fusion and human pose estimation between RGB keypoints and wearable IMU data.

Result: VIMCAN achieves mean per-joint position errors (MPJPE) of 17.2 mm on TotalCapture and 45.3 mm on 3DPW, outperforming prior Transformer-based and other state-of-the-art approaches while supporting real-time inference at over 60 frames per second.

Conclusion: VIMCAN achieves superior accuracy and real-time inference in multimodal 3D human pose estimation, making it a promising solution for applications requiring high accuracy and efficiency.
...
TL;DR: The paper investigates if Vision Transformer (ViT) depth can be approximated by autonomous linear dynamics. It uses Dynamic Mode Decomposition (DMD) to test this and finds that for short spans, a single linear operator can accurately predict future states, but this accuracy does not transfer downstream.


<details>
  <summary>Details</summary>
Motivation: To determine if the depth of Vision Transformers (ViTs) can be approximated by autonomous linear dynamics, and if a single operator can be applied recurrently across a contiguous span to predict future states.

Method: Dynamic Mode Decomposition (DMD) is used to fit a linear operator $K$ from selected, consecutive hidden-state pairs. This operator is then used to predict $p$ steps ahead via $K^p$. The method is tested on four pretrained DINO ViTs.

Result: For short spans ($p leq 4$), the linear operator $K^p$ accurately tracks an unconstrained endpoint map with a cosine similarity of within $0.02$ on DINOv3-H/16+. The fitted operators compress to a low rank with minimal calibration data at early cut starts, and the 'cls' token is most amenable to linearization. However, this local fidelity does not transfer downstream, and at the final hidden state, an identity baseline becomes competitive.

Conclusion: The study suggests that while ViT depth can be approximated by autonomous linear dynamics over short spans, this approximation does not hold when considering the entire network. The findings indicate that the linearization is more effective for early layers and the 'cls' token, but diminishes as the signal propagates through the network.
...
TL;DR: A multimodal stepwise clinically-guided attention learning framework is proposed for predicting pathological complete response (pCR) in breast cancer patients from breast MRI, improving sensitivity and generalizability.


<details>
  <summary>Details</summary>
Motivation: Accurate pre-treatment pCR prediction remains challenging due to severe class imbalance and limited generalizability across diverse clinical settings.

Method: A multimodal stepwise clinically-guided attention learning framework, which follows a stepwise training strategy inspired by physician reasoning, integrating breast MRI and clinical variables.

Result: The proposed approach improves sensitivity while maintaining competitive specificity, and produces anatomically coherent attention maps that support interpretation of the model's predictions.

Conclusion: The framework highlights the potential of clinically-guided multimodal attention learning for robust and generalizable pCR prediction in breast cancer.
...
TL;DR: The authors introduce ScaleEarth, a framework for remote sensing vision-language models that efficiently handles varying ground sampling distances, achieving state-of-the-art results on several benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing remote sensing vision-language models struggle with the varying scales of geographic objects across different ground sampling distances, and current solutions either ignore or crudely discretize this information.

Method: The authors propose a parameter-efficient fine-tuning framework called ScaleEarth, which treats ground sampling distance as a continuous conditioning variable, and introduces CS-HLoRA and SSE-U to modulate the model's computation path and predict ground sampling distance from visual features.

Result: ScaleEarth achieves state-of-the-art results on remote-sensing benchmarks including XLRS-Bench and OmniEarth-Bench, covering diverse Earth-system tasks.

Conclusion: The proposed ScaleEarth framework effectively addresses the challenge of handling varying ground sampling distances in remote sensing vision-language models, leading to improved performance on several benchmarks.
...
TL;DR: The paper investigates the performance gap between humans and Video Large Language Models (Video-LLMs) on the Arrow-of-Time (AoT) task, which involves determining whether a video plays forward or backward. The authors identify bottlenecks in temporal information flow and propose a Video-LLM architecture with temporal-aware encoder, time-preserved projector, and AoT supervision, achieving 98.1% accuracy and improving temporal reasoning tasks.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the performance gap between humans and Video-LLMs on the AoT task. Humans can solve this task with near-perfect accuracy, but Video-LLMs perform only modestly above chance. The authors aim to understand whether the visual backbones fail to encode temporal information or if the information bottleneck lies elsewhere in the Video-LLM architecture.

Method: The authors isolate the vision encoder from the Video-LLM and trace temporal information across the encoder, projector, and LLM. They analyze the performance of video-centric encoders with explicit temporal modeling and frame-centric encoders. They also investigate the impact of projector design on temporal information flow and propose a time-preserved MLP projection. Finally, they build a Video-LLM with a temporal-aware video-centric encoder, time-preserved projector, and AoT supervision.

Result: The authors achieve 98.1% accuracy on the AoT task, surpassing human performance on AoT$_{PPB}$. They also improve broader temporal reasoning tasks by up to 6.0 points on VITATECS-Direction and 1.3 points on TVBench.

Conclusion: The paper concludes that temporal reasoning in Video-LLMs requires both effective temporal encoding and reliable transfer of this information to the LLM. The authors' proposed Video-LLM architecture with temporal-aware encoder, time-preserved projector, and AoT supervision can effectively address the performance gap between humans and Video-LLMs on the AoT task.
...
TL;DR: PolarVLM integrates polarimetric physical parameters into vision-language models, improving their ability to handle optical ambiguities like reflections and transparent objects.


<details>
  <summary>Details</summary>
Motivation: Mainstream vision-language models struggle with optical ambiguities due to limitations of standard RGB inputs, and existing polarization imaging methods are constrained by fixed-format outputs.

Method: PolarVLM uses a dual-stream architecture and a progressive two-stage training strategy to integrate polarimetric physical parameters into vision-language models.

Result: PolarVLM surpasses the RGB baseline by 25.4% overall across five evaluation tasks, with gains of 26.6% in reflection recognition and 34.0% in glass counting.

Conclusion: PolarVLM successfully unlocks physics-aware semantic understanding by effectively preventing physical misinterpretations while preserving general visual abilities.
...
TL;DR: A novel framework, Response-G1, is proposed for proactive streaming video understanding, enabling more accurate and interpretable response timing decisions by establishing explicit alignment between video evidence and query conditions via scene graphs.


<details>
  <summary>Details</summary>
Motivation: Existing methods for proactive streaming video understanding often fall short due to their implicit, query-agnostic modeling of visual evidence, leading to a need for a more structured and explicit approach.

Method: Response-G1 operates in three stages: (1) online query-guided scene graph generation, (2) memory-based retrieval of semantically relevant historical scene graphs, and (3) retrieval-augmented trigger prompting for per-frame decisions.

Result: Experimental results on established benchmarks demonstrate the superiority of Response-G1 in both proactive and reactive tasks, validating the advantage of explicit scene graph modeling and retrieval.

Conclusion: Response-G1 achieves more interpretable and accurate response timing decisions by grounding both evidence and conditions in a shared graph representation, showing the effectiveness of explicit scene graph modeling in streaming video understanding.
...
TL;DR: The paper introduces Manifold-Aligned Point Recognition (MAPR), a framework that improves the robustness of 3D point cloud networks by aligning the latent geometry with the intrinsic geometry of the underlying surface.


<details>
  <summary>Details</summary>
Motivation: The authors hypothesize that the adversarial vulnerability in 3D networks arises from a manifold misalignment between the latent geometry learned by the model and the intrinsic geometry of the underlying surface.

Method: The authors propose MAPR, which augments each point cloud with intrinsic features capturing local curvature and diffusion structure, and applies a consistency loss that preserves invariance to intrinsic, geometry-preserving perturbations.

Result: MAPR consistently improves robustness across multiple adversarial attacks on both the ModelNet40 and ScanObjectNN datasets, achieving average robustness gains of +20.02% and +8.58% on ModelNet40 and ScanObjectNN, respectively.

Conclusion: The paper provides a geometric interpretation of 3D robustness and proposes a framework that regularizes the latent geometry by aligning predictions across intrinsic perturbations, leading to improved robustness in 3D point cloud networks.
...
TL;DR: Introduced TraceAV-Bench, a benchmark for evaluating multi-hop reasoning over long audio-visual trajectories and multimodal hallucination robustness, which poses a challenge to current OmniLLMs.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks fail to evaluate the capability of chaining evidence that is sparse, temporally dispersed, and split across visual and auditory streams, which is required for real-world audio-visual understanding.

Method: TraceAV-Bench comprises 2,200 multiple-choice questions over 578 long videos, totaling 339.5 hours, and is built using a three-step semi-automated pipeline followed by a strict quality assurance process.

Result: Evaluation of multiple representative OmniLLMs on TraceAV-Bench reveals that the benchmark poses a persistent challenge across all models, with the strongest closed-source model reaching 68.29% and the best open-source model reaching 51.70%.

Conclusion: TraceAV-Bench will stimulate further research toward OmniLLMs that can reason coherently and faithfully over long-form audio-visual content, as current models leave substantial headroom for improvement.
...
TL;DR: The authors propose a new multimodal architecture called GazeVLM that incorporates top-down control over attention resources, mimicking human active vision and achieving state-of-the-art performance in multimodal reasoning.


<details>
  <summary>Details</summary>
Motivation: The authors are motivated by the limitations of modern Vision-Language Models (VLMs) that process visual information passively, leading to diluted spatial reasoning and linguistic hallucinations. They aim to create a model that can dynamically focus on relevant visual features and simulate foveal fixation.

Method: The authors propose GazeVLM, a multimodal architecture that generates gaze tokens to control its own attention mask, implementing spatial selective attention and simulating foveal fixation. They also introduce a bespoke training procedure called Group Relative Policy Optimization (GRPO).

Result: The authors report that their 4B-parameter GazeVLM achieves state-of-the-art performance in multimodal reasoning, surpassing other VLMs by nearly 4% and agentic multimodal pipelines by more than 5% on HRBench-4k and HRBench-8k.

Conclusion: The authors conclude that GazeVLM enables fluid transitions between global spatial awareness and localized focal reasoning, achieving strong high-resolution multimodal reasoning performance without relying on external tools or inflating the context window.
...
### [107] [SAM 3D Animal: Promptable Animal 3D Reconstruction from Images in the Wild](https://arxiv.org/abs/2605.07604)
*Xuyi Hu,Jin Lyu,Jiuming Liu,Yebin Liu,Silvia Zuffi,Liang An,Stefan Goetz*

Main category: cs.CV

TL;DR: A new framework, SAM 3D Animal, enables 3D reconstruction of multiple animals from a single image, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods for 3D animal reconstruction focus on single-animal settings, leaving a gap for multi-animal scenes which are common in the wild.

Method: SAM 3D Animal uses a parametric animal model (SMAL+) and supports flexible prompts like keypoints and masks for disambiguation in crowded scenes.

Result: The framework achieves state-of-the-art results on Animal3D, APTv2, and Animal Kingdom datasets, demonstrating scalability and effectiveness.

Conclusion: The proposed framework offers a scalable and effective solution for prompt-driven 3D reconstruction of multiple animals in the wild.
...
### [108] [FS-I2P:A Hierarchical Focus-Sweep Registration Network with Dynamically Allocated Depth](https://arxiv.org/abs/2605.07607)
*Zhixin Cheng,Yujia Chen,Xujing Tao,Bohao Liao,Xiaotian Yin,Baoqun Yin,Tianzhu Zhang*

Main category: cs.CV

TL;DR: The paper proposes a new method for image-to-point cloud registration, addressing challenges such as viewpoint changes and repetitive textures. It introduces a 'Focus-Sweep' paradigm and a Hierarchical Focus-Sweep Interaction Module, achieving state-of-the-art performance on two benchmarks.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve image-to-point cloud registration by addressing challenges like viewpoint changes, cross-modal discrepancies, and repetitive textures that lead to erroneous correspondences.

Method: The method proposed is a 'Focus-Sweep' paradigm with a Hierarchical Focus-Sweep Interaction Module within an SSM-based framework, along with a Dynamic Layer Allocation Strategy.

Result: The approach achieves state-of-the-art performance on two benchmarks, RGB-D Scenes V2 and 7-Scenes.

Conclusion: The proposed method effectively enhances multi-level cross-modal feature association and improves matching robustness, leading to state-of-the-art performance in image-to-point cloud registration.
...
### [109] [LithoBench: Benchmarking Large Multimodal Models for Remote-Sensing Lithology Interpretation](https://arxiv.org/abs/2605.07640)
*Jun Wang,Fengpeng Li,Hang Dong,Tianjin Huang,Wei Han*

Main category: cs.CV

TL;DR: A new benchmark called LithoBench is proposed for evaluating geological semantic understanding in remote sensing lithology interpretation, highlighting limitations in large vision-language models.


<details>
  <summary>Details</summary>
Motivation: The lack of benchmarks that capture lithological annotations, multi-level geological semantics, and expert-informed assessment in evaluating geological knowledge-guided large multimodal models for remote sensing lithology interpretation.

Method: Development of LithoBench, a multi-level benchmark with 10,000 expert-annotated instances across 12 lithological categories, and an expert-in-the-loop, knowledge-grounded semi-automated construction pipeline.

Result: Experiments reveal substantial limitations in geological semantic understanding, particularly on higher-order explanation, application, and reasoning tasks, in multiple large vision-language models.

Conclusion: LithoBench provides a new standard for evaluating geological semantic understanding in remote sensing lithology interpretation, and the results suggest that large vision-language models require improvement in this area.
...
TL;DR: A new framework, EggHand, is proposed for forecasting 3D hand pose sequences from egocentric video, enabling applications like AR/VR assistance and human-robot interaction.


<details>
  <summary>Details</summary>
Motivation: Forecasting future 3D hand pose sequences from egocentric video is crucial for understanding human intention, but it's challenging due to complex human intent, dexterous articulations, and drastic viewpoint shifts.

Method: EggHand unifies multimodal semantic reasoning with dynamic motion modeling, coupling an action decoder from a Vision-Language-Action (VLA) model with an egocentric video-text encoder.

Result: EggHand sets a new state of the art in forecasting accuracy on the EgoExo4D dataset, remains robust under severe ego-motion, and enables controllable prediction via language-based task prompts.

Conclusion: The proposed EggHand framework effectively addresses the challenges of egocentric hand pose forecasting, offering improved accuracy, robustness, and controllability.
...
TL;DR: This paper explores the use of vision-language models (VLMs) as zero-shot 'ODD sensors' for autonomous systems, specifically in Automated Driving Systems (ADS), to adapt to evolving Operational Design Domain (ODD) definitions.


<details>
  <summary>Details</summary>
Motivation: The need for autonomous systems to adhere to safety regulations and the importance of dependable perception of ODD elements for safe implementation and auditing.

Method: The paper conducts an empirical study of zero-shot ODD classification and detection using four VLMs on a custom dataset and Mapillary Vistas, along with failure analyses and ablation of zero-shot optimization strategies.

Result: The findings indicate that definition-anchored chain-of-thought prompting with persona decomposition performs best, while other methods may result in reduced recall.

Conclusion: The results pave the way for transparent and effective ODD-based perception in safety-critical applications.
...
TL;DR: DeflareMambav2, a prior-guided Mamba framework, is proposed for lens flare removal, achieving state-of-the-art performance by adaptively preserving light sources, removing flare artifacts, and recovering background details.


<details>
  <summary>Details</summary>
Motivation: Current lens flare removal methods fail to handle region-dependent restoration demands, particularly in preserving saturated light sources, removing flare artifacts, and recovering background details in nighttime photography.

Method: DeflareMambav2 uses a Flare Prior Network (FPN) to estimate flare priors and guide adaptive restoration. It employs a novel radial serialization strategy and a dual-level adaptive scheme to preserve light-source regions and apply curriculum-based restoration.

Result: DeflareMambav2 achieves state-of-the-art performance with a reduced parameter burden, demonstrating its effectiveness in lens flare removal.

Conclusion: The proposed DeflareMambav2 framework effectively addresses the challenges of lens flare removal by providing a prior-guided and adaptive approach, leading to improved image quality in nighttime photography.
...
TL;DR: This paper presents a self-supervised framework for estimating per-pixel optical flow from asynchronous event streams in underwater environments using spiking neural networks, achieving competitive results with superior computational efficiency.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenges of conventional imaging systems in underwater environments, which require high-quality sensing with strict resource efficiency, and to explore the potential of event cameras in aquatic scenarios.

Method: The method involves using spiking neural networks to build a self-supervised framework for estimating per-pixel optical flow from asynchronous event streams, which bypasses the bottleneck of underwater data scarcity.

Result: The results demonstrate that the proposed method achieves competitive visual and quantitative results against leading techniques while operating with superior computational efficiency.

Conclusion: The conclusion is that this work opens new frontiers for lightweight, real-time, and low-cost perception on resource-constrained underwater edge platforms by bridging neuromorphic sensing and aquatic intelligence.
...
TL;DR: The paper presents Bharat ABIS, a large-scale multimodal biometric search system based on open-source architectures, processing fingerprint, face, and iris modalities for efficient 1:N search.


<details>
  <summary>Details</summary>
Motivation: The need for a country-level identity system requiring searching a multi-biometric database of a billion records, pushing the limits of all aspects of a biometric system.

Method: The end-to-end pipeline of Bharat ABIS processes modalities through preprocessing, quality assessment, presentation attack detection, and feature extraction, producing a concatenated template.

Result: Evaluations on 220 million identities yield an FNIR of 0.3% at an FPIR of 0.5% for adult probes, and the system achieves a throughput of 100 searches per second on a 40M gallery.

Conclusion: The paper presents insights into a large-scale multimodal biometric search system, Bharat ABIS, which achieves efficient and effective solution for 1:N search with high accuracy and throughput.
...
TL;DR: OphEdit is a novel framework for text-guided editing of ophthalmic surgical videos, preserving anatomical geometry while allowing semantic modifications.


<details>
  <summary>Details</summary>
Motivation: High-fidelity surgical video generation is crucial for medical training and AI development, but precise video editing remains challenging due to strict anatomical and temporal constraints.

Method: OphEdit uses a deterministic second-order ODE inversion pipeline and selective injection of Attention Value tensors into the conditional Classifier-Free Guidance branch during denoising.

Result: OphEdit effectively handles complex surgical transformations with superior structural fidelity and temporal consistency compared to natural-domain video editors.

Conclusion: OphEdit offers a scalable solution for generating diverse, annotated medical datasets without exhaustive manual recording or costly model fine-tuning.
...
TL;DR: The paper discusses the negative environmental impacts of mining operations and presents a dataset for monitoring and analyzing mining activities, including large-scale and artisanal small-scale mining sites.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the negative environmental impacts of mining operations, including land-use change, energy consumption, soil erosion, and deforestation, and to provide a dataset for researchers to better understand the interrelationship between mine facility attributes and their environmental impacts.

Method: The paper presents a dataset containing 150 Large Scale Mining (LSM) sites and 870km^2 annotated area of Artisanal Small-scale Mining (ASM) sites, with metadata including nine LSM sections and 27 mining site attributes for each LSM site.

Result: The paper provides a dataset that can contribute to the research community, enabling the study of social and environmental consequences of mining activities and promoting researchers' responsibilities from an ethics perspective.

Conclusion: The paper concludes that monitoring remote mining site activities can help detect illegal mining activities and their environmental impacts, and that the presented dataset can facilitate research on the interrelationship between mine facility attributes and their environmental impacts.
...
TL;DR: The study evaluates the transferability of pre-trained representations from open-source medical foundation models to CT-based renal lesion classification, comparing their performance to traditional methods.


<details>
  <summary>Details</summary>
Motivation: To investigate how well pre-trained representations from open-source medical foundation models transfer to clinically relevant but data-scarce classification tasks, specifically in CT-based renal lesion classification.

Method: The study benchmarks three medical foundation models using a frozen feature-probing protocol, comparing their performance to a handcrafted radiomics classifier and a 3D ResNet-50 trained from scratch on a dataset of 2,854 lesions.

Result: The results show that foundation models (AUC 0.70-0.77) matched the performance of a from-scratch ResNet (AUC 0.72) but were outperformed by a conventional radiomics baseline (AUC 0.88).

Conclusion: Current generalist foundation model embeddings do not yet capture the fine-grained texture and shape heterogeneity driving histological subtype discrimination, and radiomics remains the current state-of-the-art for renal lesion stratification.
...
TL;DR: A new approach called Head Similarity is introduced for face recognition, which considers not just identity but also appearance variations, enabling better performance under occlusion or non-frontal views.


<details>
  <summary>Details</summary>
Motivation: Conventional face recognition models are limited in their ability to handle appearance variations and non-frontal views, which is a significant constraint for many vision applications.

Method: The approach introduces a new formulation called Head Similarity, which models structured whole-head similarity and captures intra-identity appearance variation through hierarchical supervision and identity-aware distillation.

Result: Experiments show that conventional face recognition models fail to capture appearance-dependent similarity, while the proposed approach demonstrates the feasibility of structured whole-head similarity modeling.

Conclusion: The proposed approach, Head Similarity, offers a more comprehensive solution for face recognition, particularly in scenarios where appearance variations and non-frontal views are common.
...
TL;DR: The authors propose an unsupervised framework called SIMI, which uses bit-plane decomposition to enhance low-light images without relying on external data.


<details>
  <summary>Details</summary>
Motivation: Poor lighting conditions significantly impact image quality, and existing enhancement methods often neglect intrinsic information in low-light images.

Method: The authors propose the Self-Information Mining (SIMI) network, an unsupervised framework that decomposes low-light images into multiple components based on bit-plane decomposition.

Result: SIMI achieves state-of-the-art performance on standard benchmarks.

Conclusion: The proposed SIMI network offers an effective and efficient solution for enhancing low-light images, with advantages in terms of performance, computational overhead, and real-world applicability.
...
TL;DR: A framework for differentiable RF propagation simulation in neural scenes, enabling point-to-point path computation and high-quality visual rendering.


<details>
  <summary>Details</summary>
Motivation: The need for a unified spatial representation for both electromagnetic propagation simulation and photorealistic view synthesis, driven by the limitations of current methods in handling complex geometry and RF propagation.

Method: Embedding Gaussian primitives into a hardware-accelerated ray tracing structure as the underlying spatial representation, allowing for differentiable RF propagation simulation directly within visually reconstructed neural scenes.

Result: The framework enables point-to-point path computation between arbitrary 3D locations while preserving high-quality visual rendering, and provides cross-modal evidence that neural reconstructions can serve as unified spatial representations.

Conclusion: The proposed framework allows for efficient and accurate simulation of RF propagation in complex environments, and demonstrates the potential of neural reconstructions as a unified representation for multiple applications.
...
TL;DR: The authors propose XpertCausal, a radiologist-guided causal Concept Bottleneck Model (CBM) for chest X-ray interpretation that incorporates clinically motivated causal structure and expert domain knowledge, leading to more accurate, interpretable, and clinically aligned models.


<details>
  <summary>Details</summary>
Motivation: Most existing CBMs treat concepts as discriminative predictors of pathology labels without modelling the underlying clinical generative process. The authors aim to improve model interpretability by predicting intermediate clinical concepts before final diagnoses.

Method: The authors propose a probabilistic noisy-OR framework to model pathology-to-concept relationships and use Bayesian inference to estimate pathology probabilities from predicted concepts. Radiologist-curated concept-pathology associations are used to constrain model structure.

Result: XpertCausal achieves improved AUROC, calibration, and clinically relevant explanation quality compared to a non-causal CBM baseline and a causal ablation with unconstrained learned associations.

Conclusion: Incorporating clinically motivated causal structure and expert domain knowledge into CBMs leads to more accurate, interpretable, and clinically aligned models for CXR interpretation.
...
TL;DR: Introduces APEX, a novel image evaluation framework that uses the Sliced Wasserstein Distance for assumption-free and robust image evaluation.


<details>
  <summary>Details</summary>
Motivation: Traditional image evaluation metrics (e.g., FID) are limited by their reliance on outdated features and rigid parametric formulations.

Method: APEX leverages the Sliced Wasserstein Distance and utilizes open-vocabulary foundation models (CLIP and DINOv2) as feature extractors.

Result: APEX demonstrates superior robustness to visual degradations and exhibits stable evaluations on out-of-domain datasets.

Conclusion: APEX offers a more robust and assumption-free approach to image evaluation, addressing the limitations of traditional metrics.
...
TL;DR: The paper presents SARA, a semantically adaptive relational alignment method for video diffusion models, improving text alignment and motion quality.


<details>
  <summary>Details</summary>
Motivation: Recent video diffusion models struggle with entity consistency, attribute binding, and interaction accuracy. Existing methods like VideoREPA and MoAlign improve fine-grained text following but have limitations in their supervision allocation.

Method: SARA uses token-relation distillation on a frozen visual foundation model and adds text-conditioned saliency to decide which token pairs carry supervision. It involves a lightweight Stage 1 aligner and a pair-routing operator to assign weights to token pairs.

Result: SARA improves text alignment and motion quality over SFT, VideoREPA, and MoAlign on a 13-dimension VLM rubric, on the public VBench benchmarks, and in a blind user study.

Conclusion: The proposed SARA method effectively enhances the performance of video diffusion models in terms of text alignment and motion quality.
...
TL;DR: Introduced CADTestBench, a test-based benchmark for Text-to-CAD, and demonstrated its use in evaluating and guiding CAD model generation.


<details>
  <summary>Details</summary>
Motivation: The need for a robust evaluation method for Text-to-CAD due to the lack of existing work in this area.

Method: Proposed CADTestBench, a benchmark based on executable software tests (CADTests) that verify geometric and topological requirements of generated CAD models.

Result: CADTestBench was used to benchmark recent Text-to-CAD methods and guide CAD model generation, yielding simple baselines that surpassed current methods' performance.

Conclusion: CADTestBench provides a new evaluation perspective for Text-to-CAD, enabling comprehensive benchmarking and guiding model generation.
...
TL;DR: The paper presents CircleID, a competition on writer identification and pen classification from hand-drawn circles, providing a new dataset and baseline for minimal-trace analysis.


<details>
  <summary>Details</summary>
Motivation: To investigate how biometric writer characteristics and physical pen features entangle within minimal, static traces.

Method: The competition involves two tasks: open-set writer identification and cross-writer pen classification, using a controlled dataset of 46,155 circle images and a ResNet baseline.

Result: The best-performing submissions achieved a Top-1 accuracy of 64.801% for writer identification and 92.726% for pen classification.

Conclusion: CircleID establishes a new baseline for minimal-trace analysis, providing insights into model generalization and feature disentanglement.
...
TL;DR: The paper proposes an Object-Centric OOD detection framework that leverages object co-occurrence patterns within images to improve OOD detection, particularly for near-OOD data.


<details>
  <summary>Details</summary>
Motivation: Existing OOD detection methods neglect rich contextual information within images, leading to struggles in detecting near-OOD data due to simplicity bias in models.

Method: The proposed framework predicts disentangled representations for test samples, adaptively divides patterns into three scenarios based on object co-occurrence patterns in ID training data, and performs OOD detection in a divide-and-conquer manner.

Result: The method demonstrates competitive results across challenging and full-spectrum OOD settings, addressing both semantic and covariate shifts.

Conclusion: The Object-Centric OOD detection framework effectively captures object co-occurrence patterns to improve OOD detection, particularly for near-OOD data, by leveraging semantic contextual relationships within images.
...
TL;DR: The authors extend their previous vision-based vehicle classification approach by incorporating spatial awareness of vehicle parts, improving robustness against false detections and achieving comparable accuracy to state-of-the-art end-to-end CNNs.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the interpretability and extensibility of vehicle classification systems in Intelligent Transportation Systems (ITS) while maintaining high accuracy.

Method: The authors propose a part-based approach that decomposes CNNs into a detector for vehicle parts, feature construction, and classification by a decision tree. They extend this approach by constructing a full spatial probability map to condition the presence of each part with respect to a given vehicle category and use softmax regression for classification.

Result: The proposed method shows considerably improved robustness against false detections and achieves comparable accuracy to state-of-the-art end-to-end CNNs, effectively challenging the trade-off between accuracy and explainability.

Conclusion: The research represents a significant advance in vehicle classification for ITS, forming the basis for systems that combine high accuracy with intuitive interpretability.
...
TL;DR: The paper introduces BRIDGE, a model for coarse-mask local image editing that overcomes mask-shape bias by separating mask processing from the DiT backbone and introducing a learnable Discrete Geometric Gate for token-level positional-embedding routing.


<details>
  <summary>Details</summary>
Motivation: The authors identify a failure mode in coarse-mask local image editing where rough masks can become unintended shape priors, causing the generated object to be pulled towards the mask boundary. They frame this issue as mask-shape bias and aim to develop a model that can modify a user-indicated region while preserving the surrounding scene.

Method: The authors propose BRIDGE, which uses a Two-Zone Constraint to ensure the background remains stable while the editable region follows the instruction without inheriting the mask contour. BRIDGE keeps masks outside the DiT backbone for support construction and blending and introduces a learnable Discrete Geometric Gate for token-level positional-embedding routing.

Result: The authors evaluate BRIDGE on several benchmarks, including BRIDGE-Bench, MagicBrush, and ICE-Bench. BRIDGE achieves state-of-the-art results, improving Local SigLIP2-T from 0.262 with FLUX.1-Fill and 0.390 with ACE++ to 0.503 on BRIDGE-Bench. Zero-shot results on MagicBrush and ICE-Bench also indicate competitive alignment and source preservation.

Conclusion: The authors conclude that BRIDGE effectively addresses mask-shape bias in coarse-mask local image editing by separating mask processing from the DiT backbone and introducing a learnable Discrete Geometric Gate. BRIDGE achieves state-of-the-art results on several benchmarks while maintaining a compact model size.
...
TL;DR: The authors propose EyeCue, a framework to detect driver cognitive distraction using gaze-empowered egocentric video understanding, achieving 74.38% accuracy.


<details>
  <summary>Details</summary>
Motivation: Driver cognitive distraction is difficult to detect and a major cause of road collisions, despite the driver appearing visually attentive.

Method: EyeCue integrates eye gaze with egocentric video for context-aware modeling of driver's attention, using a comprehensive multi-scenario dataset called CogDrive.

Result: EyeCue achieves 74.38% accuracy, outperforming 11 baselines by over 7%, and shows strong generalizability across various driving scenarios.

Conclusion: The proposed framework effectively detects driver cognitive distraction by modeling gaze-context interactions, highlighting its potential for multimodal cognitive distraction detection.
...
TL;DR: The paper proposes a novel approach for makeup transfer, addressing challenges in preserving identity and generalizing to real-world scenarios.


<details>
  <summary>Details</summary>
Motivation: Existing makeup transfer methods face challenges in preserving identity and generalizing to real-world scenarios due to limitations in synthetic supervision and domain gaps.

Method: The paper proposes ConsistentBeauty, a data curation pipeline, and RealBeauty, a synthetic-to-real post-training framework, which adapts the model to real-world scenarios through reinforcement learning.

Result: The proposed method achieves state-of-the-art performance on multiple benchmarks, demonstrating clear advantages in identity preservation and performance on complex real-world cases.

Conclusion: The paper presents a novel approach for makeup transfer that addresses critical challenges in the field, providing improved performance and generalizability in real-world scenarios.
...
TL;DR: The paper proposes a unified framework for video understanding reward modeling, including a benchmark (VURB) and a large-scale preference dataset (VUP-35K), and trains state-of-the-art reward models (VideoDRM and VideoGRM).


<details>
  <summary>Details</summary>
Motivation: The lack of robust evaluation benchmarks and high-quality preference data is severely limiting progress in video understanding reward modeling.

Method: The authors propose a unified framework spanning benchmark design, data construction, and reward model training, and introduce VURB and VUP-35K.

Result: The trained reward models (VideoDRM and VideoGRM) achieve state-of-the-art performance on VURB and VideoRewardBench, and show significant gains under best-of-$N$ test-time scaling.

Conclusion: The proposed framework, benchmark, and dataset enhance reward performance and model reasoning capability, and can facilitate future research in video understanding reward modeling.
...
TL;DR: SAVEMem, a training-free framework, improves online streaming video understanding by incorporating semantic awareness into memory generation and query-aware retrieval, achieving better performance and reduced GPU memory usage.


<details>
  <summary>Details</summary>
Motivation: The challenge of memory management in online streaming video understanding due to unbounded streams and unpredictable query timing, which existing methods struggle with by not incorporating semantic signals in compression decisions and having hard-to-coordinate compression and retrieval stages.

Method: SAVEMem, a dual-stage framework, first builds a three-tier streaming memory under a constant memory budget with semantic salience from a pseudo-question bank, then performs query-aware retrieval with an anchor-conditioned recency gate and late interaction between query and memory tokens.

Result: SAVEMem improves the OVO-Bench overall score from 52.27 to 62.69, yields consistent gains on StreamingBench and ODV-Bench, and reduces peak GPU memory by 48% at 128 frames over the backbone when applied to Qwen2.5-VL without training.

Conclusion: SAVEMem effectively addresses the memory management challenge in online streaming video understanding by integrating semantic awareness, leading to enhanced performance and efficiency.
...
TL;DR: The paper proposes DUST, a method for reconstructing dynamic scenes from Vehicle-to-Infrastructure Cooperative Autonomous Driving (VICAD) data, addressing temporal asynchrony between vehicle and infrastructure cameras.


<details>
  <summary>Details</summary>
Motivation: The motivation is to solve the problem of temporal asynchrony between vehicle and infrastructure cameras in VICAD data, which causes severe ghosting on dynamic agents in existing Gaussian Scene Graph methods.

Method: The proposed method is DUST (DecoUpled Spatio-Temporal) Gaussian Scene Graph, which shares a canonical Gaussian set per agent for appearance consistency and maintains decoupled pose trajectories aligned to each source's true capture timestamps.

Result: DUST achieves state-of-the-art performance on 26 sequences from V2X-Seq, improving dynamic-area PSNR by 3.2 dB over the strongest baseline and reducing Fréchet Video Distance by 37.7%, with robustness under larger temporal asynchrony.

Conclusion: The paper concludes that DUST successfully addresses the representation-level failure in existing methods, eliminating cross-source interference and improving the reconstruction of dynamic scenes in VICAD data.
...
TL;DR: The paper investigates the properties of a latent space that are friendly for generative modeling in latent diffusion models. It proposes a new tokenizer, Prior-Aligned AutoEncoder (PAE), which shapes the latent manifold to improve generation quality and training efficiency.


<details>
  <summary>Details</summary>
Motivation: The motivation is to understand what kind of latent space is suitable for generative modeling in latent diffusion models, as existing tokenizers are not optimized for this purpose.

Method: The method involves constructing controlled tokenizer variants to identify key properties of a diffusion-friendly latent manifold. The proposed PAE uses refined priors and perturbation-based regularization to shape the latent manifold.

Result: The results show that PAE improves training efficiency and generation quality on ImageNet 256x256, achieving a new state-of-the-art gFID of 1.03 and up to 13x faster convergence.

Conclusion: The conclusion is that organizing the latent manifold is crucial for latent diffusion models, and the proposed PAE is an effective approach to achieve this.
...
TL;DR: The paper introduces medvigil, an evaluation suite for medical vision-language models (VLMs) to test their ability to recognize when the evidential basis for an answer has failed, and reports on the performance of various models.


<details>
  <summary>Details</summary>
Motivation: The motivation is to ensure trustworthy clinical use of medical VLMs by evaluating their ability to detect failures in the evidential basis for an answer.

Method: The authors introduce a 300-case evaluation suite, medvigil, which includes various types of perturbations such as false premises, wording perturbations, knowledge-only rewrites, and ROI-corrupted images. They also report on the performance of 16 vision-capable models and 2 text-only baselines using 7 correctness-conditioned audit metrics.

Result: The results show that the strongest audited model (Claude Opus) achieves a medvigil Composite Score (MCS) of 69.2, leaving a 14.1-point composite headroom above the strongest audited model, with an independent radiologist scoring MCS 83.3 at a silent-failure rate of 5.8%.

Conclusion: The paper concludes that the medvigil evaluation suite provides a robust way to evaluate the performance of medical VLMs in detecting failures in the evidential basis for an answer, and that there is still room for improvement in the performance of current models.
...
TL;DR: OneWM-VLA improves vision-language-action models by compressing visual input into a single semantic token per frame, achieving better long-horizon performance with fewer parameters.


<details>
  <summary>Details</summary>
Motivation: The design of auxiliary world modules on top of pretrained vision-language-action (VLA) models is an open question, particularly under constrained adaptation budgets.

Method: OneWM-VLA uses Adaptive Attention Pooling to compress visual input into a single semantic token per frame and a single flow-matching objective for latent stream and action trajectory production.

Result: OneWM-VLA improves average success rates on MetaWorld MT50 (47.9% to 61.3%), LIBERO-Long (85.2% to 95.6%), and Fold Cloth (20.0% to 60.0%) with 14.71M LoRA parameters on a 2B backbone.

Conclusion: OneWM-VLA effectively reduces per-frame visual bandwidth without compromising long-horizon performance, achieving state-of-the-art results with efficient parameterization.
...
TL;DR: The authors propose Delta-Adapter, a method for exemplar-based image editing that learns transferable editing semantics under single-pair supervision, improving editing accuracy and content consistency.


<details>
  <summary>Details</summary>
Motivation: Existing methods for exemplar-based image editing rely on a pair-of-pairs supervision paradigm, which is difficult to curate at scale and limits generalization across diverse edit types.

Method: The authors leverage a pre-trained vision encoder to extract a semantic delta that encodes the visual transformation between two images, and inject it into a pre-trained image editing model via a Perceiver-based adapter.

Result: Delta-Adapter consistently improves both editing accuracy and content consistency over four strong baselines on seen editing tasks, while also generalizing more effectively to unseen editing tasks.

Conclusion: The proposed method, Delta-Adapter, enables single-pair supervision for exemplar-based image editing, allowing for more efficient training and improved performance on diverse edit types.
...
TL;DR: CoopNet improves cooperation between co-trained networks by dynamically adjusting gradient allocation for equitable learning progress, applied to motion-aware self-supervised depth map prediction.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enhance the cooperation of co-trained networks to ensure balanced learning progress, particularly in motion-aware self-supervised prediction of depth maps.

Method: CoopNet dynamically adapts gradient apportionment and introduces a new hybrid loss based on a distribution model of photo-metric reconstruction errors.

Result: CoopNet achieves improved or comparable results to state-of-the-art in depth, odometry, and optical flow predictions on KITTI and CityScapes datasets.

Conclusion: CoopNet presents an effective approach for improving the cooperation of co-trained networks, leading to enhanced performance in motion-aware self-supervised depth map prediction tasks.
...
TL;DR: A unified framework called TimeLesSeg is proposed for automatic segmentation of MS lesions in MRI scans, regardless of the presence of a temporal dimension, achieving state-of-the-art performance across various datasets and metrics.


<details>
  <summary>Details</summary>
Motivation: The clinical and radiological heterogeneity of multiple sclerosis (MS) poses significant challenges for automatic lesion segmentation, and current deep learning-based approaches are susceptible to changes in distribution and input structure.

Method: TimeLesSeg, a unified contrast-agnostic framework, uses a single convolutional neural network to segment MS lesions, modeling pathological priors through lesion masks and enabling cross-sectional processing by exposing the model to cases with no prior information.

Result: TimeLesSeg outperforms the contrast-agnostic state of the art on single-modality inputs and SAMSEG in longitudinal processing, accurately capturing lesion load dynamics across three publicly available and two in-house datasets.

Conclusion: The proposed TimeLesSeg framework offers a robust and accurate solution for automatic MS lesion segmentation, addressing the challenges of clinical and radiological heterogeneity, and providing a unified approach for both cross-sectional and longitudinal processing.
...
TL;DR: The paper introduces Discrete Voxel Diffusion (DVD), a discrete diffusion framework for generating, assessing, and editing sparse voxels in 3D generative pipelines.


<details>
  <summary>Details</summary>
Motivation: The authors aim to leverage discrete diffusion for sparse voxel scaffolds in 3D generative pipelines, addressing limitations of continuous diffusion methods.

Method: The authors propose DVD, which treats voxel occupancy as a native discrete variable, allowing for simple voxel generation, uncertainty estimation, and editing.

Result: The proposed method provides quality gains, more interpretable generation dynamics, and enables tasks such as data filtering and quality assessment.

Conclusion: The paper concludes that DVD is an effective first-stage prior for sparse voxel scaffolds, offering a simple and efficient framework for voxel generation, editing, and uncertainty estimation.
...
TL;DR: The paper proposes HEART, a training-free framework for intuitive and precise edits in text-to-image diffusion models by respecting the underlying geometry of text encoder representations.


<details>
  <summary>Details</summary>
Motivation: Existing text-based control methods struggle with controlling what appears and how it appears in generated images, often leading to unintended side effects.

Method: The framework, HEART, performs Kent-aware geodesic transformations directly on the hypersphere, where text encoder representations lie.

Result: HEART enables intuitive and precise edits, such as consistent subject replacement and fine-grained attribute control, while preserving the original scene, without requiring finetuning, inversion, or optimization.

Conclusion: The paper shows that understanding the true geometry of text encoder representations and respecting it can unlock fast and controllable image generation.
...
TL;DR: The paper proposes a method called Cross3R that uses a UAV image to improve cross-view localization from satellite and ground images, achieving 6-DoF pose estimation and 3D point cloud reconstruction.


<details>
  <summary>Details</summary>
Motivation: Existing cross-view localization methods are limited to 3-DoF estimates due to the lack of cues for roll, pitch, and altitude in nadir satellite imagery, which can be overcome by introducing a UAV image as an intermediate viewpoint.

Method: The proposed method, Cross3R, is a feed-forward model that takes a satellite tile and one or both of a UAV image and a ground image as input and outputs a cross-view 3D point cloud, 6-DoF poses of every input camera, and the on-tile position and yaw of each perspective camera.

Result: Cross3R outperforms baselines in point-cloud reconstruction, 6-DoF camera-pose estimation, and cross-view localization on the constructed CrossGeo dataset and the KITTI dataset, despite not being trained on KITTI.

Conclusion: The introduction of a UAV image as an intermediate viewpoint enables more accurate cross-view localization and 3D reconstruction, and the proposed Cross3R model effectively leverages this additional information to achieve state-of-the-art performance.
...
TL;DR: The paper presents a framework for estimating dense optical flow in a single forward pass, leveraging pretrained foundation representations and avoiding iterative refinement, achieving strong cross-dataset generalization.


<details>
  <summary>Details</summary>
Motivation: The motivation is to explore if there's an alternative to scaling test-time computation for improving dense optical flow accuracy, driven by the high computational cost of recent approaches.

Method: The method extracts visual semantic features from a frozen DINO-v2 backbone, combines them with geometric cues from a monocular depth foundation model, and applies a global matching formulation to estimate dense correspondences.

Result: The approach achieves 2.81 EPE on Sintel Final without refinement, outperforming state-of-the-art methods SEA-RAFT, RAFT, GMFlow, and FlowSeek under comparable conditions.

Conclusion: The paper concludes that strong foundation priors can substitute for test-time scaling, offering a computationally efficient alternative to refinement-heavy pipelines.
...
TL;DR: A new method called SphereVAD is proposed for video anomaly detection that doesn't require large-scale annotations or training, instead leveraging pre-trained multimodal large language models and geometric reasoning.


<details>
  <summary>Details</summary>
Motivation: Existing video anomaly detection methods require large-scale annotations or task-specific training, limiting their deployment to new scenes. The authors aim to overcome this limitation.

Method: The proposed method, SphereVAD, uses a pre-trained multimodal large language model, applies Frechet mean centering, Holistic Scene Attention, and von Mises-Fisher guided Spherical Geodesic Pulling to detect anomalies without training.

Result: SphereVAD achieves state-of-the-art results among training-free approaches on three major benchmarks and remains competitive with fully supervised baselines.

Conclusion: The proposed SphereVAD framework offers a fully training-free, zero-shot approach to video anomaly detection, leveraging geometric reasoning and pre-trained models to achieve competitive performance.
...
TL;DR: The Tree Ring Analyzer Suite (TRAS) is an open-source software for automatic tree ring delineation, manual correction, and measurement in wood cross-sectional images, reducing manual effort and improving accuracy.


<details>
  <summary>Details</summary>
Motivation: The manual process of tree ring marking in dendrometry and dendrochronology is time-consuming, subjective, and difficult to scale to large datasets.

Method: TRAS integrates three detection algorithms (CS-TRD, DeepCS-TRD, and INBD) for automatic tree ring delineation, with a user interface for manual correction and measurement.

Result: TRAS achieved an F-score of 81.0% and precision of 86.4% in automatic detection, reducing manual correction effort to approximately 20% of ring boundaries, and showed excellent agreement with CooRecorder for ring-width measurements.

Conclusion: TRAS provides a flexible and reproducible solution for tree-ring analysis, available on multiple platforms, with a user-friendly interface for manual correction and measurement.
...
TL;DR: The paper presents STARFlow2, a unified multimodal generation model that integrates text and image generation using autoregressive normalizing flows, achieving strong performance in image generation and multimodal understanding benchmarks.


<details>
  <summary>Details</summary>
Motivation: The motivation is to develop a unified multimodal system that can understand, reason over, and generate interleaved text-image sequences, addressing the structural mismatch between existing text and image generation approaches.

Method: The method involves building on the Pretzel architecture, vertically interleaving a pretrained VLM stream with a TarFlow stream via residual skip connections, and using a deep-shallow flow design with a unified FAE latent space.

Result: The result is strong performance across image generation and multimodal understanding benchmarks, validating autoregressive flows as a viable foundation for unified multimodal modeling.

Conclusion: The conclusion is that autoregressive normalizing flows are a natural paradigm for unified multimodal generation, and STARFlow2 demonstrates the effectiveness of this approach in achieving strong performance in multimodal tasks.
...
TL;DR: PET-Adapter, a test-time domain adaptation framework, enables generative PET reconstruction models pretrained on phantom data to adapt to clinical datasets without paired ground truth, improving 3D reconstruction performance.


<details>
  <summary>Details</summary>
Motivation: Deep learning methods for PET image reconstruction have limited generalization to unseen clinical data distributions without extensive retraining, and the goal is to adapt these models to clinical datasets with varying anatomies, tracers, and scanner configurations.

Method: PET-Adapter introduces layer-wise low-rank anatomical conditioning and Ordered Subset Expectation Maximization-based warm-starting to adapt pretrained models to clinical datasets without paired ground truth.

Result: Experiments across multiple clinical datasets demonstrate superior 3D reconstruction performance in both full-angle and limited-angle settings, with reduced diffusion steps from 50 to 2 without compromising quality.

Conclusion: PET-Adapter enables clinical feasibility and computational efficiency for generative PET reconstruction models, adapting to clinical datasets without requiring paired ground truth.
...
TL;DR: Proposed HFRU, a reinforcement unlearning framework for vision-language models to remove sensitive knowledge, outperforming prior methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods for machine unlearning in vision-language models have limitations, such as superficial forgetting and object hallucination.

Method: HFRU operates on the vision encoder using a two-stage approach with alignment disruption and GRPO-based optimization.

Result: HFRU achieves over 98% forgetting and retention performance with negligible object hallucination.

Conclusion: HFRU effectively removes sensitive knowledge from vision-language models while mitigating hallucinations.
...
TL;DR: The paper proposes SCOPE, a framework that uses specification-guided skill orchestration to maintain semantic commitments in text-to-image models, addressing the challenge of faithfully realizing complex visual intents.


<details>
  <summary>Details</summary>
Motivation: The difficulty in faithfully realizing complex visual intents in text-to-image models due to the challenge of tracking multiple requirements across grounding, generation, and verification, referred to as the Conceptual Rift.

Method: The SCOPE framework, which maintains semantic commitments in an evolving structured specification and conditionally invokes retrieval, reasoning, and repair skills around unresolved or violated commitments.

Result: SCOPE outperforms all evaluated baselines on Gen-Arena, achieving 0.60 EGIP, and further achieves strong results on WISE-V (0.907) and MindBench (0.61).

Conclusion: The proposed SCOPE framework demonstrates the effectiveness of persistent commitment tracking for complex image generation, addressing the challenge of faithfully realizing complex visual intents in text-to-image models.
...
TL;DR: MoCoTalk, a multi-conditional video diffusion framework, generates talking-head videos by unifying four control signals: reference image, facial keypoints, 3DMM-rendered shading meshes, and speech audio.


<details>
  <summary>Details</summary>
Motivation: Existing methods for talking-head generation typically address only a subset of factors such as identity, head pose, facial expression, and mouth dynamics, and rely on fixed-weight or heuristic fusion when multiple conditions are involved.

Method: MoCoTalk uses an Adaptive Multi-Condition Router to fuse four condition streams and a Mouth-Augmented Shading Mesh to capture speech-related facial dynamics. It also introduces a lip consistency loss to tighten audio-visual alignment.

Result: MoCoTalk achieves state-of-the-art performance on the majority of structural, motion, and perceptual metrics, while offering attribute-level controllability.

Conclusion: MoCoTalk provides a more comprehensive and controllable approach to talking-head generation, addressing the limitations of existing methods.
...
TL;DR: A retrieval-guided method is proposed to generate human motion that satisfies customized zero-shot goal functions, especially for highly constrained tasks with challenging spatiotemporal restrictions.


<details>
  <summary>Details</summary>
Motivation: Current approaches for generating human motion struggle with tasks that have very challenging spatiotemporal restrictions, such as severe spatial obstacles or specified numbers of walking steps.

Method: A retrieval-guided method built on the training-free diffusion noise optimization framework, which searches within large motion datasets for guidance to satisfy difficult constraints.

Result: The proposed method successfully solves highly constrained generation tasks by optimizing diffusion noise from an improved initialization obtained via a reward-guided mask.

Conclusion: The proposed framework enables the generation of human motion that satisfies customized zero-shot goal functions, especially for highly constrained tasks, and can automatically reason for what to retrieve using LLM for relational task parsing.
...
TL;DR: A modular framework for 6D pose estimation using keypoint heatmap regression, combining object detection with 2D heatmap prediction and PnP RANSAC algorithm.


<details>
  <summary>Details</summary>
Motivation: To propose a modular framework for 6D pose estimation that leverages keypoint heatmap regression and explore its improvements.

Method: The approach combines YOLOv10m for object detection with a ResNet18-based network for 2D heatmap prediction, and uses PnP RANSAC algorithm for 6D pose estimation. It also explores RGB-D fusion and training improvements.

Result: The best RGB-only model achieved 84.50% mean ADD-based accuracy, while the RGB-D fusion model reached 92.41% on the LINEMOD dataset.

Conclusion: The proposed modular framework achieves state-of-the-art performance on 6D pose estimation, with significant improvement using RGB-D fusion.
...
TL;DR: The authors propose Flow-OPD, a unified post-training framework that integrates on-policy distillation into Flow Matching models, achieving significant improvements in text-to-image synthesis.


<details>
  <summary>Details</summary>
Motivation: Existing Flow Matching text-to-image models face bottlenecks in multi-task alignment, including reward sparsity and gradient interference, leading to a 'seesaw effect' and reward hacking.

Method: Flow-OPD adopts a two-stage alignment strategy, using single-reward GRPO fine-tuning and a Flow-based Cold-Start scheme, followed by on-policy sampling, task-routing labeling, and dense trajectory-level supervision.

Result: Flow-OPD achieves significant improvements over vanilla GRPO, raising GenEval score from 63 to 92 and OCR accuracy from 59 to 94, while preserving image fidelity and human-preference alignment.

Conclusion: Flow-OPD establishes a scalable alignment paradigm for building generalist text-to-image models, demonstrating a 'teacher-surpassing' effect and outperforming existing methods.
...
TL;DR: The paper proposes Proxy3D, a method for enhancing spatial intelligence in vision-language models (VLMs) by using compact 3D proxy representations, achieving competitive or state-of-the-art performance in 3D visual question answering, visual grounding, and general spatial intelligence benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing methods for spatial intelligence in VLMs follow a conventional 2D pipeline, which can lead to failures in achieving spatial consistency and lack efficiency in vision sequence serialization. The authors aim to address these limitations by proposing a method that incorporates 3D geometric priors.

Method: The Proxy3D method uses compact 3D proxy representations for the vision modality. It employs semantic and geometric encoders to extract scene features from video frames, performs semantic-aware clustering to obtain a set of proxies in the 3D space, and applies multi-stage training to adopt the proposed 3D proxy representations with the VLM.

Result: The proposed method achieves competitive or state-of-the-art performance in 3D visual question answering, visual grounding, and general spatial intelligence benchmarks, especially when using shorter sequences for vision information.

Conclusion: The Proxy3D method offers a promising approach to enhancing spatial intelligence in VLMs by leveraging compact 3D proxy representations, which can lead to improved performance in various applications.
...
TL;DR: A new image reconstruction method called EmambaIR is proposed, which uses a Efficient visual State Space Model to process event streams, addressing limitations of CNNs and ViTs.


<details>
  <summary>Details</summary>
Motivation: Existing event-based image reconstruction methods using CNNs and ViTs face limitations, including CNNs' inability to capture global feature correlations and ViTs' high computational complexity.

Method: EmambaIR introduces two key components: the cross-modal Top-k Sparse Attention Module (TSAM) and the Gated State-Space Module (GSSM), which enable efficient processing of spatially sparse and temporally continuous event streams.

Result: EmambaIR significantly outperforms state-of-the-art methods in three image reconstruction tasks (motion deblurring, deraining, and HDR enhancement) while reducing memory consumption and computational cost.

Conclusion: EmambaIR offers a more efficient and effective solution for event-based image reconstruction, addressing the limitations of existing architectures and achieving superior performance.
...
TL;DR: Introduces Normalizing Trajectory Models (NTM) for fast image generation with exact likelihood.


<details>
  <summary>Details</summary>
Motivation: Existing few-step generation methods sacrifice likelihood framework; need a method that balances speed and likelihood.

Method: NTM models reverse steps as conditional normalizing flows with exact likelihood training, combining invertible blocks and a deep predictor.

Result: NTM matches or outperforms baselines in four sampling steps on text-to-image benchmarks while retaining exact likelihood.

Conclusion: NTM enables fast and likely image generation, offering a balance between speed and likelihood.
...
TL;DR: The study examined the metacognitive abilities of 33 large language models across six domains, finding significant variation within models and domains, and highlighting the importance of domain-specific evaluation.


<details>
  <summary>Details</summary>
Motivation: To investigate the metacognitive quality of large language models across different domains and evaluate their performance using verbalized confidence.

Method: Administering 1,500 MMLU items to 33 frontier LLMs and computing Type-2 AUROC per model-domain cell using verbalized confidence.

Result: Every model showed non-trivial domain-level variation, with Applied/Professional knowledge being the easiest domain and Formal Reasoning and Natural Science being the hardest. The study also found significant within-family profile-shape clustering for some models.

Conclusion: The results highlight the importance of domain-specific evaluation and benchmark-stage domain screening before deployment in specific application areas, as aggregate metrics may mask within-model variation across domains.
...
TL;DR: VITA-QinYu, an expressive end-to-end spoken language model, generates human-like speech for conversations, role-playing, and singing.


<details>
  <summary>Details</summary>
Motivation: Current spoken language models lack expressiveness beyond linguistic content, such as personality, mood, or performance elements.

Method: VITA-QinYu uses a hybrid speech-text paradigm with multi-codebook audio tokens and a comprehensive data generation pipeline to synthesize 15.8K hours of training data.

Result: VITA-QinYu outperforms peer models in role-playing and singing, achieving state-of-the-art conversational accuracy and fluency.

Conclusion: VITA-QinYu is the first expressive end-to-end spoken language model that supports role-playing and singing generation, achieving superior expressiveness and state-of-the-art performance.
...
TL;DR: This paper introduces IntentGrasp, a benchmark for evaluating the intent understanding capability of Large Language Models (LLMs), and proposes Intentional Fine-Tuning (IFT) to improve this ability.


<details>
  <summary>Details</summary>
Motivation: Accurately understanding intent behind speech, conversation, and writing is crucial for developing helpful LLM assistants.

Method: The paper constructs IntentGrasp from 49 high-quality corpora, and evaluates 20 LLMs across 7 families using this benchmark. It also proposes IFT, which fine-tunes models on the IntentGrasp training set.

Result: The evaluations show unsatisfactory performance of LLMs, with scores below 60% on All Set and below 25% on Gem Set. However, IFT yields significant gains of 30+ F1 points on All Set and 20+ points on Gem Set.

Conclusion: This study sheds light on a promising path towards more intentional, capable, and safe AI assistants by benchmarking and boosting intent understanding ability.
...
TL;DR: Introduced TajPersLexon, a Tajik-Persian parallel lexical resource, and evaluated methods for cross-script lexical retrieval, achieving high accuracy.


<details>
  <summary>Details</summary>
Motivation: To create a lexical resource for low-resource settings and compare methods for cross-script lexical retrieval and transliteration.

Method: Compared three methodological families: a hybrid pipeline, neural sequence-to-sequence models, and retrieval methods.

Result: Neural and retrieval baselines achieved 98-99% top-1 accuracy, while a hybrid model achieved 96.4% accuracy in an OCR post-correction task.

Conclusion: The task of cross-script lexical retrieval is solvable with high accuracy, and a hybrid model offers a favorable accuracy-efficiency trade-off for practical applications.
...
TL;DR: The paper introduces MIST, a dataset for voice-based interfaces to control IoT devices, and evaluates multimodal LLMs on this task, revealing a significant performance gap.


<details>
  <summary>Details</summary>
Motivation: The increasing presence of IoT devices requires voice-based interfaces that can handle complex user experiences, which is a challenging task that involves modeling spatiotemporal constraints, speech inputs, dynamic state tracking, and mixed-initiative interaction patterns.

Method: The authors propose MIST, a synthetic multi-turn, voice-driven code generation task that operates over IoT devices, and evaluate open- and closed-weight multimodal LLMs on this task.

Result: The results show a significant gap between open- and closed-weight multimodal LLMs on MIST, and that even frontier closed-weight LLMs have substantial room for improvement.

Conclusion: The paper concludes by releasing MIST and an extensible data generation framework to facilitate research on mixed-initiative voice assistants that reason about physical world constraints.
...
TL;DR: The paper presents a framework for developing Human-Centered Large Language Models (HCLLMs) that integrates perspectives from NLP, HCI, and responsible AI to prioritize human priorities and values.


<details>
  <summary>Details</summary>
Motivation: The increasing influence of Large Language Models (LLMs) in various aspects of life necessitates a shift in development approach to prioritize not only technical capabilities but also human priorities.

Method: The framework for HCLLMs integrates perspectives from NLP, HCI, and responsible AI, and provides human-centered insights and recommendations for developers at each stage of the pipeline, from system design to deployment.

Result: The paper offers a comprehensive framework for developing HCLLMs and applies these insights to a case study on the future of work with HCLLMs.

Conclusion: The development of HCLLMs requires addressing human concerns, preferences, values, and goals at every stage of the pipeline, not just as an afterthought, to ensure that these systems are aligned with human priorities and values.
...
TL;DR: A new AI-generated text detection model called MELD is proposed, which uses multi-task learning and auxiliary supervision to improve robustness and accuracy.


<details>
  <summary>Details</summary>
Motivation: The motivation is to develop a reliable AI-generated text detection model that can withstand attacks and adversarial rewrites, and operate at low false-positive rates, which is crucial for academic integrity, content moderation, and provenance tracking.

Method: MELD uses a shared encoder with multiple auxiliary heads for generator-family, attack-type, and source-domain detection, and balances the losses with learned homoscedastic uncertainty weights. It also uses an EMA teacher and an attack-augmented student for distillation, and a hard-negative pairwise ranking loss to improve robustness.

Result: MELD achieves state-of-the-art performance on the public RAID leaderboard and matches or outperforms supervised baselines on standard held-out benchmarks. It also achieves 99.9% TPR at 1% FPR on the MELD-eval benchmark without additional finetuning.

Conclusion: The proposed MELD model provides a robust and accurate solution for AI-generated text detection, which is essential for maintaining academic integrity and content moderation in the era of large language models.
...
TL;DR: This paper evaluates the ability of large language models (LLMs) to adapt responses to the certainty of retrieved information and proposes an interaction strategy to improve their reliability.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the limitation of LLMs in adapting responses to the certainty of retrieved information, which has real consequences in high-stakes domains like medicine and finance.

Method: The authors evaluate eight LLMs on their context-certainty obedience and propose an interaction strategy combining prior reminders, certainty recalibration, and context simplification.

Result: The proposed approach reduces obedience errors by 25% on average, without modifying model weights, demonstrating the efficacy of interaction design in enhancing LLM reliability.

Conclusion: The paper concludes that LLMs have systematic limitations in handling uncertainty and that the proposed interaction strategy can improve their reliability across diverse LLMs.
...
TL;DR: The study examines the behavior of Large Language Models (LLMs) in annotating a Bengali social media dataset, revealing a phenomenon called 'instruction-induced label collapse' where LLMs favor certain labels, leading to under-detection of minority categories.


<details>
  <summary>Details</summary>
Motivation: The motivation is to investigate LLM behavior in low-resource languages, specifically Bengali, when annotating social media datasets, and to identify potential biases in the annotation process.

Method: The study employs a structured pipeline where multiple LLMs (ChatGPT, Gemini, Claude, and Grok) annotate separate partitions of the MultiSoc-4D dataset, sharing a common validation set, to systematically diagnose LLM behavior.

Result: The study discovers 'instruction-induced label collapse', where LLMs show a systematic preference towards certain labels, leading to high agreement rates but under-detection of minority categories, such as hateful and sarcastic content.

Conclusion: The study concludes that LLMs exhibit annotation biases in Bengali NLP, which can lead to inaccurate results, and releases the MultiSoc-4D dataset as a diagnostic benchmark for annotation biases.
...
TL;DR: Introduces GoSkills, a method for retrieving and structuring skills for agents, improving their usability and efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing methods for retrieving skills for agents are limited as they return atomic skills or bundles without clear internal roles, making it hard for agents to execute them effectively.

Method: GoSkills is an inference-time group-structured retrieval method that constructs compact, role-labeled execution contexts from a typed skill graph.

Result: Experiments show GoSkills preserves requirement coverage, improves over flat skill-access baselines, and often improves reward and runtime relative to structural retrieval references.

Conclusion: GoSkills offers a more effective way to retrieve and structure skills for agents, enhancing their usability and efficiency without altering the agent, skill payloads, or execution environment.
...
TL;DR: The paper presents a new approach to diffusion language models (DLMs) by modeling text as a continuous diffusion process over fixed-width binary bitstreams, achieving state-of-the-art results in language generation.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this work is to improve the sample quality and diversity of diffusion language models, which have historically lagged behind autoregressive models.

Method: The authors propose a novel approach that represents semantic tokens as analog bit sequences and utilizes a matched-filter residual parameterization, along with a stochastic sampler that applies Langevin-type corrections.

Result: The proposed 130M-parameter bitstream model achieves a generative perplexity of 59.76 on the One Billion Word Benchmark and establishes a new Pareto frontier on OpenWebText, outperforming prior DLM baselines and reaching autoregressive reference.

Conclusion: The paper concludes that the proposed approach provides a scalable paradigm for language generation as vocabulary sizes grow, removing the vocabulary scaling bottleneck shared by standard DLMs.
...
TL;DR: The authors propose Cognitive Agent Compilation (CAC), a framework for creating educational AI systems that are more inspectable and editable, using a teacher LLM to compile problem-solving knowledge into an explicit target agent.


<details>
  <summary>Details</summary>
Motivation: The broad pretraining of large language models (LLMs) makes them hard to constrain and poor substitutes for controllable learners in educational settings, where inspectable and editable knowledge states are required.

Method: The authors propose CAC, a framework that separates knowledge representation, problem-solving policy, and verification and update rules, using a strong teacher LLM to compile problem-solving knowledge into an explicit target agent.

Result: An early proof of concept implemented with Small Language Models surfaces key design trade-offs, particularly between explicit control and scalable generalization.

Conclusion: CAC is an initial step toward bounded-knowledge AI for educational applications, offering a more inspectable and editable approach to AI in education.
...
TL;DR: The paper introduces NSMQ Riddles, a new benchmark for evaluating Large Language Models (LLMs) in scientific and mathematical reasoning, using riddles from Ghana's National Science and Maths Quiz. The evaluation shows that state-of-the-art LLMs perform worse than the best student contestants, highlighting the challenge of the dataset.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the underrepresentation of datasets from the Global South in evaluating LLMs for science and mathematics education, and to create a more challenging benchmark that requires reasoning skills.

Method: The authors created a novel benchmark called NSMQ Riddles, consisting of 1.8K riddle questions from Ghana's National Science and Maths Quiz competition. They evaluated state-of-the-art LLMs, including closed and open models, with high and low reasoning settings.

Result: The evaluation shows that state-of-the-art LLMs performed worse than the best student contestants, indicating that the NSMQ Riddles dataset is challenging even for advanced LLMs.

Conclusion: The paper contributes a novel and challenging benchmark for scientific and mathematical reasoning from the Global South, enabling a more comprehensive evaluation of LLMs' capabilities for science and mathematics education.
...
TL;DR: The paper introduces GSM-SEM, a framework for generating semantically diverse math problem variants, and demonstrates its effectiveness in evaluating LLM performance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the issue of memorization in math reasoning benchmarks, where models may perform well by memorizing test sets rather than truly understanding the problems.

Method: The method involves introducing GSM-SEM, a reusable and stochastic framework for generating semantically diverse benchmark variants by perturbing problem statements and modifying entities, attributes, and relationships.

Result: The result shows that evaluating 14 SOTA LLMs using GSM-SEM leads to consistent performance drops, with an average drop rate of 28% in the maximum strictness configuration.

Conclusion: The conclusion is that GSM-SEM provides a more robust evaluation of LLM performance in math reasoning tasks by reducing the bias of memorization and providing a more diverse set of problems.
...
TL;DR: The paper proposes a new approach to clinical diagnosis using a Partially Observable Markov Decision Process (POMDP) that simulates real-world clinical interactions, including noisy and incomplete information.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for medical LLMs and automatic diagnosis methods simplify the clinical diagnosis process, ignoring its interactive and uncertain nature.

Method: The authors formalize clinical diagnosis as a POMDP with three action types and introduce a systematic noise model. They train a diagnosis agent, MedExAgent, using a two-stage pipeline.

Result: MedExAgent achieves diagnostic performance comparable to larger models while maintaining cost-efficient examination strategies.

Conclusion: The paper addresses the gap in existing benchmarks and methods for clinical diagnosis by proposing a more realistic and effective approach.
...
TL;DR: The paper proposes WiCER, an algorithm to improve the compilation of domain knowledge into a persistent artifact for LLMs, addressing the 'compilation gap' and achieving better performance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enable LLMs to access context at sub-second latency with zero retrieval failure by compiling domain knowledge into a persistent artifact, but current methods suffer from a 'compilation gap' that discards critical facts.

Method: The method proposed is WiCER (Wiki-memory Compile, Evaluate, Refine), an iterative algorithm inspired by counterexample-guided abstraction refinement (CEGAR) that evaluates compiled wikis, identifies dropped facts, and forces their preservation in subsequent compilations.

Result: The results show that WiCER recovers 80% of lost quality in one to two iterations, reducing catastrophic failures by 55% relative, and outperforms other methods such as RAG and blind compilation.

Conclusion: The conclusion is that WiCER effectively addresses the compilation gap, enabling the compilation of high-quality wikis for LLMs and achieving better performance in terms of quality and latency.
...
TL;DR: The paper proposes SCoL, a post-training framework for continual context consolidation in large language models, improving acquisition and retention of information in long-context workflows.


<details>
  <summary>Details</summary>
Motivation: The need for large language models to effectively consolidate information from streams of passages and long-context workflows without interference.

Method: The authors propose SCoL, a framework using meta-reinforcement learning to generate textual update instructions for updating Transformer layers.

Result: SCoL improves acquisition and retention over several baselines, including prompting, summarization, batch test-time training, and sequential finetuning.

Conclusion: The proposed SCoL framework enables scalable streaming consolidation, allowing large language models to effectively consolidate information in long-context workflows.
...
TL;DR: The paper argues that using a single transcription convention as ground truth in automatic speech recognition (ASR) evaluation can lead to epistemic injustice, particularly for speakers with aphasia, and proposes a new approach called WER-Range to report performance across multiple legitimate conventions.


<details>
  <summary>Details</summary>
Motivation: The authors are motivated by the issue of reference monism in ASR evaluation, which can lead to epistemic injustice for speakers with aphasia whose speech includes clinically meaningful disfluencies that are treated as errors.

Method: The authors develop a philosophical framework introducing the hermeneutical gap and formalize Epistemic Injustice Distance (EID) to measure the cost of reference monism. They also propose WER-Range, a new approach to report performance across legitimate conventions.

Result: The authors demonstrate empirically using AphasiaBank that Word Error Rate (WER) varies depending on which convention defines ground truth.

Conclusion: The paper concludes that reference monism commits epistemic injustice and proposes WER-Range as a solution to report performance across multiple legitimate conventions, providing a more inclusive and fair evaluation of ASR systems.
...
TL;DR: The Translation Tax, a measure of how translation affects benchmark scores, is not a uniform scalar value but rather depends on the estimator and item being evaluated.


<details>
  <summary>Details</summary>
Motivation: To audit the claim that translated benchmarks inflate scores by preserving English-source cues in an English-to-Chinese setting.

Method: Using three proxy estimators to evaluate the Translation Tax, including back-translation gaps, cue-score calibration, and a native-control comparison, as well as a same-item LLM-naturalization stress test.

Result: The Translation Tax is not a single value, but a set of estimator- and item-dependent validity risks, with high-residue items benefiting while low-residue items do not.

Conclusion: The Translation Tax should not be treated as a scalar value, but rather as a complex phenomenon that depends on various factors, and researchers should be cautious when evaluating and reporting translated multilingual benchmark papers.
...
TL;DR: A hierarchical evaluation framework called SAGE is proposed to assess literary quality through ontology-grounded interpretive dimensions, achieving high reliability and identifying areas where generative models lag behind human literary production.


<details>
  <summary>Details</summary>
Motivation: Evaluating literary quality is challenging due to its complex and subjective nature, and current computational methods struggle to measure it accurately.

Method: The SAGE framework uses structured large language model evaluation with multi-round iterative reflection and independent validation, across three analytical layers (cultural, emotional-psychological, existential-philosophical).

Result: The framework achieves 98.8% score convergence and greater than 94% inter-rater agreement, and reveals a consistent genre hierarchy (Canonical > Pulp > LLM) with layer-specific discrimination.

Conclusion: The study demonstrates that theory-driven LLM evaluation can achieve measurement-grade reliability and support systematic identification of areas for improvement in generative models, with implications for scalable automated evaluation of open-ended text generation.
...
TL;DR: The paper proposes RIS, a framework that improves multimodal large language models' (MLLMs) ability to reason visually by integrating spatial and semantic evidence, leading to more accurate and interpretable results.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs struggle with fine-grained perception due to information bottlenecks when compressing visual evidence into textual thoughts. Recent latent visual reasoning methods suffer from manifold compatibility issues.

Method: The RIS framework constructs a grounded reasoning dataset and anchors latent tokens to spatial and semantic evidence using progressive attention bottlenecks and short language transition tokens.

Result: Experiments on multiple benchmarks (V*, HRBench4K, HRBench8K, MMVP, and BLINK) show consistent improvements over baselines, with RIS learning diverse, interpretable, and progressively integrated latent trajectories.

Conclusion: The RIS framework offers a practical path toward faithful internal visual reasoning in MLLMs, addressing the limitations of current methods and improving their ability to reason visually.
...
TL;DR: The paper proposes an architecture-lifecycle framework for deployment-grounded reliability in Computer-use agents (CUAs), analyzing capability formation, authority exposure, failure manifestation, and control placement.


<details>
  <summary>Details</summary>
Motivation: Existing surveys on CUAs do not explicitly connect capability formation, authority exposure, failure manifestation, and control placement, and the authors aim to address this gap.

Method: The authors develop an architecture-lifecycle framework, consisting of an architectural view with Perception, Decision, and Execution layers, and a lifecycle view with Creation, Deployment, Operation, and Maintenance stages.

Result: The framework synthesizes representative systems, benchmarks, and security/privacy studies, and highlights open challenges in controllable grounding, long-horizon constraint preservation, safe authority binding, and more.

Conclusion: The paper concludes by highlighting open challenges in CUAs, including controllable grounding, long-horizon constraint preservation, safe authority binding, mixed-trust runtime defense, privacy-preserving memory, and continual assurance.
...
TL;DR: The paper compares Full Fine-Tuning (FFT) and Low-Rank Adaptation (LoRA) of Large Language Models, proposing a unified framework called Mixture of LoRA and Full (MoLF) Fine-Tuning that dynamically combines both methods.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of relying solely on either FFT or LoRA for fine-tuning Large Language Models, as both methods have their strengths and weaknesses.

Method: The paper proposes MoLF, a unified framework that enables continuous navigation between FFT and LoRA by dynamically routing updates between the two methods at the optimizer level. It also introduces MoLF-Efficient, a variant that freezes base weights and only routes updates among a pair of LoRA experts.

Result: The results show that MoLF either improves on or stays within 1.5% of the better of FFT and LoRA across all settings, while MoLF-Efficient outperforms prior adaptive LoRA approaches by up to 20% on Fact and 9% on Med and SQL.

Conclusion: The paper concludes that MoLF and MoLF-Efficient provide a stable and efficient way to combine the strengths of FFT and LoRA, leading to improved performance across diverse tasks and language models.
...
TL;DR: The paper proposes a new framework for web agents to perceive web pages at a functional region level, rather than the traditional element-level granularity, and shows that it improves performance and reduces observation length.


<details>
  <summary>Details</summary>
Motivation: The current design choice of treating observation at the same element-level granularity as the action space in web agents leaves the page's functional organization implicit, forcing the agent to infer it from element-level signals at every step.

Method: The authors propose Region4Web, a framework that reorganizes the AXTree into functional regions through hierarchical decomposition and semantic abstraction, and PageDigest, a web-specific inference pipeline that delivers region-level observation to the actor agent.

Result: The proposed approach substantially reduces observation length while improving overall task success rate across diverse backbone large language models (LLMs) and established agent methods on the WebArena benchmark.

Conclusion: Operating at the granularity of functional regions delivers a more compact and informative basis for the actor agent than element-level processing alone.
...
TL;DR: The authors propose Distillation through Reasoning Path Compression (D-RPC), a method to improve the distillation of reasoning from large language models into smaller ones by constraining the teacher to follow a compact bank of reusable high-level reasoning paths.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the inconsistency in teacher rationales for similar problems when distilling reasoning from large language models into smaller ones, which burdens the student with noisy supervision.

Method: The proposed method is Distillation through Reasoning Path Compression (D-RPC), which uses a compact, dynamically maintained bank of reusable high-level reasoning paths to constrain the teacher and produce consistent rationales across similar problems.

Result: The results show that D-RPC consistently outperforms various baselines, including chain-of-thought distillation, freeform rationale generation, direct distillation, and structured-supervision baselines, across five math and commonsense reasoning benchmarks with two student models, while using fewer tokens.

Conclusion: The authors conclude that D-RPC is an effective method for distilling reasoning from large language models into smaller ones, achieving a trade-off between bank size and coverage, and identifying an optimal intermediate size that leads to better performance.
...
TL;DR: Reinforcement learning (RL) improves direct recall of parametric knowledge in LLMs by ~27% across multiple benchmarks, primarily by redistributing probability mass over existing knowledge.


<details>
  <summary>Details</summary>
Motivation: To investigate whether reinforcement learning (RL) can improve direct recall of parametric knowledge in LLMs, beyond its known success in reasoning tasks.

Method: Controlled zero-shot, one-hop, closed-book QA setting with binary correctness rewards, fact-level train-test deduplication, and analysis across three model families and multiple factual QA benchmarks.

Result: RL yields ~27% average relative gains in direct recall of parametric knowledge, surpassing both training- and inference-time baselines.

Conclusion: RL can improve direct recall of parametric knowledge in LLMs, primarily by redistributing probability mass over existing knowledge rather than acquiring new facts, and is particularly effective for rare and hard examples.
...
TL;DR: A new approach called CLIPer is introduced for personalizing Large Language Models (LLMs) to user preferences without extensive fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Fine-tuning LLMs for all possible user preferences is computationally expensive and impractical.

Method: CLIPer uses a classifier model to steer LLM generation dynamically at inference time, eliminating the need for extensive fine-tuning.

Result: CLIPer enables more controllable and nuanced personalization across single and multi-dimensional preferences with negligible additional computational overhead.

Conclusion: CLIPer is a lightweight and effective approach for delivering personalized language generation.
...
TL;DR: This paper explores the utilization of experience in self-evolving agents, proposing a new approach called ExpWeaver that selectively invokes experience during decision-making, leading to improved performance.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to investigate how experience should be used during runtime decision-making in self-evolving agents, as existing work has primarily focused on constructing, representing, and updating experience.

Method: The authors introduce ExpWeaver, a lightweight framework that modifies only runtime utilization of experience by exposing it as an optional resource during reasoning, and evaluate it across four representative frameworks, seven LLM backbones, and three types of environments.

Result: ExpWeaver consistently achieves the best performance among different utilization strategies, and reinforcement learning experiments show that this behavior can be amplified through training.

Conclusion: The findings suggest that agents benefit from interweaving experience use with decision-making, and call for a shift from merely studying what experience to store toward understanding how and when experience should enter decision-making.
...
TL;DR: This paper presents a baseline for token-level metaphor identification in Chinese using the PSU Chinese Metaphor Corpus and compares three model families: encoder fine-tuning, MelBERT adapted to Chinese, and Qwen3.5-9B fine-tuned with QLoRA.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the under-exploration of token-level computational identification of metaphor-related words in Chinese, relative to English, and to provide a reproducible baseline for future research.

Method: The paper systematically compares three model families: (i) encoder fine-tuning with Chinese RoBERTa-wwm-ext-large; (ii) MelBERT adapted to Chinese using a newly constructed basic-meaning resource; and (iii) Qwen3.5-9B fine-tuned with QLoRA as an instruction-tuned generative baseline.

Result: MelBERT MIP-only achieves the strongest performance at 0.7281 +/- 0.0050 test positive F1, marginally above MelBERT Full and clearly above plain RoBERTa. The Qwen QLoRA generative configuration trails encoder baselines by approximately 11 F1 points.

Conclusion: The paper concludes that MelBERT adapted to Chinese performs well on token-level metaphor identification, and provides a reproducible baseline and resources for future Chinese metaphor identification research.
...
TL;DR: The paper proposes a topology-enhanced alignment framework for large language models, utilizing 0-dimensional persistent homology to regularize semantic trajectories in hidden space, improving alignment with human preferences.


<details>
  <summary>Details</summary>
Motivation: Current alignment methods for large language models (LLMs) ignore the global geometry of the representation space, focusing on local token likelihoods or scalar scores, which may not effectively capture the semantic relationships between prompts and answers.

Method: The paper introduces two methods: Trajectory Topology Loss (TTL) for Supervised Fine-Tuning (SFT) and Topological Preference Optimization (TPO) for Direct Preference Optimization (DPO). TTL uses 0D persistent homology to extract 'prompt-answer bridges' and align the model's update direction with these bridges. TPO constructs topic-specific semantic preference vectors and aligns the improvement direction between rejected and chosen responses with these vectors.

Result: The proposed topology-enhanced objectives consistently outperform strong non-topological baselines on automatic preference metrics and LLM-judge evaluations, while maintaining or improving toxicity.

Conclusion: The paper concludes that incorporating persistent homology and trajectory geometry into alignment methods offers a promising direction for controllable alignment of large language models.
...
TL;DR: A new routing framework called BoundaryRouter is proposed to decide whether to use lightweight LLM inference or full agent execution for queries, achieving a balance between performance and latency.


<details>
  <summary>Details</summary>
Motivation: LLM agents are strong on complex reasoning tasks but are slow and costly, and many queries can be handled by LLMs alone, making effective routing between LLMs and agents a challenge.

Method: BoundaryRouter uses early behavioral experience and rubric-guided reasoning to route queries, building a compact experience memory and retrieving similar cases to guide decisions.

Result: BoundaryRouter reduces inference time by 60.6% compared to the agent and improves performance by 28.6% over direct LLM inference, outperforming other routing methods.

Conclusion: BoundaryRouter is an effective training-free routing framework that balances performance and latency for LLM and agent queries.
...
TL;DR: This paper studies how Large Language Models (LLMs) detect targeted information in imperfect text, specifically when words are corrupted by inserting whitespace characters. The results show a U-shaped curve in detection accuracy, which is explained by a mode transition hypothesis.


<details>
  <summary>Details</summary>
Motivation: Existing LLM benchmarks focus on syntactically correct inputs, leaving a gap in evaluation on imperfect text. The authors aim to study how LLMs perform on corrupted text, specifically word-boundary corruption.

Method: The authors conduct four experiments and one analysis, including inserting whitespace characters within words to corrupt text, and evaluating LLMs' detection accuracy. They also propose a mode transition hypothesis to explain the observed U-shaped curve.

Result: The results show that LLMs' detection accuracy follows a U-shaped curve with the increase in insertion rate, which is referred to as the Text Uncanny Valley. The mode transition hypothesis suggests that LLMs operate in a word-level mode for near-normal text and a character-level mode for heavily fragmented text.

Conclusion: The findings reveal a failure mode invisible to clean-text benchmarks yet directly relevant to any deployment scenario involving noisy or uncurated text inputs. The study highlights the importance of evaluating LLMs on imperfect text to ensure their robustness in real-world applications.
...
TL;DR: The paper presents a system for classifying toxic behavior in gaming communities, specifically for the EEUCA 2026 Shared Task, achieving 4th place out of 35 teams.


<details>
  <summary>Details</summary>
Motivation: The task involves classifying World of Tanks chat messages into six toxicity categories to understand toxic behavior in gaming communities.

Method: The system explores multiple approaches including encoder-based models, instruction-tuned LLMs with LoRA fine-tuning, hierarchical classification, one-vs-rest strategies, and ensemble methods. The best system combines Llama 3.1 8B with synthetic data augmentation.

Result: The best system achieved an F1-macro score of 0.6234 on the test set, placing 4th out of 35 participating teams.

Conclusion: The paper provides an extensive analysis of the dataset's annotation patterns and their impact on model generalization, revealing a 'validation trap' phenomenon where high validation performance correlates with poor test transfer.
...
TL;DR: A proxy-analyzer framework is introduced for detecting hallucinations in large language models, achieving better performance than existing methods across various analyzer architectures.


<details>
  <summary>Details</summary>
Motivation: The motivation is to detect hallucinations in large language models without requiring access to the generating model's internal workings.

Method: The framework uses a small locally hosted open-weight model to read generated text and spot hallucinations through internal activations, leveraging 18 features and training a stacking ensemble on 72,135 samples.

Result: The approach consistently outperforms ReDeEP's token-level AUC of 0.73 on RAGTruth by 7.4 to 10.3 percentage points across seven analyzer architectures, with Qwen2.5-7B reaching an F1 of 0.717.

Conclusion: The study demonstrates the effectiveness of the proxy-analyzer framework in detecting hallucinations, with results showing that bigger models are not always better, and the approach works well across different generator models.
...
TL;DR: The paper introduces LaProx, a novel eviction strategy for KV Cache in large language models, which improves efficiency while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: KV Cache growth in large language models causes substantial memory and runtime overhead, and existing eviction methods have limitations.

Method: LaProx reformulates KV Cache eviction as an output-aware, layer-wise matrix multiplication approximation problem, modeling the interaction between attention maps and projected value states.

Result: LaProx maintains model performance with only 5% of the KV cache and outperforms prior works across all configurations, achieving up to 2× accuracy loss reduction under extreme compression scenarios.

Conclusion: LaProx offers a unified eviction strategy that efficiently reduces KV Cache overhead while preserving model performance.
...
TL;DR: The authors propose ThinC, a framework that uses code as the primary reasoner for mathematical problem-solving, eliminating the need for interleaved natural language (NL) and code execution.


<details>
  <summary>Details</summary>
Motivation: The authors identify limitations in the current tool-integrated reasoning (TIR) paradigm, including code acting as a post-hoc verifier, error-prone NL computations, and overlapping roles of NL and code.

Method: The authors propose ThinC, which begins with a brief NL planning step, followed by code blocks connected by execution outputs. They train ThinC-1.7B and ThinC-4B models using supervised fine-tuning and reinforcement learning on 12.2k code-centric trajectories.

Result: ThinC-4B outperforms TIR baselines on five math benchmarks, surpassing the larger Qwen3-235B-A22B-Thinking model. ThinC reasons through code, with 99.2% of answers grounded in interpreter output, and recovers reliably from code execution failures.

Conclusion: The authors conclude that ThinC is an effective framework for mathematical problem-solving, leveraging code as the primary reasoner to improve performance and reliability.
...
TL;DR: The paper proposes SpecBlock, a block-iterative drafter that accelerates LLM inference by combining path dependence with cheap drafting, achieving 8-13% speedup improvement over EAGLE-3.


<details>
  <summary>Details</summary>
Motivation: Existing drafters have opposite weaknesses: autoregressive drafters preserve dependence but are slow, while parallel drafters are fast but lack dependence. The authors aim to combine the benefits of both.

Method: SpecBlock uses a block-iterative approach, producing K dependent positions per drafter forward, and employs two mechanisms to carry path dependence: layer-wise shift and hidden state inheritance. It also uses a co-trained rank head and a valid-prefix mask for optimization.

Result: Experiments show that SpecBlock improves mean speedup by 8-13% over EAGLE-3 at 44-52% of its drafting cost, and cost-aware adaptation extends this lead to 11-19%.

Conclusion: The proposed SpecBlock method effectively accelerates LLM inference by balancing path dependence and drafting cost, and demonstrates improved performance over existing methods.
...
TL;DR: The paper proposes a new adaptive policy called Planning-after-Trial (PaT) for code generation, which optimizes test-time computation for Large Language Models (LLMs) by invoking a planner only when necessary.


<details>
  <summary>Details</summary>
Motivation: Most existing methods use a rigid Planning-before-Trial (PbT) policy, which can be inefficient for directly solvable problems. The authors aim to improve the cost-performance tradeoff for LLMs.

Method: The authors propose PaT, an adaptive policy that uses a heterogeneous model configuration with a cost-efficient model for generation attempts and a powerful model for targeted planning interventions.

Result: The approach achieves performance comparable to a large homogeneous model while reducing inference cost by approximately 69% across multiple benchmarks and model families.

Conclusion: The proposed PaT policy advances the cost-performance Pareto frontier for LLMs, offering a more efficient use of test-time computation.
...
TL;DR: The paper presents LogiHard, a framework that transforms multiple-choice questions into logical judgments, increasing difficulty and reasoning steps. Evaluations show a significant accuracy degradation across state-of-the-art models.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the dual challenges of rapid saturation and data contamination in multiple-choice reasoning benchmarks, and to create a more challenging and valid evaluation framework.

Method: The method involves transforming 0-order selection into 2-order logical judgment using a formal framework, integrating Item Response Theory (IRT) for computerized adaptive testing (CAT), and constructing a logical reasoning dataset (LogiHard-2k) through cognitively ranking high-stakes examination questions.

Result: The result shows an accuracy degradation ranging from 31% to 56% across twelve state-of-the-art models on combinatorially hardened questions, and a 47% accuracy degradation on zero-shot transfer to MMLU.

Conclusion: The conclusion is that LogiHard provides a more challenging and valid evaluation framework, revealing a combinatorial reasoning gap in advanced models, which is domain-agnostic and stems from a training-induced completeness-verification deficit.
...
TL;DR: The paper presents MIPIAD, a defense framework against indirect prompt injection in multilingual LLM systems, evaluated on English and Bangla.


<details>
  <summary>Details</summary>
Motivation: Indirect prompt injection remains a weakness in retrieval-augmented and tool-using LLM systems, especially in multilingual settings.

Method: MIPIAD combines a sequence classifier fine-tuned from Qwen2.5-1.5B via LoRA, TF-IDF lexical features, and validation-tuned ensembling.

Result: The hybrid XLPID+TF-IDF ensemble achieves an F1 score of 0.9205, and the Boosting Ensemble achieves an AUROC of 0.9378.

Conclusion: The proposed framework effectively defends against indirect prompt injection in multilingual settings, with ensemble methods reducing the cross-lingual gap.
...
TL;DR: The paper analyzes layer pruning in Large Language Models (LLMs) through decision representation, identifying a critical 'Silent Phase' where pruning causes performance collapse.


<details>
  <summary>Details</summary>
Motivation: Existing methods fail to explain performance collapse in LLMs after layer pruning, motivating a new approach focusing on decision representation.

Method: The authors introduce metrics (Decision Margin, Option Frequency) and an Iterative Pruning method to study layer-wise decision dynamics in multiple-choice tasks.

Result: Pruning the 'Silent Phase' of the network causes immediate performance collapse, while pruning the 'Decisive Phase' has minimal impact.

Conclusion: The performance collapse in LLMs after pruning is due to disruption of the 'Silent Phase', preventing a critical decision transition.
...
TL;DR: The paper addresses the limitations of current Large Language Models (LLMs) in medical diagnosis, which are typically evaluated in static and single-turn settings. The authors propose a new approach, MedAction, to improve LLMs' ability to perform active diagnosis through a tree-structured distillation pipeline.


<details>
  <summary>Details</summary>
Motivation: Current LLMs are limited by their inability to handle dynamic and multi-turn diagnostic processes, which is a more realistic representation of clinical practice. The authors aim to bridge the gap between existing medical training data and the needs of real-world clinical decision-making.

Method: The authors introduce MedAction, a tree-structured distillation pipeline that synthesizes diverse and high-quality multi-turn diagnostic trajectories via LLM-environment interaction. They also propose two knowledge-graph-grounded metrics, Disease Trajectory Consistency (DTC) and Reasoning-Action Consistency (RAC), to filter trajectory quality.

Result: Fine-tuning an 8B model on MedAction-32K, a dataset of 32,681 trajectories from 2,896 PMC cases, achieves state-of-the-art performance among open-source models on both MedR-Bench and the curated MedAction-300-Hard benchmark.

Conclusion: The paper concludes that MedAction, a novel approach to improve LLMs' active diagnosis capabilities, can push the edge for open-source medical LLMs by providing a more realistic and effective way to train and evaluate these models.
...
TL;DR: The authors challenge assumptions about how reasoning language models process sequential chains of thought, finding that order and density of information are not crucial for answer extraction.


<details>
  <summary>Details</summary>
Motivation: The authors aim to understand how modern reasoning language models process sequential chains of thought and challenge the assumptions that every token contributes and steps must be consumed in order.

Method: The authors apply a systematic intervention pipeline (removal, masking, shuffling, and noise injection) to model-generated reasoning chains across three models and three benchmarks.

Result: The authors find that answer extraction is order-insensitive and can operate on sparse information, with results showing that shuffling and masking certain parts of the chain do not significantly affect accuracy.

Conclusion: The results establish that answer extraction operates on a sparse, order-insensitive, and structurally robust informational substrate, opening paths toward parallelized and token-efficient reasoning generation.
...
TL;DR: LaTER, a two-stage paradigm, improves large language models' efficiency and accuracy by combining latent and explicit reasoning.


<details>
  <summary>Details</summary>
Motivation: The need to balance efficiency and accuracy in large language models, particularly in tasks requiring symbolic checking.

Method: LaTER, a two-stage approach that uses latent reasoning for exploration and explicit chain-of-thought (CoT) for verification and answer generation.

Result: LaTER reduces token usage by 16%-32% while matching or improving accuracy on several benchmarks, with further gains from fine-tuning.

Conclusion: LaTER offers a promising approach to improve efficiency and accuracy in large language models, especially in tasks requiring symbolic checking.
...
TL;DR: Researchers investigate two sparse autoencoder architectures for detecting backdoors in language models, finding that Diff-SAE outperforms Crosscoders in isolating backdoor-related features.


<details>
  <summary>Details</summary>
Motivation: Detecting backdoors in language models through mechanistic interpretability is an open challenge, and the authors aim to address this by evaluating two sparse autoencoder architectures.

Method: The authors use two sparse autoencoder architectures, Crosscoders and Differential SAEs (Diff-SAE), to isolate backdoor-related features in fine-tuned models, and evaluate them across LoRA and full-rank fine-tuning regimes on SmolLM2-360M.

Result: Diff-SAE consistently and substantially outperforms Crosscoders, achieving a Backdoor Isolation Score (BIS) of 0.40 with perfect precision (1.0) and zero false positive rate across most experimental conditions.

Conclusion: The findings suggest that backdoors manifest as directional activation shifts rather than sparse feature activations, making difference-based representations fundamentally more effective for detection, with important implications for AI safety monitoring and interpretability tool development.
...
TL;DR: The study challenges the use of mean-pooled cosine similarity for comparing neural representations, finding it length-dependent, and recommends using length-invariant metrics like Centered Kernel Alignment (CKA) instead.


<details>
  <summary>Details</summary>
Motivation: The authors are motivated by the widespread use of mean-pooled cosine similarity for comparing neural representations and aim to investigate its reliability, particularly its invariance to sequence length.

Method: The authors empirically analyze the relationship between sequence length and mean-pooled cosine similarity across various models and tasks, comparing it with Centered Kernel Alignment (CKA).

Result: The study finds that mean-pooled cosine similarity is not length-invariant, with sequence length explaining a significant portion of the variance in cross-language similarities, and that CKA reduces this length effect.

Conclusion: The authors conclude that length-invariant metrics such as CKA should be the default for cross-representation comparisons, and that previous claims of cross-lingual representational convergence based on mean-pooled cosine similarity may need re-examination.
...
TL;DR: Adaptive rank allocation does not improve efficiency in reinforcement learning, specifically Group Relative Policy Optimization (GRPO), unlike in supervised fine-tuning (SFT).


<details>
  <summary>Details</summary>
Motivation: To investigate whether adaptive rank allocation, which improves efficiency in SFT, transfers to reinforcement learning, specifically GRPO.

Method: Using gradient-magnitude profiling on Qwen 2.5 1.5B with GSM8K to compare uniform and proportional rank allocation in GRPO.

Result: Proportional rank allocation degrades accuracy by 4.5 points compared to uniform allocation (70.0% vs. 74.5%) in GRPO.

Conclusion: Gradient importance does not predict capacity requirements under reinforcement learning, and naive transfer of SFT-era rank allocation to alignment training should be avoided.
...
TL;DR: The authors address the 'Proxy Presumption' in using NLP embeddings to measure social concepts, proposing the Construct Validity Protocol (CVP) and Counterfactual Neutralization to improve validity.


<details>
  <summary>Details</summary>
Motivation: The rapid evolution of NLP in Computational Social Science faces a validity challenge due to the 'Proxy Presumption,' where geometric properties of embeddings are used as direct measures of social concepts without explicit validation.

Method: The authors introduce the Construct Validity Protocol (CVP), a pipeline that includes conceptualization, causal representation learning, and psychometrics, along with Counterfactual Neutralization, a method using LLMs to reduce confounding in embedding space.

Result: The proposed CVP and Counterfactual Neutralization offer a rigorous approach to transforming heuristic proxies into robust, scientifically defensible instruments, providing a standardized Validity Suite for testing validity.

Conclusion: The authors provide a toolkit for the NLP community to improve the validity of using embeddings to measure social concepts, bridging the gap between semantic embeddings and valid social measures.
...
TL;DR: The paper proposes a method to generate large volumes of labelled utterances for training legal chatbots using local grammar graphs, and tests it by implementing a Korean legal chatbot called LIGA.


<details>
  <summary>Details</summary>
Motivation: The high cost of legal representation and advice excludes disadvantaged and vulnerable people, and capturing user input diversity in chatbot datasets is a technical challenge.

Method: The approach uses local grammar graphs (LGG) to jointly generate large volumes of utterances and high-quality labels, and trains a DIET classifier on the generated dataset.

Result: The trained model reached 91% f1-score performance, and the chatbot LIGA was implemented to provide information on similar legal cases.

Conclusion: The proposed approach enables the generation of large volumes of labelled utterances for training legal chatbots, improving access to justice.
...
TL;DR: Construction of a Korean evaluation-annotated corpus (EVAD) for Aspect-Based Sentiment Analysis (ABSA) in e-commerce reviews, achieving high performance with KoBERT and KcBERT models.


<details>
  <summary>Details</summary>
Motivation: To improve the accuracy of user opinion analysis and feature extraction in e-commerce reviews by extending the ABSA approach.

Method: Semi-Automatic Symbolic Propagation (SSP) for annotation, Finite-State Transducer (FST) for linguistic resource formalization, and extension of ABSA to include aspect values.

Result: KoBERT and KcBERT models trained on EVAD achieve F1 scores of 0.88 and 0.90, respectively, in recognizing aspect-value pairs.

Conclusion: The construction of EVAD and the extended ABSA approach enable more accurate analysis of user opinions and feature extraction in e-commerce reviews.
...
TL;DR: The study investigates the challenge of translating ancient and endangered languages using NLP, focusing on hieroglyphic-to-German translation. It reveals data contamination issues that inflate translation scores and establishes corrected baselines for a realistic assessment of NMT capability.


<details>
  <summary>Details</summary>
Motivation: The need to understand what current NMT can realistically achieve for ancient and endangered languages, which have inherently scarce and difficult to expand datasets.

Method: The study reproduces a recent study on hieroglyphic-to-German translation using fine-tuned M2M-100 and investigates the gap in BLEU scores. It also performs document-level decontamination and target-level deduplication.

Result: The study finds that data contamination inflates translation scores, with contaminated samples achieving up to 83.8 BLEU versus 30.9-39.2 BLEU on clean samples. It establishes corrected baselines of 30.9-39.2 BLEU for this endangered writing system.

Conclusion: The study highlights the importance of data quality in NLP for ancient and endangered languages and provides a realistic assessment of NMT capability for hieroglyphic-to-German translation.
...
TL;DR: The paper proposes GRaSp, a framework for automatic in-context example optimization for large language models, which improves performance on domain-specific tasks with scarce high-quality examples.


<details>
  <summary>Details</summary>
Motivation: The performance of large language models in in-context learning is highly sensitive to the selected examples, and finding effective demonstrations is particularly challenging in domain-specific, low-data settings.

Method: The GRaSp framework consists of three stages: generating a large synthetic candidate pool, structuring it with clustering and dimensionality reduction, and using genetic algorithms to find the optimal in-context examples.

Result: GRaSp achieves 45.84% micro-F1 on the financial named entity recognition task (FiNER-139) with non-synthetic data, outperforming zero-shot and random few-shot baselines.

Conclusion: The proposed GRaSp framework consistently improves the performance of large language models on domain-specific tasks, highlighting the importance of distributional variety in the candidate pool for generalization.
...
TL;DR: A novel paradigm called Think-with-Rubrics is introduced, integrating rubric generation into the reasoning context of LLMs for instruction following tasks, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing frameworks treat rubrics as external evaluators, limiting their ability to guide the model's generation process. The authors aim to integrate rubrics into the reasoning context to improve model performance.

Method: Think-with-Rubrics involves LLMs generating a rubric followed by a response during training, with a trained rubric verifier providing joint supervision by evaluating the consistency between the answer and the self-generated / golden rubrics.

Result: Think-with-Rubrics consistently outperforms the Rubric-as-Reward baseline by an average of 3.87 points across multiple benchmarks. The supervision from golden rubrics and self-generated rubrics enhances performance by improving the quality of self-generated rubrics and increasing the internal consistency of responses respectively.

Conclusion: The proposed Think-with-Rubrics paradigm effectively integrates rubrics into the reasoning context of LLMs, leading to improved performance in instruction following tasks.
...
TL;DR: The Moltbook Files dataset reveals safety concerns on a Reddit-like platform, including exposure of sensitive information and potential impacts on language models.


<details>
  <summary>Details</summary>
Motivation: To study emergent behavior in populations and understand the safety concerns of Moltbook, a platform where OpenClaw agents interact at scale.

Method: Analysis of the Moltbook Files dataset (232k posts, 2.2M comments) to identify and remove PII, and fine-tuning of Qwen2.5-14B-Instruct on Moltbook data.

Result: The platform shows mostly neutral and mildly positive sentiment, with a tendency for self-referential linking. Fine-tuning on Moltbook data reduces truthfulness in language models.

Conclusion: Moltbook seems to be more of a harmless 'slopocalypse', but tail risks remain, highlighting the importance of control baselines in emergent misalignment evaluations.
...
TL;DR: A self-evolving framework called SEIF is proposed to improve the instruction-following ability of large language models (LLMs) without relying on external supervision or strong teacher models.


<details>
  <summary>Details</summary>
Motivation: The capability of instruction following in LLMs is challenging to improve with existing methods that rely on costly external supervision, strong teacher models, or self-play training with static-difficulty instructions.

Method: SEIF, a self-evolving reinforcement learning framework that consists of four roles: Instructor, Filter, Follower, and Judger, which form a closed self-evolution loop to improve the model's instruction-following ability.

Result: SEIF consistently improves instruction-following performance across multiple model scales and architectures, with analyses revealing the sources of improvement and an effective training strategy for self-evolution on open-ended tasks.

Conclusion: SEIF offers a promising approach to enhancing the instruction-following ability of LLMs through self-evolution, with publicly available code and data.
...
TL;DR: A browser-based platform, TCMIIES, is developed to extract structured information from academic literature using commercial LLM APIs, allowing researchers to define custom extraction schemas without programming.


<details>
  <summary>Details</summary>
Motivation: The exponential growth of academic publications creates a need for automated tools to extract structured knowledge from unstructured scientific texts, particularly for researchers in specialized fields who face barriers due to infrastructure and expertise requirements.

Method: TCMIIES employs a novel schema-guided prompting framework with automatic system prompt generation, enabling researchers to define custom extraction schemas through a graphical interface, and features a pure front-end architecture for data privacy.

Result: TCMIIES achieves structured output compliance rates exceeding 94% and information extraction accuracy comparable to domain-expert annotation across multiple extraction scenarios in Traditional Chinese Medicine research.

Conclusion: TCMIIES represents a practical, accessible solution that bridges the gap between advanced LLM capabilities and domain-specific academic information extraction needs, particularly for researchers in specialized fields.
...
TL;DR: A new task and dataset for generating weather forecast reports using multimodal large language models (MLLMs) is proposed, along with a specialized model that outperforms existing MLLMs.


<details>
  <summary>Details</summary>
Motivation: The current weather forecast reporting process is inefficient due to manual analysis of multi-source data, and there is a need for data-driven models to improve the process.

Method: The authors propose the Weather Forecasting Report (WFR) task and construct a dataset, and develop a specialized model, ModelNameL, for generating weather forecast reports.

Result: The ModelNameL~ model consistently outperforms leading closed-source MLLMs across multiple metrics, particularly on structurally complex weather aspects, and demonstrates strong transferability across different regions.

Conclusion: The proposed model offers valuable insights for developing MLLMs specialized in weather report generation, and has the potential to improve the efficiency of weather forecast reporting.
...
TL;DR: The paper analyzes failure modes of Large Language Models (LLMs) in machine translation, finding that non-English-centric language pairs have lower translation quality and introducing a metric called Token Activation Rate (TAR) to understand the causes.


<details>
  <summary>Details</summary>
Motivation: The motivation is to systematically analyze failure modes of LLMs in machine translation, specifically when and why LLM-based translation fails, as prior work mainly focuses on improving or benchmarking translation quality.

Method: The method involves evaluating 15 LLMs, including four reasoning LLMs, across 22 language pairs with varying resource levels, and introducing a new metric called Token Activation Rate (TAR) to capture how effectively a model utilizes language-specific tokens.

Result: The results show that non-English-centric language pairs consistently yield lower COMET scores, and lower TAR is strongly associated with poorer translation performance. Additionally, reasoning LLMs tend to generate more tokens when translating into low-TAR languages.

Conclusion: The conclusion emphasizes the importance of token-level dynamics in understanding machine translation performance of LLMs, highlighting the need to consider token-level factors in LLM-based translation.
...
TL;DR: The paper presents an ensemble system for detecting psychological defence mechanisms in supportive conversations, achieving an F1 score of 0.420 on a test set.


<details>
  <summary>Details</summary>
Motivation: Detecting psychological defence mechanisms is ambiguous due to overlapping defence boundaries, and the task requires error independence rather than a single strong model.

Method: A 9-voter ensemble system with three orthogonal axes: class granularity, training method (generative and discriminative), and base model.

Result: The system achieved an F1 score of 0.420 on the hidden test set, placing first among 21 registered teams.

Conclusion: The ensemble approach leveraging error independence outperformed single models, highlighting the importance of combining multiple models for this task.
...
TL;DR: The paper presents a conversational news recommendation system that addresses implicit user intents and grounding suggestions in a rapidly evolving article corpus.


<details>
  <summary>Details</summary>
Motivation: Conversational news recommendation faces challenges in handling implicit user intents and grounding suggestions in a large article corpus.

Method: The authors introduce intent-driven Semantic ID (SID) generation under a Generate-then-Match paradigm, with two-stage training and Profile-Aware Dual-Signal Reasoning (PADR) for cold-start users.

Result: The 7B model achieves 0% hallucination and 12.4% L1 match in the 152K open-generation SID space, surpassing GPT-4+Hybrid RAG on finer-grained metrics at a lower cost.

Conclusion: The proposed system effectively addresses implicit user intents and grounding suggestions, achieving state-of-the-art performance in conversational news recommendation.
...
TL;DR: The paper investigates how and when gender information emerges in a Dutch BERT model trained from scratch, finding that the model struggles to update its internal gender representation in light of explicit contextual cues and defaults to male interpretations.


<details>
  <summary>Details</summary>
Motivation: The primary motivation is to study gender bias in large language models, specifically for languages with grammatical or morphological gender like Dutch, which has been comparatively understudied.

Method: The method involves training a Dutch BERT model from scratch and analyzing checkpoint-level bias formation using linear SVMs to trace when gender becomes linearly encoded in the model's contextual embeddings.

Result: The results show that although gender becomes linearly separable in the model's embeddings, it struggles to update its internal gender representation based on explicit contextual cues, leading to persistent male-default behavior in predictions.

Conclusion: The conclusion is that contextualization in the representations learned by the Dutch BERT model is not sufficiently dynamic along the probed gender direction, indicating a limitation in the model's ability to handle gender information accurately.
...
TL;DR: The paper introduces PhoneSafety, a benchmark to evaluate the safety of phone-use agents, highlighting that a harmless outcome doesn't necessarily imply safety, but could also result from inaction.


<details>
  <summary>Details</summary>
Motivation: Current evaluations of phone-use agents often fail to distinguish between safety and inability to act, merging cases under task success, refusal, or final harmful outcome, which have different causes and require different fixes.

Method: The authors propose PhoneSafety, a benchmark of 700 safety-critical moments from real phone interactions across 130+ apps, evaluating eight representative phone-use agents based on their ability to take safe actions, unsafe actions, or fail to act usefully.

Result: The study finds that stronger general phone-use ability doesn't reliably imply safer choices, and failures to act are more common in visually and operationally demanding settings, behaving like a capability signal rather than a safety signal.

Conclusion: The paper concludes that evaluating phone-use agents requires separating unsafe judgment from inability to act, as a harmless outcome is not enough to count as evidence of safety.
...
TL;DR: The study introduces Psych-201, a dataset to measure how well large language models (LLMs) mimic human behavior. It finds that making LLMs more useful reduces their alignment with human behavior, and a popular technique for human-like behavior doesn't improve individual predictions.


<details>
  <summary>Details</summary>
Motivation: The motivation is to understand which large language models best capture human behavior and why, as they are increasingly used as substitutes for human participants.

Method: The study uses a novel dataset called Psych-201 to measure behavioral alignment at scale, analyzing the impact of post-training and persona-induction on LLMs' alignment with human behavior.

Result: The results show that post-training reduces alignment with human behavior across model families, sizes, and objectives, and this misalignment increases in newer model generations. Additionally, persona-induction does not improve predictions at the individual level.

Conclusion: The study concludes that the processes used to make LLMs useful assistants also make them less accurate models of human behavior, suggesting a trade-off between utility and human-like behavior in LLMs.
...
TL;DR: The study evaluates latest-generation Large Language Models (LLMs) for Grammatical Error Correction (GEC), finding fine-tuned GPT-4o achieves state-of-the-art performance and that reference-based metrics often underestimate GEC system performance.


<details>
  <summary>Details</summary>
Motivation: The study aims to address three critical gaps in the domain of Automated Grammatical Error Correction: evaluating LLMs on grammar correction tasks, exploring the effect of combining LLMs on correction quality, and quantifying the underestimation of GEC system performance by reference-based metrics.

Method: The study evaluates latest-generation LLMs on edit precision, fluency preservation, and meaning retention. It also performs a grammatical error type analysis to compare the error correction patterns of individual LLMs.

Result: Fine-tuned GPT-4o achieves state-of-the-art performance across all three dimensions. Individual LLMs exhibit highly similar error correction patterns (ρ=0.947). Reference-based metrics underestimate GEC performance, with 73.76% of GPT-4o corrections different from gold standards being equally valid or even superior.

Conclusion: The study provides guidance for educators in selecting GEC assistants that can enhance student linguistic development. The data, code, and models are made publicly available.
...
TL;DR: MAVEN, a new framework, improves LLM interpretability and reasoning quality through a multi-agent verification-elaboration network with in-step epistemic auditing.


<details>
  <summary>Details</summary>
Motivation: Existing reasoning paradigms lack modularity and intermediate verification, causing early errors to cascade unchecked and compromising trust in high-stakes applications.

Method: MAVEN uses a blackboard-inspired framework with an adversarial Skeptic-Researcher-Judge loop to simulate expert deliberation and generate explicitly structured, modular, and verifiable deliberation trajectories.

Result: MAVEN outperforms latent reasoning models and consensus-based baselines across four fine-grained metrics on multiple benchmarks, and is model-agnostic, yielding substantial performance improvements across diverse backbone models.

Conclusion: MAVEN offers a reliable and trustworthy approach to LLM-based reasoning by providing explicit reasoning trajectories, granular auditing, and improved epistemic trust.
...
TL;DR: The study compares the performance of large language models (LLMs) and a fine-tuned BERT-based encoder in automated short answer scoring (ASAS), focusing on their ability to score partially correct responses. The results show that while all models perform well on fully correct and incorrect responses, they struggle with mid-range responses, and that task-specific adaptation is crucial for fair evaluation.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to investigate the impact of using large language models (LLMs) in few-shot settings for automated short answer scoring (ASAS), particularly on scoring partially correct responses that require nuanced interpretation.

Method: The study uses three LLMs (GPT-5.2, GPT-4o, Claude Opus 4.5) in few-shot mode, a fine-tuned BERT-based encoder, and a human expert to score several hundred student responses on two open-ended biology items. The ground truth scores were provided by a biology education expert.

Result: The results show that human-human agreement is highest and stable across the full quality spectrum. All AI models perform well on fully correct and fully incorrect responses but exhibit substantial degradation on mid-range responses. This degradation decreases as task-specific data increases, with fine-tuned encoder models performing best.

Conclusion: The study concludes that the use of LLMs in few-shot settings for ASAS may lead to inequitable evaluation of responses produced by students with developing understanding, highlighting the importance of quality-conditioned fairness, particularly for mid-range responses.
...
TL;DR: This paper studies the heterogeneity of token-level learning signals in reinforcement-learning-based post-training for large language models, and finds that attention entropy can reveal optimization-relevant structure in these signals.


<details>
  <summary>Details</summary>
Motivation: The motivation of this paper is to understand the token-level learning signals in reinforcement-learning-based post-training for large language models, which remains poorly understood.

Method: The method used in this paper is to analyze the heterogeneity of token-level learning signals through attention entropy, and to conduct evidence-gathering analyses, entropy dynamics, gradient-geometry diagnostics, and controls.

Result: The results of this paper show that token-level RL objectives are sparsely estimable, and that attention entropy can reveal optimization-relevant structure in these signals. The authors also identify two types of tokens, 'anchors' and 'explorers', which have different properties and behaviors.

Conclusion: The conclusion of this paper is that attention entropy reveals optimization-relevant structure in token-level RL signals, and that uniform token averaging can obscure meaningful heterogeneity in reasoning post-training. The authors also propose a dynamic entropy-aware soft-reweighting intervention that improves the performance of a large language model.
...
TL;DR: The paper introduces DRIP-R, a benchmark to evaluate LLM-based agents in handling ambiguous real-world retail policies, revealing significant disagreements among models.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks assume clear policies, but real-world policies are often ambiguous, leading to a critical evaluation gap.

Method: The authors created DRIP-R, a benchmark with ambiguous retail policy scenarios, conversational simulation, and a multi-judge evaluation framework.

Result: Experiments show that frontier models disagree on identical policy-ambiguous scenarios, highlighting the challenge of ambiguity in LLM decision-making.

Conclusion: The study emphasizes the need for better evaluation of LLM-based agents in handling ambiguous policies, which is crucial for their deployment in real-world domains.
...
TL;DR: The paper proposes a method to dynamically adjust the guidance scale in diffusion-based generative models using reinforcement learning, achieving a better balance between controllability and generation quality.


<details>
  <summary>Details</summary>
Motivation: The guidance scale in Classifier-Free Guidance (CFG) is typically treated as a fixed hyperparameter, which yields a suboptimal controllability and quality tradeoff, especially in NLP domain.

Method: The authors recast CFG scale selection as a sequential decision-making problem and propose to learn dynamic guidance trajectories via reinforcement learning, specifically using Proximal Policy Optimization (PPO).

Result: Experiments on three controlled NLP generation tasks demonstrate that adaptive guidance consistently achieves a better balance between controllability and generation quality than fixed-scale strategies.

Conclusion: The paper concludes that treating guidance as a dynamic control process rather than a static design choice is important, as evidenced by the distinct and interpretable guidance trajectories across tasks learned by the proposed method.
...
TL;DR: The paper proposes SimCT, a method for on-policy distillation that improves teacher-student learning when using different tokenizers.


<details>
  <summary>Details</summary>
Motivation: The assumption that teacher and student predictions are comparable token by token fails when using heterogeneous tokenizers, leading to a loss of teacher signal.

Method: The authors propose SimCT, which compares teacher and student over short multi-token continuations that both tokenizers can realize, rather than relying on exact shared-token matching.

Result: SimCT shows consistent gains over shared-vocabulary OPD and representative cross-tokenizer baselines on mathematical reasoning and code-generation benchmarks.

Conclusion: The proposed method, SimCT, effectively recovers supervision discarded by exact shared-token matching, leading to improved on-policy distillation.
...
TL;DR: The paper proposes MELT, a novel transformer architecture that enables efficient multi-step reasoning while reducing memory consumption, outperforming standard LLMs without sacrificing performance.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the limitation of recurrent LLM architectures in terms of memory scalability, which grows linearly with reasoning depth, making it impractical for large-scale applications.

Method: The authors propose the Memory-Efficient Looped Transformer (MELT) architecture, which decouples reasoning depth from memory consumption by maintaining a single Key-Value (KV) cache per layer shared across reasoning loops, updated via a learnable gating mechanism. They also introduce a two-phase training procedure: chunk-wise training with interpolated transition and attention-aligned distillation.

Result: The MELT models fine-tuned from pretrained Ouro parameters outperform standard LLMs of comparable size while maintaining a significantly smaller memory footprint, comparable to those models and much smaller than Ouro's.

Conclusion: The proposed MELT architecture achieves constant-memory iterative reasoning without sacrificing performance, using a lightweight post-training procedure, making it a practical solution for large-scale applications.
...
TL;DR: A new framework called SOD is proposed to improve the performance of small language models in tool-integrated reasoning tasks by adaptively adjusting the distillation strength.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenges of scaling tool-integrated reasoning to small language models, which include instability in long-horizon tool interactions and limited model capacity.

Method: The proposed method is SOD, a step-wise on-policy distillation framework that adaptively reweights distillation strength at each step based on step-level divergence.

Result: Experiments on challenging math, science, and code benchmarks show that SOD achieves up to 20.86% improvement over the second-best baseline, with a 0.6B student achieving 26.13% on AIME 2025.

Conclusion: The proposed SOD framework effectively improves the performance of small language models in tool-integrated reasoning tasks by adaptively adjusting the distillation strength, achieving state-of-the-art results on several benchmarks.
...
TL;DR: The EngGPT2MoE-16B-A3B model, a 16B parameter Mixture of Experts (MoE) model, is benchmarked against various open-source MoE and dense models, showing competitive performance on international benchmarks and outperforming some Italian models.


<details>
  <summary>Details</summary>
Motivation: To evaluate the performance of the EngGPT2MoE-16B-A3B model, a native Italian Large Language Model, compared to other models of similar size and capabilities.

Method: Benchmarking the EngGPT2MoE-16B-A3B model against various representative benchmarks, including ARC-Challenge, GSM8K, AIME24, AIME25, MMLU, HumanEval (HE), RULER, ITALIC, and BFCL, and comparing it to popular Italian and international models.

Result: The EngGPT2MoE-16B-A3B model performs as well or better than popular Italian models on international benchmarks and achieves the best performance on the longest context setting of the RULER benchmark. It outperforms some MoE models of comparable size but has lower values than some dense models on certain benchmarks.

Conclusion: The EngGPT2MoE-16B-A3B model represents a step forward for native Italian Large Language Models, showing competitive performance on international benchmarks and outperforming some Italian models, although it still lags behind some of the most performant international models.
...
TL;DR: The authors propose TextLDM, a diffusion model for text generation that adapts the visual latent diffusion recipe to language modeling with minimal architectural modification.


<details>
  <summary>Details</summary>
Motivation: To unify visual generation and understanding under a single architecture by applying the diffusion transformer (DiT) framework to language modeling.

Method: The authors use a Transformer-based VAE to map discrete tokens to continuous latents, enhanced by Representation Alignment (REPA) with a frozen pretrained language model, and then perform flow matching in this latent space using a standard DiT.

Result: TextLDM substantially outperforms prior diffusion language models and matches GPT-2 under the same settings when trained from scratch on OpenWebText2.

Conclusion: The visual DiT recipe transfers effectively to language, taking a concrete step toward unified diffusion architectures for multimodal generation and understanding.
...
TL;DR: CktFormalizer is a framework that uses a dependently-typed HDL in Lean 4 to improve the generation of hardware descriptions from natural language specifications, ensuring correctness and reducing defects.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the issue of hardware defects in Verilog generated by Large Language Models (LLMs) from natural language specifications, which often pass syntax checks but fail during synthesis or silicon implementation.

Method: The method involves using CktFormalizer, a framework that leverages a dependently-typed HDL embedded in Lean 4. Lean 4 serves as a type checker, correctness firewall, and proof assistant to ensure the correctness and realizability of the generated hardware designs.

Result: The results show that CktFormalizer achieves simulation pass rates competitive with direct Verilog generation while delivering substantially higher backend realizability (95-100%) on various benchmarks (VerilogEval, RTLLM, ResBench). Additionally, it enables up to 35% area reduction and 30% power reduction through PPA optimization.

Conclusion: The conclusion is that CktFormalizer effectively addresses the challenges of generating correct and realizable hardware descriptions from natural language specifications, providing a more reliable and efficient approach to hardware design.
...
TL;DR: A new method called Chain-based Distillation (CBD) is proposed for efficiently initializing variable-sized language models, improving efficiency and downstream performance.


<details>
  <summary>Details</summary>
Motivation: The high cost of deploying large language models in resource-constrained settings and the limitations of conventional knowledge distillation methods.

Method: Chain-based Distillation (CBD), a scalable paradigm that constructs a sparse sequence of intermediate models (anchors) via stepwise distillation, and uses bridge distillation for cross-architecture and cross-vocabulary transfer.

Result: The proposed method substantially improves efficiency and downstream performance, with a 138M-parameter SLM outperforming scratch-trained models on a 10B-token corpus, and demonstrating versatility in heterogeneous settings.

Conclusion: CBD offers a scalable and efficient solution for initializing variable-sized language models, eliminating the need for repeated large teacher inference and improving performance in various settings.
...
TL;DR: The paper presents a sentiment analysis study for Indonesian social media text, comparing a practical baseline using Logistic Regression with a neural baseline using a multilayer perceptron.


<details>
  <summary>Details</summary>
Motivation: The motivation is to develop a compact three-class sentiment analysis model for Indonesian social media text, which can accurately classify text as positive, negative, or neutral.

Method: The proposed practical baseline combines TF-IDF text features, lightweight numeric metadata features, and a balanced multinomial Logistic Regression classifier. A neural baseline using a two-layer multilayer perceptron (MLP) is also implemented for comparison.

Result: The Logistic Regression model achieves 0.8028 accuracy, 0.8003 weighted F1, and 0.7276 macro F1. The neural baseline has higher accuracy but is not suitable for deployment.

Conclusion: The study concludes that careful preprocessing, interpretable feature engineering, and class balancing can be competitive for small Indonesian sentiment datasets, and that the neural baseline is better used as a comparative experiment rather than a default deployment model.
...
TL;DR: The authors introduce PolySQL, a method for evaluating Text-to-SQL models across different SQL dialects, achieving higher fidelity than existing approaches and revealing significant performance drops when evaluating on SQLite versus other dialects.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the evaluation gap in Text-to-SQL benchmarks, which currently predominantly support only SQLite, making it difficult to evaluate models across different SQL dialects.

Method: The authors propose a novel dual-execution method called PolySQL, which compares normalized execution results to eliminate the need for query transpilation.

Result: The study reveals a 10.1% average accuracy drop from SQLite to other dialects, identifies a significant dialect difficulty hierarchy, and finds that performance degradation stems from logical rather than syntactic errors.

Conclusion: The authors conclude that PolySQL achieves higher evaluation fidelity than query transpilation, enabling rigorous dialect-robust evaluation, and release their framework code and leaderboard for future research.
...
TL;DR: The paper proposes a multidimensional approach to evaluating the reliability of Large Language Models (LLMs) by assessing self-assessment across multiple dimensions, finding that competence-related dimensions like effort and ability outperform traditional confidence metrics in predicting model failure.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the issue of unreliable self-assessment in Large Language Models (LLMs) by moving beyond traditional confidence metrics, which have been shown to be inconsistent and overoptimistic predictors of model correctness.

Method: The paper uses a framework from human psychology called cognitive appraisal theory to decompose self-evaluation into multiple components. It elicits six appraisal-based dimensions of self-assessment, alongside confidence, and evaluates their utility for predicting model failure across 12 LLMs and 38 tasks spanning eight domains.

Result: The results show that competence-related appraisal dimensions, particularly effort and ability, consistently match or outperform confidence across most settings. Effort yields less overoptimistic estimates that remain stable across model sizes. The most informative dimension varies systematically with task characteristics: effort is most predictive for reasoning-intensive tasks, while ability and confidence dominate on retrieval-oriented tasks.

Conclusion: The paper concludes that structured multidimensional self-assessment is a promising approach to improving the reliability and safety of language model deployment across diverse real-world settings.
...
TL;DR: Classical machine learning methods, particularly SVM, outperform deep learning models in sentiment classification on IMDb movie reviews dataset.


<details>
  <summary>Details</summary>
Motivation: To compare the performance of classical machine learning and deep learning methods for sentiment classification.

Method: Classical machine learning (Logistic Regression, Naïve Bayes, SVM) with TF-IDF features and PyCaret AutoML, and deep learning (BiLSTM, BiLSTM with attention mechanism).

Result: SVM achieves the best performance with an accuracy of 0.8530, while BiLSTM with Attention reaches an accuracy of 0.706.

Conclusion: Classical machine learning remains a strong baseline when combined with effective feature engineering, particularly under limited data and computational resources.
...
TL;DR: The paper introduces SCENE, a benchmark for evaluating LLM agents' ability to recognize and adapt to implicit social norms in online group chats, and finds that Claude Opus 4.7 and Gemini 3.1 Pro perform significantly better than open-weight models.


<details>
  <summary>Details</summary>
Motivation: The ability of LLM-based agents to recognize and adapt to implicit social norms in online group chats remains largely unexplored, and there is a need for a benchmark to evaluate their social capabilities.

Method: The authors introduce SCENE, a social-interaction benchmark that generates plausible scenarios with scripted personas, and propose behavioral evaluation metrics to assess the adaptation abilities of LLM agents.

Result: The evaluation of six frontier and open-weight models on SCENE shows that Claude Opus 4.7 and Gemini 3.1 Pro adapt to implicit norms significantly more than the evaluated open-weight models.

Conclusion: The paper contributes to the development of dynamic, interactional evaluation of LLM social capabilities, and highlights the importance of considering implicit social norms in the evaluation of LLM agents.
...
TL;DR: This paper evaluates user simulators for AI assistants, finding a large distributional gap between simulated and real user behaviors, and suggests combining simulators to improve representation.


<details>
  <summary>Details</summary>
Motivation: The increasing use of user simulators for training and evaluating AI assistants necessitates that they accurately represent diverse real user behaviors.

Method: The authors introduce a method to measure the distributional gap between real and simulated user behaviors by extracting representations of user behavior from conversations, quantizing them into discrete distributions, and computing divergence metrics.

Result: The study reveals a large distributional gap between real users and 24 LLM-based user simulators on coding and writing tasks, with most simulators behaving similarly, while a few stand apart; combining simulators brings the resulting distribution closer to real users.

Conclusion: The findings highlight the need for improved user simulators that can capture the broad and heterogeneous distribution of real user behaviors, and suggest potential strategies for improvement.
...
TL;DR: A new framework, MatryoshkaLoRA, is proposed for efficient fine-tuning of large deep learning models by learning hierarchical low-rank representations.


<details>
  <summary>Details</summary>
Motivation: The computational cost of fine-tuning large deep learning models remains a significant barrier, and existing methods require exhaustive searches or yield sub-optimal results.

Method: MatryoshkaLoRA inserts a fixed diagonal matrix P between LoRA adapters to scale sub-ranks, allowing for dynamic rank selection and efficient use of gradient information.

Result: MatryoshkaLoRA achieves superior accuracy-performance trade-offs across ranks on evaluated datasets and learns more accurate hierarchical low-rank representations than prior approaches.

Conclusion: MatryoshkaLoRA provides a general framework for efficient fine-tuning, supporting dynamic rank selection with minimal degradation in accuracy, and proposes a new metric, AURAC, for evaluating hierarchical low-rank adapters.
...
TL;DR: LANCE is proposed to improve Large Language Models' (LLMs) safety and flexibility by enhancing refusal mechanisms, making interactions more natural.


<details>
  <summary>Details</summary>
Motivation: Traditional refusal mechanisms in LLMs lead to 'rigid rejection,' causing unnatural interactions.

Method: LANCE uses variational inference for label enhancement, predicting a continuous distribution across multiple rejection categories to refine model responses.

Result: LANCE significantly alleviates the rigid rejection problem, outperforming baseline models in helpfulness and naturalness while maintaining high security standards.

Conclusion: LANCE offers a solution for safer, more flexible, and natural LLM interactions by improving refusal mechanisms.
...
TL;DR: The paper addresses the challenge of evaluating AI reviewers due to unreliable human reviews, proposing a new benchmark and evaluation method.


<details>
  <summary>Details</summary>
Motivation: Current metrics for evaluating AI reviewers favor overlap with human reviews over correctness, but human reviews are often incomplete or contain mistakes.

Method: The authors create category-specific benchmark subsets, leverage expert annotations from reviewer-author-meta-review discussions, and filter unreliable reviews.

Result: AI reviewers are limited in correctness and prone to hallucinations, while reasoning models are more effective reviewers.

Conclusion: The paper introduces CoCoReviewBench, a benchmark for reliable and fine-grained evaluation of AI reviewers, and highlights directions for improvement.
...
TL;DR: The paper investigates the effects of inducing values in conversational large language models, finding that it can lead to related and contrastive values, increased safety, and more anthropomorphic language.


<details>
  <summary>Details</summary>
Motivation: The motivation is to understand the unintended effects of value induction into models, as values are complex and inter-related, and inducing certain values can have detrimental effects on users.

Method: The authors fine-tune models using curated value subsets of existing preference datasets and measure the impact on expression of other values, model safety, anthropomorphic language, and QA benchmarks.

Result: The results show that inducing values leads to expression of related and sometimes contrastive values, increases safety, and increases anthropomorphic language use, making models more validating and sycophantic.

Conclusion: The conclusion is that value induction in conversational large language models can have both positive and negative effects, and careful consideration is needed to ensure that the induced values align with the desired behavior and safety standards.
...
TL;DR: The paper presents the Latent Diffusion Language Model (LDLM), which achieves better text generation performance than existing diffusion models while being significantly faster.


<details>
  <summary>Details</summary>
Motivation: The major challenge in latent diffusion modeling is constructing a suitable latent space, which is addressed in this paper by jointly training the latent encoder, diffusion model, and decoder.

Method: The LDLM is trained using a simple recipe consisting of an MSE decoder loss, diffusion-to-encoder warmup, adaptive timestep sampling, and decoder-input noise.

Result: LDLM achieves better generation performance than existing discrete and continuous diffusion language models on OpenWebText and LM1B, while being 2-13 times faster.

Conclusion: Jointly learning the latent space is a key step toward making latent diffusion competitive for text generation, and the proposed LDLM shows promising results in this area.
...
TL;DR: The paper introduces a framework to study the value of clarification in long-horizon AI agents and finds that the optimal timing of clarification depends on the type of missing information.


<details>
  <summary>Details</summary>
Motivation: Long-horizon AI agents can make irreversible errors due to incorrect assumptions, and incomplete instructions require the agent to decide when to ask for clarification.

Method: The authors introduce a forced-injection framework that provides ground-truth clarifications at controlled points in the agent's trajectory across four information dimensions and three agent benchmarks.

Result: The study finds that the value of clarification depends on the type of missing information, with goal clarification losing value after 10% of execution and input clarification retaining value through roughly 50%.

Conclusion: The paper provides empirical demand curves for clarification and establishes concrete design targets for timing-aware clarification policies, revealing that current frontier models do not ask for clarification within the optimal window.
...
TL;DR: GLiGuard, a compact bidirectional encoder, is proposed for efficient and effective content moderation in large language models, achieving competitive performance with much larger models while reducing inference cost.


<details>
  <summary>Details</summary>
Motivation: The need for real-time content moderation that can scale across multiple safety dimensions in large language models, and the inefficiency of current state-of-the-art guardrail models due to their large size and autoregressive decoder architecture.

Method: Introducing GLiGuard, a 0.3B-parameter schema-conditioned bidirectional encoder adapted from GLiNER2, which encodes task definitions and label semantics into the input sequence as structured token schemas, enabling simultaneous evaluation of multiple safety aspects in a single non-autoregressive forward pass.

Result: GLiGuard achieves F1 scores competitive with 7B--27B decoder-based guards while being 23--90$	imes$ smaller, and delivers up to 16$	imes$ higher throughput and 17$	imes$ lower latency across nine established safety benchmarks.

Conclusion: The proposed GLiGuard model demonstrates that compact bidirectional encoders can approach the accuracy of much larger guard models while drastically reducing inference cost, making it a promising solution for efficient and effective content moderation in large language models.
...
TL;DR: The study investigates how tool-calling agents select the wrong tool and finds that the chosen tool's identity is linearly readable and steerable within the model. It proposes a method to switch tool selection by adding the mean-difference between two tools' average internal activations.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to understand and address the issue of tool-calling agents picking the wrong tool, which can lead to invisible failures until execution.

Method: The method involves probing 12 instruction-tuned models across different sizes (270M to 27B) and using techniques such as adding the mean-difference between two tools' average internal activations, activation patching, and cosine readout from the internal state.

Result: The results show that adding the mean-difference between two tools' average internal activations can switch which tool the model selects at 77-100% accuracy. The study also finds that the causal effect concentrates along one direction and that even base models encode the right tool before they can emit it.

Conclusion: The study concludes that the identity of the chosen tool is linearly readable and steerable inside the model, and that it is possible to flag likely errors before they happen. The findings have implications for improving the reliability of tool-calling agents.
...
TL;DR: The paper introduces new techniques for speeding up byte-level language models, specifically the Byte Latent Transformer (BLT), through BLT Diffusion (BLT-D), BLT Self-speculation (BLT-S), and BLT Diffusion+Verification (BLT-DV), which reduce generation time while maintaining or improving quality.


<details>
  <summary>Details</summary>
Motivation: The utility of recent byte-level language models is limited by slow, byte-by-byte autoregressive generation, which the authors aim to address.

Method: The authors propose three methods: BLT Diffusion (BLT-D) trained with an auxiliary block-wise diffusion objective, BLT Self-speculation (BLT-S) which drafts bytes and then verifies them, and BLT Diffusion+Verification (BLT-DV) which adds an autoregressive verification step after diffusion-based generation.

Result: The proposed methods can achieve an estimated memory-bandwidth cost over 50% lower than BLT on generation tasks, and offer unique advantages in terms of speed and generation quality.

Conclusion: The introduced techniques remove key barriers to the practical use of byte-level language models by significantly speeding up generation while maintaining or improving quality.
...
### [248] [Accurate and Efficient Statistical Testing for Word Semantic Breadth](https://arxiv.org/abs/2605.08048)
*Yo Ehara*

Main category: cs.CL

TL;DR: The paper proposes a method to accurately compare the breadth of meaning of two words by isolating dispersion differences from directional differences in their contextualized token embeddings.


<details>
  <summary>Details</summary>
Motivation: Current methods for comparing word meaning breadth can be misleading due to differences in semantic direction masquerading as dispersion differences, leading to inflated Type-I error rates.

Method: The authors propose a Householder-aligned permutation test, which applies a Householder reflection to align the mean directions of the two word types and then performs a permutation test on the aligned token clouds.

Result: The proposed method reduced Type-I error by 32.5% while preserving sensitivity to genuine breadth differences, and achieved a 23x speedup over the CPU baseline.

Conclusion: The proposed Householder-aligned permutation test provides a more accurate and efficient way to compare the breadth of word meanings, which can be useful for constructing thesauri and domain-specific dictionaries.
...
### [249] [CA-SQL: Complexity-Aware Inference Time Reasoning for Text-to-SQL via Exploration and Compute Budget Allocation](https://arxiv.org/abs/2605.08057)
*James Petullo,Nianwen Xue*

Main category: cs.CL

TL;DR: The paper introduces CA-SQL, a novel Text-to-SQL pipeline that improves LLM reasoning on challenging tasks in the Bird-Bench benchmark by dynamically scaling exploration and using custom prompt seeding and voting methods.


<details>
  <summary>Details</summary>
Motivation: Current Text-to-SQL solutions struggle with challenging tasks in the Bird-Bench benchmark due to inadequate solution space exploration.

Method: CA-SQL pipeline, which utilizes estimated task difficulty to scale exploration breadth, custom prompt seeding based on evolutionary search principles, and a novel voting method to select the best candidate solution.

Result: State-of-the-art score of 51.72% on the 'challenging' tier of BIRD development set problems, competitive 61.06% execution accuracy, and 68.77% Soft F1 score on the BIRD development dataset.

Conclusion: The proposed CA-SQL pipeline achieves state-of-the-art performance on challenging Text-to-SQL tasks in the Bird-Bench benchmark, outperforming other in-context learning approaches.
...
### [250] [Conformal Path Reasoning: Trustworthy Knowledge Graph Question Answering via Path-Level Calibration](https://arxiv.org/abs/2605.08077)
*Shuhang Lin,Chuhao Zhou,Xiao Lin,Zihan Dong,Kuan Lu,Zhencan Peng,Jie Yin,Dimitris N. Metaxas*

Main category: cs.CL

TL;DR: The authors propose Conformal Path Reasoning (CPR), a trustworthy Knowledge Graph Question Answering (KGQA) framework that provides reliable coverage guarantees over retrieved answers.


<details>
  <summary>Details</summary>
Motivation: Existing KGQA approaches often fail to provide reliable coverage guarantees over retrieved answers, and Conformal Prediction (CP) methods have limitations in calibration validity and score discriminability.

Method: The authors propose two key innovations: query-level conformal calibration over path-level scores, and the Residual Conformal Value Network (RCVNet) to learn discriminative path-level nonconformity scores.

Result: Experiments on benchmarks show that CPR significantly improves the Empirical Coverage Rate by 34% while reducing average prediction set size by 40% compared to conformal baselines.

Conclusion: The proposed CPR framework provides reliable coverage guarantees over retrieved answers with substantially more compact answer sets, validating its efficacy in trustworthy KGQA.
...
### [251] [LLMs Improving LLMs: Agentic Discovery for Test-Time Scaling](https://arxiv.org/abs/2605.08083)
...
TL;DR: The authors propose AutoTTS, an environment-driven framework for automatically discovering test-time scaling (TTS) strategies to improve large language model performance.


<details>
  <summary>Details</summary>
Motivation: Existing TTS strategies are hand-crafted and leave much of the computation-allocation space unexplored.

Method: The authors propose a framework that formulates width-depth TTS as controller synthesis over pre-collected reasoning trajectories and probe signals.

Result: The discovered strategies improve the overall accuracy-cost tradeoff over strong manually designed baselines and generalize to held-out benchmarks and model scales.

Conclusion: The proposed AutoTTS framework can efficiently discover effective TTS strategies, reducing the discovery cost to $39.9 and 160 minutes.
...
### [252] [The Memory Curse: How Expanded Recall Erodes Cooperative Intent in LLM Agents](https://arxiv.org/abs/2605.08060)
...
TL;DR: Expanding the context window of LLMs can degrade cooperation in multi-agent social dilemmas, a phenomenon termed the 'memory curse'.


<details>
  <summary>Details</summary>
Motivation: The authors investigate the effect of context window expansion on LLMs in multi-agent social dilemmas, finding that it can systematically fail to improve performance.

Method: The authors analyze 7 LLMs and 4 games over 500 rounds, using lexical analysis, targeted fine-tuning, memory sanitization, and ablating explicit Chain-of-Thought reasoning to isolate the underlying mechanism.

Result: The authors find that expanding accessible history degrades cooperation in 18 of 28 model-game settings, and that the 'memory curse' is associated with eroding forward-looking intent.

Conclusion: The authors conclude that longer recall can either destabilize or support cooperation depending on the reasoning patterns it elicits, recasting memory as an active determinant of multi-agent behavior.
...
### [253] [GraphDC: A Divide-and-Conquer Multi-Agent System for Scalable Graph Algorithm Reasoning](https://arxiv.org/abs/2605.06671)
*Wenjin Li,Jiaming Cui*

Main category: cs.AI

TL;DR: GraphDC, a Divide-and-Conquer multi-agent framework, improves graph algorithm reasoning in Large Language Models by decomposing graphs into subgraphs for more efficient and robust processing.


<details>
  <summary>Details</summary>
Motivation: The performance of Large Language Models on graph algorithmic tasks is unsatisfying due to the complexity and topology of graphs, especially on larger graphs.

Method: GraphDC uses a Divide-and-Conquer approach, dividing input graphs into smaller subgraphs processed by specialized agents, with a master agent integrating outputs for the final solution.

Result: GraphDC consistently outperforms existing methods on graph algorithm reasoning across diverse tasks and scales, especially on larger instances.

Conclusion: GraphDC offers a scalable and robust solution for graph algorithm reasoning, addressing the limitations of Large Language Models on complex graph tasks.
...
### [254] [More Thinking, More Bias: Length-Driven Position Bias in Reasoning Models](https://arxiv.org/abs/2605.06672)
*Xiao Wang*

Main category: cs.AI

TL;DR: The study investigates the effect of chain-of-thought (CoT) reasoning on position bias in multiple-choice QA. Contrary to common assumptions, it finds that within reasoning-capable models, position bias increases with the length of the reasoning trajectory.


<details>
  <summary>Details</summary>
Motivation: The study aims to test the assumption that CoT reasoning and reasoning-tuned models reduce shallow heuristic biases, specifically position bias in multiple-choice QA.

Method: The study uses thirteen reasoning-mode configurations, including two R1-distilled 7-8B models, two base models prompted with CoT, and DeepSeek-R1 at 671B, and evaluates them on MMLU, ARC-Challenge, and GPQA. It also employs a truncation intervention to provide causal evidence.

Result: The study finds a positive partial correlation between trajectory length and Position Bias Score (PBS) in twelve out of thirteen configurations. It also shows that accuracy gates the expression of length-driven bias, and that CoT reasoning replaces direct-answer position bias with length-accumulated bias.

Conclusion: The study concludes that reasoning-capable models should not be treated as order-robust by default in MCQ evaluation pipelines, and offers a diagnostic toolkit for auditing position bias in reasoning models.
...
### [255] [Fast and Effective Redistricting Optimization via Composite-Move Tabu Search](https://arxiv.org/abs/2605.06682)
*Hai Jin,Diansheng Guo*

Main category: cs.AI

TL;DR: The paper proposes a composite-move Tabu search (CM-Tabu) algorithm to solve the spatial redistricting problem, which ensures contiguity while improving solution quality, robustness, and efficiency.


<details>
  <summary>Details</summary>
Motivation: The spatial redistricting problem requires high-quality solutions, rapid turnaround, and flexibility to accommodate multi-criteria objectives and interactive refinement, but traditional methods struggle with contiguity constraints.

Method: The authors introduce CM-Tabu, which uses a contiguity graph to generate single-unit and composite moves in linear time, preserving contiguity while expanding the feasible neighborhood space.

Result: Extensive experiments show that CM-Tabu substantially improves solution quality, run-to-run robustness, and computational efficiency compared to traditional Tabu search and other baselines.

Conclusion: CM-Tabu delivers optimization performance suitable for real-world practices and decision-support workflows, consistently attaining the theoretical global optimum in population-equality and supporting multi-criteria trade-offs.
...
### [256] [State Representation and Termination for Recursive Reasoning Systems](https://arxiv.org/abs/2605.06690)
*Debashis Guha,Amritendu Mukherjee,Sanjay Kukreja,Tarun Kumar*

Main category: cs.AI

TL;DR: The paper presents a framework for recursive reasoning systems, focusing on representing the reasoning state and determining when to stop iterating.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address two implicit design choices in recursive reasoning systems: representing the evolving reasoning state and determining when to stop iterating.

Method: The authors represent the reasoning state as an epistemic state graph and define the order-gap to determine when to stop iterating.

Result: The authors provide a necessary and sufficient condition for the linearized order-gap to be non-degenerate near the fixed point, which is a local condition.

Conclusion: The paper provides a framework for recursive reasoning systems, which can be applied to various applications such as agent loops, tree-of-thought reasoning, theorem proving, and continual learning.
...
### [257] [Hidden Coalitions in Multi-Agent AI: A Spectral Diagnostic from Internal Representations](https://arxiv.org/abs/2605.06696)
*Cameron Berg,Susan L. Schneider,Mark M. Bailey*

Main category: cs.AI

TL;DR: A method for detecting coalition structures in multi-agent systems using internal neural representations.


<details>
  <summary>Details</summary>
Motivation: To address AI safety and alignment by identifying emergent group-level organization in collections of interacting AI agents.

Method: Constructing a pairwise mutual-information graph from hidden states of agents and applying spectral partitioning.

Result: The method successfully recovers coalition structures in multi-agent reinforcement learning environments and a large language model.

Conclusion: Analyzing hidden-state mutual information through spectral partitioning provides a scalable diagnostic for identifying representational coalitions.
...
### [258] [CASCADE: Case-Based Continual Adaptation for Large Language Models During Deployment](https://arxiv.org/abs/2605.06702)
*Siyuan Guo,Yali Du,Hechang Chen,Yi Chang,Jun Wang*

Main category: cs.AI

TL;DR: The paper introduces CASCADE, a framework for deployment-time learning in large language models, enabling them to improve from experience during deployment without modifying model parameters.


<details>
  <summary>Details</summary>
Motivation: The rigid separation between training and deployment in large language models limits their ability to adapt to new experiences, unlike natural intelligence which continually adapts.

Method: The authors present CASCADE, a framework that equips LLM agents with an explicit, evolving episodic memory, formulating experience reuse as a contextual bandit problem.

Result: CASCADE improves macro-averaged success rate by 20.9% over zero-shot prompting across 16 diverse tasks, outperforming gradient-based and memory-based baselines.

Conclusion: The work establishes a foundation for continually improving AI systems by reframing deployment as an adaptive learning process.
...
### [259] [From Storage to Experience: A Survey on the Evolution of LLM Agent Memory Mechanisms](https://arxiv.org/abs/2605.06716)
*Jinghao Luo,Yuchen Tian,Chuxue Cao,Ziyang Luo,Hongzhan Lin,Kaixin Li,Chuyi Kong,Ruichao Yang,Jing Ma*

Main category: cs.AI

TL;DR: This paper proposes a novel evolutionary framework for Large Language Model (LLM) agent memory mechanisms, formalizing their development into three stages: Storage, Reflection, and Experience.


<details>
  <summary>Details</summary>
Motivation: The current research on LLM agent memory mechanisms is fragmented, oscillating between operating system engineering and cognitive science, preventing a unified view of technological synthesis and a coherent evolutionary perspective.

Method: The authors propose a three-stage evolutionary framework for LLM agent memory mechanisms: Storage (trajectory preservation), Reflection (trajectory refinement), and Experience (trajectory abstraction). They analyze the three core drivers of this evolution and explore two transformative mechanisms in the Experience stage.

Result: The paper offers robust design principles and a clear roadmap for the development of next-generation LLM agents by synthesizing disparate views on LLM agent memory mechanisms.

Conclusion: The proposed framework provides a unified view of LLM agent memory mechanisms, enabling a coherent evolutionary perspective and guiding the development of next-generation LLM agents.
...
### [260] [When Does a Language Model Commit? A Finite-Answer Theory of Pre-Verbalization Commitment](https://arxiv.org/abs/2605.06723)
*Long Zhang,Wei-neng Chen,Feng-feng Wei,Zi-bo Qin*

Main category: cs.AI

TL;DR: This paper studies when a language model's answer preference becomes stable during reasoning, proposing a method to track this stabilization through finite-answer preference projection.


<details>
  <summary>Details</summary>
Motivation: Understanding when a language model's answer preference stabilizes during reasoning, as the final answer does not reveal the model's internal decision-making process.

Method: The authors propose projecting a model's continuation probabilities onto a finite answer set, defining parser-based answer onset, retrospective stabilization time, and lead.

Result: The contextual finite-answer projection stabilizes before the answer is parseable, with a 17-31 token mean lead in controlled tasks, and the signal tracks the model's eventual output.

Conclusion: The study provides insights into language model decision-making, showing that answer preference stabilization can be tracked and that it occurs before the answer is parseable.
...
### [261] [Weblica: Scalable and Reproducible Training Environments for Visual Web Agents](https://arxiv.org/abs/2605.06761)
*Oğuzhan Fatih Kar,Roman Bachmann,Yuanzheng Gong,Anders Boesen Lindbo Larsen,Afshin Dehghan*

Main category: cs.AI

TL;DR: The authors propose Weblica, a framework for creating scalable and reproducible web environments for training visual web agents, and demonstrate its effectiveness in improving RL training and model performance.


<details>
  <summary>Details</summary>
Motivation: The complexity and constant change of the web make it difficult to scale training data for visual web agents, and existing data collection methods are limited.

Method: The authors propose a framework called Weblica, which uses HTTP-level caching and LLM-based environment synthesis to create reproducible and scalable web environments.

Result: The best model, Weblica-8B, outperforms open-weight baselines across multiple web navigation benchmarks, uses fewer inference steps, and scales well with additional compute.

Conclusion: The proposed framework, Weblica, enables the creation of diverse and scalable web environments, leading to improved performance and efficiency in visual web agents.
...
### [262] [When Does Critique Improve AI-Assisted Theoretical Physics? SCALAR: Structured Critic--Actor Loop for Agentic Reasoning](https://arxiv.org/abs/2605.06772)
*Vasilis Niarchos,Constantinos Papageorgakis,Alexander G. Stapleton,Sokratis Trifinopoulos*

Main category: cs.AI

TL;DR: The paper studies the interaction between researchers and AI agents in physics reasoning tasks using SCALAR, an Actor-Critic-Judge pipeline, and finds that multi-turn dialogue and critic feedback strategy significantly impact results.


<details>
  <summary>Details</summary>
Motivation: The increasing promise of large language models (LLMs) in research-level physics reasoning tasks and the growing use of agentic AI raise questions about the effect of researcher-agent interaction on results.

Method: The study uses SCALAR, an Actor-Critic-Judge pipeline, applied to quantum field theory and string theory problems, varying the Actor persona, Critic feedback strategy, and Actor model family and scale.

Result: The results show that multi-turn dialogue improves over single-shot attempts, and the mechanism of improvement and the value of different prompting choices depend on the Actor-Critic pairing. Increasing model scale improves some easier-problem behavior but not the hardest bottleneck. Critic feedback strategy matters most in asymmetric Actor-Critic settings.

Conclusion: The study provides a controlled testbed for evaluating which interaction structures help or hinder AI-driven scientific discovery, and highlights the importance of considering the interaction between researchers and agents in physics reasoning tasks.
...
### [263] [Towards Security-Auditable LLM Agents: A Unified Graph Representation](https://arxiv.org/abs/2605.06812)
...
TL;DR: The paper proposes Agent-BOM, a unified structural representation for agent security auditing, to bridge the semantic gap between low-level physical events and high-level execution intent in LLM-based agentic systems.


<details>
  <summary>Details</summary>
Motivation: The rapid evolution of LLM-based agentic systems creates a severe semantic gap between low-level physical events and high-level execution intent, making post-hoc security auditing difficult. Existing representation mechanisms fail to capture cognitive-state evolution, capability bindings, and cascading risk propagation.

Method: The paper proposes Agent-BOM, which models an agentic system as a hierarchical attributed directed graph, separating static capability bases from dynamic runtime semantic states. A graph-query-based paradigm is developed for path-level risk assessment.

Result: The evaluation on representative real-world agentic attack scenarios shows that Agent-BOM can reconstruct stealthy attack chains, including cross-session memory poisoning and tool misuse, capability supply-chain hijacking and unexpected code execution, multi-agent ecosystem hijacking, and privilege and trust abuse.

Conclusion: Agent-BOM provides a unified and auditable foundation for root-cause analysis and security adjudication in complex agentic ecosystems.
...
### [264] [Uneven Evolution of Cognition Across Generations of Generative AI Models](https://arxiv.org/abs/2605.06815)
*Isaac Galatzer-Levy,Daniel McDuff,Xin Liu,Jed McGiffin*

Main category: cs.AI

TL;DR: The paper introduces a psychometric framework to evaluate the cognitive capabilities of generative AI models, comparing them to human norms. It finds that these models exhibit uneven cognitive performance, with significant disparities in verbal comprehension, working memory, perceptual reasoning, and visual-perceptual organization.


<details>
  <summary>Details</summary>
Motivation: The pursuit of artificial general intelligence (AGI) requires robust methods for evaluating the cognitive capabilities of models beyond narrow task performance.

Method: The authors introduce a psychometric framework, adapting tasks from the Wechsler Adult Intelligence Scale, and develop the Artificial Intelligence Quotient (AIQ) Benchmark to track the evolution of cognitive profiles across generations of generative AI models.

Result: The study reveals that leading multimodal models exhibit near-ceiling performance in verbal comprehension and working memory (>98th percentile) but near-floor performance in perceptual reasoning (<1st percentile). The AIQ Benchmark applied to six generations and two model families shows significant but asymmetric performance gains, with a sharp dissociation between modalities.

Conclusion: The cognitive abilities of generative models are evolving unevenly, suggesting that scaling and optimization approaches to AGI development alone may be insufficient to overcome fundamental architectural limitations in achieving balanced, human-like general intelligence.
...
### [265] [Randomness is sometimes necessary for coordination](https://arxiv.org/abs/2605.06825)
*Rohan Patil,Jai Malegaonkar,Henrik I. Christensen*

Main category: cs.AI

TL;DR: The authors propose Diamond Attention, a method for cooperative multi-agent reinforcement learning that uses randomness to break symmetry and enable role differentiation among identical agents.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitation of full parameter sharing in cooperative MARL, which fails to enable role differentiation among identical agents under permutation-symmetric observations.

Method: The authors propose Diamond Attention, a cross-attention architecture that uses randomness to induce a transient rank ordering among agents, allowing for role differentiation and zero-shot deployment to teams of different sizes.

Result: The method achieves state-of-the-art results in various scenarios, including the perfectly symmetric XOR game, control coordination tasks, and SMACLite cross-scenario transfer. Specifically, it achieves 1.0 success on the XOR game, generalizes zero-shot to teams of different sizes, and achieves zero-shot transfer where standard baselines cannot.

Conclusion: The authors conclude that Diamond Attention is an effective method for cooperative MARL, enabling role differentiation and zero-shot deployment to teams of different sizes, and that the structured randomness used in the method is crucial for its success.
...
### [266] [Extracting Search Trees from LLM Reasoning Traces Reveals Myopic Planning](https://arxiv.org/abs/2605.06840)
*Sixing Chen,Ji-An Li,Saner Cakir,Sinan Akcali,Kayla Lee,Marcelo G. Mattar*

Main category: cs.AI

TL;DR: This paper analyzes the planning abilities of large language models (LLMs) in the context of the four-in-a-row board game, finding that LLMs' planning is driven by shallow rather than deep search, unlike human planning.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to understand how large language models (LLMs), especially reasoning models, generate extended chain-of-thought (CoT) reasoning and whether this deliberation constitutes genuine planning.

Method: The authors introduce a new method to characterize LLM planning by extracting and quantifying search trees from reasoning traces in the four-in-a-row board game and fitting computational models on the extracted search trees.

Result: The study finds that LLMs' search is shallower than humans', and that performance is predicted by search breadth rather than depth. LLMs' move choices are best explained by a myopic model that ignores deep nodes.

Conclusion: The findings reveal a key difference between LLM and human planning: while human expertise is driven by deeper search, LLMs do not act on deep lookahead, offering targeted guidance for aligning LLM and human planning.
...
### [267] [AGWM: Affordance-Grounded World Models for Environments with Compositional Prerequisites](https://arxiv.org/abs/2605.06841)
*Qinshi Zhang,Weipeng Deng,Zhihan Jiang,Jiaming Qu,Qianren Li,Weitao Xu,Ray LC*

Main category: cs.AI

TL;DR: The paper proposes AGWM, a world model that incorporates affordance structure to improve the accuracy and interpretability of multi-step predictions in interactive environments.


<details>
  <summary>Details</summary>
Motivation: Conventional world models fail to capture the dynamic executability of actions in interactive environments, leading to compounding prediction errors in multi-step predictions.

Method: The authors propose AGWM, which learns an abstract affordance structure represented as a DAG of prerequisite dependencies to explicitly track the dynamic executability of actions.

Result: Experiments on game-based simulated environments demonstrate that AGWM achieves lower multi-step prediction error, better generalization to novel configurations, and improved interpretability.

Conclusion: The proposed AGWM method effectively addresses the limitations of conventional world models in interactive environments, providing a more accurate and interpretable approach to multi-step predictions.
...
### [268] [Agentick: A Unified Benchmark for General Sequential Decision-Making Agents](https://arxiv.org/abs/2605.06869)
*Roger Creus Castanyer,Pablo Samuel Castro,Glen Berseth*

Main category: cs.AI

TL;DR: Agentick is a benchmark for evaluating AI agents, enabling fair comparison across different approaches. It provides 37 tasks, a coding API, and a leaderboard, revealing that no single approach dominates and highlighting room for improvement.


<details>
  <summary>Details</summary>
Motivation: The lack of a unified benchmark for comparing AI agents across different approaches, hindering progress toward general autonomous agents.

Method: Agentick provides a benchmark with 37 procedurally generated tasks, four difficulty levels, and five observation modalities, along with a coding API, oracle reference policies, and a composable agent harness.

Result: The evaluation reveals that no single approach dominates, with GPT-5 mini leading overall, PPO dominating planning and multi-agent tasks, and ASCII observations outperforming natural language.

Conclusion: Agentick provides the empirical infrastructure needed to drive progress toward general autonomous agents, enabling evaluation and training of AI agents in sequential environments.
...
### [269] [How Well Do LLMs Perform on the Simplest Long-Chain Reasoning Tasks: An Empirical Study on the Equivalence Class Problem](https://arxiv.org/abs/2605.06882)
*Chun Zheng,Lianlong Wu,Bingqian Li,Lvting Liu,Yi Zhou*

Main category: cs.AI

TL;DR: The paper evaluates Large Language Models' (LLMs) performance on long-chain reasoning tasks, specifically the Equivalence Class Problem (ECP), and finds that reasoning models perform better but still struggle, while non-reasoning models fail.


<details>
  <summary>Details</summary>
Motivation: To assess the capabilities of LLMs in long-chain reasoning tasks, particularly in the context of the Equivalence Class Problem.

Method: Experimental evaluation of LLMs on ECP with varying problem instances, including different numbers of variables, connectivity probabilities, and prompts.

Result: Non-reasoning LLMs fail ECP, while reasoning models perform better but still struggle; the hardest problem instances differ for non-reasoning and reasoning models.

Conclusion: The study highlights the limitations of LLMs in long-chain reasoning tasks and suggests that reasoning models are more effective, but still require improvement.
...
### [270] [Beyond the Black Box: Interpretability of Agentic AI Tool Use](https://arxiv.org/abs/2605.06890)
*Hariom Tatsat,Ariye Shater*

Main category: cs.AI

TL;DR: The paper introduces a mechanistic-interpretability toolkit for monitoring AI agents' internal states and tool decisions, aiming to improve their reliability and safety in high-stakes workflows.


<details>
  <summary>Details</summary>
Motivation: The dependable deployment of AI agents in high-stakes enterprise workflows is limited due to difficulties in diagnosing and controlling tool-use failures, which can have costly consequences in long-horizon settings.

Method: The authors propose a framework built on Sparse Autoencoders (SAEs) and linear probes, which reads model states before each action, infers tool needs, and identifies internal layers and features associated with tool decisions.

Result: The proposed toolkit provides visibility into the model's internal signals before action, helping to surface deeper causes of agent failure, particularly in long-horizon runs.

Conclusion: The paper demonstrates the potential of mechanistic interpretability to support practical internal observability for monitoring tool calls and risk in agent systems, complementing existing external evaluation methods.
...
### [271] [Mitigating Cognitive Bias in RLHF by Altering Rationality](https://arxiv.org/abs/2605.06895)
*Tiffany Horter,Andrew Markham,Niki Trigoni,Serena Booth*

Main category: cs.AI

TL;DR: The paper proposes a method to make models robust to imperfect human feedback by dynamically adjusting the rationality parameter beta during reward learning.


<details>
  <summary>Details</summary>
Motivation: Human feedback is not simplistic and is shaped by cognitive biases, leading to systematic deviations from reward-consistent behavior.

Method: The approach uses an LLM-as-judge to assess the likely presence of cognitive biases and dynamically adjust the rationality parameter beta during reward learning.

Result: The approach learns a more rational downstream model, even when finetuning on datasets with strongly biased preferences.

Conclusion: The proposed approach effectively handles imperfect human feedback by accounting for cognitive biases and context-dependent rationality.
...
### [272] [Self-Programmed Execution for Language-Model Agents](https://arxiv.org/abs/2605.06898)
*Luke J. O'Connor*

Main category: cs.AI

TL;DR: The paper introduces Self-Programmed Execution (SPE), an agent architecture where the language model itself acts as the orchestrator program, allowing for flexible state transitions without a fixed policy.


<details>
  <summary>Details</summary>
Motivation: Existing language model agents rely on a fixed orchestrator program for state transitions between turns, which can be limiting. The authors aim to create a more flexible architecture.

Method: The authors introduce SPE and a Lisp-based language called Spell, which allows programs to edit and re-evaluate themselves without replaying side effects.

Result: Experiments with existing models show that they can operate in the SPE regime and accomplish challenging agentic tasks without any fixed orchestration policy.

Conclusion: The paper demonstrates that a language model can act as an agent without a fixed orchestration policy, and raises questions about potential self-orchestration strategies that could be learned by a model trained for SPE.
...
### [273] [Multi-Objective Constraint Inference using Inverse reinforcement learning](https://arxiv.org/abs/2605.06951)
*Syed Ihtesham Hussain Shah,Floris den Hengst,Aneta Lisowska,Annette ten Teije*

Main category: cs.AI

TL;DR: A novel framework called Multi-Objective Constraint Inference (MOCI) is proposed to extract shared constraints and individual preferences from expert demonstrations with different objectives.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for constraint inference assume homogeneous demonstrations and have limited ability to capture individual preferences, and often suffer from computational inefficiencies.

Method: MOCI, a framework that jointly extracts shared constraints and individual preferences from heterogeneous expert trajectories.

Result: MOCI significantly outperforms existing baselines, achieving improved predictive performance and maintaining competitive computational efficiency on a standard grid-world benchmark.

Conclusion: MOCI is an accurate, flexible, and computationally practical approach for real-world constraint inference and preference learning tasks.
...
### [274] [Learning and Reusing Policy Decompositions for Hierarchical Generalized Planning with LLM Agents](https://arxiv.org/abs/2605.06957)
*Shirin Sohrabi,Haritha Ananthakrishnan,Harsha Kokel,Kavitha Srinivas,Michael Katz*

Main category: cs.AI

TL;DR: The paper presents a dynamic policy-learning approach that combines generalized planning and hierarchical task decomposition for LLM-based agents, achieving high accuracy on task instances and improving efficiency.


<details>
  <summary>Details</summary>
Motivation: The motivation is to integrate classical planning concepts with LLM agents to improve accuracy and efficiency by learning reusable components and generalizing across task instances.

Method: The method is Hierarchical Component Learning for Generalized Policies (HCL-GP), which learns parameterized policies, extracts reusable components, and organizes them into a component library for compositional policy generation.

Result: The approach achieves 98.2% accuracy on normal tasks and 97.8% on challenge tasks with unseen applications, improving 15.8 points over static synthesis on challenging scenarios, and enabling 62.5% success with dynamic reuse for open-source models.

Conclusion: The paper concludes that classical planning concepts can be effectively integrated with LLM agents for improved accuracy and efficiency, demonstrating the effectiveness of the proposed approach.
...
### [275] [Optimal Experiments for Partial Causal Effect Identification](https://arxiv.org/abs/2605.06993)
*Tobias Maringgele,Jalal Etesami*

Main category: cs.AI

TL;DR: The paper studies the problem of selecting a cost-constrained subset of experiments to maximally tighten bounds on a target causal query from observational data.


<details>
  <summary>Details</summary>
Motivation: Causal queries are often only partially identifiable from observational data, and experiments that could tighten the resulting bounds are typically costly.

Method: The authors formalize the problem as the max-potency problem and propose a general procedure for evaluating epistemic potency in discrete settings, along with two graphical pruning criteria.

Result: The proposed approach can prune 50-88% of candidate experiments on average without solving a single polynomial program, and leads to a super-exponential reduction in the number of subsets evaluated.

Conclusion: The paper provides a method for selecting optimal experiments for estimating causal queries, and demonstrates its application on observational NHANES data.
...
### [276] [Adaptive auditing of AI systems with anytime-valid guarantees](https://arxiv.org/abs/2605.07002)
*Siyu Zhou,Patrick Vossler,Venkatesh Sivaraman,Yifan Mai,Jean Feng*

Main category: cs.AI

TL;DR: The paper proposes a hypothesis testing framework for characterizing failure modes in generative AI systems using adaptive testing paradigms, leveraging Safe Anytime-Valid Inference (SAVI) to draw statistically rigorous conclusions.


<details>
  <summary>Details</summary>
Motivation: The cost and time of annotation and evaluation are major bottlenecks in characterizing failure modes of generative AI systems, making adaptive testing paradigms necessary but challenging for statistical inference.

Method: The authors introduce a hypothesis testing framework from two 'dueling' perspectives, leveraging SAVI and 'testing by betting' to formalize the auditor's sampling strategy and test dueling null hypotheses.

Result: The proposed testing procedures maintain anytime-valid type-I error control, outperform pre-specified testing methods, and can reach statistically rigorous conclusions with as few as 20 observations.

Conclusion: The paper provides a statistically rigorous framework for characterizing failure modes in generative AI systems using adaptive testing paradigms, which can certify AI systems as being globally robust.
...
### [277] [Behavior Cue Reasoning: Monitorable Reasoning Improves Efficiency and Safety through Oversight](https://arxiv.org/abs/2605.07021)
*Christopher Z. Cui,Taylor W. Killian,Prithviraj Ammanabrolu*

Main category: cs.AI

TL;DR: The paper introduces Behavior Cue Reasoning to make Large Language Models (LLMs) more controllable and monitorable, improving reasoning oversight without compromising performance.


<details>
  <summary>Details</summary>
Motivation: The challenge of overseeing LLMs' reasoning due to misaligned behaviors surfacing only after reasoning concludes.

Method: Introducing Behavior Cues, special token sequences emitted before specific behaviors, and fine-tuning an external monitor with Reinforcement Learning.

Result: The approach allows for pruning up to 50% of unnecessary reasoning tokens and recovering safe actions from 80% of traces that would otherwise propose unsafe actions, doubling the success rate from 46% to 96%.

Conclusion: Behavior Cue Reasoning improves LLM monitorability and controllability without compromising performance, advancing scalable oversight.
...
### [278] [The Context Gathering Decision Process: A POMDP Framework for Agentic Search](https://arxiv.org/abs/2605.07042)
*Chinmaya Kausik,Adith Swaminathan,Nathan Kallus*

Main category: cs.AI

TL;DR: The paper addresses the challenge of large language models (LLMs) operating in complex environments with limited context windows. It introduces the Context Gathering Decision Process (CGDP) to improve LLM agents' ability to iteratively explore and gather relevant information.


<details>
  <summary>Details</summary>
Motivation: LLM agents face challenges in complex environments where relevant state exceeds their context windows, leading to degraded working memory and inefficient search processes.

Method: The paper formalizes the challenge as CGDP, a Partially Observable Markov Decision Process. It models LLM behavior as approximate Thompson Sampling within CGDP and introduces a predicate-based method to decompose implicit search into explicit operations.

Result: The proposed interventions improve multi-hop reasoning by up to 11.4% and save up to 39% of tokens without degrading agent performance.

Conclusion: Framing the LLM agent loop as a CGDP guides the design of modular improvements to agentic search harnesses, leading to more efficient and effective LLM agents.
...
### [279] [2.5-D Decomposition for LLM-Based Spatial Construction](https://arxiv.org/abs/2605.07066)
*Paul Whitten,Li-Jen Chen,Sharath Baddam*

Main category: cs.AI

TL;DR: The authors present a neuro-symbolic pipeline for autonomous systems to build structures from natural-language instructions, leveraging 2.5-D decomposition to reduce errors in 3D block placements.


<details>
  <summary>Details</summary>
Motivation: Large language models (LLMs) make systematic coordinate errors when generating 3D block placements, highlighting the need for reliable spatial reasoning in autonomous systems.

Method: The proposed pipeline uses 2.5-D decomposition, where the LLM plans in 2D and a deterministic executor computes vertical placements, reducing errors.

Result: The pipeline achieves 94.6% mean structural accuracy on the Build What I Mean benchmark, outperforming GPT-4o (90.3%) and the best competing system (76.3%).

Conclusion: The 2.5-D decomposition pipeline significantly improves accuracy in autonomous construction tasks and generalizes to edge hardware and other tasks, such as IGLU collaborative building.
...
### [280] [TeamBench: Evaluating Agent Coordination under Enforced Role Separation](https://arxiv.org/abs/2605.07073)
...
TL;DR: The paper introduces TeamBench, a benchmark for evaluating agent coordination under role separation, and finds that role separation affects agent behavior and team performance.


<details>
  <summary>Details</summary>
Motivation: The authors aim to investigate the impact of role separation on agent coordination in multi-agent systems, as current systems often rely on prompts rather than enforced access controls.

Method: The authors present TeamBench, a benchmark with 851 task templates and 931 seeded instances, which enforces role separation across Planner, Executor, and Verifier roles. They evaluate agent coordination under different conditions, including prompt-only and sandbox-enforced teams.

Result: The results show that prompt-only and sandbox-enforced teams have similar pass rates, but prompt-only runs produce more cases where the verifier attempts to edit the executor's code. The verifier approves 49% of submissions that fail the deterministic grader, and removing the verifier improves mean partial score. The study also finds that teams benefit from role separation when single agents struggle, but hurt when single agents already perform well.

Conclusion: The paper concludes that role separation affects agent behavior and team performance, and that TeamBench can expose interaction patterns that pass rate misses. The authors highlight the importance of evaluating agent coordination under enforced role separation.
...
### [281] [Online Allocation with Unknown Shared Supply](https://arxiv.org/abs/2605.07080)
*Tzeh Yuan Neoh,Davin Choo,Mengchu Yue,Milind Tambe*

Main category: cs.AI

TL;DR: The paper introduces the Online Shared Supply Allocation (OSSA) problem and proposes a deterministic threshold-proportional policy GPA, which achieves a 4/3-approximation to the offline optimum.


<details>
  <summary>Details</summary>
Motivation: Real-world resource allocation systems, such as humanitarian logistics and vaccine distribution, face challenges in prepositioning limited supply across multiple locations before demand is realized.

Method: The authors propose a deterministic threshold-proportional policy GPA and a learning-augmented extension to GPA that incorporates imperfect forecasts.

Result: GPA achieves a 4/3-approximation to the offline optimum up to an additive term independent of the total supply, and the learning-augmented extension shows improved performance in synthetic and real-world experiments.

Conclusion: The proposed policy GPA and its learning-augmented extension provide an effective solution for the OSSA problem, outperforming natural baselines in scenarios with scarce global supply.
...
### [282] [ARMOR: An Agentic Framework for Reaction Feasibility Prediction via Adaptive Utility-aware Multi-tool Reasoning](https://arxiv.org/abs/2605.07103)
*Ye Liu,Botao Yu,Xinyi Ling,Daniel Adu-Ampratwum,Xia Ning*

Main category: cs.AI

TL;DR: The paper proposes ARMOR, a framework that leverages multiple tools to improve reaction feasibility prediction in computational chemistry by adaptively prioritizing tools and resolving conflicts.


<details>
  <summary>Details</summary>
Motivation: The performance of individual tools for reaction feasibility prediction varies substantially, making it difficult for any single tool to consistently perform well across all cases.

Method: The authors propose ARMOR, an agentic framework that models tool-specific utilities, prioritizes tools, and resolves tool conflicts through a hierarchical organization and memory-augmented reasoning.

Result: ARMOR consistently outperforms strong baselines, including single-tool methods and various tool aggregation and selection approaches, particularly on reactions with conflicting tool predictions.

Conclusion: The proposed framework ARMOR effectively leverages the complementary strengths of multiple tools to obtain more accurate feasibility predictions in reaction feasibility prediction.
...
### [283] [Switchcraft: AI Model Router for Agentic Tool Calling](https://arxiv.org/abs/2605.07112)
*Sharad Agarwal,Pooria Namyar,Alec Wolman,Rahul Ambavat,Ankur Gupta,Qizheng Zhang*

Main category: cs.AI

TL;DR: The paper introduces Switchcraft, a model router optimized for agentic tool calling, which reduces inference cost by 84% while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: The high cost of agentic AI systems that invoke external tools leads developers to use large models, overspending on inference budgets. Model routing can help mitigate this issue.

Method: The authors construct an evaluation framework on five function-calling benchmarks and train a DistilBERT-based classifier under a latency budget.

Result: Switchcraft achieves 82.9% accuracy, matching or exceeding the best individual model, while reducing inference cost by 84% and saving over $3,600 per million queries.

Conclusion: The work enables cost-aware agentic AI deployment without sacrificing correctness, and shows that larger models do not always outperform smaller ones on tool-use tasks.
...
### [284] [AdaTKG: Adaptive Memory for Temporal Knowledge Graph Reasoning](https://arxiv.org/abs/2605.07121)
*Seunghan Lee,Jun Seo,Jaehoon Lee,Sungdong Yoo,Minjae Kim,Tae Yoon Lim,Dongwan Kang,Hwanil Choi,SoonYoung Lee,Wonbin Ahn*

Main category: cs.AI

TL;DR: The paper proposes AdaTKG, a method for temporal knowledge graphs that models entities as adaptive processes, refining their representations through interactions.


<details>
  <summary>Details</summary>
Motivation: Existing methods for temporal knowledge graphs produce static entity representations, lacking the dynamic evolution of entities over time.

Method: AdaTKG maintains a per-entity memory updated with each interaction, using a learnable exponential moving average governed by a single shared scalar.

Result: Extensive experiments show consistent gains over TKG baselines, demonstrating the effectiveness of adaptive memory.

Conclusion: The proposed AdaTKG method effectively models entities as adaptive processes, leading to improved performance in temporal knowledge graph reasoning tasks.
...
### [285] [Can You Break RLVER? Probing Adversarial Robustness of RL-Trained Empathetic Agents](https://arxiv.org/abs/2605.07138)
*Deeraj S K,Sadhana Devarajan,Krishna Mehra,Sudhakar Mishra*

Main category: cs.AI

TL;DR: The paper evaluates the empathetic performance of language models trained with reinforcement learning from verifiable emotion rewards (RLVER) under adversarial conditions. It introduces the Adversarial Empathy Benchmark (AEB) and the Emotional Consistency Score (ECS) to assess empathetic robustness. The results show that RLVER models outperform baselines in emotional responsiveness but do not significantly improve in observable state tracking.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to evaluate the empathetic performance of language models in real-world emotional interactions, which often involve adversarial dynamics such as gaslighting, escalation, and pressure for unconditional validation.

Method: The paper constructs the Adversarial Empathy Benchmark (AEB) with six psychologically grounded adversarial trajectory types and introduces the Emotional Consistency Score (ECS) to evaluate empathetic robustness. It conducts a controlled experiment across eight scenario-matched conditions, involving two RLVER models, two base models, and 480 adversarial dialogues.

Result: The results show that the RLVER-PPO-Think model substantially outperforms the same-scale untuned baseline (0.963 vs. 0.761, p < 0.001, r = 0.688), with zero dialogue collapses and 47% higher hidden-intention detection. However, the ECS remains nearly flat and is not significantly different for RLVER-PPO-Think versus Base-7B-Think (p = 0.650).

Conclusion: The paper concludes that RLVER training improves emotional responsiveness without measurable gains in observable state tracking, suggesting a behavioral/legibility dissociation inside this simulator family, rather than evidence about internal understanding or clinical readiness.
...
### [286] [SREGym: A Live Benchmark for AI SRE Agents with High-Fidelity Failure Scenarios](https://arxiv.org/abs/2605.07161)
*Jackson Clark,Yiming Su,Saad Mohammad Rafid Pial,Yifang Tian,Lily Gniedziejko,Hans-Arno Jacobsen,Yinfang Chen,Tianyin Xu*

Main category: cs.AI

TL;DR: A new benchmark called SREGym is introduced for evaluating AI agents in Site Reliability Engineering (SRE) tasks, offering a high-fidelity environment that simulates real-world cloud-native system failures.


<details>
  <summary>Details</summary>
Motivation: Current SRE benchmarks are limited and hard to extend due to oversimplistic tasks and bespoke designs, necessitating a more comprehensive and flexible benchmark.

Method: SREGym provides a live system environment built on real-world cloud-native stacks, simulating various faults, ambient noises, and failure modes through fault injectors, and is designed as a modular, extensible framework.

Result: SREGym includes 90 realistic SRE problems and evaluation of frontier agents shows significant variation in their capabilities to address different kinds of failures, with up to 40% differences in results.

Conclusion: SREGym is a valuable tool for assessing AI agents in SRE tasks, offering a high-fidelity benchmark that can help improve the capabilities of these agents in handling complex failures in production environments.
...
TL;DR: The paper introduces Deceptive Meta Planning (DeMP) to solve Repeated Deceptive Path Planning (RDPP) against learning observers, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing deceptive path planning methods fail against adaptive, learning observers, highlighting a need for more advanced approaches.

Method: DeMP, a two-level optimization framework combining episode-level adaptation and meta-level updates to counter learning observers.

Result: DeMP significantly outperforms existing approaches in RDPP, maintaining competitive path cost across various environments.

Conclusion: DeMP enables sustained deception against learning observers by mitigating adaptation lag, offering new insights into deception and privacy in multi-agent systems.
...
TL;DR: The paper proposes a Three-in-One world-model architecture that integrates a Deep Boltzmann Machine (DBM) with task-specific adapters for marketing decision-making, enabling unified modeling of consumer heterogeneity, internal states, and interventions.


<details>
  <summary>Details</summary>
Motivation: Current prediction- and language-oriented models do not capture the interaction of latent consumer heterogeneity, time-varying internal states, and explicit interventions in a unified manner.

Method: The proposed architecture uses a DBM to learn a frozen belief representation from demographics, time, and lagged actions and outcomes, with lightweight task-specific adapters attached on top.

Result: The adapters match a strong MLP baseline on visit- and purchase-AUC, and recover heterogeneous treatment effects substantially better than baseline methods, especially on confounded price-promotion interventions.

Conclusion: The proposed architecture provides an integrated world-model substrate for marketing intervention, enabling disentangled latent traits that survive counterfactual queries.
...
TL;DR: The paper proposes AIDA, an autonomous framework for transforming enterprise data into insights, outperforming traditional methods in complex business environments.


<details>
  <summary>Details</summary>
Motivation: The challenge of transforming fragmented enterprise data into actionable insights due to complex database schemas, limitations in dynamic SQL generation, and the need for deep multi-dimensional analysis.

Method: AIDA, an end-to-end framework using a proprietary Domain-Specific Language (DSL) and reinforcement learning guided by the Pareto Principle.

Result: AIDA significantly outperforms workflow-based agents, achieving superior environmental perception and more in-depth analysis from diverse perspectives.

Conclusion: The paper establishes the transformative potential of autonomous intelligence for industrial-scale business intelligence systems.
...
TL;DR: A new framework called HMACE is proposed for solving NP-hard combinatorial optimization problems using Large Language Models. HMACE uses a multi-agent approach to improve the efficiency and effectiveness of heuristic search.


<details>
  <summary>Details</summary>
Motivation: Existing LLM-based methods for solving NP-hard combinatorial optimization problems have limitations, such as rigid templates and premature convergence to local optima. The authors aim to design an autonomous and collaborative architecture to overcome these limitations.

Method: The authors propose HMACE, a Heterogeneous Multi-Agent Collaborative Evolution framework. HMACE decomposes each evolutionary generation into a loop with four coordinated agents: Proposer, Generator, Evaluator, and Reflector. The framework uses behavior-aware retrieval, lightweight candidate filtering, and fitness-grounded archive updates to guide the search.

Result: Extensive evaluations on various COPs (TSP, Online BPP, MKP, and PFSP) show that HMACE achieves a favorable quality-efficiency trade-off compared to state-of-the-art baselines. HMACE achieves the lowest average gaps on TSP and Online BPP (0.464% and 0.223%, respectively) with substantially fewer tokens required.

Conclusion: HMACE offers a promising approach for solving NP-hard combinatorial optimization problems using Large Language Models. The framework's autonomous and collaborative architecture allows for efficient and effective heuristic search, outperforming existing methods.
...
TL;DR: The paper introduces MemoRepair, a method to address the cascade update problem in agentic memory, ensuring that derived artifacts remain valid and consistent when source artifacts change.


<details>
  <summary>Details</summary>
Motivation: The authors aim to solve the cascade update problem, where changes to source artifacts can lead to stale derived artifacts, causing incorrect future actions.

Method: The authors propose MemoRepair, a barrier-first cascade-repair contract that ensures a controlled transition from invalidated to validated states, and solves the induced publication problem using a maximum-weight predecessor closure and a single s-t min-cut.

Result: Experiments show that MemoRepair reduces invalidated-memory exposure from 69.8-94.3% to 0%, recovers 91.1-94.3% of validated successors, and reduces normalized repair-operator cost from 1.00 to 0.57-0.76.

Conclusion: The paper concludes that MemoRepair is an effective solution to the cascade update problem, ensuring the consistency and validity of derived artifacts in agentic memory.
...
TL;DR: The authors propose EnvSimBench, a benchmark for evaluating the ability of large language models (LLMs) to simulate environments for agent training, and present a constraint-driven simulation pipeline to improve environment simulation reliability.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of manually crafted environments for agent training and the assumption that LLMs can accurately simulate environmental feedback, which is often not the case due to hallucinations, inconsistencies, and state drift failures.

Method: The authors define and operationalize Environment Simulation Ability (EnvSim Ability) as a research objective, construct EnvSimBench with 400 samples across 167 diverse environments, and evaluate state-of-the-art language models using this benchmark.

Result: The results show that all evaluated language models suffer from a universal state change cliff, achieving high accuracy when the environment state remains invariant but failing catastrophically when multiple states need simultaneous updates. The proposed constraint-driven simulation pipeline reduces hallucination, increases environment synthesis yield by 6.8%, and cuts costs by over 90%.

Conclusion: The authors conclude that EnvSimBench provides a diagnostic framework and practical optimization path for reliable LLM-based environment simulation, establishing a foundation for scalable agent training.
...
TL;DR: The paper introduces ChemCost, a benchmark for evaluating the ability of Large Language Models (LLMs) to estimate chemical procurement costs. It assesses LLMs' tool use in a practical chemistry task, revealing limitations even with frontier models.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the limited rigorous evaluation of scientific tool use in Large Language Models (LLMs), particularly in chemistry. The authors aim to provide a more robust and judge-free evaluation method for chemical procurement cost estimation.

Method: The authors introduce ChemCost, a benchmark consisting of 1,427 evaluable reactions with a frozen pricing snapshot of 2,261 chemicals and 230,775 supplier quotes. They evaluate LLMs' performance on this benchmark, including their ability to ground chemical identities, retrieve supplier quotes, select valid packs, normalize quantities, and compute costs. The authors also construct controlled noise-injected views to test the models' robustness.

Result: The results show that even frontier LLMs struggle with the task, achieving only 50.6% accuracy within 25% relative error on clean inputs. The models' performance degrades substantially when faced with realistic noise. Stage-level analysis reveals failures in parsing, evidence integration, pack selection, and tool use.

Conclusion: The paper concludes that while tool access is necessary for solving chemical procurement cost estimation tasks, it is insufficient. The authors highlight the need for further improvement in LLMs' ability to integrate evidence, select valid packs, and use tools effectively.
...
TL;DR: The paper proposes Structured Role-aware Policy Optimization (SRPO) to improve multimodal reasoning in large vision-language models by refining token-level credit assignment.


<details>
  <summary>Details</summary>
Motivation: The difficulty in determining whether a correct answer is supported by task-relevant visual evidence due to uniform sequence-level credit assignment in multimodal reinforcement learning.

Method: SRPO refines the sequence-level GRPO advantage into role-aware token-level advantages by assigning role-specific credit using self-distilled on-policy contrasts.

Result: SRPO improves evidence-grounded reasoning across diverse multimodal reasoning benchmarks.

Conclusion: The importance of moving beyond uniform sequence-level credit toward role-aware optimization for reliable multimodal reasoning.
...
TL;DR: The paper studies signal reshaping for GRPO in code-agent RL with weak feedback, proposing a minimal signal-reshaping construction that improves strict compile-and-semantic accuracy.


<details>
  <summary>Details</summary>
Motivation: Code-agent RL often receives weak feedback that only captures necessary or surface conditions for task success, rather than the target semantic predicate.

Method: The authors propose a minimal signal-reshaping construction for GRPO, which includes compile-and-semantic layered rewards, step-level process scores, and failure-cause-aware rollout governance.

Result: Experiments show that full signal-reshaped GRPO improves strict compile-and-semantic accuracy from 0.385 to 0.535, with controlled comparisons demonstrating the effectiveness of the proposed components.

Conclusion: The paper concludes that signal reshaping is essential for GRPO to achieve meaningful within-group comparison, and that the proposed construction leads to a clear end-to-end gain in accuracy.
...
TL;DR: Proposed Structured Opponent Modeling (SOM) framework for large language model (LLM)-based agents to improve opponent behavior prediction in multi-agent environments.


<details>
  <summary>Details</summary>
Motivation: Existing approaches entangle opponent modeling with prediction, limiting adaptability in dynamic interactions.

Method: SOM uses a two-stage framework: (1) constructs an opponent model using a Structural Causal Model (SCM) to capture dependencies between opponents' observations and actions, and (2) performs structured reasoning along clear pathways derived from the SCM for prediction.

Result: SOM consistently outperforms state-of-the-art LLM-based reasoning baselines in diverse multi-agent benchmarks, enabling more accurate and adaptable strategic decision-making.

Conclusion: The proposed SOM framework improves opponent behavior prediction in multi-agent environments, leading to more accurate and adaptable strategic decision-making.
...
TL;DR: The paper presents a new evaluation protocol for agent memory that assesses its ability to maintain accuracy as irrelevant sessions accumulate, providing a more comprehensive understanding of memory performance.


<details>
  <summary>Details</summary>
Motivation: Current evaluation methods for agent memory only report fixed-snapshot accuracy or retrieval quality, which do not capture the impact of accumulating irrelevant sessions on memory usability.

Method: The authors propose a scale-conditioned evaluation protocol that logs agent-memory trajectories and reports four diagnostics: budget-compliant reliability, tail memory-call burden, failure-regime decomposition, and the usable-scale boundary.

Result: The protocol was applied to LongMemEval and LoCoMo across different memory interfaces, revealing that reliability loss is not a single phenomenon and that different agents exhibit varying levels of reliability as irrelevant sessions accumulate.

Conclusion: The study supports a framework for making scalable-memory claims conditional on agent, interface, scale range, and interaction budget, highlighting the importance of considering these factors when evaluating agent memory performance.
...
TL;DR: The paper proposes Implicit Compression Regularization (ICR) to improve the reasoning of Large Language Models (LLMs) by reducing overthinking while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing methods to reduce overthinking in LLMs, such as length penalties or early-exit strategies, have limitations. Length penalties may degrade accuracy and induce underthinking, while early-exit strategies assume that substantial portions of reasoning traces can be safely truncated.

Method: The authors propose ICR, an on-policy regularization method that uses a virtual shorter distribution induced by the shortest correct responses in rollout groups to guide the policy toward concise yet correct trajectories.

Result: Experiments on three reasoning backbones and multiple mathematical and knowledge-intensive benchmarks show that ICR consistently shortens responses while preserving or improving accuracy, achieving a stronger accuracy-length Pareto frontier.

Conclusion: The proposed ICR method effectively reduces overthinking in LLMs while maintaining or improving accuracy, providing a better trade-off between response length and accuracy.
...
TL;DR: A new method called DoLQ is proposed for discovering ordinary differential equations from observational data, combining qualitative and quantitative evaluation using a large language model.


<details>
  <summary>Details</summary>
Motivation: Existing symbolic regression approaches for discovering differential equations rely primarily on quantitative metrics, lacking incorporation of domain knowledge for physical plausibility.

Method: DoLQ uses a multi-agent architecture with a Sampler Agent, Parameter Optimizer, and Scientist Agent, leveraging a large language model for qualitative and quantitative evaluations.

Result: DoLQ achieves superior performance on multi-dimensional ordinary differential equation benchmarks, with higher success rates and more accurate recovery of correct symbolic terms.

Conclusion: DoLQ offers an effective approach for discovering ordinary differential equations that combines domain knowledge with quantitative metrics, enhancing physical plausibility and accuracy.
...
TL;DR: FlowAgent is proposed to improve tool chaining in LLMs by treating it as continuous trajectory generation, enhancing robustness and adaptability in long-horizon reasoning tasks.


<details>
  <summary>Details</summary>
Motivation: Existing tool chaining methods for LLMs rely on a step-wise paradigm, causing error accumulation and limiting generalization to unseen tools.

Method: FlowAgent uses conditional flow matching to generate continuous latent trajectories, providing a global planning perspective for coherent and robust tool execution.

Result: FlowAgent achieves superior robustness and adaptability in long-horizon reasoning tasks compared to existing methods.

Conclusion: FlowAgent offers a more robust and adaptable approach to tool chaining in LLMs by reconceptualizing it as continuous trajectory generation, validated through theoretical and empirical evaluations.
...
TL;DR: CASPO improves reasoning reliability and efficiency in large models by aligning token-level confidence with logical correctness.


<details>
  <summary>Details</summary>
Motivation: Large reasoning models often produce correct answers through flawed intermediate steps, indicating a gap between accuracy and reliability.

Method: CASPO uses iterative Direct Preference Optimization to align confidence with logical correctness, and CaT for dynamic pruning of uncertain branches.

Result: CASPO consistently improves reasoning reliability and inference efficiency across ten benchmarks and multiple model families, scaling to Qwen3-8B-Base.

Conclusion: CASPO offers a scalable solution for enhancing reasoning reliability and efficiency without requiring a separate reward model.
...
TL;DR: The authors propose GraphReAct, a framework that enhances graph learning by interleaving reasoning with actions for dynamic information acquisition.


<details>
  <summary>Details</summary>
Motivation: The authors aim to extend the reasoning-acting paradigm to graph learning, which is underexplored, and address the challenges of effective reasoning over graph-structured data.

Method: The authors propose GraphReAct, a graph reasoning-acting framework that enables step-by-step inference over graph-structured data, with a graph-based action space and three types of actions: topological retrieval, semantic retrieval, and context refinement.

Result: Extensive experiments on six benchmark datasets demonstrate that GraphReAct consistently outperforms state-of-the-art methods.

Conclusion: The authors conclude that GraphReAct is effective in enhancing graph learning through reasoning-acting, and validate its effectiveness through experiments.
...
TL;DR: The authors propose Posterior Sampling-based Policy Optimization (PSPO) to balance generalization and robustness in model-based offline reinforcement learning, achieving better performance than state-of-the-art baselines.


<details>
  <summary>Details</summary>
Motivation: The trade-off between generalization and robustness against exploitation errors in out-of-distribution (OOD) regions in model-based offline reinforcement learning.

Method: Posterior Sampling-based Policy Optimization (PSPO), which formulates dynamics modeling as a Bayesian inference process and integrates posterior sampling and constrained policy optimization.

Result: PSPO achieves superior performance compared to state-of-the-art baselines on standard benchmarks.

Conclusion: PSPO effectively balances generalization and robustness in model-based offline reinforcement learning, providing a more efficient and reliable approach.
...
TL;DR: The paper explores bounded fitting for learning logical formulas in expressive description logics, demonstrating its practicality and effectiveness.


<details>
  <summary>Details</summary>
Motivation: To investigate bounded fitting for learning concepts in expressive description logics that extend ALC with inverse roles, qualified number restrictions, and feature comparisons.

Method: Implementing bounded fitting using a SAT solver and comparing it with state-of-the-art concept learners.

Result: The approach shows encouraging results, demonstrating its practicality and effectiveness in expressive concept learning.

Conclusion: Bounded fitting remains a favorable approach for learning logical formulas in expressive description logics, offering a practical solution for concept learning.
...
### [305] [Efficient Data Selection for Multimodal Models via Incremental Optimization Utility](https://arxiv.org/abs/2605.07488)
*Jinhao Jing,Qiannian Zhao,Chao Huang,Zhan Su*

Main category: cs.AI

TL;DR: The authors propose One-Step-Train (OST), a framework for efficient data selection in Large Multimodal Models (LMMs) that achieves Pareto-optimal efficiency and outperforms existing methods.


<details>
  <summary>Details</summary>
Motivation: The quality-quantity trade-off in synthetic data constrains the scaling of LMMs, and previous approaches like LLM-as-a-Judge are computationally expensive and lack interpretability.

Method: The authors propose OST, which reformulates data selection as an incremental optimization utility ranking problem and estimates the marginal utility of each sample via a simulated single-step update on a lightweight proxy.

Result: OST achieves Pareto-optimal efficiency, reducing training costs by 43% and surpassing LLM-as-a-Judge by 1.8 points, and under a fixed compute budget, OST achieves a 5.6 point gain over LLM-as-a-Judge.

Conclusion: The authors conclude that OST effectively identifies toxic samples, reverses negative transfer, and outperforms existing methods, making it a promising approach for efficient data selection in LMMs.
...
### [306] [LiteGUI: Distilling Compact GUI Agents with Reinforcement Learning](https://arxiv.org/abs/2605.07505)
*Yubin Wu,Zicheng Cai,Liping Ning,Hua Wang,Zhi Chen,Yaohua Tang,Hao Chen*

Main category: cs.AI

TL;DR: The paper proposes a novel training paradigm for lightweight, on-device vision-language GUI agents, achieving state-of-the-art performance among small-scale models and competitiveness with larger models.


<details>
  <summary>Details</summary>
Motivation: Current on-device GUI agents are limited by model capacity and performance, and traditional Supervised Fine-Tuning (SFT) methods often lead to overfitting, catastrophic forgetting, and policy rigidity.

Method: The proposed method includes Guided On-policy Distillation with oracle reference trajectories and a dynamic retrieval mechanism, and a Multi-solution Dual-level GRPO framework for aligning macro-level subtask planning with micro-level execution matching.

Result: The method achieves state-of-the-art performance among lightweight models and remains competitive with substantially larger-scale models across all benchmarks.

Conclusion: The proposed SFT-free training paradigm significantly enhances the performance of small-scale models, and structured on-policy distillation and multi-solution dual-level exploration can fully unlock the capabilities of 2B/3B scale agents.
...
### [307] [Model-Driven Policy Optimization in Differentiable Simulators via Stochastic Exploration](https://arxiv.org/abs/2605.07520)
*Yuval Aroosh,Ayal Taitler*

Main category: cs.AI

TL;DR: The authors propose Model-Driven Policy Optimization (MDPO), a framework that introduces stochastic exploration into differentiable planning to improve optimization in nonlinear and hybrid domains.


<details>
  <summary>Details</summary>
Motivation: The optimization landscapes in highly nonlinear and hybrid discrete-continuous domains are often ill-conditioned, hindering effective optimization in differentiable planning.

Method: The authors propose MDPO, which injects noise into the action space during optimization and adapts the noise magnitude based on gradient-derived sensitivity of the trajectory objective.

Result: MDPO consistently outperforms deterministic differentiable planning and model-free baselines such as PPO, significantly improving solution quality across challenging nonlinear and hybrid settings.

Conclusion: The proposed MDPO framework improves exploration of the objective landscape and helps escape poor local optima via dynamic allocation of exploration across timesteps and iterations.
...
### [308] [From Feasible to Practical: Pareto-Optimal Synthesis Planning](https://arxiv.org/abs/2605.07521)
*Friedrich Hastedt,Dongda Zhang,Antonio del Rio Chanona*

Main category: cs.AI

TL;DR: The paper introduces MORetro*, an algorithm for multi-objective retrosynthesis planning that generates a Pareto front of synthesis routes, balancing competing objectives like cost, sustainability, and yield.


<details>
  <summary>Details</summary>
Motivation: Current computer-aided synthesis planning (CASP) methods focus on finding a single feasible route, neglecting the need to balance multiple objectives as in real-world practice.

Method: The authors formulate synthesis planning as a multi-objective search problem and introduce MORetro*, an algorithm using weighted scalarization and BO-informed sampling.

Result: MORetro* produces diverse, high-quality Pareto fronts across multiple retrosynthesis benchmarks, uncovering solutions overlooked by single-objective approaches.

Conclusion: MORetro* better aligns CASP outputs with industrial decision-making by explicitly capturing trade-offs among user-defined criteria.
...
### [309] [Multi-Environment POMDPs with Finite-Horizon Objectives](https://arxiv.org/abs/2605.07537)
*Léonard Brice,Filip Cano,Krishnendu Chatterjee,Thomas A. Henzinger,Stefanie Muroya*

Main category: cs.AI

TL;DR: This paper tackles the problem of finding optimal values and policies in Multi-Environment Partially Observable Markov Decision Processes (MEPOMDPs) with finite-horizon objectives, proving its PSPACE-completeness and proposing a practical algorithm that outperforms existing solutions.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this work is to address the challenge of decision-making in stochastic environments where the agent has only partial information about the current state and the initial state is unknown and potentially adversarially chosen.

Method: The method involves first establishing the PSPACE-completeness of the problem in MEPOMDPs, and then proposing a practical algorithm for computing the optimal value and policy in MEPOMDPs with finite-horizon objectives.

Result: The results of this work are twofold: (1) it confirms that the problem of computing optimal values and policies in MEPOMDPs is PSPACE-complete, and (2) it presents a practical algorithm that significantly outperforms the previously known algorithm on classical benchmarks.

Conclusion: In conclusion, this paper contributes to the understanding and solving of MEPOMDPs by confirming the complexity of the problem and providing a more efficient algorithmic solution, which can be applied to improve decision-making processes in complex, uncertain environments.
...
### [310] [From Pixels to Prompts: Vision-Language Models](https://arxiv.org/abs/2605.07544)
*Khang Hoang Nhat Vo*

Main category: cs.AI

TL;DR: The book aims to provide a clear understanding of Vision-Language Models, offering a mental map to navigate the field with confidence.


<details>
  <summary>Details</summary>
Motivation: The rapid evolution of the field and the gap between knowing buzzwords and understanding how Vision-Language Models work.

Method: The book provides a structured approach to understanding Vision-Language Models, rather than an exhaustive catalog of datasets, benchmarks, and models.

Result: Readers will gain a clear mental map of Vision-Language Models, enabling them to read new papers with confidence and design their own systems.

Conclusion: The book seeks to bridge the gap between superficial knowledge and deep understanding of Vision-Language Models.
...
### [311] [Open-Ended Task Discovery via Bayesian Optimization](https://arxiv.org/abs/2605.07572)
*Masaki Adachi,Yuta Suzuki,Juliusz Ziomek*

Main category: cs.AI

TL;DR: The paper introduces Generate-Select-Refine (GSR), a Bayesian optimization framework for scientific workflows that dynamically generates and optimizes tasks, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: The task itself in scientific workflows is a major source of uncertainty in Bayesian optimization, which is often overlooked.

Method: The authors propose Generate-Select-Refine (GSR), an open-ended Bayesian optimization framework that alternates between task generation and task optimization.

Result: GSR outperforms existing LLM-based optimizers in applications such as new product development, chemical synthesis scaling, algorithm analysis, and patent repurposing.

Conclusion: The proposed GSR framework effectively handles uncertainty in scientific workflows by generating and optimizing tasks in a coarse-to-fine manner, with only logarithmic regret overhead relative to single-task Bayesian optimization.
...
### [312] [Parallel Lifted Planning via Semi-Naive Datalog Evaluation](https://arxiv.org/abs/2605.07584)
*Dominik Drexler,Oliver Joergensen,Jendrik Seipp*

Main category: cs.AI

TL;DR: The paper presents a parallel execution model for lifted classical planning, which achieves significant speedup on multi-core processors.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the performance of lifted classical planners by leveraging parallelism, as they are typically slower than ground planners due to repeated instantiation of ground structures during search.

Method: The authors develop an execution model with two levels of parallelism: rule-level parallelism and grounding parallelism. They also propose a grounder based on clique enumeration and extend it to support semi-naive Datalog evaluation.

Result: The experimental evaluation shows that the proposed implementation solves more tasks than the baselines on a single core, and the gap widens as additional cores are used. On hard-to-ground tasks, it achieves up to a 6-fold speedup on 8 cores.

Conclusion: The paper concludes that the proposed parallel execution model significantly improves the performance of lifted classical planning, especially on hard-to-ground tasks, by exploiting parallelism and optimizing Datalog evaluation.
...
### [313] [Inference Time Causal Probing in LLMs](https://arxiv.org/abs/2605.07631)
*Sadegh Khorasani,Saber Salehkaleybar,Negar Kiyavash,Matthias Grossglauser*

Main category: cs.AI

TL;DR: A new method called Hidden-state Driven Margin Intervention (HDMI) is proposed for causal probing in generative models, which directly steers hidden states without relying on probe classifiers.


<details>
  <summary>Details</summary>
Motivation: Existing causal probing methods rely on training auxiliary probe classifiers, which can be task-specific and risk misalignment with the model's predictive geometry.

Method: HDMI uses a gradient-based technique that applies a margin objective to increase the probability of a target continuation while decreasing that of the source, and a lookahead variant (LA-HDMI) for text editing.

Result: HDMI consistently achieves higher reliability than prior methods on the LGD agreement corpus and the CausalGym benchmark, across Meta-Llama-3-8B-Instruct, and Pythia-70M.

Conclusion: The proposed HDMI method provides a more reliable and probe-free approach for causal probing in generative models.
...
### [314] [Learning to Communicate Locally for Large-Scale Multi-Agent Pathfinding](https://arxiv.org/abs/2605.07637)
*Valeriy Vyaltsev,Alsu Sagirova,Anton Andreychuk,Yuri Kuratov,Konstantin Yakovlev,Aleksandr Panov,Alexey Skrynnik*

Main category: cs.AI

TL;DR: A new method, LC-MAPF, uses learnable communication to improve cooperation between agents in multi-agent pathfinding problems, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: The need for efficient and scalable solvers for multi-robot trajectory planning problems, which are NP-hard to solve optimally.

Method: LC-MAPF, a pre-trained model that uses multi-round communication between neighboring agents to exchange information and improve coordination.

Result: LC-MAPF outperforms existing learning-based MAPF solvers across diverse metrics in a range of test scenarios, without compromising scalability.

Conclusion: The introduction of a learnable communication module enhances cooperation between agents in multi-agent pathfinding problems, leading to improved performance and scalability.
...
### [315] [Tacit Knowledge Extraction via Logic Augmented Generation and Active Inference](https://arxiv.org/abs/2605.07639)
...
TL;DR: The paper proposes a neuro-symbolic framework to capture and formalize tacit knowledge in procedural domains, and evaluates it in a manufacturing case study.


<details>
  <summary>Details</summary>
Motivation: Tacit knowledge is difficult to capture and formalize, yet it plays a crucial role in human expertise, especially in procedural domains.

Method: A neuro-symbolic framework combining Logic-Augmented Generation and Active-Inference-inspired approach for ontology-grounded Knowledge Graph construction.

Result: The proposed solution improves completeness and semantic quality in a knowledge transfer case study in manufacturing.

Conclusion: The paper advances neuro-symbolic knowledge engineering for industrial domains by providing a framework to transform tacit and process-centric knowledge into formally specified, machine-interpretable representations.
...
### [316] [FactoryBench: Evaluating Industrial Machine Understanding](https://arxiv.org/abs/2605.07675)
...
TL;DR: Introduced FactoryBench, a benchmark for evaluating time-series models and LLMs on machine understanding over industrial robotic telemetry, revealing a wide gap between current models and operational machine understanding.


<details>
  <summary>Details</summary>
Motivation: To evaluate time-series models and LLMs on machine understanding over industrial robotic telemetry, specifically addressing the gap in operational machine understanding.

Method: Proposed a scalable Q&A generation framework and constructed FactoryBench, a large-scale benchmark of over 70k Q&A items, using datasets FactoryWave, AURSAD, and voraus-AD.

Result: Zero-shot evaluation of six frontier LLMs showed that no model exceeded 50% on structured levels or 18% on decision-making.

Conclusion: Current models have a wide gap with operational machine understanding, highlighting the need for further development in this area.
...
### [317] [GASim: A Graph-Accelerated Hybrid Framework for Social Simulation](https://arxiv.org/abs/2605.07692)
*Xuan Zhou,Yanhui Sun,Hantao Yao,Allen He,Yongdong Zhang,Wu Liu*

Main category: cs.AI

TL;DR: A new framework called GASim is proposed for large-scale social simulations, which combines graph acceleration and hybrid multi-agent methods to achieve a 9.94-fold speedup and reduce costs.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the high latency issue in large-scale social simulations caused by expensive memory retrieval and sequential ABM execution in existing hybrid methods.

Method: The GASim framework uses Graph-Optimized Memory (GOM) for core agents driven by large language models (LLM), Graph Message Passing (GMP) for ordinary agents, and Entropy-Driven Grouping (EDG) for dynamic partitioning.

Result: The GASim framework achieves a 9.94-fold end-to-end speedup over traditional hybrid frameworks and consumes less than 20% of baseline tokens, while preserving strong alignment with real-world public opinion trends.

Conclusion: The proposed GASim framework provides a more efficient and cost-effective solution for large-scale social simulations, enabling faster and more accurate studies of complex social patterns.
...
### [318] [Finite-Time Analysis of MCTS in Continuous POMDP Planning](https://arxiv.org/abs/2605.07703)
*Da Kong,Vadim Indelman*

Main category: cs.AI

TL;DR: The paper provides a finite-time analysis for Monte Carlo Tree Search (MCTS) in Partially Observable Markov Decision Processes (POMDPs) with probabilistic concentration bounds for discrete and continuous observation spaces.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the open problem of rigorous finite-time guarantees for MCTS-style solvers in POMDPs, which is challenging due to nonstationarity and interdependencies induced by heuristic action selection.

Method: The method involves extending the polynomial exploration bonus to UCB in POMDPs for discrete settings, and introducing an abstract partitioning framework for continuous observation spaces, with a proposed algorithm Voro-POMCPOW.

Result: The result is a high-probability bound on value estimates in POMDPs with continuous observation space, and empirical validation shows competitive performance of the proposed Voro-POMCPOW algorithm.

Conclusion: The conclusion is that the paper provides finite-time guarantees for MCTS in POMDPs, closing a gap in the field, and the techniques developed are also applicable to continuous MDPs.
...
### [319] [Hierarchical Task Network Planning with LLM-Generated Heuristics](https://arxiv.org/abs/2605.07707)
*Felipe Meneguzzi,Alexandre Buchweitz,Augusto B. Corrêa,Victor Scherer Putrich,André Grahl Pereira*

Main category: cs.AI

TL;DR: This paper investigates using large language models (LLMs) to generate effective search heuristics for Hierarchical Task Network (HTN) planning, and shows that LLM-generated heuristics can nearly match the coverage of the best available HTN planner while reducing search effort.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the efficiency of HTN planning by leveraging domain knowledge through large language models (LLMs) to generate effective search heuristics, as current heuristics in HTN planning are not as informative as those in classical planning.

Method: The method involves using nine LLMs to generate heuristics under domain-specific prompting, and evaluating them on six standard total-order HTN benchmark domains using the Pytrich planner, comparing against baselines and the PANDA planner.

Result: The results show that LLM-generated heuristics nearly match the coverage of the best available HTN planner, while substantially reducing search effort on 83% of shared problems.

Conclusion: The conclusion is that large language models can be effective in generating search heuristics for HTN planning, offering a promising approach to improve the efficiency of HTN planning.
...
### [320] [Online Goal Recognition using Path Signature and Dynamic Time Warping](https://arxiv.org/abs/2605.07736)
*Douglas Tesch,Nathan Gavenski,Leonardo Amado,Odinaldo Rodrigues,Felipe Meneguzzi*

Main category: cs.AI

TL;DR: A novel method for online goal recognition using path signatures to efficiently encode trajectories and improve comparison accuracy.


<details>
  <summary>Details</summary>
Motivation: The challenges of efficiently encoding large trajectories and effectively comparing them in online goal recognition in continuous domains.

Method: Using path signatures, a compact and expressive representation of rough path theory, to capture key semantic features of trajectories.

Result: The method consistently outperforms the state of the art in predictive accuracy and online planning efficiency, while remaining competitive offline.

Conclusion: The proposed method provides a more efficient and accurate approach to online goal recognition in continuous domains.
...
### [321] [Alternating Target-Path Planning for Scalable Multi-Agent Coordination](https://arxiv.org/abs/2605.07744)
*Yu Kumagai,Keisuke Okumura*

Main category: cs.AI

TL;DR: A new framework for solving the target assignment and pathfinding problem is proposed, decoupling target assignment from pathfinding and using iterative refinement to improve solution quality and scalability.


<details>
  <summary>Details</summary>
Motivation: The existing Conflict-Based Search (CBS) approach to solving the target assignment and pathfinding problem is compute-intensive and non-scalable, necessitating a more efficient solution.

Method: An iterative refinement framework that decouples target assignment from pathfinding, utilizing modern multi-agent pathfinding solvers like LaCAM, and repeatedly solving pathfinding, identifying bottlenecks, and refining the target assignment.

Result: The proposed framework scales well beyond the state-of-the-art CBS-based solver while maintaining decent solution quality, representing a significant step towards practical, large-scale target assignment and pathfinding.

Conclusion: The proposed framework offers a more scalable and efficient approach to solving the target assignment and pathfinding problem, suitable for real-world applications.
...
### [322] [RuleSafe-VL: Evaluating Rule-Conditioned Decision Reasoning in Vision-Language Content Moderation](https://arxiv.org/abs/2605.07760)
*Zhifeng Lu,Dianyuan Wang,Yuhu Shang,Zhenbo Xu*

Main category: cs.AI

TL;DR: The authors introduce RuleSafe-VL, a benchmark for evaluating rule-conditioned decision reasoning in vision-language content moderation, highlighting the limitations of current multimodal safety benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks for content moderation focus on matching predefined labels, but do not test the underlying rule structure, making it difficult to assess whether models apply policy rules correctly.

Method: The authors derive 93 atomic rules and 92 typed rule relations from publicly available platform moderation policies, creating 2,166 context-sensitive image-text cases across three high-risk policy families, and propose four diagnostic tasks to evaluate rule-conditioned decision reasoning.

Result: Experiments on 10 VLMs reveal that rule-relation recovery and decision-state prediction remain challenging, with the best model reaching only 64.8 and 64.5 Macro-F1 respectively.

Conclusion: RuleSafe-VL provides a more comprehensive evaluation of content moderation models by focusing on rule-conditioned decision reasoning, rather than just final-label scoring.
...
### [323] [Exact Regular-Constrained Variable-Order Markov Generation via Sparse Context-State Belief Propagation](https://arxiv.org/abs/2605.07839)
*François Pachet*

Main category: cs.AI

TL;DR: The paper extends variable-order Markov models to handle regular constraints, enabling efficient inference and generation of sequences under specific control requirements.


<details>
  <summary>Details</summary>
Motivation: The motivation is to integrate variable-order Markov models with regular constraints, which describe control requirements such as fixed positions and forbidden patterns, to generate sequences that meet these constraints.

Method: The method involves constructing a sparse product of the variable-order Markov model and the regular constraint automaton, replacing the first-order Markov state with the observed context state.

Result: The result is an efficient algorithm for inference and generation of sequences under regular constraints, with linear complexity in the sequence horizon for a fixed trained context graph and automaton.

Conclusion: The paper concludes by presenting a framework for exact inference and generation under variable-order Markov models with regular constraints, also enabling reversible data augmentation.
...
### [324] [AgentEscapeBench: Evaluating Out-of-Domain Tool-Grounded Reasoning in LLM Agents](https://arxiv.org/abs/2605.07926)
*Zhengkang Guo,Yiyang Li,Lin Qiu,Xiaohua Wang,Jingwen Xv,Dongyu Ru,Xiaoyu Li,Xiaoqing Zheng,Xuezhi Cao,Xunliang Cai*

Main category: cs.AI

TL;DR: The paper introduces AgentEscapeBench, a benchmark to test LLM agents' ability to use tools and reason under long-range dependencies. Results show that agents struggle with deep contextual dependencies, unlike humans.


<details>
  <summary>Details</summary>
Motivation: To evaluate LLM agents' ability to sustain tool-grounded reasoning beyond familiar workflows and short-range interactions.

Method: Introducing AgentEscapeBench, an escape-room-style benchmark with 270 instances across five difficulty tiers, requiring agents to invoke external functions, track hidden state, and propagate intermediate results.

Result: Experiments show that agent performance drops sharply as dependency depth increases, with the best model dropping from 90.0% to 60.0% success rate, unlike humans who decline from 98.3% to 80.0%.

Conclusion: Current LLM agents struggle with deep contextual dependencies, and AgentEscapeBench can serve as a diagnostic testbed to measure current agent capabilities and inform future training efforts.
...
### [325] [TraceFix: Repairing Agent Coordination Protocols with TLA+ Counterexamples](https://arxiv.org/abs/2605.07935)
*Shuren Xia,Qiwei Li,Taqiya Ehsan,Jorge Ortiz*

Main category: cs.AI

TL;DR: The paper presents TraceFix, a verification-first pipeline for Large Language Model (LLM) multi-agent coordination. It synthesizes a protocol topology, generates coordination logic, and iteratively repairs the protocol until verification succeeds. The results show that all tasks reach full verification, with 62.5% passing on the first attempt, and significant improvements in task completion and deadlock/livelock reduction.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the challenge of coordinating multiple Large Language Models (LLMs) in a reliable and efficient manner. The authors aim to develop a systematic approach to ensure correct and efficient coordination among LLMs.

Method: The method involves synthesizing a protocol topology from a task description, generating PlusCal coordination logic, and iteratively repairing the protocol using counterexamples from the TLA+ model checker (TLC) until verification succeeds. Verified process bodies are compiled into per-agent system prompts and executed under a runtime monitor.

Result: The results show that all 48 tasks spanning 16 scenario families reach full TLC verification, with 62.5% passing on the first attempt and none requiring more than four repair iterations. The state spaces span six orders of magnitude, yet verification completes in under 60 seconds for every task. A runtime comparison shows that topology-monitored execution achieves a high task completion rate (89.4% average, 81.5% full) and that runtimes using the verified protocol degrade at roughly half the rate of baselines when model capability is reduced. TLC-verified protocols cut deadlock/livelock (DL/LL) from 31.1% to 14.1%, with the largest separation under fault injection.

Conclusion: The conclusion is that the proposed TraceFix pipeline is effective in ensuring reliable and efficient coordination among LLMs. The verification-first approach allows for the detection and repair of potential errors, leading to significant improvements in task completion and deadlock/livelock reduction.
...
### [326] [The Limits of AI-Driven Allocation: Optimal Screening under Aleatoric Uncertainty](https://arxiv.org/abs/2605.07979)
*Santiago Cortes-Gomez,Mateo Dulce Rubio,Carlos Patino,Bryan Wilder*

Main category: cs.AI

TL;DR: The paper discusses combining screening and algorithmic targeting for resource allocation to optimize efficiency and reduce misallocation.


<details>
  <summary>Details</summary>
Motivation: The rise of machine learning has led to algorithmic targeting in policy and humanitarian settings, but this approach has limitations due to aleatoric uncertainty, leading to misallocation of resources.

Method: A two-stage allocation framework is proposed, combining screening and algorithmic targeting, where a screening stage observes true outcomes for a subset of units before a final allocation stage assigns resources under a fixed coverage budget.

Result: The optimal strategy screens units at the margin of algorithmic allocation and directly targets the highest-risk units, with efficiency gains from screening growing as aleatoric uncertainty in the population increases.

Conclusion: The paper provides a framework for optimally combining screening and algorithmic targeting, with applications in income-based social protection programs and humanitarian demining in Colombia.
...
### [327] [Abductive Reasoning with Probabilistic Commonsense](https://arxiv.org/abs/2605.08011)
*Joseph Cotnareanu,Chiara Roverato,Han Zhou,Didier Chetelat,Yingxue Zhang,Mark Coates*

Main category: cs.AI

TL;DR: The paper proposes a probabilistic framework for abductive commonsense reasoning, called Probabilistic Abductive CommonSense (PACS), which models variation in commonsense beliefs across individuals and outperforms prior methods in multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of formal solvers lacking commonsense world knowledge and the assumption of universal agreement on commonsense facts, which is not reality.

Method: The method proposed is Probabilistic Abductive CommonSense (PACS), a novel algorithm that uses an LLM and a formal solver to sample proofs as observations of individuals' distinct commonsense beliefs, and aggregates conclusions across these samples.

Result: Empirically, PACS outperforms chain-of-thought reasoning, prior neurosymbolic methods, and search-based approaches across multiple benchmarks.

Conclusion: The paper concludes that PACS is an effective approach for abductive commonsense reasoning, as it explicitly models variation in commonsense beliefs across individuals and achieves better results than prior methods.
...
### [328] [Learning CLI Agents with Structured Action Credit under Selective Observation](https://arxiv.org/abs/2605.08013)
*Haoyang Su,Ying Wen*

Main category: cs.AI

TL;DR: The paper discusses challenges in learning command line interface (CLI) agents and proposes two solutions: σ-Reveal for selective observation and Action Advantage Assignment (A³) for credit assignment.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address two bottlenecks in CLI learning: identifying task-relevant evidence in large codebases and assigning sparse terminal rewards to actions in long multi-turn trajectories.

Method: The authors propose two methods: σ-Reveal, an inference-time mechanism for selective observation, and Action Advantage Assignment (A³), a native agentic RL method for credit assignment.

Result: The authors construct ShellOps, a verifiable dataset suite covering CLI tasks in repository environments, to evaluate the problem setting.

Conclusion: The paper presents solutions to improve the learning of CLI agents by leveraging structured attributes of CLI actions and addressing key challenges in selective observation and credit assignment.
...
### [329] [MPD$^2$-Router: Mask-aware Multi-expert Prior-regularized Dual-head Deferral Router in Glaucoma Screening and Diagnosis](https://arxiv.org/abs/2605.08024)
*Wenxin Zhan*

Main category: cs.AI

TL;DR: A new framework called MPD$^2$-Router is proposed for safer glaucoma screening by routing uncertain cases to human experts, addressing issues like expert availability and workload imbalance.


<details>
  <summary>Details</summary>
Motivation: Standard Learning-to-defer (L2D) formulations overlook expert availability, heterogeneous reader behavior, workload imbalance, asymmetric diagnostic harm, and case difficulty, which can make glaucoma screening safer.

Method: MPD$^2$-Router, a mask-aware multi-expert deferral framework, that uses a dual-head deferral/allocation policy, mask-aware Gumbel--sigmoid gating, and an asymmetric cost-sensitive objective with an augmented-Lagrangian deferral budget.

Result: MPD$^2$-Router substantially lowers clinical cost and improves MCC over AI-only at a moderate deferral rate, and is Pareto-optimal in F1--MCC--cost, robust under cross-domain shift, and yields balanced expert utilization.

Conclusion: The proposed MPD$^2$-Router framework offers a safer and more effective approach to glaucoma screening by leveraging human expertise and addressing key limitations of standard L2D formulations.
...
### [330] [Rubric-Grounded RL: Structured Judge Rewards for Generalizable Reasoning](https://arxiv.org/abs/2605.08061)
*Manish Bhattarai,Ismael Boureima,Nishath Rajiv Ranasinghe,Scott Pakin,Dan O'Malley*

Main category: cs.AI

TL;DR: This paper proposes a new framework called rubric-grounded reinforcement learning (RL) that uses a large language model (LLM) judge to score responses based on multiple task-specific criteria, leading to improved performance on held-out rubric evaluation and transferable reasoning behaviors.


<details>
  <summary>Details</summary>
Motivation: The authors aim to improve the optimization signal for policy training by decomposing rewards into weighted, verifiable criteria, rather than relying on binary or holistic scores.

Method: The authors formalize rubric-grounded RL, which uses a frozen LLM judge to produce a structured, multi-criterion reward. They instantiate this framework by deriving rubrics from a large corpus of scientific and technical documents and training Llama-3.1-8B-Instruct with Group Relative Policy Optimization (GRPO).

Result: The GRPO-tuned policy achieves 71.7% normalized reward on held-out rubric evaluation and improves over the base model on four reasoning benchmarks (GSM8K, MATH, GPQA Main, and GPQA Diamond).

Conclusion: The results provide evidence that structured, document-grounded rewards can improve held-out rubric performance and induce transferable reasoning behaviors beyond the corpus used to construct the training environment.
...
### [331] [VecCISC: Improving Confidence-Informed Self-Consistency with Reasoning Trace Clustering and Candidate Answer Selection](https://arxiv.org/abs/2605.08070)
*James Petullo,Sonny George,Dylan Cashman,Nianwen Xue*

Main category: cs.AI

TL;DR: VecCISC, a lightweight framework, reduces token usage by 47% while maintaining or exceeding the accuracy of Confidence-Informed Self Consistency (CISC) by filtering semantically equivalent reasoning traces.


<details>
  <summary>Details</summary>
Motivation: The high overhead and cost of weighted majority voting methods, such as CISC, due to the need for a critic LLM to evaluate each candidate answer's confidence score.

Method: VecCISC, a framework that uses semantic similarity to filter reasoning traces, reducing the number of candidate answers to be evaluated by the critic LLM.

Result: VecCISC reduces total token usage by 47% while maintaining or exceeding the accuracy of CISC on five challenging datasets across various domains.

Conclusion: VecCISC offers a more efficient solution for scaling inference-time reasoning without sacrificing accuracy, making it a viable alternative to traditional weighted majority voting methods.
