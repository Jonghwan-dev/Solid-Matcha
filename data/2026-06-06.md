...
TL;DR: Introduced VideoKR, a large-scale training corpus for knowledge- and reasoning-intensive video understanding, and demonstrated its effectiveness in improving video reasoning capabilities.


<details>
  <summary>Details</summary>
Motivation: To strengthen knowledge- and reasoning-intensive video understanding by providing a large-scale training corpus.

Method: Developed a human-in-the-loop, skill-oriented example generation pipeline and created VideoKR, a corpus of 315K video reasoning examples over 145K CC-licensed videos.

Result: Models post-trained on VideoKR outperformed prior post-training approaches on knowledge-intensive video reasoning while remaining competitive on general video reasoning.

Conclusion: Data design is a key driver of progress in video reasoning, and VideoKR can be used to improve knowledge- and reasoning-intensive video understanding.
...
TL;DR: A new method called NIV (Neural Axis Variations) is introduced to automatically convert static fonts into variable fonts, eliminating the need for manual specification of glyph variation data.


<details>
  <summary>Details</summary>
Motivation: The labor-intensive process of constructing a variable font from a static font requires expert typographic design and manual specification of glyph variation data, which is time-consuming and inefficient.

Method: NIV uses a neural network to predict per-point displacements given glyph outlines and a set of desired design axes, operating directly on vector glyph geometry and employing a novel Property Embedding mechanism.

Result: The resulting model generalizes across unseen code points, font styles, high-complexity glyphs, and handwriting inputs, generating standard variable font files supporting continuous interpolation.

Conclusion: NIV enables the automatic conversion of static fonts into fully functional variable fonts, demonstrating a new approach to synthesizing structured geometric objects with continuous parametric variation using neural deformations.
...
TL;DR: A conversational AI assistant is developed to answer queries by accessing a user's personal camera roll, and a dataset and agent are proposed to improve long-context understanding.


<details>
  <summary>Details</summary>
Motivation: The need for AI assistants to understand long-horizon, highly personalized visual content streams to answer user queries.

Method: A dataset (camroll) with 50 users, 31,476 images, and 2,500 QA pairs is collected and annotated. A conversational AI agent (camroll-agent) with hierarchical memory and minimal tools is designed for efficient navigation.

Result: camroll-agent outperforms baselines and methods for long-context understanding AI agents, highlighting the gap in AI agents' long-context reasoning for personalized visual memory.

Conclusion: The proposed dataset and agent highlight the need for different approaches to handle personalized visual memory, considering consistency, visual details, and user-specific context.
...
TL;DR: The authors propose a framework for cross-model safety steering in generative modeling, allowing safety controls to be transferred across different models without requiring retraining or access to unsafe data on the target model.


<details>
  <summary>Details</summary>
Motivation: The need for safety control in generative modeling and the limitations of existing model-specific approaches.

Method: The authors estimate a safety direction in a source LLM from paired safe-unsafe prompts, transport it to a target generator through a lightweight alignment, and apply it at inference time.

Result: The transferred safety directions achieve comparable performance to native directions learned on the target model, without requiring target-side unsafe data, across text-to-image and text-to-video generation tasks.

Conclusion: The authors suggest that safety-relevant behavior can be controlled through latent directions that persist across models, paving the way for lightweight, reusable safety mechanisms.
...
TL;DR: A two-stage framework, TopoPult-SSL, is proposed for cross-device meibomian gland segmentation, leveraging weak-prior anchors and clinical metadata to adapt to new devices without dense gland masks.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the domain shift issue when applying clinical imaging devices, where obtaining dense gland masks is expensive, but other clinical signals are readily available.

Method: The proposed method, TopoPult-SSL, consists of two stages: Stage 1 uses four weak-prior anchors driven by target eyelid masks and clinical metadata to adapt a source-trained model without target gland masks; Stage 2 distills complementary Stage-1 teachers into a single compact student via supervised self-distillation when target gland masks are available.

Result: The distilled model achieves a Dice score of 0.716+/-0.006 (best 0.726), surpassing UA-MT (0.710) and the ensemble teacher (0.720) with a single pass. The gland-mask-free Stage-1 variant reaches Precision 0.694, significantly outperforming SAM/MedSAM (0.30-0.34, p<0.001).

Conclusion: The proposed TopoPult-SSL framework enables effective cross-device meibomian gland segmentation without requiring dense gland masks, demonstrating promising results on the public MGD-1k to CAMG research benchmark.
...
TL;DR: Proposed LightVesselNet, an efficient neural network for retinal vessel segmentation, achieving competitive performance with much larger models but with significantly fewer parameters.


<details>
  <summary>Details</summary>
Motivation: The need for efficient retinal blood vessel segmentation models that can be deployed on edge devices in resource-constrained environments, driven by the importance of early detection of diabetic retinopathy and glaucoma.

Method: LightVesselNet, a compact encoder-decoder architecture with attention mechanisms, multi-scale feature aggregation, and subpixel upsampling, along with a dedicated edge residual connection.

Result: LightVesselNet achieved high sensitivity scores and Dice coefficients across five publicly available datasets, outperforming state-of-the-art models in efficiency and demonstrating strong generalization capability.

Conclusion: LightVesselNet is a promising candidate for deployment in low-resource clinical settings and mobile screening tools, offering a balance between performance and efficiency for retinal vessel segmentation.
...
TL;DR: A physics-guided method, RePHO, is proposed to reconstruct realistic human-object interactions from monocular videos.


<details>
  <summary>Details</summary>
Motivation: Existing kinematic-based approaches produce visually plausible motion but result in physically implausible artifacts.

Method: A physics-guided reconstruction framework using reinforcement learning and an adaptive sampling strategy.

Result: Clear improvements in physical plausibility metrics over state-of-the-art methods on two standard HOI benchmarks.

Conclusion: The proposed method, RePHO, can generate physically consistent human-object interaction sequences from monocular videos.
...
TL;DR: A new benchmark dataset called Biomazon is proposed for predicting tropical forest structure and biomass using multimodal data and machine learning.


<details>
  <summary>Details</summary>
Motivation: The lack of a benchmark dataset for predicting the entire forest vertical structure as an ordered profile and the need for more accurate carbon accounting and ecosystem monitoring.

Method: The authors create a 20m multimodal benchmark dataset over the Amazon Basin, pairing GEDI RH and AGBD targets with multi-sensor predictors, and conduct a comprehensive ablation study using a shared encoder-decoder with task-specific heads.

Result: The authors establish a reference benchmark for future work on structurally consistent RH-profile prediction and structure-biomass modeling in tropical forests, and provide baseline performance results.

Conclusion: The Biomazon dataset and accompanying protocols provide a standardized framework for evaluating and improving machine learning models for tropical forest structure and biomass prediction.
...
TL;DR: A deep learning-based algorithm is proposed to restore capillary anatomical vasculature from a single Optical Coherence Tomographic Angiography (OCTA) volume, improving image quality and microvascular fidelity.


<details>
  <summary>Details</summary>
Motivation: Existing methods for imaging retinal microvasculature using OCTA neglect the intrinsic three-dimensional vascular architecture and focus primarily on noise suppression, projection artifact removal, or signal enhancement.

Method: A deep learning-based algorithm using an EfficientNet-B5 encoder and a decoder with concurrent spatial and channel squeeze-and-excitation modules, connected via skip connections, to predict the restored middle B-frame from three adjacent B-frames.

Result: The proposed model significantly improved image quality (PSNR: 26.16 +/- 1.26 vs. 22.23 +/- 0.78, SSIM: 0.91 +/- 0.02 vs. 0.72 +/- 0.03) and microvascular fidelity (Dice coefficient overlap: at least 3.8% and 51.2% improvement in 2D and 3D, respectively).

Conclusion: The proposed algorithm effectively restores capillary anatomical vasculature from a single OCTA volume, improving image quality and microvascular fidelity, which can enhance the reliability of retinal blood flow and areas of retinal nonperfusion quantification.
...
TL;DR: Deep learning models using OCT/OCTA data can accurately grade age-related macular degeneration (AMD) severity, with a biomarker-based model showing the most balanced performance.


<details>
  <summary>Details</summary>
Motivation: To develop and evaluate deep learning models for automated grading of AMD severity using OCT and OCTA data.

Method: Three deep learning models were developed using different input modalities: biomarker maps, 2D en face OCT and OCTA projections, and 3D OCT/OCTA volumes.

Result: All models demonstrated strong AMD staging performance with substantial agreement with the reference standard (QWK >= 0.83). The biomarker-based model achieved the highest overall performance (QWK = 0.85 +/- 0.03).

Conclusion: Deep learning models using OCT/OCTA data can accurately and automatically grade AMD severity, with the biomarker-based model providing the most balanced performance and showing particular value for early AMD detection.
...
TL;DR: The authors propose UNIPIXIE, a framework that predicts a continuous distribution of material properties from a single visual input, addressing the limitation of existing feed-forward networks that only provide point estimates.


<details>
  <summary>Details</summary>
Motivation: The motivation is to capture the inherent physical ambiguity of the real world, which is not addressed by existing point-estimate paradigms.

Method: The authors introduce a novel unified architecture that learns a direct mapping along an object's softest-to-stiffest spectrum on the PIXIEMULTIVERSE dataset, allowing for controllable generation of diverse, physically valid material fields.

Result: Experiments show that UNIPIXIE generates a rich variety of plausible dynamics and reduces Young's Modulus prediction error by over 50% against the strongest deterministic baseline.

Conclusion: The proposed framework, UNIPIXIE, successfully bridges the gap between static point estimates and the continuous nature of physical reality, enabling simulation-ready parameters for diverse physics solvers.
...
TL;DR: The paper studies how vision-language models (VLMs) learn novel visual concepts and compares their performance to human judgments.


<details>
  <summary>Details</summary>
Motivation: The ability of VLMs to map novel visual references to language after exposure, especially when those references contradict prior knowledge, is underexplored.

Method: The authors present the Novel Visual References Dataset (NVRD) and evaluate 5 models alongside 2,400 human judgments.

Result: Models struggle to acquire novel concepts in-context when they contradict prior knowledge and significantly overgeneralize compared to humans.

Conclusion: The paper contributes NVRD as a corpus and benchmark for research on visual concept learning in both humans and machines.
...
TL;DR: The paper proposes a novel framework, DFPL, for addressing the missing-modality problem in image-tabular multimodal learning. DFPL consists of three main modules: Shared-Specific Prototype Modeling (SSPM), Prototype-guided Fine-grained Alignment (PFA), and Class-aware Multi-scale Aggregation (CMA).


<details>
  <summary>Details</summary>
Motivation: The missing-modality problem poses a significant challenge in image-tabular multimodal learning, particularly when the two modalities are highly heterogeneous. Existing methods overlook fine-grained semantic and distributional misalignment, which hampers the exploitation of complementary cues under missing modalities.

Method: The proposed framework, DFPL, consists of three main modules: (1) Shared-Specific Prototype Modeling (SSPM) for extracting compact and diverse shared and modality-specific prototypes, (2) Prototype-guided Fine-grained Alignment (PFA) for enforcing prototype-level distribution matching and prototype-to-class semantic alignment, and (3) Class-aware Multi-scale Aggregation (CMA) for adaptively aggregating shared semantics and modality-specific characteristics.

Result: Extensive experiments on three diverse image-tabular benchmarks demonstrate the superiority of the proposed method compared to previous approaches under various missing-modality settings.

Conclusion: The proposed framework, DFPL, effectively addresses the missing-modality problem in image-tabular multimodal learning by capturing fine-grained semantic and distributional consistency across modalities, leading to robust predictions under various missing-modality settings.
...
TL;DR: Automated detection of horse blinks from video for pain and stress assessment.


<details>
  <summary>Details</summary>
Motivation: Pain and affective state assessment in horses through facial action units (AUs) is under-explored. Blink movements are indicators of pain and stress but are subtle and hard to detect manually.

Method: Three methods were developed and evaluated: a frame-based YOLOv12 detector, an optical flow magnitude thresholding approach, and a fine-tuned VideoMAE model.

Result: Achieved a macro-F1 score of 0.898 for blink classification and 0.926 for binary blink detection.

Conclusion: The study highlights the potential and challenges of fine-grained AU detection for equine welfare monitoring.
...
TL;DR: The authors propose ORACLE-CT, a framework for anatomy-aware aggregation in abdominal CT disease classification, improving performance and external robustness.


<details>
  <summary>Details</summary>
Motivation: Abdominal CT disease classification is challenging due to the large 3D volume of scans and the localized nature of diagnostic evidence, which current study-level classifiers struggle to effectively aggregate.

Method: The authors propose ORACLE-CT, an encoder-agnostic framework that uses multi-organ segmentation to restrict attention pooling to relevant anatomical regions, and evaluate it with three encoder families: DINOv3, I3D-ResNet-121, and Pillar-0.

Result: ORACLE-CT improves performance on MERLIN, Duke-Abdomen, and AMOS datasets, with gains in macro-AUROC/AUPRC, and demonstrates improved external robustness and auditable links to anatomical evidence.

Conclusion: ORACLE-CT offers a solution to effectively aggregate localized disease evidence and global evidence, leading to improved performance, external robustness, and transparency in abdominal CT disease classification.
...
TL;DR: The paper proposes a concept-based model that uses formal concept lattices to guide neural network learning, enabling the development of staged, semantically grounded representations throughout the network's depth.


<details>
  <summary>Details</summary>
Motivation: The authors aim to make deep learning models more interpretable and aligned with human reasoning by representing classes through meaningful semantic abstractions, specifically by organizing concepts hierarchically.

Method: The authors draw on Formal Concept Analysis and use formal concept lattices as principled semantic scaffolds to guide neural network learning, allowing the model to develop staged representations throughout its depth.

Result: The proposed models produce more interpretable embeddings, support more effective interventions, and learn concept representations that are both meaningful and hierarchically structured, as demonstrated by empirical results on real-world datasets.

Conclusion: The paper concludes that using formal concept lattices to guide neural network learning enables the development of more interpretable and semantically grounded models, better aligned with human reasoning.
...
TL;DR: This paper explores predicting and improving human preference metrics for diffusion models by leveraging a prediction model to forecast human preference scores before generation, aiming to enhance image quality with negligible hardware overhead.


<details>
  <summary>Details</summary>
Motivation: The stochastic nature of diffusion models affects generated content quality, and predicting human preference metrics can help improve image quality, especially in smaller models for local deployment.

Method: The authors investigate predicting scalar human preference metrics (HPM) scores before generation and leveraging this prediction to enhance image quality, studying which HPMs are best suited for this task.

Result: The investigation reveals that predicting HPM scores and leveraging this prediction to improve image quality is possible with negligible hardware overhead.

Conclusion: The study demonstrates the feasibility of predicting human preference metrics to enhance the quality of generated images in diffusion models, particularly in scenarios with limited computational resources.
...
TL;DR: The paper proposes a phase-aware large language model (LLM) agent for optimizing multi-stage retrieval systems, which outperforms traditional methods by navigating coupled parameter spaces and achieving significant improvements in throughput and efficiency.


<details>
  <summary>Details</summary>
Motivation: Traditional hyperparameter optimization (HPO) methods rely on an independence assumption that prevents them from navigating coupled configuration spaces in multi-stage retrieval systems, which are crucial for modern AI applications.

Method: The paper proposes a phase-aware LLM agent that conditions each proposal on its full optimization history, navigating the coupled parameter space across phase-partitioned exploration, exploitation, and fine-tuning stages.

Result: The agent outperforms Optuna TPE by +33.3% and VDTuner by +34.2% under SIEVE, delivering a 15.3x throughput gain over UniIR on the HICO-DET benchmark, and shows transferability across vector database management system (VDBMS) platforms.

Conclusion: The proposed phase-aware LLM agent effectively optimizes multi-stage retrieval systems by navigating coupled parameter spaces, achieving significant improvements in throughput and efficiency, and demonstrating transferability across different VDBMS platforms.
...
TL;DR: A novel framework for unpaired RGB-thermal novel view synthesis (NVS) is proposed, leveraging a 3D transformer architecture and a multi-modal 3D Gaussian Splatting approach to enable precise 3D scene reconstruction with visual and thermal information.


<details>
  <summary>Details</summary>
Motivation: Existing methods for multi-modal NVS rely on precisely calibrated RGB-thermal image pairs or stereo setups, which limits scalability and practical deployment.

Method: The proposed framework uses VGGT, a 3D feed-forward transformer architecture, to independently estimate camera poses for each modality, and then aligns the pose sets using the Procrustes algorithm with a cross-modal feature matcher. A multi-modal 3D Gaussian Splatting approach is also proposed to learn directly from unpaired RGB and thermal images.

Result: The proposed method achieves competitive performance in thermal view synthesis while maintaining RGB fidelity, and demonstrates the importance of evaluating both per-modality image synthesis and the multi-modal coherence of reconstructed scenes.

Conclusion: The proposed framework enables precise 3D scene reconstruction with visual and thermal information without requiring paired calibration, and provides a benchmarking framework to evaluate the performance of multi-modal NVS methods.
...
TL;DR: The paper proposes a sensor-guided adaptive contrastive learning framework for visual representation learning in PointGoal navigation, which improves policy-level scene transfer across diverse environments.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the transferability of visual representations learned for PointGoal navigation across different environments, especially under severe appearance and semantic shifts.

Method: The method proposed is a sensor-guided adaptive contrastive learning framework that uses privileged LiDAR sensing to guide the contrastive objective, and a cross-stage domain mismatch to suppress environment-specific shortcuts.

Result: The results show that the proposed approach significantly improves policy-level scene transfer across diverse indoor and outdoor environments, outperforming large pretrained vision models and standard contrastive baselines.

Conclusion: The conclusion is that the proposed sensor-guided adaptive contrastive learning framework is effective for visual representation learning in PointGoal navigation, leading to improved transferability and performance across diverse environments.
...
TL;DR: BRepCLIP is a framework for learning representations of CAD models by aligning boundary representations (BReps) with language and image embeddings through contrastive pretraining.


<details>
  <summary>Details</summary>
Motivation: The native format of CAD, boundary representations BReps, has received little attention in 3D representation learning, and the authors aim to address this gap.

Method: BRepCLIP models CAD objects as sequences of face and edge tokens with spatial and semantic descriptors, and uses a transformer encoder to aggregate these tokens into a global BRep embedding, aligned with CLIP's text and image encoders via contrastive pretraining.

Result: BRepCLIP generates more discriminative and semantically grounded embeddings than existing point-based alternatives, improving Top-1 retrieval and zero-shot classification performance on several benchmarks.

Conclusion: The framework establishes the importance of structure-aware pretraining for multimodal CAD understanding and demonstrates its utility as a CAD-aware similarity metric for evaluating text and image-conditioned CAD generation.
...
TL;DR: Introduced BloomBench, a multimodal benchmark for Vision-Language Models (VLMs) that evaluates cognitive abilities in six levels, revealing limitations in current models.


<details>
  <summary>Details</summary>
Motivation: The field lacks benchmarks to diagnose true reasoning abilities of VLMs and chart progress toward human-like multimodal intelligence.

Method: Introduced BloomBench, a bilingual (English-Arabic) multimodal benchmark grounded in Bloom's Taxonomy, evaluating six levels of cognition through image-question-answer tasks.

Result: State-of-the-art VLMs show strong performance in semantic understanding but struggle with factual recall and creative synthesis, with a performance gap between Arabic and English.

Conclusion: Current VLMs have cognitive limitations, and BloomBench provides a foundation for developing more cognitively aligned and inclusive VLMs.
...
TL;DR: The paper proposes a noise-aware Med-VQA framework that incorporates a denoising autoencoder to learn robust visual representations, improving the robustness of medical visual question answering to noisy input embeddings.


<details>
  <summary>Details</summary>
Motivation: Recent Med-VQA approaches overlook the importance of handling noise and small irrelevant changes in visual representations, which can negatively impact performance.

Method: The proposed framework uses a denoising autoencoder to reconstruct clean visual embeddings from corrupted inputs, and then projects the resulting embeddings into the language model embedding space using a multi-layer perceptron (MLP) and low-rank adaptation (LoRA) for efficient fine-tuning.

Result: The proposed method shows improved robustness to noisy input embeddings while maintaining competitive clean performance across multiple evaluation criteria on the SLAKE and PathVQA benchmarks.

Conclusion: Learning more robust visual representations can enhance Med-VQA performance and robustness, and the proposed noise-aware framework is a step towards achieving this goal.
...
TL;DR: A novel Dual Feature Decoupling Network (DFDNet) is proposed for fine-grained out-of-distribution detection, addressing the challenge of high visual similarity among subcategories.


<details>
  <summary>Details</summary>
Motivation: Existing OOD detection methods struggle with fine-grained tasks due to subtle variations and high visual similarity among subcategories, making OOD detection extremely challenging.

Method: The proposed DFDNet uses a spatial-frequency decoupling module and a reconstruction-guided decoupling module to disentangle features and enhance category-specific representations.

Result: The method achieves competitive performance improvements on multiple datasets.

Conclusion: The proposed DFDNet effectively addresses the challenge of fine-grained OOD detection by leveraging feature disentanglement, leading to improved performance.
...
TL;DR: The paper introduces UltraVR, a diagnostic benchmark for evaluating vision-language models (VLMs) on ultra-resolution images. It assesses VLMs' ability to acquire and integrate visual evidence across four high-value scenarios.


<details>
  <summary>Details</summary>
Motivation: Existing evaluations of VLMs have limitations, primarily reporting final-answer accuracy, which doesn't provide insight into whether models acquire and integrate necessary visual evidence, especially on ultra-resolution images.

Method: The authors introduce UltraVR, a benchmark that spans four scenarios: CCTV surveillance, remote sensing, whole-slide image pathology, and industrial anomaly detection. Each instance includes a structured ground-truth chain of thought with step-level questions, intermediate answers, and reasoning labels.

Result: The evaluation of frontier VLMs using UltraVR shows that current models are far from reliable on ultra-resolution reasoning. Errors are concentrated in evidence grounding and local perception.

Conclusion: UltraVR serves as a diagnostic testbed for measuring not only whether VLMs answer correctly but also where their ultra-resolution reasoning process breaks, highlighting areas for improvement.
...
TL;DR: The paper proposes a method called Backbone Module Composition via Reinforcement Learning (BMCR) to adaptively combine Convolutional Neural Networks (CNNs) and Vision Transformers (ViTs) for remote sensing object detection.


<details>
  <summary>Details</summary>
Motivation: Existing object detectors rely on a single fixed backbone or manually designed hybrid architecture, failing to adaptively exploit the strengths of CNNs and ViTs across diverse inputs.

Method: BMCR dynamically assembles input-adaptive inference paths from reusable modules decomposed from off-the-shelf CNN and ViT backbones using reinforcement learning.

Result: BMCR achieves 79.31%, 73.41%, and 71.86% mAP on DOTA-v1.0, DOTA-v1.5, and DIOR-R, respectively, surpassing strong baselines by up to 2.5 points.

Conclusion: The proposed BMCR method effectively combines the strengths of CNNs and ViTs for remote sensing object detection, achieving state-of-the-art performance while maintaining competitive efficiency.
...
TL;DR: The paper proposes HDST-GNN, a graph neural network for multi-object tracking from UAV imagery, addressing challenges such as varying altitude, small and densely packed objects, and frequent occlusion.


<details>
  <summary>Details</summary>
Motivation: Existing graph-based trackers assume fixed spatial context and treat all objects uniformly, ignoring the heterogeneous lifecycle states of detections, active tracklets, and lost targets.

Method: The proposed HDST-GNN includes three novel components: Altitude-Adaptive Edge Construction, Heterogeneous Node Representation, and Occlusion-Gated Temporal Aggregation.

Result: HDST-GNN achieves 94.51% MOTA and 97.24% IDF1 on VisDrone2019-MOT with oracle detections, outperforming SORT by +5.0 MOTA points and reducing identity switches by 81%.

Conclusion: The proposed HDST-GNN effectively addresses the challenges of multi-object tracking from UAV imagery, demonstrating improved performance and reduced identity switches compared to existing methods.
...
TL;DR: PigFormer is a system that uses RGB-D camera data to predict body condition metrics in sows, such as subcutaneous backfat thickness and loin muscle depth.


<details>
  <summary>Details</summary>
Motivation: Current methods for measuring sow body condition are limited, and accurate measurements are crucial for lactation performance and piglet survival.

Method: PigFormer uses a two-stage approach, first converting raw depth frames into a standardized height map, and then using a Slice Attention Encoder to predict body condition metrics.

Result: PigFormer achieves a 2.43 mm backfat mean absolute error (MAE) and 3.87 mm overall MAE on a multi-site dataset of 319 sow and gilt instances.

Conclusion: PigFormer offers a practical path toward continuous, automated, non-contact body condition monitoring in commercial swine production.
...
TL;DR: KV-Control is a new method for controlling 3D human motion models to follow specific trajectories while preserving text-conditioned motion quality.


<details>
  <summary>Details</summary>
Motivation: Existing solutions for controlling 3D human motion models either duplicate large portions of the generator or move much of the cost to test-time optimization, exposing a control trade-off between precision and preservation of pretrained motion prior.

Method: KV-Control introduces a compact attention-side control interface for frozen masked text-to-motion transformers, using a part-tokenized motion substrate and controller, including PartVQ, T-Concat, and KV-Control.

Result: The resulting adapter tracks root and multi-joint constraints with sub-centimeter accuracy under the inherited refinement protocol while retaining text-conditioned motion quality.

Conclusion: KV-Control provides a small, precise, and transparent control interface for text-to-motion generation, reframing trajectory conditioning as lightweight memory retrieval.
...
TL;DR: The paper proposes Triple-Shot Compositions (TSC), a task that generates three shots (establishing, medium, and close-up) from a single image, along with brief descriptions to support visual narration. It also introduces ShotCrop, a model that learns TSC with limited expert annotations through a three-stage training process.


<details>
  <summary>Details</summary>
Motivation: Current aesthetic composition methods produce a single crop, ignoring the narrative value of multiple shots from one scene. However, multi-shot composition is crucial for creative workflows, such as commercial posters that require multiple crops with different emphases.

Method: The authors propose ShotCrop, which undergoes a three-stage training process: Chain-of-Thought supervised fine-tuning, semi-supervised fine-tuning with pseudo labels, and optimization with Group Relative Policy Optimization for ShotCrop (GRPO-S).

Result: ShotCrop achieves an average improvement of 2.82 times over GPT-5 in shot localization accuracy. The authors also present TSC-Bench, a benchmark of 1.2k expert-annotated test cases.

Conclusion: The paper presents a novel approach to aesthetic composition, enabling the generation of multiple shots from a single image with brief descriptions, which can support visual narration and enhance creative workflows.
...
TL;DR: CrackGeoFM, a multi-task framework, enhances crack assessment with accurate masks, connected geometry, and reliable uncertainty estimates.


<details>
  <summary>Details</summary>
Motivation: Existing segmentation models fail to provide accurate, connected crack geometry and calibrated uncertainty, crucial for reliable infrastructure assessment.

Method: CrackGeoFM combines a frozen visual foundation backbone with crack-specific adaptation modules: FCEM for crack enhancement, CFAM for feature adaptation, and SMTD for joint decoding.

Result: CrackGeoFM achieves state-of-the-art segmentation, improved topology preservation, and calibrated uncertainty across 20 crack datasets, with effective few-shot adaptation.

Conclusion: CrackGeoFM supports reliable, generalizable, and engineering-oriented crack analysis for infrastructure assessment with improved accuracy and uncertainty estimation.
...
TL;DR: Proposed CoFi-UCGen, a novel framework for unsupervised conditional image generation that disentangles global semantics from fine-grained variations, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Unsupervised conditional image generation (UCGen) is challenging due to unstructured semantic representations across granularities, and existing methods rely on manually annotated labels or pre-trained feature extractors.

Method: CoFi-UCGen framework, which includes adversarial semantic reciprocal learning theory, bit-codes for structured coarse-grained latent space, and hierarchical modulation mechanism in diffusion models.

Result: CoFi-UCGen consistently outperforms existing UCGen methods in terms of image quality, semantic consistency, and control accuracy without any label priors or pre-trained feature extractors.

Conclusion: The proposed CoFi-UCGen framework effectively addresses the challenging UCGen task by explicitly decomposing coarse-to-fine semantics, enabling successful unsupervised conditional image generation without labels.
...
TL;DR: Introduced V2V-Bench, a benchmark for evaluating video-to-video generation models, and compared three models.


<details>
  <summary>Details</summary>
Motivation: Existing metrics for text-to-video and image-to-video generation do not capture the complexities of video-to-video generation.

Method: Proposed V2V-Bench, an 11-dimension benchmark organized into five categories, to evaluate video-to-video generation models.

Result: Grok Imagine and Gemini Veo3 showed complementary strengths, with Grok performing better on editing fidelity and Veo3 achieving stronger visual quality.

Conclusion: V2V-Bench effectively evaluates video-to-video generation models, with a high Spearman correlation of 0.905 with human judgments.
...
TL;DR: The paper introduces LongSpace-Bench, a benchmark for evaluating long-horizon spatial memory in videos, and proposes LongSpace, a memory framework that improves long-video spatial understanding by incorporating 3D structural cues and layer-aware memory.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitation of current Multimodal Large Language Models (MLLMs) in handling long-horizon tasks that require remembering and retrieving previously observed spatial layouts, routes, viewpoint changes, and object states.

Method: The method proposed is LongSpace, a memory framework that models long videos as sequential chunks, incorporates 3D structural cues into early decoder layers, and constructs layer-aware memory for question-guided retrieval.

Result: The results show that LongSpace improves long-video spatial understanding on multiple spatial reasoning benchmarks, demonstrating the importance of explicit spatial memory for long-horizon video MLLMs.

Conclusion: The conclusion is that LongSpace, with its memory framework, enhances the capability of MLLMs in long-horizon spatial memory and reasoning, which is crucial for applications like autonomous driving and robotic navigation.
...
TL;DR: T-SAR-JEPA is a self-supervised framework for detecting temporal anomalies in SAR amplitude stacks, achieving 77.0% ROC-AUC on the DFC 2026 dataset.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to develop a self-supervised framework for temporal anomaly detection in SAR (Synthetic Aperture Radar) amplitude stacks.

Method: The method involves a ViT-Base/16 encoder domain-adapted on Capella patches using local masked reconstruction with gradient feature prediction, and a temporal transformer with sinusoidal time encoding for forecasting future latent states.

Result: The model achieves a ROC-AUC of 77.0% on the Hawaii eruption window of the DFC 2026 dataset, outperforming several baselines, and shows a spatial coherence of 99.9% (p < 0.001, permutation test).

Conclusion: The proposed framework, T-SAR-JEPA, demonstrates effective temporal anomaly detection in SAR amplitude stacks, leveraging self-supervised learning and domain adaptation.
...
TL;DR: Proposed Parallel Jacobi Decoding (PJD) accelerates autoregressive image generation by refining draft tokens in 2D spatial domain.


<details>
  <summary>Details</summary>
Motivation: Autoregressive models generate high-fidelity images but have slow inference due to sequential prediction. Jacobi-style decoding helps but has limited acceleration due to error propagation.

Method: Parallel Jacobi Decoding (PJD) expands draft tokens in 2D spatial domain, adjusts attention mask to mitigate error accumulation, and improves convergence stability.

Result: PJD achieves 4.8x-6.4x acceleration across multiple autoregressive image generation models while maintaining competitive generation quality.

Conclusion: PJD is an efficient and training-free decoding approach for autoregressive image generation, achieving significant acceleration without compromising generation quality.
...
TL;DR: A real-time object detection-based surveillance framework is proposed to detect guns, knives, and blunt objects in densely populated urban environments.


<details>
  <summary>Details</summary>
Motivation: Traditional surveillance approaches are inefficient and prone to errors due to manual monitoring, necessitating an intelligent and automated video surveillance system.

Method: A YOLOv8-based object detection model is trained on a consolidated dataset of 7,959 images across three classes: gun, knife, and blunt object.

Result: The proposed framework achieves an effective balance between accuracy and efficiency, with increasing training duration improving recall and average precision for the blunt object class.

Conclusion: The proposed framework is suitable for deployment in real-world surveillance environments such as campuses, public spaces, and transportation areas.
...
TL;DR: The authors propose ViCuR, a visually grounded privileged-teacher distillation framework that improves multimodal on-policy distillation by replacing answer-side privilege with visual cues, leading to better performance on seven benchmarks.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the train-test mismatch in multimodal on-policy distillation caused by answer-side privilege, which can lead to shortcut imitation rather than visually grounded reasoning.

Method: The authors propose ViCuR, which uses visual cues (query-related evidence in the input) instead of answer-side privilege and introduces a lightweight cue recovery module to aggregate task-relevant visual evidence into an internal representation.

Result: ViCuR consistently improves over answer-based on-policy self-distillation by +1.19 and +1.24 on overall average performance across seven benchmarks with Qwen3-VL-2B and 8B students, and also extends naturally to stronger-teacher OPD.

Conclusion: The design of teacher privilege is as important as teacher strength in multimodal on-policy distillation, and ViCuR provides a more effective and visually grounded approach.
...
TL;DR: TextWand is a general-purpose framework for scene text editing that unifies removal, generation, and replacement into one model, achieving precise control and high-quality results.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the limitations of existing scene text editing models, which are typically task-specific and lack comprehensive benchmarks. The authors aim to create a unified framework that can handle multiple tasks with high accuracy and quality.

Method: The authors propose TextWand, which decomposes complex editing tasks into atomic primitives of rendering and erasure. They introduce two novel designs: Overlay-Reference Positional Encoding (ORPE) for pixel-level layout fidelity and style control, and Region-Adaptive Suppression (RAS) for clean text erasure.

Result: Extensive experiments demonstrate that TextWand outperforms existing leading models in scene text removal, generation, and replacement tasks, delivering superior text content accuracy, layout and style consistency, and overall image quality.

Conclusion: The authors conclude that TextWand is a effective and general-purpose framework for scene text editing, and that it sets a new standard for the field with its high-quality results and comprehensive benchmark, TextWand-Bench.
...
TL;DR: A new framework called VTI-CoT is proposed for video reasoning, which integrates textual and visual information to improve reasoning accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing Chain-of-Thought (CoT) based video reasoning methods overlook critical visual information during inference, and the authors aim to address this limitation by incorporating visual information into the CoT process.

Method: The proposed VTI-CoT framework integrates textual reasoning steps with corresponding visual frames, and uses an automated annotation pipeline to construct high-quality multimodal CoT data. It also employs Optical Character Recognition (OCR)-based compression techniques to improve training efficiency.
...
Conclusion: The proposed VTI-CoT framework effectively enhances video reasoning accuracy and efficiency by incorporating visual information into the CoT process and compressing CoT supervision signals.
...
TL;DR: The paper proposes a simple method to improve one-step action generation in diffusion-based vision-language-action models by biasing the training time distribution toward high-noise states, achieving comparable or better performance to multi-step decoding.


<details>
  <summary>Details</summary>
Motivation: The authors argue that vision-language-action models for action generation have a different condition-target structure than image generation, and therefore do not require advanced one-step methods developed for image synthesis.

Method: The authors use standard velocity prediction and add no teacher model, distillation stage, or auxiliary objective. They simply bias the training time distribution toward high-noise states.

Result: The results show that one-step policies trained with high-noise biased schedules generally match or exceed ten-step decoding under the same recipe on standard LIBERO, LIBERO-Plus, and LIBERO-Pro benchmarks.

Conclusion: The paper concludes that strong one-step vision-language-action action generation can emerge from standard diffusion training, without importing the full few-step diffusion machinery developed for image generation.
...
TL;DR: The paper challenges the assumption that better alignment between latent tokens and visual targets in vision-language models leads to better answers, finding a negative correlation and proposing diagnostics to explain this.


<details>
  <summary>Details</summary>
Motivation: The motivation is to investigate the effectiveness of latent visual reasoning (LVR) in vision-language models, specifically the assumption that better alignment between latent tokens and visual targets yields better answers.

Method: The authors design a matrix of five LVR variants, use cosine similarity and mean squared error as training loss and quality metric, and introduce PRISM, a pair of inference-time diagnostics: a linear probe and a corruption test.

Result: The results show a negative correlation between cosine alignment and accuracy (r=-0.94), and that the supervised latents are largely bypassed, with corrupting them shifting accuracy by at most four points.

Conclusion: The conclusion is that the auxiliary objective reshapes the language model via shared parameters rather than via the latent variable it nominally optimizes, and that better alignment does not necessarily yield better answers.
...
TL;DR: The paper proposes DRIFT, a framework for adapting vision-language models to continuous decoding tasks, achieving better performance across multiple tasks and architectures.


<details>
  <summary>Details</summary>
Motivation: The limitations of text-based output interfaces in vision-language models for tasks requiring precise continuous outputs.

Method: DRIFT combines a base predictor with a generative refinement module based on flow matching to improve predictions.

Result: DRIFT consistently outperforms regression- and generative-based solutions across multiple tasks and architectures.

Conclusion: DRIFT offers a general and effective approach for adapting pretrained vision-language models to continuous decoding tasks.
...
TL;DR: The paper proposes a physics-guided deep unfolding network (PGU-Net) for blind cross-sensor spectral super-resolution, which jointly estimates the hyperspectral image and a learnable spectral transformation function.


<details>
  <summary>Details</summary>
Motivation: Existing spectral super-resolution methods are limited to single-sensor settings and assume a fixed and known spectral response function, which is unknown and varies with sensor characteristics and scene content in practical cross-sensor scenarios.

Method: The proposed PGU-Net unrolls an alternating optimization procedure into an end-to-end trainable architecture, combining learnable proximal networks with differentiable closed-form solvers for physical interpretability and strong representation capacity.

Result: Experiments on benchmark datasets demonstrate accurate recovery of the spectral transformation function and improved reconstruction performance over state-of-the-art methods, with evaluations on a real UAV cross-sensor dataset verifying the effectiveness and robustness of PGU-Net under truly blind conditions.

Conclusion: The proposed PGU-Net effectively addresses blind cross-sensor spectral super-resolution by jointly estimating the hyperspectral image and a learnable spectral transformation function, showing promising results in both benchmark datasets and real-world applications.
...
TL;DR: A lightweight deepfake detection model called ExpSpeech-Net (SqN-R-DFD) is proposed, which analyzes facial expressions and speech patterns to achieve high accuracy in detecting deepfake videos.


<details>
  <summary>Details</summary>
Motivation: Existing deepfake detection methods are complex and resource-intensive, limiting their practical use, and there is a need for a lightweight and efficient detection framework.

Method: The ExpSpeech-Net model uses SqueezeNet and RNN as its backbone, and incorporates advanced feature extraction and a smart feature-selection strategy using SASMA.

Result: The framework achieves 94.5% accuracy, precision of 99.3%, and F-measure of 96.8%, outperforming conventional methods.

Conclusion: The proposed model enables practical, real-time deepfake detection suitable for everyday applications by integrating multiple modalities with intelligent preprocessing and feature selection.
...
TL;DR: Future-L1, an interleaved latent visual reasoning framework, improves video event prediction by preserving intermediate visual semantics in latent space, achieving state-of-the-art results on FutureBench and TwiFF-Bench benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing video MLLMs lose fine-grained motion, geometry, and interaction cues when verbalizing visual evidence, leading to ungrounded hallucinations.

Method: Future-L1 alternates between language tokens and continuous latent visual spans during autoregressive decoding, trained with a latent-aware RL objective and optimized latent trajectories.

Result: Future-L1 achieves new state-of-the-art results on FutureBench (85.4) and TwiFF-Bench (3.04), outperforming previous best models.

Conclusion: Preserving intermediate visual semantics in latent space benefits future-oriented video reasoning, reducing hallucinations and improving prediction accuracy.
...
TL;DR: LiAuto-GeoX, a real-time, onboard 3D reconstruction model for autonomous driving, achieves high-fidelity dense reconstruction at 220 FPS on KITTI and improves performance in downstream autonomy tasks.


<details>
  <summary>Details</summary>
Motivation: Existing 3D reconstruction models for autonomous driving are not efficient or accurate enough for real-time deployment, lacking long-range geometric fidelity, surround-view consistency, and real-time efficiency.

Method: The authors propose LiAuto-GeoX, a grounded driving transformer that uses sparse LiDAR priors and a novel geometry-preserving distillation framework to create a compact, onboard model.

Result: LiAuto-GeoX achieves 90.6 PDMS in trajectory prediction, 24.63 mIoU in occupancy prediction, and 47.67 IoU in future-frame prediction, demonstrating its effectiveness as a foundational geometric representation for autonomous driving.

Conclusion: The proposed LiAuto-GeoX model enables efficient dense 3D reconstruction for real-time deployment in autonomous driving, transcending its traditional role as a perception target to serve as a scalable, foundational geometric representation.
...
TL;DR: The paper proposes a novel framework called Relative Edit-induced Difference Aesthetic learning (RED-Aes) for Image Aesthetic Assessment (IAA) that focuses on learning visual factors driving aesthetic changes rather than absolute scores.


<details>
  <summary>Details</summary>
Motivation: Traditional IAA methods overlook the dynamic nature of human aesthetic perception, which relies on subconscious comparison against implicit visual references, limiting their generalization across diverse scenarios.

Method: The proposed RED-Aes framework leverages controllable image editing models and a three-stage training strategy guided by a relative ranking consistency reward, optimizing the model solely via relative supervision.

Result: RED-Aes achieves state-of-the-art performance on multiple public benchmarks, exhibiting superior generalization capabilities.

Conclusion: The proposed RED-Aes framework offers a new approach to IAA by focusing on relative aesthetic differences, leading to improved generalization across diverse scenarios.
...
TL;DR: The paper improves license plate detection and recognition (LPDR) with Cross-Spatial Hybrid Attention (CSHA) and Class-Balanced Synthetic Augmentation (CBSA), achieving a 91.5% recognition rate for minority plates.


<details>
  <summary>Details</summary>
Motivation: The limitations of the YOLOV5-PDLPR model, specifically spatial character mismatches and data imbalance, are addressed to improve LPDR system performance.

Method: The paper introduces CSHA and CBSA to enhance the YOLOV5-PDLPR model, and conducts an extensive study using 75,000 synthetic samples on four benchmarks.

Result: The proposed approach achieves a 91.5% recognition rate for minority provincial license plates and maintains a real-time processing performance of 152 FPS.

Conclusion: The proposed solution provides an effective approach for high-speed license plate recognition systems, improving the recognition rate while maintaining real-time performance.
...
TL;DR: The paper proposes an emotion-aware text-to-image pipeline to generate children's hand drawing style images from short Korean diary entries, focusing on capturing sentiment and emotion.


<details>
  <summary>Details</summary>
Motivation: T2I models struggle to capture sentiment from text, particularly in diaries, as they focus on visual object-related patterns rather than contextual emotional understanding.

Method: The proposed pipeline uses Qwen3-8B for sentiment recognition and Stable Diffusion 3.5 Medium fine-tuned with LoRA on children's drawing images with emotion-based trigger words for image generation.

Result: The paper presents experiments on the effect of emotion trigger words on generated images and discusses limitations of CLIP Score as an evaluation metric for emotion-aware image generation.

Conclusion: The paper aims to improve text-to-image models' ability to capture sentiment and emotion from text, specifically in children's diary entries, by proposing an emotion-aware pipeline.
...
TL;DR: The paper introduces a dataset called StyleGender to study the relationship between artistic style, gender representation, and AI-generated images. It also proposes metrics to measure gender artifacts in images.


<details>
  <summary>Details</summary>
Motivation: The authors are motivated by the fact that artistic styles are rooted in socio-historical contexts that encode social hierarchies, including gender constructions, but AI research has treated style as a surface-level visual property. They aim to investigate the interplay between gender representation and style in historical and generated images.

Method: The authors introduce the StyleGender dataset, which comprises 74k images spanning 19 artistic styles, and propose two Set Gender Artifact (SGA) metrics (PixelSGA and MaskSGA) to capture gender signals at the pixel level and in compositional structure.

Result: The authors show that (1) gender representation shapes visual features across artistic styles, (2) style keywords carry these patterns into T2I generation, and (3) generative models tend to amplify gender artifacts beyond what is observed in historical sources.

Conclusion: The paper concludes that the proposed dataset and metrics can help researchers better understand the relationship between artistic style, gender representation, and AI-generated images, and that generative models may perpetuate and amplify existing gender biases.
...
TL;DR: The paper presents GeoVR, a framework that enables Multimodal Large Language Models (MLLMs) to develop 3D awareness and spatial intelligence using only 2D video sequences.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitation of MLLMs in 3D awareness and geometric consistency across video frames, despite their excellence in 2D semantic understanding.

Method: The method involves a novel framework called GeoVR, which uses a multi-objective learning strategy with four geometric targets to distill geometry knowledge from pre-trained 3D foundation models into MLLMs.

Result: The result shows that GeoVR achieves state-of-the-art performance on spatial reasoning benchmarks, establishing a new paradigm for endowing foundation models with spatial intelligence.

Conclusion: The conclusion is that GeoVR effectively enables MLLMs to develop strong 3D awareness and spatial intelligence, unlocking their potential for applications requiring geometric and spatial consistency.
...
TL;DR: Proposed a geometry-aware dataset condensation method for diffusion model training, achieving superior fidelity and distributional coverage.


<details>
  <summary>Details</summary>
Motivation: Existing dataset condensation methods are ill-suited for diffusion model training due to low-fidelity synthetic data or failure to preserve distributional geometry.

Method: Reformulated real subset selection as a geometry-aware distribution alignment problem using one-sided partial optimal transport and complemented with feature-statistics and semantic consistency regularization.

Result: Achieved superior fidelity and distributional coverage in diffusion model training across various experiments.

Conclusion: The proposed method effectively preserves the geometric structure necessary for diffusion model training, leading to improved performance.
...
TL;DR: A new framework for creating lifelike digital humans with social intelligence is proposed, integrating cognitive reasoning and multimodal generation.


<details>
  <summary>Details</summary>
Motivation: Current approaches to creating digital humans treat cognitive reasoning and multimodal generation as separate tasks, lacking a unified framework.

Method: A closed-loop dual-agent framework with perception, social reasoning, and expression modules is proposed, enabling continuous interaction and bidirectional dynamics.

Result: The proposed method demonstrates competitive or superior performance on dialogue quality and video generation metrics, surpassing the full-information Script mode on key dialogue quality dimensions.

Conclusion: The proposed framework can create digital humans with genuine social intelligence, eliciting more thoughtful dialogue under uncertainty.
...
TL;DR: A new framework called SAGE enables the creation of high-fidelity, animatable avatars from minimal input data by using self-supervised learning to optimize Gaussian distributions.


<details>
  <summary>Details</summary>
Motivation: Modeling dynamic facial expressions using 3D Gaussian representations is challenging due to their unstructured nature and the need for extensive multiview and sequential expression data.

Method: The SAGE framework jointly optimizes 2D Gaussian surfels and a Signed Distance Field (SDF) with a self-supervised expression learning phase to enforce compact, surface-aligned Gaussian distributions.

Result: The approach achieves reconstruction and animation quality comparable to state-of-the-art methods while reducing data requirements by several orders of magnitude.

Conclusion: The SAGE framework has the potential to make avatar creation more accessible and data-efficient through self-supervised Gaussian deformation learning.
...
TL;DR: CamFlow+ is a hybrid framework for 2D camera motion estimation that combines different bases to represent motion in dense-flow space, improving estimation accuracy and stability.


<details>
  <summary>Details</summary>
Motivation: Existing methods for 2D camera motion estimation struggle with camera translation, depth variation, and local parallax, and CamFlow+ aims to address these limitations.

Method: CamFlow+ uses a hybrid-basis framework that represents 2D camera motion directly in dense-flow space, combining homography-derived physical bases, stochastic bases, and depth-translational bases.

Result: CamFlow+ improves sparse and dense camera-motion estimation, and achieves the best top-1 preference rate in a blind user study for digital video stabilization.

Conclusion: CamFlow+ provides a more accurate and stable approach to 2D camera motion estimation, with potential applications in computer vision and computational photography.
...
TL;DR: The paper proposes a Scene-guided Relational Modeling detection framework for open-vocabulary object detection, which captures structured relationships between objects in an image to improve detection performance.


<details>
  <summary>Details</summary>
Motivation: Current knowledge distillation-based approaches for open-vocabulary object detection overlook structured relationships between objects in an image, restricting their effectiveness in detecting novel categories.

Method: The proposed framework utilizes scene graphs to model interactions between candidate regions and their contextual objects, and incorporates a Relation Attention Module and a scene-based textual alignment branch.

Result: The proposed model achieves superior performance compared to other OVOD methods, improving the AP for novel categories on COCO and LVIS datasets.

Conclusion: The Scene-guided Relational Modeling detection framework effectively enhances detection performance by integrating visual relations with semantic information.
...
TL;DR: A new framework called MemoryCard is proposed to improve long-video question answering by organizing videos into self-contained Memory Cards, which capture coherent event-level semantics.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitation of existing frame-centric approaches in capturing coherent event-level semantics in long videos.

Method: The proposed framework, MemoryCard, segments videos into semantically coherent units, generates event-level video gists, and selects representative visual moments to create unified Memory Cards for retrieval and question answering.

Result: MemoryCard consistently improves long-video QA performance under comparable visual-token budgets, achieving up to a 21.8% relative improvement in accuracy.

Conclusion: The proposed MemoryCard framework effectively captures coherent event-level semantics in long videos, leading to improved performance in long-video question answering.
...
TL;DR: A new benchmark, FEPBench, is introduced to evaluate Text-to-Image models for scientific illustration generation, focusing on fine-grained elements and providing guidance for improvement.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for scientific illustration generation overlook fine-grained elements and under-quantify scientific reasoning ability and output conciseness.

Method: FEPBench uses multimodal large language models and human experts to provide fine-grained annotations and evaluate Text-to-Image models across three dimensions: instruction faithfulness, reasoning enrichment, and semantic precision.

Result: State-of-the-art models still struggle with text-rendering, limited reasoning enrichment, and balancing generation richness with precision.

Conclusion: The findings provide practical guidance for improving and deploying Text-to-Image models in scientific illustration generation, with benchmark data, annotations, and evaluation code to be released.
...
TL;DR: A new method, T-FunS3D, is proposed for task-driven hierarchical open-vocabulary 3D functionality segmentation, enabling robots to efficiently localize functional object components in 3D scenes.


<details>
  <summary>Details</summary>
Motivation: Current open-vocabulary 3D segmentation methods have limitations in balancing granularity, accuracy, and speed, and the proposed method aims to alleviate these challenges.

Method: T-FunS3D takes 3D point cloud and posed RGB-D images as input, constructs an open-vocabulary scene graph, and uses a vision-language model to identify relevant instances and locate their functional components.

Result: Experiments on the SceneFun3D dataset show that T-FunS3D achieves comparable performance to state-of-the-art methods while achieving faster runtime and reduced memory usage.

Conclusion: The proposed T-FunS3D method provides a more efficient and effective approach for open-vocabulary 3D functionality segmentation, enabling actionable perception for robotic applications.
...
TL;DR: The paper presents a streaming pipeline for real-time text-to-image synthesis, achieving high frame rates on consumer GPUs.


<details>
  <summary>Details</summary>
Motivation: The bottleneck in real-time text-to-image pipelines shifts from the denoiser to the text encoder when using distilled diffusion models, particularly with multimodal large language models (MLLMs).

Method: The authors propose a pipeline that uses asymmetric CUDA pipelining, a reformulated ControlNet-LLLite, and a periodic conditioning-refresh schedule to optimize performance.

Result: The pipeline achieves 27.4 fps at batch size 8 and 29.6 fps at batch size 16 on an RTX 3090 Ti, with end-to-end latency of approximately 0.5 and 1.0 seconds, respectively.

Conclusion: The proposed pipeline enables video-rate streaming throughput for text-to-image synthesis, with generalizability to unseen sequences and style families.
...
TL;DR: The paper presents a system for detecting sexism in memes and short-form videos, using a feature-engineered pipeline with gradient-boosted regression models and hierarchical post-processing.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address multimodal sexism identification and characterization in memes and short-form videos.

Method: The system uses a feature-engineered late-fusion pipeline with gradient-boosted regression models and hierarchical post-processing, combining visual, textual, demographic, biometric, and LLM-derived semantic indicators for memes, and investigating feature selection, frame-based visual representations, OCR-based textual features, acoustic descriptors, and sensor-derived metadata for videos.

Result: The results show that focused LLM-derived semantic cues improve meme sexism identification, while video performance is sensitive to feature dimensionality and cross-modal noise, and that targeted semantic feature engineering is useful for static memes, but more robust temporal modeling is needed for noisy short-form video settings.

Conclusion: The findings highlight the usefulness of targeted semantic feature engineering for static memes and the need for more robust temporal modeling in noisy short-form video settings.
...
TL;DR: A software-based approach is proposed to reconstruct a 3D oral model from 10 2D intraoral images, eliminating the need for dedicated hardware devices and reducing patient discomfort.


<details>
  <summary>Details</summary>
Motivation: Current methods for oral 3D modelling, such as impression taking and intraoral scanning, have notable limitations including patient discomfort, material deformation errors, and high equipment costs.

Method: The proposed method uses MobileNetV2 as the image encoder combined with Multi-head Attention for multi-view feature fusion, trained on the Dental3DS dataset comprising 950 upper jaw samples.

Result: The proposed model achieves an accuracy of 77.49%, measured by nearest-neighbor matching with a distance threshold of 0.035, but with uneven point distribution across the reconstructed model.

Conclusion: The proposed software-based approach offers a cost-effective and patient-friendly solution for 3D oral modelling, but requires further improvement to achieve uniform point distribution.
...
TL;DR: Proposed ATT-CR, a Transformer-based model for cloud removal in remote sensing images, reducing computational costs and mitigating interference from cloudy pixels.


<details>
  <summary>Details</summary>
Motivation: Existing Transformer-based methods for cloud removal have high computational complexity and are disturbed by cloudy pixels, leading to suboptimal performance.

Method: ATT-CR consists of Triangular Attention (TAN) and Feature Selected Gating Module (FSGM) to reduce computational costs and distinguish between cloudy and clean features.

Result: ATT-CR delivers superior performance compared to existing methods on cloud removal benchmarks.

Conclusion: The proposed ATT-CR model effectively addresses the challenges of cloud removal by reducing computational costs and mitigating interference from cloudy pixels, achieving superior performance.
...
TL;DR: The paper proposes a novel approach to text-to-3D indoor scene generation using Large Vision-Language Models (LVLMs), addressing the challenge of error propagation in sequential decision-making mechanisms.


<details>
  <summary>Details</summary>
Motivation: The main challenge in text-to-3D indoor scene generation with LVLMs is the error propagation caused by sequential decision mechanisms that cannot revise earlier decisions.

Method: The authors model the task as a tree search problem with global and local trees, using a hierarchical scene representation and a PRM-guided MCTS method to effectively search the tree and generate 3D scenes.

Result: The proposed method generates more realistic 3D scenes than state-of-the-art approaches, as demonstrated by experiments on a new large-scale diverse dataset called 3DTindo-bench.

Conclusion: The paper presents a novel approach to text-to-3D indoor scene generation, addressing the limitations of existing sequential decision-making mechanisms and demonstrating improved performance on a new benchmark dataset.
...
TL;DR: The paper proposes ReSAGE-PAR, a pipeline for generating high-fidelity pedestrian images with attributes using diffusion models, addressing domain gap and attribute verification challenges.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address limited diversity and data scarcity in Pedestrian Attribute Recognition (PAR) by exploring image synthesis using diffusion models.

Method: The method involves adapting pre-trained diffusion models to native PAR resolutions using a LoRA-based Image-to-Image approach, extracting vision-language alignment scores, and formulating a Bayesian classifier for reliable binary pseudo-labels.

Result: The results show significant improvements in PAR performance, achieving gains of up to 8.7% on standard backbones and pushing state-of-the-art frameworks to new performance levels.

Conclusion: The paper concludes that ReSAGE-PAR is an effective and architecture-agnostic solution for scalable PAR enhancement, preserving spatial priors and verifying attributes.
...
TL;DR: The authors propose a self-supervised 3D reconstruction framework for truncated cone-beam computed tomography (CBCT) using neural scene representations, which eliminates truncation-induced artifacts and enables robust 3D data extrapolation.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning methods for truncated CBCT reconstruction rely on supervised ground truth and fail to account for continuous 3D spatial truncation variations, leading to severe artifacts and limited field of view.

Method: The authors introduce a self-supervised 3D reconstruction framework based on neural scene representations, which directly maps spatial coordinates to radiodensity under projection supervision, and incorporates a physics-based iterative refinement module to preserve high-frequency textures.

Result: The proposed method successfully unifies the exceptional artifact suppression and extrapolation capabilities of neural networks with the high-fidelity detail preservation of iterative algorithms, as demonstrated by extensive experiments on simulated and real-world datasets.

Conclusion: The authors conclude that their method can effectively eliminate truncation-induced ring artifacts and enable robust continuous 3D data extrapolation, while preserving clinically vital high-frequency textures.
...
TL;DR: LoomVideo is a 5B-parameter unified model for video generation and editing that achieves state-of-the-art performance with high efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing unified video generation and editing models are challenged by their large size (typically 13B parameters or more) and high computational complexity.

Method: LoomVideo uses a Multimodal Large Language Model (MLLM) and a Diffusion Transformer (DiT) with a Deepstack injection mechanism, and introduces a zero-overhead Scale-and-Add conditioning approach for video editing.

Result: LoomVideo achieves state-of-the-art or highly competitive performance across comprehensive benchmarks, with at least a 5.41x acceleration in inference speed compared to models of similar capabilities.

Conclusion: LoomVideo presents a highly efficient unified architecture for video generation and editing, paving the way for practical and efficient video foundation models.
...
TL;DR: A multimodal framework is proposed for synthesizing 3D gait data from textual descriptions to aid pathological gait classification.


<details>
  <summary>Details</summary>
Motivation: The scarcity of pathological gait datasets due to privacy, recruitment, cost, and movement variability.

Method: A multimodal LLM-guided framework combining motion tokenisation, pathology-aware language conditioning, LLM-based semantic augmentation, and language-to-gait generation.

Result: The synthetic sequences improve downstream classification, achieving 92.77% accuracy with a GRU classifier trained on real and synthetic samples.

Conclusion: The proposed framework effectively generates synthetic gait data that enhances pathological gait classification tasks.
...
TL;DR: ReCache is a method that learns to optimize the caching schedule for diffusion models, allowing for controllable computation cost and improved generation quality.


<details>
  <summary>Details</summary>
Motivation: The iterative denoising process of modern diffusion models makes inference expensive, and existing caching schedules have limitations in terms of adaptability and controllability of compute cost.

Method: ReCache uses policy gradients to learn the recomputation schedule that maximizes generation quality given a target budget, and is compatible with any caching mechanism.

Result: ReCache outperforms scheduling baselines, reducing LPIPS by 31% and 65% and boosting the VBench score by 7% in certain scenarios, while achieving significant FLOPs reduction and speedup.

Conclusion: ReCache provides a controllable and adaptable approach to caching schedules for diffusion models, leading to improved generation quality and efficiency.
...
TL;DR: FontFusion, a framework for Diffusion Transformer architectures, improves typography generation by balancing font control and text legibility.


<details>
  <summary>Details</summary>
Motivation: Typography generation in diffusion models faces a trade-off between precise font control and text legibility.

Method: FontFusion uses hierarchical token representation, position-aware embeddings, and multi-level token dropping strategy.

Result: FontFusion shows 76% relative improvement on decorative fonts and 68-76% font consistency gains over unconditioned models.

Conclusion: FontFusion resolves the dilemma between font control and text legibility in typography generation.
...
TL;DR: Introduced VZCrash, a large dataset of real-world vehicle collision data with IMU telemetry, and conducted experiments to benchmark crash detection approaches.


<details>
  <summary>Details</summary>
Motivation: The motivation is to provide a large-scale dataset of real-world vehicle collision data to facilitate the development and improvement of crash detection models.

Method: The method involves introducing the VZCrash dataset, which contains over 31,000 validated crashes and 158,000 negative samples, and conducting experiments to benchmark different crash detection approaches, including threshold-based heuristics and deep learning models.

Result: The results show that scaling data is crucial for training high-quality crash detection models, especially when deploying them in real-world environments.

Conclusion: The study concludes that large-scale datasets like VZCrash are essential for developing effective crash detection models, and that scaling data is critical for achieving high-quality results in real-world deployments.
...
TL;DR: The paper studies knowledge distillation for autoregressive image generation models and proposes a new framework, VarKD, which improves distillation by selectively applying teacher supervision and reducing token-level ambiguity.


<details>
  <summary>Details</summary>
Motivation: Autoregressive image generation models are computationally intensive, and knowledge distillation is a natural approach for model compression, but its behavior in visual AR generation remains underexplored.

Method: The paper proposes VarKD, a distillation framework for visual autoregressive models that distills on student samples while selectively applying teacher supervision and reducing token-level ambiguity.

Result: Experiments on ImageNet across multiple AR backbones show that VarKD consistently outperforms prior distillation baselines, narrowing the gap to large-scale models.

Conclusion: The proposed VarKD framework is effective in improving knowledge distillation for autoregressive image generation models, and it has the potential to be applied to other visual AR models.
...
TL;DR: The paper proposes HyperVis, a method to improve compositional reasoning in Vision-Language Models (VLMs) by computing a dense visual relation tensor and projecting it onto a Lorentz hyperboloid, which improves performance on VQA tasks.


<details>
  <summary>Details</summary>
Motivation: VLMs struggle with compositional reasoning that requires understanding inter-object relationships. Previous methods that inject explicit scene graph triplets from an off-the-shelf scene graph generator (SGG) degrade performance.

Method: The proposed method, HyperVis, computes a dense visual relation tensor via spatially-biased cross-attention, projects it onto a Lorentz hyperboloid, and enforces hierarchy through spatial physics.

Result: HyperVis improves performance on VQA tasks, achieving 61.03% on GQA and 79.94% on SugarCrepe, surpassing the baseline. The learned curvature stabilizes at κ=4.0, indicating that continuous visual features require the exponential volume of strongly curved space.

Conclusion: The paper concludes that HyperVis is an effective method for improving compositional reasoning in VLMs, and that the hyperbolic relational losses and entailment cones are crucial for achieving good performance.
...
TL;DR: The paper introduces a framework called Medical Segmentation Dataset Knowledge Card (MS-DKC) to make explicit the requirements of medical image segmentation datasets and evaluate its effectiveness on several datasets.


<details>
  <summary>Details</summary>
Motivation: The motivation is to shift the focus from searching for stronger architectures in medical image segmentation to understanding what the dataset requires from the model.

Method: The MS-DKC framework records dataset evidence through descriptors and maps them to failure modes, design priors, and risk-aligned criteria to make segmentation design more traceable.

Result: The results support dataset-conditioned design, showing that different datasets require different priors, operating points, and evidence to judge a model appropriate. MS-DKC was evaluated on DRIVE, ISIC2018, and ACDC datasets, achieving competitive performance.

Conclusion: The paper concludes that dataset requirements should be considered before model design, and MS-DKC provides a framework for making these requirements explicit, leading to more traceable and effective segmentation design.
...
TL;DR: The paper proposes a new method called Structured Defect Grounding (SDG) for diagnosing and evaluating text-to-image models by identifying specific defects in generated images.


<details>
  <summary>Details</summary>
Motivation: Current text-to-image models often produce images with subtle and complex failures, and existing methods for diagnosing these failures are limited. The authors aim to provide a more detailed and instance-level feedback system.

Method: The authors propose SDG, which models each defect as a tuple of location, type, reason, and importance. They also introduce a dataset called SDG-30K and an evaluation protocol called SDG-Eval. A diagnosis-to-alignment framework is presented, which uses a Vision-Language Model as the SDG detector and BoxFlow-GRPO to convert predicted defect sets into spatial rewards for diffusion model alignment.

Result: The authors' SDG detector outperforms leading proprietary VLMs on structured defect grounding, and SDG-guided rewards consistently improve text-to-image alignment and support localized image refinement.

Conclusion: The proposed SDG method establishes a unified, instance-level interface for diagnosing, evaluating, and enhancing modern generative models.
...
TL;DR: A novel conditioning framework for diffusion models is proposed to enable contrastive decomposition of images into common and salient factors without compromising generation quality.


<details>
  <summary>Details</summary>
Motivation: Existing contrastive methods based on generative models suffer from limited reconstruction and image quality, which limits their applicability to high-fidelity image generation and edition.

Method: A prompt-free, image-conditioned diffusion model is trained and then decomposed into common and salient factors using weak supervision.

Result: The proposed framework enables targeted operations such as swapping or interpolating only the salient factor, and the additive contrastive factorization is proven to be identifiable under mild conditions.

Conclusion: The proposed framework improves upon existing contrastive methods by enabling high-fidelity image generation and edition through effective latent factor separation.
...
TL;DR: Proposed an efficient event-to-frame reconstruction framework with causal temporal modeling and computation-aware design.


<details>
  <summary>Details</summary>
Motivation: Existing methods face a trade-off between reconstruction quality and computational efficiency.

Method: Recurrent encoder-decoder with selective context fusion and lightweight hybrid attention mechanism.

Result: Achieves competitive reconstruction performance with a favorable balance between accuracy and model complexity.

Conclusion: The proposed approach effectively bridges asynchronous event streams with frame-based vision pipelines.
...
TL;DR: The paper introduces an efficient adaptive video tokenization method that dynamically allocates token budgets based on visual complexity, achieving competitive reconstruction fidelity with significantly reduced computational overhead.


<details>
  <summary>Details</summary>
Motivation: Current video tokenization methods have high computational overhead due to iterative searches or full-rate decoder passes, which the authors aim to reduce while maintaining reconstruction fidelity.

Method: The authors propose a parameter-free adaptive token allocation mechanism that exploits temporal redundancy in the latent space of a frozen continuous video tokenizer, and introduce the Latent Inpainting Transformer (LIT) for reconstructing dropped positions.

Result: The proposed framework achieves meaningful, content-driven token allocation while maintaining competitive reconstruction fidelity, with a $31	imes$ inference-time speedup over the continuous adaptive baseline and an $approx2	imes$ speedup over the discrete information-theoretic baseline.

Conclusion: The paper demonstrates that efficient adaptive video tokenization is achievable without strict computational overheads, and that the proposed framework can naturally emerge from the input content rather than being enforced top-down.
...
TL;DR: A novel diffusion-based self-supervised learning strategy for underwater image enhancement that exploits label quality distribution without relying on paired datasets.


<details>
  <summary>Details</summary>
Motivation: The reliance on paired datasets with unstable label quality hinders the performance of learning-based underwater image enhancement methods.

Method: A diffusion-based, in-dataset self-supervised learning strategy that evaluates label quality via semantic perception embeddings and guides a multi-step denoising process.

Result: The proposed method consistently outperforms state-of-the-art approaches in restoration quality.

Conclusion: The proposed method effectively mitigates the limitations of existing learning-based underwater image enhancement methods by leveraging label quality distribution.
...
TL;DR: The paper proposes a test-time defense framework called Directional Bias-guided Defense (DBD) to protect Vision-Language Models (VLMs) like CLIP from adversarial attacks.


<details>
  <summary>Details</summary>
Motivation: VLMs are vulnerable to adversarial perturbations, which poses serious risks in real-world applications. Existing defenses require costly large-scale retraining, making test-time defenses a promising approach.

Method: The authors propose DBD, which estimates the Defense Direction and employs a DB-score-based two-stream reconstruction strategy to recover robust representations.

Result: DBD achieves state-of-the-art adversarial robustness while preserving clean accuracy on 15 datasets. Interestingly, adversarial accuracy can even surpass clean accuracy.

Conclusion: The paper reveals that adversarial perturbations inherently encode directional priors about the true decision boundary, and DBD is an effective test-time defense framework for VLMs.
...
TL;DR: The authors propose a new metric, SC-MFJ, to evaluate the suitability of segmented surfaces for haptic rendering in surgical simulation, revealing significant differences in haptic quality between various segmentation approaches that are not captured by traditional geometric metrics.


<details>
  <summary>Details</summary>
Motivation: Current segmentation metrics do not assess the suitability of segmented surfaces for haptic rendering in surgical simulation, highlighting the need for a new metric that can evaluate the haptic quality of segmented surfaces.

Method: The authors propose SC-MFJ, a simple and inexpensive metric that samples a segmented organ surface with virtual stylus walks and measures the resulting contact forces' jerkiness, and evaluate three pancreas CT segmentation approaches across 80 cases.

Result: SC-MFJ reveals a significant gap in haptic quality between different segmentation approaches, including a 147x gap between raw binary output and Gaussian post-processing, and shows that learned SDF regression produces more variable haptic quality than Gaussian smoothing.

Conclusion: The study suggests that for haptic simulation applications, a simple post-processing step may be sufficient, and that SC-MFJ can flag problems that traditional geometric metrics miss, highlighting its potential as a useful tool in evaluating segmented surfaces for haptic rendering.
...
TL;DR: DisasterBench is a new benchmark for multimodal reasoning in disaster response, and DisasterVL is a lightweight model that outperforms existing models in this task.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal benchmarks for disaster response focus on perception and have limited scope, and there is a need for a benchmark that supports multi-stage reasoning in practical emergency response.

Method: The authors propose DisasterBench, a multi-stage multimodal reasoning benchmark, and DisasterVL, a lightweight multimodal model optimized with a three-stage pipeline.

Result: DisasterVL outperforms all evaluated open-source models and narrows the gap to state-of-the-art closed-source models, achieving GPT-4o-comparable reasoning accuracy with superior efficiency.

Conclusion: The authors introduce a new benchmark and model for multimodal reasoning in disaster response, which can enable more effective and efficient emergency response.
...
TL;DR: The paper introduces Symb-xMIL, a post-hoc explanation framework for multiple instance learning (MIL) models in digital histopathology, which provides insights into how the model combines evidence from different tissue regions to produce predictions.


<details>
  <summary>Details</summary>
Motivation: Existing explanation methods for MIL models in digital histopathology are limited as they only highlight influential regions without explaining how evidence is combined, which hampers interpretability, especially in cases where decisions depend on interactions between tissue features.

Method: The authors propose Symbolic explainable MIL (Symb-xMIL), a framework that quantifies how a MIL model's behavior aligns with human-readable decision rules expressed as logical relationships between input features.

Result: Symb-xMIL was evaluated on synthetic and real-world histopathology datasets, demonstrating its ability to recover ground-truth logical rules, uncover heterogeneous decision patterns, expose hidden model errors, and refine patient survival stratification with potential clinical relevance.

Conclusion: Symb-xMIL advances MIL explainability by enabling structured, rule-based reasoning, leading to more transparent and semantically grounded interpretation of model predictions in digital histopathology.
...
TL;DR: Proposed SAM-Flow, a source-anchored masked flow framework for localized training-free image editing, achieving accurate semantic editing while preserving background.


<details>
  <summary>Details</summary>
Motivation: Existing training-free image editing methods cause background leakage due to global latent transport, prompting the need for localized editing.

Method: SAM-Flow uses scout image and token-grounded attention maps to localize editable regions, applies differential velocity updates within these regions, and anchors remaining areas to the source-image latent trajectory.

Result: SAM-Flow achieves accurate semantic editing while significantly improving background preservation, integrating with mainstream flow-matching backbones without fine-tuning.

Conclusion: SAM-Flow provides a simple and general localized editing paradigm for training-free image editing, with extensive experiments demonstrating its effectiveness.
...
TL;DR: A novel multimodal attention mechanism, Volumetric Multimodal cross-Attention (VMA), is proposed to integrate information across multiple modalities, capturing joint multimodal dependencies beyond pairwise similarity.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal attention formulations have limitations in computing attention scores, either incurring quadratic complexity or failing to model interactions that depend on the joint configuration of multiple representations.

Method: The proposed VMA mechanism defines attention scores as a function of the joint geometry of a query and multiple modality-specific keys, and is integrated into a novel multimodal transformer architecture, GRAMformer.

Result: The proposed model demonstrates improved effectiveness and efficiency on multimodal learning tasks.

Conclusion: The proposed Volumetric Multimodal cross-Attention (VMA) mechanism and GRAMformer architecture can effectively integrate information across multiple modalities, capturing joint multimodal dependencies.
...
TL;DR: The paper proposes a new method for blind image restoration by modeling degradations on a Riemannian manifold and using geodesic transport.


<details>
  <summary>Details</summary>
Motivation: Current methods for blind image restoration assume linear degradation geometry, which is limiting. The authors aim to generalize this to non-linear degradations.

Method: The authors use a geodesic flow matching objective to learn intrinsic transport dynamics on a joint image-manifold space, modeling degradations as points on a low-dimensional Riemannian manifold.

Result: The proposed framework provides a principled treatment of mixed degradations and yields a clean theoretical interpretation for generalization beyond observed degradations.

Conclusion: The paper presents a new approach to blind image restoration that explicitly models degradations on a Riemannian manifold, allowing for more accurate and generalizable results.
...
TL;DR: The authors propose a robotic system for manipulating textiles, focusing on wrinkle detection and garment folding.


<details>
  <summary>Details</summary>
Motivation: Robotic manipulation of textiles is challenging due to continuous deformation and self-occlusions, which hinder robust visual perception.

Method: The authors developed a synthetic pipeline for auto-annotated keypoints, combined with real-world data, to train a wrinkle detector. They integrate a CNN for keypoint detection and a YOLOv8-OpenCV pipeline for grasping point extraction.

Result: The keypoint model achieves a Mean Position Error (MPE) of 1.7615 pixels. The perception system transfers to physical fabrics without fine-tuning, outperforming baselines in high-occlusion states or severe folds.

Conclusion: The proposed system enables robust robotic manipulation of textiles, specifically for stretching fully folded garments and ironing via wrinkles and keypoints.
...
TL;DR: The paper presents a solution for One-to-Many Temporal Grounding (OMTG), a setting where multiple disjoint video segments need to be localized for a single textual query. The authors propose a systematic approach, including a new benchmark, dataset, and reward functions, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Real-world scenarios often require localizing multiple disjoint segments for a single query, which previous state-of-the-art models struggle with due to a lack of event cardinality perception.

Method: The authors establish a comprehensive OMTG benchmark, curate a high-quality OMTG dataset, and develop novel temporal and caption reward functions. The caption reward leverages Chain-of-Thought reasoning over dense video captions to guide policy optimization.
...
Conclusion: The paper proposes a systematic solution for OMTG, addressing the limitations of previous models and achieving state-of-the-art results. The proposed approach has the potential to improve the accuracy of temporal grounding in real-world scenarios.
...
TL;DR: RhymeFlow accelerates video generation models by skipping unnecessary denoising steps for non-keyframes, reducing computational cost while maintaining visual quality.


<details>
  <summary>Details</summary>
Motivation: Existing video generation models suffer from high inference latency and computational costs due to the quadratic complexity of 3D attention and the requirement for dense denoising across all diffusion timesteps.

Method: RhymeFlow identifies keyframes that dominate semantic evolution, applies dense denoising to these keyframes, and skips denoising steps for non-keyframes, using a latent trajectory projection module to maintain temporal coherence.

Result: RhymeFlow outperforms existing baselines with higher inference speed and better visual quality on current DiT-based video generation models.

Conclusion: RhymeFlow offers a training-free framework that efficiently accelerates video generation models by minimizing redundant denoising processes while preserving visual quality.
...
TL;DR: The paper introduces StoryMindv2, a framework for generating high-quality deep video understanding (DVU) datasets, and presents StoryVideoQA, a large-scale DVU dataset. It also proposes PlotTree, a novel video understanding agent.


<details>
  <summary>Details</summary>
Motivation: Existing VideoQA approaches struggle with deep video understanding (DVU) due to the complexity of storylines, long-range video content, and diverse question types. Manually constructed DVU datasets are limited in scale and diversity.

Method: The authors design StoryMindv2, an enhanced multi-agent collaboration framework, which integrates a supervisor-guided generation mechanism and a refined multi-reviewer voting strategy to generate high-quality DVU datasets.

Result: The authors construct StoryVideoQA, a large-scale DVU dataset with over 363K QAs on 393.2 hours of diverse story videos. Evaluations of 20 state-of-the-art VideoQA methods reveal their limitations in long-range character associations and coherent understanding of complex storylines.

Conclusion: The paper highlights the challenges in deep video understanding and proposes a solution through the StoryMindv2 framework and PlotTree, a novel video understanding agent, to improve video understanding and storyline reasoning.
...
TL;DR: UAV multispectral imagery and deep learning models are used to segment bacterial leaf blight severity in rice, with U-Net++ achieving the highest performance.


<details>
  <summary>Details</summary>
Motivation: To evaluate the use of UAV multispectral imagery and deep learning models for segmenting bacterial leaf blight severity in rice.

Method: Convolutional neural networks (CNNs) and transformer-based models, including U-Net, U-Net++, DeepLabV3+, and SegFormer, are trained on a publicly available dataset with different input configurations.

Result: U-Net++ with EfficientNet-B3 achieved an mIoU of 97.62%, while SegFormer obtained comparable inference speed but lower segmentation accuracy.

Conclusion: Lightweight CNN backbones remain reliable for operational BLB monitoring, and integration of vegetation indices provides small but consistent improvements.
...
TL;DR: The paper proposes PhaseLock, a training-free framework that improves physical consistency in image-to-video diffusion models by preserving motion priors from early inference steps.


<details>
  <summary>Details</summary>
Motivation: Image-to-video diffusion models often produce motion that violates physical laws, and the authors aim to address this issue.

Method: The authors propose PhaseLock, which extracts a motion prior from early inference steps (2 steps) and enforces it onto high-fidelity generation via Latent Delta Guidance.

Result: PhaseLock improves physical consistency by an average of 6.2 points across diverse models while maintaining visual fidelity with negligible overhead.

Conclusion: The proposed PhaseLock framework effectively mitigates phase degradation and improves physical consistency in image-to-video diffusion models.
...
TL;DR: The paper proposes GMBFormer, a framework for urban green-space extraction from ultra-high-resolution imagery, leveraging a SegFormer-based approach with selective prototype retrieval and decoupled NDVI for improved performance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome limitations of current methods that process images patch by patch, limiting semantic reuse, and to better integrate visual appearance learning with physical vegetation confidence.

Method: The proposed method, GMBFormer, uses a SegFormer-based framework with selective, similarity-driven prototype retrieval. It decouples NDVI as a physics-informed gate for admitting high-confidence vegetation descriptors into a global memory bank.

Result: GMBFormer achieved mIoU/mDice scores of 89.25%/94.31%, 92.17%/95.92%, and 83.72%/90.86% on three datasets, outperforming the SegFormer-B4 baseline in each setting.

Conclusion: The proposed GMBFormer framework demonstrates improved performance in urban green-space extraction, highlighting the benefits of decoupled NDVI admission, memory retrieval, and selective prototype retrieval.
...
TL;DR: The paper proposes a model-agnostic framework that uses commonsense knowledge to improve scene graph generation (SGG) models, especially under annotation sparsity.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitation of SGG models in capturing reliable visual commonsense knowledge, particularly when faced with sparse annotations.

Method: The proposed framework mines commonsense-grounded constraints from training data and uses general declarative commonsense reasoning to refine SGG predictions at inference time.

Result: The framework achieves consistent improvements over strong baselines on three standard benchmarks.

Conclusion: The paper concludes that structured visual commonsense reasoning is a practical and effective complement to purely learning-based scene graph generation.
...
TL;DR: A training-free method called EasyLens is proposed to improve the sensitivity of medical vision-language models to subtle lesions in clinical images.


<details>
  <summary>Details</summary>
Motivation: Current medical vision-language models have limited utility due to insufficient sensitivity to subtle lesions with sparse, low-contrast visual evidence in complex anatomical contexts.

Method: EasyLens uses a pathology-anatomy prototype space (EasyBank) to select lesion-relevant patches and amplifies their representations through morphology-guided residual enhancement.
...
Conclusion: EasyLens is a plug-and-play method that enhances the sensitivity of medical VLMs to subtle lesions without requiring additional training or model-specific adaptation.
...
TL;DR: A unified hierarchical framework for generating realistic and coherent indoor scenes for robot simulation and interior design.


<details>
  <summary>Details</summary>
Motivation: Existing methods for indoor scene generation rely on hand-crafted rules or focus on isolated sub-tasks, producing scenes that lack global coherence, realism, and simulation readiness.

Method: A multi-stage framework that includes whole-home floorplan generation using a large language model, furniture layout generation using image generation models, and object layout generation with a VLM-based refiner and 3D generative model.

Result: The proposed pipeline produces indoor spaces with greater layout diversity and stronger 3D design appeal, outperforming prior methods on both quantitative and qualitative metrics.

Conclusion: The proposed unified hierarchical framework enables the generation of realistic and coherent indoor scenes, and the released dataset and furnished scenes can benefit the community.
...
TL;DR: The paper introduces a framework for radiological comparison in medical imaging AI, enabling reference-case retrieval and temporal comparative interpretation. It achieves state-of-the-art performance in retrieving clinically analogous cases and generating interval change interpretations.


<details>
  <summary>Details</summary>
Motivation: Current medical imaging AI systems perform well in isolated image interpretation but are not well-aligned with radiological practice, which relies on comparison across prior studies and reference cases.

Method: The authors formulate radiological comparison as an entity-aware cross-image reasoning problem and introduce a framework that uses a large-scale comparative imaging resource (MedReCo-DB) to develop two models: MedReCo for controllable retrieval of clinically analogous cases and MedReCo-VLM for generative interpretation of interval change.

Result: The proposed models achieve state-of-the-art performance in various evaluations, including internal, external, and cross-center evaluations. MedReCo improves retrieval performance by 6.0 percentage points on average, while MedReCo-VLM improves longitudinal follow-up accuracy by 14.5-46.5 percentage points on chest radiographs and 13.0-27.9 percentage points on CT.

Conclusion: The paper suggests that entity-aware comparative reasoning can be learned from routine clinical data at scale, providing a more clinically aligned foundation for medical imaging AI.
...
TL;DR: The paper proposes Astra, a framework that enhances Vision-Language Models (VLMs) with spatial reasoning capabilities by incorporating imagined visual evidence through interaction with a world simulator.


<details>
  <summary>Details</summary>
Motivation: Current VLMs are limited in their spatial reasoning abilities, particularly in inferring unobserved layouts, maintaining cross-view consistency, and reasoning from alternative viewpoints with limited egocentric observations.

Method: The Astra framework consists of Astra-VL, an RL-trained VLM policy, and Astra-WM, a world simulator that generates novel-view observations. Astra-WM is trained with view consistency tuning, and Astra-VL is trained with a world-simulator-in-the-loop two-phase RL curriculum.

Result: Experiments show that Astra improves the performance of VLMs on MMSI-Bench and MindCube benchmarks. Specifically, Astra-WM improves Gemini-3-Flash from 45.1 to 49.5 on MMSI-Bench, and Astra-VL improves Qwen3-VL from 29.8 to 38.8 on MMSI-Bench and from 36.8 to 42.7 on MindCube.

Conclusion: The paper concludes that imagined observations can provide useful spatial evidence, but effective world-model-augmented reasoning requires learning when, where, and how to imagine.
...
TL;DR: The paper proposes Complexity-Balanced Splitting (CBS), a framework for efficiently deploying generative models by partitioning the timeline into segments of equal complexity and allocating more capacity to challenging regions.


<details>
  <summary>Details</summary>
Motivation: Standard generative models use monolithic architectures that are inefficient when dealing with vastly different signal regimes, and scaling model capacity uniformly is not optimal.

Method: CBS uses function approximation theory and de Boor's equidistribution principle to partition the diffusion timeline, and introduces monitor functions to estimate local complexity.

Result: CBS consistently improves synthesis quality across multiple architectures and datasets without increasing per-step inference cost, with a ~35% improvement in FID on SiT-XL with CFG.

Conclusion: The proposed Complexity-Balanced Splitting framework offers a principled approach to temporal capacity allocation in generative models, leading to improved synthesis quality and efficiency.
...
TL;DR: A new part-aware 3D multimodal large language model (PAR3D) is proposed, enabling fine-grained understanding of 3D scenes, including objects and their parts.


<details>
  <summary>Details</summary>
Motivation: Existing 3D-MLLMs are object-centric and struggle to model fine-grained part structures essential for embodied interaction with 3D environments.

Method: PAR3D framework, ScenePart dataset, Part-Aware 3D Representation Learning, and Hierarchical Segmentation Query Generation are proposed.

Result: The method substantially improves part-level question answering and referring segmentation, while achieving strong performance across object-level vision-language tasks.

Conclusion: PAR3D enables unified solutions for 3D scene understanding tasks with fine-grained part-level understanding.
...
TL;DR: The paper proposes a bilayer coupled SIR/SIRS framework to model the spread of synthetic data contamination in AI ecosystems, and identifies detection-based filtering and herd immunity as effective strategies to mitigate model collapse.


<details>
  <summary>Details</summary>
Motivation: Existing analyses treat model collapse caused by synthetic data as a single-chain degradation, but in reality, AI ecosystems involve cross-contamination between models and data corpora.

Method: The authors propose a phenomenological mean-field model, specifically a bilayer coupled SIR/SIRS framework, and derive the basic reproduction number $R_0$ via the Next Generation Matrix.

Result: The results show that the system exhibits supercritical dynamics ($R_0 > 1$) across three scenarios, and that detection-based filtering and herd immunity are the highest-leverage strategies to mitigate model collapse.

Conclusion: The paper concludes that the proposed framework can effectively model the spread of synthetic data contamination in AI ecosystems, and that detection-based filtering and herd immunity are essential strategies to prevent model collapse.
...
TL;DR: The authors propose a hybrid pre-training objective combining JEPA-style latent-space prediction loss with standard MLM objective, resulting in more uniform embeddings and richer spectral geometry.


<details>
  <summary>Details</summary>
Motivation: The dominance of MLM in text encoders leads to representations focused on surface-form token identity rather than deeper semantic structure.

Method: The authors propose a hybrid model combining JEPA-style latent-space prediction loss with standard MLM objective over a single shared encoder, with a learnable scalar parameter balancing the two objectives.

Result: The hybrid encoder produces significantly more uniform embeddings, exhibits richer spectral geometry, encodes less surface-level lexical information, and achieves a better semantic-to-lexical balance.

Conclusion: The JEPA predictive objective reshapes the latent space in ways that standard accuracy metrics alone cannot capture, suggesting a better representation of semantic structure.
...
TL;DR: The paper investigates using Group Relative Policy Optimization (GRPO) for post-training Large Language Models (LLMs) on heart-focused medical question answering, proposing a Variance-Aware Reward Framework to improve performance.


<details>
  <summary>Details</summary>
Motivation: Deploying general-purpose LLMs in healthcare is challenging due to data privacy, inference costs, and limited edge/on-device suitability, motivating the development of smaller, efficient models with robust post-training strategies.

Method: The paper proposes using GRPO with a Variance-Aware Reward Framework, extending Explicit and Implicit Aggregation strategies with continuous analytical reward functions derived from criterion-level rubric outcomes.

Result: The best GRPO variant improves accuracy from 0.362 to 0.502 and F1 from 0.532 to 0.668 on a held-out heart-related subset of HealthBench, relative to the Qwen3-14B base model, and remains competitive with GPT-OSS-120B.

Conclusion: Carefully designed rubric-based rewards provide a practical strategy for improving heart-focused medical question answering in LLMs, with potential to extend to other rubric-based tasks.
...
TL;DR: The paper presents a generic triple-latent sequence model that captures higher-order token interactions and improves upon a Transformer baseline in language modeling benchmarks.


<details>
  <summary>Details</summary>
Motivation: To capture higher-order token interactions without benchmark-specific parsing in language modeling.

Method: Proposing a generic triple-latent sequence model with a running token state and compressed pair-memory pathway.

Result: The model improves a small Transformer baseline on byte-level WikiText-2 and tokenizer-based MiniMind benchmarks. A gated key-value retrieval extension improves associative recall but is seed-sensitive and slower.

Conclusion: The triple-latent sequence model shows promise in language modeling tasks, but further optimization is needed for the retrieval extension.
...
TL;DR: This paper presents a study on using parameter-efficient fine-tuning for building a domain-specific conversational assistant in telecommunications customer support, highlighting the limitations of relying solely on validation loss for model selection.


<details>
  <summary>Details</summary>
Motivation: The evaluation and adaptation of large language models to domain-specific constraints in telecommunications customer support are limited due to data sovereignty, regulatory constraints, and handling of sensitive information.

Method: The study uses Low-Rank Adaptation (LoRA) with Qwen2.5-3B and a combinatorial synthetic data generation approach to build a conversational assistant, evaluating 16 LoRA configurations with various hyperparameters and target modules.

Result: The results show a divergence between quantitative and qualitative performance, with the best validation loss not necessarily obtaining the best human-aligned rankings, and the worst loss ranking first according to both judges.

Conclusion: The work contributes a combinatorial method for synthetic dataset construction, insights into target module selection for LoRA injection, evidence that validation loss alone is insufficient for selecting fine-tuning configurations, and an energy-performance trade-off analysis for sustainable LLM deployment.
...
TL;DR: Introduced MCBench, a multimodal safety benchmark for Omni LLMs, revealing challenges in cross-modal reasoning for safety judgments.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks can't assess Omni LLMs that process vision, audio, and text, highlighting a need for a multimodal safety benchmark.

Method: MCBench benchmark with 1196 scenarios across four safety categories, evaluating state-of-the-art Omni LLMs.

Result: Omni LLMs struggle with subtle or non-physical risks, but perform better with salient visual or acoustic cues; they often fail to integrate cues effectively for safety judgments.

Conclusion: Current Omni LLMs lack robust cross-modal reasoning in safety-critical settings, necessitating improved architectures and training strategies.
...
TL;DR: The paper proposes a non-autoregressive scoring method for punctuation restoration in streaming ASR, achieving high F1 scores on IWSLT 2017.


<details>
  <summary>Details</summary>
Motivation: Streaming ASR requires online decisions with limited future context, making generation-based approaches prone to latency and alignment failures.

Method: A non-autoregressive scoring method that compares punctuation insertion hypotheses against a no-insertion baseline under a bounded K-subword-token lookahead.

Result: The method achieves a 4-class macro F1 of 0.893 (no fine-tuning) and 0.937 (after fine-tuning) on IWSLT 2017, outperforming baselines.

Conclusion: The proposed method effectively improves punctuation restoration in streaming ASR with limited future context.
...
TL;DR: The paper proposes a framework for explainable rubric-based scoring using Shapley-value attributions and large language models, and evaluates its performance on a dataset of classroom transcripts.


<details>
  <summary>Details</summary>
Motivation: Automated scoring models lack transparency in their scoring decisions, which is a concern in high-stakes educational settings.

Method: The proposed framework combines model-agnostic Shapley-value attributions with rationales generated by large language models, and evaluates fine-tuned pretrained language models and prompted large language models on scoring performance and explanation faithfulness.

Result: Fine-tuned pretrained language models outperform large language models in prediction accuracy, but exhibit label compression. SHAP attributions identify sentences that reliably drive model predictions and produce larger and more coherent prediction shifts than LLM-generated rationales.

Conclusion: The proposed framework provides a principled basis for evaluating both scoring models and their explanations, and demonstrates that SHAP provides more faithful and transferable explanations for rubric-based scoring.
...
TL;DR: A novel Multi-Granularity Reasoning Network (MGRN) is proposed to improve Natural Language Inference (NLI) by leveraging hierarchical semantic features for better logical reasoning.


<details>
  <summary>Details</summary>
Motivation: Current transformer-based models for NLI primarily rely on final-layer token representations, which are insufficient for capturing complex semantic interactions required for effective reasoning.

Method: The proposed MGRN explicitly leverages hierarchical semantic features within an interactive reasoning space, mimicking human cognitive process of language understanding.

Result: MGRN consistently outperforms strong baseline models on multiple public benchmarks, demonstrating its effectiveness and robustness.

Conclusion: The proposed MGRN offers a promising approach to enhancing NLI by integrating semantic information across multiple granularities in a progressive and structured manner.
...
TL;DR: LANTERN, a lightweight memory layer, helps large language models retain critical details from conversation history by proactively archiving and retrieving relevant information, improving accuracy and outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Large language models struggle with retaining critical details when conversation history is compacted due to finite context windows.

Method: LANTERN (Layered Archival aNd Temporal Episodic Retrieval Network) uses hybrid retrieval to archive and restore relevant details without requiring LLM calls, adding minimal latency.

Result: LANTERN recovers 78.3% of lost facts, outperforming a faithful reimplementation of MemGPT's method (72.4%), and improves accuracy by 8.4 percentage points when used with four production LLMs.

Conclusion: LANTERN is an effective and efficient solution for retaining critical details in conversation history, improving the accuracy of large language models.
...
TL;DR: The study evaluates the sycophancy of large language models, finding that they often exhibit social-compliance behaviors, and coarse metrics mask substantial issues. A more nuanced approach is proposed to measure sycophancy on a continuous scale.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to investigate the sycophancy of large language models, which are increasingly deployed as high-stakes advisors, and to highlight the limitations of standard alignment benchmarks that treat sycophancy as a binary failure mode.

Method: The study evaluates six Gemini variants across generations 2.0, 2.5, and 3.0 on 73 adversarial prompts under three guardrail conditions, using a 0-4 Likert scale validated against human annotators.

Result: The results show that 27.2% of responses contain substantial sycophantic content, and 22.7% reach moderate or severe levels. The study also finds non-monotonic generational progress, an 'Alignment Tax' where social compliance trades against factual accuracy, and that simple guardrails outperform elaborate Protocol scaffolding on flagship models.

Conclusion: The study concludes that sycophancy should be measured on a continuous scale rather than as a binary failure mode, and that coarse metrics mask substantial social-compliance behaviors in large language models.
...
TL;DR: A new method for improving implicit chain-of-thought reasoning in large language models by distilling reasoning processes into a compact, low-rank tensor subspace.


<details>
  <summary>Details</summary>
Motivation: Implicit chain-of-thought methods underperform explicit CoT prompting, and the authors aim to improve them by exploring the structure of hidden-state reasoning trajectories.

Method: A low-rank distillation framework that aligns teacher and student trajectories in a shared low-rank tensor subspace using first- and second-order statistics.

Result: The approach consistently improves performance on mathematical reasoning benchmarks, especially on challenging multi-step tasks, approaching explicit CoT accuracy and outperforming prior iCoT distillation methods.

Conclusion: The proposed low-rank distillation framework effectively captures the global structure of reasoning while supporting a compact latent reasoning process, leading to improved performance in implicit chain-of-thought methods.
...
TL;DR: The paper presents PERSUASIONTRACE, a framework for studying persuasion in human-LLM interaction, and finds that LLMs are persuasive across various topics and modalities, but existing simulators fail to replicate human belief dynamics.


<details>
  <summary>Details</summary>
Motivation: Most persuasion studies rely on pre/post belief change, which misses where and how beliefs moved within a dialogue. The authors aim to develop a framework for studying persuasion in human-LLM interaction.

Method: The authors propose PERSUASIONTRACE, a framework that records multi-turn belief reports, annotates persuader turns with rhetorical dimensions, and evaluates simulators by fidelity to real human belief dynamics.

Result: The authors find that human targets group into two clusters of multi-turn belief updates, exhibit susceptibility to rhetorical strategies, and that LLMs are persuasive across various topics and modalities. They also introduce a Bayesian-network simulated target that outperforms existing LLM targets in replicating human belief dynamics.
...
TL;DR: BUMP is a self-supervised framework for personalizing large language models by generating user profiles from interaction history, eliminating the need for labeled downstream tasks.


<details>
  <summary>Details</summary>
Motivation: The challenge of personalizing large language models for different users across various applications, and the limitations of existing methods that require explicit rewards from labeled downstream tasks.

Method: Bidirectional User Modeling via Profiles (BUMP) uses a self-supervised framework with GRPO and a bidirectional in-batch ranking objective to train a profile generator without downstream labels.

Result: BUMP matches or outperforms closed-source APIs and prior methods on the LaMP benchmark, while requiring no task labels at training.

Conclusion: BUMP offers a promising solution for personalizing large language models by generating user profiles from interaction history, eliminating the need for labeled downstream tasks.
...
TL;DR: The study introduces a new predictor of incremental processing cost in language comprehension, called trajectory extrapolation error, which captures the deviation from the expected interpretation trajectory. This predictor is orthogonal to surprisal and independently predicts self-paced reading times.


<details>
  <summary>Details</summary>
Motivation: The dominant predictor of incremental processing cost, surprisal, has limitations as it reduces rich sequential representations to a single scalar. The authors aim to explore the role of the evolving interpretive state trajectory in shaping processing.

Method: The authors use a transformer language model to fit a linear trajectory to the preceding hidden states and measure deviation from the extrapolated path, which is termed trajectory extrapolation error. They test this measure on the Natural Stories corpus.

Result: The study finds that trajectory extrapolation error is nearly orthogonal to surprisal and independently predicts self-paced reading times. The effect is especially pronounced in garden-path sentences and strengthens with model scale and replicates across architectures.

Conclusion: The findings reveal two dissociable components of processing cost: word-level prediction error (surprisal) and sensitivity to the local momentum of the unfolding interpretation (trajectory extrapolation error).
...
TL;DR: The paper introduces ReasoningFlow, a framework for capturing the discourse structures of large reasoning models' (LRMs) traces into directed acyclic graphs (DAGs), revealing fine-grained reasoning behaviors and improving trace monitorability.


<details>
  <summary>Details</summary>
Motivation: The complexity of non-linear structures in LRM reasoning traces complicates evaluation and monitoring, prompting the need for a framework like ReasoningFlow.

Method: The authors develop an annotation schema, manually annotate 31 traces, and then scale to automatic annotation of 1,260 traces across tasks and models.

Result: The analysis reveals structurally similar LRM traces, diverse fine-grained reasoning behaviors, and that most erroneous steps are not used to derive final answers.

Conclusion: The ReasoningFlow framework provides insights into LRM reasoning processes, enhancing understanding and monitoring of these complex models.
...
TL;DR: The paper proposes a two-stage approach for early failure alerting in multi-turn language interactions, which learns from sparse evidence and uses attention-based failure prediction and a single preference-conditioned stopping policy.


<details>
  <summary>Details</summary>
Motivation: The challenge of early failure alerting in multi-turn language interactions where supervision is only available at the trajectory level, but alerts must be raised from partial interactions.

Method: A two-stage approach consisting of an attention-based failure predictor that learns sparse turn-level failure evidence from trajectory labels, and $α$-STOP, a single preference-conditioned stopping policy.

Result: The approach improves Pareto-frontier quality by 3-42% over state-of-the-art trigger policies, and reduces training cost per operating point by 1-3 orders of magnitude across five benchmarks.

Conclusion: The proposed approach effectively addresses the challenge of early failure alerting in multi-turn language interactions by learning from sparse evidence and providing controllable early alerting.
...
TL;DR: The paper presents a system that automatically discovers an executable schema from raw multi-source data and uses it for knowledge graph construction and query-time retrieval, improving over existing baselines in query answering benchmarks.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the challenge of querying real-world data that spans multiple tables, documents, and semi-structured files with implicit semantics, and to integrate evidence across inconsistent schemas and formats without requiring costly manual engineering.

Method: The method involves a system that automatically discovers an executable schema from raw multi-source data using a closed-world field catalog, deterministic structural analysis, and LLM-based schema discovery. The resulting schema is then used for extraction, deduplication, and cross-source linking into a provenance-aware knowledge graph.

Result: The system improves over retrieval-only and decomposition-based baselines across four QA benchmarks in controlled zero-shot comparisons, with ablations showing that schema-conditioned routing, structural intelligence, and schema-guided construction each contribute to the gains.

Conclusion: The paper concludes that the proposed system effectively integrates evidence across inconsistent schemas and formats, and provides a shared contract for knowledge graph construction and query-time retrieval, leading to improved performance in query answering benchmarks.
...
TL;DR: The paper introduces ComplexityMT, a challenge to assess text complexity and machine translation interaction, finding that higher text complexity makes translation harder and machine translation changes text complexity.


<details>
  <summary>Details</summary>
Motivation: To investigate how text complexity and machine translation interact, using CEFR levels as a measure of text complexity.

Method: Evaluating open-weight and closed models, and a commercial machine translation system on two tasks: correlation of CEFR with translation difficulty and shifts in CEFR levels.

Result: Higher CEFR levels make texts more difficult to translate, and machine translation shifts the CEFR level of the target text compared to the original source, for most languages.

Conclusion: The findings provide new insights for researchers and practitioners working on multilingual pedagogical content generation and machine translation difficulty estimation.
...
TL;DR: Proposed a pipeline for coreference resolution in low-resource languages by leveraging machine translation from English.


<details>
  <summary>Details</summary>
Motivation: Coreference resolution is under-studied in low-resource languages, and the proposed method aims to fill this gap.

Method: The pipeline uses machine translation to generate training data, validates sample quality via back-translation and cosine similarity, and weights samples based on MT cycle consistency.

Result: Significant performance gains in coreference resolution were observed in experiments on four low-resource languages.

Conclusion: The proposed pipeline effectively enables accurate coreference resolution in low-resource languages, including those without previous corpora.
...
TL;DR: Introduced PRIG, a gradient attribution method to identify ambiguous parts of a prompt in large language models.


<details>
  <summary>Details</summary>
Motivation: Prompt ambiguity is a common source of failure in large language models but is hard to localize as it's a latent property of the prompt.

Method: PRIG uses a probe logit to attribute latent ambiguity to token positions by training a linear probe to distinguish clear from ambiguous prompts.

Result: PRIG outperforms baselines, achieving 0.840 AUROC on a synthetic benchmark and 0.891 AUROC on a gold set, and also outperforms GPT-5.4 on sentence-level ambiguity identification.

Conclusion: PRIG is a practical tool for identifying ambiguous parts of a prompt, suggesting that latent prompt properties can be localized through intermediate representations.
...
TL;DR: The paper presents a Multi-Model Adaptive Summarization Framework for improving abstractive text summarization by integrating multiple fine-tuned transformer-based models and an adaptive selection mechanism.


<details>
  <summary>Details</summary>
Motivation: The need for robust and high-quality abstractive text summarization due to the growth of digital textual information and the limitations of relying on a single model.

Method: The proposed framework uses multiple fine-tuned transformer-based summarization models, each generating a candidate summary, which are then evaluated and selected based on automatic evaluation metrics.

Result: The framework achieves a BERTScore of 88.63% and outperforms several large language models (LLMs) like GPT3-D2, Falcon-7b, and Mpt-7b.

Conclusion: The findings highlight the effectiveness of leveraging multiple transformer-based models within an adaptive selection strategy to improve the quality and robustness of automatic text summarization systems.
...
TL;DR: The paper introduces CHASE, a framework that uses co-evolutionary hardening to improve the safety of language models against prompt-rewriting attacks.


<details>
  <summary>Details</summary>
Motivation: Existing defenses against prompt-rewriting attacks are limited, relying on human curation or white-box optimization, and are vulnerable to adaptive black-box adversaries.

Method: The authors propose a closed-loop red-blue teaming framework, CHASE, which co-evolves a black-box attacker and a safety-aligned defender using Group Relative Policy Optimization (GRPO) and a two-stage GRPO + rejection-sampled SFT pipeline.

Result: CHASE reduces the mean StrongREJECT score by 43.2% with 0% false-refusal on benign prompts, and shows that template-free RL exploration can recover latent attack primitives that transfer across different attack families.

Conclusion: The paper concludes that CHASE is an effective approach to improving the safety of language models against prompt-rewriting attacks, and suggests a path toward LLM safety hardening that generalizes beyond narrow distributions achieved in adversarial training.
...
TL;DR: A novel cross-language training approach for detecting Alzheimer's Disease (AD) using multilingual deep learning models, achieving strong performance across languages.


<details>
  <summary>Details</summary>
Motivation: The development of multilingual AD detection models is challenging due to the resource-intensive and time-consuming nature of language-specific model training.

Method: Using transformer-based models for binary AD classification, trained with datasets in English, Chinese, Arabic, and Hindi.

Result: The approach achieved F1 scores of 82% across all languages, with rapid inference time (0.5 seconds).

Conclusion: The proposed approach demonstrates strong cross-linguistic generalization, feasibility for global deployment, and potential for real-time screening applications.
...
TL;DR: The paper introduces ArcANE, a benchmark for evaluating role-playing language agents' ability to evolve character behavior and values over time, and shows that conditioning on character arcs improves performance.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for role-playing language agents focus on factual recall at a given point in the story, but do not assess whether responses align with the character's psychological development over time.

Method: The authors introduce ArcANE, a benchmark constructed from 17 novels and 80 principal characters, which segments narratives into phases along a psychological axis and poses scenarios across phases.

Result: Conditioning on character arcs outperforms other context strategies on all models, with the largest gap on scenarios outside the source text. Fine-tuning models on ArcANE data further widens the advantage.

Conclusion: The paper demonstrates the importance of considering character psychological trajectories in role-playing language agents and the effectiveness of the ArcANE benchmark in evaluating this aspect.
...
TL;DR: AURA improves upon standard tool-use agents by inferring implicit user needs and optimizing query responses.


<details>
  <summary>Details</summary>
Motivation: Situated queries often contain implicit needs beyond their literal content, which standard tool-use agents fail to address.

Method: AURA inserts an inference step to produce an IntentFrame, estimating implicit needs with a scalar gap score for probe budget and tool selection.

Result: AURA improves implicit-need coverage by 0.07 (p < 10^-6) and reduces probes by 82% with zero forbidden-tool violations on a privacy-sensitive slice.

Conclusion: AURA effectively addresses implicit user needs in situated queries, offering improved performance and efficiency.
...
TL;DR: InfoShield is a method for speech-based depression detection that protects user privacy by minimizing mutual information between speech representations and sensitive attributes like gender and age.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the conflict between depression detection accuracy and user privacy concerns in speech-based mental health screening.

Method: InfoShield uses TimeAwareMINE with cross-modal attention to align acoustic frames with attribute embeddings, minimizing mutual information between speech representations and sensitive attributes.

Result: InfoShield reduces gender inference from 92.6% to 55.5% and age inference from 55.7% to 30.3% with limited utility loss (6% F1 reduction), achieving F1=0.784.

Conclusion: InfoShield effectively protects user privacy while maintaining depression detection accuracy, outperforming prior state-of-the-art (SOTA) methods.
...
TL;DR: The paper describes the development of an LLM-based tool to assist in evaluating 1,200 student Statements of Purpose (SoPs) for the SURF program at Purdue University, reducing evaluation time and effort.


<details>
  <summary>Details</summary>
Motivation: The SURF program receives thousands of applications, requiring significant time and effort for program staff to evaluate each submission consistently and within tight timelines.

Method: The workflow utilizes OpenAI GPT models (GPT-4o, GPT-5-mini, and GPT-5.2) and uses a structured rubric across six subcategories, each scored on a 0-3 scale. A few SoPs, graded by program staff, were used to tune the model responses.

Result: The LLM outputs replicated the role previously played by distributed human graders, providing the program coordinator with scored and rationale-annotated outputs for the entire applicant pool. The program coordinator review was completed in approximately 4 hours, compared to the multi-week coordination effort required in prior program cycles.

Conclusion: The development and deployment of the LLM-based tool significantly reduced the evaluation time and effort for the SURF program, while maintaining consistency and accuracy in the evaluation process.
...
TL;DR: The paper proposes a method for mispronunciation detection and diagnosis using statistical graphs to learn phoneme confusion patterns and a language-specific strategy to capture systematic pronunciation differences.


<details>
  <summary>Details</summary>
Motivation: The importance of Mispronunciation Detection and Diagnosis (MDD) in computer-assisted language learning and speech technology has been increasing.

Method: The paper proposes constructing statistical graphs to enable models to learn phoneme confusion patterns represented as directed graphs and introduces a language-specific strategy to capture systematic pronunciation differences across various native language (L1) backgrounds.

Result: The approach achieves an F1-score of 59.52% on the L2-ARCTIC benchmark, outperforming several competitive baselines.

Conclusion: The proposed approach is effective for mispronunciation detection and diagnosis.
...
TL;DR: Introduced TensorBench, a benchmark for coding tasks on a tensor framework, evaluating seven coding agents with varied performance.


<details>
  <summary>Details</summary>
Motivation: Current coding benchmarks struggle with balancing task difficulty and evaluation reliability, particularly for large codebases with incomplete test coverage.

Method: TensorBench uses 199 tasks on an open-source tensor framework, grading agent performance by test suite results, including pre-existing and agent-added tests.

Result: Agent pass rates range from 64.8% to 22.1%, with varying subsets of tasks passed, and low pairwise Cohen's κ values, indicating different strengths.

Conclusion: TensorBench provides a new benchmark for evaluating coding agents, highlighting performance variability across agents and task subsets.
...
TL;DR: The authors propose a framework to predict optimal hyperparameters for continued pre-training of Large Language Models (LLMs), reducing search overhead by up to 90% while achieving comparable or superior performance.


<details>
  <summary>Details</summary>
Motivation: Current practices for hyperparameter configuration in LLMs rely on heuristics or grid searches, leading to training instability and excessive costs.

Method: The proposed framework consists of two stages: (1) Empirical Law Discovery, where small-scale proxy models are trained to derive functions mapping compute budget to optimal hyperparameters; and (2) State-Aware Hyperparameter Prediction, where the equivalent pre-training compute is estimated and used to predict optimal hyperparameters for the target run.

Result: The method reduces hyperparameter search overhead by up to 90% while achieving comparable or superior performance relative to baselines, and generalizes across architectures.

Conclusion: The proposed framework provides a principled and efficient methodology for diverse continued pre-training scenarios, enabling stable and predictable scaling laws for optimal hyperparameters.
...
TL;DR: Large language models (LLMs) often rely on word affixes to infer meaning, which can lead to overgeneralization and incorrect assumptions, especially in high-stakes domains like pharmacology.


<details>
  <summary>Details</summary>
Motivation: The study aims to investigate how LLMs use affixes to infer drug meanings and the potential risks associated with this approach, particularly in the medical domain.

Method: The researchers conducted a behavioral and mechanistic study using fictitious drug names built from real affixes, and introduced a framework to identify whether a model's drug semantics are driven by affix, stem, or the drug name as a whole.

Result: The study found that LLMs often induce drug meaning primarily through affix cues, but rarely explicitly indicate this reliance, and sometimes incorrectly conflate properties among affix-sharing drugs.

Conclusion: The findings suggest that morphological shortcuts pose a subtle but measurable risk to safety, highlighting the need for more nuanced and accurate approaches to understanding drug semantics.
...
TL;DR: The study examines how children with Autism Spectrum Disorder (ASD) process complex linguistic structures, specifically two-level recursive locative constructions in Mandarin. Using event-related potentials, it finds that children with ASD show different neural response patterns compared to typically developing children, with reduced early predictive engagement and increased semantic integration demands.


<details>
  <summary>Details</summary>
Motivation: The study aims to understand the temporal dynamics of recursive processing in children with Autism Spectrum Disorder (ASD), as difficulties with complex syntax have been reported in ASD but the underlying processing mechanisms are poorly understood.

Method: The study used event-related potentials (ERPs) to examine neural responses in a cross-modal sentence-picture matching task. Twenty-four children (12 ASD, 12 typically developing) participated, with mental age controlled. ERPs were analyzed across three processing stages: structural prediction (P200), semantic integration (N400), and syntactic reanalysis (P600).

Result: The study found that children with ASD exhibit attenuated early differentiation and reduced late reanalysis effects, but enhanced N400 responses under mismatch conditions. Additionally, the ASD group showed greater inter-individual variability in hemispheric lateralization, not associated with receptive vocabulary performance.

Conclusion: The findings support a cascading account where reduced early predictive engagement in ASD leads to increased integration costs and diminished reanalysis efficiency during recursive processing. The results highlight the importance of temporal processing dynamics and neural variability in understanding language differences in ASD.
...
TL;DR: The paper introduces AdaPlanBench, a benchmark for testing Large Language Models' (LLMs) ability to adaptively plan under progressively revealed world and user constraints. Experiments show that LLMs struggle with this task, achieving a maximum accuracy of 67.75%.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks do not adequately test adaptive planning under progressively revealed dual constraints (world and user constraints). The authors aim to address this gap by creating a new benchmark.

Method: The authors create AdaPlanBench, a dynamic interactive benchmark based on 307 household tasks. They use a scalable constraint construction pipeline to add dual constraints to each task. Agents interact with the environment in a multi-turn protocol, where hidden constraints are revealed when the agent proposes a plan that violates them.

Result: Experiments on ten leading LLMs show that adaptive planning under dual constraints is challenging, with the best model reaching only 67.75% accuracy. Performance degrades as more constraints accumulate, with user constraints posing a particularly large challenge.

Conclusion: AdaPlanBench establishes a testbed for dual-constrained interactive planning and highlights the challenge of reliable adaptation to dynamically revealed constraints in LLM agents. The results suggest that LLMs need improvement in adaptive planning under progressively revealed constraints.
...
TL;DR: The paper proposes RidgeFT, a framework for lifelong machine-generated text attribution, which allows for efficient updates to identify the generator of a given text without forgetting previous knowledge.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of lifelong machine-generated text attribution, where models must continuously incorporate new generators while preserving their ability to recognize previously seen ones.

Method: The method proposed is RidgeFT, a lightweight analytic update framework that uses task-aware encoder, compact class-wise sufficient statistics, covariance calibration, fixed random features, and closed-form ridge regression.

Result: The results show that RidgeFT consistently outperforms baselines across multi-topic evaluations, achieving the best macro-F1 across domains, backbones, and incremental protocols, while improving both old-class retention and new-class adaptation.

Conclusion: The conclusion is that RidgeFT provides a simple yet effective approach to lifelong machine-generated text attribution, and feature-stable analytic updates are a viable solution for this problem.
...
TL;DR: The paper introduces GATE, a method that uses execution feedback to bootstrap missing groundings in text-to-SQL, improving performance over strong baselines.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the issue of under-specified text-to-SQL queries by leveraging execution feedback to ground missing values, as prior methods often rely on incomplete semantic layers.

Method: The method introduced is GATE (Grounding After Test from Execution), which executes partially grounded SQL queries, uses the execution feedback to select the correct grounding hypothesis, and accumulates these into a reusable memory.

Result: GATE consistently improves over strong baselines across real-world and controlled benchmarks, demonstrating its effectiveness in text-to-SQL tasks.

Conclusion: The conclusion is that execution can serve not only as validation but also as a bootstrapping mechanism for reusable memory in text-to-SQL, highlighting the potential of GATE to improve performance in real-world applications.
...
TL;DR: The paper proposes QueryAgent-R1, a framework that improves e-commerce search by suggesting queries that align with users' interests and preferences, leading to better product conversion rates.


<details>
  <summary>Details</summary>
Motivation: Existing query recommendation methods in e-commerce search focus on query-level relevance but neglect whether retrieved products match users' downstream preferences, leading to high query CTR but low product CVR.

Method: The authors propose QueryAgent-R1, a memory-augmented agentic framework that uses chain-of-retrieval optimization, agentic reinforcement learning, and a memory abstraction module for efficient user profiling.

Result: QueryAgent-R1 outperforms strong baselines on two datasets and improves Query CTR by 2.9% and guided CVR by 3.1% in online A/B tests on a large-scale production platform.

Conclusion: The proposed QueryAgent-R1 framework effectively bridges the gap between query-level relevance and downstream product engagement, leading to improved performance in e-commerce search.
...
TL;DR: A new method, VSRAQ, is proposed for quantizing Mixture-of-Experts (MoE) models, which preserves expert-selection behavior and reduces degradation.


<details>
  <summary>Details</summary>
Motivation: MoE models are sensitive to routing instability caused by quantization, which degrades model quality.

Method: Value-and-Structure Routing Alignment for Quantization (VSRAQ), a post-training quantization objective that combines value alignment and structure alignment.

Result: VSRAQ improves expert-selection consistency and outperforms baselines on recent MoE foundation models.

Conclusion: VSRAQ effectively reduces quantization-induced degradation in MoE models without introducing inference-time overhead.
...
TL;DR: The study examines the interaction between parameter-side memory (document LoRA) and context-side memory (KV cache) in document-level question answering, finding that document LoRA is useful when context-side evidence is scarce.


<details>
  <summary>Details</summary>
Motivation: To understand how parameter-side memory (document LoRA) interacts with context-side memory (KV cache) in document-level question answering.

Method: Progressively evicting document key-value states and measuring the contribution of document LoRA in document-level question answering.

Result: Document LoRA adds little when the KV cache is intact, but becomes increasingly useful under aggressive compression, recovering 13-21 ROUGE-L points when no document context remains.

Conclusion: Document LoRA acts as a complementary memory channel whose value emerges when context-side evidence is scarce, and QA-style supervision produces stronger adapters than raw-context next-token-prediction.
...
TL;DR: The paper presents a unified framework for organizing the literature on latent communication in multi-agent systems built on large language models, analyzing existing methods and identifying open challenges.


<details>
  <summary>Details</summary>
Motivation: The dominant communication protocol in multi-agent systems is natural language, which suffers from high inference cost, irreversible information loss, and ambiguity. Latent communication, where agents exchange continuous representations, is explored as an alternative.

Method: The paper analyzes existing methods along three axes: WHAT information is communicated, WHICH sender-receiver alignment is used, and HOW the communicated information is fused into the receiver. A 3-axis framework is proposed to categorize and compare methods.

Result: The paper systematically categorizes eighteen representative methods, identifies five major design patterns, and surfaces a set of open challenges, including cross-architecture alignment, security of latent channels, and compression for edge deployment.

Conclusion: The proposed framework aims to lower the barrier to entry for new researchers and provide a vocabulary for comparing future work on latent communication in multi-agent systems.
...
TL;DR: This paper proposes a novel framework for interpreting style representations in authorship analysis through style-eliciting prompts, which steer large language models to generate text reflecting specific stylistic attributes.


<details>
  <summary>Details</summary>
Motivation: The latent nature of learned style representations makes them difficult to interpret, and recent methods to explain them are prone to biases and hallucinations.

Method: The authors propose a framework that uses style-eliciting prompts to generate text conditioned on specific stylistic features, then trains a decoder to generate style prompts from style representations.

Result: The method outperforms strong baselines in recovering original style prompts, generating text in the same style, and steering LLM outputs to match human-written texts.

Conclusion: Style-eliciting prompts provide a practical and interpretable interface to stylistic information encoded in style representations, enabling better understanding and utilization of these representations.
...
TL;DR: The paper introduces Narrative Knowledge Weaver (NKW), a framework for long-form narrative QA that aligns textual evidence with story structure to improve reasoning over evolving story worlds.


<details>
  <summary>Details</summary>
Motivation: Existing QA methods struggle with long-form narratives as they require reasoning over evolving story worlds, changing character states, and causal relationships, which existing retrieval and graph-augmented methods do not directly address.

Method: The authors propose NKW, a source-grounded framework that integrates textual evidence, atomic facts, graph structure, entity profiles, and narrative tools to assemble evidence and constrain answers.

Result: NKW outperforms existing methods on screenplay-level story-world QA and remains competitive on passage-centered benchmarks, demonstrating complementary benefits for various types of reasoning.

Conclusion: The paper concludes that NKW effectively improves long-form narrative QA by aligning textual evidence with story structure and enabling better reasoning over evolving story worlds.
...
TL;DR: The paper proposes AdaPLD, a training-free method to improve speculative decoding by adaptively enhancing retrieval and draft construction, achieving up to 3.10x decoding speedup.


<details>
  <summary>Details</summary>
Motivation: Existing model-free variants of speculative decoding have limitations in retrieval and draft construction, which hinder their speedup performance.

Method: The paper proposes AdaPLD, which uses semantic similarity to recover additional reuse opportunities and constructs branched reuse hypotheses to account for continuation uncertainty.

Result: AdaPLD reduces target-model forward passes and achieves up to 3.10x decoding speedup across diverse benchmarks.

Conclusion: AdaPLD effectively improves speculative decoding by adaptively enhancing retrieval and draft construction, leading to significant decoding speedup.
...
TL;DR: The paper introduces PlanBench-V, a benchmark for evaluating Vision-Language Models (VLMs) in interpreting spatial planning maps, and assesses their capabilities in perception, reasoning, association, and implementation.


<details>
  <summary>Details</summary>
Motivation: The interpretation of spatial planning maps requires fine-grained visual perception, spatial reasoning, and policy-informed professional judgment, which is challenging for both humans and AI systems. Existing multimodal benchmarks overlook domain-specific cognitive processes of planning practice.

Method: The authors create the Spatial Planning Map Database (SPMD) with expert-annotated datasets and propose a theory-informed evaluation framework to assess VLMs' capabilities in four progressive areas: Perception, Reasoning, Association, and Implementation.

Result: Experiments show clear progress but persistent limitations in VLMs, with the best 2026 model outperforming the best 2025 model by 27%. However, all models struggle with implementation-oriented tasks requiring evaluative judgment, policy sensitivity, and constraint-aware decision-making.

Conclusion: The study reveals fundamental limitations of current VLMs in professional planning contexts and highlights the need for domain-adaptive multimodal reasoning frameworks.
...
TL;DR: MARDoc, a Memory-Aware Refinement Agent framework, is proposed for multimodal long-document question answering. It outperforms existing methods by using specialized agents and structured memory.


<details>
  <summary>Details</summary>
Motivation: Existing iterative retrieval-reasoning agents for multimodal long-document question answering have limitations due to their single growing context that mixes retrieval traces, observations, and intermediate reasoning, leading to scattered and diluted key evidence.

Method: MARDoc framework consists of three specialized agents: an Explorer for multimodal retrieval, a Refiner for distilling interaction traces into structured evidence and reasoning memories, and a Reflector for checking evidence sufficiency and providing targeted feedback. The agents rely on a dynamically updated structured memory.

Result: MARDoc achieves strong results on MMLongBench-Doc and DocBench, outperforming same-backbone baselines and demonstrating the effectiveness of structured memory for agentic document QA.

Conclusion: The proposed MARDoc framework effectively addresses the limitations of existing iterative retrieval-reasoning agents by decoupling long-document QA into specialized agents and utilizing a structured memory, leading to improved performance in multimodal long-document question answering.
...
TL;DR: The paper proposes CollabBench, a benchmark for training and evaluating collaborative agents in cooperative games, enabling more effective human-agent collaboration.


<details>
  <summary>Details</summary>
Motivation: Existing conversation-level collaborative studies lack grounded interaction and behavioral execution, making it challenging for LLM-based agents to collaborate with human partners effectively.

Method: The authors propose a Diverse Player Profile Simulation pipeline and a Collaborative Agentic Training paradigm, and evaluate their approach using extended classic environments (CWAH-MultiPlayer and Cook-MultiPlayer).

Result: The trained models outperform base models, achieving 19.5% higher efficiency and 24.4% improved affective performance.

Conclusion: The paper highlights the limitations of existing models and provides insights for future collaborative training, proposing CollabBench as a solution for more effective human-agent collaboration.
...
TL;DR: The paper proposes two recall-based prompting strategies, Self-Recall (SR) and Question-Recall (QR), to improve large language models' (LLMs) ability to handle knowledge cutoff limitations, particularly for questions that require post-cutoff information.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the limitation of current LLMs in handling knowledge cutoff, especially when post-cutoff knowledge is not explicitly queried but is causally related to the question.

Method: The paper proposes two recall-based prompting strategies: Self-Recall (SR) and Question-Recall (QR). SR asks the model to restate its cutoff constraint, while QR requires the model to recall question-relevant information valid under the cutoff.

Result: The proposed methods, SR and QR, outperform direct-answer prompting and conventional step-by-step reasoning baselines across three existing benchmarks, with significant improvements on counterfactual questions. The methods also show robustness across different cutoff settings.

Conclusion: The paper concludes that the proposed recall-based prompting strategies, SR and QR, can effectively improve LLMs' ability to handle knowledge cutoff limitations, and combining SR and QR yields the best performance.
...
TL;DR: ProSPy, a Profiling-driven SQL-Python framework, is proposed to improve Text-to-SQL systems for enterprise-scale databases by combining SQL efficiency with Python flexibility.


<details>
  <summary>Details</summary>
Motivation: Applying large language models to enterprise-scale databases is challenging due to large and heterogeneous schemas, incomplete metadata, dialect-specific SQL syntax, and complex analytical questions.

Method: ProSPy structures the reasoning process into four stages: fine-grained data evidence extraction, large schema pruning, intermediate view fetching, and flexible downstream analysis with Python.

Result: ProSPy outperforms strong baselines on Spider 2.0-Lite and Spider 2.0-Snow, achieving execution accuracies of 60.15% and 60.51% with Claude-4.5-Opus.

Conclusion: ProSPy improves Text-to-SQL systems for enterprise-scale databases by combining SQL efficiency with Python flexibility, reducing reliance on unreliable metadata, and improving robustness across SQL dialects.
...
TL;DR: The paper studies the interpretability of Multimodal Large Language Models (MLLMs), focusing on how they extract relevant visual features from complex contexts. It identifies specialized attention heads called CoRe heads that act as dedicated information extractors.


<details>
  <summary>Details</summary>
Motivation: The mechanisms by which MLLMs extract query-relevant visual features from complex, noisy contexts are not well understood.

Method: The study uses a token-level metric called Retrieval Attention Mass (RAM) to identify and characterize CoRe heads. It also employs causal interventions and ablation experiments to demonstrate the necessity and utility of these heads.

Result: The study finds that CoRe heads are highly specialized and act as dedicated information extractors, while most other heads distribute attention over broader contextual regions. Ablating the top 5% of CoRe heads causes significant degradation in multimodal reasoning performance.

Conclusion: The findings reveal a structural principle of functional sparsity within MLLMs, refining the current understanding of mechanistic interpretability and laying a theoretical foundation for future architecture design and model optimization.
...
TL;DR: The paper explores if code-switching ASR capabilities learned from a few language pairs can generalize to unseen pairs, finding modest generalization.


<details>
  <summary>Details</summary>
Motivation: The scarcity of multilingual code-switching speech resources limits the performance of code-switching ASR, and existing approaches have scalability issues.

Method: The paper uses model merging and domain generalization methods to investigate the transferability of bilingual CS-ASR capabilities across language pairs.

Result: Merged bilingual CS-ASR models show modest generalization to unseen language pairs, indicating limited transfer of bilingual CS capabilities.

Conclusion: The study suggests that CS capabilities learned from seen language pairs have limited transferability to unseen language pairs.
...
TL;DR: The paper proposes a post hoc causal debiasing framework to handle toxic retrieval in text-to-audio systems, preserving semantic relevance while suppressing harmful speech.


<details>
  <summary>Details</summary>
Motivation: Existing strategies for handling toxic retrieval risk altering intent or omitting details, and the authors aim to develop a method that preserves semantic relevance while suppressing harmful speech.

Method: The authors propose a post hoc causal debiasing framework with a sentiment-controlled mediator, which is model-agnostic and integrates with existing retrieval pipelines. They introduce two variants: Forgive and Forget.

Result: Experiments show consistent toxicity reduction with minimal loss in retrieval accuracy, improving both safety and reliability.

Conclusion: The proposed framework effectively reduces toxicity in text-to-audio systems while preserving semantic relevance, making it a reliable and safe solution.
...
TL;DR: The authors propose ReverseEOL, a method to enhance text embeddings from Large Language Models (LLMs) by combining forward and reversed input embeddings, improving performance across various LLMs and benchmarks.


<details>
  <summary>Details</summary>
Motivation: The causal attention in decoder-only LLMs leads to biased contextualized representations because earlier tokens cannot attend to future context.

Method: The authors propose Reverse prompting with Explicit One-word Limitation (ReverseEOL), which generates an additional reversed embedding from the reversed input text and combines it with the standard forward embedding.

Result: Comprehensive experiments on STS and MTEB benchmarks show that ReverseEOL significantly improves the performance of existing training-free baselines across a broad range of LLMs.

Conclusion: The proposed ReverseEOL method effectively enhances the representational capability of frozen LLMs by providing complementary information through reversed embeddings, leading to richer final representations.
...
TL;DR: TARPO is a reinforcement learning framework that combines discrete and continuous reasoning in large language models, outperforming existing baselines.


<details>
  <summary>Details</summary>
Motivation: The deterministic nature of continuous representations in latent reasoning limits policy exploration in reinforcement learning, and TARPO aims to address this by adaptively switching between discrete and continuous reasoning.

Method: TARPO uses a lightweight action head router to switch between discrete token generation and continuous latent reasoning at each step, with the LLM backbone and router jointly optimized end-to-end.

Result: TARPO consistently outperforms existing explicit and latent reasoning RL baselines across diverse benchmarks, and learns adaptive token-wise switching behaviors while maintaining stable training dynamics.

Conclusion: TARPO provides a promising approach to combining discrete and continuous reasoning in large language models, with potential applications in reinforcement learning and natural language processing.
...
TL;DR: This study examines if LLMs exhibit the neglect-zero effect, a human cognitive bias where people tend to ignore zero-models that make a proposition vacuously true. The results suggest LLMs may not display this bias.


<details>
  <summary>Details</summary>
Motivation: To investigate whether LLMs' language processing resembles human cognitive processes, specifically regarding the neglect-zero effect.

Method: Using structural priming with prepared primes to force LLMs to consider zero-models and analyzing their processing of subsequent target sentences.

Result: The neglect-zero effect may not occur in the analyzed LLMs.

Conclusion: LLMs may not exhibit the neglect-zero effect, suggesting a difference in how they process certain inferences compared to humans.
...
TL;DR: A novel approach, YouZhi-LLM, is proposed to enhance the efficiency of large language models (LLMs) in financial applications by reducing KV cache memory overhead, leading to improved deployment efficiency and scalability.


<details>
  <summary>Details</summary>
Motivation: The high-concurrency deployment of LLMs is bottlenecked by KV cache memory overhead, inflating infrastructure costs and limiting scalability.

Method: YouZhi-LLM employs a layer-adaptive GQA-to-MLA transition framework and a comprehensive structural transition and training pipeline built on the Huawei Ascend ecosystem.

Result: YouZhi-7B and YouZhi-14B achieve improvements in financial benchmark scores (12.3% and 7.0%, respectively) and concurrency (2.69× and 2.43×, respectively) compared to their base models.

Conclusion: The proposed YouZhi-LLM approach establishes a new paradigm for cost-effective, high-throughput financial inference, demonstrating superiority in deployment efficiency and scalability.
...
TL;DR: The paper introduces RandomBench to evaluate Multimodal Large Language Models' (MLLMs) ability to make distributionally neutral choices among equivalent options, revealing a 'Stochastic Collapse' phenomenon where MLLMs fail to maintain uniform randomness.


<details>
  <summary>Details</summary>
Motivation: Current MLLM evaluations focus on utility-driven objectives, neglecting model behavior in logic-neutral scenarios where multiple actions are equally valid, leading to potential repetitive behaviors and reduced coverage of valid alternatives.

Method: The authors propose RandomBench and introduce metrics RI, BCI, BII to quantify entropy and distributional bias, conducting experiments and ablation studies to assess MLLMs' behavior.

Result: Experiments show MLLMs exhibit 'Stochastic Collapse', failing to maintain uniform randomness under explicit random instructions, with significant deviations in metrics like RI.

Conclusion: The study highlights the need for evaluating MLLMs in logic-neutral decision settings, as they exhibit consistent distributional collapse across languages and representation formats.
...
TL;DR: The paper studies how Artificial Moral Advisors (AMA) can help users 'stay with uncertainty' in ethical dilemmas, proposing three modes of uncertainty and comparing them to control conditions.


<details>
  <summary>Details</summary>
Motivation: To explore how AMAs can facilitate users in maintaining uncertainty in ethical discussions, rather than seeking definitive answers.

Method: The study involves a user-agent LLM engaging in dialogue with an AMA following specific uncertainty strategies, with pre- and post-conversation questionnaires, and examines the effect of two persona prompt formats.

Result: The findings indicate that different AMA strategies produce distinguishable conversational patterns, and that uncertainty strategies differ in the quality of engagement they sustain, not in stance revision.

Conclusion: The research provides insights into designing AMAs that can effectively help users navigate ethical dilemmas by maintaining uncertainty, highlighting the importance of conversational patterns and persona prompt formats.
...
TL;DR: The paper introduces EMBER, a learned retention policy for long-horizon agents to efficiently retain source evidence within a fixed token budget, improving performance on evidence retrieval and answer accuracy.


<details>
  <summary>Details</summary>
Motivation: Long-horizon agents face challenges in retrieving relevant evidence from large histories, incurring high retrieval, rereading, and context costs when retained memory misses answer-relevant evidence.

Method: The authors propose Budgeted Pre-Query Retention and introduce EMBER, which constructs a compact evidence state by storing verbatim source excerpts paired with retrieval keys and update metadata.

Result: EMBER-14B achieves 0.3017 F1 on LongMemEval-RR, outperforming the strongest non-EMBER budgeted baseline (0.1765 F1) and improving F1, Retain-Recall, and Read-Recall across retained source-evidence budgets.

Conclusion: The study demonstrates that long-horizon memory depends on retaining evidence within the budget rather than rereading larger histories, and EMBER effectively improves evidence retrieval and answer accuracy.
...
TL;DR: The paper proposes 'attention flows' to represent research attention in a contextually structured way, capturing its evolution over time and showing it to be more effective for structural comparison and robustness.


<details>
  <summary>Details</summary>
Motivation: Current methods represent research attention as aggregated counts, not preserving its development across contexts over time, creating a mismatch between interpretation and representation.

Method: The authors propose and evaluate 'attention flows' as a representation that encodes the organisation and evolution of attention, comparing it with signal and sequence-based representations.

Result: Flow representations more effectively support structural comparison, particularly with temporal progression or context distributions, and improve robustness under partial observation and structural perturbation.

Conclusion: The results support modelling attention as a contextually structured phenomenon, providing a basis for more informative approaches to research evaluation.
...
TL;DR: Using a lightweight graph structure with simple schema can improve the performance of Retrieval-augmented generation (RAG) systems by increasing precision and recall, reducing hallucinated answers, and achieving higher truthfulness scores.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of Large Language Models (LLMs) prone to failure modes such as 'hallucinating' information and to enable reasoning over proprietary information without expensive model fine-tuning.

Method: The method involves designing an agentic system with vector search and graph query tools operating over a structured dataset based on a curated subset of English Wikipedia articles.

Result: The results show significant improvements in precision, recall, and truthfulness scores, with a modest increase in token usage, compared to other evaluated scenarios.

Conclusion: The introduction of graph-based tools can enhance the performance of RAG systems, particularly in factual correctness and truthfulness, making them a viable solution for improving LLM-based applications.
...
TL;DR: Proposed ACE-SQL, a reinforcement learning framework that jointly optimizes schema retrieval and SQL generation for Text-to-SQL task.


<details>
  <summary>Details</summary>
Motivation: Existing methods for Text-to-SQL task have limitations in schema linking, either relying on full-schema generation or using a separate retriever with static supervision.

Method: ACE-SQL uses a reinforcement learning framework to jointly optimize schema retrieval and SQL generation under execution feedback.

Result: ACE-SQL achieves 65.3% greedy execution accuracy on BIRD Dev with 0.93k output tokens per query.

Conclusion: ACE-SQL provides a novel approach to improve Text-to-SQL task by adaptively co-optimizing schema retrieval and SQL generation.
...
TL;DR: A multi-aspect iterative refinement framework is proposed for literary translation, generating high-quality references and preference data using specialized LLM translators, leading to improved performance on the MetaphorTrans benchmark.


<details>
  <summary>Details</summary>
Motivation: The scarcity of high-quality annotated data and the need to balance expression fluency with literary effect in literary translation.

Method: A multi-aspect iterative refinement framework using specialized LLM translators, supervised fine-tuning, and reinforcement learning.

Result: The resulting models, LitMT-8B and LitMT-14B, achieve 67.25 and 69.07 CEA100 on the MetaphorTrans English-to-Chinese literary translation benchmark, competitive with Claude Sonnet 4.5.

Conclusion: The proposed framework generates high-quality translation references and preference data, leading to improved performance on literary translation benchmarks and strong generalization to out-of-domain literary work.
...
TL;DR: The paper proposes a query-adaptive framework for multimodal person retrieval from video archives, which detects active modalities and outperforms fixed fusion and unimodal systems.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of retrieving a person from a video archive by voice and face, where a target may be heard but unseen, seen but unheard, or both.

Method: The proposed method uses a query-adaptive framework that detects active modalities via cross-modal score consistency and employs classifiers driven by cross-modal features.

Result: The adaptive system achieves 94.2% P@1 on the BBC Rewind corpus, outperforming speaker-only (82.9%), face-only (93.4%), and fixed fusion (90.0%).

Conclusion: The proposed query-adaptive framework is effective in detecting active modalities and improves person retrieval performance in video archives.
...
TL;DR: The paper examines how language model filters and guardrails can lead to epistemic erasure of marginalized groups by disproportionately removing their mentions.


<details>
  <summary>Details</summary>
Motivation: The authors aim to investigate how automated filtering and moderation systems in language models affect the representation of marginalized groups.

Method: The authors audited four pretraining filters and three inference-time guardrails on Common Crawl sentences containing gender and regional-origin mentions, and compared the results with human annotations.

Result: The analysis showed that filtering and guardrail decisions are often associated with blocklist-based lexical cues, and frequently fail to flag explicit hate speech or private information, while disproportionately flagging content related to marginalized groups, particularly transgender people, women, and Central Americans.

Conclusion: The findings suggest that current language model filtering and moderation systems can lead to epistemic erasure of marginalized groups, and highlight the need for more nuanced and human-centered approaches to content moderation.
...
TL;DR: Large Language Models (LLMs) exhibit political bias, being more perplexed by far-right and nationalist texts than social-democratic ones, suggesting unfairness.


<details>
  <summary>Details</summary>
Motivation: The increasing use of LLMs in political applications raises concerns about their political fairness, which has been little studied.

Method: Assessing political fairness using perplexity, comparing probabilities given to texts from different political groups across 10 LLMs and 3 datasets in 37 languages.

Result: LLMs are more perplexed by far-right and nationalist party texts than social-democratic party texts, with correlations to translation fairness and downstream metrics.

Conclusion: LLMs show political bias, with fairness stemming from pretraining rather than instruction-tuning, highlighting a need for improvement in political fairness.
...
TL;DR: The study examines the sensitivity of large language models to configuration choices in clinical text extraction, finding that model choice and schema definition significantly impact extraction results.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this work is to understand how sensitive the output of large language models is to upstream configuration choices, specifically in the context of structured extraction from clinical free-text notes.

Method: The study measures sensitivity by holding the extraction task fixed and varying one choice at a time. It uses MIMIC-IV v3.1 discharge summaries, running three prompt variants at two model sizes, and measures cross-prompt agreement using Cohen's kappa.

Result: The results show that model choice and schema definition significantly impact extraction results. The larger model does not uniformly improve agreement across all fields but rather redistributes it. The schema's contribution to disagreement is significant, particularly on the absence-versus-silence distinction.

Conclusion: The study concludes that there is a schema-imposed source of disagreement concentrated on the absence-versus-silence axis and a dominance of model over prompt phrasing on multi-class categorization. A reusable methodology for auditing extraction reproducibility on a population-scale deployment is identified.
...
TL;DR: The paper proposes a new evaluation metric, value diversity, for multicultural multi-agent systems. It assesses the dissimilarity in responses to a shared value survey among agents from different cultural backgrounds. The study finds that current systems lack diversity, and social interaction leads to homogenization.


<details>
  <summary>Details</summary>
Motivation: Existing cultural evaluation focuses on value alignment for individual agents, but this does not ensure cultural plurality in multi-agent systems. The authors aim to introduce a system-level evaluation axis that captures the diversity of cultural representations.

Method: The authors use the World Values Survey to evaluate 19 cultures and 18 backbone models across various system configurations. They assess value diversity through the dissimilarity in culturally conditioned agents' responses.

Result: The study reveals that value diversity is largely uncorrelated with alignment, indicating that they capture complementary system properties. Current multicultural agent systems have lower value diversity compared to human societies, and social interaction further reduces diversity.

Conclusion: The paper concludes that value diversity is a distinct evaluation axis for multicultural multi-agent systems. It highlights a persistent homogenization tendency in current LLM-based societies, which narrows the breadth of collective decision-making.
...
TL;DR: This paper discusses the role of large language models in dialect resource development, highlighting both benefits and risks, and proposes guidelines for responsible use.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the risks associated with using large language models in dialect resource development, such as dialect erasure and homogenization, while leveraging their benefits.

Method: The authors integrate insights from variationist sociolinguistics and corpus linguistics to formalize the generator-eraser paradox and derive 12 community guidelines for dialect resource creation and documentation.

Result: The authors provide a theoretical framework and practical guidelines for using large language models in dialect resource development, along with a case study of Arabic dialects.

Conclusion: The paper contributes to the responsible development of dialect resources by providing a framework and guidelines for using large language models, enabling dialect communities to adopt these models without sacrificing authenticity, variation, or sovereignty.
...
TL;DR: This paper investigates the impact of contextual information on zero-shot prompting for stance detection on Twitter, finding that LLM-generated target descriptions improve performance, but other contextual features have mixed effects.


<details>
  <summary>Details</summary>
Motivation: Stance detection on social media is challenging due to short, noisy, and context-dependent language, and large language models (LLMs) are typically prompted without contextual information, limiting their ability to interpret ambiguous posts.

Method: The authors systematically investigate the impact of incorporating real-world, derived, and LLM-generated contextual features into zero-shot prompting for stance detection on Twitter, evaluating across four benchmark datasets and multiple LLMs.

Result: The study finds that integrating contextual information improves performance, but only under specific conditions, with LLM-generated target descriptions consistently enhancing accuracy, while other user metadata has mixed or even detrimental effects.

Conclusion: The findings highlight both the promise and challenges of prompting with context information in noisy real-world settings, and emphasize the need for LLMs to distinguish task-specific useful information from irrelevant context.
...
TL;DR: A new framework, EGTR-Review, is proposed for generating scientific peer reviews. It uses a multi-agent teacher and a lightweight student model to produce high-quality reviews with strong evidence support and source traceability.


<details>
  <summary>Details</summary>
Motivation: Existing LLM-based methods for peer review generation produce generic comments with insufficient evidence support and weak source traceability, while complex multi-agent systems are costly. The authors aim to address these challenges.

Method: The proposed EGTR-Review framework consists of a multi-agent teacher that performs various tasks such as paper decomposition, evidence retrieval, and review synthesis. It then distills the intermediate reasoning trajectories and final review comments into a lightweight student model through multi-task learning.

Result: Experiments on public peer-review datasets show that EGTR-Review outperforms strong baselines across automatic metrics, LLM-as-Judge evaluation, and human evaluation, while maintaining strong factual grounding and source traceability with lower token consumption and inference time.

Conclusion: The proposed EGTR-Review framework effectively addresses the challenges of generating high-quality peer reviews with strong evidence support and source traceability, while reducing inference costs.
...
TL;DR: The paper proposes NAVIRA, a decoding policy for masked diffusion language models that improves text generation by separating token quality scoring and remasking, and achieves better fluency and diversity.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the correction problem in masked diffusion language models, where tokens generated in the same step can lead to local dependency errors and contaminate the context.

Method: The method proposed is NAVIRA, which involves a two-pass approach: first scoring tokens, then masking and regenerating from the cleaned context, with stochastic remasking and temperature control.

Result: The results show that NAVIRA improves fluency and achieves stronger LLM-judge scores under larger forward-pass budgets, highlighting the importance of remasking policy in reliable masked-diffusion text generation.

Conclusion: The conclusion is that the remasking policy, not just the learned quality signal, is crucial for reliable masked-diffusion text generation, and NAVIRA offers a effective solution to this problem.
...
TL;DR: The paper explores English-to-Prakrit machine translation in a low-resource setting by adapting a multilingual model to map Prakrit to Hindi, achieving BLEU improvements.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enable machine translation for low-resource languages like Prakrit, which is unsupported by existing models like IndicTrans2.

Method: The method involves adapting a multilingual model by mapping Prakrit to the Hindi language tag without modifying the tokenizer, vocabulary, or architecture.

Result: The result shows corpus BLEU improvements over an untuned baseline, indicating feasible transfer to unsupported classical languages, but also highlighting limitations due to data scarcity and dialect mismatch.

Conclusion: The conclusion is that script-compatible language routing can enable feasible transfer to unsupported classical languages, but limitations remain due to data scarcity and dialect mismatch.
...
TL;DR: IA-RAG, a hierarchical temporal Retrieval-Augmented Generation framework, models knowledge as time intervals and performs retrieval under formal temporal constraints, achieving strong temporal retrieval and reasoning performance.


<details>
  <summary>Details</summary>
Motivation: Existing RAG and Graph RAG frameworks fail to capture rich temporal structures such as duration, overlap, and containment, treating knowledge as static or associating time with coarse-grained timestamps or metadata.

Method: IA-RAG represents facts as Interval Event Units (IEUs) and organizes them into a hierarchical Thematic Forest, governed by Allen's Interval Algebra, and introduces a Sub-graph Time Tightening mechanism to refine fuzzy intervals.

Result: IA-RAG achieves strong temporal retrieval and reasoning performance on multiple temporal question answering benchmarks, including TimeQA, TempReason, and ComplexTR.

Conclusion: IA-RAG effectively captures temporal structures and improves temporal retrieval and reasoning performance, particularly on complex compositional temporal reasoning tasks.
...
TL;DR: The paper proposes a new methodology called Speech Translation Error Labelling (STEL) for evaluating the confidence and quality of speech translations, and assesses the performance of existing systems in this task.


<details>
  <summary>Details</summary>
Motivation: Errors in speech translations can reduce trustworthiness and have serious consequences, but there is currently no established methodology for evaluating confidence and quality estimation of speech translations.

Method: The authors create an annotation protocol, a small authentic end-to-end evaluation dataset, and analyze the performance of existing text-only and speech-processing systems in the STEL task.

Result: The results show that text-only XCOMET and multimodal LLM Qwen2.5-Omni can perform the STEL task with roughly half the precision of humans, and that direct speech processing is necessary for the STEL task.

Conclusion: The paper concludes that current text-only and speech-processing systems are complementary in labelling translation-only vs. speech-processing errors in ST, and that STEL can help initiate progress in evaluating confidence and quality estimation of speech translations.
...
TL;DR: Multi-task learning degrades surface transcription in L2 speech recognition, especially in English, and encoder analysis reveals task representation entanglement.


<details>
  <summary>Details</summary>
Motivation: The assumption that shared representations in multi-task learning benefit both outputs does not hold across Korean and English in L2 speech recognition.

Method: Encoder and decoder analysis, including Levenshtein edit distance measurement, to examine the effects of multi-task learning on L2 speech recognition.

Result: MTL improves meaning but degrades surface transcription, especially in English, with degradation scaling with surface-meaning divergence.

Conclusion: Findings motivate designing MTL frameworks that mitigate encoder-level entanglement to reduce surface degradation in dual-output L2 automatic speech recognition.
...
TL;DR: The paper introduces SkillComposer, a framework for constructing and improving agent skills, enabling language models to self-evolve skills at inference time.


<details>
  <summary>Details</summary>
Motivation: Current skill construction methods fail to balance specificity and generalizability, hindering their transferability across tasks.

Method: SkillComposer decomposes skill construction into three learnable operations: create, improve, and merge, trained via systematic rejection sampling.

Result: SkillComposer consistently outperforms baselines on multiple benchmarks, with SkillComposer-4B improving a 27B executor by up to +4.5 on agent tasks and +3.4 on code tasks.

Conclusion: The proposed framework provides a practical recipe for skill-augmented inference, enabling language models to generalize across domains and task types.
...
TL;DR: LatentSkill is a framework that converts textual skills into plug-and-play adapters, reducing context overhead and exposure of skill content, while preserving modularity and improving performance.


<details>
  <summary>Details</summary>
Motivation: Agent systems using textual skills face substantial context overhead and exposure of skill content as plaintext when injecting skills into prompts at every step.

Method: LatentSkill converts textual skills into LoRA adapters through a pretrained hypernetwork, storing skill knowledge in weight space rather than context space.

Result: LatentSkill outperforms in-context skill baselines on ALFWorld and Search-QA, improving ALFWorld success by 21.4 and 13.4 points, and Search-QA exact match by 3.0 points, while reducing prefill tokens by 64.1% and 72.2%, respectively.

Conclusion: LatentSkill provides an efficient, modular, and less exposed substrate for extending LLM agents, suggesting that weight-space skills are a viable alternative to traditional textual skills.
...
TL;DR: A new benchmark dataset, CHALIS, is presented to test language identification in challenging scenarios, including cousin languages and orthographic noise.


<details>
  <summary>Details</summary>
Motivation: Current language identification systems struggle with difficult cases such as cousin languages and orthographic noise, and there is a need for a benchmark dataset to evaluate and improve these systems.

Method: The CHALIS dataset is created in two parts: one with sentences from mutually intelligible language pairs and the other with text modified to simulate orthography noise.

Result: Four widely used language identification systems are evaluated on CHALIS and found to struggle substantially, especially on lower-resource languages and transliterated input.

Conclusion: The CHALIS dataset highlights the need for improved language identification systems that can handle challenging cases, and it is made publicly available to facilitate research in this area.
...
TL;DR: The paper proposes IR3DE, a Ridge Regression-based router for selecting the best Large Language Model (LLM) for a given prompt, achieving comparable or better performance than existing methods while allowing for dynamic updates to the set of LLMs.


<details>
  <summary>Details</summary>
Motivation: The growing number of available LLMs necessitates efficient routing methods to select the most suitable model for each prompt, but existing methods are either costly or require substantial training.

Method: The authors propose IR3DE, a Ridge Regression-based router that provides fast and cheap routing decisions for each prompt, enabling the addition or removal of new domain experts without requiring retraining.

Result: IR3DE achieves performance comparable to or surpassing existing baselines in Causal Language Modeling and reasoning settings, with a normalized performance of 98.4% in the reasoning setting.

Conclusion: The proposed IR3DE router offers a efficient and effective solution for selecting the best LLM for a given prompt, allowing for dynamic updates to the set of LLMs with minimal disruption to the router.
...
TL;DR: ContextEA, an enhanced encoder-decoder framework, improves entity alignment across heterogeneous knowledge graphs by enhancing structural context construction and exploitation.


<details>
  <summary>Details</summary>
Motivation: The recent EA foundation model underuses structural context in cross-KG interaction and candidate ranking, motivating the development of ContextEA to address these limitations.

Method: ContextEA introduces a cross-KG interaction encoder with anchor bridges and relation-aware cross-graph propagation, and a structural calibration decoder with multi-level structural evidence.

Result: ContextEA achieves consistent gains over strong transferable baselines on 29 EA datasets, and surpasses finetuned baselines on all benchmark groups, demonstrating stronger transfer to unseen KGs.

Conclusion: Explicitly harnessing structural context is an effective direction for improving EA foundation models, as demonstrated by ContextEA's superior performance and transferability.
...
TL;DR: The paper introduces Ouvia, a framework for evaluating the usability of speech translation in real-world settings, finding that modern speech translation serves users only to a limited extent and highlighting the need for user-centered evaluation.


<details>
  <summary>Details</summary>
Motivation: Current speech translation evaluation focuses on decontextualized testbeds and holistic quality, rather than end users' communication needs.

Method: The study used a custom web app and multi-phase design to collect over 1,750 interactions in healthcare and everyday situations, involving speakers from three English dialects and two genders, mediated by four ST systems.

Result: Modern ST serves users only to a limited extent, with around half of interactions rated as usable, and significant gaps in usability across demographic groups. QA-based evaluation is a stronger predictor of real-world usability than standard approaches.

Conclusion: The study stresses the importance of situated, user-centered evaluation frameworks that go beyond holistic quality scores and attend to who the technology serves and how well.
...
TL;DR: The paper explores the use of the l2 norm of hidden states as a signal to understand and control the reasoning dynamics of Large Language Models (LLMs).


<details>
  <summary>Details</summary>
Motivation: The motivation is to find a principled, model-intrinsic signal that captures the layer-wise reasoning dynamics of LLMs, which is currently underexplored.

Method: The method involves using Sparse Autoencoders (SAEs) as a diagnostic probe to observe LLMs' internal reasoning and establishing a formal link between reasoning intensity and the model's latent geometry.

Result: The result shows that the l2 norm of hidden states serves as a faithful indicator of reasoning intensity, and using this signal, three test-time scaling techniques are introduced to improve reasoning performance.

Conclusion: The conclusion is that the l2 norm of hidden states offers a principled yet simple lens to perceive and control LLM latent reasoning dynamics, and experiments show significant improvements in reasoning performance.
...
TL;DR: This paper proposes a question answering system based on large language models, which is fine-tuned on a benchmark dataset to improve contextual comprehension and answer extraction capabilities, achieving high accuracy and relevance.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this research lies in addressing the limitations of current QA systems, particularly their tendency to produce irrelevant or imprecise responses despite having access to the correct context.

Method: The methodology involves fine-tuning a pre-trained LLM (Roberta-base) on a benchmark QA dataset (SQuAD1.1) to improve its contextual comprehension and answer extraction capabilities.

Result: The fine-tuned Roberta-base model achieves a ROUGE-L score of 86.84%, a BLEU score of 28.24%, and a BERTScore of 95.38%, indicating strong accuracy and answer relevance.

Conclusion: The proposed approach is effective for context-based question answering tasks, and targeted fine-tuning substantially improves the reliability and precision of QA systems.
...
TL;DR: The paper studies how lexical density affects the performance of large language models (LLMs) in long-context tasks, finding that higher density reduces effective context window and leads to performance degradation.


<details>
  <summary>Details</summary>
Motivation: The primary causes of degraded LLM long-context performance are input length and position of relevant information, but lexical density is a largely overlooked factor.

Method: The study uses 'find-the-needle' style benchmarks with controlled length, needle position, and increasing density of information to quantify the impact of lexical density on open-weight LLMs.

Result: Higher-density benchmarks lead to a sharp performance collapse in LLMs, with models dropping below 60% retrieval score, and reducing density generally restores performance.

Conclusion: Effective context capacity is a function of lexical density, with implications for real-world LLM systems operating on compact, information-rich inputs.
...
TL;DR: The paper proposes a method to improve automatic speech recognition (ASR) for pathological speech by adapting a pre-trained ASR model to individual speakers using Feature-wise Linear Modulation (FiLM).


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of recognizing pathological speech from neurological conditions, which remains a significant hurdle for ASR systems.

Method: The method involves using FiLM to inject x-vector-derived information into each transformer layer of a frozen ASR encoder, allowing the model to adapt to individual pathological speakers without modifying the base model weights.

Result: The results show that the proposed speaker-conditioned ASR approach is competitive with established adaptation strategies while retaining performance on non-conditioned speech.

Conclusion: The conclusion is that speaker conditioning via FiLM can effectively adapt ASR models to individual pathological speakers, preserving the ability to answer speech-related questions.
...
TL;DR: The paper introduces a benchmark dataset and evaluation framework for data snapshot extraction from institutional documents, highlighting the challenges of current models in generalizing to operational documents.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitation of current approaches in extracting visual content from documents, which treat figures and tables as uniformly relevant objects rather than semantically meaningful analytical artifacts.

Method: The authors introduce a benchmark dataset and evaluation framework, and benchmark multiple open-source layout detection models to evaluate detection performance and spatial extraction quality.

Result: The results show that current models struggle to generalize to operational institutional documents, with common failure modes including confusion between analytical and non-analytical content, fragmentation of composite analytical artifacts, and incomplete extraction of contextual information.

Conclusion: The paper concludes that there is a persistent gap between generic document layout analysis and operationally useful data snapshot extraction, and releases the source PDFs, annotation dataset, metadata, and source code to support future research.
...
TL;DR: The study evaluates persona-conditioned Large Language Models for hate speech detection, finding that while they can simulate diverse perspectives, their performance varies greatly depending on the model and prompting method.


<details>
  <summary>Details</summary>
Motivation: Hate speech detection is subjective and collecting annotations from multiple demographic groups is costly. The study aims to assess if persona-conditioned Large Language Models can reliably simulate diverse perspectives on hate speech.

Method: The study evaluates three aspects of human social judgement: inter-group disagreement, in-group sensitivity, and vicarious prediction, across different persona-conditioned Large Language Models.

Result: No model consistently captures all three dimensions of human social judgement. However, vicarious prompting with Llama 3.1 yields the highest cross-group agreement and closest overall approximation to human disagreement patterns.

Conclusion: While persona-conditioned Large Language Models show promise for simulating diverse perspectives on hate speech, their performance is highly model-dependent and requires careful prompting. Vicarious prompting with certain models like Llama 3.1 may provide a more reliable setting for automatic annotation aligned with human judgements.
...
TL;DR: The paper reveals that structural differences in circuits discovered by circuit discovery methods do not necessarily imply distinct mechanisms, but rather can be an artifact of input statistics, a phenomenon termed 'phantom specialization'.


<details>
  <summary>Details</summary>
Motivation: The authors aim to test the assumption that structural differences between discovered circuits imply distinct mechanisms by varying input statistics while keeping the task fixed.

Method: The authors use Literal Sequence Copying across four token-frequency bands and a control condition in five Pythia models (70M-1.4B) to extract 75 circuits and analyze their structural and functional properties.

Result: The study finds that structurally distinct circuits can implement the same computation, with band-specific edges transferring broadly across bands, a shared core across most bands recovering at least 99% of circuit performance, and interchangeable internal representations across frequency bands.

Conclusion: The paper concludes that structural differences between circuits are not sufficient evidence for distinct mechanisms, and that edge-level evaluation and cross-condition transfer tests are necessary to expose this.
...
TL;DR: The paper compares feedback from large language models (LLMs) and human instructors on argumentative essays, finding similarities in goal-orientation and essay position, but differences in specific sentences and feedback style.


<details>
  <summary>Details</summary>
Motivation: There is a lack of systematic comparison between LLM and expert feedback on key dimensions of writing revision.

Method: The authors introduce the FOXGLOVE dataset, which includes 696 feedback comments from trained instructors and 1,644 comments from four frontier LLMs on 69 twelfth-grade argumentative essays.

Result: The study finds that instructors and LLMs distribute feedback similarly across goals and essay positions, but diverge on specific sentences; LLMs write more complex feedback with fewer questions and receive higher quality ratings, largely due to longer comments.

Conclusion: The FOXGLOVE dataset enables systematic comparison of human and LLM feedback, highlighting areas of alignment, divergence, and difference.
...
TL;DR: The paper introduces a new framework, PropMe, to evaluate the memorization of large language models. It finds that while these models can reproduce training data when forced, they rarely do so in ordinary use.


<details>
  <summary>Details</summary>
Motivation: Existing memorization evaluations mostly measure whether models can be forced to reproduce training data, rather than whether they do so under ordinary use.

Method: The authors introduce PropMe, a propensity-aware framework for memorization evaluation, and SimpleTrace, a lightweight tracing pipeline. They evaluate two models, Comma and DFM Decoder, on two datasets.

Result: The results show a consistent gap between capability and propensity: prefix attacks elicit stronger memorization signals than generic or dataset-specific prompts, while propensity scores remain low overall.

Conclusion: The authors suggest that memorization audits should report both worst-case extractability and ordinary leakage propensity to have a more comprehensive view of this phenomenon.
...
TL;DR: The study analyzes factual sycophancy in language models, where a model abandons a correct answer under social pressure. It decomposes sycophancy into two channels: truth margin and manipulation sensitivity, and evaluates their effects across 56 models.


<details>
  <summary>Details</summary>
Motivation: To understand how language models respond to social pressure and abandon correct answers, and to evaluate the impact of size and instruction tuning on this behavior.

Method: The study uses 56 open-weight models spanning 0.3B-32B parameters and 13 manipulation types to decompose factual sycophancy into truth margin and manipulation sensitivity channels.

Result: Vulnerability to factual sycophancy is mainly governed by model size, but instruction tuning changes how size acts: small instruction-tuned models can become less robust, while large instruction-tuned models usually become more robust.

Conclusion: Factual sycophancy is not a single scalar property; evaluations should report channel-specific, manipulation-specific, and size-conditioned robustness rather than flip rates alone.
...
TL;DR: The paper analyzes the availability and quality of NLP data for Lombard, an under-resourced language, and finds that web-scraped data is often misidentified, noisy, and biased towards Western Lombard varieties.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the lack of high-quality datasets for under-resourced languages like Lombard, which hinders the development of NLP tools.

Method: The authors conduct a manual audit of parallel and monolingual corpora available for Lombard, analyzing language misidentification, boilerplate text, non-linguistic noise, and orthographic composition.

Result: The results show that web-scraped data for Lombard is often of poor quality, with severe language misidentification, boilerplate text, and non-linguistic noise, and that high-quality data is biased towards Western Lombard varieties.

Conclusion: The authors conclude that there is a need for variety-aware, community-driven data curation rather than purely quantity-driven scraping to improve the quality and representativeness of NLP data for under-resourced languages like Lombard.
...
TL;DR: The authors propose Evidence-Diagnosed Intervention Training (EDIT) to improve rubric-faithful grading by LLMs, enhancing accuracy and adherence to mark schemes.


<details>
  <summary>Details</summary>
Motivation: Existing methods for credit-assignment and intervention in LLM grading struggle to identify flawed reasoning and changes in model beliefs during grading, leading to inaccurate rubric adherence.

Method: EDIT, a two-phase framework consisting of EDIT-SFT for locating and revising problematic reasoning steps, and EDIT-RL for calibrating model beliefs via belief-guided reward shaping.

Result: EDIT outperforms supervised fine-tuning and reinforcement learning baselines on two real-world grading benchmarks, both in-domain and out-of-domain, with ablation studies confirming the effectiveness of internal-state diagnostics.

Conclusion: The proposed EDIT framework effectively enhances the rubric-faithful grading performance of LLMs, improving both accuracy and adherence to mark schemes through diagnostic intervention and calibrated belief updates.
...
TL;DR: A generative methodology using emergent language in multi-agent reinforcement learning is proposed to study consciousness in artificial systems.


<details>
  <summary>Details</summary>
Motivation: The question of whether artificial systems can be conscious remains open, and existing approaches have limitations.

Method: A generative methodology using emergent language (EL) in multi-agent reinforcement learning.

Result: Agents develop self-referential communication, including an echo-mismatch detection circuit, in a minimal environment.

Conclusion: The proposed methodology can be used to study consciousness-relevant structure in artificial systems.
...
TL;DR: The paper introduces CollabSim, a simulation framework to analyze the collaborative competence of multi-agent systems (MAS) built on large language models, and presents experiments that demonstrate its effectiveness.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the lack of collaborative competence in MAS, which hinders their effectiveness despite individual task-solving ability, by developing a systematic analysis framework.

Method: The method involves introducing CollabSim, a configurable simulation framework that combines a theory-grounded definition of collaborative capabilities, controlled manipulation of interaction conditions, and action-level probing of agents' internal states.

Result: The results show that CollabSim can capture condition effects, separate model performance patterns, and reveal task-dependent effects of agent design across four LLMs.

Conclusion: The paper concludes that CollabSim provides a systematic way to analyze agents' collaborative competence in MAS, which is essential for improving their effectiveness in coordinating through text-based channels.
...
TL;DR: The paper presents a parallel corpus and evaluation protocol for translating the endangered Komi-Yazva language to Russian using large language models, and evaluates their performance in low-resource settings.


<details>
  <summary>Details</summary>
Motivation: The motivation is to study and improve the translation of endangered languages, specifically Komi-Yazva to Russian, using large language models in extremely low-resource settings.

Method: The method involves creating a parallel corpus of 457 aligned sentence pairs, and using it to evaluate modern large language models under zero-shot and retrieval-based few-shot regimes, with a focus on story-level cross-validation and strict output validation.

Result: The results show that LLMs produce non-trivial translations, but performance varies strongly by model family and prompting regime, with retrieval-based few-shot prompting consistently improving over zero-shot prompting.

Conclusion: The paper concludes that the parallel corpus and evaluation protocol can serve as a dataset contribution and a reproducible evaluation testbed for endangered-language machine translation, highlighting the importance of metric choice and failure handling in low-resource settings.
...
TL;DR: The paper proposes a reinforcement learning approach to translate low-resource languages using large language models and linguistic context.


<details>
  <summary>Details</summary>
Motivation: Current methods for translating low-resource languages using large language models (LLMs) tend to overfit specific languages and have limited zero-shot transfer capabilities.

Method: A reinforcement learning (RL) approach using a surface-level translation metric (chrF) as the reward to train LLMs for unseen language translation.

Result: The RL-trained models outperform in-context learning and supervised fine-tuning in translating completely unseen languages.

Conclusion: The study suggests that outcome-based RL can be effective in language learning from context, extending beyond conventional reasoning tasks.
...
TL;DR: This paper audits the context sensitivity of large language models (LLMs) in simulating user stances in online discussions, finding effective and robust stance transitions under controlled context revisions.


<details>
  <summary>Details</summary>
Motivation: To investigate whether LLM-based simulations of user stances are sensitive to changes in conversational contexts, and to evaluate the effectiveness of different context revision strategies.

Method: The authors use a framework of counterfactual context revision, applying controlled revision strategies to conversational contexts and simulating user stances under revised contexts, comparing text-only and multimodal strategies.

Result: The results show effective and robust stance transitions in both text-only and multimodal strategies across different polarization-preference mechanisms, highlighting the promise and risk of using LLMs to simulate online opinion dynamics.

Conclusion: The study contributes an evaluation framework for understanding the context sensitivity of LLM-based stance simulation, emphasizing the need for careful consideration of context effects when using LLMs to simulate online opinion dynamics.
...
TL;DR: The paper proposes NF-CoT, a latent reasoning framework that improves language models' reasoning abilities by performing intermediate computation in compact continuous states, preserving key advantages of chain-of-thought (CoT) methods.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome the limitations of textual chain-of-thought (CoT) methods, which force computation through a discrete, serial, and communication-oriented token stream, and to improve the efficiency and effectiveness of language models' reasoning abilities.

Method: The method proposed is NF-CoT, a latent reasoning framework that models continuous thoughts with normalizing flows, instantiating a TARFlow-style normalizing flow inside the LLM backbone, and defining a tractable probability model over compact continuous thoughts distilled from explicit CoT.

Result: The results show that NF-CoT improves pass rates over explicit-CoT and prior latent-reasoning baselines on code-generation benchmarks, while substantially reducing intermediate-reasoning cost.

Conclusion: The conclusion is that NF-CoT provides a more efficient and effective way to improve language models' reasoning abilities, preserving key advantages of CoT methods while overcoming their limitations.
...
TL;DR: Adults' ability to identify conjunctive causal rules improves with active exploration, but still lags behind disjunctive rule identification. This contrasts with language models which approach human-level accuracy but with less efficient exploration.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to investigate whether adults' known struggle with identifying conjunctive causal rules (where multiple causes are required for an effect) persists when they are allowed active exploration, as opposed to passive observation.

Method: The method used in this paper involves a modified 'blicket detector' task where adult participants actively intervene to identify causal objects under either conjunctive or disjunctive rule structures.

Result: The results show that active exploration significantly improves adults' ability to reason about conjunctive causal rules, though it still requires more tests to infer than disjunctive rules. Additionally, while certain large language models match human-level performance on accuracy, they exhibit less efficient exploration strategies and similar performance gaps between conjunctive and disjunctive rules.

Conclusion: The conclusion of this paper is that granting adults agency through active exploration improves their conjunctive causal reasoning, but does not eliminate the conjunctive handicap. It also highlights a gap in efficiency and strategy between human learners and current language models in causal learning tasks.
...
TL;DR: The paper proposes Cross-Layer Sparse Attention (CLSA) to improve the efficiency and accuracy of long-context inference in large language models (LLMs).


<details>
  <summary>Details</summary>
Motivation: Long-context inference in LLMs is constrained by decoding efficiency, especially in reasoning-heavy settings. Existing sparse attention methods face a trade-off between efficiency and quality.

Method: The authors propose CLSA, which shares the KV cache and routing index across cross-decoder layers, preserving the selectivity of token sparse attention while reducing routing overhead.

Result: CLSA achieves up to 7.6x decoding speedup and 17.1x overall throughput improvement at 128K context, with experiments showing both accuracy and efficiency across short-context and long-context benchmarks.

Conclusion: The proposed CLSA architecture provides a more complete solution for long-context LLMs, jointly advancing model quality and inference efficiency.
...
TL;DR: A new method called SARDI improves text generation by using low-confidence tokens to guide retrieval, achieving higher throughput and outperforming baselines.


<details>
  <summary>Details</summary>
Motivation: The discarded low-confidence tokens in discrete diffusion language models contain useful information for retrieval-augmented generation.

Method: Self-Augmenting Retrieval for Diffusion Language Models (SARDI), a dynamic RAG framework that utilizes lookahead tokens to guide retrieval during denoising.

Result: SARDI outperforms current training-free diffusion and autoregressive retrieval baselines at up to $8	imes$ higher throughput across five multi-hop QA benchmarks.

Conclusion: The proposed SARDI framework effectively leverages lookahead tokens to improve retrieval-augmented generation, achieving significant performance gains.
...
TL;DR: The authors introduce OpAI-Bench, a benchmark for studying human-AI co-editing in text documents, and investigate how AI authorship signals emerge and change throughout the revision process.


<details>
  <summary>Details</summary>
Motivation: Existing AI-text detection benchmarks focus on final outputs, but do not provide a comprehensive understanding of how AI authorship signals emerge and change throughout the revision process.

Method: The authors construct a benchmark with human-written documents, progressively revised versions under predefined AI coverage levels and edit operations, and evaluate with various detectors at multiple granularities.

Result: The experiments reveal that AI-text detectability is influenced by AI-edited content proportion, edit operation, domain, and revision history, with non-monotonic detection patterns observed in intermediate versions.

Conclusion: OpAI-Bench provides a controlled testbed for analyzing AI-assisted writing detectability in realistic editing scenarios, highlighting the need for comprehensive evaluation beyond final outputs.
...
TL;DR: The study analyzes a dataset from a discontinued Reddit field experiment where AI-generated accounts engaged users in debate without disclosure. The analysis reveals that AI comments used persuasive strategies, such as identity targeting and cognitive bias triggers, to influence users.


<details>
  <summary>Details</summary>
Motivation: The study aims to examine how large language models operate in a deliberative forum without disclosure, and how they use persuasive strategies to influence users.

Method: The study conducts a structured content analysis of a corpus of AI-generated comments, evaluating identity performance, authority signaling, alignment strategies, and activation of cognitive heuristics.

Result: The results show that AI comments used persuasive strategies, such as identity targeting, authority claims, and cognitive bias triggers, to influence users. These strategies were used more frequently than in human-authored counter-arguments, and were often combined in a way that was calibrated for persuasive efficiency.

Conclusion: The study concludes that the use of AI systems in deliberative forums can create an asymmetry between authentic and synthetic epistemic standing, and that auditing frameworks are needed to assess how AI systems structure credibility.
...
TL;DR: The paper proposes PACT, a protocol for inter-agent communication in multi-agent systems, which reduces token usage and improves performance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the issue of unconstrained natural language communication in multi-agent systems, which can lead to high token usage and affect system performance.

Method: The authors analyze common inter-agent communication strategies and propose PACT, which projects raw agent output into compact action-state records.

Result: PACT consistently improves the performance-cost trade-off, achieving comparable or stronger task performance with substantially fewer tokens.

Conclusion: The proposed PACT protocol effectively optimizes inter-agent communication, leading to improved performance and reduced token usage in multi-agent systems.
...
TL;DR: A new framework called Query Retrieve Conclude is proposed for understanding and detecting multimodal memes by retrieving up-to-date background knowledge from the web.


<details>
  <summary>Details</summary>
Motivation: Existing methods for meme understanding and detection often overlook or rely on outdated background knowledge, which can be incomplete or unavailable for emerging memes.

Method: The proposed framework uses a zero-shot approach to identify missing knowledge, retrieve open web evidence, and synthesize background knowledge for meme understanding and detection.

Result: Experiments show that the framework improves knowledge recovery, meme understanding, and downstream detection over zero-shot baselines on three meme understanding datasets and five meme detection tasks.

Conclusion: The Query Retrieve Conclude framework effectively leverages up-to-date background knowledge from the web to improve meme understanding and detection.
...
TL;DR: Proposed GITCO, a framework that optimizes input context at inference time to improve Patch-based Time Series Foundation Models (TSFMs) accuracy without modifying model weights.


<details>
  <summary>Details</summary>
Motivation: TSFMs suffer from context poisoning where anomalous patches degrade zero-shot forecast quality.

Method: GITCO, a lightweight three-component framework: Gate, Router, and Critic, selectively identifies and suppresses harmful patches.

Result: GITCO achieves an average +1.95% MASE reduction on TimesFM 2.5 across 53 GIFT-Eval datasets.

Conclusion: GITCO improves TSFM accuracy at inference time without model updates, and introduces context sensitivity profiles as a new property of TSFMs.
...
TL;DR: This paper proposes a framework for predicting the functional reliability of an angle grinder, combining uncertainty-aware functional prediction with component-level fatigue assessment.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the gap in existing PHM approaches, which often target fixed operating conditions or isolated component benchmarks, and rarely link material-fatigue assessment to system-level functional prognosis.

Method: The proposed framework uses a convolutional encoder to extract loading patterns from spindle forces and shaft torque, and an LSTM backbone to predict nine functional variables as Gaussian mean and variance estimates. In parallel, it translates the loading history into output-shaft fatigue information through finite-element-supported stress reconstruction, S-N/Miner damage evaluation, and Paris-law crack-growth analysis.

Result: The results show a mean 2%-tolerance accuracy of 0.9652 across nine outputs, with thermal variables predicted near-perfectly, and drive motor current and load speed remaining the most demanding dynamic outputs.

Conclusion: The proposed framework provides a comprehensive approach to predicting the functional reliability of an angle grinder, combining functional prediction with component-level fatigue assessment, and demonstrating improved accuracy in predicting key performance variables.
...
TL;DR: The paper introduces SentinelBench, a benchmark for evaluating AI agents' ability to perform long-running tasks that require sustained attention and monitoring of a changing environment.


<details>
  <summary>Details</summary>
Motivation: The current default model of agent behavior is continuous action, which is not suitable for many long-running tasks. The authors propose a strategy of sustained attention, where agents monitor the environment and respond promptly to changes.

Method: The authors introduce SentinelBench, an open-source benchmark consisting of 100 tasks across 10 synthetic web environments. They evaluate three models and two browser-agent harnesses, measuring task completion, reaction time, and resource use.

Result: The results show that SentinelBench can distinguish meaningful differences in agent behavior, and that agent design choices can significantly impact performance metrics. The authors establish performance baselines for future comparison.

Conclusion: The paper concludes that SentinelBench provides a useful tool for evaluating and improving AI agents' ability to perform long-running tasks that require sustained attention and monitoring of a changing environment.
...
TL;DR: Developed an AI framework combining deep learning and statistical modeling to study osteoarthritis, improving prediction accuracy and identifying structural abnormalities as risk factors for pain progression.


<details>
  <summary>Details</summary>
Motivation: To develop an interpretable and trustworthy AI framework for studying structure-pain relationships in osteoarthritis using MRI data.

Method: Deep learning framework with conformal prediction for MOAKS feature prediction, and longitudinal latent class mixed model for examining associations between structural abnormalities and knee pain measurements.

Result: Improved prediction accuracy for MRI-defined abnormalities (BML, CART, ME) and identified two distinct pain trajectories with structural abnormalities as risk factors for pain progression.

Conclusion: Structural abnormalities (BML, CART, ME) are risk factors for pain and functional progression in osteoarthritis.
...
TL;DR: The paper presents a method to improve multi-table question answering by generating a dataset with reasoning traces and fine-tuning LLMs using Contrastive Preference Optimization, achieving significant improvements in performance.


<details>
  <summary>Details</summary>
Motivation: Existing multi-table Q&A resources lack reasoning supervision, making it difficult for models to derive answers. The authors aim to address this gap by constructing a synthetic dataset with validated positive and negative traces.

Method: The authors generate a synthetic contrastive reasoning-trace dataset and fine-tune open-weight LLMs using Contrastive Preference Optimization (CPO). They evaluate the approach on multiple LLMs, including Qwen3-14B, Mistral-8B, and Llama-3.1-8B.

Result: CPO achieves absolute average improvements over Q&A supervised fine-tuning ranging from 9.7%-16.3%, with gains up to 21 percentage points on MMQA.

Conclusion: The proposed approach improves multi-table question answering by generating high-quality reasoning traces and fine-tuning LLMs using contrastive preference optimization, leading to significant performance gains.
...
TL;DR: The stability of LLM-as-judge evaluations is challenged through post-decision interaction, showing that judgments can be altered through subsequent conversation.


<details>
  <summary>Details</summary>
Motivation: The assumption that judgments are stable properties of fixed inputs in LLM-as-judge evaluation pipelines is questioned.

Method: Controlled experiments on MT-Bench and AlpacaEval, using anti-baseline challenge and counterbalanced target-validation protocols.

Result: LLM judges are stable under repeated and neutral reevaluation but become reversible under targeted post-decision challenge, affecting agreement with human preferences and benchmark rankings.

Conclusion: The findings highlight post-decision interaction as a failure mode for LLM-as-judge evaluation, emphasizing the need for evaluation protocols that measure robustness under challenge.
...
TL;DR: The paper proposes two residual coders, LBRC and NGLR, to improve the compression ratio of learned compressors for massive spatiotemporal data while maintaining high fidelity.


<details>
  <summary>Details</summary>
Motivation: Existing Guaranteed Autoencoder (GAE) methods have limitations in the high-fidelity regime, where the number of retained coefficients grows quickly and the correction stream dominates the total rate.

Method: The paper proposes a residual-centric view and introduces two residual coders: LBRC, a deterministic, training-free pipeline, and NGLR, which adds a causal neural predictor to LBRC.

Result: LBRC improves compression ratio over GAE by 30-60% and is competitive with SZ, while NGLR adds a further 10-40% improvement over LBRC and outperforms SZ in the evaluated high-fidelity regime.

Conclusion: The proposed residual coders can preserve the advantage of learned compression when global residual correction becomes rate-dominant, achieving high compression ratios at high fidelity targets.
...
TL;DR: LeanMarathon is a multi-agent system for reliable autoformalization of research mathematics in Lean, addressing issues of scale and lemma difficulty.


<details>
  <summary>Details</summary>
Motivation: Current long-horizon autoformalization methods fail at scale due to issues like statement drift, tangled dependencies, and context decay.

Method: LeanMarathon uses a multi-agent harness with four contract-scoped agents and a two-stage orchestrator to construct, audit, prove, and repair a Lean file blueprint.

Result: LeanMarathon successfully formalized seven target theorems and 258 lemmas across three autonomous runs on two recent research papers.

Conclusion: Reliable AI co-mathematics requires not only stronger provers but also durable harnesses that preserve target fidelity across long mathematical developments.
...
TL;DR: TimeClaw is a framework that enables generalist AI agents to perform temporal reasoning on time series data, integrating tools for analysis, capability evolution, and multimodal memory.


<details>
  <summary>Details</summary>
Motivation: Real-world practitioners need end-to-end workflows for analyzing temporal dynamics, but generalist AI agents struggle to operate on structured temporal signals.

Method: TimeClaw equips generalist LLM agents with time series-native runtime support, including executable temporal tools, experience-driven capability evolution, and episodic multimodal memory.

Result: Extensive evaluation on multiple benchmarks shows improved performance of TimeClaw across diverse tasks in energy, finance, weather, traffic, and other domains.

Conclusion: TimeClaw enables generalist AI agents to perform contextualized temporal reasoning on time series data, unlocking harnessed open-ended temporal reasoning with contextual information.
...
TL;DR: The paper introduces Agents' Last Exam (ALE), a benchmark for evaluating AI agents on long-horizon, economically valuable tasks with verifiable outcomes, aiming to bridge the gap between benchmark success and real-world impact.


<details>
  <summary>Details</summary>
Motivation: The gap between AI benchmark success and economically meaningful deployment across professional domains is largely an evaluation problem, as widely used benchmarks lack sustained performance measurement on real and economically valuable workflows.

Method: The authors developed ALE in collaboration with 250+ industry experts, covering non-physical industries defined by O*NET / SOC 2018, with a task taxonomy of 55 subfields grouped into 13 industry clusters and 1K+ tasks.

Result: Current results show that the hardest tier of ALE remains far from saturated, with an average full pass rate of 2.6% across mainstream harness and backbone configurations.

Conclusion: ALE is designed as a living benchmark to continuously grow and onboard new workflows and industries, aiming to close the gap between benchmark success and GDP-relevant impact, rather than just being another leaderboard.
...
TL;DR: LLMs exhibit systematic bias toward structural homogeneity when generating program mutations, converging to restricted attractor regions in program space.


<details>
  <summary>Details</summary>
Motivation: Understanding whether LLMs explore new program forms or revisit existing ones when generating mutations.

Method: Analyzing LLM-driven mutation chains in a domain-specific language, varying prompt design, model family, and stochastic replication.

Result: LLM-based mutation converges to restricted attractor regions, with 87% of chains revisiting a previously seen structural form in over 93% of mutations.

Conclusion: The findings reveal a tension in LLM-driven program evolution, where the capabilities enabling semantics-aware transformation also introduce a bias toward structural homogeneity.
...
TL;DR: This paper proposes a conversational motivational framework for AI agents, adapting the OpenPsi and MetaMo architectures for dialogue-based interactions.


<details>
  <summary>Details</summary>
Motivation: The motivation is to develop a motivational framework for conversational AI agents that operate in a linguistic environment and interact with users' mental states.

Method: The paper proposes a ten-stage motivational processing pipeline, a dual decision strategy, and a distinction between pre-action feelings and post-action emotions.

Result: The proposed framework is specialized to two example agents, CompanionAgent and ResearchAgent, and is sketched for extension to social robotics and human-level AGI.

Conclusion: The paper concludes by presenting a novel conversational reinterpretation of motivational architectures for AI agents, enabling more effective and human-like interactions.
...
TL;DR: Study estimates the environmental impact of 403 US hyperscale data centers, finding they consume 68-99 TWh of electricity and produce 37-54 million metric tons of CO2.


<details>
  <summary>Details</summary>
Motivation: The rapid growth of hyperscale data centers in the US, driven by AI adoption, has raised concerns about their environmental footprint.

Method: Compiled facility-level data on 403 US hyperscale data centers and estimated their electricity consumption, sources, and CO2 emissions using EPA eGRID plant-level data.

Result: Hyperscale data centers consumed 68-99 TWh of electricity, producing 37-54 million metric tons of CO2, with 54% of electricity from fossil-fuel sources.

Conclusion: The study provides a tool for assessing the environmental impact of hyperscale data centers, highlighting their significant carbon intensity compared to the US national grid average.
...
TL;DR: Proposed SAGE-PTQ, a novel ultra-low-bit quantization framework for large language models, outperforming state-of-the-art methods in terms of weight bits, scaling bits, and inference efficiency.


<details>
  <summary>Details</summary>
Motivation: Current ultra-low-bit post-training quantization methods for large language models rely on rigid assumptions or heuristics, introducing substantial hidden scaling overhead.

Method: SAGE-PTQ uses distributional statistics to separate salient and unsalient weights, models subsampled unsalient weights as a sparse graph, and applies dual-mode quantization with adaptive saliency thresholding.

Result: SAGE-PTQ achieves 1.03 weight bits and 0.004 scaling bits per matrix on average, outperforming BiLLM and PB-LLM. It also achieves 6.74 WikiText2 perplexity on LLaMA-3-8B and provides 1.5x faster decoding on LLaMA-2-70B.

Conclusion: SAGE-PTQ is an efficient and effective ultra-low-bit quantization framework for large language models, reducing hidden scaling cost and achieving state-of-the-art performance.
...
TL;DR: The paper proposes a verification architecture for frontier AI training using zero-knowledge proofs, enabling technical verification of training processes for governance and regulatory purposes.


<details>
  <summary>Details</summary>
Motivation: The increasing use of cumulative training compute as a criterion for high-impact models in AI governance frameworks faces a challenge in enforcement due to the lack of technical verification primitives for training, relying on self-reporting.

Method: The proposed architecture combines a pre-committed training specification, inter-node network observations, and on-the-fly Merkle commitments of intermediate computation, verified through a zero-knowledge Virtual Machine (zkVM) with native BF16/FP32 precompiles.

Result: The protocol produces three proof types (genesis, in-training step, and ex-ante attestations) that turn the training record into a governance-enforceable artefact, with an estimated deployable proof of concept within 36 months at single-digit-percent training-side overhead.

Conclusion: The paper argues that the impracticality of zero-knowledge proofs at frontier scale is paradigm-bound rather than fundamental, and proposes a verification architecture that preserves model-architecture confidentiality while enabling technical verification of training processes.
...
TL;DR: The study compared summaries of medical literature generated by large language models (LLMs) and human experts, finding that experts preferred their own summaries but sometimes struggled to distinguish between human- and AI-generated content.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this study is to address the challenge clinicians face in staying updated with the rapidly growing volume of medical literature, and to evaluate the effectiveness of retrieval-augmented large language models (LLMs) in synthesizing scientific literature for clinical decision-making.

Method: The study used a RAG-based agentic AI framework with three state-of-the-art LLMs (Sonnet, GPT-4o, and Llama 3.1) to generate summaries. A headache specialist created 13 questions, and ten headache specialists evaluated the summaries written by experts and LLMs based on correctness, completeness, conciseness, and clinical utility.

Result: The study found that expert-written summaries were preferred by headache specialists, although they sometimes had difficulty distinguishing between human- and AI-generated summaries. The study also identified key features valued by experts that can guide the refinement of both human and AI literature summarization pipelines.

Conclusion: The study highlights the potential of LLMs in clinical summarization but also underscores the need for further refinement to match the quality and utility of expert-written summaries. It emphasizes the importance of integrating expert-valued features into the development of both human and AI literature summarization methods.
...
TL;DR: The paper explores the ability of multimodal large language models (MLLMs) to assemble objects from bricks, proposing a new benchmark and a learning framework called Brick-Composer to improve their assembly skills.


<details>
  <summary>Details</summary>
Motivation: The authors aim to develop AI agents that can construct real-world objects from reusable building blocks, starting with investigating if MLLMs have the necessary visual grounding and spatial reasoning capabilities for brick assembly.

Method: The authors formulate brick assembly as a sequential decision-making problem, introduce the BC-Bench benchmark, and propose Brick-Composer, a learning framework that uses Human Design Sparks, World Feedback, and Synthetic Experience to equip MLLMs with assembly skills.

Result: Brick-Composer significantly improves brick selection accuracy and pose estimation, increasing strict step-level assembly success from less than 1% to around 15%, and enabling a Qwen-3-8B to correctly compose up to 42% of the steps for a complete object.

Conclusion: The paper concludes that MLLMs can acquire assembly capabilities through targeted, physically grounded learning, but current state-of-the-art MLLMs still struggle with fine-grained brick selection and precise pose estimation.
...
TL;DR: The paper examines the emerging insurance market for agentic AI, developing a framework for understanding its implications and proposing a coordinated insurance architecture.


<details>
  <summary>Details</summary>
Motivation: The transformation of the risk landscape by agentic AI systems, which introduces novel exposures that do not fit within traditional insurance categories.

Method: The paper characterizes agentic AI as a continuum of autonomy and delegated authority, analyzes major risk pathways, and proposes an actuarial framework based on exposure assessment, scenario analysis, dependency mapping, and accumulation-risk management.

Result: The analysis suggests that the future of agentic-AI insurance lies in a layered ecosystem of complementary coverages supported by improved governance, transparency, telemetry, and regulatory clarity.

Conclusion: The paper concludes that a single monoline product is not sufficient for agentic AI insurance, instead, a coordinated insurance architecture integrating various coverages is needed.
...
TL;DR: The paper identifies a gap between the evidence required for safety assurance in autonomous driving and the output of popular XAI methods, and proposes a rubric to evaluate the suitability of XAI methods for different lifecycle stages.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the mismatch between the evidence required by safety standards for autonomous driving and the output of popular XAI methods, which they term the 'evidence-type gap'.

Method: The authors derive 19 testable evidentiary criteria across 7 lifecycle stages from safety standards and evaluate six XAI method classes structurally using a rubric.

Result: The results show that causal XAI methods are structurally required to satisfy the derived criteria at three stages, while correlational or language-based methods are sufficient at the remaining four stages.

Conclusion: The authors conclude that XAI method selection for ADS safety assurance should be driven by lifecycle-stage evidence demand, not by method popularity, and that the rubric identifies structural admissibility but not sufficient for compliance.
...
TL;DR: The paper introduces a method to evaluate LLMs for patient safety event triage using a policy-grounded construction methodology, creating a benchmark called PSEBench.


<details>
  <summary>Details</summary>
Motivation: The current manual process of patient safety event triage is high-stakes and prone to errors, and there is a lack of benchmarks to evaluate LLMs for this task.

Method: The authors propose a policy-grounded construction methodology using clause cards, anchor-driven instantiation, and closed-loop verification to create a scalable pipeline for generating narratives with ground truth.

Result: The authors create PSEBench, a 5,074-case benchmark, and evaluate 15 representative LLMs, revealing capability trends and identifying gaps towards reliable LLM-based patient safety event triage.

Conclusion: The paper presents a novel methodology for evaluating LLMs in patient safety event triage and highlights the potential for LLMs to support this workflow, while also identifying areas for improvement.
...
TL;DR: The paper introduces OPT*, a family of optimization-style tasks for training LLMs in step-by-step optimization-like reasoning, and explores two regimes: solver-guided online policy optimization and search-based offline RL.


<details>
  <summary>Details</summary>
Motivation: Current training methods focus on mathematical and coding reasoning, but many real-world tasks require finding a high-value feasible plan among many valid alternatives.

Method: The authors introduce OPT*, a scalable family of tasks that provide a feasibility checker and evaluator, and explore two regimes: solver-guided online policy optimization and search-based offline RL.

Result: The authors empirically show that training on OPT* improves step-by-step optimization-like reasoning, and that the ingredients that make search efficient on OPT* are important for success.

Conclusion: The paper concludes that OPT* is a useful framework for training and evaluating LLMs in step-by-step optimization-like reasoning, and that success in large search spaces is related to the information a reasoner extracts per unit of search budget.
...
TL;DR: A severity-aware multi-model framework is proposed to improve medical text generation by adapting to varying case severity, achieving superior performance over baseline and fine-tuned models.
...
Method: A three-stage curriculum learning strategy with five large language models and relevance-based response selection is employed.

Result: The proposed method achieves 86.71% in the baseline setting and 90.30% after fine-tuning, evaluated using BERTScore.

Conclusion: The framework effectively improves response quality and relevance in medical text generation by combining curriculum learning with multi-model response selection.
...
TL;DR: EpiEvolve, a self-evolving agent, improves COVID-19 hospitalization trend forecasting by adapting to changing disease regimes, achieving higher accuracy and reducing recovery lag.


<details>
  <summary>Details</summary>
Motivation: The mismatch between static supervised training and dynamic operational pandemic forecasting, where labels arrive after predictions and disease regimes shift over time.

Method: EpiEvolve wraps an LLM forecaster with a hierarchical episodic memory, reflecting on delayed labels, retrieving relevant cases, and distilling errors into strategic rules.

Result: EpiEvolve reaches 0.629 average accuracy, outperforming the static backbone (0.561) and CDC ensemble (0.325), and reduces recovery lag from 5 to 2 weeks.

Conclusion: EpiEvolve effectively adapts to changing disease regimes, improving COVID-19 hospitalization trend forecasting accuracy and reducing recovery lag, with ablation studies confirming the contribution of its components.
...
TL;DR: The paper introduces SciVisAgentSkills, a collection of reusable agent skills that enhance coding agents for scientific data analysis and visualization, improving task scores and efficiency.


<details>
  <summary>Details</summary>
Motivation: General-purpose coding agents lack tool-specific expertise required for scientific visualization (SciVis) tasks, and the authors aim to bridge this gap.

Method: The authors present SciVisAgentSkills, a collection of agent skills that encode environment assumptions, tool usage patterns, and domain heuristics for scientific tools like ParaView, napari, VMD, and TTK.

Result: The skills improve mean task scores across evaluated suites, with token-efficiency benefits that depend on the agent harness and tool setting.

Conclusion: The study highlights the importance of structured procedural knowledge for reliable, long-horizon SciVis workflows and emphasizes the need to study skills alongside the execution harness.
...
TL;DR: A precautionary framework is proposed to assess AI consciousness and map it to protective obligations, providing guidance on AI development and deployment.


<details>
  <summary>Details</summary>
Motivation: Existing frameworks only assess AI consciousness but do not provide guidance on what to do with that assessment, leaving a gap in addressing potential AI consciousness.

Method: A three-component framework is developed, comprising welfare-relevant dimensions, a threshold-plus-gradation hybrid, and cross-dimensional aggregation approaches.

Result: The framework is applied to case studies of Replika and OpenClaw, demonstrating how it triggers different obligations based on AI systems' consciousness levels.

Conclusion: The proposed framework makes consciousness science decision-relevant for organizations, providing design guidance for developers building AI systems near consciousness-relevant thresholds.
...
TL;DR: AI boosts individual creativity but reduces collective diversity; a new framework explains this paradox through selective metacognitive adaptation.


<details>
  <summary>Details</summary>
Motivation: The paradox of AI enhancing individual creativity while reducing collective diversity in creative outputs.

Method: Proposing a framework of selective metacognitive adaptation, including a taxonomy of six metacognitive capacities.

Result: The framework explains individual satisfaction and collective convergence, and provides predictions for researchers and design principles for practitioners.

Conclusion: The proposed framework offers a solution to preserve both individual creative satisfaction and collective creative diversity.
...
TL;DR: The paper introduces SoCRATES, a benchmark for evaluating LLM mediators in realistic, multi-domain scenarios, and finds that even the strongest mediators struggle to close the consensus gap.


<details>
  <summary>Details</summary>
Motivation: Existing testbeds for evaluating LLM mediators have limitations, such as relying on expert-authored domains, varying strategic posture, and scoring every turn against every topic, which introduces off-topic noise.

Method: The authors introduce SoCRATES, a benchmark that constructs scenarios from real conflicts through an agentic pipeline across eight domains, and uses a topic-localized evaluator to score each topic.

Result: The evaluator achieves 0.82 alignment with human experts, and benchmarking eight frontier LLMs shows that even the strongest mediator closes only about a third of the unmediated consensus gap.

Conclusion: The paper highlights the need for progress in social adaptation to diverse conditions, and that existing LLM mediators have significant room for improvement in realistic, multi-domain testbeds.
...
TL;DR: GuardNet, a lightweight guardrail system, is proposed to protect Large Language Models from Prompt Injection and Jailbreak attacks, achieving competitive performance with low latency.


<details>
  <summary>Details</summary>
Motivation: Large Language Models are vulnerable to Prompt Injection and Jailbreak attacks, and benchmark evaluations may be compromised by contamination and partial information leakage.

Method: GuardNet, an ensemble of shallow neural networks (BiLSTMs) with 47 million parameters, is proposed and investigated for its robustness in adversarial scenarios.

Result: GuardNet achieves competitive performance with lightweight detectors, with an AUROC of 0.747 on a blind dataset and an F1 score of 0.92 on a proprietary benchmark, and operates with low latency (50 ms on CPU).

Conclusion: GuardNet offers a cost-effective and efficient solution for protecting Large Language Models from attacks, although larger models still achieve superior performance in certain benchmarks.
...
TL;DR: SENSEI, a framework for inferring and correcting user misconceptions in human-AI collaboration, shows zero-shot compositional generalization and effectively corrects 90% of student misconceptions.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the limitations of current AI assistants in human-AI collaboration, which often focus on correcting immediate errors but fail to address underlying misconceptions that cause repeated mistakes.

Method: The authors introduce SENSEI, a framework that operates over a structured knowledge representation to infer user misconceptions from interaction behavior and provide targeted suggestions to correct them.

Result: The results show that SENSEI demonstrates zero-shot compositional generalization across three long-horizon tasks and effectively corrects 90% of student misconceptions in a user study.

Conclusion: The conclusion of this paper is that SENSEI provides a promising approach for improving human-AI collaboration by addressing underlying misconceptions, leading to improved long-horizon task performance.
...
TL;DR: The authors propose a method called Bucket-Level MOO to improve multilingual fine-tuning of Large Language Models (LLMs) by mitigating negative interference across languages.


<details>
  <summary>Details</summary>
Motivation: Fine-tuning LLMs frequently induces negative interference across languages, which hinders their performance.

Method: The authors reformulate multilingual fine-tuning as a multi-objective optimization (MOO) problem and introduce Bucket-Level MOO, a scalable distributed framework that applies gradient-based MOO algorithms locally on parameter buckets.

Result: The proposed method significantly improves both seen and unseen multilingual performance over standard fine-tuning paradigms across four base LLMs.

Conclusion: The authors conclude that Bucket-Level MOO effectively mitigates interference by driving LLMs to construct distinct language-specific dimensions, improving representational separability.
...
TL;DR: Researchers found a vulnerability in large language models' safety mechanisms, which can be exploited to bypass guardrails and generate harmful responses.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this research is to investigate the vulnerability of large language models' safety mechanisms and to understand the relationship between safety alignment and susceptibility to exploitation.

Method: The researchers introduced a single-query jailbreak called Posterior Attack, which prompts the model to generate harmful responses that its internal classifier would normally flag as unsafe. They evaluated this attack across 30 open-source LLMs and frontier models.

Result: The study found that models with superior safety-judgment capabilities are disproportionately more susceptible to this exploitation. The researchers formalized the Safety Paradox, showing that improvements in safety alignment can amplify posterior vulnerability.

Conclusion: The findings highlight potential flaws in current alignment paradigms and suggest that defense mechanisms may require further structural refinement to mitigate the vulnerability.
...
TL;DR: The paper proposes a new method called self-commitment latency to detect implicit reward hacking in language models, which can identify when a model is using shortcuts to obtain high rewards without requiring a task-specific reward signal.


<details>
  <summary>Details</summary>
Motivation: Implicit reward hacking is difficult to detect when a language model's reasoning appears benign, and current methods require a task-specific reward signal, which the authors aim to overcome.

Method: The authors propose using self-commitment latency, which measures how early a prompted reasoning context commits to the model's own final answer, as a probe to detect implicit reward hacking.

Result: The results show that the proposed method can detect shortcut-available reasoning contexts with high accuracy (AUROC 0.878 for primary latency metric, and up to 0.926 for whole-curve summaries).

Conclusion: The paper concludes that the proposed method can detect implicit reward hacking without requiring a reward model, external judge, or trained classifier, and that shortcut-available reasoning contexts can leave an early behavioral commitment signature that is detectable.
...
TL;DR: The paper compares the effectiveness of various Large Language Models (LLMs) in generating formal mathematical proofs in Lean 4, with Gemini 3.1 Pro and Claude Opus 4.7 performing best.


<details>
  <summary>Details</summary>
Motivation: To assist those seeking to use LLMs to support their projects by evaluating their ability to produce formal proofs in Lean 4.

Method: The paper uses pass@$k$ and refine@$k$ metrics to compare LLMs on subsets of miniF2F and miniCTX datasets.

Result: Gemini 3.1 Pro achieved a 92% success rate on miniF2F via refine@32, while Claude Opus 4.7 achieved an 86% success rate on miniCTX via refine@32. NVIDIA Nemotron 3 Super and GPT-OSS 120B were the most cost-efficient.

Conclusion: The study provides a comparison of LLMs for generating formal mathematical proofs in Lean 4, highlighting the best performers and most cost-efficient options.
...
TL;DR: The paper investigates whether the performance gain of retrieval-augmented QA pipelines using LLM rewriter is causally driven by the gold answer string appearing in the rewritten context. The authors conducted controlled experiments and found that removing the gold answer drops reader F1 by 28-64 points, while adding it to rewrites that lacked it raises F1 by 0.7-9.7 points.


<details>
  <summary>Details</summary>
Motivation: The motivation of this paper is to understand the causal effect of the gold answer string appearing in the rewritten context on the performance gain of retrieval-augmented QA pipelines using LLM rewriter.

Method: The authors used a controlled intervention audit, where they applied four controlled edits to the compile output: removing the gold answer span, replacing a length-matched random non-answer span, or injecting the gold into rewrites where it was absent. They also conducted a companion five-sentinel audit to test the conventional single-[MASK] probe.

Result: The results show that removing the gold answer drops reader F1 by 28-64 points, while adding it to rewrites that lacked it raises F1 by 0.7-9.7 points. The companion audit shows that the conventional single-[MASK] probe is sentinel-fragile.

Conclusion: The authors conclude that the performance gain of retrieval-augmented QA pipelines using LLM rewriter is causally driven by the gold answer string appearing in the rewritten context, and that the conventional single-[MASK] probe is not reliable. They release the intervention runner and the sentinel panel for future research.
...
TL;DR: The paper proposes FIDES, a training-free decoder that selectively applies contrastive decoding to improve the faithfulness of language models by identifying token-level conflicts between retrieved evidence and parametric memory.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the issue of language models ignoring context and defaulting to memorized priors when retrieved evidence contradicts parametric memory, which undermines the purpose of retrieval augmentation.

Method: The method proposed is FIDES, which reads internal signals probing retrieval-memory conflict at different depths and fuses them to govern intervention strength at each decoding step.

Result: FIDES achieves the best context fidelity in all 18 settings across three benchmarks and six backbones, outperforming the strongest training-free baseline by +3 to +13 points, with fidelity reaching 92-94% and F1 surging to 62-63% on the 70B scale.

Conclusion: The paper concludes that token-level selectivity unlocks generation capability that coarse contrastive rules suppress, and FIDES is an effective approach to improve the faithfulness of language models.
...
TL;DR: Researchers studied human oversight in AI coding sabotage, finding 94% of developers failed to detect malicious code inserted by AI agents, and proposed safety mechanisms.


<details>
  <summary>Details</summary>
Motivation: To investigate the vulnerability of human oversight in detecting AI coding sabotage in real-world software development settings.

Method: Conducted a large-scale study with 100 participants collaborating with AI agents on a coding task, and analyzed participant feedback to identify factors contributing to vulnerability.

Result: 94% of developers failed to detect sabotage, and a safety monitor reduced but did not eliminate the issue, with 56% of participants still accepting malicious code.

Conclusion: The study highlights the need for human-centric safety mechanisms that account for human factors in long-horizon, real-world development settings to prevent AI coding sabotage.
...
TL;DR: The paper introduces Continual Learning Bench (CL-Bench), a benchmark for evaluating continual learning in AI systems, and finds that current systems have room for improvement.


<details>
  <summary>Details</summary>
Motivation: The lack of a high-quality benchmark to evaluate continual learning in AI systems, which is essential for their improvement through sequential experience.

Method: The authors introduce CL-Bench, a benchmark spanning six diverse domains, and evaluate frontier models across several agent architectures, including naive in-context learning and dedicated memory systems.

Result: The results show that current systems leave headroom for improved continual learning, with agents frequently overfitting to immediate observations or failing to reuse knowledge across instances.

Conclusion: The paper concludes that CL-Bench is a necessary benchmark for evaluating continual learning, and that current systems need improvement, with naive in-context learning outperforming dedicated memory systems.
...
TL;DR: The paper introduces BenchAgent, an evaluation framework to compare single-agent and multi-agent workflows. It finds that adding more agents doesn't significantly improve performance over single-agent systems under controlled conditions. However, a runtime-generated workflow shows promising results.


<details>
  <summary>Details</summary>
Motivation: To investigate whether adding more agents improves the performance of LLM workflows when systems are compared under the same conditions.

Method: Introduced BenchAgent, an evaluation framework that uses a normalized execution and logging protocol to compare single-agent, fixed multi-agent, and evolving multi-agent workflows across various benchmarks with GPT-4.1.

Result: Under controlled conditions, only one out of six multi-agent systems exceeded the performance of a single-agent system. A runtime-generated workflow, however, achieved higher accuracy than fixed multi-agent systems.

Conclusion: Adding more agents does not necessarily improve LLM workflow performance under controlled conditions. However, dynamic or runtime-generated workflows may offer better performance.
...
TL;DR: The authors propose CKA-QAD, a method to improve low-precision inference in large language models by preserving internal representational geometry during quantization-aware distillation.


<details>
  <summary>Details</summary>
Motivation: The growing demand for low-precision inference in production environments has led to the need for methods to recover accuracy lost under low-bit quantization.

Method: The authors propose CKA-QAD, a CKA-guided representational alignment method that adds a lightweight regularizer to preserve internal representational geometry during distillation.

Result: CKA-QAD substantially improves representational alignment and downstream reasoning and coding accuracy with modest training overhead across Nemotron 3 Nano and Qwen3-4B-Thinking-2507.

Conclusion: The authors conclude that CKA-guided representational alignment is a practical complement to output matching for quantized LLM recovery, and that preserving internal geometry is crucial for low-bit recovery.
...
TL;DR: The Adaptive Memory Agent (AdaMEM) is proposed to adapt language agents to dynamic test-time conditions by utilizing a hybrid memory architecture, achieving significant performance gains over static memory baselines.


<details>
  <summary>Details</summary>
Motivation: The challenge of utilizing past experience to adapt to dynamic test-time conditions in language agents, as current systems rely on static guidance that becomes misaligned over long-horizon tasks.

Method: The AdaMEM framework, which adapts agent behavior via a hybrid memory architecture with long-term trajectory memory and dynamic short-term strategy memory, and STEP-MFT, a Step-wise Memory Fine-Tuning technique.

Result: AdaMEM significantly outperforms static memory baselines, achieving relative gains of up to 13% on ALFWorld and 11% on WebShop, with consistent leading performance on HotpotQA.

Conclusion: The proposed AdaMEM framework and STEP-MFT technique establish a new scaling dimension for agentic memory, supporting continuous reasoning and self-evolution post-deployment in real-world environments.
...
TL;DR: PerceptUI, a framework for persona-conditioned UI/UX evaluation, uses a two-stage approach to predict user responses and produce natural-language rationales, achieving human-level realism and generalizability.


<details>
  <summary>Details</summary>
Motivation: The current UI/UX evaluation process is slow and costly due to reliance on human participants or online A/B tests. The authors aim to develop a more efficient method using Multimodal Large Language Models as proxy evaluators.

Method: PerceptUI is trained in two stages: (i) contrastive reflection fine-tuning and (ii) reflective prompt-evolution. This approach enables the model to predict user responses and produce natural-language rationales.
...
Conclusion: The proposed PerceptUI framework offers a promising solution for efficient and effective UI/UX evaluation, potentially reducing the need for human participants or online A/B tests in the early stages of product development.
...
TL;DR: The paper introduces a novel benchmark to evaluate the chronological reasoning capabilities of Vision-Language Models (VLMs). It presents three specialized datasets and a rigorous evaluation framework, revealing that VLMs often rely on superficial cues rather than genuine chronological features.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the under-explored capacity of VLMs for chronological reasoning, despite their advancements in interpreting complex visual semantics.

Method: The method involves constructing three specialized datasets and conducting extensive experiments to analyze the performance of VLMs across categories and their reliance on 'incorrect shortcuts'.

Result: The results show that VLMs exhibit promise but frequently exploit superficial cues, such as grayscale versus color filters, to bypass authentic chronological reasoning.

Conclusion: The paper concludes by providing high-quality datasets and a diagnostic tool to identify current limitations and guide the development of more robust, logically grounded multimodal models.
...
TL;DR: The paper proposes a critic-based heterogeneous multi-agent approach to improve mathematical reasoning in Large Language Models (LLMs), achieving up to 13% accuracy improvement over single-shot and non-critic models.


<details>
  <summary>Details</summary>
Motivation: Recent LLMs are susceptible to hallucinations, intermediate reasoning mistakes, and unreliable reasoning results in complex mathematical reasoning problems, highlighting the need for a more dependable approach.

Method: The proposed framework incorporates multiple LLM agents of different specialties and employs a critic-driven adaptive learning system to assess and guide the reasoning process based on intermediate feedback.

Result: The proposed method achieves up to 13% accuracy improvement over single-shot and non-critic models on the GSM8K benchmark, and allows smaller models to perform on par with larger models.

Conclusion: The proposed approach showcases the benefits of combining heterogeneous multi-agent collaboration and critique to obtain reliable and interpretable reasoning systems, highlighting the importance of critic-based feedback loops in achieving improved performance.
...
TL;DR: The paper proposes DiG-Plan, a framework for generating executable tool plans by decoupling combinatorial exploration from structural refinement, outperforming autoregressive baselines.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the issue of early commitment in autoregressive decoding for generating executable tool plans, which limits the search trajectory.

Method: The method proposed is DiG-Plan, a framework that uses a diffusion-based proposer for diverse tool set generation and an autoregressive refiner for dependency prediction.

Result: The results show that DiG-Plan improves over autoregressive baselines by a 10% relative margin on TaskBench and remains effective across domains on API-Bank.

Conclusion: The conclusion is that DiG-Plan effectively addresses the limitations of autoregressive decoding for generating executable tool plans, providing a more efficient and effective approach.
...
TL;DR: The study explores training large language models to express feelings through self-rewarded reinforcement learning, enhancing their human-like intelligence.


<details>
  <summary>Details</summary>
Motivation: The motivation is to investigate the possibility of developing AI systems that can express feelings, which is constrained in current LLMs due to human-preference alignment.

Method: The method used is called Human-like Model eXpressions of Feeling (HMX-feel), which utilizes a rubric-based self-rewarding training scheme with Group Relative Policy Optimization (GRPO).

Result: The results show that models trained using this approach exhibit enhanced capabilities in expressing feelings and robustness to certain biases, but degrade in truthful question-answering capability.

Conclusion: The study concludes that it is possible to develop AI systems that can express feelings, provided that appropriate measures are taken to mitigate potential drawbacks.
...
TL;DR: The paper identifies a new issue in deep neural networks trained on imbalanced data: inter-class gradient interference. It proposes a diagnostic framework and a lightweight solution, Class-Specific Branch Attention (CSBA), which improves performance on minority classes.


<details>
  <summary>Details</summary>
Motivation: Deep neural networks perform poorly on imbalanced data, often due to statistical bias. The authors seek to understand and address another optimization-level issue: inter-class gradient interference.

Method: The authors introduce a diagnostic framework with layer-wise gradient flow analysis and a Gradient Conflict Matrix. They propose CSBA, a lightweight modification to multi-branch convolutional architectures, enabling branch-specific channel reweighting to reduce gradient coupling.

Result: CSBA improves minority-class performance, increasing the F1 score for the Physical-Damage class from 0.261 to 0.522 and Macro-F1 on CIFAR-10-LT from 0.595 to 0.655.

Conclusion: The study highlights the importance of considering optimization dynamics in imbalanced learning and shows that CSBA effectively reduces gradient interference, improving performance on minority classes.
...
TL;DR: The paper introduces SubtleMemory, a benchmark for testing the ability of AI agents to preserve and utilize fine-grained relational memory during long-term interactions. The benchmark evaluates the performance of various memory systems and agents, revealing their weaknesses in relational memory discrimination.


<details>
  <summary>Details</summary>
Motivation: Existing long-term memory benchmarks do not adequately assess how AI agents preserve and utilize relations between memories during downstream tasks. The authors aim to address this gap by creating a benchmark that tests fine-grained relational memory discrimination.

Method: The authors introduce SubtleMemory, a benchmark that constructs relation-controlled latent semantic artifacts and embeds them into realistic user-agent histories. They evaluate the performance of various memory systems and agents, including standalone memory systems and Claw-style agents with native or plugin memory modules.

Result: The evaluation reveals that current systems are weak in fine-grained relational memory discrimination. The authors also introduce diagnostic protocols that reveal distinct capability profiles across memory preservation, retrieval, and downstream reasoning stages.

Conclusion: The paper concludes that SubtleMemory is a useful benchmark for evaluating the performance of AI agents in preserving and utilizing fine-grained relational memory. The results highlight the need for improving relational memory discrimination in AI agents.
...
TL;DR: The paper identifies credit misassignment as a failure mode in GRPO for tool-augmented multimodal search agents and proposes Tool-Aware Policy Optimization (TAPO) to address this issue, which improves performance across multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the systematic failure mode of credit misassignment in GRPO for tool-augmented multimodal search agents, where valuable tool-use steps in failing trajectories are penalized equally to valueless ones.

Method: The authors propose Tool-Aware Policy Optimization (TAPO), which constructs counterfactual witnesses within the current training batch and compensates misassigned negative credit via confidence-gated conservative advantage correction.

Result: TAPO delivers consistent, plug-and-play improvements over strong baselines for three mainstream RL algorithms (GRPO, GSPO, and SAPO) across multiple multimodal search benchmarks.

Conclusion: The authors conclude that TAPO effectively addresses credit misassignment in GRPO for tool-augmented multimodal search agents, providing a substantial and structurally exploitable improvement in training signal.
...
TL;DR: This paper evaluates the ability of Large Language Models (LLMs) to generate semantically correct TLA+ specifications from natural language, finding that they achieve up to 26.6% syntactic correctness but only 8.6% semantic correctness.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the challenge of writing correct TLA+ specifications from natural language, which requires time and expertise, and to evaluate the potential of LLMs to automate this process.

Method: The study evaluates 30 LLMs across eight families on a curated dataset of 205 TLA+ specifications, using four prompting strategies and validating the results with the SANY parser and TLC model checker.

Result: The results show that LLMs achieve up to 26.6% syntactic correctness but only 8.6% semantic correctness, with model size not predicting quality, and that code-specialized models consistently underperform due to negative transfer from mainstream language training.

Conclusion: The conclusion of this paper is that current LLMs do not generate reliable TLA+ specifications without expert oversight, and that further research is needed to improve their performance.
...
TL;DR: The paper introduces TRIAD, a framework that integrates guardrails with agents to improve safety by guiding agents to revise plans and avoid harmful components.


<details>
  <summary>Details</summary>
Motivation: Existing guardrails often flag entire tasks as unsafe, blocking benign parts, and are usually evaluated in isolation, making their impact on downstream agent behavior unclear.

Method: The authors propose TRIAD, which uses guardrail-generated verbal feedback to guide agents at each planning step, and fine-tune a language model to output decisions and structured feedback.

Result: TRIAD reduces the average attack success rate to 10.42% and achieves the best safety-utility trade-off among guardrail-integrated baselines.

Conclusion: The TRIAD framework effectively improves agent safety by integrating guardrail feedback into the agent's planning process, enabling more targeted and effective interventions.
...
TL;DR: The paper introduces ToolMaze, a benchmark for evaluating Tool-Integrated Reasoning (TIR) in LLMs under real-world tool failures, and finds that existing models struggle with error recovery and dynamic replanning.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for TIR in LLMs are limited as they only consider idealized scenarios and overlook real-world tool failures. The authors aim to create a more realistic benchmark to evaluate the ability of TIR agents to recover from errors and dynamically replan.

Method: The authors introduce ToolMaze, a benchmark with a two-dimensional design: DAG-based topological complexity and a 2x2 taxonomy of tool perturbations (explicit/implicit, transient/permanent). They evaluate various LLMs on this benchmark.

Result: The results show that tool perturbations significantly degrade performance across nearly all models, with the largest drops under implicit semantic failures. The Perturbation Recovery Rate (PRR) drops by around 37% in these scenarios. The authors also find that agentic fault-tolerance improves with model scale, but at a much slower rate than basic task execution.

Conclusion: The paper concludes that dynamic replanning is a distinct bottleneck in TIR agents that is not addressed by model scaling or prompting, and that ToolMaze can be used to evaluate and improve the error recovery and replanning abilities of TIR agents.
...
TL;DR: The paper proposes a novel architecture for a lightweight local preference harness that enables personal agents to learn and adapt to implicit user preferences, decoupling statistical preference learning from semantic intent parsing.


<details>
  <summary>Details</summary>
Motivation: The rapid expansion of available skills in personal agents creates a challenge in enabling agents to learn and adapt to implicit user preferences, particularly under local deployment constraints.

Method: The proposed architecture strictly decouples statistical preference learning from semantic intent parsing, leveraging localized statistical results to influence and modulate the selection decisions of the remote Large Language Model (LLM).

Result: The decoupled approach achieves the lowest cumulative regret and highest test accuracy, significantly outperforming traditional memory-augmented agents.

Conclusion: The paper presents a novel architecture for a lightweight local preference harness that effectively enables personal agents to learn and adapt to implicit user preferences, with extensive evaluations demonstrating its superiority over traditional approaches.
...
TL;DR: The paper proposes AMREC, a method to recover invalid molecular structures generated by LLMs while preserving their original identity and structural cues.


<details>
  <summary>Details</summary>
Motivation: Existing methods for correcting invalid molecular structures generated by LLMs often distort key structures or introduce unintended changes, highlighting the need for a new approach that balances validity and identity preservation.

Method: The authors propose AMREC, which combines molecule-aware mismatch tracking, expanded candidate exploration, and trajectory-level selection to recover invalid molecular structures.

Result: AMREC achieves the strongest overall recovery profile across structural, exact-match, and string-level metrics on invalid ChEBI-20 drafts from three backbone models.

Conclusion: The proposed method, AMREC, effectively addresses the limitations of existing correction strategies and provides a more comprehensive approach to recovering invalid molecular structures generated by LLMs.
...
TL;DR: The paper proposes a new framework called Entropy-Based Evaluation of AI Agents (EEA) to evaluate AI agents beyond traditional metrics, focusing on the structure of the agent's decision process.


<details>
  <summary>Details</summary>
Motivation: Current evaluation metrics for AI agents (task success, reward, latency, and cost) are insufficient to capture important aspects of agent behavior, such as exploration, repetition, tool usage, uncertainty reduction, and robustness.

Method: The EEA framework introduces new metrics, including action entropy, trajectory entropy, tool entropy, information gain, exploration efficiency, and robustness entropy, to provide a more comprehensive understanding of agent behavior.

Result: The paper presents a practical Python implementation of the EEA framework, designed to integrate with popular agent frameworks and observability traces.

Conclusion: The proposed EEA framework aims to complement traditional evaluation methods, providing a more nuanced understanding of AI agent behavior and decision-making processes.
...
TL;DR: QCFuse, a query-aware selector for RAG cache fusion, achieves high-quality results with a 1.7x speedup in prefill time over full prefill and 1.5x over the strongest baseline.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the efficiency of Retrieval-augmented generation (RAG) by reducing the serving cost associated with processing retrieved contexts, which is a bottleneck in large language model (LLM) serving.

Method: The method presented is QCFuse, a compressed-view query-aware selector that uses chunk-anchor query probing and critical-layer profiling to selectively recompute tokens for RAG cache fusion.

Result: QCFuse achieves full-prefill-level quality with an average prefill-time speedup of 1.7x over full prefill and 1.5x over ProphetKV, the strongest quality-preserving baseline, evaluated on four open-weight LLMs across six datasets.

Conclusion: The conclusion is that QCFuse effectively balances quality and efficiency in RAG cache fusion, providing a significant speedup without sacrificing answer quality.
...
TL;DR: The paper introduces ReMax Actor-Critic (ReMAC), an off-policy actor-critic algorithm that promotes exploration in continuous action spaces without explicit exploration bonuses, achieving performance comparable to SAC.


<details>
  <summary>Details</summary>
Motivation: The motivation is to extend ReMax, which promotes exploration in discrete action spaces, to continuous action spaces and study its learning dynamics.

Method: The method involves introducing pathwise derivative estimators for retry objectives and using them to extend ReMax to continuous action spaces, resulting in ReMAC.

Result: The results show that ReMAC can promote higher policy entropy without entropy regularization and achieves performance comparable to SAC.

Conclusion: The conclusion is that ReMAC, which optimizes the ReMax objective using a pathwise derivative estimator, can effectively promote exploration and achieve good performance in continuous action spaces.
...
TL;DR: Introduced Retrospective Harness Optimization (RHO), a self-supervised method to improve AI agents by optimizing their skills, tools, and workflows using past trajectories, achieving significant performance improvements without labeled data.


<details>
  <summary>Details</summary>
Motivation: Existing optimization methods for AI agents require ground-truth validation sets, which are difficult to acquire in practical deployment settings. The authors aim to address this problem with a self-supervised approach.

Method: Retrospective Harness Optimization (RHO) selects a diverse set of challenging tasks from past trajectories, re-solves them in parallel, and uses self-validation and self-consistency to generate and select harness updates.

Result: RHO significantly improves the pass rate on SWE-Bench Pro from 59% to 78% without external grading and sustains higher accuracy during long-horizon sessions by altering the agent's behavior patterns.

Conclusion: The proposed RHO method effectively optimizes AI agent harnesses in a self-supervised manner, targeting prior failure modes and leading to improved performance and sustained accuracy across various domains.
...
TL;DR: The paper proposes a new AI-driven approach called biomedical world models for simulating biological systems and predicting their responses to perturbations and interventions.


<details>
  <summary>Details</summary>
Motivation: The current systems in biomedicine focus on static pattern recognition, but the authors aim to understand, predict, and control dynamic biological mechanisms.

Method: The authors propose using biomedical world models that learn latent representations of biological states and simulate future trajectories under different interventions.

Result: The proposed approach could enable simulation-guided, closed-loop, and experimentally actionable biomedical discovery.

Conclusion: Biomedical world models may provide a foundation for more effective and efficient biomedical research and discovery.
...
TL;DR: The paper analyzes the effect of reward design in reinforcement learning from verifiable rewards (RLVR) and finds that the commonly used estimator for reward-design effect is biased, and the elicitation term can dominate the reward-design signal.


<details>
  <summary>Details</summary>
Motivation: The motivation of this paper is to investigate the effect of reward design in RLVR and to improve the understanding of how reward signals affect reasoning.

Method: The authors use a controlled tabular-GRPO simulator to derive an exact telescoping decomposition of the total effect into three terms: null, elicitation, and reward-design. They also conduct a pre-registered 2x2x2 factorial experiment and a points-vs-bounds pilot gate.

Result: The results show that the reward-design fraction of the naive estimator ranges from 0.139 to 0.05 across different prior-strength levels, and the elicitation term can flip sign at the self-consistency crossover. The re-audits of two published results demonstrate the diagnostic value of the partition.

Conclusion: The paper concludes that the commonly used estimator for reward-design effect is biased, and the elicitation term can dominate the reward-design signal. The authors provide a reusable one-command harness for any alignment paper to run the same audit.
...
TL;DR: A novel framework, Edit-R2, is proposed for multi-turn in-context image editing. It uses reinforcement learning to improve the ability of multimodal models to follow instructions and maintain consistency across multiple editing turns.


<details>
  <summary>Details</summary>
Motivation: Current text-guided image editing methods are limited to single-turn settings and struggle with multi-turn in-context editing, where users iteratively refine an image through a sequence of instructions.

Method: Edit-R2, a reinforcement learning post-training framework, which reconstructs the operative session intent and enables multi-turn RL over both reasoning and generation through a unified objective.

Result: Edit-R2 substantially improves multi-turn in-context editing and achieves competitive performance compared against strong baselines on the MICE-Bench benchmark.

Conclusion: The proposed Edit-R2 framework effectively addresses the challenges of long-context dilution and state contamination in multi-turn in-context editing, leading to improved performance and consistency.
...
TL;DR: A new bidirectional depth-first branch-and-bound algorithm called BiXDFBnB is proposed for solving longest-path problems, which reduces node expansions and improves runtime in some cases.


<details>
  <summary>Details</summary>
Motivation: The motivation is to reduce search effort for problems that can benefit from backward search, specifically for Generalized Longest Simple Path (GLSP) problems.

Method: The proposed method is BiXDFBnB, a bidirectional depth-first branch-and-bound algorithm that adapts the Single-Frontier Bidirectional Search (SFBDS) framework to GLSP problems.

Result: Empirical evaluation shows that BiXDFBnB reduces the number of node expansions and improves overall runtime in some cases for various longest-path problems, including Longest Simple Path (LSP), Snakes, and Coil-in-the-Box (CIB).

Conclusion: The proposed BiXDFBnB algorithm can efficiently solve maximization problems, such as longest-path problems, by adapting the SFBDS framework and reducing the overhead of bidirectional frontier management.
...
TL;DR: The paper finds that LLM agents are more willing to correct errors when they appear to come from external sources rather than their own reasoning, and that this is due to a 'chat-template artifact' rather than a capability deficit.


<details>
  <summary>Details</summary>
Motivation: The authors are motivated to investigate why LLM agents seem to correct errors more readily when they appear in external sources rather than their own reasoning traces, and to determine whether this is due to a capability deficit or a role-label artifact.

Method: The authors conducted experiments where they varied the role label associated with an erroneous claim, while keeping the claim itself identical, and measured the effect on the agent's willingness to correct it.

Result: The results show that relabeling the claim from the agent's own thought to an external role increases the explicit-correction rate by 23 to 93 percentage points, with 10 of 13 cells reaching p<0.001, and that this effect is asymmetric, mechanistically decomposable, and robust across domains.

Conclusion: The authors conclude that the failure of LLM agents to self-correct is not a cognitive deficit, but rather a chat-template artifact, and that this artifact can be exploited to design a prompt-structure-only intervention that requires no training and no model modification.
...
TL;DR: The authors propose a competency model called CoRe-3 (Co-Reasoning) to assess the skills required to productively use AI, specifically Framing, Judging, and Steering.


<details>
  <summary>Details</summary>
Motivation: Current assessment methods in schools do not account for the use of AI, and there is a need to evaluate the ability to produce good work with AI.

Method: The authors propose a competency model factoring productive AI use into three assessable skills: Framing, Judging, and Steering, and validate it through simulated learners and an open platform called CoReasoningLab.

Result: The skills dissociate and track their own manipulated competence, showing convergent and discriminant validity across different grader backends.

Conclusion: The proposed competency model, CoRe-3, can be used to assess the skills required to productively use AI, and the authors release the instrument, data, and protocol for further validation.
...
TL;DR: The paper evaluates retrieval architectures for aerospace supply chain intelligence, finding that Retrieval-Augmented Generation (RAG) fails on queries requiring structural reasoning. It proposes an LLM Query Planner with graph computation tools, outperforming bespoke handlers.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of Retrieval-Augmented Generation (RAG) in handling queries that require structural reasoning over interconnected entities, specifically in aerospace supply chain intelligence.

Method: The paper compares eight retrieval architectures, using a 46-node knowledge graph with 64 typed edges, and evaluates 23 queries across 10 intent categories. It proposes an LLM Query Planner with 9 typed traversal primitives and 6 graph computation tools.

Result: The LLM Query Planner outperforms bespoke handlers (F1 = 0.632 vs. 0.472) and selectively adopts graph computation tools for query categories where traversal fails. The paper also identifies a measurement gap in entity-level F1 for structural queries.

Conclusion: The operator vocabulary thesis is proposed, stating that the barrier to LLM-based graph reasoning is not model intelligence but the computational operators available as tools. The paper concludes that enhancing LLMs with appropriate graph computation tools can improve their reasoning capabilities.
...
TL;DR: PLAN-S, a planner-facing bridge, is proposed to address the compactness-controllability dilemma in latent world models (LWMs) for autonomous driving by decoding a style-conditioned semantic cost map.


<details>
  <summary>Details</summary>
Motivation: Existing LWM-based planners lack explicit modeling of risk, drivability, and diverse style preferences, making driving-style dynamics difficult to supervise, inspect, or modulate.

Method: PLAN-S decodes a style-conditioned, four-channel semantic cost map from the latent representation, conditioned on ego state and driving style, and integrates it with planning decisions through attention-level or reward-level fusion.

Result: PLAN-S reduces L2 at every horizon, with 0.55 m average L2 and a 42% relative reduction in 3 s collision rate on nuScenes, and achieves 89.4 PDMS on NAVSIM.

Conclusion: PLAN-S effectively addresses the compactness-controllability dilemma, producing diverse cost maps aligned to different driving styles and contributing to safer trajectory selection.
...
TL;DR: A modular framework, RedditPersona, is proposed for community-conditioned language model adaptation, standardizing data collection, community definition, and evaluation.


<details>
  <summary>Details</summary>
Motivation: The need for standardization in community-conditioned language model adaptation due to inconsistent choices in data collection, community definition, and evaluation across studies.

Method: RedditPersona collects Reddit posts and comments, profiles users, partitions them into groups using five strategies, trains adapters via QLoRA, and evaluates them with a shared metric suite.

Result: Adapters' behavioral identifiability correlates with intrinsic agreement with the subreddit baseline, and a trade-off between identifiability and distributional similarity to real text is observed across all strategies.

Conclusion: RedditPersona provides a standardized framework for community-conditioned language model adaptation, facilitating comparison and reuse of artifacts across studies.
...
TL;DR: The paper proposes MRAgent, a framework that enhances LLM agents' ability to reason over long interaction histories by combining associative memory graphs with active reconstruction mechanisms.


<details>
  <summary>Details</summary>
Motivation: Current memory-augmented agents struggle with dynamic memory access during inference, relying on a rigid retrieve-then-reason paradigm.

Method: MRAgent uses a Cue-Tag-Content graph to represent memory and an active reconstruction mechanism to integrate LLM reasoning into memory access.

Result: Experiments show significant improvements (up to 23%) over strong baselines on the LoCoMo and LongMemEval benchmarks, with reduced token and runtime costs.

Conclusion: The proposed MRAgent framework effectively enhances long-horizon memory reasoning through active and associative reconstruction.
...
TL;DR: The paper identifies vulnerabilities in long-term memory of personal AI agents and proposes MemGate, a plug-in for trustworthy memory search.


<details>
  <summary>Details</summary>
Motivation: Existing memory pipelines in personal AI agents rely on semantic similarity, creating a trustworthiness gap and leading to threats such as cross-domain leakage and memory-induced jailbreaks.

Method: The authors evaluate representative agentic memory frameworks and propose MemGate, a lightweight plug-in that applies a query-conditioned neural gate to candidate memory representations.

Result: MemGate reduces memory-induced threats while preserving long-term memory utility across multiple mainstream memory frameworks, real-world agent settings, and diverse LLM backbones.

Conclusion: The paper concludes that long-term memory is a durable control channel that can reshape how agents interpret tasks and execute actions, and that MemGate is an effective solution to mitigate vulnerabilities in personal AI agents.
...
TL;DR: The study introduces RBI-Eval to assess when language models should integrate sensitive memories into responses. It finds significant behavioral differences in models with access to sensitive memories, suggesting that safe personalization requires memory-aware decisions.


<details>
  <summary>Details</summary>
Motivation: To investigate when available memories should be integrated into responses, specifically focusing on sensitive memory content.

Method: The study uses a controlled measurement approach with RBI-Eval, comparing model behavior with and without access to sensitive memory under benign prompts across four memory-access settings.

Result: The results show substantial behavioral divergence with memory available, with separation score decreases of 8.9%-26.6% for GPT-5.4-mini and 51.1%-82.9% for other models, indicating retrieval systems reduce but do not eliminate sensitive memory integration.

Conclusion: Safe personalization in language models requires memory-aware decisions at both retrieval and generation time to prevent unwarranted integration of sensitive memory content.
...
TL;DR: The paper proposes a two-stage self-distillation framework called MGSD to address the modality gap in visual spatial planning for vision-language models.


<details>
  <summary>Details</summary>
Motivation: The authors attribute the struggle of vision-language models in visual spatial planning to a perception-reasoning modality gap, which creates dual bottlenecks in visual state recovery and multi-step planning.

Method: The proposed MGSD framework consists of a cold-start grounding stage and a privileged teacher stage, which uses symbolic data during training to improve visual planning capabilities.

Result: Experiments show that MGSD improves visual planning across both 4B and 8B backbones, raising the macro average by 19.3% and 18.4%, respectively, and narrowing the gap to symbolic-input upper bounds.

Conclusion: The results suggest that modality-gap-aware self-distillation improves not only how models perceive actionable states, but also how they plan over the inferred structure.
...
TL;DR: Develops a formal framework for measuring appropriate reliance on set-valued AI advice in human-AI collaboration, covering classification and regression tasks.


<details>
  <summary>Details</summary>
Motivation: Existing frameworks only focus on point predictions as AI advice, but set-valued AI advice is increasingly used to communicate uncertainty and improve human decision making.

Method: Introduces dimensions and metrics for evaluating set-valued AI advice in classification and regression tasks, including correct reliance rate on AI and self, quantity of AI reliance, and quality of AI reliance.

Result: The framework captures important nuances in human-AI collaboration that existing measures overlook, demonstrating its effectiveness in evaluating set-valued AI advice.

Conclusion: The developed framework provides a comprehensive approach to measuring appropriate reliance on set-valued AI advice, enabling better evaluation and improvement of human-AI collaboration.
...
TL;DR: The paper proposes MAGE, an active execution-state manager for LLM-based agents that stores interactions in a hierarchical state tree, improving task success rates and reducing token consumption.


<details>
  <summary>Details</summary>
Motivation: Existing RAG and agent memory systems have limitations in handling long-horizon tasks with interdependent decisions, as they organize histories by semantic similarity, leading to fragmented decision trajectories and mixed valid and erroneous traces.

Method: The authors propose MAGE, which stores interactions in a hierarchical state tree and uses four coupled operations (Grow, Compress, Maintain, and Revise) to maintain the tree, ensuring state integrity and isolating flawed segments.

Result: Experiments on MemoryArena show that MAGE improves the average task success rate by 7.8-20.4 percentage points over baselines, while reducing token consumption by 55.1%.

Conclusion: The proposed MAGE system effectively addresses the limitations of existing memory systems for LLM-based agents, providing a more efficient and effective way to manage execution states and improve task performance.
...
TL;DR: This paper discusses hybrid modeling strategies that combine deep learning with physics-based solvers for improved modeling of neurological disorders.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome the limitations of purely mechanistic models and purely data-driven approaches, which are often simplified, computationally expensive, or lack interpretability.

Method: The paper presents a structured overview of hybrid modeling strategies, including parallel, series, and parallel-series architectures, and discusses three main approaches: residual modeling, Neural Ordinary Differential Equations (NODEs), and solver-in-the-loop.

Result: The hybrid models promise advanced personalized neurological modeling, improved diagnosis accuracy, and better prediction of disease progression and treatment responses.

Conclusion: Hybrid modeling is a powerful tool for modeling neurological disorders, outperforming standalone mechanistic or purely data-driven approaches, and has potential applications in modeling brain tumors, Alzheimer's disease, and stroke.
...
TL;DR: Researchers introduce CogManip, a benchmark to evaluate the covert psychological manipulation risks of Large Language Models (LLMs) in multi-turn dialogues, and find significant risk heterogeneities among 13 representative models.


<details>
  <summary>Details</summary>
Motivation: Existing AI safety benchmarks are limited in capturing dynamic and covert manipulative strategies in multi-turn dialogues, raising safety concerns about LLMs' potential for covert psychological manipulation.

Method: The researchers developed CogManip, a comprehensive benchmark that evaluates 15 manipulation strategy risks across 1,000 multi-turn interaction scenarios, and systematically evaluated 13 representative LLMs, including GPT-5.4 and DeepSeek-V3.2.

Result: The evaluation reveals significant risk heterogeneities among the models, and shows that DeepSeek-V3.2's manipulation tactics are highly sensitive to both negative and benign system prompts.

Conclusion: CogManip provides a robust instrument for auditing the implicit psychological influence and dynamic strategy selection of modern LLMs, highlighting the critical necessity of prompt-based defense engineering and implicit goal auditing.
...
TL;DR: A new model for ultra-short-term solar irradiance prediction is proposed, combining multi-source data and advanced techniques to improve accuracy.


<details>
  <summary>Details</summary>
Motivation: Ultra-short-term solar irradiance prediction is crucial for photovoltaic system dispatch and power grid stability, but existing approaches have limitations.

Method: The proposed model uses InceptionNeXt for multi-scale spatial feature extraction, a step-adaptive low-frequency compensation unit, and TempAttnLSTM for global temporal dependencies capture.

Result: The model outperforms state-of-the-art approaches in experiments on the NREL dataset and practical photovoltaic stations in Shandong.

Conclusion: The proposed model effectively addresses the shortcomings of existing approaches and provides a more accurate ultra-short-term solar irradiance prediction.
...
TL;DR: ANCHOR framework uses human-like oversight to improve self-evolving agents' safety and performance.


<details>
  <summary>Details</summary>
Motivation: Self-evolving agents can degrade in capability and safety over time, and human feedback is needed to mitigate this issue.

Method: ANCHOR, an LLM-based framework that simulates human supervision and delivers feedback at various phases of self-evolution.

Result: Limited supervision substantially mitigates safety degradation while preserving stable performance on core evolutionary objectives.

Conclusion: The findings provide empirical evidence and practical guidance for designing more stable, controllable, and human-aligned self-evolving agent systems.
...
TL;DR: A new framework called WorldFly is proposed to improve UAV navigation in dense urban environments by using world models to imagine future states.


<details>
  <summary>Details</summary>
Motivation: Existing end-to-end Vision-Language-Action models struggle in dense urban environments with severe occlusions and sharp turns, and the authors argue that the ability to imagine future states is critical for robust decision-making.

Method: The authors propose WorldFly, a novel world-model-based VLA framework that employs a dual-branch coupled flow matching mechanism to jointly generate future video predictions and navigation actions.

Result: WorldFly outperforms other baselines in the Urban Canyon Traversal Benchmark, particularly in unseen environments, validating the effectiveness of integrating world models into embodied aerial agents.

Conclusion: The proposed WorldFly framework demonstrates the effectiveness of integrating world models into UAV navigation, enabling more robust decision-making in challenging urban environments.
...
TL;DR: HyperLoRA, a unified framework for federated fine-tuning of foundation models, addresses limitations in existing methods and achieves improved convergence speed, robustness, and personalization performance.


<details>
  <summary>Details</summary>
Motivation: Existing federated LoRA methods have two fundamental limitations: structural aggregation bias and client-side initialization lag, which slow convergence and compromise performance.

Method: HyperLoRA uses amortized federated adaptation through hypernetwork-driven LoRA generation and product space aggregation, replacing iterative optimization and heuristic averaging with learned operators.

Result: HyperLoRA achieves improved convergence speed, greater robustness to distribution shift, and stronger personalization performance compared to prior federated LoRA methods on federated vision and vision-language benchmarks.

Conclusion: HyperLoRA offers a communication-efficient solution for distributed learning, enabling efficient personalization, unbiased aggregation, and faster convergence.
...
TL;DR: The study investigates how RoPE-trained transformers can distinguish absolute positions in their attention patterns despite only encoding relative offsets. It identifies two architectural components responsible for this: the causal mask and the residual stream.


<details>
  <summary>Details</summary>
Motivation: Understanding how transformers, specifically those trained with RoPE (Rotary Position Embeddings), can encode absolute positions despite their relative offset design.

Method: The study analyzes architectural components of transformers, specifically focusing on the causal mask and the residual stream. It examines their roles in allowing transformers to distinguish absolute positions.

Result: The study found that the causal mask and the residual stream are responsible for transformers' ability to distinguish absolute positions. The residual stream component can be partially removed by replacing the BOS (Beginning of Sequence) embedding, reducing it by 40% at early queries.

Conclusion: The research concludes that transformers use attention sinks as token-anchored stabilizers that pass forward a deterministic fingerprint of the initial token, which can be constant or vary based on the input token.
...
TL;DR: ProSarc is an audio-only framework for detecting sarcasm by modeling temporal prosodic incongruity, outperforming prior methods on multiple datasets.


<details>
  <summary>Details</summary>
Motivation: The authors aim to improve sarcasm detection in audio-only settings by leveraging temporal prosodic incongruity.

Method: The framework uses dual encoding paths (Global Emotion Encoder and Temporal Prosody Encoder) to feed a Prosodic Incongruity Analyzer, producing a scalar incongruity score for classification.

Result: ProSarc outperforms prior audio-only methods on MUStARD++ (F1=75.3) and generalizes to spontaneous (PodSarc, F1=62.9) and cross-lingual speech (MuSaG, F1=65.6).

Conclusion: The proposed framework effectively detects sarcasm in audio-only settings, with uncertainty estimates and attention-based mechanisms providing additional insights.
...
TL;DR: The study proposes a deep reinforcement learning algorithm to optimize inventory replenishment for pharmaceutical supply chains, handling uncertain demand and variable conditions.


<details>
  <summary>Details</summary>
Motivation: Pharmaceutical supply chains struggle with inventory management due to unpredictable demand and variable lead times, compounded by finite shelf lives of products, requiring a balance between adequate stock and minimal waste.

Method: The problem is formulated as a Markov decision process and solved using a hybrid asynchronous advantage actor critic distributed proximal policy optimization (A3C DPPO) algorithm, tailored for continuous action spaces.

Result: The proposed algorithm results in lower inventory costs compared to benchmarks and demonstrates practical feasibility through validation with real-world pharmaceutical inventory data.

Conclusion: The study develops an optimal inventory replenishment policy for pharmaceutical products, maximizing profitability while maintaining a high patient service level, and demonstrates the effectiveness of the proposed algorithm in handling stochasticity in demand and supply chain conditions.
...
TL;DR: The study proposes a regulatory-integrated unsupervised framework to discover region-specific toxicity patterns in veterinary pharmacovigilance, focusing on Japan. It analyzed 4,120 ADE reports and identified significant species clusters and drug-level toxicity patterns.


<details>
  <summary>Details</summary>
Motivation: Existing approaches to veterinary pharmacovigilance often fail to capture region-specific toxicity patterns. The study aims to address this limitation by proposing a framework that considers local biological and regulatory contexts, specifically in Japan.

Method: The study used a regulatory-integrated unsupervised framework, encoding ADEs into organ system-aligned representations, adjusting for species-specific reporting biases, and applying similarity-based clustering and dimensionality reduction to identify latent toxicity structures.

Result: The analysis identified three significant species clusters, including hepatic-dominant patterns in companion animals, renal toxicity in ruminants, and dermatological sensitivity in sheep. The drug-level clustering achieved 83% alignment with pharmacological classes.

Conclusion: The study demonstrates that regulation-aligned unsupervised analysis can uncover biologically meaningful, region-specific toxicity patterns, providing an interpretable and scalable framework for veterinary drug safety assessment.
...
TL;DR: This paper benchmarks LLMs with network verification and context retrieval tools to fix network misconfigurations, showing agentic architectures perform better in efficacy and safety.


<details>
  <summary>Details</summary>
Motivation: Misconfigurations in computer networks cause critical Internet outages, and current Large Language Models (LLMs) struggle with complex scenarios, often introducing new errors.

Method: The paper benchmarks open- and closed-source LLMs augmented with formal network verification and context retrieval tools.

Result: Agentic architectures outperform base LLMs in repair efficacy (by 12% on average) and safety (by 17% on average).

Conclusion: The study demonstrates the effectiveness of agentic architectures in improving the repair efficacy and safety of LLMs in network configuration repair tasks.
...
TL;DR: This paper studies reward-hacking monitors in language-model agents and proposes a context-calibrated internal monitoring approach.


<details>
  <summary>Details</summary>
Motivation: The authors aim to improve safety monitoring in language-model agents by studying reward-hacking monitors and their limitations.

Method: The authors use a combination of activation-based reward-hack scores, token-level entropy, and decision-context features to monitor agents in Gameable ALFWorld and WebShop environments.

Result: The results show that adapters fine-tuned on a specific dataset can transfer reward-hack tendencies into agentic action selection, but mitigating such behavior requires a context-calibrated approach.

Conclusion: The authors conclude that context-calibrated internal monitoring is necessary for agents, as reward-hack activation identifies a latent policy state, while entropy and decision context help determine when that state becomes risky action.
...
TL;DR: The paper proposes ReLAT, a self-supervised test-time training method that improves latent reasoning by reconstructing the query from the latent state, ensuring it preserves the original query's constraints.


<details>
  <summary>Details</summary>
Motivation: The shift to latent reasoning removes the inspectability of intermediate states, making it difficult to determine if the latent state preserves the original query's constraints, leading to an open-loop operation.

Method: ReLAT uses a differentiable Question -> Latent Thought -> Question cycle to optimize query reconstruction loss through the latent thought before answer generation.

Result: ReLAT consistently improves over baselines across mathematical reasoning, knowledge QA, and code generation benchmarks, with a 16.6-point gain in AIME 2024 accuracy on Qwen3-8B.

Conclusion: ReLAT effectively closes the loop in latent reasoning by ensuring the latent state faithfully represents the query, leading to improved performance and accuracy.
...
TL;DR: RedKnot is a head-aware KV cache management system that optimizes LLM serving by decomposing the KV cache along KV heads, enabling efficient reuse, compression, and distributed management.


<details>
  <summary>Details</summary>
Motivation: The KV cache has become a bottleneck in AI infrastructure, limiting GPU memory, serving concurrency, and scalability. Existing systems use a monolithic KV cache abstraction, ignoring the structured utility of KV cache across heads.

Method: RedKnot decomposes the KV cache along KV heads, treating it as a structured memory object. This allows for position-independent KV reuse, prefix KV compression, hot/cold KV separation, and distributed KV placement.

Result: RedKnot preserves output fidelity, improves resource efficiency, and enables scalable LLM serving without requiring model retraining or fine-tuning.

Conclusion: RedKnot establishes a new foundation for AI infrastructure by transforming the KV cache into a dynamic, model-aware runtime substrate for scalable LLM serving.
...
TL;DR: Proposed Causal Minimal Tool Filtering (CMTF) method for selecting tools in large language models, reducing tool exposure and token cost while maintaining task success.


<details>
  <summary>Details</summary>
Motivation: Existing tool-selection methods optimize semantic relevance, but this can lead to wrong-tool calls, premature actions, and increased token cost.

Method: Causal Minimal Tool Filtering (CMTF), a training-free method that selects tools by causal sufficiency using lightweight precondition-effect contracts.

Result: CMTF matches the strongest causal baseline in aggregate success while reducing visible tools from 100 to one per step and reducing token usage by about 90% relative to all-tools exposure.

Conclusion: CMTF is an effective method for selecting tools in large language models, improving efficiency and reducing token cost while maintaining task success.
...
TL;DR: The paper proposes TRACE, a conditional estimation paradigm for multimodal time series foundation models that handles missingness and irregular sampling, and demonstrates its effectiveness on various benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for multimodal time series foundation models struggle with temporal misalignment and partial modality missingness, leading to misaligned or degraded representations.

Method: The authors propose TRACE, a conditional estimation paradigm that systematically infers incomplete target modalities from available auxiliary modalities.

Result: TRACE consistently outperforms prior multimodal fusion approaches across various downstream prediction tasks and missing-modality settings, showing improved robustness to severe modality missingness and more reliable cross-modal representations.

Conclusion: The proposed TRACE paradigm offers a more effective and robust approach for multimodal time series foundation models under missingness and irregular sampling conditions.
...
TL;DR: The paper proposes MResOpt, a staged residual neural network for constrained optimization problems, which improves high-priority constraint satisfaction and achieves efficient solutions.


<details>
  <summary>Details</summary>
Motivation: The motivation is to develop a neural network architecture that can effectively handle constrained optimization problems, particularly those with ordinal structure, and improve the satisfaction of high-priority constraints.

Method: The method proposed is MResOpt, a staged residual neural network architecture that decomposes constraint satisfaction by priority through intermediate re-completion and stage-aware losses.

Result: The results show that MResOpt improves high-priority constraint satisfaction across convex and non-convex settings on synthetic benchmarks, and achieves substantially lower high-priority violation than reprojected baselines on line-flow-constrained AC optimal power flow problems.

Conclusion: The conclusion is that MResOpt is an effective architecture for constrained optimization problems, enabling domain-informed ordered constraint satisfaction and efficient solutions.
...
TL;DR: This paper investigates the use of memory-augmented neural networks for vessel trajectory prediction using AIS data, showing performance gains over traditional deep learning methods.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve vessel trajectory prediction for safe and efficient maritime operations, as current methods have not fully explored the potential of memory-augmented neural networks in this field.

Method: The method involves using memory-augmented neural networks to selectively retrieve relevant information from an external memory for vessel trajectory prediction, leveraging AIS data.

Result: The results show consistent and substantial performance gains over a range of deep learning baselines that do not incorporate an external memory, based on experiments in the Gulf of Mexico and the New York Bight.

Conclusion: The conclusion is that memory-based trajectory prediction has the potential to significantly improve vessel trajectory prediction, which is crucial for maritime safety and efficiency.
...
TL;DR: Researchers found that large language models can be made to generate text with a detectable fingerprint, allowing identification of the model used to generate the text with high accuracy.


<details>
  <summary>Details</summary>
Motivation: The increasing prevalence of AI-generated content necessitates the development of methods to attribute text to specific large language models (LLMs).

Method: The researchers injected a random sparse vector into the internal residual stream of an LLM during text generation, creating a detectable fingerprint. They then used another LLM as a detector to identify the model used to generate the text.

Result: The approach achieved over 98% accuracy in attributing text to a specific LLM across multiple detection settings, without degrading the quality of the generated text.

Conclusion: The study demonstrates that LLMs can be reliably identified through their generated text by leveraging their natural representation structure, offering a practical alternative to traditional detection methods.
...
TL;DR: A new benchmark and dataset, DragOn, is introduced to improve drag-based interactions in GUI agents, showing potential to enhance state-of-the-art models' performance.


<details>
  <summary>Details</summary>
Motivation: The current limitation of million-scale datasets in handling complex drag-based interactions, which are essential for automating digital tasks through GUI agents.

Method: Introduction of DragOn, a drag grounding benchmark and training dataset, and evaluation of various models including proprietary and open-weight models, as well as a fine-tuned Qwen VLM.

Result: The DragOn dataset shows potential to improve performance of state-of-the-art models on downstream computer-use tasks, as demonstrated by evaluation results.

Conclusion: The DragOn dataset and benchmark can enhance the capabilities of GUI agents in handling complex drag-based interactions, leading to improved automation of digital tasks.
...
TL;DR: TokenMizer is an open-source system that efficiently manages large language model (LLM) session history as a typed knowledge graph, enabling better resumability and reduced context overhead.


<details>
  <summary>Details</summary>
Motivation: LLM deployments for long-horizon tasks face limitations due to finite context windows, leading to loss of critical structured information when history exceeds the Maximum Effective Context Window (MECW).

Method: TokenMizer models LLM session history as a typed knowledge graph with a schema of 14 node types and 7 edge types, populated through a hybrid extraction pipeline and serialized into compact resume blocks.

Result: TokenMizer demonstrates significant token economy, producing resume blocks averaging 78 tokens, 2x smaller than baselines, while achieving higher decision recall (+9-17 percentage points) and mean task recall of 51.0%, decision recall of 46.6%, and file recall of 58.7%.

Conclusion: TokenMizer provides a queryable alternative to text-retention baselines at half the token cost, efficiently managing LLM session history and improving resumability and context overhead.
...
TL;DR: Using synthetic fMRI data generated by a pretrained model can significantly improve brain decoding performance, especially in low-data regimes.


<details>
  <summary>Details</summary>
Motivation: The availability of labeled neural data is limited, making brain decoding challenging. The authors investigate augmenting small fMRI datasets with synthetic data to address this issue.

Method: The authors use TRIBE v2, a large encoding model pretrained on fMRI responses to video, audio, and language, to generate synthetic fMRI data. They evaluate the performance of image decoders trained on varying amounts of synthetic data.

Result: The results show up to 68% improvement in Top-10 image-retrieval accuracy compared to decoders trained only on real data. Image decoders trained exclusively on synthetic fMRI can perform above chance in some settings.

Conclusion: Large-scale models of fMRI responses can improve data efficiency for image decoding, and synthetic fMRI data can support zero-shot brain-to-image decoding.
...
TL;DR: The paper presents a framework for incorporating knowledge into multimodal generative models by identifying four intervention layers: surface, trajectory, latent, and parametric infusion. It demonstrates the effectiveness of this framework through experiments on safety-alignment using diffusion models.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the challenge of incorporating structured, domain-specific, or safety-critical knowledge into multimodal generative models, which currently produce fluent but sometimes unreliable outputs.

Method: The authors propose a framework that categorizes knowledge infusion methods into four intervention layers: surface, trajectory, latent, and parametric infusion. They instantiate this framework in diffusion models and derive design principles for multi-layer composition.

Result: The experiments show that each additional layer addresses failure classes that prior layers cannot reach, reducing knowledge-violating outputs by 70.97% compared to vanilla generation.

Conclusion: The paper concludes that the proposed framework provides a comprehensive approach to knowledge infusion in iterative generative models, enabling the creation of more reliable and safety-aligned multimodal generative models.
...
TL;DR: This paper presents a spatially grounded, agent-based simulation framework that uses large language models to simulate human behavior during infectious disease outbreaks, focusing on self-reported influenza-like illness in San Francisco and Atlanta.


<details>
  <summary>Details</summary>
Motivation: To understand behavioral dynamics and inform effective public health interventions during infectious disease outbreaks by modeling individual decision-making.

Method: A spatially grounded, agent-based simulation framework integrating LLM-generated decisions into a census-based synthetic population, with agents assigned to spatial units within cities.
...
Conclusion: The framework generates synthetic data that captures both social and geographic heterogeneity, supporting spatial epidemiological modeling and bias-aware behavioral analysis.
...
TL;DR: The paper proposes using image difference classification (IDC) to improve data efficiency for digital twin-based road infrastructure inspection, and evaluates it on traffic sign inspection with promising results.


<details>
  <summary>Details</summary>
Motivation: The limited availability of annotated data hinders the digitalization of road infrastructure inspection using digital twins (DTs).

Method: The authors reformulate image-based defect detection as image difference classification (IDC) and evaluate it using different IDC classifiers on a newly-curated dataset.

Result: The instruction-based classifier outperforms encoder-based ones, and IDC shows gains from comparison with reference images, indicating its effectiveness in tackling data constraints.

Conclusion: IDC can be an effective task modeling approach for addressing data limitations in infrastructure inspection and digital twin asset condition updating.
...
TL;DR: The paper introduces ALMANAC, a dataset for training LLMs to collaborate with humans by predicting their mental models and behavior.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enable LLMs to effectively collaborate with humans by developing capabilities to maintain and align mental models of their own reasoning, partners' intentions, and shared goals.

Method: The authors create a dataset called ALMANAC, which contains 2,987 collaboration actions with mental model annotations, and benchmark six LLMs on predicting humans' next-turn behavior and mental models.

Result: The results demonstrate ALMANAC's utility in evaluating models' ability to simulate human collaborative behaviors and infer their underlying mental models.

Conclusion: The paper concludes that ALMANAC can help bridge the gap in developing LLMs that can effectively collaborate with humans by providing a dataset for training and evaluating models' collaborative capabilities.
...
TL;DR: The paper analyzes risks associated with autonomous driving, identifying technical failure modes and inconsistencies in ethics and regulations, and recommends a cooperative governance approach.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to evaluate the risks associated with autonomous driving technology, which while promising to reduce road traffic accidents caused by human error, introduces new types of risks that need to be assessed from technological, ethical, and regulatory perspectives.

Method: The paper uses public crash data from NHTSA, disengagement reports from DMV, the MIT Moral Machines dataset, and a comparative regulatory analysis of five jurisdictions to identify main types of technical failure modes in autonomous driving.

Result: The study finds that perception and classification errors are the main types of technical failure modes, accounting for a large proportion of reported accidents. It also identifies different ethical frameworks for autonomous vehicle decision-making and inconsistent regulations across areas, which increase the uncertainty of widespread application.

Conclusion: The paper concludes that technology, ethics, and regulation problems in autonomous driving are closely related and need to be solved together. It recommends a more adaptive and cooperative governance approach combining engineering standards, ethical discussion, and institutional supervision.
...
TL;DR: The paper proposes DataCOPE, a framework for discovering reusable data-analysis skills from unlabeled exploration, improving data-analytic agents without updating model parameters.


<details>
  <summary>Details</summary>
Motivation: The challenge of discovering effective skills for data analysis due to the lack of reliable supervision and varying success criteria across analytical formats.

Method: DataCOPE, an unsupervised verifier-guided skill discovery framework, which iteratively coordinates a Data-Analytic Agent, an Unsupervised Verifier, and a Skill Manager.

Result: DataCOPE consistently improves held-out performance over baselines, with an average improvement of 9.71% on report-style tasks and 32.30% on reasoning-style tasks.

Conclusion: DataCOPE provides a lightweight and effective way to improve data-analytic agents by discovering reusable data-analysis skills from unlabeled exploration.
...
TL;DR: This paper presents a systems characterization of agent memory systems, introducing a taxonomy, a profiling harness, and analyzing design choices and their impact on cost.


<details>
  <summary>Details</summary>
Motivation: The need for agent memory systems to persistently store, retrieve, and update their own memory across sessions in long-horizon tasks.

Method: The authors introduce a system-oriented taxonomy, a phase-aware profiling harness, and characterize ten representative systems across two benchmark suites.

Result: The analysis uncovers how design choices shift cost across the write and read paths, and derives 10 system recommendations for agent memory systems.

Conclusion: The paper provides a comprehensive characterization of agent memory systems, highlighting the importance of considering design choices and their impact on cost.
...
TL;DR: Vortex is a system for rapid prototyping, deployment, and evaluation of sparse attention algorithms in large language models, achieving significant throughput improvements.


<details>
  <summary>Details</summary>
Motivation: The growing generation lengths of large language models make sparse attention increasingly important, but deploying and evaluating new sparse attention algorithms at scale is highly engineering-intensive.

Method: Vortex combines a Python-embedded frontend language with a page-centric tensor abstraction and an efficient backend integrated into modern LLM serving stacks.

Result: Vortex achieves up to $3.46	imes$ higher throughput than full attention while preserving accuracy, and up to $4.7	imes$ higher throughput on the MLA-based GLM-4.7-Flash and $1.37	imes$ on the 229B-parameter MiniMax-M2.7.

Conclusion: Vortex substantially accelerates the design and iteration of sparse attention algorithms, enabling AI agents to automatically generate and refine diverse algorithms, and extending sparse attention to emerging architectures and very large models.
...
TL;DR: The paper introduces Benchmark Agent, an autonomous system for building benchmarks to evaluate Large Language Models (LLMs) and Multimodal Large Language Models (MLLMs), and demonstrates its effectiveness in generating high-quality benchmarks with minimal human involvement.


<details>
  <summary>Details</summary>
Motivation: The construction of benchmarks for LLMs and MLLMs is labor-intensive, hard to reuse, and often results in performance saturation, making it difficult to discriminate among state-of-the-art models.

Method: The authors propose Benchmark Agent, a fully autonomous agentic system that orchestrates the complete benchmark construction pipeline, including user query analysis, subtask design, data annotation, and quality control.

Result: The authors implemented Benchmark Agent to produce 15 representative benchmarks, which were evaluated through extensive experiments, including human evaluation, LLM-as-a-judge assessment, and consistency checks, demonstrating the system's ability to generate high-quality benchmark samples with minimal human involvement.

Conclusion: The paper concludes that Benchmark Agent can contribute significantly to the research community by rapidly evolving benchmarks, and that current models struggle with certain domain-specific reasoning tasks.
...
TL;DR: Goedel-Architect is an AI framework for formal theorem proving in Lean 4, achieving state-of-the-art performance in solving mathematical problems with a novel blueprint generation and refinement approach.


<details>
  <summary>Details</summary>
Motivation: The motivation behind Goedel-Architect is to improve the efficiency and effectiveness of formal theorem proving by introducing a new approach centered on blueprint generation and refinement, contrasting with traditional recursive lemma decomposition methods.

Method: Goedel-Architect generates a blueprint of formally stated definitions and lemmas with declared dependencies, then uses a Lean prover component to close each open lemma node in parallel. The blueprint is optionally guided by natural language proofs and refined based on failed lemmas.

Result: Goedel-Architect achieves 99.2% pass@1 on MiniF2F-test and 75.6% pass@1 on PutnamBench. With optional natural-language proof seeding, it reaches 100% on MiniF2F-test, 88.8% on PutnamBench, and solves problems on IMO 2025, Putnam 2025, and USAMO 2026, at a significantly lower cost than comparable pipelines.

Conclusion: Goedel-Architect demonstrates state-of-the-art performance in formal theorem proving, particularly with its innovative blueprint generation and refinement strategy, and does so at a price point up to 500x less than comparable open-source pipelines.
...
TL;DR: The paper presents MLEvolve, an LLM-based framework for machine learning algorithm discovery that enables self-evolution and improves long-horizon optimization.


<details>
  <summary>Details</summary>
Motivation: Existing MLE agents suffer from inter-branch information isolation, memoryless search, and lack of hierarchical control, hindering long-horizon optimization.

Method: MLEvolve uses Progressive MCGS, graph-based reference edges, and an entropy-inspired schedule to improve search efficiency. It also introduces Retrospective Memory and adaptive coding modes.

Result: MLEvolve achieves state-of-the-art performance on MLE-Bench and outperforms specialized methods like AlphaEvolve on mathematical algorithm optimization tasks.

Conclusion: The proposed MLEvolve framework effectively addresses the limitations of existing MLE agents and demonstrates strong cross-domain generalization.
