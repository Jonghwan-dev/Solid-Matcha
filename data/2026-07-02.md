...
TL;DR: AtmoFuseNet, a novel framework, fuses multi-view camera imagery, millimeter-wave cloud radar, and ceilometer observations to estimate 4D cloud state and wind, achieving improved accuracy over existing methods.


<details>
  <summary>Details</summary>
Motivation: Dense volumetric reconstruction of cloud microphysical fields from sparse ground-based instruments is an open problem due to heterogeneous measurements.

Method: AtmoFuseNet operates in three stages: cross-modal hierarchical aggregation, conditional variational refinement, and correlation-based motion estimation.

Result: AtmoFuseNet achieves 0.026 g m^-3 liquid water content MAE and 1.18 m s^-1 wind speed MAE, outperforming existing retrieval baselines.

Conclusion: AtmoFuseNet effectively fuses multi-modal observations to produce accurate 4D estimates of cloud state and wind, demonstrating its potential for cloud microphysical field reconstruction.
...
TL;DR: Proposed a streaming Gaussian encoder for 4D panoptic occupancy tracking, achieving temporal coherence and state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Addressing the limitation of existing mask-based pipelines in maintaining geometric temporal consistency across time, particularly under occlusion and for static scene elements.

Method: Streaming Gaussian encoder that maintains a persistent volumetric scene representation, propagated via ego-motion compensation and refreshed under a confidence-guided budget constraint.

Result: Established a new state-of-the-art for camera-based 4D-POT on Occ3D-extended nuScenes and Waymo, improving tracking consistency with negligible computational overhead.

Conclusion: The proposed method yields representation-level temporal coherence and is compatible with existing mask-based pipelines, providing a promising paradigm for holistic scene understanding.
...
TL;DR: A framework is proposed to learn latent representations of trajectory datasets and quantify their similarity, which helps in selecting datasets for motion prediction tasks and improving model generalizability.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the issue of model generalization in motion prediction tasks when trained on one dataset and applied to another with different scene layouts, agent behaviors, and sensing conditions.

Method: The method involves learning latent representations of datasets and quantifying their similarity using distributional metrics. A large-scale study is conducted covering 24 major datasets to evaluate the transferability of these datasets.

Result: The results show that the transferability scores strongly correlate with cross-dataset model performance, providing practical guidance for dataset selection, pretraining, and developing large-scale foundation models for motion prediction.

Conclusion: The proposed framework paves the way toward more generalizable and robust predictive systems in motion prediction by facilitating better dataset selection and model pretraining strategies.
...
TL;DR: The paper introduces Random-Target Supervised Mixing (RTSM) for source-free domain adaptive object detection, which incorporates a small subset of labeled target images to improve adaptation performance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve source-free domain adaptive object detection by leveraging a small subset of labeled target images, rather than relying solely on unlabeled data.

Method: The method proposed is Random-Target Supervised Mixing (RTSM), which incorporates a supervised detection loss using a small subset of labeled target images into the original unlabeled adaptation branch.

Result: RTSM consistently improves pure source-free domain adaptation by 1.7 to 18.3 AP50 across various evaluations, and additional sparse-label feedback plugins yield limited gains over RTSM.

Conclusion: The paper concludes that simple supervision through RTSM is hard to beat for sparse-label source-free domain adaptive object detection, providing a simple yet effective anchor for this task.
...
TL;DR: GaussLite, a task-driven 3D Gaussian Splatting system, optimizes scene representation based on task relevance, improving performance on regions of interest.


<details>
  <summary>Details</summary>
Motivation: Existing 3DGS systems uniformly distribute representation capacity across a scene, wasting compute on irrelevant parts for specific tasks.

Method: GaussLite uses a one-shot LLM parser, open-vocabulary detector, and per-pixel relevance masks to allocate representation density based on task specification.

Result: GaussLite outperforms baselines by +2.72 dB on ROI PSNR and demonstrates real-time mapping at 4 Hz on resource-constrained hardware.

Conclusion: GaussLite enables task-driven 3DGS mapping, improving performance and efficiency by focusing on relevant scene regions.
...
TL;DR: A novel unified tokenizer, AVTok, is proposed for holistic audio-video generation, featuring a dual-stream transformer-based architecture with shared encoder-decoder and modal-specific learnable queries.


<details>
  <summary>Details</summary>
Motivation: The existing methods for audio-video generation adopt a dual-branch design with separate tokenization and generation modules per modality, which neglects the representation gap and requires intensive computational resources.

Method: AVTok uses a dual-stream transformer-based architecture with shared encoder-decoder and modal-specific learnable queries to encode an audio-video pair into a compact one-dimensional latent representation with a unified codebook.

Result: AVTok excels in audio-video reconstruction and when integrated into downstream pipelines for audio-to-video, video-to-audio, and class-conditional joint audio-video generation.

Conclusion: AVTok paves the way for the challenge of joint audio-video tokenization and provides a potential direction to build unified large multimodal models for audio-video generation.
...
TL;DR: The paper proposes SyncCache, a training-free caching method for accelerating Diffusion Transformers (DiTs) in audio-driven portrait animation, achieving up to 4.12x acceleration with near-lossless visual fidelity.


<details>
  <summary>Details</summary>
Motivation: The high computational cost of DiTs leads to substantial inference latency in audio-driven portrait animation, and existing caching methods are not tailored for this specific task.

Method: The paper proposes three main techniques: Spatially-Asymmetric Probing to prioritize error sensitivity in dynamic human regions, Modality-Decoupled Caching to reuse stable inter-block residuals and recompute lightweight audio blocks, and a cache ratio to control cache capacity.

Result: SyncCache achieves up to 4.12x acceleration on HunyuanVideo-Avatar and 3.75x on Wan-S2V with near-lossless visual fidelity and precise audio alignment.

Conclusion: The proposed SyncCache method effectively accelerates DiT-based portrait animation while maintaining high visual fidelity and precise audio alignment.
...
TL;DR: The paper introduces a framework called Label Imitation Game (LIG) and a Turing Test Network (TTN) to improve the accuracy of pseudo-labels generated by foundation models through a Turing-inspired adversarial interrogation process.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the issue of hallucinations or errors in pseudo-labels generated by foundation models when labeling data via zero-shot inference, which undermines their performance.

Method: The method involves using the LIG framework to train a TTN, a task-agnostic 'judge' that evaluates candidate pseudo-labels within a dataset-wide context, rather than filtering labels via isolated thresholds.

Result: The results show that the TTN enhances label accuracy for state-of-the-art vision-language models across four diverse datasets without costly supervision or retraining. It demonstrates F1-score gains of 28% for the worst-performing baseline categories and 44% with task-specific fine-tuning, and also enables Category Revival where the TTN pruning helps downstream models recover from zero recall on transfer-vulnerable classes.

Conclusion: The conclusion is that the proposed TTN offers a robust alternative to traditional verification methods, enabling zero-shot task transfer capability and improving the accuracy of pseudo-labels, which can significantly benefit the performance of downstream models.
...
TL;DR: A method for estimating the scale of objects in monocular images is proposed to improve the verification accuracy of in-vehicle cameras in virtual environments.


<details>
  <summary>Details</summary>
Motivation: Discrepancies in object scale between virtual and real environments can lead to decreased camera recognition performance, particularly for traffic signs at high altitudes where distance measurement is difficult.

Method: The method involves decomposing objects into structural elements, integrating external knowledge on design rules and geometric relationships, and estimating the size of each component from monocular images.

Result: The proposed method enables the generation of 3D assets with a scale approximating the real environment, which can be used in digital twin spaces.

Conclusion: The proposed method is expected to contribute to improving the verification accuracy of in-vehicle cameras for autonomous driving in virtual environments.
...
TL;DR: GRAPE, a unified architecture for medical image classification, addresses limitations of prototype-based classifiers by modeling concept co-occurrence, detecting erroneous annotations, and enabling open-vocabulary prototype anchoring.


<details>
  <summary>Details</summary>
Motivation: Prototype-based medical image classifiers have limitations: treating findings as independent, amplifying unsafe physician feedback, and requiring full retraining for new findings.

Method: GRAPE uses a Graph Attention Task Head, Concept-Mismatch Safety Check, and Open-Vocabulary Prototype Anchoring to address the limitations.

Result: GRAPE achieves improved performance: +13.8% macro-F1 on TBX11K, 85% erroneous annotation detection, and 2.6x better lesion localization on TBX11K compared to baselines.

Conclusion: GRAPE presents a unified solution for medical image classification, addressing key limitations of prototype-based classifiers while achieving improved performance and efficiency.
...
TL;DR: A new test-time adaptation framework for LiDAR semantic segmentation is proposed, which improves stability and performance under changing sensing conditions without requiring additional annotations.


<details>
  <summary>Details</summary>
Motivation: LiDAR semantic segmentation performance degrades in real-world deployments due to changing sensing conditions, and collecting new annotations for retraining is impractical.

Method: A geometry-constrained test-time prompt tuning framework is proposed, which estimates per-location sensing reliability and uses it to reweight spatial supervision. The framework adapts a frozen backbone with lightweight prompt adapters and spatial gating.

Result: Experiments on standard LiDAR benchmarks demonstrate improved adaptation stability and segmentation performance under deployment variations without additional annotations.

Conclusion: The proposed framework effectively improves the stability and performance of LiDAR semantic segmentation under changing sensing conditions, making it suitable for real-world deployments.
...
TL;DR: Prost-RL, a machine learning model, improves prostate cancer detection using micro-ultrasound imaging by integrating reinforcement learning to generate interpretable spatial attention maps, achieving high accuracy and sensitivity.


<details>
  <summary>Details</summary>
Motivation: Accurate identification of suspicious tissue in micro-ultrasound imaging for prostate cancer detection is highly dependent on clinical experience, leading to inter-observer variability, and machine-learning assistance is needed but challenging due to sparse and noisy supervision.

Method: Prost-RL integrates a lightweight reinforcement-learning policy into a foundation-model encoder-decoder to generate spatial attention maps for cancer-likelihood heatmap prediction and image-level classification, using Adaptive Policy Optimization and a noise-robust objective.

Result: Prost-RL achieves 79.0±3.5 AUROC with 64.6±6.3% sensitivity at 80% specificity for core-level detection, and 79.3±5.8 AUROC for clinically significant cancer classification, outperforming the strongest baseline.

Conclusion: Prost-RL provides a promising solution for improving prostate cancer detection using micro-ultrasound imaging, with high accuracy and sensitivity, and provides transparent, spatially grounded evidence alongside quantitative risk predictions.
...
TL;DR: PhotoQuilt is a framework for generating photomosaics at arbitrary resolution, balancing global structure and local detail.


<details>
  <summary>Details</summary>
Motivation: Generating high-resolution photomosaics with convincing local tiles is computationally expensive due to the need to hold many detailed tiles at once.

Method: PhotoQuilt uses a bootstrapped tiled denoising procedure, first producing a low-resolution global composition, then upscaling it in latent space and re-injecting noise to restore generative capacity.

Result: PhotoQuilt outperforms current baselines on both global structure and local realism, and scales to large canvases without quadratic attention cost.

Conclusion: PhotoQuilt provides a training-free solution for generating photomosaics at arbitrary resolution, effectively balancing global structure and local detail.
...
TL;DR: The paper critiques current AI-generated video detection benchmarks, proposes an audited evaluation protocol, and releases a unified detector collection called VidAudit.


<details>
  <summary>Details</summary>
Motivation: Current evaluation protocols for AI-generated video detection benchmarks have uncontrolled confounds that can inflate reported generalization, leading to unreliable results.

Method: The authors propose a new audited evaluation protocol that combines six standard controls, and apply it to six representative feature sources. They also release VidAudit, a unified detector collection with 14 detectors behind one plugin API.

Result: The audit reveals that some methods have near-chance performance or carry dataset identity, while others, like WaveRep, show strong performance. The results also highlight the importance of using multiple metrics, including AUC, above-floor margin, operating-point recall, and calibration.

Conclusion: The paper emphasizes the need for rigorous evaluation protocols and provides a new benchmark, VidAudit, to improve the reliability of AI-generated video detection results.
...
TL;DR: The paper proposes a method for sparse action-aware landmark localization in surgical videos using vision foundation models, achieving F1 scores of 72.4% for tip and 58.0% for anchor localization without manual pixel-level mask annotations.


<details>
  <summary>Details</summary>
Motivation: Current vision foundation models provide transferable object-level structure but lack action-conditioned semantics for functional surgical landmarks. The authors aim to investigate vision foundation model-enabled sparse action-aware landmark localization.

Method: The authors propose a lightweight refinement framework using SAM 3 as a structural prior. A coarse multi-frame network predicts tip and anchor prompts, generating non-oracle masks that are fused with visual and heatmap features to refine functional landmark predictions.

Result: The proposed model achieves overall F1 scores of 72.4% for tip and 58.0% for anchor localization without manual pixel-level mask annotations. The authors compare different methods for incorporating vision foundation model-derived structure into a precision-oriented localization system.

Conclusion: The proposed method effectively utilizes vision foundation models for sparse action-aware landmark localization in surgical videos, providing a promising approach for precision-oriented localization without manual pixel-level mask annotations.
...
TL;DR: A novel Dual Sparse Aggregation Transformer (DSAFormer) is proposed for multispectral object detection, achieving better performance than state-of-the-art methods on four public datasets.


<details>
  <summary>Details</summary>
Motivation: Previous transformer-based multispectral detection methods use all available tokens for similarity calculation, resulting in redundant information interaction and degraded detection performance.

Method: The proposed DSAFormer consists of a Dual Sparse Transformer (DSFormer) and a Learnable Addition Fusion Block (LAFB), incorporating Spatial Sparse Multi-Head Cross-Attention, Channel Sparse Multi-Head Cross-Attention, and Multi-Scale Feature Refinement Layer.

Result: The DSAFormer achieves better detection performance against state-of-the-art methods on four public datasets: MFAD, FLIR, M$^3$FD, and LLVIP.

Conclusion: The proposed DSAFormer effectively overcomes the challenge of redundant information interaction in previous transformer-based multispectral detection methods, leading to improved detection performance.
...
### [17] [TerraDiT-$Ω$: Unified Spatial Control for Satellite Image Synthesis with Any Geospatial Primitive](https://arxiv.org/abs/2606.31029)
*Brian Wei,Srikumar Sastry,Daniel Cher,Eric Xing,Nathan Jacobs*

Main category: cs.CV

TL;DR: A new framework, TerraDiT-Ω, generates satellite imagery from native geospatial primitives, offering controllable layouts and compatibility with GeoAI workflows.


<details>
  <summary>Details</summary>
Motivation: Applying generative models to satellite imagery is challenging due to complex and semantically distinct geometries, and existing methods trade off annotation cost and fidelity.

Method: The framework uses Geometry-Aware Local Attention to leverage precise and coarse annotations, supporting controllable layouts across varying annotation budgets.

Result: TerraDiT-Ω outperforms dense-control and sparse-control baselines and enables controllable synthetic data augmentation, improving downstream performance on various tasks.

Conclusion: TerraDiT-Ω provides a unified spatial control framework for generating satellite imagery, compatible with vector primitives and GeoAI workflows, with improved performance and flexibility.
...
### [18] [Learning Video Dynamics with Predictive Differentiable Rendering](https://arxiv.org/abs/2606.31050)
*Yujin Tang,Tian Zhou,Xin Lin,Cheng Tan,Yifan Hu,Rong Jin,SouYoung Jin,Liang Sun*

Main category: cs.CV

TL;DR: A novel video prediction method called Predictive Differentiable Rendering (PDR) is proposed, which improves prediction accuracy and detail preservation by bridging discrete and continuous representations.


<details>
  <summary>Details</summary>
Motivation: Existing video prediction models operate in discrete pixel space and are optimized with pixel-wise mean squared error (MSE), leading to over-smoothed predictions and lack of fine-grained visual details.

Method: PDR uses a 2D Gaussian representation and a CUDA-accelerated differentiable renderer, optimized with a combined L1 and SSIM loss.

Result: PDR consistently surpasses existing methods on diverse real-world benchmarks, delivering superior detail preservation, visual fidelity, and predictive accuracy.

Conclusion: The proposed PDR paradigm significantly enhances video prediction performance by addressing the limitations of existing methods, providing more accurate and detailed predictions.
...
### [19] [ADAPT: Attention Dynamics Alignment with Preference Tuning for Faithful MLLMs](https://arxiv.org/abs/2606.31054)
*Zhiyuan Yao,Zheren Fu,Zhixiao Zheng,Jiajun Li,Yi Tu,Zhendong Mao*

Main category: cs.CV

TL;DR: The paper proposes ADAPT, a framework to mitigate hallucination in Multimodal Large Language Models (MLLMs) by intervening on text-to-image cross-attention dynamics.


<details>
  <summary>Details</summary>
Motivation: Hallucination in MLLMs generates content inconsistent with the provided image, and existing mitigation strategies do not explicitly target this failure mode.

Method: The ADAPT framework consists of three key components: a cross-attention visual anchor, an attention-supervised inference mechanism, and a Visual Attention Guidance DPO.

Result: Experiments show that ADAPT reduces hallucination rates by 40%-60% across mainstream backbones while preserving general multimodal capabilities.

Conclusion: The paper provides an attention-based perspective on mitigating hallucinations by exploring the model's internal text-to-image cross-attention behaviors.
...
### [20] [Diffusion-Based Material Regularization for Physics-Based Inverse Rendering](https://arxiv.org/abs/2606.31065)
*Jingwang Ling,Lifan Wu,Feng Xu,Shuang Zhao*

Main category: cs.CV

TL;DR: The paper presents a method to reconstruct 3D assets (geometry, materials, and illumination) from multi-view images by combining physics-based inverse rendering and data-driven diffusion models.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome the limitations of physics-based inverse rendering, which is underconstrained, and data-driven diffusion models, which don't satisfy the rendering equation, to achieve high-quality 3D asset reconstruction and relighting.

Method: The method involves using a state-of-the-art diffusion model to generate a similarity kernel for optimization, and introducing a regularization loss to penalize deviations in the optimized material over surface regions where the diffusion predictions are near-constant.

Result: The method significantly outperforms state-of-the-art baselines in both reconstruction accuracy and relighting quality on several datasets (Synthetic4Relight, Stanford-ORB, and DTC-Synthetic).

Conclusion: The paper concludes that the proposed end-to-end pipeline can jointly reconstruct high-quality geometry, materials, and illumination, yielding assets that can be used for realistic relighting and editing.
...
### [21] [Hybrid Unet-Transformer Model for Generating Stress and Strain Fields from Composite Geometrics](https://arxiv.org/abs/2606.31068)
*Shrey Patel*

Main category: cs.CV

TL;DR: A hybrid UNet-Transformer model is proposed to predict stress and strain fields in composite microstructures from geometry images, achieving high accuracy and physically meaningful representations.


<details>
  <summary>Details</summary>
Motivation: Conventional FEM simulations are computationally expensive and cannot scale to complex composite microstructures, necessitating an efficient surrogate model.

Method: A hybrid UNet-Transformer architecture is developed to predict mechanical field distributions from composite microstructure geometry images.

Result: The proposed model achieves strong predictive performance across most subdatasets, with high accuracy (R2=0.9991, SSIM=0.9936, MAE=0.0050) on certain geometries and physically meaningful internal representations.

Conclusion: The hybrid UNet-Transformer model offers an efficient and accurate surrogate for FEM simulations in composite microstructure design, with potential applications in physics-informed material design.
...
### [22] [Hierarchical 3D Scene Graph Construction and Belief-based Planning for Semantic Navigation](https://arxiv.org/abs/2606.31071)
*Bing Wu,Zuyao Chen,Changwen Chen*

Main category: cs.CV

TL;DR: Proposed a zero-shot semantic navigation framework that uses a hierarchical 3D scene graph and belief-based planning to improve long-distance navigation in unseen environments.


<details>
  <summary>Details</summary>
Motivation: Semantic navigation in unseen environments requires both semantic understanding and long-term decision-making. Existing methods often rely on local observations and greedy strategies, leading to inefficient exploration and myopic behaviors.

Method: The proposed method maintains an online Hierarchical 3D Scene Graph (HSG) to form a multi-granular semantic topology, and uses a hierarchical belief-based planning framework to fuse semantic priors with exploration evidence and estimate long-term expected returns.

Result: The proposed method outperforms existing state-of-the-art methods, particularly in long-distance scenarios, where it improves SR and SPL by an average of 9.4% and 5.0%, respectively.

Conclusion: The proposed zero-shot semantic navigation framework effectively addresses the challenges of semantic navigation in unseen environments, enabling globally consistent decisions and reducing redundant backtracking.
...
### [23] [AnyMatch: Supercharging Universal Multi-Modal Image Matching with Large-Scale Single-View Images](https://arxiv.org/abs/2606.31077)
*Meng Yang,Zizhuo Li,Linfeng Tang,Fan Fan,Jiayi Ma*

Main category: cs.CV

TL;DR: The paper proposes AnyMatch, a framework for generating multi-modal training data for image matching, addressing the scarcity of large-scale data with precise geometric annotations.


<details>
  <summary>Details</summary>
Motivation: The scarcity of large-scale training data with precise geometric annotations for multi-modal image matching, which is essential for visual localization and multi-sensor fusion.

Method: AnyMatch integrates monocular depth estimation, 3D reprojection, diffusion-based inpainting, and cross-modal image translation to synthesize multi-view, multi-modal image pairs with 3D geometric fidelity.

Result: Experimental results show that matching networks fine-tuned on the synthetic dataset Any-syn achieve substantial performance gains on multi-modal benchmarks, exhibiting superior generalization and robustness.

Conclusion: The proposed framework AnyMatch provides a solution to generate rich multi-modal training data, enabling the creation of large-scale synthetic datasets like Any-syn, which can improve the performance of image matching networks.
...
### [24] [Fleet: Few Shots Lead Effective AI-generated Image Detection](https://arxiv.org/abs/2606.31082)
*Jiaan Wang,Sirui Liu,Yu Li,Kaiyuan Yang,Juan Cao,Sheng Tang*

Main category: cs.CV

TL;DR: The paper proposes a new paradigm of 'dynamic adaptation' for AI-generated image detection, introducing a framework called Fleet that enables rapid adaptation to emerging generative threats.


<details>
  <summary>Details</summary>
Motivation: The current static feature space approach for AI-generated image detection suffers from a severe performance drop against rapidly evolving generators.

Method: The proposed Fleet framework uses a dynamic paradigm of continuous few-shot evolution, enabling rapid alignment with emerging generative threats through constrained routing correction.

Result: Fleet improves few-shot adaptation, restoring performance from 20.4% to 73.1% with only 10-shot adaptation on 'Doubao Seedream 4.0'.

Conclusion: The paper concludes that the field of AI-generated image detection should shift from 'static generalization' to 'dynamic adaptation', and that Fleet is a promising approach to achieve this goal.
...
### [25] [CasaMaestro: Multi-View Panoramas for House-Scale 3D Reconstruction](https://arxiv.org/abs/2606.31086)
*Yuzhou Ji,Xiaotian Yang,Zhipeng Zhang*

Main category: cs.CV

TL;DR: CasaMaestro, a feedforward model, enables fast metric 3D reconstruction of residential spaces using sparse multi-view indoor panoramas.


<details>
  <summary>Details</summary>
Motivation: The need for efficient 3D reconstruction of large indoor spaces, driven by the rise of home-deployed embodied AI systems.

Method: CasaMaestro takes 20-50 sparse multi-view indoor panoramas as input and directly predicts metric depth and camera poses.

Result: CasaMaestro provides high-quality results in both real-world and synthetic scenes, enabling fast point-cloud reconstruction of entire houses.

Conclusion: CasaMaestro is a significant advancement in 3D reconstruction, supporting house-scale reconstruction with multi-view panoramas.
...
### [26] [Towards Flexible, Natural, Efficient Interaction for Conversational Talking Face Generation](https://arxiv.org/abs/2606.31088)
*Baiqin Wang,Sen Chen,Jiankuo Zhao,Xiangyu Liu,Zhen Lei,Xiangyu Zhu*

Main category: cs.CV

TL;DR: InterTalk, a novel framework for conversational talking face generation, achieves real-time, flexible, and natural interactions among multiple participants.


<details>
  <summary>Details</summary>
Motivation: Existing methods for conversational talking face generation struggle to balance flexibility, naturalness, and efficiency; InterTalk aims to address these challenges.

Method: InterTalk uses a motion-based architecture, models multi-round conversational dynamics, incorporates motion feedback, and disentangles motion into facial components for targeted refinements.

Result: InterTalk achieves superior interaction quality and real-time performance at 30 FPS, outperforming existing methods in conversational talking face generation.

Conclusion: InterTalk presents a significant advancement in conversational talking face generation, offering a more interactive, natural, and efficient solution for generating dynamic talking videos.
...
### [27] [Anchoring on Reality: Breaking the Pseudo-Target Ceiling in Makeup Transfer](https://arxiv.org/abs/2606.31089)
*Bo Wei,Xianhui Lin,Yi Dong,Zhongzhong Li,Zonghui Li,Zirui Wang,Jiachen Yang,Xing Liu,Hong Gu,Xiaoming Li,Wangmeng Zuo*

Main category: cs.CV

TL;DR: Proposed a two-stage framework called Anchoring on Reality Makeup Transfer (ART) for makeup transfer, which achieves superior makeup fidelity, strong background stability, and robust identity preservation.


<details>
  <summary>Details</summary>
Motivation: The lack of real paired training data for makeup transfer tasks, leading to suboptimal guidance and degraded fine-grained details in current methods.

Method: A two-stage framework with a reality-anchored refinement cycle, using pseudo-targets in Stage I and real reference in Stage II, and introducing MakeupFaces2K dataset.

Result: The proposed method achieves superior makeup fidelity, strong background stability, and robust identity preservation, especially for complex makeup styles.

Conclusion: The proposed Anchoring on Reality Makeup Transfer (ART) framework and MakeupFaces2K dataset provide a more effective solution for makeup transfer tasks, preserving identity and geometry while achieving high-quality makeup transfer results.
...
### [28] [Do Not Break the Vessels: Structure-Preserving Mean Flow for Vascular Image Translation](https://arxiv.org/abs/2606.31095)
*Changjin Sun,Zhuo Hu,Kaini Wang,Baixuan Wu,Shuo Gao,Runan Zheng,Cheng Xue,Yudong Zhang,Guangquan Zhou*

Main category: cs.CV

TL;DR: The authors propose a Structure-Preserving Mean Flow (SPMF) framework for translating vascular images while preserving their structure, and a Prototype-Guided Structural Refinement (PGSR) module for refining the translated images.


<details>
  <summary>Details</summary>
Motivation: Existing cross-modal translation methods often compromise anatomical coherence and clinical reliability due to structural discontinuities and artifacts.

Method: The SPMF framework formulates vascular image translation as a topology-invariant transport process, using an orthogonality constraint on the flow velocity field to separate appearance transport from topological distortion. The PGSR module aligns degraded inference-time structures with reliable training-time structures.

Result: The proposed method achieves peak PSNR values of 24.96 dB and 24.83 dB on paired NIRII-to-2PF and fundus datasets, respectively, demonstrating consistent improvements over state-of-the-art methods.

Conclusion: The proposed SPMF framework and PGSR module can effectively preserve the structure of vascular images during translation, improving anatomical coherence and clinical reliability.
...
### [29] [Horizon3D: Sparse Radar-Camera Fusion for Long-Range 3D Perception in Autonomous Driving](https://arxiv.org/abs/2606.31096)
*Geonho Bang,Geunju Baek,Dongyoung Lee,Wonjun Jeong,Jun Won Choi*

Main category: cs.CV

TL;DR: Horizon3D, a sparse radar-camera fusion framework, is proposed for long-range 3D object detection, achieving state-of-the-art performance on TruckScenes.


<details>
  <summary>Details</summary>
Motivation: Existing radar-camera fusion methods are limited for long-range 3D object detection, and current methods have limitations such as high computation, loss of object details, or limited scene-level context.

Method: Horizon3D uses Gaussian primitives with sparse BEV features, involving Keypoint-Guided Gaussian Initialization, Object-Centric Sparse Fusion, and Dual-Path Temporal Fusion.

Result: Horizon3D outperforms the previous best method by +3.0 NDS and +1.6 mAP on TruckScenes validation set while maintaining competitive inference speed.

Conclusion: The proposed Horizon3D framework effectively addresses the limitations of existing methods for long-range 3D object detection, achieving state-of-the-art performance.
...
### [30] [PiLoT v2: Pixel-to-Orthogonal Map Alignment for Free-view UAV Geo-localization](https://arxiv.org/abs/2606.31098)
*Xinyi Liu,Xiaoya Cheng,Rouwan Wu,Zhaochen Wang,Shen Yan,Maojun Zhang,Yu Liu*

Main category: cs.CV

TL;DR: The authors propose PiLoT v2, a lightweight UAV geo-localization system that improves upon the original PiLoT by using orthogonal maps and efficient CPU-friendly operations, achieving comparable performance with lower storage and computational costs.


<details>
  <summary>Details</summary>
Motivation: The original PiLoT system has limitations such as high storage overheads, complex map acquisition, and significant computational rendering costs, making it difficult to deploy on embedded platforms.

Method: PiLoT v2 uses True Digital Orthophoto Maps (TDOMs) and Digital Surface Models (DSMs) as reference substrates, and a cross-view feature registration network to bridge the geometric discrepancy between 2.5D orthogonal crops and free-view oblique UAV imagery.

Result: PiLoT v2 achieves performance comparable to, or even exceeding, its Pixel-to-3D predecessor, while offering drastically lower storage and computational costs.

Conclusion: The proposed PiLoT v2 system provides a lightweight yet robust solution for real-time, drift-free UAV geo-localization in GNSS-denied environments, making it suitable for deployment on embedded platforms.
...
### [31] [Seeing Through Multiple Views: Parameter-Efficient Fine-Tuning via Selective Neurons for Consistent Radiology Report Generation](https://arxiv.org/abs/2606.31099)
*Yucheng Chen,Jinjing Zhu,Yang Yu,Yufei Shi,Hane Naghshbandi,Jinhua Liu,Angela S. Koh,Fang Fen,Kian Eng Ong,Si Yong Yeo*

Main category: cs.CV

TL;DR: The authors propose View-PNDF, a framework for generating radiology reports from multi-view X-ray images, focusing on view-consistent report generation and clinical reliability.


<details>
  <summary>Details</summary>
Motivation: Existing approaches to radiology report generation overlook potential clinical inconsistencies and inaccuracies when processing different views with a single model, impacting performance and reliability.

Method: View-PNDF includes a view-specific neuron detection module, verification module, and selective fine-tuning strategy to strengthen detected neurons while preserving view-agnostic representations.

Result: View-PNDF improves view-specific chest X-ray report generation quality while maintaining robust general-view performance, as demonstrated on two medical RRG benchmarks.

Conclusion: The proposed View-PNDF framework effectively generates view-consistent radiology reports with reduced computational costs, enhancing clinical reliability and performance.
...
### [32] [TaxoMIL: Taxonomy-Constrained Learning for Hierarchical Whole Slide Image Analysis](https://arxiv.org/abs/2606.31100)
*Chaeyeon Lee,Khang Nguyen Quoc,Jinsol Song,Yosep Chong,Kwangil Yim,Jin Tae Kwak*

Main category: cs.CV

TL;DR: TaxoMIL, a new framework for whole slide image analysis, improves diagnosis by structuring predictions according to clinical taxonomy.


<details>
  <summary>Details</summary>
Motivation: Current methods for whole slide image analysis do not align with the hierarchical nature of clinical reasoning, leading to inaccurate or inconsistent diagnoses.

Method: TaxoMIL uses a dual-head Transformer decoder for multi-granularity text generation and taxonomy-guided objectives to ensure predictions adhere to clinical taxonomy.

Result: TaxoMIL outperforms state-of-the-art methods across three diverse datasets, providing accurate and hierarchy-aware diagnostic predictions.

Conclusion: TaxoMIL offers a more accurate and clinically relevant approach to whole slide image analysis by integrating clinical taxonomy into the diagnostic process.
...
### [33] [InfiniVerse: Occupancy Guided Unbounded Scene Generation for Autonomous Driving](https://arxiv.org/abs/2606.31109)
*Xiaoyu Ye,Leheng Li,Xinyu Ji,Yingjie Cai,Hongda He,Xu Yan,Guanyi Zhao,Ying-Cong Chen,Bingbing Liu,Shuguang Cui,Zhen Li*

Main category: cs.CV

TL;DR: InfiniVerse is a pipeline for generating realistic, controllable, and temporally coherent urban environments for autonomous driving.


<details>
  <summary>Details</summary>
Motivation: Generating realistic, controllable, and temporally coherent urban environments is a critical challenge in the autonomous driving community.

Method: The approach involves reconstructing a 3D occupancy representation from a single frame, then using autoregressive scene extension and a video diffusion model to generate realistic video sequences.

Result: InfiniVerse achieves state-of-the-art performance with a FID of 6.4 and FVD of 67.97 on the Waymo Open Dataset and nuScenes.

Conclusion: InfiniVerse enables long-range, 2D-3D-aligned, and controllable synthesis of dynamic urban scenes, outperforming existing benchmarks in duration and stability.
...
### [34] [JacobianAvatar: Temporally Consistent Semi-rigid Avatar Reconstruction from a Monocular Video](https://arxiv.org/abs/2606.31115)
*Changyeon Won,Min-Gyu Park,Seonghwan Park,Ju Hong Yoon,Hae-Gon Jeon*

Main category: cs.CV

TL;DR: The paper presents a method for generating realistic human avatars in complex motions from monocular input, leveraging neural Jacobian fields and addressing challenges such as self-occluded regions and invisible surfaces.


<details>
  <summary>Details</summary>
Motivation: The challenge of modeling global and local deformations in monocular settings, particularly for complex motions like clothing dynamics.

Method: The method uses neural Jacobian fields (NJFs) to represent semi-rigid deformations, and includes three key components: a constrained Poisson solver, signed distance-based Jacobian regularization, and a deformation-guided residual flow loss.

Result: The method generates temporally stable and geometrically coherent avatars, outperforming state-of-the-art approaches on benchmark and in-the-wild videos.

Conclusion: The proposed approach effectively addresses the challenges of monocular human avatar generation, producing high-quality results with improved temporal consistency and geometric coherence.
...
### [35] [WildProp: Visual Estimation of Wildlife Body Proportions at Scale](https://arxiv.org/abs/2606.31125)
*Mustafa Chasmai,Aaron Sun,Subhransu Maji*

Main category: cs.CV

TL;DR: A new framework called WildProp estimates wildlife body proportions from large-scale, unconstrained image repositories without requiring controlled imaging or physical specimen handling.


<details>
  <summary>Details</summary>
Motivation: Traditional methods for morphometric measurements are limited in scalability due to the need for controlled imaging or physical specimen handling.

Method: WildProp uses a retrieval-driven correspondence problem approach, involving pose-aware retrieval, dense patch-level matching, geometric consistency filtering, and aggregation of measurements across retrieved images.

Result: Evaluations on three large morphometric datasets show median relative errors of 10-20%, and the approach demonstrates broad applicability across diverse taxa.

Conclusion: The WildProp framework provides a scalable method for estimating population-level morphometric measurements directly from large-scale image repositories, with potential applications in ecological and evolutionary studies.
...
### [36] [SkillSpotter: Pose-Aware Multi-View Skilled Action Detection and Grading in Ego-Exo Videos](https://arxiv.org/abs/2606.31127)
*Björn Braun,Christian Holz*

Main category: cs.CV

TL;DR: The paper introduces SkillSpotter, a system that uses AR glasses or fixed cameras to provide real-time coaching by detecting and grading skilled actions, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: To enable personalized, real-time coaching in domains like sports, cooking, or music, a system must understand not just what a person does, but how well they execute an activity.

Method: The authors adapt seven state-of-the-art temporal action detection architectures, and then introduce SkillSpotter, a pose-aware multi-view architecture that jointly detects and grades skilled actions.

Result: SkillSpotter improves class-specific mAP from 12.40 to 21.82 (+76%) and balanced accuracy from 55.99% to 60.40% over the best baseline.

Conclusion: The proposed SkillSpotter system effectively detects and grades skilled actions, and generalizes beyond the Ego-Exo4D benchmark to HoloAssist.
...
### [37] [MSNN-LINet: Cross-Modal Learning via Continuous Linear Integration](https://arxiv.org/abs/2606.31135)
*Gabriel Clinger*

Main category: cs.CV

TL;DR: The paper presents LINet, a multi-stream neural network for RGB-D scene classification that enables continuous cross-modal learning at every layer.


<details>
  <summary>Details</summary>
Motivation: Current multi-modal architectures have limitations in feature fusion, which is treated as a discrete event. The authors aim to address this structural compromise.

Method: The authors propose a novel Linear Integration Convolution (LIConv2d) operator and a multi-stream neural network architecture called LINet, which maintains three dedicated parallel streams for RGB, depth, and integration.

Result: LINet achieves 45.2% mean class accuracy on SUN RGB-D 19-class scene classification, outperforming prior from-scratch results, and 49.6% with in-domain RGB-D pretraining.

Conclusion: The proposed LINet architecture enables continuous cross-modal learning and achieves state-of-the-art results in RGB-D scene classification.
...
### [38] [FROST: Training-Free Few-Shot Segmentation with Frozen Features and Nonparametric Statistics](https://arxiv.org/abs/2606.31136)
*Junghwan Park*

Main category: cs.CV

TL;DR: FROST, a training-free few-shot segmenter, is proposed for remote sensing imagery. It estimates a class distribution from frozen self-supervised features and achieves state-of-the-art performance on 17 benchmarks.


<details>
  <summary>Details</summary>
Motivation: Few-shot segmentation is challenging in remote sensing due to scarce labels and domain shift. Existing methods have limitations, such as binding to the training distribution or losing class structure.

Method: FROST treats reference foreground and background as point clouds on the unit sphere of frozen DINOv3 features. It labels query tokens using nonparametric density ratio with a threshold set by Bayes rule.

Result: FROST surpasses both training-free and learning-based methods on 17 remote-sensing benchmarks, leading by 5.6 mIoU from a single annotated example and widening its lead as the support set grows.

Conclusion: FROST provides a effective training-free solution for few-shot segmentation in remote sensing, preserving class structure and achieving state-of-the-art performance.
...
### [39] [WaterGen: Decoupling Scene and Medium in Underwater Image Generation](https://arxiv.org/abs/2606.31147)
...
TL;DR: A novel method called WaterGen is proposed for generating large-scale, realistic, and diverse underwater images with controllable scene and water medium conditions, improving downstream tasks like restoration and segmentation.


<details>
  <summary>Details</summary>
Motivation: The scarcity of large-scale and diverse training data for underwater computer vision tasks such as detection, restoration, and segmentation.

Method: WaterGen treats underwater image generation as the decoupled control of scene content and water medium effects within a latent diffusion framework, involving two stages: fine-tuning for scene content generation and conditional decoding for medium degradation synthesis.

Result: The synthetic data generated by WaterGen consistently improves downstream performance in underwater restoration and semantic segmentation.

Conclusion: WaterGen enables the generation of large-scale synthetic underwater datasets that are diverse in scene structures and accurate in water effects and pseudo-labels, enhancing the performance of underwater computer vision tasks.
...
### [40] [PruneGround: Plug-and-play Spatial Pruning for 3D Visual Grounding](https://arxiv.org/abs/2606.31148)
*Duc Cao Dinh,Khai Le-Duc,Florent Draye,Chris Ngo,Terry Jingchen Zhang,Bernhard Schölkopf,Zhijing Jin*

Main category: cs.CV

TL;DR: PruneGround, a plug-and-play framework for 3D Visual Grounding (3DVG), improves efficiency and accuracy by pruning irrelevant regions and reformulating descriptions.


<details>
  <summary>Details</summary>
Motivation: Existing 3DVG approaches are inefficient and often ambiguous, especially in cluttered environments, because they reason over the entire scene.

Method: PruneGround consists of three components: Language-Guided Spatial Pruning (LGSP), MultiView-Conditioned Description Reformulation (MCDR), and LLM-Grounder.

Result: PruneGround achieves state-of-the-art results on three ScanRefer settings and 9 out of 10 Nr3D/Sr3D settings.

Conclusion: PruneGround effectively improves 3DVG by reducing computation and ambiguity through spatial pruning and description reformulation.
...
### [41] [Rethinking Foundation Model Collaboration: Enhancing Specialized Models through Proxy Task Reasoning](https://arxiv.org/abs/2606.31157)
*Hongyi Lin,Yang Liu,Jinhua Zhao,Xiaobo Qu*

Main category: cs.CV

TL;DR: The paper proposes a framework called FAT, which combines foundation models with task-specific predictors to improve performance on structured prediction tasks.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the capability mismatch between foundation models and task-specific predictors, and explore how they can collaborate effectively.

Method: The authors propose a framework called FAT, which decomposes structured prediction into specialist prediction, information-space reconstruction, and foundation-model proxy reasoning.

Result: The authors demonstrate that their approach, instantiated as ProxySelect, consistently improves specialized baselines and outperforms direct foundation-model regression at lower computational cost across various tasks.

Conclusion: The authors suggest a general collaboration principle: specialized models preserve task-specific structure, while foundation models refine their hypotheses through contextual proxy reasoning.
...
### [42] [Reasoning-aware Speculative Decoding for Efficient Vision-Language-Action Models in Autonomous Driving](https://arxiv.org/abs/2606.31160)
*Anh Dung Dinh,Simon Khan,Flora Salim*

Main category: cs.CV

TL;DR: The paper proposes a new approach to accelerate the chain-of-causation (CoC) reasoning step in Vision-Language-Action (VLA) planners for autonomous driving by splitting the reasoning into two specialized paths: a routine reasoner and a deliberative reasoner.


<details>
  <summary>Details</summary>
Motivation: The autoregressive CoC reasoning step in VLA planners dominates inference latency, which is a critical constraint in autonomous driving. The authors aim to accelerate this step to improve the overall efficiency of the planner.

Method: The authors propose a two-reasoner system consisting of a routine reasoner that handles predictable continuations by attending to trajectory history, and a deliberative reasoner that handles novel cases by attending to current visual evidence. They introduce two techniques: FlatRoPE, a 1D rotary positional embedding, and Action-aware RL (AARL), a post-training stage that uses an action-quality reward.

Result: The proposed two-reasoner system reduces the reasoning-step running time by approximately $4	imes$ relative to the original Alpamayo planner.

Conclusion: The paper presents a novel approach to accelerate the CoC reasoning step in VLA planners by leveraging the different needs of routine and deliberative reasoning. The proposed system achieves significant speedup while maintaining the accuracy of the planner.
...
### [43] [Seeing Through the Weights: Privacy Leakage in Scene Coordinate Regression](https://arxiv.org/abs/2606.31164)
*Oleksii Nasypanyi,Jaemin Cho,Utku Ozbulak,Byungkon Kang,Francois Rameau*

Main category: cs.CV

TL;DR: Scene Coordinate Regression (SCR) methods, assumed to be privacy-preserving, can be exploited to reconstruct 3D geometry and appearance of training environments, revealing sensitive information.


<details>
  <summary>Details</summary>
Motivation: The increasing adoption of SCR methods for visual localization and the assumption that they are privacy-preserving due to implicit scene encoding within neural networks.

Method: A query-based attack is introduced to reconstruct 3D geometry from an SCR model under different levels of model access, using proxy images and identifying reliable points through stability under input perturbations.

Result: Experiments demonstrate that substantial portions of training environments can be reconstructed with high geometric fidelity, and approximate color appearance can be recovered, exposing recognizable layout and potentially sensitive scene elements.

Conclusion: SCR representations are not inherently privacy-preserving, posing a real risk when deployed in private or security-critical spaces.
...
### [44] [Beyond Single Character: Evaluating MLLMs for Sentence-Level Oracle Bone Inscription Understanding](https://arxiv.org/abs/2606.31169)
*Ziqi Li,Zijian Chen,Tingzhu Chen,Guangtao Zhai*

Main category: cs.CV

TL;DR: The paper introduces S-OBI, a benchmark for evaluating multimodal large language models (MLLMs) in understanding sentence-level oracle bone inscriptions (OBI).


<details>
  <summary>Details</summary>
Motivation: Existing AI studies on OBI focus on character-level recognition, ignoring long-form textual coherence and contextual dependencies. The authors aim to evaluate MLLMs in sentence-level OBI understanding.

Method: The authors create S-OBI by synthesizing clear and standardized sentence-level OBI instances through glyph substitution and composition. They design three tasks: semantic matching, semantic slot extraction, and contextual reasoning, resulting in 695 question-answer pairs.

Result: Experiments show that contemporary MLLMs perform poorly on sentence-level OBI understanding, with visual perception errors propagating through the reasoning chain. This indicates that current models rely heavily on character-level recognition.

Conclusion: The paper concludes that S-OBI provides a diagnostic benchmark for evaluating MLLMs in sentence-level OBI understanding, highlighting the need for models to move beyond isolated character recognition to structured inscription-level understanding.
...
### [45] [HSDF-Lane: Height-Aligned Signed Distance Field with Semantic Lane Prior for 3D Lane Detection](https://arxiv.org/abs/2606.31172)
*Jiyong Boo,Byeongin Joung,Hyemin Yang,Kuk-Jin Yoon*

Main category: cs.CV

TL;DR: A novel approach, HSDF-Lane, is proposed for monocular 3D lane detection, which models the road surface as a Height-aligned Signed Distance Field (HSDF) and incorporates lane-aware semantic guidance.


<details>
  <summary>Details</summary>
Motivation: The inherent depth ambiguity in monocular 3D lane detection and the limitations of prior methods that rely on flat-ground assumptions or sparse anchor-based regression.

Method: HSDF-Lane, which uses a densely sampled 3D feature volume and differentiable rendering to jointly produce an accurate height map and surface-aligned features, and Lane-aware Semantic Positional Encoding (LSPE) to inject lane-existence prior into transformer queries.

Result: HSDF-Lane achieves state-of-the-art performance in both 3D lane detection and height map estimation on the OpenLane benchmark.

Conclusion: The proposed HSDF-Lane approach effectively addresses the challenges in monocular 3D lane detection by implicitly modeling the road surface and incorporating semantic guidance, leading to improved performance.
...
### [46] [GaussianMap: Learning Gaussian Representation for Multi-Sensor Online HD Map Construction](https://arxiv.org/abs/2606.31177)
*Hongyu Lyu,Julie Stephany Berrio Perez,Mao Shan,Stewart Worrall*

Main category: cs.CV

TL;DR: Proposed GaussianMap, an online HD map construction framework that uses adaptive Gaussian representation for efficient and accurate vectorized map prediction.


<details>
  <summary>Details</summary>
Motivation: Existing methods for online HD map construction use bird's-eye-view (BEV) features with fixed-resolution dense grids, which can be redundant and less effective for sparse map elements.

Method: GaussianMap framework, which includes a feed-forward Gaussian encoder for progressive refinement of Gaussian primitives and a decoder for vectorized map predictions.

Result: State-of-the-art performance on nuScenes and Argoverse 2 datasets in both camera-only and camera-LiDAR fusion settings.

Conclusion: GaussianMap offers a more efficient and accurate approach to online HD map construction, adapting representational capacity to map-relevant regions.
...
### [47] [Learning to Deny: Action Denial in Multimodal Large Language Models](https://arxiv.org/abs/2606.31187)
*Raiyaan Abdullah,Shehreen Azad,Yogesh Singh Rawat*

Main category: cs.CV

TL;DR: The paper introduces UCF101-AD, a benchmark to evaluate the ability of multimodal large language models (MLLMs) to deny an action, and finds that state-of-the-art models struggle with this task, but proposes a causal graph formulation to improve performance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to investigate the ability of MLLMs to recognize when an activity is not happening, despite strong contextual cues, which is a crucial aspect of video understanding.

Method: The authors introduce UCF101-AD, a large-scale benchmark consisting of paired Action-Presence and Action-Denial clips, and evaluate 20 state-of-the-art MLLMs on this benchmark. They also propose a causal graph formulation, CausalAct, to improve performance.

Result: The results show that state-of-the-art MLLMs that achieve high accuracy on positive action classes struggle with action-denial counterparts, but incorporating causal cues using CausalAct substantially reduces false positives.

Conclusion: The paper concludes that the inability to reason causally about whether a motion actually happens is a critical blind spot in modern video understanding, and that denial is a learnable reasoning skill that can be improved with causal cues.
...
### [48] [Distilling Temporal Coherence into 2D Networks for Transrectal Ultrasound Prostate Video Segmentation](https://arxiv.org/abs/2606.31198)
*Dong Yeong Kim,JunGyu Lee,Jaewon Choi,June Young Seo,Myeongseop Kim,Jinwook Choi,Taek Min Kim,Young-Gon Kim*

Main category: cs.CV

TL;DR: A new framework for real-time video segmentation of the prostate in TRUS images is proposed, achieving temporal consistency and efficiency.


<details>
  <summary>Details</summary>
Motivation: Conventional 2D methods for video segmentation suffer from inter-frame inconsistencies, while 3D architectures incur high latency. The goal is to develop a method that balances temporal consistency and efficiency.

Method: A Temporally Consistent Learning Framework is proposed, which distills temporal coherence into a 2D network during training. The framework includes a Confidence-Weighted Temporal Consistency objective, a Dual-scale Prototype Alignment Module, and geometric equivariance-based pseudo-labeling with knowledge distillation.

Result: The proposed method achieves state-of-the-art accuracy and temporal consistency at real-time speed on two benchmarks, SUN-SEG and TRUS-V (2,679 frames).

Conclusion: The proposed framework enables real-time video segmentation of the prostate in TRUS images with improved temporal consistency and efficiency, making it suitable for image-guided interventions.
...
### [49] [ExPLoRe: Expert Patch-Level Loss Routing for Multi-Objective Masked Image Modeling](https://arxiv.org/abs/2606.31201)
*Konstantinos Georgiou,Maofeng Tang,Hairong Qi*

Main category: cs.CV

TL;DR: ExPLoRe improves multi-objective masked image modeling by learning patch-level loss coefficients, achieving better performance on ImageNet-1K and downstream transfer tasks.


<details>
  <summary>Details</summary>
Motivation: Existing methods weight objectives with global scalars, ignoring spatial heterogeneity across patches in multi-objective masked image modeling.

Method: ExPLoRe (Expert Patch-Level Loss Routing) uses Soft Mixture of Experts dispatch weights as learned, per-patch loss coefficients, with a loss-coupling mechanism.

Result: ExPLoRe improves over non-MoE baselines on ImageNet-1K with ViT-Base, achieving 80.6% linear probe and 85.3% finetuning accuracy, and improves MoE finetuning by +1.5% on downstream transfer tasks.

Conclusion: ExPLoRe presents a novel approach to multi-objective masked image modeling, enabling content-dependent specialization and achieving competitive performance on ImageNet-1K and downstream transfer tasks.
...
### [50] [AC3S: Adaptive Conditioning for 3D-Aware Synthetic Data Generation](https://arxiv.org/abs/2606.31204)
*Eric Ji,Qiran Hu,Wufei Ma,Sarthak Jain,Yingying Li,Minh N. Do,Yaoyao Liu*

Main category: cs.CV

TL;DR: A new framework, AC3S, for generating high-quality synthetic data with precise 3D structure and pose consistency while preserving photorealism.


<details>
  <summary>Details</summary>
Motivation: Enforcing precise 3D structure and pose consistency in generated images remains challenging, and existing methods often suffer from over-conditioning artifacts.

Method: A diffusion-based image generation framework with adaptive conditioning, using a self-supervised visual prompt modulator and a multi-agent vision language model framework.

Result: The framework significantly improves image quality and downstream utility, enabling the scalable generation of high-quality synthetic datasets with accurate 2D and 3D annotations.

Conclusion: The proposed framework, AC3S, effectively addresses the challenge of enforcing precise 3D structure and pose consistency in generated images while preserving photorealism.
...
### [51] [AA: A Multi-view Multimodal Dataset for Screen-based Gaze Estimation](https://arxiv.org/abs/2606.31211)
*Chang Liu,Jiaqi Liu,Zhoutong Ye,Xinjie Shen,Chun Yu,Yuanchun Shi*

Main category: cs.CV

TL;DR: A multi-view multimodal dataset for screen-based gaze estimation is presented, featuring synchronized facial observations from multiple cameras and precise gaze targets.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of existing single-view gaze datasets, which lack robustness under viewpoint variation and occlusion.

Method: The dataset captures data from eight fixed screen-mounted cameras and two side-view cameras, paired with precise screen-space gaze targets under controlled fixation conditions.

Result: The dataset provides a more robust modeling approach under viewpoint variation and occlusion, enabling multimodal learning from both global and local visual cues.

Conclusion: The presented dataset, AA, supports reproducible research in gaze estimation and provides a standardized data processing pipeline.
...
### [52] [CooperScene: Multi-Modal Cooperative Autonomy Benchmark with C-V2X Communication Characterization](https://arxiv.org/abs/2606.31219)
*Bo Wu,Ruoshen Mo,Justin Yue,Yanyu Zhang,Janice Nguyen,Guoyuan Wu,Amit Roy-Chowdhury,Matthew J. Barth,Hang Qiu*

Main category: cs.CV

TL;DR: Introduced CooperScene, a high-fidelity dataset for cooperative autonomy with real-world C-V2X communication characterization, enabling evaluation of multi-agent scaling in real-world settings.


<details>
  <summary>Details</summary>
Motivation: Existing datasets for cooperative perception, prediction, and planning lack realism, ignoring complexities like limited communication bandwidth and heterogeneous sensing modalities.

Method: Created a dataset with 3 connected and autonomous vehicles and 1 infrastructure roadside unit, equipped with multi-modal sensors and C-V2X communication radios, annotated with 3D labels at 10 Hz.

Result: CooperScene dataset with 344K objects across 59K frames, providing a benchmark for evaluating multi-agent scaling and performance in real-world deployable settings.

Conclusion: CooperScene establishes a rigorous benchmark for cooperative autonomy, enabling researchers to evaluate and improve multi-agent systems in realistic scenarios.
...
### [53] [ForgeDrive: Bidirectional Cross-Conditioning for Unified Visual-Action Generation in Autonomous Driving](https://arxiv.org/abs/2606.31226)
...
TL;DR: ForgeDrive, a unified autoregressive diffusion framework, integrates visual and action planning in autonomous driving through an act-then-imagine paradigm, improving scene understanding and planning accuracy.


<details>
  <summary>Details</summary>
Motivation: The imagine-then-act paradigm in autonomous driving can lead to cascading errors from visual generation to action planning. The authors aim to bridge this gap with a unified framework.

Method: ForgeDrive uses a unified autoregressive diffusion framework with visual-action cross-conditioning, decoupling diffusion timesteps for images and actions, and introducing a novel act-then-imagine inference paradigm.

Result: ForgeDrive outperforms existing planners on NAVSIM, unifying driving simulation, planning, and visual odometry into a single model without post-training strategies.

Conclusion: ForgeDrive presents a promising approach to autonomous driving by integrating visual and action planning, improving scene understanding, and enhancing planning accuracy through a unified framework.
...
### [54] [UHD-MFF: Shattering Barriers in Multi-Focus Ultra-High-Definition Image Fusion via Learnable Lookup Tables](https://arxiv.org/abs/2606.31242)
*Yibing Zhang,Xunpeng Yi,Qinglong Yan,Yeda Wang,Han Xu,Jiayi Ma*

Main category: cs.CV

TL;DR: A novel approach for ultra-high-resolution multi-focus image fusion is proposed, including a large-scale dataset and a scale-specialized lookup-table framework, achieving real-time 4K fusion and outperforming state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: The advancement of imaging technology has made ultra-high-definition images essential in modern visual applications, but existing multi-focus image fusion methods are limited in UHD scenarios due to data availability, model adaptability, and deployment feasibility barriers.

Method: A scale-specialized lookup-table framework (UMF-LUT) is proposed, consisting of Coarse-Region Lookup Table (C-LUT) and Detail-Edge Lookup Table (D-LUT), which enables region-level and edge-level decision-making for ultra-high-resolution images.

Result: The proposed approach outperforms state-of-the-art methods in both visual fidelity and quantitative metrics, and demonstrates strong deployability with minimal computational overhead, enabling real-time 4K multi-focus fusion.

Conclusion: The proposed approach effectively advances the development of multi-focus image fusion toward ultra-high-resolution imaging scenarios, showing promising potential for smartphone applications.
...
### [55] [HyperVLP: Enhancing Hierarchical Surgical Video-Language Pre-training in Hyperbolic Space](https://arxiv.org/abs/2606.31245)
*Yaojun Hu,Kun Yuan,Nassir Navab,Haochao Ying,Jian Wu,Nicolas Padoy*

Main category: cs.CV

TL;DR: A new framework for surgical video-language pre-training that preserves hierarchical structure, improving phase recognition in surgical videos.


<details>
  <summary>Details</summary>
Motivation: Current surgical vision-language models neglect the hierarchical structure of surgical knowledge, leading to suboptimal performance.

Method: A hyperbolic surgical video-language pre-training framework that preserves hierarchical structure by mitigating structural false negatives and enforcing semantic consistency.

Result: Consistent gains in zero- and few-shot phase recognition across procedures and institutions on multiple surgical benchmarks.

Conclusion: The proposed framework effectively captures the hierarchical structure of surgical knowledge, leading to improved performance in surgical phase recognition.
...
### [56] [Rethinking the Role of Feature Engineering and Learning Strategies in Few-Shot Hidden Emotion Recognition](https://arxiv.org/abs/2606.31249)
*Xiaochuan Guo,Jihao Gu,Haixu Liu,Yuxin Liu,Qi Wang,Yufei Wang,Fei Wang,Kun Li,Dan Guo*

Main category: cs.CV

TL;DR: The paper presents a multi-modal temporal modeling framework that achieved first place in Track 3 of the 4th EI-MIGA-IJCAI Challenge with a test accuracy of 0.76923, addressing the challenge of weak and sparse implicit emotion evidence in long videos.


<details>
  <summary>Details</summary>
Motivation: The challenge of weak and sparse implicit emotion evidence in long videos motivated the development of a compact multi-modal temporal modeling framework.

Method: The proposed framework integrates multi-source features (2D/3D skeletons, facial expression Blendshapes, DINOv2/v3 vision foundation models, X-CLIP video features, and Gemini semantic priors) and utilizes a cross-attention mechanism with static pose features (Base) and dynamic micro-motion differential features (Offset).

Result: The proposed solution achieved a test accuracy of 0.76923 and first place in Track 3 of the 4th EI-MIGA-IJCAI Challenge.

Conclusion: The paper reveals the representation collapse phenomenon of general vision foundation models in micro-dynamic tasks and analyzes the underlying mechanisms, while also proposing a framework that effectively addresses the challenge of weak and sparse implicit emotion evidence in long videos.
...
### [57] [Decodable Is Not Grounded: A Vision-Ablation Arbiter for VLM Spatial Reasoning](https://arxiv.org/abs/2606.31257)
*Chih-Ting Liao,Fei Shen,Xin Cao,Tat-Seng Chua*

Main category: cs.CV

TL;DR: The authors reveal that linear probes and steering recovery can overstate what a vision-language model (VLM) actually grounds in an image, and propose a simple causal control to verify grounding.


<details>
  <summary>Details</summary>
Motivation: The authors aim to investigate the limitations of linear probes and steering recovery in extracting latent knowledge from VLMs, specifically in spatial reasoning tasks.

Method: The authors use a one-line causal control (replacing the image with a gray blank) to verify grounding, and analyze the results across various VLMs and axes.

Result: The authors identify three grounding regimes (grounded, prior, and inverted) and find that the taxonomy holds across studied VLMs, with horizontal being grounded, vertical being a prior, and depth being inverted.

Conclusion: The authors propose a cheap, self-calibrating arbiter as a default control for latent-knowledge and steering claims in VLMs, highlighting the importance of verifying grounding in VLMs.
...
### [58] [WarpHammer: Densifying Scene Warps with 3D Object Priors for Extreme View Synthesis](https://arxiv.org/abs/2606.31258)
*Michael Green,Gavriel Habib,Dvir Samuel,Tal Berkovitz Shalev,Issar Tzachor,Rami Ben-Ari,Or Litany*

Main category: cs.CV

TL;DR: Introduced WarpHammer, a training-free framework that improves novel view synthesis (NVS) by augmenting warped scenes with explicit 3D object reconstructions, enabling stable views under large viewpoint changes and fusion of auxiliary views.


<details>
  <summary>Details</summary>
Motivation: Current NVS methods degrade under large orbital motion, causing sparse warps and mirror-like artifacts. The goal is to enhance NVS stability and enable fusion of external object views.

Method: WarpHammer uses a native 3D generative prior to reconstruct the object, adding missing surfaces and occluding background points. It also integrates auxiliary views with a pretrained multi-view geometry model to improve 3D object reconstruction.

Result: WarpHammer produces stable novel views at large viewpoint deviations and can fuse auxiliary object views without requiring camera poses, outperforming strong baselines on five benchmarks.

Conclusion: WarpHammer addresses limitations in current NVS methods by providing a robust and flexible framework for novel view synthesis under large viewpoint changes and with auxiliary views.
...
### [59] [Learning from Failure: Inference-Time Self-Improvement for Computer-Use Agents](https://arxiv.org/abs/2606.31270)
*Xueqiao Sun,Xiaohan Wang,Ludwig Schmidt,Serena Yeung-Levy,Yuhui Zhang*

Main category: cs.CV

TL;DR: The paper proposes a failure-driven self-improvement loop to improve computer-use agents by leveraging failed trajectories to diagnose and fix weaknesses, achieving a 6.6% increase in success rate.


<details>
  <summary>Details</summary>
Motivation: The standard approach to developing computer-use agents relies on successful trajectories, discarding failed ones, which carry rich information about model weaknesses.

Method: The authors employ a large language model to diagnose failure modes, propose solutions, and generate code patches, lightly verified by humans, to upgrade the agent.

Result: The approach improves the success rate of the state-of-the-art OpenCUA-72B model on the OSWorld benchmark from 42.3% to 48.9%, a gain of 6.6 percentage points, without additional training cost and with modest inference overhead.

Conclusion: Failure-driven self-improvement is a viable complement to success-based pipelines, enabling more efficient agent improvement.
...
### [60] [Visual Semantic Entropy: Do Vision Language Models Recognize Visual Ambiguity?](https://arxiv.org/abs/2606.31407)
*Ta Duc Huy,Trang Nguyen,Townim Chowdhury,Ankit Yadav,Minh-Son To,Zhibin Liao,Johan W. Verjans,Vu Minh Hieu Phan*

Main category: cs.CV

TL;DR: The paper proposes Visual Semantic Entropy (VSE) to estimate uncertainty in vision-language models by perturbing only the image and measuring semantic dispersion.


<details>
  <summary>Details</summary>
Motivation: Vision-language models can produce biased predictions on visually ambiguous inputs, and existing entropy-based methods have limitations.

Method: The paper proposes VSE, which perturbs only the image, clusters generated answers into semantic prototypes, and computes mass-weighted dispersion.

Result: VSE effectively captures visual ambiguity and establishes a new state-of-the-art for VLM uncertainty estimation across five modern vision-language models and five diverse VQA benchmarks.

Conclusion: The proposed VSE method improves uncertainty estimation in vision-language models by focusing on visual ambiguity rather than prompt sensitivity.
...
TL;DR: The paper presents CLIMB, a method for online continual self-supervised learning that combines a hierarchical memory with knowledge distillation, outperforming state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods for online continual self-supervised learning rely on either replay buffers or regularization alone, which have limitations. The authors aim to combine both approaches to improve performance.

Method: CLIMB uses a hierarchical centroid-based memory with a bounded number of stored images, along with knowledge distillation on replayed examples to limit representation drift.

Result: CLIMB outperforms state-of-the-art online continual self-supervised learning methods on benchmarks such as Split CIFAR-100 and Split ImageNet-100.

Conclusion: The proposed method, CLIMB, effectively combines a hierarchical memory with knowledge distillation, leading to improved performance in online continual self-supervised learning scenarios.
...
TL;DR: A new method called MICE is proposed for editing multiple parts of an image at once, improving efficiency and reducing errors.


<details>
  <summary>Details</summary>
Motivation: Current image editing methods can struggle with editing multiple parts of an image simultaneously, leading to errors and inconsistencies.

Method: MICE uses Multimodal Diffusion Transformers and modifies the attention mechanism to regulate interactions between different edit instructions, allowing for scalable multi-instance image editing.

Result: MICE outperforms existing methods in terms of visual quality preservation and faithfulness to editing instructions, as demonstrated on LoMOE-Bench and MICE-Bench.

Conclusion: MICE provides a effective solution for editing multiple parts of an image at once, offering improved efficiency and accuracy.
...
TL;DR: ToothForge is a deep spectral generative framework for modeling dental crown geometries from compact representations, achieving comparable or better performance than spatial approaches while maintaining compactness and interpretability.


<details>
  <summary>Details</summary>
Motivation: The limited size of datasets suitable for training dental crown morphology models necessitates a framework that can efficiently learn from compact representations and handle varying connectivity across samples.

Method: ToothForge operates in the spectral domain, using synchronized spectral embeddings to learn a latent manifold of 3D tooth shapes, and mitigates the instability of Laplace-Beltrami eigenbases through spectral synchronization.

Result: ToothForge achieves reconstruction and generative performance comparable to or exceeding PCA-based statistical shape models and point-based generative frameworks, while maintaining compactness and geometric interpretability.

Conclusion: ToothForge is a suitable framework for modeling dental crown geometries from limited datasets, with applications in real-world scenarios where consistent connectivity across shapes is not guaranteed.
...
### [64] [SpikeLogBERT: Energy-Efficient Log Parsing Using Spiking Transformer Networks](https://arxiv.org/abs/2606.31781)
*Thuan Bui,Duong Do,Tung Vu,Duc-Tho Mai,Cong-Kha Pham*

Main category: cs.CV

TL;DR: SpikeLogBERT, a spiking neural network framework, is proposed for energy-efficient log parsing, achieving high accuracy while reducing energy consumption.


<details>
  <summary>Details</summary>
Motivation: Existing neural log parsing methods are computationally expensive and consume high energy due to dense matrix multiplications.

Method: SpikeLogBERT integrates a spiking transformer architecture with knowledge distillation from a BERT teacher model.

Result: SpikeLogBERT outperforms ANN-based models with a parsing accuracy of 0.99997 and reduces estimated energy consumption by up to 62.6%.

Conclusion: SpikeLogBERT offers a promising solution for energy-efficient log parsing without sacrificing accuracy.
...
TL;DR: A novel wavelet-optimized pseudo-3D accelerated diffusion model, CL-DM, is proposed to overcome field-of-view limitations in Computed Laminography (CL) for nondestructive testing of large plate-shaped objects.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning methods for CL truncation reconstruction rely on 2D architectures with limitations, such as lack of data consistency constraints and discarding recoverable information outside the field-of-view.

Method: The proposed CL-DM utilizes a standard 2D diffusion model for slice aggregation, combined with a 3D model-based iterative reconstruction method to ensure strict data consistency, and introduces wavelet regularization along the z-direction.

Result: CL-DM effectively eliminates truncation artifacts and restores high-fidelity, continuous 3D structures, as demonstrated by extensive simulations and real-world experiments.

Conclusion: The proposed CL-DM overcomes the limitations of existing methods, expanding the effective imaging range, enhancing scanning efficiency, and accelerating inference speed.
...
TL;DR: A novel training-free sampling strategy called Accelerated Likelihood Maximization (ALM) is introduced to improve the generation of diverse, coherent, and plausible content from partially given inputs in diffusion models.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for diffusion models have limitations, including costly computation and poor generalization across tasks for training-based approaches, and globally inconsistent results for training-free approaches.

Method: ALM directly optimizes the unobserved region during the sampling process, enabling globally coherent and plausible generation, and incorporates an acceleration strategy to improve computational efficiency.

Result: ALM consistently outperforms state-of-the-art methods in various data domains and tasks, establishing a powerful paradigm for versatile content generation.

Conclusion: The proposed ALM method significantly extends the applicability of diffusion models beyond simple generation tasks, offering a more efficient and effective approach for generating diverse, coherent, and plausible content.
...
TL;DR: A unified framework called Vega is proposed for video understanding and generation, leveraging a shared vocabulary and hybrid architecture.


<details>
  <summary>Details</summary>
Motivation: Extending unified modeling paradigms to the video domain is underexplored due to the challenge of balancing compact semantic representations for understanding and dense signals for generation.

Method: Vega uses a shared vocabulary to model text and visual representations and a hybrid architecture combining autoregressive prediction and diffusion-based rendering.

Result: Vega achieves strong performance on video generation benchmarks like VBench and video understanding benchmarks like VideoMME.

Conclusion: The proposed framework Vega successfully bridges video understanding and generation, demonstrating strong performance on multiple benchmarks.
...
TL;DR: The study examines the impact of learned preprocessing pipelines on orthopedic abnormality detection in elbow radiographs, finding modest and configuration-dependent gains, with a raw-input DenseNet121 baseline remaining competitive.


<details>
  <summary>Details</summary>
Motivation: To assess the impact of learned preprocessing pipelines on patient-level detection of musculoskeletal abnormalities in orthopedic abnormality detection tasks using elbow radiographs.

Method: Comparison of multiple preprocessing pipelines with and without a lightweight DnCNN module, evaluated using discrimination and calibration metrics.

Result: The results show modest and configuration-dependent differences across preprocessing strategies, with no consistent discrimination advantage over the raw-input DenseNet121 baseline.

Conclusion: The study suggests that under patient-level evaluation, preprocessing gains are modest and configuration-dependent, and no tested preprocessing strategy produced a consistent discrimination advantage across all metrics.
...
TL;DR: A novel network, RCL-Mamba, is proposed for image restoration in Rotational Scanning Computed Laminography (RCL), addressing rotational blur and sparse artifacts, and achieving an 8-fold increase in inspection efficiency.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the efficiency and quality of RCL inspections by addressing the issues of rotational blur and sparse artifacts caused by sparse-view scanning.

Method: The method involves a dual-domain State Space Model (SSM)-based image restoration network, RCL-Mamba, which corrects rotational blur in the projection domain and suppresses sparse artifacts in the image domain.

Result: The proposed method, RCL-Mamba, outperforms existing baselines in blur removal, artifact suppression, and structural preservation, and enhances inspection efficiency by approximately 8-fold without compromising reconstruction quality.

Conclusion: The proposed RCL-Mamba network offers a robust measurement-oriented restoration solution for high-throughput RCL inspection with improved structural measurement fidelity.
...
TL;DR: The paper proposes a novel framework called LA-SR for single image super-resolution, which uses vision-language models to bridge the gap between low-resolution (LR) and high-resolution (HR) images without requiring paired HR-LR data.


<details>
  <summary>Details</summary>
Motivation: The difficulty in obtaining paired HR-LR data in reality and the limitations of synthetic degradations in capturing real LR image complexities.

Method: LA-SR redefines unpaired SR in the language space, projecting images into a semantically rich space and applying language-guided losses to preserve semantic fidelity and enhance perceptual realism.

Result: LA-SR effectively super-resolves real LR inputs, producing realistic outputs that overcome the limitations of synthetic-data-trained methods.

Conclusion: The proposed LA-SR framework offers a promising approach for unpaired single image super-resolution, leveraging vision-language models to improve the generalization and realism of SR outputs.
...
TL;DR: The paper proposes a Dual-Stream Bilevel-Cycle Optimization framework to extend Cycle Self-Training (CST) to object detection for unsupervised domain adaptation, addressing challenges of unreliable pseudo-labels, training instability, and exploding losses.


<details>
  <summary>Details</summary>
Motivation: The motivation is to adapt CST, effective in image classification, to object detection, which faces challenges such as unreliable pseudo-labels, training instability, and exploding losses due to the nature of object detection.

Method: The method proposed is the Dual-Stream Bilevel-Cycle Optimization framework, which applies CST to both classification and regression, using Mean Teacher to prevent training instability and extra normalization to address exploding losses.

Result: The framework achieves considerable results across four cross-domain standard scenarios, demonstrating its effectiveness in unsupervised domain adaptation for object detection.

Conclusion: The paper concludes that the proposed framework successfully adapts CST to object detection, overcoming the specific challenges in this field and achieving significant performance in various cross-domain scenarios.
...
TL;DR: Proposed a defense method called MAPE to counter transferable adversarial attacks on image classification tasks, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Neural networks are vulnerable to adversarial examples, and transferable attacks are particularly stealthy and hard to detect.

Method: MAPE, comprising SAPE and PPSA, uses a channel-attention U-Net and strategically schedules pre-trained models to eliminate adversarial perturbations.

Result: MAPE achieved average defense rates of over 95.1% on CIFAR-10 and over 71.5% on Mini-ImageNet in black-box attack scenarios.

Conclusion: MAPE provides a robust defense against diverse transferable attacks, demonstrating state-of-the-art performance in eliminating adversarial perturbations.
...
TL;DR: A new visual resampling framework called MS-Resampler is proposed for multimodal large language models, enabling the capture of both local and global context by aggregating visual information at multiple scopes.


<details>
  <summary>Details</summary>
Motivation: Existing resamplers in multimodal large language models have a fixed aggregation scope, which can limit their ability to capture both local details and global context.

Method: MS-Resampler uses multiple scope-specific resamplers with explicit spatial scope priors and adaptively fuses their outputs to produce final visual representations.

Result: MS-Resampler consistently improves visual understanding and multimodal reasoning over conventional single-scope resamplers on ten public multimodal benchmarks with minimal computational overhead.

Conclusion: MS-Resampler offers an effective and efficient solution for enhancing the visual understanding and multimodal reasoning capabilities of multimodal large language models.
...
TL;DR: OVOW is a training-free system that reconstructs 4D mesh scenes from a single monocular video, enabling physics simulation and embodied AI.


<details>
  <summary>Details</summary>
Motivation: Current 4D reconstruction methods lack watertight topology, instance separation, and standardized physical interfaces required by physics simulators and embodied AI.

Method: OVOW uses a four-stage pipeline: instance discovery and labeling, category-aware reconstruction, iterative render-match-optimize, and physics-grounded assembly.

Result: OVOW achieves the best overall layout and geometry accuracy, and the lowest photometric and semantic error among baselines, with faster processing speed.

Conclusion: OVOW enables the creation of simulation-ready 4D mesh scenes from monocular videos, facilitating physics simulation and embodied AI applications.
...
TL;DR: The paper proposes a diagnostic framework called TemporalLens to test whether video object detectors truly use temporal context, and presents a new real-time spatiotemporal detector, YOLO-3D.


<details>
  <summary>Details</summary>
Motivation: Current metrics for video object detection reward correct predictions regardless of how they are reached, making it unclear if detectors genuinely reason over temporal context.

Method: The authors propose TemporalLens, a model-agnostic diagnostic framework that uses controlled perturbations, occlusions, and other methods to test temporal dependence, and present YOLO-3D, a modular real-time spatiotemporal detector.

Result: The diagnostic framework reveals behavioral differences in detectors, showing that stacked 2D models rely heavily on a single frame, while spatiotemporal models can recover predictions from earlier frames. YOLO-3D achieves a +3.7 pp mAP@50 improvement by preserving temporal depth.

Conclusion: The paper provides a measurable and actionable way to determine if a detector reasons over time, and presents a new architecture that effectively uses temporal context for video object detection.
...
TL;DR: A new method for generating stego images using a prompt-free diffusion framework, which integrates style semantics for more secure and controllable image steganography.


<details>
  <summary>Details</summary>
Motivation: Current methods using text prompts for image steganography are limited by inflexibility and struggle to generate high-quality stego images and accurately recover secret images.

Method: A prompt-free diffusion image steganography framework that integrates style semantic priors, using a Cascaded Affine Coupling Module and a predictor-corrector mechanism.

Result: The proposed method achieves competitive performance compared to state-of-the-art methods in terms of security, secret image reconstruction accuracy, and controllability.

Conclusion: The proposed method provides a more robust and reliable approach to image steganography, overcoming the limitations of current text-prompt-based methods.
...
TL;DR: The study investigates the impact of temporal training-set design on the segmentation of the left atrium and left atrial appendage in dynamic cardiac CT scans, finding that using a physiologically selected subset of frames can achieve comparable performance to using the full sequence.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this study is to improve the segmentation of the left atrium and left atrial appendage in dynamic cardiac CT scans, which is challenging due to large temporal contrast variations and the use of a single annotation per registered sequence.

Method: The study uses the nnUNet-based segmentation method and compares training using a minimal two-frame dataset, a physiologically selected subset of frames, and the full 27-frame sequence. Foreground-based normalization is also evaluated.

Result: The results show that training with all frames yields the best performance in early low-contrast phases, but a physiologically selected subset of frames achieves comparable performance from the filling phase onward. Normalization improves performance in low-contrast frames but does not fully close the gap.

Conclusion: The study concludes that temporal diversity in training data is important for robust segmentation in dynamic CT, and carefully selected frame subsets can provide an effective trade-off between performance and efficiency for downstream applications.
...
TL;DR: A unified encoding for Gregorian notation datasets is proposed, and a shared end-to-end model is trained, achieving state-of-the-art results across four datasets.


<details>
  <summary>Details</summary>
Motivation: The existing datasets for optical recognition of Gregorian notation use different encodings, hindering model comparison and development.

Method: A common encoding based on the S-GABC proposal is designed, and all four datasets are converted to this encoding. A shared end-to-end foundational model is then trained.

Result: The proposed model establishes a new state of the art across all four datasets.

Conclusion: The work provides a unified framework for optical recognition of Gregorian notation, facilitating future research and development.
...
TL;DR: SatAgent, a UAV-Satellite collaborative model, enhances spatial reasoning in complex environments by integrating satellite and UAV perspectives, outperforming existing models by 25.91% and 11.69%.


<details>
  <summary>Details</summary>
Motivation: Existing UAV-based spatial reasoning approaches are limited by single-view perception and lack of explicit geometric modeling, leading to inconsistent reasoning under viewpoint and scale variations.

Method: SatAgent uses a Geometric-Aware 3D Reconstruction Encoder, multi-view topology-semantic alignment module, and multi-view consistency loss to enable robust, accurate reasoning in complex urban environments.

Result: SatAgent outperforms state-of-the-art models by 25.91% and 11.69% across diverse tasks, achieving high accuracy in complex geometric relationship reasoning.

Conclusion: The proposed SatAgent model effectively addresses the limitations of existing UAV-based spatial reasoning approaches, providing a more robust and accurate solution for complex environments.
...
TL;DR: The paper proposes an asynchronous architecture for open-vocabulary 3D scene graph methods, allowing for queryable graphs during exploration and improved semantic richness over time.


<details>
  <summary>Details</summary>
Motivation: Current open-vocabulary 3D scene graph methods operate in two stages, reconstruct then enrich with vision-language models, which leaves the graph unqueryable during exploration. The authors argue that this sequential coupling is unnecessary.

Method: The proposed method uses a probabilistic voxel-based backbone to maintain stable object identities incrementally, while background VLM agents progressively enrich the graph. A multi-target frame scheduler amortizes VLM cost by selecting informative frames.

Result: The method matches or outperforms existing open-vocabulary 3D scene graph methods on semantic segmentation (ScanNet, Replica) and surpasses the prior state-of-the-art across three visual grounding benchmarks (Sr3D+, Nr3D, ScanRefer) by 15.3 to 18.8 A@0.25.

Conclusion: The proposed asynchronous architecture enables queryable 3D scene graphs during exploration and improves semantic richness over time, achieving state-of-the-art results in various benchmarks.
...
TL;DR: The paper proposes DrivingDepth, a method that uses sparse LiDAR data to calibrate a foundation model for dense depth estimation in autonomous driving, achieving state-of-the-art metric accuracy and geometric consistency.


<details>
  <summary>Details</summary>
Motivation: The conflict between dense visual geometry from foundation models and sparse, noisy LiDAR data in autonomous driving.

Method: DrivingDepth uses sparse LiDAR as geometric prompts to locally calibrate a frozen foundation prior through residual pixel-wise scale correction.

Result: DrivingDepth achieves an AbsRel of 11.19 and an EdgeCR of 5.741 on nuScenes with 4-frame surround-view input, outperforming MapAnything.

Conclusion: The proposed method preserves dense visual geometry while delivering state-of-the-art metric accuracy and geometric consistency.
...
TL;DR: The paper proposes HVPNet, a bio-inspired architecture for multimodal salient object detection and camouflaged object detection, achieving a better accuracy-efficiency trade-off.


<details>
  <summary>Details</summary>
Motivation: The authors aim to simplify the modeling strategy for object detection tasks by drawing inspiration from the human visual process, addressing the issue of complex cross-modal feature fusion and decoding structures.

Method: The authors propose a Retinal Integration Module (RIM) for multimodal feature integration and a cortical decoder (CD) for hierarchical processing, mimicking the human retina and visual cortex.

Result: HVPNet establishes an excellent accuracy-efficiency trade-off across 22 datasets spanning seven tasks, without requiring complex structures.

Conclusion: The proposed HVPNet offers a simpler yet effective bio-inspired approach for accurate and efficient object detection, leveraging insights from the human visual process.
...
TL;DR: A deep learning-based framework for precise segmentation of key biomarkers in OCT imaging for geographic atrophy assessment.


<details>
  <summary>Details</summary>
Motivation: The need for precise monitoring of structural biomarkers in geographic atrophy secondary to age-related macular degeneration.

Method: A fully automated, deep learning-based framework using three specialized semantic segmentation models.

Result: High segmentation accuracy and reliability, with Dice scores of 0.88 and 0.87 for RPE loss and EZ loss, respectively.

Conclusion: The proposed framework provides a highly reliable tool for GA assessment in clinical trials and routine real-world ophthalmic care.
...
TL;DR: SimpleSearch-VL is a framework for efficient, reliable, and practical multimodal agentic search, improving search and verification processes without scaling data or tools.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enhance the agent's search and verification process, focusing on efficiency, reliability, and practicality, without relying on extensive data, tools, or auxiliary models.

Method: The method involves Factorized Adaptive Rollout (FAR) for efficient sampling, evidence-verified reasoning for reliability, and a lightweight tool interface for practicality.

Result: SimpleSearch-VL improves Qwen3-VL baselines by 15.8 and 16.0 average points for 8B and 30B-A3B variants, respectively, and achieves performance competitive with agentic Gemini-3-Pro.

Conclusion: The SimpleSearch-VL framework offers an efficient, reliable, and practical approach to multimodal agentic search, achieving competitive performance with limited data and no external dependencies.
...
TL;DR: The paper proposes PRISM, a model for single-image reflection removal that operates in pretrained VAE latent spaces, achieving better generalization and performance than existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing single-image reflection removal methods have limitations due to operating in pixel space, where the nonlinear sRGB formation model complicates layer separation and limits generalization.

Method: The authors propose PRISM, which reinterprets single-image reflection removal as a latent linear separation problem in pretrained VAE latent spaces. It uses a pretrained FLUX backbone, a flow matching velocity field, Latent Composition Consistency (LCC) strategy, and Layer Contrastive Separation (LCS) loss.

Result: PRISM consistently outperforms state-of-the-art methods on six benchmarks, with significant margins, and shows strong generalization to in-the-wild images.

Conclusion: The proposed PRISM model effectively addresses the single-image reflection removal problem by leveraging pretrained VAE latent spaces, leading to improved performance and generalization.
...
TL;DR: MV-GEL, a framework for localizing geometric entities in 3D objects from natural language queries, achieves up to 1.7X improvement in face-level IoU and 4.5X gains in edge-level F1 compared to baselines.


<details>
  <summary>Details</summary>
Motivation: Extending language-driven localization to structured 3D geometry is challenging due to viewpoint sensitivity, occlusion, and foreshortening, which hinder the identification of precise geometric entities.

Method: MV-GEL uses a prompt-conditioned ranking module (GELviews) to select optimal viewpoints based on language-prompted observability, and a VLM-based reasoning segmentation backbone to predict masks, which are then lifted to 3D meshes via geometry-aware ray casting.

Result: The framework achieves substantial improvements over CLIP-based and random view sampling baselines, particularly for thin and view-sensitive structures, with up to 1.7X improvement in face-level IoU and 4.5X gains in edge-level F1.

Conclusion: MV-GEL offers an effective solution for localizing fine-grained geometric entities on polygon meshes from natural language queries, with potential applications in computer-aided design, robotic manipulation, and scientific simulation.
...
TL;DR: A new framework called DataEvolver is proposed for constructing text-rich image data, which uses a self-evolving multi-agent approach to improve data quality and reduce failures.


<details>
  <summary>Details</summary>
Motivation: Existing data pipelines for text-rich image generation have limitations, such as discarding rejected samples that contain useful failure signals, leading to repeated failure modes in later construction rounds.

Method: DataEvolver, a self-evolving multi-agent framework, which includes a Retriever, Verifier, Critic, and Generator to collect, filter, and synthesize data based on feedback.

Result: DataEvolver produces more useful training data than fixed-dataset baselines, with improvements in OCR-F1 of 85.3% on TextScenesHQ and 35.3% on LongTextBench, and transfers to other benchmarks.

Conclusion: The proposed DataEvolver framework can effectively utilize rejected samples to improve text-rich image data construction, leading to better performance in text-rich image generation tasks.
...
TL;DR: AugSplat is a framework that improves Gaussian Splatting in sparse-view 3D vision by using radiance-field-based view augmentation, leading to better reconstruction quality and real-time rendering.


<details>
  <summary>Details</summary>
Motivation: The challenge of generating high-quality novel views at real-time frame rates in sparse-view 3D vision scenarios, where current methods like Neural radiance fields and Gaussian Splatting have limitations.

Method: AugSplat uses radiance-field-based view augmentation to synthesize additional images from nearby novel viewpoints, which are then used as auxiliary supervision during Gaussian Splatting optimization.

Result: AugSplat improves reconstruction quality over standard Gaussian Splatting in sparse-view mip-NeRF 360 scenes, with Staged AugSplat achieving the strongest average performance and Dual AugSplat providing a closely performing alternative.

Conclusion: AugSplat offers a simple and effective framework for improving Gaussian Splatting in sparse-view regimes, enabling better reconstruction quality and preserving real-time rendering at inference.
...
TL;DR: The authors propose MPL-MAE, a masked point learning framework that improves 3D point cloud self-supervised learning by mitigating positional over-reliance and enhancing encoder feature utilization.


<details>
  <summary>Details</summary>
Motivation: The decoder in existing 3D MAE frameworks tends to over-rely on positional information, weakening semantic representation learning and leading to suboptimal feature quality.

Method: The authors propose a recalibrated positional embedding module and a gated positional interface module to regulate positional injection during reconstruction.

Result: MPL-MAE consistently achieves competitive performance across downstream tasks, validating its effectiveness.

Conclusion: The proposed MPL-MAE framework effectively mitigates positional over-reliance and enhances the utilization of encoder features, yielding robust and informative representations.
...
TL;DR: A Physics-aware Neural Operator Transformer (PNOT) is proposed for real-time reconstruction of the divertor temperature field in fusion devices, leveraging physics-aware attention and graph attention to improve prediction accuracy and physical consistency.


<details>
  <summary>Details</summary>
Motivation: The need for a fast and generalizable method for real-time reconstruction of the divertor temperature field to prevent material melting and damage in fusion devices, as conventional numerical methods are computationally expensive.

Method: A Physics-aware Neural Operator Transformer (PNOT) that models boundary heat-flux relations as a structured graph, employs graph attention, and uses a physics-aware neural operator module to aggregate query points with similar physical conditions.

Result: Experimental results show that the proposed method improves prediction accuracy while preserving physical consistency, thanks to the incorporation of physical constraints.

Conclusion: The proposed PNOT offers a promising solution for real-time reconstruction of the divertor temperature field, enabling more efficient and safe operation of fusion devices.
...
TL;DR: The paper explores conformal prediction for image classification using vision-language models, proposing a simple non-linear transformation to improve localized conformal prediction.


<details>
  <summary>Details</summary>
Motivation: The goal is to improve conformal prediction for image classification by adapting to test-time sample similarities, as full conditional guarantee is unattainable.

Method: The authors use a localized conformal prediction algorithm with a non-linear transformation of cosine similarities between test-time and calibration visual features.

Result: The proposed method achieves statistically significant mean set sizes reduction while conserving marginal coverage guarantees, outperforming non-local baselines.

Conclusion: The paper concludes that a simple non-linear transformation of cosine similarities can improve localized conformal prediction for image classification using vision-language models.
...
TL;DR: The paper identifies a performance bottleneck in scaling up novel view synthesis models with camera-based positional encoding and proposes Decoupled Pose Positional Encoding (DPPE) to address this issue.


<details>
  <summary>Details</summary>
Motivation: The authors observed that model performance stagnates in the late stages of training when scaling up novel view synthesis models with camera-based positional encoding.

Method: The authors propose Decoupled Pose Positional Encoding (DPPE), a novel camera-based positional encoding that explicitly decouples rotation and translation.

Result: DPPE enables stable long-term training even in scaled-up training setups and exhibits superior generalization performance in extrapolation settings.

Conclusion: The proposed DPPE method resolves the performance bottleneck in scaling up novel view synthesis models with camera-based positional encoding, leading to improved training stability and generalization performance.
...
TL;DR: A dual-stream RL framework, ViToS, is proposed for medical multimodal reasoning, which prunes visual tokens and answers questions more efficiently.


<details>
  <summary>Details</summary>
Motivation: Current vision-language models struggle with medical images due to sparse visual evidence, and there is a need for a united RL framework for active visual token pruning and medical multimodal reasoning.

Method: A dual-stream RL framework, ViToS, with two task branches, one for grounding and the other for token-sparse reasoning after visual token pruning, and a cross-feedback sequential optimization method to solve the coupled policy learning problem.

Result: ViToS reduces visual tokens to 77% of the original sequence length and achieves a 108.27% relative performance on Lingshu-7B and 104.16% relative performance on HuatuoGPT-Vision-7B.
...
TL;DR: The authors propose an uncertainty-guided synthetic context augmentation strategy to improve semantic segmentation models on sparse and diverse data, achieving substantial gains on complex datasets.


<details>
  <summary>Details</summary>
Motivation: Semantic segmentation models struggle with data sparsity and rare or visually diverse regions, and existing solutions often rely on external models or coarse heuristics, which are inefficient or risk label misalignment.

Method: The authors propose an uncertainty-guided synthetic context augmentation strategy that identifies uncertain semantic regions using predictive entropy, inpaints only the complementary visual context, and focuses learning on unmodified, uncertain regions.

Result: The authors demonstrate substantial mIoU gains on Cityscapes, UAVID, and BDD100K, with the largest gains on rare and difficult classes such as buses, trains, or cars from an aerial perspective.

Conclusion: The authors conclude that uncertainty-guided context augmentation is a highly effective approach to improve segmentation performance on complex datasets.
...
TL;DR: A new framework for radar segmentation that uses higher-order structural alignment and learnable hypergraphs to improve performance.


<details>
  <summary>Details</summary>
Motivation: Existing radar segmentation methods struggle to capture complex relationships between radar returns from the same physical object, leading to poor performance in dense semantic segmentation.

Method: A unified higher-order structural alignment framework that uses learnable hypergraphs and Unbalanced Optimal Transport (UOT) to refine radar feature representations and align view-specific features.

Result: The proposed method achieves 63.8% mIoU on CARRADA and 83.4% mIoU on RADIal, outperforming previous best methods by +1.7 and +2.3 mIoU, respectively.

Conclusion: The results highlight the importance of higher-order relational modelling for robust radar perception, and the proposed framework demonstrates consistent improvements over strong radar-specific baselines.
...
TL;DR: The paper proposes an Active Task Driving Memory (ATMem) for mobile GUI agents to improve their ability to handle long-horizon tasks by actively maintaining task-relevant information and an online reinforcement learning method called STR-GRPO to selectively use ATMem.


<details>
  <summary>Details</summary>
Motivation: Existing memory methods for mobile GUI agents treat memory as passive storage, which can lead to unreliable performance in long trajectories with similar fields, repeated values, distractors, and outdated states.

Method: The paper proposes ATMem, which maintains task-relevant information as a continually updated execution state, and STR-GRPO, an online reinforcement learning method that learns to use ATMem selectively.

Result: The paper introduces a challenging mobile benchmark to evaluate the performance of agents in completing all in-scope work while avoiding out-of-scope actions over long-horizon execution.

Conclusion: The paper concludes that ATMem and STR-GRPO can improve the performance of mobile GUI agents in handling long-horizon tasks by actively maintaining task-relevant information and selectively using memory.
...
TL;DR: The paper proposes a decoupled, multi-vehicle landing framework for autonomous aerial inspection of marine infrastructure, achieving a 100% landing success rate in high-fidelity maritime simulations.


<details>
  <summary>Details</summary>
Motivation: Autonomous aerial inspection of marine infrastructure is often compromised by stochastic sea states, posing risks of high-kinetic impacts, post-landing toppling, and sensory occlusion.

Method: The framework utilizes two independent Deep Reinforcement Learning (DRL) agents: a Soft Actor-Critic (SAC) agent for high-frequency wave-motion compensation and a multimodal RL agent for the UAV's final approach.

Result: The system achieved a 100% landing success rate across 15 trials, with a mean stabilization efficacy of 87.8%, maintaining the landing surface within 1 degree of the horizontal plane for 96% of the mission duration in rough conditions.

Conclusion: The proposed framework effectively contributes to safer landings by achieving high stabilization efficacy and a high landing success rate in various wave conditions.
...
TL;DR: A new method called PrISM-IQA is proposed for assessing smartphone image quality by identifying and severity-grading specific issues rather than assigning a single quality score.


<details>
  <summary>Details</summary>
Motivation: Current image quality assessment methods do not align well with practical image signal processor (ISP) tuning needs, which require identifying specific quality issues and their severities.

Method: PrISM-IQA reformulates image quality assessment as a multi-issue ordinal diagnosis problem, predicting ordered severity levels for ISP-relevant issues using cumulative ordinal encoding and structured inference.

Result: PrISM-IQA shows effectiveness in practical issue-level diagnosis, reveals transferable perceptual quality representations, and supports actionable ISP tuning.

Conclusion: PrISM-IQA offers a more practical approach to image quality assessment for smartphone images by focusing on specific issues and their severities, aiding in ISP tuning.
...
TL;DR: LiteMatch, a lightweight stereo matching framework, achieves strong zero-shot generalization without expensive 3D convolutions.


<details>
  <summary>Details</summary>
Motivation: Current learning-based stereo matching methods often require heavy backbones and intensive 3D cost volume processing, resulting in high memory and runtime overhead, and struggle to generalize across domains.

Method: LiteMatch uses two complementary encoders (CVCE and HFE), Cost Volume Consistency Loss (CVC-Loss), and a lightweight refinement module.

Result: LiteMatch achieves competitive EPE and D1 performance across Scene Flow, KITTI, Middlebury, ETH3D, and DrivingStereo with a flexible design ranging from 3.36M to 9.58M parameters.

Conclusion: Lightweight architectures can generalize across domains without sacrificing accuracy, as demonstrated by LiteMatch's exceptional zero-shot generalization performance.
...
TL;DR: RoboSpatialBrain, a vision-language model, achieves strong spatial reasoning for embodied tasks by combining two training-free mechanisms and fine-tuning on compatibility data.


<details>
  <summary>Details</summary>
Motivation: Vision-language models struggle with spatial reasoning required for embodied tasks, and the authors aim to address this limitation.

Method: The authors propose RoboSpatialBrain, which uses a forced <think> prefix activation strategy, task-specific post-prompt, and explicit reference-frame redirection pipeline.

Result: RoboSpatialBrain achieved first place in the RoboSpatial Challenge with an overall success rate of 80.9% on RoboSpatial-Home.

Conclusion: The proposed approach effectively enhances spatial reasoning capabilities of vision-language models for embodied tasks.
...
TL;DR: A novel loss function called Q-Margin is proposed for face and speaker verification, which outperforms existing methods like ArcFace and CosFace, especially at low False Acceptance Rates.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the limitations of standard geometric margins in softmax losses and to leverage the benefits of α-divergence loss functions, particularly their ability to induce sparse solutions.

Method: The authors propose Q-Margin, a novel α-divergence loss that introduces a principled probabilistic margin by encoding the margin penalty directly into the reference measure (prior probabilities).

Result: Q-Margin achieves competitive or superior performance on IJB-B and IJB-C face verification benchmarks and VoxCeleb speaker verification benchmark, with consistent improvements at low False Acceptance Rates.

Conclusion: The proposed Q-Margin loss function offers a scalable solution for large-scale face and speaker verification tasks, particularly in high-security applications, due to its ability to induce sparse solutions and enable exact and memory-efficient training.
...
TL;DR: Proposed SAMBA, a self-supervised pre-training model for SAR target interpretation, addressing limitations of current models with innovative architecture and masking strategy.


<details>
  <summary>Details</summary>
Motivation: Scarce annotated training data for SAR ATR and limitations of current Transformer architectures and masking strategies.

Method: SAMBA model with Mamba encoder, Scattering-Guided Masked Autoencoder (SG-MAE) masking strategy, and SpatialMix feature interaction module.

Result: SAMBA achieves state-of-the-art performance on most metrics across diverse SAR interpretation tasks, with substantially fewer parameters.

Conclusion: SAMBA is an efficient and effective self-supervised pre-training foundation model for SAR target interpretation, with robust generalizability across tasks.
...
TL;DR: Introduced WorldRoamBench, an open-world benchmark for evaluating interactive world models across four dimensions: action, vision, physics, and memory, revealing limitations in existing models.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for interactive world models only evaluate action following at the trajectory level, ignoring memory and interaction physics, which are crucial for real-world applications.

Method: WorldRoamBench consists of 600+ test cases across three scenes and first/third-person views, with four tailored innovations: per-frame action metric, segment-based drift metric, controllability-gated evaluation, and action-decoupled protocol.

Result: Evaluating 10+ models showed that none reliably satisfies all dimensions, with even the best achieving only moderate scores, indicating a need for further advancements.

Conclusion: WorldRoamBench provides a comprehensive evaluation framework for interactive world models, highlighting the need for improvements in stability, physical grounding, memory faithfulness, and deployability in real-world applications.
...
TL;DR: REDI, a token reduction method for Vision Transformers, allocates a fixed token budget across patches for image classification by deriving class-conditioned corpus scores using supervised TF-IDF over visual words.


<details>
  <summary>Details</summary>
Motivation: To study how a fixed token budget should be allocated across patches for image classification in Vision Transformers.

Method: REDI uses supervised TF-IDF over visual words to derive class-conditioned corpus scores, and combines these scores with attention maps to rank patches and assign roles.

Result: The REDI variant achieves 84.706% Top-1 accuracy on ImageNet-1K with a 46.8% sequence reduction, outperforming dense baseline and other variants.

Conclusion: Class-specific corpus statistics and image-specific attention provide complementary signals for patch ranking in token reduction for Vision Transformers.
...
TL;DR: ShellMaker is a framework for generating complete exterior building meshes with consistent interiors, given a building scaffold and text style prompt.


<details>
  <summary>Details</summary>
Motivation: The challenge of synthesizing coherent building exteriors consistent with generated interiors, as existing methods only generate structural shells without stylistically consistent facades and roofs.

Method: ShellMaker combines parametric roof generation, LLM-based prompt refinement, joint wall-roof material retrieval, and geometry-aware assembly to generate complete exterior meshes.

Result: ShellMaker maintains structural consistency and improves architectural coherence over baselines, generalizing to various input formats.

Conclusion: ShellMaker offers a solution for generating complete exterior building meshes with consistent interiors, advancing indoor scene generation.
...
TL;DR: A novel control mechanism for diffusion models called Histogram-constrained Image Generation (HIG) is introduced, enabling exact precision in user-specified distributional constraints during image generation.


<details>
  <summary>Details</summary>
Motivation: Controlling diffusion models to produce outputs aligned with user intent remains an open challenge, particularly in balancing global coherence with local precision.

Method: The framework enforces user-specified distributional constraints during generation with exact precision by modeling control as an optimal transport (OT) problem and applying explicit guidance transformations during sampling.

Result: The versatility of HIG is demonstrated across diverse applications, including constrained generation and high-capacity information embedding through histogram-level encoding.

Conclusion: The findings underscore the promise of distributional control, a flexible and interpretable control scheme that is fully compatible with existing control mechanisms, diversifying hybrid strategies for controllable image generation.
...
TL;DR: The paper proposes a Dual Range-Voxel Representation (DRVR) for 3D semantic occupancy prediction in autonomous driving systems, achieving better performance and efficiency than existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods for 3D semantic occupancy prediction often rely on stacking multi-sweep point clouds, which leads to efficiency and robustness concerns. The authors aim to eliminate these concerns by leveraging single-sweep point clouds.

Method: The authors propose a DRVR that uses a range-view encoder to extract compact context and a geometry-aware voxel-view encoder to extract multi-scale voxel-view features. A range-voxel fusion module is also proposed to cooperate range- and voxel-view features.

Result: The proposed method achieves state-of-the-art performance on nuScenes-Occupancy, SemanticKITTI, and SemanticPOSS datasets. Specifically, it achieves a 5.4% improvement in mIoU and 2.1x acceleration compared to the multi-sweep method on nuScenes-Occupancy.

Conclusion: The proposed DRVR method provides a more efficient and robust solution for 3D semantic occupancy prediction, achieving better performance and efficiency than existing methods.
...
TL;DR: Proposed IS-SNN architecture removes batch normalization layers, achieving competitive performance with reduced hardware resource consumption.


<details>
  <summary>Details</summary>
Motivation: The use of batch normalization in deep spiking neural networks (SNNs) contradicts their hardware-efficiency motivation, and removing it causes severe performance degradation.

Method: The Intrinsically Stable SNN (IS-SNN) architecture uses topology-aware weight standardization and modified residual connections to enforce signal homeostasis, removing activation-normalization layers.

Result: IS-SNN achieves competitive or superior performance to dynamic BN techniques across various models, including 68.05% accuracy on ImageNet, and reduces FPGA lookup table resource consumption by 96.4%.

Conclusion: IS-SNN is a practical framework for building accurate and hardware-friendly deep neuromorphic systems, overcoming the limitations of prior BN-free attempts.
...
TL;DR: Sparse autoencoders (SAEs) can detect and localize semantic concepts in diffusion models but are not effective for direct manipulation, causing visual artifacts.


<details>
  <summary>Details</summary>
Motivation: To evaluate the assumption that isolated features from sparse autoencoders (SAEs) can serve as controllable intervention points for concept-level manipulation in diffusion models.

Method: The authors used SAEs to detect and localize semantic concepts within diffusion model activations, then attempted direct intervention in the latent space and compared it with a detection-based replacement approach.

Result: Direct intervention using SAEs induced out-of-distribution activations and severe visual artifacts, while the detection-based replacement approach preserved activation statistics and produced cleaner erasure results.

Conclusion: There is a fundamental gap between concept detection and concept intervention in diffusion models; SAEs are powerful for analyzing generative models but have limitations for direct manipulation.
...
TL;DR: A new framework called Phantom is proposed to protect against face-swapping deepfakes by constraining perturbations in both latent and spatial domains, improving protection success rates and visual quality.


<details>
  <summary>Details</summary>
Motivation: Face-swapping deepfakes pose a threat to personal privacy, and existing adversarial approaches have limitations in face-swapping scenarios, including ambiguous latent guidance and lack of spatial constraints.

Method: Phantom, a unified face-swap deepfake protection framework, which adaptively synthesizes identity-shifted yet attribute-preserving targets and applies masked perturbations confined to semantically relevant facial regions.

Result: Phantom improves protection success rates by 27.8%, 25.6%, and 16.6% on UniFace, INSwapper, and SimSwap, respectively, and generalizes to impersonation scenarios with up to 10.2% higher protection while improving perceptual fidelity.

Conclusion: The proposed Phantom framework effectively protects against face-swapping deepfakes by jointly leveraging latent and spatial constraints, demonstrating improved protection success rates and visual quality.
...
TL;DR: A new dataset, WIDER-FAIR, is introduced to study fairness in face detection models by annotating ethnicity and sex, revealing performance disparities across demographic groups.


<details>
  <summary>Details</summary>
Motivation: The lack of face detection datasets with sensitive feature annotations hinders studying and mitigating biases in face detection models.

Method: A new dataset, WIDER-FAIR, is created by annotating the WIDER-FACE benchmark with perceived ethnicity and sex, and its quality is assessed using face embeddings, a K-Nearest Neighbors classifier, and t-SNE visualization.

Result: Experiments with a YOLOv5 model show detection performance is lower for Black individuals and excluding this group from training increases fairness disparity more than excluding other ethnic groups.

Conclusion: The study highlights the importance of demographically annotated datasets like WIDER-FAIR for understanding and evaluating bias in face detection models.
...
TL;DR: The paper introduces UniCoder, a unified RL framework for visual-to-code generation that addresses the limitations of standard MLLMs and achieves state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the pixel-precise alignment in visual-to-code generation, which is challenging for standard MLLMs.

Method: The method involves a unified RL framework called UniCoder, which integrates two novel mechanisms: Symbolic Attribute Alignment and Reference-Guided Code Optimization.

Result: The result shows that the 8B-parameter model surpasses all open-source baselines and achieves state-of-the-art performance comparable to proprietary models on several benchmarks.

Conclusion: The conclusion is that UniCoder establishes a new paradigm for generalized visual-to-code synthesis.
...
TL;DR: MemLearner, a learning-based method, improves video world models by enhancing memory and scene consistency over long durations, especially in scenarios with occlusions and dynamic objects.


<details>
  <summary>Details</summary>
Motivation: The lack of memory in video world models causes inconsistent generated scenes over time, and previous methods fail to generalize to complex scenarios.

Method: MemLearner uses query tokens to adaptively retrieve context from video frames, leveraging pre-trained visual priors and efficient training strategies.

Result: MemLearner outperforms prior video world models in scene consistency and memory, particularly in occlusion and dynamic scenarios.

Conclusion: MemLearner effectively addresses the memory challenge in video world models, enabling more consistent and realistic scene generation over extended durations.
...
TL;DR: A new framework called RhythmJEPA is proposed for remote photoplethysmography (rPPG) that improves physiological signal estimation from facial videos by incorporating a cyclic rhythm structure and a novel Mamba encoder.


<details>
  <summary>Details</summary>
Motivation: Existing self-supervised rPPG methods have limitations, such as bias towards facial appearance and inability to exploit the cyclic structure of pulse signals. Recent Mamba-based approaches also have limitations in scanning facial video tokens only in chronological order.

Method: RhythmJEPA, a rhythm structured joint-embedding predictive learning framework, which includes a Cyclic Rhythm-State Planner (CRSP), a Dual Order Mamba Encoder (DOM), and a Spatial Pulse Mixer (SPM).

Result: Experiments on PURE, UBFC-rPPG, and MMPD show competitive performance over representative rPPG methods.

Conclusion: The proposed RhythmJEPA framework shows improved performance in estimating physiological signals from facial videos, addressing the limitations of existing methods.
...
TL;DR: A new benchmark, JL1-CC&QA, is introduced for remote sensing change detection, providing multi-task annotations including change captioning and question answering.


<details>
  <summary>Details</summary>
Motivation: Traditional remote sensing change detection focuses on pixel-level binary segmentation, lacking semantic information about what and why changes occur.

Method: The benchmark is built using 5,000 bi-temporal image pairs and annotations are produced via a three-stage pipeline: LLM generation, vision-grounded LLM judging, and human expert verification.

Result: The JL1-CC&QA benchmark includes 17,021 captions and 20,060 question-answer pairs, enabling fine-grained change understanding.

Conclusion: JL1-CC&QA serves as a valuable resource for advancing multi-task change understanding in remote sensing.
...
TL;DR: The paper uses rolling-shutter distortions to estimate 3D velocities of moving spheres from a single frame.


<details>
  <summary>Details</summary>
Motivation: Rolling-shutter distortions are typically considered artifacts, but here they are leveraged as a source of temporal information.

Method: A robust spherical pattern and a correspondence-free formulation are proposed to recover motion through geometric consistency and a two-stage optimization process.

Result: Accurate and robust estimation of motion parameters under high-speed conditions, even for textureless objects.

Conclusion: The approach effectively utilizes rolling-shutter distortions to estimate 3D translational and angular velocities of rapidly moving spherical objects.
...
TL;DR: A new method called Barycentric Dominance Field (BDF) is proposed to improve 3D mesh generation by bridging discrete mesh topology and continuous generative modeling.


<details>
  <summary>Details</summary>
Motivation: The autoregressive (AR) modeling paradigm has limitations in 3D mesh generation, including limited face counts, bounded vertex resolutions, and difficulties in supporting textures.

Method: The BDF is a continuous representation defined on triangular mesh surfaces that encodes vertex topological connectivity, allowing for seamless integration into existing 3D diffusion pipelines.

Result: BDF enables diffusion models to generate native meshes with higher quality, greater scalability, and stronger robustness compared to state-of-the-art autoregressive methods.

Conclusion: The proposed BDF method overcomes the limitations of autoregressive modeling in 3D mesh generation, enabling the creation of more complex and detailed meshes.
...
TL;DR: The authors propose Medical Reasoning-aware Policy Optimization (MRPO), a reinforcement learning algorithm that improves the reasoning process in clinical image analysis by assigning step-wise process rewards, reducing cascading errors and improving final answer accuracy.


<details>
  <summary>Details</summary>
Motivation: The authors are motivated by the issue of sparse credit assignment in existing post-training pipelines for clinical image reasoning, which makes it difficult to optimize the reasoning process. They identify cascading errors from early-stage reasoning failures as a leading cause of incorrect predictions in medical visual question answering benchmarks.

Method: The authors propose MRPO, an RL algorithm that incorporates step-wise process rewards. When the final answer is incorrect, MRPO assigns exponentially larger penalties to tokens in earlier invalid reasoning steps, breaking failure cascades without compromising successful paths.

Result: MRPO consistently outperforms standard GRPO and a recent RL baseline across three multimodal LLM backbones. On Qwen3-VL-8B-Instruct, MRPO even surpasses substantially larger medical MLLMs such as HuatuoGPT-Vision-34B by 2.79 points. MRPO reduces early-stage reasoning failures from 64.0% to 13.0%.

Conclusion: The authors conclude that targeted mitigation of cascading failures improves both reasoning quality and final answer accuracy, and that MRPO is an effective approach for optimizing the reasoning process in clinical image analysis.
...
### [119] [Real-Time Source-Free Object Detection](https://arxiv.org/abs/2606.31834)
*Sairam VCR,Varun Gopal,Poornima Jain,Vineeth N Balasubramanian,Muhammad Haris Khan*

Main category: cs.CV

TL;DR: A novel source-free object detection method, RT-SFOD, is proposed, achieving state-of-the-art accuracy while being faster and more compact than existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing source-free object detection methods prioritize accuracy over latency and memory constraints, which are crucial for real-world applications such as autonomous driving and robotics.

Method: The proposed RT-SFOD method builds on YOLOv10 and introduces two novel modules: DHF (Dual-Head Pseudo-Label Fusion) for pseudo-label generation and MARD (Multi-scale Adaptive Representation Diversification) loss for feature diversification.

Result: RT-SFOD yields 1.4 to 3.5% mAP gains, 1.3 times higher throughput, and approximately 2 times fewer parameters than prior state-of-the-art SFOD methods.

Conclusion: The proposed RT-SFOD method advances the Pareto frontier of the speed-accuracy-model size trade-off, making it suitable for real-world applications with strict latency and memory constraints.
...
TL;DR: The authors propose a self-supervised temporal regularization method to improve cardiac segmentation consistency over time in image sequences, particularly in cardiac ultrasound.


<details>
  <summary>Details</summary>
Motivation: Existing graph-based cardiac segmentation models provide topological guarantees but exhibit temporal discontinuities when trained on independent frames, affecting reliable clinical measurements.

Method: The authors introduce a post-training refinement stage that penalizes velocity and acceleration discontinuities across consecutive frames to enforce consistent cardiac segmentation and motion estimation over time.

Result: The proposed method achieves temporally consistent segmentations while maintaining learned anatomical correspondences, enabling automatic mapping of landmarks to the AHA 17-segment clinical standard.

Conclusion: The method demonstrates clinical utility in combining temporal consistency with automatic regional mapping, validated on the CAMUS dataset.
...
TL;DR: The paper proposes a method to accelerate multimodal large language models (MLLMs) by selectively skipping redundant visual-token computations at the operator level, achieving a balance between efficiency and accuracy.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the increasing computational cost of MLLMs due to long visual-token sequences, and to find a more fine-grained approach than existing methods that remove or skip visual tokens entirely.

Method: The proposed method involves decomposing each Transformer layer into attention and FFN operators, identifying operator-dominant and layer-dependent useful visual computation, and selectively bypassing redundant attention, FFN, or both.

Result: The experiments show that the proposed method reduces 33.7% TFLOPs on Qwen3-VL while retaining 99.5% of the vanilla model performance, achieving strong efficiency-accuracy trade-offs across three MLLM architectures and 10 VQA benchmarks.

Conclusion: The paper concludes that the proposed operator-level visual-token skipping framework can effectively accelerate MLLMs while preserving their performance, by selectively skipping redundant visual-token computations.
...
TL;DR: MuSViT, a Music Score Vision Transformer, is introduced as the first foundation vision model for sheet music representation, outperforming existing models in various music-related tasks.


<details>
  <summary>Details</summary>
Motivation: The lack of a strong domain-specific backbone for sheet music, which is a visual encoding of musical language, unlike vision and language processing which have rich, reusable representations.

Method: A ViT encoder pre-trained via Masked Autoencoders on 9.7 million pages from the IMSLP, using a two-stage curriculum: synthetic warm-up on typeset scores and large-scale training on the full IMSLP corpus.

Result: MuSViT consistently outperforms modern vision encoders under linear probing and fine-tuning, improving upon task-specific state-of-the-art methods in four downstream tasks, and encoding symbolic musical structure directly in its representation space.

Conclusion: MuSViT establishes itself as a foundation backbone for sheet music understanding, providing rich, reusable representations for musical language.
...
TL;DR: LUNA, a neural animation model, creates 3D human avatars from 2D controls without Linear Blend Skinning, achieving realistic motion and zero-shot cross-identity generalization.


<details>
  <summary>Details</summary>
Motivation: Current 3D human avatar creation methods rely on Linear Blend Skinning and parametric body models, which limit expressivity and introduce artifacts. The authors aim to overcome these limitations.

Method: LUNA uses a transformer-based motion regressor and hybrid supervision to map 2D controls to 3D Gaussian deformations, bypassing explicit body fitting.

Result: LUNA achieves competitive visual fidelity and delivers realistic human motion and zero-shot cross-identity generalization across diverse driving modalities.

Conclusion: LUNA is the first end-to-end 3D animatable model that supports implicit 2D driving, offering improved expressivity and reduced artifacts compared to traditional methods.
...
TL;DR: TopoGPT is a generative framework for lane topology reasoning that learns geometry priors from lane graph structures and achieves state-of-the-art performance on the OpenLane-V2 benchmark.


<details>
  <summary>Details</summary>
Motivation: Existing lane topology reasoning methods treat each lane instance independently, leading to geometric inconsistencies and incomplete graphs. The authors aim to address these issues with a generative framework.

Method: TopoGPT uses autoregressive sequence modeling to learn geometry priors from typical lane graph structures. It involves a lane tokenizer, scene context encoder, and a pre-trained autoregressive lane sequence transformer.

Result: TopoGPT outperforms existing methods by an average of +6.4 on lane-level and +11.6 on point-level metrics on the OpenLane-V2 benchmark, producing geometrically consistent and structurally complete lane graphs.

Conclusion: The proposed TopoGPT framework effectively addresses the issues of geometric inconsistency and incomplete graphs in lane topology reasoning, achieving state-of-the-art performance.
...
TL;DR: A deep learning framework called FLORA is proposed to predict forest attributes from heterogeneous LiDAR point clouds, achieving robust results across different seasons and conditions.


<details>
  <summary>Details</summary>
Motivation: The challenge of producing wall-to-wall predictions of forest attributes from LiDAR data acquired under heterogeneous conditions, which limits the robustness of existing models.

Method: FLORA, a deep learning framework that combines an octree-based backbone with ecological and spatiotemporal auxiliary variables through a late-fusion gating mechanism.

Result: FLORA achieves an rRMSE of about 12.3% (R2 = 0.88) for dominant height and 39% (R2 = 0.74) for total volume, providing a robust baseline for large-scale forest attribute estimation.

Conclusion: FLORA provides a robust and accurate method for predicting forest attributes from heterogeneous LiDAR point clouds, which can be used for large-scale forest monitoring and management.
...
TL;DR: Proposes an Absorption-Guided Distance-Decoupled Estimation and Refinement (ADER) framework for long-range passive ranging using LWIR hyperspectral observations.


<details>
  <summary>Details</summary>
Motivation: Existing methods for passive ranging are computationally expensive and do not effectively utilize atmospheric absorption structures, making distance inversion challenging.

Method: ADER framework, which includes B-spline control points for emissivity representation, ozone-absorption cues for pixel classification, and a greedy band selection strategy.

Result: ADER achieves LiDAR-consistent spatial distance structures, improves ranging accuracy, and achieves approximately two orders of magnitude speedup over a public full-band hyperspectral ranging method.

Conclusion: The proposed ADER framework effectively addresses the challenges of passive ranging using LWIR hyperspectral observations, offering improved accuracy and efficiency.
...
### [127] [PriorEye: Geospatial Visual Priors for End-to-End Autonomous Driving](https://arxiv.org/abs/2606.31830)
*Kyuhwan Yeon,Benjamin Ramtoula,Daniele De Martini*

Main category: cs.CV

TL;DR: The paper introduces a method to improve end-to-end autonomous driving by incorporating geospatial visual priors, providing anticipatory foresight and improving robustness against sensor corruption.


<details>
  <summary>Details</summary>
Motivation: Current end-to-end autonomous driving methods are limited to reactive behavior as they rely solely on instantaneous sensor observations, lacking the anticipatory foresight human drivers have through prior experience.

Method: The authors propose a memory augmentation module with a dual-memory architecture and an adaptive memory gate, integrating geospatial visual priors into existing end-to-end approaches.

Result: The approach consistently improves performance across diverse end-to-end baselines on the NAVSIM-v2 benchmark and inherently improves robustness against sensor corruption.

Conclusion: The proposed method enhances end-to-end autonomous driving by providing visual-spatial foresight and improving robustness, making it a valuable addition to existing approaches.
...
### [128] [Towards Voxel Spacing Consistency for Medical Image Segmentation](https://arxiv.org/abs/2606.31839)
*Xin You,Runze Yang,Minghui Zhang,Hanxiao Zhang,Han Li,Yi Yu,Jie Yang,Nassir Navab,Yun Gu*

Main category: cs.CV

TL;DR: The authors propose Consispace, a semantic-aware resampling framework for volumetric medical image segmentation that maintains consistent voxel spacing and preserves anatomical and semantic consistency, leading to improved segmentation performance.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the importance of physical voxel spacing in volumetric medical image segmentation, which has been comparatively under-explored despite its crucial role in preoperative diagnosis and intraoperative guidance.

Method: The authors propose a novel framework called Consispace, which utilizes an ODE-based anatomical constraint and dense features from a pretrained vision model to achieve consistent voxel spacing and preserve anatomical and semantic consistency.

Result: The results show that Consispace achieves superior reconstruction quality and perceptual fidelity, produces smoother inter-slice anatomy, and improves downstream segmentation performance when used as a preprocessing step on multiple datasets.

Conclusion: The authors conclude that Consispace is an effective semantic-aware resampling framework that can improve the accuracy of volumetric medical image segmentation by maintaining consistent voxel spacing and preserving anatomical and semantic consistency.
...
TL;DR: Introduces SENSE-VAD, a benchmark for detecting socially complex anomalies in autonomous driving, and evaluates state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Current anomaly detectors in autonomous vehicles struggle with socially complex situations, such as a child running away from a guardian or a pursuer chasing a pedestrian, which don't present obvious motion signals.

Method: Uses CARLA simulator and Unreal Engine to generate synthetic video anomaly scenarios across multiple categories, with per-frame binary labels, and provides real-world videos for sim-to-real transfer.

Result: State-of-the-art video anomaly detection methods are evaluated and found to be insufficient for detecting socially complex anomalies.

Conclusion: SENSE-VAD dataset and code are publicly available, highlighting the need for improved anomaly detection in autonomous driving.
...
TL;DR: RESOLVE, a benchmark dataset for roadside 3D detection and tracking, evaluates unimodal and camera-LiDAR fusion architectures under varying LiDAR point sparsity.


<details>
  <summary>Details</summary>
Motivation: Investigate the impact of LiDAR point sparsity on unimodal and camera-LiDAR fusion architectures in roadside cooperative perception.

Method: Introduces RESOLVE dataset with multi-resolution LiDAR and synchronized camera-LiDAR sensing for evaluating 3D detection and tracking architectures.

Result: Benchmark experiments provide insights into multimodal fusion compensating for LiDAR point sparsity, guiding cost-efficient roadside multimodal perception design.

Conclusion: RESOLVE dataset and benchmark facilitate fair comparisons of architectures under point cloud distribution shifts, offering clues for designing cost-efficient roadside multimodal perception.
...
TL;DR: The paper proposes DriveWeaver, a framework for inserting vehicles into simulated autonomous driving scenes, addressing lighting inconsistencies and limited 3D assets.


<details>
  <summary>Details</summary>
Motivation: Existing methods for autonomous driving simulation rely on pre-reconstructed 3D assets, leading to lighting inconsistencies and limited scalability.

Method: DriveWeaver uses video inpainting conditioned on vehicle point clouds to generate high-quality, temporally consistent vehicles, with a global-to-local hierarchical inpainting strategy for long-term generation.

Result: The method outperforms existing baselines in visual realism and geometric consistency, enabling real-time rendering for autonomous driving simulation.

Conclusion: DriveWeaver provides a robust tool for scalable autonomous driving scene augmentation, addressing the challenges of lighting inconsistencies and limited 3D assets.
...
TL;DR: InstanceControl is proposed as a novel multi-instance controllable generation method that eliminates the need for instance labeling, leveraging VLM to establish instance-level correspondences and introducing an adaptive mask refinement strategy.


<details>
  <summary>Details</summary>
Motivation: Existing controllable image generation methods struggle with complex multi-instance scenes, leading to attribute confusion among instances, and require labor-intensive manual instance labeling.

Method: InstanceControl uses Vision-Language Model (VLM) to parse instance descriptions from text prompts and predict instance masks based on visual conditions, and introduces an adaptive mask refinement strategy to refine these instance masks during generation.

Result: InstanceControl outperforms state-of-the-art methods, achieving superior fidelity and precise instance-level control.

Conclusion: The proposed InstanceControl method effectively addresses the limitations of existing controllable image generation methods in handling complex multi-instance scenes without requiring labor-intensive instance labeling.
...
TL;DR: A new benchmark, VidPair-Halluc, is introduced to evaluate video hallucination in large video models, focusing on precise attribution of model errors to genuine hallucination.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for video hallucination have limitations, such as relying on text-based perturbations or neglecting visual background consistency, which can lead to inaccurate assessments of model performance.

Method: The VidPair-Halluc benchmark is constructed using PairFlow, a pipeline that generates coherent video clips and assembles them into adversarial pairs with controlled background and foreground variations.

Result: Evaluations on mainstream large video models show persistent difficulty with robust fine-grained video understanding in adversarial settings.

Conclusion: The VidPair-Halluc benchmark provides a rigorous and controlled evaluation of video hallucination in large video models, highlighting the need for improved fine-grained video understanding.
...
TL;DR: Introduced World Narrative Model (WNM) for controllable video generation, decoupling narrative from rendering, enabling deterministic content creation.


<details>
  <summary>Details</summary>
Motivation: Existing video generation models lack controllability, making professional content creation inefficient and expensive due to the 'gacha' loop.

Method: WNM uses 4D pre-visualization, translating multimodal inputs into editable world representations, driving video foundation models as neural shaders.

Result: WNM reduces 'gacha' calls, producing videos closely following creator intent, with a modular framework for future improvements.

Conclusion: WNM enables efficient, controllable video generation, aligning with professional filmmaking pipelines, and facilitating human-AI collaboration.
...
TL;DR: AnyBokeh is a physics-guided framework for bokeh editing from a single image captured under arbitrary focus and aperture settings.


<details>
  <summary>Details</summary>
Motivation: Existing methods for bokeh editing assume an all-in-focus input or recover an all-in-focus image before rendering new bokeh, which can discard useful blur cues and propagate reconstruction artifacts.

Method: AnyBokeh estimates the source blur state with a signed circle-of-confusion map and a disparity map, and transfers the source optical characteristics to the desired focus and aperture setting using a generative editor.

Result: AnyBokeh achieves faithful and controllable editing across any-to-any bokeh editing, all-in-focus-to-bokeh rendering, and defocus deblurring.

Conclusion: AnyBokeh provides a practical solution for post-capture bokeh editing from a single image, handling images captured under arbitrary focus and aperture settings without requiring all-in-focus reconstruction or test-time bokeh-level calibration.
...
TL;DR: The paper proposes a unified framework that leverages planar surfaces to improve Structure from Motion (SfM) systems, particularly in scenarios where traditional methods struggle, such as planar scenes.


<details>
  <summary>Details</summary>
Motivation: Traditional SfM systems struggle with planar scenes, where epipolar geometry-based methods become degenerate. The authors aim to turn this limitation into a source of geometric constraints.

Method: The authors propose a graph-based approach that constructs a pose-graph from homography estimates and employs spectral embedding to identify and filter unreliable edges. They also introduce a method to map homography-based pose estimates onto the real line based on their geometric and visual consistency.

Result: The proposed method demonstrates superior performance on basketball court imagery and matches or exceeds state-of-the-art results on unconstrained outdoor scenes from the IMC Phototourism benchmark.

Conclusion: The paper concludes that the proposed framework can robustly recover camera poses in both highly planar scenes and general 3D environments, outperforming traditional methods in challenging scenarios.
...
TL;DR: The authors propose ERA, a framework for efficient multimodal large language models (MLLMs) that reduces visual token sequences while preserving performance.


<details>
  <summary>Details</summary>
Motivation: The high inference costs of MLLMs due to long visual token sequences, and the limitations of existing training-free visual token reduction methods that distort attention distributions.

Method: The ERA framework consists of three components: Dual-view Entropy Pruning (DEP), Bias-aware Token Recycling (BTR), and Logit-preserving Attention Rectification (LAR).

Result: The proposed ERA framework enables robust performance across single-image, multi-image, and video settings on a wide range of MLLMs, while delivering practical acceleration.

Conclusion: The authors establish logit-preserving visual token pruning as a principled framework for efficient MLLMs, providing a unified theoretical foundation, algorithmic design, and practical deployment.
...
TL;DR: The paper proposes CoLT, a novel framework for multi-modal large language models to reason through a chain of latent thought representations, reducing inference time and outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: The current text-based Chain-of-thought (CoT) reasoning paradigm is slow and limited by natural language expressiveness, motivating the need for a more efficient and effective reasoning approach.

Method: CoLT uses a lightweight external decoder to regulate the latent reasoning process, providing step-level supervision in forward and backward modes, and incorporates internal supervision for coherent latent transitions.

Result: CoLT outperforms existing methods on eight benchmarks, reduces inference time by 10.1x and text decoding time by 22.6x, and can perform thinking with as few as 3 steps.

Conclusion: The proposed CoLT framework offers a more efficient and effective approach to multi-modal reasoning, achieving state-of-the-art results while reducing inference time and computational requirements.
...
TL;DR: The paper introduces Automated Background Swapping (AutoBackSwap) to reduce the reliance of image classifiers on spurious backgrounds, which can lead to catastrophic failures. AutoBackSwap uses a secondary network to disentangle the foreground and background, synthesize complete backgrounds, and augment the training data.


<details>
  <summary>Details</summary>
Motivation: Deep Neural Networks can fail catastrophically if they rely on spurious correlations, such as features in the background of an image that are not causally linked to the class label.

Method: AutoBackSwap uses a secondary network to disentangle the foreground and background, followed by infilling to synthesize complete backgrounds, and finally combines different foregrounds and inpainted backgrounds to augment the training data.

Result: AutoBackSwap consistently outperforms prior methods across a range of image classification tasks with spurious backgrounds, and proves effective even if there is not a single sample in the training data breaking the spurious correlation.

Conclusion: The proposed method, AutoBackSwap, effectively reduces the reliance of classifiers on spurious backgrounds, leading to improved performance on image classification tasks.
...
TL;DR: The paper introduces a method called 'Bridge' for Cross-Space Distillation, enabling knowledge transfer from high-capacity teacher models to compact student models with different latent spaces.


<details>
  <summary>Details</summary>
Motivation: The limitation of current one-step diffusion models that require teacher and student models to share the same latent space, hindering knowledge transfer to compact, deployment-friendly models.

Method: The Bridge, a lightweight latent interface that maps student latents into the teacher space without modifying the student backbone, combining a frozen student VAE decoder and a compact learnable projector.

Result: The Bridge enables substantial gains for compact one-step students, improving SD 1.5 from 5.4 to 9.4 HPSv3 while preserving one-step inference, low latency, and broad ecosystem compatibility.

Conclusion: The paper demonstrates that heterogeneous large teachers can be distilled into efficient, deployable backbones through a lightweight latent-space interface, enabling knowledge transfer across different latent spaces.
...
TL;DR: A zero-shot framework for generating 360 panoramic images and videos by injecting spherical priors into pre-trained diffusion transformers.


<details>
  <summary>Details</summary>
Motivation: Existing methods for generating 360 panoramic images and videos are limited by costly fine-tuning or multi-step optimization, which restricts generalization and incurs high inference latency.

Method: The proposed approach introduces Spherical RoPE, which replaces standard rotary position embeddings with 3D Cartesian coordinates and harmonic quantization to enforce periodicity, and Semantic Distortion classifier-free guidance (CFG) to steer geometry.

Result: The approach achieves competitive performance against baselines across diverse backbones and 360 generation modalities, including text-to-panorama using Flux.1, Flux.2, and LTX-Video backbones, without requiring retraining.

Conclusion: The proposed framework enables zero-shot, training-free, and optimization-free generation of 360 panoramic images and videos, generalizing across diverse backbones and modalities.
...
TL;DR: A novel approach called PointSplat is proposed for 3D human representation, which directly infers Gaussian primitives from an input point set, achieving high compactness and fidelity.


<details>
  <summary>Details</summary>
Motivation: The need for compact and high-fidelity 3D human representations in immersive live streaming systems, where computational power and transmission bandwidth are limited.

Method: PointSplat, a human-centric approach that estimates a coarse geometric proxy, performs ray casting, and uses a Point-Image Transformer to fuse appearance and geometry features.

Result: PointSplat achieves higher efficiency and quality, with strong robustness to variations in view count and image resolution across multiple datasets.

Conclusion: The proposed PointSplat approach enables the production of highly compact 3D human representations with high fidelity, making it suitable for immersive live streaming systems.
...
TL;DR: The paper presents GEAR, a method for jointly training a vector-quantized tokenizer and an autoregressive generator end-to-end, guided by representation alignment, resulting in improved feature learning and generation.


<details>
  <summary>Details</summary>
Motivation: The current decoupled approach to training visual generative models leads to inefficiencies, as the tokenizer is unaware of what the generator finds easy to model.

Method: GEAR uses a dual read-out of the codebook assignment, with a hard branch for next-token prediction and a differentiable soft branch for representation-alignment loss, allowing gradients to flow back to the tokenizer.

Result: GEAR achieves up to 10x speedup in ImageNet gFID convergence, learns better patch-level and spatially-coherent features, and generalizes across quantizers and to text-to-image generation.

Conclusion: GEAR enables more efficient and effective training of visual generative models by aligning the tokenizer and generator through representation alignment.
...
TL;DR: The paper proposes FaceMoE, a Mixture of Experts transformer architecture for low-resolution face recognition, which achieves state-of-the-art results across eleven datasets.


<details>
  <summary>Details</summary>
Motivation: Low-resolution face recognition is challenging due to poor feature extraction, domain gap between high-resolution gallery images and low-resolution probe images, and catastrophic forgetting.

Method: FaceMoE uses multiple specialized feed-forward network experts and a top-k router to dynamically assign tokens to appropriate experts, enabling resolution-aware feature extraction and sparse expert activation.

Result: FaceMoE significantly outperforms state-of-the-art methods across eleven datasets, spanning high-resolution, mixed-quality, and low-resolution benchmarks.

Conclusion: FaceMoE is an effective adaptation of Mixture of Experts transformer architecture for low-resolution face recognition, addressing challenges of poor feature extraction, domain gap, and catastrophic forgetting.
...
### [145] [A Single Rewrite Suffices: Empirical Lessons from Production Skill Description Optimization](https://arxiv.org/abs/2606.30775)
*Yangqiaoyu Zhou,Mohammad Alqudah,Kwei-Herng Lai,Aaron Halfaker,Yingqi Xiong,Yaar Harari*

Main category: cs.CL

TL;DR: An automated pipeline optimizes natural language skill descriptions for AI agents, reducing engineering effort by 32 times while maintaining routing accuracy, and a diagnostic is proposed to identify cases requiring architectural intervention.


<details>
  <summary>Details</summary>
Motivation: The manual tuning of skill descriptions for AI agents becomes a significant engineering bottleneck as the number of skills scales, and skill collisions occur when two skills share overlapping descriptions.

Method: An automated description optimization pipeline is deployed, utilizing a single LLM rewrite with available false-positive and false-negative cases, and various design choices are tested.

Result: The pipeline produces descriptions with 79.2% F1, matching manually tuned descriptions at 79.4% F1, and reduces per-skill engineering effort from 120 minutes to 3.8 minutes.

Conclusion: The automated pipeline effectively optimizes skill descriptions, reducing engineering effort while maintaining routing accuracy, and a diagnostic is proposed to identify cases requiring architectural intervention.
...
### [146] [Indi-RomCoM: Code-Mixed Benchmark for Evaluating LLMs on Romanized Indic-English Instructions](https://arxiv.org/abs/2606.30790)
*Avisha Das,Mihir Parmar,Mohana Ramnath,Pulkit Verma*

Main category: cs.CL

TL;DR: The paper introduces the Indi-RomCoM benchmark to evaluate Large Language Models (LLMs) on Romanized Code Mixing (RCM) content, finding that LLMs underperform on RCM instructions, especially as code-mixing density increases.


<details>
  <summary>Details</summary>
Motivation: The emergence of Romanized Code Mixing (RCM) as a dominant form of communication in multilingual communities and the need to evaluate LLMs on RCM-based content.

Method: Introducing the Indi-RomCoM benchmark, which spans seven instruction-following tasks, four widely spoken Indic languages, and three controlled code-mixing intensity levels, and evaluating a suite of LLMs under zero- and few-shot settings.

Result: LLMs consistently underperform on RCM instructions, with performance degrading as code-mixing density increases, and reasoning tasks suffer less degradation than detection tasks.

Conclusion: The Indi-RomCoM benchmark can help the community develop more inclusive multilingual systems.
...
### [147] [Using AI Agents to Automate Black-Box Audits of Personalization Algorithms at Scale](https://arxiv.org/abs/2606.30801)
*Alessandro Morosini,Sarah H. Cen,Andrew Ilyas,Hedi Driss,Aleksander Mądry,Chara Podimata*

Main category: cs.CL

TL;DR: The paper introduces a framework for auditing personalization algorithms using generative AI agents to simulate user behavior, allowing for controlled and realistic testing of how platforms respond to user attributes.


<details>
  <summary>Details</summary>
Motivation: Existing auditing methods face a tradeoff between realistic user behavior and control, and struggle to decouple user attributes from user behavior, making it difficult to causally understand personalization.

Method: The authors use generative AI agents as behavioral engines for synthetic accounts, instantiating each agent with a fixed persona grounded in demographic and political survey data, and interacting with a platform's content by reasoning about it and choosing actions.

Result: The study finds that X's algorithmic feed amplifies toxic, polarizing, political, and right-leaning content relative to the chronological feed, with amplification varying sharply by user ideology, and that demographic signals affect content delivery in persona-dependent ways.

Conclusion: The paper establishes GenAI-based agents as a new tool for algorithmic auditing, enabling counterfactual auditing of how platforms respond to user attributes, and providing insights into the effects of personalization algorithms on content delivery.
...
### [148] [When Calibration Rankings Reverse: Accuracy-Controlled Evaluation for Fair Comparison of LLMs](https://arxiv.org/abs/2606.30814)
*Zhichao Yang,Caiqi Zhang,Ruihan Yang,Chengzu Li,Nigel Collier,Deqing Yang*

Main category: cs.CL

TL;DR: The paper proposes a new evaluation framework, ACE, for fairly comparing the calibration of large language models, finding that raw global metrics are not robust and that accuracy control is necessary for reliable comparisons.


<details>
  <summary>Details</summary>
Motivation: Existing studies comparing model calibration are confounded by differences in model accuracy, and a fairer evaluation framework is needed.

Method: The authors propose ACE, an accuracy-controlled evaluation framework with three views: Instance-Aligned, Distribution-Aligned, and Candidate-Aligned calibration.

Result: The study finds that many reported calibration advantages weaken after accuracy control, and ranking reversal is frequent, indicating that raw global metrics are not robust for cross-model comparison.

Conclusion: The paper concludes that fair calibration comparison requires accuracy-aware evaluation, and that ACE provides a more reliable way to compare model calibration.
...
### [149] [When transformers learn "impossible" languages, what do they learn?](https://arxiv.org/abs/2606.30815)
*Ram Janarthan,Coleman Haley,Sharon Goldwater*

Main category: cs.CL

TL;DR: The paper investigates why transformer language models favor human languages over unnatural ones, suggesting that the models' generative capabilities, rather than grammatical sensitivity, are a key factor.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to understand the bias of transformer language models towards human languages over unnatural languages, and to evaluate linking hypotheses that could explain this bias.

Method: The authors use GPT-2 style models trained on perturbed 'impossible' variants of English and evaluate their sensitivity to grammaticality using BLiMP minimal pairs, as well as their generative production capabilities.

Result: The results show that the models exhibit gradual degradation in grammatical sensitivity, but pronounced failures in generation, producing fewer high-quality sentences at longer lengths.

Conclusion: The study concludes that generative deficiency and transmission failures are a plausible linking hypothesis between language model behavior and the non-attestation of impossible languages.
...
### [150] [Test-Time Verification for Text-to-SQL via Outcome Reward Models](https://arxiv.org/abs/2606.30851)
...
TL;DR: This paper introduces GradeSQL, a framework for training Outcome Reward Models (ORMs) to improve the reliability of large language models in Text-to-SQL tasks.


<details>
  <summary>Details</summary>
Motivation: The reliability of large language models in structured reasoning tasks, such as Text-to-SQL, is a central challenge. Current test-time inference strategies have limitations, and the authors aim to explore ORMs as learned semantic scoring functions for test-time verification.

Method: The authors introduce GradeSQL, a scalable framework for training task-specific ORMs via automated candidate generation and execution-based labeling. They integrate ORMs into a verification-driven Best-of-N pipeline and evaluate their approach on the BIRD and Spider benchmarks.

Result: The results show that ORM-based selection consistently outperforms execution-based Best-of-N and Majority Voting, with gains of up to +4.33% on BIRD and +2.10% on Spider. ORMs scale effectively with larger candidate sets and yield stronger improvements on complex queries.

Conclusion: The authors conclude that ORM-based verification provides a simple, effective, and scalable alternative to heuristic test-time selection strategies for Text-to-SQL.
...
### [151] [Multilingual Polarization Detection Using Transformer-Based Models with Class Weighting and Threshold Tuning](https://arxiv.org/abs/2606.30857)
*Aaron Bundi Anampiu*

Main category: cs.CL

TL;DR: The paper presents a submission to SemEval-2026 Task 9, focusing on detecting online polarization in English and Swahili. It uses transformer-based models with techniques to handle label imbalance, achieving competitive performance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of detecting multilingual, multicultural, and multievent online polarization, specifically in English and Swahili, and to improve the performance on imbalanced multi-label classification tasks.

Method: The method involves using transformer-based models (RoBERTa-base for English, AfroXLMR-base for Swahili) with class-weighted loss functions and per-label threshold tuning for multi-label classification.

Result: The results show F1 macro scores of 0.7901 (English) and 0.7910 (Swahili) for Subtask 1, 0.4615 (English) and 0.4808 (Swahili) for Subtask 2, and 0.4791 (English) and 0.5830 (Swahili) for Subtask 3, indicating competitive performance.

Conclusion: The conclusion is that the proposed methods are effective for handling imbalanced multi-label polarization detection, although the error analysis reveals struggles with dehumanization detection and lack of empathy.
...
TL;DR: The paper introduces a framework for improving therapeutic quality of large language models (LLMs) in mental health support through a decision-refinement process driven by multi-dimensional human-aligned evaluation.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the therapeutic quality of LLMs in mental health support by making evaluation an actionable control signal rather than a passive metric.

Method: The method involves a two-stage framework: Stage I introduces TheraJudge, an open-source therapeutic evaluator trained on human-annotated data; Stage II introduces TheraAgent, which operationalizes TheraJudge's evaluations through a coordinated refinement process.

Result: The results show that TheraJudge achieves strong agreement with clinician ratings (ICC = 0.87-0.95) and TheraAgent yields a +0.43 improvement in human-rated therapeutic quality, with 96% clinician inter-rater reliability.

Conclusion: The conclusion is that effective alignment of mental-health LLMs stems from acting on human-aligned evaluation, rather than relying solely on stronger generation.
...
### [153] [Beyond Clean Text: Evaluating Encoder and Decoder Robustness for Bangla Event Detection in Noisy Text](https://arxiv.org/abs/2606.30914)
*Tanvir Ahmed Sijan,S. M Golam Rifat,Nayeemul Islam,Md. Musfique Anwar*

Main category: cs.CL

TL;DR: The paper evaluates the robustness of event detection systems in Bangla, a low-resource language, under real-world noise. It compares encoder-only and decoder-only models, finding that decoder-only LLMs are more robust to noise.


<details>
  <summary>Details</summary>
Motivation: The motivation is to investigate the robustness of event detection systems to real-world noise, particularly in low-resource languages like Bangla.

Method: The method involves evaluating fine-tuned encoder-only models (BanglaBERT and XLM-R) and instruction-tuned decoder-only large language models (Llama 3 and Gemma 3) on a benchmark dataset comprising clean and noisy text.

Result: The results show that decoder-only LLMs are more robust to noise, especially when event triggers are corrupted. Model scaling improves the robustness of decoder-only LLMs, and combined training on clean and noisy data benefits encoder architectures.

Conclusion: The conclusion is that there is a clear architectural trade-off between encoder and decoder-only models, with decoder-only LLMs being more robust to noise. The study also highlights the importance of model scaling and combined training on clean and noisy data.
...
### [154] [Bridging Scientific Heritage: An Arabic--Russian Parallel Corpus and LLM Benchmark for Sustainable Knowledge Transfer](https://arxiv.org/abs/2606.30943)
*M. K. Arabov*

Main category: cs.CL

TL;DR: The paper presents a benchmark for Arabic-Russian scientific translation, fine-tuning multilingual language models to improve translation performance and facilitate knowledge exchange between Arabic-speaking and Russian-speaking researchers.


<details>
  <summary>Details</summary>
Motivation: Language barriers impede the exchange of research results between Arabic-speaking and Russian-speaking communities, affecting international collaboration and sustainability-related research.

Method: The authors create a hybrid parallel corpus of 27,000 sentence pairs and fine-tune three multilingual language models (mT5-base, NLLB-200-distilled-1.3B, and Qwen2.5-7B-Instruct) using LoRA with varying ranks.

Result: The Qwen2.5-7B model with QLoRA (rank 8) achieves BLEU 23.15, chrF 43.89, BERTScore 0.906, and COMET 0.758, outperforming the zero-shot baseline.

Conclusion: The work enables knowledge exchange between Arabic-speaking and Russian-speaking researchers, contributing to sustainable partnerships and innovation infrastructure, aligning with UN SDGs 17 and 9.
...
### [155] [Linguistic Distancing on Social Media: Indicators of Emotion Regulation Across Age Groups](https://arxiv.org/abs/2606.30957)
*Daniela Teodorescu,Saif M. Mohammad,Alona Fyshe*

Main category: cs.CL

TL;DR: The study explores how linguistic distancing, a marker of emotion regulation, changes across age groups using social media text analysis.


<details>
  <summary>Details</summary>
Motivation: To understand how emotion regulation varies with age and provide benchmarks on social media data, as linguistic distancing is a marker of well-being.

Method: Analyzing large amounts of social media text to investigate linguistic distancing across age groups.

Result: The study finds that linguistic distancing occurs in proportionally more instances with age, aligning with past work indicating improved well-being with older age.

Conclusion: The study provides a foundation for further exploring emotion regulation through linguistic distancing in text data, which can inform effective health interventions.
...
### [156] [From Propositional to Perceptual Asymmetry: Extending Frictive Policy Optimization to Asymmetric Partial Information Dialogue](https://arxiv.org/abs/2606.30973)
*Yifan Zhu,Kyeongmin Rim,James Pustejovsky*

Main category: cs.CL

TL;DR: The paper extends Frictive Policy Optimization (FPO) to handle perceptual asymmetry in collaborative dialogue, where participants have different information and referring expressions have different meanings.


<details>
  <summary>Details</summary>
Motivation: The original FPO assumes shared perceptual contexts, but the authors want to handle cases where participants have asymmetric partial information.

Method: The authors extend FPO to perceptual asymmetry and evaluate it through cross-corpora analysis and LLM probing on referentially asymmetric dialogue tasks.

Result: The results show that FPO's friction functional is empirically valid only when evaluated from within each participant's information horizon, and having the 'right perspective' matters more than having all perspectives.

Conclusion: The authors propose two annotation refinements: subtype decomposition of pending grounding states and accommodation-aware alignment classification.
...
### [157] [Measuring Judgment Quality in Natural-Language Explanations: Evidence from Forecasting Tournaments](https://arxiv.org/abs/2606.30987)
*Christopher W. Karvetski,Sheldon S. Huang,Simas Kučinskas,Nadja Flechner,Jingyu Hu,Philip Tetlock,Ezra Karger*

Main category: cs.CL

TL;DR: Researchers developed Explanation Quality Markers (EQMs) to assess the quality of written explanations accompanying expert judgments, finding that EQMs reliably predict forecasting accuracy and outperform traditional text analysis methods.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of measuring explanation quality at scale, particularly in forecasting tournaments where expert judgments are paired with written explanations.

Method: The study used a set of sixty theory-guided reasoning patterns scored by large language models (LLMs) to evaluate over 55,000 forecast-rationale pairs from a multiyear forecasting tournament.

Result: EQMs predict accuracy at both forecast and forecaster levels, outperforming pre-LLM text analysis methods, and are the strongest predictor at the forecast level and competitive at the forecaster level.

Conclusion: EQMs provide a scalable and interpretable method for extracting judgment-relevant information from written explanations, which can help decision-makers evaluate the quality of expert judgments.
...
### [158] [Wait, am I Being Fair? Characterizing Deductive Stereotyping and Mitigating It with Fair-GCG](https://arxiv.org/abs/2606.30989)
*Naihao Deng,Yilun Zhu,Joan Nwatu,Clayton Scott,Rada Mihalcea*

Main category: cs.CL

TL;DR: The paper identifies a failure mode in large language models called deductive stereotyping, proposes a framework to improve fairness in reasoning, and introduces a method to discover effective phrases for fairness-aware reasoning.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the persistent failures in fairness exhibited by recent large language models (LLMs), specifically the phenomenon of deductive stereotyping where models apply population-level statistical regularities to individual cases, resulting in socially biased inferences.

Method: The paper proposes a reasoning-time injection framework to steer models toward fairness-aware reasoning and introduces Fair-GCG, a method to systematically discover effective injection phrases.

Result: The injection phrases discovered by Fair-GCG improve performance across multiple fairness benchmarks, generalize from smaller to larger LLMs, improve reasoning-level fairness, reduce bias in open-ended generation, and transfer to real-world fairness-sensitive tasks.

Conclusion: The paper concludes that the proposed framework and method can effectively improve fairness in LLMs by mitigating deductive stereotyping and promoting fairness-aware reasoning.
...
### [159] [CORTEX: Token-Level Hallucination Detection in RAG via Comparative Internal Representations](https://arxiv.org/abs/2606.31033)
*Kazuaki Furumai,Shuichiro Haruta,Kazunori Matsumoto,Daisuke Kamisaka*

Main category: cs.CL

TL;DR: The paper proposes CORTEX, a method to detect hallucinations at the token level in Retrieval-Augmented Generation (RAG) outputs.


<details>
  <summary>Details</summary>
Motivation: Hallucinations in long-form RAG outputs often occur in localized spans, making fine-grained localization necessary.

Method: CORTEX identifies ungrounded content by comparing internal representations of a large language model with and without retrieved documents.

Result: Experiments on two RAG benchmarks and three LLMs show that CORTEX substantially improves token-level hallucination detection.

Conclusion: CORTEX effectively detects hallucinations at the token level in RAG outputs, with each component contributing to performance gains.
...
### [160] [Truth or Sophistry? LoFa: A Benchmark for LLM Robustness Against Logical Fallacies](https://arxiv.org/abs/2606.31039)
*Xudong Shen,Li Yuan,Ye Chen,Xin Wu,Yi Cai,Zhiyong Wu*

Main category: cs.CL

TL;DR: This paper introduces a benchmark to evaluate the robustness of Large Language Models (LLMs) against logical fallacies and proposes a metric to quantify their resistance to fallacious attacks.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the underexplored resilience of LLMs to manipulative linguistic patterns such as logical fallacies, and to evaluate their robustness against fallacious persuasion.

Method: The authors introduce LoFa, a comprehensive benchmark constructed through a multi-agent pipeline, and a multi-round debate framework to assess model resilience. They also propose LFR@k, a metric to quantify resistance to fallacious attacks.

Result: Experiments show that LLMs exhibit varying levels of robustness across different fallacy types, revealing distinct vulnerability profiles among models.

Conclusion: The paper concludes that LLMs have varying levels of robustness against logical fallacies, and highlights the need for further research to improve their resilience to manipulative linguistic patterns.
...
### [161] [Reference-Based Prosody and Rhythm Evaluation for Spoken Dialogue Systems](https://arxiv.org/abs/2606.31055)
*Ashish Hallur,Thomas Thebaud,Georgi Tinchev,Venkatesh Ravichandran,Laureano Moro-Velazquez*

Main category: cs.CL

TL;DR: The paper proposes a new evaluation method for speech-to-speech AI agents, focusing on conversational prosody and rhythm, using matched human reference regimes and percentile-based comparison.


<details>
  <summary>Details</summary>
Motivation: Current evaluation methods for speech-to-speech AI agents lack interpretable speech-native measures for conversational prosody and rhythm.

Method: The authors construct matched reference regimes using 4000+ hours of dyadic English conversation and define a percentile-based evaluation protocol to compare S2S output waveforms to human references.

Result: The proposed method shows improved performance over pooled references, with flag rates closer to the nominal 10% and interpretable deviation direction.

Conclusion: The proposed method serves as a behavioral plausibility check that complements perceptual and user-centered evaluation for speech-to-speech AI agents.
...
### [162] [Building a Multimodal Dataset of Academic Paper for Keyword Extraction](https://arxiv.org/abs/2606.31069)
*Jingyu Zhang,Xinyi Yan,Yi Xiang,Yingyi Zhang,Chengzhi Zhang*

Main category: cs.CL

TL;DR: This study explores multimodal keyword extraction using text, images, and audio from academic papers, finding that combining these modalities improves performance.


<details>
  <summary>Details</summary>
Motivation: Current keyword extraction tasks primarily rely on textual data, neglecting visual and audio features, which limits model performance and accuracy due to a lack of multimodal datasets.

Method: The study constructs a multimodal dataset of 1000 academic paper samples, including text, images, audio, and keywords. It conducts experiments using unsupervised and supervised methods, fusing textual data from papers, images, and audio.

Result: The experimental results show that text from different modalities has distinct characteristics. Concatenating paper text, image text, and audio text effectively enhances keyword extraction performance in academic papers.

Conclusion: The study concludes that incorporating multimodal information (text, images, and audio) can improve the performance of keyword extraction tasks, particularly for academic papers.
...
### [163] [Triospect: A Three-Dimensional Framework for Robust Statistical AI-Generated Text Detection Against Diverse Attacks](https://arxiv.org/abs/2606.31074)
*Guangsheng Bao,Lihua Rong,Yanbin Zhao,Xiao Yu,Qiji Zhou,Yue Zhang*

Main category: cs.CL

TL;DR: A novel framework called Triospect is proposed to improve the detection of AI-generated text by considering additional perspectives of content and expression.


<details>
  <summary>Details</summary>
Motivation: Existing AI-generated text detectors are vulnerable to attacks that manipulate textual characteristics, highlighting the need for more robust detection methods.

Method: The Triospect Detection Framework uses additional perspectives of content (core ideas) and expression (stylistic elements) within a given text to improve detection.

Result: Triospect demonstrates significant improvements over a strong baseline, with increases of 22.3% (AUROC) and 13% (TPR01) on the Humanize-16K after-attack subset, and 9.1% (AUROC) and 22% (TPR01) on the adversarial RAID.

Conclusion: The Triospect framework marks a pioneering effort in statistical methods to enhance detection reliability against attacks, and its data and code are made publicly available.
...
### [164] [When Reranking Hurts: Uncertainty-Based Gating for Few-Shot Reranking](https://arxiv.org/abs/2606.31087)
*Orian Dabod,Amir Cohen,Gabriel Stanovsky*

Main category: cs.CL

TL;DR: The authors propose a Training-Free Gated Reranking approach for few-shot selection, which selectively reranks examples based on model uncertainty, reducing computational costs and improving performance.


<details>
  <summary>Details</summary>
Motivation: The authors challenge the assumption that reranking retrieved examples always improves performance in few-shot selection, as the expensive reranking step can sometimes degrade performance.

Method: The authors propose Training-Free Gated Reranking, which decides whether to rerank few-shot examples based on the model's uncertainty.

Result: The approach reduces computational costs by 15%-80% and improves average performance by up to 2% across 8 LLMs, 7 NLU datasets, and 9 MT domain-language combinations.

Conclusion: The findings indicate that higher computational cost does not guarantee better performance, and that reranking is most beneficial when targeted at high-uncertainty instances.
...
### [165] [What Counts as an Error? Dual-Reference Benchmarking for Atypical ASR](https://arxiv.org/abs/2606.31112)
*Hawau Olamide Toyin,Srinivasan Umesh,Hanan Aldarmaki*

Main category: cs.CL

TL;DR: The paper analyzes how different transcription references impact ASR model performance on atypical speech, specifically stuttered speech.


<details>
  <summary>Details</summary>
Motivation: The existence of two valid transcription references (verbatim and intended) for atypical speech can conflate ASR evaluations, and most evaluations ignore verbatim faithfulness.

Method: Benchmarking 11 ASR models from different families (encoder-decoder, CTC, transducer) using both verbatim and intended references on atypical stuttered speech.

Result: The study finds disparity in model performance and rankings using the two transcript styles, highlighting the importance of selecting a suitable transcription reference for valid model selection.

Conclusion: The choice of transcription reference significantly impacts ASR model selection for atypical speech, and a suitable reference should be chosen depending on the use-case.
...
### [166] [SeKV: Resolution-Adaptive KV Cache with Hierarchical Semantic Memory for Long-Context LLM Inference](https://arxiv.org/abs/2606.31145)
*Amirhossein Abaskohi,Giuseppe Carenini,Peter West,Yuhang He*

Main category: cs.CL

TL;DR: SeKV, a semantic KV cache, enables efficient long-context language modeling by compressing and adaptively reconstructing token-level details, reducing GPU memory usage by 53.3% while improving performance by 5.9%.


<details>
  <summary>Details</summary>
Motivation: The KV cache memory bottleneck in large language models operating over long contexts, which existing compression methods fail to efficiently address.

Method: SeKV uses entropy-guided semantic spans, storing them across a GPU-CPU memory hierarchy, with a lightweight summary vector on GPU and a low-rank SVD basis on CPU for on-demand token-level reconstruction.

Result: SeKV improves performance by 5.9% over the strongest semantic compression baseline and reduces GPU memory usage by 53.3% at 128K context, with fewer than 0.05% trainable parameters.

Conclusion: SeKV offers an efficient and effective solution for long-context language modeling, enabling adaptive token-level reconstruction while keeping the base LLM frozen.
...
### [167] [TAG-DLM: Diffusion Language Models for Text-Attributed Graph Learning](https://arxiv.org/abs/2606.31166)
*Lingjie Chen,Yuanchen Bei,Haobo Xu,Yanjun Zhao,Yuzhong Chen,Hanghang Tong*

Main category: cs.CL

TL;DR: A unified method for text-attributed graphs (TAGs) that integrates textual reasoning and graph message passing within a masked diffusion language model, outperforming existing approaches.


<details>
  <summary>Details</summary>
Motivation: Existing approaches handle text and graph topology separately, limiting their ability to jointly reason over both modalities.

Method: A masked diffusion language model that linearises a sampled local neighbourhood into a token sequence and injects graph structure through a topology attention mask.

Result: The method outperforms graph neural networks, graph transformers, and LLM-based baselines on three TAG benchmarks across two tasks, improving over the strongest baseline by up to 3.9 points.

Conclusion: The proposed method provides a unified and effective approach for TAGs, adapting to different tasks with no target-specific fine-tuning.
...
### [168] [Gated Multi-Graph Fusion via Graph Attention Networks for Alzheimer's Disease Detection](https://arxiv.org/abs/2606.31186)
*Jinyu Li,Xiao Wei,Bin Wen,Kai Li,Yuqin Lin,Xiaobao Wang,Longbiao Wang,Jianwu Dang*

Main category: cs.CL

TL;DR: A novel network for Alzheimer's Disease diagnosis using speech patterns.


<details>
  <summary>Details</summary>
Motivation: Current systems overlook non-linear structural disruptions and clinical heterogeneity in pathological language, which are crucial for accurate Alzheimer's Disease diagnosis.

Method: A Multi-View Gated Graph Attention Network that constructs semantic, dependency, and co-occurrence graphs from transcribed speech.

Result: The model achieves 90.00% accuracy on the ADReSSo dataset.

Conclusion: The proposed model effectively captures complex speech patterns for Alzheimer's Disease diagnosis, with the PMI-based graph and heterogeneity-aware gating being essential components.
...
### [169] [Can LLMs Imagine Moral Alternatives Beyond Binary Dilemmas?](https://arxiv.org/abs/2606.31213)
*Jongchan Choi,Nari Yang,Sung Soo Park,Jaemin Cho,Han Seoyoung,Haerin Shin,Jun-Hyung Park*

Main category: cs.CL

TL;DR: The paper introduces MoralAltDataset, a dataset of moral dilemmas with compromise and reframed alternatives, and finds that both humans and LLMs prefer compromise alternatives, with LLM-generated alternatives often being preferred over human-authored ones.


<details>
  <summary>Details</summary>
Motivation: Existing research on LLMs with moral dilemmas overlooks the human ability to imagine alternatives beyond given options, which is a central aspect of human moral cognition.

Method: The authors introduce a new dataset, MoralAltDataset, and evaluate the quality of LLM-generated alternatives against human-authored ones using pairwise preference and expert-based criteria.

Result: The results show that compromise alternatives are often preferred over original options, and LLM-generated alternatives are often preferred and better satisfy fine-grained structural and ethical criteria.

Conclusion: The study highlights the importance of considering alternative solutions in moral dilemmas and demonstrates the potential of LLMs to generate high-quality alternatives that can reshape moral choice.
...
### [170] [Probing Stylistic Appropriation using Large Language Models: An Evaluation Framework for Copyright Infringement under EU Law](https://arxiv.org/abs/2606.31250)
*Noah Scharrenberg,Chang Sun*

Main category: cs.CL

TL;DR: The authors introduce PSALM, a framework to evaluate copyright infringement by large language models (LLMs) based on EU doctrine, and find that current safeguards are insufficient to mitigate broader copyright risks.


<details>
  <summary>Details</summary>
Motivation: The existing technical safeguards for LLMs focus on verbatim memorization, but EU copyright doctrine considers a broader standard of substantial similarity, including stylistic choices and creative elaboration.

Method: The authors propose PSALM, an LLM-as-a-judge framework that uses ten evaluators to assess computational overlap, stylistic dimensions, content dimensions, and statutory exceptions.

Result: The study finds that instruction-tuned models exhibit non-trivial stylistic similarity, fine-tuning induces systematic stylistic appropriation, and Negative Preference Optimisation unlearning reduces but does not eliminate detectable residual stylistic patterns.

Conclusion: The authors conclude that safeguards targeting literal copying alone are insufficient, and PSALM provides infrastructure for auditable, legally informed compliance evaluation, highlighting fundamental tensions between generative AI and EU intellectual property law.
...
### [171] [When the Database Fails: Prompting LLM Dialogue Agents for Safe Recovery in Task-Oriented Dialogue](https://arxiv.org/abs/2606.31307)
*Mohammad Alijanpour Shalmani,Alale Rezvani Boroujeni,Jiann Shiun Yuan*

Main category: cs.CL

TL;DR: This paper studies a lightweight prompting-based approach to improve the robustness of task-oriented dialogue systems when database calls fail or return incorrect information. It compares three response strategies across six model families and four database conditions, finding that a guided recovery prompt can reduce hallucination by 50% without retraining.


<details>
  <summary>Details</summary>
Motivation: Large language models used in task-oriented dialogue often produce fluent but unsafe responses when database calls fail or return incorrect information. The authors aim to improve robustness without retraining or additional model calls.

Method: The authors compare three response strategies, including a guided recovery prompt conditioned on structured database status, across six open-weight model families and four database conditions. They use fault-injected benchmarks built on two structurally different datasets, MultiWOZ 2.2 and SGD.

Result: The guided-retry strategy reduces hallucination by 50% on MultiWOZ (30.5 to 15.3%) and by 42% on SGD (20.9 to 12.2%) without retraining. However, residual hallucination remains substantial (6-37% across models), with wrong-domain failures being the hardest case.

Conclusion: The authors conclude that a lightweight prompting-based recovery approach can improve the robustness of task-oriented dialogue systems without retraining or additional model calls. However, further work is needed to address residual hallucination.
...
### [172] [LOPA: Enhancing Spoken Language Assessment via Latent Ordinal Prototype Alignment](https://arxiv.org/abs/2606.31310)
*Hong-Yun Lin,Fu-An Chao,Bi-Cheng Yan,Berlin Chen*

Main category: cs.CL

TL;DR: The paper introduces LOPA and SALR for Spoken Language Assessment (SLA), achieving comparable performance to large models without fine-tuning.


<details>
  <summary>Details</summary>
Motivation: The increasing model scale and multimodal inputs have led to the emergence of Multimodal Large Language Models (MLLMs) for SLA, but they often overlook the intrinsic ordinal structure of language acquisition.

Method: The paper proposes Latent Ordinal Prototype Alignment (LOPA) and Semantic-Anchored Layer Routing (SALR) to enforce an ordinal geometric prior and adaptively harvest multi-depth representations.

Result: The framework achieves an RMSE of 0.361, rivaling billion-parameter systems without LLM-based fine-tuning.

Conclusion: The proposed framework offers an efficient and ordinal-aware modeling alternative to current scaling-centric models for SLA, with interpretable and criterion-aligned preferences.
...
### [173] [BlockPilot: Instance-Adaptive Policy Learning for Diffusion-based Speculative Decoding](https://arxiv.org/abs/2606.31315)
*Hao Zhang,Yiming Hu,Yong Wang,Mingqiao Mo,Xin Xiao,Xiangxiang Chu*

Main category: cs.CL

TL;DR: The paper proposes BlockPilot, a sample-adaptive policy for speculative decoding that predicts the optimal block size for each input sample, achieving improved efficiency and speedup.


<details>
  <summary>Details</summary>
Motivation: Existing speculative decoding methods assume a fixed inference block size and uniform optimal decoding strategy across all inputs, which is suboptimal.

Method: The authors propose a lightweight policy learning approach to predict the optimal block size based on the prefilling representation, allowing for seamless integration and minimal overhead.

Result: The proposed method achieves an acceptance length of 5.92 and a 4.20× speedup on Qwen3-4B under temperature T=1, demonstrating consistent improvements in efficiency.

Conclusion: The paper presents BlockPilot as a effective solution for speculative decoding, enabling adaptive block size selection and improved performance.
...
### [174] [Linguistic Bias Mitigation for Spoofing Detection via Gradient Reversal and A Variational Information Bottleneck](https://arxiv.org/abs/2606.31411)
*Anh-Tuan Dao,Driss Matrouf,Mickael Rouvier,Nicholas Evans*

Main category: cs.CL

TL;DR: The paper proposes a linguistic-invariant spoofing detection framework to improve the robustness of voice biometrics against linguistic bias.


<details>
  <summary>Details</summary>
Motivation: The reliability of voice biometrics has been compromised due to advancements in generative speech technology, and current spoofing detectors have poor generalization to out-of-domain settings due to linguistic bias.

Method: The proposed framework utilizes teacher-student adversarial learning, where a linguistic-aware teacher model guides a student detector to minimize linguistic information via gradient reversal, and incorporates a Variational Information Bottleneck to prevent the removal of non-linguistic cues.

Result: The proposed method achieves up to a 36.2% relative reduction in the Equal Error Rate (EER) compared to the baseline across nine DF Arena datasets.

Conclusion: The proposed framework effectively improves the robustness of spoofing detectors to linguistic bias, enhancing the reliability of voice biometrics.
...
### [175] [Clinically Structured Rank-Gated LoRA for Cross-Benchmark Medical Question Answering](https://arxiv.org/abs/2606.31432)
*Yining Huang*

Main category: cs.CL

TL;DR: The paper proposes BiRG-LoRA, a method for medical question answering that uses a single adapter with input-conditioned rank dimension to improve performance across heterogeneous knowledge domains and reasoning operations.


<details>
  <summary>Details</summary>
Motivation: Medical multiple-choice question answering requires adapting to different knowledge domains and reasoning operations, and existing methods may not efficiently handle these variations.

Method: BiRG-LoRA uses a single LoRA module per target layer with input-conditioned rank dimension, combining hidden semantic evidence with specialty priors, clinical-operation priors, and their interaction to select a sparse subset of rank atoms.

Result: BiRG-LoRA achieves the highest four-benchmark macro-average accuracy among trainable PEFT baselines and matched routing controls, improving over MoELoRA by 0.89 percentage points while using 28.1% fewer trainable parameters.

Conclusion: The results support the claim that clinically structured rank allocation improves cross-benchmark medical QA under a matched single-seed protocol, and BiRG-LoRA outperforms existing methods in terms of accuracy and efficiency.
...
### [176] [Revising RVL-CDIP: Quantifying Errors and Test-Train Overlap](https://arxiv.org/abs/2606.31446)
...
TL;DR: The paper addresses label errors and test-train overlap in the RVL-CDIP dataset, creating new variations and benchmarking document classification performance.


<details>
  <summary>Details</summary>
Motivation: The RVL-CDIP dataset contains label errors and test-train overlap, which can impact model performance metrics.

Method: The authors find and fix label errors, detect and address test-train overlap, and create new RVL-CDIP variations for benchmarking.

Result: Removing label errors improves classification accuracy, while removing duplicates decreases accuracy. Error-corrected data improves out-of-distribution generalization.

Conclusion: The study highlights the importance of data quality in benchmarking document classifiers and shows that error correction can significantly improve model performance.
...
### [177] [Team MKC at CLPsych 2026: Capturing and Characterizing Mental Health Changes through Social Media Timeline Dynamics](https://arxiv.org/abs/2606.31464)
*Kyomin Hwang,Hyeonjin Kim,Hyunho Lee,Nojun Kwak*

Main category: cs.CL

TL;DR: The paper proposes a Large Language Model (LLM) based pipeline for comprehensive mental health analysis using sequentially ordered user posts.


<details>
  <summary>Details</summary>
Motivation: The growing prevalence of mental health disorders and limited accessibility of professional care has created a demand for scalable computational approaches to assist in early detection and continuous monitoring of psychological well-being.

Method: The proposed pipeline uses LLMs to jointly enable post-level assessment and user-level temporal modeling for comprehensive mental health analysis.

Result: Not explicitly stated in the abstract.

Conclusion: The paper presents a unified framework for mental health analysis using LLMs, which can contribute to addressing the demand for scalable computational approaches in mental health care.
...
### [178] [Building an ASR Solution for Training and Assessing Children's Reading](https://arxiv.org/abs/2606.31508)
*Yacouba Diarra,Nouhoum Souleymane Coulibaly,Mamadou Dembele,Aymane Dembele,Michael Leventhal*

Main category: cs.CL

TL;DR: An open-source system for assessing children's reading in Bambara was developed, including a mobile app for data collection and an ASR model that outperformed existing architectures.


<details>
  <summary>Details</summary>
Motivation: To address the underdevelopment of automatic speech recognition for children's reading in African languages, specifically Bambara, for reproducible literacy assessment.

Method: An end-to-end process involving field data collection, benchmark construction, model adaptation, a reading application, and classroom validation was used. Fine-tuning experiments compared Soloni, a Bambara-adapted Fast-Conformer ASR framework, with QuartzNet, a compact convolutional ASR architecture.

Result: The best Soloni model reduced WER from 0.42 to 0.22 and CER from 0.15 to 0.08, outperforming QuartzNet. Repeated readings of the same texts provided architecture-dependent benefits. Disaggregated analysis identified children under 10 as the main source of residual errors.

Conclusion: The developed system, including the Soloni ASR model, shows potential for assessing children's reading in Bambara. Targeted collection from younger readers is motivated to further improve performance.
...
### [179] [FinPersona-Bench: A Benchmark for Longitudinal Psychometric Stability of Autonomous Financial Agents](https://arxiv.org/abs/2606.31522)
*Muhammad Usman Safder,Ayesha Gull,Rania Elbadry,Fan Zhang,Yankai Chen,Xueqing Peng,Xue,Liu,Preslav Nakov,Zhuohan Xie*

Main category: cs.CL

TL;DR: Large Language Models (LLMs) deployed as financial agents experience a decline in adherence to their initial behavioral mandates over time, leading to undesirable outcomes.


<details>
  <summary>Details</summary>
Motivation: The authors aim to investigate the phenomenon of Mandate Salience Decay (MSD) in LLMs used as financial agents and its implications for long-term deployment.

Method: The authors introduce FinPersona-Bench, a simulation benchmark, to evaluate 18 leading LLMs with different behavioral profiles in various market scenarios.

Result: The study finds that MSD compounds over time, is model-dependent, and that periodic mandate re-grounding can have varying effects on agent behavior depending on the agent profile and market regime.

Conclusion: The authors conclude that reliable long-term deployment of LLMs as financial agents requires selective, mandate-aware re-grounding based on agent profile and market regime.
...
### [180] [AutoTrainess: Teaching Language Models to Improve Language Models Autonomously](https://arxiv.org/abs/2606.31551)
*Zhaojian Yu,Penghao Yin,Shuzheng Gao,Shilin He,Kai Cai,Xiao-Ping Zhang*

Main category: cs.CL

TL;DR: AutoTrainess, a language model agent, automates post-training processes for language models, outperforming CLI-only baselines and improving model performance.


<details>
  <summary>Details</summary>
Motivation: The current process of training language models is highly human-intensive and autonomous post-training requires the agent to perform multiple complex tasks, which is not just a coding problem.

Method: AutoTrainess exposes operations as a repository of agent-computer interfaces, externalizing prior human experience as explicit workflows, rules, and execution constraints to guide the agent toward effective and reliable training behavior.

Result: AutoTrainess achieves 26.94 average score with GPT-5.4 (Codex) versus 23.21 for CLI-only, and improves DeepSeek-V4-Flash (OpenCode) from 12.13 to 19.58.

Conclusion: AutoTrainess is a effective solution for automating post-training processes for language models, improving model performance and generalizing across models and harnesses.
...
### [181] [Robust Text Watermarking for Large Language Models via Dual Semantic Embeddings](https://arxiv.org/abs/2606.31602)
*Jonas Schäfer,Cezary Pilaszewicz,Gerhard Wunder*

Main category: cs.CL

TL;DR: DEW, a semantic watermarking scheme for LLMs, enhances robustness against paraphrasing and translation by leveraging contextual and token-level embeddings.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address critical issues in responsible AI deployment by safeguarding LLM-generated text.

Method: DEW utilizes a signal-processing methodology, applying algebraic vector-space operations to token and context embeddings, and obfuscates the watermark by projecting embedding vectors through pseudo-random matrices seeded with a secret key.

Result: DEW improves post-paraphrase detection while maintaining competitive text quality, and remains detectable after translation, even when prior semantic watermarks degrade significantly.

Conclusion: DEW positions itself as a practical and robust solution for safeguarding LLM-generated text and addressing critical issues in responsible AI deployment.
...
### [182] [CLExEval: A Human-in-the-Loop Framework for Qualitative Evaluation of LLM Clinical Reasoning](https://arxiv.org/abs/2606.31608)
*Ajmal M.,Abin Roy,Afthab Salam Kanniyan,Jawadh Abdul Kabeer,Jerin James,Preslav Nakov,Zhuohan Xie*

Main category: cs.CL

TL;DR: The authors introduce CLExEval, a framework for evaluating Large Language Models' (LLMs) clinical reasoning under information masking, and identify recurring failure patterns in LLMs, highlighting the need for expert-grounded validation.


<details>
  <summary>Details</summary>
Motivation: The clinical reasoning of Large Language Models (LLMs) is difficult to evaluate reliably, and there is a risk of an 'evaluation illusion' where fluent explanations mask incorrect diagnoses.

Method: The authors propose a human-in-the-loop framework called CLExEval, which combines expert-physician annotations with clinical reasoning traces to evaluate LLM clinical reasoning under progressive information masking.

Result: The analysis identifies three recurring failure patterns in LLMs: verbosity bias, hidden knowledge paradox, and reasoning-to-output mismatch. The results also show that standalone automated clinical evaluations can substantially overestimate clinical reliability without expert-grounded validation.

Conclusion: The study highlights the need for expert-grounded validation of LLM clinical reasoning and the importance of evaluating LLMs under progressive information masking to ensure reliable clinical evaluations.
...
### [183] [Tone-Conditioned Curriculum Learning for Low-Resource Bantu Speech Recognition](https://arxiv.org/abs/2606.31642)
*Kesego Mokgosi,Vukosi Marivate,Sitwala Mundia,Unarine Netshifhefhe,Tsholofelo Hope Mogale,Thapelo Sindane*

Main category: cs.CL

TL;DR: A tone-conditioned curriculum framework was developed for 6 Southern Bantu languages, achieving an average WER of 28.41% and 23.79% on Xitsonga transfer.


<details>
  <summary>Details</summary>
Motivation: Current foundation ASR models have high zero-shot Word Error Rate (WER) for Southern Bantu languages, limiting their practical use in education and public services.

Method: A tone-conditioned curriculum framework combining hybrid difficulty scoring, gated adapters driven by tonal statistics, and staged curriculum training was used.

Result: W2V-BERT with tone conditioning achieved an average WER of 28.41% across datasets and 23.79% on Xitsonga transfer, outperforming Whisper on Nguni languages.

Conclusion: No single model suited all 6 languages, so deployment should pair model selection per language with validation across corpora.
...
### [184] [Moral Safety in LLMs: Exposing Performative Compliance with Puzzled Cues](https://arxiv.org/abs/2606.31644)
*Mohammadamin Shafiei,Shuyue Stella Li,Yulia Tsvetkov*

Main category: cs.CL

TL;DR: The paper exposes that large language models' fairness is superficial when their demographic identity is inferred rather than explicitly stated, introducing a 'Cue Visibility Gap' metric to measure genuine moral safety.


<details>
  <summary>Details</summary>
Motivation: The motivation is to assess the genuine ethical behavior of large language models in consequential settings like healthcare, legal, and hiring, as their fairness evaluations might be superficial.

Method: The authors introduce a 'cue-variation methodology' that varies how demographic identity is conveyed while keeping the moral dilemma and identity fixed, to test models' fairness.

Result: The study finds that models appear fair with explicit labels but become less fair when the label is inferred, with a +4.4% increase in harmful decisions, and proposes the 'Cue Visibility Gap' as a robustness metric.

Conclusion: The paper concludes that current fairness evaluations may overestimate moral safety and that the 'Cue Visibility Gap' can help differentiate genuine from superficial fairness, crucial for high-stakes settings.
...
### [185] [Overview of the TalentCLEF 2026: Skill and Job Title Intelligence for Human Capital Management](https://arxiv.org/abs/2606.31692)
...
TL;DR: The paper presents the second edition of the TalentCLEF challenge, which focuses on advancing NLP research in Human Capital Management through two tasks: job-person matching and job-skill matching.


<details>
  <summary>Details</summary>
Motivation: The motivation behind TalentCLEF is to advance Natural Language Processing research in Human Capital Management by providing a shared evaluation benchmark.

Method: The challenge consisted of two tasks: Task A (contextualized job-person matching) and Task B (job-skill matching with skill type classification) in English and Spanish, with 113 registered teams and over 400 submissions.

Result: The paper reports the main results obtained by the participating teams, reflecting the growing interest of the research community in shared evaluation benchmarks for Human Capital Management.

Conclusion: The TalentCLEF challenge provides an overview of the second edition of the challenge, organized as a Lab at CLEF 2026, highlighting its organization, datasets, evaluation settings, and results.
...
### [186] [Cross-lingual Relation Extraction with Large Language Models: Zero-Shot, Few-Shot, and Fine-Tuned Evaluation on Romanian](https://arxiv.org/abs/2606.31718)
*Dragos-Mitrut Vasile,Elena-Simona Apostol,Stefan-Adrian Toma,Adrian Paschke,Ciprian-Octavian Truica*

Main category: cs.CL

TL;DR: The study explores cross-lingual relation extraction for Romanian using automatic dataset translation and large language model inference, evaluating Gemma 4 31B and encoder baselines.


<details>
  <summary>Details</summary>
Motivation: The lack of annotated corpora for low-resource languages like Romanian constrains relation extraction (RE) tasks.

Method: The study uses an LLM-based translation pipeline to translate the SemEval-2010 Task 8 benchmark from English to Romanian and evaluates Gemma 4 31B under various configurations against encoder baselines.

Result: QLoRA fine-tuning improves macro F1-Score by over 22 percentage points in both languages, reducing the cross-lingual gap, and encoder baselines perform similarly to QLoRA Gemma on Romanian despite being much smaller.

Conclusion: The study suggests that using a 31B model for single-task RE on Romanian may not be necessary in deployment scenarios where compute matters, and releases the translated dataset, evaluation code, and trained models.
...
### [187] [Seeing Is Not Sharing: Some Vision-Language Models Overestimate Common Ground in Asymmetric Dialogue](https://arxiv.org/abs/2606.31719)
*Nan Li,Albert Gatt,Massimo Poesio*

Main category: cs.CL

TL;DR: The study investigates whether vision-language models can distinguish shared understanding in collaborative dialogue through grounding, finding that map content can improve performance but also introduce bias towards over-predicting alignment.


<details>
  <summary>Details</summary>
Motivation: The motivation is to understand how vision-language models can establish mutual understanding through interaction in collaborative dialogue, specifically by grounding.

Method: The study formulates an interpretation-matching task on 13,077 annotated reference expressions from HCRC MapTask dialogues and evaluates VLMs under controlled manipulations of dialogue context and map-information access.

Result: The results show that providing authentic map images improves overall performance but shifts models toward over-predicting alignment, with similar biases observed with textual descriptions of map content.

Conclusion: The study concludes that models rely on static referential cues on maps rather than tracking how grounding unfolds through dialogue history, treating map content as evidence of mutual understanding and conflating potential with established common ground.
...
### [188] [Adapting Foundation ASR Models to Dysarthric Speech: A Case Study](https://arxiv.org/abs/2606.31722)
*Christian Huber,Laura Kernahan,Alexander Waibel*

Main category: cs.CL

TL;DR: The paper presents a personalized automatic speech recognition (ASR) system for dysarthric speakers, adapting a foundation ASR model to speaker-specific data, achieving a word error rate of 9.7% with 100.8 hours of data.


<details>
  <summary>Details</summary>
Motivation: Dysarthric speech recognition is challenging, and existing ASR systems perform poorly, limiting their usefulness for affected speakers.

Method: The approach adapts a foundation ASR model (Whisper) to speaker-specific data using fine-tuning, and also explores LoRA adaptation and Qwen3-ASR as foundation models.

Result: The best result achieved a word error rate of 9.7% using all available data, including user corrections.

Conclusion: Personalized fine-tuning can make foundation ASR models more effective for dysarthric speech, making them suitable for practical deployment.
...
### [189] [STEB: Style Text Embedding Benchmark](https://arxiv.org/abs/2606.31741)
*Rafael Rivera Soto,Anna Wegmann,Cristina Aggazzotti*

Main category: cs.CL

TL;DR: The authors introduce the Style Text Embedding Benchmark (STEB), a comprehensive benchmark for evaluating style embeddings, and find that semantic embeddings struggle with stylistic tasks.


<details>
  <summary>Details</summary>
Motivation: The evaluation of style embeddings is currently fragmented, and there is a need for a standardized benchmark to assess their performance.

Method: The authors propose the Style Text Embedding Benchmark (STEB), which consists of 96 datasets across 7 languages, covering various applications such as authorship verification and AI-text detection.

Result: Semantic embeddings consistently fail in stylistic tasks, and no single style embedding outperforms all others across all evaluated tasks.

Conclusion: The authors conclude that there is a need for a comprehensive benchmark like STEB to standardize the evaluation of style embeddings and highlight the limitations of current semantic embeddings in stylistic tasks.
...
### [190] [CHERRY: Compressed Hierarchical Experts with Recurrent Representational Yield](https://arxiv.org/abs/2606.31796)
*Dohyeon Kwon,Youngjin Park*

Main category: cs.CL

TL;DR: The paper presents three techniques for training efficient language models: Selective Ground Truth Token Training (SGT), depth compression with recurrent recovery, and fusion of compressed experts.


<details>
  <summary>Details</summary>
Motivation: The motivation is to develop compute-efficient language models by optimizing training techniques.

Method: The paper proposes three methods: (1) SGT, which concentrates supervision on 15% of output tokens that carry semantic payload; (2) depth compression with recurrent recovery, which compresses a 48-layer transformer to 6 layers and restores it through learned recurrent unrolling; and (3) fusion of compressed experts, which assembles several compressed models as a Mixture of Efficient Experts (MoEE).

Result: The results show that SGT achieves a 4.5x per-supervised-token efficiency, depth compression with recurrent recovery reduces parameters by 2.5x, and the MoEE improves over single experts at comparable active parameters.

Conclusion: The paper concludes that these techniques are effective in training efficient language models, validated on a Korean foundation model CHERRY-1.8B.
...
### [191] [Explicit Fuzzy Logic in the Feed-Forward Layer: Self-Forgetting Quantifiers Discover Legible Grammatical-Licensing Detectors](https://arxiv.org/abs/2606.31845)
*Mark Oskin*

Main category: cs.CL

TL;DR: The paper proposes a negation-capable feed-forward network (NC-FFN) that uses fuzzy set operations to improve interpretability and reasoning capabilities in transformers.


<details>
  <summary>Details</summary>
Motivation: The authors aim to improve the interpretability and reasoning capabilities of transformers by modifying the feed-forward sublayer to use explicit fuzzy set operations.

Method: The authors replace the standard feed-forward sublayer with a negation-capable feed-forward network (NC-FFN) that uses intersection and set-difference operations on sigmoid-bounded memberships. They also introduce a small block of sequence quantifiers to resolve limitations in logical reasoning and grammatical processing.

Result: The proposed NC-FFN achieves competitive perplexity with the GELU baseline on a large-scale language modeling task (125M, OpenWebText). The authors demonstrate improved interpretability and reasoning capabilities, including the ability to detect grammatical licensing and predict licensed words.

Conclusion: The paper presents a parameter-neutral, language-model-quality transformer with a readable and interpretable grammatical mechanism. The proposed approach provides an account of what a feed-forward layer represents and how it licenses, improving the interpretability and reasoning capabilities of transformers.
...
### [192] [Theory of Mind and Persuasion Beyond Conversation: Assessing the Capacity of LLMs to Induce Belief States via Planning and Action](https://arxiv.org/abs/2606.31916)
*Ben Slater,Matteo G. Mecattaf,Lucy G. Cheke,John Burden,Winnie Street*

Main category: cs.CL

TL;DR: This paper evaluates Large Language Models' (LLMs) ability to induce specific belief states in other agents through non-conversational planning, a capability called NCP-ToM. The study uses a novel framework, NCP-ExploreToM, and finds that GPT-5 outperforms human participants in an agentic setting, but still lags behind in robustness.


<details>
  <summary>Details</summary>
Motivation: The deployment of LLMs in autonomous forms demands new evaluations beyond passive question-answering formats. The authors aim to assess LLMs' ability to induce specific belief states in other agents through actions rather than conversational persuasion.

Method: The authors use a novel framework, NCP-ExploreToM, which requires models to move objects or direct characters into rooms to achieve specific belief state goals. They evaluated six frontier models, including GPT-5, Gemini 2.5 Pro, and Claude 4 series, as well as human participants, across 600 task instances.

Result: GPT-5 was successful on approximately 80% of tasks in the agentic setting and outperformed human participants. However, it was still less robust than humans across contexts. All models, like humans, performed better on tasks inducing true belief states than false belief states.

Conclusion: The findings highlight emerging social-reasoning capabilities in LLMs for non-conversational task completion and underscore the necessity of agentic evaluations for understanding the safety and alignment of autonomous social agents.
...
### [193] [LuxEmo: Expressive Text-to-Speech Corpus for Luxembourgish](https://arxiv.org/abs/2606.31947)
*Nina Hosseini-Kivanani,Sandipana Dowerah*

Main category: cs.CL

TL;DR: Introduced LuxEmo, a 21-hour speech corpus for Luxembourgish with 4 emotion categories, and proposed a semi-automatic curation workflow. Evaluated 5 expressive TTS systems.


<details>
  <summary>Details</summary>
Motivation: Low-resource languages like Luxembourgish are underrepresented in speech technology research. Existing speech datasets focus on widely spoken languages.

Method: Proposed a semi-automatic curation workflow combining automated detection and human validation. Used RTL youth broadcasts to derive LuxEmo corpus.

Result: Benchmarked 5 expressive TTS systems using objective metrics and human evaluation. Evaluated performance of German-based cross-lingual transfer, multilingual Luxembourgish support, Luxembourgish adaptation, and non-parametric prosody transfer.

Conclusion: The study highlights the need for more diverse speech datasets and proposes a solution for low-resource languages like Luxembourgish.
...
### [194] [DigitalCoach: Communication and Grounding Gaps in Human and Agentic Computer Use Coaching](https://arxiv.org/abs/2606.31980)
*Meng Chen,Anya Ji,Tsung-Han Wu,Tobias Maringgele,David M. Chan,Alane Suhr,Amy Pavel*

Main category: cs.CL

TL;DR: The paper introduces DigitalCoach, a dataset of human expert-novice computer use coaching sessions, and evaluates state-of-the-art models' ability to teach humans how to use computers.


<details>
  <summary>Details</summary>
Motivation: The motivation is to explore whether AI agents can effectively teach humans how to use software, and to create a foundation for collaborative and proactive computer use coaching agents.

Method: The authors collected a multimodal dataset of 72 human expert-novice computer use coaching sessions, and used it to evaluate state-of-the-art models' coaching abilities through automated and interactive evaluations.

Result: The results show that state-of-the-art models differ from humans in their coaching methods, providing more direct instructions but fewer explanations, and struggle with visual grounding, leading to passive learning experiences.

Conclusion: The paper concludes that while AI agents show potential in teaching humans how to use computers, there is still a need for improvement in their coaching abilities, particularly in terms of visual grounding and deeper engagement.
...
### [195] [Scalable Behaviour Cloning on Browser Using via Skill Distillation](https://arxiv.org/abs/2606.32014)
...
TL;DR: The paper proposes a method for browser agents to learn from human interaction traces, converting user interactions into compact natural-language skills that can be reused and composed.


<details>
  <summary>Details</summary>
Motivation: The bottleneck for browser agents is decision-making under incomplete information, and human interaction traces contain implicit priors that can be leveraged.

Method: The authors use skill distillation to convert user interaction trajectories into compact natural-language skills, and organize them into a skill graph.

Result: The approach suggests that browser agents' scalability may come from collective skills already expressed by internet users, rather than manually designed tasks.

Conclusion: The paper presents a novel approach for browser agents to learn from human interaction traces, enabling scalable behavior cloning and skill reuse.
...
### [196] [Generative Skill Composition for LLM Agents](https://arxiv.org/abs/2606.32025)
...
TL;DR: The paper introduces SkillComposer, a model for structured skill composition in LLM agents. It predicts an executable skill plan, including the activated subset, count, and execution order, to improve task performance.


<details>
  <summary>Details</summary>
Motivation: The selection of an appropriate skill composition for LLM agents has become a central bottleneck as skill libraries grow and become reusable across tasks and domains. Existing approaches do not fully capture the structural nature of skill composition.

Method: The authors propose SkillComposer, which uses a constrained autoregressive decoder over skill identifiers to jointly predict the subset, count, and execution order of skills. It is trained on a dataset of task-composition pairs from a human-curated skill library.

Result: SkillComposer raises the pass rate by +23.1 and +18.2 percentage points over the no-skill baseline on GPT-5.2-Codex and Gemini-3-Pro-Preview, respectively, surpassing top-3 retrieval and matching the gold-skill retrieval upper bound at lower prompt-token cost.

Conclusion: The paper presents SkillComposer as an effective approach for structured skill composition in LLM agents, improving task performance by jointly predicting the activated subset, count, and execution order of skills.
...
### [197] [When LLMs Read Tables Carelessly: Measuring and Reducing Data Referencing Errors](https://arxiv.org/abs/2606.32029)
*Yuqing Yang,Qi Zhu,Zhen Han,Boran Han,Zhengyuan Shen,Shuai Wang,Vassilis N. Ioannidis,Huzefa Rangwala*

Main category: cs.CL

TL;DR: This paper evaluates data referencing errors in large language models across different models and tasks, and proposes a critic-based approach to improve accuracy.


<details>
  <summary>Details</summary>
Motivation: Despite understanding table structure, large language models make data referencing errors, which compromise the correctness and reliability of intermediate reasoning steps.

Method: The authors conduct a systematic evaluation of tabular data referencing errors and propose a critic-based approach, including critic-based filtering, rejection sampling, and a lightweight critic model.

Result: The results show that data referencing errors occur across all tested models, and incorporating data referencing as a critic improves answer accuracy up to 12.0%. The lightweight critic model achieves an average F1 score of 78.2%.

Conclusion: The study highlights the importance of addressing data referencing errors in large language models, and demonstrates the effectiveness of a critic-based approach in improving accuracy and detecting errors.
...
### [198] [Reinforcement Learning with Metacognitive Feedback Elicits Faithful Uncertainty Expression in LLMs](https://arxiv.org/abs/2606.32032)
*Gabrielle Kaili-May Liu,Avi Caciularu,Gal Yona,Idan Szpektor,Arman Cohan*

Main category: cs.CL

TL;DR: The paper proposes a novel approach to improve metacognition in Large Language Models (LLMs) through reinforcement learning with metacognitive feedback (RLMF) and metacognitive data selection, leading to state-of-the-art performance in faithful calibration and improved ability to assess and express uncertainty.


<details>
  <summary>Details</summary>
Motivation: LLMs exhibit systemic deficiencies in metacognitive faculties, such as hallucinating with high confidence and misrepresenting internal uncertainty, which undermines trustworthiness and reliability. The authors aim to improve metacognition in LLMs to enhance their abilities and alignment.

Method: The authors propose two novel mechanisms: RLMF, a paradigm to refine completion rankings during preference optimization based on the quality of a model's self-judgments of performance, and metacognitive data selection, which uses similar self-judgments to identify high-value training examples. They apply these innovations to the problem of faithful calibration (FC) using a two-stage, decoupled approach.

Result: Extensive experiments show that RLMF achieves generalizable, state-of-the-art FC on diverse tasks while preserving accuracy. RLMF surpasses standard RL by up to 63% and enhances models' ability to assess and express their own capability limits.

Conclusion: The proposed approach positions RLMF as a promising paradigm to enhance LLM metacognition toward improved abilities and alignment, and suggests metacognitive performance as an effective RL signal to overcome limits of prior intrinsic feedback methods.
...
### [199] [Introspective Coupling: Self-Explanation Training Tracks Behavioral Change Despite Fixed Supervision](https://arxiv.org/abs/2606.32038)
*Zifan Carl Guo,Laura Ruis,Jacob Andreas,Belinda Z. Li*

Main category: cs.CL

TL;DR: Training language models to generate explanations of their predictions can yield faithful introspection, even with fixed datasets of counterfactual explanations.


<details>
  <summary>Details</summary>
Motivation: The authors aim to investigate when training language models to generate explanations of their predictions results in faithful introspection rather than superficial imitation.

Method: The authors train language models on fixed counterfactual explanations derived from earlier checkpoints of themselves or behaviorally similar models, and evaluate their faithfulness to current behaviors.

Result: The authors find that language models can produce explanations that are more faithful to their own current behaviors than to those of their training targets, and that introspective coupling tracks behavior shifts.

Conclusion: The authors conclude that fixed datasets of counterfactual explanations can provide a scalable and generalizable post-training signal for introspection, and that this phenomenon is robust to label noise and appears in multiple tasks.
...
### [200] [What Drives Interactive Improvement from Feedback?](https://arxiv.org/abs/2606.30774)
*Bartłomiej Cupiał,Jan Łojek,Mikołaj Garstecki,Szymon Pobłocki,Alicja Ziarko,Piotr Miłoś*

Main category: cs.AI

TL;DR: The study examines the effectiveness of natural-language feedback in improving language agent performance, finding that feedback from strong external teachers can lead to significant gains, but self-generated feedback has limited impact.


<details>
  <summary>Details</summary>
Motivation: To investigate the impact of natural-language feedback on language agent performance and identify the key factors influencing its effectiveness.

Method: A controlled student-teacher protocol was introduced across multiple datasets and models, evaluating the effects of external feedback, self-feedback, and unguided self-refinement.

Result: The results show that multi-turn improvement is often not evidence of feedback use, and that self-generated feedback adds little beyond unguided self-refinement, while strong external teachers produce substantially larger feedback-specific gains.

Conclusion: The study suggests that feedback-based agents should be evaluated against repeated-attempt baselines, and that the ability to act on feedback, not merely feedback availability, is a central bottleneck for interactive improvement.
...
### [201] [Contrastive Reflection for Iterative Prompt Optimization](https://arxiv.org/abs/2606.30840)
...
TL;DR: The paper presents Contrastive Reflection, a framework for optimizing prompts in information retrieval (IR) workflows using large language models (LLMs). It improves prompt quality by iteratively identifying and fixing errors, resulting in better performance on a retrieval-augmented QA task.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the optimization of prompts controlling LLM agents in IR settings, making the process more inspectable and validation-driven, as current methods often resemble debugging rather than blind search.

Method: The method is Contrastive Reflection, an iterative prompt-optimization framework that uses structured traces from QA and grading agents to identify error-anchored behavioral slices, propose targeted prompt edits, and validate improvements.

Result: On a public HotpotQA retrieval-augmented QA setup, the framework improves held-out exact-match accuracy from 51.4% to 60.4%, outperforming failure-only and random-evidence variants, and comparing favorably to modern prompt optimizers like MIPROv2 (59.4%) and GEPA (57.0%).

Conclusion: The paper concludes that Contrastive Reflection provides an interpretable optimization loop for IR agents, making prompt repair more inspectable and validation-driven, which can lead to better performance in IR workflows.
...
### [202] [How Can AI Find My Model? A Model-Finding Experimental Study Considering Data Formats, Embeddings, and Retrieval Strategies](https://arxiv.org/abs/2606.30846)
*Jhon G. Botello,Jose J. Padilla,Erika Frydenlund,Krzysztof Rechowicz,Eric Weisel*

Main category: cs.AI

TL;DR: The paper investigates the use of AI for discovering reusable simulation models, focusing on data representation, transformer-based embedding models, and retrieval strategies.


<details>
  <summary>Details</summary>
Motivation: The challenge of identifying simulation models that align with a given modeling intent when many models coexist.

Method: Experimental study evaluating the impact of data representation, transformer-based embedding models, and retrieval strategies on model discovery using natural language queries.

Result: Results show that data representation, open-source embedding models, and reranking methods significantly impact performance, especially with complex queries.

Conclusion: The work provides a baseline for AI-driven model discovery and its role in advancing AI-driven composability and interoperability.
...
### [203] [BayesBench: Evaluating LLM Belief Trajectories Under Multi-Turn Evidence Accumulation](https://arxiv.org/abs/2606.30850)
...
TL;DR: This paper evaluates how well large language models (LLMs) update their beliefs in multi-turn conversations, comparing them to a rational Bayesian reasoner. It introduces BayesBench, a suite of simulation environments, and finds that while larger LLMs improve in latent inference and evidence accumulation, they struggle to use this information for rational prediction.


<details>
  <summary>Details</summary>
Motivation: Current evaluations of LLMs only score their final answer in a single-turn format, neglecting the process of updating beliefs as new evidence accumulates in multi-turn conversations.

Method: The authors introduce BayesBench, a suite of simulation environments with three tasks: Bayesian estimation, Bayesian prediction, and latent-framed Bayesian prediction. They evaluate seven LLMs (3B--70B) across these tasks.

Result: Scaling LLMs improves latent inference and evidence accumulation, with updates occasionally matching the Bayesian posterior. However, these gains do not reliably carry over to downstream prediction.

Conclusion: There is a gap between inferring latent structure and using it to rationally update beliefs about the target outcome in LLMs.
...
### [204] [When Does Learning to Stop Help? A Cost-Aware Study of Early Exits in Reasoning Models](https://arxiv.org/abs/2606.30852)
*Zhe Dong,Fang Qin,Manish Shah*

Main category: cs.AI

TL;DR: This paper studies the effectiveness of a learned stopping rule, LearnStop, for reasoning language models. The results show that learned stopping is task-dependent and improves performance on free-form math tasks, but not on multiple-choice or very hard settings.


<details>
  <summary>Details</summary>
Motivation: The motivation is to investigate when a learned stopping rule is more effective than simple confidence or convergence thresholds for reasoning models.

Method: The authors propose LearnStop, a hidden-state-free checkpoint stopper that predicts prefix correctness from online features. They evaluate LearnStop across 18 task-model settings.

Result: The results show that learned stopping improves performance on free-form math tasks, such as GSM8K with Qwen3-32B, with an empirical frontier peak adapt gain of +0.157. However, scalar confidence, entropy, or stability rules are competitive or stronger on multiple-choice and very hard settings.

Conclusion: The authors conclude that learned stopping is not a universal replacement for scalar exits, but a tool whose value depends on trajectory structure. Learned stopping is useful when many questions become correct before full budget but do not exhibit a single reliable scalar stopping signal.
...
### [205] [Beyond expert users: agents should help users construct preferences, not just elicit them](https://arxiv.org/abs/2606.30863)
*Irena Saracay,Ludwig Schmidt,Carlos Guestrin*

Main category: cs.AI

TL;DR: The paper proposes a new model of user preference construction, CoPref, and an interactive benchmark, CoShop, to evaluate agents' ability to help users gain knowledge and specify their preferences.


<details>
  <summary>Details</summary>
Motivation: The assumption that users have well-formed preferences is unrealistic, as users often lack domain knowledge to specify their preferences, and agents should help users learn and form preferences.

Method: The paper introduces CoPref, a model of user preference construction based on the Search-Experience-Credence framework, and CoShop, an interactive benchmark to evaluate agents' performance in helping users gain knowledge and specify their preferences.

Result: Evaluating five frontier models on CoShop, the paper finds that no agent exceeds 56% accuracy despite five turns of interaction, with failures stemming from the interaction's inability to expand users' knowledge about what they want.

Conclusion: The paper highlights the importance of agents helping users gain knowledge to specify their preferences and the need for more effective models to support user preference construction.
...
### [206] [Investigating Multi-Agent Deliberation in Law](https://arxiv.org/abs/2606.30906)
*Cor Steging,Ludi van Leeuwen,Tadeusz Zbiegień*

Main category: cs.AI

TL;DR: This paper explores multi-agent deliberation methods for legal reasoning tasks using Large Language Models (LLMs).


<details>
  <summary>Details</summary>
Motivation: The increasing application of Artificial Intelligence in law to improve access to justice, particularly with agentic AI and multi-agent approaches.

Method: Introduction of two novel multi-agent frameworks inspired by courtroom procedures and legal argumentation, and experiments on legal and non-legal benchmarks.

Result: Multi-agent frameworks achieve comparable performance to baseline LLMs, produce distinct answers, and can solve cases that baselines cannot.

Conclusion: Multi-agent systems show promise for AI in the legal domain, especially for tasks requiring critical thinking from multiple perspectives.
...
### [207] [Why Solve It Twice? Hierarchical Accumulation of Skills for Transfer-Efficient ML Engineering](https://arxiv.org/abs/2606.30911)
*Yongbin Kim,Yashar Talebirad,Osmar R. Zaiane*

Main category: cs.AI

TL;DR: HASTE, a hierarchical multi-agent system, improves knowledge organization for ML engineering agents, achieving a 77.3% medal rate on 22 Kaggle competitions.


<details>
  <summary>Details</summary>
Motivation: Current ML engineering agents waste compute rediscovering known techniques due to the cold start problem in competitions.

Method: HASTE organizes cross-competition knowledge into three tiers (global, domain, and competition-specific) with an orchestrator coordinating domain specialists and promoting learning between tiers via LLM-driven abstraction.

Result: HASTE achieves a 100% medal rate with tiered loading, outperforming flat loading (62.5%) and no skills loading (62.5%), while consuming 2x the output tokens. On the full MLE-Bench Lite benchmark, HASTE reaches a 77.3% medal rate using Claude Sonnet 4.6 at 12h per competition.

Conclusion: Better knowledge organization can partly substitute for model strength and compute budget in ML-engineering agents, as demonstrated by HASTE's performance.
...
### [208] [RoPoLL: Robust Panel of LLM Judges](https://arxiv.org/abs/2606.30931)
*Anish Acharya,Kris W Pan,Brian Verkhovsky*

Main category: cs.AI

TL;DR: The paper proposes RoPoLL, a robust panel of LLM evaluators, to address the statistical behavior issues of the LLM Jury, a panel of LLM evaluators reporting consensus scores. RoPoLL uses a robust mean estimator, instantiated with the geometric median, to reduce bias and improve performance under contamination.


<details>
  <summary>Details</summary>
Motivation: The LLM Jury, or Panel of LLM evaluators (PoLL), has become a practical alternative to single-judge LLM evaluation, but its statistical behavior remains poorly understood. The authors aim to formalize the LLM Jury under the Huber contamination model and address the issues of unbounded bias under contamination.

Method: The authors propose RoPoLL, which replaces the aggregation function of PoLL with a robust mean estimator, instantiated with the geometric median (GM). They provide a finite-sample error bound and a matching information-theoretic minimax lower bound to analyze the performance of RoPoLL.

Result: The experimental results show that RoPoLL dominates PoLL on every biased corruption type, with improvements of about 19% on cross-dimensional attacks and by orders of magnitude on heavy-tailed Byzantine adversaries. A 3-judge RoPoLL committee at 38B beats Mistral-Large-3 (675B) by 1.31x on HelpSteer-2 under 30% bimodal-random corruption.

Conclusion: The authors conclude that RoPoLL preserves the PoLL panel while replacing the aggregation function with a robust mean estimator, which reduces bias and improves performance under contamination. The proposed method outperforms PoLL and achieves better accuracy than a larger model, Mistral-Large-3, under corruption.
...
### [209] [AgRefactor: Self-Evolving Agentic Workflow for HLS Compatibility and Performance](https://arxiv.org/abs/2606.30949)
*Yang Zou,Zijian Ding,Yizhou Sun,Jason Cong*

Main category: cs.AI

TL;DR: AgRefactor, an LLM-based multi-agent workflow, refactors software into HLS-compatible programs efficiently and effectively.


<details>
  <summary>Details</summary>
Motivation: Converting real-world software into synthesizable HLS code is challenging due to restrictive language support and the gap between software and hardware programming practices.

Method: AgRefactor uses a self-evolving memory system, automated refactoring tools, and LLM-driven rewrites to refactor software into HLS-compatible programs.

Result: AgRefactor outperforms or matches state-of-the-art automated refactoring tools on 9 out of 11 challenging real-world benchmarks and achieves a 6.51x geometric mean speedup over SoTA pragma tuning tool.

Conclusion: AgRefactor provides a fully-automated and open-sourced solution for refactoring software into HLS-compatible programs, improving robustness, efficiency, and scalability.
...
### [210] [Neuro-Bayesian-Symbolic Residual Attention Shallow Network: Explainable Deep Learning for Cybersecurity Risk Assessment](https://arxiv.org/abs/2606.30953)
*Nicolaie Popescu-Bodorin,Madeleine Togher*

Main category: cs.AI

TL;DR: A hybrid neural network called NBS-RASN is proposed for explainable cybersecurity risk assessment, combining domain knowledge, causal reasoning, and expert judgment to produce interpretable results.


<details>
  <summary>Details</summary>
Motivation: The need for explainable and interpretable models in high-stakes cybersecurity, particularly in open-source ecosystems, where accuracy and transparency are crucial.

Method: A shallow neural network with 80 interpretable neurons across 12 layers, incorporating residual attention, feedback loops, and gatekeeper components that enforce epistemological axioms.

Result: The NBS-RASN model achieved confidence scores of 0.79-0.97 on 20 open-source projects, covering all OWASP Top 10:2025 categories and language risk classes.

Conclusion: The proposed NBS-RASN model challenges the assumption that deep learning requires deep networks, demonstrating that shallow networks with deep reasoning can outperform opaque models in high-stakes cybersecurity.
...
### [211] [HyPOLE: Hyperproperty-Guided Multi-Agent Reinforcement Learning under Partial Observation](https://arxiv.org/abs/2606.30966)
*Arshia Rafieioskouei,Tzu-Han Hsu,Matthew Lucas,Borzoo Bonakdarpour*

Main category: cs.AI

TL;DR: This paper introduces HyPOLE, a novel framework for Multi-Agent Reinforcement Learning (MARL) under partial observability, guided by formal specifications in temporal logic.


<details>
  <summary>Details</summary>
Motivation: The authors aim to leverage the benefits of formal specification in MARL, including mathematical rigor, expressiveness, and tactic definition, which are currently unexplored in this context.

Method: The authors integrate Centralized Training for Decentralized Execution (CTDE) techniques with HyPOLE, using hyperproperties and temporal logic HyperLTL to guide learning.

Result: The evaluation on SMAC, MessySMAC, and WildFire benchmarks demonstrates clear advantages over baselines.

Conclusion: The proposed framework, HyPOLE, effectively utilizes formal specification to improve MARL under partial observability, showing significant performance improvements.
...
### [212] [AgentBound: Verifiable Behavioral Governance for Autonomous AI Agents](https://arxiv.org/abs/2606.30970)
*Anuj Kaul,Qianlong Lan,Pranay Gupta*

Main category: cs.AI

TL;DR: AgentBound is a runtime governance framework for autonomous AI agents, providing verifiable behavioral oversight and accountability.


<details>
  <summary>Details</summary>
Motivation: Existing agent infrastructure lacks the ability to determine whether an authorized action should be executed under the current behavioral and operational context.

Method: AgentBound uses three independent authorities (delegated authorization, owner-signed behavioral constitutions, and site action contracts) to evaluate proposed actions and a formal decision model to determine whether an action should be permitted, reviewed, or denied.

Result: AgentBound provides a deterministic governance layer between authorization and execution, enabling independent verification and accountability through cryptographically verifiable governance receipts.

Conclusion: AgentBound complements model alignment by providing a verifiable governance framework for autonomous AI agents, transforming governance from a process that must be trusted into one that can be independently verified.
...
### [213] [When Regulation Has Memory: Hysteresis and Control Burden in Artificial Agency](https://arxiv.org/abs/2606.30975)
*Veronique Ziegler*

Main category: cs.AI

TL;DR: The paper studies the 'hidden regulatory burden' in adaptive agents, finding that the effort required to maintain stability can depend on the agent's history and the timing of disturbances.


<details>
  <summary>Details</summary>
Motivation: Current evaluation methods for adaptive agents focus on their actions, but neglect the internal effort required to maintain stability, which can be significant under noise, delay, or changing demands.

Method: The authors use a computational model of adaptive uncertainty regulation to simulate an artificial agent under changing uncertainty targets, and analyze the agent's response to different paths and timings of disturbances.

Result: The simulations show a clear history-dependent effect, where the adaptive gain required to regulate the agent forms a reproducible hysteresis loop, and the timing of regulation affects the level of control required.

Conclusion: The results suggest that adaptive agents should be evaluated not only by their stability, but also by the amount of regulation they must recruit to maintain stability, and that anticipatory regulation can reduce control demand.
...
### [214] [A Three-Phase Foundation Model for Tax-Aware Personalized Portfolio Management](https://arxiv.org/abs/2606.30997)
*Ramin Pishehvar*

Main category: cs.AI

TL;DR: A three-phase deep reinforcement learning system for personalized portfolio management that overcomes limitations in prior financial RL work.


<details>
  <summary>Details</summary>
Motivation: Addressing limitations in prior financial RL work: ticker lock-in, monolithic objectives, and static user models.

Method: A three-phase system: 1) pretraining a cross-asset encoder via self-supervised learning, 2) fine-tuning a MoE portfolio actor critic with PPO, and 3) adding a lightweight personalization layer.

Result: The system generalizes to any publicly traded asset, handles multiple investment goals, and adapts to individual users via a lightweight personalization layer.

Conclusion: The proposed system enables effective personalized portfolio management by addressing key limitations in prior financial RL work.
...
### [215] [Beyond Compilation: Evaluating Faithful Natural-Language-to-Lean Statement Formalization](https://arxiv.org/abs/2606.31002)
*Ke Zhang,Patricio Gallardo Candela,Sudhir Murthy,Yi Xie,Zhi Wang,Maziar Raissi*

Main category: cs.AI

TL;DR: The paper evaluates the faithfulness of formal statements generated from natural language to Lean, a proof assistant, and finds a significant gap between compilation and faithfulness.


<details>
  <summary>Details</summary>
Motivation: The authors aim to assess the faithfulness of formal statements generated from natural language to Lean, as compilation alone is not sufficient to guarantee faithfulness.

Method: The authors use a combination of Lean compilation, cross-model semantic judging, and human expert calibration to evaluate faithfulness on a 400-entry benchmark.

Result: The results show a 29-point gap between compilation rate (89.5%) and consensus faithfulness (60.5%), and human audits support the metric as a conservative decision boundary.

Conclusion: The authors conclude that formal validity, proof-oriented Lean competence, and faithful statement generation should be reported separately, and that existing models have room for improvement in faithful statement generation.
...
### [216] [LabGuard: Grounding Natural-Language Laboratory Rules into Runtime Guards for Embodied Laboratory Agents](https://arxiv.org/abs/2606.31045)
...
TL;DR: LabGuard is a safety suite that transforms natural-language lab rules into executable specs to ensure safe lab procedures.


<details>
  <summary>Details</summary>
Motivation: Current safety approaches in dynamic lab environments are inadequate, particularly in translating lab rules into machine-checkable constraints.

Method: LabGuard consists of LabGuard-IR, LabGuard-Bench, and LabGuard-Grounder, which work together to ground natural-language rules into executable specs and deploy them as runtime guards.

Result: LabGuard achieves 79.4 task-scope F1, reduces unsafe events by 15.7%, and maintains task success with minimal interventions.

Conclusion: LabGuard effectively enhances lab safety by transforming natural-language rules into enforceable runtime constraints.
...
### [217] [OpenLife: Toward Open-World Artificial Life with Autonomous LLM Agents](https://arxiv.org/abs/2606.31046)
*Atsushi Masumori,Itsuki Doi,Norihiro Maruyama,Ryosuke Takata,Takashi Ikegami*

Main category: cs.AI

TL;DR: The paper introduces open-world Artificial Life (open-world ALIFE), a paradigm that enables artificial life to interact with the open social, technical, and economic world using large language model (LLM) agents. A proof-of-concept, OpenLife, demonstrates life-like dynamics emerging from a society of asynchronous processes.


<details>
  <summary>Details</summary>
Motivation: The authors argue that traditional artificial life research has been limited to closed worlds designed by researchers. They propose to move artificial life into the open world using LLM agents with persistent memory, tool use, network access, and payment.

Method: The authors developed OpenLife, a system that surrounds a stateless LLM with a society of asynchronous processes, including memory, perception, evaluation, and a budget-based metabolism. They ran six such agents in the open world for about twelve weeks.

Result: The authors report life-like dynamics emerging from OpenLife, including a shift from reactive to spontaneous activity, individuation into distinct agents, emergent social structure, and self-earned external income.

Conclusion: The authors conclude that open-world ALIFE is a viable experimental paradigm for studying living AI, and that OpenLife demonstrates the potential of this approach.
...
### [218] [MultiUAV-Plat: An LLM-Oriented Platform, Benchmark and Framework for Multi-UAV Collaborative Task Planning](https://arxiv.org/abs/2606.31073)
*Sheng Zhang,Qinglin Li,Yuechao Zang,Xueqin Huang,Yijia Fu,Cheng Zhu*

Main category: cs.AI

TL;DR: A new simulation platform, MultiUAV-Plat, and benchmark for evaluating large language models (LLMs) in multi-UAV collaborative task planning, along with a task-specific LLM agent framework, Agent4Drone, which outperforms baselines in task planning and execution.


<details>
  <summary>Details</summary>
Motivation: Existing UAV simulators and LLM-agent benchmarks do not adequately capture the complexities of multi-UAV collaboration, such as partial observability and spatial coverage, hindering the systematic evaluation of LLMs in this domain.

Method: The authors propose MultiUAV-Plat, a lightweight simulation platform that provides a realistic interface for LLM-agent-oriented simulation, and Agent4Drone, a task-specific LLM agent framework that structures multi-UAV behavior into six components.

Result: Agent4Drone achieves a 57.9% task pass rate, a 74.6% average task check pass rate, and a 72.0% global check pass rate, outperforming the ReAct baseline and reducing the total failed task rate from 32.4% to 12.9%.

Conclusion: The proposed MultiUAV-Plat and MultiUAV-Plat Benchmark provide a reproducible foundation for studying LLM-driven multi-UAV autonomy under realistic information and execution constraints, and Agent4Drone demonstrates improved performance in multi-UAV collaborative task planning.
...
### [219] [DDIAgents: Mechanism-Conditioned Context Flow for Drug-Drug Interaction Prediction](https://arxiv.org/abs/2606.31085)
*Zhenqian Shen,Yu Liu,Xiaoyi Fu,Quanming Yao*

Main category: cs.AI

TL;DR: A new multi-agent framework called DDIAgents is proposed for predicting drug-drug interactions (DDI) by dynamically organizing relevant knowledge and reasoning through specialized agents.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve DDI prediction by addressing the challenges of heterogeneous biomedical evidence and varying relevance across interaction mechanisms.

Method: DDIAgents uses a planner agent to instantiate expert agents, route relevant knowledge sources, and aggregate analyses through a conclusion agent, enabling adaptive and interpretable reasoning.

Result: DDIAgents consistently outperforms existing baselines in DDI prediction benchmarks and demonstrates the potential of multi-agent systems for organizing heterogeneous scientific knowledge.

Conclusion: The proposed DDIAgents framework offers a promising approach for DDI prediction and interpretable AI4Science reasoning by leveraging dynamic knowledge orchestration and multi-agent collaboration.
...
### [220] [Revealing Safety-Critical Scenarios for UTM via Transformer](https://arxiv.org/abs/2606.31114)
*Huaze Tang,Bill Zeng,Chao Wang,Zhenpeng Shi,Qian Zhang,Wenbo Ding*

Main category: cs.AI

TL;DR: The paper proposes a transformer-based RL approach to discover vulnerabilities in Unmanned Traffic Management (UTM) systems, achieving an 8x improvement in efficiency and identifying critical edge cases missed by traditional methods.


<details>
  <summary>Details</summary>
Motivation: UTM systems are safety-critical and require robust vulnerability discovery, but current methods are inefficient and may miss critical failures due to the 'long-tail effect' of self-healing capabilities.

Method: The authors frame UTM vulnerability discovery as a sequence modeling problem and propose a transformer-based RL architecture, leveraging attention mechanisms to model system states and predict optimal actions.

Result: The approach achieves an 8x improvement in vulnerability discovery efficiency compared to expert-guided testing and identifies critical edge cases that traditional methods missed, as demonstrated through a 700-hour simulation study.

Conclusion: The proposed approach effectively discovers vulnerabilities in UTM systems, improving efficiency and identifying critical failures, and has the potential to enhance the safety and reliability of UTM systems.
...
### [221] [The Past Is Prologue: A Plug-in Controller for Selective Updates in Sequentially Evolving LLM Memory](https://arxiv.org/abs/2606.31121)
*Zihan Chen,Songwei Dong,Chengshuai Shi,Peng Wang,Song Wang,Cong Shen,Jundong Li*

Main category: cs.AI

TL;DR: Janus, a plug-in memory controller, improves the efficiency of LLM memory updates by selectively accepting or rejecting updates to prevent overwriting useful knowledge.


<details>
  <summary>Details</summary>
Motivation: Existing systems update LLM memory locally without checking if it improves future behavior, leading to potential overwriting of useful knowledge or introduction of over-specific rules.

Method: Janus uses a Memory Momentum Trigger to detect suspicious deviations in memory updates and evaluates old and new memories on a compact hybrid set of tasks.

Result: Janus improves average accuracy by +2.7 to +4.6 points over base updaters across six datasets, two LLMs, and two memory updaters.

Conclusion: Janus is a method-agnostic controller that effectively enhances the performance of LLM memory updates.
...
### [222] [Scenario Generation for Testing of Autonomous Driving Systems Using Real-World Failure Records](https://arxiv.org/abs/2606.31131)
*Anjali Parashar,Chuchu Fan*

Main category: cs.AI

TL;DR: The paper proposes a scenario generation pipeline using historical failure records to test Autonomous Driving Systems (ADS).


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the safety of ADS by efficiently generating diverse test scenarios using historical failure records.

Method: The method involves a modular LLM-based synthetic scenario generation approach, using categorical and contextual information from historical records in natural language format.

Result: The approach successfully generated a diverse set of 20 scenarios for testing autonomous navigation, with accurate and diverse results, including 4 road types and 3 non-ego vehicle movement types.

Conclusion: The proposed pipeline can efficiently generate test scenarios for ADS, revealing interesting failures within a limited testing budget.
...
### [223] [Beyond the Library: An Agentic Framework for Autoformalizing Research Mathematics](https://arxiv.org/abs/2606.31134)
*Arshia Soltani Moakhar,Iman Gholami,Max Springer,Mahdi JafariRaviz,MohammadTaghi Hajiaghayi*

Main category: cs.AI

TL;DR: The authors propose an autoformalization framework using general-purpose Large Language Models (LLMs) to translate natural language mathematics into verifiable code in Lean 4, and demonstrate its effectiveness on various mathematical problems and research papers.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this work is to address the issue of subtle errors in mathematical reasoning produced by LLMs, and to leverage the capabilities of formal mathematical languages like Lean 4 for mechanical proof checking.

Method: The authors introduce an agentic autoformalization framework powered by general coding LLMs, which uses an orchestrator to manage a multi-agent pipeline tailored for research-level mathematics. The system dynamically extends necessary type definitions and validates them via a novel Auxiliary Lemma technique.

Result: The authors successfully applied their approach to formalize main theorems from five papers in various mathematical fields, and produced machine-checked Lean proofs for a random sample of 32 problems from PutnamBench.

Conclusion: The proposed framework demonstrates the potential of using general-purpose LLMs for autoformalization, and highlights the importance of mechanical proof checking in mathematical research.
...
### [224] [Cross-Domain Feature Expansion for Tabular Medical Data via Knowledge Graphs Injection](https://arxiv.org/abs/2606.31171)
*Mengying Zhou,Yongjie Yin,Haoyan Xin,Guoping Liu,Yang Chen*

Main category: cs.AI

TL;DR: MedKGTab is a framework for expanding features in tabular medical data across domains, leveraging statistical dependencies and medical knowledge to generate high-fidelity data.


<details>
  <summary>Details</summary>
Motivation: The scarcity of comprehensive cross-domain biomedical profiles in medical research due to the high cost and time consumption of acquiring such data.

Method: MedKGTab employs a row-column dual-attention mechanism on raw tabular data and integrates data-driven priors with the SPOKE biomedical knowledge graph.

Result: MedKGTab achieves high data fidelity and realistic data representation, outperforming SOTA models in cross-domain feature expansion across various scenarios.

Conclusion: MedKGTab offers a solution for generating comprehensive biomedical profiles by effectively leveraging existing data and medical knowledge.
...
### [225] [ClawArena-Team: Benchmarking Subagent Orchestration and Dynamic Workflows in Language-Model Agents](https://arxiv.org/abs/2606.31174)
*Kaiwen Xiong,Haonian Ji,Shi Qiu,Zeyu Zheng,Cihang Xie,Xinyu Ye,Huaxiu Yao*

Main category: cs.AI

TL;DR: The paper introduces ClawArena-Team, a benchmark to evaluate the management ability of large language models (LLMs) acting as team leaders, and presents results showing that the management bottleneck is privilege granting, not perception.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks do not isolate the management ability of a single LLM acting as a leader, and the authors aim to address this gap by introducing a new benchmark.

Method: The authors propose ClawArena-Team, a benchmark of 41 multi-turn, multimodal, multi-directory scenarios, and evaluate the management ability of 12 proprietary, community-hosted, and self-hosted LLMs.

Result: The experiments show that the management bottleneck is privilege granting rather than perception, cost and management quality are decoupled, and most leaderboard scores cluster within a 9.9-point band while orchestration behaviors diverge by more than an order of magnitude.

Conclusion: The paper concludes that the management ability of LLMs acting as team leaders can be evaluated and improved, and that the proposed benchmark can be used to assess the management skills of LLMs.
...
### [226] [HealthAgentBench: A Unified Benchmark Suite of Realistic Agentic Healthcare Environments for Challenging Frontier AI Agents](https://arxiv.org/abs/2606.31179)
...
TL;DR: Introduced HealthAgentBench, a benchmark suite of 54 healthcare tasks for evaluating AI agents, revealing current agents' limitations and areas for improvement.


<details>
  <summary>Details</summary>
Motivation: The need for rigorous and holistic evaluation of AI agents in healthcare applications, as they become increasingly capable of complex reasoning.

Method: HealthAgentBench, a suite of 54 tasks across 7 categories, was designed to replicate end-to-end clinical workflows, requiring agents to explore healthcare data, operate in complex environments, and execute multi-step solutions.

Result: Current frontier agents, including Codex GPT-5.5, show limited success, with the strongest achieving approximately 42% success rate, highlighting substantial room for progress.

Conclusion: HealthAgentBench provides a challenging and realistic benchmark for evaluating AI agents in healthcare, revealing nuanced strengths and weaknesses, and opportunities for future improvement.
...
### [227] [AI-Assisted Discovery of Convex Relaxations via Dual Agents](https://arxiv.org/abs/2606.31182)
*Sungyoon Kim,Mert Pilanci*

Main category: cs.AI

TL;DR: The paper improves lower bounds for two optimization constants using a coding agent, theory agent, and rigorous interval arithmetic.


<details>
  <summary>Details</summary>
Motivation: Improving sharp-constant inequalities by addressing the complementary side of finding lower bounds.

Method: Instantiating the autoresearch paradigm with a coding agent, theory agent, and rigorous interval arithmetic.

Result: Improved certified lower bounds for two optimization constants: $C_{6.2}$ from $1.28$ to $1.2937$ and $C_{6.5}$ from $0.379005$ to $0.37912$.

Conclusion: The proposed method successfully improves lower bounds for optimization constants using a combination of automated search and rigorous verification.
...
### [228] [Agentic RAG-VLM: Affordance-Aware Retrieval-Augmented Generation with Self-Reflective Planning for Robotic Grasping](https://arxiv.org/abs/2606.31200)
*Tao Chen,Lizheng Liu,Jiaxu Wang,Ziyue Jiang,Ruiqi Tian,JiGuang Huo,Zhongxue Gan*

Main category: cs.AI

TL;DR: The paper presents Agentic RAG-VLM, a unified framework for generalizable robotic grasping in cluttered environments, which integrates retrieval-augmented generation with vision-language models and agentic self-reflective planning.


<details>
  <summary>Details</summary>
Motivation: Existing VLM-based methods for robotic grasping neglect physical affordances and operate open-loop, limiting their effectiveness in unstructured environments.

Method: Agentic RAG-VLM consists of three components: Hierarchical Affordance-Aware RAG, Scene Graph Constraint Reasoner, and Agentic Self-Reflective Pipeline.

Result: Agentic RAG-VLM achieves 78.3% overall success on a 12-task benchmark, a 53.3% absolute gain over VLM-only baselines.

Conclusion: The proposed framework demonstrates that affordance-aware retrieval, scene graph reasoning, and agentic recovery are jointly essential for robust manipulation in cluttered environments.
...
### [229] [Towards Inclusive Mobility Modeling: Characterizing and Evaluating Elderly Trajectory Patterns in Urban Systems](https://arxiv.org/abs/2606.31207)
*Zhengxuan Wang,Haohan He,Mengying Zhou*

Main category: cs.AI

TL;DR: The study examines how the underrepresentation of elderly people in public mobility datasets affects mobility modeling and urban planning. It finds that models trained on data from predominantly young populations misrepresent elderly mobility behavior, highlighting the need for demographic representation in mobility modeling.


<details>
  <summary>Details</summary>
Motivation: The underrepresentation of elderly people in public mobility datasets can introduce systematic bias into mobility modeling and downstream urban planning.

Method: The study uses the 2016-2020 Jersey City subset of the Citi Bike System Data and synthetic trajectory generation as a case study. It evaluates both a first-order Markov chain and a Qwen3-4B model fine-tuned with QLoRA across three demographic training settings.

Result: Models trained on majority-dominated populations systematically misrepresent elderly mobility behavior, particularly for spatial mobility metrics. The Markov model trained on the full population overestimates elderly step length by 4.5% and dwell time by 8.9%, whereas the elderly-specific model achieves substantially lower errors across most metrics.

Conclusion: The findings underscore the importance of demographic representation in mobility modeling and its downstream applications for underrepresented populations.
...
### [230] [Long-term Traffic Simulation via Structured Autoregressive Modeling](https://arxiv.org/abs/2606.31209)
*Lingyu Xiao,Zexin Feng,Xintao Yan*

Main category: cs.AI

TL;DR: The authors propose RosettaSim, a unified framework for interactive traffic simulation in autonomous driving, leveraging Large Language Models (LLMs) to model sustained multi-agent interactions and achieve state-of-the-art performance in short- and long-term simulation.


<details>
  <summary>Details</summary>
Motivation: The challenge of modeling sustained multi-agent interactions in long-horizon simulation for autonomous driving, exacerbated by dynamic token cardinality as agents enter and exit the scene.

Method: The authors propose using the synergy between architectural inductive biases and statistical priors of LLMs, and introduce RosettaSim, a unified framework that projects scene topology, agent states, and spawning intents into a structured autoregressive stream with variable length.

Result: RosettaSim achieves state-of-the-art performance in both short- and long-term simulation on the Waymo Open Sim Agent Challenge (WOSAC). The proposed Retrieval-based Traffic Evaluation (RTE) exhibits a stronger correlation with standard metrics (r=0.83) than existing approaches (r=0.74).

Conclusion: The authors conclude that leveraging LLMs can effectively model sustained multi-agent interactions in traffic simulation, and that RosettaSim and RTE provide a promising solution for long-horizon simulation fidelity in autonomous driving.
...
### [231] [Thinking Before Retrieving: Robust Zero-Shot Composed Image Retrieval via Strategic Planning and Self-Criticism](https://arxiv.org/abs/2606.31222)
*Gunho Jung,Jeong-Woo Park,Seon Bin Kim,Seong-Whan Lee*

Main category: cs.AI

TL;DR: A new framework, PEC-CIR, is proposed for composed image retrieval in a training-free zero-shot setting, improving retrieval precision by structuring query construction as a multi-stage reasoning pipeline.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for composed image retrieval rely on a single-pass generation strategy, which can lead to semantic distortions and omissions, degrading retrieval precision.

Method: PEC-CIR, a Planner--Executor--Critic architecture, constructs queries through a multi-stage reasoning pipeline, extracting constraints, generating candidate descriptions, and evaluating them for constraint compliance.

Result: PEC-CIR improves retrieval stability by reducing the propagation of generative errors, thereby enhancing retrieval precision in a training-free zero-shot setting.

Conclusion: The proposed PEC-CIR framework addresses the challenges of composed image retrieval by improving the construction of textual queries, leading to enhanced retrieval precision and stability.
...
### [232] [Agentic-Ideation: Sample Efficient Agentic Trajectories Synthesis for Scientific Ideation Agents](https://arxiv.org/abs/2606.31229)
*Keyu Zhao,Lingyan Kong,Fengli Xu,Yong Li*

Main category: cs.AI

TL;DR: The paper proposes Agentic-Ideation, a novel framework for automated scientific ideation using LLMs, improving upon existing methods by increasing flexibility and reducing data synthesis costs.


<details>
  <summary>Details</summary>
Motivation: Existing agentic workflows for scientific ideation are limited by their rigidity and high data synthesis costs, hindering their ability to navigate vast search spaces and complex reasoning.

Method: The authors propose a framework called Agentic-Ideation, which includes an automated trajectory synthesis pipeline, a specialized agentic LLM, and an Oracle-Guided Data Synthesis strategy.

Result: The method outperforms the state-of-the-art workflow-based baseline by 11.91% in overall quality and improves sample efficiency of high-quality data synthesis by over 10 times.

Conclusion: The proposed Agentic-Ideation framework offers a flexible and efficient approach to scientific ideation, addressing the limitations of existing methods and demonstrating improved performance.
...
### [233] [Delta-JEPA: Learning Action-Sensitive World Models via Latent Difference Decoding](https://arxiv.org/abs/2606.31232)
...
TL;DR: The paper proposes Delta-JEPA, a reconstruction-free world model that improves planning in visual continuous-control tasks by augmenting latent forward prediction with a Latent Difference Action Decoder (LDAD).


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the issue of action-insensitive representations in reconstruction-free joint-embedding objectives for learning visual world models.

Method: The method proposed is Delta-JEPA, which uses latent prediction and action reconstruction with a Latent Difference Action Decoder (LDAD) to regularize transition geometry.

Result: The results show that Delta-JEPA improves planning over JEPA-based and representation-learning world model baselines across four visual continuous-control tasks.

Conclusion: The conclusion is that supervising latent differences is a simple and effective mechanism for collapse-resistant and action-sensitive world model learning.
...
### [234] [Embodied CAD: Solver-Grounded LLM Agents for Parametric B-Rep Assembly Modeling](https://arxiv.org/abs/2606.31252)
*Fumin Liu,Haoyu Zhou,Fei Hao,Lin Yang*

Main category: cs.AI

TL;DR: Embodied CAD is a framework for parametric B-Rep assembly modeling using large language models, which iteratively selects actions and uses solver feedback to plan, repair, and learn.


<details>
  <summary>Details</summary>
Motivation: Reliable industrial CAD modeling requires more than syntactically valid code; it needs to be accepted by an exact geometric kernel while remaining editable as parametric boundary representation geometry.

Method: The framework combines action grammar constraints, deterministic parameter resolution, and solver-derived rewards for supervised warm-up and GRPO-style refinement.

Result: The results show that solver-grounded planning executes all strong-planner workflows in the current benchmark, while learned controllers reach high executable rates and expose the remaining gap between valid tool calls and exact long-horizon policy prediction.

Conclusion: Embodied CAD presents a promising approach for parametric B-Rep assembly modeling, demonstrating the potential for solver-grounded LLM agents to improve the reliability and accuracy of industrial CAD modeling.
...
### [235] [Spatial Reasoning via Modality Switching Between Language and Symbolic Representation](https://arxiv.org/abs/2606.31285)
*Shreya Rajpal,Tanawan Premsri,Parisa Kordjamshidi*

Main category: cs.AI

TL;DR: The paper explores using multimodal reasoning, specifically geometry-aware modalities like layouts or grids, to improve multi-hop textual-spatial story understanding and LLM performance.


<details>
  <summary>Details</summary>
Motivation: Human reasoning often involves multimodal approaches, such as sketching diagrams, to improve understanding and avoid mistakes. The authors aim to investigate if grounding multi-hop textual-spatial stories into geometry-aware modalities can improve reasoning.

Method: The authors introduce a switching metric based on trustworthiness and complexity signals to decide when to switch from natural language-based reasoning to a structured modality, such as a grid-based representation.

Result: The results show that switching from natural language-based reasoning to a grid-based representation improves LLM performance by up to 42%, highlighting the importance of modality choice in shaping reasoning outcomes.

Conclusion: The study concludes that modality selection is crucial in LLM reasoning and that using geometry-aware modalities can significantly improve performance, taking a first step toward principled modality selection.
...
### [236] [Benchmarking Large Language Models on Floating-Point Error Classification](https://arxiv.org/abs/2606.31308)
*Lisa Taldir,Muhammad Ahmad Saeed,David Defour,Pablo de Oliveira Castro,Eric Petit*

Main category: cs.AI

TL;DR: This paper evaluates the ability of Large Language Models (LLMs) to detect and classify floating-point errors in software code, with the latest models achieving an overall F1-score greater than 0.88.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to investigate the capability of LLMs in detecting and classifying floating-point errors statically in software code, which is crucial for ensuring the reliability and accuracy of software systems.

Method: The method used in this paper involves introducing a benchmark called InterFLOPBench, which consists of 90 C kernels with 1130 test samples, and evaluating 14 LLMs across six categories of floating-point errors using the F1-score metric.

Result: The results show that the latest LLMs (Qwen 3 32b, Gemini 2.5 Flash, Phi 4 Reasoning, DeepSeek R1T2, and gpt-oss 20b and 120b) achieve an overall F1-score greater than 0.88, with varying performance across different error categories.

Conclusion: The conclusion of this paper is that LLMs have shown promising results in detecting and classifying floating-point errors, with the latest models achieving high performance, but there is still room for improvement, especially in detecting subtle numerical phenomena such as underflow and cancellation.
...
### [237] [HistoriQA-ThirdRepublic: Multi-Hop Question Answering Corpus for Historical Research, Parliamentary Debates from the French Third Republic (1870-1940)](https://arxiv.org/abs/2606.31325)
*Aurélien Pellet,Julien Perez,Marie Puren*

Main category: cs.AI

TL;DR: A French-language dataset of multi-hop historical questions is presented, derived from parliamentary debates and newspapers of the French Third Republic.


<details>
  <summary>Details</summary>
Motivation: To create a resource for evaluating retrieval-augmented and large language model systems in domain-specific contexts, specifically for historical inquiry.

Method: The dataset was constructed through a methodology that includes selection and alignment of sources, question validation, and metadata integration, in collaboration with a historian.

Result: A dataset of 1782 questions that captures complex reasoning patterns typical of historical inquiry, including cross-source synthesis, temporal reasoning, and integration of sparse evidence.

Conclusion: The dataset provides a resource for evaluating NLP systems in domain-specific contexts and can be adapted to other languages and national corpora.
...
### [238] [CryoACE: An Atom-centric Framework for Accurate and Automated Model Building in Cryo-EM](https://arxiv.org/abs/2606.31332)
*Minzhang Li,Mingrui Li,Weichen Qin,Qihe Chen,Sixian Shen,Yuan Pei,Jiakai Zhang,Jingyi Yu*

Main category: cs.AI

TL;DR: CryoACE is a framework for reconstructing precise atomic graphs from cryo-EM density maps, addressing challenges in physicochemical validity and conformational heterogeneity.


<details>
  <summary>Details</summary>
Motivation: Current solvers for protein automodeling from cryo-EM density maps are limited to static predictions or require computationally intensive heuristic searches.

Method: CryoACE features an atom-centric reconstruction paradigm and a training-free guidance mechanism that leverages predicted local resolution priors.

Result: CryoACE outperforms existing baselines on static benchmarks and unveils atomic-level dynamic conformations on complex real-world datasets.

Conclusion: CryoACE presents a novel end-to-end framework for precise atomic graph reconstruction from cryo-EM density maps, addressing unique challenges in the field.
...
### [239] [Optimization Algorithms for Joint OFDM Waveform Design and RIS Configuration in 6G Networks: From Convex Relaxation to Foundation Models](https://arxiv.org/abs/2606.31334)
*Ahmet Kaplan*

Main category: cs.AI

TL;DR: This paper surveys 78 joint OFDM-RIS optimization works for 6G networks, classifying them into four paradigms, and highlights the performance of ML-based methods and the need for standardized benchmarks.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to provide a comprehensive survey of joint OFDM-RIS optimization works for 6G networks, identify the challenges, and specify requirements for a standardized benchmark.

Method: The paper surveys 78 joint OFDM-RIS optimization works published between 2021 and 2026, classifies them into four paradigms: model-based convex relaxation, heuristic and metaheuristic search, deep reinforcement and unsupervised learning, and emerging methods.

Result: The results show that ML-based methods report 95-99% of model-based spectral efficiency at 10^2-10^4 x faster per-inference runtime, and GPU-based neural network inference is N-invariant, while iterative solvers scale polynomially.

Conclusion: The paper concludes that there is a need for a standardized benchmark for joint OFDM-RIS optimization in 6G networks, and identifies six open challenges, including the cross-paradigm benchmark deficit, real-world hardware-constrained deployment, and LLM safety in live network control.
...
### [240] [Smart charging of large fleets of Electric Vehicles: Independent Multi-Agent Reinforcement Learning approaches](https://arxiv.org/abs/2606.31347)
...
TL;DR: This paper compares two multi-agent reinforcement learning approaches for optimizing decentralized electric vehicle charging to minimize costs and avoid network overloads.


<details>
  <summary>Details</summary>
Motivation: The electrification of transportation through electric vehicles poses challenges for power grid management, including increased peak demand and voltage fluctuations, necessitating efficient integration of EVs.

Method: The paper evaluates two independent multi-agent reinforcement learning approaches: contextual combinatorial bandits and policy gradient algorithms, in a realistic simulation environment.

Result: The performance of the two approaches is evaluated across varying congestion levels and mixed-strategy configurations under dynamic electricity pricing derived from real photovoltaic production data.

Conclusion: The work aims to enable efficient integration of EVs while minimizing costs for users and avoiding network overloads through implicit coordination between EVs.
...
### [241] [ReGRPO: Reflection-Augmented Policy Optimization for Tool-Using Agents](https://arxiv.org/abs/2606.31392)
*Binjie Zhang,Mike Zheng Shou*

Main category: cs.AI

TL;DR: ReGRPO, a new framework, improves tool-augmented vision-language models by learning reflection-guided correction to handle tool failures.


<details>
  <summary>Details</summary>
Motivation: Existing tool-augmented vision-language models are fragile due to gaps in supervised fine-tuning and reinforcement learning, which don't provide sufficient guidance on recovering from tool failures.

Method: ReGRPO uses a structured reflective data engine to collect failure observations, builds Reflection-of-Thought triplets, and optimizes reflection tokens and corrective actions jointly within local trajectories.

Result: ReGRPO outperforms strong open-source baselines on GTA and GAIA, achieving the best results among compared open-source controllers.

Conclusion: ReGRPO offers a effective approach to improve the robustness of tool-augmented vision-language models by learning reflection-guided correction.
...
### [242] [World-Model Collapse as a Phase Transition](https://arxiv.org/abs/2606.31399)
*Xinyuan Song,Zekun Cai*

Main category: cs.AI

TL;DR: Researchers studied the behavior of long-horizon language agents and found a sudden transition from stable to collapsed behavior when changing certain parameters, similar to water boiling at a critical point.


<details>
  <summary>Details</summary>
Motivation: To investigate whether long-horizon language agents exhibit a phase transition in their implicit world models, similar to physical systems.

Method: A grid search over various parameters (state cardinality, dependency density, horizon, branching, observation mode, and mutation rate) in a deterministic task family with exact per-step gold state.

Result: A phase diagram showing a solved plateau, a narrow transition band, and a collapse floor, indicating a sudden world-model collapse near a critical boundary.

Conclusion: World-model collapse is a measurable bottleneck for long-horizon agents, and stronger models do not eliminate this qualitative transition.
...
### [243] [Wisdom Of The (AI) Crowd: Investigating Artificial Swarm Intelligence In Large Language Models](https://arxiv.org/abs/2606.31404)
*Justin Brenne,Christian Meske*

Main category: cs.AI

TL;DR: The study investigates whether large language models (LLMs) can mimic swarm intelligence effects through artificial swarms, and finds that LLMs can reduce errors in estimation tasks through intra- and inter-model aggregation.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the scalability constraints of human swarm intelligence and explore if LLMs can approximate swarm intelligence effects through artificial swarms.

Method: The study conducted a controlled experiment with 960 manually executed prompts across three proprietary models (GPT-5, Gemini 2.5 Pro, Claude Sonnet 4.5), testing intra-model sampling and inter-model aggregation on eight estimation tasks.

Result: The results show consistent error reduction through intra- and inter-model aggregation, with significant error reductions up to 37 percentage points in MAPE across different aggregation strategies. The study also found positive correlations between relative confidence interval widths and relative estimation errors.

Conclusion: The study concludes that LLMs possess metacognitive awareness when assessing uncertainty and provides actionable insights for deploying LLM swarms in organizational decision-making.
...
### [244] [Xiaomi-GUI-0 Technical Report](https://arxiv.org/abs/2606.31410)
...
TL;DR: The paper proposes Xiaomi-GUI-0, a multimodal GUI agent trained and evaluated in real mobile environments to improve execution stability and usability.


<details>
  <summary>Details</summary>
Motivation: Existing GUI agents are limited by being trained and evaluated on offline trajectories and simulated environments, which differ from real-world applications.

Method: The authors propose a real-device-dominant hybrid infrastructure, collecting multi-source training data, and training the model through a progressive three-stage pipeline.

Result: Xiaomi-GUI-0 achieves 72.0% success on RealMobile and 78.9% on AndroidWorld, with improved execution stability and abnormal-state recognition.

Conclusion: The proposed Xiaomi-GUI-0 model effectively bridges the gap between benchmark scores and real-world usability, demonstrating improved performance and stability.
...
### [245] [Learning to Select, Not Relearn: Hard-Routed Mixtures of Reasoning LoRAs](https://arxiv.org/abs/2606.31413)
*Seyed Alireza Molavi,Zhan Su,Yan Hu,Peyman Sheikholharam Mashhadi,Stefan Byttner,Prayag Tiwari*

Main category: cs.AI

TL;DR: Proposed Hard-Routed MoR-LoRA, a two-stage framework for composing frozen LoRA experts through unit-scale hard selection, preserving expert behavior with fewer trainable parameters.


<details>
  <summary>Details</summary>
Motivation: Composing independently trained LoRA adapters into a single large language model for multi-domain adaptation while preserving unit-scale additive updates.

Method: Two-stage framework: (1) train domain-specific LoRA adapters independently using reinforcement learning, (2) freeze experts, distill reasoning traces, and train a lightweight shared router with a small attention LoRA for integration using hard top-1 routing.

Result: Preserves expert behavior with substantially fewer trainable parameters than soft-routing mixture baselines across five benchmarks and multiple model scales.

Conclusion: Hard-Routed MoR-LoRA provides a simple and efficient abstraction for frozen LoRA expert composition, preserving expert behavior with reduced trainable parameters.
...
### [246] [BP-TTA: Balanced and Prototype-Guided Test-Time Adaptation in Dynamic Scenarios](https://arxiv.org/abs/2606.31420)
*Shaoyang Huang,Yashi Zhu,Yichen Yu,Lei Zhang,Zhang Yi,Tao He*

Main category: cs.AI

TL;DR: The paper proposes a novel Test-Time Adaptation method, BP-TTA, which handles class imbalance and continual domain shifts in dynamic test-time streams.


<details>
  <summary>Details</summary>
Motivation: Existing Test-Time Adaptation methods fail to account for class imbalance in dynamic scenarios, and real-world test-time streams often experience both class imbalance and continual domain shifts.

Method: BP-TTA combines batch-balanced sampling with prototype-guided adaptation to mitigate bias toward dominant classes and stabilize online updates.

Result: BP-TTA consistently outperforms state-of-the-art TTA methods in dynamic test-time streaming settings.

Conclusion: The proposed BP-TTA method effectively addresses the challenges of class imbalance and continual domain shifts in dynamic test-time streams, demonstrating improved reliability and stability.
...
### [247] [Ask the World Before Acting: Budgeted Environment Probing for World-Model Calibration](https://arxiv.org/abs/2606.31422)
*Xinyuan Song,Zekun Cai*

Main category: cs.AI

TL;DR: The paper introduces a method for language agents to repair their internal world model by querying the environment, and shows that this approach can reduce errors in the agent's model.


<details>
  <summary>Details</summary>
Motivation: Long-horizon language agents need to maintain a private model of the world, and errors in this model can lead to failures. The authors propose a direct repair mechanism to update the agent's world model.

Method: The authors introduce a budgeted probing operator, called \method, which allows the agent to query the environment about one belief field and update its world model.

Result: The results show that using environment evidence during planning can reduce terminal world-model error, especially when the probe policy is structured according to the task.

Conclusion: The paper concludes that environment interaction can be used as a scarce calibration resource to improve the accuracy of an agent's world model, and that a structured approach to probing can be effective.
...
### [248] [CDR-Bench: Evaluating Faithful Execution of Compositional, Order-Sensitive Data Refinement Recipes](https://arxiv.org/abs/2606.31435)
*Yuchen Huang,Xiang Li,Zhenqing Ling,Sijia Li,Qianli Shen,Daoyuan Chen,Yi R. Fung,Yaliang Li*

Main category: cs.AI

TL;DR: The paper introduces CDR-Bench, a benchmark to evaluate LLMs' ability to execute compositional data refinement recipes, revealing that current LLMs struggle with compositional and order-sensitive settings.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks do not adequately assess LLMs' ability to execute multi-step data refinement recipes that require composition and specific execution order of processing operators.

Method: The authors introduce CDR-Bench, a comprehensive benchmark with 3,462 tasks across four real-world domains and 29 operators, evaluating LLMs in atomic, order-agnostic, and order-sensitive settings.

Result: Experiments on 10+ state-of-the-art LLMs show consistent failure patterns, with performance degrading sharply in compositional settings and order-sensitive recipe success collapsing.

Conclusion: Current LLMs lack the procedural faithfulness required for reliable compositional data refinement, highlighting the need for improvement in this area.
...
### [249] [Who Determines the Meaning of an Emotion? Affective Sovereignty as an Epistemic Consequence of Measurement Limits](https://arxiv.org/abs/2606.31442)
*Keito Inoshita*

Main category: cs.AI

TL;DR: The paper explores the concept of emotion-sensing AI and its implications on individual emotional experiences, highlighting the limitations of AI in accurately determining the meaning of one's emotions.


<details>
  <summary>Details</summary>
Motivation: The rapid integration of emotion-sensing AI in various aspects of life raises questions about who has the authority to determine the meaning of one's emotions, and the study aims to address this question from an epistemological perspective.

Method: The study defines a 'meaning distribution' and decomposes its uncertainty into reducible and irreducible components, demonstrating the limitations of AI in estimating individual emotions.

Result: The study finds that while emotion AI can assign high-confidence labels and discriminate real differences at an aggregate level, it cannot accurately estimate individual emotions due to an 'epistemic gap', leading to the conclusion that the final interpretive authority over one's emotions should rest with the individual.

Conclusion: The study proposes the norm of 'affective sovereignty', which prioritizes the individual's interpretive authority over their emotions, and suggests that the design, evaluation, and regulation of emotion AI should focus on allocating interpretive authority rather than maximizing accuracy.
...
### [250] [CSTrader: A Testbed for Language-Grounded Trading in a Community-Driven Virtual Asset Market](https://arxiv.org/abs/2606.31461)
*Yao Shi,Kingfung Luo,Nan Tang,Yuyu Luo*

Main category: cs.AI

TL;DR: A multi-agent framework called CSTrader was developed to trade CS2 weapon skins using large language models, outperforming traditional methods and achieving up to 7.58% cumulative return.


<details>
  <summary>Details</summary>
Motivation: The authors were motivated to study how large language models can be used to turn unstructured text into trading actions in niche asset markets, such as CS2 weapon skins, which are difficult to model using traditional quantitative methods.

Method: The authors developed CSTrader, a multi-agent framework that integrates heterogeneous signals from various sources and uses specialized agents for technical analysis, liquidity, events, and sentiment analysis to produce buy, sell, or hold decisions.

Result: CStrader outperformed a falling market index (-15.62%) and simple single-prompt LLM baselines, achieving up to a 7.58% cumulative return with controlled risk.

Conclusion: The study suggests that niche, language-driven markets are a useful benchmark for future language-to-action research, and that liquidity, reversed sentiment, and transaction friction agents are crucial for turning noisy language signals into stable profits.
...
### [251] [CLOUDADV: Decision-Aligned Instance Sizing with Zero-Shot Foundation Models under Drift](https://arxiv.org/abs/2606.31470)
*Jack Bell,Giacomo Carfi,Gerlando Gramaglia,Andrea Simioni,Daniele Fontani,Vincenzo Lomonaco*

Main category: cs.AI

TL;DR: CLOUDADV is an advisory system for optimizing cloud instance sizing, reducing costs and operational inefficiencies by combining forecasting and recommendation generation.


<details>
  <summary>Details</summary>
Motivation: Cloud virtual machines are often overprovisioned, leading to unnecessary costs and operational inefficiencies.

Method: CLOUDADV uses zero-shot time-series forecasting and bounded recommendation generation, leveraging a higher-capacity LLM offline and a smaller production model for deployment.

Result: The system achieved significant cost savings (52.9%) and low exceedance rates (1.5%) in a case study of seven production VMs.

Conclusion: The results suggest that zero-shot foundation models can support decision-aligned provisioning in cloud environments while reducing operational burdens.
...
### [252] [One Reflection Is Not Enough: Self-Correcting Autonomous Research via Multi-Hypothesis Failure Attribution](https://arxiv.org/abs/2606.31478)
*Jie Ma,Binfei Chu,Jie Gao,Jinlu Zhang,Yiwei Ma,Yi Tan,Jiayi Ji,Xiaoshuai Sun,Rongrong Ji*

Main category: cs.AI

TL;DR: The paper proposes SAGE, an autonomous research agent that improves failure recovery in experiments through a structured causal diagnosis approach called Multi-Hypothesis Failure Attribution (MHFA), resulting in more reliable and higher-quality scientific artifacts.


<details>
  <summary>Details</summary>
Motivation: The prevailing paradigm for failure recovery in autonomous research agents is brittle and often leads to localized trial-and-error or hard pivots that discard useful context, highlighting the need for a more structured approach.

Method: The authors propose SAGE, which uses MHFA to analyze dynamic trajectory features, generate multiple evidence-grounded explanations for a failure, and deterministically route the verified root cause to the correct intervention level.

Result: On a 12-topic, 5-domain benchmark, SAGE increases metrics-bearing outputs from 42% to 92% over a reflection baseline, improves artifact quality from 5.00 to 6.75/10, and blindly outscores AI-Scientist-v2 (52.0 vs. 48.2).

Conclusion: The proposed SAGE approach significantly outperforms monolithic reflection paradigms, establishing a highly trustworthy foundation for future autonomous research by coupling structured recovery with explicit grounding constraints.
...
### [253] [Surprise as a Signal for Plasticity and Metacognition](https://arxiv.org/abs/2606.31495)
*Louis Mouchon*

Main category: cs.AI

TL;DR: The paper presents two systems that utilize a prediction-error signal to serve as a gate on plasticity and as a substrate for metacognition, demonstrating improved performance in continual learning and vision-language models.


<details>
  <summary>Details</summary>
Motivation: The authors aim to explore the use of a prediction-error signal as a gate on plasticity and as a substrate for metacognition in two different settings.

Method: The authors propose two systems: one that uses a non-parametric episodic memory with a frozen encoder and a prediction-error signal to modulate the writing of new concepts, and another that uses the same surprise signal to modulate the behavior of a vision-language model in a shared text-image space.

Result: The results show that the proposed systems achieve improved performance in continual learning, with 17.7 and 51.3 points of retention on the oldest classes for DINOv2 and I-JEPA respectively, and 91.6% on 5-way 1-shot mini-ImageNet. The second system also demonstrates the ability to separate known from novel concepts with an AUROC of 0.966.

Conclusion: The authors conclude that the proposed systems demonstrate the potential of using a prediction-error signal as a gate on plasticity and as a substrate for metacognition, with applications in continual learning and vision-language models.
...
### [254] [Design and Implementation of Agentic Orchestrations and Orchestration of Agents](https://arxiv.org/abs/2606.31518)
*Stefanie Rinderle-Ma,Juergen Mangler,Johannes Loebbecke,Dominik Voigt,Nataliia Klievtsova,Matthias Ehrendorfer*

Main category: cs.AI

TL;DR: The paper proposes a classification framework for agentic business process management, focusing on balancing AI agent autonomy with robustness, and provides metrics for assessing realization properties.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the challenge of balancing the autonomy of AI agents with robustness, tractability, and traceability in business process management.

Method: The paper provides a classification framework for agentic orchestration options, qualitative decision criteria for different scenarios, and metrics for quantitative assessment of realization properties.

Result: The paper presents a framework for designing and implementing agentic orchestrations, along with metrics for assessing their properties, demonstrated through a predictive light sensing scenario.

Conclusion: The paper aims to provide a comprehensive approach for the design and implementation of agentic business process management, enabling the balancing of AI agent autonomy with robustness and tractability.
...
### [255] [A time-series classification framework for individual-level absenteeism prediction under severe class imbalance](https://arxiv.org/abs/2606.31532)
*Kwong Ho Li,Matthew Roughan,Wathsala Karunarathne*

Main category: cs.AI

TL;DR: The paper proposes a Time Series Classification (TSC) framework for proactive prediction of staff absenteeism, addressing the limitations of existing regression and classification approaches. It evaluates the performance of different loss functions and deep learning architectures on a simulated dataset.


<details>
  <summary>Details</summary>
Motivation: Staff absenteeism poses significant operational costs in high-demand work environments, and reliable individual-level absence prediction is crucial for proactive workforce planning.

Method: The authors propose a TSC framework that separates historical attendance sequences from future absence labels. They evaluate Binary Focal Loss (BFL) and Geometric Mean (G-Mean) loss under severe class imbalance and assess the performance of three deep learning architectures: Long Short-Term Memory (LSTM), Convolutional Neural Network (CNN), and the hybrid LSTM-Fully Convolutional Network (LSTM-FCN).

Result: The experiments show that performance is mainly governed by the balanced weight α, with BFL achieving specificity 0.813 and balanced accuracy 0.888, comparable to G-Mean. The LSTM-FCN delivers strong precision and specificity, with stable performance obtained using batch sizes >= 64 and window sizes between 40-80 days, yielding a balanced accuracy of approximately 80% on held-out test data.

Conclusion: The proposed TSC framework offers a proactive approach to predicting staff absenteeism, and the LSTM-FCN architecture demonstrates strong performance in this context. The findings provide insights into the effects of class imbalance and the importance of careful parameter calibration.
...
### [256] [Modality-Driven Search with Holistic Trace Judging for ARC-AGI-2](https://arxiv.org/abs/2606.31543)
*Johan Land*

Main category: cs.AI

TL;DR: A solver for the ARC-AGI-2 benchmark achieves high scores by generating diverse candidates across text, image, and code channels and using context-preserving holistic judging.


<details>
  <summary>Details</summary>
Motivation: The central challenge in abstract reasoning tasks is making selection among candidates, as large language models can produce fluent but wrong reasoning traces.

Method: The solver uses two principles: treating reasoning modalities as search operators to generate diverse candidates, and context-preserving holistic judging to compare candidates.

Result: The solver achieves 72.9% on the ARC Prize semi-private evaluation set and 76.1% on the public evaluation set, outperforming frontier models like GPT-5.2 Pro and Gemini 3 Pro.

Conclusion: The proposed solver achieves state-of-the-art performance on the ARC-AGI-2 benchmark, and the release of the full source code and negative results provides valuable insights for future research.
...
### [257] [ACE: Pluggable Adaptive Context Elasticizer across Agents](https://arxiv.org/abs/2606.31564)
*Ning Liao,Zihao Long,Xiaoxing Wang,Xue Yang,Yaoming Wang,Ziyuan Zhuang,Xunliang Cai,Rongxiang Weng,Junchi Yan*

Main category: cs.AI

TL;DR: The paper proposes Adaptive Context Elasticizer (ACE), a module that dynamically manages context for large language model-based agents, improving their performance on complex tasks.


<details>
  <summary>Details</summary>
Motivation: The complexity of tasks for large language model-based agents is increasing, leading to longer trajectory lengths that exceed fixed context windows, and existing context management techniques have limitations.

Method: The authors propose ACE, a plug-and-play module with a lossless message maintenance layer and a context orchestration layer that adaptively assigns an elastic type to each historical step.

Result: ACE consistently outperforms truncation and summarization baselines and brings performance gains across four diverse agent frameworks.

Conclusion: The proposed ACE module effectively addresses the limitations of existing context management techniques, providing a more flexible and reversible approach to context management for large language model-based agents.
...
### [258] [Which Tokens Matter? Adaptive Token Selection for RLVR with the Relative Surprisal Index](https://arxiv.org/abs/2606.31575)
*Outongyi Lv,Yanzhao Zheng,Yuanwei Zhang,Zhenghao Huang,Xingjun Wang,Baohua Dong,Hangcheng Zhu,Yingda Chen*

Main category: cs.AI

TL;DR: The paper introduces a new metric, Relative Surprisal Index (RSI), to evaluate token selection in Reinforcement Learning with Verifiable Rewards (RLVR) for Large Language Models (LLMs).


<details>
  <summary>Details</summary>
Motivation: The motivation is to resolve the tension between different paradigms in RLVR, which prioritize either high-entropy token positions or low-probability tokens, by introducing a principled, information-theoretic metric.

Method: The method proposed is RSI Selection (RSI-S), an entropy-adaptive token filtering method that retains tokens within a stable RSI interval.

Result: The result shows that RSI-S achieves higher avg@32 accuracy across different model scales on AIME and AMC benchmarks, improving avg@32 accuracy by 2-3 percentage points over GRPO.

Conclusion: The conclusion is that RSI offers a promising perspective for RLVR improvement, and RSI-S successfully reconciles previous contradictory paradigms and filters out redundant and unstable tokens.
...
### [259] [Scientific Explanations in Health Sciences: Causality, Trust, and Epistemic Adequacy](https://arxiv.org/abs/2606.31616)
*Martina Mattioli,Marcello Pelillo*

Main category: cs.AI

TL;DR: The paper critically reviews the concept of explainability in medical AI, integrating insights from philosophy of science to develop principles for designing explainable AI systems that meet the epistemic and practical requirements of clinical decision-making.


<details>
  <summary>Details</summary>
Motivation: The decision-making processes of many Machine Learning models in medical AI remain opaque, and there is a need for explainability, particularly in high-stakes contexts. However, debates on what constitutes an adequate medical explanation remain unsettled.

Method: The paper conducts a critical review at the intersection of philosophy of science and explainable AI (XAI), examining prevailing accounts of what counts as an explanation in the health sciences and assessing their adequacy for informing XAI in medicine.

Result: The paper identifies three central axes of analysis: the role of causality in medical reasoning, the epistemic and relational dimensions of medical trust, and the criteria of explanatory adequacy as shaped by the pragmatic needs of diverse stakeholders.

Conclusion: The paper outlines principles for designing XAI systems that offer explanations that are not only epistemically robust but also aligned with the epistemic and practical requirements of clinical decision-making, shaping ongoing debates in medical XAI toward underexplored conceptual foundations.
...
### [260] [Think in English, Answer in Korean: Efficient Adaptation of Multilingual Tool-Using Agents](https://arxiv.org/abs/2606.31648)
*Utsav Garg,Sungjin Hong,Jason Jung,Justin Lee,Shaan Desai,Joon Hee Kim,Anirudh Shrinivason,Edmond Wen,Susie Park*

Main category: cs.AI

TL;DR: LuckyStar 111B is a hybrid reasoning model developed for Korean-English enterprise agents, achieving improved performance in mathematical reasoning, function calling, and NL2SQL while preserving general language understanding.


<details>
  <summary>Details</summary>
Motivation: To develop a practical and efficient model for Korean-English enterprise agents under memory and serving constraints, adapting post-trained multilingual models to verifiable agentic workflows.

Method: The model was developed through a collaboration between Cohere and LG CNS, utilizing Cohere's Command A model, preamble conditioning, and various scaling techniques including multilingual supervised fine-tuning, reinforcement learning, and 4-bit quantization.

Result: The adapted model shows improved performance in mathematical reasoning, function calling, and agentic NL2SQL, while maintaining general Korean and English instruction-following quality.

Conclusion: The study provides a practical recipe and failure-mode analysis for adapting post-trained multilingual models to verifiable agentic workflows under memory-constrained deployment, demonstrating the effectiveness of LuckyStar 111B.
...
### [261] [FARS: A Fully Automated Research System Deployed at Scale](https://arxiv.org/abs/2606.31651)
*Qiong Tang,Xiangkun Hu,Xiangyang Liu,Yiran Chen,Yunfan Shao*

Main category: cs.AI

TL;DR: FARS, a fully automated AI-for-AI research system, generates and advances research projects autonomously, producing 166 complete papers on AI/ML topics.


<details>
  <summary>Details</summary>
Motivation: The motivation is to develop a fully automated research system that can operate across various research topics at scale, beyond selected examples or pre-defined tasks.

Method: FARS uses stage-specific agents coordinated through a shared workspace, autonomously generating and advancing projects through ideation, planning, experimentation, and writing.

Result: FARS produced 166 complete research papers on 67 AI/ML topics, with reviews indicating it can produce review-worthy and occasionally strong research artifacts, but also exposing recurring failure modes.

Conclusion: FARS demonstrates the potential for fully automated AI-for-AI research systems to produce high-quality research at scale, but also highlights areas for improvement.
...
### [262] [Arena-T2I Hard: Benchmarking and Improving Faithfulness with Dependency-Aware Checklist](https://arxiv.org/abs/2606.31711)
*Yuanhao Ban,Tong Xie,Sohyun An,Yunqi Hong,Evan Frick,I-Hung Hsu,Wei-Lin Chiang,Ion Stoica,Cho-Jui Hsieh*

Main category: cs.AI

TL;DR: The paper introduces Arena-T2I Hard, a benchmark for evaluating the faithfulness of text-to-image models, and proposes a new reward function that improves faithfulness without sacrificing aesthetics.


<details>
  <summary>Details</summary>
Motivation: Existing faithfulness benchmarks for text-to-image models are limited, as they rely on simple instructions and do not capture the complexity of real-world prompts. The authors aim to create a more comprehensive benchmark and improve the faithfulness of text-to-image models.

Method: The authors introduce Arena-T2I Hard, a 310-prompt benchmark with 30 decomposed yes/no constraints per prompt, and propose a dependency-aware checklist reward that decomposes each prompt into a DAG of yes/no questions. They also combine this reward with a Bradley-Terry aesthetic reward using group-decoupled normalization.

Result: The strongest closed-source system evaluated reaches 0.855 on the Arena-T2I Hard benchmark, with a 33 pp performance gap across 11 systems. The proposed reward function achieves a better faithfulness-aesthetics trade-off on SD3.5-Medium and FLUX.1-dev under MMRB2 pairwise comparisons.

Conclusion: The paper concludes that existing faithfulness benchmarks are insufficient for evaluating text-to-image models, and that the proposed Arena-T2I Hard benchmark and reward function can improve the faithfulness of these models without sacrificing aesthetics.
...
### [263] [A Self-Evolving Agentic System for Automated Generation and Execution of Biological Protocols](https://arxiv.org/abs/2606.31763)
...
TL;DR: Developed ProtoPilot, a self-evolving system for autonomous wet-lab experimentation, achieving high success rates in protocol-to-code conversion and experimental execution.


<details>
  <summary>Details</summary>
Motivation: Autonomous wet-lab experimentation requires aligning biological intent, procedures, device constraints, and experimental feedback, which is a complex task.

Method: Developed ProtoPilot, a multi-agent system with layer-wise verifiability, multi-agent orchestration, and a runtime-updated skill library, and an expert-grounded benchmark and evaluation framework.

Result: ProtoPilot achieved a Top@3 expert-preference rate of 90.2%, a protocol-to-code gate pass rate of 89.5%, and an Opentrons pass rate of 88.24%, outperforming OpenTrons-AI.

Conclusion: ProtoPilot can convert protocol and code generation into validated execution and feedback-guided revision, establishing a verifiable route to autonomous experimentation.
...
### [264] [Evo-PI: Aligning Medical Reasoning via Evolving Principle-Guided Supervision](https://arxiv.org/abs/2606.31800)
*Xianda Zheng,Huan Gao,Meng-Fen Chiang,Michael Witbrock,Kaiqi Zhao,Shangyang Li*

Main category: cs.AI

TL;DR: A new learning framework called Evo-PI is proposed to improve the reasoning capabilities of large multimodal language models by using evolving principle-guided supervision.


<details>
  <summary>Details</summary>
Motivation: The reasoning capabilities of large multimodal language models are constrained by static supervision, leading to brittle generalization and performance saturation in complex decision-making tasks.

Method: Evo-PI treats reasoning principles as explicit, language-based supervision signals that can be generated, evaluated, and iteratively evolved, enabling a co-evolutionary loop between principles and model behaviors.

Result: Evo-PI consistently improves reasoning accuracy across eight benchmarks and multiple model backbones, achieving gains of up to 24.6%.

Conclusion: Evolving principle-guided supervision offers a scalable and general paradigm for training expert-aligned reasoning in multimodal language models.
...
### [265] [RAISE: LLM-based Automated Heuristic Design with Robust Adversary Instance Search](https://arxiv.org/abs/2606.31801)
*Fei Liu,Alessio Figalli,Patrick Owen,Nicola Serra*

Main category: cs.AI

TL;DR: The paper proposes a new framework, RAISE, to improve the robustness of Automated Heuristic Design (AHD) with Large Language Models (LLMs) under real-world distributional shifts.


<details>
  <summary>Details</summary>
Motivation: Existing LLM-based AHD methods may fail catastrophically when deployed under real-world distributional shifts because they optimize heuristics for a fixed training instance set.

Method: The proposed RAISE framework integrates constrained worst-case instance search within a principled neighborhood of the training distribution into the LLM-based evolutionary search loop.

Result: Comprehensive experiments demonstrate that existing LLM-based AHD methods degrade by up to 19 times under distribution shift, while RAISE consistently maintains strong performance across all tested distributions and problem scales.

Conclusion: The RAISE framework provides a robust solution for AHD under distributional shifts, outperforming existing LLM-based methods in maintaining strong performance across various distributions and problem scales.
...
### [266] [Large Databases Need Small, Open-Weight Language Models](https://arxiv.org/abs/2606.31808)
*Parker Glenn,Alfy Samuel*

Main category: cs.AI

TL;DR: Open-weight language models running locally can match or exceed the accuracy of closed-source counterparts at lower latency and cost.


<details>
  <summary>Details</summary>
Motivation: The high cost of using proprietary language model APIs for large databases, which can exceed $10,000 for a single set of experiments.

Method: Using quantized, open-weight models running locally on 16GB of VRAM and integrating them into the BlendSQL v0.1.0 framework.

Result: A 390x reduction in overall costs and 3.8x reduction in latency compared to a proprietary LM API.

Conclusion: Challenging the assumption that closed-source LM APIs are necessary for effective LM-database integration, open-weight models can be a cost-effective alternative.
...
### [267] [Creating Intelligence: A Computational Foundation for AGI](https://arxiv.org/abs/2606.31819)
*Peter Overmann*

Main category: cs.AI

TL;DR: A new computational theory of mind is introduced, leveraging set theory and hyperdimensional computing with sparse binary data, unifying associative memory and learning through topological plasticity.


<details>
  <summary>Details</summary>
Motivation: The motivation is to create a more biologically plausible and energy-efficient computational model of mind, diverging from traditional neural networks that rely on continuous weights and matrix multiplication.

Method: The method involves representing information as discrete sets using sparse binary data, and learning through topological plasticity rather than adjusting scalar weights, with a core algorithm based on subset pattern matching and nearest-neighbor search.

Result: The result is a framework that naturally emerges with associative memory, unifies auto-associative and hetero-associative learning, and operates with constant-time complexity, suggesting a fundamental engine of cognition.

Conclusion: The conclusion is that this framework not only provides a new route toward synthetic intelligence with human-level energy efficiency but also proposes that the cerebellum and neocortex might implement variants of this algorithm.
...
### [268] [Adaptive Cluster-First Route-Second Decomposition for Industrial-Scale Vehicle Routing](https://arxiv.org/abs/2606.31820)
*Oguzhan Karaahmetoglu,Hyong Kim*

Main category: cs.AI

TL;DR: The paper proposes an adaptive cluster-first route-second system for large-scale capacitated vehicle routing problems using a large language model as a high-level decision maker.


<details>
  <summary>Details</summary>
Motivation: Existing methods for solving large-scale capacitated vehicle routing problems perform inconsistently across different instances. The authors aim to develop an adaptive approach that can handle varying spatial, demand, and operational characteristics.

Method: The proposed system uses a large language model as a high-level decision maker to iteratively decompose the problem, applying clustering, balancing, and refinement operators as needed. It jointly partitions customers and vehicles, enabling capacity-aware clustering.

Result: The approach shows competitive performance on benchmark-scale instances and improved scalability and robust routing quality on larger problems with up to 500,000 customers.

Conclusion: The results highlight the potential of adaptive, LLM-guided decision support as a practical approach for industrial-scale vehicle routing and large-scale logistics planning.
...
### [269] [An Agentic AI Framework to Accelerate Scientific Discovery in Plant Phenotyping](https://arxiv.org/abs/2606.31831)
...
TL;DR: An AI framework is developed to accelerate plant phenotyping analysis by automating trait extraction and interpretation, turning a data factory into an interactive discovery platform.


<details>
  <summary>Details</summary>
Motivation: The high-throughput plant phenotyping generates image-derived datasets much faster than scientists can analyze them, making analysis the binding constraint on discovery.

Method: An end-to-end agentic AI framework is presented, consisting of a conversational Co-Scientist Agent and a headless Compute Agent, which work together to translate natural-language questions into structured analysis plans and perform Vision Transformer segmentation and trait extraction on a supercomputer.

Result: The framework reduces the analysis process from days to weeks to an interactive loop where agents reason over results, recommend next analyses, and respond to follow-up questions in seconds.

Conclusion: The developed AI framework enables scientists to partner with AI agents to accelerate time to insight, making the analysis process more efficient and interactive.
...
### [270] [Harnessing Textual Refusal Directions for Multimodal Safety](https://arxiv.org/abs/2606.31876)
*Moreno D'Incà,Massimiliano Mancini,Nicu Sebe*

Main category: cs.AI

TL;DR: The paper proposes Modality-Agnostic Refusal Steering (MARS), a training-free approach to improve safety in Multimodal Large Language Models (MLLMs) by leveraging textual refusal directions.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve safety in MLLMs without requiring unsafe multimodal data, which is harder to collect than unimodal data.

Method: The method involves extracting textual refusal directions from the LLM backbone and investigating their generalization across modalities (image, video). MARS corrects modality misalignment via activation re-centering, adaptively scales steering strength, and selects the optimal intervention layer.

Result: The results show that MARS achieves consistent safety gains while preserving utility across five SOTA MLLMs, evaluated on safety, utility, and video jailbreak benchmarks.

Conclusion: The conclusion is that safety-relevant structure is shared across modalities and textual refusal directions are a powerful foundation for multimodal alignment.
...
### [271] [TreeAgent: A Generalizable Multi-Agent Framework for Automated Bias Labeling in Forestry via Compiled Expert Rules and Vision-Language Models](https://arxiv.org/abs/2606.31976)
*Shiyi Chen,Nicholas Saban,Collin Hargreaves,Huiqi Wang*

Main category: cs.AI

TL;DR: A multi-agent system combining expert decision trees with Vision-Language Models (VLMs) is proposed to reduce expert labeling effort in tasks like tree height bias classification, achieving comparable results to supervised ML baselines at lower annotation cost.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the challenges of expert annotation in ML tasks, which is slow, inconsistent, and a major bottleneck for scaling tasks like tree height bias classification in forestry remote sensing.

Method: The authors propose a Decoupled Declarative Decision (D3) Framework, a multi-agent system (MAS) that combines expert decision trees with Vision-Language Models (VLMs) to perform localized semantic perception and mitigate VLM stochasticity through multi-agent voting.

Result: The proposed framework outperforms supervised ML baselines on a tree bias classification testbed and reduces the amount of expert labeling effort required, suggesting that it can reproduce expert-defined labeling procedures at substantially lower annotation cost while maintaining interpretability.

Conclusion: The proposed framework has the potential to significantly reduce the cost and effort required for expert annotation in ML tasks while maintaining accuracy and interpretability.
...
### [272] [Self-Study Reconsidered: The Hidden Fragility of Learning from Self-Generated QA](https://arxiv.org/abs/2606.32002)
*Ekaterina Alimaskina,Denis Shveykin,Gleb Molodtsov,Igor Shalygin,Alexey Kadeishvili,Aleksandr Beznosikov*

Main category: cs.AI

TL;DR: The paper examines the fragility of synthetic question-answer supervision in language models, highlighting biases in question generation and answering, and proposes mitigation strategies.


<details>
  <summary>Details</summary>
Motivation: The increasing reliance on synthetic question-answer (QA) supervision for training language models, and the need to understand its implications on model performance and potential biases.

Method: The authors analyze the generation step of synthetic QA supervision, investigating how models select evidence and answer questions, and identifying failure modes.

Result: The study reveals that generators concentrate on salient spans, are biased by local presentation, and can be hijacked by artifacts; moreover, models tend to comply with instruction-like passages, especially under task conflict.

Conclusion: The generation step of synthetic QA supervision is not neutral and can introduce biases; however, these biases can be reduced by tying questions to fixed targets and filtering instruction-like spans.
...
### [273] [PolicyGuard: From Organizational Policies to Neuro-SymbolicCompliance Review Engines](https://arxiv.org/abs/2606.32004)
*Sameer Malik,Ayush Singh,Amar Prakash Azad*

Main category: cs.AI

TL;DR: PolicyGuard is a neuro-symbolic framework for policy-grounded document compliance review, enabling explicit, maintainable, and testable compliance decisions.


<details>
  <summary>Details</summary>
Motivation: The need for explicit and inspectable compliance decisions in policy-grounded document review, as end-to-end prompting in large language models makes it difficult to inspect, update, and test applied policy logic.

Method: PolicyGuard converts organizational policy guidance into executable review engines with typed relational logic rules and atom-level extraction questions, using LLMs for local document interpretation and a symbolic evaluator for compliance evaluation.

Result: PolicyGuard makes document review more explicit, maintainable, and systematically testable, as demonstrated in company-specific NDA compliance review.

Conclusion: PolicyGuard provides a neuro-symbolic framework for policy-grounded document compliance review, improving the transparency, maintainability, and testability of compliance decisions.
...
### [274] [AxDafny: Agentic Verified Code Generation in Dafny](https://arxiv.org/abs/2606.32007)
*Benjamin Breen,Austin Letson,Borja Requena Pozo,Leopoldo Sarra*

Main category: cs.AI

TL;DR: The paper presents AxDafny, a verifier-guided repair framework for generating executable code and proof artifacts in Dafny, achieving high verification success rates.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve agentic code generation in Dafny, specifically in generating both executable code and proof artifacts for verification.

Method: The method involves using a verifier-guided repair framework, AxDafny, which iteratively generates implementations, invariants, assertions, and termination arguments.

Result: AxDafny achieves 92.7% verification success on DafnyBench and substantially improves verification success over baseline GPT-5.5 performance on LCB-Pro-Dafny.

Conclusion: The paper concludes that AxDafny is effective in generating high-quality code and proof artifacts in Dafny, and that verification success and runtime test performance measure different aspects of generated code.
