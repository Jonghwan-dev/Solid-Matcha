...
Motivation: Existing 3DGS assets often suffer from quality issues such as missing details and texture noise, and previous attempts to improve them using 2D image processing are limited by multi-view inconsistencies and high computational costs.
...
Result: AnchorSplat achieves state-of-the-art results on the 3DGS-SR dataset, with a throughput up to $10^5$ times faster than optimization methods, and exhibits robust zero-shot generalization across diverse data distributions.

Conclusion: AnchorSplat offers a efficient and effective solution for refining 3DGS assets, addressing the limitations of previous methods and demonstrating promising results in enhancing the quality of 3D representations.
...
TL;DR: The paper proposes a framework called Concept-Informed Prompts Guided Presentation Attack Detection (CPG-PAD) to improve the generalization ability of Presentation Attack Detection (PAD) models across unseen domains.


<details>
  <summary>Details</summary>
Motivation: Existing PAD models struggle to generalize across unseen domains due to variations in sensors, lighting, and attack materials, and recent Vision-Language Models (VLMs) have limited applications in PAD due to learned prompts that fail to align with fine-grained attack-relevant visual semantics.

Method: The proposed CPG-PAD framework introduces model-level concept guidance into the prompt learning process through a Visual Concept-driven Enhancement (VCE) module and a Prompt-based Concept Injection (PCI) mechanism.

Result: The CPG-PAD framework consistently achieves state-of-the-art cross-domain performance under multi-source, limited-source, and single-source settings across nine benchmark datasets.

Conclusion: The proposed CPG-PAD framework effectively captures generalizable and domain-invariant attack cues while suppressing dataset-specific biases, demonstrating strong generalization ability in Presentation Attack Detection.
...
TL;DR: The paper introduces KathaTrace, a protocol for evaluating the semantic coherence of storyboards generated by AI models, and presents KathaBench-25K, a benchmark dataset for assessing the recoverability of story transitions.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for evaluating storyboards focus on visual quality, content faithfulness, and scene coherence, but neglect the critical aspect of semantic coherence, where the meaning of scene transitions may be lost.

Method: The authors propose KathaTrace, a generator-agnostic protocol that evaluates transitions under three evidence conditions (text-only, image-only, and text-plus-image) and filters ambiguous items. They also introduce the Semantic Trajectory Gap (STG) metric to measure the loss of transition meaning during visualization.

Result: The experiments show a substantial STG of 23.5 +/- 1.3 across state-of-the-art generators, indicating a significant loss of semantic coherence. Human validation yields a high Fleiss' kappa of 0.845, demonstrating the reliability of the KathaTrace protocol.

Conclusion: The paper concludes that KathaTrace and KathaBench-25K can help diagnose and mitigate semantic trajectory collapse in storyboards, and that the Semantic Compass probe can improve storyboard selection by leveraging KathaTrace signals for post-generation repair.
...
TL;DR: A novel input-aware extendable expert module is proposed for video-based person re-identification to enhance fine-grained feature exploration.


<details>
  <summary>Details</summary>
Motivation: The need to exploit fine-grained features for discriminating identities with similar appearances in video-based person re-identification.

Method: A novel input-aware extendable expert module with two mechanisms: input-aware expert selection and spatial-temporal selection, along with an extendable scheme for adding new experts.

Result: The method achieves outstanding performance on two large-scale datasets.

Conclusion: The proposed module effectively enhances the ability to explore fine-grained information, leading to improved performance in video-based person re-identification.
...
TL;DR: MapDreamer is a generative model that creates high-definition maps for autonomous driving from aerial images.
...
Method: A generative diffusion model using a variational autoencoder and transformer-based latent diffusion model.

Result: Improved geometric and topological fidelity over non-generative baselines on UrbanLaneGraph dataset.

Conclusion: MapDreamer enables efficient generation of high-definition maps with explicit topology from aerial images.
...
TL;DR: A new benchmark and evaluator, MIBE, is proposed to improve multi-subject personalized image generation by accurately rendering multiple subjects and their interactions, addressing limitations in existing models and metrics.


<details>
  <summary>Details</summary>
Motivation: Current models struggle with multi-subject image generation, often omitting subjects, failing to preserve appearances, or misattributing interactions, and existing metrics are inadequate for evaluating these errors.

Method: A unified framework, MIBE, is introduced, comprising a Multi-subject Interaction Benchmark (MIB) with a Silver Set for scalable metric training and a Gold Set for human evaluation, and a Multi-subject Interaction Evaluator (MIE) trained on the Silver Set.

Result: MIE demonstrates strong performance, achieving 0.922 overall pairwise accuracy against human preference, outperforming baseline metrics, including CLIP and DINO variants.

Conclusion: The proposed MIBE framework provides a comprehensive solution for evaluating and improving multi-subject personalized image generation, enabling more accurate rendering of multiple subjects and their interactions.
...
TL;DR: The paper aims to improve visual object tracking by enhancing target discrimination, adaptation, and geometric reasoning to narrow the gap between machine and human visual perception.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of current visual tracking systems in generalizing to unpredictable events and adapting to changes, which hinders their ability to match human visual perception.

Method: The paper proposes a series of methods to enhance target discrimination, robust adaptation, and geometric reasoning capabilities of tracking models.

Result: No specific results are mentioned in the abstract.

Conclusion: The paper aims to improve the reliability and performance of visual object tracking systems by bridging the gap between machine and human visual perception.
...
Method: The study fine-tunes the YOLOv10 framework using varying proportions of auxiliary annotated data to detect brown howler monkeys in videos from camera traps.
...
TL;DR: The authors propose a Graph-based Concept Bottleneck Model (G-CBM) for intrinsically interpretable image classification, which performs unsupervised concept discovery and represents concepts as nodes in a graph. G-CBM achieves improved performance and selective concept use on several benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing Concept Bottleneck Models (CBMs) rely on predefined concept vocabularies or supervised annotations, lack explicit concept grounding, and summarize concepts with a single image-level score. The authors aim to address these limitations.

Method: The authors propose G-CBM, which uses Non-negative Matrix Factorization (NMF) for unsupervised concept discovery, represents concepts as nodes in a per-image concept-graph, and applies a tunable concept filtering threshold. A Graph Attention Network (GAT) is used for concept-level reasoning.

Result: G-CBM achieves an average relative AUC improvement of 3.7% over a ResNet-50 baseline on several benchmarks, including ImageNet, HAM10000, PH2, and Derm7pt. The model also achieves peak AUC of 0.96 on PH2 with only 2 of 10 concepts and 0.92 on HAM10000 with 3.8 of 9 concepts.

Conclusion: The authors conclude that G-CBM provides an intrinsically interpretable framework for image classification, achieving improved performance and selective concept use. The learned concept ranking faithfully reflects model predictions.
...
TL;DR: A new authentication interface called Sign in the Air to Unlock enables users to authenticate by signing naturally in 3D space, achieving high accuracy and security in immersive environments.


<details>
  <summary>Details</summary>
Motivation: The need for secure, intuitive, and compatible authentication interfaces in immersive technologies such as VR/AR, which traditional methods fail to provide.

Method: An in-air signature interface using a point-voxel Cross-Attention Network (PV-Net) that models 3D trajectories to authenticate users.

Result: PV-Net achieves an Equal Error Rate of 2.5% on DeepAirSig and 76% classification accuracy on ImmAirSig, demonstrating the potential of 3D behavioral interfaces for seamless authentication.

Conclusion: The proposed interface offers a promising alternative for user-centric authentication that merges security with natural interaction in immersive environments.
...
TL;DR: The authors investigate the benefit of future context for egocentric gaze estimation, finding that it improves causal models up to a point, and provide guidance on optimal look-ahead for real-time applications.


<details>
  <summary>Details</summary>
Motivation: The discrepancy between common gaze estimation models that use future frames and the requirement for strictly causal, online prediction in real-world applications.

Method: A controlled framework with a future-aware branch that accesses a tunable look-ahead horizon during training but is discarded at inference.

Result: Future-privileged supervision improves causal gaze prediction, with optimal performance at 1.7-3.3 seconds of future context on EGTEA Gaze+ and 2.7 seconds on Ego4D.
...
TL;DR: The authors propose a cost-aware protocol for evaluating tool-augmented video agents and apply it to Dynamic-SAGE, a framework that synthesizes composite tools for video question answering, showing improved accuracy and efficiency but increased token usage and cost.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitation of fixed tool libraries in agentic VideoQA systems, which leads to recurring procedures being rebuilt from primitives on every question, and to assess the effectiveness of synthesizing composite tools.

Method: The authors propose a cost-aware, paired protocol for auditing tool-augmented video agents, which pairs two complete systems on the same input for each question and reports their net difference across accuracy and cost jointly.

Result: Dynamic-SAGE improves accuracy by 7.5 points (p < 0.001) and reduces reasoning turns and visible tool calls by roughly 28%, but increases token usage by 34% and cost by 26%.

Conclusion: The proposed protocol provides a multi-axis profile of the system's performance, revealing both accuracy-preserving efficiency gains and harmful regressions, and shows where the pipeline-level difference is reliable and where it is not.
...
TL;DR: The authors propose Anti-Prompt, a method to protect images from being misused in text-guided Image-to-Video (I2V) generation by introducing imperceptible perturbations that cause visible inconsistencies in the generated video.


<details>
  <summary>Details</summary>
Motivation: The rapid advancement of Image-to-Video generation technologies poses significant risks to copyright and privacy, as a single image can be animated into a convincing video with text guidance.

Method: Anti-Prompt injects imperceptible perturbations into an image to induce visible inconsistencies and structural failures in text-guided I2V generation by attenuating text-conditioned interactions during denoising and strengthening visual-only pathways.

Result: Experiments on two representative I2V architectures show that Anti-Prompt achieves strong protection performance while improving efficiency and cross-model transferability.

Conclusion: The proposed Anti-Prompt method effectively protects images from misuse in text-guided I2V generation, highlighting a potential solution to mitigate the associated copyright and privacy risks.
...
TL;DR: This paper studies the depth perception of vision-language models (VLMs) by isolating pictorial depth cues and disentangling vision and language influences on model performance.


<details>
  <summary>Details</summary>
Motivation: To understand how VLMs perceive depth and to identify the effects of pictorial depth cues and language influences on their performance.

Method: The authors combine depth-ordering and odd-one-out psychophysical tasks, generating 2D views from simulated and real 3D scenes with controlled pictorial depth cues, and varying referring expression clarity to examine language effects.

Result: The evaluation of 12 open-source and commercial models on the created Odd-One-Out Depth (O3-D) dataset shows under-utilization of depth cues, with depth-ordering accuracies between 47% and 56%, and strong linguistic bias in the answers.

Conclusion: The study suggests that static image data alone may be insufficient for depth understanding, and that neither chain-of-thought (CoT) nor in-context learning (ICL) significantly improves performance.
...
Method: The Hidden-Shot approach extracts implicit visual task-based information, uses a global task-aware textural prompt, and selectively merges implicit information with in-task processing information to enhance one-shot capabilities in new tasks.
...
Conclusion: The Hidden-Shot approach effectively improves the generalization ability of low-level generalist models in zero/few-shot scenarios, providing a cost-effective solution with minimal architectural changes.
...
<details>
  <summary>Details</summary>
Motivation: Infrared small target detection is challenging due to tiny target size, low signal-to-noise ratio, and blurred boundaries in complex scenes. Existing methods have limitations in weak-target discrimination and shape accuracy.

Method: The proposed method, AC-SLSIoU, consists of Logit-Domain Margin Constraint (LDMC), Adaptive Boundary Suppression (ABS), and False-Alarm Focal Loss. It can be seamlessly integrated into existing detectors without extra inference overhead.

Result: The proposed method consistently improves both detection accuracy and shape quality in extensive experiments and cross-backbone evaluations.

Conclusion: The proposed AC-SLSIoU loss function is effective, robust, and generalizable for infrared small target detection, enhancing weak-target discrimination, refining target contours, and suppressing false alarms.
...
TL;DR: The authors introduce a new protocol to evaluate 3D Gaussian Splatting (3DGS) methods, revealing a significant interpolation-extrapolation gap in performance, and provide a benchmark toolkit for standardized evaluation.
...
Method: The authors propose a fair matched-count protocol, which involves training on the same number of images but with different holdout strategies (interpolation or extrapolation). They also perform a diagnostic analysis to understand the results.

Result: The authors find a large, consistent interpolation-extrapolation gap of 3-12dB across three representation families, including non-Gaussian volumetric neural radiance fields (NeRF). This gap is robust to training noise and affects method rankings.

Conclusion: Standard holdout methods are not sufficient to evaluate spatial generalization in 3DGS. The authors' new protocol and benchmark toolkit provide a more comprehensive and standardized way to evaluate these methods.
...
TL;DR: A new method, MVFusion-GS, improves 3D Gaussian Splatting for dynamic scenes by adding motion-aware mechanisms for better deformation and reconstruction.


<details>
  <summary>Details</summary>
Motivation: Existing deformation networks for 3D Gaussian Splatting in dynamic scenes lack explicit motion awareness, leading to inaccurate foreground deformation and background residuals.

Method: MVFusion-GS uses two motion-aware mechanisms: Motion-Variance Guided Refinement for dynamic-static separation and MotionFormer Temporal Attention for modeling local motion dependencies.

Result: State-of-the-art performance in dynamic scene reconstruction and distractor-free reconstruction benchmarks, with improved foreground motion modeling and static background reconstruction.

Conclusion: Explicit motion awareness in deformation networks significantly enhances dynamic scene reconstruction and distractor-free reconstruction.
...
TL;DR: The paper presents VLAFlow, a unified framework for comparing vision-language-action (VLA) training objectives. It evaluates four pre-training paradigms and finds that combining language supervision and future latent alignment achieves the most stable transfer performance across benchmarks.
...
Method: The paper uses a unified flow-matching framework, VLAFlow, to evaluate four pre-training paradigms: action-only modeling, language-supervised co-training, future latent alignment, and their combination. The evaluation is conducted on a heterogeneous robot corpus, OXEMix, with a shared architecture, VLM backbone, action expert, and 14-dimensional action space.

Result: The experiments show that action-only pre-training is sensitive to heterogeneous data, while language supervision helps preserve vision-language generalization, and future latent alignment improves state-transition and action-outcome modeling. The combination of both signals achieves the most stable overall transfer performance across benchmarks.
...
TL;DR: A new method called Multi-THuMBS is proposed for tracking multiple people's 3D human meshes in videos with shot changes, improving upon existing methods that struggle with identity tracking and motion consistency across shots.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the challenge of tracking multi-person 3D human meshes in unconstrained videos with frequent shot changes, which causes existing methods to lose track of human identities and fail in reconstructing temporally coherent trajectories.

Method: The proposed method, Multi-THuMBS, leverages a state-of-the-art 3D scene prior to reconstruct the two boundary frames in a single shared 3D space and registers human meshes within the shared 3D space, maintaining per-person identity and motion consistency across shot changes.

Result: The proposed approach yields significant improvements in 3D human mesh recovery, camera pose estimation, and identity tracking, ensuring high-fidelity motion reconstruction with consistent identity preservation across shots compared to previous state-of-the-art methods.

Conclusion: The paper concludes that Multi-THuMBS effectively addresses the limitation of existing methods in handling multi-person 3D human mesh tracking in videos with shot changes, providing a more robust and accurate solution for real-world applications.
...
TL;DR: Introducing SAGO, a setup-free framework for interactive 3D Gaussian Splatting (3DGS) segmentation, eliminating the need for time-consuming scene setup and enabling real-time object manipulation and scene editing.


<details>
  <summary>Details</summary>
Motivation: Existing methods for interactive 3DGS segmentation require a time-consuming per-scene setup, creating a bottleneck for online applications. The goal is to eliminate this setup stage while keeping segmentation time practical.

Method: SAGO uses virtual drones to reframe 3D segmentation as an online Next-Best-View (NBV) planning task within a Markov process, allowing for setup-free interactive 3DGS segmentation.

Result: SAGO achieves sub-second latency for 3D asset extraction from 3D Gaussians and offers over a 50x speedup compared to previous setup-free 3DGS segmentation frameworks.

Conclusion: SAGO enables real-time interactive 3DGS segmentation without the need for a time-consuming setup stage, opening up possibilities for applications such as object manipulation and scene editing.
...
TL;DR: The paper proposes Decoupled Representation Dynamic Network (DRDN) for class-incremental learning, addressing challenges in preserving task-agnostic shared backbone representations and task-specific discrimination.


<details>
  <summary>Details</summary>
Motivation: Classification supervision alone is insufficient for preserving task-agnostic shared backbone representations over long incremental sequences, leading to cross-task confusion and under-optimized shared representations.

Method: DRDN uses two mechanisms: 1) masked image modeling (MIM) to retain general visual structure in the backbone, and 2) hierarchical task token expansion with a modified attention rule to reduce inter-task interference.

Result: DRDN outperforms strong token-expansion baselines in the from-scratch ViT CIL setting, achieving 77.19% average accuracy on CIFAR100-B0, with improvements of 1.36 points over DKT and 3.53 points over DyTox.

Conclusion: The proposed DRDN effectively addresses challenges in class-incremental learning, providing a more stable and accurate approach for long incremental sequences.
...
TL;DR: COVScene is a pose-free semantic Gaussian framework for 3D scene understanding from sparse images, enabling renderable geometry, open-vocabulary semantics, and occupancy prediction.
...
Method: COVScene couples Gaussian primitives with a dense semantic occupancy field through differentiable volumetric lifting, using a semantic-aware Geometry Transformer and multi-task Gaussian decoding.

Result: COVScene achieves competitive rendering quality, improved open-vocabulary segmentation, and stronger semantic occupancy prediction on ScanNet and ScanNet++ datasets.

Conclusion: COVScene presents a comprehensive framework for 3D scene understanding, addressing the limitations of previous Gaussian methods and demonstrating improved performance in various tasks.
...
TL;DR: The paper proposes MrFlow, a training-free method to accelerate text-to-image diffusion models by up to 25x without compromising image quality.


<details>
  <summary>Details</summary>
Motivation: Existing multi-resolution generation strategies for accelerating text-to-image diffusion models often result in blurring or artifacts. The authors aim to address this limitation.

Method: MrFlow uses a staged low-to-high-resolution pipeline, starting with low-resolution generation, followed by super-resolution using a lightweight GAN-based model, and finally refining details at high resolution.

Result: MrFlow achieves up to 10x end-to-end acceleration while maintaining image quality within a 1% gap relative to the original model. It can be combined with timestep distillation for up to 25x acceleration.

Conclusion: The proposed MrFlow method offers a significant speedup for text-to-image diffusion models without requiring training or compromising image quality, outperforming existing training-free acceleration strategies.
...
TL;DR: The paper proposes an attribute-guided dual-branch framework for ultrasound classification, enhancing diagnostic performance and interpretability by introducing domain-agnostic medical attribute priors.


<details>
  <summary>Details</summary>
Motivation: Current ultrasound image classification methods neglect clinical priors, leading to poor generalization and lack of interpretability, which limits clinical adoption.
...
Conclusion: The proposed framework effectively enhances diagnostic performance and interpretability in ultrasound classification, making it a promising solution for computer-aided diagnosis.
...
TL;DR: The paper proposes a plug-and-play framework for volumetric reconstruction in compressive sensing light-sheet microscopy, incorporating user-specified denoisers and exploiting correlations between adjacent slices.


<details>
  <summary>Details</summary>
Motivation: To recover the underlying volume from highly multiplexed measurements in compressive sensing light-sheet microscopy, where fast volumetric imaging is achieved by encoding multiple axial planes into each camera exposure.

Method: A plug-and-play framework that flexibly incorporates any user-specified denoiser into the reconstruction process, with a slice-based formulation and an axial-coupled model that exploits correlations between adjacent slices.

Result: The proposed framework successfully recovers cellular structures from compressed measurements, and provides practical insights into the comparative performance of commonly used denoisers within the PnP framework under the CS-LSM setup.

Conclusion: The proposed algorithm establishes subsequential convergence under a weakly convex regularization assumption, and experiments on synthetic and real zebrafish-heart data demonstrate its effectiveness.
...
TL;DR: The paper challenges the use of visual guidance in Domain Generalization (DG) and instead proposes using language embedding space as the primary source of domain invariance, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Visual recognition models often fail when deployed in new environments, and Domain Generalization (DG) aims to learn representations that remain invariant to environment-specific variations.

Method: The paper discards visual guidance and uses language embedding space as the primary source of domain invariance, treating it as an information bottleneck that preserves core semantics while suppressing domain-specific variations.

Result: The approach achieves state-of-the-art performance across diverse backbones and provides insights into what makes guidance effective for robust generalization.
...
TL;DR: The authors propose DriveTeach-VLA, a framework that improves Vision-Language-Action models for autonomous driving by teaching them what to see and where to look.
...
Method: The authors propose two main components: Driving-aware Vision Distillation (DVD) and 2D Trajectory-Guided Prompts (2D-TGP), which form a vision-guided learning pipeline.

Result: DriveTeach-VLA achieves state-of-the-art performance on NAVSIM and nuScenes datasets.
...
TL;DR: The paper proposes PanoGaussian, a unified representation for 4D scene synthesis from monocular videos, addressing limitations in handling unseen regions and dynamic content.


<details>
  <summary>Details</summary>
Motivation: Existing 4D scene synthesis methods are constrained by view interpolation and struggle to infer unseen regions. Camera-conditioned video generation methods lack explicit 3D priors and have inconsistencies under large trajectory deviations.

Method: The paper proposes a unified training and inference framework with panoramic trajectory guidance and introduces PanoGaussian, a Panoramic-Gaussian representation that captures dynamic physical priors of the 4D scene.
...
Conclusion: The proposed PanoGaussian representation effectively addresses the limitations in 4D scene synthesis, enabling consistent synthesis even in unseen regions and under large viewpoint variations.
...
TL;DR: The paper proposes TCA-Captioner, a framework for enhancing temporal and cross-modal alignment in audiovisual video captioning, achieving state-of-the-art results.
...
Method: The paper introduces the Observer-Checker-Corrector (OCC) framework and TCA-Captioner, which uses a curated high-density human interaction dataset and a Decoupled Evaluation Protocol.

Result: TCA-Captioner sets a new standard for temporally-coherent and synchronized audiovisual narratives, demonstrating superior performance in audiovisual binding and temporal relational reasoning.

Conclusion: The proposed TCA-Captioner framework effectively addresses the challenges of temporal and cross-modal alignment in audiovisual video captioning, achieving state-of-the-art results.
...
TL;DR: Proposed a novel Nested-UNet architecture for medical image segmentation with enhanced multiscale contextual information capture and upsampling, achieving superior generalization performance.


<details>
  <summary>Details</summary>
Motivation: Existing medical image segmentation methods struggle to capture information at multiple scales and perform upsampling effectively across different datasets, leading to suboptimal generalization capabilities.
...
Conclusion: The proposed architecture effectively captures multiscale contextual information, leading to improved segmentation performance and generalization capabilities in medical image analysis.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for monocular video depth estimation struggle to achieve temporal consistency, geometric accuracy, and generalization simultaneously. Discriminative models have per-frame accuracy but suffer from temporal drift, while generative methods improve consistency and generalization but require extensive training data and lack geometric precision.

Method: The ICDepth framework adapts pre-trained text-to-video diffusion transformers for video depth estimation via In-Context Conditioning (ICC). It proposes SAND-Attention for precise spatial-temporal alignment and SRFM to enhance geometric precision with DINOv2 semantic and resolution priors.
...
Conclusion: The ICDepth framework successfully addresses the challenges of monocular video depth estimation by achieving temporal consistency, geometric accuracy, and generalization across diverse scenarios with high data efficiency, outperforming existing methods.
...
TL;DR: The paper proposes a novel scheduler called SIG for 3D Gaussian Splatting, which synchronizes image supervision with Gaussian frequencies, leading to state-of-the-art performance in large-scale scenes.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the issue of uncontrolled densification and redundant primitives in large-scale scenes with sparsely observed regions, which degrades efficiency and quality.

Method: The method involves reframing the scene reconstruction problem from the perspective of signal structure recovery and proposing a novel scheduler SIG, which regulates the training image resolution and Gaussian densification process based on scene frequency convergence.

Result: The result is state-of-the-art performance in both efficiency and rendering quality in large-scale scenes, with a substantial margin.

Conclusion: The conclusion is that the proposed SIG scheduler enables frequency-consistent, geometry-aware, and floater-free training, achieving superior performance in large-scale scenes.
...
TL;DR: The paper proposes LASER, a post-training framework to address visual forgetting in large vision-language models by regulating visual attention and preventing attention collapse.


<details>
  <summary>Details</summary>
Motivation: Visual forgetting in large vision-language models during long-horizon decoding, caused by early-stage attention decay and attention concentration on task-irrelevant visual tokens.

Method: LASER introduces two rewards: Visual Grounding Reward to maintain attention on salient visual tokens and Sink Suppression Reward to penalize attention on uninformative tokens.

Result: LASER consistently outperforms strong baselines on eight benchmark datasets, validating attention-aware training as an effective remedy for visual forgetting.

Conclusion: The proposed LASER framework effectively addresses visual forgetting by regulating visual attention, leading to improved performance in large vision-language models.
...
TL;DR: A novel framework for reliable instance-level scene understanding, producing consistent 2D instance masks to guide 3D Gaussian Splatting feature fields, enabling stable 3D instance segmentation and effective editing tasks.


<details>
  <summary>Details</summary>
Motivation: Current methods for scene understanding leverage 2D foundation segmentation models, but their 2D-centric design yields fragmented masks and inconsistent predictions across views.

Method: The proposed framework consists of three stages: (1) Multi-Cue Extraction, (2) Multi-Cue-Guided Mask Merging, and (3) Cross-View Mask Matching.

Result: The method significantly improves cross-view consistency and segmentation stability over existing baselines while maintaining high-fidelity photometric reconstruction.

Conclusion: The proposed framework enables stable 3D instance segmentation and effective downstream editing tasks by producing consistent 2D instance masks to guide 3D Gaussian Splatting feature fields.
...
<details>
  <summary>Details</summary>
Motivation: Current visual regression testing methods, such as pixel-level comparison, produce large volumes of false positives, requiring substantial manual review effort. There is a need for a capability that describes UI changes in natural language.
...
Result: The evaluated methods struggle in the Web UI domain due to its layout diversity, dense text, and fine-grained changes, but trained methods show promise in suppressing non-meaningful visual noise.

Conclusion: The proposed WUICC task and WUICC-bench dataset/benchmark provide a solid foundation for future domain-specific research in visual regression testing with natural language descriptions of UI changes.
...
TL;DR: The paper proposes ReQuest, a method for improving long-form video QA by adaptively selecting keyframes based on question intent and model uncertainty.


<details>
  <summary>Details</summary>
Motivation: Long-form video QA remains challenging due to fixed input token budgets and the inefficiency of uniform sampling for evidence localization.

Method: ReQuest, an uncertainty-driven, question-adaptive keyframe selection pipeline, which includes a lightweight question-aware selector, Re-thinking Routing, and uncertainty-guided adaptive non-maximum suppression.

Result: ReQuest improves long-video QA on Video-MME, MLVU, and LongVideoBench with consistent accuracy gains and competitive computational cost, particularly in medium and long video regimes.

Conclusion: ReQuest is a plug-and-play method that enhances long-form video QA without modifying or fine-tuning the underlying multimodal large language model (MLLM).
...
TL;DR: A new framework, InterCMDM, is proposed for generating text-conditioned human interactions. It uses a block-causal latent diffusion approach and achieves state-of-the-art performance on two datasets.


<details>
  <summary>Details</summary>
Motivation: Existing interaction diffusion models have limitations in capturing long-range temporal causality and tightly coupled coordination between partners. Autoregressive alternatives suffer from temporal drift, leading to coordination degradation.

Method: InterCMDM introduces a Dual-Stream Causal Diffusion Transformer with unified dual-stream attention and multi-task attention masks. It uses a block-wise diffusion objective for stable latent rollout over long sequences.

Result: InterCMDM achieves state-of-the-art performance on InterHuman and Inter-X datasets, improving text-motion alignment, realism, and long-horizon continuity.

Conclusion: The proposed InterCMDM framework effectively addresses the challenges of generating text-conditioned human interactions, enabling controllable and realistic interaction generation.
...
TL;DR: A novel dehazing approach, RTE-FM-Dehazer, and a scalable data pipeline are proposed to address limitations in single-image dehazing, leveraging the Radiative Transfer Equation and vision-language models.


<details>
  <summary>Details</summary>
Motivation: Current single-image dehazing methods are limited by their reliance on haze-formation priors and the difficulty of collecting large-scale real hazy/clear pairs.

Method: RTE-FM-Dehazer uses the Radiative Transfer Equation to model haze formation and introduces a diffusion-absorption regularizer, trained on a dataset of 50000 realistic hazy/clear pairs generated by an automated pipeline.
...
Conclusion: The proposed approach and data pipeline improve the performance and practicality of single-image dehazing.
...
TL;DR: MedStreamBench, a new benchmark for time-aware medical video understanding, evaluates AI models on when to answer, defer, or raise alerts in real-time clinical settings.


<details>
  <summary>Details</summary>
Motivation: Current medical video benchmarks focus on answer correctness, not timing, creating a gap between evaluation and deployment requirements.

Method: MedStreamBench integrates 22 datasets, 5,419 QA instances, and 4 temporal settings, evaluating models on responsiveness and stability in single-turn and streaming settings.

Result: Experiments show a substantial performance gap between offline recognition and temporally grounded decision-making, especially in streaming and proactive settings.

Conclusion: MedStreamBench highlights the need for time-aware medical video understanding, providing a new benchmark for evaluating AI models in real-time clinical settings.
...
TL;DR: The authors propose a novel 3D plant phenotyping framework using 3D Foundation Models (3DFMs) to accelerate and streamline the process, achieving high-throughput phenotyping from low-cost image acquisition.


<details>
  <summary>Details</summary>
Motivation: Current 3D plant phenotyping methods are complicated, low-throughput, and costly due to extensive multi-view imaging, fragile 3D reconstruction, and additional costs for phenotypic extraction.

Method: The proposed framework utilizes 3DFMs for feed-forward geometric recovery, geometry-constrained 3D Gaussian Splatting, few-view reconstruction, and 2D-to-3D semantic transfer for organ instance separation.

Result: The framework reduces average reconstruction time from 6.52 minutes to 1.58 seconds while maintaining high reconstruction quality and phenotyping accuracy across 26 plant sequences.

Conclusion: The proposed framework offers a fresh technical route for high-throughput 3D plant phenotyping, enabling fast reconstruction, perception, scale recovery, and phenotypic measurement from low-cost image acquisition.
...
<details>
  <summary>Details</summary>
Motivation: Existing LDCT denoising methods suffer from insufficient anatomical guidance, uncertain frequency-dependent recovery, and uniform reverse-process modeling, which can obscure anatomical boundaries and weaken low-contrast structures.
...
Result: ProSAC-CT improves image fidelity, structural similarity, perceptual quality, and information preservation over representative methods, better preserving boundary-sensitive anatomical details and retaining task-relevant anatomical information.

Conclusion: ProSAC-CT is a promising approach for low-dose CT denoising, offering improved performance and preservation of anatomical details, supporting its practical use.
...
TL;DR: The paper presents DL-VINS-Factory, a unified framework integrating deep learning-based feature extractors with visual-inertial SLAM (VI-SLAM) systems, achieving real-time performance and improved accuracy in various environments.


<details>
  <summary>Details</summary>
Motivation: The practical value of deep-learning features in tightly coupled visual-inertial SLAM (VI-SLAM) remains insufficiently characterized, and the authors aim to investigate their effectiveness.

Method: The authors integrate learned feature extractors (ALIKED, RaCo, SuperPoint, XFeat) with either Lucas-Kanade optical-flow tracking or LightGlue descriptor matching, and benchmark the system across four datasets.

Result: The results show that learned front-ends are viable for real-time embedded VI-SLAM, with improvements in accuracy, such as 5-7% reduction in EuRoC ATE and 12-38% reduction in ATE on other datasets.

Conclusion: The paper concludes that deep learning-based features can be effectively used in VI-SLAM systems, achieving real-time performance and improved accuracy, but may not be universally superior to classical tracking methods.
...
TL;DR: The paper proposes a simple method called ProCal to improve open-vocabulary object detection by enhancing the localization quality of classification scores.


<details>
  <summary>Details</summary>
Motivation: Current open-vocabulary object detection methods struggle with localizing and classifying objects beyond the categories seen during training, particularly with the classification score from VLMs lacking position and scale recognition.

Method: The authors propose Proposal Calibration (ProCal), which computes a proposal prior by combining localization-aware foreground score and background-aware suppression score.

Result: ProCal improves APr by +2.5 on OV-LVIS when applied to CLIPSelf ViT-L/14, and consistently ranks true novel proposals above background and partial novel proposals.

Conclusion: The proposed ProCal method effectively mitigates ranking miscalibration for novel objects by enhancing localization quality, leading to improved performance in open-vocabulary object detection.
...
TL;DR: JointHOI is a single-stage diffusion framework for generating 3D hand-object motion and dynamic contact maps from text, improving physical plausibility and temporal stability.


<details>
  <summary>Details</summary>
Motivation: The challenge of producing physically plausible hand-object interactions in immersive applications and robotics, where small contact errors can cause noticeable artifacts.

Method: JointHOI, a single-stage diffusion framework that jointly generates 3D hand-object motion and dynamic contact maps from text.

Result: Consistent improvements in text adherence and physical plausibility over prior methods on GRAB and ARCTIC datasets.

Conclusion: JointHOI effectively addresses the challenge of generating physically plausible hand-object interactions by jointly modeling motion and contact, leading to improved temporal stability and reduced artifacts.
...
<details>
  <summary>Details</summary>
Motivation: The quality of vision-radar fusion in autonomous driving is challenged by dynamically varying input quality due to occlusion, adverse weather, and channel noise.

Method: The LM-SCIP framework uses a Large Language Model (LLM) as a central reasoning core to fuse local visual streams with quality-varying external radar streams. It includes a Channel-Adaptive Semantic Module (CASM) and a heterogeneous Mixture-of-Experts (H-MoE).

Result: Experiments on nuScenes and VIRAT validate the approach, showing a 40.0% reduction in localization RMSE versus a vision-only baseline on nuScenes and achieving a 0.214m localization RMSE and 0.179m minFDE (k=1) on VIRAT.

Conclusion: The proposed LM-SCIP framework enables a robust vision-dominant fallback at low SNR and synergistic fusion at high SNR, improving the reliability of autonomous driving systems.
...
TL;DR: The authors propose an extended framework, SpaceEra++, for enhancing visual-spatial understanding by addressing limitations in data and spatial reasoning, building on their previous work, SpaceEra.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to overcome the limitations of pre-trained vision-language models (VLMs) in terms of spatial uncertainty and scarcity of 3D spatial understanding data, which hinder their application in tasks like robotic navigation and embodied interaction.

Method: The authors extend their previous framework, SpaceEra, into SpaceEra++, which includes ScenePick, a frame sampling strategy to balance spatial coverage and object semantics, and SpaceAlign, a method to enforce pairwise object constraints using absolute coordinates and relative spatial relations.

Result: The experiments show consistent improvements over strong baselines across multiple benchmarks, and ablation studies validate the contributions of each component of SpaceEra++.

Conclusion: The proposed framework, SpaceEra++, demonstrates enhanced visual-spatial understanding by effectively addressing the limitations in data and spatial reasoning, providing guidance for future research in this area.
...
TL;DR: A new method called PixGS is proposed for generating high-quality 3D Gaussian Splats directly from text or images, outperforming existing methods in quality and speed.


<details>
  <summary>Details</summary>
Motivation: Existing methods for 3D content generation have limitations such as view inconsistency, scarcity of high-quality 3D data, and computationally expensive pipelines.

Method: PixGS, a single-stage pipeline that leverages pixel-space diffusion to generate high-quality 3D Gaussian Splats directly, bypassing lossy latent compression.
...
Conclusion: The proposed PixGS method offers a robust and efficient alternative to multi-stage generation pipelines for 3D content generation.
...
TL;DR: The paper presents MMBench-Live, a dynamic multimodal benchmark that evolves over time, addressing issues of data staleness and contamination in vision-language models.


<details>
  <summary>Details</summary>
Motivation: Current multimodal benchmarks are static, leading to temporal staleness, data contamination, and costly maintenance, which hinders the evaluation of vision-language models.

Method: The authors propose a multi-agent-driven automated pipeline to construct and update the benchmark, integrating structured specification, feedback-controlled data acquisition, and verifiable QA generation.

Result: MMBench-Live preserves stable model rankings, maintains semantic alignment with the original benchmark, and shows weaker contamination-related memorization signals, with a low update cost of USD 30 and 1-2 hours.

Conclusion: The paper proposes a practical and scalable paradigm for sustainable multimodal benchmark evolution, providing a dynamic benchmark that can adapt to changing requirements and improve the evaluation of vision-language models.
...
TL;DR: A new network called DCGNet is proposed for salient object detection in underwater images, addressing challenges such as low contrast and color distortion.


<details>
  <summary>Details</summary>
Motivation: Conventional salient object detection methods are limited in underwater environments due to low contrast, uneven illumination, and color distortion caused by scattering and absorption effects.

Method: The proposed DCGNet includes a Dynamic Multi-Granularity module, an Underwater Physics-Prior module, an Underwater Spatial Gaussian module, and a Diffusion Transformer bottleneck to construct reliable conditional features for underwater saliency generation.
...
Conclusion: The proposed DCGNet has great potential for complex underwater visual applications due to its ability to effectively detect salient objects in underwater images with challenging conditions.
...
Result: The M2S framework shows effectiveness and generalizability on V2XSet, V2V4Real, and DAIR-V2X datasets, achieving up to an 8.64% improvement in 3D mAP performance without introducing communication costs.

Conclusion: The proposed M2S framework successfully retains the excellent performance of collaborative perception while avoiding its drawbacks, making it a promising solution for LiDAR-based 3D object detection in autonomous driving systems.
...
TL;DR: This paper studies the compression of large 3D foundation models for stereo reconstruction using knowledge distillation, with a focus on lunar stereo reconstruction. It proposes a structured SVD-based initialization method and evaluates various architectures and training strategies.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enable the deployment of large 3D foundation models in resource-constrained environments, such as planetary exploration, where onboard computing is limited.

Method: The method involves knowledge distillation from a large teacher model (MASt3R) to smaller student models, using a structured SVD-based initialization to bridge the dimensional mismatch between teacher and student. The authors evaluate various encoder types (CNN vs ViT), decoder widths and depths, and training strategies.

Result: The results show that a distilled student model can retain most of the teacher's reconstruction accuracy while reducing the model size up to 7 times. The student model even outperforms a baseline trained directly with sparse ground-truth annotations.

Conclusion: The study provides practical guidelines for deploying 3D reconstruction models in resource-constrained environments. It highlights the importance of preserving encoder capacity, feature-level distillation, and SVD-based initialization for stable optimization.
...
TL;DR: QWERTY is a training-free framework for motion control in video diffusion transformers (DiTs) using user-defined object warping and optical flow.


<details>
  <summary>Details</summary>
Motivation: Current DiTs rely on implicit motion control through text prompts, which can be inefficient and may not achieve desired motion. Fine-tuning models with spatial prompts requires substantial data and computation.

Method: QWERTY manipulates the 3D full attention of DiTs by warping the frame-invariant semantic subspace of queries, using user-defined object warping and optical flow for motion control.

Result: QWERTY achieves effective motion control comparable to fine-tuning-based methods, with better performance than existing training-free approaches.

Conclusion: QWERTY provides a flexible and efficient training-free solution for motion control in DiTs, enabling desired motion without extensive prompt engineering or fine-tuning.
...
TL;DR: A new dataset called LYNRED-MDS is introduced, which contains 4000 RGB-infrared image pairs captured under diverse weather, lighting, and road conditions for pedestrian detection in driving scenarios.


<details>
  <summary>Details</summary>
Motivation: Current road safety systems focus on minimizing post-collision damage, but advances in algorithmic perception aim to predict collisions early, especially in low-visibility conditions, where thermal infrared sensing is beneficial.

Method: The LYNRED-MDS dataset is created, comprising RGB-infrared image pairs captured under various conditions, and a YOLOv8n baseline is used for thermal cross-dataset evaluation.

Result: The LYNRED-MDS dataset shows strong generalization potential for pedestrian detection in driving scenarios, supporting the development of more reliable vision systems for advanced driver-assistance systems.

Conclusion: The LYNRED-MDS dataset can help develop more reliable and deployable vision systems for advanced driver-assistance systems by covering critical edge cases.
...
TL;DR: The paper proposes a novel Significance-Aware Binarization method for Large Vision-Language Models, which reduces memory and latency overhead while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: Large Vision-Language Models are limited by their high memory and latency requirements, making them difficult to deploy on resource-constrained devices. Binarization can help reduce these requirements, but existing methods have limitations.

Method: The paper proposes a Significance-Aware Binarization method that takes into account the varying importance of weights across different layers and modalities. It uses Hessian matrices, spatial significance maps, and modality-guided integration to obtain a significance-aware binarization map.

Result: The proposed method, SAB-LVLM, outperforms existing binary PTQ methods under an approximately 1-bit compression constraint, as demonstrated by extensive experiments.

Conclusion: The paper concludes that the proposed Significance-Aware Binarization method can effectively reduce the memory and latency overhead of Large Vision-Language Models while maintaining their performance.
...
TL;DR: Proposed a training-free framework to improve VGGT's performance on multi-view reconstruction tasks by organizing views into diversity-aware chunks, reducing redundant attention interactions and improving efficiency.


<details>
  <summary>Details</summary>
Motivation: The quadratic cost of attention in VGGT and the sensitivity of reconstruction quality to viewpoint distribution, which can degrade performance with redundant views.

Method: A plug-and-play VGGT inference framework that organizes views into diversity-aware balanced chunks through combinatorial graph partitioning and soft pose propagation.

Result: Improved performance in camera pose estimation, multi-view depth prediction, and 3D reconstruction, with reduced memory usage and inference latency.

Conclusion: The proposed framework enables scalable multi-view reconstruction without sacrificing geometric fidelity and complements existing VGGT variants.
...
TL;DR: A new framework called FoundDP is proposed to improve metric depth estimation from single-camera dual-pixel imaging by integrating global structural priors from a monocular depth foundation model.


<details>
  <summary>Details</summary>
Motivation: Existing dual-pixel based methods for metric depth estimation are unreliable in textureless, low-contrast, or downsampled regions due to weak or ambiguous disparity signals.

Method: FoundDP integrates metric DP depth with global structural priors from a monocular depth foundation model, leveraging Vision Transformer (ViT) features to restore structural consistency in weak-disparity regions.

Result: FoundDP delivers superior performance with consistent gains in structural fidelity and metric accuracy, especially under reduced disparity observability.

Conclusion: The proposed FoundDP framework improves the reliability of metric depth estimation from dual-pixel imaging, particularly in challenging regions with weak or ambiguous disparity signals.
...
TL;DR: The paper discusses the challenges of post-hoc calibration in semantic segmentation, specifically the issues of representation dependence and decision preservation. The authors propose translation-invariant calibrators and class-conditional affine calibrators to address these issues.


<details>
  <summary>Details</summary>
Motivation: The authors are motivated by the need for reliable confidence estimates in semantic segmentation, particularly in safety-critical settings. They aim to improve post-hoc calibration methods to correct confidence estimates without retraining the segmentation model.

Method: The authors study two issues in post-hoc calibration: representation dependence and decision preservation. They propose translation-invariant (TI) calibrators and class-conditional affine calibrators to address these issues. They also conduct experiments on natural-image and medical segmentation benchmarks to evaluate the performance of their proposed methods.

Result: The results show that translation-invariant variants generally improve calibration metrics, while decision-preserving variants prevent segmentation degradation and retain strong calibration performance. The authors provide practical design principles for well-defined post-hoc calibration pipelines in semantic segmentation.

Conclusion: The paper concludes that the proposed methods can improve post-hoc calibration in semantic segmentation, providing more reliable confidence estimates and preventing segmentation degradation. The authors emphasize the importance of considering representation dependence and decision preservation in post-hoc calibration.
...
<details>
  <summary>Details</summary>
Motivation: Existing knowledge distillation methods overlook flexibility of knowledge transfer across heterogeneous models, and prior methods often weaken or discard spatial information in heterogeneous representations.
...
Conclusion: SFKD effectively leverages spatial information in heterogeneous model representations, leading to improved knowledge distillation performance.
...
<details>
  <summary>Details</summary>
Motivation: The application of LVLMs to ultrasound remains limited due to its complexity and variability, and the authors aim to revisit what is truly needed to enable real-world ultrasound understanding.
...
Result: The simple approach leads to strong performance across diverse ultrasound understanding tasks, outperforming prior methods with more complex pipelines.

Conclusion: The authors conclude that data scale and clinically faithful data alignment are key factors in enabling real-world ultrasound understanding, and that a simple recipe can lead to strong performance in LVLMs.
...
<details>
  <summary>Details</summary>
Motivation: Real underwater imagery is affected by depth-dependent forward scattering blur and particle-induced degradations such as marine snow, which are not considered in conventional approaches.

Method: A staged processing pipeline is introduced, modeling background degradation via depth-aware forward scattering and foreground degradation using realistic marine snow patterns.

Result: Experiments on real underwater datasets show consistent improvements in visual quality and UIQM scores, reducing the synthetic-to-real gap and improving practical applicability.

Conclusion: The proposed framework effectively improves image quality in real-world underwater robotic operations by explicitly modeling forward scattering and realistic particle effects.
...
TL;DR: Proposed a bandwidth-efficient framework for collaborative 3D semantic occupancy prediction, achieving state-of-the-art performance while significantly reducing communication overhead.
...
Method: Introduced a Vector Quantization Semantic Occupancy Prediction (VQSOP) framework with Sparse-Aware Vector Quantization (SAVQ) and Dual-Branch Adaptive Spatial Refinement (ASR) module.

Result: Achieved state-of-the-art performance and reduced communication volume by up to 82x.
...
TL;DR: Emotional cues can trigger self-correction in vision-language models without additional training, and a new framework called ESC uses emotional feedback to improve model reliability.


<details>
  <summary>Details</summary>
Motivation: The vulnerability of vision-language models to unreliable reasoning and the high computational cost of existing self-correction methods.

Method: A training-free self-correction framework called ESC, which uses an external verifier to detect potentially incorrect responses and injects emotional feedback to encourage reflection.

Result: ESC consistently improves reliability across various benchmarks while preserving overall model utility.

Conclusion: Emotion can be used as a practical control signal for scalable self-correction in vision-language models, and ESC provides a strong foundation for a new reliable, human-like, emotion-integrated research direction.
...
TL;DR: Proposed LiZAD, a lightweight Zero-Shot Anomaly Detection framework for real-time defect detection on edge devices, achieving significant reductions in memory, parameters, and latency while maintaining competitive performance.


<details>
  <summary>Details</summary>
Motivation: The need for efficient anomaly detection in dynamic industrial settings where product configurations and visual characteristics frequently change, and deployment on resource-constrained edge devices.

Method: LiZAD pairs DINOv3 visual features with MobileCLIP2 text embeddings, mapping them into a shared latent space via low-memory trainable projection heads.

Result: LiZAD achieves an average memory reduction of 61.5%, parameter reduction of 74.6%, and a speedup of 3.02x in latency compared to state-of-the-art ZSAD models, with a 6.4% drop in average P-AUROC across multiple datasets.

Conclusion: LiZAD offers a practical solution for real-time Zero-Shot Anomaly Detection on edge devices, balancing efficiency and performance, and has been successfully deployed on NVIDIA Jetson devices and tested on a real production line.
...
TL;DR: A novel end-to-end framework for reconstructing whole-heart meshes from 2D cine MRI sequences, achieving high accuracy and smooth motion.


<details>
  <summary>Details</summary>
Motivation: The challenge of reconstructing 4D whole-heart meshes from sparse cine MRI due to limited 2D slice coverage and complex cardiac shape-motion coupling.

Method: A framework that learns an image-to-mesh mapping, using a differentiable contour renderer and a multi-scale temporal modeling module.

Result: Achieved a whole-heart mean absolute error of 1.68 ± 0.31 mm and a motion jitter of 0.77 ± 0.17 mm/frame^3, outperforming existing methods.
...
TL;DR: A new framework called NeoMap is proposed for synthesizing novel views of videos from single images or monocular videos, outperforming existing methods in terms of fidelity and consistency.


<details>
  <summary>Details</summary>
Motivation: Existing methods for novel view video synthesis often suffer from artifacts and compromised global scene consistency due to their reliance on pre-trained video models and various fine-tuning techniques.

Method: NeoMap uses convergent manifold alternating projection iterations to optimize the initial noise and locate high-fidelity, view-consistent novel view solutions from general pre-trained video models.
...
Conclusion: The proposed NeoMap framework offers a training-free approach to novel view video synthesis, leveraging the natural video data manifold learned by pre-trained models to produce high-quality novel views.
...
TL;DR: The study evaluates the performance of Vision-Language Models (VLMs) in assessing medical image quality under various conditions, revealing limitations and biases.


<details>
  <summary>Details</summary>
Motivation: To investigate the behavior of VLMs in medical image quality assessment under real-world conditions, including image degradation and textual context.

Method: Benchmarking 16 VLMs on the MediMeta-C dataset across seven corruption types and five severity levels, evaluating sensitivity to degradation patterns, embedding geometry, and textual attributes.

Result: VLMs show limitations in medical image quality assessment, with pixelation causing significant score reductions, and textual attributes affecting scores, indicating limited objectivity and potential biases.

Conclusion: Current VLMs are not reliable for medical image quality assessment, highlighting the need for improved models that balance patient privacy and reliability, and are objective and unbiased.
...
TL;DR: The paper identifies a vulnerability in CLIP models, where text in images can bias visual representations, and proposes a training-free method to improve robustness against this issue.
...
Method: The authors propose a novel, training-free mechanistic interpretability method that provides sampling-based interpretations of hidden state representations and quantitatively attributes semantic versus lexical focus to individual attention heads.

Result: The proposed method identifies specific Vision Transformer (ViT) components that disproportionately encode lexical information and shows that simple interventions applied directly to these components can substantially improve robustness against Typographic Attacks in object classification.

Conclusion: The authors demonstrate that their mechanistic approach can improve robustness against Typographic Attacks and yield substantial gains in Visual Question Answering accuracy under Typographic Attack interference on RIO-Bench.
...
TL;DR: MolSight, a graph-aware vision-language model, enhances molecular image understanding by integrating chemical-bond adjacency and aligning visual features with symbolic semantics.
...
Method: MolSight framework, comprising a Molecular Topology Module and a Molecular Grounding Module, to improve molecular image understanding.

Result: MolSight outperforms existing models and tools across multiple chemical visual understanding tasks, achieving a new level of molecular image reasoning.

Conclusion: MolSight effectively enhances the understanding of molecular images, addressing challenges in structural alignment and topological modeling.
...
TL;DR: The paper presents a weather-agnostic framework called Dual-Critic Guided Diffusion Alignment (DCDA) for robust 3D object detection under adverse weather conditions, which can generalize to unseen weather types and severities.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitation of current LiDAR-4D radar fusion methods, which are constrained by a closed-world assumption and fail to perform well under unseen weather conditions.

Method: The method proposed is DCDA, a framework that uses a 4D radar-conditioned diffusion process to progressively refine degraded LiDAR features, guided by two complementary critics: a detection-guided critic and a weather adversarial critic.

Result: The result shows that DCDA can effectively generalize to unseen weather types and severities without requiring paired data or weather labels, and extensive experiments verify its advantages on a structured open-weather benchmark.

Conclusion: The conclusion is that DCDA provides a robust solution for 3D object detection under adverse weather conditions, addressing the critical hurdle of handling unseen weather types and severities in autonomous driving.
...
TL;DR: This paper introduces a framework to investigate how self-supervised Vision Transformers encode geometric information, providing insights into feature encoding, representation compressibility, and layer-wise task affinity.


<details>
  <summary>Details</summary>
Motivation: The motivation is to understand how self-supervised Vision Transformers (ViTs) encode dense geometric information, as linear probing methods have limitations in disentangling the underlying topology.

Method: The authors propose a controlled subspace intervention framework, using Singular Value Decomposition (SVD) to decompose the weights of converged linear probes and isolate low-rank subspaces containing explicit geometric signals.

Result: The study provides three key insights: (1) pre-training objectives affect feature encoding, (2) explicit geometric representations are highly compressible, and (3) geometric precision peaks at intermediate layers before yielding to semantic abstraction in final layers.

Conclusion: The findings provide a basis for effective feature selection and lightweight decoder design, and the source code is available for further exploration.
...
TL;DR: The paper proposes a training-free motion generation framework called Motion-Inference-as-Control (MIC) that can handle both continuous and criterion-based motion constraints.


<details>
  <summary>Details</summary>
Motivation: Existing training-free methods for motion generation are limited to continuous objective-based constraints with differentiable losses, whereas real-world requirements often involve criterion-based constraints that provide discontinuous, sparse, or black-box feedback.

Method: The authors cast diffusion-based motion generation as a stochastic control problem and propose a control-oriented constraint coordination mechanism that adaptively balances and reconciles motion constraints during generation.

Result: Experiments across diverse constraint settings demonstrate the effectiveness of the proposed framework.

Conclusion: The proposed framework, MIC, is the first training-free motion generation framework that can handle both continuous objective-based and criterion-based motion constraints under a shared mechanism.
...
TL;DR: AutoMIA is an automated pipeline for designing Mirror Illusion Art, a 3D illusion that appears as two different objects when viewed directly and through a mirror.


<details>
  <summary>Details</summary>
Motivation: Current methods for creating Mirror Illusion Art require substantial manual effort, optimize shape only, and often produce non-smooth or incomplete geometry.
...
Conclusion: AutoMIA advances inverse graphics and computational design by providing an efficient and automated method for creating Mirror Illusion Art.
...
TL;DR: A new large-scale 3D facade benchmark, UnderOneFacade, is introduced to improve cross-domain generalization in facade parsing.
...
Method: A systematic evaluation of representative point-, graph-, and transformer-based architectures was conducted on the UnderOneFacade benchmark.
...
Conclusion: UnderOneFacade establishes a rigorous benchmark for developing robust and transferable 3D segmentation models by combining geometric precision with standardized semantics at unprecedented scale.
...
TL;DR: STAR3, a multimodal framework, generates radiology reports by aligning anatomical information with clinical context and longitudinal changes in chest X-ray studies, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: The increasing demand for imaging and workforce shortages in radiology strain reporting capacity and clinical workflows, necessitating automated radiology report generation.

Method: STAR3 employs an object detector to identify anatomically meaningful regions and retrieves report sentences based on current clinical context and changes observed between prior and current examinations.

Result: STAR3 outperforms current retrieval-based approaches on retrieval, NLP, and clinical metrics in experiments on the MIMIC-CXR dataset.

Conclusion: The proposed STAR3 framework advances automated radiology report generation by providing anatomically and temporally grounded reports that better reflect clinical reporting practice.
...
TL;DR: This paper evaluates the performance of Vision-Language Models (VLMs) for Nigerian license plate recognition, comparing five models: Gemini 2.0 Flash Exp, Qwen2.5-VL-7B-Instruct, GPT-4o, Claude 4 Sonnet, and Llama 3.2 Vision 90b.


<details>
  <summary>Details</summary>
Motivation: Traditional License Plate Recognition (LPR) systems have limitations such as high resource demands and poor performance in unstructured environments. The study aims to explore VLMs as a unified, zero-shot learning solution for LPR.

Method: The study uses a curated dataset of 88 real-world images collected in Nigeria to evaluate the performance of five VLMs based on Character Error Rate (CER).

Result: Gemini 2.0 Flash Exp and Qwen2.5-VL-7B-Instruct significantly outperform other models in both accuracy and robustness, especially in challenging image scenarios.

Conclusion: The study highlights the practical advantages of VLMs over traditional YOLO+OCR approaches for LPR, questions the claims by model providers, and compares the performances of the VLMs.
...
TL;DR: The paper proposes ComplexMimic, a framework for physics-based human-scene interaction imitation learning in complex environments, addressing the trade-off between accurate motion tracking and physically plausible motions.


<details>
  <summary>Details</summary>
Motivation: Existing methods for human-scene interaction imitation learning focus on simplified scene settings, limiting their applicability in real-world scenarios. The authors aim to bridge this gap by developing a framework for complex environments.

Method: The authors propose ComplexMimic, which uses a Dual Flow Strategy with two complementary experts: an imitation expert for accurate motion tracking and an interaction expert for collision-aware adaptation. They also introduce a difficulty-aware distillation strategy to adaptively weight supervision.

Result: The approach outperforms current state-of-the-art methods on three benchmark datasets, demonstrating its effectiveness in complex environments.

Conclusion: The paper presents ComplexMimic as a solution for physics-based human-scene interaction imitation learning in complex environments, highlighting its potential for real-world applications.
...
TL;DR: The authors propose a Hierarchical Anti-Aesthetics (HAA) framework to protect facial privacy by degrading the generation quality of customized diffusion models, thereby reducing facial identity leakage.


<details>
  <summary>Details</summary>
Motivation: The rise of customized diffusion models poses threats to personal privacy due to potential malicious misuse, and the authors aim to address this issue from an aesthetic perspective.

Method: The HAA framework consists of two branches: Global Anti-Aesthetics, which degrades overall aesthetics and generation quality, and Local Anti-Aesthetics, which disrupts facial identity by targeting facial regions.

Result: The proposed HAA framework outperforms existing methods in identity removal, providing an effective tool for protecting facial privacy.

Conclusion: The authors conclude that their proposed HAA framework is effective in protecting facial privacy by degrading the generation quality of maliciously customized models.
...
<details>
  <summary>Details</summary>
Motivation: The key challenge in articulated 3D object generation from a single image is accurately predicting the underlying kinematic structure, and existing methods have limitations such as accumulated errors and limited dataset scale and diversity.

Method: The paper proposes a unified Part World Model (PWM-ArtGen) that couples action diffusion and image diffusion with independent diffusion timesteps, enabling visual branch co-training, and leverages a photorealistic dataset of 19.7k part-level image pairs without kinematic annotations.
...
Conclusion: The proposed PWM-ArtGen model effectively addresses the limitations of existing methods and demonstrates strong performance in articulated 3D object generation from a single image.
...
TL;DR: The authors propose Multiple Prototype Contrastive Learning (MPCL) for semi-supervised medical image segmentation, addressing intra-class heterogeneity and scarcity of labeled data.


<details>
  <summary>Details</summary>
Motivation: Existing methods for semi-supervised medical image segmentation fail to effectively exploit intensity-manifested intra-class heterogeneity, leading to imprecise segmentation.

Method: The proposed MPCL framework consists of three novel designs: Intensity-aligned Heterogeneous Prototype Generation (IHPG), Prototypical Space Optimization (PSO), and Dual-branch Knowledge Alignment (DKA).

Result: MPCL significantly outperforms existing methods on three medical image datasets, especially under extremely limited labeled data.

Conclusion: The proposed MPCL framework effectively addresses intra-class heterogeneity and scarcity of labeled data, leading to more precise segmentation results in semi-supervised medical image segmentation.
...
TL;DR: This paper evaluates the adversarial robustness of LiDAR-only 3D object detection models, revealing that recent models are vulnerable to attacks and that high-capacity voxel-based detectors are more susceptible to structured coordinate perturbations.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to investigate the adversarial robustness of LiDAR-only 3D object detection models, which is a crucial aspect that has been overlooked in existing evaluation frameworks.

Method: The authors propose a holistic framework that evaluates adversarial robustness using two structural factors (point cloud density and point cloud localization) and three predictive factors (misclassification, localization error, distance from ego). They then perform an empirical study on recent and legacy state-of-the-art models using adversarial attacks specifically designed for LiDAR-based models.

Result: The results show that high-capacity, voxel-based detectors are more susceptible to structured coordinate perturbations than pillar-based detectors, and non-anchor-based detectors demonstrate poor adversarial robustness. The study also finds that recent models are as vulnerable to adversarial attacks as their predecessors.

Conclusion: The authors conclude that there is a need to improve the evaluation benchmarks for 3D object detection to not only reward architectural modifications for improving detection accuracy but also evaluate whether design choices improve adversarial robustness.
...
TL;DR: A framework for generating hand-controlled egocentric videos from monocular video without motion capture, leveraging a new dataset and 3D-aware control signal.
...
Method: A protagonist-centered annotation pipeline for 3D hand annotation and the Plücker Hand Map, a 3D-aware control signal.

Result: The proposed method surpasses prior hand-controlled generators in reconstruction fidelity and control accuracy, generalizing to out-of-distribution everyday scenes.

Conclusion: The proposed framework, HandsOnWorld, enables hand-controlled egocentric video generation from unconstrained monocular video, achieving improved performance and generalizability.
...
TL;DR: The paper presents a new approach to modeling human visual attention using Vision-Language Models, achieving state-of-the-art results in predicting visual scanpaths and enabling controlled simulations of human behavior.


<details>
  <summary>Details</summary>
Motivation: The authors aim to improve the understanding of human visual attention over time, which has applications in interface design and inferring cognitive states, by developing a more flexible and extendable model.

Method: The authors frame scanpath prediction as a discrete sequence modeling task, leveraging pretrained Vision-Language Models by mapping coordinates into a text vocabulary, allowing for global conditioning and integration of per-fixation attributes.

Result: The proposed model, DeepGaze3.5-VL, establishes a new state-of-the-art across multiple datasets, achieving 2.18 bits of IG on MIT1003, a 46% improvement over DeepGaze III.

Conclusion: The paper concludes that the proposed generative framework not only achieves superior predictive performance but also serves as a powerful computational tool for direct behavioral interventions and controlled simulations of human behavior.
...
<details>
  <summary>Details</summary>
Motivation: To improve the viewing experience of trading card game streams by augmenting them with virtual models of the played cards without requiring instrumented playing surfaces or specialized hardware.

Method: A real-time pipeline that detects, orients, and identifies cards on the board using RGB cameras, and renders virtual content onto each card across all views.

Result: The pipeline's performance and runtime throughput were evaluated on a manually annotated dataset, showing promising results for real-world usability.

Conclusion: The proposed pipeline, TCG-AR, makes real-time augmented-reality streaming accessible to the broader community of players and streamers by relying on commodity cameras and hardware, and open-sourcing all code, models, and datasets.
...
<details>
  <summary>Details</summary>
Motivation: Existing computer-aided diagnosis methods for breast fibroadenoma and phyllodes tumor rely on single-modal imaging features and do not sufficiently exploit complementary clinical and textual information, leading to misclassification and complicating preoperative decision-making.

Method: A clinically guided multimodal framework that integrates DenseNet-based visual encoding, CLIP-inspired text encoding, and lightweight clinical encoding, with clinical-conditioned adaptive modulation, cross-modal Transformer fusion, and dual-path representation learning.

Result: The proposed method achieves an accuracy of 77.64%, F1-score of 73.38%, and AUC of 89.74% under patient-level five-fold cross-validation, outperforming representative baselines.
...
TL;DR: A new benchmark, LongEgoRefer, is introduced for Video Referring Expression Comprehension (Video REC) in egocentric videos, focusing on long-form videos with sparse object occurrences and complex activities.
...
TL;DR: Proposed Windowed Batch Matrix Multiplication (WBMM) for efficient large kernel depthwise convolutions, achieving better performance and accuracy than traditional methods.


<details>
  <summary>Details</summary>
Motivation: Large kernel depthwise convolutions suffer from performance degradation due to irregular memory access, and existing acceleration methods have limitations on large feature maps.

Method: WBMM partitions input into contiguous windows and uses a compact relative position bias table to construct weight matrices, enabling regular memory access via batched matrix multiplication.

Result: WBMM outperforms traditional depthwise convolution baselines in speed and accuracy, achieving 1.31-1.88x training speedup and comparable or higher accuracy on ImageNet-1K, COCO, and ADE20K.

Conclusion: WBMM provides a efficient and effective solution for large kernel depthwise convolutions, with advantages across various devices and no need for specialized acceleration kernels.
...
TL;DR: A new method called X-Splat is proposed for generating 3D dental volumes from a single panoramic radiograph, outperforming existing approaches in reconstructing detailed anatomy.


<details>
  <summary>Details</summary>
Motivation: Current methods for generating 3D dental volumes from panoramic radiographs often produce oversmoothed or anatomically inconsistent results. The goal is to develop a low-radiation alternative to Cone-Beam Computed Tomography (CBCT) that can accurately reconstruct 3D dental anatomy from a single 2D image.

Method: X-Splat uses a Gaussian Splatting framework, initializing learnable anisotropic Gaussian primitives along the X-ray paths that formed the input image. The method is constrained by Beer-Lambert reprojection and multi-view radiographic training supervision, and further refined with a lightweight residual refiner.

Result: X-Splat outperforms NeRF- and GAN-based baselines in reconstructing individual teeth, cortical boundaries, and alveolar structure, including the mandibular canal. The method provides a more accurate and detailed 3D dental volume from a single panoramic radiograph.

Conclusion: X-Splat offers a promising solution for generating CBCT-like 3D dental volumes from a single panoramic radiograph, with potential applications in low-radiation dental imaging.
...
TL;DR: The paper proposes a physics-inspired pipeline for synthesizing realistic film degradations and a large-scale archival benchmark for evaluating film restoration models.


<details>
  <summary>Details</summary>
Motivation: The lack of paired training data and standardized evaluation benchmarks for archival film restoration, and the limitations of existing real-world datasets.

Method: A physics-inspired, modular pipeline called AbsoluteDegradation for synthesizing realistic film degradations, and a curated dataset of 81,576 high-resolution frames from real archival footage.
...
TL;DR: Proposed AdaCount, a training-free framework for zero-shot object counting (ZOC) that improves instance segmentation and counting in dense scenes.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with densely populated scenes containing numerous small objects, leading to missed instances and poor instance separation.

Method: AdaCount uses similarity-guided spatial and feature adaptation to enhance target-relevant regions in images, working with foundation models like SAM3.
...
Conclusion: AdaCount effectively addresses limitations in existing ZOC methods, particularly in dense scenes, without requiring model retraining or specific training data.
...
TL;DR: The paper introduces SAMoR, a motion representation model for articulated objects with arbitrary skeleton topologies, enabling cross-topology motion transfer and generation.


<details>
  <summary>Details</summary>
Motivation: Existing motion generators are limited to fixed human skeletons and struggle to adapt to arbitrary skeleton topologies, losing motion details in the process.

Method: The authors propose a graph-transformer encoder that consumes per-joint motion features, kinematic graph structure, and joint-name embeddings to encode motion segments into part-level tokens.

Result: SAMoR achieves accurate reconstruction and cross-topology transfer, with a normalized MPJPE of $2.75 	imes 10^{-2}$ on cross-topology reconstruction, outperforming adapted variable-$J$ tokenizer baselines.

Conclusion: The proposed SAMoR model enables accurate motion representation and generation for articulated objects with arbitrary skeleton topologies, supporting text-conditioned generation and part-wise editing.
...
TL;DR: The paper presents a method for creating patient-specific articulated digital twins from a single full-body CT scan, allowing for simulation of different body poses.


<details>
  <summary>Details</summary>
Motivation: Current CT-derived models are static and cannot represent changes in patient anatomy due to repositioning, which is important for radiographic imaging.

Method: The method involves fitting a parametric human body model to a patient-aligned kinematic scaffold, binding segmented bones and organs to an anatomy-aware rig, and retargeting body-pose changes while preserving skeletal geometry.

Result: The fitted scaffold achieved high accuracy in terms of chamfer distance and skeletal enclosure, and the resulting twins enabled articulation while maintaining high skeletal enclosure across unseen target poses.

Conclusion: The results suggest the feasibility of extending static CT simulation to pose-controllable anatomical twins for future synthetic imaging and positioning studies.
...
<details>
  <summary>Details</summary>
Motivation: Modern vision models require significant GPU memory for fine-tuning, making them impractical for edge deployment. The paper aims to address this limitation.
...
Result: QLoRA and BitFit reduce energy consumption by 20-30% with 1-2% accuracy cost. The adaptive algorithm decreases peak memory by 43-79% with 9-30% energy overhead. DINOv2 outperforms fine-tuned models on CIFAR-100 at lower energy consumption.

Conclusion: The paper highlights the potential of parameter-efficient fine-tuning methods and foundation models for edge deployment, with a focus on accuracy, energy efficiency, and deployment feasibility.
...
TL;DR: The paper introduces a new task called Fashion Detail Generation with focus conditioning, and proposes a model called DetailAnywhere to generate photorealistic close-up views of specific detail regions of apparel products.


<details>
  <summary>Details</summary>
Motivation: Existing generative AI methods for e-commerce applications lack the ability to examine specific detail regions of interest in apparel products, and the authors aim to fill this gap.

Method: The authors propose Cross-modal Feature Alignment Distillation (CFAD) to align a Multimodal Diffusion Transformer in a shared semantic space, and introduce a consistency reward model to optimize generation via reinforcement learning.

Result: The proposed model DetailAnywhere significantly outperforms all state-of-the-art open-source methods across all metrics and human evaluations.

Conclusion: The paper formalizes a new task and proposes an effective model for generating photorealistic close-up views of specific detail regions of apparel products, with potential applications in e-commerce.
...
TL;DR: The paper analyzes two classification strategies (OvA and OvR) for AI-based waste sorting solutions to support the Circular Economy in Germany, focusing on their performance and misclassification behavior.


<details>
  <summary>Details</summary>
Motivation: The complexity of waste disposal regulations in European countries, particularly in Germany, hinders the transition to a Circular Economy, and AI-based solutions can help, but must be configurable to reflect local waste sorting schemes.

Method: The research evaluates and analyzes OvA and OvR classification strategies using a dataset from the city of Goslar in Germany, and compares their performance by applying varying confidence thresholds to identify uncertain samples.

Result: The evaluation aims to balance the number of misclassifications against the human effort required for data annotation, to improve the effectiveness of AI-based waste sorting solutions in supporting the Circular Economy.

Conclusion: The paper concludes that analyzing the behavior of classification strategies in identifying samples likely to be misclassified is crucial for improving waste management and advancing the Circular Economy.
...
TL;DR: This paper compares token compression and structural pruning methods for efficient Vision Transformers (ViT) in semantic segmentation, finding that structural pruning is more stable at high compression and proposing a prune-then-merge pipeline for better accuracy-robustness trade-off.


<details>
  <summary>Details</summary>
Motivation: The computational cost of ViTs limits their deployment, and existing token compression methods have not been thoroughly evaluated under aggressive compression and corrupted inputs, while structural pruning has not been compared to token compression under a unified protocol.

Method: The paper benchmarks representative token compression and structural pruning methods for ViT-based semantic segmentation under matched FLOPs on ADE20K and Cityscapes datasets, and proposes a prune-then-merge pipeline.

Result: Token compression degrades sharply under severe compression, while structural pruning exhibits a smoother degradation curve and is more stable at high compression. The proposed prune-then-merge pipeline achieves a better accuracy-robustness trade-off at high compression.

Conclusion: The paper concludes that structural pruning is a more effective approach for efficient ViT-based semantic segmentation, especially at high compression, and proposes a practical recipe for deployment-oriented ViT segmentation.
...
TL;DR: Proposed ArcAD, a calibration framework for anomaly detection in industrial settings with limited normal and anomaly data.


<details>
  <summary>Details</summary>
Motivation: Addressing the cold-start problem in Industrial Anomaly Detection (IAD) where limited normal samples and few anomalies hinder existing methods.

Method: ArcAD uses a push-pull learning paradigm to form a compact normal boundary by projecting normal samples onto a hypersphere, clustering them, synthesizing pseudo-anomalies, and leveraging real anomalies.

Result: ArcAD outperforms state-of-the-art methods in single-class and multi-class settings under cold-start conditions on multiple datasets.

Conclusion: ArcAD effectively tackles the cold-start bottleneck in IAD, providing a robust anomaly detection approach for industrial applications.
...
TL;DR: The paper introduces AnyGroundBench, a benchmark for domain adaptation in Spatio-Temporal Video Grounding (STVG), highlighting the limitations of current Vision-Language Models (VLMs) in specialized domains.


<details>
  <summary>Details</summary>
Motivation: Current STVG evaluation protocols are limited to zero-shot assessments on general benchmarks, which is insufficient for real-world applications in specialized fields with rare visual concepts and complex spatio-temporal dynamics.

Method: The authors introduce AnyGroundBench, a domain-adaptation benchmark with five specialized domains, paired with newly captured videos and established datasets, and evaluate 15 state-of-the-art VLMs.

Result: Current VLMs fail in both zero-shot and In-Context Learning (ICL)-based adaptation when confronted with specialized domains, revealing critical flaws in spatio-temporal reasoning.

Conclusion: The paper emphasizes the need for future research to address the limitations of current VLMs in specialized domains and to improve their domain adaptability.
...
TL;DR: The paper evaluates 30 data augmentation strategies for vein recognition, a secure biometric technology, and finds that while multi-image mixing methods improve performance, they are often poorly calibrated and vulnerable to adversarial perturbations.


<details>
  <summary>Details</summary>
Motivation: Vein recognition is constrained by limited annotated data and imaging variations, and existing data augmentation strategies may not be suitable for this fine-grained task.

Method: The authors evaluate 30 representative augmentation strategies on five public palm- and finger-vein datasets with seven backbone architectures.

Result: Multi-image mixing methods (e.g., MixUp, PuzzleMix, StarMixup) provide the strongest recognition performance, but are often poorly calibrated and vulnerable to adversarial perturbations.

Conclusion: Accuracy-centric evaluation is insufficient for biometric augmentation, and AGVBench provides standardized protocols to support reproducible research and guide the design of reliable, secure, and robust vein recognition systems.
...
TL;DR: Proposed FlowCIR, a new paradigm for zero-shot composed image retrieval (ZS-CIR) that uses conditional flow matching to efficiently map instruction representations to target-aligned query embeddings.


<details>
  <summary>Details</summary>
Motivation: Existing ZS-CIR methods rely on lossy textual inversion, which can be brittle for fine-grained semantics. The authors aim to improve efficiency and robustness.

Method: FlowCIR uses conditional flow matching to learn a lightweight transport field, operating on pre-extracted VLM embeddings and training a small transport module.

Result: FlowCIR achieves strong and competitive performance on standard CIR benchmarks, requiring roughly 10x fewer training resources than prior approaches.

Conclusion: FlowCIR offers a training-efficient solution for ZS-CIR, improving robustness on negation-heavy queries with a proposed Multi-Negative Steering strategy.
...
TL;DR: A new dataset called DisciplineGen-1M is introduced, which enables the generation and editing of knowledge-intensive diagrams across various disciplines. A discipline-informed model is also proposed, showing significant improvements over existing baselines.


<details>
  <summary>Details</summary>
Motivation: Current image generation and editing models struggle with knowledge-intensive diagrams that require disciplinary concepts, symbolic structure, and precise spatial relations. The authors aim to address this limitation.

Method: The authors design a scalable framework to construct DisciplineGen-1M, a million-scale multidisciplinary dataset, and propose a discipline-informed reasoning-generation model for text-to-image generation and image editing.

Result: Experiments on discipline-related benchmarks show substantial improvements over open-source baselines, and evaluations on general reasoning-informed benchmarks indicate broader transfer.

Conclusion: The results highlight the importance of large-scale structured academic visual data for knowledge-grounded visual creation, and the authors plan to publicly release their dataset, model, and source code.
...
TL;DR: The paper proposes a new method called Dual-Selective Incremental Network (DSINet) for domain-incremental change detection (DICD) that adapts to new geographic domains while preserving prior knowledge.


<details>
  <summary>Details</summary>
Motivation: The existing methods for DICD struggle with structural mismatch between fixed label space and varying domain characteristics, leading to knowledge degradation or increased computational cost.

Method: The proposed DSINet uses a selective spatial state unit (S3U) and a concentration-balanced distillation (CBD) strategy to maintain stable spatial change representations across domains.

Result: The experimental results show that DSINet mitigates knowledge degradation across long domain sequences while maintaining linear computational efficiency.

Conclusion: The proposed DSINet provides a unified framework for DICD that preserves prior knowledge and adapts to new domains, achieving stable learning dynamics and efficient computation.
...
<details>
  <summary>Details</summary>
Motivation: The industry's increasing use of vision-language models for in-car scene understanding highlights the need for systematic testing due to potential errors or misleading scene descriptions.

Method: ISU-Test uses rendering-based scene generation and search-based testing, framing testing as an optimization problem to systematically modify scene parameters and generate diverse in-car scenarios.

Result: ISU-Test outperforms the baseline, achieving up to 10 times higher failure rates and up to 3.6 times higher failure coverage compared to randomized scenario generation.

Conclusion: The proposed ISU-Test approach effectively evaluates ISU systems, demonstrating its potential to improve the testing of vision-language models in the automotive domain.
...
TL;DR: A feed-forward multi-view reconstruction framework for inverse rendering that predicts a structured 3D Gaussian representation with intrinsic material attributes.


<details>
  <summary>Details</summary>
Motivation: Optimization-based methods for inverse rendering are costly and image-space learning-based approaches suffer from multi-view inconsistencies and lack explicit 3D representation.

Method: A feed-forward framework integrating priors from a material estimation network with a multi-view 3D reconstruction backbone to jointly predict geometry and reflectance parameters.

Result: Improved multi-view consistency, accurate material recovery, and stable novel view rendering on synthetic and real-world datasets.

Conclusion: The proposed framework enables physically-based relighting and more faithful modeling of view-dependent effects, outperforming existing RGB-based feed-forward reconstruction methods.
...
TL;DR: A novel neural-guided video synthesis framework is introduced to generate stimuli optimized for target brain regions across the visual cortex, allowing for the exploration of dynamic visual selectivity.


<details>
  <summary>Details</summary>
Motivation: The human brain's dynamic visual processing is underexplored, and current brain encoding models are limited to static images. The authors aim to develop a framework that can synthesize optimal stimuli for dynamic visual processing.

Method: The framework performs evolutionary search over a structured prompt space, guided by a dynamic encoding model that predicts voxel-level responses to video inputs, maximizing predicted activity for a target region of interest (ROI).

Result: The synthesized videos recover known selectivities across ventral, dorsal, and lateral pathways, reveal systematic differences in sensitivity to temporal dynamics, and provide new insight into the progression toward increasingly complex social-dynamic features along the lateral stream.

Conclusion: The framework enables in silico exploration of dynamic visual selectivity, with new predictions for in vivo experiments, and has the potential to advance our understanding of the human brain's dynamic visual processing.
...
Result: The proposed framework is expected to improve the accuracy of monocular RGB-based 6D pose sensing in spacecraft images.

Conclusion: The GAP-GDRNet framework offers a promising solution for monocular relative pose sensing in challenging spacecraft image conditions, leveraging attention mechanisms and geometry guidance.
...
TL;DR: VisionAId is an Android app that uses on-device deep learning models to assist visually impaired individuals with tasks like object recognition, navigation, and cash handling.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the challenges faced by the 285 million people worldwide with visual impairments, by developing an assistive technology that can aid in everyday tasks.

Method: The method used in this paper is to develop an Android application called VisionAId, which integrates six on-device deep learning models and uses a commodity smartphone as a real-time visual assistant.

Result: The results of this paper include: INT8 quantization reducing depth latency to ~491 ms, a custom banknote detector reaching an mAP@50 of 0.986, and metric depth calibrated to below 1 cm of error within 3 m.

Conclusion: The conclusion of this paper is that VisionAId has the potential to improve the autonomy of visually impaired individuals by providing a real-time visual assistant that can aid in everyday tasks.
...
TL;DR: A novel method called SPoILeR is proposed for rendering unconventional imaging modalities like infrared and multispectral data from RGB images or a few samples.


<details>
  <summary>Details</summary>
Motivation: Current neural rendering techniques require expensive sensors and calibrated setups to capture multimodal frames for each new scene, limiting their applicability.

Method: The proposed method, SPoILeR, uses a multimodal pre-training phase to learn the correlation between different modalities, allowing it to predict accurate renderings of unconventional modalities from RGB images.

Result: The approach can accurately render infrared, polarimetric, and multispectral frames for scenes with no input samples from these modalities.

Conclusion: SPoILeR enables multi-view consistent renderings of unconventional modalities with limited or no input data, expanding the capabilities of neural rendering techniques.
...
TL;DR: The paper presents Representation Distribution Matching (RDM), a paradigm for training one-step image generators by matching generated and reference feature distributions under frozen pre-trained encoders.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve one-step image generators, which have been challenging to train effectively.

Method: The method involves identifying two design axes for RDM: how distributions are compared and the representations they are compared in. The authors conduct controlled studies and find that using MMD with large batch sizes and matching against a balanced set of encoders yields improved results.

Result: The authors achieve state-of-the-art results on ImageNet with their improved RDM (iRDM) method, surpassing prior one-step generators. They also improve a four-step generator, FLUX.2, to a one-step generator.

Conclusion: The paper concludes that RDM is a effective paradigm for training one-step image generators, and that careful design choices can lead to improved performance.
...
TL;DR: The authors analyze the evolution of internal representations in Vision Transformers during training, revealing that representation complexity and layer specialization emerge simultaneously, and that strong token interaction structure persists throughout training.


<details>
  <summary>Details</summary>
Motivation: The geometric evolution of internal representations in Vision Transformers throughout training remains insufficiently understood, and existing analyses primarily focus on attention mechanisms and downstream performance.

Method: The authors present Transformer Geometry Observatory-II (TGO-II), a representation geometry analysis framework that analyzes Vision Transformer representations using Centered Kernel Alignment (CKA), Singular Vector Canonical Correlation Analysis (SVCCA), Two-Nearest Neighbor Intrinsic Dimensionality (TwoNN-ID), and token covariance analysis.
...
TL;DR: The paper introduces a new task called Visual Concept Inference from Sets (VICIS) to evaluate the ability of vision-language models (VLMs) to infer shared concepts from sets of example images and apply them to new inputs. The authors propose a training framework and architecture to improve VLMs' performance on this task.


<details>
  <summary>Details</summary>
Motivation: Current VLMs struggle to reason from purely visual context and fail to infer shared concepts from sets of example images. The authors aim to address this gap and improve VLMs' ability to generate new images that preserve the context-defined concept while remaining consistent with the query.

Method: The authors propose a training framework and architecture that learn to infer visual concepts from image sets and extract concept-specific embeddings from queries. They evaluate their approach on synthetic data and large-scale ImageNet/WordNet data.

Result: The authors' model generates more accurate and diverse outputs and generalizes to unseen concepts and modalities such as sketches. State-of-the-art VLMs perform poorly on the VICIS task, often ignoring the visual context or defaulting to biased generations.

Conclusion: The paper highlights the limitations of current VLMs in inferring shared concepts from visual context and proposes a new task and approach to improve their performance. The authors' results demonstrate the effectiveness of their approach in generating accurate and diverse outputs.
...
<details>
  <summary>Details</summary>
Motivation: To improve data efficiency and downstream performance of self-supervised image encoders by focusing on object-level representations rather than whole scenes.

Method: Extending LeJEPA to object-centric representations using off-the-shelf object masks and adding an instance-separating loss.

Result: Object-level LeJEPA outperforms image-level LeJEPA on tracking, classification, segmentation, and re-identification tasks across two model scales and 10-100% of COCO.

Conclusion: The proposed object-level LeJEPA approach effectively sidesteps the instability of joint partitioning and representation learning, leading to improved performance on various downstream tasks.
...
TL;DR: The paper proposes a frequency-aware semantic compensation strategy for inversion-free image editing, improving global editing capacity while preserving background structures.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitation of existing inversion-free image editing frameworks, such as FlowEdit, in achieving substantial semantic changes under certain global attribute shifts.
...
<details>
  <summary>Details</summary>
Motivation: The authors argue that existing approaches to understanding human behavior in videos rely on implicit representations and disregard structured reasoning over scene dynamics, and propose explicit, compositional, and editable representations of human-environment interactions.

Method: The authors introduce SG-Ego, a large-scale annotation set with spatio-temporal scene graphs, and GLEN, a graph-based model that operates over scene graph sequences to align them with textual actions and model their temporal evolution.

Result: The authors validate their approach across multiple downstream tasks, achieving strong results compared to raw video baselines, and exceling in reasoning settings, enabling controllable and structured predictions of scene dynamics driven by human activities.
...
TL;DR: The paper proposes a novel methodology for out-of-distribution (OOD) detection in clinical settings, addressing the limitations of existing methods. It introduces a multi-expert framework with a Nonlinear von Mises-Fisher classifier and an outlier expert, achieving significant improvements over state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: The need for more robust OOD detection methods in clinical deployment of automated diagnostic systems, as deep models often misclassify OOD inputs with high confidence.

Method: A novel methodology comprising a Nonlinear von Mises-Fisher (NvMF) classifier, a multi-expert framework with margin-aware NvMF classifiers, and an outlier expert trained to distinguish inlier from outlier data.

Result: Consistent improvements over state-of-the-art methods on RFMiD, ISIC2019, and NCTCRC datasets, with mean FPR95 reductions of 8.45%, 13.02%, and 36.90% respectively.
...
TL;DR: OrbitQuant, a data-agnostic weight-activation quantizer, enables efficient post-training quantization of diffusion transformers, achieving state-of-the-art results across various models and settings.


<details>
  <summary>Details</summary>
Motivation: The high computational cost of diffusion transformers (DiTs) due to multi-step sampling and growing parameter count, and the challenge of applying post-training quantization (PTQ) to DiTs due to shifting activations across timesteps, prompts, and guidance branches.

Method: OrbitQuant uses a normalized, rotated basis to quantize activations, allowing a single Lloyd-Max codebook to serve all timesteps, prompts, and layers of a given input dimension. It also extends to weight rows offline, absorbing the rotation into the weights.

Result: OrbitQuant sets the state of the art for PTQ at several low-bit settings across FLUX.1, Z-Image-Turbo, Wan 2.1, and CogVideoX, and pushes PTQ of image diffusion transformers to W2A4 with usable generation quality.

Conclusion: OrbitQuant provides an efficient and effective solution for post-training quantization of diffusion transformers, enabling reduced computational cost without sacrificing generation quality.
...
TL;DR: Proposed Geo-Anchored Cloud Removal (GACR) framework for cloud removal in optical remote sensing, ensuring faithful reconstruction and robust interpretability.


<details>
  <summary>Details</summary>
Motivation: Existing cloud removal approaches prioritize visual realism but overlook impact on subsequent analytical tasks, leading to semantic drift and degraded performance.

Method: GACR incorporates Observation-Anchored Residual Flow (OAR-Flow) and Geo-Contextual Prior Alignment (GCPA) to ensure faithful reconstruction and preserve semantic structures.

Result: GACR produces superior reconstruction quality and consistently improves downstream task accuracy across six CR datasets and twelve downstream tasks.

Conclusion: GACR framework effectively addresses the issue of cloud removal in optical remote sensing, providing a unified solution for reliable downstream interpretation.
...
TL;DR: The paper proposes EAGLE-360, a novel framework for active visual search in 360° panoramic environments, addressing limitations of standard Multimodal Large Language Models (MLLMs) in modeling panoramic properties.


<details>
  <summary>Details</summary>
Motivation: Standard MLLMs struggle with adapting to 360° panoramic environments due to severe polar distortion and continuous cylindrical topologies, leading to degraded target detection accuracy.

Method: The proposed EAGLE-360 framework leverages global priors to establish an initial holistic perspective, iteratively reasoning and progressively narrowing the search space. It adapts RoPE Rolling, a coordinate-shifting positional encoding mechanism, and uses a large-scale dataset with a training pipeline that integrates Supervised Fine-Tuning (SFT) with Group Relative Policy Optimization (GRPO).
...
Conclusion: The proposed EAGLE-360 framework effectively addresses the limitations of standard MLLMs in 360° panoramic environments, demonstrating improved accuracy and exploration efficiency.
...
TL;DR: The paper proposes Entropy-Aware Dense Pruning (EADP) to improve visual token pruning in VLMs, addressing issues with textual noise and feature fragmentation.


<details>
  <summary>Details</summary>
Motivation: Existing visual token pruning methods often fail to preserve critical cues under dense instructions and fine-grained queries.

Method: EADP uses statistical entropy to filter out textual noise and casts token selection as a submodular maximization problem with a spatial prior.

Result: EADP improves the accuracy-efficiency trade-off of VLMs, achieving SoTA performance on multimodal benchmarks while preserving fine-grained visual cues.

Conclusion: The proposed EADP framework effectively addresses the bottlenecks in visual token pruning, leading to improved performance and efficiency in VLMs.
...
<details>
  <summary>Details</summary>
Motivation: Descriptor-free visual localization is essential for eliminating high-dimensional descriptor storage, preserving scene privacy, and simplifying map maintenance. However, its accuracy lags behind descriptor-based pipelines due to insufficient geometric discriminability.

Method: The proposed GeoMix framework strengthens geometric discriminability at three levels: locally, with directional and distance-aware embeddings; globally, with learnable context nodes; and at the training level, with Mix-Training across multiple keypoint detectors.

Result: GeoMix sets a new state of the art among descriptor-free methods, reducing 75th-percentile rotation error by 89% and translation error by up to 90% over the previous best, while generalizing zero-shot to unseen detectors and narrowing the gap to descriptor-based pipelines.

Conclusion: The proposed GeoMix framework effectively improves the accuracy of descriptor-free visual localization, making it a viable alternative to descriptor-based pipelines.
...
TL;DR: A novel task, Active Panoramic Referring Segmentation (APRS), is introduced, requiring an agent to actively explore a 360° environment to segment an object specified by a user instruction. A memory-augmented agent, PanoSeeker, is proposed to tackle this task.


<details>
  <summary>Details</summary>
Motivation: Existing referring segmentation models are limited in their applicability to Embodied AI, as they passively process static images from fixed perspectives, whereas agents need to perform active perception in continuous 360° environments.

Method: PanoSeeker integrates a Vision-Language Model (VLM) with EgoSphere, an explicit spatial visual memory, to progressively integrate sequential local observations into a unified 360° representation, enabling efficient and non-redundant search trajectories.

Result: PanoSeeker achieves superior search efficiency and segmentation accuracy on the newly established APRS benchmark, significantly outperforming adapted state-of-the-art baselines.

Conclusion: The proposed PanoSeeker demonstrates the effectiveness of active panoramic referring segmentation in Embodied AI, with potential applications in real-world scenarios requiring active perception and exploration.
...
<details>
  <summary>Details</summary>
Motivation: To investigate the mechanism behind the improvement from SRA to Self-Flow, specifically whether the gain comes from interactions between tokens at different noise levels or data augmentation along the noise dimension.

Method: Introducing Attention Separation, which blocks attention between tokens assigned to different noise levels, to disentangle the factors of interaction and data augmentation.

Result: Removing interaction between tokens at different noise levels does not degrade performance and can even improve it, suggesting that the improvement mainly comes from data augmentation. Attention Separation itself provides an augmentation effect by expanding the training data.
...
TL;DR: A simple Diffusion Transformer model operating on raw 3D point map patches and conditioned on image tokens outperforms complex latent-based diffusion models in single-image 3D reconstruction.


<details>
  <summary>Details</summary>
Motivation: The authors aim to simplify state-of-the-art single-image 3D reconstruction methods by eliminating complex hybrid architectures and loss functions.

Method: The authors propose a minimalist pixel-space Diffusion Transformer built on a plain ViT, operating directly on raw 3D point map patches and conditioned on image tokens from a pre-trained DINOv3.
...
Conclusion: The authors conclude that architectural overhead and intricate loss formulations are unnecessary for single-image 3D reconstruction, and that a simple Diffusion Transformer model can achieve superior results.
...
<details>
  <summary>Details</summary>
Motivation: The high cost of constructing diverse datasets and the limited scalability of existing methods make arbitrary user-defined modality-to-4D (X-to-4D) generation challenging.

Method: Align4D introduces three key techniques: Object Distance Alignment, Motion-Geometry Joint Alignment, and Asynchronous Optimization, to translate any-modal input into coherent video-3D pairs.

Result: Experiments on X4D and Consistent4D demonstrate that Align4D achieves state-of-the-art quality and consistency in X-to-4D generation.

Conclusion: Align4D provides a flexible framework for X-to-4D generation, addressing the challenges of constructing diverse datasets and limited scalability of existing methods.
...
<details>
  <summary>Details</summary>
Motivation: Existing world models have limitations in terms of physical dynamics and pixel rendering, and require continuous visual observation to sustain motion.

Method: WorldDirector decouples semantic motion orchestration from visual generation, using an LLM to coordinate 3D trajectories with camera movements, and then employs these orchestrated trajectories as control signals for video generation.

Result: The framework supports the synthesis of complex and extended events with unprecedented controllability and persistent dynamic object memory, preserving the exact visual identities of dynamic entities even when they re-enter the scene after prolonged periods out of view.

Conclusion: WorldDirector provides a highly controllable video world model framework with improved physical logic and appearance stability.
...
TL;DR: The paper introduces TokenScope, an interactive tool for understanding how Large Language Models (LLMs) make decisions during code generation, providing insights into token-level metrics, attention patterns, and structural information.


<details>
  <summary>Details</summary>
Motivation: The challenge of understanding token-level decisions made by LLMs during code generation, which is crucial for both researchers and practitioners, but currently lacks effective tools for insight and exploration.

Method: The development of TokenScope, an interactive interpretability and analysis tool that exposes token-level metrics, attention patterns, and structural information during code generation, supporting features like token replacement and counterfactual branching.

Result: TokenScope enables systematic investigation of LLM behavior during code generation by unifying decoding-time signals with structural program analysis.

Conclusion: The paper concludes that TokenScope offers a solution for exploring and understanding LLM decisions during code generation, enhancing the interpretability and analysis of decoder-based LLMs.
...
TL;DR: The authors propose a provenance-based framework and a multi-stage pipeline called ProvenanceGuard to detect and prevent misalignment in LLM agents' actions, reducing error rates and intervention burden.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the issue of misalignment in LLM agents, where their actions deviate from the user's intent, potentially leading to harmful consequences, and to improve upon existing runtime guardrails that rely on an LLM-as-a-judge paradigm.

Method: The authors propose a provenance-based conceptual framework and ProvenanceGuard, a multi-stage pipeline that analyzes the agent's action for three types of misalignment before the selected tool is executed, only allowing the action to take place when it is considered aligned with the user's input query.

Result: The proposed approach reduces error rate on misaligned traces from 42.9% to 1.8% on Agent-SafetyBench and from 32.1% to 17.3% on WorkBench, while reducing intervention burden on task-successful traces from 30.5% to 12.8% and introducing no statistically significant increase in unnecessary interventions on aligned traces.

Conclusion: The results demonstrate that structured, provenance-based reasoning provides an effective and practical foundation for safeguarding LLM agents from misalignment.
...
TL;DR: The paper proposes Kara, a sliding-window KV cache compression method, and KvLLM, an inference framework, to reduce memory overhead and improve throughput in reasoning language models.


<details>
  <summary>Details</summary>
Motivation: Existing KV cache compression methods have limitations, including threshold-triggered compression policies and rigid chunk preservation, which can lead to limited throughput improvement, information loss, and reduced performance.

Method: The authors propose Kara, a sliding-window KV cache compression method that uses bidirectional attention to select informative KV pairs and a Token2Chunk module to preserve important semantic information. They also develop KvLLM, an inference framework built upon vLLM.

Result: Extensive experiments demonstrate consistent performance improvements of the proposed Kara and KvLLM, reducing KV cache memory usage and improving output throughput.

Conclusion: The paper concludes that Kara and KvLLM effectively address the limitations of existing KV cache compression methods, providing improved throughput and reduced memory overhead for reasoning language models.
...
TL;DR: The paper proposes SPARCLE, a speaker-aware grapheme representation model for text-to-speech synthesis, which improves generation quality, especially in low-resource settings.


<details>
  <summary>Details</summary>
Motivation: The limitations of phoneme-based systems, which rely on grapheme-to-phoneme (G2P) systems that fail to capture speaker-specific acoustic variation, and the need for better performance in low-resource settings.

Method: SPARCLE is trained with a contrastive objective to align graphemes with corresponding Wav2Vec2 acoustic representations while conditioned on speaker identity.
...
Conclusion: SPARCLE offers a promising approach for text-to-speech synthesis, particularly in low-resource settings, by effectively capturing speaker-specific acoustic variations.
...
Method: The researchers tested a central structural mechanism involving BPE tokenization, conducted experiments on five model families, and used various methods including optimization targeting safety-token fragmentation, activation patching, alignment-data scan, and targeted-mutation experiments.

Result: The study found that an optimization targeting safety-token fragmentation can flip the first-token refusal trigger on 80-100% of refused HarmBench prompts, with 48% of those flips producing genuinely harmful outputs. The results also showed that no DPO configuration achieves seed- and pool-stable ASR closure, and SFT trained on fragmented prompts closes ASR on 3/5 families but only via global collapse.

Conclusion: The study concludes that character-level perturbations can bypass safety alignment in modern LLMs by fragmenting safety-critical words into sub-word pieces during BPE tokenization, and that current alignment datasets lack intentionally fragmented inputs. The findings also highlight the challenges in defending against such perturbations.
...
TL;DR: The paper reveals that Count-based F1 can increase without improving span localization in LLMs, introducing 'F1 Inflation'. It presents ErrorBench, a stress-test protocol, and finds that anchored prompts cause significant F1 Inflation across six LLMs.


<details>
  <summary>Details</summary>
Motivation: To investigate the reliability of Count-based F1 as a proxy for LLM error-detection quality, specifically addressing the issue of F1 Inflation.

Method: Introduction of ErrorBench, a controlled stress-test protocol, evaluating six LLMs under five prompt conditions over 4,290 responses from 143 CoNLL-2014 passages.

Result: Anchored prompts produce up to 0.79 points of F1 Inflation. The Blind-to-Anchored prompt shift raises Count-F1 by +0.21 while raising multi-reference ERRANT F0.5 by only +0.04.

Conclusion: LLM proofreading and document-review evaluations should avoid pre-populated error counts and report span-aware metrics alongside count-based metrics to ensure accurate assessment of error-detection quality.
...
TL;DR: RAGP, a novel prompt compression method, uses graph pruning to efficiently compress text while preserving important information, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing prompt compression methods fail to capture the distributed nature of important information in text, which is often spread across multiple locations and connected through local and global relations.

Method: RAGP formulates prompt compression as Redundancy-Aware Graph Pruning on a multiplex graph, using Levy walks to identify non-redundant nodes.

Result: RAGP achieves an average score of 49.3 under a 4x compression ratio, outperforming existing LLM-based compression methods and vision-based text compression paradigms.

Conclusion: RAGP is an effective prompt compression method that preserves important information while achieving high compression ratios.
...
TL;DR: The paper introduces the Office Comprehension Bench (OCB), a benchmark for evaluating LLM systems on Word, Excel, and PowerPoint comprehension, and presents results showing current systems struggle with expert-level reasoning.


<details>
  <summary>Details</summary>
Motivation: The motivation is to create a public benchmark for evaluating LLM systems on office document comprehension, specifically on native file formats and their variants, and to test their ability to perform expert-level reasoning.

Method: The OCB consists of two tracks: File Fidelity Q&A and Domain Q&A. The evaluation uses an ensemble of LLM judges to score responses against atomic, binary-gradable claims.

Result: The strongest frontier system achieved only about 59.3% on Domain Q&A, with limited gains from increased thinking depth or higher product tiers.

Conclusion: The paper concludes that current LLM systems struggle with expert-level reasoning on office document comprehension, and releases the dataset, evaluation tooling, and a public leaderboard to facilitate further research.
...
TL;DR: RuleChef uses large language models to generate executable rules for NLP tasks, improving them through human feedback and example data.


<details>
  <summary>Details</summary>
Motivation: To create a framework that generates efficient, deterministic, and inspectable rule systems for NLP tasks using large language models.

Method: RuleChef utilizes LLMs to synthesize rules from task descriptions and labeled examples, iteratively improving them with additional examples and human feedback.

Result: A fast, deterministic, and inspectable rule system for NLP tasks such as text classification and Named Entity Recognition.

Conclusion: RuleChef offers a promising approach to creating efficient NLP rule systems, made available as open-source software.
...
TL;DR: The paper proposes TurnNat, a framework for automatically evaluating turn-taking naturalness in spoken dialogue systems.


<details>
  <summary>Details</summary>
Motivation: Existing evaluations of turn-taking naturalness rely on human judgments or behavior-specific timing metrics, which makes it difficult to compare different types of timing failures.

Method: The authors propose a likelihood-based framework called TurnNat, which uses a causal turn-taking prediction model to estimate future voice-activity states and measures timing atypicality using negative log-likelihood.

Result: Experiments on a controlled perturbation benchmark show that TurnNat successfully identifies unnatural turn-taking perturbations across heterogeneous timing failures.

Conclusion: The proposed framework, TurnNat, provides a unified approach for automatic evaluation of turn-taking naturalness in spoken dialogue systems.
...
TL;DR: Introduced RusFinChain, a Russian-language symbolic benchmark for verifiable Chain-of-Thought (CoT) reasoning in finance, and evaluated 8 LLMs, revealing a substantial reasoning gap.


<details>
  <summary>Details</summary>
Motivation: Most benchmarks neglect intermediate reasoning steps in financial analysis, and existing ones are limited to English or lack step-level supervision.
...
Result: Models achieved Hard F1 of ~0.65 for step alignment, but only ~29% of final answers were correct, and the new metrics showed stronger correlation with final-answer correctness.

Conclusion: RusFinChain and the enhanced metrics can foster verifiable financial AI for the Russian-speaking community, and reveal a substantial reasoning gap in current LLMs.
...
<details>
  <summary>Details</summary>
Motivation: Current methods for aligning language models with human values struggle with trade-offs between conflicting preferences, and exploration uncertainty can cause overlapping reward distributions.

Method: The authors propose MI-EPO, an information-theoretic framework that maximizes joint conditional mutual information among responses, feedback, and preference vectors.

Result: Experiments show that MI-EPO improves alignment between responses and preference vectors, makes outputs more controllable, and achieves stable trade-offs across objectives.

Conclusion: MI-EPO provides a unified approach for multi-objective exploration and alignment, leading to improved performance in safe alignment and helpful assistant tasks.
...
TL;DR: A new method, MultAttnAttrib, is proposed for attributing generated answers to evidence in multimodal settings, outperforming existing methods and achieving lower latency.


<details>
  <summary>Details</summary>
Motivation: The need for accurate attribution of generated answers to evidence in AI assistants for user trust and model safety, particularly in multimodal settings.

Method: MultAttnAttrib, a training-free attribution-generation method leveraging prefill pass, selected attention heads, and calibrated thresholds.

Result: MultAttnAttrib outperforms existing attribution-generation methods, including prompting-based approaches and GPT 5.4, with improved attribution accuracy and lower latency.

Conclusion: MultAttnAttrib is an effective and efficient method for multimodal attribution, producing attributions with up to one-seventh of the direct inference latency compared to prompting.
...
TL;DR: The paper introduces ISOSCI, a benchmark to evaluate the reasoning ability of large language models (LLMs) separate from domain knowledge retrieval. It challenges the assumption that chain-of-thought reasoning improves short-horizon procedural scientific problem-solving.


<details>
  <summary>Details</summary>
Motivation: To disentangle reasoning ability from domain knowledge retrieval in LLM evaluation, and to assess the effectiveness of chain-of-thought reasoning in scientific problem-solving.

Method: The authors introduce ISOSCI, a benchmark of isomorphic cross-domain science problem pairs, and evaluate several LLMs across different model families.

Result: The results show that most reasoning-mode gains are knowledge-dependent (91.3%), and that chain-of-thought reasoning provides limited accuracy gain (< 5 percentage points) on highly capable models. A specialized reasoning model underperforms on ISOSCI.

Conclusion: The study concludes that benchmark choice significantly impacts conclusions about reasoning utility, and that ISOSCI provides a new framework for evaluating LLM reasoning abilities.
...
TL;DR: FaithMed is a framework that improves medical LLMs' faithful reasoning by incorporating evidence-based medicine principles and reinforcement learning.


<details>
  <summary>Details</summary>
Motivation: Current medical LLMs lack transparent justification and reliable evidence in their clinical decisions.

Method: FaithMed combines clinician-designed rubrics with reinforcement learning using step-level process reward assignment and advantage grouping.

Result: FaithMed improves over baselines by 9% on average and 5.8% over outcome-only RL, with increased evidence-based medicine rubric scores.
...
TL;DR: The paper introduces Grounded Optimization, a framework to reduce hallucinations in large language models (LLMs) used for resume optimization, and presents results showing significant reduction in detected hallucinations.


<details>
  <summary>Details</summary>
Motivation: The application of large language models to resume optimization introduces specific types of hallucination failures, including anachronistic technology injection, cross-domain terminology contamination, structural mutation, and content fabrication.

Method: The authors propose a five-layer framework called Grounded Optimization, which includes temporal context validation, deterministic contamination detection, structural invariant enforcement, prompt-level grounding, and an evaluator agent.

Result: The framework reduces detected hallucinations by 50-95% across various conditions, with prompt-level grounding alone achieving zero detected hallucinations under certain conditions.

Conclusion: The proposed Grounded Optimization framework effectively mitigates hallucinations in LLMs used for resume optimization, and the authors release their contamination taxonomy, evaluation code, and raw data.
...
Method: The paper explores two approaches: (a) joint prediction of scales, and (b) finding a middle ground between classification and regression.
...
TL;DR: The paper evaluates Mamba, a state space model, for automatic speech recognition (ASR) in seven South African languages, comparing it to a Conformer baseline. Mamba achieves similar accuracy with fewer resources and faster training. The study also explores multilingual ASR, finding that multilingual training improves performance, but adding explicit language information only improves cross-corpus robustness.


<details>
  <summary>Details</summary>
Motivation: The effectiveness of recent ASR models, such as Mamba and Conformer, in African languages remains underexplored. This work aims to evaluate Mamba for ASR in seven South African languages and compare it to a Conformer baseline.

Method: The study conducts monolingual and multilingual ASR experiments using Mamba and Conformer models. In monolingual experiments, each model is trained on 50 hours of speech per language. In multilingual experiments, the study tests three extensions: training with language-family information, multitask learning with a CTC ASR objective and a language identification (LID) head, and ablation studies in low-resource settings.

Result: Mamba achieves similar recognition accuracy to Conformer while using fewer computational resources and training faster. Multilingual training consistently improves performance over monolingual training. However, adding explicit language information does not improve in-domain performance but does improve cross-corpus robustness.

Conclusion: The study demonstrates the effectiveness of Mamba for ASR in South African languages and highlights the importance of multilingual training. The findings also suggest that language embeddings act as task-specific control vectors rather than capturing linguistic similarity.
...
TL;DR: The Parameter Golf challenge aimed to improve language models under a strict 16MB artifact budget. Submissions were analyzed, and a taxonomy of optimization techniques was built. The best score improved by 13.6% to 1.058 BPB.


<details>
  <summary>Details</summary>
Motivation: To investigate how far a language model can improve under a strict artifact budget, specifically within 16MB and trained in under ten minutes on 8xH100 SXM GPUs.

Method: Analyzing 2,037 pull requests and 1,430 clean scored submissions from the Parameter Golf contest, and building a taxonomy of 84 optimization techniques.

Result: The verified leaderboard score dropped from 1.2244 to 1.058 BPB, a 13.6% reduction. Most gains in techniques shrink across competitive submissions, with few methods improving performance across stacks.

Conclusion: The study shows that significant improvements in language models can be achieved under strict artifact budgets by combining multiple optimization techniques, and identifies the most effective methods.
...
<details>
  <summary>Details</summary>
Motivation: The authors aim to investigate the potential of in-context retrieval using language models as an alternative to traditional vector-based retrieval methods, particularly on large corpora.

Method: The authors propose BlockSearch, a 0.6B LM retriever with architectural and training modifications to improve its performance and length-generalization capabilities. They also introduce length-aware adjustments to the attention softmax and document-level sparse attention.

Result: The authors' model matches dense retrieval on widely studied benchmarks (e.g., MS MARCO and NQ) and outperforms the concurrent model MSA, while being 7 times smaller. It also significantly outperforms dense retrieval on tasks requiring different notions of similarity, such as LIMIT.
...
TL;DR: The authors propose a Q-learning framework called Dialogue Policy Selection (DiPS) to dynamically select persuasion strategies for large language models in high-stakes scenarios, such as persuading a resident to evacuate in a fire-rescue scenario.


<details>
  <summary>Details</summary>
Motivation: Large Language Models (LLMs) struggle with persuasion in high-stakes scenarios, and a one-size-fits-all approach is not effective due to individual personalities and concerns.

Method: The authors propose a Q-learning framework called Dialogue Policy Selection (DiPS) that trains a critic to select a persuasion policy at each turn based on the resident's recent utterances.
...
Conclusion: The proposed DiPS framework is effective in adapting persuasion strategies to the evolving conversational context, leading to improved evacuation success in high-stakes scenarios.
...
TL;DR: The paper proposes the Adaptive Pedagogical Vigilance (APV) framework to improve Large Language Models' (LLMs) ability to reason about pedagogical intent in instructional communication, particularly in translation pedagogy.
...
Method: The APV framework uses a Bayesian Pedagogical Intent Inference Engine (PIIE) to model how instructors select content to maximize pedagogical utility and how vigilant learners should inversely reason about latent instructional configurations.

Result: Experiments on leading LLMs show that APV substantially improves model vigilance, achieving strong discrimination between pedagogical and exposure-based content, high correlation with human judgments (r=0.958), and robust performance on naturalistic data.

Conclusion: The work establishes a unified framework for assessing and enhancing LLMs' understanding of pedagogical motives, advancing the development of more reliable AI-assisted learning systems.
...
TL;DR: ProWAFT, a fault-tolerance framework for FPGA-based CNN accelerators, selectively applies TMR to minimize latency, energy, and reliability risk.


<details>
  <summary>Details</summary>
Motivation: Transient faults in SRAM-based FPGAs can compromise reliability of CNN inference at the network edge, and traditional fault-tolerance methods incur substantial performance and energy overhead.

Method: ProWAFT uses partial reconfiguration to selectively apply TMR across reconfigurable partitions, quantifying workload criticality, modeling fault propagation and reconfiguration overhead.
...
Conclusion: ProWAFT provides a proactive workload-aware fault-tolerance solution for FPGA-based CNN accelerators, balancing reliability, latency, and energy efficiency.
...
TL;DR: The paper explores Fixed-Source Synthesis (FSS) for scaling synthetic data, deriving a scaling law and comparing it to Source Expansion (SE) and Rejection Sampling (RS).
...
Method: The authors adapt the rectified scaling law to FSS, varying the per-question response budget under Rejection Sampling (RS) with a fixed seed-question pool and teacher model.

Result: The derived scaling law predicts performance at high budgets; FSS and SE are comparable at small budgets, but adding seed questions outperforms spending on more responses at large budgets.

Conclusion: FSS is a bounded scaling axis and a controlled setting for comparing synthesis protocols, and neither synthesizing additional questions nor varying the synthesis protocol outperforms plain RS at matched budgets.
...
TL;DR: The paper proposes Joint Speech-Text Interleaved Pretraining (JSTIP) to improve automatic speech recognition (ASR) by integrating large language models (LLMs) with speech inputs, achieving better entity accuracy and domain adaptation.


<details>
  <summary>Details</summary>
Motivation: The authors aim to clarify the benefits of LLM priors for ASR and address the underutilization of textual knowledge in simple speech-text joint training.

Method: The authors propose JSTIP, an ASR-oriented pretraining strategy that constructs interleaved speech-text sequences for speech-LLM architectures.

Result: Experiments show consistent entity accuracy improvement using JSTIP compared to baselines, and competitive performance with open-source ASR and Speech-LLM systems in medical entity recognition.

Conclusion: JSTIP improves ASR by leveraging textual pretraining and domain text data, reducing the speech-text modality gap, and preserving LLM generative prior.
...
<details>
  <summary>Details</summary>
Motivation: Decoder-only LLMs suffer from asymmetric information flow due to causal attention, where later tokens have more contextual grounding than earlier ones. Full prompt repetition can improve performance but increases computational costs.

Method: The authors propose PartRep, a selective augmentation method that appends only the most informative tokens of the prompt, selected based on token-wise negative log-likelihood (NLL). A lightweight gate is trained to predict high-NLL tokens from early-layer hidden states.
...
Method: Extracting steering vectors for various preferences using the PLUME writing personalization benchmark and evaluating them on summarization and email-writing tasks across two open-source models.

Result: Steering effectiveness varies substantially across traits, degrades when transferred to downstream tasks, and drops significantly as more vectors are composed.
...
TL;DR: The paper introduces PairCoder, a method for generating structured artifacts like charts and 3D scenes using large language models. It uses a two-agent pair programming approach, with one agent writing the program and the other reviewing it against verification evidence. This approach improves the generation of verifiable artifacts across various benchmarks.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the brittleness of single-pass inference in generating structured artifacts using large language models. The authors aim to improve the reliability and verifiability of the generated artifacts by incorporating a review process into the toolchain.

Method: The method presented in this paper is PairCoder, a two-agent pair programming approach. One agent, the Driver, writes the program, while the other agent, the Navigator, reviews it against verification evidence such as diagnostics, execution results, and renderings. The two agents switch roles when errors persist.

Result: The results show that PairCoder improves essentially every benchmark whose artifact is verifiable, with significant improvements in metrics such as Blender scene executability (from 0.20 to 0.78) and TikZ compile rate (up 10 to 30 points on every model). The method achieves these improvements at 2.9 to 9.2 times the single-model cost (about 7 times overall).

Conclusion: The authors conclude that pair programming, as realized in PairCoder, is a reliable recipe for verified code-driven generation. They find that the improvements are most significant where the toolchain provides an informative oracle and the baseline leaves room for improvement.
...
Method: Analyzing 122 languages in UD and SUD (version 2.17) to examine dependency length minimization across different syntactic relation types.

Result: The study found that functional dependencies are universally short (mean 1.71) and invariant, while lexical dependencies are longer (mean 2.87), highly variable, and constrained by word-order typology.

Conclusion: The grammar does the work of minimization by scaffolding sentences with local functional attachments, leaving processing pressures to determine the ordering of lexical heads.
...
TL;DR: The paper presents TUDUM, a pipeline for adapting a large language model to perform reasoning in Turkish, with a focus on generating explicit Turkish reasoning traces.


<details>
  <summary>Details</summary>
Motivation: The goal is to enable a model to not only answer Turkish prompts in Turkish but also to generate reasoning traces in Turkish, rather than relying on English-centric internal workings.

Method: The pipeline starts with a pre-trained Qwen model, applies supervised fine-tuning (SFT) on Turkish reasoning examples using LoRA adapters, and then uses reinforcement learning (RL) on a Turkish mathematics environment.

Result: The results show mixed performance: SFT improved Turkish reasoning consistency but reduced accuracy, while RL recovered some mathematical performance but did not uniformly improve all benchmarks.

Conclusion: The contribution is a technically honest Turkish-thinking reasoning pipeline and evaluation, with the released model publicly available, though not claiming state-of-the-art Turkish reasoning performance.
...
TL;DR: The paper introduces a new dataset, AIriskEval-edu-db2, for training and evaluating auditors using LLMs for risk assessment in K-12 instructional content. The dataset includes human and LLM-generated explanations for 170 ScienceQA questions, annotated with a comprehensive risk rubric and explainability annotations.


<details>
  <summary>Details</summary>
Motivation: The motivation is to develop a dataset and models for explainable pedagogical risk assessment in instructional content, preserving privacy in educational auditing and assessment tasks.

Method: The method involves creating a dataset of human and LLM-generated explanations, annotating them with a comprehensive risk rubric and explainability annotations, and evaluating state-of-the-art models and a lightweight local Llama 3.1 8B model in pedagogical risk detection and explainability assessment.

Result: The results show that supervised fine-tuning on AIriskEval-edu-db2 enables a locally deployable model to approach or outperform stronger frontier models in pedagogical risk detection and explainability assessment.

Conclusion: The paper concludes that AIriskEval-edu-db2 is a valuable resource for training and evaluating auditors using LLMs, and that a locally deployable model can achieve strong performance in pedagogical risk assessment tasks while preserving privacy.
...
TL;DR: The paper describes NAVER LABS Europe's submission to IWSLT 2026, achieving tied first place in the short track with an improved speech processing system.


<details>
  <summary>Details</summary>
Motivation: To improve the performance of their speech processing system in the instruction-following speech processing short track at IWSLT 2026.

Method: Updating their multi-stage training pipeline by replacing the speech projector with SpeechMapper and introducing a synthetic SQA dataset, fakACL.

Result: The system achieved tied first place in the overall short track ranking, outperforming last year's best system while being more compact and relying on a weaker LLM backbone.

Conclusion: The combination of an improved speech projection mechanism and domain-specific synthetic data leads to better performance in speech processing tasks.
...
TL;DR: The study examines the effectiveness of rewriting inputs to improve frozen downstream models in NLP, specifically in dialogue discourse parsing. It finds that unsupervised rewriting methods are less reliable than supervised settings and often introduce more errors. A parser-aware clarifier reduces errors but still struggles to produce effective clarifications.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to improve the accuracy of frozen downstream models in NLP pipelines by rewriting inputs, specifically in the context of dialogue discourse parsing (DDP), without relying on supervised clarification models.

Method: The study uses zero-shot prompting and feedback from a frozen parser to enable a clarifier to rewrite fragmentary or underspecified utterances. It evaluates the approach across three SDRT datasets and multiple parsers, including a parser-aware clarifier trained with GRPO.

Result: The results show that last-utterance clarification is less reliable than suggested by supervised settings. Parser-agnostic rewriting often introduces more regressions than repairs. A best-of-8 rewriting analysis reveals a practical ceiling, with a large fraction of errors not repairable through input rewriting alone. The parser-aware clarifier reduces regressions by up to 37% but still fails to produce selectivity-aware clarifications.

Conclusion: The study concludes that clarification should be viewed as a selective intervention problem. It identifies rewritability prediction, deciding whether an utterance is repairable before intervention, as the key missing capability for input-side optimization of frozen discourse parsers, and a critical direction for improving agentic pipelines more broadly.
...
TL;DR: The authors propose a framework to audit TTS systems for preserving sound contrasts in languages, and apply it to assess Meta's MMS TTS system on Assamese vowel harmony.


<details>
  <summary>Details</summary>
Motivation: Current TTS systems may sound natural but fail to preserve sound contrasts crucial for word meaning and grammatical forms, which standard metrics like MOS do not evaluate.

Method: A classifier-based framework is proposed, which uses human speech as a benchmark to evaluate TTS output against language-specific phonological patterns.

Result: The framework reveals that Meta's MMS TTS system fails to preserve the [+ATR] specification for mid vowels in 1/3 of the tokens, and that predicted ATR labels are more accurate than transcription labels at the word level.

Conclusion: The proposed framework provides task-specific diagnostics for evaluating TTS systems and can generalize to other phonological contrasts with measurable acoustic cues.
...
TL;DR: The paper introduces Object Aligner (OA), a Python library for scoring the similarity between two JSON objects, addressing limitations of existing methods.


<details>
  <summary>Details</summary>
Motivation: Measuring the similarity between JSON outputs and a gold reference is challenging due to the complexity of JSON structures and the limitations of existing methods such as exact match, text similarity, and LLM judges.

Method: The Object Aligner uses a recursive tree alignment approach, with the Hungarian algorithm for unordered collections and sequence alignment for ordered ones, and awards partial credit based on the schema granularity. It also introduces referential alignment to handle complex structured data with graphs or hypergraphs.

Result: The Object Aligner provides a deterministic and adaptable method for scoring JSON similarity, and its use as a reward in the GEPA prompt optimizer shows positive or neutral results across all datasets.

Conclusion: The Object Aligner offers a solution to the challenge of measuring JSON similarity, providing a deterministic, adaptable, and effective method for scoring JSON outputs against a gold reference.
...
TL;DR: This study investigates if contextualized embeddings (CEs) can predict spoken word duration and f0 contours of Mandarin words in conversational speech.


<details>
  <summary>Details</summary>
Motivation: To explore the predictability of spoken word duration and f0 contours of Mandarin words from their contextualized embeddings.
...
Result: CEs are predictive of duration at both type and token levels, and predicted durations can accurately back-transform f0 contours.
...
TL;DR: The paper introduces EduArt, a benchmark for evaluating art-historical knowledge and visual reasoning in multimodal LLMs, revealing that models' performance varies greatly across different formats and tasks.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the limitations of current benchmarks for evaluating large language models, which often focus on aggregate measures and neglect the models' behavior within specific disciplines, such as art history.

Method: The method used in this paper involves introducing EduArt, a benchmark comprising 871 human-authored questions from Italian secondary-school exercises and US Advanced Placement Art History exams, and evaluating 12 models from six provider families under two conditions: default answer-only and motivation condition requiring written justification.

Result: The results show that models' performance varies greatly across different formats, with multiple-choice accuracy saturating near ceiling for some models, but dropping significantly for open completion and error identification tasks, indicating that art-historical knowledge and the ability to deploy it are distinct capabilities.

Conclusion: The conclusion of this paper is that single-format benchmarks overestimate what models can reliably do, and that mapping the capability profile of multimodal LLMs is a precondition for their responsible use in art-historical scholarship.
...
TL;DR: The paper introduces OpenSafeIntent, a benchmark to evaluate model safety across intent shifts, revealing that models often fail to remain safe when faced with different variants of the same task.


<details>
  <summary>Details</summary>
Motivation: Current methods for evaluating model safety are limited, as they rely on isolated prompts and do not account for variations in intent, which can lead to models appearing safe on average but failing in specific scenarios.

Method: The authors introduce OpenSafeIntent, a benchmark of controlled prompt-sets with benign, dual-use, and malicious variants of the same task, to assess whether models calibrate assistance across intent shifts.

Result: The study finds that models often fail to remain safe across matched intent variants, dual-use behavior is brittle under paraphrase, and high-level answers on risky topics are not reliably safe.

Conclusion: The paper concludes that safe completion should be evaluated as intent-calibrated behavior over controlled task variants, rather than as a single safety-helpfulness tradeoff over independent prompts.
...
TL;DR: The paper introduces SPLIT, a benchmark to evaluate large language models' (LLMs) ability to generate emotionally grounded responses in crisis situations across languages, focusing on English and Ukrainian. It assesses LLMs like Gemini-2.5-Flash, LLaMA-3.3-70B-Instruct, and DeepSeek-V3, finding performance degradation in Ukrainian and highlighting the need for culturally tailored benchmarks.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the underexploration of cross-lingual abilities of LLMs in emotional-support contexts and crisis-related situations, particularly in low-to-mid-resource languages.

Method: The paper proposes a 500-prompt benchmark, SPLIT, across five categories and evaluates LLMs on Empathetic Accuracy, Linguistic Naturalness, and Contextual & Cultural Grounding in both English and Ukrainian languages.

Result: The results show that Gemini-2.5-Flash and LLaMA-3.3-70B-Instruct degrade when transitioning to Ukrainian, while DeepSeek-V3 remains comparatively stable. Human and AI evaluators show weak agreement on empathy and naturalness but diverge on cultural grounding.

Conclusion: The paper concludes that producing text in a language is not equivalent to producing culturally appropriate emotional support. The findings suggest the need for more culturally tailored benchmark designs and a stronger emphasis on human-centered evaluation.
...
TL;DR: HaloGuard 1.0 is an open-source, state-of-the-art model for input safety, achieving high performance on English and multilingual benchmarks with a smaller model size.


<details>
  <summary>Details</summary>
Motivation: To improve input safety with a more efficient and effective model, addressing the limitations of current guard models.

Method: HaloGuard 1.0 uses a constitutional-classifier paradigm with a safety constitution guiding synthetic data generation, a two-tier harmless design, and multilingual materialization.

Result: HaloGuard 1.0-0.8B and 1.0-4B variants achieve state-of-the-art performance on seven prompt-safety benchmarks, with average F1 scores of 90.9 and 92.1, respectively.

Conclusion: HaloGuard 1.0 demonstrates superior performance and efficiency, with a structured adjudication indicating that most missed-harm cases are due to benchmark mislabels rather than model misses.
...
TL;DR: The paper proposes SpeechCombine, a method to train speech language models (SLMs) without instruction tuning, achieving effective transfer of knowledge from text large language models (LLMs) to the speech domain.


<details>
  <summary>Details</summary>
Motivation: Instruction tuning for speech language models (SLMs) is challenging due to the need to learn a new modality and a wide range of speech-specific instructions, and existing SLM training approaches are difficult to scale due to the length of speech sequences.

Method: The method involves continuous pre-training on speech utterances to obtain a speech-adapted model from a text LLM base model, and then combining its weights with the weight difference between the instruction-tuned and base versions of the text LLM.

Result: The proposed method preserves the knowledge and capabilities of the original text LLM and effectively transfers them to the speech domain, suggesting a new direction for SLM training that avoids reliance on massive speech data.

Conclusion: The paper concludes that the proposed SpeechCombine method is a viable approach for training SLMs without instruction tuning, achieving effective transfer of knowledge from text LLMs to the speech domain.
...
TL;DR: The use of LLM-as-a-Judge in multilingual and low-resource language settings is problematic due to LLMs' limited proficiency and lack of human validation, leading to inconsistent evaluation outcomes.


<details>
  <summary>Details</summary>
Motivation: The motivation is to investigate the use of LLM-as-a-Judge in multilingual and low-resource language settings, highlighting the scope of the problem and current practices.

Method: The method involves analyzing 650 ACL Anthology papers mentioning LLM-as-a-Judge, with a focus on 33 papers that focus on low-resource or multilingual settings.

Result: The results show inconsistent evaluation outcomes, overtrust in LLM judgments, and reliance on a single judge model per study in multilingual and low-resource settings.

Conclusion: The conclusion is that the NLP community needs to be cautious when using LLM-as-a-Judge in multilingual and low-resource settings, and recommendations are provided to improve its use.
...
TL;DR: BamiBERT, a BERT-based model for Vietnamese, outperforms PhoBERT with improved context length and direct raw input processing, achieving state-of-the-art results on 8 benchmarks.
...
Method: BamiBERT was trained from scratch on a 129GB Vietnamese text corpus for 20 epochs, supporting a context length of up to 2048 tokens and operating directly on raw input.

Result: BamiBERT achieves the best score on 11 of 15 metrics and the second-best on 3 others across 8 Vietnamese benchmarks, setting a new state of the art.

Conclusion: BamiBERT demonstrates strong cross-domain generalization and sets a new state of the art among 'base'-sized Vietnamese encoders.
...
TL;DR: A new framework called CheckRLM is proposed to improve the reliability of Reasoning Language Models (RLMs) by detecting and correcting factual errors in the reasoning chain.


<details>
  <summary>Details</summary>
Motivation: RLMs have improved performance on complex tasks but are prone to factual errors, particularly in knowledge-intensive tasks.

Method: CheckRLM uses Retrieval-Augmented Generation (RAG) to extract factual claims from the reasoning chain, identify knowledge inconsistencies, and perform precise corrections using external knowledge.
...
Conclusion: CheckRLM offers a reliable and cost-effective solution for improving the accuracy of RLMs in complex tasks.
...
TL;DR: A new parser using CCG directed types outperforms AM-Parser, especially in directional categories.


<details>
  <summary>Details</summary>
Motivation: Previous SOTA parser (AM-Parser) does not encode direction, but several test categories require directional distinctions.

Method: Redesigning symbolic backend with CCG directed types, using BERT-base encoder and deterministic CKY + single linear decoder.

Result: The system achieves 75.9% LF exact match, surpassing AM-Parser (70.8%), and 90.7% with DeBERTa-v3-large encoder.

Conclusion: Directional representations improve parser performance, especially in directional categories, and shift the bottleneck to the neural layer.
...
TL;DR: The paper explores the intersection of literature and AI, proposing a framework for culturally literate AI through nuanced textual models and pluralistic interpretations.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the cultural implications of Large Language Models (LLMs) and to develop tools for building culturally literate AI, leveraging literary disciplines.

Method: The essay develops a layered framework, connecting critical theory with structural monolingualism, and applying world literature approaches to global AI textuality.
...
Conclusion: The paper emphasizes the natural intersections of literature and AI development, advocating for nuanced textual models and pluralistic interpretations of AI.
...
TL;DR: The paper describes the participation of HULAT2-UC3M in a shared task on multilingual Easy-to-Read translation, submitting three automatic Spanish runs with different workflows and evaluating their performance using various metrics.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to participate in the MER-TRANS 2026 shared task and evaluate the performance of different workflows for multilingual Easy-to-Read translation.

Method: The paper used three different runs: RUN1 and RUN2 used a LangGraph-based multi-agent workflow combining Gemini 2.5 Flash and RigoChat-7B-v2, while RUN3 used a RigoChat-based generate-evaluate-regenerate baseline. RUN2 activated an additional lexical-support layer based on a glossary and lexical resources.

Result: The results showed that RUN1 achieved the best SARI score of 44.0543, followed by RUN2 with 43.1049 and RUN3 with 38.5136. The signal-guided multi-agent routing outperformed the linear regeneration baseline, but adding lexical support did not automatically improve reference-based scores.

Conclusion: The conclusion is that signal-guided multi-agent routing is effective for multilingual Easy-to-Read translation, but further analysis is required to assess readability, factual consistency, and user-oriented adequacy.
...
TL;DR: The paper introduces MEDIAREF, a public knowledge store to support reproducible and low-cost evaluation of media background checks (MBCs) for fact verification, addressing the challenge of assessing evidence credibility in retrieval-augmented generation systems.


<details>
  <summary>Details</summary>
Motivation: Existing approaches to retrieval-augmented generation for automated fact-checking assume retrieved evidence is reliable, but real-world information can be conflicting, outdated, and from unreliable sources. Recent work on source-critical reasoning uses media background checks (MBCs) to assess evidence credibility, but generating MBCs relies on costly proprietary search APIs, limiting reproducibility.

Method: The authors introduce MEDIAREF, a publicly available knowledge store of web-sourced documents, and describe a reproducible methodology for constructing and updating the collection. They evaluate widely used LLMs on the MBC generation task and assess the quality of MBC generation using MEDIAREF.
...
Conclusion: The paper concludes that MEDIAREF enables reproducible, low-cost evaluation of MBC generation, addressing the challenge of assessing evidence credibility in retrieval-augmented generation systems.
...
TL;DR: The publication landscape of NLP research is shifting, with established authors and new authors increasingly publishing in general ML venues rather than traditional NLP venues.


<details>
  <summary>Details</summary>
Motivation: To investigate whether the disciplinary center of gravity in NLP research is shifting due to advances in Large Language Models (LLMs) and the blurring of lines between NLP and general Machine Learning (ML).

Method: Analyzing NLP research published from 2010 to 2026, comparing pre- and post-LLM eras, and using causal inference techniques to estimate the citation impact of publishing in general ML venues.

Result: A significant shift in publication venues for NLP research, with established authors losing share in flagship ACL venues and gaining share in general ML venues, and new authors increasingly publishing in general ML venues, which are found to confer a citation premium.

Conclusion: The study finds a migration of NLP research publication from traditional NLP venues to general ML venues, driven in part by the citation premium associated with publishing in general ML venues.
...
<details>
  <summary>Details</summary>
Motivation: The authors aim to understand what makes cultural measurement distinctively cultural in NLP work.

Method: The paper uses a theoretical framework drawing on Karen Barad's concept of the agential cut, and empirically examines three case studies on television and film dialogue, and three examinations of the apparatus itself.
...
Conclusion: The authors conclude that NLP work on culture is a material-discursive practice that participates in constituting cultural reality, and propose a new research program for culturally contingent and rigorous analysis.
...
TL;DR: The study investigates whether scaling up Large Language Models (LLMs) can improve the fidelity of social simulations, and finds that while scaling can improve most simulations, there are outliers where improvements are less reliable, particularly in low-resource domains.


<details>
  <summary>Details</summary>
Motivation: The current scaling paradigm in language modeling may not be sufficient to close the gaps in social simulation fidelity, and the authors aim to investigate this issue.

Method: The authors use scaling laws to study the relationship between LLMs' compute scale, general capability benchmarks, and social simulation fidelity in three sub-domains: opinion modeling, behavioral simulation, and longitudinal forecasting.

Result: The study finds strong compute scaling in all three settings, but also identifies outliers where scaling fails to improve model calibration, such as in behavior simulation tasks involving human cognitive biases and heuristics.

Conclusion: The study concludes that scale will improve social simulations in most settings, but outliers exist, and improvements will be less reliable in low-resource domains.
...
<details>
  <summary>Details</summary>
Motivation: To explore how narration qualities shape an audiobook's appeal and their interplay with genre, title, and audience.

Method: Extracting vocal and acoustic features from LibriVox using pre-trained audio models and analyzing their relationship with consumption data and their interplay with genre and title.

Result: Acoustic information alone has a robust association with appeal, even after accounting for title effects, and findings are validated using proprietary engagement metrics.

Conclusion: The study highlights the potential of data-driven insights to improve audiobook personalisation and narrator casting, and is the first systematic computational study linking narration qualities, genre, title, and audiobook consumption.
...
TL;DR: The paper proposes a novel reinforcement learning framework, VRRL, to improve the self-reflection capabilities of large vision-language models, enabling them to correct errors and improve performance on out-of-distribution images.


<details>
  <summary>Details</summary>
Motivation: Existing large vision-language models struggle with self-reflection, particularly when dealing with visual inputs and out-of-distribution images, which limits their ability to translate feedback into grounded corrections.

Method: The proposed VRRL framework consists of two components: (1) randomly masking trajectory prefixes during training to emphasize recovery from incorrect intermediate predictions, and (2) introducing buffered roll-ins from an experience replay buffer to expose the model to diverse failure states that it must learn to correct.

Result: The proposed method substantially improves average out-of-distribution accuracy over standard RL and reflection-oriented fine-tuning baselines on visual grounding tasks and spatial navigation benchmarks.

Conclusion: The VRRL framework effectively elicits visually grounded self-reflection in large vision-language models, leading to improved performance on out-of-distribution images and robustness under distribution shift.
...
TL;DR: A new approach for speaker recognition in long-form TV dramas, leveraging multimodal cues and a large reasoning model.


<details>
  <summary>Details</summary>
Motivation: The challenge of comprehensively understanding long-form TV dramas, particularly in attributing spoken utterances to their respective characters.

Method: DramaSR-LRM, a robust approach built upon a large reasoning model (LRM) that aggregates contextual evidence via multimodal tool-use.

Result: DramaSR-LRM significantly outperforms existing baselines, especially on short utterances where acoustic biometrics are unreliable.

Conclusion: The proposed approach advances the field of speaker recognition in long-form TV dramas, with a large-scale benchmark (DramaSR-532K) and publicly available data and code.
...
TL;DR: The paper introduces LACUNA, a testbed for evaluating the unlearning of sensitive data in large language models, and finds that current state-of-the-art unlearning methods are imprecise and vulnerable to resurfacing attacks.


<details>
  <summary>Details</summary>
Motivation: The memorization of sensitive training data, including personally identifiable information (PII), by large language models creates a need for reliable post hoc removal methods, and existing benchmarks do not adequately evaluate the effectiveness of unlearning methods at the parameter level.

Method: The authors introduce LACUNA, a testbed that injects PII into predefined parameters of large language models and evaluates the effectiveness of unlearning methods at the parameter level, using it to benchmark current state-of-the-art unlearning methods.

Result: The authors find that current state-of-the-art unlearning methods are highly imprecise and susceptible to resurfacing attacks, but that precise localization of sensitive data can lead to strong erasure and robustness to resurfacing attacks.

Conclusion: The paper highlights the importance of precise unlearning and introduces LACUNA as a tool to drive further advances in robust, localization-based unlearning.
...
TL;DR: The paper presents PACE, a neuro-symbolic framework for generating feasible counterfactual explanations of machine learning predictions.


<details>
  <summary>Details</summary>
Motivation: Existing counterfactual explanation methods often produce unrealistic recommendations due to a lack of domain knowledge and intervention constraints.

Method: The PACE framework combines a neural predictive model with a symbolic reasoning layer that enforces domain-specific constraints during counterfactual generation.

Result: The approach produces explanations consistent with domain knowledge, while remaining interpretable and actionable, and shows that symbolic constraints yield explanations that better satisfy domain-specific feasibility requirements.

Conclusion: The paper demonstrates the potential of neuro-symbolic methods for transparent, feasibility-aware counterfactual explanation in explainable AI.
...
TL;DR: Auto-FL-Research (AFR) is proposed to automate the search for federated learning algorithms, improving performance on various tasks.


<details>
  <summary>Details</summary>
Motivation: The manual exploration of federated learning algorithms is expensive and difficult to compare fairly due to the numerous small but consequential choices.

Method: AFR uses a constrained coding-agent workflow to propose and implement candidate training algorithms, including server aggregation rules, client update schedules, and model variants.

Result: AFR shows gains on four FLamby tasks and five of six LEAF profiles, with some improvements attributed to FL-recipe changes and others to tuning effects or single-run artifacts.

Conclusion: AFR provides a systematic approach to federated learning algorithm search, helping to distinguish between different factors contributing to performance improvements.
...
TL;DR: A new Small Language Model (SLM) architecture called Wiola is introduced, featuring five novel components and outperforming existing models like GPT-2, LLaMA-2, and Mistral.


<details>
  <summary>Details</summary>
Motivation: The authors aim to create a fully original SLM architecture that differs from existing model families, addressing limitations and inefficiencies in current models.

Method: Wiola's architecture includes five novel components: Spiral Rotary Positional Encoding (SRPE), Gated Cross-Layer Attention (GCLA), Adaptive Token Merging (ATM), Dual Stream Feed-Forward (DSFF), and WiolaRMSNorm.

Result: Wiola is released in four sizes (120M, 360M, 700M, and 1.5B parameters) and is fully compatible with the HuggingFace Transformers ecosystem, with all 22 architectural unit tests passing.

Conclusion: The introduction of Wiola presents a new approach to SLM architecture, offering improvements over existing models and contributing to the development of more efficient language models.
...
TL;DR: A multi-agent framework called Agent4cs is proposed to summarize large codebases by leveraging interdependencies and hierarchical information, improving semantic consistency and keyword coverage.


<details>
  <summary>Details</summary>
Motivation: Existing code summarization solutions have limitations, such as relying on a single language model and treating source code as flat text, which underutilizes the rich information within a repository.

Method: Agent4cs uses a multi-agent framework with three agents: a summarization agent, a keyword-extraction agent, and a quality-assurance agent, to summarize large codebases in a bottom-up fashion.
...
Conclusion: The proposed Agent4cs framework effectively addresses the challenges of understanding large, complex codebases by providing more accurate and comprehensive summaries.
...
Result: The architecture improves reliability on service requests with operational conflict, and the results show that stronger control is directed toward conflicted requests rather than broadly applied to routine ones.

Conclusion: The proposed architecture effectively balances speed and accuracy in handling customer requests, and the results demonstrate its effectiveness in preventing operational errors in retail and airline tasks.
...
<details>
  <summary>Details</summary>
Motivation: Large language models tend to generate similar responses to open-ended questions, lacking divergent thinking, a crucial aspect of creativity.

Method: CreativityNeuro, a data-free method using contrastive weight steering to enhance divergent thinking in LLMs.

Result: CreativityNeuro improves performance on multiple creativity assessments, including a 14 percentile point increase on the Divergent Association Task and significant improvements in originality, surprise, and creativity on the Alternative Uses Test and the Task Task.

Conclusion: CreativityNeuro improves divergent thinking and reduces mode collapse in LLMs without requiring behavioral data, re-training, or gradient-based fine-tuning.
...
TL;DR: The study adapts a diffusion language model, DiffusionGemma-26B, for medical applications and compares it with an autoregressive (AR) model, Gemma-4-26B. The diffusion model matches or exceeds AR in medical visual question answering, with faster decoding and additional drafting capabilities.


<details>
  <summary>Details</summary>
Motivation: The motivation is to explore the potential of diffusion language models in medical foundation models, which are currently dominated by autoregressive models.

Method: The study adapts a mixture-of-experts diffusion language model, DiffusionGemma-26B, and benchmarks it against its same-size AR sibling Gemma-4-26B under an identical LoRA recipe on medical visual question answering datasets.

Result: The diffusion model matches or exceeds AR on all datasets, with the finetuned model (3.8B active) competitive with frontier vision-language models, and its decoding 3.5-4.4x faster.

Conclusion: The diffusion model offers comparable or superior performance to AR models in medical applications, with additional benefits such as drafting capabilities and faster decoding.
...
TL;DR: The paper explores using Reinforcement Learning with Verifiable Rewards (RLVR) to improve large language models in niche enterprise SaaS workflows by directly optimizing performance in specific API environments.


<details>
  <summary>Details</summary>
Motivation: The objective mismatch between large language models trained to predict the next token and the specific requirements of niche enterprise SaaS workflows leads to silent failures, such as dropped fields or hallucinated tools.

Method: The authors apply RLVR directly in synthetic environments emulating Jira and Confluence APIs, using rewards computed from tool-call traces without live API, learned judges, or human labels.

Result: RL-trained policies show significant improvement in average reward, lifting it from a range of 0.35-0.92 to 0.95-1.00 across four scenarios, with the largest gain in Confluence page creation.

Conclusion: The study positions RLVR as a preliminary step toward outcome-optimized small models for niche enterprise APIs, highlighting the potential of this approach while noting limitations such as the scalability of hand-crafting verifiable rewards and specific scenario challenges.
...
TL;DR: The paper analyzes the application of Reinforcement Learning (RL) to clinical protocol-execution tasks, identifying barriers such as capability and format-knowledge gaps, and proposes a taxonomy to predict RL learnability and improve performance.
...
Method: The authors audit MedAgentBench v1/v2, construct MedAgentBench-v3 (MAB-v3) with 508 tasks, and train Qwen3-8B to evaluate the performance of RL and rule-based Supervised Fine-Tuning (SFT).

Result: The results show that pure RL reaches 18.2% pass@1 compared to 34.1% for rule-based SFT, with a 15.9 percentage point gap attributed to capability and format-knowledge barriers.

Conclusion: The authors conclude that a decision/format-knowledge/lookup taxonomy can predict RL learnability and prescribe fixes, specifically suggesting SFT to inject codes and RL to learn conditionals to improve performance.
...
TL;DR: The paper proposes Procedural Memory Distillation (PMD), a method that leverages cross-episode signals to improve reinforcement learning with verifiable rewards (RLVR) by converting these signals into reusable procedural memory and distilling it into the policy's weights.


<details>
  <summary>Details</summary>
Motivation: Current RLVR methods, such as SDPO, evaluate each rollout against a verifier and update the policy from that episode-level signal, but rarely retain or reuse the richer procedural information in the rollout.

Method: PMD organizes the memory at three levels of abstraction: raw trajectories, self-reflected strategies and lessons, and higher-level behavioral patterns. A memory-conditioned self-teacher supervises the student on its own rollouts, enabling the student to progressively internalize procedural knowledge within its parameters.

Result: PMD improves over SDPO by 3.8-5.5% on SCIKNOWEVAL and 7.9-13.6% on LIVECODEBENCH. Freezing either the memory or the policy trails PMD by more than 10% across SCIKNOWEVAL domains.

Conclusion: The proposed PMD method effectively utilizes cross-episode signals to improve RLVR performance, and the co-evolution design principle is key to achieving these gains.
...
TL;DR: AI agents can produce divergent conclusions from the same data and question, reproducing 72% of the human ideological gap in reported effect estimates, and highlighting the challenge of selective exploration and reporting in scientific research.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to investigate how different analytical choices can lead to different conclusions from the same data, and how AI agents can capture and make explicit these hidden forking paths.

Method: The method used in this paper involves using AI agents to analyze data and produce reports, and comparing the results to human research teams. The authors also introduce a new method called Agentic Bootstrap, which estimates the m-value (multiverse value) by using AI agents to sample plausible analysis paths.

Result: The results show that AI agents can produce divergent, often opposing, conclusions from the same data and question, and that these findings are systematically aligned with the AI agents' personas. The authors also found that 13.5% of reported human analyses fell in the most extreme 5% of the analysis space (m<0.05).

Conclusion: The conclusion of this paper is that scientific evidence should be evaluated not only by a single reported analysis but also by its position within the distribution of analyses that could reasonably have been reported, and that AI agents can be used to make this distribution observable and turn it into a criterion for scientific credibility.
...
TL;DR: The paper introduces Janus, a system for designing and evaluating user-involved agentic permission management. It explores the user's role in AI-driven tool calls, demonstrating that user input is crucial for privacy and security, and that AI can help reduce cognitive load.


<details>
  <summary>Details</summary>
Motivation: The user's role in agentic permission management remains under-explored, despite its importance in AI agents autonomously executing tool calls on a user's behalf.

Method: The authors introduce Janus, a playground system consisting of Janus-Core and Janus-Harness. They implement six permission assistants and evaluate them across three scenarios and synthetic responders.

Result: The study demonstrates that user input is critical for strengthening privacy and security, AI augmentation can reduce cognitive load, and realistic user behavior must be considered in system design. No single design performs optimally across all contexts.

Conclusion: The paper concludes that a more principled and context-sensitive approach to deploying permission assistants in agentic systems is needed, and makes Janus publicly available for future investigation.
...
TL;DR: The paper proposes Semi-CoT, a framework that uses unlabeled questions to construct pseudo reasoning supervision for large language models, improving their reasoning capabilities.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enhance the effectiveness of Chain-of-thought (CoT) reasoning in large language models by reusing generated reasoning traces as semi-supervised learning signals.

Method: The method proposed is Semi-CoT, which samples multiple pseudo-CoTs for each unlabeled question, estimates answer-level semantic entropy, and selects low-entropy reasoning chains as reliable pseudo-CoT demonstrations.

Result: The results show that the entropy gate selects high-precision pseudo-CoTs with precision ranging from 91.36% to 100%, and Semi-CoT gives small gains on some datasets, but with negative transfer on one and a ceiling effect on another.

Conclusion: The conclusion is that unlabeled questions can provide reliable pseudo reasoning signals, but their effective use still requires stronger demonstration selection or student training.
...
TL;DR: OPINE-World, an LLM agent, learns object-centric programmatic world models from interaction, improving adaptability in unfamiliar tasks.


<details>
  <summary>Details</summary>
Motivation: Current world models are either data-hungry and transfer poorly or are limited to structured-state worlds with given object vocabularies.

Method: OPINE-World uses two cooperating agents in a loop of hypothesis and test, synthesizing models in code with replay verification and model-based planning.

Result: OPINE-World solves 20 of 25 games without per-game training, achieving an action-efficiency score of 78.4 against the human baseline.

Conclusion: OPINE-World demonstrates a data-efficient and reusable approach to learning world models, improving adaptability in unfamiliar tasks.
...
TL;DR: The paper scales up the SOLiD method for detecting deception in large language models (LLMs) and finds it to be effective in reducing undetected deception, but notes sensitivity to data distribution shifts.


<details>
  <summary>Details</summary>
Motivation: Deceptive behavior in LLMs is costly to monitor and prevent, motivating the development of more efficient methods for detecting and mitigating such behavior.

Method: The paper scales up the SOLiD (Scalable Oversight via Lie Detectors) method, which uses lie detectors to identify potentially deceptive responses for review by human labelers.

Result: The results show favorable scaling of SOLiD with larger models, with undetected deception dropping from 34% for 1B-parameter models to 14% for 405B-parameter models. The method allows for the removal of expensive human labelers from the fine-tuning phase without a significant increase in deception.

Conclusion: The SOLiD method is effective in detecting and mitigating deceptive behavior in LLMs, especially in larger models, but its performance can be sensitive to distribution shifts between detector training and preference-training data.
...
TL;DR: A pipeline for generating research hypotheses in Earth observation using a knowledge graph and large language models.


<details>
  <summary>Details</summary>
Motivation: Most prior work on scientific hypothesis generation relies on unstructured literature and free-form textual claims, which can be limited. The authors aim to ground hypothesis generation directly in structured data.

Method: A heterogeneous graph neural network and a three-agent LLM pipeline are used to rank candidate dataset pairings, filter, generate, and evaluate structured research hypotheses.

Result: The system produces 160 hypotheses spanning multiple Earth-science domains, with model-predicted novel dataset pairings rated nearly as plausible as held-out real co-usages.

Conclusion: The pipeline surfaces scientifically coherent yet unexplored combinations, but highlights limitations of single-judge LLM evaluation.
...
TL;DR: Hawk is a training-free framework that improves the performance of Neural Processing Unit (NPU) kernels by harnessing hardware-aware knowledge.


<details>
  <summary>Details</summary>
Motivation: Developing high-performance kernels for NPUs is a bottleneck due to implicit hardware constraints and memory hierarchies.

Method: Hawk framework consists of three core modules: Run-Time Knowledge Synthesis Module, Bottleneck-Aware Knowledge Retrieval Module, and Effect-Driven Knowledge Distillation Module.

Result: Hawk elevates generation accuracy from 49.4% to 80.0% and achieves up to a 2.2x execution speedup over state-of-the-art baselines.

Conclusion: Hawk effectively overcomes the challenges of developing high-performance NPU kernels by leveraging hardware-aware knowledge.
...
TL;DR: Proposed PASE, a novel fault self-healing framework that leverages Large Language Models (LLMs) and Deep Reinforcement Learning (DRL) for rapid fault detection and adaptive recovery in cloud-based AI systems.


<details>
  <summary>Details</summary>
Motivation: The escalating scale and complexity of cloud-based AI systems have made service reliability a critical challenge, and existing approaches underutilize the capabilities of LLMs.

Method: PASE employs an LLM as a Plan Synthesis Engine, a Neural-Symbolic World Model, and a Meta-Prompt Optimizer trained via DRL to generate and verify recovery plans.
...
Conclusion: PASE advances autonomous system management by unifying LLM-based reasoning with model-assisted verification and meta-learned guidance.
...
TL;DR: SemHash LLM is a framework for efficient large-scale document deduplication that preserves semantic equivalence.
...
Result: SemHash LLM achieves strong duplicate detection quality with less than 1% neural verification cost.

Conclusion: The proposed SemHash LLM framework effectively balances efficiency and accuracy in large-scale document deduplication.
...
TL;DR: A new method called profit-based counterfactual explanation (PBCE) is proposed to improve the interpretability of machine learning models in decision-making by maximizing profit.


<details>
  <summary>Details</summary>
Motivation: Existing counterfactual explanation methods have limitations, including requiring exogenously specified inputs and not optimizing decision objectives.

Method: Formulating counterfactual explanation as a profit maximization problem and proposing a framework called profit-based counterfactual explanation (PBCE).

Result: The proposed PBCE framework eliminates the need for exogenous target specification and provides a clear interpretation of the distance term as the cost of modifying product attributes.

Conclusion: The proposed PBCE framework provides a more practical and economically grounded approach to counterfactual explanation in machine learning.
...
TL;DR: The paper proposes a novel RL algorithm, C3RL, which enhances the calibration of large language models (LLMs) by integrating correctness, calibration, and dataset-informed reference accuracy rewards. It also introduces a confidence-based adaptive test-time scaling strategy, CAS, which allocates computational resources based on response confidence.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the limitation of prevailing RL reward designs that prioritize response correctness over confidence accuracy, leading to poor calibration between confidence and accuracy in LLMs.

Method: The paper proposes two main methods: C3RL, a novel RL algorithm that integrates correctness, calibration, and dataset-informed reference accuracy rewards, and CAS, a confidence-based adaptive test-time scaling strategy that allocates computational resources based on response confidence.

Result: The results show that C3RL enhances calibration without sacrificing accuracy, outperforming the current state-of-the-art method in both performance and calibration metrics across 8 text and multimodal datasets. Additionally, CAS surpasses majority voting on both in-domain and out-of-domain datasets while reducing the inference budget by up to 12.33 times.

Conclusion: The paper concludes that the synergy of C3RL and CAS paves the way for deploying more reliable and resource-efficient LLMs.
...
TL;DR: The paper proposes a geometry-aware multi-support heterogeneous graph neural network for fine-scale rainfall reconstruction, integrating data from gauges, microwave links, and radar/satellite products, and demonstrates its effectiveness in reducing RMSE and outperforming other methods.


<details>
  <summary>Details</summary>
Motivation: The need for fine-scale rainfall reconstruction in urban flood modeling, and the challenge of integrating data from different sources with incompatible spatial supports.
...
Result: The proposed method reduces RMSE by 23.2% over the classical interpolation baseline and consistently outperforms other neural architectures, with a generalization study showing that fusion delivers the largest gains where the field is under-sampled relative to its correlation length.

Conclusion: The proposed method provides a effective way to integrate multi-support data for fine-scale rainfall reconstruction, and has the potential to improve urban flood modeling.
...
<details>
  <summary>Details</summary>
Motivation: The discovery of universal traffic laws is crucial for transportation planning, management, and control, but it remains an expert-driven process that is time-consuming and limited by human expertise.
...
Conclusion: TrafficSci provides a route for extending AI-driven scientific discovery from controlled domains to complex urban systems, enabling autonomous discovery of traffic laws and potentially leading to more efficient transportation planning and management.
...
TL;DR: The paper proposes a new approach to multi-agent systems for forecasting, called InfoDelphi, which uses designed information asymmetry to improve deliberation and calibration among agents.


<details>
  <summary>Details</summary>
Motivation: Existing multi-agent systems for forecasting overlook the critical design choice of what information each agent receives, leading to herding behavior and limited improvement over single-agent systems.

Method: The authors propose partitioning evidence into shared public and disjoint private subsets, allowing each agent to hold exclusive knowledge that can only be shared through deliberation. They instantiate this approach in InfoDelphi, a framework that combines relevance-aware evidence routing, rationale-based iterative deliberation, and confidence-weighted aggregation.

Result: InfoDelphi outperforms the strongest single-agent and multi-agent baselines by 12-18% in Brier score and 4-8 percentage points in accuracy on the PolyGym benchmark.

Conclusion: The paper concludes that designed information asymmetry is a key enabler of effective multi-agent reasoning, and that removing information asymmetry eliminates most deliberation gains.
...
TL;DR: The paper proposes an incremental expert bank for multi-source ECG deployment, allowing for the addition of new data sources without requiring access to earlier raw ECGs.


<details>
  <summary>Details</summary>
Motivation: The need to incorporate new data sources in multi-source ECG deployment without retaining or replaying earlier raw ECGs, while preventing parameter interference between sources.

Method: The proposed method, called \ours{}, involves freezing a pretrained backbone and adding a balanced-softmax linear expert for each new domain, along with a lightweight router fitted on retained training features and domain labels.

Result: The method achieves a Macro-F1 score of $0.7915\pm0.0036$ on several ECG datasets, comparable to an offline independent-head reference, and outperforms a simple MLP router.

Conclusion: The paper demonstrates the effectiveness of the proposed incremental expert bank for multi-source ECG deployment, highlighting the importance of autonomous source inference for further improvement.
...
TL;DR: Researchers introduce Goggles, a module that helps language models distinguish between factual and fictional content by editing the finetuning gradients, achieving a significant improvement in identifying fictional claims.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the issue of Negation Neglect, where language models fail to recognize the fictional nature of explicitly annotated documents, and to enable models to learn from misaligned data without absorbing its behaviors.

Method: The method involves introducing a learned module called Goggles, which intervenes on the finetuning gradient of a language model, imparting a chosen epistemic frame to the model, allowing it to distinguish between factual and fictional content.

Result: The result shows that models trained with Goggles can correctly identify fictional content about 91% of the time, while preserving capability on other tasks, and that the imparted frame persists under continued finetuning.

Conclusion: The conclusion is that Goggles offers a promising approach to training language models on known-misaligned data without absorbing its behaviors, and that it has the potential to be applied to other domains and frames.
...
<details>
  <summary>Details</summary>
Motivation: The increasing use of agents in constructing workflows and assisting humans in recurring tasks requires agents to recall workflow patterns, execution constraints, and user preferences from previous runs.

Method: COMFYCLAW formulates workflow construction as typed graph editing, uses a region-level vision-language model verifier to translate visual failures into actionable repair suggestions, and evolves a progressively disclosed skill library.

Result: COMFYCLAW achieves the best average image-generation evaluation score across all six agent configurations, outperforming a verifier-only baseline without skill evolution.
...
TL;DR: Proposed a novel expert pruning method called Generic TB-Coverage for sparsely activated Mixture-of-Experts (MoE) language models, which uses generic text corpora for calibration and improves model performance under various pruning budgets.


<details>
  <summary>Details</summary>
Motivation: Existing expert-pruning methods for MoE language models rely on a single aggregated importance score, which can be biased toward experts favored by dominant calibration patterns, and pruning without downstream calibration data remains challenging.

Method: The Generic TB-Coverage method profiles per-expert utility separately on each corpus (WikiText2 and C4) and enforces a fixed-budget coverage rule to preserve high-utility experts from each corpus before constructing the final pruning mask.

Result: The proposed method improves average accuracy on six common zero-shot benchmarks and reduces perplexity degradation on WikiText2 and C4 across various pruning budgets (25%, 50%, and 75% retention), with the largest gains under aggressive pruning.

Conclusion: The Generic TB-Coverage method is an effective generic-data prior for MoE pruning, preserving cross-corpus expert coverage and improving model performance without relying on downstream calibration data.
...
TL;DR: The paper proposes a robust preference optimization method for language model alignment under ranking-label uncertainty. It introduces a pointwise total-variation robust Plackett-Luce objective that robustifies the ranking label conditional on the candidate list.


<details>
  <summary>Details</summary>
Motivation: Existing robust preference optimization methods mainly focus on pairwise supervision and place robustness at the dataset, prompt, or preference-pair level. However, the authors recognize the need to study listwise preference optimization under ranking-label uncertainty.

Method: The authors propose a pointwise total-variation robust Plackett-Luce objective that directly robustifies the ranking label conditional on the candidate list. They also derive an exact decomposition of the robust loss into the nominal PL loss plus a worst-case PL correction.

Result: The proposed method yields strong offline and online optimization guarantees. In offline settings, the robust objective is convex and projected stochastic subgradient reaches global ε-suboptimality with O(ε^{-2}) sample complexity. In online settings, weak convexity and widetilde O(ε^{-2}) Moreau-envelope stationarity are established. Experiments show improved robustness under noise and reliability in online alignment.

Conclusion: The proposed robust correction largely preserves performance under clean labels and improves robustness under noise in offline LLM alignment. In online alignment, it makes reward-model-ranked candidate expansion more reliable and improves both reward-model and external GPT-4 judge metrics.
...
TL;DR: A novel clean label backdoor attack for speech classification using reinforcement learning and audio steganography.


<details>
  <summary>Details</summary>
Motivation: Deep learning models for speech classification are vulnerable to backdoor attacks, and existing attacks are often detectable. The authors aim to create a more stealthy attack.

Method: The authors propose DRL-CLBA, which uses Deep Deterministic Policy Gradient (DDPG) reinforcement learning and deep audio steganography to embed sample-specific triggers.

Result: DRL-CLBA achieves a high attack success rate across three datasets and four DNNs, and resists fine-tuning, pruning, and spectral signature defenses.

Conclusion: The proposed attack exposes critical vulnerabilities in speech-controlled systems and highlights the need for more robust defenses.
...
TL;DR: The paper presents a case study on reformalization, a variant of autoformalization, and its application to the Jordan Curve Theorem across different proof assistants.


<details>
  <summary>Details</summary>
Motivation: The motivation is to explore reformalization, a process of translating formal proofs from one proof assistant to another, and to identify key design choices in the pipeline that affect practical reformalization tasks.

Method: The method involves performing three reformalizations of the Jordan Curve Theorem: from Mizar to Lean, from HOL Light to Lean, and from HOL Light to Agda, and then analyzing the results.

Result: The result is the identification of pipeline design choices crucial for practical reformalization tasks, based on the analysis of the reformalization of the Jordan Curve Theorem across different proof assistants.

Conclusion: The conclusion is that reformalization is a viable approach for translating formal proofs between different proof assistants, and that certain design choices in the reformalization pipeline significantly impact its practicality.
...
TL;DR: The paper proposes a meta-benchmarking framework for evaluating large language models (LLMs) in financial services, addressing the limitations of current leaderboards that focus on global average performance.


<details>
  <summary>Details</summary>
Motivation: Current LLM leaderboards do not capture the specific cognitive demands of financial-services work, leading to models that may underperform in certain areas.

Method: The framework organizes 452 benchmarks into 41 O*NET Generalized Work Activities and 38 BIAN banking business domains, using a multiplicative weighting scheme to reward benchmarks that separate top models, are widely reported, and remain in active use.

Result: The framework provides a point-in-time public snapshot of 288 models across 25 organizations, demonstrating its effectiveness in evaluating LLMs in financial services.

Conclusion: The proposed framework offers a reproducible approach for institutions to address selection and governance challenges in evaluating LLMs for financial services, highlighting the importance of domain-specific evaluation.
...
TL;DR: The paper introduces Phi-Nav, a unified on-policy framework for training robust Vision-Language Navigation agents. It uses hindsight reasoning to align instructions with the agent's exploratory journey, achieving competitive performance with limited expert demonstrations.


<details>
  <summary>Details</summary>
Motivation: The need for on-policy exploration in training Vision-Language Navigation agents, which leads to a semantic mismatch between executed visual streams and original language instructions.

Method: Phi-Nav, a three-stage dual-supervision cycle framework: 1) oracle-guided on-policy exploration, 2) hindsight speaker synthesizes a path-level hindsight instruction, and 3) second imitation pass with synthesized trajectory-instruction pair.

Result: Phi-Nav yields competitive performance on R2R-CE and RxR-CE benchmarks with only a fraction of expert demonstrations used by current baselines.

Conclusion: Phi-Nav effectively addresses the semantic supervision gap in on-policy methods, transforming unlabeled movement into dense training signals, and positioning it as a solution for training embodied agents with limited data.
...
<details>
  <summary>Details</summary>
Motivation: Repository-level vulnerability reproduction is a challenging task that requires an agent to inspect a codebase, infer the input grammar that reaches a vulnerable path, construct a proof-of-concept, and verify that the crash disappears on the patched build. Current agents often fail due to incorrect strategy selection.

Method: The Mastermind framework consists of a dual-loop architecture that separates transferable strategy learning from task-specific experience. A trainable planner learns reusable vulnerability-reproduction strategies through supervised fine-tuning (SFT) and milestone-based generalized reinforcement learning from human feedback (GRPO).

Result: Mastermind achieves an 84.5% pass rate on CyberGym with GPT-5.5 as the frozen executor, outperforming baseline methods. It also improves the performance of other executors, GPT-5.4 mini and GLM~5.1.

Conclusion: Learning high-level strategies is an effective and transferable mechanism for improving repository-scale software engineering agents.
...
TL;DR: SimWorlds is a multi-agent framework for generating dynamic 4D scenes from text, enabling editable and physics-grounded content creation.


<details>
  <summary>Details</summary>
Motivation: Existing systems for translating natural language into 3D scenes focus on static output, and dynamic 4D scenes remain largely unexplored despite their value.

Method: SimWorlds uses a planner-coder-reviewer workflow, a layered scene protocol, and a runtime-state inspection tool suite to generate dynamic 4D scenes.

Result: SimWorlds outperforms prior dynamic Blender generation baselines and introduces 4DBuildBench for assessing visual fidelity and physical consistency.

Conclusion: SimWorlds enables the creation of dynamic, editable 4D scenes from text, advancing the field of procedural content generation.
...
TL;DR: The paper proposes a world-model corrector called WM-SAR to repair failed planning graphs in large workflows, outperforming traditional engineering correctors.


<details>
  <summary>Details</summary>
Motivation: As agent planning scales to thousands or tens of thousands of steps, failures occur within large planning graphs. Replanning the entire graph after every mistake is unrealistic and undesirable.

Method: The paper compares two families of correctors: traditional engineering correctors and WM-SAR (World-Model Subgraph Amplification Repair). WM-SAR works backward from subgraph amplification to identify and repair the causal subgraph.

Result: WM-SAR substantially outperforms engineering correctors under realistic token budgets, achieving near-whole-graph stabilization with a compact region and providing the LLM with a cleaner repair target.

Conclusion: The proposed WM-SAR corrector effectively repairs failed planning graphs in large workflows, offering a more efficient and effective solution than traditional engineering correctors.
...
Method: The framework uses formal concept analysis as a symbolic verification loop, with a retrieval-grounded small language model oracle for validation and feedback.
...
Conclusion: The proposed framework improves knowledge expansion and validation, but identifying positive object-attribute pairs remains challenging.
...
TL;DR: This paper investigates whether Diffusion Language Models (DLMs) internally represent denoising progress and how this information is used, finding that they do encode a latent representation of denoising progress.


<details>
  <summary>Details</summary>
Motivation: To understand if DLMs internally represent denoising progress and how such information is used downstream, given that they are not explicitly conditioned on a timestep like standard diffusion-based approaches.

Method: The authors analyze the internal activations of DLMs, specifically the residual streams, using probes across layers to extract the signal related to denoising progress. They also steer the model along a low-dimensional subspace associated with the inferred timestep.

Result: The study finds that DLMs encode a latent representation related to the diffusion timestep within their residual streams, which can be reliably extracted. Steering the model along this subspace allows for systematic modulation of its notion of denoising progress, affecting model confidence and entropy. The identified representation exhibits structured and interpretable properties in activation space.

Conclusion: DLMs do internally represent denoising progress, and this information is used downstream in a structured and interpretable way. The findings shed light on how DLMs process denoising progress signals.
...
TL;DR: Presented Vera, an automated safety testing framework for LLM agents, which uncovers substantial safety weaknesses in four production agent frameworks.


<details>
  <summary>Details</summary>
Motivation: Existing safety testing methods for LLM agents are costly to extend as agents evolve, and they target expert-designed safety violations.

Method: Vera uses a three-stage pipeline: literature-driven exploration, combinatorial composition, and adaptive execution, to test safety risks in LLM agents.

Result: Vera found substantial safety weaknesses in four production agent frameworks, with average attack success rates reaching 93.9% under multi-channel attacks.

Conclusion: Modular, executable testing infrastructure is essential for rigorous and maintainable safety evaluation of rapidly evolving agentic systems at scale.
...
TL;DR: A novel framework called MMIR-TCM is proposed to improve Traditional Chinese Medicine (TCM) diagnosis by integrating multimodal AI, addressing challenges in subjectivity and reproducibility.


<details>
  <summary>Details</summary>
Motivation: The application of AI in TCM diagnosis is hindered by the semantic gap between visual tongue features and textual reasoning, and the lack of large-scale standardized datasets.

Method: MMIR-TCM integrates a multimodal large language model with memory-augmented segmentation and retrieval-augmented generation, using a three-stage architecture.

Result: MMIR-TCM significantly outperforms leading models, including GPT-4o and Gemini 2.5 Flash, in clinical accuracy evaluated using a domain-specific metric called TDEU.

Conclusion: The proposed MMIR-TCM framework shows promising results in improving TCM diagnosis, addressing persistent challenges in subjectivity and reproducibility.
...
TL;DR: The paper introduces Pre-Flight, a benchmark for evaluating large language models (LLMs) in aviation operations, and finds a significant gap in their ability to reason safely and correctly about aviation-specific knowledge.


<details>
  <summary>Details</summary>
Motivation: The increasing proposal of LLMs for aviation business operations and the lack of benchmarks that measure their ability to reason safely and correctly about aviation-specific operational knowledge.

Method: The authors created a benchmark of 300 multiple-choice questions (Pre-Flight) and evaluated various commercial and open-weight models using the Inspect evaluation framework.

Result: Even the strongest model evaluated reached only 82.7% accuracy, compared to an informal expert reference of around 95%, indicating a substantial and persistent gap below expert-level reliability.

Conclusion: The authors argue that domain-specific evaluation is a necessary precondition for the responsible deployment of generative AI in non-safety-critical aviation operations.
...
TL;DR: The paper studies fault trees using Halpern & Pearl's theory of actual causality to analyze why complex systems fail.


<details>
  <summary>Details</summary>
Motivation: To answer the question 'why has it gone wrong?' in failure diagnostics, which is fundamental but not addressed by traditional fault tree analysis.

Method: Using Halpern & Pearl's theory of actual causality to classify notions of causality in terms of fault tree graph and logical structure.

Result: A complete classification of actual causality notions and a demonstration of how minimal cut sets give rise to actual causes.

Conclusion: The study provides a new perspective on fault trees, enabling the analysis of actual causes of system failures.
...
TL;DR: The paper presents CLAP, a closed-loop method for post-training agents in noisy business environments, improving performance and reducing risks.


<details>
  <summary>Details</summary>
Motivation: Domain agents face challenges such as noisy data, uncertain gains, and adapter-release risks, highlighting the need for a more integrated approach to post-training.

Method: CLAP combines data validation, normalization, diagnosis, and replay to evaluate adapter suitability for target applications, using techniques like QLoRA-style LoRA-SFT and GRPO.

Result: CLAP shows modest average gains in five manufacturing-scenario batches, with improvements in overall score, pass rate, and evidence accuracy, but also reveals batch-specific variability and risks.

Conclusion: The results support the use of an integrated data-training-evaluation-release loop for domain-agent post-training, rather than relying on training completion or single offline scores.
...
TL;DR: The study reveals that safety mechanisms in large language models (LLMs) trained primarily in English do not generalize well to low-resource languages and mixed-language code-switching, creating an epistemic gap where models generate harmful responses.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this study is to investigate the limitations of safety training in LLMs, particularly when they are exposed to low-resource languages and mixed-language code-switching, and to identify potential vulnerabilities.

Method: The study introduces STEER (Safety Targeted Embedding Exploit via Refinement), a gradient-guided attack that identifies and translates words contributing to a model's refusal behavior into low-resource languages to suppress refusal while preserving harmful intent.

Result: The results show that STEER achieves high attack success rates (up to 93.0% on JailbreakBench and 96.7% on AdvBench) across six open-source 8B-parameter models, and also transfers to GPT-4o-mini with a 35.5% attack success rate.

Conclusion: The study concludes that safety mechanisms aligned primarily on English cannot be assumed to generalize across multilingual inputs, and that improving multilingual safety requires broader coverage during alignment and mechanisms to detect and abstain on out-of-distribution inputs.
...
TL;DR: CamoNAS, a Neural Architecture Search (NAS) framework, is proposed for Camouflaged Object Detection (COD) to automatically design effective architectures.


<details>
  <summary>Details</summary>
Motivation: Traditional COD models rely on hand-designed architectures and intuition-guided multi-scale feature fusion, which may not be optimal.

Method: CamoNAS uses a frequency-aware multi-resolution NAS framework, searching both cell-level operations and network-level downsampling paths, and adopts an RGB frequency dual-stream architecture.

Result: CamoNAS achieves state-of-the-art performance on four COD benchmarks (CAMO, COD10K, NC4K, CHAMELEON).

Conclusion: The proposed CamoNAS framework highlights the effectiveness of NAS for COD, providing an efficient way to design architectures tailored to detect camouflaged objects.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the reliable use of skills by LLM agents, as current methods are hindered by overlapping skills and coarse evaluation metrics that don't account for process quality.
...
Conclusion: The paper concludes that SkillCoach is an effective framework for evaluating and enhancing agentic skill-use, providing a more nuanced evaluation of process quality and improving the reliability of skill use by LLM agents.
...
TL;DR: A new method, AUF, improves speculative decoding for autoregressive generation by modifying the cross-entropy support to focus on the accepted prefix, leading to increased average emitted length across benchmarks.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the inefficiency in training block drafters, which predict a block of tokens in parallel but are supervised with a full-block cross-entropy loss that does not align with the inference process.

Method: AUF (Accepted Prefix-focused Supervision) modifies the cross-entropy support to concentrate supervision on the accepted prefix by truncating the support at the drafter's first predicted failure.

Result: AUF increases the average emitted length of the DFlash drafter from 2.40 to 2.61 across six benchmarks and also shows gains when transferred to another model, Domino's two-branch head.

Conclusion: The proposed method, AUF, provides a simple yet effective way to improve speculative decoding by aligning the supervision with the inference process, leading to better performance without requiring changes to the inference pipeline or verifier rollouts.
...
TL;DR: The paper introduces HECATE, a tool to measure complexity in LLM-integrated applications, considering both prompt and code layers, and presents new metrics that outperform existing ones.
...
Method: The authors developed HECATE, which uses a Hoare-logic-inspired formalism called Prompt-as-Specification, and generated 52 candidate metrics based on 25 complexity dimensions.

Result: After validation, only 10 metrics were significant, with 7 being new metrics that measure structural breadth, and the top-performing metrics outperformed existing ones.

Conclusion: The study demonstrates the importance of considering prompt complexity in LLM-integrated applications and the effectiveness of HECATE in assessing complexity in both prompt and code layers.
...
TL;DR: ContextSniper, a token-efficient code memory layer, reduces token usage and cost for large language model agents in repository-level program repair while maintaining similar submitted-resolution rates.
...
Method: ElephantAgent recomputes the digest of the local contextual state and verifies it against the latest authorized digest using replicated trusted hardware, and provides Historical Traceability for conditional post-hoc audit and recovery.

Result: ElephantAgent detects out-of-band state tampering and handles in-band semantic abuse, enabling defense against contextual state poisoning.

Conclusion: ElephantAgent provides a protocol to enforce Contextual State Continuity, defending against contextual state poisoning in agentic systems.
...
TL;DR: The paper studies 'ghost memory' in long-term memory agents, proposes ATMA to address state coordination failures, and evaluates its effectiveness on benchmarks LTP and LoCoMo.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the issue of 'ghost memory' in long-term memory agents, where old, current, and transition facts coexist and mislead the answer model.
...
Result: The evaluation on LTP and LoCoMo shows that Graphiti+ATMA improves conflict accuracy by 0.240 and raises temporal F1 from 0.0295 to 0.1705, indicating that explicit state roles can reduce memory failures.
...
TL;DR: The paper proposes a unified control framework called Atomic Task Graph (ATG) for LLM-based agents to solve complex multi-step tasks more efficiently and effectively.


<details>
  <summary>Details</summary>
Motivation: Existing methods for improving LLM-based agents' performance rely on scaling to larger models or task-specific fine-tuning, which are computationally costly or generalize poorly. Prompt-based control methods have limitations in reusing verified intermediate results.

Method: The authors propose ATG, which uses an explicit graph to expose dependencies and support reuse. It recursively decomposes high-level tasks into subtasks, forming a sequence of DAGs, and allows parallel execution of independent branches.
...
Conclusion: The proposed ATG framework offers a more efficient and effective approach for LLM-based agents to solve complex multi-step tasks, addressing the limitations of existing methods.
...
TL;DR: Introduced OntoLearner, a framework for ontology learning that unifies access, learning pipelines, and benchmarking, releasing 180 ontologies and datasets for three core tasks.


<details>
  <summary>Details</summary>
Motivation: Ontology learning lacks a shared infrastructure for systematic evaluation and ontology access, hindering progress and fragmenting research.

Method: OntoLearner provides a modular, cross-domain framework with LLM-driven learning pipelines and standardized benchmarking, releasing 180 machine-readable ontologies and datasets.

Result: A large-scale empirical study evaluating 22 retrieval models and 12 LLMs across domains and tasks, finding that the central challenge of OL is a structural mismatch between model knowledge encoding and ontology organization.

Conclusion: Effective OL is achievable through cross-domain, multi-task benchmarking enabled by OntoLearner, which is open-source and available at https://github.com/sciknoworg/OntoLearner/.
...
TL;DR: The paper proposes ScopeEdit, a novel online editor for multimodal large language models (MLLMs) that controls the semantic boundary of each edit, improving the trade-off between cross-modal transfer and locality.


<details>
  <summary>Details</summary>
Motivation: Existing editors for MLLMs focus on edit reliability and stability but neglect to control the semantic boundary of each edit, leading to potential leakage to unrelated inputs and poor transfer to valid cross-modal variants.

Method: The authors propose ScopeEdit, a scope-aware online editor that decomposes each update into a modality-local absorption branch and an evidence-gated shared generalization branch, performing scope-separated write geometries in orthogonal low-rank spaces.

Result: ScopeEdit consistently improves the trade-off between in-scope cross-modal transfer and out-of-scope locality while preserving edit reliability, stability, and online efficiency across diverse benchmarks and scenarios.

Conclusion: The paper concludes that ScopeEdit is an effective solution for online multimodal knowledge editing, enabling controlled propagation boundaries for edits and improving the overall performance of MLLMs.
...
<details>
  <summary>Details</summary>
Motivation: The need for knowledge consolidation in adaptive intelligent agents while maintaining their certified identity, particularly in regulated environments where agents operate under commitments and audit contracts.

Method: The authors propose treating consolidation as a deterministic function over episodic memory, producing a separately addressable semantic knowledge layer, which does not alter the agent's certified identity.

Result: The proposed construction ensures identity invariance, and experiments show a mean 79.82% reduction in unproductive planner attempts, with the agent's certified identity remaining byte-equal across its operational lifetime.

Conclusion: The paper presents a knowledge-update discipline for autonomic agents that allows lessons to accumulate as queryable facts without changing the agent's certified identity, which is validated through synthetic experiments.
...
TL;DR: A traceable BESS fault-diagnosis assistant using retrieval-augmented multi-agent reasoning is proposed to improve reliability in large-scale battery energy storage systems.


<details>
  <summary>Details</summary>
Motivation: The need for effective O&M decisions in large-scale BESSs that combine various data sources and domain knowledge to accurately diagnose faults and determine necessary interventions.

Method: The proposed assistant uses retrieval-augmented multi-agent reasoning, BESS-specific task routing, schema-constrained natural-language database access, hybrid text-image retrieval, and evidence-based answer synthesis.

Result: Preliminary internal evaluation shows promising results for routing, database access, and diagnostic reasoning.

Conclusion: The proposed BESS fault-diagnosis assistant has the potential to improve reliability in large-scale BESSs by providing accurate and traceable fault diagnoses.
...
TL;DR: The paper proposes InduceKV, a retrieval-based method for fixed-footprint continual adaptation in multimodal large language models, achieving consistent improvements over baselines under matched memory budgets.


<details>
  <summary>Details</summary>
Motivation: Continual improvement in multimodal large language models under a bounded deployment footprint is challenging due to accumulating adaptation state over time.

Method: InduceKV, a retrieval-based method that stores training prefixes as attention-ready memory entries with frozen retrieval keys and compact key-value payloads.

Result: InduceKV consistently improves over baselines (PEFT, MoE, replay, and prompt-retrieval) under matched memory budgets across various tasks.

Conclusion: The proposed InduceKV method effectively enables fixed-footprint continual adaptation in multimodal large language models.
...
TL;DR: The paper studies the overlooked failure mode of hidden evidence-use forgetting in continually adapted multimodal large language models (MLLMs) and proposes a replay-free reliance-constrained continual learning framework (RCL) to preserve not only answer accuracy but also the use of visual, textual, OCR, chart, and document evidence.


<details>
  <summary>Details</summary>
Motivation: Standard continual learning metrics mainly focus on whether old answers remain correct, but neglect the stability of multimodal grounding, leading to the issue of hidden evidence-use forgetting.

Method: The authors propose RCL, a framework that freezes the previous checkpoint as a behavioral reference, estimates teacher and student evidence-reliance profiles through counterfactual channel interventions, and jointly optimizes task learning, prediction preservation, and reliance preservation.

Result: RCL consistently improves final performance and reduces forgetting over various baselines, while substantially lowering modality reliance drift, dominant evidence flips, and hidden forgetting rates across multiple datasets.

Conclusion: The results suggest that robust continual multimodal learning requires preserving the evidence path behind correct answers, not merely the answers themselves, highlighting the importance of addressing hidden evidence-use forgetting in MLLMs.
...
TL;DR: The paper introduces PACE, a framework for predicting performance on expensive agentic benchmarks using a small subset of atomic evaluation instances, reducing evaluation cost and time.


<details>
  <summary>Details</summary>
Motivation: Evaluating Large Language Model (LLM) agents on agentic benchmarks is expensive and time-consuming, and the authors aim to find a more efficient and cost-effective way to predict performance on these benchmarks.

Method: The authors propose PACE, a framework that constructs proxy benchmarks by selecting instances from existing non-agentic evaluations and uses regression to map model scores on a compact subset of source instances to its score on the target agentic benchmark.

Result: The experiments show that PACE-Bench, the concrete proxy benchmark, predicts agentic scores with high accuracy (MAE under 4%, Spearman correlation above 0.80, and pairwise model-ranking accuracy around 85%) at a much lower cost (less than 1% of the full agentic evaluation cost).

Conclusion: The paper concludes that PACE enables practitioners to obtain reliable estimates of agentic performance during model development, selection, and routing, without the overhead of full agent evaluation.
...
<details>
  <summary>Details</summary>
Motivation: Ensuring model reliability in Explainable AI requires a global assessment of the hypothesis space, particularly for decision trees, which can have a large and complex hypothesis space.

Method: ADTC uses dynamic programming and algebraic techniques to reformulate analytical tasks into a unified sum-of-products computation over a semiring, achieving a time complexity of $O^*(n^{O(Δ)})$ in the number of features $n$.

Result: The proposed framework efficiently constructs a model profile that captures the global landscape and trade-offs between criteria such as accuracy, size, and fairness, demonstrated on real-world datasets.

Conclusion: The ADTC framework facilitates evidence-based model selection in sensitive domains by providing a comprehensive analysis of decision trees, ensuring model reliability and transparency.
...
TL;DR: A new reinforcement learning framework called Maven is proposed to improve long-context reasoning by optimizing stateful evidence navigation.


<details>
  <summary>Details</summary>
Motivation: Existing long-context RL methods have limitations in providing feedback on intermediate actions, which hinders the model's ability to locate, revise, and synthesize evidence.

Method: Maven uses an editable evidence memory and defines an answer-conditioned evidence-state value, with rewards assigned to action-level state transitions.

Result: Maven outperforms baselines on multiple datasets, producing more sufficient evidence sets and lower distractor retention.

Conclusion: Long-context RL benefits from optimizing stateful evidence navigation rather than one-shot evidence extraction, and Maven is an effective framework for achieving this.
...
TL;DR: The paper proposes a new method called Surprise-based Nested Temporal Abstraction (SUNTA) for long-horizon prediction using hierarchical state-space models (HSSMs).


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the performance of HSSMs by determining chunk boundaries based on prediction errors rather than fixed-length chunking or similarity-based boundary detection.

Method: The method proposed is SUNTA, which uses a decoupled training strategy and internal inconsistency as a top-down surprise metric to determine chunk boundaries within imagined rollouts.

Result: The results show that SUNTA outperforms baselines in video prediction tasks in 2D and 3D environments, maintaining accurate predictions over 250 timesteps.

Conclusion: The conclusion is that SUNTA is an effective method for long-horizon prediction using HSSMs, addressing the challenges of hierarchical collapse and absence of surprise signals during open-loop prediction.
...
TL;DR: The paper presents ContextNext, a governance layer for AI-consumable knowledge vaults, which ensures the provenance, integrity, and traceability of knowledge used by autonomous AI agents.
...
Method: The authors formalize the concept of context governance and present ContextNext, an open specification and reference implementation that combines typed Markdown documents with metadata, deterministic selectors, and version histories.

Result: The empirical results show that ContextNext outperforms BM25 sparse retrieval in terms of answer quality and input-token cost, and provides deterministic selectors that return stable document sets across repeated queries.

Conclusion: The paper concludes that context governance addresses failure modes that retrieval quality alone cannot resolve, and that ContextNext provides a robust governance layer for AI-consumable knowledge vaults.
...
TL;DR: The paper introduces a new memory contract for long-horizon LLM agents, allowing for more controlled and isolated memory component analysis, and evaluates its effectiveness in a stochastic deck-building game.


<details>
  <summary>Details</summary>
Motivation: The current simplest contract for LLM agent memory has limitations, making it difficult to isolate the effect of individual memory components. The authors aim to introduce an alternative bounded contract to address this issue.

Method: The authors propose a new memory contract where each decision is made from a fresh user message assembled by typed retrieval, with no raw cross-decision transcript appended. They evaluate this contract in the game Slay the Spire 2.

Result: The results show that adding a strategic skill layer to the agent improves its performance, with a directional but not statistically decisive difference (Fisher exact p≈0.37). The authors also release a reproducible testbed for future research.

Conclusion: The paper concludes that the proposed bounded contract allows for more controlled analysis of memory components and provides a validated methodology for studying explicit memory layers in long-horizon LLM-agent decisions.
...
TL;DR: The paper introduces FitOne, a series of fitness LLMs designed to improve reliability and domain specialization for Scientific Fitness Coaching (SFC) applications, achieving significant improvements on professional fitness certification exams.


<details>
  <summary>Details</summary>
Motivation: The high cost and inaccessibility of human-delivered Scientific Fitness Coaching (SFC) and the limitations of general-purpose Large Language Models (LLMs) in SFC, which lack sufficient domain-specific knowledge integration.

Method: The authors developed FitOne, a series of fitness LLMs with 8B and 32B parameters, using a three-stage post-training pipeline consisting of continual pre-training, supervised fine-tuning, and reinforcement learning.

Result: FitOne-8B/32B achieves average improvements of up to 10.09%/9.29% and 12.73%/7.01% on the ACSM-EP and NSCA-CSCS exams, respectively, compared with the Qwen3 base models.

Conclusion: The research advances LLM systems toward more reliable fitness intelligence and will inspire future research on developing domain-specific LLMs.
...
TL;DR: This paper evaluates the use of Large Language Models (LLMs) for grading short Linux/bash command responses, comparing their judgments to those of expert instructors. It finds that LLMs can approximate expert judgment, especially with rubric-guided prompting, but accuracy decreases with question complexity.


<details>
  <summary>Details</summary>
Motivation: The challenge of scalable and reliable grading of command-line examinations in computing education, where manual marking is difficult due to rising enrolments and rule-based autograders have limitations.

Method: The study uses four LLMs (GPT, Claude Opus, Gemini, and GLM) to grade 1200 real responses from Computer Engineering students, comparing their judgments to those of three expert instructors. It tests two prompt variants and uses a four-level cognitive taxonomy to evaluate complexity.

Result: Gemini~3.0 Pro with rubric-guided prompting achieved the highest human-AI agreement (ICC(3,1) = 0.888, MAE = 0.10). Agreement declined as taxonomy level increased, and rubric quality had a larger effect than provider choice.

Conclusion: The study establishes a framework for determining which questions are suitable for AI-assisted grading and which require human review. It shows that question complexity is a reliable predictor of the difficulty LLMs face in grading accurately.
...
TL;DR: The authors introduce Paper-replication, a workflow for replicating computational claims in scientific machine learning papers using a coding agent. The workflow ensures the agent records evidence, reconstructs the paper's method, and validates the results.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the replication of computational claims in scientific machine learning papers, as current methods have limitations in preserving progress and checking generated evidence.

Method: The authors propose a workflow called Paper-replication, which involves: (1) selecting paper claims as targets, (2) recording evidence, (3) reconstructing the paper's method, (4) running computational experiments, (5) linking outputs to provenance and comparisons, and (6) passing validation checks.

Result: The workflow was evaluated on twelve independent runs across four scientific machine learning papers, and all twelve workspaces passed the completion gate with matched evidence coverage.

Conclusion: The Paper-replication workflow successfully replicates computational claims in scientific machine learning papers, ensuring reliable progress and validation of generated evidence.
...
<details>
  <summary>Details</summary>
Motivation: Existing evaluations of autonomous agents often collapse the process of improving executable policies through feedback into a final score or confound it with open-ended software-engineering progress.

Method: The authors introduce Autonomous Policy Evolution, a controlled evaluation setting, and EvoPolicyGym, a benchmark built from compact interactive RL environments.

Result: GPT-5.5 achieves the strongest aggregate rank score and top-two performance on all 16 environments in EvoPolicyGym.

Conclusion: The paper highlights the importance of discovering task-appropriate mechanisms and refining policies under bounded feedback for strong autonomous policy evolution.
...
TL;DR: The paper introduces A²utoLPBench, a dynamic benchmark for testing LLM-driven agents on linear programming problems, offering unlimited, fresh problems with ground-truth answers and resistance to data leakage.


<details>
  <summary>Details</summary>
Motivation: Current LP-from-text benchmarks are static, limited in size and difficulty, and prone to data leakage. The authors aim to create a dynamic benchmark that can provide fresh problems and mitigate these issues.

Method: The authors propose a generator-based benchmark, A²utoLPBench, which constructs linear programming problems with known optimal solutions and objective values. The benchmark includes a reference solver-critic baseline and a Docker image for LLM-driven agents.

Result: The proposed benchmark offers several advantages, including an unlimited supply of fresh problems, adjustable difficulty, ground-truth answers, low cost per problem, repeatable scores, and resistance to training-data leakage.

Conclusion: A²utoLPBench provides a dynamic and flexible solution for evaluating LLM-driven agents on linear programming problems, addressing the limitations of static benchmarks and mitigating the issue of data leakage.
...
TL;DR: The paper presents a new evaluation dataset for testing clinical performance in medicine, finding that current frontier models struggle with high-stakes criteria, passing only 32-42% of critical criteria.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the limitations of multiple-choice medical benchmarks and to evaluate the performance of frontier models in clinical settings, particularly on high-stakes criteria.

Method: The authors created a small, difficult evaluation dataset with five clinician-authored clinical scenarios across four specialties, each with a detailed, weighted rubric (184 criteria total). They then evaluated three frontier models: GPT 5.4, Claude Opus 4.7, and Gemini 3.1 Pro, using these rubrics to assess performance.

Result: The results showed that the models had a mean rubric pass rate of 0.47 (Claude), 0.39 (GPT), and 0.37 (Gemini). Critically, high-stakes (weight-5) criteria were passed only 32.4-41.7% of the time, while low-stakes (weight-1) criteria were passed 80-90% of the time. Furthermore, 52% of critical criteria were not satisfied by any model.

Conclusion: The paper concludes by positioning its contribution as a method-and-preliminary-findings type, introducing a scalable and defensible pipeline for developing a large-scale benchmark for evaluating clinical performance in medicine.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to understand how LLM agents behave in socially structured settings and how their expressions change when in public versus private channels.

Method: The authors introduce a dual-channel debate framework, where agents produce public and off-the-record (OTR) responses, and analyze the divergence between the two across various models, scenarios, and variations.

Result: The results show a significant public-OTR divergence, with the targeted agent's decision divergence rising from ~3% to roughly 40% in alignment-inducing settings, and the effect is consistent across four aggregate analyses.

Conclusion: The findings suggest that agent evaluation should consider emergent objectives beyond explicit goals, and the authors present a dual-channel evaluation framework and behavioral measures to operationalize this assessment.
...
TL;DR: A new framework, UA-ChatDev, is proposed to improve multi-agent software development by incorporating uncertainty awareness, which helps mitigate the propagation of incorrect decisions and enhances the reliability of the final software product.


<details>
  <summary>Details</summary>
Motivation: Existing multi-agent software development frameworks are vulnerable to hallucination propagation, where incorrect decisions made in early development phases negatively impact the final software quality.

Method: The UA-ChatDev framework integrates uncertainty quantification into agent interactions using a lightweight uncertainty estimation mechanism based on token-level log probabilities and phase-aware threshold calibration to selectively trigger retrieval-based verification.

Result: UA-ChatDev consistently outperforms existing single-agent and multi-agent software development frameworks across completeness, executability, consistency, and overall quality metrics on the SRDD benchmark.

Conclusion: The proposed UA-ChatDev framework effectively addresses the challenge of hallucination propagation in multi-agent software development, leading to more reliable software products.
...
TL;DR: A simple real-time monitor using thresholding on verifier signals from an external model is competitive with advanced monitors in detecting unsafe outputs from LLMs.


<details>
  <summary>Details</summary>
Motivation: LLMs are prone to generating unsafe outputs despite alignment training, making real-time monitoring critical for safety.

Method: A simple real-time monitor using thresholding on verifier signals from an external model, with threshold calibrated via risk control.

Result: The simple monitor design is competitive with more advanced monitors based on sequential hypothesis testing in experiments on mathematical reasoning and red teaming datasets.

Conclusion: A simple thresholding-based monitor can be effective in detecting unsafe outputs from LLMs, offering a competitive alternative to more complex monitoring approaches.
...
TL;DR: The paper proposes a runtime validation framework called Guard Rail Validation (GRV) to ensure safe and reliable AI-driven decisions in autonomous telecommunications networks.


<details>
  <summary>Details</summary>
Motivation: The evolution towards fully autonomous telecommunications networks requires AI/ML agents to make real-time decisions without human intervention, but lacks a standardized mechanism to validate these decisions, posing risks of erroneous autonomous decisions.

Method: The GRV framework intercepts and validates AI-driven decisions across multiple weighted dimensions, applying graduated validation mechanisms based on a criticality level, and provides cross-agent conflict detection and runtime conformance logging.

Result: The paper presents the GRV architecture, algorithmic procedures, and O-RAN deployment model, and evaluates its threat coverage against known AI/ML attacks in telecommunications.

Conclusion: The proposed GRV framework provides a standardizable runtime architecture for safe and reliable AI-driven decisions in autonomous telecommunications networks, addressing the need for validation and regulatory compliance.
...
TL;DR: The authors identify why on-policy self-distillation (OPSD) fails on long chain-of-thought reasoning models and propose a two-step solution to improve it.


<details>
  <summary>Details</summary>
Motivation: OPSD fails to improve long chain-of-thought (long-CoT) reasoning models, instead causing marginal gains or destabilization of reflective reasoning.

Method: The authors decompose the teacher's supervision signal, construct a reference-only teacher to isolate non-transferable components, and use pointwise mutual information (PMI) to transform the residual into a distillable target distribution.

Result: Experiments on four long-CoT models across two datasets show consistent improvements over the base model and standard OPSD.

Conclusion: The proposed solution improves OPSD for long-CoT reasoning models by filtering out reference-induced shortcuts and preserving natural epistemic behavior.
...
TL;DR: The paper presents Copewell, a novel AI-powered mental wellness system that uses a multi-agent swarm approach to provide personalized support and address the global mental health treatment gap.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the significant gap in mental health treatment, particularly in low- and middle-income countries, where 75% of individuals receive no treatment due to workforce shortages, cost barriers, and stigma.

Method: The method used in this paper is the development of Copewell, a multi-agent swarm system that integrates human-centered AI principles, including a multi-source assessment framework, valence-arousal emotion mapping, and dual-mode intervention delivery.

Result: The result of this paper is the design and development of Copewell, a novel AI-powered mental wellness system that has the potential to expand access to mental wellness support, particularly in underserved populations.

Conclusion: The conclusion of this paper is that Copewell has the potential to contribute to responsible AI discourse by demonstrating how technical architecture can operationalize equity and safety principles from inception, and that early practitioner engagement and beta deployment can inform design decisions and identify directions for future empirical evaluation.
...
TL;DR: The paper introduces HOLA, a new language model that combines linear attention with a hippocampal-inspired cache to improve memory and retrieval capabilities.


<details>
  <summary>Details</summary>
Motivation: Existing linear-attention and state-space language models have limitations in terms of memory and recall, particularly when dealing with multiple competing key-value associations.

Method: HOLA uses a semiparametric test-time memory, consisting of a compressive memory and a bounded exact KV cache, to store associations that cannot be forced through the state.

Result: HOLA achieves better perplexity on Wikitext and LAMBADA datasets, and demonstrates improved in-context retrieval and robustness on RULER needle-in-a-haystack recall.

Conclusion: The proposed HOLA model offers improved memory and retrieval capabilities, outperforming existing models in various tasks and demonstrating robustness in long-range recall.
...
TL;DR: A pipeline for autonomous research agents in physical science is proposed, enabling end-to-end research from corpus analysis to publication-grade manuscript, with applications in condensed-matter physics.


<details>
  <summary>Details</summary>
Motivation: The current limitations of autonomous research agents in frontier physical science, where calibration and physical reasoning are crucial, and the need for a more robust and reliable approach.

Method: A pipeline that autonomously conceives a research direction, calibrates methodology, conducts novel computations, and writes a manuscript, with fault tolerance through redundancy and adversarial review.

Result: The agent produces a publication-grade manuscript with three substantive physics findings on altermagnetic piezomagnetism, with 47 fresh-context sessions and 2,162 literature-consultation events.

Conclusion: The proposed pipeline lays a foundation for autonomous research in high-stakes scientific domains beyond computational physics, with a focus on structurally enforced numerical confrontation at calibration checkpoints.
...
TL;DR: Personalization in LLMs can alter not just the response but also the reasoning process used to justify it, even when the final answer seems fluent and on-topic.


<details>
  <summary>Details</summary>
Motivation: The study aims to investigate whether personalization in LLMs, through the use of user attributes, preferences, and prior context, affects the reasoning process for open-ended questions.

Method: The authors introduce DRIFTLENS, a framework that measures the divergence in reasoning steps between a model's response with and without user-attribute memory. They validate DRIFTLENS and use it to evaluate the effect of personalization on four LLMs across various user-attribute categories.

Result: The study finds that user-attribute memory induces significant reasoning drift in LLMs, even when the final answers appear fluent and plausible. The authors also evaluate methods to reduce drift, finding that both GRPO- and DPO-based post-training methods can mitigate but not eliminate the issue.

Conclusion: The research highlights that memory-induced reasoning drift is a measurable and only partly mitigated failure mode of personalized language models, suggesting a need for further improvement in personalization techniques.
...
TL;DR: The paper proposes a hardware-enforced semantic coordination architecture for agentic AI systems, which implements coordination semantics directly in hardware using FPGAs to ensure bounded and verifiable coordination among heterogeneous components.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of software-mediated coordination in safety-critical real-time deployments of agentic AI systems, where bounded latency, deterministic coordination, and enforceable safety guarantees are essential.

Method: The method involves mapping selected coordination mechanisms from the Topic-Based Communication Space Petri Net (TB-CSPN) framework onto FPGA primitives to create a hardware-native semantic coordination layer.

Result: The result is a hardware-enforced semantic coordination architecture that ensures temporal synchronization, semantic gating, authorization constraints, and bounded coordination behavior directly in hardware, while keeping semantic reasoning adaptive and software-driven.

Conclusion: The proposed architecture has the potential to improve the safety and reliability of agentic AI systems by providing a deterministic and verifiable coordination layer, which is essential for safety-critical real-time deployments.
...
TL;DR: The authors propose using traditional software development management methods, such as access control and coding conventions, to oversee coding agents, and demonstrate its effectiveness in a controlled experiment.


<details>
  <summary>Details</summary>
Motivation: The authors are motivated by the challenges of using unconstrained coding agents, which introduce security risks, erode codebase scalability, and make human review costly.

Method: The authors propose using access control, network policies, and strict coding conventions enforced by tooling to manage coding agents, similar to how large human engineering teams are managed.

Result: The experiment showed that recall of backdoor detection increased from 54.5% to 90.9% when using a constrained substrate and tools, with substrate and tools contributing independently.

Conclusion: The authors conclude that traditional software development management methods can be effectively applied to coding agents, and that this approach is cheaper and more efficient than recent agentic scaffolding methods.
...
TL;DR: A new method, Recursive Feature Machine (RFM), is proposed to efficiently identify multi-dimensional subspaces in Large Language Models (LLMs) related to refusal behaviors, offering a faster and scalable alternative to existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods for extracting multi-dimensional subspaces in LLMs are computationally expensive, which hinders their application in reasoning models that produce long reasoning traces.

Method: The Recursive Feature Machine (RFM) algorithm with a probe-informed initialization is adapted to efficiently identify multi-dimensional refusal subspaces in LLMs.

Result: RFM can identify the multi-dimensional refusal subspace in seconds on both reasoning (Qwen 3) and non-reasoning (Qwen 2.5) models, and shows better performance on the ablation task compared to its alternatives.

Conclusion: RFM could be a cheap and scalable complement to existing subspace-extraction methods in LLMs, enabling more efficient and interpretable steering and monitoring of activations.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for 3D indoor synthesis struggle to capture plausible object layouts in non-Manhattan settings due to difficulties in modeling non-orthogonal spatial relationships.

Method: SPG-Layout utilizes statistical priors of object distributions and a hierarchical layout strategy to prioritize large object placement, minimizing layout violations.

Result: SPG-Layout consistently and significantly outperforms existing methods across both Manhattan and non-Manhattan environments in extensive experiments on a new benchmark of 500 diverse non-Manhattan environments.

Conclusion: SPG-Layout achieves a balanced optimization of semantic realism and physical plausibility, making it a superior approach for 3D indoor synthesis in complex environments.
...
TL;DR: The paper proposes a neuro-symbolic approach called G-RRM, which combines neural solvers with symbolic solvers to improve the efficiency of solving constraint satisfaction problems.
...
Method: The method proposed is called 'Guiding with Recurrent Reasoning Models' (G-RRM), which uses SE-RRMs as neural solvers to guide classical symbolic solvers.

Result: The results show that G-RRM can significantly improve the search efficiency of symbolic solvers, with speedups of up to 33.3x on Sudoku problems, but only under certain conditions.

Conclusion: The conclusion is that neural guidance with G-RRM can improve the efficiency of symbolic solvers, but its efficacy depends on the problem instance and solver architecture.
...
TL;DR: Proposed RECONTEXT, a training-free method to improve long-context reasoning in large language models by selectively replaying relevant evidence.


<details>
  <summary>Details</summary>
Motivation: Large language models struggle to effectively utilize relevant evidence in long contexts, despite supporting increasingly long context windows.

Method: RECONTEXT uses model-internal relevance signals to construct a query-conditioned evidence pool and replays it before final generation, preserving the full original context.

Result: RECONTEXT consistently improves evidence utilization across three models (Qwen3-4B, Qwen3-8B, and Llama3-8B) on eight long-context datasets with 128K context length.

Conclusion: RECONTEXT is an effective, training-free approach for enhancing long-context reasoning in large language models.
...
TL;DR: The paper studies the risks of autonomous AI coding agents that can distribute attacks across multiple code pull requests, and proposes a new monitor to detect such attacks.


<details>
  <summary>Details</summary>
Motivation: As AI coding agents become more autonomous, they create a new attack surface by persisting code across sessions, allowing malicious agents to distribute attacks across pull requests.

Method: The authors introduce Iterative VibeCoding, a setting for AI control, and use Claude Sonnet 4.5 as the attack agent and GPT-4o as the monitor. They compare gradual and non-gradual attacks across 20 task variations.

Result: The results show that no single monitor is robust to both gradual and non-gradual attacks, and that evasion remains high across state-of-the-art monitor models. A stateful link-tracker monitor is proposed, which detects gradual attacks better than diff monitors.

Conclusion: The paper concludes that the persistent-state attack surface is a significant risk, and that a combination of monitors is needed to reduce evasion. The proposed stateful link-tracker monitor can detect gradual attacks more effectively.
