...
TL;DR: The paper presents a query-efficient black-box attack method that generates adversarial patches to deceive object detectors, achieving strong suppression across various detectors.


<details>
  <summary>Details</summary>
Motivation: Adversarial patches pose a practical threat to modern object detectors, and prior work has shown vulnerability, but with limitations in optimizing patch location, texture, and size under tight query budgets.

Method: The proposed method, a query-efficient, budget-adaptive black-box attack, couples a lightweight Contextual Thompson-Sampling placer with NES-style pixel updates, growing the patch only when progress stalls.

Result: The method achieves strong suppression on CNN-based detectors and substantial suppression on the transformer-based detector, using compact patches and exposing clear query-footprint trade-offs.

Conclusion: The paper presents a practical and efficient method for generating adversarial patches that can deceive object detectors, with a focus on query efficiency and visual footprint.
...
TL;DR: A new method, CAOA, is proposed for accurately aligning CAD models with objects in indoor RGB-D scans, improving alignment accuracy by 17% over state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: The task of aligning CAD models to their corresponding objects in indoor RGB-D scans is challenging due to noisy and incomplete scans, segmentation errors, and geometric distortions.

Method: Completion-Assisted Object-CAD Alignment (CAOA) integrates a point cloud completion module with a symmetry-aware relative pose estimation algorithm, and uses a synthetic data generation strategy and symmetry-aware loss to improve alignment accuracy.

Result: CAOA achieves a 17% accuracy improvement over state-of-the-art methods on the Scan2CAD benchmark.

Conclusion: The proposed CAOA method effectively addresses the challenge of aligning CAD models with objects in indoor RGB-D scans, achieving improved accuracy and robustness.
...
TL;DR: A new method called RegimeVGGT is proposed to accelerate the Visual Geometry Grounded Transformer (VGGT) by compressing its layers in a U-shaped manner, achieving a 6.7x speedup without sacrificing reconstruction quality.


<details>
  <summary>Details</summary>
Motivation: The Visual Geometry Grounded Transformer (VGGT) is limited by its quadratic cross-frame attention, which hinders its scalability. Existing accelerators have limitations in handling layer heterogeneity.

Method: RegimeVGGT applies layer-wise U-shaped compression along two axes: Saliency-Guided Banded Merging and Selectively Protected K/V Downsampling.

Result: RegimeVGGT achieves a 6.7x speedup over VGGT* at matched reconstruction quality.

Conclusion: The proposed RegimeVGGT method effectively accelerates VGGT while preserving its reconstruction quality, making it a promising solution for dense 3D scene structure recovery from multi-view images.
...
TL;DR: A new reward framework, Consensus Frame GRPO (CF-GRPO), is proposed for evidence-aware video reasoning in multimodal large language models, achieving competitive performance without human temporal annotations.


<details>
  <summary>Details</summary>
Motivation: Applying outcome-only rewards to video multimodal large language models provides limited guidance on which visual evidence should support the answer, and the authors aim to improve this with a new reward framework inspired by multisensory integration.

Method: The proposed CF-GRPO framework constructs a consensus frame prior from intrinsic video cues and computes a model-side frame-use score, optimizing their agreement through the Consensus Frame Reward (CFR) with salience-aware sparse aggregation and distribution sharpening.

Result: VideoCFR achieves competitive performance across complex video reasoning benchmarks and improves several metrics over representative Video-MLLM and RL baselines.

Conclusion: The proposed CF-GRPO framework provides a high-contrast reward signal without requiring human temporal annotations and offers an interpretable view of the evidence frames emphasized during training.
...
TL;DR: ReFine3D, a regularized fine-tuning framework, improves domain generalization of 3D large multimodal models by 1.36-3.11% across various benchmarks.


<details>
  <summary>Details</summary>
Motivation: Domain adaptation in 3D vision is challenging, especially for multimodal foundation models that suffer from overfitting and catastrophic forgetting when adapted to downstream domains with limited data.

Method: ReFine3D combines selective layer tuning with targeted regularization strategies, including multi-view consistency, text diversity, point-rendered vision supervision, and test-time augmentation.

Result: ReFine3D outperforms prior state-of-the-art methods, improving base-to-novel class generalization, cross-dataset transfer, robustness to corruption, and few-shot accuracy with minimal added computational overhead.

Conclusion: ReFine3D effectively addresses domain adaptation challenges in 3D vision, enabling more robust and generalizable multimodal models.
...
TL;DR: The authors propose Data-Forcing Distillation (DFD), a simple post-training framework that improves the diversity and fidelity of videos generated by Distribution Matching Distillation (DMD) models.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this work is to address the limitations of existing methods, specifically the drop in sample diversity and over-saturation of outputs, in distilling multi-step video diffusion models.

Method: The authors propose a post-training framework called Data-Forcing Distillation (DFD) that uses teacher score discrepancy to guide the student model toward the real-data distribution.

Result: The proposed framework, DFD, effectively restores diversity and fidelity in DMD models with only 100-300 steps of finetuning, outperforming the teacher model and resolving over-saturation artifacts.

Conclusion: The authors conclude that DFD is a simple yet effective approach to improve the quality of generated videos by mitigating mode collapse and over-saturation, achieving better video dynamics and appearance.
...
TL;DR: The Vines-DB dataset provides 1,218 high-resolution images of 7 ornamental vine species for multi-class instance segmentation in precision horticulture and urban ecology.


<details>
  <summary>Details</summary>
Motivation: To support the development and evaluation of deep learning models for multi-class instance segmentation in precision horticulture and urban ecology, enabling applications such as automated canopy cover estimation, species identification, and scalable field phenotyping.

Method: The dataset was generated by collecting high-resolution RGB images of 168 individual vine plants across multiple months, annotating them with polygon-based instance segmentation masks, and expanding the dataset through preprocessing and data augmentation.

Result: The Vines-DB dataset contains 2,307 images, divided into 2,019 training images, 192 validation images, and 96 test images, which can be used for model development and evaluation.

Conclusion: The Vines-DB dataset enables the development of deep learning models for multi-class instance segmentation in precision horticulture and urban ecology, with applications in automated canopy cover estimation, species identification, and scalable field phenotyping.
...
TL;DR: The paper introduces a learned generalization of phase correlation for image registration, allowing for non-rigid deformations and unitary dynamics, and demonstrates its effectiveness on various benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current learning-based methods for image registration do not explicitly represent the transformation between images, instead relying on implicit discovery through learned similarity functions or deep decoders.

Method: The authors propose a learned generalization of phase correlation, which learns the basis for transformation decomposition, allowing for dense non-rigid deformations and unitary dynamics.

Result: The framework achieves state-of-the-art results on ACDC cardiac-MRI and CAMUS echocardiography benchmarks, and successfully recovers Hermite-function eigenstates and quantized energy levels of the unknown Hamiltonian from observation pairs.

Conclusion: The proposed framework provides a unified approach to image registration and analysis, demonstrating its effectiveness across various applications, including medical imaging and quantum physics.
...
TL;DR: This paper investigates whether Vision Transformer architectures can reduce demographic bias in face presentation attack detection (PAD) systems compared to convolutional baselines.


<details>
  <summary>Details</summary>
Motivation: Existing face PAD systems exhibit performance disparities across demographic groups, disproportionately affecting individuals with darker skin tones.

Method: The paper evaluates three architectures: a Multimodal ViT-Tiny trained from scratch, a ResNet18 CNN baseline, and a pretrained DeiT-S fine-tuned on the CASIA-SURF Cross-Ethnicity Face Anti-Spoofing (CeFA) dataset.

Result: DeiT-S achieves the highest overall accuracy of 97.27% and the lowest EER of 0.86%, outperforming ResNet18. DeiT-S also reduces the inter-ethnic ACER gap between African and East Asian subjects to 0.13% and maintains a lower BPCER of 2.89% on zero-shot Central Asian subjects.

Conclusion: Pretrained Vision Transformers achieve superior PAD accuracy, produce smaller demographic performance gaps, and generalize more equitably across unseen demographic groups, suggesting that architectural design may influence cross-demographic fairness in PAD.
...
TL;DR: A data-driven strategy is proposed to generate diverse negative samples for offline handwritten signature verification, improving detection of skilled forgeries and computational efficiency.


<details>
  <summary>Details</summary>
Motivation: The random selection of negative samples from genuine signatures of other users lacks diversity, increases redundancy, and escalates computational cost.

Method: A data-driven strategy to generate diverse, informative negative samples using prototypical signatures, which are compact, non-identifiable summaries of genuine signature features.

Result: The proposed approach yields more informative negative samples, improving the detection of skilled forgeries, is backbone-agnostic, and serves as an alternative to RBF-based models while significantly improving scalability and computational efficiency.

Conclusion: The proposed approach improves the detection of skilled forgeries, is robust across architectures, and significantly improves scalability and computational efficiency.
...
TL;DR: A novel retrieval-augmented image captioning framework is proposed, leveraging external knowledge to generate comprehensive captions with deeper insights.


<details>
  <summary>Details</summary>
Motivation: Traditional image captioning methods struggle to generate context-rich descriptions, especially for details not directly observable from visual cues.

Method: A hierarchical multi-modal article retrieval mechanism and a contextual relevance refinement stage are used to retrieve relevant articles, which serve as a knowledge base for caption generation.

Result: The proposed framework achieved 5th place with an overall score of 0.2824 on the private test set of the OpenEvent-V1 dataset in the ACM Multimedia EVENTA 2025 Challenge.

Conclusion: The proposed framework effectively generates comprehensive, contextually detailed captions by leveraging external knowledge.
...
TL;DR: A new benchmark dataset called Forged Calamity is introduced to detect synthetic disaster images generated by text-to-image diffusion models. The study finds that current forensic approaches have weaknesses, particularly in generalizing to unseen generators or disaster types.


<details>
  <summary>Details</summary>
Motivation: The rapid advancement of text-to-image diffusion models has made it difficult to distinguish authentic content from AI-generated fabrications, posing challenges for cybersecurity, digital forensics, and disaster response.

Method: The study introduces a benchmark dataset called Forged Calamity, containing 30,000 images, and conducts comprehensive experiments across fine-tuned and zero-shot settings to evaluate current forensic approaches.

Result: The study finds that fine-tuned detectors perform well in-distribution but lose up to 50% accuracy on unseen generators or disaster types, while zero-shot generalized detectors struggle to maintain stable accuracy.

Conclusion: The study highlights persistent generalization gaps in current forensic approaches and emphasizes the urgent need for domain- and model-agnostic detection methods to ensure visual authenticity in the diffusion era.
...
TL;DR: The paper proposes using Stable Diffusion to generate synthetic indoor images for data augmentation, enhancing indoor image recognition models, and introduces a countermeasure to prevent misuse.


<details>
  <summary>Details</summary>
Motivation: The lack of training indoor images presents a challenge for indoor image recognition due to complex lighting, occlusions, and object arrangements.

Method: The paper uses Stable Diffusion for synthetic image generation and proposes a countermeasure based on Diffusion Reconstruction Error (DIRE) to prevent misuse.

Result: The approach enhances deep model training with limited authentic data, achieving 100% accuracy in recognizing SD-generated images using MobilenetV3.

Conclusion: The proposed approach has the potential to improve indoor image recognition models by enriching the training data pool with synthetic images.
...
TL;DR: The paper presents a method for 3D point motion forecasting, which predicts the future trajectory of 3D points on an object given a short visual history, a set of 3D query points, and a language description of the intended goal.


<details>
  <summary>Details</summary>
Motivation: The authors argue that 3D points in world coordinates provide a general representation that is useful for downstream tasks, and that anticipating object motion is crucial for visual intelligence.

Method: The authors introduce MolmoMotion, a general motion forecasting model that supports both autoregressive coordinate prediction and flow-matching-based trajectory generation, and a large corpus of action-described, object-grounded 3D point trajectories (MolmoMotion-1M) and a human-verified benchmark (PointMotionBench).

Result: MolmoMotion accurately predicts diverse motion patterns with different language instructions and outperforms existing motion prediction baselines on PointMotionBench.

Conclusion: The learned 3D motion prior transfers well to downstream applications, improving training efficiency and generalization for robot manipulation, and providing effective motion guidance for generative models to synthesize videos with more realistic object motion.
...
TL;DR: The paper experimentally investigates neural image classification on CIFAR-10 using fully connected and convolutional networks, evaluating a convolutional architecture's performance.


<details>
  <summary>Details</summary>
Motivation: To analyze the complete learning pipeline for neural image classification and provide a baseline for future studies on regularization, data augmentation, and deeper architectures.

Method: Experimental investigation using fully connected and convolutional network formulations on CIFAR-10, with a focus on the learning pipeline including image vectorization, normalization, and mini-batch training.

Result: A convolutional architecture achieved a validation accuracy of approximately 74.77% and highlighted the difference between representation learning and memorization.

Conclusion: The study provides a compact experimental baseline for future research and illustrates the practical difference between representation learning and memorization in neural image classification.
...
TL;DR: A new method for crowd counting in low-light environments is proposed, which uses a multi-modal approach and achieves state-of-the-art performance on three benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing crowd counting methods struggle in low-light environments, and there is a need for a more robust approach that can handle extreme darkness and complex illumination.

Method: The proposed method, LCNet, uses a multi-modal hyper-graph fusion module and a deformable rectangular sparse attention module to incorporate depth and edge cues, and adaptively allocate computation in dense prediction.

Result: The proposed method achieves the best overall performance against existing state-of-the-art methods on three low-light crowd counting benchmarks.

Conclusion: The proposed method provides a robust solution for crowd counting in low-light environments, and the datasets and code will be made publicly available.
...
TL;DR: The authors present a solution that won the GOOSE 2D Fine-Grained Semantic Segmentation Challenge, achieving 76.57% composite score.


<details>
  <summary>Details</summary>
Motivation: The challenge aims to evaluate dense semantic segmentation of off-road imagery with a fine-grained taxonomy of 64 classes.

Method: The solution combines a self-supervised DINOv3 ViT-L/16 backbone, ViT-Adapter, and Mask2Former decoder with a coarse-category auxiliary loss and an inference-time aggregation strategy.

Result: The method achieves 69.32% fine-class mIoU and 83.81% category-level mIoU, ranking first on the leaderboard.

Conclusion: The proposed solution effectively improves the performance of dense semantic segmentation on off-road imagery.
...
TL;DR: A novel framework for aerial-ground LiDAR place recognition is proposed, addressing domain gaps and false positives.


<details>
  <summary>Details</summary>
Motivation: Ground-level LiDAR place recognition has limitations, and using aerial prior maps offers advantages but poses challenges.

Method: A retrieval network with patch-level self-supervised learning and scene-level learning, plus an Expanded Reciprocal re-ranking algorithm.

Result: Outperforms state-of-the-art methods, achieving 9.8% and 3.2% improvements in average Recall@1 and Recall@1% respectively.

Conclusion: The proposed framework effectively addresses aerial-ground LiDAR place recognition challenges, demonstrating superior performance.
...
TL;DR: The paper introduces Atomic Physical Transitions (APTs) to make explicit the hidden process of physical events in videos, and proposes APT-Tune to teach VLMs to use causal transitions without forgetting how to answer video questions.


<details>
  <summary>Details</summary>
Motivation: Current VLMs lack understanding of physical events at a granular level, and clip-level labels hide the underlying physical process.

Method: The authors introduce APTs, construct mixed-source APT data, and propose APT-Tune, a parameter-efficient recipe for VLMs to learn APTs.

Result: APTs improve transition detection, and APT-Tune substantially improves APT recall while also improving event-level video transfer with only 11 M LoRA parameters.

Conclusion: APTs provide a human-aligned causal supervision signal for physical video understanding, and APT-Tune enables VLMs to learn APTs without forgetting how to answer video questions.
...
TL;DR: A human-AI co-creation video generation framework called CHIEF is proposed, allowing creators to guide video generation with iterative refinement and subjective feedback.


<details>
  <summary>Details</summary>
Motivation: AI-generated videos often lack narrative coherence and creative direction, particularly for longer durations, and require subjective feedback on plot, scenes, and narrative.

Method: The CHIEF framework incorporates human creative direction through iterative refinement, using a specialized refiner agent and persona-conditioned multimodal LLMs to provide subjective feedback.

Result: The framework was tested with students who created videos ranging from 1-minute to 10-minute films with complicated plots, showing its effectiveness.

Conclusion: The CHIEF framework enables effective human-AI co-creation of videos with narrative coherence and creative direction, supporting creators with automatic subjective feedback.
...
TL;DR: The paper proposes CoEV, a framework to detect and correct hallucinations in Vision-Language models (VLMs) for medical diagnosis, improving their reliability.


<details>
  <summary>Details</summary>
Motivation: The reliability of VLMs in medical diagnosis is challenged by hallucinations, and existing detection approaches have limitations, particularly in verifying visual evidence supporting generated text.

Method: The authors propose CoEV, a training-free plug-and-play framework that performs bidirectional verification between textual assertions and visual evidence to detect and correct hallucinations.

Result: CoEV outperforms existing methods in hallucination detection, improving PR-AUC and ROC-AUC by 3.0% and 3.9% respectively, and also improves hallucination correction, Micro-F1 by up to 12.5%, and reduces hallucination rates by over 11.9%.

Conclusion: CoEV enables reliable detection and correction of hallucinations in VLMs, providing clinicians with dependable, evidence-based cues for diagnosis.
...
TL;DR: A new method, Intrinsic-GS, is proposed for segmenting dynamic 3D scenes represented by 4D Gaussian Splatting. It works without 2D masks from foundation models, instead using Gaussian primitives' appearance, orientation, scale, deformation-trajectory, and rendered-boundary cues to build a sparse affinity graph, which is then partitioned with Leiden community detection.


<details>
  <summary>Details</summary>
Motivation: Current methods for segmenting dynamic 3D scenes rely on importing and lifting 2D masks from foundation models, which can be costly and dependent on mask quality. The authors aim to recover object-level structure directly from the Gaussians themselves.

Method: Intrinsic-GS builds a sparse affinity graph over Gaussian primitives using appearance, orientation, scale, deformation-trajectory, and non-learned rendered-boundary cues. The graph is then partitioned with Leiden community detection, requiring no foundation model and no learned feature field.

Result: Intrinsic-GS achieves 0.746 mIoU on Neu3D and 0.575 on HyperNeRF without mask supervision. A geometry-only variant reaches 0.902 mIoU on Neu3D, matching SAM-supervised TRASE. Intrinsic-GS runs 12.5x faster than mask-supervised pipelines on HyperNeRF.

Conclusion: The proposed Intrinsic-GS method demonstrates that substantial object structure can be recovered from the Gaussians themselves, offering a fast, mask-free direction for 3D and 4D Gaussian segmentation. This approach may lead to more generalizable and robust segmentation in settings where external masks are unreliable or expensive.
...
TL;DR: The paper proposes a spiking pyramid wavelet-based model (SPWM) for efficient image restoration, leveraging discrete wavelet transformation to improve performance while reducing computational costs and energy consumption.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the limitations of traditional spiking CNN-based methods for image restoration tasks, which are constrained by the inherent receptive field limitations of CNN operations, and to explore the potential of spiking neural networks (SNNs) in resource-limited devices.

Method: The method proposed in this paper is a spiking pyramid wavelet-based model (SPWM) that utilizes a spiking dual pyramid wavelet (SDPW) block to model long-range dependency and exploit the properties of degradation in the wavelet domain.

Result: The experimental results demonstrate that SPWM significantly lowers computational costs and energy consumption while maintaining image quality, showcasing the potential of SNNs in the field of image restoration.

Conclusion: The conclusion of this paper is that the proposed SPWM offers a high-efficient and low-energy solution for image restoration tasks, providing new insights for future applications of resource-limited devices.
...
TL;DR: The authors propose a new framework for learning unsupervised representations of medical imaging cohorts, which improves upon existing methods by preserving the geometric structure of the data and producing more accurate and stable prototypes.


<details>
  <summary>Details</summary>
Motivation: Existing deep latent-variable models have limitations in estimating Gaussian mixture priors and producing prototypes that capture true pathological heterogeneity, especially as the number of sub-populations grows.

Method: The authors propose a manifold-anchored variational framework built on a geometry-aware Expectation-Maximization (EM) algorithm, which selects sub-population prototypes as graph medoids with high diffusion centrality on a latent graph.

Result: The framework achieves the highest accuracy among compared methods on cardiac scar and brain MRI benchmarks, produces sharp prototypes, and remains stable with large sub-population counts.

Conclusion: The proposed manifold-anchored EM framework provides a general-purpose geometric tool for learning unsupervised representations of medical imaging cohorts, improving accuracy, stability, and prototype quality.
...
TL;DR: The authors propose a framework for intelligent landslide hazard interpretation, which includes a multimodal dataset, a landslide-oriented vision-language model, and a domain rule-enhanced agent. The framework achieves improved accuracy in landslide discrimination, classification, and semantic description.


<details>
  <summary>Details</summary>
Motivation: Current paradigms for landslide hazard interpretation struggle to extract visual features and high-level geoscientific semantics, while general-purpose vision-language models suffer from perceptual limitations and domain hallucinations in complex geological scenarios.

Method: The authors propose a three-component framework: (1) LandslideBench, a multimodal fine-grained dataset; (2) LandslideVLM, a landslide-oriented vision-language model fine-tuned on LandslideBench; and (3) LandslideAgent, a domain rule-enhanced agent that regulates automated tool invocation.

Result: Experiments demonstrate that LandslideBench provides effective baselines across five mainstream models, and LandslideVLM achieves accuracy improvements of 10.96%, 32.87%, and 15.91% on landslide discrimination, fine-grained classification, and semantic description quality, respectively. LandslideAgent enables autonomous multi-source spatial data inference.

Conclusion: The proposed framework enables full-process intelligence for landslide identification and analysis, and achieves improved accuracy in landslide hazard interpretation.
...
TL;DR: Developed BrainFusionNet, a hybrid model combining CNNs, ViT, and GRUs for improved brain tumor classification from MRI images, achieving 98% accuracy.


<details>
  <summary>Details</summary>
Motivation: The noise in MRI images and complex tumor boundaries pose challenges for deep learning models, necessitating a more effective approach for brain tumor classification.

Method: BrainFusionNet integrates CNNs, ViT, and GRUs to extract spatial, contextual, and sequential features from MRI images, with explainable AI for visualizing decision-making processes.

Result: BrainFusionNet achieved 98% accuracy on two publicly available MRI datasets, outperforming state-of-the-art CNNs (e.g., DenseNet121 and VGG16 with 96% accuracy).

Conclusion: BrainFusionNet effectively extracts local and global features from MRI images, even in small-scale tumor regions and small tumor sizes, with a balanced sequential CNN architecture and customized ViT.
...
TL;DR: The paper introduces SPARE, a method for reducing the computational overhead of Vision Language Models (VLMs) by selecting the most relevant visual tokens, achieving state-of-the-art performance and retaining 95% of baseline performance while removing 94% of visual tokens.


<details>
  <summary>Details</summary>
Motivation: The large number of visual tokens in VLMs incurs substantial computational overhead, and existing token reduction methods have limitations, particularly on compositional multi-skill reasoning tasks.

Method: The paper proposes SPARE, a subspace reconstruction method that reformulates token pruning as a column subset selection problem and explicitly minimizes reconstruction error, with an additional selection criterion based on anti-relevance to promote context-aware token selection.

Result: SPARE consistently achieves state-of-the-art performance across multiple VLMs and benchmarks, with strong gains on compositional tasks, and removes up to 94% of visual tokens while retaining 95% of the baseline performance.

Conclusion: The paper concludes that SPARE is an effective method for reducing the computational overhead of VLMs while preserving their performance, particularly on compositional multi-skill reasoning tasks.
...
TL;DR: This study compares the performance of five convolutional neural networks (CNNs) in classifying multi-class brain tumors from MRI images, finding that EfficientNetB0 achieves the highest accuracy at 95%.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this research is to address the challenges of accurately classifying brain tumors from MRI images despite recent advancements in deep learning.

Method: The study evaluates five different CNN architectures, including a customized baseline model and four pre-trained models (VGG16, VGG19, DenseNet121, and EfficientNetB0), using a clinically-sourced dataset of approximately 10,000 MRI images.

Result: EfficientNetB0 achieved the best overall classification accuracy at 95%, outperforming other architectures such as VGG16 (94.37%), VGG19 (92.29%), DenseNet121 (90.91%), and the customized CNN (78.00%). It also showed significant improvement in detecting meningiomas with a recall rate of 89% compared to 20% by simple CNNs.

Conclusion: EfficientNetB0 provides the optimal trade-off between classification accuracy, number of parameters used in the model, and clinically meaningful performance, making it suitable for classifying multi-class brain tumors from MRI images.
...
TL;DR: The paper proposes a new method called spatially-stratified distillation (SSD) for improving radar place recognition in heterogeneous environments, achieving state-of-the-art results on the HeRCULES dataset.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the performance degradation of existing radar place recognition methods in multi-session environments, particularly when dealing with sparse and narrow field-of-view 4D radar sensors.

Method: The proposed method is spatially-stratified distillation (SSD), which uses asymmetric spatial alignment derived from physical radar returns to improve feature alignment between different radar signals.

Result: The results show that SSD significantly outperforms prior place recognition methods on the HeRCULES dataset, achieving state-of-the-art results on its challenging dynamic sequences.

Conclusion: The conclusion is that SSD is an effective strategy for improving radar place recognition in heterogeneous environments, particularly in cases where there is a significant difference in the density of structural information between the 4D radar sensor and the reference map.
...
TL;DR: The paper proposes UniTemp, a framework for arbitrary-direction video generation using autoregressive models, enabling flexible generation orders and improving controllability.


<details>
  <summary>Details</summary>
Motivation: Existing autoregressive video diffusion models are limited to forward temporal generation, which is not sufficient for practical video creation that often requires flexible generation orders.

Method: The authors introduce blockwise anchor latents to address the causal structure issue in 3D VAE and propose a bidirectional distillation framework, UniTemp, to train a single autoregressive student model for any-direction video generation.

Result: UniTemp maintains competitive performance on short and long video generation compared to forward-only methods and enables diverse workflows such as bidirectional video extension, inbetween generation, and visual story generation.

Conclusion: The proposed UniTemp framework provides a flexible and controllable approach for video generation in arbitrary temporal directions, expanding the possibilities for practical video creation.
...
TL;DR: A new method called PEFT-MedSAM is proposed for automatically segmenting dermoscopic skin lesions using deep learning, achieving high accuracy and outperforming baseline models.


<details>
  <summary>Details</summary>
Motivation: Most deep learning methods for skin lesion segmentation perform poorly, and there is a need for a more accurate and efficient method to help detect melanomas earlier.

Method: PEFT-MedSAM, a parameter-efficient fine-tuning method that adapts the Medical Segment Anything Model (MedSAM) to segment dermoscopic skin lesions, using a lightweight mask decoder for training while keeping the pre-trained image encoder and prompt encoder frozen.

Result: PEFT-MedSAM achieves a dice coefficient of 0.9411 and an intersection over union value of 0.8918 on the ISIC 2018 benchmark dataset, outperforming a fully trained U-Net baseline and zero-shot MedSAM inference, and shows high accuracy and low standard deviation on external validation using the PH2 dataset.

Conclusion: The proposed PEFT-MedSAM method shows high accuracy and efficiency in segmenting dermoscopic skin lesions, with supportive evidence from statistical tests and evaluation methodologies, making it a promising approach for clinical applications.
...
TL;DR: The paper proposes Geometry-Aware Pointer (GAP) Loss, a modified cross-entropy loss that reweights negative candidates based on spatial proximity to improve Table Structure Recognition (TSR) accuracy.


<details>
  <summary>Details</summary>
Motivation: Pointer networks for TSR often make errors between spatially adjacent cells, and standard cross-entropy loss does not account for this spatial relationship.

Method: The authors propose GAP Loss, which applies inverse distance weighting to reweight the cross-entropy objective based on spatial proximity to ground truth.

Result: GAP consistently reduces adjacent-cell errors and achieves new state-of-the-art performance on PubTabNet and SynthTabNet datasets.

Conclusion: Incorporating geometric inductive biases at the loss level provides a simple yet effective approach to robust TSR.
...
TL;DR: A new method, GeoCat, is proposed for accurate segmentation of intravascular ultrasound (IVUS) lumen and external elastic membrane (EEM) to assess coronary plaque burden.


<details>
  <summary>Details</summary>
Motivation: Standard methods for IVUS segmentation often suffer from boundary drift and topology errors, leading to inaccurate clinical measurements.

Method: GeoCat uses a geometry-consistent network with dual Cartesian-polar encoders, cross-domain attention, and temporal fusion, trained on 12,242 annotated frames from 146 patients.

Result: GeoCat achieves a Dice of 0.93, reduces 95HD to 0.14 mm, and lowers topology violations to 1.0%, with improved geometric fidelity.

Conclusion: GeoCat provides reliable plaque burden quantification by accurately segmenting IVUS lumen and EEM with improved geometric fidelity.
...
TL;DR: Proposed CS3F, a training-free framework for zero-shot anomaly detection in 3D medical images using 2D foundation models, achieving anomaly localization in brain MRI and lung CT.


<details>
  <summary>Details</summary>
Motivation: Existing zero-shot anomaly detection methods are limited in 3D medical images due to scarcity of large-scale volumetric foundation models and difficulty in utilizing volumetric context.

Method: CS3F decomposes volumes into slices, encodes them using a 2D vision transformer, and converts them into localized volumetric tokens for anomaly scoring through cross-subject mutual similarity.

Result: CS3F shows that frozen 2D foundation models can support anomaly localization in 3D medical images, with performance varying by lesion contrast and imaging modality.

Conclusion: CS3F offers a viable approach for zero-shot anomaly detection in 3D medical images using 2D foundation models, demonstrating potential for generalizability across different imaging modalities.
...
TL;DR: SMART is a framework for creating a flexible, interpretable, and scalable brain atlas from 3D medical images, outperforming existing methods in predicting Alzheimer's disease progression.


<details>
  <summary>Details</summary>
Motivation: Existing spatio-temporal atlas construction methods are limited by their lack of flexibility, interpretability, and scalability to high-dimensional data.

Method: SMART uses a continuous disease-time atlas, guided by anatomical priors, and models global trajectories of regional progression via region-specific differential equations, personalized to individual anatomies.

Result: SMART achieves state-of-the-art forecasting accuracy and improved temporal consistency on five longitudinal MRI datasets in Alzheimer's disease, producing anatomically meaningful predictions.

Conclusion: SMART establishes a new paradigm for flexible, interpretable, and scalable modeling of spatio-temporal change in high-dimensional medical image time-series.
...
TL;DR: SpectralDiT improves Diffusion Transformers with spectral correction, enhancing image generation quality.


<details>
  <summary>Details</summary>
Motivation: Improve flow-matching Diffusion Transformers with a lightweight modification for better image generation.

Method: Adding timestep-conditioned spectral correction to the MLP residual branch of Diffusion Transformers.

Result: Improved FID scores on CIFAR-10 and ImageNet-100, with 8.7% relative FID reduction on ImageNet-100.

Conclusion: SpectralDiT is a lightweight and effective method to improve image generation quality of Diffusion Transformers.
...
TL;DR: A unified framework, SAMA, is proposed for generating high-fidelity synthetic data for multimodal information extraction tasks, outperforming state-of-the-art baselines.


<details>
  <summary>Details</summary>
Motivation: Severe data scarcity in multimodal information extraction tasks and limitations of existing data augmentation approaches.

Method: Semantic Anchor-aligned Multimodal Augmentation (SAMA) framework, utilizing a Collaborative Multi-Experts Multimodal Large Language Model and Anchor-Preserving Diffusion mechanism.

Result: SAMA consistently outperforms state-of-the-art augmentation baselines in experiments across benchmark datasets for MNER, MRE, and MEE.

Conclusion: SAMA is a versatile, robust, and effective framework for generating synthetic data, addressing data scarcity in multimodal information extraction tasks.
...
TL;DR: Proposed REEM, a lightweight framework for infrared small target detection, improving detection accuracy and reducing false alarms.


<details>
  <summary>Details</summary>
Motivation: Infrared small target detection is challenging due to background clutter, low contrast, and weak spatial responses.

Method: REEM (Reweighted Explicit-visibility Enhanced Modulation) framework, which incorporates Signal-to-Clutter Ratio (SCR) as a visibility prior.

Result: Consistent improvements over baseline, achieving higher IoU and detection probability, with reduced false alarms, especially in low-visibility conditions.

Conclusion: SCR-guided difficulty-aware optimization provides an effective complement to conventional overlap-based objectives for infrared small target detection.
...
TL;DR: Proposed a learned method to select optimal support radius for improving local-patch Unsigned Distance Field (UDF) methods in surface reconstruction from point clouds.


<details>
  <summary>Details</summary>
Motivation: Traditional methods' accuracy depends on a fixed or heuristically selected support radius, which can't capture complex local geometry.

Method: A learned per-query radius selector is proposed and trained to predict a continuous support radius for a LoSF-UDF backbone.

Result: Experiments show improved fine-scale reconstruction accuracy.

Conclusion: The proposed method improves the accuracy of local-patch UDF methods in surface reconstruction by adaptively selecting the support radius.
...
TL;DR: A new method called HandwritingAgent is introduced for synthesizing natural handwriting sequences in SVG format, conditioned on text and a reference handwriting-style image, outperforming state-of-the-art models.


<details>
  <summary>Details</summary>
Motivation: The challenge of teaching machines to emulate natural handwriting styles, which requires synthesizing stroke sequences that vary in shape, texture, pressure, and script across and within individuals.

Method: HandwritingAgent, a language-driven agent that leverages a large reasoning model to geometrically analyze and autoregressively generate target handwritten glyphs as stroke sequences in a discrete grid canvas environment.

Result: Substantial improvement in performance on diverse handwriting tasks, including imitation, recognition, multi-lingual handwriting synthesis, and generation of complex handwritten maths and science expressions, matching or surpassing state-of-the-art generative handwriting models.

Conclusion: HandwritingAgent provides a more efficient, controllable, and generalizable synthesis method for natural handwriting sequences, conditioned on text and a reference handwriting-style image.
...
TL;DR: The paper proposes a fuzzy geometry-driven structure-aware augmentation framework for handwriting recognition, particularly for complex Chinese characters, and demonstrates its effectiveness on several datasets.


<details>
  <summary>Details</summary>
Motivation: Data scarcity and structural distortion limit handwriting recognition in high-security authentication, and existing augmentation methods can cause damage to complex characters.

Method: The paper proposes a fuzzy geometry-driven structure-aware (FGSA) augmentation framework, which models branch points as fuzzy sets and constructs a continuous membership field.

Result: Extensive experiments demonstrate that FGSA significantly reduces the word-level error rate and achieves optimal recognition gains over compared baselines.

Conclusion: The proposed FGSA framework offers a highly controllable solution for handwriting augmentation, striking a robust trade-off among task gain, structural fidelity, and discriminative feature preservation.
...
TL;DR: This paper proposes MMPM, a mode-aware framework for pedestrian trajectory prediction from ego-centric camera views, which models future trajectory distributions into semantically meaningful modes based on pedestrian crossing behavior.


<details>
  <summary>Details</summary>
Motivation: Existing stochastic predictors often yield sub-optimal 'mixed-mode' trajectories that lie between distinct motion patterns and become implausible in real scenes, because they sample multiple futures from a single unimodal distribution.

Method: The proposed MMPM framework consists of two modules: a behavior-aware Pedestrian Interaction Module (PIM) and a CVAE-based Mode-aware Trajectory Predictor (MTP) module, with a query-based decoder to enforce mode consistency during decoding.

Result: Experiments on PIE and JAAD datasets show that the proposed method surpasses state-of-the-art baselines, and can be integrated into existing frameworks to further improve future trajectory prediction performance.

Conclusion: The proposed MMPM framework improves pedestrian trajectory prediction by modeling future trajectory distributions into semantically meaningful modes based on pedestrian crossing behavior, and demonstrates improved frame-wise displacement errors over previous work.
...
TL;DR: DreamReg, a belief-driven framework, improves real-time 2D-3D registration in surgical navigation by refining transformations through learned dynamics and internal imagination.


<details>
  <summary>Details</summary>
Motivation: Existing methods for real-time registration between intraoperative 2D slices and preoperative 3D volumes are limited by partial observability, speckle noise, and action-dependent US acquisition.

Method: DreamReg uses a latent belief state to summarize past observations and poses, and refines transformations through learned dynamics as new slices arrive, utilizing internal imagination to simulate probe motions and predict observations.

Result: Experiments on CAMUS and u-RegPro datasets show improved robustness and competitive registration accuracy for real-time guidance compared with state-of-the-art methods.

Conclusion: DreamReg offers a robust and accurate solution for real-time 2D-3D registration in surgical navigation, enhancing the reliability of ultrasound-guided procedures.
...
TL;DR: The paper proposes a progressive cross-task collaboration framework for air-ground perception, achieving better performance than uniform fusion.


<details>
  <summary>Details</summary>
Motivation: Existing studies on air-ground collaboration overlook functional dependencies among tasks and struggle with heterogeneous views.

Method: The proposed Socialized Co-Perception (SCP) framework uses a Dual-Layer Router (DLR) for selective cross-view and cross-task interaction.

Result: SCP achieves a 3.73% coevolutionary gain and a 7.86% improvement in average downstream performance.

Conclusion: Task-conditioned collaboration is more effective than uniform fusion for heterogeneous air-ground perception.
...
TL;DR: The authors introduce ScreenAnnotator, an open-source tool for annotating data to train vision-language models (VLMs) for advanced visual reasoning tasks.


<details>
  <summary>Details</summary>
Motivation: Existing data annotation tools are insufficient for creating data that combines spatial coordinates, open-vocabulary descriptions, structured attributes, and topological relationships, which are necessary for training VLMs for sophisticated grounded structured visual reasoning.

Method: The authors propose a unified annotation atom schema, an on-policy annotation loop with a Bayesian Annotation Verifier (BAV), and a template-driven multi-task data synthesis process to improve annotation efficiency and data reusability.

Result: The on-policy loop achieves an annotation accept rate of nearly 100% on flowcharts and 77% on GUI screenshots, with a 35.1% absolute gain in VLM accuracy (76.1% average accuracy) in the flowchart scenario.

Conclusion: The proposed ScreenAnnotator tool effectively addresses the limitations of existing annotation tools and enables the creation of high-quality data for training VLMs, leading to improved performance in advanced visual reasoning tasks.
...
TL;DR: The paper proposes QUAM-SM, a post-hoc framework for reliable pixel-level uncertainty quantification in deep learning models, which outperforms existing methods in terms of reliability and boundary sensitivity.


<details>
  <summary>Details</summary>
Motivation: Standard deep learning models often suffer from miscalibration, yielding overconfident predictions that mask underlying vulnerabilities at subtle pathological boundaries, which can have critical implications for treatment planning and surgical intervention.

Method: The authors propose a post-hoc framework called QUAM-SM, which uses targeted adversarial search to identify 'adversarially fragile' pixels and disentangles epistemic uncertainty from aleatoric uncertainty.

Result: Experiments on two public datasets with multiple expert annotations demonstrate that QUAM-SM outperforms both standard and recent uncertainty estimation approaches in terms of reliability and boundary sensitivity.

Conclusion: The proposed QUAM-SM framework has the potential to transform clinical workflows by enabling high-fidelity longitudinal monitoring and distinguishing true pathological changes from artifacts, providing the stability required for critical treatment planning and surgical intervention.
...
TL;DR: KinemaForge is a pipeline for reconstructing digital twins of articulated objects from sensor observations, improving accuracy and reducing simulation drift.


<details>
  <summary>Details</summary>
Motivation: Current methods for reconstructing digital twins of articulated objects have two main limitations: (i) part-level geometric reconstruction is separate from kinematic-parameter estimation, and (ii) the resulting models often violate dynamic invariants, leading to drift in physics simulators.

Method: KinemaForge uses a constraint-driven approach, combining part-level shape, joint topology, and joint parameter estimation with an energy-consistent verifier built on differentiable rigid-body dynamics.

Result: KinemaForge reduces average joint-axis error by 37.4% and 46.6% compared to baselines, lowers long-horizon simulation drift by 64%, and improves closed-loop manipulation success rate by 14.6 percentage points.

Conclusion: KinemaForge is an effective pipeline for reconstructing simulation-ready digital twins of articulated objects, improving accuracy, and reducing simulation drift.
...
TL;DR: A novel weakly-supervised image quality transfer framework is proposed to correct geometric distortions in prostate diffusion-weighted imaging (DWI) without paired distorted and undistorted clinical scans.


<details>
  <summary>Details</summary>
Motivation: The absence of paired distorted and undistorted clinical scans challenges the development of automated correction methods for geometric distortions in prostate DWI.

Method: A weakly-supervised image quality transfer (IQT) framework utilizing image-level quality labels and a prototype flow matching algorithm to synthesize realistic distorted and undistorted image pairs.

Result: The proposed approach leads to more capable distortion correction IQT models, outperforming existing unpaired approaches in clinical downstream task performance in PI-RADS and Gleason score classification.

Conclusion: The proposed framework enables the development of reliable distortion correction models for prostate DWI without requiring paired distorted and undistorted clinical scans.
...
TL;DR: The authors propose a few-shot learning method for automated image quality assessment of prostate MRI scans, addressing the issue of limited annotated data.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the quality assessment of prostate MRI scans, specifically diffusion-weighted imaging (DWI), which is often compromised by geometric distortion caused by rectal air, and to address the class imbalance and scarcity of annotated clinical data.

Method: The authors propose a few-shot biparametric prototypical network that utilizes a dual-branch 3D ResNet to fuse T2-weighted and DWI features, and introduces feature-wise linear modulation (FiLM) and a gradient reversal layer (GRL) to handle real-world heterogeneity.

Result: The experimental results on two datasets show that the proposed method significantly outperforms few-shot learning baselines for image quality assessment, offering a practically feasible and data-efficient solution for standardizing prostate MRI quality control in clinical workflows.

Conclusion: The proposed method can effectively adapt to predicting complex clinical quality scores using only a few representative samples, and provides a solution for standardizing prostate MRI quality control in clinical workflows.
...
TL;DR: A new method for improving image quality in optical coherence tomography (OCT) for ophthalmology, particularly for low-cost devices, by generating high-quality surrogate images from noisy inputs.


<details>
  <summary>Details</summary>
Motivation: Inconsistent image quality in OCT, especially in low-cost devices, hinders automated analysis in ophthalmology.

Method: A flow-matching-based test-time adaptation method that generates high-quality surrogate images from noisy inputs by matching the test image's histogram to synthetic reference trajectories.

Result: State-of-the-art performance in segmenting critical biomarkers for two stages of Age-related Macular Degeneration (AMD).

Conclusion: The proposed method effectively addresses the issue of inconsistent image quality in OCT, enabling improved automated analysis in ophthalmology.
...
TL;DR: The study investigates the performance gap between handwritten text recognition (HTR) systems on Arabic-script and Latin-script datasets, attributing the gap to factors like annotation quality, visual variability, and character frequency distributions.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this study is to understand why HTR systems perform worse on Arabic-script datasets compared to Latin-script datasets, which is not well understood due to a lack of controlled comparisons.

Method: The study uses a unified CRNN model for line-level HTR across nine datasets, including Arabic, Persian, Urdu, English, and German scripts, with varying training sizes.

Result: The results show a performance gap of 5-7 CER points between Arabic and Latin scripts, which decreases with more data but remains even at full scale. Annotation quality and visual variability are identified as key factors contributing to the gap.

Conclusion: The study concludes that the performance gap between HTR systems on Arabic-script and Latin-script datasets is due to several factors, including annotation quality, visual variability, and character frequency distributions, and that addressing these factors can help narrow the gap.
...
TL;DR: Proposed LARE framework improves image retrieval in crowded scenes by encoding low-attention regions alongside full images.


<details>
  <summary>Details</summary>
Motivation: Conventional visual encoders neglect low-attention regions crucial for fine-grained retrieval in crowded scenes.

Method: LARE uses a dual-encoding strategy to encode low-attention regions and full images in parallel.

Result: LARE improves retrieval performance by preserving subtle visual cues in densely crowded scenes.

Conclusion: The proposed LARE framework enhances image retrieval performance in challenging crowded scenes.
...
TL;DR: The authors propose DINO-Med3D, a two-stage framework that adapts the pre-trained DINOv3 encoder for 3D medical segmentation tasks, achieving state-of-the-art results on five public datasets.


<details>
  <summary>Details</summary>
Motivation: The direct application of DINOv3 to volumetric medical segmentation is hindered by dimension and domain disparities between natural imagery and medical data.

Method: The authors propose a two-stage progressive framework: (1) adapting the DINOv3 encoder to 3D medical data using a multi-slice embedding module and a segmentation proxy task; (2) enhancing volumetric understanding with lightweight 3D adapters and a parallel detail recovery stream.

Result: The proposed approach significantly outperforms state-of-the-art baselines on five public datasets.

Conclusion: The authors successfully adapt DINOv3 to the medical domain, demonstrating the effectiveness of their approach for 3D medical segmentation tasks.
...
TL;DR: An automated approach to distinguish between ply instances in carbon-fiber reinforced polymer micrographs using semantic segmentation masks and shortest-path algorithms.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between semantic segmentation and ply instance segmentation, enabling comprehensive quantitative ply analysis and revealing manufacturing-induced inhomogeneities.

Method: Interpreting segmentation masks as graphs and applying shortest-path algorithms to find ply-separating paths.

Result: Successful application on high-resolution micrographs with varied characteristics, enabling analysis of microstructural properties like fiber volume fraction and ply thickness.

Conclusion: The approach allows for comprehensive ply analysis, revealing manufacturing-induced inhomogeneities and linking mechanical properties to microstructural imperfections.
...
TL;DR: The paper introduces a dataset and approach for improving product-centric image editing with text fidelity constraints, achieving better results in preserving product features and text rendering.


<details>
  <summary>Details</summary>
Motivation: Current models struggle to maintain fine-grained object identity in product-centric scenarios, and there is a lack of datasets for instruction-based product image editing with text fidelity constraints.

Method: The authors propose a supervised fine-tuning dataset, a reinforcement learning dataset, and a benchmark dataset, along with a Cyclic Consistency reward for RL training. They fine-tune Qwen-Image-Edit-2511 and Flux.1-Kontext-dev models using their dataset.

Result: The fine-tuned models demonstrate consistent improvements over baseline models in OCR and Perceptual metrics, and MLLM-based evaluations, with a 5x reduction in character error rate for the Qwen-Image-Edit-2511 model.

Conclusion: The proposed approach and dataset improve product-centric image editing with text fidelity constraints, enabling better preservation of product features, branding, and textual elements.
...
TL;DR: The paper proposes BindEdit, a method for real image editing that addresses issues with existing methods in complex multi-object scenarios by preventing attention leakage.


<details>
  <summary>Details</summary>
Motivation: Existing image editing methods often fail in complex multi-object scenarios due to attention leakage, causing issues like semantic blending, object duplication, or incomplete edits.

Method: BindEdit, which enforces attention-level constraints within a single diffusion trajectory, jointly regularizing cross- and self-attention, and incorporating a cross-attention re-balancing mechanism and region fidelity term.

Result: Extensive experiments demonstrate that BindEdit consistently outperforms existing methods within a single diffusion trajectory, maintaining robust performance across both single- and multi-object editing scenarios.

Conclusion: The proposed BindEdit method effectively resolves attention leakage issues in image editing, leading to improved performance in complex multi-object scenarios.
...
TL;DR: The paper audits and improves the Physics-IQ benchmark for evaluating video generative models' physical understanding, proposing solutions to address shortcomings and introducing a refined benchmark, Physics-IQ Verified.


<details>
  <summary>Details</summary>
Motivation: To advance video generative models (VGMs) for tasks like world modeling, a good video model must understand physical reality, and evaluating this understanding is an emerging field.

Method: The authors systematically audit the Physics-IQ benchmark, expose shortcomings, and propose three solutions: improving prompt and ground-truth quality, and introducing a sample-level scoring system.

Result: The refined benchmark, Physics-IQ Verified, refines 57.6% of all samples, improves over 34.8% of prompts, and shows moderate but meaningful ranking changes (Kendall's τ = 0.46) in a comparison study using six image-to-video generative models.

Conclusion: The authors hope Physics-IQ Verified advances the community by providing a more reliable signal toward physically accurate VGMs.
...
TL;DR: A new benchmark dataset, SP-TransientBench (STB), is introduced for single-photon LiDAR perception, providing real-world data for 3D vision tasks.


<details>
  <summary>Details</summary>
Motivation: The lack of real-world data for single-photon LiDAR perception, which hinders the evaluation of depth estimation, multi-view reconstruction, and 3D semantic understanding.

Method: Introduction of SP-TransientBench (STB), a benchmark dataset captured using a solid-state single-photon LiDAR, comprising 10 diverse scenes and 10,297 views.

Result: The dataset provides full time-of-flight histograms, standardized metadata, and calibrated camera poses, enabling benchmarking of real-world single-photon LiDAR perception.

Conclusion: The SP-TransientBench (STB) dataset facilitates consistent and reproducible evaluation of single-photon LiDAR perception across multiple 3D vision tasks.
...
TL;DR: OneCanvas proposes a novel approach to 3D scene understanding in Vision-Language Models (VLMs) by aggregating patch features from all views onto a single equirectangular panoramic canvas, achieving state-of-the-art accuracy with less training compute.


<details>
  <summary>Details</summary>
Motivation: Existing approaches to 3D scene understanding in VLMs rely on complex geometry encoders or large training budgets, which can be limiting. The authors aim to simplify the process and reduce training compute.

Method: OneCanvas aggregates patch features from all views onto a single equirectangular panoramic canvas by unprojecting each patch to a 3D world coordinate using its depth and camera pose, then adding a 3D position embedding to its feature.

Result: OneCanvas achieves state-of-the-art accuracy on SQA3D and VSI-Bench, and generalizes to out-of-distribution data on SPBench, using an order of magnitude less training compute than the strongest competing methods.

Conclusion: The proposed OneCanvas approach enables efficient and effective 3D scene understanding in VLMs, supporting situated reasoning from a specific viewpoint and allowing for a spatial pretraining curriculum.
...
TL;DR: The paper proposes a framework to train generalist Vision-Language-Action (VLA) models using unlabeled human videos, achieving competitive performance with state-of-the-art models trained on annotated datasets.


<details>
  <summary>Details</summary>
Motivation: Training VLA models requires large datasets with high-fidelity action annotations, which are difficult to obtain. The authors aim to leverage abundant unlabeled human manipulation videos to train VLA models.

Method: The authors propose a latent-action-based framework featuring a Hybrid Disentangled VQ-VAE, which decouples motion dynamics from environmental backgrounds. They also introduce an intent-perception decoupling strategy for adaptation to specific embodiments.

Result: The method, pre-trained exclusively on unlabeled human videos, performs competitively with state-of-the-art VLA models trained on massive annotated datasets, requiring only 50 trajectories for downstream adaptation.

Conclusion: The proposed framework enables effective training of VLA models using unlabeled human videos, achieving competitive performance with state-of-the-art models while reducing the need for annotated datasets.
...
TL;DR: A new benchmark for detecting AI-generated text-rich images is introduced, revealing challenges in detecting such images across various domains.


<details>
  <summary>Details</summary>
Motivation: The increasing capability of multimodal image generation models to synthesize realistic textual content and structured visual designs has made detecting AI-generated text-rich images a crucial challenge for digital trust and content authenticity.

Method: A multi-domain benchmark containing 8,602 images across six categories is introduced. Five AI-generated image detectors are evaluated in a zero-shot setting, and their performance is analyzed.

Result: Detector performance is highly domain-dependent, and even the strongest conventional detector exhibits severe sensitivity to JPEG compression. A multimodal vision-language model shows promise but also limitations on structured formats.

Conclusion: The findings highlight the need for text- and layout-aware detection methods for modern AI-generated images.
...
TL;DR: The paper proposes Mem-World, a memory-augmented multi-view action-conditioned world model, which generates persistent and informative video rollouts for robot learning in complex manipulation scenarios.


<details>
  <summary>Details</summary>
Motivation: The challenge of persistent world modeling in manipulation tasks due to frequent end-effector occlusions and rapid wrist-camera motion, which causes models to forget or hallucinate scene details.

Method: The authors propose W-VMem, a 4D wrist-view-centered surfel-indexed memory that anchors historical observations to temporally evolving surface elements, and a surfel-based rendering and scoring method for selecting relevant history frames.

Result: Mem-World generates persistent rollouts in complex manipulation scenarios, improves policy evaluation reliability by 14.5% in terms of Pearson correlation with real-world performance, and increases success rates from 58% to 72% on long-horizon tasks through synthetic data generation.

Conclusion: The proposed Mem-World model effectively addresses the challenge of persistent world modeling in manipulation tasks, enabling more reliable policy evaluation and improvement through synthetic data generation.
...
TL;DR: The paper proposes Visual On-Policy Self-Distillation (Visual-OPSD) to improve multimodal models by transferring reasoning from a teacher model that sees privileged visual thoughts to a text-only student model, achieving a 3.40pp improvement over the teacher and a 63.83pp improvement over same-scale VLMs.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the limited direct benefit of generated visual thoughts in unified multimodal models, despite their high inference cost, and to transfer useful reasoning from the generation pathway to a text-only model.

Method: The authors propose Visual-OPSD, which uses token-level JSD distillation on on-policy student trajectories to transfer the teacher's reasoning to a text-only student, with the teacher seeing privileged visual thoughts and the student seeing only the question.

Result: Visual-OPSD improves over its generative teacher by +3.40pp with a 14.3x speedup (10.0s vs. 142.8s per sample) and outperforms same-scale VLMs by +63.83pp on VSP.

Conclusion: The proposed Visual-OPSD method effectively transfers useful reasoning from the generation pathway to a text-only model, achieving significant improvements in performance and efficiency.
...
TL;DR: The authors propose CLARA, a clarification framework for composed image retrieval that resolves ambiguity by showing users a panel of visual alternatives, allowing them to select the closest match.


<details>
  <summary>Details</summary>
Motivation: Composed image retrieval queries often describe multiple possible images, making user intent ambiguous. Existing methods have limitations, including coverage guarantees only holding at the first interaction and text questions being insufficient for resolving fine-grained visual differences.

Method: CLARA uses a panel of visual alternatives to clarify user intent, allowing users to select the closest match. It reweights calibration using the likelihood ratio induced by the user's selection and constrains displayed prototypes to represent the current candidate set.

Result: Experiments show that CLARA matches state-of-the-art retrieval performance, maintains nominal coverage across interaction rounds, and finds the intended target in fewer rounds than text-question baselines, especially when ambiguity involves viewpoint or fine-grained attributes.

Conclusion: CLARA provides a more effective clarification framework for composed image retrieval, leveraging visual alternatives to resolve ambiguity and improve retrieval performance.
...
TL;DR: FlowObject is a framework for 3D reconstruction from few casual image captures. It combines flow-matching models with 3D Gaussian Splatting refinement to achieve both geometric completeness and observational consistency.


<details>
  <summary>Details</summary>
Motivation: Current 3D reconstruction methods suffer from 'synthetic bias' and fail to reason about unobserved geometry. The authors aim to develop a method that balances geometric completeness and observational consistency.

Method: FlowObject uses a dual-space guidance strategy to steer the ODE trajectory of a flow-matching model, enabling completion of unseen regions while enforcing consistency with real-world observations. It also integrates a 3D Gaussian Splatting refinement stage.

Result: FlowObject outperforms state-of-the-art generative models and optimization-based frameworks in both geometric completeness and view-dependent appearance fidelity, especially under severe occlusions.

Conclusion: FlowObject presents a novel approach to 3D reconstruction from few casual image captures, achieving a balance between geometric completeness and observational consistency. Its performance is validated through comprehensive benchmarks on synthetic and real-world datasets.
...
TL;DR: The paper proposes a novel nonconvex surrogate, TNPK, for low-rank tensor completion (LRTC) and develops an efficient ADMM algorithm, showing superior performance over state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: The motivation is to accurately approximate the tensor tubal rank in LRTC by proposing a novel nonconvex surrogate, TNPK.

Method: The method involves constructing a LRTC model using TNPK and developing an efficient ADMM algorithm with guaranteed subsequential convergence.

Result: The result shows that low-rank tensors are local minimizers of the proposed model under the tensor null space property (NSP), and the proposed method outperforms state-of-the-art competitors.

Conclusion: The paper concludes that the proposed TNPK surrogate and ADMM algorithm provide a superior approach for LRTC, validated through extensive experiments.
...
TL;DR: The paper introduces FG-BMK, a comprehensive benchmark for evaluating fine-grained image tasks in Large Vision-Language Models (LVLMs). It assesses LVLMs' capabilities in semantic recognition and visual discriminability, revealing their limitations and providing guidance for future improvements.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the insufficient understanding of LVLMs' capabilities on fine-grained image tasks, which are fundamental to computer vision.

Method: The authors introduce FG-BMK, a benchmark containing 1.01 million questions and 0.28 million images. They evaluate LVLMs through human-oriented and machine-oriented paradigms, enabling diagnostic analysis of LVLM failures.

Result: Current LVLMs remain inadequate fine-grained recognizers, with failures arising from intertwined bottlenecks in visual representations, semantic grounding, modality alignment, and category-level knowledge.

Conclusion: The findings provide diagnostic insights into the limitations of current LVLMs and offer guidance for future data construction and model design in developing more reliable LVLMs for fine-grained visual tasks.
...
TL;DR: Introduced DREAM, a novel multimodal framework for video retrieval that enhances visual and textual encoding, achieving state-of-the-art results on benchmark datasets.


<details>
  <summary>Details</summary>
Motivation: The exponential growth of video content has made retrieving semantically relevant videos via natural language queries critical, but existing systems have limitations in capturing complex semantics and temporal dynamics.

Method: DREAM uses a hybrid language modeling strategy and a hierarchical vision encoder with cascaded group attention to improve cross-modal alignment and capture fine-grained temporal dependencies.

Result: DREAM achieved new state-of-the-art R1 scores of 49.4%, 49.7%, and 27.3% on MSRVTT, MSVD, and LSMDC benchmark datasets, respectively.

Conclusion: The proposed DREAM framework demonstrates the effectiveness of hierarchical attention and dual-objective textual modeling in enabling robust, context-aware video retrieval, paving the way for future research in advancing cross-modal representation learning.
...
TL;DR: This paper presents a new dataset and evaluates CRNN-based models for Urdu handwritten text recognition, achieving robust performance with a CNN-BGRU-CTC model.


<details>
  <summary>Details</summary>
Motivation: The motivation is to advance research in Urdu Handwritten Text Recognition (UHTR) due to its limited research, unique script challenges, and scarcity of benchmark datasets.

Method: The study presents a specialized real dataset called the Urdu Katib Handwritten Dataset (UKHD) and evaluates the effectiveness of different CRNN-based hybrid models for Urdu Katib Handwriting Recognition (UKHR).

Result: The CNN-BGRU-CTC model showed more robust performance, with low Character Error Rate (CER) and Word Error Rate (WER).

Conclusion: This research aims to support and encourage the research community in developing a robust recognition system for preserving Urdu handwritten literature.
...
TL;DR: The paper introduces HOI-Edit, a benchmark for evaluating human-object interaction (HOI) editing, and proposes SCPE, a self-correcting framework for dynamic HOI editing.


<details>
  <summary>Details</summary>
Motivation: Current image editing methods struggle with complex human-object interactions (HOI), and existing benchmarks conflate HOI with static attributes, lacking reliable evaluation metrics.

Method: The authors propose SCPE, a self-correcting framework that uses iteratively refined prompts to constrain I2V models for dynamic HOI editing.

Result: SCPE achieves performance competitive with state-of-the-art editing models on HOI-Edit, with the added benefit of diagnosability and self-correction.

Conclusion: The paper highlights the importance of addressing complex HOI in image editing and presents HOI-Edit and SCPE as steps towards improving dynamic HOI editing.
...
TL;DR: A new benchmark for European Portuguese OCR is introduced, highlighting the language's underrepresentation in existing benchmarks and the need for modern, culturally relevant resources.


<details>
  <summary>Details</summary>
Motivation: European Portuguese is underrepresented in OCR benchmarks, which are skewed towards high-resource languages, and existing benchmarks focus on historical artifacts rather than modern applications.

Method: A new benchmark, PorTEXTO, is created using an annotation pipeline that combines LVLM transcriptions with exhaustive review by native speakers.

Result: Most OCR models show a significant performance drop when moving from synthetic to real-world samples, and specialized multilingual data is found to be a better driver for pt-PT performance than model size or resolution.

Conclusion: The release of open pt-PT OCR resources is motivated, and the importance of culturally relevant and modern benchmarks for European Portuguese is highlighted.
...
TL;DR: The paper proposes DVANet, a deep unfolding network for unified image restoration under complex degradations, leveraging half-quadratic splitting optimization and incorporating degradation-aware and visual-prior-guided branches.


<details>
  <summary>Details</summary>
Motivation: Existing image restoration methods struggle with unified restoration under complex degradations and locally damaged content, lacking explicit optimization interpretation or relying on fixed degradation assumptions.

Method: DVANet uses a deep unfolding approach inspired by half-quadratic splitting optimization, with two branches: degradation-aware observation consistency and visual-prior-guided reconstruction.

Result: DVANet achieves superior or competitive performance on multi-scenario degradation and cross-domain image restoration tasks, demonstrating favorable degradation adaptability and generalization ability.

Conclusion: The proposed DVANet effectively addresses the limitations of existing image restoration methods, providing a unified and adaptable framework for handling diverse degradation types.
...
TL;DR: OmniAgent, a native omni-modal agent, is proposed for efficient long video understanding, achieving state-of-the-art performance by selectively processing audio-visual cues.


<details>
  <summary>Details</summary>
Motivation: Current passive models for long video understanding are computationally expensive and inefficient, while interactive frameworks still rely on global pre-scanning, leading to the need for a more efficient approach.

Method: OmniAgent uses a POMDP-based iterative Observation-Thought-Action cycle, with on-demand actions to selectively distill audio-visual cues into textual memory, and is fine-tuned using Agentic Supervised Fine-Tuning and Agentic Reinforcement Learning with TAURA.

Result: OmniAgent achieves state-of-the-art performance across ten benchmarks, outperforming larger models, such as Qwen2.5-VL-72B, with a 7B agent achieving 50.5% vs. 47.3% on LVBench.

Conclusion: OmniAgent effectively decouples reasoning complexity from raw video duration, exhibiting positive test-time scaling, and validates the efficacy of active perception in video understanding.
...
TL;DR: Introduced AMALIA-VL, the first open-source LVLM for European Portuguese, addressing the language's underrepresentation in existing multimodal models.


<details>
  <summary>Details</summary>
Motivation: European Portuguese is underserved by existing open-source multimodal models, which often conflate it with Brazilian Portuguese or severely underrepresent it.

Method: A three-stage training process: vision-language alignment, general visual instruction tuning, and preference optimization, using a pt-PT-centric multimodal data mix.

Result: AMALIA-VL establishes a strong baseline for open-source pt-PT LVLMs, with released model weights, training data, and construction pipelines.

Conclusion: The development of AMALIA-VL helps democratize pt-PT LVLM development by providing necessary resources, including model weights and evaluation benchmarks.
...
TL;DR: A new framework, Hand-4DGS, is proposed for dynamic 3D hand reconstruction from egocentric videos, enabling fast and accurate hand tracking for AR/VR applications.


<details>
  <summary>Details</summary>
Motivation: The need for dynamic 3D hand reconstruction from egocentric videos in next-generation computing platforms like AR/VR and AI glasses, which is challenging due to fast head motion, rapid hand dynamics, occlusions, and ambiguity from single-view observations.

Method: Hand-4DGS, a feed-forward framework that uses mesh-guided representation and temporal convolutions to model dynamic motion, and benefits from 2D image supervision through Gaussian splatting.

Result: Hand-4DGS demonstrates significant improvements over baselines on two challenging egocentric datasets, H2O and ARCTIC, with fast inference (~60 FPS) and strong generalization capability.

Conclusion: The proposed Hand-4DGS framework offers a promising solution for dynamic 3D hand reconstruction from egocentric videos, which can benefit various applications in AR/VR and beyond.
...
TL;DR: The paper introduces a new metric, Cross-dataset AUC (Cross-AUC), to evaluate the generalization capabilities of deepfake detectors under domain shifts, addressing limitations in current evaluation methods.


<details>
  <summary>Details</summary>
Motivation: Current deepfake detection methods are evaluated using Area Under the ROC Curve (AUC) measured separately across multiple datasets, which fails to reflect real-world scenarios with mixed data sources and varying artifact types.

Method: The authors propose a novel metric, Cross-AUC, which averages per-domain AUCs with a measure of prediction polarization, quantified by the Wasserstein Distance between class score distributions.

Result: Experiments on seven benchmark datasets demonstrate the practical relevance of Cross-AUC in assessing the generalization capabilities of deepfake detectors under domain shifts.

Conclusion: Cross-AUC provides a more realistic evaluation of deepfake detectors, is interpretable, and explains the reason behind performance drops, addressing the limitations of current evaluation methods.
...
TL;DR: Moebius, a lightweight inpainting framework, rivals the 10B-level industrial generalist FLUX.1-Fill-Dev in generation quality while using less than 2% of the parameters and achieving a >15× acceleration in inference time.


<details>
  <summary>Details</summary>
Motivation: The prohibitive computational costs of large industrial foundation models (e.g., 10B-level) hinder their practical deployment, and constructing a highly optimized task-specific specialist is a promising solution.

Method: The proposed Moebius framework reconstructs the diffusion backbone with the Local-$λ$ Mix Interaction ($LλMI$) block and pairs it with an adaptive multi-granularity distillation strategy.

Result: Moebius achieves high-fidelity inpainting results, rivalling or surpassing FLUX.1-Fill-Dev, using 0.22B parameters (vs. 11.9B) and delivering a >15× acceleration in total inference time.

Conclusion: Moebius sets a new efficiency standard for high-fidelity inpainting, demonstrating that a highly efficient lightweight framework can achieve comparable results to large industrial foundation models.
...
TL;DR: A new model, ROSA-TFormer, is proposed for accurate classification of Pinus sylvestris var. mongolica plantations using Sentinel-1/2 time-series data, achieving high performance with 99.67% overall accuracy.


<details>
  <summary>Details</summary>
Motivation: Accurate identification of Pinus sylvestris var. mongolica plantations is crucial for monitoring afforestation quality and ecological restoration in northern Shaanxi.

Method: The ROSA-TFormer model integrates separate SAR and optical embedding branches, a sensor-aware gate, and temporal attention pooling to capture multi-source seasonal features from Sentinel-1/2 time-series data.

Result: The model achieves strong classification performance with 99.67% overall accuracy, 99.56% macro F1, and 98.91% P. sylvestris F1 on the HalfMonth-dataBig dataset.

Conclusion: The results demonstrate the potential of ROSA-TFormer for point-level P. sylvestris plantation classification, but broader wall-to-wall validation is necessary.
...
TL;DR: A novel algorithm, GUMP-Net, combining improved geodesic active contour model with deep neural networks for accurate pelvic segmentation.


<details>
  <summary>Details</summary>
Motivation: Pelvic segmentation is crucial for precise diagnosis, treatment, surgical planning, and navigation in pelvic fractures.

Method: GUMP-Net, a model-data-driven algorithm with three network modules: object detection, edge detector, and iteration module for deep level set evolution.

Result: GUMP-Net shows more accurate, robust, and consistent segmentation performance compared to state-of-the-art methods, especially with small training data.

Conclusion: GUMP-Net provides an efficient segmentation method for complex fracture reduction and an interpretable geometric perspective for understanding deep learning segmentation.
...
TL;DR: The authors investigate the representational geometry and dynamics of Vision Transformers (ViTs) using a systematic framework called Transformer Geometry Observatory (TGO).


<details>
  <summary>Details</summary>
Motivation: The fundamental understanding of the dimensional and representational geometry of ViTs remains relatively underexplored despite their widespread adoption and success in computer vision applications.

Method: The authors propose TGO, a framework for analyzing the representational geometry and dynamics of ViTs. They focus on the spectral geometry of ViT representations using a ViT-Small/16 model trained on ImageNet-100 and analyze various metrics such as Effective Rank, Stable Rank, Participation Ratio, Spectral Entropy, Spectral Flatness, Spectral Anisotropy, covariance structure, eigenspectra, and singular value spectra throughout training.

Result: The results show a consistent increase in dimensional utilization, accompanied by decreasing anisotropy, increasing spectral entropy, increasing participation ratio, and progressively flatter eigenspectra. The authors observe a progressive redistribution of variance across representational dimensions, particularly pronounced in the final CLS token representation.

Conclusion: The study provides new insights into the representational geometry and dynamics of ViTs, revealing a phenomenon contrary to the common intuition that training should concentrate information into a small number of dominant directions.
...
TL;DR: CABLE is a framework that reduces communication overhead and latency in cloud-hosted large multimodal models for Vehicle-to-Everything systems by efficiently encoding and transmitting only regions of interest.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the severe communication overhead and high cloud-side prefill latency caused by transmitting full-resolution frames from edge to cloud in Vehicle-to-Everything systems.

Method: CABLE uses ego-motion compensation, residual-motion cues, and a corridor envelope to form a robust region of interest (ROI) and only uploads ROI-masked images to the cloud, forming a mask-to-ROI-to-LMM feedback loop.

Result: Experiments on five datasets show $73$--$87%$ ROI pixel-coverage reduction with $5$--$8	imes$ estimated LMM prefill speedup at a modest detection-quality trade-off relative to full-frame inference.

Conclusion: CABLE achieves significant communication savings while largely preserving perception in cloud-hosted large multimodal models for Vehicle-to-Everything systems.
...
TL;DR: This paper compares the efficiency of a Parameter Efficient Fine Tuning (PEFT) strategy called RS Adapter across three Vision Language Model (VLM) architectures for Visual Question Answering (VQA) in Remote Sensing (RS).


<details>
  <summary>Details</summary>
Motivation: The direct application of general domain Foundation Models to Remote Sensing Visual Question Answering (RSVQA) is hindered by domain shifts and the computational cost of full fine-tuning.

Method: The study uses a unified architectural surgery pipeline to inject lightweight bottleneck adapters into the attention and MLP layers of frozen backbones, enabling rapid adaptation with less than 5% of trainable parameters.

Result: The Hybrid FLAVA architecture offers a superior balance of multimodal reasoning and retrieval capabilities compared to its unimodal counterparts, achieving convergence on the RSVQA x dataset.

Conclusion: The findings establish a new baseline for resource-efficient VQA in disaster assessment and urban monitoring.
...
TL;DR: The study evaluates the use of voxel-level uncertainty estimation via Monte Carlo (MC) Dropout for identifying segmentation errors in clinically critical sub-regions of glioma segmentation in multiparametric MRI. The results show that MC dropout can reliably identify errors and preserve segmentation accuracy, but also highlight the importance of sub-region-specific calibration assessment for clinical safety.


<details>
  <summary>Details</summary>
Motivation: Glioma segmentation in multiparametric MRI is crucial for treatment planning, and a segmentation model that fails silently on treatment-critical sub-regions poses a patient safety risk. Current metrics like Dice scores cannot detect such failures.

Method: The study uses Monte Carlo (MC) Dropout for voxel-level uncertainty estimation and evaluates two models, SegResNet and UNet-Res, on 126 BraTS21 patients.

Result: MC dropout preserved segmentation accuracy and achieved strong uncertainty-error alignment. However, the study also found that global alignment can mask region-specific differences, and one model exhibited severely miscalibrated confidence on clinically critical sub-regions, highlighting the need for sub-region-specific calibration assessment.

Conclusion: The study concludes that strong uncertainty-error alignment is necessary but insufficient for clinical safety, and sub-region-specific calibration assessment must accompany AUROC evaluation when selecting models for clinical deployment.
...
TL;DR: A novel mesh-based neural rendering method enabling efficient and comprehensive editing functionalities, including geometry, texture, and semantic editing.


<details>
  <summary>Details</summary>
Motivation: Existing neural rendering methods have limited editing capabilities, such as rigid transformation and category-specific editing.

Method: A mesh-based representation encoding neural radiance fields with disentangled geometry, texture, and semantic codes on mesh vertices, along with techniques like local space parameterization and learnable modification color on vertices.

Result: The proposed method demonstrates superiority in representation quality and editing ability on real and synthetic datasets.

Conclusion: The proposed method empowers efficient and comprehensive editing functionalities for neural implicit rendering techniques.
...
TL;DR: The paper introduces RNG-Bench, a benchmark suite to evaluate the ability of multimodal foundation models to reconstruct past observations and act on them during multi-step interactions.


<details>
  <summary>Details</summary>
Motivation: The need to condition actions on observations that are no longer visible in closed-loop policies, and the lack of existing benchmarks that isolate the ability to reconstruct past observations.

Method: The authors introduce two games, Matching Pairs and 3D Maze, and evaluate them under a unified harness with three controlled difficulty axes. They also propose a head-to-head duel protocol and a Memory Gap metric.

Result: The hardest configurations of RNG-Bench require contexts of roughly 128K tokens and 350 image inputs per episode, and remain far from saturated by frontier MLLMs. Memory Gap analysis shows that most residual errors stem from forgetting earlier observations rather than from suboptimal decision making.

Conclusion: Fine-tuning Qwen3.5-9B on optimal-policy rollouts and filtered model demonstrations improves performance on RNG-Bench and transfers to existing benchmarks without degrading general multimodal capability.
...
TL;DR: This paper introduces Continuous Audio Thinking (CoAT), a framework that enhances audio language models by adding a continuous latent workspace to preserve acoustic information, improving performance across various audio tasks.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitation of large audio language models (LALMs) in preserving acoustic information, such as phonetic detail and prosody, which is lost during text generation.

Method: The method proposed is the Continuous Audio Thinking (CoAT) framework, which equips LALMs with a continuous latent workspace for organizing acoustic information prior to response generation, using distillation from audio experts.

Result: The results show performance gains across three LALMs on a broad benchmark suite spanning audio reasoning, audio understanding, music classification, speech emotion, and speech transcription.

Conclusion: The conclusion is that CoAT is effective in improving LALMs by preserving acoustic information, and the auxiliary supervision from the thinking positions propagates to the model's textual responses.
...
TL;DR: A new framework for de-identifying educational dialogue transcripts locally without sending data to third parties has been proposed, achieving high accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing methods for de-identifying educational dialogue transcripts force a tradeoff between data governance and accuracy, and current solutions have limitations.

Method: A fully local cascade framework with a recall-first union proposer and a context-aware reviewer for de-identification.

Result: The proposed framework achieves 0.958 macro F1 on math tutoring transcripts, outperforming same-family LLM-only baselines and a commercial API.

Conclusion: The proposed framework effectively de-identifies educational dialogue transcripts locally, suggesting that problem formulation is more important than model scale for this task.
...
TL;DR: SproutRAG is a new retrieval-augmented generation framework that improves information efficiency by organizing text into hierarchical chunks and using attention to capture semantic structure.


<details>
  <summary>Details</summary>
Motivation: Existing retrieval-augmented generation systems struggle to balance retrieval granularity with contextual coherence, and previous methods have limitations such as costly LLM calls, single-level context expansion, or information loss through summarization.

Method: SproutRAG uses attention-guided hierarchical chunking, learned inter-sentence attention, and hierarchical beam search to retrieve candidates at multiple granularities.

Result: Experiments across four benchmarks show that SproutRAG improves information efficiency by 6.1% on average over the strongest baseline.

Conclusion: SproutRAG presents a novel approach to retrieval-augmented generation that addresses the trade-off between retrieval granularity and contextual coherence, achieving improved information efficiency.
...
TL;DR: This paper explores activation steering as an alternative to few-shot prompting for low-resource synthetic data generation in large language models, showing improved diversity and downstream model performance.


<details>
  <summary>Details</summary>
Motivation: Current methods for synthetic data generation in low-resource languages rely on few-shot prompting, which can be costly and limit diversity. The authors seek to investigate activation steering as a more effective approach.

Method: The authors study two steering strategies, Language Steering and Quality Steering, across four open-source LLMs, multiple layers, and 11 languages. They evaluate these methods in zero-shot and few-shot prompting settings.

Result: The results show that steering on early layers improves the diversity of generated data and often yields stronger downstream model performance, particularly for low-resource languages.

Conclusion: Activation steering is a viable alternative to few-shot prompting for low-resource synthetic data generation, offering improved diversity and model performance.
...
TL;DR: JetFlow, a new speculative decoding framework, improves the efficiency of autoregressive Large Language Models by combining drafting efficiency with causal conditioning, achieving significant speedup over existing methods.


<details>
  <summary>Details</summary>
Motivation: The authors aim to overcome the scaling limitation of speculative decoding (SD) methods, which face a causality-efficiency dilemma, and improve the speedup of autoregressive Large Language Models.

Method: JetFlow uses a head-based SD framework that trains a causal parallel draft head over fused hidden states from the frozen target model, producing candidate trees whose scores align with the target model's autoregressive factorization.

Result: JetFlow consistently outperforms bidirectional-head and tree-based SD baselines across various benchmarks, achieving up to 9.64x speedup on MATH-500 and 4.58x on open-ended conversational workloads.

Conclusion: The proposed JetFlow framework effectively converts larger draft budgets into longer accepted prefixes and higher end-to-end speedup, demonstrating significant performance gains over existing SD methods.
...
TL;DR: The paper proposes CoreMem, a resource-efficient edge-cloud memory architecture for personalized dialogue agents, which uses information geometry to improve memory and compute efficiency.


<details>
  <summary>Details</summary>
Motivation: The need for continuous long-term memory in personalized dialogue agents, while facing severe memory and compute bottlenecks on consumer-grade hardware.

Method: The paper proposes two main components: Riemannian retrieval using Fisher-Rao metric with Woodbury acceleration, and Fisher-guided discrete token distillation (FDTD) for context compression.

Result: CoreMem achieves strong accuracy improvements on LOCOMO and LongMemEval-S benchmarks, with gains in Open-domain (+4.51 pp) and Temporal (+4.17 pp) reasoning, while operating within an 8 GB VRAM budget.

Conclusion: The proposed CoreMem architecture effectively bridges the gap between resource-constrained edge devices and the demand for theoretically grounded, lifelong memory agents.
...
TL;DR: The paper proposes VISUALSKILL, a multimodal skill approach for computer-use agents (CUAs) that incorporates visual information to improve performance on long-horizon tasks and unseen software.


<details>
  <summary>Details</summary>
Motivation: Existing CUAs struggle with long-horizon tasks and unseen software, and current skill libraries represent skills as text only, neglecting the visual nature of GUI interaction.

Method: The authors propose a two-stage pipeline to construct VISUALSKILL, combining authored documentation with live-application UI exploration, and evaluate it using a Claude Code CLI agent.

Result: VISUALSKILL yields an average score of 0.456 on two CUA benchmarks, outperforming a no-skill baseline (0.303) and a text-only skill approach (0.373).

Conclusion: The paper concludes that incorporating visual information into skill artifacts helps CUAs identify UI elements and verify workflow state, leading to improved performance.
...
TL;DR: The paper analyzes mathematical reasoning in large language models across languages, finding partial overlap in model parameters with language-dependent differences.


<details>
  <summary>Details</summary>
Motivation: To understand whether cross-lingual variation in mathematical reasoning performance in LLMs is due to language-specific parameters or a shared mechanism.

Method: Cross-lingual mechanistic analysis of mathematical reasoning in LLMs, comparing model parameters across languages.

Result: The extracted math-associated parameters show partial cross-lingual overlap, with English having the largest set of math-relevant parameters and lower-resource languages having smaller sets.

Conclusion: Math-related behavior in multilingual LLMs exhibits partial cross-lingual parameter overlap with systematic language-dependent differences, being neither fully language-invariant nor fully language-specific.
...
TL;DR: The Montreal Forced Aligner (MFA) 3.0 has achieved state-of-the-art performance in forced alignment across multiple languages, with significant developments and improvements since its initial release.


<details>
  <summary>Details</summary>
Motivation: To document the developments of MFA 3.0 since its initial release and evaluate its performance across multiple languages.

Method: The paper evaluates MFA 3.0's performance across English, Japanese, and Korean, benchmarked against classic and neural forced aligners, using larger open-source datasets, harmonized IPA dictionaries, model adaptation, and cross-language phone remapping.

Result: MFA 3.0 achieves state-of-the-art or near state-of-the-art performance across all four benchmark datasets with mean boundary errors below 15 ms.

Conclusion: MFA 3.0 has made significant improvements and achieved high performance in forced alignment across multiple languages, making it a reliable tool for research and industry applications.
...
TL;DR: LLMs struggle to preserve diagnostic uncertainty in clinical text, often failing to retain original uncertainty cues and nuanced distinctions between adjacent levels.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to investigate whether Large Language Models (LLMs) can correctly preserve diagnostic uncertainty in clinical text, which is crucial for guiding decisions about follow-up testing and treatment.

Method: The authors constructed a benchmark of 1,200 clinical documents with 9,184 uncertainty annotations across five levels and evaluated three LLMs on this benchmark.

Result: The results show that LLMs preserve the original uncertainty cues poorly, often less than half the time, and struggle with nuanced distinctions between adjacent levels.
...
TL;DR: The paper studies machine unlearning for large language models from a data-centric perspective, finding a decay pattern in collateral damage and identifying predictive features for auditing unlearning effects.


<details>
  <summary>Details</summary>
Motivation: The boundary between knowledge to forget and knowledge to retain in large language models is often unclear, making it challenging to remove specified knowledge without affecting the rest of the model's capabilities.

Method: The authors study unlearning effects from a data-centric perspective, measuring how effects propagate from the forget set to same-domain and distant-domain knowledge, and analyze data features predictive of downstream damage.

Result: The authors find a consistent decay pattern in collateral damage and identify interaction features between the forget set and evaluation set as most predictive of downstream damage, suggesting the possibility of auditing unlearning effects before execution.

Conclusion: The findings position forget-set auditing as an early warning tool for identifying risky unlearning runs and designing more reliable unlearning procedures.
...
TL;DR: A unified framework for customizing and efficiently deploying multi-agent systems in real-world settings, enabling rapid domain adaptation and cost-efficient serving.


<details>
  <summary>Details</summary>
Motivation: The deployment of large language model (LLM)-based multi-agent systems in production environments is challenging due to domain-specific customization requirements and high latency and inference costs.

Method: The proposed framework consists of two stages: Agentic Model Customization (continual pretraining, supervised fine-tuning, and preference optimization) and Inference Optimization (speculative decoding and FP8 quantization with targeted calibration).

Result: The framework achieves a 4.48x speedup in throughput while maintaining performance and improving robustness on long-tail scenarios across enterprise workloads.

Conclusion: The proposed framework enables efficient deployment and customization of multi-agent systems, addressing key challenges in production deployment.
...
TL;DR: Introduced MCompassRAG, a metadata-guided retrieval framework that improves efficiency and evidence quality in retrieval-augmented generation systems.


<details>
  <summary>Details</summary>
Motivation: The trade-off between chunk size and search space in retrieval-augmented generation systems limits their performance in deep research tasks.

Method: MCompassRAG uses topic-level signals to select relevant evidence, enriching chunk representations with topic metadata and training a lightweight retriever through LLM-teacher distillation.

Result: MCompassRAG improves information efficiency by 8.24% on average with over 5 times lower latency than the strongest efficient RAG baselines across six complex retrieval benchmarks.

Conclusion: MCompassRAG offers a more efficient and precise retrieval framework for retrieval-augmented generation systems, particularly in deep research tasks.
...
TL;DR: The paper explores a speech-driven approach for discriminating Chinese dialects, combining MFCC features with a CNN-based model and an end-to-end speech recognition model.


<details>
  <summary>Details</summary>
Motivation: Traditional text-driven approaches for language discrimination have poor results, and the authors aim to improve this using speech-driven features for Chinese dialect discrimination.

Method: The authors use MFCC features with a CNN-based model, an end-to-end speech recognition model based on HMM-DNN, and attention to extract discriminative words. They then combine word-level embedding and MFCC-based features using a CNN.

Result: The proposed approach shows effectiveness and appropriateness in fine-grained Chinese dialect discrimination, outperforming state-of-the-art methods on two benchmark corpora.

Conclusion: The speech-driven approach offers a promising solution for Chinese dialect discrimination, leveraging the strengths of both speech and text features.
...
TL;DR: The paper proposes Distance-Adaptive Representation (DAR), a method that adjusts the dimensionality of token representations based on their distance from the prediction target, leading to efficient KV cache reduction without sacrificing performance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to optimize the representational capacity for tokens at different distances from the prediction target, as the influence of tokens on the next word is not uniform.

Method: The method proposed is Distance-Adaptive Representation (DAR), which assigns full-dimensional representations to tokens within a local context window and reduced-dimensional representations to tokens beyond that window.

Result: The results show that DAR closely matches the performance of full-dimensional baselines across multiple pretraining scales and fine-tuning scenarios, while uniformly reducing dimensionality across all token positions leads to worse performance.

Conclusion: The conclusion is that the common assumption of uniform key and value dimensionality across token positions can be challenged, and DAR offers a new direction for designing attention architectures that adaptively allocate representational capacity across sequences.
...
TL;DR: A novel framework using transfer learning and data augmentation for Chinese dialects discrimination, achieving state-of-the-art results on two benchmark corpora.


<details>
  <summary>Details</summary>
Motivation: The scarcity of annotated resources for Chinese dialects discrimination tasks.

Method: A transfer learning and data augmentation framework (CDDTLDA) that trains a source-side ASR model on a larger corpus, augments low-resource target-side data, and fine-tunes a target ASR model with self-attention mechanism.

Result: The proposed model significantly outperforms state-of-the-art methods on two benchmark Chinese dialects corpora.

Conclusion: The CDDTLDA framework effectively addresses the resource shortage issue in Chinese dialects discrimination, demonstrating superior performance over existing methods.
...
TL;DR: This paper proposes a novel algorithm, SCPO, for training reward models that can accurately represent diverse cultural preferences in large language models, reducing bias towards any subcommunity.


<details>
  <summary>Details</summary>
Motivation: The need for large language models to serve diverse cultural sub-communities without bias towards any particular region or community.

Method: A novel reward model training algorithm, SCPO, which incorporates diverse cultural preferences in a balanced manner.

Result: The SCPO method results in performance increases of up to 7 points over the baseline model across two datasets and 7 countries, and is up to 280% more training data-efficient.

Conclusion: The proposed SCPO algorithm can effectively mitigate excessive bias towards any subcommunity, allowing for more globally applicable and fair large language models.
...
TL;DR: The paper introduces PhysAssistBench, a benchmark for testing medical LLMs' ability to assist physicians by coordinating clinical knowledge, patient communication, and EHR system interaction.


<details>
  <summary>Details</summary>
Motivation: Current evaluations of medical LLMs focus on isolated capabilities, but physician assistance requires coordinating these capabilities in a single interaction.

Method: The authors introduce a scalable pipeline to construct agentic patients and create PhysAssistBench, a curated bilingual evaluation set of 1,296 manually reviewed and physician-validated turns.

Result: Experiments with leading LLMs show that current models remain unreliable in coordinating knowledge, communication, and systems, exposing a key bottleneck for clinical LLMs.

Conclusion: The development of medical LLMs that can reliably assist physicians requires coordination across knowledge, communication, and systems, rather than isolated gains in any one area.
...
TL;DR: The paper proposes a Bayesian In-Context Learning (BCL) framework for information extraction, which uses particle filtering with Bayesian updates to improve performance across model scales and tasks.


<details>
  <summary>Details</summary>
Motivation: Current in-context learning approaches for information extraction show inconsistent performance across model scales and lack systematic optimization and generalizability.

Method: The BCL framework uses particle filtering with Bayesian updates to refine label representations across IE tasks, generalizing to sequence labeling and relation classification paradigms.

Result: Extensive experiments demonstrate substantial and consistent improvements over existing approaches.

Conclusion: The proposed BCL framework provides a systematic optimization approach for in-context learning in information extraction, leading to improved performance and generalizability.
...
TL;DR: The authors introduce PragReST, a self-supervised framework to improve pragmatic reasoning in large language models (LLMs) without human-labeled data. PragReST constructs pragmatic QA data, generates counterfactual reasoning traces, and fine-tunes models through supervised learning and reinforcement learning.


<details>
  <summary>Details</summary>
Motivation: LLMs struggle with pragmatic inferences, often choosing literal interpretations instead of implied meanings, highlighting the need for improved pragmatic reasoning.

Method: PragReST, a self-supervised framework that constructs pragmatic QA data, generates counterfactual reasoning traces, and trains models through supervised fine-tuning and reinforcement learning.

Result: PragReST improves over backbone models, task-specific pragmatic tuning baselines, and non-counterfactual variants across four pragmatic benchmarks. Specifically, it improves accuracy by 5.37% and 5.50% for Qwen3-8B and Qwen3-14B, respectively.

Conclusion: The authors conclude that PragReST effectively improves LLM pragmatic reasoning without human-labeled data, and that counterfactual reasoning is crucial for this improvement, with preserved out-of-domain performance on general-knowledge and mathematical reasoning benchmarks.
...
TL;DR: The paper discusses the limitations of current home assistants in interpreting elliptical expressions in human dialogue, proposes a new dataset PEC-Home to address this issue, and evaluates the performance of various Large Language Models (LLMs) on this task.


<details>
  <summary>Details</summary>
Motivation: Current home assistants struggle to interpret elliptical expressions in human dialogue, which limits their effectiveness in real-world applications. This is due to referential ambiguity caused by different environmental expectations among multiple users and intention ambiguity resulting from user preferences that evolve over time or change with the environment.

Method: The authors introduce PEC-Home, a simulated home dataset designed for interpreting progressively elliptical commands in smart homes. They conduct extensive experiments on various LLMs, including GPT-4o, to evaluate their performance on this task.

Result: The results show that existing home assistants struggle to execute user-intended operations based solely on elliptical commands. Even when equipped with tools for storing and retrieving user dialogue history, execution accuracy remains below that achieved with complete commands.

Conclusion: The paper highlights the need for improving the interpretation of elliptical expressions in human dialogue to enhance the effectiveness of home assistants in real-world applications.
...
TL;DR: This paper studies the issue of 'misaligned alignment' in large language models (LLMs), where safety-oriented behaviors can misfire and lead to rejecting warranted conclusions. The authors introduce a benchmark (VETO) and a metric (MAR) to quantify this phenomenon and find that all LLMs tested exhibit non-trivial MARs.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to highlight the need for more advanced and principled approaches to alignment in LLMs, as current methods can overgeneralize surface-level safety cues and override objective evidence.

Method: The authors introduce a benchmark called VETO, consisting of 2,032 BBQ-derived contrastive pairs, and define a new metric called Misfired Alignment Rate (MAR). They benchmark 25 LLMs on VETO and conduct controlled priming experiments to study the phenomenon of misfired alignment.

Result: The results show that all LLMs tested, including the most recent ones, exhibit non-trivial MARs (4.7 to 18.9%), while human participants achieve 0.0% MAR. The authors also find that alignment-induced cues can substantially amplify MAR across LLMs.

Conclusion: The authors conclude that current alignment methods can overgeneralize surface-level safety cues, leading to misfired alignment, and motivate the need for more work on alignment objectives that better preserve contextual grounding.
...
TL;DR: RegMix-D, an extension of RegMix, dynamically selects data mixtures for Large Language Model pretraining, improving performance across 13 tasks while reducing proxy compute budget.


<details>
  <summary>Details</summary>
Motivation: Existing methods like RegMix select a single static data mixture, which may not be optimal throughout the training process. The authors aim to improve data mixture selection by utilizing full loss trajectories from proxy runs.

Method: The authors propose RegMix-D, which trains a regression model on full loss trajectories from proxy runs to predict optimal data mixtures at multiple training stages. RegMix-D supports offline and online deployment modes.

Result: RegMix-D consistently outperforms RegMix and DoReMi across 13 downstream tasks on the Pile dataset with a 1B parameter target model, while being proxy-efficient.

Conclusion: RegMix-D offers a simple yet effective approach to dynamic data mixture selection, improving Large Language Model pretraining performance while reducing computational costs.
...
TL;DR: The paper introduces TW-LegalBench, a benchmark for evaluating large language models (LLMs) on Taiwanese law. It assesses 13 LLMs on multiple-choice questions, open-ended essay questions, and legal judgment prediction tasks, finding that while top models approach human-level performance on qualification exams, they struggle with legal text generation and precise statute citation.


<details>
  <summary>Details</summary>
Motivation: The performance of large language models (LLMs) on jurisdiction-specific legal reasoning, particularly in Taiwanese law, remains underexplored. The authors aim to fill this gap by creating a benchmark that evaluates LLMs on Taiwanese legal texts and tasks.

Method: The authors present TW-LegalBench, which consists of three task types: multiple-choice questions (MCQs), open-ended essay questions (OEQs), and legal judgment prediction (LJP) instances. They evaluate 13 LLMs using accuracy metrics for MCQs, a decomposed LLM-as-Judge framework for OEQs, and metrics for sentencing accuracy and statute citation for LJP.

Result: The results show that top-performing LLMs exceed the passing threshold for qualified lawyers but fall short of that for judges and prosecutors. For LJP, models demonstrate reasonable verdict type accuracy and sentence prediction capability but struggle to cite exact legal articles.

Conclusion: The study highlights that reliable legal text generation remains challenging for LLMs, even though their performance on qualification examinations approaches human level. The findings suggest that LLMs need improvement in precise legal reasoning and text generation tasks.
...
TL;DR: Large language models (LLMs) struggle to accurately capture item discrimination, a key psychometric property in educational assessments, with direct prediction and response-based calibration methods yielding weak to moderate correlations with human-calibrated discrimination values.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to investigate whether large language models (LLMs) can capture item discrimination, a fundamental psychometric property of educational assessments, which measures whether an item distinguishes students with higher proficiency from those with lower proficiency.

Method: The study evaluates 42 proprietary and open-weight LLMs using two approaches: direct discrimination prediction, where models estimate an item's discrimination value from its content, and response-based Classical Test Theory (CTT) calibration, where LLM answers are treated as synthetic student responses to compute discrimination scores.

Result: The results show weak alignment between LLM-estimated and human-calibrated discrimination values, with the best-performing model achieving a Spearman correlation of 0.152 for direct prediction and 0.241 for response-based CTT calibration.

Conclusion: The study concludes that item discrimination remains an open challenge for LLM-based psychometric evaluation, as current LLMs contain non-random discrimination-relevant signal but do not reliably capture how assessment items distinguish human students.
...
TL;DR: Morpheus, a neural morpheme-boundary model for Turkish, serves as a lossless, morphology-aware tokenizer and word-embedding producer, outperforming subword tokenizers in terms of bits-per-character and morphological alignment.


<details>
  <summary>Details</summary>
Motivation: Turkish is an agglutinative language where meaning is carried by morphemes. Existing subword tokenizers fragment semantically loaded suffixes and fail to decode their output back to the original text.

Method: A differentiable Poisson-binomial dynamic program is used to turn per-character boundary probabilities into soft morpheme memberships during training and exact segments at inference.

Result: Morpheus achieves the lowest bits-per-character (1.425), doubles the gold morphological alignment (MorphScore macro-F1 0.61), and uses 19% less GPU memory than 64K-vocabulary subword tokenizers. As an embedder, Morpheus vectors lead on lexical retrieval and same-root verification, surpassing BGE-M3 and BERTurk.

Conclusion: Morpheus is a neural morpheme-boundary model that serves as a lossless, morphology-aware tokenizer and word-embedding producer, achieving state-of-the-art results in Turkish language processing tasks.
...
TL;DR: The paper introduces LegalWorld, a life-cycle interactive environment that models Chinese civil litigation, and LongJud-Bench to evaluate agent capability across all stages.


<details>
  <summary>Details</summary>
Motivation: Existing legal benchmarks evaluate isolated subtasks, and prior legal-agent simulators reinitialize each scenario from shared ground truth, leaving cross-stage causal dependencies unmodeled.

Method: The authors present LegalWorld, a causally connected state chain of five stages (seven sub-scenarios) grounded in 75,309 paired Chinese civil judgments, paired with reusable infrastructure.

Result: The results show that LegalWorld trajectories are procedurally faithful and role-consistent, and a capability-level cross-model evaluation reveals sharp divergences in agent capability across stages.

Conclusion: The paper concludes that LegalWorld and LongJud-Bench provide a more comprehensive evaluation of agent capability in Chinese civil litigation, highlighting the importance of modeling cross-stage causal dependencies.
...
TL;DR: The paper proposes a method called output vector editing to mitigate memorization in large language models by modifying the output vectors of specific neurons to introduce a distractor in vocabulary space, thereby redirecting their residual-stream contributions.


<details>
  <summary>Details</summary>
Motivation: Large language models memorize and reproduce sequences from their training data, posing privacy, copyright, and security risks. Existing mitigation methods have limitations, and the authors aim to improve upon these by focusing on output vector editing.

Method: The authors propose output vector editing, a constrained-optimization weight edit that locates a small set of MLP neurons responsible for a memorized continuation and minimally modifies their output vectors.

Result: The method achieves up to 87.9% suppression of memorized sequences on a 7B model and 96.5% coverage with an ensemble of edit modes. A mechanistic boundary is identified at ~14% of sequences unreachable by MLP-only editing, but ablating top contributing attention heads recovers 60-64% of these.

Conclusion: The proposed output vector editing method effectively mitigates memorization in large language models by modifying output vectors of specific neurons, offering a spectrum of edit modes that can be tailored for different needs.
...
TL;DR: The paper proposes DICE, a method to improve dense retrieval on long documents by splitting them into chunks, encoding independently, and aggregating back into a single vector.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the issue of document-side early compression in dense retrieval, where a short but decisive span in a long document is weakened during encoding.

Method: The method proposed is DICE (Document Inference via Chunk Evidence), which involves splitting documents into chunks, encoding them independently with a frozen model, and aggregating them back into a single vector.

Result: The results show that DICE improves retrieval across four backbones, with significant gains on long documents (>4k tokens), and yields lower Evidence Dilution Index (EDI) in 92.8% of cases.

Conclusion: The conclusion is that document-level encoding is a practical and underexplored lever for long-document retrieval, and DICE is an effective strategy to improve dense retrieval on long documents.
...
TL;DR: The paper introduces RedactionBench, a benchmark for evaluating the redaction of personally identifiable information (PII) in sensitive domains, and R-Score, a novel metric for assessing redaction performance. It highlights the challenges of contextual redaction and the subjective nature of privacy perceptions.


<details>
  <summary>Details</summary>
Motivation: The increasing application of Large Language Models to sensitive domains requires effective redaction of personally identifiable information (PII). Existing benchmarks are limited, and the authors aim to address this gap by introducing a new benchmark and metric.

Method: The authors introduce RedactionBench, a manually annotated benchmark of 200 diverse documents across 11 domains, and R-Score, a character-level metric that treats semantically similar redactions equally. They evaluate various models, including Named Entity Recognition models, Small Language Models, and frontier models, and conduct a human evaluation with over 80 users.

Result: The evaluations demonstrate that contextual redaction remains an unsolved problem, and the human evaluation reveals a stark dichotomy in privacy perceptions, with annotators showing consensus on mandatory redactions and safe text preservations but failing to agree on contextual redactions. The authors report the performance of 35 models across families in redacting PII.

Conclusion: The paper concludes that contextual redaction is a challenging task, and the subjective nature of privacy perceptions makes it essential to develop efficient model design and standardized evaluations. The authors release RedactionBench to establish a baseline for future privacy-preserving systems.
...
TL;DR: The authors study the evaluation of generated radiology reports, focusing on clinical accuracy and the boundary between significant errors and harmless variations. They identify biases in LLM evaluators and propose a new metric that surpasses larger LLMs.


<details>
  <summary>Details</summary>
Motivation: Reliable evaluation of radiology reports is crucial for patient care, but existing metrics are medically ungrounded and Large Language Models (LLMs) struggle to distinguish between significant errors and harmless variations.

Method: The authors use the ReEvalMed benchmark to evaluate LLM evaluators and train lightweight interpretable metrics on Qwen3-8B and MedGemma-4B. They compare one-pass and two-pass settings and assess discrimination and robustness.

Result: The authors identify a widespread discrimination bias in LLM evaluators, but their trained metric surpasses 32B-scale medical LLMs and remains competitive with proprietary models. One-pass trained metrics are suggested as a practical choice for cost-sensitive deployment.

Conclusion: The study highlights the importance of clinical accuracy in radiology report evaluation and proposes a new metric that improves the boundary between significant errors and harmless variations. The findings have implications for the deployment of LLM evaluators in clinical settings.
...
TL;DR: A data-centric approach improves long-context reasoning in large language models, achieving state-of-the-art results on benchmarks and agentic tasks.


<details>
  <summary>Details</summary>
Motivation: The authors aim to improve long-context reasoning in large language models, particularly for autonomous agents, by focusing on diverse training data rather than reward engineering.

Method: The authors propose a simple data recipe, paired with a minimal outcome-based GRPO setup, and construct eight datasets totaling ~14K examples across three task families: retrieval, multi-evidence synthesis, and reasoning.

Result: The approach yields average gains of +7.2/+3.2/+6.4 points across seven long-context benchmarks for three models (Qwen3-4B/8B/30B-A3B) and improves GAIA by +4.8 and BrowseComp by +7.0 points on agentic tasks.

Conclusion: The authors' data-centric approach effectively improves long-context reasoning in large language models, achieving state-of-the-art results and demonstrating transferability to agentic tasks.
...
TL;DR: A new framework called ScholarSum is proposed for generating fluent and faithful summaries of scientific literature.


<details>
  <summary>Details</summary>
Motivation: Existing summarization approaches struggle to balance linguistic fluency and factual faithfulness.

Method: ScholarSum uses a hierarchical reflective graph-based framework that emulates a student-teacher writing process.

Result: ScholarSum significantly outperforms previous baselines in terms of both completeness and faithfulness.

Conclusion: The proposed ScholarSum framework can generate high-quality summaries that are both fluent and faithful.
...
TL;DR: The authors propose a new framework called ImpSH for detecting implicit hate speech, which improves cross-domain performance by aligning posts with implied statements and using context-bounded semi-hard negatives.


<details>
  <summary>Details</summary>
Motivation: Classifying implicit hate speech remains a challenge because intent is often masked through insinuation and context rather than explicit slurs, and prior supervised contrastive approaches can overfit surface cues and struggle to transfer across datasets.

Method: The authors propose a triplet-based framework called ImpSH, which aligns posts with implied statements when available and uses context-bounded semi-hard negatives to focus learning on near confusions. They also examine AugSH, which forms positives via data augmentation.

Result: ImpSH is a viable alternative to standard supervised contrastive baselines and often improves cross-domain performance under matched preprocessing and tuning budgets. Representation analysis indicates tighter positive pairs with balanced global spread.

Conclusion: The results demonstrate that aligning posts with their implied statements via context-bounded mining provides a more stable, bijective-like mapping to related insinuations, overcoming the volatility inherent in traditional clustering-based representation learning.
...
TL;DR: The paper proposes a method to improve sequence labeling by leveraging diffusion to train a Conditional Random Field (CRF) conditioned on an entire label sequence, achieving a 16.5% error reduction for POS-tagging.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitation of traditional CRFs in handling long-range dependencies, which can hurt performance in sequence labeling tasks.

Method: The method involves training a CRF conditioned on a noisy version of the entire label sequence using diffusion, and combining it with approximate CRF inference.

Result: The result is a 16.5% error reduction for POS-tagging, indicating improved label accuracy.

Conclusion: The paper concludes that the proposed method can improve sequence labeling performance by effectively handling long-range dependencies.
...
TL;DR: A framework for efficient financial sentiment analysis using synthetic data and distillation, enabling compact models to achieve strong performance with minimal supervision.


<details>
  <summary>Details</summary>
Motivation: Large instruction-following models are costly to deploy in finance due to limited labelled data caused by confidentiality and expert annotation costs.

Method: The framework uses distillation with synthetic data, transferring knowledge from a large instruction-tuned teacher to compact student models, and involves clustering real examples to select seeds for generating synthetic examples.

Result: The compact model trained on the complete synthetic-seed corpus outperforms the teacher model on a complex and noisy text domain, and remains competitive on formal text.

Conclusion: The framework provides a practical route toward resource-efficient domain adaptation in financial NLP with minimal human labelling effort.
...
TL;DR: The paper introduces a system to improve patient feedback in telemedicine by recommending small, interpretable changes to doctor-patient communication.


<details>
  <summary>Details</summary>
Motivation: Current patient feedback in telemedicine focuses on communication quality, not medical accuracy. The authors aim to bridge this gap.

Method: The authors propose an LM-guided counterfactual recommendation pipeline that analyzes patient-doctor interactions and recommends minimal communication changes.

Result: The system yields a mean +6.41% gain in predicted positive feedback probability and is non-negative for 93.31% of recommendations.

Conclusion: Small, interpretable communication changes can improve patient feedback while preserving doctor control over medical reasoning and final wording.
...
TL;DR: The paper proposes a new method called Robust Pair Confidence Learning (RPCL) to improve multimodal emotion-cause pair extraction by enhancing pair confidence reliability.


<details>
  <summary>Details</summary>
Motivation: Existing methods for pair scoring in multimodal emotion-cause pair extraction treat links independently, leading to under-constrained relative confidence geometry among competing causes.

Method: RPCL, a training-only framework that encourages pair confidence to be both discriminative and stable through a confidence-difference margin constraint and alignment with corrupted view predictions.

Result: RPCL improves Pair F1 by 2.58 to 2.83 percentage points and mean Pair AUPRC on three datasets (ECF, MECAD, and MEC4) in the full text-audio-video setting.

Conclusion: The results suggest that explicitly shaping pair confidence is an effective training strategy for multimodal emotion-cause pair extraction.
...
TL;DR: The paper introduces SPO, a framework for optimizing prompts in AI systems using stochastic search, and compares three strategies for prompt optimization.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve AI systems without parameter updates by optimizing prompts, as recent work shows that textual gradients do not function as real gradients.

Method: The paper proposes SPO (Stochastic Prompt Optimization) and compares three strategies: error-informed random search, a genetic algorithm, and SAGE (SPO via Agent-Guided Exploration).

Result: The results show that no single strategy dominates, and effectiveness depends on the interaction of landscape structure with error type. SAGE is deployed on a mental-health chatbot and shows a statistically robust gain in next-day retention.

Conclusion: The paper concludes that coupling qualitative diagnosis with quantitative validation makes agentic optimization effective for open-ended task-oriented dialogue.
...
TL;DR: The paper examines how well large language models (LLMs) understand text that includes both negation and figurative language, finding that their performance varies significantly with prompt style and can be particularly challenged by the combination of both.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to understand the ability of large language models (LLMs) to correctly interpret text that includes both negation and figurative language, as these are common in everyday language but challenging for current models.

Method: The authors develop new annotations for an existing dataset on figurative language and test a range of language models on this dataset to investigate their ability to interpret text with both negation and figurative language.

Result: The study finds that the combination of negation and figurative language presents a significant challenge for LLMs, and their performance is particularly dependent on the style of prompts used, with variability in performance across different types of negation.

Conclusion: The paper concludes that LLMs face challenges in interpreting text that combines negation and figurative language, and their performance is significantly affected by the prompt style, highlighting the need for further improvement in these areas.
...
TL;DR: The paper introduces SenFlow, a method for detecting AI-generated text at the sentence level in hybrid documents co-authored by humans and large language models (LLMs).


<details>
  <summary>Details</summary>
Motivation: Existing methods for sentence-level AI-generated text detection (S-AGTD) have two limitations: they classify sentences in isolation, ignoring inter-sentence dependencies, and they use outdated benchmarks. The authors aim to address these gaps.

Method: The authors propose SenFlow, which recasts S-AGTD as structured prediction over the document sentence sequence. SenFlow integrates graph-based inter-sentence propagation with linear-chain CRF decoding in a single document-level pass over a sentence graph.

Result: SenFlow achieves state-of-the-art performance on the MOSAIC benchmark, with a +4.15 pp average Macro-F1 margin on cross-domain transfer. The authors also find that AI insertions retain a generator-dependent sentence-length gap that sentence-level detectors can still exploit.

Conclusion: The paper presents SenFlow as a effective method for S-AGTD, and highlights the importance of considering inter-sentence dependencies and using up-to-date benchmarks.
...
TL;DR: The paper proposes GraphPO, a novel RL framework that optimizes large reasoning models by representing rollouts as a directed acyclic graph, reducing redundant exploration and improving inference efficiency.


<details>
  <summary>Details</summary>
Motivation: The existing paradigm of Reinforcement Learning with Verifiable Rewards (RLVR) has limitations, including redundant exploration and sparse rewards, which can be addressed by a more efficient exploration strategy.

Method: The authors propose GraphPO, a graph-based policy optimization framework that represents rollouts as a directed acyclic graph, merging semantically equivalent reasoning paths and reallocating budget for diverse exploration.

Result: GraphPO consistently outperforms chain- and tree-based baselines across three LLMs and various benchmarks, reducing advantage-estimation variance and enhancing reasoning efficiency.

Conclusion: The proposed GraphPO framework offers a more efficient and effective approach to optimizing large reasoning models, with theoretical guarantees and empirical validation.
...
TL;DR: The authors propose CADE, a novel framework for time-series question answering that addresses the tokenization bottleneck in large language models by directly embedding timesteps and aligning semantic representations.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome the limitations of current methods that use patch-based encoders, which can break patterns and hide exact timesteps, and to improve the performance of time-series question answering tasks.

Method: The proposed framework, CADE, consists of two key components: direct timestep embedding and semantic alignment. It uses a point-wise linear encoder and MLP projector to map each timestep directly into the LLM embedding space, and a one-directional supervised contrastive loss to align time-series embeddings with frozen class-name text anchors.

Result: The experimental results on the public Time-MQA benchmark demonstrate that CADE consistently improves performance across six TSQA tasks, outperforming both open-source and proprietary LLM baselines.

Conclusion: The authors conclude that CADE is a effective framework for time-series question answering, which preserves exact index-level access and eliminates the need for patching and padding, while also aligning semantic representations between time-series and language.
...
TL;DR: The paper presents G-IdiomAlign, a benchmark for idiom translation across languages, and evaluates LLMs on idiom translation tasks, showing that glosses improve performance but results remain modest.


<details>
  <summary>Details</summary>
Motivation: Idioms are difficult to translate across languages due to non-compositionality and weak surface-form grounding, making literal mappings unreliable.

Method: The authors construct G-IdiomAlign, a gloss-pivoted benchmark with English glosses from Wiktionary, and propose two evaluation protocols: Multiple-Choice Idiom Equivalence and Gloss-Contrastive Generation.

Result: The results show that glosses consistently improve Gloss-Contrastive Generation, but performance remains modest, and that cross-condition differences are concentrated more in attention heads than in layers.

Conclusion: The study highlights the challenges of idiom translation and the importance of glosses in improving performance, but also reveals substantial headroom for improvement in the open output space.
...
TL;DR: A new framework called ST-Merge is proposed to improve model merging for multilingual reasoning tasks by adaptively modulating the contribution of each source model.


<details>
  <summary>Details</summary>
Motivation: The one-size-fits-all merging strategy often leads to suboptimal performance due to conflicts between source models, and different inputs may require prioritizing certain models over others.

Method: A Steerable Model Merging (ST-Merge) framework is proposed, which uses a gated cross-attention mechanism to weight or filter the two attended source models in an adaptive manner.

Result: ST-Merge consistently outperforms multiple strong baselines on four multilingual reasoning benchmarks across 21 different languages.

Conclusion: The proposed ST-Merge framework effectively addresses the limitations of traditional model merging techniques and achieves better performance in multilingual reasoning tasks.
...
TL;DR: The paper introduces Sumi, a 7B uniform diffusion language model pretrained from scratch on 1.5T tokens, competing with autoregressive models on knowledge, reasoning, and coding benchmarks.


<details>
  <summary>Details</summary>
Motivation: To provide a clean reference point for studying scaling behavior, generation dynamics, controllability, and trade-offs of uniform diffusion language models by pretraining a large-scale model from scratch.

Method: Pretraining a 7B uniform diffusion language model (Sumi) from scratch on 1.5T tokens, releasing model weights, checkpoints, and full training recipe.

Result: Sumi performs competitively with autoregressive models on knowledge, reasoning, and coding benchmarks, but under-performs on commonsense benchmarks.

Conclusion: The release of Sumi enables the community to study native uniform diffusion at scale and catalyzes work on its as-yet poorly understood aspects.
...
TL;DR: This paper proposes a segment combination strategy for automatic multi-label classification of research methods in academic papers, utilizing full-text content and achieving improved classification performance by integrating bibliographic metadata and cross-segment combinations.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the automatic multi-label classification of research methods in academic papers, as existing studies have limitations in utilizing titles, abstracts, and full-text content.

Method: The method involves a segment combination strategy, which partitions full-text content by physical position, and evaluates the classification performance of various segments and their combinations across multiple models.

Result: The results show that methodological information is unevenly distributed within full-text content, with middle-to-late and final segments exhibiting greater discriminative power, and that integrating bibliographic metadata with cross-segment combination strategies enhances classification performance.

Conclusion: The conclusion is that the proposed segment combination strategy can effectively support knowledge services such as method retrieval, review generation, and research intelligence analysis by improving the automatic multi-label classification of research methods.
...
TL;DR: This paper explores the effectiveness of process-level coordination control in multi-agent LLM teams under various conditions, finding that its value depends on specific contingencies such as the reliability of initial majority votes and task recoverability.


<details>
  <summary>Details</summary>
Motivation: The study is motivated by team science theory, which suggests that leadership effectiveness is contingent upon specific conditions, and aims to investigate analogous conditions for multi-agent LLM teams.

Method: The researchers operationalize three classical leadership styles (transactional, transformational, situational) as controllers over a shared action vocabulary and evaluate their performance across four task regimes and three open-weight model families using behavioral signatures and per-action ablations.

Result: The results show that no controller dominates by accuracy across all conditions, with transactional control matching a shared round-0 vote on all 12 (model, regime) combinations to within 1.3pp, and gains only appearing under specific conditions where the round-0 majority is unreliable.

Conclusion: The study concludes that process-level coordination control adds value under measurable conditions that match what team science predicts, and that a largely null accuracy result is expected according to contingency theory, not a failure of the controllers.
...
TL;DR: Dango, a large language model, is introduced for studying Japanese-to-English transfer in second language acquisition, outperforming baselines in simulating human-like language production.


<details>
  <summary>Details</summary>
Motivation: Previous studies on second language acquisition in language models have limitations due to smaller or non-decoder models, and the authors aim to create a more suitable L2 simulator.

Method: The authors propose a filtering method to reduce L2 contamination in the pretraining corpus and fine-tune the model on LLM-generated L2-learning lessons.

Result: Dango develops human-like L2 production patterns, outperforming unfiltered and standard multilingual baselines.

Conclusion: The authors release Dango, along with its data and code, to facilitate reproducible computational SLA research and learner-facing applications.
...
TL;DR: The paper presents ClaMPAPP, a hybrid system that uses a large language model (LLM) as an interface to extract clinical features from free-text narratives and passes them to a machine learning model for diagnosis, achieving better performance and safety in clinical decision support.


<details>
  <summary>Details</summary>
Motivation: The direct use of large language models (LLMs) as diagnostic engines is limited by sensitivity to prompts, information order, and incorrect outputs, while structured machine-learning models require tabular inputs that are difficult to integrate with narrative clinical workflows.

Method: The ClaMPAPP system uses an LLM to extract schema-constrained clinical features from free-text narratives, applies plausibility checks, and passes validated features to an XGBoost classifier trained on clinical, laboratory, and ultrasound variables.

Result: ClaMPAPP achieved the strongest overall diagnostic performance in both internal and external validation while minimizing missed appendicitis cases, outperforming end-to-end LLM baselines.

Conclusion: The results support an LLM-as-interface, ML-as-predictor design that separates natural-language usability from predictive inference, providing a more auditable pathway for clinical decision support.
...
TL;DR: The paper introduces RECOM, a dataset for evaluating LLMs on open-ended question answering, and finds that common metrics struggle to balance validity and discriminative power.


<details>
  <summary>Details</summary>
Motivation: Current metrics for evaluating LLM-generated text are expected to perform two tasks: assessing content validity and distinguishing between better and worse systems, but these tasks are often in tension, particularly in open-ended, opinion-driven question answering.

Method: The authors introduce RECOM, a contamination-free evaluation dataset of 15,000 r/AskReddit questions paired with authentic community replies, and use it to evaluate five open-source LLMs against every reply, pairing each metric with a random-derangement noise floor.

Result: The study finds that no metric performs both tasks well: cosine similarity effectively separates real from random answers but fails to rank models, while BERTScore precision appears to rank models but its validity is weak and collapses when response length is controlled.

Conclusion: The authors conclude that the tradeoff between validity and discrimination is a property of the metrics, not the models, and recommend reporting metrics on both axes with an explicit random-baseline floor.
...
TL;DR: The paper introduces DreamReasoner-8B, a block diffusion language model for long chain-of-thought reasoning, and proposes block-size curriculum learning to improve performance.


<details>
  <summary>Details</summary>
Motivation: To investigate whether block diffusion language models can be reliably scaled for long chain-of-thought (CoT) reasoning and to address the performance disparity caused by different block sizes.

Method: The authors develop DreamReasoner-8B and conduct a systematic study on the effect of training and inference block sizes on long-CoT reasoning. They propose block-size curriculum learning to bridge the granularity gap.

Result: DreamReasoner-8B achieves results competitive with leading open autoregressive models on mathematical and code reasoning benchmarks.

Conclusion: The work establishes a practical foundation for efficient, reasoning-capable diffusion language models and releases the model for further research.
...
TL;DR: This paper studies the adaptation of large language models to the medical domain in French, comparing continual pretraining, supervised fine-tuning, and their combination.


<details>
  <summary>Details</summary>
Motivation: The effectiveness of domain adaptation strategies for large language models remains unclear, particularly in specialized domains like medicine and languages other than English.

Method: The study compares continual pretraining (CPT), supervised fine-tuning (SFT), and their combination across three model families, multiple sizes, and three initialization types.

Result: For multiple-choice QA, CPT+SFT often achieves the best scores, but gains over SFT are small. For open-ended QA, CPT improves overlap-based metrics, while SFT degrades generation quality.

Conclusion: The study provides practical guidelines for selecting adaptation strategies under computational constraints, finding SFT to be a strong and cost-effective default for multiple-choice QA.
...
TL;DR: The paper proposes a novel multi-agent system paradigm called Multi-Agent Fictitious Play (MAFP) to address decision-making tasks that involve mutually dependent stakeholders' decisions.


<details>
  <summary>Details</summary>
Motivation: The authors identified a limitation in current large language model-based multi-agent systems (MAS) in handling decision-making tasks where stakeholders' decisions are interdependent, which they term as 'stance entanglement'.

Method: The authors propose MAFP, a game-theoretic approach that represents stakeholders as agents and updates their decisions through an equilibrium-seeking process, iteratively best responding to the empirical mixture of other agents' past decisions.

Result: MAFP outperforms single-round and multi-round baselines on two metrics, tournament strength and robustness, in challenging decision-making tasks, demonstrating its effectiveness in addressing stance entanglement.

Conclusion: The paper concludes that MAFP is an effective approach for addressing decision-making tasks with mutually dependent stakeholders' decisions, and it has the potential to improve decision quality and robustness in complex scenarios.
...
TL;DR: The paper introduces LOCUS, a comprehensive corpus of U.S. local ordinances, and a county-harmonized access layer, facilitating large-scale research and analysis of local laws.


<details>
  <summary>Details</summary>
Motivation: The lack of access to authoritative legal texts at scale, particularly local ordinances, hinders progress in legal AI. Local codes govern various aspects of everyday regulation but are fragmented and not readily available for bulk research.

Method: The authors create LOCUS by collecting and processing municipal and county ordinance codes using OCR to handle various document formats. They also develop a county-harmonized access layer and train ModernBERT-based classifiers and scorers to analyze local laws.

Result: The LOCUS corpus contains codes from 9,239 cities and counties, with a smaller access layer covering 2,309 U.S. counties and a majority of the population. The corpus and derivative models are made available for research and analysis.

Conclusion: The paper presents LOCUS as a significant resource for legal AI research, enabling large-scale analysis of local laws and providing a foundation for future studies on aspects such as opacity and paternalism.
...
TL;DR: A new method called Turing-RL is proposed for training user simulator models to mimic human users in interactive settings, outperforming existing approaches.


<details>
  <summary>Details</summary>
Motivation: To advance the training of agent assistants, evaluation of personalization systems, and social science research by simulating human users effectively.

Method: Turing-RL, a Turing-Test-based reinforcement learning approach using a discriminative Turing reward with an LLM judge to score generated responses.

Result: Turing-RL consistently outperforms baseline methods on both LLM and human evaluation metrics across conversational chat and Reddit forum discussion domains.

Conclusion: Optimizing for indistinguishability rather than response matching is effective for learning user simulators.
...
TL;DR: The paper presents NAVI-Orbital, a software system on a Low Earth Orbit spacecraft that performs autonomous multi-modal inference using a vision-language model entirely onboard, enabling semantic compression of Earth observations.


<details>
  <summary>Details</summary>
Motivation: The gap between onboard collection and actionable ground intelligence has widened due to the increasing generation of Earth Observation data outpacing downlink bandwidth and human-in-the-loop processing.

Method: NAVI-Orbital uses a local vision-language model (Gemma 3) and a graph-based state machine (LangGraph) to classify scenes, produce text descriptions, and respond to operator queries via natural-language dialogue.

Result: The system achieved 88.16% accuracy on the AID benchmark, and successfully processed live in-orbit captures of new Earth imagery, demonstrating the feasibility of running foundation models on satellite-class edge computers.

Conclusion: The paper demonstrates the feasibility of running foundation models on satellite-class edge computers to invert the conventional acquire-then-downlink-everything bandwidth profile through semantic compression of Earth observations in-orbit.
...
TL;DR: The paper introduces CaVe-VLM-CoT, a framework that improves Vision-Language Models by reducing hallucinations through a five-stage pipeline, achieving better accuracy and faithfulness in its outputs.


<details>
  <summary>Details</summary>
Motivation: Vision-Language Models are prone to hallucinations, and existing methods only partially address this issue. The authors aim to create a framework that enforces evidence-grounded reasoning and corrects verification failures.

Method: The authors propose a modular reflection-based agentic-RAG framework called CaVe-VLM-CoT, which consists of a five-stage closed-loop pipeline: Extractor, Retriever, Solver, Citation Injector, and Verifier.

Result: CaVe-VLM-CoT achieves 87.1% accuracy and 56.6% CaVeScore on ScienceQA, and 55.2% accuracy and 35.7% CaVeScore on MMMU (30 subjects).

Conclusion: The proposed framework, CaVe-VLM-CoT, effectively reduces hallucinations in Vision-Language Models and improves the faithfulness of their outputs, as demonstrated by its performance on ScienceQA and MMMU datasets.
...
TL;DR: The paper studies human-AI teams and how they coordinate to improve performance on tasks, finding that structure and scaffolding are crucial for effective collaboration.


<details>
  <summary>Details</summary>
Motivation: The increasing capability of AI agents, but the need for human judgment and expertise in many tasks, leading to the study of shared-workspace human-AI teams.

Method: Using the Collaborative Gym environment with DiscoveryBench tasks, and evaluating scaffolding that combines shared group memory with simulated human-in-the-loop (HITL) gates.

Result: Adding relevant collaborators can lower performance when teams lack structure, but scaffolding yields higher mean performance, especially in three-person teams.

Conclusion: The way human-AI teams coordinate and integrate expertise is as important as the capability available to them, and that structure and scaffolding can improve performance.
...
TL;DR: The paper introduces CEO-Bench, a benchmark for evaluating language model agents' abilities in simulating a startup's operations over 500 days, testing skills like long-term planning, information acquisition, and adaptation.


<details>
  <summary>Details</summary>
Motivation: Current language model agents excel in short-horizon tasks but struggle with complex, long-term challenges. The authors aim to bridge this gap by creating a comprehensive evaluation framework.

Method: The authors simulate a startup environment where an agent must manage various aspects like pricing, marketing, and budgeting through a Python interface, facing uncertainty and noisy data.

Result: Even state-of-the-art models like Claude Opus 4.8 and GPT-5.5 struggle to consistently turn a profit, with only a few agents finishing above the $1M starting balance.

Conclusion: CEO-Bench takes a first step toward measuring the intelligence required to drive sustained, adaptive progress over time, highlighting the need for further improvement in language model agents' capabilities.
...
TL;DR: The paper introduces DeFAb, a benchmark for defeasible abduction, and shows that rule-based logic solvers outperform frontier language models in terms of accuracy and robustness.


<details>
  <summary>Details</summary>
Motivation: The authors aim to create a benchmark for measuring creativity and theoretical reasoning in defeasible abduction, which involves constructing hypotheses that explain anomalies while preserving unrelated expectations.

Method: The authors create a dataset and generation pipeline that converts publicly funded knowledge bases into formally grounded instances for defeasible abduction, and evaluate the performance of rule-based logic solvers and frontier language models on this benchmark.

Result: The results show that rule-based logic solvers achieve 100% accuracy on the benchmark, while frontier language models have lower accuracy (23.5-65%) and are not robust to rendering variations.

Conclusion: The paper concludes that DeFAb is a challenging benchmark for defeasible abduction, and that rule-based logic solvers are more effective than frontier language models in this task, highlighting the need for more robust and accurate models.
...
TL;DR: The paper presents a framework for optimizing lithium mine production decisions using a partially observable Markov decision process (POMDP) approach, which outperforms human-inspired heuristics in adapting to changing lithium price regimes and achieving better demand fulfillment and environmental outcomes.


<details>
  <summary>Details</summary>
Motivation: The challenge of making informed decisions in lithium production, considering uncertainties in geology, prices, demand, and extraction methods.

Method: The authors frame the problem as a POMDP and solve it using belief state planning methods to obtain optimal decision-making.

Result: The POMDP approach outperforms human-inspired heuristics in dynamically adapting to shifting lithium price regimes, achieving higher demand fulfillment, and more balanced economic and environmental outcomes.

Conclusion: The proposed framework provides a more robust strategy for determining when and where to open a mine, which method of production to pursue, and optimally sequencing exploration, production, and technology choice.
...
TL;DR: A new simulated-world forecasting benchmark called ForecastBench-Sim is introduced, built on a turn-based strategy game, to provide controlled and immediately resolvable tasks for studying probabilistic reasoning.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the limitations of real-world forecasting benchmarks, such as slow outcome resolution, rare tail events, and difficulty in scoring counterfactual questions.

Method: The authors propose using a simulated world based on a game (Freeciv) to generate forecasting questions with immediate resolution, allowing for controlled experiments and evaluations.

Result: The authors report validation results from model evaluations and a human pilot study, indicating the potential of ForecastBench-Sim as a complementary benchmark for real-world forecasting.

Conclusion: ForecastBench-Sim provides a new tool for studying probabilistic reasoning under dynamic world states, offering a controlled and immediately resolvable alternative to traditional real-world forecasting benchmarks.
...
TL;DR: The paper explores the memory requirements for generalist agents to act optimally across multiple environments and goals.


<details>
  <summary>Details</summary>
Motivation: To develop a formal account of what generalist agents must store in memory to act near-optimally across multiple environments and goals.

Method: The paper uses mathematical theorems and proofs to show the necessity of memory for generalist agents.

Result: The paper shows that generalist agents must preserve domain-relevant information in memory and that memory can be used to reconstruct local transition dynamics.

Conclusion: The paper concludes that memory is essential for generalist agents to support domain disambiguation, transition-model reconstruction, and planning.
...
TL;DR: R2D-RL is a new reinforcement learning environment for robot soccer, connecting RCSS2D to Python MARL interface, enabling full-field and scenario-based training.


<details>
  <summary>Details</summary>
Motivation: The difficulty of using RCSS2D with modern Python-based MARL workflows due to its competition-oriented server-client architecture.

Method: R2D-RL connects RCSS2D and HELIOS-based player clients to a Python MARL interface through shared-memory communication and cycle-level synchronization.

Result: R2D-RL supports various features and provides baseline results for front-goal scenarios and 11-vs-11 full-field benchmark.

Conclusion: R2D-RL provides a new platform for multi-agent reinforcement learning in robot soccer, making it easier to use RCSS2D with modern Python-based MARL workflows.
...
TL;DR: ProfiLLM, an LLM data pipeline, was developed to improve user profiling for ride-hailing dispatch systems, achieving significant improvements in prediction and dispatching simulations.


<details>
  <summary>Details</summary>
Motivation: The motivation is to leverage Large Language Models (LLMs) to extract semantic features from platform-scale behavioral logs to improve ride-hailing dispatch systems, addressing the limitations of current structured numerical feature-dominated matching pipelines.

Method: ProfiLLM consists of two modules: (1) Tool-Augmented Global Knowledge Mining, which equips an LLM agent with 27 analytical tools to mine platform-scale data, and (2) Utility-Aligned Profile Exploration, which generates candidate profiles, evaluates them via a utility proxy, and refines the best candidates for DPO fine-tuning.

Result: ProfiLLM achieved up to +6.14% relative AUC improvement in outcome prediction, up to +4.35% GMV gain in dispatching simulation, and consistent improvements in a 14-day online A/B test, including +0.47% GMV, +0.33% Completion Rate, and -0.82% Cancel-Before-Accept rate.

Conclusion: The ProfiLLM pipeline effectively operationalizes utility-aligned user profiling for production matching systems, demonstrating significant improvements in prediction and dispatching performance.
...
TL;DR: A new benchmark and memory framework are proposed for long-term embodied household assistance, addressing challenges in partial observability and long-term memory use.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for long-term memory and embodied agents have limitations, mainly evaluating language-centric retrieval or short-horizon task execution, and do not adequately test long-term memory use in dynamic environments.

Method: The authors introduce WorldLines, a benchmark for long-horizon embodied household assistance, and ObsMem, an observer-grounded memory framework that maintains visibility-aware memories and action-native state trails.

Result: Experiments show that ObsMem offers a stronger reference architecture for long-term embodied household assistance, but persistent challenges remain in partial observability, overwritten world states, and translating long-term memory into embodied plans.

Conclusion: The proposed benchmark and memory framework can help advance research in long-term embodied household assistance, but further work is needed to address the remaining challenges.
...
TL;DR: The authors introduce Xcientist, a research harness that externalizes and makes transparent the processes of research synthesis and experimental validation in AI systems, enabling inspectable and accountable scientific workflows.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the issue of implicit reasoning in AI systems, where the links between prior evidence, generated ideas, experiments, and final claims are not transparent, making it difficult to evaluate the validity and accountability of AI-generated research.

Method: The authors propose Xcientist, a research harness that organizes literature evidence, idea states, implementation plans, ablation records, and repair traces as persistent research artifacts, allowing for inspectable and contract-governed processes.

Result: The results show that Xcientist preserves traceable trajectories from problem formulation to mechanism design, validation, and bounded revision across various applications, including training-free memory systems, graph-structured traffic forecasting, and multi-scale physics-informed neural networks.

Conclusion: The authors conclude that AI scientists should be evaluated not only by their final artifacts but also by whether their synthesis and validation processes remain attributable, inspectable, and scientifically accountable, highlighting the importance of transparent and accountable AI-driven research.
...
TL;DR: BeliefDiffusion is a novel framework for navigation in partially observable environments. It combines generative models and planning to improve navigation success rate and path efficiency.


<details>
  <summary>Details</summary>
Motivation: Navigation in partially observable environments is challenging due to limited sensory information. Existing methods, such as belief-based methods and generative models, have limitations in capturing multimodality and planning ahead.

Method: BeliefDiffusion uses diffusion models to characterize multimodal belief distributions and Model Predictive Control (MPC) for planning. It consists of two steps: imagining environment configurations and planning navigation strategies.

Result: BeliefDiffusion outperforms model-free reinforcement learning baselines and other generative approaches in navigation success rate and path efficiency in synthetic map environments.

Conclusion: BeliefDiffusion enables more robust navigation in partially observable settings by explicitly incorporating multimodal belief representations into planning.
...
TL;DR: The paper proposes Skill-Guided Continuation Distillation (SGCD) to improve GUI agents by addressing the issue of off-trajectory states during closed-loop execution.


<details>
  <summary>Details</summary>
Motivation: The current policy deviates from the expert policy and encounters unseen states during execution, leading to a supervision gap.

Method: SGCD is an iterative self-improvement framework that runs the plain policy to reach off-trajectory states, then uses a skill-guided policy to complete the task and produce successful continuations.

Result: SGCD improves the success rate of three base models from the low-30% range to over 50% on OSWorld-Verified.

Conclusion: The proposed SGCD framework effectively addresses the supervision gap issue and demonstrates its effectiveness and generality in improving GUI agents.
...
TL;DR: The paper introduces SciRisk-Bench, a benchmark for evaluating AI safety in scientific contexts, covering 7 disciplines and 10 risk dimensions.


<details>
  <summary>Details</summary>
Motivation: The increasing use of large language models in AI for Science workflows creates a need for safety benchmarks that assess not only scientific competence but also risk recognition and avoidance in high-stakes contexts.

Method: The authors introduce SciRisk-Bench, a benchmark that evaluates AI4Science safety from two perspectives: explicit risk dimensions and scientific disciplines.

Result: The experimental evaluation of mainstream and science-oriented LLMs across risk dimensions, disciplines, and sub-disciplines enables fine-grained diagnosis of where scientific models remain unsafe.

Conclusion: The paper presents SciRisk-Bench as a comprehensive benchmark for assessing AI safety in scientific contexts, highlighting areas where models remain unsafe.
...
TL;DR: The paper presents Decoupled Search Grounding (DSG), a vendor-agnostic approach to improve the efficiency and control of search grounding in large language models (LLMs) by decoupling it from the model provider boundary.


<details>
  <summary>Details</summary>
Motivation: The current native search grounding approach in LLMs has limitations, including difficulty in inspection, tuning, reuse, or porting, and can lead to Search-Induced Verbosity. The authors aim to address these issues by proposing a decoupled approach.

Method: The authors propose DSG, which moves grounding outside the reasoning model through an MCP-compatible gateway, providing controls such as provider routing, source-aware context rendering, and caching.

Result: The results show that DSG achieves comparable or better performance than native search grounding on various benchmarks, including SimpleQA, FreshQA, and HotpotQA, while reducing search cost by up to 98% and latency by 68%.

Conclusion: The authors conclude that real-time grounding should be treated as an optimizable interface boundary, not a fixed model feature, and that DSG provides a more efficient and controllable approach to search grounding in LLMs.
...
TL;DR: The paper introduces RTSGameBench, a new benchmark for evaluating Vision-Language Models (VLMs) in real-time strategy games, highlighting their limitations in strategic reasoning and proposing a new agent, RTSGameAgent.


<details>
  <summary>Details</summary>
Motivation: Existing real-time strategy benchmarks have limited evaluation scope, lack systematic competency diagnosis, and are fixed in scenario coverage, which hinders the assessment of VLMs' strategic reasoning abilities.

Method: The authors propose RTSGameBench, built on the Beyond All Reason game, which offers diverse gameplay, diagnostic assessment via mini-games, and extensible coverage through a self-evolving generation framework. They also introduce RTSGameAgent, which manages units using an FSM with agentic memory.

Result: State-of-the-art VLMs perform poorly in large-scale RTS games, particularly when matchups require tighter coordination, multi-agent coordination, and when task scale increases.

Conclusion: The paper highlights the limitations of current VLMs in strategic reasoning and proposes a new benchmark and agent architecture to improve their performance in real-time strategy games.
...
TL;DR: The paper proposes ThinkDeception, an interpretable multimodal deception detection framework that uses Multimodal Large Language Models (MLLMs) to transform deception detection into an explicit cognitive reasoning process.


<details>
  <summary>Details</summary>
Motivation: Existing approaches to multimodal deception detection rely on black-box paradigms, lacking interpretability and struggling to capture subtle cross-modal inconsistencies. The authors aim to transcend these limitations by proposing a novel and interpretable framework.

Method: The authors propose ThinkDeception, which introduces MLLMs into deception detection and uses a foundational model, ThinkDeception Base, and a core innovation, Visual-Audio Consistency Group Relative Policy Optimization (VAC-GRPO), with a progressive training strategy.

Result: Extensive experiments on mainstream benchmarks demonstrate that ThinkDeception establishes a new State-of-the-Art (SOTA), significantly outperforming existing methods in both detection accuracy and rationale quality.

Conclusion: The paper successfully drives the field of deception detection toward interpretable, multimodal cognitive reasoning, providing a novel and effective framework for identifying fraudulent intentions.
...
TL;DR: The paper proposes RODS, a method for generating informative samples online to improve multi-turn tool-use reinforcement learning (RL) by addressing the depletion of useful data in static datasets.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the bottleneck in multi-turn tool-use RL caused by the rapid depletion of informative samples in static datasets, leading to inefficient learning.

Method: The method proposed is RODS (Reward-driven Online Data Synthesis), which uses the progress reward variance as a boundary detector to identify and synthesize new multi-turn variants of informative samples, and manages a dynamic replay buffer.

Result: RODS achieves comparable performance to a 17K-sample offline pipeline with roughly 20x fewer trajectories, improving over fixed-data RL and environment augmentation in a controlled setting.

Conclusion: The paper concludes that RODS effectively resolves the depletion of informative samples in static datasets for multi-turn tool-use RL, leading to more efficient learning with fewer samples.
...
TL;DR: ARIADNE is a training-free, adapter-agnostic routing framework for dynamic adapter selection at inference time, achieving high performance and accuracy without requiring access to adapter internals or additional training.


<details>
  <summary>Details</summary>
Motivation: The need for automatic adapter selection in a growing and heterogeneous adapter pool, where inference-time queries arrive without task labels, and existing routing methods have limitations in scalability and portability.

Method: ARIADNE represents each adapter through a set of centroids computed from embeddings of its training set, and selects an adapter by measuring proximity to these centroids in latent space.

Result: ARIADNE recovers 97.44% of the upper bound performance on 23 diverse NLP tasks with Llama 3.2 1B Instruct, and achieves 89.7% average selection accuracy on 44 tasks.

Conclusion: ARIADNE provides a scalable and portable solution for dynamic adapter selection, compatible with arbitrary PEFT methods, and requiring no modification to the adapters or training procedures.
...
TL;DR: The paper proposes a redesign of the World Wide Web to accommodate AI agents as intermediaries between humans and web content, ensuring equal access rights, a new economic model, and measures to maintain human oversight.


<details>
  <summary>Details</summary>
Motivation: The rapid emergence of AI agents as intermediaries between humans and web content invalidates the assumption that humans are the primary consumers of web content, leading to a need for a principled redesign of the web.

Method: The paper proposes a redesign across three layers: access, economic, and content. This includes agent identification metadata, a dual-layer architecture, an intent-based tier framework, a token-based subscription model, and the Agent Text Markup Language (ATML).

Result: The proposed redesign includes ten design principles for an agent-first internet, ensuring agents are first-class citizens with equal access rights and a new economic model that maintains human intentionality and oversight.

Conclusion: The paper concludes that the web needs to renegotiate its foundational social contract to integrate AI agents as intermediaries, ensuring a more inclusive and effective interaction between humans, agents, and web content.
...
TL;DR: This paper combines deep neural networks with explainable AI techniques to analyze the determinants of electricity prices across 39 European bidding zones, finding that renewable energy sources, particularly solar, and gas prices are key drivers of price formation.


<details>
  <summary>Details</summary>
Motivation: The complexity of electricity markets and the lack of interpretability in deep neural networks (DNNs) used for predicting electricity prices motivated this study to combine DNNs with explainable AI techniques.

Method: The study employs DNN models with SHapley Additive exPlanations (SHAP) and an extended version of SSHAP, an aggregation framework for improving interpretability in high-dimensional settings.

Result: The analysis identified renewable energy sources, particularly solar, as playing a disproportionately important role in price formation, while gas prices remained a dominant and consistent driver across electricity markets.

Conclusion: The study highlights the strong interdependence of European electricity systems and the importance of renewable energy sources and gas prices in shaping price dynamics, with implications for understanding the underlying drivers of price formation in electricity markets.
...
TL;DR: A new framework, HACD-H, is proposed to model human-AI interaction as a self-organizing social cognitive system, integrating emotional adaptation, relational organization, social memory, and personality consistency.


<details>
  <summary>Details</summary>
Motivation: Current conversational AI systems lack a unified framework to explain the emergence of stable social relationships and social intelligence in long-term human-AI interaction.

Method: The Human-AI Coevolution Dynamics Framework (HACD-H) is proposed, which integrates emotional adaptation, relational organization, social memory, and personality consistency into a unified dynamical framework.

Result: Results reveal a hierarchy of temporal persistence in social cognition, stable relational attractors, phase-transition-like developmental patterns, and a structured social cognitive energy landscape. Social intelligence shows a significant negative correlation with social cognitive energy.

Conclusion: HACD-H provides a unified theoretical foundation for modeling adaptive human-AI social interaction and developing socially intelligent AI systems.
...
TL;DR: The authors propose Safety Reflection Pretraining, a method to improve safety alignment in large language models by integrating self-monitoring into the pretraining stage, demonstrating improved safety classification accuracy and reduced vulnerability to attacks.


<details>
  <summary>Details</summary>
Motivation: Recent efforts focus on making data safe during pretraining, but the authors argue that this is insufficient as models can compose benign knowledge into unsafe behaviors, highlighting the need for a more comprehensive approach to safety alignment.

Method: The authors propose Safety Reflection Pretraining, which inserts short safety reflections into pretraining corpora to integrate self-monitoring into language modeling, and evaluate it with 1.7B models pretrained on FineWeb-Edu.

Result: The experiments show that Safety Reflection Pretraining improves safety classification accuracy and substantially reduces the success rates of inference-stage and finetuning attacks, outperforming data filtering and rewriting in a synthetic environment called MedSafetyWorld.

Conclusion: The findings suggest that pretraining alignment should not only focus on making training data safe but also shape the behaviors that models acquire from safe data, highlighting the importance of integrating self-monitoring into the pretraining stage.
...
TL;DR: The paper proposes a new approach called 'User as Engram' for personalizing language models, which stores user content as local edits to a memory table and shares reasoning skills across users, achieving better performance and efficiency.


<details>
  <summary>Details</summary>
Motivation: Current personalization methods in language models either store user facts outside the model weights or overwrite existing knowledge when writing new facts, leading to limitations in scalability and performance. The authors aim to create a more brain-like approach that separates content and reasoning skills.

Method: The proposed 'User as Engram' approach stores user content as surgical edits to a hash-keyed memory table, while sharing reasoning skills across users through a single adapter. This design allows for efficient storage and composition of user facts.

Result: The 'User as Engram' approach achieves 5.6x higher indirect-reasoning accuracy on average compared to per-user LoRA, while delivering similar direct recall performance. It also results in a 33,000x smaller memory footprint and can handle multiple users without growing the retriever search space.

Conclusion: The 'User as Engram' approach offers a more efficient and effective way to personalize language models, separating content and reasoning skills, and allowing for scalable and composable user facts.
...
TL;DR: The paper introduces TxBench-PP, a benchmark for evaluating AI agents in small-molecule preclinical pharmacology, and finds that current models struggle to recover accurate conclusions from real-world assay data.


<details>
  <summary>Details</summary>
Motivation: The authors aim to accelerate drug discovery by developing trusted AI agents that can evaluate realistic program decisions, but current models lack practical deployment due to limited evaluation on realistic scenarios.

Method: The authors propose TxBench-PP, a benchmark containing 100 evaluations across various program stages, assay types, and task structures, and test 16 model-harness configurations comprising 11 models and 4,800 trajectories.

Result: The strongest AI model, Claude Opus 4.8 / Pi, achieved a 59.3% success rate (178/300) in recovering preclinical pharmacology decisions, while the second-best model, GPT-5.5 / Pi, achieved a 55.3% success rate (166/300).

Conclusion: The authors conclude that current AI models are not reliable for practical deployment in preclinical pharmacology, highlighting the need for further improvement in AI agents for drug discovery.
...
TL;DR: The paper introduces X+Slides, a benchmark for audience-conditioned slide generation, evaluating systems based on audience needs and source-grounded probes.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for slide generation overlook the target audience's needs, which are crucial in real-world applications.

Method: The authors propose a dynamic evaluation framework with 8,133 source-grounded probes and four metrics: Audience Coverage, Domain-wise Coverage, Efficiency, and Correctness.

Result: Experiments on DeepPresenter, SlideTailor, and NotebookLM show that current systems can recover a substantial but incomplete part of audience-essential information.

Conclusion: The results highlight the importance of source-grounded evaluation and audience-conditioned slide generation, indicating that visual quality and broad topic coverage are not sufficient evidence without proper evaluation.
...
TL;DR: The paper presents NeSyCat Torch, an extension of NeSyCat that integrates neural networks into a unified framework for neurosymbolic semantics, enabling efficient training and outperforming existing approaches in speed and accuracy.


<details>
  <summary>Details</summary>
Motivation: The existing neurosymbolic semantics frameworks (classical, fuzzy, probabilistic, and neural systems) define truth using their own inductive rules, and there is a need for a unified framework that can subsume them under a single inductive definition of truth.

Method: The authors extend NeSyCat with NeSyCat Torch, which interprets computational symbols via neural networks, and implement the framework in probabilistic programming and tensor-based backends using monads (distribution monad, lazy log-tensor monad, and batch monad).

Result: The NeSyCat Torch implementations in HaskTorch, JAX, and PyTorch outperform LTN and DeepProbLog in speed and accuracy on MNIST addition, while achieving nearly the accuracy of DeepStochLog, within a uniform framework that can be applied to many first-order NeSy approaches.

Conclusion: The paper provides a unified framework for neurosymbolic semantics that integrates neural networks, and demonstrates its efficiency and effectiveness in a specific application, with potential for extension to continuous probability and other areas.
...
TL;DR: The paper proposes Rubric-Conditioned Self-Distillation, a framework that uses rubrics as fine-grained feedback for on-policy self-distillation, surpassing existing methods on science reasoning benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current post-training methods for reasoning language models rely on supervised distillation and reinforcement learning, which have limitations such as expensive and noisy annotations, and scalar rewards that obscure feedback.

Method: The proposed framework, Rubric-Conditioned Self-Distillation, conditions the teacher model on criterion-level rubrics and uses it to provide token-level guidance on the student's own sampled trajectories.

Result: The method surpasses GRPO by 1.0 points and OPSD by 0.9 points on average on a diverse suite of science reasoning benchmarks.

Conclusion: The proposed framework effectively converts rubric-level criteria into token-level guidance over the reasoning process, providing more fine-grained credit assignment and improving performance on science reasoning benchmarks.
