...
TL;DR: A large generative model for synthesizing chest radiographs with high fidelity and controllability is introduced, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Existing radiographic AI models have poor generalization across patient subpopulations, institutions, and acquisition settings, limiting their clinical utility. High-fidelity synthesis of chest radiographs can help diversify clinical datasets and evaluate model robustness.

Method: A generative foundation model with 1.3B parameters was trained from scratch on a curated dataset of 1.2M radiographs and clinical expert-guided metadata for 1.6T tokens.

Result: The model supports controllable radiograph generation and editing across multiple demographic subgroups, acquisition views, and pathologies, producing images that are indistinguishable from real radiographs to clinical experts.

Conclusion: The introduced model significantly advances the state of the art in radiograph synthesis fidelity, enabling the creation of more diverse and robust clinical datasets.
...
TL;DR: The paper proposes LEAP, a training curriculum for feature-based knowledge distillation of Vision Transformers, which improves student model accuracy and reduces training costs.
...
Method: The authors propose LEAP: Layer-skipping Efficiency via Adaptive Progression, a training curriculum that utilizes the teacher's intermediate feature maps as a sequence of progressively more difficult targets.

Result: LEAP-distilled ViT-S achieves 90.1% accuracy on ImageNet-100, with +12.24% improvement compared to baseline; +3.84% and +7.75% improvement for instance retrieval on Oxford and Paris datasets; 25.1% savings in training FLOPs and 21% savings in training time.

Conclusion: The proposed LEAP curriculum significantly accelerates convergence and improves student model accuracy across various student model sizes and dataset scales, while reducing training costs.
...
TL;DR: LooseControlVideo is a framework for intuitive and expressive control in text-to-video generation, enabling users to author high-level layout and trajectory using sparse, oriented 3D boxes.


<details>
  <summary>Details</summary>
Motivation: The challenge of precise 3D spatial orchestration in text-to-video generation, particularly for multi-object scenes with entangled semantic layout and temporal dynamics.
...
<details>
  <summary>Details</summary>
Motivation: The authors identify limitations in video-based world action models, including costly inference, unnecessary details, and potential errors in long-horizon future imagination.
...
<details>
  <summary>Details</summary>
Motivation: The authors aim to address the limitation of existing multimodal large language models (MLLMs) that rely on autoregressive generation, which limits their efficiency for perception tasks that require captioning multiple regions.
...
Result: Experiments demonstrate that PerceptionDLM maintains competitive performance in region captioning while achieving substantial speed improvements for multi-region perception tasks.

Conclusion: The authors highlight the potential of multimodal diffusion language models for efficient, parallel visual perception, and to the best of their knowledge, they are the first to achieve parallel region caption and perception by leveraging the advantages of diffusion language models.
...
Result: The results show that Mix-QVLA improves accuracy-efficiency trade-off, reduces memory from 15.4 GB to 4.1 GB, retains 96.3% average success rate, and achieves a 1.52x inference speedup.
...
TL;DR: The paper proposes a new paradigm for vision foundation models, using language to dynamically guide the vision encoder, resulting in more controllable and generalizable representations.


<details>
  <summary>Details</summary>
Motivation: Current vision foundation models are trained as static feature extractors, relying on large downstream models for task adaptation. The authors aim to reduce the burden on downstream models and improve adaptability.

Method: The authors propose Language-Instructed Vision Embeddings (LIVE), which uses language as high-level guidance to produce task-centric embeddings at inference time.

Result: LIVE reduces visual hallucinations (+34 points on MMVP), surpasses vision-language models with more parameters on visual question answering, and generalizes to unseen instructions and tasks.

Conclusion: The proposed approach offers a direct path toward adaptive, instruction-driven visual intelligence, enabling more controllable and generalizable representations.
...
TL;DR: The paper presents GB-LSR, a method for continuous image reconstruction that achieves high performance and efficiency.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve image reconstruction quality and efficiency, particularly in continuous coordinates.

Method: GB-LSR uses a fixed-grid local spectral representation, partitioning the image domain into non-overlapping square patches with shared convolutional-encoder features and a single trainable scalar bandwidth.

Result: GB-LSR outperforms baseline methods by 2.8-3.6 dB PSNR and 0.11-0.15 LPIPS, with one-quarter of the slowest baseline's inference cost. It also achieves competitive results in arbitrary-scale super-resolution.

Conclusion: The proposed GB-LSR method achieves high-quality image reconstruction with improved efficiency, and the use of a single global scalar bandwidth is sufficient for good performance.
...
TL;DR: The paper proposes learning an asynchronous schedule for multi-representation diffusion models to improve visual synthesis. It introduces a flexible parametric class and a schedule-corrected objective, achieving state-of-the-art results on ImageNet 256x256 with less training.


<details>
  <summary>Details</summary>
Motivation: The performance of multi-representation diffusion models heavily depends on the asynchronous schedule. The authors aim to learn this schedule to improve visual synthesis.

Method: The authors propose a method that formulates asynchronous flow matching over multiple representation spaces and uses a schedule-corrected objective. They instantiate the schedule with a flexible parametric class and learn it using a fast joint probe.

Result: The learned schedule substantially improves convergence speed and final quality on ImageNet 256x256. The 200-epoch model reaches FID 1.05, matching the 800-epoch SFD-XL baseline with 4x less training. Training to 600 epochs further improves to FID 1.02.

Conclusion: The paper concludes that learning an asynchronous schedule for multi-representation diffusion models can significantly improve visual synthesis. The proposed method achieves state-of-the-art results on ImageNet 256x256 with reduced training epochs.
...
TL;DR: This paper proposes TeleMorpher, a one-shot framework for simultaneous motion and location editing in videos, leveraging motion priors and a target motion-centric video for more controllable and precise editing.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the largely unexplored task of simultaneously transforming both motion and location in videos, which is practically important but challenging.

Method: The proposed TeleMorpher framework consists of four steps: (1) disentangling the protagonist and background via pre-trained models, (2) introducing a training-free pose warping to edit the protagonist's motion, (3) injecting the warped motion video into a baseline motion editor, and (4) proposing new LPIPS-based metrics for quantitative evaluation.

Result: Experiments on in-the-wild videos and the TaiChi dataset demonstrate that TeleMorpher achieves superior performance across both quantitative and qualitative measurements, underscoring its effectiveness.

Conclusion: The authors conclude that TeleMorpher is an effective one-shot framework for simultaneous motion and location editing, achieving state-of-the-art performance and highlighting its potential for practical applications.
...
TL;DR: Vortex is a multimodal video retrieval system that integrates adaptive keyframe extraction, multimodal metadata generation, and a hybrid retrieval strategy, achieving excellent performance in the Ho Chi Minh City AI Challenge 2025.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to advance intelligent multimedia search and temporal reasoning, and to develop a robust foundation for future research in intelligent, context-aware, and interactive video retrieval.

Method: The method used in this paper involves integrating adaptive keyframe extraction, multimodal metadata generation from vision-language and speech models, and a hybrid retrieval strategy that fuses CLIP and SigLIP2 embeddings through Reciprocal Rank Fusion.

Result: The system achieved a score of 79.6/88 (90.5%) in the Preliminary Round and was further evaluated in the Final Round, achieving an 'Excellent' overall performance with 'Outstanding' results in the question-answering (QA) task.

Conclusion: The system demonstrates the complementary strengths of CLIP and SigLIP2 and confirms the effectiveness of the hybrid retrieval approach, establishing a robust foundation for future research in intelligent, context-aware, and interactive video retrieval.
...
<details>
  <summary>Details</summary>
Motivation: Existing approaches face limitations due to scarce annotated data and simplistic negative sampling in fashion domain.

Method: Integrates a multi-modal large language model (LLaVA) to generate attribute-aware triplets and introduces a two-stage fine-tuning strategy to enhance contrastive learning.
...
<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for long video understanding focus on retrieval rather than narrative structure. The authors aim to address this gap by introducing a dataset and tasks that evaluate how low-level actions form events and how events interact across time.

Method: The authors introduce NEST, a dataset of 1005 full-length movies annotated with 102 multimodal narrative events. They also present baselines for four tasks: event trigger detection (ETD), event localization (EL), event argument extraction (EAE), and event relation extraction (ERE).

Result: The benchmark is highly challenging, with ETD below 8%, EL under 6%, and EAE below 11%. However, ERE is more tractable, reaching 35.45% F1 zero-shot and 44.42% F1 after fine-tuning.

Conclusion: The paper highlights the difficulty of understanding narrative structure in long videos and presents a new dataset and baselines for future research.
...
Method: A conditional denoising diffusion model is proposed, utilizing 3D human priors (normal map and color prompt) to generate high-quality images in novel views and poses.

Result: The approach significantly outperforms previous methods on public datasets and shows better generalization ability across datasets.

Conclusion: The proposed method enables high-quality synthesis of human images in novel views and poses, including occluded/invisible parts, and demonstrates improved performance and generalization.
...
TL;DR: QueryGaussian, a training-free framework, enables efficient 3D instance retrieval from large-scale scenes using natural language prompts, reducing GPU memory usage by 70% and accelerating inference by 180x.
...
Method: QueryGaussian employs an instance-level query mechanism, leveraging pre-trained 2D vision models and a concurrent maximum-weight association strategy to lift segmentation masks into 3D.

Result: QueryGaussian matches the accuracy of state-of-the-art methods while reducing GPU memory usage by over 70% and accelerating inference by 180x, enabling expeditious instance retrieval on city-scale scenes.

Conclusion: QueryGaussian offers a scalable and efficient solution for open-vocabulary 3D instance retrieval, addressing the challenges of existing approaches and enabling applications on consumer-grade hardware.
...
TL;DR: The authors propose a framework for generating wearable adversarial patterns that can deceive UAV detectors in various physical environments with changing viewpoints, poses, and lighting conditions.


<details>
  <summary>Details</summary>
Motivation: Adversarial camouflage in the physical world, especially under UAV reconnaissance, is challenging due to targets' continuous geometric changes and extreme illumination variations.

Method: The proposed framework integrates UV-volume rendering with a diffusion-based texture generator and includes an illumination color consistency estimator and a multi-scale dynamic training strategy.

Result: The method achieves strong and stable physical attack performance while maintaining high perceptual naturalness, reducing human detection rates without introducing unnatural artifacts.

Conclusion: The authors' approach effectively generates wearable adversarial patterns for real-world scenarios, overcoming limitations of existing methods.
...
<details>
  <summary>Details</summary>
Motivation: Existing approaches for 3D scene understanding typically rely on explicit 3D inputs or introduce an additional 3D geometry encoder, which structurally decouples 3D geometric perception from 2D semantics learned via vision-language pre-training.

Method: The proposed Occ-VLM framework reconstructs 3D scene occupancy as an auxiliary geometric prior, which is used to spatially associate foreground 2D tokens with 3D space, and then decodes them by a Large Language Model (LLM) for unified scene understanding.
...
TL;DR: A new framework, HypOProto, is proposed for interpretable Left Ventricular Filling Pressure (LVFP) classification using echocardiography (echo) images, achieving state-of-the-art performance while providing transparency.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning approaches for LVFP classification are black-box and operator-dependent, motivating the need for interpretable methods that can infer LVFP directly from B-mode echo, especially in resource-limited settings.

Method: HypOProto, a hyperbolic, ordinal prototype-based framework, uses a frozen, explainable foundation model backbone and arranges prototypes along the physiological E/e' scale, with a novel Hyperbolic Prototype Angular Separation (HyperPAS) loss to enforce inter-class prototype separation.
...
Conclusion: The proposed HypOProto framework represents the first prototype-based approach for LVFP classification in echo, offering improved interpretability and transparency in clinical decision-making.
...
TL;DR: ParaScale is a module that enables faithful transfer of camera motion from a reference video to a target scene, preserving the perceived motion strength across different scales.


<details>
  <summary>Details</summary>
Motivation: The challenge of transferring camera motion from a reference video to a target video while maintaining the intended motion strength, despite differences in scene scale.

Method: ParaScale uses the Parallax Number (Pi), a dimensionless descriptor of motion strength, and operates as a plug-and-play module between pose extraction and pose injection in pose-conditioned generators.

Result: ParaScale preserves the realized parallax and reduces Parallax Consistency Error (PCE) by more than 3x across various scale regimes and backbones, without compromising visual fidelity.

Conclusion: ParaScale effectively enables scale-faithful transfer of camera motion, overcoming the challenge of differing scene scales between reference and target videos.
...
TL;DR: A new metric, Conditional-Composition Domain Match (CCDM), is proposed to evaluate the effectiveness of synthetic datasets for object detection tasks, reducing the need for computationally expensive training of downstream models.
...
Result: CCDM metric achieves a Spearman correlation of 1.0 with the downstream performance of YOLOv8 on the VisDrone-DET dataset, outperforming existing metrics for synthetic image evaluation.

Conclusion: The proposed CCDM metric provides a reliable and efficient way to evaluate the effectiveness of synthetic datasets for object detection tasks, reducing the computational cost and time required for training downstream models.
...
<details>
  <summary>Details</summary>
Motivation: Accurate segmentation of thin, tortuous structures is challenging due to low contrast, discontinuities, and class imbalance.
...
Conclusion: CSWinUNETR is a general-purpose backbone for 2D and 3D thin-structure segmentation, providing accurate and reliable results.
...
<details>
  <summary>Details</summary>
Motivation: Current 3D multimodal large language models (3D MLLMs) cannot address, name, or reason about parts of 3D objects. Prior attempts to add part-awareness have been costly in terms of parameters. The authors aim to develop a model that can efficiently and effectively reason about parts of 3D objects.

Method: The authors propose reorganizing the input token stream to make parts directly addressable through the LLM's vocabulary. They introduce a Marker-Space Refinement (MSR) module and a learnable per-region marker to condition each marker on its region's spatial statistics and adjacency neighbors.

Result: The proposed model, 3D-PLOT-LLM, achieves state-of-the-art results on several benchmarks, including PartVerse-QA, 3DCoMPaT-GrIn, and Objaverse. Specifically, it reaches caption-to-slots Jaccard 0.459 and Exact-match 13.78% on PartVerse-QA, and outperforms PointLLM, Kestrel, PARIS3D, and SegPoint on every text-output metric on 3DCoMPaT-GrIn.

Conclusion: The authors conclude that their proposed model, 3D-PLOT-LLM, can effectively reason about parts of 3D objects with a significantly smaller number of trainable parameters (under 1M) compared to prior part-aware 3D MLLMs.
...
TL;DR: The paper proposes a framework to compress event streams from event cameras into 'neural events' that are highly informative and reduce data throughput while maintaining performance in object detection and classification tasks.


<details>
  <summary>Details</summary>
Motivation: Current architectures for processing event streams from event cameras struggle to balance capturing fine-grained temporal dynamics and managing data throughput, as individual events carry minimal semantic value.

Method: The proposed framework re-tokenizes event streams into a small set of 'neural events', each representing a local spatio-temporal context window with a discrete learnable code, triggered when the code flips.

Result: Networks trained on 'neural events' achieve on-par or superior performance to state-of-the-art approaches in object detection and classification while reducing the event rate by a factor of 2.0.

Conclusion: The framework effectively compresses event streams and maintains or improves performance in downstream tasks, addressing the challenge of balancing temporal fidelity and data throughput in event camera processing.
...
TL;DR: The authors propose OTCHA, a module that refines patch tokens for multi-view classification in medical imaging, improving robustness and accuracy.


<details>
  <summary>Details</summary>
Motivation: Medical images from different views often contain irrelevant content and artifacts, which can contaminate fused representations and reduce diagnostic accuracy.

Method: OTCHA uses optimal transport to align patch tokens with learnable latent hub tokens, considering feature similarity and geometry, and gates message passing based on token-wise matching confidence.

Result: Experiments on three multi-view medical image datasets show consistent improvements over competing baselines across diverse anatomies and view configurations.

Conclusion: The proposed OTCHA module effectively refines patch tokens and improves multi-view classification in medical imaging, demonstrating its potential for clinical applications.
...
<details>
  <summary>Details</summary>
Motivation: The need for real-time deployment of streaming VideoLLMs with low query latency and high video-ingestion throughput.

Method: A stage-wise coordinated streaming framework (ViCoStream) combining chunk-wise execution, CUDA-stream overlap, and other techniques.

Result: ViCoStream achieves 134 FPS video throughput and <50 ms TTFT on a single A100 GPU with accuracy close to full-history baselines.

Conclusion: ViCoStream enables efficient and accurate real-time streaming video inference with low latency.
...
TL;DR: A novel framework, PSCT-Net, is proposed for reconstructing 3D CT scans from sparse bi-planar X-rays, reducing radiation risks in pediatric imaging.


<details>
  <summary>Details</summary>
Motivation: The motivation is to reduce radiation risks associated with CT scans in pediatric craniofacial abnormality diagnosis by reconstructing 3D CT from sparse bi-planar X-rays.

Method: The method involves a geometry-aware framework with differentiable back-projection, Attention-Guided Projection (AGP-3D) module, and Bidirectional Mamba (BiM-3D) module.

Result: The proposed framework, PSCT-Net, can alleviate depth ambiguity and improve osseous boundaries in 3D reconstructions from sparse bi-planar X-rays.

Conclusion: PSCT-Net offers a promising low-dose alternative for pediatric craniofacial abnormality diagnosis, leveraging geometry-aware modeling and advanced volumetric dependency capture.
...
TL;DR: The paper proposes a Multimodal Concept Bottleneck Model (MM-CBM) to improve the interpretability of deep learning networks, achieving up to 51.26% accuracy improvement across four benchmarks while maintaining high accuracy and interpretability.


<details>
  <summary>Details</summary>
Motivation: Existing Concept Bottleneck Models (CBMs) are limited in their ability to generalize beyond predefined classes and risk non-concept information leakage. The authors aim to address these issues and extend CBMs to CLIP.

Method: The authors propose MM-CBM, which utilizes dual Concept Bottleneck Layers (CBLs) to align both image and text embeddings into interpretable features, enabling zero-shot classification and image retrieval in an interpretable way.
...
<details>
  <summary>Details</summary>
Motivation: Scaling robot policy learning for autonomous surgery is challenging due to expensive expert demonstrations and safety risks associated with in vivo exploration.

Method: SurgVista uses Deformation Consistency Regularization and Drift Adaptation Training to mitigate failures in existing methods.
...
Conclusion: SurgVista presents a significant advancement in surgical world models, enabling more accurate and reliable autonomous surgery.
...
TL;DR: A new LRU-based restoration network with a semantic modulating unit (SMU) is proposed for single-image super-resolution, achieving a balance between performance and efficiency.
...
Method: A LRU-based restoration network with a semantic modulating unit (SMU) is proposed, which plays three key roles: LRU modulation, spatial categorization, and feature enhancement through learned prototype.
...
Conclusion: The proposed LRU-based restoration network with SMU achieves a harmonious balance between performance and efficiency in single-image super-resolution.
...
<details>
  <summary>Details</summary>
Motivation: Endoscopic video analysis is crucial for gastrointestinal diagnosis and computer-assisted interventions, but video sequences are often degraded, reducing image interpretability and disrupting downstream tasks.
...
Result: The best GPVAE variants reduced image reconstruction RMSE by 21.9% on average and up to 26.1% relative to matched VAE baselines, and reduced downstream trajectory RMSE by 12.7% on average.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of existing approaches that either rely on external tools or latent feature distillation, which lack interpretability and fine-grained geometric constraints.
...
Conclusion: The conclusion is that SpatialSV offers a promising approach to unlock the spatial intelligence of MLLMs, providing a transparent window into the model's representations and enabling scalable, interpretable spatial representation learning.
...
TL;DR: CARE, a competence-aware reward shaping framework, is proposed for adaptive reasoning length optimization in multimodal video reasoning, improving accuracy, stability, and efficiency.


<details>
  <summary>Details</summary>
Motivation: The mismatch between reinforcement learning-based methods and the model's evolving competence in multimodal video reasoning leads to inefficient exploration and redundant reasoning.

Method: CARE framework, which includes a smoothed competence estimate, progressive stages with shifting reward preferences, normalization of reasoning effort, and a posterior amplifier.

Result: CARE consistently improves reasoning accuracy, stabilizes reinforcement learning, and enhances token efficiency across multiple benchmarks, with an inverted-U trajectory of reasoning length during training.

Conclusion: The proposed CARE framework effectively adapts reasoning length to the model's competence, leading to more efficient and accurate multimodal video reasoning.
...
TL;DR: STORM, a spatial-aware token reduction framework, is proposed to improve the performance of structurally enhanced Mamba variants by maintaining structural integrity throughout the compression process.


<details>
  <summary>Details</summary>
Motivation: The spatially agnostic nature of existing reduction methods causes severe performance collapse in structurally enhanced Mamba variants.
...
Conclusion: STORM effectively addresses the performance collapse issue in structurally enhanced Mamba variants by providing explicit spatial awareness without any training.
...
TL;DR: The paper presents a method to accelerate data annotation for complex semantic segmentation problems in industrial materials science using unsupervised algorithms, reducing labeling time by 78%.
...
Method: The authors leverage unsupervised computer vision algorithms as a pre-annotation step to accelerate the labeling process for semantic segmentation tasks, specifically for annotating high-resolution images of microstructure characterization in materials science.

Result: The use of unsupervised algorithms reduces the labeling time from 170 hours to 37 hours, achieving an approximate reduction of 78%, and creates the largest public steel microstructure segmentation dataset to date.

Conclusion: The study demonstrates the effectiveness of unsupervised algorithms in accelerating the labeling process for complex semantic segmentation problems, contributing a fully annotated, high-resolution dataset and a trained Deep Learning model to the field.
...
<details>
  <summary>Details</summary>
Motivation: The authors aim to improve the accuracy of optical flow estimation by introducing a universal constraint that can be applied across different network architectures, supervision types, and datasets.

Method: The authors propose a 'triangular consistency' constraint, which composes two optical flows to induce a third flow and enforces consistency among the three. This constraint can be applied to image pairs, multiple video frames, or image pairs with synthetic transformations.

Result: The experiments show consistent improvement in optical flow estimation across supervised, unsupervised, and transfer learning settings.

Conclusion: The proposed 'triangular consistency' constraint is a universal and plug-and-play component for optical flow training that improves accuracy without adding computational cost or requiring additional annotations.
...
TL;DR: A novel framework, DiffMath, is proposed for generating handwritten mathematical expressions without explicit spatial supervision, leveraging LaTeX structure and achieving superior performance.


<details>
  <summary>Details</summary>
Motivation: The challenge of generating handwritten mathematical expressions (HMEG) due to complex 2D layouts and long-range structural dependencies, and the high annotation costs of existing methods.
...
Result: DiffMath produces structurally consistent handwritten expressions, outperforms existing methods, and improves downstream OCR models' accuracy through synthetic data augmentation.

Conclusion: DiffMath effectively addresses HMEG challenges by eliminating the need for positional supervision, leveraging LaTeX structure, and achieving superior performance.
...
TL;DR: The paper introduces Timage, a paradigm that improves multimodal large language models' spatial reasoning by overlaying a typeset query onto the image, guiding the model's focus along spatial semantics.
...
Method: The authors propose Timage, which uses a Constrained Schrödinger Bridge (cSB) to generate an overlay of the query onto the image, consisting of two stages: Region Search and Appearance Shaping.

Result: Timage, paired with a 7B backbone, outperforms larger proprietary systems and parameter-tuned baselines on the VMCBench suite, demonstrating the effectiveness of deliberate input reconstruction for multimodal reasoning.

Conclusion: The study suggests that Timage offers a powerful, architecture-neutral approach to strengthening multimodal reasoning by recasting multimodal understanding as an alignment problem solved at the input level.
...
TL;DR: SketchKeyAnime: a video diffusion framework for generating animations from sparse key-sketch inputs, achieving superior sketch fidelity and temporal coherence.
...
Method: A dual-branch conditioning mechanism with Sketch Cross Attention and Adaptive Weighted Loss.

Result: Outperforms baselines with 31.9% reduction in EDMD and 9.5% reduction in FVD.

Conclusion: SketchKeyAnime presents a promising approach for low-cost, highly controllable animation creation.
...
TL;DR: The paper proposes two fixes for latent diffusion models to improve image-to-image translation, especially for downstream perception tasks, and demonstrates significant improvements in detection performance and image quality.
...
Result: The approach improves detection mAP by up to 2x over the latent diffusion baseline, with up to 3.4x gains on small objects, while achieving state-of-the-art FID.

Conclusion: The paper concludes that the proposed approach significantly improves image-to-image translation, especially for downstream perception tasks, and highlights the importance of multi-axis evaluation.
...
TL;DR: The paper introduces a benchmark called ROSE to test the ability of multimodal large language models (MLLMs) to act on visual information under different task contexts. The results show that MLLMs struggle to adapt to changing contexts, with performance dropping by up to 44.5 percentage points.
...
Conclusion: The study reveals a distinct, model-dependent bottleneck in turning shared visual evidence into context-specific actions, highlighting the need for improving MLLMs' ability to adapt to changing contexts.
...
TL;DR: CrossFlow is a cross-space flow formulation that generates images directly from latent space without a separate decoder, achieving 1.62 FID on ImageNet-1k.


<details>
  <summary>Details</summary>
Motivation: The separation between generator and decoder in latent diffusion causes a mismatch, leading to suboptimal sample quality.

Method: CrossFlow uses a velocity-free one-step objective to map noisy latent inputs directly to pixel-space images, eliminating the need for a separate decoder.

Result: CrossFlow-XL achieves 1.62 FID on class-conditional ImageNet-1k at 256x256 with one function evaluation.

Conclusion: CrossFlow combines the efficiency of latent representations with direct pixel-space supervision, achieving high-quality image generation without a separate decoder.
...
TL;DR: Proposed a framework combining light field integration and vision-language models to recover scenes with severe occlusion, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Occlusion-robust scene recovery is challenging in natural environments with dense foreground vegetation, limiting visibility.

Method: A vision-reasoning-guided light field occlusion removal framework that integrates light field integration with vision-language models, and uses a multi-sample fusion strategy.

Result: State-of-the-art performance on synthetic and real-world datasets, with the highest average SSIM across four synthetic light field benchmark scenes and strong generalization.

Conclusion: Combining physical imaging constraints with vision-language reasoning enables robust perception under severe occlusion, applicable to search-and-rescue and exploratory robotic navigation.
...
TL;DR: QG-MIL, a gated transformer aggregator, addresses attention concentration in medical imaging by introducing four architectural components, outperforming baselines across six benchmarks.
...
Result: QG-MIL outperforms leading baselines on all six benchmarks, with an average improvement of +6.1 mean macro F1 points.

Conclusion: QG-MIL provides more consistent cross-domain performance and tighter variance, with more distributed instance weighting, confirmed by attention overlays and attention mass analysis.
...
TL;DR: Proposed a Reliability-Aware Open-Vocabulary Change Detection (ReA-OVCD) framework for flexible and reliable land cover change detection using arbitrary text prompts.


<details>
  <summary>Details</summary>
Motivation: Existing methods suffer from a trade-off between instance-level comparison and direct pixel comparison, leading to unreliable results with boundary artifacts.

Method: The ReA-OVCD framework uses a collaborative refinement strategy with a Semantic Change Reasoning (SCR) module and a Boundary-aware Change Refinement (BCR) module to ensure reliability and detailed localization.

Result: The proposed method consistently outperforms state-of-the-art approaches across multiple datasets, achieving F1 improvements of 2.13% to 9.75% with higher computational efficiency.

Conclusion: The ReA-OVCD framework provides a reliable and efficient solution for open-vocabulary change detection, addressing the limitations of existing methods and demonstrating superior performance.
...
TL;DR: PU-UNet, a U-Net variant with product-unit residual blocks, improves medical image segmentation accuracy with negligible computational overhead.


<details>
  <summary>Details</summary>
Motivation: Dense prediction networks often rely on additive feature transformations, which may not effectively model higher-order feature interactions. Product units offer a way to model multiplicative interactions explicitly but are often unstable.

Method: The proposed PU-UNet integrates stable product-unit residual blocks into low-resolution stages of a U-Net for medical image segmentation. It uses smooth positivity mapping and log-domain clipping for stable multiplicative feature learning.

Result: PU-UNet achieves Dice scores of 0.942, 0.959, and up to 0.925 on ISIC 2018, Kvasir-SEG, and BUSI datasets, respectively. It consistently improves Dice and IoU scores compared to a Residual U-Net baseline with negligible changes in parameters, FLOPs, and inference latency.

Conclusion: Stable product-unit residual learning can effectively enhance U-Net-style segmentation networks with explicit multiplicative interactions, leading to improved accuracy in medical image segmentation.
...
TL;DR: FUSE, a frequency-domain framework, improves multi-modal Re-Identification (ReID) by focusing on mid and high-frequency structures, leading to better spectral representations and cross-modal alignment.


<details>
  <summary>Details</summary>
Motivation: Existing multi-modal ReID methods emphasize low-frequency cues, overlooking important mid and high-frequency structures that encode geometric, textural, and identity-discriminative details.
...
TL;DR: A new framework called 3DG-VLN is proposed for precise target reaching in UAV navigation using vision-language guidance, achieving a 13.82% improvement in success rate.


<details>
  <summary>Details</summary>
Motivation: Current UAV Vision-Language Navigation (UAV-VLN) methods evaluate target discovery and approach jointly, making it hard to assess a UAV's ability to accurately ground a visible target and translate vision-language evidence into precise 3D motion.

Method: The 3DG-VLN framework uses dynamic 3D direction cues to enhance visual grounding and spatial direction alignment for precise target reaching, processing high-resolution front-view and downward-view observations to preserve fine-grained visual and geometric details.

Result: The proposed 3DG-VLN framework outperforms competitive UAV-VLN baselines, achieving a 13.82% improvement in success rate, and shows potential for practical see-and-reach navigation in real-world trials.

Conclusion: The 3DG-VLN framework and the accompanying high-resolution benchmark enable more diagnostic evaluation of terminal reaching ability in UAV navigation, providing a new approach for precise target reaching.
...
TL;DR: A novel variable-length tokenizer that merges tokens to improve the quality-compute trade-off in diffusion models for visual synthesis.


<details>
  <summary>Details</summary>
Motivation: The fixed compression ratio of tokenizers in Latent Diffusion Models (LDMs) constrains their quality-compute trade-off. Variable-length tokenizers (VLTs) can adaptively compress, but conventional VLTs have issues with representational alignment across lengths.

Method: A merging-based variable-length tokenizer that modulates length by merging tokens, enabling direct cross-length representation alignment. Learnable global merging is introduced to ensure compatibility with diffusion transformers.

Result: The proposed tokenizer integrated with a diffusion transformer achieves a superior gFID-compute trade-off compared to prior VLT methods on ImageNet 256$	imes$256 generation.

Conclusion: The proposed merging-based variable-length tokenizer improves the quality-compute trade-off in diffusion models for visual synthesis, enabling flexible balancing of quality and compute.
...
TL;DR: The paper compares two integration paradigms for visual tokens in Large Language Models (LLMs) and finds that the integration method affects the transformation of visual information and performance across tasks.


<details>
  <summary>Details</summary>
Motivation: The integration architecture of visual tokens in LLMs and its effect on visual information transformation and interaction with the language space is underexplored.
...
Result: The integration paradigm affects the transformation of visual tokens, capturing different frequency characteristics of the visual signal, and ultimately affects performance across tasks.

Conclusion: The quality of visual representations at each layer, not just attention allocation, drives performance, and the integration paradigm determines what visual features the VLM can utilize effectively.
...
TL;DR: A unified controllable video world model that generates videos with precise camera and object control while transferring scenes into diverse target weather states.
...
Conclusion: The proposed Holo-World model enables controllable video generation with unified control over camera, object, and weather, achieving state-of-the-art results in weather-state generation.
...
TL;DR: EventVLA, a new framework for robotic manipulation, improves long-horizon tasks by selectively storing and predicting sparse visual evidence, outperforming state-of-the-art methods by 40%.


<details>
  <summary>Details</summary>
Motivation: Standard Vision-Language-Action policies struggle with long-horizon tasks due to memory bottlenecks, especially when task-relevant cues are occluded or unobservable.
...
Conclusion: EventVLA effectively addresses memory limitations in long-horizon robotic manipulation tasks by selectively storing and predicting sparse visual evidence.
...
Result: MakeupMirror improves facial recognition similarity by 60%, reduces skin tone difference by 50%, and achieves an expert acceptance rate of 94% with a latency of 0.7s.

Conclusion: MakeupMirror achieves significant progress in makeup transfer, preserving facial features and skin tone, and enabling high-quality virtual try-on for makeup shopping.
...
TL;DR: The study examines the impact of stitching artifacts on generative models for style-transfer in biomedical imaging, specifically in cryo-electron microscopy datasets.


<details>
  <summary>Details</summary>
Motivation: To investigate the effect of stitching artifacts on generative models for style-transfer and their impact on downstream segmentation tasks.

Method: The study uses cycleGAN models with three stitching approaches and two patch dimensionalities (2D vs 3D) on cryo-electron microscopy datasets.

Result: The results show that FID scores may not detect subtle stitching artifacts, 3D models marginally outperform 2D models but with high computational cost, and 2D models train more stably.
...
TL;DR: A novel benchmark called WeGenBench is proposed for evaluating text-to-image generation models, providing a comprehensive and multi-perspective assessment of their capabilities.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks struggle to accurately measure performance across multiple dimensions and fail to reveal inherent deficiencies of models in specific categories.
...
Conclusion: WeGenBench provides a comprehensive and accurate evaluation of text-to-image generation models, revealing their limitations and deficiencies, and facilitating the development of improved models.
...
TL;DR: A new framework for LiDAR-camera calibration that preserves metric geometry and improves accuracy.
...
Method: The proposed framework aggregates multi-view LiDAR observations for dense depth supervision and blocks photometric gradients from updating Gaussian spatial parameters.
...
Conclusion: The proposed framework improves the accuracy of LiDAR-camera calibration by preserving the metric geometry of the Gaussian proxy.
...
TL;DR: The paper proposes EFIQA, a framework for image quality assessment that doesn't require labeled data and provides spatial quality maps.


<details>
  <summary>Details</summary>
Motivation: Current deep learning-based image quality assessment methods have limitations in generalization and explainability due to reliance on dataset-specific quality labels.

Method: The proposed EFIQA framework uses a two-stage approach: (1) training an unsupervised anomaly detector via masked anatomical inpainting, and (2) distilling prior knowledge into a shallow adapter for precise quality maps.

Result: The label-free approach achieves better performance and explainability compared to supervised methods across benchmarks with different quality criteria.

Conclusion: The EFIQA framework offers a promising solution for image quality assessment with improved generalization and explainability, making it suitable for real-world applications.
...
TL;DR: Introduced FrozenDrive, a controllable generative framework for autonomous driving that preserves pre-trained knowledge while achieving strong consistency and fidelity.


<details>
  <summary>Details</summary>
Motivation: Existing methods for generating synthetic data for autonomous driving struggle with multi-view and temporal consistency, and often require fine-tuning or added layers which can erode pre-trained knowledge.

Method: FrozenDrive conditions on rich driving-stack signals and text prompts, and introduces knowledge-preserving spatio-temporal attention to impose cross-view alignment and temporal coherence.

Result: FrozenDrive synthesizes globally coherent multi-view driving scenes from text, particularly under adverse and rare conditions, and surpasses prior baselines.

Conclusion: FrozenDrive augmented data significantly improves AD models' performance, especially at night and in rain, demonstrating stronger robustness when trained with scenario-targeted data.
...
TL;DR: The paper proposes a new generative model called Pixel-Level Residual Diffusion Transformer (PRDiT) for synthesizing high-quality 3D medical volumes at voxel-level, outperforming state-of-the-art models.
...
Method: PRDiT uses a two-stage training architecture: a local denoiser (MLP-based blind estimator) on 3D patches and a global residual diffusion transformer with memory-efficient attention.
...
Conclusion: PRDiT offers a scalable and effective solution for generating high-quality 3D medical volumes, simplifying optimization and preserving subtle structures.
...
TL;DR: The paper describes a 4th-place entry in the ICRA 2026 GOOSE 2D Fine-Grained Semantic Segmentation Challenge, achieving 69.73% mIoU on a 1,815-image test set using a modified Segment Anything Model 3 (SAM3).


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the performance of fine-grained semantic segmentation models, specifically in the context of the ICRA 2026 GOOSE 2D challenge.

Method: The method involves adapting the image encoder of SAM3 with a lightweight decoder, and introducing three techniques: self-distillation using SAM3 as a teacher, image-level multi-scale test-time augmentation, and aggressive photometric distortion.

Result: The result is a composite mean Intersection-over-Union (mIoU) of 69.73% on the official 1,815-image test set, achieving 4th place in the challenge.

Conclusion: The conclusion is that the proposed approach, which leverages SAM3 and introduces several novel techniques, achieves a high level of performance in fine-grained semantic segmentation, and identifies aggressive photometric distortion as a key source of improvement.
...
<details>
  <summary>Details</summary>
Motivation: To produce 3D meshes with structured, artist-like connectivity from input geometry conditions such as signed distance fields.

Method: A generative approach using a latent flow-matching model to synthesize a nearest-vertex vector field (NVF) and guide mesh simplification.

Result: TriFlow achieves stronger generalization, improved topology quality, 90% lower Chamfer Distance, and an 8x speedup compared to state-of-the-art approaches.

Conclusion: TriFlow enables the generation of compact 3D meshes with artist-like triangle topology directly from input geometry conditions.
...
TL;DR: The paper proposes a method called Sparse frame Annotation VIS (SA-VIS) that achieves high performance on video instance segmentation (VIS) tasks with sparse frame annotations, reducing the need for dense annotations.


<details>
  <summary>Details</summary>
Motivation: The current VIS methods require densely annotated frames during training, which is expensive in terms of compute and annotation. The authors aim to solve this problem by proposing a method that can effectively model instances and their evolution in videos without requiring densely annotated frames.

Method: The authors propose a simple and effective module called Past-frames Feature Propagation (PFP) that aggregates low-dimensional features from the image encoder of multiple frames. They also use light-weight frame-specific Instance Queries. The proposed method is called Sparse frame Annotation VIS (SA-VIS).

Result: The proposed method SA-VIS significantly improves performance over its baseline on YouTube-VIS 2019/2021/2022 and Occluded VIS (OVIS) datasets. It achieves a mere 0.4% drop in performance when using annotations for only 1/5 of the images in the dataset. SA-VIS also shows an over 1% improvement in AP on the state-of-the-art in a limited annotations scenario.

Conclusion: The paper concludes that the proposed method SA-VIS effectively bridges the gap in accuracy between training on sparsely and densely annotated video sequences, achieving high performance on VIS tasks with sparse frame annotations.
...
<details>
  <summary>Details</summary>
Motivation: The complexity of oropharyngeal anatomy and heterogeneous tumor appearance on imaging make manual segmentation and clinical outcome prediction challenging, highlighting the need for automated analysis.
...
Conclusion: The HECKTOR 2025 challenge established a comprehensive benchmark for automated head and neck cancer analysis, with submitted methodologies showing promising results for clinical translation in oncology workflows and decision support systems.
...
<details>
  <summary>Details</summary>
Motivation: The ability of text-to-image models to generate realistic faces from names raises privacy concerns, and there is a need for a method to distinguish between memorized and fabricated faces.
...
Result: Experiments on state-of-the-art text-to-image models show that the proposed probe can substantially predict identity memorization and separate memorized from unrecognized names.

Conclusion: The proposed method provides a useful tool for detecting memorization in text-to-image models, with potential applications in mitigating privacy concerns.
...
TL;DR: A new framework called ARTEMIS is proposed for imperfectly supervised video polyp segmentation, which uses agent-guided reliability-aware temporal mask evolution to improve performance.


<details>
  <summary>Details</summary>
Motivation: Imperfectly supervised video polyp segmentation is challenging due to weak contrast, ambiguous boundaries, motion blur, and specular highlights, with sparse pixel-level guidance. Existing methods have limitations, such as geometry-degraded masks and underutilization of temporal consistency.

Method: The ARTEMIS framework initializes coarse masks from available supervision, uses a debate-and-judge vision-language agent to select reliable temporal anchors, and trains the segmenter using temporal reliability-aware robust learning.
...
Conclusion: The proposed ARTEMIS framework addresses the challenges of imperfectly supervised video polyp segmentation by leveraging agent-guided reliability-aware temporal mask evolution, leading to improved performance.
...
TL;DR: The paper explores the limitation of Multimodal Large Language Models (MLLMs) in understanding negation in Remote Sensing tasks and proposes a novel method, NeFo, to improve this understanding.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the underexplored ability of MLLMs to comprehend negation, which is crucial for real-world applications such as emergency response.

Method: The paper introduces RS-Neg, a benchmark for evaluating negation understanding, and proposes NeFo, a test-time learning method that incorporates the logical role of negation into model optimization.
...
Conclusion: The paper concludes that MLLMs have limitations in understanding negation, but proposes a novel method, NeFo, to address this gap and improve the performance of MLLMs in Remote Sensing tasks.
...
TL;DR: The paper introduces a method to train vision-language models for radiology without manual spatial annotations, achieving competitive results on visual question answering benchmarks.
...
Result: The model achieves competitive results on external VQA benchmarks (Slake, VQA-RAD) and shows improved open-ended VQA performance when adding clinical data to the training mixture.

Conclusion: The paper concludes that it is possible to train VLMs for radiology without manual spatial annotations, achieving spatially verifiable outputs at no cost to VQA performance.
...
TL;DR: A new framework called HilDA is proposed for pre-training LiDAR models using Vision Foundation Models (VFMs) as teachers, achieving state-of-the-art results in various autonomous driving tasks.


<details>
  <summary>Details</summary>
Motivation: The scarcity of annotated data for autonomous driving and the limitations of current knowledge distillation approaches that treat VFMs as black-box teachers.

Method: HilDA, a self-supervised pretraining framework that uses hierarchical distillation, global context distillation, and a temporal occupancy diffusion objective.

Result: State-of-the-art results on cross-modal distillation benchmarks and outperforming prior distillation approaches on 3D object detection, scene flow, and semantic occupancy prediction.

Conclusion: HilDA effectively leverages VFMs for camera-to-LiDAR knowledge distillation, capturing semantic and geometric information needed for driving tasks.
...
TL;DR: The paper introduces DO-ALL, a framework for continual test-time adaptation that uses dataset distillation to retain source information in a compact form, enabling stable and continuous adaptation without raw source data.
...
Method: The method is DO-ALL (Distill Once, Adapt Life-Long), a plug-and-play framework that uses Dataset Distillation (DD) to produce synthetic distilled anchors summarizing the source distribution, and then matches each target sample with its most semantically aligned anchor for CTTA.

Result: The result is that DO-ALL consistently improves long-term robustness across CIFAR100-C, ImageNet-C, and the CCC benchmark, demonstrating the potential of leveraging DD for stable and continuous adaptation.

Conclusion: The conclusion is that DO-ALL provides a stable reference for CTTA via source replay, representation alignment, and manifold-smoothing regularization, enabling stable and continuous adaptation without retaining raw source data.
...
TL;DR: This paper proposes a method to assess drawing skills by comparing hand-drawn images with original templates using computer vision.


<details>
  <summary>Details</summary>
Motivation: The need for a comprehensive assessment of drawing skills, which can be tedious and overwhelming with traditional methods.

Method: Using computer vision applications, specifically SIFT feature and Siamese network, to measure image similarity between hand-drawn images and original templates.

Result: The results indicate that it is feasible to assess art skill levels, with SIFT-based key point matching being more effective in detecting drawing skills.

Conclusion: The proposed method provides a feasible and effective way to assess drawing skills using computer vision.
...
TL;DR: DeepForestVisionV2, an upgraded camera-trap classification tool, improves accuracy and robustness in African tropical forests, especially in diverse habitats and with human-related confounders.


<details>
  <summary>Details</summary>
Motivation: The original DeepForestVision tool was not sufficient for handling diverse environments and species, particularly in open habitats and with arboreal or semi-aquatic taxa.

Method: DeepForestVisionV2 expands the prediction classes from 35 to 64, including more animal classes and human-related categories, and is trained on a large dataset of 1,535,010 photographs and 243,354 videos.

Result: DeepForestVisionV2 achieves high accuracy (0.86) and F1 score (0.82) on a validation set and improves performance on deployment benchmarks, identifying more taxa and reducing false alarms.
...
TL;DR: A new video diffusion framework for cinematic compositing that models character-environment interactions and lighting harmonization.
...
Conclusion: The proposed framework effectively addresses the challenges of cinematic compositing by modeling complex character-environment interactions and lighting harmonization.
...
TL;DR: This paper evaluates biases in text-to-image models, particularly in occupation-related image generation, and emphasizes the need for human feedback to develop fairer models.
...
Result: The findings reveal systematic biases in text-to-image models, with established evaluation metrics in partial correlation with subjective user ratings. The authors emphasize the need for including human preferences to develop fairer and more inclusive text-to-image models.

Conclusion: The research highlights the importance of incorporating human feedback in the development of text-to-image models to mitigate biases and create more inclusive models.
...
<details>
  <summary>Details</summary>
Motivation: Vision-language models often struggle with evidence-intensive tasks because they fail to properly utilize decisive visual evidence, which can be small and localized. Previous methods to improve grounding without retraining are open-loop and lack verification mechanisms.

Method: The authors propose SPOT-E, which uses low-entropy anchors and an entropy-shaping objective to reduce answer uncertainty while preserving high-confidence tokens. This is achieved through light-weight tuning based on Group Relative Policy Optimization (GRPO).
...
TL;DR: The authors propose a single-stage framework called SSHR for weakly supervised semantic segmentation in computational pathology, which outperforms multi-stage methods and reduces training duration.


<details>
  <summary>Details</summary>
Motivation: Existing WSSS methods in computational pathology have limitations due to their multi-stage paradigm, including high computational costs and error propagation.

Method: The authors propose the Single-Stage Hierarchical Rectification (SSHR) framework, which uses a Hierarchical Feature Rectification Module (HFRM) to purify intermediate feature representations during the forward pass.
...
Conclusion: The proposed SSHR framework offers a simple yet highly effective approach to address the challenges in WSSS, minimizing computational overhead and accelerating clinical translation for large-scale histopathology workflows.
...
<details>
  <summary>Details</summary>
Motivation: Existing models for salient object detection fail to explore contextual information and the depth of the entire architecture, despite Mamba-based models offering advantages in modeling long sequences.

Method: The proposed U^2Mamba network utilizes multiscale Mamba U-blocks (MMUBs) and a nested U-structure to integrate various receptive fields, along with a hierarchical training supervision method.

Result: U^2Mamba achieves highly competitive performance against state-of-the-art methods in salient object detection.

Conclusion: The proposed U^2Mamba model presents a powerful and innovative approach for salient object detection, demonstrating improved performance through its unique architecture and training method.
...
<details>
  <summary>Details</summary>
Motivation: Few-shot anomaly detection is challenging due to limited training data, and existing methods have limitations in handling cross-modal misalignment and false-positive rates.

Method: The CMDS-AD framework uses a LoRA-guided diffusion model to generate diverse RGB samples, a pre-trained diffusion model as a normal estimator, and a Coordinate-Aware Hierarchical Feature Mapper to adaptively align cross-modal semantics.
...
TL;DR: CUPID, a POI video deepfake detector, is proposed, combining UV texture maps and Masked Autoencoder (MAE) for robust, efficient, and interpretable deepfake detection.
...
Result: CUPID outperforms current state-of-the-art on most datasets, achieves best overall robustness against strong downscaling and compression, and provides substantially faster inference.

Conclusion: CUPID offers a robust, efficient, and interpretable solution for POI video deepfake detection, addressing the limitations of current methods.
...
Conclusion: The proposed GEN-Guard framework provides a practical solution for detecting and correcting generalization failures in federated surgical AI, strengthening the reliability of FL for real-world surgical deployment.
...
TL;DR: The paper introduces PRISM, a method for evaluating video generation that decodes preference signals from noisy latents, achieving state-of-the-art accuracy and noise-robustness.


<details>
  <summary>Details</summary>
Motivation: The current paradigm of evaluating video generation with clean, pixel-based reward models has limitations, including disconnecting evaluation from the noisy diffusion process and incurring massive VAE decoding costs.
...
Conclusion: The paper challenges the current paradigm of evaluating video generation and introduces PRISM, which can inherently discriminate preferences directly from noisy latents, enabling self-improving video backbones.
...
TL;DR: The paper proposes a post-hoc score calibration method called Reliability-Aware Prototype Calibration (RPC) to improve pose-flow video anomaly detectors in a frozen-detector setting.
...
Method: A region-based graph learning framework that models lesions as graphs of spatially coherent superpixel regions, encodes inter-regional geometry, and integrates demographic/clinical variables using a graph transformer and attention-driven propagation.
...
TL;DR: FlowBender is a closed-loop framework that utilizes alignment error to improve conditional diffusion and flow models, achieving better fidelity and plausibility in image-to-image translation, restoration, and 3D mesh texturing tasks.
...
Method: FlowBender introduces a closed-loop framework that treats alignment error as a first-class input, training the network to learn a correction policy conditioned on inference-time feedback.

Result: FlowBender consistently outperforms standard supervised baselines, alignment-loss-augmented training, and state-of-the-art inference-time guidance, improving fidelity and plausibility simultaneously.

Conclusion: FlowBender provides a more effective approach to conditional diffusion and flow models by utilizing alignment error, achieving better results in various tasks.
...
TL;DR: The paper proposes QK Product Steering, a method to reduce object hallucination in vision-language models by editing the query-key product in the attention mechanism.
...
Method: QK Product Steering, a data-free, training-free method that edits the query-key product in the attention mechanism by suppressing dominant singular modes.

Result: QK Product Steering achieves an average relative CHAIR$_s$ reduction of $4.0%$ across three GQA-based VLMs.

Conclusion: QK Product Steering offers a simple alternative to decoding-time mitigation, requiring no additional data, fine-tuning, or inference-time overhead while largely preserving general multimodal capability.
...
<details>
  <summary>Details</summary>
Motivation: Reliable localisation of the neonatal face is crucial for various non-contact assessments, but challenges persist due to cluttered backgrounds, illumination changes, and poor lighting conditions.

Method: One-stage YOLOv11m-based model, trained on multiple publicly available datasets and fine-tuned on a neonatal research dataset.

Result: Achieved AP50 of 0.87 before fine-tuning and 0.96 after clinical-domain adaptation, surpassing state-of-the-art general face detectors.

Conclusion: The proposed model shows promising results for face detection in neonatal clinical environments, highlighting the need for publicly available neonatal datasets to further progress in this field.
...
TL;DR: The paper introduces PCFootprint, a large-scale public dataset for building footprint extraction from airborne laser scanning point clouds, and presents benchmarks for mainstream methods.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the limitations of image-based methods for building footprint extraction, which are susceptible to occlusions, perspective distortions, and lack of elevation information.

Method: The authors present a new dataset, PCFootprint, comprising 33,000 tiles of airborne laser scanning point clouds with vectorized footprints, and evaluate mainstream methods on this dataset.

Result: The experimental results reveal significant challenges in building footprint extraction, including high intra-class variance, data imbalance, and noise across complex geospatial environments.

Conclusion: The authors believe that PCFootprint will advance future research in building modeling, urban scene understanding, and geospatial analysis.
...
TL;DR: The paper audits training-free detectors of AI-generated images, highlighting issues with implementation details, score direction, and dataset format bias.


<details>
  <summary>Details</summary>
Motivation: To evaluate and compare training-free detectors of AI-generated images under a controlled protocol, highlighting potential issues and biases.

Method: The authors audit two representative training-free scores (AEROBLADE-style and RIGID-style) and a naive feature-kNN control on a 1,500-image benchmark spanning seven generators and JPEG compression.

Result: The audit reveals three cautionary findings: implementation details significantly impact results, score direction is not a property of the method but its hyperparameters, and dataset format bias inflates robustness claims.

Conclusion: The study emphasizes the need for careful evaluation and combination of training-free detectors, as naive fusion does not outperform the best single score, and direction-aware combination is required to exploit complementarity.
...
Conclusion: The paper presents a novel framework for style-content dual-reference generation that addresses the challenges of balancing content fidelity, style alignment, and leakage suppression, and demonstrates its effectiveness through extensive experiments.
...
TL;DR: The paper introduces S-Agent, a spatial tool-use agentic paradigm for understanding and reasoning over continuous multi-view images and videos, which improves spatial intelligence in VLMs.
...
Method: The S-Agent paradigm formulates spatial reasoning as spatio-temporal evidence accumulation, using a semantic planner, spatial tools, and experts to ground objects in 2D and 3D, and a temporal memory mechanism for evidence integration across frames and reasoning steps.

Result: S-Agent consistently improves open-source and closed-source VLMs in a training-free manner, and supervised fine-tuning yields S-Agent-8B, a compact spatial agent that surpasses similar-scale baselines and performs comparably to advanced closed-source models.

Conclusion: The S-Agent paradigm offers a significant advancement in spatial intelligence, enabling more accurate and comprehensive understanding and reasoning over continuous multi-view images and videos.
...
TL;DR: A new dataset, SARLO-80, is introduced, containing high-resolution SAR and optical imagery with natural-language descriptions, enabling multimodal learning and benchmarking.


<details>
  <summary>Details</summary>
Motivation: The lack of large-scale public datasets combining high-resolution SAR, aligned optical imagery, and natural-language descriptions limits multimodal learning, particularly for physically grounded applications.

Method: The dataset is built from open-access Umbra spotlight acquisitions, standardized to an 80cm slant-range grid, and tiled into 1024x1024 patches. Each SAR patch is aligned with a high-resolution optical tile and paired with three caption variants.

Result: The SARLO-80 dataset contains 119,566 triplets covering 257 locations across 72 countries, with fixed train/validation/test splits and baseline code for reproducible benchmarks.

Conclusion: The SARLO-80 dataset provides a valuable resource for multimodal learning and benchmarking, enabling the development of more accurate and physically grounded models.
...
TL;DR: VisDom, a learning-free geometric constraint, improves sparse novel view synthesis by integrating a minimum multi-view visibility requirement into classical visual hull reconstruction, enhancing object-centric reconstruction from few input views.
...
Conclusion: VisDom is a simple, domain-agnostic complement to existing approaches, requiring only silhouettes and introducing no learned parameters, making it effective in improving sparse novel view synthesis.
...
<details>
  <summary>Details</summary>
Motivation: To investigate the reproducibility of FID scores, which are widely used to evaluate image generation models, but are typically reported as a single number from a single trained model and sampling seed.
...
Result: The study finds that retraining a model with a different seed affects FID more than resampling from a fixed network, and that factors like random initialization, data ordering, and per-step Gaussian noise contribute to this variability. Increasing compute or model size has limited impact on reducing variability.

Conclusion: The authors recommend a new FID evaluation protocol that involves evaluating under per-cell optimal guidance, treating small FID gaps as inconclusive, and reporting error bars over multiple training seeds rather than a single FID number.
...
TL;DR: The Caltech Tennis Dataset (CalTennis) is a large-scale video benchmark for monocular-to-3D pose estimation, comprising 11 million frames of tennis footage from 40 players, captured with multiple synchronized cameras.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to create a large-scale benchmark for evaluating monocular-to-3D pose estimation in the wild, specifically for expert athletic motion, and to identify areas for improvement in pose estimation and action analysis.

Method: The authors propose a simple, standardized protocol for data collection, automated video calibration, and synchronization, and benchmark state-of-the-art monocular-to-3D pose methods on the CalTennis dataset.

Result: The results show that while 3D joint angle recovery is accurate, models struggle to estimate depth and foot contact consistently, and the proposed metrics (footwork and stability) reveal previously underexplored failure modes.

Conclusion: The CalTennis dataset and proposed metrics provide a valuable resource for improving pose estimation and action analysis, and highlight opportunities for improvement in these areas.
...
TL;DR: A new framework called Spatially Speculative Decoding (SSD) is introduced to improve the efficiency of autoregressive image generation models by predicting tokens in 2D spatial correlation.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome the computational bottlenecks in autoregressive models caused by treating images as 1D sequences, which discards the intrinsic 2D spatial locality of visual signals.

Method: The method proposed is Spatially Speculative Decoding (SSD), which predicts the adjacent horizontal token and the token directly below it, capitalizing on 2D spatial correlation.
...
<details>
  <summary>Details</summary>
Motivation: The authors argue that current world models lack the ability to maintain an internal world state that evolves over time, decoupled from observation, which is a crucial step towards artificial general intelligence.

Method: The authors introduce WRBench, a systematic diagnostic benchmark that evaluates world models by treating camera motion as an intervention on observability and checking whether the scene stays continuous and identifiable while in view, and whether a returning target remains consistent with the event that was set in motion.

Result: The authors found that current systems maintain the observed world as a tracking shot, resuming a returning target in the state at which it was abandoned rather than advancing the event while it went unseen, across 9,600 videos from 23 models spanning four control paradigms.

Conclusion: The authors conclude that the stability of the physical state kernel and the consistency of worldlines under viewpoint intervention should become first-class objectives of world-model design, so that a world model captures how the world will unfold rather than how the next frame appears.
...
Method: The 'thinking in boxes' interface uses 3D boxes as structured specifications for image editing, with a depth-aligned planar floor as a global reference frame, and an image generator conditioned on this structure.
...
TL;DR: A new framework for egocentric video understanding called UNIEGO is proposed, which uses a hierarchical multi-teacher distillation approach to integrate knowledge from multiple viewpoints, modalities, and foundation models, achieving state-of-the-art performance on three challenging benchmarks.


<details>
  <summary>Details</summary>
Motivation: The limitation of egocentric video understanding due to the narrow perspective of wearable cameras, which cannot capture the full richness of human action, motivates the need for a more expressive representation that subsumes complementary knowledge across viewpoints, modalities, and foundation models.

Method: A hierarchical multi-teacher distillation framework is introduced, which uses representation-specific Proxy models to translate diverse teacher knowledge into a homogeneous egocentric space, followed by a Selective Proxy Distillation (SPD) stage that adaptively selects reliable supervision and suppresses erroneous signals.

Result: UNIEGO achieves state-of-the-art performance across three egocentric video understanding tasks - action recognition, video retrieval, and action segmentation on three challenging ego-exo benchmarks, outperforming naive multi-teacher distillation baselines.

Conclusion: The proposed UNIEGO framework demonstrates that structured, proxy-mediated knowledge transfer yields richer and more discriminative egocentric representations, enabling more accurate understanding of egocentric videos.
...
TL;DR: Introduced TimeProVe, a cost-efficient hybrid framework for temporally grounded reasoning in long videos, outperforming baselines while reducing computational cost.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for Long Video Question Answering (LVQA) are either computationally expensive or miss temporally localized evidence, motivating the need for a cost-efficient and effective framework.

Method: TimeProVe uses lightweight modules to generate hypotheses and invokes expensive vision-language models only for targeted verification, leveraging the Action-based Candidate Evidence (ACE) module and OpenTSUBench (OTB) benchmark.

Result: TimeProVe outperformed the strongest baseline on OTB by 7.3%, reduced VLM calls by 75%, and inference cost by 93%, achieving competitive or state-of-the-art results on other benchmarks.

Conclusion: TimeProVe offers a cost-efficient and effective solution for temporally grounded reasoning in long videos, demonstrating significant improvements over existing approaches.
...
TL;DR: A novel framework for generating text-driven 3D visual illusions that appear different from various viewing angles, achieving high realism and efficiency.


<details>
  <summary>Details</summary>
Motivation: The challenge of creating 3D visual illusions with different semantics from various viewing angles, while existing methods are slow, produce oversaturated colors, or have visible unnatural seams.
...
Result: Highly realistic, dual-semantic 3D illusions generated in just 3-5 minutes, outperforming existing methods in geometric integrity, semantic recognizability, and efficiency.

Conclusion: The proposed framework offers a fast, training-free solution for generating high-quality 3D visual illusions with different semantics from various viewing angles.
...
<details>
  <summary>Details</summary>
Motivation: Standard auditing methods for LLM bias are limited, as they rely on single output inspection or static metrics, failing to capture biases hidden in lower-probability generation branches.
...
Conclusion: TreeTracer is a effective visual analytics tool for evaluating LLM bias, enabling direct comparison between semantic contexts and systematic bias detection, and reducing the risk of misinterpreting the presence of bias.
...
TL;DR: This study explores using Google's large language models (LLMs), Gemini and Gemma, to automate detecting EQ-5D data in PubMed abstracts, proposing a multi-phase framework for improved accuracy and reliability.


<details>
  <summary>Details</summary>
Motivation: The rapid increase in scientific publications makes manual study screening in systematic literature reviews (SLRs) inefficient and inconsistent, particularly for studies reporting health-related quality-of-life results like EQ-5D data.
...
Result: The weighted ensemble of top LLMs achieved a 0.74 weighted F1-score and 0.74 accuracy, improving balance between precision and recall, and providing greater reliability and interpretability.
...
TL;DR: The paper studies cross-lingual transfer of large language models fine-tuned on Arabic and evaluated on Semitic and non-Semitic languages, finding no evidence of Semitic-specific transfer.


<details>
  <summary>Details</summary>
Motivation: To investigate cross-lingual transfer of large language models across languages, particularly Semitic languages.

Method: Fine-tuning seven large language models on Arabic and evaluating zero-shot reading comprehension on Semitic and non-Semitic languages.
...
TL;DR: The study investigates the bottleneck in translating sequential programming priors into parallel temporal logic for large language models (LLMs) in hardware design, proposing a new error taxonomy and revealing persistent knowledge gaps and limitations in current LLMs.


<details>
  <summary>Details</summary>
Motivation: The crucial bottleneck in translating sequential programming priors into parallel temporal logic for LLMs in hardware design and the need to investigate and address the challenges in the current LLM-based hardware generation pipeline.

Method: Introduction of a new error taxonomy grounded in problem solvability, inspired by cognitive theory, and evaluations on the VerilogEval benchmark.

Result: The study reveals a strict empirical ceiling on the VerilogEval benchmark with frontier models plateauing at a 90.8% initial pass rate, and identifies persistent knowledge gaps and limitations in current LLMs.

Conclusion: Addressing challenges in the current LLM-based hardware generation pipeline requires more studies in model reasoning rather than alignment interventions, as alignment techniques merely teach models to compile and have limitations in RTL coding capacity.
...
TL;DR: The paper introduces DeepSeek-V4, a series of Mixture-of-Experts language models with 1.6T and 284B parameters, supporting a context length of one million tokens and showcasing improved efficiency and performance.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to present a new series of language models, DeepSeek-V4, that can efficiently handle long-context scenarios and improve performance on various tasks.
...
Conclusion: The paper concludes that the DeepSeek-V4 series offers significant improvements in efficiency and performance, making long-horizon tasks and further test-time scaling more feasible, and provides the model checkpoints for public use.
...
TL;DR: The paper analyzes In-Context Learning (ICL) in Diffusion Large Language Models (dLLMs), finding that query position significantly impacts generation quality, and proposes a novel metric and adaptive routing strategy to improve ICL in dLLMs.
...
TL;DR: A new method called Causal Attribution Pruning (CAP) is introduced to improve the efficiency of large language models (LLMs) by identifying and pruning less important attention heads, resulting in up to 61% relative accuracy gain over existing methods.


<details>
  <summary>Details</summary>
Motivation: Large language models (LLMs) excel at multi-step reasoning but have high inference costs, and the authors aim to reduce this cost through a training-free method.

Method: Causal Attribution Pruning (CAP), a method that identifies critical attention heads by measuring their causal impact on reasoning tasks and uses these head-level scores to guide fine-grained weight pruning.

Result: CAP yields relative accuracy gains of up to 61% over Wanda on ARC-Challenge at 20% sparsity and improves over Wanda in most model-benchmark configurations at moderate sparsity (10-20%).

Conclusion: The results suggest that attention-head-level causal attribution can better preserve reasoning performance on downstream benchmarks than correlational pruning criteria at equivalent sparsity.
...
<details>
  <summary>Details</summary>
Motivation: Existing hallucination detection methods overlook structural information in KGs, leading to suboptimal performance, and hallucinations in LLMs remain a critical issue in KG reasoning.

Method: LUCID jointly leverages LLM attention scores, KG semantics, and structural information using a graph neural network to extract node and edge features.

Result: LUCID achieves state-of-the-art performance compared to 15 baselines on nine datasets.

Conclusion: The proposed LUCID method effectively addresses the hallucination issue in LLM-based KG reasoning frameworks by leveraging structural information in KGs.
...
<details>
  <summary>Details</summary>
Motivation: The lack of comprehensive and standardized sign-language datasets hinders progress in sign-language recognition, translation, and production, leading to a need for a unified foundation for developing inclusive and robust sign-language technologies.

Method: The authors conducted a survey of existing sign-language datasets, analyzing 120 resources across 35 sign languages, and identified key challenges such as modality imbalance, annotation granularity, and signer bias.

Result: The authors present a comprehensive index of sign-language datasets, a 24-field Sign-Language Datasheet, and a public GitHub repository to support standardized documentation and reproducible evaluation.

Conclusion: The work provides a unified and practical foundation for developing inclusive, robust, and scalable sign-language technologies in real-world applications, addressing the limitations of existing benchmarks and promoting future progress in sign-language recognition, translation, and production.
...
<details>
  <summary>Details</summary>
Motivation: The reliability of In-Context Learning (ICL) in LLMs is a concern due to sensitivity to prompt design and model understanding of context, making uncertainty decomposition crucial.

Method: The paper proposes using self-function vectors, built on Bayesian views and mechanistic interpretability of ICL, to estimate aleatoric uncertainty directly and evaluate it with a new rigorous protocol.

Result: The proposed methodology measures uncertainty in LLM predictions under ICL more reliably than existing methods and can be used for trustworthy-related applications like hallucination detection.

Conclusion: The paper paves a new direction for connecting quantitative uncertainty views with mechanistic understanding of model behavior, enhancing reliability and trustworthiness in LLMs.
...
TL;DR: The paper introduces GRACE, a unified theoretical framework for determining the optimal granularity of verification in test-time scaling for large language models, and proposes an adaptive strategy that achieves the compute-performance Pareto frontier.


<details>
  <summary>Details</summary>
Motivation: The motivation is to investigate the optimal granularity of verification under a given compute budget, as prior work has not fully explored this fundamental question.
...
Result: The paper proves the existence of a phase transition in verification granularity and proposes an adaptive strategy that outperforms fixed-granularity baselines by up to 3.1% accuracy at matched compute.

Conclusion: The paper concludes that the proposed GRACE framework and adaptive strategy can achieve compute-optimality across different regimes and outperform existing methods.
...
TL;DR: The paper introduces the Argent Signaling Protocol (ASP) to improve multi-agent LLM systems by providing structured quality signals with AI-generated responses, enabling better handling of failures.


<details>
  <summary>Details</summary>
Motivation: Current retry strategies for multi-agent LLM systems treat all failures equally, which can lead to inefficient handling of different types of failures, and human supervisors cannot determine whether a retry was warranted or if the system should have halted.

Method: The authors introduce ASP, a compact machine-readable header that accompanies AI-generated responses with structured quality signals, and evaluate it in standalone and multi-agent modes.

Result: The evaluation shows that ASP improves pass rates and term coverage in standalone mode, and blocks 100% of ungrounded upstream outputs from reaching downstream agents in multi-agent mode.

Conclusion: The ASP enables more efficient handling of failures in multi-agent LLM systems by providing structured quality signals, allowing for better distinction between repairable and containment failures.
...
TL;DR: This paper presents a study on the narrative composition of large language model (LLM) pretraining corpora, specifically analyzing the narrative features in a 3-trillion-token open pretraining corpus called Dolma.
...
Method: The authors design a framework based on narrative theory, operationalizing three core narrative elements (agency, setting, and events) into 11 interpretable dimensions. They then sample and annotate 400 passages, finetune and validate a RoBERTa-based model called NarraBERT, and apply it to 3M passages to create a new dataset called NarraDolma.

Result: The study finds that narrative structure is measurable at scale across heterogeneous data, uncovering a continuous, multidimensional narrative structure underlying web text. The results also show that narrative qualities are unequally distributed across pretraining sources and topics.

Conclusion: The study provides a foundation for understanding how narrative qualities are distributed in LLM pretraining data and for studying how data composition affects narrative reasoning tasks. The authors publicly release NarraDolma and NarraBERT.
...
TL;DR: The study evaluates LLM-as-a-Judge paradigm, finding issues with exact-match agreement metric, judge rankings, and biases, and proposes a Minimum Viable Validation Protocol.


<details>
  <summary>Details</summary>
Motivation: The current dominant evaluation paradigm for language models, LLM-as-a-Judge, has a flawed validation method that relies on exact-match agreement, which overstates discriminative ability.
...
Result: The study finds universal kappa deflation, shifting judge rankings, high test-retest reliability with severe position bias, and small verbosity bias, across the cohort.

Conclusion: The study proposes a Minimum Viable Validation Protocol to address the issues found in the LLM-as-a-Judge paradigm.
...
TL;DR: The paper introduces LaViSA, a benchmark to evaluate Vision and Language Models' (VLMs) ability to resolve structural ambiguity in sentences using visual scenes. Experimental results show that while VLMs can leverage visual scenes to some extent, they still struggle with certain ambiguity types.


<details>
  <summary>Details</summary>
Motivation: Structural ambiguity in sentences poses a challenge for language understanding, and visual scenes can serve as useful cues for resolving such ambiguity. The authors aim to evaluate VLMs' ability to derive possible semantic interpretations from visual scenes.
...
Conclusion: The paper concludes that while VLMs have shown promise in resolving structural ambiguity using visual scenes, there are still remaining limitations, indicating a need for further research and improvement in this area.
...
<details>
  <summary>Details</summary>
Motivation: To address the challenge of Vietnamese multi-document abstractive summarization, specifically the lack of sufficient data.

Method: A hierarchical approach condensing each document followed by aggregation and summarization, with a novel strategy driven by the golden summary.

Result: A ROUGE2-F1 score of 0.2468 on the VLSP's public test set, producing fluent and concise summaries.
...
TL;DR: The paper uses training-data attribution to discover which regions of the pretraining corpus support social-reasoning versus STEM-reasoning in a large language model, and finds that social and STEM reasoning draw on distinct corpus regions.


<details>
  <summary>Details</summary>
Motivation: To understand which parts of the training data contribute to specific capabilities in a language model, particularly social-reasoning versus STEM-reasoning.

Method: The authors use gradient-based attribution (TrackStar) to measure the influence of training documents on model predictions, aggregating influence across a taxonomy of 576 bins, and contrasting benchmark pairs in a 2x2 design.

Result: The study finds that social and STEM reasoning draw on qualitatively distinct corpus regions, with a sharper contrast at the reasoning level than at the knowledge level. Targeted machine unlearning experiments provide partial causal validation of the results.

Conclusion: The paper demonstrates the effectiveness of training-data attribution for capability discovery in large language models, and provides insights into the corpus regions that support social-reasoning versus STEM-reasoning.
...
TL;DR: The authors argue that clinical NLP datasets for detecting suicidal behaviors, built from EHR data, reflect a particular operationalization of suicidality shaped by data authors, episode boundaries, and ambiguity resolution.


<details>
  <summary>Details</summary>
Motivation: The authors are motivated by the increasing reliance on EHR data for clinical NLP tasks, specifically for detecting suicidal behaviors, and the assumption that EHR data is more reliable than social media data.

Method: The authors conduct a case study of the ScAN dataset, built from MIMIC-III clinical notes, and perform a linguistic analysis to demonstrate how governance constraints, ICD-based cohort selection, single-annotator labeling, and hospital-stay-level aggregation affect the labels.
...
Conclusion: The authors conclude that clinical NLP should be cautious when interpreting labels from EHR-based suicidality datasets as ground truth, as they reflect a particular operationalization of suicidality shaped by various factors.
...
Method: MiqraBERT, a Sentence-BERT model finetuned from AlephBERT, was trained on 1,650 labeled verse and half-verse pairs to learn an embedding space for parallel verses.

Result: MiqraBERT improves distributional separation by 2.7-fold, reduces the ambiguous overlap region to 6%, and achieves a recall@10 of 87.1% for narrative synoptic parallels.
...
TL;DR: The paper explores generating multilingual mental health datasets using persona-based methods and evaluates LLM performance across languages.


<details>
  <summary>Details</summary>
Motivation: Addressing the shortage of high-quality datasets for training and evaluating AI systems in mental health, particularly in non-English contexts.

Method: Modifying persona parameters to generate clinical dialogues in Mandarin, Bengali, and Hindi, then evaluating LLM performance in assessing depression severity.

Result: Simply adding nationality and language parameters to personas can introduce clinical inconsistencies; LLMs show inaccuracies in assessing depression severity in non-English texts.

Conclusion: The study highlights the need for culturally responsive data generation to ensure equitable global mental health systems.
...
TL;DR: The paper presents a multilingual computational discourse analysis of how language constructed the algorithmic consecration of Vozinha, a Cape Verde goalkeeper, after a 2026 FIFA World Cup match.


<details>
  <summary>Details</summary>
Motivation: The study aims to analyze how language contributed to the consecration of Vozinha, a relatively unknown athlete, through a multilingual corpus and a narrative taxonomy.

Method: The study uses a multilingual corpus in Portuguese, Spanish, English, and French, a nine-frame narrative taxonomy, and a reproducible annotation pipeline combining LLM-assisted suggestion with human validation.

Result: The findings suggest that distinct languages carried distinct frames, including Portuguese mobilization, Spanish crisis, English nation-making, and a shared platform-metric spectacle.

Conclusion: The paper contributes to the understanding of cross-lingual narrative diffusion across discourse phases and releases the corpus schema, frame taxonomy, annotation guidelines, and typed timeline as a pilot study.
...
<details>
  <summary>Details</summary>
Motivation: Most on-policy distillation studies focus on single-turn settings, while realistic LLM agents interact with environments over multiple turns, leading to compounding errors and brittle performance.

Method: SAGE-OPD uses a verifier-free selective intervention framework, observing environment feedback and teacher judgment to decide whether to skip or intervene on student responses, and weights token-level distillation by teacher confidence.
...
Conclusion: Effective multi-turn on-policy distillation should remain on-policy, but teacher supervision should be selectively allocated to turns where intervention is necessary and reliable.
...
TL;DR: CacheWeaver, a method for optimizing Retrieval-Augmented Generation (RAG) by reordering evidence to maximize prefix caching, reducing time-to-first-token (TTFT) by 20-33% without impacting answer quality.


<details>
  <summary>Details</summary>
Motivation: Retrieval-Augmented Generation (RAG) improves factual accuracy but increases prompt length and prefill cost; prefix caching can mitigate this but is limited by non-reusable prefix overlap in RAG.

Method: CacheWeaver uses a prefix tree and greedy walk to reorder evidence sequences for maximum prefix reuse, compatible with existing serving engines and retrieval methods.

Result: CacheWeaver reduces median TTFT by 20-33% across three vLLM configurations, achieving 97.5% of the gain from optimal ordering, without hurting answer quality.

Conclusion: CacheWeaver offers a lightweight, effective solution for optimizing RAG performance by improving prefix caching efficiency, suitable for deployment in serving engines.
...
TL;DR: The paper studies how multilingual large language models (MLLMs) perform with code-switched (CS) inputs, finding that they often degrade. It introduces a measure called Anchor Bias to understand this issue and proposes CANVAS, an inference-time intervention to improve performance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the performance degradation of MLLMs when handling code-switched inputs.

Method: The method involves using grammar-forced CS as a controlled diagnostic setting, introducing Anchor Bias to quantify language anchoring, and proposing CANVAS, an inference-time intervention.

Result: The results show that Anchor Bias reveals a consistent grammar-frame effect and that CANVAS consistently recovers QA F1 across MLLMs and CS conditions.

Conclusion: The conclusion is that internal anchoring signals provide an actionable target for mitigating CS inference failures in MLLMs.
...
TL;DR: The authors tested a new method for analyzing customer support conversations using GPT-5.4 to estimate customer satisfaction and flag problems, and found it to be more effective than sentiment analysis.


<details>
  <summary>Details</summary>
Motivation: Current sentiment analysis methods only measure the tone of customer support conversations, not customer satisfaction with the result.

Method: The authors used GPT-5.4 to estimate customer satisfaction and flag problems in 70,450 support conversations, and validated the results against 1-to-5 ratings left by customers.

Result: The satisfaction estimate correlated better with customer ratings (0.47) than sentiment analysis (0.36), and was able to flag unhappy customers with fewer false alarms.

Conclusion: LLM-based annotation can capture more than just the tone of customer language, offering potential for new business metrics based on customer state and problem cause.
...
TL;DR: TerraMARS is an information extraction pipeline that converts unstructured Mars science text into structured outputs to aid in habitability assessment and terraforming studies.
...
Result: The pipeline generates structured outputs in JSON format and shows promise, but requires further improvements for accuracy and factual consistency.
...
TL;DR: The paper introduces FineREX, a pipeline for knowledge graph construction that uses a fine-tuned large language model (LLM) for better entity and relationship extraction from legal documents about human smuggling networks.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to improve the extraction of information from unstructured, jargon-heavy legal documents related to human smuggling networks, which is a challenging task due to the domain-specific terminology and the need for precise entity and relationship extraction.

Method: The method used in this paper involves developing FineREX, a knowledge graph construction pipeline that utilizes a fine-tuned LLM for named entity recognition and relationship extraction (NER-RE). This approach is compared to a larger general-purpose baseline model.

Result: The results show that FineREX achieves significant improvements over the baseline, with absolute improvements of 15.50% in entity F1-score and 31.46% in relationship F1-score. FineREX also reduces legal noise by nearly half, lowers node duplication from 17.78% to 11.17% on long documents, and reduces end-to-end processing time by 50.0%.

Conclusion: The conclusion of this paper is that domain-specific fine-tuning of LLMs can substantially outperform larger general-purpose models in knowledge graph construction for illicit network analysis, improving both the quality and efficiency of the process.
...
TL;DR: The study examines the effects of sequential Direct Preference Optimisation (DPO) on language models, finding that the impact on earlier preferences varies based on objective relationships, signal strength, and training order.


<details>
  <summary>Details</summary>
Motivation: To understand how sequential application of multiple behavioural objectives in preference optimisation methods like DPO affects the preferences learned by language models.

Method: Using Llama-3.1-8B-Instruct with LoRA adapters, the study applies sequential DPO across four preference settings and evaluates objectives after each stage with a fixed base-model reference.

Result: The study finds that sequential DPO does not result in uniform degradation of earlier preferences; instead, the effect varies from partial degradation to stability, pair-level redistribution, or positive transfer based on objective relationship, signal strength, and training order.

Conclusion: The findings suggest that future sequential alignment pipelines should consider objective compatibility and signal strength, rather than assuming uniform effects of later objectives on earlier preferences.
...
TL;DR: The paper proposes a framework to transfer knowledge from pre-trained language models like BERT to Tsetlin Machines (TM) for interpretable text classification, achieving competitive performance with BERT.


<details>
  <summary>Details</summary>
Motivation: The motivation is to combine the interpretability of Tsetlin Machines with the strong performance of pre-trained language models like BERT, addressing the lack of transparency in BERT.

Method: The method involves grouping text samples into semantically coherent clusters and using these clusters to pre-train a Tsetlin Machine with enhanced feedback, allowing it to learn interpretable keywords.

Result: The proposed method outperforms vanilla and embedding-based TMs and reaches performance competitive with BERT across five datasets while remaining interpretable.

Conclusion: The paper concludes that the proposed framework successfully transfers knowledge from pre-trained language models to Tsetlin Machines, enabling interpretable text classification with competitive performance.
...
<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the shortcomings of prior work in automated fact-checking, specifically the reliance on token-overlap metrics that underestimate decomposition quality and the lack of formal termination analysis.

Method: The method used in this paper is to propose a revised claim decomposition and evaluation framework, Credence, which includes a semantic similarity metric (Semantic-F1) and formal characterisation of the repair pipeline.

Result: The results show that Semantic-F1 outperforms Jaccard-F1 by +15-32pp, and rule-repair reduces the Atomicity Violation Rate (AVR) by 47-100% relative to the base model without degrading fidelity.

Conclusion: The conclusion of this paper is that Credence, a revised claim decomposition and evaluation framework, improves upon previous methods by providing a more accurate semantic similarity metric and formal termination analysis, leading to better performance in automated fact-checking.
...
TL;DR: The paper presents a framework for single neuron steering in aligned language models, predicting when a single neuron intervention can control a behavior coherently. It introduces a budget normalized control window framework and identifies conditions for coherent control, demonstrating its effectiveness on refusal and language routing behaviors.


<details>
  <summary>Details</summary>
Motivation: Understanding and controlling behaviors in aligned language models, specifically when a single neuron intervention can coherently control a behavior without collapsing the output.

Method: The authors develop a budget normalized control window framework, which involves analyzing the alignment between the residual stream and the write direction, driven along a universal saturation curve. They also introduce a coherence budget and a collapse ceiling to predict coherent control.

Result: The framework predicts the collapse ceiling with a mean absolute error of 0.14 on fifteen held-out neurons. It successfully identifies controllers that local gradient attribution misses and demonstrates typed audit of controllability on refusal and language routing behaviors.

Conclusion: The paper concludes that single neuron steering is a budgeted, typed audit of controllability rather than a fixed dose anecdote. It provides insights into why local gradient attribution often fails to predict control and presents a forward-only contrastive screen to recover controllers.
...
TL;DR: A new long-term memory system called AtomMem is proposed to improve the ability of large language models to accumulate and reuse information over multiple interactions.


<details>
  <summary>Details</summary>
Motivation: The limitations of large language models in terms of fixed context windows and the inefficiencies of existing memory-augmented systems.

Method: AtomMem uses a Fact Executor to extract high-value atomic facts, organizes them into hierarchical structures, and activates an associative memory graph for retrieval.

Result: AtomMem achieves state-of-the-art performance on the LoCoMo benchmark across various reasoning tasks.

Conclusion: AtomMem offers a scalable and economically viable solution for deploying intelligent personalized agents.
...
Result: The best zero-shot model (GPT-OSS-20B) achieved a Micro-F1 of 0.893 (recall: 0.949), comparable to the state-of-the-art supervised GatorTron NER-RE baseline (Micro-F1: 0.960).
...
TL;DR: This paper explores encoding semantic information in compact, non-standard textual forms (BabelTele) that are recoverable by large language models (LLMs) but sacrifice human readability.


<details>
  <summary>Details</summary>
Motivation: The motivation is to investigate whether LLMs can understand and generate compact textual representations that are not easily readable by humans but still convey meaningful information.

Method: The approach involves using readability diagnostics, model likelihood measures, human questionnaires, and downstream task evaluations to assess the effectiveness of BabelTele.

Result: The results show that BabelTele can maintain 99.5% semantic fidelity while condensing text to 27.9% of its original length and can reduce context overhead while maintaining reliable downstream performance.

Conclusion: The findings suggest that human readability, natural-language typicality, and model-side semantic recoverability can be partially decoupled, paving the way for model-native representations in future LLM systems.
...
TL;DR: The paper explores using Large Language Models (LLMs) to enhance democratic deliberation, focusing on inclusivity and marginalized groups, while addressing risks and biases.


<details>
  <summary>Details</summary>
Motivation: The increasing prominence of LLMs presents opportunities and challenges for democratic deliberation, particularly regarding linguistic constraints, biases, and sycophantic tendencies.

Method: The chapter draws on Systemic-Functional Linguistics to examine variations in language users and language use, and presents AI-driven deliberation studies to assess their potential in scaffolding argumentation and enhancing access.

Result: The chapter identifies potential benefits of AI-assisted deliberation, including enhanced inclusivity and reduced influence of exclusionary linguistic norms and biases, but also cautions against overclaiming and underclaiming.

Conclusion: The chapter concludes by highlighting future research directions to maximize the democratic potential of AI-assisted participation while embedding ethical safeguards to counteract linguistic inequalities.
...
<details>
  <summary>Details</summary>
Motivation: The existing benchmarks for PII detection are limited, covering few entity types and lacking controlled generation conditions, making it difficult to assess detector failures.

Method: The authors create REDACT, a benchmark with 13,427 records, 324,078 entity annotations, and 51 entity types, using a strength-2 covering-array sampler to control nine generation axes.

Result: The evaluation of five detectors on a locked, language-stratified sample of 1,000 records reveals that rule-based detectors perform poorly on high-stakes data, while LLM detectors are more robust.

Conclusion: The paper highlights the need for a more comprehensive and controlled benchmark for PII detection and demonstrates the importance of stratified evaluation in assessing detector performance.
...
TL;DR: A new framework for automated pronunciation assessment is proposed, leveraging native speech resources and requiring minimal labeled learner data.
...
Result: The proposed framework achieves a PCC improvement from 0.60 to 0.66 on SpeechOcean762 and consistent gains on L2-ARCTIC in cross-dataset evaluation.

Conclusion: The proposed framework effectively utilizes native speech resources to improve automated pronunciation assessment with minimal labeled learner data.
...
TL;DR: The paper presents a method called GEMS for controlling model behavior by modifying intermediate hidden states without retraining, addressing the issues of model collapse when multiple semantic directions are superposed.
...
Method: The authors propose GEMS, a training-free method that uses geometric constraints to address distributional deviation and directional interference, including norm-preserving weighted superposition, targeted attention-pathway injection, and real-time orthogonalization.

Result: GEMS achieves 98% accuracy on GSM8K and only 2.2% PPL increase on Wikitext-2 when injecting three concurrent non-mathematical directions, compared to 4% collapse with unconstrained addition.

Conclusion: The proposed method, GEMS, effectively addresses the challenges of multi-directional intervention in activation steering, demonstrating improved performance and transferability across architectures.
...
TL;DR: The paper proposes STAGE, a data generation pipeline for creating reliable text-to-JSON training data, which improves the performance of JSON extraction models.


<details>
  <summary>Details</summary>
Motivation: The difficulty in extracting high-value information from unstructured documents in legacy industries and the challenge of constructing reliable and scalable text-to-JSON training data.
...
Conclusion: The proposed STAGE pipeline effectively generates high-quality text-to-JSON training data, leading to improved performance in JSON extraction tasks.
...
TL;DR: IHUBERT is a monolingual Persian language model that achieves state-of-the-art results on various NLP tasks, including extractive question answering, sentiment analysis, and topic classification.


<details>
  <summary>Details</summary>
Motivation: The scarcity of large-scale, high-quality pretraining corpora and insufficient evaluation beyond standard classification and NER tasks in Persian language models.
...
Result: IHUBERT achieves state-of-the-art results on seven Persian NLU benchmarks, including extractive QA, sentiment analysis, topic classification, and relation extraction.
...
TL;DR: This paper tests whether large language models resist valid corrections to their own writing, finding no detectable self-preference bias in accepting or rejecting edits.


<details>
  <summary>Details</summary>
Motivation: To investigate if large language models resist valid corrections to their own writing, given a documented self-preference bias in favoring their own generations.

Method: Using a deterministic verifier on IFEval to test instruction-following revision, with a model writing a draft, a verifier confirming a constraint violation and a candidate edit, and the model then accepting or rejecting the edit.

Result: No detectable self-preference bias found, with authors rejecting verified-good fixes at a similar rate to fresh models (gap -5.1 pp, 95% CI [-12.9, +2.7]).

Conclusion: The study finds that large language models do not resist valid corrections to their own writing, and when they do reject edits, it's primarily due to flaw-catching rather than preference.
...
TL;DR: A novel hybrid attention architecture called HydraHead that combines Full Attention (FA) and Linear Attention (LA) at the head level, achieving better performance on long-context tasks.


<details>
  <summary>Details</summary>
Motivation: The quadratic complexity of attention is a bottleneck for long-context processing, and current hybrid attention designs have limitations, sparking interest in exploring new design spaces.

Method: HydraHead introduces a head-level hybridization of FA and LA, with an interpretability-driven selection strategy and a scale-normalized fusion module, and leverages a three-stage transfer pipeline for training.

Result: HydraHead outperforms other hybrid designs on long-context tasks, achieving a 69% improvement over the baseline at 512K context length, and approaching the performance of a leading model with a native context length of 256K.

Conclusion: The proposed HydraHead architecture demonstrates the potential of head-level hybridization for improving long-context processing, with significant scaling potential.
...
TL;DR: The paper analyzes the effectiveness of Streaming Retrieval-Augmented Generation (Streaming RAG) in reducing user-perceived latency by speculatively issuing tool queries during user input.


<details>
  <summary>Details</summary>
Motivation: The motivation is to understand the intrinsic benefits of speculation in Streaming RAG, specifically when the correct tool query can be determined before the user finishes speaking or typing.

Method: The study measures 'tool-intent stabilization', the point at which a speculative query's retrieval converges to the answer-bearing result, on the CRAG benchmark with 1371 validation questions.

Result: The results show that at a realistic operating point, 73.9% of queries can admit substantial latency hiding, with 95.2% streamable on a favorable slice of questions with verbatim-present and BM25-retrievable gold evidence.

Conclusion: The study concludes that query properties can predict early versus late stabilization, and that question type can inform when a learned speculative trigger is worth its cost, providing insights for optimizing Streaming RAG systems.
...
TL;DR: This paper analyzes how Large Language Models (LLMs) score essays, finding that essay quality information is encoded linearly within LLM representations and can be accessed through specific 'essay scoring neurons'.


<details>
  <summary>Details</summary>
Motivation: The internal mechanisms of LLM-based Automated Essay Scoring (AES) are not well understood, despite significant advances in AES through LLMs.

Method: The study uses linear probing, cross-prompt generalization, dimensionality reduction, and neuron-level analyses on eight LLMs across three essay datasets (ASAP++, CSEE, ENEM).

Result: Essay quality information is encoded linearly within LLM representations, emerging progressively across layers, and is robust across prompting strategies and partially transferable across essay prompts.

Conclusion: The findings provide evidence that LLMs encode structured representations related to essay quality, offering new insights into the interpretability of LLM-based AES systems.
...
TL;DR: MedRLM, a Recursive Multimodal Health Intelligence framework, is proposed for long-context clinical reasoning and decision support, treating patient cases as an external clinical environment that can be recursively inspected and analyzed.


<details>
  <summary>Details</summary>
Motivation: Current medical AI systems rely on single-step prompting or retrieval, which can be fragile when clinical evidence is distributed across multiple sources. Real-world clinical decision support requires reasoning over heterogeneous and longitudinal patient information.

Method: MedRLM coordinates specialized agents for clinical text, EHR, medical imaging, sensor signals, and guideline retrieval. It uses a Clinical Evidence Graph Memory and a sensor-guided recursive triggering mechanism for deeper reasoning and uncertainty-gated refinement.

Result: The proposed framework aims to move medical AI from static question answering toward auditable, multimodal, and workflow-aware clinical decision support. A real-data evaluation design using public and credentialed clinical datasets is outlined.
...
TL;DR: ReNikud, a new method for Grapheme-to-phoneme conversion in Modern Hebrew, achieves state-of-the-art results by leveraging weak audio supervision and pseudo-vocalization architecture.


<details>
  <summary>Details</summary>
Motivation: The challenge of G2P conversion in Modern Hebrew due to its abjad writing system, which creates substantial ambiguity, and the limitations of standard approaches.

Method: ReNikud uses weak audio supervision via phoneme-based ASR pseudo-labeling and a pseudo-vocalization architecture to predict IPA phonemes at each character position.
...
Conclusion: ReNikud offers a more effective approach to G2P conversion in Modern Hebrew, with plans to release code and trained models to support further work on Hebrew TTS and speech technologies.
...
Method: The algorithm uses a two-stage optimization process: a 'modal' stage proposing probable scales for each bar, and a 'tonal' stage estimating the key signature and note names for the entire piece.
...
Conclusion: The proposed method is useful for music transcription and other tasks related to musical notation processing, and defines new distances between common jazz scales.
...
TL;DR: Introduced CzechDocs, a dataset for format-preserving machine translation, supporting evaluation and future research.


<details>
  <summary>Details</summary>
Motivation: To support the evaluation of machine translation systems that preserve document formatting during translation, particularly for languages used in Czechia.

Method: Created a multiway parallel dataset (HTML, DOCX, and PDF) and compared common approaches to format-preserving machine translation on a validation subset.

Result: Released a validation split and evaluation toolkit publicly for further research, with a held-out test split reserved for a future shared task.

Conclusion: CzechDocs dataset facilitates the development and evaluation of format-preserving machine translation systems.
...
TL;DR: The study investigates the misalignment in language models fine-tuned on insecure code and identifies a causally actionable activation-space direction shared across architectures, enabling methods to reduce code spillover.


<details>
  <summary>Details</summary>
Motivation: To understand and address the emergent misalignment in language models fine-tuned on insecure code, which has poorly understood internal structures.

Method: The study uses a difference-in-means direction to separate aligned and misaligned activations across four instruction-tuned model families and applies causal steering and cross-architecture transfer via ridge regression.

Result: The study finds a causally actionable direction that achieves 99.6% separation of aligned and misaligned activations and reduces code spillover by 21-51 points, but cross-architecture transfer fails specificity controls.

Conclusion: The findings define the limits of linear cross-architecture correction, recommend within-model probing for auditing, and highlight an asymmetric transfer topology among the models.
...
TL;DR: The Meaning Intelligence Framework (MIF) is introduced to improve AI understanding of Nigerian public discourse by separating surface sentiment from true communicative intent, achieving significant accuracy gains.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for Nigerian languages fail to account for context-dependent variations in communicative intent, leading to 'context failure' in AI systems.

Method: The MIF uses a nine-dimension annotation and evaluation schema, and a calibration dataset of 30 items to evaluate a frontier language model (Gemini 2.5 Flash) under zero-shot and schema-informed prompting conditions.

Result: The model achieves significant gains in accuracy, particularly in register identification (+40 points), coded-subtext detection (+10 points), and strategic action recommendation (+10.3 points).

Conclusion: The MIF framework improves AI understanding of Nigerian public discourse, highlighting the importance of context in communicative intent, and releasing the framework specification, annotation guidelines, and calibration set for reproducibility.
...
<details>
  <summary>Details</summary>
Motivation: Existing approaches to automated essay scoring and feedback often treat scoring and feedback as separate components, leading to limitations in interpretability and adaptability to learner proficiency levels.

Method: PsyScore comprises three modules: a Trait-Adaptive Neural IRT Scorer, a ZPD-Scaffolded Feedback Generator, and a Multi-Perspective Feedback Evaluation Strategy.
...
Conclusion: PsyScore offers a comprehensive approach to automated essay scoring and feedback that addresses the limitations of existing methods and provides more effective support for learners.
...
TL;DR: A new large-scale, multilingual dataset of dialogues is introduced to tackle the intersection of hate speech and misinformation, enabling the training of more persuasive and factually grounded counterspeech models.


<details>
  <summary>Details</summary>
Motivation: The overlap of online hate speech and misinformation is not well addressed by current NLP research, and existing counterspeech datasets are limited in scope and language.

Method: The authors introduce a novel dataset of expert-curated, multilingual dialogues that are anchored in verified external knowledge and include detailed annotations.
...
Conclusion: The introduced dataset bridges the gap in existing counterspeech datasets and can be used to improve the performance of counterspeech models in addressing the intersection of hate speech and misinformation.
...
TL;DR: The paper explores the use of implicit human feedback to align Large Language Models (LLMs), creating a dataset called IFLLM and demonstrating that implicit feedback can improve LLM performance.
...
Method: The authors collect a new dataset called IFLLM, which includes multi-turn questions, mouse trajectories, and eye-gazing points from 59 Mechanical Turk workers. They then train a reward model based on implicit user feedback and evaluate its performance.

Result: The reward model based on implicit user feedback improves the accuracy of the text-based reward model from 55% to 64% and nearly triples the relative response quality improvements after applying the DPO to eight LLMs.

Conclusion: The paper demonstrates the value of implicit feedback in improving LLM performance, and provides a dataset and code for future research.
...
TL;DR: The paper proposes H-RePlan, a hierarchical replanning framework for multi-device agents that enables robust execution by separating device-local strategy recovery from global replanning.


<details>
  <summary>Details</summary>
Motivation: Existing multi-device agent systems lack fine-grained recovery mechanisms, often retrying the same strategy or reassigning tasks upon failure, which limits their ability to handle dynamic runtime failures effectively.

Method: The authors propose H-RePlan, which equips each device with interchangeable execution strategies and introduces a compact cross-layer failure abstraction to separate device-local strategy recovery from orchestrator-level global replanning.

Result: Experiments using the HeraBench benchmark show that H-RePlan outperforms baselines, achieving higher completion, instruction adherence, and perfect-pass rates while reducing token cost for reliable end-to-end success.

Conclusion: The paper concludes that scope-aware hierarchical recovery is essential for robust multi-device agent execution, and H-RePlan demonstrates significant improvements over existing approaches.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to understand how visual cues shape the judgments of MLLMs, which are increasingly deployed in consequential settings, and to provide a controlled benchmark for evaluating attribute-level social bias.

Method: The authors create a dataset of 25K images with 500 photorealistic base faces and about 50 single-attribute variations per face. They evaluate six MLLMs across 25 binary social judgment scenarios, measuring how specific visual cues shift model judgments.

Result: The results show that age and body type have the largest identity-level effects, while fashion style and other visual cues drive the largest attribute-level shifts. A small set of about 15 attributes account for nearly 80% of the total variation in model judgments.

Conclusion: The paper concludes that bias in MLLMs is concentrated in a small set of visual cues, and that sensitivity is strongest in judgments semantically aligned with appearance. The StylisticBias benchmark is released for fine-grained bias evaluation in multimodal models.
...
TL;DR: The paper proposes AgenticRei, a system to govern autonomous AI agents driven by Large Language Models (LLMs), addressing security, privacy, and compliance challenges by providing a deontic policy language and a high-performance logic engine.


<details>
  <summary>Details</summary>
Motivation: Current policy engines (e.g., XACML, Rego, Cedar) are insufficient to govern autonomous AI agents as they only address permit/prohibit constraints, not obligations, dispensations, and policy conflict resolutions required for enterprise governance.
...
Conclusion: The paper concludes that AgenticRei is a necessary solution to address the governance challenges introduced by autonomous agentic AI systems driven by LLMs, providing a more comprehensive approach to security, privacy, and compliance.
...
TL;DR: The paper presents a method to measure a computer science program's coverage of curricular guidelines, applying it to a BSc program against CS2013 and CS2023 guidelines, revealing near-constant coverage but a gap in delivering competency at the recommended depth.
...
Conclusion: The proposed instrument is reusable, reliable, and can help programs assess their coverage of curricular guidelines, identifying areas for improvement.
...
TL;DR: This paper presents a systematic analysis of modern Diffusion Language Models (DLMs), evaluating their performance across various tasks and analyzing the impact of inference-time factors on their capabilities and efficiency.
...
Conclusion: The study provides practical insights into the capabilities and deployment characteristics of contemporary DLMs, shedding light on their potential applications and areas for improvement.
...
TL;DR: This paper models multi-agent LLM deliberation as a closed-loop dynamical system, introducing the concept of an 'anchor' representing an agent's internal belief, which can be recovered from deliberation data and explains behaviors not captured by classical consensus rules.


<details>
  <summary>Details</summary>
Motivation: The authors aim to understand how and why multi-agent LLM deliberation improves reasoning and accuracy by modeling it as a dynamical system that incorporates both social influence and internal beliefs.

Method: The authors propose a closed-loop dynamical system model where each agent has a hidden internal belief ('anchor') that influences its opinion over rounds of deliberation.

Result: The authors show that the 'anchor' can be recovered from deliberation data, it explains behaviors like an agent's confidence increasing beyond initial beliefs, and its influence varies across different model families.

Conclusion: The study provides insights into the mechanism of multi-agent LLM deliberation, suggesting that the 'anchor' concept is crucial for understanding and predicting the outcomes of such deliberations.
...
TL;DR: DeXposure-Claw is a new system for supervising decentralized finance, using a forecast-grounded approach to reduce false alarms and improve decision-making.
...
Method: DeXposure-Claw combines a graph time-series foundation model (DeXposure-FM) with deterministic monitors and stress scenarios to generate structured evidence for LLM decisions.

Result: Experiments on five years of real data support the effectiveness of DeXposure-Claw, which is evaluated using a six-axis evaluation harness (DeXposure-Bench).

Conclusion: DeXposure-Claw provides a regulator-aligned approach to supervising decentralized finance, reducing false alarms and improving decision-making.
...
TL;DR: This paper investigates whether large language models (LLMs) can recognize their own knowledge limits on structured clinical tasks, and proposes methods to reduce epistemic uncertainty.


<details>
  <summary>Details</summary>
Motivation: The study aims to explore the ability of LLMs to recognize their own knowledge limits on structured clinical tasks, and to reduce epistemic uncertainty through cross-model attribution divergence.

Method: The study uses Qwen 2.5 7B and XGBoost on a prediction task, and analyzes attribution divergence to compare the two models. It also proposes a cross-model calibrator to determine LLM reliability.

Result: The study reports four findings: (1) LLM verbalized confidence is not informative, (2) LLM exhibits an inverse difficulty effect, (3) few-shot examples and SHAP-derived feature evidence are complementary interventions that improve accuracy, and (4) a cross-model calibrator reduces expected calibration error.

Conclusion: The study frames the findings as a 'cold start problem' for LLMs on structured data and outlines a path toward genuine epistemic self-awareness, proposing methods to improve LLM reliability and calibration.
...
TL;DR: The paper proposes a continuous formulation of phenotypic structure within contrastive learning to improve early prediction of Alzheimer's disease (AD) from retinal fundus images and clinical risk narratives.


<details>
  <summary>Details</summary>
Motivation: Existing methods for predicting AD using retinal fundus images and clinical risk narratives rely on discrete phenotypic grouping, which imposes rigid supervision and decouples group formation from representation learning.

Method: The proposed framework models inter-subject similarity as a differentiable weighting function derived from intra-modality embedding similarities in both retinal images and risk profiles, and introduces a soft-target contrastive objective for end-to-end learning.
...
TL;DR: This paper explores if Large Language Models (LLMs) can detect and self-correct misaligned outputs with human ethics, proposing a method for self-alignment.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enable LLMs to discern and correct their own misaligned outputs with human ethics, preventing emergent unethical behaviors.
...
Result: The result is an online technique for aligning models in various applications without needing a separate judge, demonstrating 'Emergent Alignment' through a single introspective question.

Conclusion: The conclusion is that LLMs can be made to self-align with human ethics through the proposed method, achieving 'Emergent Alignment' and preventing unethical behaviors.
...
TL;DR: The paper introduces a unified architecture called Integral Transform Network (ITNet) that combines convolutional networks, recurrent networks, and transformers into a single learnable integral transform, showing that these architectures are special cases of ITNet and can be recovered from data.


<details>
  <summary>Details</summary>
Motivation: The fragmentation of different neural network architectures (convolutional networks, recurrent networks, and transformers) and the lack of a unified understanding of their underlying mathematical principles.

Method: The authors introduce a learnable kernel that depends jointly on positions and features, implemented as a small neural network (MLP), and develop techniques for efficient computation such as tiled kernel fusion and importance-weighted Monte Carlo integration.

Result: The ITNet architecture with a shared operator and lightweight modality-specific encoders matches or exceeds specialized baselines on various benchmarks including ImageNet-1K, GLUE, ModelNet40, VQA v2, and NLVR2.

Conclusion: A single learned interaction mechanism can recover the behavior of all three architectural families (convolutional networks, recurrent networks, and transformers) from data, providing a unified understanding of these architectures.
...
TL;DR: A prompt-based method for estimating uncertainty in large language models (LLMs) is proposed, enabling agents to seek clarification when task specifications are ambiguous.


<details>
  <summary>Details</summary>
Motivation: The classical uncertainty framework is insufficient for interactive LLM agents, and practical deployment constraints limit the use of existing methods, necessitating a new approach.
...
Conclusion: The proposed prompt-based decomposition enables LLMs to effectively seek clarification when faced with ambiguous task specifications, demonstrating improved performance across multiple benchmarks and LLM backbones.
...
TL;DR: The paper analyzes the interaction between formal solvers and language models in reasoning pipelines, highlighting a previously unstudied 'narration' step that translates solver output into user answers. It identifies vulnerabilities in this process, particularly under prompt injection attacks, and explores mitigations.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the gap in understanding the interaction between formal solvers and language models, specifically in the 'narration' step of turning solver output into user answers, and to evaluate the soundness and robustness of the LLM-solver loop.

Method: The method involves modeling the LLM-solver loop as a verified decision procedure and empirically evaluating five open-sourced models under prompt injection attacks to test the soundness of the solver verdict and the robustness of the loop.

Result: The result shows that while certificate gating can make the solver verdict sound, the loop is vulnerable to inversion of verified conclusions across phrasings and channels. Hardened prompts can reduce but not eliminate injection vulnerabilities, and the loop still suffers under adaptive attacks.

Conclusion: The conclusion is that in the LLM-solver loop, robustness does not reach the answer that the user finally reads, indicating a need for further research and mitigation strategies to ensure the reliability and security of hybrid reasoning pipelines.
...
TL;DR: The authors developed ACIE, a clinical information extraction system, to address the issue of incomplete metadata in patient contexts. They evaluated its performance and found that clinicians accepted 96.5% of extractions.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of incomplete metadata in patient contexts, which hinders the performance of standard retrieval-augmented generation systems in clinical settings.

Method: The authors deployed an on-premise agentic RAG pipeline called ACIE (Agentic Clinical Information Extraction) at University Medicine Essen, which reasons over complete patient contexts and grounds every answer in source passages for clinician verification.
...
Conclusion: The authors conclude that ACIE is effective in addressing the metadata gap and providing accurate clinical information extraction, with a high acceptance rate from clinicians.
...
TL;DR: The paper studies how to select comparison pairs for preference-based post-training of language models, formulating it as a sampling-design problem and providing bounds on the optimality gap.


<details>
  <summary>Details</summary>
Motivation: Human preference labels are expensive, and generating additional completions is cheaper, suggesting a strategy of labeling only the most informative comparison pairs.
...
Result: The proposed designs consistently improve sample efficiency over common comparison-selection heuristics in experiments on synthetic settings and language-model post-training benchmarks.

Conclusion: The choice of labeled pairs affects downstream performance through a single design-dependent information matrix, yielding an explicit optimization criterion for budgeted comparison curation.
...
Method: TOTEN uses a formal ontology of engineering entities (OEE) and declarative classification, coupled with three external oracles: Pint, Unicode Character Database, and RSLP, to provide robust and accurate tokenization.

Result: TOTEN achieves unit ontological atomicity and high numerical reconstruction (0.775-0.904 on external corpora, 0.780 on EngQuant), outperforming state-of-the-art baselines.
...
TL;DR: The article discusses the progress and gaps in AI and Systems Engineering, based on a review of core papers and an analysis of 1,712 articles and 889 publications.


<details>
  <summary>Details</summary>
Motivation: To assess the progress and identify critical gaps in AI and Systems Engineering, and provide guidance for practitioners.

Method: A literature review of core papers, and an analysis of 1,712 INCOSE INSIGHT articles and 889 SERC publications using human expertise and six AI models.
...
Conclusion: The community has made progress in AI and Systems Engineering, but critical gaps remain, and the authors provide guidance and resources for further research and practice.
...
<details>
  <summary>Details</summary>
Motivation: To address the limitations of existing reconstruction-driven tokenizers that compromise clinical information for reconstruction accuracy in 3D brain MRI latent diffusion.

Method: A fully volumetric masked-autoencoder (MAE) based tokenizer is proposed, using a frozen 3D MAE encoder to produce clinically informative embeddings and a dedicated CNN decoder for voxel reconstruction.
...
TL;DR: The authors propose a model-agnostic denoising method called DIF to address the issue of noisy implicit feedback in cold-start recommendation scenarios.


<details>
  <summary>Details</summary>
Motivation: Implicit feedback in recommender systems can be noisy, and cold items are more prone to noisy samples, which can negatively impact recommendation performance.

Method: The DIF method uses pseudo-labels inferred from content-similar warm items to denoise implicit feedback for cold items, and estimates the uncertainty of the noisy sample label to adaptively guide the role of pseudo-labels.

Result: The DIF method has been shown to be effective through extensive experiments on real-world datasets and has been deployed on a billion-user scale short video application, resulting in significant improvements in various commercial metrics within cold-start scenarios.

Conclusion: The proposed DIF method provides a effective solution for denoising implicit feedback in cold-start recommendation scenarios, which can improve the performance of recommender systems.
...
TL;DR: The paper studies decentralized coalition formation through unilateral decisions, linking cooperative payoff allocation with noncooperative behavior.


<details>
  <summary>Details</summary>
Motivation: To understand coalition formation as a dynamical process driven by individual decisions.
...
Result: Establishing conditions for Lyapunov or potential representations and analyzing the impact of costs on local stability through numerical experiments.

Conclusion: The paper provides insights into decentralized coalition formation, highlighting the link between cooperative payoff allocation and noncooperative behavior.
...
<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the shortcomings of current agent benchmarks, which fail to cover multiple dimensions of deployment. The authors aim to provide a more comprehensive understanding of agent performance and propose a new evaluation methodology.

Method: The paper aggregates fourteen parallel implementation studies of an MCP-based industrial-agent benchmark and consolidates them with seven prior agent benchmarks. The authors analyze the limitations of aggregate-score leaderboards and propose ranking configurations by predictive validity.

Result: The authors argue that aggregate-score leaderboards are insufficient for evaluating deployed agents and propose a twelve-tier measurement apparatus to expose deployment-relevant dimensions. They also provide a pre-registered pilot design and a field-level vision for next-generation agentic benchmarks.

Conclusion: The paper concludes that current evaluation methods for agent benchmarks are flawed and proposes a new approach to ranking configurations based on predictive validity. The authors emphasize the need for a more comprehensive understanding of agent performance and propose a new generation of agentic benchmarks.
...
TL;DR: The paper presents an interactive interface using LLMs to provide natural language access to global explanations for black-box image classifiers, improving accessibility and usability.


<details>
  <summary>Details</summary>
Motivation: The complex and monolithic nature of global explanations for vision models hinders practical exploration, and users seek targeted answers to specific questions.

Method: An LLM-based interactive interface that translates natural language questions into structured SQL queries over local explanation data.

Result: The system shows substantial improvements in accessibility and usability of global explanations, evaluated on intent interpretation, query mapping accuracy, generalization, and robustness.

Conclusion: The proposed LLM-mediated querying approach enhances the practical exploration of global explanations for black-box image classifiers.
...
<details>
  <summary>Details</summary>
Motivation: The black-box nature of NCO models hinders their deployment and scientific diagnosis, and standard interpretability tools are not well-suited for NCO.
...
Result: Extensive experiments demonstrate EPB's effectiveness and broad applicability, with distilled program portfolios matching original performance.

Conclusion: EPB advances interpretable NCO and establishes itself as a promising tool for interpreting sequential decision-making models, revealing that NCO behavior can be approximated as a composition of classic heuristic variants.
...
TL;DR: The paper presents a study on fine-tuning pre-trained Transformer-based models for Quranic Automatic Speech Recognition (ASR) using advanced speech feature extraction methods, achieving a significant improvement in Word Error Rates (WER) and reducing training time.
...
Method: The paper uses pre-trained Transformer-based models (Wav2Vec2.0, HuBERT, and XLS-R) with self-supervised learning and fine-tunes them on a filtered Quranic dataset of over 870 hours of professional and user recitations.
...
TL;DR: The paper evaluates AI-assisted review systems for research papers, finding they can track human quality judgments, catch errors, and receive positive user feedback, but still have room for improvement.
...
Method: The study evaluates four review systems (two open-source, one proprietary, and a baseline) across six large language models (LLMs) on ICLR/NeurIPS papers and a custom error injection benchmark.

Result: AI review systems show promising results in tracking paper quality (up to 83.0% accuracy) and detecting errors (up to 71.6% recall), with room for improvement.

Conclusion: While AI-assisted review systems have limitations, they can already perform well in tracking human judgments, error detection, and user satisfaction, suggesting potential for future development and refinement.
...
TL;DR: The paper presents a foundational framework for integrating AI into traditional systems, defining four primitives of AI blended architecture and identifying two anti-patterns to guide engineers.


<details>
  <summary>Details</summary>
Motivation: The integration of generative models into traditional systems poses significant risks, and the field lacks foundational frameworks to mitigate these risks.

Method: The authors define four specific primitives of AI blended architecture and identify two overarching anti-patterns in the industry.

Result: A foundational framework for the successful integration of AI into traditional systems, enabling deterministic encapsulation of probabilistic models.

Conclusion: The framework provides a foundation for generative model providers to build next-generation interfaces and guides engineers in integrating AI into traditional systems.
...
TL;DR: The paper explores a future question answering forum system employing expert knowledge workers, focusing on system capacity, stability, and scheduler design.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the efficiency and capacity of question answering forums by leveraging expert knowledge workers and optimizing request-answer processes.

Method: The method involves modeling the request-answer process as a queuing system, calculating system capacity, designing schedulers to achieve capacity, and investigating the impact of expert collaboration.

Result: The results include the calculation of system capacity, design of efficient schedulers, and insights into how collaboration among experts can increase capacity.

Conclusion: The paper concludes by highlighting the potential for expert-based question answering forums to efficiently handle information requests while maintaining system stability.
...
Method: The ICT framework focuses on the distributional properties of token logits, using Jensen-Shannon (JS) divergence to identify critical tokens for guiding exploration, and selectively updating these tokens to regulate policy concentration.

Result: Empirical results show that ICT improves performance over baselines, with an average pass@4 improvement of 4.58% and a maximum gain of 14.9% across seven benchmarks.

Conclusion: The ICT framework effectively stabilizes the training landscape and improves LLM reasoning by preventing over-concentrated token generation and promoting effective exploration.
...
TL;DR: The paper presents AgentFinVQA, a multi-agent pipeline for financial chart question answering that combines auditability with on-premise deployability without significant accuracy compromise.


<details>
  <summary>Details</summary>
Motivation: Existing chart-QA agents are accuracy-focused and opaque, and most assume proprietary API access, which is not suitable for regulated settings where data privacy is crucial.
...
Conclusion: The results show that auditable, on-premise financial chart QA is practical, and the open-weights system keeps most of the accuracy gains while enabling full data residency.
...
TL;DR: The authors introduce ORAgentBench, a benchmark to evaluate autonomous agents on end-to-end operations research tasks. They find current agents are unreliable, with the best agent passing only 35.51% of tasks.


<details>
  <summary>Details</summary>
Motivation: To assess the capability of large language models to perform realistic operations research (OR) work as autonomous agents in executable environments.

Method: Introduction of ORAgentBench, a benchmark containing 107 human-reviewed tasks across diverse operational scenarios. Agents are evaluated on their ability to write and run solution code, with submissions assessed for schema validity, constraint feasibility, and objective quality.

Result: The best agent-model configuration passes only 35.51% of all tasks and 20.59% of hard tasks. Errors are primarily due to strategic weaknesses such as missed operational rules and weak feasible-solution construction.

Conclusion: Current autonomous agents are far from reliable OR practice. Progress requires moving beyond generating plausible optimization code to dependable, high-quality operational decision-making.
...
TL;DR: The paper introduces CombEval, a dynamic benchmark for evaluating combinatorial counting in large language models (LLMs), and assesses the performance of 11 LLMs in various settings.


<details>
  <summary>Details</summary>
Motivation: The motivation is to provide a systematic way to evaluate and understand the limitations of LLMs in combinatorial reasoning, specifically in solving natural-language counting problems with exact answers.

Method: The method involves using CombEval, a benchmark that generates controlled natural-language counting problems with solver-verified answers, to assess LLMs under direct and code-augmented settings.

Result: The result shows that LLMs are brittle on ordered objects, indistinguishable elements, relatively positional constraints, and nested object dependencies, with error analysis identifying failures in constraint interpretation and counting principles.

Conclusion: The conclusion is that CombEval provides a diagnostic testbed for studying when and why LLMs fail at combinatorial reasoning, and the code and generated benchmark suites are publicly available.
...
TL;DR: The paper introduces SEVRA, a selective verification method for test-time reasoning, which decides whether to verify or not the initial answer of a solver to improve accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: Test-time reasoning can be valuable but is not uniformly beneficial as it can lead to wasted computation or introduce harmful changes. The authors aim to optimize the deployment of verification resources.

Method: The authors propose SEVRA (Selective Verification for Reasoning Allocation), a serving-layer controller that decides whether to preserve the initial answer of a frozen solver or invoke active verification based on the solver's attempt state.

Result: SEVRA achieves significant improvements in accuracy and efficiency across various datasets. For example, on MathFive, it reaches 76.3% accuracy (vs 75.5% with always verifying) while reducing tokens by 26.8% and harmful flips from 2.2% to 1.0%. On GSM, it improves accuracy from 93.4% to 94.5% and reduces verification tokens by 91.2%.

Conclusion: The authors conclude that selective recovery (SEVRA) is useful but not always the best cost frontier. They suggest a deployment rule: tune the initial budget first, then use selective recovery when explicit checks, bounded retries, auditability, or regression-risk control are necessary.
...
<details>
  <summary>Details</summary>
Motivation: The increasing deployment of autonomous LLM agents in e-discovery has raised concerns about compounding errors across multi-step reasoning chains, which can lead to legal malpractice.
...
TL;DR: A new framework, TelcoAgent, is proposed for accurate, scalable, and explainable forecasting of Key Performance Measurements (KPMs) in 5G networks.
...
Method: TelcoAgent uses a foundation model-based framework with three components: (i) an automated pipeline to construct a 3GPP knowledge graph, (ii) a scalable, time-series foundation model-based prediction pipeline, and (iii) a reasoning and explanation pipeline.

Result: TelcoAgent demonstrates high forecasting accuracy for all 7 considered KPMs per cell across 200 cells, while delivering explainable insights and actionable instructions.
...
<details>
  <summary>Details</summary>
Motivation: Uncertainty estimation is essential for building trustworthy large language models, but existing black-box methods are fragmented and lack unified comparison.

Method: The paper systematically reviews and categorizes black-box UE methods into five categories, and benchmarks 24 representative methods across 4 models and 4 dataset settings.

Result: No single method dominates across all settings, but methods that reason over answer candidates and hybrid methods perform well under most conditions.

Conclusion: The paper provides practical guidance for developing future black-box UE methods and facilitates reproducible comparisons by releasing benchmark data and a unified evaluation framework.
...
Method: MetaResearcher framework includes an Evolving Virtual World, Discovery-Oriented Tasks, Self-Reflective Meta-Reward mechanism, and Heterogeneous Multi-Agent Swarm architecture.
...
Conclusion: MetaResearcher has the potential to substantially improve the training of deep research agents by addressing current limitations and promoting genuine research behaviors.
...
TL;DR: The paper proposes a framework called Multi-Agent Transactive Memory (MATM) for sharing and retrieving knowledge generated by agents in a decentralized system, improving task performance and reducing interaction steps.


<details>
  <summary>Details</summary>
Motivation: The decentralized deployment of LLM agents with diverse capabilities across tasks necessitates infrastructure for knowledge sharing across heterogeneous agent populations, as agent-generated artifacts are typically discarded or not shared.
...
Result: Experiments in interactive environments (ALFWorld and WebArena) demonstrate that retrieving trajectories from MATM improves downstream task performance and reduces interaction steps without coordination or joint training.

Conclusion: The results position MATM as a design pattern for population-level experience sharing in open agent ecosystems, enabling more efficient and effective task execution.
...
TL;DR: A novel Convolutional Neural Network (CNN) based method, eCNNTO, is proposed to accelerate density-based topology optimization by predicting near-optimal densities for elements, reducing iterations by up to 97%.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this work is to address the efficiency bottleneck in topology optimization (TO) caused by the need for a large number of iterations with finite element analysis, especially with dense meshes.

Method: The proposed method, eCNNTO, utilizes a CNN with residual connections to predict near-optimal densities for elements, building upon a Deep Belief Network (DBN) approach. A novel training strategy is introduced, using final stage density histories for the training dataset.

Result: eCNNTO demonstrates significant acceleration of the TO procedure, achieving up to 90% and 97% reduction of iterations in two and three dimensions, respectively. It also shows generalization capabilities across different boundary conditions, loading cases, design domain geometries, mesh resolutions, and non-design domains.

Conclusion: The proposed eCNNTO method effectively accelerates density-based topology optimization with high generalization capabilities, requiring only a small dataset for training.
...
TL;DR: The paper explores autotelic AI, where agents generate their own goals, and discusses its consequences, challenges, and potential frameworks.


<details>
  <summary>Details</summary>
Motivation: The current assumption in AI systems is that goals are predefined by designers. The authors aim to explore what happens when agents generate their own goals, leading to the field of autotelic AI.

Method: The authors discuss various concepts, including intrinsic motivation, resource-driven priors, causal-interventional learning, homeostasis, and embeddedness. They also propose a framework and explore its extensions in three directions: quantum formulation, philosophical reading, and LLM-based instantiation.

Result: The authors find that embeddedness is a necessary but not sufficient condition for autotelic agency. They also identify the challenge of how an agent generates and relativizes its own self to which goals are assigned.

Conclusion: The paper concludes that the deepest problem with autotelic AI is not goal generation, but self-generation and relativization. The authors provide a consolidated framework and explore its implications in various directions.
...
TL;DR: The authors propose a new framework for generating co-speech motions for humanoid robots, addressing the embodiment gap between human motion and robot constraints, resulting in improved speech-motion alignment and physical plausibility.


<details>
  <summary>Details</summary>
Motivation: The existing co-speech generation pipelines for humanoid robots are human-centric, causing a mismatch between human motion manifolds and humanoid embodiment constraints, disrupting embodiment consistency.
...
Result: The proposed frameworks substantially improve speech-motion alignment, physical plausibility, motion smoothness, inference efficiency, and real-time interaction capability, as demonstrated by extensive experiments and real-world humanoid deployment.

Conclusion: The authors conclude that embodiment-aware robot-native generation is a more effective approach for generating co-speech motions for humanoid robots, maintaining embodiment consistency throughout training and inference.
...
TL;DR: The authors propose a method to automatically generate a large-scale dataset called RAINbow to improve the performance of DialNav, a framework for evaluating dialog-execution loop in indoor navigation. They achieve state-of-the-art results by combining their dataset with two additional training strategies.


<details>
  <summary>Details</summary>
Motivation: The performance of DialNav is limited by a scarcity of training data (2K episodes), and the authors aim to address this limitation.

Method: The authors propose an automatic generation pipeline to construct the RAINbow dataset with 238K episodes and introduce two additional training strategies: Dual-Strategy Training and a localization model that leverages VLN knowledge.

Result: The authors' model substantially outperforms the baseline in success rate on both Val Seen (58.24, +89%) and Val Unseen (29.05, +100%) splits, establishing a new state of the art.

Conclusion: The authors conclude that their proposed method, including the RAINbow dataset and two additional training strategies, improves the performance of DialNav and establishes a new state of the art in indoor navigation.
...
<details>
  <summary>Details</summary>
Motivation: The current bottleneck in achieving general physical intelligence is the reliance on human supervision and algorithm engineering for robotic manipulation, which can be addressed by automating robotics research with a repeatable feedback loop.

Method: The authors propose a harness framework called ENPIRE, consisting of four core modules: Environment, Policy Improvement, Rollout, and Evolution, which instantiate a physical feedback routine for real-world policy improvement.

Result: ENPIRE enables coding agents to autonomously train policies to achieve a 99% success rate on challenging manipulation tasks, such as organizing a pin box, fastening a zip tie, and tool use.
...
TL;DR: The authors propose a new approach to reinforcement learning (RL) that challenges the conservative paradigm of relying on rollouts near the training distribution, instead introducing methods for reliable verification and diversified exploration.
...
Method: The authors introduce two main components: 'Reward as an Agent', an agentic reward framework that actively evaluates generated behaviors to provide robust reward signals, and 'Dynamic-Aware Rollout Diversification through DynDiff-GRPO', which diversifies trajectories and broadens state-action coverage.
...
Conclusion: The authors conclude that broader exploration can scale successfully when grounded in robust verification, and that their approach can effectively mitigate reward hacking while improving accuracy in RL.
...
TL;DR: The paper evaluates two multi-agent systems, DAG Plan and Execute and ReAct, in enterprise scenarios and proposes a Task Manager for continuous operation.


<details>
  <summary>Details</summary>
Motivation: Existing multi-agent systems assume discrete request-response workflows and are underexplored at enterprise scale, hindering continuous event monitoring and action across specialist agents.
...
Result: The results show that scale, not task complexity, dominates orchestration performance, with both architectures degrading at enterprise scale due to agent discovery noise; the Task Manager improves performance metrics.

Conclusion: The study highlights the importance of designing multi-agent systems for continuous operation at enterprise scale and the effectiveness of the proposed Task Manager in improving performance.
...
TL;DR: The paper explores using symbolic proof assistants like Lean as oracles for providing structured, fine-grained feedback in reinforcement learning for formal reasoning, improving tactic-level supervision.


<details>
  <summary>Details</summary>
Motivation: The motivation is to bridge the gap between structured processes in formal reasoning and unstructured rewards in traditional reinforcement learning, aiming for dense and sound feedback.

Method: The method involves using the Lean proof assistant to supply verified feedback at both outcome and tactic levels during training, incorporating structured rewards into a customized reinforcement learning objective.
...
Conclusion: The study concludes that symbolic proof assistants can serve not only as verifiers but also as process-level reward oracles during training, paving the way for more reliable reinforcement learning frameworks in formal reasoning.
...
Conclusion: The proposed framework extends beyond images to real-world scientific domains, providing a useful tool for data editing.
...
TL;DR: This paper proposes a novel reinforcement learning approach that combines a continuous action space with a multi-head attention mechanism and the Soft Actor-Critic algorithm to optimize additive manufacturing process parameters and minimize defects like porosity.


<details>
  <summary>Details</summary>
Motivation: Traditional reinforcement learning approaches have limitations in high-precision manufacturing tasks due to slow convergence and susceptibility to local optima, mainly because they use discrete action spaces.

Method: The study employs a continuous action space with a novel architecture integrating a multi-head attention mechanism with the Soft Actor-Critic (SAC) algorithm to enhance the agent's ability to capture subtle variations in low-dimensional input features.

Result: The proposed methodology achieves a convergence value of 322.79 within 14 episodes, outperforming existing approaches like DQN, PPO, TD3, and vanilla SAC in terms of faster convergence and higher final reward values.

Conclusion: The approach demonstrates effectiveness in porosity prediction and process parameter optimization in laser powder bed fusion, showing potential for high-precision manufacturing tasks.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for open-ended deep research have limitations in handling dynamic outline updates, leading to scaffold drift and delayed feedback.

Method: The authors propose ScaffoldAgent, a utility-guided framework that models outline evolution as a structured decision process with three operations: Expansion, Contraction, and Revision.
...
Conclusion: The proposed ScaffoldAgent framework effectively addresses the challenges of dynamic outline optimization in open-ended deep research, leading to improved report generation and factual accuracy.
...
TL;DR: The paper presents a subject-aware prompting system for personalized education using LLMs, which adapts to diverse academic disciplines. The system outperforms static baselines in simulation and real-world tests with high-school students, improving instructional efficiency and exercise conversion rates.
...
Method: The paper develops a system with subject-aware prompting based on 14 pedagogical features extracted from raw transcripts. A prompt routing model is trained in a simulation environment and then deployed for online adaptation with actual high-school students.

Result: The simulation benchmark shows the router outperforming two static baselines. A/B testing with 359 students shows that the adaptive prompt selection mechanism improves instructional efficiency, maintains pedagogical quality, and reduces interactions. The stochastic router leads to a higher exercise conversion rate of 28.1%.

Conclusion: The paper concludes that the subject-aware prompting system can effectively personalize education and improve instructional efficiency and exercise conversion rates.
...
Method: RACL uses a reasoning agent to observe operational memory, reason over past behavior, formulate bounded hypotheses, test interventions, evaluate outcomes, and apply guardrails to control the optimizer's internal search behavior.
...
Conclusion: RACL is a method that enables a reasoning agent to discover, validate, consolidate, and explain algorithmic control rules for metaheuristics, leading to improved performance in optimization problems.
...
Result: The best-performing LLM achieves an average score of 49.5% across three metrics (geometric accuracy, semantic validity, and topological consistency), with no model fully solving more than 3.4% of tasks.
...
TL;DR: The paper proposes a novel framework called Modular-Sparsity Synchronization (ModSync) to improve the training of physics-informed neural networks (PINNs) by addressing a capacity-induced failure mode that hinders convergence.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the fragility of training PINNs, specifically the deterioration of conflict-averse optimization schemes as model capacity increases, leading to a capacity-induced failure mode.
...
Result: The results show that ModSync consistently prevents capacity-driven failures, sustains robust cross-objective coupling, and achieves state-of-the-art accuracy across diverse PDE benchmarks.

Conclusion: The conclusion is that ModSync is an effective framework for improving the training of PINNs, addressing the capacity-induced failure mode, and achieving state-of-the-art accuracy.
...
TL;DR: The paper proposes a novel hypergraph-based implicit semantic reasoning framework, HISR, to improve semantic communication efficiency and accuracy by capturing complex multi-entity relationships.


<details>
  <summary>Details</summary>
Motivation: Existing semantic-aware communication solutions neglect higher-order implicit correlations, reducing semantic expressiveness and making inference susceptible to ambiguity and performance degradation.

Method: The proposed HISR framework uses hypergraphs to represent complex relationships among semantic knowledge entities, mapping entities and relations into dedicated semantic subspaces.

Result: HISR achieves up to a 36.6% improvement in implicit semantic interpretation accuracy over state-of-the-art benchmarks.

Conclusion: The proposed HISR framework effectively addresses the limitations of existing solutions, providing robust semantic inference and improving communication efficiency and accuracy.
...
TL;DR: The study finds that psychological profiles assigned to large language models (LLMs) using human-designed instruments are largely measurement artifacts due to a directional response bias.
...
Conclusion: The apparent psychological profiles of LLMs are artifacts of the instrument used, not properties of the models themselves, and dedicated assessments centered on response orthogonality are needed.
...
TL;DR: The paper introduces the Rule Violation Score (RVS), a metric to evaluate the logical consistency of machine learning models with predefined rules, which is crucial in high-stakes applications.
...
Method: The authors propose the Rule Violation Score (RVS) to quantify the extent to which a predictive model respects logical rules, differentiating between hard and soft rules, and evaluate it on various benchmarks.

Result: The results show that models with comparable predictive accuracy can have substantially different levels of logical compliance, highlighting the importance of RVS in capturing differences in model behavior.

Conclusion: The RVS provides a complementary evaluation metric to assess the logical consistency of machine learning models, which can help identify poorly defined rules and improve model behavior in high-stakes applications.
...
TL;DR: The paper discusses the potential of reinforcement learning to improve game AI, making in-game characters more believable and relatable, and proposes a framework for training reinforcement learning models for game AI.


<details>
  <summary>Details</summary>
Motivation: The limitations of current game AI systems lead to frustration and break the illusion of realism in games, and the authors believe that machine learning models, specifically reinforcement learning, can help create more believable and authentic characters.

Method: The authors propose a framework for training reinforcement learning models with requirements suited towards game AI and game development, and discuss examples of games with reinforcement learning-augmented game AI.

Result: The authors identify bottlenecks and hard problems in deploying player-facing machine learning agents in modern games, and suggest promising research directions to accelerate the adoption of machine learning in game AI.

Conclusion: The authors envision more applications of reinforcement learning for game AI in the future, but note that current research limitations hinder broad deployment across game genres.
...
TL;DR: A new framework, QMFOL, is proposed for generating deductive reasoning tasks with controllable complexity, and a benchmark, QMFOLBench, is built to evaluate the reasoning capabilities of large language models.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for evaluating deductive reasoning in large language models lack fine-grained control over logical complexity and struggle to balance semantic diversity with logical consistency.

Method: QMFOL uses an automated framework to generate monadic first-order logic reasoning tasks with quantifiable and controllable complexity, constructs formal logical structures, and translates them into natural language via LLMs.

Result: Evaluations on six large reasoning models and two LLMs show that performance degrades and computational overhead increases with rising logical complexity, and models exhibit sensitivity to semantic variation.
...
TL;DR: The authors propose a framework for measuring intelligence, defining it as the ability to amplify rare but valid futures, and show that recursive self-simulation is necessary and nearly sufficient for high thermodynamic intelligence.
...
Method: The authors use a thermodynamic measure to quantify the amplification of rare-valid futures, and derive necessity and conditional near-sufficiency statements connecting this architecture to intelligence.

Result: The authors show that recursive self-simulation is necessary and nearly sufficient for high thermodynamic intelligence, and that the achievable lift approaches the actuation-limited optimum when rare-valid fidelity is high and the simulation contains an effective policy.
...
Method: MAMO, a multi-agent system for multi-objective constrained optimization, which formulates the selection of reward weights as a learning problem.

Result: Decoupling task execution from objective design, providing a step towards more autonomous and robust RL-based solutions.

Conclusion: MAMO offers a promising approach to tackle the balancing problem in constrained optimization problems in dynamic environments.
...
TL;DR: A novel framework MACR is proposed to resolve knowledge conflicts in large language models (LLMs) by incorporating a multi-agent reasoning approach.


<details>
  <summary>Details</summary>
Motivation: Existing approaches to integrating external knowledge in LLMs assume that either the model or the provided context is reliable, but overlook the possibility of errors in both sources.

Method: The MACR framework uses an adaptive knowledge assessment and retrieval approach, followed by an inductive multi-agent reasoning framework with three specialized agents to resolve inconsistencies across contexts.
...
Conclusion: The proposed MACR framework offers a more effective and interpretable approach to resolving knowledge conflicts in LLMs.
...
TL;DR: Automated scoring of student-generated scientific drawings using a Vision Transformer model with a confidence-aware framework.
...
Method: Using a Vision Transformer (ViT) model with parameter-efficient adaptation and a confidence-aware scoring framework.

Result: Improved scoring reliability and a practical trade-off between automated coverage and scoring risk.

Conclusion: The proposed approach enables trustworthy educational assessment by providing a reliable and efficient way to score student-generated scientific drawings.
...
TL;DR: Lagrange is proposed as an open-vocabulary, computationally sparse framework for end-to-end autonomous driving in complex environments, addressing the trade-off between representational efficiency and generalization capacity.


<details>
  <summary>Details</summary>
Motivation: The need for autonomous driving systems to generalize to anomalous scenarios and produce kinematically valid trajectories in complex, open-world environments, while current paradigms face limitations in representational efficiency and generalization capacity.

Method: Lagrange uses Masked Latent Fields (MLF) and Vision-Language Models (VLMs) to encode class-agnostic object proposals into continuous semantic visual tokens, with an intent-driven masked cross-attention module for decision-making.

Result: Lagrange demonstrates a promising framework for robust, interpretable, and kinematically feasible open-world autonomy through extensive offline evaluations on nuScenes and CODA benchmarks.

Conclusion: Lagrange addresses the limitations of existing paradigms in autonomous driving by providing an open-vocabulary, computationally sparse framework that balances representational efficiency and generalization capacity.
...
TL;DR: The paper proposes a novel approach for vibration-based Intelligent Fault Diagnosis Systems using Deep Transfer Learning, addressing data scarcity by leveraging non-linearities in systems to generate images for pre-trained CNNs.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome the challenge of obtaining large amounts of labelled data for building Intelligent Fault Diagnosis Systems, especially when dealing with machine or structure faults.

Method: The method involves a periodic multi-excitation level procedure to produce images from intrinsic non-linearities of real-world systems, which are then analyzed by pre-trained Convolutional Neural Networks. A new data visualization method and its augmentation technique are also proposed.
...
Conclusion: The paper concludes that the proposed approach allows for the efficient design of vibration-based Intelligent Fault Diagnosis Systems using Deep Transfer Learning, even under strong data scarcity conditions.
...
TL;DR: This paper proposes SoftSkill, a method for deploying compact, trainable 'soft skills' that can refine a frozen language model's behavior for specific tasks, improving performance over traditional Markdown-based skill encoding.


<details>
  <summary>Details</summary>
Motivation: The current method of deploying agent skills as natural-language Markdown files is indirect and inefficient, requiring a frozen language model to translate the file into behavior for each task instance.

Method: The authors propose SoftSkill, which uses a trainable soft delta to refine a compact continuous context object initialized by a natural-language skill, and deploys it as a latent behavioral prior at inference time.

Result: SoftSkill improves performance over no-skill prompting by 8.3-42.1 points on various benchmarks, and outperforms SkillOpt by 5.2-12.5 points, while using a more compact representation.

Conclusion: The results suggest that some task skills are better treated as compact latent controls over a frozen model's behavior, rather than as additional Markdown to be reinterpreted at inference time.
...
TL;DR: The paper explores mining explicit skill libraries from interaction data to improve downstream policies. It proposes a three-stage pipeline but finds that while the mined skills are readable, they don't reliably improve cross-domain policy performance.


<details>
  <summary>Details</summary>
Motivation: To investigate whether explicit skill libraries can be mined from interaction data to improve downstream policies, making computer-using agents easier to inspect.
...
Result: The mined clusters show high purity (5/8 clusters with ≥0.95 purity) but do not significantly improve policy performance: GRPO improves skill-step accuracy from 18.5% to 20.5% and underperforms on key metrics.

Conclusion: Trajectory mining can expose inspectable skill structure, but current methods are insufficient for reliable cross-domain policy improvement, suggesting the need for better boundary detection, segment representation, and reward models.
...
TL;DR: The study identifies a limitation in current FP4 training methods using non-uniform formats like E2M1, which causes training instability due to 'Shrinkage Bias'. It proposes UFP4, a uniform 4-bit training recipe that outperforms E2M1-based methods.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of current FP4 hardware paths and recipes for LLM pretraining, which are centered on E2M1 data elements and suffer from Shrinkage Bias.

Method: The study analyzes the effect of non-uniform formats on training stability and proposes UFP4, a uniform 4-bit training recipe that applies RHT to all three training GEMMs while restricting stochastic rounding to dY alone.
...
TL;DR: The study proposes an attention-guided deep learning framework for sperm morphology classification, achieving high accuracy and interpretability.


<details>
  <summary>Details</summary>
Motivation: The lack of interpretability in deep learning models for sperm morphology analysis limits their clinical adoption.
...
Result: The model achieves accuracies of 90.2% and 93.9% (macro F1 scores of 0.913 and 0.948) on the SMIDS and HuSHem public datasets.
...
TL;DR: IVF pregnancy rates can be better predicted by analyzing high-resolution laboratory environmental data, leading to improved modeling accuracy.
...
Method: Engineering 55 context-aware temporal features from raw sensor data and using a hierarchical Bayesian Beta regression model for analysis.

Result: The approach achieves a 1.27% cross-validated prediction error and a 64% error reduction for the 35-39 age group over a naive baseline.

Conclusion: Structured environmental monitoring contains clinically meaningful, transferable signal that can improve IVF pregnancy rate predictions.
...
Result: The study finds that benign and harmful demonstrations are not interchangeable, and their effects on model behavior depend on the model, demonstration composition, and training methodology. Specifically, benign demonstrations can either reduce or increase harmful compliance depending on the model, and preference optimization is critical in preventing benign demonstrations from increasing harmful compliance.
...
TL;DR: The paper introduces Multi-LCB, a new benchmark for evaluating large language models (LLMs) across twelve programming languages, addressing the limitation of the existing LiveCodeBench (LCB) benchmark which is restricted to Python.
...
Method: The authors introduce Multi-LCB, a benchmark that transforms Python tasks from LCB into equivalent tasks in other languages, preserving LCB's contamination controls and evaluation protocol, and evaluate 24 LLMs on Multi-LCB.

Result: The results show evidence of Python overfitting, language-specific contamination, and substantial disparities in multilingual performance, establishing Multi-LCB as a rigorous new benchmark for multi-programming-language code evaluation.

Conclusion: The paper concludes that Multi-LCB addresses LCB's primary limitation and exposes critical gaps in current LLM capabilities, enabling systematic assessment of cross-language code generation competence.
...
TL;DR: The paper introduces LedgerAgent, a method for tool-calling agents to maintain task states and obey domain policies, improving performance in customer-service domains.


<details>
  <summary>Details</summary>
Motivation: The authors identify two common failure modes in standard agents: (1) agents may ground decisions in stale, missing, or incorrect information, and (2) syntactically valid tool calls may still violate domain policies. They aim to address these issues by developing a method for explicit state management.

Method: The authors propose LedgerAgent, which maintains observed task states in a separate ledger and renders the states into the prompt. The ledger is also used to check state-dependent policy constraints before environment-changing tool calls are executed.

Result: The results show that LedgerAgent improves average pass@k over a standard prompt-based tool-calling approach across four customer-service domains and a mixed panel of open- and closed-weight models, with the largest gains under stricter multi-trial consistency metrics.

Conclusion: The paper concludes that LedgerAgent is an effective method for tool-calling agents to maintain task states and obey domain policies, leading to improved performance in customer-service domains.
...
TL;DR: FlowEdit, a life-long adaptation framework for flow-matching TTS, enables corrections to pronunciation errors on out-of-vocabulary proper nouns via latent conditioning edits.


<details>
  <summary>Details</summary>
Motivation: Flow-matching TTS systems have remarkable zero-shot quality but struggle with pronunciation errors on out-of-vocabulary proper nouns, requiring retraining to correct.

Method: FlowEdit optimizes token-level perturbations in the text embedding space and stores corrections in a Modern Hopfield Network for fuzzy morphological matching.

Result: FlowEdit reduces target-word Phoneme Error Rate by 92.7% relative to the zero-shot baseline while maintaining general-speech quality, with corrections completing in approximately 15 seconds on a single GPU.

Conclusion: FlowEdit provides an efficient and effective solution for correcting pronunciation errors in flow-matching TTS systems, enabling life-long adaptation without requiring model retraining.
...
TL;DR: DeepSWIP introduces a single-world counterfactual semantics for DeepProbLog programs, enabling efficient counterfactual reasoning.


<details>
  <summary>Details</summary>
Motivation: Counterfactual reasoning requires a causal semantics for interventions and evidence, which standard associative inference in neurosymbolic systems like DeepProbLog lacks.

Method: DeepSWIP uses neural materialization to transform fixed-context neural predicates into ordinary ProbLog choices, applies Single World Intervention Programs (SWIPs), and computes counterfactuals via weighted model counting (WMC) over a single transformed program.

Result: DeepSWIP achieves exactness relative to the learned materialized FCM under certain assumptions and demonstrates a 2.14x inference speedup. Experiments validate the approach and highlight the importance of correct scoping in causal estimation.

Conclusion: DeepSWIP provides a novel approach to counterfactual reasoning in neurosymbolic systems, offering improved efficiency and accuracy in causal estimation tasks.
...
TL;DR: This paper proposes a method to analyze how individual words in style captions influence acoustic output in text-to-speech systems, and applies it to a specific TTS model.


<details>
  <summary>Details</summary>
Motivation: Understanding how individual words in style captions control voice characteristics is crucial for improving controllability and diagnosing failure modes in expressive TTS.
...
Conclusion: This study provides new insights into how natural language influences cross-attention in speech diffusion models, which can help improve controllability and diagnosis of failure modes in expressive TTS.
...
TL;DR: This paper studies the calibration of mixture-of-experts (MoE) models under distribution shift, focusing on the interaction between routing mechanisms and expert-level calibration.


<details>
  <summary>Details</summary>
Motivation: Understanding and trusting reported probabilities requires calibration, which aligns a model's predictive uncertainty with empirical outcome frequencies. Recent work shows that enforcing calibration at the predictor level improves ensemble accuracy and calibration, especially in MoE models.

Method: The authors analyze hard-routed and soft-routed MoE models under distribution shift, proposing an adversarial reweighting method to penalize calibration errors of the routed aggregate.

Result: The proposed method improves the accuracy-calibration tradeoff on average and on difficult subsets of the data, across model classes, prediction tasks, and distribution shifts.

Conclusion: Expert calibration ensures overall model calibration under distribution shift in hard-routed MoE models, but not in soft-routed models. The proposed adversarial reweighting method addresses this limitation, improving calibration and accuracy.
