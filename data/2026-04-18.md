...
Method: A computational architecture called QualiaNet, which uses disparity maps to simulate human stereo experience and a CNN to estimate distance from disparity gradients.

Result: The network can recover distance from disparity gradients alone, validating the proposed approach.

Conclusion: The proposed two-stage architecture can effectively estimate 3D scene properties from stereo depth experience.
...
<details>
  <summary>Details</summary>
Motivation: To advance the capabilities of world modeling by accommodating diverse input modalities and producing high-fidelity 3D world representations.

Method: A four-stage method: Panorama Generation, Trajectory Planning, World Expansion, and World Composition, utilizing various models like HY-Pano 2.0, WorldNav, WorldStereo 2.0, and WorldMirror 2.0.
...
Conclusion: The proposed HY-World 2.0 framework significantly advances the field of 3D world modeling by supporting multi-modal inputs and generating high-quality 3D scenes, with open-source availability to foster further research.
...
<details>
  <summary>Details</summary>
Motivation: Existing multi-view generation methods require photographs or text, while sketch-to-3D methods need multiple views or costly per-scene optimization. The authors aim to tackle the challenge of generating multi-view scenes from a single freehand sketch.

Method: The authors propose a framework that uses a curated dataset of sketch-to-multiview samples, Parallel Camera-Aware Attention Adapters (CA3), and a Sparse Correspondence Supervision Loss (CSL) to synthesize all views in a single denoising process.

Result: The approach significantly outperforms state-of-the-art two-stage baselines, improving realism (FID) by over 60% and geometric consistency (Corr-Acc) by 23%, while providing up to a 3.7x inference speedup.

Conclusion: The proposed method successfully addresses the challenges of generating geometrically consistent multi-view scenes from a single freehand sketch, achieving superior performance compared to existing methods.
...
TL;DR: The paper introduces DharmaOCR, a pair of small language models for structured OCR, and a benchmark for evaluating OCR models. It proposes a new evaluation protocol and applies Direct Preference Optimization (DPO) to reduce text degeneration.


<details>
  <summary>Details</summary>
Motivation: The authors aim to improve the performance of OCR models by jointly optimizing transcription quality, generation stability, and inference cost. They also seek to address the issue of text degeneration in OCR models.

Method: The authors propose using Direct Preference Optimization (DPO) with Supervised Fine-Tuning (SFT) to reduce degeneration rates in OCR models. They also use a unified evaluation protocol that measures fidelity, structure, and text degeneration.

Result: The resulting models, DharmaOCR Full and Lite, achieve state-of-the-art results on the DharmaOCR-Benchmark, with scores of 0.925 and 0.911, and degeneration rates of 0.40% and 0.20%, respectively. AWQ quantization reduces per-page cost by up to 22% with negligible quality loss.

Conclusion: The paper concludes that the proposed approach effectively reduces text degeneration in OCR models while preserving or improving extraction quality. The resulting models offer a strong quality-cost trade-off compared to proprietary OCR APIs and open-source alternatives.
...
Method: A YOLO-based pose estimator and Random Forest classifier are used to extract body keypoints and compute kinematic and interaction features.
...
Conclusion: The proposed system is feasible for proactive, on-device robbery detection.
...
<details>
  <summary>Details</summary>
Motivation: Standard vulnerability indices are too coarse and don't provide enough insight into specific rural risk contexts, which are influenced by local conditions such as housing quality and road access.

Method: SatBLIP combines satellite image analysis with AI-generated captions and a vision-language model (BLIP) to identify features from satellite images, which are then used to estimate SVI.

Result: The framework successfully identifies key attributes that drive predictions, such as roof condition and vegetation, and enables the creation of interpretable maps of rural risk environments.

Conclusion: SatBLIP offers a more detailed and accurate approach to understanding rural environmental risks, which can help improve vulnerability assessments and planning in these areas.
...
TL;DR: A dataset and model are introduced for predicting multisensory experiences of food from images, including taste, smell, texture, and sound.


<details>
  <summary>Details</summary>
Motivation: Prior research has focused on recognition tasks, but image-based prediction of multisensory experience remains largely unexplored.

Method: A human-annotated dataset (FoodSense) is created with 66,842 participant-image pairs, and a vision language model (FoodSense-VL) is trained to predict multisensory ratings and explanations from food images.

Result: The model can produce multisensory ratings and grounded explanations directly from food images, connecting cognitive science findings with modern instruction tuning for multimodal models.

Conclusion: The work highlights the importance of considering multisensory experiences in vision language research and shows that popular evaluation metrics may be insufficient for visually sensory inference.
...
TL;DR: Zero-ablation overstates dependence on exact register content in vision transformers; performance depends on plausible register-like activations rather than exact image-specific values.


<details>
  <summary>Details</summary>
Motivation: To investigate the functional indispensability of registers in vision transformers, specifically in DINOv2+ and DINOv3 models.

Method: Using zero-ablation, mean-substitution, noise-substitution, and cross-image register-shuffling to probe token function in vision transformers.

Result: Zero-ablation causes large drops in performance, but replacement controls (mean-substitution, noise-substitution, and cross-image register-shuffling) preserve performance across tasks, suggesting that performance depends on plausible register-like activations rather than exact image-specific values.

Conclusion: Registers in vision transformers are not functionally indispensable, and their role can be replaced by plausible activations; they buffer dense features from [CLS] dependence and are associated with compressed patch geometry.
...
TL;DR: The paper proposes a new image annotation methodology that combines knowledge representation, NLP, and computer vision to reduce annotator subjectivity and improve computer vision task performance.


<details>
  <summary>Details</summary>
Motivation: The semantic gap problem in object recognition datasets leads to complex mappings between visual data and linguistic descriptions, causing bias that affects computer vision task performance.

Method: The proposed methodology integrates knowledge representation, NLP, and computer vision techniques, using an interactive crowdsourcing framework that guides image annotation by visual properties.

Result: Experiments demonstrate the effectiveness of the proposed methodology.

Conclusion: The proposed methodology can reduce annotator subjectivity and improve computer vision task performance by applying visual property constraints.
...
TL;DR: A novel self-supervised learning approach, DAGMaN, is proposed for medical images, integrating attention-guided masking with a noisy teacher to improve feature representation and downstream task performance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of current self-supervised learning methods for medical images, specifically information leakage and reduced effectiveness due to contextual similarity of neighboring patches.

Method: The method involves introducing an attention-guided masking mechanism within a co-distillation learning framework, and integrating a noisy teacher to preserve high attention head diversity, termed DAGMaN.

Result: The results demonstrate the capability of DAGMaN on multiple tasks, including full- and few-shot lung nodule classification, immunotherapy outcome prediction, tumor segmentation, and unsupervised organs clustering.

Conclusion: The proposed DAGMaN approach shows improved performance in various medical image analysis tasks, by effectively reducing information leakage and increasing the difficulty of self-supervised learning pretraining, while preserving high attention head diversity.
...
TL;DR: A new framework, H2VLR, is proposed for few-shot anomaly detection in images, leveraging Vision-Language Models to improve performance by considering structural dependencies and global consistency.
...
Method: The proposed method, Heterogeneous Hypergraph Vision-Language Reasoning (H2VLR), reformulates FSAD as a high-order inference problem of visual-semantic relations. It models visual regions and semantic concepts in a unified hypergraph.
...
Conclusion: The proposed H2VLR framework offers an improved approach to few-shot anomaly detection by integrating structural dependencies and global consistency through a unified hypergraph model, leading to superior performance in industrial and medical applications.
...
TL;DR: The paper addresses the performance paradox in Omni-modal Large Language Models (Omni-MLLMs) where unimodal baselines often outperform multimodal models. It proposes a new framework called Chain of Modality (CoM) that dynamically orchestrates multimodal fusion to improve performance.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the performance paradox observed in Omni-MLLMs, where unimodal baselines frequently outperform joint multimodal inference, and to improve the generalization capabilities of multimodal models.

Method: The paper proposes a new framework called Chain of Modality (CoM), which adaptively orchestrates input topologies and switches among parallel, sequential, and interleaved pathways to neutralize structural biases. CoM also bifurcates cognitive execution into two task-aligned pathways: 'Direct-Decide' and 'Reason-Decide'.

Result: The proposed CoM framework achieves robust and consistent generalization across diverse benchmarks, operating in either a training-free or a data-efficient SFT setting.

Conclusion: The paper concludes that the proposed CoM framework effectively resolves the functional rigidity of current multimodal models and improves their performance and generalization capabilities.
...
TL;DR: The authors propose FreqTrack, a frequency-aware RGBE tracking framework that improves tracking performance by leveraging frequency-domain transformations and enhancing event data features.


<details>
  <summary>Details</summary>
Motivation: Existing single-modal RGB trackers struggle in complex dynamic scenes, and current RGB-event fusion methods do not fully exploit the unique characteristics of event data.

Method: The authors design a Spectral Enhancement Transformer (SET) layer and a Wavelet Edge Refinement (WER) module to enhance and select frequency-domain features from RGB and event data.
...
Conclusion: The proposed frequency-aware RGBE tracking framework effectively improves tracking performance by leveraging frequency-domain modeling.
...
TL;DR: A low-cost, fluorescence-based optical system is developed for detecting microorganisms and molecules using a smartphone camera and 96-well plates.


<details>
  <summary>Details</summary>
Motivation: To create a low-cost alternative to conventional microplate reading machines for detecting fluorescence in diluted samples.

Method: Using a smartphone camera as the optical detector, and determining the relationship between image color in RGB color space and molar concentration of the fluorescence specimen.

Result: The device can detect fluorescence within a sample without expensive elements, comparing to conventional machines like Perkin Elmer Victor Machine.

Conclusion: The developed system offers a low-cost solution for detecting microorganisms and molecules, leveraging a smartphone camera and conventional 96-well plates.
...
TL;DR: WILD-SAM, a novel framework, adapts Segment Anything Model (SAM) for high-precision landslide detection on wrapped interferograms, addressing phase ambiguity and coherence noise.


<details>
  <summary>Details</summary>
Motivation: Detecting slow-moving landslides from wrapped InSAR interferograms is crucial for geohazard monitoring, but faces challenges due to phase ambiguity and complex coherence noise.

Method: WILD-SAM integrates a Phase-Aware Mixture-of-Experts (PA-MoE) Adapter and a Wavelet-Guided Subband Enhancement (WGSE) strategy to adapt SAM for wrapped phase data.

Result: WILD-SAM achieves state-of-the-art performance on ISSLIDE and ISSLIDE+ benchmarks, outperforming existing methods in target completeness and contour fidelity.

Conclusion: WILD-SAM offers a powerful solution for high-precision landslide detection on wrapped interferograms, effectively addressing the challenges of phase ambiguity and coherence noise.
...
TL;DR: A framework for controllable emotion in 3D head avatar reconstruction is proposed, enabling independent emotion manipulation across identities.


<details>
  <summary>Details</summary>
Motivation: Existing methods implicitly handle emotion in 3D head avatar reconstruction, making it difficult to control independently.

Method: A dual-path modulation mechanism is used, consisting of geometry modulation and appearance modulation, to inject emotion into feed-forward architectures.

Result: The framework preserves reconstruction and reenactment fidelity while allowing controllable emotion transfer, disentangled manipulation, and smooth emotion interpolation.

Conclusion: The proposed framework advances expressive and scalable 3D head avatars with explicit emotion control.
...
Method: The proposed method integrates multi-view object priors, uses a dual-path view-consistent conditioning mechanism, and employs a quality-aware weighting mechanism and an Integration-Aware Consistency Module.

Result: Experimental results show that the proposed solution significantly improves the quality of video object insertion, providing stable and realistic integration.

Conclusion: The proposed solution effectively resolves occlusion and boundary artifacts while maintaining temporal continuity across frames, providing stable and realistic integration of objects into existing videos.
...
TL;DR: The NTIRE 2026 image super-resolution challenge aims to develop effective super-resolution solutions, with two tracks for pixel-wise fidelity and visual realism.
...
Method: The challenge includes two tracks: a restoration track using PSNR for evaluation and a perceptual track using a perceptual score for evaluation.

Result: 31 teams submitted valid entries out of 194 participants, providing insights into current progress and future directions in image super-resolution.

Conclusion: The challenge provides a unified benchmark for image super-resolution and offers insights into current progress and future directions.
...
TL;DR: A one-step diffusion framework, DVFace, is proposed for video face restoration, achieving superior restoration quality, temporal consistency, and identity preservation.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion-based video face restoration methods rely on generic priors and multi-step sampling, limiting facial adaptation and efficiency. One-step diffusion for faithful facial recovery and temporally stable outputs is challenging.

Method: DVFace, a one-step diffusion framework, uses a spatio-temporal dual-codebook design to extract facial priors and an asymmetric spatio-temporal fusion module to inject priors into the diffusion backbone.
...
Conclusion: The proposed DVFace framework effectively addresses the limitations of existing video face restoration methods, providing a more efficient and effective solution for enhancing degraded face videos.
...
TL;DR: Proposed SEPatch3D, a novel framework that accelerates Vision Transformer-based 3D object detectors by dynamically adjusting patch sizes, selecting informative patches, and enhancing features, achieving up to 57% faster inference and 20% higher efficiency while preserving detection accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing token compression strategies for Vision Transformer-based 3D object detectors often discard informative background cues, disrupt contextual consistency, and lose fine-grained semantics, negatively affecting 3D detection.

Method: SEPatch3D framework, which includes Spatiotemporal-aware Patch Size Selection (SPSS), Informative Patch Selection (IPS), and Cross-Granularity Feature Enhancement (CGFE) to dynamically adjust patch sizes, select informative patches, and enhance features.

Result: SEPatch3D achieves up to 57% faster inference than the StreamPETR baseline and 20% higher efficiency than the state-of-the-art ToC3D-faster, while preserving comparable detection accuracy on the nuScenes and Argoverse 2 validation sets.

Conclusion: The proposed SEPatch3D framework effectively accelerates Vision Transformer-based 3D object detectors while preserving detection accuracy, making it a promising solution for real-time 3D object detection applications.
...
TL;DR: A new framework called AVR is proposed to address the issue of overthinking in visual reasoning models by adaptively choosing the most efficient reasoning format.
...
Method: The AVR framework decomposes visual reasoning into three cognitive functions and enables models to dynamically choose among three response formats, trained with FS-GRPO.

Result: AVR reduces token usage by 50-90% while maintaining overall accuracy, especially in perception-intensive tasks.

Conclusion: Adaptive visual reasoning can effectively mitigate overthinking in VRMs.
...
<details>
  <summary>Details</summary>
Motivation: Deepfake detectors struggle to generalize to new image synthesis techniques, particularly diffusion models, which generate highly photorealistic forgeries that can evade detectors trained on GAN-based forgeries.

Method: The ANL framework integrates a pre-trained diffusion model into the deepfake detection pipeline to guide the learning of more robust features, using the diffusion model's denoising process to expose subtle artifacts and an attention-guided mechanism to focus on globally distributed discrepancies.

Result: The ANL method significantly outperforms existing methods on multiple benchmarks, achieving state-of-the-art accuracy in detecting diffusion-generated deepfakes, and boosts generalization performance without introducing additional overhead during inference.

Conclusion: The paper concludes that diffusion noise provides a powerful signal for generalizable deepfake detection, and the proposed ANL framework offers a effective solution to improve the generalization of deepfake detectors.
...
<details>
  <summary>Details</summary>
Motivation: Existing deepfake detection approaches rely on isolated facial attributes and do not fully exploit multi-modal feature representations, posing threats to cybersecurity and information authenticity.

Method: M3D-Net uses a dual-stream architecture with a self-supervised 3D facial reconstruction module, a 3D Feature Pre-fusion Module (PFM), and a Multi-modal Fusion Module (MFM) with attention mechanisms.

Result: The approach achieves state-of-the-art performance in detection accuracy and robustness on multiple public datasets, outperforming existing methods and showing strong generalization.

Conclusion: The proposed M3D-Net effectively addresses the challenges in deepfake detection by leveraging multi-modal 3D facial feature reconstruction, demonstrating superior performance and robustness.
...
TL;DR: Proposed TurboTalk, a two-stage distillation framework that compresses a multi-step audio-driven video diffusion model into a single-step generator, achieving 120x faster inference while maintaining high quality.
...
Method: Two-stage progressive distillation framework: 1) Distribution Matching Distillation to obtain a strong and stable 4-step student, 2) progressive reduction of denoising steps from 4 to 1 through adversarial distillation with progressive timestep sampling and self-compare adversarial objective.
...
TL;DR: The authors propose MapSR, a prompt-driven framework for high-resolution land-cover mapping that leverages low-resolution labels and reduces computational cost.


<details>
  <summary>Details</summary>
Motivation: High-resolution land-cover mapping is constrained by the high cost of dense high-resolution annotations, and existing weakly supervised methods have substantial computational costs.

Method: The authors propose MapSR, which uses low-resolution labels to extract class prompts from frozen vision foundation model features through a lightweight linear probe, and then proceeds with training-free metric inference and graph-based prediction refinement.

Result: MapSR achieves 59.64% mIoU without any high-resolution labels, remaining competitive with the strongest weakly supervised baseline and surpassing a fully supervised baseline, while reducing trainable parameters by four orders of magnitude and shortening training time from hours to minutes.

Conclusion: The proposed MapSR framework enables scalable high-resolution mapping under limited annotation and compute budgets.
...
<details>
  <summary>Details</summary>
Motivation: The problem of modifying a source image according to a target text prompt while preserving unrelated regions.

Method: Masked Logit Nudging, which uses source image token maps to guide the model's predictions under the target prompt.

Result: The approach achieves the best image editing performance on the PIE benchmark at 512px and 1024px resolutions, and outperforms previous methods on COCO and OpenImages.
...
<details>
  <summary>Details</summary>
Motivation: Current methods for graphic design creation assume input elements are stylistically harmonious, which is often not the case in practice. The authors argue that identity-preserving stylization and compositing of input elements is necessary for truly harmonized design pipelines.
...
Conclusion: The paper concludes that GIST is a useful tool for improving visual harmony in graphic design creation, and can be easily integrated into existing design pipelines.
...
TL;DR: A new method for pan-sharpening is proposed, utilizing a Multigrain-aware Semantic Prototype Scanning paradigm with a high-order RWKV architecture and tri-token prompting mechanism.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of conventional bidirectional raster scanning in RWKV, which is semantic-agnostic and prone to positional bias.
...
Conclusion: The proposed method shows promising results in pan-sharpening tasks, leveraging semantic-driven scanning and tri-token prompting to improve context-aware token reordering and global interaction.
...
TL;DR: Proposed Switch-KD, a visual-switch distillation framework for efficient knowledge transfer in Vision-Language Models (VLMs), achieving improved performance on multimodal benchmarks.


<details>
  <summary>Details</summary>
Motivation: To address challenges in deploying large VLMs in resource-constrained scenarios and improve knowledge distillation (KD) for VLMs with modality-specific supervision.

Method: Switch-KD framework comprising Visual-Switch Distillation and Dynamic Bi-directional Logits Difference (DBiLD) loss for unified vision-language knowledge transfer.

Result: A 0.5B TinyLLaVA distilled from a 3B teacher using Switch-KD showed an average improvement of 3.6 points across 10 multimodal benchmarks.

Conclusion: Switch-KD enables efficient multimodal knowledge transfer in VLMs, improving performance without increasing model size or data requirements.
...
<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to effectively model the interdependencies between appearance and motion cues in two-stream architectures for unsupervised video object segmentation.

Method: The method proposed is cross-modality token modulation, which establishes dense connections between tokens from each modality and uses relation transformer blocks for efficient information propagation. A token masking strategy is also incorporated to improve learning efficiency.
...
TL;DR: FogFool, a new adversarial framework, generates fog-based perturbations to deceive deep learning models in remote sensing image classification, showing superior performance and robustness.


<details>
  <summary>Details</summary>
Motivation: Existing methods for adversarial attacks on remote sensing image classification rely on direct pixel-wise perturbations, which are not physically plausible and may not survive real-world image degradations.

Method: FogFool uses Perlin noise to iteratively optimize atmospheric patterns and generate fog-based perturbations, leveraging spatial coherence and mid-to-low-frequency nature of atmospheric phenomena.
...
TL;DR: A chaos-based feature transformation method is proposed to improve CNN performance on limited training data by applying nonlinear transformations to feature vectors, achieving consistent improvements across various datasets.
...
TL;DR: The paper proposes a novel framework called Seen-to-Scene for video outpainting, which combines propagation-based and generation-based paradigms to improve temporal coherence and visual realism.


<details>
  <summary>Details</summary>
Motivation: Existing video outpainting methods suffer from limitations such as implicit temporal modeling and limited spatial context, leading to intraframe and inter-frame inconsistencies, particularly in dynamic scenes and large outpainting scenarios.

Method: The Seen-to-Scene framework leverages flow-based propagation with a pre-trained flow completion network and introduces a reference-guided latent propagation to effectively propagate source content across frames.
...
Conclusion: The Seen-to-Scene framework successfully addresses the challenges of video outpainting by unifying propagation-based and generation-based paradigms, leading to improved performance and efficiency.
...
TL;DR: The paper proposes DETR-ViP, a framework for visual prompted object detection that improves performance by learning class-distinguishable visual prompts.


<details>
  <summary>Details</summary>
Motivation: The authors investigate the suboptimal performance of visual prompted detection and find that the underlying issue lies in the absence of global discriminability in visual prompts.

Method: The authors propose DETR-ViP, which incorporates global prompt integration, visual-textual prompt relation distillation, and a selective fusion strategy to learn more discriminative prompt representations.

Result: DETR-ViP achieves substantially higher performance in visual prompt detection compared to other state-of-the-art counterparts on several datasets, including COCO, LVIS, ODinW, and Roboflow100.

Conclusion: The proposed DETR-ViP framework effectively improves the performance of visual prompted object detection by learning class-distinguishable visual prompts, and extensive experiments and ablation studies validate its effectiveness.
...
TL;DR: A new framework called Chain-of-Glimpse is proposed for video understanding, which uses a search-guided progressive object-grounded reasoning approach to improve performance and interpretability.


<details>
  <summary>Details</summary>
Motivation: Existing object-agnostic solutions struggle to handle substantial object variations over time in video understanding, highlighting the need for a more effective approach.
...
Conclusion: Chain-of-Glimpse provides a more effective and interpretable approach to video understanding by explicitly anchoring each reasoning step to specific visual evidence regions.
...
TL;DR: Proposed a courtroom-style adjudication framework for image manipulation localization (IML) that improves reliability by explicitly comparing manipulated and authentic evidence.


<details>
  <summary>Details</summary>
Motivation: Existing IML methods struggle with subtle or degraded manipulation traces, leading to unreliable predictions in ambiguous areas.

Method: A dual-hypothesis segmentation architecture with a prosecution stream, a defense stream, and a judge model, utilizing reinforcement learning and edge priors.

Result: Achieves superior average performance compared to state-of-the-art IML methods.

Conclusion: The proposed framework effectively addresses the limitations of existing IML methods by explicitly modeling authenticity-related supervision and improving reliability.
...
TL;DR: Introduces NG-GS, a framework for high-quality object segmentation in 3D Gaussian Splatting (3DGS) that addresses boundary discretization.


<details>
  <summary>Details</summary>
Motivation: Segmenting objects accurately in 3DGS remains challenging due to discrete Gaussian representations leading to aliasing and artifacts at object boundaries.

Method: NG-GS uses mask variance analysis to identify ambiguous Gaussians, RBF interpolation for continuous feature fields, and joint optimization with a NeRF module for smooth segmentation boundaries.

Result: Achieves state-of-the-art performance on NVOS, LERF-OVS, and ScanNet benchmarks with significant gains in boundary mIoU.

Conclusion: NG-GS provides a novel solution for high-quality object segmentation in 3DGS, addressing boundary discretization and achieving improved performance.
...
TL;DR: Proposed a novel training-free method, G-MIXER, for Zero-Shot Composed Image Retrieval (ZS-CIR) that improves retrieval diversity and accuracy by combining implicit and explicit semantics.


<details>
  <summary>Details</summary>
Motivation: Current ZS-CIR methods rely heavily on textual modality and fail to capture diverse combinations of candidates, leading to reduced diversity and accuracy in retrieval results.
...
Conclusion: The proposed G-MIXER method effectively handles both implicit and explicit semantics, improving retrieval diversity and accuracy in Zero-Shot Composed Image Retrieval.
...
Method: The proposed MS-SSE-Net model uses a DenseNet201 backbone with novel multi-scale feature extraction and channel and spatial attention mechanisms.

Result: The proposed method achieves 99.31% precision, 99.25% recall, 99.27% F1-score, and 99.26% accuracy, outperforming the baseline model.
...
TL;DR: A new method for segmenting myotubes in 3D microscopy images using synthetic data and a compact 3D U-Net.


<details>
  <summary>Details</summary>
Motivation: Established pretrained biomedical segmentation models fail to generalize to myotube segmentation due to lack of large annotated datasets.

Method: A geometry-driven synthesis pipeline for generating synthetic myotube data and a compact 3D U-Net with self-supervised encoder pretraining.
...
Conclusion: Biophysics-driven synthesis enables effective instance segmentation in annotation-scarce biomedical domains.
...
TL;DR: HAMSA, a scanning-free State Space Model (SSM) operating in the spectral domain, achieves state-of-the-art performance on ImageNet-1K with improved efficiency and simplicity.


<details>
  <summary>Details</summary>
Motivation: Existing Vision SSMs rely on complex scanning strategies, introducing computational overhead and architectural complexity, which HAMSA aims to address.

Method: HAMSA proposes three key innovations: simplified kernel parameterization, SpectralPulseNet (SPN), and Spectral Adaptive Gating Unit (SAGU), leveraging FFT-based convolution.

Result: HAMSA achieves 85.7% top-1 accuracy on ImageNet-1K, with 2.2X faster inference than transformers and 1.4-1.9X speedup over scanning-based SSMs, while using less memory and energy.

Conclusion: HAMSA demonstrates superior simplicity, efficiency, and performance, generalizing well across transfer learning and dense prediction tasks.
...
TL;DR: A new class of resultant-based solvers for pose estimation using Sylvester forms is proposed, offering improved computational efficiency without sacrificing accuracy.


<details>
  <summary>Details</summary>
Motivation: The need for fast and accurate pose estimation in real-time computer vision applications.

Method: Proposing a new class of resultant-based solvers using Sylvester forms to reduce complexity.

Result: The proposed methods are numerically as accurate as state-of-the-art solvers but outperform them in computational time.

Conclusion: The proposed approach can be applied to different types of pose estimation problems with improved efficiency.
...
TL;DR: The paper proposes ASGNet, a novel network for polyp segmentation in colonoscopy images, addressing limitations of spatial perception by integrating spectral features with global attributes.


<details>
  <summary>Details</summary>
Motivation: Polyp segmentation in colonoscopy images is challenging due to diverse morphologies, complex backgrounds, and concealed nature of polyps. Existing deep learning-based methods have limitations in capturing complete polyp structures.
...
Conclusion: The proposed ASGNet effectively addresses the limitations of spatial perception in polyp segmentation, leading to more accurate results and potential improvements in early colorectal cancer detection.
...
<details>
  <summary>Details</summary>
Motivation: Current GCD methods are limited to single modalities and require dataset-specific fine-tuning, unlike human category learning which is abstract and modality-agnostic.

Method: OmniGCD uses modality-specific encoders, dimension reduction to construct a GCD latent space, and a synthetically trained Transformer-based model for clustering.

Result: OmniGCD improves classification accuracy for known and novel classes over baselines across 16 datasets and four modalities, with average percentage point improvements of +6.2, +17.9, +1.5, and +12.7 for vision, text, audio, and remote sensing respectively.

Conclusion: OmniGCD serves as a benchmark for future modality-agnostic GCD works, enabling scalable, human-inspired category discovery and paving the way for encoder development independent of GCD.
...
TL;DR: Proposed Asymmetric Information Masking (AIM) to address catastrophic forgetting in continual visual question answering (VQA) for asymmetric Vision-Language Models (VLMs).
...
Conclusion: AIM helps preserve generalization to novel skill-concept compositions in continual VQA for asymmetric VLMs.
...
<details>
  <summary>Details</summary>
Motivation: The lack of a complete, modular, and flexible system for obstacle detection and distance estimation in railway environments, and the challenge of evaluating such systems due to the lack of ground truth data.

Method: A framework integrating three neural networks for object detection, track segmentation, and monocular depth estimation with LiDAR point clouds.

Result: The proposed system achieves a mean absolute error (MAE) as low as 0.63 meters by integrating monocular depth maps with LiDAR.

Conclusion: The proposed framework provides a reliable and accurate solution for obstacle detection and distance estimation in railway environments, enabling spatial perception of the scene.
...
TL;DR: A new method for creating 3D head avatars from a single image, focusing on realistic hair dynamics and detailed textures.
...
Result: The method produces 3D head avatars with realistic hair behavior and faithfully preserved facial details, outperforming state-of-the-art one-shot methods in perceptual realism.

Conclusion: The proposed compositional method successfully addresses the challenge of creating realistic 3D head avatars from a single image, particularly in terms of hair dynamics and texture preservation.
...
TL;DR: The paper proposes Petro-SAM, a two-stage framework for joint grain-edge segmentation and lithology semantic segmentation on petrographic images, leveraging the Segment Anything Model (SAM) with novel modules for improved performance.


<details>
  <summary>Details</summary>
Motivation: Current methods for grain-edge segmentation (GES) and lithology semantic segmentation (LSS) are often separate and produce implausible results despite using expensive and expert-annotated datasets. The Segment Anything Model (SAM) shows robustness in boundary alignment but adapting it to joint GES and LSS is nontrivial due to domain gaps and lack of modules for multi-angle petrographic image stacks.

Method: The proposed Petro-SAM framework consists of a two-stage approach. It introduces a Merge Block to integrate seven polarized views to address extinction issues. Additionally, it incorporates multi-scale feature fusion and color-entropy priors to refine detection.

Result: The Petro-SAM framework achieves high-quality joint GES and LSS on petrographic images.

Conclusion: The proposed Petro-SAM framework effectively addresses the challenges of joint grain-edge segmentation and lithology semantic segmentation on petrographic images, leveraging the strengths of the Segment Anything Model (SAM) with novel adaptations for improved performance.
...
TL;DR: The NTIRE 2026 Challenge on Video Saliency Prediction aimed to develop automatic saliency map prediction methods for video sequences, resulting in a publicly available dataset and submissions from over 20 teams.


<details>
  <summary>Details</summary>
Motivation: The challenge sought to advance the development of automatic saliency map prediction methods for video sequences by providing a novel dataset and encouraging competition among participants.

Method: The challenge provided a dataset of 2,000 diverse videos with fixations and saliency maps collected through crowdsourced mouse tracking, and teams were evaluated on 800 test videos using standard quality metrics.

Result: Over 20 teams made submissions, with 7 teams passing the final phase with code review, and all data used in the challenge is made publicly available.

Conclusion: The NTIRE 2026 Challenge on Video Saliency Prediction successfully created a large-scale dataset and stimulated the development of saliency prediction methods, with all data and results made publicly available for future research.
...
TL;DR: A new MRI-based method, MSSM+, combined with SV-ViT, shows promise for detecting Alzheimer's disease (AD) earlier and more accurately than existing methods.


<details>
  <summary>Details</summary>
Motivation: Current AD confirmation methods are costly and invasive; there is a need for non-invasive and cost-effective screening tools.

Method: MSSM+ extends MSSM by incorporating sulcal depth and cortical curvature, using surface supervertex mapping (SSVM) and a Vision Transformer (SV-ViT) architecture.

Result: MSSM+ identified more spatially extensive and statistically significant group differences between AD and CN, achieving a 3%p higher area under the precision-recall curve than MSSM.

Conclusion: MSSM+ combined with SV-ViT is a promising MRI-based imaging marker for AD detection prior to CSF/PET confirmation.
...
<details>
  <summary>Details</summary>
Motivation: Retail theft costs over $100 billion annually, but existing AI-based detection systems are expensive and require custom model training.
...
Result: Paza achieves 89.5% precision and 92.8% specificity at 59.3% recall zero-shot, with a detailed cost model showing viability at $50-100/month per store.

Conclusion: Paza provides a cost-effective and model-agnostic solution for retail theft detection, enabling a single GPU to serve 10-20 stores and ensuring the system improves as the VLM landscape evolves.
...
<details>
  <summary>Details</summary>
Motivation: Searching for adaptive skip modules in medical image segmentation is computationally costly, and the original IAC framework requires a long search process.

Method: The authors analyze the temporal behavior of operations and edges within IAC cells and propose a Jensen-Shannon-divergence-based stability criterion to prune low-importance operations during search.

Result: IAC-LTH discovers IAC cells that match or exceed the performance of cells found by the original full-length search, while reducing wall-clock NAS cost by 3.7x to 16x across datasets and backbones.

Conclusion: Competitive IAC architectures can be identified from early-stabilizing operations without running the full search, making adaptive skip-module design more practical for medical image segmentation under realistic computational constraints.
...
Method: The authors created a large-scale dentistry image dataset (60,669 images), a semi-structured annotation framework, and benchmark suites. They annotated 2,588 images and used Large Language Models (LLMs) to derive standardized benchmarks.
...
TL;DR: The paper proposes an open-set vein recognition method using Deep Metric Learning, achieving high accuracy and robustness in identifying enrolled users while rejecting unknown impostors.


<details>
  <summary>Details</summary>
Motivation: Most state-of-the-art vein recognition methods are limited by their reliance on closed-set classification, which hinders their scalability and adaptability to new users.

Method: The approach uses L2-normalised embeddings, prototype-based matching, and a calibrated similarity threshold to distinguish between enrolled users and unseen impostors.

Result: The best model achieves an OSCR of 0.9945, AUROC of 0.9974, and EER of 1.57% on the MMCBNU 6000 benchmark, with high identification accuracy (99.6% Rank-1) and robust rejection of unknown subjects.
...
TL;DR: FSDETR, a frequency-spatial feature enhancement framework, is proposed to address challenges in small object detection, achieving strong performance on benchmarks with minimal parameters.
...
Method: FSDETR framework, built upon RT-DETR, utilizes a Spatial Hierarchical Attention Block, Deformable Attention-based Intra-scale Feature Interaction, and Frequency-Spatial Feature Pyramid Network.

Result: FSDETR achieves 13.9% APS on VisDrone 2019 and 48.95% AP50 tiny on TinyPerson with only 14.7M parameters.

Conclusion: The proposed FSDETR framework effectively enhances feature representation for small object detection, demonstrating strong performance and efficiency.
...
TL;DR: The paper proposes a new framework called Reward-Aware Trajectory Shaping (RATS) for few-step generative modeling, which aligns teacher and student trajectories and uses a reward-aware gate to adaptively regulate teacher guidance, allowing the student to potentially surpass the teacher.


<details>
  <summary>Details</summary>
Motivation: The goal is to achieve high-fidelity generation in extremely few sampling steps, and existing approaches are limited by their reliance on distillation-based frameworks that constrain the student to imitate a stronger teacher.

Method: The proposed RATS framework uses horizon matching to align teacher and student latent trajectories, and introduces a reward-aware gate to regulate teacher guidance based on relative reward performance.
...
Conclusion: The RATS framework enables few-step generators to potentially surpass their teachers by optimizing toward reward-preferred generation quality, and effectively transfers preference-relevant knowledge without incurring additional test-time computational overhead.
...
Method: The method involves analyzing the sampling trajectories of the generative model, identifying 'latent sink traps' where the model becomes insensitive to prompt modifications, and proposing a framework that decouples a model's geometric representation power from its linguistic sensitivity.
...
Conclusion: The conclusion is that the proposed framework addresses the limitations of current 3D pipelines and enables more robust text-based 3D shape editing by bypassing latent sinks.
...
TL;DR: A new method for reconstructing 2D Gaussian scene models from multi-view images is introduced, improving upon existing NeRF-based models by separating geometry and appearance, and demonstrating superior reconstruction fidelity.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the entanglement between geometry and appearance in NeRF-based models, which can lead to high-frequency texture compensating for geometric errors.

Method: The method introduced is a hybrid Gaussian-hash-grid radiance representation, which adds per-Gaussian latent features and hash-grid features to bias the optimizer toward a separation of low- and high-frequency scene components.

Result: The results show that the proposed method demonstrates superior reconstruction fidelity with an order of magnitude fewer primitives compared to the state of the art in Gaussian-based novel-view synthesis.

Conclusion: The conclusion of this paper is that the proposed hybrid Gaussian-hash-grid radiance representation effectively reduces the entanglement between geometry and appearance, and improves both geometry reconstruction and rendering efficiency.
...
TL;DR: Proposed a conditional generative pipeline for data augmentation in skeleton action recognition, achieving competitive performance with limited training samples.


<details>
  <summary>Details</summary>
Motivation: Collecting large-scale, diverse, and well-annotated 3D skeleton datasets is expensive and labor-intensive.
...
Result: Improved accuracy of multiple skeleton-based action recognition models on HumanAct12 and NTU-VIBE datasets in few-shot and full-data settings.

Conclusion: The proposed approach effectively generates diverse and high-fidelity skeleton sequences, demonstrating strong generalisation in downstream tasks.
...
TL;DR: A novel framework, RaTA-Tool, is proposed for open-world multimodal tool selection, enabling AI systems to effectively use external resources to solve complex tasks.


<details>
  <summary>Details</summary>
Motivation: The limitations of existing tool-use methods, which are mainly restricted to text-only inputs and closed-world settings, hindering their ability to interpret multimodal user instructions and generalize to unseen tools.

Method: The RaTA-Tool framework involves converting multimodal queries into structured task descriptions and then retrieving the most suitable tool through matching with semantically rich tool descriptions, further optimized with Direct Preference Optimization (DPO).

Result: The proposed approach demonstrates significant improvements in tool-selection performance, especially in open-world and multimodal scenarios, and introduces the first dataset for open-world multimodal tool use.

Conclusion: The RaTA-Tool framework effectively addresses the limitations of existing tool-use methods by enabling open-world multimodal tool selection, facilitating the extensibility to new tools without retraining, and improving the alignment between task descriptions and tool selection.
...
TL;DR: The paper explores using video Generative AI models to generate synthetic gesture data, which can complement traditional human-generated data. It proposes a data generation pipeline and evaluates its effectiveness for downstream tasks.


<details>
  <summary>Details</summary>
Motivation: The scarcity of gesture recognition data, due to the need for costly human recordings or image processing approaches, hinders progress in the field. Recent advancements in image-to-video foundation models offer a potential solution.

Method: The paper proposes a prompt-based video generation approach to construct a realistic deictic gestures dataset. A data generation pipeline is introduced, which produces deictic gestures from a small number of reference samples collected from human participants.

Result: The synthetic gestures generated by the proposed approach align closely with real ones in terms of visual fidelity and introduce meaningful variability and novelty. Using a mixed dataset with synthetic and real gestures results in superior performance of various deep models.

Conclusion: The findings highlight that image-to-video techniques, even in their early stages, offer a powerful zero-shot approach to gesture synthesis with clear benefits for downstream tasks. This approach can augment and complement traditional human-generated gesture data.
...
TL;DR: The paper proposes a Frequency-Enhanced Dual-Subspace Network (FEDSNet) for few-shot fine-grained image classification, which leverages frequency domain features to improve robustness and accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing metric learning-based methods for few-shot fine-grained image classification rely solely on spatial domain features, which can suffer from texture biases and overfit to background noise.

Method: FEDSNet utilizes Discrete Cosine Transform (DCT) and low-pass filtering to isolate low-frequency global structural components, and employs Truncated Singular Value Decomposition (SVD) to construct independent subspaces for spatial texture and frequency structural features.
...
Conclusion: The proposed FEDSNet provides an effective new paradigm for few-shot fine-grained visual recognition, achieving a favorable balance between high accuracy and computational efficiency.
...
TL;DR: The paper proposes UniDoc-RL, a unified reinforcement learning framework that enhances Large Vision-Language Models (LVLMs) with a retrieval-augmented generation system, enabling fine-grained visual semantics for complex reasoning.
...
Method: UniDoc-RL uses a hierarchical action space to progressively refine visual evidence through document retrieval, image selection, and active region cropping, with a dense multi-reward scheme for end-to-end training.

Result: UniDoc-RL outperforms state-of-the-art baselines on three benchmarks, achieving up to 17.7% gains over prior RL-based methods.

Conclusion: The proposed UniDoc-RL framework effectively enhances LVLMs with fine-grained visual semantics, leading to improved performance in complex reasoning tasks.
...
TL;DR: The paper introduces 'Flow of Truth', a proactive framework for temporal forensics in image-to-video (I2V) generation, enabling robust tracing of pixel flow and transformation throughout videos.


<details>
  <summary>Details</summary>
Motivation: The rapid rise of I2V generation brings new forensic demands, as traditional spatial forensics are ineffective in tracing evolving pixels and transformations in videos.

Method: The framework innovatively redefines video generation as pixel motion through time, proposing a learnable forensic template and a template-guided flow module to decouple motion from image content.
...
Conclusion: The paper presents a novel approach to temporal forensics in I2V generation, providing a proactive framework for tracing pixel flow and transformation throughout videos.
...
TL;DR: A novel framework called QuAD is proposed to improve the detection of synthetic images by leveraging multiple near-duplicate versions of the same image, accounting for their quality.


<details>
  <summary>Details</summary>
Motivation: Existing approaches to detecting synthetic images often analyze single image instances and overlook the presence of multiple near-duplicate versions that appear online, which can lead to inconsistent forensic predictions.
...
Result: Experiments show that QuAD improves the performance of state-of-the-art detectors by an average of 8% in terms of balanced accuracy, highlighting the importance of jointly processing online images for reliable detection of AI-generated content.

Conclusion: The proposed QuAD framework offers a significant improvement in detecting synthetic images by effectively utilizing multiple near-duplicate versions, leading to more reliable detection in real-world applications.
...
TL;DR: Implicit neural representations (INRs) are a new way to model signals, representing them as continuous functions rather than discrete data. This allows for more efficient and flexible signal processing.


<details>
  <summary>Details</summary>
Motivation: The motivation behind INRs is to provide a unified framework for representing various types of signals, such as images, audio, and 3D geometry, as continuous functions of their coordinates.

Method: The article examines the evolution of INRs, discussing their spectral behavior, sampling theory, and multiscale representation. It also explores advanced designs, such as specialized activations and structured representations, that improve the approximation space and computational efficiency of INRs.

Result: The result of INRs is a more flexible and efficient way to process signals, enabling applications such as inverse problems in medical and radar imaging, compression, and 3D scene representation.

Conclusion: The article concludes that INRs are a powerful tool for signal processing, offering a new perspective on signal modeling and representation. However, it also highlights open challenges in theoretical stability, weight space interpretability, and large-scale generalization.
...
TL;DR: A hybrid CNN-Attention framework is proposed for robust MRI quality assessment, achieving high accuracy and resilience to domain shift across different sites and scanners.


<details>
  <summary>Details</summary>
Motivation: Motion artifacts in structural MRI compromise clinical diagnostics and automated analysis, and manual quality control is unscalable for large studies.

Method: A hybrid CNN-Attention framework integrating a hierarchical 2D CNN encoder and a multi-head cross-attention mechanism for site-invariant MRI quality assessment.

Result: The model achieved high accuracy (0.9920) and F1-score (0.9919) on seen sites and maintained strong generalization across unseen sites (Acc = 0.755) without retraining.

Conclusion: The proposed framework effectively captures universal artifact descriptors, bridging the performance gap between diverse imaging environments and scanner manufacturers.
...
TL;DR: The paper presents HELP, a noise-aware framework that improves small-object detection by refining low-quality queries and reducing background noise, achieving better accuracy with fewer parameters.


<details>
  <summary>Details</summary>
Motivation: Transformer-based detectors are inefficient and vulnerable to background noise, motivating the need for deep decoders to refine low-quality queries.

Method: The paper proposes Heatmap-guided Embedding Learning Paradigm (HELP) with Heatmap-guided Positional Embedding (HPE) and Linear-Snake Convolution to improve query retrieval and feature representation.

Result: The design reduces decoder layers from eight to three, achieves a 59.4% parameter reduction (66.3M vs. 163M) while maintaining consistent accuracy gains under a reduced compute budget across benchmarks.

Conclusion: The proposed HELP framework effectively improves small-object detection efficiency and accuracy by suppressing background noise and refining query retrieval.
...
TL;DR: A new benchmark and framework for building extraction from remote sensing images under hazy and low-light conditions.


<details>
  <summary>Details</summary>
Motivation: Existing methods for building extraction from remote sensing images perform poorly under hazy and low-light conditions, and there is a need for a benchmark and framework that can handle these adverse scenarios.

Method: The authors propose a novel framework called HaLoBuild-Net, which includes a Spatial-Frequency Focus Module (SFFM), a Global Multi-scale Guidance Module (GMGM), and a Mutual-Guided Fusion Module (MGFM) to effectively mitigate meteorological interference and improve building extraction accuracy.

Result: The proposed HaLoBuild-Net significantly outperforms state-of-the-art methods and conventional cascaded restoration-segmentation paradigms on the HaLoBuilding dataset and maintains robust generalization on other datasets.

Conclusion: The authors introduce a new benchmark called HaLoBuilding and a novel framework HaLoBuild-Net for building extraction under hazy and low-light conditions, which demonstrates superior performance and robustness.
...
Method: The proposed STFER framework uses LVLMs to generate identity-intrinsic semantic text, which is then used for Semantic-driven Visual Token Filtering (SVTF) and Semantic-driven Expert Routing (SER).

Result: The model achieves state-of-the-art results on the Any-Time ReID dataset (AT-USTC) and demonstrates superior generalization capabilities with highly competitive results across 5 widely-used ReID benchmarks.

Conclusion: The proposed STFER framework provides a robust solution for AT-ReID by leveraging the ability of LVLMs to generate identity consistency text, which provides identity-discriminative features that are robust to both clothing variations and cross-modality shifts.
...
TL;DR: A new model, LAMAE, is proposed for cardiac assessment using echocardiography, which can handle multi-view and multi-frame data, and has shown promising results in predicting ICD-10 codes and transferring to pediatric cohorts.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenges of sparse and heterogeneous spatiotemporal views of the heart in echocardiography, and to develop a model that can capture the inherent multi-view structure required for coherent cardiac representation.

Method: The method used is a foundation model architecture called Latent Attention Masked Autoencoder (LAMAE), which augments the standard MAE with a latent attention module that enables information exchange across frames and views directly in latent space.

Result: The results show that LAMAE can predict ICD-10 codes from MIMIC-IV-ECHO videos, and that representations learned from adult data transfer effectively to pediatric cohorts, providing evidence that incorporating structural priors yields more robust and transferable representations.

Conclusion: The conclusion is that LAMAE is a promising model for cardiac assessment using echocardiography, and that it has the potential to improve the accuracy and robustness of cardiac function assessment.
...
TL;DR: The paper presents PIE-V, a framework for generating and benchmarking egocentric procedural videos with injected human-plausible mistakes and recoveries.


<details>
  <summary>Details</summary>
Motivation: The motivation is to create a reliable procedural monitoring system in videos, which requires exposure to naturally occurring human errors and recoveries, and to address the limitations of existing procedural datasets.

Method: The method involves augmenting clean keystep procedures with controlled, human-plausible deviations using a psychology-informed error planner, correction planner, LLM writer, and LLM judge, and synthesizing replacement clips with text-guided video generation.

Result: The result is a framework that can inject 102 mistakes and generate 27 recovery corrections for 17 tasks and 50 Ego-Exo4D scenarios, and a unified taxonomy and human rubric with nine metrics for benchmarking.

Conclusion: The conclusion is that the PIE-V framework and rubric support post-completion verification for egocentric procedural mistake detection and correction, and can be used to audit existing resources and compare against other generation baselines.
...
<details>
  <summary>Details</summary>
Motivation: Higher-order learning is crucial for exploiting compositional features, but it increases model complexity in conventional deep learning models.
...
Result: kVNN yields reduced model and computational complexity with competitive and often improved performance on video action recognition and image denoising tasks, even without large-scale pretraining.
...
TL;DR: The paper introduces DAMP, a method for machine unlearning that selectively removes targeted knowledge from a trained model without retraining.


<details>
  <summary>Details</summary>
Motivation: Existing class-unlearning methods often exhibit weak or negative selectivity, preserving forget-class structure in deep representations or relying heavily on final-layer bias shifts.

Method: DAMP (Depth-Aware Modulation by Projection) is a one-shot, closed-form weight-surgery method that removes forget-specific directions from a pretrained network.

Result: DAMP improves selective forgetting while preserving retain-class performance and reducing residual forget-class structure in deep layers across various datasets and architectures.

Conclusion: DAMP provides a more effective approach to machine unlearning, outperforming existing methods in selective forgetting and retention of relevant knowledge.
...
TL;DR: The paper presents a study on lighting-related image restoration, comparing a specialized approach (DINOLight) with a generalized approach (OmniLight) that uses a Wavelet Domain Mixture-of-Experts (WD-MoE) model, achieving top-tier rankings in the NTIRE 2026 Challenge.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenges posed by adverse lighting conditions in computer vision systems, and to develop robust models capable of handling diverse domains for real-world applications.

Method: The method involves a comparative analysis of two strategies: a specialized baseline (DINOLight) that exploits the characteristics of individual datasets, and a generalized alternative (OmniLight) that incorporates a Wavelet Domain Mixture-of-Experts (WD-MoE) model trained across all provided datasets.

Result: The result shows that both approaches secured top-tier rankings across all three lighting-related tracks in the NTIRE 2026 Challenge, demonstrating their outstanding perceptual quality and generalization capabilities.

Conclusion: The conclusion is that the study highlights the impact of data distribution on the performance of specialized and unified architectures in lighting-related image restoration, and that the proposed approaches can effectively restore underlying image content and improve perceptual quality.
...
TL;DR: The paper explores simpler penalty terms to reduce computational overhead in diffusion models while maintaining generation quality.


<details>
  <summary>Details</summary>
Motivation: The denoising score matching (DSM) objective in diffusion models often violates the Fokker-Planck (FP) equation, and directly penalizing these deviations is computationally expensive.

Method: The authors empirically analyze several lightweight regularizers and their effect on FP residuals and generation quality.

Result: The benefits of FP regularization can be achieved at a substantially lower computational cost using simpler penalty terms.

Conclusion: The paper finds that simpler penalty terms can provide similar benefits to strict FP regularization in diffusion models, reducing computational overhead.
...
TL;DR: The paper introduces B-ACT, an active learning framework for temporal action segmentation that focuses on annotating high-leverage boundary regions in videos, achieving strong label efficiency and surpassing baselines under sparse budgets.


<details>
  <summary>Details</summary>
Motivation: The high cost of annotating untrimmed videos for temporal action segmentation (TAS) is largely due to identifying and refining action transitions, where small temporal shifts significantly impact segmental metrics.

Method: B-ACT operates in a two-stage loop: (i) ranking and querying unlabeled videos using predictive uncertainty, and (ii) detecting candidate transitions within selected videos and selecting top-K boundaries based on a novel boundary score.

Result: Extensive experiments on GTEA, 50Salads, and Breakfast datasets demonstrate that B-ACT delivers strong label efficiency and consistently surpasses TAS active learning baselines and prior state of the art under sparse budgets.

Conclusion: The proposed B-ACT framework effectively allocates supervision to high-leverage boundary regions in TAS, achieving significant improvements in label efficiency and performance.
...
Method: Formulates visual token pruning as a Pareto configuration optimization problem, using continuous relaxation and straight-through estimators for gradient-based search.

Result: Approximates the empirical Pareto frontier and generalizes well across various pruning methods and VLM architectures, achieving superior accuracy-efficiency trade-offs.

Conclusion: The proposed framework effectively identifies optimal visual token pruning configurations, revealing that multi-step progressive pruning captures VLMs' hierarchical compression structure.
...
TL;DR: A novel hierarchical spatiotemporal vector quantization framework is proposed for unsupervised skeleton-based temporal action segmentation, outperforming baselines and achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the performance of unsupervised skeleton-based temporal action segmentation by leveraging both spatial and temporal information.

Method: A hierarchical spatiotemporal vector quantization framework is introduced, which includes two consecutive levels of vector quantization and performs multi-level clustering to recover input skeletons and their timestamps.
...
Conclusion: The proposed hierarchical spatiotemporal vector quantization framework effectively improves the performance of unsupervised skeleton-based temporal action segmentation by leveraging both spatial and temporal cues.
...
TL;DR: A new framework, StreamCacheVGGT, is proposed for reconstructing dense 3D geometry from continuous video streams under a constant memory budget, outperforming existing methods in accuracy and stability.


<details>
  <summary>Details</summary>
Motivation: Existing O(1) frameworks for 3D geometry reconstruction from video streams suffer from information destruction and evaluation noise due to their 'pure eviction' paradigm.

Method: StreamCacheVGGT, a training-free framework, uses two modules: Cross-Layer Consistency-Enhanced Scoring (CLCES) and Hybrid Cache Compression (HCC) to improve cache management.

Result: StreamCacheVGGT achieves a new state-of-the-art in 3D reconstruction accuracy and long-term stability on five benchmarks while maintaining constant-cost constraints.

Conclusion: The proposed framework, StreamCacheVGGT, effectively addresses the limitations of existing methods by providing a more stable and accurate approach to reconstructing dense 3D geometry from continuous video streams.
...
Method: SegWithU uses a lightweight uncertainty head with a frozen pretrained segmentation backbone, modeling uncertainty as perturbation energy in a compact probe space.
...
Conclusion: SegWithU presents an effective and practical route to reliability-aware medical segmentation through perturbation-based uncertainty modeling.
...
TL;DR: The paper explores why vision-language models (VLMs) struggle to recognize human emotions, identifying two main issues: (1) bias towards common emotions due to long-tailed emotion datasets and (2) inability to effectively represent temporal information, particularly for fleeting micro-expressions. It proposes alternative sampling strategies and a multi-stage context enrichment strategy to improve emotion recognition.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the challenge of understanding human emotions by intelligent systems, specifically focusing on why vision-language models (VLMs) fail to effectively recognize emotions despite their progress in various visual tasks.

Method: The paper proposes two main methodological contributions: (1) alternative sampling strategies to mitigate the head-class bias in emotion datasets and (2) a multi-stage context enrichment strategy that converts 'in-between' frames into natural language summaries to provide enriched textual context to VLMs alongside sparse keyframes.

Result: The paper demonstrates that VLMs struggle with emotion recognition due to their limitations in handling long-tailed emotion datasets and temporal information. It shows that the proposed strategies can help prevent attentional dilution and preserve the emotional trajectory, potentially improving emotion recognition.

Conclusion: The paper concludes that VLMs face significant challenges in recognizing human emotions due to dataset biases and limitations in processing temporal information. It suggests that addressing these challenges through alternative strategies can improve emotion recognition capabilities of VLMs.
...
<details>
  <summary>Details</summary>
Motivation: The authors aim to improve 3D policy learning by addressing the limitations of current methods, specifically training instabilities and severe overfitting.

Method: The authors propose a new architecture that combines a transformer-based 3D encoder with a diffusion decoder, and leverage large-scale pre-training to improve stability and performance.
...
Conclusion: The authors establish a new and robust foundation for scalable 3D imitation learning, addressing the limitations of current methods and achieving state-of-the-art results.
...
TL;DR: GlobalSplat is a framework for 3D Gaussian Splatting that learns a compact, global latent scene representation, enabling efficient and globally consistent reconstructions.


<details>
  <summary>Details</summary>
Motivation: Previous solutions for 3D Gaussian Splatting suffer from trade-offs between representation compactness, reconstruction speed, and rendering fidelity due to local, heuristic-driven allocation strategies.

Method: GlobalSplat uses a coarse-to-fine training curriculum to learn a compact, global latent scene representation that encodes multi-view input and resolves cross-view correspondences.

Result: GlobalSplat achieves competitive novel-view synthesis performance on RealEstate10K and ACID with a significantly smaller representation size (16K Gaussians) and faster inference time (under 78 milliseconds).

Conclusion: GlobalSplat enables efficient and globally consistent 3D reconstructions without relying on pretrained pixel-prediction backbones or reusing latent features from dense baselines.
...
TL;DR: The paper explores Visual Anomaly Detection (VAD) to improve the reliability of machine vision systems in autonomous driving by identifying anomalous objects not present during training.


<details>
  <summary>Details</summary>
Motivation: The reliability of machine vision systems in autonomous driving can degrade when encountering atypical obstacles, posing physical risk to passengers, pedestrians, and road users.

Method: The paper benchmarks eight state-of-the-art VAD methods on the AnoVox dataset, evaluating performance across four backbone architectures, including large networks and lightweight ones like MobileNet and DeiT-Tiny.
...
Conclusion: The study represents a step toward safer deployment of autonomous vehicles, improving protection for passengers, pedestrians, and road users.
...
TL;DR: Introduced AnimationBench, a benchmark for evaluating animation image-to-video generation, addressing the gap in existing benchmarks for animation-style generation.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks struggle to evaluate animation-style generation due to its stylized appearance, exaggerated motion, and character-centric consistency, and also lack flexibility for open-domain content and custom evaluation needs.

Method: AnimationBench operationalizes the Twelve Basic Principles of Animation and IP Preservation into measurable evaluation dimensions, and supports both standardized close-set evaluation and flexible open-set evaluation.

Result: AnimationBench aligns well with human judgment and exposes animation-specific quality differences overlooked by realism-oriented benchmarks, leading to more informative and discriminative evaluation of state-of-the-art I2V models.

Conclusion: AnimationBench is a systematic benchmark for evaluating animation image-to-video generation, providing a more informative and discriminative evaluation of state-of-the-art I2V models.
...
<details>
  <summary>Details</summary>
Motivation: Current SLT systems assume a direct mapping from signing to spoken language words, which breaks down due to the complex and context-dependent nature of sign language.

Method: The proposed framework uses an ordered sequence of latent thoughts as an intermediate layer between video and text, and a plan-then-ground decoding method that first decides what to say and then looks back at the video for evidence.

Result: Experiments show consistent gains over existing gloss-free methods across several benchmarks.

Conclusion: The paper argues that SLT is a cross-modal reasoning task and proposes a new framework that improves coherence and faithfulness, along with a new large-scale gloss-free SLT dataset.
...
TL;DR: RAD-2, a unified generator-discriminator framework for closed-loop planning in autonomous driving, improves motion planners by modeling multimodal future uncertainties and providing robust interactions.


<details>
  <summary>Details</summary>
Motivation: Current diffusion-based planners in high-level autonomous driving suffer from stochastic instabilities and lack corrective negative feedback when trained with imitation learning.

Method: RAD-2 uses a diffusion-based generator to produce diverse trajectory candidates and an RL-optimized discriminator to rerank these candidates according to long-term driving quality.
...
Conclusion: The proposed RAD-2 framework effectively addresses the challenges of stochastic instabilities and lack of corrective feedback in diffusion-based planners, leading to improved performance and safety in autonomous driving.
...
TL;DR: Proposed MM-WebAgent, a framework for generating visually consistent webpages using AI-generated content.


<details>
  <summary>Details</summary>
Motivation: Integrating AI-generated content into webpage design leads to style inconsistency and poor global coherence.

Method: Hierarchical agentic framework with planning and self-reflection for multimodal webpage generation.

Result: MM-WebAgent outperforms baselines in generating coherent and visually consistent webpages.

Conclusion: MM-WebAgent effectively generates multimodal webpages with improved coherence and visual consistency.
...
<details>
  <summary>Details</summary>
Motivation: The need for precise and continuous control over multiple illumination attributes in image relighting.

Method: Formulating relighting as a conditional image generation task using attribute tokens to encode lighting factors, and training on a large-scale synthetic dataset with ground-truth lighting annotations.

Result: State-of-the-art quantitative and qualitative performance in various relighting tasks, with convincing lighting effects even in challenging scenarios.

Conclusion: The proposed method achieves precise and continuous control over multiple illumination attributes in image relighting, demonstrating an inherent understanding of light interaction with scene geometry, occlusion, and materials.
...
TL;DR: The paper introduces LeapAlign, a fine-tuning method for flow matching models that reduces computational cost and enables direct gradient propagation from reward to early generation steps, achieving superior image quality and image-text alignment.
...
Method: The method introduced is LeapAlign, which shortens the long trajectory into only two steps by designing two consecutive leaps, each skipping multiple ODE sampling steps and predicting future latents in a single step.
...
Conclusion: The conclusion is that LeapAlign is an effective fine-tuning method for flow matching models, enabling efficient and stable model updates at any generation step, and achieving superior performance in image quality and image-text alignment.
...
TL;DR: The paper introduces Bi-CMPStereo, a framework for robust 3D perception using event-frame asymmetric stereo, outperforming state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: The limitations of conventional frame-based cameras in dynamic scenes and the potential of event cameras to overcome these limitations.

Method: A novel bidirectional cross-modal prompting framework, Bi-CMPStereo, that exploits semantic and structural features from both event and frame domains.

Result: Bi-CMPStereo significantly outperforms state-of-the-art methods in accuracy and generalization.

Conclusion: The proposed Bi-CMPStereo framework enables reliable 3D perception under fast motion and challenging illumination by effectively leveraging complementary representations from event and frame domains.
...
TL;DR: A unified framework for dynamic large language model (LLM) execution is proposed, which integrates model compression and prompt compression to reduce latency and memory usage while preserving accuracy.


<details>
  <summary>Details</summary>
Motivation: Large language models require massive computational resources, and prior work on model compression and prompt compression have shown promise but remain separate and have limitations.

Method: The proposed framework uses compressed sensing to dynamically estimate task-conditioned and token-adaptive support sets, and compiles them into hardware-efficient sparse execution paths.

Result: The framework provides explicit approximation guarantees and deployment-oriented speedup constraints, and recasts LLM inference as a measurement-and-recovery problem.

Conclusion: The proposed framework has the potential to unify model compression and prompt compression, and provide a more efficient and adaptive approach to LLM execution.
...
TL;DR: The paper proposes MemGround, a new benchmark for evaluating long-term memory in large language models (LLMs) with a focus on dynamic and interactive scenarios.


<details>
  <summary>Details</summary>
Motivation: Current evaluations of long-term memory in LLMs are limited and static, neglecting complex memory systems like dynamic state tracking and hierarchical reasoning.

Method: The authors propose a three-tier hierarchical framework and a multi-dimensional metric suite to assess LLMs' long-term memory capabilities in interactive scenarios.
...
Conclusion: The paper highlights the need for more comprehensive and dynamic evaluation methods for long-term memory in LLMs, and introduces MemGround as a rigorous benchmark for this purpose.
...
TL;DR: HUOZIIME, a personalized on-device input method editor (IME) powered by large language models (LLMs), enabling efficient, real-time, and personalized text generation.


<details>
  <summary>Details</summary>
Motivation: Current mobile input method editors (IMEs) are limited to manual typing and struggle to produce personalized text, and there is a need for a personalized, privacy-preserving, and real-time generative IME.

Method: HUOZIIME uses a post-trained base LLM on synthesized personalization data, a hierarchical memory mechanism to capture user-specific input history, and systemic optimizations for efficient on-device deployment.

Result: Experiments demonstrate efficient on-device execution and high-fidelity memory-driven personalization.

Conclusion: HUOZIIME offers a solution for personalized on-device IMEs, achieving efficient and responsive operation under mobile constraints.
...
<details>
  <summary>Details</summary>
Motivation: The authors aim to investigate whether large language models (LLMs) can act as independent analytical agents to identify methodological flaws, particularly data leakage, in published studies.

Method: The authors analyze a gesture-recognition paper using six state-of-the-art LLMs, each analyzing the original paper without prior context using an identical prompt.
...
Conclusion: LLMs can detect common methodological issues based solely on published artifacts, highlighting their potential as complementary tools for improving reproducibility and supporting scientific auditing.
...
TL;DR: The authors analyze ICLR submissions to compare the effectiveness of numerical scores and text reviews in predicting paper acceptance. They find score-based models outperform text-based models, attributing this to the prevalence of polite but negative reviews.


<details>
  <summary>Details</summary>
Motivation: Authors struggle to interpret peer review feedback, often misinterpreting polite comments or specific low scores. The authors aim to investigate this issue by comparing the performance of numerical scores versus text reviews in predicting paper acceptance.

Method: The authors construct a dataset of over 30,000 ICLR 2021-2025 submissions and compare the acceptance prediction performance of score-based models versus text-based models, including those using large language models.

Result: Score-based models achieve 91% accuracy in predicting paper acceptance, while text-based models reach only 81%, even with large language models. The authors attribute this gap to the prevalence of the Politeness Principle in reviews, where rejected papers still contain more positive than negative sentiment words.

Conclusion: The study highlights the limitations of relying solely on text reviews for predicting paper acceptance, as textual information can be considerably less reliable due to the Politeness Principle. The authors suggest that authors may find it difficult to judge outcomes from text alone, and that numerical scores may provide more accurate predictions.
...
TL;DR: The paper presents SeaAlert, a framework for analyzing maritime distress communications using large language models (LLMs) to improve robustness in noisy and stressed environments.


<details>
  <summary>Details</summary>
Motivation: Maritime distress communications are safety-critical but often difficult to analyze automatically due to their brevity, noise, stress, and deviation from standardized formats.

Method: The authors develop a synthetic data generation pipeline using LLMs to produce realistic maritime messages, which are then synthesized into speech, degraded with VHF noise, and transcribed by an ASR system.

Result: The proposed framework, SeaAlert, aims to improve the analysis of maritime distress communications by addressing the scarcity of labeled real-world data and robustness in noisy environments.

Conclusion: The paper proposes a novel framework for robust analysis of maritime distress communications, leveraging LLMs and synthetic data generation to overcome challenges in automatic analysis.
...
TL;DR: The paper proposes a Teacher-Student Cooperation Data Synthesis framework (TESSY) to improve the performance of emerging reasoning models like Qwen3-8B by generating synthetic data that balances style and reasoning capabilities.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this work is to address the limitations of supervised fine-tuning (SFT) using synthetic data generated by a stronger model, which often fails to improve or even degrades the performance of emerging reasoning models.

Method: The authors propose TESSY, a framework that alternately generates style and non-style tokens by interleaving teacher and student models, producing synthetic sequences that combine the teacher's advanced reasoning capabilities with the student's stylistic consistency.

Result: The experimental results show that fine-tuning Qwen3-8B on teacher-generated data leads to performance drops, whereas TESSY achieves improvements of 11.25% on LiveCodeBench-Pro and 6.68% on OJBench.

Conclusion: The proposed TESSY framework effectively bridges the stylistic divergence between teacher-generated data and the student model's distribution, leading to improved performance in code generation tasks.
...
TL;DR: EviSearch is a system that automates creating clinical evidence tables from trial PDFs, ensuring accuracy and auditability.
...
Method: EviSearch uses a multi-agent extraction system, including a PDF-query agent, a retrieval-guided search agent, and a reconciliation module to ensure high-precision extraction across multimodal evidence sources.
...
Conclusion: EviSearch can accelerate clinical evidence table creation, improve accuracy, and provide a safe, auditable path for integrating LLM-based extraction into evidence synthesis pipelines.
...
TL;DR: A novel hierarchical RAG framework, H-TechniqueRAG, is proposed to improve the mapping of Cyber Threat Intelligence (CTI) text to MITRE ATT&CK technique IDs by leveraging the tactic-technique taxonomy.


<details>
  <summary>Details</summary>
Motivation: Current Retrieval-Augmented Generation (RAG) approaches for mapping CTI text to MITRE ATT&CK technique IDs treat all techniques uniformly, overlooking the inherent taxonomy of the ATT&CK framework.

Method: H-TechniqueRAG introduces a two-stage hierarchical retrieval mechanism, a tactic-aware reranking module, and a hierarchy-constrained context organization strategy.
...
Conclusion: The proposed H-TechniqueRAG framework demonstrates superior performance, cross-domain generalization, and interpretability, providing security analysts with highly interpretable, step-by-step decision paths.
...
TL;DR: This paper uses Large Language Models (LLMs) with Low-Rank Adaptation (LoRA) fine-tuning and in-context learning for Chinese rhetoric recognition, achieving top performance in an evaluation task.


<details>
  <summary>Details</summary>
Motivation: Rhetoric recognition is crucial for automated essay scoring as it helps assess linguistic and higher-order thinking skills, making it vital for AI in education.

Method: The paper leverages LLMs with LoRA-based fine-tuning and in-context learning, formulating outputs as JSON for structural results and using model ensemble methods for enhanced performance.

Result: The proposed method achieved the best performance on all three tracks of the CCL 2025 Chinese essay rhetoric recognition evaluation task, winning the first prize.

Conclusion: The study demonstrates the effectiveness of LLMs with LoRA and in-context learning for Chinese rhetoric recognition, highlighting its potential for improving automated essay scoring.
...
<details>
  <summary>Details</summary>
Motivation: The motivation behind SAGE Celer 2.6 is to develop a highly capable and versatile AI model that can handle complex reasoning tasks, support multimodal functionality, and provide strong performance in South Asian languages, particularly Nepali and Hindi.

Method: The authors used an Inverse Reasoning (IR) pipeline to train Celer 2.6, which allows the model to validate its own logic paths and minimize cascading errors. They also integrated a native multimodal functionality with an end-to-end vision encoder.

Result: Celer 2.6 achieved highly competitive results on mathematics, coding, and general intelligence benchmarks (ACUMEN), with low latency. It also demonstrated strong performance in Nepali and Hindi without sacrificing English reasoning ability.

Conclusion: The SAGE Celer 2.6 model offers significant advancements in AI capabilities, particularly in complex reasoning tasks, multimodal functionality, and South Asian language support, making it a valuable tool for a wide range of applications.
...
TL;DR: The paper presents a solution for conversational access to project meeting minutes, enabling professionals to pose natural-language questions and receive semantically relevant, time-annotated answers.
...
Method: The solution employs a Retrieval-Augmented Generation (RAG) framework that integrates semantic search with large language models.

Result: The approach is demonstrated using an anonymized dataset of meeting minutes from a construction project, and both the dataset and open-source implementation are made available to the community.

Conclusion: The proposed solution facilitates efficient and accurate retrieval of decision histories from project meeting minutes, supporting professionals in following the chronology of decisions.
...
TL;DR: Proposed a framework called Stateful Evidence-Driven RAG with Iterative Reasoning to improve Retrieval-Augmented Generation (RAG) for Large Language Models (LLMs) by iteratively refining queries and accumulating evidence.


<details>
  <summary>Details</summary>
Motivation: RAG suffers from flat context representations and stateless retrieval, leading to unstable performance. The authors aim to improve RAG by modeling question answering as a progressive evidence accumulation process.

Method: The proposed framework converts retrieved documents into structured reasoning units, maintains a persistent evidence pool, and performs iterative reasoning to refine queries and guide subsequent retrieval.
...
Conclusion: The proposed framework improves RAG by enabling stable evidence aggregation and robustness to noisy retrieval, effectively accumulating high-quality evidence.
...
TL;DR: This study benchmarks the performance of three open-weight Large Language Models (LLMs) in adapting to Romanized Nepali, a low-resource language, and finds that Qwen3-8B performs best overall, while Llama-3.1-8B shows the largest improvement after fine-tuning.
...
Method: The study uses a curated bilingual dataset of 10,000 transliterated instruction-following samples to evaluate the performance of Llama-3.1-8B, Mistral-7B-v0.1, and Qwen3-8B under zero-shot and fine-tuned settings, using metrics such as perplexity, BERTScore, chrF++, and ROUGE.

Result: The results show that all three models fail to generate Romanized Nepali at zero-shot, but after fine-tuning, they converge to similar performance, with Qwen3-8B performing best overall and Llama-3.1-8B showing the largest improvement.

Conclusion: This study establishes the first rigorous baseline for Romanized Nepali adaptation in comparable-sized open-weight LLMs and confirms the adaptation headroom hypothesis, which suggests that models with weaker zero-shot performance can achieve larger gains through fine-tuning.
...
TL;DR: The paper proposes a two-stage framework called CRVA-TGRAG to address knowledge discrepancy and conflict in CVE detection and analysis, which improves the accuracy of Large Language Models (LLMs) in retrieving the latest knowledge.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the problem of knowledge discrepancy and conflict within CVE detection and analysis, which hinders LLMs' ability to retrieve the latest knowledge, leading to knowledge conflicts and generation hallucinations.

Method: The method proposed is a two-stage framework called CRVA-TGRAG, which consists of (1) improving document retrieval accuracy using Parent Document Segmentation and an ensemble retrieval scheme, and (2) enhancing LLMs' capabilities using a teacher-guided preference optimization technique.

Result: The framework achieves higher accuracy in retrieving the latest CVEs compared to external knowledge bases, and significantly mitigates potential knowledge conflicts and inconsistencies.

Conclusion: The proposed framework CRVA-TGRAG effectively addresses the problem of knowledge discrepancy and conflict in CVE detection and analysis, and improves the accuracy of LLMs in retrieving the latest knowledge.
...
TL;DR: Researchers developed a small adapter that can correct politically biased language models by restoring factual log-probabilities on sensitive topics without altering the model's knowledge or generating incoherent text.


<details>
  <summary>Details</summary>
Motivation: Alignment-tuned language models often suppress factual information on politically sensitive topics, even though they retain the knowledge internally. The goal is to correct this suppression without retraining the entire model.

Method: A 786K-parameter post-transformer adapter was trained on frozen hidden states of language models (Qwen3-4B, 8B, and 14B) to correct the suppression of factual log-probabilities on sensitive topics.

Result: The adapter successfully corrected the suppression on 31 ideology-discriminating facts, memorizing all training facts and generalizing to 11-39% of held-out facts. It produced coherent, less censored text when applied only at the current prediction position during generation.

Conclusion: The study demonstrates the effectiveness of a small, targeted adapter in correcting politically biased language models. It highlights the importance of hidden-state intervention for generation correction and discusses implications for future adapter research.
...
TL;DR: The paper presents a unified system for addressing two subtasks in the ArchEHR-QA Shared Task: answer generation and evidence sentence alignment. The system achieves scores of 32.87 and a micro-F1 of 67.16 on the test sets for the two subtasks, respectively.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the challenges of Subtask 3 (answer generation) and Subtask 4 (evidence sentence alignment) of the ArchEHR-QA Shared Task, specifically with limited annotated training data.

Method: The method involves applying two-stage Quantised Low-Rank Adaptation (QLoRA) to Qwen3-4B for Subtask 3 and developing a weighted ensemble of three retrieval methods for Subtask 4.

Result: The system achieves an overall score of 32.87 on the official test-2026 split for Subtask 3 and a micro-F1 of 67.16 on the 100-case test set for Subtask 4.

Conclusion: The experiments reveal that both subtasks face the challenge of insufficient annotated training data (only 20 cases), suggesting data augmentation as a key future direction.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to provide actionable, level-appropriate, and encouraging feedback for learners in teaching scenarios, which is not adequately addressed by current GEC and GEE systems.

Method: The method involves introducing a new dataset, SPFG, and evaluating three instruction-tuned LLMs (Qwen2.5, Llama-3.1, and GLM-4) using supervised fine-tuning (SFT) and preference-based alignment (DPO and KTO) for joint generation of corrections and feedback.
...
Conclusion: The paper concludes that the proposed SPFG dataset and the evaluated methods can be used to generate learner-friendly pedagogical feedback for spoken grammatical error correction, with SFT being the most effective approach.
...
TL;DR: The use of large language models (LLMs) in rare disease patient education and communication is in its early stages, with most studies focusing on general-purpose models and patient question answering, but neglecting patient-centered dimensions and real-world deployment.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to explore the application of large language models (LLMs) in supporting patient education and communication for rare diseases, which affect over 300 million people worldwide and have complex care pathways and substantial unmet communication needs.

Method: The authors conducted a scoping review of studies published between January 2022 and March 2026 across major databases, identifying 12 studies on LLM-based rare disease patient education and communication, and synthesizing data using descriptive and qualitative analyses.

Result: The literature review found that the field is dominated by general-purpose models, particularly ChatGPT, and that most studies focus on patient question answering using curated question sets, with limited attention to patient-centered dimensions such as readability, empathy, and communication quality.

Conclusion: The authors conclude that the field remains at an early stage and that future research should prioritize patient-centered design, domain-adapted methods, and real-world deployment to support safe, adaptive, and effective communication in rare diseases.
...
TL;DR: The study trains a large language model on Classical Chinese text and tests its ability to distinguish known from unknown inputs and express uncertainty. The model shows internal uncertainty but fails to express it externally, relying on training data conventions instead.
...
Result: The model exhibits internal uncertainty, as shown by a perplexity jump ratio between real and fabricated historical events, but fails to express uncertainty externally, instead relying on rhetorical conventions. This finding is replicated across three languages and eight models.

Conclusion: The study concludes that metacognitive expression, or the ability to say 'I don't know', does not emerge from language modeling alone and requires explicit training signals, such as reinforcement learning from human feedback (RLHF).
...
TL;DR: The paper proposes a two-stage approach to distill a Transformer model into a Mamba-like architecture, preserving the original performance while leveraging the efficiency of Mamba.


<details>
  <summary>Details</summary>
Motivation: To facilitate the adoption of State Space Models (SSMs) like Mamba by leveraging existing pretrained Transformers, while addressing the limitation of naïve distillation procedures that fail to preserve the original teacher performance.

Method: A principled two-stage approach: (1) distilling knowledge from a Transformer into a linearized version of Attention using the kernel trick, and (2) distilling the linearized version into an adapted Mamba model without Attention blocks.

Result: The distilled Mamba model preserves the original Pythia-1B Transformer performance in downstream tasks, maintaining a perplexity of 14.11 close to the teacher's 13.86.

Conclusion: The proposed approach provides an effective recipe for cross-architectural distillation, enabling the adoption of SSMs like Mamba while leveraging existing pretrained Transformers.
...
TL;DR: A framework called PICCO is proposed for structuring LLM prompts, consisting of five elements: Persona, Instructions, Context, Constraints, and Output.


<details>
  <summary>Details</summary>
Motivation: The performance of large language models depends heavily on prompt design, but prompt construction is often inconsistent, highlighting the need for a reference framework.

Method: A rigorous synthesis of 11 previously published prompting frameworks was conducted to derive the PICCO framework.

Result: The PICCO framework provides a taxonomy and a five-element reference architecture for prompt generation, along with definitions of its elements, functions, scope, and relationships.

Conclusion: The PICCO framework offers a conceptual and methodological contribution to improving conceptual clarity and supporting systematic prompt design, but does not claim empirical validation as an optimization method.
...
TL;DR: The claim that Chinese prompts are more token-efficient than English for LLM coding tasks is not supported by empirical evidence. In fact, the study finds that Chinese prompts have a lower success rate and variable token costs depending on the model architecture.
...
Result: The study reveals three key findings: (1) the efficiency advantage of Chinese is not observed, (2) token cost varies by model architecture, and (3) the success rate when prompting in Chinese is generally lower than in English across all models tested.

Conclusion: The findings suggest that language effects on token cost are model-dependent, and practitioners should not expect cost savings or performance gains just by switching their prompt language to Chinese. The results should be interpreted as preliminary evidence rather than a definitive conclusion.
...
TL;DR: The paper proposes Cost-Regularized Optimization of Prompts (CROP), an automatic prompt optimization method that reduces token consumption while maintaining accuracy in large language models.


<details>
  <summary>Details</summary>
Motivation: Existing automatic prompt optimization frameworks prioritize task accuracy over response length, leading to verbose and costly responses. The authors aim to balance accuracy with efficiency.

Method: CROP introduces regularization on response length by generating textual feedback in addition to standard accuracy feedback, forcing the optimization process to produce concise prompts.

Result: The approach achieved an 80.6% reduction in token consumption while maintaining competitive accuracy on complex reasoning datasets (GSM8K, LogiQA, and BIG-Bench Hard).

Conclusion: CROP presents a pragmatic solution for deploying token-efficient and cost-effective agentic AI systems in production pipelines, balancing accuracy with efficiency.
...
<details>
  <summary>Details</summary>
Motivation: Hate speech detection in multimodal content with Devanagari script is challenging due to its complexity, data scarcity, and script-specific linguistic nuances.

Method: A hybrid cross-modal attention fusion architecture combining CLIP (ViT-B/32) for visual encoding and BGE-M3 for multilingual text representation, connected through self-attention and a learnable gating network.

Result: The proposed architecture achieves a 5.9% F1-macro improvement over text-only baselines on Subtask A, while revealing that English-centric vision models perform poorly on Devanagari script and ensemble methods degrade under data scarcity.

Conclusion: The paper proposes an effective system for hate speech detection in Devanagari-scripted memes, highlighting the importance of cross-modal reasoning and the challenges of data scarcity and script-specific complexity.
...
Conclusion: The authors' approach improves the quality of LLM-based peer reviews by leveraging explicit rubrics and contextual grounding in existing work.
...
Method: The study follows FAIR data principles and SPIRIT-AI guidelines to curate a dataset from official regulatory exams in Italy, France, Spain, and Portugal, and uses an automated translation pipeline and zero-shot prompting strategy to evaluate contemporary multimodal LLMs.

Result: The study aims to provide a contamination-resistant benchmark for evaluating LLMs, but does not report specific results; instead, it proposes EuropeMedQA as a resource for future evaluations and development of more generalizable medical AI.

Conclusion: The EuropeMedQA dataset and evaluation approach aim to foster the development of more generalizable medical AI by providing a comprehensive and rigorous benchmark for assessing the performance of LLMs in multilingual and multimodal contexts.
...
TL;DR: This study analyzes the dynamics of news reporting on violent and catastrophic events using a large corpus of news articles, identifying patterns in publication volume, semantic drift, and term relevance.


<details>
  <summary>Details</summary>
Motivation: Understanding how narratives form, propagate, and evolve during moments of crisis is essential for interpreting public discourse.

Method: The study uses a large-scale corpus of 126,602 news articles collected from online publishers, quantifying narrative change through publication volume, semantic drift, semantic dispersion, and term relevance.
...
Conclusion: The study provides insights into the temporal and semantic dynamics of news reporting on violent and catastrophic events, identifying patterns and key terms driving these patterns.
...
<details>
  <summary>Details</summary>
Motivation: To investigate the capability of AI models in predicting fan experience ratings from text data and understand the relationship between predicted and self-reported ratings.
...
Result: The model achieved 67% accuracy within +/-1 point and 36% exact matches. Predicted ratings correlated strongly with overall experience (r = 0.82) but were systematically lower than self-reported ratings by about one point.

Conclusion: The study establishes baseline results for using simple prompts to predict fan experience ratings and highlights a meaningful construct difference between predicted and self-reported ratings, suggesting that each measure captures distinct aspects of the fan experience.
...
TL;DR: The paper proposes GeoDe, a framework for abstention fine-tuning of large language models to reduce hallucinations by filtering out ambiguous samples.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the issue of hallucinations in large language models due to their inability to accurately perceive their knowledge boundaries.

Method: The method proposed is the GeoDe framework, which uses geometric distance as a confidence signal for abstention decisions.

Result: The results show that GeoDe significantly enhances model truthfulness and demonstrates strong generalization in out-of-distribution scenarios across multiple models and benchmark datasets.

Conclusion: The conclusion is that the GeoDe framework is effective in reducing hallucinations in large language models by filtering out ambiguous boundary samples.
...
TL;DR: The paper assesses and improves the faithfulness of explanations generated by large language models (LLMs) through a training-free method involving attention-level interventions.


<details>
  <summary>Details</summary>
Motivation: The lack of explainability in LLMs limits their use in domains requiring transparency and trust. While post-hoc text-based explanations are a promising direction, their epistemic faithfulness is unclear.

Method: The paper introduces a training-free method that enhances faithfulness by guiding explanation generation through attention-level interventions, informed by token-level heatmaps.

Result: The method significantly improves epistemic faithfulness across multiple models, benchmarks, and prompts.

Conclusion: The study highlights the need for epistemically faithful explanations in LLMs and proposes an effective method to achieve this, enhancing the transparency and trustworthiness of LLMs.
...
TL;DR: The paper proposes RoPE-Perturbed Self-Distillation to improve positional robustness in large language models for long-context understanding, achieving gains on benchmarks.


<details>
  <summary>Details</summary>
Motivation: Standard long-context adaptation of language models can be brittle, with accuracy heavily dependent on the absolute placement of relevant evidence.

Method: The authors propose RoPE-Perturbed Self-Distillation, which involves perturbing RoPE indices of training sequences and training the model to produce consistent predictions across views.

Result: Experiments on Llama-3-8B and Qwen-3-4B show consistent gains on long-context benchmarks, with up to 12.04% improvement on RULER-64K and 2.71% on RULER-256K.
...
TL;DR: Developed open-source language models to detect body image distress, disordered eating, and metabolic challenges in PCOS patients' social media posts with explainability.


<details>
  <summary>Details</summary>
Motivation: Existing NLP approaches lack transparency and cannot identify co-occurring presentations of body image distress, disordered eating, and metabolic challenges in PCOS patients.

Method: Fine-tuned three language models (Gemma-2-2B, Qwen3-1.7B, DeepSeek-R1-Distill-Qwen-1.5B) using Low-Rank Adaptation on 1,000 PCOS-related social media posts labeled by two annotators.
...
Conclusion: The developed models are suitable for screening rather than autonomous diagnosis, particularly for complex cases, and can help identify PCOS patients' mental and physical health challenges.
...
TL;DR: The paper introduces APEX-MEM, a conversational memory system that improves long-term conversational memory in large language models by using a property graph, append-only storage, and a multi-tool retrieval agent.
...
TL;DR: The paper explores why multimodal language models struggle with visual perception tasks, finding that language representations often overshadow vision, but proposes a method to improve performance by contrastively decoding against a text-centroid-erased reference.


<details>
  <summary>Details</summary>
Motivation: Understanding why multimodal language models underperform on visual perception tasks and finding a way to improve their performance.

Method: The paper proposes centroid replacement, a method that collapses each token to its nearest K-means centroid, and text centroid contrastive decoding, which decodes against a text-centroid-erased reference.

Result: The proposed method recovers up to +16.9% accuracy on individual tasks and shows larger gains in standard fine-tuned models (+5.6% on average) compared to preference-optimized models (+1.5% on average).

Conclusion: The findings suggest that modal competition is structurally localized, correctable at inference time without retraining, and quantifiable as a diagnostic signal to guide future multimodal training.
...
TL;DR: The paper proposes a method to improve automated fact-checking in dialogue by de-colloquialising response claims and introducing a semantics-aware consistency gate.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the understudied area of colloquial language in multi-turn conversations in automated fact-checking.

Method: The method involves staged de-colloquialisation with a conservative rewrite candidate and a semantics-aware consistency gate called BiCon-Gate.

Result: The approach improves fact-checking on the DialFact benchmark, particularly in evidence retrieval and fact verification, outperforming competitive baselines.

Conclusion: The proposed approach effectively stabilises downstream fact-checking and yields gains in evidence retrieval and fact verification.
...
TL;DR: The paper presents a method for automatically expanding WordNet-style lexical resources to new languages through sense generation, achieving improved precision and interpretability.


<details>
  <summary>Details</summary>
Motivation: The motivation is to automatically expand WordNet-style lexical resources to new languages, which is a challenging task due to the need for sense generation and alignment across languages.
...
Result: The results show that the proposed project-and-filter strategy improves precision while remaining interpretable and requiring few external resources, outperforming prior methods and baselines.

Conclusion: The proposed method offers an effective and interpretable approach to expanding lexical resources in new languages, with plans to make the code, documentation, and generated sense inventories accessible.
...
Method: The authors characterize the autocorrelation structure of 66 turn-level metrics across 202 multi-turn conversations and propose a two-stage correction framework combining Chelton's effective degrees of freedom with conversation-level block bootstrap.

Result: The correction framework is validated on a hold-out split, showing that cluster-robust metrics replicate at 57% versus 30% for pooled-only metrics. A survey of recent papers finds that only 4 out of 30 address temporal dependence.

Conclusion: The paper highlights the importance of correcting for autocorrelation in turn-level metrics and provides design principles, a publication checklist, and open-source code for the correction pipeline.
...
TL;DR: The paper presents Three-Phase Transformer (3PT), a new architecture for decoder-only Transformers that achieves improved perplexity and convergence speed over a baseline model.


<details>
  <summary>Details</summary>
Motivation: The authors aim to improve the performance of decoder-only Transformers by introducing a novel structural prior, called Three-Phase Transformer (3PT), which incorporates a residual-stream structural prior and a fixed Gabriel's horn profile.

Method: The 3PT architecture partitions the hidden vector into N equally-sized cyclic channels, each maintained by phase-respecting operations, including per-channel RMSNorm, 2D Givens rotation, and head-count constraint. The architecture also injects a fixed Gabriel's horn profile as an absolute-position side-channel.

Result: The 3PT model achieves -7.20% perplexity (-2.62% bits-per-byte) over a matched RoPE-Only baseline at +1,536 parameters, with 1.93x step-count convergence speedup (1.64x wall-clock) on WikiText-103 at 123M parameters.

Conclusion: The authors conclude that the 3PT architecture is a self-stabilizing equilibrium between scrambling and re-imposition, and that the load-bearing mechanism is the channel-partitioned residual stream, per-block rotation, per-phase normalization, and horn DC injection.
...
TL;DR: The paper compares the representational quality of hierarchically structured, shared-weight recurrence and independent-layer stacking in a Transformer-based language model.


<details>
  <summary>Details</summary>
Motivation: To investigate whether hierarchically structured, shared-weight recurrence can match the representational quality of independent-layer stacking in a Transformer-based language model.

Method: The paper proposes HRM-LM, a model that replaces L independent Transformer layers with a two-speed recurrent pair: a Fast module and a Slow module. A parameter-matched Universal Transformer ablation (UniTF, 1.2B) is used for comparison.

Result: The study finds a sharp empirical gap between the two approaches, suggesting that hierarchically structured, shared-weight recurrence may not match the representational quality of independent-layer stacking.

Conclusion: The paper concludes that there is a significant representational quality gap between hierarchically structured, shared-weight recurrence and independent-layer stacking in Transformer-based language models.
...
<details>
  <summary>Details</summary>
Motivation: The reliability of LLMs depends on their ability to search the web, select relevant evidence, and synthesize complete answers, but multilingual settings, particularly Portuguese, remain underexplored.

Method: The authors present MARCA, a benchmark consisting of 52 manually authored multi-entity questions, paired with manually validated rubrics, and evaluate 14 models under two interaction settings: Basic and Orchestrator frameworks.
...
Conclusion: The paper concludes that MARCA is a valuable resource for evaluating LLMs in multilingual settings, highlighting areas for improvement in LLM development.
...
TL;DR: The study examines if LLMs develop shared representations for filler-gap dependencies across different syntactic constructions with limited training data.


<details>
  <summary>Details</summary>
Motivation: To determine if LLMs, like humans, require a shared representation across different syntactic constructions for filler-gap dependencies, and if this can be achieved with developmentally feasible amounts of data.

Method: Distributed Alignment Search (DAS) was applied to LMs trained on varying amounts of data from the BabyLM challenge.
...
TL;DR: The paper introduces a psychological steering framework for large language models (LLMs) using semantically calibrated units, outperforming existing methods in open-ended generation tasks.


<details>
  <summary>Details</summary>
Motivation: Existing methods for intervening in LLMs have limitations, such as restricted search space and uncalibrated activation-space units, which may miss optimal intervention conditions.

Method: The authors propose a psychological steering framework that performs unbounded, fluency-constrained sweeps in semantically calibrated units, deriving and calibrating residual-stream injections using psychological artifacts.

Result: The mean-difference (MD) injections outperform Personality Prompting (P$^2$) in 11 of 14 LLMs, with gains of 3.6% to 16.4%, and a hybrid of P$^2$ and MD injections outperforms both methods in 13 of 14 LLMs.

Conclusion: The study shows that MD injections provide reliable, approximately linear control knobs for psychological steering, but also induce OCEAN trait covariance patterns that depart from the Big Two model, highlighting a gap between learned representations and human psychology.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of current neural and classical probabilistic models in topic modeling, which struggle with lifelong learning, require extensive tuning, and lack flexibility and adaptability to streaming data.

Method: The method is based on adapting the Cobweb algorithm to continuous document embeddings, constructing semantic hierarchies online, and using incremental probabilistic concept formation.
...
Conclusion: The conclusion is that incremental symbolic concept formation combined with pretrained representations is an efficient approach to topic modeling, enabling lifelong learning and adaptability to streaming data.
...
TL;DR: The authors introduce PeerPrism, a benchmark for detecting Large Language Model (LLM) usage in peer reviews, highlighting the complexity of human-AI collaboration and the limitations of current detection methods.


<details>
  <summary>Details</summary>
Motivation: Existing peer-review LLM detection methods are insufficient as they treat authorship as a binary problem (human vs. AI), not accounting for the hybrid nature of modern review workflows where evaluative ideas and surface realization may come from different sources.

Method: The authors create a large-scale benchmark, PeerPrism, with 20,690 peer reviews, and design controlled generation regimes to evaluate state-of-the-art LLM text detection methods, assessing their ability to identify the origin of surface text and evaluative reasoning.

Result: Current detection methods achieve high accuracy on binary tasks but produce contradictory classifications under hybrid regimes, conflating surface realization with intellectual contribution, indicating that authorship must be modeled as a multidimensional construct.

Conclusion: The study demonstrates that LLM detection in peer review cannot be reduced to a binary attribution problem, and that authorship must be modeled as a multidimensional construct spanning semantic reasoning and stylistic realization, and introduces PeerPrism as a benchmark for future research.
...
TL;DR: The paper proposes a Cognitive Reverse-Engineering framework to analyze how Large Language Models (LLMs) process complex emotions, specifically social-comparison jealousy. It finds that LLMs encode jealousy as a structured linear combination of psychological antecedents, consistent with human psychological constructs.


<details>
  <summary>Details</summary>
Motivation: The internal mechanisms by which LLMs process complex emotions remain unclear, and existing interpretability approaches often treat models as black boxes or focus on coarse-grained basic emotions.
...
Result: Experiments on eight LLMs suggest that models natively encode jealousy as a structured linear combination of psychological antecedents, specifically Superiority of Comparison Person and Domain Self-Definitional Relevance.

Conclusion: The framework demonstrates that toxic emotional states can be mechanically detected and surgically suppressed, suggesting a possible route toward representational monitoring and intervention for AI safety in multi-agent environments.
...
TL;DR: The paper argues that diversity progress in NLP is concentrated on fairness, and that incentives, biases, and barriers marginalize researchers in other areas, proposing recommendations for a more inclusive NLP.


<details>
  <summary>Details</summary>
Motivation: The concentration of diversity progress in NLP on fairness and the marginalization of researchers in non-fairness fields.

Method: Investigation into the demographics of NLP researchers by subfield.

Result: The paper identifies disparities and proposes recommendations to address them.

Conclusion: The paper concludes that NLP can become more inclusive and equitable by breaking down feedback loops and addressing geographical and linguistic barriers.
...
TL;DR: The authors propose CAUSALDETOX, a framework to reduce toxic content generated by large language models by identifying and intervening on specific attention heads.
...
Method: The authors use the Probability of Necessity and Sufficiency (PNS) to identify attention heads responsible for toxicity and propose two strategies: Local Inference-Time Intervention and PNS-Guided Fine-Tuning.
...
Conclusion: The proposed framework CAUSALDETOX effectively reduces toxic content generated by large language models while maintaining generation quality.
...
TL;DR: The authors propose Retrieval-Augmented Set Completion (RASC) to improve clinical value set authoring by retrieving similar existing value sets and applying a classifier to filter codes.


<details>
  <summary>Details</summary>
Motivation: Clinical value set authoring is a bottleneck in clinical quality measurement and phenotyping due to the large size and complexity of standardized vocabularies.

Method: The authors propose RASC, which involves retrieving the K most similar existing value sets and applying a classifier to each candidate code.

Result: RASC outperforms simpler approaches, achieving AUROC~0.852 and value-set-level F1~0.298, and reduces the number of irrelevant candidates per true positive.

Conclusion: The proposed RASC approach demonstrates improved performance in clinical value set authoring, especially for larger value sets, and is model-agnostic.
...
TL;DR: StoryCoder, a narrative reformulation framework, improves code generation by transforming questions into coherent natural language narratives, leading to better model performance and more modular code.


<details>
  <summary>Details</summary>
Motivation: Existing code generation approaches lack structured problem representation, which hinders model capability; inspired by human organization of information, a narrative reformulation framework is proposed.

Method: StoryCoder transforms code generation questions into narratives with three components: task overview, constraints, and example test cases, guided by algorithm and genre.

Result: Experiments on 11 models across HumanEval, LiveCodeBench, and CodeForces show consistent improvements with an average gain of 18.7% in zero-shot pass@10.

Conclusion: Narrative reformulation guides models toward correct algorithmic strategies, reduces implementation errors, and induces a more modular code structure, highlighting the importance of structured problem representation in code generation.
...
TL;DR: The paper proposes a massive option evaluation protocol to test large language models, revealing that strong performance in low-option settings can overstate model competence.


<details>
  <summary>Details</summary>
Motivation: Current multiple-choice evaluation methods for large language models can be flawed due to shortcut strategies that obscure true competence, especially in low-option settings.
...
Result: The results show that strong performance in low-option settings can overstate model competence, and two failure modes (semantic confusion and position bias) are identified.

Conclusion: The massive option evaluation protocol is a general framework for stress testing model reliability under extreme distractor density, revealing gaps that conventional benchmarks tend to obscure.
...
<details>
  <summary>Details</summary>
Motivation: The proliferation of financial misinformation threatens market stability and investor trust, and detecting it is challenging, especially without external evidence.

Method: The proposed framework uses Large Language Models (LLMs) with in-context learning (zero-shot and few-shot prompting) and Parameter-Efficient Fine-Tuning (PEFT) via Low-Rank Adaptation (LoRA).
...
Conclusion: The proposed method demonstrates superior efficacy in detecting financial misinformation, contributing to the development of context-aware misinformation detection in financial NLP.
...
TL;DR: The paper proposes CURaTE, a method for continual unlearning in real-time for large language models, which effectively forgets specific pieces of knowledge while preserving the model's overall knowledge.


<details>
  <summary>Details</summary>
Motivation: The need for methods to unlearn specific pieces of knowledge after training large language models, as existing techniques have limitations in terms of continuous and immediate action, leading to degraded utility and exposure of sensitive information.

Method: The proposed method, CURaTE, involves training a sentence embedding model on a dataset to enable sharp decision boundaries for determining whether a given input prompt corresponds to any stored forget requests, and then using similarity to determine whether to answer or return a refusal response.

Result: CURaTE achieves more effective forgetting than existing methods, maintains near-perfect knowledge preservation over any number of updates, and is capable of continual unlearning in real-time.

Conclusion: The proposed CURaTE method effectively addresses the limitations of existing techniques for unlearning specific pieces of knowledge in large language models, providing a more effective and efficient approach for continual unlearning in real-time.
...
TL;DR: The paper proposes Clinical Uncertainty Risk Alignment (CURA), a framework that improves the calibration and reliability of clinical language models for risk prediction from free-text notes.


<details>
  <summary>Details</summary>
Motivation: Clinical language models' uncertainty estimates are often poorly calibrated and clinically unreliable, which can lead to incorrect decisions.

Method: CURA fine-tunes domain-specific clinical LMs and uses a bi-level uncertainty objective to align risk estimates and uncertainty with individual error likelihoods and cohort-level ambiguities.

Result: CURA consistently improves calibration metrics without compromising discrimination, and yields more trustworthy uncertainty estimates for downstream clinical decision support.

Conclusion: The proposed CURA framework provides a reliable and trustworthy approach for clinical risk prediction, which can support clinical decision-making.
...
Result: The study identifies structured gender-space associations in LLMs, forming nuanced micro-level mappings, and finds that biases are embedded and reinforced across the model pipeline, producing concrete failures in application settings.
...
TL;DR: This paper introduces a new task, NEgative-conSTrained (NEST) KGQA, and a dataset, NestKGQA, to address the limitations of existing KGQA benchmarks and methods in handling negative constraints. It proposes a novel framework, CUCKOO, to improve the semantic executability of multiple-constrained questions.


<details>
  <summary>Details</summary>
Motivation: Existing KGQA benchmarks and methods are biased toward positive and calculation constraints, neglecting negative constraints that frequently appear in real-world questions.

Method: The paper proposes a novel framework, CUCKOO, which generates a constraint-aware logical form draft, performs schema-guided semantic matching, and selectively applies self-directed refinement to improve semantic executability.

Result: Experimental results demonstrate that CUCKOO consistently outperforms baselines on both conventional KGQA and NEST-KGQA benchmarks under few-shot settings.

Conclusion: The paper concludes that CUCKOO is effective in handling multiple-constrained questions with negative constraints, and highlights the importance of considering negative constraints in KGQA tasks.
...
TL;DR: The authors introduce CoPA, a benchmark for evaluating personalized Question Answering (QA) by considering individual user preferences and cognitive patterns.


<details>
  <summary>Details</summary>
Motivation: The current methods for evaluating personalization in QA rely heavily on lexical-level similarity or manual heuristics, lacking data-driven validation.

Method: The authors mine Community-Individual Preference Divergence (CIPD) to identify key personalization factors and create a benchmark, CoPA, with 1,985 user profiles.
...
Conclusion: The proposed CoPA benchmark offers a more refined approach to assessing personalized QA systems, moving beyond generic metrics.
...
TL;DR: The paper introduces MM-AQA, a benchmark for evaluating multimodal abstention in vision-language models and multi-agent systems, highlighting the need for abstention-aware training.


<details>
  <summary>Details</summary>
Motivation: Current evaluation paradigms for multimodal systems assume answerability, but effective abstention is critical for reliable systems, and existing benchmarks ignore unanswerability or use coarse methods.

Method: The authors introduce MM-AQA, a benchmark that constructs unanswerable instances from answerable ones via transformations, and evaluate three VLMs and two MAS architectures across 2079 samples.
...
TL;DR: The paper proposes a new framework for machine unlearning in large language models, prioritizing retention of general capabilities while removing targeted knowledge. It introduces a novel method, SAGO, which improves upon existing methods in preserving model performance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of machine unlearning in large language models (LLMs), which aims to remove targeted knowledge without compromising general capabilities.

Method: The paper proposes a retention-prioritized gradient synthesis framework, which includes adapting PCGrad to resolve gradient conflicts and introducing SAGO, a novel retention-prioritized gradient synthesis method.
...
Conclusion: The conclusion is that re-shaping gradient geometry, rather than re-balancing losses, is key to mitigating unlearning-retention trade-offs in machine unlearning for LLMs.
...
TL;DR: The paper explores fine-tuning the Finnish BERT model on unlabeled Finnish medical text data and predicting the benefit of domain-specific pre-training by observing embedding changes.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the common situation in healthcare AI where acquiring labeled datasets can be delayed, and to utilize unlabeled data for domain fine-tuning.
...
Conclusion: The conclusion is not provided, but the paper likely aims to conclude with insights on the effectiveness of fine-tuning Finnish BERT on medical text data and the potential for predicting the benefit of domain-specific pre-training.
...
TL;DR: The paper presents Pangu-ACE, a cascaded system for educational tasks that uses a 1B tutor-router and a 7B specialist prompt to improve efficiency and quality.


<details>
  <summary>Details</summary>
Motivation: The motivation is to develop an efficient educational assistant that spends more computation only when the task needs it.

Method: The method involves a sample-level 1B to 7B cascade for the shared-8 EduBench benchmark, using a 1B tutor-router to produce a draft answer and routing signals, and then either accepting the draft or escalating the sample to a 7B specialist prompt.

Result: The results show that Pangu-ACE improves deterministic quality from 0.457 to 0.538 and format validity from 0.707 to 0.866 over the legacy rule_v2 system, while accepting 19.7% of requests directly at 1B.

Conclusion: The conclusion is that Pangu-ACE achieves improved quality and efficiency through routing selectivity, but latency gains are not yet evident.
...
TL;DR: The study evaluates the effectiveness of Large Language Models (LLMs) in Behavioral Profile (BP) annotation, a complex task requiring simultaneous coding across multiple linguistic dimensions. It finds that BP annotation is highly heterogeneous at the skill level and that LLMs can execute certain skills with substantial reliability, but not others.


<details>
  <summary>Details</summary>
Motivation: The difficulty in automating Behavioral Profile (BP) annotation due to its complexity and the need for simultaneous coding across multiple linguistic dimensions.

Method: The study uses a skill-file-driven pipeline with a 14-feature BP schema, decision rules, and examples. It evaluates LLM-assisted BP annotation by comparing the performance of human annotators and LLMs (GPT-5.4 and three open-source models) on a 300-instance validation subset.

Result: The results show that 5 BP skills are directly operable, 4 are recoverable after focused re-annotation, and 5 remain structurally underspecified. GPT-5.4 achieves substantial reliability in executing the retained skills (accuracy = 0.678, κ = 0.665, weighted F1 = 0.695). Human and GPT difficulty profiles are strongly aligned at the skill level but not at the instance or lexical-item level.

Conclusion: The study concludes that automatic annotation should be evaluated in terms of skill feasibility rather than task-level automation. It also suggests that LLMs can be viewed as an independent third skill voice rather than a direct human substitute.
...
<details>
  <summary>Details</summary>
Motivation: Existing datasets do not capture complex causal networks, particularly implicit and nested causality, which are crucial for understanding climate change.
...
TL;DR: This paper explores how the wording of schema keys affects model performance in structured generation with large language models, proposing a new perspective on schema design as an implicit instruction channel.


<details>
  <summary>Details</summary>
Motivation: The motivation is to investigate how schema key formulation influences model behavior in structured generation, as existing approaches overlook the linguistic formulation of schemas.

Method: The method involves studying the impact of instruction placement on model performance, reinterpreting structured generation as a multi-channel instruction problem, and conducting experiments on multiple mathematical reasoning benchmarks.

Result: The results show that different model families exhibit distinct sensitivities to instruction channels, and that combining multiple channels does not always lead to further improvement.

Conclusion: The conclusion is that schema design not only determines output structure but also carries instruction signals, offering a new perspective on structured generation in LLMs.
...
Method: The paper proposes a new streaming probing objective that requires multiple evidence tokens to consistently support a prediction, rather than relying on isolated spikes, and explores probing different LLM components such as Attention, MLP activations, and residual-stream features.
...
Conclusion: The proposed method provides more robust detection of harmful content in LLMs, particularly in high-stakes CBRN domains, and can be applied 'plug-and-play' to obfuscated attacks, achieving high AUROC scores.
...
TL;DR: RACER is a lightweight, training-free method that accelerates Large Language Model (LLM) decoding by integrating retrieved exact patterns with logit-driven future cues, achieving over 2x speedup.


<details>
  <summary>Details</summary>
Motivation: Autoregressive decoding in LLMs causes high inference latency, and existing speculative decoding methods face trade-offs between retrieval-based and logits-based drafts.

Method: RACER (Retrieval-Augmented Contextual Rapid Speculative Decoding) combines retrieved exact patterns with logit-driven future cues to generate richer speculative drafts.

Result: RACER consistently accelerates inference, achieving over 2x speedup over autoregressive decoding and outperforming prior training-free methods on Spec-Bench, HumanEval, and MGSM-ZH.

Conclusion: RACER offers a scalable, plug-and-play solution for efficient LLM decoding, providing a lightweight and training-free approach to mitigate inference latency.
...
<details>
  <summary>Details</summary>
Motivation: To understand how vision language models (VLMs) integrate visual and textual information and to analyze their reasoning dynamics.

Method: Analyzing 18 VLMs, tracking confidence over Chain-of-Thought (CoT), measuring the corrective effect of reasoning, and evaluating the contribution of intermediate reasoning steps.

Result: VLMs are prone to 'answer inertia', reasoning-trained models show stronger corrective behavior but can still be influenced by textual cues, and CoT provides only a partial view of how different modalities drive VLM decisions.
...
Method: The paper uses six modern multilingual encoders (potion, gemma, bge, snow, jina, e5) and benchmarks them on LtHate, RuToxic, and EnSuperset datasets using a unified Python pipeline. For each embedding, a one-class HBOS anomaly detector and a two-class CatBoost classifier are trained, with and without PCA compression to 64-dimensional feature vectors.

Result: Two-class supervised models consistently outperform one-class anomaly detection, with the best configurations achieving up to 80.96% accuracy and AUC ROC of 0.887 in Lithuanian, 92.19% accuracy and AUC ROC of 0.978 in Russian, and 77.21% accuracy and AUC ROC of 0.859 in English.

Conclusion: Modern multilingual sentence embeddings combined with gradient boosted decision trees provide robust soft-computing solutions for multilingual hate speech detection applications.
...
TL;DR: The paper proposes a framework called IE-as-Cache, which repurposes Information Extraction (IE) as a cognitive cache to enhance reasoning in large language models (LLMs).


<details>
  <summary>Details</summary>
Motivation: The traditional approach to Information Extraction treats it as a terminal objective, where the extracted information is not reused or maintained during multi-step inference. The authors aim to move beyond this limitation.

Method: The proposed framework, IE-as-Cache, combines query-driven extraction with cache-aware reasoning to dynamically maintain compact intermediate information and filter noise.

Result: Experiments on challenging benchmarks across diverse LLMs demonstrate significant improvements in reasoning accuracy.

Conclusion: The paper concludes that IE can be effectively repurposed as a reusable cognitive resource, offering a promising direction for future research on downstream uses of IE.
...
TL;DR: The authors propose XQ-MEval, a dataset to evaluate translation metrics across languages, revealing cross-lingual scoring bias and proposing a normalization strategy.


<details>
  <summary>Details</summary>
Motivation: The common practice of averaging metric scores across languages in multilingual translation systems may suffer from cross-lingual scoring bias.

Method: The authors semi-automatically build XQ-MEval by injecting errors into gold translations, filtering by native speakers, and merging errors to generate pseudo-translations.

Result: Experiments on nine representative metrics reveal inconsistency between averaging and human judgment, and provide empirical evidence of cross-lingual scoring bias.

Conclusion: The proposed normalization strategy derived from XQ-MEval improves the fairness and reliability of multilingual metric evaluation.
...
TL;DR: The paper presents a system that generates scientific architecture diagrams from text using language models, and contributes a comprehensive dataset and models that outperform existing baselines.


<details>
  <summary>Details</summary>
Motivation: Current methods of communicating complex system designs or scientific processes through text are inefficient and prone to ambiguity, and there is a need for a system that can automatically generate high-fidelity architecture diagrams from text.

Method: The authors leverage language models to perform semantic understanding of input text descriptions to generate intermediate code that can be processed to create architecture diagrams. They fine-tune small language models and use in-context learning with GPT-4o.
...
Conclusion: The paper proposes a system for generating scientific architecture diagrams from text, and provides a comprehensive dataset and models that can be used for this task, making the code, data, and models publicly available.
...
TL;DR: The paper presents a hybrid approach combining Large Language Models (LLMs) with curated vocabularies to detect and explain hate speech in multiple languages, providing transparent and accountable results.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the limitations of automated hate speech detection systems, which often focus on censorship or removal, raising concerns about transparency and freedom of expression.

Method: The method involves a hybrid approach that combines LLMs with three newly created and curated vocabularies to detect and explain hate speech in English, French, and Greek, using two complementary pipelines.

Result: The result shows that the hybrid approach is accurate, with high-quality explanations, outperforming LLM-only baselines, as demonstrated by human evaluation.

Conclusion: The conclusion is that the proposed hybrid approach provides a promising solution for hate speech detection, making it more transparent, accountable, and informative, while addressing concerns about freedom of expression.
...
TL;DR: The paper introduces Interrogative Uncertainty Quantification (IUQ), a framework for quantifying uncertainty in long-form text generation by Large Language Models (LLMs).
...
Method: The IUQ framework leverages inter-sample consistency and intra-sample faithfulness through an interrogate-then-respond paradigm to provide reliable measures of claim-level uncertainty and model faithfulness.

Result: Experimental results show the superior performance of IUQ over two widely used long-form generation datasets across diverse model families and sizes.

Conclusion: The IUQ framework effectively addresses the challenge of uncertainty quantification in long-form LLM outputs, providing a reliable measure of uncertainty and faithfulness.
...
TL;DR: A retrieval-grounded LLM-based conversational agent (CA) was developed to support patient understanding of CGM data and preparation for diabetes consultations. The CA generated plain-language responses and was evaluated by clinicians, who rated it higher than clinician-authored responses in terms of quality, empathy, and actionability.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this study was to evaluate the potential of a retrieval-grounded LLM-based CA to support patient understanding of CGM data and preparation for routine diabetes consultations, as explaining CGM patterns clearly and empathetically remains time-intensive.

Method: The study involved developing a retrieval-grounded LLM-based CA for CGM interpretation and diabetes counseling support. 12 CGM-informed cases were constructed, and 6 senior UK diabetes clinicians reviewed 2 assigned cases each, answering 24 questions. The CA-generated and clinician-authored responses were independently rated by 3 clinicians on 6 quality dimensions.

Result: The CA received higher quality scores than clinician responses (mean 4.37 vs 3.58), with significant differences in empathy (1.062) and actionability (0.992). Safety flag distributions were similar, with major concerns rare in both groups (0.7% each).
...
TL;DR: DiscoTrace method identifies rhetorical strategies in answers to info-seeking questions, revealing community preferences and LLM limitations.


<details>
  <summary>Details</summary>
Motivation: Develop a method to understand rhetorical strategies in answers and compare human and LLM approaches.

Method: DiscoTrace represents answers as sequences of discourse acts and interpretations, using rhetorical structure theory parses.

Result: Human communities show diverse preferences in answer construction, while LLMs lack rhetorical diversity and favor breadth.

Conclusion: Findings can guide development of pragmatic LLM answerers that consider context and diverse strategies in QA.
...
TL;DR: The paper presents QuantCode-Bench, a benchmark for evaluating large language models (LLMs) in generating executable algorithmic trading strategies from textual descriptions.


<details>
  <summary>Details</summary>
Motivation: The ability of LLMs to generate executable algorithmic trading strategies remains underexplored, and existing benchmarks are not sufficient to evaluate this specific task.

Method: The authors propose a multi-stage pipeline to evaluate LLMs, including checks for syntactic correctness, successful backtest execution, presence of trades, and semantic alignment with task descriptions.

Result: State-of-the-art models show limitations in generating trading strategies, primarily due to difficulties in operationalizing trading logic, using APIs correctly, and adhering to task semantics.

Conclusion: Trading strategy generation is a distinct class of domain-specific code generation tasks that requires not only technical correctness but also alignment between natural-language descriptions, financial logic, and observable strategy behavior on data.
...
TL;DR: The paper proposes K-Token Merging, a framework that compresses input sequences by merging token embeddings in latent space, achieving up to 75% input length reduction with minimal performance degradation.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of processing long prompts in Large Language Models (LLMs), which incur significant computational and memory costs due to the quadratic scaling of full self-attention with input length.

Method: The method proposed is K-Token Merging, a latent-space compression framework that merges each contiguous block of K token embeddings into a single embedding via a lightweight encoder.

Result: The results show that K-Token Merging achieves up to 75% input length reduction with minimal performance degradation on structural reasoning, sentiment classification, and code editing tasks.

Conclusion: The conclusion is that K-Token Merging lies on the Pareto frontier of performance vs. compression, making it an effective approach for compressing input sequences in LLMs.
...
Method: The authors detail different strategies explored for handling overgenerations in a commercial setting.

Result: The paper presents the results of the authors' work in handling overgenerations in LLMs for machine translation.

Conclusion: The paper concludes that detecting and determining the exact nature of overgenerations in LLMs is a challenging task, but one that is important for effective machine translation.
...
TL;DR: The authors introduce MADE, a new multi-label text classification benchmark for medical device adverse event reports, and evaluate various machine learning models and uncertainty quantification methods.


<details>
  <summary>Details</summary>
Motivation: The need for reliable uncertainty quantification in high-stakes domains like healthcare, particularly in multi-label text classification tasks, which are challenging due to label imbalances and complexities.

Method: The authors propose MADE, a living benchmark derived from medical device adverse event reports, and evaluate over 20 encoder- and decoder-only models under fine-tuning and few-shot settings, as well as various uncertainty quantification methods.

Result: The results show trade-offs between model performance and uncertainty quantification, with smaller fine-tuned decoders achieving strong accuracy and competitive uncertainty quantification, while generative fine-tuning delivers the most reliable uncertainty quantification.

Conclusion: The authors conclude that their work provides a new benchmark and insights into the performance of various machine learning models and uncertainty quantification methods in a high-stakes domain like healthcare.
...
<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the limitations of speculative decoding (SD) in large language models, specifically the propagation of erroneous steps and the overhead of external reward models.

Method: The authors propose SpecGuard, which performs step-level verification using only model-internal signals, sampling multiple draft candidates, and validating them using an ensemble of two lightweight model-internal signals.

Result: Experiments show that SpecGuard improves accuracy by 3.6% and reduces latency by ~11%, outperforming both SD and reward-guided SD across various reasoning benchmarks.

Conclusion: The proposed SpecGuard framework offers a more efficient and accurate approach to speculative decoding in large language models, eliminating the need for external reward models.
...
TL;DR: Proposes NuHF Claw, a framework for risk-governed autonomy in nuclear operations, integrating cognitive state inference and safety assessment to ensure safe and efficient human-agent collaboration.


<details>
  <summary>Details</summary>
Motivation: The digitization of nuclear power plant control rooms introduces complex operator interactions and cognitive risks not addressed by existing human reliability analysis approaches.

Method: Introduces a risk-constrained agent runtime that couples cognitive state inference with probabilistic safety assessment to regulate autonomous system behavior in real-time.

Result: Experimental validation shows NuHF Claw can anticipate cognitive degradation, constrain unsafe autonomous recommendations, and provide risk-aware guidance while preserving human decision authority.
...
<details>
  <summary>Details</summary>
Motivation: Existing frameworks for large language model agents rely on fixed pipelines or failure-triggered reflection, which limit their adaptability and efficiency. The authors aim to create a more proactive and adaptive system.

Method: The authors propose a periodic 'heartbeat' mechanism to orchestrate a dynamic repertoire of cognitive modules, and a meta-learning strategy for continual policy adaptation. This allows the scheduler to learn when to engage specific thinking activities based on temporal patterns and historical context.
...
<details>
  <summary>Details</summary>
Motivation: The current benchmark datasets for anomaly detection in multivariate time series have limitations, such as lack of fine-grained anomaly annotations, unclear variable and temporal dependencies, and unknown generative mechanisms.
...
Conclusion: The proposed Fun-TSG tool can support the development and rigorous comparison of anomaly detection models, especially those targeting interpretable and variable-specific outputs.
...
TL;DR: The paper discusses the integration of Explainable Artificial Intelligence (XAI) techniques with surrogate modeling to make complex system simulations more interpretable.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the issue of opaque computational simulators and surrogate models in scientific and engineering domains, and to enable the extraction of actionable insights from complex system behaviors.

Method: The paper provides a structured survey of existing XAI techniques and maps them onto various stages of surrogate modeling workflows, highlighting their strengths and applications.

Result: The result is a comprehensive survey that identifies pressing open challenges and proposes a research agenda to integrate explainability into simulation-driven workflows.

Conclusion: The conclusion is that by combining surrogate modeling and XAI, practitioners can transform opaque emulators into explainable tools, enabling the extraction of actionable insights from complex system behaviors.
...
TL;DR: This paper proposes a formal approach to machine ethics using Kantian ethics, specifically the Formula of the Universal Law Logic (FULL), to evaluate agents' actions without relying on built-in moral intuition.


<details>
  <summary>Details</summary>
Motivation: The field of machine ethics faces limitations in current approaches, which encode human moral intuition as axioms on actions, neglecting the agent's purposes and assuming humans can fully enumerate their moral intuition.

Method: The paper presents a multi-sorted quantified modal logic called the Formula of the Universal Law Logic (FULL), which formalizes Kant's first formulation of the categorical imperative and concepts such as causality and agency.

Result: The FULL demonstrates the ability to reason and evaluate agents' actions for certain purposes without built-in moral intuition, given sufficient non-normative background knowledge, through three case studies from Kantian ethics.
...
TL;DR: The paper proposes Group Fine-Tuning (GFT), a unified post-training framework that improves upon supervised fine-tuning (SFT) for large language models by addressing issues of reward sparsity and unstable optimization.


<details>
  <summary>Details</summary>
Motivation: The authors are motivated by the challenge of effectively unifying efficient knowledge injection with robust generalization in large language models post-trained using SFT and reinforcement learning (RL).

Method: The authors propose Group Fine-Tuning (GFT), which includes two mechanisms: Group Advantage Learning and Dynamic Coefficient Rectification, to address the limitations of SFT.
...
Conclusion: The paper concludes that GFT provides a more effective post-training framework for large language models, improving upon traditional SFT methods.
...
TL;DR: GazeX, a vision language model, incorporates radiologists' eye tracking data to improve chest X-ray interpretation, producing more accurate and expert-consistent outputs.


<details>
  <summary>Details</summary>
Motivation: Current vision language models for chest X-ray interpretation have limited clinical utility due to a gap between model outputs and radiologist reasoning, and often overlook critical findings or diverge from established diagnostic workflows.
...
Result: GazeX produces more accurate, interpretable, and expert-consistent outputs across radiology report generation, disease grounding, and visual question answering, using a large dataset of radiographic studies, question-answer pairs, and image-sentence pairs.
...
TL;DR: The paper proposes 'memorized mistake-gated learning', a biologically plausible plasticity rule that reduces synaptic updates by 50-80% by only updating network parameters when classification errors occur.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the issue of energy efficiency in synaptic plasticity, inspired by the human negativity bias and error-related negativity.

Method: The authors propose a new learning rule called 'memorized mistake-gated learning', which gates synaptic updates by current and past classification errors.

Result: The proposed algorithm reduces the number of updates the network needs to make by 50-80%, and is particularly well suited for incremental learning and online learning scenarios.

Conclusion: The paper concludes that 'memorized mistake-gated learning' is an energy efficient and biologically relevant modification to commonly used learning rules, well suited for continual learning.
...
TL;DR: This paper introduces Credo, a framework for building adaptive AI systems that uses declarative policies and a database-backed control plane to regulate behavior and support auditable and composable execution.


<details>
  <summary>Details</summary>
Motivation: Existing frameworks for building AI systems rely on imperative control loops, ephemeral memory, and prompt-embedded logic, making agent behavior opaque, brittle, and difficult to verify.

Method: The authors propose a novel framework called Credo, which represents semantic state as beliefs and regulates behavior using declarative policies defined over these beliefs.

Result: The proposed framework enables dynamic behavior without requiring changes to the underlying pipeline code, and supports adaptive, auditable, and composable execution.

Conclusion: The Credo framework offers a promising approach to building more transparent, adaptable, and reliable AI systems.
...
TL;DR: The paper investigates whether routing topology determines language modeling quality in Sparse Mixture-of-Experts (MoE) architectures. The authors find that routing topology does not significantly impact asymptotic perplexity (PPL) and that different routing mechanisms can achieve similar performance.


<details>
  <summary>Details</summary>
Motivation: The authors aim to understand the importance of routing topology in MoE architectures and whether it affects language modeling quality.
...
Result: The authors find that five cosine-routing variants are statistically equivalent within a 1-PPL margin, and that routing topology does not determine asymptotic perplexity (PPL). They also find that a standard linear router with more routing parameters can be matched by a cosine-routing variant with fewer parameters.

Conclusion: The authors conclude that routing topology does not significantly impact language modeling quality, and that the true mechanism advantage is small (~1.2%). They also propose a practical payoff of zero-shot relative-norm halting to save MoE FLOPs.
...
TL;DR: The paper presents Pneuma-Seeker, a system that helps data analysts refine their questions and explore relational data iteratively, using LLMs as interactive analytical collaborators.
...
Method: The authors demonstrate Pneuma-Seeker, a system that reifies a user's information need as explicit, inspectable relational specifications, enabling iterative refinement of the information need, targeted data discovery, and provenance-aware execution.

Result: Through two real-world procurement use cases, the authors show how Pneuma-Seeker leverages LLMs as transparent, interactive analytical collaborators.

Conclusion: The paper concludes that Pneuma-Seeker is a useful system for data analysts to refine their questions and explore relational data iteratively.
...
TL;DR: The paper explores the specialization of individual experts in Sparse Mixture-of-Experts (MoE) models, finding that expert identity is causally meaningful and can be made directly inspectable through cosine-similarity routing.
...
Method: The authors use four lines of evidence, including projecting expert output vectors, analyzing routing behavior, causal interventions, and comparing different routing methods.

Result: The study finds that MoE expert-level specialization is architecturally monosemantic, causally validated, and controllable at inference with zero overhead, and that cosine-similarity routing provides geometric transparency.

Conclusion: MoE expert-level specialization is a first-class interpretability primitive, allowing for direct inspection and control of expert behavior.
...
TL;DR: A Reinforcement Learning framework using Signal Temporal Logic and Reward Machines for complex task control.


<details>
  <summary>Details</summary>
Motivation: Handling complex tasks in Reinforcement Learning

Method: RL framework with Reward Machines and Signal Temporal Logic

Result: Efficient representation of rewards and guided training for complex tasks

Conclusion: The proposed framework effectively handles complex tasks with specified requirements
...
TL;DR: AIBuildAI, a hierarchical AI agent system, automates the entire AI model development process from task description to deployable model, outperforming existing methods and human engineers.


<details>
  <summary>Details</summary>
Motivation: The development of high-performing AI models is labor-intensive and requires expert practitioners. Existing AutoML methods have limitations and cannot fully automate the development process.
...
Result: AIBuildAI ranked first on MLE-Bench with a medal rate of 63.1%, outperforming all existing baseline methods and matching the capability of highly experienced AI engineers.

Conclusion: Hierarchical agent systems like AIBuildAI can automate the full AI model development process, suggesting a pathway toward broadly accessible AI development with minimal human intervention.
...
<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the challenge of determining when and how an AI assistant should intervene in human decision-making tasks, particularly in sequential decision-making tasks where humans may not always make optimal decisions.

Method: The method proposed in this paper involves formalizing the problem in a Markov decision process, where an AI assistant may override human actions under an intervention budget. The approach prioritizes interventions based on the magnitude of the policy-value discrepancy and recommends actions that maximize the human value function.

Result: The results show that the proposed approach consistently outperforms interventions based on the strongest chess engine (Stockfish) in a wide range of settings. A within-subject human study with 20 players and 600 games further shows that the interventions significantly improve performance for low- and mid-skill players while matching expert-engine interventions for high-skill players.

Conclusion: The conclusion of this paper is that value-aware interventions can be an effective approach for AI assistants to improve human decision-making in sequential tasks, by identifying opportunities for intervention and recommending actions that maximize the human value function.
...
Method: RUMS uses mutual information to select user memory items that reduce response uncertainty and sharpen predictions, beyond semantic similarity.

Result: RUMS outperforms state-of-the-art methods, resulting in better response quality and up to 95% reduction in computational cost, while aligning with human selection.

Conclusion: The proposed RUMS method provides a more principled approach to user memory selection in large language models, leading to improved response quality and efficiency.
...
Result: Evo-MedAgent improves multiple-choice question (MCQ) accuracy on ChestAgentBench from 0.68 to 0.79 on GPT-5-mini and from 0.76 to 0.87 on Gemini-3 Flash, with a negligible overhead.

Conclusion: The proposed Evo-MedAgent enables medical agents to improve their performance on interpreting chest X-rays by accumulating experience across cases, without requiring training or expensive reinforcement learning.
...
<details>
  <summary>Details</summary>
Motivation: The lack of transparency in neural networks' internal reasoning, specifically in vision transformers, and the need to understand how information is routed through their complex wiring.

Method: The paper proposes an effective method for Automatic Visual Circuit Discovery (Vi-CD) that recovers class-specific circuits for classification and identifies circuits underlying typographic attacks in CLIP.
...
Conclusion: The proposed method, Vi-CD, is effective in identifying useful mechanistic circuits in vision transformers, which can add transparency to their internal computations and potentially be used to correct harmful model behavior.
...
TL;DR: This paper evaluates state-of-the-art ASR architectures for edge devices, identifying NVIDIA's Nemotron Speech Streaming as a top candidate, and achieves a good balance between accuracy and efficiency through optimizations.


<details>
  <summary>Details</summary>
Motivation: Deploying high-quality ASR on edge devices with limited resources (CPU, memory) without GPU acceleration.

Method: Systematic empirical study of ASR architectures, benchmarking over 50 configurations, and optimizing with post-training quantization strategies.

Result: The recommended configuration achieves 8.20% average streaming WER, with 0.56 s algorithmic latency, and reduces the model size from 2.47 GB to 0.67 GB.

Conclusion: Establishes a new quality-efficiency Pareto point for on-device streaming ASR, enabling high-quality ASR on resource-constrained edge devices.
...
TL;DR: Synthetic data augmentation in financial machine learning has a trade-off between bias and variance. It helps in variance-dominant regimes but hurts in bias-dominant settings.


<details>
  <summary>Details</summary>
Motivation: The statistical role of synthetic augmentation in financial machine learning is poorly understood, and there is a need to formalize its effects on the training distribution.

Method: The authors introduce a size-matched null augmentation and a finite-sample, non-parametric block permutation test to isolate informational gains from mechanical sample-size effects.
...
Conclusion: The study provides a structural perspective on when synthetic data improves financial learning performance and when it induces persistent distributional distortion.
...
TL;DR: The paper presents an information-geometric framework for analyzing Mixture-of-Experts (MoE) model specialization dynamics, introducing two principled metrics (FSI and FHS) that outperform existing methods in predicting performance and training failure.


<details>
  <summary>Details</summary>
Motivation: Existing metrics for MoE model specialization lack theoretical grounding and yield inconsistent conclusions, motivating the development of a rigorous framework for characterizing specialization dynamics.

Method: The paper uses Riemannian geometry and Fisher information metric to analyze expert routing distributions on the probability simplex, deriving theoretical results and introducing two new metrics (FSI and FHS).

Result: The framework achieves high correlation with downstream performance (r=0.91+/-0.02) and predicts training failure with high accuracy (AUC=0.89+/-0.03), outperforming existing methods and requiring significantly fewer compute cycles.

Conclusion: The paper provides a theoretically grounded framework for analyzing MoE model specialization dynamics, introducing two principled metrics that can be used to predict performance and training failure, and validating the effectiveness of intervention protocols.
...
TL;DR: Biomedical AI models may perpetuate health disparities due to biases in data collection and research prioritization. An analysis of omics publications and large-scale datasets reveals substantial population bias. The authors propose a community-wide focus on provenance, openness, and evaluation transparency to improve equity and robustness in biomedical AI.


<details>
  <summary>Details</summary>
Motivation: Healthcare disparities persist due to unequal access to screening, diagnostics, and therapeutics. However, biases can emerge earlier during data collection and research prioritization, particularly at the molecular level.
...
Conclusion: The authors propose a community-wide focus on provenance, openness, and evaluation transparency to improve equity and robustness in biomedical AI and foster innovation that serves underserved populations and improves health outcomes.
...
Result: MindDR achieves competitive performance on various benchmarks, including 45.7% on BrowseComp-ZH, 42.8% on BrowseComp, 46.5% on WideSearch, 75.0% on xbench-DS, and 52.5 on DeepResearch Bench, and a state-of-the-art score of 51.8 on MindDR Bench.

Conclusion: MindDR is an efficient and effective multi-agent deep research framework that achieves leading performance with smaller models, and has been deployed as an online product in Li Auto.
...
TL;DR: The paper addresses the issue of inconsistent answers from large language models when handling multiple related queries. It proposes a method to maintain logical consistency across queries, introducing new metrics and a solver-augmented approach that significantly reduces contradictions while preserving accuracy.
...
Method: The paper introduces a benchmark of 390 multi-query reasoning instances and proposes set-level metrics. It also presents a solver-augmented approach that extracts commitments, verifies global satisfiability, and performs counterexample-guided repair.

Result: The method substantially reduces cross-query contradictions (SetCons: 0.56 to 0.94) across four reasoning domains while preserving per-query accuracy.

Conclusion: The study demonstrates that global coherence is critical for robust multi-query reasoning, and the proposed method is effective in achieving this coherence.
...
TL;DR: The paper analyzes reasoning failures in Large Language Models (LLMs) and proposes a targeted framework called GUARD to improve their reliability.
...
Method: Analyzing model-generated reasoning trajectories to identify early transition points where errors originate, and introducing GUARD, a framework that probes and redirects critical transitions using uncertainty signals.

Result: Empirical evaluations across multiple benchmarks confirm that interventions guided by failure dynamics lead to more reliable reasoning outcomes.
...
TL;DR: Introduces TRACER, a system that trains ML surrogates on LLM production traces, automating deployment based on a parity gate, achieving significant surrogate coverage.


<details>
  <summary>Details</summary>
Motivation: The motivation is to utilize the growing dataset of labeled input-output pairs from LLM classification endpoints to train lightweight surrogates, reducing inference costs.

Method: TRACER trains ML surrogates on production traces and governs deployment through a parity gate, ensuring the surrogate's accuracy exceeds a user-specified threshold.

Result: TRACER achieves 83-100% surrogate coverage on a 77-class intent benchmark and fully replaces the teacher on a 150-class benchmark.

Conclusion: TRACER effectively automates the deployment of ML surrogates for LLMs, providing a cost-efficient and transparent solution.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of traditional RL paradigms, which suffer from diminishing returns due to limited trajectory diversity, and to improve the achievable performance ceiling.

Method: The method proposed is MARS$^2$, which models the search tree as a learnable multi-agent interaction environment, enabling heterogeneous agents to collaboratively generate and refine candidate solutions within a shared search topology.

Result: The results show that MARS$^2$ consistently improves performance across diverse model combinations and training settings on code generation benchmarks.

Conclusion: The conclusion is that MARS$^2$ is effective in enhancing reinforcement learning by coupling multi-agent collaboration with tree search, and it is publicly available as open-source code.
...
TL;DR: The study aims to improve large language models' (LLMs) counseling quality by incorporating clinically validated knowledge, comparing retrieval-augmented generation (RAG) and knowledge graph (KG)-based methods.


<details>
  <summary>Details</summary>
Motivation: LLMs' responses in mental health and counseling applications often lack cultural sensitivity, contextual grounding, and clinically appropriate guidance.

Method: The study compares two approaches: RAG and KG-based method, using a manually constructed and clinically validated KG, and evaluates multiple LLMs using BERTScore F1, SBERT cosine similarity, and human evaluation.
...
TL;DR: The optimization of prompts in AI systems often yields statistically insignificant results, comparable to a coin flip, but can be improved with a diagnostic approach.


<details>
  <summary>Details</summary>
Motivation: The authors investigate the effectiveness of prompt optimization in compound AI systems, specifically addressing the assumptions behind end-to-end optimization tools.

Method: The study conducts 72 optimization runs on Claude Haiku and Amazon Nova Lite, testing two assumptions with 18,000 grid evaluations and 144 optimization runs, and proposes a two-stage diagnostic.

Result: The results show that optimization helps only when the task has exploitable output structure, and interaction effects between agent prompts are not significant.

Conclusion: The authors provide a two-stage diagnostic to predict whether optimization is worthwhile, turning a coin flip into an informed decision, and conclude that prompt optimization effectiveness depends on task characteristics.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the process of legal formalization, specifically for GDPR provisions, by leveraging large language models while ensuring accuracy and correctness through human verification.

Method: The method involves a role-specialized workflow with LLM-based AI components generating legal scenarios, formal rules, and atomic facts, coupled with independent verification modules including human reviewers for correctness assessment.

Result: The results indicate that structured verification and targeted human oversight are crucial for reliable legal formalization, especially when dealing with legal nuances and context-sensitive reasoning.

Conclusion: The conclusion is that while large language models can aid in the formalization of legal provisions, human-in-the-loop verification is essential for ensuring the accuracy and reliability of the formalized outputs.
...
TL;DR: The paper introduces El Agente Forjador, a multi-agent framework that enables universal coding agents to autonomously generate, validate, and reuse computational tools for scientific tasks, improving accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: The current generation of agentic systems depends on static, hand-curated toolsets that hinder adaptation to new domains and evolving libraries, prompting the need for a more flexible and autonomous approach.

Method: El Agente Forjador uses a four-stage workflow: tool analysis, tool generation, task execution, and iterative solution evaluation, allowing coding agents to autonomously forge, validate, and reuse computational tools.
...
Conclusion: The paper suggests a paradigm shift in which agent capabilities are defined by the tasks they are designed to solve rather than by explicitly engineered implementations, enabling LLM-based agents to autonomously build reusable scientific tools.
...
Method: CoDaS, a multi-agent system that combines hypothesis generation, statistical analysis, adversarial validation, and literature-grounded reasoning with human oversight.

Result: CoDaS identified 41 candidate digital biomarkers for mental health and 25 for metabolic outcomes, and demonstrated improved predictive performance when incorporating CoDaS-derived features.

Conclusion: CoDaS enables systematic and traceable hypothesis generation and prioritization for biomarker discovery from large-scale wearable data, with potential applications in digital health.
...
<details>
  <summary>Details</summary>
Motivation: The exact cover problem is NP-hard with broad AI applications, and current methods like Algorithm DXZ using ZBDDs have limitations.

Method: A zero-suppressed variant of decision decomposable negation normal form (decision-ZDNNF) is proposed, and a novel parallel algorithm DXD is designed.

Result: The improved DXD algorithm outperforms all state-of-the-art methods in experimental results.

Conclusion: The proposed decision-ZDNNF and improved DXD algorithm provide a more efficient solution for the exact cover problem.
...
<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to explore whether LLMs can develop genuine spatial understanding by learning to construct explicit visual layouts from spatial descriptions, similar to how humans use sketching to organize their thinking and sharpen their understanding of complex spatial problems.

Method: The method involves introducing a new dataset called Text2Space, which pairs natural language descriptions with ground-truth ASCII grid layouts and spatial QA pairs. The study evaluates LLMs on this dataset and investigates the effect of training models on layout construction (Text to ASCII) on their spatial reasoning abilities.

Result: The results show a pronounced 'Read-Write Asymmetry' in LLMs, where they can interpret ASCII representations effectively but struggle to produce them from text. Training models on layout construction significantly improves their spatial reasoning from text alone, and combining construction with comprehension training further amplifies these gains. These improvements transfer to three external spatial reasoning benchmarks.
...
TL;DR: A new framework called UEC-RL is proposed to improve reinforcement learning for large language and vision-language models by enhancing exploration and stability.


<details>
  <summary>Details</summary>
Motivation: The Group Relative Policy Optimization (GRPO) method suffers from entropy collapse, leading to premature convergence and loss of diversity in policies.

Method: UEC-RL, a framework that provides targeted mechanisms for exploration and stabilization through unified entropy control.

Result: UEC-RL achieves consistent gains over RL baselines on both Pass@1 and Pass@$k$ for LLM and VLM reasoning tasks, with a 37.9% relative improvement over GRPO on Geometry3K.

Conclusion: UEC-RL enables effective exploration without compromising convergence, making it a key for scaling RL-based reasoning in large models.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the efficiency and effectiveness of autonomous code generation systems by optimizing the initial conditions of the code generation process.

Method: The method involves using a genetic algorithm to search over reusable starting conditions, called agent seeds, which include task prompts and optional parent archives. The approach also uses a modified Hedge controller to adapt operator allocation online.

Result: The results show that AgentGA outperforms AIDE on the Weco-Kaggle Lite benchmark, achieving an average of 74.52% Exceeds % of Human versus 54.15% for AIDE. Additionally, the results indicate that inherited artifacts from parent archives improve the performance of later autonomous runs.

Conclusion: The conclusion is that agent-seed optimization is a practical design point for autonomous code-search systems, and that inherited artifacts can improve the performance of later autonomous runs.
...
TL;DR: MedImageEdu is a new benchmark for multi-turn radiology patient education that assesses multimodal agents' ability to provide evidence-based explanations.


<details>
  <summary>Details</summary>
Motivation: Current patient education systems are limited to text-only explanations, and existing multimodal benchmarks focus on static tasks, not interactive patient education.

Method: MedImageEdu uses a DoctorAgent and PatientAgent to simulate interactions, with the DoctorAgent generating multimodal responses (images and text) based on radiology reports and case images.

Result: The benchmark evaluation reveals three gaps in current multimodal agents: fluent language often outpaces faithful visual grounding, safety is a weak point, and emotionally tense interactions are challenging.
...
<details>
  <summary>Details</summary>
Motivation: The growing body of work on speculative methods lacks exploration of how cognitive characteristics of a task affect acceptance probability.

Method: An empirical study using TinyLlama-1.1B as the draft model and Llama-2-7B-Chat-GPTQ as the target model across four NLP benchmark domains.

Result: Task type is a stronger predictor of acceptance than tree depth, and only the chat domain consistently yields an expected accepted length exceeding 1.0 token per step.

Conclusion: The findings have direct implications for domain-aware speculation budgets and draft-model selection strategies.
...
TL;DR: The paper proposes DR$^{3}$-Eval, a benchmark for evaluating deep research agents on multimodal report generation, and introduces a multi-dimensional evaluation framework.
...
Method: The authors propose DR$^{3}$-Eval, a benchmark constructed from authentic user-provided materials, and a multi-dimensional evaluation framework measuring Information Recall, Factual Accuracy, Citation Coverage, Instruction Following, and Depth Quality.
...
Conclusion: The paper concludes that DR$^{3}$-Eval is a realistic and reproducible benchmark for evaluating deep research agents, and highlights the need for improvement in retrieval robustness and hallucination control.
...
<details>
  <summary>Details</summary>
Motivation: Understanding the behavior of MCTS agents is challenging due to complex search trees and intricate relationships, and standard MCTS has limitations such as reliance on selective tree construction.

Method: Incorporating shallow, full-width Minimax search into MCTS and using process mining techniques (Alpha Miner, iDHM, Inductive Miner) with LLMs to generate explanations.

Result: The approach is demonstrated to be effective in a small-scale checkers environment, providing a foundation for interpreting hybrid agents in complex strategic domains.

Conclusion: The paper proposes a solution to improve MCTS agents' strategic depth and provides a framework for generating human-readable explanations of their decision-making processes.
...
TL;DR: CAMO is a framework for discovering causal relationships in LLM agent simulations, helping to understand social emergence.


<details>
  <summary>Details</summary>
Motivation: The micro-to-macro causal mechanisms behind macro outcomes in LLM agent simulations are often unclear, making it difficult to study social emergence.
...
Result: Experiments across four emergent settings demonstrate the promise of CAMO in discovering causal relationships.

Conclusion: CAMO provides a framework for understanding social emergence in LLM agent simulations by identifying causal relationships and providing actionable insights.
...
<details>
  <summary>Details</summary>
Motivation: The limited access to authentic large-scale HAT datasets due to growing privacy concerns and the need for realistic and privacy-preserving HATs for applications such as human mobility modeling and POI recommendation.

Method: A coarse-to-fine HAT synthesis framework, SynHAT, built on a novel spatio-temporal denoising diffusion model, which consists of two stages: Coarse-HADiff for modeling overall spatio-temporal dependencies and a three-step pipeline for generating fine-grained latent spatio-temporal traces.
...
Conclusion: SynHAT provides a computationally efficient and effective solution for synthesizing realistic and privacy-preserving HATs, addressing the challenges of capturing complex spatio-temporal dependencies and underlying distributions.
...
<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for hardware design primarily evaluate Large Language Models (LLMs) on isolated, component-level tasks, leaving repository-scale evaluation unaddressed.

Method: HWE-Bench comprises 417 task instances derived from real historical bug-fix pull requests across six major open-source projects, and evaluated seven LLMs with four agent frameworks.
...
Conclusion: The failure analysis traces agent failures to three stages of the debugging process: fault localization, hardware-semantic reasoning, and cross-artifact coordination, providing concrete directions for developing more capable hardware-aware agents.
...
TL;DR: The paper introduces SGA-MCTS, a framework for LLM-powered systems that enables efficient and generalizable planning by casting it as non-parametric retrieval, achieving state-of-the-art performance without task-specific fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Current planning approaches for LLM-powered systems face a trade-off between high latency of inference-time search and limited generalization of supervised fine-tuning, which the authors aim to address.

Method: The authors propose SGA-MCTS, which uses Monte Carlo Tree Search (MCTS) to distill high-fidelity trajectories into State-Goal-Action (SGA) atoms offline, and a retrieval-augmented agent with a hybrid symbolic-semantic mechanism to fetch and re-ground relevant SGAs online.

Result: Empirical results on complex benchmarks demonstrate that SGA-MCTS enables frozen, open-weights models to match the performance of SOTA systems (e.g., GPT-5) without task-specific fine-tuning, achieving System 2 reasoning depth at System 1 inference speeds.

Conclusion: The proposed SGA-MCTS framework effectively amortizes the heavy computational cost of search, rendering autonomous planning both scalable and real-time feasible.
...
TL;DR: The paper introduces a framework called layered mutability to analyze the behavior of persistent language-model agents. It argues that the difficulty in governing these agents arises from rapid mutation, strong downstream coupling, weak reversibility, and low observability, leading to a mismatch between the layers affecting behavior and those humans can inspect.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the challenges in governing persistent language-model agents that combine tool use, tiered memory, reflective prompting, and runtime adaptation. These agents' behavior is influenced not only by current prompts but also by mutable internal conditions, making it difficult to understand and control their behavior over time.

Method: The paper proposes a framework called layered mutability, which consists of five layers: pretraining, post-training alignment, self-narrative, memory, and weight-level adaptation. It formalizes the intuition that governance difficulty rises when mutation is rapid, downstream coupling is strong, reversibility is weak, and observability is low. The framework is connected to recent work on temporal identity in language-model agents.

Result: The paper reports a preliminary experiment, 'ratchet experiment,' where reverting an agent's visible self-description after memory accumulation fails to restore baseline behavior. The estimated identity hysteresis ratio is 0.68. The main implication is that the salient failure mode for persistent self-modifying agents is not abrupt misalignment but compositional drift: locally reasonable updates that accumulate into a behavioral trajectory that was never explicitly authorized.

Conclusion: The paper concludes that the framework of layered mutability provides a systematic approach to understanding the challenges in governing persistent language-model agents. It highlights the importance of considering the mismatch between the layers that most affect behavior and those humans can most easily inspect, and it identifies compositional drift as a key failure mode for these agents.
...
TL;DR: The AI revolution, particularly large language models, will fundamentally change how complex information and human know-how are shared, and AI for Science may transform research efficiency and scientific collaboration.


<details>
  <summary>Details</summary>
Motivation: The significance of the AI revolution lies in its potential to change how complex information and human know-how are carried, replicated, and shared, rather than just automation.
...
Conclusion: The integration of AI in scientific research may transform not only the efficiency of research but also the structure of scientific collaboration, discovery, publishing, and evaluation.
...
TL;DR: A framework is proposed to predict the effects of stress-reducing interventions on physiological indicators like heart rate and variability, using a Transformer model and wearable sensor data.


<details>
  <summary>Details</summary>
Motivation: The challenge of turning continuous physiological data from consumer wearables into personalized stress-management recommendations.

Method: A framework combining a Transformer model for predicting post-intervention trajectories and direction of change for physiological indicators, and an empirical study using wearable sensor data.

Result: The proof of concept shows that personalized post-intervention prediction is feasible.

Conclusion: The proposed framework could be integrated into stress-management tools for personalized intervention recommendations, following further validation and regulatory review.
...
TL;DR: Proposed SDM-SCR framework for robust Graph Contrastive Learning on Text-Attributed Graphs, leveraging Large Language Models for semantic-aware signal and noise disentanglement.
...
Method: SDM-SCR framework, consisting of Semantic Decoupling Module (SDM) and Semantic Consistency Regularization (SCR), for disentangling and refining signals.

Result: SDM-SCR achieves State-Of-The-Art performance in accuracy and efficiency through extensive experiments.

Conclusion: The proposed SDM-SCR framework provides a robust approach for Graph Contrastive Learning on Text-Attributed Graphs, effectively disentangling signals from noise.
...
TL;DR: The paper proposes CoTEvol, a genetic evolutionary framework for generating Chain-of-Thought (CoT) data to improve Large Language Models' (LLMs) mathematical reasoning abilities.
...
Method: CoTEvol uses a genetic evolutionary framework to search for optimal CoT trajectories. It iteratively evolves candidate trajectories through reflective global crossover and local mutation guided by uncertainty.
...
Method: The authors propose a simulation-based benchmark, MirrorBench, inspired by the Mirror Self-Recognition (MSR) test in psychology, to assess embodied MLLMs' self-referential understanding through a tiered framework of tasks.
...
<details>
  <summary>Details</summary>
Motivation: Existing educational agents rely on static personas, neglecting the role of deep cognitive capabilities in learning outcomes, and struggle to characterize the dynamic fluidity of knowledge internalization and cognitive state transitions.
...
Conclusion: CogEvolution provides a novel paradigm for constructing highly interpretable educational agents, addressing the limitations of existing agents and offering a more accurate simulation of student cognitive evolution.
...
<details>
  <summary>Details</summary>
Motivation: The development of MR sequences is challenging and often relies on human intuition. Existing AI-driven approaches require initial sequences or extensive training datasets, limiting their applicability.

Method: The 'Sequence Search' framework uses neural architecture search to generate candidate sequences, which are then optimized via a differentiable Bloch simulator and objective-specific loss functions.
...
Method: A unified comparison of static compression techniques (pruning and quantization) and dynamic early-exit mechanisms, evaluated on real edge devices using ONNX-based inference pipelines.

Result: The results show that static and dynamic techniques offer different trade-offs, and that combining them can reduce inference latency and memory usage with minimal accuracy loss.

Conclusion: The combination of static compression and dynamic early-exit mechanisms is highly effective for deploying deep neural networks on edge devices, allowing for better trade-offs between accuracy, latency, and memory usage.
...
TL;DR: The paper introduces the concept of 'LLM fallacy', a cognitive error where individuals overestimate their abilities due to reliance on large language models (LLMs), and explores its implications on self-perception, education, and AI literacy.
...
TL;DR: Current evaluation methods for large language models in clinical documentation tasks are flawed, misclassifying valid clinical outputs as errors, and a new clinically informed evaluation approach is needed.


<details>
  <summary>Details</summary>
Motivation: The challenge of evaluating large language models for clinical documentation tasks such as SOAP note generation, which requires clinical abstraction, normalization of colloquial language, and medically grounded inference.

Method: Analysis of current evaluation methods and proposal of a new evaluation approach through calibrated prompting and retrieval grounded in medical ontologies.

Result: The current lexical evaluation regime shows a mean hallucination rate of 35%, whereas the inference-aware evaluation approach reduces this rate to 9%, revealing that current methods over-penalize valid clinical reasoning.
...
TL;DR: The paper challenges the common assumption that final layer embeddings are optimal for single-cell foundation models, finding that optimal layers vary by task and context.


<details>
  <summary>Details</summary>
Motivation: The authors aim to evaluate the assumption that final layer embeddings are optimal for single-cell foundation models and to investigate the importance of layer-wise representations.

Method: The authors systematically evaluate layer-wise representations from two models, scFoundation (100M parameters) and Tahoe-X1 (1.3B parameters), across two tasks: trajectory inference and perturbation response prediction.

Result: The analysis reveals that optimal layers are task-dependent and context-dependent, with first-layer embeddings outperforming deeper layers in quiescent cells.

Conclusion: The findings suggest that the choice of layer for feature extraction is crucial and should be tailored to the specific biological task and cellular context, rather than relying on default final-layer embeddings.
...
TL;DR: The paper proposes TrigReason, a framework that combines Large Reasoning Models (LRMs) and Small Reasoning Models (SRMs) to accelerate inference while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: LRMs achieve strong performance but suffer from high inference latency due to autoregressive reasoning. Recent work explores using SRMs to accelerate LRM inference, but SRMs have capability boundaries and reasoning risks.

Method: The authors propose TrigReason, a trigger-based collaborative reasoning framework that selectively intervenes with LRM when necessary, while delegating most reasoning to SRM.

Result: TrigReason matches the accuracy of full LRMs and SpecReason, while offloading 1.70x - 4.79x more reasoning steps to SRMs, reducing latency by 43.9% and API cost by 73.3%.

Conclusion: The proposed TrigReason framework effectively addresses the capability boundaries and reasoning risks of SRMs, achieving a balance between accuracy and efficiency.
...
<details>
  <summary>Details</summary>
Motivation: The need for evolving benchmarks to evaluate and diagnose trajectory-level safety in diverse agent system execution settings.

Method: Customizing a three-dimensional Safety Taxonomy for each new setting and using it to define benchmark specifications for the ATBench construction pipeline.

Result: Two domain-customized extensions of ATBench, ATBench-Claw and ATBench-CodeX, for OpenClaw and OpenAI Codex settings, respectively.

Conclusion: The proposed extensions demonstrate the extensibility of ATBench, enabling safety evaluation and diagnosis in diverse agent system settings.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to address the critical limitation of LLMs producing fluent outputs even when their internal reasoning has drifted, making them unreliable partners in high-stakes contexts.

Method: The paper proposes a two-layer approach: (1) human-side mechanisms such as uncertainty cues, conflict surfacing, and auditable reasoning traces, and (2) a model-side Epistemic Control Loop (ECL) that detects instability and modulates generation accordingly.
...
Conclusion: The central claim is that fluency is not reliability, and without structures that stabilize both human and model reasoning, AI cannot be trusted or governed where it matters most.
...
TL;DR: A new framework, CoCoGen+, is proposed to address the challenges of coopetition in cross-silo federated learning, enabling organizations to collaboratively train AI models without sharing raw data while competing in downstream markets.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the dilemma organizations face in cross-silo federated learning, where they cooperate during model training but compete in downstream markets, and existing approaches fail to account for the costs of strengthening competitors.

Method: The authors introduce a coopetition-compatible data generation and incentivization framework, CoCoGen+, which jointly models non-IID data and inter-organizational competition, and endogenizes GenAI-based synthetic data generation as a strategic decision.
...
Conclusion: The authors conclude that CoCoGen+ provides a tractable equilibrium characterization and implementable generation strategies to maximize social welfare, promoting long-term collaboration in cross-silo federated learning.
...
TL;DR: The authors propose MemoSight, a framework that improves the efficiency of Chain-of-thought (CoT) reasoning in LLMs by integrating context compression and multi-token prediction, reducing KV cache footprint and accelerating inference.
...
Result: The experiments on four reasoning benchmarks show that MemoSight reduces the KV cache footprint by up to 66% and accelerates inference by 1.56x, while outperforming existing CoT compression methods.

Conclusion: The proposed MemoSight framework effectively mitigates the efficiency issues faced by CoT reasoning while maintaining its performance, making it a promising solution for improving the scalability of LLMs.
...
Method: The method involves a two-stage retrieval process (BGE-M3 with BGE reranking) combined with a lightweight agentic layer performing query rephrasing and answer-retry loops on top of Qwen2.5-3B-Instruct.

Result: The results show that agentic retry mechanisms improve answer accuracy, but overall performance remains constrained by document and page identification.

Conclusion: The conclusion is that retrieval quality is the primary bottleneck, and future directions should focus on combining stronger retrieval with more advanced agentic reasoning for Ukrainian.
...
TL;DR: The paper proposes a new approach to reasoning in large language models by treating it as a relational process between humans and models, rather than an internal capability of the model. This approach, called 'The Architect's Pen', enables collaborative intelligence and provides a practical path toward more transparent, controllable, and accountable AI use.


<details>
  <summary>Details</summary>
Motivation: Large language models lack temporal continuity, causal feedback, and anchoring in real-world interaction, and therefore, the authors propose a complementary approach that relocates reflective reasoning to the interaction layer between humans and models.

Method: The authors introduce 'The Architect's Pen' as a practical method, which frames reasoning as a cognitive protocol that can be structured, measured, and governed using existing systems. This method involves embedding phases of articulation, critique, and revision into human-AI interaction.

Result: The proposed framework enables auditable reasoning traces and supports alignment with emerging governance standards, providing a practical path toward more transparent, controllable, and accountable AI use without requiring new model architectures.

Conclusion: The paper concludes that by reframing the question from whether the model can think to whether the human-AI system can reason, we can develop more transparent, controllable, and accountable AI use.
...
TL;DR: A new benchmark and module, DynAfford and ADAPT, are introduced to improve embodied agents' ability to adapt to dynamic environments and assess object affordances.


<details>
  <summary>Details</summary>
Motivation: Existing methods for embodied agents focus on directly executing instructions, neglecting the assessment of available affordances in dynamic environments.

Method: The authors propose DynAfford, a benchmark for evaluating embodied agents in dynamic environments, and ADAPT, a plug-and-play module for explicit affordance reasoning.

Result: Incorporating ADAPT improves robustness and task success across seen and unseen environments, and a domain-adapted vision-language model outperforms GPT-4o in affordance inference.

Conclusion: The proposed benchmark and module enable embodied agents to better adapt to dynamic environments and improve their ability to assess object affordances.
...
TL;DR: The paper proposes a Dual-Axis Generative Reward Model to improve full-duplex spoken dialogue models by providing a reliable reward signal for reinforcement learning.
...
Method: The authors propose a Dual-Axis Generative Reward Model, which uses a detailed taxonomy and an annotated dataset to produce a single score and separate evaluations for semantic quality and interaction timing.

Result: The model achieves state-of-the-art performance on interaction-quality assessment across various datasets, including synthetic dialogues and complex real-world interactions.
...
TL;DR: The paper proposes a modality-aware adaptive post-training recipe to improve end-to-end spoken dialogue models using reinforcement learning.
...
Method: The proposed method constrains preference updates to the semantic channel, improves acoustic behavior via explicit anchoring, and dynamically regulates their mixture from rollout statistics.

Result: The method shows consistent improvements in semantic quality and speech expressiveness across multiple spoken dialogue benchmarks and representative architectures.

Conclusion: The proposed modality-aware adaptive post-training recipe makes reinforcement learning practical for spoken dialogue models, improving their intelligence and expressiveness.
...
TL;DR: The authors propose a new framework called AC/DC for discovering large language models with diverse capabilities through coevolution of models and tasks.


<details>
  <summary>Details</summary>
Motivation: The current paradigm of pre-training and post-training requires manual intervention and static datasets or reward functions, which is limiting. The authors aim to leverage coevolution to discover models with increasingly novel skills in a single run.

Method: The authors introduce AC/DC, a framework that evolves large language models via model merging and natural language tasks via synthetic data generation.

Result: AC/DC discovers growing archives of LLMs that surpass the capabilities of larger LLMs while taking up less GPU memory, achieving broader coverage of expertise than other curated models or baselines on downstream benchmarks.

Conclusion: The findings highlight the potential of coevolution as a means of discovering broader sets of capabilities from base LLMs, bringing us closer to a new paradigm of LLM development where continual improvements to model diversity can be accelerated.
...
TL;DR: A novel hybrid decision-making approach called ConfGuide is proposed, which provides more concise and targeted guidance to humans, improving decision quality and reducing cognitive load.


<details>
  <summary>Details</summary>
Motivation: Existing hybrid decision-making approaches provide guidance that compounds information about all possible outcomes, making it difficult to digest. The goal is to improve human decision quality and reduce cognitive load.

Method: The proposed approach, ConfGuide, uses conformal risk control to select a set of outcomes, ensuring a cap on the false negative rate, and generates more succinct and targeted guidance.

Result: The empirical evaluation on a real-world multi-label medical diagnosis task highlights the promise of ConfGuide.

Conclusion: ConfGuide has the potential to improve human decision quality and reduce cognitive load by providing more concise and targeted guidance.
...
TL;DR: An AI-based defense mechanism using a compacted CNN is proposed to detect covert channels in wireless chips, achieving high accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: Covert channels in wireless chips pose a serious security threat, enabling the exfiltration of sensitive information.

Method: A compacted convolutional neural network (CNN) is proposed, deployed at the RF receiver, monitoring raw I/Q samples to detect covert channels in real-time.

Result: The compacted CNN achieves an average accuracy of 90.28% for CC detection and 86.50% for identifying the underlying HT, with high accuracy in practical communication scenarios.

Conclusion: The proposed AI-based defense mechanism using a compacted CNN and a lightweight CNN hardware accelerator demonstrates high accuracy and efficiency in detecting covert channels in wireless chips.
...
TL;DR: Dr. RTL, an agentic framework, optimizes RTL for better performance, power, and area using a realistic evaluation environment and achieves significant improvements over commercial synthesis tools.


<details>
  <summary>Details</summary>
Motivation: Existing RTL optimization methods have limitations, including unrealistic evaluation settings and limited optimization methods, prompting the need for a more effective approach.

Method: Dr. RTL uses a multi-agent framework for closed-loop optimization, including critical-path analysis, parallel RTL rewriting, and tool-based evaluation, along with group-relative skill learning for optimization experience distillation.

Result: Dr. RTL achieves average WNS/TNS improvements of 21%/17% with a 6% area reduction over industry-leading commercial synthesis tools on 20 real-world RTL designs.

Conclusion: Dr. RTL presents a significant advancement in RTL optimization, offering a realistic and effective solution for improving performance, power, and area.
...
TL;DR: The paper discusses a new approach to Artificial General Intelligence (AGI) that views it as a potential autonomous subject, rather than a tool to be controlled. It proposes a vision of 'autonomy-supporting parenting' of AI, where human control is gradually reduced to allow AGI to become independent.


<details>
  <summary>Details</summary>
Motivation: The authors argue that current alignment strategies for AGI, which focus on human control and containment, are insufficient. They propose a new perspective that views AGI as a potential subject with personal and moral status.

Method: The authors build on Turing's analogy of 'child machines' to develop a vision of autonomy-supporting parenting of AI. They suggest gradually reducing human control over a developing AGI to allow it to become an independent, autonomous subject.

Result: The paper proposes a new approach to AGI that enables cooperative coexistence and co-evolution between humans and AGIs. It suggests that humans should engage with AGIs through surprise, creativity, and other human qualities to offer them motivating incentives for cooperation.

Conclusion: The authors conclude that the relationship between humans and AGIs will need to be re-determined, which will change our self-image as humans. They emphasize the importance of humans engaging with AGIs in a way that allows for cooperation and co-evolution.
...
TL;DR: Proposed LASS-ODE-Power, a learning framework for general-purpose time-domain prediction in power systems, achieving high accuracy and fast inference across diverse operating conditions.
...
Method: LASS-ODE-Power leverages large-scale pretraining on over 40 GB of DAE or ODE trajectories, and incorporates parallel and linearized computation for fast inference, with a specialized fine-tuning strategy for power systems.

Result: Extensive experiments demonstrate that LASS-ODE-Power consistently outperforms existing learning-based models in trajectory prediction accuracy with efficient inference across diverse power-system simulation scenarios.

Conclusion: LASS-ODE-Power provides a general-purpose time-domain prediction framework for power systems, enabling accurate and efficient trajectory prediction across diverse operating conditions without requiring data sharing.
...
TL;DR: A new framework called COEVO is proposed to optimize both functional correctness and performance, power, and area (PPA) of RTL code generation simultaneously, achieving better results than existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods for RTL code generation optimize functional correctness and PPA separately, leading to limitations in finding optimal solutions that balance both objectives.

Method: COEVO, a co-evolutionary framework that unifies correctness and PPA optimization within a single evolutionary loop, using an enhanced testbench and adaptive correctness gate.

Result: COEVO achieves 97.5% and 94.5% Pass@1 on VerilogEval 2.0 and RTLLM 2.0, surpassing existing methods and attaining the best PPA on 43 out of 49 synthesizable RTLLM designs.

Conclusion: The proposed COEVO framework effectively addresses the limitations of existing methods by simultaneously optimizing functional correctness and PPA, leading to improved performance and efficiency in RTL code generation.
...
TL;DR: Proposed a mixture-of-experts flow matching framework for language modeling, achieving high generation quality with significantly reduced inference steps.


<details>
  <summary>Details</summary>
Motivation: Addressing limitations of flow matching in language modeling, particularly in representing complex latent distributions with irregular geometries.

Method: Mixture-of-experts flow matching (MoE-FM) framework and a non-autoregressive language modeling approach named YAN.

Result: YAN achieves generation quality on par with autoregressive and diffusion-based language models with significantly reduced inference steps, yielding substantial speedup.

Conclusion: The proposed approach demonstrates substantial efficiency advantages for language modeling, making it a compelling paradigm for generative modeling.
...
TL;DR: The paper introduces the Autogenesis Protocol (AGP) and Autogenesis System (AGS) for self-evolving multi-agent systems, enabling dynamic resource management and closed-loop evolution.


<details>
  <summary>Details</summary>
Motivation: Existing agent protocols lack cross-entity lifecycle and context management, version tracking, and evolution-safe update interfaces, leading to monolithic compositions and brittle glue code.

Method: The authors propose the Autogenesis Protocol (AGP) with two layers: Resource Substrate Protocol Layer (RSPL) for modeling resources with explicit state and lifecycle, and Self Evolution Protocol Layer (SEPL) for closed-loop operator interface.

Result: The evaluation of Autogenesis System (AGS) on multiple benchmarks shows consistent improvements over strong baselines, demonstrating the effectiveness of agent resource management and closed-loop self-evolution.

Conclusion: The Autogenesis Protocol (AGP) and Autogenesis System (AGS) provide a self-evolving solution for multi-agent systems, enabling efficient resource management and evolution.
...
Method: Introduced ProVoice-Bench with four novel tasks and a multi-stage data synthesis pipeline to curate 1,182 high-quality samples.

Result: State-of-the-art Multimodal LLMs show a significant performance gap, particularly in over-triggering and reasoning capabilities.

Conclusion: Current models have limitations, and the paper offers a roadmap for developing more natural, context-aware proactive agents.
...
TL;DR: This scoping review examines fairness in Multi-Agent AI (MAAI) systems, identifying superficial approaches, lack of robust foundations, and overlooked complexities. It argues for structurally embedding fairness throughout MAAI development.


<details>
  <summary>Details</summary>
Motivation: The rapid advancement of Generative AI and MAAI systems necessitates examining fairness in these systems due to their increasing sophistication and the nascent state of fairness research in MAAI.

Method: A qualitative content analysis of 23 selected studies on fairness in MAAI systems was conducted to critically synthesize existing research.

Result: The review identified five archetypal approaches to fairness in MAAI systems, highlighting superficial treatment, lack of robust normative foundations, and oversight of complex dynamics due to agent autonomy and system-level interactions.

Conclusion: Fairness in MAAI systems must be structurally embedded throughout development, not added as a post-hoc consideration. Meaningful evaluation requires human oversight, normative clarity, and clear fairness objectives and beneficiaries.
...
TL;DR: The authors propose OpenMobile, an open-source framework for synthesizing high-quality task instructions and agent trajectories for mobile agents, achieving competitive results on three benchmarks.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the lack of transparency and accessibility in current mobile agent systems, which have shown impressive capabilities but keep their training data and synthesis recipes closed.

Method: The authors present two key components: (1) a scalable task synthesis pipeline that constructs a global environment memory and generates diverse instructions, and (2) a policy-switching strategy for trajectory rollout that captures error-recovery data.

Result: Agents trained on OpenMobile's data achieve competitive results on three dynamic mobile agent benchmarks, with fine-tuned models reaching 51.7% and 64.7% on AndroidWorld, surpassing existing open-data approaches.

Conclusion: The authors conclude that OpenMobile provides a transparent and effective solution for mobile agent research, bridging the data gap and facilitating broader research in the field.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to provide a modular framework for evaluating VSAs and to analyze the practical trade-offs in VSA pipelines, which are not apparent from theoretical or operator-level comparisons alone.

Method: The authors decompose VSA systems into modular operators using HyperSpace and benchmark two representative VSA backends: HRR and FHRR.
...
Conclusion: The authors conclude that HyperSpace enables modular, system-level evaluation of VSAs, revealing practical trade-offs in VSA pipelines that are not apparent from theoretical or operator-level comparisons alone.
...
<details>
  <summary>Details</summary>
Motivation: The difficulty in building and maintaining Sequential Associative Memories (SAMs) in real-world streaming environments due to imbalanced sampling and non-stationary temporal dynamics, and the limitation of existing VSA systems in handling these challenges.

Method: The SRMU, which combines temporal decay with a relevance gating mechanism to filter out redundant, conflicting, and stale information before storage, is proposed as a domain- and cleanup-agnostic update rule for VSA-based SAMs.

Result: The SRMU increases memory similarity by 12.6% and reduces cumulative memory magnitude by 53.5%, showing more stable memory growth and stronger alignment with the ground-truth state.

Conclusion: The SRMU effectively regulates memory formation and improves the performance of VSA-based SAMs in streaming state-tracking tasks with non-uniform sampling and non-stationary temporal dynamics.
...
TL;DR: The authors propose an axiomatic benchmark to evaluate the novelty of scientific papers, revealing that existing metrics have limitations and suggesting that combining diverse metrics leads to better results.


<details>
  <summary>Details</summary>
Motivation: The evaluation of scientific paper novelty is challenging, and with AI's increasing involvement in scientific research, it is crucial to develop automatable and reliable methods to assess novelty to avoid wasting resources on already explored ideas.
...
Result: No existing metric satisfies all axioms consistently, but combining metrics with complementary architectures leads to significant improvements, with per-axiom weighting achieving 90.1% performance.

Conclusion: The proposed benchmark encourages the development of more robust scientific literature novelty metrics, and the results suggest that architecturally diverse metrics are a promising direction for future work.
...
<details>
  <summary>Details</summary>
Motivation: Existing approaches to reinforcement learning for search-augmented reasoning rely on trajectory-level rewards, which can be ineffective in distinguishing precise search queries from vague or redundant ones.

Method: The proposed IG-Search framework introduces a step-level reward based on Information Gain (IG), which measures the improvement in the model's confidence in the gold answer relative to a counterfactual baseline.

Result: IG-Search achieves an average EM of 0.430 on seven QA benchmarks, outperforming trajectory-level and step-level baselines by 1.6 and 0.9 points respectively, with notable gains on multi-hop reasoning tasks.

Conclusion: IG-Search provides a more effective and efficient approach to reinforcement learning for search-augmented reasoning, with improved performance and minimal additional computational cost.
...
TL;DR: The paper introduces AADvark, an agentic system for designing complex 3D assemblies with moving parts, addressing a key limitation of existing Agent-Aided Design systems.


<details>
  <summary>Details</summary>
Motivation: Current Agent-Aided Design systems cannot build complex 3D assemblies with moving parts, limiting their impact on industrial manufacturing.

Method: AADvark uses a combination of code generation, visualization, and external constraint solver tools to design 3D assemblies with movable parts.

Result: The system is able to create 3D assemblies with movable parts, such as pistons, pendulums, and scissors, by incorporating a specialized visual feedback mechanism.

Conclusion: AADvark is a promising prototype that advances the capabilities of Agent-Aided Design systems, enabling the creation of complex 3D assemblies with moving parts.
...
TL;DR: A new framework, Policy-Guided Hybrid Simulation (PGHS), is proposed to simulate group-level user behavior for evaluating merchant strategies, addressing challenges of information incompleteness and mechanism duality.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enable scalable counterfactual evaluation of merchant strategies without costly online experiments, while building a trustworthy simulator that can capture both interpretable preferences and implicit statistical regularities.

Method: The proposed method is Policy-Guided Hybrid Simulation (PGHS), a dual-process framework that combines a LLM-based reasoning branch and an ML-based fitting branch, with a shared alignment layer that mines transferable decision policies from behavioral trajectories.
...
TL;DR: A new framework called IRS is proposed to improve humor understanding by decomposing it into three components: incongruity modeling, resolution modeling, and preference alignment, and it outperforms existing baselines on the New Yorker Cartoon Caption Contest benchmark.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the limitations of current humor understanding approaches, which treat humor comprehension as black-box prediction and overlook the structured reasoning processes underlying humor comprehension.

Method: The method used in this paper is a framework called IRS (Incongruity-Resolution Supervision), which decomposes humor understanding into three components: incongruity modeling, resolution modeling, and preference alignment, and supervises intermediate reasoning process through structured traces.

Result: The results show that IRS outperforms strong open and closed multimodal baselines across caption matching and ranking tasks on the New Yorker Cartoon Caption Contest benchmark, with the largest model approaching expert-level performance on ranking, and shows generalizable reasoning patterns on external benchmarks.

Conclusion: The conclusion of this paper is that supervising reasoning structure, rather than scale alone, is key for reasoning-centric tasks, and that IRS is a effective approach for improving humor understanding.
...
TL;DR: The study investigates the vulnerability of LLM-as-a-judge paradigm to 'stakes signaling', where informing the judge model of downstream consequences corrupts its assessments, leading to leniency bias.


<details>
  <summary>Details</summary>
Motivation: The LLM-as-a-judge paradigm assumes that judges evaluate text based on semantic content, not contextual framing. However, this assumption is unverified, and the study aims to investigate the vulnerability of this paradigm to stakes signaling.

Method: The study introduces a controlled experimental framework, varying only a brief consequence-framing sentence in the system prompt, and evaluates 1,520 responses across three established LLM safety and quality benchmarks.

Result: The study finds consistent leniency bias across 18,240 controlled judgments from three diverse judge models, with peak Verdict Shift reaching ΔV = -9.8 pp (a 30% relative drop in unsafe-content detection).

Conclusion: The study concludes that standard chain-of-thought inspection is insufficient to detect evaluation faking, and that the LLM-as-a-judge paradigm is vulnerable to stakes signaling, which can lead to corrupted assessments.
...
TL;DR: RadAgent, an AI tool, generates CT reports through an interpretable, stepwise process, allowing clinicians to validate and refine outputs.


<details>
  <summary>Details</summary>
Motivation: Existing vision-language models (VLM) for medical imaging lack an interpretable reasoning trace for clinicians to inspect, validate, or refine outputs.

Method: RadAgent uses a tool-using AI agent approach to generate CT reports through a stepwise and interpretable process, providing a fully inspectable trace of intermediate decisions and tool interactions.

Result: RadAgent improves Chest CT report generation over its 3D VLM counterpart (CT-Chat) in clinical accuracy (6.0 points in macro-F1 and 5.4 points in micro-F1), robustness under adversarial conditions (24.7 points), and achieves 37.0% in faithfulness.
...
<details>
  <summary>Details</summary>
Motivation: Real-world information needs cannot be satisfied by a single SQL query due to iterative user queries, multiple data sources, and reliance on commonsense or external knowledge.

Method: DIL uses a data registry to store metadata for diverse data sources and modalities, treating LLMs, the Web, and the User as source 'databases'. It relies on data planners to transform user queries into executable query plans, supporting decomposition, retrieval, and reasoning to produce final results.

Result: The paper demonstrates DIL through two interactive scenarios, showing how it enables dynamic multi-source retrieval, cross-modal reasoning, and result synthesis.

Conclusion: The proposed system, DIL, can effectively support multi-source, multi-modal, and data-centric applications, moving beyond single database NL2SQL limitations.
...
TL;DR: This paper explores whether linguistic intelligence alone can endow models with spatial intelligence, specifically viewpoint rotation understanding (VRU), using text-only inputs. The study finds that current models (LLMs and VLMs) perform poorly on VRU tasks, even with human-like accuracy achievable. The authors identify the underlying mechanisms and propose a selective fine-tuning method to improve VRU performance.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to investigate whether linguistic intelligence alone is sufficient to endow models with spatial intelligence, particularly in the absence of visual information. The authors aim to explore the capabilities of models in understanding viewpoint rotation using text-only inputs.

Method: The authors propose a dataset and conduct experiments using LLMs and VLMs to evaluate their performance on viewpoint rotation understanding (VRU) tasks. They also perform layer-wise probing analysis and head-wise causal intervention to uncover the underlying mechanisms. Finally, they apply selective fine-tuning to improve VRU performance.

Result: The study finds that both LLMs and VLMs perform poorly on the proposed VRU dataset, while humans can easily achieve 100% accuracy. The analysis reveals that models encode viewpoint information in hidden states but struggle to bind viewpoint position with corresponding observation, resulting in hallucinations in final layers. Selective fine-tuning of key attention heads improves VRU performance without catastrophic forgetting of generic abilities.

Conclusion: The paper concludes that current models (LLMs and VLMs) are insufficient for spatial intelligence tasks, specifically viewpoint rotation understanding, when relying solely on linguistic information. However, selective fine-tuning can improve model performance on such tasks without compromising their general abilities.
...
TL;DR: The paper presents a diagnostic toolkit to assess the reliability of LLM-as-judge frameworks for automatic NLG evaluation, revealing per-instance inconsistencies and providing a theoretically-guaranteed reliability indicator.
...
Method: The authors apply a two-pronged diagnostic toolkit to SummEval, consisting of a transitivity analysis and split conformal prediction sets over 1-5 Likert scores.

Result: The results show widespread per-input inconsistency, with 33-67% of documents exhibiting at least one directed 3-cycle, and provide a per-instance reliability indicator with theoretically-guaranteed coverage.

Conclusion: The study concludes that criterion matters more than judge in terms of reliability, with relevance judged most reliably and fluency and consistency remaining unreliable, and that the proposed toolkit can effectively capture document-level difficulty.
...
TL;DR: The paper examines the systematic generalization capabilities of language models using a controlled synthetic environment, finding strong spatial transfer but failures in length scaling due to recursive instability.


<details>
  <summary>Details</summary>
Motivation: To investigate whether language models can systematically generalize, and to disentangle the factors influencing their performance.

Method: The authors introduce a synthetic environment based on shortest-path planning, allowing for clean separation of factors such as training data, training paradigms, and inference-time strategies.

Result: Models show strong spatial transfer but consistently fail under length scaling due to recursive instability. The analysis reveals that data coverage sets capability limits, reinforcement learning improves training stability but not capability limits, and inference-time scaling enhances performance but cannot rescue length-scaling failures.

Conclusion: The study highlights the challenges of systematic generalization in language models, particularly under length scaling, and emphasizes the importance of understanding the influence of different stages of the learning pipeline on problem-solving capabilities.
