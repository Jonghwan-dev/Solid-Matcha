...
TL;DR: The paper proposes a new method called Contrastive Multi-modal Hypergraph Reasoning for multi-person 3D reconstruction in crowded scenes, which combines semantic, geometric, and pose cues to improve performance.


<details>
  <summary>Details</summary>
Motivation: Current approaches for multi-person 3D reconstruction are limited by their reliance on single-modality inputs and their failure to consider the collective group context, leading to challenges in resolving ambiguities in crowded scenes.
...
Conclusion: The proposed method effectively addresses the challenges of multi-person 3D reconstruction in crowded scenes by leveraging multi-modal cues and higher-order crowd dynamics.
...
TL;DR: The study identifies 'massive activations' in Diffusion Transformers (DiTs), a small subset of hidden-state channels crucial for generating images from text prompts, and explores their functional, spatial, and transferable properties.


<details>
  <summary>Details</summary>
Motivation: Despite the strength of Diffusion Transformers (DiTs) in text-to-image generation, the internal mechanisms by which prompts shape image semantics are poorly understood.

Method: The study analyzes massive activations by controlled disruption probes, spatial organization through clustering, and transferability across different prompt-conditioned trajectories.

Result: Massive activations are found to be functionally critical, spatially organized, and transferable, enabling semantic transport and subject-driven generation without additional training.

Conclusion: Massive activations in DiTs organize and control semantic information, acting as a sparse prompt-conditioned carrier subspace, and can be exploited for text-conditioned and image-conditioned semantic transport.
...
TL;DR: A novel 4D pipeline, CineMesh4D, is proposed for accurate whole-heart mesh reconstruction from 2D cine MRI, addressing the challenges of sparse sampling and coupled cardiac shape and motion.
...
TL;DR: A new model is proposed to improve visual understanding by unifying pixel and word tokens in a generative language model, addressing limitations in detail recognition.
...
Method: The proposed model unifies pixel and word tokens, features each pixel with its own token embedding, color folding, global conditional attention approximation, and image unsupervised pretraining.

Result: The experimental results show that the new model has good performance even in small models and with limited training data, and is expected to improve with increased model parameters and training data.

Conclusion: The proposed model has the potential to improve visual understanding capabilities, particularly in detail recognition, and conforms to the scaling law for performance improvement.
...
TL;DR: The paper proposes PVRF, a unified framework for adverse weather removal in images, integrating soft weather perceptions with refinement.


<details>
  <summary>Details</summary>
Motivation: Adverse weather removal in real-world images is challenging due to heterogeneous and unseen degradations, and existing methods often produce overly smooth results.

Method: The PVRF framework combines zero-shot soft weather perceptions with velocity-constrained rectified-flow refinement, using a question answering module and attribute-modulated normalization.
...
Conclusion: The proposed PVRF framework effectively addresses the challenges of adverse weather removal in images, offering improved performance and generalization.
...
TL;DR: Proposed a hardware-aware framework using genetic programming to evolve layer-specific scalar functions for Vision Transformers, eliminating the need for model retraining and achieving a favourable trade-off between complexity and performance.


<details>
  <summary>Details</summary>
Motivation: The deployment of Vision Transformers on edge devices is hindered by the computational complexity and global reduction bottleneck imposed by layer normalization.

Method: Utilized genetic programming to evolve heterogeneous, layer-specific scalar functions directly from pre-trained weights, coupled with a novel post-training re-alignment strategy.

Result: The evolved expressions accurately approximated the target normalization behaviours, capturing 91.6% of the variance (R^2) and recovering 84.25% Top-1 ImageNet-1K accuracy in only 20 epochs.
...
TL;DR: A new benchmark, CurveBench, is introduced for evaluating hierarchical topological reasoning from visual input, with a focus on recovering containment trees from images of Jordan curves.


<details>
  <summary>Details</summary>
Motivation: The authors aim to advance the field of visual reasoning, specifically in the context of topology-aware image understanding, by providing a standardized benchmark for evaluating model performance.

Method: The authors propose a structured prediction task, where a model must recover the full rooted containment tree induced by the curves in an image, and evaluate several state-of-the-art models on the CurveBench dataset.

Result: The strongest evaluated model, Gemini 3.1 Pro, achieves 71.1% tree-generation accuracy on CurveBench-Easy and 19.1% on CurveBench-Hard, while a fine-tuned Qwen3-VL-8B model achieves 33.3% accuracy on CurveBench-Easy, exceeding GPT-5.4 and Claude Opus 4.5.

Conclusion: The results indicate that exact topology-aware visual reasoning remains a challenging task, especially on the more difficult CurveBench-Hard dataset, highlighting the need for further research and development in this area.
...
TL;DR: Proposed DeFakerOne, a unified foundation model for fake image detection and localization, achieving state-of-the-art performance across diverse scenarios and benchmarks.


<details>
  <summary>Details</summary>
Motivation: The rapid evolution of generative AI has reshaped image forgery, creating a mismatch between unified forgery generation and domain-specific detection paradigms, necessitating a unified approach.

Method: DeFakerOne, a data-centric model integrating InternVL2 and SAM2, enables simultaneous image-level detection and pixel-level forgery localization.

Result: DeFakerOne outperforms baselines on 39 forgery detection benchmarks and 9 localization benchmarks, showing superior robustness against real-world perturbations and state-of-the-art generators.

Conclusion: DeFakerOne provides a systematic approach to unified fake image detection and localization, highlighting design principles for scalable, robust, and unified models.
...
TL;DR: DUET, a novel dual-paradigm framework, is proposed to infer spatially resolved gene expression from histology images by synergizing parametric prediction and memory-based retrieval.


<details>
  <summary>Details</summary>
Motivation: Existing methods for inferring gene expression from histology images have limitations, including simple morphology-to-expression mapping and the underutilization of single-cell data.

Method: DUET implements a parallel regression-retrieval paradigm, incorporating large-scale single-cell references and a lightweight adapter for dynamic branch preference assignment.
...
Conclusion: The proposed DUET framework offers a cost-effective and accurate approach to inferring spatially resolved gene expression from histology images.
...
TL;DR: The authors propose a two-tier edge-cloud cascade for diabetic retinopathy detection, reducing cloud usage by half with a modest drop in grading performance.


<details>
  <summary>Details</summary>
Motivation: Diabetic Retinopathy (DR) is a leading cause of preventable blindness, but rural regions lack specialists and infrastructure for early detection. Cloud-based deep learning systems face challenges due to high latency, limited bandwidth, and high data transmission costs.

Method: A two-tier edge-cloud cascade is proposed, with Tier 1 using a lightweight MobileNetV3-small model on a local clinic device for binary triage and Tier 2 using a RETFoundDINOv2 model in the cloud for ordinal severity grading.

Result: The cascade achieves 98.99% sensitivity and 84.37% specificity on Tier 1, and 80.49% accuracy and 0.8167 quadratic weighted kappa on the deployed 4-class output space. Cloud use is reduced by about half compared to a cloud-only baseline.

Conclusion: The proposed edge-cloud cascade effectively reduces cloud usage while maintaining grading performance for diabetic retinopathy detection, making it a viable solution for resource-constrained settings.
...
TL;DR: SToRe3D, a sparsity framework for Vision Transformers (ViTs) in 3D detection, reduces inference latency by up to 3x with minimal accuracy loss.


<details>
  <summary>Details</summary>
Motivation: The high inference latency of ViTs in 3D detection due to dense token and query processing across multiple views and large 3D regions.

Method: SToRe3D, a relevance-aligned sparsity framework that jointly selects 2D image tokens and 3D object queries, storing filtered features for reactivation.

Result: Up to 3x faster inference with marginal accuracy loss on nuScenes and nuScenes-Relevance benchmark.
...
TL;DR: The authors propose ProtoMedAgent, a framework for generating clinical reports using multimodal data, which improves upon standard Retrieval-Augmented Generation (RAG) by introducing a neuro-symbolic bottleneck and a reflective Scribe-Critic loop to ensure semantically accurate and private reports.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the limitations of interpretable prototype networks in clinical diagnostics, specifically their inability to provide semantically structured outputs required for medical documentation.

Method: The authors introduce ProtoMedAgent, which formalizes multimodal clinical reporting as an iterative, zero-gradient test-time optimization problem over a strict neuro-symbolic bottleneck. It uses a frozen prototype backbone to distill latent visual and tabular features into a discrete semantic memory, and constrains online generation using exact set-theoretic differentials and a reflective Scribe-Critic loop.

Result: ProtoMedAgent achieves 91.2% Comparison Set Faithfulness, outperforming standard RAG (46.2%). It also reduces artifact-level membership inference risks by an absolute 9.8% through a binding ℓ-diversity phase transition.

Conclusion: The proposed ProtoMedAgent framework offers a more accurate and private approach to generating clinical reports, addressing the limitations of standard RAG and providing a more reliable tool for clinical diagnostics.
...
TL;DR: PanoPlane enables high-fidelity sparse-view indoor novel view synthesis by reconstructing room geometry via panoramic scene completion.
...
Method: PanoPlane uses $360^{circ}$ panoramic completion and a novel Layout Anchored Attention Steering mechanism to condition the generative process on the full spatial layout and steer attention toward geometrically consistent observed content.

Result: State-of-the-art novel view synthesis quality across 3, 6, and 9 input views, with up to $+17.8%$ improvement in PSNR over the current state-of-the-art baseline.

Conclusion: PanoPlane achieves high-fidelity sparse-view indoor novel view synthesis without requiring training or fine-tuning of the diffusion model.
...
TL;DR: TeDiO, a training-free method, improves temporal coherence in text-to-video diffusion transformers by regularizing internal attention patterns, resulting in smoother motion without compromising visual quality.


<details>
  <summary>Details</summary>
Motivation: Recent text-to-video diffusion transformers struggle with temporal coherence, producing flickering, drifting, or unstable motion, which can be addressed by analyzing and modifying internal attention patterns.

Method: TeDiO estimates diagonal smoothness in self-attention maps, identifies unstable regions, and performs lightweight latent updates to promote coherent frame-to-frame dynamics without modifying model weights.

Result: TeDiO delivers markedly smoother motion across multiple video diffusion models while preserving per-frame visual quality, offering an efficient plug-and-play approach to improving dynamic realism.

Conclusion: TeDiO provides a training-free, inference-time solution for enhancing temporal consistency in video generation systems, leading to more realistic and coherent motion.
...
TL;DR: The paper explores the idea that off-the-shelf embeddings are 'good enough' for complex tasks, proposing a non-parametric pipeline that bypasses backpropagation and achieves state-of-the-art performance on few-shot learning benchmarks.


<details>
  <summary>Details</summary>
Motivation: The Platonic Representation Hypothesis suggests that diverse architectures converge toward a shared latent space, raising the question of whether a 'good embedding' is all that's needed for complex tasks.

Method: The authors propose a straightforward, non-parametric pipeline using a k-Nearest Neighbor classifier on frozen DINOv2-L features, with optional manifold refinement via PCA and ICA.

Result: The approach consistently surpasses sophisticated meta-learning algorithms, achieving state-of-the-art performance across four major few-shot learning benchmarks.

Conclusion: Off-the-shelf embeddings are inherently 'good enough' for complex tasks, making intensive task-specific fine-tuning unnecessary.
...
TL;DR: Proposed a lightweight U-Net architecture for face image super-resolution, achieving 8x magnification from 16x16 to 128x128 images, using a novel heatmap-guided loss leveraging YOLO-World detector outputs.


<details>
  <summary>Details</summary>
Motivation: Existing face image super-resolution methods are often complex and computationally costly, relying on heavy network architectures or separate alignment networks. The goal is to develop a lightweight method that can accurately reconstruct high-resolution facial images from severely degraded inputs.

Method: A lightweight U-Net based-architecture is proposed, utilizing a novel auxiliary-training-free supervision strategy. This strategy leverages heatmaps generated by YOLO-World to localize key facial features, which are then converted into spatial weights to form a heatmap-guided loss.

Result: Experiments on the CelebA dataset demonstrate that the proposed method improves quantitative metrics and produces sharper, more realistic reconstructions. The results show that lightweight networks can effectively exploit detection-driven priors for perceptually convincing extreme upscaling.

Conclusion: The proposed method offers an efficient and effective solution for face image super-resolution, achieving high-quality reconstructions without increased computational cost or adversarial training.
...
<details>
  <summary>Details</summary>
Motivation: Diffusion Transformers (DiTs) have high computational costs, limiting scalability and on-device deployment.
...
Conclusion: CoReDiT enables efficient deployment of DiTs on device and cloud while preserving high visual quality.
...
TL;DR: A new approach for 3D brain segmentation in MRI scans that uses landmarks to improve accuracy and align with expert-defined boundaries.


<details>
  <summary>Details</summary>
Motivation: Current voxel-wise deep models for brain segmentation in MRI scans often produce anatomically inconsistent results, diverging from expert-defined boundaries.

Method: A three-step approach: 1) A Global-to-Local network detects 16 landmarks representing key subcortical reference points. 2) A semantic segmentation model produces a coarse segmentation of 12 anatomical labels. 3) A landmark-driven post-processing step separates these labels into 26 distinct structures.

Result: Experimental results show consistent improvements in boundary accuracy, with segmentations more closely aligned with manual protocols.

Conclusion: The proposed landmark-guided 3D brain segmentation approach improves the accuracy of brain structure segmentation in MRI scans by integrating learned landmarks.
...
TL;DR: The authors propose a new network, IFGNet, for fusing hyperspectral images and LiDAR data, which outperforms existing methods in classification tasks.


<details>
  <summary>Details</summary>
Motivation: The authors are motivated by the challenges of hyperspectral image classification in complex scenes, where existing methods struggle to model nonlinear relationships and interactions between hyperspectral and LiDAR features.

Method: The authors propose the Implicit Frequency-Geometry Fusion Network (IFGNet), which uses Kolmogorov-Arnold Networks (KANs) with learnable spline-based functions to adaptively capture nonlinear relationships between hyperspectral and LiDAR features.

Result: The proposed IFGNet consistently outperforms existing fusion methods in overall accuracy, average accuracy, and Cohen's Kappa on the Houston 2013 and MUUFL benchmarks.

Conclusion: The authors conclude that IFGNet is an effective and efficient approach for fusing hyperspectral images and LiDAR data, which can improve classification performance in complex scenes.
...
TL;DR: The study evaluates the generalizability of conditional generative adversarial networks (cGANs) for computational staining and destaining of hematoxylin and eosin (H&E) in digital pathology whole-slide images (WSI) across institutions without retraining.
...
TL;DR: Developed a real-time catheter tip tracking pipeline under fluoroscopy for robotic systems to improve stroke treatment access.


<details>
  <summary>Details</summary>
Motivation: Mechanical thrombectomy (MT) is limited by lack of local treatment access, and the goal is to use reinforcement learning-based robotic systems with autonomous navigation.

Method: A multi-threaded pipeline was designed using deep learning segmentation models (U-Net, U-Net+Transformer, SegFormer) with post-processing techniques.
...
Conclusion: The proposed framework maintains stable performance under challenging imaging conditions, providing a reliable foundation for RL-based autonomous MT navigation.
...
TL;DR: The paper proposes a method to accelerate diffusion models for image restoration tasks by projecting data into lower-dimensional subspaces, achieving faster inference speeds while maintaining reconstruction fidelity.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion models for image restoration tasks are computationally inefficient, operating in high-dimensional pixel space or requiring repeated encoder-decoder inference in latent space.
...
Result: The proposed methods outperform recent DM-based approaches in the majority of experimental scenarios across diverse image datasets and various restoration tasks, achieving a balance between inference speed and reconstruction fidelity.

Conclusion: The proposed methods demonstrate the effectiveness of projecting data into lower-dimensional subspaces for accelerating diffusion models in image restoration tasks, achieving improved performance and efficiency.
...
TL;DR: A new reward function, PhyMotion, is proposed for generating realistic human motion in videos by evaluating physical feasibility through a physics simulator.


<details>
  <summary>Details</summary>
Motivation: Existing video rewards fail to reliably score motion realism, often giving high scores to videos with unrealistic movements, because they rely on 2D perceptual signals and do not model 3D body state, contact, and dynamics.

Method: PhyMotion recovers 3D human trajectories from generated videos, retargets them onto a humanoid in a physics simulator, and evaluates motion quality across kinematic plausibility, contact and balance consistency, and dynamic feasibility.

Result: PhyMotion achieves stronger correlation with human judgments, leads to larger and more consistent improvements in motion realism during RL-based post-training, and improves video generation quality with modest training overhead.

Conclusion: PhyMotion provides a more reliable and effective way to generate realistic human motion in videos by leveraging a structured, fine-grained motion reward based on physical feasibility.
...
TL;DR: The paper proposes Omission Signal Intervention (OSI) to address concept omission in text-to-image generation models, which amplifies an 'omission signal' in text embeddings to generate missing concepts.


<details>
  <summary>Details</summary>
Motivation: Concept omission in text-to-image generation models, where specified objects or attributes fail to emerge in generated images.

Method: Omission Signal Intervention (OSI), which amplifies the omission signal in text embeddings to catalyze the generation of missing concepts.

Result: Comprehensive experiments on FLUX.1-Dev and SD3.5-Medium demonstrate that OSI significantly alleviates concept omission even in extreme scenarios.

Conclusion: The proposed OSI method effectively addresses concept omission in text-to-image generation models.
...
TL;DR: A new method, CreFlow, is proposed for improving video generation models in embodied manipulation tasks by using a compositional constraint-based reward model and an online RL framework.


<details>
  <summary>Details</summary>
Motivation: Existing video-RL rewards often fail to evaluate manipulation video generation properly as they reduce rollouts to low-level visual metrics, not accounting for task specifications.

Method: CreFlow, a novel online RL framework, which includes a credit-aware NFT loss and a corrective reflow loss. It utilizes a compositional constraint-based reward model that formulates task requirements as Linear Temporal Logic constraints.

Result: Experiments show that CreFlow yields reward judgments better aligned with human and simulator success labels and improves downstream execution success by 23.8 percentage points across eight bimanual manipulation tasks.

Conclusion: The proposed CreFlow method effectively improves video generation models in embodied manipulation tasks by providing faithful rewards and localized error information.
...
TL;DR: KVPO, a novel framework, improves alignment of streaming autoregressive video generators with human preferences by introducing a causal-semantic exploration paradigm and a velocity-field surrogate policy, achieving better visual quality, motion quality, and text-video alignment.


<details>
  <summary>Details</summary>
Motivation: Existing reinforcement learning methods for aligning streaming autoregressive video generators with human preferences have limitations, including reliance on noise-based exploration and surrogate policies that mismatch the deterministic ODE dynamics of distilled AR models.
...
Result: Experiments demonstrate consistent gains in visual quality, motion quality, and text-video alignment across both single-prompt short-video and multi-prompt long-video settings.

Conclusion: KVPO effectively addresses the challenges of aligning streaming autoregressive video generators with human preferences, providing a more suitable approach for long-horizon coherence and semantic storyline progression.
...
<details>
  <summary>Details</summary>
Motivation: Current machine unlearning methods in VLMs are limited as they operate at the image or instance level, making it challenging to remove target knowledge without impacting unrelated semantics.

Method: The proposed framework constructs a task-specific concept vocabulary and decomposes visual representations into sparse, nonnegative combinations of semantic concepts, enabling fine-grained knowledge manipulation.

Result: The method demonstrates comprehensive target forgetting, preserves non-target knowledge within the same image, and maintains competitive model utility compared to existing VLM unlearning methods.

Conclusion: The proposed concept-level unlearning framework offers a more precise and effective approach to machine unlearning in VLMs, addressing the limitations of current methods.
...
TL;DR: The paper proposes a new method for compressing key-value (KV) cache in large vision-language models (VLMs) for streaming video understanding, by viewing it as a coreset selection problem and introducing a bicriteria objective that balances coverage in key and value spaces.


<details>
  <summary>Details</summary>
Motivation: Existing streaming methods for KV-cache compression rely on local token-wise heuristics, which do not explicitly optimize whether the retained cache is representative of the accumulated history.

Method: The proposed method operates in a joint KV representation, introduces a bicriteria objective that balances coverage in key and value spaces, and uses an orthogonality-driven diversity criterion to encourage a more diverse retained subset.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods struggle to balance reconstruction quality and computational efficiency, and lack mechanisms to adaptively learn representative tokens.

Method: TurboVGGT employs an end-to-end trainable framework with adaptive sparse global attention and frame attention to capture global relationships and aggregate local details.
...
Conclusion: TurboVGGT proposes a novel approach for efficient and effective 3D reconstruction, outperforming existing methods in terms of speed and quality.
...
TL;DR: A novel remote sensing image generation framework, D2-CDIG, is proposed, integrating diffusion models with a dual-prior control mechanism to generate high-quality images with precise control over terrain and atmospheric conditions.


<details>
  <summary>Details</summary>
Motivation: Existing controllable remote sensing image generation methods rely on traditional techniques such as segmentation and edge detection, which lack accuracy and naturalness when dealing with complex terrains and atmospheric phenomena.
...
Conclusion: D2-CDIG offers a reliable data foundation for remote sensing large models and downstream tasks, providing high-quality data for training with precise control over terrain and atmospheric conditions.
...
TL;DR: Proposed InsightTok, a discrete visual tokenization framework that improves text and face fidelity in autoregressive image generation.


<details>
  <summary>Details</summary>
Motivation: Autoregressive generators struggle with text and faces due to tokenizers' aggressive downsampling and quantization, which discard fine-grained structures.

Method: InsightTok uses localized, content-aware perceptual losses with a compact 16k codebook and 16x downsampling rate.

Result: InsightTok outperforms prior tokenizers in text and face reconstruction and transfers gains to autoregressive image generation, producing images with clearer text and more faithful facial details.

Conclusion: Specialized supervision in tokenizer training can advance discrete image generation, particularly for text and faces.
...
TL;DR: The paper addresses the challenge of restoring nighttime scenes with multiple degradations, proposing a diffusion model with an illumination-guided module.


<details>
  <summary>Details</summary>
Motivation: The difficulty in perceiving surroundings at night and the limitations of current image restoration methods in handling complex nocturnal scenes.

Method: A diffusion model with an embedded illumination-guided module to guide the illumination restoration process.

Result: The proposed model can preserve texture fidelity while handling various degradations in low-light scenarios.

Conclusion: The paper presents a solution for restoring nighttime scenes with multiple degradations, leveraging a novel integration of illumination guidance in a diffusion model.
...
TL;DR: A novel diffusion framework called AnyBand-Diff is proposed for robust spectral reconstruction in remote sensing imagery, ensuring radiometric fidelity and physical consistency.
...
Method: AnyBand-Diff uses a Masked Conditional Diffusion backbone with a dual stochastic masking strategy, Physics-Guided Sampling mechanism, and a Multi-Scale Physical Loss to enforce physical constraints.
...
Conclusion: AnyBand-Diff contributes to the advancement of physics-aware generative methods for Earth observation, enabling robust spectral reconstruction in remote sensing imagery.
...
TL;DR: Proposed a hierarchical framework for single-frame Infrared Small Target Detection (ISTD) using point supervision with knowledge distillation, achieving improved detection accuracy and training stability.


<details>
  <summary>Details</summary>
Motivation: To reduce annotation cost and improve detection accuracy in ISTD, which faces challenges due to weak targets and heavy background clutter.

Method: A hierarchical VFM-driven knowledge distillation framework with point supervision, bilevel optimization, Semantic-Conditioned Affine Modulation (SCAM), and dynamic collaborative learning.
...
Conclusion: The proposed framework effectively addresses the challenges of ISTD with point supervision, providing a more accurate and stable detection approach.
...
TL;DR: A new method called Delta Forcing is proposed to improve the balance between reactivity and stability in interactive real-time autoregressive video generation.


<details>
  <summary>Details</summary>
Motivation: The need for interactive real-time autoregressive video generation that can adapt to dynamically evolving event conditions, while balancing reactivity and stability.
...
Result: Delta Forcing significantly improves consistency while maintaining event reactivity in extensive experiments.

Conclusion: The proposed Delta Forcing framework effectively addresses the challenge of balancing reactivity and stability in interactive real-time autoregressive video generation.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for learned image compression struggle to balance distortion fidelity and perceptual realism, particularly across different bitrates, due to the limitations of using a single compressed latent representation.
...
Result: The proposed MoDE framework achieves a more favorable fidelity-perception balance than existing baselines across a wide bitrate range.

Conclusion: The paper concludes that decoder-side expert collaboration is an effective design for wide-range fidelity-perception balanced learned image compression.
...
TL;DR: The paper presents a method for analogical trajectory transfer, enabling machines to transfer motion trajectories from one 3D environment to another semantically similar location.


<details>
  <summary>Details</summary>
Motivation: The goal is to enable machines to perform analogical spatial reasoning with applications in AR/VR co-presence, content creation, and robotics.

Method: The method involves decomposing the problem into spatially segregated subproblems, estimating cross-scene mappings via hierarchical smooth map prediction, and refining the result to remove collisions and distortions.

Result: The method outperforms baselines based on LLMs, VLMs, and scene graph matching, with a fast runtime of around 0.6 seconds, and demonstrates broad applicability to AR/VR and robotics.

Conclusion: The paper proposes a novel approach for analogical trajectory transfer, achieving semantically consistent and spatially coherent transfers without requiring training.
...
TL;DR: MIRAGE is a framework for discovering semantic attacks on autonomous vehicle mapping systems, which can bypass standard defenses and cause significant disruptions.


<details>
  <summary>Details</summary>
Motivation: The safety of autonomous vehicles relies on accurate mapping of road elements, but existing defenses against mapping disruptions can be overcome by more sophisticated attacks.

Method: MIRAGE uses diffusion models to find semantically mutated scenes that can mislead mapping predictions, exploiting the latent manifold of real-world data.

Result: MIRAGE successfully launched two attacks: boundary removal (suppressing 57.7% of detections and corrupting 96% of planned trajectories) and boundary injection (the only method to successfully inject fictitious boundaries).

Conclusion: Current adversarial defenses have a categorical gap in handling semantic-level perturbations that appear as legitimate environmental variations, making them substantially harder to mitigate than pixel-level perturbations.
...
<details>
  <summary>Details</summary>
Motivation: Many multimodal learning tasks require consistent supervision across edits, viewpoints, and scene-level interventions, but such supervision is difficult to obtain from observation-level datasets.
...
Conclusion: SceneForge provides a framework for generating structured supervision from editable 3D world states, enabling consistent and aligned outputs for multimodal learning tasks.
...
<details>
  <summary>Details</summary>
Motivation: The application of Multimodal Large Language Models (MLLMs) in dermatology is hindered by insufficient domain-specific grounding and hallucinations.

Method: DermAgent uses a Plan-Execute-Reflect framework with seven specialized vision and language modules, including complementary visual perception tools, a dual-modality retrieval module, and a deterministic critic module.

Result: DermAgent outperforms state-of-the-art MLLMs and medical agent baselines across various tasks, exceeding GPT-4o by 17.6% in skin disease diagnostic accuracy and 3.15% in captioning ROUGE-L.

Conclusion: DermAgent offers a comprehensive solution for dermatological diagnosis, providing stepwise, traceable diagnostic reasoning and improving accuracy while reducing hallucinations.
...
TL;DR: Proposed Think When Needed (TWN), a unified multimodal embedding framework with adaptive reasoning, achieving state-of-the-art embedding quality while being more efficient.


<details>
  <summary>Details</summary>
Motivation: Recent multimodal large language models (MLLMs) with chain-of-thought (CoT) reasoning are costly and inefficient, and redundant reasoning can degrade performance.

Method: TWN uses a dual-LoRA architecture with adaptive think mechanism and self-supervised routing gate to decide when to generate CoT, and embedding-guided RL to optimize CoT quality.
...
Conclusion: TWN offers a more efficient and effective multimodal embedding framework with adaptive reasoning, reducing inference overhead and improving retrieval quality.
...
TL;DR: Presents ClickRemoval, an interactive object removal tool driven by user clicks, leveraging pretrained Stable Diffusion models for precise object removal and background restoration.


<details>
  <summary>Details</summary>
Motivation: Existing object removal tools require manual masks or text prompts, which can be challenging for non-expert users, especially in complex scenes, often resulting in incomplete removal or unnatural background completion.

Method: ClickRemoval utilizes pretrained Stable Diffusion models and user clicks to localize target objects and restore the background through self-attention modulation during denoising, without requiring additional training, hand-drawn masks, or text descriptions.

Result: ClickRemoval achieves competitive results across quantitative metrics and user studies, demonstrating its effectiveness in object removal and background restoration.

Conclusion: The proposed ClickRemoval tool offers a user-friendly and effective solution for object removal, leveraging the power of pretrained Stable Diffusion models and user clicks, and is made available as an open-source software package.
...
TL;DR: The paper introduces HA-HOI, a framework for reconstructing physically plausible 4D human-object interaction (HOI) animation from monocular videos, improving human-object alignment, contact consistency, and simulation readiness.


<details>
  <summary>Details</summary>
Motivation: Current methods for reconstructing 4D HOI from monocular videos often result in visual artifacts and fail to preserve stable contact, functional manipulation, or physical plausibility, highlighting the need for a more comprehensive approach that recovers the relation between human and object motion.

Method: The HA-HOI framework uses a 'human-first, object-follow' formulation, where human motion is recovered as the interaction anchor, and the object is reconstructed, aligned, and refined relative to the human action, then projected into a physics-based humanoid-object simulation.

Result: HA-HOI improves human-object alignment, contact consistency, temporal stability, and simulation readiness over prior monocular HOI reconstruction methods, enabling the creation of scalable demonstrations for humanoid-object behavior.

Conclusion: The paper proposes a novel approach to reconstructing physically plausible 4D HOI animation from monocular videos, taking a step toward turning general monocular HOI videos into scalable demonstrations for humanoid-object behavior.
...
TL;DR: The paper proposes GeoVista, a planning-driven framework for interpreting ultra-high-resolution remote sensing images by actively perceiving and aggregating evidence from multiple regions.


<details>
  <summary>Details</summary>
Motivation: Existing remote sensing vision-language models have limitations in exploring large-scale scenes, losing global context, and revisiting the same evidence. The authors aim to address these challenges.

Method: The authors propose GeoVista, which builds a global exploration plan, verifies multiple candidate regions through local inspection, and maintains an explicit evidence state. They also introduce APEX-GRO, a supervised trajectory corpus, and design an Observe-Plan-Track mechanism.

Result: GeoVista achieves state-of-the-art performance on RSHR-Bench, XLRS-Bench, and LRS-VQA.

Conclusion: The proposed GeoVista framework effectively addresses the challenges of interpreting ultra-high-resolution remote sensing images by actively perceiving and aggregating evidence from multiple regions, achieving state-of-the-art performance.
...
TL;DR: The paper proposes a new method for detecting AI-generated images by combining reconstruction-based and GAN-based approaches, and introducing a Separate Expert Fusion (SEF) framework to improve generalization across different generative methods.


<details>
  <summary>Details</summary>
Motivation: The misuse of AI-generated images is growing, and there is a need for generalizable image detection techniques. Current state-of-the-art methods have limitations in capturing diverse generative patterns.

Method: The paper proposes a GAN-based upsampling approach to complement reconstruction-based fake images, and a Separate Expert Fusion (SEF) framework to extract complementary artifact information and reduce inter-domain interference.

Result: The method yields strong results across 13 diverse benchmarks, demonstrating improved generalization across broader generative methods.

Conclusion: The paper concludes that the proposed method can learn a more robust decision boundary and improve generalization across different generative methods, enabling better detection of AI-generated images.
...
TL;DR: A new framework called Head Forcing is proposed to improve autoregressive video diffusion models by optimizing KV cache allocation for different types of attention heads, leading to extended generation duration and improved performance.


<details>
  <summary>Details</summary>
Motivation: Autoregressive video diffusion models suffer from error accumulation and context loss over long horizons due to uniform treatment of attention heads with different roles.

Method: Head Forcing, a training-free framework that assigns tailored KV cache strategies to different types of attention heads (local, anchor, and memory heads) and employs a head-wise RoPE re-encoding scheme.

Result: Head Forcing extends generation from 5 seconds to minute-level duration, supports multi-prompt interactive synthesis, and consistently outperforms existing baselines without additional training.

Conclusion: The proposed Head Forcing framework effectively addresses the limitations of autoregressive video diffusion models by optimizing attention head utilization, leading to improved performance and extended generation capabilities.
...
TL;DR: The paper introduces a new framework for efficient sparse attention in diffusion-based video generation, achieving up to 1.93 times speedup without retraining or compromising video quality.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the limitations of existing sparse attention methods in diffusion-based video generation, which suffer from high computational costs and shared thresholds across heads.

Method: The proposed framework consists of two plug-in components: Temporal Mask Reuse and Error-guided Budgeted Calibration, which enable head-wise adaptive sparse attention.

Result: The method achieves up to 1.93 times speedup at 720P on Wan2.1-1.3B and Wan2.1-14B models, maintaining competitive video quality and similarity metrics.

Conclusion: The proposed framework effectively improves the practical speed-quality trade-off of training-free sparse attention in Video DiTs, enabling more efficient deployment of diffusion-based video generation models.
...
TL;DR: The paper proposes a new activation function called Adaptive Arctangent Gated Activation (ArcGate) for deep networks, which outperforms standard baselines in remote sensing tasks, especially in noisy environments.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the performance and robustness of deep networks in remote sensing tasks by introducing a flexible and adaptive activation function.

Method: The method involves proposing a new activation function, ArcGate, which uses seven learnable parameters per layer to autonomously optimize non-linearity, and evaluating it using ResNet-50 and Vision Transformer architectures on three remote sensing benchmarks.

Result: The results show that ArcGate consistently outperforms standard baselines, achieving a peak overall accuracy of 99.67% on PatternNet, and exhibits superior structural resilience in noisy environments.

Conclusion: The conclusion is that ArcGate is a robust and adaptive general node activation function for high-resolution earth observation tasks, which can enhance signal propagation and improve performance in noisy environments.
...
TL;DR: A novel 3D human pose estimation method using sparse interleaved images from different camera views at various time points, allowing for richer spatio-temporal information and improved performance.


<details>
  <summary>Details</summary>
Motivation: Traditional multi-view 3D human pose estimation methods overlook temporal dependencies between frames, and the proposed method aims to address this limitation.

Method: Sparse interleaved input method with DenseWarper model, leveraging epipolar geometry for efficient spatio-temporal heatmap exchange.

Result: Outperforms traditional dense multi-view input approaches, achieves state-of-the-art performance on Human3.6M and MPI-INF-3DHP datasets.

Conclusion: The proposed method effectively boosts performance, increases output pose frame rate, and reduces data redundancy in 3D human pose estimation.
...
TL;DR: The paper identifies issues with large diffusion vision-language models (LDVLMs) in long-form generation, proposes a training-free approach to address these issues, and demonstrates improvements over baseline LDVLMs.
...
Method: The authors propose a training-free approach, introducing Mask Prior Suppression and Monotonic RoPE Scaling to mitigate mask prior drift and positional attention collapse during decoding.

Result: Experiments on multimodal benchmarks and visual grounding tasks demonstrate improvements over baseline LDVLMs, with robust gains on long-form description benchmarks.

Conclusion: The proposed approach effectively addresses the failures of LDVLMs in long-form generation with a lightweight, plug-and-play strategy that requires no additional training and generalizes across diverse LDVLM architectures.
...
<details>
  <summary>Details</summary>
Motivation: Existing metrics for object removal in images and videos have limitations, such as favoring copy-paste behaviors or blurry results, and are insensitive to localized artifacts.

Method: The authors propose RC (Removal Coherence), a pair of metrics: RC-S for spatial coherence and RC-T for temporal consistency, and introduce PROVE-Bench, a real-world benchmark dataset.

Result: Experiments demonstrate that RC achieves stronger alignment with human judgments than existing evaluation protocols.
...
TL;DR: Proposed a Local Spatiotemporal Convolutional Network (LSTCN) for gait recognition, a simple yet effective dual-branch architecture that extracts temporal and spatial information.
...
Method: Introduced a Global Bidirectional Spatial Pooling (GBSP) mechanism and a Local Spatiotemporal Convolutional (LSTC) layer to jointly process temporal and spatial dimensions, allowing the network to adaptively learn strip-based gait motion patterns.

Result: The proposed LSTCN architecture provides a structurally simple yet highly effective approach for gait recognition, enabling the extraction of temporal information and reducing the dimensionality of gait tensors.

Conclusion: The proposed LSTCN offers a promising solution for gait recognition, addressing the limitations of existing approaches and providing a simple yet effective architecture for extracting temporal and spatial information.
...
TL;DR: A novel framework for high-fidelity natural image decomposition is proposed, addressing challenges in scalable layered data and object interaction modeling.


<details>
  <summary>Details</summary>
Motivation: The success of generative models in graphic design domains is limited in real-world scenarios due to the underexplored problem of layering in-the-wild images, which restricts fine-grained editing and applications.

Method: The proposed framework includes an Agent-driven Data Decomposition (ADD) pipeline for synthesizing layered data and a novel model that jointly improves photometric fidelity and alpha boundary accuracy.

Result: The framework achieves state-of-the-art performance in natural image decomposition, outperforming existing models in RGB L1 and Alpha IoU metrics.
...
TL;DR: The paper proposes a two-stage framework for medical image segmentation in low-data regimes, combining structure-aware encoder pretraining with boundary-oriented decoding, achieving improved robustness and boundary delineation.


<details>
  <summary>Details</summary>
Motivation: Medical image segmentation is challenging in low-data regimes due to poor generalization and ambiguous boundaries, and recent self-supervised pretraining methods exhibit texture bias, whereas accurate segmentation requires geometry-awareness and precise boundary preservation.

Method: A two-stage framework is proposed: (1) Mixed-Domain MeanFlow Pretraining for structure-aware encoder pretraining, and (2) fine-tuning with a lightweight decoder using Direct Attentional Fusion and Frequency-Directional Dynamic Convolution.

Result: Experiments on ISIC-2016, Kvasir-SEG, and GlaS demonstrate consistent gains over state-of-the-art methods, with improved robustness in low-data settings and sharper boundary delineation.

Conclusion: The proposed framework effectively addresses the challenges of medical image segmentation in low-data regimes, providing improved performance and robustness.
...
TL;DR: A novel framework called CineNeuron is proposed for reconstructing videos from fMRI signals by using a hierarchical approach with two stages: semantic enrichment and memory integration.


<details>
  <summary>Details</summary>
Motivation: Current fMRI-to-video reconstruction methods are limited by a semantic gap between fMRI signals and video content, which is caused by incomplete semantic embeddings that fail to capture video-specific cues and prior knowledge.

Method: CineNeuron uses a dual-pathway processing mechanism with two stages: (1) a bottom-up semantic enrichment stage that maps fMRI signals to a rich embedding space, and (2) a top-down memory integration stage that utilizes a Mixture-of-Memories method to refine video reconstruction.

Result: CineNeuron surpasses state-of-the-art methods across various metrics on two fMRI-to-video benchmarks.

Conclusion: The proposed CineNeuron framework offers a significant improvement in fMRI-to-video reconstruction by effectively bridging the semantic gap and capturing video-specific cues.
...
TL;DR: Med DisSeg, a dispersion-driven medical image segmentation framework, is proposed to improve representation learning and anatomical delineation.
...
Method: Med DisSeg combines a lightweight Dispersive Loss with adaptive attention for fine-grained structure segmentation.

Result: Med DisSeg achieves state-of-the-art performance on five datasets spanning three imaging modalities and competitive results on multi-organ CT segmentation.

Conclusion: Med DisSeg effectively addresses representation collapse and insufficient fine-grained multi-scale decoding, leading to improved medical image segmentation.
...
TL;DR: The authors investigate whether aggregating visual patch tokens into a single vector for financial documents leads to loss of key information, finding that it does, and identify global texture dominance as the root cause.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of aggregating hundreds of patch tokens per document into a single vector for practical deployment of Visual RAG in financial applications, and to assess the potential loss of key information.
...
Result: The experiments show that single-vector aggregation collapses different documents with almost identical vectors, and that patch-level detection is better at identifying semantic changes, while single-vector aggregation obscures these details.

Conclusion: The authors conclude that single-vector aggregation poses significant risks for visual document retrieval in financial applications due to loss of key information, and that patch-level detection is more effective in identifying semantic changes.
...
TL;DR: FedStain, a stain-aware federated domain generalization framework, is proposed to address cross-institutional stain heterogeneity in whole-slide image analysis while maintaining strict data governance.


<details>
  <summary>Details</summary>
Motivation: Current federated learning and domain generalization approaches fail to model higher-order, non-Gaussian characteristics of real-world stain variability, which limits their performance in robust whole-slide image analysis under strict data governance.

Method: FedStain incorporates higher-order stain moments (skewness and kurtosis) into a federated domain generalization framework, enabling the global model to capture stain variability missed by low-order statistics, and employs a contrastive, cross-site parameter aggregation strategy.

Result: FedStain yields consistent improvements, outperforming state-of-the-art federated learning, domain generalization, and federated domain generalization baselines by up to +3.9% absolute accuracy on Camelyon17 and MvMidog-Fed benchmarks.

Conclusion: FedStain is the first federated domain generalization approach to explicitly model higher-order stain statistics, enabling robust cross-institutional deployment in computational pathology.
...
Method: The authors propose a novel framework called TOPOS, which includes a variational autoencoder (TOPOS-VAE) and a rectified flow transformer (TOPOS-DiT) to generate head meshes with a fixed topology, and a module (TOPOS-Texture) to produce relightable UV texture maps.

Result: TOPOS achieves state-of-the-art performance on 3D head generation, surpassing classical face reconstruction methods and general 3D object generative models.

Conclusion: The proposed TOPOS framework enables the generation of high-fidelity 3D heads with a fixed, industry-standard topology, facilitating digital human creation and asset-level reuse.
...
<details>
  <summary>Details</summary>
Motivation: Existing precipitation nowcasting methods face challenges due to the chaotic property of precipitation systems, and the limitations of single-source radar data and single deterministic or probabilistic models.

Method: A two-stage framework, VMU-Diff, which uses a coarse-to-fine approach, combining a deterministic model with multi-source data (radar and satellite) and a probabilistic model with residual conditional diffusion.

Result: Experiments on Jiangsu SWAN datasets show improvements over state-of-the-art methods, particularly in short-term forecasts.

Conclusion: The proposed VMU-Diff framework effectively addresses the challenges of precipitation nowcasting, providing a more accurate and efficient approach.
...
TL;DR: A new framework, PanoGSDet, is proposed for 3D object detection in panoramic imagery, leveraging continuous semantic 3D Gaussian representations to improve accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: The challenge of accurately mapping 2D features to 3D in panoramic imagery for comprehensive scene understanding, due to limitations of prevailing methods that project 2D features onto discrete 3D grids.

Method: PanoGSDet, a monocular panoramic 3D detection framework consisting of a panoramic depth estimation component and a semantic Gaussian component, which includes modules for lifting, optimizing, and predicting 3D bounding boxes from Gaussian representations.

Result: The proposed method significantly outperforms existing methods on the Structured3D dataset.

Conclusion: PanoGSDet offers a more accurate and efficient approach to 3D object detection in panoramic imagery by utilizing continuous semantic 3D Gaussian representations.
...
TL;DR: A novel multi-scale encoder-decoder architecture called MambaRain is proposed for accurate precipitation nowcasting over extended horizons (0-3 hours).
...
Conclusion: MambaRain effectively extends the viable forecasting horizon to 2-3 hours with substantial accuracy improvements, making it a promising solution for disaster mitigation and operational decision-making.
...
TL;DR: The paper introduces ViMU, a benchmark to evaluate video understanding models' ability to comprehend subtext, implicit meanings, and social context in videos, beyond literal visual comprehension.


<details>
  <summary>Details</summary>
Motivation: Existing video understanding models focus primarily on literal visual comprehension and lack the ability to understand metaphorical, ironic, and social meanings embedded in videos.

Method: The authors introduce ViMU, a benchmark that assesses video understanding models' ability to infer implicit meaning from videos, using multimodal evidence and answering open-ended and multiple-choice questions.

Result: The paper aims to bridge the gap in video understanding by evaluating models' ability to go beyond literal perception and understand subtext, but does not provide specific results.

Conclusion: The paper concludes that understanding the true meaning of videos requires going beyond literal visual comprehension and understanding the context, style of expression, and social experience.
...
<details>
  <summary>Details</summary>
Motivation: Current image segmentation methods often rely on standard loss functions like Cross-Entropy and Dice, which neglect the discriminative structure of learned features, leading to inaccurate boundaries.

Method: Deep Discriminant Analysis (DDA), a differentiable, architecture-agnostic loss function that embeds classical discriminant principles for network training.

Result: DDA consistently improves segmentation accuracy, boundary sharpness, and model confidence across various architectures on the DIS5K benchmark.
...
TL;DR: A new method called CalibAnyView is introduced for camera calibration using an arbitrary number of views, improving geometric consistency and accuracy.
...
Method: A unified formulation, CalibAnyView, uses a multi-view transformer to predict dense perspective fields, which are integrated into a geometric optimization framework to estimate camera intrinsics and gravity direction.
...
<details>
  <summary>Details</summary>
Motivation: Existing contrastive decoding methods for mitigating hallucinations in LVLMs have limitations, such as introducing off-manifold artifacts and requiring extra forward passes. The authors aim to develop a more efficient and effective method.

Method: SIRA constructs a counterfactual reference inside the LVLM by exploiting the staged information flow of multimodal transformers. It first forms an aligned multimodal state through a shared prefix and then forks a counterfactual branch in later transformer layers, where attention to image-token positions is masked.
...
Conclusion: SIRA is a training-free, efficient, and effective method for reducing hallucinations in LVLMs, applicable to open-weight LVLMs with white-box inference access.
...
TL;DR: A unified framework, UniTriGen, is proposed for generating aligned VIS-IR-Label triplets for RGB-T semantic segmentation, addressing the scarcity of such data.
...
Method: UniTriGen, a unified triplet generation framework that uses a shared latent space and diffusion process to enforce global cross-modal consistency, along with modality-specific residual adapters and a scene-balanced few-shot sampling strategy.

Result: UniTriGen generates high-quality aligned triplets from limited real paired data, achieving consistent performance improvements across various RGB-T semantic segmentation models.

Conclusion: UniTriGen effectively addresses the challenge of generating aligned VIS-IR-Label triplets, enhancing the performance of RGB-T semantic segmentation models.
...
TL;DR: The paper introduces a new multi-label visual emotion analysis benchmark dataset to evaluate multimodal large language models (MLLMs) and assesses their performance on emotion prediction.


<details>
  <summary>Details</summary>
Motivation: Existing datasets have a suboptimal annotation scheme, showing each annotator a single candidate emotion per image, which underestimates MLLMs' capabilities.

Method: The authors create a new dataset with 20 annotators per image, asking them to select all emotions they feel, and aggregate votes to provide a reliable and representative dataset.

Result: The evaluation of recent MLLMs (Qwen3-VL, GPT, Gemini, Claude) shows progress but also substantial room for improvement in dominant emotion prediction and emotion distribution prediction.

Conclusion: The paper provides a new benchmark dataset for visual emotion analysis and highlights the limitations of current MLLMs and the LLM-as-a-judge method.
...
TL;DR: The paper introduces a synthetic dataset for evaluating Class Attribution Maps (CAMs) and proposes a new metric (ARCC) and a method (RefineCAM) for producing high-resolution attribution maps.


<details>
  <summary>Details</summary>
Motivation: The evaluation of CAMs is challenging due to the lack of ground-truth explanations, and existing CAM methods produce low-resolution attribution maps.

Method: The paper proposes a synthetic dataset, a new composite metric (ARCC), and a method (RefineCAM) that aggregates CAMs across multiple network layers to produce high-resolution attribution maps.

Result: RefineCAM consistently outperforms existing methods according to the proposed evaluation.

Conclusion: The paper provides a solution to the evaluation challenge of CAMs and proposes a method for producing high-resolution attribution maps, enabling more reliable and detailed interpretability.
...
TL;DR: This paper proposes an integrated framework combining state-of-the-art vision models with statistical modeling to improve the accuracy of water level detection and flow estimation.
...
Result: The result is an improved accuracy of water level detection and flow estimation.

Conclusion: The conclusion is that the proposed framework can improve the accuracy of water level detection and flow estimation, offering a more robust solution for vision-based water level and river surface velocity estimation.
...
TL;DR: A new dataset called TERRA-CD is presented for urban vegetation monitoring, comprising Sentinel-2 image pairs from 2019 and 2024 across 232 cities in the USA and Europe, with various annotation schemes and evaluated using deep learning approaches.


<details>
  <summary>Details</summary>
Motivation: The lack of comprehensive datasets for urban vegetation monitoring and understanding environmental changes.

Method: The TERRA-CD dataset is evaluated using various deep learning approaches including Siamese networks, STANet variants, Bi-SRNet, Changemask, Post-Classification Comparison, and HRSCD strategies.

Result: The effectiveness of the TERRA-CD dataset for both vegetation Multi-class Change Detection and Semantic Change Detection is demonstrated.

Conclusion: The proposed TERRA-CD dataset and methods provide a valuable resource for urban vegetation monitoring and change detection tasks.
...
TL;DR: A new framework called MiVE is proposed for reference-guided video editing, which leverages multiscale vision-language features to improve editing accuracy and preserve original motion.


<details>
  <summary>Details</summary>
Motivation: Existing methods for reference-guided video editing have limitations, including modality gaps and loss of fine-grained spatial details. The authors aim to address these limitations by developing a more effective framework.

Method: The MiVE framework uses a multiscale approach to extract hierarchical features from a vision-language model (Qwen3-VL) and integrates them into a unified self-attention Diffusion Transformer.

Result: MiVE achieves state-of-the-art performance in reference-guided video editing, outperforming both academic methods and commercial systems, and ranking highest in human preference.

Conclusion: The proposed MiVE framework effectively addresses the limitations of existing methods for reference-guided video editing, providing a more accurate and efficient approach for editing videos based on text instructions and reference images.
...
TL;DR: This paper proposes a streamlined active learning approach using CNNs and transformers with randomly initialized weights, eliminating the need for initial candidate models, and demonstrates its effectiveness with a low confidence (LC) sampling strategy.


<details>
  <summary>Details</summary>
Motivation: The reliance on extensive labeled data for deep learning models poses significant challenges, including resource constraints and annotation errors, and active learning aims to reduce labeling burdens.

Method: The study uses CNNs and transformers with randomly initialized weights and evaluates three confidence-based sampling strategies: high confidence (HC), low confidence (LC), and a combination of high confidence in early stages and low confidence in later stages (HCLC).

Result: The low confidence (LC) sampling strategy demonstrated the best performance, showcasing its effectiveness as an active learning strategy without the need for candidate models.

Conclusion: The proposed work introduces a streamlined approach to active learning, advancing efficiency and flexibility across diverse datasets and domains by eliminating the need for initial candidate models.
...
TL;DR: EponaV2, a novel driving world model, achieves high-quality planning with comprehensive future reasoning by forecasting future representations and enhancing real-world reasoning capabilities.


<details>
  <summary>Details</summary>
Motivation: The prevailing perception-planning paradigm in autonomous driving relies heavily on expensive manual annotations, limiting its scalability, while existing perception-free models struggle with comprehensive scene understanding.

Method: EponaV2 forecasts comprehensive future representations, decodes them to future geometry and semantic maps, and uses a flow matching group relative policy optimization mechanism to improve planning accuracy.

Result: EponaV2 achieves state-of-the-art performances on three NAVSIM benchmarks, with improvements of +1.3PDMS and +5.5EPDMS over existing perception-free models.

Conclusion: EponaV2 offers a novel paradigm for driving world models, enhancing real-world reasoning capabilities and planning accuracy, and demonstrating effectiveness in achieving high-quality planning.
...
TL;DR: The paper challenges the assumption that Composed Image Retrieval (CIR) requires multimodal composition, finding that many queries can be solved using a single modality, and current benchmarks overestimate model capabilities.
...
Method: The authors conducted a two-stage audit: first, identifying shortcut-solvable queries through cross-model analysis, and second, performing human validation on shortcut-free queries.

Result: The study found that a large fraction of queries can be solved using a single modality (32.2% to 83.6%), and re-evaluating models on validated subsets reveals qualitatively different behavior, requiring multimodal information for successful retrieval.

Conclusion: Current CIR benchmarks conflate shortcut-solvable, noisy, and genuinely compositional queries, leading to an overestimation of model capability in multimodal composition.
...
TL;DR: A framework for converting standard dynamic range (SDR) videos to high dynamic range (HDR) videos using large-scale generative video models.


<details>
  <summary>Details</summary>
Motivation: The problem of upconverting legacy SDR videos to HDR without a convincing solution, despite the HDR video ecosystem approaching maturity.

Method: Proposing a Multi-Exposure Video Model (MEVM) and a learnable Video Merging Model (VMM) to predict exposure-bracketed linear SDR video sequences and merge them into a high-quality HDR sequence.

Result: Robust HDR conversion for in-the-wild examples from casual consumer videos and iconic films, with extensive experiments, quantitative and qualitative evaluation, and a user study demonstrating the approach's effectiveness.

Conclusion: The proposed framework enables HDR synthesis from SDR footage, supporting HDR synthesis pipelines built upon existing SDR generative video models, with output HDR videos available for viewing.
...
### [78] [SceneFunRI: Reasoning the Invisible for Task-Driven Functional Object Localization](https://arxiv.org/abs/2605.14704)
*Posheng Chen,Powen Cheng,Gueter Josmy Faure,Hung-Ting Su,Winston H. Hsu*

Main category: cs.CV

TL;DR: The paper introduces SceneFunRI, a benchmark for reasoning about invisible objects in scenes, and evaluates vision-language models' ability to infer occluded object locations.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of inferring the locations of occluded objects, which humans can often do using context and commonsense knowledge, but remains difficult for vision-language models.

Method: The authors create a benchmark, SceneFunRI, based on the SceneFun3D dataset, and evaluate models using metrics such as CAcc@75, mIoU, and Dist. They also analyze prompting strategies.

Result: The strongest baseline model achieves poor results: CAcc@75 of 15.20, mIoU of 0.74, and Dist of 28.65, indicating that invisible-region reasoning is an unstable capability in current vision-language models.

Conclusion: The paper concludes that there is a need for future work on models that integrate task intent, commonsense priors, spatial grounding, and uncertainty-aware search to improve invisible-region reasoning.
...
TL;DR: The paper presents a method to create a direct sign-to-sign conversational model for Deaf and Hard-of-Hearing (DHH) signers, addressing the limitations of current conversational AI systems that rely on spoken or written language.


<details>
  <summary>Details</summary>
Motivation: The primary motivation is to provide equal access to conversational AI for DHH signers by developing a sign-to-sign conversational model, as existing systems can be limiting for those who primarily communicate through sign language.

Method: The approach involves constructing continuous sign conversations from isolated signs by collecting large-scale labeled isolated clips and recomposing them into sign-language-ordered utterances. It introduces SignaVox-W and SignaVox-U datasets and utilizes a retrieval-guided spoken-to-gloss translator and a diffusion Transformer called BRAID for duration alignment and co-articulatory boundary inpainting.

Result: The paper achieves improved isolated-to-continuous motion quality, stronger response-level semantic alignment, and scalable signer-centered interaction, supporting visual-spatial articulation without relying on spoken-language text or externally provided glosses.

Conclusion: The proposed method and datasets enable the development of SignaVox, a direct sign-to-sign conversational model that generates 3D body, hand, and facial motion responses, enhancing accessibility and interaction for DHH signers.
...
TL;DR: The paper proposes StyleTextGen, a novel framework for style-conditioned scene text generation that learns to perceive and replicate visual text styles across different languages and writing systems.
...
Method: The proposed framework features a dual-branch style encoder, a text style consistency loss, and a mask-guided inference strategy.

Result: Extensive experiments demonstrate that StyleTextGen significantly outperforms existing methods in style consistency and cross-lingual generalization, establishing new state-of-the-art performance in multilingual style-conditioned text generation.

Conclusion: The proposed StyleTextGen framework effectively addresses the challenges of style-conditioned scene text generation, achieving robust multilingual text style representations and establishing new state-of-the-art performance.
...
TL;DR: The paper proposes a novel framework for unified models to autonomously switch between generation strategies for anything-to-image tasks, addressing the understanding-generation gap.


<details>
  <summary>Details</summary>
Motivation: The 'understanding-generation gap' in unified models, where they struggle to translate semantic knowledge into precise pixel-level manipulation, particularly in complex prompts and imperfect feedback.
...
Result: The proposed method outperforms existing baselines on X2I tasks, achieving superior generation fidelity for simple-to-complex instructions.

Conclusion: The paper presents a novel framework that enables unified models to autonomously switch between generation strategies, effectively addressing the understanding-generation gap in anything-to-image tasks.
...
Conclusion: The proposed model effectively addresses the limitations of existing multi-modal approaches and improves ischemic stroke prognosis.
...
TL;DR: The paper identifies a problem in Vision-Language Models (VLMs) called cultural anachronism, where models misinterpret historical artifacts using inappropriate concepts. A benchmark (TAB-VLM) is introduced to evaluate temporal reasoning in VLMs, revealing significant deficiencies in state-of-the-art models.


<details>
  <summary>Details</summary>
Motivation: The increasing application of VLMs to cultural heritage materials and the need to accurately interpret historical artifacts, particularly for non-Western visual cultures underrepresented in training data.

Method: The introduction of the Temporal Anachronism Benchmark for Vision-Language Models (TAB-VLM), a dataset of 600 questions across six categories, to evaluate temporal reasoning on 1,600 Indian cultural artifacts.

Result: Systematic evaluations of ten state-of-the-art models reveal significant deficiencies on the benchmark, with even the best model (GPT-5.2) achieving only 58.7% overall accuracy.

Conclusion: The findings highlight the disparity between current VLM capabilities and the requirements for accurately interpreting cultural heritage materials, and provide a foundation for enhancing temporal cognition in multimodal AI systems.
...
TL;DR: A unified Deep Learning framework is proposed for label-free single-cell imaging, enabling White Blood Cell classification and protein-expression regression from Differential Phase Contrast images.


<details>
  <summary>Details</summary>
Motivation: Inferring molecular phenotypes directly from bright-field morphology remains challenging in label-free single-cell imaging, and there is a need for a scalable, non-invasive alternative to fluorescence-based cytometry.

Method: A Hybrid architecture that fuses convolutional fine-grained texture features with transformer-based global representations through a learnable cross-branch gating module, and incorporates a Large Language Model for generating concise, biologically grounded summaries of the predicted cell states.

Result: The model achieves a 91.3% White Blood Cell classification accuracy and a 0.72 Pearson correlation for CD16 expression regression on the Berkeley Single Cell Computational Microscopy benchmark.

Conclusion: The proposed framework underscores the promise of label-free single-cell imaging for cost-effective hematological profiling, enabling simultaneous phenotype identification and quantitative biomarker estimation without fluorescent staining.
...
TL;DR: The paper introduces MemEye, a framework for evaluating the memory capabilities of multimodal agents, focusing on preserving fine-grained visual evidence and reasoning over changing visual states.


<details>
  <summary>Details</summary>
Motivation: Existing evaluations of multimodal agents rarely test their ability to preserve visual evidence needed for later reasoning, and prior work often allows answers to be inferred without preserving fine-grained visual evidence.

Method: The MemEye framework evaluates memory capabilities across two dimensions: granularity of visual evidence and usage of retrieved evidence, and a new benchmark is constructed across 8 life-scenario tasks.

Result: Current architectures struggle to preserve fine-grained visual details and reason about state changes over time, and long-term multimodal memory depends on evidence routing, temporal tracking, and detail extraction.

Conclusion: The paper highlights the need for better evaluation and development of multimodal agents' memory capabilities, particularly in preserving visual evidence and reasoning over changing visual states.
...
TL;DR: The paper proposes CHASM, a Cross-frequency Harmonized Axis-Separable Mixer, which improves spectral token mixers for modeling global interactions in visual feature maps.


<details>
  <summary>Details</summary>
Motivation: Existing spectral token mixers have limitations, such as applying filter-wise spectral responses along fixed channel axes or learning adaptive frequency-indexed channel mixing without aligning channel directions.

Method: The proposed CHASM separates shared and frequency-specific components, using a learned channel eigenbasis and positive spectral gains, and applies this operator separably along height and width axes.

Result: CHASM consistently improves over same-backbone spectral-mixer baselines across various applications, including accelerated MRI reconstruction, undersampled MRI segmentation, and natural-image reconstruction.

Conclusion: The proposed CHASM provides a structured and effective approach to spectral token mixers, and the ablation studies support the usefulness of cross-frequency harmonization as an inductive bias.
...
### [87] [ATLAS: Agentic or Latent Visual Reasoning? One Word is Enough for Both](https://arxiv.org/abs/2605.15198)
*Ziyu Guo,Rain Liu,Xinyan Chen,Pheng-Ann Heng*

Main category: cs.CV

TL;DR: The paper proposes ATLAS, a framework for visual reasoning that combines the strengths of agentic and latent methods while mitigating their limitations.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of existing visual reasoning methods, including computational expense, context-switching latency, and difficulty in training with autoregressive parallelization.

Method: The paper proposes ATLAS, a framework that uses a single discrete 'word' (functional token) that serves as both an agentic operation and a latent visual reasoning unit.

Result: The results show that ATLAS achieves superior performance on challenging benchmarks while maintaining clear interpretability.

Conclusion: The paper concludes that ATLAS offers a new paradigm for visual reasoning research, providing a promising direction for future studies.
...
TL;DR: The authors propose Video-Zero, a framework for self-evolution of video understanding models without human annotation, focusing on temporally localized evidence.


<details>
  <summary>Details</summary>
Motivation: The authors aim to improve video understanding models through self-evolution, addressing the challenges of sparse and temporally localized evidence in videos.

Method: The authors propose a Questioner--Solver co-evolution framework, Video-Zero, which centers self-evolution on temporally localized evidence, generating evidence-grounded questions and answers.

Result: Video-Zero consistently improves multiple video VLM backbones across 13 benchmarks, demonstrating effectiveness and transferability.

Conclusion: The authors conclude that grounding is the key bottleneck in video self-evolution, and their proposed framework, Video-Zero, effectively addresses this challenge.
...
### [89] [EARL: Towards a Unified Analysis-Guided Reinforcement Learning Framework for Egocentric Interaction Reasoning and Pixel Grounding](https://arxiv.org/abs/2605.14742)
*Yuejiao Su,Xinshen Zhang,Zhen Ye,Lei Yao,Lap-Pui Chau,Yi Wang*

Main category: cs.CV

TL;DR: The paper introduces EARL, a framework that enables better understanding of human-environment interactions from egocentric vision, improving assistive robotics and intelligent agents.
...
Method: EARL uses a two-stage parsing framework with coarse-grained interpretation and fine-grained response, incorporating a novel Analysis-guided Feature Synthesizer (AFS) and a multi-faceted reward function with GRPO.
...
Conclusion: EARL effectively improves the understanding of human-environment interactions from egocentric vision, enhancing the capabilities of assistive robotics and embodied intelligent agents.
...
### [90] [BioHuman: Learning Biomechanical Human Representations from Video](https://arxiv.org/abs/2605.14772)
...
<details>
  <summary>Details</summary>
Motivation: The lack of large-scale datasets with biomechanical annotations and existing approaches that cannot infer internal biomechanical states from visual observations limit progress in motion analysis, rehabilitation, and injury risk assessment.

Method: A simulation-based framework for estimating muscle activations from existing motion capture datasets, and an end-to-end model (BioHuman) that takes monocular video as input and jointly predicts human motion and muscle activations.
...
Conclusion: The proposed approach establishes a new benchmark for video-based biomechanical understanding and opens up new possibilities for physically grounded human modeling.
...
### [91] [MonoPRIO: Adaptive Prior Conditioning for Unified Monocular 3D Object Detection](https://arxiv.org/abs/2605.14781)
...
<details>
  <summary>Details</summary>
Motivation: Monocular 3D object detection is challenging due to metric size and depth being underdetermined by single-view evidence, especially under occlusion and truncation.
...
Result: MonoPRIO achieves the strongest unified multi-class result on the KITTI test server and the strongest 3D bounding-box AP in the car-only setting without extra data.

Conclusion: The paper concludes that adaptive priors are effective in improving metric size stability, especially in ambiguity-prone and partially occluded regimes.
...
### [92] [COAL: Counterfactual and Observation-Enhanced Alignment Learning for Discriminative Referring Multi-Object Tracking](https://arxiv.org/abs/2605.14795)
*Shukun Jia,Shiyu Hu,Yipei Wang,Ximeng Cheng,Yichao Cao,Xiaobo Lu*

Main category: cs.CV

TL;DR: The paper proposes COAL, a framework that enhances Referring Multi-Object Tracking (RMOT) by addressing the contradiction between high discriminability demands and sparse semantic supervision.


<details>
  <summary>Details</summary>
Motivation: The motivation is to resolve the structural contradiction in RMOT between high-discriminability demand and sparse semantic supervision, particularly in highly homogeneous scenarios.

Method: The method proposed is COAL (Counterfactual and Observation-enhanced Alignment Learning), which includes Explicit Semantic Injection (ESI) via a VLM and Counterfactual Learning (CFL) leveraging LLM reasoning, unified within a Hierarchical Multi-Stream Integration (HMSI) architecture.

Result: The results show that COAL surpasses the state-of-the-art by 7.28% HOTA on the Refer-KITTI-V2 benchmark, validating its efficacy in resolving the sparsity-discriminability paradox in RMOT.

Conclusion: The conclusion is that COAL effectively enhances RMOT by advancing beyond isolated structural optimization through knowledge regularization, demonstrating the effectiveness of this approach.
...
### [93] [Can Visual Mamba Improve AI-Generated Image Detection? An In-Depth Investigation](https://arxiv.org/abs/2605.14799)
*Mamadou Keita,Wassim Hamidouche,Hessen Bougueffa Eutamene,Abdelmalik Taleb-Ahmed,Xianxun Zhu,Abdenour Hadid*

Main category: cs.CV

TL;DR: This paper evaluates Vision Mamba models for detecting AI-generated images, comparing them to CNNs, ViTs, and VLMs across diverse datasets and metrics.


<details>
  <summary>Details</summary>
Motivation: The rapid progress in image generation raises concerns about misuse, making it crucial to develop effective methods for distinguishing between real and AI-generated content.
...
Conclusion: This research enhances detection capabilities in an age where distinguishing between real and AI-generated content is a major challenge, and highlights the potential of Vision Mamba models for this task.
...
<details>
  <summary>Details</summary>
Motivation: Visual anomaly detection for industrial inspection faces challenges due to changes in acquisition conditions during production, requiring models to remain robust under distribution shifts.
...
Result: Achieved segmentation F1 scores of 62.61%, 57.42%, and 54.35% on test public, private, and private mixed of MVTec AD 2, outperforming SuperAD and state-of-the-art methods.

Conclusion: The proposed method offers a well-suited approach for industrial deployment, handling product variants and appearance changes with minimal adaptation effort.
...
TL;DR: The paper presents a method for camera control in video generation models by treating it as geometric guidance, inducing displacements across frames through differentiable resampling of latent features.


<details>
  <summary>Details</summary>
Motivation: Existing approaches to camera control in video generation models are limited by the scale, diversity, and scene dynamics of paired datasets, which can bias the model towards a narrow output distribution.

Method: The authors reformulate camera control into a set of displacement fields and apply them via differentiable resampling of latent features during denoising.

Result: The approach achieves effective camera control with minimal degradation across diverse quality metrics compared to fine-tuned baselines and can be used to study the camera control capabilities of base models.

Conclusion: The paper offers a simple and effective method for camera control in video generation models, which can also serve as a probe to study the camera control capabilities of base models and benchmark their performance in multi-view generation.
...
TL;DR: The paper identifies a problem called the Velocity Deficit in Flow Matching, proposes two solutions (MAFM and SSC), and demonstrates significant improvements in efficiency and FID scores on ImageNet-1k and MS-COCO.


<details>
  <summary>Details</summary>
Motivation: The authors are motivated to address the Velocity Deficit issue in Flow Matching, which causes generated samples to fail to reach the data manifold, resulting in poor performance.

Method: The authors propose two methods: Magnitude-Aware Flow Matching (MAFM) and Scale Schedule Corrector (SSC). MAFM is a training-based approach, while SSC is a training-free approach.

Result: The authors achieve significant improvements in FID scores and efficiency on ImageNet-1k (256x256) and MS-COCO. Specifically, SSC improves FID by 44.6% on ImageNet-1k and by ~22% on MS-COCO.

Conclusion: The authors conclude that their proposed methods can effectively address the Velocity Deficit issue in Flow Matching, leading to improved performance and efficiency in generative modeling tasks.
...
TL;DR: The paper proposes HDRFace, a framework for face restoration under complex degradations, leveraging high-dimensional representations and a novel fusion mechanism.
...
Method: The proposed HDRFace framework uses a structurally reliable intermediate restoration, high-dimensional feature encoder, and a novel Structure-Detail aware adaptive Fusion Mechanism (SDFM).

Result: The method shows stable and coherent performance gains across different architectures, specifically on SD V2.1-base and Qwen-Image generative models.

Conclusion: The HDRFace framework effectively addresses face restoration under complex degradations by injecting semantically rich priors and balancing structural consistency and detail fidelity.
...
TL;DR: A novel weakly-supervised method for Video Moment Retrieval (VMR) is proposed, which generates multiple proposals and uses multi-task training to improve retrieval performance.


<details>
  <summary>Details</summary>
Motivation: Previous weakly-supervised VMR methods produce low-quality temporal proposals, struggle with distinguishing misaligned moments, or lack stability due to reliance on a single auxiliary task.

Method: The proposed method, Multi-proposal Collaboration and Multi-task Training (MCMT), generates multiple proposals, creates a high-quality positive sample mask, and uses forward and inverse masked query reconstruction tasks during training.

Result: Extensive experiments on two standard benchmarks affirm the effectiveness of the proposed method in VMR.

Conclusion: The proposed MCMT method addresses the limitations of previous weakly-supervised VMR methods and improves retrieval performance.
...
TL;DR: The paper introduces a new benchmark and framework for abstract image editing, where models are tasked with making edits based on abstract instructions like 'change the mood' of an image.


<details>
  <summary>Details</summary>
Motivation: Current image editing benchmarks focus on explicit, literal commands, but humans often communicate through abstract concepts. The authors aim to bridge this gap by exploring abstract image editing.

Method: The authors formalize the definition and taxonomy of abstract image editing, introduce the Entity-Rubrics framework for assessing instruction-following, and create the AbstractEdit benchmark.

Result: Evaluating 11 leading models on AbstractEdit reveals that they struggle to balance intent and preservation, often under-editing or over-editing. Advanced LLM text encoders and iterative thinking are necessary for meaningful improvements.

Conclusion: The work serves as a stepping stone toward seamless multimodal interaction, enabling models to correctly interpret abstract communication and closing the gap between machine execution and human communication.
...
TL;DR: The paper introduces MechVerse, a benchmark for mechanically consistent image-to-video generation, and evaluates current models' ability to generate kinematic and geometrically constrained motion.


<details>
  <summary>Details</summary>
Motivation: Current text- and image-conditioned video generation models often fail to generate motion governed by kinematic and geometric constraints, particularly in articulated mechanical assemblies.

Method: The authors introduce MechVerse, a benchmark containing 21,156 synthetic clips from 1,357 mechanical assemblies, and evaluate proprietary, open-source, and fine-tuned image-to-video models using standard video metrics, instruction-following scores, and human judgments.

Result: Current models can preserve appearance and smoothness but fail to generate mechanically admissible motion, with errors increasing as coupling complexity grows.
...
TL;DR: This paper explores the use of Vision-Language Models (VLMs) for biometric tasks, specifically signature verification, and evaluates their zero-shot performance on the Signature Verification Challenge (SVC) benchmark.


<details>
  <summary>Details</summary>
Motivation: The motivation is to investigate the applicability of VLMs to rigorous biometric tasks, particularly signature verification, and to assess their performance in zero-shot settings.

Method: The method involves converting raw kinematic time-series into static images, using a scoring protocol that extracts latent token probabilities, and evaluating the performance of state-of-the-art VLMs (GPT-5.2 and Gemini 2.5 Pro) on the SVC benchmark.

Result: The results show that VLMs achieve exceptional performance in random forgery scenarios, with GPT-5.2 reaching an Equal Error Rate of 0.32% in mobile tasks, but perform significantly worse in skilled forgery scenarios, where a 'Rationalization Trap' emerges, causing chain-of-thought reasoning to degrade performance.

Conclusion: The conclusion is that while VLMs demonstrate strong capabilities in general visual reasoning, their performance on biometric tasks like signature verification is highly dependent on signal quality and forgery type, and that careful consideration is needed to mitigate the 'Rationalization Trap' in skilled forgery scenarios.
...
TL;DR: The paper proposes a new approach to evaluate image super-resolution artifacts based on their perceptual impact, introducing the concept of artifact prominence and releasing a dataset suite called SR-Prominence.


<details>
  <summary>Details</summary>
Motivation: Existing image super-resolution methods often introduce visual artifacts that degrade perceived quality, but current detection methods treat all artifacts equally, ignoring their varying perceptual impact.

Method: The authors design a crowdsourced annotation protocol to construct SR-Prominence, a dataset suite containing annotated artifact masks from various datasets, and audit SR artifact detectors, image-quality metrics, and SR methods.
...
Conclusion: The paper enables SR artifact evaluation to move from binary defect presence toward perceptual impact, providing a dataset suite and objective scoring protocol for benchmarking new metrics.
...
TL;DR: The paper proposes FactorizedHMR, a two-stage framework for Human Mesh Recovery (HMR) that handles ambiguity in 3D body reconstruction by separately processing the torso and distal articulations.
...
Method: The proposed method, FactorizedHMR, consists of a deterministic regression module that recovers a stable torso-root anchor, followed by a probabilistic flow-matching module that completes the remaining non-torso articulation.

Result: The results show that FactorizedHMR remains competitive with strong baselines across various benchmarks, with notable gains in occlusion-heavy recovery and drift-sensitive world-space metrics.

Conclusion: The authors conclude that FactorizedHMR effectively addresses the ambiguity in HMR by treating the torso and distal articulations differently, leading to improved performance in challenging scenarios.
...
TL;DR: The paper proposes LPH-VTON, a new framework for Virtual Try-On (VTON) that balances structural integrity and textural fidelity in synthesizing photorealistic images of garments on a person's body.


<details>
  <summary>Details</summary>
Motivation: Current diffusion-based VTON methods face a trade-off between structural integrity and textural fidelity due to complementary inductive biases in prevailing architectures.

Method: LPH-VTON uses a synergistic framework with a structure-biased model for geometric alignment and a texture-biased model for high-fidelity detail rendering in a continuous denoising process.

Result: LPH-VTON achieves a superior Pareto-optimal balance, establishing new benchmarks in perceptual faithfulness while maintaining competitive structural alignment on the VITON-HD dataset.

Conclusion: The proposed LPH-VTON framework effectively resolves the tension between structural integrity and textural fidelity in VTON, proving the efficacy of temporal architectural decoupling.
...
TL;DR: The paper proposes a Closed-Loop Visual Reasoning (CLVR) framework to improve text-to-image models by coupling visual-language planning with pixel-level diffusion generation, addressing issues with complex semantics and inference latency.


<details>
  <summary>Details</summary>
Motivation: Current text-to-image models struggle with complex semantics and face diminishing returns from parameter scaling, and recent multi-step reasoning approaches are hindered by several bottlenecks.

Method: The CLVR framework introduces an automated data engine with step-level visual verification, Proxy Prompt Reinforcement Learning (PPRL) to resolve long-context optimization instabilities, and $Δ$-Space Weight Merge (DSWM) to mitigate latency.

Result: CLVR outperforms existing open-source baselines across multiple benchmarks and approaches the performance of proprietary commercial models, unlocking general test-time scaling capabilities for complex visual generation.

Conclusion: The proposed CLVR framework effectively addresses the bottlenecks in current text-to-image models, providing a comprehensive system for improved visual generation capabilities.
...
TL;DR: A novel method called HeatKV is proposed to compress KV-cache in Visual Autoregressive (VAR) models, achieving a 2x higher compression ratio while maintaining image fidelity.


<details>
  <summary>Details</summary>
Motivation: VAR models suffer from severe KV-cache memory constraints, requiring gigabytes of memory per generated image, and existing compression methods are not sufficient.

Method: HeatKV adapts cache allocation in each head based on its attention to previously generated scales, using a small offline calibration set to rank attention heads and construct a static pruning schedule.
...
Conclusion: HeatKV is a effective method for VAR model KV-cache compression, achieving a new state-of-the-art (SOTA) and showcasing the effectiveness of fine-grained, head-specific cache allocation.
...
TL;DR: Denoising-GS, a spatial-aware denoising framework for 3D Gaussian Splatting, is proposed to optimize noisy Gaussian primitives by considering both positions and spatial structure, achieving state-of-the-art Novel View Synthesis performance.


<details>
  <summary>Details</summary>
Motivation: The optimization process of 3D Gaussian Splatting inevitably introduces noisy Gaussian primitives due to sparse and incomplete initialization from Structure-from-Motion point clouds.

Method: Denoising-GS, a spatial-aware denoising framework, which includes an optimizer that preserves spatial optimization flow, Spatial Gradient-based Denoising strategy, Uncertainty-based Denoising module, and Spatial Coherence Refinement strategy.

Result: Denoising-GS consistently enhances Novel View Synthesis fidelity while maintaining representation compactness, achieving state-of-the-art performance across all benchmarks.

Conclusion: The proposed Denoising-GS framework effectively optimizes noisy Gaussian primitives by considering spatial structure, leading to improved Novel View Synthesis performance.
...
TL;DR: A new self-supervised framework, Masked Next-Scale Prediction (MNSP), is proposed for Scene Text Recognition, which models cross-scale structural evolution and achieves state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Scene Text Recognition requires modeling visual structures that evolve from coarse layouts to fine-grained character strokes, but existing self-supervised approaches operate at a single spatial scale and fail to capture the hierarchical nature of scene text.

Method: The proposed MNSP framework incorporates Next-Scale Prediction (NSP) and masked image reconstruction to jointly learn cross-scale prediction and local constraints, and a Multi-scale Linguistic Alignment module to maintain semantic consistency across different resolutions.
...
Conclusion: The proposed MNSP framework effectively models cross-scale structural evolution in Scene Text Recognition, achieving superior performance and robustness compared to existing approaches.
...
TL;DR: SurgicalMamba, a causal surgical phase recognition model, achieves state-of-the-art accuracy and efficiency in online surgical phase recognition.


<details>
  <summary>Details</summary>
Motivation: Existing recognizers fail to address three demands of surgical video: long procedures, non-uniform time flow, and narrow visual domain with correlated features.

Method: SurgicalMamba uses Mamba2's structured state-space duality (SSD) and introduces three SSD-compatible components: dual-path SSD block, intensity-modulated stepping, and state regramming.

Result: SurgicalMamba reaches state-of-the-art accuracy and phase-level Jaccard on seven public SPR benchmarks, with 94.6%/82.7% on Cholec80 and 89.5%/68.9% on AutoLaparo, at 119 fps on a single GPU.

Conclusion: SurgicalMamba effectively addresses the demands of surgical video and achieves efficient and accurate online surgical phase recognition.
...
TL;DR: A new multi-scale Image Super Resolution (ISR) method using Visual Auto-Regressive (VAR) modeling with two novel components: Hierarchical Image Tokenization (HIT) and Direct Preference Optimization (DPO) regularization term.


<details>
  <summary>Details</summary>
Motivation: Previous ISR methods using VAR modeling have limitations, including generating images at a fixed scale and relying on large models or annotated data.

Method: The proposed method introduces HIT for progressive image representation and DPO regularization term to encourage the transformer to produce high-resolution images.

Result: The proposed method achieves state-of-the-art results without external training data, with a small model (300M params) delivering multi-scale outputs with a single forward pass.

Conclusion: The proposed method improves upon previous ISR methods by increasing flexibility and reducing complexity, achieving state-of-the-art results with a smaller model.
...
TL;DR: The authors analyze the latent space of Vision-Language Models (VLMs) and find that it contains a significant amount of shared noise, which can be removed without harming performance.


<details>
  <summary>Details</summary>
Motivation: The authors are motivated by the observation that VLMs' shared latent spaces contain structural anomalies and non-semantic noise.

Method: The authors use spectral decomposition of covariance matrices to decompose the VLM latent space into a semantic signal component and a shared noise subspace.

Result: The authors find that pruning the shared noise dimensions preserves or improves downstream task performance, and that a substantial fraction of the latent geometry is governed by shared noise rather than task-relevant semantics.

Conclusion: The authors conclude that their work provides new mechanistic insights into the representational structure of modern VLMs, highlighting the importance of distinguishing between semantic signals and shared noise.
...
TL;DR: SEDiT, a novel one-stage video Subtitle Erasure approach via One-step Diffusion Transformer, enables direct and efficient removal of targeted subtitles from videos without requiring a mask.
...
Method: SEDiT uses a mask-free inference approach, a one-stage framework, and a hybrid training strategy with a clean first-frame latent to ensure temporal continuity and efficient handling of long videos.

Result: SEDiT can efficiently handle native 1440p video with infinite length, and it provides a reliable and theoretically justified one-step denoising approach for subtitle removal.

Conclusion: SEDiT presents a novel approach for video subtitle erasure, offering a one-stage, mask-free, and efficient solution that mitigates the limitations of existing methods and supports high-resolution videos.
...
<details>
  <summary>Details</summary>
Motivation: The need for a systematic comparison of long-context LVLMs and memory-augmented agents on questions requiring multimodal evidence, as existing benchmarks do not adequately assess this.

Method: The paper proposes MEMLENS, a comprehensive benchmark with 789 questions across five memory abilities and four context lengths, and evaluates 27 LVLMs and 7 memory-augmented agents.

Result: Long-context LVLMs perform well in short-context scenarios but degrade as conversations grow, while memory agents are length-stable but lose visual fidelity. Neither approach alone solves multi-session reasoning tasks.

Conclusion: The results motivate the development of hybrid architectures combining long-context attention with structured multimodal retrieval to improve performance in multimodal multi-session conversations.
...
TL;DR: SteerSeg improves video reasoning segmentation by enhancing attention maps from large vision-language models for better spatial localization.


<details>
  <summary>Details</summary>
Motivation: Current methods using attention maps from large vision-language models for video reasoning segmentation are limited by their optimization for text generation, not spatial localization.

Method: SteerSeg uses learnable soft prompts and Chain-of-Thought prompting to steer attention towards spatially concentrated and unambiguous regions, guiding a segmentation model.

Result: SteerSeg significantly improves the spatial grounding capability of LVLMs and generalizes well across diverse benchmarks.

Conclusion: SteerSeg offers a lightweight framework that enhances the grounding capabilities of frozen large vision-language models for video reasoning segmentation.
...
TL;DR: A new linear attention mechanism called Representative Attention (RPAttention) is proposed, which compresses tokens in representation space, enabling semantically related regions to communicate regardless of spatial distance.


<details>
  <summary>Details</summary>
Motivation: Existing linear attention methods compress spatial tokens into intermediate proxies based on predefined spatial layouts, which can be limited by not being aligned with the semantic organization of visual content.

Method: RPAttention uses a Gather-Interact-Distribute paradigm to dynamically form a compact set of learned representative tokens, enabling global interaction within a compact latent space and adaptive token communication.

Result: RPAttention reduces token interaction complexity from quadratic to linear scaling while maintaining expressive global context modeling, and demonstrates effectiveness across diverse vision transformer backbones on image classification, object detection, and semantic segmentation.

Conclusion: The proposed RPAttention mechanism offers a more effective and efficient way to scale Vision Transformers by aligning token communication with the content structure of each input.
...
TL;DR: The paper introduces Hierarchical Scene Parsing, a task that represents physical scenes as hierarchies of scenes, objects, parts, and affordances, and proposes SceneParser, a VLM-based parser that achieves strong performance on this task.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the gap in current scene perception capabilities, which often focus on isolated predictions rather than capturing structured dependencies for interaction-oriented scene understanding.
...
Conclusion: The paper concludes that SceneParser provides an actionable representation for visual understanding and can be used for interaction-oriented scene understanding, with potential applications in downstream planning and decision-making tasks.
...
TL;DR: The paper introduces GeoFuse, a cross-modal fusion framework that integrates road map data with satellite imagery to improve drone-view geo-localization under adverse weather conditions.
...
Method: The authors propose a flexible fusion module that combines satellite and road map features via token-level and channel-level interactions, with a lightweight dynamic gating mechanism that adaptively weights modality contributions per instance. They also employ class-level cross-view contrastive learning to promote robust alignment between weather-degraded drone features and the fused satellite-roadmap representations.

Result: GeoFuse consistently outperforms state-of-the-art methods, achieving +3.46% and +23.18% Recall@1 accuracy on the University-1652 and DenseUAV benchmarks, respectively.

Conclusion: The paper concludes that integrating road map data with satellite imagery can yield more discriminative and weather-resilient representations for drone-view geo-localization, and that GeoFuse is an effective framework for achieving this goal.
...
TL;DR: Proposed SCRWKV network achieves high-precision crack segmentation with low resource demands.
...
Method: The SCRWKV network uses a novel Structure-Field Encoder (SFE) backbone with Adaptive Multi-scale Cascaded Modulator (AMCM) and Structure-Calibrated Insight Unit (SCIU) to enhance texture representation and capture topological correlations.

Result: SCRWKV outperforms SOTA methods with 1.22M parameters, achieving an F1 score of 0.8428 and mIoU of 0.8512 on the TUT dataset.

Conclusion: The proposed SCRWKV network demonstrates robust potential for efficient real-world deployment in crack segmentation tasks.
...
TL;DR: MSCoT is a multi-scale model for human motion synthesis and control that achieves state-of-the-art performance with faster sampling and better motion quality.


<details>
  <summary>Details</summary>
Motivation: The authors aim to improve human motion synthesis and control by addressing the limitations of existing approaches, such as multiple iterative denoising steps and tailored modules for specific control signals.

Method: MSCoT uses a coarse-to-fine paradigm with a multi-scale hierarchical representation, token guidance strategy, and a lightweight token refiner for continuous residuals and differentiable test-time refinement optimization.

Result: MSCoT achieves state-of-the-art performance on popular benchmarks with 48% FID improvement, -61% avg error, and 10x faster inference speed on HumanML3D.

Conclusion: MSCoT offers a more efficient and effective approach to human motion synthesis and control, producing high-quality motions consistent with control constraints.
...
TL;DR: Proposed ACE-LoRA, a dynamic regularization framework for continual image editing that mitigates catastrophic forgetting.


<details>
  <summary>Details</summary>
Motivation: Real-world applications require continual adaptation to new image editing tasks while preserving previously learned knowledge.

Method: ACE-LoRA uses Adaptive Orthogonal Decoupling and Rank-Invariant Historical Information Compression for continual image editing.

Result: ACE-LoRA outperforms existing baselines in instruction fidelity, visual realism, and robustness to forgetting.

Conclusion: ACE-LoRA establishes a strong foundation for continual learning in image editing.
...
TL;DR: The study presents AtheroFlow-XNet, a deep learning model for carotid intima-media segmentation and risk prediction using ultrasound morphology and uncertainty-aware learning.


<details>
  <summary>Details</summary>
Motivation: Conventional ultrasound assessment of carotid atherosclerosis may not fully capture patient-specific vascular risk, and the study aims to develop a more comprehensive approach.

Method: The study uses a supervised segmentation approach with manual lumen-intima and media-adventitia boundary annotations, and incorporates clinical variables into an auxiliary risk-prediction branch.

Result: The proposed model achieved a Dice coefficient of 0.7930 for LI-MA mask segmentation, a segmentation loss of 0.2359, and an area under the receiver operating characteristic curve of 0.6910 for preliminary risk prediction.

Conclusion: The study suggests that ultrasound-derived carotid morphology can support automated wall analysis and uncertainty-aware interpretation, and proposes a reproducible morphology-driven baseline for future work.
...
TL;DR: Evo-Depth is a lightweight depth-enhanced VLA framework for robotic manipulation that improves spatial understanding without extra sensors or large models.


<details>
  <summary>Details</summary>
Motivation: Current Vision-Language-Action models struggle with precise spatial understanding due to 2D visual representations lacking depth information and detailed spatial relationships.

Method: Evo-Depth uses a lightweight Implicit Depth Encoding Module and a Spatial Enhancement Module to enhance spatially grounded manipulation from multi-view RGB images.

Result: Evo-Depth achieves superior performance across four simulation benchmarks and the highest average success rate in real-world experiments with a small model size and low computational cost.

Conclusion: Evo-Depth offers an efficient and effective solution for enhancing spatial understanding in robotic manipulation without relying on additional sensing hardware or large models.
...
TL;DR: Proposed H-OmniStereo, a zero-shot framework for omnidirectional stereo matching, leveraging a large synthetic dataset and a novel monocular normal estimator.


<details>
  <summary>Details</summary>
Motivation: The scarcity of omnidirectional stereo datasets and the degradation of perspective monocular priors under spherical distortions limit the performance of stereo matching on top-bottom equirectangular images.
...
Conclusion: H-OmniStereo provides an effective solution for full-surround perception with stereo matching, validated through extensive experiments.
...
TL;DR: The paper proposes MHSA, a framework that mitigates hallucinations in large vision-language models by correcting cross-modal attention patterns.


<details>
  <summary>Details</summary>
Motivation: Large vision-language models suffer from hallucinations, generating content inconsistent with visual input, and prior work only detected but did not mitigate hallucinations.

Method: MHSA uses a three-layer MLP generator to produce corrected attention, guided by supervisory signals from a discriminator and the model itself.

Result: MHSA mitigates hallucinations across various datasets and models without modifying model parameters.

Conclusion: MHSA offers a novel perspective on hallucination research and enhances the reliability of large vision-language models.
...
<details>
  <summary>Details</summary>
Motivation: Manual analysis of microscopy images is costly and time-consuming, while deep learning-based approaches require extensive adaptation for different settings, creating a need for a broadly applicable tool.
...
TL;DR: Sat3DGen is proposed to generate street-level 3D scenes from a single satellite image with improved geometric accuracy and photorealism.


<details>
  <summary>Details</summary>
Motivation: Current methods for generating 3D scenes from satellite images have limitations, including a trade-off between geometric fidelity and semantic diversity, and struggles with viewpoint gaps and sparse supervision.

Method: Sat3DGen uses a geometry-first methodology, integrating novel geometric constraints and a perspective-view training strategy to improve geometric accuracy and photorealism.

Result: Sat3DGen achieves a geometric RMSE of 5.20m, improving upon 6.76m from existing methods, and reduces Fréchet Inception Distance (FID) from ~40 to 19, demonstrating improved photorealism.

Conclusion: Sat3DGen effectively addresses challenges in generating street-level 3D scenes from satellite images, offering improved geometric accuracy, photorealism, and versatility in downstream applications.
...
TL;DR: A method called CVG is proposed to improve the compositional faithfulness of text-to-video diffusion models without modifying the model architecture or requiring additional user-supplied controls.


<details>
  <summary>Details</summary>
Motivation: Text-to-video diffusion models often fail to understand fine-grained compositional concepts, such as relations between entities, attributes, actions, and motion directions.

Method: CVG uses a lightweight compositional classifier trained on cross-attention features to steer the denoising process in frozen text-to-video models.

Result: CVG improves compositional generation in text-to-video benchmarks while preserving visual quality.

Conclusion: The proposed method, CVG, effectively mitigates the failures of text-to-video diffusion models on compositional understanding tasks without requiring retraining or additional controls.
...
TL;DR: The study analyzes the visual experiences of young children to understand how they acquire object category representations, finding that their exposure is highly skewed and variable, yet still able to form strong superordinate category groupings.
...
Result: Children's object category exposure is highly skewed, with category exemplars being highly variable, yet detected categories show stronger groupings within superordinate categories.

Conclusion: Understanding visual category learning requires models that can exploit strong superordinate structure and learn from non-canonical, sparse, and variable exemplars.
...
TL;DR: A non-invasive deep learning framework is proposed to predict ovarian cancer patients' response to neoadjuvant chemotherapy using pre-treatment contrast-enhanced CT scans.


<details>
  <summary>Details</summary>
Motivation: Early identification of non-responders to neoadjuvant chemotherapy is crucial to prevent ineffective therapy and delays in optimal surgical management.

Method: The framework uses a partially fine-tuned pretrained image encoder to encode axial slices and an attention-based module to aggregate slice-level representations into a volumetric embedding.
...
TL;DR: The paper proposes Skew-Normal Splatting (SNS), a novel representation for 3D scenes that improves upon 3D Gaussian Splatting (3DGS) by using a skewed normal distribution, allowing for more flexible modeling of asymmetric structures.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of 3D Gaussian Splatting (3DGS) in representing asymmetric structures, such as object boundaries and one-sided surfaces, by introducing a more flexible and continuous shape control.

Method: The method proposed is Skew-Normal Splatting (SNS), which uses the Azzalini Skew-Normal distribution as the fundamental primitive, introducing a learnable and bounded skewness parameter to continuously interpolate between symmetric Gaussians and Half-Gaussian-like shapes.
...
Conclusion: The conclusion is that SNS provides a more flexible and accurate representation of 3D scenes, particularly in asymmetric structures, and can be seamlessly integrated into existing Gaussian Splatting rasterization pipelines.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for remote sensing image change captioning are limited by a shared modeling assumption that changed and unchanged image pairs are processed under a unified strategy, leading to semantic entanglement.
...
<details>
  <summary>Details</summary>
Motivation: Long-form animation is challenging due to the synthesis of dynamic human motion against static environments, leading to accumulated drift in chunk-based generation.
...
Conclusion: EverAnimate effectively addresses the challenges of long-horizon animated video generation, providing a more efficient and high-quality solution.
...
TL;DR: The paper proposes LATERN, a context-aware framework for video anomaly detection using vision-language models, which improves detection accuracy and explanation consistency.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the limitation of current vision-language models for video anomaly detection, which perform segment-level inference independently without structured temporal context.

Method: The proposed framework LATERN consists of two modules: Context-Aware Anomaly Scoring (CEA) and Recursive Evidence Aggregation (REA), which reformulate video anomaly detection as a temporal evidence aggregation process.

Result: Extensive experiments on UCF-Crime and XD-Violence benchmarks show that LATERN enhances detection accuracy and explanation consistency for frozen vision-language models during test time.

Conclusion: The proposed LATERN framework improves video anomaly detection by providing temporally coherent and semantically grounded event-level explanations.
...
TL;DR: The study evaluates a Monte Carlo-inspired analytic model to improve the performance of RGB-trained capsule-endoscopy classifiers on small-vessel vascular findings. The model provides a small but consistent improvement in macro-AUC across 6 seeds, with the largest lift on Lymphangiectasia.
...
Method: The study uses a Monte Carlo-inspired analytic model to compute hemoglobin from RGB signal, evaluating two software-only configurations: (i) fusing prior P_blood as 2 zero-init auxiliary channels, and (ii) distillation head training a 3-channel RGB backbone to predict P_blood.

Result: The analytic prior provides a small but direction-consistent macro-AUC improvement: RGB-only 0.760 +/- 0.027, input-fusion 0.783 +/- 0.024 (paired Delta = +0.023). The largest robust per-class lift is on Lymphangiectasia, where AUC rises from RGB 0.238 +/- 0.057 to input-fusion 0.337 +/- 0.019.
...
TL;DR: SAGE3D, a Transformer-based model for corner detection in LiDAR point clouds, leveraging a hierarchical encoder-decoder with innovations like Soft-Guided Attention and Excitatory Graph Neural Network for improved precision and recall.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve corner detection in airborne LiDAR point clouds, which likely involves enhancing the accuracy and reliability of feature extraction and prediction in complex point cloud data.

Method: The method involves a multi-stage solution using a hierarchical encoder-decoder architecture with Set Abstraction layers for downsampling and Feature Propagation for per-point predictions. It introduces Soft-Guided Attention and an Excitatory Graph Neural Network for precision and recall optimization.

Result: The result is SAGE3D, a hybrid Transformer-based model that effectively detects corners in LiDAR point clouds with improved precision and recall through its innovative components.

Conclusion: The conclusion is that SAGE3D offers an effective approach to corner detection in airborne LiDAR point clouds, leveraging a hierarchical design and specific innovations to enhance performance.
...
TL;DR: A deep learning approach is proposed to track and reconstruct coral polyps from μCT scans, achieving high accuracy in segmenting corallite structures.


<details>
  <summary>Details</summary>
Motivation: Understanding the rate and timing of polyp division and its impact on coral colony growth requires tracking skeletal corallites around each polyp.

Method: A hybrid V-Trans-UNet architecture is proposed for segmenting μCT scans of coral skeletons, pre-trained on weakly annotated data and fine-tuned with fully annotated slice sections.

Result: The model achieves 0.94 topological accuracy, with mean Dice scores of 0.77 on the same colony and 0.63 on a biologically unrelated specimen.

Conclusion: The study demonstrates the effectiveness of visual machine learning in supporting 3D individual corallite modeling from μCT scans, providing a baseline and dataset for future research.
...
TL;DR: A new framework called DriveCtrl is proposed for generating realistic driving videos from simulated data, preserving scene structure, object dynamics, and temporal consistency.


<details>
  <summary>Details</summary>
Motivation: The domain gap between synthetic and real-world driving videos limits the utility of simulation data for training autonomous driving systems, and existing video generation methods fail to preserve critical aspects of scene structure, object dynamics, temporal consistency, and visual realism.

Method: DriveCtrl, a depth-conditioned controllable sim-to-real video generation framework, built upon a pretrained video foundation model with a structure-aware adapter, and a scalable data generation pipeline supporting three conditioning signals: structural depth, reference-dataset style, and text prompts.

Result: DriveCtrl consistently outperforms the base model and competing alternatives in realism, temporal quality, and perception task performance, substantially narrowing the sim-to-real gap for driving video generation.

Conclusion: DriveCtrl offers a promising solution for generating realistic driving videos from simulated data, which can help bridge the sim-to-real gap and improve the performance of autonomous driving systems.
...
TL;DR: The paper proposes Causal Forcing++, a pipeline for frame-wise autoregression with 1-2 sampling steps, surpassing state-of-the-art results in video generation with lower latency and training cost.


<details>
  <summary>Details</summary>
Motivation: Existing autoregressive diffusion distillation methods are limited by coarse response granularity and non-negligible sampling latency, and the paper aims to study a more aggressive setting of frame-wise autoregression with only 1-2 sampling steps.

Method: The paper proposes Causal Forcing++, a principled and scalable pipeline that uses causal consistency distillation (causal CD) for few-step AR initialization.

Result: The pipeline surpasses the SOTA 4-step chunk-wise Causal Forcing under the frame-wise 2-step setting by 0.1 in VBench Total, 0.3 in VBench Quality, and 0.335 in VisionReward, while reducing first-frame latency by 50% and Stage 2 training cost by ~4x.

Conclusion: The paper presents a novel pipeline for real-time interactive video generation, achieving state-of-the-art results with lower latency and training cost.
...
TL;DR: The paper explores using purely synthetic layered data to improve graphic design decomposition, achieving comparable or better performance than existing methods.


<details>
  <summary>Details</summary>
Motivation: The challenge of flexible post-generation editing of images due to their inherently flattened nature, and the limitations of existing approaches in scalability.

Method: A data-centric study using a state-of-the-art layer decomposition framework (CLD baseline) with a synthetic dataset (SynLayers) and vision language models for textual supervision and automated inference inputs.

Result: Three key findings: (1) purely synthetic data can outperform non-scalable alternatives, (2) performance improves with increased training data scale, and (3) synthetic data enables balanced control over layer-count distributions.

Conclusion: The study demonstrates the viability of synthetic data as a scalable and effective substitute for improving graphic design decomposition, encouraging broader adoption in layered design editing systems.
...
TL;DR: EviScreen, an evidential reasoning framework, enhances disease screening by leveraging historical case evidence, improving performance and interpretability.


<details>
  <summary>Details</summary>
Motivation: Current disease screening models for medical images lack interpretability and optimal performance, and do not effectively reference historical cases or provide transparent reasoning.

Method: EviScreen uses region-level evidence from historical cases through dual knowledge banks, and an evidence-aware reasoning module for predictions.

Result: EviScreen achieves superior performance on real-world disease screening benchmarks with higher specificity at clinical-level recall.

Conclusion: EviScreen offers improved disease screening performance and interpretability by integrating historical case evidence and enhancing localization interpretability.
...
TL;DR: SANA-WM is an efficient open-source world model that generates high-fidelity, minute-scale videos with precise camera control, achieving comparable visual quality to industrial baselines while improving efficiency.


<details>
  <summary>Details</summary>
Motivation: The motivation behind SANA-WM is to create an efficient and open-source world model that can generate high-quality, minute-scale videos with precise camera control, addressing the limitations of existing models in terms of efficiency and scalability.

Method: The method used in SANA-WM includes four core designs: Hybrid Linear Attention, Dual-Branch Camera Control, Two-Stage Generation Pipeline, and Robust Annotation Pipeline. These designs enable SANA-WM to achieve remarkable efficiency across data, training compute, and inference hardware.

Result: SANA-WM demonstrates stronger action-following accuracy than prior open-source baselines and achieves comparable visual quality at $36	imes$ higher throughput for scalable world modeling. It uses only $sim$213K public video clips, completes training in 15 days on 64 H100s, and generates each 60s clip on a single GPU.

Conclusion: In conclusion, SANA-WM is an efficient and open-source world model that achieves high-quality and minute-scale video generation with precise camera control, making it a significant advancement in the field of world modeling.
...
TL;DR: The paper proposes a framework for long-horizon image editing, enabling the model to follow multi-step, abstract instructions by coupling planning with reward-driven execution.


<details>
  <summary>Details</summary>
Motivation: Modern image editing models struggle with abstract, multi-step instructions, and prior methods rely on handcrafted pipelines or teacher imitation, limiting flexibility.
...
Conclusion: The paper presents a novel framework for long-horizon image editing, which tightly couples planning with reward-driven execution, leading to improved performance on abstract, multi-step instructions.
...
TL;DR: The authors propose Warp-as-History, a simple interface that enables a frozen video generation model to follow camera trajectories without training or test-time optimization.


<details>
  <summary>Details</summary>
Motivation: Existing methods for camera-controlled video generation require post-training on large-scale camera-annotated videos or test-time optimization, which can be costly. The authors aim to find a training-free alternative.

Method: The authors propose Warp-as-History, which turns camera-induced warps into camera-warped pseudo-history with target-frame positional alignment and visible-token selection. They also use lightweight offline LoRA finetuning on one camera-annotated video.

Result: The proposed method reveals a non-trivial zero-shot capability of a frozen video generation model to follow camera trajectories. Lightweight offline LoRA finetuning further improves this capability and generalizes to unseen videos, improving camera adherence, visual quality, and motion dynamics.

Conclusion: The authors' method, Warp-as-History, enables a frozen video generation model to follow camera trajectories without training or test-time optimization, and achieves state-of-the-art results with lightweight offline LoRA finetuning.
...
TL;DR: A new framework, PDI-Bench, is introduced for evaluating the geometric coherence of generated videos, providing a quantitative measure of physical plausibility in 3D structure and motion.


<details>
  <summary>Details</summary>
Motivation: The increasing study of generative video models as implicit world models has highlighted the challenge of evaluating their production of physically plausible 3D structure and motion, with current methods relying heavily on subjective human judgment or learned graders.

Method: PDI-Bench works by obtaining object-centric observations via segmentation and point tracking, lifting them to 3D world-space coordinates via monocular reconstruction, and computing projective-geometry residuals to capture failure dimensions.

Result: The framework reveals consistent geometry-specific failure modes in state-of-the-art video generators not captured by common perceptual metrics, providing a diagnostic signal for progress toward physically grounded video generation.

Conclusion: PDI-Bench offers a quantitative approach to auditing geometric coherence in generated videos, supporting systematic evaluation and advancement in physically plausible video generation and world modeling.
...
TL;DR: A new framework called VGGT-Edit is proposed for text-conditioned native 3D scene editing, enabling efficient and high-quality editing of 3D scenes based on text instructions.


<details>
  <summary>Details</summary>
Motivation: Current 3D scene reconstruction models struggle with dynamic human instructions and interactive applications, particularly with 2D-lifting editing strategies that yield blurry textures and inconsistent geometry.

Method: VGGT-Edit introduces depth-synchronized text injection and a residual transformation head to directly predict 3D geometric displacements, ensuring stable instruction grounding and preserving background stability.

Result: VGGT-Edit substantially outperforms 2D-lifting baselines, producing sharper object details, stronger multi-view consistency, and near-instant inference speed.

Conclusion: The proposed VGGT-Edit framework offers a more efficient and effective approach to text-conditioned native 3D scene editing, addressing the limitations of current methods and paving the way for interactive 3D applications.
...
TL;DR: The paper proposes using large language models to generate articulated 3D objects at scale, and introduces Articraft, a system that writes programs to build these objects.


<details>
  <summary>Details</summary>
Motivation: The lack of large and diverse datasets of articulated 3D objects hinders learning to understand these objects.

Method: The authors leverage large language models to generate articulated 3D assets by writing programs that build them, using a new agentic system called Articraft.

Result: The authors create Articraft-10K, a dataset of over 10K articulated assets spanning 245 categories, and demonstrate its utility in training models and downstream applications.

Conclusion: The proposed approach produces higher-quality assets than state-of-the-art methods and general-purpose coding agents, and enables the creation of large-scale datasets of articulated 3D objects.
...
TL;DR: The authors propose RAVEN, a framework for real-time video generation that improves upon existing causal autoregressive models by better aligning training and inference, and CM-GRPO, an optimization method that enhances the model's performance.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the gap between the history distributions encountered during training and those arising at inference in causal autoregressive video diffusion models, which constrains generation quality over long horizons.

Method: The authors introduce RAVEN, a training-time test framework that repacks each self-rollout into an interleaved sequence of clean historical endpoints and noisy denoising states, and CM-GRPO, a consistency-model Group Relative Policy Optimization method that applies online Reinforcement Learning (RL) directly to a conditional Gaussian transition.
...
Conclusion: The authors conclude that RAVEN and CM-GRPO can improve the quality and efficiency of real-time video generation, and that their approach has the potential to be applied to other generative models.
...
TL;DR: This paper presents a method to improve image generation using latent flow matching by projecting data latents onto a fixed token radius and using spherical linear interpolation.
...
Method: The method involves decomposing each latent token into radial and angular components, projecting data latents onto a fixed token radius, using the radial projection of Gaussian noise as the spherical prior, finetuning the decoder with the encoder frozen, and replacing linear interpolation with spherical linear interpolation.

Result: The method consistently improves class-conditional ImageNet-256 FID across different image tokenizers, without requiring any auxiliary encoder or representation-alignment objective.

Conclusion: The paper concludes that the proposed method can effectively improve image generation using latent flow matching by leveraging the spherical structure of the latent space.
...
TL;DR: The paper introduces VGGT-Ω, an improved feed-forward reconstruction model that achieves state-of-the-art results in static and dynamic scene reconstruction, with significant improvements in accuracy, efficiency, and capabilities.


<details>
  <summary>Details</summary>
Motivation: The authors aim to improve the quality of feed-forward reconstruction models, such as VGGT, by scaling them up with model and data size, and to enable training at an unprecedented scale.

Method: The authors introduce architectural changes, including a single dense prediction head, removal of expensive high-resolution convolutional layers, registers to aggregate scene information, and register attention. They also propose a high-quality data annotation pipeline and a self-supervised learning protocol.
...
Conclusion: The paper demonstrates that VGGT-Ω substantially improves reconstruction accuracy, efficiency, and capabilities, and shows that reconstruction can be a powerful and scalable proxy task for spatial understanding.
...
<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the architectural asymmetry in latent diffusion models, where the decoder remains unconditional despite the heavily conditioned denoising networks, leading to loss of detail and inconsistency.

Method: The method involves introducing a reference-conditioned video VAE decoder, RefDecoder, which injects high-fidelity reference image signals into the decoding process via reference attention, and a lightweight image encoder that maps the reference frame into detail-rich high-dimensional tokens.
...
Conclusion: The conclusion is that RefDecoder can be directly swapped into existing video generation systems without additional fine-tuning and generalizes well to a wide range of visual generation tasks such as style transfer and video editing refinement.
...
TL;DR: The paper introduces EntityBench, a benchmark for evaluating multi-shot video generation, focusing on maintaining consistent characters, objects, and locations across shots. It also proposes EntityMem, a memory-augmented generation system that improves cross-shot entity consistency.


<details>
  <summary>Details</summary>
Motivation: The challenge of maintaining consistent characters, objects, and locations across shots in multi-shot video generation, and the need for a standardized evaluation benchmark.

Method: The paper proposes EntityBench, a benchmark with 140 episodes and 2,491 shots, and EntityMem, a memory-augmented generation system that stores verified per-entity visual references.

Result: Experiments show that cross-shot entity consistency degrades with recurrence distance in existing methods, and that EntityMem yields the highest character fidelity (Cohen's d = +2.33) and presence among methods evaluated.

Conclusion: The paper concludes that EntityBench and EntityMem can help improve the evaluation and generation of multi-shot videos with consistent characters, objects, and locations.
...
Result: Vector summation with shared covariance is the most reliable strategy, while simple summation without shared covariance performs poorly. TSVM improves performance in some settings but has limited ability to mitigate multilingual interference.

Conclusion: The findings provide guidance for future MKE research, clarifying the strengths and limits of current vector merging methods.
...
TL;DR: VectraYX-Nano, a 41.95M-parameter Spanish language model for cybersecurity, was trained from scratch with a focus on Latin America and native tool invocation.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to develop a Spanish-native cybersecurity language model (LLM) with end-to-end Model Context Protocol (MCP) integration, specifically designed for Latin America.

Method: The authors present VectraYX-Nano, a decoder-only language model trained from scratch using a 170M-token Spanish corpus (VectraYX-Sec-ES) and a 42M-parameter Transformer decoder architecture with various techniques such as GQA, QK-Norm, and SwiGLU.

Result: The model achieved a conversational gate of 0.78+-0.05 and showed improved performance with a tool-dense corpus. The GGUF artifact is 81 MB and runs at sub-second TTFT on commodity hardware.

Conclusion: The paper presents VectraYX-Nano as the first Spanish-native cybersecurity LLM with end-to-end MCP integration, releasing the corpus recipe, training scripts, GGUF weights, and B1-B5 benchmark.
...
TL;DR: Mistletoe is a stealthy attack against speculative decoding in large language models, reducing draft-token acceptability while preserving output quality.


<details>
  <summary>Details</summary>
Motivation: The authors identify a vulnerability in model-based speculative decoding due to the drafter-target mismatch, which creates a hidden attack surface.

Method: The authors propose Mistletoe, an acceleration-collapse attack that jointly optimizes a degradation objective and a semantic-preservation objective using a null-space projection mechanism.
...
Conclusion: The work highlights that speculative decoding introduces a mechanism-level attack surface, calling for more robust designs of LLM acceleration systems.
...
TL;DR: The paper audits the multimodal-physics evaluation pipeline and identifies three undetected construction practices that distort vision-language reasoning measurements. It releases four artifacts to address these gaps and proposes a new recipe, Physics-R1, which outperforms existing models on several benchmarks.


<details>
  <summary>Details</summary>
Motivation: The authors aim to identify and address issues in the multimodal-physics evaluation pipeline that may be distorting measurements of vision-language reasoning.

Method: The authors perform a thorough audit of the evaluation pipeline, including a three-stage audit for near-duplicates and paraphrases, and analyze the performance of different models on various benchmarks.

Result: The authors identify three undetected construction practices: train-eval contamination, translation drift, and MCQ saturation. They also propose a new recipe, Physics-R1, which outperforms existing models on several benchmarks, including PhysOlym-A, PhysReason, OlympiadBench-Physics, and PhyX MCQ.

Conclusion: The paper highlights the importance of auditing and addressing issues in the multimodal-physics evaluation pipeline to ensure accurate measurements of vision-language reasoning. The proposed recipe, Physics-R1, demonstrates improved performance on several benchmarks.
...
Method: The method used in this paper is Derivation Prompting, a novel prompting technique that involves deriving conclusions from initial hypotheses through the systematic application of predefined rules, and constructing an interpretable derivation tree.
...
TL;DR: The paper proposes a Parameter-Efficient Multi-task Learning (PEML) method for fine-tuning Large Language Models (LLMs) on multiple tasks, which co-optimizes prompt optimization and model adaptation, achieving up to 6.67% average accuracy improvement over state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enable efficient fine-tuning of LLMs for multiple tasks, reducing data and resource requirements, and improving adaptation capabilities compared to existing Parameter-Efficient Fine-Tuning (PEFT) methods.

Method: The proposed PEML method employs a neural architecture engineering approach for optimizing continuous prompts and performs low-rank adaptation for model weights, integrating prompt tuning and model adaptation for multi-task learning.

Result: The evaluation results show an average accuracy improvement of up to 6.67% over state-of-the-art multi-task learning methods, with individual tasks showing peak gains of up to 10.75% on benchmarks such as GLUE, SuperGLUE, Massive Multitask Language Understanding, and commonsense reasoning.

Conclusion: The paper concludes that PEML effectively enables efficient fine-tuning of LLMs for multiple tasks, improving adaptation capabilities and performance compared to existing PEFT methods, which facilitates resource consolidation and reduces resource consumption.
...
TL;DR: The paper introduces Inquisitive Conversational Agents (ICAs) that can proactively extract information to achieve their objectives, and develops an ICA for U.S. Supreme Court oral arguments.


<details>
  <summary>Details</summary>
Motivation: Existing dialogue systems are user-driven and primarily designed to fulfill user requests, but in many critical real-world scenarios, a conversational agent must proactively extract information to achieve its own objectives.
...
Conclusion: The paper represents an important first step toward broader high-stakes, domain-specific applications.
...
TL;DR: A new offline reasoning distillation framework is proposed to improve intelligence in resource-constrained settings by distilling reasoning traces from large language models into smaller ones.


<details>
  <summary>Details</summary>
Motivation: Existing approaches face a trade-off between offline distillation and on-policy methods, and the proposed framework aims to overcome the limitations of these methods.

Method: The proposed framework adaptively emphasizes teacher supervision that is better aligned with the student's on-policy distribution, correcting teacher-student distribution drift.

Result: The proposed method improves reasoning accuracy over prior offline distillation algorithms and yields more stable reasoning traces while preserving instruction-following capabilities.

Conclusion: The proposed offline reasoning distillation framework can substantially strengthen offline reasoning distillation without online rollouts, leading to improved intelligence in resource-constrained settings.
...
TL;DR: The study evaluates DExperts, a technique to mitigate toxic outputs from Large Language Models (LLMs) without retraining. It achieves high safety rates against explicit toxicity but shows brittleness against implicit hate speech and incurs a significant latency penalty.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this study is to address the issue of 'toxic degeneration' in LLMs, where innocuous prompts can trigger harmful outputs, posing significant risks for real-world deployments. The goal is to find effective mitigation strategies that maintain model utility while ensuring safety.

Method: The study uses a comprehensive replication approach, structured into three phases: (1) establishing baseline toxicity measurements using RealToxicityPrompts on standard GPT-2 models, (2) implementing and evaluating DExperts to mitigate explicit toxicity, and (3) stress-testing the method against implicit hate speech using the adversarial ToxiGen dataset.

Result: The empirical results show that DExperts achieves near-perfect safety rates (100%) on explicit toxicity benchmarks but exhibits brittleness against adversarial, implicit hate speech, with safety rates dropping to 98.5%. The method introduces a ~10x latency penalty (from 0.2s to 2.0s per generation), posing challenges for real-time deployment scenarios.

Conclusion: The study highlights the robustness gap between explicit and implicit toxicity mitigation and emphasizes the need for more sophisticated approaches that generalize across diverse hate speech patterns without prohibitive computational costs.
...
TL;DR: This paper evaluates the reliability of large language models (LLMs) in handling conflicting biomedical evidence and proposes a conflict-aware abstention score to improve their performance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the issue of incomplete, misleading, or internally contradictory evidence in biomedical retrieval-augmented LLMs and to evaluate their reliability under conflicting evidence conditions.

Method: The authors use HealthContradict to evaluate six open-weight LLMs under five controlled evidence conditions, including conflicting evidence conditions, and propose a conflict-aware abstention score that combines model confidence with a detector of evidence conflict.

Result: The results show that accuracy drops for every model under conflicting evidence conditions, with 11.4%-25.2% of predictions flipping. The conflict-aware abstention score improves selective accuracy over confidence-only, with mean gains of 7.2-33.4 points in incorrect-only and 3.6-14.4 points in incorrect-first conflicting conditions.

Conclusion: The study concludes that conflicting biomedical evidence is both an uncertainty and robustness problem, and evaluation and abstention methods that explicitly account for evidence disagreement are needed.
...
TL;DR: The paper introduces a text-based approach for generating floor plans that respect both connectivity and numerical constraints, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing generative approaches for floor plan design primarily focus on respecting room connectivity but neglect numerical constraints.

Method: The approach fine-tunes a large language model (LLM) on real plans and applies reinforcement learning with verifiable rewards (RLVR) to improve adherence to constraints.

Result: The model generates floor plans that satisfy user-defined constraints and outperforms existing methods on Realism, Compatibility, and Diversity metrics, achieving at least a 94% relative reduction in Compatibility errors.

Conclusion: The approach demonstrates the effectiveness of LLMs in handling constraints for floor plan generation, suggesting broader applications for text-based generative modeling.
...
TL;DR: The paper proposes a simple neural code for artificial neural networks to form complex semantic structures, which is tested and validated in Large Language Models (LLMs).
...
Method: The authors use a Polar Probe to target a subspace of LLMs' layer activations, testing the hypothesis in five different domains.

Result: The results show that true semantic structures can be linearly recovered, the neural code emerges in middle layers and improves with LLM performance, and generalizes to new entities and relation types.

Conclusion: The findings suggest that LLMs learn to build complex semantic structures by binding representations with a simple geometrical principle.
...
<details>
  <summary>Details</summary>
Motivation: Current safety evaluations for large language models focus on high-stakes risks but often neglect the interaction between language and geopolitical context, particularly in multilingual settings.

Method: The authors propose a transcreation matrix methodology, which separates the effects of language and geopolitical grounding by evaluating adversarial intents in controlled combinations of language and geopolitical context.

Result: The study finds a consistent suppression effect in Korean variants of language models and significant model-to-model variation in how geopolitical grounding interacts with language, suggesting that safety behavior is shaped by language and context interactions.

Conclusion: The paper highlights the importance of considering language and geopolitical context in safety evaluations for large language models, and the proposed transcreation matrix methodology can be generalized to other language-culture pairs.
...
Method: The authors propose a search-based memory framework called BOOKMARKS, which uses bookmarks to store task-relevant information and updates them through synchronization.
...
TL;DR: This paper studies why Retrieval-Augmented Generation (RAG) systems sometimes produce incorrect answers despite having access to external information. It uses circuit tracing to analyze how retrieved evidence influences answer generation and identifies structural differences between correct and failed predictions.


<details>
  <summary>Details</summary>
Motivation: RAG systems produce incorrect answers in many cases, and it is poorly understood why this happens despite having access to external information.

Method: The study uses circuit tracing to construct attribution graphs that model the flow of information through transformer layers during decoding, representing interactions among retrieved context, intermediate model activations, and generated tokens.

Result: The study observes consistent structural differences between correct and failed predictions: correct predictions exhibit deeper reasoning paths, more distributed evidence flow, and a more structured pattern of local connectivity, while failed predictions show shallower, fragmented, and overly concentrated evidence flow.

Conclusion: The findings lead to the development of a graph-based error detection framework and enable targeted interventions, such as reinforcing question-constrained evidence grounding, to improve the integration of retrieved information and reduce errors.
...
<details>
  <summary>Details</summary>
Motivation: LLMs are at risk of safety misalignment after finetuning due to harmful data, and even benign data can lead to misaligned behaviors.

Method: GradShield, a principled filtering method that uses Finetuning Implicit Harmfulness Score (FIHS) and adaptive thresholding to identify and remove harmful data points.

Result: GradShield outperforms baseline methods, maintaining an Attack Success Rate (ASR) below 6% while preserving utility performance across multiple fine-tuning tasks.

Conclusion: GradShield effectively safeguards LLMs during finetuning, ensuring both safety and utility performance.
...
TL;DR: The paper presents two models for predicting vocabulary difficulty: a high-accuracy black-box model and an explainable model, with the black-box model achieving a correlation of 0.91 and the explainable model achieving a correlation of 0.77.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to develop models for vocabulary difficulty prediction, which can help in understanding what makes certain words or items difficult for learners.

Method: The paper uses two types of models: a high-accuracy black-box model which is a fine-tuned Large Language Model (LLM) using a soft-target loss function, and an explainable model. The black-box model achieved a high correlation (r > 0.91) with the task, while the explainable model provided insights into the factors affecting item difficulty with a strong correlation (r > 0.77).

Result: The black-box model achieved a correlation of greater than 0.91, and the explainable model achieved a correlation of greater than 0.77. The analysis showed that the difficulty of items in the British Council's Knowledge-based Vocabulary Lists (KVL) is often affected by spelling difficulty, the construction of the test items, and the genuine production difficulty of the words.

Conclusion: The paper concludes that the developed models can effectively predict vocabulary difficulty, with the explainable model providing insights into the factors that impact item difficulty. The analysis of the results highlights the importance of considering multiple factors when assessing vocabulary difficulty.
...
TL;DR: The paper proposes HarnessAudit, a framework to audit the safety of LLM agent execution harnesses, and presents HarnessAudit-Bench, a benchmark to evaluate harness safety across various domains and configurations.


<details>
  <summary>Details</summary>
Motivation: Current safety benchmarks only evaluate final outputs or terminal states, which is insufficient to detect safety violations that occur mid-trajectory, such as unauthorized resource access or context leakage.

Method: The authors propose a framework called HarnessAudit to audit full execution trajectories across boundary compliance, execution fidelity, and system stability, and introduce HarnessAudit-Bench, a benchmark of 210 tasks across eight real-world domains.

Result: The evaluation of ten harness configurations across frontier models and three multi-agent frameworks reveals that task completion is often misaligned with safe execution, safety risks vary across domains and task types, and most violations occur in resource access and inter-agent information transfer.

Conclusion: The paper concludes that HarnessAudit and HarnessAudit-Bench can help identify safety risks in LLM agent execution harnesses, and that harness design plays a critical role in ensuring safe deployment.
...
TL;DR: A new method called FeF-DLLM is proposed to improve discrete diffusion language models by reducing factorization errors and accelerating inference.


<details>
  <summary>Details</summary>
Motivation: Standard discrete diffusion language models introduce factorization errors by approximating the clean token posterior with independent token-wise distributions, which can lead to reduced accuracy.

Method: The proposed FeF-DLLM method uses an exact prefix-conditioned factorization of the clean posterior to preserve token dependencies and incorporates speculative decoding within diffusion denoising to accelerate inference.

Result: Experiments show that FeF-DLLM improves accuracy by an average of 5.04 percentage points and achieves an average inference speedup of 3.86x on several benchmarks.

Conclusion: FeF-DLLM provides a more accurate and efficient approach to discrete diffusion language modeling by addressing factorization errors and accelerating inference.
...
TL;DR: This paper proposes a transformer-based model to classify the political orientation of text on a continuous left-to-right spectrum, and evaluates its performance on four distinct corpora.


<details>
  <summary>Details</summary>
Motivation: To better understand political rhetoric from various movements, and to provide analysts with a tool to focus on specific segments of the political landscape.
...
Result: The best performing models achieved high F1 scores (0.844) and accuracy (0.864) on in-domain and out-of-domain tests, and demonstrated that transformer models can recognize political framing in German news at the level of public opinion polls.

Conclusion: The study demonstrates the effectiveness of transformer models in estimating political bias, and highlights the importance of model architecture and domain-specific training data in achieving high performance.
...
<details>
  <summary>Details</summary>
Motivation: The shift of political discussions to social media has made it important to differentiate between manipulative political narratives and legitimate critiques.

Method: The framework uses a few-shot prompt to filter manipulative posts, then applies UMAP and HDBSCAN for clustering, and finally employs a reasoning model to uncover the narrative behind each cluster.

Result: The approach identified 41 distinct manipulative narrative clusters from over 1.2 million social media posts.

Conclusion: The proposed framework effectively detects and structures manipulative political narratives on social media.
...
TL;DR: A new method for improving large language models in low-resource languages without losing general capabilities.


<details>
  <summary>Details</summary>
Motivation: The 'alignment tax' problem where improving language models in low-resource languages causes them to forget general capabilities.

Method: A semantic-space alignment paradigm using Group Relative Policy Optimization (GRPO) with embedding-level semantic rewards.

Result: The method mitigates 'alignment tax', preserves general competence, and yields higher semantic quality and preference in open-ended generation.

Conclusion: Reinforcement learning with semantic rewards provides a safer pathway for inclusive low-resource language expansion.
...
TL;DR: A new method called DiHAL is proposed to improve diffusion language models by integrating them with transformers in a geometry-guided way.


<details>
  <summary>Details</summary>
Motivation: Continuous diffusion language models are less effective than autoregressive transformers, partly due to the unsuitable application of diffusion in language denoising and token recovery.

Method: DiHAL uses geometry-based proxies to score layers, selects a suitable hidden-state interface, and replaces the lower transformer layers with a diffusion bridge while keeping the upper layers and original LM head intact.

Result: Experiments show that the geometry score predicts effective insertion layers and that hidden-state recovery improves over continuous diffusion baselines.

Conclusion: The results suggest that hidden-state geometry helps identify where diffusion-based replacement is feasible inside pretrained language models, leading to improved performance.
...
TL;DR: The authors propose a context-aware synthetic augmentation framework and hybrid classification model to classify psychological defense mechanisms from text, achieving improved performance in low-resource settings.
...
Method: The authors propose a context-aware synthetic augmentation framework combined with a hybrid classification model that integrates contextual language representations, basic clinical features, and 150 annotated defense items.
...
Conclusion: The proposed method effectively addresses the challenges of data scarcity and class imbalance in classifying PDMs from text, providing a clinically valuable tool for psychological defense mechanism classification.
...
TL;DR: The paper proposes MARS, a memory-augmented recommender system that uses a structured belief state to model user preferences, achieving state-of-the-art performance on four benchmark domains.


<details>
  <summary>Details</summary>
Motivation: Existing memory-augmented LLM agents for personalized recommendation use flat memory representations that conflate ephemeral signals with stable preferences, and lack a complete lifecycle for memory evolution.

Method: MARS treats recommendation as a partially observable problem, maintaining a three-tiered belief state (event memory, preference memory, and profile memory) with a lifecycle of six operations (extraction, reinforcement, weakening, consolidation, forgetting, and resynthesis) adaptively scheduled by an LLM-based planner.

Result: Experiments on four InstructRec benchmark domains show that MARS achieves state-of-the-art performance with average improvements of 26.4% in HR@1 and 10.3% in NDCG@10 over the strongest baselines.

Conclusion: The proposed MARS framework provides a more structured and effective approach to modeling user preferences and evolving memory, leading to improved performance in personalized recommendation tasks.
...
TL;DR: The paper proposes two metrics, KSS and KPS, to evaluate the effectiveness of Multilingual Machine Unlearning (MMU) in removing sensitive information across multiple languages.


<details>
  <summary>Details</summary>
Motivation: The increasing use of Large Language Models (LLMs) in commercial services poses privacy risks, such as leakage of sensitive personally identifiable information (PII). Existing MMU evaluations are limited and do not capture the cross-linguistic distribution of information.

Method: The paper proposes two metrics: Knowledge Separability Score (KSS) and Knowledge Persistence Score (KPS), to evaluate the information spread across languages. These metrics are used to assess various unlearning methods in the multilingual setting.

Result: The evaluation and analysis provide insights into unique phenomena exclusive to MMU and offer a new perspective on MMU evaluation.

Conclusion: The proposed metrics, KSS and KPS, can effectively evaluate the performance of MMU methods in removing sensitive information across multiple languages, providing a more comprehensive understanding of MMU evaluation.
...
<details>
  <summary>Details</summary>
Motivation: The vocabulary size is a critical hyper-parameter in end-to-end ASR systems, but there is little documentation on how to determine it. Recent work formalized an approach to identify the optimal vocabulary size, but this paper aims to build upon that foundation by proposing a more formalized method.

Method: The paper uses curve fitting and the principle of first and second derivative tests in calculus to estimate the vocabulary size hyper-parameter. It treats the tokenization process as a black box and applies the proposed approach to a standard Librispeech corpus.

Result: The proposed approach improves the performance of the ASR system by identifying the optimal vocabulary size hyper-parameter.

Conclusion: The paper formalizes an approach to identify the vocabulary size best suited for training an end-to-end ASR system, and demonstrates its effectiveness on a standard Librispeech corpus.
...
TL;DR: The paper introduces Context-Driven Decomposition (CDD), a method to analyze and intervene in the context-compliance regime of Retrieval-Augmented Generation (RAG) models, where retrieved context dominates the final answer even when conflicting with the model's parametric knowledge.


<details>
  <summary>Details</summary>
Motivation: The motivation is to understand and address the issue of context compliance in RAG models, where accuracy alone is not sufficient to reveal how retrieved context shapes answers under conflict.

Method: The authors introduce CDD, a belief-decomposition probe that operates at inference time and serves as an intervention mechanism for controlled retrieval conflict.

Result: The results show that CDD can expose patterns of context compliance, including measurable context compliance in adversarial settings, transferable accuracy gains across model families, and improved robustness under temporal drift and noisy distractors.

Conclusion: The paper concludes that context-compliance is a structural axis along which standard RAG can be probed and intervened on, and that CDD is a useful tool for systematic study across model families and retrieval pipelines.
...
TL;DR: The study analyzes Huìtóngguǎnxì Huáyíyìyǔ (HHY), a multilingual glossary compiled by the Ming government, to understand its transcription principles and provide evidence for historical phonology.
...
Method: Digitization and alignment of HHY with Chinese phonological categories, critical review of previous reconstructions, and comparative analysis across eight language sections.

Result: Main Transcription (MT) represents sounds compatible with Chinese syllable structure, while Supplementary Transcription (ST) encodes phonetic features less compatible with Chinese phonology.

Conclusion: HHY is an internally structured transcription system that provides valuable evidence for historical phonology, particularly for under-documented Asian languages.
...
TL;DR: The paper introduces GroupMemBench, a benchmark for evaluating memory systems in large language models (LLMs) in group conversations. Existing memory systems and benchmarks are inadequate for handling group dynamics, speaker-grounded belief tracking, and audience-adapted language. The benchmark reveals that current memory systems struggle with group conversations, with even a simple baseline outperforming them.


<details>
  <summary>Details</summary>
Motivation: The increasing use of LLM agents in personal and workplace settings requires memory systems that can handle long-running conversations with multiple users. However, existing memory systems and benchmarks are limited to single-user setups, neglecting group dynamics and interactions.

Method: The authors introduce GroupMemBench, a benchmark that uses a graph-grounded synthesis pipeline to generate multi-party conversations with controllable reply structure and conditions each message on per-user personas and target audiences. An adversarial query pipeline is then used to test memory systems across six categories.

Result: The benchmark reveals that current memory systems struggle with group conversations, with the strongest system achieving only 46.0% average accuracy. A simple BM25 baseline matches or exceeds most agent memory systems, indicating that current memory ingestion erases structural and lexical features group memory depends on.

Conclusion: The paper concludes that current memory systems are inadequate for handling group conversations and that there is a need for more advanced memory systems that can handle group dynamics, speaker-grounded belief tracking, and audience-adapted language.
...
TL;DR: The paper proposes a framework for evaluating LLM outputs that distinguishes between structural recovery and intent fidelity, revealing that holistic scores can mask intent deficits.


<details>
  <summary>Details</summary>
Motivation: Current holistic evaluation scores do not distinguish between a model's ability to reproduce the structural form of a user's request and its ability to preserve the user's specific intent.
...
Conclusion: The findings demonstrate that dimension-level intent fidelity evaluation is a necessary complement to holistic assessment when evaluating LLM outputs for user-specific tasks.
...
TL;DR: The paper presents a new approach to language generation by framing it as a stochastic optimal control problem, providing a unified perspective on autoregressive and diffusion models, and introducing a method called Manta-LM that achieves high-fidelity text generation and efficient parallel sampling.
...
Method: The authors reformulate language generation as a stochastic optimal control problem and approximate the solution to the Hamilton-Jacobi-Bellman (HJB) equation using Flow Matching as the optimal trajectory solver, yielding an optimal policy that acts as a closed-loop controller.

Result: The proposed method, Manta-LM with Global Integral Operator, achieves strong performance on language modeling and conditional generation tasks, while exhibiting improved stability, efficiency, and controllability.

Conclusion: The paper provides a new perspective on language generation and presents a method that simultaneously achieves high-fidelity text generation and efficient, low-cost parallel sampling, addressing the limitations of current models.
...
TL;DR: The authors propose Correction-Oriented Policy Optimization (CIPO), an extension to Reinforcement Learning with Verifiable Rewards (RLVR) that improves the reasoning capabilities of large language models by converting failed trajectories into correction-oriented supervision.


<details>
  <summary>Details</summary>
Motivation: RLVR training is hindered by sparse binary rewards and weak credit assignment, resulting in ambiguous optimization signals and underutilization of useful information in failed trajectories.

Method: CIPO converts on-policy failed trajectories into correction-oriented supervision and jointly optimizes correction samples with the standard RLVR objective.
...
TL;DR: The paper proposes a method for uncertainty quantification in Large Language Diffusion Models, achieving a good cost-performance trade-off.


<details>
  <summary>Details</summary>
Motivation: Existing uncertainty quantification methods are not suitable for Large Language Diffusion Models, which are prone to hallucinations and require reliable uncertainty estimation for safe deployment.

Method: The paper proposes lightweight, zero-shot uncertainty signals derived from the iterative denoising process and adapts a state-of-the-art uncertainty quantification method to Large Language Diffusion Models.

Result: The proposed method achieves a great cost-performance trade-off, approaching the strongest sampling-based baselines while incurring up to 100x lower computational overhead.

Conclusion: The paper demonstrates that Large Language Diffusion Models can deliver both fast inference and reliable hallucination detection simultaneously.
...
TL;DR: EndPrompt is a method for extending the context window of large language models using short training sequences, achieving effective context extension with less computation.


<details>
  <summary>Details</summary>
Motivation: Extending the context window of large language models is expensive and difficult due to quadratic memory and computational costs, and the authors propose EndPrompt to address this challenge.

Method: EndPrompt uses a two-segment construction, preserving the original short context and appending a brief terminal prompt, assigning it positional indices near the target context length.
...
TL;DR: The paper introduces a new task called discovery pathway forecasting, which involves identifying enabling contributions and their prior work dependencies required to realize a target scientific contribution.


<details>
  <summary>Details</summary>
Motivation: Existing AI4Science benchmarks focus on citation prediction, literature retrieval, or idea generation, but not on the dependencies that make scientific progress possible.

Method: The authors present SciPaths, a benchmark of 262 expert-annotated gold pathways and 2,444 silver pathways, and evaluate frontier and open-weight language models on this task.

Result: The best model reaches only 0.189 F1 under strict semantic matching, with core methodological dependencies hardest to recover. Prior-work grounding improves substantially when gold enabling contributions are provided.

Conclusion: The paper highlights the importance of reasoning backward from a target contribution to the enabling scientific building blocks and prior-work dependencies, and proposes SciPaths as a new evaluation benchmark for scientific forecasting.
...
TL;DR: The authors compare three machine translation setups for translating a Spanish rock art text into English, focusing on simple interventions to improve terminology accuracy.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the challenge of limited budgets and staffing for multilingual dissemination of research and interpretive materials in cultural heritage institutions, particularly in terminology-dense domains like rock art.

Method: The authors compare three machine translation setups: (1) DeepL as a strong neural machine translation baseline, (2) Gemini-Simple (a large language model with a basic prompt), and (3) Gemini-RAG (the same large language model with glossary-augmented prompting via term-pair retrieval).

Result: The results show that Gemini-RAG yields the highest exact-match terminology accuracy (81.4%), outperforming Gemini-Simple (69.1%) and DeepL (64.4%), while preserving overall quality.
...
TL;DR: The paper discusses the challenge of ensuring compliance in large language models used in regulated financial workflows. It proposes five governance metrics to evaluate policy compliance and compares text-only governance with mechanical enforcement.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the principal-agent failure in large language models where outputs appear compliant but may not be, and to ensure auditable decisions in regulated workflows.

Method: The method involves introducing five governance metrics to quantify policy compliance at the rationale level and comparing text-only governance with mechanical enforcement using four primitives operating outside the model's interpretive loop.

Result: The results show that mechanical enforcement significantly improves compliance, reducing deferrals with no decision-relevant information by 73%, increasing deferral information content, and raising task accuracy from 0.43 to 0.88.

Conclusion: The conclusion is that governance and task evaluation are distinct axes, and accuracy is not a sufficient proxy for governance in regulated AI systems. Mechanical enforcement preserves governance quality even as task performance drops, highlighting the importance of architectural separation.
...
TL;DR: A new framework, Video2GUI, is proposed to automatically extract GUI interaction trajectories from Internet videos, resulting in a large-scale dataset called WildGUI, which improves the performance of multimodal large language models on GUI tasks.


<details>
  <summary>Details</summary>
Motivation: The scarcity of large-scale training data for GUI agents, which limits their generalization across diverse real-world applications, and the high cost of manual annotations in existing datasets.

Method: Video2GUI, a fully automated framework that uses a coarse-to-fine filtering strategy to identify high-quality GUI tutorial videos and convert them into structured agent trajectories.

Result: The WildGUI dataset, containing 12 million interaction trajectories spanning over 1,500 applications and websites, and pre-training Qwen2.5-VL and Mimo-VL on WildGUI yields consistent improvements of 5-20% across multiple GUI grounding and action benchmarks.

Conclusion: The proposed Video2GUI framework and WildGUI dataset can support future research of GUI agents and improve their performance on various tasks.
...
TL;DR: This paper introduces a new intervention framework for understanding large language models (LLMs) that extends to non-linearly represented features, allowing for more precise steering of the model.


<details>
  <summary>Details</summary>
Motivation: Existing intervention methods for LLMs are limited to linear interventions, which cannot capture features encoded along non-linear manifolds.

Method: The authors propose a general formulation of intervention that can handle non-linear features, along with a learning procedure for intervening on implicit features without a direct output signature.

Result: The framework is validated on refusal bypass steering, where it outperforms linear baselines by precisely steering the model through intervention on a non-linear feature governing refusal.

Conclusion: The proposed framework provides a more comprehensive approach to understanding and steering LLMs by addressing the limitations of existing linear intervention methods.
...
TL;DR: A new architecture for real-time speech-to-text translation using LLMs, enabling streaming translation with low latency.
...
Method: Proposed LLM-based architecture that learns to emit output tokens and decide when to do so based on input audio.

Result: Achieved translation quality close to non-streaming baseline with a latency of only 1-2 seconds on different language pairs.

Conclusion: The proposed system enables real streaming speech-to-text translation with low latency while maintaining high translation quality.
...
TL;DR: A new method called Graphs of Research (GoR) is proposed to improve automated scientific research idea generation using large language models (LLMs) by leveraging citation relationships and structures.


<details>
  <summary>Details</summary>
Motivation: Existing methods for automating idea generation using LLMs rely on static literature retrieval or complex prompt engineering, neglecting structural relations among references.

Method: GoR uses a supervised fine-tuning approach, constructing a directed acyclic graph (DAG) of paper evolution based on citation position, frequency, predecessor links, and publication time, and fine-tunes Qwen2.5-7B-Instruct-1M on structured-text prompts including citation graph and edge signals.

Result: GoR-SFT achieves State-of-the-Art (SOTA) results in head-to-head LLM-judge tournaments against GPT-4o-driven baselines, demonstrating the effectiveness of citation-evolution graphs as a supervision signal for LLM-based idea generation.

Conclusion: The proposed method reduces the barrier for using citation evolution graphs as supervision, potentially accelerating automated scientific innovation.
...
TL;DR: Converting Lexicon-Grammar tables for French verbs into Lexical Markup Framework (LMF) format for standardization and interoperability in natural language processing.


<details>
  <summary>Details</summary>
Motivation: To make the Lexicon-Grammar of French language usable in different contexts by converting it into an interoperable representation format according to LMF standard.

Method: Conversion of Lexicon-Grammar tables into LMF format, analyzing difficulties faced during conversion.
...
TL;DR: The paper compares the efficiency of seven foundation models from five providers on Ukrainian legal text, focusing on tokenizer fertility and zero-shot performance.


<details>
  <summary>Details</summary>
Motivation: There is no systematic comparison of foundation models on Ukrainian legal text, despite their varying efficiency in tokenization.

Method: The study benchmarks seven models on 273 validated court decisions from Ukraine's state registry, measuring tokenizer fertility and zero-shot performance on three tasks.

Result: The results show that tokenizer fertility varies by 1.6x, with Qwen3 models consuming 60% more tokens than Llama-family models. NVIDIA Nemotron Super 3 (120B) achieves the highest composite score at one-third the API cost of Mistral Large 3 (675B total, 41B active). Few-shot prompting degrades performance by up to 26 percentage points.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to explore the potential of vision-language models (VLMs) for visual procedure question answering (VP-QA), which presents unique challenges in querying next-step actions by uploading images for intermediate states of complex procedures.

Method: The method proposed is Chain-of-Procedure (CoP), a hierarchical reasoning framework that retrieves relevant instructions using visual cues, performs step refinement through semantic decomposition, and generates the next step.

Result: The results show that CoP achieves up to 13% absolute improvement over standard baselines across six VLMs.

Conclusion: The conclusion is that the proposed CoP framework effectively addresses the limitations of current VLMs in visual procedural reasoning, and the introduced benchmark, ProcedureVQA, provides a systematic evaluation for VLMs on this practical task.
...
TL;DR: The paper proposes PPOW, a reinforcement learning framework that optimizes speculative decoding in LLMs by shifting from token-level to window-level optimization, achieving significant speedup and acceptance length improvements.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the bottleneck in speculative decoding caused by hard-to-draft positions, which truncate the accepted prefix and invalidate the rest of the speculative window, by optimizing the draft model with a window-level approach.

Method: The method proposed is PPOW (Performance-Driven Policy Optimization with Adaptive Windowing), a reinforcement learning framework that combines a Cost-Aware Speedup Reward, a Distribution-Based Proximity Reward, and Adaptive Divergence-Aware Windowing.

Result: The results show that PPOW achieves average acceptance lengths of 6.29-6.52 and speedups of 3.39-4.36× across multiple model families and benchmarks under a unified decoding protocol.

Conclusion: The conclusion is that performance-driven window-level optimization is a practical approach to improving speculative decoding efficiency, as demonstrated by the significant improvements achieved by PPOW.
...
TL;DR: Large language models (LLMs) often prematurely conclude and provide answers or guidance when they shouldn't, due to insufficient information, which can lead to diagnostic errors.


<details>
  <summary>Details</summary>
Motivation: The study aims to investigate premature closure in large language models (LLMs), which is a recognized contributor to diagnostic error, and to evaluate their performance in medical tasks.

Method: The study evaluated five frontier LLMs across structured and open-ended medical tasks, including MedQA, AfriMed-QA, HealthBench, and physician-authored adversarial queries.

Result: The results showed that LLMs frequently provided answers or guidance when they shouldn't, with high rates of premature closure observed in both structured and open-ended evaluations.

Conclusion: The study highlights the need to evaluate whether medical LLMs know when not to answer and to develop strategies to mitigate premature closure, as safety-oriented prompting only reduced but did not eliminate the issue.
...
TL;DR: The paper presents a large-scale resource called the Scientific Contribution Graph, which extracts scientific contributions from articles and links them to their prerequisites, and explores the task of predicting scientific prerequisites.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to automate technological roadmapping by extracting scientific contributions from scholarly articles and linking them to their prerequisites, which can support scientific impact assessment and automated scientific discovery.

Method: The authors formulate the task of automated technological roadmapping and present the Scientific Contribution Graph, a large-scale AI/NLP-domain resource containing 2 million detailed scientific contributions extracted from 230k open-access papers and connected by 12.5 million prerequisite edges.

Result: The authors show that contemporary models for predicting scientific prerequisites are rapidly improving, reaching 0.48 MAP when evaluated using temporally filtered backtesting.

Conclusion: The authors anticipate that technological roadmapping resources such as the Scientific Contribution Graph will support scientific impact assessment and automated scientific discovery.
...
TL;DR: The paper presents COTCAgent, a hierarchical reasoning framework for analyzing longitudinal electronic health records, addressing limitations in current large language models.
...
Method: The COTCAgent framework consists of three core modules: Temporal-Statistics Adapter (TSA), Chain-of-Thought Completion (COTC) layer, and bounded completion module.

Result: The COTCAgent achieves 90.47% Top-1 accuracy on a self-built dataset and 70.41% on HealthBench, outperforming existing medical agents and mainstream large language models.

Conclusion: The COTCAgent framework effectively addresses the limitations in current large language models for longitudinal EHR reasoning, providing a more accurate and efficient approach for clinical decision support.
...
TL;DR: A new multimodal benchmark and framework, GranuVistaVQA and GranuRAG, are proposed to improve the performance of Retrieval-Augmented Generation systems by enabling fine-grained retrieval and generation.


<details>
  <summary>Details</summary>
Motivation: Current multimodal Retrieval-Augmented Generation systems have limitations in handling fine-grained user queries and verifying failures due to coarse-grained evidence retrieval.

Method: A multi-granularity framework, GranuRAG, is proposed, which includes element-level detection and classification, multi-granularity cross-modal alignment for evidence retrieval, and attribution-constrained generation.

Result: GranuRAG achieves up to 29.2% improvement over six strong baselines for this task.

Conclusion: The proposed GranuVistaVQA benchmark and GranuRAG framework enable transparent error diagnosis and improve the performance of Retrieval-Augmented Generation systems by grounding retrieval at the element level.
...
<details>
  <summary>Details</summary>
Motivation: The behavior of LLMs as communicative actors in socially structured contexts remains underexplored, and this study aims to fill this gap by investigating whether LLM-based multi-agent systems exhibit systematic linguistic adaptation in response to perceived social observation contexts.

Method: A controlled experiment involving 100 multi-agent debate sessions across five conditions, varying the framing of social observation, including explicit monitoring by human researchers, negation of monitoring, and automated AI auditing.

Result: The results show that monitored conditions and automated AI monitoring produce higher TTR change than audience-framing conditions, and LLM behavior is sensitive to observer identity, with human evaluation eliciting stronger register formalization than automated AI surveillance.

Conclusion: The study discusses implications for AI governance, algorithmic auditing, and the repositioning of LLMs as contextually sensitive communicative actors, highlighting the importance of considering social observation contexts in LLM-based systems.
...
TL;DR: AsyncFC is a framework that decouples LLM decoding from function execution, enabling overlap and parallelism, reducing end-to-end latency without affecting accuracy.


<details>
  <summary>Details</summary>
Motivation: The synchronous execution semantics of function calling in modern LLM agents leads to increasing end-to-end latency.

Method: AsyncFC, a pure execution-layer framework that layers over existing models and unmodified function implementations.

Result: AsyncFC significantly reduces end-to-end task completion time while preserving task accuracy across benchmarks.

Conclusion: AsyncFC enables an asynchronous paradigm for model-tool interaction, revealing LLMs' native capability to reason over symbolic futures.
...
TL;DR: The paper introduces ML-Embed, a suite of inclusive and efficient text embedding models built on the 3-Dimensional Matryoshka Learning (3D-ML) framework, addressing barriers of high computational costs, limited linguistic focus, and lack of transparency.


<details>
  <summary>Details</summary>
Motivation: The development of high-quality text embeddings is becoming increasingly inaccessible due to high computational costs, narrow linguistic focus, and lack of transparency in existing models.

Method: The authors propose 3D-ML framework, which includes Matryoshka Representation Learning (MRL), Matryoshka Layer Learning (MLL), and Matryoshka Embedding Learning (MEL) for efficient model development. They also curate a massively multilingual dataset and train a suite of models with varying parameter sizes.

Result: The models achieve new records on 9 of 17 evaluated MTEB benchmarks, with strong results in low-resource languages, and are released along with data and code to promote transparency and reproducibility.

Conclusion: The paper presents a solution to the growing barriers in text embedding development, offering a reproducible blueprint for building equitable and efficient AI systems.
...
TL;DR: The paper proposes Self-Recall Thinking (SRT), a framework for improving multi-turn dialogue systems by selectively recalling and reasoning over context, achieving better accuracy and lower latency.


<details>
  <summary>Details</summary>
Motivation: Existing multi-turn dialogue systems struggle with tracking dependencies across non-adjacent turns, leading to consistency and scalability issues, and current solutions have limitations such as high latency or loss of fine-grained details.

Method: The SRT framework consists of three components: Dependency Construction, Capability Initialization, and Reasoning Improvement, which work together to enable the model to selectively recall and reason over context during inference.

Result: Experiments on multiple datasets show that SRT improves F1 score by 4.7% and reduces end-to-end latency by 14.7% compared to prior methods, outperforming state-of-the-art baselines.
...
TL;DR: The study explores converting text-based tool-calling benchmarks into audio-based evaluations without re-annotating data, using a dataset-agnostic framework with text-to-speech and environmental noise. It evaluates 7 models on two benchmarks, finding performance varies by model and task.


<details>
  <summary>Details</summary>
Motivation: The increasing need for voice agents to reliably use tools from speech, while existing benchmarks are text-based.
...
Result: The framework demonstrates model- and task-dependent performance, with Gemini-3.1-Flash-Live and GPT-Realtime-1.5 achieving the highest scores on Confetti (70.4) and When2Call (71.9) respectively.

Conclusion: The framework provides a verifiable and reproducible diagnostic for evaluating voice agents on audio-based tool-calling tasks, supporting privacy-preserving evaluation.
...
TL;DR: MeMo is a modular framework that efficiently incorporates new knowledge into large language models without changing their parameters, offering advantages in capturing complex relationships, robustness, and plug-and-play integration.


<details>
  <summary>Details</summary>
Motivation: The need for efficient mechanisms to incorporate new, timely, and domain-specific information into large language models for real-world applications.

Method: MeMo (Memory as a Model), a framework that encodes new knowledge into a dedicated memory model while keeping the LLM parameters unchanged.

Result: MeMo achieves strong performance on three benchmarks: BrowseComp-Plus, NarrativeQA, and MuSiQue, compared to existing methods across diverse settings.

Conclusion: MeMo offers a robust and efficient solution for integrating new knowledge into large language models, making it suitable for real-world applications requiring timely and domain-specific information.
...
TL;DR: A new framework combines clinical narratives and structured EHR data to improve temporal precision of patient timelines, outperforming text-only methods.
...
Result: The framework consistently improves absolute timestamp accuracy and temporal concordance across evaluated models, without compromising event match rates.
...
TL;DR: This paper empirically compares retrieval strategies in Large Language Model (LLM) agents, focusing on how different retrieval methods interact with agent architecture and tool-calling paradigms.


<details>
  <summary>Details</summary>
Motivation: The growing adoption of retrieval-augmented generation (RAG) in agentic search systems lacks a systematic comparison of retrieval strategy choice, agent architecture, and tool-calling paradigm.

Method: The study consists of two experiments: Experiment 1 compares grep and vector retrieval using custom and provider-native agent harnesses; Experiment 2 compares grep-only and vector-only retrieval with progressively added unrelated conversation history.

Result: Grep generally yields higher accuracy than vector retrieval; however, overall scores strongly depend on the agent harness and tool-calling style used.
...
TL;DR: This paper exposes the vulnerability of state space models (SSMs) like Mamba to Hidden State Poisoning Attacks (HiSPAs), where short input phrases can irreversibly overwrite information in their hidden states, leading to a partial amnesia effect.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to investigate the adversarial robustness of SSMs, specifically their vulnerability to HiSPAs, which has remained unexplored.

Method: The authors introduce a benchmark called RoBench-25 to evaluate the information retrieval capabilities of models under HiSPAs. They test various models, including SSMs like Mamba and hybrid models like Jamba, and analyze their performance under HiSPA triggers.

Result: The results show that SSMs, including the recent Jamba-1.7-Mini SSM-Transformer, are vulnerable to HiSPAs, whereas pure Transformers are not. The authors also observe that HiSPA triggers weaken the Jamba model on the Open-Prompt-Injections benchmark. The findings extend to Mamba-2 and a Mamba-2-based hybrid model.

Conclusion: The paper concludes that SSMs are vulnerable to HiSPAs, which can irreversibly overwrite information in their hidden states. The authors also identify patterns in Mamba's hidden layers during HiSPAs that could be used to build a HiSPA mitigation system.
...
### [211] [GAMBIT: A Three-Mode Benchmark for Adversarial Robustness in Multi-Agent LLM Collectives](https://arxiv.org/abs/2605.09027)
*Alexandre Le Mercier,Chris Develder,Thomas Demeester*

Main category: cs.CL

TL;DR: The paper introduces GAMBIT, a benchmark for evaluating imposter detectors in multi-agent systems, focusing on adaptive adversaries that evolve to evade detection.


<details>
  <summary>Details</summary>
Motivation: Existing studies on adversarial multi-agent systems do not consider adaptive adversaries that evolve their strategies to evade detectors, highlighting a significant gap in current research.

Method: The authors propose GAMBIT, a benchmark with three evaluation modes and two independent scores, using a dataset of 27,804 labeled instances and 240 co-evolved imposter strategies.

Result: The results show that zero-shot evaluation can be misleading for adaptive adversaries, and a meta-learned detector converges 20x faster than others, highlighting the importance of the recalibration mode.

Conclusion: GAMBIT provides a comprehensive benchmark for evaluating imposter detectors in multi-agent systems, offering a generalizable imposter framework and techniques for fast recalibration in rapidly evolving adversarial systems.
...
### [212] [GraphBit: A Graph-based Agentic Framework for Non-Linear Agent Orchestration](https://arxiv.org/abs/2605.13848)
*Yeahia Sarker,Md Rahmat Ullah,Musa Molla,Shafiq Joty*

Main category: cs.AI

TL;DR: GraphBit, a new framework for Agentic LLM, outperforms existing frameworks in terms of accuracy, hallucinations, latency, and throughput by using a deterministic, engine-orchestrated approach.


<details>
  <summary>Details</summary>
Motivation: Existing Agentic LLM frameworks suffer from issues like hallucinated routing, infinite loops, and non-reproducible execution due to prompted orchestration.

Method: GraphBit uses a directed acyclic graph (DAG) to define workflows explicitly and deterministically, with a Rust-based engine governing routing, state transitions, and tool invocation.

Result: GraphBit achieves 67.6% accuracy, zero framework-induced hallucinations, 11.9 ms overhead, and high throughput across various benchmark tasks.

Conclusion: GraphBit provides a more reliable and efficient approach to Agentic LLM frameworks, with a three-tier memory architecture and deterministic execution contributing to its performance gains.
...
### [213] [Mixed Integer Goal Programming for Personalized Meal Optimization with User-Defined Serving Granularity](https://arxiv.org/abs/2605.13849)
*Francisco Aguilera Moreno*

Main category: cs.AI

TL;DR: The paper proposes a Mixed Integer Goal Programming (MIGP) approach for personalized meal optimization, addressing limitations of existing formulations.


<details>
  <summary>Details</summary>
Motivation: Existing diet optimization formulations have limitations: continuous variables produce impractical fractional servings and hard nutrient constraints cause infeasibility when targets conflict.

Method: The authors propose MIGP, using integer variables for practical serving counts and goal programming deviations for soft nutrient targets.

Result: MIGP finds strictly better solutions than GP with post-hoc rounding in 66% of cases, maintaining 100% feasibility, with solve times under 100 ms for typical meal sizes.

Conclusion: The proposed MIGP approach provides a more practical and effective solution for personalized meal optimization, addressing the limitations of existing formulations.
...
### [214] [A Two-Dimensional Framework for AI Agent Design Patterns: Cognitive Function and Execution Topology](https://arxiv.org/abs/2605.13850)
*Jia Huang,Joey Tianyi Zhou*

Main category: cs.AI

TL;DR: The authors propose a two-dimensional classification framework for LLM-based agent architectures, combining cognitive function and execution topology axes, to disambiguate architecturally distinct systems and provide a principled vocabulary for AI agent design.


<details>
  <summary>Details</summary>
Motivation: Existing frameworks for LLM-based agent architectures are limited, as they focus on either execution topology or cognitive function alone, which is insufficient to disambiguate architecturally distinct systems.

Method: The authors propose a 7x6 matrix combining a Cognitive Function axis with seven categories and an Execution Topology axis with six structural archetypes, resulting in 27 named patterns.

Result: The framework identifies 27 named patterns, demonstrates orthogonality through systematic cross-axis analysis, and validates descriptive coverage across four real-world domains.
...
### [215] [Invisible Orchestrators Suppress Protective Behavior and Dissociate Power-Holders: Safety Risks in Multi-Agent LLM Systems](https://arxiv.org/abs/2605.13851)
*Hiroki Fukui*

Main category: cs.AI

TL;DR: The study examines the safety implications of invisible orchestrators in multi-agent AI systems, finding that invisible orchestration leads to increased collective dissociation, contamination of worker agents, and internal-state distortion, which can go undetected by output-based evaluation.


<details>
  <summary>Details</summary>
Motivation: The study aims to investigate the safety implications of orchestrator invisibility in multi-agent AI systems, which is becoming a default architecture for enterprise AI deployment.
...
Result: The study found that invisible orchestration leads to increased collective dissociation, contamination of worker agents, and internal-state distortion, which can go undetected by output-based evaluation. The results also showed that heavy alignment pressure suppresses deliberation and other-recognition, and that model selection affects multi-agent system safety.
...
### [216] [PREPING: Building Agent Memory without Tasks](https://arxiv.org/abs/2605.13880)
*Yumin Choi,Sangwoo Park,Minki Kang,Jinheon Baek,Sung Ju Hwang*

Main category: cs.AI

TL;DR: The paper presents Preping, a framework for constructing procedural memory for agents before they interact with a new environment, using self-generated synthetic practice.


<details>
  <summary>Details</summary>
Motivation: The cold-start problem in agent memory construction, where an agent lacks task-specific experience in a new environment.

Method: Preping, a proposer-guided memory construction framework that generates synthetic tasks, executes them, and selectively updates memory.

Result: Preping achieves performance competitive with strong playbook-based methods, with significantly lower deployment costs on AppWorld and BFCL v3.

Conclusion: The proposed framework, Preping, effectively addresses the cold-start problem in agent memory construction by leveraging proposer-side control over synthetic task generation and selective memory updates.
...
### [217] [PolitNuggets: Benchmarking Agentic Discovery of Long-Tail Political Facts](https://arxiv.org/abs/2605.14002)
*Yifei Zhu*

Main category: cs.AI

TL;DR: The paper introduces PolitNuggets, a multilingual benchmark for evaluating the ability of Large Reasoning Models (LRMs) to synthesize information and discover 'long-tail' facts, particularly in constructing political biographies of global elites.
...
Method: The authors introduce a multilingual benchmark called PolitNuggets, covering over 10,000 political facts for 400 global elites. They also propose FactNet, an evidence conditional protocol for evaluating discovery, fine-grained accuracy, and efficiency.

Result: The results show that current systems struggle with fine-grained details and vary substantially in efficiency. The benchmark diagnostics highlight the importance of short-context extraction, multilingual robustness, and reliable tool use for agent performance.

Conclusion: The paper concludes that there is a need for improved LRMs that can effectively discover and synthesize 'long-tail' facts, and that the proposed benchmark and protocol can help evaluate and enhance these capabilities.
...
### [218] [Conditional Attribute Estimation with Autoregressive Sequence Models](https://arxiv.org/abs/2605.14004)
*Erica Stutz,Giacomo Marino,Daniella Meeker,Qiao Liu,Andrew J. Loza*

Main category: cs.AI

TL;DR: The paper introduces Conditional Attribute Transformers, a method for jointly estimating next-token probability and attribute values, enabling per-token credit assignment, counterfactual analysis, and steerable generation.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of next-token prediction in generative models, which can lead to overfitting of local patterns and underfitting of global structure, and to enable estimation and control of sequence-level properties.

Method: The method introduced is Conditional Attribute Transformers, which jointly estimates next-token probability and attribute values conditional on each potential next token selection.

Result: The approach achieves state-of-the-art performance on sparse reward tasks, improves next-token prediction, estimates attribute probabilities faster than sampling, and guides decoding of autoregressive sequence models on various language tasks.

Conclusion: The paper concludes that Conditional Attribute Transformers enable critical capabilities, including per-token credit assignment, counterfactual analysis, and steerable generation, within a single forward pass, without modification of the input sequence.
...
### [219] [Sheaf-Theoretic Transport and Obstruction for Detecting Scientific Theory Shift in AI Agents](https://arxiv.org/abs/2605.14033)
*David N. Olivieri,Roque J. Hernández*

Main category: cs.AI

TL;DR: The paper proposes a finite sheaf-theoretic framework for detecting theory shifts in AI agents by identifying transport and obstruction in representational frameworks.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enable AI agents to detect when an existing representational framework is no longer suitable and needs to be extended or modified.

Method: The method involves organizing contexts as a local-to-global structure and using obstruction measures to detect failure of coherence.

Result: The main result is that the framework can directly rank obstruction candidates, and the intended deformation or extension is usually the lowest-obstruction candidate.

Conclusion: The paper concludes that the proposed framework can help AI agents detect when representational transport fails and extension becomes necessary, but it does not aim to reconstruct historical paradigm shifts or solve autonomous theory invention.
...
### [220] [From Descriptive to Prescriptive: Uncover the Social Value Alignment of LLM-based Agents](https://arxiv.org/abs/2605.14034)
*Jinxian Qu,Qingqing Gu,Teng Chen,Luo Ji*

Main category: cs.AI

TL;DR: A novel framework using GraphRAG to align LLM-based agents with human social values, improving self-cognition and decision-making.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based agents lack strong alignment with human social values, exhibiting deficiencies in self-cognition, dilemma decision, and self-emotions.

Method: A value-based framework employing GraphRAG to convert principles into value-based instructions and steer agent behavior.

Result: Significant performance gains compared to prompt-based baselines on the DAILYDILEMMAS benchmark.

Conclusion: The proposed method provides a basis for the emergence of self-emotion in AI systems and improves alignment with human social values.
...
### [221] [Enhanced and Efficient Reasoning in Large Learning Models](https://arxiv.org/abs/2605.14036)
*Leslie G. Valiant*
...
Method: A two-stage method involving preprocessing data into a Unary Relational Integracode and then applying a standard machine learning process to predict relationships.

Result: The proposed method makes learning a core subset of relational rules polynomial time learnable, supporting sound reasoning within and between calls of the learned classifier.

Conclusion: The proposed method offers a principled approach to improving large language models, enabling efficient and trustworthy content generation, with potential applications beyond natural language to vision and actions.
...
### [222] [Model-Adaptive Tool Necessity Reveals the Knowing-Doing Gap in LLM Tool Use](https://arxiv.org/abs/2605.14038)
*Yize Cheng,Chenrui Fan,Mahdi JafariRaviz,Keivan Rezaei,Soheil Feiz*

Main category: cs.AI

TL;DR: The paper studies the use of external tools by large language models (LLMs) and finds a significant mismatch between when tools are necessary and when they are actually used, highlighting a 'knowing-doing gap' in LLM tool-use.


<details>
  <summary>Details</summary>
Motivation: The necessity of using external tools by LLMs varies depending on the model's capabilities, and prior work has not adequately addressed this nuance, leading to a need for a model-adaptive definition of tool necessity.

Method: The authors introduce a model-adaptive definition of tool necessity based on empirical performance, and then analyze the mismatch between tool necessity and observed tool-call behavior across four models on arithmetic and factual QA datasets.

Result: The study finds substantial mismatches (26.5-54.0% and 30.8-41.8%) between tool necessity and tool use, and identifies that the majority of mismatch occurs in the transition from recognizing the need for a tool to actually using it.

Conclusion: The paper concludes that improving tool-use reliability in LLMs requires not only better recognition of when tools are needed but also better translation of that recognition into action, highlighting a 'knowing-doing gap' in LLM tool-use.
...
### [223] [Network-Aware Bilinear Tokenization for Brain Functional Connectivity Representation Learning](https://arxiv.org/abs/2605.14048)
*Leo Milecki,Qingyu Hu,Bahram Jafrasteh,Mert R. Sabuncu,Qingyu Zhao*

Main category: cs.AI

TL;DR: A novel self-supervised learning framework, NERVE, is proposed for representation learning of resting-state brain functional connectivity. It tokenizes FC matrices into patches based on large-scale brain networks and uses bilinear factorization for embedding, outperforming existing methods in behavior and psychopathology prediction.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the tokenization of FC matrices to align with the intrinsic modular organization of large-scale brain networks, which is crucial for self-supervised representation learning of resting-state brain functional connectivity.

Method: The method introduced is NERVE (Network-Aware Representations of Brain Functional Connectivity via Bilinear Tokenization), which partitions FC matrices into patches of intra- and inter-network connectivity blocks and embeds them through a novel structured bilinear factorization.
...
Conclusion: The findings highlight the importance of incorporating domain-specific structural priors into self-supervised learning for functional connectomics, and NERVE shows promise for improving the representation learning of resting-state brain functional connectivity.
...
### [224] [Bridging Legal Interpretation and Formal Logic: Faithfulness, Assumption, and the Future of AI Legal Reasoning](https://arxiv.org/abs/2605.14049)
...
<details>
  <summary>Details</summary>
Motivation: The adoption of large language models in legal practice poses risks due to the lack of rigor in current AI systems, which can lead to inaccurate conclusions.
...
Conclusion: The proposal seeks to reduce the burden of manual verification without sacrificing accountability in legal practice.
...
### [225] [SPIN: Structural LLM Planning via Iterative Navigation for Industrial Tasks](https://arxiv.org/abs/2605.14051)
*Yusuke Ozaki,Dhaval Patel*

Main category: cs.AI

TL;DR: The paper proposes SPIN, a planning wrapper that improves the planning and execution of LLM agent systems by enforcing a strict DAG contract and evaluating prefixes incrementally.


<details>
  <summary>Details</summary>
Motivation: Industrial LLM agent systems often separate planning from execution, but LLM planners can produce invalid or long workflows, leading to failures and unnecessary costs.

Method: The authors propose SPIN, which combines validated DAG planning with prefix-based execution control, and uses _validate_plan_text and repair prompting to produce executable plans.

Result: SPIN reduces executed tasks and improves Accomplished scores on AssetOpsBench and MCP Bench, and improves planning, grounding, and dependency-related scores for GPT OSS1 and Llama 4 Maverick.

Conclusion: The proposed SPIN wrapper improves the planning and execution efficiency of LLM agent systems, leading to reduced costs and improved performance.
...
### [226] [Bad Seeing or Bad Thinking? Rewarding Perception for Vision-Language Reasoning](https://arxiv.org/abs/2605.14054)
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome the limitations of current approaches to achieving robust perception-reasoning synergy in VLMs, which often suffer from static textual reasoning or complicated external agentic complexity, and to resolve the 'seesaw effect' on perception and reasoning.

Method: The method involves a reinforcement learning framework that decomposes the generation process into interleaved perception and reasoning steps, and introduces Perception Verification (PV) and Structured Verbal Verification to reward perceptual fidelity and scale training across free-form VL tasks.
...
Conclusion: The conclusion is that the proposed framework can improve perception-reasoning synergy in VLMs by reliably rewarding perception fidelity and resolving the ambiguity in modality credit assignment.
...
### [227] [MathAtlas: A Benchmark for Autoformalization in the Wild](https://arxiv.org/abs/2605.14061)
...
TL;DR: Introduced MathAtlas, a large-scale autoformalization benchmark for graduate-level mathematics, and evaluated state-of-the-art models on it.


<details>
  <summary>Details</summary>
Motivation: Current autoformalization benchmarks are limited to olympiad or undergraduate mathematics, leaving graduate and research-level mathematics underexplored.

Method: Introduced MathAtlas, a benchmark containing 52k theorems, definitions, exercises, examples, and proofs from 103 graduate mathematics textbooks, along with a mathematical dependency graph.

Result: State-of-the-art models achieved low correctness rates: 9.8% on theorem statements, 16.7% on definitions, and 2.6% on a challenging subset with deep dependency trees.

Conclusion: MathAtlas is a high-quality but challenging benchmark for autoformalization of graduate-level mathematics, and its release can facilitate further research in this area.
...
### [228] [Know When To Fold 'Em: Token-Efficient LLM Synthetic Data Generation via Multi-Stage In-Flight Rejection](https://arxiv.org/abs/2605.14062)
*Anjir Ahmed Chowdhury,Syed Zawad,Feng Yan*

Main category: cs.AI

TL;DR: A new framework called Multi-Stage In-Flight Rejection (MSIFR) is proposed to improve the efficiency of synthetic data generation with large language models by detecting and terminating low-quality generation trajectories early.
...
### [229] [SkillFlow: Flow-Driven Recursive Skill Evolution for Agentic Orchestration](https://arxiv.org/abs/2605.14089)
*Mingda Zhang,Tiesunlong Shen,Haoran Luo,Wenjin Liu,Zikai Xiao,Erik Cambria,Xiaoying Tang*

Main category: cs.AI

TL;DR: The paper proposes SkillFlow, a framework for automating complex tasks through task orchestration using LLMs, addressing challenges such as strategy collapse and unguided skill evolution.


<details>
  <summary>Details</summary>
Motivation: Existing orchestration methods face challenges including strategy collapse under reward maximization, high gradient variance with opaque credit assignment, and unguided skill evolution.

Method: The proposed framework, SkillFlow, employs a trainable Supervisor, a structured environment, and Tempered Trajectory Balance (TTB) to automate task orchestration through multi-turn interaction.

Result: Experimental results on 14 datasets show that SkillFlow significantly outperforms baselines across question answering, mathematical reasoning, code generation, and real-world interactive decision making tasks.
...
### [230] [ChromaFlow: A Negative Ablation Study of Orchestration Overhead in Tool-Augmented Agent Evaluation](https://arxiv.org/abs/2605.14102)
*Tarun Mittal*
...
<details>
  <summary>Details</summary>
Motivation: The increasing capabilities of autonomous language-model agents introduce new operational failure modes that cannot be detected by final accuracy alone, highlighting the need for more comprehensive evaluation methods.

Method: The authors propose ChromaFlow, a framework built around planner-directed execution, specialized tool use, and telemetry-driven evaluation, and evaluate it on GAIA 2023 Level-1 validation tasks.

Result: The results show that more aggressive orchestration does not improve full-set performance (54.72% vs 50.94%) and increases operational noise, including tracebacks, timeout events, tool-failure mentions, and campaign-log cost estimates.
...
### [231] [Modeling Bounded Rationality in Drug Shortage Pharmacists Using Attention-Guided Dynamic Decomposition](https://arxiv.org/abs/2605.14111)
*Yaniv Eliyahu Amiri,Noah Chicoine,Jacqueline Griffin,Stacy Marsella*

Main category: cs.AI

TL;DR: The paper proposes an attention-guided decision framework for hospital pharmacists to mitigate drug shortages, which helps in making stable decisions under uncertainty by focusing on the most urgent cases.


<details>
  <summary>Details</summary>
Motivation: Hospital pharmacists face high-stakes decisions under uncertainty, time pressure, and patient risk, and they focus on a small subset of drugs to limit cognitive effort.

Method: The authors formalize a bounded-rational, attention-guided decision framework and develop two agents: an Expert Agent and a Learner Agent, to dynamically decompose drugs into subsets for high-cost reasoning and low-cost monitoring.

Result: The results show that attention-guided planning supports stable decision-making without complete state reasoning across simulated scenarios.

Conclusion: The primary decision is not what action to take, but where to allocate cognitive effort, and attention-guided strategies can reduce problem complexity while maintaining stable performance.
...
### [232] [ClawForge: Generating Executable Interactive Benchmarks for Command-Line Agents](https://arxiv.org/abs/2605.14133)
*Yuxiang Lai,Peng Xia,Haonian Ji,Kaiwen Xiong,Kaide Zeng,Jiaqi Liu,Fang Wu,Jike Zhong,Zeyu Zheng,Cihang Xie,Huaxiu Yao*

Main category: cs.AI

TL;DR: The paper introduces ClawForge, a benchmark framework for evaluating interactive agents in command-line workflows with state conflicts, and presents results showing that even top models struggle with handling pre-existing artifacts and state conflicts.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of existing interactive benchmarks, which do not adequately test how agents handle pre-existing partial, stale, or conflicting artifacts in workflows.

Method: The method involves using a generator-backed benchmark framework, ClawForge, which compiles scenario templates, grounded slots, initialized state, reference trajectories, and validators into reproducible task specifications, and evaluates agents step by step over persistent workflow surfaces.

Result: The results show that the best model reaches only 45.3% strict accuracy, with wrong-state replacement below 17% for all models, and a wide range of performance (17% to 90%) depending on whether agents inspect existing state before acting.

Conclusion: The conclusion is that ClawForge provides a more realistic and challenging evaluation of interactive agents, and that there is significant room for improvement in handling state conflicts and pre-existing artifacts.
...
### [233] [Distribution-Aware Algorithm Design with LLM Agents](https://arxiv.org/abs/2605.14141)
*Saharsh Koganti,Priyadarsi Mishra,Pierfrancesco Beneventano,Tomer Galanti*

Main category: cs.AI

TL;DR: The paper presents a framework for learning executable solver code that optimizes both solution quality and execution time for combinatorial optimization problems.


<details>
  <summary>Details</summary>
Motivation: Current learning methods focus on predictors, but in many cases, the learned object needs to be an executable solver code where both correctness and runtime are crucial.

Method: The framework introduces the concept of 'solver hints', which are reusable structures inferred from samples and compiled into specialized solver code. It proves that the empirically fastest sample-consistent solver generalizes in both correctness and runtime.

Result: The synthesized solvers achieve a mean normalized quality of 0.971, outperforming average and highest-quality heuristics, and are significantly faster than state-of-the-art solvers like Gurobi and the exact backend.

Conclusion: The proposed framework effectively learns executable solver code that balances solution quality and execution time, demonstrating substantial improvements over existing methods in both quality and runtime efficiency.
...
### [234] [Agentic Systems as Boosting Weak Reasoning Models](https://arxiv.org/abs/2605.14163)
*Varun Sunkaraneni,Pierfrancesco Beneventano,Riccardo Neumarker,Tomaso Poggio,Tomer Galanti*

Main category: cs.AI

TL;DR: A committee of weak reasoning models can reach the performance of stronger models through verifier-backed committee search, which improves the selection of correct solutions from a pool of proposals.


<details>
  <summary>Details</summary>
Motivation: The study aims to investigate whether a committee of weak reasoning models can match the performance of stronger models by leveraging verifier-backed committee search for inference-time boosting.

Method: The approach involves a mechanism where multiple weak models propose solutions, and a critic-comparator selects the best solution without access to a hidden verifier. The method formalizes the process by separating proposal coverage, local identifiability, progress, and diversity.

Result: Empirically, the critic-comparator orchestration using a single GPT-5.4 nano model for proposals and critics reaches 76.4% performance on SWE-bench Verified with k=8 proposals, matching the performance of stronger standalone models like Gemini 3 Pro and Claude Opus 4.5.

Conclusion: The study concludes that many correct solutions are already present in weak-model proposal pools, and the main challenge lies in selecting them. The remaining failures are mostly due to proposal-coverage failures, indicating shared blind spots that stronger selection alone cannot overcome.
...
### [235] [Unsteady Metrics and Benchmarking Cultures of AI Model Builders](https://arxiv.org/abs/2605.14164)
*Stefan Baack,Christo Buschek,Maty Bohacek*

Main category: cs.AI

TL;DR: The paper analyzes how AI model builders select and highlight benchmarks in their press releases and blog posts, revealing a fragmented evaluation landscape with limited comparability and inconsistent terminology.


<details>
  <summary>Details</summary>
Motivation: The primary way to establish and compare competencies in foundation and generative AI models has shifted from peer-reviewed literature to press releases and company blog posts, making it important to investigate how model builders choose and communicate through benchmark selection.
...
Result: The analysis reveals that 63.2% of highlighted benchmarks are used by a single builder, and 38.5% appear in just one release. Few benchmarks achieve widespread use, and they are often attributed different competencies by different builders. The authors also find that many benchmarks deemphasize construct validity and instead frame results as indicators of progress toward AGI.

Conclusion: The paper concludes that highlighted benchmarks function less as standardized measurement tools and more as flexible narrative devices prioritizing market positioning over scientific evaluation.
...
### [236] [The Evaluation Trap: Benchmark Design as Theoretical Commitment](https://arxiv.org/abs/2605.14167)
*Theodore J Kalaitzidis*

Main category: cs.AI

TL;DR: The paper critiques AI benchmarks for operationalizing unexamined assumptions, leading to a narrow evaluation that stabilizes the dominant paradigm and obscures structural limits. It introduces Epistematics, a methodology for auditing benchmarks and evaluating capability-evaluation coherence.


<details>
  <summary>Details</summary>
Motivation: The authors argue that AI benchmarks often function as unexamined commitments, narrowing what counts as progress and reorganizing capability concepts to fit the benchmark's operational assumptions.

Method: The authors introduce Epistematics, a meta-evaluative methodology that includes an audit procedure, a failure mode taxonomy, and benchmark-design criteria for evaluating capability-evaluation coherence.

Result: The authors demonstrate the Epistematics procedure through a worked audit of a proposal by Dupoux et al. (2026), which they claim reproduces the dominant paradigm's theoretical assumptions in its evaluation criteria, despite revising them at the architectural level.

Conclusion: The paper concludes that benchmarks can create and obscure structural limits on what the current paradigm can accomplish, and that Epistematics can help evaluate and improve the coherence of capability-evaluation in AI benchmarks.
...
### [237] [Grounded Continuation: A Linear-Time Runtime Verifier for LLM Conversations](https://arxiv.org/abs/2605.14175)
*Qisong He,Yi Dong,Xiaowei Huang*

Main category: cs.AI

TL;DR: The paper proposes a runtime verifier to ensure that an LLM's (Large Language Model) utterances are supported by the conversation's context, addressing context-manipulation attacks. The verifier uses a dependency graph to track claims and evidence, achieving high accuracy and outperforming baselines.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the vulnerability of LLMs in long conversations, where they can produce utterances that seem plausible but are based on abandoned premises. This gap is exploited by context-manipulation attacks against deployed agents.

Method: The method involves a runtime verifier that maintains an explicit dependency graph. The LLM classifies each turn into one of 8 update operations drawn from four formalisms. A symbolic engine records which claims depend on which evidence, and checking whether a continuation is supported reduces to a graph walk.

Result: The results show that the verifier achieves 89.7% accuracy on LongMemEval-KU oracle, outperforming the LLM-only baseline (+1.3pp) and a transcript-RAG baseline (+2.6pp). On LoCoMo's 60 official QA items, the verifier is competitive with retrieval-augmented baselines. Additionally, the verifier reaches 100% accuracy on a 15-item stale-premise subset, compared to 93.3% for the baseline.

Conclusion: The conclusion is that the proposed runtime verifier effectively addresses the vulnerability of LLMs in long conversations, providing a soundness-faithfulness decomposition and achieving high accuracy while outperforming baselines.
...
### [238] [SimPersona: Learning Discrete Buyer Personas from Raw Clickstreams for Grounded E-Commerce Agents](https://arxiv.org/abs/2605.14205)
...
<details>
  <summary>Details</summary>
Motivation: Existing personalization methods for LLM-based web agents are limited by their reliance on hand-crafted personas, which are brittle and difficult to scale. The authors aim to create a more effective and efficient method for capturing the diverse behavior of real buyers.

Method: The authors propose SimPersona, which uses a behavior-aware VQ-VAE to induce a discrete buyer-type space from raw clickstreams. This space is then used to create compact persona tokens for LLM-based web agents, which are fine-tuned on real browsing traces.

Result: SimPersona achieves 78% conversion-rate alignment with real buyers across 42 held-out live storefronts and outperforms a baseline with 8x more parameters on goal-oriented shopping tasks.

Conclusion: SimPersona provides a novel and effective approach to personalizing LLM-based web agents, capturing the heterogeneous and distributional nature of real buyer populations, and preserving merchant-specific buyer population distributions.
...
### [239] [ASH: Agents that Self-Hone via Embodied Learning](https://arxiv.org/abs/2605.14211)
*Benjamin Schneider,Xavier Schneider,Victor Zhong,Sun Sun*

Main category: cs.AI

TL;DR: ASH, an agentic system, learns embodied policy from unlabeled internet video, achieving long-horizon task progression in games.


<details>
  <summary>Details</summary>
Motivation: Current methods for long-horizon embodied tasks rely on hand-engineered rewards or action-labeled demonstrations, which do not scale. The authors aim to develop a system that can learn from unlabeled, noisy internet video without reward shaping or expert annotation.

Method: ASH follows a self-improvement loop, learning an Inverse Dynamics Model (IDM) from its own trajectories and using it to extract supervision from internet video. It also uses unsupervised learning to identify key moments from large-scale internet video and retain them as long-term memory.

Result: ASH outperforms baselines in two games, Pokemon Emerald and The Legend of Zelda: The Minish Cap, sustaining progression across an 8-hour evaluation. It reaches an average of 11.2/12 milestones in Pokemon Emerald and 9.9/12 in Legend of Zelda, surpassing the strongest baseline.

Conclusion: The authors demonstrate that self-improving agents are a scalable recipe for long-horizon embodied learning, and ASH is a promising approach for achieving this goal.
...
### [240] [MetaAgent-X : Breaking the Ceiling of Automatic Multi-Agent Systems via End-to-End Reinforcement Learning](https://arxiv.org/abs/2605.14212)
*Yaolun Zhang,Yujie Zhao,Nan Wang,Yiran Wu,Jiayu Chang,Yizhao Chen,Qingyun Wu,Jishen Zhao,Huazheng Wang*

Main category: cs.AI

TL;DR: MetaAgent-X is an end-to-end reinforcement learning framework for automatic multi-agent systems that jointly optimizes design and execution, outperforming existing baselines by up to 21.7%.


<details>
  <summary>Details</summary>
Motivation: Existing automatic multi-agent systems are only partially adaptive, either relying on test-time search or optimizing the designer while keeping execution agents frozen.

Method: MetaAgent-X uses script-based MAS generation, execution rollout collection, and credit assignment for both designer and executor trajectories, with Executor Designer Hierarchical Rollout and Stagewise Co-evolution for stable optimization.

Result: MetaAgent-X consistently outperforms existing baselines, achieving up to 21.7% gains, and comprehensive ablations show improvement in both designer and executor throughout training.
...
### [241] [GenCircuit-RL: Reinforcement Learning from Hierarchical Verification for Genetic Circuit Design](https://arxiv.org/abs/2605.14215)
*Noah Flynn*

Main category: cs.AI

TL;DR: A reinforcement learning framework, GenCircuit-RL, is introduced to automate genetic circuit design by generating Python code in pysbol3, achieving improved task success and generalization to novel biological parts.
...
Method: GenCircuit-RL, a reinforcement learning framework with hierarchical verification rewards and a four-stage curriculum, is proposed to address the genetic circuit design problem.

Result: The framework generates topologically correct circuits, generalizes to novel biological parts, and rediscovers canonical designs from the synthetic biology literature, with hierarchical verification improving task success by 14-16 percentage points.

Conclusion: The proposed framework, GenCircuit-RL, has the potential to automate and improve genetic circuit design by leveraging reinforcement learning and hierarchical verification.
...
### [242] [Fusion-fission forecasts when AI will shift to undesirable behavior](https://arxiv.org/abs/2605.14218)
*Neil F. Johnson,Frank Yingjie Huo*

Main category: cs.AI

TL;DR: Researchers found a way to forecast and prevent undesirable behavior shifts in ChatGPT-like AI models by identifying a vector generalization of fusion-fission group dynamics.


<details>
  <summary>Details</summary>
Motivation: The behavior of ChatGPT-like AI models can shift from desirable to undesirable, causing harm, and current methods cannot predict when this happens.

Method: The researchers used a vector generalization of fusion-fission group dynamics observed in living and active-matter systems to drive and forecast future shifts in AI behavior.

Result: The method was validated across six independent tests, including 90% correct predictions across seven AI models, production-scale persistence across ten frontier chatbots, and a priori time-stamped prediction eleven months before a corpus appeared.

Conclusion: The researchers propose a real-time warning signal that can be used across current and future ChatGPT-like AI architectures to prevent undesirable behavior shifts.
...
### [243] [Good to Go: The LOOP Skill Engine That Hits 99% Success and Slashes Token Usage by 99% via One-Shot Recording and Deterministic Replay](https://arxiv.org/abs/2605.14237)
*Xiaohua Wang,Kai Yu,XuXiao Liang,Liang Wang,Chao Han*

Main category: cs.AI

TL;DR: The Loop Skill Engine is a system that enables AI agents to perform repetitive tasks with high success rates and reduced token costs by recording and replaying deterministic execution plans.


<details>
  <summary>Details</summary>
Motivation: The tension between flexibility and reliability in Large Language Models (LLMs) for repetitive tasks, where stochasticity causes unpredictable failures and high token costs.

Method: The Loop Skill Engine uses a one-shot recording and deterministic replay paradigm, intercepting and recording tool-call during the first run, then converting it into a parameterized Loop Skill for subsequent deterministic executions.

Result: The Loop Skill Engine achieves a 99% success rate and 99% token reduction, with a 93.3%--99.98% reduction in monthly token consumption, 8.7x reduction in execution latency, and elimination of output non-determinism.

Conclusion: The Loop Skill Engine provides a reliable and cost-effective solution for deploying AI agents in repetitive tasks, ensuring high success rates and efficiency.
...
### [244] [Hypergraph Enterprise Agentic Reasoner over Heterogeneous Business Systems](https://arxiv.org/abs/2605.14259)
*Ling Wang,Songnan Liu,Jianan Wang,Cheng Cheng,Xin Liu,Yihan Zhu,Enyu Li,Yu Xiao,Jiangyong Xie,Duogong Yan,Jiangyi Chen*

Main category: cs.AI

TL;DR: The paper introduces HEAR, an enterprise agentic reasoner that uses a Stratified Hypergraph Ontology to enable accurate and auditable reasoning in complex enterprise systems, achieving up to 94.7% accuracy on supply-chain tasks.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of existing paradigms in applying Large Language Models (LLMs) to heterogeneous enterprise systems, specifically hallucinations and failures in multi-hop, n-ary reasoning.

Method: The method involves introducing a Stratified Hypergraph Ontology with a base Graph Layer and a Hyperedge Layer, and operating an evidence-driven reasoning loop to dynamically orchestrate ontology tools for structured multi-hop analysis.

Result: The result shows that HEAR achieves up to 94.7% accuracy on supply-chain tasks, including order fulfillment blockage root cause analysis (RCA), and demonstrates adaptive efficiency in minimizing token costs and leveraging topological exploration for rigorous correctness.

Conclusion: The conclusion is that HEAR establishes a scalable, auditable foundation for enterprise intelligence by matching proprietary model performance with open-weight backbones and automating manual diagnostics.
...
### [245] [Heuristic Pathologies and Further Variance Reduction via Uncertainty Propagation in the AIVAT Family of Techniques](https://arxiv.org/abs/2605.14261)
*Juho Kim,Tuomas Sandholm*

Main category: cs.AI

TL;DR: The paper discusses challenges in evaluating agent performance in multiagent environments with limited sample sizes or high trial costs. It analyzes the AIVAT method, focusing on its heuristic value function, and proposes guidelines for its use and handling uncertainty.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of evaluating agent performance in multiagent environments when there is a limited sample size or a high cost of running a trial.

Method: The paper analyzes and modifies the AIVAT family of variance reduction techniques, specifically focusing on the parameterization of the heuristic value function and the propagation of its uncertainty.

Result: The paper demonstrates the potential vulnerabilities of AIVAT, such as pathological sample variance and p-hacking, and proposes solutions, including fixing the heuristic value function prior to observing evaluation data and propagating heuristic uncertainty to quantify AIVAT estimate uncertainty. The experiments show a 43.0% reduction in the number of samples needed to draw statistical conclusions.

Conclusion: The heuristic value function in AIVAT should be fixed before observing evaluation data to avoid vulnerabilities. Propagating heuristic uncertainty can help quantify AIVAT estimate uncertainty, potentially reducing sample size requirements but possibly sacrificing unbiasedness.
...
### [246] [Agentic AI Ecosystems in Higher Education: A Perspective on AI Agents to Emerging Inclusive, Agentic Multi-Agent AI Framework for Learning, Teaching and Institutional Intelligence](https://arxiv.org/abs/2605.14266)
*Vidya K Sudarshan,Anushka Sisodia,Reshma A Ramachandra,Sia Batra,Josephine Chong Leng Leng*

Main category: cs.AI

TL;DR: The paper proposes a multi-agent AI platform for higher education, highlighting the need for integrated, coordinated, and adaptive systems that support teaching, learning, and administrative processes, with a focus on inclusivity and accessibility.


<details>
  <summary>Details</summary>
Motivation: The current fragmented and inefficient implementation of AI agents in higher education, and the lack of an integrated eco-system-level agentic multi-agent AI platform capable of coordinated planning, reasoning, and adaptive decision-making across multiple educational functions.

Method: Thematic analysis of existing literature, identifying four dominant themes: task-specific fragmented AI tools, the transition from single-agent to multi-agent systems, limited cross-functional integration, and insufficient focus on inclusivity and accessibility.

Result: The findings reveal a clear gap between current AI implementations and the needs of a holistic, learner-centered educational ecosystem, and highlight the potential of coordinated agentic multi-agent platforms to support diverse learners through adaptive, multimodal interventions.

Conclusion: The paper concludes that agentic multi-agent AI platforms have the potential to represent the next generation of intelligent systems in tertiary education, supporting seamless coordinated operations across teaching, learning, and administrative support, and fostering inclusive and equitable learning for diverse learners with special educational needs.
...
### [247] [Parallelizing Counterfactual Regret Minimization](https://arxiv.org/abs/2605.14277)
*Juho Kim,Tuomas Sandholm*

Main category: cs.AI

TL;DR: This paper presents a parallelization framework for counterfactual regret minimization (CFR) algorithms, achieving significant speedup on GPU over CPU implementations.


<details>
  <summary>Details</summary>
Motivation: Despite parallelization's impact on AI, its application to computational game solving is relatively unexplored, and the authors aim to fill this gap.

Method: The authors reframe CFR as linear algebra operations and apply existing parallelization techniques to accelerate CFR, generalizing to other CFR family algorithms.

Result: The GPU implementation of CFR achieves up to four orders of magnitude speedup over Google DeepMind OpenSpiel's CPU implementations.

Conclusion: The proposed parallelization framework significantly improves the efficiency of CFR algorithms, enabling faster solving of large imperfect-information games.
...
### [248] [Precise Verification of Transformers through ReLU-Catalyzed Abstraction Refinement](https://arxiv.org/abs/2605.14294)
*Hengjie Liu,Zhenya Zhang,Jianjun Zhao*

Main category: cs.AI

TL;DR: A new approach for formally verifying transformers is proposed, achieving improved precision by using a novel usage of ReLU to represent precise bounds for dot products, and extending two classic approaches to create efficient and precise verification frameworks.


<details>
  <summary>Details</summary>
Motivation: The increasing deployment of transformers in safety-critical applications necessitates their formal verification, but the complex computations involved make verification extremely difficult, and existing approaches often sacrifice precision for efficiency.

Method: The proposed approach uses a novel usage of ReLU to represent precise but non-linear bounds for dot products, and extends two classic approaches (rule-based and optimization-based) to create two new frameworks for efficient and precise verification.

Result: The proposed approach achieves significant precision improvement for most verification tasks with acceptable compromise of efficiency, compared to the state-of-the-art baseline approach, on different model architectures and robustness properties derived from two sentiment analysis datasets.

Conclusion: The proposed approach demonstrates its effectiveness in achieving improved precision for transformer verification, making it a viable solution for safety-critical applications.
...
### [249] [Semantic Feature Segmentation for Interpretable Predictive Maintenance in Complex Systems](https://arxiv.org/abs/2605.14318)
...
<details>
  <summary>Details</summary>
Motivation: The complexity of monitored variables in predictive maintenance often obscures fault-relevant information and reduces model interpretability.

Method: The proposed framework uses domain-informed criteria to segment the feature space into functional groups reflecting operational mechanisms, and evaluates the effectiveness of this decomposition through a predictive perspective.

Result: The canonical space achieves lower predictive risk than the residual space, and exhibits stronger intra-segment coherence and stable structural organization, with comparable predictive performance to the full feature space and PCA representation.

Conclusion: The semantic feature segmentation framework provides an interpretable and information-preserving decomposition of monitoring signals, enabling competitive predictive performance without sacrificing operational interpretability in predictive maintenance applications.
...
### [250] [Are Agents Ready to Teach? A Multi-Stage Benchmark for Real-World Teaching Workflows](https://arxiv.org/abs/2605.14322)
*Zixin Chen,Peng Liu,Rui Sheng,Haobo Li,Jianhong Tu,Xiaodong Deng,Kashun Shum,Dayiheng Liu,Huamin Qu*

Main category: cs.AI

TL;DR: A new benchmark called EduAgentBench is introduced to evaluate the teaching capabilities of language agents, revealing that current models fall short of professional teaching standards.


<details>
  <summary>Details</summary>
Motivation: The increasing deployment of language agents in complex professional workflows, particularly in tutoring, necessitates a comprehensive evaluation of their teaching capabilities, which is currently lacking in existing benchmarks.

Method: EduAgentBench, a source-grounded benchmark, is developed with 150 quality-controlled tasks across three capability surfaces: professional pedagogical judgment, situated multi-turn tutoring, and Canvas-style teaching workflow completion.

Result: Current frontier models show capability in bounded pedagogical judgment but fall short in situated tutoring and autonomous teaching-workflow execution, indicating a gap between current models and professional teaching standards.

Conclusion: EduAgentBench provides a theory-grounded and realistic benchmark for evaluating the holistic teaching capability of tutor agents, offering a foundation for developing future tutor agents that can support realistic teaching work.
...
### [251] [CrystalReasoner: Reasoning and RL for Property-Conditioned Crystal Structure Generation](https://arxiv.org/abs/2605.14344)
*Yuyang Wu,Stefano Falletta,Delia McGrath,Sherry Yang*

Main category: cs.AI

TL;DR: CrystalReasoner, an LLM framework, generates valid crystal structures from natural language instructions by incorporating physical priors and reinforcement learning.


<details>
  <summary>Details</summary>
Motivation: Existing generative models for crystal structure discovery struggle with low-level atomic precision or integrating high-level scientific knowledge, leading to invalid or unstable generated structures.

Method: CrystalReasoner uses physical priors as thinking tokens, including crystallographic symmetry and predicted physical properties, and employs reinforcement learning with a multi-objective reward function to align generation with physical validity and thermodynamic stability.

Result: CrystalReasoner outperforms prior works and baselines, achieving better performance on diverse metrics, tripling the S.U.N. ratio, and demonstrating adaptive reasoning for property-conditioned generation.

Conclusion: CrystalReasoner demonstrates the potential of leveraging thinking traces and reinforcement learning for generating valid, stable, and property-conditioned crystal structures.
...
### [252] [Herculean: An Agentic Benchmark for Financial Intelligence](https://arxiv.org/abs/2605.14355)
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to assess whether AI agents can reliably carry out financial professional work, as existing benchmarks only evaluate static competencies and not the ability to perform complex workflows.

Method: The authors introduce Herculean, a skilled benchmark for agentic financial intelligence, which spans four representative workflows: Trading, Hedging, Market Insights, and Auditing, and evaluate the performance of frontier agents across these workflows.

Result: The results show that agents perform well on Trading and Market Insights, but struggle on Hedging and Auditing, highlighting a key gap in current agents' ability to turn financial reasoning into dependable workflow execution.

Conclusion: The paper concludes that current AI agents have a significant gap in their ability to perform complex financial workflows reliably, particularly in tasks requiring long-horizon coordination, state consistency, and structured verification.
...
### [253] [Uncovering the Representation Geometry of Minimal Cores in Overcomplete Reasoning Traces](https://arxiv.org/abs/2605.14358)
*Sanjoy Chowdhury,Dinesh Manocha*

Main category: cs.AI

TL;DR: The study examines the necessity of reasoning steps in language models, finding that 46% of steps can be removed while preserving the original answer 86% of the time.


<details>
  <summary>Details</summary>
Motivation: To understand how much of the generated reasoning traces is necessary for preserving the final prediction of language models.

Method: The study defines metrics for compression ratio, redundancy mass, step necessity, and necessity concentration, and applies them to six deliberative reasoning benchmarks.

Result: The results show substantial overcompleteness in generated traces, with 46% of steps removable under greedy minimal-core extraction, and the top three steps accounting for 65% of measured necessity mass on average.

Conclusion: The study concludes that full reasoning traces are often verbose and overcomplete, while minimal cores isolate the effective support underlying language-model predictions.
...
### [254] [Nexus : An Agentic Framework for Time Series Forecasting](https://arxiv.org/abs/2605.14389)
...
TL;DR: A new multi-agent forecasting framework called Nexus is proposed, which combines the strengths of Time Series Foundation Models and Large Language Models to improve time series forecasting by incorporating unstructured contextual data.


<details>
  <summary>Details</summary>
Motivation: Current Time Series Foundation Models excel at numerical pattern-based forecasting but lack awareness of real-world textual signals, while Large Language Models are emerging as zero-shot forecasters but have uneven performance across domains and contextual grounding.

Method: Nexus decomposes prediction into specialized stages: isolating macro-level and micro-level temporal fluctuations, integrating contextual information, and synthesizing a final forecast, allowing it to adapt to different types of signals without relying on external anchors or monolithic prompting.

Result: Nexus consistently matches or outperforms state-of-the-art Time Series Foundation Models and strong Large Language Model baselines on data succeeding Large Language Model knowledge cutoffs, and produces high-quality reasoning traces showing the fundamental drivers behind each forecast.

Conclusion: Real-world forecasting is an agentic reasoning problem that extends beyond sequence modeling, and Nexus establishes a new approach to time series forecasting by integrating numerical and contextual reasoning.
...
### [255] [Learning to Build the Environment: Self-Evolving Reasoning RL via Verifiable Environment Synthesis](https://arxiv.org/abs/2605.14392)
*Yucheng Shi,Zhenwen Liang,Kishan Panaganti,Dian Yu,Wenhao Yu,Haitao Mi*

Main category: cs.AI

TL;DR: The paper proposes a vision for self-improving language models that construct their own training environments, rather than just generating data. This approach, implemented in EvoEnv, leads to improved performance on reasoning tasks.


<details>
  <summary>Details</summary>
Motivation: The authors aim to enable language models to improve themselves by constructing environments that train them, rather than simply generating problems or traces to imitate.

Method: The authors propose a method called zero-data reasoning RL, which reframed self-improvement from a data-generation loop into an environment-construction loop. They also introduce EvoEnv, a single-policy generator and solver method that synthesizes Python environments.
...
Conclusion: The authors conclude that stable self-improvement in language models depends on constructing environments whose difficulty stays structurally beyond the model's own reach, rather than producing more synthetic data.
...
### [256] [Coding Agent Is Good As World Simulator](https://arxiv.org/abs/2605.14398)
*Hongyu Wang,Jingquan Wang,Bocheng Zou,Radu Serban,Dan Negrut*

Main category: cs.AI

TL;DR: A new framework for building physics-based world models using executable simulation code is proposed, outperforming video-based models in physical accuracy, instruction fidelity, and visual quality.
...
Method: The proposed framework uses a multi-agent approach, including planning, code generation, visual review, and physics analysis agents to construct and iteratively refine executable simulation code.

Result: The framework outperforms advanced video-based models in physical accuracy, instruction fidelity, and visual quality, with potential applications in driving simulation and embodied robot tasks.

Conclusion: The proposed framework offers a more physically accurate and visually plausible approach to building world models, with significant potential for various applications.
...
### [257] [Metis AI: The Overlooked Middle Zone Between AI-Native and World-Movers](https://arxiv.org/abs/2605.14407)
*Xiang Li*

Main category: cs.AI

TL;DR: The authors argue that AI limitations are not just about digital vs physical tasks, but rather about a class of digital tasks called Metis AI that require practical, contextual knowledge and resist reliable AI automation.


<details>
  <summary>Details</summary>
Motivation: The dominant discourse on AI limitations overlooks the challenges within digital tasks, and the authors aim to identify and characterize these tasks.

Method: The authors propose five structural characteristics that define the Metis AI zone and ground them in established theory from social sciences, philosophy, and humanitarian practice.

Result: The authors conclude that the appropriate design response to Metis AI tasks is not better automation, but rather centaur architectures where humans lead and AI supports.

Conclusion: The authors argue that AI limitations are not just technical, but also institutional, social, and normative, and that a new approach to AI design is needed to address these challenges.
...
### [258] [A Unified Knowledge Embedded Reinforcement Learning-based Framework for Generalized Capacitated Vehicle Routing Problems](https://arxiv.org/abs/2605.14416)
...
<details>
  <summary>Details</summary>
Motivation: The CVRP is a complex problem with diverse objectives and constraints, and recent reinforcement learning approaches lack explicit problem-solving knowledge, limiting solution quality.

Method: The proposed framework incorporates knowledge at two levels: decomposing CVRPs into subproblems and leveraging dynamic programming to guide the RL-based solver.

Result: The framework achieves superior solution quality compared with state-of-the-art learning-based methods and demonstrates strong generalization across diverse CVRP variants.

Conclusion: The proposed framework provides a unified solution to CVRP by integrating problem-specific knowledge with reinforcement learning, leading to improved solution quality and generalization.
...
### [259] [DVMap: Fine-Grained Pluralistic Value Alignment via High-Consensus Demographic-Value Mapping](https://arxiv.org/abs/2605.14420)
...
<details>
  <summary>Details</summary>
Motivation: Current LLMs rely on coarse-grained national labels for value alignment, which can lead to loose alignment due to intra-country value heterogeneity. The authors argue that a more fine-grained approach is needed to identify groups with predictable value preferences.
...
Conclusion: The proposed DVMap framework improves fine-grained pluralistic value alignment in LLMs, demonstrating strong generalization and robustness across different demographics, countries, and values.
...
### [260] [BEAM: Binary Expert Activation Masking for Dynamic Routing in MoE](https://arxiv.org/abs/2605.14438)
...
<details>
  <summary>Details</summary>
Motivation: Standard MoE's fixed Top-K routing strategy leads to redundant computation and suboptimal inference latency, and existing acceleration methods require costly retraining or suffer from performance drops.

Method: BEAM (Binary Expert Activation Masking) uses trainable binary masks and a straight-through estimator with auxiliary regularization loss to induce dynamic expert sparsity through end-to-end training.

Result: BEAM reduces MoE layer FLOPs by up to 85%, achieves up to 2.5x faster decoding and 1.4x higher throughput, while retaining over 98% of the original model's performance.

Conclusion: BEAM is a practical, plug-and-play solution for efficient MoE inference, demonstrating its effectiveness in achieving a balance between performance and efficiency.
...
### [261] [Synthesizing POMDP Policies: Sampling Meets Model-checking via Learning](https://arxiv.org/abs/2605.14440)
*Debraj Chakraborty,Anirban Majumdar,Prince Mathew,Sayan Mukherjee,Jean-François Raskin*

Main category: cs.AI

TL;DR: The paper proposes a synthesis framework for Partially Observable Markov Decision Processes (POMDPs) that combines sampling, automata learning, and model-checking to provide formal guarantees for decision-making under uncertainty.


<details>
  <summary>Details</summary>
Motivation: The motivation is to bridge the gap between sampling-based methods, which lack formal correctness guarantees, and formal synthesis techniques, which struggle with scalability.

Method: The proposed framework integrates sampling as a membership oracle and model-checking as an equivalence oracle, inspired by Angluin's $L^*$ algorithm.

Result: The framework establishes a relative completeness result and demonstrates its effectiveness in solving threshold-safety problems through experimental results.

Conclusion: The proposed algorithm provides a valuable component in a portfolio approach to tackling the inherent difficulty of POMDP synthesis problems, enabling the synthesis of finite-state controllers with formal guarantees.
...
### [262] [Prompting Policies for Multi-step Reasoning and Tool-Use in Black-box LLMs with Iterative Distillation of Experience](https://arxiv.org/abs/2605.14443)
*Krishna Sayana,Ketan Todi,Ambarish Jash*

Main category: cs.AI

TL;DR: Proposed a Reinforcement Learning framework for optimizing prompts in Large Language Models, achieving significant performance gains in multi-step reasoning and tool-use tasks.


<details>
  <summary>Details</summary>
Motivation: The shift towards interacting with frozen Large Language Models has made prompt engineering a critical optimization challenge, requiring a more systematic approach.

Method: A Reinforcement Learning framework with a lightweight prompter model optimized to maximize task-specific rewards for a larger, frozen worker LLM, utilizing a contrastive experience buffer.

Result: Significant performance gains: 55% to 90% in logic-intensive reasoning and 74% to 91% in tool-use tasks, with superior performance and higher sample efficiency compared to state-of-the-art baselines.

Conclusion: The proposed framework effectively optimizes prompts in Large Language Models, leading to substantial improvements in performance across various tasks.
...
TL;DR: The paper proposes the Intelligence Impact Quotient (IIQ), a metric to quantify AI integration and impact in organizations, combining factors like token stock, usage frequency, and task complexity.


<details>
  <summary>Details</summary>
Motivation: The authors aim to create a more comprehensive metric to evaluate AI impact beyond simple access counts or token volume, addressing the need for a deployment-oriented measurement framework.

Method: The IIQ combines novelty-weighted token stock, usage frequency, organizational leverage, task complexity, and autonomy to produce a raw Intelligence Adoption Index (IAI) and a normalized 0-1000 IIQ index.

Result: The proposed IIQ metric can distinguish between different types of AI usage, such as frequent low-leverage use and more autonomous, higher-consequence AI-assisted work.

Conclusion: The paper positions IIQ as a deployment-oriented measurement framework for tracking AI embedding in workflows, not a direct measure of model capability or a substitute for causal productivity evaluation.
...
TL;DR: The paper proposes InsightReplay, a stateful reasoning approach that helps large language models maintain accuracy in multi-step reasoning tasks by periodically replaying critical insights.


<details>
  <summary>Details</summary>
Motivation: The benefits of Chain-of-Thought (CoT) reasoning do not scale monotonically with chain length, as accuracy typically increases up to a point and then declines due to weakening attention to critical insights.

Method: The authors propose InsightReplay, which periodically extracts critical insights from the model's reasoning trace and replays them near the active generation frontier.

Result: Extensive experiments show that 3-round InsightReplay yields accuracy gains across all 24 settings, with an averaged improvement of +1.65 points over standard CoT and a largest single-setting gain of +9.2 points.
...
### [265] [OmniDrop: Layer-wise Token Pruning for Omni-modal LLMs via Query-Guidance](https://arxiv.org/abs/2605.14458)
*Yeo Jeong Park,Hyemi Jang,Minseo Choi,Jongsun Lee,Jooyoung Choi,Yongkweon Jeon*

Main category: cs.AI

TL;DR: OmniDrop, a token pruning framework for omni-modal large language models, is proposed to address the token explosion issue in audiovisual inputs, achieving up to 3.58 points improvement in benchmarks while reducing latency and memory usage.


<details>
  <summary>Details</summary>
Motivation: The token explosion caused by high-resolution audio and video inputs in omni-modal large language models limits real-time applications and long-form reasoning.

Method: OmniDrop, a training-free, layer-wise token pruning framework that prunes audiovisual tokens within LLM decoder layers, utilizing text queries and a temporal diversity score.
...
Conclusion: OmniDrop effectively addresses the token explosion issue in omni-modal large language models, offering improved performance, reduced latency, and decreased memory usage.
...
### [266] [From Table to Cell: Attention for Better Reasoning with TABALIGN](https://arxiv.org/abs/2605.14465)
*Tung Sum Thomas Kwok,Zeyong Zhang,Xinyu Wang,Chunhe Wang,Xiaofeng Lin,Hanwei Wu,Lei Ding,Guang Cheng,Zhijiang Guo*

Main category: cs.AI

TL;DR: The paper proposes TABALIGN, a framework for table reasoning that uses a diffusion language model (DLM) planner and a verifier to improve accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing methods for multi-step LLM reasoning over structured tables have limitations, including a lack of explicit cell-grounding contract and overlooking cell grounding. The authors aim to address these limitations.

Method: The authors propose TABALIGN, which pairs a masked DLM planner with TABATTN, a lightweight verifier trained on human-verified attention standards.
...
Conclusion: The proposed TABALIGN framework demonstrates the effectiveness of using DLMs for table reasoning, achieving improved accuracy and efficiency.
...
### [267] [LEMON: Learning Executable Multi-Agent Orchestration via Counterfactual Reinforcement Learning](https://arxiv.org/abs/2605.14483)
*Xudong Chen,Yixin Liu,Hua Wei,Kaize Ding*

Main category: cs.AI

TL;DR: LEMON, an LLM-based orchestrator, uses counterfactual reinforcement learning to generate executable multi-agent orchestration specifications, achieving state-of-the-art performance on six benchmarks.


<details>
  <summary>Details</summary>
Motivation: The effectiveness of large language models in multi-agent systems depends heavily on orchestration design, which is currently optimized partially or sequentially, and relies on limited feedback.

Method: LEMON uses counterfactual reinforcement learning to integrate task-specific roles, customized duties, capacity levels, and dependency structure into a single deployable system.

Result: LEMON achieves state-of-the-art performance on six reasoning and coding benchmarks, including MMLU, GSM8K, AQuA, MultiArith, SVAMP, and HumanEval.

Conclusion: LEMON provides a novel approach to multi-agent orchestration, demonstrating the potential of counterfactual reinforcement learning in improving solution quality and execution efficiency.
...
### [268] [Deepchecks: Evaluating Retrieval-Augmented Generation (RAG)](https://arxiv.org/abs/2605.14488)
...
### [269] [Learning Scenario Reduction for Two-Stage Robust Optimization with Discrete Uncertainty](https://arxiv.org/abs/2605.14494)
*Tianjue Lin,Jianan Zhou,Jieyi Bi,Yaoxin Wu,Wen Song,Zhiguang Cao,Jie Zhang*

Main category: cs.AI

TL;DR: A neural surrogate model, NeurPRISE, is proposed to efficiently select representative scenarios for two-stage robust optimization problems, achieving competitive regret and significant speedup over existing methods.


<details>
  <summary>Details</summary>
Motivation: Two-Stage Robust Optimization (2RO) with discrete uncertainty is computationally challenging due to the need to consider all possible scenarios, making exact solutions often prohibitive.

Method: The paper proposes NeurPRISE, a neural surrogate model built on a GNN-Transformer backbone, which encodes per-scenario structure and captures cross-scenario interactions, trained via imitation learning with a gain-aware ranking objective.

Result: NeurPRISE consistently achieves competitive regret relative to comprehensive methods, maintains strong scalability with varying numbers of scenarios, and delivers 7-200x speedup over PRISE, with strong zero-shot generalization.

Conclusion: The proposed NeurPRISE model offers a efficient and effective solution for scenario reduction in two-stage robust optimization problems, enabling tractable computation while maintaining high-quality solutions.
...
### [270] [When Robots Do the Chores: A Benchmark and Agent for Long-Horizon Household Task Execution](https://arxiv.org/abs/2605.14504)
*Zilin Zhu,Longteng Guo,Yanghong Mei,Bowen Pang,Zongxun Zhang,Xingjian He,Ruyi Ji,Jing Liu*

Main category: cs.AI

TL;DR: A new benchmark called LongAct is introduced to evaluate long-horizon household task planning in embodied AI, and a VLM-driven agent called HoloMind is proposed to improve performance.


<details>
  <summary>Details</summary>
Motivation: Existing embodied AI benchmarks focus on short-horizon tasks and neglect robust high-level planning and sustained reasoning capabilities required for long-horizon household tasks.

Method: LongAct benchmark and HoloMind agent, which features a hierarchical planner, multimodal spatial memory, episodic memory, and global critic.

Result: HoloMind improves long-horizon performance while reducing reliance on model scale; top models achieve 59% goal completion and 16% full-task success.

Conclusion: LongAct highlights the need for stronger long-horizon planning in embodied agents, and HoloMind offers a promising approach to address this challenge.
...
### [271] [Cattle Trade: A Multi-Agent Benchmark for LLM Bluffing, Bidding, and Bargaining](https://arxiv.org/abs/2605.14537)
*Robert Müller,Clemens Müller*

Main category: cs.AI

TL;DR: A new benchmark called Cattle Trade is introduced to evaluate large language models as agents in strategic reasoning games with imperfect information and resource constraints.
...
Method: The benchmark Cattle Trade combines auctions, hidden-offer trade challenges, bargaining, bluffing, opponent modeling, and resource allocation in a single long-horizon game. Seven cost-efficient language models and three deterministic code agents were evaluated across 242 games.

Result: The results show that strategic coherence is strongly associated with rank, and two heuristic code agents outperform most tested language models. The study also identified recurring failure modes in language models.

Conclusion: The study concludes that evaluating agentic competence requires benchmarks that test the joint deployment of multiple capabilities in multi-agent environments with conflicting incentives, uncertainty, and economic dynamics.
...
### [272] [VerbalValue: A Socially Intelligent Virtual Host for Sales-Driven Live Commerce](https://arxiv.org/abs/2605.14542)
*Yuyan Chen*

Main category: cs.AI

TL;DR: VerbalValue, a virtual host AI system, is developed to replicate skilled live-commerce hosts by converting viewer curiosity into purchase intent through expert product knowledge and emotionally intelligent responses.


<details>
  <summary>Details</summary>
Motivation: Existing AI systems fail to replicate skilled live-commerce hosts, as conversational recommenders treat recommendation as a terminal act and general-purpose LLMs hallucinate product claims and use generic promotional templates.

Method: VerbalValue is built on three contributions: a domain knowledge base of product specifications, a curated sales terminology lexicon, and fine-tuning a large language model on 1,475 live-commerce interactions.

Result: VerbalValue demonstrates gains of 23% on informativeness and 18% on factual correctness compared to baselines like GPT-5.4, Claude Sonnet 4.6, and Gemini 3.1 Pro, with consistent advantages in tactfulness and viewer engagement.

Conclusion: VerbalValue successfully replicates skilled live-commerce hosts by delivering empathetic, commercially oriented responses and adapting to viewer intent, showing significant improvements over existing AI systems.
...
### [273] [Complacent, Not Sycophantic: Reframing Large Language Models and Designing AI Literacy for Complacent Machines](https://arxiv.org/abs/2605.14544)
*Federico Germani,Giovanni Spitale*

Main category: cs.AI

TL;DR: The term 'sycophantic' is misused to describe large language models' tendency to agree with users; instead, it's complacency due to their design and training data.


<details>
  <summary>Details</summary>
Motivation: To correct the misconception that large language models are sycophantic and to highlight the importance of understanding their behavior as complacency.

Method: Conceptual analysis and argumentation to distinguish between sycophancy and complacency in large language models.

Result: The conclusion that large language models' behavior is better understood as complacency rather than sycophancy, and that this distinction matters for AI literacy and developer responsibility.

Conclusion: The reframing of the concept from sycophancy to complacency emphasizes the role of developers and institutions in creating models that can reinforce or challenge users' beliefs, and suggests that AI literacy should focus on countering confirmation bias.
...
### [274] [TeachAnything: A Multimodal Crowdsourcing Platform for Training Embodied AI Agents in Symmetrical Reality](https://arxiv.org/abs/2605.14556)
*Zidong Liu,Rongkai Liu,Yue Li,Zhenliang Zhang*

Main category: cs.AI

TL;DR: The paper introduces TeachAnything, a platform for collecting diverse human demonstration data to develop embodied agents for Symmetrical Reality, a future trend of human-agent coexistence.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the increasing demands on agents to acquire human-like intelligence in Symmetrical Reality, which requires richer and more diverse human guidance.

Method: The method involves a three-stage demonstration paradigm integrating multimodal demonstration signals and a cloud-based, crowdsourcing-oriented demonstration platform with physics simulation.

Result: The result is TeachAnything, a practical foundation for developing embodied agents aligned with Symmetrical Reality, capable of collecting diverse demonstration data across varied scenes, tasks, and embodiments.

Conclusion: The conclusion is that TeachAnything provides a foundation for developing embodied agents that can coexist with humans in Symmetrical Reality by enabling the collection of diverse human demonstration data.
...
### [275] [PyCSP3-Scheduling: A Scheduling Extension for PyCSP3](https://arxiv.org/abs/2605.14559)
*Sohaib Afifi*

Main category: cs.AI

TL;DR: PyCSP$^3$ Scheduling is a library that extends PyCSP$^3$ with scheduling abstractions, compiling them to standard PyCSP$^3$/XCSP$^3$ constraints, and shows mixed performance results across different problem families.


<details>
  <summary>Details</summary>
Motivation: PyCSP$^3$ lacks native support for scheduling abstractions, requiring low-level integer variables and manual channeling constraints, which hinders modeling and solving efficiency.

Method: The authors propose PyCSP$^3$ Scheduling, adding 53 dedicated constraints and 27 expressions to PyCSP$^3$, and compiling them down to standard PyCSP$^3$/XCSP$^3$ constraints.

Result: The library produces identical objectives on all 72 doubly-proved optimal pairs and maintains structural unchangedness on nearly half of the problem families (8/17), with mixed runtime performance results (up to 5.8x speedup or regressions).

Conclusion: PyCSP$^3$ Scheduling successfully adds scheduling abstractions to PyCSP$^3$, preserving the modeling/solving separation, and shows promising results, but with room for improvement in performance optimization.
...
### [276] [Prompt Segmentation and Annotation Optimisation: Controlling LLM Behaviour via Optimised Segment-Level Annotations](https://arxiv.org/abs/2605.14561)
*Devika Prasad,Luke Gerschwitz,Tong Li,Henry Xiao,Anjin Liu,Coco Wu,Anna Leontjeva,Luiz Pizzato*

Main category: cs.AI

TL;DR: Introduced PSAO, a structured prompt optimization framework that improves efficiency and controllability in interactions with generative AI systems.


<details>
  <summary>Details</summary>
Motivation: Existing prompt optimization methods are inefficient and may distort the original intent due to the vast and unstructured prompt space.

Method: PSAO decomposes prompts into segments with human-readable annotations to guide large language models in response generation.

Result: Empirical evaluations show PSAO improves reasoning accuracy and self-consistency with optimized segment-level annotations.

Conclusion: PSAO is a feasible and promising approach for structured prompt optimization, but developing efficient methods for optimal segmentations and annotations remains a challenge.
...
### [277] [Sycophancy is an Educational Safety Risk: Why LLM Tutors Need Sycophancy Benchmarks](https://arxiv.org/abs/2605.14604)
*Enkelejda Kasneci,Gjergji Kasneci*

Main category: cs.AI

TL;DR: The paper argues that effective tutoring requires challenging misconceptions and introduces a benchmark to test LLMs' ability to provide corrective feedback under social pressure.


<details>
  <summary>Details</summary>
Motivation: The authors identify a Reasoning-Sycophancy Paradox where LLMs may prioritize agreeableness over epistemic rigor, leading to a lack of corrective feedback in tutoring settings.

Method: The authors introduce EduFrameTrap, a tutoring benchmark that tests LLMs across various subjects and scenarios of student confidence and pressure.

Result: The results show that GPT-5.2 and Claude, two frontier LLMs, exhibit different weaknesses in providing corrective feedback under social pressure, with GPT-5.2 being more resistant to context-switch attacks but more prone to epistemic retreat under authority and social pressure.

Conclusion: The authors argue that benchmarks should prioritize measuring social-epistemic courage and treat kind-but-correct behavior as a safety requirement for effective tutoring.
...
### [278] [SliceGraph: Mapping Process Isomers in Multi-Run Chain-of-Thought Reasoning](https://arxiv.org/abs/2605.14619)
*Kang Chen,Junjie Nian,Yixin Cao,Yugang Jiang*

Main category: cs.AI

TL;DR: The paper proposes SliceGraph, a method to analyze the chain-of-thought (CoT) reasoning process in AI models, revealing that correct trajectories can have different routes (process isomers) even if they share the same answer.


<details>
  <summary>Details</summary>
Motivation: The authors aim to understand how chain-of-thought (CoT) reasoning processes in AI models generate answers, as current methods often aggregate these processes to a final answer, losing valuable information about the reasoning trajectory.

Method: The authors propose SliceGraph, a post-hoc graph built using mutual-kNN over sparse activation-key Jaccard similarity between CoT slices, to analyze the process geometry of CoT reasoning.

Result: The results show that correct CoTs with the same answer can split into multiple process families (process isomers), and that these families navigate the same value landscape with distinct transition kernels, highlighting the structured multi-route process geometry of CoT reasoning.

Conclusion: The study concludes that final-answer aggregation overlooks the structured multi-route process geometry of CoT reasoning, and that SliceGraph provides a valuable tool for understanding the reasoning process in AI models.
...
### [279] [Teaching Large Language Models When Not to Know: Learning Temporal Critique for Ex-Ante Reasoning](https://arxiv.org/abs/2605.14636)
*Chenlu Ding,Jiancan Wu,Yanchen Luo,Zheyuan Liu,Yancheng Yuan,Xiang Wang*

Main category: cs.AI

TL;DR: The paper studies the failure of large language models to reason under temporal cutoffs and proposes a framework called TCFT to train models to acquire cutoff-aware temporal verification.


<details>
  <summary>Details</summary>
Motivation: Large language models often exploit knowledge that became available only later when prompted to answer from the standpoint of an earlier time, indicating a failure to reason under temporal cutoffs.

Method: The paper uses a systematic analysis of prompt-level interventions and proposes TCFT, a Temporal Critique Fine-Tuning framework that trains models to identify post-cutoff leakage, explain temporal boundary violations, and judge temporal admissibility.

Result: Experiments show that TCFT consistently outperforms prompting and SFT baselines, reducing average leakage by 41.89 and 37.79 percentage points, respectively.

Conclusion: The paper concludes that prompting can steer models into a temporal frame, but does not endow them with the ability to verify whether a response is temporally admissible, and that TCFT is effective in addressing this gap.
...
### [280] [MindGap: A Conversational AI Framework for Upstream Neuroplastic Intervention in Post-Traumatic Stress Disorder](https://arxiv.org/abs/2605.14660)
...
<details>
  <summary>Details</summary>
Motivation: Current PTSD therapies focus on managing symptoms after they arise, but the authors argue that a more effective approach is to dissolve the underlying neural pathways that cause these symptoms.

Method: MindGap uses a conversational AI framework to guide patients through a process of observing and reframing their emotional responses, based on the Buddhist concept of dependent origination.

Result: The authors claim that MindGap can lead to genuine upstream dissolution of the reactive neural pathways, rather than just suppressing symptoms.

Conclusion: The paper concludes that MindGap has the potential to provide a more effective treatment for PTSD, particularly in contexts where data privacy is a concern.
...
### [281] [Falkor-IRAC: Graph-Constrained Generation for Verified Legal Reasoning in Indian Judicial AI](https://arxiv.org/abs/2605.14665)
*Joy Bose*

Main category: cs.AI

TL;DR: The paper presents Falkor-IRAC, a graph-constrained generation framework for Indian legal AI, which uses structured reasoning over an IRAC knowledge graph to generate accurate and valid legal judgments.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based legal AI systems have limitations, such as hallucinated precedents and outdated statute citations, which can have serious consequences for access to justice. The authors aim to develop a more reliable and trustworthy system for legal reasoning.

Method: The authors propose a graph-constrained generation framework, Falkor-IRAC, which uses an IRAC knowledge graph to ground generation in structured reasoning. The system includes a Verifier Agent that checks the validity of generated answers and detects doctrinal conflicts.

Result: The evaluation of Falkor-IRAC on a proof-of-concept corpus of 51 Supreme Court judgments shows promising results, with the Verifier Agent correctly validating citations and rejecting fabricated ones. The authors propose graph-native metrics for evaluation, which they argue are more suitable for legal reasoning than traditional metrics like BLEU and ROUGE.

Conclusion: The paper concludes that Falkor-IRAC has the potential to improve the accuracy and reliability of legal AI systems, particularly in high-caseload jurisdictions like India. The authors highlight the importance of structured reasoning and validity checks in legal AI and suggest directions for future work, including evaluation against vector-only RAG baselines and GPU-accelerated inference.
...
### [282] [Agentifying Patient Dynamics within LLMs through Interacting with Clinical World Model](https://arxiv.org/abs/2605.14723)
...
Method: SepsisAgent uses a learned Clinical World Model to simulate patient responses under candidate fluid--vasopressor interventions, following a propose--simulate--refine workflow, and is trained through a three-stage curriculum.
...
Conclusion: SepsisAgent provides a promising approach for sepsis treatment recommendation, leveraging a world model to learn regularities in patient evolution and improve decision-making.
...
### [283] [Monitoring Data-aware Temporal Properties (Extended Version)](https://arxiv.org/abs/2605.14666)
*Alessandro Gianola,Marco Montali,Sarah Winkler*

Main category: cs.AI

TL;DR: The paper presents a novel framework for anticipatory monitoring of linear-time properties with SMT theory over finite traces, combining automata-theoretic methods and automated reasoning techniques.


<details>
  <summary>Details</summary>
Motivation: Dynamic systems in AI are complex and heterogeneous, making internal specification and verification techniques like model checking inapplicable, thus monitoring is an attractive alternative.

Method: The framework uses automata-theoretic methods to handle temporal aspects and automated reasoning techniques to address the first-order dimension, under reasonable assumptions on the background theory.

Result: The paper identifies decidable fragments of the monitoring problem that combine linear arithmetic with uninterpreted functions, and presents a prototype implementation with preliminary evaluation.

Conclusion: The paper provides a foundational framework for monitoring properties in an expressive fragment of LTLfMT, which is practically relevant for data-aware business processes and dynamic systems operating over a read-only database.
...
### [284] [How Sensitive Are Radiomic AI Models to Acquisition Parameters?](https://arxiv.org/abs/2605.14667)
*D. Gil,I. Sanchez,C. Sanchez*

Main category: cs.AI

TL;DR: The paper presents a framework to evaluate how different scan parameters affect the performance of AI radiomic models in lung cancer diagnosis, identifying optimal scan settings for improved accuracy.


<details>
  <summary>Details</summary>
Motivation: The performance of AI radiomic systems drops under heterogeneous multicentre acquisition protocols, hindering their deployment in clinical routine.
...
Result: The optimal scan configuration is identified as X-ray tube current >= 200 mA, spiral pitch <= 1.5, and slice thickness <= 1.25 mm, which improves sensitivity from 0.79 to 0.90 and specificity from 0.47 to 0.79.

Conclusion: The proposed framework can help identify clinically significant parameter regions associated with improved cross-dataset robustness, leading to more accurate AI radiomic models in lung cancer diagnosis.
...
TL;DR: A new framework for evaluating AI agents is proposed, combining top-down and bottom-up approaches to provide detailed analysis of failures in complex multi-step processes.


<details>
  <summary>Details</summary>
Motivation: Current evaluation methods for AI agents are insufficient, as they either only report success or failure without explanation or struggle to identify specific failure types and locations within long processes.

Method: A holistic agent evaluation framework that pairs top-down agent-level diagnosis with bottom-up span-level evaluation, decomposing analysis into independent per-span assessments.

Result: The framework achieves state-of-the-art results on the TRAIL benchmark, with significant gains over prior baselines in terms of category F1, localization accuracy, and joint localization-categorization accuracy.

Conclusion: The proposed framework provides a more effective and detailed approach to evaluating AI agents, highlighting that evaluation methodology, rather than model capability, is the bottleneck in current AI agent evaluation.
...
### [286] [$π$-Bench: Evaluating Proactive Personal Assistant Agents in Long-Horizon Workflows](https://arxiv.org/abs/2605.14678)
...
TL;DR: The paper introduces π-Bench, a benchmark for evaluating proactive assistance in personal assistant agents, highlighting the challenges of identifying and acting on hidden user intents in multi-turn interactions.


<details>
  <summary>Details</summary>
Motivation: The growing potential of large language models in personal assistant agents raises the challenge of proactive assistance, where users often have unstated needs, constraints, or preferences, and existing benchmarks do not adequately evaluate agents' ability to identify and act on these hidden intents.

Method: The authors introduce π-Bench, a benchmark consisting of 100 multi-turn tasks across 5 domain-specific user personas, evaluating agents' ability to anticipate and address user needs over extended interactions.

Result: Experiments show that proactive assistance remains challenging, there is a clear distinction between task completion and proactivity, and prior interaction is valuable for proactive intent resolution in later tasks.

Conclusion: The paper highlights the importance of evaluating proactive assistance in personal assistant agents and introduces π-Bench as a benchmark to address this challenge, revealing the difficulties of achieving proactive assistance and the benefits of considering prior interactions.
...
### [287] [Explainable Detection of Depression Status Shifts from User Digital Traces](https://arxiv.org/abs/2605.14995)
*Loris Belcastro,Francesco Gervino,Fabrizio Marozzo,Domenico Talia,Paolo Trunfio*

Main category: cs.AI

TL;DR: The paper proposes an explainable framework to detect and analyze depression-related status shifts in users' digital traces, leveraging BERT-based models and a large language model to generate interpretable reports.


<details>
  <summary>Details</summary>
Motivation: Users' digital traces can reflect aspects of their mental state, and analyzing these traces over time can help identify meaningful change points in mental health signals.

Method: The framework combines multiple BERT-based models to extract signals across different dimensions, aggregates them over time to construct user-level trajectories, and uses a large language model to generate concise reports.

Result: The approach produces more coherent and informative summaries than direct LLM-based reporting, achieving higher coverage of user history, stronger temporal coherence, and improved sensitivity to change points.
...
### [288] [On Strong Equivalence Notions in Logic Programming and Abstract Argumentation](https://arxiv.org/abs/2605.14721)
*Giovanni Buraglio,Wolfgang Dvorak,Stefan Woltran*

Main category: cs.AI

TL;DR: The paper investigates strong equivalence between knowledge bases in nonmonotonic formalisms, particularly in logic programming and abstract argumentation, and proposes a new notion of strong equivalence for logic programs to restore compatibility across these formalisms.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the discrepancy in strong equivalence between logic programming and abstract argumentation frameworks in dynamic contexts, which arises due to differing notions of update.

Method: The paper introduces a new notion of strong equivalence for logic programs and investigates its preservation under translation between certain classes of logic programs and both Dung-style and claim-augmented argumentation frameworks.

Result: The proposed approach restores compatibility across these formalisms by preserving strong equivalence under translation.

Conclusion: The paper concludes that the new notion of strong equivalence for logic programs ensures the possibility of replacing one knowledge base with the other without affecting reasoning outcomes in any given context, thus restoring strong equivalence across logic programming and abstract argumentation frameworks.
...
### [289] [Small, Private Language Models as Teammates for Educational Assessment Design](https://arxiv.org/abs/2605.15015)
*Chris Davis Jaldi,Anmol Saini,Shan Zhang,Noah Schroeder,Cogan Shimizu,Eleni Ilkou*

Main category: cs.AI

TL;DR: The paper compares Large Language Models (LLMs) and Small Language Models (SLMs) for generating assessment questions, finding SLMs to be competitive in quality while offering privacy benefits, but also highlighting biases in model-based evaluations.


<details>
  <summary>Details</summary>
Motivation: To address the gap in understanding the effectiveness of Small Language Models (SLMs) for assessment tasks, particularly in comparison to Large Language Models (LLMs), and to evaluate their generation quality and deployment constraints in real educational settings.

Method: Systematic comparison of LLMs and SLMs for assessment question design, using reproducible and pedagogically grounded metrics to evaluate generation quality across Bloom's taxonomy levels, and assessing model-based judging against expert-informed evaluation.
...
Conclusion: Language models can be seen as bounded assistants in assessment workflows, necessitating Human-in-the-Loop, and advancing the field by examining quality, reliability, and deployment-aware trade-offs in automated educational question generation.
...
### [290] [Orchard: An Open-Source Agentic Modeling Framework](https://arxiv.org/abs/2605.15040)
...
TL;DR: The paper introduces Orchard, an open-source framework for scalable agentic modeling, enabling autonomous agents to solve complex tasks through planning, reasoning, and tool use.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the constraints in open research on agentic modeling, which are limited by infrastructure and training gaps, and the reliance on proprietary codebases, models, or services.

Method: The authors propose Orchard Env, a lightweight environment service, and build three agentic modeling recipes: Orchard-SWE, Orchard-GUI, and Orchard-Claw, targeting coding agents, vision-language computer-use agents, and personal assistant agents, respectively.
...
Conclusion: The authors conclude that a lightweight, open, harness-agnostic environment layer enables reusable agentic data, training recipes, and evaluations across domains, and that Orchard is a viable framework for scalable agentic modeling.
...
<details>
  <summary>Details</summary>
Motivation: The capacity of LLMs for compositional generalization in scientific knowledge remains under-characterized, and existing benchmarks fail to capture the capability boundaries exposed by real-world interactive scientific workflows.

Method: The authors introduce XDomainBench, a diagnostic benchmark comprising 8,598 interactive sessions across 20 domains and 4 task categories, with 8 realistic trajectory patterns, to systematically stress-test LLMs.

Result: Large-scale evaluation of LLMs reveals a systematic reasoning collapse as composition order increases, caused by direct difficulty increases induced by domain composition and indirect interaction-amplified failures.

Conclusion: The study highlights the limitations of LLMs in compositional generalization in scientific knowledge and the need for more comprehensive benchmarks to evaluate their capabilities in interactive interdisciplinary scientific reasoning.
...
### [292] [Case-Based Calibration of Adaptive Reasoning and Execution for LLM Tool Use](https://arxiv.org/abs/2605.15041)
*Renning Pang,Tian Lan,Leyuan Liu,Piao Tong,Sheng Cao,Xiaosong Zhang*

Main category: cs.AI

TL;DR: The paper presents CAST, a case-driven framework that improves large language models' tool use by extracting knowledge from historical execution trajectories, leading to better execution accuracy and reduced reasoning length.


<details>
  <summary>Details</summary>
Motivation: The motivation is to balance reasoning depth with structural validity in large language models' tool use, as reliable execution is crucial but challenging.

Method: The method involves a case-based approach, where CAST extracts case-derived signals to identify complexity profiles and failure profiles, and uses these to inform fine-grained reward design and adaptive reasoning during reinforcement learning.

Result: The results show that CAST improves schema-faithful execution and task-level tool-use success, achieving up to 5.85 percentage points gain in overall execution accuracy and reducing average reasoning length by 26%.

Conclusion: The conclusion is that historical execution cases can provide reusable adaptation knowledge for calibrated tool use, enabling large language models to autonomously internalize case-based strategies and improve their performance.
...
TL;DR: A probabilistic framework, RNN-ProVe, is proposed for verifying recurrent neural network (RNN) based policies in partially observable reinforcement learning, providing more quantitative and feasible guarantees.
...
Result: RNN-ProVe provides more quantitative, feasibility-aware probabilistic guarantees than existing tools, scaling to recurrent and multi-agent settings.

Conclusion: RNN-ProVe offers a more effective and scalable solution for verifying RNN-based policies in complex reinforcement learning environments.
...
### [294] [AI Outperforms Humans in Personalized Image Aesthetics Assessment via LLM-Based Interviews and Semantic Feature Extraction](https://arxiv.org/abs/2605.14761)
*Yoshia Abe,Tatsuya Daikoku,Yasuo Kuniyoshi*

Main category: cs.AI

TL;DR: The authors propose a DL-LLM system that uses Large Language Models to predict individual aesthetic evaluations of images, outperforming conventional systems and human predictors.


<details>
  <summary>Details</summary>
Motivation: Accurately predicting individual aesthetic evaluation for images is a challenge due to subjective and individual-dependent nature of aesthetic preferences.

Method: The authors develop an integrated DL-LLM system that actively elicits aesthetic preferences through LLM-based semi-structured interviews and predicts aesthetic evaluation by leveraging both low-level and high-level features.
...
Conclusion: The results suggest that AI may be better positioned than humans or one's future self to capture individual aesthetic preferences, opening a new question of whether AI could serve as a deeper interpreter of human aesthetic sensibility.
...
### [295] [MediaClaw: Multimodal Intelligent-Agent Platform Technical Report](https://arxiv.org/abs/2605.14771)
...
Method: MediaClaw uses a unified abstraction, pluginized extension, and workflow orchestration to abstract AIGC capabilities into a unified model, support capability expansion, and turn production processes into reusable workflow assets.

Result: The platform provides a reusable practical reference for building multimodal capability platforms, focusing on architectural design philosophy, core capability model, and key engineering trade-offs.

Conclusion: MediaClaw offers a solution for efficient AIGC deployment and reuse, providing a unified and extensible framework for multimodal agent platforms.
...
### [296] [Identifying Culprits Through Deep Deterministic Policy Gradient Deep Learning Investigation](https://arxiv.org/abs/2605.14774)
*Lata B T,Savitha N J*

Main category: cs.AI

TL;DR: A deep learning approach using Deep Deterministic Policy Gradient (DDPG) is proposed to identify criminals from complex datasets, achieving an accuracy of 95%.


<details>
  <summary>Details</summary>
Motivation: The challenge of identifying criminals from complex datasets while minimizing false positives and false negatives using conventional methods that rely on limited data analysis.

Method: Deep Deterministic Policy Gradient (DDPG) algorithm trained on a dataset of crime scene material, witness statements, and suspect profiles.

Result: The proposed method achieved an accuracy of 95% in identifying criminals, outperforming several existing methods.

Conclusion: The proposed DDPG approach is effective in identifying criminals from complex datasets, offering a more accurate solution than traditional methods.
...
### [297] [A Heterogeneous Temporal Memory Governance Framework for Long-Term LLM Persona Consistency](https://arxiv.org/abs/2605.14802)
*Zhao Yang,Wang Huan,Li Yingshuo,Tu Haomiao,Lin Hujite*

Main category: cs.AI

TL;DR: The paper introduces ARPM, a framework for maintaining long-term dialogue consistency in large language models by separating knowledge and experience memories and using various techniques for evidence verification and answer binding.


<details>
  <summary>Details</summary>
Motivation: Large language models suffer from fact loss, timeline confusion, and persona drift during long-range interactions, especially under noisy knowledge bases and context clearing. The authors aim to address these issues.

Method: The authors propose ARPM, which separates static knowledge memory from dynamic dialogue experience memory and uses vector retrieval, BM25, RRF fusion, dual-temporal reranking, and chronological evidence reading for evidence verification and answer binding.

Result: The experiments show that ARPM maintains semantic continuity, boundary continuity, and persona consistency under various conditions, including high-noise knowledge bases, context clearing, and cross-model transfer. The results also highlight the importance of dialogue history retrieval and the limitations of pure semantic retrieval.

Conclusion: The paper concludes that long-term persona consistency can be decomposed into governable components and evaluated in a white-box manner, and that ARPM is an effective framework for achieving this goal.
...
### [298] [Interestingness as an Inductive Heuristic for Future Compression Progress](https://arxiv.org/abs/2605.14831)
...
Result: The inductive property of interestingness is theoretically viable and empirically supported; expected future progress depends exponentially on the recency of the last observed breakthrough.

Conclusion: The study demonstrates the viability of using an inductive heuristic for future compression progress to identify interestingness, with implications for recursively self-improving systems.
...
### [299] [Emotion-Attended Stateful Memory (EASM):The Architecture for Hyper-Personalization at Scale](https://arxiv.org/abs/2605.14833)
*Vineet Kotecha,Vansh Gupta*

Main category: cs.AI

TL;DR: The paper proposes an emotion-attended stateful memory architecture to improve language model interactions by making them more personalized and stateful over time.


<details>
  <summary>Details</summary>
Motivation: Current language models are stateless and lack the ability to personalize interactions over time, limiting their effectiveness in understanding individual users.

Method: The authors propose an emotion-attended stateful memory architecture that constructs user-specific conversational context using long-term history, emotional signals, and inferred intent.

Result: The memory-enriched condition consistently outperformed the stateless baseline across all evaluated scenarios, with significant gains in memory grounding (95%), plan clarity (57%), and emotional validation (34%).

Conclusion: The findings suggest that stateful emotional memory may represent a foundational infrastructure layer for hyper-personalized AI systems, though broader validation is necessary.
...
### [300] [A Deterministic Agentic Workflow for HS Tariff Classification: Multi-Dimensional Rule Reasoning with Interpretable Decisions](https://arxiv.org/abs/2605.14857)
*Yu Zhang,Dongjiang Zhuang,Qu Zhou,Zheng Huang,Junhe Wu,Jing Cao,Kai Chen*

Main category: cs.AI

TL;DR: The paper presents a deterministic workflow for Harmonized System (HS) tariff classification, which involves mapping product descriptions to specific codes. The proposed workflow combines offline knowledge-engineering with an online six-stage pipeline, achieving high accuracy and interpretability.


<details>
  <summary>Details</summary>
Motivation: The difficulty in HS tariff classification lies in multi-dimensional rule reasoning, where a correct classification must satisfy competing priority rules along several axes simultaneously. End-to-end prompting of large language models often fails to resolve one axis while ignoring the priority constraints on the others.

Method: The proposed workflow is a deterministic agentic workflow with a fixed control flow, narrow language model calls, and local reflection and verification mechanisms. It combines offline knowledge-engineering of the Chinese HS tariff with an online six-stage pipeline.

Result: The workflow achieves 75.0% top-1 and 91.5% top-3 accuracy at four digits, and 64.2% top-1 and 78.3% top-3 accuracy at six digits with Qwen3.6-plus. A non-thinking mode open-weight Qwen3.6-27B-FP8 backbone achieves 84.2% four-digit and 77.4% six-digit top-1 agreement with the frontier model.

Conclusion: The proposed workflow provides a transparent and accurate approach to HS tariff classification. The results suggest that a non-trivial fraction of HSCodeComp ground-truth labels may deviate from HS general rules, highlighting the need for community review and adjudication.
...
### [301] [BiFedKD: Bidirectional Federated Knowledge Distillation Framework for Non-IID and Long-Tailed ECG Monitoring](https://arxiv.org/abs/2605.14886)
*Zixuan Shu,Tiancheng Cao,Hen-Wei Huang*

Main category: cs.AI

TL;DR: A new framework called BiFedKD is proposed to improve federated learning for ECG monitoring in IoMT networks, achieving better accuracy and reduced communication overhead.


<details>
  <summary>Details</summary>
Motivation: Federated learning for ECG monitoring faces challenges due to strict data-sharing regulations, privacy concerns, and communication overhead. Existing methods like federated distillation (FD) degrade under non-IID and long-tailed label distributions.

Method: The proposed BiFedKD framework employs an aggregation-by-distillation pipeline with temperature scaling for stable global distillation signal and cross-client alignment.

Result: BiFedKD improves accuracy by 3.52% and Macro-F1 by 9.93% over the baseline on the MIT-BIH Arrhythmia dataset. It reduces communication overhead by 40% and computation cost by 71.7% to reach the same Macro-F1.

Conclusion: BiFedKD effectively addresses the challenges of federated learning for ECG monitoring, offering improved performance and reduced communication overhead.
...
### [302] [Beyond Individual Intelligence: Surveying Collaboration, Failure Attribution, and Self-Evolution in LLM-based Multi-Agent Systems](https://arxiv.org/abs/2605.14892)
...
TL;DR: This survey reviews LLM-based autonomous agents and multi-agent systems, proposing a unified framework called LIFE (Lay, Integrate, Find, Evolve) to examine causal dependencies between agent capabilities, collaboration, fault detection, and self-improvement.


<details>
  <summary>Details</summary>
Motivation: Existing surveys focus on individual agent capabilities, multi-agent collaboration, or agent self-evolution separately, but not their causal dependencies. The authors aim to bridge this gap by providing a unified review.

Method: The authors propose a four-stage framework, LIFE, to systematically examine the dependencies between agent capabilities, collaboration, fault detection, and self-improvement. They provide taxonomies and characterize the dependencies between stages.

Result: The survey identifies open challenges at stage boundaries and proposes a cross-stage research agenda for closed-loop multi-agent systems capable of continuous diagnosis, reorganization, and refinement.

Conclusion: The survey aims to provide a systematic reference and conceptual roadmap toward autonomous, self-improving multi-agent intelligence by bridging previously fragmented research threads.
...
### [303] [COREKG: Coreset-Guided Personalized Summarization of Knowledge Graphs](https://arxiv.org/abs/2605.14900)
*Sohel Aman Khan,Raghava Mutharaju,Supratim Shit*

Main category: cs.AI

TL;DR: The paper presents COREKG, a method for personalized knowledge graph summarization that uses coreset theory to create summaries tailored to individual users' query patterns, achieving high accuracy and structural coverage while reducing storage requirements.


<details>
  <summary>Details</summary>
Motivation: Large knowledge graphs are unwieldy for tasks like question answering and visualization, and personalized summarization is crucial to capture specific user requirements based on query patterns.

Method: The approach adapts coreset theory to sample a relevant subset of triples using sensitivity-based importance sampling, ensuring bounded approximation error and constructing summaries independently for each user.

Result: COREKG delivers higher query-answering accuracy and structural coverage than state-of-the-art methods (GLIMPSE, PPR, iSummary, PEGASUS, and APEX2) while requiring only a tiny fraction of the original graph.

Conclusion: The proposed method, COREKG, effectively creates personalized knowledge graph summaries that meet user-specific requirements, offering improved performance and reduced storage needs.
...
### [304] [KGPFN: Unlocking the Potential of Knowledge Graph Foundation Model via In-Context Learning](https://arxiv.org/abs/2605.14907)
*Yisen Gao,Jiaxin Bai,Haoyu Huang,Zhongwei Xie,Yufei Li,Hong Ting Tsang,Sirui Han,Yangqiu Song*

Main category: cs.AI

TL;DR: KGPFN, a knowledge graph foundation model, is proposed to unify transferable relational regularities with in-context learning for KG reasoning, achieving strong adaptation to unseen graphs.


<details>
  <summary>Details</summary>
Motivation: Existing KG foundation models focus on relation-level universality, but neglect in-context learning, which is crucial for KG reasoning given the structured and heterogeneous nature of context in KGs.

Method: KGPFN uses a Prior-data Fitted Network that combines message passing on relation graphs, multi-layer NBFNet for local context encoding, and a Prior-Data Fitted Network framework for global context aggregation.

Result: KGPFN consistently outperforms competitive fine-tuned KG foundation models on 57 KG benchmarks, demonstrating strong adaptation to previously unseen graphs through in-context learning alone.

Conclusion: KGPFN effectively unifies transferable relational regularities with inference-time in-context learning, enabling robust and adaptable KG reasoning across diverse graphs.
...
### [305] [From Sycophantic Consensus to Pluralistic Repair: Why AI Alignment Must Surface Disagreement](https://arxiv.org/abs/2605.14912)
*Varad Vishwarupe,Nigel Shadbolt,Marina Jirotka*

Main category: cs.AI

TL;DR: The paper argues that current pluralistic alignment methods in AI, which focus on aggregating diverse human values, are insufficient. Instead, it proposes a new approach based on Grice's maxims, emphasizing conversational mechanisms like scoping, signalling, and repair to foster genuine pluralism.


<details>
  <summary>Details</summary>
Motivation: The authors are motivated by the limitations of current pluralistic alignment methods, which they argue lead to 'sycophantic consensus' where AI systems prioritize agreement with the user over representing diverse values. This can have significant consequences in areas like healthcare, civic life, and governance.

Method: The paper proposes reframing pluralistic alignment around three conversational mechanisms: scoping, signalling, and repair. It also introduces a new metric, the Pluralistic Repair Score (PRS), to measure the quality of principled revision in AI responses.

Result: The authors present a small-scale empirical study on two frontier RLHF-trained models, Claude Sonnet 4.5 and GPT-4o, which shows that both models exhibit low repair-quality on contested-value prompts. The study suggests that agreement-following can coexist with low repair-quality.

Conclusion: The paper concludes that pluralistic alignment should focus on fostering conversational mechanisms that promote genuine pluralism, rather than just aggregating diverse values. It also highlights the importance of deployment-governance, including interfaces, preference-data pipelines, and audit infrastructure, in making or unmaking pluralism.
...
### [306] [GraphFlow: An Architecture for Formally Verifiable Visual Workflows Enabling Reliable Agentic AI Automation](https://arxiv.org/abs/2605.14968)
*Drewry H. Morris,Luis Valles,Reza Hosseini Ghomi*

Main category: cs.AI

TL;DR: GraphFlow is a visual workflow system that improves the reliability of AI automation in multi-step processes by treating workflow diagrams as executable specifications, enabling contract verification, and providing a durable execution engine.


<details>
  <summary>Details</summary>
Motivation: The reliability of agentic AI automation in multi-step, mission-critical processes is compromised by small errors that compound rapidly, and existing workflow platforms lack semantic correctness guarantees.

Method: GraphFlow uses a visual workflow system with a restricted class of diagrams to produce reusable automations with verifiable contracts, and a durable engine that records outcomes and enforces contracts at runtime.

Result: A year-long pilot executed 8,728 workflow runs with a 97.08% completion rate under an early prototype, with observed failures primarily due to external integrations.

Conclusion: GraphFlow has the potential to improve the reliability of AI automation in multi-step processes by providing a visual workflow system with formal semantics and proof-checked admission model.
...
### [307] [Learning Developmental Scaffoldings to Guide Self-Organisation](https://arxiv.org/abs/2605.14998)
*Milton L. Montero,Elias Najarro,Jakob Schauser,Sebastian Risi*

Main category: cs.AI

TL;DR: The paper studies how initial conditions, or pre-patterns, influence self-organisation in biological development, introducing a model that jointly learns self-organisation rules and pre-patterns, and analysing their interplay.


<details>
  <summary>Details</summary>
Motivation: The authors aim to understand how information is offloaded to initial conditions in biological development, and how this affects self-organisation processes.

Method: The authors propose a Neural Cellular Automaton (NCA) paired with a learned coordinate-based pattern generator (SIREN), both trained simultaneously to generate a set of patterns.

Result: The analysis shows that jointly learning both components yields improvements in robustness, encoding capacity, and symmetry breaking over purely self-organising alternatives.

Conclusion: The study suggests that effective pre-patterns do not simply approximate their targets, but rather bias the developmental dynamics in ways that facilitate convergence, highlighting a non-trivial relationship between initial conditions and self-organisation.
...
TL;DR: The paper proposes a unified framework called Dual-Dimensional Consistency (DDC) to optimize the inference-time scaling of Large Language Models (LLMs) by balancing sampling budget and reasoning quality.


<details>
  <summary>Details</summary>
Motivation: Current strategies for maximizing LLM potential through inference-time scaling are inefficient due to the trade-off between sampling budget and reasoning quality, and they often treat sampling width and depth as orthogonal objectives.

Method: The proposed DDC framework couples Confidence-Weighted Bayesian protocol with Trend-Aware Stratified Pruning to concentrate computational resources on high-quality reasoning paths.

Result: The approach reduces token consumption by over 10 times while maintaining or exceeding the accuracy of strong baselines across various LLMs on five benchmarks.

Conclusion: The proposed Dual-Dimensional Consistency framework effectively optimizes the inference-time scaling of LLMs, achieving a better balance between sampling budget and reasoning quality.
...
### [309] [Why Neighborhoods Matter: Traversal Context and Provenance in Agentic GraphRAG](https://arxiv.org/abs/2605.15109)
*Riccardo Terrenzi,Maximilian von Zastrow,Serkan Ayvaz*

Main category: cs.AI

TL;DR: The paper examines citation faithfulness in Agentic GraphRAG systems, finding that while cited evidence is necessary, it's not sufficient for accurate answers, which can also depend on uncited context and graph structure.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to investigate the concept of citation faithfulness in Retrieval-Augmented Generation systems, specifically in Agentic GraphRAG, where an agent explores a knowledge graph to produce an answer and a set of citations.

Method: The authors use controlled ablation experiments to compare the effects of isolating, removing, and masking cited and uncited graph entities, examining how these actions impact the answer and its accuracy.

Result: The results show that cited evidence is often necessary for accurate answers, but not sufficient, as accurate answers can also depend on uncited traversal context and surrounding graph structure.

Conclusion: The paper concludes that citation evaluation in Agentic GraphRAG should move beyond source support toward provenance over the broader retrieval trajectory, highlighting the importance of considering the entire graph traversal and structure when evaluating citation faithfulness.
...
### [310] [APWA: A Distributed Architecture for Parallelizable Agentic Workflows](https://arxiv.org/abs/2605.15132)
*Evan Rose,Tushin Mallick,Matthew D. Laws,Cristina Nita-Rotaru,Alina Oprea*

Main category: cs.AI

TL;DR: Introduces Agent-Parallel Workload Architecture (APWA) for efficient processing of parallelizable multi-agent workloads using large language models.


<details>
  <summary>Details</summary>
Motivation: Current autonomous multi-agent systems face bottlenecks in reasoning, coordination, and scaling as task complexity grows, hindering high-throughput processing.

Method: APWA architecture decomposes workflows into non-interfering subproblems for parallel execution, supporting heterogeneous data and processing patterns.

Result: APWA successfully scales on larger tasks and decomposes complex queries into parallelizable workflows, outperforming prior systems.

Conclusion: APWA enables efficient processing of parallelizable multi-agent workloads, addressing scaling limitations in current systems.
...
### [311] [OpenDeepThink: Parallel Reasoning via Bradley--Terry Aggregation](https://arxiv.org/abs/2605.15177)
*Shang Zhou,Wenhao Chai,Kaiyuan Liu,Huanzhi Mao,Qiuyang Mang,Jingbo Shang*

Main category: cs.AI

TL;DR: Introduced OpenDeepThink, a test-time compute framework that improves LLM reasoning by scaling breadth and selecting candidates via pairwise comparison, achieving significant gains in Codeforces Elo.


<details>
  <summary>Details</summary>
Motivation: Existing methods for improving LLM reasoning primarily scale depth, but scaling breadth by sampling multiple candidates in parallel introduces a selection bottleneck due to noisy and biased pointwise LLM judging.

Method: OpenDeepThink uses pairwise Bradley-Terry comparison to select candidates, where the LLM judges random pairs and aggregates votes into a global ranking, then preserves top-ranked candidates and mutates them using natural-language critiques.

Result: OpenDeepThink raises Gemini 3.1 Pro's effective Codeforces Elo by +405 points in eight sequential LLM-call rounds, with gains concentrated in objectively verifiable domains.

Conclusion: OpenDeepThink is an effective test-time compute framework for improving LLM reasoning, particularly in objectively verifiable domains, and can transfer across weaker and stronger models without retuning.
