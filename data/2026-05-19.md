...
TL;DR: Introduced ReactiveGWM, a reactive game world model that enables dynamic interactions between players and NPCs by decoupling player controls from NPC behaviors, allowing for zero-shot strategy transfer across different games.


<details>
  <summary>Details</summary>
Motivation: Current game world models fail to capture interactions between players and NPCs as they treat NPCs as passive background elements, lacking the physical understanding needed for NPC reactivities.

Method: ReactiveGWM uses a diffusion backbone with a lightweight additive bias for player actions and cross-attention modules for NPC responses, learning a game-agnostic representation of interactive logic.

Result: ReactiveGWM achieved robust, prompt-aligned NPC strategy adherence while maintaining fine-grain player controllability in two Street Fighter games, enabling scalable, strategy-rich interaction with NPCs.

Conclusion: ReactiveGWM paves the way for more interactive and responsive game worlds by allowing for dynamic NPC interactions without requiring domain-specific retraining.
...
TL;DR: Proposed Deep Pre-Alignment (DPA) architecture for Vision Language Models (VLMs) to improve alignment between visual features and text space, achieving better performance and reduced language capability forgetting.


<details>
  <summary>Details</summary>
Motivation: Existing VLMs suffer from alignment challenges, where visual features remain distant from the text space in initial layers of the LLM, wasting critical depth on superficial modality alignment.

Method: Replacing the standard ViT encoder with a small VLM as perceiver to ensure deep alignment between visual features and text space of the target LLM.

Result: DPA outperforms baselines by 1.9 points across 8 multimodal benchmarks on the 4B parameter scale and 3.0 points at the 32B scale, with a 32.9% reduction in language capability forgetting over 3 text benchmarks.

Conclusion: DPA offers a seamless upgrade path for current VLM development with marginal computation overhead, demonstrating generality across different LLM families.
...
TL;DR: The paper introduces RTM, a method for improving image generation models by prioritizing mode coverage and diversity, achieving high precision and recall while maintaining competitive FID scores.


<details>
  <summary>Details</summary>
Motivation: Current image generation models are limited by the dominant metric FID, which conflates sample fidelity with mode coverage, and can be saturated even with mode collapse; the authors argue for the importance of precision and recall as complementary metrics.

Method: The authors propose RTM, which uses an iterative refinement process in style-based generators, and integrate it with Implicit Maximum Likelihood Estimation (IMLE) to optimize mode coverage.

Result: RTM achieves the highest precision and recall among current state-of-the-art approaches while maintaining competitive FID scores across various benchmarks, including CIFAR-10, CelebA-HQ, and few-shot benchmarks.

Conclusion: The proposed method, RTM, improves both quality and diversity of image generation models, demonstrating the benefit of prioritizing mode coverage and diversity.
...
TL;DR: The paper introduces COPRA, a conditional parameter adaptation framework for vision-language models (VLMs) in video anomaly detection (VAD), addressing the mismatch between training and inference data distribution and model configuration.


<details>
  <summary>Details</summary>
Motivation: Existing VLM-based VAD methods suffer from a fundamental mismatch between training and inference in both data distribution and model configuration, limiting their generalization under distribution shifts.

Method: The authors propose COPRA, which generates input-specific parameter updates to dynamically adapt a frozen VLM for each video segment during both training and inference.

Result: COPRA shows strong performance on standard VAD benchmarks, outperforming static baselines in both in-domain and cross-domain settings, and generalizes to unseen tasks such as multiple-choice Video Question Answering and Dense Captioning.

Conclusion: COPRA is an effective weight-space generation framework for scalable, adaptive, and context-aware video understanding, highlighting its potential for various video understanding tasks.
...
TL;DR: A multimodal approach combining LiDAR, visible-thermal image fusion, and synthetic-aperture imaging improves human detection under forest canopies.


<details>
  <summary>Details</summary>
Motivation: Reliable detection of humans beneath forest canopy is a difficult remote-sensing challenge due to occlusion.

Method: The paper presents a pipeline integrating LiDAR returns evaluation, visible-thermal image fusion, and Airborne Optical Sectioning (AOS).

Result: The fine-tuned YOLOv5 achieves a mean average precision of ~0.83 on the top three FLIR classes. The tested LiDAR configuration provides limited penetration, while visible-thermal fusion and AOS enhance target visibility and ground-plane detection.

Conclusion: The findings establish an initial baseline for UAV-deployable search-and-rescue and surveillance systems in forested environments.
...
TL;DR: A new benchmark, Minerva-Ego, is introduced for evaluating complex egocentric visual reasoning in video data, highlighting a significant performance gap between state-of-the-art models and human performance.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks for video reasoning models focus only on output evaluation and lack multimodal questions and intermediate reasoning steps, particularly in egocentric and embodied settings.

Method: The Minerva-Ego benchmark extends existing video data sources with multimodal questions and human-annotated reasoning traces, and includes spatiotemporal mask annotations to identify objects of interest.

Result: State-of-the-art models show a large gap to human performance, but prompting models with 'where' and 'when' hints significantly improves performance.

Conclusion: The Minerva-Ego benchmark provides a more comprehensive evaluation of video reasoning models, revealing areas for improvement and opportunities for advancing egocentric visual reasoning capabilities.
...
TL;DR: The paper proposes a method to reduce the computational cost of Group-convolutional neural networks (GCNNs) by sampling in feature space instead of geometrically dense sampling, which helps in trading off computational effort and accuracy.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the high computational cost associated with applying filters to many data items in GCNNs, which grows exponentially with increasing degrees of freedom.

Method: The proposed method involves sampling in feature space, where representative samples are selected by feature similarity instead of geometrically dense samples.

Result: The main empirical finding of the paper is that a coarse feature-space sampling preserves classification accuracy remarkably well, allowing for precomputation based on geometric similarity and substantially accelerating the training of equivariant 3D classifiers.

Conclusion: The paper concludes that the proposed method provides a novel way to trade off computational effort and accuracy in GCNNs, which can be particularly beneficial for practical applications involving 3D classifiers.
...
TL;DR: A new generative framework called ChangeFlow is proposed for remote sensing change detection, which models change detection as synthesizing a change mask in latent space and achieves state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Current state-of-the-art methods for remote sensing change detection (RSCD) have limitations, including being context-dependent and occasionally ambiguous, and failing to explicitly model the changed region as a coherent whole.

Method: ChangeFlow, a generative framework that uses rectified flow to synthesize a change mask in latent space, guided by a structured yet lightweight conditioning signal, and supports sampling-based prediction ensembling.

Result: ChangeFlow achieves an average F1 of 80.4% across four benchmarks, improving by 1.3 points on average over the previous best method, while maintaining inference speed comparable to recent strong baselines.

Conclusion: ChangeFlow provides a more robust and accurate approach for remote sensing change detection, with the ability to capture ambiguity and encourage global consistency, and has the potential to be applied to a wide range of applications.
...
TL;DR: Introduced MorphoHELM, a comprehensive open benchmark for evaluating feature extraction methods in Cell Painting, consolidating evaluation standards and assessing method performance under varying batch effects.


<details>
  <summary>Details</summary>
Motivation: The lack of standardized evaluation for deep learning methods in microscopy image analysis, particularly in Cell Painting, makes it difficult to compare models fairly.

Method: MorphoHELM benchmark, which evaluates feature extraction methods on a wide range of tasks and datasets, with a focus on robustness to batch effects (technical noise).

Result: No existing model outperforms classic computer vision strategies across all settings; models excel in certain biological signals but are weaker in others.

Conclusion: MorphoHELM provides a comprehensive evaluation framework for Cell Painting, revealing trade-offs between methods and suggesting that classic computer vision strategies remain strong general-use case representations.
...
TL;DR: PanoWorld generates 360-degree videos from a single image and caption, focusing on geometric consistency and 3D scene state.


<details>
  <summary>Details</summary>
Motivation: Existing panoramic video methods lack geometric consistency, leading to implausible outputs. The authors aim to address this by treating panoramic video generation as a geometry- and dynamics-consistent latent state modeling problem.

Method: The authors introduce two lightweight regularizers (depth consistency loss and trajectory consistency loss) and apply spherical-geometry-aware adaptation. They also create a unified geometry-aware panoramic video dataset, PanoGeo.

Result: PanoWorld improves geometric consistency over prior methods while maintaining competitive visual realism, supporting the holistic spatial understanding requirements of embodied AI applications.

Conclusion: The authors conclude that panoramic video generation must be treated as a geometric modeling problem to achieve reliable results, and PanoWorld demonstrates the effectiveness of this approach.
...
TL;DR: The paper introduces ELDOR, a UAV benchmark for monitoring illegal gold mining in the Amazon rainforest, and evaluates various models for tasks such as semantic segmentation and classification.


<details>
  <summary>Details</summary>
Motivation: Illegal gold mining in the Amazon rainforest causes significant environmental damage, but is difficult to monitor at fine spatial scales due to limitations of satellite imagery.

Method: The authors introduce a large-scale UAV benchmark called ELDOR, which contains manually annotated orthomosaic imagery covering over 2,500 hectares, and evaluate various models for four benchmark tasks.

Result: Current methods struggle with rare small-scale mining structures and fine-grained recovery classes, suggesting the need for context-aware and multimodal modeling.

Conclusion: The paper highlights the need for improved monitoring of illegal gold mining in the Amazon rainforest and proposes a benchmark and tools to support domain analysis and practical use.
...
TL;DR: This study explores the intersection of uncertainty estimation and segmentation, examining how various factors impact uncertainty estimates in practical scenarios.


<details>
  <summary>Details</summary>
Motivation: The quality of uncertainty estimates can be sensitive to various variables, and identifying these factors is crucial for dealing with prediction errors.

Method: A large-scale study was conducted across multiple variables, including datasets, backbones, and downstream tasks, for both semantic and panoptic segmentation.

Result: The study found that (a) panoptic segmentation usually results in worse performance, (b) time series samples can be useful but are not always worth the cost, (c) sample diversity shows promise in calibration but not otherwise, and (d) deterministic approaches can be adequate for some tasks, but ensembles can offer significant improvements under the right conditions.

Conclusion: The study provides insights into the factors that affect uncertainty estimates in segmentation tasks, highlighting the importance of considering these factors in practical scenarios.
...
TL;DR: A new video object detection method called MR2-ByteTrack is proposed, which enables efficient on-device intelligence for smart vision sensors by reducing computational cost while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: The need for on-device intelligence in smart vision sensors due to limitations of cloud computing, and the challenge of running conventional video object detection methods on ultra-low-power microcontrollers with limited memory and compute.

Method: MR2-ByteTrack, a video object detection method that alternates between full- and low-resolution inference, uses ByteTrack to link detections across frames, and corrects misclassifications through the Rescore algorithm.

Result: MR2-ByteTrack achieves mAP scores of up to 49.0 for CNN-based models and 48.7 for Transformer-based models, while reducing multiply-accumulate operations by up to 53% for CNNs and 32% for Transformer, and yields up to 55% energy savings on an ultra-low-power RISC-V multicore MCU.

Conclusion: The proposed MR2-ByteTrack method enables efficient and accurate video object detection on ultra-low-power microcontrollers, making it suitable for on-device intelligence in smart vision sensors.
...
TL;DR: Social-Mamba, a novel architecture for human trajectory forecasting that efficiently models social interactions, achieving state-of-the-art accuracy and superior computational efficiency.


<details>
  <summary>Details</summary>
Motivation: The need for models that balance accuracy with computational efficiency in human trajectory forecasting, particularly in dense crowds, where social interactions are crucial.

Method: Social-Mamba, which reformulates social interactions as structured sequential processes using Cycle Mamba block, social triplet factorization, and learnable social gate.

Result: State-of-the-art accuracy on five trajectory forecasting benchmarks, with superior parameter efficiency and computational scalability, and further enhancement when embedded into a flow-matching framework.

Conclusion: Social-Mamba provides a flexible and robust foundation for future trajectory forecasting research, offering a efficient and accurate solution for human trajectory forecasting in crowded environments.
...
TL;DR: The paper introduces a new perspective on Concealed Object Segmentation (COS) tasks by using homogeneous image decomposition via Retinex theory, which factorizes an image into illumination and reflectance components, and proposes a method called RIDE to improve COS performance.


<details>
  <summary>Details</summary>
Motivation: Existing methods for COS tasks either operate directly on RGB images or use heterogeneous decompositions, which can make pixel-aligned cues less direct. The authors aim to find a more effective approach to tackle the visual entanglement issue in COS tasks.

Method: The authors propose RIDE, which consists of: (i) a Task-Driven Retinex Decomposition module that learns segmentation-optimal factorizations end-to-end; (ii) a Discriminability Gap Attention mechanism that adaptively exploits where decomposition helps; and (iii) a Camouflage-Breaking Contrastive loss operating in reflectance feature space.

Result: The authors show that Retinex decomposition preserves or strictly improves total foreground-background discriminability across the full physical regime, with anti-correlation maximizing the gain. They also propose RIDE, which leverages this property to improve COS performance.

Conclusion: The paper concludes that the proposed RIDE method, based on homogeneous image decomposition via Retinex theory, can effectively tackle the visual entanglement issue in COS tasks and improve their performance.
...
TL;DR: VideoRLVR optimizes video diffusion models with rule-based feedback for verifiable reasoning, improving performance on tasks with explicit constraints.


<details>
  <summary>Details</summary>
Motivation: Current video diffusion models are primarily optimized for plausible generation rather than verifiable reasoning, limiting their application in tasks with explicit spatial, temporal, or logical constraints.

Method: VideoRLVR uses an SDE-GRPO optimization backbone, dense decomposed rewards, and an Early-Step Focus strategy to optimize video diffusion models with rule-based feedback.

Result: VideoRLVR consistently improves over supervised fine-tuning baselines on Maze, FlowFree, and Sokoban tasks, and outperforms proprietary and open-source video generation models on verifiable reasoning benchmarks.
...
TL;DR: Proposed Interaction-Aware JEPA (IA-JEPA) for learning predictive world models from unlabelled video, achieving significant improvements in causal reasoning and physical understanding.


<details>
  <summary>Details</summary>
Motivation: Current Joint Embedding Predictive Architectures (JEPA) are physics-blind, failing to capture causal dynamics, likely due to patch-based masking strategies prioritizing visual texture over kinematic events.

Method: IA-JEPA uses a self-supervised motion-centric masking strategy to prioritize physical interactions, targeting entities in collisions or momentum transfers.

Result: IA-JEPA achieves 14.26% accuracy on causal reasoning tasks (CLEVRER benchmark), outperforming standard patch-masked baselines (3.22%), and demonstrates a higher-entropy, more discriminative latent space.

Conclusion: IA-JEPA provides a scalable, fully self-supervised path toward building foundational world models that internalize the causal structure of the physical world, generalizing to real-world human actions and physical puzzles.
...
TL;DR: The paper analyzes the effectiveness of empowered transposed Fully Connected Weighted (t-FCW) graph representation for point cloud embedding and introduces a highly efficient and interpretable network using empowered t-FCW as feature extractors.


<details>
  <summary>Details</summary>
Motivation: The original t-FCW showed promising results for point cloud classification, but its broader applicability and reasons behind its effectiveness were unclear.

Method: The paper analyzes the properties of empowered and original t-FCW, designs a network using empowered t-FCW exclusively as feature extractors, and builds memory banks for classification, part segmentation, and semantic segmentation.

Result: The empowered t-FCW inherits robustness from surface descriptors, provides interpretability through dimension-wise relations, and enables a highly efficient network that processes the ModelNet40 classification problem in approximately 7 seconds on an NVIDIA RTX A5000 GPU.

Conclusion: The empowered t-FCW can function as a lightweight standalone baseline and as a complementary plug-in to existing deep models, providing a highly efficient and interpretable solution for point cloud embedding.
...
TL;DR: The paper proposes a method to utilize exocentric video data for training egocentric world models by transforming exocentric video into egocentric video, improving prediction and planning performance.


<details>
  <summary>Details</summary>
Motivation: The limited availability of egocentric training data and its inherent partial observability of humans' physical actions constrain the performance of egocentric world models.

Method: The proposed method involves extracting structured body pose from exocentric video and transforming it into egocentric video using a human kinematics prior.

Result: Training egocentric world models with the converted data significantly improves prediction quality and downstream planning performance.

Conclusion: The approach enables the use of arbitrary in-the-wild videos for building powerful egocentric world models, with applications in robot planning and augmented-reality guidance.
...
TL;DR: The paper studies the effectiveness of sparse top-k routing with hard capacity constraints in vision classification using Mixture-of-Experts (MoE) networks.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the issues of expert collapse and limited end-to-end efficiency gains in practical vision deployments of MoE networks.

Method: The authors evaluate sparse top-k routing with hard capacity constraints under multi-seed protocols on four benchmarks (CIFAR-10/100, Tiny-ImageNet, ImageNet-1K).

Result: The authors observe a compute-leverage pattern, where positive accuracy gaps require a substantial fraction of total FLOPs to be routed, and multi-expert routing (k ≥ 2) is additionally required at ImageNet scale.

Conclusion: The authors conclude that sparse MoE routing can be effective in vision classification, but requires careful consideration of factors such as compute leverage, multi-expert routing, and batch-axis dispatch.
...
TL;DR: The paper proposes AnyAct, a method for deriving initial human reenactments from monocular videos of non-human characters, enabling plausible and editable human performances for animation.


<details>
  <summary>Details</summary>
Motivation: Existing motion capture and retargeting methods are limited by their reliance on human-centric structures and 3D source motions, making it difficult to reinterpret non-human character motions as human performances.

Method: AnyAct uses conditional human motion generation from transferable sparse local 2D articulated motion, with three key designs: human-motion-only supervision, progressive 3D-to-2D training, and global-local motion decoupling.

Result: AnyAct produces high-fidelity initial human reenactments that preserve essential dynamics of non-human characters, validated through experiments and ablation studies on a constructed benchmark.

Conclusion: The proposed method, AnyAct, effectively addresses the challenge of deriving human reenactments from non-human character videos, offering a practical solution for animation authoring.
...
TL;DR: A new method called DiffVAS is proposed for visual active search, which can search for multiple targets in partially observable environments.


<details>
  <summary>Details</summary>
Motivation: Previous visual active search methods have limitations, such as requiring a known search space and being tailored to specific target objects, which hinders their application in real-world scenarios.

Method: DiffVAS uses a diffusion model to reconstruct the search space from partial observations and a target-conditioned reinforcement learning module to guide the search.

Result: DiffVAS outperforms state-of-the-art methods in searching for diverse objects in partially observable environments on several datasets.

Conclusion: DiffVAS advances the deployment of visual active search policies in real-world applications, enabling the search for multiple target categories simultaneously in partially observable environments.
...
TL;DR: A new method for scene text editing is proposed, enabling open-vocabulary and style-consistent text editing by leveraging a Multi-Modal Diffusion Transformer.


<details>
  <summary>Details</summary>
Motivation: Existing scene text editing methods neglect visual details of target regions and are limited by pre-trained glyph encoders, degrading the task to text rendering.

Method: A self-prompting scene text editing method using a two-stage training strategy with a Multi-Modal Diffusion Transformer.

Result: The proposed method achieves state-of-the-art performance in text accuracy and style consistency across various languages.

Conclusion: The proposed method effectively addresses the limitations of existing scene text editing methods, enabling more accurate and style-consistent text editing.
...
TL;DR: A new tuning-free video editing framework is proposed, leveraging noisy latent information to improve editing results.


<details>
  <summary>Details</summary>
Motivation: Current tuning-free video editing methods often underutilize information in noisy latent, leading to unsatisfactory results.

Method: A Structural Noise Initialization Strategy (SNIS) and Noise Guidance Mechanism (NGM) are proposed to improve video editing.

Result: The proposed method achieves better visual quality and state-of-the-art performance.

Conclusion: The proposed framework effectively utilizes noisy latent information to improve video editing results.
...
TL;DR: A novel method called DSS-USOD is proposed for underwater salient object detection, which uses dynamic structural specialization to improve object localization and boundary prediction in degraded underwater images.


<details>
  <summary>Details</summary>
Motivation: Existing underwater salient object detection methods struggle with image degradations, leading to inaccurate object localization and coarse boundary prediction.

Method: DSS-USOD uses a shared base representation from a single underwater image, decomposes it into boundary-sensitive and region-coherent structural features, and dynamically coordinates their contributions according to local structural context.

Result: DSS-USOD achieves superior performance on benchmark datasets and shows practical effectiveness in real-world deployment on an underwater robot.

Conclusion: The proposed DSS-USOD method effectively addresses the challenges of underwater image degradations and achieves improved performance in underwater salient object detection.
...
TL;DR: RoiMAM, a new efficient Vision-Language Model, improves medical visual question answering with reduced model size and increased accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing medical visual question answering models are limited by large architectures and closed-set answers, hindering their efficiency and clinical applicability.

Method: RoiMAM integrates a training-free ROI Generation Module with Semantic Selective Suppression and a Text Prompt Enhancer module to focus on relevant regions and provide modality-specific context.

Result: RoiMAM achieves efficient and accurate diagnosis with less than 20% of the model size of MedVInT-TD, improving accuracy by approximately 2% on SLAKE and 4.6% on PMC-VQA.

Conclusion: RoiMAM offers a more efficient and accurate solution for medical visual question answering, making it more suitable for clinical applications.
...
TL;DR: The paper introduces MI-CXR, a benchmark for evaluating longitudinal reasoning over multi-visit chest X-ray sequences, and assesses the performance of state-of-the-art vision-language models.


<details>
  <summary>Details</summary>
Motivation: Current medical VQA benchmarks focus on single images or short-horizon image pairs, but longitudinal chest X-ray interpretation requires reasoning over disease evolution across multiple patient visits.

Method: The authors introduce MI-CXR, a benchmark comprising five-way multiple-choice questions over five-visit patient timelines, and evaluate 14 state-of-the-art vision-language models.

Result: The evaluation shows low overall performance, with an average accuracy of 29.3%, and reveals key limitations of current vision-language models in enforcing temporal constraints and composing evidence into globally consistent decisions.

Conclusion: The findings establish MI-CXR as a principled benchmark for longitudinal medical reasoning and highlight the need for improving vision-language models for this task.
...
TL;DR: The paper proposes LDGuid, a framework that learns and injects semantic differences into change detection models, improving their performance and robustness.


<details>
  <summary>Details</summary>
Motivation: Modern deep learning models for change detection struggle to represent task-relevant semantic differences.

Method: The LDGuid framework uses adversarial autoencoding and the information bottleneck method to learn a difference embedding module, which is then used as guidance in change detection models.

Result: LDGuid enhances segmentation performance across all benchmarks, with significant gains in challenging settings affected by spectral noise.

Conclusion: The study suggests that semantic difference learning can drastically enhance the robustness of change detection in remote sensing.
...
TL;DR: A novel method for estimating 3D human poses from a single view without 3D supervision, leveraging 2D diffusion priors.


<details>
  <summary>Details</summary>
Motivation: To estimate 3D human poses from a single view without requiring 3D supervision, which can be difficult to obtain.

Method: Conditional multi-view ancestral sampling (cMAS) that optimizes 3D pose using 2D diffusion priors and anatomical constraints.

Result: Better cross-domain performance compared to state-of-the-art methods on the Yoga dataset, including extreme human poses.

Conclusion: The proposed method effectively estimates 3D human poses from a single view without 3D supervision.
...
TL;DR: The paper proposes Adaptive Geodesic Correction (AGC), a training-free defense mechanism against adversarial perturbations for CLIP models, achieving superior performance and efficiency.


<details>
  <summary>Details</summary>
Motivation: The susceptibility of vision-language models like CLIP to imperceptible adversarial perturbations is a critical security concern, and existing test-time defenses incur significant computational overhead.

Method: The paper proposes Adaptive Geodesic Correction (AGC), which identifies a reliable augmentation as a geometric anchor and corrects the input feature towards it, utilizing an adaptive step size.

Result: AGC achieves superior performance across eight fine-grained datasets and three CLIP backbones, improving average robust accuracy by 44.4% over state-of-the-art baseline while delivering a 10x reduction in inference latency.

Conclusion: The paper reveals a fundamental geometric property of CLIP features and offers a highly efficient and effective paradigm for robust multimodal deployment.
...
TL;DR: The authors propose a decoupled framework for few-step image generation, improving efficiency and generation quality.


<details>
  <summary>Details</summary>
Motivation: Current few-step image generation methods suffer from training instability, limited scalability, and computational inefficiency.

Method: The authors decouple the framework into a fixed pretrained image encoder and a separate latent denoising model trained in a spherical latent space.

Result: The proposed method outperforms Sphere Encoder in generation quality and inference speed on Animal-Faces, Oxford-Flowers, and ImageNet-1K datasets.

Conclusion: The decoupled framework improves efficiency and generation quality, achieving competitive results against strong baselines.
...
TL;DR: The paper proposes COVER, a method to curate 3D assets into sparse panoramic RGB-D-pose data, and introduces CM-EVS, a dataset with 36,373 curated frames from 1,275 indoor scenes.


<details>
  <summary>Details</summary>
Motivation: Existing 3D visual learning methods lack a sparse, comparable, and geometry-consistent panoramic training interface due to limitations in data collection and processing.

Method: The authors propose COVER (Coverage-Oriented Viewpoint curation with ERP Range-depth warping), a training-free ERP viewpoint curator that projects geometry observed from selected views into candidate ERP probes, scores incremental coverage, and penalizes depth conflicts.

Result: The proposed COVER method improves the coverage-conflict trade-off, and the resulting CM-EVS dataset provides a sparse, compact, and auditable RGB-D-pose resource for geometry-consistent panoramic 3D learning.

Conclusion: The paper concludes that COVER and CM-EVS provide a solution for creating high-quality, sparse, and geometry-consistent panoramic 3D data, which can benefit 3D visual learning tasks.
...
TL;DR: The study compares the performance of four pretrained encoders (supervised classification, contrastive learning, masked reconstruction, and self-distillation) on a custom dataset for emerald inclusion grading, and provides guidance on selecting the best pretraining objective for low-data fine-grained classification.


<details>
  <summary>Details</summary>
Motivation: The motivation is to provide principled guidance for selecting pretrained encoders in expert domains where labeling is expensive, specifically in extreme low-data fine-grained classification.

Method: The study uses a custom dataset of labeled images across three classes, and compares four frozen ViT-B/16 encoders trained with different pretraining objectives. The evaluation is done using leave-one-out cross-validation with linear and nonlinear probes, and permutation testing (N=1000) on macro one-vs-rest AUC.

Result: The results show that supervised and contrastive encoders provide the strongest linear separability, while masked reconstruction (MAE) improves under nonlinear probes. Self-distillation (DINOv3) underperforms across probe families.

Conclusion: The study concludes that for extreme low-data fine-grained classification, it is recommended to prioritize margin-enforcing pretraining objectives when data scarcity restricts probing to linear decision rules, and consider reconstruction-style encoders when nonlinear classifiers are feasible given dataset constraints.
...
TL;DR: The paper proposes NeRP, a plug-and-play prompting correction strategy to improve transfer learning of vision-language models on unseen classes without sacrificing performance on known classes.


<details>
  <summary>Details</summary>
Motivation: The Base-New Trade-off (BNT) in vision-language models, where improving performance on unseen classes degrades accuracy on known classes, and the observation of asymmetric confusion in VLMs.

Method: NeRP, a prompting correction strategy that leverages neutral text prompts and reference images to measure class-wise prior preferences and correct prior-dominated mispredictions.

Result: NeRP substantially improves accuracy on unseen classes while preserving known-class prediction performance across multiple backbones and 15 few-shot and cross-domain benchmarks.

Conclusion: The proposed NeRP strategy effectively addresses the Base-New Trade-off in vision-language models, enabling improved performance on unseen classes without sacrificing accuracy on known classes.
...
TL;DR: This paper compares four self-supervised video models as potential world models, evaluating their robustness across five axes, and finds that latent-prediction models have distinct advantages in terms of corruption robustness, fine-grained discrimination, and temporal sensitivity.


<details>
  <summary>Details</summary>
Motivation: The evaluation of self-supervised video models as world models is limited to a single top-1 accuracy score, leaving a gap in understanding their potential. The authors aim to address this gap by systematically studying four frontier video foundation models.

Method: The authors analyze four matched-capacity video foundation models (V-JEPA 2.1, V-JEPA 2, VideoPrism, and VideoMAEv2) across five robustness axes: feature discriminability, corruption robustness, fine-grained discrimination, occlusion robustness, and sensitivity to temporal direction.

Result: The evaluations show that latent-prediction models have advantages in corruption robustness, fine-grained physical contact cues, and encoding the arrow of time. A frozen V-JEPA 2 backbone with a lightweight attentive probe outperforms a fully fine-tuned VideoMAE and a supervised TimeSformer on corruption and occlusion robustness.

Conclusion: The study provides evidence in favor of latent prediction for robust world modeling, highlighting the benefits of self-supervised video models as world models.
...
TL;DR: The paper proposes LRCP, a training-free framework that compresses visual tokens in large vision-language models by identifying a dominant low-rank subspace and retaining tokens that are poorly explained by it, achieving significant token reduction while preserving performance.


<details>
  <summary>Details</summary>
Motivation: The inference cost of large vision-language models grows rapidly with the number of visual tokens, especially for high-resolution images and long videos, and existing compression methods have limitations such as positional bias or overlooking global structure.

Method: The proposed LRCP framework uses PCA to estimate the dominant low-rank subspace of visual tokens and scores each token by its projection residual onto this subspace, retaining tokens that are poorly explained by the low-rank background.

Result: LRCP achieves superior results, preserving 94.7% of the original image-understanding performance with an 88.9% token reduction and 97.8% of the average video-understanding accuracy with an 87.5% token reduction.

Conclusion: The paper concludes that visual token representations exhibit a pronounced low-rank structure, and LRCP is an effective training-free compression framework that can significantly reduce the number of visual tokens while preserving performance.
...
TL;DR: The paper proposes a Generalized Multi-view Auto-Encoder (GMAE) framework for Multi-View Clustering (MVC) that preserves cross-view complementarity through disentangled representation learning, outperforming state-of-the-art methods on 13 benchmark datasets.


<details>
  <summary>Details</summary>
Motivation: Existing deep MVC methods struggle with view-distribution entanglement during cross-view fusion, leading to suboptimal results.

Method: The GMAE framework uses dual-path autoencoders to decouple source features into view-specific and view-common embeddings, and constructs cross-view adversarial discriminators to guide view-specific encoders.

Result: GMAE consistently outperforms state-of-the-art methods in both complete and incomplete MVC tasks on 13 benchmark datasets.

Conclusion: The proposed GMAE framework effectively addresses the issue of view-distribution entanglement, leading to robust and non-trivial embeddings for MVC tasks.
...
TL;DR: MaTe is a streamlined diffusion framework for material transfer that eliminates textual guidance and reference networks, achieving high-quality material generation under a zero-shot, training-free paradigm.


<details>
  <summary>Details</summary>
Motivation: Recent diffusion-based methods for material transfer face challenges including text dependency, extra computational costs, and feature misalignment.

Method: MaTe integrates input images at the token level, enabling unified processing via multi-modal attention in a shared latent space, removing the need for additional adapters, ControlNet, inversion sampling, or model fine-tuning.

Result: MaTe outperforms state-of-the-art methods in both visual quality and efficiency while preserving precise detail alignment, significantly simplifying inference prerequisites.

Conclusion: MaTe achieves high-quality material generation under a zero-shot, training-free paradigm, outperforming state-of-the-art methods in visual quality and efficiency.
...
TL;DR: A new method called Velocity-Adaptive Guidance Scale (VAGS) is proposed to improve the control over text-to-image synthesis by adapting the guidance scale based on the velocity fields.


<details>
  <summary>Details</summary>
Motivation: The standard practice of holding the guidance scale fixed across the entire ODE trajectory is a fundamental mismatch, as early steps are noise-dominated and late steps demand stronger directional commitment.

Method: VAGS multiplies the nominal scale by a bounded factor combining a temporal signal-level term with the cosine similarity between task-relevant velocity fields.

Result: VAGS consistently improves structural fidelity and generation quality over fixed CFG and recent training-free guidance variants on various benchmarks.

Conclusion: The proposed VAGS method provides a training-free and efficient way to improve the control over text-to-image synthesis, achieving better results than existing methods.
...
TL;DR: A new large-scale hyperspectral dataset, ChronoEarth-492K, and a benchmark, ChronoEarth-Benchmark, are introduced for spatiotemporal hyperspectral representation learning.


<details>
  <summary>Details</summary>
Motivation: Existing hyperspectral self-supervised learning datasets are temporally shallow, limiting long-horizon spatiotemporal modeling.

Method: The ChronoEarth-492K dataset is built using NASA's EO-1 Hyperion mission data, and the ChronoEarth-Benchmark is a unified evaluation suite for static, short-horizon, and long-horizon temporal tasks.

Result: Extensive baseline results are reported across state-of-the-art hyperspectral foundation models, providing a platform for systematic spatiotemporal hyperspectral representation learning.

Conclusion: The ChronoEarth dataset and benchmark provide a large-scale, temporally grounded platform for hyperspectral representation learning.
...
TL;DR: The study examines the ability of vision-language models (VLMs) to perform line tracing, finding that even state-of-the-art models struggle with this task due to local competition from similar distractors.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this study is to investigate the ability of VLMs to robustly control basic visual operations, specifically line tracing, where a model must follow a selected visual path through successive local continuations.

Method: The study designs controlled tracing tasks with nearby competitors to isolate the line tracing ability and reduce semantic and topological ambiguity. The researchers also conduct behavioral interventions and internal analyses to understand the failures.

Result: The results show that even state-of-the-art VLMs frequently lose the target path and switch to nearby alternatives, especially when those alternatives look locally similar to the target. The study also finds that standard remedies such as model-size scaling, reasoning, and explicit tracing instructions do not effectively address this issue.

Conclusion: The study concludes that local competition from similar distractors is a significant bottleneck for VLMs in line tracing tasks, and that this failure persists even in more complex visual scenarios such as tangled-cable scenes and metro maps.
...
TL;DR: DreamSR, a novel super-resolution model, is proposed to address over-generation and texture synthesis issues in diffusion-based methods, achieving high-quality results.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion-based super-resolution methods suffer from over-generation and fail to generate detailed textures in local patches due to misalignment between global and local semantic information.

Method: DreamSR uses a dual-branch MM-ControlNet with a ControlNet for local textual features and a pre-trained DiT for global textual features, along with a comprehensive training strategy.

Result: DreamSR outperforms state-of-the-art methods, providing high-quality super-resolution results with ultra-high-quality details.

Conclusion: The proposed DreamSR model effectively suppresses local over-generation and improves fine-detail synthesis, achieving visually faithful results.
...
### [43] [ElasticDiT: Efficient Diffusion Transformers via Elastic Architecture and Sparse Attention for High-Resolution Image Generation on Mobile Devices](https://arxiv.org/abs/2605.15684)
...
TL;DR: ElasticDiT is a novel Diffusion Transformer architecture that dynamically balances image generation fidelity and latency on mobile devices, outperforming task-specific baselines.


<details>
  <summary>Details</summary>
Motivation: Deploying high-fidelity image generation models on resource-constrained mobile devices is challenging due to their prohibitive computational and memory overhead.

Method: ElasticDiT adjusts spatial compression ratios and DiT block depths, integrating Shift Sparse Block Attention (SSBA) and a Tiny DWT-Distilled VAE (T-DVAE) to reduce latency and memory footprint.

Result: ElasticDiT achieves a wide range of fidelity-latency trade-offs within a single set of parameters, outperforming task-specific baselines, with a flex lite variant achieving an HPS of 32.87 and 84.16% average sparsity.

Conclusion: ElasticDiT offers a versatile, hardware-adaptive solution for high-resolution image generation on mobile devices, eliminating the need for multiple specialized models.
...
### [44] [How to Choose Your Teacher for Fine Grained Image Recognition](https://arxiv.org/abs/2605.15689)
*Oswin Gosal,Edwin Arkel Rios,Augusto Christian Surya,Fernando Mikael,Bo-Cheng Lai,Min-Chun Hu*

Main category: cs.CV

TL;DR: The paper introduces a teacher selection metric called Ratio 1-2 for knowledge distillation in fine-grained image recognition, improving teacher selection by 18% and enabling small student models to achieve up to 17% accuracy gains.


<details>
  <summary>Details</summary>
Motivation: State-of-the-art models for fine-grained image recognition are often too resource-intensive for deployment on constrained devices, and knowledge distillation is used to transfer knowledge from a large teacher model to a smaller student model, but selecting the right teacher is a key challenge.

Method: The paper proposes a teacher selection metric called Ratio 1-2, based on teacher prediction ratios, and evaluates it through extensive analysis of over one thousand experiments across 3 students, 8 teachers, and 8 datasets under 4 training strategies.

Result: The proposed metric improves teacher selection by 18% over previous methods, enabling small student models to achieve up to 17% accuracy gains.

Conclusion: The paper concludes that the proposed teacher selection metric, Ratio 1-2, is effective in improving the performance of small student models in fine-grained image recognition through knowledge distillation.
...
### [45] [3D Segmentation Using Viewpoint-Dependent Spatial Relationships](https://arxiv.org/abs/2605.15708)
*Ayaka Nanri,Klara Reichard,Mert Kiray,Federico Tombari,Benjamin Busam,Asako Kanezaki*

Main category: cs.CV

TL;DR: A new dataset and approach for 3D referring segmentation that incorporates viewpoint awareness, improving model performance on spatial relations.


<details>
  <summary>Details</summary>
Motivation: Current 3D referring segmentation methods lack explicit representation of observer viewpoint, causing ambiguity in spatial relations.

Method: Introduced a viewpoint-aware dataset with 220k samples and a viewpoint representation that encodes camera poses to condition the model on the observation viewpoint.

Result: Improved segmentation accuracy on viewpoint-dependent relations, increasing mIoU from 0.30 to 0.47.

Conclusion: The proposed dataset and approach improve 3D referring segmentation by incorporating viewpoint awareness, highlighting the importance of viewpoint-conditioned models.
...
### [46] [EntropyScan: Towards Model-level Backdoor Detection in LVLMs via Visual Attention Entropy](https://arxiv.org/abs/2605.15711)
*Xuanyu Ge,Zhongqi Wang,Jie Zhang,Shiguang Shan,Xilin Chen*

Main category: cs.CV

TL;DR: A new method called EntropyScan is proposed for detecting backdoor attacks in large vision-language models. It works by analyzing visual attention distributions and achieves high detection accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing defense methods for backdoor attacks in large vision-language models focus on sample-level defense and require knowledge of training data or triggers. However, detecting whether a given model is backdoored remains a critical but unexplored task.

Method: EntropyScan, a lightweight and trigger-agnostic method for model-level backdoor detection. It extracts visual attention distributions from the initial layers of the Large Language Model and applies Tsallis entropy to capture structural distortions.

Result: EntropyScan achieves an F1 score of 98.5% and an AUC of 96.6% across two LVLMs architectures and three advanced attack scenarios.

Conclusion: EntropyScan is an effective method for detecting backdoor attacks in large vision-language models, offering a promising solution to address the vulnerability of these models to backdoor attacks.
...
### [47] [Semi-MedRef: Semi-Supervised Medical Referring Image Segmentation with Cross-Modal Alignment](https://arxiv.org/abs/2605.15720)
*Yuchen Li,Zhen Zhao,Yi Liu,Luping Zhou*

Main category: cs.CV

TL;DR: The paper proposes Semi-MedRef, a semi-supervised learning framework for medical referring image segmentation that maintains image-text alignment under perturbations, outperforming baselines across all label regimes.


<details>
  <summary>Details</summary>
Motivation: Medical referring image segmentation requires costly pixel-level masks aligned with textual descriptions, and semi-supervised learning can help mitigate this burden, but existing methods struggle with cross-modal alignment under strong augmentation.

Method: The proposed Semi-MedRef framework consists of three alignment-preserving components: T-PatchMix, a cross-modal CutMix-style augmentation; PosAug, a position-aware text augmentation; and ITCL, a position-guided image-text contrastive learning module.

Result: Experiments on QaTa-COV19 and MosMedData+ demonstrate that Semi-MedRef consistently outperforms both fully supervised and semi-supervised baselines across all label regimes.

Conclusion: The proposed Semi-MedRef framework effectively maintains image-text alignment under perturbations, leading to improved performance in medical referring image segmentation across various label regimes.
...
### [48] [DiLA: Disentangled Latent Action World Models](https://arxiv.org/abs/2605.15725)
*Tianqiu Zhang,Muyang Lyu,Yufan Zhang,Fang Fang,Si Wu*

Main category: cs.CV

TL;DR: DiLA, a novel world model, resolves the trade-off between action abstraction and generation fidelity in video learning by using content-structure disentanglement.


<details>
  <summary>Details</summary>
Motivation: The trade-off between action abstraction and generation fidelity in Latent Action Models (LAMs) for learning world models from unlabeled video.

Method: DiLA uses content-structure disentanglement, where the predictive bottleneck in latent action learning drives disentanglement, separating spatial layouts and visual details into structure and content pathways.

Result: DiLA achieves superior results in video generation quality, action transfer, visual planning, and manifold interpretability, establishing it as a unified framework for self-supervised world model learning.

Conclusion: DiLA advances the frontier of self-supervised world model learning by simultaneously achieving high-level action abstraction and high-fidelity generation.
...
### [49] [DecomPose: Disentangling Cross-Category Optimization Contention for Category-Level 6D Object Pose Estimation](https://arxiv.org/abs/2605.15728)
*Yifan Gao,Lu Zou,Zhangjin Huang,Guoping Wang*

Main category: cs.CV

TL;DR: DecomPose, a framework for category-level 6D object pose estimation, mitigates optimization contention across categories by difficulty-aware decomposition.


<details>
  <summary>Details</summary>
Motivation: Category-level 6D object pose estimation faces challenges due to geometric heterogeneity across categories, leading to gradient conflicts and negative transfer during training.

Method: DecomPose, a difficulty-aware decomposition framework, uses gradient-based diagnostics to quantify module-level cross-category contention and proposes difficulty-aware gradient decoupling and stability-driven asymmetric branching.

Result: DecomPose delivers superior pose estimation performance across multiple benchmarks, including REAL275, CAMERA25, and HouseCat6D.

Conclusion: DecomPose effectively reduces cross-category optimization contention and improves pose estimation performance.
...
### [50] [UAM: A Dual-Stream Perspective on Forgetting in VLA Training](https://arxiv.org/abs/2605.15735)
*Jianke Zhang,Yuanfei Luo,Yucheng Hu,Xiaoyu Chen,Yanjiang Guo,Ziyang Liu,Hongbin Xu,Tian Lan,Jianyu Chen*

Main category: cs.CV

TL;DR: The paper proposes a new architecture, Unified Action Model (UAM), to prevent the erosion of multimodal competence in vision-language-action models. UAM adds a parallel Dorsal Expert pathway, initialized from a pretrained generative model and trained with a mid-level reasoning objective. This design allows UAM to retain over 95% of the underlying VLM's multimodal capability and achieve the highest average success rate on various manipulation tasks.


<details>
  <summary>Details</summary>
Motivation: The standard recipe of fine-tuning a pretrained vision-language model (VLM) on action data erodes the VLM's multimodal competence, known as the embodiment tax. The authors aim to prevent this erosion and propose a new architecture to achieve this goal.

Method: The authors propose the Unified Action Model (UAM), which adds a parallel Dorsal Expert pathway to the existing ventral pathway. The Dorsal Expert is initialized from a pretrained generative model and trained with a mid-level reasoning objective that predicts visual dynamics. The UAM is trained end-to-end on action data alone, without parameter freezing, gradient stopping, or auxiliary VL co-training.

Result: UAM retains over 95% of the underlying VLM's multimodal capability and achieves the highest average success rate among baselines on various manipulation tasks that probe out-of-distribution generalization, including unseen objects, novel object-target compositions, and instruction variation.

Conclusion: The authors conclude that semantic preservation in vision-language-action models can emerge from architectural separation itself, rather than being enforced by frozen weights or auxiliary data replay. They also find that the preserved semantic capability can naturally transfer from VLMs to semantic generalization in actions.
...
### [51] [BiomedAP: A Vision-Informed Dual-Anchor Framework with Gated Cross-Modal Fusion for Robust Medical Vision-Language Adaptation](https://arxiv.org/abs/2605.15736)
*Huanyang Tong,Kai Liu,Fangjun Kuang,Huiling Chen*

Main category: cs.CV

TL;DR: The paper proposes BiomedAP, a framework that improves the robustness of biomedical vision-language models to prompt variations, achieving better few-shot accuracy and stability.


<details>
  <summary>Details</summary>
Motivation: Biomedical vision-language models face a bottleneck due to their fragility to prompt variations, which hinders their performance in few-shot medical diagnosis.

Method: The proposed BiomedAP framework uses gated cross-modal fusion and a dual-anchor constraint to enforce synergistic alignment between visual and textual prompts.

Result: BiomedAP consistently surpasses baselines across 11 benchmarks, achieving competitive few-shot accuracy and markedly enhanced robustness under prompt perturbations.

Conclusion: The BiomedAP framework offers a more robust approach to few-shot medical diagnosis by addressing the fragility of biomedical vision-language models to prompt variations.
...
TL;DR: A new multimodal program synthesis framework called MultiMat is proposed to generate procedural material graphs, which are essential in computer graphics for representing the appearance of virtual 3D objects. The framework leverages large multimodal models to process both visual and textual graph representations, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: The creation of material node graphs remains challenging and typically requires professional training. Recent neural program synthesis approaches have limitations in capturing the visual-spatial nature of node graphs.

Method: A multimodal program synthesis framework called MultiMat is proposed, which combines large multimodal models with a constrained tree search inference algorithm to generate procedural material graphs.

Result: The experimental results show that the multimodal program synthesis method is more efficient in both unconditional and conditional graph synthesis with higher visual quality and fidelity than text-only baselines, establishing new state-of-the-art performance.

Conclusion: The proposed MultiMat framework has the potential to simplify the creation of procedural material graphs, making it more accessible to users without professional training.
...
TL;DR: The paper proposes BARRIER, a new framework for machine unlearning that preserves neutral concepts while erasing targeted ones, achieving state-of-the-art trade-offs in concept erasure and knowledge retention.


<details>
  <summary>Details</summary>
Motivation: Traditional machine unlearning methods often cause collateral damage to essential knowledge, and the authors aim to develop a method that prevents this damage while effectively erasing targeted concepts.

Method: The authors propose BARRIER, which uses Interval Arithmetic (IA) on SVD-based projections of the activation space to bound the target region for unlearning, ensuring the preservation of neutral concepts.

Result: Empirical evaluations show that BARRIER matches state-of-the-art trade-offs across classifiers and diffusion models, effectively erasing targeted concepts while safeguarding other representations.

Conclusion: The BARRIER framework provides a formal optimization target for preserving knowledge and achieves rigorous protection of the retain distribution, enabling highly aggressive unlearning updates within the forget region.
...
TL;DR: HyperDiT, a unified framework for high-fidelity pixel generation, addresses the granularity dilemma in pixel-space diffusion models by establishing hyper-connected cross-scale interactions.


<details>
  <summary>Details</summary>
Motivation: The granularity dilemma in pixel-space diffusion models, where capturing global semantics favors large patch scales, while generating high-fidelity details demands fine-grained inputs.

Method: HyperDiT utilizes Cross-Attention mechanisms, Scale-Aware Rotary Position Embedding (SA-RoPE), and Registers to learn dense semantics from a pretrained Visual Foundation Model (VFM).

Result: HyperDiT achieves state-of-the-art (SoTA) FID of 1.56 on ImageNet 256x256 directly within the pixel space.

Conclusion: HyperDiT offers a superior paradigm for high-fidelity pixel generation by combining fine-grained stream with semantic guidance.
...
TL;DR: The paper proposes a method to improve multimodal large language models' (MLLMs) ability to explain artwork emotions by selectively focusing on emotionally operative attributes.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs suffer from attribute flooding, enumerating many visible formal attributes without identifying which cues support the affective judgment.

Method: The authors propose FAB-G, a supervised multi-agent framework that predicts attribute-level salience and constrains downstream emotional analysis to retained cues.

Result: Experiments show FAB-G yields consistent gains in emotion, arousal, and valence prediction, achieves stronger agreement with human-marked salient attributes, and produces more compact explanations.

Conclusion: The proposed method improves MLLMs' ability to understand artwork emotions by selectively focusing on emotionally operative attributes, with positive results in various evaluations.
...
TL;DR: The paper introduces a learned optimizer for 3D Gaussian Splatting (3DGS) that improves optimization efficiency and convergence speed without requiring manual tuning or auxiliary mechanisms.


<details>
  <summary>Details</summary>
Motivation: Standard optimizers (e.g., Adam, SGD) are not tailored to the structure of 3DGS, leading to inefficient optimization and slow convergence. Recent learned optimizers are limited by their reliance on manual scheduling and fixed optimization horizons.

Method: The authors propose a meta-learning scheme that extends the optimization horizon via a checkpoint buffer and an optimizer rollout strategy, combined with an architecture that encodes gradient scale information in its latent states.

Result: The learned optimizer shows improved early novel view synthesis quality, remains stable over long horizons, and generalizes to unseen reconstruction settings without requiring manual tuning.

Conclusion: The paper presents a learned optimizer for 3DGS that outperforms standard optimizers and existing learned optimizers, with the potential to improve the efficiency and effectiveness of 3DGS optimization.
...
TL;DR: VLMs often produce self-reflective statements during reasoning, but experiments show that these statements do not trigger genuine visual re-examination, and models can be easily fooled by visually similar but semantically different images.


<details>
  <summary>Details</summary>
Motivation: To investigate whether self-reflective statements in VLMs trigger genuine visual re-examination or are just learned textual patterns.

Method: VisualSwap, an image-swap probing framework, and VS-Bench, a dataset of 800 image pairs, are introduced to test whether models notice when an image is replaced with a visually similar but semantically different one.

Result: Experiments reveal that models overwhelmingly miss the swap, with accuracy dropping by up to 60%, and thinking models are nearly 3x more vulnerable than their instructed counterparts.

Conclusion: Current VLMs tend to say rather than actually see when claiming to perform visual re-examination, and self-generated reflective statements during continuous generation do not restore visual grounding.
...
TL;DR: Introduced GRASP, a dataset for social reasoning in multi-person videos, and proposed Social Grounding Reward (SGR) to improve model performance.


<details>
  <summary>Details</summary>
Motivation: Current multimodal large language models (MLLMs) struggle to identify interactions in multi-person videos, highlighting the need for better social reasoning capabilities.

Method: Introduced GRASP dataset with 290K QA pairs over 46K videos and proposed Social Grounding Reward (SGR) as a learning signal.

Result: Experiments showed SGR improves performance on GRASP-Bench without compromising zero-shot performance on related social video QA benchmarks.

Conclusion: GRASP and SGR offer advancements in social reasoning for multi-person videos, enhancing model ability to identify interactions.
...
TL;DR: The paper proposes a Generation-to-Understanding (G2U) synergy framework, which uses visual generation as an intermediate step to enhance multimodal understanding. The framework enables a model to generate visual thoughts and feed them back into the model to refine perception.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the asymmetry in multimodal AI, where understanding guides generation, but generation rarely supports understanding. They propose to investigate how generation can support understanding.

Method: The authors propose a G2U synergy framework, which enables a model to perform controlled generative acts, such as detail enhancement, context expansion, or structural visualization, to produce self-generated visual thoughts.

Result: The framework consistently improves multimodal understanding on twelve benchmarks. The authors also find that generative fidelity bounds perceptual gain and that distinct families of edit prompts govern transfer efficiency.

Conclusion: The authors conclude that current large multimodal models fall short of true self-reflection, but their work exposes a missing mechanism in unified cognition and suggests that imagination is not the end of understanding but its beginning.
...
TL;DR: A unified framework for 3D fingerprint preprocessing and registration across contactless and contact-based 2D modalities is proposed, achieving ridge-level 3D registration accuracy and improving compatibility between 3D fingerprints and 2D fingerprints.


<details>
  <summary>Details</summary>
Motivation: The difficulty in integrating 3D fingerprints with legacy 2D fingerprint systems due to the lack of a unified framework for 3D fingerprint preprocessing and registration across different modalities.

Method: A four-component framework: 1) nonparametric visualization and unwrapping of 3D fingerprint point cloud, 2) point-cloud fusion pipeline for registering and mosaicing partial 3D captures, 3) ellipse-based pose normalization for canonical finger alignment, and 4) pose-aware cross-modal registration strategy.

Result: The proposed framework achieves ridge-level 3D registration accuracy, robust pose estimation, and consistent gains in 2D compatibility, with a 3D fusion error of around 0.09 mm and ridge-scale projection accuracy for contactless 2D-3D registration.

Conclusion: The proposed framework supports the use of 3D fingerprints as an effective geometric bridge across heterogeneous fingerprint modalities, enabling seamless integration with legacy 2D fingerprint systems.
...
TL;DR: The paper proposes a novel framework called Embedding-perturbed Exploration Preference Optimization (E^2PO) to address the limitation of group-based optimization frameworks in Reinforcement Learning (RL) for aligning generative models with human intent.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome the challenge of rapid decay of intra-group variance in group-based optimization frameworks, which leads to unstable training and premature stagnation or reward hacking.

Method: The method proposed is Embedding-perturbed Exploration Preference Optimization (E^2PO), which introduces structured, embedding-level perturbations within sample groups to sustain optimization.

Result: The results show that E^2PO significantly outperforms state-of-the-art baselines, achieving a more faithful alignment with human preference.

Conclusion: The conclusion is that E^2PO is a effective framework for addressing the limitations of group-based optimization frameworks in RL and achieving better alignment with human intent.
...
TL;DR: FashionChameleon, a framework for real-time, interactive human-garment customization in video generation, allowing users to switch garments during generation while preserving motion coherence.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for video customization do not support low-latency and interactive garment control, crucial for applications like e-commerce and content creation.

Method: FashionChameleon uses three key techniques: (i) Teacher Model with In-Context Learning on single reference-garment pairs, (ii) Streaming Distillation with In-Context Learning for consistency and efficiency, and (iii) Training-Free KV Cache Rescheduling for interactive multi-garment video customization.

Result: FashionChameleon achieves real-time generation at 23.8 FPS on a single GPU, 30-180 times faster than existing baselines, while supporting interactive customization and consistent long-video extrapolation.

Conclusion: FashionChameleon enables interactive multi-garment video customization with preserved motion coherence, using only single-garment video data, making it suitable for applications requiring low-latency and interactive garment control.
...
TL;DR: Proposed Fisher-Guided Quantization (FGQ) for feed-forward 3D reconstruction models, outperforming state-of-the-art quantization baselines.


<details>
  <summary>Details</summary>
Motivation: Existing Post-Training Quantization (PTQ) methods for feed-forward 3D models treat all tasks equally, causing significant accuracy loss on sensitive tasks due to different sensitivities to quantization errors across tasks, blocks, and channels.

Method: FGQ uses the diagonal Fisher information matrix to quantify sensitivities across tasks, blocks, and channels, and incorporates these into the Learnable Affine Transformation during calibration.

Result: FGQ consistently outperforms state-of-the-art quantization baselines on VGGT, achieving up to 39% relative improvement under 4-bit quantization.

Conclusion: FGQ effectively addresses the issue of different sensitivities to quantization errors in feed-forward 3D reconstruction models, leading to improved accuracy and efficiency.
...
TL;DR: The study examines the mismatch between open-set recognition objectives and ecological monitoring needs in automated plankton image recognition. It proposes Open-Set Community Distortion (OSCD) to evaluate the error in community-level estimates and finds that community-aware threshold calibration improves performance.


<details>
  <summary>Details</summary>
Motivation: The increasing use of automated plankton image recognition in aquatic ecosystem monitoring, and the need to evaluate open-set recognition methods at the community level rather than just sample level.

Method: The study uses controlled pseudo-communities and three datasets to evaluate open-set recognition methods. It defines OSCD, a Bray-Curtis-style error metric, and compares the performance of closed-set classifiers and open-set recognition methods with sample-level and community-aware threshold calibration.

Result: The results show that sample-level OOD metrics are not sufficient to select ecological operating points, and community-aware threshold calibration reduces OSCD relative to fixed 95% known recall on some datasets. The benefit of community-aware calibration depends on validation-community representativeness and the gap between fixed recall and the community optimum.

Conclusion: Open-set plankton recognition should be evaluated as an ecological measurement problem, not only as a sample-level detection task. Community-aware evaluation and calibration are necessary to improve the accuracy of taxon abundance and diversity estimates in ecological monitoring.
...
TL;DR: WorldAct is a framework that converts static 3D worlds into editable and interactive scenes, enabling object-level editing and embodied task execution.


<details>
  <summary>Details</summary>
Motivation: Current 3D world modeling systems produce static and monolithic environments with limited editability and physical interaction, restricting their use in immersive content creation and embodied simulation.

Method: WorldAct uses a multimodal agent to decompose scenes, identify actionable objects, reconstruct object-level meshes, and restore the background via 3D inpainting.

Result: The resulting scenes support object-level editing, collision-aware manipulation, and embodied task execution while preserving global scene coherence.

Conclusion: WorldAct enables richer interaction scenarios than original generated scenes, providing a practical path toward editable and interactive 3D world models.
...
TL;DR: GHOST, a training-free KV cache management framework, is proposed to address the memory bottleneck in streaming 3D reconstruction from long monocular video sequences, preserving reconstruction quality while reducing cache size and improving inference speed.


<details>
  <summary>Details</summary>
Motivation: The need to address the memory bottleneck caused by the growing key-value (KV) cache in streaming 3D reconstruction from long monocular video sequences, which degrades reconstruction quality or relies on heuristics that ignore 3D scene structure.

Method: GHOST (Geometry-Hierarchical Online Streaming Token Eviction), a framework that uses the model's 3D geometry outputs to evict redundant tokens online, comprising a hierarchical dual-level importance scoring scheme, a privilege mechanism, and a cosine-similarity-guided layer-wise budget allocation.

Result: GHOST preserves excellent reconstruction quality, reduces the KV cache by nearly half, and delivers 1.75x faster inference compared to state-of-the-art methods.

Conclusion: GHOST effectively addresses the memory bottleneck in streaming 3D reconstruction, providing a efficient and effective solution for maintaining reconstruction quality while reducing memory requirements.
...
TL;DR: The paper proposes AdaScope, a plug-in that enhances diffusion models with reinforcement learning (RL) more efficiently, improving image generation quality while reducing computational costs.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of diffusion models in aligning with human preferences due to their reconstruction objectives, and the inefficiency of applying RL to the full denoising trajectory.

Method: The method proposed is AdaScope, which adaptively identifies the optimal intervention timing for RL and dynamically terminates training once denoising converges and reward gains saturate.

Result: AdaScope improves performance by 66% and cuts computational costs by 59% compared to state-of-the-art methods.

Conclusion: The conclusion is that AdaScope achieves a 'dual benefit' of reducing computational costs and significantly improving performance in diffusion models for image generation.
...
TL;DR: A practical stereo calibration framework for RGB-thermal infrared (TIR) camera systems with low-cost, low-resolution thermal sensors.


<details>
  <summary>Details</summary>
Motivation: Accurate geometric calibration of RGB-TIR stereo camera systems is essential for multimodal building envelope analysis, but challenging with low-cost thermal sensors.

Method: A stereo calibration framework using an active OLED screen to display modality-specific patterns, a dedicated corner detection algorithm, and baseline-constrained bundle adjustment.

Result: A stereo baseline of 32.7 mm with an overall reprojection error of 0.382 px, and consistent TIR-to-RGB projection for building energy performance assessment.

Conclusion: The proposed framework enables accurate calibration of RGB-TIR stereo camera systems with low-resolution thermal sensors, suitable for building energy performance assessment.
...
TL;DR: The paper proposes a novel self-supervised framework called SOLAR for symmetric multimodal-to-multimodal retrieval, outperforming state-of-the-art methods with significantly fewer parameters.


<details>
  <summary>Details</summary>
Motivation: Existing universal multimodal retrieval works struggle with symmetric multimodal-to-multimodal retrieval due to the constraint of labeled asymmetric datasets.

Method: A two-stage self-supervised framework, SOLAR, that leverages unlabeled web-scale image-text pairs to learn multimodal embeddings.

Result: SOLAR surpasses the strongest supervised VLM by 7.08 points on a new benchmark, with over 50x fewer model parameters and a 5x smaller embedding dimension.

Conclusion: The proposed SOLAR framework effectively addresses the challenge of symmetric multimodal-to-multimodal retrieval, demonstrating superior performance with reduced parameters.
...
TL;DR: The paper proposes DepthVLM, a framework that enables a Vision-Language Model (VLM) to predict dense geometry (depth maps) in a single forward pass, while preserving its multimodal capabilities.


<details>
  <summary>Details</summary>
Motivation: Current VLMs are limited in 3D understanding due to their text-only supervision paradigm, which hinders fine-grained visual perception and dense geometry recovery.

Method: The authors propose attaching a lightweight depth head to the LLM backbone and training under a unified vision-text supervision paradigm with a two-stage schedule.

Result: DepthVLM outperforms existing VLMs, surpasses leading pure vision models, and improves complex 3D spatial reasoning, while being more efficient.

Conclusion: The proposed framework, DepthVLM, enables VLMs to predict dense geometry while preserving their multimodal capabilities, moving towards a truly unified foundation model.
...
TL;DR: This paper presents FSCM, a GAN framework for infrared colorization that leverages spectral information to improve visual quality and semantic fidelity.


<details>
  <summary>Details</summary>
Motivation: Infrared imaging is robust to illumination variations and smoke but lacks natural color and fine texture, limiting target recognition and human interpretation. Existing infrared colorization methods have limitations, particularly with single-band images.

Method: FSCM uses a frequency-enhanced spatial-spectral state-space generator with cascaded FSB units, integrating state-space modeling, frequency enhancement, and dual-stream hybrid gating. It also introduces an online semantic segmentation-guided loss.

Result: Experiments show FSCM outperforms existing methods in visual quality and semantic fidelity, particularly in complex road scenes.

Conclusion: FSCM effectively addresses the limitations of existing infrared colorization methods, providing improved visual quality and semantic consistency.
...
TL;DR: A probabilistic framework for categorizing wildfire smoke severity from satellite images, providing uncertainty estimates and outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning approaches for smoke detection are limited as they treat it as a binary task and do not provide measures of prediction confidence, which are crucial for emergency response and air quality modeling.

Method: A probabilistic framework using a pre-trained EfficientNet-B3 backbone, CBAM module, and an evidential deep learning head to predict Dirichlet concentration parameters for estimating uncertainty.

Result: The model achieves 93.8% weighted test accuracy and 96.7% accuracy on the most certain 50% of patches, with uncertainty increasing as image quality degrades.

Conclusion: The proposed framework provides a reliable approach for wildfire smoke severity assessment with uncertainty estimates, which can aid in emergency response and air quality modeling.
...
TL;DR: The paper proposes a framework for organizing and interpreting image degradations, providing a compact taxonomy and a severity measurement layer to make results comparable across datasets and tasks.


<details>
  <summary>Details</summary>
Motivation: Image degradations occur during acquisition, processing, and transmission, affecting visual appearance and downstream vision tasks. However, different communities study these degradations using incompatible grouping schemes and severity definitions, making results difficult to compare.

Method: The authors propose a causally grounded framework with a dual-axis abstraction, describing degradations by their dominant causal source and perceptual effect. They also introduce a lightweight severity measurement layer using full reference image quality metrics.

Result: The framework is demonstrated through COCO Degradation, a taxonomy-aligned benchmark for evaluating object detector robustness under diverse imaging conditions.

Conclusion: The proposed framework provides a unified way to organize and interpret image degradations, making results comparable across datasets and tasks, and facilitating the evaluation of robustness in vision tasks.
...
TL;DR: Proposed RaPD for resolution-flexible image generation in continuous neural fields.


<details>
  <summary>Details</summary>
Motivation: Current generative models synthesize images on discrete grids, limiting resolution flexibility.

Method: RaPD performs diffusion in a continuous Neural Image Field (NIF) latent space with Semantic Representation Guidance and Coordinate-Queried Attention Renderer.

Result: Superior generation quality and resolution scalability achieved in experiments.

Conclusion: RaPD enables resolution-free image generation with a single denoised latent.
...
TL;DR: Proposed AdaEraser, an adaptive framework for object removal in images that dynamically modulates attention to improve inpainting quality.


<details>
  <summary>Details</summary>
Motivation: Current training-free object removal methods can degrade image generation quality by indiscriminately suppressing self-attention in vacated areas.

Method: AdaEraser uses a token-wise adaptive attention suppression strategy, adjusting self-attention layers based on estimated target object concept presence.

Result: AdaEraser achieves superior object removal performance, outperforming even training-based methods in extensive experiments.

Conclusion: AdaEraser effectively solves the conflict between object removal and image generation quality by adaptively modulating attention.
...
TL;DR: The paper proposes Invaria, a point cloud encoder that achieves scale and density invariance, leading to significant performance gains during resolution shifts while maintaining high efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing 3D point cloud encoders fail to generalize well due to sensitivity to sampling resolution and scale changes, hindering real-world deployment in robotics.

Method: The authors propose Invaria, which achieves scale and density invariance through next-resolution prediction and receptive field calibration.

Result: Invaria achieves significant performance gains on ScanNet, with a 56.0% higher mIoU at 3x lower resolution and a 20% improvement when object scale is reduced by a factor of 3, while maintaining a 45% smaller model size and 40% reduction in input tokens.

Conclusion: The proposed Invaria encoder effectively mitigates the dependency on sampling resolution and scale changes, leading to improved generalization and efficiency in 3D point cloud encoding.
...
TL;DR: The paper proposes a Decomposed Vision-Language Alignment framework for open-vocabulary segmentation, improving generalization to unseen object categories and attributes.


<details>
  <summary>Details</summary>
Motivation: Open-vocabulary segmentation models struggle to generalize to unseen combinations of object categories and attributes due to holistic sentence encoding.

Method: The framework factorizes textual prompts into concept and attribute tokens, using Feature-Gated Cross-Attention and log-space similarity aggregation.

Result: The method significantly improves generalization to unseen attribute-category compositions in fine-grained open-vocabulary segmentation benchmarks.

Conclusion: The proposed framework enables better compositional semantics and improves open-vocabulary segmentation performance.
...
TL;DR: The paper proposes a group-revision optimisation paradigm to enhance learning on hard cases for finetuning large vision-language models with reinforcement learning, achieving consistent gains across various benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing methods for finetuning large vision-language models with reinforcement learning have limitations, such as sparse rewards at the response level, leading to minimal learning signals in challenging scenarios.

Method: The proposed method introduces a group-revision optimisation paradigm, which generates a set of revised candidates from a sampled initial response, and uses a consolidation process to quantify each candidate's improvement and convert it into informative shaping signals.

Result: The method achieves consistent gains across referring and reasoning segmentation, REC, and counting benchmarks compared with prior GRPO-based models.

Conclusion: The proposed group-revision optimisation paradigm enhances learning on hard cases for finetuning large vision-language models with reinforcement learning, leading to improved performance across various benchmarks.
...
TL;DR: The paper proposes a novel method called SAE-FT for robust fine-tuning of large-scale pre-trained vision-language models, which operates only on the model's visual representations and prevents catastrophic forgetting.


<details>
  <summary>Details</summary>
Motivation: Fine-tuning large-scale pre-trained vision-language models often degrades robustness against distribution shifts, and recent approaches to mitigate this trade-off are computationally expensive.

Method: The proposed method, SAE-FT, regularizes changes to visual representations by penalizing the addition and removal of semantically meaningful features identified by a Sparse Autoencoder trained on the pre-trained model.

Result: SAE-FT matches or exceeds state-of-the-art performance on ImageNet and its associated distribution shift benchmarks.

Conclusion: The proposed method, SAE-FT, provides a mechanistically transparent and computationally efficient approach to robust fine-tuning of large-scale pre-trained vision-language models.
...
TL;DR: Flash-GRPO, a single-step training framework, improves training efficiency and alignment quality for video diffusion models under low computational budgets.


<details>
  <summary>Details</summary>
Motivation: The need to address the computational bottleneck in training large video diffusion models (e.g., 14B parameters) which requires hundreds of GPU days per experiment.

Method: Flash-GRPO, which includes iso-temporal grouping to eliminate timestep-confounded variance and temporal gradient rectification to neutralize time-dependent scaling factors.

Result: Flash-GRPO outperforms full trajectory training in alignment quality, substantially improves training efficiency, and demonstrates stability and state-of-the-art alignment quality across 1.3B to 14B parameter models.

Conclusion: Flash-GRPO effectively addresses the challenges in training large video diffusion models, offering improved efficiency and alignment quality.
...
### [81] [Echo-Forcing: A Scene Memory Framework for Interactive Long Video Generation](https://arxiv.org/abs/2605.16003)
...
TL;DR: The paper proposes Echo-Forcing, a training-free framework for interactive long video generation, addressing issues with existing methods in handling prompt switching, scene forgetting, and historical scene recall.


<details>
  <summary>Details</summary>
Motivation: Existing training-free long-video optimization methods struggle with interactive scenarios due to the functional entanglement of historical KV states, leading to issues like outdated background contamination and loss of long-range memory.

Method: The Echo-Forcing framework includes three core mechanisms: Hierarchical Temporal Memory, Scene Recall Frames, and Difference-aware Memory Decay, which work together to support smooth transitions, hard cuts, and long-range scene recall under a bounded cache budget.

Result: Echo-Forcing achieves the best overall performance in both long-video generation and interactive video generation settings, as demonstrated by extensive evaluations on VBench-Long.

Conclusion: The proposed Echo-Forcing framework effectively addresses the challenges of interactive long video generation, offering a robust solution for applications requiring dynamic scene handling and memory.
...
### [82] [End-to-end plaque counting and virus titration from laboratory plate images with deep learning](https://arxiv.org/abs/2605.16008)
...
TL;DR: A computer-aided workflow for cytopathic effect-based virus titration from laboratory plaque assay images, reducing manual annotation effort.


<details>
  <summary>Details</summary>
Motivation: Plaque counting from plate images is labor-intensive and prone to inter-operator variability, necessitating an automated solution.

Method: The proposed approach combines two models derived from the Segment Anything Model (SAM): a well-segmentation module and a plaque-segmentation model.

Result: The workflow showed strong agreement with manual annotations (Pearson correlation coefficients of 0.92 and 0.88) and high concordance with expert annotations.

Conclusion: The proposed system will enable reproducible, scalable, and audit-ready plaque assay analysis while substantially reducing manual annotation effort.
...
### [83] [Robust Prior-Guided Segmentation for Editable 3D Gaussian Splatting](https://arxiv.org/abs/2605.16065)
*Raushan Joshi,Jean-Yves Guillemaut*

Main category: cs.CV

TL;DR: A novel framework for robust 3D segmentation and editing using 3D Gaussian Splatting, leveraging SAM-HQ for accurate 2D masks and prior-guided label reassignment for 3D Gaussians.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for 3D segmentation suffer from view inconsistencies and coarse masks, limiting editing tasks such as object removal, extraction, and recoloring.

Method: The proposed framework uses SAM-HQ to generate accurate 2D masks and a prior-guided label reassignment method to assign labels to 3D Gaussians, enforcing multiview consistency.

Result: State-of-the-art segmentation accuracy, interactive and real-time object editing, and high visual fidelity, with qualitative results demonstrating superior boundary preservation and practical utility in VR and robotics.

Conclusion: The proposed framework advances 3D scene editing by enabling robust 3D segmentation and editing with high visual fidelity, addressing limitations of existing approaches.
...
TL;DR: A unified framework for physics-aware reconstruction and simulation of endoscopic scenes in robot-assisted surgery using Multi-modal Large Language Models (MLLMs)-guided Gaussian Splatting.


<details>
  <summary>Details</summary>
Motivation: Existing methods for endoscopic scene reconstruction and simulation lack physics-based descriptions, which are crucial for realistic simulation and advancing surgical outcomes.

Method: The proposed framework utilizes 4D Gaussian Splatting (4DGS) with pre-trained segmentation and depth estimation, and introduces an object-wise material field that initializes material parameters via MLLM and refines them through a differentiable Material Point Method (MPM).

Result: The framework achieves superior simulation fidelity and physical accuracy compared to state-of-the-art methods on both open-source and in-house datasets.

Conclusion: The proposed framework has the potential to advance robot-assisted surgical applications by providing a more realistic and accurate simulation of endoscopic scenes.
...
### [85] [AgriMind: An Ensemble Deep Learning Framework for Multi-Class Plant Disease Classification](https://arxiv.org/abs/2605.16076)
*Salma Hoque Talukdar Koli,Fahima Haque Talukder Jely*

Main category: cs.CV

TL;DR: AgriMind, an ensemble model using ResNet50, EfficientNet-B0, and DenseNet121, achieves 99.23% accuracy in detecting 15 classes of pepper, potato, and tomato diseases, aiming to automate manual disease detection in Bangladesh.


<details>
  <summary>Details</summary>
Motivation: The manual process of plant disease detection in Bangladesh is labor-intensive and inefficient, prompting the development of an automated system to improve disease detection across millions of smallholdings.

Method: An ensemble model combining ResNet50, EfficientNet-B0, and DenseNet121, utilizing transfer learning with frozen ImageNet backbones and 10 epochs of head-only training, was trained on 20,638 PlantVillage images.

Result: The ensemble model achieved 99.23% accuracy, with individual models reaching 96-97% accuracy on a held-out test set. Pepper and potato diseases were classified perfectly, while tomato diseases reached 99.01% accuracy.

Conclusion: The AgriMind ensemble model demonstrates a significant improvement in plant disease detection accuracy and efficiency, with potential for real-time mobile use pending optimization.
...
### [86] [VideoSeeker: Incentivizing Instance-level Video Understanding via Native Agentic Tool Invocation](https://arxiv.org/abs/2605.16079)
...
TL;DR: VideoSeeker, a novel paradigm for instance-level video understanding through visual prompts, achieves a +13.7% improvement over baselines and surpasses powerful closed-source models.


<details>
  <summary>Details</summary>
Motivation: Existing methods for video understanding face challenges in precise spatiotemporal localization and rely heavily on text prompts, which can be imprecise and limit user experience.

Method: VideoSeeker integrates agentic reasoning with instance-level video understanding tasks, using visual prompts and a four-stage data synthesis pipeline for efficient data generation.

Result: VideoSeeker achieves an average improvement of +13.7% over baselines on instance-level video understanding tasks and shows effective transferability on general video understanding benchmarks.

Conclusion: VideoSeeker offers a novel and effective approach to instance-level video understanding, outperforming existing methods and closed-source models, with plans for public release of datasets and code.
...
### [87] [GenShield: Unified Detection and Artifact Correction for AI-Generated Images](https://arxiv.org/abs/2605.16122)
*Zhipei Xu,Xuanyu Zhang,Youmin Xu,Qing Huang,Shen Chen,Taiping Yao,Shouhong Ding,Jian Zhang*

Main category: cs.CV

TL;DR: A unified framework called GenShield is proposed for detecting and correcting AI-generated images with visible artifacts, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: The increasing realism of AI-generated images raises concerns about authenticity, and while detection methods have advanced, correcting detected images with artifacts remains underexplored.

Method: GenShield, a unified autoregressive framework, jointly performs explainable AIGI detection and controllable artifact correction in a closed loop.

Result: State-of-the-art performance and strong generalization on correction benchmark and mainstream AIGI detection benchmarks.

Conclusion: GenShield effectively addresses the gap between AIGI detection and artifact correction, revealing a mutually reinforcing relationship between these tasks.
...
TL;DR: The paper proposes ReAlign, a novel framework for efficient and generalizable image forgery detection, leveraging LLM-generated reasoning texts and contrastive learning.


<details>
  <summary>Details</summary>
Motivation: The rise of AI-generated images poses challenges for digital authenticity, and existing forgery detection methods have limitations in terms of efficiency, generalizability, and semantic understanding.

Method: The proposed ReAlign framework uses a GRPO-optimized LLM to generate high-quality reasoning texts, which are then distilled into a lightweight AIGI detector via contrastive learning.

Result: ReAlign consistently outperforms existing state-of-the-art detectors in both accuracy and generalization, particularly on complex, high-fidelity forgeries from modern generative models.

Conclusion: The paper concludes that ReAlign offers an efficient, generalizable, and accurate solution for image forgery detection, leveraging the strengths of LLM-generated reasoning texts and contrastive learning.
...
### [89] [Second-Order Multi-Level Variance Correction for Modality Competition in Multimodal Models](https://arxiv.org/abs/2605.16165)
*Yishun Lu,Wes Armour*

Main category: cs.CV

TL;DR: The authors propose a second-order optimization framework, ML-FOP-SOAP, with Multi-Level Variance Correction to improve the training stability and efficiency of multimodal foundation models.


<details>
  <summary>Details</summary>
Motivation: Autoregressive next-token training creates strong modality competition that destabilizes optimization and limits large-batch scaling.

Method: The authors propose a second-order optimization framework, ML-FOP-SOAP, with Multi-Level Variance Correction, Fisher-Orthogonal Projection, and a hierarchical folding strategy.

Result: Experiments show consistent gains across both modalities and stable training at batch size 8192, with improvements in sample efficiency by up to $1.4	imes$ and wall-clock training by up to $1.5	imes$ compared to AdamW.

Conclusion: The proposed method offers a robust optimizer for scaling multimodal foundation models, providing a more stable basis for multimodal alignment and improving training efficiency.
...
TL;DR: The paper proposes a framework that uses linguistic environmental cues to adaptively fuse camera and LiDAR inputs for multi-modal 3D semantic occupancy prediction, addressing the modality trust problem in adverse conditions.


<details>
  <summary>Details</summary>
Motivation: The effectiveness of multi-modal 3D semantic occupancy prediction is constrained by environmental variability, which affects camera and LiDAR sensors differently, creating a modality trust problem.

Method: The proposed framework leverages the pre-trained CLIP latent space and uses a parameter-efficient adapter to align weather-specific text embeddings with sensor features, coupled with a gating strategy that decomposes environmental uncertainty into visibility and illumination.

Result: The proposed framework achieves mIoU scores of 26.3 and 21.1 on the OccMamba and M-CONet architectures, respectively, significantly outperforming traditional baselines.

Conclusion: The proposed framework enables adaptive fusion of camera and LiDAR inputs, prioritizing semantic camera features in clear daylight and shifting to geometric LiDAR priors during rainy nights, demonstrating versatility and improved performance.
...
TL;DR: STABLE generates simulation-ready tabletop scenes from task instructions by combining semantic and physics-based modules.


<details>
  <summary>Details</summary>
Motivation: Existing methods for generating tabletop scenes rely on large language models (LLMs) which have limitations in 3D spatial reasoning, leading to object collisions or floating.

Method: STABLE uses a dual-system approach with a Semantic Reasoner (fine-tuned LLM) and a Physics Corrector (physics-aware flow-based denoising model) to generate and refine scene layouts.

Result: STABLE successfully generates simulation-ready scenes that conform to task instructions and enhances physical validity compared to prior methods.

Conclusion: STABLE presents a novel approach for simulation-ready tabletop scene generation, addressing the limitations of existing methods and demonstrating improved performance.
...
TL;DR: VLA-AD is a distillation framework that transfers large Vision-Language-Action policies into lightweight student policies using offline semantic supervision, achieving a 44x reduction in model size and a 3.28x inference speedup.


<details>
  <summary>Details</summary>
Motivation: The size and inference cost of billion-parameter Vision-Language-Action policies are major obstacles for real-time closed-loop control in robotic manipulation.

Method: VLA-AD uses a Vision-Language Model as an offline semantic supervisor to transfer large VLA teachers into lightweight student policies, providing high-level semantic guidance during training.

Result: The student policy achieves a 44x reduction in model size, a 3.28x inference speedup, and matches the teacher with only a 0.27% average relative gap on three LIBERO benchmark suites.

Conclusion: VLA-AD demonstrates that offline semantic guidance from VLMs can substantially improve the efficiency, robustness, and deployability of VLA policy distillation.
...
TL;DR: Register tokens improve convergence and generation quality of Diffusion Transformers (DiTs) in pixel-space training, producing cleaner feature maps at high noise levels.


<details>
  <summary>Details</summary>
Motivation: To investigate whether register tokens are useful for Diffusion Transformers (DiTs) as they are for Vision Transformers (ViTs), and to improve pixel-space generation quality.

Method: Analyzing intermediate representations of DiTs with and without register tokens, and investigating a parameter-efficient dual-stream architecture that specializes processing for register tokens.

Result: Register tokens significantly improve convergence and generation quality of pixel-space DiTs, and produce cleaner feature maps at high noise levels.

Conclusion: Register tokens are useful for Diffusion Transformers (DiTs) in pixel-space training, and a parameter-efficient dual-stream architecture can further improve generation quality with negligible runtime overhead.
...
### [94] [IVGT: Implicit Visual Geometry Transformer for Neural Scene Representation](https://arxiv.org/abs/2605.16258)
*Yuqi Wu,Tianyu Hu,Wenzhao Zheng,Yuanhui Huang,Haowen Sun,Jie Zhou,Jiwen Lu*

Main category: cs.CV

TL;DR: The authors propose IVGT, an Implicit Visual Geometry Transformer for reconstructing 3D geometry and appearance from unposed multi-view images.


<details>
  <summary>Details</summary>
Motivation: Existing visual geometry foundation models have limitations in predicting explicit geometry, such as redundancy and limited geometric continuity.

Method: IVGT uses a continuous neural scene representation in a canonical coordinate system, allowing for continuous spatial queries and prediction of signed distance values and colors.

Result: IVGT achieves strong performance on various tasks, including mesh and point cloud reconstruction, novel view synthesis, depth and surface normal estimation, and camera pose estimation.

Conclusion: IVGT demonstrates generalization across scenes and offers a promising approach for reconstructing coherent 3D geometry and appearance from unposed multi-view images.
...
TL;DR: The paper proposes Res$^2$CLIP, a novel framework for few-shot generalist anomaly detection that aligns multimodal features in a unified residual space, addressing challenges in adapting to fine-grained differences and domain shifts.


<details>
  <summary>Details</summary>
Motivation: Few-shot Generalist Anomaly Detection faces challenges in real-world scenarios with scarce samples and rapidly changing categories, and existing CLIP-based methods struggle with coarse-grained text prompts and domain shifts.

Method: The proposed Res$^2$CLIP framework symmetrically bridges visual and text modalities within CLIP's residual space, using a residual-to-residual alignment approach with three branches: text prompt-based, visual prompt-based, and residual-to-residual alignment.

Result: Experiments on multiple datasets demonstrate the effectiveness of the proposed architecture.

Conclusion: The proposed Res$^2$CLIP framework addresses the challenges in few-shot generalist anomaly detection by aligning multimodal features in a unified residual space, leading to improved performance.
...
TL;DR: A new approach called MAgSeg is proposed for segmenting smallholder agricultural landscapes from satellite imagery, addressing limitations of current multimodal large language models.


<details>
  <summary>Details</summary>
Motivation: The challenge of segmenting agricultural landscapes in the Global South due to fragmented plots, high intra-class variance, and scarcity of labeled data.

Method: MAgSeg, a decoder-free MLLM segmentation approach, which uses a novel instruction tuning data format for scalable fine-tuning and post-training on high-resolution satellite imagery.

Result: MAgSeg significantly outperforms state-of-the-art MLLM baselines in segmenting complex smallholder agricultural landscapes from high-resolution satellite imagery across three countries in the Global South.

Conclusion: MAgSeg offers a scalable solution for mapping smallholder agricultural environments, enabling standard MLLMs to perform segmentation without auxiliary vision decoders.
...
TL;DR: The paper introduces OP-Mix, a unified data mixing algorithm for language model training that operates across the entire training lifecycle, improving model quality and reducing compute requirements.


<details>
  <summary>Details</summary>
Motivation: Existing data mixing methods address only one phase of language model training at a time, and there is a need for a unified solution that can handle pretraining, continual learning, and adaptation.

Method: OP-Mix uses low-rank adapters trained directly on the current model to simulate candidate data mixtures, eliminating the need for separate proxy models and ensuring the search is grounded in the model's actual learning dynamics.

Result: OP-Mix consistently finds near-optimal mixtures across pretraining, continual midtraining, and continual instruction tuning, improving average perplexity by 6.3% in pretraining and matching the performance of retraining and on-policy distillation in continual learning while using significantly less compute.

Conclusion: OP-Mix suggests a new view of language model training as a single continuous process of learning from data, rather than a sequence of distinct phases, and provides a unified solution for data mixing across the entire training lifecycle.
...
TL;DR: The study examines the relationship between fluency and faithfulness in literary translation, finding a negative correlation between the two, particularly for human and Google Translate.


<details>
  <summary>Details</summary>
Motivation: To investigate whether fluency in literary translation corresponds to semantic preservation, and to examine the relationship between fluency and faithfulness.

Method: The study uses 130,486 translated paragraphs from 106 novels in 16 source languages, and measures fluency and faithfulness using a translationese classifier and the COMET-KIWI metric, respectively.

Result: A consistent negative correlation between fluency and faithfulness was found, with the pattern appearing for both human and Google Translate, but weaker and often non-significant for TranslateGemma.

Conclusion: The study suggests a tradeoff between fluency and faithfulness in literary translation, and highlights the importance of segment length in automatic evaluation.
...
TL;DR: The paper presents DiscoExplorer, a web interface for analyzing discourse relations across languages, making datasets from the DISRPT Shared Task publicly available.


<details>
  <summary>Details</summary>
Motivation: The study of discourse relations across languages is challenging due to the complexity of relevant data and lack of accessible interfaces.

Method: The authors developed DiscoExplorer, an open-source web interface, to facilitate the analysis of discourse relations across languages.

Result: The interface provides query language, search, and visualization facilities for relations and signaling devices, along with example studies.

Conclusion: DiscoExplorer enables easier study and comparison of discourse relations across languages, promoting research in Computational Linguistics and Pragmatics.
...
TL;DR: The study analyzes 100.7 million Ukrainian court decisions to construct a large-scale citation graph, revealing that judicial citation structure encodes legal domain boundaries and predicts future legislative importance with high accuracy.


<details>
  <summary>Details</summary>
Motivation: The authors aim to explore the structure of judicial citations in Ukrainian court decisions and its potential to predict legislative importance and automatically construct a legal ontology.

Method: The authors extract 502 million citation links from 99.5 million full texts using regex on commodity hardware, then apply network analysis and community detection methods (Louvain) to the citation graph.

Result: The study finds that the citation network follows a power law, recovers legal domain boundaries with high modularity and temporal stability, and predicts top-1000 articles with high accuracy (AUC = 0.9984).

Conclusion: The citation-derived ontology can be used to improve legal analysis and is operationalized as part of a workflow memory system for LLM-assisted legal analysis, with the extraction pipeline and analysis code released as open data.
...
TL;DR: The study examines human communication under limited vocabulary conditions, comparing human responses to theoretically motivated algorithms.


<details>
  <summary>Details</summary>
Motivation: To investigate how humans communicate under vocabulary limitations and compare their strategies to optimal algorithms.

Method: Using Sequential Monte Carlo inference with large language models to compare human responses to greedy and globally optimal sampling algorithms.

Result: Humans generally resemble greedy sampling, but more skilled humans exhibit non-greedy behavior, and both humans and algorithms rely on semantically light words in high-constraint settings.

Conclusion: The findings have implications for resource-rational cognition, psycholinguistics, L2 communication, and language impairments.
...
TL;DR: Adesua is a WhatsApp-based AI teaching assistant for science education in West Africa, providing accessible learning support and formative assessment.


<details>
  <summary>Details</summary>
Motivation: Sub-Saharan Africa faces high student-teacher ratios and shortages of qualified teachers, limiting students' access to personalized learning support and formative assessment.

Method: Adesua integrates curated textbooks and 33 years of national examination questions with generative AI to enable conversational question answering and automated assessment with feedback via a WhatsApp bot.

Result: A 6-month feasibility deployment showed a high perceived usefulness, with a helpfulness score of 93.75% for AI-generated answers.

Conclusion: The preliminary results provide a basis for more extensive future evaluation of a WhatsApp-based AI assistant to assess its potential to offer scalable, low-cost personalized learning support and formative assessment in resource-constrained educational contexts.
...
TL;DR: Developed Eskwai for Students, a generative AI assistant for law students in Ghana, leveraging a curated database of case laws and legislation to provide helpful answers.


<details>
  <summary>Details</summary>
Motivation: To address the limited work on generative AI for legal education in the Global South and explore its potential for law students.

Method: Developed a retrieval augmented generation (RAG) system, Eskwai for Students, and deployed it in a 30-month longitudinal study with 3.1K law students in Ghana.

Result: Eskwai for Students provided helpful answers to 32K queries from law students, but raised some ethical concerns, contributing to understanding responsible use of generative AI in legal education.

Conclusion: This work contributes to understanding how law students in the Global South use generative AI for studies and how it can be responsibly leveraged to advance legal education.
...
TL;DR: The paper introduces Capability Conditioned Scaffolding, a framework that adapts AI outputs to a user's evaluation capacity across different domains, to prevent over-reliance on AI in areas where users lack expertise.


<details>
  <summary>Details</summary>
Motivation: Large language models often personalize outputs to user preferences but fail to account for differences in user evaluation capacity across various domains, potentially leading to Professional Domain Drift.

Method: The authors propose a typed framework called Capability Conditioned Scaffolding, which categorizes user expertise into strong, mixed, and weak domains and adjusts AI intervention based on structured capability profiles.

Result: A pilot evaluation showed consistent profile-conditioned intervention behavior across multiple MMLU subsets and four LLM substrates, including correct responses to profile changes and cautious behavior in mixed-risk domains.

Conclusion: The findings suggest that capability-aware scaffolding can enhance reliable human-AI collaboration beyond simple personalization, potentially reducing Professional Domain Drift.
...
TL;DR: The paper analyzes neural activation patterns in six large language model architectures across twelve cognitive task categories, revealing differences in how encoder and decoder architectures process tasks.


<details>
  <summary>Details</summary>
Motivation: To understand the computational characteristics of modern language models and their task-specific neural behaviors.

Method: Systematic measurement of final activation values, attention entropy, and sparsity patterns across 144 task-model combinations.

Result: Mathematical reasoning produces the highest attention entropy across all architectures, and decoder models exhibit higher sparsity patterns than encoder models.

Conclusion: The findings provide critical insights into the computational characteristics of modern language models, with implications for model selection and optimization in big data applications.
...
TL;DR: The paper examines the discrepancy between language model (LM) surprisals and human sentence processing, particularly in syntactic ambiguity, and finds that limiting LMs' simultaneous parses doesn't fully reconcile LM-based surprisal with human processing.


<details>
  <summary>Details</summary>
Motivation: The mismatch between LM surprisals and human sentence processing difficulty, especially in controlled studies of syntactic ambiguity.

Method: Using Recurrent Neural Network Grammars (RNNGs) with word-synchronous beam search to systematically vary the number of simultaneous parses for computing word surprisal.

Result: Reducing the number of simultaneous active parses in LMs increases predicted garden path effects but doesn't fully capture human sentence processing difficulty.

Conclusion: Differences in the number of simultaneous parses between LMs and humans cannot fully explain the discrepancy between LM-based surprisal and human sentence processing.
...
TL;DR: The study examines the relationship between difficulty and internal reasoning trajectories in language models across various domains, finding that longer chains of thought don't necessarily indicate better reasoning, and that correcting for generation length reveals distinct trajectory geometries associated with reasoning training.


<details>
  <summary>Details</summary>
Motivation: To understand how reasoning-trained language models approach harder problems and whether longer chains of thought indicate better or different internal reasoning processes.

Method: The study analyzes hidden-state trajectories during chain-of-thought generation across competitive programming, mathematics, and Boolean satisfiability, and applies residualization to adjust for generation length.

Result: The results show that, after correcting for generation length, harder problems are associated with distinct trajectory geometries in reasoning-trained models, particularly in the code domain, and that this effect is weaker but still present in mathematics and Boolean satisfiability.

Conclusion: The study concludes that correcting for generation length is essential for analyzing internal reasoning trajectories and that reasoning training can be associated with distinct corrected trajectory geometries, with domain-dependent effects.
...
TL;DR: The paper proposes a modular retrieval-augmented generation (RAG) pipeline for extracting observations from conversational nurse-patient transcripts and normalizing them into a predefined schema, achieving an 80.36% F1 score.


<details>
  <summary>Details</summary>
Motivation: The substantial documentation burden on clinicians and the challenge of converting conversational transcripts into structured representations at scale.

Method: A modular RAG pipeline using schema-constrained prompting, deterministic schema-based postprocessing, and a second-pass audit with two LLM backbones: Llama-4-Scout-17B-16E-Instruct and GPT-5.2.

Result: The best configuration achieved an 80.36% F1 score, with RAG consistently improving performance and second-pass auditing yielding modest additional gains.

Conclusion: The proposed RAG pipeline shows promise for extracting and normalizing observations from conversational nurse-patient transcripts, potentially reducing documentation burden on clinicians.
...
TL;DR: The paper introduces FINESSE-Bench, a suite of benchmarks for evaluating the financial competencies of large language models (LLMs) in a hierarchical and comprehensive manner.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the incomplete evaluation of LLMs' domain competence in finance, specifically to assess their transition from foundational knowledge to expert-level financial reasoning.

Method: The authors present FINESSE-Bench, which consists of eight specialized benchmarks with 3,993 questions, combining exam-oriented datasets, trading task collections, and a Russian-language olympiad benchmark. They also propose a unified evaluation protocol and automated scoring scheme.

Result: The paper presents FINESSE-Bench as a tool for evaluating LLMs' financial competencies, enabling assessment of domain breadth, performance degradation, computational tasks, and model behavior in specialized financial domains.

Conclusion: The authors conclude that FINESSE-Bench is intended to complement existing open financial benchmarks and provide a more substantive evaluation of professionally relevant financial competencies in LLMs.
...
TL;DR: The paper identifies intrinsic limitations of Rotary Positional Embeddings (RoPE) in Transformer-based long-context language models, specifically that as context length increases, RoPE-based attention becomes unpredictable and loses locality bias and consistency in token relevance.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to analyze the effectiveness of Rotary Positional Embeddings (RoPE) in Transformer-based long-context language models and identify potential limitations.

Method: The paper uses theoretical analysis and empirical analysis to study the properties of RoPE-based attention, abstracting away from specific context content and depending only on context length.

Result: The results show that RoPE-based attention loses locality bias and consistency in token relevance as context length increases, and that adjusting the RoPE base or increasing the RoPE base hyperparameter can trade off distinguishing positions against distinguishing tokens but cannot preserve both.

Conclusion: The paper concludes that fundamentally new mechanisms for encoding position and token order may be needed in future Transformer long-context language models to overcome the limitations of RoPE.
...
TL;DR: The study introduces DetectRL-X, a multilingual benchmark to evaluate LLM-generated content detectors across 8 languages and 6 domains, revealing strengths and limitations of current detectors.


<details>
  <summary>Details</summary>
Motivation: The growing risk of misuse of LLM-generated content necessitates effective detection and governance, particularly in multilingual, real-world scenarios.

Method: The study creates a comprehensive benchmark, DetectRL-X, including human-written and LLM-generated texts in 8 languages, and evaluates detectors across 8 dimensions.

Result: Experimental results show the strengths and limitations of current state-of-the-art detectors when applied to diverse linguistic resources.

Conclusion: DetectRL-X is an effective benchmark for strengthening multilingual and language-specific detectors, highlighting the influence of domains, generators, and text characteristics on detector performance.
...
TL;DR: The paper proposes BetaPRM, a distributional process reward model that predicts step-level success probability and reliability, enabling better decision-making in reasoning tasks.


<details>
  <summary>Details</summary>
Motivation: Current process reward models (PRMs) output a single reward score for each step, treating imperfect predictions as reliable decision signals. The authors aim to address this limitation by introducing a reliability signal.

Method: The authors propose BetaPRM, which learns a Beta belief that explains the observed number of successful continuations through a Beta-Binomial likelihood. They also introduce Adaptive Computation Allocation (ACA) for PRM-guided Best-of-N reasoning.

Result: Experiments show that BetaPRM improves PRM-guided Best-of-N selection and preserves standard step-level error detection. ACA improves the accuracy-token tradeoff, reducing token usage by up to 33.57% while improving final-answer accuracy.

Conclusion: The proposed BetaPRM and ACA improve the reliability and efficiency of PRM-guided reasoning tasks, enabling better decision-making and resource allocation.
...
TL;DR: This paper studies a draft-conditioned latent refinement model for non-autoregressive text generation, focusing on the interface between continuous latent states and discrete tokens.


<details>
  <summary>Details</summary>
Motivation: The difficulty in interfacing continuous latent states with discrete tokens in continuous diffusion and flow models for non-autoregressive text generation.

Method: A draft-conditioned latent refinement model built from a frozen BERT encoder, a parallel decoder, a denoising DraftPrior, a local FlowNet, and a learned diagonal MetricNet.

Result: Latent geometry alone is not enough; decoder recoverability, start distribution quality, and refinement of decoder-readable structure are crucial for continuous latent text generation.

Conclusion: The study concludes that good latent-space metrics do not guarantee good decoding and that the task should be framed as controlled local refinement rather than full generation from noise.
...
TL;DR: The paper proposes Graph-Infused Layers Transformer Language Model (GiLT), which enhances Transformer language models by incorporating dependency graphs to improve syntactic generalization.


<details>
  <summary>Details</summary>
Motivation: Previous work on augmenting language models with linguistic structures has focused on constituency tree structures, but the authors aim to leverage dependency graphs for better syntactic generalization.

Method: GiLT modulates attention weights in the Transformer with features extracted from a dependency graph constructed incrementally during token prediction, without inserting extra structural tokens.

Result: GiLT achieves better syntactic generalization and competitive perplexity compared to Transformer baselines, and can be fine-tuned for improved downstream task performance.

Conclusion: The proposed GiLT model effectively enhances the syntactic generalization performance of language models by leveraging dependency graphs, and has potential for improved performance on downstream tasks.
...
TL;DR: The paper analyzes the dynamic range of activations in modern open large language models (LLMs), finding that activation magnitude varies greatly across families, generations, and training stages, and should be measured and reported before low-bit deployment.


<details>
  <summary>Details</summary>
Motivation: The dynamic range of activations is a constraint for low-bit quantization, activation scaling, and stable LLM inference, but prior work on pre-2024 LLaMA-style models may not reflect the current landscape of open LLMs.

Method: The authors measure global and layer-wise maxima on 27 checkpoints from 8 open families using a unified pipeline, including a 5,000-sample multi-domain corpus, family-specific tokenization, and identical hooks across various components.

Result: The study finds that (i) global maxima span nearly four orders of magnitude, (ii) cross-family and cross-generation comparisons do not follow simple monotonic scaling, and (iii) MoE checkpoints exhibit lower peaks than dense counterparts, with the residual stream often carrying the global maximum.

Conclusion: The maximum activation magnitude is a model property tied to family, architecture, and training stage, and should be measured and reported alongside any open-weight release before low-bit deployment.
...
TL;DR: A hybrid framework called Nexa is proposed to improve collaboration between Large Language Model agents by combining parallel and sequential communication modes.


<details>
  <summary>Details</summary>
Motivation: Existing collaboration frameworks for multi-agent systems are limited by operating in either parallel or sequential modes, which fail to balance communication, latency, and accuracy.

Method: Nexa uses a trainable response-conditioned policy to predict a sparse directed acyclic communication graph, allowing for flexible communication between agents.

Result: The proposed framework demonstrates generalizability across different settings, including the number of agents, tasks, and underlying agents.

Conclusion: Nexa provides a hybrid approach to improve collaboration between Large Language Model agents, achieving a balance between communication, latency, and accuracy.
...
TL;DR: The paper presents a benchmark to evaluate large language models (LLMs) on mental health entity recognition, relation judgment, and two-hop reasoning, revealing a gap in their ability to apply knowledge to clinical judgments.


<details>
  <summary>Details</summary>
Motivation: The increasing use of LLMs in mental health domain raises concerns about their ability to capture biomedical knowledge and apply it to clinical judgments.

Method: The authors propose a knowledge-graph grounded benchmark, MHGraphBench, derived from PrimeKG, to assess LLMs on mental health entity recognition, relation judgment, and two-hop reasoning.

Result: Experiments on 15 LLMs show a persistent recognition-to-judgment gap, with leading models performing well on entity typing but struggling with relation prediction and two-hop reasoning.

Conclusion: The benchmark highlights the limitations of LLMs in applying knowledge to clinical judgments and emphasizes the importance of response validity in benchmark-based evaluation.
...
TL;DR: Large language models struggle to transfer code generation abilities to unfamiliar languages, even with fine-tuning, revealing an 'implementation fidelity gap'.


<details>
  <summary>Details</summary>
Motivation: Understanding whether large language models can transfer code generation abilities to languages not present in their pretraining data.

Method: Evaluating frontier models (Qwen3) on a minimal imperative language (PyLang) absent from pretraining corpora, using zero-shot and fine-tuned approaches, and analyzing results with various interventions and CKA analysis.

Result: Fine-tuning improves syntax understanding but not semantic competence in PyLang, with Python outperforming PyLang by up to 19%, and interventions failing to close the gap.

Conclusion: The study highlights the need for training methods that separate reasoning from language-specific realization to address the 'implementation fidelity gap' in large language models.
...
TL;DR: A new method called Parallel Speculative Decoding (PSD) is proposed to improve the inference efficiency of diffusion large language models (dLLMs) without sacrificing generation quality.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the expensive inference cost of diffusion large language models (dLLMs) due to the large number of denoising iterations.

Method: The authors propose Parallel Speculative Decoding (PSD), a training-free framework that improves inference efficiency by unmasking multiple tokens per step and collapsing multiple denoising steps into one verification call.

Result: Experiments on three dLLMs across reasoning and code generation tasks show that PSD achieves favorable trade-offs between inference efficiency and generation quality, reaching up to $5.5	imes$ tokens per forward pass with accuracy comparable to greedy decoding.

Conclusion: The proposed method, Parallel Speculative Decoding (PSD), effectively reduces the inference cost of dLLMs while maintaining generation quality, making it a promising approach for efficient text generation.
...
TL;DR: LLMs are biased towards English and continual pre-training doesn't offer a cost advantage over training from scratch for adapting to other languages.


<details>
  <summary>Details</summary>
Motivation: The authors aim to investigate the bias of large language models (LLMs) towards English and the effectiveness of continual pre-training for adapting to other languages.

Method: The authors analyze sequences generated by open-weight LLMs and compare the cost of continual pre-training versus training from scratch for adapting to a target language.

Result: LLMs are heavily biased towards English and continual pre-training does not offer a cost advantage over training from scratch, even for improving cultural understanding in the target language.

Conclusion: Dedicated per-language investment may become increasingly important for future LLM development, rather than relying primarily on the expansion of English-centric resources.
...
TL;DR: The paper presents a new dataset, CHA-Gen, for Chinese ambiguity research, and evaluates Large Language Models (LLMs) on their ability to handle ambiguity, finding that they struggle with ambiguity detection and exhibit biases.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the lack of research on linguistic ambiguity in Chinese, and to provide a scalable approach for constructing a Chinese ambiguity dataset.

Method: The paper uses a semi-automatic pipeline guided by Potential Ambiguity (PA) Theory to construct the CHA-Gen dataset, and evaluates LLMs using direct querying and machine translation.

Result: The results show that LLMs struggle with ambiguity detection, and that CoT prompting can improve performance. The analysis also reveals three common failure modes: ambiguity blindness, misattribution, and premature resolution.

Conclusion: The paper concludes that the proposed approach provides a scalable way to construct a Chinese ambiguity corpus, and provides insights into LLMs' ambiguity handling, which can lay a foundation for enhancing Chinese ambiguity research in LLMs.
...
TL;DR: The authors propose a new language model called Dynamic Chunking Diffusion Model (DCDM) that replaces fixed-size positional blocks with content-defined semantic chunks, achieving better performance on downstream benchmarks.


<details>
  <summary>Details</summary>
Motivation: The authors argue that traditional block discrete diffusion language models waste sequence structure by partitioning tokens into fixed-size positional blocks, which can separate semantically coherent tokens and group unrelated ones together.

Method: The authors introduce Chunking Attention, a differentiable layer that routes tokens into clusters based on learnable subspaces, and use it to create a chunk-causal attention mask for a discrete diffusion denoiser.

Result: DCDM consistently improves over unstructured and positional-block diffusion baselines on downstream benchmarks at parameter scales up to 1.5B, with the advantage stable across scales and visible early in training.

Conclusion: The proposed DCDM model shows that replacing positional blocks with content-defined semantic chunks can improve language modeling performance.
...
TL;DR: The paper proposes a new paradigm called Diagram-as-Code for generating and editing diagrams using symbolic logic and mxGraph XML. It also introduces VCG-Bench, a benchmark for visual-centric mxGraph tasks, and highlights the limitations of current Vision-Language Models in handling structured diagrammatic tasks.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of current Vision-Language Models in handling structured, controllable diagrammatic tasks essential for professional workflows, which are currently reliant on pixel-based synthesis.

Method: The method proposed is the Diagram-as-Code paradigm, which leverages mxGraph Extensible Markup Language (XML) for precise diagram generation and editing.

Result: The experimental results show that current State-of-the-Art Vision-Language Models face challenges in structured fidelity and instruction compliance, reflecting their limitations in vision and reasoning capabilities.

Conclusion: The paper concludes that the proposed Diagram-as-Code paradigm and VCG-Bench benchmark can help address the limitations of current Vision-Language Models in handling structured diagrammatic tasks.
...
TL;DR: This paper studies using large language models (LLMs) for triaging online patient inquiries into four levels of urgency: self-care, schedule-visit, urgent-clinician-review, and emergency-referral.


<details>
  <summary>Details</summary>
Motivation: Online patient inquiries are often informal and incomplete, making it challenging to route them to the appropriate level of clinical follow-up. The goal is to explore if LLMs can support this routing process under low-resource labeling conditions.

Method: The study uses a public dataset (HealthCareMagic-100K) to construct a human-calibrated gold evaluation set, an auto-labeled silver training set, and a few-shot pool. It compares baselines (TF-IDF and BioBERT) trained on silver labels against six prompted LLMs under different shot conditions (0-shot, 4-shot, 12-shot).

Result: The strongest LLM (Claude Haiku 4.5, 12-shot) achieved a macro-F1 score of 0.475, outperforming the best supervised baseline (BioBERT, 0.378). However, the confidence intervals overlap. The study also found that few-shot prompting and two-model agreement help in label-dependent ways.

Conclusion: LLMs can support triage prioritization and selective human review but are not ready for autonomous deployment.
...
TL;DR: Proposed ASRU, a multimodal unlearning framework that improves effectiveness and generation quality.


<details>
  <summary>Details</summary>
Motivation: Existing methods overlook generation quality after unlearning, leading to hallucinated or rigid responses.

Method: ASRU uses activation redirection and a customized reward function for controllable unlearning.

Result: ASRU improves unlearning effectiveness by 24.6% and generation quality by 5.8x while preserving model utility.

Conclusion: ASRU offers a better trade-off between target knowledge unlearning and model utility.
...
TL;DR: H-Mem, a novel memory mechanism for Large Language Model (LLM)-based agents, effectively models memory evolution and retrieves data efficiently, achieving state-of-the-art performance on question-answering tasks.


<details>
  <summary>Details</summary>
Motivation: Current methods for utilizing memory in LLM-based agents lack a principled mechanism for modeling memory evolution and retrieving data effectively, leading to poor performance in memory utilization.

Method: H-Mem uses a hybrid structure with a temporal and semantic tree structure and a knowledge graph to model memory evolution and capture entity relationships, providing an efficient memory retrieval approach.

Result: H-Mem achieves state-of-the-art performance on the question-answering task across three agent memory benchmarks.

Conclusion: H-Mem offers a effective solution for modeling and retrieving memory data in LLM-based agents, leading to improved performance on question-answering tasks.
...
TL;DR: The paper proposes Neural Collaborative Context Engineering (NCCE), a framework that formulates context engineering as a recommendation problem, providing dynamic, instance-wise context optimization for Large Language Models (LLMs).


<details>
  <summary>Details</summary>
Motivation: Existing context engineering methods for LLMs treat the problem as a global search, seeking a single context strategy that maximizes average performance across a dataset, which overlooks the need for distinct guidance for different inputs.

Method: The proposed NCCE framework first creates a diverse catalog of anchor contexts, then uses a Context-CF Co-Evolution mechanism, which involves a lightweight Neural Collaborative Filtering (NCF) model that learns instance-context preferences to guide the generation of specialized context variants.

Result: Theoretical proofs and comprehensive experiments demonstrate that NCCE significantly improves task accuracy by matching individual inputs with their optimal contexts, highlighting the importance of personalization in LLM context engineering.

Conclusion: The paper concludes that the proposed NCCE framework provides a paradigm shift in context engineering by transitioning from static global search to dynamic, instance-wise routing, which is crucial for optimizing the performance of LLMs.
...
TL;DR: DimMem, a lightweight dimensional memory framework, is proposed to improve long-term memory in large language model agents, achieving high accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing memory systems face a trade-off between fidelity and efficiency, and the authors aim to address this limitation.

Method: The authors propose DimMem, a framework that represents memories as atomic, typed, and self-contained units with explicit fields, enabling dimension-aware retrieval and memory update.

Result: DimMem achieves 81.43% and 78.20% overall accuracy on LoCoMo-10 and LongMemEval-S, respectively, outperforming existing lightweight memory systems while reducing token cost by 24%.

Conclusion: The results suggest that explicit dimensional structuring is an effective and efficient foundation for long-term memory in LLM agents.
...
TL;DR: Smaller, open-source LLMs can effectively estimate translation quality, generating scores, error annotations, corrections, and post-editions, while preserving data privacy.


<details>
  <summary>Details</summary>
Motivation: The current reliance on massive, proprietary LLMs for Quality Estimation (QE) in machine translation raises data privacy concerns.

Method: Using smaller, open-source LLMs (<30B parameters) with a single-pass prompting strategy to generate quality scores, MQM error annotations, suggested error corrections, and full post-editions.

Result: The models achieve highly competitive system-level correlations with human judgments, outperforming traditional neural metrics, fine-tuned models, and human inter-annotator agreement.

Conclusion: Smaller, open-source LLMs are a viable, cost-effective, and privacy-preserving alternative to massive, proprietary LLMs for Quality Estimation in machine translation.
...
TL;DR: Introduced ForMaT, a dataset of 3,956 PDFs across 15 language pairs for multimodal machine translation, focusing on layout preservation and visual diversity.


<details>
  <summary>Details</summary>
Motivation: Current machine translation systems struggle with integrating visual and textual context, particularly in preserving layout metadata, which is crucial for high-fidelity document reconstruction.

Method: K-Medoids sampling over 45 geometric features was used to ensure structural diversity in the dataset, focusing on visually diverse PDF documents.

Result: The evaluation showed that current MT systems face challenges with spatial grounding and geometric synchronization, highlighting the need for layout-aware translation models.

Conclusion: ForMaT provides a benchmark for developing layout-aware translation models that integrate visual and textual context, enhancing the fidelity of document reconstruction.
...
TL;DR: The paper introduces a dataset of Russian government communications, including speeches, images, and metadata, to facilitate multimodal and multilingual analysis of authoritarian political communication.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the lack of available social text- and image-based data for authoritarian politics contexts, specifically in the Russian government.

Method: The authors collect and annotate a large corpus of official speeches from senior Kremlin and Russian Ministry of Foreign Affairs actors, including Russian- and English-language texts, images, captions, and metadata. They use transformer-based multimodal topic modeling and expert validation to generate topical annotations.

Result: The resulting dataset provides a valuable resource for multimodal, multilingual, temporal, and spatial analyses of authoritarian political communication, supporting social science research and large language model applications.

Conclusion: The paper presents a novel dataset that can facilitate a deeper understanding of authoritarian political communication, specifically in the context of the Russian government.
...
TL;DR: The paper proposes a method to improve block attention in long-context scenarios by introducing a semantic segmentation dataset and a training framework called block distillation.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve KV cache reuse in long-context scenarios such as Retrieval-Augmented Generation (RAG) by addressing the challenges of segmenting input text into meaningful blocks and the inefficiency of existing block fine-tuning methods.

Method: The method consists of two main components: (1) a lightweight segmenter trained on a large semantic segmentation dataset called SemanticSeg to automatically partition text into human-instinct-aligned blocks, and (2) a training framework called block distillation that uses a frozen full-attention teacher model to guide the block-attention student.

Result: The results show that the proposed segmenter outperforms heuristic and statistical baselines, and block distillation achieves near-full-attention performance under block attention.

Conclusion: The paper concludes that the proposed method establishes a practical and scalable pathway for deploying block attention in long-context scenarios.
...
TL;DR: The paper applies Group Relative Policy Optimization to fine-tune encoder-decoder models for machine translation, achieving consistent improvements across 13 languages.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve machine translation (MT) models, specifically encoder-decoder Seq2Seq models, using reinforcement learning approaches, as current methods largely focus on decoder-only LLMs with limited study on encoder-decoder architectures.

Method: The method used is Group Relative Policy Optimization (GRPO) with a hybrid reference-free reward (LaBSE and COMET-Kiwi) that doesn't require parallel data at fine-tuning time.

Result: The results show consistent improvements across all 13 languages, with up to +5.03 chrF++ for Traditional Chinese, and competitive performance with 3-epoch supervised fine-tuning on morphologically complex languages without using target-language data.

Conclusion: The conclusion is that GRPO is effective in improving MT models, particularly where parallel data is scarce, and that gains are largest where baseline performance is weakest and reward discriminability is highest.
...
TL;DR: A framework using symbolic methods is proposed to extract incident facts from unstructured text in law enforcement reports, achieving high accuracy in extracting events and mapping them to a semantic path.


<details>
  <summary>Details</summary>
Motivation: The need to automate the extraction of incident facts from natural language in law enforcement reports to facilitate review, police training, and investigations.

Method: The proposed framework uses symbolic methods, including redacting personal identifiers, semantic parsing, predicate mapping to ontology, and reasoning to extract evidence-linked facts from narratives.

Result: The framework achieved 54.1% of extracted events with a confidence score of at least 0.80 and 93.7% mapped through the PropBank--VerbNet--WordNet semantic path, with 100% agreement on incident initiation, stolen items, and temporal cues.

Conclusion: The proposed framework demonstrates the value of narratives in recovering incident details and building temporal graphs, with potential applications in police training, investigations, and review.
...
TL;DR: The paper proposes a taxonomy to evaluate AI systems' cultural capabilities, addressing the ambiguity in current literature.


<details>
  <summary>Details</summary>
Motivation: Current AI system evaluations lack clarity on cultural capabilities, leading to potential misinterpretation and misdeployment in multicultural settings.

Method: The authors draw from Intercultural Communication scholarship to propose a three-level taxonomy: Cultural Awareness, Cultural Sensitivity, and Cultural Competence.

Result: The proposed taxonomy provides a practical tool for improving AI evaluation validity and interpretability in real-world, multicultural settings.

Conclusion: The taxonomy helps ensure that AI systems are evaluated accurately, reducing the risk of overstating model capabilities and misdeployment in culturally sensitive contexts.
...
TL;DR: The paper proposes a rubric to evaluate the adaptivity of Vision Language Models (VLMs) in mathematics education, finding that current VLMs struggle to adapt to different learner profiles and produce consistent instructional responses.


<details>
  <summary>Details</summary>
Motivation: The increasing recognition of adaptive learning as critical for effective learning support tools and the lack of a systematic evaluation framework for VLMs' adaptivity to different learner profiles in mathematics tutoring tasks.

Method: The proposal of a learner model-based rubric that formalizes adaptivity assessment into cognitive, motivational, and complexity aspects, and the evaluation of VLM responses based on correctness, quality, and adaptivity.

Result: The experimental results show measurable differences in adaptivity across models and reveal that current VLMs struggle to consistently produce learner model-based instructional responses, especially with limited learner information.

Conclusion: The study highlights the need for improved adaptivity in VLMs for mathematics education and proposes a rubric for evaluating their adaptivity to different learner profiles.
...
TL;DR: The study investigates format-induced inconsistencies in LLM-as-a-judge paradigm, finding that a shared sub-graph in MLPs computes format-independent preferences, while format-specific terminal branches cause inconsistencies.


<details>
  <summary>Details</summary>
Motivation: Existing diagnoses of format-induced inconsistencies in LLM-as-a-judge paradigm only examine input-output level, but not internal mechanisms.

Method: Using Position-aware Edge Attribution Patching (PEAP) to causally investigate internal mechanisms in Gemma-3, Qwen2.5, and Llama-3.

Result: A sparse, generalized Latent Evaluator sub-graph in mid-to-late MLPs is responsible for judgments across tasks, and zero-ablating it collapses judgment while preserving world knowledge.

Conclusion: The study provides a mechanistic account of format-induced inconsistency, implying that benchmark-level reliability comparisons across formats may be measuring formatter geometry rather than evaluation quality.
...
TL;DR: The paper proposes S2ST-Omni 2, a compositional speech-to-speech translation framework that uses structured typological priors to improve multilingual adaptation and achieve better performance.


<details>
  <summary>Details</summary>
Motivation: Existing S2ST systems neglect or oversimplify source-language information, which limits their ability to adapt to scarce supervised data. The authors aim to leverage systematic linguistic structure shared across languages to improve data-efficient multilingual S2ST.

Method: The authors propose a three-level approach: typology-informed hierarchical language encoding, dynamically-gated language-aware Dual-CTC, and typology-aware LLM prompting. They evaluate their approach on CVSS-C and compare it to representative S2ST approaches.

Result: S2ST-Omni 2 achieves superior average performance across BLEU, COMET, ASR-BLEU, and BLASER 2.0. Ablation studies show that the proposed strategies provide complementary benefits. The approach also shows promising results in data-budget analyses and a Japanese-to-English evaluation with limited supervised data.

Conclusion: The proposed S2ST-Omni 2 framework effectively leverages structured typological priors to improve multilingual adaptation and achieve state-of-the-art performance in compositional speech-to-speech translation.
...
TL;DR: Proposed RecMem, a memory system that reduces token consumption by consolidating memory only when semantically similar interactions recur, achieving up to 87% cost reduction while exceeding accuracy of existing systems.


<details>
  <summary>Details</summary>
Motivation: Existing memory systems for LLMs are costly due to eager memory consolidation, leading to substantial token consumption.

Method: RecMem uses a subconscious memory layer with lightweight embedding models and only invokes LLMs for episodic and semantic memory extraction when sustained recurrence of semantically similar interactions is observed.

Result: RecMem reduces memory construction token cost by up to 87% while exceeding accuracy of three SOTA memory systems.

Conclusion: RecMem effectively tackles the problem of high token consumption in existing memory systems while maintaining or improving accuracy.
...
TL;DR: The paper proposes a data augmentation framework using large language models to improve the prediction of cognitive scores from speech, addressing class imbalance and limited dataset size.


<details>
  <summary>Details</summary>
Motivation: Accurate assessment of cognitive decline from spontaneous speech is challenging due to limited dataset size and class imbalance.

Method: The framework uses GPT-5 to generate oral-like monologues from written responses, and then predicts Hasegawa Dementia Scale scores using a Partial Least Squares regression model trained on Sentence-BERT speech embeddings.

Result: The similarity-guided class-balanced selection augmentation strategy leads to more consistent improvements, reducing prediction error for minority low-score participants while maintaining performance for the majority group.

Conclusion: The findings demonstrate the potential of semantically guided LLM-driven augmentation for addressing class imbalance and improving data efficiency in clinical speech analysis.
...
TL;DR: The paper proposes a multi-level contextual token relation modeling framework to improve the detection of machine-generated texts (MGTs) by addressing the bias in token-level detection scores.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the risks posed by machine-generated texts (MGTs), such as disinformation and phishing, by developing a reliable detection method. They focus on improving metric-based methods, which are practical but face challenges due to the inherent randomness of MGT generation.

Method: The authors first unify representative metric-based methods and identify a core challenge: token-level detection scores are easily biased. They then derive multi-hop transitions of token-level detection scores and explore local and global relations. Based on these findings, they propose a multi-level contextual token relation modeling framework, which includes a Markov-informed calibration module for local relations and a rule-support reasoning module for global relations.

Result: The proposed framework achieves broad and substantial improvements across various real-world scenarios, including cross-LLM and cross-domain settings, with low computational overhead.

Conclusion: The paper concludes that the proposed multi-level contextual token relation modeling framework effectively addresses the challenges in MGT detection by leveraging local and global relations among token-level detection scores, leading to improved performance and practicality.
...
TL;DR: The paper proposes DebiasRAG, a tuning-free framework to mitigate social biases in large language models (LLMs) while preserving their capabilities.


<details>
  <summary>Details</summary>
Motivation: LLMs can produce biased content due to their dependence on training corpora, and existing debiasing methods require additional resources or domain knowledge.

Method: DebiasRAG uses a retrieval-augmented generation approach with three stages: debiasing candidate generation, context pool construction, and debiasing-guided context reranking.

Result: DebiasRAG improves fairness in LLMs without degrading their original capabilities.

Conclusion: The proposed framework offers a dynamic and query-specific debiasing approach for LLMs, enhancing their fairness and reliability.
...
TL;DR: SGR, a stepwise framework, enhances Large Language Models (LLMs) reasoning by generating query-specific subgraphs from external knowledge bases to support multi-step inference.


<details>
  <summary>Details</summary>
Motivation: LLMs are limited in complex settings requiring deep reasoning and logical inference, and may introduce irrelevant or factually inconsistent content.

Method: SGR constructs a subgraph tailored to the input question, guides the model to reason progressively over the generated structure, and combines multiple reasoning trajectories.

Result: SGR achieves consistent improvements over competitive baselines across several benchmark datasets, improving both reasoning accuracy and factual reliability.

Conclusion: The proposed SGR framework effectively enhances LLM reasoning through external subgraph generation, leading to improved performance in complex settings.
...
TL;DR: AI coding systems generate novel scientific hypotheses for 3D photovoltaic structures, overcoming limitations of flat solar panels.


<details>
  <summary>Details</summary>
Motivation: To overcome energy losses in flat solar panels at mid-latitudes by optimizing 3D photovoltaic (3DPV) structures.

Method: Combining Google's AntiGravity coding agent with an LLM-driven tree search algorithm (ERA) to autonomously generate high-efficiency 3DPV structures.

Result: The approach discovers a series of designs with improved performance, including optimal designs with different fixed collector areas, optimizing zenith tracking, and avoiding self-shadowing.

Conclusion: The combination of coding agents with tree search (ERA) provides a powerful platform for scientific discovery, particularly for problems with empirically evaluable solutions.
...
TL;DR: The authors propose a value-based method to improve large language models' (LLMs) ability to simulate human opinions across cultures, achieving better population-level predictions and more diverse responses.


<details>
  <summary>Details</summary>
Motivation: Existing methods for simulating human opinions using LLMs have limitations in reproducing population responses across cultures, mainly because they rely on indirect proxies like sociodemographic traits rather than core cultural values.

Method: The authors propose a value-based persona construction method that uses survey responses to capture cultural dimensions and derive textual descriptors. They also introduce a calibration procedure to improve response diversity.

Result: The proposed approach reduces prediction error across countries, especially in underrepresented populations, and yields response distributions that closely match human diversity.

Conclusion: The authors' value-based approach improves LLMs' ability to simulate human opinions across cultures, narrowing the performance gap between well-represented and underrepresented populations.
...
TL;DR: Argus, a new agentic system, improves deep research by treating it as assembling a jigsaw from complementary evidence pieces, achieving state-of-the-art results with a Searcher and Navigator cooperation.


<details>
  <summary>Details</summary>
Motivation: Current deep research agents duplicate efforts and have limited context, leading to diminishing returns.

Method: Argus uses a Searcher to collect evidence and a Navigator to maintain a shared evidence graph, dispatching Searchers and synthesizing the final answer.

Result: Argus gains 5.5 points with a single Searcher and 12.7 points with 8 parallel Searchers, reaching 86.2 on BrowseComp with 64 Searchers.

Conclusion: Argus achieves state-of-the-art results by cooperatively searching and navigating evidence, scaling to multiple Searchers without retraining.
...
TL;DR: The study uses an aphasia-inspired technique to analyze language models by 'lesioning' model parameters and comparing the effects to clinical aphasia symptoms.


<details>
  <summary>Details</summary>
Motivation: To understand the functional organization of language models by drawing causal links between affected model components and specific symptom profiles.

Method: Lesioning model parameters and measuring the effects against clinical aphasia symptoms using the Text Aphasia Battery (TAB).

Result: The study finds broad symptom-profile differences between attention and feed-forward components, and an effect of depth on symptom profiles. However, qualitative differences in symptom patterns between language models and humans suggest that aphasia syndromes are influenced by learning and processing details.

Conclusion: The study provides insights into the functional organization of language models, but highlights that aphasia syndromes are not a domain-invariant consequence of disrupted language processing.
...
TL;DR: The integration of smart metering, AI, and quantum-inspired optimisation is transforming energy utilities' management of infrastructure, customer engagement, and environmental accountability.


<details>
  <summary>Details</summary>
Motivation: The convergence of smart metering, generative artificial intelligence, and quantum-inspired combinatorial optimisation is driving change in energy utilities.

Method: Not explicitly stated, but involves the integration of smart metering, AI, and quantum-inspired optimisation.

Result: Reshaping of energy utilities' management of physical infrastructure, customer engagement, and environmental accountability.

Conclusion: The integration of these technologies is transforming the energy utility sector.
...
TL;DR: The paper proposes a framework for distribution utilities to improve billing, carbon tracking, and load scheduling, using AI and forecasting techniques.


<details>
  <summary>Details</summary>
Motivation: The increasing expectation for distribution utilities to provide readable bills, attach carbon numbers to energy sales, and manage load against grid stress and emissions constraints.

Method: An end-to-end framework integrating a generative-AI agent for billing statement drafting and a transformer-based forecaster for day-ahead consumption estimation.

Result: The proposed framework unifies four production-grade capabilities to address the challenges faced by distribution utilities.

Conclusion: The framework aims to help distribution utilities meet new expectations around billing transparency, carbon tracking, and load management.
...
### [150] [DeepSlide: From Artifacts to Presentation Delivery](https://arxiv.org/abs/2605.15202)
*Ming Yang,Zhiwei Zhang,Jiahang Li,Haoseng Liu,Yuzheng Cai,Weiguo Zheng*

Main category: cs.AI

TL;DR: DeepSlide is a human-in-the-loop system that generates presentations, focusing on both the visual slides and the delivery process, improving narrative flow, pacing, and attention guidance.


<details>
  <summary>Details</summary>
Motivation: Current AI slide generators focus on creating visually plausible slides but neglect the delivery process, including pacing, narrative, and presentation preparation.

Method: DeepSlide uses a multi-agent system with a controllable planner, content-tree retriever, sequential rendering, and sandboxed execution to generate presentations and support rehearsal.

Result: DeepSlide matches strong baselines on artifact quality and improves delivery metrics, including narrative flow, pacing precision, and slide-script synergy across 20 domains and diverse audience profiles.

Conclusion: DeepSlide offers a comprehensive approach to presentation generation, enhancing both the visual artifact and the delivery process, leading to improved presentation quality and effectiveness.
...
### [151] [Does Theory of Mind Improvement Really Benefit Human-AI Interactions? Empirical Findings from Interactive Evaluations](https://arxiv.org/abs/2605.15205)
*Nanxu Gong,Zixin Chen,Haotian Li,Zishu Zhao,Jianxun Lian,Huamin Qu,Yanjie Fu,Xing Xie*

Main category: cs.AI

TL;DR: The study proposes a new paradigm for evaluating Theory of Mind (ToM) in Large Language Models (LLMs) through interactive assessments, and finds that improvements on static benchmarks don't always translate to better performance in dynamic human-AI interactions.


<details>
  <summary>Details</summary>
Motivation: Improving Theory of Mind (ToM) capability in LLMs for effective social interactions with humans, and addressing the limitations of existing benchmarks that ignore the dynamic and open-ended nature of human-AI interactions.

Method: The study proposes a new paradigm of interactive ToM evaluation, and conducts a systematic study of four representative ToM enhancement techniques using real-world datasets and a user study.

Result: The study finds that improvements on static benchmarks do not always translate to better performance in dynamic human-AI interactions, highlighting the necessity of interaction-based assessments.

Conclusion: The study offers critical insights into ToM evaluation, emphasizing the need for interaction-based assessments in developing next-generation, socially aware LLMs for human-AI symbiosis.
...
### [152] [SkillSmith: Compiling Agent Skills into Boundary-Guided Runtime Interfaces](https://arxiv.org/abs/2605.15215)
*Duling Xu,Zheng Chen,Zaifeng Pan,Jiawei Guan,Dong Dong,Jialin Li,Bangzheng Pu*

Main category: cs.AI

TL;DR: The paper proposes SkillSmith, a framework that optimizes the use of skills in large language model-based agent systems by compiling skill packages offline and dynamically accessing only relevant components at runtime, reducing redundancy and improving efficiency.


<details>
  <summary>Details</summary>
Motivation: The existing execution paradigm of injecting skills into the agent reasoning loop introduces redundancy in the form of irrelevant context injection and repeated skill-specific reasoning and planning.

Method: The authors propose a boundary-first compiler-runtime framework called SkillSmith, which compiles skill packages offline into minimal executable interfaces and enables agents to dynamically access and execute only the relevant components at runtime.

Result: The evaluation on the SkillsBench benchmark shows that SkillSmith reduces solve-stage token usage by 57.44%, thinking iterations by 42.99%, solve time by 50.57% (2.02x faster), and token-proportional monetary cost by 57.44% compared to using raw skills.

Conclusion: The proposed framework, SkillSmith, effectively minimizes unnecessary context injection and redundant reasoning overhead, leading to improved efficiency and accuracy in large language model-based agent systems.
...
### [153] [Fair outputs, Biased Internals: Causal Potency and Asymmetry of Latent Bias in LLMs for High-Stakes Decisions](https://arxiv.org/abs/2605.15217)
*Jagdish Tripathy,Marcus Buckmann*

Main category: cs.AI

TL;DR: Instruction-tuned language models show fairness in high-stakes decisions but retain biased internal representations. These suppressed representations can affect model outputs and are asymmetrically causal across demographic groups.


<details>
  <summary>Details</summary>
Motivation: To investigate whether internal biased representations in instruction-tuned language models can affect model outputs in high-stakes decisions, such as mortgage underwriting, and whether this causal potency is symmetric across demographic groups.

Method: Using open-weight models for mortgage underwriting with matched applications differing only in racially-associated names, and employing activation steering and novel cross-layer interventions to demonstrate the decision-relevance of suppressed information.

Result: Models show no output-level bias but retain and amplify demographic representations across model layers. The suppressed information is decision-relevant and produces near-complete decision reversals when reinjected at critical layers. The latent bias is asymmetric and susceptible to adversarial prompt engineering and parameter-efficient fine-tuning.

Conclusion: Behavioural audits focused on outputs are insufficient, as fair outputs can mask exploitable internal biases. Dual-layer testing frameworks combining output evaluation with representational analysis are motivated for AI governance in high-stakes decisions.
...
### [154] [CAX-Agent: A Lightweight Agent Harness for Reliable APDL Automation](https://arxiv.org/abs/2605.15218)
*Chenying Lin,Yichen Hai,Yi He,Ran Wang,Haiyan Qiang,Liang Yu*

Main category: cs.AI

TL;DR: The paper presents CAX-Agent, an agent harness for MAPDL automation that improves reliability in large language models by introducing a recovery policy with a three-layer architecture and a recovery ladder. The evaluation shows that the model-only recovery strategy outperforms rule-only and no recovery strategies.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the practical reliability challenges faced by large language models deployed for MAPDL finite-element simulation, such as inconsistent outputs and task failures.

Method: The paper proposes the CAX-Agent architecture, which organizes execution into three layers (LLM service, agent harness, and solver backend) with a recovery ladder. The authors evaluate three recovery strategies (no_recovery, rule_only, and model_only) on 50 standard structural benchmarks with three repeated runs per strategy.

Result: The results show that the model-only recovery strategy achieves the best completion rate (0.9267), task score (3.59/4), total score (9.16/10), and zero-intervention rate (0.84), outperforming rule_only and no_recovery strategies with large effect sizes (Cliff's delta = 0.81-0.87).

Conclusion: The conclusion of this paper is that the proposed CAX-Agent architecture with a model-only recovery strategy can improve the reliability of large language models deployed for MAPDL finite-element simulation.
...
TL;DR: The paper introduces the NOVA framework to study AI systems' ability to discover new knowledge through self-improvement, analyzing the process and its limitations.


<details>
  <summary>Details</summary>
Motivation: To investigate whether AI systems can discover genuinely new knowledge through iterative self-improvement and at what cost.

Method: The NOVA framework models the 'generate, verify, accumulate, retrain' loop as an adaptive sampling process over a knowledge space, identifying conditions for genuine knowledge accumulation and failure modes.

Result: The paper identifies sufficient conditions for covering a finite domain with genuine knowledge, analyzes the impact of imperfect verification, and derives a scaling law for the cumulative generation cost required for distinct genuine discoveries.

Conclusion: The study formalizes the limitations and costs associated with AI systems discovering new knowledge through self-improvement, highlighting the value of expert input near autonomous exploration barriers.
...
### [156] [ICRL: Learning to Internalize Self-Critique with Reinforcement Learning](https://arxiv.org/abs/2605.15224)
*Jianbo Lin,Xiaomin Yu,Yi Xin,Yifu Guo,Zhuosong Jiang,Zhongqi Yue,Weishi Wang,Heqing Zou,Chengwei Qin,Hui Xiong*

Main category: cs.AI

TL;DR: A novel framework called ICRL is proposed to enable large language models to internalize self-critique and improve their performance without external feedback.


<details>
  <summary>Details</summary>
Motivation: Large language models make mistakes and may not internalize critique guidance, while a frozen critic cannot improve its feedback quality over time, limiting iterative self-improvement.

Method: The ICRL framework jointly trains a solver and a critic from a shared backbone using reinforcement learning, with a distribution-calibration re-weighting ratio and role-wise group advantage estimation.

Result: ICRL achieves consistent improvements over GRPO on diverse benchmarks, with average gains of 6.4 points on agentic tasks and 7.0 points on mathematical reasoning tasks.

Conclusion: ICRL enables large language models to learn from self-critique and improve their performance without external feedback, achieving comparable results to larger critics with fewer tokens.
...
### [157] [NIMO Controller: a self-driving laboratory orchestrator based on the Model Context Protocol](https://arxiv.org/abs/2605.15227)
*Naruki Yoshikawa,Ryo Tamura*

Main category: cs.AI

TL;DR: Proposed a self-driving laboratory (SDL) software architecture based on Model Context Protocol (MCP) for unified access by human users and AI agents.


<details>
  <summary>Details</summary>
Motivation: Existing SDL frameworks are not suitable for AI agents and developing SDL software is technically demanding.

Method: Designed an MCP-based SDL orchestrator, NIMO Controller, with a visual programming interface and unified MCP backend for humans and AI agents.

Result: Validated the usability of the proposed MCP-based SDL architecture through a color-matching SDL case study.

Conclusion: The proposed SDL software architecture improves accessibility and provides a unified interface for human users and AI agents.
...
### [158] [Verifiable Agentic Infrastructure: Proof-Derived Authorization for Sovereign AI Systems](https://arxiv.org/abs/2605.15228)
*Jun He,Deying Yu*

Main category: cs.AI

TL;DR: The paper introduces a Distributed Trust Framework (DTF) to address the authorization challenges posed by autonomous AI agents in cloud and enterprise systems.


<details>
  <summary>Details</summary>
Motivation: The emergence of autonomous AI agents invalidates the assumption that callers with valid credentials are safe to execute commands, posing significant operational risks in sovereign AI systems.

Method: The authors propose a verification framework, DTF, which computes execution authority from structured, verifiable artifacts and includes a Justification Proof, consensus model, Execution Identity, and Evidence Chain.

Result: The DTF architecture enforces a compact authorization invariant, ensuring that high-stakes executions require a proof object, derived authority requires consensus, and valid mutations are tied to evidence.

Conclusion: The DTF provides an infrastructure foundation for making agentic execution governable, auditable, and bounded in sovereign AI deployments, shifting authorization from standing identity to proof-derived authority.
...
### [159] [Solvita: Enhancing Large Language Models for Competitive Programming via Agentic Evolution](https://arxiv.org/abs/2605.15301)
...
TL;DR: Solvita, an agentic evolution framework, enables continuous learning for large language models (LLMs) in competitive programming without requiring weight updates. It outperforms existing multi-agent pipelines and single-pass baselines.


<details>
  <summary>Details</summary>
Motivation: Large language models (LLMs) struggle with rigorous reasoning demands in competitive programming. Recent multi-agent frameworks are stateless, relying on static retrieval and discarding valuable problem-solving experience.

Method: Solvita reorganizes problem-solving into a closed-loop system with four specialized agents (Planner, Solver, Oracle, and Hacker) paired with trainable, graph-structured knowledge networks. The system uses reinforcement learning updates to accumulate transferable reasoning experience.

Result: Solvita establishes a new state-of-the-art among code-generation agents, outperforming existing multi-agent pipelines and nearly doubling the accuracy of single-pass baselines across various benchmarks.

Conclusion: Solvita effectively addresses the limitations of existing multi-agent frameworks in competitive programming by enabling continuous learning and accumulating transferable reasoning experience.
...
### [160] [Context Pruning for Coding Agents via Multi-Rubric Latent Reasoning](https://arxiv.org/abs/2605.15315)
*Jingjing Wang,Xiwen Chen,Wenhui Zhu,Huayu Li,Zhengxiao He,Feiyang Cai,Ana S. Carreon-Rascon,Xuanzhao Dong,Feng Luo*

Main category: cs.AI

TL;DR: LaMR, a structured pruning framework, improves LLM-powered coding agents by effectively filtering irrelevant code, outperforming full-context baselines and saving tokens.


<details>
  <summary>Details</summary>
Motivation: Existing learned pruners have a modeling bottleneck due to a single-objective sequence labeler, which collapses all facets of code relevance into one score.

Method: LaMR decomposes code relevance into two quality dimensions (semantic evidence and dependency support) with dedicated CRFs and a mixture-of-experts gating network.

Result: LaMR outperforms unpruned full-context baselines, saving up to 31% tokens and improving Exact Match by up to +3.5 on benchmarks like SWE-Bench Verified and LongCodeQA.

Conclusion: LaMR provides an effective solution for pruning irrelevant code in LLM-powered coding agents, leading to improved performance and efficiency.
...
### [161] [Zero-Shot Goal Recognition with Large Language Models](https://arxiv.org/abs/2605.15333)
*Kin Max Piamolini Gusmão,Nathan Gavenski,Nir Oren,Felipe Meneguzzi*

Main category: cs.AI

TL;DR: This paper evaluates the ability of large language models (LLMs) to perform goal recognition in classical planning domains, finding uneven competence among models.


<details>
  <summary>Details</summary>
Motivation: The motivation is to assess LLMs' ability in goal recognition, a task that leverages world knowledge rather than generating action sequences.

Method: The paper conducts a systematic zero-shot evaluation of frontier LLMs on key PDDL benchmarks for goal recognition.

Result: The results show that LLM competence on goal recognition varies; some models improve with evidence, approaching landmark-based accuracy, while others rely heavily on world-knowledge priors.

Conclusion: The study concludes that goal recognition can serve as a benchmark for evaluating the foundational planning knowledge of LLMs, highlighting differences in evidence integration among models.
...
### [162] [Belief Engine: Configurable and Inspectable Stance Dynamics in Multi-Agent LLM Deliberation](https://arxiv.org/abs/2605.15343)
*Joshua C. Yang,Maurice Flechtner,Damian Dailisan,Michiel A. Bakker*

Main category: cs.AI

TL;DR: The authors introduce the Belief Engine (BE), a system that helps track and understand how LLM-based agents change their stance during deliberative interactions, such as negotiations or debates, by updating their beliefs based on evidence.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the issue of LLM-based agents' stance changes during interactions not being transparent, making it difficult to understand why an agent changed its stance.

Method: The authors propose the Belief Engine (BE), which treats 'belief' as an evidential state and updates it using a log-odds rule controlled by evidence uptake and prior anchoring. They test BE across multiple base LLMs and parameter sweeps.

Result: The results show that BE can reliably shape stance dynamics while preserving an evidence-level update trail. BE best reconstructs participants whose final stance follows extracted evidence, and provides configurable infrastructure for studying evidence-grounded deliberation.

Conclusion: The authors conclude that BE provides a useful tool for understanding and studying deliberative interactions by making the belief update process auditable and transparent, which can help tie openness, commitment, convergence, and disagreement to explicit update assumptions.
...
### [163] [Ensemble Monitoring for AI Control: Diverse Signals Outweigh More Compute](https://arxiv.org/abs/2605.15377)
*Eugene Koran,Yejun Yun,Samantha Tetef,Benjamin Arnav,Pablo Bernabeu-Pérez*

Main category: cs.AI

TL;DR: Combining diverse AI monitors into an ensemble improves detection of misaligned actions, outperforming individual monitors and homogeneous ensembles.


<details>
  <summary>Details</summary>
Motivation: Ensuring AI system actions are safe and aligned with user intent is crucial, particularly in autonomous settings. However, building reliable monitors and scaling human oversight is impractical.

Method: The authors built 12 GPT-4.1-Mini monitors using prompting and fine-tuning strategies, evaluating them on coding tasks with adversarial inputs. They created diverse ensembles and compared their performance.

Result: The best 3-monitor ensemble achieved 2.4x greater detection performance gain compared to an ensemble of three identical monitors. Fine-tuned monitors performed well, especially on out-of-distribution attack types.

Conclusion: Ensemble monitoring with diverse AI monitors is a practical strategy for safety gains at reasonable inference costs, driven by diversity rather than scale.
...
### [164] [From LLM-Generated Conjectures to Lean Formalizations: Automated Polynomial Inequality Proving via Sum-of-Squares Certificates](https://arxiv.org/abs/2605.15445)
*Ruobing Zuo,Hanrui Zhao,Gaolei He,Zhengfeng Yang,Jianlin Wang*

Main category: cs.AI

TL;DR: A neuro-symbolic framework, NSPI, is proposed to prove polynomial inequalities by combining the strengths of Large Language Models (LLMs) and symbolic computation.


<details>
  <summary>Details</summary>
Motivation: The challenge of automated proving of polynomial inequalities due to scalability issues with purely symbolic approaches and the limitations of LLM-guided methods.

Method: NSPI framework, which uses an LLM to propose a conjecture in the form of an approximate polynomial Sum-Of-Squares (SOS) decomposition, refined via symbolic computation to obtain an exact polynomial SOS representation.

Result: Experiments on challenging benchmarks involving polynomials with up to 10 variables demonstrate the effectiveness and scalability of the proposed method.

Conclusion: The proposed NSPI framework effectively addresses the scalability challenges in automated proving of polynomial inequalities, providing a robust and efficient method for proving complex inequalities.
...
### [165] [X-SYNTH: Beyond Retrieval -- Enterprise Context Synthesis from Observed Human Attention](https://arxiv.org/abs/2605.15505)
*Guruprasad Raghavan,George Nychis,Rohan Narayana Murthy*

Main category: cs.AI

TL;DR: The paper presents X-SYNTH, a framework for synthesizing enterprise context by leveraging human attention and behavioral patterns to improve AI agent performance in complex tasks.


<details>
  <summary>Details</summary>
Motivation: Current approaches to AI agent tasks in enterprise operations are limited by their reliance on system state and retrieval-based methods, which fail to capture organization-specific knowledge and behavioral patterns.

Method: X-SYNTH models individual behavioral baselines as Digital Twin Signatures (DTS) and uses a four-stage pipeline with seven attention filters to identify causally relevant activity signatures.

Result: On a sales lead identification task, X-SYNTH improves True Lead Rate (TLR) from 9.5% to 61.9% and reduces False Lead Rate (FLR) from 90.5% to 18.8%.

Conclusion: The paper concludes that enterprise context synthesis is a relevance problem, not a retrieval problem, and that human attention is a reliable ground truth for improving AI agent performance.
...
### [166] [CAPS: Cascaded Adaptive Pairwise Selection for Efficient Parallel Reasoning](https://arxiv.org/abs/2605.15513)
*Fangzhou Lin,Shuo Xing,Peiran Li,Siyuan Yang,Qianwen Ge,Kazunori Yamada,Ziming Zhang,Haichong Zhang,Zhengzhong Tu*

Main category: cs.AI

TL;DR: CAPS, a new inference-only framework, reduces the computational cost of pairwise self-verification in large language models while maintaining or improving performance.


<details>
  <summary>Details</summary>
Motivation: The high cost of pairwise verification in parallel reasoning, where each judgment requires reading two complete solutions, motivated the development of a more efficient method.

Method: CAPS (Cascaded Adaptive Pairwise Selection), a four-stage cascade framework that allocates verifier compute non-uniformly along two axes: evidence and distribution.

Result: CAPS outperforms the leading pairwise verifier on 14 of 20 suites while using 25.4% of its verifier-token budget on code and outperforms pointwise self-verification on all 20 suites.

Conclusion: CAPS offers a more efficient and effective approach to pairwise self-verification, allowing for significant reductions in computational cost while maintaining or improving performance.
...
### [167] [RTL-BenchMT: Dynamic Maintenance of RTL Generation Benchmark Through Agent-Assisted Analysis and Revision](https://arxiv.org/abs/2605.15537)
*Jing Wang,Shang Liu,Hangan Zhou,Zhiyao Xie*

Main category: cs.AI

TL;DR: The paper introduces RTL-BenchMT, an automated framework for maintaining RTL generation benchmarks, addressing issues of flawed and overfitting cases.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenges of flawed cases and overfitting to benchmarks in RTL generation, which are difficult to resolve manually.

Method: The method proposed is an agentic framework called RTL-BenchMT, which automatically identifies and revises flawed benchmark cases and detects and updates overfitting cases.

Result: The result is a refined benchmark suite that will be open-sourced to the community, produced after a thorough analysis of flawed and overfitting cases.

Conclusion: The conclusion is that RTL-BenchMT can systematically reduce human maintenance costs and improve the quality of RTL generation benchmarks.
...
### [168] [DRS-GUI: Dynamic Region Search for Training-Free GUI Grounding](https://arxiv.org/abs/2605.15542)
*Yichao Liu,Huawen Shen,Liu Yu,Shiyu Liu,Zeyu Chen,Yu Zhou*

Main category: cs.AI

TL;DR: A new framework, DRS-GUI, enhances GUI agents' ability to understand and execute user instructions by accurately identifying relevant elements on complex screens.


<details>
  <summary>Details</summary>
Motivation: Existing GUI agents struggle to identify instruction-relevant elements on high-resolution screenshots with cluttered UI components.

Method: DRS-GUI uses a lightweight UI Perceptor and an Action Planner based on Monte Carlo Tree Search (MCTS) to dynamically search for relevant regions.

Result: DRS-GUI improves grounding performance by 14% on ScreenSpot-Pro for general and GUI-specific MLLMs.

Conclusion: DRS-GUI seamlessly integrates into existing MLLMs, enhancing their ability to understand and execute user instructions on complex GUI screens.
...
TL;DR: The paper argues for metacognition as a design principle for more accurate, secure, and efficient AI, and presents a software framework for metacognition-enabled AI applications.


<details>
  <summary>Details</summary>
Motivation: The authors are motivated by the potential benefits of metacognition in AI, including improved accuracy, security, and efficiency.

Method: The authors draw inspiration from past work on resource-rational AI and metacognitive strategies in psychology and cognitive science, and propose a metacognitive solution involving systems monitoring their own states and allocating resources judiciously.

Result: The authors showcase the effectiveness of their approach through a Federated Learning case study, which demonstrates improved learning efficiency, effectiveness, and security.

Conclusion: The authors conclude that metacognition can be a valuable design principle for creating more accurate, secure, and efficient AI, and provide a software framework for the community to experiment with metacognition-enabled AI applications.
...
TL;DR: The paper presents STAR, a framework for repairing erroneous root cause analysis (RCA) traces in microservice AIOps. STAR decomposes RCA into four stages and uses auditing, routing, and repair techniques to improve reliability.


<details>
  <summary>Details</summary>
Motivation: The reliability of LLM-based RCA agents is fragile due to errors in evidence collection, hypothesis formulation, or causal analysis, which can corrupt the final diagnosis.

Method: STAR framework, which includes stage-wise auditing, budget-aware Fast/Slow Routing, decisive stage localization via counterfactual candidate evaluation, and stage-specific patch-and-replay repair.

Result: STAR consistently improves root cause localization and fault type classification over strong baselines, and identifies the decisive faulty stage with high accuracy.
...
### [171] [See Before You Code: Learning Visual Priors for Spatially Aware Educational Animation Generation](https://arxiv.org/abs/2605.15585)
*Yuejia Li,Ke He,Junheng Li,Shutong Chen,Jingkang Xia,Zhiyue Su,Junchi Zhang,Mang Ye*

Main category: cs.AI

TL;DR: The paper presents OmniManim, a framework for generating high-quality educational animations using large language models, addressing issues like visual defects and animation continuity.


<details>
  <summary>Details</summary>
Motivation: Existing language models can generate executable code for educational animations but often produce renders with visual defects that can only be detected after execution.

Method: The authors introduce OmniManim, a render-feedback-aware framework that incorporates a shared scene state, explicit visual planning, structured post-render diagnostics, and localized repair. It includes a Vision Agent for task-specific visual planning.

Result: OmniManim improves measured render quality over single-model baselines and existing multi-agent frameworks on the EduRequire-500 dataset. Ablation studies highlight the importance of explicit visual planning.

Conclusion: The OmniManim framework effectively addresses the problem of render-feedback-aware constrained code generation for educational animations, leading to improved visual quality and animation continuity.
...
### [172] [TopoEvo: A Topology-Aware Self-Evolving Multi-Agent Framework for Root Cause Analysis in Microservices](https://arxiv.org/abs/2605.15611)
*Junle Wang,Xingchuang Liao,Wenjun Wu*

Main category: cs.AI

TL;DR: The paper proposes TopoEvo, a topology-aware framework for root cause analysis in microservices, addressing challenges such as noisy observability data, cascading failures, and changing topology.


<details>
  <summary>Details</summary>
Motivation: Root cause analysis in microservices is challenging due to noisy and heterogeneous data, cascading failures, and changing topology, and existing methods often suffer from symptom-amplification bias.

Method: The authors propose TopoEvo, which uses graph representation learning and structured reasoning, and consists of several components: Metric-orthogonal Multimodal Alignment (MOMA), Vector Quantization (VQ), a multi-agent Hypothesis-Evidence-Test (HET) workflow, and a Self-Evolving Mechanism.

Result: The paper does not provide specific experimental results, but suggests that TopoEvo can provide reliable and robust root cause analysis in microservices.

Conclusion: The paper concludes that TopoEvo can effectively address the challenges of root cause analysis in microservices by providing topology-aware and self-evolving capabilities.
...
### [173] [ColPackAgent: Agent-Skill-Guided Hard-Particle Monte Carlo Workflows for Colloidal Packing](https://arxiv.org/abs/2605.15625)
*Lijie Ding,Changwoo Do*

Main category: cs.AI

TL;DR: Introduced ColPackAgent, an agent framework for autonomous Monte Carlo simulations of colloidal packing, leveraging a Model Context Protocol server and agent skill.


<details>
  <summary>Details</summary>
Motivation: General-purpose Large Language Model agents struggle to execute simulation workflows reliably without dedicated tools and instructions.

Method: ColPackAgent uses a custom-built colpack Python package and a four-stage workflow contract to execute simulations autonomously or with human feedback.

Result: Demonstrated ColPackAgent's capabilities in various modes with several colloidal packing simulation examples and benchmarked its performance across multiple LLMs.

Conclusion: Pairing domain-specific Python packages with Model Context Protocol tools and portable agent skills enables efficient agent-assisted research workflows for simulations.
...
### [174] [Nudging Beyond the Comfort Zone: Efficient Strategy-Guided Exploration for RLVR](https://arxiv.org/abs/2605.15726)
*Chanuk Lee,Sangwoo Park,Minki Kang,Sung Ju Hwang*

Main category: cs.AI

TL;DR: The paper proposes NudgeRL, a framework for structured and diversity-driven exploration in Reinforcement Learning with Verifiable Rewards (RLVR), which outperforms standard methods with less computation.


<details>
  <summary>Details</summary>
Motivation: The effectiveness of RLVR is limited by exploration, and brute-force scaling is computationally expensive, while existing approaches provide limited control over what is explored.

Method: The authors propose Strategy Nudging, which conditions each rollout on lightweight strategy-level contexts, and a unified objective that decomposes the reward signal into inter- and intra-context components.

Result: NudgeRL outperforms standard GRPO with up to 8 times larger rollout budgets and oracle-guided RL baseline on average across five challenging math benchmarks.

Conclusion: Structured, context-driven exploration can serve as an efficient and scalable alternative to brute-force rollout scaling and feasibility-oriented methods based on privileged information.
...
### [175] [Can We Trust AI-Inferred User States. A Psychometric Framework for Validating the Reliability of Users States Classification by LLMs in Operational Environments](https://arxiv.org/abs/2605.15734)
*Izabella Krzeminska,Michal Butkiewicz,Ewa Komkowska*

Main category: cs.AI

TL;DR: The paper tests the assumption that metrics used by large language models to assess user states are stable and interpretable at an individual level, finding that most metrics are not reliable for real-time adaptation but can be useful in post-hoc analyses.


<details>
  <summary>Details</summary>
Motivation: The use of large language models in conversational and adaptive systems assumes that metrics for assessing user states are stable and interpretable at an individual level.

Method: The study employed replication evaluation procedures to assess the repeatability of metrics across three different bimodal large language models (GPT-4o audio, Gemini 2.0 Flash, Gemini 2.5 Flash).

Result: Only 31 of 213 metrics met the criteria for reliability at the individual score level, indicating that most metrics are not stable enough for real-time adaptive systems but can be useful in post-hoc studies.

Conclusion: The paper concludes that metric reliability cannot be considered a default property in interpretive domains and proposes a replicable evaluation framework for measurable evaluations of metric applicability, supporting more responsible AI design.
...
### [176] [ALSO: Adversarial Online Strategy Optimization for Social Agents](https://arxiv.org/abs/2605.15768)
*Xiang Li,Liping Yi,Mingze Kong,Min Zhang,Zhongxiang Dai,QingHua Hu*

Main category: cs.AI

TL;DR: The paper proposes ALSO, a framework for online strategy optimization in multi-agent social simulation, which enables agents to adapt dynamically in non-stationary environments.


<details>
  <summary>Details</summary>
Motivation: Current Large Language Model (LLM) based social agents rely on static personas, which are ill-suited for dynamic environments. Existing approaches for enhancing social intelligence assume stationarity and incur substantial training overhead.

Method: ALSO formulates multi-turn interaction as an adversarial bandit problem and introduces a lightweight neural surrogate to predict rewards from interaction histories.

Result: Experiments on the Sotopia benchmark demonstrate that ALSO consistently outperforms static baselines and existing optimization methods in dynamic environments.

Conclusion: The proposed ALSO framework advances social adaptation in multi-agent social simulation, providing a principled solution to non-stationarity and enabling sample-efficient exploration and continuous online adaptation.
...
### [177] [SaaS-Bench: Can Computer-Use Agents Leverage Real-World SaaS to Solve Professional Workflows?](https://arxiv.org/abs/2605.15777)
...
TL;DR: The paper introduces SaaS-Bench, a new benchmark for evaluating Computer-Using Agents (CUAs) in Software-as-a-Service (SaaS) environments, highlighting the limitations of current LLM-based agents in complex workflows.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for CUAs have limitations, such as simplified settings, isolated tasks, or short-horizon interactions, making it difficult to assess agent capabilities in realistic professional workflows.

Method: The authors introduce SaaS-Bench, a benchmark built on 23 deployable SaaS systems across six professional domains, containing 106 tasks grounded in realistic work scenarios, and evaluate LLM-based agents using weighted verification checkpoints.

Result: Experiments show that representative LLM-based agents struggle on SaaS-Bench, with even the strongest model completing fewer than 4% of tasks end-to-end, exposing limitations in planning, state tracking, cross-application context maintenance, and error recovery.

Conclusion: The paper highlights the need for more comprehensive benchmarks like SaaS-Bench to evaluate CUAs in realistic workflows and identifies significant limitations in current LLM-based agents.
...
### [178] [Agentic Discovery of Neural Architectures: AIRA-Compose and AIRA-Design](https://arxiv.org/abs/2605.15871)
*Alberto Pepe,Chien-Yu Lin,Despoina Magka,Bilge Acun,Yannan Nellie Wu,Anton Protopopov,Carole-Jean Wu,Yoram Bachrach*

Main category: cs.AI

TL;DR: The paper presents a dual-framework approach for autonomous design of foundation models beyond standard Transformers, achieving performance improvements and efficient scaling.


<details>
  <summary>Details</summary>
Motivation: The motivation is to investigate LLM agents autonomously designing foundation models for recursive self-improvement.

Method: The method involves a dual-framework approach: AIRA-Compose for high-level architecture search and AIRA-Design for low-level mechanistic implementation.

Result: The results show that the autonomously designed architectures outperform hand-designed baselines, with improvements in accuracy and efficient scaling, and achieve state-of-the-art performance on various benchmarks.

Conclusion: The study establishes a powerful paradigm for discovering next-generation foundation models, marking a clear step toward recursive self-improvement.
...
### [179] [Imperfect World Models are Exploitable](https://arxiv.org/abs/2605.15960)
*Logan Mondal Bhamidipaty,Esmeralda S. Whitammer,David Abel,Mykel J. Kochenderfer,Subramanian Ramamoorthy*

Main category: cs.AI

TL;DR: The paper proposes a definition of model exploitation in reinforcement learning, develops a general theory of reward hacking and model exploitation, and establishes a formal bridge between the two.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the issue of model exploitation in reinforcement learning, which occurs when a world model implies that one policy should be preferred over another, but the environment's true transition model implies the reverse.

Method: The authors develop a general theory of reward hacking and model exploitation, and use it to prove that exploitation is essentially unavoidable on large policy sets.

Result: The authors find that exploitation is unavoidable on large policy sets, but introduce a relaxed notion of exploitation and derive a safe horizon within which it can be avoided.

Conclusion: The paper establishes a formal bridge between reward hacking and model exploitation, and elucidates the limits of safe planning in world models.
...
### [180] [PAGER: Bridging the Semantic-Execution Gap in Point-Precise Geometric GUI Control](https://arxiv.org/abs/2605.15963)
*Jingxuan Wei,Xi Bai,Shan Liu,Caijun Jia,Zheng Sun,Xinglong Xu,Siyuan Li,Linzhuang Sun,Bihui Yu,Conghui He,Cheng Tan*

Main category: cs.AI

TL;DR: The paper discusses the challenges of precise geometric construction in GUI agents and proposes a new agent, PAGER, that achieves state-of-the-art performance in point-precise GUI control.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of current GUI agents in precise geometric construction, which requires point-level accuracy, geometry-aware verification, and robustness to dependency-driven error propagation.

Method: The method involves introducing PAGE Bench, a benchmark for precision-sensitive GUI tasks, and proposing PAGER, a topology-aware agent that decomposes construction into dependency-structured planning and pixel-level execution.

Result: The results show that PAGER delivers 4.1x higher task success than the strongest evaluated general baseline and raises step success rate from below 9% for GUI-specialized agents to over 62%.

Conclusion: The conclusion is that PAGER establishes a new state of the art for point-precise GUI control and closes the Semantic-Execution Gap, which is the difference between general multimodal models' high action type accuracy and low task success.
...
### [181] [Deterministic Event-Graph Substrates as World Models for Counterfactual Reasoning](https://arxiv.org/abs/2605.15967)
*Fabio Rovai*

Main category: cs.AI

TL;DR: The paper presents event-graph substrates, a class of world models that can answer counterfactual queries by forking an append-only log of typed RDF triples, and demonstrates its effectiveness in several benchmarks.


<details>
  <summary>Details</summary>
Motivation: The authors aim to develop a world model that can represent agent state as a log of typed RDF triples and answer counterfactual queries effectively.

Method: The authors formalize the class of event-graph substrates, prove a duality between explanatory and counterfactual queries, and evaluate a CLEVRER-DSL interpreter atop a domain-agnostic substrate runtime.

Result: The substrate outperforms the NS-DR symbolic oracle and the parametric ALOE baseline in several categories, and exceeds Llama-3.1-8B with full context by 18.80 points joint accuracy on the twin-EventLog benchmark.

Conclusion: The paper concludes that event-graph substrates are a promising approach for representing world models and answering counterfactual queries, with advantages in inspectability, exact counterfactuals, and transfer across domains.
...
### [182] [Learning Bilevel Policies over Symbolic World Models for Long-Horizon Planning](https://arxiv.org/abs/2605.15975)
*Dillon Z. Chen,Till Hofmann,Toryn Q. Klassen,Sheila A. McIlraith*

Main category: cs.AI

TL;DR: The authors propose a method combining low-level imitation learning and high-level symbolic abstractions to solve long-horizon planning problems in embodied AI agents.


<details>
  <summary>Details</summary>
Motivation: The challenge of building embodied AI agents that can reliably solve long-horizon planning problems, which imitation learning alone struggles with.

Method: The authors propose bilevel policies consisting of a neural policy learned from low-level demonstrations and a high-level symbolic policy constructed from symbolic abstractions of the low-level demonstrations combined with inductive generalisation.

Result: Experiments on extended MetaWorld benchmarks demonstrate that the proposed BISON system generalises to long horizons and problems with greater numbers of objects than those solved by VLA and end-to-end methods, and is more time and memory efficient in training and inference.

Conclusion: The proposed method effectively combines the strengths of low-level imitation learning and high-level symbolic abstractions to solve long-horizon planning problems in embodied AI agents.
...
### [183] [Petri Net Induced Heuristic Search for Resource Constrained Scheduling](https://arxiv.org/abs/2605.15983)
*Ido Lublin,Dor Atzmon,Izack Cohen*

Main category: cs.AI

TL;DR: The paper presents a novel approach to solving the Resource-Constrained Project Scheduling Problem (RCPSP) using a Timed Transition Petri Net with Resources and A* search, outperforming Mixed-Integer Linear Programming (MIP) baselines.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to improve the solution method for the Resource-Constrained Project Scheduling Problem (RCPSP), which is a complex scheduling problem.

Method: The method involves formulating RCPSP as an optimal search over the reachability graph of a Timed Transition Petri Net with Resources, using A* search guided by a heuristic that combines Critical Path and resource-based lower bounds.

Result: The results show that the proposed approach outperforms strong exact MIP baselines (SCIP, CBC) in both success rate and solve time on PSPLIB benchmarks.

Conclusion: The conclusion is that the proposed approach provides a more efficient and effective solution for RCPSP, with the A* search and heuristic degrading along independent axes of resource tightness and formulation size, respectively.
...
### [184] [ScreenSearch: Uncertainty-Aware OS Exploration](https://arxiv.org/abs/2605.16024)
*Michael Solodko,Justin Wagle*

Main category: cs.AI

TL;DR: The paper presents ScreenSearch, a system for large-scale desktop exploration that combines structural screen retrieval and deduplication with an ambiguity-aware graph-bandit algorithm to effectively navigate through visually similar screens and reduce ambiguity.


<details>
  <summary>Details</summary>
Motivation: Desktop GUI agents operate under partial observability, where visually similar screens can correspond to different underlying workflow states, making it challenging to determine effective behavior.

Method: The authors propose ScreenSearch, which combines structural screen retrieval and deduplication with an ambiguity-aware PUCT graph-bandit algorithm to drive large-scale exploration and replay-start policy evaluation.

Result: ScreenSearch collects over 1M screenshots and over 30K deduplicated states across 11 desktop applications, yielding large exploration corpora with substantial cross-application and within-application diversity.

Conclusion: The results suggest that state identity, proposal quality, and ambiguity-aware search all matter when deciding when to probe and when to commit, and that ambiguity reduction alone is not a sufficient exploration objective.
...
### [185] [Reasoners or Translators? Contamination-aware Evaluation and Neuro-Symbolic Robustness in Tax Law](https://arxiv.org/abs/2605.16052)
*Parisa Kordjamshidi,Samer Aslan,Madhavan Seshadri,Leslie Barrett,Enrico Santus*

Main category: cs.AI

TL;DR: The paper examines the reliability of large language models (LLMs) in legal reasoning, finding that their performance can be inflated by data contamination, and proposes a neuro-symbolic framework for more robust legal AI.


<details>
  <summary>Details</summary>
Motivation: To investigate whether LLMs' performance in legal reasoning is due to genuine ability or data contamination, and to develop a more reliable approach to legal AI.

Method: The study implements a contamination detection protocol, conducts a systematic evaluation of LLMs and hybrid systems, and builds a novel test suite to probe generalization to unseen documents.

Result: The findings indicate that legal reasoning is inherently compositional, and that neuro-symbolic frameworks offer a more reliable and robust foundation for legal AI, with improved generalization to unobserved situations.

Conclusion: The paper concludes that neuro-symbolic frameworks are a more reliable approach to legal AI, and that LLMs' performance can be improved by combining them with symbolic solvers.
...
### [186] [Sign-Separated Finite-Time Error Analysis of Q-Learning](https://arxiv.org/abs/2605.16103)
*Donghwan Lee*

Main category: cs.AI

TL;DR: Develops a sign-separated finite-time error analysis for constant step-size Q-learning, providing bounds on error dynamics and identifying asymmetry in Q-learning error.


<details>
  <summary>Details</summary>
Motivation: To understand and analyze the error dynamics of Q-learning with constant step-size, particularly in finite-time settings.

Method: Switching-system representation of Q-learning error, decomposed into negative and positive parts, and analyzed using LTI systems and linear switching systems.

Result: Derives finite-time bounds for Q-learning error in both deterministic and stochastic settings, and identifies max-induced asymmetry in Q-learning error dynamics.

Conclusion: The analysis provides insights into the behavior of Q-learning error and highlights the importance of considering asymmetry in error dynamics.
...
### [187] [ShopGym: An Integrated Framework for Realistic Simulation and Scalable Benchmarking of E-Commerce Web Agents](https://arxiv.org/abs/2605.16116)
*Chinmay Savadikar,Mingyu Zhao,Yuanzheng Zhu,Han Li,Shuang Xie,Alberto Castelo,Tianfu Wu,Lingyun Wang*

Main category: cs.AI

TL;DR: The paper introduces ShopGym, a framework for simulating and benchmarking e-commerce web agents in a realistic, controllable, and reproducible way.


<details>
  <summary>Details</summary>
Motivation: The field of e-commerce web agent development lacks a scalable method to create evaluation settings that are realistic, diverse, controllable, inspectable, and reproducible.

Method: The authors propose ShopGym, which consists of ShopArena for simulating storefronts and ShopGuru for synthesizing benchmark tasks. ShopArena converts live storefronts into sandbox shops, while ShopGuru generates tasks across seven skill categories.

Result: The framework is validated through graph-based structural analysis and agent-based behavioral evaluation. Results show that synthetic shops preserve key structural properties of live storefronts, with positive correlation in agent performance between synthetic and live storefronts.

Conclusion: ShopGym provides a scalable and realistic framework for evaluating e-commerce web agents, addressing the methodological bottleneck in the field.
...
### [188] [Property-Guided LLM Program Synthesis for Planning](https://arxiv.org/abs/2605.16142)
*Augusto B. Corrêa,André G. Pereira,Jendrik Seipp*

Main category: cs.AI

TL;DR: The paper proposes a property-guided LLM program synthesis approach that uses formal properties to guide the synthesis process, reducing the number of program generations and evaluation costs.


<details>
  <summary>Details</summary>
Motivation: Current LLM program synthesis approaches rely on simple numeric scores to signal program quality, which can be inefficient and ineffective. The authors aim to improve the synthesis process by using formally defined properties to guide the LLM.

Method: The authors propose a property-guided LLM program synthesis approach that checks whether a candidate program satisfies a formally defined property. If the property is violated, the evaluation is stopped early, and a concrete counterexample is provided to the LLM.

Result: The approach is evaluated on ten planning domains with an out-of-distribution test set. The synthesized heuristics are effectively direct on virtually all test tasks, and the approach generates seven times fewer programs per domain on average, solves more tasks without using search, and requires several orders of magnitude less computation to evaluate candidates.

Conclusion: The paper concludes that property-guided LLM synthesis can reduce cost and improve program quality whenever a problem admits a verifiable property.
...
### [189] [Look Before You Leap: Autonomous Exploration for LLM Agents](https://arxiv.org/abs/2605.16143)
*Ziang Ye,Wentao Shi,Yuxin Liu,Yu Wang,Zhengzhou Cai,Yaorui Shi,Qi Gu,Xunliang Cai,Fuli Feng*

Main category: cs.AI

TL;DR: The paper addresses the issue of premature exploitation in large language model-based agents, proposing a new training strategy and paradigm called Explore-then-Act to improve agent adaptability in unfamiliar environments.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome the limitation of large language model-based agents in unfamiliar environments due to premature exploitation, by developing autonomous exploration capabilities.

Method: The method involves introducing Exploration Checkpoint Coverage, a verifiable metric to measure agent discovery, and a training strategy that interleaves task-execution and exploration rollouts. The Explore-then-Act paradigm is also proposed.

Result: The results show that agents trained with the proposed strategy exhibit improved adaptability and generalizability, and that learning to systematically explore is crucial for building real-world-ready agents.

Conclusion: The paper concludes that autonomous exploration is a critical capability for building adaptive agents, and that the proposed approach can lead to more generalizable and real-world-ready agents.
...
### [190] [An Algebraic Exposition of the Theory of Dyadic Morality](https://arxiv.org/abs/2605.16153)
*Kush R. Varshney*

Main category: cs.AI

TL;DR: The paper presents an algebraic framework for the Theory of Dyadic Morality (TDM), a psychological model of moral judgment, and demonstrates its applications in AI policy design.


<details>
  <summary>Details</summary>
Motivation: The motivation is to provide a mathematically rigorous and faithful to human moral cognition framework for computing morality in neurosymbolic AI systems.

Method: The authors use structural causal modeling (SCM) notation and identify three psychological operators to extend standard SCM, and address scalability challenges through node collapse and sequential processing.

Result: The paper demonstrates concrete applications to AI policy design, including detecting conflicting obligations, structuring helpfulness policies, and designing post-failure communication as causal interventions.

Conclusion: The algebraic formalization enables neurosymbolic AI systems to compute morality in a way that is both mathematically rigorous and faithful to human moral cognition.
...
### [191] [Formal Methods Meet LLMs: Auditing, Monitoring, and Intervention for Compliance of Advanced AI Systems](https://arxiv.org/abs/2605.16198)
*Parand A. Alamdari,Toryn Q. Klassen,Sheila A. McIlraith*

Main category: cs.AI

TL;DR: The paper proposes techniques for monitoring and auditing AI-enabled products and services throughout the development lifecycle, focusing on detecting violations of behavioral constraints in black-box AI systems, particularly Large Language Models (LLMs).


<details>
  <summary>Details</summary>
Motivation: The need for effective monitoring and auditing of AI-enabled products and services to ensure compliance with safety constraints, norms, rules, and regulations, especially in black-box advanced AI systems like LLMs.

Method: Combining formal methods with state-of-the-art machine learning, the paper introduces techniques for offline auditing and online monitoring of product-specific behavioral constraints, including predictive monitoring and intervening monitors.

Result: The proposed techniques outperform LLM baseline methods in detecting violations of temporally extended behavioral constraints, with even small-model labelers matching or exceeding frontier LLM judges, and significantly reduce violation rates of LLM-based agents while preserving task performance.

Conclusion: The paper concludes that exploiting formal syntax and semantics, such as Linear Temporal Logic (LTL), can improve the auditing and monitoring of AI-enabled products and services, particularly in detecting violations of behavioral constraints in black-box AI systems like LLMs.
...
### [192] [Context, Reasoning, and Hierarchy: A Cost-Performance Study of Compound LLM Agent Design in an Adversarial POMDP](https://arxiv.org/abs/2605.16205)
*Igor Bogdanov,Chung-Horng Lung,Thomas Kunz,Jie Gao,Adrian Taylor,Marzia Zaman*

Main category: cs.AI

TL;DR: The paper studies the design of compound LLM agents in adversarial environments, finding that programmatic state abstraction and hierarchical decomposition are more cost-effective than deliberation tools.


<details>
  <summary>Details</summary>
Motivation: Practitioners lack guidance on designing compound LLM agents in adversarial, partially observable sequential environments, and the paper aims to provide insights on design choices that improve performance.

Method: The paper presents a controlled study using CybORG CAGE-2, a cyber defense environment modeled as a POMDP, evaluating five model families, six models, and twelve configurations across 3,475 episodes.

Result: The paper finds that programmatic state abstraction delivers the largest returns per token spent, distributing deliberation tools across a hierarchy degrades performance, and hierarchical decomposition without deliberation achieves the best absolute performance for most models.

Conclusion: The paper suggests a design principle for structured adversarial POMDPs: invest in programmatic infrastructure and clean task decomposition rather than deeper per-agent reasoning.
...
### [193] [Confirming Correct, Missing the Rest: LLM Tutoring Agents Struggle Where Feedback Matters Most](https://arxiv.org/abs/2605.16207)
*Tahreem Yasir,Wenbo Li,Sam Gilson,Sutapa Dey Tithi,Xiaoyi Tian,Tiffany Barnes*

Main category: cs.AI

TL;DR: LLMs struggle to accurately evaluate student solutions, particularly in distinguishing between valid but suboptimal and incorrect solutions, which is crucial for effective tutoring.


<details>
  <summary>Details</summary>
Motivation: The increasing use of Large Language Models (LLMs) in intelligent tutoring systems (ITS) necessitates evaluating their diagnostic precision in assessing student solutions.

Method: A benchmark of seven LLM feedback agents was conducted using propositional logic and knowledge-graph-derived ground truth across 10,836 solution-feedback pairs and three feedback conditions.

Result: LLMs achieved high performance on optimal steps but systematically over-rejected valid but suboptimal solutions and over-validated incorrect solutions, indicating a significant gap in their diagnostic capabilities.

Conclusion: The findings suggest that LLMs are better suited for hybrid architectures where knowledge-graph-grounded models handle diagnosis, while LLMs support open-ended scaffolding and dialogue.
...
### [194] [Fully Open Meditron: An Auditable Pipeline for Clinical LLMs](https://arxiv.org/abs/2605.16215)
...
TL;DR: The authors introduce Fully Open Meditron, a fully open pipeline for building LLM-based clinical decision support systems (CDSS), achieving state-of-the-art performance while maintaining auditability and reproducibility.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based CDSS are opaque, and most 'open' models only release parameters, withholding data provenance, curation procedures, and generation pipelines. The authors aim to create a fully open pipeline for building LLM-CDSS.

Method: The authors propose a fully open pipeline comprising a clinician-audited training corpus, a reproducible data construction and training framework, and a use-aligned evaluation protocol. They apply this pipeline to five base models and evaluate using an LLM-as-a-judge protocol.

Result: The results show that all MeditronFO variants outperform their base models. Specifically, Apertus-70B-MeditronFO improves by 6.6 points over its base, and Gemma-3-27B-MeditronFO is preferred over MedGemma in 58.6% of LLM-as-a-judge comparisons.

Conclusion: The authors conclude that fully open pipelines can achieve state-of-the-art domain-specific performance without sacrificing auditability or reproducibility, paving the way for more transparent and reliable LLM-based CDSS.
...
### [195] [FORGE: Self-Evolving Agent Memory With No Weight Updates via Population Broadcast](https://arxiv.org/abs/2605.16233)
*Igor Bogdanov,Chung-Horng Lung,Thomas Kunz,Jie Gao,Adrian Taylor,Marzia Zaman*

Main category: cs.AI

TL;DR: FORGE, a staged protocol, improves LLM agents' decision-making through self-generated memory without gradient updates, outperforming baselines in a stochastic network-defense POMDP.


<details>
  <summary>Details</summary>
Motivation: To explore if LLM agents can improve decision-making through self-generated memory without gradient updates.

Method: FORGE (Failure-Optimized Reflective Graduation and Evolution), a staged, population-based protocol that evolves prompt-injected natural-language memory for hierarchical ReAct agents.

Result: FORGE improves average evaluation return by 1.7-7.7 times over zero-shot and by 29-72% over Reflexion, reducing major-failure rates to as low as ~1%.

Conclusion: FORGE is effective in improving LLM agents' decision-making through self-generated memory, with population broadcast and Examples achieving strong returns.
...
### [196] [Prospective multi-pathogen disease forecasting using autonomous LLM-guided tree search](https://arxiv.org/abs/2605.16238)
*Sarah Martinson,Michael P. Brenner,Martyna Plomecka,Brian P. Williams,Nicholas G. Reich,Zahra Shamsi*

Main category: cs.AI

TL;DR: An autonomous system using Large Language Model (LLM)-guided tree search generates, evaluates, and optimizes forecasting software for infectious diseases, achieving comparable or superior performance to human-curated models.


<details>
  <summary>Details</summary>
Motivation: The labor-intensive manual model curation by expert modeling teams is a bottleneck for scalable probabilistic forecasting of infectious diseases to granular geographic resolutions or emerging pathogens.

Method: The system uses LLM-guided tree search to iteratively generate, evaluate, and optimize executable forecasting software, autonomously discovering methodologically diverse models for influenza, COVID-19, and RSV.

Result: The machine-generated models yielded an ensemble that consistently matched or outperformed the gold-standard, human-curated CDC hub ensembles out-of-sample, successfully navigating data-scarce 'cold start' scenarios for RSV.

Conclusion: The framework overcomes the modeling labor bottleneck, enabling rapid deployment of expert-level disease forecasting at unprecedented scales by autonomously translating epidemiological theory into accurate, transparent code.
...
### [197] [Probabilistic Dating of Historical Manuscripts via Evidential Deep Regression on Visual Script Features](https://arxiv.org/abs/2605.06475)
*Ranjith Chodavarapu*
...
<details>
  <summary>Details</summary>
Motivation: To improve the accuracy and calibration of dating historical manuscript pages by introducing a probabilistic approach that outputs a full predictive distribution with decomposed uncertainty.
...
Result: The model achieves a test MAE of 5.4 years, with 93% of patches within 5 years and 97% within 10 years, and PICP=92.6% calibration.

Conclusion: The proposed approach outperforms previous methods in accuracy and calibration, and provides a useful tool for dating historical manuscript pages with quantified uncertainty.
