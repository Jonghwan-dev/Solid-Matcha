...
TL;DR: A novel 3D fusion framework is proposed to construct metrically consistent digital twins by registering object meshes with transformer-predicted point clouds, resolving scale ambiguity and coordinate mismatch.


<details>
  <summary>Details</summary>
Motivation: Embodied AI training requires object-centric digital twin environments with accurate metric geometry and semantic grounding, but recent reconstruction methods suffer from scale ambiguity and inconsistent coordinate conventions.

Method: A scale-aware 3D fusion framework is introduced, using a Vision-Language Model-guided geometric anchor mechanism and a geometry-aware registration pipeline to enforce physical plausibility.

Result: Experiments demonstrate improved cross-network object alignment and geometric consistency for downstream tasks, including multi-primitive fitting and metric measurement.

Conclusion: The proposed framework enables the construction of metrically consistent digital twins, which is crucial for embodied AI training and evaluation, and an open-source indoor digital twin dataset is introduced.
...
### [2] [UniICL: Systematizing Unified Multimodal In-context Learning through a Capability-Oriented Taxonomy](https://arxiv.org/abs/2603.24690)
*Yicheng Xu,Jiangning Zhang,Zhucun Xue,Teng Hu,Ran Yi,Xiaobin Hu,Yong Liu,Dacheng Tao*

Main category: cs.CV

TL;DR: The paper addresses the sensitivity of in-context learning to example selection and formatting in multimodal models, proposing a taxonomy, a large-scale corpus, and a novel architectural intervention for stable few-shot adaptation.


<details>
  <summary>Details</summary>
Motivation: In-context learning's efficacy is often non-monotonic and highly task-dependent due to cross-modal interference and varying cognitive demands in unified multimodal models.

Method: The authors introduce a six-level capability-oriented taxonomy, construct a large-scale corpus (UniICL-760K), and propose the Context-Adaptive Prototype Modulator, a lightweight, plug-and-play module for stable few-shot adaptation.

Result: Evaluations on UniICL-Bench show that the proposed approach yields highly competitive unified results, outperforming larger-parameter multimodal large language model baselines on most understanding in-context learning tasks.

Conclusion: The paper provides a comprehensive framework for understanding and improving in-context learning in multimodal models, including a taxonomy, a large-scale corpus, and a novel architectural intervention.
...
TL;DR: The authors propose a framework called BCMDA to improve semi-supervised medical image segmentation under domain shift and limited annotations.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address two primary issues in semi-supervised medical image segmentation: (1) distributional differences between labeled and unlabeled data, and (2) inefficient learning from unlabeled data causing severe confirmation bias.

Method: The authors propose the BCMDA framework, which employs knowledge transfer via virtual domain bridging (KTVDB) and prototypical alignment and pseudo label correction (PAPLC) to overcome the issues.

Result: The proposed method demonstrates superior performance on three public multi-domain datasets, particularly with very limited labeled samples.

Conclusion: The authors conclude that their proposed BCMDA framework effectively addresses the challenges of domain shift and limited annotations in semi-supervised medical image segmentation.
...
TL;DR: Introduced LLaVA-LE, a vision-language model for lunar surface and subsurface characterization, and a new large-scale multimodal lunar dataset LUCID.


<details>
  <summary>Details</summary>
Motivation: The application of multimodal vision-language models to planetary science is largely unexplored due to the absence of large-scale datasets pairing real planetary imagery with detailed scientific descriptions.

Method: Fine-tuned LLaVA using a two-stage training curriculum: concept alignment for domain-specific terrain description and instruction-tuned visual question answering.
...
Conclusion: Domain-specific multimodal data and instruction tuning advance VLMs in planetary exploration, as demonstrated by LLaVA-LE's effectiveness.
...
TL;DR: The paper proposes QuatRoPE, a novel positional embedding method for spatial reasoning in 3D scenes, which improves the ability of Large Language Models (LLMs) to reason about spatial relations.


<details>
  <summary>Details</summary>
Motivation: The limited availability of 3D scene-language paired data makes it challenging to train models with strong reasoning ability from scratch, and previous approaches have limitations in encoding spatial relations.

Method: The paper proposes QuatRoPE, a positional embedding method that explicitly calculates pairwise spatial relations through the dot product in attention layers, and introduces the Isolated Gated RoPE Extension (IGRE) to minimize interference with the LLM's existing positional embeddings.

Result: Extensive experiments demonstrate the effectiveness of the proposed approaches.

Conclusion: The proposed QuatRoPE and IGRE improve the ability of LLMs to reason about spatial relations in 3D scenes, maintaining fidelity to the scene's geometric integrity.
...
<details>
  <summary>Details</summary>
Motivation: To understand where humans and machine classifiers draw the line between different concepts when visual evidence is ambiguous.

Method: A psychophysically-informed framework that generates continuous spectra of ambiguous images in the CLIP embedding space.
...
### [7] [TIGeR: A Unified Framework for Time, Images and Geo-location Retrieval](https://arxiv.org/abs/2603.24749)
...
<details>
  <summary>Details</summary>
Motivation: Many real-world applications require jointly reasoning about visual appearance, geolocation, and time, beyond standard geo-localization and time-of-capture prediction.

Method: The authors propose TIGeR, a multi-modal-transformer-based model that uses a unified geo-temporal embedding space to perform geo-localization, time-of-capture prediction, and geo-time-aware retrieval.
...
Conclusion: The paper concludes that unified geo-temporal modeling is beneficial and that TIGeR enables retrieval based on where and when a scene is, rather than purely on visual similarity.
...
TL;DR: GoldiCLIP, a framework that balances supervision signals to improve large-scale vision-language models, achieves state-of-the-art results with 300x less data than leading methods.


<details>
  <summary>Details</summary>
Motivation: The success of large-scale vision-language models relies heavily on large datasets, which can be a barrier to progress. Recent works have improved supervision quality but address only a subset of weaknesses in contrastive pretraining.

Method: GoldiCLIP combines three innovations: text-conditioned self-distillation, an encoder integrated decoder with VQA objective, and uncertainty-based weighting mechanism to balance heterogeneous losses.

Result: GoldiCLIP achieves state-of-the-art results among data-efficient approaches, improving over the best comparable baseline by 2.2 points on MSCOCO retrieval, 2.0 on fine-grained retrieval, and 5.9 on question-based retrieval.

Conclusion: GoldiCLIP demonstrates that with a balanced approach to supervision signals, large-scale vision-language models can achieve competitive results with much less data.
...
### [9] [NeuroVLM-Bench: Evaluation of Vision-Enabled Large Language Models for Clinical Reasoning in Neurological Disorders](https://arxiv.org/abs/2603.24846)
...
TL;DR: This study benchmarks vision-enabled large language models for 2D neuroimaging across multiple tasks and datasets, evaluating performance, reliability, and efficiency trade-offs.


<details>
  <summary>Details</summary>
Motivation: The reliability and operational trade-offs of multimodal large language models in neuroimaging are not well understood, and the study aims to provide insights into their performance.

Method: The study uses a multi-phase framework to evaluate twenty frontier multimodal models across four directions: discriminative classification, calibration, structured-output validity, and computational efficiency.

Result: The results show that technical imaging attributes are nearly solved, but diagnostic reasoning remains challenging, with tumor classification being the most reliable task and multiple sclerosis and rare abnormalities being difficult. Few-shot prompting improves performance but increases token usage, latency, and cost.

Conclusion: The study provides practical insights into performance, reliability, and efficiency trade-offs, supporting standardized evaluation of multimodal LLMs in neuroimaging, and identifies the best-performing models, including Gemini-2.5-Pro, GPT-5-Chat, and MedGemma-1.5-4B.
...
### [10] [TIGFlow-GRPO: Trajectory Forecasting via Interaction-Aware Flow Matching and Reward-Driven Optimization](https://arxiv.org/abs/2603.24936)
*Xuepeng Jing,Wenhuan Lu,Hao Meng,Zhizhi Yu,Jianguo Wei*

Main category: cs.CV

TL;DR: The paper proposes TIGFlow-GRPO, a two-stage framework that generates human trajectories in complex environments while ensuring social compliance and physical feasibility.


<details>
  <summary>Details</summary>
Motivation: Existing trajectory forecasting methods focus on supervised fitting, which may not capture social norms and scene constraints. The authors aim to align flow-based trajectory generation with behavioral rules.

Method: The proposed framework consists of two stages: (1) a CFM-based predictor with a Trajectory-Interaction-Graph (TIG) module to model visual-spatial interactions, and (2) Flow-GRPO post-training to enable trajectory exploration and align with behavioral rules.

Result: Experiments on ETH/UCY and SDD datasets show that TIGFlow-GRPO improves forecasting accuracy, long-horizon stability, and generates socially compliant and physically feasible trajectories.
...
### [11] [Beyond Attention Magnitude: Leveraging Inter-layer Rank Consistency for Efficient Vision-Language-Action Models](https://arxiv.org/abs/2603.24941)
...
Method: TIES uses inter-layer token ranking consistency to adaptively balance attention magnitude and ensure robust token selection.

Result: TIES improves average success rates by 6% and reduces token usage by 78% on the CogACT + SIMPLER benchmark.

Conclusion: TIES demonstrates strong generalization across diverse decoders and benchmarks, providing a efficient and effective solution for Vision-Language-Action models.
...
### [12] [Bilingual Text-to-Motion Generation: A New Benchmark and Baselines](https://arxiv.org/abs/2603.25178)
...
Method: Proposed BiMD with CLA, which explicitly aligns semantic representations across languages for robust conditional space and high-quality motion generation.
...
Conclusion: The proposed dataset and method demonstrate the critical necessity and reliability of bilingual text-to-motion benchmarks and the effectiveness of cross-lingual alignment for cross-lingual motion synthesis.
...
### [13] [GDPO-Listener: Expressive Interactive Head Generation via Auto-Regressive Flow Matching and Group reward-Decoupled Policy Optimization](https://arxiv.org/abs/2603.25020)
...
<details>
  <summary>Details</summary>
Motivation: Current methods for generating 3D head motions struggle with 'Regression-to-the-Mean' problem, static faces, and lack of parameter space for complex nonverbal motions.

Method: GDPO-Listener framework, which includes an Auto-Regressive Flow Matching architecture and Group reward-Decoupled Policy Optimization (GDPO).

Result: Superior performance on long-term kinematic variance, visual expressivity, and semantic controllability compared to existing baselines.

Conclusion: GDPO-Listener achieves highly expressive speaking and listening motion generation, enabling explicit semantic text control for customizable responses.
...
### [14] [GeoNDC: A Queryable Neural Data Cube for Planetary-Scale Earth Observation](https://arxiv.org/abs/2603.25037)
*Jianbo Qi,Mengyao Li,Baogui Jiang,Yidan Chen,Qiao Wang*

Main category: cs.CV

TL;DR: A queryable neural data cube called GeoNDC is proposed for efficient storage, transmission, and querying of large Earth observation data, enabling on-demand queries and continuous-time reconstruction.
...
Method: GeoNDC encodes planetary-scale Earth observation data as a continuous spatiotemporal implicit neural field, allowing for on-demand queries and continuous-time reconstruction without full decompression.

Result: GeoNDC achieves high accuracy and compression ratios on various datasets, including MODIS, Sentinel-2, and HiGLASS biophysical products, with near-perfect accuracy (R^2 > 0.98) and high spectral fidelity.

Conclusion: GeoNDC offers a unified AI-native representation for planetary-scale Earth observation, providing a compact, analysis-ready data layer that integrates query, reconstruction, and compression in a single framework.
...
### [15] [Learning domain-invariant features through channel-level sparsification for Out-Of Distribution Generalization](https://arxiv.org/abs/2603.25083)
...
Conclusion: The proposed HCD method effectively addresses the shortcut learning problem and improves out-of-distribution generalization in image analysis systems.
...
### [16] [Pixelis: Reasoning in Pixels, from Seeing to Acting](https://arxiv.org/abs/2603.25091)
*Yunpeng Zhou*

Main category: cs.CV

TL;DR: Pixelis, a pixel-space agent, learns from images and videos through executable operations and its consequences, improving visual intelligence with actionable outcomes.


<details>
  <summary>Details</summary>
Motivation: Current vision-language systems are passive and lack generalizable, physically grounded visual intelligence, which can be achieved by learning through action rather than static description.

Method: Pixelis operates on images and videos using a compact set of executable operations and trains in three phases: Supervised Fine-Tuning, Curiosity-Coherence Reward Fine-Tuning, and Pixel Test-Time RL.

Result: Pixelis yields consistent improvements across six public benchmarks with an average relative gain of +4.08% over the baseline, producing shorter, auditable toolchains and maintaining in-corridor KL during test-time learning.

Conclusion: Pixelis enables embodied adaptation without external feedback by grounding multimodal perception in the physical world, linking visual reasoning with actionable outcomes.
...
### [17] [Few TensoRF: Enhance the Few-shot on Tensorial Radiance Fields](https://arxiv.org/abs/2603.25008)
...
Result: Few TensoRF achieves an average PSNR of 23.70 dB (24.52 dB fine-tuned) on the Synthesis NeRF benchmark and 27.37-34.00 dB on the THuman 2.0 dataset with only eight input images.

Conclusion: Few TensoRF is an efficient and data-effective solution for real-time 3D reconstruction across diverse scenes, offering improved performance over TensorRF.
...
### [18] [AirSplat: Alignment and Rating for Robust Feed-Forward 3D Gaussian Splatting](https://arxiv.org/abs/2603.25129)
...
### [19] [MoireMix: A Formula-Based Data Augmentation for Improving Image Classification Robustness](https://arxiv.org/abs/2603.25109)
...
<details>
  <summary>Details</summary>
Motivation: Existing data augmentation methods rely on diffusion-based synthesis or complex feature mixing strategies, which introduce substantial computational overhead or require external datasets.
...
### [20] [SportSkills: Physical Skill Learning from Sports Instructional Videos](https://arxiv.org/abs/2603.25163)
*Kumar Ashutosh,Chi Hsuan Wu,Kristen Grauman*

Main category: cs.CV

TL;DR: Introduced SportSkills, a large-scale sports dataset for physical skill learning with 360k+ instructional videos from 55 sports, enabling fine-grained action understanding and improving video models' ability to personalize visual instructions.


<details>
  <summary>Details</summary>
Motivation: Current large-scale video datasets lack depth in fine-grained activities necessary for physical skill learning, prompting the creation of a sports-specific dataset.

Method: Introduced SportSkills dataset with 360k+ instructional videos and 630k+ visual demonstrations, and proposed a task formulation for mistake-conditioned instructional video retrieval.

Result: Achieved gains of up to 4x in understanding fine-grained physical actions compared to traditional activity-centric datasets, and significantly advanced the ability of video models to personalize visual instructions as evaluated by professional coaches.

Conclusion: SportSkills dataset and the proposed task formulation enable better understanding and personalization of physical skills in instructional videos, with potential applications in skill learning and improvement.
...
### [21] [Knowledge-Guided Adversarial Training for Infrared Object Detection via Thermal Radiation Modeling](https://arxiv.org/abs/2603.25170)
*Shiji Zhao,Shukun Xiong,Maoxun Yuan,Yao Huang,Ranjie Duan,Qing Guo,Jiansheng Chen,Haibin Duan,Xingxing Wei*

Main category: cs.CV

TL;DR: The paper proposes a knowledge-guided adversarial training method for infrared object detection to improve its robustness against adversarial attacks and common corruptions.


<details>
  <summary>Details</summary>
Motivation: Infrared object detection is vulnerable to adversarial examples and common corruptions, posing security risks, and current methods have limited robustness.

Method: The paper proposes Knowledge-Guided Adversarial Training (KGAT), which embeds infrared physical knowledge into the adversarial training process.
...
Conclusion: The proposed KGAT method effectively improves the robustness of infrared object detection by leveraging infrared physical knowledge.
...
### [22] [CHIRP dataset: towards long-term, individual-level, behavioral monitoring of bird populations in the wild](https://arxiv.org/abs/2603.25524)
...
TL;DR: A dataset and method for individual re-identification of wild birds using computer vision are introduced, enabling long-term behavioral monitoring.


<details>
  <summary>Details</summary>
Motivation: The need for long-term behavioral monitoring of individual animals in conservation and evolutionary biology, and the lack of datasets for automated behavior monitoring in wild populations.

Method: A novel dataset (CHIRP) and a method (CORVID) for individual re-identification of wild birds using computer vision, specifically through segmentation and classification of colored leg rings.

Result: CORVID outperforms state-of-the-art re-id methods in application-specific benchmarking, and offers a probability-based id tracking method.

Conclusion: The work provides a blueprint for curating real-world datasets from biological studies to bridge the gap between computer vision research and biological applications.
...
### [23] [Demographic Fairness in Multimodal LLMs: A Benchmark of Gender and Ethnicity Bias in Face Verification](https://arxiv.org/abs/2603.25613)
...
### [24] [MACRO: Advancing Multi-Reference Image Generation with Structured Long-Context Data](https://arxiv.org/abs/2603.25319)
*Zhekai Chen,Yuqing Wang,Manyuan Zhang,Xihui Liu*

Main category: cs.CV

TL;DR: A new dataset and benchmark are introduced to improve image generation conditioned on multiple visual references, addressing the performance degradation of current models with increasing input references.


<details>
  <summary>Details</summary>
Motivation: Current models for multi-reference image generation suffer from performance degradation as the number of input references grows, due to a lack of structured, long-context supervision in existing datasets.

Method: A large-scale dataset called MacroData is introduced, containing 400K samples with up to 10 reference images, organized across four dimensions. A benchmark called MacroBench is also proposed, assessing generative coherence across task dimensions and input scales.

Result: Fine-tuning on MacroData yields substantial improvements in multi-reference generation, with ablation studies revealing synergistic benefits of cross-task co-training and effective strategies for handling long-context complexity.

Conclusion: The introduction of MacroData and MacroBench provides a comprehensive solution to address the challenges of multi-reference image generation, enabling improved performance and evaluation of generative models.
...
### [25] [TRACE: Object Motion Editing in Videos with First-Frame Trajectory Guidance](https://arxiv.org/abs/2603.25707)
*Quynh Phung,Long Mai,Cusuh Ham,Feng Liu,Jia-Bin Huang,Aniruddha Mahapatra*

Main category: cs.CV

TL;DR: The paper presents a novel approach for editing object motion paths in videos, allowing users to design a target object's trajectory in a single frame and generating a temporally consistent edited video.


<details>
  <summary>Details</summary>
Motivation: Existing video editing methods often struggle with object motion path editing, particularly in videos with camera motion, and require complex user input. The authors aim to provide a practical, easy-to-use approach for controllable object-centric motion editing.

Method: The authors propose a two-stage pipeline: (1) a cross-view motion transformation module that maps the user-designed trajectory in a single anchor frame to frame-aligned box trajectories under camera motion, and (2) a motion-conditioned video re-synthesis module that regenerates the object along these trajectories while preserving the remaining content.

Result: The proposed method, called Trace, produces more coherent, realistic, and controllable motion edits than recent image-to-video and video-to-video methods, as demonstrated by experiments on diverse real-world videos.

Conclusion: The paper concludes that the proposed Trace framework offers a practical and effective solution for object motion path editing in videos, enabling users to design and synthesize temporally consistent edited videos with ease.
...
### [26] [PixelSmile: Toward Fine-Grained Facial Expression Editing](https://arxiv.org/abs/2603.25728)
*Jiabin Hua,Hengyuan Xu,Aojie Li,Wei Cheng,Gang Yu,Xingjun Ma,Yu-Gang Jiang*

Main category: cs.CV

TL;DR: A new method called PixelSmile is proposed for fine-grained facial expression editing, achieving precise control and robust identity preservation.


<details>
  <summary>Details</summary>
Motivation: Fine-grained facial expression editing has been limited by intrinsic semantic overlap, and the authors aim to address this challenge.
...
Result: PixelSmile achieves superior disentanglement and robust identity preservation, supporting continuous, controllable, and fine-grained expression editing.

Conclusion: The proposed PixelSmile method effectively addresses the challenge of fine-grained facial expression editing, enabling precise and stable linear expression control.
...
### [27] [Less Gaussians, Texture More: 4K Feed-Forward Textured Splatting](https://arxiv.org/abs/2603.25745)
...
Method: LGTM predicts compact Gaussian primitives coupled with per-primitive textures, allowing for efficient rendering at high resolutions.

Result: LGTM achieves high-fidelity 4K novel view synthesis without per-scene optimization, using significantly fewer Gaussian primitives than existing methods.

Conclusion: LGTM overcomes the resolution scaling barrier of existing feed-forward 3D Gaussian Splatting methods, enabling efficient and high-quality rendering at high resolutions.
...
### [28] [ShotStream: Streaming Multi-Shot Video Generation for Interactive Storytelling](https://arxiv.org/abs/2603.25746)
*Yawen Luo,Xiaoyu Shi,Junhao Zhuang,Yutian Chen,Quande Liu,Xintao Wang,Pengfei Wan,Tianfan Xue*

Main category: cs.CV

TL;DR: Proposed ShotStream, a novel causal multi-shot architecture for interactive storytelling and efficient video generation.


<details>
  <summary>Details</summary>
Motivation: Current bidirectional architectures for multi-shot video generation have limited interactivity and high latency.

Method: Introduced a dual-cache memory mechanism and a two-stage distillation strategy to improve inter-shot consistency and mitigate error accumulation.
...
Conclusion: ShotStream enables real-time interactive storytelling, paving the way for future applications.
...
### [29] [When Consistency Becomes Bias: Interviewer Effects in Semi-Structured Clinical Interviews](https://arxiv.org/abs/2603.24651)
...
TL;DR: The study analyzes the bias in depression detection models trained on doctor-patient conversations, finding that models exploit interviewer prompts to achieve high performance, rather than relying on participant language.
...
Conclusion: The study concludes that including interviewer prompts in depression detection models inflates performance by leveraging script artifacts, emphasizing the need for analyses that localize decision evidence by time and speaker to ensure models learn from participants' language.
...
### [30] [Prune as You Generate: Online Rollout Pruning for Faster and Better RLVR](https://arxiv.org/abs/2603.24840)
*Haobo Xu,Sirui Chen,Ruizhong Qiu,Yuchen Yan,Chen Luo,Monica Cheng,Jingrui He,Hanghang Tong*

Main category: cs.CL

TL;DR: The paper introduces arrol, an online rollout pruning method that accelerates Reinforcement Learning with Verifiable Rewards (RLVR) for Large Language Models (LLMs) by pruning inefficient rollouts and enhancing learning signals, achieving improved accuracy and training speedup.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the substantial computational cost and weak learning signals in existing RLVR methods, such as GRPO and DAPO, which rely on sampling many rollouts for each prompt and often result in sparse relative advantages.

Method: The method introduced is arrol (Accelerating RLVR via online Rollout Pruning), which prunes rollouts during generation, steers surviving ones to be more correctness-balanced, and uses a lightweight quality head to predict success probability and make early pruning decisions.

Result: The results show that arrol improves average accuracy by +2.30 to +2.99, achieves up to 1.7x training speedup, and yields up to +8.33 additional gains in average accuracy in test-time scaling across GRPO and DAPO on Qwen-3 and LLaMA-3.2 models (1B-8B).

Conclusion: The conclusion is that arrol effectively accelerates RLVR and enhances the reasoning capabilities of LLMs by improving the efficiency and efficacy of rollout sampling, leading to better accuracy and efficiency in training and testing.
...
### [31] [LogSigma at SemEval-2026 Task 3: Uncertainty-Weighted Multitask Learning for Dimensional Aspect-Based Sentiment Analysis](https://arxiv.org/abs/2603.24896)
*Baraa Hikal,Jonas Becker,Bela Gipp*

Main category: cs.CL

TL;DR: The paper presents LogSigma, a system for Dimensional Aspect-Based Sentiment Analysis (DimABSA) that predicts continuous Valence and Arousal scores, achieving 1st place on five datasets across two tracks.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of predicting continuous Valence and Arousal scores in DimABSA, which differs in difficulty across languages and domains.

Method: The method uses learned homoscedastic uncertainty with task-specific log-variance parameters, language-specific encoders, and multi-seed ensembling.
...
Conclusion: The paper concludes that optimal task balancing in DimABSA is language-dependent and cannot be determined a priori, and that learned homoscedastic uncertainty can effectively address the challenge of differing prediction difficulties across languages and domains.
...
### [32] [Estimating near-verbatim extraction risk in language models with decoding-constrained beam search](https://arxiv.org/abs/2603.24917)
...
TL;DR: The authors propose a method called decoding-constrained beam search to efficiently estimate near-verbatim extraction risk in LLMs, which is a more comprehensive measure of memorization than verbatim extraction.


<details>
  <summary>Details</summary>
Motivation: Standard greedy-decoding extraction methods for quantifying memorization in LLMs are limited, and probabilistic extraction is only tractable for verbatim memorization, missing near-verbatim instances that pose similar risks.

Method: The authors introduce decoding-constrained beam search, which yields deterministic lower bounds on near-verbatim extraction risk at a lower cost compared to Monte Carlo estimation.

Result: The approach surfaces more extractable sequences, larger per-sequence extraction mass, and patterns in near-verbatim extraction risk across model sizes and text types, which were invisible to verbatim methods.

Conclusion: The proposed method provides a more comprehensive understanding of memorization in LLMs, highlighting the importance of considering near-verbatim extraction risk in addition to verbatim extraction.
...
### [33] [Toward domain-specific machine translation and quality estimation systems](https://arxiv.org/abs/2603.24955)
*Javad Pourmostafa Roshan Sharami*

Main category: cs.CL

TL;DR: The dissertation focuses on adapting Machine Translation (MT) and Quality Estimation (QE) systems to specialized domains through data-focused contributions, improving performance and efficiency.


<details>
  <summary>Details</summary>
Motivation: Machine Translation (MT) and Quality Estimation (QE) systems perform poorly in specialized domains due to domain mismatch, highlighting the need for domain adaptation.

Method: The dissertation presents several methods, including similarity-based data selection for MT, a staged QE training pipeline, aligned tokenization-vocabulary setups for fine-tuning, and QE-guided in-context learning for large language models.

Result: The proposed methods show improved performance across domains, languages, and resource settings, including zero-shot and cross-lingual cases, with efficient adaptation strategies.

Conclusion: The dissertation provides effective methods for building reliable MT and QE systems in domain-specific settings, emphasizing the importance of data selection, representation, and efficient adaptation strategies.
...
### [34] [Exons-Detect: Identifying and Amplifying Exonic Tokens via Hidden-State Discrepancy for Robust AI-Generated Text Detection](https://arxiv.org/abs/2603.24981)
*Xiaowei Zhu,Yubing Ren,Fang Fang,Shi Wang,Yanan Cao,Li Guo*

Main category: cs.CL

TL;DR: Proposed Exons-Detect, a training-free method for detecting AI-generated text by reweighting tokens based on their informative exonic content.


<details>
  <summary>Details</summary>
Motivation: The increasing difficulty in distinguishing human-written from AI-generated text poses societal risks, necessitating reliable detection methods.

Method: Exons-Detect uses a dual-model setting to identify and amplify informative exonic tokens, computing a translation score from the reweighted token sequence.

Result: Exons-Detect achieves state-of-the-art detection performance with a 2.2% relative improvement in average AUROC over the strongest prior baseline.

Conclusion: Exons-Detect offers a robust and interpretable solution for AI-generated text detection, resistant to adversarial attacks and varying input lengths.
...
### [35] [Do LLMs Know What They Know? Measuring Metacognitive Efficiency with Signal Detection Theory](https://arxiv.org/abs/2603.25112)
*Jon-Paul Cacioli*

Main category: cs.CL

TL;DR: The paper introduces a new evaluation framework for assessing LLM confidence, distinguishing between a model's knowledge and its ability to recognize what it knows. It finds significant variations in metacognitive efficiency across models and domains.


<details>
  <summary>Details</summary>
Motivation: Current evaluation metrics for LLM confidence conflate two distinct capacities: knowledge and metacognitive sensitivity. The authors aim to decompose these capacities using a Type-2 Signal Detection Theory framework.

Method: The authors apply a Type-2 Signal Detection Theory framework, using meta-d' and the metacognitive efficiency ratio M-ratio, to evaluate four LLMs across 224,000 factual QA trials.

Result: The results show substantial variations in metacognitive efficiency across models and domains, with different models performing differently in various domains. The findings also highlight the distinction between confidence policy and metacognitive capacity.

Conclusion: The meta-d' framework provides a more nuanced understanding of LLM confidence, distinguishing between models that 'know what they don't know' and those that appear well-calibrated due to criterion placement, with implications for model selection, deployment, and human-AI collaboration.
...
### [36] [Goodness-of-pronunciation without phoneme time alignment](https://arxiv.org/abs/2603.25150)
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to expand speech evaluation to low-resource languages, where ASR training data is limited, by utilizing open-source weakly-supervised models.

Method: The method involves computing phoneme posteriors by mapping ASR hypotheses to a phoneme confusion network, using word-level speaking rate and duration, and combining phoneme and frame-level features using a cross-attention architecture.
...
Conclusion: The paper concludes that the proposed approach can overcome incompatibilities for feature extraction with weakly-supervised models, easing the expansion of speech evaluation to low-resource languages.
...
### [37] [To Write or to Automate Linguistic Prompts, That Is the Question](https://arxiv.org/abs/2603.25169)
*Marina Sánchez-Torrón,Daria Akselrod,Jason Rauchwerk*

Main category: cs.CL

TL;DR: The paper compares automatic prompt optimization with expert prompt engineering in linguistic tasks, finding task-dependent results with no clear winner.
...
Method: Systematic comparison of hand-crafted expert prompts, base DSPy signatures, and GEPA-optimized DSPy signatures across three linguistic tasks.

Result: Results vary by task: in terminology insertion, optimized and manual prompts are similar; in translation, each approach wins on different models; in LQA, expert prompts detect errors better while optimization improves characterization.

Conclusion: Automatic prompt optimization can be a viable alternative to expert prompt engineering, but results depend on the specific task.
...
### [38] [Prompt Attack Detection with LLM-as-a-Judge and Mixture-of-Models](https://arxiv.org/abs/2603.25176)
*Hieu Xuan Le,Benjamin Goh,Quy Anh Tang*

Main category: cs.CL

TL;DR: The paper explores using lightweight LLMs as security judges to mitigate prompt attacks on LLM systems, achieving effective low-latency guardrails.
...
Conclusion: Lightweight LLMs can reliably serve as security judges under real-world production constraints, and are currently deployed in production.
...
### [39] [Cross-Preference Learning for Sentence-Level and Context-Aware Machine Translation](https://arxiv.org/abs/2603.25183)
...
<details>
  <summary>Details</summary>
Motivation: Existing training objectives for context-aware machine translation do not explicitly model the variability of contextual signals across sentences, limiting a model's ability to adaptively exploit context.

Method: The proposed approach, Cross-Preference Learning (CPL), uses a preference-based training framework that integrates both intra- and cross-condition preferences into the preference optimization objective.

Result: Experimental results demonstrate consistent improvements in translation quality and robustness across both input conditions, achieved without any architectural modifications, on several public context-aware MT tasks using multiple models.

Conclusion: The proposed Cross-Preference Learning (CPL) approach effectively captures the complementary benefits of sentence-level and context-aware machine translation, leading to improved translation quality and robustness.
...
### [40] [Probing the Lack of Stable Internal Beliefs in LLMs](https://arxiv.org/abs/2603.25187)
*Yifan Luo,Kangping Xu,Yanzhen Lu,Yang Yuan,Andrew Chi-Chih Yao*

Main category: cs.CL

TL;DR: LLMs struggle to maintain implicit consistency in multi-turn interactions, lacking stable internal representations that anchor their responses over time.


<details>
  <summary>Details</summary>
Motivation: The need for LLMs to simulate human-like personality traits, such as persistence or reliability, by maintaining consistent behavioral tendencies across interactions.

Method: A 20-question-style riddle game paradigm where an LLM is tasked with secretly selecting a target and responding to users' guesses with 'yes/no' answers.

Result: LLMs struggle to preserve latent consistency, with their implicit 'goals' shifting across turns unless explicitly provided their selected target in context.

Conclusion: The findings highlight critical limitations in the building of persona-driven LLMs and underscore the need for mechanisms that anchor implicit goals over time for realistic personality modeling in interactive applications.
...
### [41] [A Catalog of Basque Dialectal Resources: Online Collections and Standard-to-Dialectal Adaptations](https://arxiv.org/abs/2603.25189)
*Jaione Bengoetxea,Itziar Gonzalez-Dios,Rodrigo Agerri*

Main category: cs.CL

TL;DR: The paper presents a catalog of Basque dialectal data and resources to address data scarcity in dialectal NLP, distinguishing between original dialectal data and standard-to-dialect adapted data.


<details>
  <summary>Details</summary>
Motivation: The primary limitation in dialectal NLP is data scarcity, which this paper aims to address by compiling a comprehensive catalog of Basque dialectal data and resources.

Method: The paper distinguishes between two types of data sources: online data originally written in some dialect and standard-to-dialect adapted data, with manual and automatic adaptation methods explored.

Result: The paper yields a high-quality parallel gold standard evaluation dataset through manual adaptation of the XNLI dataset into three Basque dialects and assesses the quality of automatically adapted physical commonsense dataset (BasPhyCowest).

Conclusion: The paper provides a systematic and comprehensive compilation of Basque dialectal data, which could serve as a viable substitute for full manual adaptation in dialectal NLP.
...
### [42] [Adaptive Chunking: Optimizing Chunking-Method Selection for RAG](https://arxiv.org/abs/2603.25333)
...
<details>
  <summary>Details</summary>
Motivation: Current 'one-size-fits-all' chunking approaches often fail to capture the nuanced structure and semantics of diverse texts, and there lacks a dedicated evaluation framework for chunking.

Method: Introduced a framework called Adaptive Chunking, which uses five novel intrinsic metrics (RC, ICC, DCC, BI, SC) to select the most suitable chunking strategy for each document, and two new chunkers (LLM-regex splitter and split-then-merge recursive splitter).

Result: Adaptive Chunking significantly improves downstream RAG performance, increasing answer correctness to 72% (from 62-64%) and the number of successfully answered questions by over 30% (65 vs. 49).
...
### [43] [Closing the Confidence-Faithfulness Gap in Large Language Models](https://arxiv.org/abs/2603.25052)
*Miranda Muqing Miao,Lyle Ungar*

Main category: cs.CL

TL;DR: This paper analyzes how large language models express confidence in their answers, finding that their verbalized confidence and actual accuracy are not aligned.


<details>
  <summary>Details</summary>
Motivation: The tendency of large language models to express confidence scores that do not match their actual accuracy.

Method: Using linear probes and contrastive activation addition (CAA) steering to analyze the geometric relationship between calibration and verbalized confidence signals.

Result: The study found that calibration and verbalized confidence signals are encoded linearly but are orthogonal to one another, and introduced a two-stage adaptive steering pipeline to improve calibration alignment.

Conclusion: The study reveals the 'Reasoning Contamination Effect' where reasoning processes disrupt verbalized confidence, and proposes a method to improve the alignment between verbalized confidence and actual accuracy.
...
### [44] [MolQuest: A Benchmark for Agentic Evaluation of Abductive Reasoning in Chemical Structure Elucidation](https://arxiv.org/abs/2603.25253)
*Taolin Han,Shuang Wu,Jinghang Wang,Yuhao Zhou,Renquan Lv,Bing Zhao,Wei Hu*

Main category: cs.CL

TL;DR: The paper introduces MolQuest, a new framework for evaluating large language models (LLMs) in molecular structure elucidation, revealing limitations in current LLMs' strategic scientific reasoning.


<details>
  <summary>Details</summary>
Motivation: Current scientific evaluation benchmarks are inadequate for measuring model performance in complex scientific tasks that require multi-step iteration and experimental interaction.
...
Result: Empirical results show that contemporary frontier models exhibit significant limitations, with even state-of-the-art models achieving an accuracy of only approximately 50%, and most other models below 30%.

Conclusion: The work highlights the critical gap in current LLMs' strategic scientific reasoning, setting a clear direction for future research toward AI that can actively participate in the scientific process.
...
<details>
  <summary>Details</summary>
Motivation: Agent performance depends on harness engineering, but harness design is typically buried in controller code and runtime-specific conventions, hindering its study and transfer.

Method: The authors propose NLAHs, which express harness behavior in natural language, and IHR, a shared runtime that executes these harnesses through explicit contracts, durable artifacts, and lightweight adapters.

Result: The authors conducted controlled evaluations across coding and computer-use benchmarks, assessing operational viability, module ablation, and code-to-text harness migration.

Conclusion: The paper presents a novel approach to externalizing agent control logic, making it more portable, comparable, and studyable.
...
### [46] [AutoSAM: an Agentic Framework for Automating Input File Generation for the SAM Code with Multi-Modal Retrieval-Augmented Generation](https://arxiv.org/abs/2603.24736)
*Zaid Abulawi,Zavier Ndum Ndum,Eric Cervi,Rui Hu,Yang Liu*

Main category: cs.AI

TL;DR: The paper presents AutoSAM, a framework that automates the generation of input files for the System Analysis Module (SAM) thermal-hydraulics code, reducing the labor-intensive task of manually translating design data into solver-specific syntax.
...
Method: AutoSAM combines a large language model agent with retrieval-augmented generation over the solver's user guide and theory manual, and specialized tools for analyzing PDFs, images, spreadsheets, and text files.

Result: The framework achieves 100% utilization of structured inputs, about 88% extraction from PDF text, and 100% completeness in vision-based geometric extraction, producing runnable SAM models consistent with expected thermal-hydraulic behavior.

Conclusion: The results demonstrate a practical path toward prompt-driven reactor modeling, in which analysts provide system descriptions and supporting documentation while the agent translates them into transparent, and executable, SAM simulations.
...
### [47] [Trust as Monitoring: Evolutionary Dynamics of User Trust and AI Developer Behaviour](https://arxiv.org/abs/2603.24742)
...
TL;DR: The paper models AI safety as a dynamic process of trust and interaction between users and developers, finding that safe and widely adopted AI systems require transparency, low-cost monitoring, and meaningful sanctions.


<details>
  <summary>Details</summary>
Motivation: The increasing urgency of AI safety concerns and the limitations of existing models in capturing the dynamic nature of user trust.

Method: Evolutionary game theory, infinite-population replicator analysis, stochastic finite-population dynamics, and reinforcement learning (Q-learning) simulations.

Result: Three robust long-run regimes: no adoption with unsafe development, unsafe but widely adopted systems, and safe systems that are widely adopted. The desirable outcome of safe and widely adopted systems requires penalties for unsafe behavior to exceed the extra cost of safety and occasional user monitoring.

Conclusion: The paper supports governance proposals emphasizing transparency, low-cost monitoring, and meaningful sanctions, and highlights the insufficiency of regulation alone or blind user trust in preventing unsafe AI outcomes.
...
### [48] [Formal Semantics for Agentic Tool Protocols: A Process Calculus Approach](https://arxiv.org/abs/2603.24747)
*Andreas Schlapbach*

Main category: cs.AI

TL;DR: The paper formally analyzes and compares Schema-Guided Dialogue (SGD) and Model Context Protocol (MCP) for large language model agents, providing a process calculus formalization and identifying necessary conditions for full behavioral equivalence.
...
Method: The authors present a process calculus formalization of SGD and MCP, proving they are structurally bisimilar under a well-defined mapping Phi, and identify necessary and sufficient conditions for full behavioral equivalence.

Result: The authors demonstrate that MCP is not fully expressive and identify five principles (semantic completeness, explicit action boundaries, failure mode documentation, progressive disclosure compatibility, and inter-tool relationship declaration) as necessary and sufficient conditions for full behavioral equivalence.

Conclusion: The paper provides the first formal foundation for verified agent systems and establishes schema quality as a provable safety property.
...
### [49] [Supervising Ralph Wiggum: Exploring a Metacognitive Co-Regulation Agentic AI Loop for Engineering Design](https://arxiv.org/abs/2603.24768)
*Zeda Xu,Nikolas Martelaro,Christopher McComb*

Main category: cs.AI

TL;DR: Researchers propose two novel systems, Self-Regulation Loop (SRL) and Co-Regulation Design Agentic Loop (CRDAL), to mitigate design fixation in AI agents used for engineering design, and find that CRDAL outperforms existing methods in generating better designs without increased computational cost.


<details>
  <summary>Details</summary>
Motivation: The engineering design research community faces challenges with agentic AI systems that use Large Language Model (LLM) agents, as they can suffer from design fixation, similar to human designers, leading to suboptimal solutions.

Method: The authors propose two novel systems: (1) Self-Regulation Loop (SRL), where the Design Agent self-regulates its metacognition, and (2) Co-Regulation Design Agentic Loop (CRDAL), where a Metacognitive Co-Regulation Agent assists the Design Agent in metacognition.

Result: The CRDAL system generates designs with better performance without significantly increasing computational cost, compared to SRL and a plain Ralph Wiggum Loop (RWL). CRDAL navigates the latent design space more effectively than SRL and RWL.

Conclusion: The proposed system architectures and findings provide practical implications for the future development of agentic AI systems for engineering design, highlighting the potential of CRDAL to improve system performance.
...
TL;DR: The paper introduces two methods, Attention Probe and KL-Regularized LoRA Probe (ReLope), to improve probe routing in multimodal large language models (MLLMs) by enhancing the quality of hidden states.


<details>
  <summary>Details</summary>
Motivation: Probe routing, which uses hidden states to predict the correctness of a small model, is effective in text-only LLMs but degrades in MLLMs due to the presence of visual inputs weakening the separability of correctness signals.

Method: Two approaches are proposed: Attention Probe, which aggregates hidden states based on attention scores, and ReLope, which inserts a lightweight LoRA adapter with a KL regularizer to learn routing-aware representations.

Result: Comprehensive experiments show that the proposed methods consistently outperform baselines, indicating that improving hidden state quality is key to effective routing in MLLMs.

Conclusion: The paper concludes that enhancing the quality of hidden states is crucial for effective probe routing in MLLMs, and that the proposed methods, Attention Probe and ReLope, are effective solutions.
...
### [51] [On the Foundations of Trustworthy Artificial Intelligence](https://arxiv.org/abs/2603.24904)
*TJ Dunham*

Main category: cs.AI

TL;DR: The paper proves that platform-deterministic inference is necessary and sufficient for trustworthy AI, and proposes a system to achieve this.


<details>
  <summary>Details</summary>
Motivation: The authors aim to ensure trustworthy AI by addressing the issue of non-determinism in AI systems, which is a prerequisite for fairness, robustness, privacy, safety, and alignment.

Method: The authors formalize the Determinism Thesis, introduce trust entropy, and prove the Determinism-Verification Collapse. They also construct a pure integer inference engine to achieve bitwise identical output across different architectures.

Result: The authors' system, implemented in 99,000 lines of Rust, achieves zero hash mismatches in 82 cross-architecture tests on models up to 6.7B parameters. Four geographically distributed nodes produce identical outputs, verified by 356 on-chain attestation transactions.

Conclusion: The paper concludes that AI trust is a question of arithmetic and that platform determinism is necessary and sufficient for trustworthy AI.
...
### [52] [FinMCP-Bench: Benchmarking LLM Agents for Real-World Financial Tool Use under the Model Context Protocol](https://arxiv.org/abs/2603.24943)
...
<details>
  <summary>Details</summary>
Motivation: To provide a standardized testbed for advancing research on financial LLM agents.

Method: Systematically assessing mainstream LLMs using FinMCP-Bench and proposing metrics for tool invocation accuracy and reasoning capabilities.

Result: FinMCP-Bench provides a practical and challenging testbed for evaluating LLMs in financial scenarios.

Conclusion: FinMCP-Bench is a novel benchmark that enables the evaluation of LLMs in solving real-world financial problems.
...
TL;DR: The paper proposes UniAI-GraphRAG, an enhanced framework for Retrieval-Augmented Generation systems, addressing challenges in complex reasoning and domain-specific QA.


<details>
  <summary>Details</summary>
Motivation: Existing GraphRAG frameworks have limitations in cross-industry adaptability, community report integrity, and retrieval performance.
...
Result: UniAI-GraphRAG outperforms mainstream open source solutions in comprehensive F1 scores, particularly in inference and temporal queries.

Conclusion: The proposed UniAI-GraphRAG framework shows improved performance in complex reasoning and domain-specific QA, making it a promising solution for Retrieval-Augmented Generation systems.
...
TL;DR: Introduced Trace2Skill, a framework for generating domain-specific skills for Large Language Models (LLMs) by analyzing diverse execution experiences and distilling them into comprehensive guides.


<details>
  <summary>Details</summary>
Motivation: Manual authoring of domain-specific skills for LLMs is not scalable, while automated generation often results in fragile or fragmented skills due to reliance on shallow knowledge or overfitting to specific trajectories.

Method: Trace2Skill uses a fleet of sub-agents to analyze diverse executions in parallel, extracting lessons and consolidating them into a unified, conflict-free skill directory through inductive reasoning.

Result: Trace2Skill significantly improves over strong baselines in challenging domains like spreadsheets, VisionQA, and math reasoning, with skills transferring across LLM scales and generalizing to out-of-distribution settings.

Conclusion: Complex agent experience can be packaged into highly transferable, declarative skills without requiring parameter updates, external retrieval modules, or large models, enabling the creation of scalable and generalizable domain-specific skills for LLMs.
...
### [55] [Distribution and Clusters Approximations as Abstract Domains in Probabilistic Abstract Interpretation to Neural Network Analysis](https://arxiv.org/abs/2603.25273)
*Zhuofan Zhang,Herbert Wiklicky*

Main category: cs.AI

TL;DR: The paper introduces two novel approximation methods for neural network analysis: distribution approximation and clusters approximation.


<details>
  <summary>Details</summary>
Motivation: The probabilistic abstract interpretation framework for neural network analysis requires efficient approximation methods to analyze the density distribution flow of all possible inputs.

Method: The paper proposes two new methods: distribution approximation and clusters approximation, which are used as abstract domains to approximate the concrete space.

Result: The paper theoretically demonstrates how the two methods work, along with their corresponding abstract transformers, using simple examples.

Conclusion: The introduction of distribution approximation and clusters approximation methods enhances the probabilistic abstract interpretation framework for neural network analysis.
...
TL;DR: A framework called DAGverse is proposed for constructing document-grounded semantic Directed Acyclic Graphs (DAGs) from online scientific papers, leveraging papers with explicit DAG figures as supervision.
...
<details>
  <summary>Details</summary>
Motivation: Existing educational NLP and multimodal large language models are not well-suited to handle the complexity and multimodality of handwritten scratchwork, particularly in diagnosing student errors.

Method: The authors create a dataset of 1,720 mathematics samples from primary and middle school students and evaluate 16 leading multimodal large language models on two tasks: Error Cause Explanation and Error Cause Classification.

Result: The evaluation reveals significant performance gaps between the models and human experts, especially in visual recognition and logical reasoning, with proprietary models outperforming open-source models.

Conclusion: The paper highlights the need for improved multimodal models that can accurately diagnose student errors in handwritten math scratchwork and provides a publicly available benchmark and evaluation framework for future research.
...
<details>
  <summary>Details</summary>
Motivation: Current ASR evaluations do not systematically cover real-world conditions, leading to performance degradation in voice agents, and there is a need for diagnostic tools to identify specific failure factors.

Method: The authors introduce a multilingual (four-language) diagnostic benchmark, WildASR, which evaluates ASR robustness along three axes: environmental degradation, demographic shift, and linguistic diversity.

Result: The evaluation of seven widely used ASR systems shows severe and uneven performance degradation, with model robustness not transferring across languages or conditions, and a tendency for models to hallucinate under partial or degraded inputs.

Conclusion: The paper emphasizes the importance of targeted, factor-isolated evaluation for understanding and improving ASR reliability in production systems, and provides WildASR and three analytical tools for practitioners to guide deployment decisions.
...
TL;DR: The paper introduces DreamHouse, a benchmark for evaluating vision-language models (VLMs) on physical generative reasoning, focusing on synthesizing artifacts that satisfy structural, constructability, and code-compliance constraints.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks for VLMs prioritize visual realism over physical validity, neglecting the importance of grasping step-by-step processes and physical dependencies required for automating design-to-construction pipelines.

Method: The authors introduce a novel benchmark, DreamHouse, which is grounded in residential timber-frame construction and features over 26,000 structures spanning 13 architectural styles. They develop a deterministic 10-test structural validation framework and support iterative agentic interaction.

Result: Extensive experiments with state-of-the-art VLMs reveal substantial capability gaps in physical generative reasoning, highlighting physical validity as a critical evaluation axis orthogonal to visual realism.

Conclusion: The paper establishes physical generative reasoning as a distinct and underdeveloped frontier in multimodal intelligence, emphasizing the need for VLMs to grasp physical constraints and dependencies.
...
<details>
  <summary>Details</summary>
Motivation: Current multimodal models often yield contradictory predictions for visual and textual representations of the same concept, and standard voting mechanisms can amplify systematic biases.
