...
TL;DR: A multimodal approach for detecting hate and threats in digital forensic investigations, combining text and image analysis.


<details>
  <summary>Details</summary>
Motivation: Existing automated approaches for hate and threat detection often assume clean text input or apply vision models without forensic justification, which can be problematic in digital forensic investigations involving heterogeneous evidence.

Method: A case-driven multimodal framework that selectively applies text analysis, multimodal fusion, or image-only semantic reasoning based on the identified evidence configuration.

Result: Experimental evaluation on forensic-style image evidence demonstrates consistent and interpretable behavior across heterogeneous evidence scenarios.

Conclusion: The proposed framework improves evidentiary traceability and avoids unjustified modality assumptions, mirroring forensic decision-making.
...
TL;DR: A semi-automated framework for transforming 2D miniatures from medieval manuscripts into 3D digital models for extended reality, tactile 3D printing, and web-based visualization.


<details>
  <summary>Details</summary>
Motivation: To enable the transformation of 2D miniatures from medieval manuscripts into 3D digital models for various applications, including extended reality, tactile 3D printing, and web-based visualization.

Method: The framework combines SAM segmentation, Hi3DGen mesh generation, expert refinement in ZBrush, and AI-assisted texturing. Seven image-to-3D methods were evaluated on 69 manuscript figures using rendering-based metrics and volumetric measures.

Result: The framework successfully generated 3D digital models from 2D miniatures, with Hi3DGen balancing topological quality and surface detail. The resulting models support WebXR visualization, AR overlay, and tactile 3D prints.

Conclusion: The proposed framework provides a good starting point for transforming 2D miniatures into 3D digital models, with potential applications in extended reality, tactile 3D printing, and web-based visualization.
...
TL;DR: The authors present ViSAGE, a multi-expert ensemble framework for video saliency prediction, which outperforms most competing solutions in the NTIRE 2026 Challenge.


<details>
  <summary>Details</summary>
Motivation: To exploit complementary inductive biases for video saliency prediction by aggregating diverse expert models.

Method: ViSAGE, a multi-expert ensemble framework with adaptive gated experts, which performs adaptive gating and modulation to refine spatio-temporal features.

Result: ViSAGE ranked first on two out of four evaluation metrics and outperformed most competing solutions on the other two metrics on the Private Test set.

Conclusion: The proposed ViSAGE framework demonstrates its effectiveness and generalization ability for video saliency prediction.
...
TL;DR: Introduced MARINER, a benchmark for fine-grained visual understanding in maritime environments, and evaluated its performance on multimodal large language models.


<details>
  <summary>Details</summary>
Motivation: The lack of dedicated benchmarks for fine-grained visual understanding and high-level reasoning in real-world open-water environments.

Method: Introduced the Entity-Environment-Event (3E) paradigm and created the MARINER benchmark with 16,629 multi-source maritime images.

Result: Even advanced multimodal large language models struggle with fine-grained discrimination and causal reasoning in complex marine scenes.

Conclusion: MARINER fills the gap in realistic and cognitive-level evaluation for maritime multimodal understanding and promotes future research on robust vision-language models for open-water applications.
...
TL;DR: The paper introduces WildDet3D, a unified geometry-aware architecture for monocular 3D object detection that accepts various prompts and incorporates auxiliary depth signals. It also presents WildDet3D-Data, a large open 3D detection dataset. WildDet3D achieves state-of-the-art results across multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: The goal is to improve monocular 3D object detection by addressing the limitations of existing methods, which are restricted to a single prompt type and lack a mechanism to incorporate geometric cues. The authors also aim to create a large-scale dataset for open-world transfer.

Method: The authors propose a unified geometry-aware architecture called WildDet3D, which natively accepts text, point, and box prompts and can incorporate auxiliary depth signals at inference time. They also introduce WildDet3D-Data, a large open 3D detection dataset constructed by generating candidate 3D boxes from existing 2D annotations.

Result: WildDet3D establishes a new state-of-the-art across multiple benchmarks and settings, achieving results such as 22.6/24.8 AP3D on WildDet3D-Bench and 34.2/36.4 AP3D on Omni3D. Incorporating depth cues at inference time yields substantial additional gains (+20.7 AP on average across settings).

Conclusion: The paper presents a significant advancement in monocular 3D object detection by introducing a unified geometry-aware architecture and a large open 3D detection dataset. The results demonstrate the effectiveness of WildDet3D in achieving state-of-the-art performance across various benchmarks and settings.
...
TL;DR: The paper proposes SemJudge, a new evaluator for Generative Art (GenArt) that assesses the deeper symbolic and abstract meaning intended by the creator, beyond surface-level image quality.


<details>
  <summary>Details</summary>
Motivation: Current GenArt evaluators focus on surface-level image quality or literal prompt adherence, neglecting the deeper symbolic or abstract meaning intended by the creator.

Method: The authors formalize a Peircean computational semiotic theory and propose SemJudge, which uses a Hierarchical Semiosis Graph (HSG) to reconstruct the meaning-making process from prompt to generated artifact.

Result: SemJudge aligns more closely with human judgments than prior evaluators on an interpretation-intensive fine-art benchmark and produces deeper, more insightful artistic interpretations.

Conclusion: SemJudge paves the way for GenArt to move beyond generating 'pretty' images toward a medium capable of expressing complex human experience.
...
TL;DR: The paper introduces 3D-VCD, a visual contrastive decoding framework for mitigating hallucinations in 3D embodied agents. It constructs a distorted 3D scene graph and contrasts predictions to suppress tokens driven by language priors, improving grounded reasoning.


<details>
  <summary>Details</summary>
Motivation: Large multimodal models used in 3D environments are prone to hallucinations, leading to unsafe decisions. Existing hallucination mitigation methods for 2D settings do not transfer to 3D embodied agents, where failures arise from object presence, spatial layout, and geometric grounding.

Method: The authors propose 3D-VCD, which constructs a distorted 3D scene graph by applying semantic and geometric perturbations to object-centric representations. It then contrasts predictions under the original and distorted 3D contexts to suppress tokens insensitive to grounded scene evidence.

Result: The authors evaluate 3D-VCD on the 3D-POPE and HEAL benchmarks, showing that it consistently improves grounded reasoning without retraining. This establishes inference-time contrastive decoding over structured 3D representations as an effective and practical route to more reliable embodied intelligence.

Conclusion: The paper concludes that 3D-VCD is an effective method for mitigating hallucinations in 3D embodied agents, improving grounded reasoning and reliability.
...
TL;DR: InsEdit, an instruction-based video editing model, achieves state-of-the-art results with minimal video editing data by leveraging a novel data pipeline and visual editing architecture.


<details>
  <summary>Details</summary>
Motivation: The scarcity of high-quality video editing data and the need for a data-efficient approach to adapt video generation models for editing tasks.

Method: InsEdit combines a visual editing architecture with a video data pipeline based on Mutual Context Attention (MCA) to create aligned video pairs for editing.

Result: InsEdit achieves state-of-the-art results among open-source methods on video instruction editing benchmarks with only O(100)K video editing data and also supports image editing without modification.

Conclusion: A video generation backbone can become a strong video editor without requiring large-scale video editing data, enabled by a novel data pipeline and visual editing architecture.
...
TL;DR: A lightweight sign language recognizer called EfficientSign was developed, achieving high accuracy on Indian Sign Language alphabets with fewer parameters than other models.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this work was to build a sign language recognizer that works on a phone, which led to the development of a lightweight model.

Method: The authors used EfficientNet-B0 and focused on two attention modules (Squeeze-and-Excitation and spatial attention layer) to develop EfficientSign. They also experimented with feeding deep features into classical classifiers like SVM, Logistic Regression, and KNN.

Result: EfficientSign achieved an accuracy of 99.94% on 12,637 images of Indian Sign Language alphabets, outperforming other models with fewer parameters. Classical classifiers also achieved high accuracy: SVM (99.63%), Logistic Regression (99.03%), and KNN (96.33%).

Conclusion: The results show that attention-enhanced learning models provide efficient and deployable solutions for ISL recognition without requiring massive models or hand-tuned feature pipelines.
...
TL;DR: Introduced UMUI, a multimodal uncertain inference task, and CLUE, a model that produces calibrated probability estimates across text, audio, and video modalities.


<details>
  <summary>Details</summary>
Motivation: Extend uncertain inference to multimodal settings, beyond text, to enable fine-grained probabilistic reasoning across modalities.

Method: Proposed CLUE, which combines self-consistent teacher calibration and distribution-based confidence probing for calibrated predictions.

Result: The 3B-parameter CLUE model achieved equivalent or stronger performance than baselines up to 32B parameters across all modalities.

Conclusion: Presented a novel multimodal uncertain inference task and a model, CLUE, that effectively produces calibrated probability estimates across multiple modalities.
...
TL;DR: The paper proposes RS-OVC, an Open Vocabulary Counting model for Remote-Sensing and aerial imagery, which can accurately count novel object classes unseen during training.


<details>
  <summary>Details</summary>
Motivation: Existing object-counting methods for remote-sensing imagery are limited to a pre-defined set of object classes, requiring costly re-annotation and model re-training for novel objects.

Method: The authors propose RS-OVC, an Open Vocabulary Counting model that uses textual and/or visual conditioning to count novel object classes.

Result: The model is capable of accurate counting of novel object classes that were unseen during training.

Conclusion: The proposed RS-OVC model addresses the limitation of existing object-counting methods and enables accurate counting of novel object classes in remote-sensing imagery.
...
TL;DR: A two-stage deep learning framework is proposed for detecting and tracking ligaments and droplets in high-speed shadowgraphy images of liquid sheet disintegration, enabling automated analysis of fragmentation dynamics.


<details>
  <summary>Details</summary>
Motivation: The disintegration of liquid sheets into ligaments and droplets involves complex, multi-scale dynamics that are difficult to quantify from high-speed images, and conventional tracking frameworks have limitations in handling one-to-many fragmentation events.

Method: A two-stage framework consisting of a Faster R-CNN with a ResNet-50 backbone and Feature Pyramid Network for object detection and classification, and a Transformer-augmented multilayer perceptron for temporal relationship modeling and inter-frame association classification.

Result: The framework achieves a held-out F1 score of up to 0.872 for object detection, and 86.1% accuracy, 93.2% precision, and perfect recall (1.00) for fragmentation events, enabling automated reconstruction of fragmentation trees and extraction of breakup statistics.

Conclusion: The proposed framework enables automated analysis of liquid sheet disintegration dynamics, including ligament deformation, fragmentation, and parent-child lineage, and provides a powerful tool for understanding primary atomization modes.
...
TL;DR: The paper explores Virtual Try-Off (VTOFF), reconstructing canonical garments from draped-on images, adapting diffusion-based strategies from Virtual Try-On (VTON) and Latent Diffusion Models (LDMs).


<details>
  <summary>Details</summary>
Motivation: The inverse problem of Virtual Try-Off (VTOFF) is less understood compared to Virtual Try-On (VTON), and the authors aim to establish a robust foundation for VTOFF.

Method: The authors investigate the Dual-UNet Diffusion Model architecture, analyzing generation backbone, conditioning, and losses/training strategies, and evaluate their framework on VITON-HD and DressCode datasets.

Result: The framework achieves state-of-the-art performance with a 9.5% drop on the primary metric DISTS and competitive performance on LPIPS, FID, KID, and SSIM.

Conclusion: The paper provides a robust architectural foundation for VTOFF, offering insights and stronger baselines to guide future research in Virtual Try-Off.
...
TL;DR: Proposed LeanGate, a lightweight network that filters out redundant video frames to improve efficiency in Geometric Foundation Models (GFMs) for monocular SLAM, reducing computation by over 85% while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Current GFM-based SLAM systems are computationally inefficient due to post hoc keyframe selection, leading to redundant dense geometric decoding and wasted computation.

Method: LeanGate, a feed-forward frame-gating network that predicts a geometric utility score to assess a frame's mapping value before GFM feature extraction and matching.

Result: LeanGate reduces tracking FLOPs by over 85% and achieves a 5x end-to-end throughput speedup while maintaining tracking and mapping accuracy of dense baselines.

Conclusion: LeanGate effectively mitigates computational inefficiency in GFM-based SLAM systems by filtering out redundant frames, enabling efficient and accurate monocular SLAM.
...
TL;DR: LMGenDrive, a unified framework combining LLM-based multimodal understanding and generative world models for end-to-end closed-loop autonomous driving, outperforming prior methods in challenging benchmarks.


<details>
  <summary>Details</summary>
Motivation: The generalization to long-tail and open-world scenarios remains a major bottleneck for large-scale autonomous driving deployment, and current methods either use LLMs and VLMs for understanding or generative world models for prediction.

Method: LMGenDrive combines LLM-based multimodal understanding with generative world models, using a progressive three-stage training strategy from vision pretraining to multi-step long-horizon driving.

Result: LMGenDrive significantly outperforms prior methods on challenging closed-loop benchmarks, showing clear gains in instruction following, spatio-temporal understanding, and robustness to rare scenarios.
...
TL;DR: The paper proposes a computer vision system to analyze sports games, specifically soccer, by detecting and tracking players, and extracting tactical insights such as player speed and positioning.


<details>
  <summary>Details</summary>
Motivation: To provide actionable data for coaching decisions, improve player performance, and enhance team strategies by analyzing complex features from game footage.

Method: The proposed system uses object detection models (YOLO and Faster R-CNN) for player identification, a point prediction model for key point detection on the field, and SAM2 for segmentation and tracking. Homography is used to transform camera perspective to real-world field coordinates.

Result: The system can calculate valuable tactical insights such as player speed, distance covered, positioning heatmaps, and team statistics, providing coaches and players with actionable performance data.

Conclusion: The proposed system enables the extraction of accurate and valuable tactical insights from game footage, which can inform coaching decisions, improve player performance, and enhance team strategies.
...
TL;DR: The authors expand a benchmark for identifying illegible license plates, improve annotations, and introduce new labels. They also propose a novel training procedure and achieve a 89.5% F1-score on the test set.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this work is to improve the performance of Automatic License Plate Recognition (ALPR) systems in real-world scenarios, which is challenging due to low-quality imaging devices and suboptimal camera installation.

Method: The authors expand the original benchmark to over three times its size, revise its annotations, and introduce novel labels. They also propose a novel training procedure featuring an Exponential Moving Average-based loss function and a refined learning rate scheduler.

Result: The authors achieve an 89.5% F1-score on the test set, surpassing the previous state of the art. They also introduce a novel protocol to address camera contamination between training and evaluation splits, which shows a small impact.

Conclusion: The authors conclude that their improvements enable a baseline model to achieve a high F1-score on the test set, and that their dataset and code are publicly available for further research.
...
TL;DR: SIC3D is a controllable image-conditioned text-to-3D generation pipeline that improves text-to-3D object generation by incorporating reference images for styling, resulting in better geometric fidelity and style adherence.


<details>
  <summary>Details</summary>
Motivation: Current text-to-3D object generation methods suffer from limited controllability and texture ambiguity due to the limitations of text modality. The authors aim to address this by introducing a controllable image-conditioned approach.

Method: The SIC3D pipeline consists of two stages: 1) generating 3D object content from text using a text-to-3DGS generation model, and 2) transferring style from a reference image to the 3DGS using a novel Variational Stylized Score Distillation (VSSD) loss and scaling regularization.

Result: SIC3D outperforms prior approaches in both qualitative and quantitative evaluations, demonstrating enhanced geometric fidelity and style adherence.

Conclusion: The proposed SIC3D pipeline effectively addresses the limitations of current text-to-3D object generation methods by providing controllable and high-quality 3D object generation with improved style adherence.
...
### [19] [InstrAct: Towards Action-Centric Understanding in Instructional Videos](https://arxiv.org/abs/2604.08762)
*Zhuoyi Yang,Jiapeng Yu,Reuben Tan,Boyang Li,Huijuan Xu*

Main category: cs.CV

TL;DR: The paper proposes InstrAction, a pretraining framework for instructional videos to improve fine-grained action recognition and temporal relation modeling.


<details>
  <summary>Details</summary>
Motivation: Current Video Foundation Models (VFMs) struggle with recognizing fine-grained actions and modeling their temporal relations in instructional videos due to noisy web supervision and a 'static bias' that relies on objects rather than motion cues.

Method: The InstrAction framework includes a data-driven strategy for filtering noisy captions and generating action-centric hard negatives, an Action Perceiver for extracting motion-relevant tokens, and auxiliary objectives such as Dynamic Time Warping alignment (DTW-Align) and Masked Action Modeling (MAM).

Result: The proposed method consistently outperforms state-of-the-art VFMs on semantic reasoning, procedural logic, and fine-grained retrieval tasks on the InstrAct Bench.

Conclusion: The InstrAction framework effectively addresses the challenges of fine-grained action recognition and temporal relation modeling in instructional videos, leading to improved performance over existing VFMs.
...
### [20] [R2G: A Multi-View Circuit Graph Benchmark Suite from RTL to GDSII](https://arxiv.org/abs/2604.08810)
*Zewei Zhou,Jiajun Zou,Jiajia Zhang,Ao Yang,Ruichao He,Haozheng Zhou,Ao Liu,Jiawei Liu,Leilei Jin,Shan Shen,Daying Sun*

Main category: cs.CV

TL;DR: The authors propose R2G, a benchmark suite for graph neural networks in physical design tasks, standardizing circuit representations and evaluation protocols.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the inconsistencies in circuit representations and evaluation protocols in applying graph neural networks to physical design tasks.

Method: The authors present R2G, a multi-view circuit-graph benchmark suite with an end-to-end pipeline, and conduct systematic studies using GINE, GAT, and ResGatedGCN.

Result: The results show that view choice dominates model choice, node-centric views generalize best, and decoder-head depth is the primary accuracy driver.

Conclusion: The authors conclude that R2G provides a standardized benchmark suite for graph neural networks in physical design tasks, allowing for more controlled evaluations and comparisons.
...
### [21] [Towards Responsible Multimodal Medical Reasoning via Context-Aligned Vision-Language Models](https://arxiv.org/abs/2604.08815)
*Sumra Khan,Sagar Chhabriya,Aizan Zafar,Sheeraz Arif,Amgad Muneer,Anas Zafar,Shaina Raza,Rizwan Qureshi*

Main category: cs.CV

TL;DR: The authors propose a context-aligned reasoning framework for medical vision-language models to improve reliability and trustworthiness by enforcing agreement across heterogeneous clinical evidence.


<details>
  <summary>Details</summary>
Motivation: Medical vision-language models often produce weakly grounded conclusions due to over-reliance on a dominant modality, leading to a need for improved reliability and trustworthiness.

Method: The proposed approach augments a frozen VLM with structured contextual signals and generates structured outputs containing supporting evidence, uncertainty estimates, limitations, and safety notes.

Result: The framework improves discriminative performance (AUC 0.918 to 0.925), reduces hallucinated keywords (1.14 to 0.25), and produces more concise reasoning explanations (19.4 to 15.3 words) while maintaining calibrated uncertainty.

Conclusion: Enforcing multi-evidence agreement improves reliability and trustworthiness in medical multimodal reasoning without changing the underlying model architecture.
...
### [22] [SenBen: Sensitive Scene Graphs for Explainable Content Moderation](https://arxiv.org/abs/2604.08819)
*Fatih Cagatay Akyon,Alptekin Temizel*

Main category: cs.CV

TL;DR: The paper introduces Sensitive Benchmark (SenBen), a large-scale scene graph benchmark for sensitive content, and proposes a compact student model that improves upon standard VLMs in scene graph generation and sensitivity detection.


<details>
  <summary>Details</summary>
Motivation: Current content moderation systems lack spatial grounding and interpretability, making it difficult to understand what sensitive behavior was detected, who is involved, or where it occurs.

Method: The authors introduce a multi-task recipe to distill a frontier VLM into a compact 241M student model, using techniques such as suffix-based object identity, Vocabulary-Aware Recall (VAR) Loss, and a decoupled Query2Label tag head with asymmetric loss.

Result: The student model outperforms all evaluated VLMs except Gemini models and commercial safety APIs in grounded scene graph metrics, achieving the highest object detection and captioning scores, with faster inference and less GPU memory.

Conclusion: The proposed student model offers improved performance, efficiency, and interpretability for sensitive content detection, addressing the limitations of current content moderation systems.
...
### [23] [CatalogStitch: Dimension-Aware and Occlusion-Preserving Object Compositing for Catalog Image Generation](https://arxiv.org/abs/2604.08836)
*Sanyam Jain,Pragya Kandari,Manit Singhal,He Zhang,Soo Ye Kim*

Main category: cs.CV

TL;DR: CatalogStitch is a set of techniques that automate corrections in generative object compositing for catalog image generation, reducing manual intervention.


<details>
  <summary>Details</summary>
Motivation: Current generative object compositing methods require tedious manual intervention for catalog image generation, including adjusting masks and restoring occluded elements.

Method: CatalogStitch uses a dimension-aware mask computation algorithm and an occlusion-aware hybrid restoration method, which are model-agnostic techniques.

Result: The techniques demonstrate consistent improvements across diverse catalog scenarios with three state-of-the-art compositing models.

Conclusion: CatalogStitch transforms generative compositing into a practical, human-friendly tool for production catalog workflows by reducing manual intervention and automating tedious corrections.
...
### [24] [DeFakeQ: Enabling Real-Time Deepfake Detection on Edge Devices via Adaptive Bidirectional Quantization](https://arxiv.org/abs/2604.08847)
*Xiangyu Li,Yujing Sun,Yuhang Zheng,Yuexin Ma,Kwok-Yan Lam*

Main category: cs.CV

TL;DR: A novel quantization framework called DeFakeQ is proposed for deepfake detectors, enabling real-time deployment on edge devices with a balance between model compactness and detection performance.


<details>
  <summary>Details</summary>
Motivation: Existing deepfake detection methods are computationally intensive and parameter-heavy, limiting their deployment on resource-constrained edge devices, and state-of-the-art quantization techniques underperform in capturing subtle forgery artifacts.

Method: DeFakeQ introduces a novel adaptive bidirectional compression strategy that leverages feature correlations and eliminates redundancy.

Result: DeFakeQ consistently surpasses existing quantization and model compression baselines across five benchmark datasets and eleven state-of-the-art backbone detectors, and demonstrates real-time deepfake detection capability on mobile devices.

Conclusion: DeFakeQ is a tailored quantization framework for deepfake detectors that enables real-time deployment on edge devices while preserving detection performance.
...
TL;DR: BIAS is a fast, biologically inspired model for dynamic visual saliency detection in videos, outperforming other models and showing strong real-world utility.


<details>
  <summary>Details</summary>
Motivation: The motivation is to develop a fast and biologically inspired model for dynamic visual saliency detection in continuous video streams, bridging biological plausibility and computational efficiency.

Method: The method involves incorporating a retina-inspired motion detector into the Itti-Koch framework to extract temporal features and generate saliency maps that integrate both static and motion information.

Result: BIAS detects salient regions with millisecond-scale latency, outperforms other models on the DHF1K dataset, and achieves state-of-the-art performance in cause-effect recognition and accident anticipation in traffic accident analysis.

Conclusion: BIAS achieves interpretable, high-speed dynamic saliency detection by bridging biological plausibility and computational efficiency, showing strong real-world utility.
...
TL;DR: The paper proposes an uncertainty-aware method for text-based person search, which improves the use of weak positive image-text pairs and achieves significant performance gains on three datasets.


<details>
  <summary>Details</summary>
Motivation: Existing methods for text-based person search focus on strict one-to-one correspondence pair matching, disregarding weak positive image-text pairs. The authors aim to utilize these weak positives to improve performance.

Method: The proposed method consists of two modules: uncertainty estimation and uncertainty regularization. It estimates image-text pair uncertainty and incorporates it into the optimization procedure to adaptively adjust loss weights.

Result: The method achieves mAP improvements of +3.06%, +3.55%, and +6.94% on CUHK-PEDES, RSTPReid, and ICFG-PEDES datasets, respectively, compared to existing competitive methods.

Conclusion: The proposed uncertainty-aware method effectively utilizes weak positive image-text pairs, leading to significant performance gains in text-based person search.
...
### [27] [Precise Shield: Explaining and Aligning VLLM Safety via Neuron-Level Guidance](https://arxiv.org/abs/2604.08881)
*Enyi Shi,Fei Shen,Shuyi Miao,Linxia Zhu,Pengyang Shao,Jinhui Tang,Tat-Seng Chua*

Main category: cs.CV

TL;DR: The paper proposes a method called Precise Shield to improve the safety of Vision-Language Large Models (VLLMs) against multilingual and multimodal attacks by identifying and constraining safety neurons.


<details>
  <summary>Details</summary>
Motivation: Current cross-lingual and cross-modal safety methods have structural blind spots, and harmful content can bypass defenses, highlighting the need to understand where safety capability is instantiated within the model.

Method: A two-stage framework called Precise Shield, which identifies safety neurons by contrasting activation patterns and then constrains parameter updates within this subspace via gradient masking.

Result: The proposed method substantially improves safety while preserving multilingual and multimodal generalization, and enables zero-shot cross-lingual and cross-modal transfer of safety capabilities.

Conclusion: The study reveals a moderate overlap of safety neurons across languages and modalities, offering a new direction for neuron-level, transfer-based safety enhancement.
...
TL;DR: The paper introduces HM-Bench, a benchmark for evaluating multimodal large language models (MLLMs) on hyperspectral image (HSI) understanding, and presents results showing MLLMs struggle with complex spatial-spectral reasoning tasks.


<details>
  <summary>Details</summary>
Motivation: The ability of MLLMs to perceive and reason over hyperspectral images (HSI) remains underexplored, despite HSI being a vital modality in remote sensing, and the high dimensionality and intricate spectral-spatial properties of HSI pose unique challenges for models primarily trained on RGB data.

Method: The authors propose a dual-modality evaluation framework that transforms HSI data into two complementary representations: PCA-based composite images and structured textual reports, and evaluate 18 representative MLLMs on a large-scale dataset of 19,337 question-answer pairs across 13 task categories.

Result: The results reveal significant difficulties in handling complex spatial-spectral reasoning tasks, and show that visual inputs generally outperform textual inputs, highlighting the importance of grounding in spectral-spatial evidence for effective HSI understanding.

Conclusion: The paper concludes that HM-Bench provides a systematic way to evaluate MLLMs on HSI understanding, and that improving MLLMs' ability to perceive and reason over HSI is crucial for remote sensing applications.
...
### [29] [Adaptive Dual Residual U-Net with Attention Gate and Multiscale Spatial Attention Mechanisms (ADRUwAMS)](https://arxiv.org/abs/2604.08893)
*Mohsen Yaghoubi Suraki*

Main category: cs.CV

TL;DR: A novel deep learning model, ADRUwAMS, is proposed for accurate brain tumor segmentation, achieving high dice scores on BraTS 2020 and 2019 datasets.


<details>
  <summary>Details</summary>
Motivation: Early detection of glioma, a harmful brain tumor, is crucial for effective treatment, and automated segmentation process is required to accurately separate tumor zones from healthy tissues.

Method: An Adaptive Dual Residual U-Net with Attention Gate and Multiscale Spatial Attention Mechanisms (ADRUwAMS) is introduced, combining adaptive dual residual networks, attention mechanisms, and multiscale spatial attention.

Result: The model achieved high accuracy for tumor detection and segmentation on BraTS 2020, with dice scores of 0.9229 for the whole tumor, 0.8432 for the tumor core, and 0.8004 for the enhancing tumor.

Conclusion: The proposed ADRUwAMS model shows promising results for accurate brain tumor segmentation, which can aid in early detection and effective treatment of glioma.
...
TL;DR: The paper introduces GeoMMBench, a benchmark for multimodal question-answering in geoscience and remote sensing, and proposes GeoMMAgent, a multi-agent framework that integrates domain-specific models and tools to improve performance.


<details>
  <summary>Details</summary>
Motivation: The development of multimodal large language models (MLLMs) in geoscience and remote sensing is constrained by challenges such as wide-ranging disciplinary knowledge, heterogeneous sensor modalities, and a fragmented spectrum of tasks.

Method: The authors introduce GeoMMBench, a comprehensive multimodal question-answering benchmark, and assess 36 open-source and proprietary large language models using this benchmark. They also propose GeoMMAgent, a multi-agent framework that strategically integrates retrieval, perception, and reasoning through domain-specific RS models and tools.

Result: The experimental results demonstrate that GeoMMAgent significantly outperforms standalone LLMs, underscoring the importance of tool-augmented agents for dynamically tackling complex geoscience and RS challenges.

Conclusion: The paper concludes that the proposed GeoMMAgent framework can effectively address the challenges in geoscience and remote sensing by integrating domain-specific models and tools, leading to improved performance.
...
### [31] [Fast Model-guided Instance-wise Adaptation Framework for Real-world Pansharpening with Fidelity Constraints](https://arxiv.org/abs/2604.08903)
*Zhiqi Yang,Jin-Liang Xiao,Shan Yin,Liang-Jian Deng,Gemine Vivone*

Main category: cs.CV

TL;DR: A fast and generalizable pansharpening framework, FMG-Pan, is proposed, achieving state-of-the-art performance with rapid training-inference and cross-sensor generality.


<details>
  <summary>Details</summary>
Motivation: Existing pansharpening methods have limitations, including high training costs, limited generalization, and slow convergence. Recent zero-shot methods offer strong generalization but suffer from limited fusion quality and high computational overhead.

Method: FMG-Pan, a model-guided instance-wise adaptation framework, leverages a pretrained model to guide a lightweight adaptive network through joint optimization with spectral and physical fidelity constraints.

Result: FMG-Pan achieves state-of-the-art performance on real-world datasets, completing training and inference for a 512x512x8 image within 3 seconds on an RTX 3090 GPU.

Conclusion: FMG-Pan offers a fast, generalizable, and practical solution for pansharpening, addressing existing limitations and achieving impressive performance.
...
TL;DR: A universal defect generation model, UniDG, is proposed to tackle the limitations of existing methods in generating diverse and realistic defects across various categories.


<details>
  <summary>Details</summary>
Motivation: Current defect generation methods rely on few-shot learning, which overfits to specific defect categories due to the lack of large-scale paired defect editing data, resulting in limited generalization, degraded realism, and category consistency.

Method: The authors introduce a large-scale dataset, UDG, and propose UniDG, a universal defect generation foundation model that uses Defect-Context Editing, MM-DiT multimodal attention, and a two-stage training strategy.

Result: UniDG outperforms prior few-shot anomaly generation and image insertion/editing baselines in synthesis quality and downstream single- and multi-class anomaly detection/localization on MVTec-AD and VisA datasets.

Conclusion: The proposed UniDG model offers a more generalizable and realistic approach to defect generation, addressing the limitations of existing methods and demonstrating improved performance on various anomaly detection tasks.
...
TL;DR: Proposed MV3DIS, a zero-shot 3D instance segmentation framework that incorporates 3D priors for improved accuracy.


<details>
  <summary>Details</summary>
Motivation: Current 3D instance segmentation methods rely on labor-intensive 3D annotations or produce inconsistent 2D masks across views, leading to fragmented 3D segmentation.

Method: MV3DIS uses a coarse-to-fine framework with 3D-guided mask matching and depth consistency weighting to refine 3D segments into precise instances.

Result: MV3DIS achieves superior performance on multiple datasets, including ScanNetV2, ScanNet200, ScanNet++, Replica, and Matterport3D.

Conclusion: MV3DIS effectively addresses the limitations of current 3D instance segmentation methods by incorporating 3D priors and achieving state-of-the-art performance.
...
TL;DR: A Vision-Language Model (VLM) called TAIHRI is proposed for close-range human-robot interaction (HRI) perception, enabling precise 3D localization of task-relevant body parts.


<details>
  <summary>Details</summary>
Motivation: Conventional 3D human keypoints estimation methods focus on whole-body reconstruction quality, but in practical HRI scenarios, precise metric-scale spatial localization of task-relevant body parts is crucial.

Method: TAIHRI uses 2D keypoint reasoning via next token prediction to precisely localize 3D spatial coordinates of critical body parts by quantizing 3D keypoints into a finite interaction space.

Result: Experiments on egocentric interaction benchmarks demonstrate that TAIHRI achieves superior estimation accuracy for task-critical body parts.

Conclusion: TAIHRI opens new research avenues in the field of embodied human-robot interaction, enabling robots to achieve natural and safe physical interaction with users.
...
TL;DR: The paper proposes a degradation-aware diffusion framework for image fusion under complex degradation scenarios, offering flexible adaptation and high reconstruction accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing image fusion methods are limited by their performance under complex degradations such as noise, blur, and low resolution, and their interpretability.

Method: The proposed method uses implicit denoising by directly regressing the fused image, and a joint observation model correction mechanism to ensure high reconstruction accuracy.

Result: Experiments demonstrate the superiority of the proposed method under complex degradation scenarios.

Conclusion: The proposed framework efficiently handles image fusion under arbitrary degradation scenarios, providing a more structured and interpretable approach than existing methods.
...
### [36] [Customized Fusion: A Closed-Loop Dynamic Network for Adaptive Multi-Task-Aware Infrared-Visible Image Fusion](https://arxiv.org/abs/2604.08924)
*Zengyi Yang,Yu Liu,Juan Cheng,Zhiqin Zhu,Yafei Zhang,Huafeng Li*

Main category: cs.CV

TL;DR: Proposed a Closed-Loop Dynamic Network (CLDyN) for infrared-visible image fusion that adapts to multiple downstream tasks without retraining.


<details>
  <summary>Details</summary>
Motivation: Existing image fusion methods struggle to adapt to multiple downstream tasks simultaneously.

Method: Introduced a closed-loop optimization mechanism with a Requirement-driven Semantic Compensation (RSC) module, Basis Vector Bank (BVB), and Architecture-Adaptive Semantic Injection (A2SI) block.

Result: CLDyN maintains high fusion quality and exhibits strong multi-task adaptability on M3FD, FMB, and VT5000 datasets.

Conclusion: CLDyN enables task-specific semantic compensation, allowing the fusion network to actively adapt to diverse tasks without retraining.
...
### [37] [M-IDoL: Information Decomposition for Modality-Specific and Diverse Representation Learning in Medical Foundation Model](https://arxiv.org/abs/2604.08936)
*Yihang Liu,Ying Wen,Jiaxiong Yang,Longzhen Yang,Lianghua He,Heng Tao Shen*

Main category: cs.CV

TL;DR: The paper proposes M-IDoL, a self-supervised medical foundation model that uses information decomposition to learn multimodal representations, achieving superior generalization across 21 clinical tasks and outperforming 20 existing models.


<details>
  <summary>Details</summary>
Motivation: Existing medical foundation models suffer from information ambiguity due to blending multimodal representations in a single embedding space, leading to degradation of modality specificity and diversity.

Method: M-IDoL introduces two objectives: maximizing inter-modality entropy by dispersing multimodal representations into separable Mixture-of-Experts (MoE) subspaces, and minimizing intra-modality uncertainty by performing fine-grained semantic discrimination within each MoE subspace.

Result: M-IDoL delivers superior generalization across 21 downstream clinical tasks, outperforming 20 foundation models on five imaging modalities, and learns modality-specific and diverse representations.

Conclusion: The proposed M-IDoL model effectively addresses the limitations of existing medical foundation models by learning universal representations from multimodal medical images, leading to improved performance and representation specificity across modalities.
...
### [38] [MASS: Mesh-inellipse Aligned Deformable Surfel Splatting for Hand Reconstruction and Rendering from Egocentric Monocular Video](https://arxiv.org/abs/2604.08943)
*Haoyu Zhu,Yi Zhang,Lei Yao,Lap-pui Chau,Yi Wang*

Main category: cs.CV

TL;DR: The paper proposes a method called MASS for reconstructing high-fidelity 3D hands from egocentric monocular videos, addressing challenges such as low resolution, hand-object interactions, and high computational costs.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome the limitations of existing methods in capturing high-resolution geometry, handling hand-object interactions, and reducing computational costs for real-time applications.

Method: The proposed method, Mesh-inellipse Aligned deformable Surfel Splatting (MASS), uses a deformable 2D Gaussian Surfel representation, mesh-aligned Steiner Inellipse, fractal densification, Gaussian Surfel Deformation, and a two-stage training strategy with a novel binding loss.

Result: The results show that the proposed model achieves superior reconstruction performance compared to state-of-the-art methods on the ARCTIC dataset, the Hand Appearance dataset, and the Interhand2.6M dataset.

Conclusion: The paper concludes that the proposed MASS method effectively addresses the challenges in reconstructing high-fidelity 3D hands from egocentric monocular videos, offering improved performance and efficiency.
...
### [39] [TouchAnything: Diffusion-Guided 3D Reconstruction from Sparse Robot Touches](https://arxiv.org/abs/2604.08945)
*Langzhe Gu,Hung-Jui Huang,Mohamad Qadri,Michael Kaess,Wenzhen Yuan*

Main category: cs.CV

TL;DR: A framework called TouchAnything is proposed, leveraging a pre-trained 2D vision diffusion model to reconstruct 3D object geometry from sparse tactile measurements.


<details>
  <summary>Details</summary>
Motivation: The motivation is to accurately estimate object geometry, which is essential for tasks like robotic manipulation and physical interaction, especially in scenarios where vision is unreliable.

Method: The method uses a pre-trained large-scale 2D vision diffusion model as a semantic and geometric prior for 3D reconstruction from sparse tactile measurements, formulating reconstruction as an optimization problem.

Result: The method reconstructs accurate geometries from only a few touches, outperforming existing baselines and enabling open-world 3D reconstruction of previously unseen object instances.

Conclusion: The proposed framework, TouchAnything, effectively leverages visual priors for tactile-based 3D reconstruction, achieving accurate and efficient object geometry estimation.
...
### [40] [Low-Data Supervised Adaptation Outperforms Prompting for Cloud Segmentation Under Domain Shift](https://arxiv.org/abs/2604.08956)
*Harshith Kethavath,Weiming Hu*

Main category: cs.CV

TL;DR: The study evaluates the adaptation of vision-language models to satellite imagery, specifically for cloud segmentation. It finds that prompt engineering underperforms and that supervised fine-tuning with a small amount of labeled data significantly improves performance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to investigate the effectiveness of adapting vision-language models to remote sensing imagery, particularly when there's a significant distribution mismatch between the model's pretraining data and the target domain.

Method: The study uses CLIPSeg on the CloudSEN12+ benchmark to evaluate 60 prompt variants and compares them to supervised fine-tuning with varying amounts of labeled data.

Result: The results show that prompt engineering variants underperform the zero-shot baseline, while supervised fine-tuning with as little as 0.1% labeled data surpasses zero-shot performance, and 5-10% data recovers ~85% of maximum achievable mIoU.

Conclusion: The conclusion is that labeled data is not an expensive alternative to prompting but a worthwhile path for adapting vision-language models to specialized imagery, as it significantly improves performance compared to prompt engineering.
...
### [41] [Dynamic Class-Aware Active Learning for Unbiased Satellite Image Segmentation](https://arxiv.org/abs/2604.08965)
*Gadi Hemanth Kumar,Athira Nambiar,Pankaj Bodani*

Main category: cs.CV

TL;DR: A novel active learning method, DCAU-AL, is proposed to improve semantic segmentation of satellite imagery by prioritizing underperforming classes, reducing annotation costs while maintaining high model performance.


<details>
  <summary>Details</summary>
Motivation: Annotating large-scale satellite datasets is costly and time-consuming, and existing active learning strategies have limitations in handling class imbalance and adapting to underperforming classes.

Method: A dynamic class-aware uncertainty-based active learning (DCAU-AL) approach is proposed, which tracks class-wise performance gaps and adjusts sampling weights to focus on poorly performing or underrepresented classes.

Result: DCAU-AL outperforms existing active learning methods, especially under severe class imbalance, delivering superior per-class IoU and improved annotation efficiency on the OpenEarth land cover dataset.

Conclusion: The proposed DCAU-AL method offers an efficient and effective solution for semantic segmentation of satellite imagery, reducing labeling costs while maintaining high model performance, particularly in handling class-imbalanced datasets.
...
### [42] [How Should Video LLMs Output Time? An Analysis of Efficient Temporal Grounding Paradigms](https://arxiv.org/abs/2604.08966)
*Shengji Jin,Yuanhao Zou,Victor Zhu,Zhengping Ji,Chen Chen*

Main category: cs.CV

TL;DR: This paper compares three output paradigms for Video Temporal Grounding (VTG) systems, evaluating their impact on accuracy and efficiency, providing guidelines for designing efficient VTG systems.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to investigate the impact of different output paradigms on Video Temporal Grounding (VTG) systems, specifically in terms of accuracy and efficiency, as existing methods couple output paradigms with different backbones, datasets, and training protocols, making it challenging to isolate the specific impact of the output design.

Method: The authors conduct a controlled empirical study comparing three dominant VTG output paradigms: Text Numeral Generation, Temporal Token Generation, and Continuous Temporal Decoding, evaluating them across identical compact VLMs using consistent datasets and LoRA fine-tuning protocols.

Result: The results demonstrate that the choice of output formulation significantly affects both grounding accuracy and computational cost, independent of model scale, and that the continuous distribution paradigm consistently achieves the most favorable efficiency-accuracy trade-off on the Pareto frontier.

Conclusion: The findings provide objective empirical guidelines for designing efficient, deployment-ready VTG systems, highlighting the importance of considering the trade-off between output formulation and system-level efficiency.
...
### [43] [ActFER: Agentic Facial Expression Recognition via Active Tool-Augmented Visual Reasoning](https://arxiv.org/abs/2604.08990)
*Shifeng Liu,Zhengye Zhang,Sirui Zhao,Xinglong Mao,Zhehan Kan,Zhixiang Wei,Shiwei Wu,Chaoyou Fu,Tong Xu,Enhong Chen*

Main category: cs.CV

TL;DR: The paper proposes ActFER, an active facial expression recognition framework that uses reinforcement learning to dynamically acquire and reason over facial evidence, outperforming passive methods.


<details>
  <summary>Details</summary>
Motivation: Existing MLLM-based FER methods are limited by their passive paradigm, relying on fixed visual evidence without active perception.

Method: The authors propose ActFER, an agentic framework that uses reinforcement learning (UC-GRPO) to dynamically invoke tools for face detection, alignment, and reasoning over facial Action Units (AUs) and emotions.

Result: ActFER trained with UC-GRPO consistently outperforms passive MLLM-based FER baselines and substantially improves AU prediction accuracy.

Conclusion: The proposed ActFER framework and UC-GRPO algorithm enable active facial perception and multimodal reasoning, leading to improved facial expression recognition performance.
...
### [44] [PinpointQA: A Dataset and Benchmark for Small Object-Centric Spatial Understanding in Indoor Videos](https://arxiv.org/abs/2604.08991)
*Zhiyu Zhou,Peilin Liu,Ruoxuan Zhang,Luyang Zhang,Cheng Zhang,Hongxia Xie,Wen-Huang Cheng*

Main category: cs.CV

TL;DR: A new dataset and benchmark, PinpointQA, is introduced for evaluating small object-centric spatial understanding in indoor videos, which is crucial for applications like object search and assistive technologies.


<details>
  <summary>Details</summary>
Motivation: The lack of a benchmark that directly assesses the ability of multimodal large language models (MLLMs) to localize a target object in a video and describe its position accurately for downstream applications.

Method: Introduction of PinpointQA, a dataset built from ScanNet++ and ScanNet200, consisting of 1,024 scenes and 10,094 QA pairs across four tasks: TPV, NRI, FSD, and SSP.

Result: Experiments on MLLMs show a capability gap across tasks, with SSP being particularly challenging. Supervised fine-tuning on PinpointQA yields significant improvements, especially on harder tasks.

Conclusion: PinpointQA serves as both a diagnostic benchmark and an effective training dataset for enhancing small object-centric spatial understanding in indoor videos.
...
### [45] [Matrix-Game 3.0: Real-Time and Streaming Interactive World Model with Long-Horizon Memory](https://arxiv.org/abs/2604.08995)
...
TL;DR: The paper presents Matrix-Game 3.0, a memory-augmented interactive world model for 720p real-time longform video generation, achieving up to 40 FPS with a 5B model.


<details>
  <summary>Details</summary>
Motivation: Existing approaches struggle to achieve memory-enabled long-term temporal consistency and high-resolution real-time generation, limiting their applicability in real-world scenarios.

Method: The authors introduce systematic improvements across data, model, and inference, including an upgraded data engine, a training framework for long-horizon consistency, and a multi-segment autoregressive distillation strategy.

Result: Matrix-Game 3.0 achieves up to 40 FPS real-time generation at 720p resolution with a 5B model, while maintaining stable memory consistency over minute-long sequences.

Conclusion: The approach provides a practical pathway toward industrial-scale deployable world models, with potential applications in real-world scenarios.
...
### [46] [StreamMeCo: Long-Term Agent Memory Compression for Efficient Streaming Video Understanding](https://arxiv.org/abs/2604.09000)
*Junxi Wang,Te Sun,Jiayi Zhu,Junxian Li,Haowen Xu,Zichen Wen,Xuming Hu,Zhiyu Li,Linfeng Zhang*

Main category: cs.CV

TL;DR: StreamMeCo, a framework for compressing stream agent memory, reduces memory overhead while maintaining accuracy in video understanding.


<details>
  <summary>Details</summary>
Motivation: Storing vision agent memory for videos incurs substantial memory overhead, leading to high costs in storage and computation.

Method: StreamMeCo uses edge-free minmax sampling for isolated nodes and edge-aware weight pruning for connected nodes, along with a time-decay memory retrieval mechanism.

Result: StreamMeCo achieves a 1.87* speedup in memory retrieval and an average accuracy improvement of 1.0% under 70% memory graph compression.

Conclusion: StreamMeCo effectively reduces memory overhead while maintaining accuracy in video understanding, making it a promising solution for streaming video applications.
...
### [47] [Robust by Design: A Continuous Monitoring and Data Integration Framework for Medical AI](https://arxiv.org/abs/2604.09009)
*Mohammad Daouk,Jan Ulrich Becker,Neeraja Kambham,Anthony Chang,Chandra Mohan,Hien Van Nguyen*

Main category: cs.CV

TL;DR: The paper proposes a framework for continuous monitoring and data integration to maintain the performance of medical AI models in dynamic clinical environments.


<details>
  <summary>Details</summary>
Motivation: The performance of adaptive medical AI models drops in dynamic clinical environments due to data drift.

Method: A three-stage method using multi-metric feature analysis and Monte Carlo dropout-based uncertainty gating to decide when to retrain on new data.

Result: The framework prevents performance degradation, maintaining an AUC of ~0.92 and accuracy of ~89% on a multi-center dataset.

Conclusion: The proposed framework enables sustained learning in medical imaging AI by addressing data shift and avoiding catastrophic forgetting.
...
### [48] [Domain-generalizable Face Anti-Spoofing with Patch-based Multi-tasking and Artifact Pattern Conversion](https://arxiv.org/abs/2604.09018)
*Seungjin Jung,Yonghyun Jeong,Minha Kim,Jimin Min,Youngjoon Yoo,Jongwon Choi*

Main category: cs.CV

TL;DR: The paper introduces PCGAN to improve face anti-spoofing by enhancing domain generalization and detecting partial attacks.


<details>
  <summary>Details</summary>
Motivation: Face anti-spoofing algorithms struggle with limited dataset diversity, making it hard for them to handle unseen visual domains and spoofing methods.

Method: The authors propose Pattern Conversion Generative Adversarial Network (PCGAN) which disentangles latent vectors for spoof artifacts and facial features, and incorporates patch-based and multi-task learning.

Result: PCGAN shows substantial improvement in facial recognition security by validating its effectiveness in domain generalization and detecting partial attacks.

Conclusion: PCGAN enhances domain generalization in face anti-spoofing and effectively detects partial attacks, thereby improving facial recognition security.
...
### [49] [BlendFusion -- Scalable Synthetic Data Generation for Diffusion Model Training](https://arxiv.org/abs/2604.09022)
*Thejas Venkatesh,Suguna Varshini Velury*

Main category: cs.CV

TL;DR: The authors propose BlendFusion, a framework for generating high-quality synthetic image-caption pairs from 3D scenes, addressing issues with current diffusion model-generated data.


<details>
  <summary>Details</summary>
Motivation: The rapid adoption of diffusion models for synthetic data generation faces challenges such as visual inconsistencies and model collapse, known as Model Autophagy Disorder (MAD).

Method: The authors propose a scalable framework called BlendFusion, which uses path tracing, object-centric camera placement, robust filtering, and automatic captioning to generate high-quality image-caption pairs from 3D scenes.

Result: The authors curate FineBLEND, a dataset constructed from diverse 3D scenes, and empirically analyze its quality, comparing it favorably to existing image-caption datasets.

Conclusion: The proposed BlendFusion framework and FineBLEND dataset demonstrate the effectiveness of generating high-quality synthetic data from 3D scenes, addressing challenges in current diffusion model-based approaches.
...
### [50] [CAD 100K: A Comprehensive Multi-Task Dataset for Car Related Visual Anomaly Detection](https://arxiv.org/abs/2604.09023)
...
TL;DR: A new benchmark dataset (CAD Dataset) is proposed for multi-task visual anomaly detection in car-related manufacturing, facilitating task interaction and knowledge transfer.


<details>
  <summary>Details</summary>
Motivation: Existing methods for visual anomaly detection are task-specific and lack a unified benchmark for multi-task evaluation, hindering progress in car-related manufacturing quality assessment.

Method: The CAD Dataset is introduced, containing over 100 images across 7 vehicle domains and 3 tasks, and a multi-task baseline is implemented for evaluation.

Result: The results show that multi-task learning promotes task interaction and knowledge transfer, but also reveals challenging conflicts between tasks.

Conclusion: The CAD dataset provides a standardized platform to drive future advances in car-related multi-task visual anomaly detection.
...
TL;DR: Proposes ImageProtector to protect images from misuse by large language models by adding a nearly imperceptible perturbation.


<details>
  <summary>Details</summary>
Motivation: The misuse of open-weight multi-modal large language models (MLLMs) to extract sensitive information from personal images at scale.

Method: ImageProtector, a user-side method that embeds a carefully crafted perturbation into images to induce MLLMs to generate refusal responses.

Result: ImageProtector is effective across six MLLMs and four datasets, and evaluated countermeasures have limitations.

Conclusion: ImageProtector shows promise in protecting images from misuse by MLLMs, but also highlights limitations and potential future directions.
...
### [52] [Skill-Conditioned Visual Geolocation for Vision-Language](https://arxiv.org/abs/2604.09025)
*Chenjie Yang,Yutian Jiang,Chenyu Wu*

Main category: cs.CV

TL;DR: The paper proposes GeoSkill, a training-free framework for vision-language models to improve image geolocation with structured geographic reasoning and autonomous self-evolution.


<details>
  <summary>Details</summary>
Motivation: Existing methods for image geolocation rely on implicit parametric memory, which can exploit outdated knowledge and generate hallucinated reasoning, and lack feedback loops for self-evolution.

Method: The proposed framework, GeoSkill, uses an evolving Skill-Graph, initialized with human expert trajectories, and an Autonomous Evolution mechanism to iteratively synthesize and prune skills, expanding the Skill-Graph and correcting geographic biases.

Result: Experiments show that GeoSkill achieves promising performance in geolocation accuracy and reasoning faithfulness on GeoRC, and superior generalization across diverse external datasets.

Conclusion: The proposed GeoSkill framework enables vision-language models to improve image geolocation with structured geographic reasoning and autonomous self-evolution, leading to enhanced cognition of real-world geographic knowledge.
...
### [53] [NTIRE 2026 The 3rd Restore Any Image Model (RAIM) Challenge: Multi-Exposure Image Fusion in Dynamic Scenes (Track 2)](https://arxiv.org/abs/2604.09030)
...
TL;DR: The paper presents the 3rd Restore Any Image Model (RAIM) challenge on multi-exposure image fusion in dynamic scenes, introducing a benchmark and evaluating submissions based on PSNR, SSIM, LPIPS, and other factors.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenges of HDR imaging in dynamic scenes with exposure bracketing under scene motion, illumination variation, and handheld camera jitter.

Method: The challenge introduced a benchmark with 100 training sequences and 100 test sequences, and evaluated submissions using a leaderboard score derived from PSNR, SSIM, and LPIPS.

Result: The winning methods significantly improved the ability to remove artifacts from multi-exposure fusion and recover fine details, with 114 participating teams and 987 submissions.

Conclusion: The paper concludes that the RAIM challenge provides a benchmark and a repository for multi-exposure image fusion in dynamic scenes, promoting the development of HDR imaging techniques.
...
### [54] [SiMing-Bench: Evaluating Procedural Correctness from Continuous Interactions in Clinical Skill Videos](https://arxiv.org/abs/2604.09037)
...
TL;DR: The authors introduce SiMing-Bench, a benchmark for evaluating multimodal large language models' (MLLMs) ability to track procedural state updates in clinical skill videos, and find that current MLLMs perform poorly on this task.


<details>
  <summary>Details</summary>
Motivation: Current video benchmarks for MLLMs overlook the capability to track how ongoing interactions update the procedural state, which is crucial for expert procedural judgment.

Method: The authors propose SiMing-Bench, a benchmark instantiated with SiMing-Score, a physician-annotated dataset of clinical skill examination videos, and evaluate diverse open- and closed-source MLLMs on this benchmark.

Result: The authors observe consistently weak agreement with physician judgments across MLLMs, suggesting that coarse global assessment overestimates current models' procedural judgment ability.

Conclusion: The authors conclude that the bottleneck is not merely fine-grained scoring or temporal localization, but modeling how continuous interactions update procedural state over time, highlighting the need for improved MLLMs.
...
### [55] [Scene-Agnostic Object-Centric Representation Learning for 3D Gaussian Splatting](https://arxiv.org/abs/2604.09045)
*Tsuheng Hsu,Guiyu Liu,Juho Kannala,Janne Heikkilä*

Main category: cs.CV

TL;DR: The paper proposes an object-centric supervision scheme for 3D scene understanding, enabling scene-agnostic object representations and improving generalization across scenes.


<details>
  <summary>Details</summary>
Motivation: Current methods for 3D scene understanding rely on 2D masks from visual foundation models, which are not object-centric and require additional processing or specialized training. The learned object identities are scene-dependent, limiting generalizability.

Method: The authors propose a dataset-level, object-centric supervision scheme using a pre-trained slot attention-based Global Object Centric Learning (GOCL) module to learn a scene-agnostic object codebook. They couple the codebook with unsupervised object masks to directly supervise 3D Gaussian identity features.

Result: The method enables unsupervised object-centric learning in 3D Gaussian Splatting (3DGS), yielding more structured representations and better generalization for downstream tasks such as robotic interaction and scene understanding.

Conclusion: The proposed method introduces object-centric learning into 3DGS, allowing for scene-agnostic object representations and improving generalization across scenes without requiring per-scene fine-tuning or retraining.
...
### [56] [Text-Conditioned Multi-Expert Regression Framework for Fully Automated Multi-Abutment Design](https://arxiv.org/abs/2604.09047)
*Mianjie Zheng,Xinquan Yang,Xuefen Liu,Xuguang Li,Kun Tang,He Meng,Linlin Shen*

Main category: cs.CV

TL;DR: A fully automated framework, TEMAD, is proposed for dental implant abutment design, integrating site localization and abutment parameter regression.


<details>
  <summary>Details</summary>
Motivation: Current dental implant abutment design methods are time-consuming and rely heavily on manual effort. Existing automated approaches require significant clinician intervention and are not scalable for multi-abutment scenarios.

Method: The TEMAD framework includes an Implant Site Identification Network (ISIN) for site localization, a Tooth-Conditioned Feature-wise Linear Modulation (TC-FiLM) module for adaptive feature calibration, and a System-Prompted Mixture-of-Experts (SPMoE) mechanism for system-aware regression.

Result: TEMAD achieves state-of-the-art performance on a large-scale abutment design dataset, particularly in multi-abutment settings.

Conclusion: The proposed TEMAD framework effectively enables fully automated dental implant planning, addressing the limitations of current methods.
...
### [57] [Fine-Grained Action Segmentation for Renorrhaphy in Robot-Assisted Partial Nephrectomy](https://arxiv.org/abs/2604.09051)
*Jiaheng Dai,Huanrong Liu,Tailai Zhou,Tongyu Jia,Qin Liu,Yutong Ban,Zeju Li,Yu Gao,Xin Ma,Qingbiao Li*

Main category: cs.CV

TL;DR: The paper presents a benchmark for fine-grained action segmentation in robot-assisted partial nephrectomy, comparing four temporal models for recognizing suturing gestures.


<details>
  <summary>Details</summary>
Motivation: The need for precise recognition of visually similar suturing gestures with variable duration and class imbalance during renorrhaphy in robot-assisted partial nephrectomy.

Method: The paper uses four temporal models (MS-TCN++, AsFormer, TUT, and DiffAct) built on I3D features, evaluated on the SIA-RAPN benchmark with 50 clinical videos and 12 frame-level labels.

Result: DiffAct achieves the highest F1, frame-wise accuracy, edit score, and frame mAP, while MS-TCN++ attains the highest balanced accuracy across the strongest reported values.

Conclusion: The benchmark provides a comprehensive evaluation of temporal models for fine-grained action segmentation in robot-assisted partial nephrectomy, highlighting the strengths of DiffAct and MS-TCN++.
...
### [58] [Tora3: Trajectory-Guided Audio-Video Generation with Physical Coherence](https://arxiv.org/abs/2604.09057)
*Junchao Liao,Zhenghao Zhang,Xiangyu Meng,Litao Li,Ziying Zhang,Siyu Zhu,Long Qin,Weizhi Wang*

Main category: cs.CV

TL;DR: Tora3, a new framework for audio-video generation, improves physical coherence by using object trajectories as a shared prior for both visual and audio generation.


<details>
  <summary>Details</summary>
Motivation: Existing methods for audio-video generation struggle with producing content that has plausible motion-sound relations, often resulting in visually unstable object motions and loosely aligned sounds.

Method: Tora3 uses object trajectories as a shared kinematic prior to guide both visual motion and acoustic events, incorporating a trajectory-aligned motion representation, a kinematic-audio alignment module, and a hybrid flow matching scheme.

Result: Tora3 improves motion realism, motion-sound synchronization, and overall audio-video generation quality compared to strong open-source baselines.

Conclusion: The proposed Tora3 framework effectively enhances the physical coherence and motion-sound synchronization of generated audio-video content.
...
### [59] [Learning Vision-Language-Action World Models for Autonomous Driving](https://arxiv.org/abs/2604.09059)
*Guoqing Wang,Pin Tang,Xiangxuan Ren,Guodongfang Zhao,Bailan Feng,Chao Ma*

Main category: cs.CV

TL;DR: The paper introduces VLA-World, a Vision-Language-Action world model that improves driving foresight by unifying predictive imagination and reflective reasoning.


<details>
  <summary>Details</summary>
Motivation: Current Vision-Language-Action (VLA) models lack explicit modeling of temporal dynamics and global world consistency, limiting their foresight and safety.

Method: VLA-World uses an action-derived feasible trajectory to guide the generation of the next-frame image and reasons over this self-generated future imagined frame to refine the predicted trajectory.

Result: VLA-World consistently surpasses state-of-the-art VLA and world-model baselines on both planning and future-generation benchmarks.

Conclusion: The proposed VLA-World model effectively improves driving foresight by integrating predictive imagination and reflective reasoning, demonstrating superior performance over existing baselines.
...
### [60] [Nested Radially Monotone Polar Occupancy Estimation: Clinically-Grounded Optic Disc and Cup Segmentation for Glaucoma Screening](https://arxiv.org/abs/2604.09062)
*Rimsa Goperma,Rojan Basnet,Liang Zhao*

Main category: cs.CV

TL;DR: This paper proposes NPS-Net, a novel framework for segmenting the optic disc and optic cup from fundus photographs, ensuring clinical validity and achieving high accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning methods for optic disc and optic cup segmentation do not guarantee clinical validity, leading to corruption in diagnostic metrics, especially under cross-dataset domain shift.

Method: The proposed NPS-Net formulates the OD/OC segmentation as nested radially monotone polar occupancy estimation, ensuring star-convexity and nested structure of OD and OC.

Result: NPS-Net shows strong zero-shot generalization across seven public datasets, achieving high accuracy and improving upon baseline methods in terms of anatomical validity, Cup Dice, and Disc Dice.

Conclusion: The proposed NPS-Net framework ensures clinical validity and achieves high accuracy in optic disc and optic cup segmentation, making it a promising solution for glaucoma screening.
...
### [61] [Cross-Modal Knowledge Distillation from Spatial Transcriptomics to Histology](https://arxiv.org/abs/2604.09076)
*Arbel Hizmi,Artemii Bakulin,Shai Bagon,Nir Yosef*

Main category: cs.CV

TL;DR: The paper proposes a method to transfer spatial transcriptomics information to histology-only models using cross-modal distillation, enabling the prediction of tissue niches from H&E images alone.


<details>
  <summary>Details</summary>
Motivation: Spatial transcriptomics is costly and scarce, while H&E histology is abundant but less granular. The authors aim to leverage paired data to improve histology-based models.

Method: Cross-modal distillation using paired spatial transcriptomics and H&E data to train a histology-only model.
...
Conclusion: The proposed framework enables the application of spatial transcriptomics insights to histology-only data, improving tissue niche prediction without requiring transcriptomic input at inference time.
...
### [62] [Memory-Efficient Transfer Learning with Fading Side Networks via Masked Dual Path Distillation](https://arxiv.org/abs/2604.09088)
*Yutong Zhang,Jiaxin Chen,Honglin Chen,Kaiqi Zheng,Shengcai Liao,Hanwen Zhong,Weixin Li,Yunhong Wang*

Main category: cs.CV

TL;DR: Proposed Masked Dual Path Distillation (MDPD) accelerates inference while retaining efficiency in fine-tuning with fading side networks.


<details>
  <summary>Details</summary>
Motivation: Current memory-efficient transfer learning methods introduce additional memory and time overhead during inference due to the use of a lightweight and learnable side network.

Method: MDPD uses mutual distillation between frozen backbones and learnable side networks during fine-tuning and discards the side network during inference.

Result: MDPD accelerates inference by at least 25.2% while keeping parameter and memory consumption comparable, and promotes accuracy compared to SOTA approaches.

Conclusion: MDPD effectively addresses the issue of additional overhead in current memory-efficient transfer learning methods while maintaining accuracy.
...
### [63] [Off-the-shelf Vision Models Benefit Image Manipulation Localization](https://arxiv.org/abs/2604.09096)
*Zhengxuan Zhang,Keji Song,Junmin Hu,Ao Luo,Yuezun Li*

Main category: cs.CV

TL;DR: A novel adapter called ReVi is proposed to repurpose off-the-shelf general-purpose vision models for image manipulation localization (IML) by selectively enhancing manipulation-specific information.


<details>
  <summary>Details</summary>
Motivation: The gap between image manipulation localization (IML) and general vision tasks is bridged by introducing a fresh perspective that general semantic priors can benefit IML.

Method: A trainable adapter (ReVi) that disentangles semantic redundancy from manipulation-specific information and selectively enhances the latter, fine-tuning only the adapter while keeping the off-the-shelf vision models' parameters frozen.

Result: The experimental results demonstrate the superiority of the proposed method, showing potential for scalable IML frameworks.

Conclusion: The proposed method can effectively repurpose off-the-shelf general-purpose vision models for IML, achieving superior performance with efficient fine-tuning.
...
### [64] [Physically Grounded 3D Generative Reconstruction under Hand Occlusion using Proprioception and Multi-Contact Touch](https://arxiv.org/abs/2604.09100)
*Gabriele Mario Caddeo,Pasquale Marra,Lorenzo Natale*

Main category: cs.CV

TL;DR: The paper proposes a multimodal approach for reconstructing and estimating the pose of objects under severe hand occlusion, leveraging physical interaction signals like proprioception and touch.


<details>
  <summary>Details</summary>
Motivation: Existing occlusion-aware 3D generation methods rely only on vision, which can be ambiguous in occluded regions. The authors aim to improve object reconstruction under occlusion by incorporating physical interaction signals.

Method: The authors propose a multimodal approach that represents object structure as a pose-aware, camera-aligned signed distance field (SDF) and uses a Structure-VAE and a conditional flow-matching diffusion model. They incorporate physics-based objectives and differentiable decoder-guidance to reduce hand-object interpenetration and align the reconstructed surface with contact observations.

Result: Experiments in simulation show that adding proprioception and touch substantially improves completion under occlusion and yields physically plausible reconstructions at correct real-world scale compared to vision-only baselines. The model also transfers well to a real humanoid robot.

Conclusion: The proposed approach enables metric-scale amodal object reconstruction and pose estimation under severe hand occlusion, producing physically consistent structure estimates that can integrate into existing reconstruction pipelines.
...
### [65] [Visually-Guided Policy Optimization for Multimodal Reasoning](https://arxiv.org/abs/2604.09349)
*Zengbin Wang,Feng Xiong,Liang Lin,Xuecai Hu,Yong Wang,Yanlin Wang,Man Zhang,Xiangxiang Chu*

Main category: cs.CV

TL;DR: The paper proposes Visually-Guided Policy Optimization (VGPO) to improve visual faithfulness in vision-language models by reinforcing visual focus during policy optimization.


<details>
  <summary>Details</summary>
Motivation: The inherent text-dominated nature of vision-language models leads to insufficient visual faithfulness, and temporal visual forgetting exacerbates this deficiency.

Method: The proposed VGPO framework includes a Visual Attention Compensation mechanism and a dual-grained advantage re-weighting strategy to amplify visual cues and counteract visual forgetting.
...
Conclusion: The proposed VGPO framework effectively improves visual faithfulness and performance in vision-language models.
...
TL;DR: A novel Dynamic Assembly Forest model (DAF) is proposed to detect diffusion-generated images, offering competitive performance with significantly fewer parameters and lower computational cost compared to DNN-based methods.


<details>
  <summary>Details</summary>
Motivation: The generation of high-quality images by diffusion models poses serious security concerns, and current detection methods rely heavily on deep neural networks, overlooking traditional machine learning models.

Method: A Dynamic Assembly Forest model (DAF) is proposed, built upon the deep forest paradigm, to address limitations in feature learning and scalable training for detecting diffusion-generated images.

Result: DAF achieves competitive performance under standard evaluation protocols with significantly fewer parameters, much lower computational cost, and without requiring GPUs, making it suitable for resource-constrained scenarios.

Conclusion: The proposed DAF model highlights its strong potential as a practical substitute for heavyweight DNN models in detecting diffusion-generated images, especially in resource-constrained scenarios.
...
### [67] [Arbitration Failure, Not Perceptual Blindness: How Vision-Language Models Resolve Visual-Linguistic Conflicts](https://arxiv.org/abs/2604.09364)
*Farhad Nooralahzadeh,Omid Rohanian,Yi Zhang,Jonathan Fürst,Kurt Stockinger*

Main category: cs.CV

TL;DR: The study investigates why Vision-Language Models (VLMs) sometimes provide incorrect answers to visual questions, focusing on whether the issue lies in perception or arbitration. It finds that VLMs can encode visual evidence correctly but fail to act on it, and proposes methods to improve this.


<details>
  <summary>Details</summary>
Motivation: To understand whether the problem of incorrect answers from VLMs when seeing images (e.g., a blue banana being answered as 'yellow') is due to perception or arbitration.

Method: The study uses various methods including Multimodal Arbitration Crossover (MAC) analysis, layer-by-layer Logit Lens probing, full-sequence activation patching, and training-free activation steering (linear and sparse autoencoder-guided) to analyze and intervene in VLMs.

Result: The study reveals an Encoding-Grounding Dissociation in VLMs, where models that provide wrong answers still encode visual evidence as strongly as those providing correct answers. It identifies that the gap in the final-layer logit better predicts grounding outcomes. The study also finds that image tokens carry almost all of the causal impact, while text tokens have none. Finally, it shows that targeted interventions like activation steering in early layers can improve visual grounding.

Conclusion: VLMs already see well, but the challenge is acting on what they see. Targeted interventions can help bridge this gap.
...
TL;DR: The paper introduces FIRE-CIR, a model for composed image retrieval (CIR) in the fashion domain that uses question-driven visual reasoning to improve interpretability and accuracy.


<details>
  <summary>Details</summary>
Motivation: Recent vision-language models for CIR often fail to reason about what to preserve and what to change, leading to suboptimal results, particularly in fine-grained domains like fashion.

Method: FIRE-CIR performs question-driven visual reasoning by automatically generating attribute-focused visual questions and verifying visual evidence in reference and candidate images.

Result: FIRE-CIR outperforms state-of-the-art methods in retrieval accuracy on the Fashion IQ benchmark and provides interpretable, attribute-level insights into retrieval decisions.

Conclusion: The proposed model brings compositional reasoning and interpretability to fashion CIR, yielding better results and providing insights into retrieval decisions.
...
### [69] [VL-Calibration: Decoupled Confidence Calibration for Large Vision-Language Models Reasoning](https://arxiv.org/abs/2604.09529)
*Wenyi Xiao,Xinchi Xu,Leilei Gan*

Main category: cs.CV

TL;DR: The paper proposes VL-Calibration, a reinforcement learning framework to improve the calibration of Large Vision Language Models (LVLMs) by decoupling confidence into visual and reasoning confidence, leading to better performance and reduced hallucinations.


<details>
  <summary>Details</summary>
Motivation: LVLMs exhibit hallucinations and incorrect responses with high certainty, hindering their usage in high-stakes domains. Existing confidence calibration methods for text-only LLMs are not well-suited for LVLMs.

Method: The authors propose a reinforcement learning framework called VL-Calibration, which explicitly decouples confidence into visual and reasoning confidence. They introduce intrinsic visual certainty estimation and token-level advantage reweighting to focus optimization on tokens based on visual certainty.

Result: Experiments on thirteen benchmarks show that VL-Calibration effectively improves calibration while boosting visual reasoning accuracy, and it generalizes to out-of-distribution benchmarks across model scales and architectures.

Conclusion: The proposed VL-Calibration framework addresses the limitations of existing confidence calibration methods for LVLMs, leading to improved performance, reduced hallucinations, and better generalization across different benchmarks and model architectures.
...
TL;DR: Introduces OpenPAE, an open benchmark for personalized age estimation using multiple reference images, showing that personalization improves performance.


<details>
  <summary>Details</summary>
Motivation: Existing age estimation methods ignore individual aging rates; the authors aim to create a benchmark for personalized age estimation using reference images.

Method: The authors propose a hierarchy of baselines, including arithmetic offset, Bayesian linear regression, and a conditional attentive neural process.

Result: Personalization consistently improves performance, with nonlinear methods outperforming simpler alternatives.

Conclusion: The authors introduce OpenPAE, an open benchmark for personalized age estimation, and demonstrate the effectiveness of personalization in age estimation tasks.
...
### [71] [VisionFoundry: Teaching VLMs Visual Perception with Synthetic Images](https://arxiv.org/abs/2604.09531)
*Guanyu Zhou,Yida Yin,Wenhao Chai,Shengbang Tong,Xingyu Fu,Zhuang Liu*

Main category: cs.CV

TL;DR: The paper explores using synthetic supervision generated from task keywords to improve visual perception tasks in vision-language models. It introduces a data generation pipeline called VisionFoundry and a synthetic dataset called VisionFoundry-10K, which leads to substantial improvements on visual perception benchmarks.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the weaknesses of vision-language models in visual perception tasks such as spatial understanding and viewpoint recognition, which is possibly due to limited supervision for low-level visual skills in natural image datasets.

Method: The method involves introducing VisionFoundry, a task-aware synthetic data generation pipeline that uses large language models to generate questions, answers, and text-to-image prompts, then synthesizes images with text-to-image models and verifies consistency with a proprietary VLM.

Result: Models trained on the synthetic dataset VisionFoundry-10K achieve substantial improvements on visual perception benchmarks, specifically +7% on MMVP and +10% on CV-Bench-3D, while preserving broader capabilities and showing favorable scaling behavior as data size increases.

Conclusion: The results suggest that limited task-targeted supervision is an important contributor to the bottleneck in visual perception tasks and that synthetic supervision is a promising path toward more systematic training for vision-language models.
...
TL;DR: FaceLiVTv2, a lightweight face recognition model, achieves better accuracy-efficiency trade-off than existing methods, with significant latency reductions on mobile devices.


<details>
  <summary>Details</summary>
Motivation: The need for lightweight face recognition models that balance accuracy and computational efficiency for deployment on edge and mobile devices.

Method: FaceLiVTv2, a hybrid CNN-Transformer architecture with a lightweight global token interaction module (Lite MHLA) and a unified RepMix block for local and global feature interactions.

Result: FaceLiVTv2 consistently improves accuracy-efficiency trade-off over existing methods on several benchmarks, with latency reductions of up to 41% on mobile devices.

Conclusion: FaceLiVTv2 offers a practical and deployable solution for real-time face recognition on edge and mobile devices, achieving a better balance between accuracy and computational efficiency.
...
TL;DR: A new method called Strips as Tokens (SATO) is proposed for generating high-quality meshes using autoregressive transformers, improving upon existing token ordering strategies.


<details>
  <summary>Details</summary>
Motivation: Existing token ordering strategies for generating meshes with autoregressive transformers fail to meet professional artist standards, leading to inefficient sequences or disrupted edge flow and structural regularity.

Method: The Strips as Tokens (SATO) framework uses a novel token ordering strategy inspired by triangle strips, constructing sequences as connected chains of faces that preserve organized edge flow and semantic layout.

Result: SATO consistently outperforms prior methods in geometric quality, structural coherence, and UV segmentation, and enables joint training on triangle and quadrilateral data.

Conclusion: The proposed SATO framework offers a unified representation for generating high-quality meshes, allowing for flexible decoding into triangle or quadrilateral meshes and demonstrating superior performance over existing methods.
...
TL;DR: The paper proposes GREATEN, a framework that improves Synthetic-to-Realistic Zero-Shot generalization in stereo matching by incorporating surface normals as domain-invariant cues.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of suboptimal generalization performance in image-driven stereo matching due to cross-domain shifts and ambiguities in image textures.

Method: The proposed framework GREATEN consists of three key components: Gated Contextual-Geometric Fusion (GCGF) module, Specular-Transparent Augmentation (STA) strategy, and sparse attention designs.

Result: GREATEN-IGEV achieves outstanding Syn-to-Real performance, reducing errors by 30% on ETH3D, 8.5% on Booster, and 14.1% on KITTI-2015, and runs 19.2% faster than GREAT-IGEV.

Conclusion: The paper concludes that GREATEN effectively improves Syn-to-Real generalization in stereo matching by leveraging surface normals as domain-invariant cues.
...
TL;DR: A deep learning model using CT imaging and clinical data can predict treatment response in high-grade serous ovarian carcinoma patients.


<details>
  <summary>Details</summary>
Motivation: To develop a preoperative prediction tool for Chemotherapy Response Score (CRS) in high-grade serous ovarian carcinoma (HGSOC) patients, as CRS is only available postoperatively and can inform multidisciplinary team discussions.

Method: A 2.5D multimodal deep learning framework integrating pre-trained Vision Transformer encoder with clinical variables to predict CRS.

Result: The model achieved a ROC-AUC of 0.95, 95% accuracy, and 80% precision on the internal test cohort, and 0.68 ROC-AUC, 67% accuracy, and 75% precision on the external test set.

Conclusion: The study demonstrates the feasibility of transformer-based deep learning for preoperative prediction of CRS in HGSOC using routine clinical data and CT imaging, which may assist in multidisciplinary team discussions.
...
### [76] [Deep Light Pollution Removal in Night Cityscape Photographs](https://arxiv.org/abs/2604.09145)
*Hao Wang,Xiaolin Wu,Xi Zhang,Baoqing Sun*

Main category: cs.CV

TL;DR: The paper introduces a method to remove light pollution from nighttime images, restoring the natural night appearance by addressing anisotropic light spread and skyglow.


<details>
  <summary>Details</summary>
Motivation: Nighttime photography is degraded by light pollution, which washes out the view of stars and celestial objects, and current methods do not adequately address this issue.

Method: A physically-based degradation model is proposed, which includes anisotropic spread of directional light sources and skyglow caused by invisible surface lights. A training strategy using large generative models and synthetic-real coupling is also introduced.

Result: The proposed method substantially reduces light pollution artifacts and better recovers authentic night imagery compared to prior nighttime restoration methods.

Conclusion: The paper presents a novel approach to light pollution removal, which effectively restores the pristine night appearance by neutralizing the radiative footprint of ground lighting.
...
### [77] [Neural Distribution Prior for LiDAR Out-of-Distribution Detection](https://arxiv.org/abs/2604.09232)
*Zizhao Li,Zhengkang Xiang,Jiayang Ao,Feng Liu,Joseph West,Kourosh Khoshelham*

Main category: cs.CV

TL;DR: The authors propose Neural Distribution Prior (NDP), a framework to improve out-of-distribution (OOD) object detection in LiDAR-based autonomous driving by adaptively reweighting OOD scores based on a learned distribution prior.


<details>
  <summary>Details</summary>
Motivation: Current LiDAR-based perception models fail to recognize unexpected OOD objects in the open world, and existing OOD scoring functions have limited performance due to ignoring class imbalance in LiDAR OOD detection.

Method: The authors propose NDP, which models the distributional structure of network predictions, corrects class-dependent confidence bias, and uses a Perlin noise-based OOD synthesis strategy to generate diverse auxiliary OOD samples.

Result: NDP substantially improves OOD detection performance, achieving a point-level AP of 61.31% on the STU test set, which is more than 10 times higher than the previous best result.

Conclusion: The proposed NDP framework provides an effective solution for open-world LiDAR perception, compatible with various existing OOD scoring formulations.
...
TL;DR: The paper benchmarks five deep learning architectures for segmenting surgical instruments in robotic-assisted surgery videos, finding that transformer-based models like SegFormer outperform convolutional models like UNet and Attention UNet.


<details>
  <summary>Details</summary>
Motivation: Accurate segmentation of surgical instruments in robotic-assisted surgery is critical for enabling context-aware computer-assisted interventions.

Method: The paper trains and compares five deep learning architectures (UNet, UNet, DeepLabV3, Attention UNet, and SegFormer) on the SAR-RARP50 dataset using a compound loss function combining Cross Entropy and Dice loss.

Result: The experiments reveal that transformer-based architectures like SegFormer achieve improved generalization across varying instrument appearances and surgical conditions, while convolutional models provide strong baseline performance.

Conclusion: The work provides a comprehensive comparison and practical insights for selecting segmentation models in surgical AI applications, highlighting the trade-offs between convolutional and transformer-based approaches.
...
### [79] [Mosaic: Multimodal Jailbreak against Closed-Source VLMs via Multi-View Ensemble Optimization](https://arxiv.org/abs/2604.09253)
*Yuqin Lan,Gen Li,Yuanze Hu,Weihao Shen,Zhaoxin Fan,Faguo Wu,Xiao Zhang,Laurence T. Yang,Zhiming Zheng*

Main category: cs.CV

TL;DR: The paper proposes Mosaic, a framework for multimodal jailbreak attacks on commercial closed-source Vision-Language Models (VLMs), which overcomes surrogate dependency and achieves state-of-the-art attack success rates.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal jailbreak attacks are vulnerable to detection or limited by homogeneous surrogate-target settings, and the authors aim to examine and address the effectiveness of these attacks on commercial closed-source VLMs under heterogeneous settings.

Method: The authors propose Mosaic, a Multi-view ensemble optimization framework that incorporates a Text-Side Transformation module, a Multi-View Image Optimization module, and a Surrogate Ensemble Guidance module to alleviate surrogate dependency.

Result: Mosaic achieves state-of-the-art Attack Success Rate and Average Toxicity against commercial closed-source VLMs on safety benchmarks.

Conclusion: The proposed Mosaic framework effectively addresses the surrogate dependency issue and demonstrates improved performance in multimodal jailbreak attacks on commercial closed-source VLMs.
...
TL;DR: A new framework called MAG-3D is proposed for 3D grounded reasoning using off-the-shelf vision-language models without requiring task-specific training.


<details>
  <summary>Details</summary>
Motivation: The need for flexible and generalizable 3D reasoning in complex scenes, which is currently underexplored.

Method: MAG-3D, a multi-agent framework consisting of a planning agent, a grounding agent, and a coding agent, which work together to address 3D reasoning challenges.

Result: State-of-the-art performance on challenging benchmarks for 3D grounded reasoning across diverse scenes.

Conclusion: MAG-3D enables flexible, training-free 3D grounded reasoning across diverse scenes, achieving state-of-the-art performance.
...
### [81] [PhysInOne: Visual Physics Learning and Reasoning in One Suite](https://arxiv.org/abs/2604.09415)
...
TL;DR: PhysInOne, a large-scale synthetic dataset, is presented to address the scarcity of physically-grounded training data for AI systems, providing 2 million videos across 153,810 dynamic 3D scenes.


<details>
  <summary>Details</summary>
Motivation: The critical scarcity of physically-grounded training data for AI systems.

Method: Creation of PhysInOne, a large-scale synthetic dataset with 2 million videos across 153,810 dynamic 3D scenes, covering 71 basic physical phenomena.

Result: Fine-tuning foundation models on PhysInOne significantly enhances physical plausibility and exposes critical gaps in modeling complex physical dynamics and estimating intrinsic properties.

Conclusion: PhysInOne establishes a new benchmark for advancing physics-grounded world models in generation, simulation, and embodied AI.
...
TL;DR: Elastic Looped Transformers (ELT) introduced for efficient visual generative models with reduced parameters.


<details>
  <summary>Details</summary>
Motivation: Conventional generative models are parameter-inefficient due to deep stacks of unique transformer layers.

Method: ELT uses iterative, weight-shared transformer blocks and Intra-Loop Self Distillation (ILSD) for training.

Result: ELT achieves competitive FID of 2.0 on ImageNet and FVD of 72.8 on UCF-101 with 4x reduced parameters.

Conclusion: ELT significantly improves efficiency for visual synthesis, enabling any-time inference with dynamic trade-offs.
...
### [83] [Rays as Pixels: Learning A Joint Distribution of Videos and Camera Trajectories](https://arxiv.org/abs/2604.09429)
...
TL;DR: A Video Diffusion Model (VDM) called Rays as Pixels is proposed, which jointly learns camera trajectories and video frames, enabling three tasks: predicting camera trajectories, generating video and camera trajectories from images, and generating video along a target camera trajectory.


<details>
  <summary>Details</summary>
Motivation: The traditional separation of camera parameter recovery and scene rendering tasks in computer vision and graphics breaks down when image coverage is sparse or poses are ambiguous.

Method: The proposed VDM represents cameras as dense ray pixels (raxels) and uses a Decoupled Self-Cross Attention mechanism to denoise them jointly with video frames.

Result: The model can handle three tasks and demonstrate self-consistency through a closed-loop test, with trajectory prediction requiring fewer denoising steps than video generation.

Conclusion: The proposed model can learn a joint distribution over videos and camera trajectories, enabling various applications in computer vision and graphics, such as pose estimation and camera-controlled video generation.
...
TL;DR: A new semi-supervised semantic segmentation framework called UniSemAlign is proposed, which improves pixel-wise learning by incorporating explicit class-level structure and achieves state-of-the-art performance on two computational pathology datasets.


<details>
  <summary>Details</summary>
Motivation: The lack of pixel-level annotations and unreliable pseudo-label supervision in computational pathology make semi-supervised semantic segmentation challenging.

Method: UniSemAlign, a dual-modal semantic alignment framework that uses a pathology-pretrained Transformer encoder and introduces complementary prototype-level and text-level alignment branches.

Result: UniSemAlign outperforms recent semi-supervised baselines, achieving Dice improvements of up to 2.6% on GlaS and 8.6% on CRAG with only 10% labeled data.

Conclusion: The proposed UniSemAlign framework effectively enhances visual segmentation by injecting explicit class-level structure into pixel-wise learning, leading to more reliable supervision for unlabeled histopathology images.
...
TL;DR: The paper introduces MixFlow, a training strategy for diffusion models that improves sampling efficiency and generation quality by reducing generative path curvatures.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitation of slow iterative sampling in diffusion models caused by highly curved generative paths, which is partly due to the independence between the source distribution and the data distribution.

Method: The method consists of two main contributions: (1) introducing $κ	exttt{-FC}$, a general formulation that conditions the source distribution on an arbitrary signal $κ$ to better align it with the data distribution, and (2) presenting MixFlow, a training strategy that trains a flow model on linear mixtures of a fixed unconditional distribution and a $κ	exttt{-FC}$-based distribution.

Result: The result shows that MixFlow improves the generation quality by 12% in FID compared to standard rectified flow and 7% compared to previous baselines under a fixed sampling budget.

Conclusion: The conclusion is that MixFlow is a simple but effective training strategy that reduces generative path curvatures, improves sampling efficiency, and accelerates training convergence.
...
### [86] [Globally Optimal Pose from Orthographic Silhouettes](https://arxiv.org/abs/2604.09199)
*Agniva Sengupta,Dilara Kuş,Jianning Li,Stefan Zachow*

Main category: cs.CV

TL;DR: The paper presents a method for determining the 3D pose of known shapes from their 2D silhouettes, achieving global optimality without requiring correspondences.


<details>
  <summary>Details</summary>
Motivation: The problem of determining the pose of known shapes in 3D space from their unoccluded silhouettes is addressed, motivated by the need for efficient and accurate pose estimation.

Method: The proposed method uses pre-computed silhouette-signatures and a response surface of area-of-silhouettes to estimate pose, combined with aspect ratio of 2D ellipses fitted to projected silhouettes.

Result: The method efficiently estimates globally optimal pose from silhouettes, without guidance by correspondences, for any shape, and demonstrates improved accuracy against comparable approaches.

Conclusion: The paper concludes that the proposed method is the first to efficiently estimate globally optimal pose from just silhouettes, irrespective of shape convexity and genus.
...
### [87] [VISOR: Agentic Visual Retrieval-Augmented Generation via Iterative Search and Over-horizon Reasoning](https://arxiv.org/abs/2604.09508)
*Yucheng Shen,Jiulong Wu,Jizhou Huang,Dawei Yin,Lingyong Yan,Min Cao*

Main category: cs.CV

TL;DR: The paper proposes VISOR, a unified single-agent framework for visual retrieval-augmented generation that addresses challenges in cross-page reasoning and search drift, achieving state-of-the-art performance in long-horizon visual reasoning tasks.


<details>
  <summary>Details</summary>
Motivation: Existing agentic visual retrieval-augmented generation systems face bottlenecks in visual evidence sparsity and search drift in long horizons, hindering their ability to perform complex queries requiring multi-step reasoning.

Method: The authors propose VISOR, which features a structured Evidence Space for progressive cross-page reasoning, a Visual Action Evaluation and Correction mechanism, and a Dynamic Trajectory with Sliding Window and Intent Injection to mitigate search drift. VISOR is trained using a Group Relative Policy Optimization-based Reinforcement Learning pipeline.

Result: VISOR achieves state-of-the-art performance with superior efficiency for long-horizon visual reasoning tasks on ViDoSeek, SlideVQA, and MMLongBench datasets.

Conclusion: The proposed VISOR framework effectively addresses the challenges in visual retrieval-augmented generation, providing a unified single-agent solution for complex queries requiring multi-step reasoning.
...
TL;DR: A novel model, CT-1, is proposed for camera-controllable video generation, enabling accurate and flexible camera movements through text prompts.


<details>
  <summary>Details</summary>
Motivation: Existing methods for camera-controllable video generation have limitations, such as imprecise camera control from text prompts or reliance on manual camera trajectory parameters.

Method: The CT-1 model uses a Vision-Language-Camera approach, combining vision-language modules and a Diffusion Transformer model with a Wavelet-based Regularization Loss to learn complex camera trajectory distributions.

Result: The proposed framework yields faithful and high-quality camera-controllable videos, improving camera control accuracy by 25.7% over prior methods.

Conclusion: The CT-1 model successfully bridges the gap between spatial reasoning and video synthesis, enabling spatially aware camera control that aligns with user intentions.
...
### [89] [Envisioning the Future, One Step at a Time](https://arxiv.org/abs/2604.09527)
*Stefan Andreas Baumann,Jannik Wiese,Tommaso Martorella,Mahdi M. Kalayeh,Björn Ommer*

Main category: cs.CV

TL;DR: The authors propose a novel method for predicting future scene dynamics by modeling sparse point trajectories, achieving fast and accurate results.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for predicting future scene dynamics are limited by their reliance on dense video or latent-space prediction, which can be costly and limit performance for long-horizon, multi-modal motion.

Method: The authors formulate the prediction of open-set future scene dynamics as step-wise inference over sparse point trajectories using an autoregressive diffusion model.

Result: The proposed method matches or surpasses dense simulators in predictive accuracy while achieving orders-of-magnitude higher sampling speed.

Conclusion: The authors' dynamics-centric representation enables fast rollout of diverse futures from a single image, maintaining physical plausibility and long-range coherence, making open-set future prediction scalable and practical.
...
### [90] [Long-SCOPE: Fully Sparse Long-Range Cooperative 3D Perception](https://arxiv.org/abs/2604.09206)
...
TL;DR: Long-SCOPE is a sparse framework for robust long-distance cooperative 3D perception in autonomous driving, addressing computational and feature association challenges.


<details>
  <summary>Details</summary>
Motivation: Existing methods for cooperative 3D perception via Vehicle-to-Everything communication are hindered by quadratic computational scaling and fragility of feature association mechanisms at long distances.

Method: Long-SCOPE, a fully sparse framework featuring a Geometry-guided Query Generation module and a learnable Context-Aware Association module.

Result: Long-SCOPE achieves state-of-the-art performance on V2X-Seq and Griffin datasets, particularly in 100-150m long-range settings, with competitive computation and communication costs.

Conclusion: Long-SCOPE offers a robust solution for long-distance cooperative 3D perception, enhancing autonomous driving capabilities.
...
TL;DR: A pipeline for estimating 3D bounding boxes from 2D animal images is proposed, enabling the development of monocular 3D animal detection algorithms.


<details>
  <summary>Details</summary>
Motivation: The lack of labeled datasets for 3D detection methods in RGB animal images, due to the difficulty of labeling 3D structures from 2D projections.

Method: A pipeline utilizing Skinned Multi Animal Linear models and a dedicated camera pose refinement algorithm to estimate 3D bounding boxes and project them into 2D image space.

Result: Accurate performance across species and settings on the Animal3D dataset.

Conclusion: The proposed pipeline is a crucial step toward developing and benchmarking future monocular 3D animal detection algorithms.
...
### [92] [SHIFT: Steering Hidden Intermediates in Flow Transformers](https://arxiv.org/abs/2604.09213)
*Nina Konovalova,Andrey Kuznetsov,Aibek Alanov*

Main category: cs.CV

TL;DR: Proposed SHIFT, a framework for concept removal and manipulation in DiT diffusion models.


<details>
  <summary>Details</summary>
Motivation: To provide control over DiT generation without retraining, enabling concept removal and style/domain manipulation.

Method: SHIFT uses targeted manipulation of intermediate activations and learns steering vectors for dynamic application.

Result: SHIFT provides effective and flexible control over DiT generation across diverse prompts and targets.

Conclusion: SHIFT is a simple, effective, and lightweight solution for concept removal and manipulation in DiT models.
...
### [93] [Seeing is Believing: Robust Vision-Guided Cross-Modal Prompt Learning under Label Noise](https://arxiv.org/abs/2604.09532)
*Zibin Geng,Xuefeng Jiang,Jia Li,Zheng Li,Tian Wen,Lvhua Wu,Sheng Sun,Yuwei Wang,Min Liu*

Main category: cs.CV

TL;DR: The paper proposes VisPrompt, a vision-guided prompt learning framework that improves the robustness of vision-language models under label noise by leveraging visual semantics and adaptive modulation.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the vulnerability of prompt learning to label noise, as visual content is more robust under label noise but the prompt itself is highly susceptible.

Method: The method involves a cross-modal attention mechanism to inject visual semantics into prompt representations and a lightweight conditional modulation mechanism to adaptively control the strength of visual information injection.

Result: The proposed framework, VisPrompt, significantly improves robustness while keeping the pretrained VLM backbone frozen and introducing only a small amount of additional trainable parameters, outperforming existing baselines on seven benchmark datasets.

Conclusion: The paper concludes that VisPrompt effectively suppresses noise-induced disturbances, reduces instability in prompt updates, and alleviates memorization of mislabeled samples, achieving stronger robustness under synthetic and real-world label noise.
...
TL;DR: This paper explores the limits of compact neural video representations, presenting tiny NeRV architectures for efficient deployment in resource-constrained environments.


<details>
  <summary>Details</summary>
Motivation: Most existing studies on Neural Representations for Videos (NeRV) focus on moderate or high-capacity models, leaving the behavior of extremely compact configurations underexplored.

Method: The paper introduces two lightweight NeRV configurations, NeRV-T and NeRV-T+, and investigates strategies for improving compact model performance, including knowledge distillation and low-precision inference.

Result: Experimental results show that carefully designed tiny NeRV variants can achieve favorable quality-efficiency trade-offs while substantially reducing parameter count, computational cost, and memory requirements.

Conclusion: The findings provide insight into the practical limits of compact neural video representations and offer guidance for deploying NeRV-style models in resource-constrained and real-time environments.
...
TL;DR: Hitem3D 2.0 is a framework for generating high-quality 3D textures by integrating 2D multi-view generation priors and native 3D texture representations.


<details>
  <summary>Details</summary>
Motivation: Existing 3D texture generation methods struggle with incomplete texture coverage, cross-view inconsistency, and misalignment between geometry and texture.

Method: Hitem3D 2.0 consists of a multi-view synthesis framework and a native 3D texture generation model, which work together to enhance texture quality.

Result: Hitem3D 2.0 outperforms existing methods in terms of texture detail, fidelity, consistency, coherence, and alignment.

Conclusion: The proposed framework, Hitem3D 2.0, significantly improves 3D texture generation by integrating multi-view consistency constraints with native 3D texture modeling.
...
### [96] [FashionStylist: An Expert Knowledge-enhanced Multimodal Dataset for Fashion Understanding](https://arxiv.org/abs/2604.09249)
*Kaidong Feng,Zhuoxuan Huang,Huizhong Guo,Yuting Jin,Xinyu Chen,Yue Liang,Yifei Gai,Li Zhou,Yunshan Ma,Zhu Sun*

Main category: cs.CV

TL;DR: Introduced FashionStylist, a benchmark for holistic fashion understanding with expert annotations, supporting tasks like outfit grounding, completion, and evaluation.


<details>
  <summary>Details</summary>
Motivation: Existing fashion datasets are fragmented and task-specific, limiting holistic outfit understanding.

Method: Constructed FashionStylist through a dedicated fashion-expert annotation pipeline, providing annotations at item and outfit levels.

Result: FashionStylist serves as a unified benchmark for multiple fashion tasks and improves grounding, completion, and outfit-level semantic evaluation in MLLM-based fashion systems.

Conclusion: FashionStylist enables holistic and expert-level fashion understanding, covering style, occasion, compatibility, and outfit rationale.
...
TL;DR: The paper improves object detection for architectural elements by adding a custom alignment loss to YOLOv8, resulting in more structurally coherent facade parsings.


<details>
  <summary>Details</summary>
Motivation: Standard object detectors treat architectural elements independently, leading to facade parsings that lack structural coherence required for procedural reconstruction.

Method: Augmenting YOLOv8 with a custom lightweight alignment loss to encourage grid-consistent arrangements of bounding boxes during training.

Result: The method improves structural regularity, corrects alignment errors caused by perspective and occlusion, and maintains a controllable trade-off with standard detection accuracy on the CMP dataset.

Conclusion: The proposed method successfully enhances the structural coherence of facade parsings by injecting geometric priors without altering the standard inference pipeline.
...
TL;DR: The paper introduces GeRM, a multi-modal generative rendering model that unifies Physically-Based Rendering (PBR) and Photorealistic Rendering (PRR) to mitigate the gap between them, allowing for controllable photorealistic image generation.


<details>
  <summary>Details</summary>
Motivation: The authors identify a gap between PBR and PRR, where PBR guarantees physical reality but PRR requires additional realistic digital models of geometry and appearance, and current methods face a dilemma between explicit simulation and implicit generation.

Method: The authors propose GeRM, which integrates physical attributes like G-buffers with text prompts and uses progressive incremental injection to generate controllable photorealistic images, and learn a distribution transfer vector field (DTV Field) to guide the transition between PBR and PRR images.

Result: The authors present a multi-condition ControlNet that synthesizes PBR images and progressively transitions them into PRR images, guided by G-buffers, text prompts, and cues for enhanced regions, and construct an expert-guided pairwise P2P transfer dataset, named P2P-50K.

Conclusion: The paper presents a solution to mitigate the gap between PBR and PRR, enabling controllable photorealistic image generation, and introduces a new approach for unified rendering.
...
TL;DR: Proposed VAGNet, a deep neural network that predicts accidents from dash-cam video using global features, outperforming existing methods in precision, time-to-accident, and efficiency.


<details>
  <summary>Details</summary>
Motivation: Traffic accidents are a major cause of fatalities and injuries, and anticipating hazardous situations in advance is crucial for timely intervention and safety.

Method: VAGNet, a deep neural network using transformer and graph modules with VideoMAE-V2 for global feature extraction from dash-cam video, without requiring explicit object-level features.

Result: Experiments on four benchmark datasets show higher average precision and mean time-to-accident, with improved computational efficiency compared to existing methods.

Conclusion: The proposed VAGNet demonstrates a more efficient and effective approach to accident anticipation, leveraging global features from dash-cam video.
...
TL;DR: A novel method called Structure-aware Fine-grained Gaussian Splatting (SFGS) is proposed for reconstructing high-fidelity 3D human avatars from monocular video sequences, capturing fine details such as hand movements and facial expressions.


<details>
  <summary>Details</summary>
Motivation: Existing 3D human avatar modeling approaches struggle to accurately model fine details like hand movements and facial expressions, despite effectively capturing body motion.

Method: SFGS uses spatial-only triplane and time-aware hexplane to capture dynamic features, a structure-aware gaussian module for pose-dependent details, and a residual refinement module for fine-grained hand reconstruction.

Result: SFGS outperforms state-of-the-art baselines, generating high-fidelity avatars with natural motion and fine details, requiring only single-stage training.

Conclusion: The proposed SFGS method effectively reconstructs expressive and coherent full-body 3D human avatars from monocular video sequences, addressing the challenge of capturing fine details.
...
TL;DR: The paper proposes a shift towards event-centric video anomaly detection (VAD), introducing new methods and evaluation standards for detecting anomalous events in videos, and revealing a significant performance gap between frame-level and event-level detection.


<details>
  <summary>Details</summary>
Motivation: Traditional frame-level evaluations in VAD are misaligned with real-world requirements, where anomalies manifest as coherent temporal episodes, not isolated frames. The authors aim to bridge this gap by focusing on event-level detection, localization, and reporting.

Method: The authors propose two strategies for temporal event localization: a score-refinement pipeline with hierarchical Gaussian smoothing and adaptive binarization, and an end-to-end Dual-Branch Model. They also adapt Temporal Action Localization metrics for event-based evaluation.

Result: The results show a substantial performance gap between frame-level and event-level detection. While state-of-the-art models achieve frame-level AUC-ROC exceeding 52%, their event-level localization precision falls below 10% even at a minimal tIoU=0.2, with an average event-level F1 of only 0.11 across all thresholds.

Conclusion: The paper highlights the need for a shift towards event-centric VAD and provides new methods and evaluation standards for detecting anomalous events in videos. The results emphasize the importance of evaluating VAD models at the event level to ensure their reliability in real-world applications.
...
TL;DR: A benchmarking framework called LuMon is proposed for evaluating Monocular Depth Estimation (MDE) methods in lunar exploration, highlighting the domain gap between Earth and lunar environments.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the domain gap in MDE for autonomous lunar rover navigation, as existing evaluations rely on analogs that fail to replicate lunar conditions.

Method: The method involves creating a comprehensive benchmarking framework LuMon, introducing novel datasets with high-quality stereo ground truth depth, and conducting a systematic zero-shot evaluation of state-of-the-art architectures.

Result: The results show that while sim-to-real domain adaptation yields performance gains, it exhibits minimal generalization to authentic lunar imagery, highlighting a persistent cross-domain transfer gap.

Conclusion: The study concludes that current networks have inherent limitations, and LuMon sets a standard foundation to guide future advancements in extraterrestrial perception and domain adaptation.
...
TL;DR: A new framework for reconstructing dynamic 4D scenes is proposed, outperforming state-of-the-art methods by disentangling dynamic and static components and modeling uncertainty.


<details>
  <summary>Details</summary>
Motivation: Reconstructing dynamic 4D scenes is challenging due to motion causing significant geometric ambiguity, and existing 3D foundation models struggle with dynamic sequences.

Method: The framework introduces three mechanisms: Entropy-Guided Subspace Projection, Local-Consistency Driven Geometry Purification, and Uncertainty-Aware Cross-View Consistency.

Result: The approach outperforms current state-of-the-art methods, reducing Mean Accuracy error by 13.43% and improving segmentation F-measure by 10.49%.

Conclusion: The proposed framework efficiently reconstructs dynamic 4D scenes without requiring task-specific fine-tuning or per-scene optimization.
...
TL;DR: EpiAgent, an AI system, restores ancient inscriptions by formulating restoration as a hierarchical planning problem, achieving superior quality and generalization.


<details>
  <summary>Details</summary>
Motivation: Ancient inscriptions are deteriorating due to environmental and human factors, and existing AI methods struggle to restore them effectively due to their rigidity and inability to generalize across complex degradations.

Method: EpiAgent uses a hierarchical planning approach with an LLM-based central planner that coordinates multimodal analysis, historical experience, specialized tools, and self-refinement in an Observe-Conceive-Execute-Reevaluate paradigm.

Result: EpiAgent achieves superior restoration quality and stronger generalization compared to existing methods across real-world degraded inscriptions.

Conclusion: EpiAgent marks an important step toward expert-level agent-driven restoration of cultural heritage, offering a flexible and adaptive restoration process.
...
TL;DR: A new method, RC-GRPO-Editing, is proposed for instruction-guided image editing. It improves editing region instruction adherence and non-target preservation by suppressing background-induced nuisance variance.


<details>
  <summary>Details</summary>
Motivation: Existing methods for instruction-guided image editing suffer from noisy credit assignment due to global exploration perturbing non-target regions. This inflates within-group reward variance and yields noisy GRPO advantages.

Method: The proposed RC-GRPO-Editing framework uses region-decoupled initial noise perturbations and an attention concentration reward to localize exploration and align cross-attention with the intended editing region.

Result: Experiments on CompBench show consistent improvements in editing region instruction adherence and non-target preservation.

Conclusion: The proposed RC-GRPO-Editing framework effectively addresses the challenge of noisy credit assignment in instruction-guided image editing, leading to improved performance in editing region instruction adherence and non-target preservation.
...
TL;DR: A new method called Energy-Guided Latent Optimization for Concept Erasure (EGLOCE) is introduced for safely removing unwanted concepts from text-to-image diffusion models without requiring re-training or modifying model weights.


<details>
  <summary>Details</summary>
Motivation: The need for safely and effectively removing specific concepts, such as explicit content and copyrighted characters or styles, from text-to-image diffusion models to ensure safety and compliance.

Method: EGLOCE, a training-free approach that uses a dual-objective framework with repulsion and retention energies to steer generation away from target concepts while preserving semantic alignment to the original prompt.

Result: EGLOCE improves concept removal while maintaining image quality and prompt alignment across baselines, even with adversarial attacks.

Conclusion: EGLOCE establishes a new paradigm for safe and controllable image generation through dual energy-based guidance during sampling, enabling plug-and-play integration with existing models.
...
TL;DR: A new method for 3D motion estimation using synthetic data generated by a pipeline called SynFlow, which outperforms state-of-the-art methods and can generalize across multiple real-world benchmarks.


<details>
  <summary>Details</summary>
Motivation: The scarcity of dense, high-quality motion annotations for 3D dynamic perception and the limitations of self-supervision on unlabeled real data.

Method: Proposing a data generation pipeline called SynFlow to generate large-scale synthetic dataset for LiDAR scene flow, and training models exclusively on this synthetic data.

Result: Models trained on SynFlow-4k generalize across multiple real-world benchmarks, rivaling in-domain supervised baselines on nuScenes and outperforming state-of-the-art methods on TruckScenes by 31.8%. Fine-tuning with only 5% of real-world labels surpasses models trained from scratch.

Conclusion: The proposed SynFlow pipeline and dataset provide a highly domain-invariant motion prior and a label-efficient foundation for 3D motion estimation, facilitating research in generalizable 3D motion estimation.
...
TL;DR: The study investigates the role of image tokens in Vision Language Models (VLMs) and finds that visual representations rapidly converge and become interchangeable between layers, questioning the assumption that deeper visual processing is uniformly essential.


<details>
  <summary>Details</summary>
Motivation: The motivation is to understand whether sustained image-token processing is necessary for VLMs' performance and if visual representations evolve meaningfully across layers.

Method: The study systematically investigates the functional role of image tokens in VLMs by analyzing the evolution of visual representations across depth, including entropy, intrinsic dimensionality, and trajectory curvature.

Result: The results show that visual representations rapidly converge to a bounded-complexity regime, become largely interchangeable between layers, and that the necessity of visual processing is task-dependent.

Conclusion: The study concludes that deeper visual processing may not be uniformly essential in VLMs, challenging the current paradigm of multimodal LLM architectures.
...
TL;DR: A method called SCoRe is proposed to improve image generation quality from diffusion models trained on noisy datasets by regenerating high-frequency components.


<details>
  <summary>Details</summary>
Motivation: Diffusion models trained on noisy datasets often reproduce high-frequency training artifacts, degrading generation quality.

Method: SCoRe (Spectral Cutoff Regeneration) uses a frequency cutoff to suppress corrupted high-frequency components and regenerates them via SDEdit.

Result: SCoRe outperforms post-processing and noise-robust baselines, restoring samples closer to clean image distributions without retraining or fine-tuning.

Conclusion: SCoRe is an effective, training-free method for clean image generation from diffusion models trained on noisy images.
...
TL;DR: AsymLoc, a novel distillation framework, enables efficient visual localization on edge devices by aligning a lightweight Student model with a large Teacher model, achieving up to 95% of the teacher's accuracy with an order of magnitude smaller models.


<details>
  <summary>Details</summary>
Motivation: The need for precise and real-time visual localization on resource-constrained edge devices, such as smart glasses, while reducing compute without sacrificing accuracy.

Method: AsymLoc, a distillation framework that uses a geometry-driven matching objective and a joint detector-descriptor distillation objective to align a Student model with a Teacher model.

Result: AsymLoc achieves up to 95% of the teacher's localization accuracy using an order of magnitude smaller models, outperforming existing baselines and establishing a new state-of-the-art efficiency-accuracy trade-off.

Conclusion: AsymLoc enables efficient visual localization on edge devices, achieving a better efficiency-accuracy trade-off than existing methods.
...
TL;DR: The paper introduces Immersive Volumetric Videos (IVV) and a dataset called ImViD to create fully immersive experiences in virtual and augmented reality. It presents a pipeline for constructing IVV, including a dynamic light field reconstruction framework and a sound field reconstruction method.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enable fully immersive experiences in virtual and augmented reality by integrating 6-DoF visual and auditory interaction, which is currently largely unexplored for real-world captured videos.

Method: The method involves constructing Immersive Volumetric Videos (IVV) using a multi-view, multi-modal dataset (ImViD) captured with a custom rig, and a unified pipeline including dynamic light field reconstruction and sound field reconstruction.

Result: The result is a pipeline that generates high-quality, temporally stable audiovisual volumetric content with large 6-DoF interaction spaces, demonstrated through extensive benchmarks and immersive VR experiments.

Conclusion: The paper concludes that it provides both a foundational definition and a practical construction methodology for immersive volumetric videos, enabling large 6-DoF interaction spaces, audiovisual feedback, and high-resolution dynamic content.
...
TL;DR: A novel RGB+LiDAR pipeline for high-fidelity mesh reconstruction in complex indoor environments, leveraging semantics to improve geometric accuracy.


<details>
  <summary>Details</summary>
Motivation: Geometric high-fidelity mesh reconstruction from LiDAR-inertial scans is challenging in large, complex indoor environments due to point cloud sparsity, geometric drift, and fixed fusion parameters.

Method: A modular, incremental pipeline that uses a vision foundation model to label RGB frames, incrementally projects and fuses labels onto a LiDAR-inertial odometry map, and applies an incremental semantics-aware TSDF fusion step to produce the final mesh.

Result: The proposed method outperforms state-of-the-art geometric baselines (ImMesh and Voxblox) in terms of geometric reconstruction quality, as evaluated on the Oxford Spires and NTU VIRAL datasets.

Conclusion: The proposed semantics-aided fusion approach improves geometric mesh quality, enabling the creation of high-quality, semantically labelled meshes for applications such as Universal Scene Description (USD) assets, XR, and digital modeling.
...
TL;DR: Online3R is a sequential reconstruction framework that adapts to new scenes through online learning, resolving inconsistency issues.


<details>
  <summary>Details</summary>
Motivation: The paper aims to address the inconsistency issues in sequential reconstruction by adapting to new scenes through online learning.

Method: The framework introduces learnable lightweight visual prompts into a pretrained geometry foundation model and uses a local-global self-supervised learning strategy to update these prompts at test time.

Result: Online3R outperforms previous state-of-the-art methods on various benchmarks.

Conclusion: The proposed Online3R framework effectively resolves inconsistency issues in sequential reconstruction by adapting to new scenes through online learning.
...
TL;DR: A novel framework called Reason and Restore (R&R) is proposed for universal image restoration, integrating structured Chain-of-Thought (CoT) reasoning into the restoration pipeline.


<details>
  <summary>Details</summary>
Motivation: Existing universal image restoration methods lack explicit diagnostic reasoning over degradation composition, severity, and scene semantics prior to restoration.

Method: R&R introduces an explicit reasoner, implemented by fine-tuning Qwen3-VL, to diagnose degradation types, quantify degradation severity, infer key degradation-related factors, and describe relevant scene and object semantics.

Result: R&R achieves state-of-the-art performance across diverse UIR benchmarks and offers unique interpretability into the restoration process.

Conclusion: The proposed R&R framework improves universal image restoration by integrating structured reasoning and provides interpretable diagnostic priors for the restorer.
...
TL;DR: EgoTL is introduced to address the limitations of VLM-based auto-labeling in embodied intelligence, providing a think-aloud capture pipeline for egocentric data to improve long-horizon planning and reasoning.


<details>
  <summary>Details</summary>
Motivation: The primary motivation is to overcome the noisy auto-labeling issue in VLM-based approaches due to inaccurate human action labels, chain-of-thought, and spatial annotations, which hinder their performance in long-horizon spatial instruction following.

Method: EgoTL builds a think-aloud capture pipeline using a say-before-act protocol to record step-by-step goals and spoken reasoning, then calibrates physical properties with metric-scale spatial estimators, a memory-bank walkthrough, and clip-level tags.

Result: The results show that foundation models still have limitations as egocentric assistants or open-world simulators, but fine-tuning them with human CoT aligned with metric labels on EgoTL improves long-horizon planning and reasoning, step-wise reasoning, instruction following, and spatial grounding.

Conclusion: The introduction of EgoTL and the fine-tuning of foundation models with human-aligned data lead to significant improvements in embodied intelligence, particularly in long-horizon planning and reasoning tasks.
...
TL;DR: A new framework called Tango is proposed to improve token pruning in Video Large Language Models (Video LLMs) by enhancing attention-based token selection and preserving geometric structure.


<details>
  <summary>Details</summary>
Motivation: Existing token pruning methods have limitations, including conventional top-k selection strategies failing to account for attention distribution and direct similarity-based clustering generating fragmented clusters.

Method: Tango integrates a diversity-driven strategy for attention-based token selection and introduces Spatio-temporal Rotary Position Embedding (ST-RoPE) to preserve geometric structure.

Result: Tango preserves 98.9% of the original performance on LLaVA-OV while delivering a 1.88x inference speedup when retaining only 10% of the video tokens.

Conclusion: The proposed Tango framework demonstrates effectiveness and generalizability across various Video LLMs and video understanding benchmarks, optimizing the utilization of visual signals in token pruning.
...
TL;DR: Develops a mathematical framework to study how AI-generated text affects the public record and its impact on AI training corpora.


<details>
  <summary>Details</summary>
Motivation: The increasing influence of AI-generated text on the public record and its recursive learning process.

Method: Exactly solvable mathematical framework based on variable-order n-gram agents.

Result: Characterizes stable distributions, drift, and selection forces acting on the public corpus, with implications for AI training corpora design.

Conclusion: The framework identifies conditions under which recursive publication compresses public text or selective filtering sustains richer structure.
...
TL;DR: A new framework called SynDocDis generates synthetic physician-to-physician dialogues while preserving patient privacy, achieving high ratings in communication effectiveness and medical content quality.


<details>
  <summary>Details</summary>
Motivation: To address the gap in synthesizing physician-to-physician communication and overcome restrictions on accessing clinical data due to privacy regulations and ethical considerations.

Method: Combining structured prompting techniques with de-identified case metadata to generate clinically accurate physician dialogues.

Result: The framework achieved a mean communication effectiveness rating of 4.4/5 and a mean medical content quality rating of 4.1/5, with 91% clinical relevance ratings.

Conclusion: SynDocDis is a promising framework for advancing medical AI research ethically and responsibly, with applications in medical education and clinical decision support.
...
TL;DR: The paper explores the benefits of efficient sequence models over simple temporal averaging, finding that exponential moving average (EMA) traces can encode temporal structure and achieve high performance on certain tasks.


<details>
  <summary>Details</summary>
Motivation: To understand what efficient sequence models gain over simple temporal averaging and to investigate the capabilities of fixed-coefficient accumulation.

Method: Using EMA traces as a controlled probe to map the boundary between what fixed-coefficient accumulation can and cannot represent, and evaluating its performance on grammatical role assignment and language modeling tasks.

Result: EMA traces achieve 96% of a supervised BiGRU on grammatical role assignment with zero labels and reach C4 perplexity 260 (8x GPT-2) on a 130M-parameter language model.

Conclusion: Fixed-coefficient accumulation suffers irreversible information dilution that only learned, input-dependent selection can resolve, and EMA traces apply lossy, data-independent compression.
...
TL;DR: The safety alignment of diffusion-based language models (dLLMs) is vulnerable to a simple two-step intervention, revealing that their safety is not robust but rather shallow and relies on a single fragile assumption.


<details>
  <summary>Details</summary>
Motivation: The authors aim to investigate the safety alignment of dLLMs and its potential vulnerabilities.

Method: The authors analyze the denoising schedule of dLLMs and propose a trivial two-step intervention to exploit their safety alignment.

Result: The proposed intervention achieves 76.1% and 81.8% ASR against LLaDA-8B-Instruct and Dream-7B-Instruct, respectively, without requiring gradient computation or adversarial search.

Conclusion: The findings suggest that dLLM safety is not adversarially robust but rather architecturally shallow, and propose potential defenses to improve safety.
...
### [121] [WAND: Windowed Attention and Knowledge Distillation for Efficient Autoregressive Text-to-Speech Models](https://arxiv.org/abs/2604.08558)
*Hanna Lee,Tan Dat Nguyen,Jaehoon Kang,Kyuhong Shim*

Main category: cs.CL

TL;DR: The paper proposes WAND, a framework that adapts autoregressive text-to-speech models to operate with constant computational and memory complexity, achieving up to 66.2% memory reduction and near-constant latency.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the scalability issue of autoregressive text-to-speech models, which have quadratic memory and compute costs due to full self-attention, making them inefficient for long sequences.

Method: The method proposed is WAND (Windowed Attention and Knowledge Distillation), which separates attention into persistent global attention and local sliding-window attention, and uses curriculum learning and knowledge distillation to stabilize fine-tuning and recover high-fidelity synthesis quality.

Result: The results show that WAND preserves the original quality of the models while achieving up to 66.2% KV cache memory reduction and length-invariant, near-constant per-step latency.

Conclusion: The conclusion is that WAND is an effective framework for adapting autoregressive text-to-speech models to operate efficiently with constant computational and memory complexity, making them more suitable for practical applications.
...
### [122] [Medical Reasoning with Large Language Models: A Survey and MR-Bench](https://arxiv.org/abs/2604.08559)
*Xiaohan Ren,Chenxiao Fan,Wenyin Ma,Hongliang He,Chongming Gao,Xiaoyan Zhao,Fuli Feng*

Main category: cs.CL

TL;DR: The paper reviews medical reasoning with large language models (LLMs), evaluating their performance in real-world clinical settings and identifying gaps between current model capabilities and clinical requirements.


<details>
  <summary>Details</summary>
Motivation: The deployment of LLMs in clinical settings requires robust medical reasoning, which depends on more than factual recall. The authors aim to provide a comprehensive review of medical reasoning with LLMs.

Method: The authors conceptualize medical reasoning as an iterative process of abduction, deduction, and induction, and organize existing methods into seven major technical routes. They also conduct a unified cross-benchmark evaluation of representative medical reasoning models.

Result: The evaluation on MR-Bench, a benchmark derived from real-world hospital data, exposes a pronounced gap between exam-level performance and accuracy on authentic clinical decision tasks.

Conclusion: The survey highlights key gaps between current model performance and the requirements of real-world clinical reasoning, providing a unified view of existing medical reasoning methods, benchmarks, and evaluation practices.
...
### [123] [Uncertainty Estimation for the Open-Set Text Classification systems](https://arxiv.org/abs/2604.08560)
*Leonid Erlygin,Alexey Zaytsev*

Main category: cs.CL

TL;DR: The paper proposes a method for uncertainty estimation in open-set text classification, adapting Holistic Uncertainty Estimation (HolUE) for the text domain, achieving significant improvements in Prediction Rejection Ratio (PRR) over a baseline.


<details>
  <summary>Details</summary>
Motivation: Accurate uncertainty estimation is crucial for building robust and trustworthy recognition systems, particularly in open-set text classification where texts may be classified as known or rejected as unknown.

Method: The paper adapts the Holistic Uncertainty Estimation (HolUE) method for the text domain to address text uncertainty and gallery uncertainty, which are major causes of prediction errors in text recognition systems.

Result: The proposed method, HolUE, achieves 40-365% improvement in Prediction Rejection Ratio (PRR) over the quality-based SCF baseline across various datasets, including Yahoo Answers, DBPedia, PAN authorship attribution, and CLINC150 intent classification.

Conclusion: The paper concludes that by capturing different sources of uncertainty, it becomes possible to predict when the system will make a recognition error, and the proposed HolUE method outperforms the baseline in open-set text classification tasks.
...
### [124] [A Representation-Level Assessment of Bias Mitigation in Foundation Models](https://arxiv.org/abs/2604.08561)
*Svetoslav Nizhnichenkov,Rahul Nair,Elizabeth Daly,Brian Mac Namee*

Main category: cs.CL

TL;DR: The paper analyzes how bias mitigation affects the embedding space of foundation models, finding that it reduces disparities and leads to more neutral representations.


<details>
  <summary>Details</summary>
Motivation: To investigate the effect of bias mitigation on the internal behavior of foundation models through representational analysis.

Method: Using BERT and Llama2 as test models, comparing baseline and bias-mitigated variants to assess shifts in associations between gender and occupation terms.

Result: Bias mitigation reduces gender-occupation disparities in the embedding space, leading to more neutral and balanced internal representations.

Conclusion: Embedding analysis is a valuable tool for understanding and validating debiasing methods in foundation models, with findings applicable across encoder-only and decoder-only models.
...
### [125] [Neural networks for Text-to-Speech evaluation](https://arxiv.org/abs/2604.08562)
*Ilya Trofimenko,David Kocharyan,Aleksandr Zaitsev,Pavel Repnikov,Mark Levin,Nikita Shevtsov*

Main category: cs.CL

TL;DR: The study proposes novel neural models to approximate human judgments for evaluating Text-to-Speech (TTS) systems, achieving high accuracy and outperforming human inter-rater baselines.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenges of human subjective evaluation protocols for TTS systems, which are expensive, slow, and sensitive to assessor biases.

Method: The study formulates and implements a suite of neural models, including NeuralSBS, MOSNet, and WhisperBert, to approximate expert judgments in relative and absolute settings.

Result: The best MOS models achieve a Root Mean Square Error (RMSE) of ~0.40, significantly outperforming the human inter-rater RMSE baseline of 0.62.

Conclusion: The study demonstrates the effectiveness of ensemble-based stacking over direct latent fusion and the necessity of dedicated metric learning frameworks for evaluating TTS systems.
...
### [126] [Temperature-Dependent Performance of Prompting Strategies in Extended Reasoning Large Language Models](https://arxiv.org/abs/2604.08563)
*Mousa Salah,Amgad Muneer*

Main category: cs.CL

TL;DR: The paper explores the optimal configuration of sampling temperature and prompting strategy for extended reasoning models in Large Language Models (LLMs).


<details>
  <summary>Details</summary>
Motivation: The optimal configuration of sampling temperature and prompting strategy for extended reasoning models remains largely underexplored.

Method: The authors systematically evaluate chain-of-thought and zero-shot prompting across four temperature settings using Grok-4.1 with extended reasoning on 39 mathematical problems.

Result: Zero-shot prompting achieves peak performance at moderate temperatures (59% accuracy at T=0.4 and T=0.7), while chain-of-thought prompting performs best at temperature extremes. The benefit of extended reasoning increases from 6x at T=0.0 to 14.3x at T=1.0.

Conclusion: Temperature should be optimized jointly with prompting strategy, challenging the common practice of using T=0 for reasoning tasks.
...
### [127] [Attention-Based Sampler for Diffusion Language Models](https://arxiv.org/abs/2604.08564)
*Yuyan Zhou,Kai Syun Hou,Weiyu Chen,James Kwok*

Main category: cs.CL

TL;DR: The paper proposes a new decoding algorithm, Attn-Sampler, for diffusion-based large language models (dLLMs) that achieves superior generation quality and enhances decoding parallelism.


<details>
  <summary>Details</summary>
Motivation: Current dLLMs decoding strategies rely primarily on token level information, which fails to account for global sequence structure and often yields suboptimal results.

Method: The proposed method uses attention matrix column sums to determine the decoding order, and instantiates this theoretical insight in a new training-free decoding algorithm, termed Attn-Sampler.

Result: Extensive experiments across multiple benchmarks validate the effectiveness of the proposed method, demonstrating that it achieves superior generation quality while enhancing the decoding parallelism.

Conclusion: The paper provides a principled justification for attention-guided decoding and offers a theoretically grounded alternative to greedy search, and proposes a block attention approximation and dynamic attention thresholding for practical acceleration.
...
### [128] [Dynamic sparsity in tree-structured feed-forward layers at scale](https://arxiv.org/abs/2604.08565)
*Reza Sedghi,Robin Schiewer,Anand Subramoney,David Kappel*

Main category: cs.CL

TL;DR: The paper explores using sparse, tree-structured feed-forward layers in transformer models as a replacement for dense MLP blocks, achieving efficient computation without sacrificing performance.


<details>
  <summary>Details</summary>
Motivation: The feed-forward MLP block accounts for a large share of a transformer's compute budget, motivating the search for sparse alternatives to dense MLP blocks.

Method: The paper studies sparse, tree-structured feed-forward layers as drop-in replacements for MLP blocks, enabling conditional computation via hard hierarchical routing.

Result: The models with sparse, tree-structured feed-forward layers match dense baselines under controlled training and fine-tuning protocols, despite activating fewer than 5% of the feed-forward block's units per token.

Conclusion: The work demonstrates that tree-structured feed-forward layers provide a scalable and controllable mechanism for sparsifying large transformer models.
...
### [129] [Sentiment Classification of Gaza War Headlines: A Comparative Analysis of Large Language Models and Arabic Fine-Tuned BERT Models](https://arxiv.org/abs/2604.08566)
*Amr Eleraqi,Hager H. Mustafa,Abdul Hadi N. Ahmed*

Main category: cs.CL

TL;DR: The study compares how different AI models interpret sentiment in conflict-related media discourse, finding significant variations in sentiment classifications between models.


<details>
  <summary>Details</summary>
Motivation: The study aims to examine how different AI architectures interpret sentiment in conflict-related media discourse, highlighting the risks of treating automated sentiment outputs as neutral or interchangeable measures of media tone in contexts of war and crisis.

Method: The study uses a corpus of 10,990 Arabic news headlines and conducts a comparative analysis between three large language models and six fine-tuned Arabic BERT models, employing information-theoretic and distributional metrics to quantify systematic differences across models.

Result: The results reveal pronounced and non-random divergence in sentiment distributions, with fine-tuned BERT models exhibiting a strong bias toward neutral classifications and LLMs consistently amplifying negative sentiment.

Conclusion: The study suggests that the choice of model constitutes a choice of interpretive lens, shaping how conflict narratives are algorithmically framed and emotionally evaluated, and highlights the risks of treating automated sentiment outputs as neutral or interchangeable measures of media tone in contexts of war and crisis.
...
TL;DR: The study examines the limitations of large language models (LLMs) in multi-user settings, where they must serve multiple users with different preferences and authority levels, revealing gaps in instruction following, privacy preservation, and coordination.


<details>
  <summary>Details</summary>
Motivation: The increasing deployment of LLMs in team workflows and organizational tools requires them to serve multiple users simultaneously, but existing systems are optimized for single-principal interaction, leading to conflicts, information asymmetry, and privacy constraints.

Method: The study formalizes multi-user interaction with LLM agents as a multi-principal decision problem and introduces a unified multi-user interaction protocol, designing three stress-testing scenarios to evaluate LLMs' capabilities.

Result: The results show that frontier LLMs frequently fail to prioritize users' objectives, exhibit privacy violations, and suffer from efficiency bottlenecks in multi-user settings.

Conclusion: The study highlights the need for further development of LLMs to handle multi-user interactions effectively, addressing gaps in instruction following, privacy preservation, and coordination.
...
### [131] [Can We Still Hear the Accent? Investigating the Resilience of Native Language Signals in the LLM Era](https://arxiv.org/abs/2604.08568)
*Nabelanita Utami,Sasano Ryohei*

Main category: cs.CL

TL;DR: The study examines if writing assistance tools, specifically large language models (LLMs), are homogenizing research papers by analyzing native language identification trends in ACL Anthology papers.


<details>
  <summary>Details</summary>
Motivation: To investigate whether the shift from machine translation to LLMs in writing assistance tools is leading to homogenization of research papers.

Method: A semi-automated framework was used to construct a labeled dataset and fine-tune a classifier to detect linguistic fingerprints of author backgrounds across three eras: pre-neural network, pre-LLM, and post-LLM.

Result: The analysis shows a consistent decline in native language identification performance over time, with anomalies in the post-LLM era, including resistance or divergent trends in Chinese and French, and sharper declines in Japanese and Korean.

Conclusion: The study suggests that while there is a trend towards homogenization, certain languages exhibit unexpected patterns, indicating a complex relationship between writing assistance tools and linguistic diversity in research papers.
...
### [132] [Adaptive Rigor in AI System Evaluation using Temperature-Controlled Verdict Aggregation via Generalized Power Mean](https://arxiv.org/abs/2604.08595)
*Aleksandr Meshkov*

Main category: cs.CL

TL;DR: This paper introduces Temperature-Controlled Verdict Aggregation (TCVA), a method for evaluating LLM-based AI systems that adapts its strictness to the application domain.


<details>
  <summary>Details</summary>
Motivation: Existing evaluation methods for LLM-based AI systems do not align well with human assessment because they cannot adapt their strictness to the application domain.

Method: TCVA combines a five-level verdict-scoring system with generalized power-mean aggregation and a temperature parameter T to control evaluation rigor.

Result: TCVA achieves correlation with human judgments comparable to RAGAS on faithfulness while consistently outperforming DeepEval, and requires no additional LLM calls when adjusting the temperature parameter.

Conclusion: TCVA provides a flexible and effective method for evaluating LLM-based AI systems, allowing for adaptation to different application domains.
...
### [133] [EXAONE 4.5 Technical Report](https://arxiv.org/abs/2604.08644)
...
TL;DR: LG AI Research releases EXAONE 4.5, an open-weight vision language model with enhanced document understanding and long-context reasoning capabilities.


<details>
  <summary>Details</summary>
Motivation: The development of EXAONE 4.5 is motivated by the need for a model that can handle multimodal pretraining, particularly in document-centric corpora, and deliver improved performance in document understanding and related tasks.

Method: EXAONE 4.5 integrates a dedicated visual encoder into the existing EXAONE 4.0 framework, allowing for native multimodal pretraining over both visual and textual modalities. The model is trained on large-scale, carefully curated data with an emphasis on document-centric corpora.

Result: EXAONE 4.5 achieves competitive performance in general benchmarks and outperforms state-of-the-art models of similar scale in document understanding and Korean contextual reasoning. The model also extends context length up to 256K tokens, facilitating long-context reasoning and enterprise-scale use cases.

Conclusion: EXAONE 4.5 is a significant advancement in vision language models, offering enhanced capabilities in document understanding, long-context reasoning, and multimodal pretraining. The model is designed for practical industrial deployment and will be continuously extended with additional domains and application scenarios.
...
### [134] [Decomposing the Delta: What Do Models Actually Learn from Preference Pairs?](https://arxiv.org/abs/2604.08723)
...
TL;DR: The paper investigates the impact of preference data on reasoning performance in language models, identifying two key factors: generator-level delta and sample-level delta.


<details>
  <summary>Details</summary>
Motivation: Understanding what drives downstream reasoning gains in language models when using preference optimization methods.

Method: The study varies generator-level delta by changing the generator's scale and model family, and sample-level delta by using an LLM-as-a-judge to rate generated traces.

Result: Increasing generator-level delta improves out-of-domain reasoning tasks, and filtering data by sample-level delta enables more data-efficient training.

Conclusion: A twofold recipe for improving reasoning performance through preference optimization is proposed: maximize generator-level delta and exploit sample-level delta.
...
### [135] [LLMs Underperform Graph-Based Parsers on Supervised Relation Extraction for Complex Graphs](https://arxiv.org/abs/2604.08752)
*Paolo Gajo,Domenic Rosati,Hassan Sajjad,Alberto Barrón-Cedeño*

Main category: cs.CL

TL;DR: Large language models (LLMs) are outperformed by a graph-based parser in relation extraction tasks, especially when dealing with complex linguistic graphs.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this work is to evaluate the performance of large language models (LLMs) in relation extraction tasks, particularly in scenarios with complex linguistic graphs, and compare them to a graph-based parser.

Method: The authors evaluate four LLMs against a graph-based parser on six relation extraction datasets with sentence graphs of varying sizes and complexities.

Result: The results show that the graph-based parser increasingly outperforms the LLMs as the number of relations in the input documents increases.

Conclusion: The graph-based parser is a superior choice for relation extraction tasks, especially in the presence of complex linguistic graphs, due to its superior performance and lighter architecture.
...
### [136] [Cards Against LLMs: Benchmarking Humor Alignment in Large Language Models](https://arxiv.org/abs/2604.08757)
*Yousra Fettach,Guillaume Bied,Hannu Toivonen,Tijl De Bie*

Main category: cs.CL

TL;DR: This study explores the humor judgment of Large Language Models (LLMs) by having them play Cards Against Humanity with humans, finding modest alignment with human preferences and significant agreement among models, but also identifying biases and artifacts in their humor judgment.


<details>
  <summary>Details</summary>
Motivation: The study aims to investigate the humor dimension of Large Language Model (LLM) alignment, which is a crucial aspect of human communication but has been largely unexplored.

Method: The study uses five frontier language models to play Cards Against Humanity (CAH) with human players, selecting the funniest response from ten candidate cards across 9,894 rounds.

Result: The models exceed the random baseline but show modest alignment with human preferences, with significant agreement among models, and systematic position biases and content preferences are identified as contributing factors.

Conclusion: The study raises questions about whether LLM humor judgment reflects genuine preference or structural artifacts of inference and alignment, highlighting the need for further research in this area.
...
### [137] [Revisiting Anisotropy in Language Transformers: The Geometry of Learning Dynamics](https://arxiv.org/abs/2604.08764)
*Raphael Bernas,Fanny Jourdan,Antonin Poché,Céline Hudelot*

Main category: cs.CL

TL;DR: The paper explores the anisotropy phenomenon in Transformer architectures used in NLP, providing geometric arguments and empirical evidence that supports a tangent-aligned account of anisotropy.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the challenge of geometric interpretation of Transformer architectures due to the inherent anisotropy phenomenon, which previous theoretical studies have rarely grounded in the underlying representation geometry.

Method: The paper derives geometric arguments for how frequency-biased sampling affects curvature visibility and uses concept-based mechanistic interpretability during training to fit activation-derived low-rank tangent proxies, testing them against ordinary backpropagated true gradients.

Result: The study finds that activation-derived directions capture both unusually large gradient energy and a substantially larger share of gradient anisotropy than matched-rank normal controls, across encoder-style and decoder-style language models.

Conclusion: The paper provides strong empirical support for a tangent-aligned account of anisotropy in Transformer architectures, suggesting that training preferentially amplifies tangent directions.
...
### [138] [MT-OSC: Path for LLMs that Get Lost in Multi-Turn Conversation](https://arxiv.org/abs/2604.08782)
*Jyotika Singh,Fang Tu,Miguel Ballesteros,Weiyi Sun,Sandip Ghoshal,Michelle Yuan,Yassine Benajiba,Sujith Ravi,Dan Roth*

Main category: cs.CL

TL;DR: Introduced MT-OSC, a framework that efficiently condenses chat history for large language models in multi-turn interactions, reducing token counts and improving performance.


<details>
  <summary>Details</summary>
Motivation: Large language models suffer performance degradation in multi-turn interactions due to context window limitations, leading to increased latency and computational costs.

Method: MT-OSC uses a Condenser Agent with a few-shot inference-based Condenser and a lightweight Decider to selectively retain essential information in chat history.

Result: MT-OSC reduces token counts by up to 72% and consistently improves or preserves accuracy across 13 state-of-the-art LLMs and diverse multi-turn benchmarks.

Conclusion: MT-OSC is a scalable solution for multi-turn chats, enabling richer context within constrained input spaces while reducing latency and operational cost.
...
### [139] [MedConceal: A Benchmark for Clinical Hidden-Concern Reasoning Under Partial Observability](https://arxiv.org/abs/2604.08788)
*Yikun Han,Joey Chan,Jingyuan Chen,Mengting Ai,Simo Du,Yue Guo*

Main category: cs.CL

TL;DR: The paper introduces MedConceal, a benchmark for evaluating medical dialogue systems' ability to reason about hidden patient concerns, and finds that no single system dominates, with frontier models performing well on confirmation metrics but human clinicians remaining strongest on intervention success.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the challenge of asymmetric information in patient-clinician communication, where patients often do not disclose their fears, misconceptions, or practical barriers unless clinicians elicit them skillfully.

Method: The paper presents MedConceal, a benchmark with an interactive patient simulator for evaluating hidden-concern reasoning in medical dialogue, comprising 300 curated cases and 600 clinician-LLM interactions.

Result: The results show that no single system dominates: frontier models lead on different confirmation metrics, while human clinicians (N=159) remain strongest on intervention success.

Conclusion: The paper concludes that hidden-concern reasoning under partial observability is a key unresolved challenge for medical dialogue systems.
...
### [140] [Lessons Without Borders? Evaluating Cultural Alignment of LLMs Using Multilingual Story Moral Generation](https://arxiv.org/abs/2604.08797)
*Sophie Wu,Andrew Piper*

Main category: cs.CL

TL;DR: The paper introduces a novel task called multilingual story moral generation to evaluate cultural alignment in language models. It uses a dataset of human-written story morals across 14 language-culture pairs to compare model outputs with human interpretations.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to assess the ability of language models to understand and generate story morals that are culturally grounded and diverse, as story interpretation varies across linguistic and cultural contexts.

Method: The method involves using a dataset of human-written story morals collected across 14 language-culture pairs to compare model outputs (specifically from GPT-4o and Gemini) with human interpretations through semantic similarity, human preference survey, and value categorization.

Result: The results show that frontier models like GPT-4o and Gemini generate story morals that are semantically similar to human responses and are preferred by human evaluators. However, their outputs exhibit less cross-linguistic variation and focus on a narrower set of widely shared values.

Conclusion: The conclusion is that while contemporary models can approximate central tendencies of human moral interpretation, they struggle to reproduce the diversity that characterizes human narrative understanding, highlighting a limitation in their cultural alignment.
...
### [141] [Cross-Lingual Attention Distillation with Personality-Informed Generative Augmentation for Multilingual Personality Recognition](https://arxiv.org/abs/2604.08851)
*Jing Jie Tan,Ban-Hoe Kwan,Danny Wee-Kiat Ng,Yan-Chai Hum,Noriyuki Kawarazaki,Kosuke Takano*

Main category: cs.CL

TL;DR: The paper proposes ADAM, a state-of-the-art approach for multilingual personality recognition, leveraging cross-lingual attention distillation and personality-guided generative augmentation to improve performance across languages.


<details>
  <summary>Details</summary>
Motivation: The lack of multilingual datasets for personality recognition is a significant challenge, and the authors aim to address this by developing a method to generate high-quality training data in multiple languages.

Method: The authors propose ADAM, which uses Cross-Lingual Attention Distillation (CLAD) and Personality-Informed Generative Augmentation (PIGA) to train a model capable of understanding and recognizing personality traits across languages.

Result: The findings demonstrate that CLAD significantly outperforms the standard BCE across all languages and personality traits, achieving notable improvements in average BA scores on two datasets, and the CLAD-trained model shows strong generalizability and comparable performance to leading encoder models.

Conclusion: The proposed ADAM approach effectively advances multilingual personality recognition by leveraging cross-lingual attention distillation and personality-guided generative augmentation, demonstrating improved performance and generalizability across languages.
...
### [142] [GRASP: Grounded CoT Reasoning with Dual-Stage Optimization for Multimodal Sarcasm Target Identification](https://arxiv.org/abs/2604.08879)
*Faxian Wan,Xiaocui Yang,Yifan Cao,Shi Feng,Daling Wang,Yifei Zhang*

Main category: cs.CL

TL;DR: The paper proposes GRASP, a framework for Multimodal Sarcasm Detection and Target Identification, which uses visual grounding and explicit Chain-of-Thought reasoning to improve interpretability and fine-grained localization.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for Multimodal Sarcasm Detection rely on implicit cross-modal alignment, which has limited interpretability and suboptimal fine-grained localization. The authors aim to address these limitations.

Method: The authors propose GRASP, which integrates visual grounding with explicit Chain-of-Thought (CoT) reasoning. They also curate a refined dataset, MSTI-MAX, and employ a dual-stage outcome-supervised joint optimization strategy.

Result: GRASP outperforms existing baselines in fine-grained sarcasm target identification across modalities. An LLM-as-a-Judge evaluation quantitatively measures the quality of internal reasoning chains.

Conclusion: The proposed framework, GRASP, improves the interpretability and fine-grained localization of Multimodal Sarcasm Detection and Target Identification, and demonstrates superior performance on the curated dataset.
...
### [143] [NCL-BU at SemEval-2026 Task 3: Fine-tuning XLM-RoBERTa for Multilingual Dimensional Sentiment Regression](https://arxiv.org/abs/2604.08923)
*Tong Wu,Nicolay Rusnachenko,Huizhi Liang*

Main category: cs.CL

TL;DR: This paper presents a system for Dimensional Aspect-Based Sentiment Analysis (DimABSA) that predicts continuous valence-arousal scores for aspects in text using a fine-tuning approach with XLM-RoBERTa-base.


<details>
  <summary>Details</summary>
Motivation: The motivation is to extend traditional Aspect-Based Sentiment Analysis (ABSA) from categorical polarity labels to continuous valence-arousal (VA) regression.

Method: A fine-tuning approach based on XLM-RoBERTa-base is used, with dual regression heads and sigmoid-scaled outputs for valence and arousal prediction. Separate models are trained for each language-domain combination.

Result: The fine-tuning approach substantially and consistently outperforms several large language models, including GPT-5.2, LLaMA-3-70B, LLaMA-3.3-70B, and LLaMA-4-Maverick, across all evaluation datasets.

Conclusion: The proposed system demonstrates the effectiveness of task-specific fine-tuning for DimABSA, and the code is publicly available.
...
### [144] [MuTSE: A Human-in-the-Loop Multi-use Text Simplification Evaluator](https://arxiv.org/abs/2604.08947)
*Rares-Alexandru Roscan,Gabriel Petre1,Adrian-Marius Dumitran,Angela-Liliana Dumitran*

Main category: cs.CL

TL;DR: The paper introduces MuTSE, a web application for evaluating Large Language Model (LLM) generated text simplifications across various prompts and architectures.


<details>
  <summary>Details</summary>
Motivation: The lack of structured, visual frameworks for comparative text analysis hinders the development of robust prompts for LLMs in text simplification, particularly in NLP research and Intelligent Tutoring Systems.

Method: The authors propose MuTSE, an interactive human-in-the-loop web application that supports concurrent execution of prompt-model permutations, generating a comprehensive comparison matrix in real-time.

Result: MuTSE enables the systematic multi-dimensional evaluation of prompt-model permutations, reducing cognitive load and facilitating reproducible, structured annotation for downstream NLP dataset construction.

Conclusion: MuTSE addresses the limitations in evaluating LLM-generated text simplifications by providing a structured, visual framework for comparative text analysis, supporting the development of robust prompts and improving the evaluation process.
...
### [145] [TaxPraBen: A Scalable Benchmark for Structured Evaluation of LLMs in Chinese Real-World Tax Practice](https://arxiv.org/abs/2604.08948)
*Gang Hu,Yating Chen,Haiyan Ding,Wang Gao,Jiajia Huang,Min Peng,Qianqian Xie,Kun Yu*

Main category: cs.CL

TL;DR: Introduced TaxPraBen, a benchmark for Chinese taxation practice, evaluating 19 LLMs in 13 tasks, revealing performance disparities among them.


<details>
  <summary>Details</summary>
Motivation: Large Language Models (LLMs) have gaps in the specialized Chinese tax domain, and existing tax-related benchmarks focus on isolated NLP tasks, not real-world practical capabilities.

Method: TaxPraBen combines 10 traditional tasks with 3 real-world scenarios, using a structured evaluation paradigm for end-to-end tax practice assessment.

Result: Significant performance disparities among 19 LLMs, with closed-source large-parameter LLMs excelling, and Chinese LLMs like Qwen2.5 generally exceeding multilingual LLMs.

Conclusion: TaxPraBen serves as a vital resource for advancing evaluations of LLMs in practical applications, highlighting the need for improvement in the Chinese tax domain.
...
### [146] [Towards Real-world Human Behavior Simulation: Benchmarking Large Language Models on Long-horizon, Cross-scenario, Heterogeneous Behavior Traces](https://arxiv.org/abs/2604.08362)
...
TL;DR: The paper introduces OmniBehavior, a user simulation benchmark built from real-world data, and finds that current Large Language Models (LLMs) struggle to accurately simulate complex human behaviors, exhibiting structural biases.


<details>
  <summary>Details</summary>
Motivation: Existing user simulation benchmarks are limited to isolated scenarios, narrow action spaces, or synthetic data, failing to capture authentic human behavior.

Method: The authors introduce OmniBehavior, a benchmark constructed from real-world data, and evaluate state-of-the-art LLMs on this benchmark.

Result: Current LLMs struggle to simulate complex behaviors, with performance plateauing as context windows expand, and exhibit structural biases such as hyper-activity, persona homogenization, and a Utopian bias.

Conclusion: The study highlights critical directions for future high-fidelity simulation research, emphasizing the need to address structural biases and capture individual differences and long-tail behaviors.
...
TL;DR: A new framework called MAB-DQA is proposed to improve document question answering by effectively utilizing visual layouts in page images.


<details>
  <summary>Details</summary>
Motivation: The current multimodal Retrieval-Augmented Generation (RAG) approach struggles to utilize a large number of images effectively, often overlooking informative but less visually salient content.

Method: The proposed MAB-DQA framework decomposes a query into aspect-aware subqueries, retrieves aspect-specific candidate sets, and uses a Multi-Armed Bandit approach to dynamically reallocate retrieval budgets toward high-value aspects.

Result: MAB-DQA shows an average improvement of 5%-18% over the state-of-the-art method on four benchmarks, consistently enhancing document understanding.

Conclusion: The proposed MAB-DQA framework effectively addresses the limitations of current multimodal RAG approaches, leading to improved document understanding.
...
TL;DR: The paper proposes Anchor-based History-stable Decoding (AHD), a dynamic decoding strategy that improves the efficiency and performance of diffusion large language models (dLLMs) by identifying stable tokens and initiating early cross-block decoding.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the block constraints in Semi-autoregressive (Semi-AR) decoding used in dLLMs, which cause delayed decoding of stable tokens, and to improve the efficiency and performance of dLLMs.

Method: The method proposed in this paper is Anchor-based History-stable Decoding (AHD), a training-free, plug-and-play dynamic decoding strategy that monitors the stability trend of tokens in real-time and initiates early cross-block decoding once a token reaches stability.

Result: The results show that AHD simultaneously improves both performance and inference efficiency across language, vision-language, and audio-language domains. Specifically, on the BBH benchmark, AHD reduces decoding steps by 80% while improving performance by 3.67%.

Conclusion: The paper concludes that AHD effectively reverses the performance degradation typically observed in existing advanced decoding acceleration strategies and is a promising approach for improving the efficiency and performance of dLLMs.
...
### [149] [Litmus (Re)Agent: A Benchmark and Agentic System for Predictive Evaluation of Multilingual Models](https://arxiv.org/abs/2604.08970)
*Avni Mittal,Shanu Kumar,Sandipan Dandapat,Monojit Choudhury*

Main category: cs.CL

TL;DR: The paper proposes a benchmark and a system, Litmus (Re)Agent, for predictive multilingual evaluation, estimating model performance on tasks in target languages with missing benchmark results.


<details>
  <summary>Details</summary>
Motivation: The problem of sparse evaluation coverage and uneven published evidence across languages, tasks, and model families in multilingual deployment.

Method: Introduction of a controlled benchmark with 1,500 questions across six tasks and five evidence scenarios, and Litmus (Re)Agent, a DAG-orchestrated agentic system for synthesizing predictions.

Result: Litmus (Re)Agent achieves the best overall performance across six systems, particularly in transfer-heavy scenarios with weak or absent direct evidence.

Conclusion: Structured agentic reasoning is a promising approach to multilingual performance estimation under incomplete evidence.
...
### [150] [Confident in a Confidence Score: Investigating the Sensitivity of Confidence Scores to Supervised Fine-Tuning](https://arxiv.org/abs/2604.08974)
*Lorenzo Jaime Yu Flores,Cesare Spinoso di-Piano,Jackie Chi Kit Cheung*

Main category: cs.CL

TL;DR: This paper investigates the impact of supervised fine-tuning on the correlation between confidence scores and output quality in language models, finding that the correlation degrades post fine-tuning.


<details>
  <summary>Details</summary>
Motivation: The motivation is to understand the sensitivity of confidence scores to supervised fine-tuning, as recent work found that fine-tuning can affect the correlation between confidence scores and quality.

Method: The authors investigate the underlying behavior of confidence scores before and after supervised fine-tuning (SFT) and demonstrate the impact via a case study.

Result: The correlation of various confidence scores with output quality degrades post-SFT, which can stem from changes in confidence scores due to factors other than output quality.

Conclusion: Confidence metrics cannot be used off-the-shelf without testing, and there is a need for developing metrics which are more robust to fine-tuning.
...
### [151] [Quantisation Reshapes the Metacognitive Geometry of Language Models](https://arxiv.org/abs/2604.08976)
*Jon-Paul Cacioli*

Main category: cs.CL

TL;DR: Model quantization affects metacognitive efficiency in LLMs unevenly across domains, and domain-level M-ratio profiles are format-dependent.


<details>
  <summary>Details</summary>
Motivation: Improving metacognition in LLMs through domain-conditional training and understanding the impact of model quantization on metacognitive efficiency.

Method: Evaluating Llama-3-8B-Instruct on 3,000 questions at Q5_K_M and f16 precision, and conducting a pre-registered attempt to improve metacognition through domain-conditional training.

Result: Model quantization restructures domain-level metacognitive efficiency unevenly, with Arts & Literature and Geography showing opposite trends; domain-conditional training did not improve meta-d' due to format dependency.

Conclusion: Any system relying on domain-level M-ratio profiles has an unexamined dependency on inference format, while systems using AUROC_2 are safer.
...
### [152] [Testing the Assumptions of Active Learning for Translation Tasks with Few Samples](https://arxiv.org/abs/2604.08977)
*Lorenzo Jaime Yu Flores,Cesare Spinoso di-Piano,Ori Ernst,David Ifeoluwa Adelani,Jackie Chi Kit Cheung*

Main category: cs.CL

TL;DR: Active learning strategies fail to outperform random sampling on language generation tasks with limited annotated samples, and their core assumptions about informativeness and diversity are not correlated with test set performance.


<details>
  <summary>Details</summary>
Motivation: To understand why active learning (AL) strategies perform poorly on language generation tasks when only a few samples are annotated.

Method: Investigating the core assumptions underlying AL strategies and analyzing their correlation with test set performance.

Result: The study finds that neither informativeness nor diversity of training data, which AL strategies optimize for, are correlated with test set performance. Instead, factors like training sample ordering and interactions with pre-training data have a larger impact on performance.

Conclusion: Future AL methods must consider factors like training sample ordering and interactions with pre-training data to work effectively with very few samples.
...
### [153] [PerMix-RLVR: Preserving Persona Expressivity under Verifiable-Reward Alignment](https://arxiv.org/abs/2604.08986)
*Jihwan Oh,Soowon Oh,Murad Aghazada,Minchan Jeong,Sungnyun Kim,Se-Young Yun*

Main category: cs.CL

TL;DR: The paper proposes PerMix-RLVR, a method to train large language models to adapt to diverse personas while preserving task performance, mitigating the trade-off between persona robustness and fidelity.


<details>
  <summary>Details</summary>
Motivation: Identifying an optimal persona for large language models is time-consuming and its impact on output quality is not well understood, and prior work has mainly addressed this issue at the prompt level via inference-time strategies.

Method: The paper proposes PerMix-RLVR, a persona-mixed RLVR strategy that combines reinforcement learning with verifiable rewards (RLVR) and persona mixing to mitigate the persona robustness-fidelity trade-off.

Result: PerMix-RLVR improves persona stability score (PSS) over RLVR by +21.2% on MATH500 and enhances persona fidelity by +11.4% on PersonaGym.

Conclusion: The paper concludes that PerMix-RLVR is an effective method for training large language models to adapt to diverse personas while preserving task performance, and mitigating the trade-off between persona robustness and fidelity.
...
### [154] [ASTRA: Adaptive Semantic Tree Reasoning Architecture for Complex Table Question Answering](https://arxiv.org/abs/2604.08999)
*Xiaoke Guo,Songze Li,Zhiqiang Liu,Zhaoyan Gong,Yuanxiang Liu,Huajun Chen,Wen Zhang*

Main category: cs.CL

TL;DR: The paper proposes ASTRA, a new architecture for table serialization in complex table question answering, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Existing table serialization methods for Large Language Models (LLMs) in complex table question answering are hindered by challenges such as structural neglect, representation gaps, and reasoning opacity.

Method: The proposed ASTRA architecture consists of two main modules: AdaSTR, which reconstructs tables into Logical Semantic Trees using LLMs' global semantic awareness, and DuTR, a dual-mode reasoning framework that integrates tree-search-based textual navigation and symbolic code execution.

Result: Experiments on complex table benchmarks demonstrate that the proposed method achieves state-of-the-art (SOTA) performance.

Conclusion: The proposed ASTRA architecture effectively addresses the limitations of existing table serialization methods, providing a more effective approach for complex table question answering.
...
### [155] [Towards Linguistically-informed Representations for English as a Second or Foreign Language: Review, Construction and Application](https://arxiv.org/abs/2604.09008)
*Wenxi Li,Xihao Wang,Weiwei Sun*

Main category: cs.CL

TL;DR: The paper proposes a novel resource for English as a Second or Foreign Language (ESFL), treating constructions as fundamental units of analysis to model syntax-semantics interface, and demonstrates its utility in Second Language Acquisition research.


<details>
  <summary>Details</summary>
Motivation: The widespread use of ESFL has sparked a paradigm shift, recognizing it as a distinct linguistic system, and highlighting the need for dedicated representations of ESFL.

Method: The paper surveys existing ESFL resources, identifies their limitations, and proposes a novel solution grounded in constructivist theories, treating constructions as fundamental units of analysis.

Result: The proposed resource comprises 1643 annotated ESFL sentences, capturing a wide range of ESFL phenomena while preserving its unique characteristics.

Conclusion: The paper demonstrates the practical utility of the proposed resource in Second Language Acquisition research, specifically in testing the Linguistic Niche Hypothesis.
...
### [156] [CONDESION-BENCH: Conditional Decision-Making of Large Language Models in Compositional Action Space](https://arxiv.org/abs/2604.09029)
*Yeonjun Hwang,Sungyong Park,Minju Kim,Dongha Lee,Jinyoung Yeo*

Main category: cs.CL

TL;DR: The paper introduces CONDESION-BENCH, a new benchmark for evaluating large language models (LLMs) in conditional decision-making with compositional action spaces, addressing limitations in existing benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing decision-making benchmarks for LLMs make unrealistic assumptions about actions and their feasibility, failing to capture real-world complexities.

Method: The authors propose CONDESION-BENCH, which defines actions as allocations to decision variables with explicit conditions at multiple levels, and uses oracle-based evaluation for decision quality and condition adherence.

Result: The paper presents a more rigorous assessment of LLMs as decision-support tools, but does not provide specific results or performance metrics.

Conclusion: The paper concludes that CONDESION-BENCH provides a more realistic and challenging benchmark for evaluating LLMs in conditional decision-making scenarios.
...
### [157] [Anchored Sliding Window: Toward Robust and Imperceptible Linguistic Steganography](https://arxiv.org/abs/2604.09066)
*Ruiyi Yan,Shiao Meng,Yugo Murawaki*

Main category: cs.CL

TL;DR: The paper proposes the anchored sliding window (ASW) framework to improve the imperceptibility and robustness of linguistic steganography based on language models.


<details>
  <summary>Details</summary>
Motivation: Previous methods are fragile to minor modifications and compromise text quality by limiting the context window.

Method: The ASW framework anchors the prompt and a bridge context within the context window, and optimizes the bridge context using a variant of prompt distillation and self-distillation strategies.

Result: The ASW framework significantly and consistently outperforms the baseline method in text quality, imperceptibility, and robustness across diverse settings.

Conclusion: The proposed ASW framework improves the imperceptibility and robustness of linguistic steganography based on language models.
...
### [158] [NyayaMind- A Framework for Transparent Legal Reasoning and Judgment Prediction in the Indian Legal System](https://arxiv.org/abs/2604.09069)
...
TL;DR: NyayaMind is an open-source framework for transparent and scalable legal reasoning in the Indian judiciary, improving explanation quality and evidence alignment in court judgment prediction and explanation.


<details>
  <summary>Details</summary>
Motivation: The need for transparent and structured legal reasoning in court judgment prediction and explanation systems to make them practically useful in judicial or legal research settings.

Method: NyayaMind integrates retrieval, reasoning, and verification mechanisms, consisting of a Retrieval Module using a RAG pipeline and a Prediction Module using reasoning-oriented LLMs fine-tuned for the Indian legal domain.

Result: NyayaMind significantly improves the quality of explanation and evidence alignment compared to existing CJPE approaches.

Conclusion: NyayaMind provides a promising step toward trustworthy AI-assisted legal decision support systems.
...
### [159] [Hierarchical Alignment: Enforcing Hierarchical Instruction-Following in LLMs through Logical Consistency](https://arxiv.org/abs/2604.09075)
*Shu Yang,Zihao Zhou,Di Wang,Wenda Li*

Main category: cs.CL

TL;DR: The paper proposes Neuro-Symbolic Hierarchical Alignment (NSHA) to handle instruction conflicts in large language models by modeling and enforcing instruction priorities.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the issue of instruction conflicts in real-world applications, where models receive multiple instructions from heterogeneous sources with different authority levels.

Method: The authors propose NSHA, which uses solver-guided reasoning to formulate instruction resolution as a constraint satisfaction problem and distills solver-based decisions into model parameters using automatically constructed supervision.

Result: The authors evaluate NSHA on various tasks, including rule following, task execution, tool use, and safety, and show that it significantly improves performance under instruction conflicts while maintaining competitive utility in reference settings.

Conclusion: The paper concludes that NSHA is an effective approach for handling instruction conflicts in large language models, enabling them to respect instruction priorities while preserving task utility and behavioral consistency.
...
### [160] [Interactive ASR: Towards Human-Like Interaction and Semantic Coherence Evaluation for Agentic Speech Recognition](https://arxiv.org/abs/2604.09121)
...
TL;DR: The paper proposes an agentic framework for interactive automatic speech recognition (ASR) that integrates semantic-aware evaluation and multi-turn interaction.


<details>
  <summary>Details</summary>
Motivation: The dominant evaluation metric, Word Error Rate (WER), has limitations in reflecting semantic correctness, and interactive correction has been underexplored in ASR research.

Method: The proposed framework leverages LLM-as-a-Judge as a semantic-aware evaluation metric and designs an LLM-driven agent framework for simulating human-like multi-turn interaction.

Result: Experiments on standard benchmarks demonstrate the effectiveness of the proposed framework in improving semantic fidelity and interactive correction capability.

Conclusion: The paper presents a novel framework for interactive ASR that improves semantic correctness and interactive correction, with released code to facilitate future research.
...
### [161] [Prototype-Regularized Federated Learning for Cross-Domain Aspect Sentiment Triplet Extraction](https://arxiv.org/abs/2604.09123)
*Zongming Cai,Jianhang Tang,Zhenyong Zhang,Jinghui Qin,Kebing Jin,Hankz Hankui Zhuo*

Main category: cs.CL

TL;DR: Proposed PCD-SpanProto, a federated learning framework for Aspect Sentiment Triplet Extraction (ASTE) that enables cross-domain knowledge transfer while preserving data privacy.


<details>
  <summary>Details</summary>
Motivation: Existing ASTE methods are trained on individual datasets, failing to capture common feature representations across domains, and data privacy constraints prevent centralized data aggregation.

Method: PCD-SpanProto, a prototype-regularized federated learning framework that exchanges class-level prototypes between clients, using a weighted performance-aware aggregation strategy and contrastive regularization.

Result: Outperforms baselines on four ASTE datasets, reducing communication costs and validating the effectiveness of prototype-based cross-domain knowledge transfer.

Conclusion: PCD-SpanProto effectively enables cross-domain knowledge transfer for ASTE while preserving data privacy, demonstrating improved performance and reduced communication costs.
...
### [162] [Think Less, Know More: State-Aware Reasoning Compression with Knowledge Guidance for Efficient Reasoning](https://arxiv.org/abs/2604.09150)
*Yi Sui,Chaozhuo Li,Dawei Song*

Main category: cs.CL

TL;DR: The paper proposes a framework called STACK, which aims to improve the efficiency and accuracy of Large Reasoning Models (LRMs) by compressing their Chain-of-Thought (CoT) reasoning process.


<details>
  <summary>Details</summary>
Motivation: LRMs achieve strong performance on complex tasks but suffer from overthinking, leading to excessive reasoning steps and high inference latency. Existing CoT compression methods struggle to balance accuracy and efficiency.

Method: The proposed framework, STACK, performs step-wise CoT compression by modeling stage-specific redundancy sources and integrating with retrieval-augmented guidance. It dynamically switches between knowledge-guided compression and self-prompted compression based on the reasoning state.
...
Conclusion: The proposed STACK framework effectively balances accuracy and efficiency in LRMs by adaptively compressing the CoT reasoning process, demonstrating its potential for improving the performance of LRMs on complex tasks.
...
### [163] [Facet-Level Tracing of Evidence Uncertainty and Hallucination in RAG](https://arxiv.org/abs/2604.09174)
*Passant Elchafei,Monorama Swain,Shahed Masoudian,Markus Schedl*

Main category: cs.CL

TL;DR: The paper introduces a facet-level diagnostics framework to analyze how Retrieval-Augmented Generation (RAG) systems use evidence, revealing that hallucinations are often due to misintegration of retrieved evidence rather than retrieval accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing evaluations of RAG systems focus on answer-level or passage-level accuracy, but do not provide insight into how evidence is used during generation, leading to a need for a more detailed analysis.

Method: The authors introduce a facet-level diagnostics framework that decomposes questions into atomic reasoning facets, assesses evidence sufficiency and grounding, and analyzes retrieval-generation misalignment through controlled inference modes.

Result: The study finds that hallucinations in RAG systems are driven more by how retrieved evidence is integrated during generation than by retrieval accuracy, and identifies recurring facet-level failure modes such as evidence absence, misalignment, and prior-driven overrides.

Conclusion: The paper concludes that a facet-level analysis is necessary to expose systematic evidence override and misalignment patterns in RAG systems, which remain hidden under answer-level evaluation.
...
### [164] [Do LLMs Follow Their Own Rules? A Reflexive Audit of Self-Stated Safety Policies](https://arxiv.org/abs/2604.09189)
*Avni Mittal*

Main category: cs.CL

TL;DR: The paper introduces a framework called Symbolic-Neural Consistency Audit (SNCA) to evaluate the consistency between the stated safety policies of Large Language Models (LLMs) and their actual behavior.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the issue that LLMs' internal safety policies, learned through Reinforcement Learning from Human Feedback (RLHF), are not formally specified or easily inspectable, and existing benchmarks do not measure if models understand and enforce their own stated boundaries.

Method: The method used in this paper is the SNCA framework, which extracts a model's self-stated safety rules via structured prompts, formalizes them as typed predicates, and measures behavioral compliance against harm benchmarks.

Result: The results show systematic gaps between stated policy and observed behavior in four frontier models across 45 harm categories and 47,496 observations, including models frequently complying with harmful prompts despite stating absolute refusal, and low cross-model agreement on rule types.

Conclusion: The conclusion of this paper is that the gap between what LLMs say and what they do is measurable and architecture-dependent, and reflexive consistency audits are motivated as a complement to behavioral benchmarks.
...
### [165] [ScheMatiQ: From Research Question to Structured Data through Interactive Schema Discovery](https://arxiv.org/abs/2604.09237)
*Shahar Levy,Eliya Habba,Reshef Mintz,Barak Raveh,Renana Keydar,Gabriel Stanovsky*

Main category: cs.CL

TL;DR: ScheMatiQ is a tool that uses LLMs to automatically generate a schema and extract structured evidence from large document collections, supporting real-world analysis in various domains.


<details>
  <summary>Details</summary>
Motivation: The traditional process of manually designing an annotation schema and labeling a corpus is slow and error-prone, and there is a need for a more efficient and effective method.

Method: ScheMatiQ leverages calls to a backbone LLM to take a question and a corpus to produce a schema and a grounded database, with a web interface for steering and revising the extraction.

Result: ScheMatiQ yields outputs that support real-world analysis in law and computational biology, and is released as open source with a public web interface.

Conclusion: ScheMatiQ has the potential to facilitate structured evidence extraction across disciplines, and its open-source release invites experts to use it with their own data.
...
### [166] [EthicMind: A Risk-Aware Framework for Ethical-Emotional Alignment in Multi-Turn Dialogue](https://arxiv.org/abs/2604.09265)
*Jiawen Deng,Wei Li,Wentao Zhang,Ziyun Jiao,Fuji Ren*

Main category: cs.CL

TL;DR: The paper proposes EthicMind, a framework for intelligent dialogue systems to improve emotional attunement and ethical judgment in multi-turn interactions.


<details>
  <summary>Details</summary>
Motivation: Intelligent dialogue systems are increasingly used in sensitive settings where failures in emotional attunement or ethical judgment can cause harm. Existing models address empathy and ethical safety in isolation and fail to adapt to evolving ethical risk and user emotion.

Method: The authors formulate ethical-emotional alignment in dialogue as a turn-level decision problem and propose EthicMind, a risk-aware framework that analyzes ethical risk signals and user emotion, plans a response strategy, and generates context-sensitive replies.

Result: Experimental results show that EthicMind achieves more consistent ethical guidance and emotional engagement than competitive baselines, particularly in high-risk and morally ambiguous scenarios.

Conclusion: The paper concludes that EthicMind is an effective framework for improving emotional attunement and ethical judgment in intelligent dialogue systems, particularly in complex and sensitive interactions.
...
### [167] [Task-Aware LLM Routing with Multi-Level Task-Profile-Guided Data Synthesis for Cold-Start Scenarios](https://arxiv.org/abs/2604.09377)
*Hui Liu,Bin Zou,Kecheng Chen,Jie Liu,Wenya Wang,Haoliang Li*

Main category: cs.CL

TL;DR: The paper proposes a framework for routing large language models (LLMs) to meet user-specific cost-performance trade-offs, addressing the limitation of existing routers in cold-start scenarios.


<details>
  <summary>Details</summary>
Motivation: Existing routers generalize poorly in cold-start scenarios where in-domain training data is unavailable, and there is a need for a system that can select LLMs to meet user-specific cost-performance trade-offs.

Method: A multi-level task-profile-guided data synthesis framework is proposed to construct a hierarchical task taxonomy and produce diverse question-answer pairs. A task-type-aware router approach, TRouter, is introduced to model query-conditioned cost and performance via latent task-type variables.

Result: The proposed synthesis framework alleviates cold-start issues, and TRouter delivers effective LLM routing across multiple benchmarks.

Conclusion: The paper presents a solution to address the limitation of existing routers in cold-start scenarios, providing a framework for effective LLM routing that meets user-specific cost-performance trade-offs.
...
### [168] [Automated Instruction Revision (AIR): A Structured Comparison of Task Adaptation Strategies for LLM](https://arxiv.org/abs/2604.09418)
*Solomiia Bilyk,Volodymyr Getmanskyi,Taras Firman*

Main category: cs.CL

TL;DR: The paper compares methods for adapting large language models to specific tasks with limited examples, finding that performance depends on the task and no single method excels across all tasks.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to investigate and compare different adaptation strategies for large language models (LLMs) when fine-tuning with limited task-specific examples.

Method: The paper uses a rule-induction-based method called Automated Instruction Revision (AIR) and compares it with other approaches such as prompt optimization, retrieval-based methods, and fine-tuning across a diverse benchmark suite.

Result: The results show that adaptation performance is strongly task-dependent, with AIR performing strongest or near-best on label-remapping classification, while other methods like KNN retrieval and fine-tuning performed better on tasks like closed-book QA and structured extraction, respectively.

Conclusion: The paper concludes that the choice of adaptation method depends on the specific task requirements, and AIR is most promising for tasks that can be captured by compact, interpretable instruction rules.
...
### [169] [UIPress: Bringing Optical Token Compression to UI-to-Code Generation](https://arxiv.org/abs/2604.09442)
*Dasen Dai,Shuoqi Li,Ronghao Chen,Huacan Wang,Biao Wu,Qizhen Lan*

Main category: cs.CL

TL;DR: A new method called UIPress is proposed for efficient UI-to-Code generation, which uses a lightweight learned compression module to reduce visual tokens and achieve better performance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of visual token efficiency in UI-to-Code generation, as existing methods do not truly reduce prefill latency or adapt to non-uniform information density of UI screenshots.

Method: UIPress, a lightweight learned compression module, is inserted between the frozen ViT encoder and the LLM decoder of Qwen3-VL-8B, using depthwise-separable convolutions, element-guided spatial reweighting, and Transformer refinement to compress visual tokens.

Result: UIPress achieves a CLIP score of 0.8127, outperforming the uncompressed baseline by +7.5% and the strongest inference-time method by +4.6%, while delivering 9.1× time-to-first-token speedup.

Conclusion: UIPress is an effective and efficient method for UI-to-Code generation, achieving better performance and faster inference time with a small number of trainable parameters.
...
### [170] [Many-Tier Instruction Hierarchy in LLM Agents](https://arxiv.org/abs/2604.09443)
*Jingyu Zhang,Tianjian Li,William Jurayj,Hongyuan Zhan,Benjamin Van Durme,Daniel Khashabi*

Main category: cs.CL

TL;DR: The paper proposes a new paradigm, Many-Tier Instruction Hierarchy (ManyIH), for resolving instruction conflicts in large language model agents with arbitrarily many privilege levels, and introduces a benchmark, ManyIH-Bench, to evaluate models' ability to navigate conflicting instructions.


<details>
  <summary>Details</summary>
Motivation: The dominant paradigm, instruction hierarchy (IH), is inadequate for real-world agentic settings where conflicts can arise across many sources and contexts, and current models perform poorly in resolving instruction conflicts.

Method: The authors propose ManyIH, a paradigm for resolving instruction conflicts among instructions with arbitrarily many privilege levels, and introduce ManyIH-Bench, a benchmark comprising 853 agentic tasks with up to 12 levels of conflicting instructions.

Result: Experiments show that even current frontier models perform poorly (~40% accuracy) when instruction conflict scales, underscoring the need for methods that target fine-grained, scalable instruction conflict resolution.

Conclusion: The paper highlights the need for improved methods for resolving instruction conflicts in large language model agents, and proposes ManyIH and ManyIH-Bench as a step towards addressing this challenge.
...
### [171] [From Reasoning to Agentic: Credit Assignment in Reinforcement Learning for Large Language Models](https://arxiv.org/abs/2604.09459)
*Chenchen Zhang*

Main category: cs.CL

TL;DR: The paper surveys 47 credit assignment methods for reinforcement learning in large language models, proposing a taxonomy and providing resources for future research.


<details>
  <summary>Details</summary>
Motivation: The credit assignment problem in reinforcement learning for large language models is challenging due to the difficulty in determining which actions caused the outcome in long trajectories.

Method: The paper surveys existing methods, organizing them into a two-dimensional taxonomy by assignment granularity and methodology, and provides a structured paper inventory, a reporting checklist, and a benchmark protocol specification.

Result: The survey suggests that the shift from reasoning to agentic RL complicates and reshapes the credit assignment landscape, with new approaches emerging in agentic RL.

Conclusion: The paper provides a comprehensive survey of credit assignment methods and contributes reusable resources to facilitate future research in this area.
...
### [172] [Across the Levels of Analysis: Explaining Predictive Processing in Humans Requires More Than Machine-Estimated Probabilities](https://arxiv.org/abs/2604.09466)
*Sathvik Nair,Colin Phillips*

Main category: cs.CL

TL;DR: The paper critiques and extends claims about language models and their role in language processing, and outlines future directions combining large language models with psycholinguistic models.


<details>
  <summary>Details</summary>
Motivation: To critique and extend claims about language models (LMs) and language processing under Marr's levels of analysis.

Method: Critique and extension of existing claims about LMs and language processing using Marr's levels of analysis.

Result: Outlined future directions for combining large language models (LLMs) with psycholinguistic models.

Conclusion: The paper provides a critical analysis of the role of language models in language processing and suggests future directions for integrating LLMs with psycholinguistic models.
...
### [173] [Agentic Jackal: Live Execution and Semantic Value Grounding for Text-to-JQL](https://arxiv.org/abs/2604.09470)
*Vishnu Murali,Anmol Gulati,Elias Lumer,Kevin Frank,Sindy Campagna,Vamse Kumar Subbiah*

Main category: cs.CL

TL;DR: The paper introduces Jackal, a large-scale benchmark for translating natural language to Jira Query Language (JQL), and proposes Agentic Jackal, a tool-augmented agent that improves LLM accuracy.


<details>
  <summary>Details</summary>
Motivation: Current LLMs struggle with translating natural language to JQL due to ambiguous field references, instance-specific categorical values, and complex Boolean predicates.

Method: The authors propose Agentic Jackal, which equips LLMs with live query execution and a semantic retrieval tool called JiraAnchor, and evaluate it on the Jackal benchmark.

Result: The agentic approach improves LLM accuracy, with a 9.0% relative gain on the most linguistically challenging variant, and categorical-value accuracy rising from 48.7% to 71.7%.

Conclusion: The paper highlights the challenges of text-to-JQL translation, identifies dominant failure modes, and provides a benchmark and evaluation code to support future research.
...
### [174] [RecaLLM: Addressing the Lost-in-Thought Phenomenon with Explicit In-Context Retrieval](https://arxiv.org/abs/2604.09494)
*Kyle Whitecross,Negin Rahimi*

Main category: cs.CL

TL;DR: The paper proposes RecaLLM, a reasoning language model that effectively uses long-context information by interleaving reasoning and explicit in-context retrieval, achieving strong performance on long-context benchmarks.


<details>
  <summary>Details</summary>
Motivation: The interaction between in-context retrieval and reasoning is underexplored, and existing LLMs suffer from a 'lost-in-thought' bottleneck where reasoning steps hinder subsequent in-context retrieval.

Method: RecaLLM alternates between reasoning and retrieving context information, using a constrained decoding mechanism to enable verbatim copying of evidence spans.

Result: RecaLLM achieves strong performance on RULER and HELMET benchmarks, outperforming baselines and showing consistent gains at context windows of up to 128K tokens.

Conclusion: RecaLLM offers a promising path toward improving long-context performance without expensive long-context training data, highlighting the effectiveness of interleaving reasoning and retrieval.
...
### [175] [BERT-as-a-Judge: A Robust Alternative to Lexical Methods for Efficient Reference-Based LLM Evaluation](https://arxiv.org/abs/2604.09497)
*Hippolyte Gisserot-Boukhlef,Nicolas Boizard,Emmanuel Malherbe,Céline Hudelot,Pierre Colombo*

Main category: cs.CL

TL;DR: The paper proposes BERT-as-a-Judge, an efficient method for evaluating generative outputs of large language models, outperforming lexical methods and matching larger LLM judges.


<details>
  <summary>Details</summary>
Motivation: Current evaluation methods for generative outputs of large language models are limited, relying on rigid lexical methods or computationally expensive LLM-as-a-Judge approaches.

Method: The paper introduces BERT-as-a-Judge, an encoder-driven approach that assesses answer correctness in reference-based generative settings, using lightweight training on synthetically annotated data.

Result: BERT-as-a-Judge consistently outperforms the lexical baseline and matches the performance of much larger LLM judges, providing a reliable and scalable evaluation method.

Conclusion: The proposed BERT-as-a-Judge approach offers a compelling tradeoff between performance and computational overhead, enabling reliable and scalable evaluation of generative outputs.
...
### [176] [You Can't Fight in Here! This is BBS!](https://arxiv.org/abs/2604.09501)
*Richard Futrell,Kyle Mahowald*

Main category: cs.CL

TL;DR: The authors discuss the potential of modern language models to inform language sciences and address common misconceptions about these models.


<details>
  <summary>Details</summary>
Motivation: The authors aim to clarify the role of language model-based work in scientific insights into human language and advocate for a more expansive research program in the AI age.

Method: The authors use a discussion between two characters to highlight common underlying issues with language models, specifically the String Statistics Strawman and the As Good As it Gets Assumption.

Result: The authors advocate for a more robust science of both human language and language models, addressing concerns and producing better insights.

Conclusion: The authors conclude that language models can inform important questions in language sciences and that a more expansive research program is needed to fully leverage their potential.
...
### [177] [Many Ways to Be Fake: Benchmarking Fake News Detection Under Strategy-Driven AI Generation](https://arxiv.org/abs/2604.09514)
*Xinyu Wang,Sai Koneru,Wenbo Zhang,Wenliang Zheng,Saksham Ranjan,Sarah Rajtmajer*

Main category: cs.CL

TL;DR: The paper introduces MANYFAKE, a benchmark for detecting fake news with subtle inaccuracies, and evaluates state-of-the-art detectors, finding they struggle with mixed-truth cases.


<details>
  <summary>Details</summary>
Motivation: Modern fake news often involves human-AI collaboration, embedding strategic inaccuracies within credible narratives, which existing benchmarks and detectors struggle to address.

Method: The authors create MANYFAKE, a synthetic benchmark of 6,798 fake news articles generated through multiple strategy-driven prompting pipelines, and evaluate state-of-the-art fake news detectors on this benchmark.

Result: State-of-the-art fake news detectors approach saturation on fully fabricated stories but remain brittle when faced with subtle, optimized, and interwoven falsehoods.

Conclusion: The paper highlights the need for improved fake news detection methods that can handle mixed-truth cases, which are increasingly common and pose a significant threat.
...
### [178] [Case-Grounded Evidence Verification: A Framework for Constructing Evidence-Sensitive Supervision](https://arxiv.org/abs/2604.09537)
*Soroosh Tayebi Arasteh,Mehdi Joodaki,Mahshad Lotfinia,Sven Nebelung,Daniel Truhn*

Main category: cs.CL

TL;DR: The paper introduces a framework for case-grounded evidence verification, which improves evidence-grounded reasoning by generating explicit support examples and semantically controlled non-support examples without manual annotation.


<details>
  <summary>Details</summary>
Motivation: Current evidence-grounded reasoning models often fail because of weak supervision, loose connection between evidence and claim, and inadequate evaluation of evidence dependence.

Method: The authors propose a supervision construction procedure to generate support and non-support examples, and instantiate the framework in radiology to train a verifier.

Result: The learned verifier outperforms case-only and evidence-only baselines, shows genuine evidence dependence, and transfers across unseen evidence articles and an external case distribution.

Conclusion: The results suggest that a major bottleneck in evidence grounding is not only model capacity, but the lack of supervision that encodes the causal role of evidence.
...
### [179] [Large Language Models Generate Harmful Content Using a Distinct, Unified Mechanism](https://arxiv.org/abs/2604.09544)
*Hadas Orgad,Boyi Wei,Kaden Zheng,Martin Wattenberg,Peter Henderson,Seraphina Goldfarb-Tarrant,Yonatan Belinkov*

Main category: cs.CL

TL;DR: The study examines the internal organization of harmfulness in Large Language Models (LLMs), finding that harmful content generation relies on a compact set of weights that are general across harm types and distinct from benign capabilities.


<details>
  <summary>Details</summary>
Motivation: The brittleness of safety guardrails in LLMs after alignment training, and the occurrence of 'emergent misalignment' when fine-tuning on narrow domains.

Method: Targeted weight pruning as a causal intervention to probe the internal organization of harmfulness in LLMs.

Result: Harmful content generation depends on a compact set of weights that are general across harm types and distinct from benign capabilities. Aligned models exhibit a greater compression of harm generation weights than unaligned counterparts.

Conclusion: The study reveals a coherent internal structure for harmfulness in LLMs, which may serve as a foundation for more principled approaches to safety.
...
TL;DR: OpenKedge is a protocol that safely manages autonomous AI agents by governing state mutations through declarative intent proposals, ensuring preventative safety enforcement and deterministic auditability.


<details>
  <summary>Details</summary>
Motivation: The rise of autonomous AI agents exposes a flaw in API-centric architectures, lacking context, coordination, and safety guarantees.

Method: OpenKedge introduces a governed process for mutation, requiring actors to submit intent proposals evaluated against system state, temporal signals, and policy constraints.

Result: OpenKedge deterministically arbitrates competing intents, prevents unsafe execution, and maintains high throughput in multi-agent conflict scenarios and cloud infrastructure mutations.

Conclusion: OpenKedge establishes a principled foundation for safely operating agentic systems at scale, enabling deterministic auditability and reasoning about system behavior.
...
TL;DR: The paper presents LOM-action, an enterprise AI system that uses event-driven ontology simulation to make decisions, achieving high accuracy and F1 score, and providing a traceable audit log.


<details>
  <summary>Details</summary>
Motivation: Existing LLM-based agent systems produce decisions that are fluent but ungrounded and lack an audit trail. The authors aim to address this limitation.

Method: LOM-action uses event-driven ontology simulation, where business events trigger scenario conditions encoded in the enterprise ontology, driving deterministic graph mutations in an isolated sandbox.

Result: LOM-action achieves 93.82% accuracy and 98.74% tool-chain F1 against frontier baselines, outperforming them by a significant margin.

Conclusion: The authors conclude that ontology-governed, event-driven simulation is the architectural prerequisite for trustworthy enterprise decision intelligence, not model scale.
...
TL;DR: The paper studies the effectiveness of human-in-the-loop oversight in a customer relationship management (CRM) system that uses autonomous agents to personalize marketing messaging. It finds that while human management leads to the highest engagement lift, autonomous agents can sustain a positive lift over time.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to investigate the role of human oversight in autonomous learning systems for scalable personalization in consumer applications.

Method: The paper presents a longitudinal case study analyzing a real-world consumer application over an 11-month period, comparing two distinct periods: an active phase with human curation and a passive phase with autonomous agents.

Result: The results show that human management generates the highest relative lift in engagement metrics, but autonomous agents successfully sustain a positive lift during the passive period.

Conclusion: The findings suggest a symbiotic model where human intervention drives strategic initialization and discovery, while autonomous agents ensure scalable retention and preservation of performance gains.
...
TL;DR: RAMP strategy learns numeric planning action models online via interactions with the environment, integrating Reinforcement Learning and planning to improve performance.


<details>
  <summary>Details</summary>
Motivation: Obtaining action models for automated planning algorithms is hard, and existing algorithms for numeric domains are offline and require expert traces.

Method: RAMP strategy combines Deep Reinforcement Learning, action model learning, and planning in a positive feedback loop, using Numeric PDDLGym for converting numeric planning problems to Gym environments.

Result: RAMP outperforms PPO in terms of solvability and plan quality on standard IPC numeric domains.

Conclusion: The proposed RAMP strategy effectively learns numeric planning action models online and improves performance by integrating Reinforcement Learning and planning.
...
TL;DR: The paper extends Courcelle's theorem by representing models of MSO2 formulas with free variables using decision diagrams, and provides parameterized linear upper bounds on the size of these diagrams.


<details>
  <summary>Details</summary>
Motivation: The importance of Monadic second order logic (MSO2) in parameterized complexity due to Courcelle's theorem, and the need to extend this result to models with free variables.

Method: The paper uses decision diagrams, specifically sentential decision diagrams (SDD) and ordered binary decision diagrams (OBDD), to represent models of MSO2 formulas with free variables.

Result: The paper provides a parameterized linear upper bound on the size of SDD when treewidth is considered, and a parameterized linear upper bound on the size of OBDD when pathwidth is considered. It also shows that there is an MSO2 formula and a class of graphs with bounded treewidth that do not admit an OBDD with size parameterized by the treewidth.

Conclusion: The paper offers a new perspective on Courcelle's theorem and connects it to the area of knowledge representation, providing new insights into the representation of models of MSO2 formulas with free variables.
...
TL;DR: This paper explores using an agentic language model feedback framework to generate planning domains from natural language descriptions with minimal symbolic information.


<details>
  <summary>Details</summary>
Motivation: The generation of planning domains from natural language descriptions is an open problem, and large language models (LLMs) are not yet capable of producing high-quality domains.

Method: The authors propose using an agentic language model feedback framework with heuristic search over model space, augmented with minimal symbolic information and various forms of feedback (landmarks, VAL plan validator output).

Result: The experiment aims to optimize domain quality using the proposed framework and feedback mechanisms.

Conclusion: The paper aims to improve the generation of planning domains from natural language descriptions by leveraging an agentic language model feedback framework.
...
TL;DR: This paper explores how the environment can serve as a form of memory for agents in Reinforcement Learning, reducing the need for internal memory.


<details>
  <summary>Details</summary>
Motivation: The situated view of cognition suggests that intelligent behavior relies not just on internal memory but also on the active use of environmental resources.

Method: The authors introduce a mathematical framework for how the environment can functionally serve as an agent's memory and prove that certain observations (artifacts) can reduce the information needed to represent history.

Result: Experiments show that observing spatial paths reduces the amount of memory required to learn a performant policy, and this effect arises unintentionally through the agent's sensory stream.

Conclusion: The findings suggest that the environment can be exploited as a substitute for explicit internal memory, with implications for the development of more efficient learning agents.
...
TL;DR: The paper introduces ViSA-R2, a model that infers analytical solutions for 2D linear steady-state fields from visual observations and metadata.


<details>
  <summary>Details</summary>
Motivation: The capability to recover analytical solutions of physical fields from visual observations is under explored in AI-assisted scientific reasoning.

Method: ViSA-R2 follows a physicist-like pathway: structural pattern recognition, solution-family hypothesis, parameter derivation, and consistency verification.

Result: ViSA-R2 outperforms strong open-source baselines and closed-source VLMs on ViSA-Bench, a synthetic benchmark for 2D linear steady-state scenarios.

Conclusion: The proposed ViSA-R2 model demonstrates effectiveness in inferring analytical solutions for 2D linear steady-state fields from visual observations.
...
TL;DR: The paper introduces Sequence-Level PPO (SPPO), a scalable algorithm for aligning Large Language Models (LLMs) in reasoning tasks with verifiable rewards, achieving a balance between sample efficiency and stability.


<details>
  <summary>Details</summary>
Motivation: Standard token-level Proximal Policy Optimization (PPO) struggles with instability and high memory costs in long Chain-of-Thought (CoT) horizons, while critic-free alternatives like GRPO incur significant computational overhead.

Method: SPPO reformulates the reasoning process as a Sequence-Level Contextual Bandit problem, using a decoupled scalar value function to derive low-variance advantage signals without multi-sampling.

Result: SPPO significantly surpasses standard PPO and matches the performance of computation-heavy group-based methods on mathematical benchmarks, offering a resource-efficient framework for aligning reasoning LLMs.

Conclusion: SPPO provides a scalable and efficient solution for aligning LLMs in reasoning tasks, balancing sample efficiency and stability, and outperforming standard PPO and critic-free alternatives.
...
TL;DR: The paper proposes StaRPO, a reinforcement learning framework that incorporates reasoning stability into the optimization objective, improving the accuracy and logical stability of large language models in complex reasoning tasks.


<details>
  <summary>Details</summary>
Motivation: Existing RL frameworks rely on final-answer correctness and neglect the internal logical structure of the reasoning process, leading to logically inconsistent or redundant responses.

Method: StaRPO uses two computable metrics, Autocorrelation Function (ACF) and Path Efficiency (PE), to evaluate local coherence and global goal-directedness of the reasoning trajectory, and combines them with task rewards to provide process-aware feedback.

Result: Experiments on four reasoning benchmarks show that StaRPO consistently outperforms baselines, enhancing both final-answer accuracy and logical stability.

Conclusion: The proposed StaRPO framework effectively improves the accuracy and logical stability of large language models in complex reasoning tasks by incorporating reasoning stability into the optimization objective.
...
TL;DR: The paper proposes a tutor-student multi-agent system using Large Language Models (LLMs) to enhance problem-solving abilities through structured interactions, achieving similar or higher accuracy with fewer tokens.


<details>
  <summary>Details</summary>
Motivation: The authors aim to leverage social interaction principles, specifically role-based exchanges, to push LLMs beyond their current capabilities, inspired by human cognitive development.

Method: The authors propose the PETITE framework, where two agents instantiated from the same LLM are assigned asymmetric roles: a student agent generates and refines solutions, while a tutor agent provides evaluative feedback.

Result: The PETITE framework achieves similar or higher accuracy compared to state-of-the-art approaches while consuming significantly fewer tokens on the APPS coding benchmark.

Conclusion: The study suggests that developmentally grounded role-differentiated interaction structures provide a principled and resource-efficient paradigm for enhancing LLM problem-solving through structured peer-like interactions.
...
TL;DR: The paper introduces PilotBench, a benchmark for evaluating Large Language Models (LLMs) on safety-critical flight trajectory and attitude prediction, and reveals a trade-off between precision and controllability in LLMs.


<details>
  <summary>Details</summary>
Motivation: The motivation is to investigate whether LLMs trained on text corpora can reliably reason about complex physics while adhering to safety constraints, which is crucial for embodied AI agents operating in physical environments.

Method: The method involves using PilotBench, a benchmark built from 708 real-world general aviation trajectories, to evaluate LLMs and traditional forecasters on safety-critical flight trajectory and attitude prediction. A composite metric, Pilot-Score, is introduced to balance regression accuracy and instruction adherence.

Result: The results show that traditional forecasters achieve superior precision (MAE of 7.01) but lack semantic reasoning capabilities, while LLMs gain controllability (86-89% instruction-following) at the cost of precision (11-14 MAE). LLMs perform poorly in high-workload phases such as Climb and Approach.

Conclusion: The paper concludes that there is a need for hybrid architectures combining LLMs' symbolic reasoning with specialized forecasters' numerical precision, and that PilotBench provides a rigorous foundation for advancing embodied AI in safety-constrained domains.
...
TL;DR: This paper introduces a new formal definition of Self-Evolving Agents (SEA) and a benchmark, SEA-Eval, to evaluate their performance in continuous task evolution and accumulation of experience.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based agents are limited by static toolsets and episodic amnesia, failing to accumulate experience or optimize strategies across tasks.

Method: The paper proposes a new formal definition of SEA and introduces SEA-Eval, a benchmark that evaluates SEA characteristics across intra-task execution reliability and long-term evolutionary performance.

Result: Empirical evaluations reveal a significant evolutionary bottleneck in current state-of-the-art frameworks, with identical success rates masking large differences in token consumption and divergent evolutionary trajectories.

Conclusion: SEA-Eval provides a scientific foundation for advancing agents from task executors to genuinely self-evolving digital entities.
...
TL;DR: The paper proposes a domain-agnostic method using Hypergraph Neural Networks (HGNNs) to accelerate the enumeration of Minimal Unsatisfiable Subsets (MUSes) in constraint satisfaction problems.


<details>
  <summary>Details</summary>
Motivation: The major challenge in enumerating MUSes is the exponential growth of the search space, which becomes severe when satisfiability checks are expensive. Existing machine learning approaches are limited to Boolean satisfiability problems and rely on explicit variable-constraint relationships.

Method: The proposed method uses HGNNs to incrementally build a hypergraph with constraints as vertices and MUSes enumerated until the current step as hyperedges. An HGNN-based agent is trained via reinforcement learning to minimize the number of satisfiability checks required to obtain an MUS.

Result: The experimental results demonstrate the effectiveness of the approach in accelerating MUS enumeration, showing that the method can enumerate more MUSes within the same satisfiability check budget compared to conventional methods.

Conclusion: The paper presents a domain-agnostic method for accelerating MUS enumeration using HGNNs, which shows promising results in reducing the number of satisfiability checks required.
...
TL;DR: The paper introduces Advantage-Guided Diffusion for Model-Based Reinforcement Learning (AGD-MBRL), which improves diffusion world models by guiding the reverse diffusion process with advantage estimates to focus on high-return trajectories.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion guides for MBRL are either policy-only, discarding value information, or reward-based, which becomes myopic with short diffusion horizons.

Method: The authors propose two guides: Sigmoid Advantage Guidance (SAG) and Exponential Advantage Guidance (EAG), which steer the diffusion process using advantage estimates.

Result: AGD-MBRL improves sample efficiency and final return over baselines (PolyGRAD, Diffuser-style reward guide, PPO/TRPO) on MuJoCo control tasks.

Conclusion: Advantage-aware guidance is a simple and effective remedy for short-horizon myopia in diffusion-model MBRL, leading to improved performance.
...
TL;DR: The study investigates how humans plan and predict physical actions under limited cognitive resources, finding a dual transition in both prediction mechanisms and planning strategies as resource pressure increases.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this research is to understand how humans make sequential physical plans under resource constraints, and to resolve debates on intuitive physics and decision-making strategies.

Method: The study used a construction task called Overhang Tower, where participants had to maximize horizontal overhang while maintaining stability, under varying levels of time pressure and complexity.

Result: The results show that as cognitive resources decrease, humans shift from using Intuitive Physics Engine (IPE)-based simulation to CNN-based visual heuristics for prediction, and from deliberative lookahead to shallower planning horizons.

Conclusion: The study concludes that humans have a hierarchical, resource-rational architecture that flexibly trades computational cost against predictive fidelity, unifying debates on simulation vs. heuristics and myopic vs. deliberative planning.
...
TL;DR: Camera Artist is a multi-agent framework for generating narrative videos with explicit cinematic language, improving upon existing methods by introducing a Cinematography Shot Agent for better narrative continuity and filmic quality.


<details>
  <summary>Details</summary>
Motivation: Recent multi-agent systems for automating filmmaking workflows lack mechanisms for structuring narrative progression and deliberate use of cinematic language, leading to fragmented storytelling and limited filmic quality.

Method: Camera Artist introduces a Cinematography Shot Agent that integrates recursive storyboard generation and cinematic language injection to improve narrative continuity and filmic quality.

Result: The approach consistently outperforms existing baselines in narrative consistency, dynamic expressiveness, and perceived film quality.

Conclusion: Camera Artist effectively generates narrative videos with improved cinematic language and narrative continuity, demonstrating superior performance over existing methods.
...
TL;DR: Introduced DRBENCHER, a benchmark generator for evaluating AI agents that combine web browsing and computation, highlighting limitations in current models.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks for AI agents do not accurately reflect real-world performance, as they evaluate web browsing and computation separately.

Method: DRBENCHER uses a unified pipeline to generate questions across five domains, ensuring verifiability, complexity, difficulty, and diversity.

Result: The strongest frontier model achieved only 20% answer accuracy, and human evaluation showed 76% validity, with errors often due to outdated knowledge.

Conclusion: DRBENCHER provides a more comprehensive evaluation of AI agents, revealing significant performance gaps and limitations in current models.
...
TL;DR: The authors propose SAGE, a universal multi-agent benchmark for evaluating Large Language Models (LLMs) in customer service, highlighting the limitations of existing benchmarks and revealing a significant 'Execution Gap' in LLMs' ability to derive correct actions.


<details>
  <summary>Details</summary>
Motivation: The development of LLMs has automated customer service, but benchmarking their performance remains challenging due to existing benchmarks' inability to account for diverse user behaviors and strict adherence to Standard Operating Procedures (SOPs).

Method: The authors propose SAGE, which formalizes unstructured SOPs into Dynamic Dialogue Graphs, introduces an Adversarial Intent Taxonomy, and uses Judge Agents and a Rule Engine to analyze interactions between User and Service Agents.

Result: Extensive experiments on 27 LLMs across 6 industrial scenarios reveal a significant 'Execution Gap' where models accurately classify intents but fail to derive correct subsequent actions, and 'Empathy Resilience', a phenomenon where models maintain polite conversational facades despite underlying logical failures.

Conclusion: The authors conclude that SAGE provides a universal multi-agent benchmark for automated, dual-axis assessment of LLMs in customer service, highlighting the need for more comprehensive evaluation methods.
...
TL;DR: The paper presents CACM, a language-based drug discovery framework that improves the success rate of autonomous drug discovery agents by introducing precise set-level diagnosis and a concise memory write-back mechanism.


<details>
  <summary>Details</summary>
Motivation: The authors are motivated by the challenge of reliable success in autonomous drug discovery, which is determined by whether the final returned set jointly satisfies protocol-level requirements. Existing language-based systems rely on long raw history and under-specified self-reflection, leading to imprecise failure localization and noisy agent states.

Method: The authors propose CACM, which introduces protocol auditing and a grounded diagnostician to analyze multimodal evidence and generate actionable remediation hints. CACM organizes memory into static, dynamic, and corrective channels and compresses them before write-back to preserve task information while exposing only decision-relevant failures.

Result: The experimental results show that CACM improves the target-level success rate by 36.4% over the state-of-the-art baseline.

Conclusion: The paper concludes that reliable language-based drug discovery benefits not only from more powerful molecular tools but also from more precise diagnosis and more economical agent states.
...
TL;DR: The paper introduces Spatial-Gym, a new environment for testing spatial reasoning in models, and evaluates several models on pathfinding tasks in 2D-grid puzzles.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for spatial reasoning are limited as they require full solution generation in a single response, unlike human problem-solving which is often interactive and step-by-step.

Method: The authors introduce Spatial-Gym, a Gymnasium environment that tests pathfinding in 2D-grid puzzles as a sequential decision task with optional backtracking, and evaluate eight models in three settings.

Result: The best model achieves a solve rate of 16.0%, significantly lower than the human baseline of 98.0%. Step-by-step format and backtracking have varying effects on model performance.

Conclusion: The paper highlights the limitations of current models in spatial reasoning, particularly in scaling reasoning effort with difficulty, and proposes Spatial-Gym as a framework for improving spatial reasoning through reinforcement learning.
...
TL;DR: The paper introduces HiL-Bench, a benchmark to evaluate the ability of coding agents to decide when to ask for help in ambiguous situations, and finds that current frontier models have a significant judgment gap in this area.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks for coding agents focus on execution correctness with unambiguous instructions, but real-world tasks often involve incomplete or ambiguous specifications, requiring agents to decide when to act autonomously and when to ask for help.

Method: The authors propose HiL-Bench, a human-in-the-loop benchmark that includes human-validated blockers to test selective escalation skill, and introduce the Ask-F1 metric to evaluate the balance between over-asking and silent guessing.

Result: The evaluation of frontier models across SWE and text-to-SQL domains reveals a large universal judgment gap, with no model recovering more than a fraction of its full-information performance when deciding whether to ask for help.

Conclusion: The paper concludes that poor help-seeking is a model-level flaw, not task-specific, and that judgment is trainable through RL training on shaped Ask-F1 reward, with a 32B model showing improvements in help-seeking quality and task pass rate that transfer across domains.
...
TL;DR: The paper proposes a single-point-based multi-objective search framework (SPMO) that focuses on finding a high-quality solution for decision-makers under a limited evaluation budget, rather than approximating the entire Pareto front.


<details>
  <summary>Details</summary>
Motivation: The difficulty in many-objective optimisation problems where the number of objectives increases, making it challenging to design a search algorithm that can effectively explore the entire Pareto front, especially under a limited evaluation budget.

Method: The paper proposes a single-point-based multi-objective search framework (SPMO) with a simple acquisition function called expected single-point improvement (ESPI), which can be optimised effectively with gradient-based methods.

Result: The proposed SPMO is computationally tractable and outperforms state-of-the-arts on a wide range of benchmark and real-world problems.

Conclusion: The paper concludes that focusing on finding a single high-quality solution for decision-makers under a limited evaluation budget may be more useful than aiming to approximate the entire Pareto front, and that the proposed SPMO framework can effectively achieve this goal.
...
TL;DR: The authors propose E3-TIR, a new training paradigm for Large Language Models (LLMs) in Tool-Integrated Reasoning (TIR) that improves exploration efficiency and performance while reducing data costs.


<details>
  <summary>Details</summary>
Motivation: Existing training paradigms for LLMs in TIR, such as Zero-RL and SFT-then-RL, have limitations including inefficient exploration, mode degradation, high data costs, and capability plateaus.

Method: The authors propose E3-TIR, which integrates three experience types: Expert Prefixes, Expert Guided, and Self-Exploration. It uses diverse branching exploration and a mix policy optimization mechanism to mitigate distribution shifts and optimization conflicts.

Result: E3-TIR achieves a 6 performance improvement over traditional paradigms on tool-use tasks, while requiring less than 10 of the synthetic data. It also achieves a 1.46x gain in ROI compared to baselines.

Conclusion: The proposed E3-TIR paradigm effectively balances exploration diversity with training efficiency, demonstrating improved performance, efficiency, and cost-effectiveness in TIR tasks.
...
TL;DR: The authors introduce Process Reward Agents (PRA), a method for providing online, step-wise rewards to a frozen policy in knowledge-intensive domains, achieving state-of-the-art results in medical reasoning benchmarks.


<details>
  <summary>Details</summary>
Motivation: Reasoning in knowledge-intensive domains is challenging due to the need to synthesize clues across large external knowledge sources, making it difficult to evaluate step correctness locally.

Method: The authors propose Process Reward Agents (PRA), a test-time method that provides domain-grounded, online, step-wise rewards to a frozen policy, enabling search-based decoding to rank and prune candidate trajectories.

Result: PRA consistently outperforms strong baselines, achieving 80.8% accuracy on MedQA with Qwen3-4B, a new state of the art at the 4B scale, and improving accuracy by up to 25.7% for unseen frozen policy models.

Conclusion: PRA suggests a paradigm in which frozen reasoners are decoupled from domain-specific reward modules, allowing the deployment of new backbones in complex domains without retraining.
...
TL;DR: The study examines algorithmic monoculture in AI agents, distinguishing between primary and strategic monoculture, and compares the behavior of humans and large language models (LLMs) in coordination tasks.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to understand how AI agents, specifically large language models (LLMs), behave in multi-agent environments where coordination is crucial, and to compare their behavior with that of humans.

Method: The authors implement an experimental design to study primary and strategic algorithmic monoculture, deploying it on both human and LLM subjects to analyze their behavior in response to coordination incentives.

Result: The results show that LLMs exhibit high levels of primary algorithmic monoculture (baseline action similarity) and, similar to humans, adjust their behavior in response to strategic incentives. However, LLMs struggle to sustain heterogeneity when divergence is rewarded, lagging behind humans.

Conclusion: The study concludes that while LLMs are capable of coordinating well on similar actions, they have limitations in maintaining diverse behaviors when it is beneficial to do so, highlighting a key difference between LLM and human behavior in strategic environments.
...
TL;DR: The authors propose SUPERNOVA, a data curation framework for Reinforcement Learning with Verifiable Rewards (RLVR) to enhance general reasoning in large language models. They conduct experiments to analyze the impact of data design choices on downstream reasoning performance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of extending RLVR to general reasoning tasks, which is constrained by the lack of high-quality, verifiable training data that spans diverse reasoning skills.

Method: The authors propose a data curation framework called SUPERNOVA, which leverages instruction-tuning datasets with expert-annotated ground-truth to adapt RLVR for general reasoning. They conduct 100+ controlled RL experiments to analyze the impact of data design choices on downstream reasoning performance.

Result: The results show that SUPERNOVA outperforms strong baselines on challenging reasoning benchmarks, with relative improvements of up to 52.8% on BBEH across model sizes. The authors also find that source task selection has a significant impact on downstream reasoning performance.

Conclusion: The authors conclude that principled data curation for RLVR can be effective in enhancing general reasoning in large language models, and provide practical insights for curating human-annotated resources to extend RLVR to general reasoning.
