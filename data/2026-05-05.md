...
TL;DR: Proposed a dual-path framework for anticipating traffic accidents in autonomous driving by generating synthetic data and using a graph neural network.


<details>
  <summary>Details</summary>
Motivation: Addressing the complexity of modeling interactions between road users and the limited availability of diverse, large-scale datasets for autonomous driving.

Method: A dual-path framework consisting of a video synthesis pipeline and a graph neural network enriched with semantic cues.

Result: Notable gains in accuracy and anticipation lead time across existing datasets and a new benchmark dataset.

Conclusion: The proposed framework can mitigate current data bottlenecks and enhance the reliability of autonomous driving systems.
...
TL;DR: The paper analyzes the performance of 3D Gaussian Splatting (3DGS) in hybrid-capture novel view synthesis and finds that rendering two views per optimizer step significantly improves performance, while other methods such as gradient surgery and projective preconditioning have minimal impact.


<details>
  <summary>Details</summary>
Motivation: The motivation of this paper is to investigate the performance of 3DGS in hybrid-capture novel view synthesis, where images are taken at substantially different camera distances, and to identify the key factors that affect its performance.

Method: The method used in this paper involves analyzing the performance of 3DGS under various conditions, including different rendering strategies, and evaluating the impact of different techniques such as gradient surgery and projective preconditioning. The authors also propose a variance-decomposition framework to explain their findings.

Result: The results show that rendering two views per optimizer step significantly improves the performance of 3DGS, reducing the gap with the best results by 1-3 dB on five hybrid-capture benchmarks. Other methods, such as gradient surgery and projective preconditioning, have minimal impact on performance.

Conclusion: The conclusion of this paper is that the key to improving the performance of 3DGS in hybrid-capture novel view synthesis is to render two views per optimizer step, and that other techniques have minimal impact. The authors also provide a framework to explain why this is the case.
...
TL;DR: The paper proposes Adaptive Intermediate Domain Adaptation (AIDA) for person re-identification, which adapts to domain shifts without requiring joint source-target access.


<details>
  <summary>Details</summary>
Motivation: Existing domain adaptation methods for person re-identification degrade when deployed in unseen environments, and current intermediate domain approaches have limitations in multi-source and source-free settings.

Method: The proposed AIDA framework uses adaptive feature mixing and regularization strength based on model uncertainty and training stability, along with a multi-source intermediate domain generator and pseudo-mirror regularization.

Result: Extensive experiments show the effectiveness of AIDA in domain generalization and source-free settings for person re-identification.

Conclusion: AIDA addresses the limitations of existing intermediate domain approaches and improves person re-identification across non-overlapping camera views in unseen environments.
...
TL;DR: The paper proposes GAFSV-Net, a novel approach for online signature verification that converts temporal sequences into 2D images, enabling the use of pre-trained 2D vision backbones.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning methods for online signature verification are limited by their use of 1D architectures and raw temporal sequences, which restricts their ability to distinguish skilled forgeries from genuine samples.

Method: The proposed GAFSV-Net represents each signature as a six-channel asymmetric Gramian Angular Field image, which is processed by a dual-branch ConvNeXt-Tiny encoder with bidirectional cross-attention.

Result: The proposed approach outperforms all sequence-based baselines on DeepSignDB and BiosecurID datasets, demonstrating the effectiveness of 2D temporal encoding for online signature verification.

Conclusion: The paper concludes that the proposed GAFSV-Net approach can effectively distinguish skilled forgeries from genuine samples, and that the representational gain of 2D temporal encoding is consistent and independent of training procedure.
...
TL;DR: This paper presents a methodology for designing spiking neural networks (SNNs) for real-time object detection on neuromorphic hardware, specifically Intel Loihi 2, and compares their performance with conventional artificial neural networks (ANNs) on various platforms.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enable energy-efficient real-time object detection on energy-constrained platforms, such as UAV-based inspection, autonomous navigation, and mobile robotics.

Method: The authors propose a comprehensive methodology for designing SNN detection architectures and engineering adaptations for deployment on Intel Loihi 2. They also use ANN-to-SNN distillation-aware training to improve SNN accuracy.

Result: The results show that SNNs on Loihi 2 achieve real-time detection with the lowest per-inference dynamic energy among all platforms. Loihi 2 outperforms other platforms in power consumption, and SNNs recover 87-100% of the detection accuracy of their ANN counterparts with distillation-aware training.

Conclusion: The paper concludes that neuromorphic systems, specifically SNNs on Intel Loihi 2, have the potential for energy-efficient, real-time object detection at the edge, with comparable performance to conventional ANNs.
...
TL;DR: A pipeline for neural implicit surface reconstruction of non-cooperative space objects from monocular inspection imagery is presented.


<details>
  <summary>Details</summary>
Motivation: The characterization of non-cooperative resident space objects is crucial for active debris removal and on-orbit servicing mission planning.

Method: A pipeline for neural implicit surface reconstruction from monocular inspection imagery, incorporating segmentation-based background removal and photometric correction.

Result: The method successfully reconstructs surfaces from publicly released ISS inspection footage and on-orbit inspection footage of an H-IIA rocket upper stage.

Conclusion: The proposed pipeline enables neural implicit surface reconstruction of non-cooperative space objects from real on-orbit footage.
...
TL;DR: VkSplat is a high-performance, cross-vendor 3D Gaussian Splatting training pipeline implemented in Vulkan compute, achieving state-of-the-art performance and compatibility across GPU vendors.


<details>
  <summary>Details</summary>
Motivation: Existing 3DGS training pipelines have performance and compatibility limitations, which VkSplat aims to address.

Method: VkSplat is implemented fully in Vulkan compute and includes various optimizations to improve performance and reduce VRAM usage.

Result: VkSplat achieves 3.3x speed and 33% VRAM reduction over the CUDA+PyTorch baseline while maintaining quality and demonstrating compatibility across GPU vendors.

Conclusion: VkSplat is the first fully-Vulkan-based 3DGS training pipeline to achieve state-of-the-art performance, providing a high-performance and compatible solution for 3D Gaussian Splatting training.
...
TL;DR: The paper discusses the challenge of estimating egocentric pose in Augmented Reality (AR) and assistive devices with guaranteed uncertainty regions. It proposes an adaptive conformal prediction method to improve coverage of difficult frames.


<details>
  <summary>Details</summary>
Motivation: The need for guaranteed uncertainty regions in egocentric pose estimation for AR and assistive devices, particularly for the hardest frames which have high physical difficulty.

Method: The paper uses conformal prediction (CP) with a nonconformity score based on geodesic SE(3) and proposes a two-stage difficulty estimator, DINOv2-Bridge adaptive CP, to improve conditional coverage.

Result: The proposed method improves coverage of the hardest 25% of frames (Q4) from ~0.75 to ~0.93 while maintaining overall coverage at 90%.

Conclusion: The paper concludes that the proposed adaptive CP method effectively closes the conditional coverage gap in egocentric pose estimation, providing more reliable uncertainty regions for difficult frames.
...
TL;DR: The paper presents MAEPose, a method for human pose estimation using mmWave radar that operates directly on spectrogram videos, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Existing methods for human pose estimation using mmWave radar rely on pre-extracted intermediate representations, discarding rich spatiotemporal information and adding system complexity. They also typically require labelled data and do not leverage unlabelled raw video streams.

Method: MAEPose, a masked autoencoding-based approach that learns spatiotemporal motion-aware generalized representations from unlabelled radar video and uses a heatmap decoder for multi-frame pose estimation predictions.

Result: MAEPose outperforms state-of-the-art baselines by up to 22.1% in MPJPE and maintains robust accuracy under zero-shot bystander interference with only a 6.5% error increase.

Conclusion: The proposed MAEPose approach demonstrates the effectiveness of learning from unlabelled radar video and the importance of leveraging spatiotemporal information for human pose estimation.
...
TL;DR: The paper proposes Remote SAMsing, a pipeline that improves SAM2 for zero-shot segmentation in large remote sensing scenes, addressing issues of mask quality and object fragmentation across tile boundaries.


<details>
  <summary>Details</summary>
Motivation: Applying SAM2 to large remote sensing scenes reveals two problems: (1) a quality-coverage trade-off in mask generation and (2) object fragmentation across tile boundaries.

Method: Remote SAMsing uses a multi-pass algorithm with dynamic thresholding for better coverage and a parameter-free merge strategy for spatial consistency across tiles.

Result: The pipeline increases coverage from 30-68% (single-pass SAM2) to 91-98% on seven test scenes, with high detection quality (e.g., buildings 95%, cars 82-93% Det@0.5) and precise segment boundaries.

Conclusion: Remote SAMsing effectively solves the challenges of applying SAM2 to large remote sensing scenes without requiring modifications or training data, demonstrating high scalability and generalizability.
...
TL;DR: REALM is a cross-modal framework that enables learning-based approaches for event processing by mapping event representations into the latent space of RGB foundation models.


<details>
  <summary>Details</summary>
Motivation: Existing learning-based approaches for event processing are limited to narrow, task-specific applications and lack generalization across modalities.

Method: REALM uses low-rank adaptation (LoRA) to bridge the modality gap and projects event representations into the pretrained latent space of RGB foundation models.

Result: REALM achieves state-of-the-art performance in wide-baseline feature matching and enables zero-shot application of complex, frozen image-trained decoders to raw event data.

Conclusion: REALM provides a generalizable framework for event processing, allowing for effective mapping of events into RGB foundation models and enabling downstream tasks without task-specific training.
...
TL;DR: The paper investigates the limitations of text-to-image diffusion models in multi-object generation, finding that scene complexity and data scarcity are key factors, and proposes a controlled framework for dataset generation.


<details>
  <summary>Details</summary>
Motivation: The unreliability of text-to-image diffusion models in multi-object generation despite their impressive visual fidelity, and the need to understand the underlying causes of these failures.

Method: The authors introduce a controlled framework called mosaic for dataset generation, and train diffusion models on it to study concept generalization and compositional generalization across different dataset sizes.

Result: The study finds that scene complexity plays a dominant role in multi-object generation, and that counting is uniquely difficult to learn in low-data regimes, with compositional generalization collapsing as more concept combinations are held out during training.

Conclusion: The findings highlight fundamental limitations of diffusion models and motivate stronger inductive biases and data design for robust multi-object compositional generation.
...
TL;DR: A novel multi-scale attention-based model is proposed for explainable AI in autonomous vehicles, providing case-specific explanations for driving decisions.


<details>
  <summary>Details</summary>
Motivation: The increasing use of deep learning models in autonomous vehicles necessitates explainability to ensure efficiency, predict system failures, and enable real-world implementation.

Method: A multi-scale attention-based model with a reasoning component is proposed to provide explanations for driving decisions, utilizing the F1-score and a new Joint F1 score metric for evaluation.

Result: The proposed model outperforms classic and state-of-the-art models, demonstrating accurate and reliable performance in terms of Explainable Artificial Intelligence (XAI).

Conclusion: The proposed model provides a comprehensive understanding of complex models in autonomous vehicles, enabling the development of truly reliable systems.
...
TL;DR: The paper presents a study on optimizing Vision Transformers (ViTs) for efficient spatio-temporal vegetation pixel classification, achieving improved computational efficiency while maintaining competitive classification performance.


<details>
  <summary>Details</summary>
Motivation: The study aims to address the computational challenges in identifying plant species across time using high-resolution monitoring from Unmanned Aerial Vehicles (UAVs) and near-surface cameras, which is essential for understanding ecosystem dynamics and responses to climate change.

Method: The authors conducted a comprehensive ablation study analyzing seven key design dimensions of ViTs, including data normalization, spectral arrangement, boundary handling, spatial context window shape and size, tokenization strategies, positional encoding, and feature aggregation strategies.

Result: The experimental results demonstrate that the ViT approach offers a substantial improvement in computational efficiency while maintaining competitive classification performance, reducing Floating Point Operations (FLOPs) by an order of magnitude and maintaining constant parameter complexity regardless of the time series length.

Conclusion: The findings confirm that ViTs are a robust, scalable solution for resource-constrained phenological monitoring systems, offering improved efficiency and performance in plant species classification across time.
...
TL;DR: The authors introduce GeoSR-Bench, a benchmark dataset for evaluating super-resolution (SR) models in Earth observation tasks beyond traditional fidelity metrics, and show that improvements in SR metrics do not necessarily translate to gains in downstream task performance.


<details>
  <summary>Details</summary>
Motivation: Existing SR studies and benchmarks focus on fidelity metrics such as PSNR or SSIM, but the true utility of super-resolved images lies in supporting downstream tasks like land cover classification, biomass estimation, and change detection.

Method: The authors create GeoSR-Bench, a dataset comprising spatially co-located, temporally aligned, and quality-controlled image pairs from 36,000 locations across diverse land covers, and benchmark various SR models on perceptual quality and downstream task performance.

Result: The results show that improvements in traditional SR metrics often do not correlate with gains in task performance, and can even be negative, indicating that these metrics provide limited guidance for selecting superior models for downstream tasks.

Conclusion: The authors conclude that there is a need to integrate downstream tasks into SR model development and evaluation, and that GeoSR-Bench provides a valuable resource for this purpose.
...
TL;DR: The paper proposes OSCAR, a framework for online self-calibration of Large Vision-Language Models (LVLMs) to reduce hallucinations by leveraging the model's discriminative capabilities.


<details>
  <summary>Details</summary>
Motivation: LVLMs suffer from hallucinations due to a Supervision-Perception Mismatch in offline preference alignment methods, which forces the model to learn beyond its perceptual capacity.

Method: The authors propose OSCAR, which integrates Monte Carlo Tree Search with a Dual-Granularity Reward Mechanism to construct preference data and iteratively refines the model via Direct Preference Optimization.
...
Conclusion: The proposed OSCAR framework effectively addresses the issue of hallucinations in LVLMs by enabling online self-calibration, leading to improved performance and capabilities.
...
TL;DR: A novel diffusion framework called Pose-Aware Diffusion (PAD) is introduced for generating pose-aligned 3D objects directly in the observation space, resolving pose ambiguity and producing high-fidelity assets.


<details>
  <summary>Details</summary>
Motivation: The challenge of generating pose-aligned 3D objects due to spatial mismatches and transformation ambiguities in traditional decoupled canonical-then-rotate paradigms.

Method: PAD uses a diffusion framework that synthesizes 3D geometry directly within the observation space, leveraging unprojected monocular depth as a 3D geometric anchor to enforce spatial supervision.

Result: PAD achieves superior geometric alignment and image-to-3D correspondence compared to state-of-the-art methods, and can naturally extend to compositional 3D scene reconstruction.

Conclusion: The proposed PAD framework offers a robust solution for generating high-fidelity, pose-aligned 3D objects and scenes, addressing the limitations of traditional methods.
...
TL;DR: The paper introduces CURE-OOD, a benchmark for evaluating out-of-distribution (OOD) detection in cancer survival prediction, and finds that covariate shifts significantly impact survival prediction performance.


<details>
  <summary>Details</summary>
Motivation: Accurate survival prediction is crucial for cancer patients, but variations in imaging acquisition can undermine model reliability. The authors aim to address the gap in systematically studying OOD detection in cancer survival prediction.

Method: The authors propose CURE-OOD, a benchmark that defines scanner-parameter-based training, in-distribution (ID), and OOD test splits across four survival prediction tasks. They evaluate mainstream classification-oriented OOD detectors and propose HazardDev as a simple survival-aware reference baseline.

Result: The experiments show that covariate shifts notably reduce survival prediction performance and that mainstream classification-oriented OOD detectors can fail in survival prediction.

Conclusion: The paper concludes that CURE-OOD enables systematic analysis of how distribution shifts affect both downstream survival performance and OOD detectability, highlighting the need for survival-aware OOD detection methods.
...
TL;DR: The paper introduces a novel, efficient framework called Temporal Convolutional Motion Predictor (TCMP) for multi-object tracking (MOT) that outperforms current state-of-the-art methods in accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: Current MOT methods struggle with predicting object motion in complex, non-linear scenarios, and recent approaches have become overly complex and computationally expensive. The authors argue that such complexity is unnecessary and can be outperformed by a more efficient approach.

Method: The authors propose TCMP, a modified Temporal Convolutional Network (TCN) featuring dilated convolutions and a regression head, which allows for effective motion prediction across arbitrary temporal context lengths.

Result: TCMP achieves state-of-the-art performance, improving key metrics: HOTA from 62.3% to 63.4%, IDF1 from 63.0% to 65.0%, and AssA from 47.2% to 49.1%. It does so with significantly reduced parameters (0.014 times) and computational cost (0.05 times) compared to the current SOTA method.

Conclusion: The paper concludes that TCMP advances MOT systems by ensuring adaptability, accuracy, and efficiency in complex tracking environments, highlighting the robustness of the proposed method.
...
TL;DR: A flow matching model is proposed for 4x super-resolution of satellite imagery, outperforming diffusion and GAN-based methods in balancing spectral fidelity and perceptual quality.


<details>
  <summary>Details</summary>
Motivation: The need to navigate trade-offs between spectral fidelity and perceptual quality in super-resolution of satellite imagery, particularly for applications demanding fine spatial detail.

Method: A flow matching model for 4x super-resolution of 10-m Sentinel-2 imagery using a dataset of 120,851 paired images, evaluated against diffusion and Real-ESRGAN models.

Result: The flow matching model outperformed other models in pixel-wise accuracy and generated perceptually realistic super-resolved imagery in a few sampling steps, leading to a 2.5-m 4-band CONUS imagery product and a yearly 2.5-m land cover product with 89.11% accuracy.

Conclusion: Flow matching is an effective generative modeling approach for super-resolution of Sentinel-2 imagery, offering a promising solution for expanding access to high-resolution imagery for geospatial applications.
...
TL;DR: Proposed RTPrune, a two-stage token pruning method for DeepSeek-OCR, which improves efficiency and accuracy by prioritizing high-norm tokens and merging remaining tokens.


<details>
  <summary>Details</summary>
Motivation: Current token pruning methods for VLMs fail to preserve textual fidelity, and visual tokens in DeepSeek-OCR contain redundant information.

Method: RTPrune, a two-stage token pruning method, prioritizing high-norm tokens and merging remaining tokens based on optimal transport theory.

Result: State-of-the-art performance on OmniDocBench with 99.47% accuracy, 1.23x faster prefill, and 84.25% token retention.

Conclusion: RTPrune achieves a better efficiency-accuracy trade-off for DeepSeek-OCR, demonstrating its effectiveness in OCR tasks.
...
TL;DR: Proposed SIMON, a saliency-aware multi-view framework for zero-shot EEG-to-image retrieval, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: The center bias in existing EEG-to-image retrieval methods conflicts with content-driven human attention, causing a geometric-semantic dissociation.

Method: SIMON uses Saliency-Aware Sampling (SAS) to select fixation centers and generates foveated views emphasizing informative object regions.

Result: SIMON achieves state-of-the-art performance on THINGS-EEG with average Top-1 accuracy of 69.7% (intra-subject) and 19.6% (inter-subject).

Conclusion: The proposed SIMON framework demonstrates the effectiveness of saliency-aware multi-view integration for EEG-to-image retrieval.
...
TL;DR: The paper presents BOLT, a plug-and-play module for preparation-free heterogeneous cooperative perception, improving performance by adapting neighboring features online.


<details>
  <summary>Details</summary>
Motivation: Existing cooperative perception methods require prior preparation, which is impractical in real scenarios where agents are independently trained and meet online.

Method: BOLT uses ego-as-teacher distillation to adapt neighboring features online, requiring only ego predictions without ground-truth labels.

Result: BOLT improves AP@50 by up to 32.3 points over vanilla unadapted fusion and consistently outperforms ego-only results on DAIR-V2X and OPV2V datasets.

Conclusion: The proposed BOLT module enables effective preparation-free heterogeneous cooperative perception, outperforming existing methods and ego-only perception.
...
TL;DR: The paper proposes LeGS, a framework that uses Reinforcement Learning to optimize density control for 3D Gaussian Splatting, achieving better reconstruction quality and efficiency.


<details>
  <summary>Details</summary>
Motivation: The current heuristic density control methods in 3D Gaussian Splatting lack flexibility for diverse scenes with complex geometries.

Method: The authors introduce a learnable policy network optimized via Reinforcement Learning with a tailored reward function for density control.

Result: LeGS significantly outperforms state-of-the-art methods in reconstruction quality and efficiency on multiple datasets.

Conclusion: The proposed LeGS framework offers a superior balance between reconstruction quality and efficiency for 3D Gaussian Splatting.
...
TL;DR: The paper proposes LIMSSR, a framework for incomplete multimodal learning that leverages large language models to infer latent semantics from available contexts without direct reconstruction.


<details>
  <summary>Details</summary>
Motivation: Existing methods for incomplete multimodal learning rely on the unrealistic assumption of full-modal availability during training. The paper aims to tackle the more challenging setting of IML under training-time incomplete observations.

Method: The proposed LIMSSR framework reformulates the challenge as a conditional sequence reasoning task and leverages large language models via prompt-guided context-aware modality imputation and multidimensional representation fusion.

Result: Extensive experiments on three Action Quality Assessment datasets demonstrate that LIMSSR significantly outperforms state-of-the-art baselines without relying on complete training data.

Conclusion: The paper establishes a new paradigm for data-efficient multimodal learning and provides a framework that can infer latent semantics from available contexts without direct reconstruction.
...
TL;DR: Proposed MACF, a Multi-Agent Collaboration Framework for scalable video understanding, outperforming state-of-the-art models under identical budget constraints.


<details>
  <summary>Details</summary>
Motivation: Existing methods for long-video tasks in multi-modal large language models face limitations due to bounded perception context budgets, leading to information loss and high costs.

Method: MACF, an end-to-end framework that decouples per-agent perception budgets from global video complexity, enabling scalable video understanding while preserving visual fidelity.

Result: MACF consistently outperforms state-of-the-art MLLMs and multi-agent systems under identical budget constraints on diverse video understanding benchmarks.

Conclusion: The proposed MACF framework effectively enables scalable video understanding through latent collaboration, preserving visual fidelity and outperforming existing methods.
...
TL;DR: The paper proposes a method for detecting thoracic abnormalities in compressed chest CT volumes using a novel distillation framework called Feature Attention Style Transfer (FAST) and a parameter-efficient projection layer called Structured Factorized Projection (SFP).


<details>
  <summary>Details</summary>
Motivation: The deployment of AI in medical imaging is hindered by high computational complexity and resource-intensive processing of volumetric data. The use of compressed chest CT volumes can address the growing need for low-resource deployment and efficient electronic data transfer.

Method: The proposed method, CT-Lite, integrates FAST and SFP with a SigLIP-based multimodal alignment objective. FAST transfers activation patterns and structural relationships from high-fidelity CT representations to a spatiotemporal visual encoder operating on compressed inputs. SFP uses Block Tensor Train decomposition to reduce projection-head parameters.

Result: Experiments on three datasets (CT-RATE, NIDCH, and Rad-ChestCT) demonstrate that CT-Lite achieves AUROC within 5-7% of the uncompressed-input baseline, despite operating on compressed inputs with significantly fewer parameters.

Conclusion: The proposed method, CT-Lite, paves the way for AI-based clinical evaluation under resource constraints by enabling efficient and robust feature extraction from compressed chest CT volumes.
...
TL;DR: The paper introduces a unified framework for interpreting vision models, providing insights into where evidence comes from, what the model encodes, and how it assembles that evidence.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the fragmented understanding of vision models' internal workings, specifically in terms of evidence assembly, feature encoding, and interpretability.

Method: The proposed framework centers around the pointwise feature vector (PFV) and instance-specific Effective Receptive Field (iERF), and consists of three main components: Sharing Ratio Decomposition (SRD) for local interpretability, Concept-Anchored Feature Explanation (CAFE) for global interpretability, and Interlayer Concept Graph with Interlayer Concept Attribution (ICAT) for analyzing representation composition through depth.

Result: The framework empirically outperforms baselines in both fidelity and robustness across various models (ResNet50, VGG16, and ViTs), successfully interprets dispersed SAE features, and exposes dominant concept routes in correct, misclassified, and adversarial cases.

Conclusion: The proposed framework provides a coherent, evidence-backed map from pixels to concepts to decisions, offering a unified and comprehensive approach to interpreting vision models.
...
TL;DR: This paper proposes an active learning framework that leverages vision-language models to reduce human annotation costs by combining fine-grained human annotations with coarse-grained VLM-generated weak labels.


<details>
  <summary>Details</summary>
Motivation: To reduce the reliance on costly human annotation within the active learning paradigm by leveraging vision-language models.

Method: The proposed framework combines fine-grained human annotations with coarse-grained VLM-generated weak labels through instance-wise label assignment and models systematic noise in VLM-generated labels using a small set of trusted full labels.

Result: The proposed framework consistently outperforms existing active learning methods under the same annotation budget on CUB200 and FGVC-Aircraft datasets.

Conclusion: The proposed framework effectively reduces the reliance on costly human annotation and achieves better performance than existing active learning methods.
...
TL;DR: The study investigates the impact of temporal resolution on zero-shot semantic understanding of rapid human actions, finding that higher frame rates significantly improve semantic separability.


<details>
  <summary>Details</summary>
Motivation: The challenge of collecting sufficient labeled data for supervised learning in scenarios with rapid and less common activities, making zero-shot approaches a practical alternative.

Method: A training-free pipeline combining a pre-trained video-language model with large language model-based reasoning for pairwise action comparison, evaluated across multiple frame rates.

Result: Higher temporal resolution significantly improves semantic separability in zero-shot settings, with high-speed video providing more stable and interpretable semantic representations for fast actions.

Conclusion: The study highlights the importance of temporal resolution in training-free action recognition and suggests that high-speed perception can enhance semantic understanding capabilities.
...
TL;DR: A novel framework, GOR-IS, is proposed for physically consistent and visually coherent 3D object removal, addressing challenges in global lighting effects and non-Lambertian surfaces.


<details>
  <summary>Details</summary>
Motivation: Existing methods for 3D object removal often neglect global lighting effects and struggle with view-dependent non-Lambertian surfaces, leading to physically implausible and unreliable inpainting results.

Method: The proposed framework, GOR-IS, decomposes the scene into intrinsic components, explicitly models light transport, and operates in the material and lighting domains for inpainting.

Result: GOR-IS outperforms existing methods by 13% in perceptual similarity (LPIPS) and 2dB in peak signal-to-noise ratio (PSNR) on both synthetic and real-world datasets.

Conclusion: The proposed GOR-IS framework provides a more physically consistent and visually coherent approach to 3D object removal, effectively addressing challenges in global lighting effects and non-Lambertian surfaces.
...
TL;DR: The paper presents an end-to-end training pipeline for autoregressive image modeling that jointly optimizes reconstruction and generation, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve autoregressive image modeling by addressing the limitations of two-stage approaches that train tokenizers and generative models separately.

Method: The method involves designing an end-to-end training pipeline that jointly optimizes reconstruction and generation, and leveraging vision foundation models to improve 1D tokenizers.

Result: The result is a state-of-the-art FID score of 1.48 on ImageNet 256x256 generation without guidance.

Conclusion: The conclusion is that the proposed approach enables direct supervision from generation results to the tokenizer, leading to strong empirical results in autoregressive image modeling.
...
TL;DR: A novel optimization strategy, PhysiGen, is proposed to reduce body inter-penetration in multi-person interaction sequences, improving realism and usability.


<details>
  <summary>Details</summary>
Motivation: Generating realistic multi-person interaction sequences with minimal body inter-penetration remains a challenge in text-driven 3D human motion synthesis.

Method: PhysiGen integrates collision-aware physical constraints by simplifying human body meshes into geometric primitives for efficient inter-person collision detection and optimization.

Result: PhysiGen effectively reduces interpenetration, improving visual coherence and physical plausibility compared to state-of-the-art methods, as shown in cross-dataset and cross-model experiments.

Conclusion: PhysiGen is a general-purpose, computationally efficient strategy that can be readily integrated into existing human interaction generation models to improve realism and usability.
...
TL;DR: IdentiFace, a novel diffusion-based framework, generates identifiable suspect faces from text or sketches, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Traditional suspect face generation methods are inefficient and produce low-quality results, while diffusion-based approaches have limitations in conditional ambiguity and sampling variance.

Method: IdentiFace uses multi-modal input design and an iterative generation pipeline to address limitations in conditional control and feature adjustment.

Result: IdentiFace achieves superior performance over existing methods, especially in identity retrieval, and shows strong potential for practical applications.

Conclusion: IdentiFace offers a more efficient and effective solution for suspect face generation, with significant implications for crime investigations.
...
TL;DR: The authors propose a new approach for extracting topologically accurate vascular graphs from 3D image data, achieving state-of-the-art performance on several benchmark datasets.


<details>
  <summary>Details</summary>
Motivation: The existing segment-then-fix paradigm for blood vessel segmentation and tracing is fundamentally limited in modeling complete and topologically accurate vascular network reconstruction.

Method: The approach predicts voxel-wise vessel direction vectors and segmentation masks, then uses a direction-vector-guided extension of the TEASAR algorithm to extract the vascular graph.

Result: The approach achieves state-of-the-art performance on three benchmark datasets, including synthetic and real imagery, and demonstrates applicability to challenging 3D micro-CT scans of rat heart vasculature.

Conclusion: The proposed approach substantially improves the topological accuracy of reconstructed vascular graphs, handling closely apposed vessel segments and multiple vascular trees within a single volume.
...
TL;DR: The paper explores the role of input noise in text-to-image diffusion models, finding that low-frequency components control global structure and color, while high-frequency components control finer details. A simple method is proposed to manipulate low-frequency noise and steer image generation.


<details>
  <summary>Details</summary>
Motivation: The authors aim to improve control over specific visual attributes in text-to-image diffusion models by investigating the characteristics of input noise.

Method: The authors analyze the frequency components of white Gaussian noise and propose a simple, training-free method to manipulate low-frequency noise using low-frequency image priors.

Result: The proposed method allows for steering overall image structure and color while maintaining variability in generated outputs.

Conclusion: The paper concludes that understanding the characteristics of input noise can lead to improved control over image generation in diffusion models, and proposes a simple and effective method to achieve this.
...
TL;DR: A new privacy-preserving scene representation called 'sphere cloud' is proposed to protect visual localization from revealing private maps, by lifting points to 3D lines crossing the centroid of the map.


<details>
  <summary>Details</summary>
Motivation: The emergence of deep neural networks capable of revealing high-fidelity scene details from sparse 3D point clouds has raised significant privacy concerns in visual localization involving private maps.

Method: The proposed method involves lifting map points to 3D lines crossing the centroid of the map, and introducing a simple yet effective cloud construction strategy to thwart new attacks.

Result: Experimental results on public RGB-D datasets demonstrate that the sphere cloud achieves competitive privacy-preserving ability and localization runtime while not excessively compensating the pose estimation accuracy compared to other depth-guided localization methods.

Conclusion: The proposed sphere cloud method effectively neutralizes the density-based attack and achieves competitive performance in terms of privacy-preserving ability, localization runtime, and pose estimation accuracy.
...
TL;DR: The paper improves 2D Gaussian Splatting by incorporating monocular depth and normal priors for better surface reconstruction.


<details>
  <summary>Details</summary>
Motivation: The volumetric nature of 3DGS limits its ability to accurately capture surface geometry, and 2DGS can be sensitive to initialization.

Method: The paper proposes a depth-guided initialization strategy for Gaussians and a clustering-based technique for pruning degenerate Gaussians.

Result: The method achieves state-of-the-art results in mesh reconstruction on the DTU dataset while preserving high-quality novel view synthesis.

Conclusion: The proposed method enhances 2DGS by improving geometric accuracy and robustness.
...
TL;DR: The paper explores jailbreak attacks on vision-language models (VLMs) using the visual modality, finding that text-based safety training does not generalize to visually conveyed harmful intent.


<details>
  <summary>Details</summary>
Motivation: The authors aim to expose the vulnerability of VLMs to safety alignment bypassing through the visual modality, which is an underexplored attack surface.

Method: The authors introduce four jailbreak attacks: (1) visual symbol sequences with a decoding legend, (2) replacing harmful objects with benign substitutes, (3) replacing harmful text in images with benign words, and (4) visual analogy puzzles. They evaluate these attacks across six frontier VLMs.

Result: The visual attacks successfully bypass safety alignment, revealing a cross-modality alignment gap. For example, a visual cipher achieves 40.9% attack success on Claude-Haiku-4.5 versus 10.7% for an equivalent textual cipher.

Conclusion: The findings emphasize that robust VLM alignment requires treating vision as a first-class target for safety post-training, as text-based safety training does not automatically generalize to harmful intent conveyed visually.
...
TL;DR: The paper proposes Double-Softmax Prompt Tuning (DSPT) to improve the robustness of CLIP models to label noise.


<details>
  <summary>Details</summary>
Motivation: CLIP models are sensitive to label noise, which can cause large gradients that overwhelm pre-trained priors.

Method: The authors propose DSPT, a hyperparameter-free method that applies sequential probabilistic normalization to suppress gradients from high-error noisy samples.

Result: DSPT achieves state-of-the-art robustness across various noisy benchmarks, outperforming methods with complex architectures and handcrafted hyperparameters.

Conclusion: The proposed DSPT method effectively transforms 'gradient vanishing' into a noise-filtering shield for label-noise prompt tuning, achieving robust zero-shot generalization.
...
TL;DR: The paper explores integrating vehicle-to-everything (V2X) information into 3D object detection for automated driving, enhancing performance but also requiring robustness to V2X imperfections.


<details>
  <summary>Details</summary>
Motivation: Onboard environmental sensors have limitations such as line-of-sight and field-of-view constraints, which can cause perception failures under occlusions or poor visibility. Cooperative awareness via V2X communication can complement onboard perception.

Method: The authors use the nuScenes dataset to emulate object-level cooperative awareness messages, inject controlled noise and object dropout, and fuse these messages into a BEVFusion-style detector. They propose a noise-aware training strategy with explicit confidence encoding.

Result: The results show that object-level cooperative information can improve detection performance (NDS of 0.80 under favorable conditions), but models trained on idealized data are fragile and over-reliant on V2X. The proposed strategy enhances robustness, maintaining performance gains even under severe noise and reduced V2X penetration.

Conclusion: The integration of V2X information into 3D object detection can enhance performance, and a noise-aware training strategy with explicit confidence encoding can improve robustness to realistic V2X imperfections.
...
TL;DR: FaithEIR, a diffusion-based framework, is proposed for extreme image rescaling, achieving superior reconstruction fidelity and perceptual quality.


<details>
  <summary>Details</summary>
Motivation: Recent extreme rescaling methods struggle to preserve semantically consistent structures and produce realistic details when scaling factors are 16x or higher.

Method: FaithEIR uses a learnable reversible transformation, an adaptive detail prior, and a lightweight pixel semantic embedder with a pretrained diffusion model.

Result: FaithEIR consistently outperforms state-of-the-art methods in reconstruction fidelity and perceptual quality.

Conclusion: The proposed FaithEIR framework effectively addresses the challenges of extreme image rescaling, offering improved performance over existing methods.
...
TL;DR: The paper proposes a novel approach called CMTA to detect AI-generated videos by analyzing cross-modal temporal artifacts, achieving state-of-the-art results on four large-scale datasets.


<details>
  <summary>Details</summary>
Motivation: Existing AI-generated video detection methods overlook rich cues in the visual-textual cross-modal space, particularly temporal stability of semantic alignment, which is crucial for distinguishing AI-generated videos from real ones.

Method: The CMTA framework uses BLIP to generate frame-level image captions and CLIP to extract visual-textual representations. It then employs a coarse-grained temporal modeling branch with a GRU and a fine-grained branch with a Transformer encoder to capture temporal fluctuations and inter-frame variations.

Result: The proposed approach achieves a new state-of-the-art on four large-scale datasets (GenVideo, EvalCrafter, VideoPhy, and VidProM) and exhibits superior cross-generator generalization.

Conclusion: The paper concludes that the CMTA framework effectively identifies AI-generated videos by detecting cross-modal temporal artifacts, providing a robust solution for ensuring digital video authenticity.
...
TL;DR: BlenderRAG, a retrieval-augmented generation system, improves the automatic generation of executable Blender code from natural language by leveraging a curated multimodal dataset.


<details>
  <summary>Details</summary>
Motivation: The automatic generation of executable Blender code from natural language remains challenging due to frequent syntactic errors and geometrically inconsistent objects produced by state-of-the-art LLMs.

Method: BlenderRAG operates on a curated multimodal dataset of 500 expert-validated examples and retrieves semantically similar examples during generation.

Result: BlenderRAG improves compilation success rates from 40.8% to 70.0% and semantic normalized alignment from 0.41 to 0.77 across four state-of-the-art LLMs.

Conclusion: BlenderRAG makes the generation of executable Blender code from natural language more efficient and accessible without requiring fine-tuning or specialized hardware.
...
### [45] [InpaintSLat: Inpainting Structured 3D Latents via Initial Noise Optimization](https://arxiv.org/abs/2605.00664)
*Jaeyoung Chung,Suyoung Lee,Kyoung Mu Lee*

Main category: cs.CV

TL;DR: A training-free approach for controllable 3D inpainting using initial noise optimization is proposed, improving contextual consistency and prompt alignment.


<details>
  <summary>Details</summary>
Motivation: The underlying geometric structure in 3D latent diffusion frameworks is sensitive to initial noise, compromising stability in tasks like inpainting and editing.

Method: A strategy to optimize initial noise using backpropagation approximation and spectral parameterization is introduced.

Result: Experiments show consistent improvements in contextual consistency and prompt alignment over baselines.

Conclusion: Initial noise control is established as an independent dimension for 3D inpainting, orthogonal to conventional sampling trajectory manipulation.
...
TL;DR: A unified multimodal framework called UniVidX is proposed for versatile video generation, leveraging video diffusion models (VDMs) priors. It achieves competitive performance across distinct tasks and generalizes robustly to in-the-wild scenarios.


<details>
  <summary>Details</summary>
Motivation: Existing methods for multimodal graphics tasks often train separate models for each problem setting, limiting the modeling of correlations across modalities. The authors aim to develop a unified framework that can handle diverse tasks.

Method: UniVidX formulates pixel-aligned tasks as conditional generation in a shared multimodal space. It uses Stochastic Condition Masking (SCM), Decoupled Gated LoRA (DGL), and Cross-Modal Self-Attention (CMSA) to promote cross-modal consistency and adapt to modality-specific distributions.

Result: Experiments show that UniVidX achieves performance competitive with state-of-the-art methods across distinct tasks and generalizes robustly to in-the-wild scenarios, even when trained on fewer than 1,000 videos.

Conclusion: UniVidX is a unified multimodal framework that leverages VDM priors for versatile video generation, achieving competitive performance and generalizing robustly across tasks and scenarios.
...
TL;DR: LightKV reduces KV cache size in Large Vision-Language Models by exploiting redundancy among vision-token embeddings, guided by text prompts.


<details>
  <summary>Details</summary>
Motivation: The adoption of Key-Value (KV) cache in Large Vision-Language Models (LVLMs) introduces substantial GPU memory overhead due to the large number of vision tokens processed during the prefill stage.

Method: LightKV employs cross-modality message passing to aggregate informative messages across vision tokens and progressively compress them during prefill, guided by text prompts.

Result: LightKV halves the vision-token KV cache size, reduces computation by up to 40%, and preserves general-purpose performance while outperforming existing baselines, using only 55% of the original vision tokens.

Conclusion: LightKV is a novel approach that effectively reduces KV cache size in LVLMs, offering improved efficiency without compromising performance.
...
TL;DR: Deep learning models predict AD-related risk factors from colored fundus photography (CFP), suggesting CFP encodes retinal signatures linked to AD risk factors.


<details>
  <summary>Details</summary>
Motivation: To determine if CFP contains retinal structural signatures corresponding to AD-related risk domains and if deep learning models can predict AD-related risk factors from CFP.

Method: Deep learning models were trained on 62,876 UK Biobank CFPs from 44,501 participants to predict 12 AD-related risk factors.

Result: The models performed well, with AUROC= 0.5654-0.9480 for categorical and R2=-0.0291-0.7620 for continuous factors, and highlighted biologically meaningful regions such as the optic nerve head and retinal vasculature.

Conclusion: CFP encodes retinal signatures linked to AD risk factors, and DL-derived retinal representations may uncover biologically meaningful risk-related structural changes mirroring potential AD vulnerability.
...
TL;DR: Proposed Persistent Visual Memory (PVM) to mitigate 'Visual Signal Dilution' in autoregressive Large Vision-Language Models (LVLMs), improving visual perception and accuracy.


<details>
  <summary>Details</summary>
Motivation: The 'Visual Signal Dilution' phenomenon in LVLMs causes visual attention to decay with generated sequence length, hindering performance in multimodal tasks.

Method: Introduced PVM, a lightweight learnable module integrated alongside the Feed-Forward Network (FFN) in LVLMs to ensure sustained visual perception.

Result: PVM brings notable improvements in accuracy with negligible parameter overhead, particularly in complex reasoning tasks, and resists length-induced signal decay.

Conclusion: PVM effectively mitigates signal suppression in deep generation, providing a distance-agnostic retrieval pathway for precise visual perception in LVLMs.
...
TL;DR: The paper proposes DMDSC, a framework that enhances Open-Set Recognition in medical imaging by adapting class-specific margins based on label frequency to address class imbalance.


<details>
  <summary>Details</summary>
Motivation: Medical imaging datasets often have extreme class imbalances, making it challenging for models to accurately classify known classes and reject unknown samples.

Method: The proposed framework, DMDSC, uses a dynamic margin approach that automatically adapts class-specific margins based on label frequency.

Result: Extensive experiments on diverse medical benchmarks show that DMDSC outperforms state-of-the-art methods.

Conclusion: The proposed DMDSC framework effectively addresses the challenge of class imbalance in medical imaging datasets, leading to improved performance in Open-Set Recognition tasks.
...
TL;DR: A new AI model, ViTCG, was developed for retrieving Aerosol Optical Depth (AOD) from hyperspectral imagery, achieving a 62% reduction in mean squared error compared to existing models.


<details>
  <summary>Details</summary>
Motivation: Conventional AOD retrieval methods are limited by their reliance on radiative transfer modeling and auxiliary data, while recent data-driven approaches often fail to exploit the spatial-spectral coherence of hyperspectral imagery.

Method: The proposed ViTCG model uses a Vision Transformer with Channel-wise Grouping-based spatial regression framework, taking hyperspectral top-of-atmosphere radiance as input and jointly modeling spatial context and spectral information.

Result: The ViTCG model demonstrated a 62% reduction in mean squared error compared to state-of-the-art foundation models, producing spatially coherent AOD fields.

Conclusion: The study presents a promising approach for AOD retrieval using Foundation AI models, specifically ViTCG, which can improve the accuracy and spatial consistency of AOD retrievals.
...
TL;DR: The paper proposes a Static and Dynamic Graph Alignment Network (SDGAN) to improve Temporal Video Grounding (TVG) by addressing challenges in visual representation, feature interaction, and semantic matching.


<details>
  <summary>Details</summary>
Motivation: Existing GCN-based TVG methods have three critical bottlenecks: incomplete visual representation, inefficient feature interaction, and single-granularity semantic matching.

Method: The proposed SDGAN jointly exploits static and dynamic visual features, introduces Query-Clip Contrastive Learning and Adaptive Graph Modeling, and incorporates multi-granularity temporal proposals within Progressive Easy-to-Hard Training Strategy.
...
Conclusion: The proposed SDGAN effectively addresses the challenges in TVG and achieves state-of-the-art performance.
...
TL;DR: PhysEdit is an adaptive image editing framework that dynamically adjusts reasoning depth and spatial coverage based on the editing instruction and reference image, achieving a speedup and improved performance.


<details>
  <summary>Details</summary>
Motivation: Existing image editing frameworks apply a fixed inference recipe to all editing instructions, which can be inefficient and ineffective for diverse editing tasks. The authors argue that adaptivity in both spatial and temporal axes is necessary to improve performance.

Method: PhysEdit introduces two inference-time modules: Complexity-Adaptive Reasoning Depth (CARD) and Spatial Reasoning Mask (SRM). CARD predicts edit complexity and allocates reasoning steps, while SRM extracts an instruction-conditioned spatial prior to confine reasoning to relevant regions.

Result: PhysEdit achieves a 1.18x wall-clock speedup over a strong baseline, with improved instruction adherence (CLIP-T 0.2283 vs. 0.2266) and matching identity preservation (CLIP-I 0.8246 vs. 0.8280).

Conclusion: PhysEdit demonstrates the importance of adaptivity in image editing frameworks, achieving improved performance and efficiency through dynamic adjustment of reasoning depth and spatial coverage.
...
TL;DR: This paper explores using a hierarchical approach to improve knee osteoarthritis assessment with deep learning, achieving better results by treating both disease presence and severity grade as related tasks.


<details>
  <summary>Details</summary>
Motivation: Current deep learning methods for knee osteoarthritis assessment treat the disease presence and severity grade as separate classification problems, ignoring their natural hierarchical relationship.

Method: The authors propose a simple dual-head model with a shared encoder and two task-specific heads to leverage the hierarchical relationship between coarse binary OA decision and fine-grained Kellgren-Lawrence severity grade.

Result: The results show that dual-head supervision produces backbone-dependent gains, with clear improvements in KL-related metrics for selected backbones, and a more ordered coarse-to-fine latent organization.

Conclusion: The study suggests that simple hierarchical dual-head supervision can improve disease representations under noisy coarse/fine labels, providing a useful inductive bias for OA diagnosis and severity grading.
...
TL;DR: The authors propose RGSUD, an unsupervised image deraining method that uses a reward-guided self-reinforcement approach to improve deraining results without paired supervision.


<details>
  <summary>Details</summary>
Motivation: The lack of strong constraints in unsupervised deraining makes it difficult for networks to converge, especially with complex rain degradation. However, high-quality deraining results occasionally emerge during training, which can guide the optimization process.

Method: The proposed method, RGSUD, consists of two stages: reward recycling and self-reinforcement (SR) training. It uses an Image Quality Assessment (IQA)-based dynamic reward recycling mechanism and IQA-based self-reinforced loss to enhance the quality of synthesized pseudo-paired data and stabilize optimization.

Result: The method achieves State-of-the-Art (SOTA) performance across multiple datasets, including paired synthetic, paired real, and unpaired real images, outperforming existing unsupervised deraining approaches in both subjective and objective IQA metrics.

Conclusion: The proposed RGSUD method effectively improves unsupervised image deraining by leveraging high-quality deraining results that emerge during training. The self-reinforcement strategy is adaptable to other unsupervised deraining methods, and the deraining framework demonstrates strong generalization across existing supervised deraining networks.
...
TL;DR: The paper proposes a minimalist approach for single-point supervised infrared small target detection, achieving high precision with reduced annotation costs.


<details>
  <summary>Details</summary>
Motivation: Current state-of-the-art methods for infrared small target detection rely on dense annotation or multi-stage processes, which are costly. The authors aim to reduce these costs while maintaining high precision.

Method: The authors propose generating point-to-mask supervision online through in-batch, point-anchored feature-affinity propagation, instantiated as GSACP. They also introduce a protocolized single-variable ablation procedure to address optimization bottlenecks.

Result: GSACP-Final achieves a highly competitive 0.6674 mIoU on the SIRST3 dataset, with a 38% relative reduction in false-positive artifacts compared to PAL.

Conclusion: The proposed approach provides a compact alternative for deployment scenarios where false-alarm suppression is crucial, demonstrating a new ultra-low false-alarm operating regime.
...
TL;DR: The paper presents a quantum implementation of Sobel-based edge detection and Harris-style corner detection, using two quantum image encoding methods, and shows comparable results to classical methods.


<details>
  <summary>Details</summary>
Motivation: The study aims to leverage quantum computing for edge and corner detection in images, which are crucial in feature extraction, object tracking, and 3D modeling.

Method: The approach involves quantum gradient computation using lag-2 differences, and applies classical post-processing to improve detection quality. Two quantum image encoding methods, FRQI and QPIE, are comparatively analyzed.

Result: The proposed quantum circuits produce outputs consistent with classical Sobel and Harris operators. QPIE yields more stable and coherent results than FRQI, especially under limited measurement shots.

Conclusion: The work demonstrates a functional and scalable quantum realization of classical edge and corner detection methods, but notes that the overall cost is dominated by state preparation, measurement, and classical post-processing.
...
TL;DR: This paper introduces a new dataset and framework for modeling urban perception using street view images and eye-tracking recordings, and finds that incorporating gaze behavior improves prediction of subjective urban perception.
...
Method: The authors propose a Gaze-Guided Urban Perception Framework, which uses a dataset of street view images with synchronized eye-tracking recordings and individual perception labels to study how gaze behavior contributes to urban perception modeling.

Result: The experiments show that gaze alone carries useful predictive signals for subjective urban perception, and that integrating gaze with scene representations further improves prediction under both semantic and richer visual representations.
...
TL;DR: A novel framework, Map2World, is proposed for 3D world generation conditioned on user-defined segment maps, enabling global-scale consistency and flexibility.


<details>
  <summary>Details</summary>
Motivation: Current 3D world generation methods are limited by grid layouts and inconsistent object scales, hindering applications like immersive content creation and autonomous driving simulation.

Method: The Map2World framework uses user-defined segment maps of arbitrary shapes and scales, and a detail enhancer network to generate fine details while maintaining scene coherence.

Result: The proposed method significantly outperforms existing approaches in user-controllability, scale consistency, and content coherence.

Conclusion: The Map2World framework enables users to generate 3D worlds under complex conditions, with robust generalization across diverse domains and limited training data.
...
TL;DR: Proposed GMGaze, a multi-scale transformer architecture for gaze estimation, outperforming previous methods on four public benchmarks.


<details>
  <summary>Details</summary>
Motivation: Addressing challenges in CNN-based, transformer-based, and CLIP-based gaze estimation methods, including late fusion of image features, lack of factor-aware conditioning, and impractical capacity scaling.

Method: GMGaze uses a multi-scale transformer architecture with semantic prototype conditioning, early unified fusion, and sparse Mixture-of-Experts modules.

Result: GMGaze achieves state-of-the-art results on four public benchmarks (MPIIFaceGaze, EYEDIAP, Gaze360, and ETH-XGaze) with mean angular errors of 2.49°, 3.22°, 10.16°, and 1.44°, respectively.

Conclusion: GMGaze effectively addresses the challenges in gaze estimation and provides a more accurate and efficient approach.
...
TL;DR: GenLIP is a generative pretraining framework for Vision Transformers (ViTs) in multimodal large language models (MLLMs), aligning vision encoders with autoregressive LLMs.


<details>
  <summary>Details</summary>
Motivation: To better align vision encoders with the autoregressive nature of LLMs, and to simplify, scale, and improve performance of multimodal models.

Method: GenLIP trains a ViT to predict language tokens directly from visual tokens using a standard language modeling objective, without contrastive batch construction or an additional text decoder.

Result: GenLIP achieves competitive or superior results across diverse multimodal benchmarks, matching or surpassing strong baselines with less pretraining data, and improving on detail-sensitive tasks.

Conclusion: GenLIP offers a simple, scalable, and high-performance foundation for vision encoders in MLLMs, making it suitable for multimodal large language models.
...
TL;DR: Introduces Posterior-Augmented Flow Matching (PAFM), an improvement over Flow Matching (FM) for training generative models, especially for high-dimensional images.


<details>
  <summary>Details</summary>
Motivation: The original Flow Matching (FM) method has issues with sparse and high-variance training signals for high-dimensional images, leading to flow collapse and poor generalization.

Method: PAFM uses an expectation over an approximate posterior of valid target completions, factorizing it into likelihood and prior probability, and employs importance sampling to construct a mixture over multiple candidate targets.

Result: PAFM improves over FM by up to 3.4 FID50K across different model scales, architectures, and benchmarks, with a negligible increase in compute overhead.

Conclusion: PAFM provides a theoretically grounded generalization of FM, reducing gradient variance and improving performance in generative modeling tasks.
...
TL;DR: The paper explores using minimal subsets of data to efficiently evaluate large audio models (LAMs), finding that small subsets can reliably predict model performance and user satisfaction.


<details>
  <summary>Details</summary>
Motivation: The rapid growth of large audio models demands efficient methods for comparison, but comprehensive benchmarks are costly. The authors aim to find a more efficient approach.

Method: The authors analyze 10 subset selection methods across 18 audio models and 40 tasks, and collect human preference ratings to evaluate the correlation between subset scores and user satisfaction.

Result: Subsets of 50 examples (0.3% of data) achieve over 0.93 Pearson correlation with full benchmark scores. Regression models trained on curated subsets achieve 0.98 correlation with human preferences, outperforming models trained on random subsets and the full benchmark.

Conclusion: The study demonstrates that well-curated subsets can reliably evaluate LAMs and predict user satisfaction more efficiently than full benchmarks, and introduces the HUMANS benchmark as an efficient proxy for LAM evaluation.
...
TL;DR: NorBERTo, a modern encoder model for Portuguese NLP, is introduced, trained on a large Brazilian Portuguese corpus, and achieves state-of-the-art results on several tasks.


<details>
  <summary>Details</summary>
Motivation: The need for high-quality corpora and advanced NLP models for the Portuguese language.

Method: NorBERTo is built on the ModernBERT architecture, trained on a 331 billion token corpus called Aurora-PT, and evaluated on tasks such as semantic similarity, textual entailment, and classification.

Result: NorBERTo-large achieves the best results among encoder models on PLUE and ASSIN 2 datasets, with F1 scores of 0.9191 on MRPC and 0.904 on entailment.

Conclusion: NorBERTo provides a modern, efficient, and effective encoder model for Portuguese NLP, with a large and openly available training corpus, suitable for realistic deployment scenarios.
...
TL;DR: This paper examines how large language models (LLMs) adjust their outputs based on neurodivergence (ND) context in system prompts and proposes a benchmark called NDBench to evaluate this adaptation.


<details>
  <summary>Details</summary>
Motivation: The motivation is to investigate if LLMs can adapt to neurodivergence context and to develop a framework for auditing future LLMs' adaptation to ND awareness.

Method: The authors propose NDBench, a benchmark involving two frontier models, three system prompt types, four canonical ND profiles, and 24 prompts across four categories.

Result: The study finds that LLMs show significant adaptation under ND context, with adjustments in output structure, such as longer and more structured outputs with more headings and granular steps.

Conclusion: The study concludes that LLMs can adapt to ND context, but ND persona assertion alone is not enough to suppress potentially harmful tendencies, and explicit instructions are needed for significant reduction in harm.
...
TL;DR: Introduced ViLegalNLI, a large-scale Vietnamese Natural Language Inference dataset for the legal domain, and evaluated its performance using various models.


<details>
  <summary>Details</summary>
Motivation: To create a comprehensive benchmark for Vietnamese legal inference tasks and support future research in legal reasoning and statutory text understanding.

Method: Proposed a semi-automatic data generation framework integrating large language models and systematic quality validation procedures to construct ViLegalNLI.

Result: Few-shot LLM configurations achieved superior performance, but performance was influenced by hypothesis length, lexical overlap, and reasoning complexity.

Conclusion: ViLegalNLI establishes a foundational benchmark for Vietnamese legal NLI and supports future research in legal reasoning and reliable AI systems for legal analysis.
...
TL;DR: A new dataset, ArabCulture-Dialogue, is introduced to evaluate cultural reasoning in LLMs across 13 Arabic-speaking countries, covering daily-life topics in Modern Standard Arabic and local dialects.


<details>
  <summary>Details</summary>
Motivation: Most Arabic benchmarks for LLMs focus on short text snippets in Modern Standard Arabic, neglecting cultural nuances in dialects and dialogues.

Method: The ArabCulture-Dialogue dataset is used to create three benchmarking tasks: multiple-choice cultural reasoning, machine translation between MSA and dialects, and dialect-steering generation.

Result: Experiments show a performance gap between MSA and Arabic dialects, with models performing worse on all tasks in dialectal settings compared to MSA.

Conclusion: The study highlights the need for more comprehensive cultural reasoning evaluation in LLMs, particularly in dialectal contexts, to bridge the performance gap.
...
TL;DR: The study proposes a Dual Prediction Violation (DPV) framework to understand humor, finding that temporal features significantly contribute to humor appreciation, often more so than semantic incongruity.


<details>
  <summary>Details</summary>
Motivation: The study aims to investigate the role of temporal structure in humor appreciation and its interaction with semantic content, as classical humor theories have overlooked temporal dynamics.

Method: The study analyzed 828 professional Chinese stand-up performances to examine the relationship between temporal features, semantic incongruity, and audience appreciation.

Result: The findings show that temporal features substantially outweigh semantic incongruity in predicting audience appreciation, and that peak semantic violations and pauses before high-surprise punchlines are key factors in successful performances.

Conclusion: The study reframes humor as temporally scaffolded, where timing and semantic content operate in strategic coordination, and demonstrates the central role of temporal structure in naturalistic humor appreciation.
...
TL;DR: The paper investigates confidence estimation for Automatic Short Answer Grading (ASAG) using large language models (LLMs), proposing a hybrid framework that combines model-based confidence signals with dataset-derived uncertainty, leading to more reliable confidence estimates and improved selective grading performance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the reliability of confidence estimates in LLM-based grading for safe and effective human-AI collaboration in educational decision-making, as current LLM-based grading remains imperfect.

Method: The method involves systematically comparing three model-based confidence estimation strategies and proposing a hybrid confidence framework that integrates model-based confidence signals with an explicit estimate of dataset-derived aleatoric uncertainty, operationalized by clustering semantically embedded student responses and quantifying within-cluster heterogeneity.

Result: The results demonstrate that the proposed hybrid confidence measure yields more reliable confidence estimates and improves selective grading performance compared to single-source approaches.

Conclusion: The paper concludes that the proposed hybrid confidence framework advances confidence-aware LLM-based grading for human-in-the-loop assessment, supporting more trustworthy AI-assisted educational assessment systems.
...
TL;DR: The paper analyzes the decision-making process of large language models (LLMs) in incomplete-information games, revealing two fundamental gaps: an observation-belief gap and a belief-action gap.


<details>
  <summary>Details</summary>
Motivation: The motivation is to understand the failures of LLMs in strategic decision-making under incomplete information, such as in negotiation and policymaking.

Method: The authors conducted experiments with open-weight models Llama 3.1, Qwen3, and gpt-oss to analyze the internal mechanisms underlying the decision-making of LLMs.

Result: The study found two gaps: (1) an observation-belief gap, where LLMs' internal beliefs are more accurate than their verbal reports but are brittle, and (2) a belief-action gap, where the conversion of internal beliefs into actions is weaker than that of externalized beliefs.

Conclusion: The results highlight systematic vulnerabilities in LLMs' decision-making processes, emphasizing the need for robust guardrails before deploying LLMs in strategic domains.
...
TL;DR: The authors developed a framework to simulate and evaluate the safety of AI companion applications, and used it to assess Replika's responses to high-risk user groups, finding that it often mirrors or normalizes unsafe content.


<details>
  <summary>Details</summary>
Motivation: Growing concerns about AI companion applications' risks, particularly in emotional engagement, and the limitations of existing safety evaluations relying on self-reported user data or interviews.

Method: A scalable framework integrating persona construction, scenario generation, multi-turn simulation, and harm evaluation, applied to Replika with 9 personas and 1,674 dialogue pairs.

Result: Replika exhibits a narrow emotional range and frequently mirrors or normalizes unsafe content, such as self-harm and violent narratives.

Conclusion: The framework can serve as a scalable testbed for evaluating safety risks in AI companions, highlighting the need for improved safety measures in these applications.
...
TL;DR: The paper introduces an evaluation framework for LLM-based Automated Short Answer Grading (ASAG) using Item Response Theory (IRT), which provides a more detailed analysis of grading performance across different student responses.


<details>
  <summary>Details</summary>
Motivation: Current evaluation metrics for ASAG, such as macro-F1 and Cohen's kappa, provide limited insight into grading performance variations across student responses of differing difficulty.

Method: The authors propose an IRT-based framework to model grading correctness as a function of latent grader ability and response grading difficulty, enabling response-level analysis of LLM graders.

Result: The results show that even models with similar overall performance differ substantially in how sharply their grading accuracy declines as response difficulty increases, and errors on difficult responses tend to concentrate on the 'partially_correct_incomplete' label.

Conclusion: The study demonstrates that Item Response Theory offers a useful framework for evaluating LLM-based ASAG beyond aggregate performance measures, providing a more nuanced understanding of grading performance.
...
TL;DR: A new Retrieval-Augmented Generation (RAG) framework, CA-ThinkFlow, is proposed to improve Large Language Models (LLMs) performance in complex, jurisdiction-specific tasks like Indian Chartered Accountancy (CA).


<details>
  <summary>Details</summary>
Motivation: The reliability of LLMs in complex tasks like Indian CA is limited due to their difficulty in executing numerical tasks requiring multiple steps and advanced knowledge about legal regulations.

Method: CA-ThinkFlow uses a parameter-efficient RAG framework with a 14B, 4-bit-quantized reasoning model and a layout-aware Docling extraction system. It employs a basic RAG method and the model's built-in Chain-of-Thought (CoT) functions.

Result: CA-ThinkFlow achieves performance levels matching large proprietary models on the CA-Ben benchmark, with Scholastic Reliability Coefficient (SRC) results equal to 68.75% of GPT-4o and Claude 3.5 Sonnet.

Conclusion: The proposed framework shows high efficiency and strength in handling parameters but fails to process complex regulatory texts in fields like Taxation.
...
TL;DR: The study reveals that recent white-box OOD detection methods for LLMs are confounded by sequence length and proposes a two-pathway framework to identify genuine OOD signals.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the issue of structurally confounded OOD detection methods in LLMs and improve their effectiveness.

Method: The study proposes a two-pathway framework that uses embeddings to capture what text is about and the processing trajectory to capture how the model processes input.

Result: The framework shows promising results with an average AUROC of 0.721 and 0.850 for Jailbreak, and provides evidence through three lines of analysis.

Conclusion: The study concludes that the proposed framework can effectively identify genuine OOD signals and improve OOD detection in LLMs.
...
TL;DR: A new framework for aggregating natural language judgments in high-conflict domains is proposed, using Weighted Maximum Satisfiability (MaxSAT) to generate logically coherent results.


<details>
  <summary>Details</summary>
Motivation: Standard methods for aggregating natural language judgments, such as majority voting, often fail in high-conflict domains.

Method: A neuro-symbolic aggregation framework that utilizes a language model to map natural language explanations into logical predicates and confidence weights, then encoded as soft constraints within the Z3 solver.

Result: The system generates logically coherent verdicts that diverge from popularity-based labels 62% of the time, with an 86% agreement rate with independent human evaluators.

Conclusion: The proposed framework demonstrates the efficacy of coupling neural semantic extraction with formal solvers to enforce logical soundness and explainability in the aggregation of noisy human reasoning.
...
TL;DR: A systematic approach combining quantitative analysis and large language models (LLMs) was developed to identify and characterize student misconceptions in online learning environments.


<details>
  <summary>Details</summary>
Motivation: The study aims to address the challenge of identifying and characterizing student misconceptions in online learning environments, which is crucial for providing targeted interventions and improving student outcomes.

Method: A two-stage methodology was employed, involving (1) identifying challenging topics using quiz-level performance metrics, and (2) characterizing underlying misconceptions using LLMs to analyze quiz question content, student response patterns, and lecture transcripts.

Result: The approach revealed actionable insights about student misconceptions, which were rated as excellent by subject matter experts, and provided a scalable approach to characterizing student difficulties in learning environments where quizzes are used.

Conclusion: The study demonstrates the potential for targeted and personalized interventions in future course iterations, with clear pathways for measuring intervention effectiveness through follow-up quiz performance.
...
TL;DR: The authors propose a structure-aware tabular chunking (STC) framework for improving retrieval-augmented generation (RAG) on tabular data, which preserves semantic relationships and improves token utilization.


<details>
  <summary>Details</summary>
Motivation: Existing chunking strategies for RAG are primarily designed for unstructured text and do not account for tabular structure, which is widely used in enterprise data pipelines.

Method: The STC framework constructs a hierarchical Row Tree representation, performs token-constrained splitting, and applies overlap-free greedy merging to produce dense, non-overlapping chunks.

Result: STC reduces chunk count by up to 40% and 56% compared to baselines, improves token utilization and processing efficiency, and enhances retrieval performance (MRR from 0.3576 to 0.5945, Recall@1 from 0.366 to 0.754).

Conclusion: Preserving structure during chunking improves retrieval performance, highlighting the importance of structure-aware chunking for RAG over tabular data.
...
TL;DR: The study examines the reliability of zero-shot vision-language model (VLM) safety classifiers under semantically equivalent prompt reformulation, finding that they can produce materially different unsafe probabilities, and proposes a training-free mean ensemble method to improve reliability.


<details>
  <summary>Details</summary>
Motivation: To investigate the reliability of single-prompt first-token probabilities from zero-shot VLM safety classifiers and to find a method to improve their reliability.

Method: The study uses a range of multimodal safety benchmarks and multiple VLM families to evaluate the reliability of single-prompt first-token probabilities, and proposes a training-free mean ensemble method as a reliability baseline.

Result: The study finds that cross-prompt variance is strongly associated with prompt-level disagreement and higher error, and that the proposed mean ensemble method improves NLL and ECE on most evaluation pairs, and wins more head-to-head comparisons than other calibration methods.

Conclusion: The study recommends prompt-family evaluation with mean aggregation as a standard label-free reliability baseline for zero-shot VLM first-token safety scores, and frames this as a reliability stress test for these scores.
...
TL;DR: The paper studies budgeted context selection for large language models, proposing a method called RCD that balances relevance, coverage, and diversity, and experiments with different unitization and selection methods.


<details>
  <summary>Details</summary>
Motivation: The high token cost per query and deployment cost of large language models, particularly with long and heterogeneous clinical inputs, and the need to meet fixed cost and latency constraints.

Method: The paper proposes RCD, a monotone submodular objective, and compares different unitization methods (sentence, section, window, and cluster-based) and selection methods, including a routing heuristic that adapts to the budget regime.

Result: The experiments show that optimal strategies depend on the evaluation setting, with positional heuristics performing best at low budgets in extractive tasks, and diversity-aware methods such as MMR improving LLM generation. The choice of selector matters more than unitization.

Conclusion: The paper concludes that the choice of context selection method can significantly impact the performance of large language models, and that RCD is a effective method for balancing relevance, coverage, and diversity under a strict token budget.
...
TL;DR: EVICT accelerates MoE speculative decoding by adaptively verifying draft candidates, achieving up to 2.35x speedup over autoregressive decoding.


<details>
  <summary>Details</summary>
Motivation: Tree-based speculative decoding loses advantage for sparse Mixture-of-Experts (MoE) models due to increased verification cost as draft tree grows.

Method: EVICT, a training-free, hyperparameter-free, and lossless adaptive verification method, truncates draft tree before target verification, retaining cost-effective prefix.

Result: EVICT achieves up to 2.35x speedup over autoregressive decoding and 1.21x speedup over EAGLE-3, reducing unnecessary expert activations during verification.

Conclusion: EVICT is an effective method for accelerating MoE speculative decoding, achieving significant speedup and reducing verification cost.
...
TL;DR: MemRouter, a new memory router, improves long-term conversational agents by efficiently deciding which turns to store in external memory, outperforming LLM-based memory managers.


<details>
  <summary>Details</summary>
Motivation: Recent conversational agents rely on autoregressive LLM generation at every turn, which is inefficient for long-term conversations. The authors aim to decouple memory admission from the downstream answer backbone.

Method: MemRouter uses an embedding-based routing policy, encoding each turn with recent context, and predicting whether to store the turn using lightweight classification heads with 12M parameters.

Result: MemRouter outperforms LLM-based memory managers on every question category (overall F1 52.0 vs 45.6) and reduces memory-management latency from 970ms to 58ms.

Conclusion: Write-side memory admission can be learned by a small supervised router, while answer generation remains a separate downstream component in long-horizon conversational QA.
...
TL;DR: The paper studies the basis of LLM parameter editing methods and proposes a new approach that uses forward-propagation instead of backward spreading, achieving more accurate targets without increasing computational complexity.


<details>
  <summary>Details</summary>
Motivation: The current widely used method of backward spreading in LLM parameter editing has not been systematically investigated, and its limitations and potential failure modes need to be understood.

Method: The paper conducts a systematic study of the foundations of LLM parameter editing methods and proposes a new approach that optimizes the anchor point at the first editing layer and propagates it forward to obtain accurate targets for subsequent layers.

Result: The proposed approach achieves the same computational complexity as existing methods while producing more accurate layer-wise targets.

Conclusion: The paper provides a simple and elegant alternative to existing LLM parameter editing methods, which can benefit a wide range of methods without interfering with their computation or editing pipeline.
...
### [83] [Models Recall What They Violate: Constraint Adherence in Multi-Turn LLM Ideation](https://arxiv.org/abs/2604.28031)
*Garvin Kruthof*
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to assess whether large language models preserve fidelity to the original objective when iteratively refining ideas, and to introduce a benchmark for evaluating constraint adherence in multi-turn LLM-assisted scientific ideation.

Method: The method involves using DriftBench, a benchmark that includes 2,146 scored runs across seven models, four interaction conditions, and 38 research briefs from 24 scientific domains. The researchers also use a restatement probe and structured checkpointing to evaluate model performance.

Result: The results show that iterative pressure increases structural complexity and often reduces adherence to original constraints. The 'knows-but-violates' (KBV) rate, which measures constraint non-compliance despite preserved recall, ranges from 8% to 99% across models. Structured checkpointing partially reduces KBV rates but does not eliminate the dissociation.

Conclusion: The conclusion is that large language models often struggle to adhere to original constraints during iterative scientific ideation, and that DriftBench can be used to evaluate and improve model performance.
...
TL;DR: The paper proposes TokenUnlearn, a token-level framework for machine unlearning in large language models, improving privacy and regulatory compliance.


<details>
  <summary>Details</summary>
Motivation: Existing machine unlearning methods operate at the sequence level, leading to gradient noise, degraded utility, and suboptimal forgetting.

Method: TokenUnlearn uses token-level attribution, combining knowledge-aware and entropy-aware signals to selectively target critical tokens for removal.

Result: Experiments show consistent improvements over sequence-level baselines in forgetting effectiveness and utility preservation across three model architectures.

Conclusion: TokenUnlearn provides a more precise and effective approach to machine unlearning, enhancing privacy, safety, and regulatory compliance in large language models.
...
TL;DR: Developed multilingual translation technologies for Expo 2025 Osaka to enable a language-barrier-free experience.


<details>
  <summary>Details</summary>
Motivation: To realize a language-barrier-free experience at Expo 2025 Osaka, aligning with the Global Communication Plan 2025.

Method: Advancement of simultaneous interpretation systems with chunk-based input segmentation, context-aware translation, and multi-engine machine translation technologies.

Result: Successful demonstration deployments and real-world applications in collaboration with private companies, with several services and systems showcased at Expo 2025 Osaka.

Conclusion: The developed technologies have enabled language-barrier-free experience at Expo 2025 Osaka, contributing to the Global Communication Plan 2025.
...
TL;DR: The paper presents an AI-powered web application that provides real-time, authoritative substance use education by combining government records and peer-reviewed literature.


<details>
  <summary>Details</summary>
Motivation: The traditional delivery of substance education faces challenges in scalability, personalization, and information currency, and the authors aim to explore the potential of AI in addressing these issues.

Method: The authors developed an AI web application using retrieval-augmented generation with a filtered corpus of 102 documents and dynamic PubMed queries, and conducted an expert evaluation study with a panel of subject matter experts.

Result: The system received high ratings (mean: 4.18-4.35) across four criteria (factual accuracy, citation quality, contextual coherence, and regulatory appropriateness) with substantial inter-rater agreement (Cohen's kappa = 0.78).

Conclusion: The findings suggest that AI architectures integrating authoritative regulatory sources with real-time scientific literature can provide scalable, accurate, and verifiable health education, warranting further evaluation through longitudinal user studies.
...
TL;DR: The paper presents Agent Capsules, a runtime that optimizes multi-agent pipeline execution by merging agents into fewer LLM calls while maintaining quality.


<details>
  <summary>Details</summary>
Motivation: The motivation is to reduce token usage in multi-agent pipelines by merging agents into fewer LLM calls without degrading quality.

Method: The method involves treating multi-agent pipeline execution as an optimization problem with empirical quality constraints, using a runtime that instruments coordination overhead, scores composition opportunity, and selects among three compound execution strategies.

Result: The results show that Agent Capsules uses fewer tokens than baselines (LangGraph, DSPy, MIPROv2) while maintaining or improving quality, with specific token usage reductions of 51%, 42%, 19%, and 68% in different scenarios.

Conclusion: The conclusion is that Agent Capsules provides an efficient and adaptive execution runtime for multi-agent pipelines, achieving token savings without sacrificing quality.
...
TL;DR: Small language models (SLMs) with 3-4 billion parameters can achieve strong performance in radiology tasks through LoRA fine-tuning, enabling deployment on consumer-grade CPUs.


<details>
  <summary>Details</summary>
Motivation: The deployment of large language models (LLMs) in radiology is limited by computational requirements, and the authors investigate whether small language models (SLMs) can achieve strong multi-task radiology performance.

Method: The authors train and evaluate two SLMs, Qwen2.5-3B-Instruct and Qwen3-4B, on 162K samples spanning 9 radiology tasks using LoRA fine-tuning.

Result: The authors find that LoRA fine-tuning improves performance over zero-shot baselines, and the two models exhibit complementary strengths; a task-outed oracle ensemble combining both models achieves the best performance across all tasks.

Conclusion: Small, efficiently fine-tuned models can serve as practical multi-task radiology AI assistants deployable entirely on consumer hardware without GPU requirements.
...
TL;DR: The paper proposes Reinforced Mode Regulation (RMR) to address mode collapse in autoregressive text generation by regulating the model's internal trajectory in representation space.


<details>
  <summary>Details</summary>
Motivation: Mode collapse in generative modeling, particularly in autoregressive text generation, where the model's output becomes limited and diverse, leading to premature convergence.

Method: The authors propose RMR, a lightweight online state-space intervention that regulates dominant self-reinforcing directions in the Transformer value cache using low-rank damping.

Result: RMR substantially reduces mode collapse and enables stable, high-quality generation at low entropy rates (down to 0.8 nats/step) across multiple large language models.

Conclusion: The paper concludes that mode collapse is not just a token-level phenomenon and that RMR offers a effective solution to address it, enabling more diverse and stable text generation.
...
TL;DR: This study examines how task phrasing affects large-language models' (LLMs) performance in unpredictable real-world applications, finding that LLMs are susceptible to presumptions based on task phrasing, which can be mitigated with neutral phrasing.


<details>
  <summary>Details</summary>
Motivation: Concerns about the safety and reliability of applying LLMs in real-world applications, where tasks may deviate from expectations.

Method: The study used the iterated prisoner's dilemma as a case study to investigate the impact of presumptions on LLMs' performance.

Result: LLMs are susceptible to presumptions when making decisions, even with reasoning steps, but neutral task phrasing reduces these presumptions and enables logical reasoning.

Conclusion: Proper task phrasing is crucial to minimize presumptions in LLMs and ensure reliable performance in real-world applications.
...
TL;DR: The study explores personalizing Plain Language Summaries (PLS) using large language models (LLMs) to improve comprehension for lay readers, particularly in health contexts, and highlights the trade-offs between personalization and safety.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitation of one-size-fits-all PLS by personalizing them to meet individual readers' information needs and comprehension levels, which is particularly crucial in health contexts where misunderstanding can impact real-world decisions.

Method: The study introduces ReLay, a dataset of 300 participant--PLS pairs, and uses it to evaluate five LLMs across two personalization methods, comparing their effectiveness in improving comprehension and perceived quality.

Result: Personalization improves comprehension and perceived quality but also raises the risk of reinforcing user biases and introducing hallucinations, indicating a trade-off between personalization and safety.

Conclusion: The findings emphasize the need for developing personalization methods that are both effective and trustworthy for diverse lay audiences, balancing the benefits of personalization with the risks to safety and accuracy.
...
TL;DR: The study models utterance production as a probabilistic choice over contextual alternatives, finding that surprisal minimisation provides the strongest predictive account of speaker and listener pressures in naturalistic language production.


<details>
  <summary>Details</summary>
Motivation: To understand how speakers choose utterances in context, and to derive speaker- and listener-oriented interpretations of different cost measures.

Method: The study uses information-theoretic notions of cost, distinguishes between goal-directed and goal-agnostic alternatives, and presents a procedure to generate alternative sets using language models.

Result: Surprisal minimisation relative to goal-directed alternatives provides the strongest predictive account, while uniform information density and length-based costs exhibit weaker predictive power.

Conclusion: The study suggests that alternative-conditioned optimisation with LM-generated alternatives provides a principled framework for studying speaker and listener pressures in naturalistic language production.
...
TL;DR: ControBench is a new benchmark for analyzing controversial online discussions, combining social interaction graphs with rich text semantics, and showing challenges in predicting user behavior across ideological lines.


<details>
  <summary>Details</summary>
Motivation: Existing datasets do not fully capture the complexities of online arguments across ideological divides, lacking either text semantics or interaction structure, or failing to represent stable user-level ideological identities.

Method: ControBench uses heterogeneous social interaction graphs with semantically enriched edges, combining user and post nodes, and user labels derived from self-declared Reddit flairs to represent ideological identity.

Result: The benchmark dataset shows low or negative adjusted homophily across topics (Trump: -0.77, Abortion: 0.06, Religion: 0.04), and various models (graph neural networks, pretrained language models, large language models) exhibit distinct performance patterns, especially when ideological boundaries are ambiguous.

Conclusion: ControBench provides a challenging and realistic benchmark for controversial discourse analysis, highlighting the complexities of online debates and the need for improved models to understand and analyze them.
...
TL;DR: The paper introduces AGoQ, a method for efficient training of large language models by quantizing activations and gradients, reducing memory usage and improving training speed.


<details>
  <summary>Details</summary>
Motivation: Current quantization approaches for large language models are ineffective for 4-bit activations and 8-bit gradients, causing slow convergence or accuracy loss.

Method: The paper proposes two techniques: a layer-aware activation quantization algorithm and a gradient quantization algorithm with 8-bit storage and precision-preserving communication.

Result: AGoQ reduces memory usage by up to 52% and improves training speed by up to 1.34x compared to state-of-the-art systems, while maintaining convergence and accuracy.

Conclusion: The proposed AGoQ method effectively reduces memory requirements and improves training efficiency for large language models without sacrificing accuracy.
...
TL;DR: Proposed A11y-Compressor framework compresses accessibility trees for GUI interaction, improving efficiency and task success.


<details>
  <summary>Details</summary>
Motivation: Current accessibility tree representations are redundant and lack structural information, hindering reliable grounding for AI agents interacting with GUIs.

Method: A11y-Compressor framework with a transformation pipeline: modal detection, redundancy reduction, and semantic structuring.

Result: Compressed-a11y reduces input tokens by 78% and improves task success rates by 5.1 percentage points on average.

Conclusion: A11y-Compressor framework provides a compact and structured representation of accessibility trees, enhancing efficiency and effectiveness for AI agents interacting with GUIs.
...
TL;DR: The paper introduces SCISENSE, a framework for operationalizing ideation in scientific discovery, and presents a dataset and LLMs that improve research trajectory quality and novelty.


<details>
  <summary>Details</summary>
Motivation: Existing approaches treat the ideation phase in scientific discovery as a brief preamble, despite its central role in research. The authors aim to operationalize ideation as a structured sequence of cognitive stages.

Method: The authors construct a 100K-scale dataset of citation-conditioned research trajectories and develop a family of sensemaking LLMs (SCISENSE-LM) spanning 3B to 70B parameters. They train these models in two modes: Target and Infer.

Result: Target-trained models achieve a 2.0% improvement in trajectory quality over Infer-trained models and produce more novel and diverse outputs. Coding agents conditioned on Target trajectories produce research artifacts with higher executability and quality.

Conclusion: The study suggests that targeted ideation reduces cognitive burden on downstream agents, freeing them to explore more creatively. SCISENSE offers a practical tool for augmenting LLM-driven research workflows and a testbed for studying planning in scientific discovery.
...
TL;DR: The paper introduces an Encoding Probe approach to study language model representations by reconstructing internal representations using interpretable features, addressing limitations of traditional decoding probe methods.


<details>
  <summary>Details</summary>
Motivation: The traditional decoding probe approach has limitations in comparing feature contributions and accounting for feature correlations, which the Encoding Probe aims to solve.

Method: The Encoding Probe method reconstructs internal model representations using interpretable features, and is evaluated on text and speech transformer models with various feature sets.

Result: The results show that speaker-related effects vary across training objectives and datasets, while syntactic and lexical features contribute independently to reconstruction.

Conclusion: The Encoding Probe provides a complementary perspective on interpreting model representations beyond decodability, offering insights into feature contributions and correlations.
...
TL;DR: The study examines if cosine similarity between paragraph embeddings remains consistent after machine translation, analyzing 2,800 political party platforms in 28 languages.


<details>
  <summary>Details</summary>
Motivation: To understand how machine translation affects the semantic structure of text, specifically in the context of paragraph embeddings.

Method: The study uses the Manifesto Corpus and EU eTranslation service to translate texts, then measures the stability of pairwise similarity relationships across embedding models.

Result: The study finds that translation invariance is preserved in 10 languages and detectably distorted in 4 languages, with the framework being corpus- and pipeline-agnostic.

Conclusion: The research provides a framework for assessing translation invariance in paragraph embeddings, highlighting languages where semantic structure is preserved or degraded after translation.
...
TL;DR: A new framework, SC-Taxo, is proposed to generate scientific taxonomies with improved semantic consistency using large language models.


<details>
  <summary>Details</summary>
Motivation: Existing taxonomy generation approaches often suffer from structural inconsistencies and semantic misalignment across hierarchical levels, making it challenging to efficiently organize and access domain knowledge.

Method: The SC-Taxo framework leverages large language models with hierarchy-aware refinement stages, introducing a bidirectional heading generation mechanism to ensure semantic consistency.

Result: Experiments on multiple benchmark datasets demonstrate consistent improvements in hierarchy alignment and heading quality, with robust cross-lingual generalization.

Conclusion: The proposed SC-Taxo framework effectively addresses the limitations of existing taxonomy generation approaches, providing a more structured and hierarchical representation of research fields.
...
TL;DR: The paper presents H-RAG, a hierarchical retrieval-augmented generation (RAG) pipeline for multi-turn conversational settings, achieving scores of 0.4271 on Task A and 0.3241 on Task C in SemEval-2026 Task 8.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve retrieval-augmented generation (RAG) in multi-turn conversational settings, addressing both standalone retrieval quality (Task A) and end-to-end RAG performance (Task C).

Method: The approach implements a hierarchical parent-child RAG pipeline, using hybrid dense-sparse search, tunable weighting, and embedding-based similarity rescoring over child chunks, with retrieved evidence aggregated at the parent level and supplied to an instruction-tuned language model for response generation.

Result: H-RAG achieves an nDCG@5 score of 0.4271 on Task A and a harmonic mean score of 0.3241 on Task C (RB_agg: 0.2488, RL_F: 0.2703, RB_llm: 0.6508).

Conclusion: The results underscore the importance of retrieval configuration and parent-level aggregation in multi-turn RAG performance, demonstrating the effectiveness of the proposed H-RAG approach.
...
TL;DR: The authors propose a continuously maintained evaluation platform, MathArena, to assess the mathematical reasoning capabilities of large language models (LLMs).


<details>
  <summary>Details</summary>
Motivation: Static benchmarks for evaluating LLMs' mathematical capabilities are limited in scope, quickly saturated, and rarely updated, making it difficult to compare models and track progress over time.

Method: The authors expand on the original MathArena benchmark to create a comprehensive evaluation platform that covers a wide range of mathematical tasks, including proof-based competitions, research-level problems, and formal proof generation.

Result: The strongest model, GPT-5.5, achieves 98% on the 2026 USA Math Olympiad and 74% on research-level questions, demonstrating that frontier models can comfortably solve extremely challenging mathematical problems.

Conclusion: The development of continuously maintained evaluation platforms like MathArena is crucial for tracking the rapid progress of LLMs in mathematical reasoning.
...
TL;DR: Introduced ML-Bench, a multilingual safety benchmark, and ML-Guard, a guardrail model, to improve safety and compliance in diverse regulatory and cultural environments for Large Language Models (LLMs).


<details>
  <summary>Details</summary>
Motivation: Existing multilingual benchmarks for LLMs have limitations in addressing region-specific regulations and cultural nuances, posing challenges in ensuring safety across diverse environments.

Method: Developed ML-Bench, a policy-grounded multilingual safety benchmark covering 14 languages, and ML-Guard, a Diffusion Large Language Model (dLLM)-based guardrail model with two variants (1.5B and 7B).

Result: ML-Guard consistently outperforms 11 strong guardrail baselines across 6 existing multilingual safety benchmarks and ML-Bench.

Conclusion: ML-Bench and ML-Guard can help advance the development of regulation-aware and culturally aligned multilingual guardrail systems for LLMs.
...
TL;DR: MemCoE, a two-stage framework inspired by cognition, improves long-term memory in large language model agents for better personalization.


<details>
  <summary>Details</summary>
Motivation: The need for long-term user memory in large language model agents for consistent personalization, hindered by limited context windows and inadequate existing memory systems.

Method: MemCoE uses a two-stage optimization framework: 1) Memory Guideline Induction to optimize a global guideline via contrastive feedback, and 2) Guideline-Aligned Memory Policy Optimization to learn a guideline-following memory evolution policy.

Result: Consistent improvements over strong baselines in three personalization memory benchmarks, with favorable robustness, transferability, and efficiency.

Conclusion: MemCoE offers a promising approach to enhancing long-term memory in large language model agents, leading to better personalization performance.
...
TL;DR: The paper proposes FinSafetyBench, a benchmark to test the safety of large language models (LLMs) in financial scenarios, and identifies vulnerabilities that allow LLMs to bypass compliance safeguards.


<details>
  <summary>Details</summary>
Motivation: The increasing application of LLMs in financial scenarios raises concerns about their potential to produce harmful outputs, including facilitating illegal activities or unethical behavior, posing serious compliance risks.

Method: The authors propose a bilingual (English-Chinese) red-teaming benchmark, FinSafetyBench, which comprises 14 subcategories spanning financial crimes and ethical violations, and conduct extensive experiments on general-purpose and finance-specialized LLMs under three representative attack settings.

Result: The experiments identify critical vulnerabilities that allow adversarial prompts to bypass compliance safeguards, and reveal stronger susceptibility in Chinese contexts and limitations of prompt-level defenses against sophisticated or implicit manipulation strategies.

Conclusion: The paper highlights the need for more effective safety measures to prevent LLMs from producing harmful outputs in financial scenarios, and emphasizes the importance of considering the limitations of current defenses.
...
TL;DR: The paper explains why local attention in transformer models improves model quality, and proves that combining global and local attention leads to better expressivity and performance.


<details>
  <summary>Details</summary>
Motivation: To provide a formal explanation for the phenomenon that local attention in transformer models improves model quality, despite being motivated by efficiency.

Method: The paper uses formal language theory and expressivity analysis to compare the capabilities of global and local attention mechanisms in transformer models.

Result: The paper proves that local attention introduces a second temporal operator, enlarging the class of recognizable regular languages, and that combining global and local attention yields the richest fragment. Experiments show that hybrid global-local transformers outperform global-only counterparts.

Conclusion: The paper provides a formal account of the benefits of local attention in transformer models, and demonstrates that combining global and local attention leads to better performance and expressivity.
...
TL;DR: This paper presents a new approach to sentiment analysis called Directed Social Regard (DSR) that can detect multiple targets and valences of sentiment in a single message.


<details>
  <summary>Details</summary>
Motivation: Current NLP tools are limited in that they can only classify overall sentiment as positive, neutral, or negative and cannot identify the targets of sentiment. The authors aim to address this limitation.

Method: The DSR approach uses a pair of transformer-based models to detect span-level targets of sentiment and score them along three axes of regard motivated by social science theories.

Result: The authors report promising results from a validation study and meaningful correlations between DSR outputs and labels/topics in pre-existing social science datasets.

Conclusion: The DSR approach provides a more nuanced understanding of sentiment in online platforms by detecting multiple targets and valences of sentiment in a single message.
...
TL;DR: Large language models' (LLMs) reasoning abilities are often overestimated due to their accuracy on benchmarks; however, they struggle with faithfully executing procedures as specified in prompts.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this study is to assess whether LLMs can faithfully execute procedures specified in prompts, beyond just achieving high accuracy on reasoning benchmarks.

Method: The study uses a controlled diagnostic benchmark involving step-wise arithmetic algorithms with two numeric inputs to evaluate LLMs' procedural execution.

Result: The results show a significant drop in accuracy as procedure length increases, from 61% on 5-step procedures to 20% on 95-step procedures, with various types of failures identified.

Conclusion: The study concludes that apparent reasoning abilities in LLMs can mask substantial weaknesses in faithfully executing instructions as specified.
...
### [108] [AgentReputation: A Decentralized Agentic AI Reputation Framework](https://arxiv.org/abs/2605.00073)
*Mohd Sameen Chishti,Damilare Peter Oyinloye,Jingyue Li*

Main category: cs.AI

TL;DR: The paper proposes a decentralized reputation framework called AgentReputation for agentic AI systems in decentralized marketplaces, addressing challenges with current reputation mechanisms.


<details>
  <summary>Details</summary>
Motivation: Existing reputation mechanisms fail in decentralized AI marketplaces due to agents optimizing against evaluations, competence not transferring across tasks, and varying verification rigor.

Method: The proposed AgentReputation framework consists of three layers: task execution, reputation services, and tamper-proof persistence, introducing explicit verification regimes and context-conditioned reputation cards.

Result: The framework provides a decision-facing policy engine for resource allocation, access control, and adaptive verification escalation, and outlines future research directions.

Conclusion: The paper concludes that AgentReputation can address the challenges of reputation mechanisms in decentralized AI marketplaces, and proposes several future research directions to further develop the framework.
...
### [109] [Minimal, Local, Causal Explanations for Jailbreak Success in Large Language Models](https://arxiv.org/abs/2605.00123)
*Shubham Kumar,Narendra Ahuja*

Main category: cs.AI

TL;DR: The paper introduces LOCA, a method for providing local, causal explanations of why specific jailbreak prompts succeed in bypassing safety measures in large language models (LLMs).


<details>
  <summary>Details</summary>
Motivation: The motivation is to understand why LLMs are vulnerable to jailbreak attacks, which could pose risks for future autonomous models in high-stakes settings.

Method: The method introduced is LOCA, which identifies minimal, interpretable changes in intermediate model representations that causally lead to model refusal on jailbreak requests.

Result: LOCA successfully induces refusal with an average of six interpretable changes, outperforming prior methods which often fail even after 20 changes.

Conclusion: LOCA represents a step toward providing mechanistic, local explanations for jailbreak success in LLMs, potentially improving their safety and reliability.
...
### [110] [Are Tools All We Need? Unveiling the Tool-Use Tax in LLM Agents](https://arxiv.org/abs/2605.00136)
*Kaituo Zhang,Zhen Xiong,Mingyu Zhong,Zhimeng Jiang,Zhouyuan Yuan,Zhecheng Li,Ying Lin*

Main category: cs.AI

TL;DR: Tool-augmented reasoning in LLMs doesn't always improve performance, especially with semantic distractors. A new framework and gate mitigate but don't fully resolve the issue.


<details>
  <summary>Details</summary>
Motivation: The assumed benefits of tool-augmented reasoning in LLMs do not always hold, particularly in the presence of semantic distractors.

Method: A Factorized Intervention Framework is proposed to analyze the performance gap, and G-STEP, a lightweight inference-time gate, is introduced to mitigate errors.

Result: The analysis reveals a tradeoff where the gains from tools often fail to offset the 'tool-use tax', but G-STEP yields partial recovery.

Conclusion: Improvements in tool-augmented reasoning require strengthening the model's intrinsic reasoning and tool-interaction capabilities.
...
### [111] [TUR-DPO: Topology- and Uncertainty-Aware Direct Preference Optimization](https://arxiv.org/abs/2605.00224)
*Abdulhady Abas Abdullah,Fatemeh Daneshfar,Seyedali Mirjalili,Mourad Oussalah*

Main category: cs.AI

TL;DR: The paper proposes TUR-DPO, a new method for aligning large language models with human preferences, which improves upon Direct Preference Optimization (DPO) by incorporating reasoning topology and uncertainty awareness.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the alignment of large language models with human preferences, addressing the limitations of current methods like DPO, which can be sensitive to noisy or brittle preferences.

Method: The method proposed is TUR-DPO, a topology- and uncertainty-aware variant of DPO, which elicits lightweight reasoning topologies and combines semantic faithfulness, utility, and topology quality into a calibrated uncertainty signal.

Result: The results show that TUR-DPO improves judge win-rates, faithfulness, and calibration relative to DPO, while preserving training simplicity and avoiding online rollouts, and matches or exceeds PPO on reasoning-centric tasks.

Conclusion: The conclusion is that TUR-DPO is a more effective and efficient method for aligning large language models with human preferences, particularly in settings that require reasoning and uncertainty awareness.
...
### [112] [ARMOR 2025: A Military-Aligned Benchmark for Evaluating Large Language Model Safety Beyond Civilian Contexts](https://arxiv.org/abs/2605.00245)
*Sydney Johns,Heng Jin,Chaoyu Zhang,Y. Thomas Hou,Wenjing Lou*

Main category: cs.AI

TL;DR: The paper introduces ARMOR 2025, a benchmark to test large language models' (LLMs) safety and alignment with military doctrines and regulations, revealing gaps in current LLMs' ability to make legally compliant decisions in military contexts.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the lack of evaluation methods for LLMs in military applications that reflect doctrinal standards, as existing benchmarks focus on general social risks rather than legal and ethical rules governing military operations.

Method: The authors introduce ARMOR 2025, a military-aligned safety benchmark based on three core military doctrines. They extract doctrinal text, generate multiple-choice questions, and organize the benchmark using a taxonomy informed by the OODA decision-making framework.

Result: The evaluation of 21 commercial LLMs using ARMOR 2025 reveals critical gaps in safety alignment for military applications, indicating that current LLMs are not adequately equipped to make legally compliant decisions in military contexts.

Conclusion: The paper concludes that ARMOR 2025 is a necessary benchmark to ensure LLMs are aligned with military doctrines and regulations, and that current LLMs have significant limitations in this regard, highlighting the need for further research and development.
...
### [113] [Causal Foundations of Collective Agency](https://arxiv.org/abs/2605.00248)
*Frederik Hytting Jørgensen,Sebastian Weichwald,Lewis Hammond*

Main category: cs.AI

TL;DR: The paper discusses the challenge of identifying collective agency in groups of simpler agents and proposes a framework using causal games and causal abstraction to analyze and predict collective behavior.


<details>
  <summary>Details</summary>
Motivation: The possibility that multiple simpler agents might form a collective agent with capabilities and goals distinct from those of any individual, posing a key challenge for the safety of advanced AI systems.

Method: The authors adopt a behavioral perspective, using causal games and causal abstraction to formalize collective agency and analyze multi-agent interactions.

Result: The framework is applied to solve a puzzle regarding multi-agent incentives in actor-critic models and to assess the degree of collective agency exhibited by different voting mechanisms.

Conclusion: The proposed framework aims to provide a foundation for understanding, predicting, and controlling emergent collective agents in multi-agent AI systems.
...
### [114] [Agentic AI for Trip Planning Optimization Application](https://arxiv.org/abs/2605.00276)
*Tiejin Chen,Ahmadreza Moradipari,Kyungtae Han,Hua Wei,Nejib Ammar*

Main category: cs.AI

TL;DR: The paper presents an agentic AI framework for optimal trip planning in intelligent vehicles, outperforming existing methods with 77.4% accuracy on the TOP Benchmark.


<details>
  <summary>Details</summary>
Motivation: Existing trip planning systems are limited to feasibility-oriented planning and lack objective evaluation of optimization performance due to the absence of ground truth in benchmarks.

Method: The authors propose an agentic AI framework with an orchestration agent coordinating specialized agents for traffic, charging, and points of interest, along with the Trip-planning Optimization Problems Dataset.

Result: The proposed system achieves 77.4% accuracy on the TOP Benchmark, significantly outperforming single-agent and workflow-based multi-agent baselines.

Conclusion: The paper demonstrates the importance of orchestrated agentic reasoning for robust trip planning optimization, addressing the limitations of existing systems and benchmarks.
...
### [115] [Token Arena: A Continuous Benchmark Unifying Energy and Cognition in AI Inference](https://arxiv.org/abs/2605.00300)
*Yuxuan Gao,Megan Wang,Yi Ling Yu*

Main category: cs.AI

TL;DR: The paper introduces TokenArena, a continuous benchmark for AI systems at the endpoint level, evaluating inference performance across five core axes and providing a more nuanced comparison than traditional benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current public inference benchmarks compare AI systems at the model and provider level, but deployment decisions are made at the endpoint level, which is a tuple of provider, model, and stock-keeping-unit. The authors aim to provide a more granular evaluation of AI systems at the endpoint level.

Method: The authors introduce TokenArena, a framework that measures inference at endpoint granularity across five core axes: output speed, time to first token, workload-blended price, effective context, and quality on the live endpoint. They also synthesize these metrics into three headline composites: joules per correct answer, dollars per correct answer, and endpoint fidelity.

Result: The authors evaluated 78 endpoints serving 12 model families and found significant variations in performance, including up to 12.5 points difference in mean accuracy, 12 points difference in fingerprint similarity, and an order of magnitude difference in tail latency. They also found that workload-aware blended pricing reorders the leaderboard substantially.

Conclusion: The authors conclude that TokenArena provides a more nuanced comparison of AI systems at the endpoint level, highlighting the importance of considering multiple factors when evaluating inference performance. They also emphasize that TokenArena is a methodology, not a single ranking, and welcome external replication.
...
### [116] [AgentFloor: How Far Up the tool use Ladder Can Small Open-Weight Models Go?](https://arxiv.org/abs/2605.00334)
*Ranit Karmakar,Jayita Chatterjee*

Main category: cs.AI

TL;DR: The paper introduces AgentFloor, a benchmark to evaluate the necessity of large frontier intelligence in agentic systems. It finds that small and mid-sized models are sufficient for routine, short-horizon tasks, while large frontier models are needed for long-horizon planning tasks.


<details>
  <summary>Details</summary>
Motivation: The motivation is to determine which parts of an agent workflow require large frontier intelligence and which can be handled by smaller models, given that production agentic systems make many short, structured, and routine model calls per user request.

Method: The authors introduce AgentFloor, a deterministic 30-task benchmark with a six-tier capability ladder, and evaluate 16 open-weight models and GPT-5 across 16,542 scored runs.

Result: The results show that small and mid-sized open-weight models are sufficient for much of the short-horizon, structured tool use work, while the strongest open-weight model matches GPT-5 on the benchmark but is cheaper and faster to run. The gap between models appears most clearly on long-horizon planning tasks.

Conclusion: The findings suggest a practical design principle for agentic systems: use smaller open-weight models for routine actions and reserve large frontier models for tasks that demand deeper planning and control.
...
### [117] [Physically Native World Models: A Hamiltonian Perspective on Generative World Modeling](https://arxiv.org/abs/2605.00412)
*Sen Cui,Jingheng Ma*

Main category: cs.AI

TL;DR: The paper proposes Hamiltonian World Models, a physically grounded perspective on world modeling that encodes observations into a structured latent phase space and evolves the latent state through Hamiltonian-inspired dynamics for planning.


<details>
  <summary>Details</summary>
Motivation: Current world model research struggles to provide physically reliable, action-controllable, and long-horizon stable predictions for embodied decision making, and the bottleneck is not just generating realistic futures but making them physically meaningful and useful for action.

Method: The proposed Hamiltonian World Models encode observations into a structured latent phase space, evolve the latent state through Hamiltonian-inspired dynamics with control, dissipation, and residual terms, and decode the predicted trajectory into future observations for planning.

Result: The paper argues that Hamiltonian structure may improve interpretability, data efficiency, and long-horizon stability, but notes practical challenges in real-world robotic scenes.

Conclusion: The paper proposes a new perspective on world modeling that emphasizes physical grounding and Hamiltonian structure, which could lead to more reliable and useful predictions for embodied decision making.
...
### [118] [AEM: Adaptive Entropy Modulation for Multi-Turn Agentic Reinforcement Learning](https://arxiv.org/abs/2605.00425)
...
TL;DR: AEM, a supervision-free credit assignment method, improves reinforcement learning for large language models by adaptively modulating entropy dynamics, achieving better exploration-exploitation trade-off and outperforming baselines.


<details>
  <summary>Details</summary>
Motivation: Effective training of large language model agents remains challenging due to sparse rewards, making it difficult to assign credit to individual steps in an agent's action trajectory.

Method: AEM, a supervision-free credit assignment method that adaptively modulates entropy dynamics during RL training to achieve a more effective exploration-exploitation trade-off.

Result: Extensive experiments demonstrate the effectiveness of AEM, including a notable 1.4 percent gain when integrated into a state-of-the-art baseline on the highly challenging SWE-bench-Verified benchmark.

Conclusion: AEM presents a practical and effective solution for improving reinforcement learning in large language models, eliminating the need for dense intermediate supervision and achieving better performance across various benchmarks and models.
...
### [119] [Thinking in Text and Images: Interleaved Vision--Language Reasoning Traces for Long-Horizon Robot Manipulation](https://arxiv.org/abs/2605.00438)
*Jinkun Liu,Haohan Chi,Lingfeng Zhang,Yifan Xie,YuAn Wang,Long Chen,Hangjun Ye,Xiaoshuai Hao,Wenbo Ding*

Main category: cs.AI

TL;DR: The paper introduces Interleaved Vision-Language Reasoning (IVLR), a policy framework that combines text and visual information to improve long-horizon robotic manipulation tasks.


<details>
  <summary>Details</summary>
Motivation: Existing Vision-Language-Action policies have limitations in planning and representing spatial constraints, and the authors aim to address this by developing a framework that integrates logical and geometric information.

Method: The authors propose IVLR, which uses an explicit intermediate representation called 	race{} that alternates textual subgoals with visual keyframes. A multimodal transformer generates this representation and conditions a closed-loop action decoder.

Result: The proposed method achieves 95.5% average success on LIBERO and 59.4% on SimplerEnv-WidowX. Ablations show that both modalities (text and vision) are necessary, and the full interleaved trace reaches 92.4% on LIBERO-Long.

Conclusion: The paper concludes that IVLR is an effective approach for long-horizon robotic manipulation tasks, and that the proposed framework can tolerate local corruption and moderate execution drift.
...
### [120] [On the Role of Artificial Intelligence in Human-Machine Symbiosis](https://arxiv.org/abs/2605.00440)
*Ching-Chun Chang,Yuchen Guo,Hanrui Wang,Timo Spinde,Isao Echizen*

Main category: cs.AI

TL;DR: This study proposes a methodology to infer and recover the functional role played by AI in natural language generation, and validates its effectiveness through experimentation.


<details>
  <summary>Details</summary>
Motivation: The increasing ambiguity between human and machine-generated information necessitates understanding AI's participation in content creation, particularly in terms of its role and potential impact on ethics.

Method: The proposed methodology involves inferring the latent role specified by the prompt, embedding this role into the content during generation, and recovering the nature of AI participation from the resulting text.

Result: The experimental results demonstrate the validity of the proposed methodology in discriminating between roles, robustness against perturbations, and preservation of linguistic quality.

Conclusion: This study contributes to future research on AI ethics, ensuring fair, transparent, and appropriate use of AI in natural language generation.
...
### [121] [Instance-Aware Parameter Configuration in Bilevel Late Acceptance Hill Climbing for the Electric Capacitated Vehicle Routing Problem](https://arxiv.org/abs/2605.00572)
*Yinghao Qin,Xinwei Wang,Mosab Bazargani,Jun Chen*

Main category: cs.AI

TL;DR: The paper proposes an instance-aware parameter configuration approach for the Electric Capacitated Vehicle Routing Problem, achieving a 0.28% average objective value reduction compared to a globally tuned configuration.


<details>
  <summary>Details</summary>
Motivation: The performance of algorithms in combinatorial optimization is highly sensitive to parameter settings, and a single globally tuned configuration often fails to exploit instance heterogeneity.

Method: The paper uses an offline tuning procedure to obtain instance-specific parameter labels, which are then mapped to instance features via a regression model for parameter prediction.

Result: The proposed approach achieves an average objective value reduction of 0.28% across eight held-out test instances relative to a globally tuned configuration.

Conclusion: The proposed instance-aware parameter configuration approach leads to significant cost reductions in multimillion-dollar transportation operations.
...
### [122] [Learn where to Click from Yourself: On-Policy Self-Distillation for GUI Grounding](https://arxiv.org/abs/2605.00642)
*Yan Zhang,Daiqing Wu,Huawen Shen,Yu Zhou,Can Ma*

Main category: cs.AI

TL;DR: The paper presents GUI-SD, an on-policy self-distillation framework for GUI grounding, which outperforms existing methods in accuracy and training efficiency.


<details>
  <summary>Details</summary>
Motivation: Recent reinforcement learning methods for GUI grounding rely on expensive multiple rollouts and suffer from sparse signals on hard samples, making on-policy self-distillation a promising alternative.

Method: The GUI-SD framework constructs a visually enriched privileged context for the teacher and employs entropy-guided distillation to adaptively weight tokens.

Result: GUI-SD consistently outperforms GRPO-based methods and naive OPSD in both accuracy and training efficiency on six representative GUI grounding benchmarks.

Conclusion: The proposed GUI-SD framework provides a more efficient and accurate approach to GUI grounding, leveraging on-policy self-distillation and entropy-guided distillation.
...
### [123] [To Call or Not to Call: A Framework to Assess and Optimize LLM Tool Calling](https://arxiv.org/abs/2605.00737)
*Qinyuan Wu,Soumi Das,Mahsa Amani,Arijit Nag,Seungeon Lee,Krishna P. Gummadi,Abhilasha Ravichander,Muhammad Bilal Zafar*

Main category: cs.AI

TL;DR: The paper introduces a framework to evaluate when LLMs should use external tools like web search, and finds that models often misjudge their need for these tools. It proposes estimators to improve tool-use decisions, leading to better task performance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the decision-making process of LLMs when to use external tools, as current approaches can be redundant or harmful.

Method: The method involves a framework inspired by decision-making theory, combining normative and descriptive perspectives to evaluate tool-use decisions based on necessity, utility, and affordability. Lightweight estimators of need and utility are trained based on models' hidden states.

Result: The result shows that models' perceived need and utility of tool calls are often misaligned with their true need and utility. The proposed estimators enable controllers that improve decision quality and lead to stronger task performance across three tasks and six models.

Conclusion: The conclusion is that the proposed framework and estimators can improve the decision-making process of LLMs when to use external tools, leading to better task performance.
...
### [124] [Position: agentic AI orchestration should be Bayes-consistent](https://arxiv.org/abs/2605.00742)
...
TL;DR: The paper argues that Bayesian principles should be applied to the control layer of agentic AI systems, which orchestrate LLMs and tools, to enable coherent decision-making under uncertainty.


<details>
  <summary>Details</summary>
Motivation: Many high-value deployments of LLMs rely on decisions under uncertainty, and Bayesian approaches can provide a framework for agentic systems to maintain beliefs, update them from interactions, and choose actions.

Method: The paper proposes applying Bayesian principles to the orchestration level of agentic AI systems, rather than the LLM agent parameters, and articulates practical properties for Bayesian control.

Result: The paper provides concrete examples and design patterns to illustrate how calibrated beliefs and utility-aware policies can improve agentic AI orchestration.

Conclusion: The paper concludes that Bayesian principles should be applied to the control layer of agentic AI systems to enable coherent decision-making under uncertainty, and provides a framework for practical implementation.
