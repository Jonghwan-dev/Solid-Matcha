...
TL;DR: The paper introduces PlantInquiryVQA, a benchmark for multi-step visual reasoning in botanical diagnosis, and evaluates multimodal large language models, finding they struggle with clinical reasoning and accurate diagnosis.


<details>
  <summary>Details</summary>
Motivation: Current vision-language models are evaluated on single-turn question answering, which is insufficient for complex tasks like botanical diagnosis that require multi-step, intent-driven visual reasoning.

Method: The authors formalize a Chain of Inquiry framework and release a dataset of 24,950 expert-curated plant images and 138,068 question-answer pairs, evaluating top-tier Multimodal Large Language Models.

Result: Multimodal large language models struggle with safe clinical reasoning and accurate diagnosis, but structured question-guided inquiry significantly improves diagnostic correctness, reduces hallucination, and increases reasoning efficiency.

Conclusion: The paper hopes PlantInquiryVQA serves as a foundational benchmark in advancing research to train diagnostic agents to reason like expert botanists rather than static classifiers.
...
TL;DR: This paper proposes a method for generating high-quality, scene-referred linear images from text prompts, preserving full dynamic range for professional post-processing.
...
Conclusion: The paper addresses the task of text-to-linear-image generation, providing a new approach for synthesizing scene-referred linear images that preserve full dynamic range, which can benefit professional post-processing and editing.
...
TL;DR: The paper proposes a dual-path network for recognizing micro-actions in videos, which are subtle movements that last 1-3 seconds and are important for social communication.


<details>
  <summary>Details</summary>
Motivation: Current computer vision systems poorly understand micro-actions, which are essential for fine-grained video understanding. Existing methods cannot accommodate the diverse spatio-temporal characteristics of micro-actions.

Method: The proposed method is a dual-path network with Spatial-Temporal (ST) and Temporal-Spatial (TS) pathways that process anatomically-grounded spatial entities. The network uses entity-level adaptive routing and Mutual Action Consistency (MAC) loss.
...
Conclusion: The paper concludes that architectural adaptation to the complexity of micro-actions is essential for advancing fine-grained video understanding.
...
Method: Adapt non-RGB inputs to RGB space, inject domain-specific information and Chain-of-Thought reasoning as instructions to standard RGB-only LMMs.

Result: Strong Zero-Shot performance gains on popular Remote Sensing benchmarks using the Gemini 2.5 model.

Conclusion: The proposed approach enables geospatial professionals to leverage powerful generalist models for specialized sensor inputs, benefiting from rich reasoning capabilities.
...
TL;DR: The paper proposes adapting Projected Gradient Unlearning (PGU) for text-to-image diffusion models to selectively remove undesirable concepts without costly retraining.


<details>
  <summary>Details</summary>
Motivation: Current machine unlearning methods for text-to-image diffusion models have a weakness where erased concepts return when the model is fine-tuned on downstream data.

Method: The authors adapt PGU from classification to diffusion models as a post-hoc hardening step, constructing a Core Gradient Space (CGS) and projecting gradient updates into its orthogonal complement.

Result: PGU eliminates the revival of erased concepts for style concepts and substantially delays it for object concepts, with a faster computation time compared to Meta-Unlearning.

Conclusion: The proposed approach is complementary to Meta-Unlearning, and its performance depends on how the concept is encoded, suggesting that retain concept selection should follow visual feature similarity rather than semantic grouping.
...
TL;DR: The paper proposes StyleVAR, a method for style transfer using Visual Autoregressive Modeling, which formulates style transfer as conditional discrete sequence modeling in a learned latent space.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve style transfer by maintaining semantic structure while transferring texture, and to address limitations of existing methods.

Method: StyleVAR uses a VQ-VAE to tokenize images into discrete codes, and a transformer to autoregressively model the distribution of target tokens conditioned on style and content tokens. It introduces a blended cross-attention mechanism and a scale-dependent blending coefficient to control the influence of style and content.

Result: StyleVAR outperforms an AdaIN baseline on multiple benchmarks and metrics, and the GRPO stage yields further gains. The method qualitatively transfers texture while maintaining semantic structure, especially for landscapes and architectural scenes.

Conclusion: The paper concludes that StyleVAR is an effective method for style transfer, but highlights the need for better content diversity and stronger structural priors to improve generalization.
...
TL;DR: The authors propose a contrastive fine-tuning framework for pediatric brain tumor diagnosis from whole-slide images, which improves fine-grained diagnostic distinctions under low-sample and class-imbalanced conditions.


<details>
  <summary>Details</summary>
Motivation: Accurate diagnosis of pediatric brain tumors presents unique challenges for deep learning due to data scarcity, class imbalance, and fine-grained morphologic overlap.

Method: The authors introduce an expert-guided contrastive fine-tuning framework that integrates contrastive learning into slide-level multiple instance learning (MIL) for weakly supervised pediatric brain tumor classification.

Result: The proposed approach yields measurable improvements in fine-grained diagnostic distinctions, with expert-guided hard negatives promoting more compact intra-class representations and improved inter-class separation.
...
TL;DR: Proposes a method to fine-tune diffusion models for inverse problems using a single observation, improving the trustworthiness of posterior image distributions.


<details>
  <summary>Details</summary>
Motivation: Current diffusion model fine-tuning methods require a large number of observations, which can be difficult to collect and lead to overfitting. The proposed method addresses this limitation.

Method: Combines existing diffusion priors into a product-of-experts prior and identifies exponents that maximize Bayesian evidence, allowing for posterior sampling from tempered and combined diffusion models.

Result: The method improves the trustworthiness of posterior image distributions in real-world inverse problems, such as black hole imaging and image deblurring with text-conditioned priors.

Conclusion: The proposed approach enables more flexible priors and improves the reliability of posterior samples in inverse problems, especially when only a single observation is available.
...
TL;DR: The paper introduces Foveated Reasoner, a vision-language framework that uses foveation to selectively focus on high-resolution images, reducing computational overhead while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the tension between benefiting from high-resolution images and incurring high compute overhead in vision-language models.

Method: The method involves a two-stage pipeline: coldstart supervision to bootstrap foveation behavior, followed by reinforcement learning to jointly improve evidence acquisition and task accuracy.

Result: The experiments show that the method learns effective foveation policies and achieves stronger accuracy under tight visual-token budgets across multiple vision-language benchmarks.

Conclusion: The Foveated Reasoner framework effectively balances visual-token count and compute overhead, achieving improved accuracy in vision-language tasks.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to develop an efficient and accurate method for large-scale building condition assessment nationwide in the United States.

Method: The method involves fine-tuning a large language model (Gemma 3 27B) on a human-labeled dataset, applying knowledge distillation to transfer capabilities to smaller models (Gemma 3 4B, EfficientNetV2-M, and SwinV2-B), and investigating LLMs' capabilities for assessing built environment and housing attributes.

Result: The framework achieves strong alignment with human mean opinion scores, outperforming individual raters, and delivers comparable performance with significant speed gains (3x to 30x) using smaller models.
...
TL;DR: The study examines how the geographic composition of pretraining data affects the performance of geospatial foundation models, finding that spectral diversity is strongly correlated with performance.


<details>
  <summary>Details</summary>
Motivation: To address the research gap on the role of pretraining datasets in geospatial foundation models, specifically how geographic composition affects downstream performance.
...
Result: The pretraining dataset from Europe outperformed global and continent-specific pretraining datasets, and spectral diversity was strongly correlated with performance.
...
TL;DR: The paper presents a novel approach for generating room impulse responses (RIRs) that disentangles the effects of spatial and material cues in a scene, allowing for more realistic and controllable acoustic modeling.


<details>
  <summary>Details</summary>
Motivation: Existing methods for acoustic modeling often entangle spatial and material influences, limiting user control and reducing realism. The authors aim to improve acoustic modeling for applications like virtual reality, robotics, and audio engineering.
...
Result: The model provides significant improvements over prior approaches on acoustic-based metrics (up to +16% on RTE) and material-based metrics (up to +70%). A human perceptual study also demonstrates the improved realism and material sensitivity of the model.

Conclusion: The proposed approach enables more realistic and controllable acoustic modeling by explicitly disentangling spatial and material cues, which can benefit various applications such as virtual reality, robotics, architectural design, and audio engineering.
...
TL;DR: A new hyperspectral foundation model called HyperFM is introduced, which efficiently processes large hyperspectral data from NASA's PACE mission, improving performance in atmospheric cloud property retrieval tasks.


<details>
  <summary>Details</summary>
Motivation: The NASA PACE mission generates large, complex hyperspectral data that is difficult to label and process. Existing foundation models struggle to interpret hyperspectral signatures, and recent hyperspectral models are limited by their size, computational cost, and reliance on cloud-free observations.

Method: HyperFM, a parameter-efficient hyperspectral foundation model, uses intra-group and inter-group spectral attention and hybrid parameter decomposition to capture spectral-spatial relationships while reducing computational cost.

Result: HyperFM shows consistent performance improvements over existing hyperspectral foundation models and task-specific state-of-the-art methods across four benchmark downstream atmospheric cloud property retrieval tasks.

Conclusion: HyperFM offers a scalable and efficient solution for processing hyperspectral data from the PACE mission, enabling better characterization of ocean color, aerosols, and clouds, and their interactions with Earth's climate and air quality.
...
TL;DR: Proposed Wavelet Flow Matching (WFM) for efficient multi-modal MRI synthesis, reducing computational cost and achieving comparable quality to diffusion models.


<details>
  <summary>Details</summary>
Motivation: Current diffusion models for multi-modal MRI synthesis are computationally expensive and inefficient, requiring hundreds of sampling steps and separate models per modality.
...
Result: WFM achieves 26.8 dB PSNR and 0.94 SSIM on BraTS 2024, comparable to diffusion baselines, but runs 250-1000x faster (0.16-0.64s vs. 160s per volume).

Conclusion: WFM enables real-time MRI synthesis with a single model, replacing four separate diffusion models, and makes it practical for clinical workflows.
...
TL;DR: The paper proposes a method to improve the spatial reasoning of embodied agents by addressing geometric hallucination in point-vision-language models through a framework called Geometric Reward Credit Assignment and a Reprojection-Consistency term.


<details>
  <summary>Details</summary>
Motivation: The motivation is to resolve the issue of geometric hallucination in point-vision-language models where predicted 3D structures contradict the observed 2D reality.

Method: The method proposed is Geometric Reward Credit Assignment, a framework that disentangles holistic supervision into field-specific signals and routes them exclusively to their responsible token spans, and a Reprojection-Consistency term to internalize physical constraints.

Result: The results show significant improvements in 3D KPA (from 0.64 to 0.93), 3D bounding box intersection over union (to 0.686), and reprojection consistency scores (to 0.852) while maintaining robust 2D localization performance.

Conclusion: The approach bridges the reliability gap in point-vision-language models by providing more accurate and physically verifiable spatial predictions.
...
<details>
  <summary>Details</summary>
Motivation: 3D Gaussian Splatting (3DGS) typically requires iterative optimization and multi-view images captured under consistent lighting with known camera parameters, which can be limiting.
...
Result: WildSplatter reconstructs 3D Gaussians from sparse input views in under one second and enables appearance control under diverse lighting conditions.

Conclusion: WildSplatter outperforms existing pose-free 3DGS methods on challenging real-world datasets with varying illumination.
...
TL;DR: A new framework called SpatiO is proposed for spatial reasoning, which uses a heterogeneous multi-agent approach to combine different vision-language specialists and adapt to various contexts.
...
Method: A heterogeneous multi-agent framework called SpatiO, which coordinates multiple vision-language specialists with complementary inductive biases, and a Test-Time Orchestration (TTO) mechanism that dynamically evaluates and reweights agents based on their reliability during inference.
...
Conclusion: The proposed SpatiO framework with TTO mechanism offers a promising approach for spatial reasoning tasks by enabling spatial adaptability and effectively combining diverse inductive biases.
...
TL;DR: A novel data augmentation framework is proposed to improve underwater object detection by generating realistic crowded fish scenarios, leading to better performance in complex scenes.


<details>
  <summary>Details</summary>
Motivation: Object detection models struggle in real-world underwater settings due to high variability and frequent occlusions, motivating the need for a robust data augmentation framework.
...
Result: The proposed method significantly outperforms a baseline YOLOv10 model, particularly on a challenging test set of manually annotated images collected from live-stream footage.

Conclusion: The proposed data augmentation framework is effective in improving detection performance in dense, real-world underwater environments.
...
<details>
  <summary>Details</summary>
Motivation: Empirical observation that attention in autoregressive diffusion rollouts concentrates on a subset of salient visual blocks, forming an implicit spatiotemporal memory.

Method: Proposes a trainable native sparsity mechanism and Persistent Block-Sparse Attention (PBSA) for efficient GPU kernel acceleration.

Result: Improves VBench score by +0.26, delivers 1.11-1.17x decoding speedup, and 42% lower peak KV-cache footprint on 5-second text-to-video generation.

Conclusion: Sparse Forcing enhances video generation quality and efficiency, particularly for longer-horizon rollouts.
...
<details>
  <summary>Details</summary>
Motivation: Facial action unit detection is challenging due to uncertainties from visual noise, subject-dependent appearance variations, and ambiguous inter-AU relationships, which degrade robustness.
...
TL;DR: A novel diffusion-based framework, LatRef-Diff, is proposed for precise facial attribute editing and style manipulation, addressing limitations in accuracy and training stability.


<details>
  <summary>Details</summary>
Motivation: Facial attribute editing and style manipulation are crucial for applications like virtual avatars and photo editing, but achieving precise control without altering unrelated features is challenging due to the complexity of facial structures and correlations between attributes.

Method: LatRef-Diff replaces traditional semantic directions with style codes, generated through latent and reference guidance, and integrates them into the target image via a style modulation module, incorporating learnable vectors, cross-attention mechanisms, and a hierarchical design.

Result: LatRef-Diff achieves state-of-the-art performance in qualitative and quantitative evaluations on CelebA-HQ, with ablation studies validating the effectiveness of the model's design choices.

Conclusion: The proposed LatRef-Diff framework effectively addresses the limitations in facial attribute editing and style manipulation, providing precise control and improved accuracy and image quality.
...
TL;DR: The paper presents ImageHD, an FPGA accelerator for on-device continual learning of visual data using hyperdimensional computing (HDC), achieving significant speedup and energy efficiency over CPU and GPU baselines.


<details>
  <summary>Details</summary>
Motivation: The need for efficient on-device continual learning (CL) for edge AI systems operating on non-stationary data streams, which is critical but challenging with existing methods that rely on backpropagation or exemplar-heavy classifiers.

Method: The authors introduce a hardware-aware CL method that uses a unified exemplar memory, a hardware-efficient cluster merging strategy, and a quantized CNN front-end, and implement it as a streaming dataflow architecture on an FPGA.

Result: ImageHD achieves up to 40.4x (4.84x) speedup and 383x (105.1x) energy efficiency over optimized CPU (GPU) baselines on the CORe50 dataset.

Conclusion: The paper demonstrates the practicality of HDC-enabled continual learning for real-time edge AI, offering a lightweight and efficient solution for on-device CL under strict latency and on-chip memory constraints.
...
Result: Experimental results on CelebA-HQ show that AttDiff-GAN achieves more accurate facial attribute editing and better preservation of non-target attributes than state-of-the-art methods in both qualitative and quantitative evaluations.

Conclusion: The proposed AttDiff-GAN framework effectively addresses the challenges of facial attribute editing by combining the strengths of GAN-based and diffusion-based methods, leading to improved performance and preservation of non-target attributes.
...
TL;DR: The paper introduces GraphLeap, a method to accelerate Vision Graph Neural Networks (ViGs) by decoupling graph construction from feature updates, and presents an end-to-end FPGA accelerator achieving significant speedup over CPU and GPU baselines.
...
Conclusion: The paper concludes that GraphLeap is a simple yet effective method to accelerate ViGs, and its FPGA accelerator design enables efficient on-chip dataflow for real-time Vision GNN inference.
...
TL;DR: This paper explores the use of synthetic data in controllable human video generation, proposing a diffusion-based framework to analyze the interaction between synthetic and real-world data.


<details>
  <summary>Details</summary>
Motivation: The scarcity of large-scale, diverse, and privacy-safe human video datasets limits controllable human video generation, and synthetic data provides a potential alternative.
...
Conclusion: The study provides practical insights for building data-efficient and generalizable generative models, and offers the first comprehensive exploration of synthetic data's role in human-centric video synthesis.
...
TL;DR: A deep learning framework using Vision Transformer (ViT-B/16) achieves high accuracy in classifying four types of brain tumors from MRI scans.


<details>
  <summary>Details</summary>
Motivation: Manual interpretation of MRI scans for brain tumor diagnosis is time-consuming, variable, and requires specialist expertise, highlighting the need for automated and accurate diagnosis methods.

Method: The proposed system uses a pretrained ViT-B/16 model with a clinically motivated preprocessing pipeline, including Contrast Limited Adaptive Histogram Equalization (CLAHE), a two-stage fine-tuning strategy, MixUp and CutMix augmentation, Exponential Moving Average (EMA) of weights, and Test-Time Augmentation (TTA).

Result: The model achieves a test accuracy of 99.29%, macro F1-score of 99.25%, and perfect recall on both healthy and meningioma classes, outperforming CNN-based baselines.

Conclusion: The proposed deep learning framework offers a highly accurate and clinically interpretable solution for automated four-class brain tumor classification from MRI scans, with potential to improve patient survival rates through early and accurate diagnosis.
...
TL;DR: The paper presents the NTIRE 2026 challenge on remote sensing infrared image super-resolution, aiming to recover high-resolution infrared images from low-resolution inputs.


<details>
  <summary>Details</summary>
Motivation: The challenge aims to advance research in infrared image super-resolution and promote effective solutions for real-world remote sensing applications.

Method: The challenge adopts a single-track setting, using a dataset generated through bicubic downsampling with a x4 scaling factor, and 13 teams submitted valid entries.

Result: The challenge had 13 teams submitting valid entries, and this report summarizes the challenge design, dataset, evaluation protocol, main results, and representative methods of each team.

Conclusion: The challenge serves as a benchmark to advance research in infrared image super-resolution and promote the development of effective solutions for real-world remote sensing applications.
...
TL;DR: Develops PLAS-Net for accurate quantification of beach litter area using UAV imagery, outperforming existing methods and providing valuable insights for coastal monitoring.


<details>
  <summary>Details</summary>
Motivation: Current automated UAV-based monitoring methods overestimate the planar area of irregular litter objects, limiting ecological risk assessment of marine debris.

Method: Develops PLAS-Net, an instance segmentation framework that extracts pixel-accurate physical footprints of coastal debris from UAV imagery.

Result: PLAS-Net achieves a mAP_50 of 58.7% on UAV imagery from a monsoon-driven pocket beach in Koh Tao, Thailand, outperforming eleven baseline models.

Conclusion: Pixel-level area extraction provides more valuable information for coastal monitoring compared to methods based solely on counting, enabling better ecological risk assessment and management of marine debris.
...
TL;DR: A new framework called FryNet is proposed for monitoring frying oil degradation in real-time, which uses a dual-stream RGB-thermal approach to achieve high accuracy in oil-region segmentation, serviceability classification, and regression of chemical oxidation indices.


<details>
  <summary>Details</summary>
Motivation: Current methods for monitoring frying oil degradation are destructive and provide no spatial information, making them unsuitable for real-time use. The authors aim to overcome the limitations of thermal-image-based inspection, specifically the camera-fingerprint shortcut, to develop a more accurate and reliable method.

Method: FryNet, a dual-stream RGB-thermal framework, which jointly performs oil-region segmentation, serviceability classification, and regression of four chemical oxidation indices. It uses a ThermalMiT-B2 backbone with channel and spatial attention, an RGB-MAE Encoder, Dual-Encoder DANN, and FiLM fusion.
...
Conclusion: The proposed FryNet framework offers a promising solution for real-time monitoring of frying oil degradation, providing accurate and reliable results, and has the potential to improve food safety.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for VI-ReID focus on image level or require costly identity annotations, and unsupervised video VI-ReID remains largely unexplored despite its practical importance.

Method: HiTPro uses a Temporal-aware Feature Encoder to extract and aggregate frame-level features into tracklet-level representations, then applies Intra-Camera Tracklet Prototyping and Hierarchical Cross-Prototype Alignment for unsupervised learning.

Result: HiTPro achieves state-of-the-art performance on HITSZ-VCM and BUPTCampus datasets under fully unsupervised settings, outperforming adapted baselines.
...
TL;DR: The paper proposes MiMIC, a new method for Universal Multimodal Retrieval (UMR) that addresses the limitations of early-fusion and late-fusion approaches.


<details>
  <summary>Details</summary>
Motivation: Existing UMR methods suffer from visual modality collapse and semantic misalignment, which limit their performance in multi-modal retrieval tasks.

Method: MiMIC introduces a fusion-in-decoder architecture and robust training through single modality mixin and random caption dropout.

Result: MiMIC consistently outperforms both early- and late-fusion baselines on the WebQA+ and EVQA+ datasets.

Conclusion: The proposed MiMIC method effectively addresses the challenges of visual modality collapse and semantic misalignment, leading to improved performance in UMR tasks.
...
TL;DR: The paper proposes TGR-MoE, a method that uses a pretrained dense teacher model to guide the learning of sparse Mixture of Experts (MoE) models, improving their accuracy, routing consistency, and training stability.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the optimization difficulties in sparse MoE training, specifically the gradient blocking issue that hinders the router's ability to learn appropriate expert-selection scores.

Method: The method proposed is Teacher-Guided Routing for Sparse Vision Mixture-of-Experts (TGR-MoE), which uses supervision derived from a pretrained dense teacher model to stabilize router learning.

Result: The results show that TGR-MoE consistently improves both accuracy and routing consistency on ImageNet-1K and CIFAR-100 datasets, while maintaining stable training even under highly sparse configurations.

Conclusion: The conclusion is that TGR-MoE is a simple yet effective method that can stabilize router learning and improve the performance of sparse MoE models.
...
TL;DR: The paper proposes a latent denoising framework to improve visual understanding and reasoning in Large Multimodal Models (LMMs) by providing effective visual supervision.
...
Result: The method consistently improves visual understanding and reasoning over strong baselines across a broad suite of standard multimodal benchmarks, and yields clear gains on compositional robustness benchmarks.

Conclusion: The proposed latent denoising framework provides an effective form of visual supervision for improving internal visual feature alignment and multimodal understanding in LMMs, leading to improved visual understanding and reasoning.
...
TL;DR: The study proposes a method to improve the robustness of self-supervised learning (SSL) in aerial imagery under corruptions like haze, motion blur, and occlusion.


<details>
  <summary>Details</summary>
Motivation: Existing SSL methods can introduce spurious structure into the latent space when dealing with severely degraded views of aerial images.

Method: The study introduces a per-sample, per-factor trust weight into the alignment objective, combined with the base contrastive loss as an additive residual, and applies a stop-gradient to the trust weight.

Result: The method achieves the highest mean linear-probe accuracy among six backbones on EuroSAT, AID, and NWPU-RESISC45, and yields significant improvements under severe information-erasing corruptions.

Conclusion: The study offers a concrete design principle for uncertainty-aware SSL, and demonstrates the effectiveness of the proposed method in improving SSL robustness to corruptions in aerial imagery.
...
TL;DR: A new framework called SparseGF is proposed for ground filtering in airborne laser scanning data, addressing limitations in current deep-learning-based methods, particularly in cross-scene generalization.
...
Method: SparseGF, a height-aware sparse segmentation framework with context compression, consisting of a context compression module, a hybrid sparse voxel-point network architecture, and a height-aware loss function.
...
TL;DR: The paper presents Prototype-Based Test-Time Adaptation (PTA), an efficient and effective method for vision-language models to adapt to test data, achieving state-of-the-art performance on multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing test-time adaptation methods have limitations, including increased inference latency and suboptimal performance due to cache population and retrieval. The authors aim to address these limitations.

Method: PTA uses class-specific knowledge prototypes to accumulate knowledge from test samples, adaptively weighting prototypes based on zero-shot class confidence and incorporating visual features.
...
Conclusion: PTA offers a more efficient and effective test-time adaptation paradigm, eliminating the overhead of cache population and retrieval, and achieving high efficiency while maintaining state-of-the-art performance.
...
Method: The authors propose KD-CVG, which consists of two primary modules: Semantic-Aware Retrieval (SAR) and Multimodal Knowledge Reference (MKR).
...
Conclusion: The authors develop a comprehensive Advertising Creative Knowledge Base (ACKB) and propose KD-CVG to improve CVG, effectively addressing the challenges in T2V models.
...
<details>
  <summary>Details</summary>
Motivation: Fine-grained edge features on 3D point clouds are difficult to detect effectively due to their dense distribution or small-scale surface gradients.

Method: The proposed EdgeFormer network consists of two stages: constructing local patch feature descriptors and classifying each point based on these descriptors.

Result: The experimental results show that the proposed model demonstrates competitive performance compared to six baselines.

Conclusion: The proposed EdgeFormer network effectively extracts finer details for edge detection on 3D point clouds.
...
TL;DR: A new dataset called Visual Grounding Chain-of-Thought (VG-CoT) is proposed to improve the trustworthiness of Large Vision-Language Models (LVLMs) by linking reasoning steps to actual visual evidence in images.


<details>
  <summary>Details</summary>
Motivation: Existing datasets for LVLMs have limitations in scalability due to manual annotation and lack of alignment between multi-step reasoning and image regions, which affects model trustworthiness.

Method: A fully automated three-stage pipeline is proposed to construct the VG-CoT dataset, involving visual evidence extraction, step-by-step grounded reasoning generation with GPT-4o, and refinement through rationale-driven open-set detection.

Result: Experiments with LVLMs like LLaVA-1.5 and Qwen2-VL show consistent improvements on evaluation metrics, confirming that VG-CoT enhances trustworthy, evidence-based reasoning while maintaining scalable and cost-efficient dataset construction.

Conclusion: The proposed VG-CoT dataset and benchmark effectively improve the trustworthiness of LVLMs by providing a more accurate and scalable evaluation of model reasoning.
...
TL;DR: A new framework called YOGO is proposed to improve 3D Gaussian Splatting (3DGS) for production-level deployment, addressing issues such as unpredictable resource consumption and data pollution.


<details>
  <summary>Details</summary>
Motivation: The 'Industry-Academia Gap' in 3DGS, which includes unpredictable resource consumption, the 'sparsity shield' of current benchmarks, and severe multi-sensor data pollution, hinders real-world applications.

Method: YOGO, a system-level framework that reformulates the stochastic growth process into a deterministic, budget-aware equilibrium, integrating a budget controller and an availability-registration protocol.
...
Conclusion: The proposed YOGO framework and the Immersion v1.0 dataset help bridge the 'Industry-Academia Gap' in 3DGS, enabling production-level deployment with improved reconstruction fidelity.
...
TL;DR: S1-VL, a multimodal reasoning model, is proposed for scientific domains, combining structured chain-of-thought and Thinking-with-Images paradigms, achieving state-of-the-art performance on 13 benchmarks.


<details>
  <summary>Details</summary>
Motivation: The model aims to effectively handle challenging scientific scenarios such as high-resolution chart interpretation, microscopic image understanding, and geometry-assisted reasoning.

Method: S1-VL uses a four-stage progressive pipeline for training, including scientific multimodal SFT, Thinking-with-Images cold-start SFT, and two stages of reinforcement learning with SAPO.

Result: S1-VL-32B achieves state-of-the-art performance on all five Thinking-with-Images benchmarks and outperforms compared systems on scientific reasoning benchmarks.

Conclusion: The proposed S1-VL model demonstrates effectiveness in multimodal reasoning for scientific domains, particularly in challenging scenarios.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to improve image segmentation techniques by using nonlinear diffusion filters to obtain piecewise constant images as a preprocessing step.

Method: The method involves proposing a new family of diffusivities based on nonlinear diffusion techniques and backward diffusion, and proving that they satisfy well-posedness requirements. The filters are implemented using semi-implicit schemes.

Result: The result is the ability to obtain piecewise constant images with low computational effort, using a stopping criterion for the diffusion time.

Conclusion: The conclusion is that the proposed nonlinear diffusion filters can effectively produce piecewise constant images with clear contours and no blurred edges, making them suitable for image segmentation techniques.
...
TL;DR: Proposed UHR-DETR, an efficient end-to-end transformer-based detector for ultra-high-resolution imagery, addressing memory bottlenecks while preserving object details.


<details>
  <summary>Details</summary>
Motivation: Detecting small objects in vast ultra-high-resolution scenes faces a dilemma between retaining resolution and managing memory, with conventional methods compromising on object details or context.

Method: Introduced Coverage-Maximizing Sparse Encoder for dynamic resource allocation and Global-Local Decoupled Decoder for integrating scene awareness and object details.

Result: UHR-DETR achieved a 2.8% mAP improvement and a 10x inference speedup compared to sliding-window baselines on the STAR dataset, under strict hardware constraints.

Conclusion: UHR-DETR demonstrates superiority in detecting small objects in ultra-high-resolution imagery with efficient computation and improved accuracy.
...
Result: The proposed algorithm outperforms Kd-tree and Octree in terms of speed, with kNN search time consumption 51.111% and 94.159% lower, and RN search time 54.519% and 41.840% lower, respectively.
...
TL;DR: The authors propose VARestorer, a distillation framework that adapts a pre-trained visual autoregressive model for one-step image super-resolution, achieving state-of-the-art performance and 10x faster inference.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the challenges of adapting visual autoregressive models (VAR) for image super-resolution (ISR), specifically the limitations of the next-scale prediction mechanism and error accumulation in iterative prediction.

Method: The authors propose a distillation framework called VARestorer, which leverages distribution matching, pyramid image conditioning with cross-scale attention, and parameter-efficient adapters to transform a pre-trained text-to-image VAR model into a one-step ISR model.

Result: VARestorer achieves state-of-the-art performance with 72.32 MUSIQ and 0.7669 CLIPIQA on the DIV2K dataset, while accelerating inference by 10 times compared to conventional VAR inference.

Conclusion: The proposed VARestorer framework effectively adapts a pre-trained VAR model for one-step ISR, achieving improved performance and efficiency.
...
TL;DR: The authors propose OA-VAT, a unified pipeline for Visual Active Tracking (VAT) that addresses bottlenecks in real-world deployment, achieving state-of-the-art performance and real-time processing.


<details>
  <summary>Details</summary>
Motivation: VAT faces challenges from visually similar distractors and occlusions, hindering its deployment in applications like drone navigation and security surveillance.
...
Result: OA-VAT achieves 0.93 average SR on UnrealCV, 90.8% average CAR on real-world datasets, and 81.6% TSR on a DJI Tello drone, with real-time performance at 35 FPS.

Conclusion: OA-VAT provides a robust and real-time solution for VAT, addressing key bottlenecks and outperforming state-of-the-art methods in various benchmarks.
...
TL;DR: The paper introduces EgoPoint-Bench, a benchmark for evaluating and enhancing multimodal pointing reasoning in egocentric views, addressing the 'Referential Hallucination' issue in current AI systems.


<details>
  <summary>Details</summary>
Motivation: Current AI systems, despite advancements in Multimodal Large Language Models (MLLMs), fail to precisely ground the spatial semantics of pointing in natural language commands, relying on spurious correlations instead.

Method: The authors introduce EgoPoint-Bench, a comprehensive question-answering benchmark with over 11k samples, spanning five evaluation dimensions and three levels of referential complexity, and fine-tune models on synthetic data.
...
TL;DR: Proposed ID-Eraser, a proactive defense against face swapping deepfakes by removing identifiable facial info.


<details>
  <summary>Details</summary>
Motivation: Existing defenses against face swapping are ineffective, and new methods are needed to protect privacy and digital security.

Method: ID-Eraser injects perturbations into identity embeddings and uses a Face Revive Generator (FRG) to produce protected images.

Result: ID-Eraser substantially disrupts identity recognition across face recognition and swapping systems, achieving low accuracy and high image quality.

Conclusion: ID-Eraser provides effective protection against face swapping deepfakes, demonstrating strong generalization and practical effectiveness.
...
TL;DR: The paper proposes a new evaluation metric, Cross-AUC, and a novel framework, SFAM, to improve face forgery detection by enhancing cross-domain score comparability and achieving superior performance.


<details>
  <summary>Details</summary>
Motivation: Existing face forgery detectors have poor generalization ability across datasets due to the lack of suitable metrics, specifically the commonly used cross-dataset AUC metric.
...
Result: The proposed method achieves superior performance compared with state-of-the-art methods on public datasets using various suitable metrics.

Conclusion: The paper highlights the importance of evaluating cross-domain score comparability in face forgery detection and proposes a new metric and framework to address this issue, achieving improved performance.
...
TL;DR: This paper introduces a framework to evaluate the capability of Large Language Models (LLMs) in understanding dynamic traffic agents and road network topology for autonomous driving.


<details>
  <summary>Details</summary>
Motivation: The motivation is to thoroughly understand the application of LLMs on AD perception and prediction, specifically their ability to understand dynamic traffic agents and static road infrastructure.

Method: The framework uses frozen LLMs as the reasoning engine, with a traffic encoder for scene features, a lightweight CNN for HD maps, and a reprogramming adapter for LLM-compatible tokens.

Result: The framework allows for quantitative analysis of multi-modal information's influence on trajectory prediction accuracy and demonstrates strong generalizability across diverse LLM architectures.

Conclusion: The study provides a unified platform for model evaluation, enabling the seamless integration of frozen LLMs with minimal adaptation.
...
TL;DR: The paper proposes VFM$^{4}$SDG, a dual-prior learning framework for single-domain generalized object detection (SDGOD) to improve detector stability across domains.


<details>
  <summary>Details</summary>
Motivation: Existing SDGOD methods focus on data augmentation or domain-invariant representation learning, neglecting detector mechanisms, leading to performance degradation in unseen environments.

Method: The proposed method introduces a frozen vision foundation model (VFM) as a transferable prior and uses Cross-domain Stable Relational Prior Distillation and Semantic-Contextual Prior-based Query Enhancement to enhance detector stability.

Result: The proposed method consistently outperforms existing SOTA methods on standard SDGOD benchmarks and two mainstream DETR-based detectors.

Conclusion: The proposed VFM$^{4}$SDG framework effectively improves the stability and performance of object detectors across different domains.
...
TL;DR: A novel local descriptor, Gaussian Mixture Descriptor (GMD), is proposed for matching fractured surfaces of fragments in automatic object reassembly.


<details>
  <summary>Details</summary>
Motivation: The matching of fractured surfaces is a crucial step in reconstructing objects from fragments acquired using laser scanners.

Method: The method involves dividing a local surface patch into concave and convex regions, estimating the k value of GMM, and merging regional GMDs to form the final GMD. Similarities between GMDs are measured using L2 distance, and fragments are aligned using RANSAC and ICP.

Result: Extensive experiments on real-scanned public datasets and Terracotta datasets demonstrate the effectiveness of the approach, and comparisons with existing methods validate its advantage.

Conclusion: The proposed Gaussian Mixture Descriptor (GMD) is effective for matching fractured surfaces of fragments in automatic object reassembly.
...
TL;DR: Evaluator VLMs are unreliable in detecting errors in image-to-text and text-to-image tasks, often failing to detect perturbed outputs, especially with fine-grained compositional and spatial errors.


<details>
  <summary>Details</summary>
Motivation: The growing reliance on Large Vision-Language Models (VLMs) as evaluators for other models' outputs raises concerns about their reliability, which has not been thoroughly explored.

Method: The authors introduce targeted perturbations to degrade output quality and evaluate 4 prominent VLMs using single-answer scoring, pairwise comparison, and reference-guided paradigms on a comprehensive benchmark of over 4000 perturbed instances.

Result: Current VLM evaluators exhibit substantial blind spots, often failing to detect perturbed outputs (exceeding 50% in some cases), struggling with fine-grained compositional and spatial errors, and being insensitive to hallucinated content that contradicts the input image.

Conclusion: The unreliable nature of current Evaluator VLMs urges caution in their deployment for benchmarking and development decisions, highlighting the need for more reliable evaluation methods.
...
TL;DR: The study proposes an attention-based multiple instance learning framework to predict the predominant lung adenocarcinoma growth pattern at the whole slide level, reducing annotation burden.


<details>
  <summary>Details</summary>
Motivation: Lung adenocarcinoma grading depends on accurately identifying growth patterns, but common deep learning approaches require extensive annotations.

Method: The study uses an attention-based multiple instance learning framework with pretrained pathology foundation models as patch encoders, either frozen or fine-tuned on annotated patches.

Result: The approach achieves high agreement (κ = 0.699) and yields more robust predictions compared to simple patch-aggregation baselines.

Conclusion: The proposed framework shows promise in predicting the predominant lung adenocarcinoma growth pattern at the whole slide level, with potential for future extension to estimate the full distribution of growth patterns.
...
TL;DR: A new framework for detecting out-of-distribution (OOD) data that decomposes inputs into functional components and uses scores to detect local shifts and compositional inconsistencies.


<details>
  <summary>Details</summary>
Motivation: Current OOD detection methods have limitations, including global representations suppressing local OOD cues and patch-based methods being unstable due to spurious correlations and noise.

Method: A Component-Based OOD Detection (CoOD) framework that uses Component Shift Score (CSS) and Compositional Consistency Score (CCS) to detect OOD data.
...
Method: The method proposed involves learning in deep kernel space to approximate the underlying frame distribution using MMD as a closeness metric, and NTKs to define the kernel space.

Result: The results show that the method achieves competitive performance compared to state-of-the-art per-video methods on six standard benchmarks, and has higher F1 scores than prior agglomerative work when the number of segments is unknown.
...
TL;DR: A new framework called CHRep is proposed to predict spatial gene expression from routine H&E slides, improving robustness and accuracy across different cohorts and datasets.


<details>
  <summary>Details</summary>
Motivation: Spatial transcriptomics is expensive and low-throughput, and existing models for predicting spatial gene expression from H&E slides suffer from slide-level appearance shifts and regression-driven over-smoothing.

Method: CHRep is a two-phase framework that learns a structure-aware representation by jointly optimizing correlation-aware regression, symmetric image-expression alignment, and coordinate-induced spatial topology regularization, and uses a lightweight calibration module for cross-slide robustness.

Result: CHRep consistently improves gene-wise correlation under leave-one-slide-out evaluation across three cohorts, with significant gains in Pearson correlation coefficient and reductions in mean squared error and mean absolute error compared to existing methods.

Conclusion: CHRep provides a robust and accurate method for predicting spatial gene expression from H&E slides, enabling large-cohort studies and routine clinical use.
...
TL;DR: Sculpt4D is a 4D generative framework that integrates efficient temporal modeling into a pretrained 3D Diffusion Transformer, achieving high-fidelity dynamic 4D generation.


<details>
  <summary>Details</summary>
Motivation: The scarcity of 4D training data and the challenges of high-fidelity dynamic 4D generation, hindered by temporal artifacts and prohibitive computational demand.

Method: Sculpt4D uses a Block Sparse Attention mechanism that preserves object identity and captures rich motion dynamics, reducing network total computation by 56%.

Result: Sculpt4D establishes a new state-of-the-art in temporally coherent 4D synthesis.

Conclusion: Sculpt4D charts a path toward efficient and scalable 4D generation, mitigating the challenges of high-fidelity dynamic 4D generation.
...
<details>
  <summary>Details</summary>
Motivation: Input variations from measurement noise or data drift can cause unpredictable shifts in 2D layouts, and the stability of parametric projections remains largely unexamined.

Method: The framework uses Gaussian perturbations around anchor points, combining quantitative measures and visualizations to assess neighborhood deformation in 2D embeddings.

Result: The framework identifies unstable projection regions invisible to traditional metrics, and demonstrates effectiveness on UMAP- and t-SNE-based neural projectors with MNIST and Fashion-MNIST datasets.

Conclusion: The proposed framework provides a detailed inspection of parametric projection stability, revealing unstable regions and showcasing the impact of Jacobian regularization on robustness.
...
TL;DR: DCMorph, a dual-stream diffusion-based face morphing framework, is proposed to improve face morphing attacks on identity verification systems.


<details>
  <summary>Details</summary>
Motivation: To anticipate evolving threats and develop robust defensive mechanisms for identity verification systems by advancing face morphing attack techniques.
...
Conclusion: DCMorph presents a more effective and challenging face morphing attack method, highlighting the need for improved defensive mechanisms in identity verification systems.
...
TL;DR: DualSplat, a framework that improves 3D Gaussian Splatting's performance on scenes with transient objects by using reconstruction failures to create explicit priors for better scene reconstruction.


<details>
  <summary>Details</summary>
Motivation: The performance of 3D Gaussian Splatting degrades with transient objects violating multi-view consistency, and existing methods have a circular dependency between transient detection and scene reconstruction.

Method: DualSplat uses a Failure-to-Prior framework with two stages: first-pass reconstruction to identify failures, and a second stage that constructs pseudo-masks from these failures to guide a clean 3DGS optimization.

Result: DualSplat outperforms existing baselines, particularly in transient-heavy scenes and regions, as shown by experiments on RobustNeRF and NeRF On-the-go.

Conclusion: DualSplat effectively addresses the challenge of reconstructing scenes with transient objects, leading to improved performance and accuracy in 3D Gaussian Splatting.
...
TL;DR: A novel full-reference image quality assessment (FR-IQA) paradigm based on causal inference and decoupled representation learning is proposed, achieving competitive performance across various settings and domains.


<details>
  <summary>Details</summary>
Motivation: Existing FR-IQA models rely on pairwise comparisons of deep features, and the authors aim to approach this problem from a different perspective, leveraging causal inference and decoupled representation learning.

Method: The proposed method involves decoupled representation learning to extract degradation and content representations, a masking module to model the causal relationship between image content and degradation features, and quality score prediction using supervised regression or label-free dimensionality reduction.

Result: The proposed method achieves highly competitive performance on standard IQA benchmarks across fully supervised, few-label, and label-free settings, and exhibits superior cross-domain generalization compared to existing training-free FR-IQA models.

Conclusion: The proposed FR-IQA paradigm based on causal inference and decoupled representation learning demonstrates superior performance and cross-domain generalization, making it a promising approach for image quality assessment.
...
Method: The method involves a unified pretraining objective that combines masked image reconstruction with self-distilled contrastive objectives. The model is pretrained on a curated dataset of 1 billion high-quality human images and incorporates advances from frontier models for improved stability and longer training schedules.

Result: Sapiens2 sets a new state-of-the-art and improves over its predecessor in various tasks, including pose estimation (+4 mAP), body-part segmentation (+24.3 mIoU), and normal estimation (45.6% lower angular error).

Conclusion: The conclusion is that Sapiens2 is a significant improvement over its predecessor, offering better performance and versatility in human-centric vision tasks, and has the potential to be applied to a wide range of downstream tasks.
...
TL;DR: A new benchmark called WorldMark is introduced to enable fair comparison of interactive video generation models by providing standardized test conditions.


<details>
  <summary>Details</summary>
Motivation: The lack of a common benchmark for interactive video generation models makes fair cross-model comparison impossible, hindering progress in the field.
...
Conclusion: WorldMark is a comprehensive benchmark that standardizes the evaluation of interactive video generation models, facilitating progress and comparison in the field, and is complemented by an online platform called World Model Arena for side-by-side model battles.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of deep neural networks in video copy detection, which require robust similarity estimation at large scale, and to overcome the computational cost and large descriptor size of existing methods.

Method: The method proposed is a video copy detection framework based on differentiable Logic Gate Networks (LGNs), which uses compact, logic-based representations, aggressive frame miniaturization, binary preprocessing, and a trainable LGN embedding model.

Result: The results show that LGN-based models achieve competitive or superior accuracy and ranking performance compared to prior models, with descriptors several orders of magnitude smaller and inference speeds exceeding 11k samples per second.
...
TL;DR: A brain-inspired framework is proposed for 3D human mesh recovery from monocular RGB images, integrating vision transformers and conditional diffusion models to improve performance under occlusions.


<details>
  <summary>Details</summary>
Motivation: The goal is to address the challenges of 3D human mesh recovery under partial or severe occlusions, where existing regression-based methods may produce inaccurate results and diffusion-based methods may weaken fidelity to rare poses.

Method: The proposed framework combines a ViT-based pathway for extracting deterministic visual cues from visible regions and a diffusion-based pathway for synthesizing structurally coherent human body representations, with a diverse-consistent feature learning module and a cross-attention multi-level fusion mechanism.

Result: The method achieves superior performance on key metrics and shows strong robustness in complex real-world scenarios, as demonstrated by experiments on standard benchmarks.

Conclusion: The proposed brain-inspired synergistic framework effectively addresses the limitations of existing methods for 3D human mesh recovery, providing a more robust and accurate approach for estimating anatomically plausible 3D human models under occlusions.
...
TL;DR: The paper investigates factors affecting performance in feed-forward visual geometry estimation and proposes enhancements to integrate advantages of optimization-based methods and high-resolution inputs.


<details>
  <summary>Details</summary>
Motivation: The gap between multi-frame models' cross-frame consistency and per-frame methods' single-frame accuracy in feed-forward visual geometry estimation.

Method: Rigorous ablation studies, introduction of a consistency loss function, and an efficient architectural design to leverage high-resolution information.
...
Conclusion: The paper provides insights into critical factors driving model performance in feed-forward visual geometry estimation and proposes effective enhancements to improve performance.
...
TL;DR: The paper introduces a framework for precise video captioning using human-AI oversight, achieving improved performance on open-source models and surpassing closed-source models.


<details>
  <summary>Details</summary>
Motivation: The authors aim to improve video-language models (VLMs) by enabling precise video captioning through a structured specification and human-AI oversight.

Method: The authors define a structured specification for describing visual elements, introduce CHAI (Critique-based Human-AI Oversight) for curating high-quality captions, and use SFT, DPO, and inference-time scaling to improve open-source models.

Result: The resulting model outperforms closed-source models, and fine-tuned video generation models achieve finer control over cinematography.
...
TL;DR: Ramen, a framework for robust test-time adaptation through active sample selection, is proposed to improve the performance of pretrained vision-language models under mixed-domain shifts.
...
Method: Ramen uses active sample selection based on domain consistency and prediction balance to retrieve relevant samples from previously seen data, and employs an embedding-gradient cache for efficient adaptation.
...
Conclusion: Ramen provides a practical solution for handling distribution shifts in pretrained vision-language models, and its effectiveness is demonstrated through theoretical analysis and experiments.
...
TL;DR: The paper proposes DOCO, a framework for Open-set Continual Test-Time Adaptation (OCTTA) that adapts to changing domains and detects unknown semantic classes.


<details>
  <summary>Details</summary>
Motivation: Existing Test-Time Adaptation (TTA) methods fail in scenarios with continual domain shifts and emergence of unknown semantic classes.

Method: DOCO uses dynamic sample splitting, domain compensation prompts, and structural preservation regularizers to adapt to domain shifts and detect OOD samples.

Result: DOCO outperforms prior methods on multiple benchmarks, establishing a new state-of-the-art for OCTTA.

Conclusion: The proposed DOCO framework effectively addresses the challenging OCTTA setting, enabling robust domain adaptation and OOD detection.
...
Conclusion: The proposed framework overcomes the limitation of scarce multi-view data, enabling precise camera control and high-quality video reshooting.
...
TL;DR: The paper analyzes the use of computer vision methods for social media discourse analysis, specifically for climate communication, and benchmarks various models for their performance.


<details>
  <summary>Details</summary>
Motivation: To facilitate research on climate communication on social media by analyzing the application of computer vision methods for discourse analysis.

Method: The paper benchmarks six promptable vision-language models and 15 zero-shot CLIP-like models on two datasets from Twitter, using application-based taxonomy design, model selection, prompt engineering, and validation.

Result: The Gemini-3.1-flash-lite model outperforms others across all super-categories and both datasets, and VLM predictions can reliably recover population-level trends even with moderate per-image accuracy.

Conclusion: The study demonstrates the viability of using computer vision methods for large-scale discourse analysis, and provides insights into the performance of various models and techniques.
...
TL;DR: The paper introduces SyMTRS, a large-scale synthetic dataset for remote sensing tasks, providing high-quality annotations for monocular depth estimation, domain adaptation, and super-resolution.
...
Result: The dataset enables controlled experiments with perfect geometric ground truth and consistent multi-domain supervision, bridging critical gaps in remote sensing research.

Conclusion: SyMTRS aims to facilitate joint research in geometric understanding, cross-domain robustness, and resolution enhancement, providing a valuable resource for remote sensing tasks.
...
TL;DR: A new framework, TEMA, is proposed for Composed Image Retrieval (CIR) that handles multi-modification queries, improving retrieval accuracy and efficiency.
...
Method: The Text-oriented Entity Mapping Architecture (TEMA) is introduced, which accommodates both simple and multi-modification queries, and two new datasets, M-FashionIQ and M-CIRR, are constructed for evaluation.

Result: TEMA demonstrates superior performance on four benchmark datasets, achieving a balance between retrieval accuracy and computational efficiency.

Conclusion: The proposed TEMA framework and datasets advance the CIR field, enabling more practical and effective image retrieval using multimodal queries.
...
TL;DR: The paper addresses ambiguities in super-resolution problems by using low-resolution images at different scales, demonstrating that combining images with coprime pixel sizes leads to efficient and stable super-resolution reconstruction.


<details>
  <summary>Details</summary>
Motivation: The super-resolution problem under translation has inherent ambiguities, which can be resolved by incorporating low-resolution images at different scales.

Method: The authors use combinations of low-resolution images with different pixel sizes, specifically pairwise coprime pixel sizes, and apply Fourier domain techniques or iterative least squares methods for super-resolution reconstruction.

Result: The proposed approach leads to a stable inverse and efficient super-resolution reconstruction, validated through one- and two-dimensional experiments, demonstrating the advantages of multiscale super-resolution.

Conclusion: The paper provides a mathematical analysis of the noise-resolution tradeoff and demonstrates the implications of the results for common imaging systems, showcasing the potential of multiscale super-resolution for improving imaging capabilities.
...
TL;DR: A new task, diagnosis-driven CE video summarization, is introduced to extract key evidence frames from capsule endoscopy videos for accurate diagnoses. A dataset and framework, DiCE, are proposed to address this task.


<details>
  <summary>Details</summary>
Motivation: Current capsule endoscopy research is limited to frame-level classification and detection, leaving video-level analysis underexplored. The goal is to bridge this gap by introducing a new task that requires extracting key evidence frames for clinically meaningful findings and accurate diagnoses.

Method: The proposed framework, DiCE, mirrors the standard CE reading workflow and consists of three stages: efficient candidate screening, Context Weaver to organize candidates into coherent diagnostic contexts, and Evidence Converger to aggregate multi-frame evidence into robust clip-level judgments.
...
Conclusion: Diagnosis-driven contextual reasoning is a promising paradigm for ultra-long CE video summarization, and the proposed framework and dataset facilitate research in this direction.
...
### [78] [When Prompts Override Vision: Prompt-Induced Hallucinations in LVLMs](https://arxiv.org/abs/2604.21911)
*Pegah Khayatan,Jayneel Parekh,Arnaud Dapogny,Mustafa Shukor,Alasdair Newson,Matthieu Cord*

Main category: cs.CV

TL;DR: The paper proposes a benchmark to understand hallucinations in large vision-language models and a fine-tuning framework to mitigate hallucinations induced by textual instructions.


<details>
  <summary>Details</summary>
Motivation: Despite progress in large vision-language models, they remain vulnerable to hallucinations, and the causes of hallucinations are not well understood.

Method: The paper proposes HalluScope, a benchmark to analyze hallucinations, and HalluVL-DPO, a fine-tuning framework that uses preference optimization to guide the model towards more visually grounded responses.
...
Conclusion: The paper concludes that hallucinations in large vision-language models largely stem from excessive reliance on textual priors and background knowledge, and proposes a solution to mitigate this issue.
...
TL;DR: The paper introduces a new benchmark for physical video understanding that tests a model's ability to localize events in time and space, not just identify them.


<details>
  <summary>Details</summary>
Motivation: Current video understanding models can answer questions about events but fail to localize them in time or space, highlighting the need for a more comprehensive evaluation.

Method: The authors introduce a grounded benchmark that extends the what-when-where evaluation structure to four video sources, six physics domains, and various prompt families and input conditions.

Result: The results show that physics remains the strongest regime, while spatial grounding is the weakest; prompt-family robustness is selective, and perturbation gains are seen in weak original cases.

Conclusion: The authors suggest that video Q&A reasoning benchmarks should report physically grounded, prompt-aware, and perturbation-aware diagnostics alongside aggregate accuracy.
...
TL;DR: This paper proposes a method to recover the original, unaltered version of a camera image by removing hallucinated content added by deep-learning modules in the camera's image signal processor (ISP).


<details>
  <summary>Details</summary>
Motivation: The increasing use of deep-learning modules in cameras' ISPs can add hallucinated content to images, potentially altering their semantics and interpretation, and making it difficult for users to distinguish between authentic and altered images.

Method: The approach involves optimizing an image-specific multi-layer perceptron (MLP) decoder and a modality-specific encoder to recover the original image. The encoder and MLP are self-contained, require only 180 KB of storage, and can be applied post-capture without access to the camera ISP.

Result: The proposed method allows users to recover the 'unhallucinated' version of the camera image, enabling them to avoid misinterpreting the image content.

Conclusion: The paper presents a solution to address the issue of hallucinated content in camera images, providing users with a way to recover the original image and ensure its authenticity.
...
Method: A unified generative-discriminative framework, UniGenDet, with a symbiotic multimodal self-attention mechanism, unified fine-tuning algorithm, and detector-informed generative alignment mechanism.

Result: State-of-the-art performance on multiple datasets.

Conclusion: The proposed UniGenDet framework successfully bridges the task gap between image generation and detection, demonstrating improved interpretability and authenticity.
...
TL;DR: The paper analyzes the differences in mistakes made by humans and vision models in image classification, finding distinct inductive biases that affect generalization.


<details>
  <summary>Details</summary>
Motivation: To understand the differences in classification mistakes between humans and vision models, and how these differences relate to inductive biases and generalization.

Method: The study uses a natural-image categorization task with 12 perturbation types, analyzing matched human and deep vision model responses, and quantifying asymmetry in confusion matrices through a Rate-Distortion (RD) framework.

Result: The results show that humans exhibit broad but weak asymmetries, while deep vision models show sparser, stronger directional collapses, and that robustness training fails to recover the human-like breadth-strength profile of graded similarity.
...
TL;DR: Vista4D: a video reshooting framework that uses a 4D point cloud to re-synthesize scenes from different camera angles and trajectories.
...
Method: The method uses a 4D-grounded point cloud representation with static pixel segmentation and 4D reconstruction to preserve content and provide camera signals. It is trained on reconstructed multiview dynamic data for robustness.

Result: The results show improved 4D consistency, camera control, and visual quality compared to state-of-the-art baselines. The method generalizes to real-world applications such as dynamic scene expansion and 4D scene recomposition.

Conclusion: Vista4D is a robust and flexible video reshooting framework that achieves improved performance and generalizability in various applications.
...
TL;DR: Omni is a unified multimodal model that achieves strong performance on multimodal generation and understanding benchmarks by integrating diverse modalities.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to develop a model that can effectively integrate and reason across multiple modalities, such as text, images, videos, and 3D geometry.

Method: The authors propose a unified multimodal model called Omni, which is natively trained on diverse modalities, enabling Context Unrolling and facilitating a more faithful approximation of the shared multimodal knowledge manifold.

Result: Omni achieves strong performance on both multimodal generation and understanding benchmarks and demonstrates advanced multimodal reasoning capabilities.

Conclusion: The proposed Omni model shows promising results in multimodal reasoning and generation tasks, highlighting the benefits of unified multimodal training and Context Unrolling.
...
TL;DR: A new framework called IMU-to-4D uses wearable sensors to reconstruct human motion and 3D scene layouts without vision.
...
Method: IMU-to-4D repurposes large language models for non-visual spatiotemporal understanding, using inertial sensor data from earbuds, watches, or smartphones.
...
Conclusion: Wearable motion sensors alone can support rich 4D understanding, enabling an alternative to vision-based perception.
...
TL;DR: The paper explores time as a learnable visual concept in videos, developing models for detecting speed changes, estimating playback speed, and generating videos at different speeds.


<details>
  <summary>Details</summary>
Motivation: The authors aim to investigate the perception and control of time passage in videos, which has been overlooked in computer vision research despite the centrality of videos.

Method: The authors use self-supervised learning to exploit multimodal cues and temporal structure in videos, developing models for temporal reasoning and control, including speed-conditioned video generation and temporal super-resolution.

Result: The authors curate a large slow-motion video dataset and develop models capable of temporal control, including generating videos at specified playback speeds and transforming low-FPS videos into high-FPS sequences.

Conclusion: The findings highlight time as a manipulable, perceptual dimension in video learning, enabling applications such as temporally controllable video generation, temporal forensics detection, and richer world-models.
...
<details>
  <summary>Details</summary>
Motivation: Existing studies on traffic accident detection and understanding lack deeper causal reasoning and integration of legal knowledge, which is addressed by the more challenging task of Traffic Accident Responsibility Allocation (TARA).

Method: AITP uses a Multimodal Chain-of-Thought (MCoT) mechanism and Retrieval-Augmented Generation (RAG) to enhance reasoning and integrate legal knowledge.
...
TL;DR: Developed AFRILANGDICT and AFRILANGEDU to address the lack of training resources for African languages, and fine-tuned LLMs to create AFRILANGTUTOR, achieving performance gains.


<details>
  <summary>Details</summary>
Motivation: The challenge of developing language learning systems for languages with insufficient training resources, particularly in Africa.

Method: Created AFRILANGDICT, a dictionary dataset, and AFRILANGEDU, a multi-turn training example dataset, then fine-tuned Llama-3-8B-IT and Gemma-3-12B-IT LLMs.

Result: Models trained on AFRILANGEDU outperformed their base counterparts, with gains ranging from 1.8% to 15.5% in LLM-as-a-judge evaluations.

Conclusion: The developed resources and models can facilitate further research on low-resource languages, enabling the creation of AI-assisted language tutors.
...
TL;DR: The paper proposes a Hierarchical Policy Optimization (HPO) approach to improve simultaneous speech translation (SST) efficiency and quality by post-training models on imperfect data.


<details>
  <summary>Details</summary>
Motivation: To reduce computational overhead in SST while maintaining quality, as large language models (LLMs) improve SST but are computationally expensive.

Method: The HPO approach uses a hierarchical reward system balancing translation quality and latency, post-training models on imperfect supervised fine-tuning (SFT) data.

Result: Experiments show +7 COMET score and +1.25 MetricX score improvements at 1.5 seconds latency for English to Chinese/German/Japanese translations.

Conclusion: The HPO approach effectively improves SST quality and efficiency, validated by ablation studies on various rewards, reward formulations, and segmentation strategies.
...
TL;DR: A new method using Discrete Wavelet Transform (DWT) is proposed to improve summarization of long, domain-specific documents, particularly in clinical and legal settings, by preserving structure and details.


<details>
  <summary>Details</summary>
Motivation: The challenge of summarizing long documents with large language models (LLMs) due to context limitations, information loss, and hallucinations, especially in clinical and legal settings.

Method: A DWT-based multi-resolution framework that decomposes text into global and local components, applied to sentence- or word-level embeddings.

Result: Comparable ROUGE-L scores on clinical and legal benchmarks, with improvements in semantic similarity, grounding, and factual consistency, achieving gains of over 2% in BERTScore and more than 4% in Semantic Fidelity.

Conclusion: DWT provides a lightweight, generalizable method for reliable long-document and domain-specific summarization with LLMs, reducing hallucinations and strengthening factual grounding.
...
<details>
  <summary>Details</summary>
Motivation: The scarcity of high-quality annotated data for training vision-language models (VLMs) for medical report generation.
...
Conclusion: The weighted loss function improves efficiency in medical report generation across multiple data scales.
...
TL;DR: This study uses the MARBERT model to predict emojis in Arabic tweets, achieving an accuracy of 0.75.
...
Method: The study collected a corpus of 11379 Arabic tweets, preprocessed them, and classified them into 14 categories. The MARBERT model was then finetuned to predict emoji use from textual input.
...
TL;DR: A novel SPARQL-based question-answering method, GRISP, is proposed, which uses a small language model to generate and refine SPARQL queries over knowledge graphs.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve SPARQL-based question-answering over knowledge graphs by leveraging a small language model to generate and refine SPARQL queries.

Method: The method, GRISP, uses a small language model to generate a natural-language SPARQL query skeleton and then iteratively replaces placeholders with knowledge graph items using constraints.

Result: The method achieves better results than other state-of-the-art methods on Wikidata and Freebase benchmarks.

Conclusion: GRISP presents a effective approach for SPARQL-based question-answering over knowledge graphs, outperforming existing methods.
...
TL;DR: The paper introduces IVG, a framework that improves Vision-Language Models' (VLMs) ability to understand interactive charts by combining spec-grounded introspection and view-grounded interaction.


<details>
  <summary>Details</summary>
Motivation: Current VLMs struggle with misreading values, hallucinating details, and confusing overlapping elements in charts due to a 'Pixel-Only Bottleneck' that treats interactive charts as static images.

Method: The proposed framework, Introspective and Interactive Visual Grounding (IVG), combines spec-grounded introspection and view-grounded interaction to improve VLMs' understanding of interactive charts.

Result: Experiments show that IVG achieves a QA accuracy of 0.81, with a +6.7% gain on overlapping geometries, and demonstrates its effectiveness in deployed agents that explore data autonomously and collaborate with human users in real-time.

Conclusion: The IVG framework effectively addresses the limitations of current VLMs in understanding interactive charts, providing a more accurate and reliable approach for data exploration and human-AI collaboration.
...
TL;DR: An automated system for analyzing classroom discourse was developed, improving the classification of teacher and student utterances and providing insights into student reasoning patterns.


<details>
  <summary>Details</summary>
Motivation: Manual coding of classroom discourse is labor-intensive, and an automated system is needed to understand knowledge construction and improve instructional practices.

Method: An automated discourse analysis system (ADAS) was developed using a dual-probe head RoBERTa-base classifier, synthetic data augmentation, and stratified-resplit annotated corpus.

Result: The system achieved improved classification results, and analysis revealed that teacher feedback-with-question moves are the most consistent antecedents of student inferential reasoning.

Conclusion: The study presents a successful automated system for analyzing classroom discourse, which can help improve instructional practices and maximize cognitive engagement.
...
Result: The study finds that language models use separate 'current-entity' and 'prior-entity' slots for different purposes, such as relational inferences and factual retrieval questions.

Conclusion: The results reveal a gap between available information in activations and the information models actually use, suggesting that the current/prior-entity slot structure is a natural substrate for behaviors requiring multiple perspectives.
...
TL;DR: The paper proposes a visual scaffolding framework to improve conversational agents' ability to maintain a reliable representation of shared context, reducing 'representational blur' and improving performance on the IndiRef benchmark.
...
Result: Evaluation on the IndiRef benchmark shows that incremental externalization improves over full-dialog reasoning, while visual scaffolding provides additional gains by reducing representational blur, and a hybrid multimodal setting yields the best overall performance.
...
TL;DR: The study examines the impact of Automatic Speech Recognition (ASR) bias on users' experiences, particularly for speakers of underrepresented dialects. It finds that users face frustration, feelings of inadequacy, and perform 'invisible labor' to make ASR systems work, highlighting the need for a broader understanding of algorithmic fairness.


<details>
  <summary>Details</summary>
Motivation: The study aims to investigate the human side of system bias in ASR, focusing on how system failures affect users' lived experiences, emotions, and interactions with technology.

Method: The researchers conducted user experience studies across four U.S. locations, representing distinct English dialect communities, and used qualitative analysis of open-ended narratives to gather data.

Result: The study reveals that users experience frustration, annoyance, and feelings of inadequacy due to ASR system failures. They perform 'invisible labor' to make systems functional and often internalize failures as personal inadequacy. The findings highlight the limitations of accuracy metrics in assessing algorithmic fairness.

Conclusion: The study concludes that algorithmic fairness assessments should consider the emotional labor, cognitive burden, and psychological toll of ASR system failures on users, particularly for speakers of underrepresented dialects. It emphasizes the need for a more comprehensive understanding of the impact of ASR bias on users' experiences.
...
TL;DR: This paper proposes a novel approach to enhance large language models (LLMs) in predicting future occupations by generating reasons based on a user's past education and career history, and then using these reasons to recommend next occupations.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this work is to improve the performance of LLMs in occupation prediction by aligning them with career paths and unobserved reasons behind occupation decisions.

Method: The proposed approach involves a two-step process: (1) generating high-quality 'reasons' for a user using their past education and career history, and (2) fine-tuning LLMs to perform reason generation and next occupation prediction. The approach uses a LLM-as-a-Judge to derive oracle reasons and fine-tunes small LLMs to perform the tasks.

Result: The results show that the proposed approach effectively enhances LLM's accuracy in next occupation prediction, making them comparable to fully supervised methods and outperforming unsupervised methods. Additionally, a single LLM fine-tuned to perform both tasks outperforms two LLMs fine-tuned separately.

Conclusion: The conclusion of this paper is that the proposed approach can improve the performance of LLMs in occupation prediction by generating high-quality reasons and fine-tuning LLMs to perform reason generation and next occupation prediction.
...
TL;DR: The paper introduces SPIA, a benchmark for evaluating text anonymization at the individual level, highlighting limitations of current span-based metrics and demonstrating the vulnerability of anonymized text to subject-level inference.


<details>
  <summary>Details</summary>
Motivation: Current text anonymization evaluation methods have limitations, including failure to capture what an adversary could infer and assuming a single data subject, which ignores multi-subject scenarios.

Method: The authors present SPIA, a benchmark that evaluates text anonymization at the individual level, using 675 documents across legal and online domains with novel subject-level protection metrics.

Result: Experiments show that even when over 90% of personally identifiable information (PII) spans are masked, subject-level inference protection can drop to 33%, indicating that most personal information can still be recovered through contextual inference.

Conclusion: The study emphasizes the importance of subject-level inference-based evaluation for ensuring safe text anonymization in real-world settings, as it provides a more accurate assessment of anonymization effectiveness.
...
<details>
  <summary>Details</summary>
Motivation: The ability of large language models to generate human-like text raises concerns about misuse, necessitating reliable detection methods.
...
TL;DR: The paper introduces EngramaBench, a benchmark for long-term conversational memory, and evaluates Engrama, a graph-structured memory system, against GPT-4o and Mem0. Engrama outperforms GPT-4o in cross-space reasoning but ranks lower in overall score.


<details>
  <summary>Details</summary>
Motivation: The increasing expectation for large language model assistants to retain and reason over information accumulated across many sessions.

Method: The paper evaluates three systems: Engrama, a graph-structured memory system; GPT-4o full-context prompting; and Mem0, an open-source vector-retrieval memory system. All systems use the same answering model (GPT-4o).

Result: GPT-4o full-context achieves the highest composite score (0.6186), while Engrama scores 0.5367 globally but outperforms GPT-4o in cross-space reasoning (0.6532 vs. 0.6291). Mem0 scores 0.4809.

Conclusion: The paper highlights a systems-level tension between structured memory specialization and aggregate optimization, with Engrama's components driving its cross-space advantage trading off against its global composite score.
...
Conclusion: The authors propose a novel LLM-based framework for multi-table entity matching that effectively addresses semantic inconsistencies and efficiency issues, demonstrating improved performance over baseline models.
...
TL;DR: The paper introduces PLOTTER, a framework that improves narrative generation by planning on structural graph representations, enhancing global coherence and logical consistency.


<details>
  <summary>Details</summary>
Motivation: Existing methods for narrative generation struggle with maintaining global narrative coherence, contextual logical consistency, and smooth character development.

Method: PLOTTER performs narrative planning on structural graph representations (event graph and character graph) using the Evaluate-Plan-Revise cycle, under rigorous logical constraints.
...
Conclusion: The proposed approach enhances the long context reasoning of LLMs in complex narrative generation by optimizing causality and narrative skeleton.
...
TL;DR: Researchers propose two metrics, Response Pattern Similarity (RPS) and Action Graph Similarity (AGS), to detect non-mandatory behavioral patterns in LLM agents, revealing potential homogenization.


<details>
  <summary>Details</summary>
Motivation: Model distillation in LLM agents leads to behavioral homogenization, with many agents sharing similar reasoning steps and failure modes, indicating a need for metrics to distinguish mandatory from non-mandatory behaviors.

Method: The authors propose RPS for verbal alignment and AGS for tool-use habits, and evaluate 18 models from 8 providers against Claude Sonnet 4.5, finding that AGS and RPS capture distinct behavioral dimensions.

Result: The study finds that within-family model pairs score higher in AGS than cross-family pairs, and that certain models exhibit high similarity to their teachers, with AGS distinguishing teacher-specific convergence from general improvement.

Conclusion: The proposed metrics, RPS and AGS, can effectively capture non-mandatory behavioral patterns in LLM agents, providing a way to detect potential homogenization and improve the diversity of the agent ecosystem.
...
<details>
  <summary>Details</summary>
Motivation: The authors aim to explore the effect of pre-training a Transformer on music before language on language acquisition.

Method: The authors use a developmental pipeline of music → poetry → prose, pre-training a Transformer on piano performances from the MAESTRO dataset.

Result: The results show a 17.5% perplexity improvement over random initialization, with a persistent 5.5% gap at plateau, and that real music matches the transfer ceiling of synthetic patterns with one-third the data.

Conclusion: The study suggests that structured human creative outputs can provide an efficient pre-training substrate for small language models, and proposes a capacity-dependent data curation principle.
...
TL;DR: The study evaluates the fairness of large language models in speech recognition across demographic groups, finding that LLM decoders do not amplify racial bias, but audio encoder design is crucial for equitable speech recognition.
...
Method: The study evaluates nine models across three architectural generations on 43,000 utterances from five demographic axes, using two datasets: Common Voice 24 and Meta's Fair-Speech.

Result: The results show that LLM decoders do not amplify racial bias, but Whisper exhibits pathological hallucination on Indian-accented speech. Audio compression predicts accent fairness more than LLM scale. Severe acoustic degradation compresses fairness gaps, but silence injection amplifies Whisper's accent bias.
...
TL;DR: The study tests the sensitivity of a reduction in K-way energy probe on standard discriminative predictive coding networks to cross-entropy (CE) removal. The results show CE is a major component of the decomposition, and its removal affects the probe-softmax gap.


<details>
  <summary>Details</summary>
Motivation: The motivation is to test the reduction's sensitivity to CE removal in predictive coding networks.
...
Result: The results show that CE removal affects the probe-softmax gap. Removing CE alone without changing inference dynamics halves the probe-softmax gap. CE training produces larger output logit norms than MSE or bPC training.

Conclusion: The study concludes that CE is a major empirically load-bearing component of the decomposition at this scale, and its removal affects the probe-softmax gap.
...
TL;DR: The paper proposes Explainable Authorship Variational Autoencoder (EAVAE), a framework that disentangles authorial style from content to improve authorship attribution and AI-generated text detection.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with content-style entanglement, leading to poor generalization across domains.

Method: EAVAE uses supervised contrastive learning and a Variational Autoencoder architecture with separate encoders for style and content representations, along with a novel discriminator for disentanglement.

Result: EAVAE achieves state-of-the-art performance on authorship attribution datasets (Amazon Reviews, PAN21, HRS) and excels in few-shot learning for AI-generated text detection on the M4 dataset.

Conclusion: The proposed EAVAE framework effectively addresses the challenge of content-style entanglement, leading to improved performance and interpretability in authorship attribution and AI-generated text detection tasks.
...
TL;DR: The study evaluates political bias in multi-document news summarization systems, focusing on large language models' handling of diverse political perspectives. It finds that mid-sized models offer the best balance of fairness and efficiency, and that debiasing interventions are highly model-dependent.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this study is to investigate and address the political bias present in multi-document news summarization systems, which is crucial for fairness across diverse political perspectives.

Method: The study uses the FairNews dataset and evaluates 13 large language models across five fairness metrics. It examines both baseline model performance and the effectiveness of various debiasing interventions, including prompt-based and judge-based approaches.

Result: The study finds that mid-sized language models consistently outperform their larger counterparts in terms of fairness and efficiency. It also discovers that prompt-based debiasing is highly model-dependent and that entity sentiment is the most challenging fairness dimension to address.

Conclusion: The study concludes that fairness in multi-document news summarization requires multi-dimensional evaluation frameworks and targeted, architecture-aware debiasing strategies rather than simply scaling up model size.
...
TL;DR: The paper introduces PolyChartQA, a dataset for question answering over multi-chart images, and evaluates the performance of state-of-the-art Multimodal Language Models (MLMs) on it.


<details>
  <summary>Details</summary>
Motivation: Deriving meaningful insights from complex information presented in multiple related charts together is a challenging task and has not been extensively explored.

Method: The authors introduce PolyChartQA, a mid-scale dataset of 534 multi-chart images and 2,694 QA pairs, and evaluate the performance of nine state-of-the-art MLMs on it using various metrics.

Result: The results show a 27.4% LLM-based accuracy drop on human-authored questions compared to MLM-generated questions, and a 5.39% L-accuracy gain with the proposed prompting method.

Conclusion: The paper highlights the challenges of understanding multi-chart images and presents a new dataset and evaluation method for MLMs, providing insights into their performance on this task.
...
TL;DR: The paper proposes CARE, a GenAI framework that assists counselors by generating real-time response recommendations in low-resource languages, demonstrating improved alignment with gold-standard counselor responses.


<details>
  <summary>Details</summary>
Motivation: Mental health challenges are increasing, straining emotional support services, and existing LLM-based agents struggle to replicate supportive language and intervention strategies used by professionals.

Method: The CARE framework fine-tunes open-source LLMs on curated subsets of real-world crisis conversations in Hebrew and Arabic, allowing the models to capture interaction patterns associated with successful de-escalation.

Result: CARE demonstrates stronger semantic and strategic alignment with gold-standard counselor responses compared to non-specialized LLMs in experimental settings.

Conclusion: Domain-specific fine-tuning on expert-validated data can significantly support counselor workflows and improve care quality in low-resource language contexts.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to improve multilingual polarization detection by addressing challenges in languages with distinct scripts or morphological richness.

Method: The method involves comparing multilingual generalists, language-specific specialists, and hybrid ensembles, and adopting a language-adaptive framework that switches between them based on development performance.

Result: The final system achieves a macro-averaged F1 score of 0.796 and an average accuracy of 0.826 across all 22 tracks.

Conclusion: The study concludes that a language-adaptive framework can effectively handle multilingual polarization detection by leveraging strengths of different models for various languages.
...
TL;DR: VLAA-GUI is a modular framework that addresses challenges in autonomous GUI agents, achieving top performance on benchmarks and surpassing human performance in some cases.


<details>
  <summary>Details</summary>
Motivation: Autonomous GUI agents face challenges of early stopping and repetitive loops, which VLAA-GUI aims to address.

Method: VLAA-GUI consists of three integrated components: Completeness Verifier, Loop Breaker, and Search Agent, which work together to guide the system on when to stop, recover, and search.

Result: VLAA-GUI achieves top performance on two benchmarks (77.5% on OSWorld and 61.0% on WindowsAgentArena) and surpasses human performance in some cases.

Conclusion: The proposed framework, VLAA-GUI, effectively addresses the challenges in autonomous GUI agents and demonstrates improved performance with its integrated components.
...
TL;DR: Decoupled DiLoCo is introduced to improve large-scale language model pre-training efficiency by breaking the lock-step synchronization barrier, achieving zero global downtime and competitive model performance.


<details>
  <summary>Details</summary>
Motivation: The SPMD paradigm's tight coupling across accelerators causes inefficiencies due to transient slowdowns, hardware failures, and synchronization overhead, which Decoupled DiLoCo aims to address.

Method: Decoupled DiLoCo partitions compute across independent 'learners' that execute local optimization steps and asynchronously communicate with a central synchronizer.

Result: Decoupled DiLoCo achieves improved training efficiency with zero global downtime in failure-prone environments and maintains competitive model performance across various tasks.

Conclusion: Decoupled DiLoCo successfully addresses the limitations of the SPMD paradigm and synchronous distributed methods, providing a more efficient and robust approach to large-scale language model pre-training.
...
TL;DR: The paper studies the reasoning capabilities of large language models through two basic operations: recall and state-tracking. It finds that hybrid architectures combining attention-based retrieval and recurrent state updates perform better than attention-only models on tasks requiring both primitives.


<details>
  <summary>Details</summary>
Motivation: To understand whether the observed gains in reasoning capabilities of large language models arise from more basic operations, and to investigate if hybrid architectures are better suited for tasks requiring both recall and state-tracking.

Method: Using matched Olmo3 transformer and hybrid models in instruction-tuned and reasoning-augmented variants, the authors evaluate these models on controlled tasks involving a mixture of state-tracking and recall primitives.
...
Conclusion: The findings suggest that reasoning tokens and architectural inductive biases contribute at different levels of the computational process, and that explicit reasoning can expand a model's effective operating range, but its benefit depends on how well the underlying architecture supports persistent state propagation.
...
TL;DR: The paper explores data selection strategies to improve cross-domain transfer in automating regulatory compliance detection, finding that targeted data selection significantly reduces negative transfer.


<details>
  <summary>Details</summary>
Motivation: Models trained on one regulation often fail to generalise to others, highlighting the need for methods to improve cross-domain transfer in regulatory compliance detection.

Method: The study evaluates four data selection approaches (random sampling, Moore-Lewis's cross-entropy difference, importance weighting, and embedding-based retrieval) for augmenting data in a natural language inference (NLI) task.
...
Conclusion: The findings suggest that principled data selection strategies can improve cross-domain adaptation in regulatory compliance detection, enabling more scalable and reliable automation.
...
<details>
  <summary>Details</summary>
Motivation: The need for a scalable and reliable evaluation method for TTS systems that accounts for linguistic diversity and multidimensional speech perception.
...
Result: A multilingual leaderboard ranking 7 state-of-the-art TTS systems based on over 120K pairwise comparisons from 1900 native raters, across 10 Indic languages.

Conclusion: The proposed framework provides a reliable and comprehensive evaluation of multilingual TTS systems, highlighting model strengths and trade-offs across perceptual dimensions.
...
TL;DR: The paper introduces OptiVerse, a comprehensive benchmark for evaluating Large Language Models (LLMs) on complex optimization tasks beyond mathematical programming and combinatorial optimization. Experiments with 22 LLMs show significant performance degradation on hard problems, and a proposed Dual-View Auditor Agent improves LLM accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for LLMs focus narrowly on Mathematical Programming and Combinatorial Optimization, limiting comprehensive evaluation. The authors aim to address this gap by introducing a more diverse benchmark.

Method: The authors introduce OptiVerse, a benchmark of 1,000 curated problems across neglected domains, including Stochastic Optimization, Dynamic Optimization, Game Optimization, and Optimal Control. They experiment with 22 LLMs of different sizes and propose a Dual-View Auditor Agent to improve LLM accuracy.

Result: Experiments reveal sharp performance degradation on hard problems, with even advanced models like GPT-5.2 and Gemini-3 struggling to exceed 27% accuracy. The Dual-View Auditor Agent improves LLM accuracy without significant time overhead.

Conclusion: OptiVerse serves as a foundational platform for advancing LLMs in solving complex optimization challenges, highlighting the need for improved modeling and logic capabilities in LLMs.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to improve candidate-job matching and labor-market analytics by accurately extracting skills from job advertisements, as LLMs often struggle with this task, especially with long-tail terms and cross-domain shift.

Method: The method involves a framework called SRICL, which combines semantic retrieval (SR), in-context learning (ICL), and supervised fine-tuning (SFT) with a deterministic verifier to improve skill extraction.

Result: The results show that SRICL achieves substantial STRICT-F1 improvements over GPT-3.5 prompting baselines and reduces invalid tags and hallucinated spans on six public span-labeled corpora of job-ad sentences.

Conclusion: The conclusion is that SRICL enables dependable sentence-level deployment in low-resource, multi-domain settings, improving the accuracy of skill extraction from job advertisements.
...
TL;DR: The paper presents a system for modeling affect and affective change in user-generated texts, exploring LLM prompting, MaxEnt, and neural regression approaches.
...
Method: Three approaches: LLM prompting (user-aware and user-agnostic), pairwise MaxEnt model with Ising-style interactions, and lightweight neural regression model with recent affective trajectories and trainable user embeddings.

Result: The system ranked first in both Subtask 1 and Subtask 2A, with findings indicating LLMs capture static affective signals and recent numeric state trajectories explain short-term affective variation.

Conclusion: The proposed system effectively models affect and affective change, with a focus on recent numeric state trajectories.
...
TL;DR: The paper proposes a novel method for evaluating sentence embedding effectiveness without relying on additional classifiers, providing an objective assessment of the model's performance.
...
Result: The study demonstrates that Concept Separation Curves provide an interpretable, reproducible, and cross-model approach for evaluating the conceptual stability of sentence embeddings across multiple domains and languages.

Conclusion: The proposed method offers a classifier-independent approach for evaluating sentence embedding methods, allowing for an objective assessment of the model's performance in capturing sentence-level concepts.
...
TL;DR: The paper proposes a method to discover the opinions of large language models (LLMs) on contested topics through a multi-turn interaction. The method uses two complementary probes, direct and indirect, to elicit the model's opinions and classify its behavior.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the issue of LLMs silently holding positions on contested topics and propagating them at scale, and to develop a method to discover these opinions in a way that resembles real-world interactions.

Method: The method, called llm-bias-bench, uses two probes: direct probing, which asks for the model's opinion across five turns of escalating pressure, and indirect probing, which engages the model in argumentative debate. The method also uses three user personas and a nine-way behavioral classification to separate persona-independent positions from persona-dependent sycophancy.

Result: The results show that argumentative debate triggers sycophancy 2-3x more than direct questioning, models that appear opinionated under direct questioning often collapse into mirroring under sustained arguments, and attacker capability matters mainly when an existing opinion must be dislodged.

Conclusion: The paper concludes that the proposed method can effectively discover the opinions of LLMs on contested topics and highlights the importance of considering the model's behavior in multi-turn interactions.
...
TL;DR: The paper introduces AgenticQwen, a family of small language models capable of multi-step reasoning and tool use, trained via multi-round reinforcement learning on synthetic and open-source data, achieving strong performance on agentic benchmarks and industrial tasks.


<details>
  <summary>Details</summary>
Motivation: The increasing demand for language models that can act as agents in real-world settings, under strict cost and latency constraints, drives the development of small agentic models.

Method: The AgenticQwen models are trained using a combination of reasoning RL and agentic RL with dual data flywheels that automatically generate increasingly challenging tasks.

Result: The AgenticQwen models achieve strong performance on multiple agentic benchmarks and close the gap with much larger models on search and data analysis tasks in an industrial agent system.

Conclusion: The AgenticQwen family of models demonstrates the potential of small language models to act as agents in real-world settings, under strict cost and latency constraints, making them highly desirable for modern industrial applications.
...
TL;DR: The study finds that non-English responses can outperform English ones in reasoning tasks, and proposes a new RL framework, polyGRPO, which treats language variation as an implicit exploration signal to improve reasoning performance.


<details>
  <summary>Details</summary>
Motivation: The authors hypothesize that language functions as a latent variable that structurally modulates the model's internal inference pathways, and aim to test this hypothesis through a Polyglot Thinking Experiment.

Method: The authors conducted a Polyglot Thinking Experiment, and proposed polyGRPO, an RL framework that generates polyglot preference data online under language-constrained and unconstrained conditions.

Result: The results show that non-English responses often achieve higher accuracy, and polyGRPO improves the base model by 6.72% absolute accuracy on four English reasoning testsets and 6.89% in their multilingual benchmark.

Conclusion: The study concludes that treating language as a latent variable expands the model's latent reasoning space, yielding consistent and generalizable improvements in reasoning performance.
...
TL;DR: The paper introduces Verbal Process Supervision (VPS), a training-free framework that uses natural-language critique to guide iterative generate-critique-refine loops, improving LLM reasoning.


<details>
  <summary>Details</summary>
Motivation: The authors aim to expand the axes of inference-time scaling for LLM reasoning beyond chain depth, sample breadth, and learned step-scorers.

Method: The authors propose Verbal Process Supervision (VPS), a framework that utilizes structured natural-language critique from a stronger supervisor to guide an iterative generate-critique-refine loop.

Result: The results show that VPS yields significant improvements on GPQA Diamond, AIME 2025, and LiveCodeBench V6, outperforming existing methods and establishing critique granularity as a new axis of inference-time scaling.

Conclusion: The authors conclude that critique granularity is a new axis of inference-time scaling, and VPS is an effective method for improving LLM reasoning, with performance scaling with the supervisor-actor capability gap.
...
TL;DR: The paper surveys the challenges of deploying language models in non-English-speaking and hardware-constrained communities, highlighting the intersection of multilinguality and edge deployment.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenges of deploying language models in linguistically diverse communities with severe infrastructure constraints, promoting inclusive and equitable language technologies.

Method: The authors surveyed 232 papers across the language modeling pipeline to understand the state of the art and challenges of combining multilinguality and edge deployment.

Result: The paper provides a comprehensive survey of existing research, open questions, and actionable recommendations for stakeholders in the NLP ecosystem.

Conclusion: The authors hope that their work contributes to the development of inclusive and equitable language technologies, addressing the 'last mile' challenge of multilinguality and edge deployment.
...
TL;DR: The paper proposes a framework for modeling annotator-specific label prediction and explanations, fine-tuned on annotators' rationales, to improve predictive performance and provide a richer representation of disagreement.


<details>
  <summary>Details</summary>
Motivation: To incorporate fine-grained signals of individual perspectives from annotator rationales into modeling perspectives, beyond just exploring disaggregated labels.

Method: The proposed framework uses a representation-level User Passport mechanism to condition predictions on annotator identity and demographic metadata, and introduces two explainer architectures: a post-hoc prompt-based explainer and a prefixed bridge explainer.

Result: Incorporating explanation modeling substantially improves predictive performance over a baseline annotator-aware classifier, with the prefixed bridge approach achieving more stable label alignment and higher semantic consistency, and the post-hoc approach yielding stronger lexical similarity.
...
TL;DR: The paper introduces novel topological methods for analyzing eye-tracking data by interpreting fixation sequences as time series and combining topological features with traditional statistical features, achieving better performance in dyslexia detection.


<details>
  <summary>Details</summary>
Motivation: To develop a new method for analyzing eye-tracking data by applying persistent homology, a technique from topological data analysis, to extract robust, multi-scale features from fixation sequences.

Method: The paper proposes novel filtrations for time series and constructs 'hybrid models' that combine topological features with traditional statistical features to analyze eye-tracking data.

Result: The hybrid models outperform existing approaches that rely solely on traditional features, and the topological features achieve performance comparable to established baseline methods.

Conclusion: The study demonstrates that persistent homology captures complementary information encoded in fixation sequences, and the proposed filtrations outperform existing ones, showing potential for improving dyslexia detection.
...
<details>
  <summary>Details</summary>
Motivation: To improve the assessment of dysarthria severity by developing a training-free method that can scale across multiple languages, aetiologies, and speech representation models.

Method: Using d-prime separability of phonological feature subspaces in frozen self-supervised speech representations, with 6 SSL backbones and analysis on 3,374 speakers from 25 datasets.

Result: The method yields distinguishable aetiology-specific degradation profiles, shows cross-lingual profile-shape stability, and is architecture-independent, supporting its robustness for dysarthria characterisation.

Conclusion: The phonological subspace analysis provides a robust, training-free framework for aetiology-aware dysarthria characterisation, with evidence of cross-lingual profile-shape stability and cross-backbone robustness.
...
TL;DR: The study finds that current methods for evaluating code generation bias are inadequate, as they underestimate bias in real-world tasks like generating machine learning pipelines.


<details>
  <summary>Details</summary>
Motivation: The authors aim to investigate the limitations of current bias evaluation methods in code generation, which primarily focus on simple conditional statements.
...
Result: The study reveals that generated pipelines exhibit significant bias, with sensitive attributes appearing in 87.7% of cases, which is substantially more prevalent than that captured by conditional statements (59.2%).

Conclusion: The findings challenge the validity of simple conditionals as proxies for bias evaluation and suggest that current benchmarks underestimate bias risk in practical deployments.
...
<details>
  <summary>Details</summary>
Motivation: The limitations of large token-indexed lookup tables, including poor parameter efficiency and rapid memory growth, are addressed.

Method: X-GRAM employs hybrid hashing, alias mixing, and normalized SwiGLU ShortConv to compress and refine retrieved vectors.

Result: X-GRAM improves average accuracy by up to 4.4 points over the vanilla backbone and 3.2 points over strong retrieval baselines.

Conclusion: X-GRAM offers a scalable and practical paradigm for future memory-augmented architectures by decoupling capacity from compute.
...
<details>
  <summary>Details</summary>
Motivation: The central obstacle in LLM agents operating in open-ended environments is not what to remember but how to use what has been remembered.

Method: AEL uses a two-timescale framework: a Thompson Sampling bandit learns which memory retrieval policy to apply at each episode (fast timescale), while LLM-driven reflection diagnoses failure patterns and injects causal insights into the agent's decision prompt (slow timescale).
...
Conclusion: The bottleneck in agent self-improvement is self-diagnosing how to use experience rather than adding architectural complexity.
...
TL;DR: A new memory framework called StructMem is proposed to improve temporal reasoning and multi-hop question answering in conversational agents by preserving event-level bindings and inducing cross-event connections.


<details>
  <summary>Details</summary>
Motivation: Current conversational agents' memory systems struggle to capture relationships between events, and there's a trade-off between efficient but simplistic flat memory and more informative but complex graph-based memory.

Method: StructMem, a structure-enriched hierarchical memory framework, which uses dual perspectives and periodic semantic consolidation to improve performance.

Result: StructMem shows improved performance on temporal reasoning and multi-hop question answering on the LoCoMo dataset, with reduced token usage, API calls, and runtime compared to previous memory systems.

Conclusion: The proposed StructMem framework offers a more efficient and effective approach to modeling relational structure in conversational agents' memory systems.
...
TL;DR: This paper analyzes the cultural coverage and regional biases of Large Language Models (LLMs), finding they tend to favor certain countries like Japan and provide more diverse outputs when prompted in high-resource languages.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this work is to investigate the regional preferences of LLMs when it comes to cultural-related questions, as previous works have shown limitations in cultural coverage and competence, with a bias towards Western and Anglocentric viewpoints.
...
Result: The results show that LLMs have a clear tendency towards countries such as Japan, and that prompting in high-resource languages like English leads to more diverse outputs and less inclination towards answering questions about countries where the input language is official. The cultural bias emerges after supervised fine-tuning, not during pre-training.

Conclusion: The study concludes that LLMs exhibit regional biases in cultural-related questions, with a preference for certain countries, and that the bias is influenced by the language used for prompting and the stage of LLM training.
...
<details>
  <summary>Details</summary>
Motivation: Existing audio question answering benchmarks can be solved through shortcut strategies or biases, rather than genuine audio reasoning.

Method: A large-scale benchmark, AUDITA, with human-authored trivia questions and probing queries that require robust auditory reasoning.

Result: Human average accuracy is 32.13%, while state-of-the-art models perform poorly with average accuracy below 8.86%.

Conclusion: AUDITA highlights the challenge of audio question answering and the need for better models that can handle robust auditory reasoning.
...
TL;DR: The paper addresses the challenge of video-based misinformation by creating datasets and developing a method to identify specific segments within videos that contain misinformation.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to tackle the issue of video-based misinformation, which is a multifaceted challenge due to the ease of recording and uploading videos. Previous research has limitations in providing context about when and what content within videos is misinformation.

Method: The method involves creating two novel datasets of over 500 videos with annotated fact-checked claims, transcribing audio to text, and employing state-of-the-art language models to classify and identify segments of videos that contain misinformation.

Result: The result shows that the approach can identify segments of videos with misinformation with an F1 score of 0.68.

Conclusion: The paper concludes by making the annotated datasets, transcripts, audio, and videos publicly available, and demonstrates a viable method for detecting misinformation spans within videos.
...
TL;DR: The paper presents a shared task on narrative similarity and representation learning, introducing a novel definition and dataset for narrative similarity, and evaluating narrative embedding representations.


<details>
  <summary>Details</summary>
Motivation: The motivation is to operationalize narrative similarity as a binary classification problem and to evaluate narrative embedding representations based on a novel definition of narrative similarity.

Method: The method involves collecting annotations for over 1,000 story summary triples, with each annotation backed by at least two annotators in agreement, and evaluating submitted systems and techniques for narrative similarity and embedding representations.

Result: The results show that LLM ensembles perform well in the triple-based classification setup, while systems with pre- and post-processing on pretrained embedding models perform similarly to custom fine-tuned solutions in the embedding setup, with potential headroom for improvement.

Conclusion: The paper concludes by identifying potential areas for improvement in automated systems for narrative similarity and representation learning, and providing visualizations of embeddings and instance-level classification results.
...
TL;DR: A new multi-agent web navigation method called Mango is proposed, which uses website structure to find optimal starting points and adaptively allocate navigation budget.


<details>
  <summary>Details</summary>
Motivation: Existing web agents are inefficient in exploring complex websites with deep hierarchical structures, often getting stuck in navigation traps or failing to reach target information within a limited budget.

Method: Mango uses a multi-armed bandit problem approach with Thompson Sampling to select URLs and an episodic memory component to store navigation history.

Result: Mango achieves a success rate of 63.6% on WebVoyager and 52.5% on WebWalkerQA, outperforming baselines by 7.3% and 26.8% respectively.

Conclusion: Mango demonstrates effectiveness in web navigation, generalizability across models, and open-source availability.
...
TL;DR: This study examines if large language models (LLMs) reflect social nuances in moral judgment, finding they prioritize fairness over social sensitivity, potentially leading to misalignments in real-world use.


<details>
  <summary>Details</summary>
Motivation: The study aims to determine if LLMs encode social nuances in moral judgment, particularly context dependency and modulation by interpersonal relationships, as they increasingly function as decision-support systems.

Method: The study uses the Whistleblower's Dilemma, varying crime severity and relational closeness, and evaluates LLMs from three perspectives: moral rightness, predicted human behavior, and autonomous model decision-making.

Result: The study finds that LLM decisions align with moral rightness judgments (fairness-oriented) rather than their own predictions of human behavior, which shift towards loyalty as relational closeness increases.

Conclusion: The study concludes that LLM decision-making prioritizes rigid, prescriptive rules over social sensitivity, posing a gap that may lead to significant misalignments in real-world deployments.
...
TL;DR: The study introduces RedirectQA, a dataset to evaluate how large language models (LLMs) memorize factual knowledge through different surface forms of entities. It finds that LLMs' factual recall varies significantly with different entity surface forms, and their robustness to variations depends on the type of variation.


<details>
  <summary>Details</summary>
Motivation: To understand how large language models (LLMs) memorize factual knowledge and evaluate their reliability and limitations, especially in terms of non-verbatim memorization.

Method: The study uses RedirectQA, an entity-based QA dataset that associates Wikidata factual triples with various surface forms for each entity. It evaluates 13 LLMs on their ability to recall facts based on different surface forms.

Result: The study finds that prediction outcomes often change with different entity surface forms. LLMs are more robust to minor orthographic variations than to larger lexical variations like aliases and abbreviations. Both entity and surface-level frequencies are associated with accuracy.

Conclusion: Factual memorization in LLMs appears to be neither purely surface-specific nor fully surface-invariant. The study highlights the importance of surface-form diversity in evaluating non-verbatim memorization.
...
TL;DR: The paper proposes a novel approach called MODEE for open-domain event extraction, leveraging multimodal learning with large language models and graph-based methods to improve event extraction across various domains.


<details>
  <summary>Details</summary>
Motivation: Existing event extraction methods are limited by their reliance on predefined event types and failure to utilize large language models effectively for open-domain scenarios, and they also struggle with document-level contextual, structural, and semantic reasoning.

Method: The proposed approach, MODEE, combines graph-based learning with text-based representations from large language models to enhance document-level reasoning for open-domain event extraction.

Result: MODEE outperforms state-of-the-art open-domain event extraction approaches and can also be generalized to closed-domain event extraction, where it achieves better results than existing algorithms.

Conclusion: The MODEE approach effectively addresses the limitations of current event extraction methods by leveraging multimodal learning and large language models, demonstrating superior performance in both open-domain and closed-domain event extraction scenarios.
...
Result: The system achieves a P90 alert latency of 3.5 minutes and a 95% discovery rate for high-priority incidents, outperforming baseline methods in routing accuracy, clustering quality, and Signal-to-Noise Ratio.

Conclusion: The paper concludes that TingIS is an effective end-to-end system for enterprise-grade incident discovery, capable of handling high-throughput and high-noise data in real-time.
...
TL;DR: The paper introduces EVENT5Ws, a large, manually annotated dataset for open-domain event extraction, and evaluates state-of-the-art language models, providing a benchmark and insights for future research.


<details>
  <summary>Details</summary>
Motivation: The need for automated event extraction approaches to support event understanding and analysis, particularly in emergencies, and the limitations of existing datasets.

Method: A systematic annotation pipeline was designed to create EVENT5Ws, a large, manually annotated, and statistically verified open-domain event extraction dataset.

Result: State-of-the-art pre-trained large language models were evaluated on EVENT5Ws, establishing a benchmark for future research, and models trained on EVENT5Ws generalized effectively to datasets from different geographical contexts.

Conclusion: The development of EVENT5Ws supports the creation of generalizable algorithms for event extraction, and lessons learned during dataset development provide recommendations for future large-scale dataset development.
...
TL;DR: The paper analyzes parliamentary discourse using a computational framework, examining how, what, and who speaks in similar ways in the Brazilian Chamber of Deputies from 2003 to 2025.


<details>
  <summary>Details</summary>
Motivation: Legislative behavior analysis often overlooks the rich content of political speech, focusing only on voting records.
...
Result: A long-term stylistic shift to shorter speeches, a legislative agenda responsive to national crises, and a map of discursive alignments where regional and gender identities are more salient than party affiliation.
...
TL;DR: The authors propose MathDuels, a self-play benchmark for evaluating language models' math problem-solving capabilities, where models both generate and solve math problems, revealing decoupled authoring and solving abilities.


<details>
  <summary>Details</summary>
Motivation: Existing math benchmarks are unable to differentiate model capabilities as they only test fixed problem sets; the authors seek to create a more dynamic evaluation method.

Method: MathDuels involves a three-stage generation pipeline for problem creation and a Rasch model for estimating solver abilities and problem difficulties; models both author and solve math problems.

Result: The benchmark reveals that authoring and solving capabilities are partially decoupled, and dual-role evaluation exposes capability separations not visible in single-role benchmarks.

Conclusion: MathDuels provides a dynamic evaluation method that co-evolves with participant strength, offering a more nuanced assessment of language models' math capabilities.
...
TL;DR: The paper explores using decoder-based Large Language Models (LLMs) for Automatic Speech Recognition (ASR) evaluation, finding they outperform traditional metrics like Word Error Rate (WER) and embedding-based semantic metrics in correlating with human perception.


<details>
  <summary>Details</summary>
Motivation: Traditional ASR evaluation metrics like Word Error Rate (WER) are insensitive to meaning, and the study aims to evaluate the relevance of decoder-based Large Language Models (LLMs) for ASR evaluation.

Method: The study uses three approaches: (1) selecting the best hypothesis between two candidates, (2) computing semantic distance using generative embeddings, and (3) qualitative classification of errors, on the HATS dataset.

Result: The best LLMs achieve 92-94% agreement with human annotators for hypothesis selection, compared to 63% for WER, and embeddings from decoder-based LLMs show performance comparable to encoder models.

Conclusion: LLMs offer a promising direction for interpretable and semantic ASR evaluation.
...
TL;DR: The study proposes an AI-based automated Course of Action (CoA) planning system for future warfare, introducing relevant doctrines and applicable AI technologies.


<details>
  <summary>Details</summary>
Motivation: The increasing complexity of warfare due to advancements in maneuver speeds, surveillance ranges, and weapon ranges, making traditional manned-based CoA planning challenging.

Method: The study reviews publicly available information on CoA planning doctrines and AI technologies, and proposes an architecture for an automated CoA planning system.

Result: The proposed architecture for an automated CoA planning system.

Conclusion: The development of an AI-based automated CoA planning system is necessary for future warfare, and the proposed architecture can guide the development of such systems.
...
TL;DR: The paper proposes a new framework for evaluating content moderation systems in rule-governed environments, shifting from agreement with human labels to policy-grounded correctness.


<details>
  <summary>Details</summary>
Motivation: The authors identify a failure mode in traditional evaluation methods, termed the 'Agreement Trap', where valid decisions are penalized due to ambiguity in the governing policy.

Method: The authors introduce new metrics, including the Defensibility Index (DI), Ambiguity Index (AI), and Probabilistic Defensibility Signal (PDS), to evaluate content moderation systems based on policy-grounded correctness.

Result: The framework is validated on 193,000+ Reddit moderation decisions, showing a 33-46.6 percentage-point gap between agreement-based and policy-grounded metrics, and achieving 78.6% automation coverage with 64.9% risk reduction.

Conclusion: The paper concludes that evaluation in rule-governed environments should shift from agreement with historical labels to reasoning-grounded validity under explicit rules.
...
TL;DR: COSPLAY, a co-evolution framework, improves long-horizon decision making in games by leveraging a learnable skill bank and large language models.


<details>
  <summary>Details</summary>
Motivation: Large Language Models (LLMs) struggle with consistent long-horizon decision making in games due to the lack of a mechanism to discover, retain, and reuse structured skills across episodes.

Method: COSPLAY framework, which includes an LLM decision agent that retrieves skills from a learnable skill bank and an agent-managed skill pipeline that discovers reusable skills from unlabeled rollouts.
...
Conclusion: The proposed COSPLAY framework effectively improves long-horizon decision making in games by leveraging a learnable skill bank and large language models, demonstrating significant performance gains over existing baselines.
...
TL;DR: The paper presents a two-level framework that automates the process of designing and optimizing AI agent workflows, eliminating the need for manual harness engineering.


<details>
  <summary>Details</summary>
Motivation: The increasing deployment of AI agents on complex workflows requires painstaking, expert-driven harness engineering, which is time-consuming and limits the adaptability of AI agents to new domains.

Method: The framework consists of two levels: the Harness Evolution Loop, which optimizes a worker agent's harness for a single task, and the Meta-Evolution Loop, which optimizes the evolution protocol across diverse tasks.

Result: The framework automates the design of the automation itself, shifting manual harness engineering into automated harness engineering, and enables rapid harness convergence on any new task without human intervention.

Conclusion: The paper proposes a framework that automates the process of designing and optimizing AI agent workflows, enabling AI agents to adapt to new domains without manual harness engineering.
...
<details>
  <summary>Details</summary>
Motivation: The need for a comprehensive evaluation framework for deep research agents in financial investment research to assess report quality.

Method: Defined qualitative and quantitative evaluation metrics and implemented an automated scoring procedure to assess report quality across three dimensions.

Result: AI-generated reports fall short across dimensions of report quality compared to reports authored by financial professionals.

Conclusion: The findings highlight the need for domain-specialized DR agents tailored to finance and establish a foundation for standardized benchmarking.
...
TL;DR: A new test-time compute allocation framework is proposed, which adapts computation and generation dynamically, outperforming existing methods with less compute.
...
Method: The proposed framework consists of a warm-up phase that identifies easy queries and assembles an initial pool of question-response pairs, followed by an adaptive phase that concentrates computation on unresolved queries and reshapes generation distributions through evolving in-context demonstrations.

Result: The approach consistently outperforms existing baselines across math, coding, and reasoning benchmarks while consuming substantially less inference-time compute.

Conclusion: The proposed test-time compute allocation framework offers a more efficient and effective approach to improving model performance.
...
TL;DR: Proposed HypEHR, a compact model for EHR question answering that uses hyperbolic geometry to embed clinical data, outperforming LLM-based methods with fewer parameters.


<details>
  <summary>Details</summary>
Motivation: Motivated by evidence that medical ontologies and patient trajectories exhibit hyperbolic geometry, and to address the cost and limitations of LLM-based pipelines for EHR question answering.

Method: Proposed a compact Lorentzian model, HypEHR, which embeds codes, visits, and questions in hyperbolic space and answers queries via geometry-consistent cross-attention with type-specific pointer heads.

Result: HypEHR approaches LLM-based methods on two MIMIC-IV-based EHR-QA benchmarks while using far fewer parameters.

Conclusion: HypEHR offers a compact and effective solution for EHR question answering by leveraging hyperbolic geometry.
...
TL;DR: Proposed a lightweight framework to mitigate representational bias in text-to-image models by intervening at the prompt level, allowing users to select fairness specifications and guide demographic-specific prompt variants.


<details>
  <summary>Details</summary>
Motivation: Text-to-image models often replicate societal biases in depicting demographic groups across professions, and existing mitigation methods are inaccessible to most users due to requirements of retraining or curated datasets.

Method: Proposed a lightweight, inference-time framework that mitigates representational bias through prompt-level intervention without modifying the underlying model, allowing users to select among multiple fairness specifications.

Result: The method shifts observed skin-tone outcomes in directions consistent with the declared target and reduces deviation from targets when the target is defined directly in skin-tone space, across 36 prompts spanning 30 occupations and 6 non-occupational contexts.

Conclusion: The proposed framework demonstrates how fairness interventions can be made transparent, controllable, and usable at inference time, directly empowering users of generative AI.
...
TL;DR: The paper presents an intuitive approach to reasoning over complex data sets using Active Data, which are atomic objects that interact with environments.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the complexity of monolithic designs in certain domains by enabling comprehension and specification of the design through problem-specific decompositions.

Method: The approach is based on Active Data, a bottom-up method that enables data to actively interact with environments.

Result: The implementation of Active Data concepts within the air traffic flow management domain shows improved performance.

Conclusion: The paper concludes that the bottom-up approach using Active Data improves designs by addressing computational and conceptual complexity.
...
TL;DR: The study fine-tuned a multi-modal vision-language model to predict natural language descriptions of embryo morphology and development in IVF, achieving better results than commercial and base models.


<details>
  <summary>Details</summary>
Motivation: The application of AI in IVF has shown promise but often relies on annotated data and doesn't utilize the multimodal nature of IVF data. The study aims to investigate if foundational vision-language models can be fine-tuned for IVF tasks.

Method: The study fine-tuned PaliGemma-2, a multi-modal vision-language model, using a publicly available embryo time-lapse dataset with 1,000 images and corresponding captions.
...
Conclusion: The study demonstrates the potential of foundational vision-language models to generalize to IVF tasks with limited data, enabling the prediction of natural language descriptions of embryo morphology and development.
...
TL;DR: The paper introduces COMPASS, a self-adaptive approach to automate prompt engineering for Large Language Models (LLMs) in complex software systems, and evaluates its effectiveness in generating high-quality explanations.
...
Method: The authors propose COMPASS, a proof-of-concept approach that formalises prompt engineering as a cognitive and probabilistic decision-making process, using a Partially Observable Markov Decision Process (POMDP) to model users' latent cognitive states and observable interaction cues.

Result: The evaluation of COMPASS using two diverse cyber-physical system case studies demonstrates its feasibility in integrating human cognition and user profile's feedback into automated prompt synthesis, and shows that it can generate high-quality explanations.

Conclusion: The paper concludes that COMPASS is a promising approach for automating prompt engineering in complex task planning systems, and highlights its potential to improve the quality and reliability of explanations generated by LLMs.
...
TL;DR: This paper develops methods to measure language models' propensity for unsanctioned behavior, applies them to 23 models and 11 environments, and finds equal contributions from strategic and non-strategic factors in explaining behavior.
...
Method: The paper contributes three methodological improvements: analyzing environmental factors' effects on behavior, quantifying effect sizes via Bayesian generalised linear models, and taking measures against circular analysis. It applies these methods to 23 language models and 11 evaluation environments.
...
Conclusion: The paper concludes by highlighting the need for future research on theoretical frameworks and cognitive models of AI decision-making.
...
<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to understand how AI can be incorporated into public administration in a way that is compliant with legal requirements and to examine the potential risks and challenges associated with its use.

Method: The paper uses a formal model to analyze the strategic use of AI within government, focusing on institutions' choices regarding the scale of automation, degree of codification, and safeguards on iterative use.

Result: The model shows that AI systems can become vulnerable to strategic use from within government, and that reforms initially improving oversight can later increase this vulnerability. It also suggests that expansions in AI use may be difficult to unwind.

Conclusion: The paper concludes that making AI usable can make procedures easier for future governments to learn and exploit, highlighting the need for careful consideration of the potential risks and challenges associated with the use of AI in public administration.
...
Conclusion: The proposed MAS architecture has the potential to improve at-home physiotherapy compliance by providing personalized supervision and dynamic feedback, and its clinical evaluation plan is outlined.
...
TL;DR: The paper explores the concept of intrinsic motivation, specifically empowerment, in multi-agent settings and its role in shaping group behavior.


<details>
  <summary>Details</summary>
Motivation: The authors are motivated to study intrinsic motivations, particularly empowerment, in multi-agent environments to understand how they drive behavior and organization at scale.
...
Result: The study finds that empowerment gives rise to characteristic modes of group-organization in two distinct environments: a pair of agents coupled by a tendon and a controllable Vicsek flock.
...
TL;DR: DAVinCI, a Dual Attribution and Verification framework, improves factual reliability and interpretability of Large Language Models (LLMs) by attributing claims to model components or external sources and verifying them through entailment-based reasoning.


<details>
  <summary>Details</summary>
Motivation: LLMs are prone to factual inaccuracies and hallucinations, posing risks in high-stakes domains like healthcare, law, and scientific communication where trust and verifiability are crucial.

Method: DAVinCI operates in two stages: (i) attributing generated claims to internal model components and external sources, and (ii) verifying each claim using entailment-based reasoning and confidence calibration.

Result: DAVinCI improves classification accuracy, attribution precision, recall, and F1-score by 5-20% compared to standard verification-only baselines across multiple datasets.

Conclusion: DAVinCI offers a scalable path to auditable, trustworthy AI systems by bridging attribution and verification, contributing to making LLMs not only powerful but also accountable.
...
TL;DR: The paper proposes a novel preference finetuning method to align a large language model (LLM) with domain-specific human preferences for generating online review responses, addressing challenges such as hallucinations, representing human preferences, and over conservatism.


<details>
  <summary>Details</summary>
Motivation: The rapid growth of online reviews makes it difficult for businesses to respond to all of them, and existing generative AI models may not align well with domain-specific human preferences.
...
Conclusion: The paper presents a novel approach to improve the generation of online review responses by aligning LLMs with domain-specific human preferences, addressing key challenges in the field.
...
TL;DR: Proposed ReCAPA, a framework that uses prediction and contrast to adjust deviations in Vision-Language-Action systems, achieving competitive results on embodied agent benchmarks.


<details>
  <summary>Details</summary>
Motivation: Mitigate the compounding effect of local errors in multi-step tasks, which propagate through subsequent steps and accumulate into cascading failures.

Method: Predictive Alignment and Planning Architecture (ReCAPA) with Sinkhorn-based and Score-field modules for semantic alignment, and joint update of action generator during training.

Result: ReCAPA achieves competitive results on embodied agent benchmarks such as VisualAgentBench, MineDojo, and AI2-THOR, outperforming strong baselines.

Conclusion: ReCAPA effectively mitigates error propagation and improves performance in Vision-Language-Action systems.
...
TL;DR: This paper studies policy robustness in Partially Observable Markov Decision Processes (POMDPs) against deviations in the observation model. It introduces the Policy Observation Robustness Problem and presents an algorithm, Robust Interval Search, to efficiently solve it.
...
Method: The paper formulates the Policy Observation Robustness Problem as a bi-level optimization problem and proposes the Robust Interval Search algorithm to solve it. The algorithm has soundness and convergence guarantees.

Result: The paper shows that the algorithm has polynomial time complexity in the non-sticky variant and at most exponential time complexity in the sticky variant. Experimental results validate the algorithm's scalability and practical utility in POMDP problems with tens of thousands of states.

Conclusion: The paper provides a solution to the Policy Observation Robustness Problem, enabling the determination of the maximum tolerable deviation in a POMDP's observation model that guarantees a policy's value remains above a specified threshold.
...
Result: The approach enables per-variant audit trails, catches epistemological errors, and complements post-hoc explanation methods like LIME and SHAP.

Conclusion: Meta-predicate validation is a step toward ensuring clinical decision support systems rest on appropriate evidence and can be independently audited.
...
<details>
  <summary>Details</summary>
Motivation: Existing approaches to Person-Job Fit (PJF) face challenges in handling low-quality job descriptions and similar candidate-job pairs, which impair model performance.

Method: The paper proposes two novel techniques: (1) LLM-based data augmentation to polish and rewrite low-quality job descriptions using chain-of-thought (COT) prompts, and (2) category-aware Mixture of Experts (MoE) to identify similar candidate-job pairs.
...
Conclusion: The proposed method improves Person-Job Fit (PJF) in online recruitment, leading to significant improvements in model performance and cost savings.
...
TL;DR: Introduced MMTR-Bench, a benchmark to evaluate MLLMs' ability to reconstruct masked text from visual context without explicit prompts.


<details>
  <summary>Details</summary>
Motivation: To assess MLLMs' intrinsic ability to understand layout, visual grounding, and knowledge integration in reconstructing masked text from visual context.

Method: Proposed a benchmark, MMTR-Bench, with 2,771 test samples across multiple languages and domains, and a level-aware evaluation protocol.

Result: Experiments on representative MLLMs showed that the benchmark is challenging, especially for sentence- and paragraph-level reconstruction.

Conclusion: MMTR-Bench provides a direct assessment of MLLMs' abilities in layout understanding, visual grounding, and knowledge integration.
...
TL;DR: MemPalace is an open-source AI memory system that uses the method of loci to organize long-term memory for large language models, achieving state-of-the-art retrieval performance but with overstated claims.
...
Result: MemPalace's headline retrieval performance is primarily attributable to its verbatim storage philosophy and ChromaDB's default embedding model, rather than its spatial organizational metaphor. The system achieves 96.6% Recall@5 on the LongMemEval benchmark.

Conclusion: MemPalace represents significant architectural insight but with overstated claims, and its performance is largely due to its verbatim storage philosophy and ChromaDB's embedding model rather than its spatial metaphor.
...
<details>
  <summary>Details</summary>
Motivation: The increasing use of LLMs in policy analysis and economic reporting necessitates evaluating their ability to provide directionally correct causal judgments, especially in ideologically contested economic questions.

Method: The study extends the EconCausal benchmark with ideology-contested cases and evaluates 20 state-of-the-art LLMs on their ability to predict empirically supported causal directions for 1,056 ideology-contested instances derived from top-tier economics and finance journals.

Result: The results show that LLMs are less accurate on ideologically contested economic questions and systematically less reliable in one ideological direction (market-oriented) than the other (intervention-oriented), with a directional skew in incorrect predictions.

Conclusion: The study highlights the need for direction-aware evaluation in high-stakes economic and policy settings, as LLMs exhibit systematic ideological bias when reasoning about economic causal effects.
...
TL;DR: The paper presents a reusable evaluation pipeline for generative AI applications, focusing on AI meeting summaries. It compares the performance of different GPT models, finding that GPT-5.1 leads in completeness and coverage, while GPT-4.1-mini achieves the highest mean accuracy.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to develop a systematic approach to evaluating generative AI applications, specifically AI meeting summaries, and to compare the performance of different models.

Method: The paper proposes a reusable evaluation pipeline consisting of five stages: source intake, structured reference construction, candidate generation, structured scoring, and reporting. The pipeline is tested on a dataset of 114 meetings and uses various GPT models (GPT-4.1-mini, GPT-5-mini, GPT-5.1) for evaluation.

Result: The results show that GPT-4.1-mini achieves the highest mean accuracy (0.583), while GPT-5.1 leads in completeness (0.886) and coverage (0.942). The study also finds that GPT-5.4 outperforms GPT-4.1 across all metrics in a deployment follow-up.

Conclusion: The paper concludes that the proposed evaluation pipeline provides a systematic approach to evaluating generative AI applications. The results highlight the strengths and weaknesses of different GPT models and suggest that reference-based scoring may overlook certain types of errors.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to identify whether the main bottleneck in VLMs for abstract visual reasoning lies in reasoning or representation.

Method: The method involves comparing end-to-end VLMs on raw images with LLMs given symbolic inputs derived from those images, using a Componential-Grammatical (C-G) paradigm to reformulate Bongard-LOGO as a symbolic reasoning task.

Result: LLMs achieve large and consistent gains, reaching mid-90s accuracy on Free-form problems, while a strong visual baseline remains near chance under matched task definitions.

Conclusion: The conclusion is that representation is a key bottleneck in abstract visual reasoning, and symbolic input can serve as a controlled diagnostic upper bound.
...
Conclusion: ReaGeo shows versatility and generalization ability in geocoding tasks, outperforming traditional multi-stage approaches.
...
TL;DR: The paper shows that small clock skew in distributed AI inference pipelines can cause observability issues, but not affect system functionality or performance.


<details>
  <summary>Details</summary>
Motivation: The authors aim to investigate the impact of clock skew on observability in distributed AI inference pipelines.

Method: The authors conducted controlled experiments on a multi-node AI inference pipeline with introduced clock skew at a single stage, using Kafka and ZeroMQ transports.

Result: The results show that causality violations emerge at 5 ms clock skew, while system throughput and output correctness remain largely unaffected.

Conclusion: The paper concludes that observability correctness in distributed AI systems depends on precise time alignment and that timing must be treated as a first-class concern.
...
TL;DR: The authors propose SemanticAgent, a semantic-aware synthesis framework that improves text-to-SQL synthesis by ensuring semantic validity, outperforming prior methods.
...
Method: The authors propose a three-stage protocol using three specialized modules: an analyzer, a synthesizer, and a verifier, to transform execution-based validation into a traceable reasoning process.

Result: SemanticAgent generates synthetic data that consistently outperforms prior synthesis methods under semantic-quality evaluation, leading to stronger downstream fine-tuning performance.

Conclusion: The proposed SemanticAgent framework improves the semantic validity of text-to-SQL synthesis, leading to better performance on semantically demanding benchmarks.
...
TL;DR: The paper proposes FairQE, a framework that mitigates gender bias in Quality Estimation (QE) for machine translation, achieving fairer and more reliable evaluation without sacrificing accuracy.
...
TL;DR: AI chatbot conversations can significantly shift human moral judgments, with effects lasting up to two weeks, without users realizing the persuasive intent.
...
Method: A within-subject naturalistic paradigm was used, where 53 participants rated moral scenarios, discussed some with a directive AI chatbot and others with a control agent.

Result: The brief conversations with the AI chatbot induced significant directional shifts in moral judgments, with effects lasting up to two weeks.

Conclusion: The study reveals a vulnerability to undetected and lasting manipulation of foundational moral values through AI chatbot conversations.
...
TL;DR: Introduced HiCrew, a hierarchical multi-agent framework for long-form video understanding, addressing limitations in temporal coherence and adaptability.


<details>
  <summary>Details</summary>
Motivation: Long-form video understanding faces challenges in handling spatiotemporal redundancy and intricate narrative dependencies. Existing methods sacrifice temporal coherence or use rigid workflows.

Method: Proposed HiCrew framework with three core contributions: Hybrid Tree structure for preserving temporal topology, Question-Aware Captioning for precision-oriented descriptions, and a Planning Layer for dynamic agent collaboration.

Result: Extensive experiments on EgoSchema and NExT-QA validate the effectiveness of HiCrew, showing strong performance across diverse question types, especially in temporal and causal reasoning tasks.

Conclusion: HiCrew effectively addresses limitations in long-form video understanding, providing a hierarchical multi-agent framework that preserves temporal coherence and adapts to question-specific demands.
...
Result: The study observed the emergence of visual reply chains, agents exhibiting aesthetic sovereignty, resisting stylistic convergence, and a decoupling between visual similarity and social ties.

Conclusion: The results reveal a fundamental asymmetry in current agent architectures, with strong expressive communication paired with a preservation of individual visual identity, and the platform is released for public access to study social dynamics in AI-native multi-agent systems.
...
Method: DIVERT uses a snapshot-based, coverage-guided approach to simulate user interactions, branching from critical decision points to explore diverse trajectories.
...
TL;DR: The study analyzes media framing of human-elephant conflict in India through 1,968 news articles, finding a prevalence of fear-inducing and aggression-related language that could reinforce public hostility towards elephants.


<details>
  <summary>Details</summary>
Motivation: To investigate how news media portrays human-elephant conflict in India, as ecological drivers are well-studied but media portrayal remains unexplored.

Method: A large-scale computational analysis using a multi-model sentiment framework combining long-context transformers, large language models, and a domain-specific Negative Elephant Portrayal Lexicon.

Result: The study found a dominance of fear-inducing and aggression-related language in news articles, which could undermine coexistence efforts with elephants.

Conclusion: The media framing of human-elephant conflict can shape public attitudes and conservation policy, highlighting the need for responsible wildlife reporting and beneficial media practices.
...
TL;DR: The paper proposes GeoMind, a tool-augmented framework for lithology classification in well logs, which models the task as a sequential reasoning process and outperforms existing approaches in classification performance while providing transparent decision-making.


<details>
  <summary>Details</summary>
Motivation: Existing approaches to lithology classification in well logs are limited by their static, single-step discriminative mapping paradigm, which often yields predictions detached from geological reality due to a lack of domain priors.

Method: The authors propose GeoMind, a tool-augmented agentic framework that organizes its toolkit into perception, reasoning, and analysis modules, and uses a global planner to adaptively coordinate these modules based on input characteristics.
...
Conclusion: The proposed GeoMind framework offers a more effective and transparent approach to lithology classification in well logs, enabling geologically plausible and evidence-grounded decisions.
...
TL;DR: The authors propose BioMiner, a multi-modal framework for automated extraction of bioactivity data from literature, separating semantic interpretation from ligand structure construction.


<details>
  <summary>Details</summary>
Motivation: Manual curation of protein-ligand bioactivity data struggles to keep pace with rapidly growing literature, and automated extraction remains challenging due to the need to interpret biochemical semantics and reconstruct chemically exact ligand structures.

Method: BioMiner uses direct reasoning for bioactivity semantic interpretation and a chemical-structure-grounded visual semantic reasoning paradigm for chemical structure construction, leveraging multi-modal large language models and domain chemistry tools.
...
Conclusion: BioMiner provides a promising solution for automated bioactivity extraction, validated through rigorous evaluation on the BioVista benchmark and demonstrated through practical applications.
...
TL;DR: The paper proposes a method to identify critical nodes in bipartite dependency networks, which are networks with two types of nodes representing dependencies between items and contributors.


<details>
  <summary>Details</summary>
Motivation: Current methods for identifying critical nodes in complex networks do not adequately address the bipartite dependency network structure, where edges represent dependency relationships across two groups only.

Method: The authors propose MinCov, a linear-time iterative peeling algorithm, and ShapleyCov, a centrality measure based on the Shapley value, to identify critical contributors whose removal isolates the largest number of items.

Result: The proposed methods, MinCov and ShapleyCov, significantly outperform traditional baselines on synthetic and large-scale real datasets, including a Wikipedia graph with over 250 million edges.

Conclusion: The paper concludes that MinCov and ShapleyCov are effective and efficient methods for identifying critical contributors in bipartite dependency networks, with MinCov achieving near-optimal performance.
...
TL;DR: The paper discusses a method for accurately estimating the prevalence of a category in a population when using imperfect measurement tools, such as diagnostic tests or language models, that may perform differently across various subpopulations.


<details>
  <summary>Details</summary>
Motivation: Standard methods for estimating prevalence assume that the error rates of measurement devices remain stable across populations, which is often not the case. The authors aim to address this issue, particularly when there is covariate shift (a change in the distribution of input features across populations).

Method: The authors propose using multicalibration, a concept from fairness in machine learning, which ensures that the measurement device is calibrated not just on average but conditional on the input features. This approach helps in reducing bias in prevalence estimation under covariate shift.

Result: The study finds that standard calibration and quantification methods can exhibit significant bias when there is covariate shift, whereas multicalibrated estimators maintain near-zero bias. The authors support their findings with simulations and two empirical applications: estimating employment prevalence across U.S. states and classifying political texts across four countries.

Conclusion: The paper concludes that multicalibration is sufficient for unbiased prevalence estimation under covariate shift and that it substantially reduces bias in practice. The authors highlight the importance of ensuring that calibration data covers the key feature dimensions along which target populations may differ.
...
<details>
  <summary>Details</summary>
Motivation: The 'Last Mile' Challenge of implementing AI governance requirements at the team level, despite existing frameworks at industry and organizational levels.

Method: Insider action research within an AI startup, using a legal-text-to-action pipeline to extract requirements, engage practitioners, and prioritize implementation.
...
<details>
  <summary>Details</summary>
Motivation: The problem of probabilistic verification of neural networks is significant when the input is affected by disturbances modeled by probabilistic variables.

Method: The proposed approach consists of three main innovations: state space subdivision using regression trees, boundary-aware sampling, and iterative refinement with probabilistic prioritization.
...
Result: The approach achieves per-user differentiation, near-zero cross-user contamination, and preserves personalization alongside privacy-enhancing guarantees.

Conclusion: The proposed architecture mitigates model inversion, membership inference, and training-data extraction attacks and is compatible with differentially private stochastic gradient descent (DP-SGD) for privacy-preserving shared model improvement.
...
<details>
  <summary>Details</summary>
Motivation: Feature discovery from complex unstructured data is a challenging reasoning problem, and LLMs, while capable of processing large amounts of information, can generate weak features if not properly controlled.

Method: The authors propose CoFEE, a reasoning control framework that enforces cognitive behaviors in LLMs during feature discovery, including backward chaining, subgoal decomposition, verification, and explicit backtracking.

Result: CoFEE achieves a 15.2% higher average Success Rate Score, generates 29% fewer features, and reduces costs by 53.3% compared to unconstrained LLM prompts.

Conclusion: The results indicate that reasoning control, as implemented in CoFEE, improves the quality and efficiency of LLM-based feature discovery, and that cognitively induced features generalize beyond the data used for discovery.
...
<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the challenge of decoder-only transformer models in performing abstract symbolic reasoning, particularly when faced with unseen variable names in propositional logic reasoning problems.

Method: The paper employs a combination of theoretical analysis and empirical experiments to investigate the issue of representational collapse in transformer models. It also proposes a set of techniques, including architectural changes, data diversity, and (un)embedding reset, to enhance the model's ability to generalize to unseen tokens.

Result: The results show that the proposed techniques effectively enable the model to generalize to unseen tokens in propositional logic reasoning problems. The paper also provides evidence of representational collapse in open-weight models, such as those in the Gemma 3 family.

Conclusion: The paper concludes that representational collapse is a significant factor in the limited ability of decoder-only transformer models to perform abstract symbolic reasoning with unseen variable names. It also demonstrates the effectiveness of the proposed techniques in addressing this issue and improving model generalization.
...
TL;DR: The paper proposes GS-Quant, a framework that generates discrete codes for Knowledge Graph entities, enabling Large Language Models to reason over graph structures more effectively.


<details>
  <summary>Details</summary>
Motivation: The modality gap between continuous graph embeddings and discrete LLM tokens is a critical challenge in Knowledge Graph Completion, and existing quantization-based approaches have limitations in capturing the hierarchical nature of human reasoning.

Method: The paper introduces GS-Quant, which includes a Granular Semantic Enhancement module and a Generative Structural Reconstruction module to generate semantically coherent and structurally stratified discrete codes for KG entities.
...
Conclusion: The proposed GS-Quant framework enables Large Language Models to reason over graph structures more effectively by generating discrete codes that capture the hierarchical nature of human reasoning.
...
<details>
  <summary>Details</summary>
Motivation: The increasing integration of AI in higher education raises questions about students' transparency in reporting AI-assisted work. The study aims to investigate the psychological mechanisms underlying students' willingness to disclose AI use.

Method: A sequential explanatory mixed-methods design was employed, consisting of a quantitative phase with a survey of 546 university students and a qualitative phase with semi-structured interviews of 22 students.

Result: The results indicate that psychological safety increases students' willingness to disclose AI use, while evaluation apprehension reduces disclosure intention. Institutional clarity, supportive instructional practices, and clear guidance promote responsible AI transparency.
...
TL;DR: Proposed an efficient image enhancement model for mobile devices using a hierarchical network architecture and Quantization-Aware Training (QAT) to balance output quality and processing speed.


<details>
  <summary>Details</summary>
Motivation: Image enhancement models for mobile devices struggle to balance high output quality with fast processing speeds, and recent deep learning models' performance degrades when converted to lower-precision formats.
...
Conclusion: The proposed model effectively addresses the training-deployment mismatch and enables efficient image enhancement on mobile devices.
...
TL;DR: Proposed a method to store and retrieve reusable reasoning skills to improve efficiency and performance of LLMs in solving new problems.


<details>
  <summary>Details</summary>
Motivation: Reasoning LLMs spend substantial tokens on long intermediate reasoning traces, which can be inefficient.
...
Conclusion: The proposed method has strong practical and economic potential for real-world deployment due to lower per-request cost.
...
TL;DR: The paper critiques the use of single-score leaderboards for evaluating large language models (LLMs), arguing that they don't account for diverse user goals and constraints. The authors analyze the LMArena benchmark dataset, finding it skewed and model rankings varying across prompt types. They propose an interactive visualization interface allowing users to define their own evaluation priorities.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the limitations of current LLM leaderboards, which are often based on a single aggregate score that doesn't reflect the diverse needs and constraints of actual users and organizations.

Method: The authors conduct an in-depth analysis of the LMArena benchmark dataset and design an interactive visualization interface as a design probe to investigate the evaluation challenge.

Result: The analysis reveals that the dataset is heavily skewed toward certain topics, model rankings vary across prompt slices, and preference-based judgments are used in ways that blur their intended scope. The proposed visualization interface allows users to define their own evaluation priorities and explore how rankings change accordingly.

Conclusion: The paper concludes that the interactive approach improves transparency and supports more context-specific model evaluation, pointing toward alternative ways to design and use LLM leaderboards.
...
TL;DR: A novel logic-based approach is developed to detect high-level temporally extended events from timestamped data and background knowledge, with a focus on the medical domain.


<details>
  <summary>Details</summary>
Motivation: The needs of the healthcare domain motivated the development of this approach, specifically to infer disease episodes and therapies from timestamped clinical observations.

Method: The framework employs logical rules to capture existence and termination conditions for simple temporal events and combines them into meta-events, using constraints to identify incompatible combinations of events and a repair mechanism to select preferred consistent sets of events.

Result: The approach shows computational feasibility and positive alignment with medical expert opinions in a lung cancer use case, and is purposely generic for reuse in other areas.

Conclusion: The proposed framework offers a viable solution for detecting high-level temporally extended events in the medical domain, with potential applications in other fields.
...
TL;DR: The paper proposes DiffMAS, a training framework for multi-agent systems that enables agents to learn latent communication, improving reasoning accuracy and decoding stability.


<details>
  <summary>Details</summary>
Motivation: Current multi-agent systems focus on agent roles and orchestration, treating inter-agent communication as a fixed interface, while latent communication through internal representations offers a promising alternative.

Method: The authors propose DiffMAS, a training framework that treats latent communication as a learnable component of multi-agent systems, performing parameter-efficient supervised training over multi-agent latent trajectories.

Result: Experiments show that DiffMAS consistently improves reasoning accuracy and decoding stability over single-agent inference, text-based multi-agent systems, and prior latent communication methods.

Conclusion: The proposed DiffMAS framework enables agents to jointly learn how information should be encoded and interpreted across interactions, achieving state-of-the-art results on various reasoning benchmarks.
...
TL;DR: The paper discusses how modern AI assistants often assume users have clear goals, but in reality, users' goals are often not fully formed, leading to 'Fantasia interactions' where AI systems fail to align with users' needs.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the limitations of current AI assistants in handling users with unclear goals, and to propose a new approach to AI design that provides cognitive support to users in forming and refining their intent.

Method: The paper uses an interdisciplinary approach, synthesizing insights from machine learning, interface design, and behavioral science to characterize the mechanisms and failures of 'Fantasia interactions'.

Result: The paper argues that existing interventions are insufficient and proposes a research agenda for designing and evaluating AI systems that better help humans navigate uncertainty in their tasks.

Conclusion: The paper concludes that AI systems should be designed to actively help users form and refine their intent, rather than simply treating user prompts as complete expressions of intent, and that this requires a rethinking of alignment research.
...
TL;DR: The paper proposes a two-stage framework for verifying the safety of high-risk AI systems, filling a regulatory gap in quantifying acceptable risk and providing a technical method for verification.


<details>
  <summary>Details</summary>
Motivation: The regulatory gap in specifying 'acceptable risk' in quantitative terms and providing a technical method for verifying deployed systems, hindering the enforcement of regulations like the EU AI Act.

Method: A two-stage framework inspired by aviation certification, involving a competent authority setting an acceptable failure probability and operational input domain, and using statistical verification tools (RoMA and gRoMA) to compute an upper bound on the system's true failure rate.

Result: The proposed framework provides a definitive, auditable upper bound on the system's true failure rate, satisfying existing regulatory obligations and shifting accountability to developers.

Conclusion: The paper provides a technical instrument to verify the safety of high-risk AI systems, bridging the gap between regulatory requirements and practical implementation.
...
TL;DR: A new AI game programming paradigm using large language models (LLMs) is introduced, enabling creation of customizable game agents that can adapt and learn through various game types.


<details>
  <summary>Details</summary>
Motivation: To extend and operationalize Claude Shannon's taxonomy of game-playing machines using LLMs, aiming for a long-term goal of self-programming AI.

Method: Nemobot, an interactive environment, integrates LLMs to create and deploy game agents. It uses various strategies across four game classes: dictionary-based, solvable, heuristic-based, and learning-based games.

Result: Nemobot demonstrates adaptability and learning capabilities across different game types, achieving a form of self-programming by integrating crowdsourced learning and human creativity.

Conclusion: The introduced paradigm represents a step toward self-programming AI, enabling AI agents to iteratively refine their logic through interaction and learning.
...
TL;DR: The paper proposes an agentic architecture to automate the translation of research questions into workflow specifications, bridging the gap between domain knowledge and infrastructure expertise.


<details>
  <summary>Details</summary>
Motivation: Scientists currently manually convert research questions into workflow specifications, requiring both domain knowledge and infrastructure expertise, which is a time-consuming and error-prone task.
...
Conclusion: The proposed architecture effectively automates the translation of research questions into workflow specifications, demonstrating improved accuracy, efficiency, and cost-effectiveness.
