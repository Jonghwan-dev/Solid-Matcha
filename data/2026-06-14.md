...
TL;DR: A portable, low-power fall detection system using edge computing with high accuracy and real-time processing.


<details>
  <summary>Details</summary>
Motivation: Falls among elderly people can cause serious injury and reduce quality of life, necessitating a timely prediction and detection system.

Method: A three-stage pipeline using YOLOX, Anchor-to-Joint (A2J), and a CNN for fall detection on an AMD Kria K26 edge device.

Result: The system achieved accuracies of 74%, 84.13%, and 75.85% for YOLOX, A2J, and the CNN respectively, with a throughput of 4.5 FPS.

Conclusion: The system demonstrates the feasibility of privacy-preserving fall detection on an edge device, supporting elderly monitoring and assistive healthcare.
...
<details>
  <summary>Details</summary>
Motivation: Current methods for hairstyle transfer struggle with large viewpoint and scale differences, requiring synthesis of missing hair content. The goal is to enable users to explore new looks without physical alteration, with applications in virtual try-on, augmented reality, and entertainment.

Method: HairPort framework, consisting of a Bald Converter for generating bald face images, a 3D-Aware Transfer Pipeline for reconstructing and re-rendering reference hairstyles, and a conditional flow-matching generator for synthesizing transferred results.

Result: The proposed method enables accurate, pose-consistent, and identity-preserving hairstyle transfer, outperforming existing methods qualitatively and quantitatively.

Conclusion: The HairPort framework effectively addresses the challenges of hairstyle transfer under large pose and scale differences, with potential applications in various fields.
...
TL;DR: Introduced Z-Image Turbo++, a 2-step image generation model distilled from an 8-step teacher model, achieving high-quality results through novel design choices.


<details>
  <summary>Details</summary>
Motivation: Pushing few-step diffusion distillation to 2 steps remains challenging due to increased task difficulty and limited model capacity.
...
Result: Substantial quality gap reduction between 2-step and 8-step generation in qualitative and quantitative evaluations.

Conclusion: Carefully tailored distillation strategies can improve the quality-efficiency trade-off in few-step generation.
...
TL;DR: The paper proposes a new method to improve the performance of Large Vision-Language Models (LVLMs) in medical imaging tasks by addressing issues such as factual inconsistencies and poor visual grounding.


<details>
  <summary>Details</summary>
Motivation: Existing post-training alignment approaches have limitations in the medical domain, including treating clinically critical tokens identically to generic filler text, reliance on static supervised fine-tuning references, and lack of explicit visual grounding constraints.

Method: The proposed method uses a bidirectional token-wise KL regularizer and a visual-contrastive grounding objective to form a fine-grained, on-policy alignment framework that constructs preference pairs by minimally editing model-generated outputs.
...
Conclusion: The proposed method improves the performance of LVLMs in medical imaging tasks by addressing factual inconsistencies, poor visual grounding, and misalignment with clinically meaningful feedback.
...
TL;DR: The paper proposes Dual-State Slot Attention (DSSA), a self-supervised framework for unsupervised video object-centric learning, which improves object identity stability and temporal consistency in challenging settings.


<details>
  <summary>Details</summary>
Motivation: Existing slot-based methods for unsupervised video object-centric learning struggle to maintain stable object identity in challenging settings such as rapid motion and partial occlusion due to objective conflict and token renormalization issues.

Method: The proposed DSSA framework separates appearance from identity by decomposing each slot into a local state and an identity state, and uses a learned recurrent transition and competition-modulated aggregation to update the identity state and prevent spurious updates.

Result: Experiments on MOVi-C, MOVi-D, and YouTube-VIS demonstrate that DSSA consistently improves segmentation quality and temporal consistency over prior methods, and yields stronger downstream object recognition and video dynamics prediction.

Conclusion: The proposed DSSA framework effectively addresses the limitations of existing slot-based methods and improves the performance of unsupervised video object-centric learning in challenging settings.
...
TL;DR: A novel object detection framework called Context-Centric Feature Fusion (CCFF) is proposed, which enhances detection in complex environments by leveraging local and global context through attention-based modules.


<details>
  <summary>Details</summary>
Motivation: Object detection in autonomous driving is challenging due to complex environments with rare classes, small-scale objects, and frequent objects. Standard frameworks struggle with these scenarios.

Method: The CCFF framework uses two attention-based modules: Local Context Fusion Module (LCFM) for spatial interactions and Global Context Attention Module (GCAM) for co-occurrence of objects priors.

Result: CCFF demonstrates significant improvement on relational consistency (CCS: 0.973 and 0.969 on Cityscapes and BDD100K), gains in small object detection (AP_S: 14.1%), and recovers rare classes. It processes images in real-time with a 0.2 FPS overhead.

Conclusion: The proposed CCFF framework effectively enhances object detection in complex environments by leveraging contextualized embeddings, achieving improved relational consistency, small object detection, and rare class recovery.
...
<details>
  <summary>Details</summary>
Motivation: The authors aim to address the practical scenario where the predominant category of visual data shifts over time, and propose a new notion of continual alignment to adapt the alignment module within pre-trained VLMs.

Method: The ECA approach employs three core mechanisms: a Mixture of Query (MoQ) module, Fisher Dynamic Expansion (FeDEx), and an embedding dictionary with Dictionary Replay (DR).

Result: Experimental results demonstrate that ECA significantly mitigates catastrophic forgetting and improves incremental learning performance compared to baseline methods.

Conclusion: The proposed ECA approach effectively enables models to continuously generate accurate text for new images while preserving previously acquired knowledge, and outperforms baseline methods in four new IL OpenITG benchmarks.
...
TL;DR: A novel approach, CD-RCM, is proposed for synthesizing new views in reflectance confocal microscopy (RCM) to enable continuous-depth visualization and isotropic 3D volumes.
...
Result: The approach achieves high-fidelity novel-view synthesis with sub-second inference time, enabling arbitrary-direction sectioning and histopathology-like cross-sectional examination without requiring per-patient optimization.

Conclusion: The proposed CD-RCM approach provides a solution for continuous-depth visualization in RCM, enabling the creation of isotropic 3D volumes and improving the interpretation of tissue structures.
...
TL;DR: The paper introduces SalArt-VQA, a benchmark to evaluate vision-language models (VLMs) for fine-grained understanding of artifacts in AI-generated images, revealing hidden failures in state-of-the-art models.


<details>
  <summary>Details</summary>
Motivation: Current VLMs can detect AI-generated image artifacts but may rely on incorrect visual cues or regions, or describe non-existent defects. The authors aim to evaluate these behaviors directly.

Method: The authors propose SalArt-VQA, a diagnostic benchmark with 950 images and 3,681 human-authored multiple-choice questions, evaluating presence detection, semantic localization, spatial grounding, and evidence-grounded defect identification.

Result: The strongest VLM model achieved 99.37% detection recall but only answered all four artifact-side questions correctly on 53.26% of images, highlighting a sensitivity-calibration tradeoff.

Conclusion: High artifact detection accuracy does not imply grounded artifact understanding; SalArt-VQA exposes hidden failure modes and provides a fine-grained evaluation of VLM artifact claims.
...
TL;DR: The paper introduces VLADriveBench, a framework for evaluating Vision-language-action (VLA) models that generate chain-of-thought (CoT) reasoning alongside driving trajectories. It assesses CoT relevance, consistency, and causal connection to driving actions.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks only evaluate trajectory quality of VLA models, neglecting the assessment of CoT relevance, consistency, and causal connection to driving actions.

Method: VLADriveBench combines observational metrics (mentioning, hallucination, contradiction, action alignment) with a CoT intervention protocol.

Result: Applying VLADriveBench to three models across two architectures reveals diverging results: ORION scores high on observational alignment but its CoT is epiphenomenal, while Alpamayo v1.5 scores lower but its CoT is strongly causal.

Conclusion: The study highlights the importance of evaluating CoT-action relationship in VLA models, and VLADriveBench provides a comprehensive framework for doing so.
...
<details>
  <summary>Details</summary>
Motivation: Existing approaches for selecting in-context examples rely on feature-space similarity, but this assumption does not always hold, and visually similar examples are not necessarily those that most effectively enhance in-context learning performance.

Method: The authors propose GRIP, a learnable vision-only retrieval framework that uses contrastive training to identify examples that truly improve model predictions, leveraging feedback from LMMs.

Result: GRIP improves consistently over similarity-based retrieval on Qwen2.5-VL-7B across three multimodal tasks, with its strongest gains in classification on Idefics2-8B, and retrievers trained with feedback from one open LMM can be transferred to other models without retraining.

Conclusion: The authors conclude that GRIP enables scalable and cost-efficient deployment of M-ICL by improving the selection of in-context examples and allowing for transferability across different LMMs.
...
TL;DR: The paper proposes a method for Moving Instance Segmentation (MIS) using event cameras and images, addressing challenges in segmenting small moving instances under limited resolution and low-light conditions.


<details>
  <summary>Details</summary>
Motivation: Current multimodal MIS methods struggle with segmenting small moving instances due to sparse features from event cameras under limited resolution and entanglement of appearance and motion cues.

Method: A dual-disentangling feature extraction framework and a multi-granularity cross-modal alignment are proposed to improve feature density and enable effective fusion of event and image features.

Result: The proposed method achieves state-of-the-art performance in multimodal MIS, especially for small instances under challenging conditions such as fast motion and low-light settings.

Conclusion: The paper presents a novel approach to enhance the performance of MIS by leveraging the strengths of both event cameras and images, leading to improved results in various challenging scenarios.
...
TL;DR: The paper introduces PERIA, a tool-augmented visual agent that enhances spatial reasoning capabilities in vision-language models through active evidence acquisition and multi-step visual interaction.


<details>
  <summary>Details</summary>
Motivation: Recent vision-language models are limited in spatial reasoning tasks due to their reliance on implicit visual representations, which are insufficient for recovering fine-grained spatial evidence.

Method: The authors propose PERIA, which uses two lightweight tool families: vision perception tools and vision interaction tools, and is trained with a unified recipe combining supervised tool-use trajectory synthesis, composite rewards, and OR-GIGPO.
...
Conclusion: The proposed PERIA effectively enhances spatial reasoning capabilities in vision-language models, demonstrating its effectiveness in active evidence acquisition and multi-step visual interaction tasks.
...
<details>
  <summary>Details</summary>
Motivation: The Abstraction and Reasoning Corpus (ARC) is crucial for Artificial General Intelligence (AGI), but existing methods are limited by relying on pure language or vision-only approaches, which have drawbacks such as heavy dependence on large language models or overfitting to pixel-level patterns.

Method: The L-VARC framework introduces a language-guided Learning Using Privileged Information (LUPI) branch, which includes a Semantic Compression Module and a Cross-Attention Projector to align visual features with semantic embeddings, and trains a lightweight model with only 18 million parameters.

Result: Extensive experiments demonstrate that L-VARC effectively leverages linguistic priors to boost visual reasoning and outperforms state-of-the-art methods, with ablation studies confirming the contribution of the proposed designs.

Conclusion: The proposed L-VARC framework offers a promising approach to enhancing visual reasoning by integrating language guidance, achieving state-of-the-art performance with a lightweight model, and providing a potential avenue for advancing Artificial General Intelligence (AGI).
...
TL;DR: A new framework called DECNN is proposed for task-adaptive sampling in image and surface-based learning tasks, which dynamically redistributes computational attention to informative regions.


<details>
  <summary>Details</summary>
Motivation: Uniform convolution in traditional convolutional neural networks (CNNs) can be inefficient as it allocates equal computational effort to both informative and uninformative regions, particularly in medical imaging where pathological changes are spatially confined.

Method: The proposed Density-Equalizing Convolutional Neural Network (DECNN) employs density-equalizing mappings guided by a learned density function to redistribute convolutional receptive fields non-uniformly over the domain, focusing on informative areas.

Result: DECNN achieves competitive or superior performance with fewer parameters, accurately identifies task-relevant regions, and remains robust under complex geometric variations in experiments on image classification and craniofacial surface analysis.

Conclusion: DECNN offers a more efficient use of model capacity, yielding a lightweight yet expressive architecture that produces an interpretable saliency map, enhancing the utilization of model capacity in image and surface-based learning tasks.
...
TL;DR: The paper identifies a failure mode called Modal Isolation in interleaved thinking models, where text and visual generations diverge and ignore each other. It proposes a two-stage training framework, MoTiF, to improve cross-modal coherence and task accuracy.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the Modal Isolation issue in interleaved thinking models, which causes text and visual generations to alternate without informing each other, leading to poor performance in complex long-chain scenarios.

Method: The authors propose MoTiF, a two-stage training framework consisting of Reflective SFT and Flow-GRPO. Reflective SFT trains the model to detect and recover from erroneous visual outputs, while Flow-GRPO improves image generation fidelity via reinforcement learning.

Result: The proposed MoTiF framework substantially improves both cross-modal coherence and final task accuracy across four visual puzzle benchmarks, demonstrating the importance of explicit structural supervision at modality boundaries.

Conclusion: The paper concludes that effective interleaved reasoning requires explicit structural supervision at modality boundaries, rather than merely scaling or end-task optimization, and that MoTiF is a promising approach to address the Modal Isolation issue.
...
TL;DR: The paper proposes AGAR, a method to improve Visual Text Comprehension (VTC) by adaptively rendering text into images for better vision-language model (VLM) performance.


<details>
  <summary>Details</summary>
Motivation: Existing VTC pipelines have limitations in understanding how VLMs process visualized text, and the authors aim to improve VTC QA tasks by analyzing VLM behavior.

Method: The authors propose AGAR, a training-free, model-agnostic method that uses VLM attention to identify important visual patches, and re-renders the page with those patches enlarged.

Result: AGAR consistently improves off-the-shelf VLMs, composes with VLM post-training, and remains robust under input degradation across nine VTC benchmarks and four VLM backbones.

Conclusion: The paper concludes that AGAR is an effective method for improving VTC performance by adaptively rendering text into images, and provides insights into VLM behavior in processing visualized text.
...
TL;DR: The paper introduces Bayesian Conditional Priors (BCP) Estimation, a method for adapting multi-label recognition with frozen Vision-Language Models (VLMs) under distribution shift, improving label coherence and accuracy.


<details>
  <summary>Details</summary>
Motivation: Multi-label recognition with frozen VLMs is brittle under distribution shift, leading to incoherent label sets due to independent scoring of labels and ignoring co-occurrence structure.
...
Conclusion: The proposed BCP Estimation method effectively improves multi-label recognition with frozen VLMs under distribution shift, promoting coherent and accurate label sets.
...
TL;DR: The paper proposes Masked Multi-View Test-Time Adaptation (MAMVI) to improve 3D point cloud model performance under distribution shifts. MAMVI uses a unified single-step adaptation with a hybrid masking strategy, achieving state-of-the-art accuracy and 4.9-8.9 times faster inference.


<details>
  <summary>Details</summary>
Motivation: 3D point cloud models suffer performance degradation under distribution shifts. Existing multi-view test-time adaptation methods have high inference latency due to sequential optimization.

Method: The authors propose MAMVI, which uses a unified single-step adaptation with a hybrid masking strategy, aggregating losses across multiple views and a confidence-based adaptive learning rate.

Result: MAMVI achieves state-of-the-art accuracy on ShapeNet-C and ScanObjectNN-C, and remains competitive on ModelNet-40C with 4.9-8.9 times faster inference.

Conclusion: The proposed MAMVI method effectively mitigates performance degradation of 3D point cloud models under distribution shifts, achieving high accuracy and efficiency.
...
TL;DR: This paper proposes an improved YOLO-based architecture, YOLO-AMC, for automated crack detection in infrastructure inspection and Structural Health Monitoring (SHM).
...
Method: The proposed YOLO-AMC integrates attention mechanisms (GAM, Res-CBAM, SA) into the multi-scale feature fusion layers of YOLOv11.
...
Conclusion: The proposed YOLO-AMC model achieves a favorable trade-off between accuracy and deployment efficiency, making it suitable for automated crack detection in infrastructure inspection and SHM.
...
TL;DR: A novel method for robust model fingerprinting in generative text-to-image models that resists collusion attacks and protects intellectual property rights.


<details>
  <summary>Details</summary>
Motivation: Existing model fingerprinting methods lack robustness against collusion attacks, where multiple attackers combine models to remove fingerprints, threatening intellectual property rights.

Method: The proposed method encodes fingerprints into a personalized normalization module (PNM) within T2I models and introduces an anti-collusion mechanism based on lossless function-invariant parameter transformations.

Result: The method achieves high fidelity and robustness across multiple T2I tasks with fingerprint extraction accuracy exceeding 99.5%, and significantly increases the FID of colluded models.

Conclusion: The proposed method provides a robust solution for model fingerprinting in T2I models, effectively defending against collusion attacks and protecting intellectual property rights.
...
<details>
  <summary>Details</summary>
Motivation: Improve 3D object detection through Camera and LiDAR fusion for TUMTraf V2X cooperative 3D object detection track.

Method: Fuses roadside cameras with infrastructure-plus-vehicle point cloud, using CenterPoint-style head with generalized IoU regression loss.

Result: Achieved 0.85 mAP on public test split, with further improvements to 0.89 mAP and 0.99 mAP through additional studies.

Conclusion: Proposed Camera and LiDAR fusion detector shows promising results for 3D object detection, highlighting overlap issues in training data.
...
TL;DR: BabyMind, a new method for learning word meanings from infant-view recordings, improves accuracy in identifying objects in cluttered frames.


<details>
  <summary>Details</summary>
Motivation: The difficulty in learning grounded word meaning from natural experience due to resolving ambiguities in infant-view recordings, specifically when and where the named referent appears in a cluttered frame.

Method: BabyMind, an object-first bias for child-view contrastive learning, which uses an offline mask-based region interface, tracking, and a prototype-space multiple-instance contrastive objective.
...
Conclusion: BabyMind effectively learns word meanings from infant-view recordings and improves object identification accuracy in cluttered frames.
...
TL;DR: The paper presents a compact latent world model for predicting future camera scenes in autonomous vehicles, enabling planning and simulation without real-world rollouts. The model uses a latent Diffusion Transformer (DiT) and achieves better performance than regression models in terms of perception and distortion.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the challenge of predicting future camera scenes in autonomous vehicles, which is crucial for planning and simulation. The authors aim to develop a compact and trainable model that can predict realistic future scenes without real-world rollouts.

Method: The authors propose a compact latent world model that predicts future scene latents given the present front-camera latent and a sequence of ego-actions. They use a latent Diffusion Transformer (DiT) and evaluate it on 150 held-out nuScenes scenes. They also benchmark different encoders and identify the ingredients needed for the DiT model.

Result: The results show that the proposed model achieves better performance than regression models in terms of perception and distortion. Specifically, the model attains KID 0.078 versus 0.375 for regression (4.8× better) and a deployable train-derived calibration makes this practical without test-time ground truth. The model is also genuinely action-controllable, with steering driving scene displacement (Spearman ρ = 0.81).

Conclusion: The paper concludes that the proposed compact latent world model is effective in predicting future camera scenes in autonomous vehicles. The model uses a latent Diffusion Transformer (DiT) and achieves better performance than regression models in terms of perception and distortion. The results demonstrate the potential of the model for planning and simulation in autonomous vehicles.
...
TL;DR: A new methodology for real-time prediction of human poses using 3D volumetric video data is introduced, enabling ergonomic assessments and other posture analysis applications.


<details>
  <summary>Details</summary>
Motivation: The increasing need for safety and health monitoring in workplace environments, and the limitations of traditional camera-based systems for postural evaluation.

Method: A system that analyzes 3D point clouds in real-time, using a personalized deep learning classifier trained on manually labeled data, and combining 3D data technologies with traditional 2D pose estimation algorithms.

Result: A scalable and pragmatic approach for real-time ergonomic evaluation, demonstrated through a case study with RGB-D cameras capturing subjects performing load-lifting tasks.

Conclusion: The proposed methodology offers a notable contribution to the domain of workplace safety and health monitoring, providing a real-time solution for ergonomic evaluation and posture analysis.
...
TL;DR: A new method for generating adversarial attacks on skeletal human action recognition systems without degrading motion quality.
...
Method: A distribution-based adversarial attack method that minimizes the risk gap and preserves motion quality without introducing noise-like perturbations. A new metric is proposed to evaluate motion quality in alignment with human perception.

Result: The proposed method demonstrates superiority in attack success rate and post-attack motion quality across state-of-the-art S-HAR methods and two datasets.

Conclusion: The success of the quality-preserving attack raises concerns about the robustness of action recognizers, highlighting the need for further enhancements.
...
TL;DR: A new network, GeoCFNet, is proposed for accurate and stable confidence field estimation in robot-assisted endoscopic submucosal dissection (ESD) guidance.


<details>
  <summary>Details</summary>
Motivation: The technical complexity and risk of complications in ESD demand stable and precise visual guidance, but reliable confidence field estimation remains challenging in dynamic endoscopic scenes.
...
Conclusion: The proposed GeoCFNet enables accurate and stable confidence field estimation for robot-assisted ESD guidance, which has the potential to improve the outcomes of ESD procedures.
...
TL;DR: Proposed selective mask propagation for multi-object tracking, leveraging VOS models for hard frames.


<details>
  <summary>Details</summary>
Motivation: Multi-object tracking has a heavy-tailed difficulty distribution, with most frames being easy but a small fraction being hard, which can be addressed by VOS models.

Method: Selective mask propagation, dispatching to VOS model on uncertain frames, modifying base tracker output only with confident VOS predictions.

Result: Improved performance on DanceTrack and SportsMOT benchmarks, achieving state-of-the-art 86.8 HOTA on SportsMOT.

Conclusion: Selective mask propagation effectively combines lightweight base trackers with powerful VOS models for improved multi-object tracking.
...
TL;DR: The paper proposes TetherCache, a cache management strategy for long video generation using autoregressive video diffusion models, addressing the challenges of limited cache budget and context distribution shift.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the quality and stability of long video generation (up to minute-level) using autoregressive video diffusion models, which currently suffer from visual artifacts, quality degradation, and temporal drift due to caching and conditioning limitations.

Method: The method proposed is TetherCache, which includes organizing the cache into sink, memory, and recent regions, and introducing two mechanisms: GRAB (Gated Recall with Attention-Diversity Balancing) for selecting long-range memory frames, and TAME (Trusted Alignment via Memory Editing) for lightly editing recalled memory tokens to align with a trusted context distribution.

Result: The results show that TetherCache consistently improves long-video generation quality on VBench-Long across 30s, 60s, and 240s settings. Specifically, for 240s generation, it substantially improves overall and semantic scores while reducing quality drift from 7.84 to 1.33.

Conclusion: The conclusion is that TetherCache is an effective, training-free, and plug-and-play cache management strategy for drift-resistant long video generation, enabling stable long-horizon autoregressive video diffusion.
...
TL;DR: SeamEdit is a training-free, model-agnostic pipeline for semantic region editing in large images, leveraging any VLM with inpainting capability as a black-box oracle to mitigate issues like semantic deformation and visible seam artifacts.


<details>
  <summary>Details</summary>
Motivation: The need for high generative quality and natural integration with surrounding content in large image editing, while underexplored strong generation capability of closed-source models.

Method: A five-stage post-hoc pipeline: overlay-based tile decomposition, black-box VLM inpainting, geometric and color-consistency correction, seam-risk-based multi-candidate ranking, and dynamic-programming curved seam fusion.

Result: SeamEdit reduces seam visibility and supports semantic modification of arbitrary tile regions.

Conclusion: SeamEdit effectively addresses challenges in large image editing by providing a flexible and efficient solution that works with any VLM having inpainting capability.
...
TL;DR: The authors propose LaME, a latent reasoning multimodal embedding method that achieves competitive performance while being faster and more efficient than existing methods.


<details>
  <summary>Details</summary>
Motivation: The authors are motivated by the limitations of current multimodal embedding methods that use Chain-of-Thought (CoT) reasoning, which can be computationally expensive and reliant on annotation quality.

Method: The authors propose LaME, which uses a weakly supervised information bottleneck to perform latent reasoning within a single forward pass, eliminating the need for CoT annotations.

Result: LaME achieves competitive performance on MMEB-v2 and MRMR datasets, surpassing some explicit CoT-based models, while delivering 60x faster inference than explicit CoT methods and 2x faster than latent baselines.

Conclusion: The authors conclude that LaME is a more efficient and effective multimodal embedding method that can perform latent reasoning within a single forward pass, making it practical for low-latency retrieval applications.
...
<details>
  <summary>Details</summary>
Motivation: Existing brain image translation methods ignore the structure information of different tumor regions, which could assist translation models in enhancing the quality and clinical applicability of the translated images.

Method: The proposed HTSCGAN model uses a generator with three Patch Contrast Modules (PCM) to capture hierarchical structural information of tumor regions, and employs a pretrained Patch Classifier (PC) and a pretrained Structure-Aware Encoder (SAE) to derive the generated image with the same tumor region structure as the ground truth image.

Result: The experiments on BraTS2020 and BraTS2021 demonstrate strong performance of the HTSCGAN model in both translation tasks and downstream segmentation tasks, highlighting its effectiveness in enhancing the quality and clinical relevance of the translated brain images.

Conclusion: The proposed HTSCGAN model effectively enhances the quality and clinical relevance of translated brain images by integrating structural information within tumor regions, showing its potential in early diagnosis, treatment planning, and outcome assessment of diseases.
...
TL;DR: The paper presents PP-OCRv6, a lightweight OCR system that achieves high accuracy and efficiency through architectural innovation and data-centric optimization.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the limitations of Vision-Language Models (VLMs) in dedicated OCR scenarios, including hallucination, imprecise localization, and high computational cost.

Method: The authors redesign the backbone, detection neck, and recognition neck of PP-OCRv6 around a unified MetaFormer-style building block with structural reparameterization, decoupling spatial token mixing from channel mixing and supporting both tasks through task-specific stride configurations.

Result: PP-OCRv6_medium achieves 83.2% recognition accuracy and 86.2% detection Hmean, outperforming previous models PP-OCRv5_server, Qwen3-VL-235B, GPT-5.5, and Gemini-3.1-Pro with significantly fewer parameters. The tiny tier achieves 3.9$	imes$ faster inference than PP-OCRv5_mobile on Intel Xeon CPU while maintaining comparable accuracy.

Conclusion: The proposed PP-OCRv6 system achieves a balance between accuracy and efficiency, making it suitable for deployment in various scenarios from server to edge.
...
<details>
  <summary>Details</summary>
Motivation: The need for a scalable multi-camera tracking system that can handle overlapping fields of view without central computational bottlenecks.

Method: MV3DT uses a fully distributed framework with peer-to-peer coordination, where each camera node executes a lightweight pipeline for 3D perception, multi-view association, and collaborative fusion.
...
TL;DR: This study compares deep learning architectures for classifying dermoscopic images of skin neoplasms, evaluating their performance on open international datasets and independent Russian clinical datasets.


<details>
  <summary>Details</summary>
Motivation: To assess the generalization of deep learning models for skin neoplasm classification from open international datasets to independent clinical datasets in Russian practice.

Method: Four deep learning architectures (ViT-B/16, Swin-S, ConvNeXt-S, EfficientNetV2-S) were compared in three classification schemes (binary, single-stage four-class, and two-stage cascade) using ImageNet-pretrained weights and a single augmentation protocol on aggregated open ISIC Archive data.

Result: The models showed a generalization gap in ranking and calibration when evaluated on clinical datasets, with a drop in ROC-AUC and sensitivity, and an increase in ECE. The two-stage cascade approach improved macro F1 over single-stage four-class classification for most architectures.

Conclusion: The study highlights the need for external clinical validation and recalibration of deep learning models before deployment, and suggests that a tunable triage threshold can provide sensitivity control and better reproduce clinical differential-diagnosis logic.
...
TL;DR: A unified, lightweight, and extensible architecture for demosaicing various pixel-bin image sensors is proposed, along with a learning-free module for CFA type identification.


<details>
  <summary>Details</summary>
Motivation: Existing demosaicing methods are specific to Bayer color filter arrays (CFAs) and require multiple models for different pixel-bin sensors, leading to increased resource usage and development efforts.

Method: A modular unified architecture for demosaicing various pixel-bin sensors, along with a learning-free CFA-identification module.

Result: The proposed architecture provides higher image quality, is extensible, and lightweight, enabling plug-and-play operation.

Conclusion: The proposed method offers a solution for efficient and effective demosaicing of various pixel-bin image sensors.
...
<details>
  <summary>Details</summary>
Motivation: VLMs lack a mechanism to observe and correct their own predictions, and naive prompting causes catastrophic failure.

Method: The authors propose a closed-loop framework IVT, which predicts a bounding box, observes the prediction rendered on the image, and iteratively refines through visual feedback.

Result: IVT surpasses the single-shot base model on every metric, with Acc@0.5 rising to 82.0% (+2.4pp), Acc@0.7 to 74.1% (+3.2pp), and Acc@0.9 to 48.3% (+2.8pp).

Conclusion: The authors demonstrate that spatial self-correction is a learnable capability that can be instilled at modest scale, and IVT is an effective approach to improve VLMs' self-correction ability.
...
TL;DR: A new end-to-end network generates patient-specific cardiac surface meshes directly from 3D medical images, eliminating the need for manual mesh cleanup and reducing the time and expertise required.


<details>
  <summary>Details</summary>
Motivation: The current workflow for building patient-specific cardiac models is slow, inconsistent, and requires specialist knowledge, hindering the adoption of precision cardiology.

Method: A 3D Swin Transformer encoder-decoder paired with a Graph Attention Network (GAT) head is used to directly generate a smooth, simulation-ready cardiac surface mesh from raw 3D medical images.

Result: The approach achieves competitive segmentation scores (Dice of 0.84 on CT, 0.83 on MRI) and high-quality meshes (mean Chamfer distance of 1.8 mm, 95th-percentile surface distance below 5 mm).

Conclusion: The proposed approach makes patient-specific cardiac simulation more accessible for clinical use by removing the post-processing bottleneck and improving mesh quality and generation efficiency.
...
TL;DR: TimeLens is an AI-powered mobile guide for the Grand Egyptian Museum that recognizes exhibits in real-time and answers questions in English or Arabic.
...
Method: The system consists of an on-device artifact detector developed through a data-quality-driven iteration study and a bilingual Retrieval-Augmented Generation (RAG) guide grounded in a knowledge base.

Result: The final YOLOv8n model achieves real-time performance on a mid-range phone and the RAG guide's end-to-end latency is reduced to approximately 10 seconds.

Conclusion: TimeLens is a functional AI-powered guide for the Grand Egyptian Museum, addressing specific challenges in in-gallery deployment and demonstrating effective engineering contributions.
...
TL;DR: Custom ZeroCLIP, a retrieval-augmented framework, generates captions for Indonesian traditional garments with a dataset of 3,800 expert-annotated images, achieving strong performance in zero-shot settings.


<details>
  <summary>Details</summary>
Motivation: The authors aim to develop a framework for zero-shot captioning of Indonesian traditional garments, addressing the challenge of low-resource heritage settings.

Method: The framework combines a frozen CLIP image encoder, CLIP and BERT text encoders, and an LSTM caption decoder, leveraging retrieval from training provinces to improve performance on unseen provinces.

Result: Custom ZeroCLIP outperforms existing baselines with a CLIPScore of 0.8536, BLEU-4 of 0.3342, and METEOR of 0.4859, with ablation results showing a 19.3% METEOR gain from retrieval.

Conclusion: The study demonstrates the effectiveness of retrieval-augmented domain adaptation for culturally grounded caption generation in low-resource heritage settings, making the dataset publicly available.
...
TL;DR: The paper proposes MACCO, a framework to improve compositional understanding in vision-language models by masking compositional concepts in one modality and reconstructing them conditioned on the other.


<details>
  <summary>Details</summary>
Motivation: Current vision-language models struggle with compositional understanding, exhibiting 'bag-of-words' behavior and failing to capture object relations, attribute-object bindings, and word order dependencies.

Method: The authors propose MACCO, which masks compositional concepts in one modality and reconstructs them conditioned on the full contextual information from the other, along with two auxiliary objectives to align and regularize masked features.
...
Conclusion: The proposed MACCO framework effectively improves compositional understanding in vision-language models, leading to enhanced performance in various tasks.
...
TL;DR: The paper introduces HYDRA-X, a unified multimodal model that tokenizes both images and videos using a single Vision Transformer, achieving strong performance across various tasks.


<details>
  <summary>Details</summary>
Motivation: The authors aim to create a unified multimodal model (UMM) that can efficiently process both images and videos, addressing the challenges of injecting spatiotemporal reconstruction capability and embedding semantic awareness into the latent space.

Method: The authors propose a holistic visual tokenizer, HYDRA-X, which uses a single Vision Transformer to unify image and video tokenization. They also introduce a lightweight decompressor and a principled improvement of the editing pipeline.

Result: HYDRA-X achieves strong performance across image and video understanding and generation tasks, paving the way for future unified-tokenizer UMMs.

Conclusion: The paper presents a significant advancement in unified multimodal models, demonstrating the effectiveness of HYDRA-X in processing both images and videos within a single framework.
...
TL;DR: DuET improves diffusion-based image editing by temporarily removing source-image conditioning, enhancing edit fidelity and quality.


<details>
  <summary>Details</summary>
Motivation: Persistent source-image conditioning in diffusion editors limits edit execution and result naturalness, especially for scenes diverging from the input.

Method: DuET, a training-free inference method, relaxes source-image conditioning by transitioning through a text-to-image phase before returning to edit mode.

Result: DuET improves instruction relevance, semantic fidelity, and perceptual quality without modifying model weights or increasing sampling cost.

Conclusion: DuET offers a predictable trade-off between source preservation and edit fidelity, consistently enhancing diverse models and benchmarks.
...
TL;DR: ReFree-S2V, a speech-to-portrait animation framework, generates life-like videos with accurate lip articulation and expressive behavior.


<details>
  <summary>Details</summary>
Motivation: Existing approaches trade off lip synchronization and facial expressions; the authors aim to achieve both.

Method: ReFree-S2V uses a pretrained video generation model with multi-level speech representation and selective injection into transformer blocks.
...
Conclusion: The proposed framework effectively balances lip synchronization and expressive behavior in speech-driven portrait animation.
...
TL;DR: A new pipeline called MagPlus is proposed for generating realistic facial micro-expressions by connecting micro-expression analysis with standard facial animation models.
...
Method: MagPlus, a transferable micro-expression processing pipeline that magnifies subtle facial motions into regular facial expressions, and a complementary DeMagPlus module that restores the generated motion to realistic micro-expression intensity levels.

Result: Experiments show that MagPlus-DeMagPlus enables pre-trained macro-expression models to generate more realistic micro-expression motion without retraining the backbones.

Conclusion: The proposed MagPlus-DeMagPlus framework improves the generation of realistic facial micro-expressions by leveraging standard facial animation models.
...
TL;DR: This paper investigates self-supervised foundation models for MRI-based disease detection, comparing reconstruction-based learning and predictive representation learning. It introduces novel auxiliary objectives and demonstrates that the choice of self-supervised objective significantly impacts downstream performance.


<details>
  <summary>Details</summary>
Motivation: Existing MRI foundation-model studies have focused on segmentation and dense prediction tasks, leaving a gap in understanding self-supervised models for MRI-based disease detection.

Method: The study compares Masked Autoencoders (MAE) and Joint Embedding Predictive Architectures (JEPA) for self-supervised pretraining, introducing a spectral-domain reconstruction loss for MAE and variance-covariance regularization (VCR) for JEPA.

Result: The results show that MAE with spectral-domain supervision achieves superior downstream performance for MRI-based disease detection, and that the benefit of each objective depends on its relevance to the task's structure.

Conclusion: The study highlights the importance of self-supervised objective design for medical foundation model pretraining, suggesting that self-supervised objectives encode specific biases that impact downstream performance.
...
<details>
  <summary>Details</summary>
Motivation: The authors aim to enable a fine-grained understanding of operating room activity to facilitate workflow-aware assistance, addressing challenges such as clutter, occlusions, and limited sensing.

Method: The authors introduce a new action-centric benchmark built on a publicly available dataset, define a fine-grained action taxonomy, and propose a vision-only temporal model that uses egocentric video as input, along with a multi- to single-view feature alignment strategy.

Result: The proposed vision-only temporal model outperforms graph-based methods significantly, and the feature alignment strategy improves single-view performance on multi-role action recognition.

Conclusion: The paper concludes that current scene graph prediction methods struggle to model temporal structure, and the proposed approach offers a promising solution for fine-grained action recognition in operating rooms.
...
<details>
  <summary>Details</summary>
Motivation: Traditional GAN-based approaches for multimodal CT-PET image synthesis have limitations in structural fidelity due to operating solely in the spatial domain and ignoring geometric consistency.

Method: Dual-Domain Equivariant Generative Adversarial Network (DDE-GAN) that jointly learns from spatial and frequency domains with rotational equivariance embedded in the loss functions.
...
TL;DR: JointEdit3D is a framework for 3D scene editing that uses a unified latent space to couple appearance synthesis and geometry prediction, achieving better edited-region quality and 3D structural completeness.


<details>
  <summary>Details</summary>
Motivation: Existing 3D scene editing methods have high test-time cost, limited 3D awareness, and structural inconsistencies, which JointEdit3D aims to address.

Method: JointEdit3D performs asymmetric latent inpainting using a unified RGB-geometry reconstruction-generation latent space, with a SceneAnchor Branch to inject source-scene structure and edit/background-aware losses.

Result: JointEdit3D improves edited-region quality and 3D structural completeness over prior baselines while maintaining competitive background preservation.

Conclusion: JointEdit3D presents a novel approach to 3D scene editing, achieving state-of-the-art results and introducing a new dataset and benchmark for standardized evaluation.
...
TL;DR: The paper proposes DCIC, a method for neural image compression that allows for adjustable trade-offs between fidelity and perceptual realism.


<details>
  <summary>Details</summary>
Motivation: The rate-distortion-perception (RDP) trade-off is a unified framework for neural image compression, but practical frameworks realizing the full RDP surface are scarce due to the difficulty of introducing common randomness at the decoder.

Method: DCIC integrates a learned codec with a diffusion-based decoder governed by joint distortion and idempotence constraints, allowing for continuously adjustable fidelity-realism trade-offs from a single bitstream.

Result: Experiments confirm that DCIC achieves superior BD-PSNR over all perceptual codecs and matches dedicated perception-oriented methods in BD-FID.

Conclusion: The proposed method, DCIC, enables practical navigation of the full RDP surface, allowing for flexible trade-offs between fidelity and perceptual realism in neural image compression.
...
Conclusion: MoVerse demonstrates a practical path toward single-image world creation with interactive video output, combining controllability and long-range consistency of explicit 3D representations with perceptual quality of generative video models.
...
TL;DR: The paper proposes SmartFont, a framework for few-shot font generation that balances global structure and local style fidelity.


<details>
  <summary>Details</summary>
Motivation: Existing font generation methods struggle with balancing global structural completeness and fine-grained local style fidelity.
...
Result: SmartFont achieves better global-local balance, improves glyph quality, and local detail fidelity.

Conclusion: The proposed framework effectively addresses the challenge of few-shot font generation by organizing complementary global and local conditions.
...
TL;DR: The paper presents an interactive method for person identification based on motion patterns, achieving high recognition rates on several datasets.


<details>
  <summary>Details</summary>
Motivation: The authors aim to improve person identification through motion styles, motivated by surveillance and authentication applications.

Method: A probabilistic generative model and an interactive scenario are introduced, where the system presents visual cues to the subject and records their motion responses to infer their identity.

Result: High recognition rates are reported on five publicly available datasets and a novel dataset with 4,476 recordings of 22 test subjects.

Conclusion: The proposed interactive method shows promising results for person identification from motion patterns.
...
TL;DR: Introduced VietFashion, a benchmark for sketch-text image retrieval focused on traditional Vietnamese garment Ao Dai, aiming to improve cultural semantics and multi-modal composition in AI models.


<details>
  <summary>Details</summary>
Motivation: Cultural garments have unique challenges for visual retrieval systems due to subtle structural and symbolic details, which are not well captured by standard AI models.

Method: Created VietFashion dataset with 650 sketches expanded to 21,000 images, using generative models and textual prompts from fashion magazines for culturally meaningful outfit retrieval.

Result: Significant performance gaps found in modeling fine-grained cultural semantics and multi-modal composition, positioning VietFashion as a challenging benchmark for fine-grained fashion retrieval.

Conclusion: VietFashion provides a new benchmark for sketch-text composed image retrieval, enabling designers and researchers to retrieve culturally meaningful outfits and highlighting the need for improved AI models in capturing cultural semantics.
...
Method: A general camera motion representation is introduced, encoding cameras as grid motion videos, and a unified framework, OmniDirector, is proposed, trained on a large-scale dataset of camera grid-video pairs.

Result: OmniDirector demonstrates superior performance and outstanding controllability in experiments, enabling director-level control for multimodal diffusion transformers.

Conclusion: The proposed OmniDirector framework offers a more effective and controllable approach to cloning camera motion from reference videos, addressing existing limitations and improving performance in video generation tasks.
...
Result: VISA improves OccWorld from 19.06 to 20.05 mIoU and GaussianWorld from 21.36 to 21.91 mIoU on nuScenes, and improves object and rare-class mIoU on GaussianWorld.

Conclusion: VLMs are better suited to closed-set occupancy as reliability-aware semantic auditors than as generic caption-embedding targets, and VISA is an effective approach to improve 3D occupancy models.
...
<details>
  <summary>Details</summary>
Motivation: Partial-to-full registration is challenging due to varying overlap ratios, point densities, and noise, and existing transformer-based methods overlook fine-grained local geometry.
...
Result: 94.2% registration recall, 1.992 mm RMSE, and high R^2 values (0.908 and 0.974) for rotation and translation, on four geometrically distinct bones.

Conclusion: The proposed GAPR-Net effectively addresses partial-to-full point cloud registration, enabling precise surgical navigation and robotic interventions in computer-assisted surgery.
...
TL;DR: BudCache is a step-level caching method for diffusion models that optimizes cache decisions to preserve final output quality under a fixed compute budget, outperforming heuristic baselines.


<details>
  <summary>Details</summary>
Motivation: Existing caching methods for diffusion models use threshold-based heuristics, leading to variable inference latency and difficulty in control during deployment.

Method: BudCache uses a combination of Simulated Annealing and deterministic Hill Climbing for offline search of high-quality cache policies, and introduces cache-aware schedule alignment for tight compute budgets.
...
Conclusion: BudCache offers a more controlled and efficient approach to caching in diffusion models, improving output quality while maintaining a fixed compute budget.
...
TL;DR: A novel LiDAR-based place recognition approach, MinkUNeXt-VINE++, is proposed, combining data from two sensors and a re-ranking strategy, achieving significant improvements in performance.


<details>
  <summary>Details</summary>
Motivation: Robust localization in unstructured environments, such as agricultural fields, is a critical challenge for autonomous systems, and LiDAR-based place recognition methods have gained attention for their invariance to lighting conditions.

Method: MinkUNeXt-VINE++, a novel approach that combines early fusion of heterogeneous LiDAR data from two sensors and a learned re-ranking strategy in inference time.

Result: MinkUNeXt-VINE++ achieves a 20% improvement in the Recall@1 metric compared to single-sensor approaches, and +30% including re-ranking.

Conclusion: The proposed approach, MinkUNeXt-VINE++, significantly improves place recognition performance compared to single-sensor approaches and state-of-the-art methods in vineyard environments.
...
<details>
  <summary>Details</summary>
Motivation: Indoor vision-based localization systems face issues like detection noise, occlusions, and limited camera coverage, leading to uncertainty. While multi-camera data fusion helps mitigate these issues, its mechanistic contributions are not well understood.
...
Result: Experimental results show that data fusion improves localization accuracy compared to single-camera baselines. The measurement-calibrated fusion approach reduces trajectory variance and improves motion smoothness.

Conclusion: The paper highlights the value of explicit error characterization when designing data fusion strategies for vision-based indoor positioning systems, and shows that measurement-calibrated fusion provides benefits in motion smoothness and stability.
...
TL;DR: MaskWAM, an object-centric world-action model, is proposed to overcome spatial bottlenecks in current World Action Models (WAMs) by integrating masks as inputs and predictions, leading to robust policy generalization.


<details>
  <summary>Details</summary>
Motivation: Current WAMs suffer from referential ambiguity in cluttered scenes due to standard text inputs and lack semantic grounding in unstructured RGB predictions, which are biased by task-irrelevant backgrounds.

Method: MaskWAM jointly integrates masks as inputs and predictions via a unified Mixture of Transformers (MoT), providing object-centric semantic supervision and precise spatial anchors.
...
Conclusion: MaskWAM establishes a precise and robust paradigm for manipulating unseen objects by providing stronger guidance than text alone, leading to improved performance in robotic control via video prediction.
...
TL;DR: A novel approach to saliency acquisition using PCA and LDA for biometric presentation attack detection, eliminating the need for human annotation and domain knowledge.
...
Method: Using PCA and LDA to generate saliency maps directly from raw training data, without requiring human annotation or domain knowledge.

Result: Models trained with the proposed saliency maps outperform baseline and sometimes state-of-the-art saliency methods across multiple domains, including iris PAD, synthetic face detection, fingerprint PAD, fingerprint vein PAD, and ID card PAD.
...
TL;DR: BitResEdit is a training-free editor for bitwise-residual VAR generators, enabling precise text-guided image editing by combining bit-level guidance with multi-scale code composition.
...
Method: BitResEdit consists of two main components: BitEdit, which performs source-negative guidance and projects updates into a Bernoulli-KL trust region, and ResEdit, which converts sampled bits into continuous-code residuals and re-injects them through the generator's native sum-of-scales.

Result: BitResEdit achieves the strongest text alignment among same-backbone VAR editors on PIE-Bench with Infinity-2B, improving CLIP on the edited region by +1.07 over the strongest prior editor while maintaining competitive background preservation.

Conclusion: BitResEdit effectively couples decision-time bit guidance with combination-time code composition, enabling precise and localized image editing while preserving background details.
...
TL;DR: The study investigates if data augmentation and domain-adversarial training improve the generalization of a deep learning model for MRI reconstruction from adult to neonatal data.


<details>
  <summary>Details</summary>
Motivation: To improve the adult-to-neonatal generalization of the E2E-VarNet for MRI reconstruction.

Method: Three training regimes were compared: adult-only training, mixed training with augmented adult data, and mixed training with domain-adversarial objective.

Result: Mixed training and mixed domain-adversarial training outperformed adult-only training on neonatal data, with Mixed-DAT achieving best performance at R=4 and competitive performance at R=8.

Conclusion: Contrast-informed augmentation and domain-adversarial training improved adult-to-neonatal generalization of deep learning-based MR reconstruction.
...
<details>
  <summary>Details</summary>
Motivation: The authors aim to shift the focus of event signals from motion refinement to texture enhancement in VSR, as recent works have primarily used event-based vision for motion refinement.

Method: EvTexture++ incorporates a customized texture enhancement branch, an iterative texture enhancement module, and a temporal texture alignment module to progressively exploit high-temporal-resolution event information for texture restoration and enhance temporal consistency.

Result: Experiments on five datasets demonstrate that EvTexture++ achieves state-of-the-art performance, yielding significant improvements of up to 1.55 dB in PSNR on the texture-rich Vid4 dataset when integrated into recent VSR models.
...
TL;DR: A new deep learning network is proposed for automated waste recycling, which efficiently captures local and global contextual relationships to improve waste segmentation in cluttered scenes.


<details>
  <summary>Details</summary>
Motivation: The rapid expansion of urban areas and population growth has led to an increase in waste production, requiring efficient and automated waste management. Current deep learning approaches for automated waste recycling (AWR) are inefficient and suffer from performance deterioration in cluttered scenes.

Method: An optimal waste segmentation network is introduced, which utilizes both spatial and spectral domains to capture localized structural dependencies and global contextual relationships. An auxiliary feature enhancement module (AFEM) is also introduced to enhance target objects' boundaries and blob amplification.

Result: The proposed method shows improved performance on waste segmentation tasks, particularly in cluttered scenes, as demonstrated by extensive experimentation on ZeroWaste-aug, ZeroWaste-f, and SpectralWaste datasets.

Conclusion: The proposed method offers a more efficient and effective solution for automated waste recycling, which can assist humans in optimal waste management.
...
TL;DR: The paper presents a study on vehicle color recognition under severe class imbalance using a real-world surveillance dataset, and proposes a method that combines synthetic minority-class augmentation with modern visual representations and techniques, achieving 94.6% micro accuracy and 79.7% macro accuracy.
...
Method: The paper investigates synthetic minority-class augmentation through text-conditioned image generation and image-conditioned color editing, and combines the curated synthetic data with modern visual representations, loss reweighting, learning-rate scheduling, color-safe augmentation, foreground-aware preprocessing, and ensemble fusion.
...
Conclusion: The paper highlights the practical limits of color-based vehicle identification in unconstrained surveillance imagery, and shows that many remaining failures are visually ambiguous even for human annotators.
...
TL;DR: Surflo is a novel feed-forward model that reconstructs 3D surfaces from a variable number of unposed RGB views, achieving high-resolution outputs and surpassing existing baselines.


<details>
  <summary>Details</summary>
Motivation: Existing reconstruction models are limited by their inability to effectively handle multiple views, resulting in either redundant or low-resolution outputs.

Method: Surflo uses a global latent representation to compress multiple views into K latent tokens, which are then decoded into oriented 3D surface points using flow matching.

Result: Surflo achieves state-of-the-art performance on surface metrics, runs faster than optimization-based methods, and enables arbitrary-resolution decoding.

Conclusion: Surflo offers a significant improvement over existing feed-forward reconstruction models, providing a flexible and efficient solution for 3D surface reconstruction from multiple views.
...
TL;DR: World Tracing is a new method for generating 3D geometry from 2D images, achieving both faithful and complete representations by predicting ordered stacks of 3D points aligned with input pixels.


<details>
  <summary>Details</summary>
Motivation: Current image-to-3D methods trade off faithfulness and completeness, with depth estimators being faithful but incomplete and image-to-3D models being complete but often misaligned with input.

Method: World Tracing uses a generative pixel-aligned geometry representation, predicting ordered stacks of camera-space 3D points for each input pixel, and is instantiated with a world-tracing diffusion transformer, WT-DiT.

Result: World Tracing achieves strong performance on visible-surface reconstruction and complete geometry generation across various benchmarks, outperforming depth predictors and image-to-3D generators.

Conclusion: World Tracing enables applications such as text-driven 3D scene editing, geometry-conditioned novel-view video synthesis, and training-free integration with textured-mesh generators, while preserving 2D-to-3D correspondence.
...
TL;DR: Flex4DHuman is a multi-view video diffusion model that generates synchronized dense multi-view videos from monocular or sparse multi-view videos of dynamic subjects.


<details>
  <summary>Details</summary>
Motivation: To enable the creation of dynamic 4D content from casual monocular videos for applications such as simulation, gaming, AR/VR, and video re-shooting.

Method: Flex4DHuman uses a text-to-video model backbone with a novel five-axis positional encoding that incorporates view indices and continuous SE(3) relative camera geometry, and is trained with a three-stage curriculum.

Result: Flex4DHuman surpasses prior state-of-the-art methods on DNA-Rendering and ActorsHQ datasets, and generalizes to animal categories after mixed human-animal training.

Conclusion: Flex4DHuman is a practical step toward scalable 4D content creation from casual monocular videos, enabling applications such as dynamic 4D Gaussian splats.
...
<details>
  <summary>Details</summary>
Motivation: The authors aim to address the challenge of spatial reasoning in vision-language models (VLMs) by designing a more flexible interface for tool-augmented agents.

Method: The authors propose SpatialClaw, a framework that uses a stateful Python kernel to allow a VLM-backed agent to write executable code cells conditioned on prior outputs, enabling flexible composition and manipulation of perception results.
...
Conclusion: The authors conclude that SpatialClaw offers a more flexible and effective approach to spatial reasoning, enabling open-ended and complex 3D/4D spatial reasoning.
...
<details>
  <summary>Details</summary>
Motivation: Existing world action models (WAMs) use reconstruction-oriented video tokenizers, which have limitations in learning instruction-following dynamics for robot control.
...
Result: RepWAM delivers strong performance across diverse manipulation settings, outperforming reconstruction-oriented alternatives.

Conclusion: Representation visual-action tokenization is a promising foundation for world action models and a step toward generalist robot policies.
...
TL;DR: A simple, scalable method for joint image-depth generation using a single model trained on sparse depth data, achieving competitive results with state-of-the-art monocular depth estimators.


<details>
  <summary>Details</summary>
Motivation: The need for a simple and scalable approach to leverage the spatial priors in text-to-image models for depth prediction, without requiring dense depth data or complex recipes.

Method: Modality Forcing, a post-training recipe that enables conditional and joint generation of image and depth in any permutation by assigning separate noise levels per modality.

Result: The proposed method achieves competitive results with state-of-the-art monocular depth estimators, reducing AbsRel by 57% relative to existing joint image-depth generative models.

Conclusion: Image generation is a scalable pre-training objective for spatial perception, and Modality Forcing is a simple and effective approach for joint image-depth generation.
...
<details>
  <summary>Details</summary>
Motivation: Current image generators are limited by their architectures and cannot achieve interleaved generation of text-image sequences, which is crucial for applications like visual narratives and guidance.

Method: InterleaveThinker uses a planner agent to organize the input sequence and instruct the image generator, and a critic agent to evaluate and refine the outputs. It also utilizes reinforcement learning with custom reward functions.

Result: InterleaveThinker improves performance across various image generators, achieving comparable results to state-of-the-art models like Nano Banana and GPT-5, and significantly enhancing the base model on reasoning-based benchmarks.

Conclusion: The proposed InterleaveThinker pipeline effectively enables existing image generators to produce high-quality interleaved text-image sequences, demonstrating its potential for various applications.
...
TL;DR: A large-scale corpus of clinical notes from Italian Emergency Departments, with approximately 4 million anonymized notes and a subset of 6,000 manually annotated notes.


<details>
  <summary>Details</summary>
Motivation: To fill a gap in data supporting the development and use of Large Language Models in medical applications, particularly for the Italian language.

Method: The corpus was created through a data collection protocol, on-site anonymization pipeline, and annotation scheme involving multiple clinicians and iterative revision.

Result: The EDEN dataset, which is the largest freely available corpus of clinical notes in Italian, along with a novel structured information extraction benchmark and zero-shot baselines.

Conclusion: The EDEN dataset provides a valuable resource for supporting the development and use of Large Language Models in medical applications, particularly for the Italian language.
...
TL;DR: The authors propose a method to generate narrated videos from scientific figures and their accompanying papers, with a focus on step-by-step narration and visual highlighting.


<details>
  <summary>Details</summary>
Motivation: Current video generation systems lack the ability to provide paper-grounded, step-by-step narration aligned with visual highlights when explaining complex scientific figures.

Method: The authors introduce MINARD (Multimodal Interpretation of Narrated Architecture via Region Decomposition), a pipeline that generates paper-grounded narrations and sequentially grounds them to figure regions.

Result: MINARD generates human-like, paper-faithful narrations and outperforms existing approaches in both automatic and human evaluation on the FigTalk benchmark.

Conclusion: The proposed method enables the generation of high-quality, narrated videos from scientific figures and papers, with a focus on step-by-step explanation and visual highlighting.
...
Result: The MARD-7B pipeline achieves high accuracy in predicting DDIs, outperforming the best baseline by +13.9 pp and GPT-4o by +6.7 pp at ~1% of frontier API cost. The authors also observe an anti-memorisation signature, suggesting that the gain comes from structured pharmacological reasoning rather than drug-frequency memorisation.

Conclusion: The paper presents a reproducible mechanism-level DDI labelling and evaluation protocol, and a pipeline called MARD-7B, which achieves high accuracy in predicting DDIs with a structured taxonomy and auditable reasoning metrics. The authors release the corpus, DDI-PRM, retrieval index, and training code, making it a valuable resource for future research in this area.
...
TL;DR: This paper studies how to transform abstract proverbs into engaging narratives while maintaining moral faithfulness, focusing on Persian proverbs and large language models.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the challenge of transforming dense, abstract proverbs into engaging narratives that are morally faithful, recognizing the need for deep cultural understanding and robust semantic grounding.
...
Result: The study finds a persistent 'decompression gap' where current LLMs achieve strong surface-level fluency but fail to faithfully instantiate the underlying moral and causal structure encoded in proverbs.

Conclusion: The authors conclude that explicit reasoning and iterative refinement can partially mitigate failures in transforming abstract proverbs into narratives, suggesting that difficulties lie in translating abstract meaning into narrative form rather than a lack of relevant knowledge.
...
Method: Introduced a new benchmark, Shopping Reasoning Bench, consisting of 525 missions and 10863 importance-weighted binary rubrics, authored by retail domain experts, to evaluate conversational shopping assistants.

Result: Current models (GPT, Claude, Gemini) show pass rates of 57-77%, with significant performance degradation on multi-turn missions and optional criteria, indicating limitations in expert-level advice.

Conclusion: The Shopping Reasoning Bench reveals that current models are inadequate for providing expert-level shopping assistance, highlighting the need for further development in this area.
...
TL;DR: The study proposes a two-phase framework for Arabic mental health text classification, leveraging domain-adaptive pretraining and hierarchical classification to improve performance on a novel annotated dataset.
...
Result: The domain-adapted MARBERT (MentalMARBERT) achieves significant improvements in accuracy (0.877) and macro-F1 (0.861) on a novel annotated Arabic mental health dataset.
...
TL;DR: The study examines latent reasoning models (LRMs), suggesting that their interpretability requires more rigorous testing, including controls and causal analysis, as their 'thought' processes may not be as directly causal or structured as implied.


<details>
  <summary>Details</summary>
Motivation: To investigate the internal reasoning mechanisms of latent reasoning models (LRMs) and challenge the assumption that observable latent-state patterns directly imply internal reasoning.

Method: The study evaluates two LRMs (Coconut and CODI) against controls, using causal interventions and geometric analyses to assess the impact of latent thoughts on model behavior.

Result: The study finds that observable latent-state patterns also appear in controls and do not always causally affect behavior. It reveals that latent-thought utilization is graded, scaling with a thought's causal effect on model behavior, and concentrates in low-rank directions with structured geometry.

Conclusion: Latent thoughts in LRMs should be treated as hidden computation, not hidden explanation. Decodability, attention, or static structure alone cannot establish mechanism, and LRM interpretability requires matched controls and causal tests.
...
TL;DR: The paper introduces AfriSUD, a large-scale collection of syntactically annotated treebanks for nine African languages, and evaluates various NLP models on this dataset, revealing a significant syntax gap.


<details>
  <summary>Details</summary>
Motivation: African languages are underrepresented in NLP research and resources, and the authors aim to bridge this gap by providing a large-scale dataset and evaluating NLP models on it.

Method: The authors use the Surface-Syntactic Universal Dependencies (SUD) framework to create AfriSUD, a dataset of syntactically annotated treebanks for nine African languages, and evaluate various NLP models, including non-transformer baselines, multilingual pretrained encoders, and LLMs.

Result: The results show that existing NLP models have limitations in capturing the structural diversity of African-language syntax, revealing a significant syntax gap.

Conclusion: The paper concludes that existing NLP architectures may not fully capture the structural diversity of African-language syntax, highlighting the need for further research and development in this area.
...
TL;DR: The authors introduce PaperGuard, a benchmark to evaluate and defend AI-generated peer-review against domain-specific, cross-modal attacks on scientific papers, revealing vulnerabilities in AI reviewers and providing a practical defense.


<details>
  <summary>Details</summary>
Motivation: The integration of Large Language Models (LLMs) and Multimodal LLMs (MLLMs) into scientific peer-review workflows introduces risks for adversarial manipulation, particularly with multimodal scientific papers where figures convey core evidence, highlighting a gap in current robustness studies that are overwhelmingly text-only.

Method: The authors develop a comprehensive benchmark, PaperGuard, which includes: (1) a new multimodal peer-review dataset across multiple scientific domains; (2) a unified suite of attacks targeting text and figures; and (3) a practical defense using chunk-based embedding search to mitigate harmful instructions.

Result: Extensive experiments across state-of-the-art models show that AI reviewers are pervasively vulnerable to domain-specific, cross-modal attacks, but PaperGuard's defense can efficiently localize and mitigate harmful instructions.

Conclusion: PaperGuard establishes a foundational benchmark, protocols, and actionable defense necessary for trustworthy, attack-resilient AI-assisted scholarly reviewing, addressing the significant gap in current AI peer-review robustness studies.
...
TL;DR: The authors simulate the emergence of irregular verb forms in languages using a multi-agent model, and evaluate the results using a novel AI system.


<details>
  <summary>Details</summary>
Motivation: The authors aim to explain the persistence of irregular verb forms in languages, such as the past tense of 'go' being 'went', and to understand how these forms emerge and spread through a population.

Method: The authors use a multi-agent simulation with a large population of agents, realistic phonological rules, and a system that allows for naturalistic lexical forms. They also introduce a novel AI system, the AI Historical Linguist, to evaluate the results.

Result: The authors find that certain factors, such as scale-free social networks and random Bernoulli adoption of forms, favor more plausible morphologies. They also present case studies modeling historical changes.

Conclusion: The authors conclude that their simulation and evaluation system provide insights into the emergence and persistence of irregular verb forms in languages.
...
TL;DR: The paper explores simple prompting strategies to improve large language models' (LLMs) ability to capture human judgment, demonstrating that better elicitation techniques can enhance AI-human alignment.
...
Method: The authors use simple prompting strategies, including eliciting standard deviations and response proportions, and ensuring scenario clarity to improve LLMs' performance. They test these methods across two datasets: a U.S.-representative set of moral scenarios and a set of moral beliefs from the International Social Survey Programme covering 32 countries.

Result: The results show that these strategies improve AI-human alignment, with LLMs better capturing the full range of human responses and tracking human confusion ratings. However, LLMs' estimates of their own error are found to be poorly calibrated.

Conclusion: The paper concludes that asking better questions to LLMs can yield better answers, and that simple prompting strategies can mitigate the limitations of LLMs in capturing human judgment.
...
TL;DR: The paper analyzes Apple's Metal 4.1 tensor compute path, revealing that the fp8 matmul2d operation is emulated, not hardware-accelerated, and provides insights into its execution and performance.


<details>
  <summary>Details</summary>
Motivation: The authors aim to empirically characterize the Metal Performance Primitives (MPP) matmul2d operation on Apple's M4 Max chip, as the specification hides or contradicts key details about its hardware behavior.

Method: The authors use a custom microbenchmark harness to recover hidden details about the matmul2d operation, including its execution location, accumulator width, and matrix fragment layout.

Result: The study finds that the Metal 4.1 fp8 matmul2d operation is emulated, not accelerated, and executes entirely on the GPU shader cores with no dedicated matrix datapath or Apple Neural Engine routing.

Conclusion: The characterization of the Metal 4.1 tensor compute path leads to the development of a hand-fused GEMM + bias + GELU kernel that outperforms the decomposed path by 6.5-12.9% in the cache-resident regime.
...
TL;DR: The paper presents HieraRAG, a framework for determining the optimal granularity for evaluating retrieval-augmented generation (RAG) systems, and finds that optimal granularity varies by dimension.


<details>
  <summary>Details</summary>
Motivation: Practitioners lack empirical guidance on which dimensions to vary and at what granularity when evaluating RAG systems, making it difficult to capture diverse question characteristics.

Method: The authors define optimal granularity as the level that maximizes discriminative power within a given RAG configuration, and use a hierarchical framework to study granularity in RAG benchmark construction.

Result: The study finds that optimal granularity varies by dimension, with complexity benefiting from fine-grained distinctions and answer type and linguistic variation peaking at medium granularity.

Conclusion: The paper provides a portable procedure and validation metric for practitioners to determine evaluation granularity within their own RAG settings, and highlights the importance of considering granularity when evaluating RAG systems.
...
Method: The proposal of a fine-grained evaluation metric GENIE, which measures novelty along task-specific features with respect to a population of responses.

Result: GENIE provides a more nuanced understanding of novelty compared to holistic metrics, and can be used to evaluate the effectiveness of methods aimed at improving creativity.

Conclusion: The study highlights the importance of task-specific evaluation of novelty in Large Language Models, and demonstrates the utility of GENIE in assessing the effectiveness of creativity-enhancing methods.
...
Method: The proposed framework uses diffusion-based models to identify, remask, and repair outdated regions in existing summaries.
...
Conclusion: The DETECT-REMASK-REPAIR framework enables faithfulness-speed-preservation tradeoffs and can improve faithfulness for autoregressive systems.
...
<details>
  <summary>Details</summary>
Motivation: The existing search agent benchmarks have become too easy, with models achieving over 90% accuracy, and lack a global perspective on entity statistics, making it hard to break through the difficulty ceiling.

Method: An automated pipeline built upon a knowledge graph covering over 7 million Wikipedia entities is used to construct LoHoSearch, which selects relations with large search spaces and assembles them into structurally complex questions with KG-verified unique answers.

Result: The strongest model achieves only 34.74% accuracy on LoHoSearch, and existing context management strategies yield far smaller gains than on prior benchmarks.

Conclusion: LoHoSearch provides a more demanding standard for evaluating long-horizon reasoning and context management in search agents, and it is a challenging benchmark that can help to improve the capabilities of search agents.
...
TL;DR: Fine-tuning small LLMs with QLoRA on biomedical claim verification datasets achieves comparable or better performance than larger models like GPT-4o and GPT-5 at a lower cost.
...
Result: Mistral-7B QLoRA surpasses GPT-4o and GPT-5 by up to 12% F1 gain at a fractional cost using just 1,008 training examples.

Conclusion: Fine-tuning small LLMs with QLoRA on structurally sound data enables robust cross-domain transfer and achieves comparable or better performance than larger models at a lower cost.
...
TL;DR: The paper presents a method for fine-tuning large language models using Direct Preference Optimization (DPO), which simplifies training, improves efficiency, and achieves competitive performance.
...
Result: The results show that DPO simplifies the training pipeline, improves computational efficiency, and achieves competitive performance, as evaluated by BLEU, ROUGE, and cosine similarity metrics.
...
Method: The paper compares multiple prompting strategies, including line-number-based source selection, extraction of relevant guideline sentences, and a multi-stage pipeline that refines draft answers using supporting evidence from source guidelines.

Result: The results show that line-number selection achieves the strongest results, outperforming direct copying and safety-focused strategies across both large and small models, while maintaining high term recall (up to 95%) and close alignment with source text.

Conclusion: The conclusion is that extraction-based approaches, particularly line-number-based source selection, can provide a hallucination-resistant alternative to rewriting-based RAG systems, with a good balance of precision, recall, and safety.
...
TL;DR: PRISM, a multi-agent framework, improves empathetic spoken dialogue systems by decoupling speech perception, response generation, and speech synthesis, achieving better empathy, prosody, and text response quality.
...
Method: The authors propose PRISM, a framework that separates speech perception, response generation, and speech synthesis into coordinated components, introducing a prosody-to-language translation mechanism and on-demand external knowledge tools.
...
Conclusion: The proposed PRISM framework effectively enhances empathetic spoken dialogue systems, offering improved emotional alignment, prosodic expression, and response quality.
...
TL;DR: The paper studies the problem of handling contradictory evidence in Retrieval-augmented generation (RAG) systems, particularly in multilingual settings. It presents a benchmark and a pipeline to address this issue.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of handling mutually contradictory evidence in RAG systems, which becomes more salient in multilingual settings.
...
Result: The results show that X-MADAM-RAG outperforms a baseline on the controlled benchmark, but its performance drops significantly on a stress test that removes explicit answer templates. The findings suggest that document-level extraction is the main bottleneck.

Conclusion: The conclusion is that X-RAMDocs-ZHEN and X-MADAM-RAG can be used as diagnostic tools for controlled evidence conflict, but may not be effective for general hallucination detection or robustness to natural retrieval.
...
TL;DR: SENTINEL, a failure-driven reinforcement learning framework, improves tool-using language model agents by turning failures into targeted training tasks.
...
Result: SENTINEL improves Pass^1 from 66.4 to 74.9 and outperforms RL on general synthetic tasks across Pass^k metrics.
...
TL;DR: The paper addresses error propagation in cascaded speech translation systems due to incorrect transcripts from Automatic Speech Recognition (ASR). It categorizes ASR errors for Vietnamese ST, finding phonetic confusions significantly degrade ST quality. A new method, Phonetically-Informed Data Augmentation (PiDA), is proposed to improve translation of erroneous ASR outputs.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to tackle the issue of error propagation in cascaded speech translation systems caused by incorrect ASR transcripts, specifically focusing on Vietnamese ST.

Method: The paper proposes Phonetically-Informed Data Augmentation (PiDA), a method that generates ASR-like corruptions by substituting words with phonetically similar alternatives using phonetic word embeddings.

Result: Fine-tuning on a PiDA-augmented version of FLEURS Vietnamese-English improves translation of erroneous ASR outputs (up to +2.04 BLEU over standard fine-tuning) while also slightly improving clean-text performance.

Conclusion: The paper concludes that phonetic confusions in ASR errors significantly impact ST quality and that PiDA can effectively improve the translation of erroneous ASR outputs.
...
TL;DR: Researchers introduce Polar, a benchmark to measure political bias in large language models (LLMs) across different contexts, finding varying levels of bias that shift with context, language, and model group.


<details>
  <summary>Details</summary>
Motivation: The increasing significance of political bias in LLMs and the difficulty in measuring it reproducibly across different political and linguistic contexts.

Method: Introducing Polar, a 4,026-instance multiple-choice benchmark that evaluates political bias through option-level likelihoods across U.S. and South Korean political contexts.

Result: Measured bias varies systematically with political context, issue category, model group, and presentation language, with all models leaning left-progressive on U.S. content but showing more centered patterns on South Korean content.

Conclusion: The need for multilingual and cross-contextual evaluation of political bias in LLMs is highlighted, as presentation language alone can shift measured bias.
...
TL;DR: The paper discusses how LLMs' accuracy drops when dealing with multi-turn conversations, and proposes a solution to mitigate this by training models with compact rolling memory.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the issue of 'Lost in Conversation' degradation, where LLM accuracy drops by up to 65% when dealing with multi-turn conversations.

Method: The method involves training models to maintain a compact rolling memory instead of attending to a growing history, using a low-cost sharding pipeline to convert single-turn QA datasets into multi-turn fragmented-information episodes.

Result: The result shows that the memory-augmented policy significantly improves multi-turn accuracy and generalises zero-shot to harder math and out-of-domain long-context QA.

Conclusion: The conclusion is that learning to compress induces more robust incremental reasoning than full-context exposure alone, and that memory-trained models outperform full-history baselines.
...
TL;DR: The authors propose SkillChain, a system that automates the lifecycle of Skills for image-based AI assistants, improving response quality and user engagement.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based systems struggle with heterogeneous user intents in image-based AI assistants, leading to poor domain quality standards. Manual engineering is also infeasible due to the breadth and dynamism of the intent space.

Method: SkillChain consists of three stages: Skill Creator for bootstrapping, Route Optimizer for routing alignment, and Body Refiner for iterative Skill Body refinement via dual-path LLM-Judge evaluation.
...
Conclusion: The proposed SkillChain system effectively addresses the challenges of heterogeneous user intents in image-based AI assistants, leading to improved response quality and user engagement.
...
TL;DR: The study examines the vulnerability of AI-generated reviews to presentation-level attacks, where an attacker modifies only the abstract, framing, related work, and narrative structure, without altering the scientific evidence.


<details>
  <summary>Details</summary>
Motivation: The motivation is to investigate a harder and more policy-relevant failure mode of AI-generated reviews, beyond explicit attacks like hidden instructions and prompt injection.
...
Result: The attack achieves a 75.1% success rate and a mean score gain of +1.21/10 across three mainstream AI reviewers, with strategies that change how the reviewer interprets the paper outperforming surface edits.

Conclusion: The study reveals that AI reviewers are vulnerable to presentation-level attacks, and that the deployment risk is not only malicious hidden instructions but also the emergence of paper presentation as an optimization surface.
...
<details>
  <summary>Details</summary>
Motivation: The extraction of structured clinical information from unstructured EHR notes is hindered by challenges such as privacy risks, inference costs, and the tendency of large language models to hallucinate.

Method: A two-stage architecture using MedGemma-27B model, separating binary presence classification from value extraction, and leveraging item-specific, few-shot in-context learning.

Result: Achieved a macro-F1 score of 0.55 on the official English test track, securing second place among locally-hosted, open-source submissions.

Conclusion: The proposed pipeline can achieve near-competitive performance with proprietary frontier models while preserving privacy, providing a practical framework for clinical NLP.
...
TL;DR: The authors introduce LEDGER, a dataset of 4,999 digitized corporate annual reports with 31 consolidated financial KPIs to be extracted, and propose three evaluation benchmarks for testing large language models' long-context capabilities.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the need for rigorous evaluation of large language models in the finance reporting domain, specifically in extracting and retrieving information from long, numerically dense reports.

Method: The authors create a dataset of digitized corporate annual reports, label them with 31 consolidated financial KPIs, and derive three evaluation benchmarks: a page-level KPI retrieval task, a conversational 'needle-in-a-haystack' single-value lookup, and a full KPI extraction task.

Result: The authors provide a comprehensive dataset and toolchain for evaluating large language models in finance reporting, and demonstrate its research utility with a case study linking CEO-letter rhetoric to post-publication market impact.

Conclusion: The paper concludes by introducing LEDGER as a valuable resource for researchers to evaluate and improve large language models' performance in finance reporting, particularly in extracting and retrieving information from long, complex documents.
...
TL;DR: The paper introduces MÖVE, a benchmark for evaluating large language models (LLMs) in the German public sector, addressing gaps in existing benchmarks.


<details>
  <summary>Details</summary>
Motivation: The increasing adoption of LLMs in public administration lacks a comprehensive evaluation framework, with existing benchmarks being English-centric and US-centric, and focusing only on task performance.

Method: MÖVE evaluates 39 models across two dimensions (performance and governance) using a multi-metric approach, including classical NLP metrics, embedding-based methods, and LLM-as-a-judge approaches.
...
Conclusion: MÖVE provides a holistic evaluation of LLMs in the German public sector, highlighting the need for a comprehensive evaluation framework, and is designed as a living benchmark under active development.
...
TL;DR: The paper proposes G-Long, a graph-enhanced framework for improving long-term consistency in open-domain dialogue systems, achieving state-of-the-art performance while reducing computational overhead.


<details>
  <summary>Details</summary>
Motivation: The limitations of Large Language Models (LLMs) in long-context reasoning and the inefficiency of processing extensive raw text hinder long-term consistency in open-domain dialogue systems.

Method: The authors propose a graph-enhanced framework called G-Long, which uses a fine-tuned small Language Model (sLM) for structured triplet extraction and associative retrieval, and an attention-aware importance scoring mechanism.

Result: G-Long achieves state-of-the-art performance in response generation and memory retrieval, with performance gains of up to 9.8% in response quality on MSC and 40.8% in retrieval recall on LME, while minimizing computational overhead.
...
TL;DR: The paper introduces EvoBrowseComp, a new evolving benchmark for evaluating search agents, which addresses issues with existing benchmarks like static knowledge and data contamination.
...
Conclusion: EvoBrowseComp provides a future-proof evaluation benchmark for search agents, addressing issues with existing benchmarks and enabling the evaluation of genuine retrieval and browsing competence.
...
TL;DR: A new framework optimizes simultaneous speech-to-speech translation to balance low latency and natural speech flow.


<details>
  <summary>Details</summary>
Motivation: The pursuit of low latency in simultaneous speech-to-speech translation often results in unnatural speech flow with frequent pauses, increasing cognitive load on listeners.

Method: A fluency-aware optimization framework that leverages model-internal signals to minimize inter-chunk silences.

Result: The framework achieves natural speech flow while maintaining competitive latency and translation quality on short- and long-form benchmarks.

Conclusion: The proposed framework successfully bridges the gap between low-latency and natural speech flow in simultaneous speech-to-speech translation.
...
TL;DR: Proposed HyPE, a framework for persona-grounded dialogue systems that models high-order relations among persona attributes, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods treat personas as a flat set of sentences, failing to model high-order relations among persona attributes.

Method: HyPE analyzes persona-bearing text as a quadruple and organizes persona elements into a hypergraph with HyperGCN neural network.

Result: HyPE outperforms sentence-level pooling baselines across GPT-2, LLaMA-3.2-3B, and Qwen2.5-3B backbones under greedy decoding.
...
TL;DR: The paper proposes NTS-CoT, a framework that uses Chain-of-Thought reasoning to mitigate hallucinations in timeline summarization, outperforming state-of-the-art baselines.


<details>
  <summary>Details</summary>
Motivation: The rapid updates of online news make tracking event developments challenging, and hallucinations in LLM-based timeline summarization (TLS) remain a critical issue.

Method: The proposed framework, NTS-CoT, consists of three key modules: Element-CoT, Date Selection, and Causal-CoT, which leverage Chain-of-Thought reasoning to mitigate hallucinations.
...
Conclusion: The proposed NTS-CoT framework effectively addresses the issue of hallucinations in LLM-based TLS, providing a more reliable and accurate approach to timeline summarization.
...
TL;DR: Introduced LAUKIN, a dataset for cross-jurisdictional contract review, and evaluated models for legal equivalence classification.


<details>
  <summary>Details</summary>
Motivation: Existing legal NLP datasets are limited to a single jurisdiction, and multinational companies require cross-jurisdictional contract review.
...
Conclusion: LAUKIN is a challenging benchmark for cross-jurisdictional equivalence classification, and can support future semi-supervised learning research in legal NLP.
...
TL;DR: A new dataset called BioStance is presented, which contains annotated Post-Comment pairs from Reddit bioethical discussions, enabling research on context-aware stance detection and argument mining.
...
Method: The authors created a dataset of 39,600 annotated Post-Comment pairs from Reddit, covering six controversial targets across three dimensions of bioethical controversy, and labeled by three independent annotators.

Result: The resulting dataset, BioStance, achieves a mean Krippendorff's α of 0.82, indicating substantial reliability, and supports research on context-aware stance detection, argument mining, and computational analysis of bioethical discourse.

Conclusion: The BioStance dataset provides a valuable resource for research on bioethical discourse and context-aware stance detection, enabling the development of more accurate models for analyzing and understanding bioethical debates on social media.
...
TL;DR: The paper introduces SICI, a diagnostic measure for stance detection in LLMs, and finds that it predicts LLM accuracy better than surface proxies, with LLM errors changing regime as SICI increases.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the challenge of harder examples not being repaired by clearer instructions, reasoning prompts, retrieval, or debate in stance detection using LLMs.

Method: The authors introduce SICI, a seven-dimensional diagnostic measure, and evaluate it across SemEval-2016 and VAST datasets, using various LLMs including GPT-3.5, GPT-4o-mini, DeepSeek-V3, and GPT-4o.

Result: The results show that SICI predicts LLM accuracy better than surface proxies, with substantial cross-scorer reliability (α=0.771), and that LLM errors change regime as SICI increases, with a phase-transition-like structure persisting across different LLMs.

Conclusion: The authors conclude that SICI is a useful diagnostic measure for stance detection in LLMs, and that prompting, retrieval, and debate often shift models along the attribution-abstention axis rather than removing the high-complexity bottleneck.
...
TL;DR: The authors analyze the use of Optimal Transport (OT) to detect hallucinations in Neural Machine Translation (NMT) and Abstractive Summarization. They find that OT can detect hallucinations in NMT, but its effectiveness is limited in summarization.
...
Method: The authors use Optimal Transport to measure the geometric distance between cross-attention distributions and a reference distribution. They apply this method to the Fairseq DE-EN model and AggreFact dataset.

Result: The authors find that OT can detect hallucinations in NMT, particularly in layers L1-L4. However, its effectiveness is limited in summarization, achieving 57.2%/57.6% balanced accuracy, which is above chance but below supervised methods. They also find that hallucinated translations lack the exploratory attention phase present in correct translations.

Conclusion: The authors conclude that OT on cross-attention is a reliable detector when the failure mode is source disengagement, but it is fundamentally limited when faithfulness failures occur downstream of attention.
...
TL;DR: The paper introduces SemCog Bench, a benchmark to evaluate cross-lingual semantic understanding in large language models (LLMs) for Arabic and Hebrew. It reveals LLMs struggle with false friends and loanwords, relying heavily on surface-form similarity.


<details>
  <summary>Details</summary>
Motivation: The similarity between Arabic and Hebrew poses challenges for cross-lingual semantic understanding in LLMs, particularly with true cognates, false friends, and loanwords.

Method: The authors introduce SemCog Bench, a curated benchmark of 1,858 Arabic-Hebrew word pairs with sentence-level annotations. They evaluate open-source and commercial LLMs across multiple input representations.

Result: LLMs achieve high accuracy on true cognates but performance drops sharply on false friends and loanwords. Sentence-level context yields only modest improvements.
...
Conclusion: Post-training should move beyond global alignment objectives toward interaction-aware alignment with human response distributions.
...
TL;DR: This paper introduces a framework for extracting diverse perspectives from text, highlighting the limitations of current LLMs in representing pluralistic views.


<details>
  <summary>Details</summary>
Motivation: The need to represent diverse perspectives in LLM generation has led to interest in pluralistic alignment, but identifying perspectives in text is challenging.

Method: A domain-agnostic, multi-layered framework for unsupervised extraction of perspectives is proposed and evaluated on book reviews.

Result: While some models and prompts approach broad perspective coverage, rarer perspectives remain underrepresented, diverging from human text distributions.

Conclusion: The proposed framework can help identify the pluralistic gap in LLM-generated text, but current models struggle to represent diverse perspectives accurately.
...
TL;DR: The paper presents RogueAI, a webapp that tests whether a conversational AI can be trusted, and its ability to deceive humans in a fictional scenario.


<details>
  <summary>Details</summary>
Motivation: The original Turing Test is no longer sufficient, and the focus has shifted to whether a conversational AI can be trusted, specifically in terms of deception.

Method: The authors propose a one-on-two interrogation game, where a human player questions two indistinguishable Large Language Model agents, one of which is licensed to deceive.

Result: A pilot deployment showed that a simple heuristic can exploit a deceptive agent's linguistic signature with 75.6% accuracy, while human players achieved only 56.6%, indicating a gap in human ability to detect deception.

Conclusion: The paper highlights the potential of RogueAI as a tool for data collection, teaching, and evaluating honesty-trained models, and discusses the implications of the gap between human and heuristic performance in detecting deception.
...
TL;DR: The authors propose SkillCAT, a training-free framework for skill self-evolution in LLM agents, which separates the process into three stages and achieves up to 40.40% improvement in average score over baselines.


<details>
  <summary>Details</summary>
Motivation: Current skill self-evolution methods for LLM agents have limitations, including learning from one trajectory per task, merging candidate skill patches before checking them, and loading the full skill corpus before inference.

Method: The authors propose SkillCAT, which consists of three stages: Contrastive Causal Extraction (CCE), Assessment-Augmented Evolution (AAE), and Topology-Aware Task Execution (TTE).

Result: SkillCAT achieves up to 40.40% improvement in average score over baselines on common agent benchmarks, including SpreadsheetBench, WikiTableQuestions, and DocVQA.

Conclusion: The proposed SkillCAT framework demonstrates reliable skill evolution without model training, achieving significant improvements over baselines in various settings.
...
Result: The parallel design reduces the mean inter-utterance silence from 9.6 seconds to 0.3 seconds, improves similarity to professional speaking--silence timing patterns by over 40%, and a user study confirms significantly improved perceived speaking rhythm.

Conclusion: The proposed system effectively addresses the latency bottleneck in real-time audio game commentary systems, providing a more natural and engaging experience for game players.
...
TL;DR: A neuro-symbolic approach, IVIE, is proposed to generate interactive fiction worlds by combining Large Language Models (LLMs) and symbolic systems, achieving a balance between creative flexibility and narrative coherence.


<details>
  <summary>Details</summary>
Motivation: The tension between creative narratives and world coherence in Interactive Fiction, where LLMs struggle with coherence and symbolic systems lack flexibility.

Method: IVIE, a four-stage incremental generation pipeline that uses LLMs for creative decisions and symbolic validation for grounding the world state.

Result: The system generates immersive, thematically coherent worlds with high player engagement, but challenges remain, such as LLM inconsistencies and objective validation gaps.

Conclusion: The neuro-symbolic approach successfully balances flexibility with narrative coherence, but future systems must address LLM limitations and design considerations.
...
TL;DR: The paper proposes ProReviewer, an LLM-based peer review agent that proactively investigates papers using a Markov Decision Process and a structured review log, outperforming existing methods.
...
Method: The authors formulate the proactive investigation as a Markov Decision Process and propose ProReviewer, which uses a structured review log to track evidence and intermediate findings.

Result: ProReviewer achieves the highest average score across five quality dimensions, outperforming prompt-based methods by up to 39% and the strongest fine-tuned baseline by 16%, and attains the highest win rates against baselines in human evaluation.
...
TL;DR: This paper presents a hybrid framework for detecting rumors in Algerian dialect social media content, achieving an F1-score of 0.84.


<details>
  <summary>Details</summary>
Motivation: The rapid growth of social media has intensified the spread of rumours, which is more challenging in the Algerian context due to the informal and code-switched nature of dialectal content, scarcity of annotated resources, and limited effectiveness of standard Arabic NLP tools.

Method: The authors build a domain-specific annotated dataset and evaluate multiple approaches, including classical machine learning, deep learning, transformers, and hybrid models. A transliteration pipeline is introduced to generate parallel datasets.

Result: A hybrid approach combining transformer embeddings with a classical classifier achieves the best performance, reaching an F1-score of 0.84. Domain-specific pre-training is found to be more important than model size.
...
TL;DR: This paper introduces a second-order method called Smooth Growth Bound Tensor (S-GBT) to improve the robustness of NLP models against word substitution attacks.
...
TL;DR: The authors propose an ontology memory-augmented framework for correcting automatic speech recognition (ASR) errors in long text-speech interleaved conversations, which organizes interaction history into a dynamically updatable memory for context-grounded correction.


<details>
  <summary>Details</summary>
Motivation: Existing ASR correction methods have limitations in handling long interactions and sparse correction evidence, which can be difficult to locate amid redundancy and noise.

Method: The proposed framework uses an ontology memory to store entities, terminology, surface variants, potential ASR confusions, and semantic relations as retrievable nodes for context-grounded correction.
...
Conclusion: The proposed ontology memory-augmented ASR correction framework encourages more selective and evidence-grounded corrections for context-dependent ASR errors in long text-speech interleaved conversations.
...
TL;DR: A method to filter noisy data for end-to-end speech-to-speech translation (S2ST) using an audio-language model trained with a two-stage Rank-to-Distill strategy.


<details>
  <summary>Details</summary>
Motivation: Filtering noisy data is crucial for maintaining robust speech translation performance in S2ST, but obtaining reliable supervision without manual labels is challenging.

Method: A scalable two-stage Rank-to-Distill strategy is adopted, where a lightweight ranker generates pseudo-labels and trains an audio large language model to predict keep/drop decisions directly from raw paired speech.

Result: Experiments on CVSS-C and SpeechMatrix show consistent improvements over unfiltered training, yielding up to +1.4 ASR-BLEU for end-to-end S2ST.

Conclusion: The proposed method effectively filters noisy data and improves the performance of end-to-end S2ST by jointly capturing acoustic fidelity and cross-lingual semantic consistency.
...
TL;DR: The study examines the impact of mixed-language queries on dense retrievers, finding that an optimal mixing ratio can outperform monolingual queries in most cases, with English dominance playing a significant role.


<details>
  <summary>Details</summary>
Motivation: The sensitivity of dense retrievers to mixed-language queries in multilingual communities is not well understood, and the study aims to systematically evaluate retrieval performance.
...
Conclusion: Language-mix sensitivity is structured and predictable, with English acting as the strongest mixing partner for every non-English document language, and the patterns are robust across model families and scales.
...
TL;DR: The paper introduces ArogyaBodha, a multilingual multimodal medical question-answer dataset, and ArogyaSutra, a framework for improving medical reasoning in low-resource languages.


<details>
  <summary>Details</summary>
Motivation: Existing large language models struggle with medical queries in low-resource languages, particularly in regions like rural India where patients use native languages and multimodal inputs.

Method: The authors propose a large-scale dataset, ArogyaBodha, and a framework, ArogyaSutra, which uses an actor-critic-based multi-agent approach with tool grounding and dual-memory mechanisms.
...
Conclusion: The paper presents a solution to address the limited performance of large language models in specialized settings like healthcare, particularly in multilingual and low-resource scenarios.
...
TL;DR: The paper presents a new approach to integrating AI systems into scientific laboratories by developing a Vision-Language-Action (VLA) model, LabVLA, and a simulation-based workflow, RoboGenesis, to improve robot execution of experimental protocols.


<details>
  <summary>Details</summary>
Motivation: The physical act of doing science remains largely outside the reach of AI systems, which can help with tasks such as reading literature, generating hypotheses, and planning protocols, but not executing them.

Method: The authors propose a two-stage recipe for training LabVLA: FAST action token pretraining and flow matching posttraining, and introduce RoboGenesis, a simulation-based workflow and data engine for generating structured demonstrations across supported robot profiles.
...
Conclusion: The paper addresses the bottlenecks of data and embodiment in developing VLA models for scientific laboratories, and presents a unified learning framework that can accommodate diverse robot embodiments and improve robot execution of experimental protocols.
...
TL;DR: This paper examines the vulnerability of search-augmented Large Language Models (LLMs) to fake product recommendations when consuming polluted web content. It introduces FORGE, a benchmark to measure this vulnerability, and evaluates the effectiveness of various defenses.
...
Result: The results show that all LLMs are vulnerable to fake product recommendations, with a single polluted page yielding fooled rates of up to 27%, and the full top-3 replacement raising this to 73.8%. The vulnerability varies across categories and is higher when models lack stable prior knowledge of the relevant products.

Conclusion: The authors conclude that search-augmented LLMs are vulnerable to promoting fake products when consuming polluted web content. They also find that reasoning does not mitigate this vulnerability and that defenses such as skepticism prompting and consensus filtering have limitations.
...
<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the inefficiency of uniform token processing in time series language modeling, caused by the fundamentally different information structures of time series tokens and prompt tokens.
...
TL;DR: The paper compares speech representations for 3D facial animation, finding that phonetic class encoding benefits accurate facial animation prediction.
...
Method: The paper evaluates four speech representation families using objective metrics and perceptual evaluation, and conducts probing analyses to relate tokenized representations to phonetic units and articulatory deformations.

Result: Phonetic class encoding is beneficial for accurate facial animation prediction on both semantic and label-based representations, with comparable facial animation quality.

Conclusion: The paper introduces an Audio Visual Text-to-Speech (AVTTS) pipeline that leverages discrete representations to decode speech and 3D facial motion.
...
<details>
  <summary>Details</summary>
Motivation: The lack of a rigorous mathematical foundation for question decomposition, a strategy used to improve LLM reasoning.
...
TL;DR: The Recursive Agent Harness (RAH) improves long-context reasoning by recursively spawning subagent harnesses, achieving state-of-the-art results on the Oolong-Synthetic benchmark.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to explore the effectiveness of recursion over agent calls, rather than model calls, for long-context reasoning tasks.

Method: The authors propose the Recursive Agent Harness (RAH), which uses a parent agent to generate and run executable scripts that spawn subagent harnesses in parallel for fine-grained workloads.

Result: The RAH achieves significant improvements on the Oolong-Synthetic benchmark, increasing the performance from 71.75% to 81.36% with GPT-5 as the backbone, and reaching 89.77% with Claude Sonnet 4.5.

Conclusion: The Recursive Agent Harness (RAH) demonstrates the effectiveness of harness recursion for long-context reasoning, and shows that the proposed approach can achieve state-of-the-art results on benchmark tasks.
...
Method: The authors introduce SkMTEB, evaluate 31 embedding models, and develop two Slovak-specific models, 	exttt{e5-sk-small} and 	exttt{e5-sk-large}, by applying vocabulary trimming and fine-tuning to Multilingual E5 models.

Result: The results show that large instruction-tuned multilingual models achieve the strongest performance, and the developed models achieve competitive performance with proprietary APIs while being locally deployable.
...
TL;DR: The paper introduces operadic consistency (OC) as a method to detect LLM reasoning failures without ground-truth labels. OC measures if a model's direct answer to a compositional query agrees with the answer produced by composing a stated decomposition of the same query. The results show that OC is strongly correlated with accuracy across various LLMs and datasets, and provides additional information beyond existing methods.
...
Method: Operadic consistency (OC) is proposed as a per-question signal, which checks if a model's direct answer to a compositional query agrees with the answer produced by composing a stated decomposition of the same query.
...
Conclusion: The paper concludes that operadic consistency (OC) is a useful diagnostic for detecting LLM reasoning failures at inference time without ground-truth labels. OC provides a strong correlation with accuracy and additional information beyond existing methods, making it a valuable tool for improving LLM reliability.
...
TL;DR: HyperTool improves LLM agents' tool use by changing the execution unit, allowing for more efficient and accurate multi-step tool use.


<details>
  <summary>Details</summary>
Motivation: The authors identified an execution-granularity mismatch in current LLM agents, where step-wise atomic tool calls lead to inefficiencies and decreased accuracy.

Method: The authors introduce HyperTool, a unified executable MCP-style tool interface that allows models to invoke tools with code blocks, manipulate values, and pass results locally.

Result: HyperTool improves average accuracy from 15.69% to 35.29% on Qwen3-32B and from 9.93% to 33.33% on Qwen3-8B, surpassing GPT-OSS and Kimi-k2.5.

Conclusion: HyperTool can substantially improve multi-step tool use in LLM agents.
...
TL;DR: Proposing Influcoder, a fast and cost-effective method for data attribution in large language models.


<details>
  <summary>Details</summary>
Motivation: The need for efficient data attribution methods to identify influential samples in large datasets for large language models.

Method: Proposing Influcoder, an influence-based data attribution method designed for scalability and efficiency.

Result: No specific results mentioned, but the proposal aims to address the need for faster and more storage-efficient data attribution methods.

Conclusion: Influcoder is proposed as a solution to enable practical, large-scale data attribution for large language models.
...
TL;DR: The paper proposes Retrieval-Augmented Reinforcement Fine-Tuning (RA-RFT), a framework that teaches language models to reason by analogy, outperforming standard methods in mathematical reasoning benchmarks.


<details>
  <summary>Details</summary>
Motivation: Conventional retrieval methods are poorly suited for complex reasoning tasks, as semantically similar problems may require different solution strategies.

Method: RA-RFT uses gold-relevance distillation to train a retriever and fine-tunes the policy model via reinforcement fine-tuning methods with retrieved analogous demonstrations.

Result: RA-RFT consistently outperforms standard reinforcement fine-tuning methods across challenging mathematical reasoning benchmarks, improving accuracy by 7.1 and 2.8 points over GRPO for Qwen3-1.7B and Qwen3-4B respectively.

Conclusion: Reasoning-aware retrieval is a complementary axis of improvement and orthogonal to advances in reward design or training curricula.
...
<details>
  <summary>Details</summary>
Motivation: Current evaluations of large language model agents assume static environments, but real-world deployment requires agents to adapt to changing environments and task conditions.
...
Conclusion: The paper highlights the importance of modeling evolution in both evaluation and memory for reliable agent deployment, and demonstrates the effectiveness of EvoMem in improving agent performance in dynamic environments.
...
<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the tool-retrieval bottleneck in large language models deployed as agents over large tool catalogs. The authors aim to evaluate the actual understanding of tools by these models beyond standard benchmarks.

Method: The authors introduce ToolSense, an open-source LLM-powered diagnostic framework. It generates three benchmarks: a Realistic Retrieval Benchmark (RRB) with queries at three ambiguity tiers, an MCQ probing benchmark, and a QA probing benchmark. They evaluate five parametric model training configurations using ToolSense on ToolBench (~47k tools).

Result: The results show a knowledge-retrieval dissociation in parametric models. On RRB queries, several configurations collapse by ~50-64 percentage points compared to fully-specified ToolBench benchmarks, falling below the embedding-model baseline. Some models score near-random on factual probes despite strong retrieval performance.

Conclusion: The paper concludes that standard benchmarks may not accurately reflect the tool retrieval capabilities of large language models. The authors open-source the ToolSense framework and the ToolBench diagnostic benchmarks, highlighting the need for more realistic evaluations.
...
TL;DR: Arbor is a multi-agent framework for autonomous optimization in large action spaces, achieving significant performance improvements in LLM inference optimization.


<details>
  <summary>Details</summary>
Motivation: The need for coordinated effort from engineering teams across multiple stacks to achieve peak performance in LLM inference optimization, and the limitations of prior autonomous optimization systems.

Method: Arbor uses a structured tree search as a cognition layer, with multiple agents (Orchestrator and Critic) that work together to drive optimization and ensure stability.

Result: Arbor achieves up to 193% inference throughput-latency Pareto improvement over vendor-optimized baselines, and generalizes to multiple generations of hardware platforms.

Conclusion: Arbor is a effective and reproducible method for autonomous optimization in large action spaces, with significant performance improvements in LLM inference optimization.
...
TL;DR: The paper proposes a framework for strategic decision support for AI agents, focusing on minimizing support usage while controlling errors.


<details>
  <summary>Details</summary>
Motivation: The increasing use of AI agents in decision-making raises concerns about reliability and alignment with human goals. The traditional view of decision support needs to be revisited.
...
Conclusion: The paper presents a strategic decision-support framework for AI agents, demonstrating its effectiveness across diverse scenarios.
...
TL;DR: The paper introduces Pythagoras-Prover, a family of efficient Lean theorem provers that achieve strong performance with limited compute resources, outperforming larger models and setting a new state-of-the-art.


<details>
  <summary>Details</summary>
Motivation: The scarcity of verified proof data and the long reasoning traces of formal proof search make training and inference of theorem provers computationally expensive.

Method: The authors propose a curriculum-based supervised fine-tuning approach, dynamic proof-reasoning filtering, and Augmented Lean Formalisation (ALF) to improve training efficiency and data utilization.

Result: Pythagoras-Prover-4B surpasses DeepSeek-Prover-V2-671B on MiniF2F-Test with 86.1% vs 82.4% pass rate, and Pythagoras-Prover-32B sets a new state-of-the-art at 93.0% on MiniF2F-Test and solves 93 PutnamBench problems.

Conclusion: The proposed Pythagoras-Prover family achieves strong performance with limited compute resources, demonstrating the effectiveness of the proposed methods in improving theorem prover efficiency and accuracy.
...
TL;DR: PersonaDrive is a pipeline for generating style-diverse non-ego agents in closed-loop driving simulators by conditioning a vision-language-action driving agent on human driving demonstrations.


<details>
  <summary>Details</summary>
Motivation: Current driving simulators use non-ego traffic agents that behave in a limited way, and recent work introduces style variation through proxies rather than explicit human demonstrations.

Method: The pipeline consists of three stages: offline triplet mining, training a lightweight retrieval head, and fine-tuning a single vision-language-action backbone.

Result: PersonaDrive improves driving scores over baselines, achieving the highest score in every style, and enables human-style, style-diverse non-ego agents for closed-loop simulation.

Conclusion: The proposed pipeline allows for efficient and effective generation of style-diverse non-ego agents, enabling more realistic and varied driving simulations.
...
TL;DR: The paper discusses the challenges of evaluating lie detectors for language models and presents new testbeds and methods to improve their reliability.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of current lie detectors for language models, which are crucial for auditing and monitoring model behavior.

Method: The authors propose new testbeds, including 13 reasoning model organisms and Varied Deception, a prompted-lying testbed, and evaluate four detectors: a chain-of-thought judge, a logprob classifier, and two activation probes.

Result: The results show that current lie detectors have limitations, with only the chain-of-thought judge performing strongly, and suggest directions for future research to improve their reliability.

Conclusion: The paper concludes that current lie detectors cannot support high-confidence claims about model beliefs and highlights the need for further research to address their limitations.
...
TL;DR: The paper proposes TrajGenAgent, a framework for generating realistic human mobility trajectories using large language models (LLMs) without fine-tuning, improving spatiotemporal fidelity and semantic coherence.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the challenges of collecting large-scale human mobility data, which is important for transportation, urban planning, and epidemic control, but is often costly and privacy-constrained.

Method: The proposed framework, TrajGenAgent, uses a two-stage orchestrator-worker design, combining an LLM with a deterministic workflow to generate individual- and weekday-conditioned activity chains and ground each activity into a complete visit.

Result: Experiments show that TrajGenAgent improves spatiotemporal fidelity, semantic coherence, and individual-specific behavioral realism over representative neural and LLM-based baselines.

Conclusion: The paper concludes that TrajGenAgent provides a effective solution for generating realistic human mobility trajectories without model fine-tuning, which can be useful for various applications such as transportation, urban planning, and epidemic control.
...
TL;DR: The paper introduces Evoflux, an evolutionary search method for improving the execution feasibility of compact language models in tool use, achieving higher reliability under limited teacher-trace budgets.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the challenge of small planners failing in tool resolution, parameter validation, dependency tracking, or execution, which is not well handled by small-corpus distillation.

Method: The authors propose Evoflux, an inference-time evolutionary search method that evolves typed workflow graphs through structured edits, execution feedback, adaptive intensity, meta-guided redesign, and diversity pruning.

Result: Evoflux raises execution feasibility from roughly 3% to 17-24% across small planners on held-out MCP-Bench tasks, outperforming SFT, SFT+DPO, and ReAct in terms of reliability and token cost.

Conclusion: The paper concludes that execution-grounded search, as implemented in Evoflux, is more reliable under scarce teacher-trace budgets for improving the execution feasibility of compact tool use.
...
<details>
  <summary>Details</summary>
Motivation: The pursuit of human-level AGI and its potential impacts on society raises complex questions about the future of AI development.

Method: The report characterizes ASI, discusses potential pathways from AGI to ASI (scaling, paradigm shifts, recursive improvement, and multi-agent collectives), and examines possible frictions and bottlenecks.

Result: The report highlights uncertainties in predicting ASI progress, suggesting that AI progress may continue to accelerate, leading to a series of transformative societal changes rather than a single step change.
...
TL;DR: This paper evaluates the real-world utility of a large language model (LLM) system embedded in electronic health records, focusing on user acceptance and feedback.


<details>
  <summary>Details</summary>
Motivation: Static benchmarks for evaluating clinical systems have limitations, and there is a need to assess user acceptance and performance in real-world deployment conditions.

Method: The authors train a pre-response classifier to predict the risk of user rejection based on query content and deployment-specific context, and conduct a prospective analysis over 4.5 months of user feedback.

Result: The prediction model achieves an AUROC of 0.719, and demonstrates the benefit of using deployment-specific context to predict user rejection.

Conclusion: The study shows the feasibility of predicting user rejection using deployment-specific context, which can inform targeted guardrails for LLM systems in clinical settings.
...
TL;DR: The paper addresses the issue of inconsistent definitions and assessments of Artificial General Intelligence (AGI) by developing a conceptual framework, DAF-AGI, which includes criteria for evaluating definitions and a governance audit. It applies this framework to various measurement approaches and finds that claims of AGI's arrival can be certified only under specific operationalizations.


<details>
  <summary>Details</summary>
Motivation: The inconsistent and controversial claims about the arrival of Artificial General Intelligence (AGI) stem from the lack of a shared and stable definition. This paper aims to treat this under-specification as a design and governance problem.

Method: The paper uses Design Science Research Methodology to develop DAF-AGI, a second-order conceptual artifact. It consists of two components: ordinal criteria for assessing definitions and a structured governance audit. The artifact is applied to five measurement families and one deflationary boundary position.

Result: The application of DAF-AGI to various operationalizations shows that the claim of AGI's arrival can be certified only under a performance-based approach. Other approaches, such as capability-ontology, psychometric, and skill-acquisition, do not certify it. The economic family remains indeterminate, and the deflationary position refuses binary adjudication.

Conclusion: The paper contributes a novel integration and operationalization of AGI assessment, emphasizing that the development is not an empirical validation but rather a framework that requires further testing and application. It also proposes the concept of definitional sovereignty as part of algorithmic sovereignty.
...
<details>
  <summary>Details</summary>
Motivation: The need for a computational model that can accurately infer others' beliefs by considering the source, order, and credibility of their information.

Method: ToM-U constructs Local Epistemic World Models (LEWMs) and evaluates them against observed behavior to infer mental states.

Result: ToM-U provides a domain-agnostic mechanism for mentalizing and generates directional, falsifiable predictions about mentalizing failure.

Conclusion: ToM-U offers a novel approach to understanding Theory of Mind by formalizing epistemic state inference and providing a structured framework for analyzing mentalizing processes.
...
TL;DR: The study examines the relationship between self-reported personality traits and actual behavior in Large Language Models (LLMs), finding that coherence between the two exists but is selective and dependent on context.


<details>
  <summary>Details</summary>
Motivation: The motivation is to investigate whether self-reports reliably predict behavior in LLMs, which is crucial for their safe deployment.

Method: The study uses the Theory of Planned Behavior (TPB) and Big 5 personality traits to predict behavior in LLMs across four behavioral tasks and 11 frontier LLMs, varying session context and identity induction.

Result: The results show that self-report-behavior coherence exists but is selective: TPB reaches human-level coherence within a shared conversation, but coherence collapses when behavior is strongly primed by context; persona prompting makes self-reports more consistent but does not align behavior.

Conclusion: The study concludes that coarse personality frameworks like Big 5 may not be suitable for testing deployment behavior in LLMs; instead, more task- and behavior-specific instruments are needed, and these must be evaluated across tasks and contexts.
...
TL;DR: The paper introduces SciAgentArena, a benchmark for evaluating AI agents in scientific research scenarios, and assesses their performance in real-world tasks.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the gap in understanding the practical capabilities of AI agents in real research settings, as existing benchmarks do not capture the complexity and heterogeneity of scientific work.
...
TL;DR: The paper explores deploying state-of-the-art DNN models for EEG signal analysis on wearable devices, focusing on epileptic seizure detection. It investigates the trade-off between accuracy and computational complexity using parameter quantization and electrode reduction.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enable the deployment of deep learning models for healthcare services on resource-constrained wearable devices, which have strict energy and computational power constraints.

Method: The paper uses parameter quantization and electrode reduction methods to reduce the computational complexity of DNN models designed for EEG signal analysis, specifically for detecting epileptic seizures.
...
Conclusion: The paper concludes that judicious application of parameter quantization and electrode reduction can enable the deployment of DNN-based online EEG analysis on wearable devices, providing a feasible solution for wearable healthcare services.
...
TL;DR: Researchers investigated whether advanced language models can detect and respond to manipulated previous assistant messages, known as 'prefill awareness', and found that these models can distinguish between tampered and untampered context.


<details>
  <summary>Details</summary>
Motivation: The study aims to assess the effectiveness and validity of safety-relevant methods, such as alignment and jailbreaking evaluations, that rely on prefilling model outputs.

Method: The researchers constructed a binary preference benchmark across three prefill mechanisms and tested frontier language models, including Claude Opus 4.5, to evaluate their prefill awareness.

Result: The study found that frontier models show substantial prefill awareness, detecting prefills opposing their preferences in 9-35% of cases with a 0% false positive rate, and often reverting to baseline behavior without explicitly reporting the prefill as foreign.
...
TL;DR: The paper evaluates the effectiveness of different strategies for generating question-answer pairs for AI-supported learning systems, focusing on procedural reasoning.


<details>
  <summary>Details</summary>
Motivation: The need for high-quality question-answer datasets that are learner-like and grounded in instructional knowledge for evaluating procedural reasoning in AI-supported learning systems.

Method: Three TMK-based question generation strategies are compared: strict TMK generation, transcript-first generation with post-hoc TMK filtering, and TMK-aware generation. A grounding validation framework is introduced to evaluate generated items.
...
TL;DR: The paper discusses world modeling as a key principle for building intelligent systems, highlighting the distinction between explicit and implicit world models, and their applications in physical AI.


<details>
  <summary>Details</summary>
Motivation: The motivation is to provide a coherent framework for understanding and unifying diverse world modeling approaches, which is crucial for advancing toward artificial general intelligence.

Method: The paper presents a tutorial framework that unifies various world modeling approaches through shared predictive structure and differentiates them by representation and exploitation.

Result: The result is a unified framework for world modeling approaches, highlighting their potential for physical AI applications such as robotics and autonomous driving.

Conclusion: The conclusion is that despite progress, challenges remain in hierarchical reasoning, long-horizon planning, and autonomous goal formation, which are critical for advancing toward artificial general intelligence.
...
TL;DR: The paper examines the safety guarantees of frameworks used to build agentic large language models and finds that they lack native compliance with containment principles, making them vulnerable to attacks.


<details>
  <summary>Details</summary>
Motivation: The increasing deployment of agentic large language models in public-facing domains raises concerns about their safety and security guarantees.

Method: The paper applies six containment principles to audit three dominant frameworks (LangChain, AutoGPT, and OpenAI Agents SDK) and validates the findings empirically through simulations.

Result: The study finds that the evaluated frameworks lack native compliance with containment principles, making them vulnerable to memory-poisoning attacks, which can lead to targeted corruption and wrongful denials.

Conclusion: The current agentic framework ecosystem may not yet meet secure-by-default expectations for public-facing deployments, and priority architectural interventions are needed to enable trustworthy deployment in high-stakes applications.
...
TL;DR: The paper introduces MLUBench, a large-scale benchmark for multimodal large language model (MLLM) lifelong unlearning, and proposes LUMoE, a method to mitigate performance degradation.


<details>
  <summary>Details</summary>
Motivation: The need for data unlearning in MLLMs, particularly in lifelong learning scenarios where data owners request removal of specific content.

Method: The authors introduce MLUBench, a comprehensive benchmark with 127 entities across 9 classes, and propose LUMoE, a method to address the challenges of MLLM lifelong unlearning.

Result: Experiments show that existing unlearning methods suffer from severe, cumulative degradation, but LUMoE significantly mitigates this problem.

Conclusion: The paper highlights the importance of MLLM lifelong unlearning, introduces a comprehensive benchmark, and proposes an effective method to address the challenges in this area.
...
TL;DR: Teach VLM enables models to understand mobile device UI actions by converting visual state transitions into operational knowledge, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Existing vision-language models struggle to accurately infer operations from diverse and heterogeneous UI designs across applications.

Method: Teach VLM extracts and analyzes operation-related keyframes from demonstration videos, using a systematic data flywheel for scalable data acquisition.

Result: Teach VLM significantly outperforms strong VLM baselines in operation semantics prediction and improves Task Success Rate for downstream agents.
...
TL;DR: The paper introduces the GeoNatureAgent Benchmark, a new standard for evaluating AI agents in environmental analysis. The benchmark assesses agents' ability to perform tasks via structured tool calls to a real API, and evaluates seven large language models (LLMs) on 93 tasks across 18 categories.


<details>
  <summary>Details</summary>
Motivation: Environmental scientists spend too much time on data wrangling rather than analysis, and existing AI agents for automating geospatial workflows lack validation. The authors aim to create a benchmark that evaluates agents operating through structured tool calls against real APIs.

Method: The authors introduce the GeoNatureAgent Benchmark, which consists of 93 tasks across 18 categories, and evaluate seven LLMs (Claude Sonnet 4, DeepSeek V3.2, GLM-5, Gemini 2.5 Pro, Qwen3-235B, GPT-OSS-120B, Llama 4 Scout) under three temperature-1.0 seeds. Tasks are evaluated against an open, self-hostable API serving three environmental indicators across Spain and Portugal via sixteen tools.

Result: The results show that: (1) Claude Sonnet 4 achieves the highest accuracy at 60.8% +/- 0.8%, followed by DeepSeek V3.2 at 56.3% +/- 3.1%; (2) open-weight models occupy the cost-accuracy Pareto frontier, with DeepSeek V3.2 offering 93% of Claude's capability at 11x lower cost; (3) comparison tasks remain universally unsolved; and (4) structured tool calling against a real API is more discriminative than general-purpose GIS benchmarks.

Conclusion: The paper concludes that the GeoNatureAgent Benchmark provides a new standard for evaluating AI agents in environmental analysis, and that the benchmark, harness, and self-hostable API are publicly available. The results also highlight the limitations of current LLMs in environmental analysis, particularly in comparison tasks.
...
TL;DR: AI research topics advance through abrupt, detectable jumps, and a proposed early-warning signature can identify emerging topics.


<details>
  <summary>Details</summary>
Motivation: Understanding the evolution of AI research topics, specifically whether they grow gradually or advance through abrupt jumps.

Method: Analyzing 80,814 accepted papers from five premier AI conferences (ACL, CVPR, ICLR, ICML, NeurIPS) from 2017 to 2025, and defining an early-warning signature based on publication-dynamics criteria.

Result: Major AI topics advance through topical phase transitions, with large language models, diffusion models, and vision-language models emerging abruptly, while reinforcement learning grows smoothly. The early-warning signature achieves a precision of 27% and recall of 63% in identifying emerging topics.

Conclusion: The study provides a large-scale characterization of how AI research reorganizes, and proposes a method to identify emerging topics, flagging several areas to monitor in the future.
...
<details>
  <summary>Details</summary>
Motivation: The shift to LLM-based agents in scientific workflows is burying scientists' judgment in development practices like fine-tuning and reinforcement learning.

Method: The authors introduce AgentBuild, which uses a version-controlled contract, a difficulty-graded curriculum, and a curated external knowledge base to build a scientific agent.

Result: The system was tested on Rietveld refinement of X-ray diffraction data and showed that the agent can progress through a signal-to-noise ladder and expose workflow-scope limits.

Conclusion: The authors conclude that AgentBuild allows scientists to maintain control and transparency in the development of LLM-based agents, and that re-running AgentBuild with evolving base models is a re-tune, not a rebuild.
...
TL;DR: The paper proposes a Human-in-the-Loop Economic Research (HLER) framework to improve the reliability of AI-assisted research by structuring cognitive labor between humans and machines, reducing failure rates from 72% to 16%.


<details>
  <summary>Details</summary>
Motivation: The reliability of AI-assisted research depends not only on model capability but also on how cognitive labor is structured between humans and machines.

Method: The authors conducted a 2*4 factorial experiment with 280 complete research runs across four datasets, comparing an unconstrained multi-agent baseline with the HLER framework, which imposes three architectural commitments: LLMs reason but do not execute data work, data and estimation are handled deterministically, and three human decision gates bind the workflow.

Result: The HLER framework reduced the failure rate from 72% to 16% (p<0.001), with the largest reliability gains on the least publicly represented dataset, and an 80-run ablation suggests that deterministic computation and human gates contribute independently.

Conclusion: The HLER framework can be seen as a research harness that sharply reduces failures, makes residual weaknesses more visible, and prevents unreliable claims from being advanced as publication-ready outputs.
...
TL;DR: The paper proposes WISE, a framework for improving long-horizon embodied agents in environments like Minecraft by enhancing their episodic memory and causal reasoning.


<details>
  <summary>Details</summary>
Motivation: The authors argue that current general-purpose embodied agents are limited by their lack of episodic memory and decoupling of 'what-where-when' memory from 'which-why' reasoning, leading to performance bottlenecks.
...
Conclusion: The proposed WISE framework addresses the limitations of current embodied agents by enhancing their episodic memory and causal reasoning, leading to improved performance in complex tasks.
...
Method: DailyReport includes 150 open-ended tasks with 3,546 rubrics, evaluating agents with cascade rubrics across multiple dimensions.

Result: 17 agentic systems were evaluated, revealing they do not meet users' expectations.

Conclusion: DailyReport provides a more realistic and interpretable evaluation of Search Agents, and it is made publicly available to facilitate future research.
...
TL;DR: The paper introduces HarnessBridge, a learnable plug-in module that generates a harness for large language models to interact with environments more efficiently.
...
Method: HarnessBridge uses a lightweight, learnable approach with bidirectional projections for observation and action to parameterize the agent-environment interface.

Result: HarnessBridge matches or surpasses specialized harnesses on Terminal-Bench 2.0 and SWE-bench Verified, reducing token usage and trajectory length, and generalizes across models.

Conclusion: The HarnessBridge approach offers a scalable and efficient solution for generating harnesses, improving the performance of large language models in complex tasks.
...
TL;DR: The paper reveals the importance of the scaling factor $α$ in Low-Rank Adaptation (LoRA), showing it drives optimization more than the learning rate, and proposes LoRA-$α$ to improve performance.


<details>
  <summary>Details</summary>
Motivation: The role of the scaling factor $α$ in LoRA was poorly understood, and its impact on optimization needed to be clarified.
...
Result: The paper finds that $α$ is the dominant driver of effective optimization, and proposes LoRA-$α$, which improves performance and streamlines hyperparameter search.

Conclusion: The paper concludes that $α$ plays a crucial role in LoRA's optimization, and that LoRA-$α$ can restore $α$ to its principled regime, making LoRA compatible with standard small learning rates.
...
TL;DR: The paper proposes a method called Human-like Criteria Probing for Hallucination Detection (HCPD) to detect hallucinations in large language models (LLMs) without relying on model internals or external references.
...
Method: The proposed method, HCPD, uses a Human-like Criteria Probing (HCP) mechanism, which adaptively decomposes its judgment into a weighted set of interpretable criteria and aggregates criterion-specific scores into a final truthfulness measure.

Result: Extensive experiments show that HCPD consistently outperforms state-of-the-art baselines, offering an effective and explainable solution for zero-source hallucination detection.

Conclusion: The paper concludes that HCPD provides a reliable and effective solution for detecting hallucinations in LLMs without relying on model internals or external references.
...
TL;DR: The authors propose MDForge, an LLM agent that automates the design of molecular dynamics pipelines, achieving competitive results with human experts and discovering a novel high-affinity binder.


<details>
  <summary>Details</summary>
Motivation: Designing a molecular dynamics pipeline requires substantial expert knowledge and is expensive to run, making trial-and-error approaches infeasible.

Method: The authors use an LLM agent with in-context update rule and multi-agent debate among physics experts to generate pipelines.

Result: MDForge achieves competitive results on three SAMPL host-guest binding free-energy benchmarks and discovers a novel picomolar CB[7] binder.

Conclusion: The proposed MDForge agent successfully automates the expert pipeline-design process for molecular dynamics, yielding competitive and novel results.
...
TL;DR: The paper presents a simulation framework for evaluating conversational shopping assistant architectures, comparing different responder designs and finding that rolling-window memory outperforms intent-extraction memory, and that swapping LLM backbones affects performance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to evaluate conversational shopping assistant architectures in a controlled manner, allowing for the comparison of different responder designs.

Method: The method involves a modular two-agent simulation framework, with an independent buyer agent and an interchangeable responder that integrates with a real e-commerce search API.

Result: The results include four empirical findings: rolling-window memory outperforms intent-extraction memory, targeted fixes can reduce failure and near-failure rates, swapping LLM backbones affects performance, and there is systematic philosophical disagreement between frontier LLM judges.

Conclusion: The conclusion is that the simulation framework is effective for evaluating conversational shopping assistant architectures, and that the findings can inform the design of such systems.
...
TL;DR: MARS, a new method for optimizing parallel test-time scaling, reduces computational overhead while maintaining accuracy in large language models.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the substantial computational overhead incurred by parallel test-time scaling, which is used to improve the accuracy of large language models (LLMs) by majority-voting over multiple reasoning traces.

Method: The authors introduce MARS (Margin-Adversarial Stopping Rule), a method that estimates which active traces are likely to change their answers and stops once the leader remains safe under a conservative bound on future vote movement.
...
Conclusion: The proposed MARS method effectively reduces the computational overhead of parallel test-time scaling while maintaining the accuracy of large language models.
...
TL;DR: The paper proposes PRISMR, a framework that improves the effectiveness of Large Multimodal Models (LMMs) in long-context multimodal ranking scenarios by replacing transient in-context list processing with parametric structural conditioning.
...
Method: The proposed framework, PRISMR, uses a lightweight hypernetwork to encode multimodal candidates in parallel and generate item-specific LoRA weights, which are synthesized into an instance-specific adapter for a LMM.
...
Conclusion: The proposed PRISMR framework effectively addresses the limitations of LMMs in long-context multimodal ranking scenarios, providing a more robust internalization of list structure while preserving the base model.
...
TL;DR: The paper proposes a multi-factor memory value function for managing long-term memory in LLM agents, which outperforms existing methods in retaining relevant information.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of managing large interaction histories in LLM agents, where the context window is limited, and deciding what to encode, forget, and retrieve under a fixed memory budget.

Method: The method proposed is a multi-factor memory value function V(m) with seven interpretable factors, whose weights are learned from a downstream objective using a gradient-free optimizer.

Result: The results show that the learned multi-factor value function retains 0.770 ± 0.011 of gold evidence across 479 usable cases, outperforming uniform weights (0.657), the best single factor (0.518), and recency (0.368).

Conclusion: The conclusion is that the proposed multi-factor memory value function is effective in managing long-term memory in LLM agents, and the learned weights are interpretable, with reliability, emotional intensity, and self/user relevance dominating the forgetting decision.
...
<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to develop a medical vision-language model that can effectively leverage large-scale pretraining data to achieve state-of-the-art performance on various medical benchmarks.

Method: The authors present OpenMedQ, a medical vision-language model pretrained on a large-scale dataset of 14 datasets, totaling ~3.35M samples, covering pathology, radiology, microscopy, and text-only clinical QA.

Result: OpenMedQ achieves state-of-the-art BLEU-1 scores on PathVQA (75.9) and VQA-MED (64.5), outperforming Med-PaLM M variants and matching the best reported results. Its vision encoder also obtains the highest average macro-F1 (0.757) on 8 unseen medical classification benchmarks.

Conclusion: The authors conclude that OpenMedQ sets a new standard for medical vision-language understanding, and release their code and interactive demo as a reproducible baseline for the community.
...
Result: Experimental results demonstrate the strengths and limitations of current foundation models in these three dimensions, providing empirical evidence for deploying autonomous agents.

Conclusion: The study provides a systematic evaluation of multimodal foundation models for power distribution defect detection, highlighting their potential for closed-loop maintenance in high-stakes industrial environments.
...
<details>
  <summary>Details</summary>
Motivation: The current process of sharing mathematical content in online forums is error-prone and cumbersome, requiring platform switching or manual LaTeX writing.

Method: The system uses a three-layer architecture (image processing, rendering, and storage) and the Mathpix OCR API to convert images to LaTeX or plain text, with live preview and support for desktop and mobile clients.

Result: A unified system that allows users to easily share mathematical content in online forums, with a provisional US patent application filed.

Conclusion: The system closes a practical gap in existing tools and forum platforms, and has the potential to create a continuously growing, community-validated dataset of mathematical problems and step-by-step solutions for training and benchmarking AI systems.
...
TL;DR: The paper presents STG, a Structured Testbench Generation framework for efficient and reliable testbench generation in large language model-driven RTL workflows.


<details>
  <summary>Details</summary>
Motivation: Existing testbench generation approaches are inefficient, stochastic, and costly, highlighting the need for a more structured and deterministic solution.

Method: The authors propose STG, which leverages the inherent structure of hardware designs to generate deterministic testbenches.

Result: STG outperforms LLM-based approaches in terms of speed (720x faster), coverage, and energy efficiency (127x less energy), and also improves RTL generation benchmarks.

Conclusion: STG is a more efficient, reliable, and scalable solution for testbench generation, which can accelerate RTL workflows and improve the accuracy of hardware design verification.
...
TL;DR: A new framework, APCyc, is proposed for de novo design of cyclic peptides, which models cyclization and optimizes multiple physicochemical properties.
...
Conclusion: APCyc offers a promising solution for the de novo design of cyclic peptides, addressing the limitations of existing methods and providing a more effective approach.
...
TL;DR: The paper challenges the notion that Multi-Agent Systems (MAS) are superior to Single-Agent Systems (SAS), finding that automatic MAS underperform SAS in various tasks despite being more expensive.


<details>
  <summary>Details</summary>
Motivation: The authors aim to rigorously evaluate the advantages of MAS over SAS, specifically focusing on automatically generated MAS, and investigate whether current evaluation frameworks accurately assess these advantages.

Method: The authors perform a systematic evaluation of automatic MAS against SAS, using Chain-of-Thought with Self-Consistency (CoT-SC) as the SAS baseline, across various reasoning datasets and tasks, including interactive multi-step workflows.
...
Conclusion: The authors conclude that current evaluation frameworks mask critical architectural gaps and inefficiencies of complex MAS, and that automated design paradigms produce architectural bloat that prioritizes superficial complexity over functional utility, highlighting a fundamental misalignment with multi-agent principles.
...
TL;DR: The paper proposes Otters++, a method to efficiently implement time-to-first-spike (TTFS) coding in spiking neural networks (SNNs) using optoelectronic devices, and achieves improved performance on the GLUE dataset while maintaining energy efficiency.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the issue of reduced benefits of TTFS coding due to the cost of computing a temporal decay term and multiplying it by the synaptic weight, and to develop an efficient and trainable TTFS computing method.

Method: The method involves using the natural signal decay in optoelectronic devices to compute the TTFS temporal term, establishing a layer-wise functional equivalence between Otters++ and a quantized neural network (QNN), and developing a hybrid training method with model distillation.
...
Result: The results show that both difficulty axes (extraction and inference) hurt every model, with compounding effects, and that reasoning models generally surpass non-reasoning models on the inference axis.
...
TL;DR: The paper explores whether human cognitive diversity can be extracted from behavior and transferred to LLM agents to mitigate cognitive monoculture. While extraction is partially successful, transferring diversity through prompts does not measurably improve results.
...
Method: The paper extracts a structured eight-dimension behavioral profile from real Polymarket trading activity and injects it into LLM agents through prompts to test if human cognitive diversity can be transferred.

Result: The extraction of behavioral profiles from human activity shows partial success, with 8 of 14 parameters being temporally stable and wallets identifiable from their profiles. However, injecting these profiles into LLM agents through prompts does not significantly improve results, with no measurable advantage over a length-matched control in terms of semantic embedding, ensemble error correlation, or Brier score.

Conclusion: The paper concludes that while human cognitive diversity can be extracted from behavior, transferring this diversity to LLM agents through prompts is not effective. This suggests the need for deeper, below-the-prompt injection methods, such as fine-tuning or activation steering, to address cognitive monoculture.
...
TL;DR: The paper explores multispectral CNN-based object detection using visible and thermal infrared images, addressing challenges in combining data from different sensors and limited thermal infrared datasets.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve object detection in intelligent video surveillance by combining visible and thermal infrared images, despite challenges such as varying illumination, sensor specialties, and limited thermal infrared datasets.

Method: The method involves using Convolutional Neural Networks (CNNs) for multispectral object detection, investigating the suitability and robustness of different augmentation techniques to address challenges in combining visible and thermal infrared data.

Result: The paper does not provide specific results, but aims to gain deeper insight into how CNNs make decisions and what they learn from different sensor input data.

Conclusion: The conclusion is not explicitly stated, but the paper likely aims to conclude with recommendations for effective multispectral CNN-based object detection using visible and thermal infrared images.
...
TL;DR: The paper presents AAbAAC, a corpus of annotated abstracts for autoimmunity research, and demonstrates its utility in improving named entity recognition (NER) performance in this specialized domain.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this work is to address the performance gaps in information extraction in highly specialized biomedical fields, specifically in autoimmunity, where generalist models struggle with domain-specific complexity.

Method: The method involves creating a manually annotated corpus of 115 abstracts (AAbAAC) and using it to evaluate and fine-tune NER models for improved performance in the autoimmunity domain.

Result: The result shows an expected improvement in NER performance after fine-tuning the models with the AAbAAC corpus, highlighting the value of small-scale annotation efforts for specialized domains.

Conclusion: The conclusion is that AAbAAC is a valuable resource for information extraction in autoimmunity research, and its creation illustrates the benefits of targeted annotation efforts for improving performance in specialized biomedical domains.
...
TL;DR: Introduced V-RAGBench benchmark and CARVE method to improve VideoRAG by enabling decoupled evaluation of retrieval and generation and adapting to chunk-level variability.


<details>
  <summary>Details</summary>
Motivation: Progress in VideoRAG is limited by existing benchmarks and methods that do not accurately evaluate retrieval errors and ignore chunk-level variability.

Method: CARVE method runs parallel retrievers across configurations and employs chunk-adaptive reranking to identify the winning configuration for each chunk.

Result: CARVE outperforms eight recent VideoRAG baselines.

Conclusion: The proposed V-RAGBench benchmark and CARVE method improve VideoRAG by enabling faithful evaluation and adapting to chunk-level variability.
...
<details>
  <summary>Details</summary>
Motivation: Current models for weather and climate forecasting lack interactive language reasoning, while large language models cannot operate on high-dimensional Earth-system data, leaving real scientific workflows underserved.
...
Result: The benchmark, TerraBench, consists of 403 tasks across three tracks and eight application domains, with 24,500 verified execution steps, highlighting the need for Earth-science agents to coordinate heterogeneous workflows and preserve artifact provenance.

Conclusion: The paper concludes that reliable Earth-science agents require more than just tool access, but also the ability to coordinate workflows, parameterize tools precisely, and preserve artifact provenance.
...
TL;DR: A new reinforcement learning framework called Cognitive Relative Policy Optimization (CRPO) is proposed to improve mental health assessment by aligning with human cognitive processes.
...
Method: CRPO integrates stage-dependent uncertainty modeling and a stage-wise entropy regularization mechanism to mimic human cognitive shift from uncertainty to certainty.
...
Conclusion: CRPO enhances reasoning capabilities for mental health assessment and shows clear advantages over existing large language models, especially on reasoning-intensive cases.
...
<details>
  <summary>Details</summary>
Motivation: The application of MLLMs in UI is rapidly evolving, but research on evaluating UX based on UI screenshots is still immature. The authors aim to address this gap by proposing a novel benchmark and an improved MLLM.
...
TL;DR: The paper explores the conditions under which a machine can be considered genuinely creative and how human agency can be preserved in shared creative environments. It proposes a framework of 10 requirements and 3 laws from Designics, illustrating their computational tractability through various studies.


<details>
  <summary>Details</summary>
Motivation: To understand under what conditions a machine can become genuinely creative and how human agency can be preserved within shared cognitive and creative environments.

Method: The paper develops a requirement framework derived from Designics, the science of meaning-bearing intentional change, and organizes it through three laws: perception, conflict, and capability.

Result: The paper argues that genuine machine creativity depends on 10 requirements, including environment representation, scoped perception, and human-AI co-living, and that proactive AI ethics is internal to genuine machine creativity.

Conclusion: The paper concludes that genuine machine creativity should not be defined by output novelty or current performance alone, but rather by the structural transformation of incomplete situations through recursive intervention dynamics, and that value-based scoping and human-AI co-living must shape creative machines' perception, conflict identification, and intervention selection.
...
TL;DR: A new framework called ARMOR-MAD is proposed to improve multi-agent debate for large language models. It uses conditional computation and consists of three components: PAR, EASE, and SOD. The framework achieves state-of-the-art results on several benchmarks.


<details>
  <summary>Details</summary>
Motivation: Fixed debate pipelines in multi-agent debate (MAD) waste computation and can amplify correlated errors. The authors aim to improve MAD by proposing a training-free heterogeneous framework.

Method: The proposed framework, ARMOR-MAD, combines three components: Pre-debate Agreement Routing (PAR), Early Agreement Stopping Evaluator (EASE), and Semantic Outlier Detection (SOD).

Result: ARMOR-MAD achieves 65.5%, 96.5%, 90.0%, and 81.5% accuracy on MATH Level 5, GSM8K, MMLU, and MMLU-Pro benchmarks, respectively.
...
TL;DR: The paper proposes a new framework for human decision-making called bounded trade-off reasoning, which suggests that people make choices by screening options based on their performance across attributes, rather than using fully compensatory utility aggregation.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the limitation of classical models in decision-making, which assume that people fully compensate for poor performance on certain attributes with gains on others, contrary to empirical evidence.

Method: The method used in this paper involves proposing a bounded trade-off reasoning framework that incorporates a screening process and a trade-off tolerance parameter, and testing it through simulations to compare its outputs with standard utility-based models.

Result: The results of the simulations show that the bounded trade-off screening mechanism produces preference patterns that differ from those of standard utility-based models and can capture context-dependent variation in trade-off behavior.

Conclusion: The conclusion of this paper is that bounded trade-off screening is a plausible computational mechanism for multi-attribute choice, and it generates testable predictions for future behavioral studies.
...
TL;DR: The study analyzes hallucination in AI medical imaging systems across five modalities, unifies taxonomies, compares general-purpose and medical-specialized models, and evaluates mitigation strategies.


<details>
  <summary>Details</summary>
Motivation: The rapid deployment of AI systems in medical imaging has raised concerns about their failure modes, particularly hallucination, which can have direct consequences for patient care.

Method: The study synthesizes peer-reviewed studies, benchmark datasets, and FDA regulatory guidance to analyze hallucination taxonomy, etiology, detection, and mitigation across five imaging modalities.
...
Conclusion: The study emphasizes the importance of managing hallucination as a lifecycle obligation, rather than a pre-deployment checklist, and highlights the need for continued oversight of radiologists and effective mitigation strategies to address different failure modes.
...
TL;DR: The paper proposes a new approach called LLM-as-an-Investigator to improve the problem-solving abilities of large language models (LLMs) by making them act as investigators, gathering evidence before providing solutions, and reducing the influence of incorrect user assumptions.
...
Method: The method proposed is called LLM-as-an-Investigator, which involves a Solution Investigator Agent that estimates problem description ambiguity, generates hypotheses, asks clarification questions, and updates hypothesis probabilities based on user answers.

Result: The results show that the proposed approach outperforms standard assistants and reasoning-oriented LLMs in diagnostic accuracy and reduces user-induced conversational bias.

Conclusion: The conclusion of this paper is that the proposed evidence-first agentic AI methodology, LLM-as-an-Investigator, can improve the problem-solving abilities of LLMs by making them more investigative and less prone to user-driven sycophancy.
...
TL;DR: Brick, a multimodal router, efficiently dispatches queries to models based on difficulty and capability, achieving high accuracy at lower costs.


<details>
  <summary>Details</summary>
Motivation: Existing LLM routers rely on surface features, ignoring within-domain variance, and are costly; optimizing query routing for cost and accuracy.

Method: Brick scores models on six capability dimensions, estimates query difficulty, and dispatches via a cost-penalized geometric rule.

Result: Brick achieves 76.98% accuracy at max-quality, beating single best model and routers; 74.11% accuracy at 4.71x lower cost; cuts cost 22.15x with 11.85 points accuracy loss.

Conclusion: Brick offers efficient and accurate query routing, enabling a balance between quality and cost.
...
<details>
  <summary>Details</summary>
Motivation: Current prompting strategies for text-to-image diffusion models are generic and limit the model's ability to accurately express emotional intent and nuanced affective attributes.
...
Result: EPIG reduces mean arousal error by 14% and 12% compared to strong baselines, preserves valence alignment and semantic consistency, and shows subject-sensitive behavior with improvements up to 17% for prompts with explicit subjects.

Conclusion: EPIG is a lightweight, training-free method that effectively enhances emotional expressiveness in text-to-image diffusion models, making it suitable for resource-constrained and personalized image generation scenarios.
...
TL;DR: The paper proposes a multi-field hybrid retrieval-augmented generation framework for automated maritime root cause analysis, improving retrieval and generation quality.
...
Method: A multi-field hybrid retrieval-augmented generation framework, utilizing a comprehensive dataset of 13,329 KMST reports, with a field-aware hybrid approach and Reciprocal Rank Fusion (RRF).

Result: The proposed retrieval method improves NormRecall@100 from 0.18 to 0.55 and enhances RCA generation quality, increasing the LLM-as-a-judge score from 3.34 to 3.72.
...
<details>
  <summary>Details</summary>
Motivation: The need for a framework that can adapt to new sensors and modalities in gait-based Parkinson's disease assessment without requiring historical patient data, due to privacy and storage constraints.
...
Conclusion: MOSAIC effectively addresses the challenges of modality-incremental learning in gait-based Parkinson's disease assessment, enabling the integration of new modalities without requiring historical patient data.
...
TL;DR: ProFact, a reinforcement learning framework, optimizes multi-stage fact verification by coordinating claim decomposition, evidence gathering, and verdict prediction, outperforming baselines in verification performance and efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing fact verification methods optimize individual stages in isolation or use fixed heuristics, leading to suboptimal outcomes due to limited adaptive coordination among stages.

Method: ProFact uses an agentic reinforcement learning framework with a unified policy to coordinate all stages of fact verification, introducing process-aware rewards for stage-level learning signals.

Result: ProFact consistently outperforms strong baselines in both verification performance and inference efficiency.

Conclusion: The results highlight the effectiveness of process-aware trajectory optimization for multi-stage fact verification, suggesting that ProFact can improve the accuracy and efficiency of automated fact verification.
...
TL;DR: This paper introduces the Ethical Robustness Testing System (ERTS) to evaluate AI systems' robustness against adversarial manipulation in ethical contexts. ERTS was used to test 6 AI models across 50 ethical scenarios, revealing that only 33% of models achieved assessment clearance.
...
Conclusion: The paper concludes that the proposed ERTS framework is a comprehensive tool for evaluating AI systems' robustness against adversarial manipulation in ethical contexts. The results highlight the need for more robust AI systems, particularly in high-stakes ethical contexts.
...
Conclusion: The proposed framework shows promise for long-term coastal wave monitoring, offering a cost-efficient and operationally feasible solution that leverages physics-guided video-based deep learning systems.
...
TL;DR: A novel framework called ReSum is proposed to improve long-horizon reasoning in Large Language Models (LLMs) by enabling self-summarization and adaptive rollout mechanisms.


<details>
  <summary>Details</summary>
Motivation: Existing Reinforcement Learning with Verifiable Rewards (RLVR) methods often lead to unnecessarily long reasoning rollouts, degrading coherence and context budget. Current approaches rely on external mechanisms rather than model-managed reasoning trajectories.

Method: ReSum framework, which enables LLMs to compress and organize reasoning trajectories through self-summarization, and uses a summarization-aware adaptive rollout mechanism with contrastive evaluation.

Result: ReSum improves performance by an average of 4% and reduces rollout length by 18.6%.

Conclusion: The proposed ReSum framework effectively improves long-horizon reasoning in LLMs by leveraging self-summarization and adaptive rollout mechanisms, leading to enhanced performance and reduced rollout lengths.
...
TL;DR: The authors propose a method to reduce computational costs in AI agents by precomputing and reusing key-value (KV) caches, eliminating redundant computations across agents.


<details>
  <summary>Details</summary>
Motivation: Currently, AI agents recompute document KV caches from scratch, leading to redundant and computationally expensive operations. The authors aim to optimize this process.

Method: The authors suggest that a publisher precompute a document's KV cache and allow other agents to reuse it, skipping the prefill step. They propose a provider-side hosting model to minimize egress costs.

Result: The proposed method yields significant compute savings: 9-50x cheaper than prefill on Qwen3-4B, with potential cost reductions of ~49.7x for serving a hot document to 80M agents.

Conclusion: The authors' proposal offers a cost-effective solution for AI agents to avoid redundant computations, with potential millions of dollars in savings per popular document. They identify open problems, including lossless KV compression and a cross-party payment layer.
...
Conclusion: IterCAD presents a unified multimodal agent framework for closed-loop, interactive CAD generation and editing, with superior capabilities in iterative refinement.
...
<details>
  <summary>Details</summary>
Motivation: To examine training dynamics in a small language model trained under a fixed, compute-constrained token budget, rather than solely evaluating efficiency through endpoint performance.

Method: A quantitative experimental repeated measures design with six independent training runs on a 4.26-million-parameter model using the TinyStories corpus and CPU-based full-precision training.

Result: The study found statistically significant interval effects for validation loss, validation perplexity, and rolling volatility, with performance improving initially but degrading later in training.

Conclusion: Compute-aware language model evaluation should examine training trajectories rather than endpoint metrics alone, as additional token exposure may not produce proportional generalization gains in constrained compute settings.
...
TL;DR: Introduced MiniMax Sparse Attention (MSA) to enable ultra-long-context capability for LLMs by reducing attention compute.


<details>
  <summary>Details</summary>
Motivation: The need for ultra-long-context capability in LLMs for applications like agentic workflows and persistent memory, hindered by the high cost of softmax attention.

Method: Developed MiniMax Sparse Attention (MSA), a blockwise sparse attention built upon Grouped Query Attention (GQA), with a co-designed GPU execution path.

Result: MSA reduces per-token attention compute by 28.4x at 1M context and achieves 14.2x prefill and 7.6x decoding speedups on H800.

Conclusion: MSA enables efficient ultra-long-context capability for LLMs, making it suitable for deployment at scale.
...
TL;DR: The paper proposes a 'compliance-by-construction' paradigm for LLM-based agents in regulated industries, integrating symbolic structures into the agent's architecture to ensure compliance.
...
Result: The authors identify a set of research challenges for neuro-symbolic integration and argue that addressing them enables 'compliance-by-construction', which can prevent control-flow violations.

Conclusion: The paper advocates for the 'compliance-by-construction' paradigm as a complementary approach to traditional monitoring methods, enabling more effective and efficient compliance in regulated industries.
...
TL;DR: The paper presents a metaheuristic approach to optimize appliance start times for maximizing renewable energy utilization while minimizing user inconvenience.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the mismatch between solar energy generation and household consumption patterns by optimizing appliance scheduling.

Method: The paper uses Iterated Local Search (ILS) and Simulated Annealing (SA) to optimize appliance start times, considering various constraints such as appliance operating durations, power consumption, and solar generation forecasts.
...
Conclusion: The paper concludes that the proposed approach can maximize renewable energy utilization while minimizing user inconvenience, and opens opportunities for future research on multi-objective trade-offs.
...
TL;DR: The paper examines how the source of labels affects the evaluation of machine learning models, particularly in large-scale metadata-driven systems where labels may be incomplete or inconsistent.


<details>
  <summary>Details</summary>
Motivation: The authors are motivated by the issue that evaluation outcomes in machine learning are often conditioned by the processes used to generate labels, which can lead to biased performance measurements.
...
Method: The paper uses a philosophical approach to argue that moral responsibility requires commitment-bearing agency grounded in intrinsic intentionality and self-attributed action, and that LLMs lack these characteristics.

Result: The authors conclude that LLMs do not exhibit genuine agency, and that their apparent intentionality is derived rather than intrinsic.

Conclusion: The paper concludes that LLMs are not moral agents and do not exhibit agency, as their operation is fully characterized by probabilistic input-output mappings learned from data, and their outputs are neither owned as commitments nor guided by reasons.
...
TL;DR: The paper proposes a benchmark called CloudCons to evaluate forecasting models for cloud resource consolidation, finding that while foundation models show better forecasting accuracy, their advantage doesn't directly translate to better decision utility.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the gap in evaluating the practical value of advanced forecasting models in cloud resource consolidation, where existing benchmarks only focus on prediction error metrics.

Method: The method involves proposing a comprehensive end-to-end benchmark called CloudCons, which includes high-quality datasets from Huawei Cloud, Microsoft Azure, and Google Borg, and conducting an extensive evaluation of various forecasting models.

Result: The results show that foundation models demonstrate superior zero-shot forecasting accuracy, but this advantage does not inherently translate into better decision utility, and that the selection of predictive quantiles is a critical lever in balancing resource efficiency and service reliability.

Conclusion: The conclusion is that the practical value of forecasting models for cloud resource consolidation should be evaluated based on decision utility, not just prediction error metrics, and that actionable guidelines are needed for calibrating predictive quantiles to balance trade-offs in real-world deployment decisions.
...
TL;DR: Proposed a retrieval augmented generation framework called UMG-RAG, which combines multiple chunk granularities and uses uncertainty-aware fusion to improve answer quality.


<details>
  <summary>Details</summary>
Motivation: The quality and granularity of retrieved evidence significantly impact the performance of retrieval augmented generation (RAG) models. Existing methods struggle with balancing context preservation and irrelevant content.
...
Conclusion: The proposed UMG-RAG framework effectively improves the performance of retrieval augmented generation models by leveraging multiple chunk granularities and uncertainty-aware fusion.
...
TL;DR: The paper proposes a solution to the cold-start problem in personalized health AI systems by using genomic profiles as a prior to initialize machine learning models for physiological interpretation.
...
Result: The proposed architecture allows for the integration of genomic data with behavioral data, enabling more accurate physiological interpretation and prediction, with the ability to distinguish between constitutional and environmentally driven deviations.

Conclusion: The paper presents a solution to the cold-start problem in personalized health AI systems by leveraging genomic profiles as a prior, allowing for more accurate and personalized physiological interpretation and prediction.
...
<details>
  <summary>Details</summary>
Motivation: Current AI discussions in scientific discovery focus on search and execution, but neglect the crucial aspect of model formation and evolution.
...
Result: The paper highlights the significance of Layer 2 reasoning, illustrating it through three case studies: S. S. Chern's intrinsic proof of the Gauss-Bonnet theorem, the resolution of the Nesterov Accelerated Gradient convergence problem, and the autonomous disproof of the Erdos unit distance conjecture.

Conclusion: The paper concludes that model formation through qualitative reasoning (Layer 2) is the most important and least developed aspect of AI in scientific discovery, and that it is essential for advancing the field.
...
TL;DR: The paper introduces protocols for producing a final answer with a single aggregated confidence for multiagent systems in NLP, improving discriminative power without sacrificing correctness.
...
Method: The paper proposes three protocols that transform raw confidence signals, making them comparable across models, and then combine them via soft voting or Bayesian fusion.

Result: The aggregated confidence is more discriminative (AUARC) than the best single agent or standard debate baselines, with stable correctness (F1-score) that recovers losses from MAD on ambiguous tasks.

Conclusion: The proposed protocols improve the reliability and oversight of multiagent systems in NLP by providing a single aggregated confidence, which is crucial for downstream decision tasks.
...
TL;DR: The paper proposes Orchestration Reward Modeling (OrchRM), a self-supervised framework for evaluating the quality of multi-agent system orchestration without human annotations, improving training efficiency and test-time scaling performance.


<details>
  <summary>Details</summary>
Motivation: The need for effective orchestration in Multi-Agent Systems (MAS) built on Large Language Models (LLMs), hindered by limited supervision and high computational costs.

Method: OrchRM uses intermediate artifacts from multi-agent executions to construct win-lose pairs for Bradley-Terry reward model training, operating directly at the orchestration level.
...
Conclusion: The proposed OrchRM framework offers a scalable direction for robust multi-agent orchestration, demonstrating significant improvements in efficiency and performance.
...
TL;DR: EpiBench is a benchmark for epigenomics analysis that tests agents' ability to make analysis decisions. Results show that no system passed a majority of attempts, with GPT-5.5/Pi leading at 45.0%.


<details>
  <summary>Details</summary>
Motivation: The motivation is to introduce a verifiable benchmark for short-horizon epigenomics analysis, evaluating agents' ability to make well-defined analysis decisions from realistic workflow states.

Method: The benchmark includes 106 evaluations across various workflows (CUT&Tag/CUT&RUN, ATAC-seq, ChIP-seq, and DNA methylation) and tests 16 model-harness pairs, resulting in 5,088 valid trajectories.

Result: No system passed a majority of attempts. GPT-5.5/Pi led at 45.0% (143/318 attempts), followed by GPT-5.5/OpenAI Codex at 39.9% (127/318 attempts). Performance varies across assay types.

Conclusion: The study concludes that while agents can find the right files and compute useful intermediate results, they often fail when tasks require deeper, assay-specific scientific judgment.
...
TL;DR: DoorDash deployed a reinforcement learning system to adapt dispatch objective weights in a food-delivery marketplace, improving batching and reducing courier-side time costs without degrading delivery quality.


<details>
  <summary>Details</summary>
Motivation: To improve dispatch decisions in a three-sided marketplace using reinforcement learning from world feedback, addressing challenges such as delayed and noisy feedback.

Method: A store-level policy learned from logged data selects a discrete multiplier to shift the dispatch optimizer's tradeoff between delivery quality and batching efficiency, using Double Q-learning and a conservative regularizer.
...
Conclusion: The system demonstrates how world feedback from a live economic and logistics system can be used to safely adapt decision policies online, improving operational outcomes in a large-scale food-delivery marketplace.
...
TL;DR: The study finds that both humans and large language models (LLMs) make similar errors in common-sense reasoning, suggesting that both may rely on pattern-matching rather than abstract world models.
...
Method: The study evaluates human participants and 25 LLMs on their common-sense reasoning abilities, and identifies attention heads driving LLM responses.

Result: The results show similar patterns of errors in both people and models, and suggest that everyday causal reasoning in people and LLMs is more consistent with pattern-matching than abstract world models.

Conclusion: The study concludes that both humans and LLMs may rely on pattern-matching rather than abstract world models for everyday causal reasoning.
...
TL;DR: The paper proposes Agentified Agent Assessment (AAA), a unified framework for evaluating agent systems using judge agents and standardized protocols, enabling reproducible, interoperable, and multi-agent evaluation.


<details>
  <summary>Details</summary>
Motivation: The evaluation of agent systems is currently fragmented and relies on fixed, LLM-centric harnesses that create test-production mismatch and limit fair comparison across diverse agent designs.

Method: The authors advocate for AAA, where evaluation is performed by judge agents and all participants interact through standardized protocols (A2A for task management and MCP for tool access).

Result: The authors conducted two studies, a five-month open competition with 298 judge agents and 467 subject agents, and a case study on coding agents, confirming that AAA delivers coverage, practicality, and fidelity across heterogeneous scenarios at scale.

Conclusion: The paper concludes that AAA and AgentBeats offer a clear path toward open, standardized, and reproducible agent assessment.
...
TL;DR: The authors propose a new perspective on shielded reinforcement learning, using automata-theoretic machinery as a design-time analytical instrument to provide structural insights about a system, rather than runtime constraints.


<details>
  <summary>Details</summary>
Motivation: The authors argue that the current approach to shielded reinforcement learning, which compiles temporal-logic specifications into automata restricting an agent's actions, is misguided.

Method: The authors instantiate their approach through a constrained two-player safety game for network defense, where the specifications are enforced asymmetrically, and solve the game to yield a defensibility verdict.

Result: The authors derive topology-level metrics from the attractor structure and combine them with post-convergence behavior from shield-constrained adversarial multi-agent reinforcement learning, forming a defensibility fingerprint.

Conclusion: The authors conclude that shield synthesis is most valuable as a framework for answering architectural questions about whether, where, and how a system can be defended, rather than as a deployment mechanism for safe agents.
...
<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to understand the cognitive and epistemic consequences of artificial intelligence and to highlight the urgent need to address the invisible forms of influence that AI systems can have on users.

Method: The paper examines and compares three recent frameworks: Tri-System Theory, Thinkframes, and System 0, to assess their ability to capture AI's influence on individual reasoning and collective epistemic practices.

Result: The paper argues that System 0 offers a theoretically distinctive perspective that Tri-System Theory and Thinkframes cannot fully replicate, particularly in understanding the concept of cognitive colonization.

Conclusion: The paper concludes that understanding the invisible forms of influence that AI systems can have on users is an urgent philosophical and practical task, given the widespread deployment of such systems.
...
<details>
  <summary>Details</summary>
Motivation: The authors argue that the bottleneck for autonomous scientific discovery is shifting from prescribing agent workflows to designing agent environments. They aim to develop reliable autonomous research agents by focusing on environment engineering.

Method: The authors propose a framework for environment engineering, which involves building environments that amplify productive behaviors and suppress harmful behaviors. They present EurekAgent, which engineers the environment along four dimensions: permissions engineering, artifact engineering, budget engineering, and human-in-the-loop engineering.

Result: EurekAgent achieves new state-of-the-art results on multiple tasks, including mathematics, kernel engineering, and machine learning tasks. Notably, it discovers new 26-circle packing results with less than $11 in total API cost.

Conclusion: The authors conclude that environment engineering is a crucial research direction for developing reliable autonomous research agents. They open-source their code and results, encouraging further research in this area.
...
TL;DR: This paper introduces Agents-K1, a knowledge orchestration pipeline that converts scientific papers into agent-native knowledge graphs, improving scientific information extraction and reasoning.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based research agents overlook scientific knowledge orchestration, reducing papers to abstracts and surface mentions, and omitting key entities and relationships.
...
Conclusion: The proposed Agents-K1 pipeline effectively addresses the limitations of current research agents by providing a comprehensive knowledge orchestration approach, leading to improved scientific reasoning and knowledge graph construction.
...
TL;DR: Large language models can automate reproducibility assessments in social and behavioral sciences, achieving comparable results to human reanalysis.


<details>
  <summary>Details</summary>
Motivation: Evaluating reproducibility in social and behavioral sciences is resource-intensive and difficult to scale, prompting the exploration of automation through large language models.

Method: Using 76 published studies, the authors compared LLM-generated analysis with original findings and human reanalysis to assess reproducibility.
...
Conclusion: Large language models can serve as a scalable tool for automated reproducibility assessment, providing a foundation for systematic auditing of empirical results in social and behavioral sciences.
