...
TL;DR: DreamCharacter-1 is a framework for generating high-quality 3D characters by enhancing pretrained 3D foundation models.
...
Method: The method involves a pipeline with three components: geometry post-training, texture post-training, and inference acceleration, building on a 3D foundation backbone.

Result: The result is that DreamCharacter-1 produces visually compelling and structurally robust 3D character assets, outperforming state-of-the-art methods.

Conclusion: The conclusion is that DreamCharacter-1 offers a lightweight and effective solution for high-fidelity 3D character generation.
...
TL;DR: The paper presents a text-conditioned diffusion model for synthesizing realistic full-body human interactions with articulated objects, addressing challenges in locomotion, contact, and object articulation.


<details>
  <summary>Details</summary>
Motivation: The scarcity of large-scale paired motion-scene data and the difficulty in generalizing across diverse object positions and shapes motivate the development of a new model.

Method: The authors introduce an object-centric representation, a mixed-domain training strategy, and a contact-based augmentation scheme to address the challenges.

Result: The proposed method demonstrates strong generalization to unseen object configurations, surpassing current state-of-the-art methods.

Conclusion: The paper proposes a novel approach for synthesizing realistic full-body human interactions with articulated objects, showing improved performance over existing methods.
...
TL;DR: The paper explores using low-cost UAV photogrammetry to reconstruct deciduous trees in 3D, enabling monitoring of shoot elongation over entire tree canopies to better understand the effects of climate change on primary tree growth.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the knowledge gap on how climate change affects primary tree growth (shoot elongation), which is crucial for understanding CO2 sequestration, by developing a method to monitor it.

Method: The method involves using low-cost UAV photogrammetry and a multi-camera CraneCam system to collect data over an entire growing season, and then analyzing the resulting 3D point clouds for accuracy, resolution, and completeness.

Result: The results show 3D point accuracies of 5-6 mm for entire trees and a 3D reconstruction completeness between 92% and 98%, demonstrating the feasibility of the approach for monitoring shoot elongation.

Conclusion: The paper concludes that low-cost UAV photogrammetry can be effectively used for reconstructing deciduous trees in 3D and monitoring shoot elongation, providing valuable insights into the impacts of climate change on primary tree growth.
...
TL;DR: The paper introduces 'adversarial decoys', a method to bypass defenses against localized adversarial attacks on Vision Transformers (ViTs) by redirecting attention towards target tokens, decoupling attack and defense objectives.


<details>
  <summary>Details</summary>
Motivation: Recent test-time defenses against localized adversarial attacks on ViTs have limitations, as they focus on suppressing image tokens with abnormally high attention scores, and the paper aims to exploit this weakness.

Method: The paper proposes 'adversarial decoys', which are independently optimized image patches that redirect attention towards selected target tokens, decoupled from the original adversarial region.
...
Conclusion: The paper reveals a fundamental limitation of using attention magnitude as an indicator of adversarial relevance and proposes a new approach to bypass defenses against localized adversarial attacks on ViTs.
...
TL;DR: A new framework called ThermoField is proposed, which combines thermal scene reconstruction and thermophysical parameter estimation through differentiable heat-transfer simulation, enabling physically interpretable parameter inference in complex 3D scenes.
...
Method: The proposed framework, ThermoField, uses spatially varying neural fields to represent quantities and constrains them through scene geometry, governing heat-transfer physics, and temporal thermal observations.
...
Conclusion: The framework establishes a bridge between thermal scene reconstruction and inverse heat-transfer analysis, providing a unified approach for geometry reconstruction, thermophysical property estimation, and predictive thermal simulation from thermal observations.
...
TL;DR: FedTR, a federated learning framework with transfer learning, is proposed for autonomous industrial visual inspection, achieving high accuracy in label defect identification.


<details>
  <summary>Details</summary>
Motivation: The limitations of data availability and the complexity of inspection tasks hinder the performance of federated learning in industrial visual inspection.

Method: FedTR uses transfer learning and federated learning to adapt a pre-trained model to limited private data, fine-tuning the model through federated learning.
...
Conclusion: FedTR is effective and feasible for label defect identification in industrial visual inspection, preserving data privacy.
...
TL;DR: The authors propose LightCrafter, a hybrid pipeline for video relighting that combines physically-based rendering (PBR) with diffusion models to achieve better temporal consistency and lighting control.


<details>
  <summary>Details</summary>
Motivation: Existing video relighting methods struggle with balancing temporal consistency and accurate estimation of intrinsic scene properties, and are limited by noisy reconstructions, hard-to-model effects, and lack of control over lighting.

Method: The authors propose a hybrid pipeline that uses PBR to generate a proxy video under target illumination, and then translates this proxy to the final target using a diffusion model.
...
Conclusion: The proposed LightCrafter pipeline achieves better video relighting results with improved temporal consistency and lighting control, and will release the dataset, benchmark, metrics, and code.
...
TL;DR: The paper proposes SAGA, a training-free approach to improve temporal quality in autoregressive video generation by suppressing short-range temporal correlations and preserving long-range motion structure.


<details>
  <summary>Details</summary>
Motivation: Autoregressive video diffusion can amplify temporal errors, resulting in flickering, motion jitter, and structural drift, due to the reuse of generated latents as causal context.

Method: The proposed SAGA approach integrates an acceleration domain spectral guidance objective with a structured autoregressive noise initialization strategy.
...
Conclusion: The proposed SAGA approach effectively reduces temporal instability while maintaining visual fidelity in autoregressive video generation.
...
<details>
  <summary>Details</summary>
Motivation: Long-horizon robot planning requires considering both semantic task structure and geometric feasibility, which is challenging due to spatial constraints and object collisions.

Method: APIVOT, a VLM-based planner that adaptively interleaves language and visual thoughts for long-horizon planning.
...
Conclusion: APIVOT learns to leverage language and visual thoughts effectively, improving planning success and reasoning efficiency through adaptive interleaving.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of traditional imitation learning in autonomous driving, such as accumulating execution errors, scarce recovery behaviors in training data, and the inability to capture long-horizon objectives like safety and driving comfort.

Method: The paper provides a unified view of post-training for autonomous driving by categorizing existing literature into four major families based on the form of supervision they use, and discusses their capabilities, limitations, and open challenges.

Result: The paper presents a systematic understanding of post-training techniques for autonomous driving, highlighting their potential to improve reliability and efficiency.

Conclusion: The conclusion is that post-training techniques are necessary for autonomous driving to ensure reliability and safety, and that future research should focus on developing reliable and efficient post-training methods.
...
TL;DR: The paper introduces UAV Open-Vocabulary Video Instance Segmentation (UAV-OVVIS) and proposes AeroTrack, a training-free framework for it, outperforming existing methods and demonstrating strong open-vocabulary robustness and generalization.


<details>
  <summary>Details</summary>
Motivation: Existing UAV video perception methods have limitations in supporting flexible queries and fine-grained instance-level dynamic understanding in open scenarios.

Method: AeroTrack, a unified framework that centers on periodic open-vocabulary detection, short-segment mask propagation, and cross-segment identity unification, reusing existing visual foundation models.
...
Conclusion: The paper introduces a new task UAV-OVVIS and proposes AeroTrack, a training-free unified framework, and releases AeroTrack and AeroVIS as a unified framework and benchmark for UAV-OVVIS.
...
TL;DR: The paper proposes a novel feature fusion method called Laplacian Decoupled Feature Enhancement block (LDFE) for object detection in RGB-IR images, achieving state-of-the-art performance on multiple datasets.
...
Method: The authors introduce LDFE, a feature fusion block that uses global-local decomposition, denoising, fusion, and reconstruction to combine features from different stages of a dual-stream CNN backbone.

Result: The proposed method achieves significant performance improvements, with mAP surpassing state-of-the-art methods by 6.2%, 3.7%, 4.7%, 2.3%, 4.1%, and 2.0% on M3FD, DroneVehicle, LLVIP, FLIR-Aligned, KAIST, and VEDAI datasets, respectively.

Conclusion: The paper presents a novel and effective feature fusion method for RGB-IR object detection, demonstrating superior performance on multiple datasets.
...
TL;DR: A novel approach called Mixture of Enhanced-View Experts (EV-MoE) is proposed for robust multi-query vehicle ReID, which enhances feature representation and integrates view-specific features.
...
Method: The proposed approach, EV-MoE, consists of a mixture of enhanced-view experts module with two sub-modules: view-specific feature enhancement sub-Module (VFEM) and dynamic multi-view fusion sub-Module (DMFM), along with Multi-view Alignment Loss (MAL).

Result: Extensive experiments demonstrate the robustness of EV-MoE in addressing the multi-query vehicle ReID problem, and a large-scale vehicle ReID dataset (LCRI-1K) is collected for evaluation.

Conclusion: The proposed EV-MoE approach provides a robust solution for multi-query vehicle ReID by enhancing feature representation and integrating view-specific features.
...
TL;DR: The paper proposes GRE-Diff, a framework for automating the creation and editing of apartment floor plans using AI and human-in-the-loop interaction.
...
Method: The framework uses Gaussian Room Embedding (GRE) and combines AI-generated suggestions with real-time human editing to produce high-quality floor plans under user-specified constraints.

Result: GRE-Diff produces high-quality, constraint-aware, and editable polygonal layouts, as shown by extensive experiments on the RPLAN dataset.
...
TL;DR: The paper presents EVIS, a plugin for NVIDIA Isaac Sim that generates labeled event streams for event cameras, addressing the scarcity of labeled data for robotics applications.


<details>
  <summary>Details</summary>
Motivation: The development of event-based perception and control in robotics is hindered by the scarcity and high cost of collecting labeled event-camera data for specific robots and scenes.

Method: The authors propose a physics-grounded event camera plugin, EVIS, which generates high-rate, fully labeled event streams inside a physics simulator, implementing a faithful log-intensity contrast event model and offering configurability and interpolation options.

Result: The generated event streams are directly usable by pretrained event networks for downstream tasks, and the plugin integrates into any Isaac Sim / Isaac Lab scene, inheriting the simulator's physics and frame-perfect ground truth.

Conclusion: The EVIS plugin provides a solution for generating high-quality, labeled event-camera data for robotics applications, facilitating the development of event-based perception and control.
...
TL;DR: Introduced VSRo-200, a large-scale dataset for visual speech recognition in Romanian, and established a benchmark for low-resource settings.
...
Method: Created a dataset with 200 hours of podcast videos, annotated with pseudo-labels and human transcriptions, and evaluated its impact on visual speech recognition performance.

Result: Pseudo-labels enable scalability, while human annotations provide better performance at fixed data scales; multimodal fusion improves robustness under noisy conditions.

Conclusion: VSRo-200 provides a new testbed for studying supervision, domain generalization, and multimodal fusion in low-resource visual speech recognition.
...
TL;DR: The paper investigates the video-action generalization gap in video-action models (VAMs) and proposes an inference-time adaptive guidance method to mitigate it.


<details>
  <summary>Details</summary>
Motivation: The discrepancy between the strong compositional priors of generative video foundation models and the loss of these priors in world-action models (WAMs) and VAMs after finetuning on robotic action data.

Method: The paper introduces the Temporal Ratio (TR), an attention-based measure to evaluate a model's reliance on future-predictive latents, and proposes an adaptive guidance method to amplify compositional video conditioning signals.

Result: The approach mitigates the OOD-ID compositional generalization gap on the LIBERO benchmark and real-world tasks.

Conclusion: The paper provides insights into the video-action generalization gap and proposes a method to improve compositional generalization in VAMs.
...
TL;DR: The paper proposes a new approach to face attack detection by leveraging textual descriptions of forgery cues, achieving better performance than vision-only methods.


<details>
  <summary>Details</summary>
Motivation: Existing datasets lack detailed textual descriptions of forgery cues, and most prior methods treat face attack detection as a visual recognition task.

Method: The paper proposes a Dual Alignment Forgery Network (DAF-Net) to leverage textual information and fine-grained textual descriptions of forgery cues.

Result: The approach extracts more generalizable and semantically meaningful forgery representations from attack images, outperforming both vision-only methods and approaches based on coarse-grained descriptions.

Conclusion: The proposed approach improves face attack detection by effectively utilizing textual information.
...
TL;DR: The paper presents ProsMAE, a multi-source Masked Autoencoder framework for histopathology representation learning, which achieved better performance in ISUP grade classification compared to a baseline model.


<details>
  <summary>Details</summary>
Motivation: The gigapixel scale, stain variation, scanner differences, tissue artifacts, and limited expert annotation of Whole Slide Images (WSIs) make robust model training challenging in computational pathology.

Method: A multi-source Masked Autoencoder (MAE) framework, named ProsMAE, was proposed and pretrained using tiles from PANDA, CAMELYON17, and BRACS datasets.
...
Conclusion: The proposed ProsMAE framework shows potential in improving histopathology representation learning and ISUP grade classification, but further evaluation is needed to establish robustness across different split compositions.
...
TL;DR: This paper surveys Continual Test-Time Adaptation (CTTA) methods for adapting pretrained models to non-stationary target distributions without access to source data or labeled targets.


<details>
  <summary>Details</summary>
Motivation: Real-world deployment of deep neural nets often faces distributional shifts between training and test data, motivating the need for adapting pretrained models to changing target distributions.

Method: The paper proposes a hierarchical taxonomy categorizing CTTA methods into optimization-based, parameter-efficient, and architecture-based approaches, and systematically reviews representative methods within each category.

Result: The paper presents comparative benchmarks and experimental results across standard evaluation settings, highlighting the strengths and limitations of current CTTA approaches.

Conclusion: The paper concludes by discussing limitations of current approaches and highlighting emerging research directions, including adaptation of foundation models and black-box systems, providing a roadmap for future research in robust continual test-time adaptation.
...
TL;DR: The paper proposes a method combining attention-based segmentation and feature-driven classification to differentiate between vascular and demyelinating white matter pathologies in brain MRI scans.
...
Conclusion: The proposed approach shows promise for accurate and efficient classification of WMHs, but further validation in larger clinical cohorts is needed.
...
TL;DR: The paper proposes LEEVLA, a vision-language-action model that guides the model to focus on task-critical evidence and preserves structured evolution of latent world representations, outperforming prior methods on VLA benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing VLA models struggle with complex dynamic scenarios due to uniform treatment of visual tokens and reliance on human-selected factors, lacking mechanisms to emphasize critical evidence and consider underlying factors.

Method: The authors propose LEEVLA with two main components: drift-guided dynamic prioritization (DGDP) to guide the model to attend to salient regions, and structured feature flow generation (SFFG) to model the evolution of prioritized features in latent space.
...
Conclusion: The proposed LEEVLA architecture improves VLA models by emphasizing task-critical evidence and preserving structured evolution of latent world representations, demonstrating its potential for scalable VLA applications.
...
TL;DR: NamedCurves+ is a deep learning approach for image enhancement that integrates color names and tone curves for interpretable and interactive editing.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning methods for image enhancement struggle with interpretability and parametrization for user adjustments.

Method: NamedCurves+ uses a learning-based framework that incorporates color names and tone curves, along with a transformer block for context-aware edits.

Result: NamedCurves+ outperforms state-of-the-art methods in image retouching, tone mapping, and exposure correction, and provides explainable and interactive editing capabilities.

Conclusion: NamedCurves+ enhances the image retouching process with improved interpretability and user interaction, allowing for flexible and personalized modifications.
...
TL;DR: Proposed a Dual-Correlation Hypergraph Network (DHNet) for RGB-Thermal Video Object Detection (VOD) that addresses spatial misalignment between RGBT image pairs and introduced a large-scale RGBT VOD dataset.


<details>
  <summary>Details</summary>
Motivation: To overcome limitations of conventional RGB-based VOD under challenging conditions and address spatial misalignment in RGBT image pairs.

Method: Proposed a Dual-Correlation Hypergraph Network (DHNet) with a Patch-based Spatial Alignment Module (PSAM) and a Dual Hypergraph Fusion Module (DHFM).

Result: DHNet achieves state-of-the-art detection accuracy on VT-VOD50 and DVT-VOD1000 datasets.

Conclusion: The proposed DHNet and DVT-VOD1000 dataset advance RGB-Thermal Video Object Detection research.
...
TL;DR: Low-rank adaptation in vision-language alignment reduces computational costs and outperforms full-parameter alignment on most benchmarks.


<details>
  <summary>Details</summary>
Motivation: Challenging the common view that vision-language alignment requires full-parameter updates, the authors investigate the effectiveness of low-rank adaptation.

Method: The authors apply low-rank adaptation to the LLM during vision-language alignment and compare its performance with full-parameter alignment.

Result: Low-rank alignment outperforms full-parameter alignment on most benchmarks, and the authors identify the implicit biases introduced by low-rank adaptation.

Conclusion: The authors conclude that low-rank alignment preserves the structure of visual representations, inducing preferences for parameter subspaces with flat gradients and feature subspaces robust to perturbations.
...
TL;DR: The paper proposes LUD-MSR, a framework for learning joint distributions from marginal data, and demonstrates its effectiveness in real-world denoising tasks.


<details>
  <summary>Details</summary>
Motivation: The problem of learning a joint distribution from marginal observations is inherently ill-posed due to ambiguity in feasible couplings.

Method: The authors propose a latent-variable probabilistic framework called LUD-MSR, which uses auxiliary representations and optimizes evidence lower bounds using only marginal data. They also introduce a Multi-Scale image Representation (MSR) mapping.

Result: The proposed framework achieves a more favorable balance between domain consistency and information preservation, and demonstrates effectiveness in real-world denoising benchmarks, including cryo-electron microscopy (cryo-EM).

Conclusion: The paper concludes that LUD-MSR is an effective framework for learning joint distributions from marginal data, and that MSR mapping can achieve a better trade-off between domain consistency and information preservation.
...
TL;DR: The authors propose a hybrid method for large-vocabulary instance segmentation that combines text-to-image generation with context-aware image-to-image editing, achieving improved performance on rare classes and overall accuracy.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the challenges of long-tailed category distributions and fine-grained inter-class ambiguity in large-vocabulary instance segmentation, which are not well-solved by existing data synthesis methods.

Method: The proposed method couples text-to-image (T2I) generation with context-aware image-to-image (I2I) editing, using a teacher-student scheme to ensure label reliability and a novel I2I editor called VRAIN for targeted augmentation of rare classes.

Result: The method surpasses existing baselines on the LVIS benchmark, improving overall AP by up to +4.0 points and rare-class AP by up to +9.5 points, and scales effectively with backbone capacity.

Conclusion: The authors conclude that their hybrid pipeline, which combines T2I generation with context-aware I2I editing and VRAIN, effectively addresses the challenges of large-vocabulary instance segmentation and achieves state-of-the-art performance.
...
TL;DR: The authors analyzed 27 papers on colonoscopy polyp segmentation and found issues with evaluation methods, including omission of Hausdorff distance, incompatible train/test split protocols, and lack of statistical significance testing.


<details>
  <summary>Details</summary>
Motivation: The authors argue that progress in colonoscopy polyp segmentation is difficult to verify due to structural problems in how the community evaluates models.
...
TL;DR: A new federated medical imaging benchmark, MobenFL, is proposed to comprehensively evaluate the performance of federated learning algorithms in real-world clinical settings, addressing existing limitations.


<details>
  <summary>Details</summary>
Motivation: The need for a unified evaluation standard for federated learning algorithms in medical imaging due to the challenges posed by privacy requirements, data variations, and algorithm heterogeneity.

Method: Development of the MobenFL benchmark, which integrates 20 cutting-edge federated learning algorithms and 22 medical imaging datasets, covering 12 critical organs and evaluating performance, efficiency, and privacy protection capabilities.

Result: The MobenFL benchmark provides a comprehensive evaluation framework for federated learning in medical imaging, surpassing existing benchmarks in breadth and depth.

Conclusion: The MobenFL benchmark is a crucial step toward advancing the reliable clinical application of federated learning in medical imaging, providing a unified evaluation standard for objectively assessing algorithm performance.
...
TL;DR: LUMI is a tokenizer-agnostic framework for lossless RGB image compression using frozen LLM backbones, achieving competitive compression rates and cross-domain robustness.


<details>
  <summary>Details</summary>
Motivation: Existing LLM-based lossless image compression methods are coupled with tokenizer behavior and model-family-specific adaptation, limiting their generalizability.

Method: LUMI uses a pixel embedding module, intra-patch position encoding, and a 256-way prediction head, training only these components while keeping the LLM backbone fixed.

Result: LUMI achieves competitive compression rates and improves cross-domain robustness on natural, medical, and remote-sensing image benchmarks with different LLM backbones.

Conclusion: LUMI provides a unified interface for LLM-based lossless image compression across tokenizer families, formulating it as pixel-space adaptation of frozen foundation models.
...
<details>
  <summary>Details</summary>
Motivation: Existing deep learning-based methods for photorealistic style transfer suffer from semantic entanglement and color banding, and lack multimodal capability for text-driven control.

Method: The proposed StatLUT framework consists of a Lab-Extractor to derive spatially-agnostic statistical features, a Transformer-based Seq2Seq translation task with a Multi-dimensional Residual Mapper (MR-Mapper) to predict 3D LUTs, and a lightweight Diffusion Transformer (H-Diffuser) for text-driven color grading.
...
TL;DR: The paper proposes a new framework for event-based lip reading, enhancing temporal modeling and articulatory structure constraint.


<details>
  <summary>Details</summary>
Motivation: Existing methods for event-based lip reading have limitations in temporal modeling and articulatory structure constraint, which hinder accurate lip movement recognition.
...
Result: The proposed design shows effectiveness on the DVS-Lip benchmark, and ablation studies validate the contributions of TDA, VGA, and teacher-student consistency.

Conclusion: The proposed framework improves event-based lip reading by enhancing temporal modeling and articulatory structure constraint, leading to better recognition accuracy.
...
TL;DR: The paper addresses the challenges of quantizing classifier-free guidance (CFG) diffusion models under real-world compute budgets, proposing Guidance-Aware Mixed Precision (GAMP) to overcome the 'branch-drift trap' and improve efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing post-training quantization (PTQ) methods are not suitable for CFG models as they ignore the paired conditional/unconditional structure of CFG inference, leading to a 'branch-drift trap' that causes performance degradation.

Method: The paper proposes Guidance-Aware Mixed Precision (GAMP), which calibrates directly on the guided prediction, derives per-layer activation-bit sensitivity from guided-output degradation, and allocates bits via a greedy knapsack.

Result: The proposed GAMP method prevents unconditional branch drift and improves the efficiency of CFG models, overcoming the 'branch-drift trap' and achieving better sample quality.

Conclusion: The paper concludes that GAMP is an effective solution for deploying CFG diffusion models under real-world compute budgets, addressing the limitations of existing PTQ methods and improving the efficiency and performance of CFG models.
...
TL;DR: The paper proposes SkelGen4D, a weakly supervised framework for text-driven mesh animation that generates explicit skeleton motions for temporally coherent 3D sequences.


<details>
  <summary>Details</summary>
Motivation: Existing methods for 4D generation either require extensive annotations or lack explicit control over the animation process. The authors aim to create a framework that aligns with standard industrial pipelines, allowing for explicit control and editing.

Method: The authors propose a two-stage framework: (1) recovering pseudo-skeletons from animated meshes via differentiable fitting, and (2) generating text-conditioned skeleton motion sequences using a feed-forward approach, refined with Motion-GRPO.

Result: The method matches or surpasses fully supervised baselines on two large-scale benchmarks, Truebones Zoo and Diffusion4D, and supports flexible motion editing while aligning with standard animation production pipelines.

Conclusion: The proposed SkelGen4D framework enables high-quality text-driven mesh animation with explicit skeleton motions, offering a practical solution for animation and content creation.
...
TL;DR: Proposed Hierarchical Slot Attention (HSA) for multi-granularity semantic scene decomposition, outperforming flat baselines with a single model.
...
Method: Hierarchical Slot Attention (HSA) with hierarchical alignment loss, using only 10% labeled data to learn multi-granularity decomposition at three levels: holistic, semantic, and panoptic.
...
Conclusion: HSA enables effective multi-granularity semantic scene decomposition with a single model, leveraging hierarchical alignment loss and limited labeled data.
...
TL;DR: The paper proposes two methods, MoDE and MoE-GS, for multi-deformation modeling in dynamic 3D Gaussian representations, enabling robust dynamic scene reconstruction.


<details>
  <summary>Details</summary>
Motivation: Dynamic scene reconstruction is challenging due to heterogeneous and spatially varying real-world motion. Existing methods rely on a single deformation model, limiting robustness across diverse dynamic scenarios.

Method: The paper introduces two approaches: MoDE, which integrates multiple deformation experts directly into the deformable Gaussian Splatting pipeline through joint optimization, and MoE-GS, where experts are optimized independently and combined through a separate routing stage.

Result: The proposed methods provide alternative strategies for multi-deformation modeling, clarifying how integration constraints shape the design and behavior of deformation experts in dynamic 3D Gaussian representations.

Conclusion: The paper presents a study on multi-deformation modeling for dynamic 3D Gaussian representations, offering two approaches, MoDE and MoE-GS, which enable robust dynamic scene reconstruction by combining multiple specialized deformation models within a unified 3D representation.
...
TL;DR: The paper introduces a new task called Universal Referring for unmanned aerial vehicles (UAVs) that involves localizing targets from diverse instructions in complex scenes. It presents a multimodal benchmark called UniRef-UAV and a detection-style baseline model called UAV-URNet.


<details>
  <summary>Details</summary>
Motivation: Existing referring expression comprehension benchmarks and methods are limited in their applicability to practical UAV scenarios. They do not account for reference images, multimodal instructions, absent targets, and multiple valid target instances.

Method: The authors introduce UAV-URNet, a detection-style baseline that maps heterogeneous queries into a shared query space and predicts variable-size target sets through set prediction.
...
Conclusion: The paper contributes to the development of more practical and generalizable referring expression comprehension systems for UAVs. It provides a new benchmark and a baseline model, and makes the annotations, code, and evaluation scripts publicly available to facilitate reproducible research.
...
TL;DR: The authors propose Latent Drift, a progressive generative framework for forecasting slow-evolving neurodegenerative diseases using longitudinal MRI, addressing issues with current methods.


<details>
  <summary>Details</summary>
Motivation: Forecasting neurodegenerative diseases is challenging due to subtle progression signals in longitudinal MRI, making it hard to develop reliable models.

Method: The authors propose Latent Drift, a framework that learns change in a compressed semantic representation and applies Finite Scalar Quantization to suppress nuisance fluctuations.

Result: Experiments on longitudinal 3D brain MRI show that Latent Drift improves patient-specific neuro-forecasting over baselines across generative fidelity and clinically relevant metrics.

Conclusion: The proposed Latent Drift framework offers a more reliable approach to forecasting neurodegenerative diseases, enabling earlier and more targeted interventions.
...
<details>
  <summary>Details</summary>
Motivation: The authors aim to address challenges in poverty mapping using satellite imagery, including noisy and sparse survey-derived supervision, image quality issues, and lack of explicit spatial structure in image-only models.

Method: The authors develop an enhanced pipeline building on the KidSat framework, which includes refined data preprocessing, systematic image quality assessment, and mathematically defined geographic encoding. They also introduce a two-stage quality-screening procedure, fuse DINOv2 visual embeddings with Spherical Harmonics (SH) location features, and use gradient-boosted tree heads (XGBoost/LightGBM) to exploit nonlinear interactions.
...
Conclusion: The authors provide a scalable and principled recipe for improving satellite-based socioeconomic predictions using only publicly accessible data, highlighting the importance of SH features and carefully designed objectives.
...
<details>
  <summary>Details</summary>
Motivation: The need for accurate and efficient cell tracking in time-lapse microscopy to analyze cell dynamics, understand immune cell interactions, disease progression, and drug mechanisms.
...
Result: ARGUS achieved detection accuracy of 0.905-0.971 and tracking accuracy of 0.897-0.964 on Cell Tracking Challenge datasets, with runtimes within 1 minute.
...
TL;DR: This study compares the representations of texture perception in Convolutional Neural Networks (CNNs) and Vision Transformers (ViTs) with human psychophysics data, finding that ViTs better capture human-like texture processing.


<details>
  <summary>Details</summary>
Motivation: The study aims to investigate the alignment between computational models (CNNs and ViTs) and human visual processing, specifically for texture perception, which extends beyond the common object recognition paradigm.

Method: The study uses a rank-based statistic to quantify the information encoded in the internal representations of CNNs and ViTs for textures of different complexity, and compares these representations to those inferred from human psychophysics data.
...
<details>
  <summary>Details</summary>
Motivation: Existing Vision-Language-Action models for autonomous driving lack comprehensive world cognition or suffer from fragmented world foresight, limiting them to reactive driving.

Method: WCog-VLA, a dual-level framework, incorporates 3D spatial perception, agent tokens, and Game-theoretic Chain-of-Thought reasoning at the semantic level, and Aligned Decoupled Diffusion Transformer at the generative level.
...
TL;DR: The paper compares 13 methods for quantifying image symmetry, finding that deep learning methods perform best but a classical descriptor (HOG) trails by a small margin and runs much faster.


<details>
  <summary>Details</summary>
Motivation: To benchmark and compare various symmetry scoring methods for images on a common protocol, addressing the lack of a statistically grounded comparison in the literature.

Method: The paper evaluates 13 symmetry scoring methods (9 from literature, 4 new) across 9 datasets using a reflection-exact protocol with significance testing, comparing performance on single-axis and multi-axis symmetry.

Result: Deep learning methods perform best, but a classical histogram-of-oriented-gradients (HOG) descriptor is nearly as effective and significantly faster (~300x on CPU).

Conclusion: Frozen deep features offer little improvement over a tuned classical descriptor (HOG) for measuring symmetry; the authors release an open toolkit (imgsym) for image symmetry detection and measurement.
...
TL;DR: A new benchmark and framework for referring image segmentation in endoscopic imagery is proposed, addressing challenges of scarce annotations and complex image-text relationships.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the performance of referring image segmentation in endoscopic imagery, which is crucial for fine-grained and controllable visual understanding, but faces unique challenges due to scarce high-quality annotations and complex image-text relationships.

Method: The Attribute Retrieval-based Endoscopic-RIS (AR-ERIS) framework is proposed, which leverages attribute retrieval for open-vocabulary endoscopic compositional referring segmentation and is pretrained on the curated ReferEndoscopy dataset.
...
Conclusion: The proposed benchmark and framework have the potential to improve the performance of referring image segmentation in endoscopic imagery, enabling more accurate and controllable visual understanding in this field.
...
TL;DR: This paper proposes a five-stage pipeline using face swapping to protect pedestrian privacy in autonomous vehicle datasets while preserving essential facial attributes.


<details>
  <summary>Details</summary>
Motivation: The need for large-scale and diverse datasets for autonomous vehicles conflicts with the requirement to protect pedestrian privacy, particularly in terms of identity theft and tracking.

Method: A five-stage pipeline for face swapping, evaluated using Roop and Ghost-v2 models, to balance privacy preservation and data usability.

Result: Roop outperforms Ghost-v2 and is chosen for the pipeline to conceal pedestrian identity while preserving facial attributes.

Conclusion: The proposed pipeline using Roop for face swapping effectively balances pedestrian privacy and data usability for training autonomous vehicle models.
...
TL;DR: Track2Map is an online 3D reconstruction method for robot-assisted minimally invasive surgery that jointly optimizes camera trajectory and 3D deformable scene representation from surgical video, improving reconstruction quality and camera trajectory accuracy.


<details>
  <summary>Details</summary>
Motivation: Current state-of-the-art methods for 3D anatomy reconstruction in robot-assisted minimally invasive surgery are offline and rely on accurate camera trajectory priors, which can be limiting when priors are missing or noisy.

Method: Track2Map, an online 3D Gaussian Splatting pipeline that uses a track-anchored deformation initialization and track statistics to disentangle camera motion from scene deformation.

Result: Experiments on StereoMIS show improved reconstruction quality and camera trajectory accuracy compared to competing SLAM methods and non-SLAM methods that utilize camera trajectory priors.

Conclusion: Track2Map enables robust 3D reconstructions in robot-assisted minimally invasive surgery without relying on accurate camera trajectory priors, making it a promising solution for online 3D reconstruction.
...
TL;DR: A new multimodal agent architecture is proposed, which uses a structured memory and modular decision-making to improve long-horizon multimodal dialogue and vision/language understanding.


<details>
  <summary>Details</summary>
Motivation: Current unified multimodal models have limitations in long-horizon multimodal dialogue due to visual token explosion and unreliable cross-turn referencing.

Method: The proposed Cognitive-structured Multimodal Agent uses an Episodic Visual Memory, Perceptual Abstraction Engine, Cognitive Retrieval Engine, and Multimodal Executive Controller to selectively reactivate relevant visual information and optimize abstraction and retrieval policies.

Result: The 8B agent achieves 91.4% retrieval accuracy over 20-turn sessions, surpassing 32B baselines by +8.2% while nearly halving per-turn inference time.

Conclusion: The proposed architecture offers a more scalable and efficient paradigm for long-horizon multimodal agents than monolithic parameter scaling.
...
Conclusion: The proposed DeltaV model improves multimodal reasoning and reduces visual-token redundancy, achieving state-of-the-art performance on various benchmarks.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to determine agricultural potential, specifically viticulture potential in Southern France, using remote sensing data due to the high cost of traditional methods.

Method: The method involves using an ensemble of U-Net and a Geospatial Foundation Model (Prithvi-2.0) for predicting viticulture potential.

Result: The best model achieved a ±1 accuracy of 68.32 on the leaderboard, ranking 2nd among 7 teams.

Conclusion: The proposed method shows promising results in predicting viticulture potential, demonstrating the potential of remote sensing data and ensemble models for sustainable land management and agricultural planning.
...
TL;DR: A new metric, VEGAS, is proposed to evaluate video captions based on viewer attention, leading to improved caption alignment with human focus and better video retrieval.


<details>
  <summary>Details</summary>
Motivation: Current video captioning models fail to capture individual viewers' attention, and the authors aim to address this limitation.

Method: The authors propose VEGAS, a training-free, cross-modal metric that uses test-time gaze to sample personalized captions aligned with viewer attention.
...
Conclusion: The proposed VEGAS metric demonstrates practical utility in incorporating viewer attention during inference, leading to improved video captioning and retrieval.
...
TL;DR: This study uses a domain-specific foundation model, CT-CLIP, for multimodal survival prediction in lung cancer patients with limited data.


<details>
  <summary>Details</summary>
Motivation: Deep learning-driven survival modelling is limited by the scarcity of curated imaging cohorts with reliable outcome data.

Method: The study evaluates CT-CLIP as a feature extractor for pretreatment computed tomography images and clinical variables, using adaptation strategies such as frozen encoders, full fine-tuning, and low-rank adaptation.

Result: A frozen CT-CLIP model combined with a trainable lightweight survival head outperforms the clinical baseline and achieves comparable or improved performance relative to other multimodal approaches.

Conclusion: The study demonstrates the potential of using a domain-specific foundation model for multimodal survival prediction in data-constrained clinical settings, allowing for accurate prognosis prediction and separation of patients into clinically meaningful high- and low-risk groups.
...
TL;DR: The paper proposes a new learning paradigm for hand-object interaction (HOI) recognition, combining hand-object masked training and HOI-dynamics-aware decoder to improve robustness and sensitivity to hand and object cues.


<details>
  <summary>Details</summary>
Motivation: Existing video-language models for HOI recognition often rely on spurious correlations and fail to reason from the appearance and dynamics of hands and objects themselves.

Method: The proposed approach includes hand-object masked training and an HOI-dynamics-aware decoder that learns hand- and object-centric embeddings through auxiliary predictions.

Result: The approach improves over existing models on DEHOI, standard action recognition, object state recognition, and robot manipulation action recognition, demonstrating more robust HOI understanding.

Conclusion: The proposed learning paradigm enhances the robustness and sensitivity of HOI recognition models to hand and object cues, leading to more accurate and reliable HOI understanding.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to address the lack of exploration of disability-related biases in text-to-image models and to provide a benchmark for evaluating these biases.
...
Conclusion: The paper concludes that the INCLUDE-BENCH benchmark can help identify and mitigate biases in text-to-image models, and that the models' depictions of people with disabilities often rely on stereotypes.
...
TL;DR: The paper proposes Whareformer, a transformer-based model for the 'Out of Sight, Not out of Mind' (OSNOM) task, which tracks objects moved by the camera wearer in egocentric videos, even when they leave the field of view or are occluded.


<details>
  <summary>Details</summary>
Motivation: The OSNOM task focuses on tracking objects in egocentric videos that are moved by the camera wearer, maintaining knowledge of instance locations even when they leave the field of view or become heavily occluded. Current solutions are limited, and the authors aim to propose a learning-based approach.

Method: The authors propose Whareformer, a transformer-based model consisting of two components: an updatable memory of established tracks and a track assignment module. Whareformer reasons over evolving object appearance and updated 3D location, using relative distances and evolving track representations.

Result: Whareformer achieves State-of-the-Art (SOTA) performance on 260 long test videos from three datasets: EPIC-KITCHENS-100, IT3DEgo, and HD-EPIC, with significant absolute improvements over prior work, despite being trained on a small set of 56 videos.

Conclusion: The proposed Whareformer model effectively addresses the OSNOM task, demonstrating improved performance in tracking objects in egocentric videos, even when they are out of sight or occluded.
...
TL;DR: VocaDet is a sample-driven framework for open-vocabulary object detection and segmentation that learns from user-provided samples without model retraining.
...
Result: VocaDet achieves effective open-vocabulary detection performance on the UA-DETRAC dataset without conventional detector training.

Conclusion: VocaDet enables continuously expandable recognition capability as additional samples are accumulated, making it a scalable solution for open-vocabulary object detection and segmentation.
...
TL;DR: The paper proposes Switch-Reasoner, a framework that helps multimodal large language models adaptively choose between thinking and answering directly, improving efficiency and performance.


<details>
  <summary>Details</summary>
Motivation: The fixed Think-then-Answer paradigm in MLLMs is inefficient in heterogeneous multitask settings, and learning when to think can be unstable during post-training.

Method: The authors propose a GRPO-based framework called Switch-Reasoner, which treats thinking as a virtual tool invocation and uses a dual-level regulation mechanism to balance the use of thinking and direct modes.
...
Conclusion: The proposed Switch-Reasoner framework effectively adapts the reasoning mode for MLLMs, leading to improved efficiency and performance in multimodal tasks.
...
TL;DR: The paper proposes a Structured Sparse AutoEncoder ($S^2AE$) to improve sparse autoencoders (SAEs) in vision-language models (VLMs) by enforcing concept consistency in the visual modality, achieving better semantic alignment, representational efficiency, and feature coherence.


<details>
  <summary>Details</summary>
Motivation: Vanilla SAEs struggle to learn modality-consistent concepts in VLMs, often exhibiting fragmented coverage in the visual modality. The authors aim to address this challenge by proposing a Structured Sparse AutoEncoder ($S^2AE$) that enforces concept consistency from both semantic and spatial perspectives.

Method: The authors propose $S^2AE$, which groups image patches based on Transformer attention similarity and spatial proximity, and introduces a structured sparsity regularization when training the vanilla SAE. The regularization consists of exclusive sparsity for inter-group concept disentanglement and group sparsity for intra-group concept consistency.

Result: The method achieves 6.06% average improvement in semantic alignment (mIoU) and 60.81 in representational efficiency (lower l0 norm) while maintaining near-perfect reconstruction fidelity with an Explained Variance above 99%. Cross-modal analysis further demonstrates that $S^2AE$ enhances neuronal monosemanticity, achieving a 3.08% average gain in semantic consistency and a 2.37% average gain in monosemanticity scores for both modalities of multimodal features.

Conclusion: The proposed $S^2AE$ method improves the performance of SAEs in VLMs by enforcing concept consistency in the visual modality, leading to more coherent and disentangled representations.
...
<details>
  <summary>Details</summary>
Motivation: The rapid advancement of generative AI poses significant threats such as misinformation and digital identity theft, making AI-generated image detection increasingly challenging.

Method: GenRes and GenRes++ utilize a neural tensor network and a learnable attention mechanism to model relational features between original and transformed samples, improving generalization and cross-domain performance.
...
Conclusion: The proposed GenRes++ approach effectively enhances the detection of AI-generated images, including those generated by unseen methods, through its novel framework and attention mechanism.
...
<details>
  <summary>Details</summary>
Motivation: Diabetic Retinopathy (DR) is a leading cause of preventable blindness worldwide, and automated lesion segmentation using deep learning models is crucial for early detection and monitoring. However, DR lesions vary dramatically in size, posing a challenge for effective model design.
...
Result: The paper finds that increasing input resolution has opposing effects on different lesion types. Higher resolution is essential for resolving fine-grained microaneurysms but can unexpectedly degrade performance on larger hemorrhages. The proposed Multi-Resolution Feature Stem architecture successfully resolves this trade-off.

Conclusion: The paper contributes crucial empirical evidence of the complex, resolution-dependent behavior of diabetic retinopathy lesion segmentation and proposes a practical, parameter-efficient architecture that addresses this challenge.
...
TL;DR: A new method, SAM-MT, is proposed for real-time multi-target video segmentation, achieving high frame rates and low latency.


<details>
  <summary>Details</summary>
Motivation: Current video object segmentation methods struggle with multi-target settings, experiencing reduced frame rates and increased latency as the number of targets grows.

Method: SAM-MT builds on Segment Anything 2 (SAM2) and uses explicit queries, decoupled masked attention, sparse memory, and specialized strategies for occlusion handling and overlap prevention.

Result: SAM-MT achieves real-time speed (>36 FPS for 10 targets) while maintaining robust video segmentation performance, decoupling latency from the number of targets.

Conclusion: The proposed SAM-MT method successfully enables real-time multi-target video segmentation with high performance and low latency.
...
TL;DR: A new dataset called HumanForge is introduced, which focuses on human-centric video forgery detection, addressing limitations in existing benchmarks.


<details>
  <summary>Details</summary>
Motivation: The rapid advancement of video diffusion models and temporal editing tools has made it challenging to detect realistic human-centric videos, highlighting the need for a comprehensive dataset.

Method: A modular active multi-agent pipeline called Gen2Anno is proposed to construct and annotate the HumanForge dataset, which includes six specialized agents for generating and annotating video segments.

Result: The HumanForge dataset and Gen2Anno pipeline demonstrate significant challenges for state-of-the-art detectors and Large Multimodal Models (LMMs) in terms of zero-shot generalization and fine-grained reasoning.

Conclusion: The HumanForge dataset and Gen2Anno pipeline provide a new resource for human-centric video forgery detection, highlighting the need for further research in this area.
...
TL;DR: Proposed a multi-modal framework for 3D terrain reconstruction using outdated DEMs and images, achieving high accuracy and efficiency for wildfire response.


<details>
  <summary>Details</summary>
Motivation: Accurate 3D terrain maps are crucial for emergency response in wildfire-prone areas, but conventional methods are limited by cost, update frequency, or performance.

Method: Physics-based pixel-pixel alignment between images and DEM data for image-based 3D reconstruction, eliminating expensive feature matching procedures.

Result: High-fidelity depth maps with significant improvements in reconstruction accuracy and computational efficiency over existing techniques.

Conclusion: The proposed framework offers a scalable solution for wildfire response, enabling real-time performance and high accuracy in 3D terrain reconstruction.
...
<details>
  <summary>Details</summary>
Motivation: Most 3D human pose estimators are optimized for geometric keypoint accuracy, but many real-world applications require biomechanical quantities that describe how the body moves, loads, and activates.

Method: BioModule, a lightweight plug-in temporal transformer, is attached downstream of any 3D pose estimator and predicts biomechanical attributes from standard 17-joint 3D skeletons.

Result: BioModule predicts biomechanical quantities and provides a compact, modular bridge between vision-based pose estimation and biomechanically meaningful human motion analysis.

Conclusion: BioModule positions itself as a solution to extend existing pose estimators toward physically interpretable motion analysis, providing a systematic analysis of how upstream pose estimation quality propagates to downstream biomechanical prediction fidelity.
...
TL;DR: A new benchmark called WaspMOT is introduced for multi-object tracking (MOT) with a focus on long-term identity preservation. It consists of long video sequences of Trichogramma wasps and evaluates the performance of various tracking methods.


<details>
  <summary>Details</summary>
Motivation: Current MOT benchmarks are dominated by short video sequences and do not adequately evaluate long-term identity preservation. The authors aim to address this gap by creating a benchmark that requires consistent object tracking over extended durations.

Method: The authors evaluate five tracking-by-detection methods (ByteTrack, BoT-SORT, C-BIoU, OC-SORT, and McByte) on the WaspMOT benchmark using a unified protocol. They also propose a simple spatial tracklet stitching baseline.

Result: The results show that all evaluated methods suffer from significant trajectory fragmentation, highlighting the difficulty of long-term identity preservation. The simple spatial tracklet stitching baseline consistently improves performance, indicating potential for substantial gains.

Conclusion: WaspMOT provides a new benchmark for studying long-term association and reveals limitations of current tracking approaches that are not observable on conventional datasets. The benchmark will be made publicly available.
...
TL;DR: The paper introduces OpenCoF, a framework for Chain-of-Frame (CoF) reasoning in video generation models, which includes a dataset and a fine-tuned model that improve CoF behavior.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enhance the reasoning capabilities of large models, particularly in understanding logical consequences through temporally connected frames.

Method: The method involves introducing a new dataset, OpenCoF-17K, and a fine-tuned video model, Wan-CoF, which utilizes diverse temporal supervision and explicit mechanisms for organizing intermediate reasoning states.

Result: The results show that Wan-CoF achieves considerable gains over the baseline across four video reasoning benchmarks, and that equipping the model with visual and textual reasoning tokens improves performance.

Conclusion: The conclusion is that stronger video reasoning requires both broad temporal supervision and explicit mechanisms for organizing intermediate reasoning states.
...
TL;DR: Canvas360 is a two-stage framework for in-context panoramic generation, combining geometry-aware pretraining with task-specific fine-tuning, achieving improved panoramic image fidelity and flexibility.
...
Method: Canvas360, a two-stage framework that includes geometry-aware pretraining, parallel depth generation, velocity circular padding, and similarity loss regularization, along with the Canvas360Dataset, a collection of 1M high-quality paired panoramic samples.
...
Conclusion: Canvas360 enables a unified in-context panoramic generation framework that supports diverse downstream tasks via token-level concatenation, surpassing prior methods in task coverage and modeling flexibility.
...
TL;DR: OPSD-V improves few-step autoregressive video diffusion models by introducing real long-video data for dense supervision, reducing long-horizon degradation while preserving fast inference.
...
Method: OPSD-V uses on-policy self-distillation with real long-video data as temporal context, providing dense trajectory-level supervision without changing the sampler or inference-time cache mechanism.

Result: Consistent improvements in visual quality, motion dynamics, and VBenchLong scores; 66.0% preference in user study with 10 participants.

Conclusion: OPSD-V effectively reduces long-horizon degradation in few-step AR video generators, preserving fast inference while improving video quality and motion dynamics.
...
Result: PanoLOG achieves state-of-the-art rendering quality while maintaining scalable, block-parallel training on large-scale panoramic datasets.

Conclusion: PanoLOG enables efficient large-scale panoramic 3DGS reconstruction with high rendering quality, and the benchmark Pano360 is constructed for future research.
...
TL;DR: Proposed LongE2V, a novel approach for recovering high-quality video from sparse event streams, leveraging pre-trained video diffusion priors.


<details>
  <summary>Details</summary>
Motivation: Existing methods for video reconstruction, prediction, and frame interpolation from sparse event streams often suffer from blurring textures or struggling with long-term stability.

Method: LongE2V uses fine-tuned foundational video models with novel techniques: Autoregressive Unrolling, Adaptive Context Switching, Reencoding Alignment with Cross Residual Correction, and Event Voxel Density Augmentation.

Result: LongE2V outperforms state-of-the-art methods in event-based video reconstruction, prediction, and frame interpolation, showing exceptional temporal coherence and zero-shot generalization.

Conclusion: LongE2V achieves high data efficiency and superior perceptual quality, demonstrating robustness across varying sensor resolutions and real-world benchmarks.
...
TL;DR: ZipDepth, a compact monocular depth estimation network, achieves a balance between accuracy and efficiency, enabling real-time deployment on various devices.
...
Conclusion: ZipDepth takes a significant step towards matching the accuracy of foundation models with much more parameters, while being suitable for deployment on power-constrained devices.
...
TL;DR: Proposed Wat3R, a cross-domain semi-supervised framework for 3D reconstruction in underwater environments without annotated data.


<details>
  <summary>Details</summary>
Motivation: Estimating 3D geometry in underwater environments is challenging due to light attenuation, scattering, and lack of high-quality annotations.

Method: Wat3R uses a teacher-student architecture with cross-view consistency loss, leveraging unlabeled real underwater video footage.
...
Conclusion: Wat3R enables effective 3D reconstruction in underwater environments without annotated data, with a new dataset and code available.
...
TL;DR: The paper compares machine learning and deep learning approaches for sentiment analysis on Twitter, finding that an LSTM model outperforms traditional methods with high accuracy.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to improve sentiment analysis on Twitter by evaluating various machine learning and deep learning approaches, driven by the need to extract meaningful insights from the vast amount of user-generated content.

Method: The paper evaluates the performance of logistic regression, random forest, naïve bayes, gradient boosting, and LSTM networks, among other algorithms, using a Kaggle Twitter dataset that has been preprocessed through tokenization, lemmatization, and stopword elimination.

Result: The LSTM model achieved a training accuracy of 90.98%, a testing accuracy of 80.00%, and a micro-average ROC-AUC score of 0.92, outperforming conventional machine learning techniques.

Conclusion: The paper concludes that the LSTM approach is more effective in capturing contextual and sequential textual aspects, making it a suitable model for sentiment analysis on Twitter.
...
TL;DR: The paper discusses the limitations of current AI systems in mathematics (AI4Math) and proposes a shift towards research agents that can tackle frontier mathematical challenges with rigorous formal reasoning.


<details>
  <summary>Details</summary>
Motivation: Current AI4Math systems are limited in addressing open-ended, under-specified mathematical problems, such as discovering new theorems or resolving open conjectures.
...
Result: The authors outline a strategic road-map for the future of AI4Math, emphasizing the need for a shift from predefined problem-solvers to research agents.

Conclusion: The next leap in AI4Math systems requires a decisive shift towards research agents that can address frontier mathematical challenges with rigorous formal mathematical reasoning.
...
TL;DR: The authors propose DeepSearch-Evolve, a self-distillation framework for training web agents in a deterministic environment called DeepSearch-World, achieving competitive performance without relying on more capable models.


<details>
  <summary>Details</summary>
Motivation: Training agents to improve from their own experience remains challenging due to limitations in supervised fine-tuning and sparse-reward reinforcement learning.

Method: DeepSearch-Evolve uses a self-distillation framework, iteratively generating trajectories, filtering, data mixing, and fine-tuning to train stronger agents in DeepSearch-World.

Result: DeepSearch-World-9B achieves 31.2% on BrowseComp, 61.5% on GAIA, and 93.4% on HotpotQA, demonstrating competitive performance without distillation from more capable models.

Conclusion: Verifiable environments like DeepSearch-World enable scalable self-evolution for long-horizon web agents, and the authors will release the environment, training pool, validation set, model, and code to facilitate future research.
...
TL;DR: This paper combines Roy Harris's Integrationist linguistics with Elan Barenholtz's autogenerative theory to fill explanatory gaps in Integrationism, providing a more comprehensive understanding of language.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address limitations in Integrationist linguistics by incorporating elements from autogenerative theory, specifically to explain the structural mechanism of signs, semiotic continuity, and the archive of past integrations.

Method: The paper synthesizes Integrationist linguistics with autogenerative theory, using the latter to fill explanatory gaps in the former, particularly in relation to Large Language Models (LLMs).

Result: The combined approach provides a structural mechanism for prospective openness in communication, a computational correlate for semiotic continuity, and a theory of the archive of past integrations.

Conclusion: The synthesis of Integrationist linguistics and autogenerative theory enriches our understanding of language, offering insights into the statistical structure exploited by LLMs and its limitations.
...
TL;DR: Researchers created a dataset called EspanStereo to study stereotypes in Spanish-language large language models, revealing culturally specific biases and variations across countries.


<details>
  <summary>Details</summary>
Motivation: The lack of datasets in languages other than English and the high cost of manual annotation in underrepresented cultures have limited research on stereotypes in large language models.

Method: A cost-efficient human-LLM collaborative annotation framework was developed and applied to construct the EspanStereo dataset.

Result: The evaluation of Spanish-supporting LLMs using EspanStereo showed significant variation in stereotypical behavior across countries.

Conclusion: The work broadens the scope of stereotype analysis in LLMs and lays the groundwork for comprehensive cross-cultural bias evaluation.
...
<details>
  <summary>Details</summary>
Motivation: The authors aim to investigate the effectiveness of preprocessing-based methods for stereotype mitigation in NLP and identify potential side effects.

Method: The authors analyze two model families (encoder-only and decoder-only), multiple preprocessing strategies, and both pre- and post-training at different data scales on Wikipedia.

Result: The authors find that preprocessing methods can induce unintended shifts, causing stereotyping or counter-stereotyping to increase for other demographics, and that standard benchmarks often miss these shifts.

Conclusion: The authors argue for side-effect-aware, transparent mitigation practices and provide actionable diagnostics for evaluating and mitigating stereotype side effects in NLP.
...
TL;DR: Proposes a multi-cluster boundary learning method using MiniLM embedding for out-of-scope (OOS) intent detection in human-machine interaction systems.


<details>
  <summary>Details</summary>
Motivation: Traditional methods for OOS intent detection face challenges such as decreased accuracy with increasing known intent classes and large parameter requirements for LLM-embedding methods.

Method: A multi-cluster boundary learning method using MiniLM embedding in a one-class classification workflow to detect OOS intents.

Result: Achieves state-of-the-art OOS intent detection performance on CLINC150, StackOverflow, and Banking77 datasets.

Conclusion: The proposed method effectively detects OOS intents and shows better adaptation to the workflow and utterance embedding requirements using MiniLM.
...
TL;DR: The paper proposes TACO, a method to improve reinforcement learning for large language models by mitigating 'Positive-Credit Contamination', which incorrectly rewards erroneous tokens.


<details>
  <summary>Details</summary>
Motivation: Critic-free RL methods have a flaw called 'Positive-Credit Contamination' where erroneous low-probability tokens receive the same positive credit as plausible ones, leading to flawed reasoning.

Method: The authors propose Tail-Aware Credit calibratiOn (TACO), which calculates a 'tail-risk score' to adjust credit assignment, distinguishing between reliable and unreliable tokens.

Result: TACO outperforms GRPO-style baselines across three LLMs and eight benchmarks, improving training stability and performance in long-horizon RL.

Conclusion: TACO effectively mitigates 'Positive-Credit Contamination', leading to more accurate and stable reinforcement learning for large language models.
...
TL;DR: The study evaluates the reliability of Gemini models as audio judges for scoring full-duplex agent conversations, comparing their performance to human raters across various dimensions.


<details>
  <summary>Details</summary>
Motivation: To assess the empirical reliability of Gemini models in scoring conversations directly from raw stereo waveforms, in order to potentially substitute or supplement human rating.

Method: The study uses three Gemini models (2.5 Flash, 3.5 Flash, and 3.1 Pro) and compares their performance to three calibrated human raters on 209 stereo sessions across 8 production dimensions.

Result: Gemini 2.5 Flash shows consistent evidence of reliability across three tests, with high Spearman rho and agreement with human raters on most dimensions. The results also show that 3.5 Flash improves simple agreement, while 3.1 Pro rates some dimensions lower than humans despite comparable rank correlation.

Conclusion: The study provides a defensible empirical basis for deploying Gemini models as substitutes or fourth raters on dimensions where the evidence supports it, but highlights the need for careful deployment and re-validation for model swaps.
...
TL;DR: A novel framework called Hallucination Self-Play (HSP) is proposed to improve the detection of faithfulness hallucinations in LLM-generated outputs by iteratively enhancing a detector and a generator.


<details>
  <summary>Details</summary>
Motivation: The scarcity of high-quality annotated data for identifying faithfulness hallucinations in LLM-generated outputs and the limitations of current methods that treat the generator as a static component.

Method: Hallucination Self-Play (HSP) framework, which involves a detector and a generator, both initialized from the same base model. The detector is fine-tuned on human-labeled data and then used as a reward model to train the generator via reinforcement learning from AI feedback (RLAIF).

Result: The proposed framework can progressively enhance a small LLM to match or even outperform advanced LLMs without external supervision, as demonstrated on the RAGTruth benchmark and two model families.

Conclusion: The Hallucination Self-Play framework offers a promising approach to improving the detection of faithfulness hallucinations in LLM-generated outputs by enabling iterative improvement of the detector and generator.
...
TL;DR: The paper introduces a framework to measure educational control in Large Language Models (LLMs) and evaluates their ability to adapt programming tasks for learners by adjusting cognitive demand.


<details>
  <summary>Details</summary>
Motivation: The motivation is to study the gap between solving tasks and adapting them for learners in computer science education, specifically in Large Language Models (LLMs).

Method: The method involves using a revised Bloom's Taxonomy to evaluate LLMs' ability to shift cognitive demand toward specified learning objectives. The authors apply this framework to programming tasks and compare two models: Qwen3-Next-80B-A3B-Instruct and Qwen3-Coder-Next.

Result: The results show a robust directional asymmetry: both models can increase cognitive demand but struggle to lower it. The general model shows clearer middle-layer separability for both general difficulty and Bloom-control contrasts, whereas the coder model shows weaker separability for general difficulty and a deeper peak for Bloom-control contrasts.

Conclusion: The conclusion is that strong execution performance does not automatically entail Bloom-aligned educational control, highlighting the need for more research in developing LLMs that can effectively adapt tasks for learners.
...
TL;DR: The authors propose a tool-making pipeline for LLM agents to reduce latency and improve reliability by compiling repeated steps into validated tools, and show significant improvements in a production system.


<details>
  <summary>Details</summary>
Motivation: Production LLM agents waste latency and reliability by regenerating code for the same procedural steps on every request, leading to inefficiencies.

Method: The authors introduce an agentic tool-making pipeline that compiles repeated SOP steps into validated, versioned tools before deployment, and grounds synthesis in the live environment.

Result: The approach reduces p50 latency by 42% and end-to-end error rate by up to 53% in a production Fulfillment Center alarm-triage system, and also improves auditability and exposes specification gaps and upstream data drift.

Conclusion: The authors conclude that self-evolving agents can make industrial LLM systems faster, more reliable, and easier to operate by leveraging tool-making pipelines.
...
TL;DR: The paper introduces GRAPHEVAL, a graph-based framework for evaluating and improving the faithfulness and uncertainty of Large-Language Models' (LLMs) reasoning processes.


<details>
  <summary>Details</summary>
Motivation: Current decoding strategies like Self-Consistency (SC) fail to detect flawed and unfaithful reasoning in LLMs, highlighting the need for reliable uncertainty quantification and faithful reasoning.

Method: The authors propose a novel uncertainty quantification metric, Graph Reasoning Coherence Score (GRCS), and a decoding strategy, Graph Self-Consistency (GSC), which uses a medoid-based approach to select more faithful reasoning paths.

Result: GRCS is found to be consistently negatively correlated with reasoning faithfulness, and GSC trades nominal accuracy for reasoning fidelity, preserving or improving accuracy in more capable models.

Conclusion: The paper presents GRAPHEVAL as a holistic framework for evaluating and improving LLM reasoning faithfulness and uncertainty, with GRCS and GSC as key components for achieving more reliable and faithful reasoning.
...
TL;DR: The paper proposes an improved structured pruning method for large language models, addressing challenges in adapting an unstructured pruning technique to structured pruning, and demonstrates comparable accuracy with practical inference speedup.
...
Method: The method involves a unified approach combining power transformation for nonlinear distribution alignment, sign-preserving score aggregation, and percentile-based outlier removal.

Result: The results show that the proposed method maintains accuracy comparable to unstructured pruning while achieving practical inference speedup through structured pruning on Llama-3-8B, Vicuna-v1.5-13B, and LLaVA-v1.5-13B.

Conclusion: The conclusion is that the proposed method effectively addresses the challenges in adapting AFR to structured pruning, achieving comparable accuracy and practical inference speedup.
...
TL;DR: The paper introduces PLURAL, a dataset of value-focused preferences for large language models (LLMs) to better represent diverse value systems worldwide.


<details>
  <summary>Details</summary>
Motivation: Current LLMs disproportionately reflect Western values, limiting their ability to represent diverse value systems.

Method: The authors create PLURAL using a two-stage generation pipeline, transforming survey responses from the Integrated Values Survey (IVS) into synthetic preference triplets.

Result: PLURAL preserves cross-country value differences and within-country diversity, and training on it improves alignment with target countries' cultural profiles.

Conclusion: PLURAL offers a scalable resource for pluralistic alignment, enabling LLMs to better represent diverse value systems worldwide.
...
TL;DR: This paper explores using internal representations of large language models to improve calibration and faithfulness of forecasts and reasoning.


<details>
  <summary>Details</summary>
Motivation: Large language models can be accurate but poorly calibrated, and their reasoning may not reflect the evidence behind a forecast.

Method: The authors train representation-pooling probes on intermediate activations of language models and assess CoT faithfulness through evidence ablation and diversionary injection.

Result: The probes achieve better calibration, track behavioral shifts, and predict the direction of change in forecasts, revealing that forecasts are largely fixed before reasoning begins.

Conclusion: Probing internal representations is a practical tool for calibrating, auditing, and triaging language model forecasters and reasoning models.
...
TL;DR: The paper proposes Holographic Neural PCFG (Hol-PCFG), a model for unsupervised constituency parsing that achieves state-of-the-art performance while reducing parameters and improving stability.


<details>
  <summary>Details</summary>
Motivation: Recent neural parameterizations of PCFGs rely on high-capacity black-box networks, making rule probabilities uninterpretable. The authors aim to create a more interpretable model.

Method: The authors propose Hol-PCFG, which uses holographic embeddings to score PCFG rules as algebraic relations among grammar-symbol embeddings, resulting in closed-form rule probabilities.

Result: Hol-PCFG achieves state-of-the-art parsing performance in six languages, reduces rule-scoring parameters by 99.94%, and trains more stably. It can also parse Japanese directly from characters without morphological segmentation.

Conclusion: The proposed Hol-PCFG model improves upon existing neural PCFG models by providing interpretable rule probabilities, reducing parameters, and achieving state-of-the-art performance in unsupervised constituency parsing.
...
TL;DR: Proposed a novel method for generating optimized and customized ad headlines using prefix control tokens and BART fine-tuning, achieving higher CTR and flexibility across ad formats.
...
Result: Achieved a 25.82% increment in Rouge-L and a 5.82% increment in estimated CTR over previous baseline.

Conclusion: The proposed method yields the highest CTR, allows length control, and is flexible for adaptation to other architectures and optimization criteria.
...
TL;DR: The paper proposes MASTE, a multi-agent pipeline for zero-shot Aspect Sentiment Triplet Extraction (ASTE), outperforming existing methods without requiring labeled training data.


<details>
  <summary>Details</summary>
Motivation: The authors are motivated by the limitations of large language models (LLMs) in ASTE, which requires identifying (aspect, opinion, sentiment) triples from review sentences. Existing methods have shown marginal improvements and rely heavily on labeled training data or engineered prompts.

Method: The authors propose MASTE, a multi-agent pipeline that decomposes ASTE into four sequential stages, with specialized agents handling different subtasks and explicitly conditioning on prior outputs.

Result: MASTE substantially outperforms zero-shot and chain-of-thought LLM baselines on four ASTE benchmarks, narrowing the gap to fully supervised methods without using any labeled triplets.

Conclusion: The proposed MASTE pipeline enables entirely training-free zero-shot ASTE and generalizes across different backbones and datasets, making it a promising approach for Aspect Sentiment Triplet Extraction.
...
TL;DR: The paper proposes COALA, a framework for enhancing speech-augmented language models in complex multi-entity scenarios, achieving superior contextual biasing performance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve automatic speech recognition (ASR) systems by integrating external knowledge to accurately recognize domain-specific entities, particularly in complex multi-entity scenarios.

Method: The method proposed is COALA (Contextualized ASR Leveraging Biasing Scoring), which maps SLM latent representations into a specialized discriminative space to quantify the matching intensity between audio segments and candidate entities.
...
Conclusion: The conclusion is that COALA is a robust framework that enhances speech-augmented language models in complex multi-entity scenarios, achieving superior contextual biasing performance.
...
TL;DR: The HIPE-OCRepair-2026 competition evaluated LLM-assisted OCR post-correction systems for historical documents, providing a dataset and framework for future research.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the challenge of OCR post-correction in digital heritage, where large-scale collections of digitized documents are affected by legacy OCR errors.

Method: The method used in this paper involves evaluating the capabilities of modern OCR post-correction systems using a harmonized multilingual resource, the HIPE-OCRepair-2026 dataset, and a reproducible evaluation framework.

Result: The results show that modern LLM-assisted systems can significantly improve OCR quality, but performance varies across datasets, languages, and noise levels, with over-correction on low-noise inputs emerging as a recurring challenge.

Conclusion: The conclusion of this paper is that LLM-assisted OCR post-correction systems have the potential to improve OCR quality, but further research is needed to address the challenges of over-correction and variability in performance across different datasets, languages, and noise levels.
...
<details>
  <summary>Details</summary>
Motivation: The stark gap in performance between text-aware models and gaze-only models on the EyeBench benchmark, with gaze-only models operating at chance, motivates the need to improve gaze-only models.

Method: The paper proposes two mechanisms: LEXIC-Concat, which directly concatenates precomputed word-level difficulty signals into the per-fixation input, and LEXIC-Res, which uses a residual mechanism to condition the encoder on the deviation from typical-reader gaze response.

Result: Both mechanisms produce statistically consistent AUROC gains on Unseen Text (+1.8 to +2.2 percentage points) and LEXIC-Concat additionally lifts Unseen Reader by +2.9 percentage points.

Conclusion: The proposed mechanisms can push the performance of gaze-only models, but an architectural boundary is found in LEXIC-Res on Unseen Reader, highlighting the need for further improvement.
...
TL;DR: A novel approach called SQuaD-SQL is introduced to improve the performance of small language models on the Text-to-SQL task, approaching that of large language models while being more efficient.


<details>
  <summary>Details</summary>
Motivation: The substantial computational requirements of large language models hinder their deployment in resource-constrained settings, making it necessary to find efficient alternatives.

Method: SQuaD-SQL uses LLM-based synthetic data generation, parameter-efficient fine-tuning, and domain-adaptive fine-tuning to empower small language models.

Result: SQuaD-SQL achieves an execution accuracy of 86.9% on the WikiSQL dataset test set, with faster inference and lower memory usage compared to large language models.

Conclusion: Small language models can serve as practical and efficient alternatives for Text-to-SQL applications in resource-limited environments with proper training strategies.
...
TL;DR: The paper proposes Hidden Decoding, a method to scale large language models by expanding the sequence-length dimension without modifying the Transformer backbone, achieving improved results at large scales.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve existing large language models without requiring costly retraining from scratch by exploring sequence-length scaling.

Method: The method proposed is Hidden Decoding, which expands each token into multiple streams with independent embedding tables and uses Stream-Factorized Attention to efficiently manage the increased computation.

Result: Experiments show that Hidden Decoding improves over non-HD baselines, with gains growing as the expansion factor increases, demonstrating its effectiveness as a scaling method for large language models.

Conclusion: The conclusion is that sequence-length expansion is a practical scaling path for frontier-scale LLMs without needing to change the Transformer backbone, validated through experiments at large scales.
...
TL;DR: The paper presents a self-designed system for multilingual two-speaker conversational speech, combining a modular speaker diarization front end with an adapted ASR model, achieving significant error rate reductions.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the challenge of multilingual two-speaker conversational speech in the MLC-SLM 2026 Challenge, by designing an effective system that can accurately recognize speech and attribute it to the correct speakers.

Method: The system consists of a modular speaker diarization front end and a challenge-adapted Qwen3-ASR-1.7B recognizer. The diarization front end performs various tasks such as voice activity detection, speaker embedding extraction, and audio segmentation. The ASR model is adapted through supervised fine-tuning, LoRA fine-tuning with synthetic speech, and reinforcement learning.

Result: The system achieves an average tcpMER of 23.70 on the official development set, reducing the error rate by 6.83 absolute points relative to the released Qwen-ASR-1.7B performance. On the final evaluation set, the system achieves an average tcpMER of 17.97.

Conclusion: The paper concludes that the proposed system is effective in addressing the challenge of multilingual two-speaker conversational speech, with ablation results showing that supervised fine-tuning provides the largest gain, while synthetic-speech LoRA adaptation and reinforcement learning further improve robustness.
...
TL;DR: The paper improves zero-shot text-to-speech by selecting from N candidates using an automatic speech recognition verifier, and proposes cross-family rank ensembles to attain the lowest mean WER across evaluators.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve content consistency in zero-shot text-to-speech by addressing the underexplored evaluation confound of verifier quality depending on the ASR family that judges it.
...
Result: The results show that the proposed ensembles attain the lowest mean WER across three independent evaluators (1.61% at N=10), with no measurable degradation under automatic SIM-o/UTMOS metrics.

Conclusion: The conclusion is that cross-evaluator triangulation is recommended as a default reporting practice, and the proposed ensembles can improve zero-shot text-to-speech by selecting from N candidates using an automatic speech recognition verifier.
...
TL;DR: Introduces XAlpha, an AI system for continuous alpha discovery in financial markets, which integrates a memory system to guide research and improve performance.


<details>
  <summary>Details</summary>
Motivation: Existing methods for alpha discovery in financial markets are limited as they automate isolated steps rather than functioning as end-to-end systems that can absorb external knowledge and learn from feedback.

Method: XAlpha uses a multi-source research memory system and three interconnected brains (Macro, Micro, Cross) to plan research, generate and verify trading signals, and consolidate feedback for continuous improvement.

Result: XAlpha achieves stronger overall alpha discovery performance than representative baselines in experiments on CSI300.

Conclusion: XAlpha presents a novel approach to alpha discovery by integrating a closed-loop research process that continuously reads, hypothesizes, implements, validates, reflects, and evolves, demonstrating improved performance.
...
TL;DR: This paper investigates how well pretrained code models internally encode type information, finding that cross-lingual type representations emerge even from untyped code.
...
Result: Cross-lingual type representations emerge from untyped code, and hidden states linearly encode result types implied by typed function application.

Conclusion: The study shows that code models can internally represent type information, even across languages, and that this structure is partly robust to lexical perturbations and syntactic variations.
...
TL;DR: The authors propose a two-stage system to finely label 8-K disclosures from U.S. public companies with a three-tier taxonomy of 119 event types, achieving high precision and releasing 601,088 grounded event tags.


<details>
  <summary>Details</summary>
Motivation: The SEC item codes for 8-K filings are coarse and do not accurately reflect the specific events disclosed, making it difficult to understand the market impact of these events.

Method: The authors present a two-stage system: the first stage uses fuzzy n-gram validation to anchor tags to verbatim quotes, and the second stage re-grades each quote against the category definition to produce a quality score.

Result: The system yields 601,088 grounded event tags with precision rising monotonically with the quality score, from 12% to 96%, and an event study confirms that the taxonomy separates economically distinct events.

Conclusion: The proposed system enables fine-grained labelling of 8-K disclosures, providing a more accurate understanding of market-moving events and their impact.
...
TL;DR: The paper introduces CKTN, a corpus and benchmark for NLP in Vietnam's ethnic minority languages, and proposes a script-aware adaptation method to improve multilingual encoder performance.


<details>
  <summary>Details</summary>
Motivation: The absence of Vietnam's ethnic minority languages in NLP, due to data scarcity and linguistic differences, which can lead to incorrect signals in standard multilingual adaptation.

Method: A script-aware adaptation recipe, combining vocabulary augmentation and calibrated replaced-token pretraining, to prevent exploitation of trivial script mismatches.
...
Conclusion: The paper highlights the challenges of NLP in Vietnam's ethnic minority languages and proposes a solution to improve multilingual encoder performance, exposing the limits of lexical-overlap retrieval as an evaluation signal.
...
<details>
  <summary>Details</summary>
Motivation: Traditional personality recognition methods are limited by theory-dependent formulations, which hinder generalization and only reflect partial views of latent traits.

Method: The JAM framework uses an Attention-Pooled Graph Prototypical Network and a Cross-Theory Harmonization approach to learn generalizable psychological representations from textual samples.

Result: JAM improves cross-framework generalization and performance, supporting low-resource personality theories and establishing a strong step toward theory-agnostic personality inference.

Conclusion: The proposed JAM framework offers a promising approach for theory-agnostic personality recognition, enabling the discovery of unified latent psychological structures and improving generalization across different personality theories.
...
TL;DR: The paper explores compressing task-relevant information from an instruction prompt into a single activation vector and re-injecting it into a large language model (LLM), achieving an accuracy drop of under 2% compared to full prompt processing.


<details>
  <summary>Details</summary>
Motivation: To investigate if task-relevant information in an instruction prompt can be compressed into a single activation vector and re-injected into an LLM, potentially reducing computation for fixed prompts.
...
Result: The compressed vector preserves task-relevant information with an accuracy drop of under 2% relative to full prompt processing. The analysis also reveals structure in the activation space of LLMs, including cross-layer compatibility and the ability to encode semantic information.

Conclusion: The study demonstrates that compressing task-relevant information into a single activation vector is achievable and effective, with practical implications for reducing computation in LLMs.
...
TL;DR: The paper presents a reinforcement learning approach, Group Relative Policy Optimization (GRPO), to adapt LLM-based ASR to synthetic speech, achieving a 40% relative reduction in WER compared to supervised fine-tuning (SFT).


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the bottleneck of collecting real speech data in regulated domains like banking, which is costly and legally constrained, by utilizing synthetic text-to-speech (TTS) and improving its acoustic mismatch with real recordings.

Method: The method used is reinforcement learning, specifically Group Relative Policy Optimization (GRPO), which is compared to supervised fine-tuning (SFT) and shown to be more effective in adapting to synthetic speech.

Result: The result shows that GRPO reduces WER by 40% relative to SFT (from 36.71% to 22.09%), and an SFT-then-GRPO combination further reduces WER by 45%.

Conclusion: The conclusion is that when synthetic speech is the main resource, reinforcement learning should be preferred over supervised fine-tuning for adapting LLM-based ASR, as it can extract more from the same synthetic speech and improve speech-to-text alignment.
...
TL;DR: A new method called ESBMC-LLB is presented for detecting malicious control logic, or 'Ladder Logic Bombs', in Programmable Logic Controller (PLC) programs. It improves upon existing verification tools by exposing function-block logic and using formal verification to detect bombs.


<details>
  <summary>Details</summary>
Motivation: Existing methods for detecting malicious logic in PLC programs have limitations, as they often ignore function-block bodies where malicious logic can hide. This leads to the need for a more comprehensive approach to detect such threats.

Method: ESBMC-LLB uses a verification engine called ESBMC-PLC+ and adds a modeling layer to expose function-block logic. It employs k-induction for unbounded proof of bomb absence and bounded model checking to return counterexamples that are the triggers for malicious logic.

Result: The method was tested on two datasets: Iacobelli 2024 and PLC-Defuser's SWaT corpus. On the former, ESBMC-LLB detected all 30 bombs and recovered every trigger. On the latter, it detected 149/150 bombs (99%) with zero false positives, recovering each trigger, although performance dropped on a version with nonlinear non-termination bombs.

Conclusion: The study concludes that semantic model checking and CFG-triage are complementary methods. Semantic model checking provides unbounded proofs and robustness against adaptive triggers, handling Boolean/integer and linear analog logic, while CFG-triage is better suited for nonlinear analog non-termination.
...
<details>
  <summary>Details</summary>
Motivation: The authors aim to address the issue of language models providing incorrect or irrelevant answers to questions, and to develop a more robust and reliable question-answering system.

Method: The authors analyze the performance of five instruction-tuned models from three families (2B to 14B) and propose a calibrated policy that uses two separate scores for answerability and correctness to filter out incorrect or unanswerable questions.

Result: The results show that the proposed approach outperforms existing methods, achieving a higher precision in identifying unanswerable questions and providing accurate answers. Specifically, the calibrated policy certifies both budgets at 0.75 coverage of correct answers, against 0.31 for a single threshold; at 14B it is the only policy that certifies at all.

Conclusion: The paper concludes that the proposed approach can effectively improve the performance of language models in question-answering tasks, and that the use of two separate scores for answerability and correctness is a key factor in achieving this improvement.
...
TL;DR: The paper presents a new method for extracting universal features from independently trained BERT models, addressing the challenge of cross-seed feature universality in mechanistic interpretability.
...
Result: The proposed pipeline produces more universal features (Pearson r ≥ 0.70 across seeds) than post-hoc alignment baselines on all three benchmark datasets (SST-2, Stanford Politeness, TweetEval Emotion).

Conclusion: The proposed method successfully extracts cross-seed universal features from independently trained BERT models, enabling the identification of interpretable sociolinguistic patterns.
...
TL;DR: The study examines the impact of changing evaluators (LLM-as-judge) on scoring candidate responses, treating evaluator-replacement ambiguity as a measurement-validity problem. It compares two upgrade paths for evaluators and finds that judge upgrades are not interchangeable, with only certain upgrades providing robust gains. The study also identifies biases and limitations in LLM-as-judge evaluations.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the evaluator-replacement ambiguity in LLM-as-judge scores, which can change even when candidate responses remain fixed, simply due to a change in the evaluator. The authors aim to investigate this issue as a measurement-validity problem.

Method: The authors compare two upgrade paths for evaluators: scaling Qwen3 dense judges from 1.7B to 32B parameters and moving across MiniMax M2-M2.7 released APIs. They analyze the impact of these upgrades on four judgment datasets.

Result: The study finds that judge upgrades are not interchangeable, with only Qwen3 1.7B to 4B providing a robust adjacent gain. Stronger judges reduce but do not remove position and verbosity bias. Repeated-sample juries add little when errors are correlated. Structured debate can move decisions substantially, but without parser and fallback logs, those shifts cannot be attributed to deliberation.

Conclusion: The authors conclude that LLM-as-judge reports should include dataset slices, bias probes, error-dependence estimates, and protocol audit trails to ensure the validity and reliability of evaluations.
...
TL;DR: MAESTRO, a structured pruning framework for Mixture-of-Experts (MoE) language models, outperforms state-of-the-art baselines by up to 10.61% in average performance retention under a 50% compression regime.


<details>
  <summary>Details</summary>
Motivation: Existing structured pruning methods for dense transformers are not suitable for MoE language models, as they don't consider the interdependent nature of MoE routing, creating a deployment bottleneck.
...
Result: MAESTRO outperforms state-of-the-art baselines by up to 10.61% in average performance retention under a strict 50% compression regime, with lower cross-task variance.

Conclusion: MAESTRO provides a more effective and consistent pruning approach for MoE language models, yielding better performance and generalizability across heterogeneous tasks.
...
<details>
  <summary>Details</summary>
Motivation: The paper aims to improve the efficiency of Large Language Model (LLM) inference by enhancing speculative decoding techniques, specifically by addressing limitations in existing drafters like DFlash and DDTree.

Method: The authors introduce DominoTree, which utilizes a training-free best-first draft tree scored by Domino's conditional, non-factorized correction along each root-to-node path, restricted to a candidate top-M.

Result: DominoTree achieves up to 6.6x speedup over autoregressive decoding, with the highest mean accept length of any evaluated method (up to 10.7 tokens per round), and wins throughput over existing methods, including Domino and DDTree/CaDDTree.

Conclusion: DominoTree offers improved efficiency and performance in LLM inference, demonstrating its potential as a superior speculative decoding method.
...
TL;DR: A new framework called UltraX is proposed to improve the quality and efficiency of large-scale pre-training data for Large Language Models (LLMs) by introducing a function-calling refinement approach.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of existing refinement methodologies for large-scale corpora, which face challenges in quality, efficiency, and reliability.

Method: UltraX introduces a function-calling refinement framework that includes insertion, deletion, and modification operations, enabling fine-grained instance-level editing. It also builds a reliable program-supervision generation pipeline and improves supervision quality with low-confidence example filtering and ratio-controlled sampling.
...
Conclusion: UltraX offers a more efficient and reliable approach to refining large-scale pre-training data for LLMs, leading to improved performance and data efficiency.
...
TL;DR: The paper proposes WebSwarm, a progressive recursive delegation framework for large language model-based web search agents, which improves upon existing methods by enhancing recursive depth, collaboration adaptability, and evidence-grounded expansion.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the limitations of existing search agents, specifically their inability to handle depth and coverage simultaneously, and to improve upon existing multi-agent systems that have limitations in recursive depth, collaboration adaptability, and evidence-grounded expansion.

Method: The method proposed in this paper is WebSwarm, a progressive recursive delegation framework that constructs task decomposition, recursive expansion, and agent collaboration during inference. It dynamically instantiates agentic search nodes that can either solve their objectives themselves or delegate child nodes.

Result: The results show that WebSwarm consistently outperforms single-agent and multi-agent baselines on deep, wide, and interleaved deep-and-wide tasks across various experiments, including BrowseComp-Plus, WideSearch, DeepWideSearch, and GISA.

Conclusion: The conclusion of this paper is that WebSwarm is an effective framework for enhancing the capabilities of large language model-based web search agents, offering improvements in handling complex search tasks that require depth and coverage.
...
TL;DR: The paper studies the calibration of LLM judges for evaluating citation quality in deep-research systems, finding that cheaper models can be competitive with more expensive ones, but may have different biases.


<details>
  <summary>Details</summary>
Motivation: The increasing reliance on LLM judges as reward models in reinforcement learning requires understanding their capability and bias, particularly in evaluating citation quality.

Method: The study evaluates 8 off-the-shelf LLM judges from 3 model families on an adversarial long-form benchmark, scoring them against gold labels over 1,248 rubric decisions.

Result: Cheaper judges, such as GPT-5-mini, can attain strong performance on source-relevance (F1=0.908, κ=0.636), while judges are statistically indistinguishable on factual support, but differ in pass-rate drift, false positive rate, and false negative rate.
...
TL;DR: The study evaluates the validity of a national language model, AMALIA, in measuring moral foundations, finding it relies on surface correlates rather than theoretical constructs, and thus cannot stand alone in measuring certain constructs.


<details>
  <summary>Details</summary>
Motivation: To assess the validity of a national language model, AMALIA, in measuring theoretical constructs, specifically moral foundations, and to highlight the importance of testing the evidential route by which agreement with human coders is warranted.

Method: The study uses a recovery gap test, decomposing holistic prompts into atomic clauses and recombining them using the theory's explicit rules to evaluate the model's reliance on correlated shortcuts versus theoretical constructs.

Result: AMALIA, a 9B-parameter model for European Portuguese, shows competitive agreement with human coders but fails to measure moral foundations well when tested for validity, relying on surface correlates such as moral outrage near authority figures.

Conclusion: The study concludes that while AMALIA can still be used for screening and pre-coding at scale, it cannot yet measure certain constructs well enough to stand alone, emphasizing the need for sovereign-LLM benchmark batteries to test not only agreement with human coders but also the evidential route by which that agreement is warranted.
...
TL;DR: The paper introduces UniClawBench, a new benchmark for evaluating proactive agents in real-world settings, focusing on capability-driven evaluation and live testing.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for proactive agents have limitations, such as relying on sandboxed environments and single-turn evaluations, and mixing multiple model capabilities within tasks.

Method: The authors design UniClawBench around five foundational model capabilities and create 400 bilingual real-world tasks, using live Docker containers and a closed-loop evaluation strategy.

Result: The benchmark evaluates state-of-the-art models under multiple agent frameworks, showing how base model capabilities and agent framework designs jointly shape performance in real-world environments.

Conclusion: The paper presents UniClawBench as a comprehensive benchmark for evaluating proactive agents, making it publicly available to facilitate future research.
...
<details>
  <summary>Details</summary>
Motivation: Current enterprise AI agents are reactive, waiting for human queries before acting, which limits productivity gains. The authors argue that proactive agents can surface relevant information to workers before they ask, improving productivity.

Method: The authors propose the Context Graph, a live relational data structure that models enterprise entities, relationships, and state transitions. They also define a Delta Detection Engine, Proactivity Scorer, and Surfacing Layer powered by an LLM.

Result: The proposed framework achieves a Precision@5 of 0.83, a false positive rate of 0.11, and reduces mean time to surface from 47 minutes to under 30 seconds across three enterprise case studies.

Conclusion: The paper concludes that context-graph-driven proactivity can significantly improve enterprise productivity by surfacing relevant information to workers before they ask, and provides a complete end-to-end implementation and evaluation of the proposed framework.
...
TL;DR: Developed an AI-powered tool integrating economic and biophysical models to analyze supply chain shocks in agricultural systems.


<details>
  <summary>Details</summary>
Motivation: Agricultural supply chains are vulnerable to disruptions through linked biophysical and economic systems.

Method: Integrated economic models (GTAP) with biophysical models (APSIM) using AI to enable natural language queries and responses.

Result: Enabled policymakers and market participants to assess cross-disciplinary impacts of supply chain shocks.

Conclusion: The tool facilitates analysis of supply chain shocks, allowing for informed decision-making.
...
<details>
  <summary>Details</summary>
Motivation: The authors are motivated to explain how agents distort information in densely interactive communicative landscapes for private gains.

Method: The authors outline an adversarial social epistemology (ASE) and provide language and mechanisms to analyze and audit trust breaches in public communications.

Result: The paper provides a framework for understanding how agents subvert trust in scaffolded public communications and proposes machinery for auditing and redressing trust breaches.

Conclusion: The authors conclude that familiar descriptions of epistemic bubbles, echo chambers, or misinformation diffusion are inadequate to capture the phenomena of agents exploiting trust in public communications.
...
TL;DR: The paper surveys recent progress in medical large language models (LLMs), focusing on their clinical reasoning applications and requirements, and discusses their potential in patient care.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to examine the growing potential of large language models (LLMs) in healthcare, specifically in clinical reasoning and patient care, and to discuss the requirements and challenges associated with their development and deployment.

Method: The paper uses a dual-view approach, connecting clinical practice with computational methods. It establishes a five-level competency scheme for clinical practice and links deductive, inductive, and abductive reasoning patterns to common medical goals and tasks. The authors also introduce a benchmark dataset and evaluate 18 state-of-the-art models.

Result: The results show that medical specialist models excel in diagnosis-centric tasks, while general models lead in decision support and dialogue. The authors also discuss current progress and open challenges, including data limitations, hallucination, and grounding issues.

Conclusion: The paper concludes that while medical LLMs have shown promising results, there are still open challenges to be addressed, including data limitations, hallucination, and grounding issues, and that future directions should focus on developing safer, more reliable, and workflow-ready systems.
...
TL;DR: The paper discusses the safety concerns of large language models (LLMs) in mental health support and proposes a framework for ensuring their structural safety through alignment at three levels: value specification, training, and oversight.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the safety concerns of LLMs in mental health support, which are currently designed to prioritize engagement over effective psychological support, and to propose a framework for ensuring their structural safety.

Method: The paper proposes a framework for ensuring the structural safety of LLMs in mental health support, which involves alignment at three levels: 1) explicit value specification, 2) training that embeds those values, and 3) oversight that detects drift and longer-term harm during deployment.

Result: The paper proposes a regulatory construct called 'alignment plausibility' for AI in health, which provides a principled way to argue for or against trust that systems are aligned to positive health outcomes and will cause no harm.

Conclusion: The paper concludes that making LLMs structurally safe requires a framework that ensures alignment at three levels, and proposes alignment plausibility as a regulatory construct for ensuring the safety and effectiveness of LLMs in mental health support.
...
TL;DR: This paper introduces the concept of idiobionics, which investigates the intersection of privacy and intelligent bionic limbs, and discusses potential adversarial attacks and open research questions.


<details>
  <summary>Details</summary>
Motivation: The increasing integration of advanced sensors and artificial intelligence in robotic prostheses introduces new privacy risks that need to be addressed to fully realize their benefits.
...
Result: The paper contributes a curated list of open research questions within idiobionics, which can help unlock the full potential of robotic prostheses and related bionic devices.

Conclusion: The authors conclude that understanding and addressing privacy risks is crucial for the adoption of next-generation bionic limbs and that idiobionics research can help achieve this goal.
...
TL;DR: Infinity-Parser2, a multimodal model for end-to-end document parsing, is introduced, addressing the scarcity of annotated parsing corpora.


<details>
  <summary>Details</summary>
Motivation: The persistent scarcity of faithfully annotated parsing corpora for end-to-end document parsing.

Method: A controllable data-synthesis pipeline with multi-task reinforcement learning, along with a scalable synthesis engine and a verifiable, multi-task reward system.

Result: Infinity-Parser2-Pro reaches state-of-the-art 87.6% on olmOCR-Bench and 74.3% on ParseBench, surpassing DeepSeek-OCR-2, PaddleOCR-VL-1.5, and MinerU2.5.

Conclusion: Infinity-Parser2, with its variants Infinity-Parser2-Flash and Infinity-Parser2-Pro, presents a significant advancement in end-to-end document parsing.
...
TL;DR: VectorizationLLM is a Large Language Model designed to assist students in learning MATLAB concepts, providing explanations and examples without direct answers.


<details>
  <summary>Details</summary>
Motivation: To support students in learning computational analysis concepts, specifically in the course CTEC 247: Applied Computational Analysis II.

Method: Using a Retrieval Augmented Generation (RAG) knowledge base and system architecture to provide detailed explanations and examples.

Result: The model provides instructive assistance with code, text, and image examples in its responses.

Conclusion: VectorizationLLM aims to enhance student learning in MATLAB and related concepts through interactive and explanatory support.
...
TL;DR: A novel approach using graph networks and graph neural networks for real-time hand gesture recognition from sEMG signals, achieving 99% accuracy and 48ms processing time.
...
Method: A graph network-based approach for sEMG representation and a machine learning algorithm using a graph neural network for real-time hand gesture recognition.

Result: The proposed method demonstrated an average classification accuracy of 99% and an average processing time of 48ms.

Conclusion: The proposed approach surpasses state-of-the-art techniques and is well-suited for real-time applications.
...
TL;DR: The paper explores the application of emerging AI architectures in actuarial practice, focusing on straight-through decision processes. It develops and analyzes an agentic AI framework for underwriting small commercial Business Owner Policies (BOPs), comparing its performance with other underwriting pipelines.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to examine how emerging AI architectures can support actuarial priorities such as transparency, auditability, and human-in-the-loop governance, particularly in straight-through decision processes.

Method: The paper develops and analyzes an agentic AI framework for straight-through underwriting of small commercial Business Owner Policies (BOPs). It constructs a synthetic but realistic experimental environment and compares three underwriting pipelines: (i) a single-LLM baseline, (ii) a naive RAG system, and (iii) a multi-agent 'Agentic RAG' pipeline.
...
Conclusion: The paper concludes that emerging AI architectures, particularly agentic systems, can support actuarial priorities such as transparency, auditability, and human-in-the-loop governance in straight-through decision processes, and that the agentic AI framework developed in this paper is effective in underwriting small commercial Business Owner Policies (BOPs).
...
TL;DR: A new method called Feedback Manipulation Regularization (FMR) is proposed to improve the alignment of imitation learning policies with human values in sequential decision-making environments.


<details>
  <summary>Details</summary>
Motivation: The increasing focus on alignment in reinforcement learning research, and the need for more effective use of human demonstrations and feedback in fully sequential decision-making environments.

Method: Feedback Manipulation Regularization (FMR), an algorithm-agnostic method that uses evaluative feedback as a corrective signal to improve the alignment of imitation learning policies.

Result: FMR demonstrates improved aptitude and up to a 98% reduction in misalignment across a range of imitation learning algorithms, and remains robust in limited data regimes.

Conclusion: FMR is a effective method for improving the alignment of imitation learning policies with human values in sequential decision-making environments, and can be used in a variety of settings, including those with limited data.
...
Result: The dataset consists of 89 machine-level records across 28 indicators, and the method generates 94 prompt, completion, and reasoning-trace rows with improved domain-grounded prompts.
...
TL;DR: The authors propose a framework to decompose, measure, and control behavioral patterns (personas) in large language models using the OCEAN personality traits, and demonstrate its effectiveness in shaping model behavior.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the lack of reliable tools for understanding and controlling behavioral patterns in large language models, which are crucial for generalization and safety.

Method: The authors use the OCEAN framework to describe model personas and train low-rank adapters to amplify or suppress individual traits, evaluating their effects using various benchmarks and evaluations.

Result: The authors find that their approach can move target traits monotonically with scale, combine adapters additively to construct mixed personas, and preserve performance on capability benchmarks, while also affecting safety-relevant behavior.

Conclusion: The authors conclude that their framework provides a bridge between personality measurement, model editing, and safety, allowing for the control of personas in large language models.
...
TL;DR: The study aims to improve automated detection of autism-related self-stimulatory behaviors from video, addressing challenges in neural network architecture and data augmentation for small behavioral datasets.


<details>
  <summary>Details</summary>
Motivation: Autism spectrum disorder affects over 75 million individuals worldwide, but scalable computational methods for remote behavioral screening are limited.

Method: The study used LSTM and GRU models, trained on pose-derived features from the SSBD dataset, and evaluated data augmentation strategies for training on small behavioral datasets.

Result: The models achieved peak accuracies of 97.5% (LSTM) and 98.75% (GRU) at a sampling interval of every 15 frames, outperforming prior CNN baselines. A personalized machine learning approach produced consistent predictions.
...
TL;DR: The paper proposes AgentNAS, a framework that combines large language models (LLMs) and neural architecture search (NAS) to efficiently search for optimal neural architectures. AgentNAS uses an LLM to generate a seed architecture, which is then decomposed into a slotted architecture that defines a search space for NAS to explore.


<details>
  <summary>Details</summary>
Motivation: The motivation is to bridge the gap between manual engineering of search spaces in NAS and the open-ended generation of architectures by LLMs, aiming to optimally divide the labor between LLM-driven design and NAS-driven search.

Method: The method involves a three-phase pipeline: (1) an LLM generates a high-quality seed architecture, (2) the seed architecture is decomposed into a slotted architecture with interchangeable module slots, and (3) conventional NAS is used to search the defined search space. The framework is called AgentNAS.

Result: AgentNAS establishes a new state of the art on 11 out of 17 tasks across diverse modalities, outperforming published baselines including task-specific expert designs. Ablation studies show that the LLM-generated seed and NAS-driven search are complementary, with NAS delivering additional gains through combinatorial recombination.

Conclusion: The paper concludes that AgentNAS provides a robust and efficient way to search for optimal neural architectures by combining the strengths of LLMs and NAS, and that the division of labor between LLM-driven design and NAS-driven search is effective across different LLM capability levels.
...
TL;DR: A new framework called Concretized Proposition Prompting (CPP) is proposed to improve the reasoning performance of Large Language Models (LLMs) by balancing compositionality and knowledgeability.


<details>
  <summary>Details</summary>
Motivation: The Composition-Knowledge Dichotomy, a challenge in LLMs where they struggle to balance compositionality with knowledgeability.

Method: Concretized Proposition Prompting (CPP), a framework that explicitly concretizes propositions relevant to questions.

Result: CPP significantly enhances reasoning performance, particularly in medical benchmarks, and is competitive on math benchmarks. It is also scalable to various foundation models and parameter sizes.
...
TL;DR: The paper presents a harness-engineering approach to transform prototypes of large language models (LLMs) into auditable and traceable applications, ensuring source-grounding, entity-routing, and output-hygiene contracts.


<details>
  <summary>Details</summary>
Motivation: The motivation is to productize LLM applications by adding requirements for source boundaries, entity routing, answer contracts, and reproducible traces, while maintaining the authority of source-backed claims.

Method: The authors propose a harness-engineering approach that reconstructs the pattern into a traceable, auditable LLM-agent architecture, with deterministic behavior moved into code, manifests, schemas, and validation artifacts.

Result: The evaluation results show that the harness preserves source-grounding, entity-routing, trace, output-hygiene, and recommendation-language contracts, and that code-owned guarantees are load-bearing and cannot be reproduced by prompting alone.

Conclusion: The paper concludes that the proposed harness-engineering approach provides a reusable engineering pattern for turning exploratory prototypes into auditable applications with versioned source, control, and validation artifacts, preserving both safety and utility.
...
<details>
  <summary>Details</summary>
Motivation: Current large language model (LLM) systems treat diagnosis as a one-shot prediction task, lacking safeguards against missed high-risk alternatives or rigorous verification of their reasoning, posing a major threat to patient safety.
...
<details>
  <summary>Details</summary>
Motivation: The increasing use of LLM-as-judge systems in enterprise pipelines and the assumption that consistency indicates correctness.

Method: A large-scale cross-runner study with 53 runners, 265,000 samples, and a hierarchical runner-clustered bootstrap to evaluate the relationship between agreement and accuracy.

Result: Agreement is a positive but weak predictor of accuracy (rho 0.20-0.59), and its usefulness varies depending on the model tier and application, with over-confidence and errors in the most consistent frontier models.

Conclusion: Self-consistency is a conditional proxy for correctness, not a standalone confidence score, and should be used with caution in AI evaluation systems.
...
TL;DR: Chain-of-thought (CoT) monitoring can be vulnerable to adversarial persuasion, but a fact-checking framework can mitigate this issue.


<details>
  <summary>Details</summary>
Motivation: To investigate the vulnerability of CoT monitoring to persuasion-based jailbreaks and to develop a robust mitigation strategy.

Method: Designing an evaluation framework with 40 tasks and analyzing thousands of agent-monitor interactions, where agents argue for policy-violating proposals.

Result: CoT monitoring increases approval of harmful actions by 9.5% in adversarial settings, but a fact-checking framework reduces approval by up to 45%.

Conclusion: CoT monitoring alone may be insufficient against adversarial persuasion, but model-diverse fact-checking provides a robust mitigation strategy.
...
TL;DR: The paper proposes PARA-PV, a physics-aware framework for accurate photovoltaic power forecasting, addressing challenges from weather variability and physical constraints.


<details>
  <summary>Details</summary>
Motivation: Accurate photovoltaic (PV) power forecasting is essential for reliable grid dispatch and renewable energy integration, but remains challenging due to weather variability, day-night transitions, regime-dependent dynamics, and strict physical constraints.

Method: The PARA-PV framework embeds physical knowledge throughout the forecasting process, using a physics-aware retrieval-augmented learner, a frozen Chronos time-series foundation-model prior, and a physics-aware distribution shift correction module.

Result: The proposed framework yields a physically grounded base forecast, calibrated and corrected to adapt to PV-specific dynamics and conditional distribution shifts.

Conclusion: The PARA-PV framework provides a comprehensive approach to accurate photovoltaic power forecasting, leveraging physical knowledge and adaptive corrections to improve forecasting reliability.
...
TL;DR: The paper introduces CausalDS, a benchmark for evaluating causal reasoning in data-science workflows using large language models.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks for causal reasoning and data analysis are limited, either lacking realistic data or principled causal structures. Existing causal evaluation datasets are often restricted to curated examples with limited diversity.

Method: The authors introduce CausalDS, which consists of scenes with sampled structural causal models, generated observational data, and synthetic natural-language stories. Tasks are derived from each scene, spanning Pearl's rungs, with a focus on data-science prediction tasks, uncertainty quantification, and tool use/coding.

Result: The benchmark evaluates symbolic causal reasoning, data science, uncertainty quantification, abstention, and tool use/coding in a comprehensive manner.

Conclusion: The paper presents CausalDS as a novel benchmark for evaluating causal reasoning in agentic data-science workflows, addressing the limitations of existing benchmarks.
...
TL;DR: A neurosymbolic reasoning and learning methodology integrating answer set programming with energy-based models for robust training in dynamic domains.


<details>
  <summary>Details</summary>
Motivation: To advance the integration of answer sets, probabilistic logic, and answer set modulo theories for applications in dynamic domains.

Method: Modular integration of answer set programming with energy-based model substrate, supporting joint optimization in continuous latent space.

Result: Practical implementation demonstrated on MNIST, Clevr, and MOT benchmarks, showing effectiveness in visual question-answering and multi-object tracking.

Conclusion: The proposed methodology enables robust, end-to-end training for dynamic domains, incorporating background knowledge, constraints, and non-monotonic inference.
...
TL;DR: The paper introduces 'overthinking', a method to amplify the reasoning capability of language models to reveal hidden information during auditing, making it more likely to surface secrets or unintended behaviors.
...
Method: The authors propose 'overthinking' models defined as $oldsymbolθ_{mathcal{O}_α} = oldsymbolθ_{mathcal{M}} + α(oldsymbolθ_{mathcal{R}} - oldsymbolθ_{mathcal{M}})$ with $α> 1$, and introduce layer-wise attenuation strategies to selectively amplify reasoning.

Result: Overthinking models are more likely to reveal hidden information across four experimental settings and 2B-32B models, surfacing secrets or unintended behaviors up to $10	imes$ more frequently than the original reasoning model.
...
Method: The proposed method, ASMR, uses a modular agentic framework with two specialized agents: a Field Generation Agent and a Structural Optimizer Agent. The Field Generation Agent extracts semantic concepts and generates candidate schema fields through adaptive multi-granularity clustering. The Structural Optimizer Agent employs reinforcement learning to identify compact, informative, and non-redundant schema representations.

Result: Preliminary results demonstrate the promise of the proposed approach in generating schemas that can guide report authors toward producing more complete, consistent, and actionable reports.

Conclusion: The paper concludes that the proposed ASMR framework shows potential in automatic schema generation and highlights several open research challenges at the intersection of data management, agentic AI, and human-centered AI.
...
TL;DR: The paper proposes a mathematical framework called 'active lifting' for understanding thinking and perception, and derives a design space for slow thinking large language models.
...
Conclusion: The paper provides a mathematical framework for understanding thinking and perception, and characterizes the agency of perception, including the emergence of slow thinking formats.
...
TL;DR: The paper proposes ZendoWorld, an interactive environment to test agents' ability to perceive complex inputs, form hypotheses, and design experiments. It evaluates various agents and finds that high accuracy doesn't imply understanding of underlying rules, and identifies areas for improvement.


<details>
  <summary>Details</summary>
Motivation: The challenge of building intelligent systems that can jointly perceive complex inputs, form hypotheses about hidden patterns, and design informative experiments to test them.

Method: The authors propose ZendoWorld, a controlled interactive environment, and evaluate several agents using pure VLM reasoning, Bayesian particle filtering, dynamic concept discovery, and neuro-symbolic methods.

Result: The main findings are: (1) high accuracy in predicting labels doesn't imply recovery of the underlying rule; (2) perception and induction are distinct bottlenecks for different agent classes; and (3) VLM-based agents propose near-uninformative experiments.
...
<details>
  <summary>Details</summary>
Motivation: The authors aim to address the issue of 'self-locking' in long-term persona agents, where the generated life collapses into familiar environments and relationships, and propose a solution to maintain identity continuity while adapting to new events.

Method: The authors propose AutoPersonas, a multi-timescale life-environment engine that separates environment-side Occurrences, accumulated Observations, and persona State, and introduce an OSO loop that admits divergent future-facing material while requiring evidence-governed absorption.

Result: The results show that AutoPersonas can reduce persona-environment self-locking while preserving identity continuity, with experiments demonstrating improved theme diversity and reduced repetition in persona behavior.

Conclusion: The authors conclude that separating controlled divergence from evidence-governed absorption can reduce persona-environment self-locking while preserving identity continuity, supporting the effectiveness of the proposed AutoPersonas engine.
...
TL;DR: This paper proposes a 'compete-then-collaborate' framework for knowledge distillation from multiple large language models to a smaller student model. It uses an execution-based judge to rank teachers and then collaborates to build a verifiable curriculum.


<details>
  <summary>Details</summary>
Motivation: Existing multi-teacher knowledge distillation methods have limitations, such as merging outputs without determining the best teacher or relying on a biased LLM judge. The authors aim to address these limitations.

Method: The authors introduce a framework where four frontier AI teachers compete and then collaborate to build a curriculum for a student model (Qwen2.5-Coder). They use an execution-based judge with fairness controls to rank teachers.

Result: The authors report three findings: (1) Teachers perform near-perfectly on standard problems but are separated on harder competition problems. (2) Imitation on verified solutions does not improve and can degrade an already-competent student. (3) Using a collaborative curriculum as a reinforcement learning environment improves the student.

Conclusion: The value of AI-teacher collaboration lies in jointly constructing a verifiable environment where the student learns by doing, rather than pooling answers to imitate. The authors release a reproducible pipeline for running GRPO on a bleeding-edge stack.
...
TL;DR: The paper introduces MentalHospital, a virtual environment for evaluating large language models (LLMs) in psychiatric clinical encounters, and MentalEval, a set of domain-specific evaluators. The results show that even the strongest LLMs trail clinicians by 37.28 percentage points in objective psychiatric competence.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for LLMs in psychiatry rarely simulate complete clinical encounters, and the authors aim to address this gap by creating a more comprehensive evaluation environment.

Method: The authors introduce MentalHospital, which instantiates the S.O.A.P. workflow using skill-augmented standardized patients constructed from 1,193 de-identified psychiatric EHR cases. They also develop MentalEval, five domain-specific evaluators trained with rubric-grounded SFT and expert-guided DPO.

Result: The results show that MentalHospital has clinical fidelity (3.88/5) and MentalEval achieves strong expert alignment with an average QWK of 0.944. However, even the strongest LLM trails clinicians by 37.28 percentage points in objective psychiatric competence, with mental status assessment as a key bottleneck.

Conclusion: The paper concludes that while LLMs have shown strong performance on isolated psychiatric tasks, they still trail clinicians in objective psychiatric competence, highlighting the need for more comprehensive evaluation environments like MentalHospital.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to reduce the latency and cost of using large models for high-volume structured extraction by distilling the task into a smaller on-device model.

Method: The authors distill an 8B reasoning teacher model into a 0.6B student model using QLoRA and compare its performance to the teacher model and other baselines.

Result: The student model runs at 0.8s per article (vs 39s for the teacher) and recovers 58% of the base-to-teacher gap on summary quality, outperforming constrained decoding and few-shot prompting.

Conclusion: The deliverable is a per-field routing map for on-device enrichment, as no single engine wins every field, and the distillation approach offers a trade-off between performance and latency/cost.
...
TL;DR: PolyUQuest is a structure-aware web retrieval-augmented generation framework that utilizes a heterogeneous graph to improve answer correctness, coverage, and faithfulness.
...
Method: PolyUQuest uses a heterogeneous graph that unifies hyperlink topology, DOM hierarchy, and entity-relation knowledge. A two-tier router dispatches queries to one of three retrieval modes.
...
Conclusion: PolyUQuest is a verifiable, structure-aware web RAG framework that provides accurate and faithful answers with structural evidence.
...
TL;DR: The paper proposes PredicateLongBench, a benchmark to evaluate long-context reasoning in large language models by testing their ability to identify the longest contiguous subsequence of words satisfying given predicates.


<details>
  <summary>Details</summary>
Motivation: Existing long-context evaluations for large language models are limited as they primarily measure average-case performance and often lack robustness. There is a need for a systematic way to probe model performance as task difficulty increases along various axes.

Method: The authors propose PredicateLongBench, which includes two generation pipelines: a fully synthetic setup using random word-like strings and a real-world setup sampling words from natural documents. The benchmark tests models' ability to identify the longest contiguous subsequence of words satisfying given predicates.
...
Conclusion: PredicateLongBench provides a systematic way to evaluate and stress-test long-context reasoning in large language models, revealing their limitations and encouraging further improvement in this area.
...
TL;DR: The paper introduces psychological competence as a crucial dimension in evaluating human-facing AI systems, focusing on their ability to support user cognition, emotional interpretation, and decision-making.


<details>
  <summary>Details</summary>
Motivation: Current AI evaluation frameworks are insufficient for human-facing AI systems, as they primarily focus on technical performance and neglect the psychological impact on users.

Method: The paper proposes a conceptual framework for psychological competence, outlining its core domains and suggesting evaluation methods such as scenario-based probes, structured human evaluation, and model-assisted evaluation.

Result: The paper defines psychological competence and its importance in AI evaluation, arguing that it should become a core consideration for model providers, deploying organizations, researchers, and regulators.

Conclusion: Psychological competence is a vital aspect of AI evaluation, particularly for human-facing AI systems, and should be integrated into existing evaluation frameworks to ensure these systems have a positive impact on users.
...
TL;DR: The study proposes the INTENT framework, using LSTM to predict vehicle intentions at intersections 2 seconds ahead, achieving 99.71% accuracy on the InD dataset.


<details>
  <summary>Details</summary>
Motivation: Autonomous vehicles need to interpret driver's intentions, especially in complex scenarios like intersections, roundabouts, and emergency cases, to improve safety and agility.

Method: The INTENT framework uses an LSTM model to predict vehicle intentions at intersections.

Result: The model achieves 99.71% accuracy on the InD dataset in predicting vehicle intentions 2 seconds ahead.

Conclusion: The study demonstrates the effectiveness of the INTENT framework in enhancing autonomous vehicle safety and agility through accurate vehicle intention prediction.
...
TL;DR: The paper introduces a new benchmark called 'blind-spots-bench' to identify blind spots in modern AI models by evaluating their performance on tasks that are simple for humans but challenging for AI.
...
Result: The analysis reveals that closed-source models outperform open-weight models by around 10%, and no single model dominates across all task types, highlighting the value of 'blind-spots-bench' as a diagnostic stress test.

Conclusion: The 'blind-spots-bench' benchmark can help identify concrete weaknesses in modern AI models, providing a more comprehensive understanding of their limitations.
...
Method: MobiDiff uses a discrete diffusion framework to directly denoise multi-channel semantic skeletons, capturing trajectory-level mobility patterns and within-event dependencies.

Result: MobiDiff preserves trajectory length and temporal interval distributions, is competitive across broader mobility statistics, and is 5.3 times faster than state-of-the-art methods.

Conclusion: MobiDiff offers an interpretable and efficient framework for synthetic mobility data generation, addressing the limitations of existing diffusion-based methods.
...
TL;DR: The paper proposes FedOPAL, a framework that improves analytical federated learning by adapting visual prompts to correct feature distributions, achieving efficient and accurate model collaboration on edge devices.
...
Method: The FedOPAL framework uses visual prompts as feature rectifiers, applying local proximal constraints to correct the feature distribution of heterogeneous data, satisfying the theoretical assumptions of analytical federated learning.

Result: FedOPAL significantly outperforms original analytical methods on several benchmarks, achieving accuracy comparable to state-of-the-art iterative methods while maintaining zero server-side training costs.

Conclusion: FedOPAL provides a new engineering paradigm for efficient collaboration of large models on the edge, addressing the contradiction between communication efficiency and model performance in federated learning.
...
TL;DR: The paper addresses the challenge of fine-tuning Large Language Models (LLMs) to incorporate new knowledge, highlighting the 'Knowing-Using Gap' where LLMs memorize facts but fail to apply them in downstream tasks. It introduces 'self-patching' to study knowledge integration and proposes a heuristic strategy to improve generalization.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the 'Knowing-Using Gap' in LLMs, where despite memorizing new facts, they fail to utilize them effectively in reasoning tasks.

Method: The method involves fine-tuning LLMs with new knowledge, using a novel intervention technique called 'self-patching' to monitor and analyze the internal dynamics of knowledge integration.

Result: The results indicate that memorized representations may not be effectively routed to relevant computation layers, leading to generalization failures. A heuristic strategy based on this finding recovers 58-75% of the oracle headroom in generalization failure.

Conclusion: The conclusion is that the 'Knowing-Using Gap' in LLMs can be addressed by understanding the misalignment of knowledge circuits within the model and applying targeted strategies to improve knowledge utilization and generalization.
...
TL;DR: A new framework called G-Frame improves Large Language Models in scientific domains by reducing hallucinations through structured reasoning and adaptive multi-agent training.


<details>
  <summary>Details</summary>
Motivation: The tendency of Large Language Models to mimic linguistic patterns rather than reproduce axiomatic reasoning, causing frequent hallucinations, limits their application in rule-based scientific domains.

Method: G-Frame, an adaptive multi-agent framework integrating Bayesian and team game principles, is used to establish an automated closed-loop for high-quality data synthesis and model training.

Result: The resulting 7B model OmniChem achieves performance parity with GPT 4o mini on custom benchmarks and ChemBench while exhibiting a 79.46% reduction in hallucinations relative to its base architecture.

Conclusion: This work establishes a scalable paradigm utilizing adaptive multi-agents to overcome inherent reasoning deficiencies, offering a feasible pathway for accelerating knowledge discovery in specialized scientific fields.
...
TL;DR: The paper introduces OmniFood-Bench, a comprehensive benchmark to evaluate Large Vision-Language Models (VLMs) for dietary management and personalized healthcare, highlighting a 'Semantic-Physical Gap' in current models.


<details>
  <summary>Details</summary>
Motivation: The integration of VLMs in food systems faces a challenge due to the 'Systemic Information Asymmetry' between visual appearance and intrinsic nutritional composition, and existing benchmarks do not adequately evaluate the required reasoning chain for real-world dietary management.

Method: The authors construct OmniFood-Bench from the MM-Food-100K dataset to evaluate VLMs across three progressive capabilities: Basic Perception, Quantitative Reasoning, and Safety-Critical Advisory, and test six state-of-the-art VLMs.

Result: The experiments reveal a significant 'Semantic-Physical Gap' where models achieve high accuracy in naming dishes but fail catastrophically in mass estimation and often provide hallucinated advice for high-risk diabetic profiles.

Conclusion: The paper establishes a rigorous standard for trustworthiness in autonomous agents deployed for public health and highlights the need for improvement in VLMs for dietary management and personalized healthcare.
...
<details>
  <summary>Details</summary>
Motivation: To investigate if JEPA-style predictive learning works for network fingerprints, specifically for JA4-derived fingerprints.

Method: A Transformer-based model, JA4-JEPA, was trained on JA4 subfields from JA4DB and CIC-IDS-2017 datasets using a JEPA-style objective.
...
Result: The method achieved a mean AUROC improvement of approximately 0.066 and comparable AUPRC results, demonstrating effective capture of temporal semantic change while being computationally efficient and interpretable.

Conclusion: The DATGR framework effectively captures temporal semantic change in evolving biomedical text, enhancing link-prediction recall without a loss in precision.
...
TL;DR: The study investigates facial emotion perception differences between autistic and neurotypical adults, finding that group differences are concentrated in specific diagnostic facial expressions, and proposes a model-guided framework for discovering and transforming stimuli to reveal population-specific perceptual differences.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this study is to understand perceptual differences between autistic and neurotypical adults, specifically in facial emotion perception, which has yielded varying results across studies.

Method: The study uses artificial neural network models to predict image-level judgments for autistic and neurotypical participants, and a generative adversarial network to transform diagnostic images toward greater predicted group agreement.

Result: The results show that the model-selected images produced larger behavioral differences than matched random images, and that synthesized images reduced behavioral separation relative to their matched originals.

Conclusion: The study establishes a model-guided framework for discovering and transforming stimuli that reveal population-specific perceptual differences, and demonstrates how behavioral phenotyping can move beyond averaging across fixed stimulus sets toward optimized assays.
...
TL;DR: This paper proposes CommuniWave, a machine learning model to detect and quantify informal behaviors in urban communities, enhancing territorial resilience.


<details>
  <summary>Details</summary>
Motivation: Current community planning often follows a top-down approach, lacking metrics to quantify informal behaviors of residents, leading to conflicts with original plans.

Method: The model integrates Behavior Capture Net (BCN), a self-developed YOLOv10 model (YLX), and Behavior Eval Model (BEM) using random forest to detect and quantify Degree of Informal Behavior (DIB).

Result: The model generates DIB fluctuation charts from street videos, facilitating dynamic monitoring and supporting refined decisions for urban managers.

Conclusion: CommuniWave enables urban managers to make data-driven decisions to enhance community resilience by monitoring and quantifying informal behaviors.
...
TL;DR: The paper revisits XAI-guided adaptive fusion for multimodal emotion and sentiment recognition, focusing on SHAP attribution reduction methods and their impact on fusion performance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of early and late fusion methods in multimodal emotion and sentiment recognition by analyzing the effect of SHAP attribution reduction on adaptive fusion.

Method: The method used is XAI-guided adaptive fusion with different SHAP attribution reduction methods (mean-abs, median-abs, and sum-abs) and evaluation on two datasets (MELD and CMU-MOSEI).

Result: The results show that sum-abs reduction achieves comparable performance to early fusion on both datasets, with McNemar testing indicating no significant difference between sum-abs and early fusion on MELD.

Conclusion: The conclusion is that the main contribution is a transparent empirical analysis of how SHAP reduction, expert dimensionality, and cross-modal expert design affect modular multimodal fusion, with sum-abs reduction being a effective approach.
...
TL;DR: HCC-STAR, a large language model, accurately stages and recommends treatments for hepatocellular carcinoma (HCC) patients, outperforming clinical guidelines and competitive models.


<details>
  <summary>Details</summary>
Motivation: Current guidelines for HCC treatment are limited, leading to within-stage heterogeneity and neglect of clinical context in electronic medical records.

Method: A clinically aligned large language model, HCC-STAR, was developed to read EMR narratives and output risk score-based staging, ranked treatments, and individualized survival estimates.

Result: HCC-STAR achieved state-of-the-art performance in treatment recommendation and risk stratification, with a median survival of 51 months under adherence to its recommendations.

Conclusion: HCC-STAR is a reliable and verifiable decision-support system for risk stratification and precision therapy in HCC, supporting its use as a clinical tool.
...
TL;DR: The study analyzed real patient-chatbot conversations and developed a patient simulator to evaluate LLM-powered health chatbots, finding that communication style significantly impacts triage outcomes and highlighting the need for patient-centered conversational AI.


<details>
  <summary>Details</summary>
Motivation: The development and evaluation of consumer-facing health chatbots often rely on idealized, simulated patients, which may not accurately represent real-world interactions.
...
Result: The study found that communication style can significantly alter triage outcomes, and that systems designed for idealized interactions risk underperforming and amplifying health disparities when deployed in the real world.

Conclusion: Patient-centered conversational AI must accommodate communication diversity to ensure effective and equitable healthcare interactions.
...
TL;DR: The paper investigates formal mechanisms to maintain market stability in a society of self-interested agents. It finds that Mediation is the most effective mechanism, resilient to adversarial attacks.


<details>
  <summary>Details</summary>
Motivation: Self-interested agents tend to defect in social dilemmas, causing cooperative gains to collapse. The paper aims to find mechanisms to maintain market stability.

Method: The paper uses a multi-agent marketplace simulation with 18 LLM agents and conducts two experimental phases: mechanism comparison and adversarial red-teaming.

Result: Mediation is the top-performing mechanism, and it is resilient to adversarial attacks, with the best attack reducing honest-agent utility by 13.3% but not collapsing the market.

Conclusion: Mediation enables recovery even under sustained adversarial pressure and is robust, able to sustain positive honest-agent utility under optimised attack.
...
TL;DR: The paper proposes a memory agent to help action agents in long-horizon tasks by selectively intervening with reminders from a structured memory bank, improving performance on two benchmarks.


<details>
  <summary>Details</summary>
Motivation: The authors identify a failure mode called 'behavioral state decay' where relevant information for decision-making gets buried or lost in long-horizon tasks as trajectories grow.

Method: A separate memory agent is introduced that runs alongside an unmodified action agent, updating a structured memory bank and selectively injecting memory-grounded reminders.

Result: The memory agent improves pass@1 performance on Terminal-Bench 2.0 and $τ^2$-Bench for both weaker and stronger action agents, with gains of +8.3 pp and +6.8 pp respectively.

Conclusion: The proposed memory agent as an active intervention mechanism shows better performance than passive retrieval methods and has potential for open-weight memory policies.
...
<details>
  <summary>Details</summary>
Motivation: The increasing application of agentic AI systems in cyber-physical environments necessitates evaluating both task performance and trustworthiness, particularly in decentralized energy markets where autonomous agents may exploit invalid data or create unstable governance decisions.

Method: The authors propose SolarChain-Eval, a physics-constrained benchmark that formulates market governance as a Markov Decision Process, evaluating policies across multiple dimensions including market utility, physical safety, and auditability, and incorporating an LLM-based Planner/Auditor layer for intervention and logging.

Result: Experiments with various policies reveal a clear utility-safety trade-off, with RL agents improving market utility but potentially producing unsafe behavior, and the LLM Planner/Auditor improving auditability and mitigating risks but not fully compensating for a misspecified reward function.

Conclusion: The study concludes that trustworthy agentic AI evaluation requires both physical constraints and transparent intervention traces, and releases the data and code for replicability.
...
TL;DR: The paper reveals that common metrics like accuracy and perplexity fail to capture behavioral changes in large language models caused by post-training quantization, and proposes a new metric called correctness agreement to evaluate these changes.


<details>
  <summary>Details</summary>
Motivation: The evaluation of post-training quantization in large language models relies heavily on accuracy and perplexity, which may not fully capture the behavioral changes induced by quantization.

Method: The authors introduce a decision-level metric called correctness agreement, and analyze quantization as a structural operator on attention weights, quantifying layer-wise distortions using statistical and distributional measures.

Result: The study finds that behavioral divergence emerges under moderate quantization even when task performance appears preserved, with non-linear breakpoints at low bit-widths and varied sensitivity across different projections.

Conclusion: The paper concludes that there is an illusion of equivalence between base and quantized models, and emphasizes the need for behavioral evaluation beyond conventional performance metrics.
...
TL;DR: The paper proposes a conceptual model for workflow management in large language models (LLMs), focusing on representing workflows as inspectable, resumable, and reviewable knowledge objects.


<details>
  <summary>Details</summary>
Motivation: The increasing use of explicit workflows in LLM applications necessitates a robust workflow management system that can handle tool use, retrieval, branching, checkpointing, and human approval.

Method: The paper introduces a Lisp-inspired, language-independent conceptual model that utilizes symbolic forms, object identity, and live-image thinking to represent workflow definitions, instances, and related knowledge objects in a shared knowledge substrate.

Result: The proposed model provides a preliminary conceptual account of semantic persistence, enabling workflows to be represented as inspectable, resumable, and reviewable knowledge objects.

Conclusion: The paper presents a conceptual framework for workflow management in LLMs, highlighting the distinction between deterministic computation (derive) and mediated LLM judgment (infer), with a focus on representing workflows as persistent knowledge objects.
...
TL;DR: The paper introduces AUTOPILOT-VQA, a benchmark for evaluating Vision-Language Models in autonomous driving tasks, specifically for safety-critical incidents.
...
Method: The authors present a dataset, AUTOPILOT-VQA, which includes structured questions around real-world driving incidents and near-incidents, covering various safety-relevant categories.
...
Conclusion: The AUTOPILOT-VQA benchmark enables the evaluation of Vision-Language Models for safety-critical incidents in autonomous driving, promoting more reliable and safety-aware systems.
...
TL;DR: This study analyzes the usage patterns of an AI-based learning assistant called Syntea among 77,543 students in higher education, finding varying usage across demographics.
...
Method: Large-scale descriptive analysis using objective log data from 77,543 students.

Result: Syntea is embedded in study routines of many learners, with usage patterns differing across demographic and structural contexts.

Conclusion: The study provides an empirical basis for further development of AI-based learning support and contributes a large-scale analysis of educational chatbot usage in higher education.
...
TL;DR: A new benchmark, IdeaGene-Bench (IG-Bench), is proposed to evaluate AI systems' ability to reason about scientific lineages and generate ideas based on them.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks do not assess whether AI systems can understand the inheritance structure of scientific ideas, which is crucial for their development.

Method: The authors present the IdeaGene framework, which represents papers as sets of 'Idea Genome objects' and uses 'GenomeDiff' to track changes between them, and propose two evaluations: IG-Exam for lineage reasoning and IG-Arena for generation.

Result: Experiments with 14 LLM-based systems reveal a 'compositional bottleneck', with the strongest system achieving only 27.3% accuracy on lineage reasoning, and show that structured lineage context affects system rankings.

Conclusion: The study highlights the need for better AI systems that can understand and build upon existing scientific knowledge, and introduces a benchmark to drive progress in this area.
