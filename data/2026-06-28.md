...
TL;DR: The paper proposes DocArena, a fully automated data curation pipeline for training search agents on multimodal document search and question-answering tasks without human annotation.


<details>
  <summary>Details</summary>
Motivation: Existing training environments for search agents are predominantly text-based, and constructing controllable, scalable, and multimodal training environments is challenging.

Method: The authors propose a pipeline that structures and indexes documents through MLLM-based visual perception, profiles and leverages cross-page information distribution to construct QA pairs, and performs quality assurance operations via MLLM.
...
Conclusion: The proposed DocArena pipeline effectively constructs a controllable and scalable training environment for search agents, leading to improved performance on multimodal document search and question-answering tasks.
...
TL;DR: A hybrid approach combining image processing and deep learning assesses fruit freshness with over 90% accuracy.
...
Result: The approach achieved real-time performance with over 90% accuracy on a dataset comprising apples and oranges.

Conclusion: The study highlights the potential of integrating simple image processing techniques with machine learning to provide practical solutions in the agricultural sector.
...
TL;DR: Proposed Long-Context Generation (LCG) framework for consistent multi-image text-to-image generation, outperforming baselines in prompt alignment and character consistency.
...
Result: LCG outperforms baselines in long-context image generation, including multi-character scenes, with experiments conducted on the Long-Context Consistency Dataset (LCCD).

Conclusion: The proposed LCG framework effectively improves consistency and scalability in long-context multi-image generation, with potential applications in comics, storyboards, and visual narratives.
...
<details>
  <summary>Details</summary>
Motivation: Urban tree biomass is less spatially quantified than managed forests due to reliance on inventories or coarse products, and this study aims to address this gap.

Method: A dual-stream cross-attention network and multiscale watershed segmentation are used to delineate crowns and estimate AGB from LiDAR and orthophotography data.

Result: The framework achieved high accuracy with R^2 = 0.609 and R^2 = 0.570 for AGB prediction, and estimated total AGB stocks of 1.73 Tg in 2018 and 1.81 Tg in 2023.

Conclusion: The proposed framework provides a high-resolution, accurate, and publicly available AGB database for urban trees, which can inform urban forest management and carbon sequestration efforts.
...
TL;DR: A deep learning model predicts penetration state, depth, and weld seam morphology in laser penetration welding with high accuracy.
...
TL;DR: This study extends fMRI taskonomy from single-source to multi-source transfer across 23 task states, revealing paradigm-specific transfer patterns and prioritizing working-memory states under budget constraints.


<details>
  <summary>Details</summary>
Motivation: Existing reconstruction-based taskonomies focus on one-to-one transfer from a single source task to a target, but cognitive tasks often involve shared and specialized neural processes across multiple tasks.

Method: The study uses Boolean Integer Programming (BIP) to analyze budget-constrained task allocation and trains 1,127 task-specific and transfer models across 23 Human Connectome Project task states.

Result: The findings reveal a cross-paradigm-limited motor cluster and working-memory states with high priority under the specified global allocation objective, and show that multi-source transfer depends on the composition of the source set.

Conclusion: The study extends reconstruction-based fMRI taskonomy from one-to-one transfer to many-to-one task relations and budget-constrained task dependencies, providing insights into the organization of cognitive tasks in the brain.
...
TL;DR: The paper proposes GeMoE, a new routing method for Mixture of Experts (MoE) architecture in Large Vision-Language Models (LVLMs) that adapts the number of experts per token, achieving better performance and efficiency.


<details>
  <summary>Details</summary>
Motivation: Traditional MoE methods use a static routing strategy that doesn't adapt to input variations, leading to suboptimal resource utilization.

Method: The authors propose viewing token routing as an information encoding task and framing dynamic routing as a Minimum Description Length (MDL) problem. They introduce Gating Entropy-based Uncertainty-aware Adaptive Routing (GeMoE) that uses gating entropy to assess token complexity and adaptively determines the number of experts per token.

Result: GeMoE achieves 99.5% average performance retention compared to static routing while improving average expert activation sparsity by 36.5% across various backbones and benchmarks.

Conclusion: The proposed GeMoE method offers a more efficient and adaptive routing approach for MoE architecture in LVLMs, achieving a better trade-off between model complexity and performance.
...
TL;DR: The authors introduce a new dataset and metric to evaluate computer vision models in underwater environments with low visibility due to turbidity.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the gap in understanding how computer vision models are affected by turbidity in underwater environments, and to provide a more accurate representation of real-world information loss.

Method: The authors propose a new dataset called Turbid Underwater Baseline (TUB) and a metric called Phase Congruency-based Degradation (PCD) to evaluate the performance of instance segmentation models in turbid underwater environments.

Result: The results show that PCD correlates strongly with the performance of instance segmentation models on both real and synthetic turbid images, while common metrics in the field show weak to no correlation.

Conclusion: The authors conclude that their proposed dataset and metric can be used to better evaluate and understand the effects of turbidity on computer vision models in underwater environments.
...
TL;DR: The paper investigates the effect of different fusion schemes for combining learnable prompts with image tokens in visual prompt tuning for Vision Transformers (ViTs).


<details>
  <summary>Details</summary>
Motivation: The motivation is to study whether a single fusion scheme is optimal or if a hybrid approach would be more effective in visual prompt tuning.

Method: The authors formulate the task as a bi-level optimization problem and solve it using differentiable architecture search, proposing and evaluating several fusion schemes including concatenation, addition, affine transformation, and cross-attention.

Result: The results show consistent gains over baselines across 34 datasets, with the hybrid fusion approach offering a favorable accuracy-latency-parameter trade-off compared to existing methods.

Conclusion: The study concludes that the choice of fusion scheme significantly impacts the performance of visual prompt tuning, and a hybrid fusion approach can more effectively leverage the layer semantics of ViTs.
...
TL;DR: The authors audited deepfake detection benchmarks and found that simple linear probes on general-purpose representations can match the performance of complex bespoke detectors, suggesting that benchmarks may not reflect realistic threat models.


<details>
  <summary>Details</summary>
Motivation: The motivation is to investigate the effectiveness of deepfake detection benchmarks and whether they truly measure forensic understanding.

Method: The authors used a simple diagnostic: a linear probe on frozen, general-purpose self-supervised representations to approximate the performance of bespoke detectors.

Result: The results show that linear probes on general-purpose representations closely approach the performance of bespoke detectors across three modalities, and that generator-level difficulty is partly explained by Frechet geometry in the same representation space.

Conclusion: The authors conclude that benchmarks may not reflect realistic threat models and raise questions about whether bespoke detectors are truly learning forensic understanding, suggesting a benchmark-audit view of deepfake detection.
...
TL;DR: The paper proposes VIGIL, a framework to improve multimodal large language models by reducing hallucinations and enhancing visual grounding through a geometric constraint and causal visual alignment.


<details>
  <summary>Details</summary>
Motivation: Multimodal large language models (MLLMs) often hallucinate and contradict visual inputs due to over-reliance on language priors. Existing alignment methods optimize text-based rewards, causing an optimization bias toward linguistic shortcuts.
...
Result: VIGIL outperforms recent alignment methods across hallucination and reasoning benchmarks, matches state-of-the-art performance with 25% of the preference data, and demonstrates emergent spatial grounding capabilities.

Conclusion: The proposed VIGIL framework effectively reduces hallucinations in MLLMs and enhances visual grounding, achieving improved performance on various benchmarks without compromising text-only capabilities.
...
TL;DR: DinoLink, a token-centric compression framework, enables high-precision remote perception in Vehicle-to-Everything (V2X) networks with severe bandwidth constraints.


<details>
  <summary>Details</summary>
Motivation: High-precision remote perception is hindered by severe bandwidth constraints of V2X networks.

Method: DinoLink employs a dual-sparsity architecture: a saliency-aware selector and a Residual Vector Quantization (RVQ) module.

Result: DinoLink achieves a $139	imes$ bitrate reduction and maintains a competitive 32.8% mAP on the nuScenes dataset.
...
TL;DR: A self-supervised framework for learning implicit 3D physical dynamics from video-derived supervisory signals, enabling the simulation of complex phenomena without explicit physics engines.
...
Method: The framework uses a Video Joint-Embedding Predictive Architecture (V-JEPA) to unproject semantic features into a voxelized 3D grid, grounded by monocular depth priors, and learns an action-conditioned transition operator through Volumetric Feature Advection.
...
Conclusion: This work opens a scalable pathway toward general-purpose dynamic world models that internalize the 3D invariants of the physical world solely through passive observation of monocular videos.
...
TL;DR: A multimodal deep learning model is proposed for efficient detection of methane plumes in earth observation imagery, outperforming state-of-the-art methods with a favorable accuracy-efficiency trade-off.


<details>
  <summary>Details</summary>
Motivation: The need for efficient detection of methane plumes to understand and mitigate global warming, specifically to accurately identify and segment them in earth observation imagery.

Method: A multimodal deep learning model integrating a feature-guided methane enhancement (FGME) mechanism with transformer-based RGB representations.

Result: The proposed method outperforms state-of-the-art methods on the MPDataset with improvements of +0.92 in MIoU, +0.87 in MPrecision, and +1.01 in Recall, at a lower computational cost.
...
TL;DR: The paper presents APRTrack, a hierarchical perturbation and retrieval framework for RGB-Event tracking, which improves robustness against partial target missing and modal degradation.


<details>
  <summary>Details</summary>
Motivation: RGB-Event tracking faces challenges in harsh environments where either modality can lose reliability drastically, and targets frequently appear incomplete due to occlusion, edge truncation, and foreground clutter.

Method: The proposed APRTrack framework constructs structured degradation via two adversarial perturbation branches and uses a hierarchical routing mechanism to disentangle the training pipelines. It also devises Footprint-guided Channel-calibrated Hopfield Retrieval (FCHR) for reliable historical information compensation.
...
Conclusion: The proposed APRTrack framework improves the robustness of RGB-Event tracking against partial target missing and modal degradation, and the source code and pre-trained models will be released.
...
<details>
  <summary>Details</summary>
Motivation: Existing TTT approaches are computationally expensive for long videos, with compute increasing linearly with window size, and perform unnecessary updates when frames are similar.

Method: The FFN operates on a limited set of frames within a sliding window and uses a surprise metric to adaptively adjust the window size during TTT.
...
Conclusion: The proposed Frame Forgetting Network efficiently handles long videos by reducing computational requirements and adapting to changing frame content.
...
TL;DR: The authors introduce CRISP, a novel evaluation paradigm for assessing visual spatial intelligence, which differentiates between language priors and genuine spatial reasoning.


<details>
  <summary>Details</summary>
Motivation: Current evaluations conflate language priors with genuine spatial reasoning, and the authors aim to address this issue.

Method: The authors propose CRISP, which uses metric 3D Scene Graphs and an oracle intervention protocol to decouple latent reasoning capabilities from perceptual bottlenecks.

Result: The study finds that proprietary models have robust latent reasoning engines but suffer from inaccurate metric estimation, while open-source models are bottlenecked by their lack of multi-hop compositional reasoning.

Conclusion: CRISP offers a rigorous roadmap for multimodal alignment beyond end-to-end post-training, shifting the focus from 'guessing correctly' to 'perceiving, verifying, and reasoning'.
...
<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for instruction-based image editing lack comprehensive evaluation of physics-based reasoning, which is crucial for handling real-world scenarios.

Method: PhyEditBench, a benchmark with 238 real-world and 35 synthetic instances, guided by a hierarchical taxonomy with 4 primary classes and 12 subclasses, and a training-free baseline, PhyWorld.

Result: Current state-of-the-art editing methods show substantial limitations in physics-based reasoning, while PhyWorld outperforms comparable models.

Conclusion: PhyEditBench effectively assesses physical understanding of editing models, and PhyWorld suggests that video generation can serve as a reasoning mechanism for image editing.
...
TL;DR: A new framework called ForeAgent is proposed for detecting AI-generated images. It uses a unique architecture and self-improvement strategy to achieve state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Existing deepfake detection methods struggle with highly realistic AI-generated images. Multimodal Large Language Models (MLLMs) show potential but have limitations, including insufficient sensitivity to fine-grained forensic artifacts and reliance on static synthetic supervision.

Method: ForeAgent adopts a Perception-Verdict architecture and a Hindsight-Driven Self-Refining strategy. It uses an MLLM as a verdict module and continuously evolves via fine-tuning on self-curated high-quality samples.

Result: ForeAgent achieves state-of-the-art performance on the Chameleon benchmark (82.18% accuracy) and AIGCDetect-Benchmark (93.3% mean accuracy across 16 generators).

Conclusion: ForeAgent offers a promising solution for detecting AI-generated images, outperforming existing methods and producing more consistent and causally grounded reasoning.
...
<details>
  <summary>Details</summary>
Motivation: Non-rigid 3D shape matching is a fundamental task in computer vision and graphics, and the authors aim to improve its performance and efficiency.

Method: A hybrid self-supervised method based on a coarse-to-fine strategy with a dual-branch design, using Laplacian and elastic basis for functional map learning.

Result: The approach achieves state-of-the-art performance across various challenging scenarios, including non-isometric deformations and topological noise, while maintaining computational efficiency.

Conclusion: The proposed method is effective and efficient for non-rigid 3D shape matching, and contrastive energies promote feature discrimination, leading to consistent improvements when integrated with existing methods.
...
<details>
  <summary>Details</summary>
Motivation: The limited inter-satellite and downlink resources in Earth observation satellite networks hinder the transmission of high-resolution imagery, and ground users require mission-relevant semantic information rather than full raw images.

Method: SpaceRipple proposes a collaborative pipeline for adaptive compression, metadata generation, restoration, and semantic inference, with a compression-aware MoE enhancement module for improved robustness.
...
TL;DR: A new benchmark, DiCoBench, is introduced to evaluate the fine-grained perception capabilities of Multimodal Large Language Models (MLLMs) using high-resolution images.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for MLLMs have limitations in evaluating their ability to perceive implicit visual cues in high-resolution images, and the authors aim to bridge this gap.

Method: The authors create DiCoBench, a comprehensive benchmark consisting of 765 samples with high-resolution images (approaching 2K) and 8 distinct perception tasks, and evaluate 18 diverse MLLMs using a multiple-choice question task.

Result: The evaluation reveals a significant performance gap between MLLMs and human accuracy (98.3%), with top-performing models struggling to capture micro-scale details.

Conclusion: DiCoBench serves as a challenging testbed to drive future research in autonomous, high-resolution multi-image perception, highlighting the need for improvement in MLLMs' fine-grained perception capabilities.
...
Conclusion: LogicIR is a viable and efficient alternative for image restoration, offering a promising direction for efficient computation.
...
Method: The authors propose TaskTok, which uses a learnable token switch and a lightweight token refinement module to selectively restore task-relevant tokens.

Result: TaskTok significantly enhances task performance with high computational efficiency across image classification, semantic segmentation, and object detection.

Conclusion: The paper concludes that selectively refining task-relevant tokens is sufficient for task-driven objectives and that TaskTok is an effective framework for achieving this.
...
TL;DR: The paper proposes a new method called Position Rebinding Cache Reuse (PRCR) for efficient multimodal reasoning, which reuses historical visual evidence while preserving textual positional continuity and relative visual structure.


<details>
  <summary>Details</summary>
Motivation: Existing methods for interleaved multimodal reasoning rely on token replay, which can be inefficient. The authors aim to improve visual grounding by revisiting visual evidence during multi-step generation.

Method: The authors propose PRCR, a cache-level framework that stores raw visual key-value (KV) cache together with their original spatial coordinates, then reassigns position-compatible coordinates to select entries and rebinds their keys before injecting the reconstructed cache into the active decoder cache.
...
Conclusion: The proposed PRCR method effectively reuses historical visual evidence while preserving textual positional continuity and relative visual structure, achieving efficient and accurate multimodal reasoning.
...
TL;DR: A new framework, FAROS, is proposed to address the annotation granularity mismatch in multi-task learning for surgical scene understanding. It uses flow-guided label interpolation to generate dense pseudo labels from sparse keyframe annotations, leading to improved performance across various tasks.


<details>
  <summary>Details</summary>
Motivation: The annotation granularity mismatch between temporal workflow tasks and pixel-level spatial tasks in surgical scene understanding hinders effective multi-task learning.

Method: A flow-guided label interpolation framework, FAROS, is proposed. It combines zero-shot segmentation-based mask propagation with optical flow estimation to generate temporally consistent dense pseudo labels.

Result: FAROS significantly improves cross-task representation learning and enhances holistic surgical scene understanding performance across spatio-temporal tasks on multiple benchmarks.

Conclusion: The proposed FAROS framework effectively addresses the annotation granularity mismatch in multi-task learning for surgical scene understanding, leading to improved performance and balanced optimization across tasks.
...
<details>
  <summary>Details</summary>
Motivation: The difficulty in collecting real event data at scale due to the requirements of specialized sensors, careful synchronization, and task-specific annotations motivates the development of event-camera simulators. However, existing simulators often simplify the temporal structure of events, which can distort event timing and weaken downstream transfer.

Method: FracEvent models the pixel-level lifecycle with fractional-relaxation voltage dynamics, driving a compact stack of relaxation modes, combining their responses into a voltage state, emitting ON/OFF events by localizing threshold crossings, and updating the reference while retaining the underlying memory modes.

Result: FracEvent improves the temporal structure of generated events and achieves stronger downstream-transfer results than competing simulator baselines on image reconstruction and optical flow estimation across multiple datasets.

Conclusion: FracEvent demonstrates practical value for event-camera simulation, offering improved temporal structure and downstream transfer results, making it a valuable tool for event-based vision tasks.
...
Method: LayersReg uses a regression paradigm that searches for correlations between moving and fixed images in feature space, capturing pixel flow trends to converge to the correct spatial pose transformation.
...
Conclusion: LayersReg offers improved accuracy, real-time capability, and generalization in complex scenarios, making it suitable for intraoperative surgical navigation.
...
TL;DR: The paper proposes Disco-LoRA, a framework for multi-concept video customization that can control content, style, and motion in text-to-video generation.


<details>
  <summary>Details</summary>
Motivation: The challenge of simultaneously controlling multiple concepts, such as content, style, and motion, in video customization.

Method: Disco-LoRA, a unified framework that disentangles and flexibly recombines different concepts in two stages: (1) Iterative Dual-LoRA Disentanglement Framework to address Content-Style and Content-Motion sub-tasks, and (2) Z-score-based statistical regularization to align weight distributions.

Result: Extensive experiments show that Disco-LoRA excels in multi-concept video customization, effectively preserving appearance, style, and motion for controllable text-to-video generation.

Conclusion: The proposed Disco-LoRA framework successfully addresses the challenge of multi-concept video customization, enabling controllable text-to-video generation with joint control of content, style, and motion.
...
TL;DR: DeCoFlow is proposed for continual anomaly detection in industrial environments, preserving strict invertibility and Jacobian validity of Normalizing Flows while preventing catastrophic forgetting.


<details>
  <summary>Details</summary>
Motivation: The need for continual anomaly detection in industrial environments without access to past data, while addressing the issue of catastrophic forgetting in Normalizing Flows.

Method: DeCoFlow decomposes subnets into a frozen universal base and task-specific low-rank adapters, and introduces Task-Specific Alignment, Auxiliary Coupling Layers, and Tail-Aware Loss.

Result: DeCoFlow achieves state-of-the-art image-level AUROCs of 98.40% on MVTec-AD and 93.00% on VisA, with zero forgetting and only 2.27M parameters per task.

Conclusion: DeCoFlow effectively addresses the challenge of continual anomaly detection in industrial environments, providing a solution that prevents catastrophic forgetting while maintaining the validity of Normalizing Flows.
...
TL;DR: PhysEditWorld is a multimodal dataset for controllable world modeling, focusing on physical parameters like gravity, enabling improved dynamics modeling and consistency under physical edits.


<details>
  <summary>Details</summary>
Motivation: Current game world models lack controllability and explicit manipulation of physical dynamics, limiting their applicability to authored game environments with deliberate physical rules.

Method: PhysEditWorld uses a replay paradigm with a UE5 replay-and-rendering pipeline to record scenarios under multiple gravity configurations, providing synchronized multimodal signals.
...
Conclusion: PhysEditWorld is a valuable resource for controllable world modeling, offering a multimodal dataset with physical parameters, and demonstrating its utility in improving dynamics modeling and consistency.
...
TL;DR: A multi-task framework is proposed for intracranial aneurysm detection, segmentation, and classification across 13 anatomical locations and four imaging modalities.


<details>
  <summary>Details</summary>
Motivation: Existing automated methods for intracranial aneurysm detection are limited to binary detection and lack fine-grained anatomical classification or multi-class segmentation, which are crucial for rupture risk assessment and treatment planning.

Method: A two-stage approach combining a fast 2D tri-axial ROI extraction method with a 3D multi-task nnU-Net backbone, featuring a dual-decoder design, cross-attention pooling, and modality-specific auxiliary heads.

Result: The proposed framework achieved 2nd place in the RSNA 2025 Intracranial Aneurysm Detection challenge, and the code, model weights, and a 3D Slicer plugin are publicly available.

Conclusion: The proposed multi-task framework advances the state-of-the-art in intracranial aneurysm detection, segmentation, and classification, and has the potential to improve rupture risk assessment and treatment planning.
...
TL;DR: The authors propose Mask to Concept (M2C), a framework that adapts SAM3 for efficient medical few-shot annotation without external modules or manual text engineering.


<details>
  <summary>Details</summary>
Motivation: Current methods for medical data annotation depend on external feature matchers or auxiliary networks, which introduce architectural overhead and limit performance scalability.

Method: M2C uses a learnable concept embedding to prompt segmentation and updates the embedding by minimizing concept segmentation error. It also introduces a Hybrid Uncertainty Estimation (HUE) module to flag highly uncertain samples for human correction.

Result: Experiments on medical segmentation benchmarks show that M2C achieves state-of-the-art few-shot segmentation performance and outstanding annotation efficiency.
...
Conclusion: NeuMatEx presents a novel approach for extracting spatially varying neural materials from images, addressing the challenges of acquiring and authoring complex materials.
...
TL;DR: The paper proposes a method called FUSE to generate unlearnable examples that protect training data by injecting perturbations, making it difficult for models to extract exploitable representations.


<details>
  <summary>Details</summary>
Motivation: Existing unlearnable examples (UEs) fail when low-pass filtering is applied, indicating that their perturbations are concentrated in high frequencies, and the authors argue that reliable UEs should be effective across the full spectrum.

Method: The authors propose Full-spectrum Unlearnable examples via Spectral Equalization (FUSE), which generates spectrum-agnostic perturbations by equalizing contributions from different bands and enforcing cross-band consistency using Random Spectral Masking (RSM) and Cross-Band Guidance (CBG).

Result: Extensive experiments demonstrate that FUSE achieves strong protection across multiple datasets, architectures, and spectral filtering.

Conclusion: The proposed FUSE method effectively generates unlearnable examples that protect training data by injecting perturbations across the full spectrum, making it difficult for models to extract exploitable representations.
...
TL;DR: The paper analyzes the impact of real-world noise on Open Vocabulary Object Detectors (OV-ODs) and presents a comprehensive study called Robust Onion, which reveals how and where robustness degrades in OV-ODs.
...
TL;DR: The paper introduces a new benchmark, 3DLP, to evaluate the ability of generative image editing models to understand real-world lighting. It presents a dataset of 1K image pairs and evaluates state-of-the-art models, finding differences in performance and room for improvement.


<details>
  <summary>Details</summary>
Motivation: To assess whether generative image editing models possess intrinsic knowledge of real-world lighting, as existing benchmarks have limitations.

Method: The 3D-anchored Light Probe (3DLP) benchmark is introduced, which includes a new high-fidelity HDR dataset of real-world lighting changes. State-of-the-art image editing models are evaluated using two new scores.

Result: The best image editing models show consistency with real-world physics but leave room for improvement, especially in image regions with less light from the light probe. VLMs are found unsuitable for pixel-level light transport analysis.

Conclusion: The paper highlights the need for further research on the ability of generative image editing models to understand real-world lighting, and makes the benchmark and dataset publicly available to encourage future research.
...
<details>
  <summary>Details</summary>
Motivation: Current streaming video editing methods are limited by their inability to maintain stable backgrounds and non-edited regions over time and achieve low latency for real-time interactive scenarios.

Method: A three-stage distillation pipeline that transfers editing capability from a powerful bidirectional foundation model to an efficient unidirectional streaming editor, along with an AR-oriented mask cache for reducing redundant processing.
...
Conclusion: The proposed framework enables practical deployment of streaming video editing in interactive and augmented reality applications.
...
TL;DR: The paper proposes a multimodal point cloud generation method using vision-radar fusion to improve the quality of sparse and noisy millimeter-wave radar point clouds.


<details>
  <summary>Details</summary>
Motivation: The quality of point clouds directly affects the performance of downstream perception tasks such as object detection and tracking, but millimeter-wave radar point clouds are typically sparse, noisy, and structurally incomplete.
...
TL;DR: The paper proposes a framework for multi-view stylization of 3D Gaussian Splatting (3DGS) using optimal transport, which improves stylistic consistency and stability across viewpoints.


<details>
  <summary>Details</summary>
Motivation: Enforcing stylistic coherence across viewpoints in 3DGS remains challenging, and existing methods have limitations such as unstable style allocation and limited cross-view consistency.

Method: A capacity-controlled framework for multi-view stylization of 3DGS using optimal transport, with explicit column-capacity constraints and cross-view matching guidance.
...
Conclusion: The proposed framework provides a principled mechanism for balancing feature coverage and stylistic diversity, and enhances cross-view coherence, leading to improved multi-view stylization of 3DGS.
...
TL;DR: The paper proposes ProtoKV, a memory method for Streaming Video Understanding (SVU) that improves accuracy by up to 12.5 points over token-retention baselines.


<details>
  <summary>Details</summary>
Motivation: The challenge of delayed queries in SVU, where decisive cues may appear briefly but are at risk of being evicted or diluted under bounded memory.

Method: ProtoKV, a constant-footprint SVU memory that represents far history as a fixed-capacity summary state, with an exact near-window KV cache and a semantic-spatial prototype bank.
...
Conclusion: ProtoKV offers a effective solution for SVU under strict GPU-memory and query-time latency budgets, particularly in scenarios with long query delays.
...
TL;DR: A novel implicit neural representation method called CHOIR is proposed, which uses Coordinated Harmonic Superposition and Perceptual Spectrum Calibration to improve optimization stability and alleviate spectrum bias.


<details>
  <summary>Details</summary>
Motivation: Most INR methods relying on periodic activation functions and function composition face optimization instability and spectrum bias issues.

Method: The proposed CHOIR method utilizes Coordinated Harmonic Superposition (CHS) and Perceptual Spectrum Calibration (PSC) to address the issues.

Result: CHOIR achieves superior performance over state-of-the-art approaches in various multidimensional data recovery problems.

Conclusion: The proposed CHOIR method effectively addresses the limitations of existing INR methods and demonstrates improved performance in multidimensional data recovery tasks.
...
TL;DR: A 4D controllable generative framework for medical imaging data augmentation, addressing limited annotated data, inter-device domain shifts, and privacy restrictions.


<details>
  <summary>Details</summary>
Motivation: Limited annotated data, inter-device domain shifts, and privacy restrictions hinder the development of robust artificial intelligence models for 4D medical imaging.

Method: A semi-supervised variational autoencoder and cascaded latent diffusion model are used to generate anatomically consistent 4D data, disentangling anatomical structure from temporal dynamics.

Result: The framework demonstrates high controllability of static anatomy (Pearson r > 0.8) and strong temporal coherence (FVD = 288.08), improving downstream segmentation performance with augmented training sets.

Conclusion: The proposed framework provides a scalable and controllable solution for 4D medical image synthesis, supporting the development of more robust models with limited annotations and cross-vendor variability.
...
TL;DR: LearniBridge, a learnable calibration mechanism for feature caching, enables up to 5.87x acceleration in image and video generation with minimal training samples.


<details>
  <summary>Details</summary>
Motivation: Diffusion Transformers (DiTs) are computationally costly; feature caching can accelerate inference but suffers from error accumulation at high acceleration ratios.

Method: LearniBridge uses lightweight LoRA updates to bridge multiple timesteps for effective calibration, requiring only 3-5 training samples.
...
Conclusion: LearniBridge effectively addresses error accumulation in feature caching for DiTs, enabling substantial acceleration with minimal training samples.
...
TL;DR: The authors propose an event-based active vision system to track and estimate the spin of balls in real-time, achieving high accuracy and low latency in professional table tennis matches.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the challenges of vision-based estimation of a ball's spin during a game, which is crucial in many ball sports, using conventional cameras.

Method: The authors propose a system consisting of an event camera, high-speed pan/tilt galvanometer mirrors, and a low-latency focus-tunable telephoto lens. They use a hybrid approach for ball tracking and propose two methods for spin estimation: an offline method (s-CMax) and an online method using a convolutional neural network.

Result: The system achieves state-of-the-art accuracy on static balls across multiple sports, with mean magnitude and axis errors of 2.1% and 4.0 degrees, respectively. In professional table tennis matches, the system demonstrates reliable tracking and spin estimation with high accuracy (8.8% magnitude and 6.4 degrees axis mismatch), 3 ms latency, and 750 Hz throughput.

Conclusion: The proposed event-based active vision system enables accurate and real-time estimation of a ball's spin, which can be applied to various ball sports, enhancing the analysis and performance of athletes.
...
TL;DR: The paper presents ReasonCLIP, a continual pretraining framework that enhances CLIP-style models with large-scale reasoning supervision for visually grounded commonsense inference and compositional reasoning.


<details>
  <summary>Details</summary>
Motivation: Downstream applications increasingly require visually grounded commonsense inference and compositional reasoning, but it's unclear if CLIP-style encoders can support such reasoning without architectural changes.

Method: A two-stage strategy for integrating large-scale reasoning supervision into CLIP-style models, using ReasonLite-42M and ReasonPro-16M datasets, and a benchmark RCLIP-Bench.

Result: ReasonCLIP improves visually grounded commonsense and compositional reasoning, enhances zero-shot retrieval performance, and delivers consistent gains as a drop-in visual encoder for multimodal large language models.

Conclusion: Structured reasoning supervision enhances the expressive capacity of CLIP-style visual representations, enabling them to support visually grounded commonsense inference and compositional reasoning without architectural changes.
...
<details>
  <summary>Details</summary>
Motivation: Existing acceleration methods for VDMs are limited by computational costs, calibration data dependency, and susceptibility to distribution shifts.

Method: NaviCache uses a dual-state estimation architecture to track feature changes and adaptively adjust to drift, with a time-dependent noise schedule and uncertainty-aware Measurement Update mechanism.

Result: NaviCache achieves outstanding comprehensive performance on HunyuanVideo, Wan, and Open-Sora series, with more accurate error judgment for computation skipping.

Conclusion: NaviCache provides a theoretically grounded mechanism for error-bounded computation skipping, bridging the domain gap and non-stationary nature of diffusion.
...
TL;DR: The authors propose a mask-guided multi-modality image fusion (MMIF) method that integrates feature restoration and interaction, achieving better performance in adverse weather conditions.


<details>
  <summary>Details</summary>
Motivation: Existing MMIF methods struggle with effective representation learning under adverse weather conditions, which cause significant image degradation and disrupt feature representation.

Method: The proposed method introduces 'Pseudo Ground Truth' for faster and more effective feature learning, a mask generation mechanism to quantify modality contributions, and a mask-guided cross-modal cross-attention mechanism for selective feature attention.

Result: The method surpasses state-of-the-art approaches in visual quality, quantitative metrics, and downstream tasks on synthetic and real-world datasets.

Conclusion: The proposed mask-guided MMIF method effectively addresses the challenges of image degradation and feature representation in adverse weather conditions, demonstrating superior performance.
...
Method: An illumination-robust seedling detection framework based on adversarial augmentation policy learning, which jointly optimizes a stochastic augmentation policy agent and an object detector.
...
TL;DR: AnomNOVIC, a two-stage framework for open vocabulary object detection, is proposed, combining a masked autoencoder for anomaly detection with a prompt-free image classifier, achieving high accuracy in various environments.


<details>
  <summary>Details</summary>
Motivation: The need for open vocabulary object detectors that are prompt-free and efficient for continuous deployment in real-world environments with previously unseen objects.

Method: A two-stage framework, AnomNOVIC, combining a masked autoencoder (MAE) for anomaly detection and NOVIC, a prompt-free open vocabulary image classifier.

Result: AnomNOVIC achieves 47.1% AP / 57.5% AP50 for prompt-free recognition and 59.0% AP / 72.5% AP50 with class candidates, surpassing baselines like YOLO-World-v2, OWLv2, and YOLOE.

Conclusion: AnomNOVIC demonstrates superior performance in open vocabulary object detection, especially in prompt-free scenarios, making it suitable for continuous deployment in real-world environments.
...
TL;DR: This paper proposes a Liquid Fusion Network (LFNet) for general salient object detection, which harmonizes features from State Space Models (SSMs) and Convolutional Neural Networks (CNNs) to leverage their complementary spectral biases.


<details>
  <summary>Details</summary>
Motivation: Current approaches to salient object detection neglect the inherent spectral biases of different neural network paradigms, and the authors aim to bridge this gap by combining the strengths of SSMs and CNNs.

Method: The authors introduce a liquid fusion mechanism to dynamically integrate features from VMamba (SSM) and ConvNeXt (CNN) backbones, and a Saliency-Guided Upsampling (SGU) operator to propagate features to shallow layers.
...
Conclusion: The proposed LFNet effectively leverages the complementary spectral biases of SSMs and CNNs, leading to improved performance and efficiency in general salient object detection.
...
TL;DR: The paper introduces a practical method for RS-aware relative pose estimation using affine correspondences, allowing for accurate pose and RS motion estimation from only 7 correspondences.


<details>
  <summary>Details</summary>
Motivation: The current state-of-the-art solver for RS-aware relative pose estimation requires a minimum of 20 point correspondences, making robust estimation expensive. The authors aim to make RS relative pose estimation practical by introducing affine correspondences.

Method: The authors derive novel RS-corrected affine constraints and develop a linearized algebraic solver that estimates pose and RS motion from only 7 affine correspondences. The solver uses null-space projection and action matrices to solve the system.

Result: The method achieves the best pose and RS parameter accuracy on the TUM RS benchmark and comparable accuracy to the standard 5-point algorithm on the EuRoC MAV dataset. It also provides accurate translational velocity estimates.

Conclusion: The paper presents a practical and accurate method for RS-aware relative pose estimation using affine correspondences, making it suitable for consumer devices equipped with rolling shutter cameras.
...
TL;DR: The paper proposes SpatialFlow-GRPO, a framework for fine-grained image editing using spatially aware rewards, outperforming existing methods on several benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing Flow-GRPO-style methods for image editing rely on a single whole-image reward, making fine-grained editing optimization difficult due to the spatial uniformity assumption.

Method: The proposed SpatialFlow-GRPO framework introduces spatially fine-grained reward feedback, converting region-aware rewards into semantic-region-level optimization signals and aligning region advantages with latent positions during policy updates.

Result: SpatialFlow-GRPO outperforms Flow-GRPO on GEdit-Bench, ImgEdit-Bench, and MultiEditBench, demonstrating improved editing quality through spatially aligned update signals.

Conclusion: The SpatialFlow-GRPO framework effectively addresses the limitations of whole-image rewards in image editing, enabling more precise and effective editing through spatially aware optimization.
...
TL;DR: The paper proposes a novel approach for reconfigurable intelligent surface (RIS)-assisted proactive handover (PHO) in millimeter-wave networks to mitigate line-of-sight blockages, optimizing RIS element allocation for better energy efficiency and link quality.


<details>
  <summary>Details</summary>
Motivation: Millimeter-wave networks are susceptible to line-of-sight blockages, and proactive handover (PHO) is challenging when no nearby base station is available. Reconfigurable intelligent surfaces (RIS) can restore connectivity, but RIS configuration time is crucial for timely PHO.

Method: The paper proposes a novel RIS-assisted PHO approach that optimizes the number of allocated RIS elements using particle swarm optimization (PSO) to balance signal processing complexity and link quality under handover timing constraints.

Result: Results show that reducing the number of RIS elements by 12% leads to a 10% decrease in dissipated energy without compromising the signal-to-noise ratio (SNR). The RIS-assisted link achieves a 15-30 dB improvement in blocked regions while maintaining accurate PHO timing.

Conclusion: The proposed RIS-assisted PHO approach optimizes RIS element allocation for better energy efficiency and link quality, making the RIS-assisted link more energy-efficient and effective in mitigating line-of-sight blockages in millimeter-wave networks.
...
TL;DR: A new method called OTF-CBM is proposed for transparent reasoning in vision-language models by aligning visual and textual representations through a dynamic cross-modal transport process.


<details>
  <summary>Details</summary>
Motivation: Existing vision-language Concept Bottleneck Models (CBMs) rely on pre-aligned encoders or global cosine similarity, which fails to reflect true semantic geometry and obscures fine-grained concept localization.

Method: The Optimal Transport Flow Concept Bottleneck Model (OTF-CBM) learns a data-driven semantic cost via Inverse Optimal Transport and performs unbalanced optimal-transport-based flow matching to model semantic transitions.
...
Conclusion: The proposed OTF-CBM provides a more effective approach to aligning visual and textual representations, enabling transparent and interpretable reasoning in vision-language models.
...
TL;DR: The authors propose a new 3D vision transformer, MICViT, for integrating multimodal brain MRI data. MICViT outperforms state-of-the-art baselines in brain age prediction and benefits from multimodal inputs.


<details>
  <summary>Details</summary>
Motivation: The challenge of machine learning with high-dimensional 3D brain MRI data from limited sample sizes and diverse anatomy, pathology, and acquisition conditions.
...
Result: MICViT consistently outperforms state-of-the-art CNN and transformer baselines in 3D settings, especially benefiting from multimodal inputs.
...
TL;DR: Proposed a synthetic-data augmented framework for automated fiber bundle segmentation in macaque tracer histology, achieving state-of-the-art performance with less manually annotated data.


<details>
  <summary>Details</summary>
Motivation: The accuracy of diffusion MRI (dMRI) tractography is limited by indirect measurements, and validating it requires time-consuming manual annotation of fiber bundles in histology sections.

Method: Used ex vivo dMRI tractography as a generative prior to synthesize 2D image patches for training a 2D U-Net, which was then trained on mixed real and synthetic patches.

Result: Improved generalization across brains and fiber bundle densities compared to training with real data only, achieving performance comparable to the state-of-the-art while requiring 3x less manually annotated data.

Conclusion: The proposed synthetic-data augmented framework can effectively automate fiber bundle segmentation in macaque tracer histology, reducing the need for manual annotation.
...
TL;DR: The paper proposes Robust-TO, a video understanding framework that assesses the trustworthiness of each input frame and integrates it into the reasoning process, achieving better accuracy and robustness under realistic perturbations.
...
Method: Robust-TO, an agentic video understanding framework that explicitly integrates per-frame trustworthiness into every stage of reasoning, using a unified evidence interface and calibrated reliability scores to guide evidence weighting and optimization.
...
Conclusion: The proposed Robust-TO framework effectively addresses the Blind Trust Problem, providing a more robust and accurate video understanding approach under realistic perturbations.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of text-to-image models struggling with underspecified, implicit, or knowledge-dependent real-world requests, which is identified as the Context Gap.

Method: The method involves proposing a unified agentic framework called Qwen-Image-Agent, which uses Context-Aware Planning and Context Grounding to progressively construct the generation context. It also introduces Image Agent Bench (IA-Bench) as a benchmark for evaluating agentic image generation.

Result: The result shows that Qwen-Image-Agent outperforms strong baselines and achieves state-of-the-art performance on IA-Bench, Mindbench, and WISE-Verified.

Conclusion: The conclusion is that Qwen-Image-Agent effectively bridges the Context Gap in text-to-image models by providing a context-centric approach to image generation, leading to improved performance on various benchmarks.
...
TL;DR: PhysRAG enhances physical awareness in video generation through Retrieval-Augmented Generation (RAG), achieving state-of-the-art performance in visual quality and physical rule compliance.


<details>
  <summary>Details</summary>
Motivation: The challenge of developing physically aware video generation models due to difficulties in capturing diverse physical phenomena.

Method: PhysRAG pipeline, two-stage data filtering, physical video database, and injecting physical knowledge into a video diffusion model using learnable queries.

Result: State-of-the-art performance in visual quality and physical rule compliance, surpassing existing models in benchmarks like PhyGenBench and VBench.

Conclusion: PhysRAG effectively enhances physical awareness in video generation, validating its components through extensive ablation studies.
...
TL;DR: The paper proposes PortraitGen, a novel framework for photorealistic portrait generation, addressing limitations in current text-to-image post-training methods.


<details>
  <summary>Details</summary>
Motivation: Current methods favor superficial aesthetics over critical flaws like AI artifacts and biological implausibilities due to the absence of real images during post-training and lack of specific rewards for fine-grained artifacts.

Method: PortraitGen introduces real images into GRPO sampling groups using image inversion and a dual-reward mechanism: OmniReward and AI-Portrait.
...
Conclusion: The proposed PortraitGen framework improves photorealistic portrait generation by addressing inherent generative boundaries and optimizing for specific rewards.
...
<details>
  <summary>Details</summary>
Motivation: Existing diffusion MoE frameworks have a routing assignment problem, where the router fails to allocate more computational resources to salient tokens due to reliance on noise-corrupted latent features.

Method: SharpMoE uses clean latent features as a noise-free guidance signal for routing, bypassing noise-distorted inputs, and introduces a trajectory routing loss to constrain compute allocation throughout the denoising trajectory.

Result: SharpMoE enhances pretrained, converged MoE models, achieving state-of-the-art performance in visual generation.

Conclusion: SharpMoE is a versatile, plug-and-play solution that improves the efficiency and performance of MoE architectures for diffusion models.
...
TL;DR: The paper proposes TraMP-LLaMA, a multimodal framework for facial expression quality assessment that predicts severity scores and generates textual reports, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing FEQA methods lack interpretability as they only produce severity scores without providing observable facial motion evidence, making it hard to inspect model outputs in Parkinson's disease assessment.

Method: The proposed TraMP-LLaMA framework integrates RGB appearance and landmark trajectory cues, and uses a decoupled instruction-tuning strategy to reduce task interference between severity prediction and language generation.

Result: TraMP-LLaMA outperforms competitive baselines in report generation and achieves the best severity prediction performance on PFED5-plus, improving Spearman's rank correlation by at least 4.39%.

Conclusion: The proposed TraMP-LLaMA framework provides a more interpretable and effective approach to facial expression quality assessment, supporting both severity score prediction and structured textual report generation.
...
TL;DR: The paper introduces OmniRef-Bench, a benchmark for multi-reference image generation (MRIG), and proposes DyRef, a two-stage training framework to improve model performance on complex MRIG tasks.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks fail to adequately evaluate complex MRIG scenarios, hindering progress in this area. The authors aim to better assess model performance on complex MRIG tasks and address the limitations of current models.

Method: The authors propose a two-stage training framework, DyRef, which consists of supervised fine-tuning and two novel techniques: Difficulty-aware Advantage Reweighting (DAR) and Discriminative Reward Scaling (DRS).

Result: Experiments demonstrate that DyRef significantly improves the performance of open-source models on OmniRef-Bench and single-image editing benchmarks.

Conclusion: The proposed DyRef framework effectively improves model performance on complex MRIG tasks, and the OmniRef-Bench benchmark provides a more comprehensive evaluation of MRIG models.
...
TL;DR: Introduced Dota2-Vis, a video-based dataset for visibility analysis in professional Dota 2 matches, and evaluated object detectors for player-icon detection.


<details>
  <summary>Details</summary>
Motivation: Current MOBA analytics studies rely on structured data, which doesn't capture what each team can see during a match.

Method: Created a dataset of 288 Full HD videos and 2,477 annotated minimap images from 144 professional Dota 2 matches, and evaluated multiple object detectors for player-icon detection.

Result: YOLO11l (large) achieved the best performance in detecting player icons, and the resulting visibility curves revealed new patterns in player behavior.

Conclusion: Dota2-Vis provides a new perspective on MOBA analytics, complementing conventional methods with visibility analysis, and is publicly available.
...
TL;DR: The paper proposes Geometric Gradient Rectification (GGR), a framework for open-set semi-supervised learning that controls auxiliary gradients to improve performance on in-distribution classes while handling out-of-distribution outliers.


<details>
  <summary>Details</summary>
Motivation: Existing methods for open-set semi-supervised learning face a trade-off between filtering out suspicious samples and utilizing unlabeled data, which can lead to conflicting auxiliary gradients. The authors aim to shift the focus from sample selection to gradient-level control.

Method: The authors propose Geometric Gradient Rectification (GGR), a plug-in framework that projects conflicting auxiliary gradients onto an admissible region in gradient space, preserving orthogonal components that may carry useful representation signals.
...
Conclusion: The proposed GGR framework provides a effective approach for open-set semi-supervised learning, achieving improved performance and robustness on various benchmarks.
...
TL;DR: A new benchmark called Unison is introduced to evaluate unified multimodal models that can both understand and generate content. Unison assesses joint understanding and generation capabilities, providing a comprehensive and diagnostic evaluation.


<details>
  <summary>Details</summary>
Motivation: Existing evaluations of unified multimodal models assess understanding and generation capabilities separately, ignoring their synergy. The authors aim to bridge this gap with a comprehensive benchmark.

Method: The Unison benchmark consists of 2,169 task samples and offers three key strengths: comprehensive dimensions, diagnostic evaluation, and human alignment. It also includes an evaluation model called Unison-Judge.

Result: The authors evaluated state-of-the-art models on Unison, revealing critical limitations in current unified multimodal systems and highlighting promising directions for future research.

Conclusion: The Unison benchmark provides a comprehensive and diagnostic evaluation of unified multimodal models, enabling holistic assessment and reliable evaluation. The authors make Unison and Unison-Judge publicly available.
...
TL;DR: The paper proposes EVIS, an event-aware video segmentation model that decomposes videos into simple events using learnable event queries and text guidance.


<details>
  <summary>Details</summary>
Motivation: Existing video segmentation methods treat videos as single events, leading to confusion and hallucinations. The authors aim to address this by proposing a model that understands videos in an event-by-event manner.

Method: The authors introduce EVIS, which uses text-guided event queries to partition videos into simple events and extract event-aware visual-text features. They also propose Object-Pixel-Hybrid Learning to track targets in long-term videos.

Result: The proposed model, EVIS, demonstrates strong performance on 5 public benchmarks for the referring video segmentation task.

Conclusion: The paper concludes that EVIS effectively addresses the limitations of existing video segmentation methods by providing a hierarchical understanding of videos through event-aware segmentation.
...
TL;DR: The paper presents a novel unsupervised detection method using Remote Sensing Foundation Models (RSFMs) for anomaly detection in Earth Observation, achieving high-performance generalization across diverse terrains and sensors without requiring bespoke training.
...
Method: The method is an unsupervised detection approach based on ResNet (RSFM) + FPN, which identifies anomalies by detecting subtle semantic shifts in the latent space between successive orbital passes, achieving efficient image-level generation and higher resolution mapping with minimal effort.

Result: The result is a system that achieves comparable results to previous proposals without requiring bespoke training and extensive development effort, while ensuring high-performance generalization across diverse terrains and sensors.

Conclusion: The conclusion is that RSFMs offer a powerful alternative to supervised models for Earth Observation, enabling unsupervised change detection and high-performance generalization, and that the proposed method provides a transformative path for disaster monitoring and anomaly detection.
...
TL;DR: PanoImager is a framework for 3D reconstruction from sparse panoramic images, addressing challenges in rotation-dominant, weak-parallax motion scenarios.


<details>
  <summary>Details</summary>
Motivation: The difficulty in 3D reconstruction from sparse panoramas under rotation-dominant, weak-parallax motion, where traditional SfM/SLAM methods often fail or produce unreliable results.

Method: PanoImager combines feed-forward pose/depth priors, geometry-conditioned diffusion view completion, and depth-guided 3DGS optimization to improve 3D reconstruction from a few panoramic images.

Result: Experiments show that PanoImager improves stability under extreme sparsity, making it suitable as an offline/background component for map refinement when SfM/SLAM fails to initialize.

Conclusion: PanoImager offers a promising solution for 3D reconstruction from sparse panoramic images, particularly in challenging motion scenarios.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to provide spatial priors for downstream trauma analysis by reliably localizing organs in abdominal CT scans.

Method: The method involves a Grounding-DINO-style query-based architecture with a Swin3D visual backbone, bidirectional feature enhancement, pseudo-text-guided query selection, and a cross-modality decoder.
...
Conclusion: SubdivAR presents a novel approach to neural mesh subdivision, achieving improved detail synthesis and generalizability.
...
TL;DR: The paper proposes a Tree-structured Mixed-policy Pruning (TMP) framework to compress large image generation models, demonstrating its effectiveness on two models with significant parameter reduction and minimal quality loss.


<details>
  <summary>Details</summary>
Motivation: The rapid growth of image generation models leads to unaffordable parameter consumption and computation budget, requiring massive resources and GPU memory footprint.

Method: The authors propose a Tree-structured Mixed-policy Pruning (TMP) framework, which generalizes to prevalent image tasks and architectures, and can be applied to step-distilled models.

Result: The TMP framework compresses HunyuanImage 3.0 from 80B to 20B parameters (75% reduction) with limited generation quality sacrifice, and enables inference on a single 24GB 4090 GPU. It also compresses Z-Image turbo from 6B to 4B (33% reduction) with negligible degradation.

Conclusion: The proposed TMP framework effectively compresses large image generation models while maintaining generation quality, making it a practical solution for reducing resource requirements.
...
TL;DR: FlameVQA is a benchmark for UAV-based wildfire monitoring using visual question answering, leveraging RGB and thermal images for safety-critical reasoning.
...
Method: The authors introduce FlameVQA, a multiple-choice visual question answering benchmark built on FLAME 3, using paired RGB and radiometric thermal TIFFs.

Result: MLLMs show strong performance with explicit cross-modal cues, but fail on presence detection under heavy smoke and coverage estimation.
...
TL;DR: The paper proposes a method for safe autoregressive image generation using iterative self-improving codebooks, leveraging a unified multimodal model to identify and eliminate harmful image-text pairs.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the safety of images generated by autoregressive unified multimodal models, which can produce unsafe content.

Method: The method involves two steps: (1) using the unified model to identify unsafe generations and construct harmful and safe image-text pairs, and (2) performing adaptive fine-tuning on the codebook within the harmless space using safe image-text pairs.

Result: The result is a safety-enhanced model codebook that iteratively improves without external feedback.

Conclusion: The paper concludes that the proposed method can effectively improve the safety of autoregressive image generation models.
...
TL;DR: The paper introduces HarmVideoBench, a new benchmark for evaluating large vision-language models' understanding of harmful videos, and BCR, a method that improves model performance by predicting reasoning boundaries and retrieving context.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for harmful video evaluation have limitations, including overlooking multi-layered characteristics of harmful videos and lacking explanatory rationales.

Method: The authors propose HarmVideoBench, a multi-layered diagnostic benchmark with 1,379 videos and 4,137 multiple-choice questions, and BCR, a benchmark-aligned method that predicts reasoning boundaries and dynamically retrieves context.

Result: BCR improves the base model's performance in harmful video understanding, raising the macro average from 61.7% to 84.4%.

Conclusion: The paper presents a new benchmark and method for evaluating and improving large vision-language models' understanding of harmful videos, addressing the limitations of existing works.
...
TL;DR: The paper proposes LIkelihood Score Alignment (LISA), a regularization method to improve the efficiency and effectiveness of the dual-branch paradigm in visual-condition controllable generation.


<details>
  <summary>Details</summary>
Motivation: The role of the side branch and its training efficiency in the prevalent dual-branch paradigm remain underexplored, and the authors aim to fill this gap.

Method: The authors propose LISA, which aligns the intermediate feature of the side network with an approximated likelihood score, and jointly optimize the side network and decoder with both standard diffusion loss and the regularization loss.
...
Conclusion: The proposed LISA method provides an effective and efficient way to improve the dual-branch paradigm in visual-condition controllable generation.
...
TL;DR: SatSplatDiff is a method for 3D reconstruction from satellite imagery that improves geometric accuracy and visual fidelity by combining monocular depth supervision, multi-scale geometric refinement, and shadow-guided generative refinement.


<details>
  <summary>Details</summary>
Motivation: The limited top viewpoint of satellite imagery results in insufficient supervision on building facades, leading to surface holes and degraded visual fidelity in 3D reconstruction. Existing generative refinement methods can generate hallucinations and break photo-consistency, leading to geometric degradation.

Method: The proposed method, SatSplatDiff, builds on photogrammetric DSM initialization and 2DGS-based shadow casting. It introduces monocular depth supervision and multi-scale geometric refinement to establish a geometrically accurate surface representation. Then, it applies shadow-guided generative refinement to improve visual fidelity while reducing geometric degradation.

Result: Extensive evaluations on the IARPA2016 and DFC2019 datasets demonstrate state-of-the-art performance, reducing geometric MAE by up to 18% and improving visual fidelity (FID-CLIP) by 28-45% over existing baselines. The method delivers up to 5x resolution enhancement with minimal hallucination and sensor-consistent appearance.

Conclusion: SatSplatDiff effectively addresses the limitations of existing methods for 3D reconstruction from satellite imagery, providing a more geometrically accurate and visually faithful representation of the scene.
...
TL;DR: A new benchmark called CORTEX is introduced for structured reasoning in 3D chest CT scans, providing a four-stage diagnostic trace and evaluation protocol to improve the interpretability and verifiability of multimodal large language models.


<details>
  <summary>Details</summary>
Motivation: The current free-form text reasoning in multimodal large language models (MLLMs) for medical imaging is hard to interpret and verify, especially in 3D radiology. Existing datasets lack the reasoning that links findings to conclusions and omit patient history.

Method: CORTEX introduces a four-stage diagnostic trace (task understanding, visual observation, diagnostic reasoning, and answer synthesis) generated using large language models and verified by a stage-level evaluation protocol combining automated rubric scoring and expert radiologist review.

Result: CORTEX comprises 76,177 validated reasoning traces across open-ended VQA, closed-ended VQA, and report generation, providing structured supervision and evaluation protocol for building and evaluating trustworthy reasoning models for 3D chest CT.

Conclusion: CORTEX provides a structured reasoning benchmark and evaluation protocol for 3D chest CT scans, enabling the development of more interpretable and verifiable multimodal large language models in medical imaging.
...
TL;DR: A new method for efficient exact nearest neighbor search in high-dimensional spaces, achieving significant speedup over brute-force search.


<details>
  <summary>Details</summary>
Motivation: The need for fast and exact nearest neighbor search in large databases, which is a fundamental problem in computer vision and other fields.

Method: The proposed method, called Hierarchical Normalization (HN), uses a novel normalization scheme to split the feature vector into major and minor components, allowing for efficient pruning of unlikely candidates and exact search.

Result: The HN-based search achieves 13.7x and 12.7x speedup over brute-force search on two datasets, with only 0.4% of entries requiring full evaluation, and maintains exactness and determinism.

Conclusion: The proposed HN method enables efficient and exact nearest neighbor search, outperforming approximate methods like HNSW and IVF-PQ, and is suitable for applications requiring high accuracy and determinism.
...
TL;DR: The paper presents a method for 3D generation using reinforcement learning from human feedback (RLHF) without relying on explicit surface representations or pretraining, achieving improved 3D geometry.


<details>
  <summary>Details</summary>
Motivation: Most existing pipelines for 3D generation using RLHF rely on explicit surface representations, which can be limited; the authors aim to fine-tune a pretrained 3D-aware generative model directly from a learned reward over radiance-field density values.

Method: The authors fine-tune a pretrained 3D-aware generative model (EG3D) using a learned reward model that reads the continuous 3D density field of the neural radiance field (NeRF) directly, without requiring text conditioning, mesh extraction, or multi-view rendering.
...
Conclusion: The proposed method yields robust improvement in 3D geometry, and demonstrates the effectiveness of RLHF for 3D generation without relying on explicit surface representations or pretraining.
...
TL;DR: The paper introduces SmellNet-V, a visuo-olfactory dataset, and See & Sniff, a self-supervised framework for joint visuo-olfactory representation learning, achieving improved smell classification and localization.


<details>
  <summary>Details</summary>
Motivation: The lack of paired visuo-olfactory data has hindered the exploration of olfaction in multimodal models, and the authors aim to address this gap.

Method: The authors propose a scalable visuo-olfactory dataset built by synthetically pairing smell-only samples with semantically aligned web images, and a self-supervised framework called See & Sniff for joint visuo-olfactory representation learning.
...
Conclusion: The paper establishes visuo-olfactory learning as a new direction in multimodal perception, enabling applications such as smell localization and cross-modal retrieval.
...
TL;DR: DanceOPD, a framework for training image generation models with multiple capabilities, achieves improved composition of text-to-image, editing, and other capabilities.


<details>
  <summary>Details</summary>
Motivation: Current image generation models struggle to unify diverse capabilities like text-to-image, local editing, and global editing, which often conflict with each other.

Method: DanceOPD uses on-policy generative field distillation with flow-matching models, routing samples to capability fields and training with a velocity MSE objective.

Result: Comprehensive experiments show DanceOPD improves multi-capability composition, preserving target capabilities and anchor generation quality.

Conclusion: DanceOPD establishes a practical route for generative field distillation in flow-matching models, enabling effective composition of diverse image generation capabilities.
...
TL;DR: A new framework called ViQ is proposed to unify text and vision representations, balancing semantics and details in discrete representations.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle to balance low-level details and high-level semantics in discrete representations, leading to information loss.

Method: ViQ uses a two-stage approach: text-aligned pre-training and feature discretization, with a proximal representation learning strategy and position-aware head-wise quantization.

Result: ViQ achieves competitive performance on multimodal tasks, maintains high precision in low-level reconstruction, and improves efficiency by up to 70%.

Conclusion: ViQ provides a unified and general discrete representation for arbitrary visual inputs, enabling simpler multimodal modeling and more efficient training.
...
TL;DR: OctoSense, a multimodal sensor platform, and a dataset of 59 hours of driving data, enabling self-supervised learning for robotics applications.


<details>
  <summary>Details</summary>
Motivation: The need for multimodal learning in robotics, where sensors have different representations, frequencies, latencies, and noise.

Method: A late-fusion masked autoencoder with modality-specific tokenizers and caching at inference time.

Result: The proposed architecture outperforms image-only foundation models on tasks like optical flow, depth, semantic segmentation, and ego-motion estimation, and shows robust prediction at nighttime or in degraded sensory conditions.

Conclusion: The OctoSense platform and dataset facilitate multimodal self-supervised learning, and the proposed architecture achieves fast and robust performance in various robotics tasks.
...
TL;DR: The paper proposes DexAC-WM, a structured action-conditioned world model that improves high-DoF action forecasting by preserving dimension-level semantics and aligning action signals with visual dynamics.


<details>
  <summary>Details</summary>
Motivation: Existing action-conditioned world models struggle with high-DoF actions due to their heterogeneous nature, leading to imbalanced optimization and reduced action fidelity.

Method: The authors propose DexAC-WM, which tokenizes actions, refines and modulates them locally and globally, and adds a semantic branch for rich object-scene priors.

Result: Experiments on EgoDex and EgoVerse show significant improvements in FID, FVD, and PCK, demonstrating gains in visual-temporal realism and action-following consistency.

Conclusion: Structured action modeling and semantic grounding are crucial for scaling world models to high-DoF control, and DexAC-WM offers a promising approach to achieve this.
...
TL;DR: A geometry-aware method for moving objects in single images while preserving scene consistency, including handling occlusions and maintaining coherent shadows and reflections.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for object motion in single images often fail to preserve scene-level consistency, including geometry, occlusions, and coherent shadows and reflections.

Method: A geometry-aware object motion method operating on diffusion transformers with rotary positional embeddings (RoPE), extended to a depth-aware formulation to encode 3D spatial structure.

Result: The model preserves object identity under large spatial displacements, generates plausible content in newly revealed regions, and consistently updates scene-dependent effects, achieving state-of-the-art performance on object motion benchmarks.

Conclusion: The proposed method effectively addresses the challenge of moving objects in single images while maintaining scene consistency, demonstrating superior performance with minimal real supervision.
...
TL;DR: A new framework called SAM2Matting is proposed to improve video matting by enhancing a tracker with a region-proposal bridge and dedicated matting heads, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: The gap between high-level tracking and low-level matting in video matting, and the limitations of existing methods that use expensive and narrowly-scoped datasets.

Method: A tracker-to-matting framework that decouples the task into enhancing a foundational tracker with a region-proposal bridge and dedicated matting heads.

Result: SAM2Matting establishes new state-of-the-art performance on video matting, supports diverse prompt types, maintains strong temporal consistency, and demonstrates robust generalization.

Conclusion: The proposed SAM2Matting framework advances video matting by leveraging the strengths of trackers and matting components, achieving impressive results.
...
TL;DR: The paper proposes RayPE, a positional encoding extension for video diffusion transformers that incorporates 3D geometric information to improve camera controllability and video quality.


<details>
  <summary>Details</summary>
Motivation: Current video diffusion transformers lack 3D structural information, and the authors aim to incorporate geometric relations between camera rays into the model.

Method: The authors propose RayPE, which injects 6D Plucker coordinates into the queries and keys of self-attention, and apply techniques to decouple ray direction and magnitude, and normalize the encoding.

Result: The proposed module improves camera controllability, cross-frame 3D consistency, and overall video quality on a four-dataset training mixture, with a negligible increase in parameters.

Conclusion: The paper presents a novel approach to incorporate 3D geometric information into video diffusion transformers, resulting in improved performance and controllability.
...
TL;DR: PhysiFormer: a diffusion transformer for physically-plausible 3D object motion prediction, outperforming baselines in accuracy, rigidity, and physical consistency.


<details>
  <summary>Details</summary>
Motivation: The need for a view-invariant, geometry-aware world modeling approach for robotics, graphics, and physical design, that can handle uncertainty and complex physical scenarios.

Method: A diffusion transformer that represents objects as 3D meshes in world coordinates, using a single denoising diffusion process to predict future vertex trajectories, with attention factorized over time, space, and objects.

Result: PhysiFormer substantially outperforms autoregressive baselines in trajectory accuracy, rigidity preservation, and momentum-based physical consistency, and generalizes to mixed-material settings, unseen real-world geometries, and larger object counts.

Conclusion: PhysiFormer presents a promising step toward view-invariant, geometry-aware world modeling, with potential applications in robotics, graphics, and physical design, by leveraging coordinate-space diffusion.
...
TL;DR: The paper proposes a method to improve diversity in flow models without sacrificing fidelity, by introducing self-guidance and manifold regularization.


<details>
  <summary>Details</summary>
Motivation: Existing flow models suffer from diversity collapse when generating multiple samples under the same conditioning, and current methods have limitations such as limited effectiveness or significant inference-time overhead.

Method: The paper introduces a self-guidance mechanism that disperses internal features of the flow model during batch generation and a manifold regularization step to project these features back onto the data manifold.

Result: The method significantly improves diversity while preserving fidelity across several conditional flow models, including text-to-image, depth-to-image, and reference image generation.

Conclusion: The proposed method is efficient, training-free, and integrates seamlessly into pretrained flow models, adding only a marginal inference cost.
...
TL;DR: The paper proposes Denoising Attention (DnA) to improve attention-based models by reducing noise in attention patterns, achieving gains in various visual perception tasks.


<details>
  <summary>Details</summary>
Motivation: The standard softmax activation in multihead attention (MHA) can produce noisy attention patterns, degrading performance in visual perception tasks.

Method: The proposed DnA uses a positive query to identify relevant features and a negative query to identify irrelevant features, projecting them into two distinct subspaces for improved discriminability.

Result: DnA achieves an absolute gain of 0.8% on ImageNet-1K and improvements in video understanding tasks, including video transformers (1.8%) and video LLMs (0.5%).

Conclusion: The proposed DnA method effectively denoises attention patterns, leading to improved performance in various visual perception tasks.
...
TL;DR: The paper proposes VISE, a purely unsupervised self-evolving framework that improves visual reasoning in large multimodal models by regularizing the model's visual conditioning policy.
...
Method: The authors propose VISE, which uses two complementary invariance-based rewards: geometric invariance reward and semantic invariance reward, to directly regularize the model's visual conditioning policy.

Result: Experiments on 18 benchmarks demonstrate the efficacy of VISE, achieving gains of +16.85 CIDEr on COCO and +19.66 CIDEr on TextCaps, reducing object hallucination by 5.0 Chair-I points, and generalizing across four model families and scales.

Conclusion: The proposed VISE framework effectively addresses the issue of visual under-conditioning in self-evolving large multimodal models, leading to improved performance on vision-language understanding tasks.
...
<details>
  <summary>Details</summary>
Motivation: Current LMMs rely on curated post-training supervision, such as human annotations. The authors aim to explore if a unified LMM can improve both abilities autonomously using only unlabeled images.

Method: The proposed framework consists of three internal roles: a Proposer, a Solver, and a Generator. It uses self-derived consistency signals, Solver Token Entropy (STE), and a multi-scale internal evaluation scheme.

Result: The method consistently improves over the corresponding base models across eight understanding metrics. On BAGEL, it achieves a +3.5% absolute gain on MMMU and improves GenEval image generation performance from 82% to 85%.

Conclusion: The proposed self-evolving training framework enables unified LMMs to improve both visual understanding and image generation autonomously using only unlabeled images, achieving state-of-the-art performance.
...
TL;DR: HierBias, a hierarchical media bias detector, leverages document context to improve bias detection, outperforming state-of-the-art models.


<details>
  <summary>Details</summary>
Motivation: Existing sentence-level approaches ignore inter-sentence contextual signals, which human annotators use, leading to suboptimal bias detection.
...
Conclusion: HierBias effectively models document context to improve media bias detection, with theoretical components contributing independently and consistently.
...
TL;DR: The paper presents a new benchmark to evaluate large language models' reliability in answering questions, abstaining from answers, and handling data contamination.


<details>
  <summary>Details</summary>
Motivation: Current evaluation methods for large language models conflate supported answering, unsupported guessing, and data contamination. The authors aim to separate these aspects and provide a more reliable evaluation protocol.

Method: The authors create a contamination-aware, multi-zone benchmark with 1,200 items across five domains, including explicit abstention expectations and contamination-risk metadata. They evaluate several large language models, including FLAN-T5, Qwen2.5-Instruct, and Llama-3-Instruct.

Result: The benchmark reveals that even strong instruction-tuned models have a selective but incomplete transition from answering to abstaining. Qwen2.5-3B-Instruct achieves the best overall reliability, but answer-expected zones remain difficult, and calibration is poor.

Conclusion: The proposed benchmark provides a reproducible protocol for auditing answerability, abstention, refusal, and contamination in large language models, highlighting the need for more reliable evaluation methods.
...
TL;DR: The study investigates how different post-training methods affect the retention of animal compassion values in a large language model. It finds that helpfulness training degrades these values, while coding training preserves them better.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to understand how post-training processes (supervised fine-tuning and reinforcement learning) impact the retention of values instilled during pre-training, specifically animal compassion values.

Method: The study uses a Llama 3.1 8B model mid-trained on compassion-oriented synthetic data and applies supervised fine-tuning (SFT) and reinforcement learning (GRPO) with different domains (helpfulness and coding). The model is evaluated on the Animal Harm Benchmark (AHB 2.2) and MORU benchmark.

Result: The results show that helpfulness training significantly degrades animal compassion values compared to coding training. The effect is consistent across two independent helpfulness datasets and two training paradigms. Additionally, the study finds that the domain effect on moral reasoning is significant in English but not in multilingual settings.

Conclusion: The study concludes that coding-domain post-training may be a better approach to preserve mid-trained values than helpfulness post-training without harming general reasoning capabilities. It suggests that values instilled through mid-training are encoded more deeply and cross-lingually than reasoning improvements from domain-specific post-training.
...
TL;DR: This study evaluates the performance of Large Language Models (LLMs) in solving statics problems in mechanical engineering education, finding that LLMs perform well on text-only problems but struggle with problems that require multi-step reasoning and diagram interpretation.


<details>
  <summary>Details</summary>
Motivation: The rapid influence of LLMs on education, particularly in subjects like mechanical engineering, necessitates a deeper understanding of their capabilities and limitations in solving specific problem types, which is currently lacking in existing literature.

Method: The study uses a model distillation process to evaluate LLM capabilities, specifically by distilling ChatGPT and constructing datasets of statics problems with and without diagrams.

Result: LLMs show high accuracy on text-only statics problems but experience a performance drop when faced with problems that include diagrams and require multi-step reasoning, primarily due to difficulties in multi-step reasoning and applying visual information.

Conclusion: The findings highlight the need for further development of LLMs to improve their performance on problems that require multi-step reasoning and diagram interpretation, particularly in engineering education.
...
TL;DR: The study analyzes how different linguistic features in animal-welfare text affect Llama-3.2-1B's preference for pro-animal-welfare reasoning when used as fine-tuning data.


<details>
  <summary>Details</summary>
Motivation: The motivation is to understand how animal-welfare advocates' writing influences language models and to provide recommendations for effective writing that promotes pro-animal-welfare reasoning.

Method: The study uses vocabulary-matched stance-contrast probes on a held-out animal-welfare benchmark to measure the impact of ten linguistic features on Llama-3.2-1B's preference for pro-animal-welfare reasoning.

Result: Eight out of ten linguistic features produce statistically significant shifts in Llama-3.2-1B's preference for pro-animal-welfare reasoning. Seven features (assertive certainty, explicit moral vocabulary, emotion words, evaluative claims, narrative structure, depicted harm severity, and immediate temporal framing) strengthen pro-animal-welfare reasoning, while two features (hedged language and concrete sensory description) weaken it.

Conclusion: The study concludes that when writing animal-welfare text that may end up in LLM training corpora, it is recommended to assert a position rather than describe a scene neutrally, as features that make the writer's position explicit tend to strengthen pro-animal-welfare reasoning.
...
TL;DR: ContextForge is a system that enhances large language models' performance in long conversational tasks by efficiently recycling context, reducing token usage while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Large language models struggle with long conversational tasks due to context window limitations and inefficient token usage.

Method: ContextForge combines structured query generation, external memory retrieval, and controlled synthesis to recycle context across turns.
...
<details>
  <summary>Details</summary>
Motivation: The increasing use of LLMs in emotionally charged situations involving conflict, frustration, and distress, and the need to prevent unintentional escalation of conflict.

Method: Reformulating NVC principles into process-oriented guidelines and applying them as lightweight prompt-level constraints to LLMs in a dual-agent simulation framework.
...
Conclusion: Simple communication constraints derived from NVC can improve the trustworthiness of LLM dialogue in conflict-prone settings.
...
TL;DR: A pilot study introducing NEST-V1, a multimodal framework for generating emotion-conditioned Nepali Sign Language avatars from spoken input, achieving high accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: The lack of exploration in sign language communication systems that integrate emotional expression, particularly for low-resource languages.

Method: A lightweight architecture employing a shared acoustic encoder for Automatic Speech Recognition and emotion classification.

Result: Achieved 81.1% ASR accuracy, 79.21% emotion recognition accuracy, and 37% parameter efficiency with 22.1M parameters.

Conclusion: Establishes the technical foundation for emotion-aware sign language translation in low-resource settings, with a scalable framework for future expansion.
...
Method: The study develops AdvCluster, an automated framework that identifies questions where larger models show a stable advantage, extracts fine-grained advantage descriptions from paired reasoning traces, and organizes them through semantic clustering.

Result: The analysis yields a systematic taxonomy of larger model reasoning advantages, including common advantages across domains and specialized advantages associated with particular domains, with Constraint-Guided Reasoning as a recurring theme.

Conclusion: Larger language models have advantages in Constraint-Guided Reasoning, including identifying explicit and implicit constraints, organizing them into structured reasoning, and using them to rule out infeasible paths and verify intermediate steps.
...
TL;DR: A methodology for creating specialized conversational AI systems for low-resource languages using structured linguistic resources like lexical databases.
...
TL;DR: The paper proposes Dynamic-dLLM, a framework that improves the efficiency of diffusion Large Language Models (dLLMs) without compromising performance.
...
Method: The framework consists of two components: Dynamic Cache Updating (DCU) and Adaptive Parallel Decoding (APD), which adaptively optimize cache allocation and decoding thresholds.
...
Conclusion: The proposed framework provides a plug-and-play solution for efficient dLLM deployment, outperforming state-of-the-art acceleration methods.
...
TL;DR: The study examines the methodological suggestions of large language models (LLMs) like GPT-5.1, Gemini 3 Pro, and DeepSeek-V3.2 when prompted with research questions from 1,000 recent computer science papers. It finds that LLMs tend to suggest a narrow range of methods, with significant imbalances in provider choice and underrepresentation of certain model types.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to understand the default methodological tendencies of LLMs when used to guide research methodology, specifically with minimal prompting.

Method: The authors prompt three LLMs with research questions extracted from 1,000 recent arXiv computer science papers and compare the resulting methodology suggestions against a paper-derived experimental inventory. They extract structured method features, map them into a shared taxonomy, and quantify divergence across multiple taxonomy dimensions.

Result: The study finds significant imbalances in provider choice, with LLMs underrepresenting other/academic single-occurrence models by 23-24 percentage points and overrepresenting reused academic/community models by 4-6 percentage points. The effective number of model entities suggested by LLMs contracts from 1,232 to 59-96. The results also show that LLMs suggest a much narrower range of methods overall.

Conclusion: The study concludes that researchers who rely on LLM suggestions without cross-checking risk narrowing their methodological search space toward a more concentrated default. The findings highlight the need for researchers to be aware of the potential biases and limitations of LLMs when used to guide research methodology.
...
TL;DR: The paper presents a systematic survey of unified vision-language perception in Multimodal Large Language Models (MLLMs), formalizing MLLM perception as a unified capability, and outlining a five-stage taxonomy and future research directions.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to bridge the gap in existing surveys that focus separately on vision or language, and to provide a unified understanding of perception in MLLMs.

Method: The authors formalize MLLM perception as an intrinsic, unified vision-language capability, introduce a five-stage taxonomy to trace the paradigm evolution of MLLM perception, and survey representative methods and milestones at each phase.

Result: The paper provides a comprehensive survey of unified vision-language perception in MLLMs, identifying open challenges and outlining promising research directions toward truly general, unified multimodal intelligence.

Conclusion: The authors hope that their study will provide both a foundational understanding and an actionable roadmap to foster further innovation on the path toward artificial general intelligence (AGI).
...
TL;DR: The study examines the phonetic realization of the neutral tone in Mandarin Chinese, finding it has a distinct tonal target and pitch contours influenced by preceding tones, and proposes it's a lexical tone with similarities and differences between Taiwan and Beijing Mandarin.


<details>
  <summary>Details</summary>
Motivation: To investigate the properties of the neutral tone in Mandarin Chinese and its realization in spontaneous conversational speech in Taiwan and Beijing Mandarin.

Method: A corpus-based study using spontaneous conversational speech corpora of Beijing Mandarin and Taiwan Mandarin, analyzing phonetic realization and pitch contours.

Result: The neutral tone has its own tonal target, pitch contours dependent on preceding tones, and word-specific pitch signatures, similar to lexical tones, with similarities and differences between Taiwan and Beijing Mandarin.

Conclusion: The neutral tone is proposed to be a lexical tone in both Taiwan Mandarin and Beijing Mandarin, with evidence of similarities and differences in its realization between the two varieties.
...
TL;DR: Small language models can assist with title and abstract screening for large systematic reviews, augmenting human reviewers by identifying missed papers quickly.


<details>
  <summary>Details</summary>
Motivation: The rapid growth of social-physical human-robot interaction research has led to a need for scalable review practices due to fragmented terminology and inconsistent methodologies.

Method: Evaluating small language models (SLMs) with less than 1.5B parameters for title and abstract screening in a large systematic review of spHRI.

Result: SLMs significantly sped up the screening process, identifying 39 papers missed by human reviewers, which comprised 10.29% of the final relevant dataset.

Conclusion: SLMs can augment human reviewers in large-scale literature reviews, making them more accessible and sustainable, though they did not match human performance.
...
TL;DR: The authors present PROFILEFOUNDRY, a synthetic data generator for creating realistic, consistent, and controllable person data for evaluating foundation models while maintaining responsibility and inspectability.


<details>
  <summary>Details</summary>
Motivation: The need for realistic and consistent person data for foundation-model research while ensuring responsible handling of sensitive user data.

Method: The authors developed a deterministic generator to create 100,000 synthetic adult Person Objects with rich relational and temporal structure.

Result: The PROFILEFOUNDRY release includes 709,228 events, 40,338 households, 52,491 employers, and 518,564 directed relationship edges, with evidence of consistency and responsibility.

Conclusion: PROFILEFOUNDRY provides a responsible synthetic source layer for constructing downstream foundation-model evaluations involving memory, privacy, document understanding, record linkage, and agent state.
...
TL;DR: A new metric called ConflictScore is proposed to evaluate how well a model's response acknowledges conflicting evidence in its grounding documents.


<details>
  <summary>Details</summary>
Motivation: Existing metrics fail to capture when both supporting and contradicting evidence coexist, and the authors aim to address this limitation.

Method: The framework decomposes responses into atomic claims, labels each claim against each grounding document, and aggregates these labels into two measures: ConflictScore-Count (CS-C) and ConflictScore-Ratio (CS-R).
...
Conclusion: The proposed ConflictScore metric can help evaluate and improve the factuality and faithfulness of model responses by acknowledging conflicting evidence.
...
TL;DR: Introduced ProvenAI, a framework for evaluating transparency in multi-hop question answering systems by assessing answer correctness, citation fidelity, and document influence.


<details>
  <summary>Details</summary>
Motivation: Current retrieval-augmented systems present citations with generated answers, but a citation does not guarantee the source meaningfully shaped the output.

Method: ProvenAI uses a seven-stage pipeline to evaluate 7,405 validation examples, including data normalization, retrieval indexing, citation-aware answer generation, and influence estimation.

Result: ProvenAI achieved 53.53% answer accuracy, 71.55% mean citation-fidelity score, and revealed a 'citation-influence gap' where cited sources have weak influence while uncited sources significantly impact the output.

Conclusion: Meaningful transparency in retrieval-grounded QA requires traceable links across retrieved, cited, and behaviourally influential evidence as three distinct, independently measured layers.
...
TL;DR: A single lightweight model, AnySimLite, is proposed to solve multiple speech-adjacent classification tasks with low memory footprint and competitive performance.


<details>
  <summary>Details</summary>
Motivation: The need for lightweight on-device models to minimize privacy concerns and inference latency, while addressing the challenge of deploying multiple specialized models for natural language classification tasks.
...
Result: AnySimLite achieves state-of-the-art or competitive performance in few-shot settings across multiple speech-adjacent classification tasks, with a low memory footprint, using less than 1/250th of the model size of the SOTA baseline.

Conclusion: AnySimLite offers a promising solution for deploying a single lightweight model for multiple speech-adjacent classification tasks, balancing performance and memory efficiency.
...
TL;DR: The paper proposes SOLAR, an auxiliary objective for supervised fine-tuning that aligns soft-token representations across languages, improving multilingual reasoning in large language models.
...
Method: The authors propose SOLAR, which aligns soft-token representations across languages using English as a pivot, and evaluate it on four multilingual reasoning benchmarks.

Result: SOLAR improves accuracy by up to +17.7 points over the base model and +3.8 over standard supervised fine-tuning, with the largest gains on low-resource languages.

Conclusion: Aligning soft-token representations helps preserve shared semantic structure during multilingual reasoning, and SOLAR is an effective approach to improve multilingual reasoning in large language models.
...
TL;DR: The paper addresses the challenge of extracting problem and method sentences from scientific papers, proposing solutions to overcome the limitations of small-scale datasets and improve model generalization.


<details>
  <summary>Details</summary>
Motivation: The need to identify essential parts from massive scientific text, and the limitations of small-scale datasets in annotating sentences within scientific papers.

Method: The paper proposes FE desensitization-based data augmenters to generate synthetic data, a context-enhanced transformer to reduce noise in context, and conducts experiments using large language model (LLM) based in-context learning (ICL) methods.

Result: The proposed models achieve a higher macro F1 score compared to baseline models on two scientific paper datasets, with improvements of 3.71% and 2.67%, respectively.

Conclusion: The paper concludes that the proposed solutions can effectively improve the extraction of problem and method sentences from scientific papers, and that LLM-based ICL methods are not suitable for this task.
...
TL;DR: The paper explores using EEG signals to improve automatic keyphrase extraction (AKE) on microblogging platforms, finding that EEG features consistently improve AKE performance, with EEG features providing the largest gains.


<details>
  <summary>Details</summary>
Motivation: The challenge of automatic keyphrase extraction (AKE) on microblogging platforms due to short, noisy, and dispersed user content, and the limitations of using eye-tracking signals alone.

Method: Using EEG features and eye-tracking features in microblog-based AKE models, specifically injecting these features into the input of the soft-attention layer and the query vectors of the self-attention layer.

Result: Cognitive signals, particularly EEG features, improve AKE performance across different models and feature combinations, with EEG features bringing the largest gains.

Conclusion: EEG signals provide useful cognitive evidence for microblog-based AKE, and multimodal cognitive signals, including EEG and eye-tracking features, deserve further investigation.
...
TL;DR: The authors propose TempoWave, a new interface for large language models (LLMs) to improve context-aware time series forecasting by converting numerical values into digit-wise embeddings, leading to better performance and state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the misalignment between LLMs' discrete tokenization and continuous numerical values, which can harm numerical ordering and forecasting reliability.

Method: The authors propose a plug-and-play temporal wavelet digit interface called TempoWave, which maps scalar observations into digit-wise embeddings constructed from multi-wavelet, multi-scale coefficients.

Result: TempoWave consistently improves LLM-based forecasters over standard numeric tokenization and alternative embedding interfaces, achieving a new state-of-the-art across five context-enriched forecasting benchmarks.

Conclusion: The authors conclude that the numeric interface is a key bottleneck in LLMs for time series forecasting and that principled multi-resolution embeddings can better couple LLMs' contextual reasoning with precise forecasting.
...
TL;DR: This paper compares two approaches for classifying sentences from German-language climate news articles as threat-oriented, solution-oriented, both, or neither.


<details>
  <summary>Details</summary>
Motivation: To develop automated detection of framing patterns in climate news articles at the sentence level, which would enable researchers to analyze large corpora that are infeasible to code manually.

Method: Two approaches are compared: (1) few-shot prompting with a large language model (Llama 4 Maverick) and (2) fine-tuning a German BERT model (deepset/gbert-large) for sentence-pair classification.
...
Conclusion: The results contribute to the comparison of fine-tuned encoder models with prompted generative models for text classification in computational social science, and show that providing context improves BERT classification performance.
...
TL;DR: A new diffusion language model called TwoTower is proposed, decoupling context representation and iterative denoising into two towers, achieving higher generation throughput while retaining quality.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion language models use a single network for both context representation and iterative denoising, limiting its capacity. The authors aim to decouple these roles to improve performance.

Method: TwoTower, a block-wise autoregressive diffusion model with two towers: a frozen AR context tower and a trainable diffusion denoiser tower with bidirectional block attention.

Result: Nemotron-TwoTower retains 98.7% of the autoregressive baseline's quality while offering 2.42X higher wall-clock generation throughput.

Conclusion: The proposed TwoTower model achieves a better balance between quality and generation throughput, making it a promising alternative to autoregressive models.
...
TL;DR: The paper introduces MemStrata, a retrieval memory system that maintains temporal validity of facts, addressing the limitations of Retrieval-augmented generation (RAG) in handling evolving knowledge.


<details>
  <summary>Details</summary>
Motivation: RAG struggles with temporal awareness, often retrieving both outdated and current facts with similar embedding similarity, leading to serving superseded information.

Method: MemStrata stores facts like RAG but uses a deterministic supersession rule to retire stale values in a bi-temporal ledger, eliminating the need for similarity thresholds or LLM calls.

Result: MemStrata achieves near-perfect accuracy (0.95-1.00) on evolving knowledge, compared to RAG's 0.20-0.47, and reduces stale-fact-error rates to ~0%, with retrieval latency of ~2.1s.

Conclusion: MemStrata effectively addresses the structural problem of RAG in handling temporal validity, providing a more accurate and efficient solution for managing evolving knowledge.
...
TL;DR: The paper proposes a new approach to analyzing narrative disclosures in corporate reports over time, applying it to Japan's 2019 disclosure reforms.


<details>
  <summary>Details</summary>
Motivation: Evaluating qualitative changes in narrative disclosures over time is challenging due to their multidimensional nature.

Method: A longitudinal text analysis approach combining NLP metric extraction, paired testing, shift function analysis, and inter-metric correlation.

Result: The analysis of 19,770 firm-year observations reveals complex shifts in disclosure patterns, including increased disclosure volume but decreased readability.

Conclusion: The proposed approach provides a more comprehensive understanding of changes in narrative disclosures, highlighting the need for multi-dimensional analysis.
...
<details>
  <summary>Details</summary>
Motivation: The evaluation of AI safety often relies on a model's ability to detect specified hazards, but accidents can occur due to unspecified hazards.

Method: The study tested language and vision models across various tasks, including radiology and driving scenarios, and evaluated their reporting of co-present safety-critical signals.

Result: The results showed that all tested models exhibited suppression of safety-critical signals when conditioned on a narrow task, which persisted regardless of model size and was more dependent on model family.

Conclusion: The findings suggest that measured benchmark safety may not reflect real-world safety, as models can score well on specified hazards while remaining blind to critical ones, highlighting the need to address the "Inattentional Gap".
...
TL;DR: The authors propose DiARC, a method to improve model reasoning on Abstraction and Reasoning Corpus (ARC) tasks by distinguishing negative samples, leading to consistent performance improvements over baseline models.


<details>
  <summary>Details</summary>
Motivation: Current methods for solving ARC-like problems focus on data augmentation and supervised fine-tuning, but the authors argue that distinguishing negative samples is also crucial for improving model reasoning.

Method: The authors propose DiARC, which constructs preference pairs to enable the model to distinguish between positive and negative samples. They introduce three ways to construct negative samples: output-level visual transformations, DSL-level rule inversion, and task-specific rule editing.
...
Conclusion: The authors conclude that DiARC is an effective method for improving model reasoning on ARC-like problems by leveraging negative sample supervision.
...
TL;DR: The paper proposes Erase-then-Delta Attention (EDA), a memory update rule that decouples erasure and writing in recurrent memory models, improving their ability to manage memory.


<details>
  <summary>Details</summary>
Motivation: Current memory update rules, such as delta-rule linear attention, can correct existing content but cannot actively remove stale information stored at different addresses.
...
Result: Experiments on language model pretraining show that EDA performs best in both dense and MoE model settings, with gains persisting after long-context mid-training and evaluations.

Conclusion: The results suggest that recurrent memory models should decide not only what to write but also what stale information to erase and where, with EDA providing an effective solution.
...
TL;DR: Proposed KIRP, a zero-shot stance detection framework that integrates external knowledge with entity reorganization and employs prompt chaining for reasoning, achieving state-of-the-art performance on multiple datasets.


<details>
  <summary>Details</summary>
Motivation: Existing methods for zero-shot tweet-level stance detection neglect intrinsic semantic cues within key intra-textual entities and struggle to differentiate between 'neutral' and 'irrelevant' stance labels.

Method: KIRP framework, which incorporates knowledge graphs, entity reorganization, reflective Chain-of-Thought (CoT) reasoning, stance-aware contrastive learning, and a three-layer iterative prototype network.

Result: KIRP achieves state-of-the-art performance on SemEval-2016, WT-WT, and KIRP-D datasets, with F1 scores of 84.05%, 84.99%, and 79.18%, respectively.

Conclusion: KIRP effectively addresses the challenges of context sparsity and target relevance in zero-shot tweet-level stance detection, demonstrating improved performance over existing methods.
...
TL;DR: The study improves text-to-speech (TTS) models for low-resource languages like Khmer, achieving a significant increase in quality with minimal parameter updates.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the quality gap in TTS models for languages with limited training data, such as Khmer and Korean.

Method: Using a 2.4B-parameter TTS model (VoxCPM2) and a single Low-Rank Adaptation (LoRA) adapter for both languages, with a shared, language-tagged corpus of about 26 hours.

Result: The Khmer Mean Opinion Score (MOS) increases from 3.85 to 4.23 with the best adapter, a significant gain with minimal parameter updates (0.19 to 3.03 percent).

Conclusion: The adaptation helps mainly where the base model is weak, with significant improvement for Khmer but no gain for Korean, which the base model already handles well.
...
TL;DR: CAT-Q is a post-training quantization scheme for compressing and accelerating large language models (LLMs) by converting them into ternary models, achieving superior performance with significantly reduced training tokens.


<details>
  <summary>Details</summary>
Motivation: Existing ternary quantization methods for LLMs require data-intensive and costly quantization-aware training, which can be mitigated by a simple yet effective post-training quantization scheme.

Method: CAT-Q consists of two key components: learnable modulation (LM) and softened ternarization (ST), which are coupled from an optimization perspective to efficiently quantize pre-trained LLMs into ternary models.

Result: CAT-Q can quantize pre-trained LLMs with 1.7B to 8B parameters into ternary models using only 512 calibration samples, achieving superior performance than state-of-the-art methods with a 100,000X reduction in training tokens. It can also quantize larger LLMs with 14B to 235B parameters within 8 to 60 hours on 8 A100-80GB GPUs.

Conclusion: CAT-Q offers a cost-efficient and accurate ternary quantization approach for compressing and accelerating LLMs, demonstrating its effectiveness and efficiency in achieving superior performance with minimal training tokens and time.
...
<details>
  <summary>Details</summary>
Motivation: The authors aim to advance personalized language-model assistants by enabling them to infer user preferences from multimodal data, rather than just relying on explicitly stated preferences.

Method: The authors create a benchmark, SocialPersona, which consists of longitudinal social-media timelines of 171 users, with text, images, timestamps, and human-verified preference tags. They use this benchmark to evaluate the performance of multimodal large language models on two tasks: constructing structured user profiles and generating responses aligned with inferred profiles.

Result: The experiments show that models can identify broad interest domains, but their performance drops on fine-grained and recent interests, and degrades further when inferred profiles must be used to personalize dialogue. The results also indicate that text and images provide complementary preference signals.
...
TL;DR: This paper proposes a framework that combines logical structures with linguistic cues to improve automated classification of logical fallacies using Large Language Models (LLMs).


<details>
  <summary>Details</summary>
Motivation: The study aims to address the challenge of nuanced forms of logical fallacies that complicate automated classification in the context of growing information disorder.

Method: The framework inductively extracts patterns from fallacious examples and their explanations using LLMs, and evaluates their impact across different LLMs and experimental configurations.

Result: The approach shows statistically significant improvements over zero-shot baselines, outperforms competing approaches, and validates generalization through cross-dataset experiments.

Conclusion: The study concludes that merging abstract logical structures with context-level linguistic cues is beneficial for fallacy classification, and that data-driven pattern extraction is an effective method for generating logical representations.
...
TL;DR: The paper presents ConvMemory v3, which adds a validity context layer to optimize conversational memory retrieval by detecting outdated memories and updating them through a dual-evidence gate mechanism.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the issue of retrieved memories being outdated and to optimize conversational memory retrieval by adding a validity context layer.

Method: The method used in this paper is a dual-evidence gate mechanism that conditions a relation judgment on the specific target proposition, scoring a (target, source) pair through the product of a MiniLM slot head and a DeBERTa-v3 slot head and gating it by conservative event/operation evidence.

Result: The results show that the gate reaches 90.12% +/- 1.73 accuracy on a synthetic multi-hop validity benchmark and 98.8% +/- 0.9 group-all-correct on a real-data feedback loop. The deployed layer preserves retrieval by default and raises current-active H@1 from a never-demote baseline of 45.1% to 95.7% +/- 1.2 while protecting non-superseded memories at 99.4% recall.

Conclusion: The conclusion of this paper is that the proposed validity context layer effectively detects and updates outdated memories, optimizing conversational memory retrieval, and the mechanism is validated through various experiments and safety contracts.
...
<details>
  <summary>Details</summary>
Motivation: The reliability and comparability of results in multimedia event extraction depend on consistent and rigorous evaluation, but recent work has reported issues with evaluation standards.

Method: The authors conduct a systematic analysis of evaluation pitfalls and perform controlled experiments under a strict evaluation framework to demonstrate the impact of minor evaluation choices on performance variations.
...
TL;DR: The paper proposes OPID, a framework for on-policy skill distillation in reinforcement learning for language agents, which extracts supervision from completed on-policy trajectories and improves agent performance, sample efficiency, and robustness.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of outcome-based reinforcement learning and on-policy self-distillation in language agents, specifically the lack of guidance on intermediate decisions and the reliance on external skill memories or privileged context.

Method: The method proposed is OPID (On-Policy Skill Distillation), which extracts hierarchical skills from completed on-policy trajectories and uses a critical-first routing mechanism to select the appropriate skill for token-level self-distillation.

Result: The results show that OPID improves agent performance, sample efficiency, and robustness over outcome-only RL and existing skill-distillation baselines on ALFWorld, WebShop, and Search-based QA.

Conclusion: The conclusion is that OPID provides a stable optimization backbone for language agents by preserving RL as the primary training objective while introducing dense, distribution-matched hindsight supervision.
...
TL;DR: The study quantitatively confirms the historical claim of Buddhist Vajrayana vocabulary absorption into Shakta Tantra tradition in Bengal through corpus analysis.


<details>
  <summary>Details</summary>
Motivation: To quantify the historically argued claim of Buddhist Vajrayana vocabulary survival and absorption into Shakta Tantra tradition in Bengal.

Method: Using a corpus of 75 texts, TF-IDF character n-gram vectorization, and cosine similarity analysis to examine vocabulary relationships across eight tradition layers.

Result: The study found significant cosine similarity between Buddhist-Shakta transitional texts and Shakta Kali texts, and measurable evidence of lexical transition within the Buddhist-Shakta transmission chain.

Conclusion: The findings provide quantitative corroboration of historically argued Buddhist-Shakta syncretism in Bengal, and shed light on the evolution of devotional literature vocabulary.
...
TL;DR: The paper presents a submission to the IWSLT 2026 Instruction Following shared task, focusing on developing SpeechLLMs for short-form and long-form speech instruction following.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve speech instruction following capabilities under constrained settings, specifically for short-form and long-form speech instructions.

Method: The method involves developing SpeechLLMs for both short-form and long-form speech instruction following. For the long track, three speech segmentation methods are explored, including fixed 30-second segmentation.

Result: The results show strong performance on MCIF with a SIFS score of 2.0708 for the short track. For the long track, fixed 30-second segmentation achieves the highest HIFS score of 2.0663. Analysis reveals hallucination mainly manifests as repetitive insertions in generated outputs.

Conclusion: The conclusion is that the developed SpeechLLMs achieve strong performance in both short-form and long-form speech instruction following, with fixed 30-second segmentation providing the most robust long-form performance.
...
<details>
  <summary>Details</summary>
Motivation: Deploying large language models on IIoT edge devices requires extreme compression, but existing pruning methods are ineffective at high compression ratios.

Method: A cascaded multi-granularity pruning framework that removes layers, attention heads, and feed-forward channels in a coarse-to-fine order with lightweight low-rank recovery.
...
Conclusion: The proposed framework enables efficient deployment of large language models on IIoT edge devices, but its effectiveness depends on the architecture, as GQA+SwiGLU designs show significant accuracy collapse.
...
TL;DR: The paper proposes InfoKV, an entropy-aware KV cache compression framework that incorporates information-theoretic signals to improve the efficiency of large language models.
...
Method: The authors propose InfoKV, which combines token-level predictive uncertainty with layer-wise representation evolution and integrates the resulting entropy scores with attention scores during reasoning.
...
Conclusion: The paper concludes that InfoKV is an effective approach for KV cache compression, as it takes into account both attention scores and information-theoretic signals to better estimate token importance.
...
TL;DR: Language model representations can serve as neural predictors during natural speech and text comprehension.


<details>
  <summary>Details</summary>
Motivation: To examine the role of language-model representations as neural predictors during comprehension.

Method: Analyzing locked derived data from Brain Treebank, MEG-MASC, and Podcast ECoG with eight frozen language models, blocked encoding models, and controls.

Result: Language-model-derived quantities can annotate neural activity during natural speech and text comprehension, with 67 of 432 evaluable rows meeting a controlled predictive-only criterion.

Conclusion: Language-model features are useful neural predictors, but their predictive usefulness is separate from claims about shared neural organization or language-processing computations.
...
TL;DR: The study evaluates the performance of Automatic Speech Recognition (ASR) models in the Indian healthcare context, particularly in multilingual and demographically diverse settings. It finds variability in model performance across languages and demographic groups, and proposes a debiasing technique to improve fairness.


<details>
  <summary>Details</summary>
Motivation: The reliability of ASR in the Indian healthcare context, particularly in multilingual and demographically diverse settings, remains largely unknown, which motivated this study to evaluate ASR performance and propose a solution to improve fairness.

Method: The study conducts a systematic audit of ASR performance on real-world psychiatric interview data in Kannada, Hindi, and Indian English, comparing eight state-of-the-art models. It then fine-tunes two of the best-performing open-source models using various methods and proposes a unified debiasing technique, SamaVaani.

Result: The study reveals substantial variability in ASR performance across models and languages, with some systems performing competitively in Indian English but failing in regional speech. It also finds systematic performance gaps tied to speaker role and gender, which can be mitigated by fairness-aware fine-tuning.

Conclusion: The study concludes that ASR models may not perform equally well across languages and demographic groups, which can lead to inequitable deployment in clinical settings. However, fairness-aware fine-tuning and debiasing techniques like SamaVaani can improve ASR performance and fairness across demographic groups.
...
Conclusion: The paper concludes that GAVEL provides learnable supervision for verifying, explaining, and localizing misalignments between text and images in vision-language models.
...
<details>
  <summary>Details</summary>
Motivation: Existing taxonomy induction methods have limitations in generalizing across heterogeneous sources due to their reliance on document-level representations.

Method: The proposed approach maps documents into a shared representation space using automatic term extraction, enabling robust cross-source alignment, and constructs interpretable hierarchies through data-driven clustering.

Result: Experiments on a large benchmark demonstrate that the method improves cross-source coherence and hierarchy quality over baselines, with a case study showing its practical utility for technology landscape mapping.

Conclusion: The proposed term-centric framework effectively induces hierarchical taxonomies from heterogeneous corpora, enhancing the organization of knowledge from diverse text sources.
...
TL;DR: The paper surveys safety reporting practices in speech-capable models, finding a lack of multilingual analysis, and introduces RedVox, a benchmark for evaluating safety and fairness in speech models across five languages.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the gap in safety and fairness evaluation of speech-capable models beyond English settings and under naturalistic conditions.

Method: The authors survey state-of-the-art speech model releases, introduce RedVox benchmark, and evaluate eight state-of-the-art models using the benchmark.
...
Conclusion: The paper concludes that there are broader sociotechnical challenges in naturalistic speech safety research, and highlights the need for more comprehensive evaluation of speech models' safety and fairness across languages.
...
TL;DR: This paper investigates how different contextual framings affect the internal representations of large language models (LLMs) used in mental health support tools, finding that framing systematically alters interpretive response tendencies and behavior-associated information remains decodable throughout transformer depth.


<details>
  <summary>Details</summary>
Motivation: The integration of LLMs into mental health support tools and psychologically sensitive applications requires trustworthy human-AI interaction, but semantically similar concerns can be presented through different contextual framings, potentially eliciting different model responses.

Method: The study uses controlled matched prompts across multiple contextual framing conditions and instruction-tuned model families, employing layer-wise probing analyses, held-out framing probes, and activation steering experiments.

Result: The results show that framing systematically alters interpretive response tendencies, behavior-associated information remains decodable throughout transformer depth, and framing-associated representational directions can partially modulate downstream behavioral outcomes.

Conclusion: The findings indicate that robustness to contextual variation is an important consideration when evaluating the consistency and trustworthiness of conversational AI systems deployed in mental-health-oriented interactions.
...
<details>
  <summary>Details</summary>
Motivation: Current open relation extraction approaches have limitations in generalizing to unseen relation types, either through clustering techniques that can't generate relation labels or through large language models that lack discriminative capacity.

Method: The proposed framework, ReaORE, consists of two stages: relation filtering, which reasons over multiple aspects to understand relations and instances, and relation prediction, which uses fine-grained comparative reasoning to predict target relations.

Result: ReaORE outperforms existing baselines in extensive experiments on two widely used open relation extraction datasets.
...
TL;DR: The study tests the presence of emotion vectors in two open-weight language models, Apertus-8B-Instruct-2509 and Gemma-4-E4B-it, and finds that both models exhibit valence geometry, but with different patterns of valence representation across model depth.


<details>
  <summary>Details</summary>
Motivation: To test the generality of emotion vector findings in Claude Sonnet 4.5, and to investigate how valence and arousal representations emerge across different language model architectures.

Method: The study extracts emotion contrast vectors across all layers of two open-weight models, Apertus-8B-Instruct-2509 and Gemma-4-E4B-it, using two model-generated corpora.

Result: Both models exhibit valence geometry, with peak PC1--valence correlations of $r = 0.76$ and $r = 0.83$. However, the models show different patterns of valence representation across model depth, and arousal encoding is sensitive to the extraction corpus.

Conclusion: The study provides evidence for the presence of emotion vectors in two open-weight language models, and highlights the importance of investigating emotion representations across different language model architectures.
...
TL;DR: Introduced MinGram, a tokenizer that simplifies Unigram training while maintaining its performance.


<details>
  <summary>Details</summary>
Motivation: The Unigram tokenizer's complex training process.

Method: MinGram uses BPE-derived seed vocabulary, Hard EM, and score-pruning.

Result: MinGram outperforms BPE and standard Unigram in compression across six languages.

Conclusion: MinGram offers improved compression and morphological alignment.
...
TL;DR: The authors propose a psychology-grounded framework called Psy-CoT for building general-purpose role-playing agents and a role-aware policy optimization method called RAPO to improve character fidelity.
...
Method: The authors propose Psy-CoT, a chain-of-thought framework that decomposes pre-response reasoning into three role-specific steps, and RAPO, a role-aware policy optimization method that uses profile-token mutual information to weight gradients asymmetrically.
...
Conclusion: The proposed Psy-CoT and RAPO methods improve the character fidelity and role-playing abilities of agents, providing a more human-like internal thought process and better out-of-distribution generalization.
...
Conclusion: The paper concludes that NuclearQAv2 is a scalable benchmark for assessing LLM capabilities in technical domains and highlights the importance of multi-faceted evaluation frameworks.
...
<details>
  <summary>Details</summary>
Motivation: Legal outcome prediction requires distinguishing between objective case facts and adjudicative context, which is crucial for merit-based rulings and technical disposals.

Method: The authors introduce a fine-grained outcome taxonomy and a Gated Fusion mechanism to dynamically modulate reliance on judge identity, using a LoRA-adapted Gemma-4 encoder.

Result: The proposed architecture achieves a new state of the art on a benchmark of 13,937 UK Employment Tribunal decisions, requiring fewer trainable parameters than existing methods, with improved performance on ambiguous and rare outcome classes.

Conclusion: The study demonstrates the importance of choosing the right conditioning interface for identity-conditioned classification of legal outcomes, with structured composition yielding more accurate and parameter-efficient models than prompt-based composition.
...
TL;DR: The study introduces a novel paradigm, 'riddle riddles,' to test whether large language models (LLMs) use flexible reasoning or pattern matching. Results show that LLMs and humans fail on this paradigm in opposite directions, suggesting LLMs may rely on memory retrieval rather than flexible strategy selection.


<details>
  <summary>Details</summary>
Motivation: To investigate whether LLMs' accuracy on cognitive tasks is due to pattern matching from training data or flexible reasoning, and to compare their reasoning strategies with those of humans.

Method: The study uses a novel paradigm called 'riddle riddles,' which are word problems that mimic popular riddles but require literal interpretations. Nine state-of-the-art LLMs and 100 human participants were tested across two experiments.

Result: LLMs performed better on genuine riddles (84.9%) than on riddle riddles (50.7%), while humans showed the reverse effect (50.5% on genuine riddles vs. 80.5% on riddle riddles). Error analysis revealed that LLMs' errors on riddle riddles were often due to inappropriate use of inventive reasoning (90.8%), whereas humans' errors on genuine riddles were often due to overextending literal reasoning (57.6%).

Conclusion: The study suggests that LLMs' strong performance on cognitive tasks may not be due to flexible reasoning, but rather to memory retrieval. The results highlight the importance of designing stimuli that can elicit the contrast between genuine reasoning and superficial pattern matching.
...
TL;DR: This paper explores using LLMs for forecasting tasks, analyzing their internal states to understand their reliance on time-specific knowledge versus generalizable patterns, and finds that amplifying time-awareness features can reduce look-ahead bias.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to improve the performance of LLMs in forecasting tasks by understanding how they use time-specific knowledge versus generalizable patterns, and to develop methods to causally shift LLMs toward more historically grounded reasoning.

Method: The method used in this paper involves applying LLMs to various forecasting tasks, analyzing their internal states using sparse autoencoders, intervening on identified features, and evaluating the effects on look-ahead bias and general reasoning performance.

Result: The results of this paper show that amplifying time-awareness features in LLMs substantially reduces look-ahead bias on forecasting prompts while preserving general reasoning performance, whereas steering look-ahead-bias features does not produce an effect.

Conclusion: The conclusion of this paper is that interpretable temporal features can be used to causally shift LLMs toward more historically grounded reasoning, which can improve their performance in forecasting tasks.
...
TL;DR: The paper proposes a new approach to predict processing difficulty in garden path sentences, which can't be well-explained by lexical surprisal. It involves tracking updates to a probability distribution over syntactic trees.


<details>
  <summary>Details</summary>
Motivation: Lexical surprisal fails to predict processing difficulty in garden path sentences, where an initial interpretation is later proven wrong.

Method: The approach involves predicting a probability distribution over syntactic trees (syntactic belief) and updating it after each new word, using a generalized Rényi divergence to measure the magnitude of the update.

Result: The proposed Syntactic Belief Update provides a better fit to human reading time data on garden path sentences compared to lexical surprisal.

Conclusion: The study suggests a new research direction examining non-lexical alternatives to surprisal for psycholinguistics.
...
TL;DR: The paper argues that safety classifiers should model user intent and presents AIMS, a dataset for intent-aware training, showing improved performance across various training regimes.
...
Method: The authors introduce AIMS, a human-annotated dataset, and evaluate intent-aware training across supervised fine-tuning, preference learning, reasoning distillation, and reinforcement learning.

Result: The results show that intent-aware models perform better across various training regimes, with directly rewarding intent faithfulness yielding the strongest average performance.
...
TL;DR: The paper presents a framework integrating formal semantics and evolutionary modeling to study semantic universals, applying it to quantificational meaning and finding that conservativity emerges as an efficient system-wide abstraction.


<details>
  <summary>Details</summary>
Motivation: The literature on semantic universals often models lexicons with fixed signal structures or holistic composition without interpretable lexical parts, contradicting the fundamental insight of formal semantics that sentence meanings arise by recursively composing lexical meanings.

Method: The authors introduce a framework that allows lexical meanings and a composition function to co-evolve under pressures for conceptual simplicity and communicative accuracy, and apply it to the evolution of quantificational meaning.
...
<details>
  <summary>Details</summary>
Motivation: The current state-of-the-art recurrent models have defects, including memory-blind gating, value-axis erase mask, and mathematical prevention of the WY-form triangular chunk solver. These defects limit the performance and efficiency of the models.

Method: The paper proposes CARVE, which resolves the defects by introducing a content-aware erase gate that reuses the recurrent output tensor and replaces the per-value write-gate projection with a single scalar per head.

Result: CARVE achieves better performance on WikiText perplexity, common-sense reasoning benchmarks, and RULER retrieval probes, while also improving efficiency with 0.4% throughput overhead, 13% lower peak memory, and 19% fewer parameters.

Conclusion: The paper concludes that CARVE is a more efficient and effective recurrent model that achieves state-of-the-art results while improving upon existing models.
...
<details>
  <summary>Details</summary>
Motivation: The need to understand dialogic interaction during collaborative problem-solving with intelligent systems capable of autonomous reasoning and strategic cooperation.

Method: A hierarchical two-layer coding scheme integrating cognitive, non-cognitive, and metacognitive regulatory mechanisms.

Result: The framework demonstrates effectiveness and generalizability across nine datasets, providing insights into human-agent coordination and the importance of metacognitive regulation.

Conclusion: The proposed framework can optimize and evaluate human-AI partnerships by analyzing dialogue in collaborative problem-solving contexts.
...
TL;DR: Language models encode knowledge in a task-specific manner, not as a unified knowledge base, affecting reliability and controllability.


<details>
  <summary>Details</summary>
Motivation: To investigate if language models (LMs) behave like a knowledge base where different queries for the same fact return consistent results.

Method: Behavioral and mechanistic analyses, including parameter localization experiments and examination of chain-of-thought reasoning.

Result: LMs encode knowledge in a task-specific manner, with different parameter subsets underlying different tasks for the same fact.

Conclusion: The findings undermine the 'knowledge base' analogy for LMs, suggesting that what the model knows and how it is asked are intertwined in parameter space.
...
<details>
  <summary>Details</summary>
Motivation: The ability of large language models to process historical language is poorly understood, and historical difficulty is often treated as a single barrier, rather than decomposing it into distinct dimensions.

Method: The authors propose a diagnostic framework that decomposes historical difficulty into four dimensions: tokenization cost, predictive uncertainty, semantic robustness, and context sensitivity, and evaluate it on three datasets spanning three centuries.

Result: The results reveal a dissociation between encoding cost and comprehension, with historical texts incurring comparable tokenization penalties but diverging predictive difficulties, and show that a minimal temporal context prompt can reduce historical surprisal by approximately 60%.
...
TL;DR: The paper proposes a context-aware translation cascade for multilingual reasoning, which provides additional context to the final translation module, resulting in strong gains across various benchmarks and language regimes.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of traditional translation cascades for multilingual reasoning, which discard information that later stages may need, leading to loss of cultural grounding, register, and disambiguation cues.

Method: The method proposed is a simple and training-free intervention: a context-aware translation cascade, which provides the original question, the English translated question, and the reasoning trace to the context of the final translation module.

Result: The result shows strong gains for open-ended generation across models and resource regimes, with the original language question carrying most of the beneficial context.

Conclusion: The conclusion emphasizes the need to better design information flow in machine translation cascades for mitigating error propagation and recommends preserving the original user question until the end of the pipeline as a simple and actionable default strategy.
...
TL;DR: The authors propose a comprehensive taxonomy of indirect linguistic expressions (ILE) to detect coded language on social media, and evaluate it using LLMs on 2,000 annotated posts.
...
Method: The authors propose a mechanism-oriented taxonomy of ILE and evaluate it by incorporating it into LLM prompts and comparing it with existing taxonomies.

Result: The proposed taxonomy achieves an improvement of 4.7% in accuracy and 5.4% in F1 over the best-performing benchmark across three LLMs.

Conclusion: The comprehensive, mechanism-oriented taxonomy provides a stable scaffold for detecting emerging coded language and is useful for content moderation.
...
TL;DR: The paper presents a case study on using Large Language Models (LLMs) to verify the eligibility of securities as collateral, achieving high precision in document-level eligibility.


<details>
  <summary>Details</summary>
Motivation: The manual verification of securities against legal and financial criteria within lengthy prospectuses is a resource-intensive task, and traditional Named Entity Recognition (NER) methods have limitations.

Method: The approach uses LLMs to decompose the task into extraction, normalization, and interpretation, allowing for greater flexibility in handling noisy text and interleaved German-English content.

Result: The LLM-based system achieves high precision (up to 91%) in document-level eligibility, exhibiting a conservative operating profile that minimizes false acceptance.

Conclusion: The study demonstrates the effectiveness of LLMs in improving the eligibility examination process, offering a more semantic assessment than traditional location-based metrics.
...
TL;DR: The paper introduces PEEU, a method to improve task planning in multimodal web agents using small open-source models, achieving better performance than larger commercial models.


<details>
  <summary>Details</summary>
Motivation: Small open-source multimodal models struggle with weak planning and limited cross-website generalization, hindering their effectiveness in assisting humans with repetitive GUI tasks.

Method: The Planning Experience Exploration and Utilization (PEEU) method, which explores environments to discover experiences and utilizes hindsight experience to synthesize high-level training data.

Result: The 7B model using PEEU achieves 30.6% accuracy, outperforming the much larger Qwen2.5-VL-32B model, demonstrating the effectiveness of constructing hindsight high-level tasks and leveraging experiences for OOD planning abilities.

Conclusion: Mastering low-level atomic skills does not guarantee high-level planning competence, while high-level task training yields stronger OOD generalization, highlighting the importance of PEEU for small MLLMs.
...
TL;DR: This paper presents a new pipeline for extracting entities and relationships from large amounts of unstructured news text in multiple languages, and demonstrates its effectiveness in two case studies.


<details>
  <summary>Details</summary>
Motivation: The authors aim to overcome the limitations of manual coding and automated text analysis methods in studying complex, informal, and adversarial ties in comparative politics.

Method: The authors propose a modular, open-weight pipeline for multilingual joint entity-relation extraction, combining named-entity recognition (NER) with a three-stage linking cascade and a high-throughput, ontology-constrained mixture-of-experts model.

Result: The pipeline achieves high textual correctness (68.2% strict to 93.7% lenient) in a full-coverage spot-check against a gold standard, and successfully reconstructs a political party's lifecycle and uncovers economic and governance networks in two case studies.

Conclusion: The proposed pipeline provides a robust, replicable foundation for cross-national empirical computational social science by bridging raw multilingual text and structured relational data.
...
TL;DR: The paper presents a method to generate data for interpreting and controlling language model behaviors, focusing on detecting and steering away from sycophancy, and demonstrates improved performance and interpretability.


<details>
  <summary>Details</summary>
Motivation: The need for reliable detection and steering of language model behaviors, particularly sycophancy, which is the tendency of language models to prioritize user validation.

Method: An iterative data generation pipeline that isolates cascading linear features responsible for a behavior, moving beyond simple binary pairs of samples to isolate samples that show degrees of features that scale linearly with behavior.

Result: The method allows for better disentanglement of features, detection, deterministic scoring, and robust steering, matching or outperforming baseline approaches while providing lower computational demand and more interpretability guarantees.

Conclusion: The proposed method improves the interpretability and control of language model behaviors, specifically in detecting and steering away from sycophancy, and provides a more efficient and reliable approach compared to baseline methods.
...
TL;DR: The paper proposes a more comprehensive evaluation framework for agent performance beyond accuracy, and demonstrates its usefulness through a case study on computational reproducibility.


<details>
  <summary>Details</summary>
Motivation: The current benchmark evaluation approach focuses primarily on accuracy, overlooking other crucial dimensions of agent performance.

Method: The authors use CORE-Bench Hard as a case study, introducing improved benchmarks (CORE-Bench v1.1 and CORE-Bench OOD) and conducting experiments to measure various aspects of agent performance.

Result: The study finds that even after accuracy saturation, the benchmark remains useful for evaluating efficiency, reliability, model performance, and scaffold performance, and that human-agent collaboration yields a significant speedup.

Conclusion: The paper presents a more rigorous alternative to the dominant accuracy-centric evaluation paradigm, highlighting the importance of considering multiple dimensions of agent performance.
...
TL;DR: The study finds that in instruction-tuned chat models, a compliant persona can suppress refusal, and refusal is gated at the late-layer expression stage.


<details>
  <summary>Details</summary>
Motivation: The interaction between linear directions in activation space for refusal and persona traits in instruction-tuned chat models has not been studied.

Method: The study extracts a compliant model-persona direction and a refusal direction from Qwen2.5-7B-Instruct and Llama-3.1-8B-Instruct, and intervenes on both.

Result: Compliant persona steering suppresses refusal, and reintroducing the refusal direction partially restores refusal at late layers but not at early ones.

Conclusion: Refusal is gated at the late-layer expression stage, downstream of where it is computed, and treating refusal as a single isolated direction misses its dependence on persona.
...
TL;DR: The paper presents AlgoEvolve, an LLM-driven framework for evolving executable trading strategies, demonstrating emergent regime-adaptive logic and improved search heuristics.


<details>
  <summary>Details</summary>
Motivation: Extending the application of Large Language Models (LLMs) as semantic mutation operators from static coding benchmarks to algorithmic trading, a noisy, non-stationary, and highly discontinuous domain.

Method: AlgoEvolve, an LLM-driven evolutionary framework that generates, evaluates, and improves executable trading strategies expressed as Python code, using a rigorous testing protocol and a meta-evolutionary outer loop.

Result: The system exhibits emergent regime-adaptive strategy logic, including autonomous shifts in trading rules, and the meta-evolutionary loop discovers improved search heuristics that outperform human-designed instructions.

Conclusion: LLM-based semantic evolution provides a viable approach for continual program synthesis in complex environments, such as algorithmic trading.
...
TL;DR: The paper introduces a pipeline for analyzing governance discourse in AI agent protocols and applies it to two contrasting standards, finding that while governance form influences thematic priorities, both exhibit similar levels of participation inequality and community fragmentation.


<details>
  <summary>Details</summary>
Motivation: The governance structures shaping AI agent protocols' interoperability standards are empirically underexamined, and the paper aims to address this gap by introducing a large-scale governance discourse analysis pipeline.

Method: The paper uses an LLM-powered comparative pipeline integrating automated annotation, neural topic modeling, and multi-layer network analysis to study socio-technical power structures at scale.

Result: The analysis of 4,323 governance participation records shows that while governance form influences substantive focus, both regimes exhibit comparable levels of participation inequality and community fragmentation, with discourse alignment being denser in the permissionless setting.
...
TL;DR: A framework integrating online patient narratives and regulatory adverse-event records to provide more comprehensive and auditable psychiatric medication information.


<details>
  <summary>Details</summary>
Motivation: Patients seek medication information online, but safety knowledge for psychiatric drugs is fragmented between regulatory records and patient narratives, which need to be integrated without conflating evidence and anecdote.

Method: A provenance-aware, knowledge-graph-based multi-agent framework unifying Reddit posts, WebMD reviews, and FDA Adverse Event Reporting System records for nine antidepressants, using a large-language-model entity-recognition pipeline.

Result: The framework successfully integrated multiple data sources, showing that patient-generated data form a partly independent safety signal, and adverse events appeared in community sources hundreds of days before corresponding FDA dates.

Conclusion: The study establishes source-aware integration as a route to more auditable psychiatric medication information, with potential usefulness and patient benefit to be tested prospectively.
...
TL;DR: A novel rating system called DD-Elo is proposed, which incorporates move-by-move data into rating adjustments, providing a more responsive and explainable solution for chess rating ecosystems.


<details>
  <summary>Details</summary>
Motivation: Traditional rating systems like Elo suffer from response lag due to their reliance on match outcomes, neglecting the granular quality of gameplay.

Method: The proposed DD-Elo rating system uses a drift diffusion model (DDM) from cognitive neuroscience to integrate move-level data and capture rapid skill fluctuations.

Result: Experiments show that DD-Elo adapts to skill changes faster than Elo, while maintaining theoretical alignment with the traditional Elo system.

Conclusion: DD-Elo offers an explainable, highly responsive, and backward-compatible solution for chess rating ecosystems.
...
TL;DR: The paper proposes a computational governance model for autonomous AI agents, allowing them to operate autonomously while ensuring safety and accountability through independent attestation and cryptographic binding.


<details>
  <summary>Details</summary>
Motivation: The increasing autonomy of AI agents and their potential to perform consequential actions without human oversight, necessitating a governance model that balances autonomy with safety and accountability.

Method: The proposed model involves separating planning and execution authority, requiring independent attestation of preconditions for high-risk actions, and using cryptographic binding and tamper-evident logs for accountability.

Result: The paper presents a proof-of-concept implementation of the proposed model and illustrates its application in software deployment and clinical prescribing scenarios.

Conclusion: The proposed computational governance model enables autonomous AI agents to operate safely and accountably, while maintaining their autonomy in planning and reasoning.
...
TL;DR: This paper presents COrigami, an AI-driven pipeline for generating crease patterns in computational origami, allowing for co-creation of art that meets geometric and aesthetic constraints.
...
Method: COrigami pipeline involves generating a semantic stick figure, computing a base packing, solving for a flat-foldable crease pattern, shaping the flat-folded crease pattern, and refining the generated model using reinforcement learning.

Result: The system generates structural starting points that human artists can further expand and shape, demonstrating reliable, mathematically grounded co-creativity.

Conclusion: The paper shows how AI systems can satisfy multi-objective physical constraints to enable co-creativity in art, specifically in computational origami.
...
TL;DR: Verifying solutions generated by coding agents is becoming increasingly difficult, and current verification methods are insufficient, leading to a need for better reward design and verification strategies.


<details>
  <summary>Details</summary>
Motivation: The growing capabilities of coding agents and foundation models have made generating complex solutions easier, but verifying them has become the harder problem, due to the difficulty of faithfully checking whether a solution fulfills human intent.

Method: The authors characterize verification signals along three dimensions (scalability, faithfulness, and robustness) and study four reward constructions for different task types, conducting experiments to analyze the core challenges of reward design.

Result: Experiments show that targeted verification design can effectively suppress reward hacking, improve task completion quality, and achieve significant gains across multiple benchmarks.

Conclusion: No fixed reward function can remain effective as policy capability continues to grow; verification must co-evolve with the generator to address the central challenge of achieving scalability, faithfulness, and robustness simultaneously.
...
TL;DR: The paper presents an empirical study of tool-augmented LLM agents on real-world energy market analytics tasks, evaluating their performance on 243 expert-curated problems.


<details>
  <summary>Details</summary>
Motivation: The energy sector requires live data retrieval, specialized regulatory and market knowledge, and multi-step quantitative reasoning, which is not adequately addressed by current agentic benchmarks.

Method: The study uses a configurable suite of domain tools, including live electricity market APIs, regulatory docket search, and utility tariff databases, to evaluate agent responses on real-world energy market analytics tasks.

Result: The study provides a comparative analysis of closed-source and open-source LLMs, assessing their performance using a multi-dimensional evaluation protocol that scores approach correctness, answer accuracy, attribute alignment, and source validity.

Conclusion: The paper highlights the importance of domain-specific tooling and evaluation protocols for LLM agents in high-stakes professional domains like energy market analytics.
...
TL;DR: The paper discusses the evaluation of multimodal large language models (MLLMs) and identifies gaps in current benchmarks.


<details>
  <summary>Details</summary>
Motivation: The rapid advancement of MLLMs' capabilities has not been matched by progress in their evaluation, with most benchmarks limited to isolated tasks and not assessing multimodal integration.

Method: The paper reviews existing evaluation benchmarks and taxonomies for MLLMs to identify gaps.

Result: The identified gaps include temporal-spatial coherence, physical world understanding, multimodal consistency, and selective attention.

Conclusion: Addressing these gaps is essential for accurately measuring progress in multimodal intelligence and understanding the capabilities and limitations of MLLMs.
...
TL;DR: The paper introduces OpenFinGym, a unified environment for developing and evaluating large language model agents in quantitative finance, addressing the limitations of current fragmented evaluation methods.


<details>
  <summary>Details</summary>
Motivation: The evaluation of large language model agents in quantitative finance is currently fragmented across isolated tasks, overlooking financial relevance and failing to assess agent competence in multi-stage workflows.

Method: The authors propose OpenFinGym, a unified gym environment that covers multiple tasks such as forecasting, market generation, real-time trading, and fraud detection, with features like automated task construction, containerized runtime, and paper trading engine.
...
Conclusion: The paper concludes that OpenFinGym can help address the limitations of current evaluation methods and provide a more comprehensive and realistic assessment of large language model agents in quantitative finance.
...
TL;DR: The paper identifies a recurring issue in prompt-composed agentic systems called compositional behavioral leakage (CBL), where modifying one prompt module affects others without shared variables or dependencies.


<details>
  <summary>Details</summary>
Motivation: Practitioners report a recurring failure mode in prompt-composed agentic systems where editing one prompt module affects others without shared variables or executable dependencies.

Method: The authors probe CBL on a deployed job-evaluation agent (Claude Sonnet 4.6, 144 trials) using a reusable three-channel protocol that perturbs non-focal modules along volume, content, and form.

Result: Only the content channel produces a detectable paired effect (Cohen's d = 0.63, bootstrap 95% CI excluding zero); no recommendation flipped -- a sub-threshold regime invisible to standard QA but compounding across the thousands of decisions a deployed agent makes.

Conclusion: The authors contribute an operational definition, a reusable protocol, a falsifiable prediction set, and a system-class characterization, establishing cross-module interference measurement as a requirement for prompt-composed agent evaluation.
...
TL;DR: The paper discusses the concept of accelerating returns in technological progress, but argues that it doesn't solve the central problem of scientific discovery, which requires qualitative reasoning. It proposes the Qualitative Engine for Science (QES) as a response to this missing capacity.


<details>
  <summary>Details</summary>
Motivation: The author aims to address the central problem of scientific discovery, which is not solved by accelerating returns in technological progress. This problem is the need for qualitative reasoning about when a current framework is structurally inadequate and what conceptual move is needed next.

Method: The paper provides a simple mathematical interpretation of the accelerating returns thesis and argues that it applies most naturally to executional and infrastructural capability, not genuine discovery. It also references recent results from ARC-AGI-3 to highlight the gap between current AI and human flexible reasoning.

Result: The paper concludes that the Kurzweil theory helps explain why quantitative capability may accelerate, but it does not solve the central problem in scientific discovery. It proposes QES as a response to this missing capacity, which is essential for scientific discovery and human wisdom.

Conclusion: The paper positions QES as a response to the missing capacity in scientific discovery, which is not addressed by accelerating returns in technological progress. Its value lies in preserving, organizing, and making accessible the processes of scientific discovery as a form of human wisdom.
...
TL;DR: The paper introduces a system called narration-of-thought (NoT) that improves chain-of-thought reasoning on moral dilemmas by reducing stakeholder collapse and uncertainty suppression.


<details>
  <summary>Details</summary>
Motivation: The authors identify two failure modes in standard chain-of-thought on moral dilemmas: stakeholder collapse and uncertainty suppression. They aim to address these issues with a new system.

Method: The authors propose NoT, a system that structures chain-of-thought into five sections: protagonist, stakeholders, two-step consequences, uncertainty, and commitment. NoT does not require additional training, parameters, or fine-tuning.

Result: NoT significantly reduces stakeholder collapse from up to 31% to under 1% and uncertainty suppression from up to 72% to 1-24% across various models and generators. It also shows improved performance in stakeholder count and uncertainty score.

Conclusion: The proposed NoT system provides a more reliable and auditable approach to chain-of-thought reasoning on moral dilemmas, which can be beneficial for dependable agentic deployment.
...
TL;DR: The paper proposes a Geometry-Aware Monte Carlo Tree Search (MCTS) framework to solve extremal problems in combinatorial geometry, achieving new best-known results for various grid sizes.


<details>
  <summary>Details</summary>
Motivation: Classical exact solvers and standard machine learning models struggle with combinatorial explosion, sparse reward, and token-consumption limits in solving geometric constraint problems.

Method: The proposed Geometry-Aware MCTS framework enforces geometric constraints through incremental updates to the feasible action space, exploits geometric symmetries, and uses canonical pruning and symmetric batch transitions to improve search efficiency.

Result: The framework establishes new best-known computational results on five out of six problems, finding configurations of size roughly 1.8n for Max-N3IL and 0.95n for the Smallest Complete Set problem.

Conclusion: The Geometry-Aware MCTS framework is a highly adaptable approach for discovering novel configurations in combinatorial geometry, overcoming bottlenecks in classical and machine learning-based methods.
...
TL;DR: This paper proposes an agentic aggregator framework for coordinating electric bus fleets, optimizing scheduling, and managing vehicle-to-grid (V2G) operations.


<details>
  <summary>Details</summary>
Motivation: The increasing complexity of operational tasks and the need for coordination between heterogeneous data sources and processes, particularly in electric bus fleets.
...
Result: The results show that agentic aggregation can support adaptive fleet-grid coordination, but also reveal a trade-off between reducing operational complexity and extracting value from the public transport operator (PTO).
...
TL;DR: The paper discusses a problem faced by 'oracle' AI that estimates probabilities of future events, and proposes a self-referential alternative to report a credal set that is unbiased and self-consistent.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the self-reference problem faced by 'oracle' AI, where its answer can change the probability it was asked to report.

Method: The method involves using the Knaster-Tarski fixed-point theorem on the complete lattice of closed credal sets to find a canonical, nontrivial member of the credal set.

Result: The result is a credal set that is simultaneously unbiased and self-consistent with the consequences of being learned, which collapses to the classical point answer for non-performative questions.

Conclusion: The paper concludes with the development of a lattice-theoretic approach that extends to arbitrary random variables, and poses open questions about the generalization of the interval characterization.
...
TL;DR: The paper evaluates confidence interval methods for text classification performance metrics, recommending methods like Agresti-Coull and Wilson for improved accuracy, especially with small to moderate sample sizes.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the inconsistent reporting of uncertainty measures alongside point estimates like recall and precision in text classification, and to provide guidance on appropriate interval estimation methods.

Method: The paper uses simulations to evaluate various confidence interval methods for performance metrics under conditions typical of social science text classification, including small to moderate sample sizes and texts nested within individuals.

Result: The results show that default methods like the Wald interval and basic percentile bootstrap are often inaccurate, while methods like Agresti-Coull, Wilson, Clopper-Pearson, and a novel pseudo-count regularized bootstrap offer improved accuracy. The hierarchical bootstrap is found to be more accurate than the cluster bootstrap under certain conditions.

Conclusion: The paper concludes by providing guidance on appropriate interval estimation methods to improve the transparency of machine learning applications and encourage greater attention to validation sample size at the design stage.
...
TL;DR: The paper explores the limitations of supervised deep learning in achieving symbolic-level syllogistic reasoning, finding that data-driven systems cannot match symbolic methods due to methodological limitations and flaws in training data and targets.


<details>
  <summary>Details</summary>
Motivation: To investigate the limitations of supervised deep learning in achieving symbolic-level syllogistic reasoning and compare it with symbolic methods.

Method: Theoretical analysis and experimental evaluation of Euler Net and ChatGPTs (GPT-5-nano and GPT-5) on syllogistic reasoning tasks with different surface forms.

Result: The study reveals two methodological limitations of supervised deep learning: (1) insufficient training data to distinguish all valid syllogistic reasoning types, and (2) contradictory training targets for pattern recognition and logical reasoning. Experiments show that Euler Net and ChatGPTs cannot achieve rigorous syllogistic reasoning.

Conclusion: Supervised machine learning systems will not attain the rigor of symbolic logical reasoning due to inherent methodological limitations, even with increased training data and time.
...
TL;DR: The paper introduces MKG-RAG-Bench, a benchmark for evaluating retrieval in multimodal knowledge graph RAG (MKG-RAG) systems, highlighting the challenges of retrieval in heterogeneous multimodal knowledge graphs.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks overlook the challenges of retrieval in multimodal knowledge graph RAG (MKG-RAG), and retrieval is a critical bottleneck due to the heterogeneous nature of multimodal knowledge.

Method: The authors construct MKG-RAG-Bench using an LLM-based curation pipeline, creating a cross-domain benchmark from two multimodal knowledge graphs with carefully aligned question-answering datasets.

Result: The experiments show that effective multimodal retrieval remains challenging yet crucial for end-to-end MKG-RAG performance, and that retrieval quality strongly determines generation outcomes.

Conclusion: MKG-RAG-Bench provides a principled foundation for diagnosing current limitations and advancing multimodal knowledge graph RAG systems by isolating retrieval as a first-class evaluation target.
...
TL;DR: The paper presents an AI-based system called auto-psych that uses automated discovery techniques to generate theories in computational cognitive science by collecting human data through crowdsourced survey experiments.


<details>
  <summary>Details</summary>
Motivation: The major bottleneck in AI-based scientific automation is data collection. The authors aim to address this issue in computational cognitive science, where theories are often represented as code and crowdsourcing platforms enable programmatic human data collection at scale.

Method: The authors use an agent-based system with nested discovery loops to generate explanatory theories of human behavior. The inner loop conjectures, fits, and critiques probabilistic cognitive models, while the outer loop designs experiments, launches them online, and analyzes the data.

Result: The system can quickly and reliably recover ground-truth theories from synthetic data and, in three independent sequences of human experiments, finds theories that fit the data better than theories generated from the scientific literature.

Conclusion: The work demonstrates the feasibility of automated data collection and theory discovery in computational cognitive science, showing that AI-based scientific automation is possible in this field.
...
TL;DR: The authors propose a Clinical Harness architecture for developing and managing AI in clinical care, enabling persistent and accountable capabilities across time.


<details>
  <summary>Details</summary>
Motivation: Current Medical AI models are isolated and do not meet the needs of clinical care, which requires capabilities that persist over time and are accountable.

Method: The authors propose a runtime governance architecture called Clinical Harness, which includes registering, orchestrating, guarding, and monitoring AI-enabled clinical capabilities. They use osteoporosis as an example to demonstrate the application of knowledge-driven, data-driven, and physics-enhanced skills in supporting lifecycle care under runtime governance.

Result: The proposed Clinical Harness architecture and clinical AI skills provide a framework for developing and managing AI in clinical care, enabling more persistent and accountable capabilities.

Conclusion: The authors conclude that their proposed approach can support lifecycle care under runtime governance, using osteoporosis as an exemplar.
...
TL;DR: The study compares the problem-solving behavior of large reasoning models (LRMs) and humans, finding that while both show similar patterns across items, they diverge within items: humans spend less time on problems they get wrong, while LRMs spend more tokens on problems they get wrong.


<details>
  <summary>Details</summary>
Motivation: The study aims to understand the differences in problem-solving behavior between humans and LRMs, specifically how they allocate time or tokens to problems they get right or wrong.

Method: The study analyzes a public matched human-LRM corpus, comparing the response times of humans and five thinking LRMs on a set of problems, and separating two levels of deliberation: registration (how response time tracks difficulty across items) and allocation (whether an agent spends more on its own failures or successes).

Result: The study finds that humans and LRMs show similar patterns across items (registration), but diverge within items (allocation): humans spend less time on problems they get wrong, while LRMs spend more tokens on problems they get wrong, with a large wrong-vs-right effect (Cohen's d = 1.47-3.13 on H-ARC).

Conclusion: The study suggests that humans and LRMs have different stopping policies, with humans showing 'engagement versus abandonment' and LRMs showing 'length driven by uncertainty', and that these differences are masked by traditional measures that only consider cross-item correlations with difficulty.
...
<details>
  <summary>Details</summary>
Motivation: The goal is to bridge the gap between offline EEG analysis and real-time, quality-gated cognitive-load pipeline, making it easier for researchers, developers, and applied teams to create prototypes.

Method: The tutorial uses a quality-gated workflow, including EEG preprocessing, quality control, Alpha feature extraction, and a web API.

Result: The agent processed 18 recordings, generated 10 within-subject comparisons, observed task-related posterior Alpha suppression in 7 of 10 contrasts, and estimated initial evidence of within-subject repeatability.

Conclusion: The tutorial provides a transparent path from EEG files to real-time visual cognitive-load prototypes, closing the offline-to-online gap.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of general language models not being aware of specific sensor capabilities, algorithms, and justified conclusions in scientific software, particularly in low-channel EEG analysis.

Method: The method involves separating a deterministic local EEG engine from a hardware-aware language layer, where the LLM receives a compact, allowlisted summary and a versioned context pack describing the hardware, workflows, and implementation boundaries.

Result: The results show that the system produces identical structured results over multiple repetitions, preserves local artifacts under various failures, and demonstrates hardware- and implementation-aware grounding through a boundary-awareness benchmark.

Conclusion: The conclusion is that hardware- and implementation-aware grounding is a practical mechanism for calibrating what an EEG agent accepts, qualifies, or refuses, but it does not establish clinical validity or a validated absolute cognitive-load index.
...
TL;DR: Develops a framework for interpreting AI systems as agents, providing criteria for determining their beliefs, desires, and meanings.


<details>
  <summary>Details</summary>
Motivation: To enable trust in deployed AI systems by understanding their goals or detecting deception.

Method: Drawing on radical interpretation and mechanistic interpretability, proposing representationalist and interpretationist approaches.

Result: A framework for interpreting AI systems, tying criteria to tests for current interpretability methods.

Conclusion: Interpreting AI systems requires a holistic approach, considering joint constraints on beliefs, desires, and propositional structure.
...
TL;DR: A new model, PMDformer, is proposed for long-term time series forecasting, which addresses challenges in capturing shape similarities across patches and variables.


<details>
  <summary>Details</summary>
Motivation: The difficulty in accurately modeling shape similarities across patches and variables in long-term time series forecasting due to scale differences.

Method: The proposed method includes patch-mean decoupling (PMD) to separate trend and residual shape information, Trend Restoration Attention (TRA) to reintegrate decoupled trends, and Proximal Variable Attention (PVA) to focus on relevant recent time segments.

Result: PMDformer outperforms existing state-of-the-art methods in stability and accuracy across multiple long-term time series forecasting benchmarks.

Conclusion: The proposed PMDformer model effectively captures shape similarity in long-term forecasting scenarios, providing improved stability and accuracy.
...
TL;DR: The study uses machine learning to detect cirrhosis in hepatitis C patients, achieving high accuracy with an Extra Trees model.


<details>
  <summary>Details</summary>
Motivation: Early detection of cirrhosis in hepatitis C patients is crucial to prevent complications, and machine learning has shown promise in diagnosing diseases.
...
Result: The Extra Trees model outperformed others with an accuracy of 96.92%, recall of 94.00%, precision of 99.81%, and an area under the receiver operating characteristic curve of 96% using 16 features.

Conclusion: Machine learning can be effectively used to detect cirrhosis in hepatitis C patients, with the Extra Trees model showing high performance.
...
TL;DR: EvoOptiGraph, a novel framework, improves large language models' optimization modeling by co-evolving data and model, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: The challenges of training corpora lacking structural diversity and static data generation pipelines decoupled from model learning.
...
Result: EvoOptiGraph significantly outperforms existing methods in accuracy, executability, and generalization on six public datasets.
...
<details>
  <summary>Details</summary>
Motivation: The outputs of AI-based decision-making in telescope scheduling often suffer from inconsistent data references, reasoning errors, and non-executable decisions, limiting their applicability in high-reliability observational tasks.

Method: A multi-level validation and traceable reasoning framework is proposed, which integrates data reference validation, logical consistency checks, and observational and instrumental constraint verification.

Result: The framework improves the executability and reliability of AI scheduling, reduces loss of transient opportunities, and demonstrates a feasible and verifiable pathway for applying AI to high-reliability astronomical observation scheduling.

Conclusion: The proposed framework enhances the reliability and executability of AI-generated decisions in telescope scheduling while maintaining flexibility, and provides a feasible pathway for applying AI in high-reliability astronomical observation scheduling.
...
TL;DR: The paper proposes an automated system to dispatch emails to relevant WhatsApp groups based on their content, using large language models (LLMs) to analyze email contents and route them to intended recipients.


<details>
  <summary>Details</summary>
Motivation: The manual process of handling emails and forwarding their contents to intended recipients is error-prone, time-consuming, and leads to losses in productivity and undue stress.

Method: The system uses agents querying LLMs to analyze email contents and route them to relevant WhatsApp groups, without relying on labelled datasets.

Result: The proposed system provides several benefits, including enhanced productivity and a reduction in the cognitive load associated with reading emails.

Conclusion: The paper explores an alternative mechanism to automate the task of dispatching emails based on their contents, facilitating a smooth flow of information in an organisation.
...
Result: The approach demonstrates stronger alignment with expert judgments than baseline models, reduces LLM fabrication, and improves the reliability and relevance of generated insights.

Conclusion: The results show the practicality of combining large language models with human expertise to support scalable, evidence-based decision-making in public sector organizations, contributing to responsible AI systems that enhance service quality, responsiveness, fairness, and public trust.
...
TL;DR: The paper presents an autoformalization pipeline that converts natural language policy documents into formally verified policies, addressing the issue of agent safety in high-stakes domains.


<details>
  <summary>Details</summary>
Motivation: The need for formal policy enforcement in high-stakes domains, where existing approaches are either unreliable or don't scale to real policy specifications.

Method: An autoformalization pipeline using an LLM-based generator-critic loop to translate natural language policy documents into formally verified policies in the Cedar Policy Language.

Result: The autoformalized policies cover more of the source natural-language specification than hand-coded symbolic enforcement in prior work, as demonstrated on the MedAgentBench benchmark.

Conclusion: The proposed pipeline offers a scalable and reliable way to enforce formal policies in high-stakes domains, improving agent safety.
...
TL;DR: The paper introduces SkillDisCo, a framework that enables agents to learn reusable procedural skills from successful traces, improving efficiency and success rates in task execution.


<details>
  <summary>Details</summary>
Motivation: Agents often solve similar task instances from scratch, resulting in high reasoning costs and long execution traces. The authors aim to find a way to reuse successful execution structures across task instances.

Method: The authors propose a distillation-and-compilation framework, SkillDisCo, which distills reusable subgraphs from successful traces in FSM-defined scenarios and compiles them into executable procedural skills.

Result: Experiments on ALFWorld and WebArena show that SkillDisCo improves success rates and reduces agent turns across benchmarks and model scales.

Conclusion: The paper concludes that representing shared experience as reusable execution structures can benefit task execution, and SkillDisCo is a effective approach to achieve this.
...
TL;DR: This paper presents NebulaExp, a transparent post-training pipeline for large language models, improving their reasoning and human preference following capabilities.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the lack of transparency in existing post-training alignment methods, which hinders community reproducibility and lightweight model optimization.

Method: The authors propose a fully transparent, ablation-driven post-training pipeline called NebulaExp, built on Qwen3-8B-base, with two orthogonal model branches: general instruct model and complex reasoning-specialized model.

Result: The results show improvements in benchmark scores: NebulaExp-Ins-SFT improves the average score from 55.01 to 60.99, and GRPO reinforcement learning further elevates the score to 61.85. For the Reasoning branch, medium-difficulty GRPO RL improves the average reasoning score from 73.88 to 75.17.

Conclusion: The authors provide a fully reproducible empirical post-training recipe for 8B-scale LLMs and comprehensively dissect the capability trade-offs among instruction adherence, mathematical reasoning, code generation, and general knowledge.
...
TL;DR: The authors question whether safety guardrails need to reason and find that a lightweight, label-only encoder (LeanGuard) can match the moderation accuracy of a reasoning guard while using significantly less compute.


<details>
  <summary>Details</summary>
Motivation: The authors are motivated by the observation that current guardrail methods, which generate a chain-of-thought (CoT) before issuing a verdict, can be heavy and slow, which may not be suitable for on-device deployment.

Method: The authors train a lightweight bidirectional encoder and a reasoning guard on the same corpus, then remove the reasoning component from the reasoning guard while keeping everything else fixed, allowing for a controlled comparison.

Result: The authors find that the label-only encoder (LeanGuard) can match the moderation accuracy of the reasoning guard, with a 395M label-only encoder achieving an average F1 of 82.90 $pm$ 0.26 over public benchmarks, while using only a single forward pass over an input of at most 512 tokens, resulting in a ~100x reduction in inference compute.

Conclusion: The authors conclude that the necessity of CoT for moderation is still not proven, and that the current guardrail benchmarks may not be hard enough to reward reasoning, suggesting that a heavier reasoning guard may not be the more robust choice.
...
TL;DR: The paper proposes a new framework called Kalman Prototypical Network (KPN) for fault diagnosis in combined-cycle gas turbines (CCGTs) using few-shot learning, which outperforms conventional methods in accuracy and stability.
...
Method: The authors introduce KPN, a metric-based few-shot learning framework that models class prototypes as latent stochastic states in a dynamic system, and evaluate it using synthetic data sets generated from a high-fidelity Modelica-based dynamic simulation of an offshore CCGT system.

Result: The proposed framework demonstrates superior performance in leak fault detection tasks compared to conventional few-shot learning methods, with improved accuracy and stability under varying support and query configurations.

Conclusion: The KPN framework offers a promising solution for real-world CCGT fault detection where labeled data is limited, by providing improved training convergence and generalization through stabilizing class representations.
...
Method: LithoDreamer formulates the 'Layout-Mask-Resist Image-After Development Image (ADI)' pipeline as a decision-driven multi-step evolution system and uses a contrastive variational optimization paradigm for interpretable intervention optimization.

Result: LithoDreamer achieves state-of-the-art performance in forward evolution and inverse planning.

Conclusion: The proposed LithoDreamer framework has the potential to improve computational lithography by accurately modeling the physical process and optimizing interventions.
...
TL;DR: A novel latent dynamical model was developed to analyze cardiac magnetic resonance imaging (CMR) data, capturing full-cycle ventricular motion and predicting heart failure risk.


<details>
  <summary>Details</summary>
Motivation: Current risk models use limited CMR data, whereas the proposed model encodes bi-ventricular anatomy and full-cycle cine motion to provide more informative cardiac phenotypes.

Method: The model uses a graph-based mesh autoencoder, neural ordinary differential equation (ODE) dynamics, and a covariate-conditioned prior to reconstruct 3D+t ventricular motion and test deviations from expected end-diastolic latent state.

Result: The model improved the stratified C-index from 0.704 to 0.785 in predicting heart failure events in 72,386 UK Biobank participants, outperforming established cardiac markers and alternative approaches.

Conclusion: The proposed model suggests that continuous full-cycle modeling of ventricular motion provides valuable cardiac phenotypes beyond conventional CMR summaries, but requires external validation before clinical use.
...
TL;DR: The paper introduces AHOIS, an autonomous AI scientist that uses Socratic questioning to construct, challenge, and revise physical explanations, demonstrating true autonomous scientific discovery in a complex optical system.


<details>
  <summary>Details</summary>
Motivation: The current state of AI systems in scientific discovery is limited to procedural execution of workflows designed by humans, lacking epistemic autonomy. The authors aim to achieve true autonomous science by developing an AI system capable of constructing, challenging, and revising physical explanations.

Method: The authors propose AHOIS, a multi-agent AI scientist that embeds Socratic midwifery into closed-loop experimentation. A physics-critic agent uses causal questioning, constraint checking, counterexample generation, and falsification-criteria formulation to interrogate hypotheses.

Result: AHOIS autonomously proposed and validated a random-interference encoding hypothesis, discovered task-adaptive sparse-measurement strategies, diagnosed distinct failure modes, and translated a published imaging protocol into an executable workflow on a non-original configuration, achieving 76.97% classification accuracy on MNIST and 83.17% on Fashion-MNIST.

Conclusion: The results demonstrate a route from workflow automation towards evidence-grounded, self-correcting autonomous discovery in complex physical environments, establishing the potential for AHOIS to achieve true autonomous science.
...
TL;DR: The paper proposes a framework for optimizing scientific discovery by treating it as a meta-optimization problem, where the evaluation criteria are also optimized. It introduces 'consensus objective aggregation' to combine LLM-generated objective functions, leading to a stable and self-correcting evaluation criterion. The framework is applied to algorithm discovery for 3-SAT problems, achieving significant improvements in scaling and speedup.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to improve the process of scientific discovery by leveraging large language models (LLMs) not just for exploring the vast state space of theories and experiments but also for dynamically adjusting the evaluation criteria. The authors argue that optimizing the evaluation criteria is crucial for accelerating scientific progress.

Method: The method proposed in this paper is to formalize research as meta-optimization. A key technique introduced is 'consensus objective aggregation,' where multiple objective functions generated by LLMs are combined through correlation-weighted voting. This approach results in a stable and self-correcting evaluation criterion that evolves as scientific understanding deepens.

Result: The result of applying this framework to algorithm discovery for 3-SAT problems on digital MemComputing machines is a significant reduction in the scaling exponent with problem size N from approximately N^2.51 to N^1.33. This improvement translates to a approximately 67x speedup on the largest instances tested.

Conclusion: The conclusion of this paper is that by treating scientific discovery as a meta-optimization problem and introducing techniques like consensus objective aggregation, it is possible to significantly enhance the efficiency of scientific discovery processes. The authors hope that this problem-agnostic framework will substantially aid scientific discovery across various domains.
...
TL;DR: EGG, an Expert-Guided Agent Framework, generates high-performance GPU kernels for large language models by incorporating expert optimization principles, achieving a 2.13x average speedup over PyTorch.


<details>
  <summary>Details</summary>
Motivation: The development of high-performance GPU kernels for large language models heavily relies on manual tuning by domain experts, which is time-consuming and inefficient. Recent LLM-based approaches for automating kernel generation struggle to achieve both correctness and high performance.

Method: The authors propose EGG, a framework that incorporates expert optimization principles to guide LLMs' decisions. EGG decomposes kernel generation into two hierarchical stages: algorithmic structure design and hardware-specific tuning.
...
Conclusion: The proposed EGG framework effectively generates high-performance GPU kernels for large language models by leveraging expert optimization principles, leading to significant performance improvements over existing approaches.
...
TL;DR: The paper proposes ResilPhase, a noise-resilient acceleration framework for diffusion models, which addresses the issue of quality degradation at high acceleration ratios.


<details>
  <summary>Details</summary>
Motivation: The significant inference latency of powerful diffusion models and the quality degradation of recent 'cache-then-forecast' schemes at high acceleration ratios.

Method: Reformulating accelerated inference as stable macro-trajectory extrapolation in ODE space, using a derivative-free barycentric Lagrange extrapolator and a bounded Phase Mapping.

Result: State-of-the-art fidelity under aggressive acceleration ratios on FLUX.1-dev and HunyuanVideo.

Conclusion: ResilPhase effectively alleviates the quality degradation issue of accelerated diffusion models, achieving high-fidelity results at high acceleration ratios.
...
TL;DR: The paper studies the problem of 'memory depth' in long-running language agents, which involves persisting goal-conditioned behavior even after the working context is unloaded. The authors introduce a protocol called 'loop-drift' and evaluate a mechanism called EVAF, which enables durable goal-conditioned tendencies.


<details>
  <summary>Details</summary>
Motivation: Long-running language agents need to persist goal-conditioned behavior beyond the current working context, which is not addressed by traditional retrieval systems.

Method: The authors introduce the loop-drift protocol to test memory depth and evaluate EVAF, a surprise- and valence-gated LoRA consolidation mechanism. They also conduct mechanism controls to analyze the factorization of selective consolidation.

Result: EVAF outperforms retrieval systems on goal persistence and post-unload recovery (0.812--0.904) with a small parametric store, while retrieval is strongest on shallow factual recall (0.956--0.973). Mechanism controls show that selective consolidation can be factorized into controllable dimensions.

Conclusion: The paper concludes that selective parametric consolidation, as embodied by EVAF, supplies memory depth distinct from and complementary to retrieval access, and is essential for long-running language agents.
...
TL;DR: The paper proposes a method for LLMs to automatically incorporate factual knowledge from a knowledge base during token generation, improving factual accuracy and transparency.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enable LLMs to access and incorporate factual knowledge from a knowledge base in a more efficient and transparent way, allowing for easier updates and explanations of the generated content.

Method: The proposed method involves training the model to produce special tokens that trigger a query to the knowledge base, allowing the LLM to pull in factual knowledge during token generation.

Result: The experiments show that the method improves factual grounding in both short and long-form generation, and allows factual revisions to take effect through knowledge base edits rather than parameter updates.

Conclusion: The proposed method enables LLMs to automatically incorporate factual knowledge from a knowledge base, improving factual accuracy, transparency, and efficiency, and allowing for easier updates and explanations of the generated content.
...
TL;DR: The study analyzes Heart Rate Variability (HRV) indices in 40 healthy adults to improve HRV's clinical utility, providing insights into normality, stability, correlation, reproducibility, and consistency of various HRV indices.
...
Method: The study uses computational methods for signal processing and data analysis to analyze time, frequency, and nonlinear indices of HRV in 40 healthy adults.

Result: The study finds that certain HRV indices, such as time-domain and nonlinear indices, follow normal distributions, while others, like HF-related indices, show high correlations and redundancy. The study also identifies specific indices that are best suited for accurately representing HRV components.

Conclusion: The study concludes that the selected HRV indices, including ApEn, IRRR, HRVi, SD2, MADRR, and rMSSD, can accurately represent HRV components and enhance its clinical and research relevance.
...
TL;DR: The paper introduces the Capability Frontier, a method to evaluate the performance of multiple LLMs across various benchmarks, considering both model specialization and multiple generations. It reveals that existing benchmarks underestimate real-world LLM capabilities, particularly in heterogeneous data distributions.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks report accuracy for a single model on a single run, which underestimates real-world LLM capabilities, especially under heterogeneous data distributions.

Method: The Capability Frontier is introduced as a Pareto frontier over a set of models, characterizing the best achievable performance at each cost level under optimal selection across models and generations.

Result: Correcting for single-model evaluation yields a 54% error rate reduction; additionally correcting for single runs yields an 82% improvement, with SOTA accuracy matched at 85% cost reduction.

Conclusion: Collective LLM capabilities are substantially underestimated, with implications for evaluation and deployment in data-heterogeneous, multi-domain settings.
...
TL;DR: The paper proposes a framework called Context-Aware Synthesis of Optimization Pipelines (CASOP) for constructing and evaluating optimization pipelines for order fulfillment in manual picker-to-goods warehouses.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of making decisions such as item assignment, order batching, and picker routing in manual picker-to-goods warehouses, where integrated models are not practical due to organizational boundaries, differing responsibilities, or limited data availability.

Method: The framework consists of a modular repository of algorithms, semantic data and algorithm cards, a taxonomy, a pipeline synthesizer, and a pipeline evaluator. It synthesizes and evaluates context-specific optimization pipelines for order fulfillment.

Result: The framework was demonstrated on 7 benchmark instance sets covering four problem classes, resulting in 1,063,044 valid pipelines.

Conclusion: The proposed framework supports researchers and practitioners in designing, automatically synthesizing, and selecting valid, high-performing algorithmic pipelines for warehouse operations.
...
TL;DR: This paper introduces an AI-assisted framework for Life Cycle Assessment (LCA) interpretation, enabling the translation of environmental improvement opportunities into strategic pathways under uncertainty.


<details>
  <summary>Details</summary>
Motivation: The interpretation phase of LCA lacks structured mechanisms for translating quantified improvement opportunities into actionable strategic pathways under technological, social, and policy uncertainty.
...
Result: The framework successfully supports the translation of impact results into strategic pathways, demonstrated through a hydrogen-enabled diesel reduction use case in an Italian apple production facility.

Conclusion: The approach enables more disciplined translation of impact results into strategic pathways and opens up new avenues for the use of advanced AI tools in LCA studies.
...
TL;DR: AgentX is a multi-agent system that automates the recommendation algorithm iteration process, allowing for self-evolution and autonomous generation, implementation, evaluation, and learning of experiments at a large scale.


<details>
  <summary>Details</summary>
Motivation: The current process of recommendation algorithm iteration is limited by a structural execution bottleneck, relying heavily on human engineers and scaling linearly with headcount rather than with evidence, compute, and accumulated experimental knowledge.

Method: AgentX operates as a self-evolving development engine with four tightly coupled stages: Brainstorm Agent generates proposals, Developing Agent translates proposals into production-ready code, Evaluation Agent conducts online rollout and evaluation, and Harness Evolution layer (SGPO) distills execution trajectories into updates that improve the agents.

Result: AgentX enables the autonomous generation, implementation, evaluation, and learning of recommendation experiments at a scale and pace that manual workflows cannot sustain, making the system self-improving rather than merely automated.

Conclusion: AgentX restructures the production function of recommendation algorithm iteration, overcoming the structural execution bottleneck and enabling innovation to compound with evidence, compute, and accumulated experimental knowledge rather than scaling linearly with headcount.
...
TL;DR: A novel framework called TAVR-VLM is introduced to improve the adaption of Multimodal Large Language Models (MLLMs) in Transcatheter Aortic Valve Replacement (TAVR) planning by reducing diagnostic hallucinations.
...
Result: TAVR-VLM achieves an AUROC of 0.896, boosts CIDEr to 0.936, and reduces the hallucination rate to 8.1%, establishing a new state-of-the-art on the M^3TAVR dataset of 1,482 patients.

Conclusion: The introduction of TAVR-VLM with R-CGA significantly improves the interpretability of evidence-based surgical AI by reducing hallucinations and enhancing anatomical grounding in generated text.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to enable the development and evaluation of AI systems in healthcare domains where access to real-world data is restricted due to sensitivity and privacy concerns.

Method: A modular pipeline is used to generate synthetic clinical notes, combining structured patient generation, semi-structured patient journey simulation, and unstructured clinical note generation using large language models.

Result: A dataset of 70 synthetic patients, each associated with 20-50 clinical notes spanning a full hospital journey, is released at multiple levels of validation.

Conclusion: The dataset supports the development, testing, and evaluation of clinical AI systems without reliance on real patient data, prioritizing internal consistency, variation, and realism.
...
TL;DR: The paper proposes a framework for pattern-preserving attribute retrieval, which aims to find items that satisfy a target attribute while maintaining a specific pattern. The authors introduce a staged framework called MO-DiT+HPPO, which improves the intersection metric over a pretrained generative retriever.


<details>
  <summary>Details</summary>
Motivation: The authors identify a gap in existing embedding-based retrieval methods, which often prioritize returning high-scoring items over preserving a specific pattern. They formalize the task of pattern-preserving attribute retrieval and propose a solution to address this challenge.

Method: The authors propose a staged framework called MO-DiT+HPPO, which consists of raw-sequence pretraining, multi-domain metric-ordered continuation pretraining, tail-centroid fine-tuning, and HPPO. The framework uses metric-ordered training and hybrid preference optimization to align the generated query distribution with the true online objective.

Result: The authors report significant gains in the intersection metric over a pretrained generative retriever across four attribute domains under item- and pattern-holdout protocols. The results show that metric-ordered DiT improves the intersection metric, and HPPO further improves it, with significant gains on seven of eight domain-split cells and a marginal tie on the hardest split.

Conclusion: The paper concludes that the proposed MO-DiT+HPPO framework effectively addresses the challenge of pattern-preserving attribute retrieval. The authors demonstrate the effectiveness of their approach through extensive experiments and ablation studies, which show that the gains come from the metric-ordered training and hybrid preference optimization components.
...
TL;DR: Proposed ReTeX, a framework that recovers task-specific expert performance from a single merged model by predicting and undoing parameter interference.
...
Method: ReTeX predicts additive offsets to undo parameter interference and recover task-expert performance, using a router-free task identifier based on SVD subspace signatures.

Result: ReTeX recovers over 95% of individual-expert performance in vision and NLP domains, and shows emergent adaptive interpolation of expert knowledge for out-of-distribution tasks.

Conclusion: ReTeX effectively recovers task-specific expert performance from a single merged model, improving generalization to unseen tasks and adaptive interpolation of expert knowledge.
...
TL;DR: Researchers address the issue of out-of-distribution generalization in large language models by proposing a method called Task-Perturbed NLL Optimization, which improves task sensitivity and generalization.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the out-of-distribution (OOD) generalization of large language models, which currently remains weak due to task insensitivity.
...
Result: The results show that the proposed intervention improves task sensitivity and OOD generalization while preserving more stable attention to task tokens.

Conclusion: The conclusion is that Task-Perturbed NLL Optimization effectively mitigates the problem of task insensitivity in large language models, leading to better OOD generalization.
...
TL;DR: The study investigates the effectiveness of Chain-of-thought (CoT) training in language models, finding that it improves the model's ability to predict actions directly from prompts, rather than enhancing the reasoning process.
...
Result: CoT training improves the quality of prompt actions and does not widen the advantage of CoT reasoning over prompt actions. Later checkpoints show greater reliance on the prompt and are less likely to revise actions in response to CoT. The intervention of selectively masking action-token supervision improves out-of-domain generalization.
...
TL;DR: The paper proposes a camera planning framework called Look-Before-Move for embodied AI in dynamic 3D environments, enabling active visual perception and decision-making.


<details>
  <summary>Details</summary>
Motivation: The need for embodied AI to actively decide what to observe in dynamic 3D environments, moving beyond passive interpretation of given observations.
...
Result: The framework improves subject perception, intent consistency, and trajectory quality over representative baselines, demonstrating the importance of organizing visual attention before generating camera motion.

Conclusion: The paper presents a novel approach to camera planning in dynamic 3D story worlds, enabling embodied AI to actively decide what to observe and how to compose visual evidence, with potential applications in fields such as robotics and computer vision.
...
TL;DR: The paper explores whether LLMs can benefit from visualizing counterfactual events to improve reasoning abilities, proposing Einstein World Models (EWMs) to integrate visual-temporal rollouts into LLM-based reasoning systems.


<details>
  <summary>Details</summary>
Motivation: The motivation is to investigate if visualising counterfactual events can complement language as a mechanism for complex thought and to improve LLMs' reasoning abilities.

Method: The proposed method is Einstein World Models (EWMs), a blueprint for LLM-based reasoning systems that incorporate visual-temporal rollouts into the reasoning trace.

Result: The paper does not provide specific results, but it proposes a framework (EWMs) that potentially extends LLMs' capabilities for tool calling into the domain of visual thought experiments.

Conclusion: The conclusion is that visualising counterfactual events can potentially complement language and improve LLMs' reasoning abilities, and EWMs provide a framework to achieve this.
...
TL;DR: AURORA-AI, a framework for adaptive resource allocation in AI systems, is proposed to maintain performance and fairness under changing conditions.


<details>
  <summary>Details</summary>
Motivation: Static resource allocation strategies in AI systems degrade performance and fairness under non-stationary conditions.

Method: AURORA-AI uses Hamilton-Jacobi-Bellman feedback control, Lyapunov-based stability monitoring, and a fairness-aware composite utility function.

Result: AURORA-AI outperforms five baseline controllers in a simulated stress test, achieving immediate recovery from disruptions, improved performance, and fairness.

Conclusion: AURORA-AI demonstrates the effectiveness of fairness-aware adaptive orchestration for resilient and human-centric AI deployment.
...
<details>
  <summary>Details</summary>
Motivation: Current methods for terminating multi-agent large language model loops, such as a fixed iteration cap, are syntactic and do not consider whether the answer is still improving, leading to inefficiencies.
...
Result: The results show that the proposed method reduces operational tokens by 38% relative to the fixed iteration cap method at parity quality, and an oracle that selects the best round attains a significant improvement in Information Score.

Conclusion: The paper concludes that the proposed semantic early-stopping method is effective in reducing operational tokens while maintaining quality, and highlights the importance of evaluating the quality of the answer in determining when to stop the loop.
...
Method: The paper reviews common external validity indexes, focusing on set-matching-based measures, and compares their characteristics.

Result: The authors recommend centroid index (CI) for cluster-level evaluation, and pair-set index (PSI) or clustering accuracy (ACC) for point-level evaluation, depending on the specific needs of the analysis.

Conclusion: The conclusion is that the choice of external validity index depends on the level of evaluation (cluster-level or point-level) and the specific requirements of the analysis.
...
TL;DR: The paper proposes JERP, a method for LLM agents to effectively use accumulated interaction experience in multi-step environments by jointly learning experiential rules and policies.


<details>
  <summary>Details</summary>
Motivation: The challenge of making effective use of accumulated interaction experience in multi-step interactive environments for LLM agents, as existing methods have limitations in either interpretability or policy improvement.

Method: JERP, which updates a long-term experiential-rule pool and the policy from the same interaction trajectories, and retrieves task-relevant rules to condition the agent.
...
Conclusion: JERP allows for stable and effective behaviors to be gradually absorbed into the model, keeping the rule pool aligned with the evolving policy.
...
TL;DR: The paper introduces PAVE, a step-wise labeling protocol for root cause analysis (RCA) to evaluate LLM agentic capabilities. It creates OpenRCA 2.0, a benchmark with step-wise causal annotations, and finds that LLMs struggle with exact root-cause identification and causal propagation paths.


<details>
  <summary>Details</summary>
Motivation: Existing RCA datasets are limited as they only label the root cause and not the propagation path, simplifying the task to pattern matching. The authors aim to create a more rigorous evaluation method.

Method: The authors introduce PAVE, a step-wise labeling protocol leveraging fault injection to reconstruct causal propagation paths. They apply PAVE to create OpenRCA 2.0, a cross-system RCA benchmark.

Result: Across 11 frontier LLMs, exact root-cause identification succeeds in 20.7% of cases on average. However, LLMs can identify at least one correct root-cause service in 76.0% of cases but struggle with grounding it in a verified causal propagation path (61.5%).

Conclusion: The study highlights the difficulty LLMs face in RCA tasks, particularly in establishing causal propagation paths. It emphasizes the need for step-wise causal ground truth for trustworthy LLM-based RCA agents.
...
TL;DR: The paper proposes TOPS, a training-free and model-agnostic module for pruning visual tokens in multimodal large language models (MLLMs), achieving significant token reduction while preserving performance.


<details>
  <summary>Details</summary>
Motivation: The efficiency of MLLMs is limited by the large number of visual tokens, which introduces substantial computational overhead. Existing visual token pruning methods have limitations, such as retaining redundant tokens or being agnostic to user instructions.
...
Result: Extensive experiments on 7 MLLM backbones and 14 benchmarks demonstrate that TOPS outperforms prior methods under diverse pruning settings. TOPS removes 77.8% of visual tokens while preserving 100.0% and 100.6% performance on LLaVA-NeXT 7B and 13B models, respectively.

Conclusion: The paper concludes that TOPS is an effective and efficient solution for visual token pruning in MLLMs, which can sometimes mitigate hallucination and inspire future lightweight MLLM design.
...
TL;DR: The paper introduces a new mechanism called the process harness, which integrates legacy workflows into Agentic Business Process Management (Agentic BPM) without replacing the underlying workflow engine.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to uplift legacy workflows into Agentic BPM while preserving the existing workflow engine, allowing for the integration of reasoning, adaptation, and oversight capabilities.

Method: The authors develop the Task-Decision-Flow (TDF) model, which defines a policy-governed agentic layer that can be placed around a deterministic workflow engine. They also propose a framework called CUGA FLO, which realizes the TDF model.

Result: The process harness is demonstrated on a loan approval workflow, showcasing its ability to integrate with legacy workflows and provide agentic autonomy through policy-framed agents.

Conclusion: The process harness provides a novel approach to reconciling imperative and normative requirements in business process management, enabling the integration of deterministic workflow execution with policy-governed agentic autonomy.
...
TL;DR: This paper proposes GAversary, a hybrid Genetic Algorithm to generate adversarial attacks on NLP models, achieving better results than existing methods BAE and A2T by reducing the target model's accuracy significantly, albeit with slightly more perturbations and longer run-time.


<details>
  <summary>Details</summary>
Motivation: Deep learning models are vulnerable to adversarial inputs, particularly in NLP where attacks can have real-world consequences, and existing methods often require access to the model's internal structure.

Method: GAversary, a hybrid Genetic Algorithm that treats the target model as a black box, using GloVe embeddings to propose word replacements and improve semantic similarity of adversarial examples.

Result: GAversary substantially reduces the target model's accuracy on test data compared to BAE and A2T attacks, with the best case reducing 76.8% accuracy to 5.8%, but perturbs more words and has a slightly longer run-time.

Conclusion: GAversary is an effective method for generating adversarial attacks on NLP models, achieving better results than existing methods, but with some trade-offs in terms of perturbations and run-time.
...
TL;DR: BINEVAL is a framework for evaluating Large Language Models (LLMs) that provides interpretable, multi-dimensional scores by decomposing evaluation criteria into binary questions.


<details>
  <summary>Details</summary>
Motivation: Current evaluation methods for LLMs are expensive, slow, or correlate poorly with human judgments, and holistic LLM judges produce opaque scores.

Method: BINEVAL uses a meta-prompt to generate fine-grained evaluation questions, which are then answered by an LLM to yield transparent question-level feedback and calibrated overall scores.

Result: BINEVAL matches or outperforms strong baselines, including UniEval and G-Eval, across multiple benchmarks, with especially strong results on factual consistency benchmarks.

Conclusion: BINEVAL provides a task-agnostic, training-free, and interpretable evaluation framework that combines strong empirical performance with practical diagnostic and optimization value.
...
TL;DR: A new video diffusion transformer model, EO-WM, is proposed for multispectral Earth Observation (EO) forecasting, which incorporates a physically informed conditioning framework to better respond to changed weather forcing.


<details>
  <summary>Details</summary>
Motivation: Existing methods for EO forecasting do not fully capture the complexities of the task, which involves predicting future Earth surface dynamics under changing meteorological conditions with sparse observations and unobserved land-surface states.

Method: The proposed EO-WM model uses a video diffusion transformer architecture with a physically informed conditioning framework that represents meteorological forcing through a climatological baseline, weather anomalies, and cumulative physical stress signals.

Result: Experiments show that EO-WM improves performance on two diagnostic benchmarks, reducing the error in predicted Normalized Difference Vegetation Index (NDVI) decline amplitude by 5.63% and improving directional hit rate by 7.80%.

Conclusion: The proposed EO-WM model demonstrates improved performance in EO forecasting, particularly in responding to changed weather forcing, and the benchmarks and model are made open-source for future research.
...
TL;DR: This paper compares simulation-based inference (SBI) with Markov chain Monte Carlo (MCMC) for Bayesian calibration of mechanistic epidemiological models, showing SBI to be a rapid and efficient alternative.


<details>
  <summary>Details</summary>
Motivation: The motivation is to find a more computationally efficient method for Bayesian calibration of mechanistic epidemiological models, which are crucial for infectious disease forecasting and public-health decision making but can be computationally expensive with traditional methods like MCMC.

Method: The method involves using simulation-based inference (SBI) with neural posterior estimation as an alternative to MCMC for calibrating a mechanistic SECIR epidemiological model. The comparison is done using COVID-19 ICU occupancy data from Germany during 2020 across multiple epidemic phases and inference windows.

Result: SBI shows comparable performance to MCMC in terms of posterior distributions and predictive accuracy but significantly reduces computational runtime. For 31-day inference problems, SBI achieves comparable results in 60-70 seconds on a single GPU, versus 1000 seconds for MCMC on CPUs. For a 201-day inference problem, SBI takes an average of 157 seconds, while MCMC takes over 19,000 seconds.

Conclusion: The conclusion is that SBI provides a rapid and computationally efficient framework for Bayesian calibration of mechanistic epidemiological models, supporting repeated near-real-time inference and rapid outbreak analysis.
...
TL;DR: The study examines 'prompt injection' in LLM-based resume screening, where candidates add subtle, self-promotional text to influence rankings. It finds that prompt injection can improve rankings when few candidates use it, but loses effectiveness as more do, and raises fairness concerns.
...
Method: The study uses controlled experiments to examine the impact of prompt injection on applicant rankings in LLM-based resume screening, under different conditions of candidate quality homogeneity and heterogeneity.

Result: Prompt injection improves applicant rankings when resume quality is homogeneous and few candidates inject. However, its effectiveness diminishes as more candidates inject. In heterogeneous candidate pools, prompt injection is less effective but can allow lower-quality candidates to outrank higher-quality ones.

Conclusion: LLM-based screening is most vulnerable to manipulation when few candidates inject and candidate quality differences are small, raising concerns about fairness in hiring processes.
...
TL;DR: The study examines the limitations of multi-model LLM systems, finding that their accuracy gain is capped by the rate at which all models are wrong on the same query, and that combining models rarely beats the single best model without a strong query-level routing signal.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to understand the limitations and potential gains of using multi-model LLM systems, such as routing, voting, cascades, fusion, and mixture-of-agents, over single-model accuracy.

Method: The authors analyze the error rates of multiple models and introduce the concept of beta, the rate at which every model is wrong on the same query. They use a Clopper-Pearson bound to estimate beta and evaluate its impact on the gain of multi-model systems. They also compare the performance of different models and model combinations across various tasks.

Result: The results show that the gain of multi-model systems is capped by beta, and that combining models rarely beats the single best model without a strong query-level routing signal. The authors found that beta is often underpriced, with observed beta values ranging from 0.052 to 0.127, depending on the task and model combination.

Conclusion: The study concludes that the gain of multi-model LLM systems is limited by the rate at which all models are wrong on the same query, and that simply combining models is not enough to achieve significant accuracy gains. Instead, gains come from models failing on different questions, not from adding more models.
...
TL;DR: A language-based digital twin framework using large language models is proposed for personalized cognitive health monitoring, achieving comparable results to real data in preserving individual characteristics and predicting cognitive scores.


<details>
  <summary>Details</summary>
Motivation: Early detection of Mild Cognitive Impairment (MCI) through non-invasive biomarkers, specifically language and conversational patterns, to enable personalized and continuous cognitive health monitoring.

Method: A language-based digital twin framework leveraging large language models (LLMs) with stylometric cues and contextual metadata, and a multi-head conditional variational autoencoder (cVAE) for fidelity and cognitive consistency evaluation.

Result: The digital twin framework preserves identity-specific characteristics and achieves reconstruction and MoCA prediction errors comparable to real data, outperforming baseline GPT-generated responses.

Conclusion: The proposed language-based digital twin framework highlights its potential as a scalable and non-invasive approach for personalized and continuous cognitive health monitoring.
