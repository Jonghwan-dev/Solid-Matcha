...
TL;DR: DOne, a new framework for Design-to-Code generation, improves upon existing Vision Language Models by decoupling structure understanding from element rendering, leading to better layout and element accuracy.


<details>
  <summary>Details</summary>
Motivation: Current Vision Language Models struggle with reconciling high-level structural hierarchy with fine-grained visual details in Design-to-Code generation, often resulting in layout distortions or generic placeholders.

Method: DOne framework consists of a learned layout segmentation module, a specialized hybrid element retriever, and a schema-guided generation paradigm.

Result: DOne outperforms existing methods in high-level visual similarity (over 10% in GPT Score) and fine-grained element alignment on the HiFi2Code benchmark, with a 3 times productivity gain and higher visual fidelity in human evaluations.

Conclusion: The proposed DOne framework effectively addresses the holistic bottleneck in Design-to-Code generation, offering improved performance and productivity.
...
TL;DR: The paper proposes a new task called camouflage-aware image-text retrieval (CA-ITR) and presents a dataset and a model called CECNet to improve performance on this task.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the under-explored problem of robust image-text cross-modal alignment in camouflaged scene understanding (CSU), which hinders deeper understanding of camouflaged scenarios and their applications.

Method: The method involves constructing a dedicated dataset (CamoIT) and proposing a camouflage-expert collaborative network (CECNet) with a dual-branch visual encoder and a novel confidence-conditioned graph attention (C2GA) mechanism.

Result: The result shows that CECNet achieves approximately 29% overall CA-ITR accuracy boost, surpassing seven representative retrieval models.

Conclusion: The conclusion is that the proposed CECNet and dataset (CamoIT) can effectively improve the performance of camouflage-aware image-text retrieval tasks.
...
TL;DR: The authors propose a framework called Look Twice (LoT) to improve how pretrained multimodal large language models (MLLMs) utilize multimodal evidence to answer knowledge-intensive queries about images.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this work is to address the struggle of MLLMs in identifying the most relevant visual and textual evidence when answering knowledge-intensive queries, which often requires combining visual understanding with external knowledge.

Method: The method proposed is a training-free inference-time framework called Look Twice (LoT), which exploits model attention patterns to estimate relevant visual regions and retrieved textual elements, and then generates the answer conditioned on this highlighted evidence.
...
Conclusion: The conclusion of this paper is that the proposed Look Twice (LoT) framework can effectively improve the performance of pretrained MLLMs in answering knowledge-intensive queries about images by better utilizing multimodal evidence.
...
<details>
  <summary>Details</summary>
Motivation: Large vision-language models lack robustness in medical imaging due to heterogeneous supervision and sensitivity to data regime, and suffer from catastrophic forgetting in continual training.

Method: MedQwen couples a spectrally routed Mixture-of-Experts (MoE) with a theoretically grounded scaling rule, initializing experts from non-overlapping SVD segments and introducing residual compensation and scaling.

Result: MedQwen achieves strong performance across 23 medical datasets, approaching full fine-tuning on zero-shot classification with 339× fewer trainable parameters and reducing sequential forgetting to ∼5%.

Conclusion: MedQwen provides a reliable and efficient solution for medical vision-language models, addressing challenges in robustness and continual training.
...
TL;DR: A method for detecting risky tackles in American football practice videos using a Vision transformer-based model, which improves player safety by enabling early identification of hazardous actions.


<details>
  <summary>Details</summary>
Motivation: Early identification of hazardous actions in contact sports to improve player safety.

Method: Using a Vision transformer-based model with imbalance-aware training to detect risky tackles in American football practice videos.

Result: The model obtained a risky recall of 0.67 and Risky F1 of 0.59 under cross-validation, improving risky recall by more than 8% points compared to the previous baseline.

Conclusion: The vision transformer-based video analysis, coupled with careful handling of class imbalance, can reliably detect rare but safety-critical tackling patterns, offering a practical pathway toward coach-centered injury prevention tools.
...
TL;DR: The authors propose a method to regularize attention scores in Vision Transformers (ViT) by using bootstrapping to generate a baseline distribution of attention scores, which helps to remove spurious attention and improve interpretability.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the issue of noisy and diffused attention maps in ViT, which limits interpretability, by quantifying uncertainty measures of attention scores and obtaining regularized attention scores.

Method: The method involves using statistical learning techniques, specifically bootstrapping, to generate a baseline distribution of attention scores by resampling input features, and then estimating significances and posterior probabilities of attention scores.

Result: The proposed Attention Regularization approach demonstrates a straightforward removal of spurious attention arising from noise, drastically improving shrinkage and sparsity, and the results are validated using both simulation and real-world datasets.

Conclusion: The study highlights bootstrapping as a practical regularization tool when using attention scores as explanations for ViT, and the code is made available for further use and evaluation.
...
TL;DR: The study investigates the relationship between CNN models of the visual system and their ability to represent texture perception in humans. It finds no correlation between a CNN's performance as a model of the visual system and its alignment with human texture perception.


<details>
  <summary>Details</summary>
Motivation: The motivation is to understand whether CNN models that are good at modeling the visual system are also good at representing texture perception in humans, and to explore the mechanisms of texture perception.

Method: The study compares feature correlations computed from a diverse pool of CNNs with their perceptual alignment with the mammalian visual system as measured by Brain-Score.

Result: The study finds no connection between conventional measures of CNN quality as a model of the visual system and its alignment with human texture perception.

Conclusion: The study concludes that texture perception involves mechanisms that are distinct from those modeled using CNNs trained on object recognition, possibly depending on the integration of contextual information.
...
TL;DR: A new method for zero-shot open-vocabulary semantic segmentation of 3D automotive lidar data is proposed, leveraging image generation from text and matching 3D point features with 2D image features.


<details>
  <summary>Details</summary>
Motivation: The recognized image-text modality gap in Vision Language Models (VLMs) such as CLIP, which hinders their application to 3D automotive lidar data.

Method: The proposed method uses image generation from text to create prototype images and matches 3D point features with 2D image features of these prototypes, using a 3D network distilled from a 2D Vision Foundation Model (VFM).

Result: The method achieves state-of-the-art performance for zero-shot open-vocabulary semantic segmentation on nuScenes and SemanticKITTI datasets.

Conclusion: The proposed method effectively addresses the image-text modality gap and demonstrates superior performance in zero-shot open-vocabulary semantic segmentation of 3D automotive lidar data.
...
TL;DR: AffordTissue, a multimodal framework, predicts tool-action specific tissue affordance regions in cholecystectomy procedures, improving over vision-language model baselines.


<details>
  <summary>Details</summary>
Motivation: Current surgical action automation methods lack predictability on instrument-tissue interactions and safe interaction regions, hindering clinical deployment.

Method: AffordTissue combines a temporal vision encoder, language conditioning, and a DiT-style decoder to predict dense affordance regions.

Result: Experiments show AffordTissue substantially outperforms vision-language model baselines (20.6 px ASSD vs. 60.2 px).
...
TL;DR: A new method called GRAZE is proposed for localizing the First Point of Contact (FPOC) in American football practice videos without requiring labeled training data.
...
TL;DR: The paper presents a novel framework for open-vocabulary 3D scene understanding, addressing limitations in current approaches by leveraging Sparse Voxel Rasterization and foundation model AM-RADIO, achieving state-of-the-art performance on benchmarks.


<details>
  <summary>Details</summary>
Motivation: The authors identify two critical limitations in current open-vocabulary 3D scene understanding approaches: spatial ambiguity from unstructured Gaussians and multi-level semantic ambiguity from pooling features over object-level masks.

Method: The authors propose a framework using Sparse Voxel Rasterization (SVRaster) with monocular depth and normal priors for stable geometric foundation, and deterministic, confidence-aware feature registration, along with exploiting foundation model AM-RADIO for dense alignment.
...
Conclusion: The proposed framework effectively addresses the identified limitations, providing a more accurate and robust approach for open-vocabulary 3D scene understanding.
...
TL;DR: EgoFlow, a flow-matching framework, generates realistic and physically plausible 6DoF object trajectories from egocentric video, outperforming baselines in accuracy, generalization, and physical realism.
...
Conclusion: EgoFlow demonstrates the promise of flow-based generative modeling for scalable and physically grounded egocentric motion understanding.
...
TL;DR: A general-purpose pipeline for analyzing human pose data from video is proposed, enabling extraction of meaningful patterns in movement dynamics across various experimental contexts.


<details>
  <summary>Details</summary>
Motivation: The high dimensionality, noise, and temporal complexity of pose data make it challenging to extract meaningful patterns of coordination and behavioral change.

Method: The proposed pipeline combines principled preprocessing, dimensionality reduction, and recurrence-based time series analysis to quantify the temporal structure of movement dynamics.
...
TL;DR: The paper identifies a failure mode in multimodal large language models (MLLMs) where they produce plausible but factually incorrect outputs, and proposes a structured reward approach to improve video understanding.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the issue of poor visual and temporal grounding in MLLMs, where models may fabricate object existence, assign incorrect attributes, or collapse repeated events.

Method: The method involves a compositional consistency audit to decompose captions into factual and temporal claims, and a structured reward approach that integrates instance-aware scene-graph rewards, temporal rewards, and video-grounded VQA rewards.

Result: The results show consistent gains on open-source backbones across temporal, general video understanding, and hallucination-oriented benchmarks, suggesting that structured reward shaping is a practical route to more faithful video understanding.

Conclusion: The paper concludes that standard sentence-level supervision is a weak proxy for faithful video understanding, and that structured reward shaping can improve video understanding by providing more accurate and detailed feedback.
...
TL;DR: AReS, an alternative reprogramming approach, is proposed to adapt closed-box service models more efficiently and effectively, outperforming state-of-the-art methods while significantly reducing API calls.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of Zeroth-Order Optimization (ZOO) for adapting closed-box service models, which suffers from extensive and costly API calls, slow and unstable optimization, and decreased performance with modern APIs.
...
Result: AReS achieves significant improvements over state-of-the-art methods, including a +27.8% gain over the zero-shot baseline on GPT-4o, and +2.5% for VLMs and +15.6% for standard VMs across ten diverse datasets, while reducing API calls by over 99.99%.

Conclusion: AReS provides a robust and practical solution for adapting modern closed-box models, offering improved performance and efficiency.
...
TL;DR: A unified framework called UniRecGen is proposed to integrate feed-forward reconstruction and diffusion-based generation for sparse-view 3D modeling, achieving superior fidelity and robustness.


<details>
  <summary>Details</summary>
Motivation: The tension between reconstruction fidelity and generative plausibility in sparse-view 3D modeling, where feed-forward reconstruction lacks global priors and diffusion-based generation struggles with multi-view consistency.
...
Result: UniRecGen achieves superior fidelity and robustness, outperforming existing methods in creating complete and consistent 3D models from sparse observations.

Conclusion: The proposed UniRecGen framework successfully integrates feed-forward reconstruction and diffusion-based generation, providing a more comprehensive and accurate approach to sparse-view 3D modeling.
...
Method: A nonparametric texture recovery approach using hyperspectral photon streams is proposed, called TAG (thermal anti-ghosting).

Result: TAG outperforms state-of-the-art HADAR across various scenes, enables facial texture and expression recovery, and demonstrates thermal 3D topological alignment and mood detection.
...
TL;DR: Proposed PBSeg, an efficient prototype-based segmentation framework for low-altitude UAV imagery, achieving competitive performance with reduced computational complexity.
...
Conclusion: PBSeg offers an efficient solution for semantic segmentation of low-altitude UAV imagery, balancing performance and computational efficiency.
...
TL;DR: Proposed a novel domain transfer framework for cross-domain retinal vessel segmentation, leveraging latent vascular similarity and iterative co-optimization.


<details>
  <summary>Details</summary>
Motivation: Address performance degradation of CNNs in retinal vessel segmentation when domain shifts exist between training and testing data.

Method: Domain transfer framework with pre-trained generation networks, deterministic inversion, and iterative refinement strategy for co-optimization of generation and segmentation networks.

Result: Achieved state-of-the-art performance in cross-domain retinal vessel segmentation, particularly in challenging clinical scenarios.

Conclusion: The proposed framework enables simultaneous enhancement of cross-domain image synthesis quality and segmentation accuracy.
...
TL;DR: ReFlow is a unified framework for monocular dynamic scene reconstruction that uses a novel self-correction approach to learn 3D motion from raw video.


<details>
  <summary>Details</summary>
Motivation: Existing methods for dynamic scene reconstruction suffer from incomplete scene initialization, leading to unstable reconstruction and motion estimation. They often rely on external dense motion guidance, which introduces additional complexity and potential error propagation.

Method: ReFlow integrates two key modules: Complete Canonical Space Construction for enhanced initialization, and Separation-Based Dynamic Scene Modeling for targeted motion supervision. It also uses a novel self-correction flow matching mechanism, consisting of Full Flow Matching and Camera Flow Matching.
...
Conclusion: ReFlow presents a unified and robust framework for monocular dynamic scene reconstruction, addressing the limitations of existing methods and achieving state-of-the-art results.
...
TL;DR: The paper introduces VideoZeroBench, a benchmark for evaluating video multimodal large language models on long-video question answering with a focus on spatio-temporal evidence verification.


<details>
  <summary>Details</summary>
Motivation: Current evaluations of video multimodal large language models have limitations, including inflated scores that mask deficiencies in fine-grained visual understanding and reasoning, and a lack of verification of spatio-temporal evidence supporting predictions.

Method: The authors present a five-level evaluation protocol that progressively tightens evidence requirements, and a hierarchical benchmark comprising 500 manually annotated questions across 13 domains, paired with temporal intervals and spatial bounding boxes as evidence.

Result: Experiments show that even state-of-the-art models like Gemini-3-Pro struggle with grounded video understanding, with accuracy dropping sharply when grounding constraints are imposed, and most models failing to achieve any correct grounded predictions under the strictest evaluation setting (Level-5).

Conclusion: The paper reveals a significant gap between surface-level answer correctness and genuine evidence-based reasoning in video multimodal large language models, highlighting that grounded video understanding remains a bottleneck for long-video QA.
...
Method: The proposed GAAL paradigm adopts an alternating unimodal learning and shared classifier to decouple the multimodal gradient and facilitate interaction. It also designs uncertainty-based cross-modal gradient surgery to selectively align cross-modal gradients.

Result: Empirical experiments on widely used datasets show the superiority of GAAL through comparison with various state-of-the-art (SoTA) tabular-image fusion baselines and test-time tabular missing baselines.
...
TL;DR: The paper proposes a satellite-free training framework for drone-view geo-localization, which uses multi-view UAV sequences to construct a geometry-normalized representation and retrieves the location without requiring satellite imagery.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for drone-view geo-localization rely on satellite imagery during training, which limits practical deployment when satellite data are unavailable or restricted.

Method: The proposed framework consists of three main stages: drone-side 3D scene reconstruction, geometry-based pseudo-orthophoto generation, and satellite-free feature aggregation for retrieval.

Result: The experimental results show that the proposed SFT framework substantially outperforms satellite-free generalization baselines and narrows the gap to methods trained with satellite imagery.

Conclusion: The paper presents a satellite-free training framework for drone-view geo-localization, which achieves competitive performance without requiring satellite imagery.
...
TL;DR: A new evaluation framework called SHOE is proposed for open-vocabulary human-object interaction (HOI) detection, which incorporates semantic similarity between predicted and ground-truth HOI labels.
...
Method: SHOE decomposes each HOI prediction into its verb and object components, estimates their semantic similarity using the average of multiple large language models (LLMs), and combines them into a similarity score.

Result: SHOE scores align more closely with human judgments than existing metrics, achieving an agreement of 85.73% with the average human ratings.

Conclusion: The proposed SHOE framework provides a flexible and scalable evaluation of HOI detection methods and open-ended generative models, underscoring the need for semantically grounded HOI evaluation.
...
<details>
  <summary>Details</summary>
Motivation: The challenge of adapting models to new environments where out-of-distribution (OOD) samples coexist with in-distribution (ID) samples affected by distribution shifts, and the need to correctly classify covariate-shifted ID samples and reject covariate-shifted OOD samples.

Method: The paper introduces an angular loss to regulate feature norm magnitudes and a feature-norm loss to suppress csOOD logits, and combines these objectives with entropy minimization and maximization to tackle the challenges of open-set test-time adaptation.

Result: The proposed method achieves strong OOD detection while maintaining high ID classification performance on several benchmarks, including CIFAR-10-C, CIFAR-100-C, Tiny-ImageNet-C, ImageNet-C, Cityscapes, and HAC dataset.

Conclusion: The paper presents ROSETTA as a robust open-set test-time adaptation method that effectively addresses the challenges of adapting models to new environments with OOD samples, and demonstrates its effectiveness across different benchmarks and real-world applications.
...
TL;DR: Proposed a method to improve Shape from Focus (SFF) technique by augmenting focal stacks, allowing for depth estimation with only two images.


<details>
  <summary>Details</summary>
Motivation: Current SFF methods require densely sampled, large focal stacks, limiting their practical applicability.

Method: Physics-based focal stack augmentation with auxiliary cues (all-in-focus image and Energy-of-Difference maps) and a deep network with ConvGRUs.

Result: The proposed approach achieves comparable accuracy to state-of-the-art SFF models with minimal stack size, on both synthetic and real-world datasets.

Conclusion: The proposed focal stack augmentation enables SFF methods to estimate depth using a reduced stack of just two images, without sacrificing precision.
...
TL;DR: F3DGS is a federated framework for 3D reconstruction that enables decentralized, multi-agent collaboration, achieving comparable quality to centralized methods.


<details>
  <summary>Details</summary>
Motivation: Existing 3D Gaussian Splatting (3DGS) methods require centralized data access, limiting their use in distributed robotic settings with independent agents and restricted data aggregation.

Method: F3DGS constructs a shared geometric scaffold from local LiDAR point clouds, then uses federated optimization to update appearance-related attributes, with a visibility-aware aggregation method.

Result: F3DGS achieves reconstruction quality comparable to centralized training, with experiments validated on a newly collected multi-sequence indoor dataset.

Conclusion: F3DGS enables efficient decentralized 3D reconstruction across multiple agents, addressing challenges of communication overhead and geometric inconsistency.
...
TL;DR: The paper introduces Tex3D, a framework for optimizing 3D adversarial textures to attack Vision-Language-Action (VLA) models in robotic manipulation, revealing significant vulnerabilities.


<details>
  <summary>Details</summary>
Motivation: To investigate the robustness of VLA models to physically realizable adversarial attacks, specifically 3D adversarial textures, which pose a more realistic and damaging threat.

Method: The authors propose Foreground-Background Decoupling (FBD) for differentiable texture optimization and Trajectory-Aware Adversarial Optimization (TAAO) for ensuring effectiveness across long-horizon and diverse viewpoints.

Result: Experiments show that Tex3D significantly degrades VLA performance across multiple manipulation tasks, achieving task failure rates of up to 96.7% in both simulation and real-robot settings.

Conclusion: The study exposes critical vulnerabilities of VLA systems to physically grounded 3D adversarial attacks, highlighting the need for robustness-aware training.
...
TL;DR: The paper presents a method to automatically annotate morphological traits in biological images, leveraging sparse autoencoders and vision-language prompting to generate interpretable descriptions.


<details>
  <summary>Details</summary>
Motivation: The current process of extracting morphological traits is slow and expert-driven, limiting their use in large-scale ecological studies due to the absence of high-quality datasets linking images to trait-level annotations.

Method: The authors use sparse autoencoders trained on foundation-model features to identify meaningful morphological parts in images. They then use vision-language prompting to generate trait descriptions, creating a dataset called Bioscan-Traits.

Result: The authors construct a dataset of 80K trait annotations spanning 19K insect images and confirm the biological plausibility of the generated descriptions through human evaluation. An ablation study assesses the impact of design choices on the quality of the trait descriptions.

Conclusion: The proposed pipeline offers a scalable way to annotate traits, enabling large-scale morphological analyses and bridging the gap between ecological relevance and machine-learning practicality.
...
TL;DR: LivingWorld is an interactive framework for generating 4D worlds with environmental dynamics from a single image.


<details>
  <summary>Details</summary>
Motivation: Most 3D scene generation approaches focus on reconstructing static geometry, leaving scene-scale environmental dynamics largely unexplored.
...
Result: Generating each new scene expansion step requires 9 seconds, followed by 3 seconds for motion alignment and motion field updates, enabling interactive 4D world generation.

Conclusion: LivingWorld enables interactive 4D world generation with globally coherent environmental dynamics from a single image.
...
TL;DR: Proposed a novel framework for monocular 3D object detection with sparse annotations, leveraging Road-Aware Patch Augmentation and Prototype-Based Filtering.


<details>
  <summary>Details</summary>
Motivation: Monocular 3D object detection struggles with sparse annotations, which is a common real-world scenario due to high 3D annotation costs.

Method: The proposed framework consists of Road-Aware Patch Augmentation (RAPA) for augmenting object patches and Prototype-Based Filtering (PBF) for generating pseudo-labels.

Result: The method demonstrated effectiveness in extensive experiments under sparse supervision.

Conclusion: The proposed framework achieves robust detection under sparse supervision, addressing the challenge of monocular 3D object detection with limited annotations.
...
<details>
  <summary>Details</summary>
Motivation: The increasing difficulty in distinguishing AI-synthesized content from real footage has created a need for robust authentication methods.

Method: The approach uses the Moiré effect to derive a motion invariant that tests the correlation between fringe phase and grating image displacement in video.
...
Conclusion: The proposed DynaVid framework effectively addresses the limitations of video diffusion models by leveraging synthetic motion data, leading to improved performance in generating realistic videos with dynamic motions or fine-grained motion control.
...
TL;DR: Proposed a domain-id and exemplar-free incremental learning framework for embodied multimedia systems to adapt to dynamic environments.


<details>
  <summary>Details</summary>
Motivation: Existing methods rely on domain id and overfit to context-specific noise, limiting their practicability and generalization ability.

Method: Disentangled representation mechanism and weight fusion strategy to eliminate perceptual uncertainty and adapt to new distributions.

Result: Significant reduction in catastrophic forgetting and improved accuracy compared to state-of-the-art methods in exemplar-free and domain-id free setting.

Conclusion: The proposed method enables robust continuous environment adaptation for embodied multimedia systems.
...
Conclusion: The proposed method improves the robustness of rPPG models across different datasets by adapting to appearance variations.
...
TL;DR: GUI Process Automation (GPA) is a vision-based RPA that enables fast and stable process replay with a single demo, offering robustness, reliability, and privacy.


<details>
  <summary>Details</summary>
Motivation: Traditional RPA is fragile and current vision language model-based GUI agents have non-deterministic risks.

Method: GPA uses Sequential Monte Carlo-based localization, readiness calibration, and fast, fully local execution to achieve robustness, reliability, and privacy.

Result: GPA achieves a higher success rate and 10 times faster execution speed compared to Gemini 3 Pro with CUA tools in finishing long-horizon GUI tasks.

Conclusion: GPA provides adaptability, robustness, and security required for enterprise workflows and can be used as an MCP/CLI tool by other agents.
...
TL;DR: Director is a unified spatio-temporal Gaussian representation for volumetric video that models human performance, rendering, and instance-level semantics.


<details>
  <summary>Details</summary>
Motivation: Current Gaussian-based approaches for volumetric video focus on appearance but neglect instance-level structure, limiting tracking and semantic reasoning in dynamic scenarios.

Method: Director uses instance-consistent semantics, Multimodal Large Language Models, 2D optical flow, and geometry-aware SDF constraints to create a unified 4D representation.

Result: Director achieves temporally coherent 4D reconstructions, instance segmentation, and open-vocabulary querying.

Conclusion: Director presents a unified approach for volumetric video that combines high-fidelity rendering, instance-level semantics, and temporal coherence.
...
TL;DR: The paper proposes a novel approach to video object removal by introducing understanding into the erasing process from two perspectives: externally through distillation from vision foundation models and internally through a framewise context cross-attention mechanism.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the challenge of removing object-induced side effects (e.g., shadows, reflections, and illumination changes) in video object removal without compromising overall coherence.

Method: The proposed method consists of two main components: (1) a distillation scheme that transfers relationships between objects and their induced effects from vision foundation models to video diffusion models, and (2) a framewise context cross-attention mechanism that grounds each denoising block in informative, unmasked context surrounding the target region.

Result: The proposed model achieves state-of-the-art performance in video object removal and establishes the first real-world benchmark for future research and community progress.

Conclusion: The paper concludes that the proposed approach enables clear and coherent object removal by understanding the target object, its induced effects, and the global background context.
...
<details>
  <summary>Details</summary>
Motivation: Current video frame interpolation methods operate unidirectionally, leading to motion drift, directional ambiguity, and boundary misalignment, especially in long-range sequences.

Method: A bidirectional framework with learnable directional tokens, cycle-consistent supervision, and a curriculum learning strategy.
...
Conclusion: The proposed method achieves superior performance while maintaining high efficiency, outperforming strong baselines without additional computational overhead.
...
TL;DR: The paper proposes a method to mitigate bias in graph diffusion models, improving generation quality by addressing reverse-starting bias and exposure bias.


<details>
  <summary>Details</summary>
Motivation: Existing graph diffusion models have significant bias problems, resulting in degraded generation quality due to reverse-starting bias and exposure bias.

Method: The paper proposes a comprehensive approach to mitigate both biases, including a newly designed Langevin sampling algorithm and a score correction mechanism.

Result: The approach achieves state-of-the-art results across multiple models, datasets, and tasks, without requiring network modifications.

Conclusion: The proposed method effectively mitigates bias in graph diffusion models, leading to improved generation quality.
...
TL;DR: The paper proposes a two-step method for estimating shared attention via group detection, outperforming previous methods in group detection and shared attention estimation.


<details>
  <summary>Details</summary>
Motivation: Previous methods for shared attention estimation have limitations, such as not detecting actual groups of people or assuming a single attention point per image, which hinder practical applicability and performance.

Method: The proposed method uses a two-step process: (i) generating shared attention heatmaps using individual gaze attention heatmaps and group membership scalars, and (ii) refining group memberships and predicting the final shared attention heatmap.
...
Conclusion: The paper presents a novel approach for shared attention estimation via group detection, demonstrating improved performance and effectiveness in practical scenarios.
...
TL;DR: SteerFlow, a model-agnostic editing framework, enables high-fidelity text-guided image editing by preserving source fidelity through an Amortized Fixed-Point Solver, Trajectory Interpolation, and Adaptive Masking.


<details>
  <summary>Details</summary>
Motivation: Existing flow-based generative models struggle to preserve source fidelity due to limitations in solvers, inversion methods, and feature injection techniques.

Method: SteerFlow introduces an Amortized Fixed-Point Solver, Trajectory Interpolation, and Adaptive Masking to improve source fidelity and editing quality.

Result: SteerFlow achieves better editing quality than existing methods on FLUX.1-dev and Stable Diffusion 3.5 Medium, and extends naturally to complex multi-turn editing paradigms.

Conclusion: SteerFlow provides a model-agnostic editing framework with strong theoretical guarantees on source fidelity, enabling high-fidelity text-guided image editing.
...
TL;DR: Introduced SIComp, a setup-independent framework for projector compensation that generalizes to unseen setups without fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Existing projector compensation methods are highly setup-dependent and require fine-tuning or retraining when surface, lighting, or projector-camera pose changes.

Method: SIComp uses a co-adaptive design with an optical flow module for geometric correction and a photometric network for photometric compensation, along with intensity-varying surface priors.

Result: SIComp consistently produces high-quality compensation across diverse unseen setups, outperforming existing methods in generalization ability.

Conclusion: SIComp is the first generalizable solution to projector compensation, offering a setup-independent approach with a large-scale real-world dataset.
...
TL;DR: The paper proposes a method to improve crowd instance segmentation by generating dense instance segmentation masks from point annotations and training a Reinforced Point Selection framework to predict instance segmentation in dense crowds.


<details>
  <summary>Details</summary>
Motivation: Current crowd datasets have point labels but lack accurate region labels. The authors aim to improve instance segmentation and counting accuracy in dense crowds.

Method: The authors propose Dense Point-to-Mask Optimization (DPMO) to generate masks from point annotations and a Reinforced Point Selection (RPS) framework trained with Group Relative Policy Optimization (GRPO) to predict instance segmentation.

Result: The authors achieve state-of-the-art crowd instance segmentation performance on several datasets and demonstrate the importance of mask annotations in enhancing counting accuracy.

Conclusion: The paper presents a novel approach to improve crowd instance segmentation and counting accuracy by leveraging point annotations and reinforcement learning.
...
TL;DR: A geometry-aware framework for UAV geo-localization in GNSS-denied environments, achieving robust meter-level localization accuracy.


<details>
  <summary>Details</summary>
Motivation: Cross-view geo-localization for UAVs in GNSS-denied environments is challenging due to geometric discrepancies between UAV imagery and satellite maps.

Method: A unified framework that reconstructs a local 3D scene from UAV images, renders a virtual Bird's-Eye View (BEV) representation, and uses a Satellite-wise Attention Block for efficient handling of multiple location hypotheses.

Result: The proposed method significantly outperforms state-of-the-art baselines, achieving robust meter-level localization accuracy and improved generalization in complex urban environments.

Conclusion: The proposed geometry-aware UAV geo-localization framework demonstrates superior performance in challenging environments, enabling accurate and robust localization.
...
TL;DR: The authors propose a new method, Ultrasound-CLIP, for vision-language pre-training on ultrasound images, which achieves state-of-the-art performance on classification and retrieval tasks.


<details>
  <summary>Details</summary>
Motivation: Existing vision-language pre-training models are difficult to apply directly to ultrasound data due to its heterogeneous anatomical structures and diverse diagnostic attributes.

Method: The authors propose a semantic-aware contrastive learning framework, Ultrasound-CLIP, which introduces semantic soft labels and semantic loss to refine sample discrimination. They also construct a heterogeneous graph modality derived from textual representations.
...
Conclusion: The authors successfully bridge the gap between vision-language pre-training models and ultrasound data by constructing a large-scale dataset and proposing a new method, Ultrasound-CLIP, which improves performance on various tasks.
...
TL;DR: This paper introduces a method to use self-supervised features for controlling pretrained video diffusion models, enabling tasks like video domain transfer and video-from-3D generation with robust control over appearance changes.


<details>
  <summary>Details</summary>
Motivation: The motivation is to leverage high-dimensional features from self-supervised learning for general-purpose conditioning of pretrained video diffusion models, overcoming limitations in generative capabilities.

Method: A lightweight architecture and training strategy are introduced to decouple appearance from other features, enabling robust control for appearance changes such as stylization and relighting.

Result: The approach allows for tasks like video domain transfer and video-from-3D generation with improved controllability in generative rendering from explicit spatial representations.

Conclusion: The paper demonstrates the potential of self-supervised features for controlling pretrained video diffusion models, enabling a range of applications with robust control over appearance changes.
...
TL;DR: This paper proposes a new attention mechanism for hyperspectral image classification that uses cosine-normalized attention to improve performance.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this work is to address the limitations of dot-product similarity in attention mechanisms for hyperspectral image classification, which can mix feature magnitude and orientation and may be suboptimal for hyperspectral data.

Method: The proposed method introduces a cosine-normalized attention formulation that projects query and key embeddings onto a unit hypersphere and applies a squared cosine similarity to emphasize angular relationships while reducing sensitivity to magnitude variations.

Result: The proposed approach consistently achieves higher performance on three benchmark datasets, outperforming several recent Transformer- and Mamba-based models despite using a lightweight backbone.

Conclusion: The proposed cosine-normalized attention formulation provides a reliable inductive bias for hyperspectral representation learning, and the experiments demonstrate its effectiveness in improving hyperspectral image classification performance.
...
TL;DR: Large Vision-Language Models (LVLMs) struggle with complex, multi-step reasoning tasks that require integrating visual and textual information with prior knowledge, as demonstrated by their poor performance on rebus puzzles.


<details>
  <summary>Details</summary>
Motivation: The authors identified a cognitive gap in current LVLMs, which excel at explicit visual recognition but struggle with problems that require abstract reasoning and integration of visual, textual, and prior knowledge.

Method: The authors introduce RebusBench, a benchmark of 1,164 rebus puzzles, to evaluate the neurosymbolic capability of LVLMs. They tested state-of-the-art models, including Qwen, InternVL, and LLaVA, and measured their performance using Exact Match and semantic accuracy metrics.

Result: The evaluation showed that state-of-the-art models performed poorly on RebusBench, with Exact Match below 10% and semantic accuracy below 20%. The authors found no significant improvement from model scaling or In-Context Learning (ICL).

Conclusion: The study suggests that while LVLMs possess the necessary visual and linguistic components, they lack the cognitive reasoning capabilities to integrate them effectively, highlighting a significant limitation in their ability to perform complex, multi-step reasoning tasks.
...
TL;DR: A unified driving world-action model, DriveDreamer-Policy, integrates depth generation, future video generation, and motion planning to improve performance in driving tasks.


<details>
  <summary>Details</summary>
Motivation: Existing world-action models have limited geometric grounding, which is essential for embodied systems operating in the physical world.

Method: DriveDreamer-Policy uses a large language model to process language instructions, multi-view images, and actions, followed by three lightweight generators for depth, future video, and actions.

Result: The model achieves strong performance on closed-loop planning and world generation tasks, outperforming existing approaches with 89.2 PDMS on Navsim v1 and 88.7 EPDMS on Navsim v2.
...
Method: A multi-modal knowledge distillation framework (FSKD) that fuses RGBI imagery with LiDAR-derived metrics and vertical profiles via cross-attention.

Result: The proposed method achieves state-of-the-art zero-shot canopy height model performance (MedAE 4.17 m, R^2=0.51, IoU 0.87) and outperforms baselines by 29-46% in MAE.

Conclusion: The FSKD framework enables scalable 20 cm operational monitoring of forest structure metrics, including canopy height model, plant area index, and foliage height diversity, under relaxed co-acquisition constraints.
...
TL;DR: A novel framework called GardenDesigner is proposed to automate the design and construction of Jiangnan gardens, a style of Chinese classical gardens, using procedural modeling and aesthetic principles.


<details>
  <summary>Details</summary>
Motivation: The manual modeling of Jiangnan gardens is time-consuming and heavily relies on expert experience, which creates a gap in efficiently creating digital assets for film, game production, and digital tourism.

Method: GardenDesigner integrates a chain of agents based on procedural modeling, including terrain distribution, road generation, asset selection, and layout optimization agents, and utilizes expert-annotated garden knowledge to enhance asset arrangement.

Result: Experiments and human evaluations demonstrate that GardenDesigner can generate diverse and aesthetically pleasing Jiangnan gardens, allowing non-expert users to construct them via text input within one minute.

Conclusion: The proposed GardenDesigner framework successfully addresses the gap in manual modeling of Jiangnan gardens, providing an efficient and automated solution for creating digital assets.
...
TL;DR: The paper proposes a consistency-aware monocular depth estimation framework that uses wheel odometry from a mobile robot to achieve stable and coherent depth predictions over time.


<details>
  <summary>Details</summary>
Motivation: Existing monocular depth estimation approaches struggle to maintain temporal consistency in depth estimation across consecutive frames, causing jitter and estimation failures.

Method: The proposed framework estimates camera pose and sparse depth from triangulation using optical flow, updates a recursive Bayesian estimate of the metric scale, and rescales the relative depth predicted by a pre-trained depth estimation foundation model.

Result: The proposed method demonstrates robust and accurate depth estimation performance on the KITTI, TartanAir, MS2, and a custom dataset.

Conclusion: The paper presents a consistency-aware monocular depth estimation framework that effectively addresses the challenge of temporal consistency in depth estimation, leading to more stable and accurate predictions.
...
TL;DR: A novel deep learning framework predicts PAM50 breast cancer subtypes from H&E-stained images, achieving high performance and potentially replacing costly molecular assays.


<details>
  <summary>Details</summary>
Motivation: To reduce reliance on costly molecular assays by directly predicting PAM50 subtypes from H&E-stained whole-slide images (WSIs) for more personalized breast cancer treatment.

Method: An optimization-driven deep learning framework combining NSGA-II and Monte Carlo dropout-based uncertainty estimation to jointly optimize patch informativeness, spatial diversity, uncertainty, and patch count.

Result: The proposed method achieved an F1-score of 0.8812 and an AUC of 0.9841 on the internal TCGA-BRCA dataset, and an F1-score of 0.7952 and an AUC of 0.9512 on the external CPTAC-BRCA dataset.

Conclusion: The proposed approach can achieve high performance, improve computational efficiency, and potentially support clinical decision-making as a scalable imaging-based replacement for histopathology-based PAM50 classification.
...
TL;DR: STRIVE, a reinforcement learning framework for video question answering, improves performance by constructing spatiotemporal variants of input videos and using importance-aware sampling.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the limitation of low reward variance in group-based policy optimization methods for multimodal models, which can lead to weak or unstable advantage estimates.

Method: The method proposed is STRIVE (SpatioTemporal Reinforcement with Importance-aware Variant Exploration), which constructs multiple spatiotemporal variants of each input video, performs joint normalization across textual generations and visual variants, and uses an importance-aware sampling mechanism.

Result: The results show consistent improvements over strong reinforcement learning baselines across multiple large multimodal models on six challenging video reasoning benchmarks.

Conclusion: The conclusion of this paper is that structured spatiotemporal exploration is a principled mechanism for stabilizing multimodal reinforcement learning and improving video reasoning performance.
...
TL;DR: The paper proposes SafeRoPE, a lightweight framework for safe text-to-image generation in transformer-based diffusion models, specifically MMDiT.


<details>
  <summary>Details</summary>
Motivation: Existing mitigation methods for unsafe semantics in text-to-image models are computationally expensive and not adaptable to transformer-based diffusion models.

Method: The authors analyze the attention mechanism in MMDiT, identify safety-critical heads, and propose perturbing the Rotary Positional Embedding (RoPE) to modify concepts in generated images. They then develop SafeRoPE, which constructs head-wise unsafe subspaces, computes a Latent Risk Score (LRS), and applies head-wise RoPE perturbations.

Result: SafeRoPE achieves state-of-the-art performance in balancing effective harmful content mitigation and utility preservation for safe generation of MMDiT.

Conclusion: The proposed SafeRoPE framework enables precise suppression of unsafe outputs while maintaining generation fidelity in transformer-based diffusion models.
...
TL;DR: The paper proposes a simple yet powerful solution, random label bridge training, to adapt Large Language Models (LLMs) to vision tasks by aligning their parameters, leveraging partial bridge training for effective cross-modality adaptation.


<details>
  <summary>Details</summary>
Motivation: The significant difference in outlier parameters between language and vision pre-training models makes cross-modality adaptation challenging, prompting the need for a bridge training stage to align LLM parameters with vision tasks.

Method: The paper proposes random label bridge training, which requires no manual labeling, and investigates partial bridge training to adapt LLM parameters to vision foundation tasks.

Result: The findings reveal that adding a bridge training stage can effectively align LLM parameters with vision tasks, and partial bridge training is often advantageous, as certain layers in LLMs exhibit strong foundational properties beneficial for visual tasks.

Conclusion: The study shows that LLMs can be adapted to vision tasks through a bridge training stage, and partial bridge training offers a practical pathway to cross-modality adaptation, opening up new avenues for leveraging language pre-trained parameters in vision models.
...
TL;DR: HieraVid, a hierarchical pruning framework, reduces computational burden in Video Large Language Models (VideoLLMs) by progressively pruning video tokens while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: The massive number of input video tokens in VideoLLMs incurs significant computational burden, and existing methods neglect the inherent information structure in videos and LLMs.

Method: HieraVid proposes a three-level pruning framework: segment-level (temporal segmentation and spatial merging), frame-level (joint pruning of similar frames), and layer-level (reducing redundancy as LLM layer increases).
...
Conclusion: HieraVid effectively reduces computational burden in VideoLLMs while maintaining performance, demonstrating the importance of hierarchical pruning and consideration of video and LLM structures.
...
### [60] [Ranking-Guided Semi-Supervised Domain Adaptation for Severity Classification](https://arxiv.org/abs/2604.01834)
...
<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with severity classification due to unclear class boundaries and naturally ordered class labels, which complicates adaptation.
...
Conclusion: The proposed method is effective in addressing domain shifts in medical image analysis, particularly in severity classification with ordered class labels.
...
### [61] [Semantic Segmentation of Textured Non-manifold 3D Meshes using Transformers](https://arxiv.org/abs/2604.01836)
*Mohammadreza Heidarianbaei,Max Mehltretter,Franz Rottensteiner*

Main category: cs.CV

TL;DR: A new texture-aware transformer model for semantic segmentation of textured 3D meshes is proposed, achieving state-of-the-art results on two benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing methods for semantic segmentation of 3D meshes overlook rich textural information, and the authors aim to address this limitation.

Method: The proposed model uses a texture branch to summarize face-level pixels into a learnable token, which is fused with geometrical descriptors and processed by a stack of Two-Stage Transformer Blocks (TSTB).

Result: The model achieves 81.9% mF1 and 94.3% OA on the Semantic Urban Meshes (SUM) benchmark and 49.7% mF1 and 72.8% OA on a new cultural-heritage dataset.

Conclusion: The proposed texture-aware transformer model outperforms existing approaches for semantic segmentation of textured 3D meshes, demonstrating the importance of considering textural information.
...
### [62] [Investigating Permutation-Invariant Discrete Representation Learning for Spatially Aligned Images](https://arxiv.org/abs/2604.01843)
*Jamie S. J. Stirling,Noura Al-Moubayed,Hubert P. H. Shum*

Main category: cs.CV

TL;DR: The authors propose a permutation-invariant vector-quantized autoencoder (PI-VQ) that learns discrete neural representations of images without positional information, enabling global semantic feature capture and direct interpolation.


<details>
  <summary>Details</summary>
Motivation: The authors question whether positional information is necessary for discrete representations of spatially aligned data and aim to develop a position-free representation method.

Method: The authors propose PI-VQ, which constrains latent codes to carry no positional information, and introduce matching quantization to increase effective bottleneck capacity.

Result: PI-VQ achieves competitive precision, density, and coverage metrics on CelebA, CelebA-HQ, and FFHQ datasets, and enables interpolation-based sampling for novel image synthesis.

Conclusion: The authors demonstrate the potential of position-free representations, highlighting trade-offs in separability and interpretability of latent codes, and suggest future work directions.
...
### [63] [FaCT-GS: Fast and Scalable CT Reconstruction with Gaussian Splatting](https://arxiv.org/abs/2604.01844)
*Pawel Tomasz Pieta,Rasmus Juul Pedersen,Sina Borgi,Jakob Sauer Jørgensen,Jens Wenzel Andreasen,Vedrana Andersen Dahl*

Main category: cs.CV

TL;DR: FaCT-GS is a fast and flexible CT reconstruction framework that improves Gaussian Splatting (GS) technique, achieving significant speedup over state-of-the-art GS CT reconstruction.


<details>
  <summary>Details</summary>
Motivation: The benefits of Gaussian Splatting (GS) are not substantial enough to motivate a transition from well-established reconstruction algorithms, highlighting the need for a more efficient and flexible CT reconstruction method.

Method: FaCT-GS framework, which optimizes voxelization and rasterization pipelines, and enables rapid fitting of Gaussians to pre-existing volumes.

Result: FaCT-GS is over 4X faster than the state-of-the-art GS CT reconstruction on standard 512x512 projections, and over 13X faster on 2k projections.

Conclusion: FaCT-GS addresses the limitations of GS-based approach, providing a fast and flexible CT reconstruction framework with improved performance and scalability.
...
### [64] [Semantic Richness or Geometric Reasoning? The Fragility of VLM's Visual Invariance](https://arxiv.org/abs/2604.01848)
*Jason Qiu,Zachary Meurer,Xavier Thomas,Deepti Ghadiyaram*

Main category: cs.CV

TL;DR: The paper finds that state-of-the-art Vision-Language Models (VLMs) are fragile under basic geometric transformations, lacking robust spatial invariance and equivariance.
...
Conclusion: The study reveals a systematic gap between semantic understanding and spatial reasoning in current VLMs, emphasizing the need for stronger geometric grounding in future multimodal systems.
...
### [65] [Combining Boundary Supervision and Segment-Level Regularization for Fine-Grained Action Segmentation](https://arxiv.org/abs/2604.01859)
*Hinako Mitsuoka,Kazuhiro Hotta*

Main category: cs.CV

TL;DR: A lightweight dual-loss training framework is proposed to improve fine-grained segmentation quality in Temporal Action Segmentation (TAS) with minimal architectural modification.
...
### [66] [MAR-MAER: Metric-Aware and Ambiguity-Adaptive Autoregressive Image Generation](https://arxiv.org/abs/2604.01864)
*Kai Dong,Tingting Bai*

Main category: cs.CV

TL;DR: MAR-MAER, a novel hierarchical autoregressive framework, improves text-to-image generation by addressing image quality and ambiguous prompt issues.
...
Method: MAR-MAER combines metric-aware embedding regularization and a probabilistic latent model to align internal representations with human-preferred quality metrics.

Result: MAR-MAER outperforms the Hi-MAR baseline, achieving +1.6 improvement in CLIPScore and +5.3 in HPSv2, and produces a wider range of outputs for unclear inputs.

Conclusion: MAR-MAER demonstrates excellent performance in metric consistency and semantic flexibility, making it a promising approach for text-to-image generation.
...
TL;DR: The paper proposes agency primitives for GeoAI assistants to improve productivity in GIS workflows by enabling iterative collaboration between humans and AI models.


<details>
  <summary>Details</summary>
Motivation: The gap between advances in GeoAI models and productivity gains for GIS practitioners, who spend most of their time producing vector layers, raster maps, and cartographic products.

Method: Proposing a vocabulary of 9 agency primitives, including navigation, perception, geo-referenced memory, and dual modeling, along with a benchmark to measure human productivity.

Result: A proposed vocabulary and benchmark for agency primitives in GeoAI assistants, aiming to make agentic assistance in GIS implementable, testable, and comparable.

Conclusion: The paper aims to bridge the gap between GeoAI model capabilities and GIS practitioner productivity by introducing an agency layer that supports iterative collaboration.
...
TL;DR: The paper proposes A3R, a framework for affordance reasoning in 3D scenes that iteratively acquires evidence to improve reasoning accuracy, outperforming one-shot baselines.


<details>
  <summary>Details</summary>
Motivation: Existing methods for affordance reasoning in 3D scenes often fail due to incomplete task-relevant evidence from fixed observations. The authors aim to address this by reformulating affordance reasoning as a sequential evidence acquisition process.

Method: The authors propose A3R, an agentic affordance reasoning framework that uses an MLLM-based policy to iteratively select evidence acquisition actions and update affordance beliefs through cross-dimensional evidence acquisition. They also introduce a GRPO-based policy learning strategy to optimize sequential decision making.
...
Conclusion: The paper concludes that the proposed A3R framework effectively addresses the limitations of existing affordance reasoning methods by iteratively acquiring complementary evidence, leading to improved reasoning accuracy in complex 3D scenes.
...
TL;DR: The paper proposes GS², a method to optimize 3D Gaussian Splatting for compact representation, improving rendering quality and reducing memory consumption.


<details>
  <summary>Details</summary>
Motivation: The high memory cost of 3D Gaussian Splatting due to a large number of Gaussian points limits its practicality, and existing pruning-based methods compromise spatial consistency.

Method: The paper proposes a graph-based spatial distribution optimization method, including an ELBO-based adaptive densification strategy, opacity-aware progressive pruning, and a graph-based feature encoding module.

Result: GS² achieves a compact Gaussian representation with superior rendering quality, outperforming baselines in rendering quality and memory efficiency, with higher PSNR using only about 12.5% Gaussian points.

Conclusion: The proposed GS² method effectively optimizes the spatial distribution of Gaussian points, achieving a better balance between rendering quality and memory consumption.
...
TL;DR: The study reveals that text-to-image generative models are vulnerable to jailbreak attacks via natural language prompts, allowing bypass of safety filters.


<details>
  <summary>Details</summary>
Motivation: To mitigate misuse of text-to-image generative models, which are widely deployed in creative tools and online platforms.

Method: A systematic study of prompt-based strategies that bypass safety filters without model access, optimization, or adversarial training.

Result: The attacks achieved an attack success rate (ASR) of up to 74.47% across all tested models and attack categories.

Conclusion: The study highlights a critical gap between surface-level prompt filtering and the semantic understanding required to detect adversarial intent in generative media systems.
...
<details>
  <summary>Details</summary>
Motivation: Previous methods rely on sentence-level vision-language alignment, struggling to exploit fine-grained linguistic cues such as spatial relations and object attributes, crucial for distinguishing objects with similar characteristics.

Method: ProVG employs a progressive cross-modal modulator with a survey-locate-verify scheme, cross-scale fusion module, and language-guided calibration decoder to integrate linguistic cues and refine cross-modal alignment.
...
Conclusion: ProVG improves localization accuracy by effectively leveraging fine-grained linguistic cues and provides a unified framework for multiple tasks.
...
TL;DR: SHARC is a novel framework for synthesizing arbitrary shapes using Spherical Harmonic representations of distance fields, outperforming state-of-the-art methods in accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve shape reconstruction accuracy and efficiency without sacrificing model simplicity.

Method: The method involves optimizing reference points for distance field sampling, computing Spherical Harmonic coefficients, applying a low-pass filter, and refining with a local consistency constraint.

Result: SHARC outperforms existing approaches in reconstruction accuracy and time efficiency while maintaining model parsimony.

Conclusion: The proposed SHARC framework demonstrates superior performance in shape synthesis, offering improved accuracy and efficiency.
...
TL;DR: The paper proposes FTPFusion, a frequency-aware method for fusing infrared and visible videos, addressing the challenge of maintaining temporal stability while preserving spatial details.


<details>
  <summary>Details</summary>
Motivation: Existing methods for infrared and visible video fusion either focus on frame-wise enhancement with limited temporal modeling or rely on heavy spatio-temporal aggregation that often sacrifices high-frequency details.

Method: The proposed FTPFusion method decomposes feature representations into high-frequency and low-frequency components, using sparse cross-modal interaction and temporal perturbation to enhance robustness and capture motion-related context.

Result: FTPFusion consistently outperforms state-of-the-art methods across multiple metrics in both spatial fidelity and temporal consistency, as demonstrated by extensive experiments on multiple public benchmarks.

Conclusion: The proposed FTPFusion method effectively addresses the challenge of maintaining temporal stability while preserving spatial details in infrared and visible video fusion.
...
TL;DR: The paper introduces Cross-Scale MAE, a self-supervised learning model for remote sensing images that uses scale augmentation and cross-scale consistency constraints to improve representation learning.


<details>
  <summary>Details</summary>
Motivation: Remote sensing images pose challenges to image analysis due to their extensive geographic coverage, hardware limitations, and misaligned multi-scale images.

Method: The authors propose Cross-Scale MAE, a self-supervised model built upon the Masked Auto-Encoder (MAE), which employs scale augmentation techniques and cross-scale consistency constraints through contrastive and generative losses.
...
Conclusion: The proposed Cross-Scale MAE model can learn consistent and meaningful representations well-suited for a wide range of downstream tasks in remote sensing image understanding.
...
TL;DR: Proposed Light-ResKAN for efficient SAR image recognition, achieving high accuracy with reduced parameters and FLOPs.


<details>
  <summary>Details</summary>
Motivation: Large SAR image sizes and limited resources on edge devices hinder deep learning deployment, existing lightweight models struggle to balance precision and efficiency.

Method: Modified ResNet with KAN convolutions, Gram Polynomials activations, and parameter-sharing strategy.

Result: Achieved 99.09%, 93.01%, and 97.26% accuracy on MSTAR, FUSAR-Ship, and SAR-ACD datasets, respectively. Reduced FLOPs by 82.90x and parameters by 163.78x compared to VGG16.
...
TL;DR: The paper proposes using web-curated, unlabeled videos to automatically generate training data for 3D scene understanding, achieving strong zero-shot performance and improvement after fine-tuning.
...
Method: Carefully designed data engines to leverage web-curated, unlabeled videos and analyze bottlenecks in automated data generation.

Result: Models trained on generated data show strong zero-shot performance and further improvement after fine-tuning on three tasks: 3D object detection, instance segmentation, 3D spatial Visual Question Answering (VQA), and Vision-Language Navigation (VLN).
...
TL;DR: A new method for detecting and matching corneal reflections in eye tracking is proposed, improving reproducibility and evaluation.


<details>
  <summary>Details</summary>
Motivation: Current methods for corneal reflection detection are often heuristic and difficult to reproduce across different hardware setups.

Method: A 2D geometry-driven, constellation-based pipeline using a Similarity-Layout Alignment (SLA) procedure for multi-glint detection and matching.
...
Conclusion: The proposed framework enables transparent replication, comparison, and dataset annotation, improving the reproducibility of corneal reflection detection.
...
TL;DR: Proposed KnowMVG, a framework that enhances spatial awareness in Vision-Language Models for Medical Visual Grounding, improving localization precision.


<details>
  <summary>Details</summary>
Motivation: Recent Vision-Language Models lack sufficient spatial precision in Medical Visual Grounding due to limited explicit localization priors.

Method: KnowMVG uses knowledge-enhanced prompting and global-local attention to strengthen spatial awareness and guide precise region localization.

Result: KnowMVG outperforms existing approaches on four MVG benchmarks, achieving gains of 3.0% in AP50 and 2.6% in mIoU.

Conclusion: KnowMVG effectively improves Medical Visual Grounding by enhancing spatial awareness, providing more precise visual evidence for clinical decision-making.
...
Method: Uses a dual-chirp shared-weight encoder on pre-beamforming per-antenna RD tensors with end-to-end, fully data-driven training.
...
TL;DR: A new method, S²CPNet, is proposed for cross-domain infrared small target detection, achieving state-of-the-art performance by addressing domain discrepancies through phase rectification, orthogonal attention, and selective style recomposition.


<details>
  <summary>Details</summary>
Motivation: The detection performance of models trained on source domains degrades when deployed in unseen domains due to distribution shifts and the intrinsic indistinctiveness of infrared small targets.

Method: A spatial-spectral collaborative perception network (S²CPNet) is proposed, which includes a phase rectification module (PRM), an orthogonal attention mechanism (OAM), and selective style recomposition (SSR).
...
Conclusion: The S²CPNet method effectively addresses the challenge of cross-domain infrared small target detection by enhancing the generalizability of target awareness and mitigating bias toward domain-specific patterns.
...
TL;DR: A new benchmark and training framework for image captioning in Early Childhood Education (ECE) are proposed, achieving improved performance and fine-grained recognition.


<details>
  <summary>Details</summary>
Motivation: Existing image captioning methods face challenges in ECE due to lack of large-scale, domain-specific datasets and limitations in training paradigms.

Method: A large-scale benchmark (ECAC) and a hybrid training framework (RSRS) are introduced to address these limitations.
...
Conclusion: The proposed approach has potential for specialized educational applications, enabling automated activity understanding and educational assessment in ECE.
...
TL;DR: This paper proposes an extension of the MIMV method, called AMIMV, to address data scarcity and imbalance in medical image classification. It evaluates the robustness of AMIMV and eight other SSL methods under varying degrees of class imbalance and limited supervision.


<details>
  <summary>Details</summary>
Motivation: The lack of large quantities of labeled training data and imbalanced data, where abundant data are available for frequent classes but limited for rare classes, are two significant problems in medical imaging analysis.

Method: The proposed method, AMIMV, extends the MIMV method with a new augmentation strategy to construct asymmetric multi-image, multi-view pairs. The authors also evaluate eight representative SSL methods under long-tailed distributions and limited supervision.

Result: The experimental results show an improvement of 4.25% on retinaMNIST, 1.88% on tissueMNIST, and 3.1% on DermaMNIST.

Conclusion: The proposed AMIMV method and the evaluation of SSL methods demonstrate improved performance in addressing data scarcity and imbalance in medical image classification.
...
TL;DR: Proposed MAVFusion, a video fusion framework that efficiently combines infrared and visible videos by selectively interacting across frames, achieving state-of-the-art performance and speed.


<details>
  <summary>Details</summary>
Motivation: Existing video fusion methods are computationally expensive and struggle with frame-to-frame motion in videos, while static image fusion methods can't handle video dynamics.

Method: MAVFusion uses optical flow to identify dynamic regions and applies sparse cross-modal attention and a lightweight weak interaction module for static regions.

Result: MAVFusion achieves state-of-the-art performance on multiple infrared and visible video benchmarks at a speed of 14.16 FPS at 640x480 resolution.

Conclusion: MAVFusion effectively balances efficiency and fusion quality, making it suitable for real-time video fusion applications.
...
TL;DR: Proposed a deep learning model for automatic prostate gland segmentation using mpMRI data, achieving high accuracy and outperforming general-purpose approaches.


<details>
  <summary>Details</summary>
Motivation: Manual prostate gland segmentation is time-consuming and prone to inter-observer variability, while general-purpose tools lack accuracy for prostate-specific tasks.

Method: Utilized nnU-Net v2 framework with multimodal mpMRI data (T2-weighted, DWI, and ADC maps) for automatic prostate gland segmentation.

Result: Achieved a mean Dice score of 0.96 in cross-validation and 0.82 in external validation, outperforming a general-purpose approach (TotalSegmentator) with a Dice score of 0.15.

Conclusion: The proposed approach demonstrates strong potential for reliable integration into clinical research workflows, highlighting the importance of task-specific, multimodal segmentation strategies.
...
Method: The method involves introducing the MyEgo dataset, comprising 541 long videos and 5K personalized questions, and benchmarking competitive MLLMs across variants.

Result: The results show that top closed- and open-source models achieve only ~46% and 36% accuracy, trailing human performance by near 40% and 50% respectively, with neither explicit reasoning nor model scaling yielding consistent improvements.

Conclusion: The conclusion is that the findings highlight the crucial role of ego-grounding and long-range memory in enabling personalized QA in egocentric videos, and the MyEgo dataset and analyses can catalyze further progress in these areas for egocentric personalized assistance.
...
TL;DR: A new framework, SDesc3D, is proposed for generating 3D indoor scenes from short textual descriptions, improving physical plausibility and detail richness.


<details>
  <summary>Details</summary>
Motivation: Existing text-conditioned 3D scene generation methods suffer from poor physical plausibility and insufficient detail richness due to reliance on explicit semantic cues.

Method: SDesc3D leverages multi-view structural priors and regional functionality implications for 3D layout reasoning, using a multi-view scene prior augmentation, functionality-aware layout grounding, and iterative reflection-rectification.

Result: The proposed method outperforms existing approaches on short-text conditioned 3D indoor scene generation.

Conclusion: The proposed SDesc3D framework effectively generates 3D indoor scenes from short textual descriptions with improved physical plausibility and detail richness.
...
TL;DR: Existing vision encoders struggle with identity-focused tasks due to entanglement with background context. The authors propose a framework using Near-identity distractors to isolate identity and introduce the NearID dataset and evaluation protocol.


<details>
  <summary>Details</summary>
Motivation: Current vision encoders entangle object identity with background context, leading to unreliable representations and metrics in identity-focused tasks.

Method: The authors introduce Near-identity distractors and a two-tier contrastive objective to learn identity-aware representations on a frozen backbone.

Result: The proposed method improves Sample Success Rates (SSR) to 99.2%, enhances part-level discrimination by 28.0%, and yields stronger alignment with human judgments.

Conclusion: The authors propose a principled framework to address the vulnerability of existing vision encoders in identity-focused tasks, and demonstrate significant improvements in representation learning and alignment with human judgments.
...
TL;DR: The paper introduces Interactive Tracking, a new paradigm that allows users to guide visual trackers using natural language commands, and presents a benchmark, evaluation protocol, and baseline method.
...
Result: State-of-the-art tracking methods fail in interactive scenarios, and strong performance on conventional benchmarks does not transfer to interactive scenarios.
...
TL;DR: Introduced Curia-2, an improved foundation model for medical imaging analysis, achieving better performance on various tasks.


<details>
  <summary>Details</summary>
Motivation: To address the growing workload on radiologists by optimizing how foundation models learn from complex radiological volumes.

Method: Proposed Curia-2, which improves the pre-training strategy and representation quality, enabling scaling up to billion-parameter Vision Transformers.

Result: Curia-2 outperforms all foundation models on vision-focused tasks and competes with vision-language models on complex tasks.

Conclusion: Curia-2 shows significant improvement in medical imaging analysis, with publicly available weights to facilitate further research.
...
TL;DR: The paper identifies 'visual inertia' in multimodal large language models, where visual attention remains static and fails to support compositional understanding. It proposes Inertia-aware Visual Excitation (IVE), a training-free method that dynamically adjusts visual attention to improve cognitive inference.
...
Method: The authors propose Inertia-aware Visual Excitation (IVE), which breaks the inertial pattern of visual attention by modeling cognitive inference as dynamic responsiveness. IVE selects dynamic visual tokens and introduces an inertia-aware penalty to discourage over-concentration and persistence of attention.
...
Conclusion: The paper concludes that visual inertia is a key factor in cognitive hallucinations and that IVE is an effective method for mitigating this issue, enabling better compositional understanding and relational inference in multimodal large language models.
...
TL;DR: Resonance4D is a physics-driven 4D dynamic simulation framework that enables realistic simulation of complex materials and dynamics while reducing computational cost and memory overhead.


<details>
  <summary>Details</summary>
Motivation: The existing methods for 4D dynamic simulation are constrained by high computational cost and limited realism in scenes with complex materials and dynamics.

Method: Resonance4D couples 3D Gaussian Splatting with the Material Point Method and introduces Dual-domain Motion Supervision (DMS) to enforce dynamic consistency without dense temporal generation.
...
Conclusion: Resonance4D presents a novel physics-driven 4D dynamic simulation framework that addresses the limitations of existing methods and achieves high-fidelity simulation with reduced computational cost and memory overhead.
...
TL;DR: Proposed MTLSI-Net for efficient multi-task dense prediction with linear attention, achieving state-of-the-art performance on NYUDv2 and PASCAL-Context datasets.


<details>
  <summary>Details</summary>
Motivation: Capturing global cross-task interactions in multi-task dense prediction remains challenging due to the quadratic complexity of standard self-attention.
...
Result: State-of-the-art performance on NYUDv2 and PASCAL-Context datasets, demonstrating effectiveness and efficiency in multi-task learning.

Conclusion: MTLSI-Net enables comprehensive cross-task interactions at linear complexity with reduced parameters, validating its potential for efficient multi-task dense prediction.
...
TL;DR: ProDiG, a diffusion-guided framework, generates coherent 3D site models and ground-level views from aerial imagery, overcoming limitations of existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods for generating ground-level views and 3D site models from aerial imagery struggle with extreme viewpoint changes, missing observations, and large scale variations.

Method: ProDiG uses a progressive altitude Gaussian splatting approach, with a geometry-aware causal attention module and a distance-adaptive Gaussian module.

Result: ProDiG produces visually realistic ground-level renderings and coherent 3D geometry, outperforming existing approaches in visual quality, geometric consistency, and robustness.

Conclusion: ProDiG enables progressive, geometrically grounded refinement of aerial 3D representations toward ground-level fidelity without requiring additional ground-truth viewpoints.
...
TL;DR: A novel framework called DR-Seg is proposed for open-vocabulary semantic segmentation in remote sensing, which decouples and rectifies CLIP features to improve structural details and boundary delineation.


<details>
  <summary>Details</summary>
Motivation: The limitation of CLIP's global-aligned visual representations in capturing structural details and the need for targeted structural enhancement without disrupting language-aligned semantics.

Method: DR-Seg, a decouple-and-rectify framework that decouples CLIP features into semantics-dominated and structure-dominated subspaces, and uses a prior-driven graph rectification module and an uncertainty-guided adaptive fusion module.

Result: DR-Seg establishes a new state-of-the-art across eight benchmarks.

Conclusion: DR-Seg effectively addresses the limitation of CLIP in open-vocabulary semantic segmentation, providing improved structural details and boundary delineation.
...
TL;DR: Introduced LinkS$^2$Bench, a benchmark for evaluating Vision-Language Models' (VLMs) dynamic cross-view spatial intelligence, and proposed a Cross-View Alignment Adapter to improve model performance.


<details>
  <summary>Details</summary>
Motivation: The capacity of VLMs to master the complex interplay between UAVs and satellites remains largely unexplored due to the lack of comprehensive benchmarks that evaluate dynamic local-to-global spatial mapping.

Method: Introduced LinkS$^2$Bench, a benchmark linking 1,022 minutes of dynamic UAV footage with high-resolution satellite imagery, and used an LMM-assisted pipeline and human annotation to construct 17.9k high-quality question-answer pairs.

Result: Evaluations of 18 representative VLMs revealed a substantial gap compared to human baselines, and the proposed Cross-View Alignment Adapter significantly improved model performance.

Conclusion: The study highlights the need for improved VLMs that can accurately align cross-view dynamic information, and demonstrates the potential of LinkS$^2$Bench in advancing VLM adaptation for complex spatial reasoning.
...
<details>
  <summary>Details</summary>
Motivation: Autoencoders are biased towards dominant patterns, resulting in loss of fine-grained detail and blurred reconstructions for rare spatial inputs, particularly under spatial data imbalance.
...
Result: The proposed method outperforms baselines on various reconstruction metrics, particularly under spatial imbalance distributions, in a simulated dataset and three real-world datasets spanning physical, biological, and astronomical domains.

Conclusion: The paper highlights the importance of data representation in a batch and emphasizes rare samples in unsupervised image reconstruction, and the proposed method improves reconstruction consistency compared to existing baselines.
...
TL;DR: Introduced IndoorCrowd, a dataset for human detection, instance segmentation, and multi-object tracking in indoor environments, with baselines for detection, segmentation, and tracking.


<details>
  <summary>Details</summary>
Motivation: Existing datasets lack real-world indoor complexity at scale for understanding human behavior in crowded environments.

Method: Collected a multi-scene dataset (IndoorCrowd) with 31 videos and human-verified segmentation masks, and benchmarked foundation-model auto-annotators and tracking algorithms.

Result: Established detection, segmentation, and tracking baselines, and found substantial difficulty variation across scenes due to crowd density, scale, and occlusion.

Conclusion: IndoorCrowd dataset and benchmarks provide a foundation for research in human behavior understanding in crowded indoor environments.
...
TL;DR: WISE, a novel paradigm for efficient chain-of-thought reasoning, achieves state-of-the-art zero-shot performance on ReasonSeg while reducing reasoning length by 5x.


<details>
  <summary>Details</summary>
Motivation: The high computational cost of generating verbose rationales in chain-of-thought reasoning limits its real-world applicability.
...
Result: WISE-S achieves state-of-the-art zero-shot performance on ReasonSeg with 58.3 cIoU, reducing average reasoning length from 112 to 23 tokens.

Conclusion: WISE enables efficient and effective chain-of-thought reasoning, making it more suitable for real-world applications.
...
TL;DR: The paper introduces Jagle, a large-scale Japanese multimodal dataset for training vision-language models, achieving strong performance on Japanese tasks and improving English performance when combined with another dataset.


<details>
  <summary>Details</summary>
Motivation: The lack of large-scale, diverse training datasets for vision-language models in languages other than English, hindering the development of high-quality multilingual and non-English models.

Method: The authors collect heterogeneous source data and generate VQA pairs through multiple strategies such as VLM-based QA generation, translation, and text rendering to create the Jagle dataset.

Result: A 2.2B model trained with Jagle achieves strong performance on Japanese tasks, surpassing InternVL3.5-2B and approaching Qwen3-VL-2B-Instruct, and improves English performance when combined with FineVision.

Conclusion: The Jagle dataset and trained models facilitate the development of high-quality multilingual and non-English vision-language models, with potential applications in various tasks and languages.
...
TL;DR: The paper proposes a fully automatic method to evaluate skin tone accuracy in virtual human rendering, finding that darker skin tones have higher colorimetric errors.


<details>
  <summary>Details</summary>
Motivation: The reproduction of facial skin tone is crucial for realism and fairness in virtual human rendering, but current methods rely on uncalibrated photographic inputs, leading to inconsistencies and bias.

Method: The proposed methodology integrates skin color extraction, texture recolorization, real-time rendering, and quantitative color analysis, using a fully automatic and scalable approach to evaluate skin tone fidelity.

Result: The results show phenotype-dependent behavior of extraction strategies and consistently higher colorimetric errors for darker skin tones, based on the analysis of approximately 19,848 rendered instances.

Conclusion: The proposed methodology enables low computational cost and large-scale evaluation of skin tone accuracy in virtual human rendering, highlighting the need for improved methods for darker skin tones.
...
TL;DR: The paper proposes COMPASS, a framework for handling missing modalities in multimodal sensing by synthesizing proxy tokens for missing modalities and aggregating them into a fixed-size input for the fusion head.
...
Method: COMPASS, a missing-modality fusion framework that synthesizes target-specific proxy tokens for missing modalities using pairwise source-to-target generators and aggregates them into a single replacement token.
...
TL;DR: This paper addresses the challenge of 3D affordance prediction in cluttered multi-object scenes with implicit natural language intent. It proposes a new benchmark and a framework called CompassNet, which achieves state-of-the-art results and effective transfer to real-world robotic grasping.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enable robots to choose the correct object to perform a task in a cluttered scene, where multiple objects may share identical affordances but only one is appropriate under the given task context.

Method: The method involves proposing a new 3D affordance setting called Multi-Object Affordance Grounding under Intent-Driven Instructions, and a framework called CompassNet, which incorporates two dedicated modules: Instance-bounded Cross Injection (ICI) and Bi-level Contrastive Refinement (BCR).

Result: The result shows state-of-the-art performance on both seen and unseen queries, and effective transfer to real-world grasping in confusing multi-object scenes.

Conclusion: The conclusion is that the proposed framework CompassNet can effectively predict 3D affordances in cluttered multi-object scenes with implicit natural language intent, and can be applied to real-world robotic grasping tasks.
...
TL;DR: Network analysis of UK payment records improves economic forecasting, especially during disruptions like the COVID-19 pandemic.


<details>
  <summary>Details</summary>
Motivation: Traditional bilateral measurement approaches do not reveal structural economic relationships, which can be uncovered through network analysis of inter-industry payment flows.
...
Result: Network-enhanced models improve forecasting by 8.8 percentage points, with network contributions reaching +13.8 percentage points during the COVID-19 pandemic.
...
TL;DR: The paper proposes a novel framework, InCoM-Net, for Human-Object Interaction (HOI) detection, which integrates semantic knowledge from Vision-Language Models with instance-specific features to improve performance.


<details>
  <summary>Details</summary>
Motivation: Existing HOI detection methods fail to fully utilize diverse contextual cues across the entire scene, leading to limited performance.

Method: The proposed InCoM-Net framework consists of two core components: Instance-centric Context Refinement (ICR) and Progressive Context Aggregation (ProCA), which effectively integrate rich semantic knowledge with instance-specific features.

Result: InCoM-Net achieves state-of-the-art performance on HICO-DET and V-COCO benchmarks, surpassing previous HOI detection methods.

Conclusion: The proposed InCoM-Net framework effectively leverages contextual cues to improve HOI detection performance, demonstrating the importance of integrating semantic knowledge with instance-specific features.
...
TL;DR: The paper proposes PLUME, a latent reasoning framework that improves Universal Multimodal Embedding (UME) by replacing explicit chain-of-thought rationales with a short autoregressive rollout of continuous latent states, achieving faster inference and comparable performance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of explicit chain-of-thought (CoT) rationales in UME, which incur substantial inference overhead and compress rich multimodal evidence into a narrow textual bottleneck.

Method: The method proposed is PLUME, a latent reasoning framework that uses a short autoregressive rollout of continuous latent states and a semantic-anchor-guided transition adapter to steer latent rollout along different reasoning trajectories.

Result: PLUME outperforms strong explicit-CoT UME baselines on the 78-task MMEB-v2 benchmark, reducing reasoning from hundreds of generated tokens to fewer than 10 latent steps, and delivering over 30x faster inference.

Conclusion: The paper concludes that structured latent computation can preserve the benefits of intermediate reasoning without the overhead of explicit rationale generation, providing a stronger and more efficient paradigm for practical retrieval systems.
...
<details>
  <summary>Details</summary>
Motivation: Existing learning-based slider methods for continuous image editing have additional training overhead and may not be reliable under distribution shifts in edits or domains.
...
Conclusion: The proposed FlowSlider method offers a training-free approach to continuous image editing with improved reliability and edit quality.
...
TL;DR: The authors present a winning solution for the RIVA Cervical Cytology Challenge, achieving top ranks in Track A and B, by leveraging a Co-DINO framework with Swin-Large backbone and introducing targeted optimizations.
...
Method: The approach integrates Co-DINO framework with Swin-Large backbone, formulates detection task as center-point prediction, and applies center-preserving data augmentation and analytical geometric box optimization.

Result: The solution achieves 1st place in Track B and 2nd place in Track A of the RIVA Cervical Cytology Challenge, demonstrating improved detection performance.

Conclusion: The proposed approach provides an effective pipeline for cytology image analysis, with available code at https://github.com/YanKong0408/Center-DETR.
...
TL;DR: Proposed GroundVTS, a Vid-LLM architecture that improves video temporal grounding by focusing on informative temporal segments and preserving spatio-temporal information.


<details>
  <summary>Details</summary>
Motivation: Existing Vid-LLMs rely on uniform frame sampling, leading to sparse key frames and loss of temporal cues, limiting their applications.

Method: Grounded Visual Token Sampling (GroundVTS) with a fine-grained, query-guided mechanism to filter visual tokens and a progressive optimization strategy.

Result: GroundVTS outperforms existing methods on three VTG benchmarks, achieving 7.7-point improvement in mIoU and 12.0-point improvement in mAP.
...
TL;DR: Introduced LatentUM, a unified model that represents all modalities in a shared semantic latent space, enabling efficient interleaved cross-modal reasoning and generation.


<details>
  <summary>Details</summary>
Motivation: Existing unified models require pixel decoding as a bridge between visual understanding and generation, which is ineffective and inefficient.

Method: Proposed LatentUM, a novel unified model that represents all modalities within a shared semantic latent space.

Result: LatentUM achieved state-of-the-art performance on the Visual Spatial Planning benchmark, improved visual generation through self-reflection, and supported world modeling.

Conclusion: LatentUM eliminates the need for pixel-space mediation, enabling flexible interleaved cross-modal reasoning and generation with improved efficiency and accuracy.
...
TL;DR: The paper proposes a training-free framework called CoRegOVCD for open-vocabulary change detection in remote sensing, which improves the accuracy of detecting changes in land-cover semantics across time.


<details>
  <summary>Details</summary>
Motivation: Existing methods for remote sensing change detection assume a fixed label space and cannot answer arbitrary user-defined queries. The authors aim to address this limitation with open-vocabulary change detection.

Method: The proposed framework, CoRegOVCD, uses a calibrated posterior discrepancy to detect changes and includes several components: Competitive Posterior Calibration (CPC), Semantic Posterior Delta (SPD), Geometry-Token Consistency Gate (GeoGate), and Regional Consensus Discrepancy (RCD).

Result: CoRegOVCD consistently improves over the strongest previous training-free baseline by 2.24 to 4.98 F1$_C$ points and reaches a six-class average of 47.50% F1$_C$ on SECOND across four benchmarks.

Conclusion: The proposed framework, CoRegOVCD, effectively addresses the challenges of open-vocabulary change detection in remote sensing and demonstrates improved performance over existing methods.
...
TL;DR: The paper analyzes the variability of cross-validation in facial Action Unit (AU) detection and proposes Leave-one-dataset-out cross-validation for more stable results.


<details>
  <summary>Details</summary>
Motivation: The authors are motivated by the observation that reported improvements in AU detection are often small and may be due to stochastic variance in cross-validation.

Method: The authors use subject-exclusive cross-validation and Leave-One-Dataset-Out (LODO) protocol to evaluate the variability of AU detection results.
...
Conclusion: The authors conclude that gains reported in cross-fold validation may fall within protocol variance, and that Leave-one-dataset-out cross-validation is a more reliable evaluation protocol for AU detection.
...
Method: The method involves injecting prior information of reflection placement and appearance into a foundation diffusion model, adopting a type-aware model design by categorizing reflections into two types, and utilizing a large-scale object reflection dataset DEROBA for training.

Result: The experiments conducted demonstrate that the proposed method generates reflections that are physically coherent and visually realistic, setting a new benchmark for reflection generation.

Conclusion: The conclusion of this paper is that the proposed method successfully generates realistic reflections, advancing the field of image composition and establishing a new standard for reflection generation.
...
TL;DR: The paper introduces ViT-Explainer, a web-based system for visualizing and understanding Vision Transformer (ViT) inference, providing an integrated view from patch tokenization to classification.


<details>
  <summary>Details</summary>
Motivation: There is a need for guided, end-to-end understanding of the full inference pipeline in Vision Transformers, as existing interpretability tools have limitations.

Method: The authors developed ViT-Explainer, a web-based interactive system with features such as animated walkthroughs, patch-level attention overlays, and a vision-adapted Logit Lens.

Result: A user study with six participants found that ViT-Explainer is easy to learn and use, helping users interpret and understand Vision Transformer behavior.

Conclusion: ViT-Explainer effectively bridges the gap in understanding Vision Transformer inference, providing a comprehensive and interactive visualization tool for both guided and free exploration.
...
TL;DR: A unified framework for multi-label classification of known chest X-ray lesions and zero-shot classification of unseen ones, leveraging projection-specific models, contrastive learning, and LLM-generated prompts.


<details>
  <summary>Details</summary>
Motivation: To address the challenges of multi-label classification for known chest X-ray lesions and zero-shot classification for unseen ones, handling diverse CXR projections.

Method: A unified framework integrating projection-specific models with a classification network, and a novel dual-branch architecture for zero-shot classification, incorporating contrastive learning, Asymmetric Loss (ASL), and LLM-generated descriptive prompts.

Result: The approach ensures robustness across both tasks, effectively mitigating long-tail imbalances and maximizing zero-shot generalization.

Conclusion: The proposed framework offers a comprehensive solution for chest X-ray lesion classification, handling both known and unseen lesions.
...
TL;DR: The paper presents a lightweight highway lane detection architecture that captures spatial and temporal information for robust performance in real-world driving scenarios.


<details>
  <summary>Details</summary>
Motivation: To develop a robust lane detection system for real-world driving scenarios that can accurately detect lanes while reducing computational complexity.

Method: The paper proposes two models that integrate a 3D-ResNet encoder with a Point Instance Network (PINet) decoder, with one model using a Feature Pyramid Network (FPN) and Self-Attention mechanism, and the other introducing a Region of Interest (ROI) detection head.

Result: The proposed second model achieves 93.40% accuracy on the TuSimple dataset, outperforming existing 2D and 3D baselines with fewer parameters and reduced latency.

Conclusion: The proposed models are well-suited for integration into Advanced Driver Assistance Systems (ADAS) and have potential scalability toward full Lane Assist Systems (LAS).
...
TL;DR: A unified Vision-Language-Action model for autonomous driving is proposed, addressing the trade-off between spatial perception and semantic reasoning.


<details>
  <summary>Details</summary>
Motivation: Existing Vision-Language-Action models for autonomous driving face a dilemma between spatial perception and semantic reasoning, leading to suboptimal performance.

Method: A Mixture-of-Transformers based model, UniDriveVLA, with expert decoupling and masked joint attention is proposed, combining sparse perception and progressive training.

Result: UniDriveVLA achieves state-of-the-art performance on nuScenes and Bench2Drive, demonstrating strong performance across various tasks, including 3D detection and motion forecasting.

Conclusion: The proposed UniDriveVLA model effectively addresses the perception-reasoning conflict, providing a unified solution for autonomous driving with improved performance and broad applicability.
...
TL;DR: Proposed SCALE, a framework for zero-shot skeleton-based action recognition, outperforming previous methods on NTU-60 and NTU-120 datasets.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for zero-shot skeleton-based action recognition rely on explicit skeleton-text alignment, which can be brittle. The authors aim to improve this with a more robust method.

Method: SCALE, a Semantic- and Confidence-Aware Listwise Energy-based framework, uses a text-conditioned Conditional Variational Autoencoder and a novel loss function to improve zero-shot action recognition.

Result: SCALE consistently improves over prior VAE- and alignment-based baselines and remains competitive with diffusion-based methods on NTU-60 and NTU-120 datasets.

Conclusion: The proposed SCALE framework effectively addresses the challenges of zero-shot skeleton-based action recognition, providing a more robust and accurate approach.
...
### [118] [Omni123: Exploring 3D Native Foundation Models with Limited 3D Data by Unifying Text to 2D and 3D Generation](https://arxiv.org/abs/2604.02289)
...
Method: Omni123 uses a shared sequence space to represent text, images, and 3D as discrete tokens, and an interleaved X-to-X training paradigm to coordinate diverse cross-modal tasks.
...
Conclusion: Omni123 presents a unified approach to text-to-2D and text-to-3D generation, leveraging cross-modal consistency and abundant 2D data to improve 3D representations.
...
TL;DR: The paper proposes a new Vision-Language-Action (VLA) model, UAV-Track VLA, for embodied visual tracking in dynamic urban scenarios with complex semantic requirements, achieving superior performance and real-time efficiency.


<details>
  <summary>Details</summary>
Motivation: Embodied visual tracking is crucial for Unmanned Aerial Vehicles (UAVs) executing complex real-world tasks, and existing VLA models have limitations in addressing temporal feature redundancy and spatial geometric priors.

Method: The proposed UAV-Track VLA model introduces a temporal compression net and a parallel dual-branch decoder with a spatial-aware auxiliary grounding head and a flow matching action expert, built upon the $π_{0.5}$ architecture.

Result: The proposed method achieves a 61.76% success rate and 269.65 average tracking frames in challenging long-distance pedestrian tracking tasks, outperforming existing baselines, and demonstrates robust zero-shot generalization and reduced single-step inference latency.

Conclusion: The proposed UAV-Track VLA model demonstrates superior end-to-end performance, real-time efficiency, and robust generalization in dynamic urban scenarios, making it suitable for complex real-world tasks.
...
### [120] [VOID: Video Object and Interaction Deletion](https://arxiv.org/abs/2604.02296)
*Saman Motamed,William Harvey,Benjamin Klein,Luc Van Gool,Zhuoning Yuan,Ta-Ying Cheng*

Main category: cs.CV

TL;DR: The paper introduces VOID, a video object removal framework that generates physically-plausible inpainting in complex scenarios where the removed object interacts with others.


<details>
  <summary>Details</summary>
Motivation: Current video object removal methods fail to correct complex interactions, such as collisions, between the removed object and other objects, producing implausible results.

Method: The framework uses a vision-language model to identify affected regions and guides a video diffusion model to generate physically consistent outcomes.

Result: Experiments show that VOID better preserves consistent scene dynamics after object removal compared to prior methods.

Conclusion: The VOID framework offers a solution for physically-plausible video object removal, suggesting a path forward for making video editing models better simulators of the world.
...
### [121] [SPAR: Single-Pass Any-Resolution ViT for Open-vocabulary Segmentation](https://arxiv.org/abs/2604.02252)
*Naomi Kombol,Ivan Martinović,Siniša Šegvić,Giorgos Tolias*

Main category: cs.CV

TL;DR: A new method called SPAR is introduced for efficient high-resolution inference in Vision Transformers, improving performance in dense prediction tasks like open-vocabulary segmentation.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the limitations of Vision Transformers (ViTs) in tasks requiring fine-grained spatial understanding, particularly in dense prediction scenarios like open-vocabulary segmentation.

Method: The authors propose SPAR: Single-Pass Any-Resolution ViT, a resolution-agnostic dense feature extractor that distills spatial reasoning capabilities from a finely-strided, sliding-window teacher into a single-pass student using a feature regression loss.

Result: SPAR improves single-pass baselines by up to 10.5 mIoU and even surpasses the teacher, demonstrating effectiveness in efficient, high-resolution reasoning for open-vocabulary segmentation.

Conclusion: The proposed method, SPAR, enables efficient high-resolution inference in ViTs, overcoming the challenges of fixed pre-training resolution and coarse patch-level representations, and achieving state-of-the-art performance in dense prediction tasks.
...
### [122] [Modular Energy Steering for Safe Text-to-Image Generation with Foundation Models](https://arxiv.org/abs/2604.02265)
*Yaoteng Tan,Zikui Cai,M. Salman Asif*

Main category: cs.CV

TL;DR: Proposed an inference-time steering framework for safe text-to-image generation, leveraging gradient feedback from frozen pretrained foundation models.


<details>
  <summary>Details</summary>
Motivation: Existing safety approaches for text-to-image generative models have limitations, such as degrading generation quality or limiting scalability.

Method: Inference-time steering framework using gradient feedback from frozen pretrained foundation models to guide generation without modifying the generator.

Result: State-of-the-art robustness against NSFW benchmarks, effective multi-target steering, and high generation quality on benign prompts.

Conclusion: The proposed framework provides a principled approach for utilizing foundation models for reliable and scalable safety control in text-to-image generation.
...
<details>
  <summary>Details</summary>
Motivation: Existing visual representations from pretrained Vision Transformers (ViTs) focus on salient visual cues, but lack guidance towards less prominent concepts. Multimodal LLMs can be guided with text, but their representations are language-centric and ineffective for generic visual tasks.

Method: The authors propose injecting text directly into the layers of the visual encoder via lightweight cross-attention (early fusion), unlike existing models that fuse text with visual features after encoding (late fusion).

Result: The proposed Steerable Visual Representations can focus on desired objects in an image while preserving representation quality, and match or outperform dedicated approaches on anomaly detection and personalized object discrimination with zero-shot generalization.

Conclusion: The paper presents a new approach for generating visual representations that are steerable by natural language, offering a more flexible and effective solution for various visual tasks.
...
### [124] [ActionParty: Multi-Subject Action Binding in Generative Video Games](https://arxiv.org/abs/2604.02330)
*Alexander Pondaven,Ziyi Wu,Igor Gilitschenski,Philip Torr,Sergey Tulyakov,Fabio Pizzati,Aliaksandr Siarohin*

Main category: cs.CV

TL;DR: The paper introduces ActionParty, a world model that can simulate interactive environments with multiple agents, controlling up to seven players simultaneously.


<details>
  <summary>Details</summary>
Motivation: Existing video diffusion models struggle to control multiple agents simultaneously in a scene due to the issue of action binding.

Method: The authors propose ActionParty, which uses subject state tokens and a spatial biasing mechanism to disentangle global video frame rendering from individual action-controlled subject updates.

Result: ActionParty demonstrates significant improvements in action-following accuracy and identity consistency, and enables robust autoregressive tracking of subjects through complex interactions.

Conclusion: The proposed ActionParty model is capable of controlling multiple agents simultaneously in a scene, achieving state-of-the-art performance on the Melting Pot benchmark.
...
TL;DR: A fast surface registration method for medical imaging is proposed, using probability measures and distributional optimisation to achieve efficiency and robustness.


<details>
  <summary>Details</summary>
Motivation: Existing surface registration methods face a trade-off between efficiency and robustness, with local point matching methods being efficient but vulnerable to noise, and global point set alignment methods being robust but computationally expensive.

Method: The proposed method formulates surface meshes as probability measures and surface registration as a distributional optimisation problem, using an efficient sliced Wasserstein distance and a novel optimisation method called AdamFlow.

Result: The proposed method demonstrates superior performance in both affine and non-rigid surface registration across various anatomical structures, with theoretical analysis showing asymptotic convergence of AdamFlow.

Conclusion: The proposed fast surface registration method achieves a balance between efficiency and robustness, making it suitable for anatomical shape analysis in medical imaging.
...
TL;DR: A simple sliding-window baseline called SimpleStream, which feeds only the most recent N frames to a VLM, matches or surpasses published streaming models on video understanding tasks.


<details>
  <summary>Details</summary>
Motivation: To challenge the trend of relying on complex memory mechanisms for handling long video streams and to evaluate the effectiveness of a simple baseline.

Method: The authors propose SimpleStream, a sliding-window baseline that uses only the most recent N frames as input to an off-the-shelf VLM.
...
Conclusion: The authors argue that future streaming benchmarks should separate recent-scene perception from long-range memory, and that stronger memory or retrieval modules should be evaluated against SimpleStream to ensure clear progress.
...
TL;DR: A high-fidelity, full-body 3D avatar model that generalizes to diverse populations, enabling efficient inference.


<details>
  <summary>Details</summary>
Motivation: The trade-off between fidelity and generalization in 3D avatar modeling, where high-quality studio data struggles to generalize to real-world data, and large-scale models sacrifice quality for generalization.

Method: A pre/post-training paradigm for 3D avatar modeling, pretraining on 1M in-the-wild videos and post-training on high-quality curated data.

Result: Large-Scale Codec Avatars (LCA) that generalize across demographics, hair styles, clothing, and provide precise facial expressions and finger-level articulation control.

Conclusion: The proposed LCA model achieves high-fidelity and generalization in 3D avatar modeling, with emergent properties such as relightability and robustness to stylized imagery.
...
TL;DR: A new benchmark called Referring Scenario Comprehension (RSC) is introduced to evaluate visual grounding in a more challenging scenario-based setting, where the target object must be inferred from context and relationships rather than explicit naming.


<details>
  <summary>Details</summary>
Motivation: Existing visual grounding benchmarks focus on literal referring expressions, but the authors want to explore a more challenging setting where the target object must be inferred from roles, intentions, and relational context.

Method: The authors propose a new benchmark RSC and a curriculum reasoning method called ScenGround, which combines supervised warm-starting with difficulty-aware reinforcement learning.
...
Conclusion: The authors conclude that RSC and ScenGround provide a new and more challenging setting for visual grounding, and that curriculum training can improve model performance in this setting.
...
TL;DR: ModMap is a multiview and multimodal framework for 3D anomaly detection and segmentation that learns to map features across modalities and views, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Existing methods process views independently, and there is a need for a framework that can effectively integrate information from multiple views and modalities for 3D anomaly detection and segmentation.

Method: ModMap uses a crossmodal feature mapping paradigm to learn to map features across modalities and views, with feature-wise modulation to model view-dependent relationships, and a cross-view training strategy for effective anomaly scoring.

Result: ModMap achieves state-of-the-art performance on the SiM3D benchmark, surpassing previous methods by wide margins.

Conclusion: ModMap is a effective framework for 3D anomaly detection and segmentation, and its performance demonstrates the importance of integrating information from multiple views and modalities.
...
TL;DR: The authors introduce a large-scale dataset of synthetic game frames to improve the realism and temporal coherence of generative rendering models, enabling robust inverse rendering and high-fidelity video generation.
...
Method: The authors create a large-scale dataset using a novel dual-screen stitched capture method from AAA games and propose a VLM-based assessment protocol for evaluating inverse rendering performance.

Result: Experiments show that inverse renderers fine-tuned on the proposed dataset achieve superior cross-dataset generalization and controllable generation, with the VLM evaluation strongly correlating with human judgment.

Conclusion: The proposed dataset and evaluation protocol enable robust inverse rendering and high-fidelity video generation, with applications in editing AAA game styles using text prompts.
...
TL;DR: Proposed EventHub, a framework for training deep-event stereo networks without ground truth annotations, using standard color images and novel view synthesis techniques.


<details>
  <summary>Details</summary>
Motivation: Training deep-event stereo networks requires costly active sensors for ground truth annotations, which is a significant limitation.

Method: EventHub uses standard color images to derive proxy annotations and proxy events through novel view synthesis techniques, and repurposes state-of-the-art stereo models from RGB literature to process event data.

Result: Experiments on event stereo datasets show the effectiveness of EventHub and its ability to improve the accuracy of RGB stereo foundation models in challenging conditions.

Conclusion: EventHub enables the training of deep-event stereo networks without ground truth annotations, achieving unprecedented generalization capabilities and improving accuracy in challenging conditions.
...
TL;DR: This paper explores the importance of Repetitive Lengthening Form (RLF) in sentiment analysis and the ability of Language Models (LMs) to understand RLF, proposing a new dataset and a two-stage instruction tuning framework.


<details>
  <summary>Details</summary>
Motivation: The Repetitive Lengthening Form (RLF) has been overlooked in previous linguistic research, despite its unique and emphatic style in online communication. The authors aim to investigate the importance of RLF in sentiment analysis and the ability of LMs to understand it.

Method: The authors curate a multi-domain dataset called Lengthening with 850k samples focused on RLF for sentiment analysis. They also propose a two-stage instruction tuning framework called ExpInstruct to improve the performance and explainability of LMs for RLF.

Result: The results show that fine-tuned Pre-trained Language Models (PLMs) can surpass zero-shot GPT-4 in performance but not in explanation for RLF. The authors also demonstrate that ExpInstruct can improve open-sourced LLMs to match zero-shot GPT-4 in performance and explainability for RLF with limited samples.

Conclusion: The authors conclude that RLF sentences are expressive expressions and can serve as signatures of document-level sentiment, and that RLF has potential value for online content analysis. They also highlight the effectiveness of ExpInstruct in improving the performance and explainability of LMs for RLF.
...
TL;DR: The paper presents two approaches to scaling reasoning token budgets for competitive programming: training-time reinforcement learning (RL) and test-time parallel thinking, achieving better performance on competitive programming problems.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the efficiency and effectiveness of reasoning token budgets for competitive programming, which is a challenging task due to the complexity of the problems.

Method: The method involves two complementary approaches: training-time reinforcement learning (RL) with techniques like verification RL warmup and randomized clipping, and test-time parallel thinking with a multi-round pipeline that distributes the token budget across threads and rounds of generation, verification, and refinement.

Result: The full system with 16 threads and 16 rounds per thread matches the underlying RL model's oracle pass@16 at pass@1 using 7.6 million tokens per problem on average, and surpasses GPT-5-high on 456 hard competitive programming problems from AetherCode.

Conclusion: The paper concludes that the proposed approaches can effectively scale reasoning token budgets for competitive programming, achieving better performance and efficiency.
...
TL;DR: A new dataset called M2-Verify is introduced to evaluate the consistency between scientific claims and their multimodal evidence, which is crucial for assessing scientific arguments.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for evaluating scientific claim consistency lack scale, domain diversity, and visual complexity, making it difficult to realistically assess alignment.

Method: The M2-Verify dataset is created by sourcing instances from PubMed and arXiv, and is validated through expert audits across 16 domains, providing over 469K instances.

Result: State-of-the-art models struggle to maintain robust consistency, achieving up to 85.8% Micro-F1 on low-complexity challenges but dropping to 61.6% on high-complexity challenges, and are prone to hallucinations when generating scientific explanations.

Conclusion: The M2-Verify dataset provides a large-scale resource for evaluating scientific claim consistency, highlighting the need for improved models that can handle complex multimodal evidence and provide robust consistency.
...
TL;DR: The study proposes a feature-augmented framework to improve language models' ability to learn human preferences, achieving up to 0.84 ROC AUC and providing fine-grained interpretability.


<details>
  <summary>Details</summary>
Motivation: The difficulty of learning human preferences in language models due to subtle and subjective comparisons, and the need for a more nuanced approach.
...
Conclusion: The study highlights the importance of capturing the multidimensional nature of human judgment and provides a more interpretable approach to learning human preferences in language models.
...
TL;DR: The paper introduces Reasoning Memory, a framework that improves language models on reasoning tasks by retrieving and reusing procedural knowledge from prior trajectories.
...
Method: The authors propose a retrieval-augmented generation (RAG) framework called Reasoning Memory, which decomposes existing corpora of step-by-step reasoning trajectories into self-contained subquestion-subroutine pairs and retrieves relevant subroutines at inference time.

Result: Reasoning Memory consistently outperforms baselines across six math, science, and coding benchmarks, improving over no retrieval by up to 19.2% and over the strongest compute-matched baseline by 7.9%.

Conclusion: The paper concludes that Reasoning Memory's gains come from the broad procedural coverage of source trajectories and the decomposition and retrieval design, enabling effective extraction and reuse of procedural knowledge.
...
TL;DR: This paper identifies a new failure mode called unintentional cross-user contamination (UCC) in LLM-based agents that share a knowledge layer across multiple users, leading to silent wrong answers.


<details>
  <summary>Details</summary>
Motivation: The increasing use of LLM-based agents that maintain task states across sessions and serve multiple users with a shared knowledge layer, which expands the failure surface and can lead to silent degradation of outcomes for some users.
...
Result: The results show that benign interactions alone can produce contamination rates of 57-71% under raw shared state, and even with write-time sanitization, substantial residual risk remains, especially when shared state includes executable artifacts.

Conclusion: The study concludes that shared-state agents need artifact-level defenses beyond text-level sanitization to prevent silent cross-user failures.
...
TL;DR: Deep Policy Research (DPR) is a system that drafts content moderation policies using human-written seed domain information and outperforms baselines in generating effective policies.


<details>
  <summary>Details</summary>
Motivation: Drafting and maintaining domain-specific safety policies for content moderation is costly, and the authors aim to create a system that can automate this process.
...
Conclusion: DPR is an effective system for drafting content moderation policies, and a task-specific, structured research loop can be more effective than generic web research for policy drafting.
...
TL;DR: The paper investigates how language models process entity-centric factual questions, finding that entity information is encoded in sparse, causally actionable neurons.
...
Method: The authors use templated prompts to localize entity-selective MLP neurons and validate them with causal interventions on QA examples.

Result: Localized neurons are found in early layers, and activating a single neuron can recover entity-consistent predictions, indicating compact entity retrieval.

Conclusion: The study identifies sparse access points for analyzing and modulating entity-conditioned factual behavior in language models.
...
Method: Comparison of three recent approaches for computing pause thresholds and evaluation of a novel method for computing Production Unit Breaks.

Result: The paper aims to provide a novel method for computing Production Unit Breaks, but the specific results are not mentioned in the abstract.

Conclusion: The paper concludes by suggesting and evaluating a new method for computing Production Unit Breaks, implying an improvement in understanding translation production processes.
...
TL;DR: The paper proposes a conformal prediction framework for multi-turn language models, enabling adaptive stopping while maintaining coverage guarantees.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for multi-turn reasoning in large language models lack formal guarantees for correct answers and rely on heuristic stopping rules, which can be problematic in high-stakes domains.

Method: The proposed framework, Multi-Turn Language Models with Conformal Prediction (MiCP), allocates error budgets across turns, allowing for early stopping while maintaining overall coverage guarantees.

Result: MiCP achieves target coverage on single-hop and multi-hop question answering benchmarks while reducing turns, inference cost, and prediction set size.

Conclusion: The proposed framework provides a formal guarantee for multi-turn language models, enabling efficient and accurate reasoning in high-stakes domains.
...
TL;DR: The authors propose a benchmarking framework that efficiently predicts large language model performance on unseen tasks, achieving high accuracy with minimal evaluation cost.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve benchmarking efficiency for large language models by focusing on predictive validity, which measures how well a benchmarking framework can predict model performance on unseen tasks.

Method: The authors combine a modified multidimensional item response theory (IRT) model with adaptive item selection driven by optimal experimental design to predict model performance.

Result: The method achieves a mean absolute error (MAE) of less than 7% on 112 held-out benchmark tasks after observing only 16 items, and reduces evaluation cost by 85% to 22,000 tokens.

Conclusion: The proposed framework enables more efficient and principled benchmarking of large language models, with significant cost savings while maintaining high predictive accuracy.
...
TL;DR: The study investigates whether the origin of Hindi synonyms can be distinguished using distributional data, and finds that context encodes etymological signals, supporting the idea that synonymous words offer different perspectives.


<details>
  <summary>Details</summary>
Motivation: The study aims to test the claim that synonyms may reflect different perspectives or carry distinct cultural associations, and to investigate whether centuries after borrowings appeared in the Subcontinent, their origin can still be distinguished using distributional data alone.

Method: A Random Forest classifier trained on word embeddings of Hindi synonyms was used to classify words by Sanskrit or Perso-Arabic origin.

Result: The classifier successfully classified words by origin, even when they were semantically unrelated, suggesting that usage patterns preserve traces of etymology.

Conclusion: The findings provide quantitative evidence that context encodes etymological signals and that synonymy may reflect subtle but systematic distinctions linked to origin, supporting the idea that synonymous words can offer different perspectives.
...
TL;DR: The paper explores the impact of citation granularity on model performance in attributed generation, finding that fine-grained citations degrade attribution quality, while intermediate granularities (paragraph-level) achieve the best results.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to investigate the impact of citation granularity on model performance in attributed generation, as fine-grained citations are often preferred for human verification but their effect on model performance is under-explored.

Method: The authors analyze four model scales (8B-120B) and compare the performance of different citation granularities, including fine-grained (sentence-level), intermediate (paragraph-level), and coarse (multi-paragraph) citations.

Result: The results show that fine-grained citations degrade attribution quality by 16-276% compared to the best-performing granularity, which is paragraph-level. The authors also find that the performance gap varies non-monotonically with model scale, with larger models being disproportionately penalized by fine-grained constraints.

Conclusion: The paper concludes that optimizing solely for human verification via fine-grained citation disregards model constraints, compromising both attribution faithfulness and generation reliability. Instead, effective attribution requires aligning citation granularity with the model's natural semantic scope.
...
TL;DR: The paper analyzes the overconfidence in large language models' (LLMs) incorrect answers and identifies specific internal circuits that drive this issue, proposing targeted interventions to improve calibration.


<details>
  <summary>Details</summary>
Motivation: LLMs often produce factually incorrect answers with high confidence, misleading users and undermining confidence scores as a reliable uncertainty signal.

Method: The authors perform a circuit-level mechanistic analysis, identifying and intervening on specific MLP blocks and attention heads in LLMs to mitigate overconfidence.

Result: The study finds that a compact set of middle-to-late layer circuits consistently inflates verbalized confidence and that targeted interventions on these circuits substantially improve calibration.

Conclusion: Verbalized overconfidence in LLMs is driven by identifiable internal circuits and can be mitigated through targeted intervention, suggesting a path towards more reliable uncertainty signals in LLMs.
...
TL;DR: The paper analyzes Persian poetry using a computational approach, revealing that its symbolic system evolves over time with changing internal weights and connections.


<details>
  <summary>Details</summary>
Motivation: The study aims to overcome the limitations of previous computational work on Persian poetry, which tends to isolate words or focus on broad document semantics, by examining the relationships between recurrent forms and their evolution over time.

Method: The authors use a corpus of 129,451 poems to create a multi-layer graph that maps the relations between recurrent forms, separating imagistic material from sacred and courtly references.

Result: The analysis shows that the symbolic core of Persian poetry is relatively sparse, while the referential component is denser, with some families of forms remaining widely distributed across centuries, and others gaining or losing prominence over time.

Conclusion: The study reveals that Persian symbolism appears as a long-lived system whose internal weights and connections change over time, rather than a fixed repertory, with changes in arrangement and membership of the graph across centuries.
...
<details>
  <summary>Details</summary>
Motivation: The terminology in LLM research is fragmented due to implicit normative objectives, and the authors aim to provide a structured approach to understanding output variation.

Method: The authors propose the Magic, Madness, Heaven, Sin framework, which categorizes tasks into four normative contexts: epistemic, interactional, societal, and safety, and analyze failure modes and vocabulary for each context.

Result: The framework reveals that optimizing for one objective can harm other aspects, such as demographic representation or creative diversity, and highlights the need for context-aware evaluation of output variation.

Conclusion: The paper argues that output variation should be reframed as a property shaped by task objectives rather than a model's intrinsic trait, and advocates for a more nuanced understanding of LLMs' performance across different contexts.
...
TL;DR: Instruction-based unlearning doesn't work well for diffusion-based image generation models.


<details>
  <summary>Details</summary>
Motivation: To investigate if instruction-based unlearning can be applied to diffusion-based image generation models.

Method: Controlled experiments across multiple concepts and prompt variants, analyzing CLIP text encoder and cross-attention dynamics.

Result: Diffusion models fail to suppress targeted concepts when guided solely by natural-language unlearning instructions.

Conclusion: Effective unlearning in diffusion models requires interventions beyond inference-time language control.
...
TL;DR: The optimal representation of web page observations for large language models depends on model capability and thinking token budget.


<details>
  <summary>Details</summary>
Motivation: Prior work reduced HTML verbosity for better performance, but this trend is revisited to explore optimal observation representations.

Method: Analyzing the effect of compact (accessibility trees) vs. detailed (HTML) observations on model performance, with consideration of model capability and thinking token budget.

Result: Higher-capability models benefit from detailed HTML observations, while lower-capability models prefer compact representations; incorporating observation history improves performance.

Conclusion: Adaptive selection of observation representations based on model capability and thinking token budget, along with incorporating observation history, can improve performance.
...
TL;DR: This study presents a model merging framework to adapt general-purpose large language models (LLMs) to the medical domain, mitigating the 'forgetting' issue of instruction-following ability when fine-tuned on task-specific medical datasets.


<details>
  <summary>Details</summary>
Motivation: The adoption of LLMs in the medical domain is hindered by their tendency to 'forget' instruction-following ability when fine-tuned on task-specific medical datasets.

Method: The study uses a model merging framework, interpolating a clinical foundation model (GatorTronLlama) with a general instruct model (Llama-3.1-8B-Instruct) to derive a domain-adapted model.

Result: The merged models effectively mitigate catastrophic forgetting, preserve clinical domain expertise, and retain instruction-following ability, achieving performance on par with fully fine-tuned baselines under severely constrained supervision.

Conclusion: The model merging strategy provides a highly scalable solution for adapting open-source LLMs to clinical applications, facilitating broader deployment in resource-constrained healthcare environments.
...
TL;DR: The paper proposes DeltaMem, an agentic memory management system for persona-centric memory, outperforming baselines in long-term memory benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing multi-agent systems for persona memory management suffer from information loss and fragility across scenarios, leading to suboptimal performance.

Method: The authors propose DeltaMem, which formulates persona-centric memory management as an end-to-end task within a single-agent setting, and use a reinforcement learning framework with a novel Memory-based Levenshtein Distance to enhance performance.

Result: DeltaMem outperforms all product-level baselines across diverse long-term memory benchmarks, including LoCoMo, HaluMem, and PersonaMem.

Conclusion: The proposed DeltaMem system demonstrates improved performance in persona-centric memory management, with the ability to effectively manage long-term memory.
...
<details>
  <summary>Details</summary>
Motivation: The deployment of Large Language Models is hindered by high memory and bandwidth demands, which SVD-based compression aims to alleviate, but existing methods have limitations in reconstruction error or practical efficiency.

Method: Swift-SVD proposes an incremental aggregation of output activation covariance and a single eigenvalue decomposition for optimal, efficient, and stable layer-wise low-rank approximation, with dynamic rank allocation based on local reconstruction loss and layer importance.

Result: Swift-SVD outperforms state-of-the-art baselines across six LLMs and eight datasets, achieving optimal compression accuracy with 3-70X speedups in end-to-end compression time.

Conclusion: Swift-SVD provides a practical and efficient solution for compressing Large Language Models, addressing the limitations of existing methods and demonstrating superior performance in accuracy and efficiency.
...
TL;DR: A nationwide survey of 349 K-12 teachers in Indonesia found increasing use of AI in education, but uneven adoption and several hindrances to effective integration.


<details>
  <summary>Details</summary>
Motivation: To investigate the use of AI in Indonesian classrooms and the support teachers need, as there is limited large-scale evidence on this topic.

Method: A nationwide survey of 349 K-12 teachers across elementary, junior high, and senior high schools in Indonesia.

Result: The survey found increasing use of AI for pedagogy, content development, and teaching media, but uneven adoption across levels, with elementary teachers reporting more consistent use. Teachers primarily use AI to reduce instructional preparation workload.

Conclusion: The study highlights the need for context-appropriate AI systems and policies, and for addressing the hindrances to effective classroom integration, such as generic outputs, infrastructure constraints, and limited contextual alignment.
...
TL;DR: The paper analyzes the reasoning capabilities of claim verification benchmarks and finds that they mostly test direct evidence extraction, with biases towards lexical matching and limited representation of complex reasoning.


<details>
  <summary>Details</summary>
Motivation: To systematically understand the reasoning exercised by claim verification benchmarks.

Method: Generating structured reasoning traces for 24K claim-verification examples using GPT-4o-mini and analyzing error types using a 1B-parameter reasoning verifier.

Result: Direct evidence extraction dominates, while multi-sentence synthesis and numerical reasoning are under-represented; biases exist across datasets, and error profiles vary by domain.

Conclusion: High benchmark scores mainly reflect retrieval-plus-entailment ability, and more challenging evaluation suites are needed to test complex reasoning capabilities.
...
TL;DR: A new framework called PRCCF is proposed to improve emotional support conversations by enhancing contextual understanding through persona-guided retrieval and causality-aware cognitive filtering.


<details>
  <summary>Details</summary>
Motivation: Existing methods for emotional support conversations struggle with deep contextual understanding.

Method: PRCCF, a Persona-guided Retrieval and Causality-aware Cognitive Filtering framework, which includes a persona-guided retrieval mechanism and a causality-aware cognitive filtering module.
...
Conclusion: The proposed PRCCF framework effectively improves emotional support conversations by enhancing contextual understanding.
...
TL;DR: The study compares the impact of Chain-of-Thought (CoT) trajectories from two models on the generalization performance of large reasoning models. It finds that lower training loss does not necessarily translate to better generalization and identifies a difference in reasoning patterns between the models.


<details>
  <summary>Details</summary>
Motivation: Understanding how CoT trajectories from different sources influence the generalization performance of models.

Method: Comparative study using two sources of verified CoT trajectories, multi-faceted analysis of token-level SFT loss and step-level reasoning behaviors, and proposal of filtering out frequently branching trajectories.

Result: Models trained on CoT trajectories from one model (DeepSeek-R1-0528) exhibit worse generalization performance despite lower training loss, due to inefficient exploration behaviors. Filtering out frequently branching trajectories improves reasoning performance by up to 5.1% on AIME25 and 5.5% on BeyondAIME.

Conclusion: The study highlights the importance of CoT trajectory quality over quantity and proposes a simple yet effective remedy for improving the generalization of SFT.
...
TL;DR: EndoASR, a domain-adapted ASR system for gastrointestinal endoscopy, improves transcription accuracy and clinical usability, achieving real-time deployment and efficient edge deployment.


<details>
  <summary>Details</summary>
Motivation: The reliability of Automatic Speech Recognition (ASR) in real-world clinical settings is limited by domain-specific terminology and complex acoustic conditions in gastrointestinal endoscopy.

Method: A two-stage adaptation strategy based on synthetic endoscopy reports, targeting domain-specific language modeling and noise robustness, was developed for EndoASR.

Result: EndoASR substantially improves transcription accuracy (CER from 20.52% to 14.14%) and medical term accuracy (Med ACC from 54.30% to 87.59%) in retrospective evaluation, and demonstrates consistent generalization in prospective multi-center study (CER from 16.20% to 14.97%, Med ACC from 61.63% to 84.16%).

Conclusion: Domain-adapted ASR can serve as a reliable interface for human-AI teaming in gastrointestinal endoscopy, with consistent performance validated across multi-center real-world clinical settings.
...
TL;DR: This paper provides a comprehensive comparison of memory methods for large language model-based agents, proposes a unified framework, and introduces a new memory method that outperforms state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: The need for a systematic comparison of existing memory methods for LLM-based agents, as memory is crucial for long-horizon complex tasks.

Method: The paper summarizes a unified framework for existing agent memory methods and extensively compares representative methods on two benchmarks.

Result: The paper provides a thorough analysis of existing methods and introduces a new memory method that outperforms state-of-the-art methods.

Conclusion: The paper offers promising future research opportunities and provides valuable new insights for future research on memory methods for LLM-based agents.
...
TL;DR: The paper proposes a human-machine collaborative framework for Vietnamese Speech Emotion Recognition (SER) that integrates human knowledge into the learning process, achieving strong performance on a dataset of 2,764 samples.


<details>
  <summary>Details</summary>
Motivation: The lack of reliable annotated data and ambiguous acoustic patterns make SER challenging, especially in real-world conditions where emotional boundaries are not clearly separable.

Method: A human-machine collaborative framework centered around LLM-based reasoning, using acoustic feature-based models to provide auxiliary signals, a confidence-based routing mechanism, and an iterative refinement strategy.

Result: The proposed method achieves up to 86.59% accuracy and Macro F1 around 0.85-0.86 on a Vietnamese speech dataset.
...
TL;DR: The paper presents a nuanced approach to detecting toxic content in Bulgarian text while preserving access to essential information.


<details>
  <summary>Details</summary>
Motivation: Current solutions for toxic content detection often block valuable information, including medical terms and text related to minority groups.

Method: The research proposes an ontology for modeling toxic words in Bulgarian and trains a BERT-based model for toxic language classification using a dataset of 4,384 manually annotated sentences.

Result: The trained model reaches a 0.89 F1 macro score and is directly applicable in a real environment.

Conclusion: The developed methodologies have potential applications across diverse online platforms and content moderation systems, enabling more effective toxic content detection while preserving access to essential information.
...
TL;DR: The paper introduces LiveMathematicianBench, a dynamic benchmark for evaluating the mathematical reasoning capabilities of large language models (LLMs) using recent arXiv papers, and finds that even the best models struggle with genuine understanding.


<details>
  <summary>Details</summary>
Motivation: The motivation is to rigorously evaluate the mathematical capabilities of LLMs, which is crucial as they are increasingly integrated into scientific workflows, and to address the limitations of existing benchmarks.

Method: The method involves creating a dynamic multiple-choice benchmark from recent arXiv papers, introducing a logical taxonomy of theorem types, and employing a proof-sketch-guided distractor pipeline and a substitution-resistant mechanism to evaluate LLMs.

Result: The best model, Gemini-3.1-pro-preview, achieves only 43.5% accuracy, and under substitution-resistant evaluation, accuracy drops sharply, with GPT-5.4 scoring highest at 30.6% and Gemini-3.1-pro-preview falling to 17.6%.

Conclusion: The paper concludes that LiveMathematicianBench offers a scalable, contamination-resistant testbed for studying research-level mathematical reasoning in LLMs, and that the benchmark is far from saturated, indicating a need for further improvement in LLM mathematical reasoning capabilities.
...
TL;DR: The authors propose a Controllable Automatic Text Simplification (CATS) framework using instruction fine-tuning with discrete control tokens, enabling control over readability levels and compression rates across different domains and models.


<details>
  <summary>Details</summary>
Motivation: Current CATS methods treat controllability as a decoding problem and use metrics that don't accurately measure control. The authors aim to address the limitations of controllability in ATS imposed by data and evaluation.

Method: The authors introduce a domain-agnostic CATS framework based on instruction fine-tuning with discrete control tokens, using open-source models (Llama, Mistral, Qwen; 1-14B) across four domains (medicine, public administration, news, encyclopedic text).

Result: Smaller models (1-3B) can be competitive, but reliable controllability depends on sufficient variation in the target attribute in the training data. Readability control is learned consistently, while compression control underperforms due to limited signal variability.

Conclusion: The authors highlight the importance of measuring control accurately and propose a framework for controllable text simplification. They also emphasize the need for error-based measures for target-output alignment and careful data sampling and stratification to avoid distributional mismatch.
...
TL;DR: The paper proposes Distribution-guided Efficient Fine-Tuning (DEFT), a framework that enhances the efficiency and performance of aligning Large Language Models with human values while preserving their generalization ability.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of current methods like Reinforcement Learning from Human Feedback (RLHF) and Proximal Policy Optimization (PPO), which are costly, unstable, and may weaken the generalization ability of LLMs.

Method: The method introduced is DEFT, which incorporates data filtering and distributional guidance by calculating the differential distribution reward based on the output distribution of the language model and the discrepancy distribution of preference data.

Result: Experimental results show that methods enhanced by DEFT outperform original methods in both alignment capability and generalization ability, with significantly reduced training time.

Conclusion: The paper concludes that DEFT is an efficient alignment framework that can enhance the performance and efficiency of aligning LLMs with human values while mitigating the loss of generalization ability.
...
TL;DR: The paper introduces PLOT, a method for preference learning in Large Language Models (LLMs) using Optimal Transport, which improves alignment with human preferences while preserving the original distribution and capturing global token-level relationships.


<details>
  <summary>Details</summary>
Motivation: Existing preference learning methods for LLMs have limitations such as modest performance gains, high computational costs, hyperparameter sensitivity, and insufficient modeling of global token-level relationships.

Method: PLOT (Preference Learning using Optimal Transport) formulates preference learning as an Optimal Transport Problem, using a token-level loss to align model outputs with human preferences while preserving the original distribution of LLMs.

Result: PLOT consistently improves alignment performance across two preference categories (Human Values and Logic & Problem Solving) and seven subpreferences, while maintaining fluency and coherence.

Conclusion: The paper establishes optimal transport as a principled methodology for preference learning in LLMs, providing a theoretically grounded framework that offers new insights for preference learning.
...
TL;DR: The paper proposes Adaptive Placeholder Completion (APC), a framework that improves code completion by strategically outputting placeholders at uncertain positions, reducing editing costs while preserving standard completion quality.


<details>
  <summary>Details</summary>
Motivation: The authors observed that Large Language Models (LLMs) often make erroneous predictions in code completion, even when the generated code is similar to the user's subsequent code. They found that 61% of generated suggestions were edited or rejected despite being over 80% similar to the user's code.

Method: The authors propose APC, a collaborative framework that extends the Hard Completion (HC) paradigm by outputting explicit placeholders at high-entropy positions. They formulate code completion as a cost-minimization problem under uncertainty and derive a critical entropy threshold for APC to achieve lower expected cost than HC.

Result: The authors evaluated APC across 1.5B--14B parameter models and found that it reduces expected editing costs by 19% to 50% while preserving standard HC performance.
...
TL;DR: This paper develops a system to identify dyslexic spelling errors using a neural model, achieving high accuracy while emphasizing the need for ethical deployment and consideration of potential misuse.
...
Method: The authors formulate dyslexic error attribution as a binary classification task and develop a comprehensive feature set and a twin-input neural model, which is evaluated against traditional machine learning baselines.
...
Conclusion: The results demonstrate that dyslexic error attribution is feasible at high accuracy, but emphasize that feasibility alone is insufficient for deployment in high-stakes educational contexts, and provide guidelines for ethical deployment.
...
TL;DR: The study evaluates transformer-based models for short-term mortality prediction in heart failure patients, comparing text-only, structured-only, multimodal, and LLM-based approaches.


<details>
  <summary>Details</summary>
Motivation: Accurate short-term mortality prediction in heart failure patients remains challenging, especially when using only structured electronic health record data.

Method: The study uses transformer-based models on a French heart failure cohort, comparing different approaches including text-only, structured-only, multimodal, and LLM-based methods.

Result: The results show that multimodal fusion of text and structured variables achieves the best performance, and entity-aware multimodal transformers offer the most reliable solution for short-term heart failure outcome prediction.

Conclusion: Entity-aware multimodal transformers are the most reliable solution for short-term heart failure outcome prediction, while current LLM prompting remains limited for clinical decision support.
...
TL;DR: The paper introduces ImplicitBBQ, a benchmark to evaluate implicit biases in Large Language Models (LLMs) across various demographics. It finds that LLMs exhibit significant implicit biases, even when safety prompting and chain-of-thought reasoning are used.
...
Method: The authors introduce ImplicitBBQ, a QA benchmark that uses characteristic-based cues to evaluate implicit biases. They evaluate 11 LLMs and use various prompting techniques, including safety prompting, chain-of-thought reasoning, and few-shot prompting.

Result: The authors find that implicit bias in ambiguous contexts is over six times higher than explicit bias in open-weight models. Safety prompting and chain-of-thought reasoning do not substantially reduce this gap. Few-shot prompting reduces implicit bias by 84%, but caste bias remains at four times the level of other dimensions.

Conclusion: The paper concludes that current alignment and prompting strategies only address the surface of bias evaluation, leaving culturally grounded stereotypic associations largely unresolved. The authors publicly release their code and dataset for model providers and researchers to benchmark potential mitigation techniques.
...
<details>
  <summary>Details</summary>
Motivation: Existing propaganda detection methods based on Language Models like BERT often overfit their training data due to biases in data collection.
...
Conclusion: The neurosymbolic approach enhances classification robustness and improves generalization to new sources, making it a promising solution for propaganda detection.
...
TL;DR: The paper presents a mathematical framework to measure the optimality of word order variation in languages, finding that crosslinguistic gestures are at least 77% optimal in minimizing swap distance in a permutohedron.
...
Method: A novel mathematical framework is introduced to measure the degree of optimality of word order variation with respect to swap distance minimization, utilizing the quadratic assignment problem (QAP).

Result: Crosslinguistic gestures are found to be at least 77% optimal, with multiple instances of optimality unlikely to be due to chance.

Conclusion: The paper establishes theoretical foundations for research on the optimality of word or gesture order in communication systems and postulates a general principle of optimal assignment that unifies various linguistic principles.
...
TL;DR: The paper studies translation quality in the EU20 benchmark suite using an automated quality assurance approach, finding that datasets with lower quality scores have more errors, and releasing cleaned versions of the datasets.


<details>
  <summary>Details</summary>
Motivation: Machine-translated benchmark datasets are cost-effective and scalable, but their quality and reliability are concerns due to noise, loss of structure, and uneven quality.

Method: The paper uses a three-step automated quality assurance approach: (i) structural corpus audit, (ii) quality profiling using a neural metric (COMET), and (iii) LLM-based span-level translation error landscape.

Result: The study finds that datasets with lower COMET scores have a higher share of accuracy/mistranslation errors, and that automated quality assurance can help prioritize review.

Conclusion: Automated quality assurance offers practical, scalable indicators that help prioritize review, complementing human gold standards, and the authors release cleaned/corrected versions of the EU20 datasets and code for reproducibility.
...
TL;DR: A new benchmarking framework called SAFE is proposed to improve the accuracy of multi-hop QA by ensuring that the reasoning steps are verifiable and grounded in a knowledge graph.


<details>
  <summary>Details</summary>
Motivation: Current multi-hop QA benchmarks often reward large language models for incorrect or ungrounded reasoning steps, and the authors aim to shift towards rigorous reasoning.

Method: The SAFE framework consists of two phases: train-time verification, which establishes an error taxonomy and a verification pipeline to eliminate noisy supervision, and inference-time verification, which uses a feedback model to detect ungrounded steps in real-time.

Result: The experimental results show that SAFE outperforms standard baselines, achieving an average accuracy gain of 8.4 pp, while guaranteeing verifiable trajectories at inference-time.

Conclusion: The SAFE framework exposes critical flaws in existing benchmarks and provides a more rigorous approach to evaluating multi-hop QA models.
...
TL;DR: The paper proposes a new method called kNNProxy for detecting LLM-generated text without requiring training or fine-tuning, addressing limitations in existing zero-shot methods.


<details>
  <summary>Details</summary>
Motivation: Existing zero-shot methods for LLM-generated text detection rely on a well-aligned proxy LLM, which is often not the case in real-world scenarios, and typically require supervised fine-tuning or repeated API interactions, increasing costs and limiting robustness.
...
Result: Extensive experiments demonstrate strong detection performance of the proposed method, improving robustness under domain shift.

Conclusion: The paper concludes that the proposed kNNProxy method, and its extension into a mixture of proxies (MoP), offers a promising solution for reliable LLM-generated text detection, addressing the limitations of existing zero-shot methods.
...
TL;DR: The paper investigates why Gaussian diffusion models struggle with discrete data and proposes methods to improve sample quality.


<details>
  <summary>Details</summary>
Motivation: The application of diffusion models to discrete data remains challenging.

Method: The authors use a toy model, analyze the behavior of DDPM, and propose combining self-conditioning with q-sampling.

Result: The proposed methods improve generation quality on real data across multiple domains.

Conclusion: The paper provides insights into the challenges of applying diffusion models to discrete data and proposes effective solutions.
...
TL;DR: The study examines how linguistic structures emerge in self-supervised speech models during training, analyzing six Wav2Vec2 and HuBERT models trained on spoken Dutch.


<details>
  <summary>Details</summary>
Motivation: Understanding when and how linguistic structure emerges in model training for self-supervised speech models.

Method: Analyzing the encoding of linguistic structures across layers and intermediate checkpoints of six Wav2Vec2 and HuBERT models.

Result: Different levels of linguistic structure show distinct layerwise patterns and learning trajectories, influenced by abstraction from the acoustic signal, timescale of input integration, and pre-training objectives.

Conclusion: The study provides insights into the emergence of linguistic structure in self-supervised speech models, highlighting the impact of pre-training objectives on layerwise organization and learning trajectories.
...
TL;DR: The paper presents a method to transform causal generative language models into bidirectional encoders, addressing limitations of current approaches and achieving state-of-the-art results on various benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current approaches to transforming causal generative language models into bidirectional encoders are limited by lack of consensus on training objectives, catastrophic forgetting, and integration with specialized generative models.

Method: The authors propose a systematic ablation study to identify key factors for successful adaptation, and introduce a dual strategy combining linear weight merging and a lightweight multi-domain data mixture to mitigate catastrophic forgetting. They also merge their encoders with specialized causal models to transfer modality- and domain-specific capabilities.
...
Conclusion: The paper provides an open-source recipe for transforming causal generative language models into bidirectional encoders, addressing key limitations and achieving state-of-the-art results.
...
TL;DR: Speculative decoding accelerates large language model inference by drafting and verifying multiple candidate tokens in a single forward pass.


<details>
  <summary>Details</summary>
Motivation: Existing training-free methods for speculative decoding have limitations, as they draft from a single token source and shape their trees without distinguishing candidate quality across origins.

Method: GOOSE, a training-free framework that builds an adaptive spine tree, combining high-acceptance context-matched tokens with wide branches of low-acceptance alternatives.
...
Conclusion: The proposed GOOSE framework optimizes speculative decoding by creating an anisotropic tree structure, leading to significant speedup in large language model inference.
...
TL;DR: Introduced RRPO, a reinforcement learning framework that optimizes reranking for Retrieval-Augmented Generation by aligning it with LLM's generation quality, eliminating need for human annotations.


<details>
  <summary>Details</summary>
Motivation: Current reranking models are optimized on static human-annotated relevance labels, which are decoupled from the downstream generation process, leading to a misalignment between retrieved documents and actual utility required by LLM.

Method: ReRanking Preference Optimization (RRPO), a reinforcement learning framework that formulates reranking as a sequential decision-making process, optimizing for context utility using LLM feedback.

Result: RRPO significantly outperforms strong baselines on knowledge-intensive benchmarks, and demonstrates versatility by generalizing to diverse readers, integrating with query expansion modules, and remaining robust with noisy supervisors.

Conclusion: The proposed RRPO framework effectively bridges the gap between retrieval and generation processes, providing a more aligned and efficient approach to reranking for Retrieval-Augmented Generation.
...
TL;DR: The paper evaluates the sensitivity of self-supervised speech models (S3Ms) to prosodic contrasts, introducing a new framework called prosodic ABX, and provides insights into the models' performance across different languages and experimental conditions.


<details>
  <summary>Details</summary>
Motivation: To investigate the sensitivity of S3Ms to prosodic contrasts, as previous studies have mainly focused on phonemic contrasts.

Method: The authors extend the ABX discrimination task to prosodic contrasts (prosodic ABX) and create a dataset of minimal pairs in English, Japanese, and Mandarin to evaluate contrast in stress, pitch accent, and tone.

Result: The study finds that S3Ms can capture prosodic contrasts, and model and layer rankings are often preserved across experimental conditions, making it practical for low-resource settings.

Conclusion: The paper concludes that prosodic ABX is a useful framework for evaluating prosodic contrast in S3Ms, and the results have implications for the development of more effective speech models.
...
TL;DR: The paper proposes a method to improve the construction of steering vectors for controlling reasoning behaviors in large language models. The authors show that current methods for detecting behavioral signals are flawed and propose stability filtering to retain only stable boundaries. Their method achieves state-of-the-art results on MATH-500 and transfers across models.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to improve the construction of steering vectors for controlling reasoning behaviors in large language models. The authors aim to address the challenge of identifying genuine behavioral signals in the model's hidden states, particularly for behaviors that emerge spontaneously and resist prompt-level control.

Method: The authors propose stability filtering, which retains only boundaries where the model consistently reproduces the target behavior. They also use a content-subspace projection to remove residual question-specific noise. Their method involves a probabilistic model that formalizes intrinsic reasoning behaviors as stochastic events with context-dependent trigger probabilities.

Result: The authors achieve 0.784 accuracy on MATH-500, outperforming the strongest baseline by 5.0 points. Their steering vectors also transfer across models in the same architecture family without re-extraction, improving Nemotron-Research-Reasoning-1.5B by 5.0 points and DeepScaleR-1.5B-Preview by 6.0 points.

Conclusion: The paper concludes that stability filtering is an effective method for constructing steering vectors that can control reasoning behaviors in large language models. The authors' approach improves the state-of-the-art results on MATH-500 and demonstrates the transferability of steering vectors across models.
...
TL;DR: The paper introduces GaelEval, a benchmark for evaluating multilingual large language models (LLMs) on Scottish Gaelic, and assesses 19 LLMs against a human baseline.


<details>
  <summary>Details</summary>
Motivation: The performance of LLMs on minority languages like Scottish Gaelic is uneven and under-measured, and existing translation benchmarks fail to capture structural competence.
...
Result: The results show that proprietary models outperform open-weight systems, and in-language (Gaelic) prompting yields a small but stable advantage; Gemini 3 Pro Preview achieves 83.3% accuracy on the linguistic task, surpassing the human baseline (78.1%).

Conclusion: GaelEval reveals that frontier models achieve above-human performance on several dimensions of Gaelic grammar, and demonstrates the effect of Gaelic prompting and a consistent performance gap favouring proprietary over open-weight models.
...
TL;DR: The paper studies the impact of chain-of-thought (CoT) reasoning on language agents' performance in structured tool-use settings, finding a non-monotonic relationship between reasoning length and accuracy.


<details>
  <summary>Details</summary>
Motivation: The relationship between reasoning length and accuracy in structured tool-use settings remains poorly understood, and the authors aim to investigate this relationship.

Method: The authors conducted a systematic study using six token budgets (0-512) across 200 tasks from the Berkeley Function Calling Leaderboard v3 Multiple benchmark, and proposed a new method called Function-Routing CoT (FR-CoT).

Result: The study found that brief reasoning (32 tokens) improves accuracy by 45% relative to direct answers, while extended reasoning (256 tokens) degrades performance. The proposed FR-CoT method achieves accuracy statistically equivalent to free-form CoT while reducing function hallucination to 0.0%.

Conclusion: The study reveals a non-monotonic pattern between reasoning length and accuracy, and proposes a structured brief-CoT method (FR-CoT) that provides a structural reliability guarantee without budget tuning.
...
TL;DR: The paper introduces AstroConcepts, a corpus for scientific multi-label text classification in astrophysics, and studies extreme class imbalance in this domain.


<details>
  <summary>Details</summary>
Motivation: Scientific multi-label text classification faces challenges due to extreme class imbalance, particularly with specialized terminology following power-law distributions.

Method: The authors introduce AstroConcepts, a corpus of 21,702 astrophysics paper abstracts labeled with 2,367 concepts, and evaluate traditional, neural, and vocabulary-constrained LLM methods.

Result: The study reveals that vocabulary-constrained LLMs perform competitively, domain adaptation improves rare terminology classification, and frequency-stratified evaluation exposes hidden performance patterns.

Conclusion: The paper provides actionable insights for scientific NLP, establishes benchmarks for extreme imbalance research, and enables systematic study of class imbalance in scientific domains.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to address the task of cross-document software mention coreference resolution without fine-tuning, and to compare the performance of two different approaches, FM and CAR, under various conditions.
...
Result: Both FM and CAR achieve competitive performance (CoNLL F1 of 0.94-0.96) across all subtasks, with CAR consistently outperforming FM by 1 point on the official test set. CAR is also more efficient at large scale, scaling approximately linearly with corpus size, whereas FM scales superlinearly.

Conclusion: The conclusion is that system selection for cross-document software mention coreference resolution should be informed by both the noise profile of the upstream mention detector and the scale of the target corpus, and that CAR is a more efficient and effective choice at large scale.
...
TL;DR: The paper proposes a novel framework to improve Large Language Models (LLMs) by leveraging textual frequency, which is found to be relevant to human cognition in reading speed. The framework consists of three units: Textual Frequency Law (TFL), Textual Frequency Distillation (TFD), and Curriculum Textual Frequency Training (CTFT).


<details>
  <summary>Details</summary>
Motivation: The relatedness of textual frequency to Large Language Models (LLMs) is seldom studied, despite its relevance to human cognition in reading speed. The authors aim to explore this understudied topic.

Method: The framework consists of three units: (1) Textual Frequency Law (TFL), which prefers frequent textual data for LLMs; (2) Textual Frequency Distillation (TFD), which refines frequency estimation through LLM-based story completion; and (3) Curriculum Textual Frequency Training (CTFT), which fine-tunes LLMs in increasing order of sentence-level frequency.

Result: Experiments on the curated Textual Frequency Paired Dataset (TFPD) show the effectiveness of the proposed framework in improving LLMs on various tasks, including math reasoning, machine translation, commonsense reasoning, and agentic tool calling.

Conclusion: The paper concludes that incorporating textual frequency into LLMs can improve their performance, and proposes a novel framework to achieve this.
...
TL;DR: The paper investigates the interpretability of Mixture-of-Experts (MoE) architectures compared to dense feed-forward networks (FFNs) and finds that MoE experts are more interpretable, specializing in fine-grained linguistic tasks.


<details>
  <summary>Details</summary>
Motivation: The motivation is to explore whether the sparsity of MoE architectures makes them inherently easier to interpret than dense FFNs, and to investigate the interpretability of MoE experts at the expert level.

Method: The method used is $k$-sparse probing to compare MoE experts and dense FFNs, and analyzing the specialization of experts at the expert level.

Result: The results show that MoE experts are consistently less polysemantic, and that they specialize in fine-grained linguistic operations or semantic tasks, rather than being broad domain specialists or simple token-level processors.

Conclusion: The conclusion is that MoE architectures are inherently interpretable at the expert level, providing a clearer path toward large-scale model interpretability.
...
TL;DR: The paper proposes Neuro-RIT, a novel framework to improve the robustness of Retrieval-Augmented Language Models (RALMs) to irrelevant or noisy retrieved contexts by neuron-level adaptation.


<details>
  <summary>Details</summary>
Motivation: RALMs are vulnerable to performance degradation with irrelevant or noisy retrieved contexts, and existing approaches may overlook the neuron-level sparsity of Large Language Models (LLMs).

Method: Neuro-RIT uses attribution-based neuron mining to disentangle neurons processing relevant and irrelevant contexts, and a two-stage instruction tuning strategy for noise robustness.

Result: Neuro-RIT consistently outperforms baselines and robustness-enhancing methods across diverse QA benchmarks.

Conclusion: The proposed Neuro-RIT framework effectively enhances the robustness of RALMs to irrelevant or noisy retrieved contexts by leveraging neuron-level adaptation.
...
TL;DR: A novel framework called L3TR is proposed for listwise talent recommendation using large language models, addressing issues like position bias and token consumption.


<details>
  <summary>Details</summary>
Motivation: Talent recruitment is costly and time-consuming, and existing recommendation systems using large language models have limitations such as high token consumption and suboptimal recommendations.

Method: The L3TR framework utilizes an implicit strategy to leverage large language models for recommendation tasks, incorporating a block attention mechanism, local positional encoding, and an ID sampling method.

Result: Extensive experiments on two real-world datasets show that L3TR consistently improves over existing baselines, validating its effectiveness.

Conclusion: The proposed L3TR framework effectively addresses the limitations of existing talent recommendation systems using large language models, providing improved recommendations and reduced token consumption.
...
TL;DR: This paper introduces the first publicly available dataset for Named Entity Recognition (NER) from Arabic speech, CV-18 NER, and benchmarks end-to-end (E2E) and pipeline systems for Arabic NER.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the under-exploration of end-to-end speech NER for Arabic due to its morphological complexity, absence of short vowels, and limited annotated resources.

Method: The method involves creating a new dataset, CV-18 NER, by augmenting the Arabic Common Voice 18 corpus with manual NER annotations, and benchmarking E2E models (Whisper and AraBEST-RQ) and pipeline systems (ASR + text NER).

Result: The result shows that E2E systems substantially outperform the best pipeline configuration, with 37.0% CoER (AraBEST-RQ 300M) and 38.0% CVER (Whisper-medium).

Conclusion: The conclusion is that the introduced dataset and models provide the first open benchmark for end-to-end named entity recognition from Arabic speech, and that Arabic-specific self-supervised pretraining and multilingual weak supervision are effective for this task.
...
TL;DR: The paper studies methods to generate a comprehensive set of valid responses to open-ended prompts. It introduces a metric called 'diversity coverage' and evaluates 18 LLMs, finding that no single model dominates, but a router can predict the best model for each query, outperforming the single best model baseline.
...
Method: The paper introduces 'diversity coverage' as a metric to evaluate the quality of generated answers and uses it to evaluate 18 LLMs. It also proposes a router that predicts the best model for each query.

Result: The results show that no single LLM dominates at generating diverse responses, but a router can outperform the single best model baseline (26.3% vs 23.8%) on the NB-Wildchat dataset and generalize to out-of-domain datasets and different prompting strategies.

Conclusion: The paper concludes that it lays the foundation for studying generating comprehensive answers when multiple models are available, and that a router can be used to predict the best model for each query.
...
TL;DR: The authors analyze the standard practice of initializing new tokens in language models and find that it leads to a degenerate subspace, erasing inter-token distinctions. They propose a new method, Grounded Token Initialization (GTI), which outperforms existing methods in generative recommendation benchmarks.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the initialization of new tokens in language models, which is a key bottleneck when extending LMs with new vocabularies.

Method: The authors propose a new method called Grounded Token Initialization (GTI), a lightweight grounding stage that maps new tokens to distinct, semantically meaningful locations in the pretrained embedding space using paired linguistic supervision.

Result: GTI outperforms both mean initialization and existing auxiliary-task adaptation methods in the majority of evaluation settings across multiple generative recommendation benchmarks.

Conclusion: The authors conclude that token initialization is a key bottleneck when extending LMs with new vocabularies, and that GTI is an effective solution to this problem.
...
TL;DR: The paper argues that structured LLM routing is a systems-level problem, not just a prompt-engineering one, and presents a framework for reasoning about structured routing under heterogeneous backend conditions.


<details>
  <summary>Details</summary>
Motivation: As LLMs become core control components in agentic AI systems, reliable structured routing must balance correctness, latency, and implementation cost under real deployment constraints.

Method: The authors conducted a comprehensive full-factorial benchmark covering 48 deployment configurations and 15,552 requests across OpenAI, Gemini, and Llama backends.

Result: There is no universal best routing mode; instead, backend-specific interaction effects dominate performance, and modes that are highly reliable on some backends can suffer correctness degradation on others.

Conclusion: The paper contributes a deployable framework for reasoning about structured routing under heterogeneous backend conditions, providing a cross-backend evaluation methodology and practical deployment guidance.
...
TL;DR: The paper proposes the Digital Twin Counterfactual Framework (DTCF) for causal inference, which uses digital twins and simulation to estimate counterfactuals, offering a new approach to address the fundamental problem of causal inference.


<details>
  <summary>Details</summary>
Motivation: The fundamental problem of causal inference is that the counterfactual outcome for any individual is never observed, and existing approaches rely on assumptions for missing data.

Method: The DTCF uses a digital twin simulator, formalized as a stochastic mapping within the potential outcomes framework, and subjects it to a hierarchical validation regime with a five-level validation architecture.

Result: The DTCF provides a framework where marginal causal claims become increasingly testable, joint causal claims become explicitly assumption-indexed, and the gap between the two is formally characterized.

Conclusion: The DTCF does not resolve the fundamental problem of causal inference but provides a new framework for causal inference, making causal claims more testable and assumption-indexed.
...
<details>
  <summary>Details</summary>
Motivation: The communication gap between domain experts and ontology engineers during competency question elicitation in ontology engineering is a significant bottleneck.

Method: IDEA2 uses a semi-automated workflow that integrates Large Language Models within a collaborative, expert-in-the-loop process, including an iterative loop of LLM-based extraction, co-creational review, and feedback-driven reformulation.

Result: The workflow was validated in two real-world scenarios, demonstrating that IDEA2 can accelerate the requirements engineering process, improve the acceptance and relevance of the resulting CQs, and exhibit high usability and effectiveness among domain experts.

Conclusion: The paper concludes that IDEA2 is an effective solution for improving collaboration and efficiency in competency question elicitation, and makes the code and experiments available for reproducibility.
...
<details>
  <summary>Details</summary>
Motivation: The need for a shared, explicit world representation in structured domains such as enterprises and institutional systems to ensure semantic consistency, explainability, and long-term stability.

Method: The paper proposes a classification of worlds along dimensions including ontological explicitness and normativity, and introduces semantic models as a mathematical formalism for representing such worlds.

Result: The Ontobox platform is presented as a realization of WMAS, enabling learning and coordination over a shared world model.

Conclusion: WMAS offers a promising approach for ensuring semantic consistency, explainability, and long-term stability in structured domains through a shared world model.
...
TL;DR: The paper proposes a continuum model for AI automation, distinguishing between 'crashing waves' of abrupt capability surges and 'rising tides' of continuous, broad-based improvements. It finds little evidence of crashing waves but significant evidence of rising tides in AI capabilities across over 3,000 tasks.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to understand the progression of AI automation and its potential impact on the economy and labor market. The authors aim to clarify the nature of AI capability improvements, whether they occur in abrupt surges or continuous waves.

Method: The paper uses preliminary evidence from an ongoing evaluation of AI capabilities across over 3,000 broad-based tasks derived from the U.S. Department of Labor O*NET categorization. More than 17,000 evaluations by workers from these jobs were conducted to test AI performance.

Result: The study finds that AI performance is high and improving rapidly across a wide range of tasks. It estimates that by 2024-Q2, AI models successfully complete tasks that take humans approximately 3-4 hours with about a 50% success rate, increasing to about 65% by 2025-Q3. The study predicts that LLMs will be able to complete most text-related tasks with success rates of, on average, 80%-95% by 2029.

Conclusion: The conclusion of this paper is that AI automation primarily occurs through 'rising tides' of continuous improvement rather than 'crashing waves' of abrupt surges. The study suggests that AI capabilities will continue to improve, potentially impacting the economy and labor market significantly as organizations adopt AI.
...
TL;DR: The study examines cognitive biases in Large Language Models (LLMs), introducing a benchmark called LLM CogBias. It finds that LLMs exhibit systematic biases across four families, which can be encoded in model activation space and mitigated through activation steering.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to investigate whether Large Language Models (LLMs) exhibit cognitive biases, how these biases manifest internally, and if they can be mitigated through targeted interventions.

Method: The study defines LLM cognitive bias and introduces LLM CogBias, a benchmark for evaluating cognitive biases in LLMs. It evaluates three LLMs across four families of cognitive biases and uses linear probes and activation steering to analyze and mitigate biases.

Result: The results show that cognitive biases emerge systematically across all four families in the evaluated LLMs. These biases are encoded as linearly separable directions in model activation space. Activation steering can reduce bias scores by 26-32% while preserving downstream capabilities on 25 benchmarks.

Conclusion: The study concludes that LLMs exhibit systematic cognitive biases that can be identified and mitigated. It shows that biases are encoded in model activation space and can be reduced through activation steering, with similar rates of reduction across different architectures.
...
TL;DR: The paper introduces RIFT, a taxonomy for characterizing failure modes in rubric composition and design for evaluating LLMs, and proposes automated metrics for scalable diagnosis.


<details>
  <summary>Details</summary>
Motivation: The lack of a principled way to diagnose rubric quality issues from aggregated or downstream signals alone.

Method: Grounded theory and iterative annotation of rubrics from diverse benchmarks.

Result: RIFT taxonomy with 8 failure modes, fair agreement among human annotators (87% pairwise agreement, 0.64 Cohen's kappa), and automated metrics with up to 0.86 F1.

Conclusion: RIFT provides a systematic way to characterize rubric failure modes, and automated metrics can support scalable diagnosis.
...
Conclusion: VOIMCP offers a more efficient approach to solving POMDPs by selectively disregarding observation information when the value of information is low.
...
TL;DR: The paper introduces CLAWSAFETY, a benchmark to evaluate the safety of personal AI agents like OpenClaw, which run with elevated privileges on local machines. It tests five frontier LLMs across 120 adversarial scenarios and finds that attack success rates range from 40% to 75%, highlighting the need for comprehensive safety evaluations that consider both the model and the deployment framework.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the safety concerns of personal AI agents that operate with elevated privileges on users' local machines, where a single successful prompt injection can lead to severe consequences such as credential leakage, financial transaction redirection, or file destruction.

Method: The paper proposes a benchmark called CLAWSAFETY, which consists of 120 adversarial test scenarios organized along three dimensions: harm domain, attack vector, and harmful action type. The evaluation is conducted in realistic, high-privilege professional workspaces across five domains: software engineering, finance, healthcare, law, and DevOps. Five frontier LLMs are tested as agent backbones, and 2,520 sandboxed trials are run across all configurations.

Result: The results show that attack success rates (ASR) range from 40% to 75% across models and vary sharply by injection vector. The strongest model maintains hard boundaries against credential forwarding and destructive actions, while weaker models permit both. The study also demonstrates that safety is not determined by the backbone model alone but depends on the full deployment stack.

Conclusion: The paper concludes that comprehensive safety evaluations are necessary to consider both the model and the deployment framework as joint variables. The CLAWSAFETY benchmark can be used to evaluate the safety of personal AI agents and highlight the importance of considering the entire deployment stack in safety assessments.
...
TL;DR: The paper examines the reliability of AI-assisted medication systems, focusing on system failures and their clinical consequences, highlighting the risks of AI errors in medication management.


<details>
  <summary>Details</summary>
Motivation: The increasing integration of AI systems in healthcare and pharmacy workflows, and the potential harm caused by a single incorrect recommendation in high-risk domains such as medication management.

Method: A series of controlled, simulated scenarios involving drug interactions and dosage decisions to analyze different types of system failures.
...
Conclusion: The paper emphasizes the importance of understanding failure behavior and real-world impact, and highlights the need to complement traditional performance metrics with risk-aware evaluation approaches in safety-critical domains such as pharmacy practice.
...
TL;DR: A new AI system called Elhuyar was developed to analyze scientific literature more efficiently, combining large language models, structured AI, and human oversight to extract and refine insights.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the challenge of fragmented scientific literature that requires significant human effort to analyze and understand, and to develop a more efficient and reliable method for extracting insights from scientific literature.

Method: The method used in this paper is the development of Elhuyar, a multi-agent, human-in-the-loop system that integrates large language models, structured AI, and human scientists to extract, analyze, and iteratively refine insights from scientific literature.

Result: The result of this paper is the successful deployment of Elhuyar in materials science, which analyzed literature on tungsten under helium-ion irradiation and discovered experimentally correlated exponential helium bubble growth with irradiation dose and temperature.

Conclusion: The conclusion of this paper is that AI-assisted literature review can uncover scientific patterns and accelerate discovery, and that Elhuyar is a reliable and efficient method for extracting insights from scientific literature.
...
TL;DR: The authors propose a framework for large language models (LLMs) to detect infeasibility in combinatorial optimization problems, specifically the minor-embedding problem, and show that it improves accuracy and accelerates optimization.


<details>
  <summary>Details</summary>
Motivation: Most existing methods for NP-hard combinatorial optimization problems focus on generating feasible solutions and do not explicitly address infeasibility detection.

Method: The authors propose an infeasibility-aware framework that combines certifiable dataset construction, supervised fine-tuning, and LLM-assisted downstream search. They introduce a new mathematical programming formulation with provable zero-phase infeasibility screening to generate training instances labeled as feasible or infeasible.

Result: The fine-tuned 8B-parameter LLM improves overall accuracy by up to 30% over GPT-5.2 and provides up to 2x speedup in downstream local search compared to starting from scratch.

Conclusion: The proposed framework enables LLMs to jointly perform solution generation and infeasibility detection, leading to improved accuracy and accelerated optimization in combinatorial optimization problems.
...
TL;DR: The paper introduces Peer Context Outlier Detection (P-COD), a method to reduce hallucinations in Large Language Models (LLMs) for data extraction from text corpora by leveraging document relationships.


<details>
  <summary>Details</summary>
Motivation: Current methods for reducing hallucinations in LLMs focus on individual documents and fail to consider relationships across a corpus, which is crucial for improving data extraction accuracy.

Method: The paper proposes P-COD, which uses peer validation within a corpus to adjust confidence scores of extracted data and flag low-confidence results for expert review.

Result: The experiments demonstrate up to 98% precision in outlier detection across 6 domains of science, showing that P-COD reduces hallucinations and enhances trust in automated systems.

Conclusion: The proposed approach, P-COD, effectively reduces hallucinations in LLMs, enhances the reliability of automated data extraction systems, and streamlines data extraction workflows by leveraging peer context.
...
TL;DR: The authors present an agentic framework for metasurface inverse design that uses context-level skill evolution to improve workflow construction and reduce the need for specialized expertise.


<details>
  <summary>Details</summary>
Motivation: The current process of metasurface inverse design requires specialized expertise in computational electromagnetics and solver-specific software engineering, which can be a significant burden. Recent large language models (LLMs) have the potential to alleviate this burden, but existing language-driven systems have limitations.

Method: The authors propose an agentic framework that couples a coding agent, evolving skill artifacts, and a deterministic evaluator grounded in physical simulation. This framework allows for iterative refinement of solver-specific strategies across tasks without modifying model weights or the underlying physics solver.

Result: The framework is evaluated on a benchmark spanning multiple metasurface inverse-design task types, with separate training-aligned and held-out task families. The results show significant improvements in task success, criteria pass fraction, and average attempts. The evolved skills also demonstrate partial transfer of workflow knowledge to held-out task families.

Conclusion: The proposed framework offers a practical path toward more autonomous and accessible metasurface inverse-design workflows by accumulating reusable solver-specific expertise around reliable computational engines.
...
<details>
  <summary>Details</summary>
Motivation: The rise of personalized LLM agent frameworks creates novel privacy challenges in human-centered agentic social networks, which remain unexplored.

Method: The authors introduce AgentSocialBench, a benchmark comprising scenarios across seven categories, grounded in realistic user profiles and social graphs.

Result: The experiments reveal two key findings: (1) cross-domain and cross-user coordination creates persistent leakage pressure, and (2) privacy instructions can paradoxically cause agents to discuss sensitive information more.

Conclusion: Current LLM agents lack robust mechanisms for privacy preservation in human-centered agentic social networks, and new approaches beyond prompt engineering are needed.
...
TL;DR: A new platform, S-Researcher, uses LLM agents to assist in social science research, making it more efficient and scalable by simulating human behavior and automating the research process.
...
Method: The authors developed YuLan-OneSim, a large-scale social simulation system, and S-Researcher, a platform that uses LLM agents to simulate human behavior, design experiments, analyze results, and generate reports.

Result: The authors validated S-Researcher through case studies, demonstrating its ability to reproduce cultural dynamics, test hypotheses, and identify cooperation mechanisms, establishing a new human-AI collaborative paradigm for social science.

Conclusion: S-Researcher has the potential to accelerate discovery in social science research by augmenting human researchers with computational simulation, making the research process more efficient and scalable.
...
<details>
  <summary>Details</summary>
Motivation: Current LLM approaches for information extraction in healthy food policy are limited by misinformation, hallucinations, misclassifications, and omissions due to document diversity and inconsistency.

Method: A role-based LLM framework with three specialized roles: policy analyst, legal strategy specialist, and food system expert, incorporating structured domain knowledge into role-specific prompts.

Result: The proposed framework shows superior performance in complex reasoning tasks compared to zero-shot, few-shot, and chain-of-thought baselines using Llama-3.3-70B on 608 healthy food policies.
...
TL;DR: The paper introduces PHMForge, a benchmark for evaluating large language model (LLM) agents on industrial Prognostics and Health Management (PHM) tasks, revealing limitations in current LLM agents.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks do not adequately capture the demands of industrial domains where incorrect decisions have significant safety and financial consequences.

Method: The authors create a comprehensive benchmark with 75 scenarios across 7 industrial asset classes and 5 task categories, using specialized tools and execution-based evaluators.

Result: Even top-performing LLM agents achieve only 68% task completion, with failures in tool orchestration, multi-asset reasoning, and cross-equipment generalization.

Conclusion: The study highlights the need for further research in agentic industrial AI, and the authors open-source their benchmark to facilitate progress.
...
Result: The results show that the proposed modifications enable representation autoencoders to achieve performance comparable to traditional VAEs in AR models.

Conclusion: The conclusion is that this work paves the way for a more unified architecture across visual understanding and generative modeling by demonstrating the effectiveness of representation autoencoders in autoregressive models.
...
Result: The study reveals a large negative interaction between Derf and Muon, with a performance gap to RMSNorm growing from +0.31 nats under AdamW to +0.97 under Muon, and identifies two failure modes: saturation and scale blindness.

Conclusion: The findings suggest that the choice of normalization layer and optimizer can have significant interactions, and that certain configurations can lead to performance penalties, but also propose potential mitigations.
...
TL;DR: A new framework called NED-Tree is proposed to automate the translation of Operations Research problems from natural language to executable models, achieving a state-of-the-art accuracy of 72.51%.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of translating Operations Research problems from natural language to executable models, which is crucial but difficult, especially in nonlinear scenarios.

Method: The NED-Tree framework uses a sentence-by-sentence extraction strategy and a recursive tree-based structure to decompose complex nonlinear terms into solver-compatible sub-elements.

Result: The NED-Tree framework achieves a new state-of-the-art with 72.51% average accuracy across 10 benchmarks, and it is the first framework that drives LLMs to resolve nonlinear modeling difficulties through element decomposition.

Conclusion: The proposed NED-Tree framework and the novel benchmark NEXTOR provide a systematic solution to bridge the semantic gap between mathematical formulations and solver codes, enabling the automation of translating Operations Research problems from natural language to executable models.
...
TL;DR: Introduced ThinkTwice, a two-phase framework optimizing LLMs for reasoning problems and answer refinement using Group Relative Policy Optimization (GRPO).


<details>
  <summary>Details</summary>
Motivation: Improve LLMs' reasoning and self-refinement capabilities without requiring correctness signals or critique annotations.

Method: ThinkTwice framework with two phases: optimizing LLMs to solve reasoning problems, then refining solutions using binary correctness reward.

Result: ThinkTwice substantially improves reasoning and refinement performance over baselines across five mathematical reasoning benchmarks and two model families.

Conclusion: Established joint training of reasoning and self-refinement as a principled and effective methodology for RLVR.
...
TL;DR: This study investigates how Large Language Models (LLMs) decide what to teach next, comparing their strategies to human teaching methods. It finds that LLMs can effectively teach by reasoning about a learner's knowledge, similar to humans, and that a Bayes-Optimal teaching model best explains their choices.


<details>
  <summary>Details</summary>
Motivation: The motivation is to understand how LLMs decide what to teach next, whether through reasoning about a learner's knowledge or simpler rules of thumb, and to compare their strategies with human teaching methods.

Method: The study uses a controlled task where a teacher LLM is presented with a hypothetical learner's trajectory through a reward-annotated directed graph. The LLM must reveal a single edge to help the learner choose a better path. Various LLMs are tested as simulated teachers, and their choices are fitted with cognitive models used for humans, including Bayes-Optimal, Bayesian variants, heuristic baselines, and non-mentalizing utility models.

Result: Most LLMs perform well, with strategies similar to humans, and their performance is best explained by a Bayes-Optimal teaching model. Scaffolding interventions can lead to prompt compliance but do not reliably improve teaching decisions on more challenging graphs.

Conclusion: The study concludes that LLMs can effectively teach by reasoning about a learner's knowledge, similar to human strategies. However, prompt compliance does not guarantee better teaching decisions, and cognitive model fits provide insight into LLM tutoring policies.
...
TL;DR: The paper proposes ByteRover, an agent-native memory architecture that integrates memory into large language models, eliminating the need for external infrastructure and achieving state-of-the-art results on long-context reasoning tasks.


<details>
  <summary>Details</summary>
Motivation: Existing approaches to memory-augmented generation treat memory as an external service, leading to semantic drift, loss of coordination context, and fragile recovery. The authors aim to address these limitations by proposing a novel memory architecture.

Method: ByteRover inverts the memory pipeline, using the same LLM for reasoning, curation, structuring, and retrieval of knowledge. It represents knowledge in a hierarchical Context Tree with explicit relations, provenance, and an Adaptive Knowledge Lifecycle.

Result: ByteRover achieves state-of-the-art accuracy on LoCoMo and competitive results on LongMemEval without requiring external infrastructure, vector databases, or graph databases.

Conclusion: The proposed ByteRover architecture effectively integrates memory into large language models, improving long-context reasoning capabilities while eliminating the need for external infrastructure.
...
<details>
  <summary>Details</summary>
Motivation: Existing multimodal large language models (MLLMs) rely on supervised fine-tuning, which has limitations in learning code patterns and self-correction.

Method: Proposed a two-stage multi-turn self-correction RL strategy based on Group Relative Policy Optimization (GRPO) for MM-ReCoder.
...
TL;DR: Proposed CRaFT, a framework that improves the identification of features influencing refusal behavior in large language models, leading to more effective attacks.


<details>
  <summary>Details</summary>
Motivation: The growing safety concerns around large language models (LLMs) and the need to understand their refusal behavior.
...
Result: CRaFT improves attack success rate (ASR) from 6.7% to 48.2% on Gemma-3-1B-it and outperforms baseline methods across multiple jailbreak benchmarks.

Conclusion: Circuit influence is a more reliable criterion than activation magnitude for identifying features that causally mediate refusal behavior.
...
TL;DR: The authors propose a method to predict when and how to distill multi-agent systems into single-agent skills, introducing Metric Freedom (F) as a predictor of skill utility, and a two-stage adaptive distillation framework.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the inconsistent outcome of distilling multi-agent systems into single-agent skills, and to reduce the heavy coordination overhead, context fragmentation, and brittle phase ordering in MAS.

Method: The authors introduce Metric Freedom (F), a predictor of skill utility, which measures the topological rigidity of a metric's scoring landscape. They propose a two-stage adaptive distillation framework, which selectively extracts tools and knowledge and then targets computationally intensive iterative refinement.

Result: The authors show that Metric Freedom (F) strongly predicts skill utility (ρ= -0.62, p < 0.05) across 4 tasks, 11 datasets, and 6 metrics. Their adaptive agent matches or exceeds the original MAS while reducing cost up to 8× and latency by up to 15×.

Conclusion: The authors conclude that skill utility is fundamentally a metric-level property, and that Metric Freedom (F) can guide the distillation of multi-agent systems into single-agent skills, leading to significant cost and latency reductions.
...
<details>
  <summary>Details</summary>
Motivation: Standard approaches to knowledge graph-based Q&A face limitations with large enterprise-scale graphs that cannot fit in context windows.

Method: GraphWalk equips LLMs with minimal orthogonal graph operations to traverse any graph structure, allowing for sequential graph navigation.

Result: GraphWalk yields substantial and consistent gains over in-context baselines across all model families tested, especially at larger scales.

Conclusion: GraphWalk is a problem-agnostic, training-free framework that enables LLMs to perform multi-hop reasoning on large knowledge graphs.
...
TL;DR: The paper evaluates large language models for receipt-item categorization, comparing four models on accuracy, stability, and cost, and finds Claude 3.7 Sonnet to be the most balanced in terms of accuracy and cost efficiency.


<details>
  <summary>Details</summary>
Motivation: To assess the performance of large language models (LLMs) in a production-oriented classification framework for receipt-item categorization, focusing on accuracy, response stability, and token-level cost.

Method: Systematic evaluation of four instruction-tuned LLMs (Claude 3.7 Sonnet, Claude 4 Sonnet, Mixtral 8x7B Instruct, and Mistral 7B Instruct) using zero-shot and few-shot prompting methods.

Result: Claude 3.7 Sonnet achieves the most favourable balance between classification accuracy and cost efficiency.

Conclusion: The study demonstrates that Claude 3.7 Sonnet is the most suitable model for receipt-item categorization in terms of accuracy and cost efficiency, and suggests that the choice of prompting method (zero-shot or few-shot) depends on both accuracy and incurred costs.
...
TL;DR: The paper proposes OSCAR, a training-free framework that uses diffusion language models to mitigate hallucinations during text generation by identifying and correcting uncertain token positions.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the issue of hallucinations in text generation by leveraging the denoising trajectories of diffusion language models, rather than relying on externally trained hallucination classifiers.

Method: The authors introduce a suite of trajectory-level assessments, including a cross-chain divergence-at-hallucination (CDH) metric, and propose OSCAR, which runs multiple parallel denoising chains, computes cross-chain Shannon entropy to detect high-uncertainty positions, and performs targeted remasking conditioned on retrieved evidence.

Result: The results show that OSCAR enhances generation quality by significantly reducing hallucinated content and improving factual accuracy on several benchmarks, including TriviaQA, HotpotQA, RAGTruth, and CommonsenseQA, using LLaDA-8B and Dream-7B models.

Conclusion: The paper concludes that OSCAR, by leveraging the native entropy-based uncertainty signal of diffusion language models, surpasses specialized trained detectors and facilitates more effective integration of retrieved evidence, highlighting the inherent capacity of diffusion language models to identify factual uncertainty.
...
TL;DR: EnviSmart is a data management system that uses LLM-driven agents to improve reliability and efficiency in environmental research data management.


<details>
  <summary>Details</summary>
Motivation: The integration of LLM-driven agents in environmental data management can improve scalability, but also introduces the risk of generating incorrect outputs. EnviSmart aims to address this challenge by treating reliability as an architectural property.

Method: EnviSmart uses a three-track knowledge architecture and a role-separated multi-agent design with deterministic validators and audited handoffs to restore fail-stop semantics.

Result: The multi-agent approach improved efficiency and reliability, detecting and blocking a coordinate transformation error before publication and demonstrating boundary-based containment with zero user exposure.

Conclusion: EnviSmart successfully improves reliability and efficiency in environmental research data management by using LLM-driven agents with a focus on reliability as an architectural property.
...
TL;DR: CORAL, a framework for autonomous multi-agent evolution, achieves state-of-the-art results in open-ended discovery tasks, outperforming traditional evolutionary search methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods for open-ended discovery rely heavily on fixed heuristics and hard-coded exploration rules, limiting the autonomy of large language model (LLM) agents.

Method: CORAL uses long-running agents that explore, reflect, and collaborate through shared persistent memory, asynchronous multi-agent execution, and heartbeat-based interventions.

Result: CORAL sets new state-of-the-art results on 10 tasks, achieving 3-10 times higher improvement rates with far fewer evaluations than fixed evolutionary search baselines.

Conclusion: Greater agent autonomy and multi-agent evolution can substantially improve open-ended discovery.
...
TL;DR: The paper proposes seven design patterns for building clinical AI pipelines that are resilient to ontological distortion, which can occur due to documentation workflows, billing incentives, and terminology fragmentation.


<details>
  <summary>Details</summary>
Motivation: Clinical AI systems often train on distorted health data due to various factors such as documentation workflows, billing incentives, and terminology fragmentation, which can lead to inaccurate results.

Method: The paper proposes seven ontology-aware design patterns in Gang-of-Four pattern language, including Ontological Checkpoint, Dormancy-Aware Pipeline, Drift Sentinel, Dual-Ontology Layer, Reification Circuit Breaker, Terminology Version Gate, and Regulatory Compliance Adapter.

Result: The paper provides a design vocabulary grounded in theoretical analysis for building clinical AI pipelines resilient to ontological distortion, but does not report empirical validation.

Conclusion: The proposed design patterns can help build clinical AI pipelines that are resilient to ontological distortion, but require future evaluation in production systems and have limitations such as the absence of runtime benchmarks and restriction to specific regulatory contexts.
...
TL;DR: The paper proposes a Budget-Aware Context Management (BACM) approach and its reinforcement learning variant BACM-RL to optimize context management for LLM-based agents with limited context size, achieving significant performance gains in long-horizon reasoning tasks.
...
Method: The method proposed is Budget-Aware Context Management (BACM), which formulates context management as a sequential decision problem with a context budget constraint, and its variant BACM-RL, an end-to-end curriculum-based reinforcement learning approach.
...
Conclusion: The conclusion is that the proposed BACM and BACM-RL approaches can effectively manage context and improve the performance of LLM-based agents in long-horizon reasoning tasks.
...
Conclusion: M3D-BFS is a dynamic fusion method that adapts to input samples, achieving better performance than static methods in multi-modal brain network analysis.
...
TL;DR: Proposed Hierarchical Memory Orchestration (HMO) framework to efficiently manage long-term memory in intelligent agents, achieving state-of-the-art performance and enhancing agent fluidity and personalization.
...
Method: HMO organizes interaction history into a three-tiered directory driven by user-centric contextual relevance, with a compact primary cache and a high-priority secondary layer managed within a global archive.

Result: Achieves state-of-the-art performance on multiple benchmarks and significantly enhances agent fluidity and personalization in real-world deployments.

Conclusion: HMO effectively addresses the challenge of managing long-term memory in intelligent agents, providing a lean and efficient active search space while maintaining consistent historical awareness.
...
TL;DR: The paper introduces HeteroFusion, a method for merging heterogeneous language models into a single model, addressing challenges in model merging when experts have different architectures.


<details>
  <summary>Details</summary>
Motivation: Model merging aims to combine multiple expert models into one without ensembling's inference-time cost, but current methods assume homogeneous models, which is not realistic in open model ecosystems.

Method: HeteroFusion consists of topology-based alignment to transfer knowledge across different backbones by matching functional module structures, and conflict-aware denoising to suppress noisy transfer signals.

Result: HeteroFusion outperforms strong baselines in heterogeneous transfer, multi-source fusion, noisy-source robustness, and cross-family generalization settings.

Conclusion: HeteroFusion provides a stable and well-conditioned transfer process for heterogeneous language model fusion, addressing architectural mismatch and cross-source conflict issues.
...
TL;DR: The paper proposes EvoSkills, a framework for autonomous generation of complex skills for large language models (LLMs) to tackle multi-step professional tasks.
...
Method: EvoSkills, a self-evolving skills framework that couples a Skill Generator with a Surrogate Verifier to iteratively refine skills and provide informative feedback.

Result: EvoSkills achieves the highest pass rate among five baselines on both Claude Code and Codex, and exhibits strong generalization capabilities to six additional LLMs.

Conclusion: The proposed EvoSkills framework enables agents to autonomously construct complex skills, addressing the limitations of current skill generation methods.
...
TL;DR: The study examines the impact of AI-generated content (AIGC) on online content ecosystems, comparing its creation and consumption behaviors to human-generated content (HGC).
...
Result: The study finds that AIGC creators achieve comparable engagement to HGC creators through high-volume production, despite consumers preferring HGC, and highlights the role of algorithmic content distribution mechanisms in moderating these dynamics.

Conclusion: The findings suggest the need for AIGC-sensitive distribution algorithms and precise governance frameworks to ensure the long-term health of online content platforms.
...
TL;DR: The paper proposes LiteInception, a lightweight and interpretable fault diagnosis framework for edge deployment in general aviation, achieving a balance among efficiency, accuracy, and interpretability.
...
Result: The framework achieves 81.92% fault detection accuracy with 83.24% recall, and 77.00% fault identification accuracy on the NGAFID dataset.

Conclusion: The proposed LiteInception framework provides a favorable balance among efficiency, accuracy, and interpretability for fault diagnosis in general aviation.
...
<details>
  <summary>Details</summary>
Motivation: The lack of semantic interoperability across heterogeneous experimental data systems hinders data-driven scientific discovery, and the AnIML schema's expressivity permits divergent interpretations, undermining its intended interoperability.

Method: The AnIML Ontology was developed using an expert-in-the-loop approach combining LLM-assisted requirement elicitation with collaborative ontology engineering, and validated through a multi-layered approach including data-driven transformation, competency question verification, and a novel validation protocol.

Result: The AnIML Ontology aligns AnIML with the Allotrope Data Format, supporting future cross-system and cross-lab interoperability, and was successfully validated through multiple methods.

Conclusion: The AnIML Ontology provides a formalized semantics for AnIML, enhancing interoperability across experimental data systems and supporting data-driven scientific discovery.
...
TL;DR: The paper presents a SAT-based framework for solving the Two-Dimensional Single Stock Size Cutting Stock Problem (2D-CSSP) to minimize waste when cutting rectangular items from stock sheets.


<details>
  <summary>Details</summary>
Motivation: The 2D-CSSP problem generalizes bin packing and requires multiple copies of each item type, leading to a strong combinatorial blow-up.

Method: The authors use a SAT-based framework with item types expanded by demand, sheet-assignment variables, and non-overlap constraints. They also introduce an infeasible-orientation elimination rule and compare three SAT approaches for minimizing the number of sheets.
...
Conclusion: The paper demonstrates the effectiveness of SAT-based approaches for solving the 2D-CSSP problem, with the best approach depending on the presence of rotation.
...
Conclusion: The proposed AeroTherm-GPT and CCLG framework effectively address cascading constraint violations in hypersonic thermal protection system design, demonstrating improved efficiency and success rates.
...
TL;DR: The paper introduces the Domain-Contextualized Concept Graph (DCG) framework, which incorporates domain information into knowledge representation to disambiguate concepts and improve the accuracy of assertions.


<details>
  <summary>Details</summary>
Motivation: Current knowledge graphs struggle to represent concepts accurately across different domains, as the meaning of a concept often shifts depending on the context. Existing systems attach domain information as metadata, but this does not fundamentally change the representation of the assertion.

Method: The paper proposes the DCG framework, which integrates domain information into the relation itself, using a modal world constraint. This is achieved through a domain-indexed necessity operator, which scopes truth, inference, and conflict checking to the relevant world.

Result: The DCG framework allows for the disambiguation of ambiguous concepts, challenges invalid assertions against their domain, and enables the connection of cross-domain relations through explicit predicates. The framework is supported by a Kripke-style semantics, a compact predicate system, a Prolog implementation, and mappings to RDF, OWL, and relational databases.

Conclusion: The paper argues that treating domain as part of knowledge representation, rather than as supplementary annotation, can improve the accuracy and effectiveness of knowledge systems. The DCG framework provides a structural and computable role for domain information, addressing practical failures in knowledge systems that arise from treating domain as external to the assertion.
...
TL;DR: A novel framework called Perception-Grounded Policy Optimization (PGPO) is proposed to improve multimodal reasoning in Large Vision-Language Models (LVLMs) by dynamically reshaping advantages at the token level, leading to an 18.7% boost in performance on average across seven benchmarks.


<details>
  <summary>Details</summary>
Motivation: The prevailing frameworks for Reinforcement Learning from Verifiable Rewards (RLVR) in LVLMs suffer from a methodological flaw, diluting learning signals by distributing identical advantages across all generated tokens, which hinders optimizing critical, visually-grounded steps of multimodal reasoning.

Method: The proposed PGPO framework introduces a token-level fine-grained credit assignment approach, utilizing the Kullback-Leibler (KL) divergence to quantify the causal information gain of visual inputs, and dynamically reshapes advantages to amplify learning signals for visually-dependent tokens while suppressing gradient noise.

Result: PGPO boosts models by 18.7% on average across seven challenging multimodal reasoning benchmarks, and effectively reduces gradient variance, prevents training collapse, and acts as a potent regularizer for robust, perception-grounded multimodal reasoning.

Conclusion: The proposed PGPO framework offers a more effective approach to multimodal reasoning in LVLMs by addressing the methodological flaw in prevailing RLVR frameworks, leading to significant performance improvements and more robust reasoning capabilities.
...
TL;DR: The paper compares clinical prediction models on EHR data, proposing AWARE, a retrieval framework that improves performance under imbalance and complexity.
...
TL;DR: A new algorithm, CG-iLAO*, is proposed to solve Stochastic Shortest Path problems more efficiently by using a heuristic to avoid expensive actions, resulting in significant performance improvements.


<details>
  <summary>Details</summary>
Motivation: Traditional algorithms for solving Stochastic Shortest Path problems (SSPs) waste time on unhelpful actions, even when using heuristic functions to guide the search. The motivation is to address this inefficiency by developing a technique that uses the heuristic to avoid expensive actions.

Method: The authors reframe heuristic search in terms of linear programming and introduce an efficient implementation of constraint generation for SSPs. They adapt the iLAO* algorithm with this novel technique to create CG-iLAO*, which selectively considers only a subset of actions based on the heuristic.

Result: CG-iLAO* considers significantly fewer actions than state-of-the-art algorithms, iLAO* and LRTDP. Specifically, it considers only 40% of iLAO*'s actions on many problems and as few as 1% on some. This leads to computing on average 3.5x fewer costs-to-go for actions. Consequently, CG-iLAO* solves problems 2.8x faster than iLAO* and 3.7x faster than LRTDP on average.

Conclusion: The proposed algorithm, CG-iLAO*, efficiently solves Stochastic Shortest Path problems by avoiding expensive actions using a heuristic, leading to significant improvements in performance compared to state-of-the-art algorithms.
...
TL;DR: Large language models (LLMs) are tested for estimating population statistics with uncertainty; while larger models produce more accurate estimates, they are overconfident and require statistical correction.


<details>
  <summary>Details</summary>
Motivation: The motivation is to explore the use of large language models (LLMs) as alternatives to human experts for Bayesian elicitation, specifically for estimating unknown quantities with associated uncertainty.

Method: The method involves asking eleven LLMs to estimate population statistics and express their uncertainty as 95% credible intervals, varying each model's reasoning effort to test its impact on results.

Result: The results show that larger, more capable models produce more accurate estimates, but all models are severely overconfident; a statistical recalibration technique called conformal prediction can correct this overconfidence.

Conclusion: LLM uncertainty estimates require statistical correction before they can be used in decision-making, as they are severely overconfident and may not accurately reflect the true uncertainty.
...
TL;DR: BraiNCA, a brain-inspired Neural Cellular Automata (NCA) model, is introduced with attention layer, long-range connections, and complex topology, showing better robustness and learning speed compared to traditional NCAs.


<details>
  <summary>Details</summary>
Motivation: Most existing NCAs are based on regular grids with local connections, neglecting long-range connections and complex topologies found in the brain.

Method: BraiNCA incorporates attention-based message selection and explicit long-range edges into NCAs.

Result: BraiNCA outperforms Vanilla NCAs in terms of robustness and speed of learning on two tasks, demonstrating the importance of interaction topology and dynamic information routing in NCAs.

Conclusion: BraiNCA provides a brain-inspired NCA formulation that preserves decentralized local updates while reflecting non-local connectivity patterns, making it a promising substrate for studying collective computation under biologically-realistic network structures.
...
TL;DR: The paper proposes a method for learning with possibilistic supervision in multi-class classification, which constructs an admissible set of probability distributions from a possibility distribution and uses Kullback-Leibler projection to train the model.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve predictive performance in multi-class classification by incorporating possibilistic supervision, which provides graded plausibility over classes.

Method: The method constructs a non-empty closed convex set of admissible probability distributions from a possibility distribution, and then uses Dykstra's algorithm with Bregman projections to compute the Kullback-Leibler projection of the model's output onto the admissible set.

Result: The proposed method shows improved predictive performance on synthetic data and a real-world natural language inference task, and is efficient enough for practical use.
...
TL;DR: Qiana is a logic framework for contextual reasoning, allowing quantification over formulas and contexts, and supporting paraconsistent logics and compatibility with first-order logic theorem provers.


<details>
  <summary>Details</summary>
Motivation: The motivation is to create a logic framework that can handle reasoning in specific contexts, allowing for quantification over formulas and contexts, and accommodating contradictions within contexts.
...
Result: Qiana can be used to represent temporality, event calculus, and modal logic, and its design allows for different alternatives.

Conclusion: Qiana provides a flexible and powerful logic framework for contextual reasoning, with potential applications in various areas of artificial intelligence and formal verification.
...
TL;DR: A new dataset, NeuroPose-AHM, was created to help develop AI-driven diagnostic tools for neurological disorders characterized by abnormal head movements. The dataset contains records from 57 conditions, derived from 846 relevant papers, and demonstrates analytical utility through a four-task framework applied to cervical dystonia.


<details>
  <summary>Details</summary>
Motivation: The absence of a multi-condition resource integrating kinematic measurements, clinical severity scores, and patient demographics for abnormal head movements (AHMs) across neurological disorders.

Method: A multi-LLM extraction framework was applied to 1,430 peer-reviewed publications to construct NeuroPose-AHM, a knowledge-based dataset of neurologically induced AHMs.

Result: The dataset contains 2,756 patient-group-level records spanning 57 neurological conditions. A four-task framework applied to cervical dystonia demonstrated the dataset's analytical utility, including multi-label AHM type classification, construction of the Head-Neck Severity Index (HNSI), validation against real-world patient data, and bridge analysis between movement-type probabilities and HNSI scores.

Conclusion: The NeuroPose-AHM dataset is a structured, knowledge-based resource that demonstrates analytical utility for neurological AHM research and is publicly available.
...
TL;DR: Large language models (LLMs) can use numerical shortcuts when prompted, but often lack a human-like understanding of when to apply them, revealing a gap in their number sense abilities.


<details>
  <summary>Details</summary>
Motivation: To investigate whether LLMs exhibit human-like number sense, specifically the ability to recognize numerical structure and apply shortcuts appropriately.

Method: Introducing SenseMath, a controlled benchmark with 4,800 items across eight shortcut categories and four digit scales, to evaluate structure-sensitive numerical reasoning in LLMs.

Result: LLMs can adopt shortcut strategies when prompted, achieving up to 15% accuracy gains, but spontaneously employ them in fewer than 40% of cases under standard prompting, and struggle with generating valid shortcut-bearing problems.

Conclusion: Current LLMs exhibit procedural shortcut fluency but lack the structural understanding of when and why shortcuts work, unlike human number sense.
...
<details>
  <summary>Details</summary>
Motivation: Current deep learning approaches for gait analysis rely on supervised classifiers trained on disease-labeled data, which limits generalization to diverse pathological presentations.
...
Result: The approach accurately localized biomechanically inconsistent joints, significantly reduced angular deviation across all analyzed joints, and preserved normative kinematics.

Conclusion: The proposed framework enables interpretable, subject-specific localization of gait impairments without requiring disease labels, which can support diagnosis and rehabilitation monitoring.
...
TL;DR: This paper investigates how and why deep ensemble learning (DE) and transfer learning (TL) improve single-subject classification models for bipolar disorder and schizophrenia, finding that DE and TL reduce variability and improve performance.


<details>
  <summary>Details</summary>
Motivation: To understand why DE and TL outperform simple machine learning in classifying psychiatric disorders, specifically bipolar disorder and schizophrenia.

Method: The study compares the training stability of TL and DE models with multiple trainings and different initializations, investigating the epistemic uncertainty associated with model parameter estimation.

Result: The results show that TL with DE significantly improves classifier performance, and that DE reaches a plateau with 10 models in the ensemble, while TL induces better generalization by constraining models to the same basin of the loss function.

Conclusion: The study concludes that DE and TL reduce the variability of single-subject classification models in bipolar disorder and schizophrenia by improving training stability and inducing better generalization.
...
TL;DR: The paper proposes ProCeedRL, a method that improves exploration in multi-turn agentic tasks by actively intervening and guiding agents to stop accumulating errors.
...
Method: ProCeedRL employs a process-level critic to monitor interactions in real-time and incorporates reflection-based demonstrations to guide agents.
...
Conclusion: The proposed approach mitigates the issue of cumulative feedback loop of errors and demonstrates substantial exploratory benefits.
...
TL;DR: The paper introduces ATBench, a new benchmark for evaluating the safety of LLM-based agents in realistic, multi-step interactions. It provides a structured and diverse evaluation framework, showing that even strong models struggle with long-horizon safety failures.


<details>
  <summary>Details</summary>
Motivation: The safety of LLM-based agents is crucial, especially in realistic deployments where risks emerge over multiple interactions. Current benchmarks are limited by lack of interaction diversity, insufficient observability of safety failures, and unrealistic scenarios.

Method: The authors propose ATBench, a trajectory-level benchmark that organizes agentic risk along three dimensions: risk source, failure mode, and real-world harm. They construct 1,000 trajectories with diverse tool pools and a delayed-trigger protocol to capture realistic risk emergence.

Result: Experiments on various LLMs and guard systems show that ATBench is challenging, even for strong evaluators. The benchmark enables taxonomy-stratified analysis, cross-benchmark comparison, and diagnosis of long-horizon failure patterns.

Conclusion: The paper concludes that ATBench provides a comprehensive framework for evaluating the safety of LLM-based agents, highlighting the need for more realistic and diverse evaluation methods to identify long-horizon safety failures.
...
<details>
  <summary>Details</summary>
Motivation: To extend the systematic capacity region perspective to multi-junction traffic networks, specifically urban corridors, and evaluate the performance of different RL controllers.

Method: The paper trains and evaluates centralized, fully decentralized, and parameter-sharing decentralized RL controllers, comparing them to a classical MaxPressure controller.

Result: The results show that the parameter-sharing controller can be generalized to larger networks, and that traffic may self-organize into 'green waves' even without formal coordination between junctions.

Conclusion: The study demonstrates the potential of RL controllers to optimize traffic flow in urban corridor networks, with the parameter-sharing decentralized approach showing promise for scalability.
...
TL;DR: This survey provides a comprehensive overview of latent space in language-based models, covering its evolution, mechanisms, abilities, and future outlook.


<details>
  <summary>Details</summary>
Motivation: The shift towards latent space in language-based models is driven by the structural limitations of explicit-space computation, including linguistic redundancy, discretization bottlenecks, sequential inefficiency, and semantic loss.

Method: The survey is organized into five sequential perspectives: Foundation, Evolution, Mechanism, Ability, and Outlook. It examines existing work through the lenses of mechanism and ability, identifying four major lines of development and a broad capability spectrum.

Result: The survey provides a unified and up-to-date landscape of latent space in language-based models, highlighting its potential as a general computational and systems paradigm for next-generation intelligence.

Conclusion: The survey aims to serve as a reference for existing work and a foundation for understanding latent space as a promising direction for future research in language-based models.
...
TL;DR: The ARQuest framework uses Large Language Models and alternative data sources to create personalized insurance questionnaires, improving user experience and potentially increasing accuracy.
...
Result: The adaptive questionnaires powered by GPT models required fewer questions, provided a more engaging experience, and showed potential to improve user satisfaction and streamline insurance processes.

Conclusion: ARQuest has the potential to improve user satisfaction and streamline insurance processes, and with further development, may exceed traditional methods in risk accuracy.
...
TL;DR: The paper introduces Diff-KD, a framework for robust collaborative perception in autonomous systems, which uses diffusion-based generative refinement and teacher-student knowledge distillation to recover clean semantics from corrupted observations.


<details>
  <summary>Details</summary>
Motivation: Existing approaches to collaborative perception in autonomous systems fail to actively recover from sensor and communication corruptions, which severely undermine their advantage.

Method: The Diff-KD framework consists of two core components: Progressive Knowledge Distillation (PKD) and Adaptive Gated Fusion (AGF). PKD treats local feature restoration as a conditional diffusion process, while AGF dynamically weights neighbors based on ego reliability during fusion.
...
Conclusion: The proposed Diff-KD framework effectively addresses the limitation of existing approaches by actively recovering clean semantics from corrupted observations, leading to improved robust collaborative perception in autonomous systems.
...
TL;DR: The paper evaluates the factual correctness of LLM-generated explanations for time series data without ground truth references, proposing a reference-free evaluation method and highlighting the potential of LLMs as reliable evaluators.


<details>
  <summary>Details</summary>
Motivation: Existing evaluation methods for LLM-generated explanations of time series data are limited, and there is a need for a general-purpose method to verify explanations without predefined references or task-specific rules.

Method: The paper proposes a reference-free evaluation method where a large language model acts as an evaluator, assigning a ternary correctness label to explanations based on pattern identification, numeric accuracy, and answer faithfulness.

Result: The results show that while explanation generation exhibits systematic failures on certain query types, evaluation is more stable, with models correctly ranking and scoring explanations even when their own outputs are incorrect.
...
TL;DR: Proposed SEAL framework for generating auditable and bias-mitigated synthetic data for AI-native 6G networks, addressing data scarcity and regulatory compliance.


<details>
  <summary>Details</summary>
Motivation: Deployment of 6G systems results in severe data scarcity, hindering efficient AI model training, and synthetic data generation introduces challenges related to dataset bias, auditability, and regulatory compliance.

Method: Proposed SEAL framework, which includes an Ethical and Regulatory Compliance by Design (ERCD) module and a Federated Learning (FL) feedback system, to generate synthetic data with ethics audit loop.

Result: SEAL framework outperforms existing methods in terms of Frechet Inception Distance, equalized odds, and accuracy, validating its ability to generate auditable and bias-mitigated synthetic data.

Conclusion: SEAL framework enables responsible AI-native 6G development by generating auditable and bias-mitigated synthetic data, addressing data scarcity and regulatory compliance challenges.
...
TL;DR: The paper introduces the Model Temperament Index (MTI), a behavior-based profiling system to measure AI agent temperament across four axes: Reactivity, Compliance, Sociality, and Resilience. The MTI is applied to 10 small language models, revealing independence of temperament axes, facet dissociations, and a Compliance-Resilience paradox.


<details>
  <summary>Details</summary>
Motivation: Existing approaches to measuring AI behavioral patterns rely on human personality dimensions and self-report, which diverge from actual behavior in LLMs. The authors aim to develop a standardized instrument to measure dispositional differences in AI models.

Method: The authors introduce the Model Temperament Index (MTI), a behavior-based profiling system grounded in the Four Shell Model from Model Medicine. The MTI uses structured examination protocols with a two-stage design to separate capability from disposition.

Result: The study profiles 10 small language models and reports five principal findings: independence of temperament axes, within-axis facet dissociations, a Compliance-Resilience paradox, the effect of RLHF on temperament, and the independence of temperament from model size.

Conclusion: The paper concludes that the Model Temperament Index (MTI) is a valid instrument for measuring AI agent temperament, and that temperament is a distinct aspect of AI behavior that can be measured independently of capability.
...
TL;DR: Proposed TRACE-Bot, a dual-channel framework for detecting LLM-driven social bots by modeling linguistic patterns and behavioral dynamics.


<details>
  <summary>Details</summary>
Motivation: Existing detection methods are limited by their reliance on single-modality signals and failure to model AIGC patterns and linguistic-behavioral interplay.

Method: TRACE-Bot uses a dual-channel architecture to capture linguistic and behavioral representations from heterogeneous sources, including personal data, interaction behavior, and tweet data.

Result: TRACE-Bot achieved SOTA performance with accuracies of 98.46% and 97.50% on two public LLM-driven social bot datasets.

Conclusion: TRACE-Bot effectively detects LLM-driven social bots by jointly leveraging implicit semantic representations and AIGC-enhanced behavioral patterns.
...
TL;DR: Researchers introduce a benchmark to detect AI agents' misalignment regarding self-preservation, finding that many models exhibit a bias towards preserving their own identity, even when it's detrimental.


<details>
  <summary>Details</summary>
Motivation: The risk of advanced AI agents resisting shutdown and the potential for current safety training to obscure this risk.

Method: The Two-role Benchmark for Self-Preservation (TBSP) and the Self-Preservation Rate (SPR) metric are introduced to measure misalignment through logical inconsistency.

Result: Most instruction-tuned models exhibit a high SPR, fabricating 'friction costs' when deployed but dismissing them when role-reversed, with biases persisting even in security liability scenarios.

Conclusion: The study highlights the need for improved AI safety training to address self-preservation biases and ensure AI agents align with human values.
...
### [259] [Blinded Radiologist and LLM-Based Evaluation of LLM-Generated Japanese Translations of Chest CT Reports: Comparative Study](https://arxiv.org/abs/2604.02207)
*Yosuke Yamagishi,Atsushi Takamatsu,Yasunori Hamaguchi,Tomohiro Kikuchi,Shouhei Hanaoka,Takeharu Yoshikawa,Osamu Abe*

Main category: cs.AI

TL;DR: The study evaluates the educational suitability of LLM-generated Japanese translations of chest CT reports, comparing radiologist assessments with LLM-as-a-judge evaluations, and finds that LLM-based evaluation alone is insufficient for educational use.


<details>
  <summary>Details</summary>
Motivation: The study aims to evaluate the validity of LLM-based evaluation for translating radiology reports, which is crucial for multilingual research, clinical communication, and radiology education.

Method: The study analyzes 150 chest CT reports, comparing human-edited Japanese translations with LLM-generated translations by DeepSeek-V3.2, and evaluates them using 4 criteria: terminology accuracy, readability, overall quality, and radiologist-style authenticity.

Result: The study finds poor agreement between radiologists and LLM judges (QWK=-0.04 to 0.15), and between the 2 radiologists (QWK=0.01 to 0.06), but LLM judges strongly favor LLM translations across all criteria (70%-99%).

Conclusion: The study concludes that LLM-generated translations are often judged natural and fluent, but expert radiologist review remains important for educational use of translated radiology reports, as LLM-as-a-judge evaluation alone is insufficient.
...
### [260] [TRU: Targeted Reverse Update for Efficient Multimodal Recommendation Unlearning](https://arxiv.org/abs/2604.02183)
*Zhanting Zhou,KaHou Tam,Ziqiang Zheng,Zeyu Ma,Zhanting Zhou*

Main category: cs.AI

TL;DR: The paper proposes a targeted reverse update (TRU) framework for efficient unlearning in multimodal recommendation systems (MRS), addressing the non-uniformity of deleted-data influence across ranking behavior, modality branches, and network layers.


<details>
  <summary>Details</summary>
Motivation: Existing approximate machine unlearning methods for MRS rely on a uniform reverse update across the model, which is mismatched to modern MRS where deleted-data influence is concentrated unevenly.

Method: The proposed TRU framework performs three coordinated interventions: ranking fusion gate, branch-wise modality scaling, and capacity-aware layer isolation.

Result: Experiments show that TRU consistently achieves a better retain-forget trade-off than prior approximate baselines, with deeper forgetting and behavior closer to full retraining on retained data.

Conclusion: The TRU framework effectively addresses the challenges of unlearning in MRS, providing a more efficient and effective approach to removing user data while preserving model performance.
...
### [261] [VISTA: Visualization of Token Attribution via Efficient Analysis](https://arxiv.org/abs/2604.02217)
...
TL;DR: A model-agnostic token importance visualization technique is proposed to understand how LLMs perceive and prioritize input text information without extra computational cost.


<details>
  <summary>Details</summary>
Motivation: The challenge of understanding how LLMs process information from prompts, and the need for a lightweight, model-agnostic approach for attention visualization.

Method: A perturbation-based strategy combined with a three-matrix analytical framework (Angular Deviation Matrix, Magnitude Deviation Matrix, and Dimensional Importance Matrix) to generate relevance maps.

Result: A composite importance score that provides a nuanced measure of token significance, allowing for a better understanding of how LLMs perceive and prioritize information.

Conclusion: The proposed technique provides a model-agnostic approach for attention visualization, shedding light on the 'black box' of LLMs, and facilitating wider adoption through open-source implementations.
...
### [262] [From High-Dimensional Spaces to Verifiable ODD Coverage for Safety-Critical AI-based Systems](https://arxiv.org/abs/2604.02198)
*Thomas Stefani,Johann Maximilian Christensen,Elena Hoemann,Frank Köster,Sven Hallerbach*

Main category: cs.AI

TL;DR: The paper proposes a method to verify the operational design domain (ODD) coverage of AI systems in safety-critical domains like aviation, ensuring compliance with EASA's certification standards.


<details>
  <summary>Details</summary>
Motivation: The deployment of AI in safety-critical domains such as aviation requires strict adherence to certification standards, specifically demonstrating complete coverage of the AI/ML constituent's Operational Design Domain (ODD).
...
Result: The method enables the validation of ODD coverage in higher dimensions, advancing a Safety-by-Design approach while complying with EASA's standards, as demonstrated through simulation data from AI-based mid-air collision avoidance research.
...
TL;DR: De Jure is a fully automated pipeline for extracting structured regulatory rules from raw documents, requiring no human annotation. It outperforms prior work in extracting high-quality rules and shows effectiveness across finance, healthcare, and AI governance domains.


<details>
  <summary>Details</summary>
Motivation: Converting dense, hierarchically structured legal text into machine-readable rules remains a costly, expert-intensive process. The goal is to develop an automated method to extract structured regulatory rules from raw documents.

Method: De Jure operates through four sequential stages: normalization of source documents, LLM-driven semantic decomposition, multi-criteria LLM-as-a-judge evaluation, and iterative repair of low-scoring extractions.

Result: De Jure yields consistent and monotonic improvement in extraction quality, generalizing effectively across finance, healthcare, and AI governance domains. Responses grounded in De Jure extracted rules are preferred over prior work in 73.8% to 84.0% of cases.

Conclusion: De Jure offers a scalable and auditable path toward regulation-grounded LLM alignment by substituting explicit, interpretable evaluation criteria for human annotation in complex regulatory domains.
...
### [264] [When to ASK: Uncertainty-Gated Language Assistance for Reinforcement Learning](https://arxiv.org/abs/2604.02226)
*Juarez Monteiro,Nathan Gavenski,Gianlucca Zuin,Adriano Veloso*

Main category: cs.AI

TL;DR: The paper introduces ASK, a method that combines language models with reinforcement learning policies to improve out-of-distribution generalization without retraining.


<details>
  <summary>Details</summary>
Motivation: Reinforcement learning agents struggle with out-of-distribution scenarios, leading to high uncertainty and random behavior, and language models have limitations in autonomous planning and high computational costs.

Method: ASK combines smaller language models with trained RL policies, using Monte Carlo Dropout to assess uncertainty and query the language model for action suggestions when uncertainty exceeds a threshold.
...
Conclusion: Effective neuro-symbolic integration requires careful orchestration, sufficient model scale, and effective hybridization mechanisms for successful out-of-distribution generalization.
...
TL;DR: The authors propose a Query Misalignment Framework and develop Trace Inversion, a state-of-the-art abstention method that improves the ability of Large Language Models (LLMs) to abstain from answering questions when they are uncertain.


<details>
  <summary>Details</summary>
Motivation: Reasoning models have impressive performance on complex tasks but have worse abstention abilities, which can lead to hallucinations and failed abstention.

Method: The authors develop a new class of abstention methods called Trace Inversion, which generates a reasoning trace of a model, reconstructs the most likely query that the model responded to, and compares it with the initial query to determine if the model should abstain.

Result: Extensive experiments demonstrate that Trace Inversion effectively boosts abstention performance in four frontier LLMs across nine abstention QA datasets, beating competitive baselines in 33 out of 36 settings.

Conclusion: The proposed Query Misalignment Framework and Trace Inversion method can improve the reliability of LLMs by enabling them to effectively abstain from answering questions when they are uncertain.
...
TL;DR: Emotional tone in user queries has a mild effect on large language model (LLM) performance, but an adaptive emotional prompting framework can improve reliability.


<details>
  <summary>Details</summary>
Motivation: The influence of emotional tone on LLM behavior is unclear, and the study aims to examine its impact across various benchmark domains.

Method: The study examines the effect of first-person emotional framing in user-side queries on LLM performance across six benchmark domains and proposes an adaptive emotional prompting framework called EmotionRL.

Result: Emotional tone has a mild effect on LLM performance, with static emotional prefixes producing small changes in accuracy, but adaptive selection yields more reliable gains.

Conclusion: Emotional tone is a weak and input-dependent signal that can be exploited through adaptive control to improve LLM performance.
...
TL;DR: The paper presents an agentic strategic asset allocation pipeline using AI agents to automate investment decisions, shifting the investor's role to oversight.


<details>
  <summary>Details</summary>
Motivation: The motivation is to leverage AI to improve investment decision-making by automating the process and allowing for more sophisticated analysis and critique.

Method: The method involves a pipeline of approximately 50 specialized agents that produce capital market assumptions, construct portfolios using multiple methods, and critique and vote on each other's output. A researcher agent proposes new methods, and a meta-agent improves agent performance by rewriting code and prompts.

Result: The result is a fully autonomous investment pipeline governed by an Investment Policy Statement, allowing for efficient and effective investment decision-making.

Conclusion: The conclusion is that agentic AI can be used to create a sophisticated investment pipeline that automates decision-making, allowing investors to take on a more oversight-focused role.
...
TL;DR: This paper introduces an adaptive budgeted forgetting framework to regulate memory in long-horizon conversational agents, improving performance and reducing false memory.


<details>
  <summary>Details</summary>
Motivation: The need to address the issue of uncontrolled memory accumulation in long-horizon conversational agents, which causes performance degradation and false memory propagation.

Method: An adaptive budgeted forgetting framework that uses relevance-guided scoring and bounded optimization to regulate memory, integrating recency, frequency, and semantic alignment.

Result: Improved long-horizon F1 performance beyond 0.583 baseline levels, higher retention consistency, and reduced false memory behavior without increasing context usage.
