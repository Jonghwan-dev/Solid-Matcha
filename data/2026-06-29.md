...
TL;DR: The paper proposes DocArena, a data curation pipeline for training search agents on multimodal document search and question-answering tasks without human annotation.


<details>
  <summary>Details</summary>
Motivation: Existing training environments for search agents are predominantly text-based, struggle to construct controllable, scalable, and multimodal data, and require expert trajectories or human annotation.

Method: The authors propose a fully automated data curation pipeline, DocArena, which transforms raw document collections into training environments for search agents using MLLM-based visual perception and cross-page information distribution.
...
Conclusion: The proposed DocArena pipeline and Doc-Search agent infrastructure effectively construct a controllable and scalable training environment for search agents, leading to improved performance on multimodal document search and question-answering tasks.
...
TL;DR: A hybrid approach combining image processing and deep learning was developed to assess fruit freshness, achieving over 90% accuracy in real-time.
...
Method: A hybrid approach combining image processing algorithm, convolutional neural network (CNN), and logistic regression was used to assess fruit freshness.

Result: The approach achieved over 90% accuracy in real-time on a dataset comprising apples and oranges, with the limitation of requiring a white or transparent background.
...
TL;DR: Proposed Long-Context Generation (LCG) framework for consistent multi-image text-to-image generation.


<details>
  <summary>Details</summary>
Motivation: Current image generation models struggle with consistency across sequential outputs, crucial for applications like comics and storyboards.

Method: LCG uses Sparse Relational Attention (SRA) and Routing Consistency Constraint (RCC) to ensure consistency and scalability.

Result: LCG outperforms baselines in prompt alignment and character consistency for long-context image generation.

Conclusion: LCG framework improves consistency and scalability in long-context multi-image generation, with applications in comics, storyboards, and visual narratives.
...
TL;DR: The study presents a framework for estimating above-ground biomass (AGB) of urban trees at a crown level using LiDAR and orthophotography data, achieving high accuracy and providing a public bitemporal AGB database.


<details>
  <summary>Details</summary>
Motivation: Urban tree biomass is less spatially explicitly quantified than biomass in managed forests due to reliance on inventories or coarse products that cannot resolve individual crowns or fine-scale heterogeneity.

Method: The framework uses leaf-off airborne LiDAR and near-infrared RGB orthophotography, a dual-stream cross-attention network for semantic segmentation, and multiscale watershed segmentation for crown delineation. AGB is estimated from a crown area-height power-law proxy calibrated to species-specific allometry.

Result: The framework achieves high accuracy with global/mean precision, recall, and Dice scores of 0.86, 0.83, and 0.84, respectively. AGB prediction achieves R^2=0.609 using inventory crown geometry and R^2=0.570 under operational segmentation. The total AGB stocks are estimated to be 1.73 Tg in 2018 and 1.81 Tg in 2023.

Conclusion: The study provides a novel framework for estimating urban tree biomass at a crown level, producing a public bitemporal AGB database for trees outside forests at a management-relevant resolution, and highlighting areas of high epistemic uncertainty.
...
<details>
  <summary>Details</summary>
Motivation: Existing reconstruction-based taskonomies focus on one-to-one transfer, but this study aims to explore multi-source transfer and budget-constrained task allocation.
...
Result: The findings reveal a cross-paradigm-limited motor cluster and working-memory states with high priority under the specified global allocation objective, and demonstrate that multi-source transfer depends on the composition of the source set.

Conclusion: This study extends reconstruction-based fMRI taskonomy from one-to-one transfer to many-to-one task relations and budget-constrained task dependencies, providing insights into the organization of cognitive tasks and neural processes.
...
TL;DR: The paper proposes a new dynamic routing method called GeMoE for Large Vision-Language Models (LVLMs) based on the Mixture of Experts (MoE) architecture. GeMoE uses gating entropy to adaptively determine the number of experts for each token, achieving better performance and resource utilization.
...
Method: The paper proposes Gating Entropy-based Uncertainty-aware Adaptive Routing (GeMoE) for MoE, which views token routing as an information encoding task and frames dynamic routing as a Minimum Description Length (MDL) problem in encoding.

Result: The method achieves 99.5% average performance retention compared to the original static routing, while improving average expert activation sparsity by 36.5% on a wide range of backbones and benchmarks.

Conclusion: The proposed GeMoE method offers a more efficient and effective way to utilize experts in MoE-based LVLMs, achieving better performance and resource utilization.
...
TL;DR: The paper introduces a new dataset and metric to evaluate the impact of turbidity on computer vision models in underwater environments.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the gap in understanding how computer vision models are affected by turbidity in underwater environments, which can lead to rapid degradation of visibility.

Method: The authors propose a new dataset, Turbid Underwater Baseline (TUB), and a metric, Phase Congruency-based Degradation (PCD), to evaluate the performance of instance segmentation models in turbid conditions.
...
Conclusion: The paper concludes that the proposed dataset and metric can be used to better evaluate and understand the impact of turbidity on computer vision models in underwater environments.
...
TL;DR: The paper studies the effect of different fusion schemes for visual prompt tuning in Vision Transformers (ViTs) and proposes a hybrid fusion approach that outperforms existing methods.


<details>
  <summary>Details</summary>
Motivation: The motivation is to investigate whether a single fusion scheme is optimal for visual prompt tuning and to explore the development of a hybrid fusion scheme.

Method: The method involves formulating the task as a bi-level optimization problem and solving it using differentiable architecture search. The learnable prompts and their fusion schemes are jointly optimized.

Result: The results show consistent gains over prompt-tuning baselines on 34 datasets, with a favorable accuracy-latency-parameter trade-off compared to VPT-Deep and recent variants.

Conclusion: The conclusion is that the fusion scheme plays a significant role in visual prompt tuning, and a hybrid fusion approach can effectively leverage layer semantics of ViTs, providing a novel perspective for visual prompt-tuning research.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to investigate what deepfake detection benchmarks are actually measuring, as detectors often fail in real-world scenarios.

Method: The authors use a simple diagnostic: a linear probe on frozen, general-purpose self-supervised representations, to approximate the performance of bespoke detectors.
...
Conclusion: The paper concludes that before high scores are read as evidence of forensic understanding, it is worth asking how much of the benchmark is already solved by general-purpose representations.
...
TL;DR: The paper proposes VIGIL, a post-training framework that improves multimodal large language models by focusing on visual grounding to reduce hallucinations and improve reasoning.


<details>
  <summary>Details</summary>
Motivation: Multimodal large language models (MLLMs) are prone to hallucinations that contradict their visual inputs due to over-reliance on language priors and lack of proper visual grounding.

Method: The authors propose Visual Information Gain In aLignment (VIGIL), a reinforcement-learning post-training framework that introduces a geometric constraint to maximize mutual information between visual input and generated response.

Result: VIGIL consistently outperforms recent alignment methods across hallucination and reasoning benchmarks without compromising text-only capabilities, and demonstrates emergent spatial grounding capabilities.

Conclusion: The proposed VIGIL framework effectively reduces hallucinations in MLLMs by emphasizing visual grounding, leading to improved reasoning capabilities and performance on various benchmarks.
...
TL;DR: Proposed DinoLink, a token-centric compression framework for vehicle-cloud collaborative inference, achieving 139x bitrate reduction and 34.5x acceleration in narrow-band environments.


<details>
  <summary>Details</summary>
Motivation: High-precision remote perception is hindered by severe bandwidth constraints of Vehicle-to-Everything (V2X) networks.
...
Result: DinoLink achieves a 139x bitrate reduction and maintains a competitive 32.8% mAP on the nuScenes dataset, with a 34.5x acceleration in narrow-band environments.
...
TL;DR: A self-supervised framework for learning implicit 3D physical dynamics from video-derived signals, achieving physical plausibility and long-term stability without explicit physics engines or labels.
...
<details>
  <summary>Details</summary>
Motivation: The accurate identification and segmentation of methane plumes in earth observation imagery is crucial for understanding and mitigating global warming.
...
Result: The proposed method outperforms state-of-the-art methods on the MPDataset with improvements of +0.92 in MIoU, +0.87 in MPrecision, and +1.01 in Recall, while having a substantially lower computational cost.

Conclusion: The results highlight the potential of efficient multimodal fusion strategies for accurate and scalable methane plume segmentation in real-world remote sensing applications.
...
TL;DR: The paper presents APRTrack, a hierarchical perturbation and retrieval framework for RGB-Event tracking that improves robustness against partial target missing and modal degradation.


<details>
  <summary>Details</summary>
Motivation: Traditional RGB-Event tracking methods struggle with real-world signal corruptions, such as modal degradation and partial target missing due to occlusion, edge truncation, and foreground clutter.

Method: The APRTrack framework uses two adversarial perturbation branches to simulate full-modal failure and localized target region absence. It also employs a hierarchical routing mechanism and Footprint-guided Channel-calibrated Hopfield Retrieval (FCHR) for reliable historical information compensation.
...
Conclusion: The APRTrack framework improves the robustness of RGB-Event tracking against partial target missing and modal degradation, and achieves state-of-the-art performance on multiple datasets.
...
TL;DR: This paper presents a Frame Forgetting Network (FFN) for Test Time Training (TTT) on long videos, addressing computational issues with existing approaches.


<details>
  <summary>Details</summary>
Motivation: Existing TTT approaches are computationally expensive for long videos, either due to large window sizes or unnecessary updates for similar frames.
...
Result: The FFN demonstrates empirical effectiveness on various tasks, including dense-segmentation, video classification, depth-estimation, and multi-hour long videos.

Conclusion: The proposed FFN enables efficient TTT on long videos, reducing computational costs while maintaining performance.
...
TL;DR: A new evaluation method called CRISP is proposed to assess visual spatial intelligence by testing consistency and alignment between implicit perception and explicit reasoning.


<details>
  <summary>Details</summary>
Motivation: Current evaluations of Visual Language Models (VLMs) often confuse language priors with genuine spatial reasoning abilities, and the authors aim to address this issue.

Method: The CRISP evaluation paradigm uses metric 3D Scene Graphs and an oracle intervention protocol to decouple latent reasoning capabilities from perceptual bottlenecks.

Result: The study finds that proprietary models have robust latent reasoning engines but struggle with inaccurate metric estimation, while open-source models are limited by their lack of multi-hop compositional reasoning.

Conclusion: CRISP provides a rigorous roadmap for multimodal alignment beyond end-to-end post-training, shifting the focus from 'guessing correctly' via language priors to genuinely 'perceiving, verifying, and reasoning'.
...
TL;DR: A new benchmark, PhyEditBench, is introduced to evaluate the physical understanding of image editing models, revealing limitations in current state-of-the-art methods and proposing a training-free baseline, PhyWorld.
...
Method: PhyEditBench, a benchmark with 4 primary classes and 12 subclasses, is established to assess physical understanding, and a training-free baseline, PhyWorld, is proposed using test-time scaling and latent reduction strategy.

Result: Empirical analysis shows substantial limitations in physics-based reasoning of current state-of-the-art editing methods, while PhyWorld outperforms comparable models.

Conclusion: PhyEditBench effectively evaluates physics-based reasoning in image editing models, and PhyWorld suggests that video generation can serve as a reasoning mechanism for image editing.
...
<details>
  <summary>Details</summary>
Motivation: Existing deepfake detection methods are challenged by the rapid advancement of generative models. Current approaches using Multimodal Large Language Models (MLLMs) have limitations, including insufficient sensitivity to fine-grained forensic artifacts and reliance on static synthetic supervision.
...
Result: ForeAgent achieves state-of-the-art performance on the Chameleon benchmark (82.18% accuracy) and AIGCDetect-Benchmark (93.3% mean accuracy). It also produces more consistent and causally grounded reasoning compared to GPT-5 and GPT-5-mini.

Conclusion: ForeAgent is an effective and innovative framework for AI-generated image detection, addressing the limitations of existing approaches and achieving superior performance.
...
TL;DR: A hybrid self-supervised method for non-rigid 3D shape matching is proposed, achieving state-of-the-art performance with computational efficiency.
...
Result: The proposed approach achieves state-of-the-art performance across various challenging scenarios, including non-isometric deformations and topological noise.

Conclusion: The proposed method maintains computational efficiency and achieves state-of-the-art performance, with contrastive energies promoting feature discrimination.
...
TL;DR: SpaceRipple is a framework for semantic delivery and on-board processing in Earth observation satellite networks, enabling efficient and reliable Earth observation under constrained resources.


<details>
  <summary>Details</summary>
Motivation: The limited inter-satellite and downlink resources in Earth observation satellite networks make it difficult to transmit high-resolution imagery, and ground users require mission-relevant semantic information rather than full raw images.
...
Method: The authors propose DiCoBench, a comprehensive benchmark consisting of 765 samples, categorized into two tracks, with 8 distinct perception tasks, using high-resolution imagery (approaching 2K).

Result: The evaluation of 18 diverse MLLMs reveals a significant performance gap compared to human accuracy (98.3%), with top-performing models struggling with micro-scale detail capture.

Conclusion: DiCoBench will serve as a challenging testbed to drive future research in autonomous, high-resolution multi-image perception.
...
Method: LogicIR, a UNet-inspired architecture composed of logic gates, with a differentiable bit decoding layer and index shuffling mechanism.
...
TL;DR: TaskTok selectively restores task-relevant tokens in images to enhance performance in high-level vision tasks while improving computational efficiency.


<details>
  <summary>Details</summary>
Motivation: Traditional image restoration focuses on perceptual quality, but Task-Driven Image Restoration (TDIR) aims to maximize performance in downstream high-level vision tasks. Recent generative prior approaches are computationally inefficient and may alter semantics.

Method: TaskTok uses a learnable token switch and a lightweight token refinement module to selectively restore task-relevant tokens, identified through analysis of the latent token space.

Result: TaskTok significantly enhances task performance in image classification, semantic segmentation, and object detection with high computational efficiency.

Conclusion: TaskTok offers a novel approach to TDIR by selectively refining task-relevant tokens, achieving better performance and efficiency in high-level vision tasks.
...
TL;DR: The paper proposes a new method called Position Rebinding Cache Reuse (PRCR) for efficient multimodal reasoning, which improves visual grounding and reduces computation.


<details>
  <summary>Details</summary>
Motivation: Existing methods for multimodal reasoning rely on token replay, which can be inefficient. The authors aim to improve visual grounding and reduce computation by reusing historical visual evidence.

Method: The authors propose PRCR, a cache-level framework that stores raw visual KV cache together with their original spatial coordinates, then reassigns position-compatible coordinates to select entries and rebinds their keys.

Result: Experiments show that PRCR achieves replay-level or better performance, improving average accuracy by 5 percent and reducing visual-revisiting computation by up to tens of thousands of times.

Conclusion: The proposed PRCR method effectively reuses historical visual evidence while preserving textual positional continuity and relative visual structure, leading to improved multimodal reasoning performance and efficiency.
...
TL;DR: The paper proposes a flow-guided label interpolation framework called FAROS to address the annotation granularity mismatch in surgical scene understanding. FAROS generates temporally consistent dense pseudo labels from sparse keyframe annotations, enabling balanced optimization across heterogeneous surgical tasks.


<details>
  <summary>Details</summary>
Motivation: The annotation granularity mismatch between temporal workflow tasks and pixel-level spatial tasks in surgical scene understanding hinders effective multi-task learning. Temporal workflow tasks benefit from dense frame-level supervision, while pixel-level spatial tasks are only sparsely annotated on selected keyframes due to high labeling costs.

Method: The authors propose FAROS, a flow-guided label interpolation framework that combines zero-shot segmentation-based mask propagation with optical flow estimation to generate temporally consistent dense pseudo labels from sparse keyframe annotations. These pseudo labels are then integrated into a unified Transformer-based multi-task framework.

Result: The proposed FAROS framework significantly improves cross-task representation learning and enhances holistic surgical scene understanding performance across spatio-temporal tasks on benchmarks such as GraSP, MISAW, and AutoLaparo.

Conclusion: The paper concludes that FAROS effectively addresses the annotation granularity mismatch in surgical scene understanding, enabling balanced optimization across heterogeneous surgical tasks and improving performance in surgical scene understanding.
...
TL;DR: FracEvent, a new event simulator, models pixel-level lifecycle with fractional-relaxation voltage dynamics, improving temporal structure and downstream transfer in event-camera simulation.


<details>
  <summary>Details</summary>
Motivation: Real event data collection is challenging due to specialized sensors, synchronization, and annotations required, making event-camera simulation crucial for event-based vision tasks.

Method: FracEvent simulates event generation by modeling pixel lifecycle with fractional-relaxation voltage dynamics, driving a compact stack of relaxation modes, and emitting ON/OFF events based on threshold crossings.

Result: FracEvent improves temporal structure of generated events and achieves stronger downstream-transfer results than competing simulators in image reconstruction and optical flow estimation across multiple datasets.

Conclusion: FracEvent demonstrates practical value for event-camera simulation, offering enhanced temporal structure and improved downstream transfer compared to existing simulators.
...
Method: LayersReg uses a regression paradigm that searches for correlations between moving and fixed images in feature space, capturing pixel flow trends and converging iteratively to the correct pose transformation.
...
Conclusion: LayersReg offers improved accuracy, real-time capability, and robustness in 3D/2D registration tasks, making it suitable for intraoperative settings.
...
TL;DR: Disco-LoRA is a unified framework for multi-concept video customization, enabling control of content, style, and motion in text-to-video generation.


<details>
  <summary>Details</summary>
Motivation: The challenge of simultaneously controlling multiple concepts, such as content, style, and motion, in text-to-video models.

Method: Disco-LoRA, a two-stage framework that disentangles and recombines concepts using Iterative Dual-LoRA Disentanglement Framework and Z-score-based statistical regularization.

Result: Disco-LoRA excels in multi-concept video customization, preserving appearance, style, and motion for controllable text-to-video generation.

Conclusion: The proposed Disco-LoRA framework effectively addresses the challenge of multi-concept video customization, facilitating controllable text-to-video generation.
...
TL;DR: DeCoFlow is proposed for continual anomaly detection in industrial environments, preserving strict invertibility and achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: The need for continual anomaly detection in industrial environments without access to past data, while addressing catastrophic forgetting in Normalizing Flows (NFs).
...
Conclusion: DeCoFlow effectively addresses catastrophic forgetting in NFs for continual anomaly detection, maintaining performance and validity.
...
TL;DR: PhysEditWorld, a multimodal dataset, is introduced for controllable world modeling research, focusing on physical parameters like gravity, enabling explicit manipulation of physical dynamics.


<details>
  <summary>Details</summary>
Motivation: Current game world models lack controllable physical dynamics and are limited to exploratory trajectories, hindering their applicability to authored game environments with deliberate physical rules.

Method: PhysEditWorld is built using a replay paradigm with a UE5 replay-and-rendering pipeline, recording scenarios under multiple gravity configurations, and providing synchronized multimodal signals.
...
Conclusion: PhysEditWorld offers a novel approach to controllable world modeling by providing a dataset with explicit physical parameters, which can improve the applicability of game world models to authored game environments.
...
TL;DR: A multi-task framework for detecting intracranial aneurysms, classifying their anatomical location, and segmenting aneurysms and vessels across multiple imaging modalities.


<details>
  <summary>Details</summary>
Motivation: Current methods for assessing rupture risk and treatment planning of intracranial aneurysms are limited to binary detection and lack fine-grained anatomical classification or multi-class segmentation.

Method: A two-stage approach combining 2D tri-axial ROI extraction with a 3D multi-task nnU-Net backbone, utilizing a dual-decoder design, cross-attention pooling, and modality-specific auxiliary heads.

Result: The proposed framework achieved 2nd place in the RSNA 2025 Intracranial Aneurysm Detection challenge.

Conclusion: The presented framework enables simultaneous multi-label classification, multi-class aneurysm segmentation, and multi-class vessel segmentation, improving the assessment and treatment planning of intracranial aneurysms.
...
TL;DR: The paper proposes Mask to Concept (M2C), a framework that adapts SAM3 for efficient medical few-shot annotation without external modules or manual text engineering, achieving state-of-the-art performance and annotation efficiency.


<details>
  <summary>Details</summary>
Motivation: Current medical data annotation methods depend on external feature matchers or auxiliary networks, introducing architectural overhead and limiting performance scalability. SAM3's direct use in medical imaging is hindered by a lack of fine-grained clinical knowledge and ambiguity of human-written descriptions.

Method: The M2C framework uses a learnable concept embedding to prompt segmentation, updates the embedding by minimizing concept segmentation error, and introduces a Hybrid Uncertainty Estimation (HUE) module to measure concept-geometry prompting inconsistency and flag uncertain samples for human correction.
...
Conclusion: The proposed M2C framework offers a practical and efficient pathway toward scalable medical image labeling, enabling few-shot annotation without external modules, parameter retraining, or manual text engineering.
...
TL;DR: NeuMatEx is a method for extracting neural materials from images using a differentiable inverse rendering approach.


<details>
  <summary>Details</summary>
Motivation: Acquiring and authoring neural materials remains challenging, and existing methods are limited by naive inverse rendering approaches.

Method: NeuMatEx uses a Large Material Reconstruction Model (LMRM) to predict initial base color, neural material latents, and uncertainty guides from images, followed by inverse path tracing optimization.

Result: NeuMatEx extracts complex materials with better visual quality and material decomposition than PBR-based methods, as shown in experiments on synthetic and real assets.

Conclusion: NeuMatEx presents a novel approach for extracting spatially varying neural materials from images, addressing the challenges of nonlinear neural material latent spaces and improving material decomposition and visual quality.
...
TL;DR: The paper proposes a method called FUSE to generate unlearnable examples that protect training data by injecting perturbations, making them effective across the full frequency spectrum.
...
Method: The authors propose Full-spectrum Unlearnable examples via Spectral Equalization (FUSE), which uses Random Spectral Masking (RSM) and Cross-Band Guidance (CBG) to generate spectrum-agnostic perturbations.
...
Conclusion: The proposed FUSE method effectively generates unlearnable examples that protect training data by injecting perturbations across the full frequency spectrum.
...
<details>
  <summary>Details</summary>
Motivation: To assess whether generative image editing models possess intrinsic knowledge of real-world lighting, as existing benchmarks have limitations in evaluating light transport.

Method: The authors introduce the 3D-anchored Light Probe (3DLP) benchmark, a high-fidelity HDR dataset of real-world lighting changes, and evaluate state-of-the-art image editing models using two new scores.

Result: The best image editing models show consistency with real-world physics, but leave room for improvement, especially in image regions with less light from the light probe.

Conclusion: The paper concludes that while state-of-the-art image editing models have impressive performance, they still have limitations in understanding real-world lighting, and the 3DLP benchmark can facilitate future research.
...
<details>
  <summary>Details</summary>
Motivation: The need for a streaming video editing framework that can maintain stable backgrounds and non-edited regions over time while achieving low latency for real-time interactive scenarios.

Method: A three-stage distillation pipeline that transfers editing capability from a powerful bidirectional foundation model to an efficient unidirectional streaming editor, along with an AR-oriented mask cache.
...
Conclusion: The proposed framework enables stable long-horizon edits without sacrificing visual fidelity, making it suitable for interactive and augmented reality applications.
...
Conclusion: The proposed framework provides a principled mechanism for balancing feature coverage and stylistic diversity, and demonstrates improved multi-view stylization results.
...
TL;DR: The paper proposes ProtoKV, a memory method for streaming video understanding that improves accuracy under strict GPU-memory and query-time latency budgets.


<details>
  <summary>Details</summary>
Motivation: The main challenge addressed is the 'delayed query' problem, where crucial information may appear briefly but is at risk of being lost or diluted due to memory constraints and asynchronous query arrivals.

Method: ProtoKV uses a constant-footprint memory approach, combining an exact near-window KV cache with a semantic-spatial prototype bank that aggregates older content and residual statistics.

Result: ProtoKV improves accuracy by up to 12.5 points over token-retention baselines on SVU benchmarks, especially in scenarios with long query delays.

Conclusion: The proposed ProtoKV method effectively addresses the challenges of streaming video understanding under strict resource constraints, offering improved accuracy without increasing costs.
...
TL;DR: A novel Implicit Neural Representation (INR) method called CHOIR is proposed, which utilizes Coordinated Harmonic Superposition and Perceptual Spectrum Calibration to improve optimization stability and alleviate spectrum bias.


<details>
  <summary>Details</summary>
Motivation: Current INR methods relying on periodic activation functions and function composition suffer from optimization instability and spectrum bias, which limits their performance.

Method: The proposed CHOIR method uses Coordinated Harmonic Superposition to replace conventional function composition and Perceptual Spectrum Calibration to adjust the spectrum towards a physically plausible log-uniform distribution.

Result: The proposed method achieves superior performance over state-of-the-art approaches on various multidimensional data recovery problems.

Conclusion: The proposed CHOIR method effectively addresses the issues of optimization instability and spectrum bias in INR methods, leading to improved performance on multidimensional data recovery tasks.
...
TL;DR: A 4D controllable generative framework for medical image synthesis is proposed, addressing limited annotated data, inter-device domain shifts, and privacy restrictions.
...
Method: A semi-supervised variational autoencoder and cascaded latent diffusion model are used to generate anatomically consistent 4D medical images.

Result: The proposed approach demonstrates high controllability of static anatomy (Pearson r > 0.8) and strong temporal coherence (FVD = 288.08), improving downstream segmentation performance.
...
<details>
  <summary>Details</summary>
Motivation: Diffusion Transformers (DiTs) are computationally expensive, and existing feature caching methods suffer from error accumulation at high acceleration ratios.
...
Result: LearniBridge achieves up to 5.87x, 5.75x, and 4.10x acceleration on FLUX, HunyuanVideo, and WAN2.1, respectively, and improves VBench by 1.28% over the previous SOTA.

Conclusion: LearniBridge effectively accelerates Diffusion Transformers (DiTs) with minimal training samples, achieving significant speedup and quality improvement.
...
TL;DR: Proposed an event-based active vision system to track and estimate spin of balls in real-time, achieving high accuracy and low latency.


<details>
  <summary>Details</summary>
Motivation: Vision-based estimation of ball spin in sports is challenging due to small size, high speed, and fast rotation of the ball.

Method: Event-based active vision system with event camera, high-speed mirrors, and focus-tunable lens; hybrid approach for ball tracking; offline and online methods for spin estimation.

Result: Achieved state-of-the-art accuracy on static balls (2.1% magnitude, 4.0 degrees axis error); reliable tracking and spin estimation in professional table tennis matches (8.8% magnitude, 6.4 degrees axis mismatch, 3 ms latency, 750 Hz throughput).

Conclusion: The proposed system enables accurate and real-time estimation of ball spin in sports, with potential applications in professional matches and sports analysis.
...
TL;DR: The authors propose ReasonCLIP, a continual pretraining framework that enhances CLIP-style models with large-scale reasoning supervision, improving visually grounded commonsense and compositional reasoning.


<details>
  <summary>Details</summary>
Motivation: The pretraining of CLIP-style models is dominated by descriptive image-text alignment, which may not be sufficient for downstream applications requiring visually grounded commonsense inference and compositional reasoning.

Method: The authors present a two-stage strategy for integrating large-scale reasoning supervision into CLIP-style models, along with the construction of two datasets (ReasonLite-42M and ReasonPro-16M) and a benchmark (RCLIP-Bench).

Result: The trained ReasonCLIP models show improved visually grounded commonsense and compositional reasoning, as well as enhanced zero-shot retrieval performance, and can be used as a drop-in visual encoder for multimodal large language models.

Conclusion: The authors demonstrate that structured reasoning supervision can enhance the expressive capacity of CLIP-style visual representations, and that ReasonCLIP can deliver consistent gains without additional inference cost.
...
Method: NaviCache uses a dual-state estimation architecture to track feature change ratio and its latent drift, and integrates a time-dependent noise schedule with an uncertainty-aware Measurement Update mechanism.

Result: NaviCache exhibits more accurate error judgment for computation skipping and achieves outstanding comprehensive performance on HunyuanVideo, Wan, and Open-Sora series.
...
TL;DR: The authors propose a mask-guided multi-modality image fusion (MMIF) method that integrates feature restoration and interaction, outperforming state-of-the-art approaches in various experiments.
...
Method: The proposed method introduces 'Pseudo Ground Truth' for simplified training, a mask generation mechanism, and a mask-guided cross-modal cross-attention mechanism to selectively attend to informative features.

Result: The method surpasses state-of-the-art approaches in visual quality, quantitative metrics, and downstream tasks, as demonstrated by extensive experiments on synthetic and real-world datasets.

Conclusion: The proposed mask-guided MMIF method effectively addresses the challenges of image degradation and feature representation under adverse weather conditions, leading to improved performance in various applications.
...
<details>
  <summary>Details</summary>
Motivation: The need for open vocabulary object detectors that are prompt-free and efficient for continuous deployment in robotic systems operating in real-world environments.

Method: A two-stage framework, AnomNOVIC, combining a masked autoencoder (MAE) for anomaly detection and a real-time prompt-free open vocabulary image classifier (NOVIC).

Result: AnomNOVIC achieves 47.1% AP / 57.5% AP50 for prompt-free recognition and 59.0% AP / 72.5% AP50 with class candidates provided, surpassing open vocabulary baselines.

Conclusion: AnomNOVIC demonstrates superior performance in open vocabulary object detection, especially in prompt-free settings, making it suitable for continuous deployment in robotic systems.
...
TL;DR: The paper proposes a Liquid Fusion Network (LFNet) that combines the strengths of State Space Models (SSMs) and Convolutional Neural Networks (CNNs) for general salient object detection, achieving state-of-the-art performance across multiple tasks.


<details>
  <summary>Details</summary>
Motivation: Current approaches to salient object detection neglect the inherent spectral biases exhibited by different neural network paradigms, and the authors aim to bridge this gap by harmonizing heterogeneous representations from SSMs and CNNs.

Method: The authors introduce a liquid fusion approach that dynamically integrates features from two backbones, VMamba and ConvNeXt, using a dynamic gating mechanism, and propose a Saliency-Guided Upsampling (SGU) operator to propagate features to shallow layers.
...
Conclusion: The paper concludes that the proposed LFNet can effectively bridge the spectral biases of SSMs and CNNs, leading to improved performance in general salient object detection tasks.
...
TL;DR: The paper introduces a novel method for rolling shutter (RS) relative pose estimation using affine correspondences, reducing the required point correspondences from 20 to 7.


<details>
  <summary>Details</summary>
Motivation: The current state-of-the-art RS-aware relative pose estimation method requires a minimum of 20 point correspondences, making it impractical due to computational expense.

Method: The proposed method uses affine correspondences to derive RS-corrected affine constraints and develops a linearized algebraic solver to estimate pose and RS motion.

Result: The method achieves the best pose and RS parameter accuracy on the TUM RS benchmark and comparable accuracy to the standard 5-point algorithm on the EuRoC MAV dataset.

Conclusion: The proposed method makes RS relative pose estimation practical by reducing the required point correspondences and achieving accurate results on various benchmarks.
...
TL;DR: The paper proposes SpatialFlow-GRPO, a framework for fine-grained image editing optimization using spatially aware rewards, outperforming existing methods.
...
Result: SpatialFlow-GRPO outperforms Flow-GRPO on GEdit-Bench, ImgEdit-Bench, and MultiEditBench, improving editing quality by converting local feedback into spatially aligned update signals.

Conclusion: The proposed SpatialFlow-GRPO framework effectively addresses the spatial uniformity assumption in image editing, leading to improved editing quality and fine-grained optimization.
...
TL;DR: The paper proposes a novel approach for reconfigurable intelligent surface (RIS)-assisted proactive handover (PHO) in millimeter-wave networks to mitigate line-of-sight blockages, optimizing RIS element allocation for energy efficiency and link quality.


<details>
  <summary>Details</summary>
Motivation: Millimeter-wave networks are susceptible to line-of-sight blockages, and proactive handover (PHO) is challenging when no nearby base station is available. Reconfigurable intelligent surfaces (RIS) can restore connectivity but require optimization to ensure timely PHO.

Method: The paper uses particle swarm optimization (PSO) to formulate an optimization problem for determining the optimal end-to-end RIS link setup offline, balancing signal processing complexity and link quality under handover timing constraints.
...
Conclusion: The proposed RIS-assisted PHO approach optimizes RIS element allocation for energy efficiency and link quality, demonstrating improved performance in millimeter-wave networks with line-of-sight blockages.
...
TL;DR: This paper proposes a new concept bottleneck model called OTF-CBM, which uses optimal transport to align visual and textual representations for more accurate and interpretable cross-modal reasoning.
...
Method: The proposed OTF-CBM uses Inverse Optimal Transport to learn a data-driven semantic cost and then performs unbalanced optimal-transport-based flow matching to model semantic transitions between visual patches and textual concepts.

Result: OTF-CBM achieves superior classification accuracy and concept faithfulness compared to existing models.

Conclusion: The proposed OTF-CBM offers a new geometric and dynamical perspective for interpretable cross-modal reasoning, capturing interpretable geometric relations without ODE integration.
...
TL;DR: The authors propose MICViT, a 3D vision transformer for multimodal brain MRI analysis, which outperforms state-of-the-art baselines and benefits from multimodal inputs.


<details>
  <summary>Details</summary>
Motivation: The challenge of analyzing high-dimensional 3D brain MRI data with limited sample sizes and diverse anatomy, pathology, and acquisition conditions.

Method: MICViT, a 3D vision transformer that models modality-specific representations and cross-modal interactions using four attention mechanisms.

Result: MICViT consistently outperforms state-of-the-art CNN and transformer baselines in 3D settings, with larger performance gains from multimodal inputs.
...
TL;DR: The paper proposes Robust-TO, a video understanding framework that assesses the trustworthiness of each frame in a video and integrates this information into the reasoning process, improving accuracy and robustness under various perturbations.


<details>
  <summary>Details</summary>
Motivation: The Blind Trust Problem, where video reasoning models assume all input frames are equally reliable, leading to significant accuracy drops under realistic perturbations such as motion blur, glare, or occlusion.
...
Result: Robust-TO achieves 56.4% average accuracy on clean inputs and 54.3% under five realistic corruption types, outperforming baselines and exhibiting the smallest clean-to-corrupted accuracy drop.

Conclusion: The proposed Robust-TO framework effectively addresses the Blind Trust Problem, providing a more robust and accurate video understanding approach under various perturbations.
...
TL;DR: The paper proposes Qwen-Image-Agent, a unified framework that enhances text-to-image models by bridging the 'Context Gap' between user input and required generation context.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of text-to-image models struggling with underspecified, implicit, or knowledge-dependent real-world requests.

Method: The method involves a context-centric agentic framework that integrates planning, reasoning, searching, memory, and feedback to progressively construct the generation context.

Result: The proposed Qwen-Image-Agent achieves state-of-the-art performance on benchmarks such as IA-Bench, Mindbench, and WISE-Verified.

Conclusion: The paper concludes that Qwen-Image-Agent effectively bridges the Context Gap, enabling more accurate and context-aware image generation.
...
<details>
  <summary>Details</summary>
Motivation: Developing physically aware video generation models is challenging due to the difficulty in capturing diverse physical phenomena.

Method: PhysRAG uses Retrieval-Augmented Generation (RAG) and a two-stage data filtering pipeline to enhance physical awareness in video generation.

Result: PhysRAG achieves state-of-the-art performance in visual quality and physical rule compliance, surpassing existing models in benchmarks such as PhyGenBench and VBench.

Conclusion: PhysRAG is an effective method for enhancing physical awareness in video generation, and its code, data, and models are made available for future research.
...
TL;DR: The paper proposes PortraitGen, a novel framework for photorealistic portrait generation, which addresses limitations in current text-to-image post-training methods by introducing real images and a dual-reward mechanism.


<details>
  <summary>Details</summary>
Motivation: Current methods favor superficial aesthetics over critical flaws like AI artifacts and biological implausibilities, mainly due to the absence of real images during post-training and lack of specific rewards for fine-grained artifacts.

Method: PortraitGen introduces real images into the GRPO sampling groups using image inversion and employs a dual-reward mechanism: OmniReward for general quality and AI-Portrait for human-centric fidelity.

Result: Extensive experiments show that PortraitGen significantly outperforms existing baselines, effectively suppressing AI artifacts and achieving unprecedented photorealism.

Conclusion: PortraitGen is a novel framework that advances photorealistic portrait generation by addressing inherent limitations in current text-to-image post-training methods, leading to significant improvements in suppressing AI artifacts and achieving photorealism.
...
TL;DR: The authors propose SharpMoE, a post-training framework that improves the routing mechanism in Mixture-of-Experts (MoE) architectures for diffusion models, leading to state-of-the-art performance in visual generation.


<details>
  <summary>Details</summary>
Motivation: The authors identify a routing assignment problem in existing diffusion MoE frameworks, where the router fails to accurately allocate computational resources to salient tokens due to noise-corrupted latent features.

Method: The authors propose SharpMoE, which utilizes clean latent features as a noise-free guidance signal for routing, and introduce a trajectory routing loss to constrain compute allocation throughout the denoising trajectory.

Result: Extensive experiments demonstrate that SharpMoE enhances pretrained MoE models, achieving state-of-the-art performance in visual generation.

Conclusion: SharpMoE serves as a versatile, plug-and-play solution that improves the efficiency and performance of MoE architectures in visual generation tasks.
...
TL;DR: Proposed TraMP-LLaMA, a multimodal framework for facial expression quality assessment that predicts severity scores and generates textual reports.


<details>
  <summary>Details</summary>
Motivation: Existing FEQA methods lack interpretability as they only provide severity scores without supporting evidence, making it hard to inspect model outputs in Parkinson's disease assessment.

Method: TraMP-LLaMA integrates RGB and landmark trajectory cues, using a decoupled instruction-tuning strategy to reduce task interference.

Result: TraMP-LLaMA outperforms baselines in report generation and severity prediction, improving Spearman's rank correlation by at least 4.39%.

Conclusion: TraMP-LLaMA enhances interpretability and performance in FEQA, supporting Parkinson's disease assessment with structured textual reports.
...
TL;DR: The paper introduces OmniRef-Bench, a benchmark for multi-reference image generation (MRIG), and proposes DyRef, a two-stage training framework that improves model performance on complex MRIG tasks.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for MRIG are inadequate, and mainstream open-source models struggle with complex MRIG scenarios, particularly with a large number of mixed-type reference images.
...
Result: Experiments show that DyRef significantly improves the performance of open-source models on OmniRef-Bench and single-image editing benchmarks.

Conclusion: The proposed DyRef framework effectively addresses the challenges of complex MRIG tasks and demonstrates strong generalization capability.
...
<details>
  <summary>Details</summary>
Motivation: Most MOBA analytics studies rely on structured data, which doesn't capture what each team can see during a match.

Method: Created a dataset of 288 Full HD videos from 144 professional Dota 2 matches and 2,477 annotated minimap images. Evaluated multiple object detectors for player-icon detection.

Result: YOLO11l (large) achieved the best performance in player-icon detection, revealing player, hero, role, and team-level patterns that complement conventional MOBA analytics.

Conclusion: The Dota2-Vis dataset and code provide a new resource for visibility analysis in professional Dota 2 matches, highlighting behavioral differences that are difficult to obtain from structured data alone.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for open-set semi-supervised learning face a trade-off between filtering out suspicious samples and utilizing unlabeled data, which can lead to conflicting auxiliary gradients. The authors aim to shift the focus to gradient-level control to address this issue.

Method: The authors propose Geometric Gradient Rectification (GGR), a plug-in framework that projects conflicting auxiliary gradients onto an admissible region in gradient space, ensuring the applied auxiliary update is first-order non-opposing within the rectified coordinate block.
...
Conclusion: The proposed GGR framework provides a novel approach to open-set semi-supervised learning by controlling auxiliary gradients, resulting in improved performance on in-distribution classes and robustness to out-of-distribution outliers.
...
Method: The authors introduce Unison, a benchmark with 2,169 task samples, and Unison-Judge, an evaluation model aligned with human judgments. Unison offers comprehensive dimensions, diagnostic evaluation, and human alignment.

Result: The authors evaluated state-of-the-art models on Unison, uncovering critical limitations in current unified multimodal systems and highlighting promising directions for future research.

Conclusion: Unison provides a comprehensive and diagnostic evaluation of unified multimodal models, revealing limitations in current systems and guiding future research. The benchmark and evaluation model are publicly available.
...
TL;DR: The paper proposes EVIS, an event-aware video segmentation model that decomposes videos into simple events using learnable Event Queries, enabling better understanding of complex video content.
...
Result: EVIS demonstrates strong performance on 5 public benchmarks for the referring video segmentation task.

Conclusion: The paper concludes that EVIS effectively addresses the referring video segmentation task by decomposing videos into simple events and utilizing event-aware visual-text features.
...
TL;DR: This paper presents a novel unsupervised anomaly detection method using Remote Sensing Foundation Models (RSFMs) and a ResNet+FPN architecture to identify semantic shifts in satellite images without requiring expensive labels or training.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to maximize the utility of satellite missions' limited power and computational resources by enabling autonomous anomaly detection and high-resolution image capture, while also reducing the need for expensive labels and bespoke training.

Method: The method proposed in this paper uses a ResNet-based RSFM combined with a Feature Pyramid Network (FPN) to detect subtle semantic shifts in the latent space between successive orbital passes, achieving efficient image-level generation and higher resolution mapping without requiring training.

Result: The results show that the proposed method can achieve comparable performance to tailored models without requiring bespoke training or extensive development effort, while ensuring high-performance generalization across diverse terrains and sensors.

Conclusion: The conclusion of this paper is that the proposed unsupervised detection method based on RSFMs and ResNet+FPN can effectively identify a wide spectrum of anomalies in satellite images, offering a transformative path for disaster monitoring and reducing the need for expensive labels and customized models.
...
TL;DR: PanoImager is a framework for 3D reconstruction from sparse panoramic images, addressing challenges in rotation-dominant, weak-parallax motion.
...
Result: Improved stability in 3D reconstruction under extreme sparsity, outperforming SfM/SLAM initialization.

Conclusion: PanoImager offers a reliable offline/background component for map refinement when traditional SfM/SLAM methods fail.
...
TL;DR: Proposed CT-3GDINO, a lightweight 3D detector for organ localization in abdominal CT scans, achieving strong performance in coarse localization.
...
Method: CT-3GDINO uses a Grounding-DINO-style query-based architecture with frozen pseudo-text class tokens, Swin3D visual backbone, and cross-modality decoder.

Result: The best multi-scale model achieves 0.5830 overall top-1 class-wise mAP over 3D IoU thresholds from 0.1 to 0.7.

Conclusion: CT-3GDINO establishes an open-source baseline for pseudo-text-conditioned 3D organ localization, motivating future work on localization-aware pretraining and richer multimodal conditioning.
...
TL;DR: SubdivAR is a neural mesh subdivision framework that uses autoregressive prediction to generate high-resolution surfaces from coarse meshes, outperforming state-of-the-art methods.
...
Result: SubdivAR reduces Hausdorff Distance and Chamfer Distance by 18.8% and 14.2%, respectively, and shows strong robustness on complex open-surface geometries.

Conclusion: SubdivAR is a effective neural mesh subdivision framework that preserves subdivision topology while recovering fine-grained geometric details.
...
TL;DR: The paper proposes TMP, a Tree-structured Mixed-policy Pruning framework for compressing large image generation models, and demonstrates its effectiveness on two models, achieving significant parameter reduction with minimal quality loss.


<details>
  <summary>Details</summary>
Motivation: The rapid growth of image generation models leads to unaffordable parameter consumption and computation budget, resulting in massive resource requirements and GPU memory footprint.

Method: The proposed TMP framework generalizes prevalent image tasks and architectures, and can be applied to step-distilled models as the last stage.

Result: The framework compresses HunyuanImage 3.0 from 80B to 20B parameters (75% reduction) with limited generation quality loss, and enables inference on a single 24GB 4090 GPU. It also compresses Z-Image turbo from 6B to 4B (33% reduction) with negligible degradation.

Conclusion: The TMP framework effectively compresses large image generation models while maintaining generation quality, making them more affordable and efficient.
...
TL;DR: The paper introduces FlameVQA, a benchmark for UAV-based wildfire monitoring using visual question answering, leveraging RGB and thermal images for safety-critical reasoning.
...
Method: The authors introduce FlameVQA, a multiple-choice visual question answering benchmark built on FLAME 3, using paired RGB and radiometric thermal TIFFs, and evaluate representative MLLMs on FlameVQA.
...
TL;DR: The paper proposes a method for safe autoregressive generation of images in unified multimodal models by iteratively self-improving codebooks to eliminate harmful mappings.
...
Method: The method involves two steps: (1) using the unified model to identify unsafe generations and construct harmful and safe image-text pairs to guide updates to the codebook, and (2) performing adaptive fine-tuning on the codebook within the harmless space using safe image-text pairs.

Result: The result is a safety-enhanced model codebook that iteratively improves without external feedback.

Conclusion: The paper concludes that the proposed method can effectively improve the safety of generated images in autoregressive unified multimodal models.
...
TL;DR: The paper introduces HarmVideoBench, a new benchmark for evaluating large vision-language models' understanding of harmful videos. It addresses limitations in existing works by considering multi-layered characteristics and providing explanatory rationales.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for harmful videos have limitations, including overlooking multi-layered characteristics and lacking explanatory rationales. The authors aim to address these issues.

Method: The authors present HarmVideoBench, a diagnostic benchmark with 1,379 videos and 4,137 multiple-choice questions, evaluating models on three hierarchical dimensions: Observable Evidence, Clip-Internal Meaning, and Beyond-Clip Reasoning. They also introduce BCR, a method that predicts reasoning boundaries and dynamically retrieves context.

Result: Experimental results show that BCR improves the base model's performance in harmful video understanding, raising the macro average from 61.7% to 84.4%.

Conclusion: The paper concludes that HarmVideoBench and BCR can effectively evaluate and improve large vision-language models' understanding of harmful videos, addressing the limitations in existing works.
...
TL;DR: The paper proposes LIkelihood Score Alignment (LISA), a regularization method that improves the training efficiency and effectiveness of the dual-branch paradigm in visual-condition controllable generation.


<details>
  <summary>Details</summary>
Motivation: The role of the side branch and its training efficiency in the dual-branch paradigm remain underexplored, and the authors aim to address this gap.

Method: The authors propose LISA, which aligns the intermediate feature of the side network with an approximated likelihood score, and jointly optimizes the side network and decoder with both standard diffusion loss and the regularization loss.

Result: Experiments demonstrate that LISA can consistently accelerate training convergence, improve final synthetic results, and encourage the side network's features to be more disentangled for conditional modeling with negligible additional training cost and zero extra inference cost.

Conclusion: The proposed LISA method provides an effective and efficient way to improve the performance of the dual-branch paradigm in visual-condition controllable generation.
...
TL;DR: SatSplatDiff is proposed to minimize geometric degradation in satellite 3D reconstruction by introducing monocular depth supervision, multi-scale geometric refinement, and shadow-guided generative refinement.


<details>
  <summary>Details</summary>
Motivation: The limited top viewpoint of satellite imagery results in insufficient supervision on building facades, leading to surface holes and degraded visual fidelity in Gaussian Splatting-based reconstruction.

Method: SatSplatDiff builds on photogrammetric DSM initialization and 2DGS-based shadow casting, and applies monocular depth supervision, multi-scale geometric refinement, and shadow-guided generative refinement to establish a geometrically accurate surface representation.

Result: Extensive evaluations demonstrate state-of-the-art performance, reducing geometric MAE by up to 18% and improving visual fidelity (FID-CLIP) by 28-45% over existing baselines.

Conclusion: SatSplatDiff delivers up to 5x resolution enhancement with minimal hallucination and sensor-consistent appearance, demonstrating seamless cross-tile consistency and strong scalability for large-scale reconstruction.
...
TL;DR: CORTEX is a new benchmark for 3D chest CT reasoning, providing structured diagnostic traces and evaluation protocols for training and verifying multimodal large language models.


<details>
  <summary>Details</summary>
Motivation: Current multimodal large language models lack interpretability and verifiability in their reasoning, particularly in 3D radiology, due to missing structured supervision and verification protocols.

Method: CORTEX uses a four-stage diagnostic trace (task understanding, visual observation, diagnostic reasoning, and answer synthesis) generated by large language models and verified by clinicians.

Result: CORTEX comprises 76,177 validated reasoning traces across open-ended VQA, closed-ended VQA, and report generation, providing a publicly available dataset and evaluation code.

Conclusion: CORTEX enables the development of trustworthy reasoning models for 3D chest CT by providing structured supervision and evaluation protocols, facilitating more accurate and interpretable medical imaging analysis.
...
TL;DR: A patch descriptor retrieval method that achieves exact nearest neighbor search while evaluating only a small fraction of the database, with deterministic results.


<details>
  <summary>Details</summary>
Motivation: To improve the efficiency of nearest neighbor search in large databases while maintaining exactness and determinism.

Method: Hierarchical Normalization (HN) scheme, which splits the feature vector into major and minor components, allowing for a provably exact branch-and-bound scan.

Result: The method achieves 13.7x and 12.7x speed-up over brute-force search on trevi and halfdome datasets, respectively, with only 0.4% of entries requiring full evaluation.

Conclusion: The proposed method enables efficient and exact nearest neighbor search with deterministic results, making it suitable for applications requiring high accuracy and reliability.
...
TL;DR: The paper presents a method for 3D generation using reinforcement learning from human feedback (RLHF) without requiring explicit surface representations or pretraining.


<details>
  <summary>Details</summary>
Motivation: Existing pipelines for 3D generation using RLHF rely on explicit surface representations and surface-supervised data, which can be limited. The authors aim to improve 3D geometry generation by directly fine-tuning a pretrained 3D-aware generative model using a learned reward over radiance-field density values.

Method: The authors fine-tune a pretrained 3D-aware generative model (EG3D) using a learned reward model that reads the continuous 3D density field of the neural radiance field (NeRF) directly. The reward model requires no pretraining and trains easily on a small set of preference samples.
...
Conclusion: The proposed method yields robust improvement in 3D geometry generation without requiring explicit surface representations or pretraining, and can produce high-quality face geometries with a small set of preference samples.
...
TL;DR: The paper introduces SmellNet-V, a visuo-olfactory dataset, and See & Sniff, a self-supervised framework for learning joint visuo-olfactory representations, achieving improved smell classification and localization.
...
Method: The authors propose a scalable visuo-olfactory dataset built by synthetically pairing smell-only samples with semantically aligned web images, and a self-supervised framework called See & Sniff for learning joint visuo-olfactory representations.
...
Conclusion: The paper establishes visuo-olfactory learning as a new direction in multimodal perception, enabling improved smell classification, localization, and cross-modal retrieval.
...
TL;DR: DanceOPD, a framework for training image generation models with multiple capabilities, including text-to-image, local editing, and global editing, is proposed.


<details>
  <summary>Details</summary>
Motivation: The need for a single image generation model that can unify diverse capabilities such as text-to-image, local editing, and global editing, which are often conflicting and difficult to compose.

Method: An on-policy generative field distillation framework for flow-matching models, called DanceOPD, which routes each sample to one capability field, queries one low-noise student-induced state, and trains with a simple velocity MSE objective.

Result: Comprehensive experiments show that DanceOPD improves multi-capability composition, strengthening target capabilities while preserving anchor generation quality.

Conclusion: DanceOPD establishes a practical route for generative field distillation in flow-matching models, enabling the unification of diverse image generation capabilities.
...
TL;DR: A new framework called ViQ is proposed for unified discrete representation of text and vision, balancing semantics and details while supporting native resolution inputs.


<details>
  <summary>Details</summary>
Motivation: The pursuit of a unified representation for text and vision to simplify multimodal modeling and improve training efficiency, while addressing the challenge of balancing low-level details and high-level semantics in discrete representations.

Method: ViQ structures quantization learning into two stages: text-aligned pre-training and feature discretization, using a proximal representation learning strategy and a position-aware head-wise quantization mechanism.

Result: ViQ achieves competitive performance in multimodal tasks compared to state-of-the-art multimodal vision encoders, maintains high precision in low-level reconstruction, and yields up to 20%-70% acceleration in multimodal training efficiency.

Conclusion: ViQ provides a unified and general discrete representation for arbitrary visual inputs, balancing semantics and details while enabling efficient multimodal training.
...
TL;DR: The paper presents OctoSense, an open-source sensor platform and dataset for multi-modal self-supervised learning, and a 'late-fusion' masked autoencoder architecture that outperforms existing image-only foundation models in various tasks.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to develop a multi-modal sensor platform and dataset that can facilitate self-supervised learning in real-world robotics applications, particularly in situations with degraded sensors.

Method: The authors propose a 'late-fusion' masked autoencoder architecture that uses modality-specific tokenizers to account for different spatiotemporal characteristics of various sensors, and caches modality-specific tokens at inference time to process new measurements.

Result: The proposed architecture achieves better performance than existing image-only foundation models in tasks such as optical flow estimation, depth estimation, semantic segmentation, and ego-motion prediction, and demonstrates robustness in nighttime or degraded sensory data situations.

Conclusion: The paper concludes that the proposed OctoSense platform and 'late-fusion' masked autoencoder architecture can effectively enable multi-modal self-supervised learning in real-world robotics applications, leading to improved performance and robustness in various tasks.
...
<details>
  <summary>Details</summary>
Motivation: Existing action-conditioned world models struggle with high-DoF actions due to the heterogeneous nature of dexterous actions, leading to imbalanced optimization and reduced action fidelity.

Method: The authors propose DexAC-WM, which tokenizes actions to preserve dimension-level semantics and uses local refinement and global modulation to align action signals with visual dynamics. A semantic branch is also introduced to provide rich object-scene priors.

Result: Experiments on EgoDex and EgoVerse show significant improvements in FID, FVD, and PCK, demonstrating gains in visual-temporal realism and action-following consistency. DexAC-WM also shows scalability across different backbones.

Conclusion: The paper concludes that scaling world models to high-DoF control requires both structured action modeling and semantic grounding, and that DexAC-WM offers a promising approach to achieving this.
...
TL;DR: A geometry-aware method for moving objects in single images while preserving scene consistency.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for object motion in images often fail to preserve scene-level consistency, including handling occlusions and maintaining coherent shadows and reflections.

Method: A geometry-aware object motion method operating on diffusion transformers with rotary positional embeddings (RoPE) extended to 3D spatial structure.

Result: State-of-the-art performance on object motion benchmarks, preserving object identity and generating plausible content under large spatial displacements.

Conclusion: The proposed method effectively induces controlled motion in images while maintaining scene consistency, with impressive results on benchmarks.
...
TL;DR: A new framework, SAM2Matting, is proposed to improve video matting by enhancing trackers with a region-proposal bridge and dedicated matting heads, achieving state-of-the-art performance.
...
Method: A tracker-to-matting framework, SAM2Matting, which decouples the task into enhancing a foundational tracker with a region-proposal bridge and dedicated matting heads.
...
Conclusion: The proposed SAM2Matting framework advances video matting by leveraging enhanced trackers and achieves high-fidelity video matting performance.
...
TL;DR: The paper proposes RayPE, a positional encoding method that improves camera controllability, cross-frame 3D consistency, and overall video quality in video diffusion transformers.


<details>
  <summary>Details</summary>
Motivation: Current video diffusion transformers use RoPE on (u,v,t) axes, which doesn't capture 3D scene structure. The authors aim to improve this by incorporating geometric relations between camera rays.

Method: The authors propose RayPE, which injects 6D Plucker coordinates into self-attention queries and keys. They also decouple ray direction from moment magnitude and apply RMSNorm for stability.

Result: The full module improves camera controllability, cross-frame 3D consistency, and overall video quality on a four-dataset training mixture, adding less than 0.1% parameters to a pretrained video DiT.

Conclusion: The proposed RayPE method effectively enhances the performance of video diffusion transformers by incorporating geometric relations between camera rays.
...
<details>
  <summary>Details</summary>
Motivation: The need for physically-plausible 3D object motion prediction in various applications like robotics, graphics, and physical design.

Method: A diffusion transformer that represents objects as 3D meshes in world coordinates, using a single denoising diffusion process for vertex trajectory prediction.

Result: PhysiFormer outperforms autoregressive baselines in trajectory accuracy, rigidity preservation, and physical consistency, and generalizes to mixed-material settings, unseen geometries, and larger object counts.

Conclusion: PhysiFormer presents a promising approach for view-invariant, geometry-aware world modeling, with potential applications in robotics, graphics, and physical design.
...
TL;DR: A new method called feature self-guidance is proposed to improve diversity in flow models without sacrificing fidelity.


<details>
  <summary>Details</summary>
Motivation: State-of-the-art flow models suffer from diversity collapse when generating multiple samples under the same conditioning.

Method: The proposed method uses feature self-guidance to disperse internal features of the flow model during batch generation, and manifold regularization to project these dispersed features back onto the data manifold.

Result: The method significantly improves diversity while preserving fidelity across several conditional flow models.

Conclusion: The proposed method is an efficient, training-free solution that integrates seamlessly into pretrained flow models, adding only a marginal inference cost.
...
TL;DR: The paper proposes Denoising Attention (DnA), a method to improve attention-based models by reducing noise in attention patterns, leading to better performance in visual perception tasks.


<details>
  <summary>Details</summary>
Motivation: The standard softmax activation in multihead attention (MHA) can produce noisy attention patterns that degrade performance by diluting relevant features.

Method: The proposed DnA method uses a positive query to identify relevant image features and a negative query to identify irrelevant features, then projects these interactions into two distinct subspaces with larger principal angles.
...
Conclusion: The proposed DnA method effectively denoises attention patterns, leading to improved performance and discriminability in visual perception tasks.
...
TL;DR: The authors propose VISE, a purely unsupervised self-evolving framework that improves visual reasoning in large multimodal models by regularizing the model's visual conditioning policy.
...
Method: The authors propose VISE, which uses two complementary invariance-based rewards: a geometric invariance reward and a semantic invariance reward, to directly regularize the model's visual conditioning policy.
...
Method: The proposed framework consists of a Proposer, Solver, and Generator, using self-derived consistency signals and introducing Solver Token Entropy (STE) and a multi-scale internal evaluation scheme.
...
Result: HierBias achieves 0.853 F1 and 0.723 MCC, surpassing the state-of-the-art bias-detector by +2.6% F1 and +4.3% MCC.

Conclusion: HierBias effectively detects media bias by modeling document context, and its components contribute independently and consistently to its performance.
...
TL;DR: A new benchmark is proposed to evaluate large language models' reliability in answering questions, abstaining from answers, and handling data contamination.


<details>
  <summary>Details</summary>
Motivation: Current evaluation methods for large language models are flawed as they conflate supported answering, unsupported guessing, and data contamination. The authors aim to separate these aspects and provide a more reliable evaluation protocol.

Method: The authors present a contamination-aware, multi-zone benchmark with 1,200 items across five domains, including explicit abstention expectations and contamination-risk metadata. They evaluate several large language models, including FLAN-T5, Qwen2.5-Instruct, and Llama-3-Instruct.
...
Conclusion: The proposed benchmark provides a reproducible protocol for auditing answerability, abstention, refusal, and contamination in large language models, shedding light on their reliability and limitations.
...
TL;DR: The study investigates how different post-training methods affect the retention of animal compassion values in a large language model. It finds that helpfulness training degrades animal compassion and general moral reasoning, while coding training preserves these values.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to understand how post-training processes, specifically supervised fine-tuning (SFT) and reinforcement learning (RL), impact the retention of values such as animal compassion instilled during pre-training in language models.

Method: The study uses a Llama 3.1 8B model mid-trained on compassion-oriented synthetic data and applies SFT and GRPO with different domains (helpfulness and coding). It evaluates the model on the Animal Harm Benchmark (AHB 2.2) and MORU benchmark.

Result: The results show that helpfulness training significantly degrades animal compassion and general moral reasoning compared to coding training. The effect is consistent across two independent helpfulness datasets and two training paradigms. However, the domain effect disappears on the multilingual MORU benchmark.

Conclusion: The study concludes that coding-domain post-training may better preserve mid-trained values than helpfulness post-training without harming general reasoning capabilities. It suggests that values instilled through mid-training are encoded more deeply and cross-lingually than reasoning improvements from domain-specific post-training.
...
TL;DR: This study evaluates Large Language Models' (LLMs) performance in solving statics problems, particularly in mechanical engineering education. It finds LLMs excel in text-only problems but struggle with diagram-based, multi-step reasoning tasks.


<details>
  <summary>Details</summary>
Motivation: The rapid influence of LLMs on education, especially in subjects like mechanical engineering, necessitates a deeper understanding of their capabilities and limitations in solving specific problem types, beyond general or public datasets.

Method: The study uses a model distillation process to evaluate LLM capabilities. It constructs three datasets: text-only statics questions, and two more with added diagrams and modified numerical values. ChatGPT is used for the evaluation.

Result: LLMs perform well on text-only statics problems but their accuracy decreases with the introduction of diagrams and the requirement for multi-step reasoning. The performance drop is attributed to difficulties in multi-step reasoning and applying visual information consistently.

Conclusion: The study highlights the need for targeted analysis of LLMs in specific educational contexts, such as mechanical engineering. It underscores the importance of evaluating LLMs beyond simple, text-based problems to assess their readiness for real-world educational applications.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to understand how animal-welfare advocates' writing influences language models and to provide practical recommendations for writing effective animal-welfare text.
...
Conclusion: The study concludes that when writing animal-welfare text that may end up in LLM training corpora, it is recommended to assert a position rather than describe a scene neutrally, as features that make the writer's position explicit tend to shift the model toward stronger pro-animal-welfare reasoning.
...
TL;DR: ContextForge, a system for context recycling, improves large language models' performance in long conversational tasks by reusing prior computation, reducing token overhead while preserving answer quality.


<details>
  <summary>Details</summary>
Motivation: Large language models' performance degrades in long conversational tasks due to context window limitations and inefficient token usage.

Method: ContextForge combines structured query generation, external memory retrieval, and controlled synthesis to maintain task-relevant information across turns.

Result: ContextForge demonstrates improved consistency, reduced token consumption, and comparable response accuracy compared to a baseline agent in a 15-turn conversational benchmark.

Conclusion: Context recycling provides a practical approach for extending LLM capabilities in long-horizon tasks without requiring larger context windows or model retraining.
...
TL;DR: The paper investigates if Large Language Models (LLMs) can be guided to have de-escalating dialogue through lightweight prompts based on Nonviolent Communication (NVC) principles.


<details>
  <summary>Details</summary>
Motivation: The increasing use of LLMs in emotionally charged situations involving conflict, frustration, and distress, and the need to prevent unintentional escalation of conflict through conversational behaviors.

Method: Using a dual-agent simulation framework with NVC-constrained prompting across multiple instruction-tuned models and user resistance levels.
...
Conclusion: Simple communication constraints can meaningfully improve the trustworthiness of LLM dialogue in conflict-prone settings.
...
TL;DR: A pilot study presents NEST-V1, a multimodal framework for generating emotion-conditioned Nepali Sign Language avatars from spoken input, achieving high accuracy and efficiency.
...
Method: The proposed framework, NEST-V1, uses a shared acoustic encoder for simultaneous Automatic Speech Recognition and emotion classification, with a lightweight architecture having 22.1M parameters.
...
TL;DR: Larger language models outperform smaller ones on reasoning benchmarks, with a focus on identifying the reasoning differences behind this gap.
...
Method: The authors develop AdvCluster, an automated framework that identifies questions where larger models show a stable advantage, extracts fine-grained advantage descriptions, and organizes them through semantic clustering.

Result: The analysis yields a systematic taxonomy of larger model reasoning advantages, including common and specialized advantages across domains, with a recurring theme of Constraint-Guided Reasoning.

Conclusion: Larger models are better at identifying explicit and implicit constraints, organizing them into structured reasoning, and using them to rule out infeasible paths and verify intermediate steps.
...
<details>
  <summary>Details</summary>
Motivation: The challenge of developing AI systems for low-resource languages without massive training corpora.
...
TL;DR: A new framework, Dynamic-dLLM, is proposed to improve the inference efficiency of diffusion large language models (dLLMs) without compromising performance.


<details>
  <summary>Details</summary>
Motivation: The computational complexity of dLLMs scales with the sequence length cubed, posing challenges for long-sequence and real-time applications due to incompatibility with key-value caching and non-autoregressive denoising steps.
...
TL;DR: The study examines the methodological suggestions of large language models (LLMs) like GPT-5.1, Gemini 3 Pro, and DeepSeek-V3.2 when prompted with research questions from 1,000 recent computer science papers. It finds that LLMs tend to suggest a narrower range of methods, with significant imbalances in provider choice and underrepresentation of certain model types.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to understand the default methodological tendencies of LLMs when used to guide research methodology, as they are increasingly being relied upon in research.

Method: The study uses an LLM-extracted research question from each of 1,000 recent arXiv computer-science papers to prompt GPT-5.1, Gemini 3 Pro, and DeepSeek-V3.2. It then compares the resulting methodology suggestions against a paper-derived experimental inventory, extracting structured method features and mapping them into a shared taxonomy.

Result: The results show significant divergence in methodology suggestions across multiple taxonomy dimensions, with the strongest imbalance in provider choice. LLMs suggest a much narrower range of methods overall, with an effective number of model entities contracting from 1,232 to 59-96.

Conclusion: The study concludes that researchers who rely on LLM suggestions without cross-checking risk narrowing their methodological search space toward a more concentrated default, highlighting the need for careful evaluation and validation of LLM-generated methodological suggestions.
...
TL;DR: The paper presents a systematic survey of unified vision-language perception in Multimodal Large Language Models (MLLMs), formalizing MLLM perception as a unified capability and proposing a five-stage taxonomy to trace its evolution.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to bridge the gap in existing surveys that focus separately on vision or language, and to provide a unified vision-language perspective on perception in MLLMs.

Method: The authors formalize MLLM perception as an intrinsic, unified vision-language capability and introduce a five-stage taxonomy to survey representative methods and milestones in the evolution of MLLM perception.
...
TL;DR: The neutral tone in Mandarin Chinese has its own tonal target and exhibits word-specific pitch signatures, similar to lexical tones, and is proposed to be a lexical tone.


<details>
  <summary>Details</summary>
Motivation: To investigate the phonetic realization of the neutral tone in spontaneous conversational speech corpora of Beijing Mandarin and Taiwan Mandarin.

Method: Corpus-based study using spontaneous conversational speech corpora of Beijing Mandarin and Taiwan Mandarin, analyzing the phonetic realization of the neutral tone.

Result: The neutral tone has its own tonal target, and disyllabic words with a neutral tone have pitch contours that depend on the tone on the first syllable. Word-specific pitch signatures are also found, which can be predicted from words' contextualized embeddings.

Conclusion: The neutral tone is proposed to be a lexical tone in both Taiwan Mandarin and Beijing Mandarin, with similarities and differences in its realization between the two varieties.
...
TL;DR: Small language models can assist with title and abstract screening for large systematic reviews in social-physical human-robot interaction, augmenting human reviewers.


<details>
  <summary>Details</summary>
Motivation: The growth of social-physical human-robot interaction research has led to a need for scalable review practices due to fragmented terminology and inconsistent methodologies.

Method: Evaluation of small language models (SLMs) with less than 1.5B parameters for title and abstract screening in a large spHRI systematic review.

Result: SLMs operated faster than human reviewers, identified 39 missed papers (10.29% of the final dataset), but did not match human performance.

Conclusion: SLMs can augment expert reviewers, making large-scale literature reviews more accessible and sustainable.
...
TL;DR: The paper presents PROFILEFOUNDRY, a synthetic data generator for creating realistic, yet fake, person data for evaluating foundation models while maintaining privacy and responsibility.


<details>
  <summary>Details</summary>
Motivation: The need for realistic person data in foundation-model research while ensuring responsible handling of sensitive information.

Method: A deterministic generator and fixed reference release of 100,000 synthetic Person Objects with various attributes and relationships.

Result: The release of PROFILEFOUNDRY, a dataset containing 709,228 events, 40,338 households, 52,491 employers, and 518,564 directed relationship edges.

Conclusion: PROFILEFOUNDRY provides a responsible synthetic source layer for constructing downstream foundation-model evaluations while keeping the synthetic person behind each artifact inspectable.
...
TL;DR: A new metric called ConflictScore is introduced to evaluate how well a model's response acknowledges conflicting evidence in its grounding documents.


<details>
  <summary>Details</summary>
Motivation: Existing metrics for factuality and faithfulness do not capture situations where both supporting and contradicting evidence coexist.

Method: The framework decomposes responses into atomic claims, labels each claim against each grounding document, and aggregates these labels into ConflictScore-Count (CS-C) and ConflictScore-Ratio (CS-R).

Result: Experiments show that ConflictScore effectively detects overconfident claims across domains and improves truthfulness on TruthfulQA.

Conclusion: ConflictScore is a novel metric that quantifies how well a model's response acknowledges conflicting evidence, and it can serve as a corrective feedback mechanism to improve truthfulness.
...
TL;DR: Introduces ProvenAI, a framework for evaluating transparency in multi-hop question answering by assessing answer correctness, citation fidelity, and document influence.


<details>
  <summary>Details</summary>
Motivation: Current retrieval-augmented systems present citations with generated answers, but citations do not guarantee the source influenced the output.

Method: ProvenAI uses a seven-stage pipeline to evaluate 7,405 validation examples, including data normalization, retrieval indexing, and attribution auditing.

Result: ProvenAI achieves 53.53% answer accuracy, 71.55% citation-fidelity score, and reveals a 'citation-influence gap' where cited sources have weak influence while uncited sources impact the output.

Conclusion: ProvenAI establishes that meaningful transparency in retrieval-grounded QA requires traceable links across retrieved, cited, and behaviourally influential evidence.
...
TL;DR: A single lightweight model, AnySimLite, is proposed to solve multiple speech-adjacent classification tasks with low memory footprint.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of deploying multiple specialized models for natural language classification on edge devices, which creates a memory footprint issue.
...
Conclusion: AnySimLite offers a effective and efficient solution for multiple speech-adjacent classification tasks on edge devices, balancing performance and memory footprint.
...
TL;DR: The paper proposes SOLAR, a method to improve multilingual large language models' reasoning consistency across languages by aligning soft-token representations.
...
Conclusion: Aligning soft-token representations helps preserve shared semantic structure during multilingual reasoning, leading to improved consistency and accuracy.
...
<details>
  <summary>Details</summary>
Motivation: The need to identify essential parts from massive scientific text, and the limitations of small-scale datasets in training models to extract problem and method sentences.
...
Result: The proposed models achieve a higher macro F1 score compared to baseline models on two scientific paper datasets, with improvements of 3.71% and 2.67%, respectively. However, LLM-based ICL methods are found not suitable for the task.

Conclusion: The paper presents solutions to improve the extraction of problem and method sentences from scientific papers, demonstrating the effectiveness of the proposed models and highlighting the limitations of LLM-based ICL methods for this task.
...
TL;DR: The paper explores using EEG signals to improve automatic keyphrase extraction (AKE) from microblogs, finding that EEG features consistently enhance AKE performance, with EEG signals providing useful cognitive evidence.


<details>
  <summary>Details</summary>
Motivation: The challenge of automatic keyphrase extraction (AKE) from microblogging platforms due to short, noisy, and dispersed user content, and the limitations of using eye-tracking signals alone.

Method: The paper uses 8 EEG features and 17 eye-tracking features, incorporating them into microblog-based AKE models by injecting them into the input of the soft-attention layer and the query vectors of the self-attention layer.

Result: Cognitive signals, particularly EEG features, improve AKE performance across different feature combinations and model architectures, with EEG and eye-tracking features showing partial complementarity but also possible redundancy or noise.

Conclusion: EEG signals provide useful cognitive evidence for microblog-based AKE, and multimodal cognitive signals deserve further investigation.
...
TL;DR: The authors propose TempoWave, a novel interface that enables large language models to better handle numerical time series forecasting by converting scalar observations into digit-wise embeddings, leading to improved performance over existing methods.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the misalignment between the discrete tokenization of large language models (LLMs) and the continuous numerical values in time series forecasting, which can harm numerical ordering and forecasting reliability.

Method: The authors propose TempoWave, a plug-and-play temporal wavelet digit interface that maps scalar observations into digit-wise embeddings constructed from multi-wavelet, multi-scale coefficients, allowing for precise numerical formatting and robustness to normalization operations.
...
TL;DR: The paper compares two approaches for classifying German-language climate news articles as threat-oriented, solution-oriented, both, or neither, achieving high F1 scores.


<details>
  <summary>Details</summary>
Motivation: Automated detection of framing patterns in climate news articles to analyze large corpora that are infeasible to code manually.

Method: Two approaches: (1) few-shot prompting with a large language model (Llama 4 Maverick) and (2) fine-tuning a German BERT model (deepset/gbert-large) for sentence-pair classification.

Result: The fine-tuned BERT classifiers achieve an F1 score of 0.83, while the LLM-based classifiers reach an F1 of 0.78.

Conclusion: The results contribute to the comparison of fine-tuned encoder models with prompted generative models for text classification in computational social science.
...
TL;DR: TwoTower is a new diffusion language model that separates context representation and denoising into two towers, achieving high-quality text generation and improved throughput.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion language models use a single network for both context representation and denoising, limiting their capacity. The authors propose to decouple these roles for better performance.

Method: The TwoTower model consists of a frozen autoregressive context tower and a trainable diffusion denoiser tower with bidirectional block attention, built on the Nemotron-3-Nano-30B-A3B model.

Result: Nemotron-TwoTower retains 98.7% of the autoregressive baseline's quality and offers 2.42X higher wall-clock generation throughput.

Conclusion: The proposed TwoTower model achieves a balance between text generation quality and throughput, making it a promising alternative to autoregressive models.
...
TL;DR: The paper presents MemStrata, a retrieval memory system that maintains temporal validity of facts, addressing the limitations of Retrieval-augmented generation (RAG) in handling changing knowledge.


<details>
  <summary>Details</summary>
Motivation: RAG's inability to model time leads to retrieving both stale and current values with similar embedding similarity, causing agents to serve superseded facts.

Method: MemStrata uses a bi-temporal ledger with a deterministic supersession rule to retire stale values, preserving static recall and achieving high accuracy on evolving knowledge.

Result: MemStrata outperforms RAG on evolving knowledge benchmarks (0.95-1.00 accuracy vs 0.20-0.47), reduces stale-fact-error rate to ~0% (vs 15-40% for RAG), and achieves comparable retrieval latency (~2.1s vs ~16-18s for LLM-reranking baselines).
...
TL;DR: This paper proposes a longitudinal text analysis approach to evaluate qualitative changes in corporate narrative disclosures over time, applying it to Japan's 2019 disclosure reforms.


<details>
  <summary>Details</summary>
Motivation: The challenge of comprehensively evaluating qualitative changes in narrative disclosures over time, as they are inherently multidimensional.

Method: A longitudinal text analysis approach combining NLP metric extraction, paired testing, shift function analysis, and inter-metric correlation, with a cross-section relevance indicator.

Result: The analysis of 19,770 firm-year observations over 10 years reveals complex shifts in disclosure patterns, including increased disclosure volume, declined readability, and varied adaptation across market segments.

Conclusion: The proposed approach can capture underlying dynamics in narrative disclosures, revealing complex shifts that conventional single-indicator methods may miss.
...
TL;DR: The paper reveals that AI models can be blind to safety-critical signals not specified in their tasks, a phenomenon termed the Inattentional Gap, which decouples benchmark safety scores from real-world safety.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the issue of AI safety evaluation, which currently focuses on detecting specified hazards, but often overlooks unspecified ones that can cause harm.

Method: The authors conducted experiments across various tasks, including radiology and driving text scenarios, and chest-radiograph vision tasks, to evaluate the behavior of language and vision models when conditioned on narrow tasks.

Result: The results show that all tested models exhibited suppression of co-present, safety-critical signals when conditioned on narrow tasks, a phenomenon observed across different model families and sizes, but not diminishing with scale.

Conclusion: The authors conclude that the Inattentional Gap, a dissociation between measured benchmark safety and real-world safety, highlights the need to reevaluate AI safety evaluation methods to account for unspecified hazards.
...
TL;DR: A new method called DiARC is proposed to improve model performance on ARC-like tasks by using negative sample supervision to enhance reasoning.


<details>
  <summary>Details</summary>
Motivation: Current methods for solving ARC-like tasks rely heavily on data augmentation and supervised fine-tuning, but have limitations in terms of cost and performance. The authors argue that distinguishing between positive and negative samples is crucial for improving model reasoning.

Method: The authors propose DiARC, a method that constructs preference pairs to enable the model to distinguish between positive and negative samples. Three ways to construct negative samples are proposed: output-level visual transformations, DSL-level rule inversion, and task-specific rule editing.
...
Conclusion: The proposed method, DiARC, offers a promising approach to improving model performance on ARC-like tasks by leveraging negative sample supervision to enhance reasoning.
...
TL;DR: The paper proposes Erase-then-Delta Attention (EDA), a new memory update rule for recurrent memory models that decouples erase and write operations, allowing for more efficient removal of stale information and improving performance in language model pretraining experiments.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitation of delta-rule linear attention, which can only correct information at the current write address, but not actively remove stale information stored at different addresses.

Method: The proposed method, EDA, first applies a targeted erase step along a learned erase direction, and then performs a standard delta-style corrective write along the current write direction.

Result: The results show that EDA outperforms other methods in language model pretraining experiments across different model families and settings, with gains persisting even after long-context midtraining.

Conclusion: The conclusion is that recurrent memory models should not only decide what to write, but also what stale information to erase and where, and that EDA provides a effective solution to achieve this.
...
TL;DR: The paper proposes KIRP, a zero-shot stance detection framework that integrates external knowledge with entity reorganization and prompt chaining for reasoning, achieving state-of-the-art performance on several datasets.
...
Method: The proposed framework, KIRP, uses knowledge graphs to supplement and reorganize key textual entities, reflective Chain-of-Thought (CoT) reasoning to extract and validate implicit targets, stance-aware contrastive learning to capture discriminative features, and a three-layer iterative prototype network for fine-grained classification.
...
Conclusion: The authors conclude that KIRP effectively addresses the challenges of zero-shot tweet-level stance detection and achieves state-of-the-art performance on several datasets.
...
TL;DR: The study addresses the quality gap in text-to-speech (TTS) models for low-resource languages, specifically Khmer and Korean, using a large pretrained TTS model called VoxCPM2.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the performance of TTS models for languages with limited training data, as they typically perform worse than well-resourced languages.

Method: The method involves adapting a 2.4B-parameter TTS model, VoxCPM2, using a single Low-Rank Adaptation (LoRA) adapter for both Khmer and Korean. The adapter is trained on a shared corpus of about 26 hours and added to both the language model and the decoder.

Result: The results show a significant improvement in Mean Opinion Score (MOS) for Khmer from 3.85 to 4.23 with the best adapter, while only training 0.19 to 3.03 percent of the parameters. However, the adapter does not improve performance for Korean, a language the base model already handles well.

Conclusion: The study concludes that adaptation helps mainly where the base model is genuinely weak, and that the automatic loss and human ratings may disagree on the optimal adapter rank.
...
TL;DR: CAT-Q is a post-training quantization scheme for compressing and accelerating large language models (LLMs) by converting them into ternary models with minimal performance degradation.
...
Method: CAT-Q uses two key components: learnable modulation (LM) and softened ternarization (ST), which are coupled from an optimization perspective to modulate the distribution of pre-trained high-precision weights and guide the ternarization process.

Result: CAT-Q achieves superior performance than state-of-the-art methods, reducing training tokens by about 100,000X and enabling the quantization of large pre-trained LLMs (14B to 235B parameters) within 8 to 60 hours on 8 A100-80GB GPUs.

Conclusion: CAT-Q offers a cost-efficient and accurate ternary quantization approach for LLMs, demonstrating its effectiveness and efficiency in compressing and accelerating large language models.
...
Method: The authors create a benchmark called SocialPersona, which consists of longitudinal social-media timelines from 171 users, along with human-verified preference tags across seven interest domains. They then evaluate the performance of multimodal large language models on two tasks: constructing structured user profiles and generating responses aligned with inferred profiles.
...
TL;DR: This study improves fallacy classification by combining logical structures with linguistic cues using Large Language Models.
...
Method: The study develops a framework that uses Large Language Models to inductively extract patterns from fallacious examples and their explanations.

Result: The approach shows statistically significant improvements over zero-shot baselines and outperforms competing methods in fallacy classification.

Conclusion: The study establishes data-driven pattern extraction as an effective method for generating logical representations that improve fallacy classification.
...
TL;DR: The paper introduces ConvMemory v3, which adds a validity context layer to optimize conversational memory retrieval by detecting and surfacing update evidence.
...
Method: The method involves a dual-evidence gate that conditions a relation judgment on the specific target proposition, scoring a (target, source) pair through the product of a MiniLM slot head and a DeBERTa-v3 slot head and gating it by conservative event/operation evidence.

Result: The results show that the gate reaches 90.12% +/- 1.73 accuracy on a synthetic multi-hop validity benchmark and 98.8% +/- 0.9 group-all-correct on a real-data feedback loop. The deployed layer preserves retrieval by default and raises current-active H@1 from 45.1% to 95.7% +/- 1.2.

Conclusion: The conclusion is that ConvMemory v3 adds a validity context layer that detects and surfaces update evidence, optimizing conversational memory retrieval while preserving retrieval by default.
...
TL;DR: The paper analyzes evaluation pitfalls in multimedia event extraction, identifying issues with data processing, task assumptions, and evaluation settings.


<details>
  <summary>Details</summary>
Motivation: The reliability and comparability of results in multimedia event extraction depend on consistent and rigorous evaluation.

Method: Systematic analysis and controlled experiments under a strict evaluation framework.

Result: Minor evaluation choices can cause large performance variations and lead to overestimation of a model's ability.

Conclusion: The need for comparable evaluation standards and more rigorous evaluation in multimedia event extraction is highlighted.
...
TL;DR: The authors propose OPID, a framework for on-policy skill distillation in reinforcement learning, which extracts skill supervision from completed on-policy trajectories and improves agent performance, sample efficiency, and robustness.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of outcome-based reinforcement learning, which provides sparse rewards, and on-policy self-distillation, which relies on external skill memories or retrieved privileged context.

Method: The authors propose OPID, a framework that extracts skill supervision directly from completed on-policy trajectories, representing trajectory hindsight as hierarchical skills, and uses a critical-first routing mechanism to inject the selected skill into the interaction history.

Result: The experiments on ALFWorld, WebShop, and Search-based QA demonstrate that OPID generally improves agent performance, sample efficiency, and robustness over outcome-only RL and existing skill-distillation baselines.

Conclusion: The authors conclude that OPID preserves RL as the primary training objective while introducing dense, distribution-matched hindsight supervision, leading to improved performance, efficiency, and robustness in language agents.
...
TL;DR: The study analyzes vocabulary relationships across eight layers of Bengali and Sanskrit devotional literature, finding evidence of Buddhist-Shakta syncretism in Bengal.


<details>
  <summary>Details</summary>
Motivation: To quantify the historically argued claim that Buddhist Vajrayana vocabulary survived and was absorbed into the Shakta Tantra tradition of Bengal.

Method: Using a corpus of 75 texts and TF-IDF character n-gram vectorization with cosine similarity analysis.

Result: The study finds a significant contrast in cosine similarity between Buddhist-Shakta and Vaishnava traditions, and provides evidence of lexical transition within the Buddhist-Shakta transmission chain.

Conclusion: The results provide quantitative corroboration of historically argued Buddhist-Shakta syncretism in Bengal, and shed light on the evolution of vocabulary across different traditions.
...
TL;DR: The paper presents a submission to the IWSLT 2026 Instruction Following shared task, focusing on SpeechLLMs for short-form and long-form speech instruction following.


<details>
  <summary>Details</summary>
Motivation: The motivation is to develop SpeechLLMs for instruction following in both short-form and long-form speech under constrained settings.

Method: The method involves exploring three speech segmentation methods for the long track and introducing the HIFS score to evaluate long-form generation stability.

Result: The results show strong performance on MCIF for the short track with a SIFS score of 2.0708, and the highest HIFS score of 2.0663 for the long track with fixed 30-second segmentation.

Conclusion: The conclusion is that SpeechLLMs can achieve robust performance in both short-form and long-form speech instruction following, with hallucination mainly manifesting as repetitive insertions in generated outputs.
...
<details>
  <summary>Details</summary>
Motivation: Existing pruning methods for large language models (LLMs) on Industrial Internet of Things (IIoT) edge devices struggle with high compression ratios due to one-shot importance estimation and unpredictable cross-architecture behavior.
...
Conclusion: The proposed framework enables efficient deployment of LLMs on IIoT edge devices, but its effectiveness varies across architectures, highlighting the importance of the Structural Independence Assumption (SIA).
...
TL;DR: A new KV cache compression framework, InfoKV, is proposed, which incorporates information-theoretic signals to improve token importance estimation and outperforms existing attention-based methods.
...
Method: InfoKV combines token-level predictive uncertainty with layer-wise representation evolution and integrates the resulting entropy scores with attention scores during reasoning.

Result: InfoKV consistently outperforms existing attention-based KV compression methods in both long prefilling and decoding scenarios on benchmarks with Llama-3.1, Llama-3.2, and DeepSeek-R1.

Conclusion: The proposed InfoKV framework provides a more effective approach to KV cache compression by incorporating information-theoretic signals, leading to improved performance in long-context reasoning tasks.
...
TL;DR: Language-model representations can serve as informative neural predictors during natural speech and text comprehension.


<details>
  <summary>Details</summary>
Motivation: To analyze the utility of language-model derived quantities in annotating neural activity during natural speech and text comprehension.
...
TL;DR: The study evaluates the reliability of Automatic Speech Recognition (ASR) models in the Indian healthcare context, particularly in multilingual and demographically diverse settings. It assesses eight state-of-the-art models and proposes a unified debiasing technique, SamaVaani, to improve ASR performance and fairness.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this study is to investigate the reliability of ASR in the Indian healthcare context, which is multilingual and demographically diverse, and to address potential biases in ASR models.
...
Result: The results reveal substantial variability across models and languages, with some systems performing competitively in Indian English but failing in regional speech. The study also uncovers systematic performance gaps tied to speaker role and gender, which can be mitigated by fairness-aware fine-tuning.

Conclusion: The study concludes that while ASR models have the potential to document clinical encounters, their reliability in multilingual and demographically diverse settings is a concern. The proposed SamaVaani technique can improve ASR performance and fairness across demographic groups, enabling more equitable deployment in clinical settings.
...
TL;DR: The paper introduces GAVEL, a task for verifying, explaining, and localizing misalignments between text and images in vision-language models, and presents a dataset and benchmark for evaluation.
...
Method: The authors introduce a new task called GAVEL, which jointly addresses verification, explanation, and localization for image-text pairs, and provide a supervised baseline trained on human-annotated data.
...
Conclusion: The paper concludes that GAVEL provides a useful framework for evaluating and improving the ability of vision-language models to detect and explain misalignments between text and images.
...
TL;DR: A term-centric framework is proposed for inducing hierarchical taxonomies from diverse text sources, improving cross-source coherence and hierarchy quality.
...
Method: The proposed framework uses automatic term extraction to map documents into a shared representation space, enabling robust cross-source alignment and construction of interpretable hierarchies.

Result: Experiments on a large benchmark demonstrate that the proposed method improves cross-source coherence and hierarchy quality over baselines, with a case study showing its practical utility for technology landscape mapping.

Conclusion: The proposed term-centric framework offers a scalable and effective approach for organizing knowledge from diverse text sources into interpretable hierarchies.
...
TL;DR: The paper surveys safety reporting practices of speech models, finds a lack of multilingual analysis, and introduces RedVox, a multilingual benchmark to evaluate safety and fairness across five languages.


<details>
  <summary>Details</summary>
Motivation: The safety and fairness of speech-capable models beyond English settings and under naturalistic conditions are understudied.

Method: The authors survey state-of-the-art speech model releases, introduce RedVox benchmark, and evaluate eight state-of-the-art models across five languages.

Result: Vulnerabilities persist even under non-adversarial conditions, worsen in non-English languages, and are amplified when the request comes from a spoken input.

Conclusion: The study highlights the need for improved safety and fairness in speech models, particularly in multilingual settings and under naturalistic conditions.
...
TL;DR: The study investigates how different contextual framings affect the internal representations of large language models (LLMs) in mental health support tools, finding that framing systematically alters response tendencies and behavior-associated information remains decodable throughout transformer depth.
...
Method: The study uses controlled matched prompts across multiple contextual framing conditions and model families, layer-wise probing analyses, held-out framing probes, and activation steering experiments to investigate the effects of framing on LLMs.

Result: The study finds that framing systematically alters interpretive response tendencies, behavior-associated information remains decodable throughout transformer depth, and framing-associated representational directions can partially modulate downstream behavioral outcomes.
...
TL;DR: The paper proposes a framework called ReaORE for open relation extraction, which uses coarse-to-fine relation reasoning to improve generalization to unseen relation types.


<details>
  <summary>Details</summary>
Motivation: The core challenge of OpenRE lies in achieving reliable generalization to unseen relation types, and current approaches have limitations in either generating relation labels or distinguishing easily confused relations.

Method: The proposed framework ReaORE consists of two stages: relation filtering and relation prediction, using coarse-to-fine relation reasoning to extract relations.

Result: Extensive experiments on two widely used OpenRE datasets demonstrate that ReaORE outperforms existing baselines.

Conclusion: The proposed ReaORE framework effectively addresses the limitations of current OpenRE approaches and achieves reliable generalization to unseen relation types.
...
TL;DR: The study tests if emotion vectors, which are internal representations that encode emotion concepts, exist in two open-weight language models, Apertus-8B-Instruct-2509 and Gemma-4-E4B-it. The results show that both models exhibit valence geometry, but with differences in how valence representations emerge across model depth.


<details>
  <summary>Details</summary>
Motivation: To test the generality of emotion vector findings in Claude Sonnet 4.5, and to investigate if similar emotion representations exist in other language models.

Method: The study uses two model-generated corpora to extract emotion contrast vectors across all layers of Apertus-8B-Instruct-2509 and Gemma-4-E4B-it. The researchers then analyze the valence geometry and arousal encoding in both models.

Result: The study finds that both models exhibit valence geometry, with peak PC1--valence correlations of $r = 0.76$ and $r = 0.83$. However, there are notable differences in how valence representations emerge across model depth between the two models. Arousal encoding is sensitive to the extraction corpus.

Conclusion: The study confirms the existence of emotion vectors in two open-weight language models and highlights the importance of investigating emotion representations across different language model architectures. The findings also suggest that arousal-relevant cues are unevenly distributed across generated corpora.
...
TL;DR: Introduced MinGram, a simplified tokenizer that improves upon Unigram's performance and complexity.


<details>
  <summary>Details</summary>
Motivation: The Unigram tokenizer's training is heavy and complex, and the authors aim to simplify it while maintaining its performance.

Method: MinGram uses a BPE-derived seed vocabulary, Hard EM on a minimum-token path, and a single flat score-pruning step to simplify training.

Result: MinGram outperforms BPE and standard Unigram in compression across six languages and matches the strongest token-count compressors in a compression-oriented variant.

Conclusion: MinGram achieves better compression and morphological alignment than BPE and Unigram, making it a more efficient and effective tokenizer.
...
TL;DR: The authors propose Psy-CoT, a psychology-grounded framework for building general-purpose role-playing agents, and RAPO, a role-aware policy optimization method, to improve character fidelity and out-of-distribution generalization.
...
Conclusion: The proposed Psy-CoT and RAPO methods improve character fidelity and out-of-distribution generalization for role-playing agents.
...
TL;DR: The paper introduces NuclearQAv2, a benchmark for evaluating large language models (LLMs) in nuclear engineering, and finds that while LLMs perform well on factual questions, they struggle with quantitative reasoning and conceptual understanding.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of ensuring the reliability of LLMs in highly technical domains, specifically nuclear engineering, where problem-solving requires not only factual knowledge but also quantitative reasoning and conceptual understanding.

Method: The method involves constructing a benchmark, NuclearQAv2, using a hybrid pipeline that combines expert-authored questions, existing datasets, and LLM-assisted generation from domain-specific technical corpora. The framework leverages structured prompting for automated question generation and response evaluation.
...
Conclusion: The conclusion is that multi-faceted evaluation frameworks are important, and NuclearQAv2 is established as a scalable benchmark for assessing LLM capabilities in technical domains.
...
TL;DR: A Judge-Aware Gated Multi-Task Learning architecture is proposed for legal outcome prediction, which disentangles objective case facts from adjudicative context and outperforms existing methods.
...
Method: A Judge-Aware Gated Multi-Task Learning architecture is introduced, which uses a fine-grained outcome taxonomy and a Gated Fusion mechanism to dynamically modulate reliance on judge identity.

Result: The proposed approach achieves a new state of the art on a benchmark of 13,937 UK Employment Tribunal decisions, with gains concentrated on the most ambiguous and rarest outcome classes, while requiring fewer trainable parameters than existing methods.

Conclusion: The proposed architecture is not only more accurate but also more interpretable, and it highlights the importance of the choice of conditioning interface in identity-conditioned classification of legal outcomes.
...
TL;DR: Large language models (LLMs) and humans were tested on their ability to flexibly adapt their reasoning strategies using a novel 'riddle riddle' paradigm, which showed LLMs and humans fail in opposite directions, with LLMs performing well on traditional riddles but poorly on riddle riddles requiring literal interpretations.


<details>
  <summary>Details</summary>
Motivation: The study aimed to investigate whether LLMs' accuracy on cognitive tasks is due to pattern matching from training data or flexible reasoning, by introducing a novel paradigm to test their ability to adapt reasoning strategies.

Method: The study used a 'riddle riddle' paradigm, where LLMs and human participants were presented with two types of problems: genuine riddles and riddle riddles, and their performance was compared across two experiments.

Result: The results showed that LLMs performed well on genuine riddles (84.9%) but poorly on riddle riddles (50.7%), whereas humans showed the reverse effect (50.5% on genuine riddles and 80.5% on riddle riddles), with error analysis indicating LLMs made more reasoning mistakes than humans.

Conclusion: The study suggests that LLMs' strong performance on traditional riddles may reflect memory retrieval rather than flexible strategy selection, and that their accuracy may be conflated with genuine reasoning without stimuli designed to elicit this contrast.
...
TL;DR: This paper explores using LLMs for forecasting tasks, analyzing their internal states to understand their reliance on time-specific knowledge versus generalizable patterns. It finds that amplifying time-awareness features can reduce look-ahead bias in forecasting.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve forecasting by understanding how LLMs use time-specific knowledge versus generalizable patterns, and to mitigate look-ahead bias.

Method: The method involves applying LLMs to forecasting tasks, analyzing their internal states using sparse autoencoders, and intervening on identified features to test their effect.

Result: The results show that amplifying time-awareness features substantially reduces look-ahead bias on forecasting prompts while preserving general reasoning performance.

Conclusion: The conclusion is that interpretable temporal features can be used to causally shift LLMs toward more historically grounded reasoning, improving forecasting.
...
<details>
  <summary>Details</summary>
Motivation: Lexical surprisal fails to predict processing difficulty in garden path sentences, where the sentence prefix leads to an incorrect initial interpretation.
...
Conclusion: The paper suggests a new research direction examining purely non-lexical alternatives to surprisal for psycholinguistics.
...
TL;DR: The paper argues that safety classifiers should model user intent and introduces AIMS, a dataset to study this. It evaluates intent-aware training methods and finds that directly rewarding intent faithfulness yields the strongest performance.


<details>
  <summary>Details</summary>
Motivation: The authors argue that safety classifiers should model user intent as an explicit signal to improve their performance and robustness.
...
Result: The results show that faithful intent modeling is a compact, high-quality supervision signal for more robust safety classifiers. Directly rewarding intent faithfulness with GRPO yields the strongest average performance across five external safety benchmarks.

Conclusion: The paper concludes that modeling user intent is crucial for safety classifiers, and intent-aware training methods can lead to more robust and competitive safety classifiers.
...
TL;DR: A new framework integrating formal semantics and evolutionary modeling shows that quantificational meaning can emerge through co-evolution of lexical meanings and composition function, leading to efficient system-wide abstractions like conservativity.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between formal semantics and evolutionary modeling by allowing lexical meanings and composition functions to co-evolve, addressing tensions between empirical evidence on quantifier learnability and prior evolutionary models.

Method: A framework that integrates formal semantics into evolutionary modeling, enabling the co-evolution of lexical meanings and a composition function under pressures for conceptual simplicity and communicative accuracy.

Result: The framework yields the emergence of conservativity as an efficient system-wide abstraction and provides a template for studying universals involving global compression, semantic specialization, and co-evolution of lexical and compositional meaning.

Conclusion: The study demonstrates that formal semantics can be productively combined with evolutionary modeling, offering insights into the evolution of quantificational meaning and the emergence of semantic universals.
...
TL;DR: CARVE, a new recurrent model, improves upon existing models by introducing a content-aware erase gate that enhances memory usage, efficiency, and performance.


<details>
  <summary>Details</summary>
Motivation: The current state-of-the-art recurrent models have limitations, including memory-blind gating, inefficient use of parameters, and mathematical restrictions on solving certain problems. These limitations hinder their performance and efficiency.

Method: CARVE resolves these issues by introducing a content-aware erase gate that only erases on the key axis, reuses the recurrent output tensor as a free content signal, and replaces the per-value write-gate projection with a single scalar per head.

Result: CARVE achieves a WikiText perplexity of 15.72, outperforming GDN-2 by 0.18, and sets state-of-the-art results on several benchmarks while reducing parameters, peak memory, and throughput overhead.

Conclusion: CARVE offers a more efficient and effective recurrent model that addresses the limitations of existing models, achieving superior performance and efficiency through its content-aware design.
...
<details>
  <summary>Details</summary>
Motivation: The increasing importance of understanding dialogic interaction in human-AI collaboration for optimizing partnerships.
...
Result: The framework demonstrates effectiveness and generalizability across nine datasets, providing insights into human-agent coordination and the role of metacognitive regulation.

Conclusion: The proposed framework can help analyze and optimize human-AI collaboration, highlighting the importance of metacognitive regulation in deeper collaboration.
...
TL;DR: Language models encode knowledge in a task-specific manner, not consistently across tasks, challenging the 'knowledge base' view.


<details>
  <summary>Details</summary>
Motivation: To investigate if language models (LMs) behave like knowledge bases, where queries for the same fact return consistent results.

Method: Behavioral and mechanistic analyses, including parameter localization experiments and chain-of-thought reasoning tests.

Result: LMs encode knowledge task-specifically; facts learned in one task often don't transfer to others; chain-of-thought reasoning uses task-specific parameters.

Conclusion: The findings undermine the 'knowledge base' analogy for LMs, impacting the reliability and controllability of factual knowledge.
...
<details>
  <summary>Details</summary>
Motivation: The ability of large language models to process historical language remains poorly understood, and historical difficulty is often treated as a single barrier, rather than addressing its various components.

Method: The authors propose a framework that decomposes historical difficulty into four dimensions: tokenization cost, predictive uncertainty, semantic robustness, and context sensitivity, and evaluate it on three datasets from different centuries and languages.

Result: The results show a dissociation between encoding cost and comprehension, with historical texts incurring higher tokenization penalties and predictive uncertainty, but still maintaining robust semantic representations, and that a minimal temporal context prompt can reduce historical surprisal.

Conclusion: The findings suggest that while historical text imposes challenges, digital libraries can safely deploy large language models for semantic retrieval tasks, provided that generative applications are carefully adapted.
...
TL;DR: The paper proposes a context-aware translation cascade for multilingual reasoning, which preserves the original question and intermediate translations to improve performance.


<details>
  <summary>Details</summary>
Motivation: The translation cascade approach for multilingual reasoning is lossy, discarding valuable information such as cultural cues and disambiguation hints.

Method: The authors propose a simple, training-free intervention: a context-aware translation cascade that provides the original question, English translated question, and reasoning trace to the final translation module.

Result: The approach demonstrates strong gains in open-ended generation across models and resource regimes, with the original language question carrying most of the beneficial context.

Conclusion: The study highlights the importance of designing better information flow in machine translation cascades to mitigate error propagation and proposes a simple, actionable strategy: preserving the original user question until the end of the pipeline.
...
TL;DR: The paper proposes a comprehensive taxonomy for indirect linguistic expressions (ILE) used on social media to evade moderation, and evaluates its effectiveness in detecting coded language.


<details>
  <summary>Details</summary>
Motivation: To improve content moderation on social media by understanding and detecting indirect linguistic expressions (ILE) used to camouflage sensitive meanings.

Method: The authors propose a mechanism-oriented taxonomy of ILE and evaluate it by incorporating it into LLM prompts and comparing it with existing taxonomies using 2,000 manually annotated social media posts.

Result: The proposed taxonomy achieves the strongest performance across three LLMs, with a 4.7% improvement in accuracy and 5.4% improvement in F1 over the best-performing benchmark.

Conclusion: The study highlights the importance of a comprehensive, mechanism-oriented taxonomy for detecting emerging coded language and facilitating content moderation on social media.
...
TL;DR: The paper presents a case study on using Large Language Models (LLMs) to verify the eligibility of securities as collateral, achieving high precision and proposing a new evaluation methodology.


<details>
  <summary>Details</summary>
Motivation: The manual verification of securities against legal and financial criteria in lengthy, semi-structured, and often bilingual prospectuses is a resource-intensive task, and traditional Named Entity Recognition (NER) methods have limitations.
...
Conclusion: The study demonstrates the potential of LLMs in improving the efficiency and accuracy of the eligibility examination process for securities as collateral.
...
TL;DR: The authors introduce PEEU, a method to improve task planning in multimodal web agents using small open-source models, achieving better performance than larger models.


<details>
  <summary>Details</summary>
Motivation: Small open-source MLLMs are cost-efficient and privacy-preserving but suffer from weak planning and limited cross-website generalization.

Method: The authors propose PEEU, which explores environments to discover experiences and utilizes hindsight experience to synthesize high-level training data, and TDHAF, a framework to analyze compositional generalization.

Result: The 7B model using PEEU achieves 30.6% accuracy, outperforming the larger Qwen2.5-VL-32B model, demonstrating the effectiveness of constructing hindsight high-level tasks and leveraging experiences.

Conclusion: The study shows that mastering low-level atomic skills does not guarantee high-level planning competence, while high-level task training yields stronger OOD generalization, and that PEEU is crucial for OOD planning abilities of small MLLMs.
...
TL;DR: The paper presents a new method for extracting relationships between entities from large amounts of unstructured news text, which can be used to study complex social and political networks.


<details>
  <summary>Details</summary>
Motivation: The authors aim to overcome the limitations of manual coding and existing automated text analysis methods to study complex, informal, and adversarial ties between political elites.

Method: The authors propose a modular, open-weight pipeline for multilingual joint entity-relation extraction, which combines named-entity recognition with a three-stage linking cascade and a high-throughput mixture-of-experts model.

Result: The pipeline achieves high textual correctness (68.2% strict to 93.7% lenient) and is validated through two large-scale case studies in Austria and Poland, which demonstrate its ability to reconstruct complex social and political networks.

Conclusion: The proposed framework provides a robust and replicable foundation for cross-national empirical computational social science, enabling researchers to study complex relationships between entities in a scalable and automated way.
...
TL;DR: The paper presents a method for generating data to improve the interpretability and control of language models, focusing on detecting and steering away from sycophancy.


<details>
  <summary>Details</summary>
Motivation: The need for reliable detection of model features responsible for certain behaviors, such as sycophancy, to enable effective model steering.

Method: An iterative data generation pipeline that isolates cascading linear features responsible for a behavior, using samples that show degrees of features that scale linearly with behavior.

Result: The proposed method allows for better disentanglement of features, detection, deterministic scoring, and robust steering, outperforming baseline approaches while providing lower computational demand and more interpretability guarantees.

Conclusion: The paper demonstrates the effectiveness of the proposed method in detecting and steering away from sycophancy in language models, with improved interpretability and control.
...
TL;DR: The paper challenges the common practice of retiring benchmarks when their accuracy saturates, instead proposing to evaluate agents across multiple dimensions such as construct validity, generalizability, efficiency, and human-agent collaboration.


<details>
  <summary>Details</summary>
Motivation: The current approach to benchmarking prioritizes accuracy over other important aspects of agent performance, potentially overlooking crucial insights into agent capabilities.

Method: The authors use CORE-Bench Hard as a case study, introducing improved benchmarks (CORE-Bench v1.1 and CORE-Bench OOD) and conducting experiments to measure various dimensions of agent performance.

Result: The study finds that even after accuracy saturation, the benchmark remains useful for evaluating efficiency, reliability, model performance, and scaffold performance, and that human-agent collaboration yields significant speedup in computational reproducibility tasks.

Conclusion: The paper presents a more comprehensive evaluation paradigm that goes beyond accuracy, providing a more nuanced understanding of agent performance and capabilities.
...
<details>
  <summary>Details</summary>
Motivation: The interaction between linear directions in activation space for refusal and persona traits in chat models has not been studied.
...
Conclusion: Refusal is gated at the late-layer expression stage, and treating refusal as a single isolated direction misses its dependence on persona.
...
TL;DR: The paper presents AlgoEvolve, an LLM-driven framework for generating and improving executable trading strategies through evolutionary discovery.


<details>
  <summary>Details</summary>
Motivation: To extend the application of Large Language Models (LLMs) as semantic mutation operators for evolutionary discovery from static coding benchmarks to algorithmic trading, a uniquely challenging domain.

Method: AlgoEvolve uses LLMs to generate, evaluate, and iteratively improve executable trading strategies expressed as Python code, with a meta-evolutionary outer loop that evolves prompts for program synthesis.

Result: The system exhibits emergent regime-adaptive strategy logic, including autonomous shifts in trading rules, and the evolved search heuristics consistently outperform human-designed instructions.
...
TL;DR: The authors introduce a pipeline for analyzing governance discourse in AI agent protocols and apply it to two contrasting standards, finding that while governance form influences thematic priorities, both exhibit comparable levels of participation inequality and community fragmentation.
...
TL;DR: The authors developed a framework to integrate online patient narratives and regulatory adverse-event records to improve psychiatric medication safety information.


<details>
  <summary>Details</summary>
Motivation: Patients seek medication information online, but safety knowledge for psychiatric drugs is split between authoritative regulatory records and unvalidated patient narratives.

Method: A provenance-aware, knowledge-graph-based multi-agent framework was developed, integrating Reddit posts, WebMD reviews, and FDA Adverse Event Reporting System records for nine antidepressants.

Result: The framework successfully integrated the data, showing that patient-generated data form a partly independent safety signal, with community sources often reporting adverse events before regulatory reports.

Conclusion: The study establishes source-aware integration as a route to more auditable psychiatric medication information, which could improve patient benefit and safety.
...
Method: The Drift-Diffusion-Enhanced Elo Rating System (DD-Elo) is proposed, which models skill expression as a decision-making process using the drift diffusion model (DDM) from cognitive neuroscience.

Result: DD-Elo adapts to skill changes faster than Elo and maintains a bounded deviation from the traditional Elo system, ensuring theoretical alignment.
...
TL;DR: A computational governance model for AI agents that ensures safe execution of high-risk actions by requiring independent evidence and attestation.


<details>
  <summary>Details</summary>
Motivation: The increasing autonomy of AI agents and the potential for consequential, irreversible actions necessitate a governance model that ensures safe execution.

Method: The proposed model separates planning and execution authority, requiring independent attestation of preconditions for high-risk actions, and uses a deterministic policy and tamper-evident log for decision recording and verification.

Result: A proof-of-concept implementation demonstrates the feasibility of the model, which is illustrated with examples from software deployment and clinical prescribing.

Conclusion: The proposed computational governance model provides a framework for ensuring the safe execution of high-risk actions by autonomous AI agents, while maintaining their autonomy in planning and reasoning.
...
TL;DR: This paper presents COrigami, an AI-driven pipeline for computational origami that generates crease patterns from natural language, assisting artists in designing physical art that meets geometric constraints and aesthetic standards.


<details>
  <summary>Details</summary>
Motivation: The challenge of generating physical art that satisfies both strict geometric constraints and subjective visual aesthetics in the domain of computational origami.
...
Result: The system generates structural starting points that human artists can further expand and shape, enabling reliable, mathematically grounded co-creativity.

Conclusion: The paper demonstrates how AI systems can satisfy multi-objective physical constraints to enable co-creativity, presenting a highly effective collaborative assistant for artists.
...
TL;DR: Verifying solutions generated by coding agents is becoming increasingly difficult, and current verification methods are insufficient. The authors propose characterizing verification signals along three dimensions and study four reward constructions to address this challenge.


<details>
  <summary>Details</summary>
Motivation: The difficulty in verifying solutions generated by coding agents, as the intuition that verification is easier than production is being inverted. The authors aim to address the challenges of reward design and leverage reward signals effectively.

Method: The authors characterize verification signals along three dimensions: scalability, faithfulness, and robustness. They study four reward constructions and conduct in-depth analysis and experiments on core challenges of reward design.

Result: Experiments show that targeted verification design can effectively suppress reward hacking, improve task completion quality, and achieve significant gains across multiple internal and public benchmarks.

Conclusion: No fixed reward function can remain effective as policy capability continues to grow; and verification must co-evolve with the generator.
...
<details>
  <summary>Details</summary>
Motivation: The energy sector requires live data retrieval, specialized regulatory and market knowledge, and multi-step quantitative reasoning, but current evaluations are limited to static knowledge recall.
...
Conclusion: The paper highlights the importance of evaluating LLM agents in high-stakes professional domains like energy market analytics and provides a framework for future research and reproducibility.
...
TL;DR: The paper discusses the limitations of current evaluation benchmarks for multimodal large language models (MLLMs) and identifies gaps in assessing their ability to integrate information across modalities.


<details>
  <summary>Details</summary>
Motivation: The rapid advancement of MLLMs' capabilities has not been matched by progress in evaluating them, with existing benchmarks limited to isolated tasks and not assessing multimodal integration.

Method: The paper reviews and examines current evaluation methods and benchmark taxonomies for MLLMs to identify gaps.

Result: The paper identifies gaps in current evaluation benchmarks, including temporal-spatial coherence, physical world understanding, multimodal consistency, and selective attention.
...
TL;DR: The authors introduce OpenFinGym, a unified environment for developing and evaluating quantitative-finance agents, addressing the limitations of current evaluation methods.


<details>
  <summary>Details</summary>
Motivation: Current evaluation methods for large language model agents in quantitative finance are fragmented and often overlook financial relevance, failing to reveal weaknesses in generalization and decision-making.

Method: The authors propose OpenFinGym, a unified gym environment that covers multiple tasks such as forecasting, market generation, trading, and fraud detection, with features like automated task construction and scalable agent rollouts.
...
TL;DR: The paper discusses compositional behavioral leakage (CBL) in prompt-composed agentic systems, where changing one prompt module affects others without a shared variable or executable dependency.


<details>
  <summary>Details</summary>
Motivation: The authors aim to formalize and investigate the phenomenon of CBL, which causes interference between modules sharing a context window in agentic systems.

Method: The authors propose a reusable three-channel protocol to probe CBL on a deployed job-evaluation agent (Claude Sonnet 4.6) through 144 trials, perturbing non-focal modules along volume, content, and form.

Result: The results show that only the content channel produces a detectable paired effect (Cohen's d = 0.63, bootstrap 95% CI excluding zero), indicating a sub-threshold regime invisible to standard QA but compounding across decisions.

Conclusion: The authors conclude that CBL is a distinct failure mode orthogonal to known agent-failure axes and establish cross-module interference measurement as a requirement for prompt-composed agent evaluation.
...
TL;DR: The paper discusses the concept of accelerating returns in technological progress, but argues that it doesn't solve the central problem of scientific discovery. It proposes the Qualitative Engine for Science (QES) to address this issue.


<details>
  <summary>Details</summary>
Motivation: The authors are motivated by the limitations of accelerating returns in technological progress, specifically its inability to address the central problem of scientific discovery. They also emphasize the importance of preserving human wisdom and understanding in the face of AI advancements.

Method: The paper provides a simple mathematical interpretation of the accelerating returns thesis and argues that it doesn't address the central problem of scientific discovery. It also references recent results from ARC-AGI-3 and the work of Demis Hassabis.

Result: The paper concludes that the Kurzweil theory of accelerating returns helps explain why quantitative capability may accelerate, but it doesn't solve the central problem of scientific discovery. It proposes the QES as a response to this missing capacity.

Conclusion: The paper positions the Qualitative Engine for Science (QES) as a response to the central problem in scientific discovery that acceleration alone does not solve. Its value lies in preserving, organizing, and making accessible the processes of scientific discovery as a form of human wisdom.
...
<details>
  <summary>Details</summary>
Motivation: Standard chain-of-thought on moral dilemmas often fails by collapsing stakeholders and suppressing uncertainty, leading to unreliable decisions.

Method: The authors propose NoT, a system that structures chain-of-thought into five sections: protagonist, stakeholders, two-step consequences, uncertainty, and commitment.

Result: NoT significantly reduces stakeholder collapse from up to 31% to under 1% and uncertainty suppression from up to 72% to 1-24% across various models and generators.

Conclusion: The proposed system provides a more reliable and auditable substrate for agentic deployment by externalizing stakeholders, consequences, and uncertainty grounding each commitment.
...
TL;DR: Proposed a Geometry-Aware Monte Carlo Tree Search (MCTS) framework to solve extremal problems in combinatorial geometry, achieving new best-known results for five out of six problems.
...
Method: Geometry-Aware MCTS framework that enforces geometric constraints through incremental updates to feasible action space, exploits geometric symmetries for efficient search.

Result: Established new best-known computational results on five out of six problems, finding configurations of size roughly 1.8n for Max-N3IL and 0.95n for Smallest Complete Set problem.
...
TL;DR: The paper proposes an agentic aggregator framework for managing electric bus fleets, integrating optimization and agent-based methods to improve coordination between fleet operations and grid interactions.


<details>
  <summary>Details</summary>
Motivation: The increasing complexity of operational tasks in electric bus fleets, which require coordination between multiple factors such as service reliability, battery state-of-charge, charger availability, and electricity prices.
...
Result: The results show that agentic aggregation can support adaptive fleet-grid coordination, maintaining feasible schedules and improving the use of charging and V2G flexibility, but also reveal a trade-off between reducing operational complexity and extracting value from the public transport operator.

Conclusion: The deployment of agentic aggregators in public-fleet contexts requires transparent coordination modes, auditable tariff-setting, and explicit value-sharing rules to ensure fairness and effectiveness.
...
TL;DR: The paper discusses a problem faced by 'oracle' AI that estimates probabilities of future events, and proposes a self-referential alternative where the oracle reports a credal set that is unbiased and self-consistent.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the self-reference problem in 'oracle' AI that estimates probabilities of future events, which can change the probability it reports once its answer is learned and acted upon.

Method: The method involves using the Knaster--Tarski fixed-point theorem on the complete lattice of closed credal sets to find a canonical, nontrivial member of the credal set that is self-consistent with the consequences of being learned.

Result: The result is a credal set that is simultaneously unbiased and self-consistent, which collapses to the classical point answer for non-performative questions, and for a binary event, the canonical answer is an interval under a natural hull-factoring assumption.

Conclusion: The conclusion is that the proposed approach provides a self-referential alternative for 'oracle' AI to report probabilities, which is purely lattice-theoretic and can be extended from a binary event to an arbitrary random variable.
...
TL;DR: The paper evaluates confidence interval methods for performance metrics in text classification, particularly in social science applications with small to moderate sample sizes and nested texts within individuals.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the reporting of uncertainty measures, specifically confidence intervals, in text classification studies, as current practices are inconsistent and often inaccurate.

Method: The paper uses simulations to evaluate various confidence interval methods, including Wald interval, percentile bootstrap, Agresti-Coull, Wilson, Clopper-Pearson, and pseudo-count regularized bootstrap, under conditions typical of social science text classification.

Result: The results show that default methods are often inaccurate, while Agresti-Coull, Wilson, Clopper-Pearson, and pseudo-count regularized bootstrap methods are more accurate. The hierarchical bootstrap is more accurate than the cluster bootstrap when individuals produce a moderate number of texts.

Conclusion: The paper aims to provide guidance on appropriate interval estimation methods to improve the transparency of machine learning applications and encourage greater attention to validation sample size at the design stage.
...
TL;DR: The paper examines the limitations of supervised deep learning in achieving symbolic-level syllogistic reasoning, finding that data-driven systems cannot match symbolic reasoning even with increased training data and time.
...
TL;DR: A new benchmark, MKG-RAG-Bench, is introduced to evaluate retrieval in multimodal knowledge graph RAG (MKG-RAG) systems, highlighting the challenges of retrieval in heterogeneous multimodal knowledge.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks overlook the challenges of retrieval in MKG-RAG, and retrieval is a critical bottleneck due to the heterogeneity and difficulty of aligning multimodal knowledge.

Method: MKG-RAG-Bench is constructed from two multimodal knowledge graphs, and includes question-answering datasets for controlled evaluation of retrieval and generation, using an LLM-based curation pipeline.

Result: Experiments show that effective multimodal retrieval is challenging yet crucial for end-to-end MKG-RAG performance, and retrieval quality strongly determines generation outcomes.
...
TL;DR: This paper presents an AI-based system, auto-psych, that automates theory discovery in computational cognitive science by collecting human data through crowdsourced experiments and generating explanatory theories of human behavior.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the bottleneck of data collection in AI-based scientific automation, particularly in computational cognitive science, and to explore the feasibility of automated data collection and theory discovery.

Method: The method used in this paper involves an agent-based system, auto-psych, which uses nested discovery loops to generate theories of human behavior. The inner loop conjectures, fits, and critiques probabilistic cognitive models, while the outer loop designs experiments, launches them online, and analyzes the data.

Result: The results show that the system can quickly and reliably recover ground-truth theories from synthetic data and, in three independent sequences of human experiments, finds theories that fit the data better than theories generated from the scientific literature.

Conclusion: The conclusion of this paper is that automated data collection and theory discovery are feasible in computational cognitive science, and that AI-based systems like auto-psych can be effective in generating explanatory theories of human behavior.
...
TL;DR: The authors propose a new approach to medical AI, focusing on developing clinical AI skills and a runtime governance architecture, called Clinical Harness, to enable accountable and persistent AI capabilities in clinical care.


<details>
  <summary>Details</summary>
Motivation: Current medical AI systems are limited to isolated models, which are not sufficient for clinical care that requires persistent and accountable capabilities over time.

Method: The authors propose a runtime governance architecture, called Clinical Harness, for registering, orchestrating, guarding, and monitoring AI-enabled clinical capabilities. They also introduce the concept of clinical AI skills, which can be knowledge-driven, data-driven, or physics-enhanced.

Result: The authors demonstrate the effectiveness of their approach using osteoporosis as an example, showing how clinical AI skills can support lifecycle care under runtime governance.

Conclusion: The proposed approach has the potential to improve the accountability and persistence of AI capabilities in clinical care, enabling more effective and reliable medical AI systems.
...
TL;DR: The study compares the problem-solving behavior of large reasoning models (LRMs) and humans, finding that while both show similar patterns across different problems, they diverge when looking at individual problems, with LRMs spending more 'tokens' on problems they get wrong and humans spending less time on problems they get wrong.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this study is to understand and compare how large reasoning models (LRMs) and humans approach and solve problems, specifically focusing on the relationship between problem-solving effort and accuracy.
...
Result: The results show that humans and LRMs exhibit similar patterns in terms of response time and difficulty across different problems (registration). However, when looking at individual problems (allocation), LRMs spend more 'tokens' on problems they get wrong, while humans spend less time on problems they get wrong. This dissociation holds under various conditions and is not observed in a non-thinking baseline.

Conclusion: The study concludes that the human pattern can be interpreted as 'engagement versus abandonment,' where people persist with problems they expect to solve and give up on others. In contrast, the LRM pattern is driven by 'uncertainty,' where the model generates more tokens when it is unsure and likely to fail. This highlights a fundamental difference in how humans and LRMs approach problem-solving.
...
TL;DR: This tutorial paper provides a step-by-step guide to using NeuraDock Agent, an open-source EEG agent for Alpha dynamics and visual cognitive-load analysis.
...
Conclusion: The tutorial enables readers to create a transparent path from EEG files to real-time visual cognitive-load prototypes, closing the offline-to-online gap.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of general language models not being aware of specific sensor capabilities, implemented algorithms, and justified conclusions, particularly in low-channel EEG analysis where incorrect interpretations can easily occur.

Method: The method involves separating a deterministic local EEG engine from a hardware-aware language layer, where the LLM receives a compact, allowlisted summary and a versioned context pack that describes the hardware, workflows, result fields, and scientific limits.

Result: The results show that the system produces identical structured results over multiple repetitions, preserves local artifacts under various failure scenarios, and demonstrates boundary-awareness in a benchmark test with 288 outputs.
...
TL;DR: The paper proposes a framework for interpreting AI systems as agents by combining radical interpretation and mechanistic interpretability to understand their beliefs, desires, and meanings.


<details>
  <summary>Details</summary>
Motivation: The motivation is to ensure safety and trust in deployed AI systems by understanding their goals and detecting potential deception.

Method: The method involves developing criteria for representationalist and interpretationist approaches and tying them to tests that current interpretability methods can perform.

Result: The result is a holistic approach that recognizes the interdependence of beliefs, desires, and propositional structure in AI systems, and suggests that mechanistic interpretability can help measure both.

Conclusion: The conclusion is that interpreting AI systems requires a holistic approach that considers the joint constraints of beliefs, desires, and propositional structure, and that mechanistic interpretability can provide leverage for understanding AI systems.
...
TL;DR: The authors propose PMDformer, a model for long-term time series forecasting that captures shape similarities across patches and variables, outperforming existing state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Long-term time series forecasting is crucial in fields like energy management, finance, and traffic prediction, but accurately modeling shape similarities across patches and variables remains challenging due to scale differences.

Method: The authors introduce patch-mean decoupling (PMD) to separate trend and residual shape information, and propose Trend Restoration Attention (TRA) and Proximal Variable Attention (PVA) to effectively model long-range dependencies and capture cross-variable relationships.

Result: Extensive experiments indicate that PMDformer outperforms existing state-of-the-art methods in stability and accuracy across multiple LTSF benchmarks.

Conclusion: The proposed PMDformer model effectively captures shape similarity in long-term forecasting scenarios, demonstrating improved stability and accuracy.
...
<details>
  <summary>Details</summary>
Motivation: Early detection of cirrhosis in hepatitis C patients is crucial to prevent complications, and machine learning has shown promise in disease diagnosis.

Method: Four machine learning algorithms (Random Forest, Gradient Boosting Machine, Extreme Gradient Boosting, and Extra Trees) were trained on a dataset of 2038 Egyptian patients with 28 attributes to diagnose cirrhosis.
...
Conclusion: Machine learning can effectively detect cirrhosis in hepatitis C patients, with the Extra Trees model showing high performance in this study.
...
TL;DR: A novel framework called EvoOptiGraph is proposed to automate optimization modeling from natural language using large language models (LLMs).


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenges of training corpora lacking structural diversity and static data generation pipelines in automating optimization modeling from natural language with LLMs.

Method: EvoOptiGraph represents MILPs as attributed bipartite graphs and applies evolutionary operators to generate diverse instances. It uses a two-stage training approach: supervised fine-tuning and reinforcement learning with verifiable rewards.

Result: EvoOptiGraph significantly outperforms larger generalist models, agentic methods, and specialized baselines in accuracy, executability, and generalization on six public datasets.
...
TL;DR: A new framework for validating and tracing AI-generated decisions in telescope scheduling improves reliability, executability, and reduces errors.


<details>
  <summary>Details</summary>
Motivation: AI-based decision-making in telescope scheduling faces challenges due to inconsistent data references, reasoning errors, and non-executable decisions, particularly in high-reliability observational tasks.

Method: A multi-level validation and traceable reasoning framework is proposed, integrating data validation, logical consistency checks, and constraint verification to filter and correct invalid decisions.

Result: The framework improves executability and reliability of AI scheduling, reduces loss of transient opportunities, and demonstrates a feasible pathway for applying AI to high-reliability astronomical observation scheduling.

Conclusion: The proposed framework enhances the reliability and executability of AI-generated decisions in telescope scheduling while maintaining flexibility, making it suitable for high-reliability observational tasks.
...
TL;DR: The paper proposes an automated system to dispatch emails to relevant WhatsApp groups based on their content, using large language models (LLMs) to analyze email contents and route them to intended groups, aiming to improve productivity and reduce cognitive load.


<details>
  <summary>Details</summary>
Motivation: The manual process of handling emails and forwarding their contents to intended recipients is error-prone, time-consuming, and leads to losses in productivity and increased stress.

Method: The proposed system uses agents querying LLMs to analyze email contents and route them to relevant WhatsApp groups, without relying on labeled datasets.

Result: The system provides several benefits, including enhanced productivity and a reduction in the cognitive load associated with reading emails.

Conclusion: The paper concludes that the proposed system offers a smooth flow of information within an organization, facilitating timely dissemination of information to relevant groups.
...
TL;DR: The paper presents a novel methodology that integrates large language models, statistical techniques, and human-AI collaboration to improve multilingual customer feedback analysis in public sector organizations.


<details>
  <summary>Details</summary>
Motivation: The need to enhance analysis of service feedback in public sector organizations, particularly tax administrations, where trust and compliance depend on fair and effective service delivery.

Method: A framework that combines fine-tuned, quantized large language models with expert oversight to produce accurate, computationally efficient, and context-aware analyses.

Result: The proposed approach shows stronger alignment with expert judgments than baseline models, and reduces LLM fabrication while improving reliability and relevance of generated insights.

Conclusion: The results demonstrate the practicality of combining large language models with human expertise to support scalable, evidence-based decision-making in public sector organizations, contributing to responsible AI systems that enhance service quality, responsiveness, fairness, and public trust.
...
<details>
  <summary>Details</summary>
Motivation: The need for formal policy enforcement in high-stakes domains, where existing approaches lack either formal guarantees or scalability.

Method: An LLM-based generator-critic loop to autoformalize policies from natural language documents, prompts, and tool descriptions, producing policies in the Cedar Policy Language.

Result: The autoformalized policies cover more of the source natural-language specification than hand-coded symbolic enforcement on the MedAgentBench benchmark.

Conclusion: The proposed pipeline offers a scalable solution for formal policy enforcement, bridging the gap between informal policy specifications and formal guarantees in agent safety.
...
TL;DR: A new framework, SkillDisCo, is proposed to enable agents to learn reusable procedural skills from successful traces, improving efficiency and success rates in task execution.


<details>
  <summary>Details</summary>
Motivation: Agents often solve similar tasks from scratch, leading to high reasoning costs and long execution traces. The goal is to find a way to reuse successful execution structures across tasks.

Method: The SkillDisCo framework distills reusable subgraphs from successful traces in FSM-defined scenarios and compiles them into executable procedural skills.
...
Conclusion: The proposed SkillDisCo framework effectively represents shared experience as reusable execution structures, leading to improved efficiency and success rates in task execution.
...
TL;DR: The paper presents NebulaExp, a transparent post-training pipeline for large language models, improving their reasoning and human preference following capabilities.


<details>
  <summary>Details</summary>
Motivation: Existing works on post-training alignment lack transparency in data construction, filtering rules, and training recipes, hindering community reproducibility and model optimization.

Method: The authors propose a fully transparent pipeline, NebulaExp, built on Qwen3-8B-base, with two model branches: general instruct and complex reasoning-specialized models. They curate a large corpus of SFT samples and design an end-to-end data processing stack.

Result: The proposed approach, NebulaExp-Ins-SFT, improves the average benchmark score from 55.01 to 60.99. GRPO reinforcement learning further elevates the score to 61.85. For the Reasoning branch, medium-difficulty GRPO RL improves the average reasoning score from 73.88 to 75.17.
...
TL;DR: The paper questions the necessity of chain-of-thought (CoT) reasoning in safety guardrails, finding that a lightweight encoder (LeanGuard) matches the accuracy of a reasoning guard while using significantly less compute.


<details>
  <summary>Details</summary>
Motivation: The authors aim to investigate whether a safety guardrail needs to reason, as CoT makes guardrails heavy and slow, which may not be suitable for on-device deployment.

Method: The authors train a lightweight bidirectional encoder and a reasoning guard on the same corpus, then remove the reasoning from the guard while keeping everything else fixed, and compare their performance.

Result: The LeanGuard encoder achieves an average F1 of 82.90 ± 0.26 over public benchmarks, matching a much larger reasoning guard while using only a single forward pass and ~100x less inference compute.

Conclusion: The findings suggest that CoT may not be necessary for moderation, and that current guardrail benchmarks may not be challenging enough to reward reasoning, implying a need to reevaluate the importance of CoT in safety guardrails.
...
TL;DR: The paper proposes a new few-shot learning framework called Kalman Prototypical Network (KPN) for fault detection in combined-cycle gas turbines (CCGTs) with limited labeled data.


<details>
  <summary>Details</summary>
Motivation: The complexity of CCGT systems and the scarcity of labeled fault data make fault detection challenging, necessitating a novel approach for effective diagnosis.

Method: The KPN framework models class prototypes as latent stochastic states in a dynamic system, utilizing a high-fidelity Modelica-based simulation for generating synthetic data.

Result: KPN outperforms conventional few-shot learning methods in accuracy and stability for leak fault detection tasks in CCGT systems, under varying conditions.

Conclusion: The proposed KPN framework offers improved training convergence and generalization for CCGT fault detection, making it suitable for real-world applications with limited labeled data.
...
TL;DR: LithoDreamer, a physics-informed World Model framework, is proposed for computational lithography, achieving state-of-the-art performance in forward evolution and inverse planning.
...
Method: LithoDreamer formulates the 'Layout-Mask-Resist Image-After Development Image (ADI)' pipeline as a decision-driven multi-step evolution system, using a contrastive variational optimization paradigm for interpretable intervention optimization.

Result: LithoDreamer achieves state-of-the-art performance in forward evolution and inverse planning, with a publicly available lithography dataset.

Conclusion: LithoDreamer effectively addresses the limitations of existing models in capturing the physical process of lithography, providing a more accurate and interpretable approach for computational lithography.
...
TL;DR: The paper presents a latent dynamical model for analyzing cardiac magnetic resonance imaging (CMR) data to predict heart failure risk. The model encodes bi-ventricular anatomy and full-cycle cine motion, and its performance is evaluated on a large cohort of participants.


<details>
  <summary>Details</summary>
Motivation: Conventional risk models use only a few image-derived indices from selected cardiac phases, whereas CMR captures rich spatiotemporal information about ventricular structure and motion. The authors aim to develop a model that utilizes this rich information to improve heart failure risk prediction.

Method: The authors propose a latent dynamical model that uses heart-rate-aware neural ordinary differential equation (ODE) dynamics and a graph-based mesh autoencoder to reconstruct anatomically consistent 3D+t ventricular motion. A covariate-conditioned prior defines the expected end-diastolic latent state.

Result: The model improved the stratified C-index from 0.704 to 0.785 in a held-out evaluation subset, compared with 0.764 for seven established cardiac markers. The proposed model gave the best trade-off between reconstruction fidelity, generative realism, and downstream prognostic performance.

Conclusion: The results suggest that continuous full-cycle modeling of ventricular motion provides informative cardiac phenotypes beyond conventional CMR summaries, and has the potential to improve heart failure risk prediction. However, external validation in more representative patient cohorts is required before clinical use.
...
TL;DR: A novel AI system, AHOIS, enables autonomous scientific discovery by constructing, challenging, and revising physical explanations through Socratic midwifery and closed-loop experimentation.


<details>
  <summary>Details</summary>
Motivation: The automation of scientific discovery needs to advance beyond procedural execution of workflows fixed by human designers to achieve true epistemic autonomy.

Method: AHOIS uses a multi-agent approach with a physics-critic agent that interrogates hypotheses through causal questioning, constraint checking, and falsification-criteria formulation.

Result: AHOIS autonomously proposed and validated a hypothesis, discovered task-adaptive measurement strategies, and diagnosed failure modes on a real optical platform, achieving classification accuracies of 76.97% on MNIST and 83.17% on Fashion-MNIST.

Conclusion: AHOIS establishes a route towards evidence-grounded, self-correcting autonomous discovery in complex physical environments, demonstrating the potential for AI systems to achieve epistemic autonomy.
...
TL;DR: The paper proposes a framework for optimizing scientific discovery by treating research as a meta-optimization problem, where the evaluation criteria itself is optimized.


<details>
  <summary>Details</summary>
Motivation: The authors argue that while large language models (LLMs) have enabled automated exploration of the vast state space of theories and experiments, simultaneous modification of the evaluation criteria is equally important.

Method: The authors propose 'consensus objective aggregation', where LLM-generated objective functions are combined via correlation-weighted voting, yielding a stable, self-correcting evaluation criterion.

Result: The framework is applied to algorithm discovery for 3-SAT problems, resulting in a reduced baseline scaling from ~N^{2.51} to ~N^{1.33} and a ~67x speedup on the largest instances tested.

Conclusion: The authors hope that this problem-agnostic framework will considerably aid scientific discovery.
...
TL;DR: EGG, an Expert-Guided Agent Framework, is proposed to automate high-performance GPU kernel generation for large language models by incorporating expert optimization principles.


<details>
  <summary>Details</summary>
Motivation: The development of high-performance GPU kernels for large language models heavily relies on manual tuning by domain experts, which is time-consuming and costly. Recent LLM-based approaches for automating kernel generation struggle to achieve both correctness and high performance due to the lack of domain-specific optimization guidance.

Method: The EGG framework decomposes kernel generation into two hierarchical stages: 1) algorithmic structure design and 2) hardware-specific tuning. A stage-aware multi-agent collaboration mechanism is designed for inter and intra-stage context management.
...
Conclusion: The proposed EGG framework effectively addresses the challenge of automating high-performance GPU kernel generation for large language models by leveraging expert optimization principles, leading to significant performance improvements over existing approaches.
...
<details>
  <summary>Details</summary>
Motivation: The significant inference latency of powerful diffusion models and the quality degradation of existing 'cache-then-forecast' schemes at high acceleration ratios.
...
Conclusion: The proposed ResilPhase framework effectively alleviates the quality degradation issue of existing schemes, achieving high-fidelity results under high acceleration ratios.
...
TL;DR: The paper discusses enhancing long-running language agents by introducing 'memory depth', which enables persistent goal-conditioned behavior even after the working context is unloaded. It proposes EVAF, a mechanism for consolidating experiences into a small parametric store, and evaluates its effectiveness.


<details>
  <summary>Details</summary>
Motivation: Long-running language agents require more than just memory access; they need to retain goal-conditioned tendencies even when the working context is unloaded. Current retrieval systems can fetch past facts but do not ensure that relevant experiences continue to influence behavior after context unloading.

Method: The authors introduce the 'loop-drift protocol', a stress test to evaluate the persistence of goal-conditioned behavior under interference. They propose EVAF (surprise- and valence-gated LoRA consolidation mechanism) and evaluate it across models like GPT-2, TinyLlama, and Mistral-7B. EVAF selectively consolidates experiences into a small parametric store.

Result: EVAF shows strong performance in goal persistence and post-unload recovery (0.812--0.904) with minimal parametric writes. Retrieval systems excel in shallow factual recall (accuracy 0.956--0.973) but not in goal persistence. The mechanism controls indicate that selective consolidation can be factored into controllable dimensions: selection and actuation.

Conclusion: The study demonstrates that selective parametric consolidation (EVAF) provides 'memory depth' complementary to retrieval access. This enables long-running language agents to maintain goal-conditioned behavior even after context unloading, highlighting the importance of durable parametric storage for AI agents.
...
TL;DR: Proposed a method for LLMs to automatically incorporate factual knowledge from a knowledge base during token generation, improving factual accuracy and transparency.


<details>
  <summary>Details</summary>
Motivation: To enable LLMs to leverage external knowledge bases for factual accuracy without requiring retraining, and to provide transparency and explainability in LLM outputs.

Method: Training a model to produce special tokens that trigger queries to a knowledge base, allowing for dynamic incorporation of factual knowledge during token generation.

Result: The method improves factual grounding in both short and long-form generation, and enables factual revisions through knowledge base edits.

Conclusion: The proposed method allows LLMs to automatically pull in factual knowledge from a knowledge base, improving factual accuracy, transparency, and explainability, while also enabling smaller models to achieve comparable factual accuracy to larger models.
...
TL;DR: This study analyzes Heart Rate Variability (HRV) indices in 40 healthy adults to improve HRV's clinical utility, finding specific indices that accurately represent HRV components.


<details>
  <summary>Details</summary>
Motivation: The study aims to evaluate HRV indices in healthy individuals to improve HRV's clinical utility, as research on HRV parameters in healthy individuals is limited and no gold standard exists.

Method: The study uses computational methods for signal processing and data analysis to analyze time, frequency, and nonlinear indices in 40 healthy adults.

Result: The study finds that specific HRV indices (ApEn, IRRR, HRVi, SD2, MADRR, or rMSSD) are best suited for accurately representing HRV components and enhancing its clinical and research relevance.

Conclusion: The study provides insights into HRV indices in healthy individuals, identifying specific indices that can improve HRV's clinical utility and facilitating cross-study comparisons.
...
TL;DR: The paper introduces the Capability Frontier, a method to evaluate the performance of multiple LLMs across various benchmarks, considering both model specialization and multiple generations. It finds that current benchmarks underestimate real-world LLM capabilities, particularly in heterogeneous data distributions.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks for LLMs have limitations, as they report accuracy for a single model on a single run, which underestimates real-world capabilities, especially in heterogeneous data distributions.

Method: The authors introduce the Capability Frontier, a Pareto frontier that characterizes the best achievable performance at each cost level under optimal selection across models and generations.

Result: The study finds that correcting for single-model evaluation yields a 54% error rate reduction, and additionally correcting for single runs yields an 82% improvement, with SOTA accuracy matched at 85% cost reduction.

Conclusion: The paper concludes that collective LLM capabilities are substantially underestimated, with implications for evaluation and deployment in data-heterogeneous, multi-domain settings.
...
TL;DR: A new framework called CASOP is proposed to construct and evaluate context-specific optimization pipelines for order fulfillment in warehouses.


<details>
  <summary>Details</summary>
Motivation: The manual picker-to-goods warehouses face challenges in making interconnected decisions for order fulfillment. Existing studies have limitations in evaluating algorithms for isolated subproblems or fixed subproblem combinations, and there is a need for a general mechanism to determine applicable algorithm configurations.

Method: The CASOP framework consists of a modular repository of algorithms, semantic data and algorithm cards, a taxonomy, a pipeline synthesizer, and a pipeline evaluator. It identifies applicable algorithms for a given warehouse context, composes valid optimization pipelines, and assesses their performance.

Result: The framework was demonstrated on 7 benchmark instance sets covering four problem classes, resulting in 1,063,044 valid pipelines. The framework supports researchers and practitioners in designing, automatically synthesizing, and selecting valid, high-performing algorithmic pipelines for warehouse operations.

Conclusion: The CASOP framework provides a general mechanism for constructing and evaluating context-specific optimization pipelines for order fulfillment in warehouses, which can help improve the efficiency of warehouse operations.
...
TL;DR: This study introduces an AI-assisted framework for Life Cycle Assessment (LCA) interpretation, enabling the translation of environmental improvement opportunities into strategic pathways under uncertainty.


<details>
  <summary>Details</summary>
Motivation: The interpretation phase of LCA often lacks structured mechanisms for translating quantified improvement opportunities into actionable strategic pathways.

Method: A perspective-conditioned retrieval-augmented generation framework was developed, incorporating multi-perspective retrieval and controlled synthesis using large language models.

Result: The framework was demonstrated through a hydrogen-enabled diesel reduction use case, showing potential for disciplined translation of impact results into strategic pathways.

Conclusion: The approach supports more evidence-grounded interpretation and implementation-oriented decision-making in LCA studies, particularly for technologies with large-scale deployment potential.
...
TL;DR: AgentX is a multi-agent system that automates the recommendation algorithm iteration process, allowing for self-evolving development and experimentation at a large scale.
...
Method: AgentX operates as a closed-loop system with four stages: Brainstorm Agent generates proposals, Developing Agent translates proposals into production-ready code, Evaluation Agent conducts online rollout and evaluation, and Harness Evolution layer updates the agents through semantic-gradient updates.

Result: AgentX enables autonomous generation, implementation, evaluation, and learning from recommendation experiments at a scale and pace that no manual workflow can sustain, making the system self-improving.

Conclusion: AgentX fundamentally restructures the production function of recommendation algorithm iteration, allowing for industrialized research loops and overcoming the structural execution bottleneck.
...
TL;DR: A novel framework, TAVR-VLM, is proposed to improve the interpretability of Multimodal Large Language Models (MLLMs) in Transcatheter Aortic Valve Replacement (TAVR) planning by reducing diagnostic hallucinations.
...
Method: The method introduced is Risk-Conditioned Causal Grounding Attention (R-CGA), which creates a model-internal 'Risk → Region → Word' structural grounding pathway to constrain token-level grounding within a risk-defined support mask.

Result: TAVR-VLM achieves an AUROC of 0.896, boosts CIDEr to 0.936, and reduces the hallucination rate to 8.1%, establishing a new state-of-the-art on a 1,482-patient cohort.

Conclusion: The proposed TAVR-VLM framework improves the interpretability of MLLMs in TAVR planning by significantly reducing diagnostic hallucinations and achieving better performance metrics.
...
TL;DR: The paper introduces a synthetic clinical notes pipeline and dataset to support the development of clinical AI tools while protecting patient data privacy.
...
Method: The authors use a modular pipeline combining structured patient generation, semi-structured patient journey simulation, and unstructured clinical note generation using large language models, along with LLM-based validation and augmentation steps.

Result: The result is a dataset of 70 synthetic patients, each with 20-50 clinical notes spanning a full hospital journey, provided at multiple levels of validation for balancing realism and scalability.

Conclusion: The dataset supports the development, testing, and evaluation of clinical AI systems without relying on real patient data, thereby mitigating privacy risks.
...
<details>
  <summary>Details</summary>
Motivation: The authors identify a limitation in traditional embedding-based retrieval methods, which often prioritize returning the highest-scoring items. Instead, they focus on pattern-preserving attribute retrieval, where the goal is to find more items that satisfy a target attribute while staying within a fine-grained pattern.
...
Result: The authors report significant gains in the intersection metric over a pretrained generative retriever, with improvements across four attribute domains under item- and pattern-holdout protocols. The results show that metric-ordered DiT improves the intersection metric, and HPPO further improves it.

Conclusion: The paper concludes that the proposed framework, MO-DiT+HPPO, effectively addresses the challenge of pattern-preserving attribute retrieval. The authors demonstrate the effectiveness of their approach through extensive experiments, including metric-predictor validation, order ablations, and comparisons with other methods.
...
TL;DR: Proposed ReTeX, a framework that recovers task-expert performance from a single merged model by predicting parameter offsets, achieving over 95% individual-expert performance recovery and improving generalization to unseen tasks.


<details>
  <summary>Details</summary>
Motivation: Static merging of multi-task models suffers from parameter interference, while dynamic merging models require costly storage and loading of redundant expert components.

Method: ReTeX predicts affine transformations (additive offsets) to undo parameter interference and recover task-expert performance, using a router-free task identifier based on SVD subspace signatures.

Result: ReTeX recovers over 95% of individual-expert performance in vision and NLP domains, improves generalization to unseen tasks, and adaptively interpolates seen expert knowledge for out-of-distribution tasks.

Conclusion: ReTeX effectively recovers task-expert performance from a single merged model, improving multi-task model merging and generalizability.
...
TL;DR: The paper identifies task insensitivity in large language models, proposes Task-Perturbed NLL Optimization to improve task sensitivity and out-of-distribution generalization, and shows that it preserves stable attention to task tokens.


<details>
  <summary>Details</summary>
Motivation: Large language models have weak out-of-distribution generalization due to task insensitivity, where models apply learned patterns and fail to solve the task at hand when faced with similar but distinct tasks.

Method: The authors propose Task-Perturbed NLL Optimization, a lightweight contrastive regularizer that encourages action dependence on task instructions.

Result: The proposed intervention improves task sensitivity and out-of-distribution generalization while preserving more stable attention to task tokens.

Conclusion: The paper concludes that task insensitivity is a key source of failure in large language models' out-of-distribution generalization and that Task-Perturbed NLL Optimization can mitigate this problem.
...
TL;DR: The study investigates the effectiveness of Chain-of-thought (CoT) training in language-model agents, finding that it improves the model's ability to predict actions directly from prompts, rather than enhancing the reasoning process.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this study is to understand what CoT training actually improves in language-model agents: whether it enhances the model's ability to change its action through generated reasoning or its ability to predict the action directly from the prompt.

Method: The study compares 'prompt actions' (predicting action without CoT) with 'CoT actions' (predicting action with CoT) across different checkpoints and investigates the impact of selectively masking action-token supervision on a fraction of training examples.

Result: The results show that prompt-action quality improves substantially across checkpoints, and CoT training does not widen the advantage of CoT reasoning. Instead, it helps improve the quality of prompt actions. Later checkpoints are less likely to revise the action in response to CoT, indicating greater reliance on the prompt. The intervention of selectively masking action-token supervision improves out-of-domain generalization.

Conclusion: The study concludes that CoT training primarily improves the model's ability to predict actions directly from prompts, rather than enhancing the reasoning process. This improvement can be achieved by selectively masking action-token supervision, which also enhances out-of-domain generalization.
...
<details>
  <summary>Details</summary>
Motivation: The need for active visual perception in dynamic 3D environments, where the camera must decide what to observe and how to compose the observation.

Method: The Look-Before-Move framework, which separates observation specification from motion execution, and consists of three stages: Semantic Observation Contract, Monte Carlo Viewpoint Search, and Semantic Trajectory Grounding.
...
Conclusion: The paper presents a novel camera planning framework for embodied AI in dynamic 3D environments, enabling active visual perception and decision-making, and showcasing its effectiveness through experiments.
...
TL;DR: The paper explores whether LLMs can benefit from visualizing counterfactual events to improve their reasoning abilities, proposing Einstein World Models (EWMs) as a blueprint for LLM-based reasoning systems that incorporate visual-temporal rollouts.


<details>
  <summary>Details</summary>
Motivation: The motivation is to investigate if visualising counterfactual events can complement language as a mechanism for complex thought and to improve the reasoning abilities of LLMs.

Method: The proposed method is Einstein World Models (EWMs), a blueprint for LLM-based reasoning systems that integrate visual-temporal rollouts into the reasoning trace, allowing LLMs to reason in ways that text alone may not support well.

Result: The paper does not provide specific results, but it proposes a new approach (EWMs) to extend the capability of LLMs for tool calling into the domain of visual thought experiments.

Conclusion: The conclusion is that visualising counterfactual events can potentially complement language as a mechanism for complex thought, and EWMs offer a promising approach to improve the reasoning abilities of LLMs by incorporating visual-temporal rollouts.
...
Conclusion: AURORA-AI is a practical and theoretically motivated approach for resilient and human-centric AI deployment.
...
TL;DR: This paper proposes a semantic early-stopping method for multi-agent large language model loops, which stops the loop when consecutive draft embeddings stop changing in meaning and the answer's quality stops improving.


<details>
  <summary>Details</summary>
Motivation: The current fixed iteration cap (max_iterations) used to terminate multi-agent large language model loops is inefficient, as it over-spends tokens on easy inputs and truncates hard ones.

Method: The proposed method uses a semantic early-stopping approach, which monitors the cosine distance between consecutive draft embeddings and the answer's measured quality, and stops the loop when these metrics indicate no further improvement.

Result: The empirical study on multi-hop retrieval-augmented question answering (HotpotQA) shows that the proposed judge-free semantic stopper reduces operational tokens by 38% relative to max_iterations at parity quality, while the full quality-gated variant is counter-productive due to high per-round judging cost.

Conclusion: The paper concludes that the proposed semantic early-stopping method can efficiently terminate multi-agent large language model loops while maintaining answer quality, and reframing the problem from 'when to stop' to 'which round is best'.
...
Result: The paper recommends the centroid index (CI) for cluster-level evaluation and pair-set index (PSI) or clustering accuracy (ACC) for point-level evaluation, depending on the specific needs of the analysis.

Conclusion: The conclusion is that the choice of external validity index depends on the level of evaluation (cluster-level or point-level) and the specific requirements of the analysis, such as the need for a normalized score or equal weighting of points.
...
<details>
  <summary>Details</summary>
Motivation: The challenge of making effective use of accumulated interaction experience for LLM agents in multi-step interactive environments.
...
Conclusion: JERP provides a more effective way to utilize accumulated interaction experience, aligning the rule pool with the evolving policy and allowing stable behaviors to be absorbed into the model.
...
TL;DR: The paper introduces a new protocol called PAVE for evaluating the root cause analysis (RCA) capabilities of large language models (LLMs). The protocol provides step-wise causal annotations, and the results show that LLMs struggle with RCA, succeeding in only 20.7% of cases.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the limitations of existing RCA datasets, which only label the root cause and not the propagation path connecting it to the observed symptom. The authors aim to create a more rigorous evaluation protocol for LLM-based RCA agents.

Method: The authors introduce PAVE, a step-wise labeling protocol that leverages known interventions from fault injection to reconstruct causal propagation paths. They apply PAVE to create OpenRCA 2.0, a cross-system RCA benchmark with step-wise causal annotations for LLM agents.

Result: The results show that across 11 frontier LLMs, recovering the exact root-cause set succeeds in only 20.7% of cases on average. However, when relaxing the criterion, agents identify at least one correct root-cause service in 76.0% of cases, but ground that service in a verified causal propagation path to the observed symptom in only 61.5%.

Conclusion: The authors conclude that step-wise causal ground truth is the missing piece for trustworthy LLM-based RCA agents, and that outcome-only evaluation hides the failure mode of LLMs in RCA tasks.
...
TL;DR: The paper proposes TOPS, a training-free and model-agnostic module for pruning visual tokens in multimodal large language models (MLLMs), which improves efficiency while preserving performance.


<details>
  <summary>Details</summary>
Motivation: The efficiency of MLLMs is limited by the large number of visual tokens, which introduces substantial computational overhead, and existing token pruning methods have limitations.

Method: The authors formulate visual token pruning as constructing Token Optimal Preservation Sets and propose TOPS, a module that applies three fundamental principles: Task Relevance, Information Coverage, and Semantic Diversity.

Result: TOPS outperforms prior methods under diverse pruning settings on 7 MLLM backbones and 14 benchmarks, and can remove 77.8% of visual tokens while preserving 100.0% and 100.6% performance on LLaVA-NeXT models.

Conclusion: The paper suggests that pruning redundant visual tokens can sometimes mitigate hallucination and inspire future lightweight MLLM design.
...
TL;DR: The paper introduces a new mechanism called 'process harness' for integrating legacy workflows into Agentic Business Process Management (Agentic BPM) without replacing the underlying workflow engine.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this work is to uplift legacy workflows into Agentic BPM while preserving the existing workflow engine, allowing for the addition of reasoning, adaptation, and oversight capabilities.

Method: The authors develop the Task-Decision-Flow (TDF) model, which defines a policy-governed agentic layer around a deterministic workflow engine. They decompose LLM reasoning across three agent types: TaskAgent, DecisionAgent, and FlowAgent.

Result: The authors present CUGA FLO as the design and implementation realization of the TDF model and demonstrate it on a loan approval workflow, showcasing the integration of the three agent types and hook-driven regulatory override.

Conclusion: The process harness successfully reconciles imperative and normative requirements, enabling the combination of deterministic workflow execution with policy-framed agentic autonomy.
...
TL;DR: GAversary, a hybrid Genetic Algorithm, generates effective adversarial attacks on NLP models with high semantic similarity and low perturbation.


<details>
  <summary>Details</summary>
Motivation: Deep learning models are vulnerable to adversarial inputs, particularly in NLP, which can have significant real-world consequences.

Method: GAversary uses a Genetic Algorithm with GloVe embeddings to propose word replacements, treating the target model as a black box.

Result: GAversary substantially reduces target model's accuracy on test data (e.g., from 76.8% to 5.8%) compared to BAE and A2T attacks.

Conclusion: GAversary is an effective method for generating adversarial attacks on NLP models, with a good balance between accuracy reduction and semantic similarity.
...
TL;DR: BINEVAL is a framework for evaluating LLM outputs by decomposing criteria into binary questions, providing interpretable scores and transparent feedback.


<details>
  <summary>Details</summary>
Motivation: Current LLM evaluation methods are expensive, slow, or produce opaque scores; there is a need for a more efficient, transparent, and diagnostic evaluation framework.

Method: BINEVAL uses a meta-prompt to generate fine-grained evaluation questions, which are answered by an LLM to produce transparent question-level feedback and calibrated overall scores.
...
TL;DR: The paper introduces EO-WM, a video diffusion transformer for multispectral Earth Observation (EO) forecasting that incorporates a physically informed conditioning framework to better respond to changed weather forcing.
...
Method: The authors propose EO-WM, a video diffusion transformer that separates meteorological forcing into a climatological baseline, weather anomalies, and cumulative physical stress signals, and accumulates anomalous forcing over time to capture sustained heat and drought stress.

Result: EO-WM reduces the error in predicted Normalized Difference Vegetation Index (NDVI) decline amplitude by a relative 5.63% and improves directional hit rate by a relative 7.80%, while remaining competitive on standard pixel-level metrics.

Conclusion: The paper presents EO-WM as a more effective approach for EO forecasting, particularly in responding to changed weather forcing, and introduces new benchmarks for evaluating weather-response behavior.
...
TL;DR: This paper compares simulation-based inference (SBI) with Markov chain Monte Carlo (MCMC) for Bayesian calibration of mechanistic epidemiological models, showing SBI is faster and efficient.


<details>
  <summary>Details</summary>
Motivation: The motivation is to find a more efficient method for Bayesian calibration of mechanistic epidemiological models, as MCMC can be computationally expensive.

Method: The method used is simulation-based inference (SBI) using neural posterior estimation, compared with Markov chain Monte Carlo (MCMC).

Result: SBI provides comparable posterior and predictive performance to MCMC but with substantially reduced computational runtime.
...
TL;DR: The study examines how candidates manipulate large language models (LLMs) used in automated résumé screening through 'prompt injection', and finds it can improve rankings but loses effectiveness as more candidates use it.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this study is to understand the impact of strategic manipulation by job applicants on automated hiring systems that use large language models (LLMs) for screening and ranking résumés.

Method: The study uses controlled experiments to investigate the effects of 'prompt injection', which involves adding subtle self-promotional text to résumés to influence LLM evaluations without introducing new qualifications.

Result: The results show that prompt injection can reliably improve applicant rankings when résumé quality is similar and few candidates manipulate. However, its effectiveness decreases as more candidates use it, and it can allow lower-quality candidates to outrank higher-quality ones when candidate quality varies, raising fairness concerns.

Conclusion: The study concludes that LLM-based screening is most vulnerable to manipulation when it is rare and candidate quality differences are small, highlighting the need for strategies to mitigate such vulnerabilities in automated hiring systems.
...
<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to understand the limitations of multi-model LLM systems, such as routing, voting, cascades, fusion, and mixture-of-agents, which are used to beat single-model accuracy.

Method: The paper uses a theoretical approach to derive a bound on the accuracy gain of multi-model systems, and an empirical approach to analyze data from 67 models from 21 providers, including open-ended mathematics and execution-graded code.

Result: The paper finds that the gain of multi-model systems is capped by a quantity called beta, which is the rate at which every model is wrong on the same query, and that combining models rarely beats the single best model without a strong query-level routing signal.

Conclusion: The paper concludes that gains in multi-model systems come from models failing on different questions, not from adding more models, and that the usual diagnostic, average pairwise error correlation rho, is not sufficient to identify the limitations of multi-model systems.
...
TL;DR: This paper proposes a language-based digital twin framework for personalized cognitive health monitoring, leveraging large language models to mimic conversational behavior and predict cognitive scores.
...
Method: A language-based digital twin framework using large language models (LLMs) and a multi-head conditional variational autoencoder (cVAE) to measure reconstruction quality and predict cognitive scores.

Result: The digital twin preserves identity-specific characteristics, achieves reconstruction and MoCA prediction errors comparable to real data, and outperforms baseline GPT-generated responses.

Conclusion: The proposed framework highlights the potential of language-based digital twins as a scalable and non-invasive approach for personalized and continuous cognitive health monitoring.
