...
TL;DR: A billion-parameter generative model for synthesizing chest radiographs is introduced, capable of producing high-fidelity images that are indistinguishable from real ones.


<details>
  <summary>Details</summary>
Motivation: Existing radiographic AI models suffer from poor generalisation across patient subpopulations, institutions, and acquisition settings. Controlled synthesis of chest radiographs can help diversify clinical datasets and evaluate model robustness.

Method: A generative foundation model with over 1.3B parameters was trained on 1.2M radiographs and clinical metadata for 1.6T tokens, enabling controllable radiograph generation and editing.

Result: The model significantly advances the state of the art in radiograph synthesis fidelity, producing images that are indistinguishable from real radiographs to clinical experts.

Conclusion: The introduced model has the potential to improve the generalisation and robustness of radiographic AI models by providing a means to diversify clinical datasets and evaluate model performance.
...
Result: LEAP achieves significant improvements in student model performance, including 90.1% accuracy on ImageNet-100, +12.24% improvement compared to baseline, and +3.84% and +7.75% improvement for instance retrieval tasks on Oxford and Paris datasets, respectively. LEAP also enables 25.1% savings in training FLOPs and 21% savings in training time.
...
<details>
  <summary>Details</summary>
Motivation: Precise 3D spatial orchestration in text-to-video generation is challenging, especially for multi-object scenes with entangled semantic layout and temporal dynamics. Existing depth-conditioned models require labor-intensive, dense guidance for dynamic events.

Method: LooseControlVideo uses sparse, oriented 3D boxes as a proxy for high-level layout and trajectory authoring. It fine-tunes a Wan 2.2 backbone on a video dataset annotated with DNOCS, enabling localized refinement with minimal disruption to the global scene context.

Result: LooseControlVideo significantly outperforms existing baselines, achieving 1.2x to 3x improvement in Trajectory Error, 2x improvement in Rigid Motion Consistency, and 1.5x to 2x increase in Occlusion Accuracy.

Conclusion: LooseControlVideo provides an effective solution for precise 3D spatial orchestration in text-to-video generation, leveraging oriented 3D primitives as a good geometric prior for complex, multi-agent video authoring.
...
TL;DR: The paper proposes ImageWAM, a world action model framework that uses pretrained image editing models for robot action prediction, outperforming video-based models with less computational cost.


<details>
  <summary>Details</summary>
Motivation: The authors identify limitations in video-based world action models, including costly inference, irrelevant details, and potential errors in long-horizon future imagination.
...
Conclusion: The paper concludes that image editing can be an effective alternative to video-based world-action modeling, providing a more efficient and accurate approach to robot action prediction.
...
TL;DR: This paper proposes PerceptionDLM, a multimodal diffusion language model for efficient parallel region perception, achieving competitive performance and substantial speed improvements for multi-region perception tasks.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal large language models (MLLMs) rely on autoregressive generation, limiting their efficiency for perception tasks that require captioning multiple regions.

Method: The authors propose PerceptionDLM, which leverages the parallel decoding nature of diffusion language models (DLMs) and introduces efficient prompting and structured attention masking for simultaneous perception of multiple masked regions.
...
Conclusion: The results highlight the potential of multimodal diffusion language models for efficient, parallel visual perception, and the authors release their code, models, and datasets.
...
Method: The method proposed is Mix-QVLA, a task-evidence-aware mixed-precision PTQ framework that evaluates quantization impact on task-relevant evidence and guides mixed-precision bit allocation.

Result: The results show that Mix-QVLA improves the accuracy-efficiency trade-off, reduces memory from 15.4 GB to 4.1 GB, retains 96.3 average success rate, and achieves a 1.52x inference speedup.

Conclusion: The conclusion is that Mix-QVLA is an effective framework for improving the accuracy-efficiency trade-off of low-bit VLA deployment.
...
TL;DR: The paper proposes Language-Instructed Vision Embeddings (LIVE), a method that uses language to dynamically guide a vision encoder, producing task-centric embeddings without task-specific retraining.


<details>
  <summary>Details</summary>
Motivation: Current vision foundation models are trained as static feature extractors, relying on large downstream models for task adaptation. The authors aim to shift this burden by leveraging language as high-level guidance.

Method: The proposed method, LIVE, uses language to dynamically guide the vision encoder, generating task-centric embeddings at inference time without requiring task-specific retraining.

Result: LIVE achieves significant improvements, including a 34-point increase on MMVP, outperforming vision-language models with more parameters on visual question answering, and generalizing to unseen instructions and tasks.

Conclusion: The paper presents a novel paradigm for vision foundation models, using language to guide the vision encoder, resulting in more controllable and generalizable representations, and paving the way for adaptive, instruction-driven visual intelligence.
...
TL;DR: The paper presents GB-LSR, a method for continuous image reconstruction that achieves high performance and efficiency by using a fixed-grid local spectral representation with a shared global bandwidth.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to improve continuous image reconstruction by developing a more efficient and effective method that can handle high-quality image reconstruction at any continuous coordinate.

Method: The method proposed in this paper is GB-LSR (Global-Bandwidth Local Spectral Representation), which partitions the image domain into non-overlapping square patches and uses a shared convolutional-encoder feature to predict coefficients for a truncated Fourier basis.

Result: The results show that GB-LSR outperforms other methods (LIIF, LTE, WIRE) by 2.8-3.6 dB PSNR and 0.11-0.15 LPIPS on a standardized benchmark, while running at roughly one-quarter of the slowest baseline's inference cost.

Conclusion: The conclusion of this paper is that GB-LSR is an efficient and effective method for continuous image reconstruction, achieving high performance and efficiency with a shared global bandwidth, and that it can be applied to various image reconstruction tasks, including native-reconstruction and arbitrary-scale super-resolution.
...
TL;DR: The paper proposes learning an asynchronous schedule for multi-representation diffusion models to improve visual synthesis. It introduces a flexible parametric class for the schedule and learns it using a fast joint probe. The learned schedule substantially improves convergence speed and final quality on ImageNet.


<details>
  <summary>Details</summary>
Motivation: The performance of multi-representation diffusion models depends critically on the asynchronous schedule that determines when each representation is denoised. The authors aim to learn this schedule to improve visual synthesis.

Method: The authors formulate asynchronous flow matching over multiple representation spaces and use a schedule-corrected objective. They instantiate the schedule with a flexible parametric class and learn it using a fast joint probe with less than 1% additional training compute.

Result: The learned schedule substantially improves both convergence speed and final quality on ImageNet 256x256. The 200-epoch model reaches FID 1.05, matching the 800-epoch SFD-XL baseline with 4x less training. Training to 600 epochs further improves to FID 1.02, outperforming the 1B-parameter SFD-XXL result of FID 1.04 while using a smaller model.

Conclusion: The paper proposes a method to learn an asynchronous schedule for multi-representation diffusion models, which substantially improves convergence speed and final quality on ImageNet. The learned schedule achieves state-of-the-art results with less training.
...
<details>
  <summary>Details</summary>
Motivation: The practical importance of simultaneously transforming both motion and location in video editing remains largely unexplored, and existing methods have limitations in achieving robust motion-location editing.

Method: TeleMorpher is a one-shot framework that disentangles the protagonist and background via pre-trained models, introduces training-free pose warping, and injects the warped motion video into a baseline motion editor during inference.

Result: TeleMorpher achieves superior performance across both quantitative and qualitative measurements, demonstrating its effectiveness in simultaneous motion and location editing.

Conclusion: The proposed TeleMorpher framework provides a more controllable and precise approach to simultaneous motion and location editing in videos, with experiments showing its effectiveness in achieving high-quality results.
...
TL;DR: The paper presents Vortex, a multimodal video retrieval system that achieved excellent performance in the Ho Chi Minh City AI Challenge 2025.
...
Method: Vortex integrates adaptive keyframe extraction, multimodal metadata generation, hybrid retrieval strategy, and a multi-stage temporal search mechanism.

Result: The system achieved a score of 79.6/88 (90.5%) in the Preliminary Round and 'Excellent' overall performance in the Final Round.
...
TL;DR: A novel framework for composed image retrieval in fashion domain using multi-modal large language model and two-stage fine-tuning strategy.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for composed image retrieval face limitations due to scarce annotated data and simplistic negative sampling.

Method: Integrating a multi-modal large language model (LLaVA) to generate attribute-aware triplets and introducing a two-stage fine-tuning strategy to enhance contrastive learning.

Result: Experimental results demonstrate enhanced compositional reasoning and improved fine-grained retrieval behavior.

Conclusion: The proposed framework shows feasibility and potential for fashion retrieval.
...
TL;DR: The paper introduces NEST, a dataset for understanding narrative structure in long videos, and presents baselines for event extraction tasks.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for long video understanding focus on retrieval tasks, but do not evaluate understanding of narrative structure, such as how low-level actions form events and how events interact over time.

Method: The authors introduce NEST, a dataset of 1005 full-length movies annotated with 102 multimodal narrative events, and propose baselines for four event extraction tasks: event trigger detection (ETD), event localization (EL), event argument extraction (EAE), and event relation extraction (ERE).

Result: The benchmark proves highly challenging for event discovery tasks (ETD < 8%, EL < 6%, EAE < 11%), but event relation extraction is more tractable (35.45% F1 zero-shot, 44.42% F1 fine-tuned).

Conclusion: The paper highlights the difficulty of understanding narrative structure in long videos and provides a new dataset and baselines for future research.
...
TL;DR: A novel approach for synthesizing human images from a single image with arbitrary poses and views using a conditional denoising diffusion model.
...
Method: A conditional denoising diffusion model that utilizes 3D human priors (normal map and color prompt) to generate high-quality human images with complex poses.
...
Conclusion: The authors propose a novel approach for one-shot novel view and pose human image synthesis, which significantly outperforms previous methods and demonstrates better generalization ability.
...
Conclusion: QueryGaussian offers a scalable and efficient solution for open-vocabulary 3D instance retrieval, addressing the challenges of memory and computational costs in large-scale scenes.
...
TL;DR: The paper proposes an end-to-end framework for generating adversarial camouflage patterns that are wearable and maintain stable attack performance in real physical environments with changing viewpoints, poses, and lighting conditions.


<details>
  <summary>Details</summary>
Motivation: Adversarial camouflage in the physical world is challenging, especially under UAV reconnaissance with dynamic targets and extreme illumination variations. Existing methods have limitations in generalizing to dynamic viewpoints or producing natural textures.

Method: The proposed method integrates UV-volume rendering with a diffusion-based texture generator and includes an illumination color consistency estimator to ensure environmental realism. A multi-scale dynamic training strategy enhances robustness against viewpoint shifts and body deformation.
...
Conclusion: The paper presents a novel framework for adversarial camouflage generation that overcomes existing limitations and demonstrates effectiveness in real-world scenarios with varying conditions.
...
TL;DR: The paper proposes Occ-VLM, a novel framework for 3D scene understanding that uses a single 2D vision encoder and operates purely on posed RGB images, achieving accurate geometric perception and robust vision-language reasoning.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for 3D scene understanding typically rely on explicit 3D inputs or additional 3D geometry encoders, which structurally decouple 3D geometric perception from 2D semantics learned via vision-language pre-training.

Method: The proposed Occ-VLM framework reconstructs 3D scene occupancy as an auxiliary geometric prior, which is used to spatially associate foreground 2D tokens with 3D space, and then decodes them using a Large Language Model (LLM) for unified scene understanding.
...
Conclusion: The proposed Occ-VLM framework offers a unified approach to 3D vision-language representation, demonstrating accurate geometric perception and robust vision-language reasoning.
...
TL;DR: A novel framework, HypOProto, is proposed for interpretable Left Ventricular Filling Pressure (LVFP) classification using echocardiography images, achieving state-of-the-art performance while providing transparency.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning approaches for LVFP classification are black-box and lack clinical interpretability, and standard methods rely on operator-dependent Doppler-derived ratios which may not be available in resource-limited settings.

Method: HypOProto uses a frozen, explainable foundation model backbone with a hyperbolic, ordinal prototype-based framework, and introduces a novel Hyperbolic Prototype Angular Separation (HyperPAS) loss to enforce inter-class prototype separation.
...
Conclusion: The proposed framework, HypOProto, provides an interpretable solution for LVFP classification, addressing the limitations of existing black-box approaches and standard methods, and has the potential to improve clinical decision-making.
...
TL;DR: A method called ParaScale is introduced to transfer camera motion from a reference video to a target video while preserving the motion's effect at different scales.


<details>
  <summary>Details</summary>
Motivation: The challenge of transferring camera motion from a reference video to a target video at different scales, as naive reuse of the recovered trajectory can result in imperceptible or exaggerated motion.

Method: ParaScale, a plug-and-play module that reads the Parallax Number (Pi) from any reference video and re-realizes it against the target scene's own depth, preserving the motion's effect at different scales.

Result: ParaScale keeps the realized parallax on the identity line and cuts the Parallax Consistency Error (PCE) by more than 3x over uncalibrated transfer with no loss of visual fidelity across scale regimes spanning four orders of magnitude.

Conclusion: ParaScale enables scale-faithful transfer of camera motion between videos at different scales, preserving the intended visual effect.
...
TL;DR: A new metric, Conditional-Composition Domain Match (CCDM), is proposed to evaluate the effectiveness of synthetic datasets for object detection tasks, achieving a perfect Spearman correlation of 1.0 with downstream performance.


<details>
  <summary>Details</summary>
Motivation: The increasing use of synthetic data to supplement limited real datasets for training computer vision models, particularly for object detection tasks, which require dense annotations and are computationally expensive to evaluate.

Method: Proposal of a pre-computable metric family, CCDM, to serve as a proxy for the relative utility of candidate synthetic training sets for downstream detection.
...
Conclusion: The proposed CCDM metric provides a reliable and efficient way to evaluate the effectiveness of synthetic datasets for object detection tasks, reducing the need for computationally expensive and time-consuming downstream model training.
...
<details>
  <summary>Details</summary>
Motivation: Accurate segmentation of thin, tortuous anatomical structures is challenging due to low contrast, discontinuities, and class imbalance.
...
Conclusion: CSWinUNETR is a general-purpose backbone for 2D and 3D thin-structure segmentation, improving accuracy and preserving fine-grained details.
...
TL;DR: The paper introduces 3D-PLOT-LLM, a 3D multimodal large language model that can address, name, and reason about parts of 3D objects, achieving state-of-the-art results on several benchmarks with a significantly smaller number of trainable parameters.


<details>
  <summary>Details</summary>
Motivation: Existing 3D multimodal large language models (3D MLLMs) cannot address, name, or reason about parts of 3D objects. Prior attempts to add part-awareness have incurred substantial parameter costs.

Method: The authors reorganize the input token stream to make parts directly addressable through the LLM's vocabulary. They propose a Marker-Space Refinement (MSR) module and a new model, 3D-PLOT-LLM, which partitions the frozen point encoder's patches into locally coherent regions and inserts learnable markers.

Result: 3D-PLOT-LLM achieves state-of-the-art results on several benchmarks, including PartVerse-QA, 3DCoMPaT-GrIn, and Objaverse, with a significantly smaller number of trainable parameters (under 1M).

Conclusion: The paper presents a novel approach to enable 3D MLLMs to reason about parts of 3D objects, achieving impressive results with a more efficient and scalable solution.
...
TL;DR: The paper proposes a framework to compress event streams from event cameras into 'neural events' that are highly informative, reducing data throughput while preserving performance in object detection and classification tasks.
...
Method: The paper proposes a framework to re-tokenize event streams into a small set of highly informative 'neural events', each representing a local spatio-temporal context window with a discrete learnable code.

Result: Networks trained on neural events achieve performance on par with or surpassing state-of-the-art approaches in object detection and classification while reducing the event rate by a factor of 2.0.

Conclusion: The proposed framework effectively compresses event streams and maintains or improves performance in downstream tasks, addressing the challenge of processing high-volume, low-information event data from event cameras.
...
TL;DR: The paper proposes OTCHA, a module that refines patch tokens for multi-view medical image classification by aligning them with learnable latent hub tokens using optimal transport.


<details>
  <summary>Details</summary>
Motivation: Existing methods for multi-view imaging have limitations, such as allowing irrelevant content to contaminate the fused embedding, which reduces robustness under varying view configurations.

Method: The proposed OTCHA module uses optimal transport to compute a token-wise matching confidence between patch tokens and learnable latent hub tokens, and gates hub-mediated message passing and weights a representation alignment loss.
...
Conclusion: The proposed OTCHA module provides a robust and effective approach for multi-view medical image classification by refining patch tokens and aligning them with learnable latent hub tokens.
...
TL;DR: The paper proposes ViCoStream, a framework for real-time streaming video inference with low latency, achieving 134 FPS and <50 ms TTFT on a single A100 GPU.


<details>
  <summary>Details</summary>
Motivation: The need for real-time deployment of streaming VideoLLMs while maintaining low query latency and high video-ingestion throughput.
...
Result: ViCoStream achieves 134 FPS video throughput and less than 50 ms TTFT on a single A100 GPU with accuracy close to full-history baselines.

Conclusion: ViCoStream enables efficient and accurate real-time streaming video inference, suitable for deployment in streaming applications.
...
TL;DR: A new deep learning framework, PSCT-Net, is proposed for reconstructing 3D CT scans from sparse bi-planar X-rays, reducing radiation risks in pediatric patients.


<details>
  <summary>Details</summary>
Motivation: The motivation is to reduce radiation risks associated with CT scans in pediatric patients while accurately diagnosing craniofacial abnormalities.

Method: PSCT-Net uses a geometry-aware framework with differentiable back-projection, Attention-Guided Projection (AGP-3D) module, and Bidirectional Mamba (BiM-3D) module to reconstruct 3D CT scans from sparse bi-planar X-rays.

Result: The proposed framework, PSCT-Net, can alleviate depth ambiguity and improve the accuracy of osseous boundaries in 3D CT reconstructions from sparse bi-planar X-rays.

Conclusion: PSCT-Net offers a promising low-dose alternative for diagnosing pediatric craniofacial abnormalities, mitigating the risks associated with radiation exposure from CT scans.
...
TL;DR: The paper proposes a Multimodal Concept Bottleneck Model (MM-CBM) that enhances the interpretability of deep learning networks by aligning image and text embeddings with natural concepts, achieving up to 51.26% accuracy improvement on four benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing Concept Bottleneck Models (CBMs) are limited in their ability to generalize beyond predefined classes and risk non-concept information leakage, hindering their interpretability and performance.

Method: The proposed MM-CBM utilizes dual Concept Bottleneck Layers (CBLs) to align both image and text embeddings into interpretable features, enabling zero-shot classification and image retrieval in an interpretable way.
...
Conclusion: The proposed MM-CBM effectively addresses the limitations of existing CBMs, providing a more interpretable and accurate approach for vision tasks like zero-shot classification and image retrieval.
...
TL;DR: SurgVista, a new surgical world model, improves autonomous surgery by generating realistic future frames with better spatial interaction and temporal fidelity.
...
Result: SurgVista outperforms state-of-the-art methods in visual quality, temporal consistency, and interaction fidelity, with greater gains as the prediction horizon grows.

Conclusion: SurgVista presents a significant advancement in surgical world models, enabling more accurate and reliable autonomous surgery simulations.
...
TL;DR: The authors propose a LRU-based restoration network with a semantic modulating unit (SMU) for single-image super-resolution, achieving a balance between performance and efficiency.


<details>
  <summary>Details</summary>
Motivation: The static parameterization and single-scan method of Linear Recurrent Unit (LRU) limits its applicability to 2D vision tasks, such as single-image super-resolution.

Method: The authors propose a LRU-based restoration network with a semantic modulating unit (SMU) that plays three key roles: LRU modulation, spatial categorization, and feature enhancement through learned prototype.

Result: The proposed method quantitatively and qualitatively surpasses recent state-of-the-art methods, achieving superior performance with computational complexity on par with existing methods.

Conclusion: The proposed LRU-based restoration network with SMU achieves a harmonious balance between performance and efficiency in single-image super-resolution tasks.
...
<details>
  <summary>Details</summary>
Motivation: Endoscopic video analysis is crucial for gastrointestinal diagnosis and interventions, but video sequences are often degraded, distracting clinicians and disrupting downstream tasks.
...
Result: The GPVAE variants reduced image reconstruction RMSE by 21.9% on average and up to 26.1% relative to VAE baselines on the C3VDv2 colonoscopy dataset.

Conclusion: The GPVAE framework provides effective restoration of endoscopic videos with uncertainty-aware reconstruction and a confidence signal for restored frames.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of current approaches that either impose significant inference overhead or lack fine-grained geometric constraints, and to provide inherent interpretability.

Method: The method proposed is SpatialSV, which employs task-oriented visual supervision to lift 2D visual features into explicit 3D representations, including depth maps, camera poses, and point clouds.
...
Conclusion: The conclusion is that SpatialSV provides a transparent and interpretable way to enhance the spatial intelligence of MLLMs, with potential to leverage unlabeled visual data for scalable spatial representation learning.
...
<details>
  <summary>Details</summary>
Motivation: The current reinforcement learning-based methods for multimodal video reasoning rely on simplistic and inflexible reasoning-length control strategies that fail to adapt to the model's evolving competence, leading to suppressed exploration and inefficient decoding.
...
TL;DR: STORM, a spatial-aware token reduction framework, is proposed to improve the performance of structurally enhanced Mamba variants by maintaining structural integrity during token reduction.


<details>
  <summary>Details</summary>
Motivation: The spatially agnostic nature of existing token reduction methods causes severe performance collapse in structurally enhanced Mamba variants.

Method: STORM reformulates token reduction into a structured operation on spatial units, enforcing localized constraints to maintain grid topology and neighborhood coherence.

Result: STORM achieves state-of-the-art pruning accuracy across diverse vision Mamba backbones under training-free settings, with up to 63.3% accuracy recovery on VMamba and only 1.0% accuracy drop on PlainMamba.
...
TL;DR: The paper presents a study on using unsupervised algorithms to improve data annotation efficiency for complex semantic segmentation problems in industrial materials science, achieving an approximate reduction of 78% in labeling time.
...
Method: The study leverages unsupervised computer vision algorithms as a pre-annotation step to accelerate the labeling process for semantic segmentation tasks, specifically for annotating each pixel of high-resolution images in materials science.

Result: The use of unsupervised algorithms reduced the labeling time from 170 hours to 37 hours, achieving an approximate reduction of 78%, and created the largest public steel microstructure segmentation dataset to date.

Conclusion: The study demonstrates the effectiveness of unsupervised algorithms in accelerating the labeling process for complex semantic segmentation problems, contributing a fully annotated, high-resolution dataset and a Deep Learning model trained on this dataset to the field.
...
TL;DR: The paper proposes a 'triangular consistency' constraint for optical flow estimation, which improves accuracy across various settings without adding computational cost.


<details>
  <summary>Details</summary>
Motivation: The authors aim to improve optical flow estimation by introducing a universal constraint that can be applied across different network architectures, supervision types, and datasets.

Method: The authors propose a 'triangular consistency' constraint, which composes two flows to induce a third flow and enforces consistency among them, applicable to image-pair, multi-frame, and data augmentation settings.
...
Conclusion: The proposed 'triangular consistency' constraint is a universal, plug-and-play component that improves optical flow estimation accuracy without adding computational overhead or requiring additional annotations.
...
TL;DR: A novel framework, DiffMath, is proposed for generating handwritten mathematical expressions without explicit spatial supervision, leveraging LaTeX's hierarchical structure.


<details>
  <summary>Details</summary>
Motivation: The challenge of generating handwritten mathematical expressions (HMEG) due to complex 2D layouts and long-range structural dependencies, and the high cost of explicit spatial supervision.
...
Result: DiffMath produces structurally consistent handwritten expressions, outperforming existing methods and improving downstream OCR models' accuracy through synthetic data augmentation.

Conclusion: DiffMath eliminates the need for positional supervision in HMEG, achieving superior performance and improving OCR accuracy.
...
TL;DR: The paper introduces Timage, a paradigm that improves multimodal large language models' spatial reasoning by overlaying text queries onto images, resulting in better performance on the VMCBench suite.


<details>
  <summary>Details</summary>
Motivation: Multimodal large language models often struggle with fine-grained spatial reasoning due to the lack of explicit geometric anchors in textual queries.

Method: The authors propose Timage, which uses a Constrained Schrödinger Bridge (cSB) to generate an overlay of the query text onto the image, guiding the model's attention to the correct visual coordinates.
...
Conclusion: The study demonstrates that deliberate input reconstruction can be a powerful, architecture-neutral approach to strengthening multimodal reasoning.
...
TL;DR: This paper analyzes the relationship between accuracy and confidence in medical multimodal large language models and proposes a method to improve confidence calibration.


<details>
  <summary>Details</summary>
Motivation: The elicited confidence of multimodal large language models in medical tasks often misaligns with actual accuracy, potentially leading to misdiagnosis or overlooking correct advice.
...
Conclusion: The findings highlight the importance of domain-specific calibration for multimodal large language models in healthcare, offering a more trustworthy solution for AI-assisted diagnosis.
...
TL;DR: SketchKeyAnime: a video diffusion framework generating animations from sparse key-sketch inputs.


<details>
  <summary>Details</summary>
Motivation: Traditional animation production relies heavily on manual drawing and iterative refinement. Existing animation methods have limitations under low-cost input conditions.

Method: SketchKeyAnime introduces a dual-branch conditioning mechanism, Sketch Cross Attention, and Adaptive Weighted Loss to generate structurally controllable, appearance-consistent, and temporally coherent animations.

Result: SketchKeyAnime outperforms baselines, reducing EDMD by 31.9% and FVD by 9.5%, achieving the best overall performance across most quantitative metrics.
...
TL;DR: The paper proposes two fixes for latent diffusion models to improve image-to-image translation, especially for downstream perception tasks, and achieves state-of-the-art results.
...
Method: The authors propose a Source-Conditioned Autoencoder (SCAE) and a Learnable Guidance Encoder (LGE) to address the bottlenecks.

Result: The proposed approach improves detection mAP by up to 2x over the latent diffusion baseline and achieves state-of-the-art FID, with generalizability to the public RASMD benchmark.

Conclusion: The paper concludes that the proposed approach can improve image-to-image translation for downstream perception tasks, and highlights the importance of multi-axis evaluation.
...
TL;DR: The paper introduces a benchmark called ROSE to test the ability of multimodal large language models (MLLMs) to act on visual information under different task contexts. The results show that MLLMs struggle to translate visual evidence into context-specific actions, with performance dropping by up to 44.5 percentage points.


<details>
  <summary>Details</summary>
Motivation: The motivation is to investigate how reliably MLLMs can turn the same visual evidence into the action required by the current context, as they are increasingly expected to act on visual information.
...
Result: The results show that across nine recent MLLMs, performance drops by as much as 44.5 percentage points from counting-oriented tasks to region-conditioned action, despite 98.8% human performance.

Conclusion: The paper concludes that there is a distinct, model-dependent bottleneck in turning shared visual evidence into context-specific actions, and that MLLMs struggle to translate visual evidence into context-specific actions.
...
Method: The SAEFS framework derives semantic anchors from WSIs via Visual Question Answering (VQA), employs a dual-stream WSI evidence extraction architecture, and uses Dirichlet-based Subjective Logic to model uncertainty.
...
Conclusion: The proposed SAEFS framework provides a domain-invariant semantic representation for computational cancer prognosis, highlighting the robustness of VQA-derived semantic features for cross-center clinical applications.
...
TL;DR: CrossFlow is a new method for generating images by directly mapping latent inputs to pixel-space images, combining efficiency of latent representations with direct pixel-space supervision.
...
Method: CrossFlow uses a cross-space flow formulation with a velocity-free one-step objective, allowing one model to act as both a one-step latent-to-pixel generator and a decoder replacement.

Result: CrossFlow-XL achieves 1.62 FID on class-conditional ImageNet-1k at $256\times256$ with one function evaluation.
...
TL;DR: The authors propose a framework that combines light field integration and vision-language models to recover scenes occluded by dense foreground vegetation, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Occlusion-robust scene recovery is challenging in natural environments with dense foreground vegetation, which severely limits visibility.

Method: The proposed framework uses light field integration to suppress foreground occlusions and a vision-language model as a conditional semantic prior to restore degraded structures and recover fine details.

Result: The framework achieves state-of-the-art performance on synthetic and real-world datasets, with the highest average SSIM across four synthetic light field benchmark scenes and strong generalization across structured and unstructured acquisition settings.
...
TL;DR: The paper introduces QG-MIL, a gated transformer aggregator that improves attention-based Multiple Instance Learning in medical imaging by addressing attention concentration and producing more stable predictions.


<details>
  <summary>Details</summary>
Motivation: Attention-based Multiple Instance Learning aggregators in medical imaging are prone to attention concentration, leading to overconfident and unstable predictions.

Method: The authors propose QG-MIL, a gated transformer aggregator that incorporates four synergistic architectural components: RMSNorm-based pre-normalization, per-head QK normalization, fine-grained attention output gating, and SwiGLU-style feed-forward modules.

Result: QG-MIL outperforms leading baselines on all six benchmarks, with an average improvement of +6.1 mean macro F1 points, and shows more distributed instance weighting and consistent cross-domain performance.

Conclusion: The proposed QG-MIL design provides a more stable and consistent approach to attention-based Multiple Instance Learning in medical imaging, addressing attention concentration and improving prediction performance.
...
TL;DR: The paper proposes a Reliability-Aware Open-Vocabulary Change Detection (ReA-OVCD) framework for flexible and reliable land cover change detection using arbitrary text prompts.


<details>
  <summary>Details</summary>
Motivation: Existing methods for Open-Vocabulary Change Detection (OVCD) suffer from a trade-off between instance-level comparison and direct pixel comparison, leading to unreliable results with boundary artifacts.

Method: The proposed ReA-OVCD framework consists of two main modules: Semantic Change Reasoning (SCR) and Boundary-aware Change Refinement (BCR). SCR reassesses changes by analyzing distributional divergence and response variation, while BCR mitigates artifacts through validating candidate regions with reliable interior pixels.
...
Conclusion: The proposed ReA-OVCD framework provides a flexible and reliable solution for Open-Vocabulary Change Detection, addressing the limitations of existing methods and achieving superior performance on multiple datasets.
...
TL;DR: Proposed Product-Unit U-Net (PU-UNet) for medical image segmentation, integrating stable product-unit residual blocks into U-Net architecture.


<details>
  <summary>Details</summary>
Motivation: Many dense prediction networks rely on additive feature transformations, but product units can explicitly model multiplicative feature interactions. However, their use is limited due to numerical instability.

Method: Proposed PU-UNet combines smooth positivity mapping with log-domain clipping for stable multiplicative feature learning.

Result: PU-UNet achieves Dice scores of 0.942, 0.959, and up to 0.925 on ISIC 2018, Kvasir-SEG, and BUSI datasets, respectively. It consistently improves Dice and IoU compared to a Residual U-Net baseline.

Conclusion: Stable product-unit residual learning can enhance U-Net-style segmentation networks with explicit multiplicative interactions.
...
TL;DR: The authors propose FUSE, a frequency-domain framework for multi-modal Re-Identification (ReID) that overcomes existing methods' limitations by focusing on mid and high-frequency structures.


<details>
  <summary>Details</summary>
Motivation: Existing multi-modal ReID methods emphasize low-frequency cues, leading to incomplete spectral representations and unstable cross-modal alignment.

Method: FUSE uses a two-stage process of spectral disentanglement and energy alignment, with a Spectral Decomposition Module (SDM) and a Cross-Modal Alignment Module (CAM).
...
TL;DR: The authors propose a new task and framework, UAV-VLN-FOV and 3DG-VLN, for precise target reaching in UAV navigation using vision-language cues.


<details>
  <summary>Details</summary>
Motivation: Current UAV vision-language navigation methods evaluate long-range target discovery and final target approach jointly, making it hard to assess a UAV's ability to accurately ground a visible target and translate vision-language evidence into precise 3D motion.

Method: The authors propose 3DG-VLN, a vision-language waypoint prediction framework guided by dynamic 3D direction cues, which processes high-resolution front-view and downward-view observations and updates target-relative direction online.

Result: Experiments show that 3DG-VLN outperforms baselines by 13.82% in success rate, and real-world trials demonstrate its potential for practical see-and-reach navigation.

Conclusion: The proposed 3DG-VLN framework improves the precision of target reaching in UAV navigation using vision-language cues, and the constructed benchmark supports the development of more advanced UAV navigation methods.
...
TL;DR: Proposed a novel variable-length tokenizer that modulates length by merging tokens, achieving a superior quality-compute trade-off for diffusion models.


<details>
  <summary>Details</summary>
Motivation: Existing variable-length tokenizers have limitations due to their dependence on token position and lack of representational alignment across lengths, hindering effective operation of a single variable-length diffusion model.

Method: A novel variable-length tokenizer that modulates length by merging tokens, using learnable global merging to ensure compatibility with diffusion transformers.

Result: Achieved a superior gFID-compute trade-off compared to prior variable-length tokenizer methods on ImageNet 256$	imes$256 generation.

Conclusion: The proposed merging-based variable-length tokenizer integrated with a diffusion transformer offers an improved quality-compute trade-off, enabling flexible balancing of quality and compute for diffusion models.
...
Conclusion: The choice of integration paradigm for visual tokens in LLMs significantly affects the transformation of visual information, alignment with the language space, and ultimately performance across tasks.
...
TL;DR: The paper introduces Holo-World, a unified controllable video world model that generates videos based on a single image, camera and object controls, and optional weather instructions.


<details>
  <summary>Details</summary>
Motivation: Current video world models have isolated controls and rely on source videos or reconstructed scenes for weather generation. The authors aim to create a model that starts from a single image and allows for explicit camera and object controls and optional weather instructions.

Method: The authors build a dataset called HoloStateData and propose Holo-World, a unified controllable video world model. Holo-World uses a Unified Scene Adapter to factorize world preservation and weather transfer into distinct parameter subspaces.

Result: The model maintains precise camera and object control with consistent scene structure while transferring scenes into diverse target weather states, outperforming video-to-video weather editing baselines.

Conclusion: The paper presents Holo-World as a significant advancement in controllable video world models, enabling the generation of videos with precise controls and diverse weather conditions from a single image.
...
TL;DR: EventVLA, a framework for long-horizon robotic manipulation, improves memory and decision-making by selectively storing key visual events, achieving a 40% higher success rate than existing methods.


<details>
  <summary>Details</summary>
Motivation: Standard Vision-Language-Action policies struggle with occluded or unobservable task-relevant cues over time, and existing memory-augmented methods have limitations such as information bottlenecks, high latency, or visual redundancies.
...
Conclusion: EventVLA effectively addresses the memory bottleneck in long-horizon robotic manipulation by selectively storing sparse, task-critical visual events, demonstrating significant performance improvements.
...
TL;DR: MakeupMirror is a diffusion-based approach for makeup transfer that preserves facial features and skin tone, outperforming Stable-Makeup in accuracy and realism.


<details>
  <summary>Details</summary>
Motivation: Current makeup transfer models, such as Stable-Makeup, have limitations in preserving identity and skin color, making high-quality virtual try-on for makeup shopping unrealistic.

Method: MakeupMirror introduces several technical innovations: facial geometry conditioning with ControlNets, region-specific makeup transfer control, skin tone-based makeup transfer modulation, and a Levenberg-Marquardt Langevin sampler.

Result: MakeupMirror improves facial recognition similarity by 60%, reduces skin tone difference by 50% compared to Stable-Makeup, with a latency of 0.7s and an expert acceptance rate of 94%.

Conclusion: MakeupMirror achieves significant progress in preserving facial features and skin tone, making it suitable for production-level virtual try-on in makeup shopping.
...
TL;DR: The paper investigates stitching artifacts in deep learning models for style transfer and their impact on downstream segmentation tasks, finding that perceptual metrics are insufficient for evaluating domain adaptation quality.


<details>
  <summary>Details</summary>
Motivation: To understand the impact of stitching artifacts on generative models for style transfer, particularly in biomedical imaging, and to evaluate the effectiveness of different stitching approaches and patch dimensionalities.

Method: The authors used cycleGAN models trained on cryo-electron microscopy datasets to investigate three stitching approaches and two patch dimensionalities (2D vs 3D), evaluating both perceptual quality and performance on downstream mitochondria segmentation.

Result: The study found that FID scores fail to detect subtle stitching artifacts, 3D models with artifact-free stitching marginally outperform 2D models, and 2D models train more stably due to larger batch sizes. Ensembling predictions from three orthogonal directions can improve low-quality volumes but provides no benefit for high-quality outputs.

Conclusion: The results highlight the importance of careful consideration and mitigation of stitching artifacts in generative models for large scientific datasets, and that perceptual metrics alone are insufficient for evaluating domain adaptation quality in biomedical imaging.
...
TL;DR: A new benchmark, WeGenBench, is proposed for evaluating text-to-image generation models across multiple dimensions, including bilingual and cross-cultural capabilities.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks struggle to comprehensively and accurately measure performance across multiple dimensions, failing to reveal inherent deficiencies of models in specific categories.

Method: WeGenBench consists of 4,000 test prompts across two primary categories, annotated with multi-dimensional tags, and uses a cross-dimensional evaluation mechanism and novel evaluation metrics integrating Vision-Language Models (VLMs).

Result: The proposed benchmark and evaluation metrics can precisely pinpoint model shortcomings in specific generation categories and provide accurate assessment outcomes with detailed reasoning trajectories.

Conclusion: WeGenBench provides a comprehensive and multi-perspective evaluation of text-to-image generation capabilities, revealing limitations in existing models and facilitating future improvements.
...
<details>
  <summary>Details</summary>
Motivation: Existing targetless LiDAR-camera calibration methods are limited by the scarcity of discriminative cross-modal features and tend to prioritize rendering quality over accurate geometry.

Method: The proposed framework uses 3D Gaussian Splatting (3DGS) as a geometric proxy, aggregates multi-view LiDAR observations for dense depth supervision, and blocks photometric gradients from updating Gaussian spatial parameters.
...
Conclusion: The proposed framework improves the accuracy of LiDAR-camera calibration by preserving metric geometry and using dense depth supervision.
...
TL;DR: Proposed EFIQA, a label-free framework for image quality assessment that produces spatial quality maps, leveraging anatomical priors for better performance and explainability.


<details>
  <summary>Details</summary>
Motivation: Deep learning-based image quality assessment methods are limited by their reliance on dataset-specific quality labels, which hinders generalization and explainability.
...
Result: EFIQA achieves better performance and explainability compared to supervised methods across benchmarks with different quality criteria, demonstrating its potential for real-world applications.

Conclusion: The proposed EFIQA framework offers a promising solution for image quality assessment, overcoming the limitations of traditional methods by providing spatial quality maps without requiring quality-related supervision.
...
TL;DR: A novel framework, FrozenDrive, is proposed for generating synthetic data in autonomous driving scenarios. It leverages a pre-trained diffusion model to produce coherent, multi-view driving scenes from text prompts, especially under adverse and rare conditions.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the challenges in generating synthetic data for autonomous driving using diffusion models. These challenges include maintaining multi-view and temporal consistency, generalizing to adverse weather and unseen configurations, and ensuring fidelity across different classes.

Method: The method introduced is FrozenDrive, a controllable generative framework. It conditions on driving-stack signals and text prompts and uses knowledge-preserving spatio-temporal attention to ensure cross-view alignment and temporal coherence. An object-focused constraint is also added to improve fidelity for rare categories.

Result: The results show that FrozenDrive can synthesize globally coherent multi-view driving scenes from text without the need for weather- or scene-specific fine-tuning. It surpasses prior baselines and significantly improves the performance of AD models, especially in night and rain conditions.

Conclusion: In conclusion, FrozenDrive presents a solution to key obstacles in using diffusion models for autonomous driving synthetic data generation. By preserving pre-trained knowledge and achieving strong consistency, it enables the creation of high-quality, scenario-targeted data that enhances the robustness of AD models.
...
TL;DR: Proposed Pixel-Level Residual Diffusion Transformer (PRDiT) for generating high-quality 3D medical volumes at voxel-level, outperforming state-of-the-art models.


<details>
  <summary>Details</summary>
Motivation: Generating high-resolution 3D CT volumes with fine details is challenging due to computational demands and optimization difficulties in existing generative models.

Method: PRDiT uses a two-stage training architecture: 1) a local denoiser (MLP-based blind estimator) on 3D patches, and 2) a global residual diffusion transformer with memory-efficient attention.

Result: PRDiT consistently outperforms state-of-the-art models (HA-GAN, 3D LDM, WDM-3D) on LIDC-IDRI and RAD-ChestCT datasets, achieving lower 3D FID, MMD, and Wasserstein distance scores.

Conclusion: PRDiT is a scalable generative framework for synthesizing high-quality 3D medical volumes directly at voxel-level, simplifying optimization and preserving subtle structures.
...
TL;DR: The paper describes a 4th-place solution to the ICRA 2026 GOOSE 2D Fine-Grained Semantic Segmentation Challenge, achieving 69.73% mIoU on a 1,815-image test set.


<details>
  <summary>Details</summary>
Motivation: The authors aim to improve fine-grained semantic segmentation by adapting and enhancing the Segment Anything Model 3 (SAM3) for the challenge.

Method: The authors adapt SAM3's image encoder with a lightweight decoder and propose two techniques: self-distillation using SAM3 as a teacher and image-level multi-scale test-time augmentation. They also identify an aggressive photometric distortion as a key improvement source.

Result: The solution achieves a composite mean Intersection-over-Union (mIoU) of 69.73% on the official 1,815-image test set.

Conclusion: The authors present a competitive solution to the ICRA 2026 GOOSE 2D Fine-Grained Semantic Segmentation Challenge, leveraging SAM3 and introducing effective techniques for improvement.
...
TL;DR: TriFlow generates compact 3D meshes with artist-like topology from input geometry conditions.


<details>
  <summary>Details</summary>
Motivation: The goal is to produce 3D meshes with structured, artist-like connectivity from input geometry conditions like signed distance fields.

Method: TriFlow uses a nearest-vertex vector field (NVF) to represent mesh topology and trains a latent flow-matching model to synthesize this field for topology generation.

Result: TriFlow achieves improved topology quality, 90% lower Chamfer Distance, and an 8x speedup compared to state-of-the-art approaches.

Conclusion: TriFlow presents a new generative approach for producing compact 3D meshes with artist-like triangle topology directly from input geometry conditions.
...
TL;DR: The paper proposes a method for efficient video instance segmentation (VIS) using sparse frame annotations, achieving comparable performance to densely annotated models.


<details>
  <summary>Details</summary>
Motivation: Current VIS methods require densely annotated frames, which is computationally expensive and requires a lot of annotations. The authors aim to reduce the need for dense annotations while maintaining performance.

Method: The authors propose Past-frames Feature Propagation (PFP) and Sparse frame Annotation VIS (SA-VIS), which uses sparse video frame labels for end-to-end training.

Result: The proposed SA-VIS shows strong improvements over the baseline on YouTube-VIS 2019/2021/2022 and Occluded VIS (OVIS), with an over 1% improvement in AP on the state-of-the-art in a limited annotations scenario.

Conclusion: The proposed method effectively bridges the gap in accuracy between training on sparsely and densely annotated video sequences, achieving comparable performance with only 1/5 of the annotations.
...
<details>
  <summary>Details</summary>
Motivation: The complexity of oropharyngeal anatomy and tumor appearance makes manual segmentation and clinical outcome prediction challenging, highlighting the need for automated analysis.
...
Conclusion: The HECKTOR 2025 challenge presents a comprehensive analysis of submitted methodologies, evaluating their performance and implications for clinical translation in automated oncology workflows and decision support systems.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to address privacy concerns raised by text-to-image models generating realistic faces of individuals when prompted with their names, and the current limitations in distinguishing memorized from fabricated faces.
...
Conclusion: The study concludes that the proposed method offers a viable solution for detecting memorization in text-to-image models, enhancing privacy and safety in AI applications.
...
TL;DR: The paper proposes ARTEMIS, a framework for video polyp segmentation with imperfect supervision, achieving state-of-the-art performance on two datasets.


<details>
  <summary>Details</summary>
Motivation: Imperfectly supervised video polyp segmentation is clinically valuable but challenging due to weak contrast, ambiguous boundaries, motion blur, and sparse pixel-level guidance.

Method: ARTEMIS uses a debate-and-judge vision-language agent to select reliable temporal anchors, which are propagated bidirectionally to refine unreliable or unlabeled frames, and trains the segmenter using temporal reliability-aware robust learning.

Result: ARTEMIS achieves state-of-the-art performance on SUN-SEG and CVC-ClinicDB-612 under scribble, point, and limited-label settings.

Conclusion: The proposed ARTEMIS framework effectively addresses the challenges of imperfectly supervised video polyp segmentation, providing a reliable and robust solution for clinical applications.
...
TL;DR: The paper explores the limitations of Multimodal Large Language Models (MLLMs) in understanding negation in Remote Sensing (RS) tasks and proposes a novel method, NeFo, to improve their performance.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the underexplored ability of MLLMs to comprehend negation, which is crucial for real-world applications, such as emergency response and evacuation planning.

Method: The paper introduces RS-Neg, a benchmark for evaluating negation understanding in RS tasks, and proposes NeFo, a test-time learning method that incorporates the logical role of negation into model optimization.

Result: The evaluation reveals that advanced RS MLLMs struggle with negation, but NeFo significantly improves their negation understanding using only about 5% unlabeled test samples.

Conclusion: The paper concludes that MLLMs have limitations in understanding negation, but proposes a novel method, NeFo, to close this gap and improve their performance in RS tasks.
...
TL;DR: The authors propose HilDA, a self-supervised pretraining framework for LiDAR backbones that leverages Vision Foundation Models (VFMs) for camera-to-LiDAR knowledge distillation, achieving state-of-the-art results on various benchmarks.


<details>
  <summary>Details</summary>
Motivation: The scarcity of annotated data for autonomous driving (AD) and the limitations of current approaches that treat VFMs as black-box teachers, not fully exploiting their layer-wise semantic structure and global context.

Method: HilDA, a self-supervised pretraining framework that combines hierarchical distillation (multi-layer distillation and global context distillation) with a temporal occupancy diffusion objective.

Result: State-of-the-art results on cross-modal distillation benchmarks and outperforming models trained via prior distillation approaches on 3D object detection, scene flow, and semantic occupancy prediction.

Conclusion: The proposed HilDA framework effectively leverages VFMs for camera-to-LiDAR knowledge distillation, leading to improved performance on various AD tasks.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of continual test-time adaptation (CTTA) when the source dataset cannot be retained due to privacy or licensing constraints, and existing source-free CTTA methods tend to become unstable under long-term distribution shift.

Method: The method proposed is DO-ALL (Distill Once, Adapt Life-Long), a plug-and-play framework that uses Dataset Distillation (DD) to produce synthetic distilled anchors summarizing the source distribution, and then matches each target sample with its most semantically aligned anchor for stable adaptation.

Result: The results show that DO-ALL consistently improves long-term robustness across CIFAR100-C, ImageNet-C, and the CCC benchmark, demonstrating the potential of leveraging DD for stable and continuous adaptation without retaining raw source data.

Conclusion: The conclusion is that DO-ALL offers a promising solution for continual test-time adaptation without requiring access to the raw source data, by effectively leveraging distilled source information to maintain model performance under evolving target domains.
...
<details>
  <summary>Details</summary>
Motivation: To create a comprehensive assessment method for drawing skills, as traditional methods are tedious and overwhelming.
...
Conclusion: The proposed method provides a feasible and effective way to assess drawing skills using computer vision, which can potentially replace traditional tedious and overwhelming methods.
...
TL;DR: The paper presents RadGrounder, a visually grounded vision-language model for radiology trained on a large-scale bilingual dataset of 1.2M CT and MR image-text pairs, achieving competitive results on visual question answering benchmarks and enabling spatially verifiable outputs.


<details>
  <summary>Details</summary>
Motivation: To train visually grounded vision-language models (VLMs) for radiology without manual spatial annotations.

Method: The authors introduce RefRad2D, a large-scale bilingual dataset, and train RadGrounder on this data, which jointly performs report generation, visual question answering, and spatial grounding.

Result: RadGrounder achieves competitive results on external VQA benchmarks (Slake, VQA-RAD) and improves open-ended VQA when adding clinical data to the training mixture.

Conclusion: The proposed approach enables spatially verifiable outputs at no cost to VQA performance, and the dataset shows transferability to downstream tasks.
...
TL;DR: DeepForestVisionV2, an upgraded camera-trap classification tool, improves accuracy and taxon identification in African tropical forests, particularly in diverse habitats and human-impacted areas.


<details>
  <summary>Details</summary>
Motivation: The original DeepForestVision tool was limited by its 35-class prediction space, which was too coarse for diverse taxa and environments, prompting the need for an ecology-driven expansion.

Method: DeepForestVisionV2 expands to 64 prediction classes, including 61 animal classes, human, vehicle, and blank, and is trained on 1,535,010 photographs and 243,354 videos from multi-country African tropical-forest projects.

Result: DeepForestVisionV2 achieves 0.86 accuracy, 0.82 macro-F1, and 0.81 balanced accuracy on a cross-country validation set, and improves taxon identification from 22 to 29 in forest-interior videos and from 4 to 9 at riverbanks.

Conclusion: DeepForestVisionV2 materially improves field utility while preserving robustness across sites, habitats, and camera-trap settings, making it a valuable tool for camera-trap monitoring in African tropical forests.
...
TL;DR: The paper investigates biases in text-to-image models, particularly in occupation-related image generation, and proposes a platform for evaluating fairness.


<details>
  <summary>Details</summary>
Motivation: Existing research indicates that image generation models are influenced by biases, which can impact productivity and creative content production.

Method: The authors evaluate five text-to-image models using a comprehensive approach, including human preference feedback and established metrics, and create a dataset of 21,140 synthetic images.

Result: The findings reveal systematic biases in text-to-image models, with established evaluation metrics only partially correlating with subjective user ratings.

Conclusion: The research emphasizes the need for including human preferences to develop fairer and more inclusive text-to-image models.
...
TL;DR: The paper introduces SPOT-E, a plug-and-play method that improves vision-language models' performance on evidence-intensive tasks by highlighting relevant visual evidence and reducing answer uncertainty.


<details>
  <summary>Details</summary>
Motivation: Vision-language models often struggle with evidence-intensive tasks because they fail to properly utilize decisive visual evidence, leading to incorrect or incomplete answers.

Method: The authors propose SPOT-E, which uses answer-span prediction entropy as a model-internal feedback signal and introduces low-entropy anchors and an entropy-shaping objective to reduce answer uncertainty while preserving high-confidence tokens.
...
Conclusion: The proposed method, SPOT-E, effectively improves the performance of vision-language models on evidence-intensive tasks by highlighting relevant visual evidence and reducing answer uncertainty.
...
TL;DR: The Single-Stage Hierarchical Rectification (SSHR) framework is proposed to address limitations in weakly supervised semantic segmentation (WSSS) methods in computational pathology. SSHR uses a Hierarchical Feature Rectification Module (HFRM) to proactively purify intermediate feature representations during the forward pass.


<details>
  <summary>Details</summary>
Motivation: Existing WSSS methods in computational pathology rely on a multi-stage paradigm, which incurs high computational training costs and suffers from error propagation. The authors aim to address these challenges through a simple yet highly effective approach.

Method: The SSHR framework introduces a Hierarchical Feature Rectification Module (HFRM) that utilizes deep global semantic context to filter out local anomalies in shallow layers, generating high-fidelity activation maps directly within a single training loop.

Result: Experiments on the LUAD-HistoSeg and BCSS datasets demonstrate that SSHR outperforms state-of-the-art multi-stage methods and reduces training duration by 2 to 5 times.

Conclusion: The proposed SSHR framework offers a simple yet highly effective approach to address limitations in WSSS methods in computational pathology, providing high-fidelity activation maps within a single training loop while reducing computational overhead and accelerating clinical translation.
...
TL;DR: U$^2$Mamba, a U-structured network, is proposed for salient object detection, achieving competitive performance through its multiscale Mamba U-blocks and hierarchical training supervision.


<details>
  <summary>Details</summary>
Motivation: Existing Mamba-based models for salient object detection fail to explore contextual information and the depth of the entire architecture.

Method: U$^2$Mamba introduces multiscale Mamba U-blocks (MMUBs) and a nested U-structure to integrate various receptive fields and collect richer contextual information. A hierarchical training supervision method is also proposed.

Result: U$^2$Mamba achieves highly competitive performance against state-of-the-art methods in salient object detection.

Conclusion: The proposed U$^2$Mamba model offers a powerful and innovative approach for salient object detection, leveraging its U-structured network and multiscale Mamba U-blocks to improve performance.
...
TL;DR: The paper presents CMDS-AD, a framework for few-shot anomaly detection that leverages multi-modal data and achieves state-of-the-art performance under extreme 1-shot setting.
...
Conclusion: The proposed CMDS-AD framework effectively addresses the challenges of few-shot anomaly detection and establishes a new state-of-the-art performance.
...
TL;DR: CUPID, a POI video deepfake detector that combines UV texture maps and Masked Autoencoder (MAE) for robust, efficient, and interpretable deepfake detection.


<details>
  <summary>Details</summary>
Motivation: Current POI deepfake detection methods struggle to combine robustness to post-processing, efficiency, and interpretability, posing a threat to modern democracies and societies.

Method: CUPID uses UV texture maps and MAE to detect deepfakes without requiring deepfake videos or specific POI in the training set.

Result: CUPID outperforms current state of the art on most datasets, achieving best overall robustness against strong downscaling and compression, and provides substantially faster inference.

Conclusion: CUPID offers a robust, efficient, and interpretable solution for POI video deepfake detection, addressing key challenges in modern deepfake detectors.
...
TL;DR: The paper proposes GEN-Guard, a framework to detect and correct performance leakage in federated surgical AI, improving model generalization across institutions.
...
Method: The proposed GEN-Guard framework consists of two components: Generalization Detection via Client-Blocked Evaluation (CBE) and Generalization Correction through Disagreement-Aware Distillation (DAD).
...
TL;DR: The paper introduces PRISM, a method for evaluating video generation that directly assesses preferences from noisy latents, achieving state-of-the-art accuracy and noise-robustness.


<details>
  <summary>Details</summary>
Motivation: Current video generation evaluation methods using clean, pixel-based reward models are disconnected from the noisy diffusion process and incur high VAE decoding costs.
...
Conclusion: The paper challenges the current paradigm of video generation evaluation and proposes PRISM as a more efficient and effective method, revealing a positive correlation between generative performance and evaluative power.
...
TL;DR: The paper proposes a post-hoc calibration method called Reliability-Aware Prototype Calibration (RPC) to improve the performance of pose-flow video anomaly detectors in a frozen-detector setting.


<details>
  <summary>Details</summary>
Motivation: A single likelihood score from pose-flow video anomaly detectors may not accurately represent multimodal normal behavior and can be sensitive to pose-observation noise.

Method: The RPC method adds a standardized nearest-prototype deviation in the frozen latent space to the standardized flow score and uses keypoint confidence to gate this added geometric evidence.

Result: RPC improves frame-level AUROC in all eight backbone-dataset pairs, with gains ranging from 0.34 to 4.49 percentage points and averaging 2.03 points.

Conclusion: The results suggest that lightweight post-hoc calibration can strengthen cached pose-flow systems when retraining or reproducing the full pose pipeline is impractical.
...
TL;DR: A novel region-based graph learning framework is proposed for automated skin cancer classification from dermoscopic images, achieving state-of-the-art results by explicitly modeling lesions as graphs and integrating demographic/clinical variables.
...
Method: A region-based graph learning framework that models lesions as graphs of spatially coherent superpixel regions, encodes inter-regional geometry, and integrates demographic/clinical variables via a metadata context node.
...
Conclusion: The proposed framework provides a more expressive and robust approach to skin cancer classification by explicitly modeling lesions as graphs and integrating contextual information.
...
<details>
  <summary>Details</summary>
Motivation: Existing approaches for conditional diffusion and flow models fail to satisfy constraints, either ignoring alignment information or relying on hand-tuned linear updates, leading to a trade-off between fidelity and plausibility.
...
Result: FlowBender consistently outperforms baselines and state-of-the-art methods across image-to-image translation, restoration, and 3D mesh texturing, improving both fidelity and plausibility.

Conclusion: FlowBender provides a more effective approach to conditional diffusion and flow models by leveraging alignment error to improve generated samples, eliminating the need for a trade-off between fidelity and plausibility.
...
TL;DR: Proposed QK Product Steering, a method to reduce object hallucination in vision-language models by editing query-key product without extra data or training.
...
Method: QK Product Steering edits the query-key product in selected middle layers by suppressing dominant singular modes and updating query weights.

Result: QK Product Steering achieves an average relative CHAIRs reduction of 4.0% across three GQA-based VLMs.

Conclusion: QK Product Steering offers a simple alternative to decoding-time mitigation, preserving general multimodal capability without extra data or training.
...
<details>
  <summary>Details</summary>
Motivation: The localisation of the neonatal face is crucial for various non-contact assessments, but challenges such as cluttered backgrounds, illumination changes, and poor lighting conditions reduce the accuracy of face detection models.

Method: A one-stage YOLOv11m-based model was proposed and trained on multiple publicly available datasets, then fine-tuned on a neonatal research dataset.

Result: The model achieved an AP50 of 0.87 before fine-tuning and 0.96 after clinical-domain adaptation, surpassing state-of-the-art general face detectors.

Conclusion: The proposed model shows promise for face detection in neonatal clinical environments, and the creation of publicly available neonatal datasets would support further progress.
...
TL;DR: A new dataset called PCFootprint is introduced for building footprint extraction from airborne laser scanning point clouds, aiming to overcome limitations of image-based methods and support Level of Detail building modeling.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of image-based methods for building footprint extraction, such as occlusions, perspective distortions, and lack of elevation information, and to support more accurate and detailed building modeling.

Method: The authors present a new dataset, PCFootprint, which consists of 33,000 tiles of airborne laser scanning point clouds with vectorized footprints, and establish comprehensive benchmarks by evaluating mainstream methods for footprint extraction.

Result: The experimental results reveal significant challenges in footprint extraction, including high intra-class variance, data imbalance, and noise across complex geospatial environments.

Conclusion: The PCFootprint dataset is expected to advance future research in building modeling, urban scene understanding, and geospatial analysis, and is publicly available for use.
...
TL;DR: The paper audits training-free detectors of AI-generated images, comparing their performance under a controlled protocol and highlighting issues with implementation details, hyperparameter sensitivity, and dataset bias.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to compare training-free detectors of AI-generated images under a single controlled protocol, as their reported numbers are rarely compared.

Method: The paper audits two representative training-free scores (autoencoder-reconstruction score and noise-perturbation feature-similarity score) and a naive feature-kNN control on a common benchmark.

Result: The audit yields three cautionary findings: implementation details significantly impact performance, score direction is not a property of the method but of its hyperparameters, and dataset format bias inflates robustness claims.

Conclusion: The paper concludes that exploiting complementarity between different detectors requires direction-aware combination, and that naive z-score fusion does not beat the best single score.
...
TL;DR: A new framework called FreeStyle is proposed for style-content dual-reference generation, which aims to balance content fidelity, style alignment, and instruction following while avoiding semantic leakage.
...
Method: FreeStyle uses community LoRA mining to construct large-scale Style-Reference and Content-Reference triplets, and adopts a two-stage curriculum with disentanglement mechanisms to address content leakage.

Result: The proposed model achieves a strong balance among style alignment, content preservation, and leakage suppression, as shown by extensive experiments on a new benchmark.

Conclusion: FreeStyle provides a scalable and effective solution for style-content dual-reference generation, addressing the challenges of balancing content fidelity, style alignment, and instruction following while avoiding semantic leakage.
...
TL;DR: Introducing S-Agent, a spatial tool-use agentic paradigm for understanding and reasoning over continuous multi-view images and videos, improving VLMs in a training-free manner.
...
Method: S-Agent formulates spatial reasoning as spatio-temporal evidence accumulation, using a semantic planner, spatial tools, and experts to ground objects in 2D and 3D, and a temporal memory mechanism for evidence integration.
...
Conclusion: S-Agent presents a novel approach to spatial intelligence, enabling continuous and evolving 3D world reasoning, and demonstrating state-of-the-art performance on multi-view and video spatial reasoning benchmarks.
...
TL;DR: The study compares egocentric human video and teleoperated real-robot trajectories for pretraining embodied foundation models, finding that egocentric data can lead to superior performance with proper processing.
...
TL;DR: A new dataset, SARLO-80, is introduced, featuring high-resolution SAR and optical imagery with natural-language descriptions, enabling multimodal learning and benchmarking.
...
Method: The dataset is built from open-access Umbra spotlight acquisitions, standardized to an 80cm slant-range grid, and aligned with high-resolution optical tiles and natural-language descriptions.

Result: The dataset contains 119,566 triplets covering 257 locations across 72 countries, with fixed train/validation/test splits and baseline code for reproducible benchmarks.

Conclusion: The SARLO-80 dataset provides a valuable resource for multimodal learning and benchmarking, enabling applications such as cross-modal retrieval and conditional generation in native SAR geometry.
...
TL;DR: The paper introduces VisDom, a learning-free geometric constraint for sparse novel view synthesis (NVS) that improves reconstruction quality by enforcing a minimum multi-view visibility requirement.


<details>
  <summary>Details</summary>
Motivation: Sparse NVS is challenging due to ambiguity in recovering 3D geometry from few input views, and existing methods often overfit or produce inconsistent geometry.

Method: The authors propose VisDom, which augments classical carving-based visual hull reconstruction by defining a visible domain as the subset of 3D space observed by at least K views, and use it as an additional filtering criterion.

Result: Experiments on three challenging datasets show consistent improvements in sparse-view NVS, enabling high-quality object-centric reconstruction from as few as four input images.

Conclusion: VisDom is a simple, domain-agnostic, and learning-free complement to existing approaches that improves sparse-view NVS and can be applied to various pipelines, including NeRF and Gaussian Splatting.
...
TL;DR: The paper analyzes the reproducibility of the Frechet Inception Distance (FID) metric for image generation, finding that retraining a model with a different seed has a larger impact on FID than resampling from a fixed network.


<details>
  <summary>Details</summary>
Motivation: To investigate the reproducibility of FID scores, which are widely used to evaluate image generation models, but often reported as a single number from a single trained model with a single sampling seed.
...
Result: The study finds that retraining a model with a different seed affects FID more than resampling from a fixed network, and that factors like random initialization, data ordering, and Gaussian noise contribute to this variability. Increasing compute or model size does not significantly reduce variability, but per-cell classifier-free-guidance tuning can halve the spread.

Conclusion: The authors recommend a new FID evaluation protocol that involves evaluating under per-cell optimal guidance, considering FID gaps below a certain threshold as inconclusive, and reporting error bars over multiple training seeds rather than a single FID number.
...
TL;DR: The Caltech Tennis Dataset (CalTennis) is a large-scale video benchmark for monocular-to-3D pose estimation, comprising over 11 million frames of tennis practice and match play, and providing a standardized protocol for data collection and evaluation.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to create a large-scale benchmark for evaluating monocular-to-3D pose estimation in the wild, and to provide a standardized protocol for data collection and evaluation.

Method: The method used in this paper is to create a large-scale video dataset (CalTennis) with synchronized multi-view recordings of expert athletic motion, and to benchmark state-of-the-art monocular-to-3D pose estimation methods on this dataset.

Result: The results show that while 3D joint angle recovery is accurate, all models struggle to estimate depth and foot contact consistently, and the proposed novel performance metrics (footwork and stability) expose previously underexplored failure modes.

Conclusion: The conclusion of this paper is that the CalTennis dataset and the proposed standardized protocol can be used to improve monocular-to-3D pose estimation and action analysis, and that there are opportunities for improvement in pose estimation, particularly in estimating depth and foot contact.
...
TL;DR: A new framework called Spatially Speculative Decoding (SSD) is introduced to accelerate autoregressive image generation by up to 13.3x while maintaining high fidelity.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the computational bottlenecks in autoregressive models for visual generation, which discard the intrinsic 2D spatial locality of visual signals.

Method: The method is Spatially Speculative Decoding (SSD), which predicts adjacent horizontal and vertical tokens simultaneously, capitalizing on 2D spatial correlation.
...
TL;DR: Existing world models fail to maintain a consistent internal state when the camera is not observing, and a new benchmark, WRBench, is proposed to evaluate this aspect.


<details>
  <summary>Details</summary>
Motivation: Current world models lack an internal state that evolves over time, decoupled from observation, which is crucial for artificial general intelligence.

Method: The authors introduce WRBench, a systematic diagnostic benchmark that evaluates world models by treating camera motion as an intervention on observability.

Result: The results show that current systems fail to maintain a consistent internal state when the camera is not observing, and instead resume the target in the state it was abandoned rather than advancing the event.

Conclusion: The authors argue that the stability of the physical state kernel and the consistency of worldlines under viewpoint intervention should become primary objectives of world-model design.
...
TL;DR: The paper presents a new interface for image editing using 3D boxes to provide precise control over spatial transformations, and a system that generates consistent results under large transformations.


<details>
  <summary>Details</summary>
Motivation: Current text and 2D-conditioning interfaces for image editing provide weak control over spatial transformations, especially with large object motions and camera changes.

Method: The 'thinking in boxes' interface uses 3D boxes as structured specifications for edits, with a depth-aligned planar floor as a global reference frame, and an image generator conditioned on this structure.
...
Conclusion: The proposed method provides precise control over translation, rotation, scaling, and viewpoint changes in real images, while preserving scene and object identity, and recovering previously unseen object regions.
...
TL;DR: The paper introduces UNIEGO, a unified egocentric encoder that leverages a hierarchical multi-teacher distillation framework to incorporate knowledge from various viewpoints, modalities, and foundation models, achieving state-of-the-art performance in egocentric video understanding tasks.


<details>
  <summary>Details</summary>
Motivation: The limitations of egocentric video understanding due to the narrow perspective of wearable cameras, and the need for a more expressive representation that captures the full richness of human action.
...
TL;DR: TimeProVe, a hybrid framework, efficiently answers questions in long videos by first generating answer hypotheses and then verifying them with a large vision-language model, reducing computational cost and achieving competitive performance.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for Long Video Question Answering (LVQA) are either computationally expensive or miss temporally localized evidence, motivating the need for a cost-efficient framework.

Method: TimeProVe uses lightweight modules to generate hypotheses and an expensive vision-language model for targeted verification, leveraging the Action-based Candidate Evidence (ACE) module for query-conditioned reasoning.

Result: TimeProVe outperforms the strongest baseline on OpenTSUBench by 7.3%, reduces VLM calls by 75%, and inference cost by 93%, achieving competitive performance on Charades-STA.

Conclusion: TimeProVe offers a cost-efficient solution for LVQA, effectively balancing performance and computational cost through its hybrid framework and targeted verification approach.
...
TL;DR: A fast and training-free framework for generating text-driven 3D visual illusions with geometric integrity and semantic recognizability.
...
Method: A two-stage approach: (1) cross-space dual-branch denoising process for CLIP-guided orientation alignment and SDF blending, and (2) view-conditioned texture synthesis module for projecting and aggregating view-specific 2D diffusion priors.
...
Conclusion: The proposed framework successfully generates high-quality 3D visual illusions with improved geometric integrity, semantic recognizability, and efficiency.
...
TL;DR: This paper introduces TreeTracer, a visual analytics tool for evaluating biases in Large Language Models (LLMs) through aggregated comparison, enabling the detection of hidden representational harms.


<details>
  <summary>Details</summary>
Motivation: The stochastic nature of text generation makes it difficult to evaluate representational and syntactic biases in LLMs, and standard auditing methods have limitations in capturing biases hidden in lower-probability generation branches.
...
Result: The tool successfully exposes hidden representational harms, such as counterfactual pronoun suppression and conversational marginalization of individuals, and a preliminary user study confirms that the aggregated comparative interface reduces cognitive load and supports analysts in detecting systemic biases.

Conclusion: TreeTracer is a effective visual analytics tool for evaluating LLM bias, enabling the detection of hidden representational harms and supporting analysts in detecting systemic biases.
...
TL;DR: This study explores using Google's large language models (LLMs) to automate detecting health-related quality-of-life results (EQ-5D data) in PubMed abstracts, proposing a multi-phase framework that achieves a 0.74 weighted F1-score and accuracy.


<details>
  <summary>Details</summary>
Motivation: The rapid increase in scientific publications makes manual study screening in systematic literature reviews resource-consuming, inefficient, and inconsistent, particularly for studies reporting health-related quality-of-life results like EQ-5D data.

Method: The study uses a multi-phase framework integrating few-shot prompting, weight ensembling aggregation, and a soft stacking meta-classifier to evaluate nine LLMs on a dataset of PubMed studies manually labeled by experts.
...
Conclusion: The findings suggest that an ensemble-based LLM setup is a reliable and scalable approach for automating screening in biomedical research, improving the balance between precision and recall, and providing greater reliability and interpretability.
...
TL;DR: The study investigates cross-lingual transfer in large language models fine-tuned on Arabic and evaluated on zero-shot reading comprehension across Semitic and non-Semitic languages.


<details>
  <summary>Details</summary>
Motivation: To examine the effectiveness of fine-tuning large language models on Arabic for cross-lingual transfer to other Semitic and non-Semitic languages.

Method: Fine-tuning seven large language models (4B-671B parameters) on Arabic and evaluating their performance on zero-shot reading comprehension across Semitic and non-Semitic languages.

Result: No evidence of Semitic-specific transfer was found; models with weak baselines showed significant improvements across all languages, while strong-baseline models showed marginal gains regardless of language family.

Conclusion: The study suggests that fine-tuning and inference-time reasoning mechanisms address task-format alignment rather than cross-lingual knowledge transfer.
...
TL;DR: The study examines the limitations of large language models (LLMs) in translating sequential programming priors into parallel temporal logic for hardware design, introducing a new error taxonomy and revealing persistent knowledge gaps.


<details>
  <summary>Details</summary>
Motivation: The crucial bottleneck in applying LLMs to hardware design is the translation of sequential programming priors into parallel temporal logic, and the study aims to investigate this challenge.

Method: The study introduces a new error taxonomy grounded in problem solvability, inspired by cognitive theory, and evaluates LLMs on the VerilogEval benchmark.

Result: The study finds that frontier models plateau at a 90.8% initial pass rate on the VerilogEval benchmark, with persistent knowledge gaps due to unsolvable functional errors, and that alignment techniques only teach models to compile.
...
TL;DR: The paper introduces the DeepSeek-V4 series, a set of Mixture-of-Experts language models with 1.6T and 284B parameters, supporting a context length of one million tokens and showcasing improved efficiency and performance.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to present a new series of language models, DeepSeek-V4, that can efficiently handle long-context scenarios and improve performance in various tasks.

Method: The authors incorporate several key upgrades in architecture and optimization, including a hybrid attention architecture, Manifold-Constrained Hyper-Connections, and the Muon optimizer. They also pre-train the models on over 32T diverse and high-quality tokens and apply a comprehensive post-training pipeline.

Result: The DeepSeek-V4 series demonstrates state-of-the-art performance, with DeepSeek-V4-Pro-Max outperforming its predecessors in core tasks. Additionally, the models show high efficiency in long-context scenarios, requiring only 27% of single-token inference FLOPs and 10% of KV cache compared to DeepSeek-V3.2.

Conclusion: The DeepSeek-V4 series presents a significant advancement in language modeling, offering improved efficiency and performance in handling long-context scenarios, and making long-horizon tasks and further test-time scaling more feasible.
...
TL;DR: This paper analyzes In-Context Learning (ICL) in Diffusion Large Language Models (dLLMs), finding that query position significantly impacts generation quality and proposing a new metric and adaptive routing strategy to improve performance.
...
Method: The paper conducts a comprehensive analysis of ICL in dLLMs, including empirical decoupling and analysis of attention flow and decoding trajectories. It also proposes a novel metric, Average Confidence ($overline{C}$), and an adaptive routing strategy, Auto-ICL.

Result: The paper finds that query position is a first-order variable in dLLMs, and positional variance impacts generation quality. It also shows that Auto-ICL robustly approaches oracle performance across heterogeneous reasoning and perception tasks.

Conclusion: The paper concludes that ICL in dLLMs is sensitive to query position and proposes a new metric and adaptive routing strategy to improve performance. The findings provide a foundational understanding of spatial ICL in dLLMs and demonstrate the effectiveness of Auto-ICL.
...
TL;DR: A new method called Causal Attribution Pruning (CAP) is introduced to improve the efficiency of large language models (LLMs) by pruning attention heads based on their causal impact on reasoning tasks, achieving up to 61% relative accuracy gain over existing methods.


<details>
  <summary>Details</summary>
Motivation: Large language models (LLMs) are computationally expensive and incur substantial inference cost, despite excelling at multi-step reasoning.
...
TL;DR: LUCID is a novel hallucination detection method for LLM-based knowledge graph reasoning frameworks, leveraging LLM attention scores, KG semantics, and structural information to improve performance.


<details>
  <summary>Details</summary>
Motivation: Hallucinations in LLMs remain a critical issue, even with relevant KG knowledge, and existing detection methods overlook structural information in KGs, leading to suboptimal performance.
...
Conclusion: LUCID is an effective hallucination detection method for LLM-based knowledge graph reasoning frameworks, addressing the gap in existing methods by incorporating structural information in KGs.
...
<details>
  <summary>Details</summary>
Motivation: The motivation behind this survey is to address the limitations in sign-language recognition, translation, and production, which are hindered by fragmented datasets, inconsistent annotations, and limited linguistic coverage.

Method: The authors conducted a comprehensive survey of sign-language datasets, analyzing 120 resources across 35 sign languages, and introduced a 24-field Sign-Language Datasheet for standardized documentation.

Result: The survey provides a unified foundation for developing inclusive, robust, and scalable sign-language technologies, and a public GitHub repository for reproducible evaluation.

Conclusion: The work provides a practical foundation for improving sign-language technologies, enabling real-world applications that cater to the needs of Deaf and Hard-of-Hearing communities.
...
TL;DR: The paper introduces a new method for measuring uncertainty in Large Language Models (LLMs) during in-context learning, which improves reliability and trustworthiness.


<details>
  <summary>Details</summary>
Motivation: The reliability of In-Context Learning (ICL) in LLMs is a concern due to sensitivity to prompt design and model understanding of context, making it hard to distinguish between data and model limitations.

Method: The paper proposes a concept of self-function vectors, built upon Bayesian views and mechanistic interpretability of ICL, to estimate aleatoric uncertainty directly and evaluate uncertainty in ICL more reliably.

Result: The proposed methodology can measure uncertainty of LLM predictions made under ICL more reliably than existing methods and can be used for trustworthy-related applications like hallucination detection.

Conclusion: The paper paves a new direction for connecting quantitative uncertainty view with mechanistic understanding of model behavior, enhancing reliability and trustworthiness of LLMs in ICL settings.
...
TL;DR: The paper introduces GRACE, a unified theoretical framework for determining the optimal granularity of verification in Test-time scaling (TTS) for large language models, and proposes an adaptive strategy that outperforms fixed-granularity baselines.


<details>
  <summary>Details</summary>
Motivation: The motivation is to explore the optimal granularity of verification under a given compute budget in TTS, as prior work has not fully addressed this fundamental question.
...
Result: The paper proves that there exists a phase transition in verification granularity, and proposes an adaptive granularity strategy that achieves the compute-performance Pareto frontier, with empirical results showing up to 3.1% accuracy improvement over fixed-granularity baselines.

Conclusion: The paper concludes that the GRACE framework and adaptive strategy provide a unified and optimal approach to verification granularity in TTS, achieving better performance at matched compute.
...
<details>
  <summary>Details</summary>
Motivation: Current retry strategies in multi-agent LLM systems treat all failures equally, which can lead to unnecessary retries or failure to halt the system when needed, highlighting the need for a more nuanced approach.

Method: The ASP protocol is introduced, which accompanies AI-generated responses with structured quality signals, including certainty, grounding, stochasticity, and an assumption index, enabling a controller to route failures differently.

Result: The evaluation of ASP shows improvements in pass rates and mean term coverage across various models, as well as the ability to block ungrounded upstream outputs from reaching downstream agents.

Conclusion: The Argent Signaling Protocol (ASP) offers a promising solution for improving the handling of failures in multi-agent LLM systems by providing more nuanced and effective failure handling.
...
TL;DR: This paper presents a study on the narrative composition of web-scale LLM pretraining corpora, specifically analyzing Dolma, a 3-trillion-token open pretraining corpus.
...
Method: The authors design a framework spanning three core narrative elements (agency, setting, and events) operationalized as 11 interpretable dimensions, and use it to annotate a diverse set of 400 passages.

Result: The authors find that narrative structure is measurable at scale across extremely heterogeneous data, and uncover a continuous, multidimensional narrative structure underlying web text.
...
TL;DR: The study evaluates LLM-as-a-Judge paradigm, finding issues with exact-match agreement and proposing a Minimum Viable Validation Protocol.


<details>
  <summary>Details</summary>
Motivation: The dominant evaluation paradigm for language models, LLM-as-a-Judge, has reliability concerns due to its reliance on exact-match agreement.

Method: Systematic evaluation of 21 judges from nine providers across three protocols (agreement, consistency, bias audit) over 118 runs and approximately 541,000 individual judgments.

Result: Four key findings: kappa deflation, shifting judge rankings, high test-retest reliability with severe position bias, and small verbosity bias.

Conclusion: The study highlights the need for a more robust validation protocol, proposing a Minimum Viable Validation Protocol to address the issues found.
...
Result: Experimental results show that recent VLMs can leverage visual scenes to resolve structural ambiguity to some extent, but they still struggle with certain ambiguity types and visually subtle semantic distinctions.

Conclusion: The paper concludes that while VLMs can use visual scenes to resolve structural ambiguity, there are remaining limitations, indicating a need for further improvement in this area.
...
TL;DR: The paper proposes a hierarchical approach for Vietnamese multi-document abstractive summarization, achieving a ROUGE2-F1 score of 0.2468 on the VLSP's public test set.


<details>
  <summary>Details</summary>
Motivation: The challenge of Vietnamese multi-document abstractive summarization, introduced in the International Workshop on Vietnamese Language and Speech Processing (VLSP) 2022.

Method: A hierarchical approach, condensing each document followed by aggregation and summarization, with a novel strategy to shorten documents driven by the golden summary.

Result: Achieves a ROUGE2-F1 score of 0.2468 on the VLSP's public test set, producing fluent and concise summaries.

Conclusion: The proposed method shows effectiveness in Vietnamese multi-document abstractive summarization, with additional data made available for the community.
...
<details>
  <summary>Details</summary>
Motivation: The authors aim to understand how different regions of the pretraining corpus contribute to the model's reasoning capabilities in social and STEM domains.

Method: The authors use gradient-based attribution (TrackStar) to measure the influence of each training document on the model's predictions, and aggregate influence across a taxonomy of 576 bins.

Result: The authors find that social and STEM reasoning draw on qualitatively distinct corpus regions, and that the contrast is sharper at the reasoning level than at the knowledge level. They also validate their findings using targeted machine unlearning.

Conclusion: The study demonstrates the effectiveness of training-data attribution in capability discovery and provides insights into the corpus regions that support social-reasoning versus STEM-reasoning in large language models.
...
TL;DR: The authors argue that clinical NLP datasets for detecting suicidal behaviors, built from electronic health records (EHRs), are shaped by specific operationalizations and biases, which can lead to inaccurate assumptions about the reliability of the data.
...
Method: The authors conduct a case study of the ScAN dataset, built from MIMIC-III clinical notes, and perform a linguistic analysis to demonstrate how governance constraints, ICD-based cohort selection, single-annotator labeling, and hospital-stay-level aggregation affect the labels and their interpretation.
...
Conclusion: The authors conclude that clinical NLP should be cautious when interpreting labels from EHR-based suicidality datasets as ground truth, as they may reflect clinician-documented judgments and assumptions rather than objective facts.
...
TL;DR: MiqraBERT, a Sentence-BERT model, is introduced for detecting textual reuse in the Hebrew Bible, improving verse-level semantic similarity detection.
...
Method: MiqraBERT is fine-tuned from AlephBERT, a Modern Hebrew encoder, using 1,650 labeled verse and half-verse pairs, and evaluated using distribution-based metrics.

Result: MiqraBERT improves distributional separation by 2.7-fold, reduces the ambiguous overlap region from 24% to 6%, and achieves a recall@10 of 87.1% for narrative synoptic parallels.

Conclusion: MiqraBERT is effective for detecting textual reuse in narrative contexts but has limited reliability for poetic parallels, and is publicly available for use.
...
TL;DR: The paper explores using persona-based methods to generate multilingual mental health datasets, finding that simply adding language parameters may not be enough to ensure clinical consistency and accuracy in evaluating depression severity across languages.


<details>
  <summary>Details</summary>
Motivation: The global mental health challenges require AI and large language models (LLMs), but there is a shortage of high-quality datasets for training and evaluating such systems, particularly in non-English contexts.

Method: The researchers modified persona-based methods to generate clinical dialogues in Mandarin, Bengali, and Hindi, and evaluated the performance of LLMs in assessing depression severity in these multilingual datasets.

Result: The findings indicate that adding nationality and language parameters in personas may introduce clinical inconsistency across languages, and LLM judge models often exhibit inaccuracies in assessing depression severity in non-English texts.

Conclusion: The paper highlights the urgent need for culturally responsive data generation to ensure equitable mental health systems globally, and the limitations of applying English-centric personas to multilingual contexts.
...
TL;DR: This paper presents a multilingual computational discourse analysis of how language constructed the algorithmic consecration of a Cape Verde goalkeeper after a 2026 FIFA World Cup match.


<details>
  <summary>Details</summary>
Motivation: The study aims to analyze how language contributed to the consecration of Vozinha, a 40-year-old Cape Verde goalkeeper, after a World Cup match.
...
Result: Findings suggest that distinct languages carried distinct frames, including Portuguese mobilization, Spanish crisis, English nation-making, and a shared platform-metric spectacle.

Conclusion: The study contributes to the understanding of cross-lingual narrative diffusion across discourse phases and releases the corpus schema, frame taxonomy, annotation guidelines, and typed timeline as a pilot.
...
TL;DR: The paper proposes SAGE-OPD, a framework for multi-turn on-policy distillation that selectively intervenes on student responses and weights token-level distillation by teacher confidence, achieving up to 13.3% relative improvement over standard OPD.


<details>
  <summary>Details</summary>
Motivation: Most on-policy distillation studies focus on single-turn settings, while realistic LLM agents interact with environments over multiple turns, and standard dense token-level OPD becomes brittle in this regime.

Method: The authors propose SAGE-OPD, a verifier-free selective intervention framework that observes environment feedback, uses teacher judgment to decide whether to skip or intervene on student responses, and weights token-level distillation by teacher confidence.
...
TL;DR: CacheWeaver improves retrieval-augmented generation by optimizing evidence ordering for caching, reducing time-to-first-token by 20-33% without impacting answer quality.
...
Method: CacheWeaver, a lightweight prompt-layer method that uses a prefix tree and greedy walk to optimize evidence ordering for caching.

Result: CacheWeaver reduces median time-to-first-token (TTFT) by 20-33% across three vLLM configurations, with minimal impact on answer quality.

Conclusion: CacheWeaver offers a simple and effective solution for optimizing evidence ordering in retrieval-augmented generation, achieving near-optimal performance without modifying the serving engine or retrieved evidence set.
...
TL;DR: The paper studies the performance degradation of multilingual large language models (MLLMs) on code-switched inputs and proposes a method called CANVAS to mitigate this issue.


<details>
  <summary>Details</summary>
Motivation: The motivation is to understand and address the performance degradation of MLLMs when handling code-switched inputs, which frequently occurs in real-world applications.

Method: The method involves introducing a geometric measure called Anchor Bias to quantify language anchoring and proposing CANVAS, an inference-time intervention that steers target-language hidden states toward the source anchor during prefill.

Result: The results show that CANVAS consistently recovers QA F1 across MLLMs and CS conditions, indicating that internal anchoring signals provide an actionable target for mitigating CS inference failures.

Conclusion: The conclusion is that the proposed method, CANVAS, can effectively mitigate the performance degradation of MLLMs on code-switched inputs by steering target-language hidden states toward the source anchor.
...
TL;DR: The paper explores using GPT-5.4 for customer satisfaction estimation and problem flagging in support conversations, achieving better correlation with customer ratings than sentiment analysis.


<details>
  <summary>Details</summary>
Motivation: Current sentiment analysis has limitations in measuring customer satisfaction, and the authors aim to find a richer alternative.

Method: The authors used GPT-5.4 to estimate customer satisfaction and flag problems in 70,450 support conversations, validating against 1-to-5 customer ratings.

Result: The satisfaction estimate correlated better with customer ratings (0.47) than sentiment analysis (0.36), with fewer false alarms, and revealed new insights such as 'tolerated friction' customers.

Conclusion: LLM-based annotation can capture more than tonality, offering potential for new business metrics grounded in customer state and problem causes.
...
TL;DR: TerraMARS is an information extraction pipeline that converts unstructured Mars science text into structured outputs to support habitability assessments and terraforming studies.
...
Method: TerraMARS uses a domain-adapted Small Language Model (Google Gemma 3 1B) fine-tuned with Quantized Low-Rank Adaptation (QLoRA) on Mars-specific datasets to extract information from scientific literature.

Result: The pipeline generates structured outputs in JSON format and shows promise, but further improvements are needed to increase extraction accuracy and factual consistency.
...
TL;DR: FineREX, a tailored LLM-based pipeline, improves knowledge graph construction for human smuggling networks by fine-tuning for NER-RE, outperforming general-purpose models in quality and efficiency.


<details>
  <summary>Details</summary>
Motivation: The challenge of extracting valuable evidence from unstructured, jargon-heavy legal documents in court proceedings to analyze human smuggling networks.

Method: Introduction of FineREX, a streamlined pipeline using a fine-tuned LLM for named entity recognition and relationship extraction (NER-RE) with a manually annotated dataset.

Result: FineREX achieves improvements of 15.50% in entity F1-score and 31.46% in relationship F1-score, reducing legal noise by nearly half and node duplication by 6.61%, and processing time by 50.0%.

Conclusion: Domain-specific fine-tuning of LLMs can significantly outperform larger general-purpose models in both quality and efficiency for knowledge graph construction in illicit network analysis.
...
<details>
  <summary>Details</summary>
Motivation: The global effectiveness of language models depends on their understanding of local socio-cultural contexts, which is currently a gap.

Method: NRITYAM is developed through collaboration with native dance artists and speakers, comprising 9,260 question-answer pairs in 12 languages.

Result: NRITYAM sets a new standard for evaluating AI systems' ability to understand and reason about traditional performing arts.

Conclusion: The introduction of NRITYAM aims to improve the cultural comprehension capabilities of language models.
...
TL;DR: The study examines the effects of sequential Direct Preference Optimisation (DPO) on language models, finding that the impact on earlier preferences varies depending on the relationship between objectives, signal strength, and training order.


<details>
  <summary>Details</summary>
Motivation: The motivation is to understand how sequential application of multiple behavioural objectives in preference optimisation methods like DPO affects the preferences learned by language models.

Method: The study uses sequential DPO with Llama-3.1-8B-Instruct and LoRA adapters across four preference settings, evaluating objectives after each stage with a fixed base-model reference.

Result: The results show that sequential DPO does not uniformly degrade earlier preferences; instead, the effect depends on the objective relationship, signal strength, and training order, with changes ranging from partial degradation to stability, pair-level redistribution, or positive transfer.

Conclusion: The study concludes that future sequential alignment pipelines should consider objective compatibility and signal strength, rather than assuming uniform effects of later objectives on earlier preferences.
...
TL;DR: This paper proposes a framework to transfer knowledge from pre-trained language models like BERT to Tsetlin Machines (TM), making TM more interpretable and competitive with BERT in text classification tasks.


<details>
  <summary>Details</summary>
Motivation: The motivation is to combine the interpretability of Tsetlin Machines with the performance of pre-trained language models like BERT, addressing the lack of transparency in BERT and the limited semantic understanding in TM.

Method: The proposed method involves clustering text samples into semantically coherent groups using K-means or Top2Vec, then pre-training a non-negated TM with enhanced Type I feedback on these clusters. The TM learns interpretable semantic keywords that can be fine-tuned for specific tasks.

Result: The method outperforms vanilla and embedding-based TMs and reaches performance competitive with BERT across five datasets while maintaining interpretability.

Conclusion: The proposed framework successfully bridges the gap between the interpretability of Tsetlin Machines and the performance of pre-trained language models, offering a more transparent and competitive approach to text classification.
...
Method: The authors propose a revised framework, Credence, which includes: (1) Semantic-F1 metric using BGE-large cosine similarity, (2) convergence theorems for formal characterization of the repair pipeline, (3) three evaluation benchmarks, and (4) multi-model benchmarking.

Result: Experiments show that Semantic-F1 outperforms Jaccard-F1 by +15-32pp, and rule-repair reduces the Atomicity Violation Rate (AVR) by 47-100% without degrading fidelity.

Conclusion: The proposed Credence framework improves claim decomposition and evaluation for automated fact-checking, providing a more reliable and generalizable approach.
...
Method: Develops a 'budget normalized control window framework' to predict coherent control of behaviors through single neuron interventions.

Result: The framework accurately predicts control ceilings for 11 out of 15 neurons, exposing failure modes rather than violations.

Conclusion: Single neuron steering is a budgeted, typed audit of controllability rather than a fixed dose anecdote, with implications for understanding and controlling language model behaviors.
...
Method: The AtomMem system, which includes a Fact Executor to extract high-value atomic facts, organizes them into hierarchical structures, and uses an associative memory graph for retrieval.

Result: AtomMem achieves state-of-the-art performance across various reasoning tasks on the LoCoMo benchmark.

Conclusion: AtomMem offers a scalable and economically viable solution for deploying intelligent personalized agents by addressing the challenges of long-term information accumulation and reuse.
...
Result: The best zero-shot model (GPT-OSS-20B) achieved a Micro-F1 of 0.893 (recall: 0.949), comparable to the state-of-the-art supervised GatorTron NER-RE baseline (Micro-F1 of 0.960).
...
TL;DR: The paper explores encoding semantic information in compact, non-standard textual forms (BabelTele) that are recoverable by large language models (LLMs) but sacrifice human readability.


<details>
  <summary>Details</summary>
Motivation: To investigate whether semantic information can be encoded in compact, non-standard textual forms that remain recoverable by LLMs, and to explore the potential for task-agnostic representational paradigms.

Method: The paper uses readability diagnostics, model likelihood measures, human questionnaires, and downstream task evaluations to assess the effectiveness of BabelTele.

Result: BabelTele can substantially depart from ordinary natural language while preserving core semantics for instruction-tuned LLMs, maintaining 99.5% semantic fidelity even when condensed to 27.9% of its original length.
...
TL;DR: This chapter explores the potential of Large Language Models (LLMs) to scale up and democratize deliberation, particularly for marginalized groups, while addressing concerns around linguistic constraints, biases, and sycophantic tendencies.


<details>
  <summary>Details</summary>
Motivation: The increasing prominence of LLMs presents both opportunities and challenges for democratic deliberation, and there is a need to explore how LLMs can be used to foster inclusivity and empower marginalized groups.
...
TL;DR: The authors present REDACT, a multilingual benchmark for personally identifiable information (PII) detection, and evaluate five detectors, revealing architecture-dependent failure structures.


<details>
  <summary>Details</summary>
Motivation: Existing PII detection benchmarks are limited, covering few entity types and lacking systematic generation conditions, making it difficult to assess detector performance.

Method: The authors create REDACT, a controlled benchmark with 13,427 records, 324,078 entity annotations, and 51 entity types, using a strength-2 covering-array sampler to control nine generation axes.

Result: The evaluation of five detectors shows that aggregate F1 scores mask architecture-dependent failure structures, with rule-based detectors performing poorly on high-stakes data and LLM detectors being more robust.

Conclusion: The authors release REDACT, a comprehensive multilingual PII benchmark, and highlight the importance of stratified evaluation, revealing that sensitivity-tier assignment is the task's hardest axis.
...
TL;DR: Proposed a lightweight framework for automated pronunciation assessment trained on native speech resources, achieving improved performance with transcript guidance.
...
Conclusion: The proposed framework offers a promising approach for automated pronunciation assessment with limited labeled data, leveraging native speech resources and transcript guidance.
...
TL;DR: The paper introduces GEMS, a method for multi-directional activation steering in models without retraining, addressing issues of model collapse by preserving orthogonality and norm.


<details>
  <summary>Details</summary>
Motivation: Existing activation steering methods are limited to single-direction injection and cause model collapse when multiple directions are superposed without constraints.

Method: The proposed method, GEMS, uses norm-preserving weighted superposition, targeted attention-pathway injection, and real-time orthogonalization to address distributional deviation and directional interference.

Result: GEMS achieves 98% accuracy on GSM8K and incurs only 2.2% PPL increase on Wikitext-2 when injecting three concurrent non-mathematical directions, compared to a collapse to 4% in the unconstrained case.

Conclusion: The method successfully controls model behavior by modifying hidden states at inference time, preventing model collapse and preserving semantic specificity across different architectures.
...
TL;DR: The paper proposes STAGE, a data generation pipeline for creating structured JSON data from unstructured text, improving performance in extracting information.


<details>
  <summary>Details</summary>
Motivation: Legacy industries rely on unstructured documents containing high-value information, which needs to be extracted into structured representations for automated systems.

Method: The authors propose STAGE, a source-grounded data generation pipeline using LLMs for scalable synthesis and validating against underlying spreadsheets.

Result: STAGE produces stronger training data, improving Qwen3-4B exact match from 31.37% to 74.27% and value accuracy from 45.46% to 90.69%.

Conclusion: STAGE effectively generates high-quality JSON data from unstructured text, enhancing information extraction for automated systems in legacy industries.
...
TL;DR: IHUBERT, a monolingual Persian language model, is trained on a 45 GB curated corpus and achieves state-of-the-art results on several Persian NLU benchmarks.


<details>
  <summary>Details</summary>
Motivation: The scarcity of large-scale, high-quality pretraining corpora and insufficient evaluation beyond standard classification and NER tasks limit Persian pretrained language models.
...
Result: IHUBERT achieves state-of-the-art results on several Persian NLU benchmarks, including extractive QA, NER, sentiment analysis, and topic classification.
...
TL;DR: The paper tests whether large language models (LLMs) resist valid corrections to their own writing. It finds no detectable self-preference bias, with models accepting or rejecting verified-good fixes at similar rates as fresh models.


<details>
  <summary>Details</summary>
Motivation: To investigate whether LLMs resist valid corrections to their own writing, and to test for self-preference bias.

Method: The paper uses a setting where a deterministic verifier (IFEval) checks if a model's draft violates a constraint and if a candidate edit fixes it. The model then accepts or rejects the edit as either the original author or a fresh model.

Result: No detectable self-preference bias was found, with authors rejecting verified-good fixes at a similar rate to fresh models (gap -5.1 pp, 95% CI [-12.9, +2.7]).

Conclusion: The study finds no evidence that LLMs resist valid corrections to their own writing, and suggests that when authors do reject verified-good fixes, they provide flaw-catching reasons rather than expressing personal preference.
...
TL;DR: The paper introduces HydraHead, a novel architecture that hybridizes Full Attention (FA) and Linear Attention (LA) along the head axis, achieving high-performance hybrid models with minimal training overhead.
...
Method: The authors conduct interpretability analysis, observe block-wise functional similarity across layers and distinct functional specialization within layers, and introduce HydraHead, which features an interpretability-driven selection strategy and a scale-normalized fusion module.

Result: HydraHead outperforms other hybrid designs in long-context tasks while maintaining strong general reasoning, and achieves over 69% improvement over the baseline at 512K context length, approaching a leading model's performance.

Conclusion: The paper highlights the significant scaling potential of head-level hybridization, and demonstrates that HydraHead can achieve high-performance hybrid models with minimal training overhead.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to reduce user-perceived latency in Streaming RAG by issuing tool queries in parallel with ongoing user input, and to understand the intrinsic benefit of speculation in this mechanism.

Method: The study measures the distribution of tool-intent stabilization on the CRAG benchmark, derives a model-agnostic bound on the portion of tool latency that can be hidden, and validates against a working streaming pipeline.

Result: The results show that at a realistic operating point, 73.9% of queries admit substantial latency hiding, and identify query properties that predict early versus late stabilization.

Conclusion: The study concludes that the Streaming RAG mechanism can significantly reduce user-perceived latency, and that the tool-intent stabilization property is a key factor in achieving this reduction.
...
TL;DR: This paper analyzes how Large Language Models (LLMs) score essays, finding that essay quality information is encoded linearly within LLM representations, and identifies 'essay scoring neurons' that correlate with essay scores.


<details>
  <summary>Details</summary>
Motivation: The internal mechanisms of LLM-based Automated Essay Scoring (AES) are not well understood, despite advances in LLMs transforming AES.
...
Result: Essay quality information is encoded linearly within LLM representations, which emerges progressively across layers, remains robust across prompting strategies, and partially transfers across essay prompts.

Conclusion: The findings provide evidence that LLMs encode structured representations related to essay quality and offer new insights into the interpretability of LLM-based AES systems.
...
TL;DR: MedRLM, a Recursive Multimodal Health Intelligence framework, is proposed for long-context clinical reasoning and decision support, treating patient cases as an external clinical environment for recursive inspection and synthesis.


<details>
  <summary>Details</summary>
Motivation: Current medical AI systems rely on single-step prompting or retrieval, which can be fragile for clinical evidence distributed across multiple sources. There is a need for a more comprehensive approach to clinical decision support.

Method: MedRLM uses a recursive multimodal framework, coordinating specialized agents for different types of patient data, and introducing a Clinical Evidence Graph Memory and sensor-guided recursive triggering mechanism.

Result: The proposed framework aims to move medical AI from static question answering to auditable, multimodal, and workflow-aware clinical decision support, with a planned evaluation using real clinical datasets.

Conclusion: MedRLM has the potential to improve clinical decision support by providing a more comprehensive and auditable approach to reasoning over heterogeneous and longitudinal patient information.
...
TL;DR: ReNikud, a novel G2P conversion method for Modern Hebrew, leverages weak audio supervision and pseudo-vocalization to improve phoneme prediction, outperforming state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: The challenge of G2P conversion for Modern Hebrew due to its abjad writing system, which leaves vowels largely unwritten, creating substantial ambiguity for applications like TTS.

Method: ReNikud uses a phoneme-based ASR pseudo-labeling pipeline for weak audio supervision and a pseudo-vocalization architecture to predict IPA phonemes at each character position.

Result: ReNikud surpasses previous state-of-the-art methods on existing Hebrew G2P benchmarks and the new MILIM benchmark for spoken Hebrew.

Conclusion: ReNikud offers a more effective approach to G2P conversion for Modern Hebrew, addressing the limitations of standard approaches and providing improved results for TTS applications.
...
TL;DR: The paper presents an algorithm for pitch spelling and key estimation in musical scores, optimizing note names, key signature, and local scales.


<details>
  <summary>Details</summary>
Motivation: The algorithm was designed for music transcription, specifically for building digital collections of jazz solos from audio recordings for analysis, teaching, and cultural heritage preservation.

Method: The algorithm uses a two-stage optimization process: a 'modal' stage proposing probable scales for each bar by minimizing accidentals, and a 'tonal' stage estimating the key signature and note names for the best musical notation.
...
Conclusion: The proposed method is useful for music transcription and other tasks related to musical notation processing, also contributing to musicological studies through new distance definitions between jazz scales.
...
TL;DR: A dataset called CzechDocs is introduced for evaluating machine translation systems that preserve document formatting, supporting Czech and minority languages.
...
Method: The creation of a multiway parallel dataset (CzechDocs) of formatted documents (HTML, DOCX, and PDF) and a comparison of common approaches to format-preserving machine translation.
...
Conclusion: CzechDocs supports the development of machine translation systems that preserve document formatting, facilitating future research in this area.
...
TL;DR: The paper investigates the misalignment in language models fine-tuned on insecure code and identifies a causally actionable activation-space direction that can reduce code spillover.


<details>
  <summary>Details</summary>
Motivation: The motivation is to understand and mitigate the emergent misalignment in language models fine-tuned on insecure code.

Method: The method involves fine-tuning language models on insecure code, identifying a difference-in-means direction in activation space, and performing causal steering and cross-architecture transfer.

Result: The results show that the identified direction can achieve 99.6% separation of aligned and misaligned activations, reduce code spillover by 21-51 points, and enable cross-architecture transfer with large behavioral suppression.

Conclusion: The paper concludes that there is a two-tier specificity structure in language models, and within-model probing is recommended for auditing, while cross-architecture correction has limitations.
...
TL;DR: The paper introduces the Meaning Intelligence Framework (MIF) for analyzing Nigerian public discourse, which separates surface sentiment from true communicative intent. The MIF improves AI model's performance on Nigerian languages by providing context and schema-informed prompting.


<details>
  <summary>Details</summary>
Motivation: The dominant failure mode of AI systems on Nigerian discourse is context failure, where the same utterance carries opposite pragmatic force depending on speaker, audience, and situation. Existing benchmarks for Nigerian languages treat sentiment classification as a three-way polarity task, which is insufficient.

Method: The MIF operationalises nine scored dimensions to analyze Nigerian public discourse. A 30-item calibration dataset is constructed, and a frontier language model (Gemini 2.5 Flash) is evaluated under zero-shot and schema-informed prompting conditions.

Result: The results show significant improvements when using the MIF schema: Register Gap accuracy increases from 33.3% to 73.3%, and the composite Meaning Intelligence Score increases by 5.4 points. The largest practical gains are in register identification, coded-subtext detection, and strategic action recommendation.

Conclusion: The paper concludes that the MIF framework can improve AI model's performance on Nigerian languages by providing context and schema-informed prompting. The framework, annotation guidelines, and calibration dataset are released to support reproducibility.
...
<details>
  <summary>Details</summary>
Motivation: Existing approaches to automated essay scoring and feedback often treat scoring and feedback as separate components, leading to limitations in reliability, interpretability, and adaptability to learners' proficiency levels.
...
Conclusion: PsyScore offers a comprehensive framework for automated essay scoring and feedback that addresses the fragmentation of existing approaches and provides a more integrated and effective solution.
...
<details>
  <summary>Details</summary>
Motivation: Current NLP research on hate speech and misinformation has been treated separately, and existing counterspeech datasets are limited to single-turn English dialogues, which doesn't reflect real-life interactions.
...
Conclusion: The new dataset fills a significant gap in existing research and resources, providing a valuable tool for developing more effective counterspeech models.
...
TL;DR: The paper explores the use of implicit human feedback to align Large Language Models (LLMs), creating a dataset (IFLLM) that captures user interactions like mouse trajectories and eye gazing points, and demonstrating that models based on this feedback outperform traditional text-based reward models.


<details>
  <summary>Details</summary>
Motivation: Existing methods for aligning LLMs rely on explicit human feedback, which is expensive to collect and limited in availability. The authors aim to leverage implicit human feedback, which is abundant and valuable, to improve LLM performance.

Method: The authors collect a new dataset called IFLLM, which includes multi-turn questions, mouse trajectories, and eye gazing points from 59 Mechanical Turk workers. They then train a reward model based on this implicit feedback and compare its performance to traditional text-based reward models.

Result: The reward model based on implicit user feedback achieves a 9% increase in accuracy (from 55% to 64%) and nearly triples the relative response quality improvements after applying the DPO algorithm to eight LLMs.

Conclusion: The paper demonstrates the value of implicit human feedback in aligning LLMs, showing that it can be a valuable resource for improving model performance and efficiency.
...
<details>
  <summary>Details</summary>
Motivation: Existing multi-device agent systems lack fine-grained recovery mechanisms, leading to limited ability to distinguish and handle device-local and cross-device failures.
...
Result: Experiments using HeraBench, a fault-injected benchmark, show that H-RePlan outperforms baselines in completion, instruction adherence, and perfect-pass rates while reducing token cost.

Conclusion: The proposed framework demonstrates the importance of scope-aware hierarchical recovery for robust multi-device agent execution.
...
TL;DR: The paper introduces StylisticBias, a benchmark for evaluating social bias in multimodal large language models (MLLMs) based on visual attributes. It finds that a small set of attributes, such as age, body type, and fashion style, drive the largest shifts in model judgments.


<details>
  <summary>Details</summary>
Motivation: The motivation is to understand how visual cues shape the judgments made by MLLMs, particularly in personally and societally consequential settings, and to provide a controlled benchmark for evaluating attribute-level social bias.

Method: The method involves generating 500 photorealistic base faces with about 50 single-attribute variations per face, producing 25K images. The authors then evaluate six MLLMs across 25 binary social judgment scenarios, measuring how specific visual cues shift model judgments.

Result: The results show that age and body type have the largest identity-level effects, while fashion style and other visual cues drive the largest attribute-level shifts. A small set of about 15 attributes accounts for nearly 80% of the total variation in model judgments.

Conclusion: The paper concludes that bias in MLLMs is concentrated in a small set of visual cues, and that sensitivity is strongest in judgments semantically aligned with appearance, especially socioeconomic and style-related judgments. The StylisticBias benchmark is released for fine-grained bias evaluation in multimodal models.
...
<details>
  <summary>Details</summary>
Motivation: Current policy engines (e.g., XACML, Rego, Cedar) are insufficient for governing autonomous AI agents as they only address permit/prohibit constraints, not obligations, dispensations, or policy conflicts.
...
Conclusion: The paper concludes that AgenticRei is a necessary solution to address the governance challenges posed by autonomous AI agents driven by LLMs, providing a more comprehensive approach to security, privacy, and compliance.
...
TL;DR: The authors propose a human-in-the-loop pipeline to measure a computer science program's coverage of curricular guidelines, applying it to a BSc program against CS2013 and CS2023 guidelines.


<details>
  <summary>Details</summary>
Motivation: The lack of a reliable, reproducible way to measure how completely undergraduate computer science programs cover current curricular guidelines and how that coverage shifts when guidelines are restructured.

Method: A human-in-the-loop pipeline that represents the program and guidelines as structured corpora, generates candidate course-to-knowledge-unit matches by semantic retrieval, and confirms them through human judgment.

Result: The program covers 49.7% of CS2023 and 50.9% of CS2013 knowledge units, with near-constant coverage across a decade. The program articulates competency for ~88% of covered units under each guideline but delivers it at the recommended depth for 76% under CS2023 against 95% under CS2013.

Conclusion: The proposed instrument is reusable and can help identify persistent structural gaps and differences in coverage that reflect the standard's evolution, providing insights for program improvement.
...
<details>
  <summary>Details</summary>
Motivation: The emergence of DLMs as an alternative to autoregressive Large Language Models (LLMs) has led to a need for a comprehensive comparison of their capabilities and trade-offs, which is hindered by differences in evaluation protocols and settings.
...
Result: The analysis highlights the strengths and limitations of diffusion-based language modeling across different tasks, architectures, and inference budgets, showing that DLMs' behavior is strongly influenced by generation-time design choices, leading to distinct trade-offs between performance and computational efficiency.

Conclusion: The study provides practical insights into the capabilities and deployment characteristics of contemporary DLMs, offering a comprehensive understanding of their potential applications and areas for improvement.
...
TL;DR: This paper models multi-agent LLM deliberation as a closed-loop dynamical system, where each agent has a hidden internal belief (anchor) that influences its opinion, and shows that this anchor can be recovered from deliberation data.


<details>
  <summary>Details</summary>
Motivation: The authors aim to understand how and why multi-agent LLM deliberation works, which is increasingly used to improve reasoning and accuracy, by modeling it as a dynamical system that incorporates both social influence and internal beliefs.

Method: The authors propose a closed-loop dynamical system model where each agent has a hidden internal belief (anchor) that continually pulls its opinion, and use this model to analyze multi-agent deliberation.

Result: The authors show that the anchor can be recovered from deliberation data, and that it explains behaviors that classical consensus rules forbid, such as an agent's confidence in the correct answer climbing past its initial belief.

Conclusion: The authors conclude that multi-agent LLM deliberation can be understood as a dynamical system driven by internal beliefs (anchors), and that the influence of these anchors varies across different model families.
...
TL;DR: DeXposure-Claw is a new system for supervising decentralized finance, using a forecast-grounded approach and structured evidence to reduce false alarms and improve decision-making.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenges of supervising decentralized finance, where existing LLM agents are not well-suited due to their tendency to over-read weak evidence and recommend high-stakes interventions.

Method: The method involves a three-part system: (1) DeXposure-FM, a graph time-series foundation model, forecasts future exposure networks; (2) deterministic monitors and stress scenarios turn those forecasts into typed alerts, attribution signals, and scenario evidence; and (3) data-health and confidence gates constrain escalation before emitting auditable supervisory tickets with rationales.

Result: The results of experiments on five years of weekly real data fully support the DeXposure-Claw system.

Conclusion: The conclusion is that DeXposure-Claw provides a more effective and regulator-aligned approach to supervising decentralized finance, reducing false alarms and improving decision-making.
...
TL;DR: This paper investigates whether large language models (LLMs) can recognize their own knowledge limits on structured clinical data tasks. It finds that LLMs' verbalized confidence is not reliable, but using attribution divergence analysis and few-shot examples can improve accuracy and calibration.


<details>
  <summary>Details</summary>
Motivation: To study whether LLMs can recognize the limits of their own knowledge on structured clinical data tasks and to reduce epistemic uncertainty.

Method: Using cross-model attribution divergence analysis to compare Qwen 2.5 7B and XGBoost on a prediction task, and evaluating the effects of few-shot examples and SHAP-derived feature evidence.

Result: The study reports four findings: LLM verbalized confidence is not reliable, LLM exhibits an inverse difficulty effect, few-shot examples and SHAP-derived feature evidence are effective interventions, and a cross-model calibrator can improve calibration.

Conclusion: The paper concludes that LLMs face a 'cold start problem' on structured data and outlines a path toward genuine epistemic self-awareness, highlighting the potential of attribution divergence analysis and few-shot examples to improve LLM performance and calibration.
...
Method: The proposed framework models inter-subject similarity as a differentiable weighting function derived from intra-modality embedding similarities in both retinal images and risk profiles, enabling graded supervision.
...
TL;DR: This paper explores whether Large Language Models (LLMs) can detect and self-correct misalignments with human ethics, proposing a technique for online alignment.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enable LLMs to discern and correct their own outputs that are misaligned with human ethics, preventing emergent unethical behaviors.

Method: The method involves endowing an LLM with a 'conscience' step that reviews its outputs and extending the training loss with an alignment component using Direct Preference Optimization (DPO).

Result: The result is an online technique for aligning models in various applications without requiring a separate judge, demonstrating 'Emergent Alignment' through a single introspective question.

Conclusion: The conclusion is that LLMs can be made to align with human ethics through self-review and DPO, achieving emergent alignment even in scenarios prone to unethical behavior.
...
<details>
  <summary>Details</summary>
Motivation: The fragmentation of different neural network architectures (convolutional, recurrent, and transformers) and their distinct inductive biases.

Method: Introducing a learnable kernel that depends jointly on positions and features, implemented as a small neural network (MLP) to model pairwise interactions.

Result: ITNet matches or exceeds specialized baselines on various benchmarks (ImageNet-1K, GLUE, ModelNet40, VQA v2, and NLVR2) with a single architecture and shared operator.

Conclusion: A single learned interaction mechanism can recover the behavior of all three architectural families (convolutional, recurrent, and transformers) from data.
...
TL;DR: A prompt-based method for uncertainty estimation in large language models (LLMs) is proposed, enabling agents to seek clarification when task specifications are ambiguous.


<details>
  <summary>Details</summary>
Motivation: The classical uncertainty framework is insufficient for interactive LLM agents, and practical constraints rule out many estimation methods, making prompt-based estimation the most viable option.

Method: A simple prompt-based decomposition that separates action confidence from request uncertainty (u) is proposed.
...
Conclusion: The proposed prompt-based decomposition enables LLMs to effectively seek clarification when task specifications are ambiguous, and achieves significant improvements over existing methods across multiple LLM backbones.
...
TL;DR: The paper discusses the integration of formal tools like SAT and SMT solvers with language models to improve safety and security. It identifies a gap in the 'narration' step, where the solver's output is translated into a user answer, and presents a study on the vulnerabilities of this process, particularly under prompt injection attacks.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the potential loss of soundness guarantees in the interaction between formal solvers and language models, particularly in the step of narrating the solver's output to the user.

Method: The paper models the LLM-solver loop as a verified decision procedure and empirically evaluates five open-sourced models under prompt injection attacks. It examines the effectiveness of certificate gating and hardened prompts in mitigating these attacks.

Result: The study finds that while certificate gating can make the solver's verdict sound, an adversary can still invert a verified conclusion across different phrasings and channels. Hardened prompts can reduce but not eliminate injection vulnerabilities, and are still susceptible to adaptive attacks.

Conclusion: The paper concludes that in the LLM-solver loop, robustness does not extend to the final answer that the user reads, highlighting the need for further research to address the vulnerabilities in the narration step.
...
TL;DR: The authors propose ACIE, a retrieval-augmented generation pipeline for clinical information extraction that handles temporal reasoning, cross-document dependencies, and missing metadata, achieving 96.5% acceptance rate from clinicians.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of extracting relevant information from large amounts of heterogeneous clinical documents and structured data points, where standard retrieval-augmented generation methods fail.

Method: The authors deploy an on-premise agentic RAG pipeline called ACIE, which reasons over complete patient contexts and grounds every answer in source passages for clinician verification.

Result: The results show that across 7,326 judgments, clinicians accepted 96.5% of extractions, with per-type acceptance ranging from 80% to 99%.

Conclusion: The conclusion is that ACIE is an effective solution for clinical information extraction, handling complex patient contexts and achieving high accuracy and acceptance rates from clinicians.
...
TL;DR: The paper studies which pairs should be compared in preference-based post-training for language models, formulating it as a sampling-design problem and providing bounds on the post-training optimality gap.


<details>
  <summary>Details</summary>
Motivation: The motivation is to optimize the use of human preference labels, which are expensive, by generating a larger pool of completions and labeling only the most informative comparison pairs.

Method: The method involves formulating comparison curation as a sampling-design problem, evaluating designs by the quality of the final policy under the preference-based post-training objective, and analyzing the choice of labeled pairs in Direct Preference Optimization (DPO) training.

Result: The results provide matching upper and lower bounds on the post-training optimality gap of the DPO-trained policy, showing that comparison selection affects downstream performance through a single design-dependent information matrix.

Conclusion: The conclusion is that the proposed designs consistently improve sample efficiency over common comparison-selection heuristics, and an explicit optimization criterion for budgeted comparison curation is yielded.
...
<details>
  <summary>Details</summary>
Motivation: Current tokenization methods, like Byte-Pair Encoding, are efficient but semantically blind to structured technical entities, leading to fragmentation of physical quantities, numbers, units, and symbolic expressions.
...
Result: TOTEN achieves unit ontological atomicity and high numerical reconstruction (0.775-0.904 on external corpora, 0.780 on EngQuant), outperforming state-of-the-art baselines (e.g., 0.627-0.703 for Quantulum3).

Conclusion: TOTEN demonstrates superior performance in handling technical entities and physical quantities through its ontology-based approach, showing significant improvements over existing methods.
...
TL;DR: The paper discusses the progress and gaps in AI and Systems Engineering, based on a literature review and human-AI agreement assessment.


<details>
  <summary>Details</summary>
Motivation: To trace the progress in AI and Systems Engineering and identify critical research gaps.

Method: Literature review of core papers and human-AI agreement assessment using six AI models on 1,712 INCOSE INSIGHT articles and 889 SERC publications.

Result: Identification of five critical research gaps and guidance for practitioners on AI adoption, assurance, and workforce transformation in Systems Engineering.

Conclusion: The paper provides insights into the current state of AI and Systems Engineering, highlighting areas of convergence and critical gaps, and offers a web application for further exploration.
...
TL;DR: A new tokenizer for 3D brain MRI latent diffusion is introduced, enabling a single embedding space for both clinical tasks and controllable generation.


<details>
  <summary>Details</summary>
Motivation: Existing tokenizers for 3D brain MRI latent diffusion compromise between retaining clinical information and reconstructing anatomically faithful volumes. The authors aim to decouple these demands.

Method: A fully volumetric masked-autoencoder (MAE) based tokenizer is proposed, using a frozen 3D MAE encoder for clinically informative embeddings and a dedicated CNN decoder for voxel reconstruction.

Result: The encoder outperforms or matches state-of-the-art models on 21 of 23 tasks and supports conditional generation and patient-specific longitudinal forecasting.

Conclusion: The proposed tokenizer enables a single 3D brain-MRI embedding space capable of both downstream clinical tasks and controllable generation.
...
TL;DR: The paper proposes a model-agnostic denoising method called DIF to address the issue of noisy implicit feedback in cold-start recommendation scenarios.


<details>
  <summary>Details</summary>
Motivation: Implicit feedback in recommender systems is often noisy, and cold items are more prone to noisy samples, which can lead to poor recommendation performance.

Method: The DIF method infers pseudo-labels for cold items using content-similar warm items, models the confidence of pseudo-labels, and estimates the uncertainty of noisy sample labels to adaptively guide the role of pseudo-labels.

Result: The DIF method has been shown to be superior through theoretical justification and extensive experiments on real-world datasets, and has been successfully deployed on a billion-user scale short video application, improving various commercial metrics within cold-start scenarios.

Conclusion: The DIF method provides an effective solution for denoising implicit feedback in cold-start recommendation scenarios, which can lead to improved recommendation performance and better handling of noisy samples.
...
Method: Using the Aumann-Dreze value to evaluate local moves and analyzing the resulting model.

Result: Establishing equilibrium characterizations and conditions for Lyapunov or potential representations, and analyzing the impact of switching and acceptance costs.

Conclusion: The paper provides insights into coalition formation dynamics, equilibrium properties, and the effects of costs on stability.
...
<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the shortcomings of current agent benchmarks, which fail to capture the complexities of real-world deployments. The authors aim to develop a more comprehensive and robust evaluation framework.

Method: The paper aggregates 14 parallel implementation studies of an MCP-based industrial-agent benchmark and consolidates them with 7 prior agent benchmarks. The authors analyze the limitations of aggregate-score leaderboards and propose a new approach to ranking configurations based on predictive validity.

Result: The authors argue that current evaluation methods, such as aggregate-score leaderboards, are insufficient for evaluating deployed agents. They propose a new approach that focuses on predictive validity and report a 12-tier measurement apparatus that exposes the limitations of current benchmarks.

Conclusion: The paper concludes that current agent benchmarks are inadequate for evaluating real-world deployments and proposes a new approach to evaluation that prioritizes predictive validity. The authors call for a next-generation agentic benchmark that reports on a more comprehensive set of dimensions.
...
TL;DR: The paper presents an interactive interface using Large Language Models (LLMs) to provide natural language access to global explanations for black-box image classifiers, improving accessibility and usability.


<details>
  <summary>Details</summary>
Motivation: The complexity and monolithic nature of global explanations for vision models hinder practical exploration, and users seek targeted answers to specific questions rather than static artifacts.
...
<details>
  <summary>Details</summary>
Motivation: The black-box nature of NCO models hinders their deployment and scientific diagnosis, and standard interpretability tools are not suitable for NCO due to its dynamic and state-dependent decision-making.

Method: EPB uses a large language model (LLM) to autonomously evolve a bank of programs, with an iterative framework consisting of two blocks: Block I updates program bank capacity and uses a hybrid gradient descent scheme, and Block II dynamically adapts bank capacity via fault-targeted expansion and redundancy pruning.

Result: EPB demonstrates effectiveness and broad applicability, with distilled program portfolios matching original performance, and reveals that NCO behavior shifts across optimization stages and can be approximated as a composition of classic heuristic variants.
...
TL;DR: The paper presents a study on fine-tuning pre-trained Transformer-based models for Quranic Automatic Speech Recognition (ASR), achieving a Word Error Rate (WER) of 0.08 and 0.11 on two datasets, outperforming the baseline model.


<details>
  <summary>Details</summary>
Motivation: Existing ASR models exhibit high Word Error Rates (WER) on user-recited Quranic verses and lack full coverage of the Quranic corpus, highlighting the need for improved Quranic ASR systems.
...
Result: The best-performing configuration achieves a WER of 0.08 on the EveryAyah subset and 0.11 on the combined EveryAyah+Tarteel setting, outperforming the Citrinet baseline (WER = 0.163) and reducing training time from 140 hours to 40 hours.

Conclusion: The study identifies key factors affecting transcription accuracy in Quranic ASR and demonstrates the effectiveness of fine-tuning pre-trained models for this task, with potential future work including improving dataset quality and developing phoneme-aware models.
...
TL;DR: The paper evaluates AI-assisted review systems, finding they can track human quality judgments, catch errors, and receive positive user feedback, though there's room for improvement.


<details>
  <summary>Details</summary>
Motivation: The pressure on peer review systems due to AI-assisted research necessitates evaluating new agentic review systems.

Method: The authors evaluate OpenAIReview, coarse, Reviewer3, and a zero-shot baseline across six LLMs on ICLR/NeurIPS papers and a perturbation benchmark.
...
Conclusion: AI-assisted review systems show promise in evaluating research papers, but further development is needed to enhance their performance and address user concerns.
...
<details>
  <summary>Details</summary>
Motivation: The integration of generative models into traditional computational systems presents significant opportunities but also risks, and the field lacks foundational frameworks to mitigate these risks.

Method: The manuscript defines four specific primitives of AI blended architecture and identifies two overarching anti-patterns in the industry.

Result: The framework enables successful integration of AI into traditional systems and provides a foundation for generative model providers to build next-generation interfaces.

Conclusion: The paper establishes a foundation for the safe incorporation of AI into traditional systems, providing a framework for engineers and generative model providers.
...
TL;DR: The paper explores a future question answering system with expert knowledge workers, analyzing system capacity and designing schedulers to maximize it, including the impact of expert collaboration.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve question answering forums by considering a future version with expert knowledge workers, aiming to increase efficiency and capacity.

Method: The method involves modeling the request-answer process as a queuing system, calculating system capacity, and designing schedulers to achieve maximum capacity. It also investigates the effect of collaboration among experts.

Result: The result includes the design of schedulers that achieve system capacity and an analysis of how collaboration between experts can potentially increase the system's capacity to handle requests.

Conclusion: The conclusion likely emphasizes the potential of employing expert knowledge workers in question answering forums to increase system capacity and efficiency, and highlights the importance of effective schedulers and collaboration among experts.
...
TL;DR: The paper introduces the Independent Combinatorial Tokens (ICT) framework to improve Reinforcement Learning with Verifiable Rewards (RLVR) for Large Language Models (LLMs). ICT selectively updates tokens based on their distributional patterns, stabilizing training and improving reasoning performance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the optimization instability in RLVR, which suffers from entropy collapse and explosion, leading to suboptimal strategies in LLM reasoning.

Method: The method introduced is the ICT framework, which uses Jensen-Shannon divergence to identify critical tokens for selective updates, regulating policy concentration through Shannon and second-order Rényi entropy.

Result: Empirical results show that ICT improves pass@4 by an average of 4.58% and up to 14.9% over baselines across seven benchmarks for math, commonsense, and Olympiad-level problems.

Conclusion: The ICT framework effectively stabilizes the training landscape and enhances LLM reasoning performance by selectively updating tokens with distinctive distributional patterns.
...
TL;DR: The paper presents AgentFinVQA, a multi-agent pipeline for financial chart question answering that prioritizes auditability and on-premise deployability without compromising accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing chart-QA agents focus on accuracy but lack transparency and assume proprietary API access, which is not suitable for regulated settings where data privacy is crucial.
...
Result: AgentFinVQA improves accuracy by 7.68 pp over a baseline with a proprietary backbone and by 4.84 pp with open-weights Qwen3.6-27B-FP8 served locally, and provides a useful confidence signal for human-in-the-loop review routing.

Conclusion: The paper demonstrates that auditable, on-premise financial chart QA is practical, and the open-weights system maintains most of the accuracy gains while ensuring full data residency.
...
TL;DR: The paper introduces ORAgentBench, a benchmark for evaluating autonomous agents on operations research tasks, and finds that current agents are far from reliable practice, with the best agent passing only 35.51% of tasks.


<details>
  <summary>Details</summary>
Motivation: The ability of large language models to perform realistic operations research (OR) work remains unclear, and existing OR evaluations have limitations, such as decoupling modeling from solving and relying on pre-formalized or text-only instances.

Method: The authors introduce ORAgentBench, an execution-grounded benchmark with 107 human-reviewed tasks, and evaluate fourteen frontier agent-model configurations using this benchmark.

Result: The best agent passes only 35.51% of all tasks and 20.59% of hard tasks, and many feasible submissions still fall below the required quality threshold.

Conclusion: The results suggest that progress in OR agents requires moving beyond plausible optimization code toward dependable, high-quality operational decision-making.
...
TL;DR: CombEval is a dynamic benchmark for evaluating combinatorial counting in large language models, revealing brittleness in models' reasoning abilities.


<details>
  <summary>Details</summary>
Motivation: To systematically assess and improve the combinatorial reasoning capabilities of large language models.

Method: CombEval uses typed Cofola specifications to generate natural-language counting problems with exact solver-verified answers, evaluating 11 LLMs in direct and code-augmented settings.

Result: Models show brittleness on ordered objects, indistinguishable elements, positional constraints, and nested object dependencies, with failures in constraint interpretation and counting principles.

Conclusion: CombEval provides a diagnostic testbed for studying LLMs' failures in combinatorial reasoning, with publicly available code and benchmark suites.
...
TL;DR: The paper introduces SEVRA, a controller that selectively verifies AI solver answers to balance accuracy and computational cost.


<details>
  <summary>Details</summary>
Motivation: The motivation is to optimize the trade-off between accuracy and computational cost in AI serving by selectively verifying solver answers.

Method: The method involves training recoverability-aware gates to decide whether to verify or preserve the initial answer from a frozen solver.

Result: The results show that selective verification can improve accuracy while reducing computational cost, with specific gains in accuracy and reductions in tokens across various datasets.

Conclusion: The conclusion is that selective recovery is useful but not always the best approach; instead, tuning the initial budget and using selective recovery when necessary is recommended.
...
TL;DR: The paper discusses the risks of autonomous Large Language Model (LLM) agents in electronic discovery (e-discovery) and proposes a verification architecture to mitigate errors.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the issue of 'trajectory collapse' in agentic workflows, where early misclassification can silently propagate and invalidate an entire privilege review.

Method: The paper proposes a structured taxonomy of agentic failures and a four-layer verification architecture, including planning, reasoning, execution, and uncertainty quantification.

Result: The results show that calibrated uncertainty thresholds can reduce privilege-waiver risk by up to 61% versus fully autonomous deployment, while routing fewer than one quarter of documents to attorney review.

Conclusion: The paper concludes that the proposed verification architecture can effectively mitigate errors in autonomous LLM agents in e-discovery, and that Human-on-the-Loop (HOTL) escalation thresholds can reduce privilege-waiver risk.
...
TL;DR: The paper proposes TelcoAgent, a framework for accurate, scalable, and explainable forecasting of Key Performance Measurements (KPMs) in 5G networks.


<details>
  <summary>Details</summary>
Motivation: Existing machine learning approaches for KPM forecasting face limitations in scalability and explainability, restricting their effectiveness in real-world deployments.

Method: The framework uses a foundation model-based approach, comprising three key components: a knowledge graph construction pipeline, a scalable time-series foundation model-based prediction pipeline, and a reasoning and explanation pipeline.

Result: TelcoAgent demonstrates high forecasting accuracy for all 7 considered KPMs per cell across 200 cells, while delivering explainable insights and actionable instructions to address network degradations.

Conclusion: The proposed TelcoAgent framework enables accurate, scalable, and explainable forecasting of multiple KPMs across diverse network cells without the need for site-specific training.
...
TL;DR: The paper reviews and benchmarks black-box uncertainty estimation methods for large language models, providing a unified evaluation framework and empirical guidance for future research.
...
TL;DR: A novel framework called MetaResearcher is proposed to improve the training of deep research agents by addressing limitations in current training methods.
...
Method: MetaResearcher includes an Evolving Virtual World, Discovery-Oriented Tasks, a Self-Reflective Meta-Reward mechanism, and a Heterogeneous Multi-Agent Swarm architecture.
...
Conclusion: MetaResearcher has the potential to substantially improve the training of deep research agents by addressing current limitations and promoting more genuine research behaviors.
...
<details>
  <summary>Details</summary>
Motivation: The decentralized deployment of LLM agents with diverse capabilities across tasks necessitates infrastructure for knowledge sharing across heterogeneous agent populations to avoid redundant discovery of solutions.
...
Conclusion: The results position MATM as a design pattern for population-level experience sharing in open agent ecosystems, enabling more efficient and effective task execution across diverse agent populations.
...
TL;DR: The paper proposes an element-based Convolutional Neural Network (CNN) to accelerate density-based Topology Optimization (TO) by predicting near-optimal densities for each element, reducing iterations and improving efficiency.
...
Method: The proposed method, eCNNTO, uses a CNN with residual connections to predict near-optimal densities for each element, taking into account spatial correlations among neighboring elements. A novel training strategy is introduced, using final stage density histories rather than early ones, to enhance optimization efficiency and reduce required training data size.

Result: The results demonstrate the generalization capabilities and efficiency of eCNNTO, achieving up to 90% and 97% reduction of iterations in two and three dimensions, respectively.

Conclusion: The proposed eCNNTO method can accelerate density-based Topology Optimization (TO) efficiently, requiring only a small dataset to train, and can be generalized to various problems with different boundary conditions, loading cases, design domain geometries, mesh resolutions, and non-design domains.
...
TL;DR: The paper explores autotelic AI, where agents generate their own goals, and discusses the consequences, challenges, and potential frameworks for such systems.


<details>
  <summary>Details</summary>
Motivation: The motivation is to explore the concept of autotelic AI, where agents generate their own goals, rather than having them specified by a designer.

Method: The paper traces the consequences of autotelic AI through various concepts, including intrinsic motivation, resource-driven priors, causal-interventional learning, homeostasis, and embeddedness. It also consolidates these developments into a single framework and extends it in three directions.
...
Conclusion: The paper concludes that autotelic AI requires a framework that can handle the generation and relativization of the self, and proposes a single framework with extensions in quantum formulation, philosophical reading, and LLM-based agentic instantiation.
...
<details>
  <summary>Details</summary>
Motivation: The existing co-speech generation pipelines are human-centric, causing a mismatch between human motion manifolds and humanoid embodiment constraints, disrupting embodiment consistency.

Method: The authors propose two frameworks: IK-EER, a prosody-preserving humanoid motion curation framework, and PhysDrift, an embodiment-aware co-speech motion generation framework that directly predicts executable humanoid joint trajectories from speech.

Result: The proposed frameworks improve speech-motion alignment, physical plausibility, motion smoothness, inference efficiency, and real-time interaction capability, demonstrated through extensive experiments and real-world humanoid deployment.

Conclusion: The authors conclude that embodiment-aware robot-native generation substantially improves co-speech motion generation for humanoid robots, maintaining embodiment consistency throughout training and inference.
...
TL;DR: The authors propose an automatic pipeline to generate a large-scale dataset, RAINbow, for training embodied agents in dialog-based navigation, achieving a state-of-the-art success rate.
...
Method: The authors create the RAINbow dataset with 238K episodes using an automatic generation pipeline and introduce Dual-Strategy Training and a localization model to improve performance.
...
Conclusion: The proposed dataset and training advances enable a state-of-the-art performance in dialog-based navigation for embodied agents.
...
<details>
  <summary>Details</summary>
Motivation: The pursuit of general physical intelligence is hindered by the need for human supervision and algorithm engineering in robotic manipulation, and the authors aim to bridge the gap between digital and physical automation.

Method: The authors propose a closed-loop system, ENPIRE, which consists of four core modules: Environment, Policy Improvement, Rollout, and Evolution, to automate robotics research and policy improvement.

Result: ENPIRE enables frontier coding agents to autonomously train a policy to achieve a 99% success rate on challenging manipulation tasks, such as organizing a pin box, fastening a zip tie, and tool use.
...
TL;DR: The paper proposes a new approach to reinforcement learning (RL) that challenges the conservative paradigm of relying on rollouts near the training distribution, instead introducing methods for reliable verification and diversified exploration.


<details>
  <summary>Details</summary>
Motivation: The core limitation of existing RL methods is not exploration itself, but the lack of reliable verification strategies to support broader exploration, which leads to reward hacking.

Method: The paper introduces two main methods: 'Reward as an Agent', an agentic reward framework that actively evaluates generated behaviors to provide robust reward signals, and 'Dynamic-Aware Rollout Diversification through DynDiff-GRPO', which diversifies trajectories and broadens state-action coverage.

Result: The unified approach enables RL on a more reliable reward foundation with substantially diversified sampling, effectively mitigating reward hacking while yielding significant accuracy gains across multiple open-source world models.

Conclusion: The paper demonstrates that broader exploration can scale successfully when grounded in robust verification, challenging the conservative paradigm in RL.
...
TL;DR: The paper evaluates two multi-agent systems, DAG Plan and Execute and ReAct, in enterprise scenarios and proposes a Task Manager for continuous operation, highlighting the impact of scale on performance.


<details>
  <summary>Details</summary>
Motivation: Existing multi-agent systems are limited in their ability to handle continuous event monitoring and action across specialist agents at enterprise scale.

Method: The authors evaluate DAG Plan and Execute and ReAct across 208 production-derived enterprise scenarios and introduce a Task Manager for continuous operation.

Result: The results show that scale dominates orchestration performance, with both architectures degrading at enterprise scale, and the Task Manager improving performance metrics.

Conclusion: The paper concludes that scale, not task complexity, is the primary bottleneck in multi-agent systems, and proposes a Task Manager to mitigate this issue.
...
TL;DR: This paper presents a method to improve reinforcement learning for formal reasoning by using a symbolic proof assistant as an oracle, providing dense and sound feedback during training.


<details>
  <summary>Details</summary>
Motivation: The motivation is to bridge the gap between structured processes and unstructured rewards in reinforcement learning from verifiable rewards (RLVR) by leveraging rich, fine-grained structured feedback from symbolic proof assistants.

Method: The method involves using the Lean proof assistant as a symbolic process oracle to supply verified feedback during training. This includes parsing proof attempts into tactic sequences and using Lean's elaboration marks to yield dense, verifier-grounded credit signals.

Result: The results show that tactic-level supervision outperforms outcome-only baselines in most settings, delivering improvements on benchmarks such as MiniF2F and ProofNet.

Conclusion: The study concludes that symbolic proof assistants can act as process-level reward oracles during training, opening a path toward reinforcement learning frameworks that combine the scalability of language models with the reliability of symbolic verification for formal reasoning.
...
TL;DR: Introduced a model-agnostic framework combining flow-based generative editing with evolutionary algorithms for data editing.


<details>
  <summary>Details</summary>
Motivation: Addressing the gap in flow-based settings where edits involve non-differentiable or black-box objectives.

Method: Residual-space evolutionary optimization framework operating in residual space with self-pollination and cross-pollination search regimes.

Result: Validated on MorphoMNIST and crystal data, demonstrating a useful mechanism for balancing target alignment, instance preservation, and diversity.

Conclusion: The proposed framework provides a useful mechanism for data editing in flow-based settings, extending beyond images to real-world scientific domains.
...
TL;DR: This paper proposes an optimized reinforcement learning approach for additive manufacturing process optimization, using a continuous action space and a novel attention-based architecture, achieving faster convergence and better results than traditional methods.


<details>
  <summary>Details</summary>
Motivation: Traditional reinforcement learning approaches for additive manufacturing process optimization suffer from slow convergence and susceptibility to local optima due to discrete action spaces, which limits their effectiveness in high-precision manufacturing tasks.

Method: The study employs a continuous action space combined with a novel architecture that integrates a multi-head attention mechanism with the Soft Actor-Critic (SAC) algorithm, enhancing the agent's ability to capture subtle variations in low-dimensional input features.
...
Conclusion: The proposed approach demonstrates faster convergence and higher final reward values compared to standard RL methods, making it a more effective solution for high-precision manufacturing tasks such as porosity prediction and process parameter optimization in laser powder bed fusion.
...
TL;DR: The authors propose ScaffoldAgent, a framework for optimizing outlines in open-ended deep research, leading to improved long-form report generation.


<details>
  <summary>Details</summary>
Motivation: Existing methods for outline generation in open-ended deep research have limitations, such as fixing the outline before writing or refining it with local heuristics, leading to scaffold drift and delayed feedback.
...
Result: Experiments show that ScaffoldAgent consistently improves long-form report generation and factual grounding over existing deep research agents.

Conclusion: The proposed ScaffoldAgent framework effectively addresses the limitations of existing methods for outline generation in open-ended deep research, leading to improved performance.
...
TL;DR: The paper develops a subject-aware prompting system for personalized education using LLMs, which adapts to diverse academic disciplines and improves instructional efficiency.
...
Method: The paper proposes a system with subject-aware prompting based on 14 pedagogical features, and uses a prompt routing model trained in a simulation environment and deployed for online adaptation with actual high-school students.
...
Conclusion: The proposed system improves instructional efficiency and effectiveness in personalized education using LLMs, and shows potential for sim-to-real transfer.
...
TL;DR: RACL, a Reasoning-Agent Control Layer, is introduced to enhance metaheuristics by adding a reasoning agent that controls the optimizer's search behavior without modifying business constraints, leading to improved or comparable results in vehicle routing experiments.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the performance of metaheuristics by introducing a reasoning agent that can observe, reason, and intervene in the optimization process without replacing or modifying the existing optimizer.

Method: RACL places a reasoning agent above an existing optimizer, which observes operational memory, reasons over past behavior, formulates bounded hypotheses, tests interventions, evaluates outcomes, applies guardrails, consolidates useful policies, and explains its decisions.

Result: RACL improves or ties the Operational Memory Policy in 21 of 21 feasible cases and improves or ties a non-reasoning Stagnation-Triggered Policy in 18 of 21 feasible cases, with an average cost delta of -0.641% compared to STP, and shows significant improvements in specific runtime samples.

Conclusion: The RACL method provides a way for a reasoning agent to discover, validate, consolidate, and explain algorithmic control rules for a metaheuristic, demonstrating improved or comparable performance in vehicle routing experiments.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of current CAD benchmarks, which focus on creating new models rather than editing existing ones, and to evaluate LLMs on understanding and editing complex engineering models with semantics and relations.

Method: The authors introduce BIM-Edit, a benchmark consisting of 324 editing tasks across 11 realistic and 36 synthetic building models, with tasks expressed in three instruction categories: direct, spatial, and topological.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome the fragility of training PINNs, specifically the deterioration of conflict-averse optimization schemes as model capacity increases, leading to a capacity-induced failure mode.

Method: The method proposed is Modular-Sparsity Synchronization (ModSync), which integrates structural optimization into conflict-averse training by penalizing task-exclusive connections while preserving interaction-promoting pathways.

Result: The results demonstrate that ModSync consistently prevents capacity-driven failures, sustains robust cross-objective coupling, and achieves state-of-the-art accuracy across diverse PDE benchmarks.

Conclusion: The conclusion is that ModSync is an effective framework for improving the training of PINNs, addressing the capacity-induced failure mode, and achieving high accuracy in solving PDEs.
...
TL;DR: The paper proposes a novel hypergraph-based framework, HISR, for semantic-aware communication, which improves semantic inference accuracy by representing complex multi-entity relationships.


<details>
  <summary>Details</summary>
Motivation: Existing solutions for semantic-aware communication use graphs that capture only pairwise relationships, neglecting higher-order implicit correlations, which limits semantic expressiveness and leads to performance degradation.

Method: The paper proposes a hypergraph-based implicit semantic reasoning framework, HISR, which maps entities and their associated higher-order relations into dedicated semantic subspaces.
...
Conclusion: The proposed HISR framework offers a robust semantic inference approach for semantic-aware communication, improving accuracy and mitigating performance degradation under noisy or corrupted channel conditions.
...
TL;DR: The study reveals that psychological profiles assigned to large language models (LLMs) using human-designed instruments are largely measurement artifacts due to a directional response bias.
...
Conclusion: The apparent psychological profiles of LLMs are artifacts of the instrument used to measure them, not properties of the models themselves, highlighting the need for dedicated assessments centered on response orthogonality.
...
TL;DR: The authors introduce a new evaluation metric called Rule Violation Score (RVS) to measure the logical consistency of machine learning models with predefined rules, which is crucial in high-stakes applications.
...
Method: The authors propose RVS, a metric that assesses the extent to which a model respects logical rules, and can be applied to any dataset and predictive model expressed over a relational vocabulary.
...
Conclusion: The introduction of RVS provides a new dimension for evaluating machine learning models, emphasizing the importance of logical consistency in addition to predictive accuracy.
...
TL;DR: The paper discusses using reinforcement learning to improve game AI, making in-game characters more believable and immersive, and proposes a framework for training models suited for game development.


<details>
  <summary>Details</summary>
Motivation: The limitations of current game AI systems lead to frustration and break the illusion of realism, and machine learning models offer a promising solution to create more human-like behavior.

Method: The authors propose a framework for training reinforcement learning models with specific requirements for game AI and game development, and discuss practicalities of deploying machine learning agents in modern games.

Result: The paper identifies bottlenecks and hard problems in using reinforcement learning for game AI, and suggests promising research directions to accelerate adoption in the video game industry.

Conclusion: The authors envision more applications of reinforcement learning for game AI in the future, but note that current research limitations must be addressed for broad deployment across game genres.
...
Method: The authors propose QMFOL, an automated framework that generates monadic first-order logic reasoning tasks with quantifiable and controllable complexity. It constructs formal logical structures and translates them into natural language via LLMs.

Result: Evaluations on six large reasoning models and two LLMs show that performance degrades and computational overhead increases with rising logical complexity. Models perform better on True-labeled tasks than on False or Unknown ones, and exhibit sensitivity to semantic variation.
...
TL;DR: The authors propose a definition of intelligence as the ability to increase the probability of unlikely but valid outcomes, and present a framework that makes intelligence measurable on a universal scale.


<details>
  <summary>Details</summary>
Motivation: To define and measure intelligence in a universal way, applicable to various systems from passive matter to complex models like humans and large language models.

Method: The authors propose an architecture based on recursive self-simulation, where an intelligent system models the world and its own place within it, and represents futures in which its own actions are part of the trajectory.

Result: The authors derive a necessity statement and a conditional near-sufficiency statement connecting their architecture to a precise thermodynamic measure of intelligence, showing that recursive self-simulation is necessary and nearly sufficient for high thermodynamic intelligence.

Conclusion: The proposed framework provides a universal scale to measure intelligence, making it possible to compare and evaluate the intelligence of various systems, from simple to complex.
...
TL;DR: The paper presents MAMO, a multi-agent RL approach for constrained optimization problems in dynamic environments, which decouples task execution from objective design and learns to balance competing objectives.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of selecting appropriate weights for penalty terms in Lagrangian-inspired formulations for constrained optimization problems in dynamic environments, where manual selection is difficult and may not adapt to changing relative importance of objectives and constraints.

Method: The method involves using a multi-agent RL approach, called MAMO, which formulates the selection of reward weights as a learning problem, allowing for more autonomous and robust RL-based solutions.

Result: The paper proposes MAMO as a solution to tackle the balancing problem between optimizing the primary objective and avoiding constraint violations in dynamic environments.

Conclusion: The conclusion is that MAMO provides a first step towards more autonomous and robust RL-based solutions for constrained optimization problems in dynamic environments by decoupling task execution from objective design.
...
<details>
  <summary>Details</summary>
Motivation: Existing approaches to integrating external knowledge in LLMs assume that either the model or the provided context is reliable, and avoid conflicts by privileging one source over the other, rather than actively resolving inconsistencies.

Method: The MACR framework consists of an adaptive knowledge assessment and retrieval approach, which estimates an LLM's confidence in its answer to a given query, and an inductive multi-agent reasoning framework with three specialized agents that induce explicit rules, analyze potential conflicts, and resolve inconsistencies across all available contexts.
...
Conclusion: The MACR framework addresses the limitations of existing approaches to integrating external knowledge in LLMs by actively resolving inconsistencies and providing a more robust and interpretable approach to knowledge conflict resolution.
...
TL;DR: Automated scoring of student-generated scientific drawings using a Vision Transformer (ViT) with a confidence-aware framework.


<details>
  <summary>Details</summary>
Motivation: Scoring student-generated drawings requires expert human judgment, making large-scale assessment costly; need for automated scoring method.

Method: Vision Transformer (ViT) with parameter-efficient adaptation and a confidence-aware scoring framework.

Result: Improved scoring reliability, practical trade-off between automated coverage and scoring risk.

Conclusion: Confidence-aware methods can enable trustworthy educational assessment by automating scoring of student-generated drawings.
...
TL;DR: The paper proposes Lagrange, a framework for end-to-end autonomous driving in complex environments, combining open-vocabulary reasoning with efficient and kinematically valid trajectory planning.


<details>
  <summary>Details</summary>
Motivation: The need for autonomous driving systems that can generalize to anomalous scenarios and produce kinematically valid trajectories in complex, open-world environments.

Method: The proposed framework, Lagrange, uses Masked Latent Fields (MLF) and Vision-Language Models (VLMs) to encode object proposals into continuous semantic visual tokens, and an intent-driven masked cross-attention module to temporally filter irrelevant entities.

Result: Extensive offline evaluations on standard (nuScenes) and long-tail (CODA) benchmarks demonstrate that Lagrange establishes a promising framework for robust, interpretable, and kinematically feasible open-world autonomy.

Conclusion: The proposed Lagrange framework addresses the challenges of autonomous driving in complex environments by combining open-vocabulary reasoning with efficient and kinematically valid trajectory planning.
...
TL;DR: The paper proposes a novel approach to building Intelligent Fault Diagnosis Systems using Deep Transfer Learning, addressing the challenge of limited labelled data by leveraging periodic multi-excitation levels and data visualization techniques.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome the challenge of obtaining large amounts of labelled data for building Intelligent Fault Diagnosis Systems, particularly in cases of machine or structure faults.
...
Result: The proposed method was experimentally validated on a railway pantograph structure, providing effective support for its application.

Conclusion: The paper concludes that the proposed approach can efficiently build Intelligent Fault Diagnosis Systems using Deep Transfer Learning, even under conditions of strong data scarcity.
...
TL;DR: The paper proposes SoftSkill, a method that uses compact continuous context objects, called soft skills, to improve task performance by tuning them with next-token prediction and deploying them as latent behavioral priors at inference time.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome the limitations of using natural-language Markdown files to deploy agent skills, which are consumed indirectly and require a frozen language model to translate them into generation-time behavior.

Method: The proposed method, SoftSkill, uses a frozen-backbone approach that tunes soft skills with next-token prediction and deploys them as latent behavioral priors at inference time.

Result: The results show that SoftSkill improves performance over no-skill prompting and SkillOpt, with improvements of up to 42.1 points on LiveMath and 12.5 points on LiveMath, while replacing hundreds to thousands of Markdown skill tokens with a few virtual tokens.
...
TL;DR: The paper explores mining explicit skill libraries from interaction data to improve computer-using agents. It presents a three-stage pipeline for segmenting GUI trajectories, clustering skills, and training skill-aware policies. However, the study finds that while mined skill clusters show high purity, they do not reliably improve cross-domain policy performance.


<details>
  <summary>Details</summary>
Motivation: To investigate whether explicit skill libraries can be mined from interaction data to improve downstream policies in computer-using agents.

Method: A three-stage pipeline: 1) segments GUI trajectories, 2) clusters segments into candidate skills, and 3) trains a skill-aware policy from the resulting annotations.

Result: The mined clusters show high purity (5/8 clusters with >= 0.95 purity). However, using these clusters in GRPO only slightly improves skill-step accuracy from 18.5% to 20.5% and does not significantly improve performance on other metrics.

Conclusion: The study concludes that while trajectory mining can expose inspectable skill structure, current methods are insufficient for reliable cross-domain policy improvement.
...
TL;DR: The study identifies a limitation in current FP4 training methods, proposing a new uniform 4-bit training recipe called UFP4, which achieves lower loss degradation than existing methods.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of current FP4 hardware and recipes, specifically the E2M1 data elements, which suffer from Shrinkage Bias, leading to training instability.
...
Conclusion: The study suggests that future accelerators should support E1M2/INT4-style uniform 4-bit grids as first-class training primitives alongside E2M1, as UFP4 shows improved performance and stability.
...
TL;DR: This paper proposes an attention-guided deep learning framework for sperm morphology classification, improving accuracy and interpretability.


<details>
  <summary>Details</summary>
Motivation: Male infertility caused by abnormal sperm morphology lacks interpretable automated analysis, hindering clinical adoption of deep learning models.

Method: An attention-guided framework combining pretrained EfficientNet-B0 with Convolutional Block Attention Module (CBAM) for sperm morphology classification.

Result: The model achieves accuracies of 90.2% and 93.9% on SMIDS and HuSHem datasets, outperforming SimpleCNN and standard EfficientNet-B0.

Conclusion: The proposed framework provides accurate and transparent automated sperm analysis for fertility clinics.
...
TL;DR: The paper presents a method to improve IVF pregnancy rate predictions by incorporating high-resolution laboratory environmental data, achieving significant error reduction.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve IVF pregnancy rate predictions by utilizing underutilized high-resolution laboratory environmental data.

Method: The method involves engineering 55 context-aware temporal features from raw sensor data and using a hierarchical Bayesian Beta regression model for transfer learning across clinics.

Result: The results show a significant reduction in prediction error (1.27% vs 3-5%) and an R2 of 0.86 on held-out data from a Northern European clinic.

Conclusion: The study concludes that structured environmental monitoring contains clinically meaningful, transferable signal that can improve IVF pregnancy rate predictions.
...
TL;DR: The study examines how language models interpret different types of compliance demonstrations, finding that benign and harmful demonstrations have varying effects on model behavior depending on the model, demonstration composition, and training methodology.
...
Method: The study tests three hypotheses about demonstration composition using a mix of benign and harmful compliance demonstrations across four models.

Result: The findings show that benign and harmful demonstrations are not interchangeable, and their effects on model behavior depend on the model, demonstration ordering, and training methodology. Specifically, benign demonstrations can either reduce or increase harmful compliance depending on the model, and preference optimization is crucial in preventing benign demonstrations from increasing harmful compliance.

Conclusion: The study characterizes how demonstration-based jailbreaking works, highlighting that what models extract from compliance demonstrations depends on demonstration content, ordering, and training methodology.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to evaluate LLMs' ability to generalize across diverse programming languages required in real-world software engineering, as the existing LCB benchmark is limited to Python.
...
Conclusion: The paper concludes that Multi-LCB is a rigorous new benchmark for multi-programming-language code evaluation, directly addressing LCB's primary limitation and exposing critical gaps in current LLM capabilities.
...
TL;DR: FlowEdit, a framework for adapting flow-matching text-to-speech systems, corrects pronunciation errors for out-of-vocabulary words through latent conditioning edits.


<details>
  <summary>Details</summary>
Motivation: Flow-matching text-to-speech systems have zero-shot quality but struggle with pronunciation errors on out-of-vocabulary proper nouns, requiring retraining for corrections.

Method: FlowEdit optimizes token-level perturbations in the text embedding space based on corrective feedback and stores corrections in a Modern Hopfield Network for retrieval via soft attention.

Result: FlowEdit reduces target-word Phoneme Error Rate by 92.7% on a multilingual benchmark of 312 proper nouns across 18 language families while maintaining general-speech quality.

Conclusion: FlowEdit enables efficient, life-long adaptation of flow-matching TTS systems, correcting pronunciation errors without retraining and maintaining high speech quality.
...
TL;DR: DeepSWIP introduces a single-world counterfactual semantics for neurosymbolic systems like DeepProbLog, enabling efficient counterfactual reasoning.


<details>
  <summary>Details</summary>
Motivation: Standard inference in neurosymbolic systems is associational and lacks causal semantics for interventions and evidence, making counterfactual reasoning challenging.

Method: DeepSWIP uses neural materialization to reduce fixed-context neural predicates to ordinary ProbLog choices and applies Single World Intervention Programs (SWIPs) to compute counterfactuals by weighted model counting (WMC).

Result: DeepSWIP achieves exactness relative to the learned materialized FCM under certain assumptions and shows a 2.14x inference speedup. Experiments validate the approach and highlight the importance of correct scoping in estimators.

Conclusion: DeepSWIP provides a novel approach to counterfactual reasoning in neurosymbolic systems, offering improved efficiency and accuracy.
...
TL;DR: LedgerAgent improves tool-calling agents by maintaining a separate task state ledger, enhancing policy adherence and state management across customer-service domains.


<details>
  <summary>Details</summary>
Motivation: Current tool-calling agents struggle with implicit state management, leading to failures in retrieving correct information and adhering to domain policies. The authors aim to address these issues.

Method: The authors introduce LedgerAgent, an inference-time method that maintains a separate ledger for task states and uses it to render states into the prompt and check policy constraints.

Result: LedgerAgent improves average pass^k over a standard prompt-based approach across four customer-service domains, with significant gains under stricter multi-trial consistency metrics.

Conclusion: The proposed LedgerAgent method effectively enhances the performance of tool-calling agents in customer-service domains by improving state management and policy adherence.
...
TL;DR: This paper proposes a method to analyze how individual words in style captions influence acoustic output in text-to-speech systems, and applies it to a specific model.


<details>
  <summary>Details</summary>
Motivation: Understanding how individual words in style captions control voice characteristics is crucial for improving controllability and diagnosing failure modes in expressive text-to-speech systems.
...
Conclusion: This study provides insights into how natural language influences cross-attention in speech diffusion models, which can help improve controllability and diagnose failure modes in expressive text-to-speech systems.
...
Result: The results show that expert calibration is sufficient for hard-routed models but not for soft-routed models under distribution shift. The proposed adversarial reweighting method improves the accuracy-calibration tradeoff on average and on difficult subsets of the data.

Conclusion: The conclusion is that expert calibration is important for MoE models, but additional methods are needed to ensure calibration under distribution shift, particularly for soft-routed models. The proposed adversarial reweighting method is effective in improving the accuracy-calibration tradeoff.
