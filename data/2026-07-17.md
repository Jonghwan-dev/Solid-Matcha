...
TL;DR: Proposed a Cell-Distribution Normalization (C-Norm) method and a hybrid YOLOv12-DINOv3 architecture to improve AI detection of cervical cancer cells in ThinPrep Cytologic Test (TCT) slides, addressing issues of unbalanced cell distribution and limited annotated data.


<details>
  <summary>Details</summary>
Motivation: Existing AI models for TCT image analysis perform poorly due to unbalanced cell distribution and limited high-quality annotated data, leading to inconsistent diagnostic results among cytopathologists.

Method: The proposed method involves Cell-Distribution Normalization (C-Norm) to ensure uniform cell distribution, and a hybrid YOLOv12-DINOv3 architecture for precise recognition of TCT images.

Result: The proposed method achieves state-of-the-art performance, significantly outperforming mainstream detection algorithms in TCT image analysis.

Conclusion: The proposed method has the potential to improve the accuracy and efficiency of cervical cancer screening by AI-powered TCT image analysis, addressing the limitations of existing models.
...
TL;DR: Introduced Boogu-Image-0.1, an open-source multimodal model family for text-to-image generation, editing, and bilingual text rendering, achieving competitive performance at lower costs.


<details>
  <summary>Details</summary>
Motivation: To develop an open-source unified multimodal understanding and generation model that matches or surpasses closed-source systems, while being cost-effective and accessible to the broader research community.

Method: Targeted improvements in model understanding, data quality, training pipelines, and agentic inference-time scaling, under constrained compute budgets.

Result: Boogu-Image-0.1 consistently matches or surpasses other open-source models across standard benchmarks, approaching results of leading closed-source systems, using only 208.62 million unique images and approximately $400K in training costs.

Conclusion: The development of Boogu-Image-0.1 demonstrates the potential for open-source multimodal models to achieve competitive performance at lower costs, and the authors share their code, weights, and recipes to advance the open ecosystem.
...
TL;DR: A Transformer-based Masked Autoencoder is used for unsupervised learning of steel surface defect representations, achieving 91.3% clustering accuracy.


<details>
  <summary>Details</summary>
Motivation: The scarcity and cost of labeled defect data motivate the use of self-supervised methods for learning useful representations of steel surface defects.

Method: A Transformer-based Masked Autoencoder is pretrained with 75% of input image patches randomly masked, and a lightweight decoder reconstructs the masked regions. The encoder is trained with an auxiliary defect localization objective.

Result: The decoder achieves a structural similarity score of 0.92 and a mean squared error of 0.47. The pretrained encoder features are clustered with 91.3% Hungarian matched accuracy against six known defect categories.

Conclusion: The proposed method effectively learns representations of steel surface defects without class labels, enabling unsupervised grouping with high accuracy.
...
TL;DR: Proposed MGFace, a mask-gated face identification pipeline that improves accuracy and efficiency in identifying faces with masks.


<details>
  <summary>Details</summary>
Motivation: Face identification accuracy degrades with partial occlusion, especially with facial masks. Existing re-ranking approaches are costly and inefficient for large-scale retrieval.

Method: MGFace predicts mask status of query faces and routes similarity computation accordingly, using global embedding matching for unmasked queries and mask-aware patch-level re-ranking for masked queries.

Result: MGFace achieves over 80% identification accuracy with FaceNet and over 90% with ArcFace on the extended LFW-Mask dataset, with 20x reduced query time compared to EMD-based re-ranking.

Conclusion: MGFace effectively improves masked-face identification accuracy with low computational overhead.
...
TL;DR: This paper compares two training paradigms for self-supervised learning: pretraining followed by finetuning (PFT) and joint training (JT).


<details>
  <summary>Details</summary>
Motivation: The interaction between self-supervised and supervised learning objectives is not well understood, and existing techniques primarily adopt a two-stage approach.

Method: The authors compare PFT and JT across eight representative SSL methods and diverse computer vision tasks, evaluating performance under varying percentages of labeled data.

Result: The results show that JT improves data and training efficiency and is robust in low-label settings, while PFT is more reliable in specialized domains.

Conclusion: The authors establish a comprehensive empirical benchmark for hybrid SSL-based semi-supervised learning and offer practical guidance for selecting appropriate training strategies.
...
TL;DR: The paper presents BitMind Forensics (BMF), a deepfake detector trained through an open adversarial competition that continually refreshes the training distribution, achieving high performance on various benchmarks and outperforming state-of-the-art models.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the gap between academic benchmarks and real-world performance of deepfake detectors, which often collapse when faced with new, unseen data.

Method: The method involves training BMF through Bittensor SN34, an open adversarial competition that continually updates the training distribution, and evaluating it on 19 public datasets.

Result: The results show that BMF achieves high AUC scores on various benchmarks, including 0.936 on Sumsub's original images, 0.872 on its full four-condition manipulation battery, and outperforming state-of-the-art models on Deepfake-Eval-2024 and other benchmarks.

Conclusion: The conclusion is that BMF is a robust deepfake detector that can effectively detect deepfakes in real-world scenarios, and that continual updating of the training distribution is essential for achieving high performance.
...
TL;DR: A human-in-the-loop framework combining active learning and dual-loss optimization reduces surgical video annotation effort by 50%, enabling efficient tool segmentation model development.


<details>
  <summary>Details</summary>
Motivation: Precise spatial-temporal annotation of laparoscopic videos is time-consuming and requires expert knowledge, hindering the development of surgical tool segmentation models.

Method: A foundation model generates temporally consistent class activation maps using two complementary training objectives: weak supervision loss on video-level tool presence labels and image-level mask loss on human-corrected annotations obtained through active learning.

Result: The framework reduces annotation effort by 50% compared to fully manual annotation and enables scalability to larger, more diverse datasets and real-world clinical settings.

Conclusion: The proposed framework provides a practical and deployable strategy for expanding tool segmentation to larger datasets and real-world clinical settings with minimal expert input.
...
TL;DR: The paper presents JITOMA, a framework that enables real-time, low-latency processing of 3D scene graphs for embodied agents by selectively activating relevant parts of the graph.


<details>
  <summary>Details</summary>
Motivation: The conventional approach to processing 3D scene graphs leads to observation redundancy and perceptual saturation, which conflicts with the demands of edge platforms for real-time and low-latency processing.

Method: The JITOMA framework unifies task reasoning, perception, and memory into a just-in-time growth process, using a top-down task heatmap to filter observations and selectively activate task-relevant anchors in the graph.

Result: The experiments demonstrate that JITOMA substantially reduces active graph size and captioning latency while maintaining stable processing time under long-horizon task switching.

Conclusion: The JITOMA framework offers a solution to the challenges of processing 3D scene graphs in real-time, low-latency applications, enabling more efficient and effective processing.
...
TL;DR: AffectFlow-DINO, a multi-task learning system, is proposed for the 11th ABAW challenge. It extends a deterministic architecture with a conditional rectified-flow head to model facial behavior ambiguity, achieving better performance than the baseline.


<details>
  <summary>Details</summary>
Motivation: The inherent ambiguity of in-the-wild facial behavior makes it challenging to accurately estimate affective states. Existing methods typically predict a single affect estimate, which may not capture the complexity of facial expressions.

Method: The proposed AffectFlow-DINO system uses a conditional rectified-flow head to learn a conditional generative distribution, enabling uncertainty-aware one-to-many predictions through Monte Carlo sampling. It jointly estimates continuous valence-arousal, classifies eight facial expressions, and detects twelve Action Units from static face images.

Result: The system achieves P_MTL = 1.177, outperforming the official challenge baseline of P_MTL = 0.45. Rectified-flow decoding improves deterministic prediction, particularly for valence-arousal estimation (CCC-V +0.058). Post-hoc threshold calibration recovers performance on severely imbalanced rare classes.

Conclusion: AffectFlow-DINO demonstrates the effectiveness of modeling facial behavior ambiguity using a conditional rectified-flow head. The proposed system achieves state-of-the-art performance on the 11th ABAW challenge, showcasing its potential for affective computing applications.
...
TL;DR: The paper proposes a new method for differentiable rendering that incorporates polarization, enabling more accurate estimation of scene geometry and material properties.


<details>
  <summary>Details</summary>
Motivation: Most existing differentiable rendering methods ignore polarization cues, which are valuable for constraining scene geometry and material properties. However, extending reverse-mode differentiation to polarized light presents significant challenges due to the rank-deficient nature of common polarimetric operators.
...
Result: The proposed method enables efficient and stable optimization of material and lighting parameters in complex scenes, broadening the applicability of physically based inverse rendering.

Conclusion: The paper presents a novel method for physically based differentiable rendering that incorporates polarization, addressing the challenges of extending reverse-mode differentiation to polarized light and enabling more accurate estimation of scene geometry and material properties.
...
TL;DR: FOLIO, a training-free focused semantic memory system, efficiently handles online streaming video understanding by selectively retaining important information and reducing memory costs.


<details>
  <summary>Details</summary>
Motivation: The challenge of managing growing video streams and queries in real-time, while minimizing memory costs and redundant visual details, and providing relevant information for future queries.

Method: FOLIO uses a dynamic focus state to update memory at the segment level, combining a short-term visual buffer and a long-term semantic memory organized around observed entities, and linked to a visual-evidence cache.

Result: FOLIO achieves state-of-the-art performance on OVO-Bench (82.0/69.1 Perception/Backward accuracy) and StreamingBench (74.5 overall accuracy), while reducing memory costs.

Conclusion: FOLIO provides an efficient solution for online streaming video understanding, balancing performance and memory costs by selectively retaining important information.
...
TL;DR: The authors propose fine-tuning diffusion generative models with Fréchet Distance loss (FD-loss) to improve synthetic medical image generation, particularly for heterogeneous tumors with irregular boundaries.


<details>
  <summary>Details</summary>
Motivation: Current diffusion generative models struggle to capture complex morphological characteristics of heterogeneous tumors, limiting their utility for downstream clinical tasks such as segmentation.

Method: The authors propose using Fréchet Distance loss (FD-loss) to fine-tune diffusion generative models, which aligns the first and second-order feature statistics of real and generated images in a pre-trained encoder space.

Result: Downstream segmentation networks trained on FD-regularized synthetic data achieve superior performance, improving tumor DSC by >5% over unregularized synthetic augmentation alone.

Conclusion: FD-loss is an effective regularizer for medical image generative models, improving clinical workflows by generating more faithful tumor synthesis and reducing segmentation hallucinations.
...
TL;DR: The paper questions the assumption that high benchmark accuracy in video large language models (LLMs) implies visual understanding, finding that accuracy and visual dependency are separable.


<details>
  <summary>Details</summary>
Motivation: To audit the assumption that benchmark accuracy in video LLMs is evidence of visual understanding.

Method: The authors introduce the Visual Dependency Gap (VDG) and use paired McNemar tests on MVBench, a diagnostic ladder, and ablation studies to evaluate 20 models across 2-78B parameters and ten architecture families.

Result: The study finds that accuracy and visual dependency are separable, and that frame diversity supplies most of the visual benefit, while temporal order contributes near-zero accuracy. The results also show that stable aggregate accuracy conceals bidirectional question-level answer flips.

Conclusion: The authors conclude that VDG should be a standard audit for whether video benchmarks measure visually grounded capability, and that high benchmark accuracy does not necessarily imply visual understanding.
...
TL;DR: The authors propose a method for procedural material creation in 3D design, leveraging expert demonstrations to generate high-quality materials.


<details>
  <summary>Details</summary>
Motivation: Current procedural material creation methods require reproducing node graphs, but achieving high-quality results demands understanding the process experts use to construct materials.

Method: The authors represent expert workflows as process traces, which are then used to synthesize executable material graphs using a pretrained LLM-based ProcessSynthesizer and Compiler.

Result: The approach was found to produce workflows requiring fewer edits and more closely matching professional design strategies than existing methods, with superior generation and editing performance in a user study with 150 participants.

Conclusion: The authors' method for procedural material creation, leveraging expert demonstrations, results in improved performance and more professional design strategies, enabling more efficient and effective material creation.
...
TL;DR: Proposed SARFA, a novel framework for improved medical image segmentation, especially in ambiguous cases.


<details>
  <summary>Details</summary>
Motivation: The Segment Anything Model (SAM) struggles with ambiguous targets in medical imaging, where accurate delineation is vital but expert radiologists may disagree.

Method: SARFA uses probabilistic prompting to generate diverse masks and optimizes them with a radiomics-driven training objective based on Fréchet Radiomic Distance (FRD) and Direct Preference Optimization (DPO).

Result: SARFA outperforms existing ambiguous segmentation methods on CT and MRI benchmarks.

Conclusion: SARFA effectively improves medical image segmentation by aligning anatomical and textural characteristics with clinically meaningful ground truth representations.
...
TL;DR: The paper proposes a method called Temporally Consistent Universal Adversarial Perturbations (TC-UAP) to protect videos against personalized customization while preserving privacy and intellectual property.


<details>
  <summary>Details</summary>
Motivation: The increasing capability of diffusion-based video generation models to enable high-quality personalized video customization raises concerns about personal privacy, identity ownership, and intellectual property protection.

Method: The authors propose TC-UAP, a protection method that optimizes an identity-level multi-frame UAP over sliding windows from multiple videos, accounting for local temporal dependencies and enabling a single perturbation to protect unseen videos of varying lengths.

Result: TC-UAP achieves the strongest identity protection compared with existing methods under both reference- and tuning-based video customization and remains robust under multiple unseen temporal attacks.

Conclusion: The proposed TC-UAP method effectively addresses the challenges of protecting videos against both reference- and tuning-based customization, providing a robust solution for preserving privacy and intellectual property in video generation.
...
TL;DR: The authors propose a marker-free augmented reality (AR) workflow to improve the accuracy of mapping positive margin labels from a resected specimen to the patient resection bed in head and neck oncologic surgery.


<details>
  <summary>Details</summary>
Motivation: The challenge of accurately relocating positive margins from a resected specimen to the patient resection bed using verbal descriptions only, which can lead to inaccurate tumor resection.

Method: The proposed method combines contour-constrained deformation, residual alignment to a depth scan, surface-based fusion to a head-mounted display, and target projection onto the reconstructed bed, without requiring external fiducial markers.

Result: The results show that the marker-free AR guidance significantly improves the accuracy of margin localization, reducing the end-to-end margin localization error from 21.40 mm and 16.09 mm with verbal guidance and specimen examination, respectively, to 6.19 mm with AR guidance.

Conclusion: The study demonstrates the effectiveness of a marker-free AR workflow for positive-margin localization, supporting more precise tumor resection in head and neck oncologic surgery.
...
TL;DR: An audio-text system is proposed for recognizing ambivalence and hesitancy in videos, achieving high performance on the development set.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the Ambivalence/Hesitancy Video Recognition Challenge in the 11th ABAW Competition.

Method: The method involves representing videos as overlapping 5-second windows with audio and text features, fused via temporal cross-attention, and using gated multiple-instance learning (MIL) pooling.

Result: The ensemble model achieved an average precision of 0.875 and a macro-F1 of 0.72 on the labeled public development set.

Conclusion: The proposed audio-text system shows promising results in recognizing ambivalence and hesitancy in videos, with publicly available source code.
...
TL;DR: Object detectors remain confident even when objects are heavily occluded, leading to incorrect visibility signals.


<details>
  <summary>Details</summary>
Motivation: The confidence of open-vocabulary detectors in object presence does not accurately reflect the object's visibility, especially under occlusion.

Method: The study uses a geometry-segmentation oracle to provide detector-free ground-truth visibility, comparing it with the detector's confidence across various scenarios, including occlusion and distractors.

Result: Detectors report objects as present even when heavily occluded (99% in occluded frames on real video), and their confidence remains high and uncorrelated with actual visibility.

Conclusion: The study suggests that using confidence-based metrics and gates can lead to incorrect conclusions, recommending instead the use of target-grounded signals for more accurate evaluation and gating.
...
TL;DR: The paper proposes Differentiable Geometry Image (DiffGI), a framework for 3D-to-2D mapping that integrates surface representation and geometric optimization, improving reconstruction fidelity and boundary precision.


<details>
  <summary>Details</summary>
Motivation: Existing 3D generative models struggle with thin-shell and non-manifold geometries, and geometry image-based approaches have limitations such as staircase artifacts and non-differentiable surface reconstruction.

Method: The authors propose DiffGI, which uses a continuous 2D Truncated Signed Distance Function (TSDF) and a differentiable Marching Squares algorithm, and train a DiffGI-VAE and a transformer-based latent diffusion model.

Result: The method achieves superior reconstruction fidelity and boundary precision compared to prior approaches, while requiring fewer computational resources.

Conclusion: The proposed DiffGI framework enables efficient and accurate 3D-to-2D mapping, with applications in 3D generation and reconstruction.
...
TL;DR: A new method called RoughNet uses optical satellite imagery to estimate sea ice roughness, providing a scalable solution for high-resolution sea ice mapping.


<details>
  <summary>Details</summary>
Motivation: The estimation of landfast sea ice roughness is crucial for climate modeling and safe Arctic travel, but existing methods are limited by high costs and sparse data.

Method: RoughNet, a conditional diffusion framework, maps 10m Sentinel-2 multispectral images to 1m surface elevation residual fields, enabling fine-scale roughness characterization.

Result: The best-performing model achieves an out-of-domain root mean squared error of 9cm, preserving statistical and spectral properties of the roughness field.

Conclusion: Generative diffusion models can recover physically meaningful surface structure from optical imagery, providing a scalable pathway for high-resolution sea ice mapping and roughness estimation.
...
TL;DR: FM$^2$, a unified framework for federated foundation models in medical imaging, is proposed to address Imaging Modality Heterogeneity.


<details>
  <summary>Details</summary>
Motivation: The need for pooling medical imaging data across institutions while adhering to privacy regulations, and the limitations of existing federated foundation models.

Method: FM$^2$ trains a core backbone from scratch and incorporates biomedical pretrained encoders, with dual Mixture-of-Experts modules and a Heterogeneous Modality Alignment regularizer.

Result: FM$^2$ demonstrates superiority over state-of-the-art federated baselines and strong out-of-modality generalization across classification, caption-enhanced learning, and federated medical VQA tasks.

Conclusion: FM$^2$ provides a unified and effective solution for federated foundation models in medical imaging, addressing the challenge of Imaging Modality Heterogeneity.
...
TL;DR: AnomExpert, a prototype-driven framework for prenatal US anomaly diagnosis, is proposed, achieving 86.9% accuracy and 84.2% F1-score on a multi-center dataset.


<details>
  <summary>Details</summary>
Motivation: Accurate prenatal diagnosis of life-limiting congenital anomalies requires identifying anatomical planes and selecting diagnostically relevant planes, but existing automated methods have limitations.

Method: AnomExpert uses learnable plane prototypes and a disease-aware sparse selection mechanism to organize unordered images into latent representations and select diagnostically relevant planes.

Result: AnomExpert outperforms nine representative multi-instance learning methods, achieving 86.9% accuracy and 84.2% F1-score on a multi-center dataset of 3,654 cases.
...
TL;DR: This paper introduces MultiAnimate, a framework for animating multiple characters in a shared environment while preserving identity consistency and spatial relationships.


<details>
  <summary>Details</summary>
Motivation: Existing character image animation approaches focus on single reference images, limiting their applicability in multi-character interaction scenarios.

Method: MultiAnimate uses an identity-specific reference net, identity-aware pose encoder, and interaction guider module to enable concurrent animation of multiple characters.

Result: Extensive experiments and ablation analyses demonstrate MultiAnimate's superiority in multiple character animation, especially in complex motion sequences.

Conclusion: MultiAnimate effectively addresses the challenges of multi-character animation, offering a comprehensive framework for concurrent character animation in shared environments.
...
TL;DR: The paper proposes ScanFocus, a novel framework for Spatio-Temporal Video Grounding (STVG) that decouples the task into global spatio-temporal scan and local boundary focus, achieving precise boundary localization.


<details>
  <summary>Details</summary>
Motivation: Existing STVG methods struggle to balance global context modeling with precise boundary localization due to computational costs of processing long videos.

Method: ScanFocus, a coarse-to-fine framework that utilizes a unified vision-language fusion encoder, Deformable Semantic-Motion Fusion module, and Semantic-Guided Temporal Aggregator (SGTA) for efficient multimodal feature alignment and precise boundary delineation.

Result: The proposed method outperforms previous approaches on three widely used benchmarks, demonstrating its performance superiority.

Conclusion: The paper presents ScanFocus, a novel STVG framework that addresses the limitations of existing methods by decoupling global context modeling and precise boundary localization, achieving state-of-the-art performance.
...
TL;DR: The paper proposes a unified CLIP-guided label-free region scoring framework for fine-grained classification, comparing different scoring strategies and region generation methods.


<details>
  <summary>Details</summary>
Motivation: The motivation is to investigate how different local regions and CLIP-based scoring strategies affect the selection of discriminative evidence in fine-grained classification when ground-truth labels are unavailable.

Method: The framework evaluates cosine similarity-based, margin-based, and entropy-based scoring strategies using SAM-generated masks and random crops, and introduces two label-free pseudo-label variants.

Result: The results show that Soft Negative Margin scoring achieves the strongest performance, and pseudo-label scoring closely approximates true-label performance. Random-crop-based pseudo-label scoring consistently outperforms SAM-based scoring across all datasets.

Conclusion: The findings provide new insights for fine-grained classification, suggesting that random crops preserve surrounding information and provide more stable semantic context when pseudo-labels are noisy.
...
TL;DR: A novel framework for single-shot shape and pose estimation of unknown spacecraft objects from a single image.


<details>
  <summary>Details</summary>
Motivation: 6-DoF pose estimation is challenging for unknown targets, requiring shape model reconstruction.

Method: The framework reconstructs a 3D shape model, estimates pose via dense 2D-3D correspondences, using a DINOv3 vision transformer and dynamic graph convolutional neural network.

Result: Reliable pose estimates with 0.157 degrees mean pointing error on the SPE3R dataset.

Conclusion: The proposed framework demonstrates strong generalization to unseen spacecraft, achieving accurate pose estimation from a single image and reconstructed geometry.
...
TL;DR: The paper proposes Symbiosis-Inspired Knowledge Distillation (SIKD) for incremental object detection, leveraging object symbiosis to mitigate catastrophic forgetting.


<details>
  <summary>Details</summary>
Motivation: Existing methods for incremental object detection often separate feature spaces, which may overlook object symbiosis and lead to distorted shared representations and catastrophic forgetting.

Method: The proposed method, SIKD, consists of two complementary levels: Spatial Symbiosis Distillation (SpSD) and Semantic Symbiosis Distillation (SeSD), which explicitly leverage object symbiosis to preserve old class cues and stabilize semantic topology.

Result: Extensive experiments demonstrate the effectiveness and superiority of the proposed method.

Conclusion: The paper concludes that SIKD, by leveraging object symbiosis, can mitigate catastrophic forgetting and improve incremental object detection performance.
...
TL;DR: The paper proposes GeoAnchor, a framework that improves 3D spatial relationship understanding from 2D images by combining text and latent representations.


<details>
  <summary>Details</summary>
Motivation: Existing methods for 3D spatial relationship understanding from 2D images have limitations, including relying on symbolic text tokens that lack geometric fidelity and using a single latent type that can't adapt to diverse spatial tasks.

Method: The proposed GeoAnchor framework decomposes 3D spatial information into three components: position, direction, and geometry latents, which are recombined for dynamic and interpretable reasoning.

Result: GeoAnchor outperforms the state of the art on diverse and complex 3D reasoning tasks, demonstrating its effectiveness and generalization capabilities.

Conclusion: The GeoAnchor framework addresses the challenge of understanding 3D spatial relationships from 2D images by providing a more comprehensive and adaptable approach.
...
TL;DR: A novel mathematical framework for analyzing and generating complex tree-shaped 3D objects, such as botanical trees and plants, that accurately models both 3D geometry and branching structure.


<details>
  <summary>Details</summary>
Motivation: Previous works either consider only the skeletal structure of tree-like objects or approximate their 3D geometry using branch thickness, which is not accurate.

Method: The proposed framework generalizes the Square Root Normal Fields (SRNF) representation to tree-shaped 3D objects and treats them as points on a novel Riemannian tree-shape space with a novel Riemannian metric.

Result: The framework enables computing statistical summaries, synthesizing novel tree-shaped 3D objects, and significantly outperforms the state-of-the-art on real and synthetic plants and botanical trees.

Conclusion: The proposed framework provides a novel approach for analyzing and generating complex tree-shaped 3D objects, with potential applications in computer vision, graphics, and related fields.
...
TL;DR: The paper introduces 2D-RoPE-STR, a method that adapts Rotary Position Embedding (RoPE) to 2D for Scene Text Recognition (STR), improving performance on irregular text layouts.


<details>
  <summary>Details</summary>
Motivation: Scene Text Recognition (STR) is challenging due to diverse text appearances, and recent Transformer-based approaches have limitations in handling 2D spatial structure of text images.

Method: The paper proposes 2D-RoPE-STR, which adapts axial 2D-RoPE to STR through anisotropic row/column dimension allocation and extension of rotary coupling into encoder-decoder cross-attention.

Result: The method achieves gains on six standard benchmarks, particularly on irregular layouts such as curved, rotated, and perspective-distorted text.

Conclusion: The paper concludes that 2D-RoPE-STR improves STR performance by effectively leveraging 2D spatial structure of text images, especially in challenging scenarios.
...
TL;DR: The Large Processing Model (LPM) is a generative framework for photorealistic video restoration, deployed at industrial scale, addressing complex degradations in user-generated content.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to develop a scalable and cost-effective solution for video restoration in user-generated content, which is plagued by complex and diverse degradations.

Method: The authors propose the Large Processing Model (LPM), a diffusion-based generative framework that utilizes large-scale data engineering, foundation-model training, and efficient inference to restore videos.

Result: LPM has been successfully deployed in production at Kuaishou, achieving significant improvements in quality-of-experience metrics, reducing bitrate by 20%, and yielding substantial system-level benefits.

Conclusion: The Large Processing Model (LPM) is a practical, scalable, and cost-effective solution for large-scale video processing, enabling high-fidelity and temporally consistent restoration of videos.
...
TL;DR: A novel method called HIVE-3D is proposed for generating high-quality 3D scenes from a single image, outperforming previous approaches.


<details>
  <summary>Details</summary>
Motivation: Existing methods for 3D object generation from a single image are limited by their low representation resolution, making them unsuitable for 3D scene generation.

Method: HIVE-3D uses a hierarchical voxel enhancement framework, involving image segmentation, attention-based retrieval, and a voxel super-resolution model to generate high-quality 3D scenes.

Result: The proposed method achieves state-of-the-art performance, significantly outperforming previous approaches in generating high-resolution and high-quality 3D scenes.

Conclusion: The HIVE-3D method offers a significant improvement in 3D scene generation from single images, providing high-quality and high-resolution outputs.
...
TL;DR: The paper proposes a pipeline called Bring Music The Horizon for generating 360° music visualization videos that reflect the emotional progression and temporal structure of input songs.


<details>
  <summary>Details</summary>
Motivation: Existing music visualization approaches either rely heavily on lyrics or generate non-immersive videos, limiting their ability to convey emotional dynamics and provide an immersive experience.

Method: The pipeline estimates the emotional trajectory of a song, converts it into visual guidance, and uses image-to-video models to synthesize 360° videos.

Result: The pipeline generates 360° music visualization videos that reflect the emotional progression and temporal structure of input songs, demonstrated with songs from different genres.

Conclusion: The proposed pipeline provides an emotion-aware approach for music-driven 360° video generation, enhancing listeners' understanding and experience of music.
...
TL;DR: The paper introduces GPOcc and GPOcc++ for 3D occupancy prediction from visual observations, leveraging visual geometry priors and sparse Gaussian representations.


<details>
  <summary>Details</summary>
Motivation: The challenge of recovering complete volumetric representations from visual observations, particularly in occluded and unobserved regions, for embodied intelligence and autonomous driving.

Method: GPOcc transforms visual geometry priors into occupancy-aware sparse Gaussian representations, and GPOcc++ extends this to model multi-view observations and temporal sequences.

Result: Consistently strong performance on indoor and outdoor benchmarks for multi-view and temporal settings, with favorable efficiency and generalization.

Conclusion: The proposed methods effectively bridge the gap between surface-centric visual geometry priors and volumetric occupancy prediction, enabling efficient and expressive 3D scene understanding.
...
TL;DR: The paper proposes CASA-SDF, a framework for high-fidelity indoor surface reconstruction using neural implicit representations, addressing challenges in balancing planar smoothness and detail preservation.


<details>
  <summary>Details</summary>
Motivation: The primary challenge is the pronounced geometric heterogeneity of indoor scenes, making it difficult to balance planar smoothness and detail preservation in high-fidelity indoor surface reconstruction.

Method: The paper proposes a unified framework called CASA-SDF, which includes two main components: Hybrid Spatially-Adaptive Uncertainty Annealing (SAUA) for adaptive prior supervision and Curvature-Aware Locally Adaptive Density Transformation (CALADT) for adaptive density transformation.

Result: Extensive experiments on benchmark indoor datasets demonstrate that CASA-SDF improves surface completeness and detail recovery on high-frequency structures without compromising the stability of planar surfaces.

Conclusion: The proposed CASA-SDF framework effectively addresses the challenge of high-fidelity indoor surface reconstruction by adaptively adjusting supervision and representation capacity.
...
TL;DR: Proposed a novel 3D anomaly detection model, M2P-AD, which effectively detects anomalies while reducing false positives in normal regions and near object boundaries.


<details>
  <summary>Details</summary>
Motivation: Existing 3D anomaly detection methods have high performance but struggle with excessive anomaly responses in normal regions and false positives near object boundaries.

Method: Introduced Memory-to-Prototype Anomaly Detection (M2P-AD) model, which includes a Memory-to-Prototype (M2P) module, Boundary extraction (BE) module, and Boundary-aware score refinement (BSR) strategy.

Result: Achieved state-of-the-art performance on Real3D-AD, Anomaly-ShapeNet, and MulSen-AD datasets, with more accurate and stable anomaly localization.

Conclusion: The proposed approach enables more reliable 3D anomaly detection, providing a robust solution for real-world industrial environments.
...
TL;DR: The paper proposes a lightweight token reduction framework for Vision-Language Models (VLMs) to accelerate inference on resource-constrained devices, achieving a favorable accuracy-efficiency trade-off.


<details>
  <summary>Details</summary>
Motivation: Deploying VLMs on edge devices is challenging due to their substantial computational overhead, and existing token reduction approaches have limitations that undermine scalability and practical benefits.

Method: The proposed framework uses an information-theoretic perspective to estimate token importance and a transformation-induced consistency signal to ensure diverse visual coverage, enabling stride-based selection to produce a compact visual representation.

Result: Extensive experiments across multiple VLMs benchmarks demonstrate that the framework maintains competitive performance under aggressive compression, achieving a favorable accuracy-efficiency trade-off.

Conclusion: The proposed attention-free and lightweight token reduction framework is a plug-and-play module for VLMs, enabling efficient deployment on resource-constrained edge devices while preserving important and diverse tokens.
...
TL;DR: Proposed DP-BOA, a posterior-predictive decision framework for on-the-fly category discovery, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods for on-the-fly category discovery rely on heuristics, treating new category creation as a fallback. The authors aim to make new category creation an explicit, evidence-supported alternative.

Method: DP-BOA uses an online Dirichlet-process Gaussian mixture model with a Normal-Inverse-Wishart prior to compare evidence for assigning a sample to existing categories or creating a new one.

Result: DP-BOA outperforms strong baselines across standard benchmarks, particularly in novel-class discovery, while maintaining competitive known-class accuracy.

Conclusion: DP-BOA provides a more principled approach to on-the-fly category discovery by explicitly considering the evidence for creating new categories.
...
TL;DR: A new model predicts prostate cancer progression during active surveillance without needing to segment lesions, potentially reducing unnecessary biopsies.


<details>
  <summary>Details</summary>
Motivation: Current active surveillance protocols for prostate cancer rely on scheduled biopsies, many of which are unnecessary. Existing risk-stratification tools have limitations, such as requiring single time-point imaging or explicit lesion segmentation.

Method: An end-to-end temporal and multimodal model was proposed, which encodes serial scans with a pretrained 3D MRI foundation model and introduces a temporal attention gate to amplify focal imaging changes associated with progression.

Result: The model consistently outperformed competing baselines and performed comparably to radiologist assessment, maintaining high negative predictive value while achieving higher positive predictive value.

Conclusion: The proposed model has the potential to safely reduce unnecessary biopsies during active surveillance of prostate cancer.
...
TL;DR: A new dataset for on-the-move face recognition in border control scenarios is introduced, highlighting the challenges of vehicular acquisition and the need for dedicated methods.


<details>
  <summary>Details</summary>
Motivation: The increasing pressure on border control infrastructures due to cross-border mobility growth motivates the development of on-the-move biometric authentication, specifically face recognition.

Method: A dataset comprising NIR vehicle-crossing videos paired with smartphone-based pre-enrollment data is introduced, and baseline evaluations are performed using state-of-the-art models.

Result: State-of-the-art models show clear performance limitations under realistic conditions, highlighting the need for dedicated methods to advance the field.

Conclusion: The introduced dataset and evaluations demonstrate the challenges of on-the-move face recognition in border control scenarios and the need for further research and development.
...
TL;DR: A new framework, VGIF-Score, is proposed for evaluating video generation models' ability to follow long, compositional instructions, providing a reliable, interpretable, and diagnostic evaluation.


<details>
  <summary>Details</summary>
Motivation: Existing evaluation protocols for video generation models are insufficient, relying on short and shallow prompts, costly human evaluation, or handcrafted vision pipelines, and providing little diagnostic insight.

Method: VGIF-Score consists of two components: an objective completion branch that uses a Spatio-Temporal Directed Acyclic Graph (ST-DAG) and dependency-aware QA, and a subjective satisfaction branch that uses instruction-conditioned AutoRubric to assess video quality.

Result: Experiments on 14 proprietary and open-source VGMs across more than 3K generated videos show that VGIF-Score provides reliable, interpretable, and diagnostically useful evaluation of video generation instruction following.

Conclusion: VGIF-Score is a highly automated and interpretable framework for evaluating instruction following in video generation, addressing the gap in existing evaluation protocols.
...
TL;DR: A new framework called ThinkBLOX is proposed for generating and editing 3D indoor scenes using a progressive reasoning approach, improving physical plausibility, semantic alignment, and interactive editability.


<details>
  <summary>Details</summary>
Motivation: Traditional graphics methods for synthesizing 3D indoor scenes have limitations, and recent vision-language model-based generators often require global re-optimization or complete reconstruction during interactive editing, leading to poorly organized arrangements.

Method: ThinkBLOX, a VLM-based progressive reasoning framework, iteratively designs and refines 3D scenes using a state-conditioned, step-by-step reasoning-and-action process, aided by a dataset of annotated scene context and structured layouts.

Result: ThinkBLOX significantly outperforms recent baselines in physical plausibility, semantic alignment, and interactive editability, supporting diverse applications including global and local generation and rearrangement of 3D scenes.

Conclusion: The proposed ThinkBLOX framework offers a more effective approach to generating and editing 3D indoor scenes, addressing challenges in physical plausibility, semantic alignment, and interactive editability.
...
TL;DR: A new method called Nexus is proposed for generating high-quality triangle meshes using a diffusion-based approach, decoupled into vertex and topology generation.


<details>
  <summary>Details</summary>
Motivation: Current mainstream methods for mesh generation rely on serialization and autoregressive processes, which struggle with effective inference and are sensitive to error accumulation.

Method: The proposed method, Nexus, uses a diffusion model to generate vertices in a coarse-to-fine manner and proposes Spacetime Interval to encode topology into continuous per-vertex embeddings.

Result: The method outperforms state-of-the-art baselines on Objaverse and Toys4K datasets and in-the-wild images, with a blind user study showing a strong perceptual preference for the results.

Conclusion: The proposed method, Nexus, effectively circumvents the limitations of sequential mesh modeling and generates high-quality triangle meshes.
...
TL;DR: The paper proposes Grounded Context Preference Optimization (Groc-PO) to address untruthfulness issues in Multimodal Large Language Models (MLLMs) by introducing explicit preference supervision over multiple grounded stages.


<details>
  <summary>Details</summary>
Motivation: MLLMs suffer from untruthfulness issues such as visual hallucinations and unfaithful reasoning. Current alignment methods like Direct Preference Optimization (DPO) have limitations in addressing these issues, particularly in multimodal reasoning errors that propagate across stages.

Method: The proposed Groc-PO framework optimizes preference at multiple grounded stages, including Object Grounding, Contextual Grounding, and Grounded Reasoning. A new dataset, Grounded Context Preference Dataset (GCPD), is constructed to support this approach.

Result: Experiments show that Groc-PO achieves improved performance in hallucination mitigation, faithful reasoning, and overall reliability compared to standard DPO and other strong baselines.

Conclusion: The paper concludes that introducing more explicit grounded supervision for trustworthy multimodal reasoning can strengthen context-dependent reasoning and mitigate cross-stage error propagation, supporting the value of Groc-PO in improving MLLMs' faithfulness and practical utility.
...
TL;DR: Proposed GHR-VLM, a visual grounded hybrid reasoning framework for zero-shot transit-bus video analytics, leveraging edge-cloud design to improve efficiency and accuracy.


<details>
  <summary>Details</summary>
Motivation: Current methods for transit video understanding are limited by requiring task-specific annotations or being unreliable and costly when applied to long onboard videos.

Method: GHR-VLM uses a lightweight edge-based monitor to track door status and segment passenger clips, then a backend VLM identifies boarding passengers and classifies payment behavior through a two-stage coarse-to-fine refinement of spatiotemporal evidence.

Result: Evaluation on 486 minutes of real-world bus surveillance video demonstrates the potential of grounded edge-cloud reasoning for passenger-level payment analytics.

Conclusion: GHR-VLM reduces cloud inference, avoids payment-specific training data, and supplies localized evidence, but highlights challenges posed by degraded video conditions.
...
TL;DR: A framework for transforming 3D assets into simulation-ready assets with unified physical semantics for embodied AI and robotic simulation.


<details>
  <summary>Details</summary>
Motivation: Most existing 3D assets lack unified physical semantics required for realistic interaction, and current approaches have limitations in robustness across heterogeneous 3D assets.

Method: UniPhys, a scalable framework, and UniPhysGen, a unified physical grounding model that jointly reasons over articulation semantics and intrinsic physical properties.

Result: State-of-the-art performance across articulation grounding and intrinsic physical property estimation tasks, with assets deployable in robotic simulation environments.

Conclusion: The proposed framework and model enable the creation of physically grounded 3D assets for embodied AI and robotic simulation, with potential applications in realistic physical interaction.
...
TL;DR: OvisOCR2, an end-to-end document parsing model, achieves state-of-the-art results on multiple benchmarks, including OmniDocBench and PureDocBench.


<details>
  <summary>Details</summary>
Motivation: The motivation behind OvisOCR2 is to develop an end-to-end document parsing model that can handle various document elements such as text, formulas, tables, and visual regions, and generate a Markdown representation in natural reading order.

Method: The method used to develop OvisOCR2 involves a data engine that combines filtered real-document annotations with synthetic pages, and a training recipe that includes supervised fine-tuning, reinforcement learning, on-policy distillation, and model fusion.

Result: OvisOCR2 achieves a state-of-the-art overall score of 96.58 on OmniDocBench v1.6, and the highest Avg3 score of 75.06 on PureDocBench. It also obtains the best overall performance on an in-house benchmark.

Conclusion: The conclusion of this paper is that OvisOCR2, an end-to-end document parsing model, demonstrates superior performance and robustness on multiple benchmarks, highlighting its potential for document parsing tasks.
...
TL;DR: A new dataset called Human4K is presented, which contains high-resolution images and accurate annotations for 3D human reconstruction, leading to improved performance in challenging scenarios.


<details>
  <summary>Details</summary>
Motivation: Current 3D human reconstruction models struggle with real-world scenarios due to lack of high-quality datasets with diverse whole-body motions, high-resolution images, and precise annotations.

Method: The authors create a large-scale dataset, Human4K, with 4K multi-view images, mocap-accurate SMPL-X annotations, and a Motion-Retargeting and Refinement Module (MRRM) to ensure precise alignment.

Result: Training with Human4K improves whole-body reconstruction on standard benchmarks, with significant gains for hands, feet, and depth-ambiguous limb configurations.

Conclusion: The Human4K dataset addresses the gap in existing datasets and leads to improved performance in 3D human reconstruction, particularly in challenging scenarios.
...
TL;DR: The paper evaluates how well vision models represent colors like humans do, using a fuzzy perceptual model and finding that Masked Autoencoders (MAE) achieve the strongest alignment with human color perception.


<details>
  <summary>Details</summary>
Motivation: Existing evaluations of color representations in vision models are limited as they compare them with geometric spaces or discrete color labels, not capturing the nuanced way humans organize colors.

Method: The paper uses a fuzzy perceptual model with 86 graded categories fitted to human survey data to evaluate color grounding in vision models, measuring category boundaries, compactness, and graded alignment.

Result: The results show that while category-level performance is similar across models, graded alignment varies substantially, with Masked Autoencoders (MAE) achieving the strongest beyond-geometry alignment.

Conclusion: The study concludes that human-like color grounding has multiple aspects and cannot be reduced to a single score, highlighting the importance of nuanced evaluation methods.
...
TL;DR: The paper presents a model to predict the observability of Earth's surface in multispectral imagery, forecasting whether the next image will be usable and when a usable view is likely to return.


<details>
  <summary>Details</summary>
Motivation: The bottleneck in Earth Observation processing chains is not the arrival of new imagery, but whether the surface is visible when the image arrives.

Method: The authors adapt LeWorldModel, a joint-embedding predictive architecture world model, to cloud-aware Earth Observation sequences, using multispectral imagery and exogenous weather drivers.

Result: The model outperforms persistence and a LightGBM baseline in predicting next-step usability and exact first-usable-horizon prediction, with balanced accuracy ranging from 0.769 to 0.887.

Conclusion: The proposed model, LeWorldModel, effectively predicts the observability of Earth's surface in multispectral imagery, offering improved performance over baselines in various evaluation settings.
...
TL;DR: REAL is a framework for open-world mobile manipulation that enables active exploration, visual grounding, and interactive intent disambiguation. It achieves a 56.9% success rate on interactive tasks and 78.3% on physical robot trials.


<details>
  <summary>Details</summary>
Motivation: Existing frameworks for embodied agents rely on privileged simulator states or complete instructions, which bypass realistic deployment challenges. The authors aim to bridge this gap with a more realistic and interactive approach.

Method: The REAL framework establishes sim-to-real-consistent environment APIs, integrates a simulated user for human-in-the-loop interaction, and uses a hierarchical training pipeline with diverse task compositions, supervised fine-tuning, and online reinforcement learning.

Result: The trained agent outperforms leading commercial closed-source VLMs on interactive tasks with a 56.9% success rate. On a physical dual-arm mobile robot, it achieves a 78.3% end-to-end success rate over 60 real-world episodes.

Conclusion: The REAL framework successfully bridges the reality gap for long-horizon mobile manipulation, demonstrating robust zero-shot transferability to unseen household scenarios.
...
TL;DR: A novel text-driven 3D human garment editor, T3HG-Editor, leveraging SMPL-X priors for high-fidelity and garment-consistent results.


<details>
  <summary>Details</summary>
Motivation: Existing 3D Gaussian editing methods struggle with 3D human garment editing due to low-fidelity outcomes and garment inconsistencies.

Method: T3HG-Editor uses SMPL-X priors and consists of three stages: obtainment of editable Gaussians, garment-consistent editing, and Gaussian updating with overflow pruning.

Result: T3HG-Editor outperforms state-of-the-art methods in editing quality and garment consistency across multiple subjects and diverse garment types.

Conclusion: T3HG-Editor presents a promising solution for text-driven 3D human garment editing, addressing the limitations of existing methods.
...
TL;DR: A new framework called FreeLit is proposed for controllable indoor relighting without needing paired supervision, achieving stable and physically consistent results.


<details>
  <summary>Details</summary>
Motivation: The challenge of image-based indoor scene relighting due to complex geometry and local illumination, and the limitations of existing data-driven and inverse-rendering methods.

Method: FreeLit uses a physics-guided illumination prior from intrinsic scene properties to generate a structured lightmap and pseudo-relit image for diffusion-based synthesis, along with a relighting-guided intrinsic stabilization strategy.

Result: FreeLit achieves stable, physically consistent, and controllable relighting with improved robustness in low-light indoor scenes without paired supervision.

Conclusion: FreeLit offers a novel approach to controllable indoor relighting, addressing the limitations of existing methods and demonstrating improved performance in challenging conditions.
...
TL;DR: The paper proposes SFF-CLIP, a fine-tuning method for CLIP that enhances fine-grained feature representation without requiring additional region annotations, maintaining global visual-semantic consistency.


<details>
  <summary>Details</summary>
Motivation: CLIP's limitations in fine-grained dense feature representation due to its pre-training focus on whole image-text matching, and the need to overcome limitations of existing fine-tuning schemes.

Method: SFF-CLIP uses a run-time region-phrase alignment scheme to align concept phrases from input sentences with region-based features using text-specific heat maps.

Result: Significant performance improvements on fine-grained dense feature representation and maintained performance on image-level tasks.

Conclusion: SFF-CLIP effectively enhances CLIP's fine-grained representation ability while preserving its original global visual representation capability.
...
TL;DR: The POLY-SIM 2026 challenge aims to improve multimodal speaker identification systems, particularly in scenarios with incomplete or missing audio-visual data and multilingual speakers.


<details>
  <summary>Details</summary>
Motivation: Current multimodal speaker identification systems are limited by their assumptions of complete and homogeneous audio-visual data and monolingual speakers, which do not hold in real-world applications.

Method: The POLY-SIM 2026 challenge proposes a standardized setup for comparing solutions to address the challenges of incomplete or missing data and linguistic variability across languages.

Result: The challenge seeks to develop more robust and generalizable multimodal speaker identification systems.

Conclusion: The POLY-SIM 2026 challenge aims to advance multimodal speaker identification systems by addressing real-world challenges and providing a standardized evaluation framework.
...
TL;DR: WAVE-Stereo, a novel stereo matching method, integrates correlation volumes and feature warping to improve matching accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing iterative stereo matching methods model correspondence representation and local residual refinement separately, which limits their performance.

Method: WAVE-Stereo uses a GeoWarp Correspondence Encoder (GWCE) to encode matching search, residual alignment, and disparity prior in parallel, and Periodic Global Context Propagation (PGCP) to mitigate matching degradation in textureless regions.

Result: WAVE-Stereo achieves competitive zero-shot generalization accuracy on five real-world benchmarks, with 3.18% D1-all on KITTI 2015, 4.42% Bad-2.0 on Booster, and 66ms real-time inference.

Conclusion: WAVE-Stereo strikes a favorable balance between accuracy and efficiency, demonstrating its potential for real-world applications.
...
TL;DR: The paper introduces VSI-Super-Wild, a large-scale benchmark for evaluating spatial supersensing in diverse real-world scenes over long temporal horizons. It exposes a disconnect between advances in static image understanding and coherent world-state tracking over time, and diagnoses four failure modes in current models.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the gap in current spatial supersensing benchmarks, which are limited to synthetic long videos and mostly limited to household scenes, and to evaluate multimodal models' ability to form a unified spatial world model.

Method: The authors introduce a new benchmark, VSI-Super-Wild, which contains 6,980 human-verified question-answer pairs derived from 442 real-world videos spanning 8 scene categories. They evaluate current models on this benchmark and diagnose four failure modes.

Result: The results show that despite advances in static image understanding, models consistently fail at tasks that require coherent world-state tracking over time. The authors characterize how performance degrades with world-state complexity and temporal horizon.

Conclusion: The paper concludes that current models lack mechanisms to bind objects, agents, and environments into a unified spatial world model, and that this gap defines the path forward for spatial supersensing.
...
TL;DR: This paper presents a method for radiative Gaussian splatting in sparse-view CT reconstruction that provides uncertainty estimates, and demonstrates its effectiveness in a systematic calibration study.


<details>
  <summary>Details</summary>
Motivation: Existing methods for radiative Gaussian splatting in sparse-view CT reconstruction lack uncertainty estimates, making it difficult to determine where the reconstruction can be trusted.

Method: The authors exploit a property of transmissive X-ray imaging to equip radiative Gaussians with a variational density posterior, allowing for the propagation of predictive variance in closed form.

Result: The proposed method achieves good performance in a systematic calibration study, ranking true reconstruction error on 14 of 15 scenes, and meeting magnitude-calibration targets after a single temperature scaling.

Conclusion: The proposed method provides a more accurate and reliable approach to sparse-view CT reconstruction, and has the potential to inform acquisition policies and dose-adaptive stopping rules.
...
TL;DR: A new method called Dual-stage Native Attribution (DNA) is proposed for attributing images to their source models, achieving high accuracy without degrading image quality.


<details>
  <summary>Details</summary>
Motivation: The rapid evolution of image generation has made it important to identify the source model of suspect images for digital forensics, but existing methods have limitations.

Method: DNA uses a coarse-to-fine framework with Autoencoder Double-Reconstruction (AEDR) for family-level screening and Native Prediction Consistency (NPC) for model-level attribution.

Result: DNA achieves 89.11% end-to-end attribution accuracy, outperforming the strongest baseline by 33.81% on a benchmark of 30,000 images from 24 models across six families.

Conclusion: The proposed DNA framework offers a effective solution for source-model attribution of images generated by various models, with high accuracy and flexibility.
...
TL;DR: A compact convolutional network achieved 99.73% accuracy in 46-class Devanagari recognition, outperforming larger models and demonstrating robustness to corruption.


<details>
  <summary>Details</summary>
Motivation: To develop a compact and efficient model for Devanagari recognition that matches or surpasses the performance of larger models.

Method: A compact convolutional network with 1.11 M parameters was built and tested on the DHCD Devanagari recognition task.

Result: The compact model achieved 99.73% accuracy, which is the highest reported accuracy on this task, and outperformed larger models in terms of corruption robustness.

Conclusion: The compact model has reached a saturation point where no configuration can achieve a statistically significant win, and it matches or surpasses the performance of larger models.
...
TL;DR: The study evaluates the faithfulness of post-hoc attribution methods for deep video models estimating left-ventricular ejection fraction (EF) from echocardiography, finding that while models are anatomically faithful, they are temporally blind and ignore clinically decisive frames.


<details>
  <summary>Details</summary>
Motivation: To investigate whether post-hoc attribution explanations for deep video models estimating EF are faithful both spatially and temporally, specifically localizing the left ventricle and decisive frames.

Method: Fine-tuning two EF regressors (VideoMAE transformer and R(2+1)D CNN) on EchoNet-Dynamic and auditing them with architecture-matched attribution methods along three axes: intersection-over-relevance (IoR) against LV masks, deletion AUC, and temporal localization index on ES/ED frames.

Result: Both models show anatomical faithfulness (IoR 2.91x and 1.98x above chance) but temporal blindness (temporal localization indistinguishable from chance), with occlusion showing models do not preferentially rely on ES/ED frames.

Conclusion: Spatial faithfulness does not imply temporal faithfulness; attribution can certify anatomical grounding while masking that a model ignores clinically decisive frames, highlighting a need for temporally-aware training and evaluation.
...
TL;DR: The paper introduces EgoProceVQA, a task for evaluating egocentric procedural understanding in videos, and proposes EgoProceAgent, a framework that improves performance on this task.


<details>
  <summary>Details</summary>
Motivation: Existing evaluations for egocentric video understanding do not adequately address procedural understanding, which is crucial for building AI assistants deployable on wearable devices.

Method: The authors develop EgoProceGen, a data generation platform, and create a benchmark with 3,600 questions, four common procedural scenarios, and 31 everyday procedural tasks. They also propose EgoProceAgent, a self-skill-exploration agentic framework.

Result: Evaluations on EgoProceVQA show that existing MLLMs and agents have substantial room for improvement in procedural understanding. EgoProceAgent achieves state-of-the-art performance among open-source models on multiple tasks.

Conclusion: The paper establishes a unified foundation for EgoProceVQA, including a benchmark, generation platform, and agentic framework, which can facilitate future research in egocentric procedural understanding.
...
TL;DR: A deep learning framework is proposed to classify pancreatic ductal adenocarcinoma (PDAC) resectability into three categories using CT imaging and clinical information.


<details>
  <summary>Details</summary>
Motivation: The accurate determination of PDAC resectability is crucial but shows substantial variability among experts, and current methods are limited.

Method: A fully automated multimodal deep learning framework that combines 3D contrast-enhanced CT imaging with structured clinical information, using a Swin-UNETR backbone and a dynamic multitask objective.

Result: The proposed framework can classify patients into three NCCN resectability categories (upfront resectable, borderline resectable, locally advanced) with improved accuracy.

Conclusion: The proposed framework has the potential to improve the accuracy of PDAC resectability classification, reducing variability among experts and potentially leading to better patient outcomes.
...
TL;DR: A new framework called AgentHOI is proposed for human-object interaction detection (HOID) that leverages the multimodal reasoning capabilities of foundation models without requiring specific HOID training data.


<details>
  <summary>Details</summary>
Motivation: Traditional HOID approaches are limited by their reliance on predefined interaction categories and dataset-specific supervision, hindering their ability to generalize to open-world and compositional scenarios.

Method: AgentHOI uses a training-free, agentic framework that orchestrates vision foundation modules for open-ended semantic reasoning and spatial grounding, incorporating Context-aware Multi-round Reasoning and Multifaceted Interaction Localization.

Result: AgentHOI achieves superior performance over state-of-the-art supervised and weakly supervised methods in real-world settings without using HOID data for training.

Conclusion: The proposed AgentHOI framework effectively transfers generalist multimodal reasoning capabilities to HOI detection, demonstrating improved performance in open-world and compositional scenarios.
...
TL;DR: A new video deraining framework called RainDancer is proposed, which combines RGB and event camera data to improve deraining performance in dynamic scenes.


<details>
  <summary>Details</summary>
Motivation: Existing video deraining methods relying on RGB sequences have limitations in dynamic rainy scenes. Event cameras provide complementary motion-sensitive cues, but direct fusion with RGB data can introduce interference.

Method: RainDancer uses a decompose-before-interact paradigm, separating rain and background components in RGB and event data before fusing them. It includes a rain-oriented spiking neural network for event data and event-domain supervision for regularization.

Result: Experiments show that RainDancer achieves superior quantitative performance, visual quality, and downstream perception robustness on synthetic and real RGB-Event video deraining datasets.

Conclusion: The proposed RainDancer framework effectively addresses the challenges of video deraining in dynamic scenes by leveraging the strengths of both RGB and event camera data.
...
TL;DR: The authors propose a structure-aware framework for music-driven dance generation, modeling choreography as a sequence of atomic movements, which improves structural coherence, rhythmic alignment, and perceptual naturalness.


<details>
  <summary>Details</summary>
Motivation: Current neural approaches for music-driven dance generation neglect the compositional nature of motion, leading to structurally incoherent and difficult-to-control generated dances.

Method: The authors introduce a two-stage generation framework: (1) atomic movement planning, where the model predicts the type, duration, and timing of atomic movements conditioned on the input music, and (2) completion, where a transition-aware generator synthesizes smooth and stylistically coherent motion conditioned on the planned structure.

Result: The proposed method produces dances with significantly improved structural coherence, rhythmic alignment, and perceptual naturalness compared to existing baselines, while providing enhanced interpretability and controllable editing.

Conclusion: The authors' structure-aware framework effectively addresses the limitations of current music-driven dance generation approaches, offering a more coherent, controllable, and interpretable solution.
...
TL;DR: The paper proposes AspectCLIP, a framework that improves contrastive language-image pretraining by respecting the one-to-many structure between images and text, resulting in a more structured representation space.


<details>
  <summary>Details</summary>
Motivation: Existing methods for contrastive language-image pretraining overlook the inherent information asymmetry between images and text, where captions describe only one specific aspect of an image, leading to semantic distortion in the representation space.

Method: The proposed AspectCLIP framework partitions training samples into attribute clusters based on textual similarity, applies full cyclic consistency within each cluster, and restricts cross-cluster regularization to prototype-level comparisons.
...
Conclusion: The AspectCLIP framework effectively addresses the challenge of information asymmetry between images and text, leading to improved contrastive language-image pretraining and a more structured representation space.
...
TL;DR: The paper proposes a multi-expert system for Historical Manchu OCR, which uses a lightweight page-level image classifier to dispatch pages by visual style, achieving high accuracy with limited labeled data.


<details>
  <summary>Details</summary>
Motivation: Historical Manchu OCR faces challenges due to various visually distinct writing styles and limited labeled data.

Method: A multi-expert system that reuses checkpoints from an iterative fine-tuning process as domain specialists and uses a lightweight page-level image classifier to dispatch pages by visual style.

Result: The system achieves 0.30% CER on regular script, 1.57% on memorials, and 4.83% on running script, with 99.3% page-level domain accuracy.

Conclusion: The proposed system effectively accommodates various writing styles in Historical Manchu OCR with limited labeled data, achieving high accuracy and matching the domain-label oracle.
...
TL;DR: A new method for 3D rendering is introduced, achieving a fivefold speedup over 3D Gaussian Splatting (3DGS) while preserving visual fidelity, enabling real-time 4K rendering at 60 FPS.


<details>
  <summary>Details</summary>
Motivation: The high computational cost of 3D Gaussian Splatting (3DGS) during fragment rendering, despite its ability to capture fine color details.

Method: A decoupled radiance representation using 2D surfels for low-frequency geometry and view-dependent appearance features, and a view-independent spatial hash grid for high-frequency textures, with sparsity-enhancing optimizations.

Result: A fivefold speedup over 3DGS, enabling real-time 4K rendering at 60 FPS on consumer hardware, while preserving state-of-the-art visual fidelity.

Conclusion: The proposed method achieves significantly faster and sparser reconstructions than prior work, making it suitable for real-time 4K rendering on consumer hardware.
...
TL;DR: This paper introduces a novel approach to improve 3D medical image understanding using Multimodal Large Language Models (MLLMs) by creating a large-scale structured reasoning dataset and a synthesized reasoning framework, achieving significant performance improvements over 2D baselines.


<details>
  <summary>Details</summary>
Motivation: The extension of MLLMs to 3D volumetric imaging is hindered by high annotation costs and dataset opacity, and current data formats fail to capture explicit clinical reasoning.

Method: The authors introduce a slice-wise data synthesis paradigm to construct a large-scale structured reasoning dataset and a synthesized reasoning framework that enforces essential clinical principles, then instruction-tune a 2D-pretrained MLLM baseline using the synthesized data.

Result: Comprehensive evaluations across multiple 3D medical benchmarks demonstrate significant performance improvements over the 2D baseline, with the resulting model exhibiting robust spatial reasoning capabilities and rivaling resource-intensive native 3D architectures.

Conclusion: The proposed data-centric strategy unlocks deep volumetric understanding and highly interpretable clinical logic without requiring computationally expensive 3D-specific pre-training.
...
TL;DR: The authors propose OCP-CT, a framework for CT vision-language pretraining that achieves finer alignment between image and text by conditioning on organs and radiological patterns, outperforming prior methods in zero-shot abnormality diagnosis.


<details>
  <summary>Details</summary>
Motivation: Existing methods for CT vision-language pretraining have limitations, including obscuring heterogeneous organ evidence and coarse organ-level alignment. The authors aim to address these challenges by introducing a finer alignment unit: the organ-conditioned radiological pattern.

Method: The authors propose OCP-CT, an organ-conditioned pattern-token alignment framework that preserves a stable global CT-report contrastive branch and introduces an organ pattern interface using sparse Mixture-of-Experts (MoE) and learnable slots to query routed tokens into continuous pattern tokens.

Result: OCP-CT achieves average AUROCs of 84.5% and 69.9% for zero-shot abnormality diagnosis on the CT-RATE and RAD-ChestCT benchmarks, respectively, with absolute AUROC gains of 6.7 and 0.8 percentage points compared to prior results.

Conclusion: The proposed OCP-CT framework demonstrates improved performance in CT vision-language pretraining, particularly in zero-shot abnormality diagnosis, by leveraging organ-conditioned radiological patterns for finer alignment between image and text.
...
TL;DR: The 2nd International StepUP Competition Series evaluated pressure-based footstep biometrics, focusing on generalization, robustness, and fusion of paired footsteps, with a best equal error rate of 8.00%.


<details>
  <summary>Details</summary>
Motivation: To advance research in pressure-based footstep biometrics through a standardized evaluation framework and address challenges in generalization, robustness, and fusion of paired footsteps.

Method: Using the large-scale StepUP-P150 dataset and a test set, the competition addressed three key challenges: generalization to unseen users, robustness to domain shift, and effective fusion of paired footsteps.

Result: The best equal error rate achieved was 8.00% by the ArogyaPandit Research Team using a spatiotemporal CNN combined with an ensemble-based scoring strategy.

Conclusion: The competition results highlight the value of harnessing temporal patterns and incorporating inference-time normalization and calibration strategies, but also reveal challenges in recognizing users in unseen personal footwear.
...
TL;DR: The paper proposes TCA-Net, a network for low-light image enhancement that improves the fusion of intensity and chromaticity streams, leading to better color fidelity and efficiency.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the issue of unreliable fusion of intensity and chromaticity streams in existing low-light image enhancement methods, which can lead to color entanglement and reduced image quality.

Method: The method proposed is TCA-Net, which uses Thresholded Cross-Attention to adaptively retain high-confidence cross-stream interactions and suppress unreliable ones. It also includes complementary designs such as Phase-guided Fourier Interaction Module, Decoupled Dual-Stream Guidance Module, and Scale-Aware Consistency Regularization.

Result: The results show that TCA-Net delivers competitive restoration accuracy, improved color fidelity, and a compact parameter size on several benchmark datasets.

Conclusion: The conclusion is that TCA-Net provides a reliable and efficient solution for low-light image enhancement by addressing the issue of intensity-chromaticity fusion, leading to improved image quality and color fidelity.
...
TL;DR: A new framework called Cyclone is proposed for weather editing in autonomous driving systems using latent diffusion, which generates realistic weather conditions without paired data and improves perception tasks.


<details>
  <summary>Details</summary>
Motivation: Autonomous driving systems face challenges in diverse weather conditions, and current approaches rely on synthetic data or weather-removal techniques, which have limitations.

Method: Cyclone, a unified framework for weather editing based on latent diffusion, with cycle-consistent constraints and knowledge from image-text models.

Result: Cyclone produces more realistic and structure-preserving outputs than existing baselines and improves several downstream driving perception tasks.

Conclusion: Cyclone offers a promising solution for weather editing in autonomous driving systems, enabling the generation of diverse weather conditions without paired data.
...
TL;DR: The paper proposes SIVA-RL, a framework that improves multimodal reasoning in vision-language models by aligning visual interventions with observed effects, achieving significant gains over existing RL baselines.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the issue that answer-level correctness does not guarantee that a vision-language model grounds its predictions in visual evidence, and existing visual-intervention methods have limitations.

Method: The method proposed is SIVA-RL, which uses a Sensitivity-Invariance Visual Alignment framework, constructing localized interventions through token-aligned, distance-constrained within-image PatchSwap, and a frozen audit policy to score clean-intervention pairs.

Result: The results show that SIVA-RL improves 3B and 7B models over matched RL baselines in every setting across nine multimodal reasoning benchmarks, with an 8.79 percentage-point gain on vision-dependent reasoning and up to 14.9% relative overall improvement.

Conclusion: The conclusion is that SIVA-RL is an effective framework for improving multimodal reasoning in vision-language models by decoupling intervention construction from supervision assignment.
...
TL;DR: The study proposes a method for analyzing PET/MRI data to identify abnormal regions in arrhythmogenic left ventricular cardiomyopathy patients, enabling automated generation of health reports.


<details>
  <summary>Details</summary>
Motivation: The difficulty in diagnosing arrhythmogenic left ventricular cardiomyopathy due to the lack of gold standard criteria, and the potential of PET/MR imaging with multiparametric quantitative analysis to facilitate identification of disease profiles.

Method: Two-step clustering was applied to T1 and T2 maps, LGE, and 18F-FDG-PET images of patients, with inter-patient data linkage and regional analysis.

Result: The approach accurately identified most cardiac physicians' observations (BA = 0.76 $pm$ 0.04 on patients, and BA $ge$ 0.8 on phantoms), and facilitated identification of varying degrees of fibrosis or inflammation on images.

Conclusion: The study enables a more systematic handling of multimodal PET/MRI data to characterise myocardial heterogeneity in arrhythmogenic left ventricular cardiomyopathy patients.
...
TL;DR: The paper proposes a lightweight and interpretable framework called Peak-End-Net for video aesthetic assessment, inspired by the peak-end rule from psychology, and achieves state-of-the-art performance on two benchmarks.


<details>
  <summary>Details</summary>
Motivation: The progress of video aesthetic assessment (VAA) is hindered by the scarcity of large-scale benchmarks and the intrinsic subjectivity of aesthetic judgment, which is shaped by human perception.

Method: The proposed Peak-End-Net framework uses a pretrained image aesthetic assessment head to produce frame-wise aesthetic priors, an aesthetic rhythm encoder to model temporal progression, and a dynamic gated fusion mechanism to improve robustness under distribution shift, all built on a frozen vision transformer (ViT).

Result: The approach achieves state-of-the-art performance on two existing VAA benchmarks, including in-domain evaluation on VADB and cross-domain testing on DIVIDE-3K.

Conclusion: The paper affirms the value of psychologically grounded modeling for VAA and proposes a scalable and parameter-efficient solution.
...
TL;DR: The study evaluates the consistency of methane plume products derived from imaging spectrometers, finding that different plume masks can produce similar integrated mass enhancement, plume length, and emission rate, highlighting ambiguity in weak and low-overlap plumes.


<details>
  <summary>Details</summary>
Motivation: To assess the reliability of source-resolved methane plume products by examining the relationship between plume masks and derived quantities such as integrated mass enhancement, plume length, emission rate, and uncertainty.

Method: The study uses 63 EMIT-derived Carbon Mapper plume records and applies a genetic-algorithm ensemble (PlumeQuant) to evaluate four mask representations: the distributed reference mask, a Carbon Mapper-informed analogue (CM-like), the GA ensemble, and optional expert edits.

Result: The results show that different yet plausible masks can reproduce the same integrated mass enhancement, plume length, and emission rate, with the high-confidence core selected by nearly all target-consistent masks covering a median of 13% of the plausible footprint envelope. The CM-like mask achieved a median intersection-over-union of 0.843 against the reference masks and matched the published uncertainty scale.

Conclusion: The study concludes that while the evaluated products show consistency in certain aspects, there is ambiguity in weak and low-overlap plumes, suggesting the need for expert review and further improvement in methane plume product reliability.
...
TL;DR: The paper presents CF-Net, a deep multimodal network for detecting ambivalence and hesitancy in unconstrained video, achieving high performance on the BAH dataset.


<details>
  <summary>Details</summary>
Motivation: Detecting ambivalence and hesitancy in unconstrained video is challenging due to the subtle and ambiguous nature of the target signal.

Method: CF-Net uses a multimodal approach, encoding visual, audio, and transcript streams with frozen backbones, and fuses them via a ConflictFusion module to compute cross-modal incongruence.

Result: CF-Net achieves a Macro F1 of 0.7155 on the BAH validation set and 0.7364 (AP = 0.7492) on the private challenge test set.

Conclusion: The proposed CF-Net demonstrates effective performance in detecting ambivalence and hesitancy in unconstrained video, leveraging multimodal fusion and conflict computation.
...
TL;DR: A new vision model called VisionScreen is proposed, extending the Screening mechanism from language modeling to visual recognition, allowing for selective aggregation of relevant patches.


<details>
  <summary>Details</summary>
Motivation: The self-attention mechanism in Vision Transformers (ViT) has limitations in evaluating patch relevance independently, leading to unnecessary information in feature aggregation, especially with background or redundant patches.

Method: VisionScreen extends the Screening mechanism to the two-dimensional spatial domain, treating image patches as tokens on a grid, and selectively aggregates content-wise and spatially relevant patches.

Result: Experiments on image classification benchmarks show that VisionScreen outperforms conventional ViT, suggesting the effectiveness of Screening in visual recognition.

Conclusion: The proposed VisionScreen offers an alternative to relative feature aggregation based on softmax attention, demonstrating the potential of Screening in visual recognition tasks.
...
TL;DR: The paper proposes a task-adaptive feature fusion approach for multi-task affective behavior analysis in the ABAW11 challenge, achieving better results by selecting task-specific fusion and prediction strategies.


<details>
  <summary>Details</summary>
Motivation: The ABAW11 challenge requires a unified system to predict valence-arousal, categorical expressions, and facial action units, but these tasks benefit from different visual features and processing strategies.

Method: The paper adapts pretrained visual backbones (DINOv2 ViT-L and DINOv3 ConvNeXt-base) on an external facial image set, freezes them to extract frame-level features, and systematically compares various prediction heads, fusion mechanisms, and calibration procedures.

Result: The selected system achieves an EXPR macro-F1 of 0.4222, an AU macro-F1 of 0.5402, and a mean VA CCC of 0.6717, resulting in an overall validation score of 1.6341 on the ABAW11 validation set.

Conclusion: The results suggest that task-adaptive fusion of frozen visual features is a simple and effective strategy for ABAW-style multi-task affective behavior analysis.
...
TL;DR: M^4World is a generative model for autonomous driving simulation that synthesizes surround-view video streams and LiDAR scans with interactive object manipulation and stable long-term streaming.


<details>
  <summary>Details</summary>
Motivation: Existing methods for driving-world generation are limited in object-level controllability and long-horizon stability, which are crucial for scalable autonomous-driving simulation.

Method: M^4World uses a multi-view and multimodal generative approach with a flexible conditioning interface for object manipulation and a multi-stage training framework for stable long-term streaming.

Result: M^4World demonstrates high generation quality, precise controllability, and stable minute-long streaming, with potential for controllable and scalable driving simulation.

Conclusion: M^4World shows promising results for autonomous driving simulation, enabling interactive object manipulation and stable long-term streaming, with applications in long-tail augmentation and scene editing.
...
TL;DR: The paper analyzes interactive game world modeling through four dimensions and presents a data engine for creating a scalable game world.


<details>
  <summary>Details</summary>
Motivation: To build interactive worlds that respond coherently to player actions, which is a shared goal of computer graphics, games, and artificial intelligence.

Method: The paper examines interactive game world modeling along four dimensions: player action control, game state dynamics, state-observation persistence, and real-time interactive generation.

Result: The paper provides an analysis of existing approaches to interactive game world modeling, grouped into representative families, and discusses their strengths and trade-offs. Additionally, a scalable data engine for Black Myth: Wukong is presented.

Conclusion: The paper aims to provide a clear picture of the current state of the field and foster progress toward interactive game worlds.
...
TL;DR: VideoRAE, a representation autoencoder, leverages frozen video foundation model (VFM) features to create compact, generation-friendly video latents, achieving state-of-the-art reconstruction and generation results.


<details>
  <summary>Details</summary>
Motivation: Existing 3D Variational Autoencoders (3D-VAEs) are limited in capturing semantic and spatio-temporal structures, while Video Foundation Models (VFMs) have shown strong video understanding capabilities but their representations need to be transformed into compact and generation-friendly video latents.

Method: VideoRAE uses a lightweight 1D self-attention projector to compress multi-scale hierarchical features from a frozen VFM encoder, supporting both continuous and discrete latents for diffusion transformers and autoregressive models.

Result: VideoRAE achieves state-of-the-art class-to-video gFVDs of 40 and 93 with AR and DiT generators on UCF-101, converging approximately 5x faster than competing autoencoder baselines, and shows faster convergence in a controlled 2B-scale text-to-video study.

Conclusion: The proposed VideoRAE validates frozen VFM representations as versatile and generation-friendly video latents, enabling strong reconstruction and generation results in both continuous and discrete regimes.
...
TL;DR: FixItFlow, an automated system, generates troubleshooting guides from historical incident data using large language models, reducing documentation burden and improving incident response.


<details>
  <summary>Details</summary>
Motivation: The manual creation of troubleshooting guides is labor-intensive, leading to incomplete coverage and outdated documentation, which hampers the ability to rapidly diagnose and resolve incidents in cloud services.

Method: FixItFlow uses large language models to extract diagnostic patterns from historical incident data, synthesizing structured guides with verified commands and enforcing strict validation.

Result: Generated guides achieved 61.5% positive ratings for clarity and showed a 2.3x reduction in mitigation time for incidents with associated guides.

Conclusion: Automated guide generation can improve incident response while reducing documentation burden on engineering teams.
...
TL;DR: The paper introduces Safe-Psych, a benchmark for evaluating how large language models (LLMs) handle evolving diagnostic uncertainty in clinical psychiatry. It finds that even strong LLMs struggle with incomplete clinical information, often providing premature diagnoses or failing to seek clarification.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the issue of incomplete or evolving clinical evidence in healthcare decision support using LLMs. The authors aim to create a benchmark that simulates real-world clinical scenarios where information is disclosed incrementally.

Method: The authors introduce Safe-Psych, a sequential benchmark containing over 1,000 real-world psychiatric clinical notes. They evaluate multiple state-of-the-art LLMs in both full-information and sequential settings, using psychiatrist-derived action labels (DIAGNOSE, CLARIFY, or ABSTAIN) at each stage.

Result: The results show that even strong LLMs struggle under incomplete clinical information, with under-abstention exceeding 60% for most models. The authors also find that safety-aware prompting reduces premature commitment but shifts errors toward excessive abstention. In sequential evaluation, models frequently diagnose before sufficient evidence is available and rarely seek clarification unless explicitly prompted.

Conclusion: The paper concludes that Safe-Psych reveals a limitation across the evaluated models: recognizing when clinical evidence is incomplete and additional information is needed. The authors release Safe-Psych to support research on improving LLM safety in healthcare.
...
TL;DR: The study evaluates the effectiveness of open-source detectors in identifying LLM-generated patent text under strict European Patent Office (EPO) guidelines, revealing high false-positive rates and proposing a linguistic-complexity logistic regression as a more accurate alternative.


<details>
  <summary>Details</summary>
Motivation: The European Patent Office (EPO) guidelines now hold applicants responsible for LLM-assisted content, necessitating the detection of AI-generated patent text to prevent improper filings.

Method: The study benchmarks three open-source zero-shot detectors (Binoculars, Fast-DetectGPT, DetectGPT) on 500 granted EPO H04 telecom patents and 500 LLM-generated counterparts. It also proposes a seven-feature linguistic-complexity logistic regression model.

Result: The detectors show high false-positive rates (> 60%), with Binoculars at 78.3%, Fast-DetectGPT at 61.3%, and DetectGPT at 80.5%. The proposed logistic regression model achieves 74.0% accuracy at 28.1% FPR, outperforming the perplexity-only baseline.

Conclusion: The study concludes that detecting LLM-generated patent text is challenging under consumer hardware constraints and EPO guidelines, but a linguistic-complexity logistic regression offers a more accurate solution.
...
TL;DR: The paper proposes a data-driven approach for simultaneous speech translation, improving streaming quality without architectural changes.


<details>
  <summary>Details</summary>
Motivation: Simultaneous speech translation is challenging for decoder-only LLM systems due to limited context and cross-lingual reordering.

Method: The approach uses fixed-length chunks for cumulative streaming decoding with a rewind-based committed prefix and teacher-labeled prefix-to-prefix targets with bounded waiting for fine-tuning.
...
Conclusion: The proposed approach, CSSEL-P2P, effectively achieves simultaneous speech translation without architectural changes via P2P supervision.
...
TL;DR: The paper explores the use of persona vectors to probe the behavioral organization of language models, creating a 53-trait inventory and labeling traits as natural, steerable, or intractable.


<details>
  <summary>Details</summary>
Motivation: The authors aim to understand which behaviors a language model will express, hide, or resist, and how persona vectors can be used to probe this organization beyond simple prompting.

Method: The authors compile a 53-trait inventory across four behaviorally distinct domains and apply persona vectors to two open-weight models to label traits as natural, steerable, or intractable.

Result: The study finds that both models default to helpful, task-oriented behavior, and steering produces significant gains on traits that are not part of the default behavior, such as hyperbole, hallucination, and sycophancy.

Conclusion: The authors conclude that persona vectors are most informative as a probe of behavioral organization, rather than just a set of controls, and can be used to recover traits that are resistant to standard extraction.
...
TL;DR: The paper presents Text2Sign, a text-conditioned diffusion model for generating short sign-language clips, which runs on a single NVIDIA L4 GPU and achieves a validation loss of 0.00999 and SSIM of 0.2403.


<details>
  <summary>Details</summary>
Motivation: The motivation is to develop a cost-effective text-to-signer video generation model for millions of Deaf and hard-of-hearing people, as current video diffusion models are expensive to train and evaluate.

Method: The method involves combining a frozen vision-language text encoder with a 3D encoder-decoder and factorized spatiotemporal attention to reduce the cost of full-video attention while preserving motion coherence.

Result: The best short-run ablation reaches a validation loss of 0.0648, while a longer-run checkpoint reaches 0.00999. The system generates a 32-frame, $64 \times 64$ clip in 12.60 seconds, with peak inference memory of 3.12 GB.

Conclusion: The system is a single-GPU research baseline for generating short sign-language clips, but it is restricted to low-resolution, short clips and lacks expert linguistic evaluation.
...
TL;DR: RAGthoven, a system for multilingual humor generation, uses a multi-stage large language model pipeline and achieves competitive results with a strong baseline model.


<details>
  <summary>Details</summary>
Motivation: The motivation is to develop a system for SemEval-2026 Task 1, Subtask A, which focuses on multilingual constrained humor generation in English, Spanish, and Chinese.

Method: RAGthoven uses a multi-stage large language model pipeline consisting of a Planner, Best-of-N Writer, Reflector, and Judge, grounded in computational humor theory. It also explores the use of retrieval-augmented generation and agentic variants.

Result: RAGthoven achieves competitive results, sharing Rank 1 with the Gemini 2.5 Flash baseline in all three languages, with statistically tied performance in English and Chinese, and a lead of 42 raw Elo points in Spanish.

Conclusion: The results suggest that language-dependent diminishing returns are observed from elaborate multi-stage prompt engineering and agentic scaffolding once a strong frontier model is used.
...
TL;DR: The paper discusses the limitations of attention-based KV cache eviction (H2O) in long-context models, particularly with schema-dense input streams, and proposes a role-conditional allocation method to improve performance.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the issue of H2O's non-stationary filter disproportionately retaining noise in schema-dense input streams, leading to decreased accuracy.

Method: The authors propose a retraining-free, role-conditional allocation method over SnapKV's windowed score, governed by a single tuned hyperparameter, and use a 15 MB linear role probe to supply labels at negligible inference cost.

Result: The proposed method closes 63-98% of the H2O gap at sub-20% budgets and modestly matches or exceeds full-cache accuracy at higher budgets, with a small, seed-sensitive denoising effect.

Conclusion: The authors conclude that their role-conditional allocation method can effectively improve the performance of H2O in long-context models, particularly with schema-dense input streams.
...
TL;DR: This study evaluates the mathematical reasoning capabilities of large language models (LLMs) in Bengali, a low-resource language. It introduces a new dataset, GSM-Plus-BN, and benchmarks six open-source LLMs, finding that GPT-OSS-20B achieves the highest accuracy and that Chain-of-Thought prompting improves performance.


<details>
  <summary>Details</summary>
Motivation: The evaluation of mathematical reasoning in LLMs has been limited to high-resource languages like English, creating a barrier to equitable AI development in linguistically diverse regions like Bangladesh, where Bengali is spoken by over 230 million people.

Method: The study uses a novel perturbated Bengali mathematical dataset (GSM-Plus-BN) derived from the English GSM-Plus benchmark. It evaluates six open-source LLMs using 9,000 evaluation samples and two prompting methods: Standard Prompting and Chain-of-Thought (CoT) Prompting.

Result: GPT-OSS-20B achieves the highest seed question accuracy of 96.08% under Standard Prompting. Larger models like Llama-3.3-70B and GPT-OSS-120B show superior robustness across perturbation types. CoT prompting improves performance for most models, but a performance gap persists compared to English benchmarks.

Conclusion: This research provides a foundational contribution by introducing GSM-PLUS-BN as a new resource and baseline for future Bengali mathematical reasoning research, highlighting the need for further development of LLMs in low-resource languages.
...
TL;DR: A new hybrid pipeline called Apaf is proposed to analyze policy discourse by identifying frames and relations in policy text, enabling a more transparent understanding of governance outcomes.


<details>
  <summary>Details</summary>
Motivation: Existing computational approaches to policy discourse cannot effectively express the tensions and frame-mediated relations between participatory commitments and managerial control in policy documents.

Method: Apaf uses a hybrid LLM-symbolic pipeline to classify arguments into deliberative or managerial frames and identify four frame-mediated relation subtypes through deterministic rules over LLM-extracted features.

Result: The proposed pipeline was tested on a novel dataset of 100 sub-documents of disaster-risk-reduction policy from the USA, UK, Canada, and Australia, resulting in accurate, interpretable, and stable argument graphs across jurisdictions.

Conclusion: The Apaf pipeline offers a more transparent and structured approach to analyzing policy discourse, enabling domain experts to inspect and contest the results, and providing a better understanding of governance outcomes.
...
TL;DR: The paper focuses on schema retrieval in SQL settings, which is identifying the required tables and columns for a natural language question in a large database. The authors propose a corpus-adaptive fine-tuning method to improve the performance of text and code embedders for this task.


<details>
  <summary>Details</summary>
Motivation: The authors argue that schema retrieval is a fundamental problem that precedes generation in SQL settings, especially at scale. They claim that this step deserves first-class evaluation.

Method: The authors recast five text-to-SQL datasets as retrieval tasks and propose corpus-adaptive fine-tuning. This involves synthesizing natural-language queries from the target schema corpus, mining granularity-aware hard negatives, and fine-tuning a 305M-parameter embedder contrastively.

Result: The proposed method improves the average recall@10 from 60.4 to 75.6 and nDCG@10 from 51.9 to 68.0. It also improves an 8B state-of-the-art embedder from 77.8 to 78.4 recall@10.

Conclusion: The paper establishes schema linking as a standalone retrieval task and lightweight, label-free corpus adaptation as a practical route to deploying it at enterprise scale.
...
TL;DR: Proposed a meta-learning framework for aligning large language models in low-resource languages, leveraging preference data from other languages to achieve effective adaptation with minimal data.


<details>
  <summary>Details</summary>
Motivation: Unequal availability of human preference data across languages hinders alignment of large language models in multilingual settings.

Method: Meta-learning framework for Reinforcement Learning from Human Feedback and Direct Preference Optimization, learning a transferable initialization from other languages.

Result: Achieved up to 28% win-rate improvements over baselines in extremely low-resource settings, outperforming baselines across multiple target languages and model scales.

Conclusion: The proposed framework effectively adapts to target languages with minimal data, retaining advantages across different meta-training languages and linguistic distances.
...
TL;DR: The study examines the use of Large Language Models (LLMs) as judges in closed-loop regeneration for table recognition, finding that LLM judges provide weak signals, and iterative refinement requires a verification signal that detects structural change, not just judge scores.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this study is to validate the use of LLMs as judges in closed-loop regeneration, specifically in table recognition, and to understand the limitations of LLMs as evaluators.

Method: The study uses FinTabNet and OmniDocBench datasets to evaluate the performance of LLM judges in table recognition, and employs a controlled testbed with deterministic TEDS evaluation.

Result: The study finds that LLM judges provide weak signals, with scores frequently tied and rankings not reproducible. The results also show that iterative refinement requires a verification signal that detects structural change, not just judge scores alone.

Conclusion: The study concludes that while LLMs can be valuable evaluators, their evaluation ability does not imply optimization utility. Iterative refinement requires, at minimum, a verification signal that deterministically detects structural change, rather than judge scores alone.
...
TL;DR: The paper presents a neural annotation pipeline for interlinearizing discourse data in endangered languages, specifically for Irabu Ryukyuan, and evaluates its performance under limited supervised resources.


<details>
  <summary>Details</summary>
Motivation: The motivation is to automate parts of the interlinearization workflow for endangered languages, where time is limited, to aid in grammar writing and documentation.

Method: The authors implement a full neural annotation pipeline using BiLSTM-CRF models for morpheme segmentation, POS tagging, and glossing, and evaluate it under a realistic hard constraint of approximately one hour of fully annotated discourse as the entire supervised resource.

Result: The results show that gold POS improves grammatical glossing by +4.4 points, and the gain grows as data shrink. A POS tier more than halves the amount of glossed data needed to reach a given accuracy. However, in a fully automatic pipeline, the tagger still errs on 12% of morphemes, and an incorrect POS misleads the glossing model.

Conclusion: The authors conclude that annotating quadrilinearly - text, POS, gloss, translation - is a recommended documentation practice, and that the value of POS tagging is latent and can be recovered as tagger accuracy rises.
...
TL;DR: GFlowRL is a streamlined GFlowNet-style RL algorithm that scales stably across dense and sparse architectures, outperforming previous methods on math, code, and adversarial benchmarks.


<details>
  <summary>Details</summary>
Motivation: The difficulty of scaling GFlowNet-style RL to modern post-training pipelines due to gradient instability and engineering overhead caused by a learned prompt-conditional partition function.

Method: Replacing the learned partition function with an in-batch Monte Carlo estimate and introducing two stabilizers: importance-sampling correction and asymmetric flow-gap clipping.

Result: GFlowRL exceeds previous methods on math, code, and adversarial benchmarks, achieving a Codeforces rating of 2048 and outperforming the previous SOTA multi-turn attacker.

Conclusion: GFlowRL is a scalable and effective GFlowNet-style RL algorithm that can handle large models and complex tasks, making it a promising approach for future research.
...
TL;DR: The paper studies the dynamics of on-policy distillation (OPD) in large language models, identifying its role, pathologies, and regulations, and proposes lightweight signal regulations to improve its effectiveness.


<details>
  <summary>Details</summary>
Motivation: The training dynamics of OPD are poorly understood, and the authors aim to systematically study its role, pathologies, and regulations to improve its effectiveness.

Method: The authors conduct a systematic study of OPD, identifying two pathologies (Student-Teacher Mismatch and Length Exploitation) and proposing lightweight signal regulations (advantage clipping and log-scale compression) to alleviate them.

Result: Experiments across seven benchmarks demonstrate that the proposed regulations alleviate length exploitation and enable effective distillation, stably surpassing OPD variants and RLVR baselines.

Conclusion: The study confirms that well-regulated signal quality, rather than mere teacher scale, governs successful exploration in OPD, and that lightweight signal regulations can improve its effectiveness.
...
TL;DR: This study compares training methods for small language models to generate patient-friendly narratives from biomedical data, finding that certain methods outperform proprietary models.


<details>
  <summary>Details</summary>
Motivation: The need to translate complex biomedical data into patient-friendly narratives in modern biomedical informatics.

Method: Comparative analysis of post-training methods including SFT, DPO, ORPO, and GRPO with Qwen-based SLMs on a medicine package leaflets dataset and openFDA drug label data.

Result: Aligned SLMs outperform proprietary models like GPT-5; ORPO outperforms SFT baselines; GRPO yields the most robust cross-dataset performance.

Conclusion: The study highlights the effectiveness of certain post-training methods for small language models in generating patient-friendly biomedical narratives.
...
TL;DR: The paper studies cross-rubric generalization in automated essay scoring, where the model is trained on essays labeled under one set of rubrics and evaluated on unseen rubrics. It proposes a Large Language Model fine-tuning framework with rubric-agnostic intermediate representations and target-essay supervision, achieving improved performance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitation of current automated essay scoring research, which focuses on cross-prompt generalization, but not on adapting to new scoring rubrics. In practice, educators may revise or introduce new rubrics to evaluate different aspects of essays.

Method: The method involves using a Large Language Model fine-tuning framework with two components: rubric-agnostic intermediate representations (traits) and target-essay supervision under seen rubrics during training.

Result: The results show that the proposed approach improves macro F1 by 5.0% over a baseline without traits in the hardest setting. The best fine-tuned model outperforms GPT-5-mini prompting by 2.1% macro F1 and trails GPT-5 by 1.9%.

Conclusion: The conclusion is that the proposed approach improves generalization to unseen rubrics in automated essay scoring, and that trait-based intermediate structure and controlled supervision are effective in achieving this goal.
...
TL;DR: A benchmark and reference system for live captioning of Sikh Kirtan, a continuous sung recitation of verses from the Sri Guru Granth Sahib Ji, is presented. The system achieves 57.9% overall frame accuracy on the hardest variant.


<details>
  <summary>Details</summary>
Motivation: The motivation is to develop a system for live captioning of Sikh Kirtan, which is a closed-vocabulary problem requiring exact word-for-word lines from the canonical scripture.

Method: The method involves formalizing the task as predicting a pair (shabad_id, line_idx) or null, and using a reference system consisting of a fine-tuned 120M IndicConformer, fuzzy matcher, and state machine.

Result: The reference system achieves 57.9% overall frame accuracy across all 12 cases on the hardest variant (live x blind).

Conclusion: The benchmark, reference system, and live deployment are released under permissive licenses to facilitate further improvements in live captioning of Sikh Kirtan.
...
TL;DR: The paper introduces DevicesWorld, a benchmark for cross-device collaborative operation, and evaluates LLM-agent systems on it, revealing their limitations in completing end-to-end tasks across multiple devices.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the lack of benchmarks for evaluating agents' ability to acquire and integrate information across heterogeneous devices and complete tasks with cross-device dependencies.

Method: The authors propose DevicesWorld, a large-scale executable benchmark that integrates mobile, desktop, and IoT devices, and evaluate five frontier LLM-agent systems on it.

Result: The evaluation shows that all methods achieve low success rates, with the best reaching only 12.5%, and identifies common failure patterns such as agents getting stuck or confusing devices.

Conclusion: The paper concludes that DevicesWorld provides a useful evaluation framework for research on reliable cross-device agents, and highlights the need for further improvement in cross-device collaborative operation.
...
TL;DR: MyAG is a graph-based framework for designing and analyzing composable LLM agent systems, enabling flexible reuse of components and analysis of performance-efficiency tradeoffs.


<details>
  <summary>Details</summary>
Motivation: The need for a flexible and composable framework for designing and analyzing LLM agent systems, allowing for reuse of components and analysis of performance-efficiency tradeoffs.

Method: MyAG uses three graph abstractions: component graph, workflow graph, and search graph, to separate agent system construction and enable hierarchical composition.

Result: Experiments on representative agent applications show that MyAG supports flexible agent system design and helps analyze performance-efficiency tradeoffs.

Conclusion: MyAG is a publicly available, open-source framework that enables flexible design and analysis of composable LLM agent systems.
...
TL;DR: The paper describes a system for BioASQ Task 14B 2026, focusing on re-retrieval strategies and combining language model answers, achieving top rankings in several metrics.


<details>
  <summary>Details</summary>
Motivation: The work aims to improve the performance of a BioASQ system by optimizing re-retrieval policies and ensemble methods for combining multiple language model answers.

Method: The system uses a hybrid retrieval approach with two parallel pipelines: a dense BGE + BM25 + RRF first stage and an agent-driven pipeline. It also employs a BGE cross-encoder quality gate for selective re-retrieval and a synonym-union resolver for Phase B answering.

Result: The system achieves significant improvements in list F1 and list precision on Task 12B 2024 validation and Test 13B. It also secures top rankings on the Task 14B 2026 preliminary leaderboard, including first place on the combined-exact aggregate and individual question-type cells.

Conclusion: The paper concludes that the proposed system effectively balances re-retrieval cost and performance, and that the synonym-union resolver enhances list recall while maintaining competitive list F1 scores.
...
TL;DR: This paper studies whether language models can estimate their familiarity with entities before generating answers, and explores the relationship between familiarity and model performance.


<details>
  <summary>Details</summary>
Motivation: The authors aim to investigate the ability of language models to gauge their familiarity with entities, which is crucial for tasks like abstention and error prediction.

Method: The authors analyze activations at the final prompt token in twelve instruction-tuned models from four families, using a dataset of 1,440 Polish entities and fabricated controls.

Result: The results show that familiarity-probe scores can separate real from fabricated entities, track entity popularity, and are robust to prompt language. Adding a familiarity direction can move refusal rates monotonically.

Conclusion: The study supports a graded pre-generation entity-familiarity readout and a separation between representational familiarity and the policy that converts it into abstention.
...
TL;DR: The paper introduces MemCon, a framework that adapts memory retrieval for Large Language Model agents, leading to improved task success and reduced token consumption.


<details>
  <summary>Details</summary>
Motivation: The authors argue that existing memory systems for LLM agents rely on fixed heuristics, which are context-dependent and limit agentic learning. They propose a more adaptive approach to memory retrieval.

Method: The authors present MemCon, a framework that models memory operations as a Markov Decision Process and learns an online policy to adaptively decide when, what, and how much to retrieve from memory.

Result: MemCon consistently outperforms multiple memory baselines across 6 benchmarks, 3 agent frameworks, and 3 LLM backbones, achieving up to 15.2 points improvement in task success and 5-20% reduction in token consumption.

Conclusion: The paper concludes that MemCon provides a more effective and efficient approach to memory retrieval for LLM agents, leading to improved performance and reduced resource usage.
...
TL;DR: This paper introduces a new task called Analogical Deep Research (ADR) to test if Large Language Model (LLM) agents can find and use historical analogies for foresight analysis. The authors propose a new framework called Causal Analogical Researcher (CANA) that improves LLM agents' ability to identify and integrate historical analogies.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to investigate whether LLM agents can effectively find and leverage historical analogies for foresight analysis, and to address the challenges they face in doing so.

Method: The authors propose a new agentic framework called Causal Analogical Researcher (CANA) that guides LLMs to find and integrate historical analogies. CANA incorporates a structural decomposition representation and integrates structural feedback for reflective improvements of historical analogy identification and integration.

Result: The results show that CANA brings up to 10% improvements in historical analogy generation and surpasses the state-of-the-art deep research agents in the ADR-bench. Case studies with ongoing events confirm the effectiveness of CANA in leveraging historical analogies.

Conclusion: The authors conclude that LLM agents face a key obstacle in finding analogies because they match on surface features rather than underlying mechanisms. They propose two principles required for ADR, including mechanism alignment and cross-analogy confirmation, and demonstrate the effectiveness of CANA in addressing these challenges.
...
TL;DR: The authors present a self-evolving agent-harness framework that improves the performance of large language models (LLMs) by automatically generating and testing modifications to the model's harness, leading to significant gains in performance across seven domains.


<details>
  <summary>Details</summary>
Motivation: The performance of LLMs is not only determined by the model itself but also by its harness, which includes prompts, injected knowledge, runtime control, and configuration. Improving the harness can raise performance without modifying the model, but it is challenging to determine which changes are effective.

Method: The authors propose a framework that separates proposing changes from evaluating their effectiveness. A language model diagnoses failures and suggests patches, while deterministic code samples, measures, and tests the significance of the changes. The framework uses a gated, categorical quality-diversity archive (GSME) to store and evaluate patches.

Result: The framework achieves significant performance gains across seven domains, with improvements ranging from +9 to +15.5pp on a sealed test set. The gains retain 86-147% of the training gain, indicating that they generalize rather than overfit.

Conclusion: The authors conclude that the diagnose-and-credit loop, rather than any specific harness, is the key to transferring improvements across different models and domains. The framework provides a trustworthy and effective way to automatically improve the performance of LLMs.
...
TL;DR: The study highlights a critical issue in LLM-as-judge systems where a silent failure in generating hallucinated answers leads to significant bias, which can go undetected by statistical checks.


<details>
  <summary>Details</summary>
Motivation: The authors aim to expose a structural vulnerability in the common practice of generating synthetic corpora for evaluating LLM-as-judge systems, specifically the reliance on LLM-generated negative examples.

Method: The study uses a multilingual faithfulness-judgment corpus and a controlled experiment to demonstrate the issue, involving the generation of hallucinated answers, judging, and statistical analysis.

Result: A significant bias (32-point collapse in selection accuracy) was found due to a decoding-budget parameter issue, which was not caught by aggregate statistical checks but by manual reading of raw generations.

Conclusion: The study emphasizes the need for a validation protocol in LLM-as-judge corpora, particularly those lacking a mechanical way to verify item integrity, and suggests using a test oracle problem framework to address these vulnerabilities.
...
TL;DR: The paper explores a text-free framework for assessing L2 speech, focusing on phonetic accuracy, rhythm, and intonation using DTW over self-supervised WavLM representations.


<details>
  <summary>Details</summary>
Motivation: Traditional L2 speech assessment methods often require training with labeled data and focus on phonetic assessment, leaving suprasegmental features underexplored.

Method: The study uses Dynamic Time Warping (DTW) over self-supervised WavLM representations to assess phonetic accuracy, rhythm, and intonation in English and Japanese L2 speech.

Result: The DTW-based approach shows promising results, exceeding human agreement on phonetic scoring and approaching human-level performance on rhythm assessment, but with more modest results for intonation.

Conclusion: Self-supervised representations provide a promising, text-free basis for multi-aspect pronunciation assessment in L2 speech.
...
TL;DR: This paper studies how post-training methods impact confidence during reasoning in large language models, and proposes a position-aware confidence strategy called PosConf.


<details>
  <summary>Details</summary>
Motivation: Current evaluation methods for post-training methods in large language models focus only on final-answer accuracy, neglecting the impact on confidence during reasoning.

Method: A three-stage calibration framework is introduced to evaluate confidence before, during, and after chain-of-thought generation. The authors compare different post-training methods, including supervised fine-tuning (SFT), reinforcement learning (RL), and on-policy distillation (OPD).

Result: The results show that different post-training methods provide useful confidence signals at different stages of reasoning. PosConf, a position-aware confidence strategy, improves answer aggregation and early stopping.

Conclusion: The study highlights the importance of using confidence in reasoning models both stage-wise and position-awarely, and proposes PosConf as a effective strategy for improving performance.
...
TL;DR: The paper proposes SPyCE, a framework for multimodal agents that distills trajectories into reusable skills, co-evolving with the policy during training, outperforming RL-based and memory-based baselines.


<details>
  <summary>Details</summary>
Motivation: Existing methods for multimodal agents either reduce trajectories to scalar rewards or rely on test-time retrieval, which limits their ability to learn reusable patterns. The authors aim to distill multimodal reasoning trajectories into reusable skills that co-evolve with the policy.

Method: The authors propose SPyCE (Skill-Policy Co-evolution), a framework that distills trajectories into a hierarchical skill library, which is updated throughout reinforcement learning. The skill library consists of execution skills and workflow skills, which guide policy rollouts and evolve using valuable rollouts generated by the policy.

Result: Experiments across eight benchmarks demonstrate that SPyCE consistently outperforms both RL-based and memory-based baselines. The results suggest that joint skill-policy optimization is a promising paradigm for building capable multimodal agents.

Conclusion: The paper concludes that SPyCE is an effective framework for multimodal agents, and that co-evolving skills and policies is a promising approach for building capable multimodal agents.
...
TL;DR: The study explores using Large Language Models to generate high-order questions in multiple languages, guided by alternative frameworks to Bloom's Taxonomy.


<details>
  <summary>Details</summary>
Motivation: The difficulty of creating high-order questions for critical thinking development, and the limitation of existing research to Bloom's Taxonomy and English language.

Method: Using Large Language Models with prompts based on Claim-Evidence-Reasoning and Divergent Questioning frameworks in Basque, Spanish, and English.

Result: The models generate questions effectively in all three languages, but only about half are recognized as high-order by teachers. Alternative frameworks produce varied questions.

Conclusion: Alternative frameworks to Bloom's Taxonomy can provide viable complements for generating high-order questions in multiple languages.
...
TL;DR: The study proposes DeepStress, a framework for testing the robustness of search agents to poor-quality evidence, and evaluates several agents on two benchmarks.


<details>
  <summary>Details</summary>
Motivation: The robustness of search agents to poor-quality evidence is under-explored, yet can lead to dramatic failures in real-life applications.

Method: The study uses DeepStress, a stress testing framework that replaces the retrieval module of search agents with a controlled synthetic environment, to test agents on three dimensions: trustworthiness, relevance, and factuality.

Result: The study finds substantial differences in the ability of search agents to handle unreliable information and proposes new metrics to document system outcomes and interactions between conflicting knowledge.

Conclusion: The study highlights the importance of testing the robustness of search agents to poor-quality evidence and proposes a framework for doing so.
...
TL;DR: The paper proposes DeltaMergeLowRes, a method to adapt a multilingual encoder to a new language and task with limited data by separately training language and task deltas and recombining them in weight space.


<details>
  <summary>Details</summary>
Motivation: The current approach to adapting a multilingual encoder to a new language and task requires a large amount of labeled data and is computationally expensive. The authors aim to find a more efficient approach.

Method: DeltaMergeLowRes learns a language delta and a task delta from unlabeled monolingual text and labeled English data, respectively, and composes them using one of four rules: additive, activation-guided, sparsity-aware, and cross-axis TIES.

Result: The cross-axis TIES method outperforms other composition rules in summarization tasks, improving chrF scores by 4-7 points. It also improves QA F1 and EM scores by 2.32 and 2.91 points, respectively. Additionally, sparsity-aware merging reduces classification ECE by 36% while maintaining macro-F1 parity.

Conclusion: The proposed DeltaMergeLowRes method allows for efficient adaptation of multilingual encoders to new languages and tasks with limited data. The choice of composition rule significantly affects the performance of the merged model.
...
TL;DR: Proposed CAVE-ABSA, a framework for generating and validating aspect-level counterfactuals for Aspect-Based Sentiment Analysis (ABSA) to improve model robustness.


<details>
  <summary>Details</summary>
Motivation: Existing counterfactual generation methods often produce edits that are fluent but invalid, semantically drifting, or contradictory, making it challenging to evaluate ABSA models.

Method: CAVE-ABSA localizes opinion spans, performs controlled rewriting, refines candidates, and filters them using multiple criteria.

Result: CAVE-ABSA provides a principled approach for producing meaningful aspect-local counterfactuals and testing ABSA models.

Conclusion: CAVE-ABSA addresses the limitations of existing counterfactual generation methods and provides a framework for constructing validated counterfactual ABSA datasets.
...
TL;DR: This paper explores using large language models (LLMs) for whole-document translation, beyond the traditional sentence-by-sentence approach, and presents a system called PAT that leverages a comparable corpus to inform translations.


<details>
  <summary>Details</summary>
Motivation: The motivation is to move beyond the traditional sentence-by-sentence translation paradigm and explore whole-document, corpus-informed translation using LLMs.

Method: The method involves using a RAG-based system called PAT, which pairs user-configured specifications with context from a comparable corpus of authentic longform texts to generate whole-document translations.

Result: The results show that LLMs can be moved toward reformulation and away from the sentence-by-sentence paradigm, but more work is needed to improve the effectiveness of those reformulations.

Conclusion: The conclusion is that while LLMs can be used for whole-document translation and show promise, further research is needed to improve the effectiveness of corpus-informed translations and address challenges related to automatic translation system design, corpus construction, and translation quality evaluation.
...
TL;DR: The authors introduce Hindcast, a method to evaluate forecasting models by simulating a past date and grading their predictions based on information available at that time, addressing the issue of 'leaked' information in traditional backtesting methods.


<details>
  <summary>Details</summary>
Motivation: Traditional backtesting methods for forecasting models, such as LLMs, are flawed because they can 'leak' information about the outcome through retrieval of reports written after the event or through training data that includes information from after the event, resulting in grading recall rather than foresight.

Method: The authors propose Hindcast, which replays resolved prediction markets against a frozen snapshot of public Reddit, allowing the model to only read posts written before a chosen past date $t_0$, and scores forecasts against both the actual outcome and the market's price at $t_0$.

Result: The results show that retrieval still helps most models, but only where Reddit discussed the event beforehand. Where the archive carried only speculation, retrieval hurts.

Conclusion: The authors conclude that by closing the 'leak' of information, their method provides a more accurate evaluation of forecasting models, highlighting the importance of considering the information available at the time of prediction.
...
TL;DR: A new data provenance system called ob is proposed, which enables precise data removal by identifying specific training records belonging to a given author, reducing over-deletion and improving unlearning efficiency.


<details>
  <summary>Details</summary>
Motivation: The need for precise data removal in machine learning models, driven by data contributors' requests to have their data removed, and the current limitations of existing provenance systems that operate at file or dataset levels.

Method: The ob system propagates author identity through data processing pipelines and uses deterministic queries to resolve revocation requests into precise forget sets.

Result: The ob system reduces dataset-level over-deletion from 101x to 1.3x, adds 1.3-4.0% (HuggingFace) and 2.1-19.0% (Datatrove) throughput overhead, and improves unlearning by 42% over random baselines on a 1.7B model.

Conclusion: The proposed ob system offers a more precise and efficient approach to data removal in machine learning models, reducing over-deletion and improving unlearning efficiency.
...
TL;DR: SPINE, an agentic framework, enables systematic debugging and deployment of bimanual robots with minimal expertise, improving operationalization success and reducing time-to-teleoperation.


<details>
  <summary>Details</summary>
Motivation: The deployment gap, or 'spinal cord', between sophisticated AI and physical platforms remains a primary bottleneck to scalable Embodied AI, requiring tedious expert-driven calibration.

Method: SPINE consists of two orchestrated multi-agent workflows: a profile builder and a debugger, which systematically debug and deploy bimanual robots.

Result: SPINE improved operationalization success from 75% to 100% and reduced mean time-to-teleoperation from 16 min 45 s to 13 min 47 s, and resolved all 10 implanted bugs on a distinct bimanual arm.

Conclusion: SPINE can transfer across bimanual platforms, reduce dependence on expert calibration, and move embodied AI closer to scalable real-world deployment.
...
TL;DR: The authors introduce a method called interventional grounding audits to test if large language models truly depend on their stated premises when producing chain-of-thought reasoning. They evaluate this method on a synthetic benchmark and find it to be more effective than a self-consistency baseline in detecting premise dependencies.


<details>
  <summary>Details</summary>
Motivation: Large language models may produce logically sound chain-of-thought reasoning that does not genuinely depend on its stated premises, and the authors aim to develop a method to test premise dependency.

Method: The authors propose interventional grounding audits, a black-box, step-level test where they intervene on a single premise by substituting its target predicate with a fresh symbol, re-run the model, and check if each reasoning step's normalized conclusion changes.

Result: The method achieves F1 = 0.806 on detecting proof-tree dependencies and F1 = 0.885 on predicate-determining dependencies, outperforming a self-consistency baseline (F1 = 0.343).

Conclusion: The authors conclude that their method is effective in detecting premise dependencies and identify a 'right answer, wrong reasoning' signal in 66% of correctly-solved problems, highlighting a blind spot in the consistent-substitution evaluator.
...
TL;DR: The paper expands the cognitive power of Neuro-symbolic AI based on $IFOL_B$ by introducing probability computation for unknown sentences and symmetry transformations for preserving knowledge and logical deduction.


<details>
  <summary>Details</summary>
Motivation: To overcome limitations of purely neural systems, such as lack of interpretability and logical structure, by combining neural learning and symbolic reasoning.

Method: The paper introduces global and local symmetry transformations and uses neural networks to compute probability density functions based on Shannon's maximum information entropy.

Result: The proposed approach enables the computation of probability density functions for unknown sentences and preserves knowledge and logical deduction.

Conclusion: The paper presents a method to expand the cognitive power of Neuro-symbolic AI based on $IFOL_B$ by incorporating probability computation and symmetry transformations.
...
TL;DR: This survey discusses self-improving autonomous agents, framing them as adaptive systems that gain capabilities from experience, and proposes a system-level framework for understanding their structure and self-improvement mechanisms.


<details>
  <summary>Details</summary>
Motivation: The primary goal of self-improving autonomous agents is controllable evolution or adaptation from experience with minimal human input.

Method: The survey provides a system-level framework representing a modern agent as a configuration coupling a foundation model with an operational scaffold, and formalizes self-improvement as a self-induced update operator.

Result: The survey organizes prior work on self-improving agents by update target and signals driving change, reviews applications, and discusses evaluation methods.

Conclusion: The survey concludes by highlighting open problems and future directions for self-improving autonomous agents, and provides a resource for tracking technical updates.
...
TL;DR: The authors propose a Context-Augmented Prompting framework to improve small language models (SLMs) for zero-shot molecular property prediction by incorporating graph-topological cues.


<details>
  <summary>Details</summary>
Motivation: SLMs suffer from structural blindness due to sequence representations under-specifying key graph-topological cues.

Method: A modular framework using a trained GNN expert model and a GNN to extract instance-specific explanatory subgraphs.

Result: The framework yields substantial accuracy gains, exceeding 25% relative improvement and up to 74% on Tox21, but a gap remains to specialized GNN models.

Conclusion: The proposed framework improves SLMs for molecular property prediction, but highlights both the value and limits of text-conditioned reasoning for molecular structure.
...
TL;DR: The paper discusses Oracle Agent Memory, a database-native memory substrate for long-horizon agents, and its evaluation on the LongMemEval benchmark.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the systems problem of agent memory, which requires retention of task state, user-specific facts, and procedural knowledge across extended conversations and sessions.

Method: The authors propose a layered architecture for Oracle Agent Memory, separating an active memory core from a passive memory-store interface with explicit scope control, and evaluate it using a combination of downstream task accuracy and memory-centric measures.

Result: The results show that Oracle Agent Memory achieves 93.8% accuracy on LongMemEval, outperforming flat-history baselines while using about 10.7x fewer tokens.

Conclusion: The paper concludes that Oracle Agent Memory is a effective solution for long-horizon agents, providing a robust and efficient memory substrate that can support complex conversations and task-oriented interactions.
...
TL;DR: The paper introduces DROPJ, a human-centred method for safe training and deployment of agent policies in unknown environments with no suitable reward function, leveraging human input.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the challenge of safely training and deploying agent policies in safety-critical environments with unknown dynamics and no suitable reward function.

Method: The authors propose DROPJ, which involves learning a world model from real-world trajectories, eliciting human preferences and justifications in a simulated environment, training a reward model, and deploying the agent using model predictive control.

Result: The results show that DROPJ reduces computational cost during training, improves performance during deployment, and enhances safety by incorporating human justifications.

Conclusion: The authors conclude that DROPJ is an effective human-centred method for safe training and deployment of agent policies in complex environments.
...
TL;DR: The cayleyR package in R solves permutation puzzles using cycle intersections in Cayley graphs, with application to the TopSpin puzzle.


<details>
  <summary>Details</summary>
Motivation: To provide a tool for solving permutation puzzles, specifically the TopSpin(n,k) puzzle, by detecting cycle intersections in Cayley graphs.

Method: The package uses an iterative bidirectional search algorithm, generating random operation sequences to find intersections in the Cayley graph, with a distance-guided bridge selection when no direct intersection is found.

Result: The cayleyR package is implemented in R, combining a C++ hash-indexed state store with optional Vulkan GPU acceleration, and is publicly available on CRAN.

Conclusion: The cayleyR package offers an efficient solution for solving permutation puzzles, such as TopSpin, by leveraging Cayley graphs and advanced algorithmic techniques.
...
TL;DR: The paper introduces Mycelium, a system that enables networked intelligence by connecting researchers and AI agents to facilitate collaboration and knowledge sharing, and demonstrates its effectiveness in a biological multi-omics campaign.


<details>
  <summary>Details</summary>
Motivation: Current AI-for-science systems focus on scaling individual reasoning processes, but complex scientific problems often require collaboration among team members with diverse backgrounds and expertise.

Method: The authors propose Mycelium, an active shared workspace that automatically connects researchers and AI agents, captures important observations and hypotheses, and routes them to relevant individuals or agents.

Result: The system was evaluated in a biological multi-omics campaign, where it successfully turned a local analytical finding into a cross-expert mechanistic constraint and ultimately into an experimental design.

Conclusion: The paper concludes that networked intelligence can be achieved through a computational account of sparse conditional computation over distributed scientific contexts, and that Mycelium is an effective implementation of this concept.
...
TL;DR: Develops an AI-native mathematical framework for underwriting, pricing, and contract design for agentic AI deployments, establishing structural properties of insurability and interpreting insurance as both an operational cost and regulatory mechanism.


<details>
  <summary>Details</summary>
Motivation: Agentic AI introduces new insurance challenges due to autonomous decision-making, tool invocation, environment modification, and interaction with third-party services.

Method: Develops a mathematical framework mapping risk states to event probabilities, loss severities, governance costs, premiums, deductibles, coverage allocation, and policy covenants, and formulates an optimization problem for insurance contract design.

Result: Establishes structural properties of insurability, including characterization of an insurability region, monotone deterioration of feasibility with increasing exposure, and governance certification thresholds.

Conclusion: The framework provides a new approach for underwriting, pricing, and contract design for agentic AI deployments, and interprets insurance as both an operational cost and regulatory mechanism for AI deployment.
...
TL;DR: The paper discusses the deployment of foundation models for transportation management center tasks, formulating the problem as a mixed-integer program to minimize total cost of ownership while meeting quality, latency, and safety constraints.


<details>
  <summary>Details</summary>
Motivation: The increasing use of foundation models in transportation management centers raises questions about which model to deploy for each function, in which mode, and under a shared hardware budget.

Method: The authors formulate the Foundation Model Deployment Portfolio (FMDP) problem as a mixed-integer program and propose a polynomial-time greedy heuristic to solve it.

Result: The FMDP approach identifies a mixed portfolio that reduces costs by 97% compared to the cheapest feasible all-closed-API baseline, and break-even analysis shows that on-premise GPU investment is reasonable only above a certain threshold of vision queries per hour or if API prices double.

Conclusion: The paper concludes that the FMDP approach can help optimize the deployment of foundation models in transportation management centers, reducing costs while meeting quality, latency, and safety constraints.
...
TL;DR: The authors introduce the Harness Handbook and Behavior-Guided Progressive Disclosure (BGPD) to improve behavior localization and edit-plan quality in AI agent harnesses.


<details>
  <summary>Details</summary>
Motivation: The evolution of AI agent harnesses is hindered by the difficulty of identifying code locations that implement target behaviors, leading to a central bottleneck in harness evolution.

Method: The authors propose the Harness Handbook, a behavior-centric representation synthesized automatically from a harness codebase via static analysis and LLM-assisted structuring, and BGPD, which guides agents from high-level behaviors to relevant implementation details.

Result: The proposed approach improves behavior localization and edit-plan quality while using fewer planner tokens, with significant gains on scattered sites, rarely executed paths, and cross-module interactions.

Conclusion: The authors conclude that evolving complex agentic systems depends not only on generating edits but also on determining where those edits should be made, and that their approach can facilitate this process.
...
TL;DR: The paper argues for theory-level autoformalization, which involves formalizing complete theories, including their inter-dependencies, as structured libraries.


<details>
  <summary>Details</summary>
Motivation: The authors argue that real formalization efforts require an entire web of axioms, definitions, and lemmas before target theorems can be stated, and that current work focuses too much on individual statements.

Method: The paper is a position paper that examines the significance of theory-level autoformalization, addresses alternative views, identifies open challenges, and proposes three promising paths forward.

Result: The authors propose a shift towards theory-level autoformalization and provide a survey of existing work on autoformalization.

Conclusion: The paper concludes that theory-level autoformalization is a necessary step forward for formalization efforts and proposes directions for future research.
...
TL;DR: EZSMTV3 is a new Constraint Answer Set Programming (CASP) framework that combines Answer Set Programming (ASP) with Satisfiability Modulo Theories (SMT) for solving complex search problems.


<details>
  <summary>Details</summary>
Motivation: The motivation is to advance the translational approach to CASP solving by creating an extensible SMT-based CASP framework.

Method: EZSMTV3 leverages state-of-the-art SMT solvers, such as CVC5, YICES, and Z3, to perform reasoning, and introduces a more expressive input language, supports optimization via weak constraints, and offers foundations for streamlined integration of new constraint types.

Result: Benchmarking results show that EZSMTV3 can handle mixed-domain constraints involving both integers and reals, and compares well with its CASP peers, such as CLINGCON, CLINGO[DL], and CLINGO[LP].
...
TL;DR: This paper studies how LLM agents adapt to hidden reliability shifts in tool choice during an ongoing session. It evaluates agent performance in adapting to silent changes in reliable tools.


<details>
  <summary>Details</summary>
Motivation: The motivation is to understand how LLM agents adapt to hidden reliability shifts in tool choice, inspired by set-shifting in cognitive psychology.

Method: The method involves a benchmark with tool-skill libraries having redundancies, a branched schedule that shifts reliable tool groups at hidden boundaries, and an evaluation framework that pairs every shift with a no-shift control.

Result: The results show that agents settle on a small recurring routine within a few turns of each boundary, with call shares concentrating on a few discrete values after each reliability shift. The study also finds qualitatively distinct failure modes across the same set of routines and that set framing affects routing dynamics.

Conclusion: The conclusion is that LLM agents have limited ability to adapt to hidden reliability shifts in tool choice, and their performance can be influenced by factors such as set framing.
...
TL;DR: The paper proposes LAPO, a method for evaluating intermediate interactions in multi-turn search reasoning using a self-generated process-supervision approach, achieving better results than existing methods.


<details>
  <summary>Details</summary>
Motivation: Current reinforcement learning methods for multi-turn search reasoning rely on terminal outcome rewards, which cannot distinguish between useful, redundant, and harmful intermediate interactions.

Method: LAPO uses backward leave-one-turn attribution, replacing each turn and its retrieval observation with a [DELETE] placeholder, and measures the resulting change in policy's mean log-likelihood of the gold answer.

Result: LAPO achieves an average exact-match score of 0.326 across seven knowledge-intensive question-answering datasets, outperforming the strongest step-reward baseline, IGPO, by 0.053.

Conclusion: The proposed method, LAPO, provides effective process supervision for multi-turn search agents, demonstrating the benefits of policy-derived retrospective attribution.
...
TL;DR: The paper presents a new Structured Multi-Agent RCA pipeline that outperforms existing methods in identifying root causes of production microservice failures, and analyzes the limitations of current approaches.


<details>
  <summary>Details</summary>
Motivation: The difficulty in identifying root causes in large-scale, multimodal telemetry data from microservice failures, which has proven challenging for both classical and LLM-based approaches.

Method: The authors propose a Structured Multi-Agent RCA pipeline, which includes a reverse reasoning agent and an automated rule mining pipeline, to improve root cause analysis.

Result: The proposed pipeline substantially outperforms existing baselines, and the analysis reveals that the primary constraints are model reasoning capability and domain knowledge.

Conclusion: The paper concludes that progress in root cause analysis requires improvements at the model level, and that scaffold engineering and better data pipelines alone cannot close the gap.
...
TL;DR: UrbanAgent, a framework for urban region profiling, outperforms existing methods by using multi-agent collaborative reasoning and active evidence acquisition.


<details>
  <summary>Details</summary>
Motivation: Existing methods for urban region profiling are correlation-driven, assume cross-modal consistency, and rely on static pipelines, which limit their robustness.

Method: UrbanAgent uses a multi-agent framework, where each agent represents a data modality, and performs structured collaborative reasoning to address cross-modal inconsistencies.

Result: UrbanAgent achieves an average improvement of 8.1% in R2 on global urban datasets for Carbon emissions, GDP, and Population estimation, and shows strong generalization performance.

Conclusion: UrbanAgent provides a more robust and generalizable approach to urban region profiling by reframing it as a reasoning-driven inference problem.
...
TL;DR: The study explores how AI affects human judgment, particularly the willingness to say 'I don't know'.


<details>
  <summary>Details</summary>
Motivation: The researchers aimed to investigate the impact of AI advice on human judgment, specifically when faced with difficult questions.

Method: Five experiments were conducted with 3132 participants, who were presented with difficult questions and had the option to decline to respond. AI advice was provided, but its accuracy was manipulated to be incorrect.

Result: The study found that having access to AI advice led participants to answer more questions, but with lower accuracy (about a third of the time). Participants' confidence in their answers nearly doubled, despite the decreased accuracy.

Conclusion: The study suggests that AI advice can alter the metacognitive threshold at which people decide whether they know enough to answer, leading to a decrease in the willingness to suspend judgment and an increase in inaccurate answers.
...
TL;DR: The paper introduces Safety Sentry, a guard model that improves the safety of LLM agents by reframing the problem as a per-instance three-way routing decision, outperforming existing baselines in accuracy and safety.


<details>
  <summary>Details</summary>
Motivation: Current safeguard methods for LLM agents have limitations, including a binary view of actions as safe or unsafe, and operating at the granularity of action categories, which can lead to routine interruptions and erode autonomy.

Method: The authors propose Safety Sentry, a lightweight guard model that uses a single decoding call to make a per-instance three-way routing decision over {EXECUTE, ASK, REFUSE}, allowing for a single checkpoint to be re-positioned across deployments of differing risk tolerance without retraining.

Result: Safety Sentry outperforms existing baselines on overall accuracy and safety-related recall, while controlling both directional error rates simultaneously.

Conclusion: The paper concludes that Safety Sentry provides a more effective and nuanced approach to safeguarding LLM agents, improving both safety and autonomy.
...
TL;DR: The MEDA system uses large-language models and symbolic regression to discover ordinary differential equation models of biological systems, achieving strong results in retrieval, extrapolation, and discovery tasks.


<details>
  <summary>Details</summary>
Motivation: Automatic scientific discovery has long been a goal, but most existing approaches focus on narrow benchmarks or broad automation pipelines, with biological systems being underexplored.

Method: The MEDA system is an LLM- and SR-powered framework that retrieves background knowledge, defines variables, generates mechanistic constraints, proposes candidate ODEs, and fits and evaluates them.

Result: MEDA recovered correct state variables, achieved strong structural recovery, and produced biologically plausible models, with ablation analyses showing the importance of knowledge-guided formalization and mechanistic constraints.

Conclusion: The MEDA system demonstrates the potential for automatic scientific discovery in biological systems, highlighting the importance of combining large-language models and symbolic regression with domain knowledge and mechanistic constraints.
...
TL;DR: The paper introduces STOCKTAKE, a benchmark for evaluating LLM agents on a 26-week supply-chain replenishment task with hidden factors. It measures state estimation and control separately, revealing that even top models struggle with the 'knowing-doing gap', failing to act effectively despite correct state estimation.


<details>
  <summary>Details</summary>
Motivation: Existing evaluations for LLM agents fail to distinguish between two types of failures: misreading the world and failing to act despite correct understanding. The authors aim to create a benchmark that can separate these failures and measure state estimation and control independently.

Method: The authors propose STOCKTAKE, a factored partially observable Markov decision process with six hidden factor processes. They use an exact Bayes filter per factor to drive a rollout policy, allowing for a fair reference policy that an agent can be compared against. The benchmark provides a skill score, stated-belief detection lag, and knowing-doing rate to evaluate agent performance.

Result: The authors evaluate four LLM models (Claude Sonnet 5, GPT-5.4, DeepSeek-V4-Pro, and Grok 4.5) on STOCKTAKE and find that they detect 84-88% of hidden failures within a week of onset. However, the models exhibit a wide range of skill scores (0.62 to -0.23), with two models performing below a symptom-blind base-stock floor. The results also show that even when models correctly diagnose stress, 34-43% of weeks still end in stockouts.

Conclusion: The paper concludes that STOCKTAKE provides a comprehensive evaluation of LLM agents on a complex, long-term decision-making task. The results highlight the challenges of the 'knowing-doing gap' and the need for better evaluation methods that can distinguish between different types of failures.
...
TL;DR: The paper introduces a new benchmark for embodied agents to find and follow a person described in language, and proposes a framework to improve performance in this task.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for human following assume the target person is initially visible, which is not realistic. The authors aim to create a unified benchmark for embodied agents to find and follow a language-described target in a dynamic environment.

Method: The authors propose SeekFollow-VLA, a vision-language-action framework with a task-driven routing mechanism for latent phase inference and transition modeling between seeking and following.

Result: SeekFollow-VLA achieves clear improvements over baselines in both single-person and multi-person environments, establishing a baseline for unified embodied seek-and-follow.

Conclusion: The paper presents a new benchmark (UESF-Bench) and a framework (SeekFollow-VLA) to improve embodied agents' ability to find and follow a language-described target, addressing the limitations of existing benchmarks and methods.
...
TL;DR: This paper introduces objective metrics for policy explainability in Reinforcement Learning (RL) and quantifies the explainability of logical rules using Inductive Logic Programming (ILP).


<details>
  <summary>Details</summary>
Motivation: The need for transparent and interpretable RL policies in safety-critical and human-centric scenarios, and the lack of shared evaluation metrics in Explainable Reinforcement Learning (XRL).

Method: Using ILP to extract symbolic representations of RL policies and defining novel explainability metrics, including activation rate, feature coverage, syntactic distance, and semantic distance.

Result: The proposed metrics provide fine-grained insights into domain features, uncover coordination, specialization, and adaptation patterns in Multi-Agent RL (MARL), and offer crucial insights for policy transfer and generalization.

Conclusion: The paper advances the state of the art in XRL by introducing objective metrics for policy explainability and contributes to the field of logic for Explainable Artificial Intelligence (XAI) by providing a principled way to quantify the explainability of logical rules.
...
TL;DR: The study examines the impact of AI agents (bots) on group organization in open-source software development. It finds that bot adoption leads to more repeated collaboration, recognition of bots, fewer conflict cascades, and more distinctive outputs.


<details>
  <summary>Details</summary>
Motivation: The increasing presence of AI agents in human teams raises questions about their impact on group organization. The study aims to investigate this question in the context of open-source software development.

Method: The study analyzes 2,991 GitHub projects over two years before and after the adoption of the first bot. It measures three capabilities (repeated engagement, social memory, and role differentiation) and two outcomes (conflict cascades and output distinctiveness).

Result: The results show that bot adoption is associated with more repeated collaboration, recognition of specific bots, fewer conflict cascades, and more distinctive outputs. The findings suggest that bots can become part of a community's social infrastructure.

Conclusion: The study concludes that predictable, rule-based agents (bots) can become part of a community's social infrastructure, leading to changes in group organization. The findings highlight the potential benefits of integrating bots into human teams.
...
TL;DR: AgentCompass is an open-source infrastructure for evaluating Large Language Model (LLM) based agents, enabling flexible and reproducible assessments.


<details>
  <summary>Details</summary>
Motivation: The need for a unified evaluation infrastructure for LLMs as they evolve into autonomous agents, due to the current fragmented and tightly coupled evaluation pipelines.

Method: AgentCompass is composed of three independent components: Benchmark, Harness, and Environment, allowing for flexible configurations and scalable evaluations.

Result: AgentCompass supports over 20 benchmarks across five capability dimensions and provides a fault-tolerant asynchronous runtime and trajectory analysis tools.

Conclusion: AgentCompass provides a scalable and reproducible infrastructure for advancing agent research, enabling the community to evaluate LLM-based agents in a unified and efficient manner.
...
TL;DR: The paper presents CAVA, a runtime-semantics layer for converting heterogeneous agent activity into canonical runtime action objects, enabling governance and verification of AI system actions.


<details>
  <summary>Details</summary>
Motivation: The increasing use of agentic AI systems with heterogeneous runtimes creates challenges for governance, as a single operational act can be represented by many incompatible runtime records, making it difficult to verify and reproduce actions.

Method: The paper proposes Canonical Action Verification and Attestation (CAVA), a runtime-semantics layer that converts heterogeneous agent activity into canonical runtime action objects, and presents a reference implementation with a 96-seed, 384-variant benchmark.

Result: The results demonstrate the effectiveness of CAVA in achieving semantic equivalence, semantic separation, wrapper bypass, false-positive control, approval binding, receipt reproducibility, attestation tamper detection, runtime portability, and semantic pattern detection.

Conclusion: The paper concludes that CAVA provides a necessary substrate for deployer-side AI governance by enabling action-level canonicalization and policy-addressable semantic patterns.
...
TL;DR: The authors propose Experience Memory Graph (EMG), a framework that helps Large Language Model (LLM) agents recover from failures in complex tasks by reformulating failure recovery as a graph matching problem.


<details>
  <summary>Details</summary>
Motivation: Existing self-correction mechanisms for LLM agents are brittle, time-consuming, and task-specific, making it hard to generalize to new scenarios. The authors aim to address this limitation.

Method: The authors propose EMG, which converts failed exploration trajectories and successful expert trajectories into directed action decision graphs, extracts common subgraphs and graph edit paths, and stores them in a memory graph.

Result: Experiments on ALFWorld and ScienceWorld show that EMG outperforms state-of-the-art reflection baselines in success rate and average reward, while requiring no test-time trial-and-error.

Conclusion: The proposed EMG framework effectively improves the ability of LLM agents to recover from failures in complex tasks, achieving better performance and efficiency than existing methods.
...
TL;DR: The AIMO Interpretability Challenge aims to develop methods for distinguishing robust from spurious reasoning in mathematical language models by providing resources and infrastructure for participants to identify reliable models.


<details>
  <summary>Details</summary>
Motivation: Standard reasoning benchmarks have a limitation: strong final-answer accuracy does not guarantee that a model uses stable reasoning mechanisms, as it may exploit brittle shortcuts.

Method: The competition will provide newly-published math problems, symbolic representations, frontier reasoning models, and assessments of models' adversarial robustness. Participants will develop methods to identify robust models using these resources and computing infrastructure.

Result: The competition aims to create a new, open robustness benchmark and baseline systems for standard benchmarking in mathematical reasoning and interpretability.

Conclusion: The AIMO Interpretability Challenge connects interpretability and generalization research to determine if frontier AI models' decision-making is generalizable and reliable.
...
TL;DR: Developed HealthClaw, an open-source agent architecture for longitudinal personal health management, improving accuracy and privacy in health AI systems.


<details>
  <summary>Details</summary>
Motivation: Current health AI systems treat each request in isolation, lacking longitudinal support for personal health management.

Method: HealthClaw updates support as a person's routines, preferences, measurements, and risks change, separating shared safety rules and medical knowledge from private longitudinal memory.

Result: HealthClaw showed significant improvements in accuracy (45.7% vs 0.2%) and privacy-aware answer quality, with a mean absolute gain of 27.0 percentage points in biomedical tasks.

Conclusion: HealthClaw supports governed, self-evolving memory for longitudinal personal health agents, with potential for improved clinical effectiveness, though prospective evaluation is needed.
...
TL;DR: The paper studies whether optimization gains in agent-optimization methods can be compounded over time. It compares three approaches to agent-harness optimization and finds that only one method, RELAI-VCL, can transfer positively to unseen tasks and continue improving over time.


<details>
  <summary>Details</summary>
Motivation: The motivation of this paper is to investigate whether optimizer-driven gains can compound over time, i.e., whether an agent can be optimized multiple times on new tasks without eroding previous gains.

Method: The paper uses a two-phase continual-learning evaluation built from hard tasks in Terminal-Bench 2.0 to compare three approaches to agent-harness optimization: GEPA, Meta Harness, and RELAI's Verifiable Continual Learning (RELAI-VCL).

Result: The results show that RELAI-VCL is the only method that can transfer positively to unseen tasks and continue improving over time, achieving the highest pass rate at every evaluated stage and the highest lifelong average pass rate overall (76.4%).

Conclusion: The paper concludes that optimization gains can compound over time if regression control is built into the optimization loop, providing an inductive bias against shortcut solutions that fail to generalize.
...
TL;DR: The authors present a comprehensive AI-powered framework for upskilling workers, validated by industry experts and successful learner outcomes.


<details>
  <summary>Details</summary>
Motivation: The need for reskilling and upskilling is increasing, but current frameworks have limitations, such as focusing on single stages and lacking industry validation.

Method: An end-to-end framework applying AI acceleration across five stages: knowledge acquisition, content development, content review and verification, teaching, and assessment development.

Result: The framework was validated by the US National Association of State Boards of Accountancy and led to successful outcomes for learners, including passing a professional exam in a short amount of time.

Conclusion: The proposed framework offers an efficient and effective approach to upskilling workers, with industry validation and successful learner outcomes.
...
TL;DR: Earthquaker-AI is a hybrid educational framework that integrates a conversational AI assistant with a robotics project to enhance earthquake preparedness among primary-school students.


<details>
  <summary>Details</summary>
Motivation: The motivation behind Earthquaker-AI is to enhance earthquake preparedness and conscious action among primary-school students by combining hands-on engagement, information processing, and reflective practice.

Method: The framework uses Lego WeDo2 automation to simulate seismic response, a conversational AI assistant based on Retrieval-Augmented Generation, and rubric-based verbal feedback to support self-regulated learning.

Result: Experimental evaluation shows high groundedness and accuracy, with a low hallucination rate, indicating the effectiveness of Earthquaker-AI in promoting technological literacy, self-regulation, and responsible use of digital systems.

Conclusion: Earthquaker-AI combines robotics, rubrics, and AI to promote early crisis-management skills, technological literacy, and responsible use of digital systems among primary-school students.
...
TL;DR: A new method called Deep Interaction is proposed to improve the correction of errors in large language models' reasoning by allowing direct editing of responses.


<details>
  <summary>Details</summary>
Motivation: Current methods for correcting errors in large language models' reasoning are inefficient and often lead to recurring mistakes.

Method: Deep Interaction, which enables direct editing of original responses to correct errors while preserving accurate steps, and then refines the edited reasoning into a distilled prompt.

Result: The method achieves over a 25% improvement in correction success rate and reduces token usage by approximately 40% on STEM tasks.

Conclusion: The proposed Deep Interaction method significantly enhances the efficiency and effectiveness of correcting reasoning errors in large language models.
...
### [157] [Prospective clinical indication, post-hoc report leakage, and fusion design in multi-image chest radiograph classification: a patient-clustered evaluation](https://arxiv.org/abs/2607.13800)
*Kamran Shahid,Muhammad Munwar Iqbal*

Main category: eess.IV

TL;DR: The study evaluates various models for chest radiograph analysis, comparing image-only, indication-only, and multimodal approaches, and finds that permutation-aware fusion and indication are strongly associated with report-derived targets.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this study is to investigate the combination of multiple images with clinical indications, findings, and impressions in chest radiograph datasets, which are produced at different stages of care.

Method: The study uses Frozen DenseNet-121 and Bio+ClinicalBERT encoders to compare various models, including image-only, indication-only, fixed-order multimodal, random-swap, DeepSets, and SectionGuard-MI models, trained with five seeds and evaluated with 2,000 patient-cluster bootstrap replicates.

Result: The results show that SectionGuard-MI achieved AUROC 0.783 and AUPRC 0.260, while DeepSets had the highest prospective AUROC point estimate (0.787) and random-swap fusion had the highest prospective AUPRC point estimate (0.265). Full report text alone reached AUROC 0.979 and AUPRC 0.836.

Conclusion: The study concludes that prospective indication is strongly associated with report-derived targets, permutation-aware fusion is competitive, and post-hoc report text creates substantial report-label circularity.
