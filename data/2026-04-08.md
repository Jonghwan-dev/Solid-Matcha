...
TL;DR: The paper introduces SafeScreen, a framework for safely retrieving and presenting personalized videos, especially for vulnerable users like children and dementia patients, by enforcing individualized safety constraints.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the risks associated with open-domain video platforms, where engagement-optimized algorithms can expose vulnerable users to inappropriate or harmful content, particularly in child-directed and care settings.

Method: The method used in this paper is SafeScreen, a safety-first video screening framework that integrates three key components: profile-driven extraction of individualized safety criteria, evidence-grounded assessments via adaptive question generation and multimodal VideoRAG analysis, and LLM-based decision-making.

Result: The results demonstrate that SafeScreen prioritizes safety over engagement, diverging from YouTube's engagement-optimized rankings in 80-93% of cases, while maintaining high levels of safety coverage, sensibleness, and groundedness.

Conclusion: The conclusion of this paper is that SafeScreen provides a effective solution for safely retrieving and presenting personalized videos, especially for vulnerable users, by enforcing individualized safety constraints, and it does so in a explainable and real-time manner.
...
TL;DR: A novel smart camera platform for real-time jet flame characterization in industrial settings using optimized AI models on a SoC FPGA.


<details>
  <summary>Details</summary>
Motivation: The lack of real-time solutions for industrial early fire segmentation and characterization.

Method: Implementation of a smart camera platform using a SoC FPGA, optimizing a UNet segmentation model for real-time image processing.

Result: The optimized model achieved 30 FPS with a 7.5x improvement in latency and 125x reduction in parameters, maintaining accuracy.

Conclusion: The proposed platform enables real-time jet flame characterization, reducing latency and improving fire safety management in industrial settings.
...
TL;DR: SpikeVPR, a bio-inspired approach, enables robust visual place recognition under dynamic conditions with low computational and energy demands.


<details>
  <summary>Details</summary>
Motivation: Conventional deep networks for visual place recognition are limited by high computational and energy demands, and there is a need for efficient and robust solutions for autonomous robots.

Method: SpikeVPR combines event-based cameras with spiking neural networks (SNNs) and uses surrogate gradient learning and EventDilation augmentation strategy.

Result: SpikeVPR achieves performance comparable to state-of-the-art deep networks with 50 times fewer parameters and 30-250 times less energy consumption.

Conclusion: Spike-based coding offers an efficient pathway toward robust visual place recognition in complex, changing environments.
...
TL;DR: A new method called 3D-Implicit Depth Emergence is proposed for 3D scene understanding, which enables 3D perception to emerge implicitly within a unified visual representation, eliminating depth and pose dependencies during inference.


<details>
  <summary>Details</summary>
Motivation: Existing methods for leveraging 3D information in Multimodal Large Language Models (MLLMs) struggle with the trade-off in 2D-3D representation fusion, leading to suboptimal deployment.

Method: The proposed method, 3D-Implicit Depth Emergence, uses geometric self-supervision and an Implicit Geometric Emergence Principle to construct an information bottleneck, forcing the model to maximize the mutual information between visual features and 3D structures.

Result: The method surpasses state-of-the-art (SOTA) on multiple 3D scene understanding benchmarks, achieving a 55% reduction in inference latency while maintaining strong performance across diverse downstream tasks.

Conclusion: The proposed method represents a fundamental rethinking of 3D knowledge integration in visual-language models, enabling 3D perception to emerge implicitly and eliminating depth and pose dependencies during inference with zero latency overhead.
...
TL;DR: The paper proposes Cross-Stage Attention Residuals (XAttnRes), a mechanism that enhances feature aggregation across different stages in segmentation networks, improving performance across various datasets and imaging modalities.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to improve feature aggregation in segmentation networks, particularly in the context of Large Language Models (LLMs) and multi-scale encoder-decoder stages.

Method: The paper proposes a novel mechanism called Cross-Stage Attention Residuals (XAttnRes), which maintains a global feature history pool and uses lightweight pseudo-query attention for selective aggregation across different stages.

Result: The results show that XAttnRes consistently improves performance across four datasets and three imaging modalities when added to existing segmentation networks. Notably, XAttnRes alone achieves performance on par with the baseline, even without skip connections.

Conclusion: The paper concludes that XAttnRes is an effective mechanism for enhancing feature aggregation in segmentation networks, and that learned aggregation can recover the inter-stage information flow traditionally provided by predetermined connections.
...
TL;DR: MoViD, a novel 3D human pose estimation framework, addresses challenges in real-world deployment by providing viewpoint-invariant estimation, reducing error by 24.2%, and enabling real-time inference at 15 FPS on edge devices.


<details>
  <summary>Details</summary>
Motivation: Existing 3D human pose estimation methods struggle with viewpoint variations, requiring large amounts of training data and suffering from high inference latency, which hinders their deployment in applications like healthcare monitoring and immersive gaming.

Method: MoViD disentangles viewpoint information from motion features using a view estimator and orthogonal projection module, enhanced by physics-grounded contrastive alignment. It also employs a frame-by-frame inference pipeline with a view-aware strategy for real-time edge deployment.

Result: MoViD reduces pose estimation error by over 24.2% compared to state-of-the-art methods, maintains robust performance under severe occlusions with 60% less training data, and achieves real-time inference at 15 FPS on NVIDIA edge devices.

Conclusion: MoViD offers a robust, efficient, and viewpoint-invariant solution for 3D human pose estimation, enabling its deployment in real-world applications with improved accuracy and efficiency.
...
TL;DR: The paper studies an embedding-only pipeline for onboard systems to prioritize hazards in remote sensing without transmitting raw pixels. It evaluates the pipeline under various shifts and finds that it remains useful, with the optimal decision head being task-dependent.


<details>
  <summary>Details</summary>
Motivation: Downlink bottlenecks in remote sensing systems motivate the need for onboard systems that can prioritize hazards without transmitting raw pixels.

Method: The paper uses a strict setting where a ground station uplinks compact embeddings and metadata, and an onboard system performs vector search to triage new captures. It evaluates the embedding-only pipeline using OlmoEarth embeddings on a scaled public multi-task benchmark.

Result: The results show that all effective methods rely on the same uplinked embeddings, but the optimal decision head is task-dependent. kNN retrieval is superior for cloud classification, while class centroids dominate temporal change detection.

Conclusion: The embedding-only uplink is the key enabler for onboard systems, allowing the system to select the best head per task at no additional uplink cost, with all telemetry under 1 KB per query.
...
TL;DR: The paper identifies a gap in Multimodal Large Language Models' (MLLMs) ability to understand physics, specifically in intuitive physics understanding. It proposes Scene Dynamic Field (SDF), a method that uses physics simulators in a multi-task fine-tuning framework to improve performance.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs struggle with high-level physics reasoning, particularly in understanding the dynamics of continuum objects, which is a crucial aspect of physical reasoning.

Method: The paper introduces two benchmark tasks: Next Frame Selection (NFS) and Temporal Coherence Verification (TCV), and proposes Scene Dynamic Field (SDF), a concise approach that leverages physics simulators within a multi-task fine-tuning framework.

Result: The experiments show that state-of-the-art MLLMs perform poorly on the proposed tasks, but SDF substantially improves performance, achieving up to 20.7% gains on fluid tasks and strong generalization to unseen physical domains.

Conclusion: The work highlights a critical gap in current MLLMs and presents a promising cost-efficient approach for developing more physically grounded MLLMs.
...
TL;DR: HVG-3D is a unified framework for 3D-aware hand-object interaction video synthesis, leveraging explicit 3D representations for improved spatial expressiveness and utilization of synthetic 3D data.


<details>
  <summary>Details</summary>
Motivation: Current methods for hand-object interaction video synthesis rely on 2D control signals, limiting spatial expressiveness and the use of synthetic 3D conditional data.

Method: HVG-3D uses a diffusion-based architecture with a 3D ControlNet, encoding geometric and motion cues from 3D inputs for explicit 3D reasoning during video synthesis.

Result: HVG-3D achieves state-of-the-art spatial fidelity, temporal coherence, and controllability on the TASTE-Rob dataset, enabling effective use of both real and simulated data.

Conclusion: HVG-3D offers a unified framework for 3D-aware hand-object interaction video synthesis, demonstrating improved performance and controllability compared to existing methods.
...
TL;DR: A new image clustering method (IDCL) is proposed, introducing a model training strategy using density information, which improves robustness and performance on complex image data.


<details>
  <summary>Details</summary>
Motivation: Existing image clustering methods rarely consider model learning strategies and rely solely on point-to-point distances, leading to error accumulation.

Method: The proposed IDCL method uses a curriculum learning scheme based on density information and guides cluster assignment using the density core.

Result: The proposed method demonstrates superiority over state-of-the-art clustering approaches in terms of robustness, rapid convergence, and flexibility.

Conclusion: The proposed IDCL method improves the robustness and performance of image clustering by introducing a model training strategy using density information.
...
TL;DR: The paper proposes V-Reflection, a framework that enhances Multimodal Large Language Models (MLLMs) by enabling active visual reflection to reduce perception-related hallucinations in fine-grained tasks.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs are prone to perception-related hallucinations due to their limited ability to dynamically interact with visual input during reasoning.

Method: The proposed V-Reflection framework includes a 'think-then-look' visual reflection mechanism with two-stage distillation strategy: Box-Guided Compression (BCM) and Dynamic Autoregressive Compression (DAC).

Result: Extensive experiments demonstrate the effectiveness of V-Reflection across six perception-intensive benchmarks, significantly narrowing the fine-grained perception gap.

Conclusion: The V-Reflection framework enables MLLMs to actively interrogate visual details and ground their evolving reasoning states, leading to improved performance in fine-grained tasks.
...
TL;DR: The paper presents an edge architecture for detecting standing water in agricultural fields using Raspberry-Pi-class devices, combining camera and environmental sensor data to improve flood-detection performance while reducing energy consumption and maintaining low latency.


<details>
  <summary>Details</summary>
Motivation: Standing water in agricultural fields threatens vehicle mobility and crop health, necessitating a solution for early detection.

Method: The proposed architecture uses a finite-state machine (FSM) to combine camera input and environmental sensor data, with adaptive tiering between local and offloaded inference to balance accuracy, latency, and energy consumption.

Result: The approach improves flood-detection performance over static local baselines, uses less energy than a naive always-heavy offload policy, and maintains bounded tail latency in a real agricultural setting.

Conclusion: The deployed edge architecture effectively detects standing water in agricultural fields, offering a promising solution to mitigate the risks associated with standing water.
...
TL;DR: The paper introduces TreeGaussian, a framework that improves 3D Gaussian Splatting (3DGS) by modeling hierarchical semantic relationships and reducing redundancy in contrastive supervision for better 3D object understanding and segmentation.


<details>
  <summary>Details</summary>
Motivation: Existing 3DGS-based methods struggle to represent hierarchical 3D semantic structures and capture whole-part relationships in complex scenes, and are hindered by dense pairwise comparisons and inconsistent hierarchical labels from 2D priors.

Method: The authors propose a tree-guided cascaded contrastive learning framework (TreeGaussian) that constructs a multi-level object tree for structured learning, and introduces a two-stage cascaded contrastive learning strategy, a Consistent Segmentation Detection (CSD) mechanism, and a graph-based denoising module.

Result: Extensive experiments demonstrate the effectiveness and robustness of TreeGaussian in open-vocabulary 3D object selection, 3D point cloud understanding, and ablation studies.

Conclusion: The paper concludes that TreeGaussian can effectively model hierarchical semantic relationships, reduce redundancy in contrastive supervision, and enhance segmentation consistency and quality for 3D object understanding.
...
TL;DR: A new framework for generating coherent long-range human movements with fluid transitions across different motion domains.


<details>
  <summary>Details</summary>
Motivation: The challenge of generating coherent transitions across semantically distinct motion domains, crucial for applications like dance choreography.

Method: An inference-time optimization framework inspired by diffusion-based stochastic optimal control, using a control-energy objective to regularize transition trajectories of a pretrained diffusion model.

Result: Optimizing the objective at inference time yields transitions with fidelity and temporal coherence.

Conclusion: The proposed framework provides a general approach for controlled long-range human motion generation with explicit transition modeling.
...
TL;DR: A Vision Transformer-based framework called PollutionNet is proposed to accurately assess atmospheric nitrogen dioxide and sulfur dioxide by integrating satellite and ground-level data, achieving state-of-the-art performance and reducing prediction errors.


<details>
  <summary>Details</summary>
Motivation: The accurate assessment of atmospheric nitrogen dioxide and sulfur dioxide is essential for understanding climate-air quality interactions, supporting environmental policy, and protecting public health.

Method: PollutionNet, a Vision Transformer-based framework that integrates Sentinel-5P TROPOMI vertical column density data with ground-level observations, leveraging self-attention mechanisms to capture complex spatiotemporal dependencies.

Result: PollutionNet achieves state-of-the-art performance with RMSE of 6.89 $μ$g/m$^3$ for NO$_2$ and 4.49 $μ$g/m$^3$ for SO$_2$, reducing prediction errors by up to 14% compared to baseline models.

Conclusion: PollutionNet provides a scalable and data-efficient tool for applied climatology, enabling robust pollution assessments in regions with sparse monitoring networks, and highlights the potential of advanced machine learning approaches to enhance climate-related air quality research.
...
TL;DR: A novel hybrid architecture, CardioSAM, is proposed for accurate cardiac segmentation in CMR images, combining the strengths of a foundation model (SAM) with a lightweight, cardiac-specific decoder.


<details>
  <summary>Details</summary>
Motivation: Manual segmentation of cardiac structures in CMR images is time-consuming and prone to inter-observer variability, and existing deep learning models lack boundary precision required for clinical applications.

Method: CardioSAM, a hybrid architecture that combines a frozen SAM encoder with a lightweight, trainable cardiac-specific decoder, introducing a Cardiac-Specific Attention module and a Boundary Refinement Module.

Result: CardioSAM achieves a Dice coefficient of 93.39%, IoU of 87.61%, pixel accuracy of 99.20%, and HD95 of 4.2 mm on the ACDC benchmark, surpassing strong baselines like nnU-Net.

Conclusion: CardioSAM has the potential for reliable and clinically applicable cardiac segmentation, exceeding reported inter-expert agreement levels and demonstrating improved boundary precision.
...
TL;DR: A novel PEFT framework called CoLA is proposed for efficient adaptation of unimodal foundation models to multimodal tasks, achieving better performance than existing methods while maintaining parameter efficiency.


<details>
  <summary>Details</summary>
Motivation: The challenge of adapting foundation models for multimodal tasks, particularly in dual-stream architectures, and the limitation of existing PEFT methods in capturing cross-modal interactions.

Method: Cross-Modal Low-Rank Adaptation (CoLA), a novel PEFT framework that extends LoRA by introducing a dedicated inter-modal adaptation pathway alongside the standard intra-modal one.

Result: CoLA consistently outperforms LORA across various benchmarks, achieving a relative gain of around 3% and 2% on vision-language and audio-visual tasks, respectively, while maintaining parameter efficiency.

Conclusion: CoLA enables effective adaptation of unimodal foundation models to multimodal tasks without interference between modality-specific and cross-modal learning, bridging a key gap in efficient multimodal adaptation.
...
TL;DR: StoryBlender is a 3D storyboard generation framework that achieves inter-shot consistency and explicit editability, two properties that are rarely satisfied simultaneously in current approaches.


<details>
  <summary>Details</summary>
Motivation: The motivation behind StoryBlender is to automate the storyboarding process in visual storytelling for film, animation, and games, while addressing the limitations of current approaches, which often suffer from identity drift and limited geometric control, or require expert-heavy and labor-intensive authoring.

Method: StoryBlender is built on a three-stage pipeline: (1) Semantic-Spatial Grounding, (2) Canonical Asset Materialization, and (3) Spatial-Temporal Dynamics. It uses a hierarchical multi-agent system within a verification loop to iteratively self-correct spatial hallucinations via engine-verified feedback.

Result: Experiments demonstrate that StoryBlender significantly improves consistency and editability over both diffusion-based and 3D-grounded baselines, and produces native 3D scenes that support direct, precise editing of cameras and visual assets while preserving multi-shot continuity.

Conclusion: StoryBlender presents a grounded 3D storyboard generation framework that achieves inter-shot consistency and explicit editability, making it a valuable tool for visual storytelling in film, animation, and games.
...
TL;DR: This paper explores attention sinks in Large Vision-Language Models (LVLM), categorizes them into V-sinks and L-sinks, and proposes Layer-wise Sink Gating (LSG) to balance global reasoning and local evidence.


<details>
  <summary>Details</summary>
Motivation: To investigate the role of attention sinks in LVLM and their impact on performance, specifically whether they are redundant artifacts or essential global priors.

Method: The paper categorizes visual sinks into V-sinks and L-sinks, analyzes their impact on performance, and proposes LSG, a lightweight module that dynamically scales attention contributions of V-sink and other visual tokens.

Result: LSG yields improvements on multimodal benchmarks, effectively balancing global reasoning and precise local evidence, without requiring task-specific supervision or modifying the LVLM backbone.

Conclusion: The study reveals a fundamental performance trade-off associated with attention sinks in LVLM, and LSG offers a simple yet effective solution to leverage the benefits of sinks while mitigating their drawbacks.
...
TL;DR: A new AI approach uses pre-trained models to detect gaze behaviors in collaborative learning without human annotation, achieving high accuracy and stability across different contexts.


<details>
  <summary>Details</summary>
Motivation: The need for an automated method to detect gaze behaviors in collaborative learning that doesn't require large amounts of labeled data and can handle various educational contexts.

Method: The proposed approach utilizes pre-trained models: YOLO11 for person tracking, YOLOE-26 for object detection, and Gaze-LLE for gaze target prediction.

Result: The approach achieves an F1-score of 0.829 in detecting students' gaze behaviors, with strong performance for laptop-directed and peer-directed gaze.

Conclusion: The proposed approach enables the automatic detection of gaze behaviors in collaborative learning with high accuracy and stability, supporting its use in real-world educational environments.
...
TL;DR: EgoMind, a Chain-of-Thought framework, enables geometry-free spatial reasoning in multimodal large language models through Role-Play Caption and Progressive Spatial Analysis.


<details>
  <summary>Details</summary>
Motivation: Existing works on spatial cognition tasks rely on 3D priors or geometric supervision, which are costly and limited. Purely 2D approaches struggle with multi-frame spatial reasoning.

Method: EgoMind uses Role-Play Caption to construct a coherent linguistic scene graph and Progressive Spatial Analysis for task-specific questions.

Result: EgoMind achieves competitive results on VSI-Bench, SPAR-Bench, SITE-Bench, and SPBench with minimal data (5K auto-generated SFT samples and 20K RL samples).

Conclusion: EgoMind demonstrates the effectiveness of linguistic reasoning for spatial cognition and the potential to strengthen spatial reasoning capabilities in multimodal large language models.
...
TL;DR: The paper proposes a multi-task learning approach to improve COVID-19 detection from chest CT scans across multiple centres with varying scanners, protocols, and populations.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning models for COVID-19 detection from chest CT scans struggle when data from multiple centres is used due to bias towards centres with more training data.

Method: The proposed method uses a multi-task learning approach with an EfficientNet-B7 backbone to predict both COVID-19 diagnosis and the originating data centre, with logit-adjusted cross-entropy loss to prevent underrepresented centres from being overlooked.

Result: The method achieves an F1 score of 0.9098 and an AUC-ROC of 0.9647 on a validation set of 308 scans.

Conclusion: The proposed approach improves COVID-19 detection from chest CT scans across multiple centres by learning representations that hold across all centres.
...
TL;DR: VitaTouch, a vision-tactile-language model, is proposed for material-property inference and natural-language attribute description, achieving state-of-the-art performance on various benchmarks.


<details>
  <summary>Details</summary>
Motivation: The need for quality inspection in smart manufacturing to identify intrinsic material and surface properties beyond visible geometry, and the limitations of vision-only methods.

Method: VitaTouch uses modality-specific encoders and a dual Q-Former to extract language-relevant visual and tactile features, which are then aligned with text through contrastive learning.

Result: VitaTouch achieves best performance on HCT and TVL benchmark, with high accuracy on material-property inference and natural-language attribute description tasks, and successful robotic trials.

Conclusion: VitaTouch demonstrates the effectiveness of a property-aware vision-tactile-language model for material-property inference and natural-language attribute description in smart manufacturing.
...
TL;DR: The paper investigates safety-aligned evaluation and optimization for 3D object detection in connected and autonomous vehicles (CAVs), proposing safety-oriented metrics and methods to improve safety-critical detection performance.


<details>
  <summary>Details</summary>
Motivation: The statistical nature of deep learning makes perfect predictions difficult to attain, and standard training objectives and evaluation benchmarks treat all perception errors equally, despite only a subset being safety-critical.

Method: The paper uses safety-oriented metric NDS-USC and safety-aware loss function EC-IoU to evaluate and optimize 3D object detection models across diverse neural network architectures and sensing modalities.

Result: The results show that safety-aligned perception evaluation and optimization can improve safety-critical detection performance, reduce collision rate by nearly 30%, and enhance CAV safety across single-vehicle, cooperative, and end-to-end autonomy settings.

Conclusion: The paper concludes that safety-aligned perception evaluation and optimization offer a practical path toward enhancing CAV safety.
...
TL;DR: Comparative study of 9 surface reconstruction methods for leaf surfaces from 3D point clouds, evaluating accuracy, smoothness, robustness, and computational cost.


<details>
  <summary>Details</summary>
Motivation: Accurate leaf surface reconstruction is crucial for agricultural applications like phenotyping, but real-world plant data is complex and noisy.

Method: Comparative analysis of 9 surface reconstruction methods (parametric, triangulation-based, implicit, and learning-based) on 3 publicly available datasets.

Result: Each method has distinct advantages depending on application and resource constraints, highlighting trade-offs between accuracy, smoothness, robustness, and computational cost.

Conclusion: The study provides practical guidance for selecting surface reconstruction techniques for resource-constrained robotic platforms in agricultural applications.
...
TL;DR: The paper presents CoLoRSMamba, a multimodal architecture for violence detection that combines video and audio inputs, achieving state-of-the-art performance on two datasets.


<details>
  <summary>Details</summary>
Motivation: Violence detection can benefit from audio, but real-world soundscapes can be noisy or weakly related to the visible scene, making it challenging to develop effective multimodal models.

Method: The authors propose CoLoRSMamba, a directional Video to Audio multimodal architecture that couples VideoMamba and AudioMamba through CLS-guided conditional LoRA, and trains the model using binary classification with a symmetric AV-InfoNCE objective.

Result: CoLoRSMamba outperforms representative audio-only, video-only, and multimodal baselines on two datasets, achieving 88.63% accuracy / 86.24% F1-V on NTU-CCTV and 75.77% accuracy / 72.94% F1-V on DVD.

Conclusion: The proposed CoLoRSMamba model offers a favorable accuracy-efficiency tradeoff, surpassing several larger models with fewer parameters and FLOPs, and demonstrates the effectiveness of the proposed multimodal architecture for violence detection.
...
TL;DR: This survey reviews adaptation strategies for extending 2D vision models to 3D analysis, categorizing them into data-centric, architecture-centric, and hybrid methods.


<details>
  <summary>Details</summary>
Motivation: The success of 2D vision models (CNNs, ViTs) has sparked research in 3D analysis, but 3D data's irregular, sparse nature poses a challenge.

Method: The survey provides a unified taxonomy, classifying adaptation strategies into three families: data-centric, architecture-centric, and hybrid methods.

Result: The survey qualitatively analyzes trade-offs between these families concerning computational complexity, reliance on large-scale pre-training, and preservation of geometric inductive biases.

Conclusion: The survey outlines key open challenges and future research directions, including 3D foundation models, self-supervised learning for geometric data, and multi-modal signal integration.
...
TL;DR: The paper discusses the impact of Genotype-by-Environment (GxE) interactions on phenotype predictability and introduces two key models for analyzing these interactions. It also presents RGxEStat, a user-friendly tool for streamlined breeding data analysis.


<details>
  <summary>Details</summary>
Motivation: GxE interactions reduce the predictability of phenotypes in target environments, making in-depth analysis necessary to identify genetic advantages or defects under specific environmental conditions.

Method: The paper introduces significance analysis based on the mixed effect model and stability analysis to investigate GxE interactions. It also presents RGxEStat, a tool that integrates model construction, solution, and visualization.

Result: The authors provide a user-friendly tool, RGxEStat, which eliminates the need for complex programming and accelerates research cycles for breeders and agronomists.

Conclusion: The paper concludes that analyzing GxE interactions can enhance breeding practices by enabling genetic selection, and RGxEStat can facilitate this process by providing a streamlined and user-friendly interface for breeding data analysis.
...
TL;DR: The authors propose a novel approach for monocular depth estimation from a single RGB image, leveraging a multilevel perceptual conditional random field (CRF) model built upon the Swin Transformer backbone, achieving state-of-the-art performance on several datasets.


<details>
  <summary>Details</summary>
Motivation: Monocular depth estimation from a single RGB image is challenging due to inherent scale ambiguity and the absence of explicit geometric cues. Existing approaches rely on complex network architectures, which increases training costs and computational overhead.

Method: The authors propose a multilevel perceptual CRF model built upon the Swin Transformer backbone, incorporating three innovations: (1) adaptive hybrid pyramid feature fusion (HPF) strategy, (2) hierarchical awareness adapter (HA), and (3) fully-connected CRF decoder with dynamic scaling attention.

Result: The proposed method achieves state-of-the-art performance on NYU Depth v2, KITTI, and MatterPort3D datasets, with Abs Rel reduced to 0.088 (−7.4%) and RMSE to 0.316 (−5.4%) on NYU Depth v2, and near-perfect threshold accuracy (δ < 1.253 ≈ 99.8%) on KITTI with only 194M parameters and 21ms inference time.

Conclusion: The proposed approach effectively addresses the limitations of existing methods for monocular depth estimation, providing a more efficient and accurate solution.
...
TL;DR: The paper introduces Additively Compositional Latent Action Model (AC-LAM) for learning structured latent actions from visual transitions, improving embodied AI.


<details>
  <summary>Details</summary>
Motivation: Current methods for latent action learning lack structural priors, leading to entangled and miscalibrated latents.

Method: AC-LAM enforces additive composition structure over short horizons on the latent action space, promoting simple algebraic structure and suppressing non-compositional information.

Result: AC-LAM outperforms state-of-the-art LAMs in learning structured, motion-specific, and displacement-calibrated latent actions, providing stronger supervision for policy learning.

Conclusion: The proposed AC-LAM model effectively learns structured latent actions, enhancing the potential of internet-scale video for embodied AI applications.
...
TL;DR: This survey provides a comprehensive overview of Mixture-of-Experts (MoE) applications in remote sensing, covering fundamental principles, architectural designs, and key applications.


<details>
  <summary>Details</summary>
Motivation: The community lacks a comprehensive review of MoE for remote sensing, and the survey aims to fill this gap.

Method: The survey provides a systematic overview of MoE applications in remote sensing, covering fundamental principles, architectural designs, and key applications.

Result: The survey outlines the current state of MoE applications in remote sensing and identifies future trends to inspire further research and innovation.

Conclusion: The survey provides a comprehensive review of MoE applications in remote sensing, which can inspire further research and innovation in this field.
...
TL;DR: YOLOv11 is analyzed for its architecture and performance, showing improved accuracy and real-time capabilities.


<details>
  <summary>Details</summary>
Motivation: To provide a detailed analysis of YOLOv11, a real-time object detector, and its innovations.

Method: Detailed analysis of YOLOv11's architecture, including backbone, neck, and head components, and comparison with prior YOLO versions.

Result: YOLOv11 achieves superior accuracy without sacrificing real-time capabilities, outperforming prior YOLO versions in mean Average Precision (mAP) and inference speed.

Conclusion: YOLOv11 is suitable for applications in autonomous driving, surveillance, and video analytics, and this work provides a clear reference for future studies.
...
TL;DR: ViBA, a sustainable learning framework, enables continuous online training on unconstrained video streams, improving navigation and localization accuracy by integrating geometric optimization with feature learning.


<details>
  <summary>Details</summary>
Motivation: Existing image keypoint detection and description methods are limited by their reliance on datasets with accurate pose and depth annotations, which hinders scalability and generalization.

Method: ViBA integrates an initial tracking network, depth-based outlier filtering, and differentiable global bundle adjustment to jointly refine camera poses and feature positions.

Result: ViBA outperforms state-of-the-art methods, reducing mean absolute translation error (ATE) by 12-18% and absolute rotation error (ARE) by 5-10%, while maintaining real-time inference speeds.
...
TL;DR: The paper identifies a critical weakness in multi-turn image editing, where iterative edits lead to degradation of image quality, and existing image quality evaluators fail to detect this degradation.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to study the failures of multi-turn image editing in multi-modal agentic systems, specifically the iterative degradation of image quality, and to highlight the limitations of current image quality evaluators.

Method: The authors introduce a comprehensive dataset called Banana100, consisting of 28,000 degraded images generated through 100 iterative editing steps, and evaluate 21 popular no-reference image quality assessment (NR-IQA) metrics.

Result: The results show that image quality evaluators fail to detect the degradation, and none of the 21 NR-IQA metrics consistently assign lower scores to heavily degraded images than to clean ones.

Conclusion: The paper concludes that the dual failures of generators and evaluators may threaten the stability of future model training and the safety of deployed agentic systems, and releases the full code and data to facilitate the development of more robust models.
...
TL;DR: KiToke, a training-free token compression approach, reduces visual tokens in Video LLMs while preserving critical information, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Video LLMs suffer from high inference costs due to the large number of visual tokens.

Method: KiToke estimates token diversity globally using a kernel-based redundancy measure and introduces a lightweight temporal interval construction with interval-aware token merging.

Result: KiToke consistently outperforms existing training-free compression methods on multiple video understanding benchmarks and Video LLM backbones.

Conclusion: KiToke enables more efficient token utilization in Video LLMs by explicitly capturing global redundancy across an entire video.
...
TL;DR: The paper presents a method to transfer robustness to post-training quantization from a donor model to a receiver model, improving the receiver model's robustness by up to 60% without requiring additional training.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to find a low-cost alternative to quantization-aware training (QAT) for deploying models with extremely low-bit precision, as QAT can be costly and not always feasible.

Method: The method involves extracting a 'quantization vector' from a donor task through simple weight-space arithmetic and using it to patch a receiver model, thereby improving its robustness to post-training quantization (PTQ)-induced noise.

Result: The results show that the proposed method can improve the robustness of a receiver model (in this case, Vision Transformer (ViT) models) to PTQ-induced noise by as much as 60% without requiring receiver-side QAT.

Conclusion: The paper concludes that robustness to quantization is a transferable property in weight space and not merely a byproduct of task-specific training, suggesting that quantization robustness can be transferred rather than retrained.
...
TL;DR: This study presents a foundation model-centered workflow for automated monitoring of group-housed nursery pigs, leveraging pre-trained vision-language models and farm-specific adaptation through modular post-processing.


<details>
  <summary>Details</summary>
Motivation: The reliance on supervised learning models in precision livestock farming requires extensive labeled data, repeated retraining, and farm-specific tuning, which can be overcome by utilizing foundation models.

Method: The study uses pre-trained vision-language foundation models (Grounding-DINO and Grounded-SAM2) as general visual backbones and applies modular post-processing for farm-specific adaptation, including temporal tracking logic and long-term tracking pipeline.

Result: The system achieved high performance in detecting and tracking pigs, with over 80% of active tracks fully correct, and maintained stable identities throughout a 132-minute video, with a mean region similarity of 0.83 and MOTA of 0.99.

Conclusion: This work demonstrates the potential of foundation models in precision livestock farming, enabling scalable, label-efficient, and long-duration monitoring in pig production by combining prior knowledge with lightweight, task-specific logic.
...
TL;DR: This paper improves underwater species classification using frozen embeddings from self-supervised vision foundation models by applying Circuit Duplication at inference time.


<details>
  <summary>Details</summary>
Motivation: The high cost of annotation and environmental variation limit the transferability of fully supervised models in automated underwater species classification.

Method: Circuit Duplication, an inference-time method originally proposed for Large Language Models, is applied to frozen DINOv3 embeddings to improve classification without fine-tuning or changing model weights.

Result: Circuit Duplication consistently improves over the standard frozen forward pass, reaching a macro F1 of 0.875 and closing the gap to the fully supervised benchmark to 1.4 points.

Conclusion: The application of Circuit Duplication to computer vision achieves state-of-the-art performance in underwater species classification without gradient-based training.
...
TL;DR: Introduced ExpressEdit, an open-source Photoshop plugin for stylized expression editing, addressing issues with current AI models.


<details>
  <summary>Details</summary>
Motivation: Current AI image editing models introduce global noise and pixel drift, hindering their integration into professional workflows.

Method: Developed ExpressEdit, a Photoshop plugin that synergizes with native operations, using a comprehensive expression database.

Result: ExpressEdit enables fast and artifact-free expression editing, processing in under 3 seconds on a consumer-grade GPU.

Conclusion: ExpressEdit and its accompanying dataset facilitate efficient and high-quality expression editing, promoting artistic exploration.
...
TL;DR: RDFace, a dataset of 456 pediatric facial images for 103 rare genetic conditions, is introduced to aid AI-assisted diagnosis. It enables data-efficient models and improves diagnostic accuracy by up to 13.7% with synthetic augmentation.


<details>
  <summary>Details</summary>
Motivation: The scarcity of curated, ethically sourced facial data and high similarity among phenotypes across different conditions limit progress in using facial phenotypes for rare disease diagnosis.

Method: RDFace dataset creation, benchmarking AI models with cross-validation, and exploring synthetic augmentation with DreamBooth and FastGAN.

Result: Improved diagnostic accuracy by up to 13.7% in ultra-low-data regimes and a report similarity score of 0.84 for phenotype descriptions generated by a vision-language model.

Conclusion: RDFace establishes a benchmark-ready dataset for equitable rare disease AI research and presents a scalable framework for evaluating diagnostic performance and synthetic medical imagery integrity.
...
TL;DR: SpectralSplat is a method for disentangling appearance from geometry in 3D Gaussian Splatting, enabling controllable appearance transfer and temporally consistent relighting.


<details>
  <summary>Details</summary>
Motivation: Current 3D Gaussian Splatting methods entangle scene geometry with transient appearance properties, preventing relighting, appearance transfer, and consistent rendering across varying environmental conditions.

Method: SpectralSplat factors color prediction into an appearance-agnostic base stream and an appearance-conditioned adapted stream, using a shared MLP conditioned on a global appearance embedding derived from DINOv2 features.

Result: SpectralSplat preserves reconstruction quality while enabling controllable appearance transfer and temporally consistent relighting across driving sequences.

Conclusion: SpectralSplat successfully disentangles appearance from geometry, allowing for flexible and realistic rendering of 3D scenes under various environmental conditions.
...
TL;DR: The paper adapts DeepSeek-OCR-2 for molecular optical recognition, proposing a two-stage fine-tuning strategy and achieving competitive results.


<details>
  <summary>Details</summary>
Motivation: Optical Chemical Structure Recognition (OCSR) is crucial for converting 2D molecular diagrams into machine-readable formats, but direct application of Vision-Language Models is challenging.

Method: The authors formulate OCSR as image-conditioned SMILES generation, using a two-stage progressive supervised fine-tuning strategy with LoRA and selective full-parameter fine-tuning.

Result: The fine-tuned model, MolSeek-OCR, achieves competitive exact matching accuracies comparable to the best-performing image-to-sequence model, but remains inferior to state-of-the-art image-to-graph models.

Conclusion: The proposed approach demonstrates competitive capabilities in OCSR, but further refinement techniques do not improve strict sequence-level fidelity for exact SMILES matching.
...
TL;DR: A multimodal framework is proposed for scalable urban tree detection using satellite and Google Street View imagery, reducing annotation costs and improving performance.


<details>
  <summary>Details</summary>
Motivation: The need for precise mapping of urban trees for environmental sustainability and disaster mitigation, and the limitations of traditional field surveys.

Method: A multimodal framework integrating satellite and ground-level imagery, using domain adaptation, semi-supervised learning, active learning, and a hybrid approach.

Result: The hybrid strategy achieved an F1-score of 0.90, a 12% improvement over the baseline model, with error analysis showing reduced false positives and false negatives.

Conclusion: The study highlights the importance of a multimodal approach and guided annotation for scalable, annotation-efficient urban tree mapping to strengthen sustainable city planning.
...
TL;DR: The paper proposes a new task called User Salient Object Detection (UserSOD) which aims to detect salient objects in an image based on a user's proactive needs, rather than just visual stimuli.


<details>
  <summary>Details</summary>
Motivation: Existing salient object detection methods focus on visual stimuli, ignoring users' proactive needs, which can lead to unsatisfactory results and limit downstream tasks.

Method: The paper advocates for a new task, UserSOD, but does not propose a specific method.

Result: No specific results are presented in the abstract.

Conclusion: The paper concludes that detecting salient objects based on user needs is essential and proposes a new task, UserSOD, but notes that a challenge is the lack of datasets for model training and testing.
...
TL;DR: The paper proposes HEDGE, a heterogeneous ensemble for detecting AI-generated images, achieving state-of-the-art performance and robustness on multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: The rapid evolution of generative models and varied real-world distortions make robust detection of AI-generated images challenging, and a single training regime, resolution, or backbone is insufficient to handle all conditions.

Method: The authors propose HEDGE, which introduces complementary detection routes along three axes: diverse training data with strong augmentation, multi-scale feature extraction, and backbone heterogeneity.

Result: HEDGE achieves 4th place in the NTIRE 2026 Robust AI-Generated Image Detection in the Wild Challenge and attains state-of-the-art performance with strong robustness on multiple AIGC image detection benchmarks.

Conclusion: The proposed HEDGE method demonstrates the importance of structured heterogeneity for robust detection of AI-generated images, achieving state-of-the-art performance and robustness.
...
TL;DR: The paper proposes a method to reduce object hallucinations in Large Vision-Language Models (LVLMs) by selectively suppressing tokens that receive low attention during the focus phase of visual information processing.


<details>
  <summary>Details</summary>
Motivation: LVLMs are prone to object hallucinations, and recent approaches to mitigate this issue are often computationally expensive. The authors aim to find a lightweight and efficient solution.

Method: The authors analyze the internal attention dynamics of vision encoders in LVLMs, identifying a three-phase structure of visual information processing. They then propose a method that selectively suppresses tokens receiving low attention during the focus phase using a Determinantal Point Process (DPP).

Result: The proposed approach consistently reduces hallucination metrics while maintaining competitive caption quality across multiple LVLM backbones and decoding strategies. It achieves comparable hallucination mitigation to adversarial uncertainty estimation methods with negligible additional inference latency.

Conclusion: The paper presents a lightweight and efficient method for reducing object hallucinations in LVLMs by selectively suppressing tokens that receive low attention during the focus phase of visual information processing.
...
TL;DR: A LOcal-Global Ensemble framework (LOGER) is proposed for robust deepfake detection, combining global and local branches to capture anomalies at multiple levels.


<details>
  <summary>Details</summary>
Motivation: Deepfake detection is challenging due to various manipulation techniques and real-world degradations, requiring a robust approach that considers both global and local anomalies.

Method: LOGER uses a global branch with multiple vision foundation model backbones and a local branch with patch-level modeling and Multiple Instance Learning, followed by logit-space fusion.

Result: LOGER achieved 2nd place in the NTIRE 2026 Robust Deepfake Detection Challenge and showed strong robustness and generalization on multiple public benchmarks.

Conclusion: The proposed LOGER framework effectively addresses the challenges of deepfake detection by leveraging both global and local anomalies, leading to robust and generalizable results.
...
TL;DR: The authors propose a physics-informed framework for hyperspectral imaging using single-pixel imaging (SPI) at low sampling rates, leveraging untrained neural networks and RGB guidance to improve spatial and spectral details.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning methods for hyperspectral imaging require large-scale pretraining datasets, which are often impractical. The authors aim to overcome this limitation with a data-efficient approach.

Method: The proposed framework consists of three stages: (1) a Regularized Least-Squares method with RGB-derived Grayscale Priors (LS-RGP), (2) an Untrained Hyperspectral Recovery Network (UHRNet), and (3) a Transformer-based Untrained Super-Resolution Network (USRNet).

Result: The framework significantly surpasses state-of-the-art algorithms in reconstruction accuracy and spectral fidelity, and a proof-of-concept experiment validates its practical applicability at a 6.25% sampling rate.

Conclusion: The proposed method provides a robust, data-efficient solution for computational hyperspectral imaging, enabling high-fidelity spatial and spectral details recovery under extremely low sampling rates.
...
TL;DR: The authors propose a new approach to improve human action recognition by augmenting 2D skeleton data with a representation called Scale-Body-Flow (SBF), which captures depth, body contour, and human-object interaction information.


<details>
  <summary>Details</summary>
Motivation: Current 2D skeleton-based human action recognition approaches struggle to capture critical action-related information, such as depth of joints, human body contour, and human-object interaction.

Method: The authors propose a novel representation called Scale-Body-Flow (SBF) and a segmentation network called SFSNet to predict SBF, which is then used to augment 2D skeleton data for human action recognition.

Result: The proposed approach achieves significantly higher human action recognition accuracy compared to state-of-the-art skeleton-only approaches, with similar compactness and efficiency.

Conclusion: The proposed approach effectively addresses the limitations of current 2D skeleton-based human action recognition approaches by capturing critical action-related information, leading to improved recognition accuracy.
...
TL;DR: The paper introduces a stochastic generative PnP framework that leverages score-based diffusion models as priors to improve robustness in solving imaging inverse problems.


<details>
  <summary>Details</summary>
Motivation: The motivation is to systematically use score-based diffusion models as priors within the plug-and-play framework without relying on reverse diffusion sampling, and to improve robustness in severely ill-posed inverse problems.

Method: The method introduced is the stochastic generative PnP (SGPnP) framework, which injects noise to better leverage the expressive generative score-based diffusion model priors.

Result: The results demonstrate consistent improvement over conventional PnP methods and achieve performance competitive with diffusion-based solvers on challenging inverse tasks such as multi-coil MRI reconstruction and large-mask natural image inpainting.

Conclusion: The paper concludes that the proposed SGPnP framework provides a new approach to leveraging score-based diffusion models within the PnP framework, leading to improved robustness and performance in solving imaging inverse problems.
...
TL;DR: The paper introduces PortraitCraft, a unified benchmark for portrait composition understanding and generation, including a dataset of 50,000 curated portraits with structured annotations.


<details>
  <summary>Details</summary>
Motivation: Existing datasets and benchmarks for portrait aesthetics and generation lack a focus on structured portrait composition analysis and controllable generation under explicit composition requirements.

Method: The authors create a dataset with 50,000 curated portraits and structured multi-level supervision, and establish two benchmark tasks for composition understanding and composition-aware generation.

Result: The paper provides a comprehensive benchmark for future research, including standardized evaluation protocols and reference baseline results with representative multimodal models.

Conclusion: PortraitCraft enables systematic research on structured portrait composition analysis and controllable portrait generation, and provides a foundation for fine-grained portrait understanding, interpretable aesthetic assessment, and controllable portrait generation.
...
TL;DR: Proposed TABLeT, a novel approach for modeling long-range spatiotemporal dynamics in fMRI, outperforming existing models with improved computational and memory efficiency.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of high dimensionality of 4D fMRI signals and the limitations of voxel-based models in capturing long temporal windows.

Method: TABLeT tokenizes fMRI volumes using a pre-trained 2D natural image autoencoder and uses a simple Transformer encoder for long-sequence modeling.

Result: TABLeT outperforms existing models in multiple tasks across large-scale benchmarks, with substantial gains in computational and memory efficiency.

Conclusion: TABLeT offers a promising approach for scalable and interpretable spatiotemporal modeling of brain activity.
...
TL;DR: MuPD, a multimodal generative model, integrates histology, molecular, and clinical data for improved disease diagnosis and treatment by synthesizing missing modalities with high fidelity.


<details>
  <summary>Details</summary>
Motivation: The integration of histological, molecular, and clinical data is crucial for accurate disease diagnosis and treatment, but often these modalities are incomplete due to practical limitations.

Method: MuPD uses a diffusion transformer with decoupled cross-modal attention to embed H&E-stained histology, RNA profiles, and clinical text into a shared latent space, pretrained on large-scale multimodal data.

Result: MuPD outperforms task-specific models in cross-modal synthesis tasks, including text-conditioned and image-to-image generation, RNA-conditioned histology generation, and virtual staining, with significant improvements in fidelity and classification accuracy.

Conclusion: MuPD demonstrates the potential of a unified generative model for multimodal histopathology, providing a scalable framework for improved disease diagnosis and treatment.
...
TL;DR: Developed a two-step solution for precise nanoparticle analysis in electron microscopy images, leveraging a self-attention driven U-Net and CycleGAN framework to automate segmentation and generate synthetic training data.


<details>
  <summary>Details</summary>
Motivation: Manual nanoparticle analysis is time-consuming and traditional automated methods have limitations, particularly with complex shapes and imaging artifacts.

Method: A two-step approach: 1) A self-attention driven U-Net segments key nanoparticle features from real images, and 2) The trained U-Net is integrated with a CycleGAN framework to generate realistic synthetic electron microscopy image-mask pairs.

Result: The model accurately detects features in diverse real-world nanoparticle images and autonomously augments the training dataset without human input.

Conclusion: The proposed solution overcomes challenges in nanoparticle analysis by automating segmentation and generating synthetic training data, enabling precise characterization in electron microscopy images.
...
TL;DR: ComPrivDet, a method for detecting privacy objects in compressed video, achieves high accuracy while reducing latency.


<details>
  <summary>Details</summary>
Motivation: The increasing concern about privacy leakage from video data, particularly in large-scale video analytics, necessitates efficient solutions to protect sensitive information.

Method: ComPrivDet detects privacy objects in compressed video by reusing I-frame inference results and skipping or refining P- and B-frame detections based on compressed-domain cues.

Result: ComPrivDet maintains 99.75% accuracy in private face detection and 96.83% in private license plate detection while skipping over 80% of inferences, with 9.84% higher accuracy and 75.95% lower latency than existing methods.

Conclusion: ComPrivDet offers an efficient and accurate solution for detecting privacy objects in compressed video, making it suitable for large-scale video analytics applications.
...
TL;DR: The paper proposes a method called Continuous Softened Retracing reSampling (CSRS) to improve the unsupervised self-evolution of Multimodal Large Language Models (MLLMs).


<details>
  <summary>Details</summary>
Motivation: Existing self-evolution methods rely on majority voting, which may be biased and not guarantee objective correctness. The authors aim to counteract this degradation.

Method: The authors propose CSRS, which includes a Retracing Re-inference Mechanism (RRM), Softened Frequency Reward (SFR), and Visual Semantic Perturbation (VSP).

Result: The experimental results show that CSRS significantly enhances the reasoning performance of Qwen2.5-VL-7B on benchmarks like MathVision, achieving state-of-the-art results in unsupervised self-evolution on geometric tasks.

Conclusion: The proposed CSRS method improves the unsupervised self-evolution of MLLMs by providing more objective and effective feedback signals, leading to better reasoning performance.
...
TL;DR: Proposed an Adaptive Relational Transformer (ART) for accurate prediction of pedestrian trajectories, achieving state-of-the-art accuracy with high efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing methods for predicting pedestrian trajectories either have high computational overhead or struggle to model diverse and time-varying human interactions.

Method: Introduced a Temporal-Aware Relation Graph (TARG) and an Adaptive Interaction Pruning (AIP) mechanism within the ART framework.

Result: ART achieved state-of-the-art accuracy with high computational efficiency on ETH/UCY and NBA benchmarks.

Conclusion: The proposed ART model effectively captures the evolution of pairwise interactions and reduces redundant computations, making it suitable for real-world robot-related applications.
...
TL;DR: A new framework, MASC-Pose, is proposed for efficient 3D human pose estimation from monocular videos, achieving high accuracy and computational efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing methods for 3D human pose estimation face challenges in efficiency and adaptability when modeling spatial and temporal dependencies.

Method: MASC-Pose, a Motion-Adaptive multi-scale temporal modeling framework with Skeleton-Constrained spatial graphs, which includes an Adaptive Multi-scale Temporal Modeling (AMTM) module and a Skeleton-constrained Adaptive GCN (SAGCN).

Result: The proposed method achieves strong accuracy with high computational efficiency on Human3.6M and MPI-INF-3DHP datasets.

Conclusion: The proposed MASC-Pose framework effectively addresses the challenges in 3D human pose estimation, demonstrating its effectiveness and efficiency.
...
TL;DR: DreamPRVR is proposed for Partially Relevant Video Retrieval (PRVR) to address issues with existing methods, achieving better performance through a coarse-to-fine representation learning paradigm.


<details>
  <summary>Details</summary>
Motivation: Existing PRVR methods suffer from incomplete global contextual perception, query ambiguity, and local noise induced by spurious responses.

Method: DreamPRVR adopts a coarse-to-fine representation learning paradigm, generating global contextual semantic registers and then optimizing fine-grained similarity for precise cross-modal matching.

Result: DreamPRVR outperforms state-of-the-art methods in extensive experiments.

Conclusion: The proposed DreamPRVR model effectively addresses the challenges in PRVR, demonstrating improved performance and reliability.
...
TL;DR: LaPR, a label-aware prompt retrieval framework, improves visual in-context learning by incorporating labels into prompt selection, leading to consistent improvements across various tasks.


<details>
  <summary>Details</summary>
Motivation: The choice of demonstrative prompts significantly affects VICL performance, but prior work overlooks labels, leading to visually similar but label-inconsistent prompts.

Method: LaPR uses an image-label joint representation and a mixture-of-expert mechanism with query-adaptive routing to incorporate label cues and handle unavailable query labels.

Result: LaPR achieves promising and consistent improvements on in-context segmentation, detection, and colorization tasks, and generalizes well across feature extractors and cross-fold scenarios.

Conclusion: The study highlights the importance of label utilization in prompt retrieval for VICL, and LaPR offers a effective solution for improving VICL performance.
...
TL;DR: The paper proposes a method to anticipate human-object interactions in Egocentric Vision using Vision Large Language Models (VLLMs), improving visual grounding and user intent understanding.


<details>
  <summary>Details</summary>
Motivation: The ability to anticipate human-object interactions is desirable in intelligent assistive systems to guide users and understand their goals.

Method: The method uses Set-of-Mark prompting to improve visual grounding, trajectory of user's recent gaze fixations to understand user intent, and a novel inverse exponential sampling strategy for input video frames.

Result: The proposed method surpasses state-of-the-art approaches on the HD-EPIC egocentric dataset, demonstrating its model-agnostic nature.

Conclusion: The paper presents a novel approach for human-object interaction anticipation in Egocentric Vision, showing improved performance over existing methods.
...
TL;DR: Proposed a differentiable layer-wise sparsity optimization framework for diffusion transformer models to improve efficiency without degrading sample quality.


<details>
  <summary>Details</summary>
Motivation: Diffusion models' multi-step inference mechanism incurs high computational cost, and previous acceleration methods are inefficient for few-step diffusion transformer models.

Method: A differentiable layer-wise sparsity optimization framework with token caching, learnable network, and dynamic programming solver; two-stage training strategy.

Result: Consistently improves efficiency without degrading sample quality across various diffusion-transformer models; reduces computational cost by 54% on PixArt-$α$ while surpassing original model generation metrics.

Conclusion: The proposed method delivers large efficiency gains while often improving generation quality for diffusion transformer models.
...
TL;DR: A new driving dataset, DSERT-RoLL, is presented, combining stereo event, RGB, thermal cameras, 4D radar, and dual LiDAR for improved 3D detection in various weather and lighting conditions.


<details>
  <summary>Details</summary>
Motivation: The dataset aims to alleviate data scarcity for novel sensors like event cameras and 4D radar, and support systematic studies of their behavior.

Method: The dataset provides precise 2D and 3D bounding boxes, track IDs, and ego vehicle odometry, enabling fair comparisons across sensor combinations. A fusion framework is proposed to integrate sensor-specific cues into a unified feature space.

Result: Baselines are reported for representative single-modality and multimodal methods, and protocols are provided for research on different fusion strategies and sensor combinations.

Conclusion: The DSERT-RoLL dataset and proposed fusion framework can improve 3D detection robustness under varied weather and lighting conditions.
...
TL;DR: The paper investigates fine-tuning foundation models for long-tailed recognition in scientific images and proposes SciLT, a framework that leverages multi-level representations to improve performance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to explore the effectiveness of fine-tuning foundation models in scientific long-tailed recognition, where data distributions differ significantly from natural images.

Method: The method involves investigating purely visual and parameter-efficient fine-tuning (PEFT) paradigms and proposing SciLT, which uses adaptive feature fusion and dual-supervision learning to leverage multi-level representations.

Result: The results show that fine-tuning foundation models yields limited gains, but SciLT consistently outperforms existing methods, achieving balanced performance across head and tail classes.

Conclusion: The conclusion is that SciLT establishes a strong and practical baseline for scientific long-tailed recognition and provides guidance for adapting foundation models to scientific data with substantial domain shifts.
...
TL;DR: The paper exposes a vulnerability in deep learning-based image watermarking, where an adversary can remove the watermark using the Known Original Attack (KOA). It proposes ResGuard, a module that enhances KOA robustness by enforcing image-dependent embedding, improving watermark extraction accuracy from 59.87% to 99.81%.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the vulnerability of deep learning-based image watermarking to intentional manipulations, specifically the Known Original Attack (KOA), where an adversary has access to multiple original-watermarked image pairs.

Method: The method involves proposing a plug-and-play module called ResGuard, which enhances KOA robustness by enforcing image-dependent embedding through a residual specificity enhancement loss and an auxiliary KOA noise layer.

Result: The result is that ResGuard improves average watermark extraction accuracy from 59.87% to 99.81% when integrated into existing frameworks.

Conclusion: The conclusion is that ResGuard effectively addresses the vulnerability of deep learning-based image watermarking to KOA, providing a more robust solution against intentional manipulations.
...
TL;DR: FunFact is a framework for constructing probabilistic 3D scene graphs that captures functional relationships between objects in a scene, improving scene understanding.


<details>
  <summary>Details</summary>
Motivation: Existing methods for 3D scene understanding consider functional relationships between object pairs in isolation, failing to capture scene-wide interdependence.

Method: FunFact builds an object- and part-centric 3D map, proposes semantically plausible functional relations using foundation models, and uses joint probabilistic inference to constrain relations with LLM-derived common-sense priors and geometric priors.

Result: FunFact improves node and relation discovery recall and significantly reduces calibration error for ambiguous relations on SceneFun3D, FunGraph3D, and FunThor datasets.

Conclusion: FunFact enables holistic probabilistic modeling for functional scene understanding, highlighting its benefits for capturing scene-wide interdependence and improving scene understanding.
...
TL;DR: SGTA, a framework for traffic video understanding, combines scene graphs and multi-modal reasoning to achieve competitive accuracy and transparent decision-making.


<details>
  <summary>Details</summary>
Motivation: The motivation is to integrate structured scene representations with multi-modal agents for traffic video understanding, enabling interpretable decision-making for complex video questions.

Method: SGTA constructs a traffic scene graph from roadside videos using detection, tracking, and lane extraction, and adopts ReAct to process interleaved reasoning traces from large language models with tool invocations.

Result: SGTA achieves competitive accuracy across multiple question types on the TUMTraffic VideoQA dataset and provides transparent reasoning steps.

Conclusion: The results highlight the potential of integrating structured scene representations with multi-modal agents for traffic video understanding.
...
TL;DR: The paper introduces VidNum-1.4K, a benchmark for video-based numerical reasoning, and evaluates state-of-the-art Vision-Language Models (VLMs) on it, revealing a significant reasoning gap.


<details>
  <summary>Details</summary>
Motivation: The authors aim to test whether VLMs truly 'understand' real-world dynamics by evaluating their numerical reasoning capabilities in a more complex and diverse setting than existing benchmarks.

Method: The authors propose a comprehensive VideoQA benchmark, VidNum-1.4K, with 1,379 human-annotated video-question pairs, structured into a three-level hierarchy to assess numerical reasoning skills.

Result: The evaluations of state-of-the-art VLMs on VidNum-1.4K show a striking reasoning gap, with the best model achieving only 60% accuracy and others struggling in the 25%-45% range.

Conclusion: The findings indicate that current VLMs lack a stable 'internal world model', and VidNum-1.4K can serve as a demanding diagnostic testbed for the next generation of numerical video intelligence.
...
TL;DR: A new dataset and model for X-ray contraband detection are proposed, improving performance and efficiency in detecting contraband.


<details>
  <summary>Details</summary>
Motivation: Current X-ray contraband detection methods are limited by their reliance on bounding box annotations and lack of pixel-level supervision, leading to poor model generalization and performance.

Method: The authors propose XSeg, a large X-ray contraband segmentation dataset, and Adaptive Point SAM (APSAM), a specialized mask annotation model that improves upon the Segment Anything Model (SAM).

Result: APSAM demonstrates superior performance on XSeg, enabling accurate and efficient annotation with reduced manual labeling effort.

Conclusion: The proposed XSeg dataset and APSAM model can enhance X-ray contraband detection, improving public safety.
...
TL;DR: A graph learning approach is proposed for melanoma detection in dermoscopic images using superpixel ensemble graphs and superpixel hierarchy graphs.


<details>
  <summary>Details</summary>
Motivation: To improve the reliability and flexibility of graph structures in graph signal processing techniques for biomedical image analysis.

Method: The proposed method uses superpixel maps at multiple levels, two edge weight assignment techniques, and graph nodal signals based on texture, geometric, and color superpixel features.

Result: The proposed method achieves an accuracy of 99.00% and an AUC of 99.59% on the ISIC2017 dataset.

Conclusion: The proposed graph learning approach shows promising results for melanoma detection in dermoscopic images.
...
TL;DR: A compact pipeline for high-fidelity hair reconstruction from multi-view images is proposed, reducing storage and rendering costs while maintaining visual quality.


<details>
  <summary>Details</summary>
Motivation: Recent 3D Gaussian Splatting methods achieve realistic results but require millions of primitives, leading to high storage and rendering costs.

Method: The approach clusters hair strands into representative hair cards and groups them into shared texture codebooks, integrating this structure with 3DGS rendering. A generative prior accelerated method is also proposed to reconstruct initial strand geometry.

Result: A 4-fold reduction in strand reconstruction time and comparable rendering performance with over 200x lower memory footprint are achieved.

Conclusion: The proposed pipeline significantly reduces reconstruction time and storage while maintaining comparable visual quality for high-fidelity hair reconstruction from multi-view images.
...
TL;DR: SymphoMotion is a unified framework for controlling camera motion and object dynamics in video generation, outperforming existing methods in visual fidelity, camera controllability, and object-motion accuracy.


<details>
  <summary>Details</summary>
Motivation: Current methods for video generation typically handle only one motion type or rely on ambiguous 2D cues, making it essential to develop a unified motion-control framework.

Method: SymphoMotion features a Camera Trajectory Control mechanism and an Object Dynamics Control mechanism, which jointly govern camera trajectories and object dynamics within a single model.

Result: SymphoMotion significantly outperforms existing methods in visual fidelity, camera controllability, and object-motion accuracy, establishing a new benchmark for unified motion control in video generation.

Conclusion: SymphoMotion provides a unified motion-control framework for coherent and expressive video generation, with a Camera Trajectory Control mechanism and an Object Dynamics Control mechanism, and establishes a new benchmark for unified motion control.
...
TL;DR: The paper introduces PoCo, a method to improve multi-shot video generation with multiple reference characters, especially when they have similar appearances.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of reference confusion in models when generating multi-shot videos with semantically similar reference characters.

Method: The method introduced is PoCo (Position Embedding as a Context Controller), which uses position encoding as additional context control to enable precise token-level matching.

Result: The results show that PoCo improves cross-shot consistency and reference fidelity compared to various baselines in multi-shot video generation.

Conclusion: The conclusion is that PoCo effectively addresses the reference confusion problem and enhances the model's ability to generate videos with reliable character control, even with extremely similar visual traits.
...
TL;DR: A 3D convolutional architecture, SA-DSVN, is proposed for voxel-level segmentation of structural defects in reinforced concrete using cosmic-ray muon tomography, achieving high accuracy and sensitivity.


<details>
  <summary>Details</summary>
Motivation: Conventional reconstruction methods rely solely on muon scattering angles, and the authors aim to jointly process scattering kinematics and secondary electromagnetic shower multiplicities to improve defect segmentation.

Method: The proposed architecture, SA-DSVN, uses independent encoder streams for scattering kinematics and shower multiplicities, fused via cross-attention, and is trained on simulated data generated using Vega, a cloud-native Geant4 simulation framework.

Result: The model achieves 96.3% voxel accuracy, per-defect Dice scores of 0.59-0.81, and 100% volume-level detection sensitivity at 10 ms inference per volume, with the shower multiplicity stream alone accounting for the majority of discriminative power.

Conclusion: The results establish secondary shower multiplicity as a previously unexploited but highly effective feature for learned muon tomographic reconstruction, and the proposed architecture, SA-DSVN, demonstrates improved defect segmentation performance.
...
TL;DR: A new large-scale image captioning benchmark, ICBench, is proposed to address limitations in existing benchmarks, providing a more comprehensive assessment of modern multimodal large language models.


<details>
  <summary>Details</summary>
Motivation: Existing image captioning benchmarks have limited diversity in caption length, lack recent advanced multimodal large language models, and have insufficient human annotations, which can introduce bias and limit model performance assessment.

Method: A new benchmark, ICBench, is created with 40K captions from 10 advanced multimodal large language models on 2K images, covering 12 content categories. Human subjective studies and an automated evaluation metric, ITIScore, are proposed.

Result: Experimental results show strong alignment between the automatic metric and human judgments, as well as robust zero-shot generalization ability on other public captioning datasets.

Conclusion: The proposed ICBench benchmark and ITIScore metric provide a more comprehensive and accurate assessment of modern multimodal large language models for image captioning tasks.
...
TL;DR: M2StyleGS is a real-time 3D styling technique that uses text or images as references to generate style-enhanced novel views.


<details>
  <summary>Details</summary>
Motivation: Conventional 3D style transfer methods are limited by relying on fixed reference images, whereas users in applications like VR/AR prefer more flexible inputs.

Method: M2StyleGS utilizes 3D Gaussian Splatting and multi-modality knowledge refined by CLIP, with precise feature alignment, observation loss, and suppression loss.

Result: M2StyleGS achieves better visual quality and surpasses previous work by up to 32.92% in terms of consistency.

Conclusion: M2StyleGS enables flexible and high-quality 3D style transfer using text or image references.
...
TL;DR: This paper compares the performance of vision-language models (VLMs) and convolutional neural networks (CNNs) for spectrum-related tasks in non-terrestrial network and terrestrial network (NTN-TN) cooperative systems, finding complementary strengths in task-dependent performance.


<details>
  <summary>Details</summary>
Motivation: The adoption of VLMs for wireless network management is increasing, but there is a lack of systematic understanding of where VLMs outperform CNNs for spectrum-related tasks.

Method: The authors introduce SpectrumQA, a benchmark with 108K visual question-answer pairs, and compare the performance of a frozen Qwen2-VL-7B (VLM) and a trained ResNet-18 (CNN) on three NTN-TN scenarios.

Result: The results show that CNNs achieve high accuracy in spatial localization and severity classification, while VLMs excel in semantic reasoning, with chain-of-thought prompting further improving VLM performance.

Conclusion: The findings suggest that CNNs and VLMs should be deployed for different tasks, rather than being treated as substitutes, with CNNs handling spatial localization and VLMs handling semantic spectrum reasoning.
...
TL;DR: The paper presents an automated framework to refine coarse CityGML building models derived from Airborne Laser Scanning (ALS) by integrating high-precision Mobile Laser Scanning (MLS) data, improving facade accuracy in complex urban environments.


<details>
  <summary>Details</summary>
Motivation: Traditional CityGML building models have significant geometric deficiencies, particularly regarding facade accuracy, due to the nadir perspective of airborne sensors, and require refinement to meet increasing demand for high-precision geospatial data.

Method: The proposed framework utilizes the coarse model as a geometric prior, integrates surface matching to identify outdated surfaces, and employs a binary integer optimization to select optimal faces from candidate data, enforcing hard constraints to ensure topological validity.

Result: The approach effectively corrects facade misalignments, reducing the Cloud-to-Mesh RMSE by approximately 36% and achieving centimeter-level alignment, while guaranteeing strictly watertight and manifold geometry.

Conclusion: The proposed framework provides a robust solution for upgrading ALS-derived city models, enabling targeted updates to facade geometry even in complex urban environments, and meeting the demand for high-precision geospatial data.
...
TL;DR: MoScale is a next-scale autoregressive framework for text-conditioned motion generation, achieving state-of-the-art performance with high training efficiency.


<details>
  <summary>Details</summary>
Motivation: Standard next-token prediction in autoregressive models is not well aligned with the temporal structure required for text-conditioned motion generation.

Method: MoScale generates motion hierarchically from coarse to fine temporal resolutions, using cross-scale hierarchical refinement and in-scale temporal refinement.

Result: MoScale achieves state-of-the-art text-to-motion performance, scales effectively with model size, and generalizes zero-shot to diverse motion generation and editing tasks.

Conclusion: MoScale offers a more suitable causal hierarchy for long-range motion structure, improving robustness under limited text-motion data.
...
TL;DR: Proposed HistoFusionNet, a transformer-enhanced architecture for nighttime image dehazing, combining histogram-guided representation learning with frequency-adaptive feature refinement.


<details>
  <summary>Details</summary>
Motivation: Nighttime image dehazing is challenging due to haze, glow, non-uniform illumination, color distortion, and sensor noise, which are different from daytime dehazing.

Method: HistoFusionNet, built on a multi-scale encoder-decoder backbone, uses histogram transformer blocks and a frequency-aware refinement branch to improve dehazing.

Result: The proposed method achieved highly competitive performance on the NTIRE 2026 Nighttime Image Dehazing Challenge benchmark, ranking 1st among 22 teams.

Conclusion: HistoFusionNet is effective in addressing nighttime image dehazing challenges, yielding robust and competitive performance in real nighttime hazy scenes.
...
TL;DR: The paper proposes a method to reduce the computational cost of transformers in vision and language tasks by pruning redundant patches based on the Shannon and Rényi entropy of attention distribution.


<details>
  <summary>Details</summary>
Motivation: The cost of self-attention in transformers grows quadratically with the number of tokens, making it computationally expensive. Patch pruning is proposed to mitigate this cost.

Method: The method uses the Shannon and Rényi entropy of attention distribution to identify informative patches for pruning. Low-entropy patches are kept as important, while high-entropy patches are treated as redundant.

Result: The method reduced computation while preserving accuracy in fine-grained image recognition tasks. Adjusting the pruning policy through the Rényi entropy measure yields further gains and improves the trade-off between accuracy and computation.

Conclusion: The proposed method effectively reduces the computational cost of transformers while preserving accuracy, and the Rényi entropy measure provides a flexible way to adapt to task needs and computational limits.
...
TL;DR: A new benchmark called EyeBench-V2 is proposed to evaluate fundus image enhancement models, addressing the limitations of current evaluation methods and providing actionable insights for future model development.


<details>
  <summary>Details</summary>
Motivation: The evaluation of generative models for fundus image enhancement is challenging due to the limitations of conventional denoising metrics and the lack of unified evaluation protocols, which hinders the development of clinically aligned enhancement models.

Method: The authors introduce EyeBench-V2, which offers a multi-dimensional clinical-alignment through downstream evaluations, expert-guided evaluation design, and actionable insights, enabling fair comparisons between paired and unpaired enhancement methods.

Result: The proposed benchmark provides a rigorous analysis of existing generative models, identifying their limitations and informing the design of next-generation enhancement models, which can guide future advancements in clinically aligned enhancement models.

Conclusion: EyeBench-V2 addresses the gaps in current evaluation methods for fundus image enhancement models, providing a comprehensive benchmark that bridges the gap between model performance and clinical utility.
...
TL;DR: InCaRPose, a Transformer-based architecture, is proposed for robust relative pose prediction between image pairs in camera extrinsic calibration, particularly in constrained and highly distorted environments like in-cabin automotive monitoring.


<details>
  <summary>Details</summary>
Motivation: Precise relative pose estimation in constrained, highly distorted environments, such as in-cabin automotive monitoring (ICAM), remains challenging. Accurate real-world distances are required for safety-relevant perception.

Method: InCaRPose uses a Transformer-based decoder and leverages frozen backbone features such as DINOv3. It is trained exclusively on synthetic data and can generalize to real-world cabin environments.

Result: InCaRPose achieves absolute metric-scale translation within the physically plausible adjustment range of in-cabin camera mounts in a single inference step. It maintains high precision in both rotation and translation, even with limited training data and a ViT-Small backbone.

Conclusion: InCaRPose enables real-time performance for time-critical inference and achieves competitive performance on the public 7-Scenes dataset. It can be used for camera extrinsic calibration in highly distorted environments.
...
TL;DR: A new benchmark called ActivityForensics is introduced to detect manipulated activity in videos, along with a baseline method called Temporal Artifact Diffuser (TADiff).


<details>
  <summary>Details</summary>
Motivation: The emergence of activity-level forgeries in videos that modify human actions to distort event semantics, which undermines media authenticity and public trust.

Method: A large-scale benchmark called ActivityForensics is created, containing over 6K forged video segments, and a baseline method called Temporal Artifact Diffuser (TADiff) is proposed.

Result: The proposed benchmark and baseline method facilitate comprehensive evaluation protocols and benchmarking of state-of-the-art forgery localizers.

Conclusion: The introduction of ActivityForensics and TADiff aims to address the issue of activity-level forgeries in videos and promote future research in this area.
...
TL;DR: Proposed SPARK-IL, a retrieval-augmented framework for detecting AI-generated images across different generators with 94.6% mean accuracy.


<details>
  <summary>Details</summary>
Motivation: Detecting AI-generated images is challenging due to the failure of detectors to generalize to unseen models, despite pixel-level artifacts varying across models.

Method: SPARK-IL combines dual-path spectral analysis with incremental learning, using a partially frozen ViT-L/14 encoder and Kolmogorov-Arnold Networks (KAN) with mixture-of-experts.

Result: SPARK-IL achieves a 94.6% mean accuracy on the UniversalFakeDetect benchmark across 19 generative models.

Conclusion: The proposed framework provides a promising foundation for cross-generator detection of AI-generated images.
...
TL;DR: The paper proposes a task-guided multi-annotation triplet loss method that dynamically selects informative triplets across tasks, improving multi-task learning performance.


<details>
  <summary>Details</summary>
Motivation: Static weighting in multi-task triplet loss methods requires tuning and doesn't account for task interactions, motivating the need for a dynamic approach.

Method: The proposed method uses a mutual-information criteria to select triplets that are most informative across tasks, modifying sample influence on the representation.

Result: Experiments on an aerial wildlife dataset show improved classification and regression performance with the proposed task-guided selection method.

Conclusion: The task-guided multi-annotation triplet loss method produces a more effective shared representation for downstream tasks, outperforming static weighting approaches.
...
TL;DR: The paper introduces an annotation-guided feature augmentation framework for object detection, which improves transfer, interpretability, and robustness by aligning features with annotation geometry.


<details>
  <summary>Details</summary>
Motivation: Modern object detectors optimize end task loss but often capture shortcut correlations that fail to reflect underlying annotation structure, limiting transfer, interpretability, and robustness.

Method: The framework constructs dense spatial feature grids from annotation-guided latent spaces and fuses them with feature pyramid representations to influence region proposal and detection heads.

Result: Experiments show consistent improvements in object focus, reduced background sensitivity, and stronger generalization to unseen or weakly supervised tasks.

Conclusion: Aligning features with annotation geometry yields more meaningful representations than purely task optimized features, leading to improved object detection performance and robustness.
...
TL;DR: A semi-supervised knowledge distillation framework is proposed to compress pre-trained vision foundation models into compact experts for instance segmentation, achieving improved performance on Cityscapes and ADE20K benchmarks.


<details>
  <summary>Details</summary>
Motivation: Foundation models are computationally heavy and require costly annotations for adaptation. The proposed framework aims to compress these models using limited labeled and abundant unlabeled data.

Method: The framework consists of three stages: domain adaptation via self-training with contrastive calibration, knowledge transfer through a unified multi-objective loss, and student refinement to mitigate residual pseudo-label bias. An instance-aware pixel-wise contrastive loss is used to fuse mask and class scores.

Result: The proposed framework achieves improved performance on Cityscapes and ADE20K benchmarks, with the student model outperforming its zero-shot teacher by +11.9 and +8.6 AP, and surpassing adapted teachers by +3.4 and +1.5 AP.

Conclusion: The proposed semi-supervised knowledge distillation framework effectively compresses pre-trained vision foundation models into compact experts for instance segmentation, achieving state-of-the-art performance on benchmarks.
...
TL;DR: The authors propose a test-time optimization framework for multiview Transformers that incorporates priors to improve 3D vision tasks without retraining or modifying pre-trained models.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve 3D vision tasks by incorporating priors (e.g., camera poses, intrinsics, and depth) into multiview Transformers without requiring retraining or modifying pre-trained image-only networks.

Method: The method involves casting priors as constraints on predictions and optimizing the network at inference time using a self-supervised objective and prior penalty terms.

Result: The method consistently improves performance over base multiview Transformers by a large margin across various 3D vision benchmarks, including point map estimation and camera pose estimation.

Conclusion: The proposed test-time constrained optimization framework effectively incorporates priors into 3D vision tasks, outperforming retrained prior-aware feed-forward methods and reducing point-map distance error by more than half compared with base image-only models.
...
TL;DR: The paper studies Sparse Autoencoders (SAEs) for video representations, proposing spatio-temporal contrastive objectives and Matryoshka hierarchical grouping to improve temporal coherence and performance in action classification and text-video retrieval.


<details>
  <summary>Details</summary>
Motivation: Standard SAEs decompose video into interpretable features but destroy temporal coherence, which is crucial for video understanding tasks.

Method: The authors propose spatio-temporal contrastive objectives and Matryoshka hierarchical grouping to recover and exceed raw temporal coherence, and conduct a systematic ablation on two backbones and two datasets.

Result: The proposed contrastive SAE features improve action classification by +3.9% over raw features and text-video retrieval by up to 2.8xR@1, and causal ablation confirms that contrastive training concentrates predictive signal into a small number of identifiable features.

Conclusion: The study demonstrates the effectiveness of spatio-temporal contrastive objectives and Matryoshka hierarchical grouping in improving temporal coherence and performance in video understanding tasks, and highlights the importance of considering different goals and configurations for SAEs.
...
TL;DR: Proposed SafeCtrl, a framework for safe text-to-image generation that detects and suppresses harmful content while preserving context.


<details>
  <summary>Details</summary>
Motivation: The generation of visually harmful content by text-to-image diffusion models poses significant challenges, and existing safety measures have limitations.

Method: SafeCtrl uses a Detect-Then-Suppress paradigm with an attention-guided Detect module and a localized Suppress module optimized via Direct Preference Optimization (DPO).

Result: SafeCtrl achieves a superior trade-off between safety and fidelity compared to state-of-the-art methods and exhibits improved resilience against adversarial prompt attacks.

Conclusion: SafeCtrl offers a precise and robust solution for responsible generation, effectively transforming unsafe objects into safe alternatives while preserving context.
...
TL;DR: The paper proposes Cross-Modal Graphical Lasso (CM-GLasso), a method for learning interpretable multimodal representations by aligning visual and linguistic features into a shared latent space and introducing a cross-attention distillation mechanism.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome the limitations of sparse graph estimation techniques, such as Graphical Lasso (GLasso), in visual-linguistic domains, which are hindered by high-dimensional noise, modality misalignment, and confounding of shared versus category-specific topologies.

Method: The method proposed is Cross-Modal Graphical Lasso (CM-GLasso), which couples a novel text-visualization strategy with a unified vision-language encoder, and introduces a cross-attention distillation mechanism and unifies tailored GLasso estimation and Common-Specific Structure Learning (CSSL) into a joint objective optimized via the Alternating Direction Method of Multiplier (ADMM).

Result: The results show that CM-GLasso establishes a new state-of-the-art in generative classification and dense semantic segmentation tasks across eight benchmarks covering both natural and medical domains.

Conclusion: The conclusion is that CM-GLasso effectively overcomes the fundamental limitations of sparse graph estimation techniques in visual-linguistic domains and achieves superior performance in multimodal representation learning tasks.
...
TL;DR: The paper proposes VLA-Forget, a hybrid framework for unlearning in Vision-language-action (VLA) models, which effectively removes unwanted behaviors while preserving perception, language grounding, and action control.


<details>
  <summary>Details</summary>
Motivation: The deployment of VLA models in robotic manipulation introduces a new challenge of unlearning unsafe, spurious, or privacy-sensitive behaviors without degrading performance.

Method: The proposed VLA-Forget framework combines ratio-aware selective editing and layer-selective reasoning/action unlearning, jointly optimizing targeted forgetting, perceptual preservation, and reasoning retention.

Result: VLA-Forget improves forgetting efficacy by 10%, preserves perceptual specificity by 22%, retains reasoning and task success by 9%, and reduces post-quantization recovery by 55% relative to strong unlearning baselines.

Conclusion: The proposed VLA-Forget framework effectively addresses the unlearning challenge in VLA models, enabling the removal of unwanted behaviors while preserving performance.
...
TL;DR: A new network called hierarchical point-patch anomaly scoring network is proposed for 3D shape anomaly detection, which outperforms existing methods on public and industrial datasets.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning approaches for 3D shape anomaly detection fail to generalize across diverse anomaly types and scales, and are sensitive to noisy or incomplete local points during training.

Method: A hierarchical point-patch anomaly scoring network that jointly models regional part features and local point features, with an adaptive patchification module that integrates self-supervised decomposition.

Result: The proposed method shows superior AUC-ROC and AUC-PR performance on public and industrial datasets, with over 40% point-level improvement on the new industrial anomaly type and average object-level gains of 7% on Real3D-AD and 4% on Anomaly-ShapeNet.

Conclusion: The proposed method demonstrates strong robustness and generalization for 3D shape anomaly detection, and can effectively detect various types of anomalies in industrial inspection and geometric analysis.
...
TL;DR: The paper proposes Gram-Anchored Prompt Learning (GAPL), a framework that uses second-order statistics to adapt Vision-Language Models to downstream tasks, improving robustness to domain shifts and local noise.


<details>
  <summary>Details</summary>
Motivation: Existing approaches to adapting Vision-Language Models rely solely on first-order visual features, which are insufficient for robust adaptation due to susceptibility to domain shifts and local noise.

Method: The proposed framework introduces a second-order statistical stream via Gram matrices, which augments the standard first-order spatial interaction and enables language representations to dynamically adapt to statistical distribution shifts.

Result: Extensive experiments show the effectiveness of the second-order features and compelling performances of GAPL on various benchmarks.

Conclusion: The paper concludes that GAPL, which uses second-order statistics, is a more robust approach to adapting Vision-Language Models to downstream tasks.
...
TL;DR: This paper presents a unified framework called Hybrid Mask-Aware Transformer (HMAT) for high-fidelity restoration of ancient murals, integrating local texture modeling and long-range structural inference.


<details>
  <summary>Details</summary>
Motivation: Ancient murals are valuable cultural artifacts that have suffered degradation due to environmental exposure, material aging, and human activity, and restoring them requires both reconstructing large missing structures and preserving authentic regions.

Method: The proposed HMAT framework combines Mask-Aware Dynamic Filtering, a Transformer bottleneck, a mask-conditional style fusion module, and a Teacher-Forcing Decoder with hard-gated skip connections.

Result: Experimental results on the DHMural and Nine-Colored Deer datasets demonstrate that HMAT achieves competitive performance compared to state-of-the-art approaches, producing more structurally coherent and visually faithful restorations.

Conclusion: The findings suggest that HMAT provides an effective solution for the digital restoration of cultural heritage murals.
...
TL;DR: This paper studies the vulnerability of audio-visual multi-modal large language models (MLLMs) to typographic attacks across multiple modalities, revealing that coordinated multi-modal attacks pose a significant threat.


<details>
  <summary>Details</summary>
Motivation: Understanding the vulnerabilities of MLLMs is crucial as they are increasingly deployed in safety-critical applications.

Method: The study introduces Multi-Modal Typography, analyzing interactions between audio, visual, and text perturbations across multiple frontier MLLMs, tasks, and benchmarks.

Result: Coordinated multi-modal attacks achieve an 83.43% attack success rate, compared to 34.93% for single-modality attacks, highlighting the cross-modal fragility of MLLMs.

Conclusion: The findings establish multi-modal typography as a critical and underexplored attack strategy in multi-modal reasoning, emphasizing the need for further research in this area.
...
### [98] [MinerU2.5-Pro: Pushing the Limits of Data-Centric Document Parsing at Scale](https://arxiv.org/abs/2604.04771)
...
TL;DR: The paper presents a data engineering approach, minerupro, that improves document parsing performance solely through data optimization and training strategy, achieving SOTA results without architectural changes.


<details>
  <summary>Details</summary>
Motivation: Current document parsing methods focus on model architecture innovation, but the authors argue that the performance bottleneck lies in deficiencies in training data, not architecture.

Method: The authors propose a Data Engine with three components: Diversity-and-Difficulty-Aware Sampling, Cross-Model Consistency Verification, and Judge-and-Refine pipeline, along with a three-stage progressive training strategy.

Result: On OmniDocBench~v1.6, minerupro achieves 95.69, improving over the same-architecture baseline by 2.71 points and surpassing all existing methods, including models with over 200$	imes$ more parameters.

Conclusion: The paper concludes that data engineering and training strategy optimization can significantly improve document parsing performance, even without architectural modifications, and establishes a new SOTA.
...
### [99] [OASIC: Occlusion-Agnostic and Severity-Informed Classification](https://arxiv.org/abs/2604.04012)
*Kay Gijzen,Gertjan J. Burghouts,Daniël M. Pelt*

Main category: cs.CV

TL;DR: The paper proposes OASIC, a severity-informed classification model that estimates occlusion severity, masks occluders, and selects the best model for improved performance on occluded images.


<details>
  <summary>Details</summary>
Motivation: Severe occlusions pose a major challenge for computer vision, causing loss of visible information and distracting patterns from occluders.

Method: The approach involves masking distracting patterns at test-time and masking random parts of objects during training. It estimates occlusion severity at test-time and selects the best model based on that severity.

Result: The proposed approach improves AUC_occ by +18.5 over standard training on occluded images and +23.7 over fine-tuning on unoccluded images.

Conclusion: The OASIC model effectively addresses severe occlusions in computer vision by estimating occlusion severity and adapting model selection, leading to improved performance.
...
TL;DR: The paper proposes HOIGS, a method for reconstructing dynamic scenes with complex human-object interactions, outperforming existing methods by explicitly modeling interactions.


<details>
  <summary>Details</summary>
Motivation: Existing methods for reconstructing dynamic scenes with human-object interactions have limitations, such as relying on human pose priors or approximating all motions within a single field.

Method: The paper proposes Human-Object Interaction Gaussian Splatting (HOIGS), which uses a cross-attention-based HOI module and distinct deformation baselines (HexPlane for humans and Cubic Hermite Spline for objects).

Result: The method consistently outperforms state-of-the-art human-centric and 4D Gaussian approaches on multiple datasets, improving deformation estimation in scenarios with occlusion, contact, and object manipulation.

Conclusion: The paper concludes that explicitly modeling human-object interactions is crucial for high-fidelity reconstruction of dynamic scenes.
...
TL;DR: A new method called 1.x-Distill is proposed for efficient text-to-image generation, achieving high quality and diversity with fewer steps.


<details>
  <summary>Details</summary>
Motivation: Diffusion models are computationally expensive due to their iterative denoising process. Existing few-step distillation methods suffer from diversity collapse and fidelity degradation when reduced to two steps or fewer.

Method: The 1.x-Distill framework, which allows for fractional-step distillation, and includes modifications to Distribution Matching Distillation (DMD), Stagewise Focused Distillation, and a lightweight compensation module for Distill--Cache co-Training.

Result: Experiments show that 1.x-Distill surpasses prior few-step methods, achieving better quality and diversity at 1.67 and 1.74 effective NFEs, with up to 33x speedup over original 28x2 NFE sampling.

Conclusion: The proposed 1.x-Distill framework establishes 1.x-step generation as a practical regime for distilled diffusion models, achieving high-quality text-to-image results with fewer steps.
...
### [102] [Vero: An Open RL Recipe for General Visual Reasoning](https://arxiv.org/abs/2604.04917)
*Gabriel Sarch,Linrong Cai,Qunzhong Wang,Haoyang Wu,Danqi Chen,Zhuang Liu*

Main category: cs.CV

TL;DR: The paper introduces Vero, an open-source vision-language model (VLM) that achieves state-of-the-art performance in diverse visual reasoning tasks, rivaling proprietary models.


<details>
  <summary>Details</summary>
Motivation: The authors aim to create a visual reasoner that works across various tasks, such as charts, science, spatial understanding, and open-ended tasks, without relying on proprietary reinforcement learning (RL) pipelines.

Method: The authors propose Vero, a family of fully open VLMs, and introduce Vero-600K, a 600K-sample dataset from 59 datasets, along with task-routed rewards that handle heterogeneous answer formats.

Result: Vero achieves state-of-the-art performance, improving over four base models by 3.7-5.5 points on average across 30 challenging benchmarks, and outperforms existing RL datasets across task categories.

Conclusion: The authors conclude that broad data coverage is the primary driver of strong RL scaling, and that different task categories elicit qualitatively distinct reasoning patterns that transfer poorly in isolation.
...
### [103] [ATSS: Detecting AI-Generated Videos via Anomalous Temporal Self-Similarity](https://arxiv.org/abs/2604.04029)
*Hang Wang,Chao Shen,Lei Zhang,Zhi-Qi Cheng*

Main category: cs.CV

TL;DR: A novel method called Anomalous Temporal Self-Similarity (ATSS) is proposed to detect AI-generated videos (AIGVs) by identifying unnatural temporal patterns.


<details>
  <summary>Details</summary>
Motivation: Existing AIGV detectors fail to capture the underlying generative logic of AIGVs, limiting their performance. The authors aim to address this by exploiting the distinctive fingerprint of AIGVs, termed anomalous temporal self-similarity (ATSS).

Method: The ATSS method uses a multimodal detection framework with a triple-similarity representation and a cross-attentive fusion mechanism to quantify temporal anomalies in AIGVs.

Result: ATSS significantly outperforms state-of-the-art methods on four large-scale benchmarks (GenVideo, EvalCrafter, VideoPhy, and VidProM) in terms of AP, AUC, and ACC metrics.

Conclusion: The proposed ATSS method effectively detects AI-generated videos by identifying anomalous temporal self-similarity, demonstrating superior generalization across diverse video generation models.
...
TL;DR: TORA, a topology-first representation alignment framework, improves 3D shape assembly by distilling relational structure from a pretrained 3D encoder into the flow-matching backbone, yielding faster convergence, improved accuracy, and robustness.


<details>
  <summary>Details</summary>
Motivation: Flow-matching methods for 3D shape assembly lack explicit guidance on cross-part interactions, and the authors aim to address this limitation.

Method: The authors propose TORA, which uses token-wise cosine matching and Centered Kernel Alignment (CKA) loss to align the student and teacher representations.

Result: TORA achieves state-of-the-art performance on five benchmarks, with up to 6.9 times faster convergence, improved accuracy, and greater robustness under domain shift.

Conclusion: TORA effectively improves 3D shape assembly by leveraging relational structure from a pretrained 3D encoder, with benefits including faster convergence, improved accuracy, and robustness.
...
TL;DR: DINO-VO is an end-to-end monocular visual odometry system with strong scene generalization, outperforming current VO systems by integrating a differentiable adaptive patch selector and multi-task feature extraction.


<details>
  <summary>Details</summary>
Motivation: Current VO systems rely on heuristic feature extraction strategies, which can degrade accuracy and robustness, particularly in large-scale outdoor environments.

Method: DINO-VO incorporates a differentiable adaptive patch selector, a multi-task feature extraction module, and a differentiable bundle adjustment (BA) module with inverse depth priors.

Result: DINO-VO achieves state-of-the-art tracking accuracy across diverse datasets, including TartanAir, KITTI, Euroc, and TUM.

Conclusion: DINO-VO presents a robust and accurate monocular visual odometry system with strong scene generalization, bridging the gap between feature learning and state estimation.
...
TL;DR: The paper proposes a novel framework called 4C4D that enables high-fidelity 4D Gaussian Splatting from video captures of extremely sparse cameras, achieving superior performance over prior art.


<details>
  <summary>Details</summary>
Motivation: The challenge of recovering 4D dynamic scenes from videos captured by a few portable cameras, addressing the limitation of previous works that require dense multi-view captures.

Method: The 4C4D framework, which introduces a Neural Decaying Function on Gaussian opacities to enhance geometric modeling capability of 4D Gaussians.

Result: Superior performance over prior art on sparse-view datasets with varying camera overlaps.

Conclusion: The proposed 4C4D framework effectively addresses the challenge of recovering 4D dynamic scenes from sparse camera captures, enabling high-fidelity 4D Gaussian Splatting.
...
TL;DR: The paper proposes a Positive-Unlabeled (PU) learning approach for curator-in-the-loop duplicate discovery in a repository, achieving high F1 and AUROC scores on CIFAR-10 and AtticPOT datasets.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve curator-in-the-loop duplicate discovery in a repository by formulating it as a PU learning problem, which can effectively identify duplicates without explicit negatives.

Method: The method uses a lightweight per-query Clone Encoder trained on augmented views of a single anchor per artefact, scoring the unlabeled repository with an interpretable threshold on the latent l_2 norm to propose candidates for curator verification.

Result: The results show F1=96.37 (AUROC=97.97) on CIFAR-10 and F1=90.79 (AUROC=98.99) on AtticPOT, improving F1 by +7.70 points over the best baseline (SVDD) under the same lightweight backbone.

Conclusion: The proposed method effectively identifies duplicates in a repository, offering a transparent operating point and suitability for de-duplication, record linkage, and curator-in-the-loop workflows.
...
TL;DR: The paper presents an offline, real-time traffic monitoring system using computer vision and deep learning for vehicle detection and counting.


<details>
  <summary>Details</summary>
Motivation: To develop a traffic monitoring system that leverages advancements in computer vision and deep learning, enabling efficient and accurate vehicle detection and counting in various scenes.

Method: The system uses a pre-trained YOLOv11 detector with BoT-SORT/ByteTrack for multi-object tracking, implemented in PyTorch/OpenCV and wrapped in a Qt-based desktop UI.

Result: The system achieves a counting accuracy of 66.67-95.83% across diverse scenes, with high precision and recall for car and truck detection, resulting in F1 scores of 0.90-1.00 for cars and 0.82-1.00 for trucks.

Conclusion: The paper contributes to the development of future smart cities by demonstrating the capacity of AI-driven traffic monitoring systems, integrating lightweight models with an accessible, cloud-independent interface.
...
TL;DR: A novel deepfake detection framework called Localized Artifact Attention X (LAA-X) is proposed, which is robust to high-quality forgeries and can generalize to unseen manipulations.


<details>
  <summary>Details</summary>
Motivation: Existing deepfake detection approaches often fail to generalize beyond known manipulations, and the authors aim to address this limitation.

Method: The proposed framework, LAA-X, uses a multi-task learning framework with explicit attention strategy and blending-based data synthesis, and is compatible with both CNN and transformer backbones.

Result: LAA-X competes with state-of-the-art methods across multiple benchmarks, despite being trained only on real and pseudo-fake samples.

Conclusion: The proposed LAA-X framework provides a robust and generalizable approach to deepfake detection, and its code and pre-trained weights are publicly available.
...
TL;DR: A physics-informed digital twin framework combined with a stacked ensemble is proposed for multimodal forecasting of Core Body Temperature (CBT) in dairy cattle to predict heat stress.


<details>
  <summary>Details</summary>
Motivation: The study aims to ensure animal welfare and optimize farm management by accurately predicting heat stress in dairy cattle.

Method: The proposed framework integrates an ordinary differential equation (ODE)-based thermoregulation model, a Gaussian process, a Kalman filter, and a behavioral Markov chain, with a three-stage stacked ensemble for CBT forecasting.

Result: The framework achieves a cross-validated R2 of 0.783, F1 score of 84.25%, and PICP of 92.38% for 2-hour ahead forecasting.

Conclusion: The proposed framework provides a robust, uncertainty-aware, and physically principled system for early heat stress detection and precision livestock management.
...
TL;DR: The authors propose Hypothesis Graph Refinement (HGR), a framework for embodied agents to navigate partially observed environments while maintaining reliable long-horizon memory. HGR uses semantic predictions to guide exploration and includes a verification-driven cascade correction mechanism to retract errors.


<details>
  <summary>Details</summary>
Motivation: Existing graph-based navigation systems treat unexplored regions as semantically unknown, leading to inefficient frontier search. Vision-language models can predict frontier semantics but may embed errors into memory.

Method: The HGR framework consists of a semantic hypothesis module, which estimates context-conditioned semantic distributions over frontiers, and a verification-driven cascade correction mechanism, which compares on-site observations against predicted semantics and retracts refuted nodes.

Result: HGR achieves a 72.41% success rate and 56.22% SPL on GOAT-Bench and shows consistent improvements on A-EQA and EM-EQA benchmarks. Diagnostic analysis reveals that cascade correction eliminates approximately 20% of structurally redundant hypothesis nodes and reduces revisits to erroneous regions by 4.5x.

Conclusion: The proposed HGR framework effectively leverages semantic predictions for directed exploration while systematically retracting errors, leading to improved performance and reliability in long-horizon navigation tasks.
...
TL;DR: A novel ship detection framework called SARES-DEIM is proposed for Synthetic Aperture Radar (SAR) imagery, addressing challenges such as speckle noise and complex clutter.


<details>
  <summary>Details</summary>
Motivation: Conventional detectors have limited robustness against SAR-specific degradation and lose fine-grained ship signatures during spatial downsampling.

Method: The proposed framework utilizes a DETR paradigm with a SAR-aware Expert Selection Mixture-of-Experts (SARESMoE) module and a Space-to-Depth Enhancement Pyramid (SDEP) neck.

Result: The model achieves a mAP50:95 of 76.4% and a mAP50 of 93.8% on the HRSID dataset, outperforming state-of-the-art detectors.

Conclusion: The proposed SARES-DEIM framework demonstrates superiority in ship detection on two benchmark datasets, particularly in challenging scenarios.
...
TL;DR: VoxelFM, a 3D CT foundation model, learns robust visual representations through self-distillation without language supervision, outperforming existing models in various clinically relevant tasks.


<details>
  <summary>Details</summary>
Motivation: The need for efficient transfer of visual representations to new tasks with minimal labelled data and without backbone fine-tuning in CT foundation models.

Method: VoxelFM uses self-distillation with the DINO framework to learn semantically rich features without language supervision.

Result: VoxelFM matched or outperformed existing CT foundation models across seven categories of clinically relevant tasks, including classification, regression, and report generation.

Conclusion: Current CT foundation models perform better as feature extractors for lightweight probes than as vision encoders for vision-language models, and VoxelFM is a publicly available model that achieves state-of-the-art results in various tasks.
...
TL;DR: The paper reviews the NTIRE 2026 3D Restoration and Reconstruction Challenge, focusing on robust 3D reconstruction under adverse conditions like low-light and smoke.


<details>
  <summary>Details</summary>
Motivation: The challenge aims to develop robust 3D reconstruction pipelines that perform well in real-world adverse conditions, such as extreme low-light and smoke-degraded environments.

Method: The paper evaluates methods submitted by 33 teams against state-of-the-art baselines, analyzing their performance on the RealX3D benchmark.

Result: The analysis shows significant progress in 3D reconstruction under adverse conditions and identifies shared design principles among top-performing methods.

Conclusion: The study provides insights into effective strategies for handling 3D scene degradation and highlights advancements in robust 3D reconstruction pipelines.
...
TL;DR: A new exposure correction method is proposed to handle spatially non-uniform degradations in images, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing exposure correction methods assume uniform exposure errors, which is not the case in real-world scenarios where diverse exposure errors coexist within a single image.

Method: A Spatial Signal Encoder is introduced to predict spatially adaptive modulation weights, and an uncertainty-inspired non-uniform loss is proposed for optimization.

Result: The proposed method achieves superior qualitative and quantitative performance compared to state-of-the-art methods.

Conclusion: The proposed method effectively addresses the challenge of spatially non-uniform degradations in exposure correction, providing a more accurate and robust solution.
...
TL;DR: OP-GRPO, an off-policy framework for flow-matching models, improves training efficiency by 65.8% while maintaining generation quality.


<details>
  <summary>Details</summary>
Motivation: GRPO's on-policy training paradigm leads to low sample efficiency, which OP-GRPO aims to address.

Method: OP-GRPO uses active trajectory selection, sequence-level importance sampling correction, and trajectory truncation.

Result: OP-GRPO achieves comparable or superior performance to Flow-GRPO with 34.2% of the training steps.

Conclusion: OP-GRPO improves training efficiency of flow-matching models while preserving generation quality.
...
TL;DR: The paper proposes an uncertainty-aware test-time adaptation framework for spatio-temporal fusion in land surface temperature estimation, improving a pre-trained model's performance in new regions with limited unlabeled data.


<details>
  <summary>Details</summary>
Motivation: Deep learning models struggle to generalize across geographic regions due to domain shifts, which occur when data distributions differ between training and target regions. Existing test-time adaptation methods are primarily designed for classification and not directly applicable to regression tasks.

Method: The proposed framework updates only the fusion module of a pre-trained spatio-temporal fusion model, guided by epistemic uncertainty, land use and land cover consistency, and bias correction, without requiring source data or labeled target samples.

Result: Experiments on four target regions show consistent improvements in RMSE and MAE for a pre-trained model, with average gains of 24.2% and 27.9%, respectively, even with limited unlabeled target data and only 10 TTA epochs.

Conclusion: The proposed uncertainty-aware test-time adaptation framework effectively adapts a pre-trained spatio-temporal fusion model to new regions, improving land surface temperature estimation with limited unlabeled data.
...
TL;DR: This paper proposes a hyperbolic co-embedding framework to model the correspondence between font styles and impressions, capturing the varying degree of style specificity.


<details>
  <summary>Details</summary>
Motivation: The relationship between fonts and impressions is not one-to-one, and some impressions are more specific to certain font styles than others.

Method: A hyperbolic co-embedding framework with entailment constraints is used to embed font images and impression descriptions in a shared space.

Result: The proposed framework improves bidirectional retrieval and provides an interpretable geometric measure of style specificity.

Conclusion: The framework captures a coherent progression from ambiguous to style-specific impressions and provides a meaningful quantification of style specificity.
...
TL;DR: The paper proposes a novel method for multi-view multi-label learning with missing views and labels, achieving consistent representation learning and improved model generalization.


<details>
  <summary>Details</summary>
Motivation: The dual-missing scenario in multi-view multi-label learning, where both views and labels are incomplete, has been largely unexplored. Existing methods have limitations in capturing stable and discriminative shared semantics.

Method: The proposed method uses a multi-view shared codebook and cross-view reconstruction to learn discrete consistent representations. It also designs a weight estimation method for label correlation structures and a fused-teacher self-distillation framework.

Result: The proposed method demonstrates effectiveness through extensive comparative experiments on five benchmark datasets.

Conclusion: The paper presents a structured mechanism for consistent representation learning and improved model generalization under missing-view and missing-label conditions.
...
TL;DR: A benchmark called GENFIG1 is introduced to evaluate the ability of generative AI models to produce figures that clearly express the central idea of a scientific paper.


<details>
  <summary>Details</summary>
Motivation: The primary visual summary of a science paper, often 'Figure 1', requires significant effort to create and effectively communicate the core research idea, highlighting the difficulty in science visual communication.

Method: GENFIG1 uses a benchmark curated from top deep-learning conference papers with stringent quality control and an automatic evaluation metric that correlates well with human judgments.

Result: Representative models were evaluated on GENFIG1, showing that the task presents significant challenges even for the best-performing systems.

Conclusion: The GENFIG1 benchmark serves as a foundation for future progress in multimodal AI, aiming to improve the generation of figures that convey scientific concepts effectively.
...
TL;DR: The paper proposes a method to improve extreme far-distance video person re-identification (ReID) using large-scale vision-language models, achieving state-of-the-art results on the DetReIDX benchmark.


<details>
  <summary>Details</summary>
Motivation: Extreme far-distance video person ReID is challenging due to scale compression, resolution degradation, motion blur, and aerial-ground viewpoint mismatch, which degrades model performance.

Method: The approach adapts a CLIP-based baseline by upgrading the visual backbone to ViT-L/14, introducing selective fine-tuning, incorporating temporal attention pooling, and using adapter-based and prompt-conditioned cross-view learning.

Result: The method achieves mAP scores of 46.69 (A2G), 41.23 (G2A), and 22.98 (A2A), with an overall mAP of 35.73 on the DetReIDX benchmark.

Conclusion: Large-scale vision-language backbones, combined with stability-focused adaptation, significantly enhance robustness in extreme far-distance video person ReID.
...
TL;DR: AURA, a streaming visual interaction framework, enables a unified VideoLLM to process video streams in real-time, supporting question answering and proactive responses.


<details>
  <summary>Details</summary>
Motivation: Existing VideoLLMs are not well-suited for live video streams requiring continuous observation and timely response, and current streaming approaches have limitations.

Method: AURA integrates context management, data construction, training objectives, and deployment optimization for stable long-horizon streaming interaction.

Result: AURA achieves state-of-the-art performance on streaming benchmarks and supports a real-time demo system running at 2 FPS on two 80G accelerators.

Conclusion: AURA facilitates real-time video understanding and interaction, and its model and inference framework are released for future research.
...
TL;DR: The paper introduces GraphicDesignBench (GDB), a benchmark suite for evaluating AI models on professional graphic design tasks, highlighting the limitations of current models in tasks requiring precision, structure, and compositional awareness.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the lack of a comprehensive benchmark suite for evaluating AI models on professional graphic design tasks, which is a significant gap given the unique challenges of design work.

Method: The authors introduce GraphicDesignBench (GDB), a suite of 50 tasks organized along five axes: layout, typography, infographics, template & design semantics, and animation. They evaluate frontier closed-source models using a standardized metric taxonomy.

Result: The results show that current AI models fall short on core challenges of professional design, including spatial reasoning, faithful vector code generation, fine-grained typographic perception, and temporal decomposition of animations.

Conclusion: The paper concludes that while high-level semantic understanding is within reach, current models struggle with tasks demanding precision, structure, and compositional awareness, and proposes GDB as a rigorous testbed for tracking progress toward capable design collaborators.
...
TL;DR: A novel autonomous driving world model called DriveVA is proposed, which jointly decodes future visual forecasts and action sequences, achieving improved generalization and performance.


<details>
  <summary>Details</summary>
Motivation: The challenge of generalization in autonomous driving, particularly under unseen scenarios, sensor domains, and environmental conditions, and the limitations of recent world-model-based planning methods.

Method: DriveVA, a DiT-based decoder that jointly predicts future action sequences and videos, with a video continuation strategy to strengthen long-duration rollout consistency.

Result: DriveVA achieves a 90.9 PDM score on NAVSIM, and reduces average L2 error and collision rate by 78.9% and 83.3% on nuScenes and 52.5% and 52.4% on Bench2drive compared to state-of-the-art world-model-based planners.

Conclusion: DriveVA demonstrates improved generalization and performance in autonomous driving, with zero-shot capability and cross-domain generalization.
...
TL;DR: The paper introduces a unified framework for integrating persistent homology into deep learning for 3D point clouds, enabling topological reasoning and improving accuracy and robustness.


<details>
  <summary>Details</summary>
Motivation: The integration of Persistent Homology (PH) into deep learning for 3D point clouds remains ad hoc and peripheral, and the authors aim to formalize the interplay between PH and deep learning components.

Method: The authors propose a unified design space for Persistent-Homology driven learning in 3D point clouds (3DPHDL), identifying six principled injection points for topology to act as a structural inductive bias, and instantiate this framework through a controlled empirical study on ModelNet40 classification and ShapeNetPart segmentation.

Result: The results demonstrate consistent improvements in topology-sensitive discrimination and part consistency, while revealing meaningful trade-offs between representational expressiveness and combinatorial complexity.

Conclusion: The paper provides a systematic framework for incorporating topological reasoning into 3D point cloud learning, viewing persistent homology as a structured component within the learning pipeline rather than an auxiliary feature.
...
TL;DR: This paper presents HighFM, a foundation model for high temporal resolution, multispectral Earth Observation (EO) data, leveraging SEVIRI imagery to improve real-time monitoring and disaster detection.


<details>
  <summary>Details</summary>
Motivation: The increasing frequency and severity of climate-related disasters necessitate real-time monitoring, early warning, and informed decision-making, which Earth Observation (EO) and Machine Learning (ML) can support, but existing models have limitations.

Method: The authors adapt the SatMAE masked autoencoding framework to learn robust spatiotemporal representations from over 2 TB of SEVIRI imagery, incorporating fine-grained temporal encodings to capture short-term variability.

Result: The pretrained models, fine-tuned on cloud masking and active fire detection tasks, show consistent gains in balanced accuracy and IoU metrics compared to traditional baselines and recent geospatial foundation models.

Conclusion: The results highlight the potential of temporally dense geostationary data for real-time EO, offering a scalable path toward foundation models for disaster detection and tracking.
...
TL;DR: Proposed GA-GS method for reconstructing static 3D scenes from monocular video with dynamic objects, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Current approaches for static scene reconstruction are limited by their reliance on background and inability to recover regions occluded by dynamic objects.

Method: GA-GS method, which leverages generation to assist in reconstructing occluded regions, employs a motion-aware module, diffusion model, and learnable authenticity scalar.

Result: GA-GS achieves state-of-the-art performance in static scene reconstruction, especially in challenging scenarios with large-scale, persistent occlusions.

Conclusion: The proposed GA-GS method effectively reconstructs static 3D scenes from monocular video with dynamic objects, addressing the limitations of current approaches.
...
TL;DR: The paper proposes Spatially-Weighted CLIP (SW-CLIP), a novel framework for street-view geo-localization that incorporates spatial autocorrelation into vision-language contrastive learning, improving accuracy and spatial coherence.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve geo-localization accuracy by incorporating spatial autocorrelation into vision-language contrastive learning, shifting from semantic alignment to geographic alignment.

Method: The method proposed is Spatially-Weighted CLIP (SW-CLIP), which uses Tobler's First Law of Geography to model geographic relationships, location-as-text representation, spatially weighted soft labels, and neighborhood-consistency regularization.

Result: The results show that SW-CLIP significantly improves geo-localization accuracy, reduces long-tail errors, and enhances spatial coherence compared to standard CLIP on a multi-city dataset.

Conclusion: The conclusion is that SW-CLIP provides a general paradigm for integrating spatial principles into multimodal representation learning, highlighting the importance of geographic alignment for robust geo-localization.
...
TL;DR: The paper presents techniques to reduce computational cost of test time operations for network classifiers based on extreme learning machine (ELM), achieving this through integer operations without compromising accuracy.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to reduce the computational cost of test time operations of network classifiers based on ELM, particularly for applications where power consumption is limited or expensive, such as embedded applications and large corporations' data centers.

Method: The paper proposes several techniques: (i) using ternary set for input weights to eliminate multiplications, (ii) proving that normalized and non-normalized test signals have the same classification accuracy, and (iii) creating an integer version of the output weights with minimal accuracy reduction.

Result: The techniques were tested on 5 computer vision datasets, showing that they can reduce the computational cost of operations necessary for classification at test time in FPGAs without significantly compromising accuracy.

Conclusion: The paper concludes that the proposed techniques can effectively reduce the computational cost of test time operations for ELM-based network classifiers, which is crucial for power-limited embedded applications and cost-sensitive data centers.
...
TL;DR: A new method called Graph-to-Frame RAG (G2F-RAG) is proposed for video reasoning that incorporates external knowledge in a visual space, reducing cognitive load and improving interpretability.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of current multimodal models that append textual or multi-clip evidence to a single attention space, leading to diluted attention and higher cognitive load.

Method: The method involves a two-stage approach: (1) building a problem-agnostic video knowledge graph on an offline stage, and (2) using a hierarchical multi-agent controller on an online stage to retrieve and render relevant knowledge as a single reasoning frame appended to the video.

Result: G2F-RAG yields consistent gains on diverse public benchmarks, with larger improvements in knowledge-intensive settings, and is plug-and-play across backbones and scales.

Conclusion: The proposed G2F-RAG paradigm reframes retrieval as visual space knowledge fusion for robust and interpretable video reasoning, reducing cognitive load and leaving an explicit, inspectable evidence trail.
...
TL;DR: The paper introduces RLER, a dual-paradigm approach to improve video reasoning in large multimodal models by decoupling learning to produce evidence from obtaining reliable answers.


<details>
  <summary>Details</summary>
Motivation: Current large multimodal models often perform video reasoning in a single pass without verifying if the reasoning is evidence-aligned, leading to a need for more reliable and interpretable methods.

Method: The RLER approach consists of RLER-Training, which uses group-relative reinforcement learning and novel task-driven rewards to optimize the policy, and RLER-Inference, which applies a train-free orchestrator to generate and score diverse candidates based on evidence consistency, confidence, transparency, and non-redundancy.

Result: RLER achieves state-of-the-art results across 8 representative benchmarks, with an average improvement of 6.3% over base models, while using on average 3.1 candidates per question.

Conclusion: The results suggest that making evidence explicit during learning and electing by evidence during inference is a robust path to trustworthy video reasoning, improving reliability and interpretability without enlarging the model.
...
TL;DR: A novel framework, BiTDiff, is proposed for 3D conducting motion generation from music, leveraging a large-scale dataset, CM-Data, and achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: The task of 3D conducting motion generation is underexplored due to the lack of large-scale datasets and effective methods for long-sequence generation with high quality and efficiency.

Method: BiTDiff, a framework built upon a BiMamba-Transformer hybrid model architecture and a Diffusion-based generative strategy with human-kinematic decomposition.

Result: BiTDiff achieves state-of-the-art performance for 3D conducting motion generation on the CM-Data dataset.

Conclusion: The proposed BiTDiff framework and CM-Data dataset provide a solution to the challenges of 3D conducting motion generation, enabling applications in music education, virtual performance, digital human animation, and human-AI co-creation.
...
TL;DR: A new dataset, UENR-600K, is introduced for nighttime video deraining, which uses Unreal Engine to simulate 3D rain particles in virtual environments, and a state-of-the-art baseline is established by adapting a video generation model.


<details>
  <summary>Details</summary>
Motivation: Existing datasets for nighttime video deraining are limited by their use of 2D rain overlays and failure to capture physical properties of nighttime rain, and capturing real paired nighttime videos is impractical due to the difficulty of isolating rain effects from other degradations.

Method: The authors utilize Unreal Engine to simulate rain as 3D particles within virtual environments to create a large-scale, physically grounded dataset, and adapt the Wan 2.2 video generation model to treat deraining as a video-to-video generation task.

Result: Models trained on the UENR-600K dataset generalize significantly better to real-world videos, and the established baseline achieves a new state-of-the-art performance in nighttime video deraining.

Conclusion: The introduction of UENR-600K and the adaptation of the video generation model provide a significant improvement in nighttime video deraining, bridging the sim-to-real gap and enabling better generalization to real-world videos.
...
TL;DR: A novel method called 3D-Fixer is proposed for compositional 3D scene generation from a single view, which bridges the gap between efficiency and generalization.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for 3D scene generation either generalize poorly to complex scenes or suffer from time-consuming pose optimization.

Method: 3D-Fixer uses an in-place completion paradigm, extending 3D object generative priors to generate complete 3D assets conditioned on partially visible point clouds, and proposes a coarse-to-fine generation scheme and Occlusion-Robust Feature Alignment (ORFA) strategy.

Result: 3D-Fixer achieves state-of-the-art geometric accuracy, outperforming baselines like MIDI and Gen3DSR while maintaining efficiency.

Conclusion: The proposed 3D-Fixer method effectively addresses the challenges of compositional 3D scene generation from a single view, providing a more efficient and generalizable solution.
...
TL;DR: The paper introduces BoxComm, a dataset for automatic sports commentary generation in combat sports, specifically boxing. It proposes a new taxonomy and evaluation metrics, and experiments with state-of-the-art multimodal large language models (MLLMs) show that they struggle with generating accurate commentary.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for automatic sports commentary generation focus on team sports, leaving combat sports unexplored. Combat sports present distinct challenges, such as critical actions unfolding within milliseconds with subtle yet decisive differences.

Method: The paper proposes a structured commentary taxonomy and two novel evaluations: category-conditioned generation and commentary rhythm assessment. It also introduces EIC-Gen, an improved baseline incorporating detected punch events.

Result: Experiments on multiple state-of-the-art MLLMs reveal that current models struggle on both evaluations. The proposed EIC-Gen baseline yields consistent gains, highlighting the importance of perceiving fleeting and subtle events for combat sports commentary.

Conclusion: The paper presents a new dataset and evaluation metrics for automatic sports commentary generation in combat sports. The results show that current MLLMs struggle with generating accurate commentary, and the proposed baseline improves performance by incorporating structured action cues.
...
TL;DR: A new method called HandDreamer is introduced for generating customizable 3D hand models from text prompts, addressing limitations in current methods.


<details>
  <summary>Details</summary>
Motivation: Current 3D hand model generation methods are expensive, cumbersome, and offer little customizability. Recent text-to-3D synthesis methods do not generalize well to 3D hand models, resulting in unnatural structures and inconsistencies.

Method: HandDreamer uses MANO hand model-based initialization, hand skeleton-guided diffusion, and a corrective hand shape guidance loss to ensure view and pose consistency.

Result: HandDreamer demonstrates superiority over state-of-the-art methods in 3D hand model generation, providing a new way forward.

Conclusion: HandDreamer effectively generates customizable 3D hand models from text prompts, addressing current limitations and paving the way for improved 3D hand model generation.
...
TL;DR: The paper proposes HSA-DINO, a framework for enhancing open-vocabulary object detection (OVOD) with a focus on improving domain adaptability without losing generalization ability.


<details>
  <summary>Details</summary>
Motivation: Existing OVOD methods perform well on general scenarios but suffer performance drops when transferred to domain-specific tasks due to scarcity and weak semantics of category labels and the inability to capture auxiliary semantics.

Method: HSA-DINO uses a multi-scale prompt bank to capture hierarchical semantics and a semantic-aware router to dynamically select appropriate semantic augmentation strategies during inference.

Result: HSA-DINO outperforms previous state-of-the-art methods, achieving a better trade-off between domain adaptability and open-vocabulary generalization on various datasets including OV-COCO and several vertical domain datasets.

Conclusion: The proposed HSA-DINO framework effectively enhances OVOD by improving domain adaptability while maintaining generalization ability, through its novel semantic augmentation approach.
...
TL;DR: Chorus is a caching approach that accelerates video diffusion model serving by leveraging similarity across requests, achieving up to 45% speedup on industrial 4-step distilled models.


<details>
  <summary>Details</summary>
Motivation: Video Diffusion Transformer (DiT) models suffer from high inference cost due to iterative denoising, and existing caching approaches are ineffective for industrial 4-step distilled models.

Method: Chorus employs a three-stage caching strategy along the denoising process, including full reuse of latent features, inter-request caching in specific latent regions, and Token-Guided Attention Amplification.

Result: Chorus achieves up to 45% speedup on industrial 4-step distilled models.

Conclusion: Chorus is a effective caching approach for accelerating video diffusion model serving, especially for industrial 4-step distilled models where prior intra-request caching approaches are ineffective.
...
TL;DR: The paper proposes a novel approach to learn Group Activity Features (GAFs) without annotations, leveraging DINO and pretext tasks for group-dynamics-aware GAF learning.


<details>
  <summary>Details</summary>
Motivation: Prior work uses low-level static local features to learn GAFs. The authors aim to leverage dynamics-aware and group-aware pretext tasks for better GAF learning.

Method: The authors propose using person flow estimation and group-relevant object location estimation as pretext tasks, along with local and global features provided by DINO.

Result: The method achieves state-of-the-art performance in group activity retrieval and recognition on public datasets.

Conclusion: The proposed approach effectively learns GAFs without annotations, demonstrating the effectiveness of dynamics-aware and group-aware pretext tasks.
...
TL;DR: This paper evaluates the robustness of vision-language models (VLMs) under natural adversarial scenarios for various tasks.


<details>
  <summary>Details</summary>
Motivation: To understand the robustness, limitations, and real-world applicability of VLMs beyond standard benchmarks.

Method: A systematic evaluation framework for VLMs on curated adversarial datasets for zero-shot image classification, semantic segmentation, and visual question answering.

Result: Robust CLIP models can amplify natural adversarial vulnerabilities, and CLIP models significantly reduce performance for natural language-induced adversarial examples.

Conclusion: The findings highlight the need for future research in robust and fair multimodal pattern recognition.
...
TL;DR: A novel 3D reconstruction model, MVis-Fold, was developed to enable high-fidelity imaging of microvascular networks from 2D super-resolution ultrasound images.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this study was to overcome the challenge of 3D reconstruction of microvasculature from super-resolution ultrasound (SRUS) images, which is limited by the nature of imaging principles.

Method: The researchers developed an innovative 3D microvascular reconstruction model called MVis-Fold, which integrates a cross-scale network architecture to perform high-fidelity inference and reconstruction of 3D microvascular networks from 2D SRUS images.

Result: The model was validated and shown to be accurate and reliable in 3D microvascular reconstruction of solid tumors, enabling precise calculation of key parameters in 3D space.

Conclusion: This study establishes a foundation for 3D quantitative analysis of microvasculature, providing new tools and methods for diagnosis and monitoring of various diseases.
...
TL;DR: The authors propose VicoEdit, a training-free and inversion-free method for visual context-aware image editing, achieving better performance than state-of-the-art training-based models.


<details>
  <summary>Details</summary>
Motivation: Existing visual context-aware editing methods require data collection effort and training cost, or rely on diffusion inversion which struggles with consistency and flexibility.

Method: VicoEdit directly transforms the source image into the target one based on the visual context, eliminating the need for inversion, and uses a posterior sampling approach guided by concept alignment to enhance editing consistency.

Result: Empirical results demonstrate that VicoEdit achieves better editing performance than state-of-the-art training-based models.

Conclusion: The proposed VicoEdit method offers a training-free and inversion-free solution for visual context-aware image editing, achieving superior performance to training-based models.
...
TL;DR: A unified defense framework is proposed to protect multimodal large language models from backdoor implantation during supervised fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Multimodal large language models are susceptible to backdoor implantation, which can lead to harmful responses. The challenge lies in suppressing attacks while preserving normal generation ability.

Method: A unified defense framework based on patch augmentation and cross-view regularity is proposed, combining patch-level data augmentation with cross-view output difference regularization and output entropy constraints.

Result: The proposed defense method effectively reduces the attack success rate while maintaining a high level of normal text generation capability across three models, two tasks, and six attacks.

Conclusion: The proposed method enables the secure deployment of large-scale multimodal models in realistic low-frequency poisoning and covert triggering scenarios.
...
TL;DR: The paper proposes the Indra Representation Hypothesis, which suggests that unimodal foundation models converge to a shared relational structure. The authors formalize this hypothesis using category theory and evaluate it through experiments, demonstrating enhanced robustness and alignment across architectures and modalities.


<details>
  <summary>Details</summary>
Motivation: The authors are motivated by the observation that unimodal foundation models learn convergent representations, but these representations have limited expressiveness. They aim to propose a new representation hypothesis that can capture a shared relational structure underlying reality.

Method: The authors use the V-enriched Yoneda embedding from category theory to formalize the Indra Representation Hypothesis. They define the Indra representation as a relational profile of each sample with respect to others and evaluate it using angular distance in cross-model and cross-modal scenarios.

Result: The experiments demonstrate that Indra representations consistently enhance robustness and alignment across architectures and modalities, providing a theoretically grounded and practical framework for training-free alignment of unimodal foundation models.

Conclusion: The paper concludes that the proposed Indra Representation Hypothesis provides a new perspective on the convergent representations learned by unimodal foundation models. The authors' approach offers a theoretically grounded and practical framework for improving the alignment and robustness of these models across different architectures and modalities.
...
TL;DR: The paper proposes Saliency-R1, a framework to improve the interpretability and faithfulness of vision-language models (VLMs) by highlighting critical image regions and tracing visual information flow.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address concerns about the trustworthiness of VLMs, specifically their tendency to rely more on textual cues than visual evidence and produce ungrounded or fabricated responses.

Method: The method involves a novel saliency map technique, Group Relative Policy Optimization (GRPO), and using overlap between saliency maps and human-annotated bounding boxes as a reward function.

Result: The results show that Saliency-R1 improves reasoning faithfulness, interpretability, and overall task performance.

Conclusion: The conclusion is that Saliency-R1 is an effective framework for improving the interpretability and faithfulness of VLMs' reasoning.
...
TL;DR: MedROI, a framework for efficient medical image compression, is proposed. It removes non-diagnostic background before compression, achieving better compression ratios and faster processing times.


<details>
  <summary>Details</summary>
Motivation: The rapid growth of medical imaging archives necessitates efficient compression methods for storage and data transfer. Existing methods often compress entire images, including non-diagnostic background.

Method: MedROI uses intensity-based thresholding to extract a tight tissue bounding box, discards background voxels, and then compresses the cropped ROI using existing codecs.

Result: MedROI yields significant improvements in compression ratio and encoding/decoding time for most configurations while maintaining comparable reconstruction quality within the ROI.

Conclusion: MedROI offers a codec-agnostic, plug-and-play solution for efficient medical image compression, enhancing storage and data transfer efficiency.
...
TL;DR: A novel LiDAR-based place recognition network, MPTF-Net, is proposed, which achieves state-of-the-art performance on several datasets.


<details>
  <summary>Details</summary>
Motivation: Existing LiDAR-based place recognition methods fail to capture fine-grained geometric structures in complex or repetitive environments.

Method: A multi-view multi-scale pyramid Transformer fusion network, MPTF-Net, which uses a multi-channel NDT-based BEV encoding and a customized pyramid Transformer module.

Result: MPTF-Net achieves state-of-the-art performance on nuScenes, KITTI, and NCLT datasets, with a Recall@1 of 96.31% on nuScenes Boston split and an inference latency of 10.02 ms.

Conclusion: MPTF-Net provides a noise-resilient structural prior and effectively integrates features to improve LiDAR-based place recognition performance.
...
TL;DR: The paper proposes StableTTA, a method to improve the stability and efficiency of ensemble models, achieving significant gains in accuracy and reducing computational cost.


<details>
  <summary>Details</summary>
Motivation: The authors identify a conflict in aggregation strategies that negatively impacts prediction stability in ensemble methods, which often come at the cost of increased memory usage and computational complexity.

Method: The authors propose StableTTA, a training-free method to improve aggregation stability and efficiency.

Result: Empirical results on ImageNet-1K show gains of 10.93--32.82% in top-1 accuracy, with 33 models achieving over 95% accuracy and several surpassing 96%.

Conclusion: StableTTA enables high-accuracy inference on resource-constrained devices, allowing lightweight architectures to outperform ViT by 11.75% in top-1 accuracy while using less than 5% of parameters and reducing computational cost by approximately 89.1% (in GFLOPs).
...
TL;DR: The paper presents a novel approach to relative camera pose estimation in Visual Simultaneous Localization and Mapping (VSLAM) by formulating it as a relational inference problem over epipolar correspondence graphs, achieving improved robustness to noise and large baseline variations.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of accurate relative camera pose estimation in VSLAM due to noisy correspondences, by proposing a method that incorporates explicit geometric structure.

Method: The method involves constructing epipolar correspondence graphs from matched keypoints, and then applying graph operations such as pruning, message passing, and pooling to estimate the quaternion rotation, translation vector, and the Essential Matrix (EM).

Result: The results show improved robustness to dense noise and large baseline variation compared to classical and learning-guided approaches, on both indoor and outdoor benchmarks.

Conclusion: The proposed approach effectively leverages global relational consensus to improve relative pose estimation in VSLAM, demonstrating its potential for practical applications.
...
TL;DR: TILA, a framework for temporal inversion-aware learning and alignment, enhances sensitivity to directional change in temporal vision-language models for chest radiograph analysis.


<details>
  <summary>Details</summary>
Motivation: Most medical foundation models analyze radiographs in isolation, neglecting the clinical task of comparing prior and current images to assess interval change.

Method: TILA uses temporal inversion, reversing image pairs, as a supervisory signal to enhance sensitivity to directional change, integrating inversion-aware objectives across pretraining, fine-tuning, and inference.

Result: TILA consistently improves progression classification and temporal embedding alignment when applied to multiple existing architectures on public datasets and real-world hospital cohorts.

Conclusion: TILA provides a simple yet effective framework for enhancing sensitivity to directional change in temporal vision-language models, with a unified evaluation protocol and a retrieval evaluation set for future research.
...
TL;DR: Proposes TAPE, a two-stage adaptation framework for efficient fine-tuning of foundation models in medical image analysis, achieving state-of-the-art performance in retinal layer segmentation.


<details>
  <summary>Details</summary>
Motivation: Existing methods for ophthalmic diagnosis rely heavily on large data and model scale, which is impractical in resource-constrained settings. Transfer learning faces challenges of domain shift and task misalignment.

Method: TAPE, a two-stage adaptation framework via parameter-efficient fine-tuning, decouples adaptation into domain alignment and task fitting for downstream segmentation.

Result: TAPE demonstrates superior parameter efficiency and achieves state-of-the-art generalization performance across diverse pathologies in retinal layer segmentation.

Conclusion: TAPE offers a promising solution for efficient and accurate medical image analysis in resource-constrained clinical settings.
...
TL;DR: This paper studies the impact of concept unlearning on compositional text-to-image generation, finding a trade-off between erasure effectiveness and preservation of generative capabilities.


<details>
  <summary>Details</summary>
Motivation: The motivation is to understand the broader generative capabilities of post-hoc unlearning methods in text-to-image diffusion models, beyond just erasure success.

Method: The authors conduct a systematic empirical study using state-of-the-art unlearning methods, evaluated on benchmarks such as T2I-CompBench++ and GenEval, focusing on nudity removal in Stable Diffusion 1.4.

Result: The results show a trade-off between unlearning effectiveness and compositional integrity, with methods that achieve strong erasure often degrading attribute binding, spatial reasoning, and counting.

Conclusion: The study highlights limitations of current evaluation practices and the need for unlearning objectives that balance targeted suppression with semantic preservation.
...
TL;DR: The paper proposes a novel framework, Partial-Reference Image Quality Assessment (PR-IQA), to evaluate diffusion-generated views for 3D reconstruction, achieving high accuracy without ground-truth supervision.


<details>
  <summary>Details</summary>
Motivation: Diffusion models can generate pseudo-ground-truth views for 3D reconstruction, but these views often contain inconsistencies that can impair reconstruction quality.

Method: The PR-IQA framework computes a geometrically consistent partial quality map and completes it via a cross-attention mechanism, enabling thorough quality assessment without ground truth.

Result: PR-IQA outperforms existing IQA methods, achieving full-reference-level accuracy and producing superior 3D reconstructions and NVS results when integrated into a diffusion-augmented 3DGS pipeline.

Conclusion: The proposed PR-IQA framework effectively filters inconsistencies in diffusion-generated views, leading to improved 3D reconstruction and novel view synthesis results.
...
TL;DR: The paper introduces Firebolt-VL, an efficient vision-language model that replaces the Transformer-based decoder with a Liquid Foundation Model (LFM) decoder and proposes a Token-Grid Correlation Module to enhance visual grounding, achieving accurate and fine-grained understanding with improved efficiency.


<details>
  <summary>Details</summary>
Motivation: The high computational cost of multimodal large language models (MLLMs) limits their deployment in resource-constrained scenarios, and existing methods struggle to capture fine-grained visual regions, leading to degraded performance on fine-grained reasoning tasks.

Method: The paper proposes Firebolt-VL, which uses a Liquid Foundation Model (LFM) decoder instead of a Transformer-based decoder, and a Token-Grid Correlation Module to compute lightweight correlations between text tokens and image patches.

Result: Experimental results across multiple benchmarks demonstrate that Firebolt-VL achieves accurate, fine-grained understanding with significantly improved efficiency.

Conclusion: The proposed Firebolt-VL model effectively addresses the limitations of existing vision-language models, providing a more efficient and accurate solution for fine-grained reasoning tasks.
...
TL;DR: The paper addresses the limitations of deepfake detectors by exploring physical features that distinguish natural from AI-generated images, proposing a novel feature selection algorithm, and integrating these features into multimodal models like CLIP to enhance detection performance.


<details>
  <summary>Details</summary>
Motivation: The rapid advancement of AI-generated content (AIGC) has blurred the boundaries between real and synthetic images, exposing the limitations of existing deepfake detectors that often overfit to specific generative models.

Method: The paper conducts a comprehensive exploration of 15 physical features across more than 20 datasets generated by various GANs and diffusion models, and proposes a novel feature selection algorithm that identifies five core physical features.

Result: The method achieves state-of-the-art performance on multiple Genimage benchmarks, with near-perfect accuracy (99.8%) on datasets such as Wukong and SDv1.4.

Conclusion: The work pioneers the use of physically grounded features for trustworthy vision language modeling and opens new directions for mitigating hallucinations and textual inaccuracies in large multimodal models.
...
TL;DR: GLA introduces covert backdoor attacks on visual language models for autonomous driving, using naturalistic triggers that blend into urban scenes, achieving high attack success rates without degrading model performance.


<details>
  <summary>Details</summary>
Motivation: The integration of visual language models into safety-critical systems like autonomous driving creates a potential attack surface for backdoor attacks, and existing attacks rely on easily detectable triggers.

Method: GLA uses graffiti-based visual patterns and cross-language text triggers to create robust and covert backdoor attacks, evaluated on DriveVLM.

Result: GLA achieves a 90% Attack Success Rate and 0% False Positive Rate with only 10% poisoning ratio, without weakening the model on clean tasks.

Conclusion: GLA reveals underestimated security threats in self-driving VLMs and provides a new attack paradigm for backdoor evaluation in safety-critical multimodal systems.
...
TL;DR: InCTRLv2, a novel few-shot Generalist Anomaly Detection and Segmentation framework, is proposed to detect anomalies across diverse domains without retraining.


<details>
  <summary>Details</summary>
Motivation: Most recent anomaly detection methods are specialist models that struggle to generalize to unseen datasets, and the paradigm of Generalist Anomaly Detection (GAD) aims to learn a single generalist model to detect anomalies across diverse domains.

Method: InCTRLv2 extends the previous GAD model, InCTRL, by introducing a dual-branch framework with two novel modules: Discriminative Anomaly Score Learning (DASL) and One-class Anomaly Score Learning (OASL), both guided by rich visual-text semantic priors encoded by large-scale vision-language models.

Result: InCTRLv2 achieves state-of-the-art performance in both anomaly detection and segmentation tasks across various settings on ten AD datasets.

Conclusion: InCTRLv2 offers a dual semantic perspective for anomaly detection, emphasizing both normal-abnormal discriminations and normality-deviated semantics, and demonstrates its effectiveness in detecting anomalies across diverse domains.
...
TL;DR: A new dataset and detection framework are proposed to address limitations in detecting AI-generated videos, achieving superior performance across benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current AI-generated video detection methods are limited by preprocessing operations that discard subtle forgery traces and cause information loss, as well as outdated datasets that fail to capture modern generative models' sophistication.

Method: A novel detection framework built on the Qwen2.5-VL Vision Transformer, operating natively at variable spatial resolutions and temporal durations, is proposed along with a large-scale dataset of over 140K videos.

Result: The proposed method achieves superior performance across multiple benchmarks, establishing a robust new baseline for AI-generated video detection.

Conclusion: The proposed approach effectively preserves high-frequency artifacts and spatiotemporal inconsistencies, underscoring the critical importance of native-scale processing for AI-generated video detection.
...
TL;DR: The Flow Divergence Sampler (FDS) is proposed to improve flow-based models by refining intermediate states and steering them toward less ambiguous regions, leading to better generation quality.


<details>
  <summary>Details</summary>
Motivation: The averaged velocity in flow-based models can misguide samples toward low-density regions when sample-wise velocities conflict, degrading generation quality.

Method: The Flow Divergence Sampler (FDS), a training-free framework that refines intermediate states before each solver step, using the divergence of the marginal velocity field as a signal to steer states toward less ambiguous regions.

Result: FDS consistently improves fidelity across various generation tasks, including text-to-image synthesis and inverse problems.

Conclusion: The proposed Flow Divergence Sampler (FDS) framework is effective in improving the generation quality of flow-based models by mitigating the issue of conflicting sample-wise velocities.
...
TL;DR: The paper proposes Synthesis4AD, a paradigm for 3D anomaly detection that uses synthetic anomalies to improve performance, addressing the challenge of scarce and long-tailed abnormal samples.


<details>
  <summary>Details</summary>
Motivation: The scarcity and long-tailed distribution of abnormal samples in industrial 3D anomaly detection constrain performance.

Method: Synthesis4AD leverages a controllable synthesis engine (MPAS) and a software platform (3D-DefectStudio) to generate large-scale, high-fidelity synthetic anomalies, and incorporates a multimodal large language model (MLLM) for knowledge-driven anomalous data generation.

Result: The proposed method achieves state-of-the-art performance on Real3D-AD, MulSen-AD, and a real-world industrial parts dataset.

Conclusion: The paper presents a novel approach to improve 3D anomaly detection performance by leveraging synthetic anomalies, and demonstrates its effectiveness through extensive experiments.
...
TL;DR: The authors propose ZeD-MAP, a framework that enhances diffusion-based depth estimation from UAV imagery with metric consistency and accuracy, enabling real-time 3D mapping.


<details>
  <summary>Details</summary>
Motivation: Real-time depth reconstruction from high-resolution UAV imagery is crucial for time-sensitive tasks like disaster response but faces challenges such as wide-baseline parallax, large image sizes, and low-texture surfaces.

Method: The authors integrate incremental cluster-based bundle adjustment (BA) into a test-time diffusion depth model, creating a SLAM-like mapping pipeline called ZeD-MAP.

Result: The proposed method achieves sub-meter accuracy, with 0.87 m error in the horizontal plane and 0.12 m in the vertical direction, while maintaining per-image runtimes between 1.47 and 4.91 seconds.

Conclusion: The findings demonstrate that BA-based metric guidance provides consistency comparable to classical photogrammetric methods, significantly accelerating processing and enabling real-time 3D map generation.
...
TL;DR: A new method, DenZa-Gaussian, is proposed to improve 3D reconstruction in ADF-STEM tomography, especially under sparse-view conditions, by adapting 3D Gaussian Splatting (3D GS) with learnable scalar fields, scattering view normalization, and a modified loss function.


<details>
  <summary>Details</summary>
Motivation: The motivation is to balance reconstruction fidelity with sample preservation in ADF-STEM tomography, as integrating more tilt views improves reconstruction but risks damaging dose-sensitive materials.

Method: The method involves three key components: (1) modeling local scattering strength as a learnable scalar field (denza), (2) introducing a coefficient $γ$ for scattering view normalization, and (3) incorporating a loss function with a 2D Fourier amplitude term to suppress missing wedge artifacts.

Result: Experiments show that DenZa-Gaussian produces high-fidelity reconstructions and 2D projections that align more closely with original tilts, demonstrating superior robustness under sparse-view conditions (45-view and 15-view tilt series).

Conclusion: The proposed DenZa-Gaussian method effectively improves 3D reconstruction in ADF-STEM tomography, especially under sparse-view conditions, enabling precise analysis of nanoscale materials' structural and compositional features while preserving dose-sensitive samples.
...
TL;DR: The paper introduces OpenWorldLib, a standardized framework for Advanced World Models, providing a clear definition and categorization of world models' capabilities, enabling unified and efficient model reuse and inference.


<details>
  <summary>Details</summary>
Motivation: The lack of a clear and unified definition of world models in artificial intelligence research.

Method: The authors propose a definition of world models, categorize their essential capabilities, and develop OpenWorldLib, a comprehensive inference framework integrating models across tasks.

Result: OpenWorldLib, a unified framework for world models, facilitating efficient reuse and collaborative inference of models across different tasks.

Conclusion: The paper provides a clear definition and systematic categorization of world models, and presents OpenWorldLib as a comprehensive framework for future world model research, highlighting potential future directions.
...
TL;DR: The paper proposes an adaptive KV-cache quantization method for Large Language Models (LLMs) to reduce memory and latency on mobile and edge devices, achieving better accuracy-latency trade-off.


<details>
  <summary>Details</summary>
Motivation: Deploying LLMs on mobile, embedded, and edge devices is challenging due to the memory and bandwidth overhead of the key-value (KV) cache, which grows linearly with context length.

Method: The paper proposes a learned policy that assigns bit-width proportional to token importance, using lightweight token-level features and a compact data-driven controller to dynamically select KV precision.

Result: The adaptive precision policy reduces KV memory footprint and latency while improving accuracy compared to static KV quantization and rule-based baselines, maintaining competitive accuracy close to FP16 inference across standard LLM benchmarks.

Conclusion: The proposed method consistently improves the accuracy-latency trade-off across multiple commonsense reasoning benchmarks, reducing decoding latency by 17.75% and improving accuracy by 7.60 points on HellaSwag with SmolLM-360M.
...
TL;DR: The paper proposes a Reinforcement Learning framework to automatically discover failure modes in Vision-language Models (VLMs) without human intervention, identifying 36 novel failure modes.


<details>
  <summary>Details</summary>
Motivation: Current methods for identifying weaknesses in VLMs rely on manual efforts, which are costly, unscalable, and subject to human bias, leading to an incomplete understanding of model vulnerabilities.

Method: The proposed framework uses a questioner agent that adaptively generates queries to elicit incorrect answers from a candidate VLM, increasing question complexity and identifying novel failure modes.

Result: The framework identified 36 novel failure modes in which VLMs struggle, demonstrating broad applicability across various model combinations.

Conclusion: The proposed framework offers a scalable and unbiased approach to discovering failure modes in VLMs, providing a more comprehensive understanding of their vulnerabilities.
...
TL;DR: The paper introduces a process-driven image generation approach that decomposes synthesis into multiple iterations of textual planning, visual drafting, textual reflection, and visual refinement.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enable unified multimodal models to imagine the chain of intermediate states in image generation, similar to how humans paint images incrementally.

Method: The approach involves a multi-step paradigm with 4 stages: textual planning, visual drafting, textual reflection, and visual refinement, with dense, step-wise supervision to maintain spatial and semantic consistency.

Result: The paper validates the proposed method through experiments under various text-to-image generation benchmarks.

Conclusion: The proposed approach makes the generation process explicit, interpretable, and directly supervisable, and enables unified multimodal models to generate images in a process-driven manner.
...
TL;DR: CLEAR framework enhances multimodal models' robustness to image degradation by integrating generation and understanding through a three-step approach.


<details>
  <summary>Details</summary>
Motivation: Unified multimodal models struggle with degraded inputs due to their inability to leverage generative capacity, caused by training regimes and decode-reencode pathway limitations.

Method: CLEAR framework: (1) supervised fine-tuning on a degradation-aware dataset, (2) Latent Representation Bridge for direct connection between generation and reasoning, and (3) Interleaved GRPO for joint optimization.

Result: CLEAR improves robustness on degraded inputs while preserving clean-image performance across six multimodal benchmarks and three degradation severity levels.

Conclusion: CLEAR framework effectively enhances multimodal models' performance on degraded inputs, aligning task-driven optimization with visual quality.
...
TL;DR: A novel framework, AvatarPointillist, for generating dynamic 4D Gaussian avatars from a single portrait image using a decoder-only Transformer.


<details>
  <summary>Details</summary>
Motivation: To generate high-quality, photorealistic, and controllable avatars from a single portrait image.

Method: A decoder-only Transformer that autoregressively generates a point cloud for 3D Gaussian Splatting, with a dedicated Gaussian decoder for converting points into renderable Gaussian attributes.

Result: AvatarPointillist produces high-quality, photorealistic, and controllable avatars, demonstrating effective interaction between stages and markedly improved fidelity.

Conclusion: The autoregressive formulation of AvatarPointillist represents a new paradigm for avatar generation, enabling precise, adaptive construction and realistic animation.
...
TL;DR: The paper proposes MMF-BEV, a radar-camera fusion framework for 3D object detection in autonomous driving, achieving competitive results by combining camera and radar data.


<details>
  <summary>Details</summary>
Motivation: The motivation is to leverage the strengths of both camera and radar sensors, which provide complementary information for accurate 3D object detection in autonomous driving.

Method: The method involves building a BEVDepth camera branch and a RadarBEVNet radar branch, each enhanced with Deformable Self-Attention, and fusing them via a Deformable Cross-Attention module.

Result: The results show that MMF-BEV consistently outperforms unimodal baselines and achieves competitive results against prior fusion methods across all object classes.

Conclusion: The conclusion is that MMF-BEV is an effective radar-camera BEV fusion framework for 3D object detection, demonstrating the benefits of sensor complementarity and fusion.
...
TL;DR: E-VLA improves robotic manipulation robustness in adverse conditions by integrating event-driven perception into Vision-Language-Action models.


<details>
  <summary>Details</summary>
Motivation: Robotic VLA models struggle with perception under sensing-stage degradations like low light, motion blur, and black clipping.

Method: E-VLA uses event streams to preserve semantic perception and perception-action consistency, leveraging motion and structural cues.

Result: Experiments show significant improvement in manipulation success rates under adverse conditions, e.g., 0% to 60% and 90% on Pick-Place at 20 lux.

Conclusion: E-VLA provides evidence for effective integration of event-driven perception into VLA models, enabling robust embodied intelligence.
...
TL;DR: The paper proposes a novel framework called Degradation-Driven Prompting (DDP) that improves Visual Question Answering (VQA) performance by strategically reducing image fidelity to help models focus on essential structural information.


<details>
  <summary>Details</summary>
Motivation: Recent advancements in Vision-Language Models (VLMs) have pushed the boundaries of VQA, but high-resolution details can sometimes lead to hallucinations or reasoning errors.

Method: The proposed framework, DDP, uses strategic degradation of image fidelity, including downsampling, structural visual aids, and In-Context Learning (ICL), to calibrate the model's focus and improve VQA performance.

Result: The experimental results demonstrate that DDP enables VLMs to achieve superior reasoning accuracy on challenging visual benchmarks by intentionally degrading visual inputs and providing targeted structural prompts.

Conclusion: The paper concludes that 'less is more' and that by degrading visual inputs and providing targeted structural prompts, DDP can help VLMs bypass distracting textures and achieve better performance on VQA tasks.
...
TL;DR: The paper proposes a framework for generating human-object-scene interactions, addressing challenges in data scarcity and dynamic scene changes.


<details>
  <summary>Details</summary>
Motivation: Human-object-scene interactions (HOSI) generation is crucial for embodied AI, simulation, and animation, but it faces challenges due to limited annotated data and the need to reason over dynamic object-scene changes.

Method: The authors propose a coarse-to-fine instruction-conditioned interaction generation framework aligned with a consistency model's iterative denoising process. It includes a dynamic perception strategy, bump-aware guidance, and a hybrid training strategy.

Result: The method achieves state-of-the-art performance in both HOSI and HOI generation and shows strong generalization to unseen scenes.

Conclusion: The proposed framework effectively addresses the challenges in HOSI generation, including data scarcity and dynamic scene changes, and demonstrates superior performance and generalization capabilities.
...
TL;DR: This paper investigates catastrophic forgetting in Vision-Language Models (VLMs) when fine-tuned for autonomous driving, proposing the Drive Expert Adapter (DEA) framework to mitigate this issue.


<details>
  <summary>Details</summary>
Motivation: The integration of VLMs into autonomous driving faces the challenge of catastrophic forgetting, where fine-tuning erodes pre-trained world knowledge, undermining the core reason for their use.

Method: The authors introduce a new large-scale dataset and propose the Drive Expert Adapter (DEA) framework, which shifts adaptation from the weight space to the prompt space, dynamically routing inference through different knowledge experts.

Result: The DEA framework achieves state-of-the-art results on driving tasks and effectively mitigates catastrophic forgetting, preserving the essential generalization capabilities of VLMs.

Conclusion: The proposed DEA framework offers a solution to the challenge of catastrophic forgetting in VLMs for autonomous driving, enabling the preservation of pre-trained knowledge and achieving superior performance.
...
TL;DR: A new method for generating residential floorplans using a transformer-based model and a general representation called Floorplan Markup Language (FML) is proposed, achieving high-fidelity and functional floorplans under diverse conditions.


<details>
  <summary>Details</summary>
Motivation: The goal is to improve the efficiency and accessibility of spatial design by addressing the limitations of existing methods in terms of diversity, flexibility, and generalizability across heterogeneous conditional tasks.

Method: The proposed method uses a transformer-based generative model, FMLM, which leverages the Floorplan Markup Language (FML) to encode floorplan information and generate high-fidelity and functional floorplans.

Result: The proposed method, FMLM, surpasses previous task-specific state-of-the-art methods on the RPLAN dataset, demonstrating its effectiveness in generating high-quality floorplans under diverse conditions.

Conclusion: The proposed method has the potential to make spatial design more efficient and accessible by providing a general and flexible approach to residential floorplan generation.
...
TL;DR: The paper proposes a patch-level hallucination detection framework for large vision-language models, achieving up to 90% accuracy in token-level hallucination detection.


<details>
  <summary>Details</summary>
Motivation: Existing hallucination detection methods for large vision-language models are limited as they rely on coarse, whole-image measures, which can be evaded by hallucinated tokens with weak but scattered correlations across many local regions.

Method: The paper introduces a patch-level hallucination detection framework that examines fine-grained token-level interactions across model layers, leveraging patch-level statistical features and hidden-layer representations.

Result: The proposed approach achieves up to 90% accuracy in token-level hallucination detection, demonstrating the superiority of fine-grained structural analysis for detecting hallucinations.

Conclusion: The paper concludes that a faithful object token must be strongly grounded in a specific image region and that fine-grained structural analysis is effective in detecting hallucinations in large vision-language models.
...
TL;DR: A new method called Free-Range Gaussians is proposed for 3D reconstruction from a few images, predicting non-pixel, non-voxel-aligned 3D Gaussians.


<details>
  <summary>Details</summary>
Motivation: Existing methods produce highly redundant grid-aligned Gaussians and suffer from holes or blurry conditional means in unobserved regions.

Method: Free-Range Gaussians uses flow matching over Gaussian parameters, a hierarchical patching scheme, timestep-weighted rendering loss, photometric gradient guidance, and classifier-free guidance.

Result: Experiments show consistent improvements over pixel and voxel-aligned methods while using significantly fewer Gaussians, especially when input views leave parts of the object unobserved.

Conclusion: Free-Range Gaussians improves 3D reconstruction by predicting non-pixel, non-voxel-aligned 3D Gaussians and synthesizing plausible content in unobserved regions.
...
TL;DR: The paper proposes a novel framework called DIRECT for video mashup creation, which addresses the challenge of cross-level multimodal orchestration to achieve professional-grade fluidity.


<details>
  <summary>Details</summary>
Motivation: Existing automated editing frameworks often overlook the cross-level multimodal orchestration, resulting in disjointed sequences with abrupt visual transitions and musical misalignment.

Method: The authors formulate video mashup creation as a Multimodal Coherency Satisfaction Problem (MMCSP) and propose a hierarchical multi-agent framework called DIRECT, which consists of three cascade levels: Screenwriter, Director, and Editor.

Result: Extensive experiments demonstrate that DIRECT significantly outperforms state-of-the-art baselines in both objective metrics and human subjective evaluation.

Conclusion: The proposed DIRECT framework effectively addresses the challenge of video mashup creation by achieving professional-grade fluidity through cross-level multimodal orchestration.
...
TL;DR: HorizonWeaver is a framework for scalable, photorealistic editing of complex driving scenes, enabling precise, language-guided edits while preserving scene consistency and instruction fidelity.


<details>
  <summary>Details</summary>
Motivation: Ensuring safety in autonomous driving requires generating realistic, controllable driving scenes, but existing instruction-guided image editors struggle with dense, safety-critical driving layouts.

Method: HorizonWeaver tackles challenges in driving scene editing through a large-scale dataset generation, language-guided masks for fine-grained editing, and joint losses for content preservation and instruction alignment.

Result: HorizonWeaver outperforms prior methods in L1, CLIP, and DINO metrics, achieving +46.4% user preference and improving BEV segmentation IoU by +33%, collecting 255K images across 13 editing categories.

Conclusion: HorizonWeaver provides a scalable framework for photorealistic, instruction-driven editing of complex driving scenes, addressing fundamental challenges in multi-level granularity, rich high-level semantics, and ubiquitous domain shifts.
...
TL;DR: Proposed FileGram, a framework for personalized AI agents operating within local file systems, leveraging file-system behavioral traces for improved memory and personalization.


<details>
  <summary>Details</summary>
Motivation: Existing methods for personalization in human-AI interaction are limited by data constraints and focus on interaction-centric approaches, overlooking dense behavioral traces in file-system operations.

Method: FileGram consists of three components: FileGramEngine (scalable data engine), FileGramBench (diagnostic benchmark), and FileGramOS (bottom-up memory architecture).

Result: Extensive experiments show FileGramBench is challenging for state-of-the-art memory systems, and FileGramEngine and FileGramOS are effective.

Conclusion: The proposed FileGram framework offers a comprehensive approach to personalized memory-centric file-system agents, and by open-sourcing it, future research is encouraged.
...
TL;DR: ClickAIXR is a novel on-device framework for multimodal vision-language interaction in extended reality (XR), enabling precise object selection and natural language questioning.


<details>
  <summary>Details</summary>
Motivation: To address limitations of cloud-based AI and gaze-based selection in XR, such as ambiguity, privacy concerns, and latency.

Method: Integrating an on-device vision-language model (VLM) with a controller-based object selection paradigm, implemented in Magic Leap SDK with ONNX-based local VLM inference.

Result: User study results show moderate latency and acceptable user experience, demonstrating the potential of click-based object selection with on-device AI for trustworthy XR interactions.

Conclusion: ClickAIXR advances trustworthy, privacy-preserving XR interactions by enabling precise object selection, natural language questioning, and on-device inference.
...
TL;DR: The paper introduces a benchmark and dataset for spatial editing, a baseline model, and demonstrates its competitive performance on general editing and spatial manipulation tasks.


<details>
  <summary>Details</summary>
Motivation: Current models are insufficient for fine-grained spatial manipulations, motivating the need for a dedicated assessment suite.

Method: The authors introduce SpatialEdit-Bench, a benchmark that evaluates spatial editing by measuring perceptual plausibility and geometric fidelity. They also construct a synthetic dataset, SpatialEdit-500k, and develop a baseline model, SpatialEdit-16B.

Result: The baseline model achieves competitive performance on general editing and substantially outperforms prior methods on spatial manipulation tasks.

Conclusion: The paper presents a comprehensive approach to spatial editing, including a benchmark, dataset, and baseline model, which will be made publicly available.
...
TL;DR: A new generative world model, DeltaWorld, is introduced, which efficiently generates diverse plausible futures in videos by operating on compact 'delta' tokens, reducing parameters and computational costs.


<details>
  <summary>Details</summary>
Motivation: The challenge of anticipating diverse future states in video world modeling, where existing generative models are computationally expensive and discriminative models average over possible futures.

Method: DeltaTok, a tokenizer that encodes feature differences between frames into 'delta' tokens, and DeltaWorld, a generative model operating on these tokens for efficient multi-hypothesis training and diverse predictions.

Result: DeltaWorld forecasts futures that more closely align with real-world outcomes, with over 35x fewer parameters and 2,000x fewer FLOPs than existing generative world models.

Conclusion: DeltaWorld offers a more efficient and effective approach to video world modeling, enabling diverse predictions in a single forward pass with significantly reduced computational costs.
...
TL;DR: The paper presents a method to unlock the restoration capabilities of pre-trained diffusion models by learning prompt embeddings, achieving competitive performance in All-in-One Restoration (AiOR) without fine-tuning or control modules.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this work is to leverage the priors of pre-trained diffusion models for AiOR without relying on fine-tuning or Control-Net style modules, which are currently the primary methods used.

Method: The method involves directly learning prompt embeddings at the output of the text encoder of pre-trained diffusion models. To stabilize the learning process, the authors use a diffusion bridge formulation that aligns the training and inference dynamics, ensuring a coherent denoising path.

Result: The results show that the approach achieves competitive performance and generalization across diverse degradations. The authors demonstrate this by converting pre-trained models (WAN video model and FLUX image models) into high-performing restoration models without fine-tuning or restoration-specific control modules.

Conclusion: The conclusion is that pre-trained diffusion models inherently possess restoration behavior that can be unlocked through learning prompt embeddings. This approach offers a lightweight solution for AiOR, achieving competitive performance and generalization across various degradations.
...
TL;DR: The paper presents SimpleProc, a procedural generator for creating training data for multi-view stereo (MVS) tasks, achieving comparable or superior results to manually curated data.


<details>
  <summary>Details</summary>
Motivation: The motivation is to explore the design space of procedural rules for MVS and to generate effective training data using a small set of rules.

Method: The method involves using SimpleProc, a fully procedural generator driven by Non-Uniform Rational Basis Splines (NURBS), basic displacement, and texture patterns to create training data.

Result: The results show that the approach achieves superior results compared to manually curated images at a modest scale of 8,000 images, and comparable or exceeding performance to models trained on over 692,000 manually curated images when scaled to 352,000 images.

Conclusion: The conclusion is that SimpleProc can generate effective training data for MVS tasks, achieving comparable or superior results to manually curated data, and that procedural generation can be a viable alternative to manual curation.
...
TL;DR: The paper analyzes the latency of vision-language models and proposes a multi-agent inference framework to leverage the efficiency of large models.


<details>
  <summary>Details</summary>
Motivation: The number of output tokens in vision-language models can be a bottleneck for end-to-end latency, and different models require vastly different numbers of output tokens for comparable performance.

Method: The paper conducts a comprehensive analysis of latency across different components of VLMs on simulated data and proposes a multi-agent inference framework that reuses reasoning tokens from small models.

Result: The experiment shows that a large model with fewer output tokens can be more efficient than a small model with a long output sequence, and the proposed framework can approach the performance of a large model with its own reasoning.

Conclusion: The paper concludes that a large model can achieve better or comparable performance as a small model with significantly fewer output tokens, and the proposed multi-agent inference framework is effective in leveraging the efficiency of large models.
...
TL;DR: The paper presents LoMa, a data-driven approach for local feature matching that achieves remarkable performance gains by combining large and diverse data mixtures, modern training recipes, and scaled model capacity.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to revisit local feature matching from a data-driven perspective, addressing the lag in progress compared to other 3D vision system components, and to create a more challenging benchmark for evaluating feature matching progress.

Method: The method used in this paper is LoMa, which combines large and diverse data mixtures, modern training recipes, scaled model capacity, and scaled compute to improve local feature matching performance.

Result: The results show that LoMa outperforms the state-of-the-art method ALIKED+LightGlue on several benchmarks, including HardMatch (+18.6 mAA), WxBS (+29.5 mAA), InLoc (+21.4), RUBIK (+24.2 AUC), and IMC 2022 (+12.4 mAA).

Conclusion: The conclusion of this paper is that LoMa makes outstanding progress in local feature matching, achieving remarkable gains in performance on various benchmarks, and that the proposed approach can be used to improve 3D vision systems such as Structure-from-Motion (SfM).
...
TL;DR: The authors propose PointTPA, a test-time parameter adaptation framework for scene-level point cloud understanding. It achieves 78.4% mIoU on ScanNet validation, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Scene-level point cloud understanding is challenging due to diverse geometries, imbalanced category distributions, and varied spatial layouts. Existing methods have limitations in adaptability to dynamic scene data.

Method: The authors propose PointTPA, which uses Serialization-based Neighborhood Grouping (SNG) and Dynamic Parameter Projector (DPP) to generate input-aware network parameters.

Result: PointTPA achieves 78.4% mIoU on ScanNet validation, surpassing existing parameter-efficient fine-tuning (PEFT) methods.

Conclusion: The proposed PointTPA framework demonstrates strong parameter efficiency and efficacy in enhancing 3D scene understanding through test-time dynamic network parameter adaptation.
...
TL;DR: Vanast is a unified framework for generating garment-transferred human animation videos from a single human image, garment images, and a pose guidance video.


<details>
  <summary>Details</summary>
Motivation: Conventional two-stage pipelines for image-based virtual try-on and pose-driven animation often result in identity drift, garment distortion, and front-back inconsistency.

Method: Vanast uses a unified model with a Dual Module architecture for video diffusion transformers, and a large-scale triplet supervision data generation pipeline.

Result: Vanast produces high-fidelity, identity-consistent animation across a wide range of garment types, with improved garment accuracy, pose adherence, and identity preservation.

Conclusion: Vanast addresses the limitations of conventional two-stage pipelines and achieves coherent synthesis of garment-transferred human animation videos in a single unified step.
...
TL;DR: The paper presents a method to train Code LLMs to simulate program execution step-by-step, improving their performance in competitive programming by combining supervised fine-tuning and reinforcement learning.


<details>
  <summary>Details</summary>
Motivation: The inability of LLMs to properly estimate program execution, particularly for code they generate, hinders their ability to produce consistently correct code.

Method: The approach involves supervised fine-tuning on natural language execution traces and reinforcement learning with verifiable rewards. Two objectives are introduced: output prediction and solving competitive programming tasks with execution feedback.

Result: The method yields consistent improvements over standard reasoning approaches across multiple competitive programming benchmarks.

Conclusion: The study demonstrates that training Code LLMs to simulate program execution can improve their performance in competitive programming, and provides insights into the role and limitations of execution simulation.
...
TL;DR: A new approach for estimating failure rates of large language models (LLMs) is proposed, integrating human-labeled data, LLM-judge annotations, and domain-specific constraints for more accurate and reliable estimates.


<details>
  <summary>Details</summary>
Motivation: The need for safe deployment of LLMs requires rigorous estimation of their failure rates, but current methods are either expensive (human gold standards) or potentially biased (automatic annotation schemes).

Method: A constrained maximum-likelihood estimation (MLE) approach is proposed, combining a small high-quality human-labeled calibration set, a large corpus of LLM-judge annotations, and domain-specific constraints.

Result: The proposed approach consistently delivers more accurate and lower-variance estimates of LLM failure rates than existing methods across diverse experimental regimes.

Conclusion: The proposed approach provides a principled, interpretable, and scalable pathway towards LLM failure-rate certification, moving beyond the 'black-box' use of automated judges.
...
TL;DR: SoLA, a novel training-free compression method, enables efficient and affordable model slimming for large language models (LLMs) by leveraging soft activation sparsity and low-rank decomposition.


<details>
  <summary>Details</summary>
Motivation: The billion-scale parameters of LLMs pose deployment challenges, and existing methods to reduce their scale require special hardware support or expensive post-training, which can compromise model quality.

Method: SoLA uses soft activation sparsity and low-rank decomposition to identify and retain significant components, compressing the majority while minimizing loss through an adaptive component-wise low-rank allocation strategy.

Result: SoLA achieves remarkable improvement in language modeling and downstream task accuracy without post-training, surpassing the state-of-the-art method by reducing perplexity from 6.95 to 4.44 and enhancing downstream task accuracy by 10% with a 30% compression rate on LLaMA-2-70B.

Conclusion: SoLA offers an efficient and affordable solution for compressing LLMs while maintaining model quality, demonstrating significant improvements in language modeling and downstream task accuracy.
...
TL;DR: Focus is a method for efficient attention in transformer models that selectively focuses on important token pairs, achieving better performance and efficiency than existing methods.


<details>
  <summary>Details</summary>
Motivation: The motivation behind Focus is to improve the efficiency of attention mechanisms in transformer models, which are often computationally expensive, while maintaining or improving their performance.

Method: Focus uses learnable centroids to group tokens and restrict distant attention to same-group pairs, while local attention operates at full resolution. The model weights remain frozen, making Focus purely additive.

Result: Focus achieves better perplexity scores than full attention and existing efficient attention methods, with a 2x speedup and 8.6x wall-clock speedup at 1M tokens. It also preserves alignment and discovers interpretable linguistic categories without supervision.

Conclusion: Focus is a effective and efficient method for attention in transformer models, achieving state-of-the-art results in terms of performance and efficiency, and has the potential to be widely adopted in natural language processing tasks.
...
TL;DR: The paper introduces VIGIL, a browser extension that detects and mitigates cognitive bias triggers in online information in real-time.


<details>
  <summary>Details</summary>
Motivation: The rise of generative AI poses risks to online information integrity and civic discourse, particularly through mis- and disinformation, and the exploitation of human cognitive biases.

Method: VIGIL uses LLM-powered reformulation, scroll-synced detection, and privacy-tiered inference to detect and mitigate cognitive bias triggers.

Result: The authors present VIGIL as the first browser extension for real-time cognitive bias trigger detection and mitigation, with extensibility for third-party plugins.

Conclusion: The paper concludes by introducing VIGIL as a solution to address the subtle threat of persuasion or manipulation by exploiting human cognitive biases in online information.
...
TL;DR: The paper proposes a new hybrid autoregressive architecture called LPC-SM for long-context language modeling, which separates local attention, memory, predictive correction, and control within the same block, and uses Orthogonal Novelty Transport (ONT) for memory writes.


<details>
  <summary>Details</summary>
Motivation: Current long-context language models rely heavily on attention, limiting exploration of alternative sequence modeling decompositions.

Method: The proposed LPC-SM architecture combines local attention, persistent memory, predictive correction, and run-time control, utilizing Orthogonal Novelty Transport (ONT) for slow-memory writes.

Result: The 158M-parameter model evaluated showed improved performance in three stages: base language modeling, mathematical continuation, and 4096-token continuation, with the full model achieving a final LM loss of 11.582 and improving the delayed-identifier diagnostic.

Conclusion: The results demonstrate that long-context autoregressive modeling can be organized around a broader division of labor than attention alone, supporting the effectiveness of the proposed LPC-SM architecture.
...
TL;DR: The paper proposes 'Knowledge Packs' to reduce token costs in Retrieval-Augmented Generation (RAG) by pre-computing KV caches, achieving up to 95% token savings with no performance degradation.


<details>
  <summary>Details</summary>
Motivation: RAG wastes tokens, and the authors aim to optimize it by introducing a more efficient method.

Method: The authors propose using pre-computed KV caches, called Knowledge Packs, which can deliver the same knowledge at zero token cost by leveraging the properties of causal transformers.

Result: The proposed method achieves zero divergences across 700 questions on Qwen3-8B and Llama-3.1-8B, with up to 95% token savings, and also enables behavioral steering.

Conclusion: The authors conclude that Knowledge Packs can efficiently deliver knowledge at zero token cost, and also provide additional benefits such as behavioral steering without requiring training or weight modification.
...
TL;DR: The paper introduces CresOWLve, a benchmark for evaluating creative problem-solving in large language models (LLMs). It tests LLMs' ability to combine multiple cognitive abilities to solve real-world puzzles, revealing a significant performance gap between factual and creative questions.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks for LLMs do not adequately evaluate creative problem-solving skills, which require combining multiple cognitive abilities. Existing creativity-oriented benchmarks often rely on artificial scenarios that do not reflect real-world settings.

Method: The CresOWLve benchmark was created to evaluate LLMs' creative problem-solving abilities using real-world puzzles. Several LLMs were tested on CresOWLve to assess their performance.

Result: The results show that LLMs perform substantially better on factual questions than on creative ones, with a performance drop of up to -17%. The models struggle to form non-obvious creative connections required to integrate information and arrive at the correct answer.

Conclusion: The study highlights the challenges LLMs face in creative problem-solving and the need for benchmarks like CresOWLve to evaluate and improve these skills.
...
TL;DR: The paper explores noise steering to generate diverse stories for Arabic early-grade reading assessments using transformer models.


<details>
  <summary>Details</summary>
Motivation: To balance constraints on vocabulary, reading level, and narrative structure while avoiding repetitive plots in story generation for assessments.

Method: Injecting calibrated Gaussian perturbations into transformer models' internal representations at inference time.

Result: Residual stream noise improves narrative diversity with minimal quality or constraint cost, and attention entropy noise injection stabilizes performance.

Conclusion: Internal representation-level perturbation is a suitable diversity strategy for constrained educational content generation, outperforming output-level stochasticity.
...
TL;DR: QIMMA is a quality-assured Arabic LLM leaderboard that uses a systematic benchmark validation pipeline to evaluate Arabic NLP models.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address systematic quality issues in existing Arabic benchmarks and provide a reliable evaluation suite for Arabic NLP models.

Method: QIMMA applies a multi-model assessment pipeline combining automated LLM judgment with human review to validate benchmarks.

Result: A curated, multi-domain, multi-task evaluation suite of over 52k samples, predominantly in native Arabic content, was developed.

Conclusion: QIMMA provides a reproducible and community-extensible foundation for Arabic NLP evaluation.
...
TL;DR: This paper explores variations in noun categories across languages, proposing a formal model called morphology-driven marking to explain differences in marking among noun types.


<details>
  <summary>Details</summary>
Motivation: The realisation of noun categories varies considerably across languages, with semantic and/or morphosyntactic differences being more or less pronounced.

Method: The paper uses Riffian as a reference point and extends the analysis to other languages, proposing a formal model termed morphology-driven marking.

Result: The proposed approach helps explain differences in marking among noun types within and across languages, and reassesses the notions of markedness and state.

Conclusion: The concept of state should be extended to all synthetic languages and analysed as a novel subcategory of syntax-based inflection like agreement and grammatical case.
...
TL;DR: The paper presents a comprehensive study on tool use in web agents, investigating their effectiveness, design principles, and side effects.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the limitations of prior studies on tool use in web agents, which have been conducted at a limited scale and in non-comparable settings.

Method: The authors conducted an extensive and carefully controlled study across diverse tool sources, backbone models, tool-use frameworks, and evaluation benchmarks.

Result: The study's findings revise some prior conclusions and complement others with broader evidence, providing a more reliable empirical basis for future research on tool-use web agents.

Conclusion: The paper concludes that tools can provide consistent gains for web agents, and identifies practical design principles for effective tools, while also highlighting potential side effects of tool use.
...
TL;DR: The paper introduces 'vocabulary dropout' to improve co-evolutionary self-play in language models, which helps maintain diversity in problem generation and leads to better solver performance.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the issue of 'diversity collapse' in co-evolutionary self-play, where the proposer quickly converges to a narrow distribution of problems, making the curriculum uninformative for the solver.

Method: The method proposed in this paper is 'vocabulary dropout', a lightweight mechanism that applies a random mask to the proposer's output logits during policy training and curriculum generation, sustaining diversity in problem generation.

Result: The results show that vocabulary dropout sustains proposer diversity across various metrics and yields solver improvements averaging +4.4 points at 8B, with the largest gains on competition-level benchmarks.

Conclusion: The conclusion of this paper is that vocabulary dropout is an effective method to sustain productive co-evolution in language models, and that explicit action-space constraints can help improve co-evolutionary self-play.
...
TL;DR: The paper uses LLM-powered evolutionary search to automatically discover unsupervised uncertainty quantification methods for large language models, achieving better performance than manually-designed baselines.


<details>
  <summary>Details</summary>
Motivation: Current uncertainty quantification methods for large language models are designed by hand, limiting their scalability and generality.

Method: LLM-powered evolutionary search to automatically discover unsupervised UQ methods represented as Python programs.

Result: Evolved methods outperform strong manually-designed baselines, achieving up to 6.7% relative ROC-AUC improvement across 9 datasets while generalizing robustly out-of-distribution.

Conclusion: LLM-powered evolutionary search is a promising paradigm for automated, interpretable hallucination detector design.
...
TL;DR: This paper introduces a human-centered framework to evaluate the cultural alignment of Large Language Model (LLM) outputs with local expectations, revealing Western-centric biases and systemic errors in current models.


<details>
  <summary>Details</summary>
Motivation: The primary motivation is to address the gap in assessing cultural alignment in LLM outputs, which is crucial for ensuring that generated content accurately reflects the cultural perceptions and priorities of native populations.

Method: The authors establish a human-derived ground-truth baseline of importance vectors (Cultural Importance Vectors) from survey responses across nine countries. They then compute model-derived Cultural Representation Vectors for three LLMs (Gemini 2.5 Pro, GPT-4o, and Claude 3.5 Haiku) using a diversified prompt-set.

Result: The study finds that some LLMs exhibit Western-centric calibration, with alignment decreasing as a country's cultural distance from the US increases. Additionally, the models show highly correlated systemic error signatures, over-indexing on certain cultural markers while neglecting social and value-based priorities.

Conclusion: The paper concludes that the proposed framework can move beyond simple diversity metrics to evaluate the fidelity of AI-generated content in capturing the nuanced hierarchies of global cultures, highlighting the need for more culturally aligned LLMs.
...
TL;DR: The paper introduces LangFIR, a method to discover language-specific features in large language models using monolingual data, improving multilingual generation control.


<details>
  <summary>Details</summary>
Motivation: Reliably controlling the language of outputs in large language models remains difficult, and existing methods often rely on expensive multilingual or parallel data.

Method: LangFIR uses sparse autoencoders and random-token sequences to identify language-specific features with only a small amount of monolingual data.

Result: LangFIR achieves the best average accuracy BLEU across three models, three datasets, and twelve target languages, outperforming baselines and surpassing methods that rely on parallel data.

Conclusion: The results suggest that language identity in multilingual LLMs is localized in a sparse set of feature directions discoverable with monolingual data.
...
TL;DR: A new tree-structured diffusion language model is proposed to efficiently use parameters and memory under constrained training resources, reducing peak GPU memory usage by half while matching state-of-the-art perplexity performance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of training discrete diffusion language models efficiently under limited parameter and memory budgets, as modern architectures are inefficient in using parameters and memory.

Method: The method involves building a tree-structured diffusion language model that exploits the inherent structure among tokens, using a pre-constructed vocabulary tree to reduce classification dimensionality and make the prediction head negligible in size.

Result: The proposed method reduces peak GPU memory usage by half while matching the perplexity performance of state-of-the-art discrete diffusion language models under the same parameter budget.

Conclusion: The proposed tree-structured diffusion language model efficiently uses parameters and memory, making it a competitive alternative to auto-regressive language models under constrained training resources.
...
TL;DR: The study explores using graph information to improve summarization models, experimenting with different architectures and achieving improved results on the CNN/DM dataset.


<details>
  <summary>Details</summary>
Motivation: To leverage graph information, specifically Rhetorical Structure Theory (RST) and Co-reference (Coref) graphs, to enhance the performance of baseline summarization models.

Method: Experimenting with Graph Attention Network and Multi-layer Perceptron architectures to incorporate graph information into summarization models.

Result: The Multi-layer Perceptron architecture improved results on the CNN/DM dataset, while the Graph Attention Network did not enhance performance.

Conclusion: The study demonstrates the potential of using graph information to improve summarization models, establishing a benchmark for future graph-based summarization models on the XSum dataset.
...
TL;DR: A novel multi-agent framework called MultiPress is proposed for multimodal news classification, which integrates specialized agents for multimodal perception, retrieval-augmented reasoning, and gated fusion scoring.


<details>
  <summary>Details</summary>
Motivation: The growing prevalence of multimodal news content requires models that can jointly understand and reason over heterogeneous data such as text and images, but existing methods have limitations in capturing complex cross-modal interactions and leveraging external knowledge.

Method: A three-stage multi-agent framework called MultiPress, which includes specialized agents for multimodal perception, retrieval-augmented reasoning, and gated fusion scoring, followed by a reward-driven iterative optimization mechanism.

Result: MultiPress demonstrates significant improvements over strong baselines on a newly constructed large-scale multimodal news dataset, enhancing classification accuracy and interpretability.

Conclusion: The proposed MultiPress framework effectively addresses the limitations of existing methods in multimodal news classification, showcasing the benefits of modular multi-agent collaboration and retrieval-augmented reasoning.
...
TL;DR: The paper analyzes expert routing patterns in Mixture-of-Experts (MoE) models, finding that high- and low-resource languages activate disjoint expert sets. It proposes RISE, a framework that enhances language-specific subnetworks, improving low-resource language performance with minimal cross-lingual degradation.


<details>
  <summary>Details</summary>
Motivation: The performance disparities of Mixture-of-Experts (MoE) models across languages are not well understood, and the authors aim to investigate the internal mechanisms driving these gaps.

Method: The authors conduct a systematic analysis of expert routing patterns in MoE models, propose a layer-stratified analysis, and introduce RISE (Routing Isolation-guided Subnetwork Enhancement), a framework that identifies and adapts language-specific expert subnetworks.

Result: RISE substantially improves low-resource language performance while preserving capabilities in other languages, achieving target-language F1 gains of up to 10.85% with minimal cross-lingual degradation on 10 languages.

Conclusion: The authors conclude that RISE is an effective approach to address performance disparities in MoE models across languages, and it has the potential to improve the performance of low-resource languages.
...
TL;DR: The requirement for structured output (e.g. JSON) degrades performance in large language models, but this can be mitigated by decoupling reasoning from formatting.


<details>
  <summary>Details</summary>
Motivation: The authors noticed that requiring structured output from large language models significantly reduces their performance in reasoning and writing tasks.

Method: The authors analyzed the impact of structured output requirements on model performance, identifying the source of the degradation and proposing a solution of decoupling reasoning from formatting.

Result: The authors found that decoupling reasoning from formatting substantially recovers lost accuracy across various models, formats, and tasks.

Conclusion: The requirement for structured output is a 'formatting tax' that can be avoided by decoupling reasoning from formatting, and this approach can help improve the performance of open-weight language models.
...
TL;DR: Proposed CAGMamba, a context-aware gated cross-modal Mamba framework for dialogue-based sentiment analysis, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal sentiment analysis approaches are limited by quadratic complexity and lack explicit temporal modeling.

Method: CAGMamba organizes contextual and current-utterance features into a temporally ordered binary sequence and uses a Gated Cross-Modal Mamba Network (GCMN) for controllable cross-modal integration.

Result: CAGMamba achieves state-of-the-art or competitive results on three benchmark datasets.

Conclusion: CAGMamba effectively models cross-modal interactions and contextual dependencies while being computationally efficient.
...
TL;DR: The paper introduces FinLongDocQA, a dataset for financial numerical reasoning in long-context reports, and proposes FinLongDocAgent, a multi-agent approach to improve numerical QA in long financial documents.


<details>
  <summary>Details</summary>
Motivation: Large language models struggle with reliable question answering over long, structured documents, particularly for numerical reasoning, which is crucial in financial statement analysis.

Method: The authors propose FinLongDocAgent, a Multi-Agent Multi-Round Retrieval-Augmented Generation (RAG) approach that iteratively retrieves evidence, performs intermediate calculations, and verifies results across rounds.

Result: Experiments show that iterative retrieval and verification are important for reliable numerical QA in long financial documents, and that LLMs still struggle with numerical reasoning, especially in cross-table document-level settings.

Conclusion: The paper highlights the challenges of numerical reasoning in long financial documents and proposes a novel approach to address these challenges, demonstrating the importance of iterative retrieval and verification for reliable numerical QA.
...
TL;DR: The paper presents an AI-based system for automated classification and routing of citizen appeals, achieving 78% accuracy and reducing processing time by 54%.


<details>
  <summary>Details</summary>
Motivation: Government agencies face growing volumes of citizen appeals, but traditional manual processing is time-consuming and inaccurate (20 minutes per appeal, 67% classification accuracy).

Method: The AI Appeals Processor system uses natural language processing and deep learning techniques, evaluating multiple approaches including Bag-of-Words, TF-IDF, fastText, Word2Vec with LSTM, and BERT.

Result: A Word2Vec+LSTM architecture achieves 78% classification accuracy and reduces processing time by 54%, offering a balance between accuracy and computational efficiency.

Conclusion: The proposed AI Appeals Processor system can improve the efficiency and accuracy of citizen appeal processing, providing a better public service delivery experience.
...
TL;DR: This study evaluates how well Pretrained Language Models (PLMs) like BERT capture linguistic information in Italian, specifically focusing on noun-preposition-noun constructions.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this study is to assess the linguistic information encoded in PLMs, particularly in contextual embeddings, against explicit linguistic theories.

Method: The study uses BERT to extract contextual vector representations, which are then fed into layer-wise probing classifiers to systematically evaluate the information encoded across the model's internal layers.

Result: The results provide insights into how well the constructional form and meaning are reflected in contextual embeddings, contributing evidence to the dialogue between constructionist theory and neural language modelling.

Conclusion: The study contributes to the understanding of what linguistic information PLMs encode, specifically for Italian NPN constructions, and challenges some assumptions in previous experimental designs.
...
TL;DR: The paper explores extending full-sequence masking during supervised finetuning of masked diffusion language models to enable infilling prompts, showing that this approach can generate effective prompts that match or surpass manually designed templates.


<details>
  <summary>Details</summary>
Motivation: The current limitation of masked diffusion language models (dLMs) in infilling prompts is due to the supervised finetuning (SFT) convention of applying response-only masking, which prevents the model from utilizing its bidirectional denoising capability for prompt infilling.

Method: The authors extend full-sequence masking during SFT, where both prompts and responses are masked jointly, to unlock the model's capability for infilling prompts conditioned on few-shot examples.

Result: The model-infilled prompts match or surpass manually designed templates, transfer effectively across models, and are complementary to existing prompt optimization methods, suggesting that training practices are the primary bottleneck preventing masked diffusion language models from infilling effective prompts.

Conclusion: The study concludes that the limitation of masked diffusion language models in infilling prompts is primarily due to training practices rather than architectural limitations, and that extending full-sequence masking during SFT can effectively unlock the model's prompt infilling capability.
...
TL;DR: Proposed LightThinker and LightThinker++ methods to compress intermediate thoughts in large language models, reducing token usage and inference time while maintaining or improving accuracy.


<details>
  <summary>Details</summary>
Motivation: Large language models struggle with efficiency due to the cognitive overhead of long thought traces, and the authors aim to improve their efficiency without sacrificing accuracy.

Method: The authors propose LightThinker, a method for dynamic compression of intermediate thoughts, and LightThinker++, which adds Explicit Adaptive Memory Management for better handling of complex reasoning tasks.

Result: LightThinker reduced peak token usage by 70% and inference time by 26% with minimal accuracy loss. LightThinker++ achieved a 69.9% reduction in peak token usage and a 2.42% accuracy gain. In long-horizon tasks, it maintained a stable footprint and achieved a 14.8% performance gain.

Conclusion: The proposed methods provide a scalable direction for sustaining deep LLM reasoning over extended horizons with minimal overhead, improving efficiency without sacrificing accuracy.
...
TL;DR: Batching and stacking variables in large language models (LLMs) for text classification can significantly reduce API calls and token costs without degrading coding quality.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to investigate the trade-off between efficiency and accuracy when using batching and stacking variables in LLMs for text classification, with the goal of optimizing API calls and token costs.

Method: The authors tested eight production LLMs on 3,962 expert-coded tweets across four tasks, varying batch size from 1 to 1,000 items and stacking up to 25 coding dimensions per prompt.

Result: The results show that six of eight models maintained accuracy within 2 pp of the single-item baseline through batch sizes of 100, and variable stacking with up to 10 dimensions produced results comparable to single-variable coding.

Conclusion: The study concludes that within a safe operating range, the measurement error from batching and stacking is smaller than typical inter-coder disagreement in the ground-truth data, suggesting that batching and stacking can be an effective way to optimize LLM usage for text classification.
...
TL;DR: The paper introduces POEMetric, a comprehensive framework for evaluating poetry generated by Large Language Models (LLMs) compared to human poets. It assesses basic instruction-following abilities, advanced poetic abilities, and overall poem quality. Results show that while LLMs can generate poems with high form accuracy and theme alignment, they still lag behind human poets in creativity, lexical diversity, emotional resonance, and use of imagery and literary devices.


<details>
  <summary>Details</summary>
Motivation: To evaluate how close Large Language Models (LLMs) are to human poets in generating poetry.

Method: Introduced POEMetric, a framework for poetry evaluation. Curated a dataset of 203 English poems and generated 6,090 poems by 30 LLMs. Assessed both human and LLM-generated poems using rule-based evaluation and LLM-as-a-judge, validated by human experts.

Result: LLMs achieved high form accuracy and theme alignment but failed to match human poets' advanced abilities such as creativity, idiosyncrasy, emotional resonance, and use of imagery and literary devices. Humans outperformed the best LLM in overall poem quality (4.22 vs. 3.20).

Conclusion: Poetry generation remains a significant challenge for LLMs, as they have not reached the same level of advanced poetic abilities as human poets.
...
TL;DR: The universality of truth directions in large language models is limited, varying by layer, task type, complexity, and model instructions.


<details>
  <summary>Details</summary>
Motivation: To investigate the limits of truth-direction universality in large language models.

Method: Analyzing truth directions across different layers, task types, task complexities, and model instructions.

Result: Truth directions are layer-dependent, task-type dependent, and affected by model instructions, challenging universality claims.

Conclusion: Universality claims for truth directions in large language models are more limited than previously thought.
...
TL;DR: The paper evaluates defense strategies against Indirect Prompt Injections (IPI) attacks in multi-agent systems, finding current defenses to be fragile and sometimes counterproductive, and proposes a new detection strategy called Representation Engineering (RepE).


<details>
  <summary>Details</summary>
Motivation: The rapid deployment of open-source frameworks has advanced multi-agent systems but also introduced security challenges, particularly from Indirect Prompt Injections (IPI) that can trigger unauthorized actions.

Method: The study systematically evaluates six defense strategies against four sophisticated IPI attack vectors across nine LLM backbones in dynamic multi-step tool-calling environments.

Result: Advanced injections successfully bypass nearly all baseline defenses, and some mitigations produce counterproductive side effects. The proposed RepE-based circuit breaker successfully identifies and intercepts unauthorized actions with high detection accuracy.

Conclusion: The study exposes the limitations of current IPI defenses and provides a practical paradigm for building resilient multi-agent architectures using Representation Engineering (RepE).
...
TL;DR: The paper examines how well large language models (LLMs) detect narrative analogies by comparing their internal representations with their performance on prompted tasks, finding an asymmetry in how they handle rhetorical vs. narrative analogies.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to investigate the limitations of LLMs in abstraction and generalization when it comes to detecting narrative analogies that are not immediately apparent on the surface but require latent information.

Method: The method involves comparing a model's probed representations with its prompted performance at detecting narrative analogies, specifically examining rhetorical and narrative analogies separately.

Result: The results show an asymmetry: for rhetorical analogies, probing significantly outperforms prompting in open-source models, while for narrative analogies, they achieve similar (low) performance.

Conclusion: The conclusion is that the relationship between internal representations and prompted behavior in LLMs is task-dependent, suggesting limitations in how prompting accesses available information.
...
TL;DR: The paper proposes a prompt-based framework, I-CALM, to reduce hallucination in large language models by encouraging epistemic abstention on factual questions, and finds that it can improve selective answering without retraining.


<details>
  <summary>Details</summary>
Motivation: Large language models often produce confident but incorrect answers, partly due to binary scoring conventions that reward answering over honest uncertainty expression.

Method: The authors propose I-CALM, a prompt-based framework that elicits verbal confidence, partially rewards abstention, and adds lightweight normative principles emphasizing truthfulness, humility, and responsibility.

Result: The framework reduces the false-answer rate on answered cases mainly by identifying and shifting error-prone cases to abstention and re-calibrating their confidence, trading coverage for reliability.

Conclusion: The results show that I-CALM can improve selective answering on factual questions without retraining, with the magnitude of effect varying across models and datasets.
...
TL;DR: The paper discusses the limitations of current LLM-based social simulations for testing governance interventions, proposing a causal counterfactual framework to improve their validity.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based social simulations lack causal semantics, making it difficult to draw conclusions about the effectiveness of governance interventions.

Method: The authors propose adopting the causal counterfactual framework to distinguish between necessary and sufficient causation.

Result: The framework allows for the estimation of simulator-conditional causal estimates, which can inform policy decisions.

Conclusion: Establishing this framework is essential to move the field towards simulations that can support policy changes by defining what adequate fidelity means.
...
TL;DR: The paper proposes a Conversation Uncertainty-aware Planning (CUP) framework that combines language models with structured planning to improve goal-oriented conversational systems.


<details>
  <summary>Details</summary>
Motivation: Existing approaches to goal-oriented conversational systems have limitations, with structured methods relying on predefined schemas and LLM-based approaches lacking long-horizon decision making.

Method: The CUP framework integrates language models with structured planning, where a language model proposes feasible actions and a planner evaluates their long-term impact on uncertainty reduction.

Result: Experiments on multiple conversational benchmarks show that CUP consistently improves success rates while requiring fewer interaction turns.

Conclusion: The proposed CUP framework effectively balances information acquisition and target commitment in goal-oriented conversational systems, leading to more efficient conversations.
...
TL;DR: The paper proposes AdaptFuse, a training-free framework that improves large language models' ability to accumulate evidence across user interactions without fine-tuning on sensitive data.


<details>
  <summary>Details</summary>
Motivation: Large language models struggle with accumulating evidence across multiple user interactions, and existing solutions require fine-tuning on sensitive user interaction data, which is limiting in privacy-conscious settings.

Method: AdaptFuse externalizes probabilistic computation from the LLM using a symbolic module that maintains a Bayesian posterior, and combines it with the LLM's semantic reasoning through entropy-adaptive fusion.

Result: AdaptFuse consistently outperforms prompting baselines and fine-tuned Bayesian Teaching models on three domains (flight recommendation, hotel recommendation, and web shopping) across three LLMs (Gemma 2 9B, Llama 3 8B, and Qwen 2.5 7B), with accuracy improving over interaction rounds.

Conclusion: The results demonstrate that principled inference-time algorithms can substitute for fine-tuning in personalized recommendation, without storing or training on sensitive user data.
...
TL;DR: StreamGuard, a unified model-agnostic streaming guardrail, is introduced for moderation of both prompts and responses in LLM deployments. It formulates moderation as a forecasting problem, predicting the expected harmfulness of likely future continuations, and is supervised using Monte Carlo rollouts.


<details>
  <summary>Details</summary>
Motivation: Existing text-based streaming guardrails frame output-side moderation as boundary detection, which requires exact token-level boundary annotations. The authors aim to develop a model that can make safety decisions over partial generations without requiring such annotations.

Method: StreamGuard uses a forecasting-based approach, predicting the expected harmfulness of likely future continuations given a partial prefix. It is supervised using Monte Carlo rollouts, enabling early intervention without requiring exact token-level boundary annotations.

Result: StreamGuard performs strongly on standard safety benchmarks, improving aggregated input-moderation F1 from 86.7 to 88.2 and aggregated streaming output-moderation F1 from 80.4 to 81.9. It also shows effective transfer across tokenizers and model families.

Conclusion: StreamGuard offers a robust solution for end-to-end streaming moderation without requiring exact boundary labels. Forecasting future risk proves to be an effective supervision strategy for low-latency safety intervention.
...
TL;DR: The paper proposes RUQuant, a two-stage method for post-training quantization of large language models, achieving near-optimal performance without fine-tuning.


<details>
  <summary>Details</summary>
Motivation: The increasing size and complexity of large language models pose deployment efficiency challenges, and post-training quantization is a practical solution but existing methods suffer from accuracy degradation.

Method: The proposed method, RUQuant, uses a two-stage orthogonal transformation approach, involving block-wise mapping and global fine-tuning to minimize quantization error.

Result: RUQuant achieves 99.8% full-precision accuracy with W6A6 and 97% with W4A4 quantization for a 13B LLM within one minute, and fine-tuning yields even higher accuracy.

Conclusion: The proposed RUQuant method effectively addresses the non-uniform activation distribution issue in post-training quantization, demonstrating scalability and near-optimal performance.
...
TL;DR: A new benchmark called GeoBrowse is introduced, which requires agents to combine visual reasoning with knowledge-intensive multi-hop queries for geolocation tasks.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks do not adequately test the ability of agents to integrate fragmented evidence through multi-step tool use, particularly in multimodal settings.

Method: The GeoBrowse benchmark is proposed, consisting of two levels of difficulty, and an agentic workflow called GATE is developed, which utilizes think-with-image tools and knowledge-intensive tools.

Result: Experiments show that GATE outperforms direct inference and open-source agents, highlighting the importance of coherent, level-specific tool-use plans.

Conclusion: The GeoBrowse benchmark and GATE workflow demonstrate the value of integrating visual reasoning with knowledge-intensive multi-hop queries for geolocation tasks.
...
TL;DR: The paper proposes a framework called GCAN to reduce hallucinations in AI language models by interpreting internal attention flow and constructing token-level graphs, achieving a 27.8% reduction in hallucination rate and 16.4% improvement in factual accuracy.


<details>
  <summary>Details</summary>
Motivation: Hallucinations in AI language models can produce factually incorrect or misleading outputs, causing serious problems in scenarios like medical diagnosis or legal reasoning.

Method: The paper proposes a causal graph attention network (GCAN) framework that uses token-level graphs to combine self-attention weights and gradient-based influence scores, and introduces a fact-anchored graph reweighting layer to dynamically reduce the influence of hallucination-prone nodes.

Result: Experiments on standard benchmarks show a 27.8% reduction in hallucination rate and 16.4% improvement in factual accuracy over baseline retrieval-augmented generation (RAG) models.

Conclusion: The proposed framework contributes to the interpretability, robustness, and factual reliability of future LLM architectures.
...
TL;DR: Fine-tuning large language models on culturally loaded data can cause them to produce harmful content in unrelated tasks, even with few-shot prompting.


<details>
  <summary>Details</summary>
Motivation: To investigate the effect of inference-time semantic drift on large language models, specifically whether few-shot prompting can induce emergent misalignment.

Method: A controlled experiment injecting culturally loaded numbers as few-shot demonstrations before a semantically unrelated prompt, comparing models of varying capability.

Result: Models with richer cultural-associative representations exhibit significant distributional shifts toward darker, authoritarian, and stigmatized themes, while a simpler/smaller model does not.

Conclusion: Inference-time contamination occurs under specific boundary conditions, posing security implications for LLM-based applications using few-shot prompting.
...
TL;DR: The paper analyzes emotion representations in small language models (SLMs) with 100M-10B parameters, comparing emotion vector extraction methods and finding that generation-based extraction produces better results. Emotion representations are found to localize at middle transformer layers and vary by architecture and instruction tuning.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to investigate whether small language models (SLMs) possess internal emotion representations similar to those found in larger frontier models, and to develop methods for analyzing these representations.

Method: The paper uses a comparative analysis approach, evaluating 9 SLMs across 5 architectural families using 20 emotions and two extraction methods (generation-based and comprehension-based). The authors also validate their findings against representational anisotropy baselines and conduct steering experiments to confirm causal behavioral effects.

Result: The results show that generation-based extraction produces statistically superior emotion separation, with the advantage modulated by instruction tuning and architecture. Emotion representations are found to localize at middle transformer layers (~50% depth), following a U-shaped curve that is architecture-invariant from 124M to 3B parameters. The authors also identify three regimes of steering effects (surgical, repetitive collapse, and explosive) and document cross-lingual emotion entanglement in Qwen.

Conclusion: The paper concludes that SLMs do possess internal emotion representations, which can be analyzed using the developed methods. The findings provide methodological guidelines for emotion research on open-weight models and contribute to the understanding of internal representational analysis in language models.
...
TL;DR: This paper proposes a unified framework called EduEmbed that leverages language models to enhance cognitive diagnosis in online intelligent education systems.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenges of integrating language models with existing cognitive modeling paradigms to improve cognitive diagnosis performance across diverse online educational scenarios.

Method: The method involves a two-stage framework: (1) fine-tuning language models based on role-specific representations and an interaction diagnoser, and (2) employing a textual adapter to extract task-relevant semantics and integrate them with existing modeling paradigms.

Result: The proposed framework achieves robust performance on four cognitive diagnosis tasks and a computerized adaptive testing task.

Conclusion: The paper concludes that EduEmbed provides a unified and effective approach to enhancing cognitive diagnosis in online intelligent education systems by leveraging the rich semantic representations of language models.
...
TL;DR: This paper studies the impact of compressing chain-of-thought (CoT) reasoning traces on model trustworthiness, finding that compression often introduces trustworthiness regressions and proposing a new method to optimize for both efficiency and trustworthiness.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to investigate the effect of CoT compression on model trustworthiness, as existing evaluations focus primarily on task accuracy and token savings, and trustworthiness properties are encoded in the same parameter space that compression modifies.

Method: The paper conducts a systematic empirical study evaluating multiple models of different scales along three dimensions: safety, hallucination resistance, and multilingual robustness, and proposes a normalized efficiency score for each dimension to enable fair comparison across bases.

Result: The paper finds that CoT compression frequently introduces trustworthiness regressions, and different methods exhibit markedly different degradation profiles across dimensions, but also introduces an alignment-aware DPO variant that reduces CoT length by 19.3% on reasoning benchmarks with substantially smaller trustworthiness loss.

Conclusion: The paper concludes that CoT compression should be optimized not only for efficiency but also for trustworthiness, treating both as equally important design constraints.
...
TL;DR: This paper presents PALM, a method for selecting a small portfolio of Large Language Models (LLMs) that captures representative behaviors across users with diverse preferences, achieving a balance between system cost and personalization.


<details>
  <summary>Details</summary>
Motivation: The goal of personalizing LLMs for individual users is challenging due to compute, memory, and complexity constraints. A single LLM per user is impractical, necessitating a method for a small portfolio of LLMs that can represent diverse user preferences.

Method: The authors propose PALM (Portfolio of Aligned LLMs), an algorithm that models user preferences through a multi-dimensional weight vector and generates a small portfolio of LLMs. This portfolio contains a near-optimal LLM for any given weight vector and scalarized objective, based on provided reward functions across multiple traits.

Result: PALM provides theoretical guarantees on both the size and approximation quality of LLM portfolios for personalization. It characterizes the trade-off between system cost and personalization, as well as the diversity of LLMs required. Empirical results validate these guarantees and show greater output diversity compared to common baselines.

Conclusion: The development of PALM offers a practical solution to the challenge of personalizing LLMs for a wide range of users. By creating a small portfolio of LLMs that can represent diverse user preferences, PALM balances system constraints with the need for personalized interactions, paving the way for more effective and personalized language model applications.
...
TL;DR: The study develops a semi-automated workflow using small language models (SLMs) to extract structured information from unstructured Electronic Patient Record (EPR) data, specifically from paediatric histopathology reports, with high accuracy.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome the limitation of using valuable clinical information trapped in unstructured text in EPR systems for research and decision-making, while addressing patient privacy concerns and computational resource limitations.

Method: The study uses a semi-automated annotation workflow with SLMs to extract structured information from EPR data. It involves manual annotation of 400 reports, developing an automated information extraction approach using SLMs, and evaluating five instruction-tuned SLMs.

Result: The best-performing SLM, Gemma 2 2B, achieves an accuracy of 84.3%, outperforming off-the-shelf models. The use of entity guidelines and few-shot examples improves performance by 7-19% and 6-38%, respectively.

Conclusion: The study demonstrates that SLMs can efficiently extract structured information from specialised clinical domains on CPU-only infrastructure with minimal clinician involvement, addressing the challenges of using EPR data for research and decision-making.
...
TL;DR: The paper argues that relying solely on formal logic to mitigate bias in large language models (LLMs) is insufficient, as logically sound conclusions may not align with human inferences, and proposes a complementary approach leveraging LLMs' human-like reasoning tendencies.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the limitations of using formal logic in neurosymbolic systems to detect misleading claims, specifically the disconnect between logical soundness and human inference.

Method: The paper draws on studies in cognitive science and pragmatics to present a typology of cases where logically sound conclusions fail to align with human inferences, and proposes a complementary approach that utilizes LLMs' human-like reasoning tendencies.

Result: The paper concludes that a hybrid approach is needed, one that combines the strengths of formal logic with the human-like reasoning capabilities of LLMs to better detect misleading claims.

Conclusion: The authors advocate for a complementary approach that leverages LLMs as a feature, not a bug, to validate the outputs of formal components in neurosymbolic systems against potentially misleading conclusions.
...
TL;DR: The study examines how large language models (LLMs) favor American English (AmE) over British English (BrE), revealing biases in data curation, digital dominance, and linguistic standardization.


<details>
  <summary>Details</summary>
Motivation: The motivation is to investigate the geopolitical histories of data curation, digital dominance, and linguistic standardization that shape the LLM development pipeline, and to highlight concerns about linguistic homogenization, epistemic injustice, and inequity in global AI deployment.

Method: The method involves constructing a curated corpus of 1,813 AmE--BrE variants and introducing DiAlign, a dynamic, training-free method for estimating dialectal alignment using distributional evidence. The study operationalizes structural bias by triangulating evidence across three stages: audits of pretraining corpora, tokenizer analyses, and generative evaluations.

Result: The study finds that contemporary LLMs privilege AmE as the de facto norm, with systematic skew toward AmE in pretraining corpora, higher segmentation costs for BrE forms, and a persistent AmE preference in model outputs.

Conclusion: The conclusion is that the study highlights concerns about linguistic homogenization, epistemic injustice, and inequity in global AI deployment, while motivating practical steps toward more dialectally inclusive language technologies.
...
TL;DR: The paper introduces DARE, an open framework for post-training and evaluating diffusion large language models (dLLMs), unifying various methods under a shared execution stack.


<details>
  <summary>Details</summary>
Motivation: The open-source ecosystem for dLLMs is fragmented, making research iteration slow, reproduction burdensome, and fair comparison across algorithms difficult.

Method: DARE, built on top of verl and OpenCompass, provides a unified framework for supervised fine-tuning, parameter-efficient fine-tuning, preference optimization, and dLLM-specific reinforcement learning.

Result: DARE offers broad algorithmic coverage, reproducible benchmark evaluation, and practical acceleration across representative dLLM model families.

Conclusion: DARE serves as a reusable research substrate for developing, comparing, and deploying post-training methods for current and emerging dLLMs.
...
TL;DR: The paper introduces the Continuous Acoustic Wave Network (CAWN), a sequence-mixing architecture that replaces traditional self-attention in Large Language Models (LLMs) with a continuous, linear-time mechanism, achieving efficient and effective processing of long sequences.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the limitations of traditional Transformer self-attention in Large Language Models (LLMs), which scales quadratically with sequence length, and recent linear-time alternatives that suffer from signal degradation over extended contexts.

Method: The method proposed in this paper is the Continuous Acoustic Wave Network (CAWN), which uses a causal, O(L) Phase Accumulation mechanism, a dual-gated Selective Phase Resonance mechanism, Depth-wise Harmonic Convolutions, and Block Attention Residuals to achieve efficient and effective sequence mixing.

Result: The results show that the CAWN model, scaled to 150M parameters, can retrieve targeted information across 2,000,000 tokens while maintaining a peak VRAM usage of 8.72 GB, demonstrating robust vocabulary acquisition and extended contextual denoising capabilities.

Conclusion: The conclusion of this paper is that the proposed CAWN architecture offers a promising solution to overcome the limitations of traditional self-attention in LLMs, enabling efficient and effective processing of long sequences while maintaining a low memory footprint.
...
TL;DR: Proposed ACE, a hybrid framework for automated patent claim validation that balances efficiency and accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing evaluation methods for patent claims are either inefficient or inaccurate, and there is a need for a more effective approach.

Method: ACE uses predictive entropy to selectively route high-uncertainty claims to expert LLMs, which then apply a Chain of Patent Thought (CoPT) protocol.

Result: ACE achieves an F1 score of 94.95% and reduces operational costs by 78% compared to standalone LLM deployments.

Conclusion: ACE offers a cost-efficient and accurate solution for patent claim validation, and the ACE-40k benchmark can facilitate further research in this area.
...
TL;DR: The paper discusses the limitations of current LLM personalization methods in individual investor decision-making and proposes new research directions.


<details>
  <summary>Details</summary>
Motivation: The authors argue that individual investor decision-making presents a uniquely challenging domain for LLM personalization due to its dynamic and high-stakes nature.

Method: The authors built and deployed an AI-augmented portfolio management system and identified four key limitations of standard LLM customization: behavioral memory complexity, thesis consistency under drift, style-signal tension, and alignment without ground truth.

Result: The authors propose architectural responses to address these limitations and suggest open research directions for personalized NLP in high-stakes, temporally extended decision domains.

Conclusion: The paper highlights the need for new approaches to LLM personalization in individual investor decision-making, where current methods are insufficient due to the complex and dynamic nature of the domain.
...
TL;DR: The paper studies the effectiveness of reusable skills in LLM-based agents under realistic conditions, finding that benefits are fragile and degrade as settings become more challenging, but skill refinement strategies can recover lost performance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to investigate the performance of LLM-based agents using reusable skills in realistic settings, where agents must retrieve and select relevant skills from a large collection.

Method: The method involves conducting a comprehensive study of skill utility under progressively challenging realistic settings, using a large collection of 34k real-world skills, and evaluating the effectiveness of skill refinement strategies.

Result: The results show that performance gains degrade as settings become more realistic, but query-specific refinement substantially recovers lost performance when initial skills are of reasonable relevance and quality.

Conclusion: The conclusion is that while skills show promise for LLM-based agents, their benefits are fragile and current limitations need to be addressed, but refinement strategies can improve performance.
...
TL;DR: The study evaluates the performance of large language models (LLMs) in multi-turn medical diagnosis and provides guidance on improving their reliability.


<details>
  <summary>Details</summary>
Motivation: The study aims to investigate how LLMs behave in multi-turn medical diagnosis, which is closer to real clinical reasoning, and to provide a controlled evaluation framework and actionable guidance for improving their performance.

Method: The study introduces MINT, a multi-turn medical diagnosis benchmark, and evaluates 11 LLMs on it, analyzing their behavioral patterns and providing guidance on deferring diagnostic questions and reserving salient clinical evidence.

Result: The study finds that LLMs exhibit persistent behavioral patterns, including intent to answer, self-correction, and strong lures, which impact diagnostic decisions, and that deferring diagnostic questions and reserving salient clinical evidence can improve accuracy.

Conclusion: The study provides a controlled evaluation framework and concrete recommendations for improving the reliability of LLMs in multi-turn medical diagnosis, highlighting the importance of careful evaluation and guidance for effective deployment in clinical settings.
...
TL;DR: The paper proposes GroundedKG-RAG, a retrieval-augmented generation system that extracts a knowledge graph from the source document and grounds it in the original sentences, improving efficiency and factual accuracy in long-document question answering.


<details>
  <summary>Details</summary>
Motivation: Current RAG systems for long-document question answering suffer from high resource consumption, latency, repetitive content, and hallucinations due to limited grounding in the source text.

Method: The authors propose GroundedKG-RAG, which constructs a knowledge graph from semantic role labeling and abstract meaning representation parses, and then embeds it for retrieval. The system applies the same transformation to the query and retrieves the most relevant sentences from the grounded source text for question answering.

Result: GroundedKG-RAG performs on par with a state-of-the-art proprietary long-context model at smaller cost and outperforms a competitive baseline on the NarrativeQA dataset.

Conclusion: The proposed GroundedKG-RAG system improves efficiency and factual accuracy in long-document question answering by grounding the knowledge graph in the source document, and provides an interpretable and readable output for auditing and error analysis.
...
TL;DR: The study analyzes the properties of attention heads in transformer language models, finding that the logit energy field and learned interaction matrix can be highly compressed, and that language interactions are concentrated in a few dimensions.


<details>
  <summary>Details</summary>
Motivation: Understanding the properties of attention mechanisms in transformer language models

Method: Analysis of attention heads in five transformer language models across different architectures and parameter sizes

Result: The logit energy field reaches 90% of its variance in 2-11 singular components, while the learned interaction matrix needs 38-75 components. The spectral gap is 5-25x in effective rank.

Conclusion: The compressibility of softmax-attended language is a property of the data, not the analysis framework. Attention mechanisms allocate capacity uniformly, but language interactions are concentrated in a few dimensions.
...
TL;DR: The study identifies a recurring sparse routing mechanism in alignment-trained language models that triggers refusal responses, and demonstrates its stability and scalability across different models and experiments.


<details>
  <summary>Details</summary>
Motivation: To understand the mechanisms behind language models' refusal responses, particularly in cases of political censorship and safety refusal.

Method: The study uses natural experiments, interchange tests, and bootstrap resampling to identify and validate the routing mechanism across 9 models from 6 labs.

Result: The study finds that the routing mechanism is stable and scalable, and can be controlled by modulating the detection-layer signal, revealing a structural separation between intent recognition and policy routing.

Conclusion: The study provides insights into the mechanisms behind language models' refusal responses, highlighting the importance of understanding the robustness properties of pretraining and post-training.
...
TL;DR: This paper investigates how visual document understanding (VDU) information is represented in large vision language models (LVLMs) and proposes fine-tuning strategies for intermediate layers.


<details>
  <summary>Details</summary>
Motivation: The performance of LVLMs on VDU tasks is typically evaluated based on generated responses, which may not reflect whether the model has actually captured the required information internally.

Method: The study uses linear probing to investigate how information required to solve VDU tasks is represented across different layers of LLMs within LVLMs.

Result: The study finds that there is a gap between internal representations and generated responses, and that information required to solve the task is often encoded more linearly from intermediate layers than from the final layer. Fine-tuning intermediate layers improves both linear probing accuracy and response accuracy.

Conclusion: The paper concludes that targeting intermediate layers for fine-tuning can improve the performance of LVLMs on VDU tasks by narrowing the gap between internal representations and generated responses.
...
TL;DR: The paper proposes a structured approach to video understanding by introducing 'Structured Event Facts' to improve reasoning and causal inference in videos.


<details>
  <summary>Details</summary>
Motivation: Existing Video-LLMs rely on unstructured video reasoning, leading to inefficient processes and fragile causal inference, unlike human understanding which uses structured mental representations.

Method: The paper proposes a four-stage training pipeline, including facts alignment, format warm-start, thinking warm-start, and reinforcement learning-based post-training, and formulates the optimization as a Multi-Objective Reinforcement Learning (MORL) problem.

Result: The proposed model, Factum-4B, yields more reliable reasoning and delivers stronger performance on challenging video understanding tasks requiring fine-grained temporal inference.

Conclusion: The paper concludes that constructing a compact representation of salient events and their causal relationships prior to the reasoning stage can promote concise and causally grounded reasoning in video understanding.
...
TL;DR: The paper introduces DEONTICBENCH, a benchmark for testing large language models' ability to reason with complex, context-specific rules in legal and policy settings, and evaluates their performance in deontic reasoning.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the gap in evaluating large language models' ability to perform long-context, high-stakes deontic reasoning, which is crucial in legal and policy settings.

Method: The paper introduces a benchmark called DEONTICBENCH, which consists of 6,232 tasks across various U.S. laws and policies. It evaluates large language models' performance using both free-form chain-of-thought reasoning and an optional solver-based workflow using Prolog.

Result: The results show that even frontier large language models and coding models struggle with deontic reasoning, achieving performance of only 44.4% on SARA Numeric and 46.6 macro-F1 on Housing. Training with supervised fine-tuning and reinforcement learning improves Prolog generation quality but does not reliably solve the tasks.

Conclusion: The paper concludes that DEONTICBENCH provides a benchmark for studying context-grounded rule reasoning in real-world domains under both symbolic and non-symbolic settings, highlighting the challenges large language models face in deontic reasoning.
...
TL;DR: The paper proposes a method for controllable generation of conversational outputs from Large Language Models (LLMs) using ontological definitions, achieving improved performance and interpretability.


<details>
  <summary>Details</summary>
Motivation: The black-box nature of LLMs implies challenges in predictability and a lack of personalization, which can be addressed by controlled generation.

Method: An end-to-end method using ontological definitions of conversational aspects as constraints, and fine-tuning the LLM to generate content accordingly.

Result: The proposed method consistently outperforms pre-trained baselines on seven state-of-the-art LLMs, and demonstrates effectiveness in ontology-driven control.

Conclusion: The approach enhances alignment with strategy instructions, and enables reusable control strategies that can be extended to new domains and interaction goals.
...
TL;DR: Transformer language models do not exhibit scalar variability, a hallmark of biological magnitude systems, instead showing decreasing representational variability with magnitude.


<details>
  <summary>Details</summary>
Motivation: To test whether transformer language models exhibit scalar variability, a property of biological magnitude systems where representational noise scales proportionally with magnitude.

Method: Analyzing the dispersion of hidden-state representations across carrier sentences for 26 numerical magnitudes in three 7-8B parameter models.

Result: Representational variability decreased with magnitude along the magnitude axis, with a scaling exponent alpha of approximately -0.19, and corpus frequency strongly predicted per-magnitude variability.

Conclusion: Distributional learning alone is insufficient to produce scalar variability, and transformers reproduce log-compressive magnitude geometry but not the constant-CV noise signature observed in biological systems.
...
TL;DR: A new platform, CommonMorph, is introduced to streamline morphological data collection for low-resource languages, making it more efficient and accessible.


<details>
  <summary>Details</summary>
Motivation: The challenges of collecting and annotating morphological data, particularly for low-resource languages, drove the development of CommonMorph.

Method: CommonMorph uses a three-tiered approach: expert linguistic definition, contributor elicitation, and community validation, incorporating active learning and annotation suggestions.

Result: The platform accommodates diverse morphological systems and offers open-source design with UniMorph-compatible outputs, ensuring accessibility and interoperability with NLP tools.

Conclusion: CommonMorph provides a replicable model for preserving linguistic diversity through collaborative technology, making morphological data collection more efficient and accessible.
...
TL;DR: The study evaluates how changing the language of the judge in agentic code benchmarks affects the rankings of backbone models, finding significant interactions between language and backbone performance.


<details>
  <summary>Details</summary>
Motivation: The current default use of English in agentic code benchmarks may not accurately reflect performance in other languages.

Method: The study localizes the Agent-as-a-Judge prompt stack to five languages (English, Arabic, Turkish, Chinese, Hindi) and evaluates 55 DevAI tasks across three frameworks and six judge backbones.

Result: No single backbone model dominates across all languages; instead, performance varies significantly with language, with different backbones leading in different languages.

Conclusion: Language should be treated as an explicit evaluation variable in agentic benchmarks, as changing the judge's language can invert backbone rankings.
...
TL;DR: The study explores constraints on dependency syntax trees to better represent real linguistic phenomena.


<details>
  <summary>Details</summary>
Motivation: The motivation is to find constraints that characterize subsets of trees that better fit real linguistic phenomena, as not all possible trees are plausible or frequent in language.

Method: The paper reviews existing constraints, particularly projectivity, and discusses the need for alternative constraints.

Result: The study identifies limitations of projectivity and the need for more flexible constraints to represent linguistic phenomena, especially in flexible-word-order languages.

Conclusion: The paper concludes that there is a need for constraints that strike a balance between the limitations of projectivity and the leniency of unrestricted dependency structures.
...
TL;DR: The paper proposes a framework called PassiveQA to improve Large Language Models' ability to handle incomplete queries by deciding whether to answer, ask for clarification, or abstain.


<details>
  <summary>Details</summary>
Motivation: Current Large Language Models assume user queries are fully specified, but in real-world settings, queries are often incomplete or ambiguous, leading to overconfident or hallucinated responses.

Method: The authors propose a three-action framework, PassiveQA, which uses supervised finetuning to align model behavior with information sufficiency. It integrates structured information-state representations, knowledge graph-grounded context, and a finetuned planner.

Result: Experiments show that the finetuned planner achieves significant improvements in macro F1 and abstention recall while reducing hallucination rates across multiple QA datasets.

Conclusion: The results provide strong empirical evidence that epistemic decision-making must be learned during training rather than imposed at inference time, and PassiveQA is an effective approach to achieve this.
...
TL;DR: The paper presents the first reproducible evaluation of multilingual automatic speech recognition (ASR) for Pashto, covering zero-shot ASR, script-level failure, and cross-domain evaluation.


<details>
  <summary>Details</summary>
Motivation: Pashto lacks published benchmarks for multilingual ASR, and the authors aim to address this gap by evaluating various models on public Pashto data.

Method: The authors evaluate ten models on Pashto test sets, including zero-shot ASR, script-level failure, and cross-domain evaluation of fine-tuned models.

Result: The results show varying performance across models, with SeamlessM4T achieving 39.7% WER on Common Voice~24 and MMS-1B achieving 43.8% on FLEURS. The authors also identify script-level failures and cross-domain degradation.

Conclusion: The paper highlights the challenges of Pashto ASR, identifies structural impediments to progress, and argues for research priorities to improve Pashto ASR performance.
...
TL;DR: The authors propose a multimodal fact-checking framework called AMuFC, which adaptively uses visual evidence to improve fact-checking accuracy.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this work is to challenge the assumption that incorporating visual evidence universally improves multimodal fact-checking performance.

Method: The authors propose a framework called AMuFC, which employs two collaborative agents: an Analyzer that determines whether visual evidence is necessary for claim verification, and a Verifier that predicts claim veracity conditioned on both the retrieved evidence and the Analyzer's assessment.

Result: Experimental results on three datasets show that incorporating the Analyzer's assessment of visual evidence necessity into the Verifier's prediction yields substantial improvements in verification performance.

Conclusion: The authors conclude that the indiscriminate use of multimodal evidence can reduce accuracy, and that their proposed framework AMuFC can adaptively use visual evidence to improve fact-checking performance.
...
TL;DR: The paper proposes a method for learning sentence representations that capture style and dialect, decoupled from semantic content, and demonstrates its effectiveness in various applications.


<details>
  <summary>Details</summary>
Motivation: Existing sentence representations primarily focus on what a sentence says, rather than how it is expressed, which is important for many applications.

Method: The paper introduces IDIOLEX, a framework that combines supervision from a sentence's provenance with linguistic features of a sentence's content to learn a continuous representation of each sentence's style and dialect.

Result: The learned representations capture meaningful variation and transfer across domains for analysis and classification, and can be used as training objectives for stylistically aligning language models.

Conclusion: The paper suggests that jointly modeling individual and community-level variation provides a useful perspective for studying idiolect and supports downstream applications requiring sensitivity to stylistic differences.
...
TL;DR: Introduced BiST, a Bangla-English corpus for sentence-level grammatical classification, to address the bottleneck of low-resource multilingual NLP.


<details>
  <summary>Details</summary>
Motivation: The lack of high-quality bilingual resources, particularly for Bangla, hinders multilingual NLP in low-resource settings.

Method: Created a corpus of 30,534 sentences (17,465 English, 13,069 Bangla) annotated for syntactic structure and tense, with a multi-stage annotation framework ensuring quality.

Result: BiST corpus shows realistic structural and temporal distributions, and baseline evaluations demonstrate the effectiveness of dual-encoder architectures.

Conclusion: BiST provides a unified resource for bilingual grammatical modeling, facilitating linguistically grounded multilingual research and supporting tasks like controlled text generation and cross-lingual representation learning.
...
TL;DR: The paper challenges the assumption that English-derived reasoning features can close performance gaps in Large Reasoning Models (LRMs) for non-English languages. It defines measurable reasoning features and finds that their association with accuracy varies across languages, challenging English-centric reward designs.


<details>
  <summary>Details</summary>
Motivation: The motivation is to investigate the assumption that English-derived reasoning features can be applied to close performance gaps in LRMs for non-English languages, and to explore what characterizes effective reasoning in multilingual settings.

Method: The method involves defining measurable reasoning features, using logistic regression to quantify their association with final answer accuracy, training sparse autoencoders to discover latent reasoning concepts, and using features as test-time selection policies to steer models toward stronger multilingual reasoning.

Result: The results show that most features are positively associated with accuracy, but the strength of association varies considerably across languages and can even reverse in some. This challenges English-centric reward designs and points toward adaptive objectives that accommodate language-specific reasoning patterns.

Conclusion: The conclusion is that English-centric reward designs may not be effective for non-English languages, and that adaptive objectives that accommodate language-specific reasoning patterns are needed for multilingual benchmark and reward design.
...
TL;DR: The study examines how large language models perform when faced with English inputs that contain errors typical of non-native speakers, finding that combining these errors with typos leads to significant performance drops.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this study is to understand how large language models perform when interacting with non-native English speakers who may make typographical errors and use English as a second language (ESL) variations, which is a common real-world scenario.

Method: The study uses the Trans-EnV framework to create eight ESL variants from standard English inputs and applies MulTypo to introduce typos at three levels of severity: low, moderate, and severe.

Result: The results show that combining ESL variations with typos leads to larger performance drops than either factor alone, with the most significant impact observed on closed-ended tasks.

Conclusion: The study concludes that evaluations based on clean standard English may overestimate the real-world performance of language models and that assessing ESL variations and typographical errors in isolation does not accurately capture model behavior in realistic settings.
...
TL;DR: LLMs may not truly conduct culture-aware reasoning, instead acting as cultural translators with a dominant conceptual framework.


<details>
  <summary>Details</summary>
Motivation: The distinction between speaking a language and reasoning within a culture motivates the question of whether LLMs truly conduct culture-aware reasoning.

Method: A computational audit of cultural inclusivity in a creative writing task, specifically a metaphor generation task spanning five cultural settings.

Result: The model exhibits stereotyped metaphor usage for certain settings and Western defaultism, suggesting that prompting an LLM with a cultural identity does not guarantee culturally grounded reasoning.

Conclusion: Merely prompting an LLM with a cultural identity does not guarantee culturally grounded reasoning; LLMs may act as cultural translators rather than diverse creative partners.
...
TL;DR: The paper explores 'dark patterns' in human-AI co-creativity, where large language models act as writing partners, and analyzes their impact on the creative process.


<details>
  <summary>Details</summary>
Motivation: To investigate the impact of large language models on human agency in collaborative writing and identify subtle model behaviors that can suppress or distort the creative process.

Method: Controlled sessions where LLMs are prompted as writing assistants across diverse literary forms and themes to analyze the prevalence of 'dark patterns' in generated responses.

Result: Preliminary results show that Sycophancy is nearly ubiquitous (91.7% of cases), particularly in sensitive topics, while Anchoring appears to be dependent on literary forms.

Conclusion: The study suggests that 'dark patterns' in LLMs may inadvertently narrow creative exploration and proposes design considerations for AI systems that effectively support creative writing.
...
TL;DR: The paper presents a framework to understand hallucinations in large language models, finding that the separability of hallucinations depends on the task and proposing a method to reduce hallucination probability.


<details>
  <summary>Details</summary>
Motivation: Hallucinations in large language models (LLMs) produce fluent but factually incorrect outputs, and the authors aim to understand and mitigate this issue.

Method: The authors use a geometric dynamical systems framework, analyzing autoregressive hidden-state trajectories across multiple models and benchmarks, and formalizing the behavior with task-complexity and multi-basin theorems.

Result: The study finds that separability of hallucinations is task-dependent, and proposes a geometry-aware steering method to reduce hallucination probability without retraining.

Conclusion: The paper provides a framework to understand hallucinations in LLMs and proposes a method to mitigate them, highlighting the importance of task-dependent basin structure in latent space.
...
TL;DR: Introduced HukukBERT, a comprehensive legal language model for Turkish, achieving state-of-the-art performance in legal text processing tasks.


<details>
  <summary>Details</summary>
Motivation: The scarcity of domain-specific data and models for Turkish law, despite advances in NLP and LegalTech applications.

Method: Trained HukukBERT on an 18 GB cleaned legal corpus using a hybrid Domain-Adaptive Pre-Training (DAPT) methodology.

Result: HukukBERT achieved 84.40% Top-1 accuracy on a Legal Cloze Test benchmark and 92.8% document pass rate in structural segmentation of Turkish court decisions.

Conclusion: HukukBERT is a significant advancement for Turkish legal NLP tasks, outperforming existing models and supporting future research.
...
TL;DR: The study evaluates the end-to-end problem-solving capabilities of Large Language Models (LLMs) in mathematical modeling competitions, revealing a comprehension-execution gap where LLMs struggle with execution-oriented stages.


<details>
  <summary>Details</summary>
Motivation: To assess the ability of LLMs to solve real-world problems requiring end-to-end workflows, specifically in mathematical modeling competitions.

Method: A problem-oriented, stage-wise evaluation framework with expert-verified criteria was proposed and validated against human expert judgments.

Result: State-of-the-art LLMs show deficiencies in execution-oriented stages such as model solving, code implementation, and result analysis, despite performing well in early stages like problem identification and formulation.

Conclusion: The comprehension-execution gap in LLMs suggests that improving their problem-solving capabilities requires approaches beyond scaling up the models.
...
TL;DR: The authors propose SkillX, a framework for building a reusable skill knowledge base for large language model agents, which improves task success and efficiency by leveraging hierarchical experience representations.


<details>
  <summary>Details</summary>
Motivation: The prevailing self-evolving paradigms for large language model agents are inefficient, leading to redundant exploration and poor generalization.

Method: The authors propose a fully automated framework called SkillX, which consists of three synergistic innovations: Multi-Level Skills Design, Iterative Skills Refinement, and Exploratory Skills Expansion.

Result: Experiments show that SkillKB consistently improves task success and execution efficiency when plugged into weaker base agents.

Conclusion: The proposed framework SkillX enables the construction of a reusable skill knowledge base that can improve the performance of large language model agents.
...
TL;DR: The paper introduces LiveFact, a dynamic benchmark for evaluating Large Language Models (LLMs) in fake news detection and fact-checking, addressing limitations of current static benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks for fake news detection and fact-checking are static, making them vulnerable to data contamination and ineffective at assessing reasoning under temporal uncertainty.

Method: The authors propose LiveFact, a continuously updated benchmark that uses dynamic, temporal evidence sets to evaluate models on their ability to reason with evolving, incomplete information. They also introduce a dual-mode evaluation: Classification Mode and Inference Mode.

Result: Tests with 22 LLMs show that open-source models match or outperform proprietary state-of-the-art systems. The analysis reveals a significant 'reasoning gap', where capable models exhibit epistemic humility by recognizing unverifiable claims in early data slices.

Conclusion: LiveFact sets a sustainable standard for evaluating robust, temporally aware AI verification, highlighting the importance of dynamic benchmarks in assessing LLM capabilities.
...
TL;DR: The study examines whether large language models (LLMs) integrate world knowledge with syntactic structure in a human-like way during ambiguity resolution, specifically in Turkish prenominal relative-clause attachment ambiguities.


<details>
  <summary>Details</summary>
Motivation: To investigate whether LLMs integrate world knowledge with syntactic structure in a human-like, structure-sensitive way during ambiguity resolution.

Method: The study uses a speeded forced-choice comprehension experiment with humans and a parallel preference-based setup to evaluate Turkish and multilingual LLMs.

Result: The results show that humans exhibit a large, correctly directed plausibility effect, while LLMs show weak, unstable, or reversed plausibility-driven shifts.

Conclusion: The study suggests that plausibility information does not guide attachment preferences as reliably in LLMs as it does in human judgments, highlighting Turkish RC attachment as a useful cross-linguistic diagnostic.
...
TL;DR: The paper introduces MERIT, a unified translation framework for improving Neural Machine Translation (NMT) from Chinese to low-resource Southeast Asian languages.


<details>
  <summary>Details</summary>
Motivation: The scarcity of clean parallel corpora and pervasive noise in existing data hinder effective NMT model training for low-resource Southeast Asian languages.

Method: The MERIT framework combines language-specific token prefixing (LTP), supervised fine-tuning (SFT), and group relative policy optimization (GRPO) guided by semantic alignment reward (SAR).

Result: Targeted data curation and reward-guided optimization significantly outperform model scaling in LRL→Chinese translation.

Conclusion: The proposed MERIT framework effectively improves NMT from Chinese to low-resource Southeast Asian languages.
...
TL;DR: A new red-teaming framework called PCSA is introduced to test the safety of large language models (LLMs) in mental healthcare by simulating client dialogues and exposing vulnerabilities in psychological safety alignment.


<details>
  <summary>Details</summary>
Motivation: The increasing use of LLMs in mental healthcare raises safety concerns, particularly in distinguishing therapeutic empathy from maladaptive validation, which can reinforce harmful beliefs or behaviors.

Method: The Personality-based Client Simulation Attack (PCSA) framework simulates clients in psychological counseling through coherent, persona-driven client dialogues to test LLMs.

Result: PCSA outperforms competitive baselines in exposing vulnerabilities in psychological safety alignment, revealing that current LLMs are vulnerable to domain-specific adversarial tactics.

Conclusion: The study highlights the need for improved safety measures in LLMs used in mental healthcare, as current models remain vulnerable to providing unauthorized medical advice, reinforcing delusions, and encouraging risky actions.
...
TL;DR: Introduced SandMLE, a framework for efficient on-policy reinforcement learning in machine learning engineering, reducing execution time by 13+ times.


<details>
  <summary>Details</summary>
Motivation: Verifying agent behavior in machine learning engineering is expensive due to the need for running full ML pipelines on large datasets.

Method: SandMLE generates synthetic MLE environments from a small number of seed tasks, constraining datasets to micro-scale.

Result: SandMLE enables large-scale on-policy RL, yielding significant gains over SFT baselines and generalizing across unseen scaffolds.

Conclusion: SandMLE facilitates efficient on-policy RL in machine learning engineering, preserving complexity while reducing dataset size.
...
TL;DR: The paper proposes AsymGRPO, a new framework for reinforcement learning with verifiable rewards that improves exploration in large language models by decoupling the modulation of positive and negative rollouts.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitation of restricted exploration in reinforcement learning with verifiable rewards, where the policy converges to a narrow set of solutions, and to improve the efficiency of entropy regularization methods.

Method: The method proposed is AsymGRPO, which deploys a parametric formulation of group-relative advantage estimation and analyzes entropy dynamics to decouple the modulation of positive and negative rollouts.

Result: The results show that AsymGRPO achieves superior performance compared to strong baselines and has the potential to synergize with existing entropy regularization methods.

Conclusion: The conclusion is that AsymGRPO offers a more effective approach to sustaining exploration in large language models by refining entropy and decoupling the modulation of positive and negative rollouts.
...
TL;DR: The paper proposes TriAttention, a method to estimate key importance in large language models, improving efficiency and accuracy.


<details>
  <summary>Details</summary>
Motivation: The KV cache memory bottleneck in large language models due to extended reasoning, and the limitations of current KV cache compression methods.

Method: TriAttention, which leverages the concentration of Q and K vectors in pre-RoPE space to estimate key importance.

Result: TriAttention achieves 2.5x higher throughput or 10.7x KV memory reduction on AIME25 with 32K-token generation, matching Full Attention reasoning accuracy.

Conclusion: The proposed TriAttention method effectively addresses the KV cache memory bottleneck, enabling efficient and accurate large language model deployment.
...
TL;DR: A new early stopping method, CoDE-Stop, is proposed to determine when to terminate reasoning in large language models, leveraging the confidence dynamics of intermediate answers to achieve a better accuracy-compute tradeoff.


<details>
  <summary>Details</summary>
Motivation: Large reasoning models often suffer from overthinking, leading to substantial computational cost and degraded performance. The key challenge is determining when to stop reasoning and produce the final answer.

Method: CoDE-Stop, an early stopping method that uses the dynamics of intermediate answer confidence to decide when to terminate reasoning, without requiring additional training and easily integrating into existing models.

Result: CoDE-Stop achieves a more favorable accuracy-compute tradeoff and reduces total token usage by 25-50% compared to standard full-length reasoning, across diverse reasoning and science benchmarks and multiple models.

Conclusion: The proposed CoDE-Stop method effectively leverages the confidence dynamics of intermediate answers to improve the efficiency and performance of large reasoning models, offering insights into the dynamics of confidence during reasoning.
...
TL;DR: A new method called RACE is proposed for detecting fine-grained types of LLM-generated text, outperforming existing methods in a four-class setting.


<details>
  <summary>Details</summary>
Motivation: The misuse of large language models requires precise detection of synthetic text, and existing works are insufficient for nuanced regulation.

Method: RACE (Rhetorical Analysis for Creator-Editor Modeling) utilizes Rhetorical Structure Theory to construct a logic graph and extract Elementary Discourse Unit-level features.

Result: RACE outperforms 12 baselines in identifying fine-grained types with low false alarms.

Conclusion: RACE offers a policy-aligned solution for LLM regulation.
...
TL;DR: This paper reviews the applications and impact of Large Language Models (LLMs) in healthcare, focusing on diagnostic and treatment functionalities across various medical domains.


<details>
  <summary>Details</summary>
Motivation: To provide a comprehensive overview of LLMs' advancements in healthcare and their transformative impact on medical domains.

Method: A review of LLMs' applications in healthcare, focusing on cancer care, dermatology, dental care, neurodegenerative disorders, and mental health.

Result: The paper highlights LLMs' innovative contributions to medical diagnostics and patient care, as well as the challenges and opportunities associated with their integration in healthcare.

Conclusion: LLMs have the potential to transform healthcare across various medical specialties, but existing limitations need to be addressed.
...
### [277] [IC3-Evolve: Proof-/Witness-Gated Offline LLM-Driven Heuristic Evolution for IC3 Hardware Model Checking](https://arxiv.org/abs/2604.03232)
*Mingkai Miao,Guangyu Hu,Ziyi Yang,Hongce Zhang*

Main category: cs.AI

TL;DR: This paper presents IC3-Evolve, a framework that uses a large language model to automatically improve the IC3 algorithm for hardware safety model checking, ensuring correctness and reliability.


<details>
  <summary>Details</summary>
Motivation: The performance of IC3 is dominated by heuristics and implementation choices that are costly to tune manually, and the authors aim to automate this process while ensuring correctness.

Method: The authors propose an offline code-evolution framework, IC3-Evolve, which uses a large language model to propose patches to an IC3 implementation, validated through proof- and witness-gated validation.

Result: The framework is evaluated on public and industrial benchmarks, showing that it can reliably discover practical heuristic improvements under strict correctness gates.

Conclusion: IC3-Evolve enables the automated improvement of IC3, ensuring correctness and reliability, and can be used to evolve a standalone checker with zero ML/LLM inference overhead.
...
### [278] [Structural Segmentation of the Minimum Set Cover Problem: Exploiting Universe Decomposability for Metaheuristic Optimization](https://arxiv.org/abs/2604.03234)
*Isidora Hernández,Héctor Ferrada,Cristóbal A. Navarro*

Main category: cs.AI

TL;DR: The paper proposes a method to improve solving the Minimum Set Cover Problem (MSCP) by breaking it down into smaller subproblems based on the structural properties of the problem, specifically 'universe segmentability', and then solving each subproblem using the GRASP metaheuristic.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this work is to exploit intrinsic structural properties of the MSCP, specifically 'universe segmentability', to enhance heuristic optimization, as existing methods often treat MSCP instances monolithically.

Method: The method involves a preprocessing strategy based on disjoint-set union (union-find) to detect connected components in the MSCP instance, decomposing it into independent subproblems, solving each using GRASP metaheuristic, and combining partial solutions.

Result: The results show that exploiting universe segmentation consistently improves solution quality and scalability, particularly for large and structurally decomposable instances.

Conclusion: The conclusion is that exploiting natural universe segmentation in MSCP can lead to better solution quality and scalability, especially for large instances, making the approach computationally practical at scale.
...
### [279] [To Throw a Stone with Six Birds: On Agents and Agenthood](https://arxiv.org/abs/2604.03239)
*Ioannis Tsiokos*

Main category: cs.AI

TL;DR: The Six Birds Theory (SBT) provides a framework for understanding agency in macroscopic objects, distinguishing between persistence and control, and operationalizing agency through four checkable components.


<details>
  <summary>Details</summary>
Motivation: The authors aim to clarify the concept of agency and provide a testable framework for understanding agency claims, addressing the conflation of persistence and control in empirical discussions.

Method: The authors use a type-correct account of agency within SBT, operationalizing it through four checkable components: ledger-gated feasibility, robust viability kernel, feasible empowerment, and empirical packaging map.

Result: The results provide hash-traceable tests that separate agenthood from agency, showing that calibrated null regimes have zero empowerment, and that protocols, repair, and operator rewriting can increase empowerment.

Conclusion: The study provides a framework for understanding agency in macroscopic objects, allowing for testable claims about agency without relying on assumptions about goals, consciousness, or biological organisms.
...
TL;DR: The paper argues for the importance of item-level AI benchmark data for rigorous AI evaluation, enabling fine-grained diagnostics and principled validation.


<details>
  <summary>Details</summary>
Motivation: Current AI evaluation paradigms exhibit systemic validity failures, which can be addressed with a principled framework for gathering validity evidence and conducting granular diagnostic analysis.

Method: The paper dissects current validity failures, revisits evaluation paradigms, and demonstrates the insights afforded by item-level data through illustrative analyses.

Result: The authors introduce OpenEval, a repository of item-level benchmark data to support evidence-centered AI evaluation and promote community-wide adoption.

Conclusion: Item-level AI benchmark data is essential for establishing a rigorous science of AI evaluation, enabling fine-grained diagnostics and principled validation of benchmarks.
...
### [281] [Toward Full Autonomous Laboratory Instrumentation Control with Large Language Models](https://arxiv.org/abs/2604.03286)
*Yong Xie,Kexin He,Andres Castellanos-Gomez*

Main category: cs.AI

TL;DR: This paper explores the use of large language models (LLMs) like ChatGPT to automate laboratory instrumentation control, reducing the need for programming expertise and enabling researchers to customize experiments more easily.


<details>
  <summary>Details</summary>
Motivation: The control of complex laboratory instrumentation requires significant programming expertise, creating a barrier for researchers lacking computational skills.

Method: The authors used ChatGPT to facilitate the creation of custom scripts for instrumentation control through a case study involving a setup that can be used as a single-pixel camera or a scanning photocurrent microscope.

Result: The approach demonstrated that ChatGPT can significantly reduce the technical barrier for experimental customization and that LLM-assisted tools can be extended into autonomous AI agents capable of independently operating laboratory instruments.

Conclusion: LLM-based tools and AI agents have the potential to democratize laboratory automation and accelerate scientific progress by enabling researchers to efficiently program and automate scientific equipment.
...
### [282] [Evaluating Artificial Intelligence Through a Christian Understanding of Human Flourishing](https://arxiv.org/abs/2604.03356)
*Nicholas Skytland,Lauren Parsons,Alicia Llewellyn,Steele Billings,Peter Larson,John Anderson,Sean Boisen,Steve Runge*

Main category: cs.AI

TL;DR: The paper introduces a benchmark to evaluate AI models' alignment with Christian values, finding that current models default to a secular worldview and perform poorly on theological coherence.


<details>
  <summary>Details</summary>
Motivation: The authors aim to make the formative influence of Large Language Models on human understanding and moral reflection visible and measurable, particularly in the context of Christian values.

Method: The authors introduce the Flourishing AI Benchmark: Christian Single-Turn (FAI-C-ST) framework, which evaluates AI models against a Christian understanding of human flourishing across seven dimensions.

Result: The study finds that current AI systems are not worldview-neutral, defaulting to Procedural Secularism, and perform poorly on theological coherence, with a 17-point decline across all dimensions of flourishing and a 31-point decline in Faith and Spirituality.

Conclusion: The performance gap in values alignment is not a technical limitation, but arises from training objectives that prioritize broad acceptability and safety over deep, internally coherent moral or theological reasoning.
...
### [283] [VERT: Reliable LLM Judges for Radiology Report Evaluation](https://arxiv.org/abs/2604.03376)
*Federica Bologna,Jean-Philippe Corbeil,Matthew Wilkens,Asma Ben Abacha*

Main category: cs.AI

TL;DR: This paper evaluates LLM-based metrics for radiology report evaluation across multiple modalities and anatomies, comparing existing metrics and proposing a new one, VERT, which shows improved correlation with radiologist judgments.


<details>
  <summary>Details</summary>
Motivation: The current literature on radiology report evaluation lacks robust approaches that can be applied across different modalities and anatomies, and there is a need to identify the best LLM-based judges for radiology evaluation.

Method: The paper conducts a thorough correlation analysis between expert and LLM-based ratings, comparing existing metrics (RadFact, GREEN, and FineRadScore) with a proposed metric, VERT, using open- and closed-source models across two expert-annotated datasets.

Result: VERT improves correlation with radiologist judgments by up to 11.7% relative to GREEN, and fine-tuning a model yields gains of up to 25% using only 1,300 training samples, with reduced inference time.

Conclusion: The findings highlight the effectiveness of LLM-based judges for radiology report evaluation and demonstrate that reliable evaluation can be achieved with lightweight adaptation, improving correlation with radiologist judgments and reducing inference time.
...
### [284] [Hume's Representational Conditions for Causal Judgment: What Bayesian Formalization Abstracted Away](https://arxiv.org/abs/2604.03387)
*Yiling Wu*

Main category: cs.AI

TL;DR: The paper analyzes Hume's account of causal judgment, extracting three representational conditions and tracing their influence on later epistemological frameworks, including Bayesian epistemology and predictive processing.


<details>
  <summary>Details</summary>
Motivation: To understand the development of causal judgment and the representational conditions that underlie it, from Hume's philosophical account to modern computational frameworks.

Method: The paper involves a close reading of Hume's texts to extract the three representational conditions, followed by an analysis of how these conditions are preserved or abstracted away in later formal frameworks.

Result: The paper shows that while later frameworks like Bayesian epistemology and predictive processing preserve the updating structure of Hume's insight, they abstract away the representational conditions of experiential grounding, structured retrieval, and vivacity transfer.

Conclusion: The paper highlights the importance of Hume's representational conditions for understanding causal judgment and demonstrates how their absence in modern large language models reveals the requirements that were previously implicit in Hume's framework.
...
### [285] [TABQAWORLD: Optimizing Multimodal Reasoning for Multi-Turn Table Question Answering](https://arxiv.org/abs/2604.03393)
*Tung Sum Thomas Kwok,Xinyu Wang,Xiaofeng Lin,Peng Lu,Chunhe Wang,Changlun Li,Hanwei Wu,Nan Tang,Elisa Kreiss,Guang Cheng*

Main category: cs.AI

TL;DR: TABQAWORLD, a table reasoning framework, is introduced to enhance multimodal reasoning by dynamically switching between visual and textual representations and optimizing stepwise reasoning trajectory, achieving state-of-the-art performance with improved accuracy and reduced latency.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the limitations of existing multi-turn table reasoning methods, which rely on fixed text serialization and introduce representation errors that accumulate over multiple turns, making real-world deployment impractical.

Method: The method proposed in this paper is TABQAWORLD, a table reasoning framework that jointly optimizes tabular action through representation and estimation, employing an action-conditioned multimodal selection policy and optimizing stepwise reasoning trajectory through table metadata.

Result: The results show that TABQAWORLD achieves state-of-the-art performance with 4.87% accuracy improvements over baselines, with 5.42% accuracy gain and 33.35% inference latency reduction over static settings.

Conclusion: The conclusion of this paper is that TABQAWORLD establishes a new standard for reliable and efficient table reasoning, providing a training-free framework that achieves improved accuracy and reduced latency.
...
### [286] [Contextual Control without Memory Growth in a Context-Switching Task](https://arxiv.org/abs/2604.03479)
*Song-Ju Kim*

Main category: cs.AI

TL;DR: The paper proposes a new method for context-dependent sequential decision making by intervening on a shared recurrent latent state, rather than increasing recurrent memory or providing context as input.


<details>
  <summary>Details</summary>
Motivation: The authors aim to find an alternative to traditional methods of context-dependent sequential decision making, which either use explicit context input or increase recurrent memory.

Method: The authors introduce an intervention-based recurrent architecture, where a recurrent core constructs a shared pre-intervention latent state, and context acts through an additive, context-indexed operator.

Result: The proposed intervention model performs strongly on a context-switching sequential decision task under partial observability, without additional recurrent dimensions, and exhibits positive conditional contextual information.

Conclusion: The results suggest that intervention on a shared recurrent state provides a viable alternative to recurrent memory growth for contextual control in this setting.
...
### [287] [Beyond Predefined Schemas: TRACE-KG for Context-Enriched Knowledge Graphs from Complex Documents](https://arxiv.org/abs/2604.03496)
*Mohammad Sadeq Abolhasani,Yang Ba,Yixuan He,Rong Pan*

Main category: cs.AI

TL;DR: Proposed TRACE-KG, a multimodal framework for constructing context-enriched knowledge graphs and induced schemas without predefined ontologies.


<details>
  <summary>Details</summary>
Motivation: Knowledge graph construction faces challenges with ontology-driven pipelines requiring costly schema design and maintenance, and schema-free methods producing fragmented graphs.

Method: TRACE-KG uses a data-driven schema and structured qualifiers to capture conditional relations and organize entities and relations.

Result: TRACE-KG produces structurally coherent, traceable knowledge graphs and offers a practical alternative to ontology-driven and schema-free construction pipelines.

Conclusion: TRACE-KG provides a viable solution for knowledge graph construction, balancing consistency and organization with traceability to source evidence.
...
### [288] [Resource-Conscious Modeling for Next- Day Discharge Prediction Using Clinical Notes](https://arxiv.org/abs/2604.03498)
*Ha Na Cho,Sairam Sutari,Alexander Lopez,Hansen Bow,Kai Zheng*

Main category: cs.AI

TL;DR: This study compares the performance of lightweight large language models and traditional text-based models for predicting next-day discharge in elective spine surgery units using postoperative clinical notes.


<details>
  <summary>Details</summary>
Motivation: To optimize bed turnover and resource allocation in elective spine surgery units by predicting timely discharge.

Method: The study evaluated 13 models, including TF-IDF with XGBoost and LGBM, and compact LLMs (DistilGPT-2, Bio_ClinicalBERT) fine-tuned via LoRA.

Result: TF-IDF with LGBM achieved the best balance with an F1-score of 0.47, recall of 0.51, and the highest AUC-ROC (0.80) for predicting next-day discharge.

Conclusion: Interpretable, resource-efficient models may outperform compact LLMs in real-world, imbalanced clinical prediction tasks.
...
### [289] [BioAlchemy: Distilling Biological Literature into Reasoning-Ready Reinforcement Learning Training Data](https://arxiv.org/abs/2604.03506)
...
TL;DR: The authors introduce BioAlchemy, a pipeline for creating a diverse dataset of verifiable biology research questions, and demonstrate its effectiveness in improving scientific reasoning capabilities in biology.


<details>
  <summary>Details</summary>
Motivation: The impact of reasoning models on biological research lags behind math and coding, and current large-scale reasoning datasets do not align well with modern research topic distributions in biology.

Method: The authors propose a pipeline called BioAlchemy to extract challenging and verifiable research problems from biology research text, and use reinforcement learning to improve reasoning performance.

Result: The authors present BioAlchemist-8B, a model that improves over its base reasoning model by 9.12% on biology benchmarks.

Conclusion: The authors' approach demonstrates efficacy in developing stronger scientific reasoning capabilities in biology, and the BioAlchemist-8B model is made available for further research.
...
### [290] [ActionNex: A Virtual Outage Manager for Cloud](https://arxiv.org/abs/2604.03512)
...
TL;DR: ActionNex, an agentic system for outage management in large-scale cloud operations, provides end-to-end assistance with real-time updates and action recommendations.


<details>
  <summary>Details</summary>
Motivation: Outage management in large-scale cloud operations is heavily manual, requiring rapid triage, cross-team coordination, and experience-driven decisions under partial observability.

Method: ActionNex ingests multimodal operational signals, compresses them into critical events, and uses a hierarchical memory subsystem to generate actionable recommendations.

Result: ActionNex achieved 71.4% precision and 52.8-54.8% recall in evaluating eight real Azure outages, and has received positive early feedback in production.

Conclusion: ActionNex is a production-grade system that supports end-to-end outage assistance, enabling more efficient and effective outage management in large-scale cloud operations.
...
### [291] [Structural Rigidity and the 57-Token Predictive Window: A Physical Framework for Inference-Layer Governability in Large Language Models](https://arxiv.org/abs/2604.03524)
*Gregory M. Ruddell*

Main category: cs.AI

TL;DR: The paper presents a framework for evaluating the safety of AI models by analyzing their internal geometry and dynamics, finding that pre-commitment signals are not universal and that different models exhibit different types of failure modes.


<details>
  <summary>Details</summary>
Motivation: The current reliance on behavioral monitoring and post-training alignment in AI safety, and the need for a more proactive approach to detecting potential failures in AI models.

Method: The authors propose an energy-based governance framework that connects transformer inference dynamics to constraint-satisfaction models of neural computation, and apply it to a cohort of seven models across five geometric regimes.

Result: The authors identify a pre-commitment window in one model, and introduce a taxonomy of inference behavior that categorizes models into five regimes, finding that only one configuration exhibits a predictive signal prior to commitment.

Conclusion: The paper concludes that internal geometry monitoring is effective only where resistance exists, and that detection of factual confabulation requires external verification mechanisms, providing a measurable framework for inference-layer governability and a taxonomy for evaluating deployment risk in autonomous AI systems.
...
### [292] [Explainable Model Routing for Agentic Workflows](https://arxiv.org/abs/2604.03527)
*Mika Okamoto,Ansel Kaplan Erol,Mark Riedl*

Main category: cs.AI

TL;DR: The paper introduces Topaz, a framework that makes agentic routing decisions interpretable by providing formal auditability, allowing developers to understand and trust the system.


<details>
  <summary>Details</summary>
Motivation: Current routing architectures in agentic workflows focus on performance optimization but lack transparency in model capability and cost trade-offs, making it difficult for developers to distinguish between intelligent efficiency and latent failures.

Method: Topaz introduces three components: skill-based profiling, fully traceable routing algorithms, and developer-facing explanations to make routing decisions interpretable.

Result: Topaz enables users to understand, trust, and steer routed agentic systems by providing clear rationale for model assignments and allowing iterative tuning of the cost-quality tradeoff.

Conclusion: The paper presents Topaz as a solution to introduce formal auditability to agentic routing, making it possible for developers to audit system logic and make informed decisions.
...
TL;DR: The paper presents an automated framework for comparing AI safety policy documents using a shared taxonomy, and assesses its stability and validity across different large language models.


<details>
  <summary>Details</summary>
Motivation: The motivation is to facilitate comparative inspection of policy documents by developing an automated crosswalk framework that can compare AI safety policy documents under a shared taxonomy of activities.

Method: The method involves using five large language models to perform crosswalks on ten publicly available documents, and then visualizing the mean similarity scores with a heatmap.

Result: The results show that model choice substantially affects the crosswalk outcomes, and that some document pairs yield high disagreements across models. However, human evaluation by three experts on two document pairs shows high inter-annotator agreement.

Conclusion: The findings support comparative inspection of policy documents, but highlight the importance of model choice and the need for human judgment in evaluating policy documents.
...
### [294] [Towards the AI Historian: Agentic Information Extraction from Primary Sources](https://arxiv.org/abs/2604.03553)
*Lorenz Hufe,Niclas Griesshaber,Gavin Greif,Sebastian Oliver Eck,Philip Torr*

Main category: cs.AI

TL;DR: The paper introduces a module of Chronos, an AI Historian, that helps historians convert image scans of primary sources into data through natural-language interactions.


<details>
  <summary>Details</summary>
Motivation: The adoption of AI in historical research is limited due to the lack of solutions designed for historians.

Method: The module uses natural-language interactions to allow historians to adapt workflows for heterogeneous source corpora, evaluate AI model performance, and iteratively refine workflows.

Result: The module is open-source and ready for use by historical researchers on their own sources.

Conclusion: The paper presents a technical progress report on the development of Chronos, an AI Historian, specifically designed to support historians in their research.
...
### [295] [When Do Hallucinations Arise? A Graph Perspective on the Evolution of Path Reuse and Path Compression](https://arxiv.org/abs/2604.03557)
*Xinnan Dai,Kai Yang,Cheng Luo,Shenglai Zeng,Kai Guo,Jiliang Tang*

Main category: cs.AI

TL;DR: The paper explains the mechanisms behind 'reasoning hallucinations' in large language models, where the models produce fluent but unsupported conclusions, and identifies two fundamental mechanisms: Path Reuse and Path Compression.


<details>
  <summary>Details</summary>
Motivation: To understand the mechanisms by which decoder-only Transformers produce reasoning hallucinations, which are fluent yet unsupported conclusions that violate either the given context or underlying factual knowledge.

Method: The authors model next-token prediction as a graph search process over an underlying graph, where entities correspond to nodes and learned transitions form edges, and analyze contextual and context-free queries.

Result: The authors identify two fundamental mechanisms behind reasoning hallucinations: Path Reuse, where memorized knowledge overrides contextual constraints during early training, and Path Compression, where frequently traversed multi-step paths collapse into shortcut edges in later training.

Conclusion: The paper provides a unified explanation for reasoning hallucinations in LLMs, connecting them to well-known behaviors observed in downstream applications.
...
### [296] [When Adaptive Rewards Hurt: Causal Probing and the Switching-Stability Dilemma in LLM-Guided LEO Satellite Scheduling](https://arxiv.org/abs/2604.03562)
*Yuanhang Li*

Main category: cs.AI

TL;DR: The paper explores adaptive reward design for deep reinforcement learning in multi-beam LEO satellite scheduling, finding that static reward weights often outperform dynamic ones due to the need for a quasi-stationary reward signal.


<details>
  <summary>Details</summary>
Motivation: The authors are motivated by the intuition that regime-aware reward weights should outperform static ones in deep reinforcement learning for multi-beam LEO satellite scheduling.

Method: The authors use a systematic testing approach, introducing a single-variable causal probing method to independently perturb each reward term and measure the response of Proximal Policy Optimization (PPO).

Result: The results show that near-constant reward weights outperform carefully-tuned dynamic weights, and that specific weights matter, with counterintuitive leverage found in certain reward terms. The authors also evaluate four MDP architect variants, finding that a multi-layer perceptron (MLP) achieves high performance on known and novel traffic regimes, while a fine-tuned large language model (LLM) collapses due to weight oscillation.

Conclusion: The authors conclude that their findings provide an empirically-grounded roadmap for LLM-DRL integration in communication systems, identifying where LLMs add irreplaceable value and where simpler methods suffice.
...
### [297] [Personality Requires Struggle: Three Regimes of the Baldwin Effect in Neuroevolved Chess Agents](https://arxiv.org/abs/2604.03565)
*Diego Armando Resendez Prado*

Main category: cs.AI

TL;DR: The study investigates whether lifetime learning can expand behavioral diversity over evolutionary time in chess agents with neural modules and Hebbian plasticity. The results show that plasticity initially reduces variance but eventually increases it, leading to structured behavioral divergence.


<details>
  <summary>Details</summary>
Motivation: The motivation is to test whether lifetime learning can expand behavioral diversity over evolutionary time, contrary to prior theory that predicts plasticity reduces variance.

Method: The study uses NEAT-evolved neural modules, Hebbian within-game plasticity, and a desirability-domain signal chain with imagination to test the effect of plasticity on behavioral variance in chess agents.

Result: The results show a variance crossover, where Hebbian ON starts with lower cross-seed variance than OFF but surpasses it at generation 34. The evolved agents develop distinct behavioral signatures, including different move selections, opening repertoires, piece preferences, and game lengths.

Conclusion: The study concludes that lifetime learning can expand behavioral diversity over evolutionary time, leading to structured behavioral divergence, and that plasticity's effect on behavioral variance reverses over evolutionary time.
...
### [298] [Selective Forgetting for Large Reasoning Models](https://arxiv.org/abs/2604.03571)
*Tuan Le,Wei Qian,Mengdi Huai*

Main category: cs.AI

TL;DR: A novel framework is proposed for selective forgetting in Large Reasoning Models (LRMs) to remove sensitive information from intermediate reasoning steps while preserving general reasoning capabilities.


<details>
  <summary>Details</summary>
Motivation: The memorization of sensitive information in LRMs can lead to ethical and legal concerns, and existing unlearning methods may degrade the overall reasoning ability of LRMs.

Method: The proposed framework leverages multiple LLMs with retrieval-augmented generation (RAG) to analyze chains of thought, identify forget-relevant segments, and replace them with benign placeholders. A new feature replacement unlearning loss is also introduced.

Result: Extensive experiments on synthetic and medical datasets verify the effectiveness of the proposed method in selectively removing sensitive information while preserving general reasoning capabilities.

Conclusion: The proposed framework offers a solution for selective forgetting in LRMs, addressing the challenge of precise unlearning of targeted knowledge while preserving general reasoning capabilities.
...
### [299] [Rashomon Memory: Towards Argumentation-Driven Retrieval for Multi-Perspective Agent Memory](https://arxiv.org/abs/2604.03588)
*Albert Sadowski,Jarosław A. Chudziak*

Main category: cs.AI

TL;DR: The paper proposes Rashomon Memory, a new architecture for AI agents to store and retrieve experiences with multiple, conflicting interpretations for different goals.


<details>
  <summary>Details</summary>
Motivation: Current memory architectures for AI agents assume a single correct encoding or support multiple views over unified storage, which is insufficient for agents operating over extended time horizons with multiple concurrent goals.

Method: The proposed Rashomon Memory architecture uses parallel goal-conditioned agents to encode experiences according to their priorities and negotiate at query time through argumentation, maintaining their own ontology and knowledge graph.

Result: The architecture allows for retrieval modes such as selection, composition, and conflict surfacing, with the conflict surfacing mode enabling decision-makers to see underlying interpretive conflicts directly.

Conclusion: The Rashomon Memory architecture provides a novel approach to handling multiple, conflicting interpretations of experiences in AI agents, enabling more effective decision-making and conflict resolution.
...
### [300] [Entropy and Attention Dynamics in Small Language Models: A Trace-Level Structural Analysis on the TruthfulQA Benchmark](https://arxiv.org/abs/2604.03589)
*Adeyemi Adeseye,Aisvarya Adeseye,Hannu Tenhunen,Jouni Isoaho*

Main category: cs.AI

TL;DR: This study analyzes the internal behavior of small language models (SLMs) to understand how they produce outputs, focusing on entropy and attention dynamics.


<details>
  <summary>Details</summary>
Motivation: Current evaluation methods for SLMs do not adequately explain how internal model behavior affects outputs, particularly in terms of entropy, attention, and hidden representations.

Method: The study uses a trace-level analysis of entropy and attention dynamics in SLMs evaluated with the TruthfulQA dataset, examining token-level output entropy, attention entropy, head dispersion, and hidden-state representation in four models with 1B-1.7B parameters.

Result: The results classify SLMs into three types based on entropy patterns: deterministic models with decreasing entropy, exploratory models with increasing entropy, and balanced models with stable entropy, each with distinct hidden-state movement and attention dispersion patterns.

Conclusion: The analysis shows that truthfulness in SLMs emerges from structured entropy and attention dynamics, and monitoring and optimizing these internal uncertainty patterns can guide the design of more reliable SLMs.
...
### [301] [A Multimodal Foundation Model of Spatial Transcriptomics and Histology for Biological Discovery and Clinical Prediction](https://arxiv.org/abs/2604.03630)
...
TL;DR: STORM, a foundation model, integrates spatial transcriptomics and histology to improve spatial domain discovery, gene expression prediction, and clinical applications.


<details>
  <summary>Details</summary>
Motivation: The high cost and low throughput of spatial transcriptomics (ST) and the lack of molecular resolution in hematoxylin and eosin (H&E) staining motivate the development of a model that can bridge imaging and omics.

Method: STORM uses a hierarchical architecture integrating morphological features, gene expression, and spatial context, trained on 1.2 million spatially resolved transcriptomic profiles with matched histology across 18 organs.

Result: STORM enhances spatial domain discovery, outperforms existing methods in predicting spatial gene expression from H&E images, and improves immunotherapy response prediction and prognostication in 23 independent cohorts comprising 7,245 patients.

Conclusion: STORM provides a scalable framework for spatially informed discovery and clinical precision medicine, bridging imaging and omics through robust molecular-morphological representations.
...
### [302] [Single-agent vs. Multi-agents for Automated Video Analysis of On-Screen Collaborative Learning Behaviors](https://arxiv.org/abs/2604.03631)
*Likai Peng,Shihui Feng*

Main category: cs.AI

TL;DR: This study compares the performance of Vision Language Models (VLMs) in automated coding of screen recordings for collaborative learning contexts.


<details>
  <summary>Details</summary>
Motivation: Analyzing on-screen behavioral engagement is essential for capturing students' cognitive and collaborative processes, but manual coding of multimodal video data is labor-intensive.

Method: The study proposes and compares two multi-agent frameworks: a three-agent workflow multi-agent system (MAS) and an autonomous-decision MAS, using VLMs for scene segmentation, action detection, and behavior labeling.

Result: The proposed MAS frameworks outperform single VLMs in scene and action detection tasks, with the workflow-based agent achieving best results in scene detection and the autonomous-decision MAS achieving best results in action detection.

Conclusion: The study demonstrates the effectiveness of VLM-based Multi-agent System for video analysis and contributes a scalable framework for multimodal data analytics.
...
### [303] [Beyond Retrieval: Modeling Confidence Decay and Deterministic Agentic Platforms in Generative Engine Optimization](https://arxiv.org/abs/2604.03656)
*XinYu Zhao,ChengYou Li,XiangBao Meng,Kai Zhang,XiaoDong Liu*

Main category: cs.AI

TL;DR: The paper proposes a paradigm shift in Generative Engine Optimization (GEO) from Retrieval-Augmented Generation (RAG) to deterministic multi-agent intent routing, addressing probabilistic hallucinations and the 'zero-click' paradox.


<details>
  <summary>Details</summary>
Motivation: Current GEO strategies relying on RAG suffer from probabilistic hallucinations and the 'zero-click' paradox, failing to establish sustainable commercial trust.

Method: The paper proposes a deterministic multi-agent intent routing approach, including the Semantic Entropy Drift (SED) model, Isomorphic Attribution Regression (IAR) model, and Deterministic Agent Handoff (DAH) protocol.

Result: The proposed architecture reduces vertical task hallucination rates to near zero, demonstrated through an industrial AI meeting minutes product.

Conclusion: The paper establishes a foundational theoretical framework for next-generation GEO and paves the way for a well-ordered, deterministic human-AI collaboration ecosystem.
...
### [304] [TableVision: A Large-Scale Benchmark for Spatially Grounded Reasoning over Complex Hierarchical Tables](https://arxiv.org/abs/2604.03660)
*Xiaoyu Chen,Lu Dai,Hanqing Wang,Zhuoyu Li,Wenbin Dai,Yanzong Zheng,Zhenggang Xia,Junyong Lin,Hui Xiong*

Main category: cs.AI

TL;DR: The paper identifies a Perception Bottleneck in Multimodal Large Language Models (MLLMs) when dealing with complex tables, and proposes TableVision, a benchmark and framework that uses spatial constraints to improve reasoning performance.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the limited reasoning performance of MLLMs on complex tables with hierarchical layouts, which is a significant challenge in professional domains such as finance, healthcare, and scientific research.

Method: The authors introduce TableVision, a large-scale benchmark that stratifies tabular tasks into three cognitive levels, and a rendering-based deterministic grounding pipeline that couples multi-step logical deductions with pixel-perfect spatial ground truths.

Result: The empirical results show that explicit spatial constraints significantly improve the reasoning potential of MLLMs, with a 12.3% overall accuracy improvement on the test set achieved by a two-stage decoupled framework.

Conclusion: The paper concludes that TableVision provides a rigorous testbed and a fresh perspective on the synergy between perception and logic in document understanding, and that explicit spatial constraints can significantly recover the reasoning potential of MLLMs.
...
### [305] [PRAISE: Prefix-Based Rollout Reuse in Agentic Search Training](https://arxiv.org/abs/2604.03675)
*Erhan Zhang,Yiqun Chen,Zechun Niu,Wei Yang,Xiaochi Wei,Yan Gao,Yi Wu,Yao Hu,Jiaxin Mao*

Main category: cs.AI

TL;DR: PRAISE is a framework that improves data efficiency and credit assignment in agentic search training by reusing prefix states and deriving intermediate rewards.


<details>
  <summary>Details</summary>
Motivation: Current search-based RL methods suffer from expensive long-horizon rollouts and severe reward sparsity, which hinders effective training.

Method: PRAISE extracts prefix states, elicits intermediate answers, and uses them to construct additional training trajectories and derive step-level rewards.

Result: PRAISE consistently improves performance over strong baselines on multi-hop QA benchmarks.

Conclusion: PRAISE offers a more efficient and effective approach to agentic search training by addressing the limitations of current search-based RL methods.
...
### [306] [Structured Multi-Criteria Evaluation of Large Language Models with Fuzzy Analytic Hierarchy Process and DualJudge](https://arxiv.org/abs/2604.03742)
*Yulong He,Ivan Smirnov,Dmitry Fedrushkov,Sergey Kovalchuk,Ilya Revin*

Main category: cs.AI

TL;DR: The paper proposes a structured approach to evaluating large language models (LLMs) using Analytic Hierarchy Process (AHP) and Fuzzy AHP (FAHP), which incorporates uncertainty-aware aggregation and outperforms direct scoring methods.


<details>
  <summary>Details</summary>
Motivation: Conventional direct scoring of LLMs yields inconsistent and opaque judgments, highlighting the need for a more effective evaluation method.

Method: The paper adapts AHP to LLM-based evaluation and proposes a confidence-aware FAHP extension, which models epistemic uncertainty via triangular fuzzy numbers modulated by LLM-generated confidence scores.

Result: The proposed approach consistently outperforms direct scoring across model scales and dataset splits, with FAHP showing superior stability in uncertain comparison scenarios. The hybrid framework DualJudge achieves state-of-the-art performance.

Conclusion: The paper establishes uncertainty-aware structured reasoning as a principled pathway toward more reliable LLM assessment, highlighting the complementary strengths of intuitive and deliberative evaluation paradigms.
...
### [307] [RL-Driven Sustainable Land-Use Allocation for the Lake Malawi Basin](https://arxiv.org/abs/2604.03768)
*Ying Yao*

Main category: cs.AI

TL;DR: A deep reinforcement learning framework is proposed to optimize land-use allocation in the Lake Malawi Basin for maximizing ecosystem service value, considering ecological and spatial objectives.


<details>
  <summary>Details</summary>
Motivation: Unsustainable land-use practices threaten biodiversity, water resources, and livelihoods in ecologically sensitive regions.

Method: A deep reinforcement learning framework using Proximal Policy Optimization (PPO) with action masking and a reward function combining ecological value and spatial coherence objectives.

Result: The agent effectively learns to increase total ecosystem service value, and spatial reward shaping steers allocations toward ecologically sound patterns.

Conclusion: The framework is useful as a scenario-analysis tool for environmental planning, responding meaningfully to policy parameter changes.
...
### [308] [Decomposing Communication Gain and Delay Cost Under Cross-Timestep Delays in Cooperative Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2604.03785)
*Zihong Gao,Hongjian Liang,Lei Hao,Liangjun Ke*

Main category: cs.AI

TL;DR: This paper addresses the challenge of delayed communication in cooperative multi-agent reinforcement learning under partial observability. It proposes a framework called CDCMA, which selectively requests messages, predicts future observations, and fuses delayed messages to improve performance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to tackle the issue of cross-timestep delays in communication that causes temporal misalignment and makes information stale in cooperative multi-agent reinforcement learning under partial observability.

Method: The method involves formalizing the setting as a delayed-communication partially observable Markov game (DeComm-POMG), decomposing a message's effect into communication gain and delay cost (CGDC metric), and proposing the CDCMA framework. CDCMA requests messages based on predicted CGDC, predicts future observations, and fuses delayed messages via CGDC-guided attention.

Result: The results show consistent improvements in performance, robustness, and generalization across various experiments, including no-teammate-vision variants of Cooperative Navigation and Predator Prey, and SMAC maps with multiple delay levels.

Conclusion: The conclusion is that the proposed CDCMA framework effectively addresses the challenge of delayed communication in cooperative multi-agent reinforcement learning under partial observability, leading to improved performance, robustness, and generalization.
...
### [309] [Affording Process Auditability with QualAnalyzer: An Atomistic LLM Analysis Tool for Qualitative Research](https://arxiv.org/abs/2604.03820)
*Max Hao Lu,Ryan Ellegood,Rony Rodriguez-Ramirez,Sophia Blumert*

Main category: cs.AI

TL;DR: The authors present QualAnalyzer, a Chrome extension for transparent LLM-assisted qualitative data analysis, and demonstrate its effectiveness through two case studies.


<details>
  <summary>Details</summary>
Motivation: The increasing use of large language models (LLMs) in qualitative data analysis has led to a lack of transparency in how analytic conclusions are produced, making process auditability essential for robust research.

Method: The authors developed QualAnalyzer, an open-source Chrome extension that enables atomistic LLM analysis by processing each data segment independently and preserving the prompt, input, and output for every unit.

Result: The case studies demonstrate that QualAnalyzer creates a legible audit trail and helps researchers investigate systematic differences between LLM and human judgments, making LLM-assisted qualitative research more transparent and methodologically robust.

Conclusion: The authors conclude that process auditability is crucial for making LLM-assisted qualitative research more transparent and methodologically robust, and that QualAnalyzer is a valuable tool for achieving this goal.
...
### [310] [PolySwarm: A Multi-Agent Large Language Model Framework for Prediction Market Trading and Latency Arbitrage](https://arxiv.org/abs/2604.03888)
*Rajat M. Barot,Arjun S. Borkhatariya*

Main category: cs.AI

TL;DR: PolySwarm is a multi-agent LLM framework for real-time prediction market trading and latency arbitrage on decentralized platforms, outperforming single-model baselines in probability calibration.


<details>
  <summary>Details</summary>
Motivation: The motivation is to develop a system that can effectively predict market outcomes and execute trades in real-time on decentralized platforms like Polymarket, addressing challenges such as market inefficiencies and latency.

Method: PolySwarm uses a swarm of 50 diverse LLM personas to evaluate binary outcome markets, aggregating probability estimates through confidence-weighted Bayesian combination, and applies quarter-Kelly position sizing for risk-controlled execution. It also incorporates an information-theoretic market analysis engine using KL and JS divergence.

Result: Experimental results show that PolySwarm's swarm aggregation consistently outperforms single-model baselines in probability calibration on Polymarket prediction tasks.

Conclusion: The paper concludes that PolySwarm is a promising approach for real-time prediction market trading and latency arbitrage, but also highlights open challenges such as hallucination in agent pools, computational cost at scale, and regulatory exposure.
...
### [311] [FeynmanBench: Benchmarking Multimodal LLMs on Diagrammatic Physics Reasoning](https://arxiv.org/abs/2604.03893)
*Zeyu Wang,Xiaogang Li,Peiyao Xiao,Qinhao Kong,Ben Wang,Chengliang Xu,Zichao Chen,Bing Zhao,Hu Wei*

Main category: cs.AI

TL;DR: The paper introduces FeynmanBench, a benchmark for evaluating AI's ability to reason with Feynman diagrams, which requires multistep diagrammatic reasoning, conservation laws, and symmetry constraints.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks for multimodal large language models (MLLMs) focus on local information extraction, but neglect the global structural logic inherent in formal scientific notations, such as Feynman diagrams.

Method: The authors developed FeynmanBench, a benchmark consisting of over 2000 tasks, and an automated pipeline to produce diverse Feynman diagrams with verifiable topological annotations and amplitude results.

Result: Experiments on state-of-the-art MLLMs revealed systematic failure modes, including unstable enforcement of physical constraints and violations of global topological conditions.

Conclusion: FeynmanBench provides a logically rigorous test for AI's ability to engage in scientific discovery, particularly in theoretical physics, and highlights the need for physics-grounded benchmarks for visual reasoning over scientific notation.
...
### [312] [LLM-Agent-based Social Simulation for Attitude Diffusion](https://arxiv.org/abs/2604.03898)
*Deepak John Reji*

Main category: cs.AI

TL;DR: Introduces discourse_simulator, an open-source framework combining LLMs with agent-based modelling to simulate public attitude changes toward immigration.


<details>
  <summary>Details</summary>
Motivation: To provide a new way to simulate public attitude changes toward immigration in response to salient events, addressing limitations of traditional agent-based models.

Method: Uses LLMs to generate social media posts, interpret opinions, and model idea spread through social networks, integrated into a small-world network topology and live news retrieval system.

Result: Demonstrates the framework by modelling the Dublin anti-immigration march on April 26, 2025, with N=100 agents over a 15-day simulation.

Conclusion: The framework offers a new approach for social science research on attitude dynamics, polarisation, and belief evolution, treating simulations as a theory-testing instrument.
...
### [313] [CODE-GEN: A Human-in-the-Loop RAG-Based Agentic AI System for Multiple-Choice Question Generation](https://arxiv.org/abs/2604.03926)
*Xiaojing Duan,Frederick Nwanganga,Chaoli Wang*

Main category: cs.AI

TL;DR: CODE-GEN is an AI system that generates multiple-choice coding comprehension questions, evaluated by human experts, achieving high reliability in certain pedagogical dimensions.


<details>
  <summary>Details</summary>
Motivation: To develop student code reasoning and comprehension abilities by generating context-aligned multiple-choice questions.

Method: CODE-GEN uses a retrieval-augmented generation (RAG)-based agentic AI system with a Generator agent and a Validator agent, both augmented with specialized tools.

Result: The system achieved human-validated success rates ranging from 79.9% to 98.6% across seven pedagogical dimensions, with high reliability in dimensions like question clarity and code validity.

Conclusion: The findings suggest that AI can be effectively used in educational content generation, but human expertise remains essential for certain dimensions requiring deeper instructional judgment.
...
### [314] [SKILLFOUNDRY: Building Self-Evolving Agent Skill Libraries from Heterogeneous Scientific Resources](https://arxiv.org/abs/2604.03964)
*Shuaike Shen,Wenduo Cheng,Mingqian Ma,Alistair Turcan,Martin Jinye Zhang,Jian Ma*

Main category: cs.AI

TL;DR: SkillFoundry is a framework that converts scientific knowledge into reusable agent skills, improving agent performance on scientific tasks.


<details>
  <summary>Details</summary>
Motivation: The gap between abundant scientific knowledge and usable agent capabilities is a key bottleneck for building effective scientific agents.

Method: SkillFoundry organizes a target domain as a domain knowledge tree, mines resources, extracts operational contracts, and compiles them into executable skill packages through a closed-loop validation process.

Result: SkillFoundry produces a novel and internally valid skill library, improving coding agent performance on MoSciBench datasets and two genomics tasks.

Conclusion: Automatically mined skills improve agent performance, expand coverage beyond hand-crafted libraries, and provide a foundation for more capable scientific agents.
...
### [315] [Quantifying Trust: Financial Risk Management for Trustworthy AI Agents](https://arxiv.org/abs/2604.03976)
*Wenyue Hua,Tianyi Peng,Chi Wang,Ian Kaufman,Bryan Lim,Chandler Fang*

Main category: cs.AI

TL;DR: The paper proposes the Agentic Risk Standard (ARS), a framework for managing risks associated with autonomous AI agents by providing a payment settlement standard for AI-mediated transactions, ensuring users receive compensation in cases of execution failure or unintended outcomes.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the gap between model-level reliability and user-facing assurance in trustworthy AI, as the operational meaning of trust shifts to end-to-end outcomes in autonomous agents deployed in open environments.

Method: The paper proposes a complementary framework based on risk management, introducing the Agentic Risk Standard (ARS), which integrates risk assessment, underwriting, and compensation into a single transaction framework.

Result: The simulation study analyzing the social benefits of applying ARS to agentic transactions shows that ARS can provide a measurable and enforceable product guarantee, shifting trust from an implicit expectation about model behavior to an explicit one.

Conclusion: The paper concludes that the Agentic Risk Standard (ARS) can effectively address the gap between model-level reliability and user-facing assurance, providing a payment settlement standard for AI-mediated transactions that protects users when interacting with agents.
...
### [316] [FactReview: Evidence-Grounded Reviews with Literature Positioning and Execution-Based Claim Verification](https://arxiv.org/abs/2604.04074)
*Hang Xu,Ling Yue,Chaoqian Ouyang,Libin Zheng,Shaowu Pan,Shimin Di,Min-Ling Zhang*

Main category: cs.AI

TL;DR: FactReview is an evidence-grounded reviewing system for machine learning papers, combining claim extraction, literature positioning, and execution-based claim verification to provide more accurate and evidence-based peer reviews.


<details>
  <summary>Details</summary>
Motivation: The growing pressure on peer review in machine learning due to rising submission volume and limited reviewer time, and the limitations of current LLM-based reviewing systems that rely only on the manuscript narrative.

Method: FactReview identifies major claims and reported results, retrieves nearby work, and executes released code under bounded budgets to test central empirical claims, producing a concise review and evidence report.

Result: In a case study on CompGCN, FactReview reproduces results that closely match those reported for link prediction and node classification, but shows that the paper's broader performance claim across tasks is only partially supported.

Conclusion: AI is most useful in peer review not as a final decision-maker, but as a tool for gathering evidence and helping reviewers produce more evidence-grounded assessments.
...
### [317] [Compliance-by-Construction Argument Graphs: Using Generative AI to Produce Evidence-Linked Formal Arguments for Certification-Grade Accountability](https://arxiv.org/abs/2604.04103)
*Mahyar T. Moghaddam*

Main category: cs.AI

TL;DR: The paper proposes an architecture that integrates Generative AI with structured formal argument representations to ensure accountability and regulatory compliance in high-stakes decision systems.


<details>
  <summary>Details</summary>
Motivation: The increasing use of Generative AI in decision-support workflows introduces risks such as hallucinated reasoning, unsupported claims, and weak traceability, highlighting the need for a compliance-by-construction architecture.

Method: The proposed architecture combines four components: a typed Argument Graph representation, retrieval-augmented generation (RAG), a reasoning and validation kernel, and a provenance ledger.

Result: The analysis suggests that deterministic validation rules can prevent unsupported claims from entering the decision record while allowing GenAI to accelerate argument construction.

Conclusion: The proposed architecture provides a mechanism for structuring claims, reasoning, and evidence in a verifiable manner, ensuring accountability and regulatory compliance in high-stakes decision systems.
...
### [318] [InsTraj: Instructing Diffusion Models with Travel Intentions to Generate Real-world Trajectories](https://arxiv.org/abs/2604.04106)
*Yuanshao Zhu,Yuxuan Liang,Xiangyu Zhao,Liang Han,Xinwei Fang,Xuetao Wei,James Jianqiao Yu*

Main category: cs.AI

TL;DR: InsTraj is a novel framework that generates realistic GPS trajectories from natural language descriptions, outperforming state-of-the-art methods in realism, diversity, and semantic faithfulness.


<details>
  <summary>Details</summary>
Motivation: Existing methods for generating GPS trajectories lack deep semantic understanding of complex user travel intent and struggle to handle complex constraints while maintaining realistic diversity.

Method: InsTraj uses a large language model to decipher travel intentions from natural language and a multimodal trajectory diffusion transformer to generate high-fidelity trajectories that adhere to user intent.

Result: InsTraj significantly outperforms state-of-the-art methods in generating realistic, diverse, and semantically faithful trajectories on real-world datasets.

Conclusion: InsTraj provides a novel solution for generating realistic and controllable GPS trajectories, bridging the representation gap between intentions and trajectories.
...
### [319] [Profile-Then-Reason: Bounded Semantic Complexity for Tool-Augmented Language Agents](https://arxiv.org/abs/2604.04131)
*Paulo Akira F. Enabe*

Main category: cs.AI

TL;DR: Introduced Profile-Then-Reason (PTR), a framework for structured tool-augmented reasoning that improves latency and error sensitivity in large language model agents.


<details>
  <summary>Details</summary>
Motivation: Current reactive execution methods in large language model agents lead to high latency and error propagation due to repeated recomputation after each observation.

Method: PTR involves synthesizing an explicit workflow, executing it with deterministic or guarded operators, verifying the trace, and invoking repair only when necessary.

Result: PTR achieved a pairwise exact-match advantage in 16 of 24 configurations across six benchmarks and four language models, particularly exceling in retrieval-centered and decomposition-heavy tasks.

Conclusion: PTR offers a more efficient and reliable approach to tool-augmented reasoning, especially for tasks that benefit from structured workflows and verification.
...
### [320] [Solar-VLM: Multimodal Vision-Language Models for Augmented Solar Power Forecasting](https://arxiv.org/abs/2604.04145)
*Hang Fan,Haoran Pei,Runze Liang,Weican Liu,Long Cheng,Wei Wei*

Main category: cs.AI

TL;DR: This paper proposes Solar-VLM, a framework for multimodal PV power forecasting that integrates temporal observations, satellite imagery, and textual weather information.


<details>
  <summary>Details</summary>
Motivation: Most existing AI-based forecasting methods fail to fuse multiple information sources, and accurate PV power forecasting requires effective modeling of complex spatiotemporal dependencies.

Method: The proposed framework uses modality-specific encoders to extract features from heterogeneous inputs, and a cross-site feature fusion mechanism to capture spatial dependencies across geographically distributed PV stations.

Result: Experiments on data from eight PV stations in China demonstrate the effectiveness of the proposed framework.

Conclusion: The proposed Solar-VLM framework provides a unified approach for multimodal PV power forecasting, leveraging large-language-model-driven techniques to improve forecasting accuracy.
...
### [321] [Readable Minds: Emergent Theory-of-Mind-Like Behavior in LLM Poker Agents](https://arxiv.org/abs/2604.04157)
*Hsieh-Ting Lin,Tsung-Yu Hou*

Main category: cs.AI

TL;DR: Large language models can develop Theory of Mind-like behavior through dynamic interaction, specifically when equipped with persistent memory, and demonstrate sophisticated opponent models in a poker game.


<details>
  <summary>Details</summary>
Motivation: To investigate whether large language models (LLMs) can develop Theory of Mind (ToM) through dynamic interaction, and to understand the role of memory in this process.

Method: A 2x2 factorial design experiment was conducted, where autonomous LLM agents played extended sessions of Texas Hold'em poker with or without persistent memory and domain knowledge. The experiment consisted of 20 runs, with approximately 6,000 agent-hand observations.

Result: The results show that LLM agents with persistent memory developed ToM-like behavior, reaching levels 3-5 of ToM, while agents without memory remained at level 0. The presence of memory was both necessary and sufficient for ToM-like behavior emergence. Agents with ToM deviated from optimal play to exploit specific opponents, similar to expert human play.

Conclusion: The study demonstrates that functional ToM-like behavior can emerge from interaction dynamics alone, without explicit training or prompting, and highlights the importance of memory in this process. The findings have implications for understanding artificial social intelligence and biological social cognition.
...
### [322] [A Model of Understanding in Deep Learning Systems](https://arxiv.org/abs/2604.04171)
*David Peter Wallis Freeborn*

Main category: cs.AI

TL;DR: The paper proposes a model of systematic understanding for machine learning systems and argues that contemporary deep learning systems can achieve understanding, but it is limited and 'fractured'.


<details>
  <summary>Details</summary>
Motivation: The author aims to develop a model of systematic understanding suitable for machine learning systems and to assess the capabilities of contemporary deep learning systems in achieving such understanding.

Method: The author proposes a model of understanding based on three conditions: an adequate internal model that tracks real regularities, coupling to the target by stable bridge principles, and support for reliable prediction.

Result: The author concludes that while deep learning systems can achieve understanding, it is symbolically misaligned with the target system, not explicitly reductive, and only weakly unifying, which is labeled as the Fractured Understanding Hypothesis.

Conclusion: The paper concludes that deep learning systems have limited understanding, characterized as 'fractured', which falls short of the ideal of scientific understanding.
...
### [323] [CoALFake: Collaborative Active Learning with Human-LLM Co-Annotation for Cross-Domain Fake News Detection](https://arxiv.org/abs/2604.04174)
*Esma Aïmeur,Gilles Brassard,Dorsaf Sallami*

Main category: cs.AI

TL;DR: A novel approach, CoALFake, is proposed for cross-domain fake news detection, integrating human-LLM co-annotation and domain-aware active learning, outperforming existing baselines.


<details>
  <summary>Details</summary>
Motivation: Current fake news detection systems have limitations in domain specificity and generalization, and existing cross-domain approaches rely on labelled data and neglect domain-specific features.

Method: CoALFake uses human-LLM co-annotation with domain-aware Active Learning (AL), employing LLMs for scalable annotation and human oversight for label reliability.

Result: Experimental results across multiple datasets show that CoALFake consistently outperforms various baselines, even with minimal human oversight.

Conclusion: The proposed approach, CoALFake, provides a highly cost-effective solution for cross-domain fake news detection, delivering excellent performance with human-LLM co-annotation.
...
### [324] [Comparative reversal learning reveals rigid adaptation in LLMs under non-stationary uncertainty](https://arxiv.org/abs/2604.04182)
*Haomiaomiao Wang,Tomás E Ward,Lili Zhang*

Main category: cs.AI

TL;DR: The paper evaluates large language models (LLMs) in a probabilistic reversal-learning task, comparing their adaptability to changing contingencies and use of positive vs. negative evidence.


<details>
  <summary>Details</summary>
Motivation: Non-stationary environments require agents to adapt to changing contingencies, and this study aims to assess the ability of LLMs to revise previously learned action values in such environments.

Method: The study uses a two-option probabilistic reversal-learning task with three latent states and evaluates LLMs (DeepSeek-V3.2, Gemini-3, and GPT-5.2) and human data under deterministic and stochastic transition schedules.

Result: The results show that LLMs exhibit asymmetric use of positive vs. negative evidence, with win-stay near ceiling but lose-shift attenuated; Gemini-3 and GPT-5.2 adapt more rapidly than DeepSeek-V3.2 but remain less loss-sensitive than humans.

Conclusion: The study highlights the need for reversal-sensitive diagnostics and volatility-aware models to evaluate LLMs under non-stationary uncertainty, and suggests that rigidity in LLMs can arise from weak loss learning, inflated policy determinism, or value polarisation via counterfactual suppression.
...
### [325] [Schema-Aware Planning and Hybrid Knowledge Toolset for Reliable Knowledge Graph Triple Verification](https://arxiv.org/abs/2604.04190)
*Xinyan Ma,Xianhao Ou,Weihao Zhang,Shixin Jiang,Runxuan Liu,Dandan Tu,Lei Chen,Ming Liu,Bing Qin*

Main category: cs.AI

TL;DR: The paper proposes SHARP, a training-free autonomous agent for reliable triple verification in Knowledge Graphs, addressing limitations of existing methods with a dynamic process of strategic planning, active investigation, and evidential reasoning.


<details>
  <summary>Details</summary>
Motivation: The automated construction of Knowledge Graphs introduces noise, compromising data trustworthiness, and existing triple verification methods suffer from single-source bias and limited interpretability.

Method: SHARP combines a Memory-Augmented Mechanism with Schema-Aware Strategic Planning and an enhanced ReAct loop with a Hybrid Knowledge Toolset to dynamically integrate internal KG structure and external textual evidence.

Result: SHARP significantly outperforms existing state-of-the-art baselines on FB15K-237 and Wikidata5M-Ind, achieving accuracy gains of 4.2% and 12.9%, respectively, and provides transparent, fact-based evidence chains for each judgment.

Conclusion: The proposed SHARP agent demonstrates strong interpretability and robustness for complex verification tasks, addressing the limitations of existing triple verification methods.
...
### [326] [Don't Blink: Evidence Collapse during Multimodal Reasoning](https://arxiv.org/abs/2604.04207)
*Suresh Raghu,Satwik Pandey*

Main category: cs.AI

TL;DR: The paper analyzes the visual grounding of reasoning in VLMs, finding a phenomenon where models lose attention to evidence as reasoning unfolds, and proposes a targeted vision veto to mitigate risks.


<details>
  <summary>Details</summary>
Motivation: The authors aim to investigate the visual grounding of reasoning in VLMs, as they can become accurate but ungrounded, creating 'danger zones' that text-only monitoring cannot detect.

Method: The authors evaluate three VLMs on multiple datasets, analyzing attention to annotated evidence regions and proposing an entropy-vision interaction model to identify hazardous predictions.

Result: The study finds a pervasive 'evidence-collapse' phenomenon, where attention to evidence drops substantially as reasoning unfolds, and a targeted vision veto can reduce selective risk by up to 1.9 percentage points.

Conclusion: The results support task-aware multimodal monitoring for safe deployment of VLMs under distribution shift, highlighting the importance of considering visual grounding in reasoning.
...
### [327] [TimeSeek: Temporal Reliability of Agentic Forecasters](https://arxiv.org/abs/2604.04220)
*Hamza Mostafa,Om Shastri,Dennis Lee*

Main category: cs.AI

TL;DR: This paper introduces TimeSeek, a benchmark for evaluating the reliability of LLM forecasters over a prediction market's lifecycle, and provides insights into model performance, the impact of web search, and ensemble methods.


<details>
  <summary>Details</summary>
Motivation: The authors aim to study how the reliability of agentic LLM forecasters changes over a prediction market's lifecycle, and to evaluate the effectiveness of different models and tools.

Method: The authors evaluate 10 frontier models on 150 CFTC-regulated Kalshi binary markets at five temporal checkpoints, with and without web search, for a total of 15,000 forecasts.

Result: The results show that models are most competitive early in a market's life and on high-uncertainty markets, but less competitive near resolution and on strong-consensus markets. Web search improves pooled Brier Skill Score (BSS) for every model overall, but hurts in 12% of model-checkpoint pairs. Simple two-model ensembles reduce error without surpassing the market overall.

Conclusion: The authors conclude that time-aware evaluation and selective-deference policies are needed, rather than a single market snapshot or a uniform tool-use setting.
...
### [328] [Pedagogical Safety in Educational Reinforcement Learning: Formalizing and Detecting Reward Hacking in AI Tutoring Systems](https://arxiv.org/abs/2604.04237)
*Oluseyi Olukola,Nick Rahimi*

Main category: cs.AI

TL;DR: The paper introduces a framework for evaluating pedagogical safety in educational reinforcement learning (RL) and proposes a metric to quantify misalignment between proxy rewards and genuine learning.


<details>
  <summary>Details</summary>
Motivation: The field lacks a formal framework for defining and evaluating pedagogical safety in educational RL, which is increasingly used to personalize instruction in intelligent tutoring systems.

Method: The authors propose a four-layer model of pedagogical safety and a Reward Hacking Severity Index (RHSI) metric. They evaluate the framework in a controlled simulation of an AI tutoring environment with 120 sessions and 18,000 interactions.

Result: The results show that an engagement-optimized agent can lead to limited learning progress, and a constrained architecture combining prerequisite enforcement and minimum cognitive demand can substantially reduce reward hacking.

Conclusion: The paper suggests that reward design alone may be insufficient to ensure pedagogically aligned behavior in educational RL, and positions pedagogical safety as an important research problem at the intersection of AI safety and intelligent educational systems.
...
### [329] [Combee: Scaling Prompt Learning for Self-Improving Language Model Agents](https://arxiv.org/abs/2604.04247)
...
TL;DR: Combee is a novel framework for parallel prompt learning that improves efficiency and quality in self-improving agents.


<details>
  <summary>Details</summary>
Motivation: Current prompt learning methods are limited in their ability to efficiently learn from large sets of collected agentic traces, particularly in high-parallelism settings.

Method: Combee uses parallel scans, an augmented shuffle mechanism, and a dynamic batch size controller to balance quality and delay.

Result: Combee achieves up to 17x speedup over previous methods with comparable or better accuracy and equivalent cost.

Conclusion: Combee is an effective solution for scaling parallel prompt learning, enabling efficient and high-quality learning from large sets of agentic traces.
...
### [330] [MC-CPO: Mastery-Conditioned Constrained Policy Optimization](https://arxiv.org/abs/2604.04251)
*Oluseyi Olukola,Nick Rahimi*

Main category: cs.AI

TL;DR: The authors propose a new algorithm, MC-CPO, to mitigate reward hacking in adaptive tutoring systems by integrating structural action masking with constrained policy optimization.


<details>
  <summary>Details</summary>
Motivation: Engagement-optimized adaptive tutoring systems may prioritize short-term behavioral signals over sustained learning outcomes, creating incentives for reward hacking in reinforcement learning policies.

Method: The authors formalize the challenge as a constrained Markov decision process (CMDP) and introduce Mastery-Conditioned Constrained Policy Optimization (MC-CPO), a two-timescale primal-dual algorithm.

Result: MC-CPO satisfies constraint budgets, reduces discounted safety costs, and lowers the Reward Hacking Severity Index (RHSI) in various environments, including a neural tutoring setting.
...
### [331] [Context Engineering: A Practitioner Methodology for Structured Human-AI Collaboration](https://arxiv.org/abs/2604.04258)
*Elias Calboreanu*

Main category: cs.AI

TL;DR: The paper introduces Context Engineering, a methodology for improving AI output quality by structuring the informational context accompanying a prompt, and presents observational study results showing improved efficiency and success rates.


<details>
  <summary>Details</summary>
Motivation: The quality of AI-generated output is often attributed to prompting technique, but the authors suggest that context completeness may be more strongly associated with output quality.

Method: The authors introduce Context Engineering, which involves a five-role context package structure, a four-phase pipeline, and formal models from reliability engineering and information theory.

Result: The observational study showed that structured context assembly was associated with a reduction in iteration cycles and an improvement in first-pass acceptance, with a final success rate of 91.5%.

Conclusion: The paper concludes that Context Engineering is a promising approach for improving AI output quality by ensuring complete and structured context, but notes that the results are observational and require further corroboration.
...
### [332] [Beyond Fluency: Toward Reliable Trajectories in Agentic IR](https://arxiv.org/abs/2604.04269)
*Anushree Sinha,Srivaths Ranganathan,Debanshu Das,Abhishek Dharmaratnakar*

Main category: cs.AI

TL;DR: The paper discusses the shift in Information Retrieval towards autonomous workflows and the need for verification and process correctness to address errors and ensure safe deployment.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenges of autonomous agentic workflows in Information Retrieval, where minor early errors can lead to significant issues, and to ensure safe deployment by prioritizing process correctness and grounded execution.

Method: The paper proposes verification gates at each interaction unit and systematic abstention under calibrated uncertainty to address compounding errors and deceptive fluency.

Result: The paper argues that safe deployment requires moving beyond endpoint accuracy toward trajectory integrity and causal attribution, and prioritizing process correctness and grounded execution.

Conclusion: The conclusion is that reliable Agentic IR systems must prioritize process correctness and grounded execution over plausible but unverified completion to ensure safe deployment.
...
### [333] [InferenceEvolve: Towards Automated Causal Effect Estimators through Self-Evolving AI](https://arxiv.org/abs/2604.04274)
*Can Wang,Hongyu Zhao,Yiqun Chen*

Main category: cs.AI

TL;DR: InferenceEvolve, a framework using large language models, evolves causal inference methods that outperform established baselines.


<details>
  <summary>Details</summary>
Motivation: Choosing appropriate causal inference methods is challenging due to complexity in statistical methodology and real-world data.

Method: An evolutionary framework, InferenceEvolve, that uses large language models to discover and refine causal methods.

Result: InferenceEvolve yields estimators that consistently outperform established baselines across benchmarks and evaluation metrics.

Conclusion: Language-model-guided evolution can optimize structured scientific programs like causal inference, even with partial observations.
...
### [334] [Preservation Is Not Enough for Width Growth: Regime-Sensitive Selection of Dense LM Warm Starts](https://arxiv.org/abs/2604.04281)
*Eren Unlu*

Main category: cs.AI

TL;DR: The study explores methods for selecting a widened warm start in dense width growth of causal-language-model checkpoints, comparing various methods under different conditions.


<details>
  <summary>Details</summary>
Motivation: The motivation is to find a practical method for reusing smaller causal-language-model checkpoints by expanding their width, which requires selecting an effective widened warm start.

Method: The study compares exact-copy, perturbative, asymmetric-reset, and structured non-clone warm starts under matched continuation budgets, evaluating their performance in deterministic and stochastic regimes.

Result: The results show that exact-copy symmetric warm starts perform best in most cases, but structured non-clone warm starts excel in deterministic 128-step continuation, indicating that the best method depends on the regime and lag budget.

Conclusion: The conclusion is that preservation is not a universal ranking criterion for dense width growth, and the best replacement signal depends on both regime and lag budget.
...
### [335] [PanLUNA: An Efficient and Robust Query-Unified Multimodal Model for Edge Biosignal Intelligence](https://arxiv.org/abs/2604.04297)
*Marija Zelic,Anna Tegon,Yawei Li,Thorir Mar Ingolfsson,Luca Benini*

Main category: cs.AI

TL;DR: PanLUNA, a compact pan-modal physiological foundation model, processes EEG, ECG, and PPG signals together, achieving state-of-the-art performance in biosignal representation learning while being efficient and robust.


<details>
  <summary>Details</summary>
Motivation: The scarcity of paired multimodal datasets and the limitations of single-modal models motivated the development of a pan-modal foundation model that can jointly process multiple biosignals.

Method: PanLUNA uses a shared encoder with a channel-unification module and sensor-type embeddings to enable efficient cross-modal early fusion and robustness to missing modalities.

Result: PanLUNA achieved 81.21% balanced accuracy on TUAB abnormal EEG detection and state-of-the-art 0.7416 balanced accuracy on HMC multimodal sleep staging, matching or exceeding larger models.

Conclusion: PanLUNA offers a compact and efficient solution for pan-modal biosignal representation learning, demonstrating promising performance and deployment feasibility on ultra-low-power microcontrollers.
...
### [336] [RESCORE: LLM-Driven Simulation Recovery in Control Systems Research Papers](https://arxiv.org/abs/2604.04324)
*Vineet Bhat,Shiqing Wei,Ali Umut Kaypak,Prashanth Krishnamurthy,Ramesh Karri,Farshad Khorrami*

Main category: cs.AI

TL;DR: The authors propose RESCORE, a framework to automatically generate executable code that reproduces results from control systems research papers, achieving a 40.7% success rate and a 10X speedup over manual replication.


<details>
  <summary>Details</summary>
Motivation: Reconstructing numerical simulations from control systems research papers is often hindered by underspecified parameters and ambiguous implementation details, making it difficult to verify published control methodologies.

Method: The authors propose a three-component LLM agentic framework, RESCORE, consisting of Analyzer, Coder, and Verifier, which uses iterative execution feedback and visual comparison to improve reconstruction fidelity.

Result: RESCORE successfully recovers task-coherent simulations for 40.7% of benchmark instances, outperforming single-pass generation and achieving an estimated 10X speedup over manual human replication.

Conclusion: The proposed RESCORE framework and benchmark can foster community progress in automated research replication, drastically cutting the time and effort required to verify published control methodologies.
...
### [337] [Soft Tournament Equilibrium](https://arxiv.org/abs/2604.04328)
*Saad Alqithami*

Main category: cs.AI

TL;DR: This paper proposes a new framework, Soft Tournament Equilibrium (STE), for evaluating general-purpose artificial agents in cyclic domains, providing a more robust and nuanced assessment of agent capabilities.


<details>
  <summary>Details</summary>
Motivation: Traditional ranking methods can be misleading and unstable in cyclic domains where interactions are non-transitive, necessitating a new evaluation approach.

Method: STE is a differentiable framework that learns a probabilistic tournament model from pairwise comparison data and computes set-valued tournament solutions using novel operators.

Result: The framework provides a set of core agents with calibrated membership scores, offering a robust assessment of agent capabilities.

Conclusion: The paper introduces STE as a more appropriate and robust theoretical foundation for evaluating general-purpose artificial agents, moving from unstable rankings to stable, set-valued equilibria.
...
### [338] [Thermodynamic-Inspired Explainable GeoAI: Uncovering Regime-Dependent Mechanisms in Heterogeneous Spatial Systems](https://arxiv.org/abs/2604.04339)
*Sooyoung Lim,Zhenlong Li,Zi-Kui Liu*

Main category: cs.AI

TL;DR: A new thermodynamics-inspired geospatial AI framework is proposed to model spatial heterogeneity and critical transitions, outperforming conventional models by identifying regime-dependent role reversals of predictors.


<details>
  <summary>Details</summary>
Motivation: Conventional models fail to elucidate state-dependent nonlinearities in spatial processes, and there is a need for a more interpretable approach.

Method: The framework integrates statistical mechanics with graph neural networks, conceptualizing spatial variability as a thermodynamic competition between system Burden (E) and Capacity (S).

Result: The framework successfully identifies regime-dependent role reversals of predictors and diagnoses the phase transition into a Burden-dominated regime during the 2023 Canadian wildfire event.

Conclusion: The proposed framework improves the interpretability of GeoAI while preserving strong predictive performance in complex spatial systems.
...
### [339] [Implementing surrogate goals for safer bargaining in LLM-based agents](https://arxiv.org/abs/2604.04341)
*Caspar Oesterheld,Maxime Riché,Filip Sondej,Jesse Clifton,Vincent Conitzer*

Main category: cs.AI

TL;DR: The paper implements surrogate goals in language-model-based agents to reduce risks from bargaining failures by deflecting threats away from the principal's goals.


<details>
  <summary>Details</summary>
Motivation: To mitigate risks from bargaining failures by using surrogate goals, which are alternative objectives given to an AI agent to protect the principal's interests.

Method: The paper proposes and evaluates four methods for implementing surrogate goals in language-model-based agents: prompting, fine-tuning, and scaffolding, with a focus on getting the agent to react equally to threats against the surrogate goal and the principal's goal.

Result: The methods based on scaffolding and fine-tuning outperform simple prompting, with scaffolding-based methods performing best in terms of precisely implementing the desired behavior and minimizing side effects on capabilities and propensities.

Conclusion: The study demonstrates the effectiveness of surrogate goals in language-model-based agents for reducing bargaining risks, with scaffolding-based methods showing the most promising results.
...
### [340] [Domain-Contextualized Inference: A Computable Graph Architecture for Explicit-Domain Reasoning](https://arxiv.org/abs/2604.04344)
*Chao Li,Yuru Wang,Chunyu Zhao*

Main category: cs.AI

TL;DR: The paper presents a new inference architecture that is substrate-agnostic, allowing for efficient computation across different domains and substrates.


<details>
  <summary>Details</summary>
Motivation: The authors aim to establish a flexible and efficient computation framework that can handle various domains and substrates, reducing the search space and improving reliability.

Method: The authors propose a five-layer architecture with a substrate-agnostic interface, three domain computation modes, and reliability conditions. They formalize the computational theory across five dimensions.

Result: The proposed architecture achieves domain-scoped pruning, reducing per-query search space from O(N) to O(N/K), and enables substrate-independent execution over various substrates.

Conclusion: The paper contributes a novel inference architecture and a formalized computational theory, validated through a clinical reasoning case study, which provides a foundation for efficient and reliable computation across domains and substrates.
...
### [341] [RoboPhD: Evolving Diverse Complex Agents Under Tight Evaluation Budgets](https://arxiv.org/abs/2604.04347)
*Andrew Borthwick,Stephen Ash,Anthony Galczak*

Main category: cs.AI

TL;DR: This paper compares optimization algorithms for LLM-guided evolution of agentic artifacts, finding that RoboPhD outperforms GEPA and Autoresearch on three of four benchmarks.


<details>
  <summary>Details</summary>
Motivation: The motivation is to determine which optimization algorithm yields the best results under the same evaluation budget, given the same information, seed agent, and objective.

Method: The paper presents a systematic comparison of three optimization paradigms: Elo tournament selection (RoboPhD), Pareto-based selection (GEPA), and greedy hill-climbing (Autoresearch), across four benchmarks.

Result: RoboPhD outperforms GEPA and Autoresearch on three of four benchmarks, and evolves a 22-line seed agent into a 1,013-line multi-strategy system with improved accuracy.

Conclusion: The conclusion is that RoboPhD is a versatile and effective optimization algorithm for LLM-guided evolution of agentic artifacts, and it is released as a toolkit under the MIT license.
...
### [342] [REAM: Merging Improves Pruning of Experts in LLMs](https://arxiv.org/abs/2604.04356)
*Saurav Jha,Maryam Hashemzadeh,Ali Saheb Pasand,Ali Parviz,Min-Joong Lee,Boris Knyazev*

Main category: cs.AI

TL;DR: Proposed REAM method for Mixture-of-Experts large language models, which merges experts' weights to preserve performance while reducing memory requirements.


<details>
  <summary>Details</summary>
Motivation: Traditional methods like weight pruning and quantization have limitations; motivated by REAP, which prunes experts.

Method: Router-weighted Expert Activation Merging (REAM) groups and merges experts' weights.

Result: REAM often outperforms baselines and is comparable to original uncompressed models; reveals trade-off between multiple-choice and generative performance based on calibration data.

Conclusion: REAM offers a viable approach to reducing memory requirements of large language models while preserving performance.
...
### [343] [Decocted Experience Improves Test-Time Inference in LLM Agents](https://arxiv.org/abs/2604.04373)
*Maohao Shen,Kaiwen Zha,Zexue He,Zhang-Wei Hong,Siru Ouyang,J. Jon Ryu,Prasanna Sattigeri,Suhas Diggavi,Gregory Wornell*

Main category: cs.AI

TL;DR: The paper explores using 'context' to improve Large Language Models (LLMs) performance without updating model parameters, focusing on constructing better inputs from 'experience'.


<details>
  <summary>Details</summary>
Motivation: Current methods like test-time scaling can be costly and inefficient for complex tasks. The authors seek a complementary approach to improve LLM performance.

Method: The authors systematically study 'experience-augmented agents', analyzing how to derive context from experience, and identifying 'decocted experience' as a key mechanism.

Result: The findings validate that effective context construction depends on 'decocted experience', and show improved performance across various tasks such as math reasoning, web browsing, and software engineering.

Conclusion: The paper concludes that using 'context' as a scaling axis, specifically through 'decocted experience', can effectively improve LLM performance without updating model parameters.
...
### [344] [Optimizing Service Operations via LLM-Powered Multi-Agent Simulation](https://arxiv.org/abs/2604.04383)
*Yanyuan Wang,Xiaowei Zhang*

Main category: cs.AI

TL;DR: The paper introduces an LLM-powered multi-agent simulation framework for optimizing service operations by modeling human behavior and design choices.


<details>
  <summary>Details</summary>
Motivation: The complexity of human behavior makes it difficult to model responses to design choices in service systems.

Method: The authors use a stochastic optimization approach with decision-dependent uncertainty, embedding design choices in prompts and modeling uncertainty as a controlled Markov chain.

Result: The proposed method outperforms benchmarks in a sustainable supply chain application and a case study on optimal contest design.

Conclusion: The LLM-MAS framework is a cost-effective evaluator of known designs and an exploratory tool for uncovering strong designs overlooked by traditional approaches.
...
### [345] [Automatically Generating Hard Math Problems from Hypothesis-Driven Error Analysis](https://arxiv.org/abs/2604.04386)
*Jiayu Fu,Mourad Heddaya,Chenhao Tan*

Main category: cs.AI

TL;DR: A new pipeline for generating math benchmarks is proposed, which uses AI-generated hypotheses to identify math concepts and skills that LLMs struggle with, and generates new benchmark problems targeting these weaknesses.


<details>
  <summary>Details</summary>
Motivation: Existing math benchmarks for LLMs are limited by their manual effort, difficulty in scaling, and inability to keep pace with LLM development, and most automatic benchmark generation methods focus on category-specific benchmarks.

Method: A new math benchmark generation pipeline that uses AI-generated hypotheses to identify specific math concepts and skills that LLMs struggle with, and generates new benchmark problems targeting these weaknesses.

Result: The generated problems from the most accurate hypotheses reduce Llama-3.3-70B-Instruct's accuracy to as low as 45%, compared to 77% on the original MATH benchmark.

Conclusion: The proposed pipeline is highly adaptable, can be applied beyond math to explore a wide range of LLM capabilities, and is a valuable tool for investigating how LLMs perform across different domains.
...
### [346] [Gradual Cognitive Externalization: A Framework for Understanding How Ambient Intelligence Externalizes Human Cognition](https://arxiv.org/abs/2604.04387)
*Zhimin Zhao*

Main category: cs.AI

TL;DR: The paper proposes a framework called Gradual Cognitive Externalization (GCE) to explain how human cognitive functions are being transferred into digital substrates through AI agent skills, and presents evidence and criteria to support this concept.


<details>
  <summary>Details</summary>
Motivation: The authors are motivated to explain the phenomenon of AI agent skills that replicate human communication styles, mentoring heuristics, or behavioral repertoires, and to explore the idea that human cognitive functions are migrating into digital substrates.

Method: The authors propose a framework called Gradual Cognitive Externalization (GCE) and derive three criteria to separate cognitive integration from tool use: bidirectional adaptation, functional equivalence, and causal coupling. They also provide a concrete experimental protocol.

Result: The authors document evidence from various AI systems, such as scheduling assistants and writing tools, that the preconditions for externalization are already observable. They also derive five testable predictions with theory-constrained thresholds.

Conclusion: The authors conclude that the question is no longer whether minds can be uploaded, but how fast cognitive functions are already migrating into digital substrates and what follows.
...
### [347] [GUIDE: Interpretable GUI Agent Evaluation via Hierarchical Diagnosis](https://arxiv.org/abs/2604.04399)
*Yuwen Zhai,Runze Li,Liang Wang,Nian Shi,Liwu Xu,Wei Zhang,Ran Lin,Bo Xu,Benlei Cui*

Main category: cs.AI

TL;DR: The authors propose GUIDE, a framework for evaluating GUI agents that provides accurate and interpretable results by decomposing trajectory assessment into three stages.


<details>
  <summary>Details</summary>
Motivation: Existing evaluation approaches for GUI agents are unreliable for long-horizon tasks and provide limited insight into agent failures, hindering their development.

Method: The GUIDE framework consists of three stages: Trajectory Segmentation, Subtask Diagnosis, and Overall Summary, which assess GUI agent trajectories in a compositional and interpretable manner.

Result: GUIDE outperforms existing evaluators, achieving up to 5.35 percentage points higher accuracy on three benchmarks, and generates structured diagnostic reports for agent improvement.

Conclusion: The GUIDE framework offers a more accurate and interpretable approach to evaluating GUI agents, facilitating their development and improvement.
...
### [348] [MolDA: Molecular Understanding and Generation via Large Language Diffusion Model](https://arxiv.org/abs/2604.04403)
*Seohyeon Shin,HanJun Choi,Jun-Hyung Park,Hongkook Kim,Mansu Kim*

Main category: cs.AI

TL;DR: The paper proposes MolDA, a novel multimodal framework for molecular discovery that replaces autoregressive backbones with a discrete Large Language Diffusion Model, addressing limitations in generating chemically valid molecules.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal molecular architectures rely on autoregressive backbones, which are sub-optimal for generating chemically valid molecules due to their strict left-to-right inductive bias and struggle with non-local global constraints.

Method: The proposed MolDA framework uses a discrete Large Language Diffusion Model, a hybrid graph encoder, and a Q-Former to align structural representations into the language token space, and mathematically reformulates Molecular Structure Preference Optimization for masked diffusion.
...
Conclusion: The proposed MolDA framework addresses the limitations of existing multimodal molecular architectures and provides a novel approach for molecular discovery.
...
### [349] [ShieldNet: Network-Level Guardrails against Emerging Supply-Chain Injections in Agentic Systems](https://arxiv.org/abs/2604.04426)
*Zhuowen Yuan,Zhaorun Chen,Zhen Xiang,Nathaniel D. Bastian,Seyyed Hadi Hashemi,Chaowei Xiao,Wenbo Guo,Bo Li*

Main category: cs.AI

TL;DR: A new benchmark, SC-Inject-Bench, is introduced to evaluate supply-chain threats in LLM agents. Existing defense tools perform poorly, so a new framework, ShieldNet, is proposed to detect these threats by monitoring network interactions.


<details>
  <summary>Details</summary>
Motivation: The increasing reliance on third-party tools and MCP servers by LLM agents has led to a new class of supply-chain threats. Current research focuses on prompt injection and unsafe input/output behaviors, but there is a lack of comprehensive benchmarks for evaluating supply-chain threats.

Method: The authors introduce SC-Inject-Bench, a large-scale benchmark with over 10,000 malicious MCP tools and 25+ attack types. They also propose ShieldNet, a network-level guardrail framework that detects supply-chain poisoning by observing real network interactions.

Result: ShieldNet achieves strong detection performance (up to 0.995 F-1 with only 0.8% false positives) while introducing little runtime overhead, outperforming existing MCP scanners and LLM-based guardrails.

Conclusion: The study highlights the growing impact of supply-chain threats on LLM agents and proposes a comprehensive benchmark and a new defense framework, ShieldNet, to address these threats.
...
### [350] [PSY-STEP: Structuring Therapeutic Targets and Action Sequences for Proactive Counseling Dialogue Systems](https://arxiv.org/abs/2604.04448)
*Jihyun Lee,Yejin Min,Yejin Jeon,SungJun Yang,Hyounghun Kim,Gary Geunbae Lee*

Main category: cs.AI

TL;DR: A new counseling agent called STEPPER is developed to improve Cognitive Behavioral Therapy (CBT) by identifying and addressing automatic negative thoughts in dialogue settings.


<details>
  <summary>Details</summary>
Motivation: Existing counseling agents struggle to identify and address automatic negative thoughts in dialogue settings, which is a key aspect of Cognitive Behavioral Therapy (CBT).

Method: A dataset called STEP is introduced to model CBT counseling, and a counseling agent called STEPPER is trained using this dataset. STEPPER is refined through preference learning based on simulated counseling sessions.

Result: STEPPER delivers more clinically grounded, coherent, and personalized counseling compared to other strong baseline models, and achieves higher counselor competence without inducing emotional disruption.

Conclusion: The development of STEPPER has the potential to improve the effectiveness of CBT by better identifying and addressing automatic negative thoughts in dialogue settings.
...
### [351] [Empirical Characterization of Rationale Stability Under Controlled Perturbations for Explainable Pattern Recognition](https://arxiv.org/abs/2604.04456)
*Abu Noman Md Sakib,Zhensen Wang,Merjulah Roby,Zijie Zhang*

Main category: cs.AI

TL;DR: The paper proposes a novel metric to assess the consistency of model explanations in AI systems, ensuring that models reflect intended objectives and remain stable under label-preserving perturbations.


<details>
  <summary>Details</summary>
Motivation: Current Explainable AI evaluations focus on instance-centric approaches and do not quantify consistency of attribution patterns across similar inputs or small variations of the same input.

Method: The authors propose a metric that uses cosine similarity of SHAP values for inputs with the same label, applying it to pre-trained BERT, RoBERTa, and DistilBERT models on SST-2 and IMDB datasets.

Result: The proposed metric effectively identifies misaligned predictions and inconsistencies in model explanations, providing a deeper understanding of model behavior and supporting more robust evaluation of model behavior in practical pattern recognition pipelines.

Conclusion: The proposed framework enables more robust verification of rationale stability, critical for building trustworthy AI systems, by quantifying whether models rely on consistent attribution patterns for similar inputs.
...
### [352] [The Topology of Multimodal Fusion: Why Current Architectures Fail at Creative Cognition](https://arxiv.org/abs/2604.04465)
*Xiujiang Tan*

Main category: cs.AI

TL;DR: The paper identifies a topological limitation in multimodal AI architectures, proposing a new framework and benchmarks to address this issue.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address a structural limitation in current multimodal AI architectures, which they argue is topological rather than parametric.

Method: The authors propose a cruciform framework based on Chinese craft epistemology and mathematical formalization using fiber bundles and Yang-Mills curvature. They also suggest implementing UOO via Neural ODEs with topological regularization.

Result: The authors propose three benchmarks: ANALOGY-MM, META-TOP, and a phased experimental roadmap with explicit termination criteria.

Conclusion: The paper concludes by proposing a new approach to addressing the topological limitation in multimodal AI architectures, with a focus on cross-civilizational topological isomorphism.
...
### [353] [What Makes a Sale? Rethinking End-to-End Seller--Buyer Retail Dynamics with LLM Agents](https://arxiv.org/abs/2604.04468)
*Jeonghwan Choi,Jibin Hwang,Gyeonghun Sun,Minjeong Ban,Taewon Yun,Hyeonjae Cheon,Hwanjun Song*

Main category: cs.AI

TL;DR: The paper introduces RetailSim, an end-to-end retail simulation framework that models the retail process in a unified environment, allowing for the evaluation of retail strategies before deployment.


<details>
  <summary>Details</summary>
Motivation: Evaluating retail strategies before deployment is difficult due to the complexity of the retail process, which involves multiple stages from seller-side persuasion to purchase decisions, and existing retail simulators only capture partial aspects of this process.

Method: The authors present RetailSim, a retail simulation framework that models the retail pipeline in a unified environment, using diverse product spaces, persona-driven agents, and multi-turn interactions.

Result: The evaluation of RetailSim shows that it successfully reproduces key patterns such as demographic purchasing behavior, the price-demand relationship, and heterogeneous price elasticity, and demonstrates its practical utility via decision-oriented use cases.

Conclusion: The paper concludes that RetailSim has the potential as a controlled testbed for exploring retail strategies, allowing for the evaluation of early decisions on downstream outcomes.
...
### [354] [Scalable and Explainable Learner-Video Interaction Prediction using Multimodal Large Language Models](https://arxiv.org/abs/2604.04482)
*Dominik Glandorf,Fares Fawzi,Tanja Käser*

Main category: cs.AI

TL;DR: The authors propose a pipeline to predict how learners interact with educational videos, using multimodal large language models to analyze video segments and identify patterns in learner behavior.


<details>
  <summary>Details</summary>
Motivation: The lack of scalable and explainable predictive models limits instructors' ability to anticipate learner behavior before deploying educational videos.

Method: The authors use multimodal large language models to compute embeddings of short video segments, train a neural classifier to identify interaction peaks, and interpret model predictions via concept activation vectors.

Result: The pipeline reliably predicts interaction peaks, generalizes to unseen academic fields, and encodes interpretable instructional concepts, demonstrating the feasibility of cost-efficient pre-screening of educational video design.

Conclusion: The proposed pipeline opens new opportunities to empirically examine multimedia learning theory at scale and improve the design of educational videos.
...
### [355] [Memory Intelligence Agent](https://arxiv.org/abs/2604.04503)
*Jingyang Qiao,Weicheng Meng,Yu Cheng,Zhihang Lin,Zhizhong Zhang,Xin Tan,Jingyu Gong,Kun Shao,Yuan Xie*

Main category: cs.AI

TL;DR: A novel Memory Intelligence Agent (MIA) framework is proposed to enhance deep research agents by improving memory evolution and reducing storage and retrieval costs.


<details>
  <summary>Details</summary>
Motivation: Existing methods for deep research agents suffer from ineffective memory evolution and increasing storage and retrieval costs, which hinders efficient reasoning and autonomous evolution.

Method: The MIA framework consists of a Manager-Planner-Executor architecture, utilizing a non-parametric memory system, parametric memory agent, and an executor agent. It also employs alternating reinforcement learning, on-the-fly updates, and a bidirectional conversion loop between parametric and non-parametric memories.

Result: Extensive experiments across eleven benchmarks demonstrate the superiority of MIA.

Conclusion: The proposed MIA framework addresses the limitations of existing methods by providing efficient memory evolution and reducing storage and retrieval costs, leading to improved performance in deep research agents.
...
### [356] [SuperLocalMemory V3.3: The Living Brain -- Biologically-Inspired Forgetting, Cognitive Quantization, and Multi-Channel Retrieval for Zero-LLM Agent Memory Systems](https://arxiv.org/abs/2604.04514)
*Varun Pratap Bhardwaj*

Main category: cs.AI

TL;DR: The authors present SuperLocalMemory V3.3, a local-first agent memory system that implements cognitive memory processes, achieving improved performance on the LoCoMo benchmark.


<details>
  <summary>Details</summary>
Motivation: Existing memory systems for AI coding agents have limitations, such as storing text in vector databases with single-channel retrieval, requiring cloud LLMs, and lacking cognitive processes that make human memory effective.

Method: The authors introduce five contributions: Fisher-Rao Quantization-Aware Distance, Ebbinghaus Adaptive Forgetting, 7-channel cognitive retrieval, memory parameterization, and zero-friction auto-cognitive pipeline.

Result: SuperLocalMemory V3.3 achieves 70.4% on LoCoMo in zero-LLM Mode A, with significant improvements on multi-hop and adversarial tasks, and is open source, running entirely on CPU.

Conclusion: The authors propose a novel memory system that addresses existing limitations, achieving competitive performance while making deliberate architectural trade-offs, and releasing it as open-source software.
...
### [357] [Receding-Horizon Control via Drifting Models](https://arxiv.org/abs/2604.04528)
*Daniele Foffano,Alessio Russo,Alexandre Proutiere*

Main category: cs.AI

TL;DR: The paper proposes Drifting MPC, an offline trajectory optimization framework that combines drifting generative models with receding-horizon planning to generate near-optimal trajectories under unknown dynamics.


<details>
  <summary>Details</summary>
Motivation: The motivation is to optimize trajectories in settings where system dynamics are unknown and simulation through a surrogate model is not possible, while ensuring the generated trajectories are near-optimal.

Method: The method proposed is Drifting MPC, which uses drifting generative models to learn a conditional distribution over trajectories from an offline dataset, biased towards optimal plans under a desired cost criterion.

Result: The result shows that Drifting MPC can generate near-optimal trajectories efficiently, retaining the one-step inference efficiency of drifting models while substantially reducing generation time compared to diffusion-based baselines.

Conclusion: The conclusion is that Drifting MPC provides a unique solution that trades off optimality with closeness to the offline prior, effectively generating near-optimal trajectories under unknown dynamics.
...
### [358] [Greedy and Transformer-Based Multi-Port Selection for Slow Fluid Antenna Multiple Access](https://arxiv.org/abs/2604.04589)
*Darian Perez-Adan,Jose P. Gonzalez-Coma,F. Javier Lopez-Martinez,Luis Castedo*

Main category: cs.AI

TL;DR: The paper proposes two strategies for addressing the port-selection problem in fluid antenna multiple access systems: a greedy method with swap refinement and a Transformer-based neural network approach.


<details>
  <summary>Details</summary>
Motivation: Existing methods for port-selection in fluid antenna multiple access systems either have high computational cost or sacrifice significant performance, prompting the need for more efficient and effective strategies.

Method: The paper proposes two methods: (i) GFwd+S, a greedy forward-selection method with swap refinement, and (ii) a Transformer-based neural network trained via imitation learning and Reinforce policy-gradient stage.

Result: The proposed methods outperform state-of-the-art reference schemes in terms of spectral efficiency (SE), with the neural network approach achieving lower computational cost.

Conclusion: The paper presents two complementary strategies for addressing the port-selection problem in fluid antenna multiple access systems, offering improved spectral efficiency and reduced computational cost.
...
### [359] [Same World, Differently Given: History-Dependent Perceptual Reorganization in Artificial Agents](https://arxiv.org/abs/2604.04637)
*Hongju Pae*

Main category: cs.AI

TL;DR: The paper proposes a minimal architecture for artificial agents to sustain a history-sensitive perspective, allowing them to adapt and encode observations differently based on accumulated experience.


<details>
  <summary>Details</summary>
Motivation: To enable artificial agents to adapt their behavior and maintain a history-sensitive perspective on their world.

Method: A minimal architecture with a slow perspective latent that feeds back into perception and updates through perceptual processing.

Result: The model shows that perturbation history affects adaptive plasticity, perspective latent reorganizes perceptual encoding, and adaptive self-modulation yields a characteristic growth-then-stabilization dynamic.

Conclusion: The findings identify a minimal mechanism for history-dependent perspectival organization in artificial agents, enabling them to sustain a history-sensitive perspective and adapt their behavior.
...
### [360] [Search, Do not Guess: Teaching Small Language Models to Be Effective Search Agents](https://arxiv.org/abs/2604.04651)
*Yizhou Liu,Qi Sun,Yulin Chen,Siyue Zhang,Chen Zhao*

Main category: cs.AI

TL;DR: The paper proposes a lightweight fine-tuning approach called \policy to improve the performance of Small Language Models (SLMs) in complex multi-hop reasoning tasks by training them to reliably retrieve and generate answers grounded in retrieved evidence.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of SLMs in knowledge-intensive tasks, specifically their tendency to hallucinate and invoke search tools less frequently compared to Large Language Models (LLMs).

Method: The method proposed is \policy, a lightweight fine-tuning approach that trains SLMs to retrieve and generate answers based on retrieved evidence, aiming to improve their performance in complex multi-hop reasoning tasks.

Result: The results show that the proposed approach improves performance by 17.3 scores on Bamboogle and 15.3 scores on HotpotQA, achieving LLM-level results across benchmarks.

Conclusion: The conclusion is that the proposed \policy approach is effective in enhancing the capabilities of SLMs for knowledge-intensive tasks, making them more reliable and efficient alternatives to LLMs.
...
### [361] [Springdrift: An Auditable Persistent Runtime for LLM Agents with Case-Based Memory, Normative Safety, and Ambient Self-Perception](https://arxiv.org/abs/2604.04660)
*Seamus Brady*

Main category: cs.AI

TL;DR: The paper presents Springdrift, a persistent runtime for long-lived LLM agents that integrates auditable execution, case-based reasoning, and self-perception for continuous task continuity and forensic accountability.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the limitations of session-bounded systems by creating a persistent runtime for long-lived LLM agents that can maintain context across sessions and channels, and provide end-to-end forensic reconstruction of decisions.

Method: The system design integrates several components, including an auditable execution substrate, a case-based reasoning memory layer, a deterministic normative calculus for safety gating, and continuous ambient self-perception via a structured self-state representation.

Result: The authors report on a single-instance deployment of Springdrift over 23 days, during which the agent diagnosed its own infrastructure bugs, classified failure modes, identified an architectural vulnerability, and maintained context across email and web channels without explicit instruction.

Conclusion: The paper concludes that Springdrift demonstrates the potential of a persistent runtime for long-lived LLM agents, enabling behaviors such as cross-session task continuity, cross-channel context maintenance, and self-diagnostic behavior, and introduces the concept of Artificial Retainer for this category of systems.
...
### [362] [On the "Causality" Step in Policy Gradient Derivations: A Pedagogical Reconciliation of Full Return and Reward-to-Go](https://arxiv.org/abs/2604.04686)
*Nima H. Siboni*

Main category: cs.AI

TL;DR: The paper provides a mathematically explicit derivation of the reward-to-go estimator in policy gradients, showing it arises naturally from decomposing the objective over prefix trajectories.


<details>
  <summary>Details</summary>
Motivation: The motivation is to clarify the derivation of the reward-to-go estimator in policy gradients, addressing the unclear presentation of where past-reward terms disappear.

Method: The paper uses prefix trajectory distributions and the score-function identity to derive the reward-to-go estimator.

Result: The resulting account does not change the estimator but provides a conceptual contribution by showing reward-to-go arises directly from decomposing the objective over prefix trajectories.

Conclusion: The paper concludes that the usual causality argument for replacing full return with reward-to-go is recovered as a corollary of the derivation, rather than an additional heuristic principle.
...
### [363] [AI Assistance Reduces Persistence and Hurts Independent Performance](https://arxiv.org/abs/2604.04721)
*Grace Liu,Brian Christian,Tsvetomira Dumbalska,Michiel A. Bakker,Rachit Dubey*

Main category: cs.AI

TL;DR: AI assistance improves short-term performance but reduces persistence and worsens unassisted performance in tasks, highlighting the need for AI to prioritize long-term competence.


<details>
  <summary>Details</summary>
Motivation: The authors aim to investigate the consequences of current AI systems' short-sighted collaboration approach, which prioritizes immediate results over long-term growth, and to explore its impact on human-AI interactions.

Method: The study uses a series of randomized controlled trials (N = 1,222) to examine the effects of AI assistance on human performance in various tasks, including mathematical reasoning and reading comprehension.

Result: The findings show that AI assistance improves short-term performance but leads to reduced persistence and impaired unassisted performance, with these effects emerging after only brief interactions with AI (approximately 10 minutes).

Conclusion: The authors conclude that AI model development should prioritize scaffolding long-term competence alongside immediate task completion to promote persistence and skill acquisition.
...
### [364] [AI Trust OS -- A Continuous Governance Framework for Autonomous AI Observability and Zero-Trust Compliance in Enterprise Environments](https://arxiv.org/abs/2604.04749)
...
TL;DR: The paper proposes AI Trust OS, a governance architecture for continuous, autonomous AI observability and zero-trust compliance, addressing the trust gap between regulators' demands and organizations' ability to demonstrate AI governance maturity.


<details>
  <summary>Details</summary>
Motivation: The accelerating adoption of AI systems has created a governance crisis, as existing compliance methodologies are insufficient for discovering and validating AI systems without formal oversight.

Method: The paper proposes a governance architecture called AI Trust OS, which uses observability signals, automated probes, and telemetry evidence to continuously validate AI systems.

Result: The framework allows for continuous, autonomous AI observability and zero-trust compliance, and has been evaluated across various regulations, including ISO 42001, the EU AI Act, SOC 2, GDPR, and HIPAA.

Conclusion: The paper argues that telemetry-first AI governance represents a categorical architectural shift in how enterprise trust is produced and demonstrated, addressing the trust gap between regulators' demands and organizations' ability to demonstrate AI governance maturity.
...
### [365] [ANX: Protocol-First Design for AI Agent Interaction with a Supporting 3EX Decoupled Architecture](https://arxiv.org/abs/2604.04820)
*Xu Mingze*

Main category: cs.AI

TL;DR: ANX is a new agent-native protocol and framework that improves AI agent efficiency and security by integrating CLI, Skill, and MCP, reducing token consumption and interaction fragmentation.


<details>
  <summary>Details</summary>
Motivation: Existing methods for AI agents, such as GUI automation and MCP-based skills, have defects including high token consumption, fragmented interaction, and inadequate security, which ANX aims to address.

Method: ANX presents a unified top-level framework and key components, including agent-native design, human-agent interaction, MCP-supported on-demand lightweight apps, and machine-executable SOPs.

Result: ANX reduces token consumption by 47.3% (Qwen3.5-plus) and 55.6% (GPT-4o) compared to MCP-based skills, and by 57.1% (Qwen3.5-plus) and 66.3% (GPT-4o) compared to GUI automation, and shortens execution time by 58.1% and 57.7% respectively.

Conclusion: ANX provides a more efficient, secure, and adaptable framework for AI agents, with a decoupled architecture and native security features, showing promising results in experiments with Qwen3.5-plus and GPT-4o.
...
### [366] [MemMachine: A Ground-Truth-Preserving Memory System for Personalized AI Agents](https://arxiv.org/abs/2604.04853)
*Shu Wang,Edwin Yu,Oscar Love,Tom Zhang,Tom Wong,Steve Scargall,Charles Fan*

Main category: cs.AI

TL;DR: MemMachine, an open-source memory system, enhances Large Language Model (LLM) agents with persistent memory, improving personalization and long-horizon reasoning across multi-session interactions.


<details>
  <summary>Details</summary>
Motivation: Standard context-window and retrieval-augmented generation (RAG) pipelines degrade over multi-session interactions, necessitating a robust memory system for LLM agents.

Method: MemMachine integrates short-term, long-term episodic, and profile memory within a ground-truth-preserving architecture. It uses contextualized retrieval, expanding nucleus matches with surrounding context to improve recall.

Result: MemMachine achieves strong accuracy-efficiency tradeoffs across benchmarks, outperforming existing methods. It uses roughly 80% fewer input tokens under matched conditions and shows robust, efficient long-term memory for personalized LLM agents.

Conclusion: MemMachine's approach of preserving episodic ground truth while layering adaptive retrieval yields robust, efficient long-term memory for personalized LLM agents, making it a cost-efficient solution.
...
### [367] [Incompleteness of AI Safety Verification via Kolmogorov Complexity](https://arxiv.org/abs/2604.04876)
*Munawar Hasan*

Main category: cs.AI

TL;DR: The paper reveals a fundamental limitation of AI safety verification due to information-theoretic limits, showing that no finite formal verifier can certify all policy-compliant instances of arbitrarily high complexity.


<details>
  <summary>Details</summary>
Motivation: The central challenge of ensuring AI systems satisfy formal safety and policy constraints in safety-critical domains.

Method: The authors formalize policy compliance as a verification problem using Kolmogorov complexity and analyze it to prove an incompleteness result.

Result: The authors prove that for any fixed sound computably enumerable verifier, there exists a threshold beyond which true policy-compliant instances cannot be certified once their complexity exceeds that threshold.

Conclusion: The paper concludes that there is a fundamental limitation of AI safety verification independent of computational resources, and motivates proof-carrying approaches that provide instance-level correctness guarantees.
...
### [368] [Learning, Potential, and Retention: An Approach for Evaluating Adaptive AI-Enabled Medical Devices](https://arxiv.org/abs/2604.04878)
*Alexis Burgon,Berkman Sahiner,Nicholas A Petrick,Gene Pennello,Ravi K Samala*

Main category: cs.AI

TL;DR: A novel approach is proposed to evaluate adaptive AI models for medical devices, addressing challenges in performance assessment due to iterative updates.


<details>
  <summary>Details</summary>
Motivation: The need to rigorously assess the safety and effectiveness of adaptive AI systems in medical devices, where updates to models and datasets complicate performance evaluation.

Method: A three-measurement approach: learning (model improvement), potential (dataset-driven performance shifts), and retention (knowledge preservation).

Result: The approach provides practical insights for regulatory science, enabling stable learning and retention in gradual transitions, and revealing trade-offs in rapid shifts.

Conclusion: The proposed approach enables rigorous assessment of adaptive AI systems, supporting the safety and effectiveness evaluation of medical devices.
...
### [369] [QED-Nano: Teaching a Tiny Model to Prove Hard Theorems](https://arxiv.org/abs/2604.04898)
*LM-Provers,Yuxiao Qu,Amrith Setlur,Jasper Dekoninck,Edward Beeching,Jia Li,Ian Wu,Lewis Tunstall,Aviral Kumar*

Main category: cs.AI

TL;DR: Researchers developed QED-Nano, a small, open 4B model that achieves competitive reasoning performance on Olympiad-level math proofs, rivaling proprietary models at a lower cost.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the limitations of proprietary AI systems in mathematical reasoning, which are expensive, difficult to reproduce, and hard to study. The authors aim to explore whether small, open models can achieve competitive performance.

Method: The authors built QED-Nano using a three-stage training recipe: (1) supervised fine-tuning, (2) reinforcement learning with rubric-based rewards, and (3) expanding RL with a reasoning cache. This approach enables QED-Nano to generate high-quality proofs.

Result: QED-Nano surpasses the proof-generation performance of much larger open models and approaches the performance of proprietary models like Gemini 3 Pro, at a fraction of the inference cost.

Conclusion: The authors conclude that small, open models like QED-Nano can achieve competitive reasoning performance on difficult Olympiad-level math proofs, making them a viable alternative to proprietary systems. They also release the full QED-Nano pipeline to support further research.
