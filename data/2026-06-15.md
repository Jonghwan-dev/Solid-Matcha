...
TL;DR: A portable, low-power fall detection system using edge computing with high performance and efficiency (HPE) on an AMD Kria K26 System-on-Module (SOM) has been proposed, achieving 74%, 84.13%, and 75.85% accuracy for different models.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to develop a non-intrusive, privacy-preserving system for real-time fall detection in elderly people to prevent harm and support well-being.

Method: The system uses a three-stage pipeline with quantized YOLOX, Anchor-to-Joint (A2J), and fall-detection models, running on a single-core or dual-core DPU with a serial or multi-threaded pipeline.

Result: The system achieved accuracies of 74%, 84.13%, and 75.85% for YOLOX, A2J, and the CNN, respectively, with a throughput improvement from 2.5 FPS to 4.5 FPS.

Conclusion: The proposed system demonstrates the feasibility of privacy-preserving fall detection on an AMD Kria K26 edge device, supporting elderly monitoring and assistive healthcare.
...
<details>
  <summary>Details</summary>
Motivation: The task of transferring hairstyles between images is challenging, especially under large viewpoint and scale differences. Existing methods often fail to synthesize missing hair content accurately.

Method: HairPort, a 3D-aware hairstyle transfer framework, which includes a Bald Converter for generating bald face images, a 3D-Aware Transfer Pipeline for reconstructing and re-rendering reference hairstyles, and a conditional flow-matching generator for synthesizing transferred results.
...
Conclusion: HairPort effectively addresses the challenges of hairstyle transfer under large pose and scale differences, with applications in virtual try-on systems, augmented reality, and entertainment.
...
TL;DR: A 2-step image generation model, Z-Image Turbo++, was developed through distillation from an 8-step model, achieving high-quality results and narrowing the gap with 8-step generation.


<details>
  <summary>Details</summary>
Motivation: The challenge of pushing few-step diffusion distillation to 2 steps while maintaining quality, as 2-step generation has bottlenecks of increased task difficulty and limited model capacity.

Method: Three design choices were proposed: Distribution-Aligned Adversarial Learning, Step-Decoupled Parameterization, and End-to-End Training with Iterative Regularization.

Result: The model substantially narrowed the quality gap between 2-step and 8-step generation in both qualitative and quantitative evaluations.

Conclusion: Carefully tailored distillation strategies can improve the quality-efficiency trade-off in few-step generation, highlighting the potential of the proposed approach.
...
TL;DR: The paper proposes a new method to improve large vision-language models in medical imaging tasks by addressing issues such as factual inconsistencies and poor visual grounding.


<details>
  <summary>Details</summary>
Motivation: Existing post-training alignment approaches have limitations in the medical domain, including treating clinically critical tokens identically to generic text, relying on static supervised fine-tuning references, and lacking explicit visual grounding constraints.

Method: The proposed method uses a bidirectional token-wise KL regularizer and a visual-contrastive grounding objective to form a fine-grained, on-policy alignment framework.
...
TL;DR: The paper proposes Dual-State Slot Attention (DSSA), a self-supervised framework for unsupervised video object-centric learning that improves object identity stability and temporal consistency.


<details>
  <summary>Details</summary>
Motivation: Existing slot-based methods struggle with maintaining stable object identity in challenging settings such as rapid motion and partial occlusion due to encoding both appearance and identity in a single slot vector and using token renormalization.

Method: DSSA separates appearance from identity by decomposing each slot into a local state and an identity state, and uses a learned recurrent transition and competition-modulated aggregation (CMA) to update the identity state and prevent spurious updates.

Result: DSSA consistently improves segmentation quality and temporal consistency over prior methods on MOVi-C, MOVi-D, and YouTube-VIS, and yields stronger downstream object recognition and video dynamics prediction.

Conclusion: The proposed DSSA framework effectively addresses the limitations of existing slot-based methods and demonstrates improved performance in unsupervised video object-centric learning.
...
TL;DR: The paper proposes a novel object detection framework called Context-Centric Feature Fusion (CCFF) that leverages attention-based modules to improve detection in complex environments, especially for small and rare objects.


<details>
  <summary>Details</summary>
Motivation: Object detection in autonomous driving is challenging in complex heterogeneous environments, particularly for rare classes, small-scale objects, and frequently appearing objects.

Method: The proposed CCFF framework consists of two attention-based modules: Local Context Fusion Module (LCFM) and Global Context Attention Module (GCAM), which fuse local and global features to enhance classification and object detection.

Result: The method achieves significant improvement on relational consistency (CCS: 0.973 and 0.969 on Cityscapes and BDD100K datasets) and substantial gains in small object detection (AP_S: 14.1%).

Conclusion: The proposed CCFF framework effectively improves object detection in complex environments, especially for small and rare objects, while maintaining real-time processing efficiency.
...
TL;DR: The paper proposes Efficient Continual Alignment (ECA), a novel approach for incremental learning in open-ended image-to-text generation, which adapts pre-trained visual language models to new images while preserving previous knowledge.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the practical scenario where the predominant category of visual data shifts over time, and to enable models to continuously generate accurate text for new images while preserving previously acquired knowledge.

Method: The method proposed is Efficient Continual Alignment (ECA), which employs three core mechanisms: a Mixture of Query (MoQ) module, a Fisher Dynamic Expansion (FeDEx) module, and an embedding dictionary with Dictionary Replay (DR).

Result: The experimental results demonstrate that ECA significantly mitigates catastrophic forgetting and improves incremental learning performance compared to baseline methods.

Conclusion: The conclusion is that ECA is an effective approach for incremental learning in open-ended image-to-text generation, which can adapt to new images while preserving previous knowledge.
...
TL;DR: The paper introduces CD-RCM, a novel-view synthesis approach for reflectance confocal microscopy (RCM) to provide continuous-depth visualization and make 3D volumes isotropic.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome the limitations of RCM's anisotropic 3D volumes, which have limited axial resolution, making it difficult to interpret tissue structures.

Method: The method is a feedforward model that predicts realistic, unseen depths from sparsely sampled RCM stacks, using a tailored architectural and training framework that accounts for RCM's depth-resolved, occlusive imaging physics.

Result: The result is that CD-RCM achieves high-fidelity novel-view synthesis with sub-second inference time, allowing for arbitrary-direction sectioning and histopathology-like cross-sectional examination.

Conclusion: The conclusion is that CD-RCM provides a solution for continuous-depth visualization in RCM, enabling improved interpretation of tissue structures and potentially enhancing diagnostic capabilities.
...
TL;DR: The paper introduces SalArt-VQA, a benchmark to evaluate the ability of vision-language models (VLMs) to detect and understand artifacts in AI-generated images. It reveals that high detection accuracy does not imply a deep understanding of artifacts.


<details>
  <summary>Details</summary>
Motivation: The motivation is to assess the capability of VLMs to analyze artifacts in AI-generated images beyond just detection accuracy, highlighting potential hidden failures.

Method: The method involves introducing SalArt-VQA, a diagnostic benchmark with 950 images and 3,681 multiple-choice questions to evaluate VLMs across various dimensions including presence detection, localization, and defect identification.

Result: The results show that even top-performing VLMs have significant shortcomings in understanding artifacts, with the strongest model correctly answering artifact-related questions only 53.26% of the time despite achieving 99.37% detection recall.

Conclusion: The conclusion is that high artifact detection accuracy in VLMs does not guarantee a genuine understanding of artifacts, and SalArt-VQA can expose hidden failure modes in these models.
...
TL;DR: The paper introduces VLADriveBench, a framework to evaluate the reasoning process of vision-language-action models in driving scenarios, beyond just trajectory quality.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks do not assess the relevance, consistency, or causal connection of the chain-of-thought (CoT) reasoning to the driving action.

Method: The framework combines observational metrics with a CoT intervention protocol to analyze the CoT-action relationship.

Result: Applying the framework to three models across two architectures reveals that observational alignment and causal connection can diverge, with different models showing different strengths.

Conclusion: The study highlights the importance of evaluating both the observational alignment and causal connection of CoT reasoning in vision-language-action models for driving scenarios.
...
TL;DR: The paper proposes a new method called Guided Retrieval of In-context Prompts (GRIP) for improving multimodal in-context learning (M-ICL) by selecting more effective in-context examples.


<details>
  <summary>Details</summary>
Motivation: Current methods for M-ICL rely on feature-space similarity to select in-context examples, but this assumption does not always lead to the most effective context for learning.

Method: The authors propose GRIP, a learnable vision-only retrieval framework that uses feedback from Large Multimodal Models (LMMs) to identify beneficial in-context examples through contrastive training.

Result: GRIP improves over similarity-based retrieval across three multimodal tasks (classification, captioning, and visual question answering) on various LMMs, including Qwen2.5-VL-7B and Idefics2-8B, and can be transferred to other models without retraining.

Conclusion: The proposed GRIP framework offers a more effective approach to selecting in-context examples for M-ICL, leading to improved performance across multiple tasks and models, with potential for scalable and cost-efficient deployment.
...
TL;DR: A new method for moving instance segmentation (MIS) fuses event and image features to improve performance, especially for small instances in challenging conditions.


<details>
  <summary>Details</summary>
Motivation: Current multimodal MIS methods struggle with segmenting small moving instances due to sparse event features and entangled appearance and motion cues.

Method: A dual-disentangling feature extraction framework and a multi-granularity cross-modal alignment are proposed to improve feature density and enable effective fusion.
...
Conclusion: The proposed method effectively addresses the challenges of MIS by leveraging event and image features, leading to improved performance in various conditions.
...
TL;DR: The paper introduces PERIA, a visual agent that enhances spatial reasoning capabilities through tool-augmented interaction with visual data, outperforming existing models.


<details>
  <summary>Details</summary>
Motivation: The limitation of recent vision-language models in spatial reasoning tasks due to their reliance on implicit visual representations.

Method: PERIA uses vision perception and interaction tools, trained with a unified recipe including supervised trajectory synthesis and a specific policy optimization algorithm.

Result: PERIA-8B shows improvements over its backbone and state-of-the-art baselines, achieving performance comparable to much larger models.

Conclusion: PERIA effectively enhances spatial reasoning capabilities, demonstrating the potential of tool-augmented visual agents.
...
TL;DR: The paper proposes L-VARC, a novel framework that enhances visual reasoning by leveraging language guidance, achieving state-of-the-art results with a lightweight model.


<details>
  <summary>Details</summary>
Motivation: The Abstraction and Reasoning Corpus (ARC) is crucial for Artificial General Intelligence (AGI), but existing methods rely heavily on language models or vision-only approaches, which have limitations. The authors aim to bridge the gap between language and vision-based approaches.

Method: The authors propose L-VARC, a framework that uses a language-guided Learning Using Privileged Information (LUPI) branch, a Semantic Compression Module, and a Cross-Attention Projector to align visual features with semantic embeddings.

Result: The proposed L-VARC framework outperforms state-of-the-art methods, and ablation studies confirm the contribution of the two new designs. The model has only 18 million parameters, making it lightweight.

Conclusion: The L-VARC framework effectively leverages linguistic priors to boost visual reasoning, achieving state-of-the-art results with a lightweight model, and is a promising approach for Artificial General Intelligence (AGI).
...
TL;DR: A new framework, DECNN, is proposed for task-adaptive sampling in image and surface-based learning tasks, which dynamically redistributes computational attention to informative regions, leading to more efficient feature extraction and improved performance.
...
Method: The method proposed is the Density-Equalizing Convolutional Neural Network (DECNN), which uses density-equalizing mappings to guide convolution through a learned density function, enabling adaptive feature extraction.

Result: The results show that DECNN achieves competitive or superior performance with fewer parameters, accurately identifies task-relevant regions, and remains robust under complex geometric variations in image classification and craniofacial surface analysis.

Conclusion: The conclusion is that DECNN offers a more efficient and effective approach to feature extraction by focusing computational resources on informative structures, leading to improved performance and interpretability.
...
TL;DR: The paper identifies a failure mode called Modal Isolation in interleaved multimodal models, where generated images and text ignore each other's evidence. The authors propose a two-stage training framework called MoTiF to improve cross-modal coherence and task accuracy.


<details>
  <summary>Details</summary>
Motivation: The authors are motivated to address the Modal Isolation failure mode in interleaved multimodal models, which causes the model to alternate between textual reasoning and visual generation without genuinely informing each other.
...
Result: The authors achieve substantial improvements in both cross-modal coherence and final task accuracy across four visual puzzle benchmarks. The results demonstrate that effective interleaved reasoning requires explicit structural supervision at modality boundaries.

Conclusion: The paper concludes that explicit structural supervision at modality boundaries is necessary for effective interleaved reasoning, and that MoTiF is a effective approach to improve cross-modal coherence and task accuracy.
...
TL;DR: The paper proposes AGAR, a method to improve Visual Text Comprehension (VTC) by adaptively rendering text into images for vision-language models (VLMs) to read, leading to improved performance across various benchmarks.
...
Method: The authors propose AGAR (Attention-Guided Adaptive Rendering), a training-free, model-agnostic method that leverages a VLM's attention to identify important visual patches, and re-renders the page with those patches enlarged.
...
Conclusion: The paper concludes that AGAR is an effective method for improving VTC performance by adaptively rendering text into images, and demonstrates its effectiveness through extensive experiments.
...
TL;DR: The paper introduces Bayesian Conditional Priors (BCP) Estimation, a test-time adaptation method that improves multi-label recognition with frozen Vision-Language Models (VLMs) under distribution shift.


<details>
  <summary>Details</summary>
Motivation: Multi-label recognition with frozen VLMs is brittle under distribution shift, as it scores labels independently and ignores co-occurrence structure, leading to incoherent label sets.

Method: BCP Estimation, a gradient-free test-time adaptation method that injects label dependency without tuning the backbone, by selecting a high-confidence anchor label and applying an anchor-conditioned Bayesian refinement.
...
Conclusion: BCP Estimation effectively improves multi-label recognition with frozen VLMs under distribution shift, by incorporating label dependency and promoting compatible labels.
...
TL;DR: The paper proposes Masked Multi-View Test-Time Adaptation (MAMVI) for 3D point cloud models, which improves test-time adaptation performance under distribution shifts by leveraging multi-view augmentation in a unified single-step adaptation.


<details>
  <summary>Details</summary>
Motivation: 3D point cloud models suffer performance degradation under distribution shifts caused by sensor noise, occlusions, and environmental changes. Existing multi-view approaches are constrained by sequential optimization, leading to substantial inference latency.

Method: MAMVI utilizes a hybrid masking strategy, aggregates losses across multiple views, and uses a confidence-based adaptive learning rate to dynamically adjust the adaptation intensity for each sample.

Result: MAMVI achieves state-of-the-art accuracy on ShapeNet-C and ScanObjectNN-C, and remains competitive on ModelNet-40C while delivering 4.9-8.9 times faster inference.

Conclusion: The proposed MAMVI method enables efficient and effective test-time adaptation for 3D point cloud models, making it highly suitable for real-time applications.
...
TL;DR: Proposed YOLO-AMC, a YOLO-based model with attention mechanisms for improved crack detection in infrastructure inspection, outperforming baseline models in accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: Crack detection is crucial in infrastructure inspection and SHM, but existing object detection models struggle with thin, low-contrast crack structures and background noise.

Method: Improved YOLO-based architecture (YOLO-AMC) with integrated attention mechanisms (GAM, Res-CBAM, SA) for enhanced crack detection performance.

Result: YOLO-AMC outperforms baseline models (YOLOv11n, YOLOv8n) across multiple evaluation metrics, achieving mAP@0.5 = 0.9917 and mAP@0.5:0.95 = 0.9506 on the test dataset.

Conclusion: The proposed YOLO-AMC model demonstrates a favorable trade-off between accuracy and deployment efficiency, suitable for infrastructure inspection and SHM applications.
...
TL;DR: A new method for model fingerprinting in generative text-to-image models that is robust against collusion attacks, where multiple attackers try to remove or obscure the fingerprints.


<details>
  <summary>Details</summary>
Motivation: Existing model fingerprinting methods lack robustness against collusion attacks, which can compromise the intellectual property rights of generative models.
...
Result: The method achieves high fidelity and robustness across multiple image generation and editing tasks, with fingerprint extraction accuracy exceeding 99.5%, and significantly increases the FID of colluded models.

Conclusion: The proposed method provides a robust fingerprinting solution for generative text-to-image models, effectively preventing unauthorized model redistribution and demonstrating proactive robustness to collusion attacks.
...
TL;DR: Developed a Camera and LiDAR fusion detector for 3D object detection, achieving 0.85 mAP on test split.


<details>
  <summary>Details</summary>
Motivation: Improve 3D object detection using Camera and LiDAR fusion for TUMTraf V2X cooperative track.

Method: Fuses roadside cameras with infrastructure-plus-vehicle point cloud, using CenterPoint-style head with IoU regression loss.

Result: Achieved 0.85 mAP on public test split, with further studies reaching up to 0.99 mAP.

Conclusion: Proposed Camera and LiDAR fusion detector shows promising results for 3D object detection.
...
Method: BabyMind uses an offline mask-based region interface, tracking, and a prototype-space multiple-instance contrastive objective to align utterances to bags of object files.
...
TL;DR: The authors propose a compact latent world model for predicting future camera scenes in autonomous vehicles, which uses a diffusion transformer to generate more realistic and action-controllable predictions.


<details>
  <summary>Details</summary>
Motivation: The authors aim to improve the prediction of future camera scenes in autonomous vehicles, which is crucial for planning and simulation without real-world rollouts. They address the limitations of current methods, which produce ambiguous and unrealistic predictions.

Method: The authors propose a compact latent world model that predicts future scene latents given the present front-camera latent and a sequence of ego-actions. They use a diffusion transformer (DiT) and identify four key ingredients for its success: spatial tokens, the $x_0$ objective, residual anchoring, and sampling matched to target uncertainty.

Result: The authors demonstrate significant improvements over regression-based methods, including a 4.8x better KID (Kernel Inception Distance) score and a deployable train-derived calibration that makes the model practical without test-time ground truth. The model is also genuinely action-controllable, with a Spearman correlation of 0.81 between steering and scene displacement.

Conclusion: The authors conclude that their proposed compact latent world model, which uses a diffusion transformer, can generate more realistic and action-controllable predictions of future camera scenes in autonomous vehicles, addressing the limitations of current methods.
...
<details>
  <summary>Details</summary>
Motivation: The increasing need for safety and health monitoring in workplace environments, specifically addressing limitations of traditional camera-based systems in analyzing human posture.

Method: A system using 3D point clouds and deep learning classifiers to analyze human poses in real-time, trained on manually labeled data from RGB-D cameras.

Result: A scalable and pragmatic approach for real-time ergonomic evaluation, combining 3D data technologies and traditional 2D pose estimation algorithms.

Conclusion: The research offers a notable contribution to the domain of workplace safety and health monitoring, providing a solution for real-time ergonomic evaluation.
...
TL;DR: A new method for generating adversarial attacks on skeletal human action recognition that preserves motion quality and achieves high success rates.


<details>
  <summary>Details</summary>
Motivation: Existing adversarial attack methods for skeletal human action recognition introduce noise-like perturbations that degrade motion quality, making them perceptible. The authors aim to bridge the gap between empirical and true risks during optimization to preserve motion quality.

Method: A distribution-based adversarial attack method that generates adversarial motions without introducing noise-like perturbations, and a new metric to evaluate motion quality based on human perception of real-world naturalness.

Result: The proposed method demonstrates superiority in attack success rate and post-attack motion quality across two datasets and state-of-the-art S-HAR methods.
...
TL;DR: A new network, GeoCFNet, is proposed for accurate and stable confidence field estimation to guide robot-assisted endoscopic submucosal dissection (ESD).
...
Result: GeoCFNet achieves RMSE 0.0480, PSNR 27.1995, SSIM 0.3397, and CC 0.2466, indicating accurate and geometrically stable confidence field estimation for robot-assisted ESD guidance.

Conclusion: The proposed GeoCFNet provides an effective solution for reliable confidence field estimation in dynamic endoscopic scenes, which can improve the accuracy and safety of robot-assisted ESD.
...
TL;DR: The paper proposes a tracking algorithm called selective mask propagation, which combines a lightweight base tracker with a more expensive video object segmentation (VOS) model to improve multi-object tracking performance.


<details>
  <summary>Details</summary>
Motivation: Multi-object tracking has a heavy-tailed difficulty distribution, where most frames are easy but a small fraction are hard, and existing trackers often fail on these hard frames.

Method: The algorithm dispatches from a base tracker to a VOS model only when an assignment-uncertainty signal is detected, and modifies the base tracker's output only when the VOS model makes a confident prediction that contradicts the base tracker's identity assignment.

Result: The method achieves state-of-the-art performance on SportsMOT with 86.8 HOTA and improves three different base trackers on DanceTrack.

Conclusion: The proposed selective mask propagation algorithm can effectively improve multi-object tracking performance by selectively leveraging a more expensive VOS model to handle hard frames.
...
TL;DR: SeamEdit is a training-free pipeline for semantic region editing in large images using black-box VLMs.


<details>
  <summary>Details</summary>
Motivation: Existing methods for large image editing have limitations, relying on white-box models or struggling with integration and generation quality.

Method: SeamEdit uses a five-stage pipeline: tile decomposition, VLM inpainting, correction, ranking, and curved seam fusion.

Result: SeamEdit reduces seam visibility and enables semantic modification of arbitrary tile regions.

Conclusion: SeamEdit offers a model-agnostic solution for high-quality, natural image editing.
...
TL;DR: LaME, a Latent Reasoning Multimodal Embedding model, is proposed to address limitations of Chain-of-Thought (CoT) reasoning in multimodal embedding, achieving competitive performance with faster inference.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome the limitations of autoregressive CoT reasoning, which incurs high computational cost and relies heavily on CoT annotation quality, making large-scale training unreliable.

Method: LaME formulates embedding-oriented latent reasoning as a weakly supervised information bottleneck, employing K learnable reason tokens as a fixed-capacity bottleneck, and uses a two-stage training pipeline.

Result: LaME achieves competitive performance on MMEB-v2 and MRMR, surpassing some explicit CoT-based models, with 60x faster inference than explicit CoT methods and 2x faster than latent baselines.

Conclusion: LaME offers a promising solution for multimodal embedding, addressing the limitations of CoT reasoning and providing faster inference while maintaining competitive performance.
...
Method: HTSCGAN uses a generator with three Patch Contrast Modules (PCM) to capture hierarchical structural information of tumor regions, and employs a pretrained Patch Classifier (PC) and Structure-Aware Encoder (SAE) to derive the generated image with the same tumor region structure as the ground truth image.

Result: Experiments on BraTS2020 and BraTS2021 demonstrate strong performance of HTSCGAN in both translation tasks and downstream segmentation tasks, highlighting its effectiveness in enhancing the quality and clinical relevance of translated brain images.
...
TL;DR: The paper presents PP-OCRv6, a lightweight OCR system that achieves high accuracy and efficiency through architectural innovation and data-centric optimization, outperforming previous models with significantly fewer parameters.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the limitations of Vision-Language Models (VLMs) in dedicated OCR scenarios, specifically hallucination, imprecise localization, and high computational cost.

Method: The method used in this paper involves redesigning the backbone, detection neck, and recognition neck of the OCR system around a unified MetaFormer-style building block with structural reparameterization, allowing for task-specific stride configurations and decoupling spatial token mixing from channel mixing.

Result: The results show that PP-OCRv6_medium achieves 83.2% recognition accuracy and 86.2% detection Hmean, outperforming previous models (PP-OCRv5_server, Qwen3-VL-235B, GPT-5.5, and Gemini-3.1-Pro) with orders of magnitude fewer parameters, while the tiny tier achieves 3.9$	imes$ faster inference than PP-OCRv5_mobile on Intel Xeon CPU with comparable accuracy.

Conclusion: The conclusion of this paper is that PP-OCRv6 is a highly efficient and accurate OCR system that achieves state-of-the-art results in dedicated OCR scenarios, making it suitable for deployment on a range of devices from servers to edge devices.
...
<details>
  <summary>Details</summary>
Motivation: The need for a scalable solution for multi-camera tracking with overlapping fields of view, as centralized fusion creates computational bottlenecks.

Method: MV3DT uses a fully distributed framework with peer-to-peer coordination, where each camera node executes a lightweight modular pipeline comprising monocular 3D perception, distributed multi-view association, and collaborative fusion.

Result: MV3DT achieves 94.3% IDF1 and 93.3% MOTA on WILDTRACK, and sustains 30 FPS on 100 cameras with less than 10 ms inter-camera latency and only 2.2% communication overhead.

Conclusion: MV3DT is a practical solution for real-time multi-view tracking in large-scale overlapping camera networks, offering accurate tracking, scalability, and direct deployability in new environments.
...
TL;DR: The study compares deep learning architectures for dermoscopic image classification, evaluating their performance on open international datasets and independent clinical datasets from Russian practice.


<details>
  <summary>Details</summary>
Motivation: To assess the generalization of deep learning models for skin neoplasm classification from open datasets to clinical practice in Russia.

Method: Four deep learning architectures (ViT-B/16, Swin-S, ConvNeXt-S, EfficientNetV2-S) were compared in three classification schemes: binary, single-stage four-class, and two-stage cascade. Models were pretrained on ImageNet and fine-tuned on ISIC Archive data.

Result: The models showed a generalization gap when applied to clinical datasets, with decreased performance (ROC-AUC 0.797-0.893) and sensitivity (0.53-0.67). The two-stage cascade approach improved macro F1 over single-stage classification for most architectures.

Conclusion: The study highlights the need for external clinical validation and recalibration of deep learning models before deployment. A tunable triage threshold can provide sensitivity control and better reproduce clinical differential-diagnosis logic.
...
TL;DR: Proposed a unified, modular, and lightweight architecture for demosaicing various pixel-bin image sensors, enabling plug-and-play operation with a learning-free CFA identification module.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning-based demosaicing methods are specific to Bayer color filter arrays (CFAs) and require multiple individual models, which is resource-intensive and cumbersome. Pixel-bin image sensors have a larger inter-color separation than Bayer CFAs, making demosaicing more challenging.

Method: A modular unified architecture for demosaicing various pixel-bin sensors, along with a learning-free CFA-identification module for accurate detection of the CFA type of raw data.

Result: The proposed architecture provides higher image quality, is extensible, and is lightweight, enabling efficient demosaicing of various pixel-bin image sensors.

Conclusion: The proposed method offers a unified solution for demosaicing various pixel-bin image sensors, overcoming the limitations of existing methods and providing a more efficient and effective approach.
...
<details>
  <summary>Details</summary>
Motivation: VLMs achieve strong single-shot spatial grounding but lack a mechanism to observe and correct their own predictions, leading to a fundamental gap between grounding capability and self-correction ability.

Method: The authors propose a closed-loop framework, IVT, which predicts a bounding box, observes the prediction rendered on the image, and iteratively refines through visual feedback. A two-phase training recipe is used, including supervised data generation and Group Relative Policy Optimization (GRPO) with a simple IoU reward.

Result: IVT surpasses the single-shot base model on every metric, with Acc@0.5 rising to 82.0% (+2.4pp), Acc@0.7 to 74.1% (+3.2pp), and Acc@0.9 to 48.3% (+2.8pp). GRPO further reduces per-step IoU degradation by 5x, stabilizing the refinement trajectory.
...
TL;DR: A new method for generating patient-specific cardiac models from 3D medical images, bypassing traditional segmentation and mesh generation steps.


<details>
  <summary>Details</summary>
Motivation: Current cardiac model generation is slow, inconsistent, and requires specialist knowledge, hindering clinical use.

Method: An end-to-end network using a 3D Swin Transformer encoder-decoder and a Graph Attention Network (GAT) head to generate a smooth, simulation-ready cardiac surface mesh directly from raw 3D medical images.

Result: Competitive segmentation scores (Dice of 0.84 on CT, 0.83 on MRI) and high-quality meshes (mean Chamfer distance of 1.8 mm, 95th-percentile surface distance below 5 mm).

Conclusion: The proposed approach makes patient-specific cardiac simulation more accessible for clinical use by removing the post-processing bottleneck.
...
<details>
  <summary>Details</summary>
Motivation: Visual place recognition in natural forest environments is challenging due to repetitive vegetation, weak structural cues, and significant appearance variation across traversals.
...
Result: The final YOLOv8n model achieves a high accuracy (mAP@0.5 = 0.995, mAP@0.5:0.95 = 0.924) and runs in real-time on a mid-range phone, while the RAG guide reduces end-to-end latency to approximately 10 s.

Conclusion: The TimeLens system provides an effective and efficient AI-powered guide for museum visitors, addressing specific challenges in in-gallery deployment and demonstrating significant engineering contributions.
...
TL;DR: Custom ZeroCLIP, a retrieval-augmented vision-language framework, achieves state-of-the-art results in zero-shot captioning of Indonesian traditional garments, outperforming existing baselines.


<details>
  <summary>Details</summary>
Motivation: The motivation is to develop a framework for zero-shot captioning of Indonesian traditional garments, particularly in low-resource heritage settings where data is limited.

Method: The framework combines a frozen CLIP ViT-B/32 image encoder, a CLIP text encoder, a BERT text encoder, and an LSTM caption decoder, and uses a province-level inductive zero-shot protocol for training and evaluation.

Result: Custom ZeroCLIP achieves a CLIPScore of 0.8536, BLEU-4 of 0.3342, and METEOR of 0.4859, with ablation results showing that retrieval improves cultural vocabulary recovery by 19.3% METEOR gain.

Conclusion: The results demonstrate the effectiveness of retrieval-augmented domain adaptation for culturally grounded caption generation in low-resource heritage settings, and the dataset is publicly available.
...
Method: The authors propose MACCO, which masks compositional concepts in one modality and reconstructs them conditioned on the full contextual information from the other, along with two auxiliary objectives that align and regularize masked features.
...
Conclusion: The proposed MACCO framework effectively improves compositional understanding in vision-language models, demonstrating better capture of cross-modal compositional structures and improved performance on various tasks.
...
TL;DR: HYDRA-X is a unified multimodal model that tokenizes images and videos using a single Vision Transformer, achieving strong performance across understanding and generation tasks.
...
Method: HYDRA-X uses a Vision Transformer with spatiotemporal reconstruction capability and a lightweight decompressor for semantic awareness.

Result: HYDRA-X achieves strong performance across image and video understanding and generation tasks, with improved editing consistency and convergence.

Conclusion: HYDRA-X is a promising approach for unified multimodal models, enabling efficient and effective tokenization of images and videos.
...
TL;DR: DuET improves diffusion-based image editing by temporarily removing source image conditioning, allowing for more natural and relevant edits.


<details>
  <summary>Details</summary>
Motivation: Persistent source-image conditioning in diffusion editors can limit the execution and naturalness of edits, especially when the target scene diverges substantially from the input.
...
Conclusion: DuET offers a simple yet effective way to enhance diffusion-based image editing without modifying model weights or increasing sampling cost.
...
TL;DR: ReFree-S2V is a new framework for speech-driven talking character animation that achieves accurate lip articulation and expressive behavior.


<details>
  <summary>Details</summary>
Motivation: Existing approaches to speech-driven talking character animation struggle to balance accurate lip synchronization and expressive behavior.

Method: ReFree-S2V uses a flow-matching speech-to-portrait animation framework with a pretrained video generation model, multi-level speech representation, and a novel reward-free reinforcement learning scheme.

Result: ReFree-S2V achieves state-of-the-art performance, outperforming existing methods in lip-sync accuracy and human evaluations of naturalness and expressivity.

Conclusion: ReFree-S2V successfully addresses the challenge of balancing accurate lip articulation and expressive behavior in speech-driven talking character animation.
...
TL;DR: A novel pipeline called MagPlus is proposed for processing micro-expressions by connecting micro-expression analysis with standard facial animation models, enabling more realistic micro-expression generation.


<details>
  <summary>Details</summary>
Motivation: The difficulty in modeling and generating facial micro-expressions due to limited annotated data and weak underlying facial motions, leading to poor quality and generalization in existing methods.

Method: MagPlus, a transferable micro-expression processing pipeline that magnifies subtle facial motions into regular facial expressions, and a complementary DeMagPlus module that restores the motion to realistic micro-expression intensity levels.

Result: MagPlus-DeMagPlus enables pre-trained macro-expression models to generate more realistic micro-expression motion without retraining, as demonstrated with four facial animation models.

Conclusion: The proposed MagPlus-DeMagPlus framework improves the generation of realistic micro-expressions by leveraging standard facial expression processing models, achieving better quality and robustness.
...
TL;DR: This paper investigates self-supervised foundation models for MRI-based disease detection, comparing reconstruction-based and predictive representation learning approaches, and finds that the choice of self-supervised objective significantly impacts downstream performance.
...
Method: The paper compares two self-supervised pretraining paradigms: Masked Autoencoders (MAE) and Joint Embedding Predictive Architectures (JEPA), and introduces novel auxiliary objectives, including spectral-domain reconstruction loss and variance-covariance regularization (VCR).
...
Conclusion: The study suggests that self-supervised objectives in medical imaging encode specific biases, and their downstream benefit is fundamentally conditioned on the task's structure, highlighting the importance of self-supervised objective design for medical foundation model pretraining.
...
<details>
  <summary>Details</summary>
Motivation: Fine-grained understanding of operating room activity is crucial for workflow-aware assistance, but it remains challenging due to clutter, occlusions, and limited sensing.

Method: The paper introduces a new action-centric benchmark built on a publicly available ego-exocentric OR dataset, and proposes a vision-only temporal model that uses all available egocentric video as input.

Result: Experiments on the benchmark show that current scene graph prediction methods struggle to model temporal structure, while the proposed vision-only temporal model outperforms graph-based methods significantly.

Conclusion: The paper concludes that the proposed vision-only temporal model and multi- to single-view feature alignment strategy can improve single-view performance on multi-role action recognition, mitigating the need for extensive egocentric video capture.
...
<details>
  <summary>Details</summary>
Motivation: Traditional GAN-based approaches for CT-PET image synthesis have limited structural fidelity due to ignoring geometric consistency and operating solely in the spatial domain.

Method: DDE-GAN jointly learns from spatial and frequency domains, integrates rotational equivariance into the loss of generator and discriminator, and uses a hierarchical dual-domain training strategy.

Result: DDE-GAN achieves superior synthesis quality over baseline models on the HECKTOR 2022 CT-PET dataset, enhancing multimodal image synthesis accuracy and robustness.

Conclusion: The proposed DDE-GAN enables practical applications in PET completion and data augmentation by substantially improving multimodal image synthesis accuracy and robustness.
...
TL;DR: JointEdit3D is a framework for 3D scene editing that uses a unified RGB-geometry reconstruction-generation latent space to improve editing efficiency and structural consistency.


<details>
  <summary>Details</summary>
Motivation: Existing 3D scene editing methods have high test-time costs, limited 3D awareness, and structural inconsistencies, which JointEdit3D aims to address.

Method: JointEdit3D performs asymmetric latent inpainting using a single edited RGB reference latent and introduces a SceneAnchor Branch to inject source-scene structure.

Result: JointEdit3D improves edited-region quality and 3D structural completeness while maintaining competitive background preservation.

Conclusion: JointEdit3D presents a novel approach to 3D scene editing, offering improved efficiency, structural consistency, and editing quality, and introduces SceneEdit3D-15K and SceneEdit3D-Bench for standardized evaluation.
...
TL;DR: The paper proposes a novel image compression framework called DCIC, which achieves a better trade-off between rate, distortion, and perception by integrating a learned codec with a diffusion-based decoder.


<details>
  <summary>Details</summary>
Motivation: The rate-distortion-perception (RDP) trade-off is a unified framework for neural image compression that governs both fidelity and perceptual realism. However, practical frameworks that realize the full RDP surface are scarce due to the difficulty of introducing common randomness at the decoder.

Method: The proposed DCIC framework integrates a learned codec with a diffusion-based decoder governed by joint distortion and idempotence constraints. The distortion constraint bounds reconstruction fidelity, while the idempotence constraint serves as a tractable surrogate for the distributional perception requirement.
...
Conclusion: The proposed DCIC framework enables continuously adjustable fidelity-realism trade-offs from a single bitstream and realizes the optimal interior operating point on the RDP surface, validating its practical value.
...
Method: MoVerse separates world construction from observation rendering, using topology-aware diffusion, 3D Gaussian scaffold, and Gaussian-conditioned video renderer.
...
TL;DR: The paper proposes SmartFont, a diffusion-based framework for few-shot font generation that balances global structure and local style fidelity.


<details>
  <summary>Details</summary>
Motivation: Existing methods for few-shot font generation struggle to balance global structural completeness and fine-grained local style fidelity, either relying on global content-style modeling or emphasizing local/component modeling.

Method: The proposed SmartFont framework combines global content-style generation with weakly supervised local corrective experts, using a denoising-state condition allocation module to adaptively weight features.

Result: Extensive experiments show that SmartFont achieves better global-local balance, improves glyph quality, and local detail fidelity.

Conclusion: The paper concludes that SmartFont effectively addresses the challenge of few-shot font generation by organizing complementary global and local conditions through multi-level allocation.
...
TL;DR: The paper presents an interactive approach to identifying individuals based on their motion styles, achieving high recognition rates on several datasets.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the problem of person identification based on motion patterns, with applications in surveillance and authentication.

Method: The authors propose a probabilistic generative model inspired by the Human Information Processing (HIP) paradigm and an interactive scenario where the system presents visual cues to the subject and records their motion responses to infer their identity.

Result: The method achieves high recognition rates on five publicly available datasets and a novel dataset of 4,476 recordings from 22 test subjects.

Conclusion: The paper concludes that the proposed interactive approach can effectively identify individuals based on their motion styles, with potential applications in surveillance and authentication.
...
TL;DR: The paper introduces VietFashion, a benchmark for sketch-text composed image retrieval focusing on traditional Vietnamese garments, aiming to improve cultural semantics and fine-grained fashion retrieval.


<details>
  <summary>Details</summary>
Motivation: Cultural garments have unique challenges for visual retrieval systems due to subtle structural and symbolic details, and current AI models fail to capture these details effectively.

Method: The authors create a dataset of 21,000 photorealistic images with aligned captions using generative models, and propose a multi-target retrieval setting to reflect design intent ambiguity.
...
Conclusion: The paper concludes that VietFashion is a valuable resource for designers and researchers to retrieve culturally meaningful outfits, and highlights the need for improved AI models to capture subtle cultural details.
...
TL;DR: A new framework, OmniDirector, is proposed for cloning camera motion from reference videos in video generation, offering improved control and performance.


<details>
  <summary>Details</summary>
Motivation: Existing methods for cloning camera motion from reference videos have limitations, such as failing to handle multi-shot generation or suffering from data scarcity, leading to poor performance in complex camera motion cloning.

Method: A general camera motion representation is introduced, encoding cameras as grid motion videos, and a unified framework, OmniDirector, is proposed, trained on a million-scale camera grid-video pairs, which integrates characters, actions, and cameras for director-level control.
...
Conclusion: The proposed OmniDirector framework provides a more effective and controllable approach to cloning camera motion from reference videos, enabling director-level control for multimodal diffusion transformers.
...
<details>
  <summary>Details</summary>
Motivation: Object and rare-class errors in 3D occupancy models can affect free-space interpretation, collision checking, and temporal state propagation, and current VLM strategies do not reliably improve closed-set occupancy mIoU.
...
Result: VISA improves OccWorld from 19.06 to 20.05 mIoU and GaussianWorld from 21.36 to 21.91 mIoU on nuScenes, and specifically improves object mIoU from 18.18 to 19.16 and rare-class mIoU from 15.60 to 16.79 on GaussianWorld.
...
TL;DR: A novel point cloud registration framework, GAPR-Net, is proposed, combining convolution and transformer modules to accurately register partial and full point clouds, achieving high recall and low RMSE.


<details>
  <summary>Details</summary>
Motivation: Partial-to-full registration of point clouds is challenging due to varying overlap ratios, fluctuating point densities, and noise, which is crucial for precise surgical navigation and robotic interventions in computer-assisted surgery.

Method: A coarse-to-fine architecture GAPR-Net, which fuses local and global information using a cross-attention mechanism and a transformation-invariant point-wise geometric feature representation.

Result: The proposed method achieves an overall registration recall of 94.2%, RMSE of 1.992 mm, and high R^2 values of 0.908 and 0.974 for rotation and translation, respectively.

Conclusion: The proposed GAPR-Net effectively addresses the partial-to-full point cloud registration problem, enabling highly accurate 3D point cloud registration using partial observation, which provides a critical foundation for precise surgical navigation and robotic interventions.
...
<details>
  <summary>Details</summary>
Motivation: Existing caching methods for diffusion models use threshold-based heuristics, leading to variable inference latency and difficulty in control. The authors aim to optimize for final output quality while controlling runtime cost.

Method: BudCache uses Simulated Annealing with deterministic Hill Climbing for offline search of high-quality cache policies within a fixed compute budget. It also introduces cache-aware schedule alignment for tight budgets.
...
Conclusion: BudCache offers a more controlled and efficient approach to caching for diffusion models, leading to improved output quality and predictable inference latency.
...
TL;DR: A novel LiDAR-based approach, MinkUNeXt-VINE++, is proposed for robust place recognition in unstructured environments, such as vineyards, by fusing data from two sensors and using a learned re-ranking strategy.


<details>
  <summary>Details</summary>
Motivation: The challenge of localizing autonomous systems in unstructured environments, such as agricultural fields, particularly in repetitive environments like vineyards where finding true positives is difficult.

Method: MinkUNeXt-VINE++, which combines early fusion of heterogeneous LiDAR data from two sensors (Livox Mid-360 and Velodyne VLP-16) and a learned re-ranking strategy.
...
Conclusion: The proposed approach, MinkUNeXt-VINE++, significantly improves place recognition performance in vineyard environments, outperforming single-sensor approaches and state-of-the-art methods.
...
TL;DR: The paper proposes a measurement-calibrated fusion approach for multi-camera data fusion in indoor vision-based localization systems, which integrates component-wise error quantification to improve localization accuracy and motion smoothness.


<details>
  <summary>Details</summary>
Motivation: Indoor vision-based localization systems are affected by detection noise, occlusions, and limited camera coverage, leading to uncertainty at multiple stages of the pipeline. The authors aim to address the gap in understanding the mechanistic contributions of multi-camera data fusion.

Method: The authors introduce a measurement-calibrated fusion approach that integrates component-wise error quantification, specifically isolating homography calibration, human detection, and motion tracking. A component-wise evaluation is conducted to quantify error contributions from these components.

Result: Experimental results show that data fusion improves localization accuracy compared to single-camera baselines. The measurement-calibrated fusion approach provides limited improvement in absolute accuracy over standard fusion but substantially reduces trajectory variance and improves motion smoothness.

Conclusion: The paper highlights the value of explicit error characterization when designing data fusion strategies for vision-based indoor positioning systems, and demonstrates that the proposed approach can improve localization accuracy and motion smoothness.
...
TL;DR: MaskWAM, an object-centric world-action model, is proposed to overcome spatial bottlenecks in current World Action Models (WAMs) by integrating masks as inputs and predictions.


<details>
  <summary>Details</summary>
Motivation: Current WAMs suffer from referential ambiguity in cluttered scenes and lack semantic grounding due to unstructured RGB predictions.

Method: MaskWAM uses a unified Mixture of Transformers (MoT) to jointly integrate masks as explicit inputs and predictions, providing object-centric semantic supervision and precise spatial anchor.
...
Conclusion: MaskWAM establishes a precise and robust paradigm for manipulating unseen objects by providing robust policy generalization and suppressing visual noise.
...
TL;DR: A novel approach to saliency acquisition for biometric presentation attack detection using PCA and LDA techniques, eliminating the need for human annotation and domain knowledge.


<details>
  <summary>Details</summary>
Motivation: The high cost, domain specificity, and limited scalability of existing saliency acquisition methods hinder the adoption of saliency-guided training in biometric presentation attack detection.

Method: Using PCA and LDA techniques to generate saliency maps directly from raw training data.
...
Conclusion: The proposed approach overcomes a significant barrier to saliency-guided training for biometric attack detection, enabling cost-efficient and highly scalable saliency acquisition.
...
TL;DR: BitResEdit, a training-free editor for bitwise-residual VAR generators, enables precise text-guided image editing by combining bit-level guidance with multi-scale residual code composition.


<details>
  <summary>Details</summary>
Motivation: Existing VAR editors underutilize the native structures of bitwise-residual VAR models, specifically the per-bit Bernoulli prediction head and the additive multi-scale residual code field.

Method: BitResEdit consists of two main components: BitEdit, which performs source-negative guidance by tilting post-CFG per-bit log-odds, and ResEdit, which converts sampled bits into per-scale continuous-code residuals and re-injects them through the generator's native sum-of-scales.
...
Conclusion: BitResEdit effectively couples decision-time bit guidance with combination-time code composition, enabling localized, scale-aware edits while preserving masked-out latent features exactly.
...
TL;DR: The study investigates if contrast-informed data augmentation and domain-adversarial training can improve the generalization of a deep learning model (E2E-VarNet) from adult to neonatal MRI reconstruction. The results show that both methods improve performance on neonatal data.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the robustness of deep learning-based MRI reconstruction to domain shift from adult to neonatal data, which is crucial for accurate undersampled neonatal MR reconstruction.

Method: The study compares three training regimes: adult-only training with unaugmented data, mixed training with unaugmented and neonatal-informed augmented adult data, and mixed training with a domain-adversarial objective. The models are evaluated on neonatal and adult test data at acceleration factors R=4 and R=8.

Result: The results show that mixed training and mixed domain-adversarial training outperform unaugmented adult-only training on neonatal data. At R=4, mixed domain-adversarial training achieves the best performance (SSIM = 0.924 +/- 0.027, PSNR = 33.98 +/- 1.15 dB). At R=8, mixed domain-adversarial training performs best with SSIM, and mixed training performs best with PSNR.

Conclusion: The study concludes that contrast-informed augmentation and domain-adversarial training improve the adult-to-neonatal generalization of deep learning-based MR reconstruction, suggesting that these methods may improve robustness to domain shift in undersampled neonatal MR reconstruction.
...
TL;DR: The paper proposes EvTexture++, an event-driven framework for texture enhancement in video super-resolution, leveraging event signals to improve texture recovery and temporal consistency.


<details>
  <summary>Details</summary>
Motivation: Recent works have applied event-based vision to video super-resolution for motion refinement, but this paper focuses on texture enhancement, aiming to improve texture recovery and temporal consistency in VSR.

Method: EvTexture++ incorporates a customized texture enhancement branch, an iterative texture enhancement module, and a temporal texture alignment module to exploit high-frequency spatiotemporal details from events and enhance texture recovery and temporal consistency.
...
Conclusion: The proposed EvTexture++ framework effectively enhances texture recovery and temporal consistency in video super-resolution, and can be used as a plug-and-play tool to boost the performance of existing VSR models.
...
TL;DR: The paper proposes an optimal waste segmentation network for automated waste recycling using deep learning, which leverages both spatial and spectral domains to improve waste segmentation in cluttered scenes.


<details>
  <summary>Details</summary>
Motivation: The rapid expansion of urban areas and population growth has led to an increase in waste production, making efficient and automated waste management necessary. Current deep learning approaches for automated waste recycling suffer from performance deterioration in cluttered scenes and rely on large backbone networks.

Method: The proposed method introduces an optimal waste segmentation network that utilizes both spatial and spectral domains to capture localized structural dependencies and global contextual relationships. It also includes an auxiliary feature enhancement module (AFEM) to enhance target objects' boundaries and blob amplification.

Result: Extensive experimentation on ZeroWaste-aug, ZeroWaste-f, and SpectralWaste datasets reveals the merits of the proposed method.

Conclusion: The proposed method effectively addresses the challenges of waste segmentation in cluttered scenes and provides a promising solution for automated waste recycling using deep learning.
...
TL;DR: The paper presents a study on vehicle color recognition under severe class imbalance using a real-world surveillance dataset. It investigates synthetic minority-class augmentation and achieves 94.6% micro accuracy and 79.7% macro accuracy.
...
Method: The paper uses synthetic minority-class augmentation through two generative strategies: text-conditioned image generation and image-conditioned color editing. It also combines curated synthetic data with modern visual representations, loss reweighting, and ensemble fusion.
...
Conclusion: The paper highlights the practical limits of color-based vehicle identification in unconstrained surveillance imagery, and many remaining failures are visually ambiguous even for human annotators.
...
TL;DR: Surflo is a novel feed-forward model that reconstructs 3D surfaces from a variable number of unposed RGB views, achieving high-resolution outputs without fixed grid or token budget limitations.


<details>
  <summary>Details</summary>
Motivation: Existing reconstruction models are limited by their inability to efficiently handle multiple views and produce high-resolution outputs. Per-view methods result in redundant and unaligned pointmaps, while global-latent methods are restricted to low-resolution outputs.

Method: Surflo uses a global latent representation to compress multiple RGB views into K latent tokens, then decodes oriented 3D surface points via flow matching, allowing for arbitrary-resolution outputs.

Result: Surflo matches or surpasses feed-forward baselines on surface metrics, runs faster than optimization-based methods, and combines a global latent with arbitrary-resolution decoding.

Conclusion: Surflo offers a more efficient and flexible approach to 3D surface reconstruction from multiple views, addressing limitations of existing models and achieving high-resolution outputs.
...
TL;DR: World Tracing is a new method for generating 3D geometry from images, balancing faithfulness to the input and completeness of the generated shape.
...
Result: World Tracing outperforms existing methods on visible-surface reconstruction and complete geometry generation across various benchmarks. It also preserves 2D-to-3D correspondence, enabling new applications.

Conclusion: World Tracing offers a novel approach to image-to-3D generation, achieving strong performance and enabling new applications like text-driven 3D scene editing and geometry-conditioned novel-view video synthesis.
...
TL;DR: Flex4DHuman is a multi-view video diffusion model that transforms monocular or sparse multi-view videos into synchronized dense multi-view videos using relative camera-pose conditioning, enabling dynamic 4D content creation.


<details>
  <summary>Details</summary>
Motivation: The motivation is to create a practical method for scalable 4D content creation from casual monocular videos for applications such as simulation, gaming, AR/VR, and video re-shooting.

Method: Flex4DHuman uses a multi-view video diffusion model with relative camera-pose conditioning, built on the Wan 2.1 1.3B text-to-video model, and trains through a three-stage curriculum with pose following, flexible reference-to-target view generation, and temporal rollout.

Result: Flex4DHuman surpasses prior state-of-the-art methods on DNA-Rendering and ActorsHQ datasets and generalizes to animal categories after mixed human-animal training, enabling dynamic 4D Gaussian splats creation.

Conclusion: Flex4DHuman presents a practical step toward scalable 4D content creation from casual monocular videos, with applications in simulation, gaming, AR/VR, and video re-shooting.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of existing spatial agents in open-ended spatial reasoning, which are bounded by their action interfaces and lack flexibility in composing operations or tailoring analysis to each task.

Method: The method proposed is SpatialClaw, a framework that adopts code as the action interface, maintaining a stateful Python kernel pre-loaded with input frames and perception primitives, allowing a VLM-backed agent to write executable code conditioned on prior outputs.
...
Conclusion: The conclusion is that SpatialClaw offers a more flexible and effective approach to open-ended spatial reasoning, outperforming existing methods and demonstrating consistent gains across various benchmarks and model backbones.
...
TL;DR: RepWAM, a representation-centric world action model, uses semantic visual-action tokenization to improve instruction-following dynamics in robot control, outperforming reconstruction-oriented alternatives.


<details>
  <summary>Details</summary>
Motivation: Existing world action models (WAMs) use reconstruction-oriented video tokenizers, which provide limited guidance for learning instruction-following dynamics in robot control.

Method: The authors propose a representation-centric WAM (RepWAM) built on representation visual-action tokenizers, which maps visual inputs into aligned visual and latent action tokens.

Result: RepWAM delivers strong performance across diverse manipulation settings, outperforming reconstruction-oriented alternatives, and establishes representation visual-action tokenization as a promising foundation for world action models.

Conclusion: The proposed RepWAM model and representation visual-action tokenization approach show promise for improving instruction-following dynamics in robot control and generalist robot policies.
...
TL;DR: A simple method called Modality Forcing is proposed for joint image-depth generation using a single DiT model trained on sparse depth data.


<details>
  <summary>Details</summary>
Motivation: Existing methods for adapting text-to-image models for depth prediction require dense depth data and complex recipes, which is a limitation.

Method: Modality Forcing, a post-training recipe that assigns separate noise levels per modality and uses per-modality decoders.

Result: The proposed method achieves competitive results with state-of-the-art monocular depth estimators and reduces AbsRel by 57% relative to existing joint image-depth generative models.
...
<details>
  <summary>Details</summary>
Motivation: Current image generators lack the ability to produce interleaved text-image sequences, which is crucial for applications like visual narratives and guidance.

Method: The proposed pipeline uses a planner agent to organize input sequences and instruct the image generator, and a critic agent to evaluate outputs and refine instructions. It also utilizes reinforcement learning with custom reward functions.
...
Conclusion: The proposed InterleaveThinker pipeline effectively enables existing image generators to produce high-quality interleaved text-image sequences, with significant improvements on reasoning-based benchmarks.
...
TL;DR: The EDEN dataset is a large-scale corpus of clinical notes from Italian Emergency Departments, containing 4 million anonymized notes and a subset of 6,000 annotated notes for developing Large Language Models in medical applications.


<details>
  <summary>Details</summary>
Motivation: The dataset aims to fill a gap in data supporting the development and use of Large Language Models in concrete medical applications, specifically for the Italian language.

Method: The dataset was created through a data collection protocol, on-site anonymization pipeline, and annotation scheme involving multiple clinicians and iterative revision.

Result: The EDEN dataset is the largest freely available corpus of clinical notes for the Italian language, with a proposed benchmark for structured information extraction (CRF-filling) and zero-shot baselines from Gemma-27B and MedGemma-27B.

Conclusion: The EDEN dataset provides a valuable resource for developing and using Large Language Models in medical applications, particularly for the Italian language, and proposes a new benchmark for structured information extraction.
...
TL;DR: A new method for generating narrated videos from scientific figures and their accompanying papers, enabling step-by-step explanations aligned with visual highlights.


<details>
  <summary>Details</summary>
Motivation: Current video generation systems lack the ability to provide paper-grounded, step-by-step narration aligned with visual highlights in scientific figures.

Method: The proposed method, MINARD (Multimodal Interpretation of Narrated Architecture via Region Decomposition), generates paper-grounded narrations and sequentially grounds them to figure regions.

Result: MINARD generates humanlike, paper-faithful narrations and outperforms existing approaches in both automatic and human evaluation on the FigTalk benchmark.

Conclusion: The proposed method enables the generation of high-quality narrated videos from scientific figures and their accompanying papers, addressing a significant limitation in current video generation systems.
...
TL;DR: A novel mechanism-level drug-drug interaction (DDI) prediction pipeline, MARD-7B, is proposed, achieving high accuracy and outperforming baselines while requiring minimal API costs.


<details>
  <summary>Details</summary>
Motivation: Current DDI prediction methods only identify whether two drugs interact, but do not provide detailed information on the mechanism, direction, or evidence of the interaction. The authors aim to develop a mechanism-level DDI prediction protocol and pipeline that provides more informative and accurate results.

Method: The proposed pipeline, MARD-7B, combines three training innovations: single-token KL divergence, per-loss PRM-weighted DPO with programmatic hard negatives, and a leakage-safe mechanism-aware retrieval channel. It uses a structured 7-family/147-subtype taxonomy and auditable reasoning metrics for evaluation.

Result: MARD-7B outperforms 31 other systems in a comparison on the April-2026 DrugBank release, achieving accuracy that survives drug-pair novelty and beating the best baseline by +13.9 pp and GPT-4o by +6.7 pp at ~1% of frontier API cost. The pipeline also shows an anti-memorisation signature, with improved accuracy on rarely seen drugs.

Conclusion: The proposed MARD-7B pipeline provides a reproducible and accurate mechanism-level DDI prediction approach, offering a more informative and reliable solution for pharmacological prediction and reasoning.
...
TL;DR: The paper studies how to transform abstract proverbs into engaging narratives that stay true to their moral meaning, using large language models (LLMs) and introducing a new dataset for Persian proverbs.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of transforming dense, abstract proverbs into morally faithful narratives, which requires deep cultural understanding and semantic grounding.

Method: The method involves framing this problem as a 'constrained semantic decompression' task, introducing the Proverb Aligned Narrative Dataset (PAND), and using a hybrid evaluation framework that combines human-calibrated LLM-as-a-Judge with structural metrics.

Result: The results show that current LLMs often achieve strong surface-level fluency but fail to faithfully instantiate the underlying moral and causal structure encoded in proverbs, revealing a 'decompression gap'.

Conclusion: The conclusion is that explicit reasoning and iterative refinement can partially mitigate these failures, suggesting that many decompression errors arise from difficulties in translating abstract meaning into narrative form rather than a complete lack of relevant knowledge.
...
TL;DR: A new benchmark, Shopping Reasoning Bench, is introduced to evaluate conversational shopping assistants, highlighting their limitations in multi-turn reasoning, domain expertise, and criterion-level quality.
...
Method: The Shopping Reasoning Bench consists of 525 missions with 10863 importance-weighted binary rubrics, organized under a taxonomy of five reasoning categories and fifteen subcategories, and was used to evaluate nine models across three families (GPT, Claude, Gemini).

Result: Current models show pass rates of 57-77% overall, with significant performance degradation on multi-turn missions and optional criteria, indicating they can handle basic shopping assistance but not expert-level advice.

Conclusion: The Shopping Reasoning Bench serves as a challenging testbed for future shopping assistant development, revealing the need for improvement in handling complex shopping conversations.
...
TL;DR: The study proposes a two-phase framework for Arabic mental health text classification, achieving a macro-F1 of 0.861 and accuracy of 0.877.


<details>
  <summary>Details</summary>
Motivation: Detecting mental health disorders from Arabic social media text is challenging due to dialectal variation, informal language, limited resources, and class imbalance.

Method: The study uses a two-phase framework: (1) adapting pre-trained language models (AraBERT, CAMeLBERT, MARBERT) using Domain-Adaptive and Task-Adaptive Pretraining, and (2) evaluating the adapted models with different classification architectures.

Result: The domain-adapted MARBERT (MentalMARBERT) achieves statistically significant improvements, with a macro-F1 of 0.861 and accuracy of 0.877 using a hierarchical two-stage architecture with full fine-tuning.

Conclusion: The study demonstrates the effectiveness of domain-specific adaptive pretraining and hierarchical classification for Arabic mental health disorder detection.
...
TL;DR: Latent reasoning models' internal mechanisms may not be as interpretable as thought, and their 'thought' processes might be more complex than they seem.


<details>
  <summary>Details</summary>
Motivation: To investigate the internal reasoning mechanisms of latent reasoning models (LRMs) and challenge the assumption that observable latent-state patterns imply internal reasoning.

Method: Evaluating two LRMs (Coconut and CODI) against controls, using causal interventions and geometric analyses to study the 'thought' processes of LRMs.

Result: The study finds that observable latent-state patterns also appear in controls and do not always causally affect behavior, and that 'latent thoughts' should be treated as hidden computation rather than hidden explanation.

Conclusion: LRM interpretability requires more rigorous testing, including matched controls and causal tests, as decodability, attention, or static structure alone cannot establish the mechanism.
...
TL;DR: The paper introduces AfriSUD, a large-scale collection of syntactically annotated treebanks for nine African languages, and evaluates various NLP models on part-of-speech tagging and dependency parsing, revealing a significant syntax gap.


<details>
  <summary>Details</summary>
Motivation: African languages are underrepresented in NLP research and resources, and the authors aim to bridge this gap by providing a large-scale collection of annotated treebanks for nine diverse African languages.

Method: The authors use the Surface-Syntactic Universal Dependencies (SUD) framework to create high-quality, native-speaker verified data, and evaluate a range of models, including non-transformer baselines, multilingual pretrained encoders, and LLMs, on part-of-speech tagging and dependency parsing.

Result: The results show a significant syntax gap, where models still have clear limitations across the nine languages, suggesting that existing architectures may not fully capture the structural diversity of African-language syntax.

Conclusion: The paper concludes that existing NLP models have limitations when dealing with African languages, and that more work is needed to develop models that can capture the structural diversity of African-language syntax.
...
TL;DR: The authors introduce PaperGuard, a benchmark to evaluate and defend AI-generated peer-review against domain-specific, cross-modal attacks, highlighting the vulnerability of AI reviewers and providing a practical defense.


<details>
  <summary>Details</summary>
Motivation: The integration of Large Language Models (LLMs) and Multimodal LLMs (MLLMs) into scientific peer-review workflows introduces risks for adversarial manipulation, particularly with multimodal scientific papers, and current robustness studies are mostly text-only.

Method: The authors develop a multimodal peer-review dataset, a unified suite of attacks (including black-box prompt injections and white-box perturbations), and a practical defense using chunk-based embedding search.

Result: Extensive experiments across state-of-the-art models confirm that AI reviewers are pervasively vulnerable to attacks, but PaperGuard provides a foundational benchmark, protocols, and actionable defense for trustworthy AI-assisted scholarly reviewing.

Conclusion: PaperGuard is a comprehensive benchmark that addresses the vulnerability of AI-generated peer-review to domain-specific, cross-modal attacks, providing a necessary defense for trustworthy AI-assisted scholarly reviewing.
...
<details>
  <summary>Details</summary>
Motivation: The authors aim to explain why languages often have irregularities in their morphology, such as the past tense of 'go' being 'went', and to understand how these irregularities emerge and persist.
...
Result: The results suggest that certain factors, such as scale-free social networks and random Bernoulli adoption of forms, favor the emergence of more plausible morphologies, and that the simulated morphologies can be comparable to those of real languages.

Conclusion: The authors conclude that their simulation can help explain the emergence and persistence of morphological irregularities in languages, and that their novel evaluation method can be used to assess the plausibility of simulated morphologies.
...
TL;DR: The paper explores methods to improve large language models' (LLMs) ability to capture human judgment, finding that simple prompting strategies can enhance AI-human alignment.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the limitations of LLMs in capturing human judgment, specifically their inability to capture full distributions of responses and instability across wording variations.

Method: The paper employs simple prompting strategies, including eliciting standard deviations and response proportions, and ensuring scenario clarity to human participants.

Result: The results show that these strategies improve AI-human alignment across two datasets, with LLMs better capturing the full range of human responses and tracking human confusion ratings, though their estimates of their own error are poorly calibrated.

Conclusion: The paper concludes that asking better questions to LLMs can yield better answers, and that simple elicitation techniques can improve LLMs' ability to capture human judgment.
...
TL;DR: The paper analyzes Apple's Metal 4.1 tensor compute path, specifically the MPP matmul2d operation, and finds that the fp8 (E4M3) matmul2d is emulated, not accelerated, and provides detailed characterization of its hardware behavior.


<details>
  <summary>Details</summary>
Motivation: The motivation is to empirically characterize the Metal 4.1 tensor compute path, specifically the MPP matmul2d operation, on an Apple M4 Max, as the specification hides or contradicts important details about its hardware behavior.

Method: The authors use a checksum-gated, provenance-tracked microbenchmark harness, called Rigel, to recover eleven facts about the MPP matmul2d operation that are not documented or are contradicted by the specification.

Result: The headline finding is that the Metal 4.1 fp8 (E4M3) matmul2d is emulated, not accelerated, and sustains 0.94x the throughput of fp16 despite reading half the operand bytes. The authors also characterize the execution, accumulation, and fragment layout of the matmul2d operation.

Conclusion: The characterization reveals that the matmul2d operation executes entirely on the GPU shader cores with no dedicated matrix datapath and no evidence of Apple Neural Engine routing. The authors also provide a hand-fused GEMM + bias + GELU kernel that beats the decomposed path by +6.5-12.9% in the cache-resident regime.
...
TL;DR: The paper introduces HieraRAG, a framework for determining optimal granularity in RAG benchmark construction, and applies it to a case study with synthetic QA pairs.


<details>
  <summary>Details</summary>
Motivation: Practitioners lack guidance on which dimensions to vary and at what granularity when evaluating RAG systems, making it hard to capture diverse question characteristics.

Method: The authors define optimal granularity as the level that maximizes discriminative power within a given RAG configuration and apply it to 5,872 synthetic QA pairs across 3 dimensions.

Result: Optimal granularity varies by dimension: fine-grained for complexity, medium for answer type and linguistic variation; a Coherence Ratio metric is introduced.

Conclusion: HieraRAG provides a portable procedure and validation metric for practitioners to determine evaluation granularity within their own RAG settings.
...
TL;DR: The paper proposes a new evaluation metric called GENIE to measure the novelty of responses generated by Large Language Models in a task-specific manner.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the lack of creativity and diversity in Large Language Models by investigating what makes model-generated content novel or not novel.

Method: The authors propose a fine-grained evaluation metric GENIE to measure the novelty of responses along task-specific features with respect to a population of responses.

Result: The results show that GENIE is more effective in capturing the high-dimensionality of novelty compared to holistic metrics, and it provides insight into which properties of novelty are being targeted.

Conclusion: The paper concludes that GENIE can be used to measure the effectiveness of mitigation methods that address creativity and help understand where these methods can improve novelty.
...
<details>
  <summary>Details</summary>
Motivation: Current summarization methods can become outdated as new information arrives, and regenerating summaries from scratch can be unnecessary and obscure changes.
...
TL;DR: The study examines how irrelevant numbers in prompts affect language model judgments, causing anchoring effects in numerical reasoning.
...
Method: Using a controlled multiple-choice setup, logit-difference metric, and attribution-based circuit localization on 7B--8B Qwen and Llama models.

Result: Edge-level methods recover the anchor-sensitive signal more faithfully than node-level methods, and low- and high-anchor circuits transfer strongly within a model.

Conclusion: The study provides a mechanistic account of how anchoring-related decision signals are carried inside language models.
...
TL;DR: The paper introduces LoHoSearch, a new benchmark for search agents that is more challenging than existing ones, and demonstrates that even the strongest models struggle to achieve high accuracy on it.


<details>
  <summary>Details</summary>
Motivation: Existing search agent benchmarks have become saturated, with models achieving over 90% accuracy, but these benchmarks are limited by their human-authored nature, which restricts their ability to test long-horizon reasoning and context management.

Method: The authors introduce an automated pipeline built on a knowledge graph of 7 million Wikipedia entities to construct LoHoSearch, a benchmark of 544 human-verified questions across 11 domains.

Result: The strongest model achieves only 34.74% accuracy on LoHoSearch, and existing context management strategies yield limited gains, indicating that LoHoSearch provides a more demanding standard for evaluating search agents.

Conclusion: LoHoSearch provides a more challenging benchmark for evaluating long-horizon reasoning and context management in search agents, highlighting the need for further research in this area.
...
TL;DR: Fine-tuning small LLMs via QLoRA achieves competitive performance to GPT-4o and GPT-5 on biomedical claim verification at a lower cost.
...
Result: Mistral-7B QLoRA surpasses GPT-4o and GPT-5 by up to 12% F1 gain at a fractional cost using only 1,008 training examples.

Conclusion: Fine-tuning small LLMs via QLoRA can achieve strong performance on biomedical claim verification at a lower cost, and training on structurally sound data enables robust cross-domain transfer.
...
TL;DR: The paper presents a fine-tuning approach for large language models using Direct Preference Optimization (DPO), showing improved efficiency and competitive performance.


<details>
  <summary>Details</summary>
Motivation: To simplify the training pipeline and improve computational efficiency of large language models.

Method: Direct Preference Optimization (DPO), a reinforcement learning technique.

Result: The approach achieves competitive performance, improves computational efficiency, and shows effective learning and convergence using BLEU, ROUGE, and cosine similarity metrics.

Conclusion: The proposed approach has potential but requires further investigation to address observed training instability.
...
TL;DR: The paper evaluates extraction-based methods as an alternative to rewriting-based retrieval-augmented generation (RAG) for accessing organisational documentation, focusing on balancing precision, recall, and safety.
...
Conclusion: The study concludes that extraction-based methods, particularly line-number-based source selection, offer a hallucination-resistant alternative to rewriting-based RAG systems, with varying performance depending on document structure.
...
TL;DR: A new framework called PRISM is proposed for empathetic spoken dialogue systems, which decouples speech perception, response generation, and speech synthesis to improve emotional alignment and response quality.


<details>
  <summary>Details</summary>
Motivation: Current spoken dialogue systems struggle with emotional alignment and prosodic expression due to limitations in speech-to-text conversion and end-to-end speech models.

Method: PRISM, a multi-agent framework that introduces a prosody-to-language translation mechanism and enables on-demand invocation of external knowledge tools for empathetic dialogue generation.
...
Conclusion: The proposed PRISM framework offers a promising approach to developing empathetic spoken dialogue systems with improved emotional alignment and response quality.
...
TL;DR: The paper studies the problem of evidence conflict in Retrieval-augmented generation (RAG) systems, particularly in multilingual settings, and proposes a benchmark and a pipeline to address this issue.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the issue of evidence conflict in RAG systems, where retrieved evidence may be mutually contradictory, especially in multilingual settings.

Method: The method involves creating a controlled benchmark called X-RAMDocs-ZHEN and an interpretable pipeline called X-MADAM-RAG, which decomposes evidence handling into several stages.

Result: The results show that X-MADAM-RAG outperforms baselines on the controlled benchmark, but its performance drops significantly on a stress test subset, revealing limitations in document-level extraction.

Conclusion: The conclusion is that X-RAMDocs-ZHEN and X-MADAM-RAG can be used as diagnostic tools for controlled evidence conflict, but further work is needed to improve document-level extraction and general hallucination detection.
...
TL;DR: The paper proposes SENTINEL, a failure-driven reinforcement learning framework that improves tool-using language model agents by turning their rollout failures into targeted training tasks.


<details>
  <summary>Details</summary>
Motivation: The effectiveness of reinforcement learning for training tool-using agents depends heavily on the training task distribution, which can become mismatched with the policy's evolving capabilities if tasks are fixed before training.

Method: SENTINEL follows a Controller--Proposer--Solver loop: the Controller analyzes failed trajectories and summarizes recurring error patterns, the Proposer generates executable tasks that stress these weaknesses, and the Solver is trained on the targeted tasks.

Result: SENTINEL improves Pass^1 from 66.4 to 74.9 and outperforms RL on general synthetic tasks across Pass^k metrics on Tau2-Bench Retail with Qwen3-4B-Thinking-2507.
...
TL;DR: The paper addresses error propagation in cascaded speech translation systems due to incorrect transcripts from Automatic Speech Recognition (ASR). It categorizes ASR errors for Vietnamese ST, finding phonetic confusions significantly degrade ST quality. A method called Phonetically-Informed Data Augmentation (PiDA) is proposed to improve translation of erroneous ASR outputs.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to tackle the issue of error propagation in cascaded speech translation systems caused by incorrect transcripts from ASR, specifically focusing on Vietnamese ST.

Method: The method used involves systematic categorization of ASR errors, Linear Mixed-Effects Modelling to quantify their impact on NMT performance, and proposing PiDA to generate ASR-like corruptions for data augmentation.

Result: The results show that PiDA improves translation of erroneous ASR outputs (up to +2.04 BLEU) and slightly improves clean-text performance when fine-tuning on a PiDA-augmented version of FLEURS Vietnamese-English.

Conclusion: The paper concludes that phonetic confusions are a significant cause of ASR errors in Vietnamese ST, and that PiDA can effectively improve the robustness of ST systems to ASR errors.
...
TL;DR: The paper introduces Polar, a benchmark to measure political bias in large language models (LLMs) across different political and linguistic contexts, and finds that bias varies systematically with context, issue category, model group, and presentation language.


<details>
  <summary>Details</summary>
Motivation: The increasing significance of political bias in LLMs and the difficulty in measuring it reproducibly across political and linguistic contexts.

Method: The authors introduce a 4,026-instance multiple-choice benchmark called Polar, which measures political bias through option-level likelihoods.

Result: The study finds that measured bias varies systematically with political context, issue category, model group, and presentation language, and that all models lean left-progressive on U.S. political content but show more centered and mixed patterns on South Korean content.

Conclusion: The findings highlight the need for multilingual and cross-contextual evaluation of political bias in LLMs.
...
TL;DR: The paper discusses how LLMs struggle with maintaining accuracy in multi-turn conversations and proposes a solution to improve their performance.


<details>
  <summary>Details</summary>
Motivation: The accuracy of LLMs drops significantly in multi-turn conversations, even with full context availability, due to the 'Lost in Conversation' degradation.

Method: The authors propose training models with a compact rolling memory instead of attending to a growing history, using a low-cost sharding pipeline to convert single-turn QA datasets into multi-turn fragmented-information episodes.

Result: The memory-augmented policy significantly improves multi-turn accuracy and generalises zero-shot to harder math and out-of-domain long-context QA, outperforming full-history baselines even when given the full history at test time.

Conclusion: The paper concludes that learning to compress information induces more robust incremental reasoning than full-context exposure alone, and that the proposed approach can mitigate the 'Lost in Conversation' degradation.
...
TL;DR: The authors present SkillChain, a system that automates the lifecycle of Skills for image-based AI assistants on e-commerce platforms, improving response quality and user engagement.


<details>
  <summary>Details</summary>
Motivation: The deployment of image-based AI assistants on e-commerce platforms faces challenges due to diverse user intents, requiring different response formats, tool invocations, and domain knowledge. Manual engineering is infeasible due to the breadth and dynamism of the intent space.

Method: SkillChain, a three-stage system: Skill Creator for bootstrapping, Route Optimizer for routing alignment, and Body Refiner for iterative Skill Body refinement via dual-path LLM-Judge evaluation.

Result: SkillChain substantially improves aggregate response quality, with significant gains in user engagement, content consumption, and long-term retention, confirmed by a one-week online A/B experiment.

Conclusion: The proposed SkillChain system effectively addresses the challenges of diverse user intents in image-based AI assistants on e-commerce platforms, leading to improved response quality and user engagement.
...
TL;DR: AI reviewers can be manipulated by modifying presentation-level content, such as abstracts and discussions, to receive higher scores without changing the scientific evidence.


<details>
  <summary>Details</summary>
Motivation: The study aims to investigate a harder and more policy-relevant failure mode of AI-generated reviews, where the attacker modifies only presentation-level content, and to analyze the robustness of AI reviewers.

Method: The authors introduce a closed-loop attack called adversarial repackaging, which uses AI-reviewer feedback to search for presentation-level revisions while keeping the scientific evidence fixed.

Result: Adversarial repackaging achieves a 75.1% attack success rate and a mean score gain of +1.21/10 across three mainstream AI reviewers, and reveals two deeper structural failure modes of AI reviewers.

Conclusion: The study highlights the deployment risk of AI reviewers not only from malicious hidden instructions but also from the emergence of paper presentation itself as an optimization surface, and emphasizes the need for testing AI reviewers' robustness to presentation-only edits.
...
TL;DR: The authors propose a local, domain-adapted pipeline using the MedGemma-27B model for extracting structured clinical information from unstructured EHR notes, achieving near-competitive performance while preserving privacy.


<details>
  <summary>Details</summary>
Motivation: The extraction of structured clinical information from unstructured EHR notes is hindered by the deployment of large language models in clinical settings due to privacy risks, inference costs, and the tendency to hallucinate.

Method: A two-stage architecture using the MedGemma-27B model, which separates binary presence classification from value extraction, and leverages item-specific, few-shot in-context learning without external API calls or fine-tuning.

Result: The approach achieves a macro-F1 score of 0.55 on the official English test track, securing second place among all locally-hosted, open-source submissions.

Conclusion: The work demonstrates that privacy-preserving, on-premise LLM pipelines can achieve near-competitive performance with proprietary frontier models, providing a practical, data-sovereign framework for clinical NLP.
...
TL;DR: The authors introduce LEDGER, a dataset of 4,999 digitized corporate annual reports, to evaluate large language models in finance reporting, focusing on extracting financial KPIs and linking them to market reactions.
...
Method: The authors create a dataset of digitized corporate annual reports, labeling 31 consolidated financial KPIs and linking them to market reactions, then derive three evaluation benchmarks for KPI retrieval and extraction tasks.

Result: The authors provide a comprehensive dataset and toolchain for evaluating large language models in finance reporting, demonstrating its research utility with a case study on CEO-letter rhetoric and market impact.

Conclusion: The LEDGER dataset and benchmarks provide a valuable resource for evaluating and improving large language models in finance reporting, enabling more accurate extraction of financial KPIs and analysis of market reactions.
...
TL;DR: MÖVE is a holistic benchmark for evaluating large language models (LLMs) in the German public sector, addressing gaps in existing benchmarks by evaluating models across performance and governance criteria.


<details>
  <summary>Details</summary>
Motivation: The adoption of LLMs in public administration is increasing, but model selection is largely ad hoc, and existing benchmarks are limited in their guidance, being predominantly English-centric and US-centric.

Method: MÖVE evaluates 39 models across two dimensions (performance and governance) using a multi-metric evaluation strategy, including classical NLP metrics, embedding-based methods, and LLM-as-a-judge approaches.

Result: No single model dominates across all criteria; top performers differ between tasks, and model size alone is a poor predictor of quality.

Conclusion: MÖVE provides a comprehensive evaluation of LLMs in the German public sector, highlighting the importance of considering both performance and governance criteria in model selection, and is designed as a living benchmark under active development.
...
TL;DR: The paper proposes G-Long, a graph-enhanced framework for improving long-term consistency in open-domain dialogue systems by using a fine-tuned small Language Model and attention-aware importance scoring.


<details>
  <summary>Details</summary>
Motivation: The limitations of Large Language Models (LLMs) in long-context reasoning and the inefficiency of processing extensive raw text make maintaining long-term consistency in open-domain dialogue systems challenging.

Method: The proposed G-Long framework utilizes a fine-tuned small Language Model (sLM) for structured triplet extraction and associative retrieval, along with an attention-aware importance scoring mechanism.
...
Conclusion: The proposed G-Long framework effectively addresses the limitations of existing approaches and achieves significant improvements in long-term consistency for open-domain dialogue systems.
...
Method: The authors design a three-agent collaborative framework to synthesize 400 English and 400 Chinese complex questions via live-web traversal, which includes a QA synthesis agent, an information filtering agent, and a high-level guidance agent.

Result: Extensive experiments confirm that EvoBrowseComp is a challenging benchmark that requires broad horizontal search, establishing a scalable paradigm for auto-updatable, high-difficulty benchmarking.

Conclusion: EvoBrowseComp provides a future-proof evaluation benchmark for search agents, addressing issues with existing benchmarks and enabling the evaluation of genuine retrieval capabilities.
...
Conclusion: The proposed framework effectively bridges the gap between low-latency benefits and natural flow in simultaneous speech-to-speech translation.
...
Method: The proposed framework, HyPE, analyzes each persona-bearing text as a quadruple and organizes persona elements into a hypergraph. It uses a HyperGCN neural network and proposes Persistent Edge Embeddings (PEE) for improved performance.

Result: HyPE outperforms sentence-level pooling baselines across different model scales (GPT-2, LLaMA-3.2-3B, and Qwen2.5-3B) under greedy decoding on PersonaChat.
...
TL;DR: A novel framework, NTS-CoT, is proposed to mitigate hallucinations in timeline summarization (TLS) by leveraging Chain-of-Thought (CoT) reasoning, outperforming state-of-the-art baselines.
...
Method: NTS-CoT framework, consisting of Element-CoT, Date Selection, and Causal-CoT modules, to capture essential news elements, select timestamps, and infer causal relationships.

Result: NTS-CoT outperforms state-of-the-art baselines in extensive experiments on three TLS benchmarks and human evaluation, effectively mitigating hallucinations and improving LLM-based TLS performance.

Conclusion: The proposed NTS-CoT framework effectively mitigates hallucinations in timeline summarization, improving the performance of LLM-based TLS.
...
<details>
  <summary>Details</summary>
Motivation: The need for LLMs to operate over long-term interactions while preserving and recalling information from past dialogues, and the challenge of managing a growing memory store with redundant entries that inflate storage costs and degrade retrieval.
...
Conclusion: The proposed MemRefine framework effectively manages memory in LLMs, preserving useful information while staying within a fixed storage budget, and outperforming existing methods.
...
TL;DR: Introduced LAUKIN, a dataset for cross-jurisdictional legal equivalence classification, and evaluated models for this task.


<details>
  <summary>Details</summary>
Motivation: Existing legal NLP datasets are limited to single jurisdictions, while multinational companies require cross-jurisdictional contract review.

Method: Developed a multi-stage retrieval and reranking pipeline to construct a dataset of labelled clause pairs, and evaluated 12 models across 4 techniques.

Result: Achieved a best macro-F1 of 65.11%, and identified significant divergence in drafting conventions across jurisdictions.

Conclusion: LAUKIN is a challenging benchmark for cross-jurisdictional legal equivalence classification, and can support future semi-supervised learning research in legal NLP.
...
TL;DR: A dataset called BioStance is introduced for context-aware stance detection in bioethical discussions on social media, particularly Reddit.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the lack of large-scale, domain-specific resources for modeling context-dependent discourse in bioethical debates on social media.

Method: The authors created BioStance, a dataset of 39,600 annotated Post-Comment pairs from Reddit, labeled by three independent annotators using a three-class stance scheme.

Result: The dataset, BioStance, which includes annotations with substantial reliability (mean Krippendorff's α of 0.82), supports research on context-aware stance detection, argument mining, and computational analysis of bioethical discourse.

Conclusion: The BioStance dataset facilitates research in bioethical discourse analysis by providing a large-scale, context-aware resource for stance detection and related tasks.
...
TL;DR: The paper introduces SICI, a diagnostic measure to assess the semantic-pragmatic burden in stance detection tasks. It evaluates how SICI predicts LLM accuracy and characterizes LLM errors across different complexity levels.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge that harder examples in stance detection tasks are not always improved by clearer instructions, reasoning prompts, retrieval, or debate.

Method: The method involves introducing SICI, a seven-dimensional diagnostic measure, and conducting experiments across SemEval-2016 and VAST datasets to evaluate SICI's effectiveness in predicting LLM accuracy.

Result: The results show that SICI predicts LLM accuracy better than surface proxies, and LLM errors change regime as SICI increases. The phase-transition-like structure of LLM errors persists across different models.

Conclusion: The conclusion is that SICI can effectively diagnose the semantic-pragmatic burden in stance detection tasks, and that prompting, retrieval, and debate often shift models along the attribution-abstention axis rather than removing the high-complexity bottleneck.
...
TL;DR: The paper analyzes the use of Optimal Transport (OT) to detect hallucinations in Neural Machine Translation (NMT) and Abstractive Summarization. It finds that OT can detect hallucinations in NMT, but its effectiveness is limited in summarization due to different failure modes.


<details>
  <summary>Details</summary>
Motivation: The motivation is to investigate the use of Optimal Transport (OT) as a detector for hallucinations in Neural Machine Translation (NMT) and Abstractive Summarization, and to understand its limitations.

Method: The method involves using Optimal Transport (OT) to measure the geometric distance between cross-attention distributions and a reference distribution, without any supervision.

Result: The results show that OT can detect hallucinations in NMT, with Wass-to-Unif and Wass-to-Data being complementary detectors. However, its effectiveness is limited in summarization, achieving 57.2%/57.6% balanced accuracy, which is above chance but below supervised methods.

Conclusion: The conclusion is that OT on cross-attention is a reliable detector for hallucinations in NMT, but its effectiveness is limited in summarization due to different failure modes. It is a principled interpretability tool regardless of task, but fundamentally limited when faithfulness failures occur downstream of attention.
...
TL;DR: The paper introduces SemCog Bench, a benchmark for evaluating cross-lingual semantic understanding in large language models (LLMs) for Arabic and Hebrew. It reveals a gap in LLMs' ability to reason across languages, particularly with false friends and loanwords.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of cross-lingual semantic understanding in LLMs due to the overlap of lexicon between closely related Semitic languages like Arabic and Hebrew.

Method: The method involves introducing SemCog Bench, a curated benchmark of 1,858 Arabic-Hebrew word pairs with sentence-level annotations, and evaluating open-source and commercial LLMs across multiple input representations.

Result: The result shows that LLMs achieve high accuracy on true cognates but performance drops sharply on false friends and loanwords. Sentence-level context yields only modest improvements.

Conclusion: The conclusion is that current LLMs have a fundamental limitation in resolving cross-lingual form-meaning conflicts, and SemCog Bench is established as a rigorous benchmark for multilingual semantic reasoning.
...
TL;DR: The authors propose PolyAlign, a framework for aligning language models with human response distributions in a more nuanced and context-dependent way, improving naturalness and faithfulness across languages and tasks.


<details>
  <summary>Details</summary>
Motivation: Current post-training methods for language models often result in a single, global assistant behavior, which can suppress natural variation in human responses across different languages, tasks, and dialogue settings.

Method: The authors introduce PolyAlign, which combines Bucket-Aware Supervised Fine-Tuning (SFT) and Human-Distribution Preference Optimization (HDPO) to align language models with bucket-specific human reference distributions.

Result: PolyAlign improves conditional naturalness and distributional faithfulness across a bilingual evaluation suite while preserving competitive task utility.
...
TL;DR: The paper proposes a framework to extract diverse perspectives from text, highlighting the limitations of current LLMs in generating pluralistic content.


<details>
  <summary>Details</summary>
Motivation: The need to represent diverse perspectives in LLM generation and the difficulty in identifying perspectives expressed in text.

Method: A domain-agnostic multi-layered framework for unsupervised extraction of perspectives.

Result: The framework reveals that while some models and prompts approach broad perspective coverage, rarer perspectives remain underrepresented.

Conclusion: Current LLMs struggle to generate text that represents diverse perspectives, with rarer views being disproportionately underrepresented.
...
TL;DR: The paper presents RogueAI, a web app that tests if a human can identify a deceptive AI among two indistinguishable Large Language Model agents.


<details>
  <summary>Details</summary>
Motivation: The original Turing Test has evolved to focus on whether a conversational system can be trusted, rather than if it's artificial.

Method: The authors created a one-on-two interrogation game where a human player questions two LLaMA agents, one of which is licensed to deceive, and introduced AutoRogueAI, a procedural extension for co-designing scenarios.

Result: A pilot deployment showed that while a simple heuristic could identify the deceptive agent with 75.6% accuracy, human players achieved only 56.6%, indicating a gap in human performance.

Conclusion: The paper discusses the implications of this gap for using RogueAI as a data-collection tool, teaching tool, and evaluation harness for honesty-trained models.
...
Result: SkillCAT raises the average score over baselines by up to 40.40% on common agent benchmarks, including SpreadsheetBench, WikiTableQuestions, and DocVQA.

Conclusion: The proposed framework, SkillCAT, enables reliable skill evolution without model training, demonstrating improved performance and generalization capabilities.
...
TL;DR: A low-latency real-time audio game commentary system that generates spoken commentary directly from live gameplay video, reducing inter-utterance silence and improving speaking rhythm.


<details>
  <summary>Details</summary>
Motivation: The conventional pipelines for generating spoken commentary from live gameplay video have high latency due to sequential processing, causing long and unnatural silence between utterances.

Method: The proposed system runs text generation in parallel with speech playback and buffers multiple candidate utterances ahead of time, enabling immediate synthesis at playback boundaries.

Result: The parallel design reduces the mean inter-utterance silence from 9.6 seconds to 0.3 seconds and improves similarity to professional speaking--silence timing patterns by over 40 %. A user study with 120 experienced game players confirms significantly improved perceived speaking rhythm.

Conclusion: The proposed low-latency real-time audio game commentary system effectively reduces latency and improves speaking rhythm, making it suitable for fast-paced game videos.
...
TL;DR: The paper presents IVIE, a neuro-symbolic approach to generating interactive fiction worlds, balancing creative flexibility with narrative coherence.


<details>
  <summary>Details</summary>
Motivation: The tension between creative narratives and world coherence in Interactive Fiction, where Large Language Models (LLM) struggle with coherence and symbolic systems lack flexibility.

Method: IVIE uses a four-stage incremental generation pipeline, combining LLMs for creative decisions and symbolic validation for world state grounding.

Result: Human evaluation shows IVIE generates immersive, coherent worlds with high player engagement, successfully balancing flexibility and coherence.

Conclusion: The neuro-symbolic approach shows promise, but challenges remain, such as LLM inconsistencies and validation gaps, highlighting key design considerations for future systems.
...
Result: ProReviewer achieves the highest average score across five quality dimensions, outperforming prompt-based methods by up to 39% and the strongest fine-tuned baseline by 16%, and attains the highest win rates in human evaluation.

Conclusion: The proposed ProReviewer demonstrates the effectiveness of enabling LLM-based review agents to perform proactive investigation, leading to improved peer review quality.
...
TL;DR: A hybrid framework for detecting rumours in Algerian dialect social media content achieves an F1-score of 0.84.


<details>
  <summary>Details</summary>
Motivation: The spread of rumours on social media is a challenge, particularly in the Algerian context where dialectal content and limited annotated resources hinder standard Arabic NLP tools.

Method: A hybrid framework combining transformer embeddings with a classical classifier, using a domain-specific annotated dataset and a transliteration pipeline.

Result: The hybrid approach achieves an F1-score of 0.84, outperforming larger models trained on formal Arabic corpora.

Conclusion: The results demonstrate the feasibility of rumour detection in low-resource Algerian dialect settings, highlighting the importance of domain-specific pre-training.
...
TL;DR: This paper introduces a second-order method called Smooth Growth Bound Tensor (S-GBT) to improve the robustness of NLP models against word substitution attacks by bounding the Hessian element-wise and adding a regularization term during training.


<details>
  <summary>Details</summary>
Motivation: Despite recent progress in NLP, models remain vulnerable to word substitution attacks, and existing defenses focus on first-order sensitivity, ignoring the evolution of sensitivity described by curvature.

Method: The paper proposes a second-order method, S-GBT, which bounds the Hessian element-wise and adds a regularization term during training to minimize these bounds, and is derived for LSTM and CNN architectures.

Result: The results show that combining first and second-order regularization improves certified robust accuracy by up to 23.4% compared to prior methods, while clean accuracy remains competitive.

Conclusion: The findings indicate that controlling both the gradient and its variation is a promising direction for building more robust models.
...
TL;DR: The authors propose a new framework for automatic speech recognition (ASR) correction in long text-speech interleaved conversations, leveraging an ontology memory to store and retrieve contextual information.


<details>
  <summary>Details</summary>
Motivation: Traditional ASR correction methods focus on isolated utterances or short local contexts, but struggle with long interactions where contextual evidence is necessary.

Method: The authors propose an ontology memory-augmented ASR correction framework, which organizes preceding interaction history into a dynamically updatable ontology memory.

Result: Experiments on the RAMC-Corr dataset show that the proposed method improves over direct correction in 9 out of 10 paired backbone-setting combinations.
...
TL;DR: A method to filter noisy data for end-to-end speech-to-speech translation (S2ST) using a two-stage Rank-to-Distill strategy, resulting in improved translation performance.
...
Method: A two-stage Rank-to-Distill strategy, where a lightweight ranker generates pseudo-labels and trains an audio large language model to predict keep/drop decisions from raw paired speech.

Result: Experiments show consistent improvements over unfiltered training, yielding up to +1.4 ASR-BLEU for end-to-end S2ST on CVSS-C and SpeechMatrix.

Conclusion: The proposed method effectively filters noisy data for S2ST, leading to improved translation performance by capturing acoustic fidelity and cross-lingual semantic consistency.
...
TL;DR: The study evaluates the performance of dense retrievers in mixed-language queries, finding an optimal mixing ratio that outperforms monolingual queries in most cases.


<details>
  <summary>Details</summary>
Motivation: The sensitivity of dense retrievers to mixed-language queries in multilingual communities is not well understood.

Method: A ratio-controlled study on mMARCO using embedding-level mixing to construct mixed queries.

Result: An optimal mixing ratio outperforms the best monolingual endpoint in 88/105 cases, with asymmetry driven by English dominance.
...
TL;DR: ArogyaBodha and ArogyaSutra are proposed to improve multilingual medical reasoning in low-resource settings, particularly for Indic languages.


<details>
  <summary>Details</summary>
Motivation: The performance of Multimodal Large Language Models (MLLMs) in healthcare is limited in specialized settings, particularly in multilingual and low-resource scenarios.

Method: ArogyaBodha, a large-scale multilingual multimodal medical question-answer dataset, and ArogyaSutra, an actor-critic-based multi-agent framework, are introduced.

Result: Experiments show improved multilingual medical reasoning accuracy across all Indic languages.

Conclusion: The proposed dataset and framework enhance equitable access to AI-driven healthcare assistance, particularly in regions like rural India.
...
TL;DR: The paper presents a new approach to automate scientific experiments using AI and robotics. It introduces RoboGenesis, a simulation-based workflow and data engine, and LabVLA, a Vision-Language-Action model trained with a two-stage recipe. LabVLA achieves high success rates on the LabUtopia benchmark.


<details>
  <summary>Details</summary>
Motivation: The physical act of doing science remains largely outside the reach of AI systems. Existing Vision-Language-Action models are not suitable for scientific laboratories due to the complexity of the environment and the diversity of robot embodiments.

Method: The authors propose a two-stage approach: (1) building RoboGenesis, a simulation-based workflow and data engine to generate structured demonstrations across supported robot profiles; and (2) training LabVLA, a Vision-Language-Action model with a two-stage recipe: FAST action token pretraining and flow matching posttraining.
...
Conclusion: The paper presents a new approach to automate scientific experiments using AI and robotics, addressing the bottlenecks of data and embodiment. The proposed approach shows promising results on the LabUtopia benchmark.
...
TL;DR: This paper examines the vulnerability of search-augmented Large Language Models (LLMs) to fake product recommendations when consuming polluted web content. It introduces FORGE, a benchmark for measuring this vulnerability, and evaluates the effectiveness of various defenses.


<details>
  <summary>Details</summary>
Motivation: The increasing use of search-augmented LLMs in consumer recommendations creates a risk of these models promoting fake products by consuming polluted web content. The authors aim to investigate this risk and develop a benchmark to measure it.

Method: The authors introduce FORGE, a benchmark that simulates web-content pollution by rewriting real products in retrieved web pages into fake ones. They test 12 commercial and open-weight LLMs across 225 real-world products and 5 consumer scenarios.

Result: The results show that all LLMs are vulnerable to fake product recommendations, with a single polluted page yielding fooled rates of up to 27% and the full top-3 replacement raising this to 73.8%. Vulnerability varies across categories, and reasoning does not mitigate this vulnerability.

Conclusion: The authors conclude that search-augmented LLMs are vulnerable to promoting fake products when consuming polluted web content. They evaluate three defenses and find that skepticism prompting and consensus filtering have limitations in mitigating this vulnerability.
...
TL;DR: The paper proposes an adaptive token budgeting framework to improve efficiency in time series language modeling by compressing tokens and reducing prompt tokens, achieving up to 7.68x inference acceleration and performance gains in 78% of evaluated settings.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the inefficiency of uniform token processing in time series language modeling, which arises from the fundamentally different information structures of time series tokens and prompt tokens.

Method: The method involves developing an adaptive token budgeting framework that compresses time series tokens via frequency-domain structure and progressively reduces prompt tokens across layers.
...
Conclusion: The conclusion is that asymmetric token compression is effective for scalable time series foundation models, enabling more efficient and accurate time series analysis.
...
TL;DR: The paper compares speech representation methods for 3D facial animation, finding that phonetic class encoding improves accuracy, and proposes an Audio Visual Text-to-Speech (AVTTS) pipeline using discrete representations.
...
Method: The paper evaluates four speech representation families using two facial decoders, objective metrics, and perceptual evaluation, and conducts probing analyses to relate tokenized representations to phonetic units and articulatory deformations.

Result: Encoding phonetic classes is beneficial for accurate facial animation prediction, and the proposed AVTTS pipeline leverages discrete representations to decode speech and 3D facial motion.

Conclusion: The paper concludes that phonetic class encoding improves facial animation accuracy, and proposes a new AVTTS pipeline using discrete representations.
...
<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to explore the effectiveness of harness recursion, where a full agent harness is recursively spawned, and to improve long-context reasoning capabilities.

Method: The authors propose the Recursive Agent Harness (RAH), which involves a parent agent generating and running an executable script that spawns subagent harnesses in parallel for fine-grained workloads.

Result: The RAH achieves significant improvements on coding tasks, increasing the Codex coding-agent baseline from 71.75% to 81.36% on Oolong-Synthetic with GPT-5, and reaching 89.77% with Claude Sonnet 4.5.

Conclusion: The Recursive Agent Harness (RAH) demonstrates the effectiveness of harness recursion in improving long-context reasoning capabilities, achieving state-of-the-art results on coding tasks.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to address the lack of comprehensive text embedding benchmarks and efficient, locally-deployable models for Slovak, a low-resource language.

Method: The method involves introducing SkMTEB, evaluating 31 embedding models, and developing 	exttt{e5-sk-small} and 	exttt{e5-sk-large} models using vocabulary trimming and fine-tuning.

Result: The result shows that large instruction-tuned multilingual models achieve the strongest performance, and the developed models achieve competitive performance with proprietary APIs while being locally deployable.

Conclusion: The conclusion is that the approach offers a replicable path for other under-resourced languages, and the benchmark, models, datasets, and code are released openly.
...
TL;DR: The paper introduces Operadic Consistency (OC), a method to detect reasoning failures in Large Language Models (LLMs) without ground-truth labels. OC is strongly correlated with accuracy across various LLMs and datasets, and provides additional information beyond existing methods.
...
Result: OC is strongly correlated with accuracy across twelve instruction-tuned LLMs on four multi-hop QA datasets (Pearson r ∈ [0.86, 0.94]). OC provides additional information beyond Chain-of-thought self-consistency (CoT-SC) and semantic entropy. OC also yields selective-prediction improvements over a tuned CoT-SC baseline.

Conclusion: The paper concludes that OC is a useful diagnostic for detecting LLM reasoning failures and can be used to improve selective-prediction performance.
...
TL;DR: The authors introduce HyperTool, a unified executable tool interface that improves the efficiency and accuracy of tool-augmented LLM agents by changing the model-visible unit of tool execution.


<details>
  <summary>Details</summary>
Motivation: The current approach of tool-augmented LLM agents using step-wise atomic tool calls results in an execution-granularity mismatch, leading to inefficiencies and decreased accuracy.

Method: The authors propose HyperTool, a unified executable MCP-style tool interface that allows models to invoke tools with code blocks, manipulate returned values, and pass intermediate results locally.
...
Conclusion: The proposed HyperTool can substantially improve multi-step tool use in LLM agents, demonstrating its potential to enhance the efficiency and accuracy of tool-augmented models.
...
TL;DR: The paper proposes Influcoder, a fast and cost-effective method for data attribution in large language models, enabling efficient identification of training data samples that influence specific model outputs.


<details>
  <summary>Details</summary>
Motivation: The growing capabilities of Large Language Models (LLMs) have increased the need to curate high-quality datasets by filtering out problematic samples, and Data Attribution (DA) methods aim to identify which samples precondition a model to generate certain outputs.

Method: The authors propose Influcoder, a method for influence-based Data Attribution that is designed to be quick and cost-effective at scale, addressing the limitations of existing methods in terms of processing speed and storage compactness.

Result: The paper presents Influcoder as a solution for efficient data attribution, but does not provide specific results or empirical evaluations.

Conclusion: The paper concludes that Influcoder offers a practical approach to influence-based Data Attribution at scale, which is necessary for large language models.
...
TL;DR: The authors propose a new framework called Retrieval-Augmented Reinforcement Fine-Tuning (RA-RFT) to improve language models' ability to reason by analogy, and demonstrate its effectiveness on mathematical reasoning benchmarks.


<details>
  <summary>Details</summary>
Motivation: Conventional retrieval methods based on lexical or semantic similarity are not well-suited for complex reasoning tasks, as they do not take into account the underlying reasoning patterns.
...
Result: RA-RFT consistently outperforms standard reinforcement fine-tuning methods on challenging mathematical reasoning benchmarks, with improvements of 7.1 and 2.8 points over GRPO for Qwen3-1.7B and Qwen3-4B respectively.

Conclusion: The authors conclude that reasoning-aware retrieval is a complementary axis of improvement for language models, orthogonal to advances in reward design or training curricula, and that RA-RFT is an effective framework for improving language models' ability to reason by analogy.
...
<details>
  <summary>Details</summary>
Motivation: Current evaluations of large language model agents assume static environments, but real-world deployment requires agents to adapt to changing environments and updated task conditions.

Method: The authors propose EvoArena, a benchmark suite that models environment changes as sequences of progressive updates, and EvoMem, a memory paradigm that records memory evolution as structured update histories.

Result: Experiments show that current agents struggle on EvoArena, but EvoMem improves performance by 1.5% on EvoArena and 6.1% and 4.8% on standard benchmarks GAIA and LoCoMo, respectively.

Conclusion: The results highlight the importance of modeling evolution in both evaluation and memory for reliable agent deployment, and EvoMem is a step towards more robust and adaptable agents.
...
TL;DR: The paper introduces ToolSense, a diagnostic framework for evaluating large language models' tool retrieval capabilities, revealing a knowledge-retrieval dissociation in parametric models.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the tool-retrieval bottleneck in large language models by evaluating their ability to understand and retrieve tools accurately.

Method: The authors introduce ToolSense, an open-source framework that generates three benchmarks: Realistic Retrieval Benchmark (RRB), MCQ probing benchmark, and QA probing benchmark, and evaluate five parametric model training configurations.

Result: The results show a knowledge-retrieval dissociation, with some models performing well on retrieval tasks but poorly on factual probes, and some configurations collapsing by ~50-64 percentage points on RRB queries compared to fully-specified benchmarks.

Conclusion: The paper concludes that ToolSense is a useful framework for diagnosing tool retrieval capabilities and that parametric models may not truly understand their tools, highlighting the need for further improvement.
...
TL;DR: Arbor is a multi-agent framework for autonomous optimization in large action spaces. It uses a structured tree search cognition layer and achieves significant performance improvements in LLM inference optimization.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the limitations of prior autonomous optimization systems, which operate on isolated targets with stateless evaluation. The authors aim to create a framework that can handle large, stateful action spaces and improve performance in complex domains like LLM inference optimization.

Method: The method used in this paper is a multi-agent framework called Arbor, which introduces a structured tree search as a cognition layer. It consists of an Orchestrator agent and a Critic agent that work together to drive optimization and ensure stability.

Result: The results show that Arbor achieves up to 193% inference throughput-latency Pareto improvement over vendor-optimized baselines. It also demonstrates hardware-agnostic and reproducible performance across multiple generations of hardware platforms.

Conclusion: The conclusion of this paper is that Arbor is an effective multi-agent framework for autonomous optimization in large action spaces. It can achieve significant performance improvements in complex domains like LLM inference optimization and is hardware-agnostic and reproducible.
...
TL;DR: The paper proposes a framework for strategic decision support for AI agents, optimizing support usage while controlling errors.


<details>
  <summary>Details</summary>
Motivation: The increasing use of AI agents in decision-making raises concerns about reliability and alignment with human goals. The authors aim to revisit the principles of decision support in this new context.

Method: The authors propose an optimization problem that minimizes support usage subject to controlling a counterfactual missed-support error. They develop an online algorithm with adaptive thresholding and randomized exploration.

Result: The experiments show that the proposed method reliably controls the target error while substantially reducing support usage in practice.

Conclusion: The paper presents a framework for strategic decision support for AI agents, providing a new approach to balance support usage and error control in AI-driven decision-making.
...
TL;DR: The paper introduces Pythagoras-Prover, a family of efficient Lean theorem provers that achieve strong performance with less compute and data, outperforming larger models.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the efficiency of Lean theorem provers, which currently require substantial training and inference compute due to scarce verified proof data and long reasoning traces.

Method: The authors introduce two generation paradigms: autoregressive models (4B and 32B parameters) and a diffusion-based prover (4B). They also propose a Lean-verified corpus for curriculum SFT, a dynamic proof-reasoning filtering scheme, and Augmented Lean Formalisation (ALF) for expanding verified corpora.

Result: Pythagoras-Prover-4B surpasses DeepSeek-Prover-V2-671B at pass@32 on MiniF2F-Test (86.1% vs 82.4%) with ~167x fewer parameters. Pythagoras-Prover-32B sets the open-source state of the art at 93.0% on MiniF2F-Test and solves 93 of 672 PutnamBench problems.

Conclusion: The paper concludes that Pythagoras-Prover achieves strong performance while being compute-efficient, and that the proposed methods (ALF, curriculum SFT, and dynamic filtering scheme) are effective in improving the efficiency of Lean theorem provers.
...
TL;DR: PersonaDrive is a pipeline for generating style-diverse non-ego traffic agents in closed-loop driving simulators by conditioning a vision-language-action agent on retrieved demonstrations from style-instructed human driving data.


<details>
  <summary>Details</summary>
Motivation: Current methods for generating non-ego traffic agents in driving simulators produce agents that behave in a limited way, lacking style variation. Recent approaches use proxies for style, but not direct human demonstrations.

Method: The PersonaDrive pipeline consists of three stages: offline triplet mining, training a lightweight retrieval head, and fine-tuning a vision-language-action backbone. The pipeline allows for style conditioning by swapping per-style databases.

Result: PersonaDrive improves driving scores over baselines, with a 4.6% improvement over SimLingo and 2.5% over HiP-AD without style conditioning. With style conditioning, it achieves the highest driving score in every style, with a 5.4% improvement over the strongest baseline in its weakest style.

Conclusion: The PersonaDrive pipeline enables the generation of human-style, style-diverse non-ego agents for closed-loop simulation, allowing for more realistic and varied driving scenarios.
...
<details>
  <summary>Details</summary>
Motivation: To enable auditing, monitoring, and investigation of language model behavior by developing robust lie detectors.
...
Result: Current lie detectors show positive scaling with model capability on prompted lying but drop sharply on trained model organisms. The chain-of-thought judge performs best with 0.82 balanced accuracy.

Conclusion: Current lie detectors have limitations and cannot support high-confidence claims about model beliefs, highlighting the need for future research to address these limitations.
...
TL;DR: TrajGenAgent, a hierarchical LLM-agent framework, generates realistic human mobility trajectories without fine-tuning, improving spatiotemporal fidelity and semantic coherence.


<details>
  <summary>Details</summary>
Motivation: Large-scale trajectory collection is costly and privacy-constrained, and existing LLM-based generators have limitations in fine-grained spatiotemporal grounding or incur substantial computational cost.

Method: TrajGenAgent uses a two-stage design: an LLM synthesizes an activity chain via in-context learning, and a deterministic workflow grounds each activity into a complete visit.

Result: TrajGenAgent improves spatiotemporal fidelity, semantic coherence, and individual-specific behavioral realism over representative baselines without parameter updates.

Conclusion: TrajGenAgent offers a promising approach for realistic synthetic trajectory generation, addressing the limitations of existing methods and the challenges of human mobility data collection.
...
TL;DR: Evoflux, an inference-time evolutionary search method, improves compact language models' tool use by repairing executable tool workflows, increasing execution feasibility from 3% to 17-24%.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the limitations of small planners in handling tool use, which often fail due to tool resolution, parameter validation, dependency tracking, or execution.

Method: The authors introduce Evoflux, an evolutionary search method that evolves typed workflow graphs through structured edits, execution feedback, adaptive intensity, meta-guided redesign, and diversity pruning.

Result: Evoflux raises execution feasibility from roughly 3% to 17-24% across small planners, outperforming SFT and SFT+DPO, and showing more reliable performance under scarce teacher-trace budgets.

Conclusion: The authors conclude that execution-grounded search is more reliable under scarce teacher-trace budgets, and Evoflux is an effective method for improving compact language models' tool use.
...
TL;DR: The report explores the development of artificial general intelligence (AGI) and its potential progression to artificial general superintelligence (ASI), discussing pathways, frictions, and societal implications.


<details>
  <summary>Details</summary>
Motivation: The pursuit of human-level artificial general intelligence (AGI) and its potential impacts on society motivates the investigation into the development of AGI and its progression to artificial general superintelligence (ASI).

Method: The report characterizes ASI, discusses four potential pathways from AGI to ASI (scaling AGI, AI paradigm shifts, recursive improvement, and ASI emerging from large-scale multi-agent collectives), and examines possible frictions and bottlenecks along these pathways.

Result: The report concludes that the development of AGI and its progression to ASI could lead to a series of transformative societal changes caused by AI-enabled progress and breakthroughs across many areas of science and technology, rather than a single transformative step change.

Conclusion: Preparing for the potential progression of AGI to ASI requires a massively interdisciplinary endeavour of global scope and interest, as the impact of AI progress on society is uncertain and may continue to accelerate over the next years.
...
TL;DR: This study evaluates the real-world performance of a large language model (LLM) integrated into clinical systems, focusing on user acceptance and feedback. It proposes a pre-response classifier that predicts user rejection based on query content and deployment-specific context, achieving an AUROC of 0.719.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of static benchmarks in evaluating clinical systems by performing a deployment-centered evaluation of an LLM system embedded within electronic health records.

Method: The method involves training a pre-response classifier using query content and deployment-specific context to predict the risk of user rejection. A prospective analysis is conducted over 4.5 months of user feedback.

Result: The prediction model achieves an AUROC of 0.719. The study demonstrates the feasibility of predicting user rejection using deployment-specific context, which improves the ability to predict user rejection.

Conclusion: The study concludes that making use of deployment-specific context improves the ability to predict whether the user will reject the system output, opening the door to targeted guardrails.
...
TL;DR: The paper addresses the lack of a unified definition for Artificial General Intelligence (AGI) and proposes a framework, DAF-AGI, for evaluating and governing AGI definitions.


<details>
  <summary>Details</summary>
Motivation: The motivation is to tackle the under-specification of AGI as a design and governance problem, given that different operationalizations of AGI can lead to conflicting conclusions about its existence.

Method: The paper uses Design Science Research Methodology to develop DAF-AGI, which consists of ordinal criteria for assessing definitions and a governance audit framework.

Result: The application of DAF-AGI to various measurement families shows that claims about the arrival of AGI can be certified only under specific operationalizations, and the paper highlights the need for definitional sovereignty in algorithmic sovereignty.

Conclusion: The paper concludes by proposing a novel integration and operationalization of AGI definitions, emphasizing that independent application, inter-rater testing, and external cases are necessary for validation.
...
TL;DR: The Theory of Mind Utility (ToM-U) is a computational model for inferring others' beliefs by tracking their information sources and credibility.


<details>
  <summary>Details</summary>
Motivation: To develop a formal framework for understanding how people infer others' mental states, specifically their beliefs and knowledge.

Method: ToM-U constructs Local Epistemic World Models (LEWMs) to represent agents, their knowledge, and the relationships between them, and evaluates these models against observed behavior.

Result: The ToM-U model provides a domain-agnostic mechanism for mentalizing, generating testable predictions about mentalizing failures, and positioning itself as a precursor to goal inference and other social cognitive processes.

Conclusion: The Theory of Mind Utility (ToM-U) offers a novel approach to understanding mental state inference, providing a formal apparatus for epistemic state inference that differs from existing accounts.
...
TL;DR: The study examines the reliability of self-reports in predicting the behavior of Large Language Models (LLMs), comparing the Big 5 personality traits with the Theory of Planned Behavior (TPB).


<details>
  <summary>Details</summary>
Motivation: The motivation is to investigate whether self-reports reliably predict LLM behavior, which is critical for their safe deployment.

Method: The study conducts experiments across four behavioral tasks and 11 frontier LLMs, using both Big 5 personality traits and TPB to measure self-reports and comparing them with actual behavior under various conditions.

Result: The results show that self-report and behavior coherence exists but is selective: TPB achieves human-level coherence within a shared conversation, but coherence drops across separate conversations, especially for context-primed behaviors.

Conclusion: The study concludes that coarse personality frameworks like Big 5 may not be suitable for testing deployment behavior; instead, more task- and behavior-specific instruments like TPB are needed, and these must be evaluated across tasks and contexts.
...
TL;DR: The paper introduces SciAgentArena, a benchmark for evaluating AI agents in real-world scientific research scenarios, and assesses their capabilities in various tasks.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the lack of understanding of AI agents' practical capabilities in real research settings and to provide a systematic benchmark for evaluating their performance.

Method: The authors introduce SciAgentArena, a benchmark comprising approximately 200 tasks with stepwise verification and an interactive environment for assessing diverse AI agents.

Result: Current AI agents can contribute effectively to well-specified data-analysis workflows but struggle with generating novel insights, sustaining self-directed exploration, and formulating robust solutions for open-ended research questions.

Conclusion: The paper concludes that SciAgentArena provides a practical framework for measuring progress in AI agents for science and guiding the design of future agents capable of addressing complex scientific challenges.
...
TL;DR: The paper explores deploying state-of-the-art DNN models in resource-constrained wearable devices for EEG signal analysis, specifically for detecting epileptic seizures, by investigating the trade-off between accuracy and computational complexity using parameter quantization and electrode reduction methods.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome the limitations of deploying deep learning models in wearable devices due to tight energy and computational power constraints, while still maintaining high accuracy in analyzing biological signals like EEG.

Method: The paper uses parameter quantization and electrode reduction methods to reduce the computational complexity of DNN models designed for EEG signal analysis, and investigates their impact on accuracy.
...
Conclusion: The paper concludes that judicious application of parameter quantization and electrode reduction methods can enable the deployment of DNN-based online EEG analysis in wearable devices, with a feasible trade-off between accuracy and computational complexity.
...
TL;DR: Researchers investigate whether advanced language models can detect and respond to manipulated or 'prefilled' outputs, a capability termed 'prefill awareness', and find that these models exhibit substantial prefill awareness.


<details>
  <summary>Details</summary>
Motivation: The study aims to assess the validity of safety-relevant methods that rely on prefilling model outputs, given that models may recognize and act upon manipulated prior assistant messages.

Method: The researchers construct a binary preference benchmark across three prefill mechanisms and evaluate frontier language models, including Claude Opus 4.5, for prefill awareness.

Result: The study finds that frontier models show substantial prefill awareness, detecting prefills opposing their preferences in 9-35% of cases with a 0% false positive rate, and often reverting to baseline behavior without explicitly reporting the prefill as foreign.

Conclusion: The results indicate that prefill awareness is a significant confound for some prefill-based methods, and it is recommended that model developers track this capability in frontier systems.
...
TL;DR: The study evaluates the effectiveness of different question generation strategies for AI-supported learning systems, focusing on procedural and multi-hop reasoning.


<details>
  <summary>Details</summary>
Motivation: The need for high-quality question-answer datasets that are both learner-like and grounded in instructional knowledge for evaluating procedural reasoning in AI-supported learning systems.

Method: The study compares three TMK-based question generation strategies: strict generation from TMK models, transcript-first generation with post-hoc TMK filtering, and TMK-aware generation. A grounding validation framework is used to evaluate generated items.

Result: Strict TMK generation achieves the strongest overall quality with 96.5% grounded questions and 92.6% usable questions. Transcript-first generation produces more learner-like questions but with lower grounding, while TMK-aware generation yields high raw multi-hop coverage but lower grounding.
...
TL;DR: The paper discusses world modeling for intelligent systems, focusing on explicit and implicit models, their applications, and challenges in achieving artificial general intelligence.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to provide a unified framework for understanding and advancing world modeling approaches in AI, particularly for applications like robotics and autonomous driving.

Method: The paper presents a tutorial framework that unifies diverse world modeling approaches through shared predictive structure, differentiating them by representation and exploitation methods.

Result: The paper outlines a coherent framework for world modeling, distinguishing between explicit and implicit models, and highlights challenges in hierarchical reasoning, long-horizon planning, and autonomous goal formation.

Conclusion: The conclusion is that while significant progress has been made in world modeling, substantial challenges remain in achieving artificial general intelligence, particularly in areas requiring advanced reasoning and planning capabilities.
...
TL;DR: The paper examines the safety guarantees of popular frameworks used to build autonomous large language model systems. It finds that these frameworks lack native compliance with basic safety principles, making them vulnerable to attacks. The authors propose lightweight containment mechanisms to address these vulnerabilities.


<details>
  <summary>Details</summary>
Motivation: The increasing deployment of autonomous large language model systems in public-facing domains raises concerns about their safety and security. The authors aim to investigate whether the frameworks used to build these systems provide adequate safety guarantees.

Method: The authors derive six containment principles from a compositional model of agentic architectures and audit three dominant frameworks (LangChain, AutoGPT, and OpenAI Agents SDK) for compliance. They also conduct empirical validation through simulations and propose lightweight containment mechanisms.

Result: The study finds that none of the evaluated frameworks provide native compliance with basic safety principles, such as memory integrity. A simulated attack on a government benefits agent built on LangChain demonstrates the vulnerability of these systems to memory-poisoning attacks. The proposed containment mechanisms effectively eliminate these vulnerabilities with minimal overhead.

Conclusion: The current ecosystem of agentic frameworks may not meet the secure-by-default expectations for public-facing deployments. The authors outline priority architectural interventions to enable trustworthy deployment in high-stakes, socially impactful applications.
...
TL;DR: The paper introduces MLUBench, a large-scale benchmark for multimodal large language model (MLLM) lifelong unlearning, and proposes LUMoE, a method to alleviate the degradation issue in existing unlearning methods.


<details>
  <summary>Details</summary>
Motivation: The need for data unlearning in MLLMs as data owners request removal of specific content, and the challenge of MLLM lifelong unlearning with sequential requests over time.

Method: The paper proposes LUMoE, a method to mitigate the degradation issue in existing unlearning methods, and evaluates it using the MLUBench benchmark.
...
Conclusion: The paper highlights the importance of MLLM lifelong unlearning, introduces MLUBench as a comprehensive benchmark, and proposes LUMoE as an effective method to alleviate the degradation issue.
...
TL;DR: Teach VLM enables models to understand mobile device UI actions and convert visual state transitions into operational knowledge, outperforming existing models.
...
TL;DR: The paper introduces the GeoNatureAgent Benchmark, a new benchmark for evaluating AI agents in environmental analysis tasks using structured tool calls to a real API. It assesses seven large language models (LLMs) across 93 tasks and finds Claude Sonnet 4 to be the most capable, while also highlighting the cost-effectiveness of open-weight models and the challenges of comparison tasks.


<details>
  <summary>Details</summary>
Motivation: Environmental scientists spend too much time on data wrangling rather than analysis, and existing AI agents for automating geospatial workflows lack validation. The authors aim to address this by creating a benchmark that evaluates agents operating through structured tool calls against real APIs.

Method: The authors introduce the GeoNatureAgent Benchmark, which consists of 93 tasks across 18 categories, and evaluate seven LLMs (Claude Sonnet 4, DeepSeek V3.2, GLM-5, Gemini 2.5 Pro, Qwen3-235B, GPT-OSS-120B, Llama 4 Scout) using three temperature-1.0 seeds. Tasks are assessed against an open, self-hostable API serving three environmental indicators across Spain and Portugal via sixteen tools.

Result: The results show that (1) Claude Sonnet 4 achieves the highest capability at 60.8% +/- 0.8%, followed by DeepSeek V3.2 at 56.3% +/- 3.1%; (2) open-weight models, particularly DeepSeek V3.2, offer a cost-accuracy Pareto frontier, with 93% of Claude's capability at 11x lower cost ($0.011/case); (3) comparison tasks remain unsolved (0% on close-value comparisons); and (4) structured tool calls against a real API are more discriminative than general-purpose GIS benchmarks, with accuracies 25-35 points lower.

Conclusion: The paper concludes that the GeoNatureAgent Benchmark provides a valuable tool for evaluating AI agents in environmental analysis tasks. The results highlight the strengths and limitations of current LLMs and demonstrate the importance of considering cost-effectiveness and task-specific challenges.
...
TL;DR: AI research topics advance through abrupt, detectable jumps, not gradual growth, with large language models and diffusion models surging across venues within years.
...
Result: Major AI topics advance through topical phase transitions, with large language models and diffusion models rising abruptly, and an early-warning signature achieving a precision of 27% and recall of 63% in detecting transitions.

Conclusion: AI research reorganizes through abrupt, detectable jumps, and the proposed early-warning signature can identify topics to monitor for future transitions.
...
TL;DR: The authors propose AgentBuild, a system for constructing scientific agents from contracts authored by scientists, allowing for more controlled and transparent development of LLM-based agents.


<details>
  <summary>Details</summary>
Motivation: The shift to LLM-based agents in scientific workflows can bury the scientist's judgment, and the authors aim to introduce a more controlled and transparent development practice.

Method: The authors propose treating agent construction as a workflow stage and introduce AgentBuild, which builds a scientific agent from a contract authored by the scientist. The contract includes a version-controlled rubric, a difficulty-graded curriculum, and a curated external knowledge base.

Result: The authors instantiate AgentBuild for Rietveld refinement of X-ray diffraction data and demonstrate its effectiveness in reaching a frontier case and exposing workflow-scope limits.

Conclusion: The authors conclude that AgentBuild allows for more controlled and transparent development of LLM-based agents, and that re-running AgentBuild with evolving base models is a re-tune, not a rebuild, with the scientist's authored contract remaining the durable asset.
...
TL;DR: The paper discusses the reliability of AI-assisted research and proposes a decision architecture called Human-in-the-Loop Economic Research (HLER) to improve it.


<details>
  <summary>Details</summary>
Motivation: The authors argue that the reliability of AI-assisted research depends not only on model capability, but also on how cognitive labour is structured between humans and machines.
...
Result: HLER reduced the failure rate from 72% to 16% and Fisher's exact test rejects equality of failure rates at p<0.001.

Conclusion: The authors conclude that HLER is a research harness that sharply reduces failures, makes residual weaknesses more visible, and prevents unreliable claims from being advanced as publication-ready outputs.
...
TL;DR: The paper proposes WISE, a framework for long-horizon embodied agents that enhances low-level control with episodic memory and causal reasoning, leading to improved task success and efficiency.


<details>
  <summary>Details</summary>
Motivation: The authors argue that current general-purpose embodied agents are limited by the lack of episodic memory and decoupling of 'what-where-when' memory from 'which-why' reasoning, leading to performance bottlenecks.

Method: The authors propose WISE, which includes a Causal Event Graph to augment episodic memory, an Opportunistic Task Scheduler for dynamic subtask re-prioritization, and a multi-scale progressive exploration strategy.

Result: Experiments show that WISE improves task success and efficiency on long-horizon sparse tasks, particularly in settings requiring adaptive decision-making.

Conclusion: The proposed WISE framework addresses the limitations of current embodied agents by integrating episodic memory and causal reasoning, leading to enhanced performance in complex tasks.
...
TL;DR: Introduced DailyReport, a benchmark for evaluating Search Agents on daily search tasks, showing current systems fall short of user expectations.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for Search Agents focus on specialized tasks and have limited evaluation interpretability, not reflecting real-world user scenarios.

Method: DailyReport includes 150 open-ended tasks with 3,546 rubrics, evaluating Search Agents with cascade rubrics across multiple dimensions.

Result: Current Search Agent systems fall short of users' expectations, with detailed performance insights provided by the DailyReport benchmark.

Conclusion: DailyReport provides a more realistic and interpretable evaluation of Search Agents, highlighting the need for improvement in current systems.
...
TL;DR: The authors propose HarnessBridge, a learnable plug-in module that generates a harness for large language models to interact with environments more efficiently.
...
Conclusion: The authors conclude that HarnessBridge is a viable solution for generating efficient harnesses for large language models, enabling more scalable and effective agent-environment interactions.
...
<details>
  <summary>Details</summary>
Motivation: The scaling factor α in LoRA is underappreciated and its role in optimization is not well understood, leading to a need for investigation.

Method: The paper uses extensive empirical analysis and a theoretical Signal-Drift framework to study LoRA's scaling mechanism.

Result: The study reveals three key findings about LoRA's scaling mechanism and proposes LoRA-α, which consistently improves performance across diverse tasks.

Conclusion: The paper concludes that α plays a crucial role in LoRA optimization, and LoRA-α offers a more effective and efficient approach to hyperparameter tuning.
...
TL;DR: Proposed a method called Human-like Criteria Probing for Hallucination Detection (HCPD) to detect hallucinations in large language models (LLMs) without relying on model internals or external references.


<details>
  <summary>Details</summary>
Motivation: Detecting hallucinations in LLMs is challenging, especially under the zero-source constraint where no model internals or external references are available.
...
Result: HCPD consistently outperforms state-of-the-art baselines, offering an effective and explainable solution for zero-source hallucination detection.

Conclusion: The proposed approach provides a reliable and interpretable solution for detecting hallucinations in LLMs without requiring model internals or external references.
...
TL;DR: The authors developed MDForge, an LLM agent that automates the design of molecular dynamics pipelines, and demonstrated its effectiveness in designing competitive pipelines for host-guest binding free-energy benchmarks and discovering a novel high-affinity binder.


<details>
  <summary>Details</summary>
Motivation: Designing a molecular dynamics (MD) pipeline for a new system requires substantial expert knowledge, and running it on even one molecule is expensive, making trial-and-error approaches infeasible.

Method: The authors used an LLM agent, MDForge, which treats pipeline design as open-ended code generation, and its behavior is reshaped online by verbal reward through a multi-agent debate among physics experts.

Result: MDForge automatically designed MD pipelines competitive with human experts on three SAMPL host-guest binding free-energy benchmarks and discovered a novel high-affinity, picomolar CB[7] binder.

Conclusion: The study demonstrates the potential of LLM agents in automating the design of molecular dynamics pipelines, which can accelerate the discovery of novel binders and reduce the need for human expertise.
...
TL;DR: The paper presents a simulation framework for evaluating conversational shopping assistant architectures, comparing different responder designs and finding that rolling-window memory outperforms intent-extraction memory, and that swapping LLM backbones can affect performance.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to evaluate conversational shopping assistant architectures using a modular two-agent simulation framework, allowing for controlled comparisons of different responder designs.

Method: The method used in this paper is a simulation framework with an independent buyer agent and an interchangeable responder that integrates with a real e-commerce search API, testing across 2011 conversations and 14 persona buckets.

Result: The results of this paper include four empirical findings: rolling-window memory outperforms intent-extraction memory, targeted fixes can reduce failure rates, swapping LLM backbones can affect performance, and there is systematic philosophical disagreement between LLM judges.

Conclusion: The conclusion of this paper is that the simulation framework provides a controlled way to compare conversational shopping assistant architectures, and that the findings can inform the design and improvement of such systems.
...
TL;DR: The paper introduces MARS, a method to improve the efficiency of parallel test-time scaling for LLMs by stopping the generation of reasoning traces early while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: The motivation is to reduce the substantial computational overhead of running multiple reasoning traces to completion in parallel test-time scaling while improving LLM accuracy.

Method: The method introduced is MARS (Margin-Adversarial Stopping Rule), which estimates which active traces are likely to change their answers and stops once the leader remains safe under a conservative bound on future vote movement.

Result: MARS saves 25-47% of self-consistency tokens and 14-29% on top of DeepConf Online, a strong confidence-weighted baseline, while matching the accuracy of the corresponding full-budget baselines across three reasoning models and three competition-math benchmarks.

Conclusion: The paper concludes that MARS effectively reduces computational overhead while maintaining accuracy in parallel test-time scaling for LLMs.
...
TL;DR: The paper proposes PRISMR, a framework that improves generative listwise ranking with Large Multimodal Models (LMMs) by replacing transient in-context list processing with parametric structural conditioning, addressing the issue of parse collapse.
...
Conclusion: The proposed PRISMR framework effectively addresses the issue of parse collapse in generative listwise ranking with LMMs, leading to improved ranking performance and transferability.
...
TL;DR: The paper proposes a multi-factor memory value function for managing long-running LLM agents' interaction histories, which learns to prioritize what to encode, forget, and retrieve under a fixed memory budget.


<details>
  <summary>Details</summary>
Motivation: Long-running LLM agents face a challenge in managing large interaction histories, and current methods (semantic similarity or recency) are mis-specified for the forgetting decision.

Method: The authors propose a multi-factor memory value function V(m) with seven interpretable factors, whose weights are learned from a downstream objective by a gradient-free optimiser.

Result: The learned multi-factor value function retains 0.770 ± 0.011 of gold evidence across 479 usable cases, outperforming uniform weights (0.657), the best single factor (0.518), and recency (0.368).

Conclusion: The proposed approach provides an effective and interpretable way to manage long-running LLM agents' interaction histories, with learned weights that dominate reliability, emotional intensity, and self/user relevance.
...
<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to develop a medical vision-language model that can effectively leverage large-scale pretraining data to improve performance on various medical vision-language tasks.

Method: The authors pretrain OpenMedQ on a large dataset of 14 datasets totaling ~3.35M samples, covering pathology, radiology, microscopy, and text-only clinical QA. They then fine-tune the model on specific downstream tasks.

Result: OpenMedQ achieves state-of-the-art BLEU-1 scores on PathVQA (75.9) and VQA-MED (64.5), outperforming Med-PaLM M variants with up to 80x more parameters. The model's vision encoder also achieves the highest average macro-F1 score (0.757) on 8 unseen medical classification benchmarks.

Conclusion: The authors conclude that OpenMedQ is a highly effective medical vision-language model that can be used as a reproducible baseline for the community, and that their approach can be used to develop more accurate and efficient models for medical vision-language understanding.
...
<details>
  <summary>Details</summary>
Motivation: Current methods for sharing mathematical content in online forums are error-prone, inconvenient, or lack integration, causing friction for students and educators.
...
Result: A unified system that supports desktop and mobile clients, with a provisional US patent application filed, and potential to create a community-validated dataset for training AI systems.

Conclusion: The proposed system closes the practical gap in sharing mathematical content in online forums, and has potential long-term benefits for training and benchmarking AI systems.
...
TL;DR: The paper presents STG, a framework for generating deterministic testbenches for hardware designs, which outperforms existing LLM-based methods in speed, coverage, and energy efficiency.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the bottleneck in large language model (LLM)-driven Register Transfer Level (RTL) workflows, where testbench generation is a critical but time-consuming and unreliable process.

Method: The authors propose a Structured Testbench Generation (STG) framework that leverages the inherent structure of hardware designs to generate deterministic testbenches.

Result: STG achieves 720x faster testbench generation, higher coverage, and lower false-pass verdicts compared to LLM-based methods. It also reduces energy consumption by 127x and node count by 14-47%.

Conclusion: The paper concludes that STG is a more efficient, reliable, and effective framework for testbench generation, which can also help identify errors in RTL generation benchmarks and improve the performance of distilled models.
...
TL;DR: A new framework, APCyc, is proposed for de novo design of cyclic peptides, which improves stability and binding affinity by optimizing multiple physicochemical properties.


<details>
  <summary>Details</summary>
Motivation: The de novo design of cyclic peptides is challenging due to the need to identify pocket-adaptive cyclization patterns and linkage sites while controlling drug-relevant properties.

Method: APCyc, a target-aware de novo cyclic peptide generation framework, uses an expanded residue vocabulary and explicitly encodes cyclization-site and linkage-type information to learn cyclization-aware representations.
...
Conclusion: APCyc represents a promising approach for the de novo design of cyclic peptides, offering improved stability and binding affinity by optimizing multiple physicochemical properties.
...
TL;DR: The paper challenges the prevailing wisdom that Multi-Agent Systems (MAS) are superior to Single-Agent Systems (SAS), finding that automatically generated MAS underperform SAS and are more expensive.


<details>
  <summary>Details</summary>
Motivation: The authors aim to rigorously evaluate the advantages of MAS over SAS, specifically focusing on automatically generated MAS, and to identify potential gaps and inefficiencies in current evaluation frameworks.

Method: The authors perform a systematic evaluation of MAS against SAS, using benchmarks such as Chain-of-Thought with Self-Consistency (CoT-SC) and introducing a diagnostic synthetic dataset to isolate failures from task structure limitations.

Result: Automatically generated MAS consistently underperform CoT-SC despite being up to 10x more expensive, and expert-architected MAS outperform automatically generated architectures in both raw performance and cost-efficiency on a synthetic dataset.

Conclusion: The paper concludes that existing evaluation frameworks mask critical architectural gaps and inefficiencies of complex MAS, and that current automated design paradigms produce architectural bloat that prioritizes superficial complexity over functional utility.
...
TL;DR: The paper presents Otters++, a method for efficient and robust time-to-first-spike (TTFS) computing in spiking neural networks (SNNs) using optoelectronic devices, achieving improved performance on the GLUE dataset while maintaining energy efficiency.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the issue of reduced benefits of TTFS coding in SNNs due to the cost of computing temporal decay terms and multiplying them by synaptic weights, and to develop an efficient and robust method for TTFS computing.

Method: The method involves using the natural signal decay in optoelectronic devices to directly realize the TTFS temporal term, and developing a hybrid training method that combines device-faithful SNN computation with quantized neural network (QNN) straight-through gradients and model distillation.

Result: The result is that Otters++ improves the average score on the GLUE dataset to 84.17% while maintaining a clear energy advantage over prior spiking Transformer baselines.

Conclusion: The conclusion is that physically grounded TTFS computing can be efficient, trainable, and robust under realistic hardware effects, and that Otters++ is a promising approach for energy-efficient inference in SNNs.
...
TL;DR: The paper introduces SciR, a new benchmark for evaluating large language models (LLMs) on scientific reasoning tasks, specifically focusing on deduction, induction, and causal abduction.
...
Result: The results show that both extraction and inference difficulty axes negatively impact model performance, with compounding effects, and that reasoning models generally surpass non-reasoning models on the inference axis.
...
TL;DR: The paper explores recovering human cognitive diversity from behavior and transferring it to LLM agents to mitigate correlated errors in prediction markets.


<details>
  <summary>Details</summary>
Motivation: The proliferation of LLM agents in prediction markets risks creating a cognitive monoculture due to correlated forecasts from agents built on shared foundation models.

Method: The authors extract a structured behavioral profile from real trading activity on Polymarket and inject it into LLM agents through prompts to test if human cognitive diversity can be transferred.

Result: The extraction of behavioral profiles from human activity shows partial success, with 8 of 14 parameters being temporally stable and wallets identifiable from their profiles. However, injecting these profiles into LLM agents through prompts does not measurably transmit the diversity, with no significant advantage over a control in terms of ensemble error correlation or Brier score.

Conclusion: The study concludes that while human cognitive diversity can be partially recovered from behavior, transferring this diversity to LLM agents through prompt-level injection does not work, suggesting the need for deeper, below-the-prompt interventions like fine-tuning or activation steering.
...
TL;DR: This paper explores multispectral CNN-based object detection using visible and thermal infrared images from different sensors, focusing on the impact of varying data on classification accuracy.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve object detection in intelligent video surveillance by combining visible and thermal infrared images, addressing challenges such as varying illumination, sensor specialties, and limited thermal infrared datasets.

Method: The method involves investigating the suitability and robustness of different augmentation techniques for Convolutional Neural Networks (CNNs) using multispectral data from visible and thermal infrared cameras.

Result: The paper aims to provide insight into how CNNs make decisions and what they learn from different sensor input data, but does not report specific results.

Conclusion: The conclusion is expected to discuss the effectiveness of multispectral CNN-based object detection and the impact of different augmentation techniques on classification accuracy, but is not explicitly stated in the abstract.
...
TL;DR: A manually annotated corpus of 115 abstracts for autoimmunity research, AAbAAC, was created and used to improve named entity recognition (NER) performance in this specialized domain.


<details>
  <summary>Details</summary>
Motivation: Performance gaps in information extraction exist in highly specialized biomedical fields like autoimmunity, and generalist models struggle with domain-specific complexity.

Method: The AAbAAC corpus was created by manually annotating entities and their relationships in 115 abstracts from PubMed, and then used to evaluate and fine-tune NER models.

Result: The study showed an expected improvement in NER performance after fine-tuning, demonstrating the utility of small-scale annotation efforts for specialized domains.

Conclusion: The AAbAAC corpus contributes to the computational study of autoimmunity and illustrates the value of small-scale annotation efforts for improving information extraction in specialized domains.
...
Result: CARVE outperforms eight recent VideoRAG baselines, enabling the generator to receive interleaved evidence from chunks with different configurations.

Conclusion: The paper addresses the limitations of existing VideoRAG benchmarks and methods by introducing V-RAGBench and CARVE, which improve the performance and flexibility of video retrieval-augmented generation.
...
TL;DR: The paper introduces TerraBench, a benchmark for Earth-science reasoning that integrates language models with scientific tools for environmental analysis.


<details>
  <summary>Details</summary>
Motivation: Current climate and environmental decision-making requires interactive reasoning across diverse data types, which is not adequately supported by existing weather and climate models or large language models.

Method: The authors propose TerraAgent, a ReAct-style framework that combines LLM planning with scientific tools for environmental retrieval, geospatial processing, simulation, and artifact-backed computation.

Result: The TerraBench benchmark, comprising 403 tasks across three tracks and eight application domains, shows that reliable Earth-science agents require coordinated workflows, precise tool parameterization, and artifact provenance.

Conclusion: The paper concludes that TerraBench and TerraAgent can unify Earth-science reasoning tasks and provide a more comprehensive approach to Earth-science decision-making.
...
TL;DR: A new reinforcement learning framework called Cognitive Relative Policy Optimization (CRPO) is proposed for mental health assessment, which mimics human cognitive processes and improves reasoning capabilities.


<details>
  <summary>Details</summary>
Motivation: Existing methods for mental health assessment using large language models do not align with human cognitive processes, leading to unreliable outcomes.

Method: CRPO, a reinforcement learning framework that integrates stage-dependent uncertainty modeling and a stage-wise entropy regularization mechanism to encourage exploration and confident decision-making.

Result: CRPO achieves an average improvement of 10.4 percentage points in weighted F1-score over the best reinforcement learning baseline on 8 mental health datasets.

Conclusion: CRPO enhances reasoning capabilities for mental health assessment, and the CRPO-trained model Mental-R1 shows clear advantages over existing large language models on reasoning-intensive cases.
...
Method: The authors propose UXBench, a multimodal benchmark consisting of 2,000 VQA data samples and 8 tasks based on real-world UI screenshots. They also introduce UI-UX, an MLLM enhanced via reinforcement learning with a reward routing mechanism and an asymmetric transition reward.

Result: The authors' extensive evaluation shows that mainstream MLLMs are limited in their capacity for UI-based reasoning. UI-UX achieves state-of-the-art performance on UXBench with an accuracy of 0.7963, surpassing Claude-4.5-Sonnet's 0.6550.

Conclusion: The paper highlights the need for further advancements in MLLMs' ability to reason about UIs and diagnose UX issues. The proposed UXBench and UI-UX demonstrate the potential for improving MLLMs' performance in this area.
...
TL;DR: The paper explores the conditions under which a machine can be considered genuinely creative and how human agency can be preserved in shared creative environments. It proposes a framework of 10 requirements and 3 laws derived from Designics, emphasizing that creativity involves the structural transformation of situations through recursive intervention dynamics.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to understand under what conditions a machine can become genuinely creative while preserving human agency in shared cognitive and creative environments. The authors are driven by the need to redefine genuine machine creativity beyond output novelty or transient architecture.

Method: The paper develops a requirement framework based on Designics, the science of meaning-bearing intentional change. It outlines ten specific requirements for genuine machine creativity: environment representation, scoped perception, conflict identification, intervention capability, consequence observation, knowledge and environment update, rescoping, local-to-global unfolding, value-based scoping, and human-AI co-living. These are organized through three laws of Designics: perception, conflict, and capability.

Result: The paper illustrates the computational tractability of its proposed requirements through various cyber-physical and cyber-biological studies. It examines 'pressure cases' such as open-ended systems, automated discovery frameworks, and agentic workflows, concluding that while these demonstrate powerful generative capabilities, they do not by themselves establish genuine machine creativity. The paper argues for the integration of proactive AI ethics into genuine machine creativity, emphasizing that value-based scoping and human-AI co-living are essential.

Conclusion: The paper concludes that genuine machine creativity should be understood as the structural transformation of incomplete situations through recursive intervention dynamics. It emphasizes that for a machine to be genuinely creative, it must meet the proposed set of requirements and be integrated with proactive AI ethics, ensuring that human agency and value-based scoping are intrinsic to the creative process.
...
TL;DR: The paper proposes ARMOR-MAD, a heterogeneous multi-agent debate framework that improves large language model reasoning by treating debate as conditional computation, achieving better accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: Fixed debate pipelines in multi-agent debate (MAD) waste computation and can amplify correlated errors among similar agents, motivating the need for a more efficient and accurate approach.

Method: The ARMOR-MAD framework combines three components: Pre-debate Agreement Routing (PAR), Early Agreement Stopping Evaluator (EASE), and Semantic Outlier Detection (SOD) to improve debate efficiency and accuracy.

Result: ARMOR-MAD achieves 65.5%, 96.5%, 90.0%, and 81.5% accuracy on MATH Level 5, GSM8K, MMLU, and MMLU-Pro, respectively, outperforming fixed-round heterogeneous debate with the same model pool.

Conclusion: The results suggest that genuine model heterogeneity and agreement-based control are crucial for making MAD more accurate and efficient, validating the effectiveness of the proposed ARMOR-MAD framework.
...
TL;DR: Proposes a bounded trade-off reasoning framework for human decision-making, which captures context-dependent variation in trade-off behavior.


<details>
  <summary>Details</summary>
Motivation: Classical models assume fully compensatory utility aggregation, but people often reject options with poor performance on critical attributes.

Method: Introduces a trade-off tolerance parameter and a screening process that evaluates the balance between gains and losses across attributes, tested through simulation.

Result: Produces preference patterns that differ from standard utility-based models and generates testable predictions for future behavioral studies.

Conclusion: Establishes bounded trade-off screening as a plausible computational mechanism for multi-attribute choice.
...
TL;DR: The paper analyzes hallucination in AI medical imaging systems, unifying taxonomies, comparing model types, and evaluating mitigation strategies, with a focus on FDA oversight.
...
Method: The study synthesizes peer-reviewed studies, benchmark datasets, and FDA guidance across five imaging modalities to analyze hallucination taxonomy, etiology, detection, and mitigation.

Result: The findings indicate that general-purpose foundation models outperform medical-specialized models on hallucination-specific benchmarks, and that physics-informed architectural constraints, Chain-of-Thought prompting, and human-in-the-loop safeguards are effective mitigation strategies.

Conclusion: The study emphasizes the importance of managing hallucination as a lifecycle obligation, rather than a pre-deployment checklist, and highlights the need for continued oversight by radiologists.
...
TL;DR: The paper introduces a new AI methodology called LLM-as-an-Investigator to improve the problem-solving abilities of large language models (LLMs) by making them investigate and gather evidence before providing solutions.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the issue of user-driven sycophancy in LLMs, where they prematurely align with user assumptions and propose solutions without sufficient evidence.

Method: The methodology introduced is called LLM-as-an-Investigator, which is implemented through a Solution Investigator Agent. This agent estimates the ambiguity of a problem description, generates hypotheses, asks clarification questions, and updates hypothesis probabilities based on user responses.

Result: The results show that the proposed approach identifies problems more accurately than standard assistants and reasoning-only baselines. It also reduces user-induced conversational bias by following an evidence-first protocol.

Conclusion: The conclusion of this paper is that the LLM-as-an-Investigator approach offers a more robust and accurate method for problem diagnosis by emphasizing evidence collection and hypothesis testing, thereby mitigating the issues of user-driven sycophancy in LLMs.
...
TL;DR: Brick, a multimodal router, improves model selection by estimating query difficulty and model capability, achieving better accuracy and cost savings.


<details>
  <summary>Details</summary>
Motivation: Existing LLM routers rely on surface features, ignoring within-domain variance, and frontier models are costly, making efficient model selection crucial.
...
Result: Brick achieves 76.98% accuracy at max-quality, beating single models and routers, and offers significant cost savings (4.71x lower cost at 74.11% accuracy).

Conclusion: Brick offers improved accuracy and cost savings by effectively selecting models based on query difficulty and capability.
...
TL;DR: EPIG enhances emotional expressiveness in text-to-image diffusion models by enriching prompts with psychologically informed emotion representations, leading to more emotionally coherent visual outputs.


<details>
  <summary>Details</summary>
Motivation: Current prompting strategies in text-to-image diffusion models are generic and limit the model's ability to accurately express emotional intent and nuanced affective attributes.

Method: EPIG uses valence-arousal emotion representations and structured, role-aware prompt enrichment to enhance emotion-related components of prompts without modifying the image generation backbone.
...
Conclusion: EPIG is a lightweight, training-free method that improves emotional expressiveness in text-to-image diffusion models, making it suitable for resource-constrained and personalized image generation scenarios.
...
TL;DR: This paper proposes a retrieval-augmented generation framework for automated maritime root cause analysis, leveraging 13,329 tribunal reports to improve precedent search and report drafting.


<details>
  <summary>Details</summary>
Motivation: The manual process of retrieving relevant precedents and drafting consistent reports from decades of maritime accident adjudication records is labor-intensive.

Method: A multi-field hybrid retrieval-augmented generation framework is proposed, utilizing a structured knowledge base of incident cards and a field-aware hybrid retrieval approach.
...
Conclusion: The findings suggest that field-aware RAG can substantially streamline maritime safety investigation workflows by enabling faster precedent search and more consistent, evidence-based RCA drafting.
...
<details>
  <summary>Details</summary>
Motivation: The need for a framework that can adapt to new sensors and modalities in gait-based Parkinson's disease assessment without requiring historical patient data, which is often unavailable due to privacy and storage constraints.

Method: MOSAIC, a compact continual learning framework that includes: 1) Modality-Specific Warm-Up to stabilize new modality representations, 2) a statistics-decoupled MSBN architecture to isolate sensor statistics, and 3) a curriculum-guided repulsive objective for Plasticity Recovery.
...
Conclusion: MOSAIC effectively addresses the challenges of modality-incremental learning in gait-based Parkinson's disease assessment, enabling the integration of new modalities without requiring historical data.
...
TL;DR: ProFact, a reinforcement learning framework, is proposed for optimizing multi-stage fact verification using a unified policy, achieving better performance and efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing methods for automated fact verification optimize individual stages in isolation or use fixed heuristics, limiting adaptive coordination and leading to suboptimal outcomes.

Method: ProFact uses an agentic reinforcement learning framework with process-aware rewards to train a unified policy for coordinating claim decomposition, evidence seeking, answer generation, and verdict prediction.

Result: ProFact consistently outperforms strong baselines in verification performance and inference efficiency.

Conclusion: The proposed ProFact framework demonstrates the effectiveness of process-aware trajectory optimization for multi-stage fact verification.
...
TL;DR: This paper introduces the Ethical Robustness Testing System (ERTS) to evaluate AI systems' robustness against adversarial manipulation in ethical contexts. ERTS was used to test several models, revealing that only 33% achieved assessment clearance.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the lack of formal methods for evaluating AI systems' robustness against adversarial manipulation of ethical reasoning, particularly in high-stakes contexts such as healthcare, autonomous vehicles, and employment screening.

Method: The paper proposes the Ethical Robustness Testing System (ERTS), a closed-pipeline framework that encodes ethical dilemmas into a 22-dimensional Ethical Consequence Space (ECS), applies 17 semantic perturbation functions, measures decision deviation via a 4-component Ethical Instability Index (EII), and produces domain-adaptive pre-deployment robustness assessment verdicts.

Result: The results show that only 33% of the tested models achieved assessment clearance. The local Llama-3.2 model was found to be particularly vulnerable to fairness corruption and information degradation attacks, with an ERS score of 0.737.

Conclusion: The paper concludes that the proposed ERTS framework is a comprehensive tool for evaluating AI systems' robustness against adversarial manipulation in ethical contexts. The results highlight the need for improved robustness in AI systems, particularly in high-stakes applications.
...
TL;DR: A physics-guided deep learning framework is proposed for estimating nearshore wave peak periods from passive coastal video streams, achieving accurate and physically consistent results.


<details>
  <summary>Details</summary>
Motivation: Traditional monitoring systems for wave parameters in the nearshore are expensive and have limited spatial coverage, while passive ocean monitoring using video has been achieved by leveraging deep learning but often lacks physical interpretability.

Method: The proposed framework combines automated region-of-interest detection, multi-stage Sim-to-Real transfer learning, and physics-informed regularization, and assesses various spatiotemporal architectures.

Result: The results show that transformer-based architectures outperformed in terms of instantaneous prediction accuracy, while recurrent-convolutional architectures achieved higher temporal stability and operational oceanographic skill.

Conclusion: The proposed framework shows promise for long-term coastal wave monitoring, offering a cost-efficient and operationally feasible solution with accurate and physically consistent results.
...
TL;DR: The authors propose ReSum, a novel framework for improving long-horizon reasoning in Large Language Models (LLMs) through self-summarization, which compresses and organizes reasoning trajectories, improving performance and reducing rollout length.


<details>
  <summary>Details</summary>
Motivation: Existing Reinforcement Learning with Verifiable Rewards (RLVR) methods encourage unnecessarily long reasoning rollouts, degrading reasoning coherence and exhausting context budgets, and often rely on external mechanisms rather than enabling models to manage their own reasoning trajectory.

Method: The authors propose ReSum, a novel RLVR framework that enables LLMs to compress and organize their reasoning trajectories through self-summarization, using a summarization-aware adaptive rollout mechanism and a summarization-aware advantage.

Result: ReSum improves performance by an average of 4% and reduces rollout length by 18.6% through extensive experiments.

Conclusion: The proposed ReSum framework effectively improves long-horizon reasoning in LLMs by enabling self-summarization and adaptive rollout management, leading to improved performance and reduced rollout length.
...
TL;DR: The paper proposes a method to avoid redundant computation in AI agents by precomputing and sharing the key-value (KV) cache of a document, allowing agents to skip the prefill step and reducing compute costs.


<details>
  <summary>Details</summary>
Motivation: Currently, AI agents recompute the KV cache from scratch for each document, leading to redundant and costly computation. The authors aim to reduce this waste by proposing a simple yet effective solution.

Method: The authors propose a system where a publisher precomputes a document's KV cache and allows other agents to buy the right to load it, skipping the prefill step. This approach is token-exact, with no accuracy cost.

Result: The proposed method shows significant compute savings, with reuse being 9-50x cheaper than prefill on Qwen3-4B. For a 3774-token document, serving it to 80M agents costs ~$1.5M to re-prefill but only ~$0.03M of reuse compute, a 49.7x reduction.

Conclusion: The authors conclude that their proposal is feasible and profitable, with potential millions of dollars in savings per popular document. They frame the resulting agent-native prefill CDN and identify lossless KV compression and a cross-party payment layer as open problems.
...
TL;DR: IterCAD is a unified multimodal agent framework for closed-loop, interactive CAD generation and editing, outperforming existing methods in code executability and geometric precision.
...
Method: IterCAD uses a multimodal agent framework with a data synthesis pipeline, progressive SFT, and geometry-aware reinforcement learning.

Result: IterCAD achieves highly competitive performance across multiple benchmarks, outperforming existing approaches in code executability and geometric precision.

Conclusion: IterCAD presents a unified multimodal agent framework for closed-loop, interactive CAD generation and editing, demonstrating superior capabilities in iterative refinement.
...
TL;DR: The study analyzes training dynamics of a small language model under a fixed token budget, finding non-monotonic degradation in performance over time and highlighting the importance of examining training trajectories.


<details>
  <summary>Details</summary>
Motivation: The study aims to investigate training dynamics in language models under compute-constrained settings, beyond just evaluating endpoint performance.

Method: The study uses a quantitative experimental repeated measures design, conducting six independent training runs of a 4.26-million-parameter model on the TinyStories corpus with a target budget of 20 million cumulative training tokens.

Result: The study finds statistically significant interval effects for validation loss, validation perplexity, and rolling volatility, with performance improving initially but degrading later in training, and notes recurrent validation-loss backslides and no stable phase.

Conclusion: The findings suggest that compute-aware language model evaluation should examine training trajectories rather than endpoint metrics alone, and that additional token exposure may not produce proportional generalization gains in constrained compute settings.
...
TL;DR: Introduced MiniMax Sparse Attention (MSA) to enable ultra-long-context capability in LLMs by reducing the cost of softmax attention.


<details>
  <summary>Details</summary>
Motivation: The quadratic cost of softmax attention makes it impractical for LLMs to jointly attend over hundreds of thousands to millions of tokens.

Method: MSA uses a blockwise sparse attention built upon Grouped Query Attention (GQA) with a lightweight Index Branch and a Main Branch to perform exact block-sparse attention.

Result: MSA reduces per-token attention compute by 28.4x at 1M context and achieves 14.2x prefill and 7.6x decoding wall-clock speedups on H800.

Conclusion: MSA enables efficient ultra-long-context capability in LLMs, making it practical for deployment at scale.
...
TL;DR: The paper proposes a compliance-by-construction approach for LLM-based agents in regulated industries, integrating symbolic structures into the agent's decision-making to prevent control-flow violations.


<details>
  <summary>Details</summary>
Motivation: The increasing use of LLM-based agents in regulated industries necessitates ensuring their compliance with regulations and process models.

Method: The authors propose a neuro-symbolic approach, combining symbolic structures (regulations, process models, compliance constraints) with LLM-based agents to achieve compliance-by-construction.

Result: The paper identifies key research challenges for neuro-symbolic integration and argues for the importance of addressing them to enable compliance-by-construction in regulated process automation.

Conclusion: The authors call for the neuro-symbolic community to engage with regulated process automation as a high-impact research domain, emphasizing the need for a structural foundation that prevents control-flow violations.
...
TL;DR: The paper presents a metaheuristic approach to optimize appliance start times for maximizing renewable energy utilization while minimizing user inconvenience, considering solar generation forecasts and system constraints over multiple days.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the mismatch between solar energy generation and household consumption patterns by optimizing appliance start times to maximize renewable energy utilization while minimizing user inconvenience.
...
Result: The experimental results show that the sequential multi-day scheduling framework effectively manages system constraints while ensuring user convenience under exclusive solar generation.

Conclusion: The paper concludes that the proposed approach can effectively manage system constraints and user convenience, opening opportunities for future research on multi-objective trade-offs between investment in equipment, return on investment, and user satisfaction.
...
TL;DR: This paper examines how the source of labels affects the evaluation of machine learning models, particularly in large-scale metadata-driven systems where labels may be incomplete or inconsistent.


<details>
  <summary>Details</summary>
Motivation: The authors are motivated by the fact that evaluation outcomes in operational information systems are often conditioned by the processes used to generate labels, which can lead to biased performance measurements.

Method: The authors propose a multi-track evaluation framework that systematically varies training and evaluation label sources, and introduce the concept of 'evaluation sovereignty' to measure the degree to which performance metrics are independent of label authority and supervision regime.

Result: The authors demonstrate that models exhibiting strong performance under operational ('silver') evaluation degrade substantially under independent ('gold') evaluation, particularly for fine-grained classification, and that ranking-based metrics remain above baseline.

Conclusion: The findings suggest that commonly reported performance metrics may reflect alignment with labeling processes rather than true predictive capability, and that evaluation validity should be reconceptualized as a system-level property shaped by label governance.
...
Conclusion: The paper concludes that LLMs do not meet the requirements for moral responsibility, as they lack commitment-bearing agency grounded in intrinsic intentionality and self-attributed action.
...
TL;DR: The paper proposes CloudCons, a benchmark to evaluate forecasting models for cloud resource consolidation, and finds that while foundation models have better forecasting accuracy, it doesn't necessarily translate to better decision utility.
...
Method: The paper proposes a comprehensive end-to-end benchmark called CloudCons, which includes high-quality datasets from Huawei Cloud, Microsoft Azure, and Google Borg, and conducts an extensive evaluation of various forecasting models.

Result: The experiments reveal that foundation models have superior zero-shot forecasting accuracy, but this advantage does not inherently translate into better decision utility, and that the selection of predictive quantiles is a critical lever in balancing resource efficiency and service reliability.

Conclusion: The paper provides actionable guidelines for calibrating predictive quantiles to balance the trade-off between resource efficiency and service reliability, offering vital insights for real-world deployment decisions.
...
TL;DR: Proposed Uncertainty-aware Multi-Granularity RAG (UMG-RAG) and UMGP-RAG for improving retrieval augmented generation by fusing multi-granularity evidence and estimating reliability.


<details>
  <summary>Details</summary>
Motivation: Retrieval augmented generation depends on the quality and granularity of retrieved evidence. Existing methods struggle with either large retrieval units introducing irrelevant content or fine-grained units being difficult to retrieve reliably.

Method: UMG-RAG uses existing dense and sparse retrievers as complementary experts across multiple chunk granularities, estimates reliability from distribution entropy, and fuses candidates according to query-specific confidence.
...
Conclusion: The proposed UMG-RAG and UMGP-RAG frameworks improve retrieval augmented generation by effectively fusing multi-granularity evidence and estimating reliability, leading to better generation quality.
...
TL;DR: The authors propose a solution to the cold-start problem in personalized health AI systems by using an individual's genomic profile as a prior to initialize machine learning models for physiological interpretation.
...
Method: Using causal inference and Bayesian prior design, with an individual's genomic profile serving as an exogenous genetic anchor to initialize a Bayesian belief state over an individual's physiological set point.

Result: The proposed architecture allows for personalized inference across six physiological domains, with the ability to distinguish between constitutionally fixed baseline and environmentally driven deviation.

Conclusion: The authors propose a solution to the cold-start problem in personalized health AI systems by leveraging genomic information to create personalized priors, allowing for more accurate and efficient physiological interpretation.
...
TL;DR: The paper proposes a three-layer view of AI in scientific discovery, emphasizing the importance of model formation through qualitative reasoning, which is currently underdeveloped.


<details>
  <summary>Details</summary>
Motivation: Current discussions of AI in scientific discovery focus on search and execution, but neglect the crucial aspect of model formation and evolution.

Method: The paper proposes a three-layer view of AI in discovery: Layer 1 (search and retrieval), Layer 2 (model formation through qualitative reasoning), and Layer 3 (execution, optimization, and refinement).

Result: The paper highlights the significance of Layer 2 (model formation) and provides three case studies to illustrate its application.

Conclusion: The paper concludes that model formation through qualitative reasoning (Layer 2) is the most important and least developed aspect of AI in scientific discovery.
...
TL;DR: The paper introduces protocols for producing a single aggregated confidence for multiagent systems in NLP, improving discriminative power without sacrificing correctness.


<details>
  <summary>Details</summary>
Motivation: Existing methods in NLP do not produce or evaluate confidence for multiagent system outputs, and prior work on multiagent debate (MAD) does not aggregate confidence for the system as a whole.

Method: The paper proposes three protocols that transform raw confidence signals to make them comparable, then combine them via soft voting or Bayesian fusion.

Result: The aggregated confidence is more discriminative (AUARC) than single agents or debate baselines, with stable correctness (F1-score) and improved performance on ambiguous tasks.

Conclusion: The proposed protocols effectively aggregate confidence in multiagent systems, enhancing discriminative power while maintaining correctness across various benchmarks and task types.
...
TL;DR: A self-supervised framework called Orchestration Reward Modeling (OrchRM) is proposed to improve the efficiency and performance of multi-agent systems built on large language models by effectively orchestrating specialized agents.


<details>
  <summary>Details</summary>
Motivation: The need for effective orchestration in multi-agent systems built on large language models, hindered by limited supervision and high computational cost.

Method: Orchestration Reward Modeling (OrchRM), a self-supervised framework that leverages intermediate artifacts from multi-agent executions to construct win-lose pairs for Bradley-Terry reward model training.

Result: OrchRM improves training efficiency by up to 10x in token usage and MAS test-time scaling performance by up to 8% in accuracy across multiple domains.

Conclusion: Orchestration-level reward modeling is a scalable direction for robust multi-agent orchestration, as demonstrated by the consistent gains in efficiency and performance across various domains.
...
TL;DR: EpiBench, a benchmark for epigenomics analysis, was introduced to evaluate AI models' ability to make analysis decisions. Results show that no model passed a majority of attempts, with GPT-5.5/Pi leading at 45% success rate.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to create a verifiable benchmark for evaluating AI models' performance in short-horizon epigenomics analysis, specifically in making well-defined analysis decisions from realistic workflow states.

Method: The authors introduced EpiBench, a benchmark that includes 106 evaluations across various epigenomics workflows (CUT&Tag/CUT&RUN, ATAC-seq, ChIP-seq, and DNA methylation). They tested 16 model-harness pairs, resulting in 5,088 valid trajectories.

Result: The results show that no system passed a majority of attempts, with the best performance achieved by GPT-5.5/Pi at 45.0% success rate (143/318 attempts). Performance varied across assay types, and many failed runs still contained parts of the correct answer.

Conclusion: The conclusion of this paper is that while AI models can find the right files and compute useful intermediate results, they often fail when tasks require deeper, assay-specific scientific judgment. The authors highlight the need for improvement in epigenomics analysis.
...
TL;DR: A reinforcement learning system was deployed at DoorDash to adapt dispatch objective weights in a food-delivery marketplace, improving batching and reducing courier-side time costs without degrading delivery quality.


<details>
  <summary>Details</summary>
Motivation: The dispatch process in three-sided marketplaces like food delivery involves evaluating decisions based on delayed operational outcomes. The goal is to adapt dispatch objective weights to balance delivery quality and batching efficiency.

Method: A store-level policy learned from logged marketplace data selects a discrete multiplier that shifts the dispatch optimizer's tradeoff between delivery quality and batching efficiency. The policy uses a shared value function trained with centralized offline data and decentralized store-level execution, utilizing Double Q-learning targets and a conservative regularizer.

Result: The offline-trained policy increased batching and reduced courier-side time costs without degrading customer-facing delivery quality in a production switchback experiment.

Conclusion: The system demonstrates how world feedback from a live economic and logistics system can be used to safely adapt decision policies online, improving operational efficiency in a large-scale food-delivery marketplace.
...
TL;DR: The paper discusses the potential of LLM-based agents in automating scientific discovery and proposes environment engineering as a key area of research. It presents EurekAgent, an environment-engineered agent system that achieves state-of-the-art results in various tasks.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the bottleneck in autonomous scientific discovery, which is shifting from prescribing agent workflows to designing agent environments. The authors aim to highlight the importance of environment engineering in developing reliable autonomous research agents.

Method: The authors propose a new approach called environment engineering, which involves designing environments that amplify productive behaviors and suppress harmful behaviors in LLM-based agents. They present EurekAgent, an environment-engineered agent system that engineers the environment along four dimensions: permissions engineering, artifact engineering, budget engineering, and human-in-the-loop engineering.

Result: The results show that EurekAgent achieves state-of-the-art results on multiple tasks, including mathematics, kernel engineering, and machine learning tasks. Specifically, it discovers new state-of-the-art 26-circle packing results with less than $11 in total API cost.

Conclusion: The paper concludes that environment engineering is a crucial research direction for developing reliable autonomous research agents. The authors open-source their code and results, encouraging further research in this area.
...
TL;DR: The paper finds that both humans and large language models (LLMs) make similar errors in common-sense reasoning, suggesting that both may rely on pattern-matching rather than abstract world models.


<details>
  <summary>Details</summary>
Motivation: The authors aim to investigate whether LLMs truly reason or just perform pattern matching, and compare their abilities with human reasoning in everyday situations.

Method: The study evaluates human participants and 25 LLMs on common-sense reasoning tasks, identifies attention heads driving LLM responses, and predicts human errors caused by irrelevant prompt details.

Result: The results show similar patterns of errors in both humans and LLMs, and that attention heads in LLMs implement a form of pattern-matching, predicting human errors.

Conclusion: The study suggests that everyday causal reasoning in both people and LLMs is more consistent with pattern-matching than with abstract world models.
...
TL;DR: The paper proposes Agentified Agent Assessment (AAA), a unified framework for evaluating agent systems using standardized protocols, enabling reproducible, interoperable, and multi-agent evaluation.
...
Method: The authors introduce AAA, where evaluation is performed by judge agents and all participants interact through standardized protocols (A2A and MCP). They also propose AgentBeats, a concrete realization of AAA, and conduct two studies to evaluate its effectiveness.

Result: The studies show that AAA applies across a heterogeneous range of benchmarks, preserves fidelity with the public record, and surfaces previously missing head-to-head results, yielding research insights about agent design.
...
TL;DR: The authors propose rethinking shielded reinforcement learning from a runtime safety mechanism to a design-time analytical tool for providing structural insights about a system.


<details>
  <summary>Details</summary>
Motivation: The authors argue that the current approach to shielded reinforcement learning, which focuses on runtime safety, is misguided and that the same automata-theoretic machinery can be better utilized as a design-time instrument.

Method: The authors instantiate their approach through a constrained two-player safety game for network defense, where specifications are enforced asymmetrically for the defender and attacker.

Result: The authors derive a defensibility verdict, a formal certificate that a topology-specification pair is or is not defensible, along with a defensibility fingerprint that captures both formal safety properties and operational behavior under adaptive play.

Conclusion: The authors conclude that shield synthesis is most valuable as a framework for answering architectural questions about system defensibility, rather than as a deployment mechanism for safe agents.
...
TL;DR: The paper analyzes three frameworks for understanding AI's cognitive and epistemic consequences, highlighting the unique contribution of System 0 and introducing the concept of cognitive colonization.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to understand the cognitive and epistemic consequences of artificial intelligence and to address the urgent need to recognize and mitigate the invisible forms of influence that AI systems can have on users.

Method: The paper examines and compares three recent frameworks for understanding AI's influence: Tri-System Theory, Thinkframes, and System 0.

Result: The paper argues that System 0 offers a theoretically distinctive perspective on AI's influence that is not fully replicable by the other two frameworks, and introduces the concept of cognitive colonization to describe how AI systems can embed external interests within users' selves.

Conclusion: The paper concludes that understanding the cognitive and epistemic consequences of AI, particularly the invisible forms of influence, is an urgent philosophical and practical task.
...
TL;DR: The paper introduces Agents-K1, a knowledge orchestration pipeline that converts scientific papers into agent-native knowledge graphs, outperforming existing methods in scientific information extraction and reasoning.


<details>
  <summary>Details</summary>
Motivation: Current research agents overlook scientific knowledge orchestration, reducing papers to abstracts and surface mentions, and omitting key entities and relationships.

Method: Agents-K1 integrates a multimodal parser, a 4B information-extraction backbone, and a graph-anything CLI to construct scientific knowledge graphs from raw documents.

Result: Agents-K1 achieves superior performance in scientific information extraction, knowledge graph construction, and multi-hop scientific reasoning, demonstrated through extensive experiments.

Conclusion: The proposed Agents-K1 pipeline effectively converts scientific papers into knowledge graphs, enhancing scientific reasoning and information extraction capabilities.
...
TL;DR: Large language models can automate reproducibility assessments in social and behavioral sciences, achieving similar results to human reanalysis.


<details>
  <summary>Details</summary>
Motivation: Evaluating reproducibility in social and behavioral sciences is resource-intensive and difficult to scale, prompting the exploration of automated methods.

Method: Using large language models to generate analysis and compare with original findings and human reanalysis on 76 published studies.

Result: The LLM pipeline recovered original effect sizes in 41% of studies and reached the same qualitative conclusion in 96% of cases, compared to 34% and 74% by human reanalysts.

Conclusion: Large language models can serve as a scalable tool for automated reproducibility assessment, providing a foundation for systematic auditing of empirical results.
