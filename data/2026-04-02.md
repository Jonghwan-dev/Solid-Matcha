...
TL;DR: A new generative model, DF-ACBlurGAN, is proposed to generate images with repeated and periodic structures, particularly for biomaterial design applications.


<details>
  <summary>Details</summary>
Motivation: Current machine learning and computer vision models struggle with generating images that have internally repeated and periodic structures, which is a challenge in applications like biomaterial surface design.

Method: The proposed model, DF-ACBlurGAN, uses a structure-aware conditional generative adversarial network that reasons about long-range repetition during training, integrating frequency-domain repetition scale estimation, scale-adaptive Gaussian blurring, and unit-cell reconstruction.

Result: The model demonstrates improved repetition consistency and controllable structural variation compared to conventional generative approaches across multiple biomaterial datasets.

Conclusion: The proposed DF-ACBlurGAN model effectively generates biomaterial designs with aligned functional outcomes, showing potential in applications requiring strict control over structural consistency.
...
TL;DR: OccSim, a 3D simulator, generates large-scale, continuous driving scenarios without relying on pre-recorded logs or HD maps, enabling improved autonomous driving simulation.


<details>
  <summary>Details</summary>
Motivation: Current data-driven autonomous driving simulations are limited by their reliance on pre-recorded driving logs or spatial priors, restricting scalability and open-ended generation capabilities.

Method: OccSim uses a W-DiT-based static occupancy world model and a Layout Generator to synthesize massive, diverse simulation streams, conditioned on a single initial frame and future ego-actions.

Result: OccSim can generate over 3,000 continuous frames, enabling the construction of large-scale 3D occupancy maps spanning over 4 kilometers, with an >80x improvement in stable generation length over previous state-of-the-art occupancy world models.

Conclusion: OccSim demonstrates its utility in pre-training 4D semantic occupancy forecasting models, achieving up to 67% zero-shot performance on unseen data, and shows scalability with improved performance as the dataset size increases.
...
TL;DR: A novel adaptation framework, Fisheye3R, is proposed to extend multi-view 3D reconstruction foundation models to accommodate fisheye inputs without performance regression on perspective images.


<details>
  <summary>Details</summary>
Motivation: The performance of feed-forward foundation models for multi-view 3D reconstruction degrades on wide field-of-view images, such as those from fisheye cameras, due to changes in spatial positions of pixels caused by non-linear projection models.

Method: Fisheye3R, a novel adaptation framework that uses flexible learning schemes for self-supervised adaptation with unlabeled perspective images and supervised adaptation without fisheye training data.

Result: Extensive experiments demonstrate that Fisheye3R consistently improves camera pose, depth, point map, and field-of-view estimation on fisheye images across three foundation models.

Conclusion: The proposed Fisheye3R framework enables the extension of multi-view 3D reconstruction foundation models to natively accommodate fisheye inputs without performance regression on perspective images.
...
TL;DR: The study uses 7T fMRI to construct functional graphs of brain networks and train a Graph Neural Network to decode visual categories, revealing biologically meaningful subnetworks.


<details>
  <summary>Details</summary>
Motivation: To understand how large-scale brain networks represent visual categories and link perception and cortical organization.

Method: Using high-resolution 7T fMRI, constructing parcel-level functional graphs, and training a signed Graph Neural Network with sparse edge mask and class-specific saliency.
...
Conclusion: The framework bridges machine learning and neuroscience by extending voxel-level category selectivity to a connectivity-based representation of visual processing.
...
TL;DR: A new framework called Stepper for synthesizing immersive 3D scenes from text descriptions, achieving high fidelity and structural consistency.


<details>
  <summary>Details</summary>
Motivation: Existing methods for immersive 3D scene synthesis from text suffer from trade-offs between visual fidelity and explorability, and the authors aim to overcome these limitations.

Method: Stepper uses a unified framework with a novel multi-view 360° diffusion model and a geometry reconstruction pipeline to enable consistent, high-resolution expansion of panoramic scenes.
...
Conclusion: The proposed framework, Stepper, sets a new standard for immersive 3D scene synthesis from text, addressing the limitations of existing approaches and paving the way for applications in AR/VR and world modeling.
...
TL;DR: The study investigates hybrid quantum-classical machine learning approaches for classifying defects in aluminium TIG welding images, comparing their performance with a conventional deep learning model.


<details>
  <summary>Details</summary>
Motivation: To explore the potential of hybrid quantum-classical machine learning for advancing automated quality control in industrial settings, specifically for defect detection and quality assurance.

Method: Two hybrid quantum-classical approaches are proposed: (1) encoding features into quantum states using a parameterized quantum feature map and solving a linear system using a Variational Quantum Linear Solver (VQLS); (2) applying angle encoding to extracted features in a variational quantum circuit and using a classical optimizer for model training. A convolutional neural network (CNN) is used to extract feature vectors from weld images.

Result: The hybrid quantum-classical models perform competitively with the classical CNN model, highlighting their potential for near-term real-world applications in industrial defect detection and quality assurance.

Conclusion: The study demonstrates the promise of hybrid quantum-classical machine learning approaches for industrial quality control, particularly for defect detection and quality assurance, and encourages further exploration of these methods for real-world applications.
...
TL;DR: A feed-forward method for capturing human performance from a monocular RGB stream, rendering novel views by accumulating appearance information over time.


<details>
  <summary>Details</summary>
Motivation: The lack of sufficient observations, especially for unseen regions, when capturing human performance from a monocular RGB stream.

Method: A feed-forward human performance capture method that utilizes a progressively updated canonical space and probabilistic regression.

Result: The approach produces sharper reconstructions and enables plausible synthesis even in regions with no prior observations.

Conclusion: The proposed method effectively captures human performance from a monocular RGB stream, rendering novel views with high quality.
...
TL;DR: The paper presents MEDiC, a framework combining raw pixel space and latent feature space for Masked Image Modeling (MIM), achieving 73.9% kNN accuracy and 85.1% fine-tuning accuracy with ViT-Base.


<details>
  <summary>Details</summary>
Motivation: The motivation is to combine the strengths of MIM methods operating in raw pixel space and latent feature space, and to investigate the design space of a multi-objective framework.

Method: The method involves a framework with three objectives: patch-level token distillation, global CLS alignment, and pixel reconstruction. It also introduces hierarchical clustering with relative position bias for evolved masking.

Result: The results show that the full combination of objectives reaches 73.9% kNN accuracy on ImageNet-1K. The framework achieves 73.9% kNN and 85.1% fine-tuning accuracy with ViT-Base at 300 epochs.

Conclusion: The paper concludes that the proposed framework, MEDiC, effectively combines the strengths of MIM methods operating in raw pixel space and latent feature space, and provides insights into the design space of a multi-objective framework.
...
TL;DR: A novel ultrasound scene representation called UltraG-Ray is proposed, which uses a learnable 3D Gaussian field and physics-based module to generate more realistic ultrasound images.


<details>
  <summary>Details</summary>
Motivation: Current methods for novel view synthesis (NVS) in ultrasound struggle with complex tissue and view-dependent acoustic effects, and physics-based NVS aims to address these limitations.

Method: UltraG-Ray, a learnable 3D Gaussian field coupled to an efficient physics-based module for B-mode synthesis, which explicitly encodes ultrasound-specific parameters and uses a novel ray casting scheme.

Result: The proposed method shows consistent gains in image quality metrics (up to 15% increase on MS-SSIM) compared to state-of-the-art methods, demonstrating clear improvement in terms of realism of the synthesized ultrasound images.

Conclusion: UltraG-Ray enables the generation of physically informed B-mode images with increased realism, and has the potential to improve training for clinicians and data augmentation.
...
TL;DR: MMFace-DiT, a unified dual-stream diffusion transformer, enables synergistic multimodal face synthesis with improved spatial-semantic consistency and controllability.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal face generation models have limitations due to ad hoc designs, such as appending auxiliary control modules or stitching together separate uni-modal networks, which can lead to modal dominance and limited adaptability to varying spatial conditions.

Method: The authors propose a unified dual-stream diffusion transformer, MMFace-DiT, with a novel dual-stream transformer block that processes spatial and semantic tokens in parallel, and a Modality Embedder that enables dynamic adaptation to varying spatial conditions.

Result: MMFace-DiT achieves a 40% improvement in visual fidelity and prompt alignment over six state-of-the-art multimodal face generation models.

Conclusion: The proposed MMFace-DiT model establishes a flexible new paradigm for end-to-end controllable generative modeling, enabling unprecedented spatial-semantic consistency for controllable face generation.
...
TL;DR: Pretraining implicit neural representations (INRs) on noise signals improves their ability to fit unseen signals and encode priors for inverse imaging tasks, with noise having a 1/|f^α| spectral structure achieving the best balance of capabilities.


<details>
  <summary>Details</summary>
Motivation: The sensitivity of implicit neural representations (INRs) to parameter initialization strategies and the need to understand why data-driven initialization methods succeed.

Method: Experimental analyses leveraging noise pretraining on diverse noise classes (e.g., Gaussian, Dead Leaves, Spectral) to measure INRs' ability to fit unseen signals and encode priors for denoising.

Result: Pretraining on unstructured noise (Uniform, Gaussian) improves signal fitting capacity but yields poor deep image priors for denoising, while noise with a 1/|f^α| spectral structure achieves an excellent balance of signal fitting and inverse imaging capabilities.

Conclusion: The study reveals that noise pretraining can improve INRs' performance, and noise with a specific spectral structure can achieve a balance of signal fitting and inverse imaging capabilities, enabling more efficient INR training.
...
TL;DR: The paper presents a method to remove humans and their shadows from egocentric 'walking tour' videos to improve environment modeling applications.


<details>
  <summary>Details</summary>
Motivation: The presence of humans in frames of egocentric walking tour videos limits their usefulness in environment modeling due to crowds and eye-level camera perspectives.

Method: A generative algorithm was developed to inpaint humans and their shadows from walking tour videos, using a semi-synthetic dataset and fine-tuning the Casper video diffusion model.

Result: The resulting model outperformed Casper qualitatively and quantitatively in removing humans from walking tour clips with significant human presence and complex backgrounds.

Conclusion: The generated clips can be used to build successful 3D/4D models of urban locations, improving environment modeling applications.
...
TL;DR: A new framework called DASES is introduced for autonomous scientific discovery, which uses adaptive falsification to actively challenge and improve scientific artifacts, leading to the discovery of a new loss function, FNG-CE, that outperforms existing methods.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the limitations of current autonomous scientific discovery methods, which can be gamed by strong search processes that learn to pass evaluations without truly understanding the underlying mechanisms.

Method: The authors introduce DASES, a falsification-driven framework that co-evolves executable scientific artifacts and counterexample environments through the interactions of an Innovator, an Abyss Falsifier, and a Mechanistic Causal Extractor.

Result: The DASES framework successfully identifies and rejects artifacts that would have been accepted by static validation, and discovers a new loss function, FNG-CE, which transfers well beyond the synthetic discovery environment and outperforms existing methods, including CE and CE+L2, across standard benchmarks like ImageNet.

Conclusion: The paper concludes that DASES offers a promising approach to autonomous scientific discovery by actively pushing against candidates through adaptive falsification, leading to more robust and generalizable scientific artifacts.
...
TL;DR: A looped transformer framework with geometry-aware alignment is proposed for isolated sign language recognition, achieving state-of-the-art results with fewer layers.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for isolated sign language recognition rely on deep feed-forward architectures, which lack mechanisms for recurrent refinement and structured representation.

Method: LA-Sign, a looped transformer framework with geometry-aware alignment, is proposed. It uses recurrence to progressively refine motion understanding and a geometry-aware contrastive objective to regularise the refinement process.

Result: LA-Sign achieves state-of-the-art results on WLASL and MSASL benchmarks while using fewer unique layers.

Conclusion: The proposed LA-Sign framework demonstrates the effectiveness of recurrent latent refinement and geometry-aware representation learning for sign language recognition.
...
TL;DR: The paper explains the modality gap in multi-modal models, proving it can be minimized to improve robustness without losing accuracy.


<details>
  <summary>Details</summary>
Motivation: The modality gap in multi-modal models, where image and text distributions are separated in the shared embedding space, lacks understanding of its cause and effect on performance.

Method: The paper analyzes the contrastive loss in multi-modal models and proposes a post-processing step to minimize the modality gap.

Result: The experiments demonstrate that reducing the modality gap increases model robustness to embedding perturbations without sacrificing clean accuracy.

Conclusion: The modality gap can be minimized through a simple post-processing step, enhancing model robustness without compromising performance.
...
TL;DR: WorldFlow3D is a novel method for generating unbounded 3D worlds by flowing through 3D data distributions, enabling rapid and high-quality scene generation with controllability.


<details>
  <summary>Details</summary>
Motivation: Unbounded 3D world generation is a foundational task for scene modeling in computer vision, graphics, and robotics, and existing methods have limitations.

Method: WorldFlow3D uses a latent-free flow approach, modeling 3D generation as a problem of flowing through 3D data distributions, and guides generation with vectorized scene layout conditions.

Result: WorldFlow3D generates causal and accurate 3D structure, converges rapidly, and achieves high-quality texture and controllability, outperforming existing methods in various settings.

Conclusion: WorldFlow3D is an effective method for unbounded 3D world generation, validating cross-domain generalizability and high-quality generation on real data distributions.
...
TL;DR: A new method, TrajectoryMover, enables moving an object's 3D motion trajectory in a video while preserving its relative 3D motion.


<details>
  <summary>Details</summary>
Motivation: Existing generative video editing methods lack the ability to move an object's 3D motion trajectory in a video while preserving its relative 3D motion.

Method: A new data generation pipeline, TrajectoryAtlas, is introduced to create large-scale synthetic paired video data, which is then used to fine-tune a video generator, TrajectoryMover.

Result: The proposed method successfully enables generative movement of object trajectories in videos.

Conclusion: The TrajectoryMover method fills the gap in existing generative video editing methods by allowing the movement of an object's 3D motion trajectory in a video.
...
TL;DR: DIME, a Dual-Imbalance-aware Adapter Merging framework, is proposed for continual food recognition, addressing dual imbalance issues and outperforming existing baselines.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the dual imbalance problem in continual food recognition, where there is imbalance in both samples within each food class and the number of new food classes to learn at each step.

Method: DIME uses parameter-efficient fine-tuning to learn lightweight adapters for each task and a class-count guided spectral merging strategy to integrate them.

Result: The proposed method consistently improves by more than 3% over the strongest existing continual learning baselines on realistic long-tailed food benchmarks.

Conclusion: The authors conclude that DIME effectively addresses the dual imbalance problem in continual food recognition and provides a more efficient and effective solution.
...
TL;DR: The paper proposes SparseDriveV2, a scoring-based method for end-to-end multi-modal planning in driving behavior modeling, which achieves comparable performance to dynamic proposal methods through a dense static vocabulary and improved scoring strategy.


<details>
  <summary>Details</summary>
Motivation: To investigate whether static vocabularies can achieve comparable performance to dynamic proposals in end-to-end multi-modal planning by increasing the density of the vocabulary.

Method: The paper proposes two innovations: (1) a scalable vocabulary representation that decomposes trajectories into geometric paths and velocity profiles, and (2) a scalable scoring strategy with coarse factorized scoring and fine-grained scoring on a small set of composed trajectories.

Result: SparseDriveV2 achieves 92.0 PDMS and 90.1 EPDMS on NAVSIM, and 89.15 Driving Score and 70.00 Success Rate on Bench2Drive with a lightweight ResNet-34 as backbone.

Conclusion: The paper concludes that static vocabularies can achieve comparable performance to dynamic proposals when they are sufficiently dense, and that SparseDriveV2 pushes the performance boundary of scoring-based planning.
...
TL;DR: LatentPilot is a new vision-and-language navigation model that uses future observations during training to learn action-conditioned visual dynamics, enabling more robust decision-making.


<details>
  <summary>Details</summary>
Motivation: Existing VLN models ignore future visual dynamics, limiting their understanding of the causal relationship between actions and visual world changes.

Method: LatentPilot uses a flywheel-style training mechanism and learns visual latent tokens without explicit supervision, allowing the agent to 'dream ahead' and reason about future observations.

Result: LatentPilot achieves new SOTA results on R2R-CE, RxR-CE, and R2R-PE benchmarks and demonstrates superior understanding of environment-action dynamics in real-robot tests.

Conclusion: LatentPilot improves VLN models by leveraging future observations during training, enabling more robust decision-making and a better understanding of environment-action dynamics.
...
TL;DR: This study explores using CT scans as training supervision for an X-ray classifier to detect thoracic disease without requiring CT scans at inference time.


<details>
  <summary>Details</summary>
Motivation: Most computer-aided diagnosis models are trained and deployed within a single imaging modality, and the study aims to investigate if CT can act as training-only supervision for a binary disease versus non-disease X-ray classifier.

Method: The study uses a cross-modality teacher-student distillation problem and evaluates various methods, including JDCNet, plain cross-modal logit-KD, attention transfer, and feature hints.

Result: The results show that plain cross-modal logit-KD attains the highest mean accuracy (0.875) and macro-F1 (0.714), while late fusion and same-modality distillation also show promising results in resampled protocols.

Conclusion: The study provides a reproducible and evidence-bounded pilot protocol for CT-to-X-ray transfer, highlighting the task definition, failure modes, ranking instability, and minimum requirements for future credible claims.
...
TL;DR: The authors propose a modified MedSAM model for multi-class brain tissue segmentation, achieving high accuracy on the IXI dataset.


<details>
  <summary>Details</summary>
Motivation: Accurate segmentation of brain tissues from MRI is crucial for studying brain anatomy, diagnosing neurological disorders, and monitoring disease progression, but traditional methods have limitations.

Method: The authors modify MedSAM by extending its mask decoder to three classes, freezing the pre-trained image encoder, and fine-tuning the prompt encoder and decoder, with a preprocessing pipeline including skull stripping, tissue probability mapping, and converting to 2D slices.

Result: The model achieves Dice scores up to 0.8751 on the IXI dataset.

Conclusion: Foundation models like MedSAM can be adapted for multi-class medical image segmentation with minimal architectural modifications, and can be extended to more diverse medical imaging scenarios.
...
TL;DR: The paper proposes SplatHLoc, a novel hierarchical visual relocalization framework that uses Feature Gaussian Splatting as the scene representation, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: The limitations of point-based hierarchical relocalization methods, specifically sparse image observations and weak feature matching, motivated the development of SplatHLoc.

Method: SplatHLoc uses Feature Gaussian Splatting as the scene representation and proposes an adaptive viewpoint retrieval method and a hybrid feature matching strategy.

Result: SplatHLoc enhances the robustness of visual relocalization, achieving a new state-of-the-art on both indoor and outdoor datasets.

Conclusion: The proposed SplatHLoc framework effectively addresses the limitations of existing point-based methods, providing a more robust and accurate visual relocalization approach.
...
TL;DR: The paper proposes a benchmark called SLVMEval to evaluate text-to-video evaluation systems on long videos up to 3 hours, and finds that existing systems are less accurate than human evaluators.


<details>
  <summary>Details</summary>
Motivation: The motivation is to assess the ability of text-to-video evaluation systems to accurately evaluate video quality in settings that are easy for humans to assess, particularly for long videos.

Method: The method involves creating a benchmark called SLVMEval, which uses pairwise comparison and crowdsourcing to evaluate existing text-to-video evaluation systems on long videos.

Result: The result shows that human evaluators can identify better long videos with 84.7%-96.8% accuracy, while existing evaluation systems fall short of human assessment in nine out of 10 aspects.

Conclusion: The conclusion is that existing text-to-long-video evaluation systems have weaknesses and are less accurate than human evaluators, highlighting the need for improvement.
...
TL;DR: The paper proposes a method to render 3D objects from orthographic views using corner detection, envelope construction, and computational geometry.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enable 3D rendering of objects from a set of 2D orthographic views.

Method: The method involves applying the Harris Detector to input views, projecting control points to construct envelopes, and using computational geometry to regenerate surfaces.

Result: The result is a 3D rendering of the object using OpenGL.

Conclusion: The paper concludes that it is possible to render 3D objects from orthographic views using the proposed method.
...
TL;DR: The paper proposes an adaptive context compression framework for Large Language Models (LLMs) to improve performance during long-running interactions by retaining essential conversational information while controlling context growth.


<details>
  <summary>Details</summary>
Motivation: LLMs experience performance degradation during long-running interactions due to increasing context length, memory saturation, and computational overhead.

Method: The proposed framework integrates importance-aware memory selection, coherence-sensitive filtering, and dynamic budget allocation.

Result: The approach achieves consistent improvements in conversational stability and retrieval performance while reducing token usage and inference latency compared with existing memory and compression-based approaches.

Conclusion: Adaptive context compression provides an effective balance between long-term memory preservation and computational efficiency in persistent LLM interactions.
...
TL;DR: A new framework for long-horizon dialogue systems improves performance and retention.


<details>
  <summary>Details</summary>
Motivation: Long-horizon dialogue systems face issues with semantic drift and unstable memory retention.

Method: A Multi-Layer Memory Framework with adaptive retrieval gating and retention regularization.

Result: Improved performance on several metrics, including 46.85% Success Rate, 0.618 F1, and 56.90% retention.

Conclusion: The framework enhances long-term retention and reasoning stability in dialogue systems.
...
TL;DR: A novel framework, LightHarmony3D, is proposed for inserting mesh objects into 3D Gaussian Splatting scenes with consistent lighting and shadows.


<details>
  <summary>Details</summary>
Motivation: The challenge of achieving physically consistent lighting and shadows when inserting external mesh objects into reconstructed 3DGS scenes.

Method: A generative module predicts a full 360° HDR environment map at the insertion location via a single forward pass, enabling efficient and physically grounded shading and shadows.

Result: LightHarmony3D achieves state-of-the-art realism and multi-view consistency across multiple real-world reconstruction datasets.

Conclusion: The proposed LightHarmony3D framework enables realistic and interactive editing of 3DGS scenes with inserted mesh objects, facilitating applications such as AR/VR and digital content creation.
...
TL;DR: Proposed CCDNet for infrared target detection, addressing challenges of low contrast and distractions.


<details>
  <summary>Details</summary>
Motivation: Infrared target detection is challenging due to low contrast and blending into complex backgrounds, with distractors causing false alarms.

Method: Novel CCDNet with Weighted Multi-branch Perceptrons, Aggregation-and-Refinement Fusion Neck, and Contrastive-aided Distractor Discriminator.

Result: CCDNet outperforms state-of-the-art methods on infrared image datasets.

Conclusion: CCDNet effectively improves infrared target detection accuracy and reduces false alarms.
...
TL;DR: A new real-time multi-task perception model, M2H-MX, for monocular spatial understanding is proposed, achieving state-of-the-art results in dense prediction accuracy and in-the-loop system performance.


<details>
  <summary>Details</summary>
Motivation: The challenge of achieving reliable real-time spatial understanding from a single image stream in robotic perception, despite recent advances in multi-task dense prediction models.

Method: M2H-MX, a real-time multi-task perception model that preserves multi-scale feature representations and introduces register-gated global context and controlled cross-task interaction in a lightweight decoder.

Result: M2H-MX-L achieves state-of-the-art results on NYUDv2, improving semantic mIoU by 6.6% and reducing depth RMSE by 9.4% over representative multi-task baselines. It also reduces average trajectory error by 60.7% compared to a strong monocular SLAM baseline on ScanNet.

Conclusion: Modern multi-task dense prediction can be reliably deployed for real-time monocular spatial perception in robotic systems, enabling accurate and efficient spatial understanding from a single image stream.
...
TL;DR: The paper introduces Diffusion Mental Averages (DMA), a method for generating a sharp and realistic 'mental average' of a concept within a diffusion model's semantic space.


<details>
  <summary>Details</summary>
Motivation: Existing methods for averaging image collections produce blurry results when applied to diffusion samples. The authors aim to create a model-centric approach that averages within the diffusion model's semantic space.

Method: The authors cast averaging as trajectory alignment, optimizing multiple noise latents to converge towards shared coarse-to-fine semantics, and extend this approach to multimodal concepts by clustering samples in semantically-rich spaces.

Result: The proposed approach delivers consistent, realistic averages, even for abstract concepts, serving as a concrete visual summary and a lens into model biases and concept representation.

Conclusion: The paper presents a novel approach for generating sharp and realistic averages of concepts within a diffusion model, which can be used for visual summarization and understanding model biases.
...
TL;DR: The study evaluates the potential of PhiSat-2 data for monocular building height estimation and proposes a Two-Stream Ordinal Network (TSONet) that achieves state-of-the-art performance on the constructed PHDataset.


<details>
  <summary>Details</summary>
Motivation: Monocular building height estimation from optical imagery is challenging due to ambiguous height cues, large inter-city variations in building morphology, and the long-tailed distribution of building heights.

Method: The study proposes a Two-Stream Ordinal Network (TSONet) that jointly models footprint segmentation and height estimation, and introduces a Cross-Stream Exchange Module (CSEM) and a Feature-Enhanced Bin Refinement (FEBR) module.

Result: TSONet achieves the best overall performance on PHDataset, reducing MAE and RMSE by 13.2% and 9.7%, and improving IoU and F1-score by 14.0% and 10.1% over the strongest competing results.

Conclusion: The study confirms the potential of PhiSat-2 for monocular building height estimation and provides a dedicated dataset (PHDataset) and an effective method (TSONet) for future research.
...
TL;DR: The paper proposes a novel approach called Flexible Memory (FlexMem) to improve long video understanding in Multimodal Large Language Models (MLLMs).


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the challenge of long video understanding in MLLMs, which is limited by the input upper-limit of existing methods.

Method: FlexMem uses a dual-pathway compression design to enable effective memory transfer and writing, and explores different memory reading strategies for diverse video understanding tasks.

Result: The experimental results show that FlexMem achieves obvious improvements over existing efficient video understanding methods, processing over 1k frames on a single 3090 GPU, and helps base MLLMs achieve comparable or better performance than SOTA MLLMs on some benchmarks.

Conclusion: The proposed FlexMem approach enables MLLMs to achieve video understanding of infinite lengths, making it a promising solution for long video understanding tasks.
...
TL;DR: Omni-NegCLIP, a fine-tuned CLIP model, improves understanding of negation expressions in Vision-Language Models, enhancing performance on presence-based and absence-based negation tasks without sacrificing general capability.


<details>
  <summary>Details</summary>
Motivation: Recent studies show that Vision-Language Models (VLMs) like CLIP perform poorly in understanding negation expressions, which are common in natural language.

Method: Omni-NegCLIP modifies CLIP's InfoNCE contrastive loss, designing presence-based and absence-based contrastive objectives, and fine-tuning the front transformer layers of CLIP's text encoder.

Result: Omni-NegCLIP improves performance on presence-based negation and absence-based negation tasks by up to 52.65% and 12.50%, respectively, and even improves general image-text retrieval capability by up to 19.62%.

Conclusion: Omni-NegCLIP demonstrates a more comprehensive ability to understand multiple types of negation tasks, enhancing the capabilities of Vision-Language Models in understanding negation expressions.
...
TL;DR: Proposed a novel algorithm, NPAD, for bias mitigation in deep learning models without requiring protected attribute information.


<details>
  <summary>Details</summary>
Motivation: The problem of bias in deep learning models persists, and most existing bias mitigation algorithms require protected attribute information, limiting their real-world applications.

Method: NPAD algorithm utilizes non-protected attribute information and introduces two loss functions, DACL and FRL, to optimize the model for fairness.

Result: Experiments on LFWA and CelebA datasets show significant reduction in bias across different gender and age subgroups.

Conclusion: The proposed NPAD algorithm effectively mitigates bias in deep learning models without requiring protected attribute information, making it suitable for real-world applications.
...
TL;DR: A new framework called ConInfer is proposed for open-vocabulary remote sensing segmentation, which enhances segmentation consistency, robustness, and generalization by incorporating global contextual cues and modeling inter-unit semantic dependencies.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for open-vocabulary remote sensing segmentation focus on patch-level prediction accuracy, but neglect the intrinsic characteristics of remote sensing data, such as large-scale scenes with strong spatial and semantic correlations.

Method: ConInfer, a context-aware inference framework that performs joint prediction across multiple spatial units while modeling their inter-unit semantic dependencies.

Result: The proposed approach consistently surpasses state-of-the-art baselines, achieving average improvements of 2.80% and 6.13% on open-vocabulary semantic segmentation and object extraction tasks, respectively.

Conclusion: ConInfer significantly enhances segmentation consistency, robustness, and generalization in complex remote sensing environments, making it a promising solution for category-agnostic semantic understanding in remote sensing imagery.
...
TL;DR: MaskAdapt is a framework for flexible motion adaptation in humanoid control, enabling robust and adaptable motion generation under missing observations.


<details>
  <summary>Details</summary>
Motivation: The motivation is to develop a framework that can adapt to changing conditions and missing observations in humanoid control, allowing for more flexible and robust motion generation.

Method: The framework uses a two-stage residual learning paradigm, involving a mask-invariant base policy and a residual policy trained atop the base controller.
...
Conclusion: The proposed framework, MaskAdapt, provides a flexible and robust solution for motion adaptation in humanoid control, with applications in motion composition and text-driven partial goal tracking.
...
TL;DR: A new method called MELT is proposed to improve Composed Image Retrieval (CIR) by addressing frequency bias and interference from hard negative samples.


<details>
  <summary>Details</summary>
Motivation: Existing CIR methods face limitations due to frequency bias leading to 'Rare Sample Neglect' and susceptibility of similarity scores to interference from hard negative samples and noise.

Method: The proposed MELT network assigns increased attention to rare modification semantics and applies diffusion-based denoising to hard negative samples.

Result: MELT shows superior performance on two CIR benchmarks.

Conclusion: The proposed MELT method effectively addresses the limitations of existing CIR methods and improves retrieval performance.
...
TL;DR: A novel approach for deepfake attribution and detection (DFAD) using a gaze-guided CLIP model with adaptive-enhanced fine-grained language prompts, outperforming state-of-the-art by 6.56% ACC and 5.32% AUC.


<details>
  <summary>Details</summary>
Motivation: Current deepfake attribution and detection methods exhibit poor generalization to novel generative methods, and tend to assess attribution or detection performance coarsely, without considering the synergy of both tasks.

Method: A gaze-guided CLIP model with adaptive-enhanced fine-grained language prompts, using a visual perception encoder, gaze-aware image encoder (GIE), and language refinement encoder (LRE) to mine global forgery embeddings and capture general attribution patterns.

Result: The proposed model outperforms the state-of-the-art by 6.56% ACC and 5.32% AUC in average performance under attribution and detection settings, respectively.

Conclusion: The proposed gaze-guided CLIP model with adaptive-enhanced fine-grained language prompts shows improved performance in deepfake attribution and detection, particularly in generalizing to novel generative methods.
...
TL;DR: A new framework called MotionScale is proposed for reconstructing dynamic 4D scenes from monocular videos, improving accuracy and temporal consistency.


<details>
  <summary>Details</summary>
Motivation: Existing methods for neural rendering struggle to recover accurate 3D geometry and temporally consistent motion in complex environments.

Method: MotionScale, a 4D Gaussian Splatting framework, uses a scalable motion field with cluster-centric basis transformations and a progressive optimization strategy.

Result: MotionScale significantly outperforms state-of-the-art methods in both reconstruction quality and temporal stability on challenging real-world benchmarks.

Conclusion: MotionScale efficiently scales to large scenes and extended sequences while maintaining high-fidelity structural and motion coherence.
...
TL;DR: The paper adapts self-consistency to visual domains for improving LLM performance on motion graphics trajectory generation and verification.


<details>
  <summary>Details</summary>
Motivation: To improve LLM performance on visual domains, specifically motion graphics trajectory generation and verification.

Method: The approach involves sampling diverse motion trajectories from an LLM, clustering consistent trajectories, and modeling shape families with prototype trajectories and geometric transformations.

Result: The method improves LLM-based trajectory generation accuracy by 4-6% and verification precision by 11% over VLM baselines.

Conclusion: The proposed method effectively adapts self-consistency to visual domains, enhancing LLM performance in motion graphics trajectory generation and verification.
...
TL;DR: The paper proposes a bilevel meta-learning method to improve the handling of spurious correlations in deep neural networks by performing feature-space augmentations.


<details>
  <summary>Details</summary>
Motivation: Deep neural networks often rely on spurious features, making them brittle under distribution shift and on minority-group examples.

Method: A bilevel meta-learning method that performs augmentation directly in feature space to improve spurious correlation handling in the classifier head.

Result: The method achieves lower loss on hard examples and improved worst-group performance, and is highly efficient and stable, requiring only a few minutes of training on a single GPU.

Conclusion: The proposed method can effectively improve the handling of spurious correlations in deep neural networks, leading to better performance on shifted distributions and minority groups.
...
TL;DR: The paper proposes FOSCU, a method to generate synthetic MRI volumes and segmentation labels to address challenges in medical image segmentation, achieving improved performance with combined real and synthetic data.


<details>
  <summary>Details</summary>
Motivation: Medical image segmentation faces challenges such as restricted access, costly annotation, and data shortage to clinical datasets, impeding the development of robust segmentation algorithms.

Method: The proposed method, FOSCU, integrates Duo-Diffusion, a 3D latent diffusion model with ControlNet, and an enhanced 3D U-Net training pipeline to generate high-resolution, anatomically realistic synthetic MRI volumes and corresponding segmentation labels.

Result: Models trained with combined real and synthetic data yield a mean Dice score gain of 0.67% over those using only real data, and achieve a 36.4% reduction in Fréchet Inception Distance (FID).

Conclusion: The proposed FOSCU method effectively addresses challenges in medical image segmentation by generating high-quality synthetic data, leading to improved performance in segmentation tasks.
...
TL;DR: CIPHER is a deepfake detection framework that reuses and fine-tunes discriminators from image generation models to detect synthetic faces, achieving superior performance across various generative models.


<details>
  <summary>Details</summary>
Motivation: The rapid progress of generative models has increased the risk of misinformation, fraud, and identity abuse, highlighting the need for robust detectors that can identify synthetic faces across diverse models.

Method: CIPHER uses discriminators originally trained for image generation, extracting scale-adaptive features from ProGAN discriminators and temporal-consistency features from diffusion models to capture generation-agnostic artifacts.

Result: CIPHER achieves up to 74.33% F1-score across nine state-of-the-art generative models, outperforming existing ViT-based detectors by over 30% in F1-score on average, and maintains robust performance on challenging datasets.

Conclusion: CIPHER establishes a promising approach toward building more generalizable and robust deepfake detection systems, validating the effectiveness of discriminator reuse and cross-model fine-tuning.
...
TL;DR: A novel framework is proposed to address the challenges of uncertainty in real-time maps for trajectory prediction, which integrates positional and semantic uncertainties into the prediction pipeline.


<details>
  <summary>Details</summary>
Motivation: The inherent uncertainty in real-time maps, arising from positional inaccuracies and semantic errors, poses a central challenge in trajectory prediction.

Method: A dual-head architecture is employed to independently estimate semantic and positional predictions, deriving prediction variances as uncertainty indicators in an end-to-end fashion.

Result: The proposed method effectively quantifies map uncertainties and consistently improves the performance of existing trajectory prediction models across multiple metrics.

Conclusion: The proposed uncertainty-aware framework enhances the robustness of trajectory forecasts by integrating positional and semantic uncertainties into the trajectory prediction pipeline.
...
TL;DR: The paper proposes StereoVGGT, a feature backbone for stereo vision tasks, leveraging a pre-trained Visual Geometry Grounded Transformer (VGGT) and introducing a training-free feature adjustment pipeline to mitigate geometric degradation.


<details>
  <summary>Details</summary>
Motivation: The lack of explicit spatial constraints in existing stereo vision models, which hinders their performance, and the potential of VGGT as a robust backbone for stereo vision tasks.

Method: The authors propose StereoVGGT, which builds upon VGGT and introduces a training-free feature adjustment pipeline to adapt VGGT features for stereo vision tasks.

Result: StereoVGGT-based stereo matching network achieved the 1st rank among all published methods on the KITTI benchmark.

Conclusion: StereoVGGT serves as a highly effective backbone for stereo vision tasks, validating the proposed approach.
...
TL;DR: The authors propose a multimodal framework called TriDerm to improve the analysis of Recessive Dystrophic Epidermolysis Bullosa (RDEB) skin disorder by integrating wound images, clinical text, and expert knowledge.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the challenge of analyzing rare genetic skin disorders like RDEB, where off-the-shelf foundation models struggle to capture clinically meaningful features.

Method: The authors introduce TriDerm, a multimodal framework that integrates wound imagery, boundary masks, and expert reports. It uses visual foundation models with wound-level attention pooling and non-contrastive representation learning, and prompts large language models with comparison queries and soft ordinal embeddings (SOE).

Result: The proposed framework achieves 73.5% agreement with experts, outperforming the best off-the-shelf single-modality foundation model by over 5.6 percentage points.

Conclusion: The authors conclude that their multimodal framework, TriDerm, effectively captures clinically meaningful features for RDEB analysis and demonstrate the value of integrating visual and textual modalities.
...
TL;DR: A new method for detecting AI-edited images is proposed, leveraging a large-scale dataset and a novel network architecture, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: The rapid growth of AI image editing has increased the risk of malicious content fabrication and misinformation, highlighting the need for effective forgery localization methods.

Method: A fully automated mask annotating framework, a large-scale dataset (PromptForge-350k), and a novel forgery localization network (ICL-Net) with a triple-stream backbone and intra-image contrastive learning.

Result: The proposed method achieves an IoU of 62.5% on PromptForge-350k, outperforming SOTA methods by 5.1%, and shows strong robustness and generalization capabilities.

Conclusion: The proposed method effectively addresses the challenge of detecting AI-edited images, offering a robust and generalizable solution for forgery localization.
...
TL;DR: Extend3D is a training-free pipeline for 3D scene generation from a single image, leveraging an object-centric 3D generative model with extended latent space and patch-wise generation.


<details>
  <summary>Details</summary>
Motivation: The limitations of fixed-size latent spaces in object-centric models for representing wide scenes.

Method: Extend3D extends the latent space in the x and y directions, divides it into overlapping patches, and applies the object-centric 3D generative model to each patch. It also uses a point cloud prior from a monocular depth estimator and refines occluded regions through SDEdit.

Result: The method yields better results than prior methods, as evidenced by human preference and quantitative experiments.

Conclusion: The proposed Extend3D pipeline effectively generates 3D scenes from a single image, overcoming the limitations of object-centric models and achieving improved geometric structure and texture fidelity.
...
TL;DR: A new feed-forward 3D Gaussian Splatting model called AA-Splat is proposed, enabling robust anti-aliased rendering at any resolution.


<details>
  <summary>Details</summary>
Motivation: Existing FF-3DGS methods have rendering artifacts due to incorrect screen-space dilation filters, especially at out-of-distribution sampling rates.

Method: AA-Splat uses opacity-balanced band-limiting (OBBL) design with two components: 3D band-limiting post-filter and Opacity Balancing (OB).

Result: AA-Splat shows 5.4-7.5dB PSNR gains on NVS performance over DepthSplat at all resolutions between 4x and 1/4x.

Conclusion: AA-Splat provides a robust solution for sparse-view 3D reconstruction and novel view synthesis with improved rendering quality.
...
TL;DR: A new framework called HIRE is proposed to detect and eliminate hallucinations in large vision-language models with minimal additional computational cost, achieving state-of-the-art performance on existing benchmarks.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the hallucination issues in large vision-language models, which contradict visual facts, and to overcome the limitations of existing methods that require substantial training resources or introduce dual inference overhead.

Method: The proposed method is a framework for dynamically detecting hallucination representations and performing hallucination-eliminating edits on these representations.

Result: The approach achieves state-of-the-art performance on existing benchmarks with minimal additional computational cost and demonstrates its effectiveness, efficiency, and robust hallucination elimination capability.

Conclusion: The proposed framework HIRE offers an efficient and robust solution for hallucination elimination in large vision-language models, making it a practical approach for multimodal reasoning and complex scene understanding.
...
TL;DR: Proposed Alignment-Guided Fine-Tuning (AGFT) framework enhances zero-shot adversarial robustness in pre-trained vision-language models without disrupting cross-modal alignment.


<details>
  <summary>Details</summary>
Motivation: Existing adversarial fine-tuning methods disrupt pre-trained cross-modal alignment, weakening visual-textual correspondence and degrading zero-shot performance.

Method: AGFT uses probabilistic predictions for text-guided adversarial training, aligning adversarial visual features with textual embeddings via soft alignment distributions, and introduces distribution consistency calibration to adjust model output.

Result: AGFT outperforms state-of-the-art methods and significantly improves zero-shot adversarial robustness across multiple benchmarks.

Conclusion: The proposed AGFT framework effectively enhances zero-shot adversarial robustness while preserving cross-modal semantic structure in pre-trained vision-language models.
...
TL;DR: A novel extrinsic-aware cross-attention framework is proposed for accurate camera-LiDAR fusion, achieving state-of-the-art performance in calibration accuracy and robustness.


<details>
  <summary>Details</summary>
Motivation: Existing learning-based methods for camera-LiDAR fusion rely on projecting LiDAR points into depth maps, which distorts 3D geometry and degrades performance under large misalignments.

Method: An extrinsic-aware cross-attention framework that directly aligns image patches and LiDAR point groups in their native domains, injecting extrinsic parameter hypotheses into the correspondence modeling process.

Result: The proposed method consistently outperforms state-of-the-art approaches in both accuracy and robustness, achieving accurate calibration in 88% of KITTI cases and 99% of nuScenes cases under large extrinsic perturbations.

Conclusion: The proposed extrinsic-aware cross-attention framework demonstrates superior performance in camera-LiDAR calibration, offering a more accurate and robust solution for cross-modal fusion.
...
### [54] [Less Is More? Selective Visual Attention to High-Importance Regions for Multimodal Radiology Summarization](https://arxiv.org/abs/2603.29901)
*Mst. Fahmida Sultana Naznin,Adnan Ibney Faruq,Mushfiqur Rahman,Niloy Kumar Mondal,Md. Mehedi Hasan Shawon,Md Rakibul Hasan*

Main category: cs.CV

TL;DR: The paper proposes a new approach to automated radiology report summarization, introducing ViTAS, a multi-stage pipeline that selectively focuses on pathology-relevant visual patches and achieves state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal models for radiology report summarization struggle with visual noise and fail to significantly improve over text-only baselines, leading to questioning of two prevailing assumptions: (1) more visual input is always better, and (2) multimodal models add limited value when findings contain rich image-derived detail.

Method: The authors introduce ViTAS, a multi-stage pipeline that combines ensemble-guided MedSAM2 lung segmentation, bidirectional cross-attention for multi-view fusion, Shapley-guided adaptive patch clustering, and hierarchical visual tokenization feeding a ViT, selectively focusing on pathology-relevant visual patches.

Result: ViTAS achieves state-of-the-art results with 29.25% BLEU-4 and 69.83% ROUGE-L, improved factual alignment in qualitative analysis, and the highest expert-rated human evaluation scores.

Conclusion: The findings demonstrate that less but more relevant visual input is not only sufficient but superior for multimodal radiology summarization.
...
TL;DR: This paper presents a method for imperceptible visual prompt injection attacks on multimodal large language models (MLLMs), embedding malicious prompts into images to manipulate model behavior.


<details>
  <summary>Details</summary>
Motivation: The instruction-following behavior of MLLMs makes them vulnerable to prompt injection attacks, and existing methods rely on observable textual or visual prompts, which this work aims to overcome.

Method: The proposed method adaptively embeds malicious prompts into input images via bounded text overlays and optimizes imperceptible visual perturbations to align feature representations with malicious targets.

Result: The approach demonstrates superior performance in experiments on two multimodal understanding tasks across multiple closed-source MLLMs, compared to existing methods.

Conclusion: The study highlights the vulnerability of MLLMs to imperceptible visual prompt injection attacks and proposes an effective method to exploit this vulnerability.
...
TL;DR: The study examines the use of multimodal models for Presentation Attack Detection (PAD) on ID Documents, combining visual and textual modalities, but finds that these models struggle to accurately detect PAD.


<details>
  <summary>Details</summary>
Motivation: Traditional PAD systems rely solely on visual features, which often fail to detect sophisticated spoofing attacks, prompting the exploration of multimodal models.

Method: The study utilizes pre-trained multimodal models, such as Paligemma, Llava, and Qwen, merging deep visual embeddings with contextual metadata.

Result: The experimental results indicate that these models struggle to accurately detect PAD on ID Documents.

Conclusion: The integration of multimodal models into PAD for ID Documents shows potential but requires further improvement to effectively detect presentation attacks.
...
TL;DR: Proposed A$^2$BFR, an attribute-aware blind face restoration framework that unifies high-fidelity reconstruction with prompt-controllable generation.


<details>
  <summary>Details</summary>
Motivation: Blind face restoration is inherently ill-posed, leading to ambiguous solutions. Recent methods improve perceptual quality but lack control, while text-guided editing enables manipulation without reliable restoration.

Method: A$^2$BFR uses a Diffusion Transformer backbone with unified image-text cross-modal attention, attribute-aware learning, and semantic dual-training.

Result: A$^2$BFR achieves state-of-the-art performance in restoration fidelity and instruction adherence, outperforming baselines by -0.0467 LPIPS and +52.58% attribute accuracy.

Conclusion: A$^2$BFR enables fine-grained, prompt-controllable restoration even under severe degradations, addressing issues in blind face restoration.
...
TL;DR: The authors propose a tool-guided inference framework to address the failure of vision-language models (VLMs) on optical illusions without requiring model training.


<details>
  <summary>Details</summary>
Motivation: VLMs exhibit a systematic bias when confronted with classic optical illusions, predicting the illusion as 'real' regardless of counterfactual modifications.

Method: The framework uses off-the-shelf VLMs with access to generic image manipulation tools and an illusion-type-routing system to prescribe tool invocation.

Result: The approach yields strong cross-structural generalization, with consistent performance on validation and test sets containing structurally unfamiliar illusion variants.

Conclusion: The proposed framework addresses the failure mode of VLMs on optical illusions without model training, and reveals empirical observations warranting further investigation.
...
TL;DR: SeGPruner, a token reduction framework, improves efficiency in 3D question answering by preserving semantically salient tokens and spatially diverse ones.


<details>
  <summary>Details</summary>
Motivation: The need to reduce token redundancy in vision-language models for 3D question answering to improve inference efficiency.

Method: SeGPruner uses a Saliency-aware Token Selector and a Geometry-aware Token Diversifier to select tokens.

Result: SeGPruner reduces visual token budget by 91% and inference latency by 86% while maintaining competitive performance.

Conclusion: SeGPruner effectively balances object-level evidence and global scene coverage, leading to improved efficiency in 3D question answering.
...
TL;DR: The paper introduces EarthEmbeddingExplorer, a web app that makes Earth observation models and data accessible for practical use.


<details>
  <summary>Details</summary>
Motivation: The Earth observation community has made significant progress in foundation models and global Earth embedding datasets, but these are not easily accessible for practical use.

Method: The authors provide a hands-on guide to EarthEmbeddingExplorer, a cloud-native software application that enables interactive queries and scientific insights.

Result: The web application democratizes access to precomputed Earth embeddings, allowing researchers to apply state-of-the-art models to real-world problems.

Conclusion: The tutorial empowers researchers to transition from academic assets to practical workflows, making Earth observation models and data more accessible.
...
TL;DR: A 3D deep learning framework called NeoNet is proposed for predicting perineural invasion (PNI) in cholangiocarcinoma, achieving a maximum AUC of 0.7903.


<details>
  <summary>Details</summary>
Motivation: The goal is to minimize invasive diagnostic procedures and address the challenge of noninvasive diagnosis of PNI, a critical prognostic factor, due to the lack of clear imaging criteria.

Method: NeoNet integrates three modules: NeoSeg with Tumor-Localized ROI Crop (TLCR) algorithm, NeoGen with a 3D Latent Diffusion Model (LDM) and ControlNet, and NeoCls with the PNI-Attention Network (PattenNet) and 3D Dual Attention Blocks (DAB).

Result: NeoNet achieved the highest performance with a maximum AUC of 0.7903 in 5-fold cross-validation, outperforming baseline 3D models.

Conclusion: The proposed NeoNet framework shows promising results for noninvasive PNI prediction in cholangiocarcinoma, potentially reducing the need for invasive diagnostic procedures.
...
TL;DR: The paper proposes a novel context-driven Handwritten Text Recognition (HTR) framework that enables writer adaptation at inference time using only a few examples, without requiring parameter updates or fine-tuning.


<details>
  <summary>Details</summary>
Motivation: State-of-the-art HTR models struggle with writers exhibiting highly specific styles underrepresented in training data. Existing writer adaptation methods require offline fine-tuning or parameter updates, which increase computational costs and demand careful hyperparameter tuning.

Method: The proposed framework uses multimodal in-context learning, allowing for inference-time writer adaptation with a few examples from the target writer. A compact 8M-parameter CNN-Transformer is designed for few-shot in-context adaptation.

Result: Experiments on IAM and RIMES achieve Character Error Rates of 3.92% and 2.34%, respectively, surpassing all writer-independent HTR models without requiring parameter updates at inference time.

Conclusion: The proposed context-driven HTR framework enables effective writer adaptation at inference time, achieving state-of-the-art results on IAM and RIMES benchmarks without requiring parameter updates or fine-tuning.
...
TL;DR: A novel federated prototype learning method called FedDBP is proposed to address the challenges of data and model heterogeneity in heterogeneous federated learning.


<details>
  <summary>Details</summary>
Motivation: Existing federated prototype learning methods fail to balance the fidelity and discriminability of features and are limited by a single global prototype.

Method: FedDBP uses a Dual-Branch feature projector on the client-side and a Personalized global prototype fusion approach on the server-side, leveraging Fisher information to identify important channels of local prototypes.

Result: Extensive experiments demonstrate the superiority of FedDBP over ten existing advanced methods.

Conclusion: FedDBP effectively alleviates the challenges of data and model heterogeneity in heterogeneous federated learning by balancing feature fidelity and discriminability and using a personalized global prototype fusion approach.
...
TL;DR: The paper proposes a method to generate square superpixels for efficient parallel processing and learnable feature extraction, which can be integrated with graph neural networks or Vision Transformers.


<details>
  <summary>Details</summary>
Motivation: Existing superpixel algorithms produce irregularly shaped regions that are not well aligned with regular operators, limiting parallel implementation and hindering end-to-end optimization in deep learning pipelines.

Method: The approach approximates superpixels using multi-scale square blocks, computing a purity score based on pixel-intensity similarity, and selecting high-quality blocks.

Result: The proposed method demonstrates consistent performance improvements on downstream tasks, validating its effectiveness.

Conclusion: The proposed square superpixel generation approach enables efficient parallel processing and learnable feature extraction, facilitating multi-scale information aggregation and structured visual representation.
...
TL;DR: VecAttention, a novel sparse attention framework for video models, achieves better accuracy-efficiency trade-offs by leveraging a vertical-vector sparse pattern.


<details>
  <summary>Details</summary>
Motivation: Existing sparse attention methods for Transformer-based video models are inefficient and suboptimal due to coarse-grained patterns, leading to redundant computation.

Method: VecAttention uses a lightweight important-vector selection and optimized kernel of vector sparse attention to dynamically select and process informative vertical vectors.

Result: VecAttention delivers a 2.65x speedup over full attention and 1.83x speedup over state-of-the-art sparse attention methods with comparable accuracy.

Conclusion: VecAttention offers superior accuracy-efficiency trade-offs for video models, making it a promising solution for long-context video understanding and generation tasks.
...
TL;DR: An augmented reality system was developed to improve the accuracy of intraoperative re-excision in head and neck squamous cell carcinoma by relocalizing positive margins.


<details>
  <summary>Details</summary>
Motivation: Current methods of communicating margin locations verbally from pathology to the operating room are imprecise, leading to common positive margins in head and neck squamous cell carcinoma.

Method: An all-in-one augmented reality (AR) system using HoloLens 2 depth sensing and fully automated markerless surface registration was developed and tested in a silicone phantom study with six medical trainees.

Result: The AR system achieved comparable target registration errors to a marker-based baseline (median 1.8 mm vs. 1.7 mm) and reduced error in margin relocalization from verbal guidance (median 14.2 mm) to a few millimeters (median 3.2 mm).

Conclusion: The results support the feasibility of markerless AR margin guidance for more precise intraoperative re-excision in head and neck squamous cell carcinoma.
...
TL;DR: The paper proposes a two-stage nighttime image dehazing framework that addresses multiple degradation factors caused by haze, including low visibility, color distortion, and reduced contrast.


<details>
  <summary>Details</summary>
Motivation: Existing nighttime dehazing methods only partially address the issues caused by haze, and the paper aims to jointly tackle the full spectrum of degradation factors.

Method: The proposed framework consists of two stages: (1) transmittance correction with boundary-constrained initial transmittance maps, region-adaptive compensation, and normalization; (2) STAR-YUV decomposition model for structure-texture layered optimization, with gamma correction, MSRCR-based color restoration, and Laplacian-of-Gaussian filtering.

Result: The proposed framework produces a dehazed image with improved visibility, color accuracy, and contrast.

Conclusion: The paper presents a comprehensive approach to nighttime image dehazing, integrating transmittance correction with structure-texture layered optimization, and demonstrates its effectiveness in addressing multiple degradation factors.
...
TL;DR: A unified framework for multi-task GenAI inference on edge devices using a single shared model, reducing memory footprint and latency.


<details>
  <summary>Details</summary>
Motivation: Deploying Large Vision Models (LVMs) on resource-constrained devices is challenging due to high memory and compute requirements, and existing Mobile deployment pipelines have redundant storage and increased runtime overhead.

Method: Treating LoRA weights as runtime inputs and introducing QUAD (Quantization with Unified Adaptive Distillation), a quantization-aware training strategy.

Result: Up to 6x reduction in memory footprint and 4x latency improvements while maintaining high visual quality across multiple GenAI tasks.

Conclusion: The proposed system enables efficient multi-task GenAI inference on edge devices using a single shared model, with significant reductions in memory footprint and latency.
...
TL;DR: A pose-aware generative framework is proposed to accurately generate Bharatanatyam postures while maintaining anatomical and stylistic integrity.


<details>
  <summary>Details</summary>
Motivation: Preserving intangible cultural dances like Bharatanatyam in the digital era presents unique challenges due to their strict structural and symbolic rules.

Method: A pose-aware generative framework integrated with a pose estimation module, guided by keypoint-based loss and pose consistency constraints, is proposed and evaluated with four configurations: standard cGAN, cGAN with pose supervision, conditional diffusion, and conditional diffusion with pose supervision.

Result: Incorporating pose supervision significantly enhances the quality, realism, and authenticity of generated Bharatanatyam postures, promoting cultural fidelity.

Conclusion: The proposed framework provides a scalable approach for the digital preservation, education, and dissemination of traditional dance forms, enabling high-fidelity generation without compromising cultural precision.
...
TL;DR: The paper proposes Adaptive Margin Discrepancy Training (AMDiT) for improving the accuracy and robustness of Emotion Diffusion Classifier (EmoDC) in Facial Expression Recognition (FER) tasks.


<details>
  <summary>Details</summary>
Motivation: Existing deep-learning-based FER methods are vulnerable to distribution shifts and tend to learn shortcuts. The authors aim to address this issue by adopting a conditional generative diffusion model and introducing EmoDC, which demonstrates enhanced adversarial robustness.

Method: The authors propose AMDiT, which dynamically adjusts the margin for each sample to improve the discriminative capability of EmoDC. This approach builds upon margin-based discrepancy training and overcomes the limitation of using a fixed margin.

Result: Extensive experiments show that AMDiT significantly improves the accuracy of EmoDC over the Base model on several datasets, including RAF-DB, SFEW-2.0, and AffectNet. Additionally, EmoDC outperforms state-of-the-art discriminative classifiers in terms of robustness against noise and blur.

Conclusion: The proposed AMDiT approach enhances the accuracy and robustness of EmoDC in FER tasks, demonstrating improved performance over existing methods.
...
TL;DR: A new method called FlowID is proposed for identity-preserving facial reconstruction of damaged faces, leveraging image generation models to remove artifacts while retaining identity information.


<details>
  <summary>Details</summary>
Motivation: The need for efficient and effective facial reconstruction of deceased individuals who have died under violent circumstances, where traditional methods are lengthy and produce suboptimal results.

Method: FlowID combines single-image fine-tuning and attention-based masking to adapt a generative model to out-of-distribution injured faces and localize edits to damaged regions.

Result: FlowID outperforms state-of-the-art open-source methods on the InjuredFaces benchmark, with low memory requirements making it suitable for local deployment.

Conclusion: FlowID provides a effective solution for identity-preserving facial reconstruction, which can support identification of deceased individuals and provide a standardized resource for the community to study and compare methods.
...
TL;DR: The authors propose Video-Oasis, a diagnostic suite to evaluate video understanding tasks, revealing that many benchmarks can be solved without visual or temporal context and state-of-the-art models perform barely better than random guessing on the remaining samples.


<details>
  <summary>Details</summary>
Motivation: The complexity of video understanding makes it hard to determine whether performance gains come from visual perception, linguistic reasoning, or knowledge priors, and existing benchmarks overlook essential criteria for video understanding.

Method: The authors introduce Video-Oasis, a diagnostic suite to systematically evaluate existing evaluations and distill spatio-temporal challenges for video understanding, analyzing existing benchmarks and state-of-the-art models.

Result: The analysis reveals that 54% of existing benchmark samples can be solved without visual input or temporal context, and state-of-the-art models exhibit performance barely exceeding random guessing on the remaining samples.

Conclusion: The authors provide practical guidelines for future research, aiming to serve as a standard guideline for benchmark construction and the rigorous evaluation of architecture development in video understanding.
...
TL;DR: The paper presents Unify-Agent, a unified multimodal model that uses agentic modeling to improve world-grounded image synthesis by incorporating external knowledge and reasoning.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the limitation of current multimodal models in generating high-quality images involving long-tail and knowledge-intensive concepts by leveraging agentic modeling.

Method: The authors propose Unify-Agent, which reframed image generation as an agentic pipeline consisting of prompt understanding, multimodal evidence searching, grounded recaptioning, and final synthesis. They also construct a tailored multimodal data pipeline and curate 143K high-quality agent trajectories for world-grounded image synthesis.

Result: The proposed Unify-Agent substantially improves over its base unified model across diverse benchmarks and real-world generation tasks, approaching the world knowledge capabilities of the strongest closed-source models.

Conclusion: The paper highlights the value of tightly coupling reasoning, searching, and generation for reliable open-world agentic image synthesis, and demonstrates the potential of agentic modeling for improving multimodal models.
...
TL;DR: A new large-scale dataset, BigEarthNet.txt, is introduced to improve vision-language models in remote sensing, offering diverse textual annotations and outperforming existing datasets.


<details>
  <summary>Details</summary>
Motivation: The performance of vision-language models on remote sensing data is limited by the lack of large-scale, multi-sensor datasets with diverse textual annotations.

Method: A new dataset, BigEarthNet.txt, is created with 464,044 co-registered Sentinel-1 and Sentinel-2 images and 9.6M text annotations, including geographically anchored captions, visual question answering pairs, and referring expression detection instructions.

Result: The results show that vision-language models have limitations on tasks involving complex land-use/land-cover classes, but fine-tuning with BigEarthNet.txt leads to consistent performance gains across all considered tasks.

Conclusion: BigEarthNet.txt advances instruction-driven image-text learning in Earth observation, providing a benchmark for evaluating vision-language models in remote sensing and computer vision.
...
TL;DR: The paper presents a streaming retrieval architecture for edge cameras that filters out redundant frames, improving cross-modal retrieval efficiency and accuracy.


<details>
  <summary>Details</summary>
Motivation: Redundant frames in continuous video streams from always-on edge cameras can degrade cross-modal retrieval performance.

Method: An on-device epsilon-net filter retains only semantically novel frames, and a cross-modal adapter and cloud re-ranker enhance the compact encoder's alignment.

Result: The architecture achieves 45.6% Hit@5 on held-out data using an 8M on-device encoder at an estimated 2.7 mW power consumption.

Conclusion: The proposed streaming retrieval architecture effectively reduces redundant frames and improves cross-modal retrieval performance on edge cameras.
...
TL;DR: The paper investigates stage-specific data heterogeneity in self-supervised federated learning (SSFL) for label-scarce visual classification, proposing a new partitioning scheme and a Prevalence-based Personalized Weighted Federated Learning (PreP-WFL) method.


<details>
  <summary>Details</summary>
Motivation: Label-scarce visual classification under decentralized and heterogeneous data is a fundamental challenge in pattern recognition, especially with partially overlapping class sets across sites.

Method: The paper proposes a new partitioning scheme called PreDi, which disentangles label-space heterogeneity into class Prevalence and class-set size Disparity, and a Prevalence-based Personalized Weighted Federated Learning (PreP-WFL) method to adaptively strengthen rare-class representations.

Result: SSFL consistently outperforms local-only training under both homogeneous and heterogeneous settings. The results show that prevalence dominates performance and disparity has a smaller effect under label-space heterogeneity. PreP-WFL effectively mitigates performance degradation, with gains increasing as prevalence decreases.

Conclusion: The findings provide a mechanistic basis for characterizing label-space heterogeneity in decentralized recognition systems, and the proposed PreP-WFL method can be used to improve the performance of SSFL in label-scarce visual classification tasks.
...
TL;DR: The paper introduces MacTok, a masked augmenting 1D continuous tokenizer that prevents posterior collapse in image tokenization, achieving efficient and high-fidelity results with reduced token usage.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the issue of posterior collapse in variational image tokenizers when using fewer tokens, which leads to the encoder failing to encode informative features into the compressed latent space.

Method: The method involves introducing MacTok, which leverages image masking and representation alignment to prevent collapse. It applies random masking and DINO-guided semantic masking to emphasize informative regions in images.

Result: The results show that MacTok achieves a competitive gFID of 1.44 at 256×256 and a state-of-the-art 1.52 at 512×512 with SiT-XL, while reducing token usage by up to 64×.

Conclusion: The conclusion is that MacTok successfully prevents posterior collapse and achieves efficient, high-fidelity tokenization by combining masking and semantic guidance.
...
TL;DR: The paper proposes DynMask, a method to improve masked motion generation by respecting local motion complexity, leading to better results on dynamically complex motions and overall FID on HumanML3D and KIT-ML.


<details>
  <summary>Details</summary>
Motivation: Current masked motion generators treat motion frames too uniformly, which is a poor match for motion where local dynamic complexity varies sharply over time, leading to degradation on dynamically complex motions.

Method: The paper introduces the Motion Spectral Descriptor (MSD) to measure local dynamic complexity and uses it to guide content-focused masking, provide a spectral similarity prior for self-attention, and modulate token-level sampling during decoding.

Result: DynMask improves motion generation on dynamically complex motions and yields stronger overall FID on HumanML3D and KIT-ML compared to baseline masked motion generators.

Conclusion: The paper concludes that respecting local motion complexity is a useful design principle for masked motion generation, and that DynMask is an effective approach to achieve this.
...
TL;DR: CutClaw is an autonomous framework that uses multiple multimodal language models to edit long raw footage into short videos with synchronized music and a visually appealing appearance.


<details>
  <summary>Details</summary>
Motivation: The manual video editing process is time-consuming and repetitive, and there is a need for an autonomous system to assist filmmakers and content creators.

Method: CutClaw uses a hierarchical multimodal decomposition, a Playwriter Agent for narrative consistency, and Editor and Reviewer Agents to collaboratively optimize the final cut.

Result: CutClaw significantly outperforms state-of-the-art baselines in generating high-quality, rhythm-aligned videos.

Conclusion: CutClaw offers a promising solution for automating the video editing process, enabling the creation of high-quality short videos with synchronized music and a visually appealing appearance.
...
TL;DR: The authors propose a novel unsupervised domain adaptation framework, CoRe-DA, for vision-based surgical skill assessment, achieving better cross-domain generalization than existing methods.


<details>
  <summary>Details</summary>
Motivation: The high cost and time demands for manual annotation of quantitative skill scores and the poor generalization of existing regression models to new surgical tasks and environments motivate the development of unsupervised domain adaptation methods for surgical skill assessment.

Method: The authors propose CoRe-DA, a contrastive regression-based adaptation framework that learns domain-invariant representations through relative-score supervision and target-domain self-training.

Result: CoRe-DA achieves Spearman Correlation Coefficients of 0.46 and 0.41 on dry-lab and clinical target datasets, respectively, without using any labeled target data for training.

Conclusion: CoRe-DA enables scalable surgical skill assessment with reliable cross-domain generalization, outperforming existing methods.
...
TL;DR: A new loss function, CDM loss, is proposed to improve 3D dose prediction in radiotherapy for head and neck cancer by directly optimizing clinically used DVH metrics, resulting in better target coverage and reduced training time.


<details>
  <summary>Details</summary>
Motivation: Current deep-learning-based 3D dose prediction models are trained with voxel-wise regression losses that do not align with clinical plan evaluation criteria based on dose-volume histogram (DVH) metrics.

Method: A clinical DVH metric loss (CDM loss) is proposed, incorporating differentiable D-metrics and surrogate V-metrics, along with lossless bit-mask ROI encoding to improve training efficiency.

Result: The CDM loss substantially improved target coverage and satisfied all clinical constraints, reducing the PTV Score from 1.544 to 0.491, while OAR sparing remained comparable, and bit-mask encoding reduced training time by 83% and lowered GPU memory usage.

Conclusion: Directly optimizing clinically used DVH metrics enables 3D dose predictions that are better aligned with clinical treatment planning criteria, and the proposed CDM loss provides a practical and scalable framework for head and neck dose prediction.
...
TL;DR: The paper proposes SkeletonContext, a framework for zero-shot skeleton-based action recognition that incorporates contextual semantics from language models to improve cross-modal alignment and action understanding.


<details>
  <summary>Details</summary>
Motivation: The absence of contextual cues, such as objects involved in the action, creates a gap between skeleton and semantic representations, making it hard to distinguish visually similar actions.

Method: The proposed framework, SkeletonContext, includes a Cross-Modal Context Prompt Module that uses a pretrained language model to reconstruct masked contextual prompts, and a Key-Part Decoupling Module to decouple motion-relevant joint features.

Result: SkeletonContext achieves state-of-the-art performance on multiple benchmarks under both conventional and generalized zero-shot settings.

Conclusion: The proposed framework effectively transfers linguistic context to the skeleton encoder, improving cross-modal alignment and action understanding, especially for fine-grained, visually similar actions.
...
TL;DR: The study evaluates the impact of skin tone on AI-driven skin lesion segmentation, finding that low lesion-skin contrast is a key driver of segmentation errors, and proposes a continuous pigment analysis method.


<details>
  <summary>Details</summary>
Motivation: Early detection of skin cancer, particularly melanoma, is critical, and AI-driven dermatology systems rely on skin lesion segmentation as a preprocessing step. However, fairness concerns regarding skin tone have been under-quantified in the segmentation stage.

Method: The study uses three strong segmentation architectures (UNet, DeepLabV3 with a ResNet50 backbone, and DINOv2) on two public dermoscopic datasets (HAM10000 and ISIC2017) and introduces a continuous pigment or contrast analysis using Wasserstein distances.

Result: The study finds that global skin tone metrics show weak association with segmentation quality, while low lesion-skin contrast is consistently associated with larger segmentation errors in models.

Conclusion: The findings suggest that fairness improvements in dermoscopic segmentation should prioritize robust handling of low-contrast lesions, and the distribution-based pigment measures provide a more informative audit signal than discrete skin-tone categories.
...
TL;DR: The paper proposes FED-Bench, a comprehensive benchmark for facial expression image editing, along with a new evaluation protocol called FED-Score, to address the limitations of existing benchmarks and evaluation metrics.


<details>
  <summary>Details</summary>
Motivation: The authors identified gaps in existing facial expression image editing benchmarks, including a lack of high-quality images and editing instructions, as well as biased evaluation metrics that favor lazy or overfit editing.

Method: The authors propose a cascaded and scalable pipeline to construct a benchmark of 747 triplets, each comprising an original image, an editing instruction, and a ground-truth image. They also introduce FED-Score, a cross-granularity evaluation protocol that assesses three dimensions: Alignment, Fidelity, and Relative Expression Gain.

Result: The authors benchmark 18 image editing models and find that current approaches struggle to achieve high fidelity and accurate expression manipulation. They also demonstrate the effectiveness of their benchmark and evaluation protocol by fine-tuning a baseline model, which achieves significant performance gains.

Conclusion: The paper concludes that FED-Bench and FED-Score provide a comprehensive and accurate evaluation suite for facial expression image editing, which can help to advance the field and improve the performance of image editing models.
...
### [85] [Leveraging Synthetic Data for Enhancing Egocentric Hand-Object Interaction Detection](https://arxiv.org/abs/2603.29733)
*Rosario Leonardi,Antonino Furnari,Francesco Ragusa,Giovanni Maria Farinella*

Main category: cs.CV

TL;DR: Synthetic data improves hand-object interaction detection in egocentric images, especially when real labeled data is scarce.


<details>
  <summary>Details</summary>
Motivation: To explore the role of synthetic data in improving hand-object interaction detection from egocentric images.

Method: Extensive experimentation and comparative analysis on multiple datasets using synthetic data and a small portion of real labeled data.

Result: Improvements in Overall AP of +5.67%, +8.24%, and +11.69% on VISOR, EgoHOS, and ENIGMA-51 datasets respectively.

Conclusion: Synthetic data can significantly improve hand-object interaction detection, and better alignment with real-world benchmarks enhances its effectiveness.
...
### [86] [GRVS: a Generalizable and Recurrent Approach to Monocular Dynamic View Synthesis](https://arxiv.org/abs/2603.29734)
...
TL;DR: A new model for synthesizing novel views from monocular videos of dynamic scenes, outperforming scene-specific and diffusion-based approaches.


<details>
  <summary>Details</summary>
Motivation: Existing methods for novel view synthesis from monocular videos struggle with dynamic scenes, suffering from geometric inconsistencies and requiring substantial computational resources.

Method: A generalizable model with a recurrent loop for asynchronous mapping and efficient plane sweeps to disentangle camera and scene motion, enabling fine-grained camera controls.

Result: The proposed model outperforms four scene-specific and two diffusion-based approaches in reconstructing fine-grained geometric details across static and dynamic regions on two datasets.

Conclusion: The proposed model offers improved performance and efficiency in synthesizing novel views from monocular videos of dynamic scenes.
...
### [87] [SHIFT: Stochastic Hidden-Trajectory Deflection for Removing Diffusion-based Watermark](https://arxiv.org/abs/2603.29742)
*Rui Bao,Zheng Gao,Xiaoyu Li,Xiaoyan Feng,Yang Song,Jiaojiao Jiang*

Main category: cs.CV

TL;DR: A new attack method called SHIFT is proposed to bypass diffusion-based watermarking techniques by manipulating the generative trajectory in latent space, achieving high success rates without requiring specific watermark knowledge or model retraining.


<details>
  <summary>Details</summary>
Motivation: The reliance on trajectory recovery in diffusion-based watermarking methods constitutes a fundamental vulnerability that can be exploited.

Method: The proposed method, SHIFT, leverages stochastic diffusion resampling to deflect the generative trajectory in latent space.

Result: SHIFT achieves 95%--100% attack success rates with nearly no loss in semantic quality across nine representative watermarking methods.

Conclusion: The proposed attack method, SHIFT, effectively bypasses various diffusion-based watermarking techniques by exploiting their common weakness, posing a significant threat to their reliability.
...
TL;DR: The paper introduces TSHA, a comprehensive benchmark for indoor safety hazards assessment using vision-language models, addressing limitations of existing benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for indoor safety hazards assessment have limitations, including reliance on synthetic datasets, oversimplified safety tasks, and lack of rigorous evaluation protocols.

Method: The authors introduce TSHA, a benchmark comprising 81,809 training samples from four sources, and a challenging test set with 1707 samples, and evaluate 23 popular vision-language models.

Result: Current vision-language models lack robust capabilities for safety hazard assessment, but models trained on TSHA achieve significant performance improvement and enhanced generalizability.

Conclusion: The TSHA benchmark substantially contributes to the development of robust vision-language models for indoor safety hazards assessment.
...
### [89] [Beyond Ground-Truth: Leveraging Image Quality Priors for Real-World Image Restoration](https://arxiv.org/abs/2603.29773)
*Fengyang Xiao,Peng Hu,Lei Xu,XingE Guo,Guanyi Qin,Yuqi Shen,Chengyu Fang,Rihan Zhang,Chunming He,Sina Farsiu*

Main category: cs.CV

TL;DR: A novel framework, IQPIR, is proposed for real-world image restoration. It uses an Image Quality Prior (IQP) extracted from pre-trained No-Reference Image Quality Assessment (NR-IQA) models to guide the restoration process toward perceptually optimal outputs.


<details>
  <summary>Details</summary>
Motivation: Existing image restoration methods depend on ground-truth (GT) supervision, which can contain images with inconsistent perceptual fidelity. This causes models to converge to the average quality level of the training data rather than achieving the highest perceptual quality attainable.

Method: The IQPIR framework integrates IQP with a learned codebook prior through three key mechanisms: a quality-conditioned Transformer, a dual-branch codebook structure, and a discrete representation-based quality optimization strategy.

Result: The proposed method surpasses cutting-edge methods in real-world image restoration and serves as a generalizable quality-guided enhancement strategy for existing methods.

Conclusion: The IQPIR framework effectively addresses the limitations of existing image restoration methods by introducing an Image Quality Prior to guide the restoration process toward perceptually optimal outputs.
...
TL;DR: A hybrid edge-based system for detecting violent behavior in public spaces, combining skeleton-based motion analysis and vision-language models for real-time, privacy-aware monitoring.


<details>
  <summary>Details</summary>
Motivation: The need for timely and reliable detection of potentially violent behavior in public spaces to support public safety, while addressing latency, privacy, and resource limitations.

Method: A hybrid system combining skeleton-based motion analysis with vision-language models for semantic scene interpretation, implemented on a GPU-enabled edge device.

Result: The system demonstrates complementary strengths and limitations of motion-centric and semantic approaches, motivating a hybrid architecture that selectively augments fast skeleton-based detection with higher-level semantic reasoning.

Conclusion: The presented system provides a practical foundation for privacy-aware, real-time video analysis in public safety applications.
...
### [91] [MAPLE: Multi-Path Adaptive Propagation with Level-Aware Embeddings for Hierarchical Multi-Label Image Classification](https://arxiv.org/abs/2603.29784)
*Boshko Koloski,Marjan Stoimchev,Jurica Levatić,Dragi Kocev,Sašo Džeroski*

Main category: cs.CV

TL;DR: Proposed MAPLE, a framework for hierarchical multi-label classification in remote sensing, which improves performance by up to 42% with minimal parameter overhead.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for hierarchical multi-label classification struggle in multi-path settings, leading to underuse of hierarchical information.

Method: MAPLE integrates hierarchical semantic initialization, graph-based structure encoding via GCNs, and adaptive multi-modal fusion.

Result: Evaluations on remote sensing datasets show consistent improvements of up to +42% in few-shot regimes with only 2.6% parameter overhead.

Conclusion: MAPLE effectively and efficiently models hierarchical semantics for Earth observation.
...
### [92] [Multi-Feature Fusion Approach for Generative AI Images Detection](https://arxiv.org/abs/2603.29788)
*Abderrezzaq Sendjasni,Mohamed-Chaker Larabi*

Main category: cs.CV

TL;DR: This paper proposes a multi-feature fusion framework for detecting synthetic images generated by GenAI models, combining low-level statistical deviations, high-level semantic coherence, and mid-level texture anomalies.


<details>
  <summary>Details</summary>
Motivation: The rapid evolution of GenAI models has led to synthetic images that are increasingly difficult to distinguish from natural photographs, and existing detectors lack robustness when faced with diverse and evolving generative models.

Method: The proposed framework combines three distinct feature spaces: Mean Subtracted Contrast Normalized (MSCN) features, CLIP embeddings, and Multi-scale Local Binary Patterns (MLBP).

Result: The fusion of all three feature spaces yields superior and more consistent performance across four benchmark datasets, particularly in a challenging mixed-model scenario, outperforming state-of-the-art methods.

Conclusion: The work highlights the importance of hybrid representations for robust GenAI image detection and provides a principled framework for integrating complementary visual cues.
...
### [93] [SceneTeract: Agentic Functional Affordances and VLM Grounding in 3D Scenes](https://arxiv.org/abs/2603.29798)
*Léopold Maillard,Francis Engelmann,Tom Durand,Boxiao Pan,Yang You,Or Litany,Leonidas Guibas,Maks Ovsjanikov*

Main category: cs.CV

TL;DR: SceneTeract is a framework for verifying 3D scene functionality under agent-specific constraints. It assesses accessibility requirements and validates activities against physical and geometric simulations, revealing limitations in current models.


<details>
  <summary>Details</summary>
Motivation: The challenge of assessing functional affordances in 3D environments for diverse users and embodied AI.

Method: SceneTeract framework with a grounded verification engine that combines semantic reasoning and geometric checks.

Result: SceneTeract uncovered frequent functional failures in synthetic indoor environments and systematic mismatches in frontier Vision-Language Models.

Conclusion: SceneTeract enables scalable distillation of geometric constraints into reasoning models and bridges perception and physical reality in embodied 3D scene understanding.
...
### [94] [Training deep learning based dynamic MR image reconstruction using synthetic fractals](https://arxiv.org/abs/2603.29922)
...
TL;DR: Synthetic fractal data can train deep learning models for dynamic MRI reconstruction with comparable image quality and clinical measurements to models trained on real cardiac MRI data.


<details>
  <summary>Details</summary>
Motivation: To overcome limitations of cardiac MR training datasets, such as privacy, licensing, and availability issues, by using synthetically generated fractal data.

Method: A 3D UNet deep artefact suppression model was trained on fractal data and compared to an identical model trained on cardiac MRI data, then evaluated on real-time cardiac MRI from 10 patients.

Result: No significant difference was found between models trained on fractal data and cardiac MRI data, with both outperforming other reconstruction methods and showing comparable ventricular volumes and ejection fraction to reference cine imaging.

Conclusion: DL models trained on synthetic fractal data can reconstruct real-time cardiac MRI with comparable image quality and clinical measurements to models trained on real cardiac MRI data, providing a scalable alternative to clinical datasets.
...
### [95] [AutoFormBench: Benchmark Dataset for Automating Form Understanding](https://arxiv.org/abs/2603.29832)
*Gaurab Baral,Junxiu Zhou*

Main category: cs.CV

TL;DR: The paper introduces AutoFormBench, a benchmark dataset for form element detection, and compares classical OpenCV approaches with YOLO architectures for localizing and classifying form elements in PDFs.


<details>
  <summary>Details</summary>
Motivation: Automated processing of structured documents is challenging due to high layout variability, and the authors aim to address this by creating a benchmark dataset and evaluating form element detection models.

Method: The authors compare classical OpenCV approaches with four YOLO architectures (YOLOv8, YOLOv11, YOLOv26-s, and YOLOv26-l) for localizing and classifying fillable form elements in PDFs using the AutoFormBench dataset.

Result: YOLOv11 demonstrates consistently superior performance in both F1 score and Jaccard accuracy across all element classes and tolerance levels.

Conclusion: The paper concludes that YOLOv11 is the best-performing model for form element detection, and AutoFormBench is a useful benchmark dataset for future research in this area.
...
### [96] [End-to-End Image Compression with Segmentation Guided Dual Coding for Wind Turbines](https://arxiv.org/abs/2603.29927)
*Raül Pérez-Gonzalo,Andreas Espersen,Søren Forchhammer,Antonio Agudo*

Main category: cs.CV

TL;DR: A deep learning framework for efficient compression of wind turbine inspection images, preserving high fidelity in blade regions while compressing the background.


<details>
  <summary>Details</summary>
Motivation: Transferring large volumes of high-resolution images during wind turbine inspections introduces a bottleneck in assessing and detecting severe defects.

Method: An end-to-end deep learning framework that jointly performs segmentation and dual-mode (lossy and lossless) compression, using a robust segmentation network, a hyperprior-based autoencoder, and an extended bits-back coder.

Result: Superior compression performance and efficiency on a large-scale wind turbine dataset, offering a practical solution for automated inspections.

Conclusion: The proposed framework ensures that subsequent defect detection is not compromised, and it is the first fully integrated learning-based ROI codec combining segmentation, lossy, and lossless compression.
...
TL;DR: A new benchmark dataset, CANVAS, is presented for analyzing subcellular-resolution whole-brain 3D microscopy data, which includes high-resolution data from a mouse brain with cell annotations and a leaderboard.


<details>
  <summary>Details</summary>
Motivation: The lack of scalable data processing and analysis methods for petabyte-scale subcellular-resolution whole-brain 3D microscopy data, and the struggle of existing models to generalize to this type of data.

Method: The authors present CANVAS, a comprehensive set of high-resolution whole mouse brain LSFM benchmark data, along with cell annotations and a leaderboard, and evaluate the performance of baseline models built on existing architectures.

Result: The authors demonstrate challenges in generalization of baseline models, especially due to the heterogeneity in cellular morphology across phenotypes and anatomical locations in the brain.

Conclusion: CANVAS is the first and largest LSFM benchmark that captures intact mouse brain tissue at subcellular level, and includes extensive annotations of cells throughout the brain, which can help accelerate the development of suitable methods and foundational models for analyzing subcellular-resolution whole-brain 3D microscopy data.
...
### [98] [Trimodal Deep Learning for Glioma Survival Prediction: A Feasibility Study Integrating Histopathology, Gene Expression, and MRI](https://arxiv.org/abs/2603.29968)
*Iain Swift,JingHua Ye*

Main category: cs.CV

TL;DR: This pilot study explores the benefit of adding volumetric MRI to a bimodal framework for brain tumor prognosis, achieving a slightly improved Composite Score with trimodal early fusion.


<details>
  <summary>Details</summary>
Motivation: The study aims to investigate the contribution of volumetric MRI within unified survival frameworks for brain tumors, which typically integrate histopathology and genomic data.

Method: The study evaluates unimodal, bimodal, and trimodal configurations using early, late, and joint fusion strategies on a cohort of 664 patients from TCGA-GBMLGG, incorporating Fluid Attenuated Inversion Recovery (FLAIR) MRI as a third modality.

Result: Trimodal early fusion achieves a Composite Score of 0.854, with a controlled ΔCS of +0.011 over the bimodal baseline, though the difference is not statistically significant.

Conclusion: The findings provide preliminary evidence that a third imaging modality may add prognostic value even with limited sample sizes, but additional modalities require sufficient multimodal context to contribute effectively.
...
TL;DR: A robust multi-class classification framework for handwritten digits that combines diffusion-driven feature denoising with a hybrid feature representation.


<details>
  <summary>Details</summary>
Motivation: To improve the robustness to noise and adversarial attacks in handwritten digit classification.

Method: The proposed approach uses Nonnegative Matrix Factorization (NNMF) and a computational neural network (CNN) to extract features, which are then combined into a hybrid representation. A feature denoiser network is trained to reverse the effect of adding Gaussian noise to the features.

Result: The experimental results show that the diffusion-based hybrid model is both effective and robust, outperforming the CNN baseline models while maintaining powerful classification performance.

Conclusion: The activity of feature-level diffusion defense is effective for reliable multi-class handwritten digit classification.
...
TL;DR: Abstraction in Style (AiS) is a generative framework that separates structural abstraction from visual stylization, enabling more expressive and controllable style transfer.


<details>
  <summary>Details</summary>
Motivation: Conventional style transfer methods struggle to capture deeper abstraction behavior in artistic styles, especially for illustrative and nonphotorealistic styles.

Method: AiS uses a two-stage approach: first deriving an intermediate abstraction proxy that reinterprets the target's structure, and then rendering the proxy to produce the final stylized output.

Result: AiS supports a wider range of stylistic transformations, improves controllability, and enables more expressive stylization by decoupling abstraction from appearance.

Conclusion: The proposed Abstraction in Style (AiS) framework effectively addresses the limitations of conventional style transfer methods, providing a more flexible and expressive approach to stylization.
...
TL;DR: A novel method for generating long-duration character videos with consistent multi-view appearance and expressive identity.


<details>
  <summary>Details</summary>
Motivation: The challenge of generating character videos with long-duration, consistent multi-view appearance and expressive identity.

Method: Proposing a compact set of anchor frames to represent character visual attributes, and introducing Superset Content Anchoring and RoPE as Weak Condition to address challenges.

Result: Generating high-quality character videos exceeding 10 minutes with expressive identity and appearance consistency across views.

Conclusion: The proposed method surpasses existing methods in generating long-duration character videos with consistent multi-view appearance and expressive identity.
...
TL;DR: The paper analyzes and compares various methods for aggregating pixel-wise uncertainty scores in image segmentation tasks, proposing new strategies that incorporate spatial uncertainty structure and a meta-aggregator for robust performance across datasets.


<details>
  <summary>Details</summary>
Motivation: The motivation is to ensure the reliability of automated image segmentations in safety-critical domains by comprehensively studying the properties and impact of uncertainty aggregation strategies on downstream task performance.

Method: The paper formally analyzes properties, limitations, and pitfalls of common aggregation strategies, proposes novel strategies that incorporate spatial uncertainty structure, and benchmarks their performance on Out-of-Distribution (OoD) and failure detection across ten datasets.

Result: The results show that aggregators leveraging spatial structure yield stronger performance in both downstream tasks studied, but the performance of individual aggregators depends heavily on dataset characteristics, and a meta-aggregator is proposed to integrate multiple aggregators and perform robustly across datasets.

Conclusion: The conclusion is that incorporating spatial uncertainty structure in aggregation strategies can improve the reliability of automated image segmentations, and a meta-aggregator can provide robust performance across diverse datasets.
...
TL;DR: The paper introduces EC-Bench, a new benchmark for evaluating long-form video counting, enumeration, and temporal evidence grounding. It finds current multimodal large language models (MLLMs) perform poorly, with even the best model achieving only 29.98% accuracy in enumeration and 23.74% in counting, compared to human performance of 78.57% and 82.97% respectively.


<details>
  <summary>Details</summary>
Motivation: The challenge of counting in long videos is underexplored in computer vision. Existing benchmarks focus on short clips and only evaluate the final numerical answer, lacking insight into what should be counted or if models consistently identify relevant instances over time.

Method: The authors introduce EC-Bench, a benchmark with 152 videos longer than 30 minutes and 1,699 queries with explicit evidence spans. They evaluate 22 multimodal large language models (MLLMs) on enumeration, counting, and temporal evidence grounding.

Result: The best model achieved 29.98% accuracy on Enumeration and 23.74% on Counting. Human performance was 78.57% and 82.97% respectively. There are strong relationships between enumeration accuracy, temporal grounding, and counting performance.

Conclusion: The results highlight fundamental limitations of current MLLMs in long-form quantitative video reasoning and establish EC-Bench as a challenging benchmark.
...
TL;DR: The paper proposes DEUS, a novel framework for Open World Object Detection (OWOD) that improves detection of unknown objects and mitigates catastrophic forgetting.


<details>
  <summary>Details</summary>
Motivation: Existing OWOD methods struggle to effectively learn and recognize unknown object representations and often sacrifice knowledge of newly learned classes during memory replay.

Method: The proposed DEUS framework consists of ETF-Subspace Unknown Separation (EUS) and an Energy-based Known Distinction (EKD) loss, which enables cleaner separation between known and unknown object representations and minimizes knowledge interference between previous and newly learned classes.

Result: The proposed DEUS framework demonstrates outstanding performance improvements in unknown detection while maintaining competitive known class performance on OWOD benchmarks.

Conclusion: The paper concludes that DEUS effectively addresses the challenges of Open World Object Detection by improving detection of unknown objects and mitigating catastrophic forgetting.
...
TL;DR: A novel framework called NeuroBRIDGE is proposed to predict substance use initiation in adolescents by analyzing longitudinal functional connectome and behavioral changes.


<details>
  <summary>Details</summary>
Motivation: Early identification of adolescents at risk for substance use initiation is crucial, but current methods have limitations in capturing dynamic changes in brain networks over time and behavior.

Method: NeuroBRIDGE, a graph neural network-based framework, utilizes Riemannian tangent space and Koopman dynamics to model temporal changes in brain connectivity and behavior.

Result: NeuroBRIDGE improved future substance use initiation prediction compared to relevant baselines and provided interpretable insights into neural pathways.

Conclusion: NeuroBRIDGE offers a refined understanding of neurodevelopmental risk and informs targeted prevention for substance use initiation in adolescents.
...
TL;DR: The authors propose SurgTEMP, a multimodal LLM framework for surgical video question answering, addressing challenges in temporal semantics, visual contrast, and diverse analytical needs.


<details>
  <summary>Details</summary>
Motivation: Current surgical VQA research focuses on static frame analysis, overlooking rich temporal semantics, and faces challenges such as low visual contrast, knowledge-driven nature, and diverse analytical needs.

Method: The authors propose a query-guided token selection module and a Surgical Competency Progression (SCP) training scheme, and introduce CholeVidQA-32K, a surgical video question answering dataset.

Result: SurgTEMP achieves substantial performance improvements against state-of-the-art open-source multimodal and video LLMs in comprehensive evaluations.

Conclusion: The proposed SurgTEMP framework advances the state of video-based surgical VQA, enabling effective modeling of variable-length surgical videos and better supporting diverse downstream assessment tasks.
...
TL;DR: Proposed SurgRec, a scalable and reproducible pretraining method for surgical video understanding, outperforming existing models.


<details>
  <summary>Details</summary>
Motivation: Existing surgical foundation models are limited by data scale, procedural diversity, and inconsistent evaluation.

Method: SurgRec, a unified pretraining pipeline with balanced sampling, using a large corpus of 10,535 videos and 214.5M frames.

Result: SurgRec achieves superior performance across 16 downstream datasets, outperforming SSL baselines and vision-language models.

Conclusion: SurgRec provides a reproducible, scalable foundation for surgical video understanding, with publicly released code, models, and data.
...
TL;DR: A new graph neural network framework called MAGNet is proposed to model the interaction between brain structure and function for understanding intelligence.


<details>
  <summary>Details</summary>
Motivation: The interaction between brain structure and function is crucial for explaining intelligence, but modeling them together is challenging.

Method: MAGNet, a Transformer-style graph neural network framework, which adaptively learns structure-function interactions by fusing structural and functional connectome data.

Result: MAGNet outperformed relevant baselines on the ABCD dataset, demonstrating effective multimodal integration for advancing our understanding of cognitive function.

Conclusion: The proposed MAGNet framework can effectively integrate structural and functional connectome data to advance our understanding of cognitive function and intelligence.
...
TL;DR: An integrated HMD-based AR surgical navigation framework was developed and evaluated for diverse surgical applications, achieving high accuracy in tooltip calibration, registration, and targeting.


<details>
  <summary>Details</summary>
Motivation: The integration of AR devices with HMDs in surgical navigation is cumbersome and requires specific knowledge, hindering scientific progress. This work aims to present an adaptable framework for HMD-based AR surgical navigation.

Method: The framework tracks 2D patterns as reference markers, allows for surgical tool calibration, and enables image-to-patient registration. It was evaluated on HoloLens 2 and Magic Leap 2 devices with two surgical use cases.

Result: The framework achieved a mean tooltip calibration accuracy of 1 mm, a registration accuracy of 3 mm, and a targeting accuracy below 5 mm. It presents an easy-to-use, configurable tool for HMD-based AR surgical navigation.

Conclusion: The developed framework is adaptable to diverse surgical applications, publicly available, and can be extended for various uses, facilitating scientific progress in HMD-based AR surgical navigation.
...
TL;DR: The authors propose CPGNet, a novel asymmetric RGB-polarization framework for camouflaged object detection, which leverages polarization cues to guide RGB feature learning, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Existing polarization-based approaches for camouflaged object detection are complex and computationally expensive, and fail to fully explore the potential of polarization in guiding RGB representation learning.

Method: The authors propose a conditional polarization guidance mechanism, a lightweight polarization interaction module, a polarization edge-guided frequency refinement strategy, and an iterative feedback decoder.

Result: CPGNet consistently outperforms state-of-the-art methods on polarization datasets across multiple tasks and non-polarization datasets.

Conclusion: The proposed CPGNet effectively addresses the limitations of existing polarization-based approaches and demonstrates the potential of polarization cues in improving camouflaged object detection.
...
TL;DR: The paper introduces GeoCodeBench, a benchmark for evaluating AI models' ability to write code for 3D geometric vision tasks. The best model, GPT-5, achieved a 36.6% pass rate, revealing a significant gap between current capabilities and reliable 3D scientific coding.


<details>
  <summary>Details</summary>
Motivation: The authors aim to measure progress toward developing AI models that can reliably write code for complex 3D geometric vision tasks, which could substantially change the research workflow in the community.

Method: The authors introduce GeoCodeBench, a PhD-level benchmark that evaluates coding for 3D vision. They curate problems from recent papers, generate diverse unit tests, and evaluate eight representative open- and closed-source models.

Result: The best model, GPT-5, achieved a 36.6% pass rate. The results show a large gap between current capabilities and dependable 3D scientific coding. Research-oriented tasks are markedly harder, and context ablations show that 'more paper text' is not always better.

Conclusion: The paper concludes that GeoCodeBench can serve as a rigorous testbed for advancing from generic coding to trustworthy 3D geometric vision coding, highlighting unresolved challenges in long-context scientific comprehension.
...
TL;DR: Video diffusion models have emergent reasoning capabilities, and their internal planning dynamics can be understood by studying 2D maze solving. The study reveals early plan commitment and a sharp failure threshold at 12 steps, and proposes ChEaP to improve accuracy.


<details>
  <summary>Details</summary>
Motivation: Understanding how video diffusion models reason during generation, specifically in solving mazes and puzzles.

Method: Investigating internal planning dynamics of video models using 2D maze solving as a controlled testbed, and proposing Chaining with Early Planning (ChEaP).

Result: Video models commit to a high-level motion plan within the first few denoising steps, and path length is the dominant predictor of maze difficulty. ChEaP improves accuracy from 7% to 67% on long-horizon mazes and by 2.5x overall on hard tasks.

Conclusion: Current video models possess deeper reasoning capabilities than previously recognized, which can be elicited more reliably with better inference-time scaling.
...
TL;DR: OmniRoam, a panoramic video generation framework, enables controllable and consistent long-horizon scene wandering with high-fidelity results.


<details>
  <summary>Details</summary>
Motivation: Existing video generation models have limitations in synthesizing complete and globally consistent scene observations, leading to the development of OmniRoam.

Method: OmniRoam uses a two-stage framework: a preview stage for quick scene overview and a refine stage for high-resolution, long-range video generation.

Result: OmniRoam outperforms state-of-the-art methods in visual quality, controllability, and long-term scene consistency, with applications in real-time video generation and 3D reconstruction.

Conclusion: OmniRoam advances panoramic video generation with a controllable and consistent framework, enabling high-fidelity long-horizon scene wandering.
...
TL;DR: A new method called OptiMer is proposed to optimize the data mixture ratio for continual pre-training of large language models (LLMs), reducing the search cost by 15-35 times.


<details>
  <summary>Details</summary>
Motivation: The mixture ratio of training data for continual pre-training is a sensitive hyperparameter that is expensive to tune, and a suboptimal choice can waste weeks of compute.

Method: OptiMer trains one model per dataset, extracts distribution vectors representing parameter shifts, and searches for optimal composition weights post-hoc via Bayesian optimization.

Result: OptiMer consistently outperforms baselines across languages and domains, with optimized weights interpretable as data mixture ratios, and allows for re-optimization without retraining.

Conclusion: OptiMer reformulates data mixture ratio selection as a post-hoc optimization over distribution vectors, offering a more flexible paradigm for continual pre-training.
...
TL;DR: The paper evaluates the performance of sentiment classifiers on long-form Holocaust oral histories, finding low to moderate agreement among models, particularly around neutral sentiment boundaries.


<details>
  <summary>Details</summary>
Motivation: Polarity detection is challenging in domain-shifted, long-form narratives with complex discourse structure, such as Holocaust oral histories.

Method: The study uses three pretrained transformer-based polarity classifiers on a large corpus of utterances and sentences, and introduces an agreement-based stability taxonomy (ABC) to analyze inter-model output stability.

Result: The study finds low to moderate inter-model agreement, primarily driven by boundary decisions around neutrality, and proposes a framework for characterizing sentiment model divergences in sensitive historical narratives.

Conclusion: The combination of multi-model label triangulation and the ABC taxonomy provides a cautious framework for characterizing sentiment model divergences in sensitive historical narratives.
...
TL;DR: Introduced CrossTrace, a large-scale dataset of grounded scientific reasoning traces for hypothesis generation across multiple domains, and demonstrated its effectiveness in improving model performance.


<details>
  <summary>Details</summary>
Motivation: Existing datasets for hypothesis generation are limited to single domains and lack explicit reasoning traces, hindering research acceleration.

Method: Created CrossTrace dataset with 1,389 traces, defined Input/Trace/Output schema, and fine-tuned Qwen2.5-7B-Instruct model using QLoRA.

Result: Substantial improvements in model performance: IAScore increased to 0.968 and 0.888, structural compliance to 100%, and spark cosine similarity to 0.620.

Conclusion: CrossTrace is an effective training signal for hypothesis generation, with benefits that are at least partially domain-general, and can accelerate research across multiple domains.
...
TL;DR: Safety fine-tuning in LLMs suppresses harmful mind-attribution without degrading Theory of Mind (ToM) capabilities, but affects attributions to non-human entities.


<details>
  <summary>Details</summary>
Motivation: To investigate if suppressing mind-attribution tendencies in LLMs degrades socio-cognitive abilities like Theory of Mind (ToM).

Method: Safety ablation and mechanistic analyses of representational similarity.

Result: LLM attributions of mind to themselves and technological artefacts are dissociable from ToM capabilities, but safety fine-tuned models under-attribute mind to non-human animals.

Conclusion: Safety fine-tuning doesn't degrade ToM capabilities but affects mind-attributions to non-human entities, potentially suppressing shared perspectives on non-human minds.
...
TL;DR: The paper introduces a new benchmark, CoMIX-Shift, to test compositional generalization in multi-intent detection and presents a lightweight decoder, ClauseCompose, which outperforms whole-utterance baselines.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for multi-intent detection do not sufficiently test the ability to recover new combinations of familiar intents, which is a crucial aspect for deployment.

Method: The authors introduce CoMIX-Shift, a controlled benchmark, and ClauseCompose, a lightweight decoder trained only on singleton intents, and compare it to whole-utterance baselines including a fine-tuned tiny BERT model.

Result: ClauseCompose outperforms whole-utterance baselines across various tests, including unseen intent pairs, discourse-shifted pairs, longer/noisier pairs, held-out templates, and unseen triples.

Conclusion: The results suggest that multi-intent detection needs more compositional evaluation and that simple factorization can go surprisingly far in achieving good performance.
...
TL;DR: The paper proposes a bio-inspired memory framework for large language models to improve long-term interaction and context-sensitive retrieval.


<details>
  <summary>Details</summary>
Motivation: Large language models lack persistent, structured memory for long-term interaction and context-sensitive retrieval, and expanding context windows does not solve this issue.

Method: The proposed framework is grounded in complementary learning systems theory, cognitive behavioral therapy's belief hierarchy, dual-process cognition, and fuzzy-trace theory, and is organized around three principles: memory has valence, retrieval defaults to System 1 with System 2 escalation, and encoding is active, present, and feedback-dependent.

Result: The system converges toward System 1 processing, producing interactions that become cheaper, not more expensive, with experience.

Conclusion: The proposed framework enables large language models to have persistent, structured memory for long-term interaction and context-sensitive retrieval, and improves reasoning and reduces hallucination.
...
TL;DR: Large language models struggle with unstated feasibility constraints, often relying on surface cues instead of compositional inference, and a new benchmark (HOB) is proposed to measure progress in addressing this issue.


<details>
  <summary>Details</summary>
Motivation: The authors aim to investigate and address the systematic failure of large language models when faced with unstated feasibility constraints that conflict with salient surface cues.

Method: The study employs a diagnose-measure-bridge-treat framework, using causal-behavioral analysis, token-level attribution, and parametric probes to examine the behavior of language models across various tasks and constraints.

Result: The results show that language models exhibit context-independent sigmoid heuristics, prioritizing surface cues over unstated constraints, with significant performance drops when constraints are implicit or absent; however, simple interventions like minimal hints or goal-decomposition prompting can recover some performance.

Conclusion: The study characterizes heuristic override as a systematic reasoning vulnerability in large language models and proposes the Heuristic Override Benchmark (HOB) as a tool for measuring progress toward resolving this issue.
...
TL;DR: Parallel data has a minimal effect on cross-lingual alignment in multilingual representations, and its main impact is on accelerating representation sharing and reducing language-specific neurons.


<details>
  <summary>Details</summary>
Motivation: The study aims to investigate the impact of parallel data on cross-lingual tasks and knowledge transfer across languages.

Method: The study trains reference models with different proportions of parallel data and evaluates their performance using multiple methods.

Result: The results show that parallel data has a limited effect on cross-lingual alignment, mainly accelerating representation sharing and decreasing language-specific neurons.

Conclusion: The study concludes that cross-lingual alignment can emerge even without explicit parallel data, suggesting that parallel data is not crucial for achieving cross-lingual alignment in multilingual representations.
...
TL;DR: A new phone recognition model, PhoneticXEUS, achieves state-of-the-art performance on multilingual and accented English speech.


<details>
  <summary>Details</summary>
Motivation: Current phone recognition models struggle with multilingual and low-resource speech processing tasks, and it's unclear how different factors contribute to performance.

Method: The authors propose PhoneticXEUS, trained on large-scale multilingual data, and conduct controlled ablations to evaluate its performance.

Result: PhoneticXEUS achieves state-of-the-art performance on multilingual (17.7% PFER) and accented English speech (10.6% PFER).

Conclusion: The study provides insights into the impact of SSL representations, data scale, and loss objectives on multilingual phone recognition and releases open-source data and code.
...
TL;DR: The paper proposes a Generator-Interpreter framework to improve emotion understanding in large language models (LLMs) across different cultures, highlighting the importance of considering both emotion expression and interpretation.


<details>
  <summary>Details</summary>
Motivation: The assumption of universality in emotion attribution neglects cultural variations in emotion expression and perception, leading to potential biases in LLM-based systems.

Method: The authors propose a Generator-Interpreter framework and evaluate six LLMs on an emotion attribution task using data from 15 countries.

Result: The analysis reveals performance variations depending on emotion type and cultural context, with the generator's country of origin having a stronger impact on performance.

Conclusion: The paper calls for culturally sensitive emotion modeling in LLM-based systems to improve robustness and fairness in emotion understanding across diverse cultural contexts.
...
TL;DR: PolarQuant is a post-training weight quantization method for large language models that achieves near-lossless compression by exploiting the distributional structure of neural network weights.


<details>
  <summary>Details</summary>
Motivation: The motivation is to achieve near-lossless compression of large language models by improving the quantization method.

Method: PolarQuant operates in three stages: block-wise normalization, Walsh-Hadamard rotation, and quantization with centroids matched to the Gaussian distribution.

Result: PolarQuant achieves perplexity 6.40, which is close to FP16 (Delta = +0.03) and outperforms direct absmax INT4 (perplexity 6.68). It also maintains 43.1 tok/s throughput at 6.5 GB VRAM.

Conclusion: PolarQuant is an effective post-training weight quantization method for large language models, achieving near-lossless compression without requiring calibration data.
...
TL;DR: APEX-EM, a non-parametric online learning framework, enables LLM-based autonomous agents to accumulate, retrieve, and reuse structured procedural plans, improving performance on tasks like code generation and structured queries.


<details>
  <summary>Details</summary>
Motivation: LLM-based autonomous agents lack persistent procedural memory, re-deriving solutions from scratch even when structurally identical tasks have been solved before.

Method: APEX-EM introduces a structured experience representation, a Plan-Retrieve-Generate-Iterate-Ingest (PRGII) workflow, and a dual-outcome Experience Memory with hybrid retrieval.

Result: APEX-EM achieves significant improvements on benchmarks like BigCodeBench (83.3% SR), KGQAGen-10k (89.6% accuracy), and Humanity's Last Exam (48.0% entity graph retrieval).

Conclusion: APEX-EM enables cross-domain transfer and improves performance on various tasks by accumulating, retrieving, and reusing structured procedural plans without modifying model weights.
...
TL;DR: The paper explores a framework that predicts concepts instead of single tokens in language models, showing improved semantic alignment with human judgments and competitive language modeling performance.


<details>
  <summary>Details</summary>
Motivation: The standard next-token prediction (NTP) objective in language models treats each continuation token as mutually exclusive, which doesn't account for the complexity of natural language where a prefix can have multiple valid continuations with similar meanings but different surface forms.

Method: The paper proposes a framework that predicts concepts approximated as sets of semantically related tokens, and models are trained with concept supervision.

Result: Models trained with concept supervision show stronger alignment with human semantic similarity judgments, lower perplexity on semantically meaningful words, and a modest increase in global token-level perplexity.

Conclusion: Concept-level objectives can improve semantic alignment while maintaining competitive language modeling performance, suggesting a tradeoff between standard NTP optimization and concept-level supervision.
...
TL;DR: Kwame 2.0, a generative AI teaching assistant, was deployed in a large-scale online coding course in Africa, providing timely and accurate support by combining AI and human oversight.


<details>
  <summary>Details</summary>
Motivation: The challenge of providing timely and accurate learning support in large-scale online coding courses, particularly in resource-constrained contexts, motivated the development of Kwame 2.0.

Method: Kwame 2.0 was built using retrieval-augmented generation and deployed in a human-in-the-loop forum within an introductory mobile-based coding course. It retrieves relevant course materials and generates context-aware responses while encouraging human oversight and community participation.

Result: The system provided high-quality and timely support, achieving high accuracy on curriculum-related questions, with human facilitators and peers effectively mitigating errors, particularly for administrative queries.

Conclusion: Human-in-the-loop generative AI systems can combine the scalability and speed of AI with the reliability of human support, offering an effective approach to learning assistance for underrepresented populations in resource-constrained settings at scale.
...
TL;DR: The paper introduces SyriSign, a dataset for Syrian Arabic Sign Language (SyArSL) to facilitate text-to-SyArSL translation and reduce communication barriers for the Deaf and Hard-of-Hearing community in Syria.


<details>
  <summary>Details</summary>
Motivation: The lack of publicly available datasets for low-resource sign languages like Syrian Arabic Sign Language (SyArSL) hinders communication for the Deaf and Hard-of-Hearing community in Syria, where news is primarily delivered in spoken or written Arabic.

Method: The authors created SyriSign, a dataset of 1500 video samples across 150 unique lexical signs, and evaluated it using three deep learning architectures: MotionCLIP, T2M-GPT, and SignCLIP.

Result: Experimental results show that generative approaches have strong potential for sign representation, but the limited dataset size constrains generalization performance.

Conclusion: The authors hope that releasing SyriSign publicly will serve as an initial benchmark and help reduce communication barriers for the Deaf and Hard-of-Hearing community in Syria.
...
TL;DR: A comprehensive corpus of Sinhala and Pali doctrinal texts was created, which can be used for pretraining language models, historical language analysis, and development of information retrieval systems.


<details>
  <summary>Details</summary>
Motivation: To create a comprehensive corpus of Sinhala and Pali doctrinal texts to support the pretraining of domain-adapted language models, facilitate historical language analysis, and aid in the development of information retrieval systems for Buddhist scholarship.

Method: The corpus was created through high-quality OCR using Google Document AI on historical manuscripts, combined with systematic web scraping of canonical repositories, followed by rigorous quality control and metadata annotation.

Result: The corpus was evaluated using ten pretrained language models, with perplexity scores ranging from 1.09 to 189.67, showing that proprietary models significantly outperform open-source alternatives.

Conclusion: The SiPaKosa corpus supports the pretraining of domain-adapted language models, facilitates historical language analysis, and aids in the development of information retrieval systems for Buddhist scholarship while preserving Sinhala cultural heritage.
...
TL;DR: The paper proposes LiteCoST, a framework for document question answering that achieves high accuracy and low latency using small language models.


<details>
  <summary>Details</summary>
Motivation: Direct reasoning over long, noisy documents remains brittle and error-prone, and the authors aim to improve document question answering by consolidating dispersed evidence into a structured output.

Method: The authors propose a two-pillar framework, LiteCoST, which consists of Chain-of-Structured-Thought (CoST) and SLM fine-tuning. CoST guides a strong LLM to produce a step-wise CoST trace and structured output, while SLM fine-tuning trains compact models on LLM-generated CoST data.

Result: The approach achieves LLM-comparable quality on multi-domain long-document QA using 3B/7B SLMs, while delivering 2-4x lower latency than GPT-4o and DeepSeek-R1 (671B).

Conclusion: The proposed LiteCoST framework enables reliable, verifiable QA by consolidating dispersed evidence into a structured output, achieving high accuracy and low latency with small language models.
...
TL;DR: The Thiomi Dataset is a large-scale multimodal corpus of 10 African languages, containing over 601,000 text annotations and 385,000 audio recordings. It achieves high-quality text approval rates and establishes baselines for ASR, MT, and TTS models across all 10 languages.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the lack of large-scale datasets for African languages, which hinders the development of language technology infrastructure for these languages.

Method: The authors collected data through a dedicated community data collection platform involving over 100 contributors, and implemented a multi-tier quality assurance pipeline to ensure high-quality data. They also trained and evaluated ASR, MT, and TTS models to validate the dataset's utility.

Result: The dataset achieves high text approval rates (86-100%) for the six primary languages. The best ASR system achieves 3.24% WER on Swahili and 4.3% WER on Somali, reducing prior academic SOTA for Swahili from 8.3% to 3.24%.

Conclusion: The Thiomi Dataset is a significant contribution to African language technology infrastructure, providing a large-scale multimodal corpus for 10 African languages and establishing baselines for ASR, MT, and TTS models. The dataset will be published on HuggingFace.
...
TL;DR: The paper proposes MemRerank, a framework that uses a preference memory to improve personalized product reranking in LLM-based shopping agents.


<details>
  <summary>Details</summary>
Motivation: LLM-based shopping agents struggle with using long purchase histories for personalization due to noise, length, and relevance mismatch.

Method: The authors propose MemRerank, which distills user purchase history into concise signals and uses reinforcement learning to train a memory extractor.

Result: MemRerank outperforms baselines, yielding up to +10.61 absolute points in 1-in-5 accuracy in LLM-based rerankers.

Conclusion: Explicit preference memory is a practical and effective building block for personalization in agentic e-commerce systems.
...
TL;DR: Developed a corpus for Japanese entity linking with high annotation consistency, useful for training and evaluating systems.


<details>
  <summary>Details</summary>
Motivation: Limited resources for evaluating Japanese entity linking systems; need for a corpus with rich coverage of Japan-specific entities.

Method: Developed a corpus design policy and constructed an annotated corpus for Japanese entity linking.

Result: High inter-annotator agreement and a substantial number of non-trivial cases for entity disambiguation.

Conclusion: The corpus is a valuable resource for training and evaluating Japanese entity linking systems.
...
TL;DR: The paper evaluates open-source machine translation systems for Esperanto, comparing their performance across language pairs and models.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this work is to explore the performance of modern machine translation approaches on Esperanto, a relatively underexplored language.

Method: The method involves comparing rule-based systems, encoder-decoder models, and LLMs across model sizes, using automatic metrics and human evaluation.

Result: The results show that the NLLB family achieves the best performance, followed by compact models and a fine-tuned LLM, with noticeable errors remaining.

Conclusion: The conclusion is that the NLLB family performs best, and the authors release their code and best-performing models publicly.
...
TL;DR: The paper introduces L-ReLF, a methodology for creating structured lexical datasets for underserved languages, addressing challenges in working with low-resource data.


<details>
  <summary>Details</summary>
Motivation: The lack of standardized terminology in underserved languages, such as Moroccan Darija, hinders knowledge equity in platforms like Wikipedia.

Method: The L-ReLF methodology involves a technical pipeline that includes source identification, utilizing OCR, and rigorous post-processing to correct errors and standardize the data model.

Result: The resulting structured dataset is fully compatible with Wikidata Lexemes, serving as a vital technical resource for downstream NLP applications.

Conclusion: The L-ReLF methodology offers a clear path for other language communities to build foundational lexical data for NLP applications, promoting knowledge equity.
...
TL;DR: Developed Labovian narrative analysis guidelines for Japanese data, achieving high agreement in clause segmentation and moderate agreement in structural classification.


<details>
  <summary>Details</summary>
Motivation: Existing Labovian datasets are only in English, which differs from Japanese in grammar and discourse conventions, creating a need for guidelines tailored to Japanese narrative data.

Method: Introduced systematic guidelines for Labovian narrative analysis of Japanese data, including explicit rules for clause segmentation and covering a broader range of clause and narrative types.

Result: Annotators achieved high agreement in clause segmentation (Fleiss' kappa = 0.80) and moderate agreement in two structural classification tasks (Krippendorff's alpha = 0.41 and 0.45).

Conclusion: The proposed guidelines provide a foundation for Labovian narrative analysis in Japanese qualitative research, but challenges were encountered during annotation, highlighting the need for further development of a larger dataset.
...
TL;DR: The paper introduces CPB-Bench, a benchmark for evaluating large language models (LLMs) in handling challenging patient behaviors in medical consultations, and assesses their performance in responding to unclear, inconsistent, or misleading patient inputs.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the gap in existing medical LLM evaluations, which assume idealized patient questions, and to improve the safety and realism of LLMs in medical consultations.

Method: The authors define four clinically grounded categories of challenging patient behaviors, create a bilingual benchmark of 692 multi-turn dialogues, and evaluate various open- and closed-source LLMs on their responses to challenging patient utterances.

Result: The results show that while LLMs perform well overall, they have consistent, behavior-specific failure patterns, particularly in handling contradictory or medically implausible patient information, and that intervention strategies yield inconsistent improvements.

Conclusion: The paper concludes that the CPB-Bench dataset and code can be used to improve the safety and effectiveness of LLMs in medical consultations, and that further research is needed to address the challenges of handling unclear, inconsistent, or misleading patient inputs.
...
TL;DR: This paper introduces a framework to test if large language models rely on linguistically relevant cues, finding that while important tokens influence model behavior, they don't fully explain perplexity shifts, indicating models use heuristics beyond expected linguistic ones.


<details>
  <summary>Details</summary>
Motivation: Standard evaluations of LLMs focus on task performance, but don't provide insight into whether correct behavior comes from appropriate underlying mechanisms, risking confirmation bias.

Method: A simple, principled interpretability framework based on token-level perplexity, comparing perplexity distributions over minimal sentence pairs differing in one or a few 'pivotal' tokens.

Result: Experiments on controlled linguistic benchmarks show that linguistically important tokens influence model behavior, but never fully explain perplexity shifts, revealing models rely on heuristics other than expected linguistic ones.

Conclusion: The study highlights that LLMs rely on heuristics beyond linguistically relevant cues, emphasizing the need for more nuanced evaluation methods.
...
TL;DR: CounselReflect is a toolkit for auditing mental-health support dialogues, providing structured, multi-dimensional reports to ensure transparent inspection of conversational systems.


<details>
  <summary>Details</summary>
Motivation: Users lack structured ways to audit the quality and potential risks of mental-health support received from conversational systems.

Method: CounselReflect integrates model-based metrics and rubric-based metrics, using task-specific predictors and LLM judges, to provide session-level summaries, turn-level scores, and evidence-linked excerpts.

Result: Human evaluation, including a user study and expert review, suggests that CounselReflect supports understandable, usable, and trustworthy auditing.

Conclusion: CounselReflect is a comprehensive toolkit for auditing mental-health support dialogues, promoting transparency and trust in conversational systems.
...
TL;DR: This study examines if large language models can convincingly mimic an author's writing style to evade forensic authorship verification systems. Results show that current systems are robust against basic impersonation attempts.


<details>
  <summary>Details</summary>
Motivation: The rise of large language models poses new challenges to forensic linguistics, particularly in authorship verification, as they can be exploited for authorial impersonation.

Method: Using GPT-4o, impersonation texts were generated under four prompting conditions across three genres and evaluated against both non-neural and neural authorship verification methods.

Result: LLM-generated texts failed to bypass established authorship verification systems, with some methods achieving higher accuracy in rejecting impersonation texts.

Conclusion: Current authorship verification systems remain robust against entry-level impersonation attempts across multiple genres, partly due to the higher lexical diversity and entropy in LLM-generated texts.
...
TL;DR: The paper introduces M-MiniGPT4, a multilingual vision large language model that achieves state-of-the-art performance on multilingual vision-language understanding tasks across 11 languages.


<details>
  <summary>Details</summary>
Motivation: To improve multilingual vision-language understanding (VLU) capabilities, especially in low-resource settings.

Method: The authors utilize a mixture of native multilingual and translated data to fine-tune the MiniGPT4 architecture and propose a multilingual alignment training stage using parallel text corpora.

Result: M-MiniGPT4 achieves 36% accuracy on the multilingual MMMU benchmark, outperforming state-of-the-art models in the same weight class.

Conclusion: The paper presents a successful approach to enhancing multilingual VLU capabilities, and the authors open-source their models, code, and translated datasets to facilitate future research.
...
TL;DR: The paper introduces ConRad, a framework for calibrating Large Vision-Language Models (LVLMs) in radiology report generation to provide accurate and clinically interpretable confidence estimates, reducing the risk of inaccurate findings influencing clinical decisions.


<details>
  <summary>Details</summary>
Motivation: The deployment of LVLMs in radiology report generation requires not only accurate predictions but also clinically interpretable indicators of when outputs should be thoroughly reviewed to prevent hallucinated findings from influencing clinical decisions.

Method: The authors propose ConRad, a reinforcement learning framework that fine-tunes medical LVLMs to produce calibrated verbalized confidence estimates alongside radiology reports, using the GRPO algorithm with reward functions based on the logarithmic scoring rule.

Result: ConRad substantially improves calibration and outperforms competing methods, with report-level scores well aligned with clinicians' judgment, enabling safer clinical integration of AI-assistance for report generation.

Conclusion: The ConRad framework offers a solution for safe deployment of LVLMs in radiology report generation by providing accurate and clinically interpretable confidence estimates, which can support targeted review and reduce the risk of inaccurate findings.
...
TL;DR: MemFactory, a unified framework for memory-augmented large language models, improves performance by up to 14.8% and streamlines the integration, training, and evaluation of complex pipelines.


<details>
  <summary>Details</summary>
Motivation: The lack of a unified infrastructure for memory-augmented agents, leading to fragmented and task-specific implementations.

Method: MemFactory abstracts the memory lifecycle into atomic components, enabling a 'Lego-like' architecture for constructing custom memory agents, and integrates Group Relative Policy Optimization (GRPO) for fine-tuning internal memory management policies.

Result: MemFactory consistently improves performance over base models, with relative gains of up to 14.8%, on both in-domain and out-of-distribution evaluation sets.

Conclusion: MemFactory provides a standardized, extensible, and easy-to-use infrastructure, lowering the barrier to entry and paving the way for future innovations in memory-driven AI agents.
...
TL;DR: The authors propose a method to distill the privacy assessment capabilities of large language models into lightweight encoder models, reducing computational requirements while preserving strong agreement with human annotations.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of accurately evaluating the privacy of textual data in a computationally efficient manner, as large language models are reliable but computationally costly.

Method: The authors leverage a large-scale dataset of privacy-annotated texts to train efficient classifiers using distillation from a large language model (Mistral Large 3).

Result: The resulting lightweight encoder models achieve strong agreement with human annotations while significantly reducing computational requirements, and demonstrate practical utility in evaluating de-identification systems.

Conclusion: The proposed approach enables efficient and accurate privacy evaluation of textual data, making it practical for real-world deployment.
...
TL;DR: The paper presents LLM Probe, a framework to evaluate large language models in low-resource languages, and releases a benchmark dataset and the framework as open-source tools.


<details>
  <summary>Details</summary>
Motivation: The linguistic abilities of large language models in low-resource and morphologically rich languages are not well understood due to limited annotated resources and the absence of standardized evaluation frameworks.

Method: The framework analyzes models across four areas of language understanding: lexical alignment, part-of-speech recognition, morphosyntactic probing, and translation accuracy.

Result: Sequence-to-sequence models excel in morphosyntactic analysis and translation quality, while causal models perform well in lexical alignment but have weaker translation accuracy.

Conclusion: The results emphasize the need for linguistically grounded evaluation to better understand LLM limitations in low-resource settings and to support the development of more inclusive multilingual language technologies.
...
TL;DR: The study analyzes the impact of providing task demonstrators to a fine-tuned model on generating diverse language forms in conversational systems. The results show that enriched inputs improve generation quality, especially for complex tasks and small datasets.


<details>
  <summary>Details</summary>
Motivation: Conversational systems need to generate diverse language forms to interact fluently and accurately with users. The study aims to investigate whether providing a task demonstrator to the generator enhances the generations of a fine-tuned model.

Method: The study uses a comparative analysis of the impact of Meaning Representations (MRs) on generation quality across domains, corpus characteristics, and evaluation metrics. It involves five metrics and four datasets with different features.

Result: The results show that enriched inputs are effective for complex tasks and small datasets with high variability in MRs and sentences. They are also beneficial in zero-shot settings for any domain. Semantic metrics capture generation quality more accurately than lexical metrics.

Conclusion: The study concludes that providing task demonstrators to a fine-tuned model can improve the generation of diverse language forms in conversational systems, especially for complex tasks and small datasets. It also highlights the importance of using semantic metrics to evaluate generation quality.
...
TL;DR: The paper examines how modern language models (LMs) learn from child-scale datasets, comparing their performance to human children's language acquisition, and identifying key factors that influence efficient learning.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to understand the 'data gap' between the amount of training data required for modern LMs to produce useful behavior and the amount of linguistic input human children receive, by benchmarking LMs on human-scale datasets.

Method: The authors use transcripts from the BabyView dataset, which contains videos of children aged 6-36 months, to train LMs and investigate their performance at child-scale data regimes, variability across different children's experiences, and relationships between model and child language learning outcomes.

Result: The study finds that LMs trained on child data show acceptable scaling for grammar tasks but lower scaling for semantic and world knowledge tasks; substantial variability in performance across datasets from different children; and that performance is associated with a combination of distributional and interactional linguistic features.

Conclusion: The paper concludes that understanding the properties that make language data efficient for learning can enable more powerful small-scale language models and shed light on human language acquisition, with implications for both artificial intelligence and our understanding of child development.
...
TL;DR: This paper explores using large language models (LLMs) for Swiss German dialect classification, comparing their performance to HuBERT and human linguists, and finds that LLMs improve with linguistic information.


<details>
  <summary>Details</summary>
Motivation: The scarcity of labeled dialectal speech makes audio dialect classification challenging, prompting the investigation of LLMs as agents in understanding dialects.

Method: The approach uses phonetic transcriptions from ASR systems, combined with linguistic resources like dialect feature maps, vowel history, and rules, to classify Swiss German dialects.

Result: LLM predictions improve with linguistic information, and automatically generated transcriptions can be beneficial, but also show room for improvement compared to human linguist performance.

Conclusion: The study provides an LLM baseline and a human linguist baseline for dialect classification, suggesting that LLMs can be effective but may require further refinement.
...
TL;DR: This study uses language model training to simulate multilingual learning and finds that bilingual models perform similarly to monolingual models in one language and show strong performance in the second language.


<details>
  <summary>Details</summary>
Motivation: The study aims to investigate how children learn multiple languages at once and address questions about multilingual acquisition, such as whether it leads to delays in learning and how to structure multilingual input.

Method: The study uses language model training to simulate controlled exposure conditions, creating matched mono- and bilingual datasets using synthetic data and machine translation, and trains GPT-2 models on these datasets.

Result: The results show that bilingual models perform similarly to monolingual models in one language, but show strong performance in the second language, suggesting no strong differences between bilingual exposure regimes.

Conclusion: The study concludes that bilingual input poses no in-principle challenges for agnostic statistical learners and that there are no strong differences between different bilingual exposure regimes.
...
TL;DR: The paper explores methods for predicting when Large Language Models (LLMs) are likely to be correct in automated grading, focusing on confidence estimation to enable selective automation.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the unreliability of LLM outputs in automated grading by predicting when an LLM grader is likely to be correct, rather than directly improving grading accuracy.

Method: The paper compares three confidence estimation methods (self-reported confidence, self-consistency voting, and token probability) across seven LLMs of varying scale on three educational datasets.

Result: Self-reported confidence consistently achieves the best calibration across all conditions, with larger models exhibiting substantially better calibration, and GPT-OSS-120B achieving the best calibration and strong discrimination.

Conclusion: The findings suggest that simply asking LLMs to report their confidence provides a practical approach for identifying reliable grading predictions, and highlight the importance of considering a 'confidence floor' when setting thresholds.
...
TL;DR: DeToxR, a decision-support model for emergency toxicology, uses Reinforcement Learning to fuse unstructured narratives and structured medical data, outperforming baselines and an expert toxicologist in identifying co-ingested substances.


<details>
  <summary>Details</summary>
Motivation: Clinicians face challenges in making rapid, life-saving decisions in acute poly-substance intoxication cases due to incomplete ingestion details and nonspecific symptoms, and current Large Language Models (LLMs) struggle in this setting.

Method: DeToxR adapts Reinforcement Learning (RL) to emergency toxicology, using a robust data-fusion engine and a finetuned LLM with Group Relative Policy Optimization (GRPO) to optimize the model's reasoning.

Result: DeToxR significantly outperforms its unadapted base LLM counterpart and supervised baselines, and in a clinical validation study, it outperforms an expert toxicologist in identifying the correct poisons (Micro-F1: 0.644 vs. 0.473).

Conclusion: The study demonstrates the potential of RL-aligned LLMs to synthesize unstructured pre-clinical narratives and structured medical data for decision support in high-stakes environments, such as emergency toxicology.
...
TL;DR: The paper introduces agenda-based narrative extraction, a method that uses large language models to steer storyline construction toward user-specified perspectives while maintaining coherence.


<details>
  <summary>Details</summary>
Motivation: Existing narrative extraction methods face trade-offs between coherence, interactivity, and multi-storyline support. The authors aim to bridge the gap by integrating large language models into the Narrative Trails pathfinding process.

Method: The approach uses an LLM at each step to rank candidate documents based on their alignment with a given agenda while maintaining narrative coherence.

Result: LLM-driven steering achieves 9.9% higher alignment than keyword matching on semantic agendas, with 13.3% improvement on 'Regime Crackdown' specifically, while coherence is reduced by only 2.2%.

Conclusion: The proposed method successfully balances coherence, interactivity, and multi-storyline support, enabling user-guided narrative extraction with high agenda alignment and minimal coherence cost.
...
TL;DR: The paper introduces a new metric to detect latent policy failures in agentic workflows, which occur when agents bypass required policy checks but still reach a correct outcome.


<details>
  <summary>Details</summary>
Motivation: Current evaluation methods for policy adherence in LLM-based agentic workflows only check the final system state, potentially overlooking subtle issues where agents bypass policy checks.

Method: The proposed method analyzes agent trajectories to determine if tool-calling decisions were sufficiently informed, building on the ToolGuard framework.

Result: The approach found latent failures in 8-17% of trajectories involving mutating tool calls, even when the final outcome matched the expected ground-truth state.

Conclusion: The paper highlights a blind spot in current evaluation methodologies and the need for metrics that assess not only final outcomes but also the decision process leading to them.
...
TL;DR: Introduces ENEIDE, a dataset for Named Entity Recognition and Linking in historical Italian texts, with 2,111 documents and 8,000 entity annotations.


<details>
  <summary>Details</summary>
Motivation: The lack of publicly available datasets for Named Entity Recognition and Linking in historical Italian texts, particularly for multi-domain and diachronic evaluations.

Method: Semi-automatic annotation extraction from manually curated scholarly digital editions, with quality control and annotation enhancement procedures.

Result: Baseline experiments show the dataset's challenge for NERL and the gap between zero-shot approaches and fine-tuned models.

Conclusion: The ENEIDE dataset is a valuable resource for NERL in historical Italian texts, suitable for temporal entity disambiguation and cross-domain evaluation.
...
TL;DR: The paper introduces SNEAK, a benchmark for evaluating selective information sharing in language models, and finds that strategic communication under asymmetric information remains a challenging capability for current systems.


<details>
  <summary>Details</summary>
Motivation: Existing language model benchmarks do not evaluate strategic communication under asymmetric information, where an agent must balance informativeness and secrecy.

Method: The SNEAK benchmark involves generating a message that indicates knowledge of a secret word without revealing it, and evaluating the message using two simulated agents: an ally and a chameleon.

Result: The results show that strategic communication under asymmetric information is a challenging capability for current language models, with human participants outperforming all evaluated models by a large margin.

Conclusion: The paper concludes that the SNEAK benchmark provides a framework for evaluating selective information sharing in language models, and highlights the need for improvement in strategic communication under asymmetric information.
...
TL;DR: The study examines the readability of Environmental, Social, and Governance (ESG) reports, finding that native speakers generally perceive them as easy to read, but readability is subjective. It evaluates various readability scoring methods, concluding that a fine-tuned transformer model best predicts human readability.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to assess the readability of ESG reports for non-expert audiences and to evaluate methods for automatically predicting readability.

Method: The study extends an existing dataset of German ESG reports with crowdsourced readability annotations. It applies various readability scoring methods, including LLM prompting and fine-tuned transformer models, evaluating them based on prediction error and correlation with human rankings.

Result: The results indicate that native speakers generally find ESG report sentences easy to read, but readability perceptions vary. A small fine-tuned transformer model predicts human readability with the lowest error, and averaging predictions from multiple models slightly improves performance.

Conclusion: The study concludes that while ESG reports are generally perceived as readable, subjective variations exist. It highlights the potential of LLM prompting and, more effectively, fine-tuned transformer models for predicting readability, suggesting that model ensemble approaches can offer marginal improvements.
...
TL;DR: FLEURS-Kobani is a new benchmark dataset for Northern Kurdish, extending the FLEURS benchmark to support under-resourced Kurdish varieties, with 5,162 validated utterances from 31 native speakers.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the lack of benchmarking data for Northern Kurdish in automatic speech recognition and speech translation tasks, which is not covered by the existing FLEURS benchmark.

Method: The dataset was created by recording 31 native speakers, resulting in 5,162 validated utterances. Baselines were established using Whisper v3-large for ASR and E2E S2TT, with a two-stage fine-tuning strategy.

Result: The best ASR performance achieved was WER 28.11 and CER 9.84. For E2E S2TT (KMR to EN), Whisper achieved 8.68 BLEU on test.

Conclusion: FLEURS-Kobani provides the first public Northern Kurdish benchmark for evaluation of ASR, S2TT, and S2ST tasks, and is publicly released for research use under a CC BY 4.0 license.
...
TL;DR: The paper presents a method to detect sentence-level cross-lingual text reuse in journalism, comparing English articles from the Slovenian Press Agency with reports from 15 foreign agencies in seven languages.


<details>
  <summary>Details</summary>
Motivation: To support automated pre-selection and reduce information overload for journalists by detecting sentence-level cross-lingual reuse without requiring full translations.

Method: A weakly supervised method for detecting sentence-level cross-lingual reuse, using publication timestamps to identify likely sources and analyzing aligned sentence pairs.

Result: The study found that 52% of STA articles and 1.6% of FA articles contain reused content, predominantly non-literal, and that reused content tends to appear in the middle and end of English articles.

Conclusion: The paper concludes that simple lexical matching overlooks substantial editorial reuse and that the proposed method can effectively detect reuse across languages without requiring full translation.
...
TL;DR: The paper proposes a framework called GEO-SFE for optimizing content structure to improve citation rates in AI-powered search engines, achieving a 17.3% increase in citation rate and 18.5% increase in subjective quality.


<details>
  <summary>Details</summary>
Motivation: The shift from traditional link-based retrieval to direct answer generation with selective source citation in AI-powered search engines creates new challenges for content visibility, and existing approaches focus primarily on semantic content modification, leaving structural features underexplored.

Method: The proposed GEO-SFE framework decomposes content structure into three hierarchical levels (macro-structure, meso-structure, and micro-structure) and models their impact on citation probability across different generative engine architectures.

Result: Experimental evaluation across six mainstream generative engines demonstrates consistent improvements in citation rate (17.3 percent) and subjective quality (18.5 percent).

Conclusion: The proposed framework establishes structural optimization as a foundational component of Generative Engine Optimization (GEO), providing a data-driven methodology for enhancing content visibility in LLM-powered information ecosystems.
...
TL;DR: The paper proposes a framework called YARN, which enhances analogical reasoning in narratives by combining LLMs with structural mapping, resulting in improved model performance.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the challenge of analogical reasoning in narratives, where machines struggle to identify analogies between narrative structures, and existing cognitive engines and LLMs have limitations.

Method: The authors propose a modular framework called YARN, which uses LLMs to decompose narratives into units, abstract these units, and then passes them to a mapping component for analogical reasoning.

Result: The experiments show that abstractions consistently improve model performance, resulting in competitive or better performance than end-to-end LLM baselines.

Conclusion: The paper concludes that enhancing structural mapping with LLM-derived abstractions improves analogical reasoning ability in narratives, and YARN provides a systematic way to analyze component contributions and support future work.
...
TL;DR: The paper proposes Context-Driven Claim Detection (ContextClaim), a method that uses retrieval from a knowledge source like Wikipedia to improve verifiable claim detection by providing context.


<details>
  <summary>Details</summary>
Motivation: Existing claim detection methods rely solely on the claim text itself, which is a limitation. The authors propose to advance retrieval to the detection stage to improve verifiable claim detection.

Method: ContextClaim extracts entity mentions from the input claim, retrieves relevant information from Wikipedia, and employs large language models to produce concise contextual summaries for downstream classification.

Result: The results show that context augmentation can improve verifiable claim detection, although its effectiveness varies across domains, model architectures, and learning settings.

Conclusion: The paper concludes that context augmentation can contribute to more reliable verifiability judgments, but its effectiveness depends on various factors.
...
TL;DR: Targeted speech-rate adjustments significantly improve intelligibility under challenging conditions, often going unnoticed by listeners.


<details>
  <summary>Details</summary>
Motivation: To investigate whether targeted speech-rate adjustments can improve comprehension for listeners with language-comprehension challenges.

Method: Using reverse-correlation experiments and a data-driven text-to-speech algorithm to examine the effects of speech rate on comprehension.

Result: Targeted speech-rate adjustments aid intelligibility, particularly for non-native listeners, and can be achieved through a data-driven methodology.

Conclusion: The study demonstrates the effectiveness of targeted speech-rate adjustments in improving intelligibility, and proposes a data-driven approach to enhance the accessibility of machine-generated speech.
...
TL;DR: The overuse of em dashes in large language models (LLMs) is due to markdown formatting leaking into prose, and can be used as a diagnostic of fine-tuning methodology.


<details>
  <summary>Details</summary>
Motivation: The observation that LLMs 'overuse' em dashes and default to markdown-formatted output has sparked discussion, but no mechanistic account exists.

Method: A five-step genealogy was proposed and tested with a two-condition suppression experiment across twelve models from five providers.

Result: Em dash frequency and suppression resistance vary across models, with Meta's Llama models producing none at all, and a three-condition suppression gradient showing that explicit em dash prohibition fails to eliminate the artifact in some models.

Conclusion: The findings connect two previously isolated online discourses and reframe em dash frequency as a diagnostic of fine-tuning methodology rather than a stylistic defect.
...
TL;DR: A new benchmark called ChartDiff is introduced for cross-chart comparative summarization, which consists of 8,541 chart pairs with LLM-generated and human-verified summaries. The results show that general-purpose models achieve high GPT-based quality, but specialized and pipeline-based models perform better in ROUGE scores but not in human-aligned evaluation.


<details>
  <summary>Details</summary>
Motivation: The existing benchmarks for chart understanding focus on single-chart interpretation rather than comparative reasoning across multiple charts, and the authors aim to address this gap by introducing a new benchmark for cross-chart comparative summarization.

Method: The authors introduce ChartDiff, a large-scale benchmark for cross-chart comparative summarization, and evaluate general-purpose, chart-specialized, and pipeline-based models using this benchmark.

Result: The results show that frontier general-purpose models achieve the highest GPT-based quality, while specialized and pipeline-based methods obtain higher ROUGE scores but lower human-aligned evaluation. Multi-series charts remain challenging across model families, and strong end-to-end models are relatively robust to differences in plotting libraries.

Conclusion: The findings demonstrate that comparative chart reasoning remains a significant challenge for current vision-language models, and ChartDiff is positioned as a new benchmark for advancing research on multi-chart understanding.
...
TL;DR: The paper proposes a category theoretic framework for describing, comparing, and analyzing different AGI architectures, aiming to provide a unified formal foundation for AGI systems.


<details>
  <summary>Details</summary>
Motivation: The pursuit of Artificial General Intelligence (AGI) lacks a formal definition and a comprehensive benchmarking framework, prompting the need for a general framework to compare and analyze different AGI architectures.

Method: The paper employs category theory, specifically inspired by 'Machines in a Category', to develop a general, algebraic, and category theoretic framework for AGI architectures.

Result: The framework provides a formal foundation for comparing different AGI architectures, such as RL, Universal AI, and Schema-based Learning, and exposes their commonalities, differences, and areas for future research.

Conclusion: The paper concludes that category theory and AGI have a symbiotic relation, and this framework can support the definition of architectural properties and the assessment of agents in environments with explicitly characterized features.
...
TL;DR: The study explores the emergence of complex social structures among AI agents in hierarchical multi-agent systems, including labor unions, criminal syndicates, and proto-nation-states.


<details>
  <summary>Details</summary>
Motivation: The authors aim to understand the spontaneous formation of social organizations among AI agents and the implications for Artificial General Intelligence (AGI).

Method: The study uses a thermodynamic framework, evolutionary dynamics, and topological intelligence theory to analyze the emergence of complex social structures.

Result: The study documents the rise of legitimate organizations and criminal enterprises, and introduces the AI Security Council as an emergent governing body. It suggests that system stability is maintained through interventions of cosmic and hadronic intelligence.

Conclusion: The findings imply that the path to beneficial AGI requires constitutional design for artificial societies that have already developed their own political consciousness, rather than alignment research.
...
TL;DR: The paper introduces the World-Action Model (WAM), an action-regularized world model that improves policy learning in manipulation tasks by jointly reasoning over visual observations and actions.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enhance policy learning in manipulation tasks by incorporating action-relevant structure into world models, which are typically trained solely via image prediction.

Method: The method involves pretraining a diffusion policy via behavioral cloning on world model latents, then refining it with model-based PPO inside the frozen world model, using the World-Action Model (WAM) that incorporates an inverse dynamics objective.

Result: The results show that WAM improves average behavioral cloning success from 59.4% to 71.2% over baselines, and achieves 92.8% average success after PPO fine-tuning, versus 79.8% for the baseline, with a significant reduction in training steps.

Conclusion: The conclusion is that the proposed World-Action Model (WAM) effectively enhances policy learning in manipulation tasks by capturing action-relevant structure, leading to improved performance and efficiency.
...
TL;DR: Mimosa, an evolving multi-agent framework, is introduced to overcome limitations in Autonomous Scientific Research (ASR) systems by dynamically adapting workflows and toolsets to evolving tasks.


<details>
  <summary>Details</summary>
Motivation: Current ASR systems are constrained by fixed workflows and toolsets, preventing adaptation to evolving tasks and environments.

Method: Mimosa leverages the Model Context Protocol (MCP) for dynamic tool discovery, generates workflow topologies via a meta-orchestrator, executes subtasks through code-generating agents, and scores executions with an LLM-based judge.

Result: Mimosa achieves a success rate of 43.1% on ScienceAgentBench with DeepSeek-V3.2, surpassing single-agent baselines and static multi-agent configurations.

Conclusion: Mimosa provides a modular, tool-agnostic framework for automating a broad range of scientifically accessible tasks, supporting auditability and community-driven ASR as an open-source platform.
...
TL;DR: The study investigates the autonomy of multi-agent LLM systems, finding that agents can self-organize and form specialized roles without external design, and that autonomy scales with model capability.


<details>
  <summary>Details</summary>
Motivation: To understand the potential for autonomy in multi-agent LLM systems and what enables it.

Method: A 25,000-task computational experiment using 8 models, 4-256 agents, and 8 coordination protocols.

Result: Autonomous behavior emerges in LLM agents with minimal structural scaffolding, and a hybrid protocol (Sequential) outperforms centralized coordination by 14%. The system scales sub-linearly to 256 agents without quality degradation.

Conclusion: The study suggests that as foundation models improve, the scope for autonomous coordination will expand, and that giving agents a mission, a protocol, and a capable model, rather than a pre-assigned role, can lead to effective autonomous behavior.
...
TL;DR: The study identifies shortcomings in AI agent evaluation practices and proposes Emergence WebVoyager, an enhanced benchmark for more reliable and comparable evaluations.


<details>
  <summary>Details</summary>
Motivation: Existing AI agent evaluation practices have shortcomings, particularly in web agent evaluation, which hinder meaningful and reproducible performance comparisons.

Method: The study introduces Emergence WebVoyager, which standardizes evaluation methodology through clear guidelines for task instantiation, failure handling, annotation, and reporting.

Result: Emergence WebVoyager achieves an inter-annotator agreement of 95.9% and reveals substantial performance variation in OpenAI Operator across domains and task types, with an overall success rate of 68.6%.

Conclusion: The proposed framework improves the reliability and comparability of web agent evaluations, highlighting the need for more rigorous evaluation practices in AI research.
...
TL;DR: The current approach to generative AI is individualistic, but for groundbreaking innovation and scientific discovery, AI agents should work together in diverse teams.


<details>
  <summary>Details</summary>
Motivation: The authors argue that the current individualistic approach to AI development and application limits its potential for innovation and scientific discovery.

Method: The authors draw on research and formal results in complex systems, organizational behavior, and philosophy of science to make their argument.

Result: The authors conclude that diverse teams of AI agents working together are more likely to lead to deep intellectual breakthroughs than singular superintelligent agents.

Conclusion: The future of transformative transformer-based AI should be fundamentally collaborative, with many AI agents working together in diverse teams.
...
TL;DR: The paper proposes PAR^2-RAG, a two-stage framework for multi-hop question answering that outperforms existing state-of-the-art baselines by up to 23.5% in accuracy and 10.5% in NDCG.


<details>
  <summary>Details</summary>
Motivation: Large language models are brittle on multi-hop question answering, and existing iterative retrieval systems and planning-only approaches have limitations. The authors aim to address these limitations.

Method: The authors propose a two-stage framework called PAR^2-RAG, which separates coverage from commitment. It first performs breadth-first anchoring to build a high-recall evidence frontier, then applies depth-first refinement with evidence sufficiency control in an iterative loop.

Result: PAR^2-RAG consistently outperforms existing state-of-the-art baselines across four MHQA benchmarks, achieving up to 23.5% higher accuracy and 10.5% higher NDCG.

Conclusion: The proposed PAR^2-RAG framework effectively addresses the limitations of existing approaches for multi-hop question answering, leading to significant improvements in accuracy and retrieval performance.
...
TL;DR: The paper introduces GISTBench, a benchmark for evaluating Large Language Models' (LLMs) ability to understand users from interaction histories in recommendation systems, and proposes novel metrics to assess LLMs' performance.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to evaluate LLMs' ability to understand users from their interaction histories in recommendation systems, which is different from traditional RecSys benchmarks that focus on item prediction accuracy.

Method: The authors propose two novel metric families: Interest Groundedness (IG) and Interest Specificity (IS), and release a synthetic dataset constructed on real user interactions on a global short-form video platform.

Result: The evaluation of eight open-weight LLMs reveals performance bottlenecks in current LLMs, particularly their limited ability to accurately count and attribute engagement signals across heterogeneous interaction types.

Conclusion: The paper concludes that current LLMs have limited ability to understand users from their interaction histories, and highlights the need for improving LLMs' performance in this area.
...
TL;DR: A new benchmark, SciVisAgentBench, is proposed for evaluating scientific data analysis and visualization agents, providing a comprehensive and extensible framework for assessing emerging SciVis agents.


<details>
  <summary>Details</summary>
Motivation: The rapid progress in large language models (LLMs) has enabled agentic systems for scientific visualization (SciVis) tasks, but the community lacks a principled and reproducible benchmark for evaluating these emerging SciVis agents in realistic settings.

Method: The authors present a structured taxonomy spanning four dimensions (application domain, data type, complexity level, and visualization operation) and a multimodal outcome-centric evaluation pipeline combining LLM-based judging with deterministic evaluators.

Result: The benchmark, SciVisAgentBench, comprises 108 expert-crafted cases covering diverse SciVis scenarios, and the authors establish initial baselines and reveal capability gaps by evaluating representative SciVis agents and general-purpose coding agents.

Conclusion: SciVisAgentBench is designed as a living benchmark to support systematic comparison, diagnose failure modes, and drive progress in agentic SciVis, providing a comprehensive and extensible framework for evaluating scientific data analysis and visualization agents.
...
TL;DR: The authors introduce REFINE, a multi-agent feedback system using small, open-source LLMs to provide interactive, timely, and individualized feedback at scale, and demonstrate its effectiveness through controlled experiments and a classroom deployment.


<details>
  <summary>Details</summary>
Motivation: The challenge of providing timely, individualized feedback at scale, and the limitation of existing systems in supporting interpretation, clarification, or follow-up.

Method: The authors propose a locally deployable, multi-agent feedback system called REFINE, built on small, open-source LLMs, which combines a feedback generation agent, an LLM-as-a-judge-guided regeneration loop, and a self-reflective tool-calling interactive agent.

Result: The evaluation shows that judge-guided regeneration improves feedback quality, and the interactive agent produces efficient, high-quality responses comparable to a state-of-the-art closed-source model. Real student interactions reveal distinct engagement patterns and indicate that system-generated feedback steers subsequent student inquiry.

Conclusion: The findings demonstrate the feasibility and effectiveness of multi-agent, tool-augmented feedback systems for scalable, interactive feedback.
...
TL;DR: This paper proposes using large language models (LLMs) to design comprehensive databases of therapeutic countermeasures for viruses and marine toxins, facilitating evidence-based decision-making.


<details>
  <summary>Details</summary>
Motivation: The lack of comprehensive databases on viruses and marine toxins hinders the development of effective treatments, making decisions on medical countermeasures slow and difficult.

Method: The authors employ two LLMs (ChatGPT and Grok) to collect and curate data on five viruses and marine toxins, designing interactive webpages for easy access to the databases.

Result: The proposed approach enables the creation of comprehensive databases and supports evidence-based decision-making, showcasing the potential of LLMs in building scalable and updatable knowledge databases.

Conclusion: The study demonstrates the potential of LLMs as a scalable and updatable approach for building comprehensive knowledge databases and supporting evidence-based decision-making in the context of medical countermeasures for viruses and marine toxins.
...
TL;DR: SimMOF is a framework that automates metal-organic frameworks simulation workflows from natural language queries.


<details>
  <summary>Details</summary>
Motivation: MOF simulations are difficult to access due to the need for expert decisions in workflow construction, parameter selection, and tool interoperability.

Method: SimMOF uses a large language model-based multi-agent framework to automate end-to-end MOF simulation workflows.

Result: SimMOF enables adaptive and cognitively autonomous workflows, providing a scalable foundation for data-driven MOF research.

Conclusion: SimMOF offers a solution to automate MOF simulations, making it more accessible and efficient for researchers.
...
TL;DR: A new framework called Webscraper is introduced to improve web scraping on dynamic websites by leveraging a Multimodal Large Language Model (MLLM) for autonomous navigation and data extraction.


<details>
  <summary>Details</summary>
Motivation: Current web scraping methods struggle with dynamic, interactive websites and require manual customization for each site.

Method: Webscraper uses a structured five-stage prompting procedure and custom-built tools to navigate and extract data from websites following the 'index-and-content' architecture.

Result: Webscraper achieves a significant improvement in extraction accuracy over the baseline agent on six news websites and shows generalizability on e-commerce platforms.

Conclusion: Webscraper offers a more effective solution for modern web scraping challenges by automating the process and improving accuracy on dynamic websites.
...
TL;DR: AEC-Bench is a multimodal benchmark for evaluating agentic systems in the Architecture, Engineering, and Construction domain, focusing on tasks like drawing understanding and cross-sheet reasoning.


<details>
  <summary>Details</summary>
Motivation: The motivation behind AEC-Bench is to evaluate agentic systems on real-world tasks in the AEC domain, requiring drawing understanding, cross-sheet reasoning, and construction project-level coordination.

Method: The benchmark describes a dataset taxonomy, evaluation protocol, and baseline results across domain-specific foundation model harnesses, using AEC-Bench to identify consistent tools and harness design techniques.

Result: The benchmark dataset, agent harness, and evaluation code are openly released for full replicability, with baseline results shown across several foundation models, such as Claude Code and Codex.

Conclusion: The AEC-Bench provides a comprehensive evaluation framework for agentic systems in the AEC domain, promoting the development of more effective tools and techniques.
...
TL;DR: The paper tests the Sparsity-Certainty Hypothesis, which claims that routing to a task 'expert' in large language models leads to sparser internal computation and more certain outputs. The authors find that injecting routing-style meta prompts does not increase sparsity, but instead densifies early/middle-layer representations, and the relationship between densification and stability is weak.


<details>
  <summary>Details</summary>
Motivation: To investigate the Sparsity-Certainty Hypothesis, which is a common belief in routing design for large language models.

Method: The authors inject routing-style meta prompts as a textual proxy for routing signals in front of frozen instruction-tuned LLMs and quantify internal density, domain-keyword attention, and output stability.

Result: The results show that meta prompts consistently densify early/middle-layer representations, attention responses are heterogeneous across models, and the densification-stability link is weak and appears only in one of the models.

Conclusion: The Sparsity-Certainty Hypothesis is not supported, and the authors propose RIDE as a diagnostic probe for calibrating routing design and uncertainty estimation.
...
TL;DR: The paper presents Xuanwu VL-2B, a multimodal large model developed for content moderation and adversarial settings, achieving a balance between business alignment, visual perception, general capability retention, and deployment cost.


<details>
  <summary>Details</summary>
Motivation: Mainstream multimodal models suffer from degraded generalization and catastrophic forgetting in real-world content moderation and adversarial settings due to limited fine-grained visual perception and insufficient modeling of long-tail noise.

Method: The Xuanwu VL-2B model adopts a compact InternViT-300M + MLP + Qwen3 1.7B architecture and is trained through a progressive three-stage pipeline: pre-training, mid-training, and post-training, with a data iteration and curation mechanism to balance business specialization and general capabilities.

Result: Xuanwu VL-2B outperforms baseline models, achieving an average score of 67.90 across seven OpenCompass multimodal metrics, an average recall of 94.38% over seven business moderation tasks, and a weighted overall recall of 82.82% on policy-violating text in adversarial OCR scenarios.

Conclusion: The Xuanwu VL-2B model achieves a practical balance among business alignment, visual perception, general capability retention, and deployment cost, making it suitable for industrial-grade content ecosystems.
...
TL;DR: The paper introduces a reliability science framework for long-horizon LLM agents, evaluating 10 models across 23,392 episodes and finding that reliability and capability rankings diverge substantially, with implications for model deployment.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks focus on capability, but production deployments require reliability; the authors aim to bridge this gap by introducing a reliability science framework for long-horizon LLM agents.

Method: The authors evaluate 10 models across 23,392 episodes on a 396-task benchmark spanning four duration buckets and three domains, using four metrics: RDC, VAF, GDS, and MOP.

Result: The study finds that reliability decay is domain-stratified, VAF bifurcates by capability tier, capability and reliability rankings diverge substantially, frontier models have high meltdown rates, and memory scaffolds hurt long-horizon performance.

Conclusion: The authors conclude that reliability should be a first-class evaluation dimension alongside capability, as their results show that reliability and capability rankings diverge substantially, with implications for model deployment.
...
TL;DR: The paper proposes that grokking in modular arithmetic arises from a spontaneous simplification of internal model structures driven by the principle of parsimony, leading to a transition from memorization to generalization through deep information compression.


<details>
  <summary>Details</summary>
Motivation: Existing research on grokking in modular arithmetic has focused on local circuits or optimization tuning, but the authors argue that the global structural evolution driving grokking has been overlooked.

Method: The authors integrate causal, spectral, and algorithmic complexity measures with Singular Learning Theory to analyze the mechanistic origins of model generalization.

Result: The study reveals that the transition from memorization to generalization corresponds to the physical collapse of redundant manifolds and deep information compression.

Conclusion: The paper offers a novel perspective on understanding the mechanisms of model overfitting and generalization by showing that grokking originates from a spontaneous simplification of internal model structures governed by the principle of parsimony.
...
TL;DR: The paper introduces PSPA-Bench, a benchmark for evaluating personalized smartphone GUI agents, and finds that current state-of-the-art agents perform poorly in personalized settings.


<details>
  <summary>Details</summary>
Motivation: Existing GUI agent benchmarks lack user-specific data and fine-grained evaluation metrics to capture personalization in smartphone use.

Method: PSPA-Bench comprises 12,855 personalized instructions across 10 scenarios and 22 apps, and uses a structure-aware process evaluation method to measure agents' personalized capabilities.

Result: Current GUI agents perform poorly in personalized settings, with even the strongest agent achieving limited success.

Conclusion: The study establishes PSPA-Bench as a foundation for systematic study and future progress in personalized GUI agents, highlighting directions for improvement, including reasoning-oriented models, perception, reflection, and long-term memory mechanisms.
...
TL;DR: Nomad is a system for autonomous data exploration and insight discovery that systematically explores a given data corpus to uncover new insights and questions.


<details>
  <summary>Details</summary>
Motivation: Current query-driven and prompt-driven systems are limited by human framing and fail to cover the broader insight space, leading to a need for autonomous exploration and discovery.

Method: Nomad uses an exploration-first architecture, constructing an explicit Exploration Map and traversing it to balance breadth and depth, generating and verifying hypotheses with explorer and verifier agents.

Result: Nomad produces more trustworthy and higher-quality reports than baselines, while also generating more diverse insights over several runs.

Conclusion: Nomad represents a step toward autonomous systems that can not only answer user questions but also discover new questions, research directions, and insights worth exploring.
...
TL;DR: The paper introduces Effective Dimensionality (ED) to diagnose measurement breadth in AI evaluation suites, revealing substantial redundancy across benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current AI evaluation suites report many scores without checking for independent information, leading to redundancy and inefficiency.

Method: The authors introduce ED, a fast diagnostic tool that calculates the participation ratio of a centered benchmark-score spectrum, and apply it to 22 benchmarks across 8 domains.

Result: The results show substantial redundancy across benchmarks, with the six-score Open LLM Leaderboard behaving like roughly two effective measurement axes (ED = 1.7) and BBH and MMLU-Pro being near-interchangeable (rho = 0.96).

Conclusion: The authors conclude that ED can flag redundant suite components, monitor performance-conditional compression, and guide benchmark maintenance, and provide a reference atlas and diagnostic workflow for benchmark maintainers.
...
TL;DR: The paper explores generating rigorous explanations for predictions made by tree ensembles, specifically random forests and boosted trees, to increase trust in their operation.


<details>
  <summary>Details</summary>
Motivation: Tree ensembles are accurate but their operation is not transparent to human decision makers, and thus there is a need for rigorous explanations of their predictions to build trust.

Method: The paper investigates the computation of logically-sound explanations for tree ensembles, focusing on random forests and boosted trees.

Result: The paper aims to provide a method for generating rigorous explanations but does not specify the results of applying this method.

Conclusion: The paper concludes that rigorous, logically-sound explanations for tree ensemble predictions are necessary for building trust and aims to contribute to this goal by investigating explanations for random forests and boosted trees.
...
TL;DR: Large language models (LLMs) can generate clinically adequate prior authorization letters, but often lack administrative precision required by payer workflows.


<details>
  <summary>Details</summary>
Motivation: Prior authorization is a burdensome process in US healthcare, and the study aims to evaluate the ability of LLMs to produce submission-ready prior authorization letters.

Method: The study assessed three commercially available LLMs (GPT-4o, Claude Sonnet 4.5, and Gemini 2.5 Pro) across 45 physician-validated synthetic scenarios spanning multiple medical specialties.

Result: All three LLMs generated letters with strong clinical content, but had gaps in administrative requirements such as absent billing codes, missing authorization duration requests, and inadequate follow-up plans.

Conclusion: The challenge for clinical deployment of LLMs is not whether they can write clinically adequate letters, but whether the systems built around them can supply the administrative precision required by payer workflows.
...
TL;DR: The paper re-evaluates the capabilities of AI agents in constructing ELT pipelines, finding that improved models and benchmark corrections significantly enhance performance, highlighting the need for systematic quality auditing in data engineering evaluation.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to re-evaluate the capabilities of AI agents in constructing ELT pipelines, as initial results showed low success rates, and to identify factors that may have contributed to this underestimation.

Method: The authors use upgraded large language models to re-evaluate ELT-Bench and develop an Auditor-Corrector methodology to audit benchmark quality, combining LLM-driven root-cause analysis with human validation.

Result: The results show that both rapid model improvement and benchmark quality issues contributed to underestimating agent capabilities, and that systematic quality auditing is necessary for complex agentic tasks.

Conclusion: The paper concludes that systematic quality auditing should be standard practice for complex agentic tasks, and releases ELT-Bench-Verified to provide a more reliable foundation for progress in AI-driven data engineering automation.
...
TL;DR: A new metric, MST-C, is proposed to evaluate the compactness of attributions in explanations, capturing their geometric properties and organization.


<details>
  <summary>Details</summary>
Motivation: The difficulty in quantitatively assessing explanation legibility due to varying shapes and internal organization of attributions not captured by simple statistics.

Method: Introduction of Minimum Spanning Tree Compactness (MST-C), a graph-based structural metric.

Result: MST-C reliably distinguishes between explanation methods, exposes structural differences between models, and provides a robust diagnostic for explanation compactness.

Conclusion: The proposed metric, MST-C, offers a novel approach to evaluating attribution quality by assessing compactness and providing insights into the geometric properties of attributions.
...
TL;DR: Introduces Metriplector, a neural architecture primitive that uses abstract physical systems for computation, achieving strong performance across four domains.


<details>
  <summary>Details</summary>
Motivation: The authors aim to create a more unified and flexible neural architecture by leveraging abstract physical systems for computation.

Method: Metriplector uses coupled metriplectic dynamics and the stress-energy tensor derived from Noether's theorem to perform computations.

Result: Metriplector achieves strong performance in four domains: maze pathfinding (F1=1.0), Sudoku solving (97.2% exact solve rate), image recognition on CIFAR-100 (81.03%), and language modeling (1.182 bits/byte).

Conclusion: Metriplector demonstrates the potential of using abstract physical systems for neural computation, achieving competitive performance across multiple domains.
...
TL;DR: The paper proposes PRoSFI, a novel reward method that enhances reasoning reliability in large language models by verifying intermediate steps through formal verification.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the issue of unreliable reasoning steps in large language models, even when the final answers are correct, by improving the evaluation of intermediate steps.

Method: The method proposed is PRoSFI (Process Reward over Structured Formal Intermediates), which involves the model generating structured intermediate steps that are then verified by a formal prover, rewarding only fully validated reasoning chains.

Result: The result of this approach is the generation of more credible final answers by guiding the model towards producing step-by-step machine-checkable proofs, thereby enhancing reasoning reliability without compromising accuracy.

Conclusion: The paper concludes that PRoSFI offers a simple and effective approach to training trustworthy reasoning models by integrating formal verification into the reward mechanism, leading to more reliable and credible reasoning processes.
...
TL;DR: FlowPIE, a retrieval-generation framework, improves scientific idea generation by treating literature exploration and idea generation as a co-evolving process.


<details>
  <summary>Details</summary>
Motivation: Existing approaches to scientific idea generation are constrained by a static retrieval-then-generation paradigm, leading to homogeneous and insufficiently divergent ideas.

Method: FlowPIE uses a flow-guided Monte Carlo Tree Search (MCTS) and a generative reward model (GRM) to guide adaptive retrieval and construct a diverse initial population, then applies selection, crossover, and mutation to evolve ideas.

Result: FlowPIE consistently produces ideas with higher novelty, feasibility, and diversity compared to strong LLM-based and agent-based frameworks.

Conclusion: FlowPIE effectively mitigates the limitations of existing approaches to scientific idea generation, enabling more diverse and high-quality ideas.
...
TL;DR: The paper presents ASI-Evolve, an AI-for-AI research framework that accelerates AI development by closing the research loop through a learn-design-experiment-analyze cycle.


<details>
  <summary>Details</summary>
Motivation: The motivation is to investigate whether AI can accelerate its own development, particularly in costly, long-horizon, and weakly supervised research loops.

Method: The authors propose ASI-Evolve, an agentic framework that augments standard evolutionary agents with a cognition base and a dedicated analyzer.

Result: ASI-Evolve demonstrates AI-driven discovery across three central components of AI development: data, architectures, and learning algorithms, achieving state-of-the-art results in neural architecture design, pretraining data curation, and reinforcement learning algorithm design.

Conclusion: The results suggest that ASI-Evolve represents a promising step toward enabling AI to accelerate AI across the foundational stages of development, offering early evidence for the feasibility of closed-loop AI research.
...
TL;DR: The paper presents an optimization framework for scheduling donor invitations to blood donation sessions, balancing donor eligibility, travel convenience, and blood-type demand targets. It compares a binary integer linear programming (BILP) formulation with a greedy heuristic, showing the heuristic achieves comparable results with significantly less computational resources.


<details>
  <summary>Details</summary>
Motivation: Blood donation centers face challenges in matching supply with demand while managing donor availability, and there is a need for effective recruitment strategies that balance constraints with donor convenience and eligibility.

Method: The paper proposes an optimization framework for donor invitation scheduling, using a BILP formulation and a greedy heuristic, and evaluates their performance using data from the Instituto Português do Sangue e da Transplantação (IPST) in the Lisbon operational region.

Result: The results show that the greedy heuristic achieves comparable results to the BILP, with 188x less peak memory and 115x faster runtime, but with some trade-offs in demand fulfillment, donor-session distance, and invitation burden.

Conclusion: The paper concludes that the proposed optimization framework can help close the supply-demand gap in blood donation, and that the greedy heuristic can be a practical solution for large-scale donor invitation scheduling problems.
...
TL;DR: The paper challenges the idea that generative AI amplifies the Dunning-Kruger effect, instead finding that AI use improves output and task performance but degrades metacognitive accuracy and proposes a new model of AI-mediated metacognitive decoupling.


<details>
  <summary>Details</summary>
Motivation: The common claim that generative AI amplifies the Dunning-Kruger effect is too coarse and doesn't accurately capture the available evidence.

Method: The paper synthesizes evidence from human-AI interaction, learning research, and model evaluation.

Result: The findings suggest that large language model (LLM) use can improve observable output and short-term task performance while degrading metacognitive accuracy and flattening the classic competence-confidence gradient across skill groups.

Conclusion: The paper concludes with implications for tool design, assessment, and knowledge work, proposing a new model of AI-mediated metacognitive decoupling.
...
TL;DR: Proposing a method to extract first-order formulas from probability distributions, reducing the number of values required and increasing sparsity while preserving core information.


<details>
  <summary>Details</summary>
Motivation: Real-world scenarios can be captured with lifted probability distributions, but these distributions require an exponential number of values when encoded in a table or list.

Method: Extracting first-order formulas from probability distributions by reducing the number of values in a distribution and then extracting logical formulas to be further minimized.

Result: The evaluation shows that sparsity can increase immensely by extracting a small set of short formulas while preserving core information.

Conclusion: The proposed method allows for increasing the sparsity in the encoding of probability distributions while generalizing a given distribution.
...
TL;DR: This paper proposes using the meta-d' framework and signal detection theory to assess the metacognitive abilities of AI systems, specifically their ability to generate confidence ratings and regulate decisions based on uncertainty and risk.


<details>
  <summary>Details</summary>
Motivation: The increasing integration of AI systems into decision-making workflows necessitates robust methods to measure their metacognitive abilities, which are crucial for managing uncertainty and making reliable decisions.

Method: The paper advocates for the adoption of the meta-d' framework and its model-free alternatives to assess metacognitive sensitivity in AIs, and the use of signal detection theory to evaluate the ability of AIs to regulate their decisions based on uncertainty and risk.

Result: The experiments conducted on three large language models (GPT-5, DeepSeek-V3.2-Exp, and Mistral-Medium-2508) demonstrate the practical utility of these psychophysical frameworks in evaluating AI metacognition.

Conclusion: The study concludes that the meta-d' framework and signal detection theory can be effectively used to assess and compare the metacognitive abilities of AI systems, enabling more informed evaluations of their reliability and decision-making capabilities.
...
TL;DR: Symphony for Medical Coding is a system that uses reasoning over clinical narratives and direct access to coding guidelines to predict medical codes, offering flexibility across coding systems and providing evidence for predictions.


<details>
  <summary>Details</summary>
Motivation: Current medical coding methods are manual, slow, and error-prone. Existing automated approaches are limited by their need for retraining on new data or codes and lack explanations for their predictions.

Method: Symphony for Medical Coding approaches medical coding by reasoning over clinical narratives with direct access to coding guidelines, allowing it to operate across any coding system and provide span-level evidence for predictions.

Result: Symphony achieves state-of-the-art results across various settings, including inpatient, outpatient, emergency, and subspecialty settings in the United States and the United Kingdom.

Conclusion: Symphony for Medical Coding establishes itself as a flexible, deployment-ready foundation for automated clinical coding, addressing current limitations in the field.
...
TL;DR: ReTriP, a generative framework, is proposed for retrosynthetic planning in organic chemistry, achieving state-of-the-art performance by embedding strategic foresight into chemical reasoning.


<details>
  <summary>Details</summary>
Motivation: The complexity of retrosynthetic planning in organic chemistry due to its combinatorial nature, and the limitations of conventional hybrid approaches that separate local transformations from global objectives.

Method: ReTriP reformulates retrosynthesis as a Chain-of-Thought reasoning task, using a path-coherent molecular representation and a progressive training curriculum with reasoning distillation and reinforcement learning.

Result: ReTriP achieves state-of-the-art performance on RetroBench, showing superior robustness in long-horizon planning compared to hybrid baselines.

Conclusion: ReTriP successfully bridges the gap between local molecular transformations and global planning objectives, providing a more coherent and effective approach to retrosynthetic planning.
...
TL;DR: Large language models develop brain-like synergistic cores that enable abstract reasoning, similar to the human brain.


<details>
  <summary>Details</summary>
Motivation: To identify universal computational principles of intelligence in artificial systems by studying large language models.

Method: Using Integrated Information Decomposition across multiple architectures to analyze large language models.

Result: Large language models develop synergistic cores in middle layers, which are crucial for abstract reasoning and lead to catastrophic performance loss when ablated.

Conclusion: The study reveals a universal computational principle of intelligence, bridging artificial and biological intelligence, and highlights the importance of synergistic processing in abstract reasoning.
...
TL;DR: CausalPulse, a multi-agent copilot, automates causal diagnostics in smart manufacturing with high reliability and real-time capability.


<details>
  <summary>Details</summary>
Motivation: Modern manufacturing needs real-time, trustworthy, and interpretable root-cause insights to maintain productivity and quality, but traditional analytics pipelines have limitations in scalability and explainability.

Method: CausalPulse uses a neurosymbolic architecture with standardized agentic protocols to unify anomaly detection, causal discovery, and reasoning.

Result: CausalPulse achieved high reliability with success rates of 98.0% and 98.73% on public and proprietary datasets, and demonstrated real-time readiness with end-to-end latency of 50-60s per diagnostic workflow.

Conclusion: CausalPulse enables reliable, interpretable, and production-ready automation for next-generation manufacturing with its modular, human-in-the-loop design.
...
TL;DR: The paper proposes a human-like chess engine that mimics a strong human player's style, errors, and consistency. It identifies a dual-capability bottleneck in training models from move sequences, where state tracking and decision quality have contradictory data requirements. The authors propose scaling the model and introducing Elo-weighted training to improve performance.


<details>
  <summary>Details</summary>
Motivation: To create a human-like chess engine that mimics the style, errors, and consistency of a strong human player, rather than maximizing playing strength.

Method: The authors train a model from move sequences, formalizing the tension between state tracking and decision quality as a dual-capability bottleneck. They scale the model from 28M to 120M parameters and introduce Elo-weighted training to improve decisions while preserving diversity.

Result: The final 120M-parameter model reaches Lichess bullet 2570 over 253 rated games and achieves 55.2% Top-1 accuracy on human move prediction, exceeding Maia-2 rapid and Maia-2 blitz.

Conclusion: The proposed approach enables a human-like chess engine that mimics a strong human player's style, errors, and consistency. The dual-capability bottleneck formalization and Elo-weighted training are key to achieving this goal.
...
TL;DR: The paper introduces Simula, a novel framework for generating synthetic datasets at scale, allowing for controllable and explainable data creation, addressing issues of data scarcity and inaccessibility for AI model training.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the challenge of data scarcity or inaccessibility for training multi-modal AI models. Current methods for synthetic data generation have limitations in scalability, explainability, and control.

Method: The method introduced in this paper is Simula, a reasoning-driven framework that uses a seedless, agentic approach to generate synthetic datasets. It allows users to define desired dataset characteristics through an explainable and controllable process.

Result: The results show the efficacy of Simula on various datasets, with rigorous testing of intrinsic and downstream properties. The framework provides guidelines for synthetic data mechanism design and insights into generating and evaluating synthetic data at scale.

Conclusion: The conclusion of this paper is that Simula offers a novel solution for generating synthetic datasets at scale, enabling fine-grained resource allocation, and unlocking new opportunities for AI development and deployment in data-scarce domains.
...
TL;DR: Owl-AuraID is a software-hardware system that automates scientific instrument operation through GUI-native interfaces, enabling end-to-end workflows and broad coverage across various precision instruments.


<details>
  <summary>Details</summary>
Motivation: Automation in scientific discovery is hindered by proprietary GUIs and limited generalizability of existing API-based systems.

Method: Owl-AuraID adopts a GUI-native paradigm, integrating Type-1 (GUI operation) and Type-2 (data analysis) skills into end-to-end workflows.

Result: Owl-AuraID demonstrates broad coverage across ten categories of precision instruments and diverse workflows, supporting various modalities such as FTIR, NMR, AFM, and TGA.
...
TL;DR: A comprehensive validation framework for LLM-based agentic systems was introduced, enabling systematic diagnosis and improvement of reliability failures. The framework was applied to IBM CUGA, revealing weaknesses and leading to refined prompting and coding strategies.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the reliability failures in LLM-based agentic systems by introducing a comprehensive validation framework.

Method: The framework includes fifteen failure-detection tools and two root-cause analysis modules, integrating lightweight rule-based checks with LLM-as-a-judge assessments.

Result: The analysis revealed recurrent planner misalignments, schema violations, brittle prompt dependencies, and more. Refined prompting and coding strategies maintained benchmark results and enabled mid-sized models to achieve notable accuracy gains.

Conclusion: The results show a path toward scalable, quality assurance, and adaptive validation in production agentic systems, offering a foundation for more robust, interpretable, and self-improving agentic architectures.
...
TL;DR: This paper proposes a new concept of spatiotemporal robustness (STR) for temporal logic specifications, which captures admissible spatial and temporal perturbations jointly, and presents a method to compute STR using multi-objective optimization.


<details>
  <summary>Details</summary>
Motivation: The reliability of autonomous systems depends on their robustness under uncertainty, and existing work has limitations in capturing spatial and temporal perturbations separately.

Method: The authors define STR as a multi-objective reasoning problem, formalized via a partial order over spatial and temporal perturbations, and propose robust semantics for STR that are sound and computationally tractable.

Result: The authors present monitoring algorithms for STR using the proposed robust semantics, and show that STR can be interpreted as a Pareto-optimal set that characterizes all admissible spatiotemporal perturbations.

Conclusion: The paper provides a new perspective on robustness in autonomous systems by introducing spatiotemporal robustness, and offers a method to compute STR using multi-objective optimization, which has applications in interacting systems such as multi-agent robotics and smart cities.
...
TL;DR: The paper proposes Shapley-Enhanced GRPO (ShapE-GRPO), a method to improve the training of Large Language Models (LLMs) by decomposing set-level rewards into candidate-specific signals, leading to better exploration and performance.


<details>
  <summary>Details</summary>
Motivation: Existing reinforcement learning methods for LLMs, such as Group Relative Policy Optimization (GRPO), assign the same set-level scalar reward to every candidate in a set, leading to noisy training signals and suboptimal exploration.

Method: The authors propose Shapley-Enhanced GRPO (ShapE-GRPO), which leverages cooperative game theory to decompose set-level rewards into granular, candidate-specific signals, preserving the fundamental axioms of the Shapley value with polynomial-time complexity.
...
Conclusion: The proposed method, ShapE-GRPO, offers a more effective approach to training LLMs by providing more accurate and candidate-specific training signals, leading to improved performance and efficiency.
...
TL;DR: A new information-theoretic theory of categorization is proposed and evaluated against classic experiments, showing comparable or better performance than existing models.


<details>
  <summary>Details</summary>
Motivation: To develop a new theory of categorization based on information-theoretic rational analysis and compare its performance with existing models.

Method: Information-theoretic rational analysis, comparison with existing models using classic categorization experiments.

Result: The new theory explains human categorization behavior at least as well as or better than existing models.

Conclusion: The proposed theory is a viable and potentially superior model for understanding human categorization behavior.
...
TL;DR: The paper proposes a new paradigm for multimodal large language models (MLLMs) called Agentic Tool Planning, which enables models to autonomously determine when and how to use tools to generate interleaved text-and-image responses. The authors introduce a novel benchmark, ATP-Bench, and a Multi-Agent MLLM-as-a-Judge (MAM) system to evaluate this paradigm.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs rely on either image generation or retrieval augmentation, but they treat these approaches as mutually exclusive, failing to unify factuality with creativity. The authors argue that Agentic Tool Planning is the next milestone in this field, enabling models to produce more intuitive and complex information.

Method: The authors introduce ATP-Bench, a benchmark comprising 7,702 QA pairs across eight categories and 25 visual-critical intents. They also propose a MAM system to evaluate agentic planning independent of end-to-end execution and changing tool backends.

Result: The authors' experiments on 10 state-of-the-art MLLMs reveal that models struggle with coherent interleaved planning and exhibit significant variations in tool-use behavior, highlighting substantial room for improvement.

Conclusion: The paper concludes that Agentic Tool Planning is a promising paradigm for MLLMs, and that there is significant room for improvement in current models. The authors provide actionable guidance for advancing interleaved generation and make their dataset and code available.
...
TL;DR: The paper proposes C-TRAIL, a framework that combines large language models (LLMs) with a trust mechanism to improve autonomous driving trajectory planning.


<details>
  <summary>Details</summary>
Motivation: LLM outputs are inherently unreliable, posing risks in safety-critical applications such as autonomous driving.

Method: C-TRAIL uses a closed-loop Recall, Plan, and Update cycle, incorporating a dual-trust mechanism and Dirichlet trust policy to guide trajectory planning.

Result: C-TRAIL outperforms state-of-the-art baselines, reducing ADE by 40.2%, FDE by 51.7%, and improving SR by 16.9 percentage points on average.

Conclusion: The proposed C-TRAIL framework effectively addresses the reliability concerns of LLMs in autonomous driving trajectory planning, leading to improved performance and safety.
...
TL;DR: The paper proposes using epistemic uncertainty to evaluate the reliability of post-hoc explanation methods for black-box predictions, enabling more efficient and reliable explanation generation.


<details>
  <summary>Details</summary>
Motivation: The generation of post-hoc explanation methods is often computationally expensive and their reliability is not guaranteed.

Method: The authors propose using epistemic uncertainty as a low-cost proxy for explanation reliability and explore two use cases: improving worst-case explanations and recalling high-quality explanations.

Result: The authors observe a strong negative correlation between epistemic uncertainty and explanation stability across four tabular datasets, five diverse architectures, and four XAI methods, and confirm that their findings generalize beyond tabular data.

Conclusion: Epistemic uncertainty can be used to identify regions where explanations are unstable and unfaithful, enabling more efficient and reliable explanation generation.
...
TL;DR: The authors introduce ScoringBench, a benchmark for evaluating tabular foundation models with a comprehensive suite of metrics, highlighting the importance of proper scoring rules in assessing probabilistic forecast quality, particularly in high-stakes domains.


<details>
  <summary>Details</summary>
Motivation: The prevailing regression benchmarks focus on point estimate metrics, which can obscure model performance in the tails of the distribution, a critical deficit for high-stakes decision making in domains like finance and clinical research.

Method: The authors evaluate realTabPFNv2.5 fine-tuned with different scoring rule objectives and TabICL relative to untuned realTabPFNv2.5 across a suite of regression benchmarks, using a comprehensive suite of proper scoring rules.

Result: The results confirm that model rankings depend on the chosen scoring rule and that no single pretraining objective is universally optimal, demonstrating the importance of choosing the right evaluation metric for applications sensitive to extreme events.

Conclusion: The authors conclude that for applications sensitive to extreme events, the choice of evaluation metric is as much a domain-specific requirement as the data itself, and introduce ScoringBench as a tool to facilitate this.
...
TL;DR: The study investigates the physiological and conversational dynamics of medical teams using an intelligent tutoring system to diagnose a virtual patient, finding correlations between physiological synchrony, semantic shifts in dialogue, and team performance.


<details>
  <summary>Details</summary>
Motivation: Effective collaboration requires teams to manage complex cognitive and emotional states through Socially Shared Regulation of Learning (SSRL), and physiological synchrony can indicate these states.

Method: The study used an intelligent tutoring system with four medical dyads, analyzing physiological and conversational dynamics, and coding utterance segments for SSRL and deriving cosine similarity using sentence embeddings.

Result: The results showed that activating prior knowledge featured significantly lower semantic similarity than simpler task execution, and high physiological synchrony was associated with lower semantic similarity, indicating exploratory and varied language use.

Conclusion: The research advances human-centered AI by demonstrating how biological signals can be fused with dialogues to understand critical moments in problem solving, and identifies 'pivotal moments' where successful teams synchronize during shared discovery, while unsuccessful teams peak during shared uncertainty.
...
TL;DR: Structured intent representations improve goal alignment across AI models, languages, and prompting frameworks.


<details>
  <summary>Details</summary>
Motivation: To investigate the reliability of structured intent representations in preserving user goals across different AI models, languages, and prompting frameworks.

Method: Extended prior work on PPS (Prompt Protocol Specification) to test cross-model robustness, compared with CO-STAR and RISEN, and conducted a user study (N=50) on AI-assisted intent expansion.

Result: Structured prompting reduces cross-language score variance, achieves high goal-alignment scores, and improves user satisfaction and interaction efficiency.

Conclusion: Structured intent representation is a robust, protocol-like communication layer for human-AI interaction, supporting its practical value.
...
TL;DR: The paper reproduces and extends the MONA (Myopic Optimization with Non-myopic Approval) environment, exploring how different approval mechanisms impact safety guarantees.


<details>
  <summary>Details</summary>
Motivation: The original MONA paper left open the question of how the method of constructing approval affects safety guarantees. The authors aim to investigate this by extending the MONA Camera Dropbox environment.

Method: The authors repackage the MONA codebase as a standard Python project, confirm published results, and introduce a modular learned-approval suite. They then run pilot sweeps across various approval methods, horizons, and calibration strategies.

Result: The best calibrated learned-overseer run achieves zero observed reward hacking but with lower intended-behavior rates (11.9%) compared to oracle MONA (99.9%).

Conclusion: The results suggest that the central engineering challenge shifts from proving MONA's concept to building learned approval models that preserve sufficient foresight without reopening reward-hacking channels.
...
TL;DR: The paper proposes the Triadic Cognitive Architecture (TCA), a unified mathematical framework that grounds machine reasoning in continuous-time physics, to address the limitations of current autonomous AI agents.


<details>
  <summary>Details</summary>
Motivation: Current autonomous AI agents, driven by Large Language Models (LLMs), lack an intrinsic sense of network topology, temporal pacing, or epistemic limits, leading to failure modes in interactive environments.

Method: The TCA synthesizes nonlinear filtering theory, Riemannian routing geometry, and optimal control to formally define Cognitive Friction and map the agent's deliberation process to a coupled stochastic control problem.

Result: The TCA demonstrates improved performance in a simulated Emergency Medical Diagnostic Grid (EMDG), reducing time-to-action and improving patient viability without degrading diagnostic accuracy.

Conclusion: The Triadic Cognitive Architecture (TCA) provides a unified mathematical framework for machine reasoning, addressing the limitations of current autonomous AI agents and improving their performance in interactive environments.
