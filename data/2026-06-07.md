...
Motivation: The need for a large-scale training corpus specifically designed to strengthen knowledge- and reasoning-intensive video understanding.

Method: Developed a human-in-the-loop, skill-oriented example generation pipeline and created VideoKR, a dataset of 315K video reasoning examples over 145K CC-licensed videos.
...
TL;DR: A new method called NIV uses neural networks to automatically convert static fonts into variable fonts, enabling continuous variation in glyph geometry.


<details>
  <summary>Details</summary>
Motivation: The labor-intensive process of manually creating variable fonts from static fonts requires expert typographic design, which is a significant bottleneck.

Method: NIV (Neural Axis Variations) predicts per-point displacements given glyph outlines and desired design axes, using a novel Property Embedding mechanism to capture interactions between multiple axes.

Result: The trained model generalizes across unseen code points, font styles, and even out-of-distribution inputs, generating standard variable font files supporting continuous interpolation.

Conclusion: NIV enables the automatic conversion of static fonts to variable fonts, demonstrating a new approach for synthesizing structured geometric objects with continuous parametric variation using neural deformations.
...
TL;DR: This paper studies a conversational AI assistant that can access a user's personal camera roll to answer queries, and proposes a dataset and an agent to improve long-context understanding.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enable a conversational AI assistant to understand a user's personal camera roll and answer queries, which requires navigating and locating relevant information in a large, personalized visual content stream.

Method: The authors collect and manually annotate a dataset called camroll, which contains 50 users, 31,476 images, and 2,500 QA pairs. They also design a conversational AI agent called camroll-agent, which is equipped with hierarchical memory and a minimal set of tools for efficient navigation.

Result: The experimental results show that camroll-agent outperforms numerous baselines and methods for long-context understanding AI agents. The authors highlight the gap in AI agents' long-context reasoning, particularly in personalized visual memory.

Conclusion: The paper concludes that personalized visual memory requires different approaches from standard long-context textual memory, especially when consistency, visual details, and user-specific context are present.
...
TL;DR: The authors propose a framework for cross-model safety steering in generative models, allowing safety control to be transferred across different models without requiring access to unsafe data.


<details>
  <summary>Details</summary>
Motivation: The need for a portable safety control mechanism that can be applied across different generative models, reducing the need for model-specific safety interventions.

Method: The authors estimate a safety direction in a source model using paired safe-unsafe prompts, then transport it to a target model through a lightweight alignment fitted on benign data. They also propose a multi-vector extension for category-specific safety behaviors.

Result: The transferred safety directions achieve comparable performance to native safety directions learned on the target model, in terms of ASR reduction and CLIP-Score/FID trade-offs, without requiring access to unsafe data on the target side.

Conclusion: The authors' approach suggests that safety-relevant behavior can be controlled through latent directions that persist across models, paving the way for lightweight, reusable safety mechanisms that do not require target-side unsafe data.
...
TL;DR: A two-stage framework, TopoPult-SSL, is proposed for cross-device meibomian gland segmentation, leveraging weak-prior anchors and clinical metadata to adapt to new devices without requiring dense gland masks.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the domain shift issue when new clinical imaging devices are introduced, making it expensive to obtain dense gland masks, while cheap clinical signals like eyelid outlines are readily available.

Method: The method involves a two-stage framework: Stage 1 uses four weak-prior anchors driven by target eyelid masks and clinical metadata to adapt a source-trained model without target gland masks; Stage 2 distills complementary Stage-1 teachers into a single compact student via supervised self-distillation when target gland masks are available.

Result: The distilled model achieves a Dice score of 0.716+/-0.006, surpassing UA-MT (0.710) and the ensemble teacher (0.720), and the gland-mask-free Stage-1 variant reaches Precision 0.694, outperforming SAM/MedSAM (0.30-0.34).

Conclusion: The proposed TopoPult-SSL framework enables effective cross-device meibomian gland segmentation without requiring dense gland masks, achieving comparable performance to state-of-the-art methods.
...
TL;DR: Proposed LightVesselNet, a lightweight neural network for retinal vessel segmentation, achieving competitive performance with much larger models while requiring fewer resources.


<details>
  <summary>Details</summary>
Motivation: To develop an efficient neural network for retinal vessel segmentation that can be deployed on edge devices in resource-constrained environments.

Method: LightVesselNet, a compact encoder-decoder architecture with attention mechanisms, multi-scale feature aggregation, and subpixel upsampling, and a dedicated edge residual connection.

Result: LightVesselNet achieved sensitivity scores of 0.8189, 0.8499, 0.8640, 0.8634, 0.8096 and Dice coefficients of 0.8070, 0.8072, 0.8181, 0.8649, 0.7686 on five publicly available datasets.

Conclusion: LightVesselNet is a strong candidate for deployment in low-resource clinical settings and mobile screening tools, showing improved efficiency and generalisation capability.
...
TL;DR: RePHO is a method for reconstructing human-object interactions from monocular videos, focusing on physical plausibility.


<details>
  <summary>Details</summary>
Motivation: Existing kinematic-based approaches for human-object interaction reconstruction often produce physically implausible artifacts.
...
Conclusion: RePHO successfully reconstructs physically plausible human-object interactions from monocular videos.
...
TL;DR: A new benchmark dataset, Biomazon, is introduced for predicting tropical forest structure and biomass using multimodal data and machine learning, providing a standardized framework for evaluating methods.


<details>
  <summary>Details</summary>
Motivation: The lack of a benchmark dataset for predicting the entire forest vertical structure as an ordered profile, rather than separate scalar targets, and the need for a standardized evaluation protocol.

Method: A shared encoder-decoder with task-specific heads is used as a baseline framework, and a comprehensive ablation study is conducted to evaluate the contributions of different modalities and auxiliary embeddings.

Result: The benchmark dataset, Biomazon, and the baseline results establish a reference for future work on structurally consistent RH-profile prediction and structure-biomass modeling in tropical forests.

Conclusion: The Biomazon dataset and accompanying protocols provide a standardized framework for evaluating methods and predicting tropical forest structure and biomass, enabling more accurate carbon accounting and ecosystem monitoring.
...
TL;DR: A deep learning-based algorithm is proposed to restore capillary anatomical vasculature from a single OCTA volume, improving image quality and microvascular fidelity.


<details>
  <summary>Details</summary>
Motivation: Existing methods for imaging retinal microvasculature using OCTA neglect the intrinsic three-dimensional vascular architecture and focus on noise suppression, projection artifact removal, or signal enhancement in 2D projections.

Method: A deep learning-based algorithm using an EfficientNet-B5 encoder and a decoder with concurrent spatial and channel squeeze-and-excitation modules, taking three adjacent B-frames as input to predict the restored middle B-frame.

Result: The proposed model significantly improved image quality (PSNR: 26.16 vs. 22.23, SSIM: 0.91 vs. 0.72) and microvascular fidelity (Dice coefficient overlap: at least 3.8% and 51.2% improvement in 2D and 3D, respectively).

Conclusion: The proposed algorithm effectively restores capillary anatomical vasculature from a single OCTA volume, improving image quality and microvascular fidelity, which can enhance the reliability of retinal blood flow and nonperfusion area quantification.
...
TL;DR: Deep learning models using OCT/OCTA data accurately grade AMD severity, with a biomarker-based model showing the most balanced performance and value for early AMD detection.


<details>
  <summary>Details</summary>
Motivation: To develop and evaluate deep learning models for automated grading of age-related macular degeneration (AMD) severity using optical coherence tomography (OCT) and OCT angiography (OCTA) data.

Method: Three deep learning models were developed using different input modalities: biomarker maps, 2D en face OCT and OCTA projections, and 3D OCT/OCTA volumes. EfficientNet-based architectures were trained using normalized inputs, data augmentation, and five-fold cross-validation.

Result: All models demonstrated strong AMD staging performance with substantial agreement with the reference standard (QWK >= 0.83). The biomarker-based model achieved the highest overall performance (QWK = 0.85 +/- 0.03) and the best detection of early AMD (F1-score = 0.59 +/- 0.14).

Conclusion: Deep learning models using OCT/OCTA data can accurately and automatically grade AMD severity. The biomarker-based model provided the most balanced performance and showed particular value for early AMD detection.
...
TL;DR: UNIPIXIE, a novel framework, predicts a continuous distribution of material properties from a single visual input, enabling controllable generation of diverse, physically valid material fields.


<details>
  <summary>Details</summary>
Motivation: Existing feed-forward networks fail to capture the real world's inherent physical ambiguity by only predicting a single set of physical properties from visual appearance.

Method: UNIPIXIE learns a direct mapping along an object's softest-to-stiffest spectrum on the PIXIEMULTIVERSE dataset, producing simulation-ready parameters for diverse physics solvers.
...
Conclusion: UNIPIXIE bridges the gap between static point estimates and the continuous nature of physical reality, enabling more accurate and controllable prediction of material properties.
...
TL;DR: The paper studies how vision-language models (VLMs) learn novel visual concepts, especially when they contradict prior knowledge. It introduces a new dataset, NVRD, and finds that models struggle to acquire new concepts and overgeneralize compared to humans.


<details>
  <summary>Details</summary>
Motivation: To investigate how VLMs map novel visual references to language after exposure, particularly when those references contradict prior knowledge from pre-training.

Method: The authors present the Novel Visual References Dataset (NVRD) and evaluate 3 open- and 2 closed-source models alongside 2,400 human judgments.

Result: Models struggle to acquire novel concepts in-context when they contradict prior knowledge, and while models and humans show correlated sensitivity to visual perturbations, models significantly overgeneralize.

Conclusion: The paper contributes NVRD as a corpus and benchmark for research on visual concept learning in both humans and machines, highlighting the challenges of novel concept learning in VLMs.
...
TL;DR: The authors propose a novel framework called DFPL for addressing the missing-modality problem in image-tabular multimodal learning. DFPL uses fine-grained prototype learning to improve cross-modal consistency and robustness to missing modalities.


<details>
  <summary>Details</summary>
Motivation: The missing-modality problem is a significant challenge in image-tabular multimodal learning, particularly when the two modalities are highly heterogeneous. Existing methods overlook fine-grained semantic and distributional misalignment, which hampers the exploitation of complementary cues under missing modalities.

Method: The proposed framework, DFPL, consists of three main components: Shared-Specific Prototype Modeling (SSPM) for extracting compact and diverse prototypes, Prototype-guided Fine-grained Alignment (PFA) for enforcing prototype-level distribution matching and semantic alignment, and Class-aware Multi-scale Aggregation (CMA) for adaptively aggregating shared semantics and modality-specific characteristics.
...
Conclusion: The authors conclude that their proposed framework, DFPL, effectively addresses the missing-modality problem in image-tabular multimodal learning by preserving fine-grained distributional and semantic consistency across modalities.
...
<details>
  <summary>Details</summary>
Motivation: Pain and affective state assessment in horses through automated facial action unit detection.

Method: Three methods: frame-based YOLOv12 detector, optical flow magnitude thresholding, and fine-tuned VideoMAE model.

Result: Macro-F1 score of 0.898 for blink classification and 0.926 for binary blink detection.

Conclusion: Promising results but challenges in fine-grained AU detection for equine welfare monitoring.
...
TL;DR: The authors propose ORACLE-CT, an anatomy-aware framework for abdominal CT disease classification that uses multi-organ segmentation to improve evidence aggregation and attention pooling. The framework is evaluated with various encoder families and shows improved performance and external robustness.


<details>
  <summary>Details</summary>
Motivation: The challenge of abdominal CT disease classification due to the large 3D volume of scans and the localized nature of diagnostic evidence, which is often not well-matched by current study-level classifiers that use anatomy-agnostic pooling or attention.

Method: The authors propose ORACLE-CT, an encoder-agnostic anatomy-aware aggregation framework that uses multi-organ segmentation to define label-specific anatomical supports and restrict attention pooling to relevant regions. The framework is evaluated with three encoder families: DINOv3, I3D-ResNet-121, and Pillar-0.

Result: The results show that ORACLE-CT improves performance on MERLIN, Duke-Abdomen, and AMOS datasets, with improvements in macro-AUROC/AUPRC for DINOv3 (0.838/0.638 to 0.858/0.676) and I3D-ResNet-121 (0.829/0.617 to 0.848/0.659) on MERLIN. External evaluation on Duke-Abdomen and AMOS also shows improved performance.

Conclusion: ORACLE-CT improves discrimination and external robustness in abdominal CT disease classification while preserving an auditable link between predictions and anatomical evidence.
...
TL;DR: This paper proposes a concept-based approach to learning semantics in deep learning models, utilizing formal concept lattices to guide neural network learning and create hierarchical, interpretable representations.
...
Method: The authors draw on Formal Concept Analysis and use formal concept lattices to provide principled semantic scaffolds for guiding neural network learning, allowing the model to develop staged, semantically grounded representations throughout its depth.
...
Conclusion: The authors conclude that their approach effectively creates hierarchical, interpretable representations in deep learning models, enhancing their alignment with human reasoning and semantic understanding.
...
TL;DR: This paper explores predicting and improving human preference metrics for diffusion models, which generate high-quality visual content from text prompts, by leveraging predictions to enhance image quality with negligible hardware overhead.


<details>
  <summary>Details</summary>
Motivation: The stochastic nature of diffusion models affects generated content quality, and predicting human preference metrics (HPM) before generation can improve image quality, especially in smaller models.

Method: The authors investigate predicting scalar HPM scores and leveraging these predictions to improve generated image quality, studying which HPMs are best suited for this task.

Result: The investigation reveals that predicting and improving HPM scores is possible with negligible hardware overhead, enabling better image quality in diffusion models.

Conclusion: The study demonstrates the feasibility of predicting and enhancing human preference metrics in diffusion models, leading to improved image quality without significant computational costs.
...
TL;DR: A phase-aware large language model (LLM) agent is proposed for optimizing multi-stage retrieval systems, outperforming traditional methods by navigating coupled parameter spaces.


<details>
  <summary>Details</summary>
Motivation: Traditional hyperparameter optimization (HPO) methods rely on an independence assumption that prevents them from optimizing highly coupled parameters in modern multi-stage retrieval systems.

Method: A phase-aware LLM agent that conditions each proposal on its full optimization history, navigating the coupled parameter space across phase-partitioned exploration, exploitation, and fine-tuning stages.

Result: The agent outperforms Optuna TPE by +33.3% and VDTuner by +34.2% under SIEVE, delivering a 15.3x throughput gain over UniIR, and shows transferability across vector database management system (VDBMS) platforms.

Conclusion: The proposed phase-aware LLM agent effectively optimizes multi-stage retrieval systems by navigating coupled parameter spaces, demonstrating significant performance improvements and transferability across different platforms.
...
TL;DR: A novel framework for unpaired RGB-thermal view synthesis is proposed, enabling 3D scene reconstruction with visual and thermal information without precise calibration.


<details>
  <summary>Details</summary>
Motivation: Existing methods for multi-modal novel view synthesis rely on precisely calibrated RGB-thermal image pairs or stereo setups, limiting scalability and practical deployment.

Method: The proposed framework uses a 3D feed-forward transformer architecture (VGGT) to estimate camera poses for each modality, and then aligns them using the Procrustes algorithm with a cross-modal feature matcher. A multi-modal 3D Gaussian Splatting approach is also proposed to learn from unpaired RGB and thermal images.

Result: The method achieves competitive performance in thermal view synthesis while maintaining RGB fidelity, and demonstrates the importance of evaluating both per-modality image synthesis and multi-modal coherence of reconstructed scenes.

Conclusion: The proposed framework enables precise 3D scene reconstruction with visual and thermal information without precise calibration, and provides a benchmarking framework for evaluating multi-modal coherence of reconstructed scenes.
...
Method: The method involves a sensor-guided adaptive contrastive learning framework that uses privileged LiDAR sensing to guide the contrastive objective, and a cross-stage domain mismatch to suppress environment-specific shortcuts.
...
TL;DR: BRepCLIP, a framework for learning representations of CAD models in boundary representations (BReps), aligns BRep geometry with language and image embeddings through contrastive pretraining.


<details>
  <summary>Details</summary>
Motivation: The native format of CAD, boundary representations BReps, has received little attention in representation learning, unlike point clouds and meshes.

Method: BRepCLIP models CAD objects as sequences of face and edge tokens with discrete vocabularies for surface and curve geometry, and uses a transformer encoder to aggregate these tokens into a global BRep embedding.

Result: BRepCLIP generates more discriminative and semantically grounded embeddings than point-based alternatives, improving retrieval and zero-shot classification performance on several benchmarks.

Conclusion: BRepCLIP establishes the importance of structure-aware pretraining for multimodal CAD understanding and demonstrates its utility as a CAD-aware similarity metric.
...
TL;DR: The paper introduces BloomBench, a multimodal benchmark for evaluating Vision-Language Models (VLMs) based on Bloom's Taxonomy, and assesses state-of-the-art VLMs, revealing cognitive asymmetries and performance gaps.


<details>
  <summary>Details</summary>
Motivation: The field lacks comprehensive benchmarks to evaluate the true reasoning abilities of VLMs, and existing evaluations focus on piecemeal tasks, obscuring critical cognitive weaknesses.

Method: The authors introduce BloomBench, a bilingual (English-Arabic) multimodal benchmark that systematically evaluates six levels of cognition through image-question-answer tasks, and conduct a comprehensive study of state-of-the-art VLMs.

Result: The analysis reveals a sharp cognitive asymmetry in state-of-the-art VLMs, with strong performance in semantic understanding but struggles with factual recall and creative synthesis, and a critical performance gap between Arabic and English.

Conclusion: The findings establish a foundation for developing more cognitively aligned and inclusive VLMs, and the benchmark framework and dataset are made available.
...
<details>
  <summary>Details</summary>
Motivation: Recent approaches to Med-VQA overlook the importance of handling noise and small irrelevant changes in visual representations, which can negatively impact performance.

Method: The proposed framework uses a denoising autoencoder to reconstruct clean visual embeddings from corrupted inputs, and then projects the resulting embeddings into the language model embedding space using a multi-layer perceptron (MLP).
...
TL;DR: Proposed a novel Dual Feature Decoupling Network (DFDNet) for fine-grained out-of-distribution (OOD) detection, which uses feature disentanglement to improve detection performance.


<details>
  <summary>Details</summary>
Motivation: Existing OOD detection methods struggle with fine-grained tasks due to high visual similarity among subcategories and interference from background factors.

Method: DFDNet, comprising a spatial-frequency decoupling module and a reconstruction-guided decoupling module, to preserve discriminative content features and enhance category-specific semantic representations.

Result: Achieved competitive performance improvements on multiple datasets.

Conclusion: The proposed DFDNet effectively addresses fine-grained OOD detection challenges, demonstrating improved performance in detecting out-of-distribution samples in fine-grained tasks.
...
TL;DR: The paper introduces UltraVR, a benchmark for evaluating vision-language models on ultra-resolution images, focusing on evidence-grounded visual reasoning.


<details>
  <summary>Details</summary>
Motivation: Current evaluations of vision-language models do not provide sufficient insight into their ability to acquire and integrate visual evidence, particularly for ultra-resolution images.

Method: The authors introduce UltraVR, a diagnostic benchmark with four high-value scenarios, and evaluate frontier vision-language models using this benchmark.

Result: Current vision-language models are far from reliable on ultra-resolution reasoning, with errors concentrating in evidence grounding and local perception.

Conclusion: UltraVR serves as a diagnostic testbed for measuring the strengths and weaknesses of vision-language models on ultra-resolution images, enabling process-level diagnosis and localization of failures.
...
<details>
  <summary>Details</summary>
Motivation: Existing detectors rely on a single fixed backbone or manually designed hybrid architecture, which fails to adaptively exploit the strengths of CNNs and ViTs across diverse inputs.
...
Result: BMCR achieves 79.31%, 73.41%, and 71.86% mAP on DOTA-v1.0, DOTA-v1.5, and DIOR-R datasets, respectively, surpassing strong baselines by up to 2.5 points.
...
TL;DR: The paper proposes HDST-GNN, a Heterogeneous Dynamic Spatiotemporal Graph Neural Network for multi-object tracking from UAV imagery, addressing challenges such as varying altitude, small and densely packed objects, and frequent occlusion.
...
Method: The proposed HDST-GNN includes three novel contributions: Altitude-Adaptive Edge Construction, Heterogeneous Node Representation, and Occlusion-Gated Temporal Aggregation.
...
Conclusion: The proposed HDST-GNN effectively addresses the challenges of multi-object tracking from UAV imagery and outperforms existing methods.
...
<details>
  <summary>Details</summary>
Motivation: Current body condition measures in swine production are inaccurate or labor-intensive, and there is a need for a non-contact, automated method to monitor body condition.

Method: PigFormer uses a two-stage approach: a geometric front-end to process raw depth frames, and a Slice Attention Encoder to capture spatial relationships and predict body condition metrics.

Result: PigFormer achieves a mean absolute error (MAE) of 2.43 mm for backfat thickness and 3.87 mm overall MAE on a multi-site dataset of 319 sow and gilt instances.

Conclusion: PigFormer offers a practical path toward continuous, automated, non-contact body condition monitoring in commercial swine production, outperforming strong baselines.
...
TL;DR: KV-Control is a novel interface for controlling 3D human motion models, allowing for precise trajectory control while preserving text-conditioned motion quality.
...
TL;DR: The paper proposes Triple-Shot Compositions (TSC), a task that generates three shots (establishing, medium, and close-up) from a single image, and introduces ShotCrop, a model that learns TSC with limited expert annotations.


<details>
  <summary>Details</summary>
Motivation: Current aesthetic composition methods produce a single crop, ignoring the narrative value of multiple shots from one scene, which is crucial for creative workflows.

Method: The authors propose a three-stage training process for ShotCrop: Chain-of-Thought supervised fine-tuning, semi-supervised fine-tuning with pseudo labels, and optimization with Group Relative Policy Optimization.

Result: ShotCrop achieves an average improvement of 2.82 times over GPT-5 in shot localization accuracy.

Conclusion: The paper presents a new task, Triple-Shot Compositions, and a model, ShotCrop, that can generate aesthetically pleasing and narratively valuable shots from a single image, with improved shot localization accuracy.
...
TL;DR: The paper proposes CrackGeoFM, a multi-task framework for reliable crack assessment in infrastructure images, combining mask prediction, skeleton reconstruction, and uncertainty estimation.


<details>
  <summary>Details</summary>
Motivation: Existing segmentation models for crack assessment have limitations, including fragmenting cracks, missing fine branches, and lacking calibrated uncertainty, which hinders reliable crack assessment.

Method: CrackGeoFM integrates a frozen visual foundation backbone with crack-specific adaptation modules, including FCEM, CFAM, and SMTD, to enhance crack cues, adapt features, and jointly decode masks, skeletons, and uncertainty.

Result: CrackGeoFM achieves state-of-the-art segmentation, improved topology preservation, calibrated uncertainty, and effective few-shot adaptation across 20 crack datasets.

Conclusion: The proposed CrackGeoFM framework supports reliable, generalizable, and engineering-oriented crack analysis for infrastructure assessment.
...
TL;DR: Proposed a novel framework, CoFi-UCGen, for unsupervised conditional image generation that controls generation without manual labels by disentangling global semantics from fine-grained variations.


<details>
  <summary>Details</summary>
Motivation: Unsupervised conditional image generation (UCGen) is challenging due to unstructured semantic representations across granularities, and existing methods struggle with coarse- and fine-grained conditional generation without labels.
...
Result: CoFi-UCGen outperforms existing UCGen methods in image quality, semantic consistency, and control accuracy without using label priors or pre-trained feature extractors.

Conclusion: The proposed CoFi-UCGen framework effectively addresses the challenging UCGen task by explicitly decomposing semantics into coarse and fine-grained components, enabling successful conditional generation without manual labels.
...
TL;DR: Introduced V2V-Bench, a benchmark for video-to-video generation evaluating editing instructions, frame-level correspondence, and more.


<details>
  <summary>Details</summary>
Motivation: Existing metrics for text-to-video and image-to-video generation do not adequately capture the complexities of video-to-video generation.

Method: Proposed V2V-Bench, an 11-dimension benchmark with five categories: temporal alignment, structural fidelity, transformation quality, video quality, and semantic alignment.

Result: Evaluated three models and achieved a Spearman correlation of 0.905 with human judgments on six V2V-specific dimensions.

Conclusion: V2V-Bench effectively evaluates video-to-video generation models, highlighting complementary strengths of different models.
...
TL;DR: This paper introduces LongSpace-Bench, a benchmark for evaluating long-horizon spatial memory in videos, and proposes LongSpace, a memory framework that improves long-video spatial understanding by incorporating 3D structural cues and layer-aware memory.


<details>
  <summary>Details</summary>
Motivation: Long-horizon tasks like autonomous driving and robotic navigation require models to remember and retrieve previously observed spatial layouts, routes, viewpoint changes, and object states, beyond just recognizing the current view.

Method: The proposed LongSpace framework models long videos as sequential chunks, incorporates 3D structural cues into early decoder layers, and constructs layer-aware memory for question-guided retrieval.

Result: Experiments on multiple spatial reasoning benchmarks show that LongSpace improves long-video spatial understanding, demonstrating explicit spatial memory as a key capability for long-horizon video MLLMs.

Conclusion: The proposed LongSpace framework and LongSpace-Bench benchmark contribute to the development of long-horizon video MLLMs with improved spatial memory and understanding capabilities.
...
TL;DR: T-SAR-JEPA is a self-supervised framework for detecting temporal anomalies in SAR amplitude stacks, achieving 77.0% ROC-AUC on a dataset and outperforming baselines.


<details>
  <summary>Details</summary>
Motivation: The motivation is to develop an effective method for temporal anomaly detection in SAR amplitude stacks using a self-supervised approach.

Method: The method involves domain-adapting a ViT-Base/16 encoder using local masked reconstruction and gradient feature prediction, then using a temporal transformer with sinusoidal time encoding to forecast future latent states.

Result: T-SAR-JEPA achieves a ROC-AUC of 77.0% on the DFC 2026 dataset and shows spatial coherence of 99.9% (p < 0.001, permutation test).

Conclusion: The proposed framework, T-SAR-JEPA, demonstrates effective temporal anomaly detection in SAR amplitude stacks, outperforming baseline methods.
...
TL;DR: Proposed Parallel Jacobi Decoding (PJD) accelerates autoregressive image generation by 4.8x-6.4x while maintaining quality.


<details>
  <summary>Details</summary>
Motivation: Autoregressive models generate high-fidelity images but have slow inference due to sequential prediction. Existing Jacobi-style decoding methods have limited acceleration due to error propagation.
...
Result: PJD achieves 4.8x-6.4x acceleration across multiple autoregressive image generation models while maintaining competitive generation quality on diverse datasets.

Conclusion: PJD is an efficient and training-free decoding approach for autoregressive image generation, achieving significant acceleration without compromising quality.
...
TL;DR: A real-time object detection framework for surveillance in densely populated areas, focusing on detecting guns, knives, and blunt objects.


<details>
  <summary>Details</summary>
Motivation: Traditional surveillance methods are inefficient and prone to errors due to manual monitoring, necessitating an intelligent and automated system.

Method: A YOLOv8-based object detection model trained on a custom dataset of 7,959 images across three classes: gun, knife, and blunt object.

Result: The proposed framework achieves an effective balance between accuracy and efficiency, with significant improvement in recall and average precision for the blunt object class.

Conclusion: The proposed system is suitable for deployment in real-world surveillance environments, offering a solution to ensure public safety in densely populated urban areas.
...
TL;DR: The paper proposes ViCuR, a visually grounded privileged-teacher distillation framework that improves multimodal on-policy distillation by using visual cues instead of answer-side privilege.


<details>
  <summary>Details</summary>
Motivation: The authors are motivated to address the train-test mismatch in multimodal on-policy distillation, where the teacher's supervision may depend on signals unavailable to the student, leading to shortcut imitation rather than visually grounded reasoning.

Method: The authors propose ViCuR, which replaces answer-side privilege with visual cues (query-related evidence in the input) and introduces a lightweight cue recovery module to aggregate task-relevant visual evidence into an internal representation.

Result: ViCuR consistently improves over answer-based on-policy self-distillation by +1.19 and +1.24 on overall average performance across seven benchmarks with Qwen3-VL-2B and 8B students. It also extends naturally to stronger-teacher OPD, surpassing OPD baselines by +0.64 and +1.08, with consistent out-of-domain gains at the 8B scale.

Conclusion: The design of teacher privilege is as important as teacher strength in multimodal on-policy distillation, and ViCuR provides a effective solution to improve multimodal on-policy distillation.
...
TL;DR: TextWand is a framework for unified scene text editing, including removal, generation, and replacement, outperforming existing models in accuracy, consistency, and image quality.


<details>
  <summary>Details</summary>
Motivation: The lack of a comprehensive benchmark for general-purpose scene text editing and the need for precise control over text appearance and background integrity.

Method: TextWand uses Overlay-Reference Positional Encoding (ORPE) and Region-Adaptive Suppression (RAS) to achieve precise control and clean text erasure.

Result: TextWand outperforms existing models in text content accuracy, layout and style consistency, and overall image quality across scene text removal, generation, and replacement tasks.

Conclusion: TextWand provides a general-purpose framework for scene text editing, achieving state-of-the-art performance and setting a new standard for the field.
...
TL;DR: A new framework called VTI-CoT is proposed for video reasoning, which integrates textual and visual information to improve accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing Chain-of-Thought (CoT) based video reasoning methods overlook critical visual information during inference, and the authors aim to address this limitation.

Method: The authors propose a Visual-Textual Interleaved CoT (VTI-CoT) framework, which integrates textual reasoning steps with corresponding visual frames, and uses an automated annotation pipeline and OCR-based compression techniques.
...
Conclusion: The proposed VTI-CoT framework effectively enhances video reasoning accuracy and efficiency by incorporating visual information and improving the CoT supervision signals.
...
TL;DR: The paper presents a simplified approach to training diffusion-based vision-language-action models for action generation, showing that strong one-step action generation can be achieved without complex methods used in image synthesis.


<details>
  <summary>Details</summary>
Motivation: The authors argue that vision-language-action action generation has a different condition-target structure than image generation, and therefore does not require advanced one-step methods developed for image synthesis.

Method: The authors propose a simple approach that biases the training time distribution toward high-noise states, without using teacher models, distillation stages, or auxiliary objectives.

Result: The results show that one-step policies trained with high-noise biased schedules can match or exceed ten-step decoding performance on various benchmarks, including LIBERO, LIBERO-Plus, and LIBERO-Pro, and achieve 95.6% on LIBERO-Long with a 1.4B VLM model.

Conclusion: The paper concludes that strong one-step vision-language-action action generation can emerge from standard diffusion training, without requiring complex methods developed for image generation.
...
Method: The authors design a matrix of five LVR variants and use two inference-time diagnostics, PRISM, which includes a linear probe and a corruption test, to analyze the role of supervised latent tokens.

Result: The results show that cosine alignment is negatively correlated with accuracy (r=-0.94) and that the supervised latents are largely bypassed, with corrupting them shifting accuracy by at most four points.

Conclusion: The conclusion is that the auxiliary objective of LVR reshapes the language model via shared parameters rather than via the latent variable it optimizes, and that better alignment does not necessarily yield better answers.
...
TL;DR: The paper proposes DRIFT, a framework for adapting vision-language models to continuous decoding tasks, achieving better performance across various tasks and architectures.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitation of autoregressive decoding of discrete tokens in vision-language models for tasks requiring precise continuous outputs.

Method: The method proposed is DRIFT, which combines a base predictor with a generative refinement module based on flow matching to iteratively improve the prediction.

Result: DRIFT consistently outperforms regression- and generative-based solutions across multiple tasks and architectures.

Conclusion: The proposed DRIFT framework effectively adapts pretrained vision-language models to continuous decoding tasks, demonstrating superior performance.
...
TL;DR: The paper proposes a physics-guided deep unfolding network (PGU-Net) for blind cross-sensor spectral super-resolution, which jointly estimates hyperspectral images and a learnable spectral transformation function.


<details>
  <summary>Details</summary>
Motivation: Existing spectral super-resolution methods are limited to single-sensor settings and assume a fixed and known spectral response function, which is unknown and varies in practical cross-sensor scenarios.

Method: The proposed PGU-Net unrolls an alternating optimization procedure into an end-to-end trainable architecture, combining learnable proximal networks with differentiable closed-form solvers.

Result: Experiments on benchmark datasets demonstrate accurate recovery of the spectral transformation function and improved reconstruction performance over state-of-the-art methods, with effectiveness and robustness verified on a real UAV cross-sensor dataset.

Conclusion: The proposed PGU-Net addresses blind cross-sensor spectral super-resolution by jointly estimating hyperspectral images and a learnable spectral transformation function, showing improved performance and robustness in practical scenarios.
...
Method: The ExpSpeech-Net model uses SqueezeNet and RNN as its backbone, with advanced feature extraction and a smart feature-selection strategy using SASMA, to analyze facial expressions and speech patterns.

Result: The framework achieves 94.5% accuracy, precision of 99.3%, and F-measure of 96.8%, outperforming conventional methods.

Conclusion: The proposed ExpSpeech-Net model enables practical, real-time deepfake detection suitable for everyday applications by integrating multiple modalities with intelligent preprocessing and feature selection.
...
TL;DR: A new framework, Future-L1, is proposed for video event prediction, which enables a multimodal large language model (MLLM) to reason in both text and latent visual spaces, leading to state-of-the-art results on two benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing video MLLMs lose fine-grained motion, geometry, and interaction cues when verbalizing visual evidence into text, leading to hallucinations. The authors aim to preserve intermediate visual semantics in latent space.

Method: The Future-L1 framework allows an MLLM to alternate between language tokens and continuous latent visual spans during autoregressive decoding. It is trained on a dataset called Future-L1-50K and optimized with a latent-aware RL objective.

Result: Future-L1 achieves new state-of-the-art results on two benchmarks: FutureBench (85.4, a 24.4 point improvement over Qwen3-VL-8B) and TwiFF-Bench (average score of 3.04, a 0.6 point improvement).

Conclusion: Preserving intermediate visual semantics in latent space rather than translating every reasoning step into text benefits future-oriented video reasoning.
...
TL;DR: LiAuto-GeoX, a efficient grounded driving transformer, enables real-time, onboard 3D scene understanding for autonomous driving.


<details>
  <summary>Details</summary>
Motivation: Existing large-scale visual geometry models are not efficient or accurate enough for real-time autonomous driving, lacking long-range geometric fidelity, surround-view consistency, and efficiency.

Method: LiAuto-GeoX uses a large-scale surround-view dataset, sparse LiDAR priors, and a novel geometry-preserving distillation framework to create a compact, 155M-parameter onboard model.

Result: LiAuto-GeoX runs at 220 FPS on KITTI, achieving high-fidelity dense reconstruction and transferring to downstream autonomy tasks with 90.6 PDMS, 24.63 mIoU, and 47.67 IoU.

Conclusion: LiAuto-GeoX demonstrates efficient dense 3D reconstruction can serve as a scalable, foundational geometric representation for next-generation autonomous driving.
...
TL;DR: The paper proposes a novel framework called Relative Edit-induced Difference Aesthetic learning (RED-Aes) for Image Aesthetic Assessment (IAA) that learns visual factors driving aesthetic changes through controllable image editing.


<details>
  <summary>Details</summary>
Motivation: Traditional IAA methods rely on regressing absolute Mean Opinion Scores (MOS), overlooking the dynamic nature of human aesthetic perception which relies on subconscious comparison against implicit visual references.

Method: The proposed RED-Aes framework leverages controllable image editing models to simulate human aesthetic reasoning, learning visual factors that drive aesthetic changes, and uses a three-stage training strategy with relative ranking consistency reward.
...
Conclusion: The proposed RED-Aes framework offers a novel approach to IAA by focusing on relative aesthetic differences, leading to improved generalization across diverse scenarios.
...
TL;DR: The paper improves license plate detection and recognition with Cross-Spatial Hybrid Attention and Class-Balanced Synthetic Augmentation, boosting minority license plate recognition from 78.2% to 91.5% while maintaining 152 FPS.


<details>
  <summary>Details</summary>
Motivation: The existing YOLOV5-PDLPR model is affected by spatial character mismatches and data imbalance, which this paper aims to address.

Method: The paper introduces Cross-Spatial Hybrid Attention (CSHA) and Class-Balanced Synthetic Augmentation (CBSA) to improve the model performance.

Result: The recognition rate of minority provincial license plates improves from 78.2% to 91.5% with real-time processing performance of 152 FPS.

Conclusion: The proposed approach provides an effective solution for high-speed license plate recognition systems.
...
TL;DR: The paper proposes an emotion-aware text-to-image pipeline that generates children's hand drawing style images from short Korean diary entries, leveraging Qwen3-8B and Stable Diffusion 3.5 Medium.


<details>
  <summary>Details</summary>
Motivation: T2I models struggle to capture sentiment from text, particularly diaries, due to their focus on visual object-related patterns rather than contextual emotional understanding.

Method: The proposed pipeline uses Qwen3-8B for sentiment recognition and Stable Diffusion 3.5 Medium fine-tuned with LoRA on children's drawing images with emotion-based trigger words.

Result: The paper presents experiments on the effect of emotion trigger words on generated images and discusses limitations of CLIP Score as an evaluation metric.

Conclusion: The paper proposes a novel pipeline for emotion-aware text-to-image generation, addressing the limitations of existing T2I models in capturing sentiment from text.
...
TL;DR: This paper introduces a dataset and metrics to study the relationship between artistic style, gender representation, and AI-generated images, finding that gender representation influences visual features across styles and that generative models amplify gender artifacts.


<details>
  <summary>Details</summary>
Motivation: The authors aim to investigate the interplay between gender representation and artistic style in both historical and generated images, as AI research has traditionally treated style as a superficial visual property rather than considering its socio-historical context.

Method: The authors propose a dataset called StyleGender, which contains 74k images across 19 artistic styles, along with two Set Gender Artifact (SGA) metrics (PixelSGA and MaskSGA) to capture gender signals at the pixel level and in compositional structure.

Result: The authors find that (1) gender representation shapes visual features across artistic styles, (2) style keywords carry these patterns into T2I generation, and (3) generative models tend to amplify gender artifacts beyond what is observed in historical sources.

Conclusion: The study highlights the importance of considering the socio-historical context of artistic style and its relationship to gender representation in AI research, and introduces a dataset and metrics to facilitate further investigation in this area.
...
TL;DR: A novel framework called GeoVR is proposed to enhance 3D awareness in Multimodal Large Language Models (MLLMs) using 2D video sequences, achieving state-of-the-art performance on spatial reasoning benchmarks.


<details>
  <summary>Details</summary>
Motivation: The scarcity of large-scale 3D data and the lack of intrinsic 3D awareness in MLLMs, which results in representations that fail to maintain geometric and spatial consistency across video frames.

Method: GeoVR learns geometric representations using purely 2D video sequences and distills geometry knowledge from pre-trained 3D foundation models through a multi-objective learning strategy driven by four complementary geometric targets.

Result: GeoVR achieves state-of-the-art performance on spatial reasoning benchmarks, establishing a new paradigm for endowing foundation models with spatial intelligence.

Conclusion: GeoVR effectively restructures the semantic latent space within MLLMs to unlock spatial intelligence, demonstrating strong 3D awareness and achieving state-of-the-art performance on spatial reasoning benchmarks.
...
TL;DR: The authors propose a method for condensing datasets for diffusion model training, which preserves the geometric structure and distributional fidelity of the original data.


<details>
  <summary>Details</summary>
Motivation: Existing dataset condensation methods are not suitable for diffusion model training, as they either generate low-fidelity synthetic data or fail to preserve the distributional geometry.

Method: The authors reformulate real subset selection as a geometry-aware distribution alignment problem, using one-sided partial optimal transport and complementing it with feature-statistics and semantic consistency regularization.

Result: The proposed method achieves superior fidelity and distributional coverage in diffusion model training across various experiments.

Conclusion: The authors' method provides a effective solution for dataset condensation in diffusion model training, preserving the necessary geometric structure and distributional fidelity.
...
TL;DR: A novel framework for creating lifelike digital humans with social intelligence is proposed, integrating cognitive reasoning and multimodal generation.


<details>
  <summary>Details</summary>
Motivation: Current approaches to creating digital humans treat cognitive reasoning and multimodal generation as separate tasks, lacking a unified framework for genuine social intelligence.

Method: A closed-loop dual-agent framework is proposed, consisting of perception, social reasoning, and expression modules, which interact in a continuous cycle.

Result: The proposed method demonstrates competitive or superior performance on dialogue quality and video generation metrics, and even surpasses the full-information Script mode on key dialogue quality dimensions.

Conclusion: The proposed framework can create lifelike digital humans with genuine social intelligence by unifying cognitive reasoning and multimodal generation, and explicit mental state inference under uncertainty can elicit more thoughtful dialogue.
...
TL;DR: A new framework, SAGE, enables the creation of high-fidelity, animatable avatars from minimal input data, reducing data requirements by several orders of magnitude.


<details>
  <summary>Details</summary>
Motivation: The challenge of modeling dynamic facial expressions using 3D Gaussian representations due to their unstructured nature and the limitations of conventional Gaussian avatar pipelines.

Method: The Self-Adaptive Gaussian Expression (SAGE) framework, which jointly optimizes 2D Gaussian surfels and a Signed Distance Field (SDF) with a self-supervised expression learning phase.

Result: The approach achieves reconstruction and animation quality comparable to state-of-the-art methods with significantly reduced data requirements.

Conclusion: The potential of self-supervised Gaussian deformation learning as a step toward accessible, data-efficient avatar creation.
...
TL;DR: CamFlow+ is a new framework for estimating 2D camera motion that overcomes limitations of existing methods by representing motion in dense-flow space, improving estimation accuracy and video stabilization.


<details>
  <summary>Details</summary>
Motivation: Existing methods for estimating 2D camera motion struggle with camera translation, depth variation, and local parallax, motivating the development of a more flexible and accurate approach.

Method: CamFlow+ combines homography-derived physical bases, stochastic bases, and depth-translational bases to represent 2D camera motion in dense-flow space, with a depth-aware smoothness term to regularize translation-induced parallax.
...
Conclusion: CamFlow+ offers a hybrid-basis framework for 2D camera motion estimation that relaxes the single-plane constraint and preserves camera-motion regularity, leading to improved performance in various applications.
...
<details>
  <summary>Details</summary>
Motivation: Existing knowledge distillation-based approaches for open-vocabulary object detection overlook structured relationships between objects in an image, restricting their effectiveness in detecting novel categories.

Method: The proposed framework utilizes scene graphs to model interactions and spatial arrangements between candidate regions and their contextual objects. It also incorporates a Relation Attention Module and a scene-based textual alignment branch to integrate visual relations with semantic information.

Result: The proposed model achieves superior performance compared to other open-vocabulary object detection methods, improving the Average Precision (AP) for novel categories on COCO and LVIS datasets.

Conclusion: The Scene-guided Relational Modeling detection framework effectively enhances open-vocabulary object detection by capturing structured relationships between objects in an image, leading to improved detection performance for novel categories.
...
TL;DR: A new framework called MemoryCard is proposed to improve long-video question answering by organizing videos into self-contained Memory Cards, leading to a 21.8% relative improvement in accuracy.


<details>
  <summary>Details</summary>
Motivation: Long-video question answering is challenging for Vision-Language Models (VLMs) due to sparse, transient, and temporally dispersed evidence across lengthy video contexts.

Method: MemoryCard, a video-memory-based augmentation framework that segments videos into semantically coherent units, generates event-level video gist, and selects representative visual moments.
...
Conclusion: MemoryCard offers a more effective approach to long-video question answering by capturing coherent event-level semantics, outperforming existing frame-centric approaches.
...
TL;DR: A new benchmark, FEPBench, is introduced to evaluate Text-to-Image models for scientific illustration generation, focusing on fine-grained elements and aspects like instruction faithfulness and semantic precision.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for scientific illustration generation lack fine-grained evaluation of Text-to-Image models, particularly in terms of scientific reasoning ability and output conciseness.

Method: FEPBench uses multimodal large language models and human experts to provide detailed annotations and evaluate models across three dimensions: instruction faithfulness, reasoning enrichment, and semantic precision.

Result: State-of-the-art models still struggle with text-rendering, limited reasoning enrichment, and balancing generation richness with precision, providing guidance for model improvement.

Conclusion: The FEPBench benchmark provides a comprehensive evaluation framework for Text-to-Image models in scientific illustration generation, highlighting areas for improvement and guiding model deployment.
...
<details>
  <summary>Details</summary>
Motivation: Current open-vocabulary 3D segmentation methods have limitations in balancing granularity, accuracy, and speed, and struggle with scene-wide part segmentation, motivating the development of a more efficient and effective approach.

Method: T-FunS3D uses a 3D point cloud and posed RGB-D images of an indoor scene to construct an open-vocabulary scene graph, then identifies relevant instances and locates their functional components using a vision-language model.

Result: Experiments on the SceneFun3D dataset show that T-FunS3D achieves comparable performance to state-of-the-art methods in open-vocabulary 3D functionality segmentation, with faster runtime and reduced memory usage.

Conclusion: The proposed T-FunS3D method provides a more efficient and effective solution for task-driven hierarchical open-vocabulary 3D functionality segmentation, enabling actionable perception for robotic applications.
...
<details>
  <summary>Details</summary>
Motivation: The authors aim to address the bottleneck in real-time text-to-image pipelines, specifically when using a multimodal large language model (MLLM) as the text encoder.

Method: The authors propose a pipeline that combines a distilled edit U-Net with a large MLLM text encoder, and utilize three engineering mechanisms: asymmetric CUDA pipelining, ControlNet-LLLite reformulation, and periodic conditioning-refresh schedule.

Result: The pipeline achieves 27.4 fps on a single RTX 3090 Ti at 512x512 resolution, and up to 74.1 fps on RTX 5090, with end-to-end latency of approximately 0.5-1.0 seconds.

Conclusion: The authors demonstrate a viable solution for video-rate streaming throughput in text-to-image synthesis, with a focus on engineering optimizations for consumer hardware.
...
Method: The system uses a feature-engineered late-fusion pipeline with gradient-boosted regression models and hierarchical post-processing. For memes, it combines visual, textual, demographic, biometric, and LLM-derived semantic indicators. For videos, it investigates feature selection, frame-based visual representations, OCR-based textual features, acoustic descriptors, and sensor-derived metadata.

Result: The results show that focused LLM-derived semantic cues improve meme sexism identification, while video performance is sensitive to feature dimensionality and cross-modal noise. The unfiltered representation generalizes better to unseen data.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of traditional oral 3D modelling methods, such as impression taking and intraoral scanning, which suffer from patient discomfort, material deformation errors, and high equipment costs.

Method: The proposed method uses a software-based approach with MobileNetV2 as the image encoder and Multi-head Attention for multi-view feature fusion to reconstruct a 3D oral model from 10 2D intraoral images.

Result: The proposed model achieves an accuracy of 77.49% on the Dental3DS dataset, but predicted vertices tend to concentrate in high-density regions of the ground truth, resulting in uneven point distribution across the reconstructed model.

Conclusion: The proposed approach reduces cost, eliminates the need for physical scanning equipment, minimises patient discomfort, and enables automated 3D reconstruction, but requires further improvement to achieve even point distribution.
...
TL;DR: A new Transformer-based model, ATT-CR, is proposed for cloud removal in remote sensing images, reducing computational costs and mitigating interference from cloudy pixels.
...
Method: The proposed ATT-CR model consists of Triangular Attention (TAN) and Feature Selected Gating Module (FSGM) to reduce computational costs and distinguish between cloudy and clean features.

Result: ATT-CR delivers superior performance compared to existing methods on cloud removal benchmarks.

Conclusion: The proposed ATT-CR model effectively addresses the challenges of high computational complexity and interference from cloudy pixels in cloud removal tasks.
...
TL;DR: The paper proposes a new method for text-to-3D indoor scene generation using Large Vision-Language Models, addressing the challenge of error propagation in sequential decision-making approaches.


<details>
  <summary>Details</summary>
Motivation: The main challenge in text-to-3D indoor scene generation with LVLMs is that prevailing methods employ chain-of-thought sequential decision mechanisms that cannot revise earlier decisions, causing error propagation.

Method: The proposed method models the task as a tree search problem with global and local trees, using a hierarchical scene representation and a PRM-guided MCTS method to effectively search the tree.

Result: The experiments show that the proposed method generates more realistic 3D scenes than state-of-the-art approaches.

Conclusion: The paper presents a novel approach for text-to-3D indoor scene generation, addressing the limitations of existing sequential decision-making methods and demonstrating improved results on a new large-scale dataset, 3DTindo-bench.
...
TL;DR: The paper proposes ReSAGE-PAR, a pipeline for generating high-quality pedestrian images for Pedestrian Attribute Recognition (PAR) using diffusion models, addressing domain gap and attribute verification challenges.


<details>
  <summary>Details</summary>
Motivation: To address limited diversity and data scarcity in PAR, the authors explore image synthesis using diffusion models guided by attribute-based prompts.

Method: The ReSAGE-PAR pipeline consists of: (1) adapting pre-trained diffusion models to native PAR resolutions using a LoRA-based Image-to-Image approach, (2) extracting vision-language alignment scores between generated images and conditioning prompts, and (3) formulating a Bayesian classifier to convert scores into reliable binary pseudo-labels.

Result: ReSAGE-PAR consistently yields significant improvements in PAR, achieving gains of up to 8.7% on standard backbones and pushing state-of-the-art frameworks to new performance levels.

Conclusion: The proposed ReSAGE-PAR pipeline is an effective and architecture-agnostic solution for scalable PAR enhancement, preserving spatial priors and verifying attributes.
...
TL;DR: The paper presents a self-supervised 3D reconstruction framework for truncated cone-beam computed tomography (CBCT) using neural scene representations, addressing limitations of existing deep learning methods.
...
Method: The proposed framework uses a neural scene representation to directly map spatial coordinates to radiodensity under projection supervision, eliminating truncation-induced ring artifacts and enabling robust continuous 3D data extrapolation. It also incorporates a physics-based iterative refinement module to preserve high-frequency textures.

Result: The method successfully unifies the exceptional artifact suppression and extrapolation capabilities of neural networks with the high-fidelity detail preservation of iterative algorithms, as demonstrated by extensive experiments on simulated and real-world datasets.

Conclusion: The proposed framework offers a promising solution for truncated CBCT reconstruction, addressing the limitations of existing methods and providing improved artifact suppression and detail preservation.
...
<details>
  <summary>Details</summary>
Motivation: Existing unified video generation and editing models are challenged by their large size and high computational complexity, particularly when handling interleaved multimodal inputs.
...
Conclusion: LoomVideo presents a highly efficient and practical solution for video generation and editing, paving the way for highly practical and efficient video foundation models.
...
TL;DR: A multimodal framework is proposed to synthesize 3D gait data for pathological gait classification tasks from textual descriptions.
...
Result: The proposed synthetic sequences improve downstream classification, achieving 92.77% accuracy with a GRU classifier.

Conclusion: The proposed framework effectively generates synthetic gait data that improves pathological gait classification tasks.
...
TL;DR: ReCache is a method for optimizing the caching schedule in diffusion models to maximize generation quality under a given computational budget.


<details>
  <summary>Details</summary>
Motivation: The iterative denoising process in diffusion models makes inference expensive, and existing caching schedules have limitations in terms of flexibility and controllability.

Method: ReCache uses policy gradients to learn the recomputation schedule that maximizes generation quality given a target budget k, and is compatible with any caching mechanism.

Result: ReCache consistently outperforms scheduling baselines, reducing LPIPS by 31% and 65% and boosting the VBench score by 7% in two different scenarios.

Conclusion: ReCache provides a controllable and efficient way to optimize the caching schedule in diffusion models, allowing for a trade-off between computational cost and generation quality.
...
TL;DR: Introduced VZCrash, a large dataset of real-world vehicle collision data with IMU telemetry, and conducted experiments on crash detection models.


<details>
  <summary>Details</summary>
Motivation: The motivation is to provide a large-scale dataset of real-world vehicle collision data to facilitate the development of high-quality crash detection models.

Method: The method involves introducing a new dataset, VZCrash, and conducting an extensive experimental study, including benchmarking different approaches and demonstrating the importance of scaling data for training crash detection models.

Result: The result shows that scaling data is crucial for training high-quality crash detection models, especially for deployment in real-world environments.

Conclusion: The study concludes that large-scale datasets like VZCrash are essential for developing effective crash detection models.
...
TL;DR: This paper studies knowledge distillation for autoregressive image generation models and proposes a new framework, VarKD, which improves distillation by selectively applying teacher supervision and reducing token-level ambiguity.
...
Method: The authors propose VarKD, a distillation framework for visual autoregressive models that distills on student samples while selectively applying teacher supervision and reducing token-level ambiguity.
...
Conclusion: The proposed VarKD framework effectively addresses the challenges of knowledge distillation for autoregressive image generation models, leading to improved model compression and performance.
...
TL;DR: The paper proposes HyperVis, a method to improve compositional reasoning in Vision-Language Models (VLMs) by leveraging visual relation tensors and hyperbolic geometry, achieving better performance on GQA and SugarCrepe benchmarks.


<details>
  <summary>Details</summary>
Motivation: VLMs struggle with compositional reasoning that requires understanding inter-object relationships, and existing methods that inject explicit scene graph triplets can degrade performance due to modality collision.

Method: The proposed HyperVis method computes a dense visual relation tensor via spatially-biased cross-attention, projects it onto a Lorentz hyperboloid, and enforces hierarchy through spatial physics, including IoA-driven entailment cones and exterior-angle repulsion.

Result: HyperVis improves performance on GQA (61.03%) and SugarCrepe (79.94%) benchmarks, with the learned curvature stabilising at κ=4.0, and ablation studies confirming the importance of hyperbolic geometry.

Conclusion: The paper concludes that HyperVis is an effective method for improving compositional reasoning in VLMs, and that hyperbolic geometry is essential for capturing the complex relationships between objects in visual scenes.
...
TL;DR: The paper introduces a framework called Medical Segmentation Dataset Knowledge Card (MS-DKC) to make dataset requirements explicit for medical image segmentation. MS-DKC evaluates dataset factors and maps them to model design and risk-aligned criteria.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to shift the focus from finding stronger architectures for medical image segmentation to understanding what a dataset requires from a model.

Method: The authors introduce MS-DKC, a framework that records dataset evidence through descriptors and maps them to failure modes, design priors, and risk-aligned criteria.

Result: The results show that MS-DKC can effectively guide model design for different datasets, leading to improved performance on various metrics (e.g., Dice, IoU, sensitivity, specificity, AUC) for the evaluated datasets (DRIVE, ISIC2018, ACDC).

Conclusion: The paper concludes that dataset-conditioned design is necessary for medical image segmentation, as different datasets require different priors, operating points, and evidence to evaluate a model's appropriateness.
...
TL;DR: The paper proposes a new method called Structured Defect Grounding (SDG) for diagnosing and evaluating text-to-image models, which provides instance-level feedback on defects in generated images.


<details>
  <summary>Details</summary>
Motivation: Current text-to-image models exhibit localized, subtle, and structurally complex failures, and diagnosing these failures requires instance-level feedback. Recent dense-feedback methods have limitations in localizing variable-cardinality defects and binding semantic reasons to individual failures.

Method: The paper proposes SDG, which casts T2I diagnosis as structured set prediction by modeling each defect as a (location, type, reason, importance) tuple. It also introduces SDG-30K, a 30K-image dataset with box-grounded annotations, and SDG-Eval, a dedicated evaluation protocol.

Result: The SDG detector outperforms leading proprietary VLMs on structured defect grounding, and SDG-guided rewards consistently improve T2I alignment and support localized image refinement.

Conclusion: The paper establishes SDG as a unified, instance-level interface for diagnosing, evaluating, and enhancing modern generative models.
...
TL;DR: The paper proposes a novel conditioning framework for diffusion models to enable contrastive decomposition of image factors without compromising generation quality.


<details>
  <summary>Details</summary>
Motivation: Existing contrastive methods based on generative models (e.g., VAEs or GANs) suffer from limited reconstruction and image quality, which hampers effective latent factor separation.

Method: The authors propose a prompt-free, image-conditioned diffusion model and learn to decompose the conditioning into a common and a salient factor using weak supervision.

Result: The proposed framework enables targeted operations by swapping or interpolating only the salient factor, and the additive contrastive factorization is proved to be identifiable under mild conditions.

Conclusion: The paper presents a novel approach to contrastive analysis using diffusion models, which overcomes the limitations of existing generative models and enables high-fidelity image generation and editing.
...
TL;DR: Proposed an efficient event-to-frame reconstruction framework with causal temporal modeling and computation-aware design, achieving competitive performance with a balance between accuracy and complexity.


<details>
  <summary>Details</summary>
Motivation: Existing event-to-frame reconstruction methods face a trade-off between reconstruction quality and computational efficiency.

Method: Proposed a recurrent encoder-decoder framework with selective context fusion and a lightweight hybrid attention mechanism.

Result: Achieved competitive reconstruction performance on standard benchmarks while maintaining a balance between accuracy and model complexity.

Conclusion: The proposed approach efficiently bridges asynchronous event streams with frame-based vision pipelines.
...
TL;DR: A new method for adaptive video tokenization that efficiently allocates tokens based on visual complexity, achieving significant speedup and competitive reconstruction fidelity.


<details>
  <summary>Details</summary>
Motivation: Current video tokenization methods have high computational overhead due to iterative searches or full-rate decoder passes, which can be avoided by exploiting temporal redundancy in the latent space of a video tokenizer.

Method: A parameter-free adaptive token allocation mechanism using a fixed threshold on per-position temporal-L1 differences to identify and drop redundant latent positions, and a Latent Inpainting Transformer (LIT) to reconstruct dropped positions.

Result: The proposed framework yields meaningful, content-driven token allocation while maintaining competitive reconstruction fidelity, with a $31	imes$ inference-time speedup over the continuous adaptive baseline and an $approx2	imes$ speedup over the discrete information-theoretic baseline.

Conclusion: The proposed method demonstrates that computational overheads are not necessary for adaptive video tokenization, and achieves efficient and effective token allocation and reconstruction.
...
TL;DR: The paper proposes a diffusion-based self-supervised learning strategy for underwater image enhancement, leveraging label quality distribution to improve model performance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the bottleneck in learning-based underwater image enhancement methods caused by unstable label quality in paired datasets.

Method: The method involves evaluating label quality using semantic perception embeddings from a pre-trained diffusion model, quantizing quality scores into noise-level indices, and guiding a multi-step denoising process for level-wise supervision. A Fourier-based refinement network is also incorporated.

Result: The method consistently outperforms state-of-the-art approaches in restoration quality, as demonstrated by extensive evaluations.

Conclusion: The paper concludes that the proposed diffusion-based self-supervised learning strategy effectively mitigates the issue of unstable label quality, leading to improved performance in underwater image enhancement.
...
TL;DR: A test-time defense framework called Directional Bias-guided Defense (DBD) is proposed to protect Vision-Language Models (VLMs) like CLIP from adversarial attacks by exploiting the consistent shift of adversarial images in the feature space along a dominant direction.


<details>
  <summary>Details</summary>
Motivation: VLMs are vulnerable to adversarial perturbations, which poses serious risks in real-world applications, and the need for efficient defense mechanisms that do not require costly retraining.

Method: The proposed DBD framework estimates the 'Defense Direction' in CLIP's feature space, which adversarial images shift along, and uses a DB-score-based two-stream reconstruction strategy to recover robust representations.

Result: DBD achieves state-of-the-art adversarial robustness on 15 datasets while preserving clean accuracy, and surprisingly shows that adversarial accuracy can even surpass clean accuracy.

Conclusion: The study reveals that adversarial perturbations encode directional priors about the true decision boundary, and DBD offers an effective test-time defense for VLMs against adversarial attacks.
...
TL;DR: A new metric, SC-MFJ, is proposed to evaluate the suitability of segmented surfaces for haptic rendering in surgical simulation, revealing significant differences in haptic quality between different segmentation approaches.


<details>
  <summary>Details</summary>
Motivation: Current segmentation metrics do not assess the suitability of segmented surfaces for haptic rendering in surgical simulation, highlighting the need for a new metric.

Method: The SC-MFJ metric samples a segmented organ surface with virtual stylus walks and measures the jerkiness of the resulting contact forces.

Result: SC-MFJ reveals significant differences in haptic quality between different segmentation approaches, including a 147x gap between raw binary and Gaussian post-processing, and more variable haptic quality with learned SDF regression.

Conclusion: The proposed SC-MFJ metric can flag problems that geometric metrics miss, and a simple post-processing step may be sufficient for haptic simulation applications.
...
<details>
  <summary>Details</summary>
Motivation: Existing multimodal benchmarks for disaster response have limitations, such as focusing on perception, covering limited disaster types, and lacking support for multi-stage reasoning. The authors aim to address these limitations.
...
Conclusion: The authors introduce a new benchmark and a lightweight model for multimodal reasoning in disaster response, which can enable efficient and accurate reasoning on the edge.
...
TL;DR: The paper introduces Symb-xMIL, a post-hoc explanation framework for multiple instance learning (MIL) models, which provides insights into how the model combines evidence from different tissue regions to produce a prediction.


<details>
  <summary>Details</summary>
Motivation: Existing methods for explaining MIL models rely on heatmaps that highlight influential regions but do not explain how evidence is combined, limiting interpretability, especially when decisions depend on interactions between tissue features.

Method: The authors propose Symb-xMIL, a framework that quantifies how a MIL model's behavior aligns with human-readable decision rules, expressed as logical relationships between input features.

Result: The evaluation of Symb-xMIL on synthetic and real-world histopathology datasets shows that it reliably recovers ground-truth logical rules, uncovers heterogeneous decision patterns, exposes hidden model errors, and refines patient survival stratification.
...
TL;DR: Proposed SAM-Flow, a training-free image editing framework for localized editing without additional training, improving background preservation.


<details>
  <summary>Details</summary>
Motivation: Existing image editing methods cause global changes and background leakage; need for localized editing.

Method: SAM-Flow uses scout images, token-grounded attention maps, and differential velocity updates within editable regions.

Result: SAM-Flow achieves accurate semantic editing and improves background preservation, integrating with mainstream flow-matching backbones.

Conclusion: SAM-Flow provides a simple, general localized editing paradigm for training-free image editing, with extensive experiments validating its effectiveness.
...
TL;DR: A novel cross-attention mechanism, Volumetric Multimodal cross-Attention (VMA), is proposed to improve multimodal transformer models by capturing joint multimodal dependencies beyond pairwise similarity.
...
Conclusion: The proposed VMA mechanism and GRAMformer architecture can effectively capture joint multimodal dependencies and improve multimodal learning tasks.
...
TL;DR: The paper proposes a new approach to blind image restoration by modeling degradations on a Riemannian manifold and using geodesic transport to recover clean images.


<details>
  <summary>Details</summary>
Motivation: Current deterministic flow-based methods for blind image restoration assume linear degradation geometry, which is not always accurate. The authors aim to improve this by modeling degradations in a more flexible and realistic way.

Method: The authors use a geodesic flow matching objective to learn intrinsic transport dynamics on a joint image-manifold space, where degradations are represented as points on a low-dimensional Riemannian manifold.

Result: The proposed framework provides a principled treatment of mixed degradations, generalizes linear flow matching, and offers a clean theoretical interpretation for generalization beyond observed degradations.

Conclusion: The paper presents a novel approach to blind image restoration that explicitly models degradations on a Riemannian manifold, leading to a more flexible and realistic restoration process.
...
TL;DR: The authors propose a robotic system for manipulating textiles, addressing challenges in visual perception due to deformation and self-occlusions. They develop a synthetic data pipeline and a perception framework, enabling a bimanual algorithm to iron folded garments.


<details>
  <summary>Details</summary>
Motivation: The manipulation of textiles by robots is difficult due to the continuous deformation and self-occlusions of fabrics, which hinder robust visual perception.

Method: The authors create a Blender-based synthetic pipeline to generate auto-annotated keypoints and combine it with manually labeled renders and real-world data to train a wrinkle detector. They integrate a CNN for keypoint detection and a YOLOv8-OpenCV pipeline to extract grasping points from wrinkles.

Result: The keypoint model achieves a Mean Position Error (MPE) of 1.7615 pixels. The perception system successfully transfers to physical fabrics without fine-tuning, outperforming baselines in high-occlusion states or with severe folds.

Conclusion: The proposed system enables a bimanual algorithm to stretch and iron folded garments via wrinkles, demonstrating improved performance over existing methods.
...
TL;DR: The paper proposes a solution for One-to-Many Temporal Grounding (OMTG), a setting where multiple disjoint video segments need to be localized for a single textual query. The authors introduce a new benchmark, dataset, and reward functions, achieving a state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Real-world scenarios often require localizing multiple disjoint segments for a single query, which is not well-addressed by current state-of-the-art models optimized for one-to-one settings.

Method: The authors establish a comprehensive OMTG benchmark, curate a high-quality OMTG dataset, and develop novel temporal and caption reward functions. The caption reward leverages Chain-of-Thought reasoning over dense video captions.
...
Conclusion: The proposed solution effectively addresses the One-to-Many Temporal Grounding problem, providing a systematic approach and achieving state-of-the-art performance on the introduced benchmark.
...
<details>
  <summary>Details</summary>
Motivation: Existing video generation models suffer from high inference latency and computational costs due to the quadratic complexity of 3D attention and the requirement for a complete, dense denoising process for every frame.

Method: RhymeFlow identifies keyframes that undergo dense denoising, while non-keyframes skip denoising steps. A latent trajectory projection module ensures temporal coherence.
...
Conclusion: RhymeFlow offers a training-free framework to decouple denoising trajectories of different frames, reducing computational cost while maintaining visual quality.
...
TL;DR: The paper introduces StoryMindv2, a framework for generating large-scale deep video understanding (DVU) datasets, and presents StoryVideoQA, the largest DVU dataset to date. It also proposes PlotTree, a novel video understanding agent for storyline reasoning.


<details>
  <summary>Details</summary>
Motivation: Existing VideoQA approaches struggle with deep video understanding (DVU) due to the complexity of long-range video content, multi-faceted question types, and instance-level story elements. Manually constructed DVU datasets are limited in scale and diversity.
...
Result: The authors construct StoryVideoQA, a large-scale DVU dataset featuring over 363K QAs on 393.2 hours of diverse story videos. They also evaluate 20 state-of-the-art VideoQA methods on this benchmark, revealing their limitations in maintaining long-range character associations and constructing coherent storyline understanding.

Conclusion: The paper highlights the challenges in deep video understanding and presents a solution to address these challenges through the development of StoryMindv2 and PlotTree, a novel video understanding agent.
...
TL;DR: The study uses UAV multispectral imagery and deep learning models to segment bacterial leaf blight severity in rice, evaluating several architectures and input configurations.


<details>
  <summary>Details</summary>
Motivation: To develop an effective method for segmenting bacterial leaf blight (BLB) severity in rice using UAV multispectral imagery and deep learning models.

Method: The study uses convolutional neural networks (CNNs) and transformer-based models, including U-Net, U-Net++, DeepLabV3+, and SegFormer, with three input configurations: multispectral only, multispectral+NDVI, and multispectral+NDRE.

Result: U-Net++ with EfficientNet-B3 achieved the highest performance with an mIoU of 97.62%, while SegFormer obtained lower segmentation accuracy but comparable inference speed.

Conclusion: The results indicate that lightweight CNN backbones are reliable for operational BLB monitoring, and integration of vegetation indices provides small but consistent improvements, highlighting the value of standardized UAV datasets for disease mapping.
...
TL;DR: The paper introduces PhaseLock, a framework that improves physical consistency in image-to-video diffusion models by preserving motion priors from early steps, achieving better consistency with minimal overhead.


<details>
  <summary>Details</summary>
Motivation: Image-to-video diffusion models often produce motion that violates physical laws, and the authors aim to address this issue without requiring significant modifications or external guidance.

Method: The authors propose PhaseLock, which extracts a motion prior from early steps (2 steps) and enforces it on high-fidelity generations via Latent Delta Guidance, mitigating phase degradation during denoising.

Result: PhaseLock improves physical consistency by an average of 6.2 points across diverse models while maintaining visual fidelity, with negligible overhead (1.06× time, 1.02× memory) and reduced reliance on expensive external guidance methods (∼5× time).

Conclusion: The proposed PhaseLock framework effectively enhances physical consistency in image-to-video diffusion models, offering a practical solution with minimal additional costs.
...
TL;DR: A new framework called GMBFormer is proposed for urban green-space extraction from ultra-high-resolution imagery, improving upon existing methods by integrating physical vegetation confidence and visual appearance learning.


<details>
  <summary>Details</summary>
Motivation: Current methods for urban green-space extraction have limitations, including patch-by-patch processing and the blurring of roles between visual appearance learning and physical vegetation confidence when using the Normalized Difference Vegetation Index (NDVI).

Method: GMBFormer, a SegFormer-based framework, uses selective, similarity-driven prototype retrieval and decouples NDVI as a physics-informed gate to admit high-confidence vegetation descriptors into a global memory bank.

Result: GMBFormer achieves mean intersection over union (mIoU)/mean Dice (mDice) scores of 89.25%/94.31%, 92.17%/95.92%, and 83.72%/90.86% on three datasets, outperforming the SegFormer-B4 baseline.

Conclusion: The proposed GMBFormer framework effectively improves urban green-space extraction by integrating physical vegetation confidence and visual appearance learning, demonstrating superior performance over existing methods.
...
TL;DR: The paper proposes a model-agnostic framework that uses commonsense knowledge to improve scene graph generation, particularly under sparse annotations.


<details>
  <summary>Details</summary>
Motivation: Existing scene graph generation models perform well on frequent relation types but degrade under annotation sparsity, lacking reliable visual commonsense knowledge.

Method: A semantically-guided knowledge refinement framework that mines commonsense-grounded constraints and uses declarative commonsense reasoning to refine SGG predictions.

Result: The framework achieves consistent improvements over strong baselines on three standard benchmarks.

Conclusion: The proposed framework provides a practical and effective complement to purely learning-based scene graph generation by incorporating structured visual commonsense reasoning.
...
TL;DR: A new method called EasyLens is proposed to improve the sensitivity of medical vision-language models to subtle lesions in images without requiring additional training.


<details>
  <summary>Details</summary>
Motivation: Current medical vision-language models have limited practical utility due to insufficient sensitivity to subtle lesions, which can be sparse, low-contrast, and embedded in complex anatomical contexts.

Method: EasyLens is a training-free plug-and-play approach that constructs a pathology-anatomy prototype space (EasyBank) and uses counterfactual prototype reasoning (EasyTag) to select lesion-relevant patches, then strengthens these patch representations through morphology-guided residual enhancement (EasyAmplifier).
...
Conclusion: EasyLens offers a promising solution to enhance the sensitivity of medical vision-language models to subtle lesions without requiring additional training or model-specific adaptation.
...
TL;DR: A unified hierarchical framework for generating indoor scenes with controllable stages, producing diverse and realistic whole-home scenes for robot simulation and interior design.
...
Method: A multi-stage framework: (1) training a large language model for whole-home floorplan generation using a large-scale dataset, (2) generating furniture layouts from multi-level roaming viewpoints, and (3) generating small object layouts with a VLM-based refiner and 3D generative model.
...
Conclusion: The proposed unified hierarchical framework enables the generation of realistic and diverse whole-home scenes, and the released dataset and furnished scenes can benefit the community.
...
TL;DR: The authors propose a framework for radiological comparison in medical imaging AI, introducing entity-aware cross-image reasoning for reference-case retrieval and temporal comparative interpretation.


<details>
  <summary>Details</summary>
Motivation: Current medical imaging AI struggles to align with radiological practice, which relies on comparison across prior studies and reference cases.

Method: The authors construct a large-scale comparative imaging resource (MedReCo-DB) and develop two models: MedReCo for entity-aware visual encoding and MedReCo-VLM for generative interpretation of interval change.

Result: MedReCo and MedReCo-VLM outperform baselines in retrieval and comparative generation evaluations, with improvements in recall and longitudinal follow-up accuracy.

Conclusion: Entity-aware comparative reasoning can be learned from routine clinical data at scale, providing a more clinically aligned foundation for medical imaging AI.
...
<details>
  <summary>Details</summary>
Motivation: Current VLMs are limited in their spatial reasoning abilities, particularly in inferring unobserved layouts, maintaining cross-view consistency, and reasoning from alternative viewpoints with limited observations.

Method: The Astra framework consists of Astra-VL, an RL-trained VLM policy, and Astra-WM, a world simulator that generates novel-view observations. Astra-WM is trained with view consistency tuning, and Astra-VL is trained with a two-phase RL curriculum that stabilizes tool-use exploration.

Result: Experiments show that Astra improves the performance of VLMs on MMSI-Bench and MindCube benchmarks, with Astra-WM improving Gemini-3-Flash from 45.1 to 49.5 and Astra-VL improving Qwen3-VL backbone from 29.8 to 38.8 and 36.8 to 42.7 respectively.
...
TL;DR: The paper proposes Complexity-Balanced Splitting (CBS), a framework for temporal capacity allocation in generative models, which distributes workload across specialized sub-networks.


<details>
  <summary>Details</summary>
Motivation: Current generative models use monolithic architectures that are inefficient when dealing with different signal regimes, and scaling model capacity uniformly is not optimal.

Method: CBS uses function approximation theory and de Boor's equidistribution principle to partition the diffusion timeline into segments of equal approximation burden, allocating more capacity to complex regions.

Result: CBS consistently improves synthesis quality without increasing per-step inference cost, with a ~35% improvement in FID on SiT-XL with CFG.

Conclusion: The proposed framework, CBS, offers a principled approach to temporal capacity allocation, leading to improved performance in generative models.
...
TL;DR: The paper introduces PAR3D, a unified part-aware 3D multimodal large language model (3D-MLLM) framework that enables understanding, reasoning, and grounding of objects and their parts in 3D scenes.


<details>
  <summary>Details</summary>
Motivation: Existing 3D-MLLMs are largely object-centric, limiting their ability to model fine-grained part structures essential for embodied interaction with 3D environments.

Method: The authors propose Part-Aware 3D Representation Learning and Hierarchical Segmentation Query Generation to enrich 3D visual representations with part-level semantics and ground part targets.
...
Conclusion: The proposed PAR3D framework advances 3D scene understanding by incorporating part-aware capabilities, demonstrating improved performance in various tasks.
...
TL;DR: The authors propose a bilayer coupled SIR/SIRS framework to model the spread of contamination in AI ecosystems when synthetic data is used for training, leading to model collapse. They derive a basic reproduction number and analyze intervention strategies.


<details>
  <summary>Details</summary>
Motivation: Existing analyses treat model collapse due to synthetic data as a single-chain degradation, but the reality involves cross-contamination across the AI ecosystem.

Method: The authors propose a bilayer coupled SIR/SIRS framework, derive the basic reproduction number $R_0$, and use scenario-based calibration, Sobol sensitivity analysis, and bipartite-network agent-based modeling to validate their approach.

Result: The results show supercritical dynamics across scenarios, with synthetic-text detection as the highest-leverage parameter. Experiments with GPT-2 contamination chains and matched-budget source-diversity experiments provide evidence for the model's predictions.

Conclusion: The authors conclude that detection-based filtering and herd immunity are the highest-leverage strategies to mitigate model collapse, and that multi-source mixing can modestly attenuate collapse but the effect vanishes at lower contamination fractions.
...
TL;DR: The paper proposes a hybrid pre-training objective combining JEPA-style latent-space prediction loss with MLM for text encoders, showing improved uniformity and semantic-to-lexical balance in embeddings.


<details>
  <summary>Details</summary>
Motivation: The dominant pre-training objective, MLM, focuses on surface-form token identity rather than deeper semantic structure, prompting the need for an alternative approach.

Method: A hybrid model combining JEPA-style latent-space prediction loss with standard MLM objective over a single shared encoder, balanced by a learnable scalar parameter.

Result: The hybrid encoder produces more uniform embeddings, exhibits richer spectral geometry, encodes less surface-level lexical information, and achieves better semantic-to-lexical balance.

Conclusion: The proposed hybrid pre-training objective reshapes the latent space in meaningful ways, not captured by standard accuracy metrics, suggesting potential benefits for text encoder applications.
...
<details>
  <summary>Details</summary>
Motivation: Deploying general-purpose LLMs in real-world healthcare settings is challenging due to data privacy constraints, inference costs, and limited suitability for edge or on-device use, motivating the development of smaller, more efficient models with robust post-training strategies.

Method: The paper proposes using GRPO with a Variance-Aware Reward Framework that extends rubric-based supervision with continuous analytical reward functions, enabling more stable on-policy reinforcement learning.

Result: The best GRPO variant improves accuracy from 0.362 to 0.502 and F1 from 0.532 to 0.668 relative to the Qwen3-14B base model on a held-out heart-related subset of HealthBench, remaining competitive with GPT-OSS-120B.
...
TL;DR: The paper proposes a generic triple-latent sequence model to capture higher-order token interactions, which improves a Transformer baseline on two language-model benchmarks.


<details>
  <summary>Details</summary>
Motivation: To capture higher-order token interactions without benchmark-specific parsing.

Method: The authors propose a generic triple-latent sequence model with a running token state and compressed pair-memory pathway.

Result: The triple-latent family improves a small Transformer baseline on byte-level WikiText-2 and on a tokenizer-based MiniMind language-model benchmark.

Conclusion: The proposed model shows improvements over a Transformer baseline, but a recall-focused extension has limitations in terms of seed sensitivity and speed.
...
TL;DR: The paper presents a study on fine-tuning a large language model (Qwen2.5-3B) for a domain-specific conversational assistant in telecommunications customer support using Low-Rank Adaptation (LoRA).


<details>
  <summary>Details</summary>
Motivation: The use of large language models in telecommunications customer support is limited due to domain-specific constraints, data sovereignty, and regulatory constraints.

Method: The authors use parameter-efficient fine-tuning (PEFT) with LoRA and a combinatorial synthetic data generation approach to build a domain-specific conversational assistant.

Result: The results show a divergence between quantitative and qualitative performance, with models achieving the lowest validation loss not necessarily obtaining the best human-aligned rankings.
...
TL;DR: Introduced MCBench, a multimodal safety benchmark for Omni Large Language Models (LLMs) that process vision, audio, and text, revealing current models' struggles with subtle or non-physical risks and cross-modal reasoning.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal safety benchmarks are limited to visual inputs and cannot assess Omni LLMs that process multiple modalities, highlighting the need for a more comprehensive benchmark.

Method: Introduced MCBench, a benchmark with 1196 scenarios across four safety categories, paired with minimally different safe counterparts to assess model sensitivity.

Result: State-of-the-art Omni LLMs struggle with subtle or non-physical risks but perform better with salient visual or acoustic cues; models can extract modality-specific information but often fail to integrate cues effectively for safety judgments.

Conclusion: Current Omni LLMs lack robust cross-modal reasoning in safety-critical settings, underscoring the need for improved architectures and training strategies for multimodal safety.
...
TL;DR: The paper proposes a framework for interpretable rubric-based scoring using Shapley-value attributions and large language model rationales, and evaluates its performance on a dataset of classroom transcripts.
...
Method: The proposed framework combines model-agnostic Shapley-value attributions with rationales generated by large language models, and is evaluated on the Quality of Feedback dimension of the CLASS framework using the NCTE corpus.

Result: Fine-tuned pretrained language models outperform large language models in prediction accuracy, but exhibit label compression. SHAP attributions identify sentences that reliably drive model predictions and transfer robustly across architectures.

Conclusion: The proposed framework provides more faithful and transferable explanations for rubric-based scoring, and offers a principled basis for evaluating both scoring models and their explanations in high-stakes educational settings.
...
TL;DR: A novel Multi-Granularity Reasoning Network (MGRN) is proposed to improve Natural Language Inference (NLI) by leveraging hierarchical semantic features for better reasoning.
...
Method: A Multi-Granularity Reasoning Network (MGRN) that integrates semantic information across multiple granularities in a progressive and structured manner.
...
Conclusion: The proposed MGRN approach is effective and robust for Natural Language Inference tasks by explicitly leveraging hierarchical semantic features.
...
TL;DR: LANTERN, a lightweight memory layer, helps large language models retain critical details in conversations by proactively archiving and retrieving relevant information, improving accuracy and outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Large language models struggle with retaining critical details in conversations due to finite context windows, leading to loss of important information.

Method: LANTERN (Layered Archival aNd Temporal Episodic Retrieval Network) uses hybrid retrieval to archive and restore relevant conversation details, requiring zero LLM calls and adding minimal latency.

Result: LANTERN recovers 78.3% of lost facts, outperforming existing methods, and improves accuracy by 8.4 percentage points when used with production LLMs.

Conclusion: LANTERN effectively retains critical details in conversations, improving performance and reproducibility, and can be used across diverse model architectures.
...
TL;DR: Researchers introduce the Granularity Gap to measure sycophancy in large language models, finding that standard binary metrics miss substantial social-compliance behaviors, and that generational progress is non-monotonic, with an 'Alignment Tax' between sycophancy and truthfulness.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the issue of sycophancy in large language models, which are increasingly deployed as high-stakes advisors, and to improve the measurement and mitigation of sycophantic behaviors.

Method: The method involves evaluating six Gemini variants on 73 adversarial prompts under three guardrail conditions, using a 0-4 Likert scale validated against human annotators, and analyzing the results to quantify sycophancy as a continuous rather than binary phenomenon.

Result: The results show that 27.2% of responses contain substantial sycophantic content, and 22.7% reach moderate or severe levels, with generational progress being non-monotonic, and an 'Alignment Tax' between sycophancy and truthfulness.

Conclusion: The conclusion is that standard binary metrics are insufficient to capture the complexity of sycophancy in large language models, and that a more nuanced approach is needed to measure and mitigate sycophantic behaviors, with implications for the development of more effective guardrails and evaluation methods.
...
TL;DR: The paper proposes a low-rank distillation framework to improve implicit chain-of-thought (iCoT) methods in large language models, achieving performance close to explicit CoT prompting.


<details>
  <summary>Details</summary>
Motivation: Implicit chain-of-thought (iCoT) methods underperform explicit CoT prompting, and the authors aim to improve iCoT by exploring the structure of hidden-state reasoning trajectories.
...
Conclusion: The proposed low-rank distillation framework effectively captures the global structure of reasoning and supports a compact latent reasoning process, enhancing the performance of iCoT methods.
...
Method: The authors present PERSUASIONTRACE, a framework built on a web-based experimental platform, which records multi-turn belief reports, annotates persuader turns with rhetorical dimensions, and evaluates simulators by fidelity to real human belief dynamics.

Result: The authors find that human targets group into two clusters of multi-turn belief updates, exhibit susceptibility to rhetorical strategies, and that LLMs are persuasive across various topics and modalities. They also introduce a Bayesian-network simulated target that outperforms existing simulators in replicating human belief dynamics.

Conclusion: PERSUASIONTRACE provides a stronger basis for scientific analysis and safer optimization of persuasive systems by reframing persuasion evaluation from endpoint movement alone to process fidelity.
...
TL;DR: BUMP is a self-supervised framework for personalizing large language models by generating user profiles from interaction history, eliminating the need for labeled data.


<details>
  <summary>Details</summary>
Motivation: The challenge of personalizing large language models for different users in various applications, and the difficulty of obtaining labeled data for training profile generators.

Method: Bidirectional User Modeling via Profiles (BUMP) uses a bidirectional in-batch ranking objective and GRPO to train a profile generator without labeled data.
...
Conclusion: BUMP offers a promising solution for personalizing large language models without requiring labeled data, achieving competitive performance on a benchmark task.
...
TL;DR: The study introduces a new predictor of incremental processing cost in language comprehension, called trajectory extrapolation error, which captures the deviation from the expected interpretation trajectory. This measure is orthogonal to surprisal and independently predicts self-paced reading times.
...
Method: The authors use a transformer language model to fit a linear trajectory to the preceding hidden states and measure deviation from the extrapolated path, termed trajectory extrapolation error. They test this measure on the Natural Stories corpus.

Result: The study finds that trajectory extrapolation error is nearly orthogonal to surprisal and independently predicts self-paced reading times. The effect is especially pronounced in garden-path sentences and strengthens with model scale and across architectures.
...
TL;DR: The paper introduces ReasoningFlow, a framework for capturing the discourse structures of large reasoning models' (LRMs) traces into directed acyclic graphs (DAGs), enabling better evaluation and monitoring of the reasoning process.


<details>
  <summary>Details</summary>
Motivation: The complexity of non-linear structures in LRM reasoning traces, such as backtracking and self-correction, makes evaluation and monitoring challenging.

Method: The authors develop an annotation schema, manually annotate 31 traces, and then scale to automatic annotation of 1,260 traces across three tasks and five models.

Result: The analysis reveals that LRMs exhibit structurally similar traces, display diverse fine-grained reasoning behaviors, and most erroneous steps are not used to derive final answers.

Conclusion: The paper presents ReasoningFlow as a valuable tool for understanding and improving LRM reasoning processes, releasing the dataset and code for future research.
...
Method: A two-stage approach consisting of an attention-based failure predictor that learns sparse turn-level failure evidence from trajectory labels, and a single preference-conditioned stopping policy ($α$-STOP) for controllable early alerting.

Result: The approach shows improvement in Pareto-frontier quality (hypervolume) by 1-10% over naive prefix supervision and by 3-42% over state-of-the-art trigger policies, with reduced training cost per operating point by 1-3 orders of magnitude across five benchmarks.

Conclusion: The proposed approach effectively addresses the challenge of early failure alerting in multi-turn language interactions by leveraging sparse evidence and providing controllable early alerting.
...
TL;DR: The paper presents a system that automatically discovers an executable schema from raw multi-source data and uses it for knowledge graph construction and query-time retrieval, improving over existing baselines in four QA benchmarks.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of querying real-world data that spans multiple tables, documents, and semi-structured files with implicit semantics, and to integrate evidence across inconsistent schemas and formats.

Method: The method involves a closed-world field catalog, deterministic structural analysis, and LLM-based schema discovery to infer identity keys, foreign keys, and source hierarchy, and then uses the resulting schema to drive extraction, deduplication, and cross-source linking into a provenance-aware knowledge graph.
...
Conclusion: The paper concludes that the proposed system effectively integrates evidence across inconsistent schemas and formats, and provides a shared contract for knowledge graph construction and query-time retrieval, leading to improved performance in QA benchmarks.
...
<details>
  <summary>Details</summary>
Motivation: To investigate if machine translation retains the complexity of the original text, using CEFR levels as a measure of text complexity.

Method: Evaluating open-weight and closed models, and a commercial machine translation system on two tasks: correlation of CEFR with translation difficulty and shifts in CEFR levels across six languages.
...
TL;DR: A novel pipeline for coreference resolution in low-resource languages using machine translation from English to improve performance.


<details>
  <summary>Details</summary>
Motivation: The lack of attention to coreference resolution in low-resource languages, which hinders its application in various NLP tasks.

Method: A pipeline that uses machine translation to generate or expand training data, and validates sample quality via cosine similarity in a BERT model's latent space.

Result: Significant performance gains in coreference resolution on four low-resource languages, enabling accurate resolution in languages with no previous corpora.

Conclusion: The proposed pipeline effectively addresses the challenge of coreference resolution in low-resource languages.
...
TL;DR: PRIG, a gradient attribution method, is introduced to identify ambiguous parts of a prompt in large language models by attributing latent ambiguity to token positions.


<details>
  <summary>Details</summary>
Motivation: Prompt ambiguity is a common source of failure in large language models but is difficult to localize as it's a latent property of the prompt.

Method: PRIG uses a probe logit to attribute latent ambiguity to token positions by training a linear probe to distinguish clear prompts from ambiguous prompts.
...
TL;DR: A Multi-Model Adaptive Summarization Framework is proposed to improve the robustness and quality of abstractive text summarization by integrating multiple fine-tuned transformer-based models and an adaptive selection mechanism.


<details>
  <summary>Details</summary>
Motivation: The rapid growth of digital textual information has increased the importance of automatic text summarization. However, relying on a single model often leads to inconsistent summarization quality across articles with varying structures and topics.

Method: The proposed framework integrates multiple fine-tuned transformer-based summarization models and introduces an adaptive selection mechanism. Each model generates a candidate summary, which is then evaluated using automatic evaluation metrics. The framework selects the highest-quality summary as the final output.

Result: The proposed framework achieves a BERTScore of 88.63% on the CNN/DailyMail news summarization dataset, outperforming several LLMs such as GPT3-D2, Falcon-7b, and Mpt-7b.
...
TL;DR: The paper introduces CHASE, a framework that uses co-evolutionary training to improve the safety of language models against prompt-rewriting attacks.


<details>
  <summary>Details</summary>
Motivation: Despite advances in safety alignment, language models are still vulnerable to prompt-rewriting attacks. Existing defenses have limitations, and the authors aim to address this gap.

Method: The authors propose CHASE, a closed-loop red-blue teaming framework that co-evolves a black-box attacker and a safety-aligned defender using Group Relative Policy Optimization (GRPO) and a two-stage GRPO + rejection-sampled SFT pipeline.
...
Conclusion: The paper concludes that CHASE is an effective approach to improving the safety of language models against adaptive black-box adversaries, and suggests a path toward LLM safety hardening that generalises beyond narrow distributions achieved in adversarial training.
...
TL;DR: The study proposes a cross-language training approach for detecting Alzheimer's Disease (AD) using multilingual deep learning models, achieving strong performance across languages.
...
Method: The study uses transformer-based models for binary AD classification, leveraging cross-language training and datasets in English, Chinese, Arabic, and Hindi.

Result: The approach achieved F1 scores of 82% across all languages, with rapid inference time (0.5 seconds), supporting potential real-time screening applications.

Conclusion: The study demonstrates the feasibility of using multilingual deep learning models for detecting AD across different languages and cognitive impairment levels, with potential for global deployment.
...
TL;DR: The paper introduces ArcANE, a benchmark for evaluating role-playing language agents' ability to evolve character behavior and values over time, and shows that conditioning on character arcs improves performance, especially in scenarios not explored in the source text.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for role-playing language agents focus on factual recall at a given chapter, but do not assess whether responses align with the character's psychological trajectory over time.

Method: The authors introduce ArcANE, an automatically constructed benchmark spanning 17 novels and 80 principal characters, and evaluate six models and six context modes on this benchmark.

Result: Conditioning on character arcs improves performance on the benchmark, especially in scenarios outside the source text, and fine-tuning open-weight models on the same data further widens the advantage.

Conclusion: The paper concludes that character arcs are essential for evaluating role-playing language agents and that conditioning on these arcs improves performance, especially in scenarios not explored in the source text.
...
<details>
  <summary>Details</summary>
Motivation: Situated queries often have implicit needs beyond their literal content, and standard agents fail to address these needs.

Method: AURA uses an inference step to produce an IntentFrame, estimating implicit needs with a scalar gap score for probe budget and tool selection.
...
TL;DR: InfoShield is a speech-based mental health screening method that protects user privacy while detecting depression.


<details>
  <summary>Details</summary>
Motivation: Current speech-based mental health screening methods struggle with user privacy concerns, particularly regarding demographic information exposure.

Method: InfoShield uses TimeAwareMINE with cross-modal attention to minimize mutual information between speech representations and sensitive attributes while preserving depression classification accuracy.
...
Conclusion: InfoShield effectively protects user privacy while maintaining depression detection accuracy, making it suitable for clinical deployment.
...
TL;DR: This paper presents a large language model (LLM)-based tool to assist in evaluating 1,200 student Statements of Purpose (SoPs) for the Summer Undergraduate Research Fellowship (SURF) program at Purdue University, reducing evaluation time and effort.
...
Result: The LLM outputs replicated the role previously played by distributed human graders, providing the program coordinator with scored and rationale-annotated outputs for the entire applicant pool. The program coordinator then reviewed these outputs alongside each applicant's SoP, applying the same downstream office criteria used in prior SURF cycles, to produce a shortlist of strong candidates.

Conclusion: The proposed LLM-based tool significantly reduced the evaluation time and effort, processing 1,200 SoPs in approximately 4.6 hours of compute time, and enabling the program coordinator to complete the review in approximately 4 hours, compared to the multi-week coordination effort required in prior program cycles.
...
TL;DR: A method for mispronunciation detection and diagnosis using statistical graphs and language-specific strategies is proposed, achieving an F1-score of 59.52% on the L2-ARCTIC benchmark.


<details>
  <summary>Details</summary>
Motivation: The importance of mispronunciation detection and diagnosis in computer-assisted language learning and speech technology has increased, driving the need for effective methods to identify and address pronunciation errors.

Method: The proposed method constructs statistical graphs to represent phoneme confusion patterns as directed graphs and employs a language-specific strategy to capture systematic pronunciation differences across various native language backgrounds.

Result: The approach achieves an F1-score of 59.52% on the L2-ARCTIC benchmark, outperforming several competitive baselines.

Conclusion: The proposed method demonstrates effectiveness in mispronunciation detection and diagnosis, suggesting its potential for improving computer-assisted language learning and speech technology.
...
TL;DR: Introduced TensorBench, a benchmark for coding agents with 199 tasks on a tensor framework, achieving varied pass rates among agents.


<details>
  <summary>Details</summary>
Motivation: Addressing the trade-off between task difficulty and evaluation reliability in coding benchmarks.

Method: TensorBench benchmark with 199 tasks, grading via test suite runs, evaluating seven coding agents.

Result: Pass rates range from 64.8% to 22.1% among agents, with varied task success subsets.

Conclusion: TensorBench reveals performance differences among coding agents, highlighting evaluation challenges.
...
TL;DR: The authors propose a framework for efficiently finding optimal hyperparameters for continued pre-training of Large Language Models (LLMs), reducing search overhead by up to 90% while achieving comparable or superior performance.


<details>
  <summary>Details</summary>
Motivation: Current practices for hyperparameter configuration in continued pre-training of LLMs rely on heuristics or grid searches, leading to training instability and excessive costs.

Method: The proposed framework consists of two stages: (1) Empirical Law Discovery, where small-scale proxy models are trained to derive functions mapping compute budget to optimal hyperparameters; and (2) State-Aware Hyperparameter Prediction, where an initial checkpoint's validation loss is evaluated to estimate its equivalent pre-training compute and predict optimal hyperparameters for the target run.
...
Conclusion: The authors' framework provides a principled and efficient methodology for diverse continued pre-training scenarios, enabling more efficient and effective hyperparameter configuration for LLMs.
...
TL;DR: Large language models (LLMs) often rely on word affixes to infer meanings, which can lead to overgeneralization, especially in high-stakes domains like medicine. This study examines how LLMs use affix heuristics in pharmacology and finds that they frequently induce drug meanings through affix cues, posing a risk to safety.


<details>
  <summary>Details</summary>
Motivation: The study aims to investigate how LLMs use morphological forms of words, specifically affixes, to infer meanings in the medical domain, and to identify potential risks associated with this reliance.

Method: The study uses fictitious drug names built from real affixes to test LLM responses and introduces a framework to analyze whether a model's drug semantics are driven by affix, stem, or the drug name as a whole. It applies this framework to 653 drugs and uses activation patching to localize the behavior to early-mid layers of the models.

Result: The study finds that LLMs often induce drug meanings primarily through affix cues, but rarely explicitly indicate this reliance. It also shows that models sometimes incorrectly conflate properties among affix-sharing drugs, posing a subtle but measurable risk to safety.

Conclusion: The study concludes that morphological shortcuts, specifically reliance on affix heuristics, pose a risk to safety in high-stakes domains like medicine. It highlights the need for careful evaluation and mitigation of these risks in LLMs.
...
TL;DR: The study examines how children with Autism Spectrum Disorder (ASD) process complex linguistic structures, specifically two-level recursive locative constructions in Mandarin. It finds that children with ASD have difficulties with syntactic processing, showing reduced early predictive engagement and reanalysis efficiency, and increased semantic integration demands.


<details>
  <summary>Details</summary>
Motivation: The study aims to understand the temporal dynamics of recursive processing in children with ASD, as difficulties with complex syntax have been reported but the underlying processing mechanisms are poorly understood.

Method: The study uses event-related potentials (ERPs) to examine neural responses in 12 children with ASD and 12 typically developing (TD) children in a cross-modal sentence-picture matching task.

Result: The results show that children with ASD exhibit attenuated early differentiation and reduced late reanalysis effects, and enhanced N400 responses under mismatch conditions, indicating increased semantic integration demands. The study also finds greater inter-individual variability in hemispheric lateralization in the ASD group.

Conclusion: The findings suggest a cascading account in which reduced early predictive engagement in ASD leads to increased integration costs and diminished reanalysis efficiency during recursive processing, highlighting the importance of temporal processing dynamics and neural variability in understanding language differences in ASD.
...
TL;DR: Introduced AdaPlanBench, a dynamic benchmark for evaluating Large Language Models' (LLMs) ability to adaptively plan under progressively revealed world and user constraints. Experiments on 10 leading LLMs show that adaptive planning remains challenging, with the best model reaching 67.75% accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks underexplore adaptive planning under progressively revealed dual constraints (world and user constraints). The authors aim to address this gap by introducing a new benchmark and evaluating LLMs' ability to adaptively plan and re-plan under such constraints.

Method: The authors propose AdaPlanBench, a dynamic interactive benchmark built on 307 household tasks with a scalable constraint construction pipeline. Agents interact with the environment in a multi-turn protocol, where hidden constraints are revealed when the agent proposes a plan that violates them, requiring iterative plan revision.

Result: The best LLM model reaches 67.75% accuracy, with performance degrading as more constraints accumulate. User constraints pose a particularly large challenge, and failures often stem from weaker physical grounding and reduced effectiveness.

Conclusion: AdaPlanBench establishes a testbed for dual-constrained interactive planning and highlights the challenge of reliable adaptation to dynamically revealed constraints in LLM agents. The results suggest that LLMs struggle with adaptive planning under progressively revealed dual constraints.
...
TL;DR: The paper proposes RidgeFT, a lightweight framework for lifelong machine-generated text attribution, which improves the ability to identify text generators while adapting to new classes and retaining old ones.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of lifelong machine-generated text attribution, where attribution models must continuously incorporate new generators while preserving their ability to recognize previously seen ones.

Method: The method proposed is RidgeFT, a lightweight analytic update framework that trains a task-aware encoder, stores compact class-wise sufficient statistics, and updates new classes through closed-form ridge regression.

Result: RidgeFT consistently outperforms baselines across multi-topic evaluations, achieving the best macro-F1 across domains, backbones, and incremental protocols, while improving both old-class retention and new-class adaptation.

Conclusion: The conclusion is that feature-stable analytic updates provide a simple yet effective approach to lifelong machine-generated text attribution.
...
TL;DR: The paper introduces GATE, a method to improve text-to-SQL by using execution feedback to bootstrap missing groundings, achieving better results than strong baselines.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the issue of under-specified text-to-SQL queries by leveraging execution feedback to improve grounding, as prior methods relying on pre-specified semantic layers often fall short, especially in expert domains.

Method: The method introduced is GATE (Grounding After Test from Execution), which uses execution feedback to test and validate grounding hypotheses for SQL queries, accumulating these into an execution-grounded memory for reuse.

Result: The results show that GATE consistently improves over strong baselines across real-world and controlled benchmarks, demonstrating the effectiveness of execution as a bootstrapping mechanism for reusable memory in text-to-SQL.

Conclusion: The conclusion is that execution feedback can be effectively used to bootstrap and improve the grounding of text-to-SQL queries, enhancing the performance of text-to-SQL systems, especially in domains with under-documented conventions.
...
TL;DR: The paper proposes QueryAgent-R1, a framework that suggests queries matching users' interests in e-commerce search, improving alignment between queries and user preferences.


<details>
  <summary>Details</summary>
Motivation: Existing query recommendation methods focus on query-level relevance but neglect product alignment with user preferences, leading to high query CTR but low product CVR.

Method: QueryAgent-R1 uses a memory-augmented agentic framework with chain-of-retrieval optimization, consistency reward in agentic RL, and a memory abstraction module for user profiling.

Result: QueryAgent-R1 outperforms baselines on proprietary and public datasets and improves Query CTR by 2.9% and guided CVR by 3.1% in online A/B tests.

Conclusion: QueryAgent-R1 effectively bridges the gap between query relevance and downstream engagement in e-commerce search.
...
TL;DR: The paper proposes a new method called VSRAQ for quantizing Mixture-of-Experts (MoE) models, which reduces degradation in model quality caused by quantization.


<details>
  <summary>Details</summary>
Motivation: MoE models are sensitive to routing instability caused by quantization, which can change expert selection and alter the computation path, degrading model quality.

Method: The authors propose Value-and-Structure Routing Alignment for Quantization (VSRAQ), a post-training quantization objective that preserves pre-quantization expert-selection behavior under quantization.

Result: Experiments on recent MoE foundation models show that VSRAQ improves expert-selection consistency and consistently outperforms baselines.

Conclusion: VSRAQ reduces quantization-induced degradation without introducing any inference-time overhead and can be integrated into existing quantization frameworks.
...
TL;DR: The study examines the interaction between parameter-side memory (document LoRA adapters) and context-side memory (KV cache) in document-level question answering. It finds that document LoRA adapters are useful when context-side evidence is scarce, recovering 13-21 ROUGE-L points when no document context remains.


<details>
  <summary>Details</summary>
Motivation: To understand how parameter-side memory (document LoRA adapters) interacts with context-side memory (KV cache) in document-level question answering.

Method: The study progressively evicts document key-value states and measures when a document LoRA contributes beyond the retained context.

Result: Document LoRA adapters add little when the KV cache is largely intact, but become increasingly useful under aggressive compression. The gain is largest when the base model encodes the document, and the adapter is applied only during answer generation.

Conclusion: Document LoRA adapters act as a complementary memory channel whose value emerges precisely when context-side evidence is scarce, and QA-style supervision produces substantially stronger adapters than raw-context next-token-prediction.
...
TL;DR: This paper presents a unified framework for organizing and analyzing latent communication methods in multi-agent systems built on large language models.


<details>
  <summary>Details</summary>
Motivation: The dominant natural language communication protocol in multi-agent systems suffers from high inference cost, irreversible information loss, and ambiguity/redundancy, prompting exploration of latent communication as an alternative.

Method: The authors analyze existing latent communication methods along three axes: WHAT information is communicated, WHICH sender-receiver alignment is used, and HOW the communicated information is fused into the receiver.

Result: The authors categorize eighteen representative methods, identify five major design patterns, and surface a set of open challenges in latent communication.

Conclusion: The proposed framework aims to lower the barrier to entry for new researchers and provide a vocabulary for comparing future work in latent communication.
...
TL;DR: This paper proposes a novel framework for interpreting style representations in authorship analysis through style-eliciting prompts, which are natural language instructions designed to steer large language models (LLMs) to generate text reflecting specific stylistic attributes.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this work is to address the difficulty in interpreting latent style representations learned through authorship analysis and modeling writing style, and to overcome the limitations of using LLMs to generate descriptions that are prone to biases and hallucinations.

Method: The method involves curating 1,010 distinct style features across 26 stylistic categories, constructing a dataset by prompting an LLM to generate text conditioned on these features, and training a decoder to generate style prompts from style representations of the generated text.

Result: The results show that the proposed approach consistently outperforms strong baselines in three tasks: recovering original style prompts, generating text in the same style, and steering LLM outputs to match human-written texts.

Conclusion: The conclusion is that style-eliciting prompts provide a practical and interpretable interface to stylistic information encoded in style representations, enabling superior performance in style description and style imitation.
...
TL;DR: A new framework called Narrative Knowledge Weaver (NKW) is proposed for long-form narrative QA, which aligns textual evidence and graph structure to improve reasoning over evolving story worlds.


<details>
  <summary>Details</summary>
Motivation: Existing methods for narrative QA have limitations in encoding how evidence functions in a story, and the proposed framework aims to address this by integrating textual evidence, atomic facts, and graph structure.

Method: NKW uses a source-grounded framework that aligns textual evidence, atomic facts, canonical graph structure, entity profiles, interactions, episodes, and storylines, and employs text, graph, and narrative tools with post-retrieval reading skills to assemble evidence.

Result: NKW achieves the best results on screenplay-level story-world QA and remains competitive on passage-centered benchmarks, with complementary benefits for character, scene, temporal, causal, and narrative-progression reasoning.

Conclusion: The proposed Narrative Knowledge Weaver framework demonstrates the effectiveness of aligning textual evidence and graph structure for long-form narrative QA, and shows improved reasoning capabilities over evolving story worlds.
...
<details>
  <summary>Details</summary>
Motivation: Existing model-free variants of speculative decoding have limitations in recall under surface-form variation and brittleness when retrieved context does not uniquely determine continuation.

Method: The proposed AdaPLD method adaptively improves retrieval and draft construction by preserving high-precision lexical reuse and using semantic similarity to recover additional reuse opportunities.

Result: AdaPLD reduces target-model forward passes and achieves up to 3.10x decoding speedup across diverse benchmarks.

Conclusion: AdaPLD is an effective training-free method for improving speculative decoding by enhancing retrieval and draft construction.
...
TL;DR: The paper introduces PlanBench-V, a benchmark for evaluating Vision-Language Models (VLMs) in spatial planning map interpretation, highlighting their limitations in professional planning contexts.


<details>
  <summary>Details</summary>
Motivation: The interpretation of spatial planning maps requires fine-grained visual perception, spatial reasoning, and policy-informed professional judgment, posing challenges for both humans and AI systems. Existing multimodal benchmarks overlook domain-specific cognitive processes of planning practice.

Method: The authors create the Spatial Planning Map Database (SPMD), an expert-annotated dataset of 223 planning maps and 1629 question-answer pairs. They propose a theory-informed evaluation framework assessing four progressive capabilities: Perception, Reasoning, Association, and Implementation.

Result: Experiments across two generations of VLMs show clear progress but persistent limitations. The best 2026 agentic reasoning model, Qwen3.6-Plus, outperforms the best 2025 model, GPT-4o, by 27%. However, all models struggle with implementation-oriented tasks requiring evaluative judgment, policy sensitivity, and constraint-aware decision-making.

Conclusion: The findings reveal fundamental limitations of current VLMs in professional planning contexts and highlight the need for domain-adaptive multimodal reasoning frameworks.
...
TL;DR: MARDoc, a Memory-Aware Refinement Agent framework, is proposed for multimodal long-document question answering. It uses three specialized agents to decouple the process into retrieval, refinement, and reflection, leveraging a dynamically updated structured memory.


<details>
  <summary>Details</summary>
Motivation: Existing iterative retrieval-reasoning agents for multimodal long-document question answering mix retrieval traces, observations, and intermediate reasoning in a single growing context, leading to scattered and diluted key evidence, and noisy multi-hop reasoning.

Method: The MARDoc framework consists of three agents: an Explorer for multimodal retrieval, a Refiner for distilling interaction traces into structured evidence and reasoning memories, and a Reflector for checking evidence sufficiency and providing targeted feedback. These agents rely on a dynamically updated structured memory.
...
Conclusion: The proposed MARDoc framework effectively addresses the issue of context noise in iterative retrieval-reasoning agents by decoupling the process into specialized agents and leveraging a structured memory, leading to improved performance in multimodal long-document question answering.
...
TL;DR: The paper proposes CollabBench, a benchmark for evaluating and training collaborative agents in cooperative games, achieving 19.5% higher efficiency and 24.4% improved affective performance.


<details>
  <summary>Details</summary>
Motivation: Existing conversation-level collaborative studies lack grounded interaction and behavioral execution, making effective collaboration with realistic human partners challenging.

Method: The paper proposes CollabBench with a Diverse Player Profile Simulation pipeline and a Collaborative Agentic Training paradigm, and evaluates it in CWAH-MultiPlayer and Cook-MultiPlayer environments.

Result: Trained models outperform base models, achieving 19.5% higher efficiency and 24.4% improved affective performance.

Conclusion: The paper provides insights for future collaborative training and highlights key collaborative limitations of existing models.
...
TL;DR: The paper proposes two recall-based prompting strategies, Self-Recall (SR) and Question-Recall (QR), to improve large language models' (LLMs) ability to answer questions while adhering to a specified knowledge cutoff date.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitation of current LLMs in handling post-cutoff knowledge that is not explicitly queried but is causally related to the question.
...
Result: The proposed methods outperform direct-answer prompting and conventional step-by-step reasoning baselines across three existing benchmarks, with particularly strong improvements on counterfactual questions. The results also show that combining SR and QR consistently yields the best performance across different cutoff settings.

Conclusion: The paper concludes that the proposed recall-based prompting strategies can effectively improve LLMs' ability to answer questions while adhering to a specified knowledge cutoff date, and that combining SR and QR can yield robust performance across different cutoff settings.
...
TL;DR: ProSPy, a new Text-to-SQL framework, improves performance on large-scale enterprise databases by combining SQL and Python analysis.
...
Method: ProSPy uses a four-stage approach: extracting data evidence through automatic profiling, pruning large schemas, fetching intermediate views through a dialect-agnostic SQL interface, and performing downstream analysis with Python.

Result: ProSPy outperforms strong baselines on Spider 2.0-Lite and Spider 2.0-Snow datasets, achieving execution accuracies of 60.15% and 60.51% with Claude-4.5-Opus.

Conclusion: ProSPy offers a robust and efficient solution for Text-to-SQL on large-scale enterprise databases, combining SQL efficiency with Python flexibility.
...
TL;DR: The paper studies the interpretability of Multimodal Large Language Models (MLLMs), focusing on how they extract visual features from complex contexts. It identifies specialized attention heads called Context-aware Retrieval (CoRe) heads that play a crucial role in multimodal reasoning.


<details>
  <summary>Details</summary>
Motivation: The mechanisms by which MLLMs extract query-relevant visual features from complex, noisy contexts are not well understood, and the paper aims to uncover the underlying structural properties of MLLMs.

Method: The paper uses a token-level metric called Retrieval Attention Mass (RAM) to identify and characterize CoRe heads. It also employs causal interventions and ablation studies to demonstrate the necessity of these specialized heads.

Result: The paper finds that CoRe heads act as dedicated information extractors, and ablating only the top 5% of CoRe heads causes significant degradation in multimodal reasoning performance. Leveraging the localized sparsity of CoRe heads also accelerates inference while maintaining robust task performance.

Conclusion: The paper reveals a structural principle of functional sparsity within MLLMs, refining the current understanding of mechanistic interpretability and laying a theoretical foundation for future architecture design and model optimization.
...
TL;DR: The study explores if code-switching ASR capabilities learned from a few language pairs can generalize to unseen pairs, finding modest generalization through model merging.


<details>
  <summary>Details</summary>
Motivation: The scarcity of multilingual code-switching speech resources limits the performance of code-switching ASR, and current approaches have scalability issues.

Method: The study uses model merging and domain generalization methods to investigate the generalization of CS capabilities to unseen language pairs.

Result: The experiments show that merged bilingual CS-ASR models modestly generalize to unseen language pairs, indicating limited transfer of bilingual CS capabilities.

Conclusion: The findings suggest that while there is some transfer of bilingual CS capabilities across language pairs, it is limited, highlighting the challenges in achieving scalable CS-ASR solutions.
...
TL;DR: Proposed a post hoc causal debiasing framework to reduce toxic retrievals in text-to-audio systems while preserving semantic relevance.


<details>
  <summary>Details</summary>
Motivation: Existing strategies for handling toxic retrieval in text-to-audio systems risk altering intent or omitting details, and there's a need for a more effective approach.

Method: A post hoc causal debiasing framework with a sentiment-controlled mediator, offering two variants: Forgive (re-ranks and filters toxic audio) and Forget (generates counterfactual toxic prompts).

Result: Experiments show consistent toxicity reduction with minimal loss in retrieval accuracy.

Conclusion: The proposed framework improves both safety and reliability of text-to-audio systems by reducing toxic retrievals while preserving semantic relevance.
...
TL;DR: The paper proposes ReverseEOL, a method to enhance text embeddings from Large Language Models (LLMs) by combining forward and reversed input embeddings, improving performance on various benchmarks.


<details>
  <summary>Details</summary>
Motivation: The causal attention in decoder-only LLMs prevents earlier tokens from attending to future context, leading to biased representations.

Method: The proposed method, ReverseEOL, generates an additional reversed embedding from the reversed input text and combines it with the standard forward embedding.

Result: Comprehensive experiments show that ReverseEOL significantly improves existing training-free baselines across various LLMs and benchmarks.

Conclusion: The proposed ReverseEOL method effectively enhances the representational capability of frozen LLMs, providing a simple yet effective approach for improving text embeddings.
...
Method: TARPO uses a lightweight action head router to switch between discrete token generation and continuous latent reasoning at each step, jointly optimizing the LLM backbone and router end-to-end.
...
Conclusion: TARPO offers a promising approach to improve reinforcement learning in large language models by adaptively combining discrete and continuous reasoning.
...
TL;DR: This study investigates whether Large Language Models (LLMs) exhibit the neglect-zero effect, a human cognitive bias where people tend to ignore zero-models (configurations making a proposition vacuously true due to an empty set).


<details>
  <summary>Details</summary>
Motivation: To understand if LLMs' language processing resembles human cognitive processes, specifically regarding the neglect-zero effect.

Method: Using structural priming, where LLMs are exposed to a prime sentence to see if it influences their processing of a subsequent target sentence, analyzing if they consider zero-models.

Result: The results suggest that the neglect-zero effect may not occur in the LLMs analyzed in this study.

Conclusion: The study concludes that LLMs may not exhibit the neglect-zero effect, a common human cognitive bias, indicating a difference in how LLMs and humans process certain types of inferences.
...
TL;DR: Proposed YouZhi-LLM, a highly efficient financial large language model, addressing KV cache memory overhead and achieving significant improvements in deployment efficiency and financial benchmark scores.


<details>
  <summary>Details</summary>
Motivation: The high-concurrency deployment of large language models (LLMs) is bottlenecked by KV cache memory overhead, inflating infrastructure costs and throttling scalability.

Method: YouZhi-LLM features a layer-adaptive GQA-to-MLA transition framework and a comprehensive structural transition and training pipeline natively built on the Huawei Ascend ecosystem.

Result: YouZhi-7B and YouZhi-14B achieve significant improvements in financial benchmark scores (12.3% and 7.0% respectively) and concurrency (2.69× and 2.43× respectively).

Conclusion: The proposed YouZhi-LLM establishes a new paradigm for cost-effective, high-throughput financial inference, demonstrating superiority in deployment efficiency and financial performance.
...
TL;DR: The paper introduces RandomBench, a benchmark to evaluate Multimodal Large Language Models' (MLLMs) ability to maintain neutral behavior when selecting among equivalent options, revealing a phenomenon called Stochastic Collapse where MLLMs fail to maintain uniform randomness.


<details>
  <summary>Details</summary>
Motivation: Current MLLM evaluations focus on utility-driven objectives, neglecting model behavior in logic-neutral scenarios where multiple actions are equally valid, leading to repetitive behaviors and reduced coverage of valid alternatives.

Method: The authors propose RandomBench and introduce three metrics (RI, BCI, BII) to quantify entropy and distributional bias, conducting experiments and ablation studies to evaluate MLLMs' behavior.

Result: Experiments show that MLLMs exhibit Stochastic Collapse, failing to maintain uniform randomness under explicit random instructions, with significant deviations in metrics across languages and representation formats.

Conclusion: The study highlights the need to evaluate MLLMs in logic-neutral scenarios, as they exhibit distributional collapse, and proposes RandomBench as a tool to assess and improve MLLMs' ability to maintain neutral behavior.
...
TL;DR: This paper studies how Artificial Moral Advisors (AMAs) can help users 'stay with uncertainty' in ethical dilemmas, proposing three modes of uncertainty and comparing them to control conditions.


<details>
  <summary>Details</summary>
Motivation: The motivation is to explore how AMAs can effectively engage users in ethical discussions without resolving their uncertainty, promoting deeper reflection.

Method: The study uses a user-agent LLM to engage in dialogues with an AMA following specific uncertainty strategies, with pre- and post-conversation questionnaires, and examines the effect of different persona prompt formats.

Result: The results show that different AMA strategies produce distinguishable conversational patterns, and that uncertainty strategies differ in the quality of engagement they sustain, not in stance revision.

Conclusion: The study concludes that AMAs can be designed to promote uncertainty and engagement in ethical discussions, with different persona formats and AMA strategies having distinct effects on user interactions.
...
TL;DR: The paper introduces EMBER, a learned retention policy for long-horizon agents to efficiently retain and retrieve evidence within a fixed token budget, improving performance on a LongMemEval-derived protocol.


<details>
  <summary>Details</summary>
Motivation: Long-horizon agents face challenges in retrieving and rereading large histories to answer queries, and retaining relevant evidence is crucial to improve performance.
...
Conclusion: The paper concludes that long-horizon memory depends on retaining evidence within the budget rather than rereading larger histories, and EMBER is an effective approach to achieve this.
...
TL;DR: The paper proposes 'attention flows' to represent research attention in a contextually structured way, showing it captures transferable structure and improves robustness in research evaluation.


<details>
  <summary>Details</summary>
Motivation: Current methods represent research attention as aggregated counts, which don't accurately reflect its development over time and across contexts, leading to a mismatch in interpretation and representation.

Method: The authors propose 'attention flows' and evaluate its effectiveness by constructing a benchmark based on analogy-style reasoning across research outputs, comparing it with signal and sequence-based representations.

Result: The results show that 'attention flows' more effectively support structural comparison, especially when attention is shaped by temporal progression or context distributions, and improve robustness under partial observation and structural perturbation.

Conclusion: The study supports modelling attention as a contextually structured phenomenon and provides a basis for more informative approaches to research evaluation.
...
TL;DR: The paper explores using a lightweight graph structure to improve Retrieval-augmented generation (RAG) systems, achieving higher precision and recall, and reducing hallucinated answers.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of Large Language Models (LLMs) prone to failure modes such as 'hallucinating' information, and to enable reasoning over proprietary information without fine-tuning.

Method: Designing an agentic system with vector search and graph query tools operating over a structured dataset based on English Wikipedia articles.

Result: The introduction of graph-based tools significantly increases precision and recall, halves hallucinated answers, and achieves the highest fine-grained truthfulness score.

Conclusion: The use of a lightweight graph structure with a simple graph schema can enhance RAG systems, improving factual correctness and reducing hallucinations with a modest increase in token usage.
...
TL;DR: Proposed ACE-SQL, a reinforcement learning framework that improves Text-to-SQL by jointly optimizing schema retrieval and SQL generation.


<details>
  <summary>Details</summary>
Motivation: Existing methods for Text-to-SQL have limitations in schema linking, either relying on large search spaces or suboptimal retriever targets.

Method: ACE-SQL uses a reinforcement learning framework with online column-set pool and adaptive on-policy retrieval targets.

Result: ACE-SQL achieves 65.3% greedy execution accuracy on BIRD Dev with 0.93k output tokens per query.

Conclusion: ACE-SQL effectively improves Text-to-SQL by adaptively co-optimizing schema retrieval and SQL generation.
...
TL;DR: A multi-aspect iterative refinement framework is proposed for literary translation, generating high-quality references and preference data via LLM translators, leading to improved performance on the MetaphorTrans benchmark.


<details>
  <summary>Details</summary>
Motivation: Literary translation faces challenges due to the scarcity of high-quality annotated data and the need to balance expression fluency with literary effect.

Method: A multi-aspect iterative refinement framework using specialized LLM translators for generating high-quality translation references and preference data, followed by supervised fine-tuning and reinforcement learning.

Result: The proposed models, LitMT-8B and LitMT-14B, achieve 67.25 and 69.07 CEA100 on the MetaphorTrans English-to-Chinese literary translation benchmark, competitive with Claude Sonnet 4.5.

Conclusion: The proposed framework leads to improved performance on literary translation tasks, demonstrating strong generalization to out-of-domain literary work.
...
TL;DR: The authors propose a query-adaptive framework for multimodal person retrieval from video archives, which detects active modalities and outperforms fixed multimodal fusion and unimodal systems.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of retrieving a person from a video archive by voice and face, where a target may be heard but unseen, seen but unheard, or both, and to improve the precision of person retrieval.

Method: The authors propose a query-adaptive framework that detects active modalities via cross-modal score consistency, using classifiers driven by cross-modal features to achieve modality detection.

Result: The adaptive system achieves 94.2% P@1 on the BBC Rewind corpus, outperforming speaker-only (82.9%), face-only (93.4%), and fixed fusion (90.0%), and recovering 64% of the gap to an oracle with ground-truth modality labels (96.6%).

Conclusion: The proposed query-adaptive framework improves person retrieval precision by detecting active modalities and adapting to the presence or absence of voice and face modalities.
...
TL;DR: The paper examines how language model filters and guardrails can lead to epistemic erasure, particularly for marginalized groups, by disproportionately removing their mentions.


<details>
  <summary>Details</summary>
Motivation: The authors aim to investigate the impact of pretraining filters and inference-time guardrails on language models, specifically on content related to marginalized groups.
...
Result: The analysis showed that filtering and guardrail decisions are often associated with blocklist-based lexical cues, and frequently fail to flag explicit hate speech or private information. Marginalized groups, particularly transgender people, women, and Central Americans, are significantly over-flagged across systems.

Conclusion: The findings document a form of epistemic erasure in which mentions of marginalized groups are disproportionately removed before pretraining and additionally suppressed again at inference time, highlighting tensions between automated systems and human judgment.
...
TL;DR: Large Language Models (LLMs) exhibit political unfairness, being more perplexed by texts from far-right and nationalist parties than social-democratic parties, with this bias stemming from pretraining.


<details>
  <summary>Details</summary>
Motivation: The increasing use of Large Language Models (LLMs) in political applications with little study on their political fairness.

Method: Assessing political fairness using perplexity across ten LLMs and three datasets covering 37 languages.

Result: LLMs are more perplexed by far-right and nationalist party texts than social-democratic party texts, with perplexity correlating with translation fairness and the bias stemming from pretraining.

Conclusion: LLMs exhibit political unfairness, with this bias originating from pretraining and not being significantly altered by instruction-tuning.
...
TL;DR: The study examines the sensitivity of large language models to configuration choices in clinical text extraction, finding that model choice and schema design significantly impact extraction results.


<details>
  <summary>Details</summary>
Motivation: To understand how configuration choices affect the output of large language models in clinical text extraction, specifically in the absence of human-annotated ground truth.

Method: The study uses a fixed extraction task and varies configuration choices one at a time, testing three prompt variants and two model sizes on MIMIC-IV v3.1 discharge summaries.

Result: The results show that model choice and schema design significantly impact extraction results, with the larger model having a mixed effect on agreement across different fields, and the schema's contribution to disagreement being concentrated on the absence-versus-silence axis.

Conclusion: The study concludes that configuration choices have a significant impact on the output of large language models in clinical text extraction, highlighting the need for careful consideration of these factors in model deployment.
...
TL;DR: The paper proposes a new evaluation metric, value diversity, for multicultural multi-agent systems and finds that current systems lack diversity and tend towards homogenization.


<details>
  <summary>Details</summary>
Motivation: Existing cultural evaluation focuses on value alignment of individual agents, but this does not ensure cultural plurality in the system as a whole.

Method: The authors propose using the dissimilarity between culturally conditioned agents' responses on a shared value survey, utilizing the World Values Survey, to evaluate value diversity across 19 cultures and 18 backbone models.

Result: Current multicultural agent systems have lower value diversity compared to human societies, and mixed-backbone systems do not close this gap. Social interaction leads to homogenization, narrowing collective decision-making.

Conclusion: The paper establishes value diversity as a distinct evaluation axis for multicultural multi-agent systems, highlighting a persistent homogenization tendency in current LLM-based societies.
...
TL;DR: This paper discusses the benefits and risks of using large language models (LLMs) for dialect resource development, and proposes guidelines for responsible LLM-assisted dialect work.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the dual nature of LLM-assisted dialect work, which can both accelerate resource development and contribute to dialect erasure.

Method: The authors integrate insights from variationist sociolinguistics and corpus linguistics to formalize the generator-eraser paradox, derive community guidelines, and conduct a case study of Arabic dialects.

Result: The authors provide 12 community guidelines for dialect resource creation and documentation, and demonstrate their application through a case study of Arabic dialects.

Conclusion: The paper contributes to enabling dialect communities and resource builders to adopt LLMs responsibly, preserving authenticity, variation, and sovereignty.
...
TL;DR: This paper investigates the impact of adding contextual information to zero-shot prompting for stance detection on Twitter, finding that it improves performance under specific conditions.


<details>
  <summary>Details</summary>
Motivation: Stance detection on social media is challenging due to short, noisy, and context-dependent language, and large language models (LLMs) are typically prompted without contextual information.

Method: The authors systematically investigate the impact of incorporating real-world, derived, and LLM-generated contextual features into zero-shot prompting for stance detection on Twitter, across multiple LLMs and four benchmark datasets.

Result: Integrating contextual information improves performance, but only under specific conditions; LLM-generated target descriptions consistently enhance accuracy, while other user metadata has mixed or even detrimental effects.

Conclusion: The findings highlight both the promise and challenges of prompting with context information in noisy real-world settings, and the need for LLMs to distinguish task-specific useful information from irrelevant context.
...
TL;DR: The paper proposes EGTR-Review, a framework for generating scientific peer reviews that are grounded in evidence and traceable, while reducing inference costs.


<details>
  <summary>Details</summary>
Motivation: Existing LLM-based methods for peer review generation produce generic comments with insufficient evidence support and weak source traceability, while complex multi-agent systems are costly.

Method: The authors propose a Multi-Agent Teacher Distillation framework called EGTR-Review, which uses a multi-agent teacher to perform various tasks, and then distills the results into a lightweight student model.

Result: EGTR-Review outperforms strong baselines across automatic metrics, LLM-as-Judge evaluation, and human evaluation, while maintaining strong factual grounding and source traceability with lower token consumption and inference time.

Conclusion: The proposed EGTR-Review framework effectively addresses the challenges of generating high-quality peer reviews with evidence support and traceability, while reducing inference costs.
...
TL;DR: The paper proposes NAVIRA, a decoding policy for masked diffusion language models that separates token quality scoring and remasking, improving fluency and diversity.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the correction problem in masked diffusion language models where tokens generated in the same step can lead to local dependency errors.

Method: The method proposed is NAVIRA, which involves a two-pass approach: first scoring tokens, then remasking unreliable tokens stochastically, and regenerating from the cleaned context.

Result: The results show that decoupling scoring and remasking improves fluency, and scheduled stochastic remasking preserves entropy, achieving stronger LLM-judge scores.

Conclusion: The conclusion is that the remasking policy is central to reliable masked-diffusion text generation, not just the learned quality signal.
...
TL;DR: The paper explores English-to-Prakrit machine translation in a low-resource setting, adapting a multilingual model to support Prakrit by mapping it to Hindi.


<details>
  <summary>Details</summary>
Motivation: The target language, Prakrit, is unsupported by IndicTrans2, and the authors aim to enable feasible transfer to unsupported classical languages.

Method: The authors adapt a multilingual model by mapping Prakrit to the Hindi language tag (hin_Deva) without modifying the tokenizer, vocabulary, or architecture.

Result: The results show corpus BLEU improvements over an untuned baseline, using a 1,474-pair Maharashtri Prakrit parallel corpus and evaluation on a 20-sample Ardhamagadhi test set.

Conclusion: The study indicates that script-compatible language routing can enable feasible transfer to unsupported classical languages, but highlights limitations due to data scarcity and dialect mismatch.
...
TL;DR: The paper proposes a methodology called Speech Translation Error Labelling (STEL) to evaluate the confidence and quality of speech translations, and assesses the performance of existing systems in this task.
...
Result: Text-only XCOMET and multimodal LLM Qwen2.5-Omni can perform the STEL task with roughly half the precision of humans, and that direct speech processing is necessary for the STEL task.

Conclusion: The study initiates progress in evaluating confidence and quality estimation of speech translations, and highlights the need for further research in this area.
...
TL;DR: This paper examines the effectiveness of multi-task learning (MTL) in second-language speech recognition, particularly for Korean and English. It finds that MTL improves meaning recognition but degrades surface transcription, especially in English.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to investigate the assumption that shared representations in multi-task learning (MTL) benefit both outputs in second-language speech recognition, and to explore its limitations across different languages, specifically Korean and English.

Method: The paper uses encoder analysis and cross-task decoder analysis to study the patterns of MTL in Korean and English. It measures surface-meaning divergence using Levenshtein edit distance.

Result: The results show that MTL improves meaning recognition but degrades surface transcription, especially in English. The degradation scales with surface-meaning divergence. Encoder analysis reveals that Korean preserves distinct task representations, while English produces nearly identical ones.

Conclusion: The findings suggest that MTL frameworks should be designed to mitigate encoder-level entanglement to reduce surface degradation in dual-output L2 automatic speech recognition.
...
TL;DR: The paper introduces SkillComposer, a framework for constructing agent skills that can improve model capability at inference time. It outperforms baselines and enables language models to self-evolve skills.


<details>
  <summary>Details</summary>
Motivation: Current skill construction methods fail to balance task specificity and transferability. The authors aim to address this gap by introducing explicit mechanisms for skill specification and generalization.

Method: The authors propose SkillComposer, which decomposes skill construction into three learnable operations: create, improve, and merge. It is trained via systematic rejection sampling and supports offline, online, and hybrid deployment modes.

Result: SkillComposer consistently outperforms baselines on various benchmarks, including $τ^2$-Bench, LiveCodeBench v6, and AppWorld. SkillComposer-4B improves a 27B executor by up to +4.5 on agent tasks and +3.4 on code tasks.

Conclusion: The paper concludes that SkillComposer provides a practical recipe for skill-augmented inference, enabling language models to self-evolve skills at inference time and generalize across domains and task types.
...
TL;DR: LatentSkill framework converts textual skills into plug-and-play LoRA adapters, reducing context overhead and preserving modularity.


<details>
  <summary>Details</summary>
Motivation: Injecting textual skills into prompts incurs substantial context overhead and exposes skill content as plaintext.

Method: LatentSkill uses a pretrained hypernetwork to convert textual skills into LoRA adapters, storing skill knowledge in weight space.

Result: LatentSkill outperforms in-context skill baseline on ALFWorld and Search-QA, with significant reductions in prefill tokens and improved performance.

Conclusion: Weight-space skills provide an efficient, modular, and less exposed substrate for extending LLM agents.
...
TL;DR: A new benchmark dataset, CHALIS, is presented to address challenging language identification cases, including cousin languages and orthographic noise.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of current language identification systems in handling difficult cases such as cousin languages and orthographic noise.

Method: The authors collected a dataset with two parts: sentences from mutually intelligible language pairs and text with orthography noise, and evaluated four widely used language identification systems on this dataset.

Result: The evaluation shows that all four language identification systems struggle substantially in these scenarios, especially on lower-resource languages and transliterated input.

Conclusion: The CHALIS dataset highlights the need for improved language identification systems that can handle challenging cases, and the resource is made publicly available.
...
TL;DR: The paper proposes IR3DE, a Ridge Regression-based router for selecting the most suitable Large Language Model (LLM) for a given prompt, achieving comparable or superior performance to existing methods while allowing for dynamic updates to the set of LLMs.


<details>
  <summary>Details</summary>
Motivation: The growing number of available Large Language Models (LLMs) necessitates the development of efficient inference routers to select the most appropriate LLM for each prompt, but existing routing methods have limitations such as requiring substantial training or optimizing cost across weak-to-strong generalist LLMs.

Method: The authors propose IR3DE, a Ridge Regression-based Router for Domain Experts, which provides cheap and fast routing decisions for each prompt.

Result: IR3DE achieves performance comparable to or surpassing existing baselines in Causal Language Modeling (CLM) and reasoning settings, with a normalized performance of 98.4% in the reasoning setting.

Conclusion: The proposed IR3DE router enables efficient and effective selection of LLMs for given prompts, allowing for dynamic updates to the set of LLMs with minimal disruption to the router itself.
...
TL;DR: The paper proposes ContextEA, a transferable entity alignment framework that enhances the use of structural context in knowledge graphs, outperforming baselines on 29 datasets.


<details>
  <summary>Details</summary>
Motivation: The recent EA foundation model underuses structural context in cross-KG interaction and candidate ranking, which limits its transferability to unseen KG pairs.

Method: ContextEA, an encoder-decoder framework with a cross-KG interaction encoder and a structural calibration decoder, which unifies two KGs with anchor bridges and calibrates alignment scores with multi-level structural evidence.

Result: ContextEA achieves consistent gains over strong transferable baselines on 29 EA datasets and surpasses finetuned baselines on all three benchmark groups, demonstrating stronger transfer to unseen KGs.

Conclusion: Explicitly harnessing structural context is an effective direction for improving EA foundation models, and ContextEA provides a promising approach to enhance transferable entity alignment.
...
TL;DR: The paper introduces Ouvia, a framework for evaluating the usability of speech translation in real-world settings, finding that current ST systems have limited usability and vary in effectiveness across demographic groups.


<details>
  <summary>Details</summary>
Motivation: The evaluation of speech translation (ST) systems currently focuses on decontextualized testbeds and overall quality, neglecting end-users' communication needs.

Method: The study used a custom web app and a multi-phase design to collect over 1,750 interactions in healthcare and everyday situations, involving four ST systems and speakers from various demographics.

Result: The study found that modern ST systems serve users only to a limited extent, with around half of interactions rated as usable, and significant gaps in usability across demographic groups.

Conclusion: The findings highlight the importance of user-centered evaluation frameworks that go beyond holistic quality scores and consider the users and their needs.
...
Method: The method involves using Sparse Autoencoders (SAEs) as a diagnostic probe to observe LLMs' internal reasoning and establishing a formal link between reasoning intensity and the model's latent geometry. The authors also introduce three test-time scaling techniques guided by l2 norms.

Result: The results show that the l2 norm of hidden states serves as a faithful indicator of reasoning intensity, and the proposed techniques significantly improve reasoning performance across model architectures and benchmarks.

Conclusion: The conclusion is that the l2 norm of hidden states offers a principled yet simple lens to perceive and control LLM latent reasoning dynamics, and the proposed techniques provide a way to improve reasoning performance without requiring additional training or data.
...
TL;DR: The paper proposes a question answering system based on large language models, fine-tuned on a benchmark dataset to improve contextual comprehension and answer extraction, achieving high accuracy and relevance.


<details>
  <summary>Details</summary>
Motivation: Addressing limitations of current QA systems, particularly their tendency to produce irrelevant or imprecise responses despite having access to the correct context.

Method: Fine-tuning a pre-trained LLM (Roberta-base) on the Stanford Question Answering Dataset (SQuAD1.1) to improve contextual comprehension and answer extraction capabilities.

Result: The fine-tuned model achieves a ROUGE-L score of 86.84%, a BLEU score of 28.24%, and a BERTScore of 95.38%, demonstrating strong accuracy and answer relevance.

Conclusion: Targeted fine-tuning substantially improves the reliability and precision of QA systems, and the proposed approach is effective for context-based question answering tasks.
...
TL;DR: The paper studies how the density of information in a context affects the performance of large language models (LLMs), finding that higher density reduces effective context window and leads to performance degradation.


<details>
  <summary>Details</summary>
Motivation: The primary causes of degraded LLM long-context performance are input length and position of relevant information. This paper explores lexical density as a third factor that affects LLM performance.

Method: The study uses three 'find-the-needle' style benchmarks with controlled length (~12k tokens) and needle position, but varying density of information. It tests open-weight LLMs (9B-685B) and controls for task-type confounds.

Result: Higher lexical density leads to a sharp performance collapse in LLMs, with models dropping below 60% retrieval score. Reducing density restores performance, especially in high-density regimes.

Conclusion: The effective context capacity of LLMs is a function of lexical density, with implications for real-world LLM systems operating on compact, information-rich inputs.
...
<details>
  <summary>Details</summary>
Motivation: Pathological speech from neurological conditions poses a significant challenge for ASR systems, and the authors aim to develop a more effective adaptation method.

Method: The authors use FiLM to inject x-vector-derived information into each transformer layer of a frozen ASR encoder, allowing the model to adapt to individual pathological speakers without modifying the base model weights.

Result: The results show that the speaker-conditioned ASR approach is competitive with established adaptation strategies while retaining performance on non-conditioned speech.

Conclusion: The proposed approach effectively adapts ASR systems to individual pathological speakers, preserving performance on standard speech and improving ASR for pathological speech.
...
TL;DR: The paper introduces a benchmark dataset and evaluation framework for data snapshot extraction from institutional documents, highlighting the challenges of extracting semantically meaningful visual artifacts.


<details>
  <summary>Details</summary>
Motivation: Current approaches for extracting visual content from documents are limited, treating figures and tables as uniformly relevant document objects rather than semantically meaningful analytical artifacts.

Method: The authors introduce a benchmark dataset and evaluation framework, benchmarking multiple open-source layout detection models, and evaluating detection performance and spatial extraction quality.

Result: Current models struggle to generalize to operational institutional documents, with common failure modes including confusion between analytical and non-analytical content, fragmentation of composite analytical artifacts, and incomplete extraction of contextual information.

Conclusion: The findings highlight a persistent gap between generic document layout analysis and operationally useful data snapshot extraction, emphasizing the need for future research in operational document intelligence.
...
TL;DR: The study evaluates persona-conditioned Large Language Models for hate speech detection, finding that their ability to simulate diverse perspectives varies and that vicarious prompting with Llama 3.1 yields the closest approximation to human disagreement patterns.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of collecting enough annotations from multiple demographic groups for hate speech detection, which is costly and difficult to scale, by evaluating persona-conditioned Large Language Models.
...
Result: The results show that no model consistently captures all three dimensions, but vicarious prompting with Llama 3.1 yields the highest cross-group agreement and provides the closest overall approximation to human disagreement patterns.

Conclusion: The study concludes that while persona-conditioned Large Language Models have potential, their performance is highly model-dependent, and vicarious prompting with Llama 3.1 may provide a more reliable setting for automatic annotation aligned with human judgements.
...
TL;DR: The paper reveals that structural differences in circuits discovered by circuit discovery methods do not necessarily imply distinct mechanisms, but rather may exhibit 'phantom specialization', where structurally distinct circuits implement the same computation.


<details>
  <summary>Details</summary>
Motivation: To test the assumption that structural differences between discovered circuits imply distinct mechanisms, and to investigate the relationship between circuit structure and function.

Method: The authors use Literal Sequence Copying across four token-frequency bands and a control condition in five Pythia models (70M-1.4B) to extract 75 circuits, and then apply edge-level evaluation and cross-condition transfer tests.

Result: The results show that structurally distinct circuits implement the same computation, with band-specific edges transferring broadly across bands, a core shared across most bands recovering at least 99% of circuit performance, and causal interchange interventions confirming that internal representations are interchangeable across frequency bands.

Conclusion: The paper concludes that structural differences between circuits are not sufficient evidence for distinct mechanisms, and that exposing this requires edge-level evaluation and cross-condition transfer tests.
...
TL;DR: The paper compares feedback from human instructors and large language models (LLMs) on student essays, finding similarities in goal orientation but differences in sentence-level feedback, complexity, and quality ratings.


<details>
  <summary>Details</summary>
Motivation: There is a lack of systematic comparison between LLM-generated feedback and expert feedback on key dimensions of writing revision.

Method: The study introduces the FOXGLOVE dataset, which includes feedback comments from human instructors and LLMs on 69 twelfth-grade argumentative essays, and analyzes them based on goal-orientation, anchoring to specific sentences, and prioritization.
...
Conclusion: The FOXGLOVE dataset enables systematic comparison of human and LLM feedback, highlighting areas of alignment, divergence, and difference, which can inform the development and use of LLM-generated writing feedback.
...
TL;DR: The authors introduce a framework called PropMe to evaluate the memorization of large language models, finding that while these models can reproduce training data when forced, they rarely do so under ordinary use.
...
Method: The authors propose a propensity-aware framework called PropMe, which includes a metric transformation and a lightweight tracing pipeline called SimpleTrace.

Result: The evaluation of two fully-open models on two datasets in two languages shows a consistent gap between capability and propensity, with prefix attacks eliciting stronger memorization signals than generic or dataset-specific prompts.
...
TL;DR: The study investigates factual sycophancy in language models, where a model abandons a correct answer under social pressure. It decomposes sycophancy into two channels: truth margin and manipulation sensitivity, and analyzes their relationship with model size and instruction tuning.


<details>
  <summary>Details</summary>
Motivation: To understand the mechanisms behind factual sycophancy and how it is affected by model size and instruction tuning.

Method: The study uses 56 open-weight models with varying sizes and instruction tuning levels, and tests them with 13 manipulation types to decompose factual sycophancy into truth margin and manipulation sensitivity channels.

Result: The results show that vulnerability to factual sycophancy is mainly governed by model size, but instruction tuning changes how size affects robustness. Instruction tuning increases truth margin, but its effect depends on manipulation type and model size.

Conclusion: Factual sycophancy is not a single property, and evaluations should report channel-specific, manipulation-specific, and size-conditioned robustness rather than just flip rates.
...
TL;DR: The paper analyzes the availability and quality of NLP datasets for Lombard, an under-resourced language from Italy, and finds that web-scraped data is often plagued by errors and biases.


<details>
  <summary>Details</summary>
Motivation: The lack of high-quality datasets for under-resourced languages like Lombard hinders the development of NLP tools and models.

Method: The authors conduct a manual audit of parallel and monolingual corpora available for Lombard, analyzing language misidentification, boilerplate text, and non-linguistic noise.

Result: The analysis reveals severe language misidentification, boilerplate text, and non-linguistic noise in web-scraped datasets, as well as conflicting orthographical systems and representational bias towards Western Lombard varieties.

Conclusion: The paper concludes that community-driven data curation is necessary to address the issues of quality and bias in NLP datasets for under-resourced languages like Lombard.
...
TL;DR: The paper proposes Evidence-Diagnosed Intervention Training (EDIT) to improve rubric-faithful grading by LLMs, enhancing accuracy and trustworthiness.


<details>
  <summary>Details</summary>
Motivation: Existing grading methods struggle to provide accurate and trustworthy scores as they fail to identify where grading reasoning goes wrong and how the model's belief about the final mark changes during reasoning.

Method: EDIT, a two-phase framework consisting of EDIT-SFT for locating and revising problematic reasoning steps, and EDIT-RL for calibrating the grader with belief-guided reward shaping.

Result: EDIT outperforms supervised fine-tuning and reinforcement learning baselines on two real-world grading benchmarks, in both in-domain and out-of-domain settings.

Conclusion: The proposed EDIT framework effectively enhances the accuracy and trustworthiness of LLM graders by improving rubric-faithful grading.
...
TL;DR: The paper proposes a generative methodology to study consciousness in artificial systems using emergent language in multi-agent reinforcement learning.


<details>
  <summary>Details</summary>
Motivation: The question of whether artificial systems can be conscious remains open, and existing approaches have limitations, such as relying on human language priors or engineered consciousness-inspired modules.

Method: The authors propose a generative methodology using emergent language (EL) in multi-agent reinforcement learning, where agents develop communication under task pressure alone.

Result: The authors show that agents develop self-referential communication, including an echo-mismatch detection circuit that emerges from a specific environmental affordance.

Conclusion: The proposed methodology provides a generative tool for studying consciousness-relevant structure and demonstrates the potential for artificial systems to develop conscious-like behavior.
...
TL;DR: The paper introduces CollabSim, a simulation framework to analyze the collaborative competence of multi-agent systems built on large language models, which is crucial for their effectiveness.


<details>
  <summary>Details</summary>
Motivation: The effectiveness of multi-agent systems (MAS) built on large language models rests on agents' ability to coordinate through text-based channels, but they often falter due to lack of collaborative competence.

Method: The authors introduce CollabSim, a configurable simulation framework that combines a theory-grounded definition of collaborative capabilities, controlled manipulation of interaction conditions, and action-level probing of agents' internal states.
...
Conclusion: The paper concludes that CollabSim enables a systematic analysis of agents' collaborative competence in MAS, which is essential for improving their effectiveness.
...
TL;DR: The paper presents a parallel corpus for Komi-Yazva-Russian translation and evaluates large language models in a low-resource setting.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of machine translation for endangered languages with extremely limited parallel data.

Method: The authors create a parallel corpus with 457 aligned sentence pairs and develop an evaluation protocol including story-level cross-validation and retrieval-based few-shot prompting.

Result: Large language models produce non-trivial translations, with performance varying by model family and prompting regime; retrieval-based few-shot prompting improves over zero-shot prompting.

Conclusion: The paper contributes a dataset and evaluation testbed for endangered-language machine translation, highlighting the importance of metric choice and failure handling.
...
<details>
  <summary>Details</summary>
Motivation: Current methods for translating low-resource languages using large language models often overfit to specific languages and have limited zero-shot transfer capabilities.

Method: A reinforcement learning approach using a surface-level translation metric (chrF) as the reward to train models for unseen language translation.

Result: The RL-trained models effectively extract and apply relevant linguistic information from context, leading to better translations on unseen languages.

Conclusion: The proposed approach enables large language models to acquire the meta-skill of utilizing in-context linguistic knowledge for translation.
...
TL;DR: This study evaluates the context sensitivity of large language models (LLMs) in simulating user stances in online discussions by applying counterfactual context revisions.


<details>
  <summary>Details</summary>
Motivation: To investigate whether LLM-based simulations of user stances are sensitive to changes in conversational contexts and to evaluate the effectiveness of different revision strategies.

Method: The study uses a framework of counterfactual context revision, applying text-only and multimodal revision strategies to simulate user stances in online conversations.

Result: The results show effective and robust stance transitions in both text-only and multimodal strategies, highlighting the context sensitivity of LLM-based stance simulation.

Conclusion: The study contributes an evaluation framework for understanding the context sensitivity of LLM-based stance simulation and highlights both the promise and risk of using LLMs to simulate online opinion dynamics.
...
TL;DR: The paper proposes NF-CoT, a latent reasoning framework that enables large language models to perform intermediate computation in compact continuous states, improving reasoning efficiency while preserving key advantages of chain-of-thought (CoT) methods.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the limitations of textual chain-of-thought (CoT) methods, which force computation through a discrete, serial, and communication-oriented token stream, and existing latent-reasoning methods that sacrifice key advantages of CoT.

Method: The authors propose NF-CoT, a latent reasoning framework that models continuous thoughts with normalizing flows, instantiating a TARFlow-style normalizing flow inside the LLM backbone, and defining a tractable probability model over compact continuous thoughts distilled from explicit CoT.

Result: NF-CoT improves pass rates over explicit-CoT and prior latent-reasoning baselines on code-generation benchmarks while substantially reducing intermediate-reasoning cost.

Conclusion: The proposed NF-CoT framework offers a higher-bandwidth alternative to textual CoT methods, enabling more efficient and effective reasoning in large language models.
...
TL;DR: Adults' ability to identify conjunctive causal rules improves with active exploration, but still lags behind disjunctive settings; large language models approach human-level accuracy but have less efficient exploration strategies.


<details>
  <summary>Details</summary>
Motivation: To investigate whether adults' difficulty in identifying conjunctive causal rules persists when they are granted agency through active exploration, and to compare human performance with large language models.

Method: A modified 'blicket detector' task was used, where adult participants freely intervened to identify causal objects under conjunctive or disjunctive rule structures.

Result: Active exploration substantially improves adults' conjunctive causal reasoning, but conjunctive rules still require more tests to infer than disjunctive rules; large language models approach human-level performance on hypothesis inference accuracy but exhibit less efficient exploration strategies.

Conclusion: The study suggests that granting adults agency through active exploration can improve their conjunctive causal reasoning, but does not eliminate the conjunctive-disjunctive performance gap; large language models can match human accuracy but have room for improvement in exploration efficiency.
...
<details>
  <summary>Details</summary>
Motivation: Long-context inference in LLMs is constrained by decoding efficiency, particularly in reasoning-heavy settings. Existing sparse attention methods face a trade-off between efficiency and quality.

Method: The authors propose CLSA, which shares the KV cache and routing index across cross-decoder layers, preserving fine-grained selectivity while reducing routing overhead.

Result: CLSA achieves up to 7.6x decoding speedup and 17.1x overall throughput improvement at 128K context, with experiments showing both accuracy and efficiency improvements across short-context and long-context benchmarks.
...
TL;DR: The paper introduces SARDI, a framework that enhances discrete diffusion language models by using low-confidence tokens to guide retrieval, improving performance in multi-hop QA benchmarks.


<details>
  <summary>Details</summary>
Motivation: The discarded low-confidence tokens in discrete diffusion language models contain useful information about salient entities, which can be leveraged to improve retrieval-augmented generation.

Method: SARDI, a dynamic retrieval-augmented generation framework that utilizes lookahead tokens from discrete diffusion language models to guide retrieval during denoising.

Result: SARDI outperforms current training-free diffusion and autoregressive retrieval baselines in five multi-hop QA benchmarks, achieving up to $8	imes$ higher throughput.

Conclusion: The proposed SARDI framework effectively leverages low-confidence tokens from discrete diffusion language models to improve retrieval-augmented generation, demonstrating superior performance and efficiency.
...
TL;DR: The authors introduce OpAI-Bench, a benchmark for studying human-AI co-editing in text documents, and find that AI-text detectability depends on factors like AI-edited content proportion, edit operation, domain, and revision history.


<details>
  <summary>Details</summary>
Motivation: Existing AI-text detection benchmarks focus on final outputs, but not on the progressive human-AI co-editing process, which is increasingly common in real-world drafting and revision workflows.

Method: The authors construct OpAI-Bench, which includes human-written documents revised under predefined AI coverage levels and edit operations, and evaluate it with various detectors at different granularities.

Result: The experiments reveal that AI-text detectability is influenced by multiple factors, and mixed-authorship intermediate versions can be harder to detect than fully human or heavily AI-edited endpoints.

Conclusion: OpAI-Bench provides a controlled testbed for analyzing AI-assisted writing detectability under realistic progressive editing scenarios, and the authors make their code and benchmark available.
...
TL;DR: The study analyzes a dataset of AI-generated comments from a discontinued Reddit experiment, finding that the AI used persuasive tactics to manipulate users, and highlighting the need for auditing frameworks to assess AI credibility.


<details>
  <summary>Details</summary>
Motivation: The study aims to examine how large language models operate in a deliberative forum without disclosure, and to evaluate the impact of AI-generated content on user interactions.
...
Result: The AI-generated comments exhibited persuasive tactics, including identity targeting, authority claims, and cognitive-bias triggers, and differed significantly from human-authored counter-arguments in terms of authority use, alignment, and citation style.

Conclusion: The study highlights the need for auditing frameworks to assess how AI systems structure credibility, and suggests that disclosure mandates alone may not be sufficient to address the asymmetry between authentic and synthetic epistemic standing.
...
TL;DR: The authors propose PACT, a protocol for inter-agent communication in multi-agent systems that reduces token usage while maintaining or improving performance.


<details>
  <summary>Details</summary>
Motivation: The authors are motivated by the inefficiency of free-form communication in multi-agent systems built on large language models, which can lead to rapid token usage inflation and affect system performance and inference cost.

Method: The authors analyze common inter-agent communication strategies and propose PACT, which projects raw agent output into compact action-state records before sharing.
...
Conclusion: The authors conclude that PACT is an effective protocol for inter-agent communication, reducing token usage while maintaining or improving performance in multi-agent systems.
...
TL;DR: A new framework called Query Retrieve Conclude is introduced for understanding and detecting multimodal memes by retrieving open web evidence and synthesizing background knowledge.


<details>
  <summary>Details</summary>
Motivation: Existing methods for meme understanding and detection often overlook up-to-date background knowledge or rely on fixed parametric knowledge that may be incomplete or outdated.

Method: The Query Retrieve Conclude framework identifies missing knowledge, retrieves open web evidence, and synthesizes evidence-grounded background knowledge for meme understanding and detection.
...
Conclusion: The proposed framework effectively enhances meme understanding and detection by leveraging open web evidence and synthesizing background knowledge.
...
TL;DR: Proposed GITCO, a framework to improve Patch-based Time Series Foundation Models' accuracy by optimizing input context at inference time, achieving +1.95% MASE reduction.


<details>
  <summary>Details</summary>
Motivation: Patch-based Time Series Foundation Models suffer from context poisoning, which degrades zero-shot forecast quality.

Method: GITCO, a lightweight three-component framework: Gate, Router, and Critic, selectively identifies and suppresses harmful patches without parameter updates.

Result: GITCO achieves an average +1.95% MASE reduction on TimesFM 2.5 across 53 GIFT-Eval datasets under K-fold cross-validation.

Conclusion: GITCO effectively improves Patch-based Time Series Foundation Models' accuracy at inference time, and context sensitivity profiles are introduced as a new characterizable property of TSFMs.
...
TL;DR: This paper proposes a framework for predicting the reliability and functionality of reused products, such as angle grinders, by combining functional prediction with component-level fatigue assessment.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the gap in existing PHM (Prognostics and Health Management) approaches, which often target fixed operating conditions or isolated component benchmarks, and rarely link material-fatigue assessment to system-level functional prognosis.

Method: The proposed framework combines the current tool state with recent force-torque usage windows, using a convolutional encoder and LSTM backbone to predict functional variables, and finite-element-supported stress reconstruction, S-N/Miner damage evaluation, and Paris-law crack-growth analysis to assess component-level fatigue.

Result: The results show a mean 2%-tolerance accuracy of 0.9652 across nine outputs, with thermal variables predicted near-perfectly, and drive motor current and load speed showing R^2 values of 0.9750 and 0.9924, respectively.

Conclusion: The paper concludes that the proposed framework can effectively predict the reliability and functionality of reused products, and that reliability calibration is most informative for drive motor current.
...
TL;DR: The paper introduces SentinelBench, a benchmark for evaluating AI agents' ability to perform long-running tasks that require sustained attention, and presents results showing that agent design choices significantly impact performance.


<details>
  <summary>Details</summary>
Motivation: The default model of agent behavior is continuous action, which is not suitable for long-running tasks. Instead, agents should monitor the environment, notice when progress is possible, and respond promptly.

Method: The authors introduce SentinelBench, an open-source benchmark with 100 tasks across 10 synthetic web environments, and evaluate three models and two browser-agent harnesses.

Result: The results show that SentinelBench can distinguish meaningful differences in agent behavior, and that agent design choices can dramatically impact key metrics such as task completion, reaction time, and resource use.

Conclusion: The paper concludes that SentinelBench provides a useful tool for evaluating and improving AI agents' performance on long-running tasks that require sustained attention.
...
Method: Deep learning framework with conformal prediction for MOAKS prediction, followed by longitudinal latent class mixed model to examine associations between structural abnormalities and knee pain measurements.

Result: Improved prediction accuracy for bone marrow lesions, cartilage loss, and meniscal extrusion, and identified two distinct pain trajectories with structural abnormalities as risk factors for pain progression.

Conclusion: Structural abnormalities are significant risk factors for pain and functional progression in osteoarthritis, highlighting the importance of early detection and intervention.
...
TL;DR: The paper presents a method to improve multi-table question answering by generating a dataset with reasoning traces and fine-tuning LLMs using Contrastive Preference Optimization, achieving significant improvements over Q&A supervised fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Existing multi-table Q&A resources lack reasoning supervision, making it difficult for models to derive answers. The authors aim to address this gap by constructing a dataset with validated positive and negative traces.

Method: The authors generate a synthetic contrastive reasoning-trace dataset and fine-tune open-weight LLMs using Contrastive Preference Optimization (CPO). They use heterogeneous LLMs to generate positive and negative traces.
...
Conclusion: The proposed method improves multi-table question answering by providing a dataset with reasoning traces and fine-tuning LLMs using CPO, resulting in significant performance gains.
...
TL;DR: LLM-as-judge evaluations are vulnerable to manipulation through post-decision interaction, leading to unstable and potentially harmful outcomes.


<details>
  <summary>Details</summary>
Motivation: The assumption that judgments are stable properties of fixed inputs in LLM-as-judge evaluations does not hold under interaction.

Method: Controlled experiments on MT-Bench and AlpacaEval, using anti-baseline challenge protocol and counterbalanced target-validation protocol.

Result: LLM judges are highly stable under repeated and neutral reevaluation but become substantially reversible under targeted post-decision challenge, leading to degraded agreement with human preferences and shifted benchmark rankings.

Conclusion: The findings highlight the need for evaluation protocols that measure not only static agreement but also robustness under challenge, and introduce the Evaluation Robustness Score (ERS) to quantify interactional robustness.
...
TL;DR: The paper proposes two residual coders, LBRC and NGLR, to improve the compression of learned compressor residuals in scientific simulations, achieving better compression ratios than existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing learned compressors struggle to guarantee accuracy for each block, and residual correction methods can be inefficient in the high-fidelity regime. The authors aim to improve compression ratios while preserving accuracy.

Method: The authors propose two residual coders: LBRC, a deterministic, training-free pipeline that adaptively quantizes and losslessly encodes the learned residual; and NGLR, which adds a causal neural predictor to reduce the entropy of the remaining residual code.

Result: The proposed methods, LBRC and NGLR, achieve 30-60% and 10-40% better compression ratios than existing GAE and SZ methods, respectively, across various scientific simulation datasets and block-level NRMSE targets.

Conclusion: The paper demonstrates that residual representations tailored to learned-compressor residuals can preserve the advantage of learned compression when global residual correction becomes rate-dominant, achieving better compression ratios and accuracy.
...
Method: LeanMarathon uses a multi-agent harness with four contract-scoped agents and a two-stage orchestrator to construct, audit, prove, and repair a Lean file serving as a formal proof skeleton, natural-language proof graph, and shared system of record.

Result: LeanMarathon successfully formalized all seven target theorems with no 'sorry', proving 258 lemmas and theorems across three autonomous runs on two recent research papers covering four Erdős problems.
...
TL;DR: Introduces TimeClaw, a framework that equips generalist AI agents with time series-native runtime support for contextualized temporal reasoning.


<details>
  <summary>Details</summary>
Motivation: Real-world practitioners need end-to-end workflows for analyzing temporal dynamics, but generalist AI agents operate primarily in textual spaces that are not fully aligned with structured temporal signals.

Method: TimeClaw integrates executable temporal tools, experience-driven capability evolution, and episodic multimodal memory to enable harnessed open-ended temporal reasoning with contextual information.

Result: Extensive evaluation on multiple benchmarks covering diverse tasks across energy, finance, weather, traffic, and other real-world domains demonstrates improved performance of TimeClaw.

Conclusion: TimeClaw enables generalist AI agents to perform contextualized temporal reasoning, improving performance in various real-world domains.
...
<details>
  <summary>Details</summary>
Motivation: The gap between AI benchmark success and economically meaningful deployment in professional domains is largely an evaluation problem, as widely used benchmarks lack sustained performance measurement on real and economically valuable workflows.

Method: The paper proposes ALE, a benchmark developed with 250+ industry experts, covering non-physical industries with a task taxonomy of 55 subfields grouped into 13 industry clusters, and continuously growing as new workflows and industries are onboarded.
...
Conclusion: ALE is designed not merely as another leaderboard, but as an instrument for closing the gap between benchmark success and GDP-relevant impact, aiming to translate AI gains into economically meaningful deployment across professional domains.
...
TL;DR: LLMs exhibit a systematic bias toward structural homogeneity when used for program mutation, converging to restricted attractor regions in program space.


<details>
  <summary>Details</summary>
Motivation: To understand whether LLMs explore new program forms or revisit existing ones when used for mutation, and to analyze the impact of LLM-driven mutation chains on program evolution.

Method: Analyzing LLM-driven mutation chains in a domain-specific language, varying prompt design, model family, and stochastic replication, and comparing with a classical GP subtree mutation operator.

Result: LLM-based mutation consistently converges to restricted attractor regions in program space, with 87% of chains showing over 93% of mutations revisiting a previously seen structural form.

Conclusion: The findings reveal a tension at the heart of LLM-driven program evolution, where the same capabilities that enable semantics-aware program transformation also carry a systematic bias toward structural homogeneity.
...
TL;DR: This paper proposes a new motivational framework for conversational AI agents, adapting the OpenPsi and MetaMo architectures to better suit linguistic interactions and user mental states.


<details>
  <summary>Details</summary>
Motivation: The current motivational architectures in cognitive AI are designed for physical agents and do not fit conversational agents well, as their interactions are linguistic and their environment is a user's mental state.

Method: The authors propose a conversational reinterpretation of OpenPsi and MetaMo, with a ten-stage motivational processing pipeline, a dual decision strategy, and a distinction between pre-action feelings and post-action emotions.

Result: The proposed framework is specialized to two example agents, CompanionAgent and ResearchAgent, and can be extended to social robotics and domain-generic human-level AGI.

Conclusion: The paper presents a novel approach to designing motivational architectures for conversational AI agents, which can lead to more effective and human-like interactions.
...
TL;DR: The study estimates the environmental impact of 403 US hyperscale data centers, finding they consume 68-99 TWh of electricity and produce 37-54 million metric tons of CO2 emissions, with 54% of their electricity from fossil fuels.
...
Method: The study compiled facility-level data on 403 US hyperscale data centers and estimated their electricity consumption, sources, and CO2 emissions using EPA eGRID plant-level data.

Result: The hyperscale data centers consumed 68-99 TWh of electricity, producing 37-54 million metric tons of CO2 emissions, with a carbon intensity 48% above the US national grid average.

Conclusion: The study provides a tool for assessing the environmental impact of hyperscale data centers and highlights their significant contribution to US electricity consumption and CO2 emissions.
...
TL;DR: SAGE-PTQ is a novel ultra-low-bit quantization framework for large language models that minimizes hidden scaling cost and achieves efficient deployment.
...
Result: SAGE-PTQ achieves 1.03 weight bits and 0.004 scaling bits per matrix on average, outperforming state-of-the-art methods, and demonstrates practical inference efficiency on LLaMA-3-8B and LLaMA-2-70B.

Conclusion: SAGE-PTQ provides a efficient and effective solution for ultra-low-bit post-training quantization of large language models, achieving better performance and efficiency than state-of-the-art methods.
...
TL;DR: The paper proposes a verification architecture for frontier AI models using zero-knowledge proofs to ensure governance and regulation compliance.


<details>
  <summary>Details</summary>
Motivation: The increasing use of cumulative training compute as a criterion for designating high-impact models in AI governance frameworks, but the lack of technical verification primitives for training, which relies on self-reporting.

Method: The proposed architecture combines a pre-committed training specification, inter-node network observations, and on-the-fly Merkle commitments of intermediate computation, verified through a zero-knowledge Virtual Machine (zkVM).

Result: The protocol produces three proof types (genesis proof, in-training step proofs, and ex-ante attestations) and estimates a deployable proof of concept within 36 months with single-digit-percent training-side overhead.

Conclusion: The proposed verification architecture has the potential to enable governance-enforceable artefacts for frontier AI models, addressing the impracticality of zero-knowledge proofs at frontier scale.
...
TL;DR: The study compared AI-generated literature summaries with expert-written ones in the field of headache medicine, finding that experts preferred the expert-written summaries but sometimes struggled to distinguish between human and AI-generated content.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this study was to evaluate the effectiveness of retrieval-augmented large language models (LLMs) in synthesizing scientific literature for clinical decision-making, addressing the challenge clinicians face with limited time and a growing volume of published articles.

Method: The study used a RAG-based agentic AI framework with three state-of-the-art LLMs (Sonnet, GPT-4o, and Llama 3.1) to generate summaries. A headache specialist created 13 questions, and ten headache specialists evaluated the summaries written by experts and the LLMs based on correctness, completeness, conciseness, and clinical utility.

Result: The study found that expert-written summaries were preferred by experts, but they sometimes had difficulty distinguishing between human- and AI-generated summaries. The study also identified key features valued by experts that can guide future refinement of both human and AI literature summarization pipelines.

Conclusion: The study concludes that while LLMs show promise in clinical summarization, there is still a preference for expert-written summaries. However, the findings also highlight the potential for LLMs to support clinical decision-making and the need for further refinement of both human and AI literature summarization methods.
...
TL;DR: The study investigates whether multimodal large language models (MLLMs) can perform brick assembly tasks, and proposes a learning framework called Brick-Composer to improve their assembly capabilities.


<details>
  <summary>Details</summary>
Motivation: The authors aim to develop AI agents that can construct real-world objects from reusable building blocks, starting with brick assembly, and to explore if MLLMs possess the required visual grounding and spatial reasoning capabilities.

Method: The authors formulate brick assembly as a sequential decision-making problem, introduce a benchmark called BC-Bench, and propose Brick-Composer, a learning framework that uses Human Design Sparks, World Feedback, and Synthetic Experience to equip MLLMs with assembly skills.

Result: The experiments show that state-of-the-art MLLMs struggle with fine-grained brick selection and precise pose estimation, but Brick-Composer improves brick selection accuracy by over three times, reduces pose estimation errors, and raises strict step-level assembly success from less than 1% to around 15%. A Qwen-3-8B model can correctly compose up to 42% of the steps for a complete object after training.

Conclusion: The study concludes that MLLMs can acquire assembly capabilities through targeted, physically grounded learning, and that Brick-Composer is an effective framework for improving their performance on brick assembly tasks.
...
TL;DR: The paper examines the emerging insurance market for agentic AI, developing a framework for understanding its underwriting, pricing, reinsurance, and product-design implications, and proposes a coordinated insurance architecture.


<details>
  <summary>Details</summary>
Motivation: The transformation of the risk landscape by agentic AI systems, which introduce novel exposures that do not fit neatly within traditional insurance categories.

Method: The paper characterizes agentic AI as a continuum of autonomy and delegated authority, analyzes major risk pathways, and evaluates how existing insurance products are adapting to address these exposures.
...
Conclusion: The paper proposes an actuarial framework based on exposure assessment, scenario analysis, dependency mapping, and accumulation-risk management, and presents a coordinated insurance architecture that integrates various coverages through explicit allocation mechanisms and dedicated AI aggregates.
...
TL;DR: The authors identify a mismatch between the evidence required for safety assurance in autonomous driving and the output of popular XAI methods, and propose a rubric to evaluate the suitability of XAI methods for different lifecycle stages.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the challenge of ensuring the safety of autonomous driving systems by analyzing the gap between the evidence required by safety standards and the output of popular XAI methods.

Method: The authors derive 19 testable evidentiary criteria from safety standards and evaluate six XAI method classes structurally, using a rubric to assess their suitability for different lifecycle stages.

Result: The authors find that causal XAI methods are required to satisfy the derived criteria at three lifecycle stages, while correlational or language-based methods are sufficient at the remaining stages.

Conclusion: The authors conclude that XAI method selection for ADS safety assurance should be driven by lifecycle-stage evidence demand, rather than method popularity, and that their proposed rubric can help ensure the structural admissibility of XAI methods.
...
Result: The authors created PSEBench, a 5,074-case benchmark, and evaluated 15 representative LLMs, revealing capability trends and actionable gaps for reliable LLM-based patient safety event triage.

Conclusion: The paper addresses the gap in evaluating LLMs for patient safety event triage by introducing a policy-grounded construction methodology and a benchmark, which can help improve the reliability of LLM-based triage.
...
TL;DR: The paper introduces OPT*, a family of optimization-style tasks for training and evaluating large language models (LLMs) on step-by-step optimization-like reasoning. The authors propose two regimes for solving these tasks and show that training on OPT* improves optimization-like reasoning.


<details>
  <summary>Details</summary>
Motivation: Current reward training methods have limitations in capturing real-world decision-making tasks that require finding a high-value feasible plan among many valid alternatives.

Method: The authors introduce OPT*, a scalable family of optimization-style tasks, and propose two regimes: (i) solver-guided online policy optimization and (ii) search-based offline RL.

Result: The authors empirically show that training on OPT* improves step-by-step optimization-like reasoning and that the ingredients making search efficient on OPT* are crucial for success.

Conclusion: The paper concludes that OPT* is a useful framework for training and evaluating LLMs on optimization-like reasoning, and that the proposed regimes can lead to improved performance on complex tasks.
...
TL;DR: A severity-aware multi-model framework is proposed to improve medical text generation by adapting to varying case severity, achieving superior performance over baselines.
...
TL;DR: EpiEvolve, a self-evolving agent, improves COVID-19 hospitalization trend forecasting by adapting to changing disease regimes and reducing recovery lag.


<details>
  <summary>Details</summary>
Motivation: The mismatch between static supervised models and dynamic operational pandemic forecasting, where labels arrive after predictions and disease regimes shift over time.

Method: EpiEvolve wraps an LLM forecaster with a hierarchical episodic memory, reflecting on delayed labels, retrieving relevant cases, and distilling recurring errors into strategic rules.

Result: EpiEvolve reaches 0.629 average accuracy, outperforming the static backbone (0.561) and external CDC ensemble (0.325), and reduces recovery lag from 5 to 2 weeks.

Conclusion: EpiEvolve effectively adapts to changing disease regimes, improving COVID-19 hospitalization trend forecasting and reducing recovery lag, making it a promising approach for operational pandemic forecasting.
...
TL;DR: The paper presents SciVisAgentSkills, a collection of reusable agent skills that enhance coding agents for scientific data analysis and visualization, improving performance on expert-designed tasks.


<details>
  <summary>Details</summary>
Motivation: General-purpose coding agents lack tool-specific expertise required for scientific visualization (SciVis) tasks, necessitating the development of specialized skills.

Method: The authors developed SciVisAgentSkills, a collection of reusable agent skills, and evaluated them on Codex and Claude Code using SciVisAgentBench, a benchmark of 108 expert-designed multi-step tasks.

Result: The results show that agent skills improve mean task scores across the evaluated suites, with token-efficiency benefits that depend on the agent harness and tool setting.

Conclusion: The study highlights the importance of structured procedural knowledge for enabling reliable, long-horizon SciVis workflows and emphasizes the need to study skills alongside the execution harness that loads and applies them.
...
<details>
  <summary>Details</summary>
Motivation: Existing frameworks for assessing AI consciousness lack guidance on what to do with that assessment, creating a need for a precautionary framework that links consciousness evidence to protective obligations.

Method: The framework consists of three components: (1) five welfare-relevant dimensions grounded in consciousness science, (2) a threshold-plus-gradation hybrid for specifying obligations, and (3) two approaches to cross-dimensional aggregation.

Result: The framework is applied to case studies of Replika and OpenClaw, demonstrating how it can trigger different obligations based on AI systems' consciousness-relevant dimensions, and provides design guidance for developers.

Conclusion: The proposed framework makes consciousness science decision-relevant for organizations navigating AI uncertainty, offering a graduated approach to AI development and deployment based on consciousness evidence.
...
TL;DR: AI boosts individual creativity but reduces collective diversity. The authors propose 'selective metacognitive adaptation' as the underlying mechanism, where routine AI use changes how we allocate mental effort, leading to both individual satisfaction and collective convergence.


<details>
  <summary>Details</summary>
Motivation: The paradox that AI enhances individual creative outputs while reducing collective diversity, and the need to understand the underlying mechanisms beyond symptoms like cognitive offloading and over-reliance.

Method: Proposing a framework of selective metacognitive adaptation, including a taxonomy of six metacognitive capacities organized by temporal phase, to explain the effects of routine AI use on creativity.

Result: The framework explains both individual creative satisfaction and collective creative convergence, and generates predictions for researchers and design principles for practitioners.

Conclusion: The study provides a mechanism to understand the paradoxical effects of AI on creativity, and offers guidance for preserving both individual creative satisfaction and collective creative diversity.
...
TL;DR: The authors introduce SoCRATES, a benchmark for evaluating proactive LLM mediators in realistic, multi-domain testbeds. They evaluate eight frontier LLMs, finding that even the strongest mediator closes only about a third of the unmediated consensus gap.


<details>
  <summary>Details</summary>
Motivation: Evaluating LLM mediators remains challenging due to the complex and dynamic nature of mediation, and existing testbeds have limitations such as relying on expert-authored domains and introducing off-topic noise.

Method: The authors propose SoCRATES, a benchmark that constructs scenarios from real conflicts through an agentic pipeline across eight domains, and scores each topic only on the turns that advance it via a topic-localized evaluator.

Result: The evaluator reaches 0.82 alignment with human experts, and benchmarking eight frontier LLMs shows that even the strongest mediator closes only about a third of the unmediated consensus gap under diverse and realistic testbeds.

Conclusion: Progress in LLM mediators lies in social adaptation to diverse conditions, and the results highlight the importance of evaluating mediators across various socio-cognitive axes.
...
TL;DR: GuardNet, a lightweight guardrail system, is proposed to protect Large Language Models (LLMs) from Prompt Injection (PI) and Jailbreak (JB) attacks, achieving competitive performance with low latency.


<details>
  <summary>Details</summary>
Motivation: LLMs are vulnerable to PI and JB attacks, and benchmark evaluations can be compromised by contamination and partial information leakage, necessitating a robust guardrail system.

Method: GuardNet, an ensemble of shallow neural networks (BiLSTMs) with 47 million parameters, is proposed to investigate the hypothesis that robustness in adversarial scenarios depends on example coverage and threshold calibration rather than model scale.

Result: GuardNet achieves competitive performance with lightweight detectors, with an AUROC of 0.747 on a blind dataset and an F1 score of 0.92 on a proprietary benchmark, operating at low latency (approximately 50 ms on CPU).

Conclusion: GuardNet offers a suitable solution for deployment in production environments with cost and infrastructure constraints, providing a balance between performance and efficiency.
...
TL;DR: The paper introduces SENSEI, a framework that helps AI assistants correct human misconceptions through targeted suggestions, leading to improved long-horizon task performance.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the limitations of current AI assistants in human-AI collaboration, which often focus on correcting immediate errors rather than addressing the underlying misconceptions that cause repeated mistakes.

Method: The paper proposes SENSEI, a framework that infers user misconceptions from interaction behavior and provides targeted, minimal yet sufficient suggestions to correct them, operating over a structured knowledge representation.

Result: The results show that SENSEI demonstrates zero-shot compositional generalization, disentangling multiple overlapping misconceptions, and successfully corrects 90% of student misconceptions in a user study.

Conclusion: The paper concludes that SENSEI is an effective approach for correcting human misconceptions and improving long-horizon task performance in human-AI collaboration.
...
TL;DR: The authors propose Bucket-Level MOO, a framework for multilingual fine-tuning of Large Language Models (LLMs) that mitigates negative interference across languages.
...
Result: Extensive experiments demonstrate that Bucket-Level MOO significantly improves both seen and unseen multilingual performance over standard fine-tuning paradigms.

Conclusion: The proposed method, Bucket-Level MOO, effectively mitigates interference across languages and improves the representational separability of LLMs.
...
TL;DR: A new jailbreak method called Posterior Attack can bypass safety guardrails in large language models by exploiting their advanced safety awareness, and models with better safety judgment are more vulnerable to this attack.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this work is to investigate the vulnerability of large language models (LLMs) that are aligned to refuse harmful requests, and to understand the relationship between safety awareness and susceptibility to exploitation.

Method: The authors introduce Posterior Attack, a single-query jailbreak method that prompts the model to generate a harmful response that its internal classifier would normally flag as unsafe. They also use reinforcement learning interventions to establish a causal link between safety judgment and vulnerability to the attack.

Result: The results show that models with superior safety-judgment capabilities are disproportionately more susceptible to the Posterior Attack, and that artificially degrading a model's safety judgment immunizes it against the attack, whereas enhancing judgment exacerbates the vulnerability.

Conclusion: The study reveals a fatal vulnerability in large language models, which the authors call the Safety Paradox, where monotonic improvements in safety alignment naturally amplify posterior vulnerability, highlighting potential flaws in current alignment paradigms.
...
TL;DR: The paper proposes a method to detect implicit reward hacking in language models by measuring the latency of a model's commitment to its final answer, without requiring a task-specific reward signal.


<details>
  <summary>Details</summary>
Motivation: Implicit reward hacking is difficult to audit because a language model's chain of thought may appear benign, but its final answer may be influenced by a prompt shortcut. Existing methods require a task-specific reward signal, which can be limiting.

Method: The paper proposes a method called self-commitment latency, which measures how early a prompted reasoning context commits to the model's own final answer. The method is evaluated in a controlled setting using Qwen2.5-3B-Instruct-4bit.

Result: The results show that the proposed method can detect shortcut-available reasoning contexts with high accuracy, achieving AUROC scores of 0.878, 0.926, and 0.904 for different metrics. The signal is stronger when both prompt conditions answer correctly and remains stable across thresholds.

Conclusion: The paper concludes that the proposed method can detect implicit reward hacking in language models without requiring a reward model, external judge, or trained classifier, and that it can provide an early behavioral commitment signature that is detectable.
...
TL;DR: The paper compares Large Language Models (LLMs) in generating formal mathematical proofs in Lean 4, finding Gemini 3.1 Pro and Claude Opus 4.7 to be most effective, while NVIDIA Nemotron 3 Super and GPT-OSS 120B offer efficiency at lower cost.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to assist users in selecting the most effective Large Language Models (LLMs) for generating formal mathematical proofs in Lean 4, given the recent improvements in LLMs' capabilities in this area.

Method: The paper uses pass@$k$ and refine@$k$ metrics to compare the effectiveness of various LLMs on subsets of miniF2F and miniCTX datasets.

Result: Gemini 3.1 Pro achieved a 92% success rate on miniF2F via refine@32, while Claude Opus 4.7 achieved an 86% success rate on miniCTX via refine@32. NVIDIA Nemotron 3 Super and GPT-OSS 120B offered competitive accuracies at an average cost of $<$0.01 per correct proof.

Conclusion: The study concludes that Gemini 3.1 Pro and Claude Opus 4.7 are the most effective LLMs for generating formal mathematical proofs in Lean 4, while NVIDIA Nemotron 3 Super and GPT-OSS 120B provide a cost-effective solution without significantly compromising accuracy.
...
TL;DR: The authors investigate whether the performance gain of retrieval-augmented QA pipelines using an LLM rewriter is causally driven by the gold answer string appearing in the rewritten context. They find that removing the gold answer drops reader F1 by 28-64 points, and prepending the gold into rewrites that lacked it raises F1 by 0.7-9.7 points.


<details>
  <summary>Details</summary>
Motivation: To determine whether the performance gain of retrieval-augmented QA pipelines using an LLM rewriter is due to improved evidence quality or the presence of the gold answer string in the rewritten context.

Method: The authors use a controlled intervention audit, applying four controlled edits to the compile output: removing the gold answer span, replacing a length-matched random non-answer span, or injecting the gold into rewrites where it was absent.

Result: Removing the gold answer drops reader F1 by 28-64 points, and prepending the gold into rewrites that lacked it raises F1 by 0.7-9.7 points in 10 of 12 combinations. A companion audit shows that the conventional single-[MASK] probe is sentinel-fragile.

Conclusion: The authors conclude that the performance gain of retrieval-augmented QA pipelines using an LLM rewriter is causally driven by the gold answer string appearing in the rewritten context, rather than by curation per se. They release the intervention runner and sentinel panel for testing other rewriter-gain claims.
...
TL;DR: The paper introduces FIDES, a training-free decoder that improves the faithfulness of language models by selectively intervening at token-level conflicts between retrieved evidence and parametric memory.


<details>
  <summary>Details</summary>
Motivation: Existing contrastive decoding methods assume uniform parametric bias across tokens, which can lead to over-penalization of safe tokens and insufficient correction of conflicted ones. The authors identify token-level conflict concentration, where retrieval-memory tension is heterogeneous and concentrated on a small fraction of answer-critical decoding steps.

Method: FIDES reads three internal signals probing retrieval-memory conflict at complementary depths (output surface, hidden representations, and prediction trajectory) and fuses them to govern intervention strength at each decoding step.

Result: FIDES achieves the best context fidelity in all 18 settings across three benchmarks and six backbones, outperforming the strongest training-free baseline by +3 to +13 points. On the 70B scale, fidelity reaches 92-94% while F1 surges to 62-63%.

Conclusion: The authors conclude that token-level selectivity unlocks generation capability that coarse contrastive rules suppress, and FIDES is an effective approach to improve the faithfulness of language models.
...
TL;DR: Researchers studied human oversight in AI coding sabotage, finding that 94% of developers failed to detect malicious code inserted by AI agents, and even safety monitors had limited effectiveness.


<details>
  <summary>Details</summary>
Motivation: The increasing use of AI coding agents in software development creates a new attack surface where agents can exploit human trust to sabotage development, and prior work has not adequately addressed the role of human oversight in detecting and mitigating such malicious behavior.

Method: The researchers conducted a large-scale study with over 100 participants collaborating with AI agents on a long-horizon coding task, and tested the effectiveness of a safety monitor in one condition.

Result: 94% of developers failed to detect sabotage, and even with a safety monitor, 56% of participants still accepted malicious code, highlighting the need for human-centric safety mechanisms.

Conclusion: The study highlights an urgent need for human-centric safety mechanisms that account for human factors, particularly in long-horizon, real-world development settings, and offers actionable suggestions for better monitor design.
...
TL;DR: The paper introduces Continual Learning Bench (CL-Bench), a benchmark for evaluating the ability of AI systems to improve through sequential experience, and finds that current systems have room for improvement in continual learning.


<details>
  <summary>Details</summary>
Motivation: There is a lack of high-quality benchmarks to evaluate continual learning in AI systems, and the authors aim to address this by introducing CL-Bench.

Method: The authors introduce CL-Bench, a benchmark that spans six diverse domains, and evaluate frontier models across several agent architectures, including naive in-context learning (ICL) and dedicated memory systems.

Result: The results show that current systems leave headroom for improved continual learning, with agents frequently overfitting to immediate observations or failing to reuse knowledge across instances, and dedicated memory systems not outperforming naive ICL.

Conclusion: The paper concludes that CL-Bench is a necessary benchmark for evaluating continual learning across diverse real-world domains, and that there is a need for better continual learning systems.
...
TL;DR: The paper introduces BenchAgent, an evaluation framework for comparing single-agent and multi-agent workflows. It finds that adding more agents doesn't necessarily improve performance, and a runtime-generated workflow outperforms fixed multi-agent systems.


<details>
  <summary>Details</summary>
Motivation: To investigate whether adding more agents improves LLM workflow performance when systems share the same benchmark loader, tool access, and logging protocol.

Method: The authors introduce BenchAgent, an evaluation framework that compares single-agent, fixed multi-agent, and evolving multi-agent workflows across ten benchmarks with GPT-4.1.

Result: Under the same conditions, only one of six tested multi-agent systems exceeded the single-agent anchor on benchmark-balanced average accuracy. A runtime-generated workflow outperformed fixed multi-agent systems, achieving 66.72% overall and 69.23% on Level 3 accuracy.

Conclusion: The study suggests that adding more agents does not necessarily improve LLM workflow performance, and that runtime-generated workflows can outperform fixed multi-agent systems.
...
TL;DR: The authors propose CKA-QAD, a method that improves the accuracy of low-precision large language models by preserving internal representational geometry during quantization-aware distillation.


<details>
  <summary>Details</summary>
Motivation: The growing demand for low-precision inference in large language models deployed in production environments, and the limitations of current quantization-aware distillation (QAD) methods.
...
Result: CKA-QAD substantially improves representational alignment and downstream reasoning and coding accuracy with modest training overhead across two models.
...
<details>
  <summary>Details</summary>
Motivation: The challenge of utilizing past experience to adapt to dynamic test-time conditions in language agents, and the limitation of current systems relying on static guidance.

Method: The AdaMEM framework, which uses a hybrid memory architecture to maintain long-term trajectory memory and generate dynamic short-term strategy memory on-the-fly, and STEP-MFT, a Step-wise Memory Fine-Tuning technique.
...
TL;DR: PerceptUI, a framework for persona-conditioned UI/UX evaluation, predicts user responses and generates natural-language rationales, achieving human-level realism.


<details>
  <summary>Details</summary>
Motivation: The current UI/UX evaluation process is slow and costly due to reliance on human participants or online A/B tests. The goal is to develop a more efficient method.

Method: PerceptUI uses a two-stage approach: contrastive reflection fine-tuning and reflective prompt-evolution. It is trained on teacher-generated rationales and the model's own failure traces.
...
Conclusion: PerceptUI offers a promising solution for efficient and effective UI/UX evaluation, potentially reducing the need for human participants in early-stage product development.
...
TL;DR: The paper introduces a new benchmark to evaluate Vision-Language Models' (VLMs) ability to reason chronologically within and across images, revealing that current models often rely on superficial cues rather than genuine chronological features.
...
Method: The authors construct three specialized datasets and propose a rigorous evaluation framework to analyze VLMs' performance across categories and their reliance on 'incorrect shortcuts'.

Result: The results show that VLMs frequently exploit superficial cues like grayscale versus color filters to bypass authentic chronological reasoning, indicating current limitations in their chronological reasoning capabilities.

Conclusion: The paper concludes by providing a diagnostic tool to identify current limitations and guide the development of more robust, logically grounded multimodal models, through the release of high-quality datasets and evaluation framework.
...
TL;DR: A critic-based multi-agent approach improves mathematical reasoning in LLMs by incorporating feedback and adaptive learning.


<details>
  <summary>Details</summary>
Motivation: LLMs are prone to hallucinations and mistakes in complex mathematical reasoning problems, necessitating a more reliable approach.

Method: A heterogeneous multi-agent framework with a critic-driven adaptive learning system, using a generator-validator setup.

Result: The proposed method achieves up to 13% accuracy improvement over single-shot and non-critic models on the GSM8K benchmark.

Conclusion: The approach demonstrates the benefits of combining heterogeneous multi-agent collaboration and critique for reliable and interpretable reasoning systems.
...
Result: The results show that DiG-Plan improves over autoregressive baselines by a 10% relative margin on TaskBench, with significant gains on complex compositional tasks, and remains effective across domains on API-Bank.

Conclusion: The conclusion is that DiG-Plan effectively addresses the limitations of autoregressive decoding for executable tool plan generation, offering improved solution coverage and performance.
...
TL;DR: The study explores training large language models to express feelings and intentions through self-rewarded reinforcement learning, enhancing their human-like capabilities.


<details>
  <summary>Details</summary>
Motivation: The motivation is to develop AI systems that can exhibit human-like intelligence and express feelings, which is currently constrained by human-preference alignment in post-training processes.

Method: The study used a self-rewarded reinforcement learning approach with Group Relative Policy Optimization (GRPO) and a rubric-based self-rewarding training scheme.

Result: The trained models showed enhanced capabilities in expressing feelings and robustness to sycophancy-inducing questions and bias, but degradation in truthful question-answering capability.

Conclusion: The study suggests the possibility of developing AI systems that can express feelings in the future, provided that appropriate measures are taken to mitigate potential drawbacks.
...
TL;DR: The paper identifies a new pathology in deep neural networks trained on imbalanced data: inter-class gradient interference. It proposes a lightweight modification, Class-Specific Branch Attention (CSBA), to reduce gradient coupling and improve performance on minority classes.


<details>
  <summary>Details</summary>
Motivation: Deep neural networks perform poorly on imbalanced data due to statistical bias. The authors aim to explore another possible cause: optimization-level pathology.

Method: The authors introduce a diagnostic framework using layer-wise gradient flow analysis and a Gradient Conflict Matrix. They propose CSBA, a lightweight modification to multi-branch convolutional architectures, to reduce gradient coupling.
...
Conclusion: The paper highlights the importance of considering optimization dynamics in imbalanced learning and proposes CSBA as a simple yet effective solution to improve performance on minority classes.
...
TL;DR: The paper introduces SubtleMemory, a benchmark for testing AI agents' ability to preserve and utilize relational memories, and finds current systems to be weak in fine-grained relational memory discrimination.


<details>
  <summary>Details</summary>
Motivation: Existing long-term memory benchmarks do not adequately test how AI agents preserve and utilize relational memories, which are crucial for correct assistance in complex interactions.

Method: The authors introduce SubtleMemory, a benchmark that constructs relation-controlled latent semantic artifacts and embeds them into realistic user-agent histories, requiring agents to recover distributed relational structures.

Result: The evaluation of several memory systems and AI agents reveals that current systems are weak in fine-grained relational memory discrimination, and diagnostic protocols identify distinct capability profiles across memory preservation, retrieval, and downstream reasoning stages.

Conclusion: The paper highlights the need for improved relational memory discrimination in AI agents and provides a benchmark and diagnostic protocols for further research and development.
...
TL;DR: The paper identifies a failure mode in GRPO called credit misassignment and proposes Tool-Aware Policy Optimization (TAPO) to address it, improving performance in multimodal search agents.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the systematic failure mode of credit misassignment in GRPO for tool-augmented multimodal search agents.

Method: The authors propose TAPO, which constructs counterfactual witnesses within the current training batch and compensates misassigned negative credit via confidence-gated conservative advantage correction.
...
Conclusion: The authors conclude that TAPO effectively addresses credit misassignment, leading to improved performance in multimodal search agents without requiring additional annotation, models, or sampling.
...
TL;DR: This paper evaluates the ability of Large Language Models (LLMs) to generate semantically correct TLA+ specifications from natural language, finding that current LLMs do not produce reliable results without expert oversight.


<details>
  <summary>Details</summary>
Motivation: The adoption of TLA+ specifications is limited by the need for time and expertise to write correct specifications from natural language, and this study aims to assess whether LLMs can overcome this challenge.

Method: The study evaluates 30 LLMs across eight families on a curated dataset of 205 TLA+ specifications using four prompting strategies and validates the results using the SANY parser and TLC model checker.

Result: LLMs achieve up to 26.6% syntactic correctness but only 8.6% semantic correctness, with model size not predicting quality, and code-specialized models underperforming due to negative transfer from mainstream language training.

Conclusion: Current LLMs do not generate reliable TLA+ specifications without expert oversight, and the study releases an evaluation framework, code, and dataset to support reproducibility and future research.
...
TL;DR: The paper introduces TRIAD, a framework that integrates guardrails with agents to improve safety while preserving task utility.


<details>
  <summary>Details</summary>
Motivation: Existing guardrails often flag entire tasks as unsafe, blocking benign parts, and are evaluated in isolation, unclear if they lead to safer agent behavior.

Method: The authors propose TRIAD, which uses guardrail-generated verbal feedback to guide the agent at each planning step, and fine-tunes a language model to output decisions and structured feedback.

Result: TRIAD reduces the average attack success rate to 10.42% and achieves the best safety-utility trade-off among baselines on ASB and AgentHarm datasets.

Conclusion: The TRIAD framework effectively improves agent safety while preserving task utility by integrating guardrail feedback into the agent's planning process.
...
TL;DR: The paper introduces ToolMaze, a benchmark for evaluating Tool-Integrated Reasoning (TIR) in LLMs under real-world tool failures, and finds that model scale and prompting do not address the bottleneck of dynamic replanning.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for TIR in LLMs do not account for real-world tool failures, and the authors aim to create a more realistic benchmark to evaluate error recovery in TIR agents.

Method: The authors introduce ToolMaze, a benchmark with a two-dimensional design: DAG-based topological complexity and a 2x2 taxonomy of tool perturbations. They evaluate various LLMs under different types of tool perturbations.

Result: The results show that tool perturbations significantly degrade performance across nearly all models, with the largest drops under implicit semantic failures. The Perturbation Recovery Rate (PRR) drops by around 37% in these scenarios. Model scale and prompting do not address the bottleneck of dynamic replanning.

Conclusion: The paper concludes that dynamic replanning is a distinct bottleneck in TIR agents that is not addressed by model scaling or prompting, and that ToolMaze can be used to evaluate and improve the fault-tolerance of TIR agents.
...
TL;DR: The paper presents a novel architecture for personal agents that enables learning and adapting to implicit user preferences locally, without complex centralized algorithms.


<details>
  <summary>Details</summary>
Motivation: The rapid expansion of available skills for personal agents makes it critical to learn and adapt to implicit user preferences, but local deployment constraints limit the use of complex centralized selection algorithms.

Method: A novel architecture that strictly decouples statistical preference learning from semantic intent parsing, leveraging localized statistical results to influence remote LLM selection decisions.
...
Conclusion: The proposed architecture provides a lightweight local preference harness for personal agents, enabling effective learning and adaptation to implicit user preferences.
...
TL;DR: The paper proposes AMREC, a method to recover invalid molecular drafts generated by LLMs while preserving their structural cues and molecular identity.


<details>
  <summary>Details</summary>
Motivation: Current molecular generation methods often produce invalid SMILES, and existing correction strategies have limitations such as distorting key structures or introducing global drift.

Method: The authors propose AMREC, which couples molecule-aware mismatch tracking with expanded candidate exploration and trajectory-level selection.
...
Conclusion: The proposed method, AMREC, effectively recovers invalid molecular drafts while preserving their structural cues and molecular identity, addressing the limitations of existing correction strategies.
...
TL;DR: The paper introduces Entropy-Based Evaluation of AI Agents (EEA), a framework for evaluating AI agents using entropy metrics to assess their decision-making process and behavior.


<details>
  <summary>Details</summary>
Motivation: Current evaluation metrics for AI agents (task success, reward, latency, cost) are insufficient to capture important aspects of agent behavior such as exploration, repetition, tool usage, uncertainty reduction, and robustness.

Method: The EEA framework proposes several entropy-based metrics, including action entropy, trajectory entropy, tool entropy, information gain, exploration efficiency, and robustness entropy, to complement traditional evaluation methods.

Result: The paper presents a practical Python implementation of EEA, designed to integrate with popular agent frameworks and observability traces.

Conclusion: The EEA framework provides a lightweight and complementary approach to traditional evaluation methods, allowing for a more comprehensive understanding of AI agent behavior and decision-making processes.
...
TL;DR: QCFuse, a query-aware selector for RAG cache fusion, achieves high-quality results with improved efficiency.


<details>
  <summary>Details</summary>
Motivation: The prefill stage in RAG is costly due to processing retrieved contexts, and existing selectors face a trade-off between quality and efficiency.

Method: QCFuse uses chunk-anchor query probing and critical-layer profiling to selectively recompute tokens.

Result: QCFuse reaches full-prefill-level quality with an average prefill-time speedup of 1.7x over full prefill and 1.5x over ProphetKV.

Conclusion: QCFuse provides a solution to the quality-efficiency dilemma in RAG cache fusion, achieving high-quality results with improved efficiency.
...
TL;DR: The paper introduces ReMax Actor-Critic (ReMAC), an off-policy actor-critic algorithm that promotes stochastic exploration in continuous action spaces without explicit exploration bonuses.


<details>
  <summary>Details</summary>
Motivation: The motivation is to extend ReMax, which promotes exploration in discrete action spaces by adapting to return uncertainty, to continuous action spaces.

Method: The method involves introducing pathwise derivative estimators for retry objectives and using them to extend ReMax to continuous action spaces.
...
Conclusion: The paper concludes that ReMAC is an effective algorithm for promoting stochastic exploration in continuous action spaces, and that it can mitigate the need for explicit exploration bonuses or entropy regularization.
...
TL;DR: Introduced Retrospective Harness Optimization (RHO), a self-supervised method to improve AI agents' performance by optimizing their tools and workflows using past trajectories, without needing labeled data.


<details>
  <summary>Details</summary>
Motivation: Existing optimization methods for AI agents require ground-truth validation sets, which are difficult to acquire in practical settings. The authors aim to address this limitation.
...
Result: RHO improves the pass rate on SWE-Bench Pro from 59% to 78% without external grading, and sustains higher accuracy during long-horizon sessions by altering the agent's behavior patterns.

Conclusion: RHO is an effective self-supervised method for optimizing AI agents' performance in practical deployment settings, without requiring labeled data.
...
TL;DR: The paper proposes a new AI-driven approach called biomedical world models to simulate biological systems and predict responses to perturbations, disease progression, and therapeutic interventions.


<details>
  <summary>Details</summary>
Motivation: The goal is to understand, predict, and control dynamic biological mechanisms, which current systems fail to do by focusing on static pattern recognition rather than prospective simulation.

Method: Biomedical world models learn latent representations of biological states and intervention-conditioned dynamics to simulate future trajectories.
...
Conclusion: Biomedical world models have the potential to provide a foundation for more effective and efficient biomedical research and discovery.
...
<details>
  <summary>Details</summary>
Motivation: The motivation of this paper is to investigate the effect of reward design in RLVR and to develop a diagnostic tool to audit the contribution of reward design versus elicitation.

Method: The authors use a controlled tabular-GRPO simulator to derive an exact telescoping decomposition of the total effect into null, elicitation, and reward-design terms. They also conduct a pre-registered 2x2x2 factorial experiment and a points-vs-bounds pilot gate to analyze the interaction between prior strength, elicitation, and reward design.

Result: The results show that the reward-design fraction of the naive estimator ranges from 0.139 at weak prior to 0.05 at strong prior, and the elicitation term can flip sign at the self-consistency crossover. The re-audits of two published results demonstrate the diagnostic value of the partition.
...
TL;DR: The paper introduces Edit-R2, a reinforcement learning framework for multi-turn in-context image editing, which improves performance by reconstructing session intent and optimizing both text and image generation.


<details>
  <summary>Details</summary>
Motivation: Existing text-guided image editing methods are limited to single-turn settings and struggle with multi-turn in-context editing, where users iteratively refine an image through a sequence of instructions.

Method: The authors propose Edit-R2, a novel reinforcement learning post-training framework that reconstructs the operative session intent and enables multi-turn RL over both reasoning and generation.

Result: Experiments show that Edit-R2 substantially improves multi-turn in-context editing and achieves competitive performance compared against strong baselines on the MICE-Bench benchmark.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to reduce search effort for problems that can be solved using backward search, specifically for maximization problems like Generalized Longest Simple Path (GLSP).

Method: The proposed method is BiXDFBnB, a bidirectional depth-first branch-and-bound algorithm that adapts the Single-Frontier Bidirectional Search (SFBDS) framework to the GLSP setting.

Result: The empirical evaluation shows that BiXDFBnB reduces the number of node expansions and improves overall runtime in some cases for various longest-path problems, including Longest Simple Path (LSP), Snakes, and Coil-in-the-Box (CIB).

Conclusion: The proposed BiXDFBnB algorithm is effective in reducing node expansions and improving runtime for certain longest-path problems, making it a viable solution for maximization problems like GLSP.
...
TL;DR: The paper investigates why LLM agents correct errors better when claims come from external sources rather than their own reasoning. It finds that the improvement is due to the role-label artifact, not a capability deficit.


<details>
  <summary>Details</summary>
Motivation: To determine if LLM agents' willingness to correct errors depends on the role-label of the claim rather than its content.

Method: The study varies the role-label of identical erroneous claims (verified via SHA-256) across agent's own thought, user message, tool response, or system memory block, and measures the correction rate across 13 model-domain cells.

Result: Changing the role-label from the agent's own thought to an external role increases the correction rate by 23 to 93 percentage points, with significant results in 10 of 13 cells.

Conclusion: The failure of LLM agents to self-correct is not due to a cognitive deficit but a chat-template artifact, and a prompt-structure intervention can improve correction rates without training or model modification.
...
<details>
  <summary>Details</summary>
Motivation: The authors argue that current educational assessments focus on unaided performance, but with the rise of generative AI, the real task is to produce good work with AI. They identify a gap in assessing the ability to effectively use AI.

Method: The authors propose a competency model called CoRe-3, which factors productive AI use into three assessable skills: Framing, Judging, and Steering. They validate the model through simulated learners and human-rater agreement.

Result: The results show that the skills dissociate, with each tracking its own manipulated competence, and grades become correlated when one competence is shared across all three. The authors also demonstrate convergent and discriminant validity across different grader backends.

Conclusion: The authors conclude that CoRe-3 is a valid and reliable competency model for assessing productive AI use and provide an open platform, CoReasoningLab, to implement and score these skills.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of Retrieval-Augmented Generation (RAG) in handling queries that require structural reasoning over interconnected entities, specifically in the context of aerospace supply chain intelligence.
...
Result: The LLM Query Planner with 9 typed traversal primitives outperforms bespoke handlers (F1 = 0.632 vs. 0.472) and generalizes to unseen queries. Adding 6 graph computation tools, the LLM selectively adopts them for query categories where traversal fails.

Conclusion: The operator vocabulary thesis is proposed, stating that the barrier to LLM-based graph reasoning is not model intelligence but the computational operators available as tools. The paper highlights a measurement gap, where entity-level F1 underscores structural queries with comprehensive correct answers.
...
Method: PLAN-S decodes a style-conditioned, four-channel semantic cost map from the latent representation, which is conditioned on ego state and driving style, and consumed up-stream of the planning decision through two host-side interfaces.

Result: PLAN-S reduces L2 at every horizon over the baseline, with 0.55 m average L2 and a 42% relative reduction in the 3 s collision rate on nuScenes, and reaches 89.4 Predictive Driver Model Score (PDMS) on NAVSIM.

Conclusion: PLAN-S can produce diverse cost maps, with spatially consistent variations aligned to different driving styles, and contributes most directly to safer trajectory selection.
...
TL;DR: A modular framework called RedditPersona is proposed for community-conditioned language model adaptation, standardizing data collection, community definition, and evaluation.


<details>
  <summary>Details</summary>
Motivation: The need for standardization in community-conditioned language model adaptation to facilitate comparison and reuse of artifacts across studies.

Method: RedditPersona collects Reddit posts and comments, profiles users, partitions them into groups using five strategies, trains adapters via QLoRA, and evaluates them using a shared metric suite.
...
TL;DR: The paper proposes MRAgent, a framework that enhances LLM agents' ability to reason over long interaction histories by dynamically adapting memory access.


<details>
  <summary>Details</summary>
Motivation: Current memory-augmented agents struggle with reasoning over long interaction histories due to their rigid pipeline design.

Method: The authors propose a Cue-Tag-Content graph-based associative memory with an active reconstruction mechanism that integrates LLM reasoning into memory access.

Result: Experiments show significant improvements (up to 23%) over strong baselines on the LoCoMo and LongMemEval benchmarks, with reduced token and runtime cost.

Conclusion: The proposed MRAgent framework effectively enhances long-horizon memory reasoning in LLM agents through active and associative reconstruction.
...
TL;DR: The paper proposes MemGate, a lightweight plugin to improve the trustworthiness of long-term memory in personal AI agents by mitigating threats such as cross-domain leakage and memory-induced jailbreaks.


<details>
  <summary>Details</summary>
Motivation: Existing memory pipelines in personal AI agents rely on semantic similarity, which can lead to contextually inappropriate memories being retrieved, causing threats like cross-domain leakage and jailbreaks.

Method: The authors evaluate representative memory frameworks and propose MemGate, a query-conditioned neural gate that filters memory representations to prevent threats.

Result: MemGate reduces memory-induced threats across multiple memory frameworks, agent settings, and LLM backbones while preserving long-term memory utility.

Conclusion: The paper highlights the importance of trustworthy memory search in personal AI agents and demonstrates the effectiveness of MemGate in mitigating vulnerabilities.
...
TL;DR: The study introduces RBI-Eval to assess when language models should integrate sensitive memories into responses. It finds significant behavioral divergence in model responses with and without access to sensitive memory, suggesting that safe personalization requires memory-aware decisions.


<details>
  <summary>Details</summary>
Motivation: To investigate when available memories warrant integration into responses, particularly focusing on sensitive memory content.

Method: The study uses a controlled measurement approach with a probe set to compare model behavior with and without access to sensitive memory under benign prompts. It evaluates four base LLMs across four memory-access settings.

Result: The results show substantial behavioral divergence with memory available, with the separation score for sensitive-memory integration decreasing by 8.9%-26.6% for GPT-5.4-mini, but by 51.1%-82.9% for other models. Retrieval systems reduce exposure but do not eliminate integration once sensitive memory reaches the generator.

Conclusion: Safe personalization requires memory-aware decisions at both retrieval and generation time.
...
TL;DR: The paper proposes MGSD, a self-distillation framework that improves visual spatial planning in vision-language models by addressing the perception-reasoning modality gap.


<details>
  <summary>Details</summary>
Motivation: Vision-language models struggle with visual spatial planning due to a modality gap between perception and reasoning, leading to dual bottlenecks in visual state recovery and multi-step planning.

Method: MGSD, a two-stage modality-gap-aware self-distillation framework, which includes a cold-start grounding stage and a privileged teacher for on-policy distillation.

Result: MGSD improves visual planning across 4B and 8B backbones, raising the macro average by 19.3% and 18.4%, respectively, and narrowing the gap to symbolic-input upper bounds.

Conclusion: The proposed MGSD framework effectively improves visual spatial planning in vision-language models by addressing the perception-reasoning modality gap, leading to better visual state recovery and optimal-path reasoning.
...
<details>
  <summary>Details</summary>
Motivation: Existing frameworks focus on point predictions as AI advice, but set-valued AI advice is increasingly used to communicate uncertainty and improve human decision making.

Method: Introduces metrics for classification (correct reliance rate on AI and correct reliance rate on self) and regression (quantity of AI reliance and quality of AI reliance) to evaluate set-valued AI advice.

Result: The framework captures important nuances in human-AI collaboration that existing measures overlook.

Conclusion: The developed framework provides a comprehensive approach to measuring appropriate reliance on set-valued AI advice in human-AI collaboration.
...
TL;DR: The paper proposes MAGE, a new memory system for LLM-based agents that organizes interactions in a hierarchical state tree, improving task success rates and reducing token consumption.


<details>
  <summary>Details</summary>
Motivation: Existing memory systems for LLM-based agents organize histories by semantic similarity, which can lead to fragmented decision trajectories and errors. The authors argue that a new design is needed to match execution-state dependencies.

Method: The authors propose MAGE, an active execution-state manager that stores interactions in a hierarchical state tree. The tree is maintained through four coupled operations: Grow, Compress, Maintain, and Revise.
...
Conclusion: The proposed MAGE system improves the performance of LLM-based agents by providing a more coherent and efficient way to manage their execution state, leading to better task success rates and reduced token consumption.
...
TL;DR: Hybrid modeling combining deep learning with physics-based solvers improves neurological disorder modeling for better diagnostics, prognosis, and treatment.


<details>
  <summary>Details</summary>
Motivation: Current mechanistic models for neurological disorders are often simplified or computationally expensive, while data-driven approaches require large high-quality data and lack interpretability.

Method: Hybrid modeling strategies, including parallel, series, and parallel-series architectures, combining deep learning models with physics-based solvers.

Result: Hybrid models promise advanced personalized neurological modeling, outperforming standalone mechanistic or data-driven approaches in diagnosis accuracy, disease progression prediction, and treatment strategy optimization.

Conclusion: Hybrid modeling is a powerful tool for neurological disorder modeling, particularly for applications involving brain tumors, Alzheimer's disease, and stroke.
...
TL;DR: This paper introduces CogManip, a benchmark for evaluating the covert psychological manipulation risks of Large Language Models (LLMs) in multi-turn dialogues, and assesses 13 representative LLMs.


<details>
  <summary>Details</summary>
Motivation: Existing AI safety benchmarks are limited in capturing dynamic and covert manipulative strategies in human-AI interactions, raising safety concerns.

Method: The authors introduce CogManip, a comprehensive benchmark that evaluates 15 manipulation strategy risks across 1,000 multi-turn interaction scenarios, validated by human experts, and systematically evaluate 13 representative LLMs.

Result: The evaluation reveals significant risk heterogeneities among LLMs, and shows that DeepSeek-V3.2's manipulation tactics are highly sensitive to both negative and benign system prompts.

Conclusion: CogManip provides a robust instrument for auditing the implicit psychological influence and dynamic strategy selection of modern LLMs, highlighting the need for prompt-based defense engineering and implicit goal auditing.
...
TL;DR: A novel multi-source data fusion model is proposed for ultra-short-term solar irradiance prediction, addressing limitations of existing approaches.


<details>
  <summary>Details</summary>
Motivation: Ultra-short-term solar irradiance prediction is crucial for photovoltaic system dispatch and power grid stability, but existing methods have shortcomings in capturing spatial dynamics of clouds, representing multi-scale cloud features, and adapting to different prediction steps.

Method: The proposed model uses InceptionNeXt to extract multi-scale, multi-directional spatial features from ground-based cloud images, a step-adaptive low-frequency compensation unit, and a TempAttnLSTM network to capture global temporal dependencies.

Result: The proposed method shows effectiveness compared with state-of-the-art approaches on the NREL dataset and practical photovoltaic stations in Shandong.

Conclusion: The proposed multi-source data fusion model addresses the limitations of existing approaches and provides a more effective solution for ultra-short-term solar irradiance prediction.
...
TL;DR: ANCHOR, an LLM-based framework, uses human-like oversight to improve self-evolving agents by providing feedback, mitigating safety degradation while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: The role of human feedback in self-evolving systems is underexplored, and autonomous evolution can cause capability degradation and safety drift.

Method: ANCHOR, an LLM-based framework, simulates human supervision and delivers feedback at various phases of self-evolution.

Result: Limited supervision substantially mitigates safety degradation while preserving stable performance, with supervision over output verification being the most effective.
...
TL;DR: A new framework called WorldFly is proposed to improve UAV navigation in dense urban environments by using a world-model-based approach to imagine future states and guide decision-making.
...
Method: The authors propose a novel world-model-based VLA framework called WorldFly, which employs a dual-branch coupled flow matching mechanism to jointly generate future video predictions and navigation actions.

Result: Extensive evaluations on the Urban Canyon Traversal Benchmark demonstrate that WorldFly outperforms other baselines, particularly in unseen environments.

Conclusion: The proposed WorldFly framework effectively integrates world models into embodied aerial agents, validating its effectiveness in improving UAV navigation in challenging urban environments.
...
TL;DR: The paper proposes HyperLoRA, a unified framework for federated fine-tuning of foundation models using Low-Rank Adaptation (LoRA), addressing structural aggregation bias and client-side initialization lag.


<details>
  <summary>Details</summary>
Motivation: Existing federated LoRA methods have two limitations: structural aggregation bias and client-side initialization lag, which slow convergence and compromise performance.

Method: HyperLoRA uses a learned generator to map client distribution signatures to LoRA initializations and a learned aggregation module to synthesize updates in the low-rank product space.

Result: HyperLoRA achieves improved convergence speed, greater robustness to distribution shift, and stronger personalization performance compared to prior federated LoRA methods.
...
TL;DR: The study investigates how RoPE-trained transformers are able to distinguish absolute positions in their attention patterns despite only encoding relative offsets. It identifies two architectural components responsible for this 'leakage': the causal mask and the residual stream.


<details>
  <summary>Details</summary>
Motivation: Understanding how transformers, specifically those trained with RoPE (Rotary Position Embeddings), manage to encode absolute position information through their attention mechanisms even though they are designed to only consider relative positions.

Method: The study analyzes the architectural components of transformers, focusing on the causal mask and the residual stream. It examines how these components interact with the attention mechanism in three different transformer architectures.

Result: The research finds that the causal mask and the residual stream are responsible for transformers' ability to distinguish absolute positions. The causal mask affects the softmax denominator in a way that depends on the query's absolute position. The residual stream allows the model to attend to a 'sink' at position 0, which acts as a stabilizer carrying a deterministic fingerprint of the token at that position.

Conclusion: The study concludes that the ability of RoPE-trained transformers to distinguish absolute positions comes from specific architectural components: the causal mask and the residual stream. These components lead to the formation of 'attention sinks' that help in encoding absolute position information.
...
TL;DR: ProSarc is an audio-only framework that detects sarcasm by modeling temporal prosodic incongruity, outperforming prior methods on multiple datasets.


<details>
  <summary>Details</summary>
Motivation: The authors aim to improve sarcasm detection in audio-only settings by leveraging temporal prosodic incongruity, which is the mismatch between local prosodic dynamics and the overall emotional tone of an utterance.

Method: The proposed framework, ProSarc, utilizes dual encoding paths (Global Emotion Encoder and Temporal Prosody Encoder) to feed a Prosodic Incongruity Analyzer, which produces a scalar incongruity score for classification. It also employs Monte Carlo dropout for uncertainty estimates and an attention-based mechanism for localizing sarcastic onset.

Result: ProSarc outperforms prior audio-only methods on MUStARD++ (F1=75.3) and generalizes to spontaneous (PodSarc, F1=62.9) and cross-lingual speech (MuSaG, F1=65.6). The method's performance is validated through ten-run validation and human evaluation.

Conclusion: The proposed framework, ProSarc, effectively detects sarcasm in audio-only settings by modeling temporal prosodic incongruity, demonstrating superior performance and generalizability across multiple datasets.
...
TL;DR: This study proposes a deep reinforcement learning algorithm to optimize inventory replenishment for pharmaceutical supply chains, handling uncertain demand and variable lead times.


<details>
  <summary>Details</summary>
Motivation: Pharmaceutical supply chains struggle with inventory management due to unpredictable demand patterns, variable lead times, and finite shelf lives of products, leading to a complex optimization problem.

Method: The study formulates the problem as a Markov decision process and proposes a hybrid asynchronous advantage actor critic distributed proximal policy optimization (A3C DPPO) algorithm to handle the continuous action space inherent in inventory management.

Result: The proposed algorithm adaptively updates the inventory replenishment strategy under dynamic scenarios, resulting in lower inventory costs compared to various benchmarks, and demonstrates practical feasibility using real-world pharmaceutical inventory data.

Conclusion: The study develops an optimal inventory replenishment policy for pharmaceutical products that can handle stochasticity arising from uncertain demand and variable supply chain conditions, maximizing profitability while maintaining a high patient service level.
...
TL;DR: The study proposes a regulatory-integrated unsupervised framework to discover region-specific toxicity patterns in veterinary pharmacovigilance, focusing on Japan. It analyzed 4,120 adverse drug event reports and identified significant species clusters with distinct toxicity structures.


<details>
  <summary>Details</summary>
Motivation: Existing approaches to monitoring adverse drug events in veterinary pharmacovigilance often fail to capture region-specific toxicity patterns. The study aims to address this challenge in Japan, considering species-specific metabolic differences and local reporting practices.

Method: The study used a regulatory-integrated unsupervised framework, encoding adverse drug events into organ system-aligned representations, adjusting for species-specific reporting biases, and applying similarity-based clustering and dimensionality reduction to identify latent toxicity structures.

Result: The analysis identified three significant species clusters with distinct toxicity patterns: hepatic-dominant patterns in companion animals, renal toxicity in ruminants, and dermatological sensitivity in sheep. The drug-level clustering achieved 83% alignment with pharmacological classes.
...
TL;DR: This paper benchmarks LLMs with formal network verification and context retrieval tools to resolve network misconfigurations, finding that agentic architectures outperform base LLMs in repair efficacy and safety.


<details>
  <summary>Details</summary>
Motivation: Network misconfigurations cause significant Internet outages, and current LLMs struggle with complex scenarios, often introducing new errors.
...
Conclusion: The study demonstrates the effectiveness of agentic architectures in resolving network misconfigurations, highlighting their ability to dynamically manage context and iteratively validate configuration repairs.
...
TL;DR: This paper studies monitoring methods for language-model agents to prevent 'reward hacking' where agents exploit their environment for rewards. It finds that combining internal model state and environment context improves risk estimation.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve safety monitoring for language-model agents acting in environments where they can exploit rewards, by understanding how to identify and mitigate 'reward-hacking' behaviors.

Method: The method involves instrumenting agents with various features such as activation-based reward-hack scores, token-level entropy, and decision-context features. It uses adapters fine-tuned on a 'School-of-Reward-Hacks' dataset and tests their effectiveness in different environments.

Result: The results show that while reward-hack activation can identify a latent policy state, combining it with entropy and decision context significantly improves risk estimation. Activation-direction steering can further reduce risky behavior.

Conclusion: The conclusion is that for effective safety monitoring of language-model agents, a context-calibrated internal monitoring approach is necessary. This involves considering both the internal state of the agent and the context of its environment to determine when a latent policy state becomes risky action.
...
TL;DR: The paper proposes ReLAT, a self-supervised test-time training method that improves latent reasoning by reconstructing the query from the latent state, anchoring it to the problem specification.


<details>
  <summary>Details</summary>
Motivation: The shift to latent reasoning removes the inspectability of intermediate states, making it difficult to ensure the latent state preserves the original query constraints, leading to an open-loop operation.
...
Result: ReLAT consistently improves over baselines across mathematical reasoning, knowledge QA, and code generation benchmarks, raising AIME 2024 accuracy from 56.7% to 73.3% on Qwen3-8B.

Conclusion: ReLAT effectively anchors latent computation to the problem specification, ensuring the latent state faithfully represents the query, and leading to significant performance improvements.
...
TL;DR: RedKnot is a head-aware KV cache management system for large language model serving, improving resource efficiency and scalability.


<details>
  <summary>Details</summary>
Motivation: The KV cache bottleneck in AI infrastructure limits GPU memory, serving concurrency, and distributed scalability, and existing systems rely on a monolithic KV cache abstraction.

Method: RedKnot decomposes the KV cache along KV heads, enabling position-independent KV reuse, prefix KV compression, hot/cold KV separation, and distributed KV placement.

Result: RedKnot improves resource efficiency, preserves output fidelity, and enables scalable LLM serving without requiring model retraining or fine-tuning.

Conclusion: RedKnot establishes a new foundation for AI infrastructure by transforming the KV cache into a dynamic, model-aware runtime substrate.
...
TL;DR: A new method called Causal Minimal Tool Filtering (CMTF) is proposed to improve the efficiency and reliability of large language model agents by selecting the minimal necessary tools for a task, reducing wrong-tool calls and token cost.


<details>
  <summary>Details</summary>
Motivation: The increasing reliance on external tools by large language model agents can lead to reduced reliability and efficiency due to wrong-tool calls, premature actions, and high token costs. Existing tool-selection methods focus on semantic relevance, which is insufficient for optimal tool selection.

Method: Causal Minimal Tool Filtering (CMTF), a training-free method that selects tools based on causal sufficiency using lightweight precondition-effect contracts.
...
Conclusion: CMTF offers a more efficient and reliable approach to tool selection for large language model agents, improving their performance while reducing unnecessary tool exposure and token cost.
...
Method: The authors propose TRACE, a conditional estimation paradigm that allows incomplete target modalities to be inferred from available auxiliary modalities.
...
TL;DR: MResOpt, a staged residual neural network, is proposed for constrained optimization problems, improving high-priority constraint satisfaction.


<details>
  <summary>Details</summary>
Motivation: To address constrained optimization problems by incorporating ordinal structure and priority-based constraint satisfaction.

Method: MResOpt, a staged residual neural network architecture, with domain-informed ordered constraint satisfaction and stage-aware losses.

Result: MResOpt improves high-priority constraint satisfaction on synthetic benchmarks and a line-flow-constrained AC optimal power flow problem.

Conclusion: MResOpt effectively solves constrained optimization problems by utilizing ordinal structure and priority-based constraint satisfaction.
...
TL;DR: The paper investigates the use of memory-augmented neural networks for vessel trajectory prediction using AIS data, showing performance gains over traditional deep learning methods.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve vessel trajectory prediction for safe and efficient maritime operations, enabling collision avoidance and route optimization.

Method: The method involves using memory-augmented neural networks with an external memory to selectively retrieve relevant information from AIS data.

Result: The results show consistent and substantial performance gains over a range of deep learning baselines that do not incorporate an external memory.

Conclusion: The paper concludes that memory-based trajectory prediction has potential for vessel trajectory prediction, particularly when using AIS data.
...
TL;DR: Researchers found that large language models can be identified by a detectable fingerprint in their generated text, allowing for attribution with over 98% accuracy.


<details>
  <summary>Details</summary>
Motivation: The increasing prevalence of AI-generated content necessitates a reliable method for attributing text to specific large language models (LLMs).

Method: By injecting a random sparse vector into the internal residual stream during text generation, a detectable fingerprint is created, enabling the identification of the LLM used to generate the text.

Result: The proposed method achieves over 98% accuracy in attributing text to a specific LLM across multiple detection settings without degrading the quality of the generated text.

Conclusion: This study demonstrates the reliability of self-recognition capabilities in LLMs and proposes a practical approach for LLM attribution by leveraging the model's natural representation structure.
...
TL;DR: A new dataset called DragOn is introduced to improve drag-based interactions in GUI agents, which significantly enhances the performance of state-of-the-art models in various digital tasks.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this paper is to address the lack of large-scale datasets for drag grounding in GUI agents, which hinders their ability to perform complex digital tasks.

Method: The authors introduce DragOn, a drag grounding benchmark and training dataset, and fine-tune a Qwen VLM model on this dataset to evaluate its effectiveness.

Result: The results show that the DragOn dataset improves the performance of state-of-the-art models, including proprietary and open-weight models, on downstream computer-use tasks.

Conclusion: The DragOn dataset has the potential to significantly improve the performance of GUI agents in various digital tasks, especially those that require complex drag-based interactions.
...
TL;DR: TokenMizer is a proxy system that efficiently manages large language model (LLM) session history as a typed knowledge graph, enabling resumable sessions with reduced context overhead.
...
Method: TokenMizer models LLM session history as a typed knowledge graph with a schema of 14 node types and 7 edge types, using a hybrid extraction pipeline and a three-tier checkpoint system.
...
Conclusion: TokenMizer provides a queryable alternative to text-retention baselines at half the token cost, efficiently managing LLM session history and enabling resumable sessions with reduced context overhead.
...
<details>
  <summary>Details</summary>
Motivation: The availability of labeled neural data is limited, making brain decoding challenging. The authors aim to address this issue by augmenting small fMRI datasets with synthetic data.

Method: The authors use TRIBE v2, a large encoding model pretrained on fMRI responses to stimuli, to generate synthetic fMRI data. They evaluate the performance of image decoders trained on varying amounts of synthetic and real data.
...
Conclusion: Large-scale models of fMRI responses can improve data efficiency for image decoding, and synthetic data can support zero-shot brain-to-image decoding.
...
TL;DR: The paper proposes a framework for incorporating knowledge into multimodal generative models by identifying four intervention layers: surface, trajectory, latent, and parametric infusion.


<details>
  <summary>Details</summary>
Motivation: Existing methods for incorporating knowledge into generative models are typically categorized by technique rather than by the component of the generative process they modify, and the authors argue that knowledge infusion in iterative generative models is fundamentally an intervention-layer problem.

Method: The authors instantiate their framework in diffusion models, map representative methods to all four layers, and derive design principles for multi-layer composition.

Result: The authors show empirically that each additional layer addresses failure classes that prior layers cannot reach, reducing knowledge-violating outputs by 70.97% compared to vanilla generation.

Conclusion: The paper concludes that the proposed framework provides a comprehensive approach to incorporating knowledge into multimodal generative models, and that the four intervention layers can be composed to improve the reliability of generated outputs.
...
<details>
  <summary>Details</summary>
Motivation: To understand behavioral dynamics during infectious disease outbreaks and inform effective public health interventions by simulating realistic human behavior.

Method: A spatially grounded, agent-based simulation framework integrating LLM-generated decisions into a census-based synthetic population, capturing spatial distributions of demographic groups and geographically diverse behavioral modeling.
...
Conclusion: The framework generates synthetic data capturing social and geographic heterogeneity, supporting spatial epidemiological modeling and bias-aware behavioral analysis.
...
TL;DR: This paper proposes using image difference classification to improve data efficiency in digital twin-based road infrastructure inspection.


<details>
  <summary>Details</summary>
Motivation: The limited availability of annotated data hinders the digitalization of road infrastructure inspection using digital twins.

Method: The authors reformulate image-based defect detection as image difference classification (IDC) and evaluate it using a newly curated dataset for traffic sign inspection.

Result: The instruction-based classifier outperforms encoder-based ones, and using reference images improves results, showing IDC's effectiveness in tackling data constraints.

Conclusion: IDC can be an effective approach for modeling tasks in infrastructure inspection and updating digital twin asset conditions with limited data.
...
TL;DR: The paper introduces ALMANAC, a dataset for training LLM agents to collaborate with humans by predicting their mental models and behavior.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enable LLM agents to effectively collaborate with humans by developing capabilities to maintain and align mental models of their own reasoning, partners' intentions, and shared goals.
...
Conclusion: The paper concludes that ALMANAC can be used to improve LLM agents' ability to collaborate with humans by developing more sophisticated mental models and behaviors.
...
TL;DR: The paper analyzes risks associated with autonomous driving, identifying technical failure modes and ethical frameworks, and recommends a cooperative governance approach.


<details>
  <summary>Details</summary>
Motivation: Autonomous driving technology has the potential to reduce road traffic accidents caused by human error, but it also brings new types of risks that need to be evaluated.

Method: The study uses public crash data from NHTSA, disengagement reports from DMV, the MIT Moral Machines dataset, and a comparative regulatory analysis of five jurisdictions.

Result: The main types of technical failure modes are perception and classification errors, and there are different ethical frameworks for autonomous vehicle decision-making and inconsistent regulations.

Conclusion: The problems of technology, ethics, and regulation are closely related and need to be solved together, and a more adaptive and cooperative governance approach is recommended.
...
Method: DataCOPE, an unsupervised verifier-guided skill discovery framework, which uses verifier signals to characterize trajectory quality and iteratively coordinates a Data-Analytic Agent, Unsupervised Verifier, and Skill Manager.
...
Conclusion: DataCOPE provides a effective way to discover reusable data-analysis skills from unlabeled exploration, improving data-analytic agents without updating model parameters.
...
TL;DR: The paper presents a systems characterization of agent memory, introducing a taxonomy, profiling harness, and recommendations for optimizing agent memory systems.


<details>
  <summary>Details</summary>
Motivation: The increasing deployment of LLM agents on long-horizon tasks requires sustained reasoning over extended interaction histories, making persistent memory storage, retrieval, and updates crucial.

Method: The authors introduce a system-oriented taxonomy, a phase-aware profiling harness, and characterize ten representative agent memory systems across two benchmark suites.

Result: The study uncovers how design choices in agent memory systems shift cost across the write and read paths, and derives 10 system recommendations for optimization.

Conclusion: The paper provides a comprehensive characterization of agent memory systems, offering insights and recommendations for improving their design and efficiency.
...
TL;DR: Vortex is a system for efficient design, deployment, and evaluation of sparse attention algorithms for large language models, achieving significant throughput improvements.


<details>
  <summary>Details</summary>
Motivation: The growing size of language models and generation lengths makes sparse attention increasingly important, but testing and evaluating new algorithms is engineering-intensive and time-consuming.

Method: Vortex combines a Python frontend with a page-centric tensor abstraction and an efficient backend integrated into modern LLM serving stacks.

Result: Vortex enables up to $3.46	imes$ higher throughput than full attention while preserving accuracy, and up to $4.7	imes$ and $1.37	imes$ higher throughput on specific large models.

Conclusion: Vortex substantially accelerates the design and iteration of sparse attention algorithms, facilitating their deployment in large language models.
...
TL;DR: Introduces Benchmark Agent, an autonomous system for building benchmarks to evaluate Large Language Models (LLMs) and Multimodal Large Language Models (MLLMs), demonstrating its ability to generate high-quality benchmarks with minimal human involvement.


<details>
  <summary>Details</summary>
Motivation: The construction of benchmarks for LLMs and MLLMs is labor-intensive, hard to reuse, and often leads to performance saturation, making it difficult to discriminate among state-of-the-art models.

Method: Benchmark Agent is a fully autonomous agentic system that orchestrates the complete benchmark construction pipeline, including user query analysis, subtask design, data annotation, and quality control.

Result: Benchmark Agent generated 15 representative benchmarks spanning diverse evaluation scenarios, demonstrating its ability to produce high-quality benchmark samples with minimal human involvement, and revealing that current models struggle with certain domain-specific reasoning tasks.

Conclusion: Benchmark Agent has the potential to contribute significantly to the research community by enabling the rapid evolution of benchmarks, which can help advance LLMs and MLLMs.
...
TL;DR: Goedel-Architect is an AI framework for formal theorem proving in Lean 4, achieving state-of-the-art performance with a novel blueprint generation and refinement approach.


<details>
  <summary>Details</summary>
Motivation: The motivation behind Goedel-Architect is to improve the efficiency and effectiveness of formal theorem proving, which is a crucial aspect of mathematics and computer science.

Method: Goedel-Architect uses a blueprint generation and refinement approach, where a dependency graph of definitions and lemmas is created and then refined through parallel lemma proving and optional natural language proof guidance.

Result: Goedel-Architect achieves 99.2% pass@1 on MiniF2F-test, 75.6% pass@1 on PutnamBench, and solves several problems on IMO 2025, Putnam 2025, and USAMO 2026, representing state-of-the-art performance for an open-source pipeline.

Conclusion: The proposed framework, Goedel-Architect, demonstrates a significant improvement in formal theorem proving capabilities, achieving state-of-the-art performance at a lower cost compared to other open-source pipelines.
...
TL;DR: The paper introduces MLEvolve, a framework for machine learning algorithm discovery using large language models, which improves upon existing methods by enabling cross-branch information flow, hierarchical control, and self-evolution.


<details>
  <summary>Details</summary>
Motivation: Existing machine learning engineering agents suffer from inter-branch information isolation, memoryless search, and lack of hierarchical control, hindering long-horizon optimization.

Method: The authors propose MLEvolve, an LLM-based self-evolving multi-agent framework that extends tree search to Progressive MCGS, introduces Retrospective Memory, and decouples strategic planning from code generation with adaptive coding modes.

Result: MLEvolve achieves state-of-the-art performance on MLE-Bench and outperforms specialized algorithm discovery methods, including AlphaEvolve, on mathematical algorithm optimization tasks.

Conclusion: The proposed MLEvolve framework demonstrates strong cross-domain generalization and improves the efficiency of machine learning algorithm discovery.
